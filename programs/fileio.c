...
begin_comment
comment|/*! FIO_loadFile() : *   creates a buffer, pointed by `*bufferPtr`, *   loads `filename` content into it, *   up to MAX_DICT_SIZE bytes. *   @return : loaded size */
end_comment
...
literal|32
argument_list|,
literal|"Dictionary file %s is too large"
argument_list|,
name|fileName
argument_list|)
expr_stmt|;
...
literal|2
argument_list|,
literal|"Dictionary %s is too large : using last %u bytes only \n"
argument_list|,
name|fileName
argument_list|,
operator|(
...
condition|)
block|{
comment|/* note : is there any possibility of endless loop ? for example, if outBuff is not large enough ? */
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
