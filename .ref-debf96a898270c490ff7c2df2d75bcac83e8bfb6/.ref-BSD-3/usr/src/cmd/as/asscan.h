...
begin_comment
comment|/*  *	The character scanner is called to fill up one token buffer  *  *	In the first pass, the tokens in this buffer may be overwriten  *	to eliminate .stabs, and to change fully assemblable instructions  *	into ascii strings.  However, once the tokens are filled up by the  *	character scanner, they are used in both the first and the second  *	pass.  Holes created by .stab removal and preassembly are replaced  *	with 'skip' tokens that direct the second pass to ignore the  *	following tokens.  *  *	While the first pass could write a second version of the intermediate  *	file and really purge the .stabs and such, the buffering required  *	to do this seems to be too complex and too slow.  */
end_comment
...
directive|define
name|AVAILTOKS
value|TOKBUFLG -\ 		sizeof(short) -\ 		sizeof (struct tokbufdesc *) -\ 		MAXVAX - SAFETY
end_define
...
begin_comment
comment|/*  *	All variables handling these resources are local to astmpfil.c;  *	we must have the structure defnitions here so that  *	asscan.c can touch the stuff in a token buffer  */
end_comment
