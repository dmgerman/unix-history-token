...
index|[]
init|=
literal|"@(#)interp.c	5.6 (Berkeley) 1/9/89"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * stuff for pdx to watch what the interpreter is doing.  * The .globl is #ifndef DBX since it breaks DBX to have a global  * asm label in the middle of a function (see _loopaddr: below).  */
end_comment
...
name|stack
decl_stmt|;
comment|/* Interpreted stack */
name|mypid
operator|=
name|getpid
argument_list|()
expr_stmt|;
comment|/* 	 * Setup sets up any hardware specific parameters before 	 * starting the interpreter. Typically this is macro- or inline- 	 * replaced by "machdep.h" or interp.sed. 	 */
name|setup
argument_list|()
expr_stmt|;
...
comment|/* 		 * Save away the program counter to a fixed location for pdx. 		 */
name|pdx_pc
operator|=
name|pc
expr_stmt|;
comment|/* 		 * Having the label below makes dbx not work 		 * to debug this interpreter, 		 * since it thinks a new function called loopaddr() 		 * has started here, and it won't display the local 		 * variables of interpreter().  You have to compile 		 * -DDBX to avoid this problem... 		 */
ifndef|#
directive|ifndef
name|DBX
empty_stmt|;
