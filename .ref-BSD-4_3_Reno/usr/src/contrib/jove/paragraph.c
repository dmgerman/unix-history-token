...
control|)
block|{
comment|/* The while loop succeeds at least once, when curchar == 		   indent.  So we know that okay_char>= indent when we 		   exit the loop. */
while|while
condition|(
name|calc_pos
argument_list|(
...
name|outahere
goto|;
comment|/* Can't fit in small margin, so if' we're at the end of 		   the line then we just move to the next line.  Otherwise 		   we divide the line where we are and start over. */
if|if
condition|(
name|eolp
argument_list|()
...
name|curline
condition|)
comment|/* didn't actuall go anywhere */
goto|goto
name|outahere
goto|;
block|}
