...
begin_comment
comment|/*  * Copyright (c) 1982, 1986 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)tm.c	7.13 (Berkeley) 7/1/90  */
end_comment
...
goto|;
block|}
comment|/* 	 * For raw I/O, fudge the current block number 	 * so we don't seek except on a retry. 	 * For raw I/O, fudge the current block number 	 * so we don't seek except on a retry. 	 */
if|if
condition|(
name|bp
operator|->
