...
begin_comment
comment|/*  * Copyright (c) 1982, 1986 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)vmmac.h	7.6 (Berkeley) 7/2/90  */
end_comment
...
name|v
parameter_list|)
value|((v)>= stoc(ctos((p)->p_tsize))&& \ 	(v)< (p)->p_tsize + (p)->p_dsize + (p)->p_mmsize)
end_define
...
name|v
parameter_list|)
value|(((v) - LOWPAGES)>= stoc(ctos((p)->p_tsize))&& \ 	(v)< LOWPAGES + (p)->p_tsize + (p)->p_dsize + (p)->p_mmsize)
end_define
...
parameter_list|)
define|\
value|(((v)< LOWPAGES + (p)->p_tsize + (p)->p_dsize + (p)->p_mmsize) ? \ 	((p)->p_p0br + (v)) : ((p)->p_addr - (BTOPUSRSTACK - (v))))
end_define
...
name|p
parameter_list|)
value|(&((p)->p_p0br[(p)->p_szpt * NPTEPG - HIGHPAGES]))
end_define
...
parameter_list|)
define|\
value|(clrnd(ctopt((p)->p_tsize + (p)->p_dsize + (p)->p_mmsize + \ 	(p)->p_ssize + HIGHPAGES)))
end_define
...
parameter_list|)
define|\
value|(clrnd(ctopt((p)->p_tsize + (p)->p_dsize + (p)->p_mmsize)) + \ 	clrnd(ctopt((p)->p_ssize + HIGHPAGES)))
end_define
