...
begin_comment
comment|/*  * $Source: /usr/src/kerberosIV/kdb_util/RCS/kdb_util.c,v $  * $Author: kfall $  *  * Copyright 1987, 1988 by the Massachusetts Institute of Technology.  *   * For copying and distribution information, please see the file  *<mit-copyright.h>.  *  * Kerberos database manipulation utility. This program allows you to  * dump a kerberos database to an ascii readable file and load this  * file into the database. Read locking of the database is done during a  * dump operation. NO LOCKING is done during a load operation. Loads  * should happen with other processes shutdown.   *  * Written July 9, 1987 by Jeffrey I. Schiller  */
end_comment
...
specifier|static
name|char
name|rcsid_kdb_util_c
index|[]
init|=
literal|"$Id: kdb_util.c,v 4.5 90/06/25 22:12:42 kfall Exp $"
decl_stmt|;
end_decl_stmt
...
name|prog
expr_stmt|;
comment|/* required by libkdb (yuck!) */
if|if
condition|(
name|argc
operator|!=
...
directive|ifndef
name|NOENCRYPTION
comment|/* get clear key, old style */
operator|(
name|void
operator|)
