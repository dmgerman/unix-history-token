...
name|obuf
expr_stmt|;
comment|/* 		 * There is a bug in Win32 WriteConsole() if we're 		 * writing in the last cell with a different color. 		 * To avoid color problems in the bottom line, 		 * we scroll the screen manually, before writing. 		 */
...
name|norm_attr
decl_stmt|;
comment|/* 			 * Only dark colors mentioned here, so that 			 * bold has visible effect. 			 */
specifier|static
name|enum
name|COLORS
name|screen_color
...
block|}
decl_stmt|;
comment|/* Normal text colors are used as baseline. */
name|bg
operator|=
name|nm_bg_color
operator|&
...
name|p_next
expr_stmt|;
comment|/* 				 * Handle the null escape sequence 				 * (ESC-[m), which is used to restore 				 * the original color. 				 */
if|if
condition|(
name|p
index|[
...
block|{
comment|/* 					 * If some chars seen since 					 * the last escape sequence, 					 * write it out to the screen 					 * using current text attributes. 					 */
if|if
condition|(
name|p
operator|>
