...
begin_comment
comment|/* Nonzero means we are inserting a block level HTML element that must not be    enclosed in a<p>, such as<ul>,<ol> and<h?>.  */
end_comment

begin_decl_stmt
name|int
name|in_html_block_level_elt
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* True when we are inside a<li> block of a menu.  */
end_comment
...
name|_
argument_list|(
literal|"\ Options for XML and Docbook:\n\       --output-indent=VAL       indent XML elements by VAL spaces (default %d).\n\                                   If VAL is 0, ignorable whitespace is dropped.\n\ "
argument_list|)
argument_list|,
...
begin_comment
comment|/* We use handle_variable_internal for -D and -U, and it depends on    execute_string, which depends on input_filename, which is not defined    while we are handling options. :-\  So we save these defines in this    struct, and handle them later.  */
end_comment

begin_typedef
typedef|typedef
struct|struct
name|command_line_define
block|{
name|struct
name|command_line_define
modifier|*
...
directive|endif
comment|/* If TEXINFO_OUTPUT_FORMAT envvar is set, use it to set default output.      Can be overridden with one of the output options.  */
if|if
condition|(
name|getenv
argument_list|(
literal|"TEXINFO_OUTPUT_FORMAT"
argument_list|)
operator|!=
name|NULL
condition|)
...
name|_
argument_list|(
literal|"%s: Ignoring unrecognized TEXINFO_OUTPUT_FORMAT value `%s'.\n"
argument_list|)
argument_list|,
name|progname
argument_list|,
...
name|argv
argument_list|,
literal|"D:de:E:f:hI:i:o:p:P:r:s:t:U:vV:wx"
argument_list|,
name|long_options
argument_list|,
operator|&
...
name|_
argument_list|(
literal|"%s: could not open macro expansion output `%s'"
argument_list|)
argument_list|,
name|progname
argument_list|,
...
name|_
argument_list|(
literal|"%s: ignoring second macro expansion output `%s'.\n"
argument_list|)
argument_list|,
name|progname
argument_list|,
...
condition|)
block|{
comment|/* not found, move current position to end of string */
name|input_text_offset
operator|=
name|input_text_length
expr_stmt|;
...
condition|)
block|{
comment|/* Give a more descriptive feedback, if we are looking for ``@end ''              during macro execution.  That means someone used a multiline              command as an argument to, say, @section ... style commands.  */
name|char
modifier|*
name|end_block
init|=
...
name|_
argument_list|(
literal|"Multiline command %c%s used improperly"
argument_list|)
argument_list|,
name|COMMAND_PREFIX
argument_list|,
...
comment|/* found, move current position to after the found string */
name|input_text_offset
operator|=
name|temp
operator|+
...
argument_list|)
decl_stmt|;
comment|/* Prepend file directory to the search path, so relative links work.  */
name|prepend_to_include_path
argument_list|(
name|pathname_part
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Pop the prepended path, so multiple filenames in the      command line do not screw each others include paths.  */
...
condition|)
block|{
comment|/* And it's a directory, so silently reuse it.  */
if|if
condition|(
name|S_ISDIR
argument_list|(
...
literal|1
return|;
comment|/* Not a directory, so move it out of the way if we are allowed.  */
elseif|else
if|if
condition|(
name|can_remove_file
...
argument_list|)
expr_stmt|;
comment|/* First try, do not remove existing file.  */
if|if
condition|(
operator|!
name|create_html_directory
...
condition|)
block|{
comment|/* That failed, try subdir name with .html.          Remove it if it exists.  */
name|strcpy
argument_list|(
name|output_filename
argument_list|,
...
condition|)
block|{
comment|/* Just before the real main loop, let's handle the defines.  */
name|COMMAND_LINE_DEFINE
modifier|*
name|temp
decl_stmt|;
...
block|}
block|}
comment|/* Write stuff makeinfo generates after @bye, ie. info_trailer.  */
name|flush_output
argument_list|()
expr_stmt|;
if|if
...
begin_comment
comment|/* If enable_encoding is set and @documentencoding is used, return a    Local Variables section (as a malloc-ed string) so that Emacs'    locale features can work.  Else return NULL.  */
end_comment
...
comment|/* Insert a colon only if there is a description of this menu item.  */
block|{
name|int
...
literal|0
decl_stmt|;
comment|/* Get the number of consequtive dashes.  */
while|while
condition|(
name|input_text
index|[
...
begin_comment
comment|/* These two functions below, add_html_block_elt and add_html_block_elt_args,    are mixtures of add_html_elt and add_word_args.  They inform makeinfo that    the current HTML element being inserted should not be enclosed in a<p>    element.  */
end_comment
...
directive|else
function|add_html_block_elt_args
parameter_list|(
name|format
parameter_list|,
name|va_alist
...
literal|0
expr_stmt|;
comment|/* This check for in_html_block_level_elt prevents<p> from being                inserted when we already have html markup starting a paragraph,                as with<ul> and<h1> and the like.  */
if|if
condition|(
name|html
operator|&&
...
parameter_list|)
block|{
comment|/* We don't want to strip trailing whitespace in multitables.  Otherwise      horizontal separators confuse the font locking in Info mode in Emacs,      because it looks like a @subsection.  Adding a trailing space to those      lines fixes it.  */
if|if
condition|(
name|character
operator|==
...
name|i
decl_stmt|;
comment|/* We don't need these newlines in XML and Docbook outputs for      paragraph seperation.  We have<para> element for that.  */
if|if
condition|(
name|xml
condition|)
...
begin_comment
comment|/* search_forward until n characters.  */
end_comment
...
begin_comment
comment|/* Return next non-whitespace and non-cr character.  */
end_comment
...
parameter_list|)
block|{
comment|/* First check the current input_text.  Start from the next char because      we already have input_text[input_text_offset] in ``current''.  */
name|int
name|pos
init|=
name|input_text_offset
...
block|}
block|{
comment|/* Can't find a valid character, so go through filestack        in case we are doing @include or expanding a macro.  */
name|FSTACK
modifier|*
name|tos
init|=
...
condition|)
block|{
comment|/* Backwards compatibility (4.6<= version< 4.7): 		 try prefixing @image's EXTENSION parameter with a period. */
name|sprintf
argument_list|(
name|fullname
argument_list|,
...
name|START
argument_list|,
literal|"width=\"%s\" height=\"%s\" name=\"%s\" extension=\"%s\""
argument_list|,
name|w_arg
argument_list|,
name|h_arg
...
decl_stmt|;
name|int
name|image_in_brackets
init|=
name|paragraph_is_open
decl_stmt|;
comment|/* Write magic ^@^H[image ...^@^H] cookie in the info file, if                  there's an accompanying bitmap.  Otherwise just include the                  text image.  In the plaintext output, always include the text                  image without the magic cookie.  */
name|int
name|use_magic_cookie
init|=
operator|!
name|no_headers
operator|&&
...
literal|1
decl_stmt|;
comment|/* xml_add_char() skips any content inside menus when output format is      Docbook, so @value{} is no use there.  Also start_pos and end_pos does not      get updated, causing name to be empty string.  So just return.  */
if|if
condition|(
name|docbook
operator|&&
...
condition|)
block|{
comment|/* We need to get past the closing brace since the value may 	     expand to a context-sensitive macro (e.g. @xref) and produce 	     spurious warnings */
name|input_text_offset
operator|++
expr_stmt|;
name|execute_string
...
argument_list|()
expr_stmt|;
comment|/* If insertion stack level changes during execution, that means a multiline      command is used inside braces or @section ... kind of commands.  */
if|if
condition|(
name|insertion_level_at_start
operator|!=
...
argument_list|)
expr_stmt|;
comment|/* We also need to keep insertion_level intact to make sure warnings are          issued for @end ... command.  */
while|while
condition|(
name|insertion_level
operator|>
...
begin_comment
comment|/* Return what would be output for STR (in newly-malloced memory), i.e.,    expand Texinfo commands according to the current output format.  If    IMPLICIT_CODE is set, expand @code{STR}.  This is generally used for    short texts; filling, indentation, and html escapes are disabled.  */
end_comment
...
begin_comment
comment|/* Do HTML escapes according to DO_HTML_ESCAPE.  Needed in    cm_printindex, q.v.  */
end_comment
...
begin_comment
comment|/* Expand STR (or @code{STR} if IMPLICIT_CODE is nonzero).  No change to    any formatting parameters -- filling, indentation, html escapes,    etc., are not reset.  Always returned in new memory.  */
end_comment
