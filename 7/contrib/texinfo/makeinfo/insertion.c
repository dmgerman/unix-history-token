...
begin_comment
comment|/* Since an insertion is already in the stack before we reach the switch    statement, we cannot use is_in_insertion_of_type (always returns true.) Also    making it return the level found, and comparing it with the current level is    no use, due to the order of stack.  */
end_comment
...
begin_comment
comment|/* Unsetting escape_html blindly causes text inside @html/etc. to be escaped if    used within a rmacro.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|raw_output_block
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Non-zero if a<dl> element has a<dt> element in it.  We use this when    deciding whether to insert a<br> or not.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|html_deflist_has_term
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Simple function to query insertion_stack to see if we are inside a given    insertion type. */
end_comment
...
comment|/* Like @example but no fixed width font. */
case|case
name|display
case|:
case|case
...
name|html
condition|)
comment|/* Kludge alert: if<pre> is followed by a newline, IE3,            mozilla, maybe others render an extra blank line before the            pre-formatted block.  So don't output a newline.  */
name|add_html_block_elt_args
argument_list|(
literal|"<pre class=\"%s\">"
argument_list|,
...
block|{
comment|/* We are just starting, so this<dl>                  has no<dt> children yet.  */
name|html_deflist_has_term
operator|=
literal|0
expr_stmt|;
...
expr_stmt|;
break|break;
comment|/* @group produces no output in info. */
case|case
name|group
case|:
...
name|add_html_block_elt
argument_list|(
literal|"<p><table class=\"cartouche\" summary=\"cartouche\" border=\"1\"><tr><td>\n"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|floatenv
case|:
comment|/* Cannot nest floats, so complain.  */
if|if
condition|(
name|float_active
condition|)
...
name|_
argument_list|(
literal|"%cfloat environments cannot be nested"
argument_list|)
argument_list|,
name|COMMAND_PREFIX
argument_list|)
...
expr_stmt|;
block|{
comment|/* Collect data about this float.  */
comment|/* Example: @float [FLOATTYPE][,XREFLABEL][,POSITION] */
name|char
name|floattype
index|[
literal|200
...
argument_list|)
expr_stmt|;
comment|/* Move to the start of the @float so the contents get processed as            usual.  */
name|input_text_offset
operator|=
name|save_input_text_offset
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* Anchor now.  Note that XML documents get their          anchors with<float name="anchor"> tag.  */
if|if
condition|(
operator|(
operator|!
...
block|}
block|{
comment|/* Some deuglification for improved readability.  */
specifier|extern
name|int
name|xml_in_para
decl_stmt|;
...
name|add_html_block_elt
argument_list|(
literal|"<p><strong class=\"float-caption\">"
argument_list|)
expr_stmt|;
else|else
name|close_paragraph
...
literal|1
expr_stmt|;
comment|/* Legend:                1) @float Foo,lbl& no caption:    Foo 1.1                2) @float Foo& no caption:        Foo                3) @float ,lbl& no caption:       1.1                4) @float& no caption:                    */
if|if
condition|(
operator|!
name|xml
...
expr_stmt|;
block|}
comment|/* Indent the following paragraph. */
name|inhibit_paragraph_indentation
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* The complex code in close_paragraph that kills whitespace              does not function here, since we've inserted non-whitespace              (the</whatever>) before it.  The indentation already got              inserted at the end of the last example line, so we have to              delete it, or browsers wind up showing an extra blank line.  */
...
expr_stmt|;
block|}
comment|/* The ending of one of these insertions always marks the          start of a new paragraph, except for the XML output. */
if|if
condition|(
operator|!
name|xml
...
argument_list|()
expr_stmt|;
comment|/*</pre> closes paragraph without messing with</p>.  */
if|if
condition|(
name|html
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* close the div and blockquote which has been opened in defun.c */
if|if
condition|(
operator|!
...
parameter_list|)
block|{
comment|/* We start the blockquote element in the insertion.  */
name|begin_insertion
argument_list|(
name|quotation
argument_list|)
...
condition|)
block|{
comment|/* Rollback previous newlines.  These occur between</para> and<example>.  */
if|if
condition|(
name|output_paragraph
index|[
...
argument_list|)
expr_stmt|;
comment|/* Make sure example text is starting on a new line          for improved readability.  */
if|if
condition|(
name|docbook
condition|)
...
condition|)
block|{
comment|/* See cm_example comments about newlines.  */
if|if
condition|(
name|output_paragraph
index|[
...
name|warning
argument_list|(
literal|"@copying not used before %s"
argument_list|,
name|command
argument_list|)
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|/* no blank lines if not at outer level */
name|inhibit_paragraph_indentation
operator|=
literal|1
expr_stmt|;
...
condition|)
block|{
comment|/* If inside @example, we'll be preceded by the indentation          already.  Browsers will ignore those spaces because we're about          to start another<pre> (don't ask me).  So, wipe them out for          cleanliness, and re-insert.  */
name|int
name|i
decl_stmt|;
name|kill_self_indent
...
condition|)
block|{
comment|/* See comments in example case above.  */
name|kill_self_indent
argument_list|(
name|default_indentation_increment
argument_list|)
...
begin_comment
comment|/* Generic xrefable block with a caption.  */
end_comment
...
name|temp
decl_stmt|;
comment|/* This is a no_op command for most formats, as we handle it during @float      insertion.  For XML though, we handle it here to keep document structure      as close as possible, to the Texinfo source.  */
comment|/* Everything is already handled at START.  */
if|if
condition|(
name|arg
operator|==
...
condition|)
return|return;
comment|/* Check if it's mislocated.  */
if|if
condition|(
name|current_insertion_type
argument_list|()
...
name|_
argument_list|(
literal|"@%s not meaningful outside `@float' environment"
argument_list|)
argument_list|,
name|command
argument_list|)
...
condition|)
block|{
comment|/* FIXME Ideally, we should use an attribution element,              but they are supposed to be at the start of quotation              blocks.  So to avoid looking ahead mess, let's just              use mdash like HTML for now.  */
...
name|_
argument_list|(
literal|"@%s not meaningful outside `@titlepage' and `@quotation' environments"
argument_list|)
argument_list|,
name|command
argument_list|)
...
name|_
argument_list|(
literal|"@%s not meaningful outside `@titlepage' environment"
argument_list|)
argument_list|,
name|command
argument_list|)
...
name|_
argument_list|(
literal|"Bad argument `%s' to `@%s', using `%s'"
argument_list|)
argument_list|,
name|temp
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* We are about to insert a<dt>, so this<dl> has a term.                  Feel free to insert a<br> next time. :)  */
name|html_deflist_has_term
operator|=
literal|1
expr_stmt|;
