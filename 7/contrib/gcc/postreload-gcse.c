...
specifier|static
name|rtx
name|reg_set_between_after_reload_p
parameter_list|(
name|rtx
parameter_list|,
name|rtx
...
specifier|static
name|rtx
name|reg_used_between_after_reload_p
parameter_list|(
name|rtx
parameter_list|,
name|rtx
...
name|after_insn
condition|)
comment|/* If the last CUID setting the insn is less than the CUID of 	   INSN, then reg X is not changed in or after INSN.  */
return|return
name|reg_avail_info
index|[
name|REGNO
...
comment|/* Reg X is not set before INSN in the current basic block if 	   we have not yet recorded the CUID of an insn that touches 	   the reg.  */
return|return
name|reg_avail_info
index|[
name|REGNO
...
begin_comment
comment|/* Return the insn that sets register REG or clobbers it in between    FROM_INSN and TO_INSN (exclusive of those two).    Just like reg_set_between but for hard registers and not pseudos.  */
end_comment
...
begin_comment
comment|/* Return the insn that uses register REG in between FROM_INSN and TO_INSN    (exclusive of those two). Similar to reg_used_between but for hard    registers and not pseudos.  */
end_comment
...
begin_comment
comment|/* Return true if REG is used, set, or killed between the beginning of    basic block BB and UP_TO_INSN.  Caches the result in reg_avail_info.  */
end_comment

begin_function
specifier|static
name|bool
name|reg_set_or_used_since_bb_start
parameter_list|(
name|rtx
name|reg
parameter_list|,
