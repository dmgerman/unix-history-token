...
begin_comment
comment|/* Compute register class preferences for pseudo-registers.    Copyright (C) 1987, 1988, 1991, 1992, 1993, 1994, 1995, 1996    1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005    Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* We leave it to target hooks to decide if we have secondary reloads, so    assume that we might have them.  */
end_comment
...
begin_comment
comment|/* Array containing all of the register class names.  */
end_comment
...
begin_comment
comment|/* 1 if there is a register of given mode.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Sanity check: make sure the target macros FIXED_REGISTERS and      CALL_USED_REGISTERS had the right number of initializers.  */
name|gcc_assert
argument_list|(
sizeof|sizeof
name|fixed_regs
...
control|)
block|{
comment|/* call_used_regs must include fixed_regs.  */
name|gcc_assert
argument_list|(
operator|!
name|fixed_regs
...
directive|ifdef
name|CALL_REALLY_USED_REGISTERS
comment|/* call_used_regs must include call_really_used_regs.  */
name|gcc_assert
argument_list|(
operator|!
name|call_really_used_regs
...
name|altclass
condition|)
comment|/* This isn't simply a copy-to-temporary situation.  Can't guess        what it is, so MEMORY_MOVE_COST really ought not to be calling        here in that case.         I'm tempted to put in an assert here, but returning this will        probably only give poor estimates, which is what we would've        had before this code anyways.  */
return|return
name|partial_cost
return|;
...
name|reg_pref
block|{
comment|/* (enum reg_class) prefclass is the preferred class.  May be      NO_REGS if no class is better than memory.  */
name|char
name|prefclass
decl_stmt|;
...
begin_comment
comment|/* Wrapper around REGNO_OK_FOR_INDEX_P, to allow pseudo registers.  */
end_comment
...
begin_comment
comment|/* A version of regno_ok_for_base_p for use during regclass, when all pseudos    should count as OK.  Arguments as for regno_ok_for_base_p.  */
end_comment
...
condition|)
block|{
comment|/* ??? There are two assumptions here; that the base class does not 		     depend on the exact outer code (POST_INC vs. PRE_INC etc.), and 		     that it does not depend on the machine mode of the memory 		     reference.  */
name|enum
name|reg_class
name|base_class
init|=
...
expr_stmt|;
block|}
comment|/* If no register class is better than memory, use memory. */
if|if
condition|(
name|p
operator|->
...
argument_list|)
expr_stmt|;
comment|/* We know this operand is an address, so we want it to be 		     allocated to a register that can be the base of an 		     address, i.e. BASE_REG_CLASS.  */
name|classes
index|[
name|i
index|]
...
argument_list|)
expr_stmt|;
comment|/* If we need a secondary reload for an intermediate, the      cost is that to load the input into the intermediate register, then      to copy it.  */
name|sri
operator|.
name|prev_sri
operator|=
...
begin_comment
comment|/* Record the pseudo registers we must reload into hard registers    in a subexpression of a memory address, X.     If CONTEXT is 0, we are looking at the base part of an address, otherwise we    are looking at the index part.     MODE is the mode of the memory reference; OUTER_CODE and INDEX_CODE    give the context that the rtx appears in.  These three arguments are    passed down to base_reg_class.     SCALE is twice the amount to multiply the cost by (it is twice so we    can represent half-cost adjustments).  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If both operands are registers but one is already a hard register 	   of index or reg-base class, give the other the class that the 	   hard register is not.  */
elseif|else
if|if
condition|(
name|code0
