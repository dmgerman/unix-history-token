...
begin_comment
comment|/* Reload pseudo regs into hard regs for insns that require hard regs.    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007    Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* Element N is an invariant value to which pseudo reg N is equivalent.    eliminate_regs_in_insn uses this to replace pseudos in particular    contexts.  */
end_comment
...
begin_comment
comment|/* We allocate reg_equiv_memory_loc inside a varray so that the garbage    collector can keep track of what is inside.  */
end_comment
...
begin_comment
comment|/* Element N is an EXPR_LIST of REG_EQUIVs containing MEMs with    alternate representations of the location of pseudo reg N.  */
end_comment
...
begin_comment
comment|/* Indicate whether the register's current value is one that is not    safe to retain across a call, even for registers that are normally    call-saved.  */
end_comment

begin_decl_stmt
specifier|static
name|HARD_REG_SET
name|reg_reloaded_call_part_clobbered
decl_stmt|;
end_decl_stmt
...
name|can_eliminate
decl_stmt|;
comment|/* Nonzero if this elimination can be done.  */
name|int
...
specifier|static
name|rtx
name|emit_insn_if_valid_for_reload
parameter_list|(
name|rtx
parameter_list|)
function_decl|;
...
argument_list|)
expr_stmt|;
comment|/* There can be multiple ways to eliminate a register;      they should be listed adjacently.      Elimination for any register fails only if all possible ways fail.  */
for|for
control|(
name|ep
operator|=
...
literal|1
expr_stmt|;
comment|/* Even if the frame size remained the same, we might still have 	 changed elimination offsets, e.g. if find_reloads called  	 force_const_mem requiring the back end to allocate a constant 	 pool base register that needs to be saved on the stack.  */
elseif|else
if|if
condition|(
operator|!
...
argument_list|)
decl_stmt|;
comment|/* If we already deleted the insn or if it may trap, we can't 		 delete it.  The latter case shouldn't happen, but can 		 if an insn has a variable address, gets a REG_EH_REGION 		 note added to it, and then gets converted into a load 		 from a constant address.  */
if|if
condition|(
name|NOTE_P
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Come here (with failure set nonzero) if we can't get enough spill      regs.  */
name|failed
label|:
name|CLEAR_REG_SET
argument_list|(
...
continue|continue;
block|}
comment|/* Some CLOBBERs may survive until here and still reference unassigned 	   pseudos with const equivalent, which may in turn cause ICE in later 	   passes if the reference remains in place.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Discard obvious no-ops, even without -O.  This optimization 	   is fast and doesn't interfere with debugging.  */
if|if
condition|(
name|NONJUMP_INSN_P
argument_list|(
...
directive|endif
comment|/* Simplify (subreg (reg)) if it appears as an operand.  */
name|cleanup_subreg_operands
argument_list|(
name|insn
argument_list|)
expr_stmt|;
comment|/* Clean up invalid ASMs so that they don't confuse later passes. 	   See PR 21299.  */
if|if
condition|(
name|asm_noperands
argument_list|(
...
name|insn
argument_list|,
literal|"%<asm%> operand has impossible constraints"
argument_list|)
expr_stmt|;
name|delete_insn
argument_list|(
...
directive|ifdef
name|STACK_BOUNDARY
comment|/* init_emit has set the alignment of the hard frame pointer      to STACK_BOUNDARY.  It is very likely no longer valid if      the hard frame pointer was used for register allocation.  */
if|if
condition|(
operator|!
name|frame_pointer_needed
...
name|dump_file
argument_list|,
literal|"reload failure for reload %d\n"
argument_list|,
name|r
argument_list|)
expr_stmt|;
...
name|insn
argument_list|,
literal|"can't find a register in class %qs while "
literal|"reloading %<asm%>"
argument_list|,
...
name|error
argument_list|(
literal|"unable to find a register to spill in class %qs"
argument_list|,
name|reg_class_names
index|[
name|class
...
argument_list|)
return|;
comment|/* There exists at least one use of REGNO that cannot be 	     eliminated.  Prevent the defining insn from being deleted.  */
name|reg_equiv_init
index|[
name|regno
index|]
...
name|SUBREG
case|:
comment|/* Similar to above processing, but preserve SUBREG_BYTE. 	 Convert (subreg (mem)) to (mem) if not paradoxical. 	 Also, if we have a non-paradoxical (subreg (pseudo)) and the 	 pseudo didn't get a hard reg, we must replace this with the 	 eliminated version of the memory location because push_reload 	 may do the replacement in certain circumstances.  */
if|if
condition|(
name|REG_P
argument_list|(
...
comment|/* On these machines, combine can create rtl of the form 		      (set (subreg:m1 (reg:m2 R) 0) ...) 		      where m1< m2, and expects something interesting to 		      happen to the entire word.  Moreover, it will use the 		      (reg:m2 R) later, expecting all bits to be preserved. 		      So if the number of words is the same, preserve the 		      subreg so that push_reload can see it.  */
operator|&&
operator|!
operator|(
operator|(
...
block|}
block|}
comment|/* We allow one special case which happens to work on all machines we      currently support: a single set with the source or a REG_EQUAL      note being a PLUS of an eliminable register and a constant.  */
name|plus_src
operator|=
name|plus_cst_src
operator|=
...
argument_list|)
expr_stmt|;
comment|/* First see if the source is of the form (plus (...) CST).  */
if|if
condition|(
name|plus_src
operator|&&
...
condition|)
block|{
comment|/* Otherwise, see if we have a REG_EQUAL note of the form 	     (plus (...) CST).  */
name|rtx
name|links
decl_stmt|;
for|for
...
block|}
block|}
comment|/* Check that the first operand of the PLUS is a hard reg or 	 the lowpart subreg of one.  */
if|if
condition|(
name|plus_cst_src
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If we have a nonzero offset, and the source is already 	       a simple REG, the following transformation would 	       increase the cost of the insn by replacing a simple REG 	       with (plus (reg sp) CST).  So try only when we already 	       had a PLUS before.  */
if|if
condition|(
name|offset
operator|==
...
expr_stmt|;
block|}
comment|/* Companion to the above plus substitution, we can allow 	     invariants as the source of a plain move.  */
name|is_set_src
operator|=
name|false
expr_stmt|;
...
directive|endif
comment|/* Count the number of eliminable registers and build the FROM and TO      REG rtx's.  Note that code in gen_rtx_REG will cause, e.g.,      gen_rtx_REG (Pmode, STACK_POINTER_REGNUM) to equal stack_pointer_rtx.      We depend on this.  */
for|for
control|(
name|ep
operator|=
...
begin_comment
comment|/* A subroutine of reload_as_needed.  If INSN has a REG_EH_REGION note,    examine all of the reload insns between PREV and NEXT exclusive, and    annotate all that may trap.  */
end_comment

begin_function
specifier|static
name|void
name|fixup_eh_region_note
parameter_list|(
name|rtx
name|insn
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* Adjust the exception region notes for loads and stores.  */
if|if
condition|(
...
name|insn
argument_list|,
literal|"%<asm%> operand requires "
literal|"impossible reload"
argument_list|)
expr_stmt|;
name|delete_insn
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Don't assume a reload reg is still good after a call insn 	 if it is a call-used reg, or if it contains a value that will          be partially clobbered by the call.  */
elseif|else
if|if
condition|(
name|CALL_P
...
name|data
decl_stmt|;
comment|/* note_stores does give us subregs of hard regs,      subreg_regno_offset requires a hard reg.  */
while|while
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Forget the reloads marked in regset by previous function.  */
end_comment
...
begin_comment
comment|/* ??? Actually, the places where we use this expect exactly what is    tested here, and not everything that is function invariant.  In    particular, the frame pointer and arg pointer are special cased;    pic_offset_table_rtx is not, and we must not spill these things to    memory.  */
end_comment
...
block|else if (GET_CODE (rld[r].in) == SUBREG&& REG_P (SUBREG_REG (rld[r].in))) 		regno = subreg_regno (rld[r].in);
endif|#
directive|endif
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* If we choose EQUIV as the reload register, but the 			 loop below decides to cancel the inheritance, we'll 			 end up reloading EQUIV in rld[r].mode, not the mode 			 it had originally.  That isn't safe when EQUIV isn't 			 available as a spill register since its value might 			 still be live at this point.  */
for|for
control|(
name|i
operator|=
...
index|]
expr_stmt|;
comment|/* If this reload won't be done because it has been canceled or is      optional and not inherited, clear reload_reg_rtx so other      routines (such as subst_reloads) don't get confused.  */
for|for
control|(
name|j
operator|=
...
operator|)
expr_stmt|;
comment|/* Check to see if we accidentally converted two 		     reloads that use the same reload register with 		     different inputs to the same type.  If so, the 		     resulting code won't work.  */
if|if
condition|(
name|rld
index|[
...
begin_comment
comment|/* Check if *RELOAD_REG is suitable as an intermediate or scratch register    of class NEW_CLASS with mode NEW_MODE.  Or alternatively, if alt_reload_reg    is nonzero, if that is suitable.  On success, change *RELOAD_REG to the    adjusted register, and return true.  Otherwise, return false.  */
end_comment

begin_function
specifier|static
name|bool
name|reload_adjust_reg_for_temp
parameter_list|(
name|rtx
modifier|*
name|reload_reg
...
name|reg
operator|=
name|reload_adjust_reg_for_mode
argument_list|(
name|reg
argument_list|,
name|new_mode
...
begin_comment
comment|/* Check if *RELOAD_REG is suitable as a scratch register for the reload    pattern with insn_code ICODE, or alternatively, if alt_reload_reg is    nonzero, if that is suitable.  On success, change *RELOAD_REG to the    adjusted register, and return true.  Otherwise, return false.  */
end_comment

begin_function
specifier|static
name|bool
name|reload_adjust_reg_for_icode
parameter_list|(
name|rtx
modifier|*
name|reload_reg
...
name|reloadreg
expr_stmt|;
comment|/* Verify that resulting insn is valid.  */
name|extract_insn
argument_list|(
name|temp
argument_list|)
...
expr_stmt|;
comment|/* We'd have to add more code for quartary reloads.  */
name|gcc_assert
argument_list|(
name|rld
index|[
...
name|CODE_FOR_nothing
condition|)
comment|/* We currently lack a way to express this in reloads.  */
name|gcc_unreachable
argument_list|()
expr_stmt|;
else|else
...
comment|/* This could be handled more intelligently too.  */
name|oldequiv
operator|=
name|old
operator|,
...
condition|)
block|{
comment|/* We'd have to add extra code to handle this case.  */
name|gcc_assert
argument_list|(
operator|!
name|third_reload_reg
...
begin_comment
comment|/* Reload number R reloads from or to a group of hard registers starting at    register REGNO.  Return true if it can be treated for inheritance purposes    like a group of reloads, each one reloading a single hard register.    The caller has already checked that the spill register and REGNO use    the same number of registers to store the reload value.  */
end_comment
...
comment|/* If a register gets output-reloaded from a non-spill register, 	 that invalidates any previous reloaded copy of it. 	 But forget_old_reloads_1 won't get to see it, because 	 it thinks only about the original insn.  So invalidate it here. 	 Also do the same thing for RELOAD_OTHER constraints where the 	 output is discarded.  */
if|if
condition|(
name|i
operator|<
...
name|src_reg
expr_stmt|;
comment|/* We have to set reg_has_output_reload here, or else  		     forget_old_reloads_1 will clear reg_last_reload_reg 		     right away.  */
name|SET_REGNO_REG_SET
argument_list|(
operator|&
name|reg_has_output_reload
...
begin_comment
comment|/* Go through the motions to emit INSN and test if it is strictly valid.    Return the emitted insn if valid, else return NULL.  */
end_comment
...
name|set
return|;
comment|/* If that failed, move the inner operand to the reload 	 register, and try the same unop with the inner expression 	 replaced with the reload register.  */
if|if
condition|(
name|GET_MODE
argument_list|(
...
name|fatal_insn
argument_list|(
literal|"Failure trying to reload:"
argument_list|,
name|set
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* IN may contain a LABEL_REF, if so add a REG_LABEL note.  */
...
name|substed
decl_stmt|;
comment|/* It is possible that this reload has been only used to set another reload      we eliminated earlier and thus deleted this instruction too.  */
if|if
condition|(
name|INSN_DELETED_P
argument_list|(
...
condition|)
return|return;
comment|/* If the pseudo-reg we are reloading is no longer referenced      anywhere between the store into it and here,      and we're within the same basic block, then the value can only      pass through the reload reg and end up here.      Otherwise, give up--return.  */
for|for
control|(
name|i1
operator|=
...
condition|)
return|return;
comment|/* ??? We can't finish the loop here, because dst might be 		 allocated to a pseudo in this block if no reload in this 		 block needs any of the classes containing DST - see 		 spill_hard_reg.  There is no easy way to tell this, so we 		 have to scan till the end of the basic block.  */
block|}
if|if
condition|(
name|reg_set_p
...
argument_list|)
expr_stmt|;
comment|/* Sometimes there's still the return value USE. 			 If it's placed after a trapping call (i.e. that 			 call is the last insn anyway), we have no fallthru 			 edge.  Simply delete this use and don't try to insert 			 on the non-existent edge.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
block|}
block|}
comment|/* It may be that we don't find any such trapping insn.  In this 	     case we discovered quite late that the insn that had been  	     marked as can_throw_internal in fact couldn't trap at all. 	     So we should in fact delete the EH edges out of the block.  */
else|else
...
block|}
block|}
comment|/* We've possibly turned single trapping insn into multiple ones.  */
if|if
condition|(
name|flag_non_call_exceptions
condition|)
...
comment|/* Verify that we didn't turn one trapping insn into many, and that      we found and corrected all of the problems wrt fixups on the      fallthru edge.  */
