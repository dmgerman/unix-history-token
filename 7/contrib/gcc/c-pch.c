...
begin_comment
comment|/* This is a list of flag variables that must match exactly, and their    names for the error message.  The possible values for *flag_var must    fit in a 'signed char'.  */
end_comment
...
begin_comment
comment|/* The value of the checksum in the dummy compiler that is actually    checksummed.  That compiler should never be run.  */
end_comment
...
begin_comment
comment|/* Information about flags and suchlike that affect PCH validity.     Before this structure is read, both an initial 8-character identification    string, and a 16-byte checksum, have been read and validated.  */
end_comment
...
begin_comment
comment|/* Prepare to write a PCH file, if one is being written.  This is    called at the start of compilation.     Also, print out the executable checksum if -fverbose-asm is in effect.  */
end_comment
...
name|fatal_error
argument_list|(
literal|"%qs is not a valid output file"
argument_list|,
name|asm_file_name
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* asm_out_file can be written afterwards, so fseek to clear      _IOREAD flag.  */
if|if
condition|(
name|fseek
argument_list|(
...
name|v
decl_stmt|;
comment|/* Perform a quick test of whether this is a valid      precompiled header for the current language.  */
name|gcc_assert
argument_list|(
name|memcmp
argument_list|(
...
name|CPP_DL_WARNING
argument_list|,
literal|"%s: too short to be a PCH file"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
name|CPP_DL_WARNING
argument_list|,
literal|"%s: created by a different GCC executable"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* At this point, we know it's a PCH file created by this      executable, so it ought to be long enough that we can read a      c_pch_validity structure.  */
if|if
condition|(
name|read
argument_list|(
...
return|;
block|}
comment|/* Check flags that must match exactly.  */
block|{
name|size_t
name|i
decl_stmt|;
...
name|CPP_DL_WARNING
argument_list|,
literal|"%s: settings for %s do not match"
argument_list|,
name|name
argument_list|,
name|pch_matching
...
block|{
comment|/* If we're preprocessing, don't write to a NULL 	 asm_out_file.  */
if|if
condition|(
name|fseek
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Give the front end a chance to take action after a PCH file has      been loaded.  */
if|if
condition|(
name|lang_post_pch_load
condition|)
...
begin_comment
comment|/* Handle #pragma GCC pch_preprocess, to load in the PCH file.  */
end_comment
...
name|error
argument_list|(
literal|"pch_preprocess pragma should only be used with -fpreprocessed"
argument_list|)
expr_stmt|;
name|inform
argument_list|(
...
name|fatal_error
argument_list|(
literal|"%s: couldn%'t open PCH file: %m"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
name|fatal_error
argument_list|(
literal|"%s: PCH file was invalid"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Print out executable_checksum[].  */
end_comment
...
name|fputs
argument_list|(
literal|"Compiler executable checksum: "
argument_list|,
name|f
argument_list|)
expr_stmt|;
