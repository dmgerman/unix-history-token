...
name|MIN_EXPR
case|:
comment|/* Disallow<? and>? as lvalues if either argument side-effects.  */
if|if
condition|(
...
begin_comment
comment|/* Test whether DECL is a builtin that may appear in a    constant-expression. */
end_comment
...
parameter_list|)
block|{
comment|/* At present BUILT_IN_CONSTANT_P is the only builtin we're allowing      in constant-expressions.  We may want to add other builtins later. */
return|return
...
begin_comment
comment|/* If EXPR is a bitfield reference, convert it to the declared type of    the bitfield, and return the resulting expression.  Otherwise,    return EXPR itself.  */
end_comment
...
begin_comment
comment|/* EXPR is being used in an rvalue context.  Return a version of EXPR    that is marked as an rvalue.  */
end_comment
...
return|;
block|}
comment|/* A reference or method type shall not be cv qualified.      [dcl.ref], [dct.fct]  */
if|if
condition|(
name|type_quals
operator|&
...
expr_stmt|;
block|}
comment|/* A restrict-qualified type must be a pointer (or reference)      to object or incomplete type, or a function type. */
if|if
condition|(
operator|(
name|type_quals
...
name|error
argument_list|(
literal|"%qV qualifiers cannot be applied to %qT"
argument_list|,
name|bad_type
argument_list|,
name|type
...
begin_comment
comment|/* Makes a copy of BINFO and TYPE, which is to be inherited into a    graph dominated by T.  If BINFO is NULL, TYPE is a dependent base,    and we do a shallow copy.  If BINFO is non-NULL, we do a deep copy.    VIRT indicates whether TYPE is inherited virtually or not.    IGO_PREV points at the previous binfo of the inheritance graph    order chain.  The newly copied binfo's TREE_CHAIN forms this    ordering.     The CLASSTYPE_VBASECLASSES vector of T is constructed in the    correct order. That is in the order the bases themselves should be    constructed in.     The BINFO_INHERITANCE of a virtual base class points to the binfo    of the most derived type. ??? We could probably change this so that    BINFO_INHERITANCE becomes synonymous with BINFO_PRIMARY, and hence    remove a field.  They currently can only differ for primary virtual    virtual bases.  */
end_comment
...
condition|)
block|{
comment|/* See if we've already made this virtual base.  */
name|new_binfo
operator|=
...
name|type
expr_stmt|;
comment|/* Chain it into the inheritance graph.  */
name|TREE_CHAIN
argument_list|(
operator|*
name|igo_prev
...
condition|)
block|{
comment|/* Push it onto the list after any virtual bases it contains 	 will have been pushed.  */
name|VEC_quick_push
argument_list|(
name|tree
argument_list|,
...
begin_comment
comment|/* Build a representation for the qualified name SCOPE::NAME.  TYPE is    the type of the result expression, if known, or NULL_TREE if the    resulting expression is type-dependent.  If TEMPLATE_P is true,    NAME is known to be a template because the user explicitly used the    "template" keyword after the "::".     All SCOPE_REFs should be built by use of this function.  */
end_comment
...
begin_comment
comment|/* Returns non-zero if X is an expression for a (possibly overloaded)    function.  If "f" is a function or function template, "f", "c->f",    "c.f", "C::f", and "f<int>" will all be considered possibly    overloaded functions.  Returns 2 if the function is actually    overloaded, i.e., if it is impossible to know the the type of the    function without performing overload resolution.  */
end_comment
...
begin_comment
comment|/* Returns true iff X is an expression for an overloaded function    whose type cannot be known without performing overload    resolution.  */
end_comment
...
begin_comment
comment|/* Check if the type T depends on a type with no linkage and if so, return    it.  If RELAXED_P then do not consider a class type declared within    a TREE_PUBLIC function to have no linkage.  */
end_comment
...
begin_comment
comment|/* Returns true if decl is within an anonymous namespace, however deeply    nested, or false otherwise.  */
end_comment
...
name|true
return|;
comment|/* Classes and namespaces inside anonymous namespaces have          TREE_PUBLIC == 0, so we can shortcut the search.  */
elseif|else
if|if
condition|(
name|TYPE_P
...
expr_stmt|;
block|}
comment|/* We can get here with --disable-checking.  */
return|return
name|false
return|;
block|}
...
name|error
argument_list|(
literal|"%qE attribute can only be applied to Java class definitions"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
argument_list|,
literal|"%qE attribute can only be applied "
literal|"to class definitions"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"%qE is obsolete; g++ vtables are now COM-compatible by default"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"can only use %qE attribute on file-scope definitions "
literal|"of objects of class type"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"%qE attribute is not supported on this platform"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Making a new main variant of a class type is broken.  */
name|gcc_assert
argument_list|(
operator|!
name|CLASS_TYPE_P
...
parameter_list|)
define|\
value|do							\     {							\       result = walk_tree (&(NODE), func, data, pset);	\       if (result) goto out;				\     }							\   while (0)
...
literal|1
return|;
comment|/* Don't auto-inline functions that might be replaced at link-time      with an alternative definition.  */
if|if
condition|(
operator|!
...
begin_comment
comment|/* Like save_expr, but for C++.  */
end_comment
...
parameter_list|)
block|{
comment|/* There is no reason to create a SAVE_EXPR within a template; if      needed, we can create the SAVE_EXPR when instantiating the      template.  Furthermore, the middle-end cannot handle C++-specific      tree codes.  */
if|if
condition|(
name|processing_template_decl
condition|)
...
name|lk_external
return|;
comment|/* Linkage of a CONST_DECL depends on the linkage of the enumeration      type.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|lk_none
return|;
comment|/* Members of the anonymous namespace also have TREE_PUBLIC unset, but      are considered to have external linkage for language purposes.  DECLs      really meant to have internal linkage have DECL_THIS_STATIC set.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* EXP is an expression that we want to pre-evaluate.  Returns (in    *INITP) an expression that will perform the pre-evaluation.  The    value returned by this function is a side-effect free expression    equivalent to the pre-evaluated expression.  Callers must ensure    that *INITP is evaluated before EXP.  */
end_comment
...
begin_comment
comment|/* Add NEW, an expression whose value we don't care about, after the    similar expression ORIG.  */
end_comment
...
begin_comment
comment|/* Like stabilize_expr, but for a call whose arguments we want to    pre-evaluate.  CALL is modified in place to use the pre-evaluated    arguments, while, upon return, *INITP contains an expression to    compute the arguments.  */
end_comment
...
begin_comment
comment|/* Like stabilize_expr, but for an initialization.       If the initialization is for an object of class type, this function    takes care not to introduce additional temporaries.     Returns TRUE iff the expression was successfully pre-evaluated,    i.e., if INIT is now side-effect free, except for, possible, a    single call to a constructor.  */
end_comment
...
return|;
block|}
comment|/* The initialization is being performed via a bitwise copy -- and      the item copied may have side effects.  */
return|return
name|TREE_SIDE_EFFECTS
argument_list|(
name|init
...
name|expr
return|;
comment|/* Fold C++ front-end specific tree codes.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Returns true if a cast to TYPE may appear in an integral constant    expression.  */
end_comment
