...
begin_comment
comment|/* Work around an OpenSSH problem: it can put its standard file    descriptors into nonblocking mode, which will mess us up if we    share file descriptions with it.  The simplest workaround is    to create an intervening process between OpenSSH and the    actual stderr.  */
end_comment

begin_function
specifier|static
name|void
name|work_around_openssh_glitch
parameter_list|(
name|void
parameter_list|)
block|{
...
name|sb
decl_stmt|;
comment|/* Do nothing unless stderr is a file that is affected by        nonblocking mode.  */
if|if
condition|(
operator|!
operator|(
...
condition|)
block|{
comment|/* Still in child of original process.  Act like "cat -u".  */
name|char
name|buf
index|[
literal|1
...
do|;
block|}
comment|/* Done processing output from grandchild.  Propagate 	   its exit status back to the parent.  */
while|while
condition|(
operator|(
name|w
...
name|errno
argument_list|,
literal|"child did not exit cleanly"
argument_list|)
expr_stmt|;
block|}
name|_exit
...
expr_stmt|;
block|}
comment|/* Grandchild of original process.  */
if|if
condition|(
name|close
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* Okay to cast out const below - execvp don't return nohow.  */
name|execvp
argument_list|(
operator|(
name|char
