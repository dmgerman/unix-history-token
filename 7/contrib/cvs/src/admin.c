...
name|usage_error
label|:
comment|/* Don't use cvs_cmd_name; it might be "server".  */
name|error
argument_list|(
literal|1
argument_list|,
...
block|}
block|}
comment|/* Attempt to perform the requested tagging.  */
if|if
condition|(
operator|(
operator|*
...
literal|'\0'
expr_stmt|;
comment|/* temporarily make arg+2 its own string */
name|rev
operator|=
name|RCS_gettag
argument_list|(
...
literal|':'
expr_stmt|;
comment|/* restore the full text of the -m argument */
continue|continue;
block|}
name|msg
operator|=
...
argument_list|)
expr_stmt|;
comment|/* tags may exist against non-existing versions */
if|if
condition|(
name|n
operator|==
...
literal|0
argument_list|,
literal|"%s: no such revision %s: %s"
argument_list|,
name|rcs
operator|->
name|path
