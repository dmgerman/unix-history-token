...
begin_comment
comment|/* Called from read.c:s_comm after we've parsed .comm symbol, size.    Parse a possible alignment value.  */
end_comment
...
condition|)
block|{
comment|/* For sparc.  Accept .common symbol, length, "bss"  */
name|input_line_pointer
operator|++
expr_stmt|;
comment|/* Some use the dot, some don't.  */
if|if
condition|(
operator|*
name|input_line_pointer
...
return|;
block|}
comment|/* ??? Don't ask me why these are always global.  */
name|is_local
operator|=
literal|0
expr_stmt|;
...
operator|==
name|NULL
comment|/* FIXME: gcc, as of 2002-10-22, will emit  		 .section .init_array,"aw",@progbits  		 for __attribute__ ((section (".init_array"))). 		 "@progbits" is incorrect.  */
operator|&&
name|ssect
operator|->
name|type
...
condition|)
block|{
comment|/* We allow to specify any type for a .note section.  */
if|if
condition|(
name|ssect
operator|->
...
condition|)
block|{
comment|/* As a GNU extension, we permit a .note section to be 	     allocatable.  If the linker sees an allocatable .note 	     section, it will create a PT_NOTE segment in the output 	     file.  We also allow "x" for .note.GNU-stack.  */
if|if
condition|(
name|ssect
operator|->
...
condition|)
empty_stmt|;
comment|/* Allow different SHF_MERGE and SHF_STRINGS if we have 	     something like .rodata.str.  */
elseif|else
if|if
condition|(
name|ssect
...
condition|)
empty_stmt|;
comment|/* .interp, .strtab and .symtab can have SHF_ALLOC.  */
elseif|else
if|if
condition|(
name|attr
...
name|_
argument_list|(
literal|"unrecognized .section attribute: want a,w,x,M,S,G,T"
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
...
name|_
argument_list|(
literal|"assuming all members of group `%s' are COMDAT"
argument_list|)
argument_list|,
name|group_name
argument_list|)
...
literal|0
block|,
comment|/* obj_frob_file_before_fix */
name|elf_frob_file_after_relocs
block|,
name|elf_s_get_size
block|,
