...
begin_comment
comment|/* O_register, in parentheses */
end_comment
...
begin_comment
comment|/* The alpha_reloc_op table below depends on the ordering of these.  */
end_comment
...
begin_comment
comment|/* !lituse_addr relocation */
end_comment
...
begin_comment
comment|/* !lituse_base relocation */
end_comment
...
begin_comment
comment|/* !lituse_bytoff relocation */
end_comment
...
begin_comment
comment|/* !lituse_tlsgd relocation */
end_comment
...
begin_comment
comment|/* !lituse_tlsldm relocation */
end_comment
...
value|(BFD_RELOC_UNUSED + 1)
end_define

begin_define
define|#
directive|define
name|DUMMY_RELOC_LITUSE_BASE
value|(BFD_RELOC_UNUSED + 2)
end_define

begin_define
define|#
directive|define
name|DUMMY_RELOC_LITUSE_BYTOFF
value|(BFD_RELOC_UNUSED + 3)
end_define

begin_define
define|#
directive|define
name|DUMMY_RELOC_LITUSE_JSR
value|(BFD_RELOC_UNUSED + 4)
end_define

begin_define
define|#
directive|define
name|DUMMY_RELOC_LITUSE_TLSGD
value|(BFD_RELOC_UNUSED + 5)
end_define

begin_define
define|#
directive|define
name|DUMMY_RELOC_LITUSE_TLSLDM
value|(BFD_RELOC_UNUSED + 6)
end_define
...
begin_comment
comment|/* XXX: The non-shift version appears to trigger a compiler bug when    cross-assembling from x86 w/ gcc 2.7.2.  */
end_comment
...
begin_comment
comment|/* Whether we are emitting an mdebug section.  */
end_comment
...
begin_comment
comment|/* Don't fully resolve relocations, allowing code movement in the linker.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|alpha_flag_relax
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* A table to map the spelling of a relocation operand into an appropriate    bfd_reloc_code_real_type type.  The table is assumed to be ordered such    that op-O_literal indexes into it.  */
end_comment

begin_define
define|#
directive|define
name|ALPHA_RELOC_TABLE
parameter_list|(
name|op
parameter_list|)
define|\
...
parameter_list|)
define|\
value|{ #NAME, sizeof(#NAME)-1, O_##NAME, RELOC, REQ, ALLOW}
end_define
...
name|name
decl_stmt|;
comment|/* string to lookup */
name|size_t
name|length
decl_stmt|;
comment|/* size of the string */
name|operatorT
name|op
decl_stmt|;
comment|/* which operator to use */
name|bfd_reloc_code_real_type
name|reloc
decl_stmt|;
comment|/* relocation before frob */
name|unsigned
name|int
name|require_seq
range|:
literal|1
decl_stmt|;
comment|/* require a sequence number */
name|unsigned
name|int
name|allow_seq
range|:
literal|1
decl_stmt|;
comment|/* allow a sequence number */
block|}
name|alpha_reloc_op
index|[]
init|=
...
begin_comment
comment|/* Maximum # digits needed to hold the largest sequence # */
end_comment

begin_define
define|#
directive|define
name|ALPHA_RELOC_DIGITS
value|25
end_define
...
name|master
decl_stmt|;
comment|/* the literal reloc */
name|fixS
modifier|*
name|slaves
decl_stmt|;
comment|/* head of linked list of lituses */
name|segT
name|segment
decl_stmt|;
comment|/* segment relocs are in or undefined_section*/
name|long
name|sequence
decl_stmt|;
comment|/* sequence # */
...
decl_stmt|;
name|unsigned
name|multi_section_p
range|:
literal|1
decl_stmt|;
comment|/* true if more than one section was used */
name|char
name|string
index|[
literal|1
index|]
decl_stmt|;
comment|/* printable form of sequence to hash with */
block|}
struct|;
end_struct

begin_comment
comment|/* Hash table to link up literals with the appropriate lituse */
end_comment
...
name|hash_control
modifier|*
name|alpha_literal_hash
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Sequence numbers for internal use by macros.  */
end_comment
...
begin_comment
comment|/* A table of CPU names and opcode sets.  */
end_comment
...
directive|endif
comment|/* OBJ_ELF */
comment|/* Create literal lookup hash table.  */
name|alpha_literal_hash
operator|=
name|hash_new
argument_list|()
...
name|str
argument_list|,
literal|"abcdefghijklmnopqrstuvwxyz_/46819"
argument_list|)
expr_stmt|;
name|trunclen
operator|=
...
name|fx_next
decl_stmt|;
comment|/* With user-specified !gpdisp relocations, we can be missing 	   the matching LO16 reloc.  We will have already issued an 	   error message.  */
if|if
condition|(
name|next
condition|)
...
name|BFD_RELOC_ALPHA_TPREL16
case|:
comment|/* ??? No idea why we can't return a reference to .tbss+10, but 	 we're preventing this in the other assemblers.  Follow for now.  */
return|return
literal|0
return|;
ifdef|#
...
name|BFD_RELOC_ALPHA_BRSGP
case|:
comment|/* If we have a BRSGP reloc to a local symbol, adjust it to BRADDR and          let it get resolved at assembly time.  */
block|{
name|symbolS
modifier|*
name|sym
...
name|_
argument_list|(
literal|"!samegp reloc against symbol without .prologue: %s"
argument_list|)
argument_list|,
name|name
argument_list|)
...
begin_comment
comment|/* Before the relocations are written, reorder them, so that user    supplied !lituse relocations follow the appropriate !literal    relocations, and similarly for !gpdisp relocations.  */
end_comment
...
condition|)
return|return;
comment|/* If there are no relocations, skip the section.  */
if|if
condition|(
operator|!
name|seginfo
...
name|_
argument_list|(
literal|"No !literal!%ld was found"
argument_list|)
argument_list|,
name|fixp
operator|->
...
name|_
argument_list|(
literal|"No !tlsldm!%ld was found"
argument_list|)
argument_list|,
name|fixp
operator|->
...
name|_
argument_list|(
literal|"No ldah !gpdisp!%ld was found"
argument_list|)
argument_list|,
name|fixp
operator|->
...
name|_
argument_list|(
literal|"too many !literal!%ld for %s"
argument_list|)
argument_list|,
name|fixp
operator|->
...
name|_
argument_list|(
literal|"No lda !gpdisp!%ld was found"
argument_list|)
argument_list|,
name|fixp
operator|->
...
directive|ifdef
name|RELOC_OP_P
comment|/* ??? Wrest control of ! away from the regular expression parser.  */
name|is_end_of_line
index|[
operator|(
name|unsigned
...
literal|'!'
case|:
comment|/* A relocation operand can be placed after the normal operand on an 	     assembly language statement, and has the following form: 		!relocation_type!sequence_number.  */
if|if
condition|(
name|reloc_found_p
condition|)
...
name|_
argument_list|(
literal|"More than one relocation op per insn"
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
...
name|_
argument_list|(
literal|"Unknown relocation operand: !%s"
argument_list|)
argument_list|,
name|p
argument_list|)
...
name|_
argument_list|(
literal|"no sequence number after !%s"
argument_list|)
argument_list|,
name|p
argument_list|)
...
name|_
argument_list|(
literal|"!%s does not use a sequence number"
argument_list|)
argument_list|,
name|p
argument_list|)
...
name|_
argument_list|(
literal|"Bad sequence number: !%s!%s"
argument_list|)
argument_list|,
name|r
operator|->
...
break|break;
default|default:
comment|/* This is only 0 for fields that should contain registers, 	     which means this pattern shouldn't have matched.  */
if|if
condition|(
name|operand
operator|->
...
argument_list|()
expr_stmt|;
comment|/* There is one special case for which an insn receives two 	     relocations, and thus the user-supplied reloc does not 	     override the operand reloc.  */
if|if
condition|(
name|operand
operator|->
...
argument_list|)
expr_stmt|;
comment|/* ??? My but this is hacky.  But the OSF/1 assembler uses the same 	 relocation tag for both ldah and lda with gpdisp.  Choose the 	 correct internal relocation based on the opcode.  */
if|if
condition|(
name|reloc
operator|==
...
name|_
argument_list|(
literal|"invalid relocation for instruction"
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* If this is a real relocation (as opposed to a lituse hint), then 	 the relocation width should match the operand width.  */
elseif|else
if|if
condition|(
name|reloc
...
name|_
argument_list|(
literal|"invalid relocation for field"
argument_list|)
argument_list|)
expr_stmt|;
return|return;
...
argument_list|)
expr_stmt|;
comment|/* Turn off complaints that the addend is too large for some fixups,          and copy in the sequence number for the explicit relocations.  */
switch|switch
condition|(
name|fixup
operator|->
...
name|_
argument_list|(
literal|"too many ldah insns for !gpdisp!%ld"
argument_list|)
argument_list|,
name|insn
operator|->
...
name|_
argument_list|(
literal|"both insns for !gpdisp!%ld must be in the same section"
argument_list|)
argument_list|,
name|insn
operator|->
...
name|_
argument_list|(
literal|"too many lda insns for !gpdisp!%ld"
argument_list|)
argument_list|,
name|insn
operator|->
...
name|_
argument_list|(
literal|"too many lituse insns for !lituse_tlsgd!%ld"
argument_list|)
argument_list|,
name|insn
operator|->
...
name|_
argument_list|(
literal|"too many lituse insns for !lituse_tlsldm!%ld"
argument_list|)
argument_list|,
name|insn
operator|->
...
name|_
argument_list|(
literal|"sequence number in use for !tlsldm!%ld"
argument_list|)
argument_list|,
name|insn
operator|->
...
name|_
argument_list|(
literal|"sequence number in use for !tlsgd!%ld"
argument_list|)
argument_list|,
name|insn
operator|->
...
directive|ifdef
name|RELOC_OP_P
comment|/* If a user-specified relocation is present, this is not a macro.  */
if|if
condition|(
name|ntok
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* Copy the sequence number for the reloc from the reloc token.  */
if|if
condition|(
name|reloc
operator|!=
...
name|_
argument_list|(
literal|"can not resolve expression"
argument_list|)
argument_list|)
expr_stmt|;
name|addend
...
argument_list|)
expr_stmt|;
comment|/* XXX: Disable this .got minimizing optimization so that we can get 	   better instruction offset knowledge in the compiler.  This happens 	   very infrequently anyway.  */
if|if
condition|(
literal|1
operator|||
...
name|_
argument_list|(
literal|"can't handle expression"
argument_list|)
argument_list|)
expr_stmt|;
name|addend
...
begin_comment
comment|/* Handle the OSF/1 and openVMS .comm pseudo quirks.    openVMS constructs a section for every common symbol.  */
end_comment
...
directive|ifdef
name|OBJ_EVAX
comment|/* Make a section for the common symbol.  */
name|new_seg
operator|=
name|subseg_new
argument_list|(
...
directive|ifdef
name|OBJ_EVAX
comment|/* alignment might follow  */
if|if
condition|(
operator|*
name|input_line_pointer
...
name|_
argument_list|(
literal|".end directive names different symbol than .ent"
argument_list|)
argument_list|)
expr_stmt|;
...
name|_
argument_list|(
literal|".prologue directive without a preceding .ent directive"
argument_list|)
argument_list|)
expr_stmt|;
return|return;
...
name|_
argument_list|(
literal|"Invalid argument %d to .prologue."
argument_list|)
argument_list|,
name|arg
argument_list|)
...
name|_
argument_list|(
literal|"ECOFF debugging is disabled."
argument_list|)
argument_list|)
expr_stmt|;
name|ignore_rest_of_line
...
expr_stmt|;
block|}
comment|/*  *symbol_get_obj (symbolP) = (signed char) temp; */
name|as_warn
argument_list|(
name|_
argument_list|(
...
block|as_warn (_("File overrides no-base-register option."));       first_32bit_quadrant = 0;     }
endif|#
directive|endif
name|SKIP_WHITESPACE
argument_list|()
...
begin_comment
comment|/* Here come the .uword, .ulong, and .uquad explicitly unaligned    pseudos.  We just turn off auto-alignment and call down to cons.  */
end_comment
...
begin_comment
comment|/* Switch the working cpu type.  */
end_comment
...
block|}
block|,
comment|/* Unaligned data pseudos.  */
block|{
literal|"uword"
block|,
name|s_alpha_ucons
...
directive|ifdef
name|OBJ_ELF
comment|/* Dwarf wants these versions of unaligned.  */
block|{
literal|"2byte"
block|,
name|s_alpha_ucons
...
begin_comment
comment|/* Map 's' to SHF_ALPHA_GPREL.  */
end_comment
...
name|_
argument_list|(
literal|"Bad .section directive: want a,s,w,x,M,S,G,T in string"
argument_list|)
expr_stmt|;
return|return
operator|-
...
begin_comment
comment|/* Map SHF_ALPHA_GPREL to SEC_SMALL_DATA.  */
end_comment
