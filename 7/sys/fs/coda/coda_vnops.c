...
name|coda_lookup
block|,
comment|/* uncached lookup */
operator|.
name|vop_lookup
operator|=
name|vfs_cache_lookup
block|,
comment|/* namecache lookup */
operator|.
name|vop_create
operator|=
name|coda_create
...
argument_list|)
expr_stmt|;
comment|/* 		 * Enter the new vnode in the namecache only if the top bit 		 * isn't set. 		 * 		 * And don't enter a new vnode for an invalid one! 		 */
if|if
condition|(
operator|!
operator|(
...
condition|)
block|{
comment|/* 		 * XXXRW: We only enter a negative entry if ENOENT is 		 * returned, not other errors.  But will Venus invalidate dvp 		 * properly in all cases when new files appear via the 		 * network rather than a local operation? 		 */
if|if
condition|(
name|cnp
operator|->
...
return|;
block|}
comment|/* 	 * Invalidate the parent's attr cache, the modification time has 	 * changed.  We don't yet know if the last reference to the file is 	 * being removed, but we do know the reference count on the child has 	 * changed, so invalidate its attr cache also. 	 */
name|VTOC
argument_list|(
name|dvp
argument_list|)
...
return|;
block|}
comment|/* 	 * Remove the entries for both source and target directories, which 	 * should catch references to the children.  Perhaps we could purge 	 * less? 	 */
name|cache_purge
argument_list|(
name|odvp
argument_list|)
...
return|;
block|}
comment|/* 	 * Possibly somewhat conservative purging, perhaps we just need to 	 * purge vp? 	 */
name|cache_purge
argument_list|(
name|dvp
argument_list|)
...
name|err
decl_stmt|;
comment|/* 	 * XXXRW: This really needs a moderate amount of reworking.  We need 	 * to properly tolerate failures of getnewvnode() and insmntque(), 	 * and callers need to be able to accept an error back from 	 * make_coda_node.  There may also be more general issues in how we 	 * handle forced unmount.  Finally, if/when Coda loses its dependency 	 * on Giant, the ordering of this needs rethinking. 	 */
name|cp
operator|=
name|coda_find
argument_list|(
