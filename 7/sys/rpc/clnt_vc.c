begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	$NetBSD: clnt_vc.c,v 1.4 2000/07/14 08:40:42 fvdl Exp $	*/
end_comment

begin_comment
comment|/*  * Sun RPC is a product of Sun Microsystems, Inc. and is provided for  * unrestricted use provided that this legend is included on all tape  * media and as a part of the software program in whole or part.  Users  * may copy or modify Sun RPC without charge, but are not authorized  * to license or distribute it to anyone else except as part of a product or  * program developed by the user.  *   * SUN RPC IS PROVIDED AS IS WITH NO WARRANTIES OF ANY KIND INCLUDING THE  * WARRANTIES OF DESIGN, MERCHANTIBILITY AND FITNESS FOR A PARTICULAR  * PURPOSE, OR ARISING FROM A COURSE OF DEALING, USAGE OR TRADE PRACTICE.  *   * Sun RPC is provided with no support and without any obligation on the  * part of Sun Microsystems, Inc. to assist in its use, correction,  * modification or enhancement.  *   * SUN MICROSYSTEMS, INC. SHALL HAVE NO LIABILITY WITH RESPECT TO THE  * INFRINGEMENT OF COPYRIGHTS, TRADE SECRETS OR ANY PATENTS BY SUN RPC  * OR ANY PART THEREOF.  *   * In no event will Sun Microsystems, Inc. be liable for any lost revenue  * or profits or other special, indirect and consequential damages, even if  * Sun has been advised of the possibility of such damages.  *   * Sun Microsystems, Inc.  * 2550 Garcia Avenue  * Mountain View, California  94043  */
end_comment

begin_if
if|#
directive|if
name|defined
argument_list|(
name|LIBC_SCCS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
end_if

begin_decl_stmt
specifier|static
name|char
modifier|*
name|sccsid2
init|=
literal|"@(#)clnt_tcp.c 1.37 87/10/05 Copyr 1984 Sun Micro"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|sccsid
init|=
literal|"@(#)clnt_tcp.c	2.2 88/08/01 4.0 RPCSRC"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
name|sccsid3
index|[]
init|=
literal|"@(#)clnt_vc.c 1.19 89/03/16 Copyr 1988 Sun Micro"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/cdefs.h>
end_include

begin_expr_stmt
name|__FBSDID
argument_list|(
literal|"$FreeBSD$"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_comment
comment|/*  * clnt_tcp.c, Implements a TCP/IP based, client side RPC.  *  * Copyright (C) 1984, Sun Microsystems, Inc.  *  * TCP based RPC supports 'batched calls'.  * A sequence of calls may be batched-up in a send buffer.  The rpc call  * return immediately to the client even though the call was not necessarily  * sent.  The batching occurs if the results' xdr routine is NULL (0) AND  * the rpc timeout value is zero (see clnt.h, rpc).  *  * Clients should NOT casually batch calls that in fact return results; that is,  * the server side should be aware that a call is batched and not produce any  * return message.  Batched calls that produce many result messages can  * deadlock (netlock) the client and the server....  *  * Now go hang yourself.  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/lock.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_include
include|#
directive|include
file|<sys/mutex.h>
end_include

begin_include
include|#
directive|include
file|<sys/pcpu.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/socketvar.h>
end_include

begin_include
include|#
directive|include
file|<sys/syslog.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/uio.h>
end_include

begin_include
include|#
directive|include
file|<rpc/rpc.h>
end_include

begin_include
include|#
directive|include
file|<rpc/rpc_com.h>
end_include

begin_define
define|#
directive|define
name|MCALL_MSG_SIZE
value|24
end_define

begin_struct
struct|struct
name|cmessage
block|{
name|struct
name|cmsghdr
name|cmsg
decl_stmt|;
name|struct
name|cmsgcred
name|cmcred
decl_stmt|;
block|}
struct|;
end_struct

begin_function_decl
specifier|static
name|enum
name|clnt_stat
name|clnt_vc_call
parameter_list|(
name|CLIENT
modifier|*
parameter_list|,
name|struct
name|rpc_callextra
modifier|*
parameter_list|,
name|rpcproc_t
parameter_list|,
name|xdrproc_t
parameter_list|,
name|void
modifier|*
parameter_list|,
name|xdrproc_t
parameter_list|,
name|void
modifier|*
parameter_list|,
name|struct
name|timeval
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|clnt_vc_geterr
parameter_list|(
name|CLIENT
modifier|*
parameter_list|,
name|struct
name|rpc_err
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|bool_t
name|clnt_vc_freeres
parameter_list|(
name|CLIENT
modifier|*
parameter_list|,
name|xdrproc_t
parameter_list|,
name|void
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|clnt_vc_abort
parameter_list|(
name|CLIENT
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|bool_t
name|clnt_vc_control
parameter_list|(
name|CLIENT
modifier|*
parameter_list|,
name|u_int
parameter_list|,
name|void
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|clnt_vc_destroy
parameter_list|(
name|CLIENT
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|bool_t
name|time_not_ok
parameter_list|(
name|struct
name|timeval
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|clnt_vc_soupcall
parameter_list|(
name|struct
name|socket
modifier|*
name|so
parameter_list|,
name|void
modifier|*
name|arg
parameter_list|,
name|int
name|waitflag
parameter_list|)
function_decl|;
end_function_decl

begin_decl_stmt
specifier|static
name|struct
name|clnt_ops
name|clnt_vc_ops
init|=
block|{
operator|.
name|cl_call
operator|=
name|clnt_vc_call
block|,
operator|.
name|cl_abort
operator|=
name|clnt_vc_abort
block|,
operator|.
name|cl_geterr
operator|=
name|clnt_vc_geterr
block|,
operator|.
name|cl_freeres
operator|=
name|clnt_vc_freeres
block|,
operator|.
name|cl_destroy
operator|=
name|clnt_vc_destroy
block|,
operator|.
name|cl_control
operator|=
name|clnt_vc_control
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * A pending RPC request which awaits a reply. Requests which have  * received their reply will have cr_xid set to zero and cr_mrep to  * the mbuf chain of the reply.  */
end_comment

begin_struct
struct|struct
name|ct_request
block|{
name|TAILQ_ENTRY
argument_list|(
argument|ct_request
argument_list|)
name|cr_link
expr_stmt|;
name|uint32_t
name|cr_xid
decl_stmt|;
comment|/* XID of request */
name|struct
name|mbuf
modifier|*
name|cr_mrep
decl_stmt|;
comment|/* reply received by upcall */
name|int
name|cr_error
decl_stmt|;
comment|/* any error from upcall */
block|}
struct|;
end_struct

begin_expr_stmt
name|TAILQ_HEAD
argument_list|(
name|ct_request_list
argument_list|,
name|ct_request
argument_list|)
expr_stmt|;
end_expr_stmt

begin_struct
struct|struct
name|ct_data
block|{
name|struct
name|mtx
name|ct_lock
decl_stmt|;
name|int
name|ct_threads
decl_stmt|;
comment|/* number of threads in clnt_vc_call */
name|bool_t
name|ct_closing
decl_stmt|;
comment|/* TRUE if we are destroying client */
name|struct
name|socket
modifier|*
name|ct_socket
decl_stmt|;
comment|/* connection socket */
name|bool_t
name|ct_closeit
decl_stmt|;
comment|/* close it on destroy */
name|struct
name|timeval
name|ct_wait
decl_stmt|;
comment|/* wait interval in milliseconds */
name|struct
name|sockaddr_storage
name|ct_addr
decl_stmt|;
comment|/* remote addr */
name|struct
name|rpc_err
name|ct_error
decl_stmt|;
name|uint32_t
name|ct_xid
decl_stmt|;
name|char
name|ct_mcallc
index|[
name|MCALL_MSG_SIZE
index|]
decl_stmt|;
comment|/* marshalled callmsg */
name|size_t
name|ct_mpos
decl_stmt|;
comment|/* pos after marshal */
specifier|const
name|char
modifier|*
name|ct_waitchan
decl_stmt|;
name|int
name|ct_waitflag
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|ct_record
decl_stmt|;
comment|/* current reply record */
name|size_t
name|ct_record_resid
decl_stmt|;
comment|/* how much left of reply to read */
name|bool_t
name|ct_record_eor
decl_stmt|;
comment|/* true if reading last fragment */
name|struct
name|ct_request_list
name|ct_pending
decl_stmt|;
block|}
struct|;
end_struct

begin_decl_stmt
specifier|static
specifier|const
name|char
name|clnt_vc_errstr
index|[]
init|=
literal|"%s : %s"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|char
name|clnt_vc_str
index|[]
init|=
literal|"clnt_vc_create"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|char
name|clnt_read_vc_str
index|[]
init|=
literal|"read_vc"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|char
name|__no_mem_str
index|[]
init|=
literal|"out of memory"
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Create a client handle for a connection.  * Default options are set, which the user can change using clnt_control()'s.  * The rpc/vc package does buffering similar to stdio, so the client  * must pick send and receive buffer sizes, 0 => use the default.  * NB: fd is copied into a private area.  * NB: The rpch->cl_auth is set null authentication. Caller may wish to  * set this something more useful.  *  * fd should be an open socket  */
end_comment

begin_function
name|CLIENT
modifier|*
name|clnt_vc_create
parameter_list|(
name|struct
name|socket
modifier|*
name|so
parameter_list|,
comment|/* open file descriptor */
name|struct
name|sockaddr
modifier|*
name|raddr
parameter_list|,
comment|/* servers address */
specifier|const
name|rpcprog_t
name|prog
parameter_list|,
comment|/* program number */
specifier|const
name|rpcvers_t
name|vers
parameter_list|,
comment|/* version number */
name|size_t
name|sendsz
parameter_list|,
comment|/* buffer recv size */
name|size_t
name|recvsz
parameter_list|)
comment|/* buffer send size */
block|{
name|CLIENT
modifier|*
name|cl
decl_stmt|;
comment|/* client handle */
name|struct
name|ct_data
modifier|*
name|ct
init|=
name|NULL
decl_stmt|;
comment|/* client handle */
name|struct
name|timeval
name|now
decl_stmt|;
name|struct
name|rpc_msg
name|call_msg
decl_stmt|;
specifier|static
name|uint32_t
name|disrupt
decl_stmt|;
name|struct
name|__rpc_sockinfo
name|si
decl_stmt|;
name|XDR
name|xdrs
decl_stmt|;
name|int
name|error
decl_stmt|,
name|interrupted
decl_stmt|;
if|if
condition|(
name|disrupt
operator|==
literal|0
condition|)
name|disrupt
operator|=
operator|(
name|uint32_t
operator|)
operator|(
name|long
operator|)
name|raddr
expr_stmt|;
name|cl
operator|=
operator|(
name|CLIENT
operator|*
operator|)
name|mem_alloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|cl
argument_list|)
argument_list|)
expr_stmt|;
name|ct
operator|=
operator|(
expr|struct
name|ct_data
operator|*
operator|)
name|mem_alloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|ct
argument_list|)
argument_list|)
expr_stmt|;
name|mtx_init
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|,
literal|"ct->ct_lock"
argument_list|,
name|NULL
argument_list|,
name|MTX_DEF
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_threads
operator|=
literal|0
expr_stmt|;
name|ct
operator|->
name|ct_closing
operator|=
name|FALSE
expr_stmt|;
if|if
condition|(
operator|(
name|so
operator|->
name|so_state
operator|&
operator|(
name|SS_ISCONNECTED
operator||
name|SS_ISCONFIRMING
operator|)
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|soconnect
argument_list|(
name|so
argument_list|,
name|raddr
argument_list|,
name|curthread
argument_list|)
expr_stmt|;
name|SOCK_LOCK
argument_list|(
name|so
argument_list|)
expr_stmt|;
name|interrupted
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISCONNECTING
operator|)
operator|&&
name|so
operator|->
name|so_error
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|msleep
argument_list|(
operator|&
name|so
operator|->
name|so_timeo
argument_list|,
name|SOCK_MTX
argument_list|(
name|so
argument_list|)
argument_list|,
name|PSOCK
operator||
name|PCATCH
argument_list|,
literal|"connec"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
if|if
condition|(
name|error
operator|==
name|EINTR
operator|||
name|error
operator|==
name|ERESTART
condition|)
name|interrupted
operator|=
literal|1
expr_stmt|;
break|break;
block|}
block|}
if|if
condition|(
name|error
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|so
operator|->
name|so_error
expr_stmt|;
name|so
operator|->
name|so_error
operator|=
literal|0
expr_stmt|;
block|}
name|SOCK_UNLOCK
argument_list|(
name|so
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
if|if
condition|(
operator|!
name|interrupted
condition|)
name|so
operator|->
name|so_state
operator|&=
operator|~
name|SS_ISCONNECTING
expr_stmt|;
name|rpc_createerr
operator|.
name|cf_stat
operator|=
name|RPC_SYSTEMERROR
expr_stmt|;
name|rpc_createerr
operator|.
name|cf_error
operator|.
name|re_errno
operator|=
name|error
expr_stmt|;
goto|goto
name|err
goto|;
block|}
block|}
if|if
condition|(
operator|!
name|__rpc_socket2sockinfo
argument_list|(
name|so
argument_list|,
operator|&
name|si
argument_list|)
condition|)
goto|goto
name|err
goto|;
name|ct
operator|->
name|ct_closeit
operator|=
name|FALSE
expr_stmt|;
comment|/* 	 * Set up private data struct 	 */
name|ct
operator|->
name|ct_socket
operator|=
name|so
expr_stmt|;
name|ct
operator|->
name|ct_wait
operator|.
name|tv_sec
operator|=
operator|-
literal|1
expr_stmt|;
name|ct
operator|->
name|ct_wait
operator|.
name|tv_usec
operator|=
operator|-
literal|1
expr_stmt|;
name|memcpy
argument_list|(
operator|&
name|ct
operator|->
name|ct_addr
argument_list|,
name|raddr
argument_list|,
name|raddr
operator|->
name|sa_len
argument_list|)
expr_stmt|;
comment|/* 	 * Initialize call message 	 */
name|getmicrotime
argument_list|(
operator|&
name|now
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_xid
operator|=
operator|(
operator|(
name|uint32_t
operator|)
operator|++
name|disrupt
operator|)
operator|^
name|__RPC_GETXID
argument_list|(
operator|&
name|now
argument_list|)
expr_stmt|;
name|call_msg
operator|.
name|rm_xid
operator|=
name|ct
operator|->
name|ct_xid
expr_stmt|;
name|call_msg
operator|.
name|rm_direction
operator|=
name|CALL
expr_stmt|;
name|call_msg
operator|.
name|rm_call
operator|.
name|cb_rpcvers
operator|=
name|RPC_MSG_VERSION
expr_stmt|;
name|call_msg
operator|.
name|rm_call
operator|.
name|cb_prog
operator|=
operator|(
name|uint32_t
operator|)
name|prog
expr_stmt|;
name|call_msg
operator|.
name|rm_call
operator|.
name|cb_vers
operator|=
operator|(
name|uint32_t
operator|)
name|vers
expr_stmt|;
comment|/* 	 * pre-serialize the static part of the call msg and stash it away 	 */
name|xdrmem_create
argument_list|(
operator|&
name|xdrs
argument_list|,
name|ct
operator|->
name|ct_mcallc
argument_list|,
name|MCALL_MSG_SIZE
argument_list|,
name|XDR_ENCODE
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|xdr_callhdr
argument_list|(
operator|&
name|xdrs
argument_list|,
operator|&
name|call_msg
argument_list|)
condition|)
block|{
if|if
condition|(
name|ct
operator|->
name|ct_closeit
condition|)
block|{
name|soclose
argument_list|(
name|ct
operator|->
name|ct_socket
argument_list|)
expr_stmt|;
block|}
goto|goto
name|err
goto|;
block|}
name|ct
operator|->
name|ct_mpos
operator|=
name|XDR_GETPOS
argument_list|(
operator|&
name|xdrs
argument_list|)
expr_stmt|;
name|XDR_DESTROY
argument_list|(
operator|&
name|xdrs
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_waitchan
operator|=
literal|"rpcrecv"
expr_stmt|;
name|ct
operator|->
name|ct_waitflag
operator|=
literal|0
expr_stmt|;
comment|/* 	 * Create a client handle which uses xdrrec for serialization 	 * and authnone for authentication. 	 */
name|cl
operator|->
name|cl_refs
operator|=
literal|1
expr_stmt|;
name|cl
operator|->
name|cl_ops
operator|=
operator|&
name|clnt_vc_ops
expr_stmt|;
name|cl
operator|->
name|cl_private
operator|=
name|ct
expr_stmt|;
name|cl
operator|->
name|cl_auth
operator|=
name|authnone_create
argument_list|()
expr_stmt|;
name|sendsz
operator|=
name|__rpc_get_t_size
argument_list|(
name|si
operator|.
name|si_af
argument_list|,
name|si
operator|.
name|si_proto
argument_list|,
operator|(
name|int
operator|)
name|sendsz
argument_list|)
expr_stmt|;
name|recvsz
operator|=
name|__rpc_get_t_size
argument_list|(
name|si
operator|.
name|si_af
argument_list|,
name|si
operator|.
name|si_proto
argument_list|,
operator|(
name|int
operator|)
name|recvsz
argument_list|)
expr_stmt|;
name|SOCKBUF_LOCK
argument_list|(
operator|&
name|ct
operator|->
name|ct_socket
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_socket
operator|->
name|so_upcallarg
operator|=
name|ct
expr_stmt|;
name|ct
operator|->
name|ct_socket
operator|->
name|so_upcall
operator|=
name|clnt_vc_soupcall
expr_stmt|;
name|ct
operator|->
name|ct_socket
operator|->
name|so_rcv
operator|.
name|sb_flags
operator||=
name|SB_UPCALL
expr_stmt|;
name|SOCKBUF_UNLOCK
argument_list|(
operator|&
name|ct
operator|->
name|ct_socket
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_record
operator|=
name|NULL
expr_stmt|;
name|ct
operator|->
name|ct_record_resid
operator|=
literal|0
expr_stmt|;
name|TAILQ_INIT
argument_list|(
operator|&
name|ct
operator|->
name|ct_pending
argument_list|)
expr_stmt|;
return|return
operator|(
name|cl
operator|)
return|;
name|err
label|:
if|if
condition|(
name|cl
condition|)
block|{
if|if
condition|(
name|ct
condition|)
block|{
name|mem_free
argument_list|(
name|ct
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|ct_data
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|cl
condition|)
name|mem_free
argument_list|(
name|cl
argument_list|,
sizeof|sizeof
argument_list|(
name|CLIENT
argument_list|)
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
operator|(
name|CLIENT
operator|*
operator|)
name|NULL
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|enum
name|clnt_stat
name|clnt_vc_call
parameter_list|(
name|CLIENT
modifier|*
name|cl
parameter_list|,
name|struct
name|rpc_callextra
modifier|*
name|ext
parameter_list|,
name|rpcproc_t
name|proc
parameter_list|,
name|xdrproc_t
name|xdr_args
parameter_list|,
name|void
modifier|*
name|args_ptr
parameter_list|,
name|xdrproc_t
name|xdr_results
parameter_list|,
name|void
modifier|*
name|results_ptr
parameter_list|,
name|struct
name|timeval
name|utimeout
parameter_list|)
block|{
name|struct
name|ct_data
modifier|*
name|ct
init|=
operator|(
expr|struct
name|ct_data
operator|*
operator|)
name|cl
operator|->
name|cl_private
decl_stmt|;
name|AUTH
modifier|*
name|auth
decl_stmt|;
name|XDR
name|xdrs
decl_stmt|;
name|struct
name|rpc_msg
name|reply_msg
decl_stmt|;
name|bool_t
name|ok
decl_stmt|;
name|int
name|nrefreshes
init|=
literal|2
decl_stmt|;
comment|/* number of times to refresh cred */
name|struct
name|timeval
name|timeout
decl_stmt|;
name|uint32_t
name|xid
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|mreq
init|=
name|NULL
decl_stmt|;
name|struct
name|ct_request
modifier|*
name|cr
decl_stmt|;
name|int
name|error
decl_stmt|;
name|cr
operator|=
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|ct_request
argument_list|)
argument_list|,
name|M_RPC
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
if|if
condition|(
name|ct
operator|->
name|ct_closing
condition|)
block|{
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cr
argument_list|,
name|M_RPC
argument_list|)
expr_stmt|;
return|return
operator|(
name|RPC_CANTSEND
operator|)
return|;
block|}
name|ct
operator|->
name|ct_threads
operator|++
expr_stmt|;
if|if
condition|(
name|ext
condition|)
name|auth
operator|=
name|ext
operator|->
name|rc_auth
expr_stmt|;
else|else
name|auth
operator|=
name|cl
operator|->
name|cl_auth
expr_stmt|;
name|cr
operator|->
name|cr_mrep
operator|=
name|NULL
expr_stmt|;
name|cr
operator|->
name|cr_error
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|ct
operator|->
name|ct_wait
operator|.
name|tv_usec
operator|==
operator|-
literal|1
condition|)
block|{
name|timeout
operator|=
name|utimeout
expr_stmt|;
comment|/* use supplied timeout */
block|}
else|else
block|{
name|timeout
operator|=
name|ct
operator|->
name|ct_wait
expr_stmt|;
comment|/* use default timeout */
block|}
name|call_again
label|:
name|mtx_assert
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|,
name|MA_OWNED
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_xid
operator|++
expr_stmt|;
name|xid
operator|=
name|ct
operator|->
name|ct_xid
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
comment|/* 	 * Leave space to pre-pend the record mark. 	 */
name|MGETHDR
argument_list|(
name|mreq
argument_list|,
name|M_WAIT
argument_list|,
name|MT_DATA
argument_list|)
expr_stmt|;
name|MCLGET
argument_list|(
name|mreq
argument_list|,
name|M_WAIT
argument_list|)
expr_stmt|;
name|mreq
operator|->
name|m_len
operator|=
literal|0
expr_stmt|;
name|mreq
operator|->
name|m_data
operator|+=
sizeof|sizeof
argument_list|(
name|uint32_t
argument_list|)
expr_stmt|;
name|m_append
argument_list|(
name|mreq
argument_list|,
name|ct
operator|->
name|ct_mpos
argument_list|,
name|ct
operator|->
name|ct_mcallc
argument_list|)
expr_stmt|;
comment|/* 	 * The XID is the first thing in the request. 	 */
operator|*
name|mtod
argument_list|(
name|mreq
argument_list|,
name|uint32_t
operator|*
argument_list|)
operator|=
name|htonl
argument_list|(
name|xid
argument_list|)
expr_stmt|;
name|xdrmbuf_create
argument_list|(
operator|&
name|xdrs
argument_list|,
name|mreq
argument_list|,
name|XDR_ENCODE
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_SUCCESS
expr_stmt|;
if|if
condition|(
operator|(
operator|!
name|XDR_PUTINT32
argument_list|(
operator|&
name|xdrs
argument_list|,
operator|&
name|proc
argument_list|)
operator|)
operator|||
operator|(
operator|!
name|AUTH_MARSHALL
argument_list|(
name|auth
argument_list|,
operator|&
name|xdrs
argument_list|)
operator|)
operator|||
operator|(
operator|!
call|(
modifier|*
name|xdr_args
call|)
argument_list|(
operator|&
name|xdrs
argument_list|,
name|args_ptr
argument_list|)
operator|)
condition|)
block|{
if|if
condition|(
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|==
name|RPC_SUCCESS
condition|)
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_CANTENCODEARGS
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
goto|goto
name|out
goto|;
block|}
name|m_fixhdr
argument_list|(
name|mreq
argument_list|)
expr_stmt|;
comment|/* 	 * Prepend a record marker containing the packet length. 	 */
name|M_PREPEND
argument_list|(
name|mreq
argument_list|,
sizeof|sizeof
argument_list|(
name|uint32_t
argument_list|)
argument_list|,
name|M_WAIT
argument_list|)
expr_stmt|;
operator|*
name|mtod
argument_list|(
name|mreq
argument_list|,
name|uint32_t
operator|*
argument_list|)
operator|=
name|htonl
argument_list|(
literal|0x80000000
operator||
operator|(
name|mreq
operator|->
name|m_pkthdr
operator|.
name|len
operator|-
sizeof|sizeof
argument_list|(
name|uint32_t
argument_list|)
operator|)
argument_list|)
expr_stmt|;
name|cr
operator|->
name|cr_xid
operator|=
name|xid
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
name|TAILQ_INSERT_TAIL
argument_list|(
operator|&
name|ct
operator|->
name|ct_pending
argument_list|,
name|cr
argument_list|,
name|cr_link
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
comment|/* 	 * sosend consumes mreq. 	 */
name|error
operator|=
name|sosend
argument_list|(
name|ct
operator|->
name|ct_socket
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|,
name|mreq
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|curthread
argument_list|)
expr_stmt|;
name|mreq
operator|=
name|NULL
expr_stmt|;
name|reply_msg
operator|.
name|acpted_rply
operator|.
name|ar_verf
operator|=
name|_null_auth
expr_stmt|;
name|reply_msg
operator|.
name|acpted_rply
operator|.
name|ar_results
operator|.
name|where
operator|=
name|results_ptr
expr_stmt|;
name|reply_msg
operator|.
name|acpted_rply
operator|.
name|ar_results
operator|.
name|proc
operator|=
name|xdr_results
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|TAILQ_REMOVE
argument_list|(
operator|&
name|ct
operator|->
name|ct_pending
argument_list|,
name|cr
argument_list|,
name|cr_link
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_errno
operator|=
name|error
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_CANTSEND
expr_stmt|;
goto|goto
name|out
goto|;
block|}
comment|/* 	 * Check to see if we got an upcall while waiting for the 	 * lock. In both these cases, the request has been removed 	 * from ct->ct_pending. 	 */
if|if
condition|(
name|cr
operator|->
name|cr_error
condition|)
block|{
name|TAILQ_REMOVE
argument_list|(
operator|&
name|ct
operator|->
name|ct_pending
argument_list|,
name|cr
argument_list|,
name|cr_link
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_errno
operator|=
name|cr
operator|->
name|cr_error
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_CANTRECV
expr_stmt|;
goto|goto
name|out
goto|;
block|}
if|if
condition|(
name|cr
operator|->
name|cr_mrep
condition|)
block|{
name|TAILQ_REMOVE
argument_list|(
operator|&
name|ct
operator|->
name|ct_pending
argument_list|,
name|cr
argument_list|,
name|cr_link
argument_list|)
expr_stmt|;
goto|goto
name|got_reply
goto|;
block|}
comment|/* 	 * Hack to provide rpc-based message passing 	 */
if|if
condition|(
name|timeout
operator|.
name|tv_sec
operator|==
literal|0
operator|&&
name|timeout
operator|.
name|tv_usec
operator|==
literal|0
condition|)
block|{
name|TAILQ_REMOVE
argument_list|(
operator|&
name|ct
operator|->
name|ct_pending
argument_list|,
name|cr
argument_list|,
name|cr_link
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_TIMEDOUT
expr_stmt|;
goto|goto
name|out
goto|;
block|}
name|error
operator|=
name|msleep
argument_list|(
name|cr
argument_list|,
operator|&
name|ct
operator|->
name|ct_lock
argument_list|,
name|ct
operator|->
name|ct_waitflag
argument_list|,
name|ct
operator|->
name|ct_waitchan
argument_list|,
name|tvtohz
argument_list|(
operator|&
name|timeout
argument_list|)
argument_list|)
expr_stmt|;
name|TAILQ_REMOVE
argument_list|(
operator|&
name|ct
operator|->
name|ct_pending
argument_list|,
name|cr
argument_list|,
name|cr_link
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
comment|/* 		 * The sleep returned an error so our request is still 		 * on the list. Turn the error code into an 		 * appropriate client status. 		 */
name|ct
operator|->
name|ct_error
operator|.
name|re_errno
operator|=
name|error
expr_stmt|;
switch|switch
condition|(
name|error
condition|)
block|{
case|case
name|EINTR
case|:
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_INTR
expr_stmt|;
break|break;
case|case
name|EWOULDBLOCK
case|:
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_TIMEDOUT
expr_stmt|;
break|break;
default|default:
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_CANTRECV
expr_stmt|;
block|}
goto|goto
name|out
goto|;
block|}
else|else
block|{
comment|/* 		 * We were woken up by the upcall.  If the 		 * upcall had a receive error, report that, 		 * otherwise we have a reply. 		 */
if|if
condition|(
name|cr
operator|->
name|cr_error
condition|)
block|{
name|ct
operator|->
name|ct_error
operator|.
name|re_errno
operator|=
name|cr
operator|->
name|cr_error
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_CANTRECV
expr_stmt|;
goto|goto
name|out
goto|;
block|}
block|}
name|got_reply
label|:
comment|/* 	 * Now decode and validate the response. We need to drop the 	 * lock since xdr_replymsg may end up sleeping in malloc. 	 */
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
name|xdrmbuf_create
argument_list|(
operator|&
name|xdrs
argument_list|,
name|cr
operator|->
name|cr_mrep
argument_list|,
name|XDR_DECODE
argument_list|)
expr_stmt|;
name|ok
operator|=
name|xdr_replymsg
argument_list|(
operator|&
name|xdrs
argument_list|,
operator|&
name|reply_msg
argument_list|)
expr_stmt|;
name|XDR_DESTROY
argument_list|(
operator|&
name|xdrs
argument_list|)
expr_stmt|;
name|cr
operator|->
name|cr_mrep
operator|=
name|NULL
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
if|if
condition|(
name|ok
condition|)
block|{
if|if
condition|(
operator|(
name|reply_msg
operator|.
name|rm_reply
operator|.
name|rp_stat
operator|==
name|MSG_ACCEPTED
operator|)
operator|&&
operator|(
name|reply_msg
operator|.
name|acpted_rply
operator|.
name|ar_stat
operator|==
name|SUCCESS
operator|)
condition|)
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_SUCCESS
expr_stmt|;
else|else
name|_seterr_reply
argument_list|(
operator|&
name|reply_msg
argument_list|,
operator|&
operator|(
name|ct
operator|->
name|ct_error
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|==
name|RPC_SUCCESS
condition|)
block|{
if|if
condition|(
operator|!
name|AUTH_VALIDATE
argument_list|(
name|cl
operator|->
name|cl_auth
argument_list|,
operator|&
name|reply_msg
operator|.
name|acpted_rply
operator|.
name|ar_verf
argument_list|)
condition|)
block|{
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_AUTHERROR
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_why
operator|=
name|AUTH_INVALIDRESP
expr_stmt|;
block|}
if|if
condition|(
name|reply_msg
operator|.
name|acpted_rply
operator|.
name|ar_verf
operator|.
name|oa_base
operator|!=
name|NULL
condition|)
block|{
name|xdrs
operator|.
name|x_op
operator|=
name|XDR_FREE
expr_stmt|;
operator|(
name|void
operator|)
name|xdr_opaque_auth
argument_list|(
operator|&
name|xdrs
argument_list|,
operator|&
operator|(
name|reply_msg
operator|.
name|acpted_rply
operator|.
name|ar_verf
operator|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* end successful completion */
comment|/* 		 * If unsuccesful AND error is an authentication error 		 * then refresh credentials and try again, else break 		 */
elseif|else
if|if
condition|(
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|==
name|RPC_AUTHERROR
condition|)
comment|/* maybe our credentials need to be refreshed ... */
if|if
condition|(
name|nrefreshes
operator|>
literal|0
operator|&&
name|AUTH_REFRESH
argument_list|(
name|cl
operator|->
name|cl_auth
argument_list|,
operator|&
name|reply_msg
argument_list|)
condition|)
block|{
name|nrefreshes
operator|--
expr_stmt|;
goto|goto
name|call_again
goto|;
block|}
comment|/* end of unsuccessful completion */
block|}
comment|/* end of valid reply message */
else|else
block|{
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_CANTDECODERES
expr_stmt|;
block|}
name|out
label|:
name|mtx_assert
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|,
name|MA_OWNED
argument_list|)
expr_stmt|;
if|if
condition|(
name|mreq
condition|)
name|m_freem
argument_list|(
name|mreq
argument_list|)
expr_stmt|;
if|if
condition|(
name|cr
operator|->
name|cr_mrep
condition|)
name|m_freem
argument_list|(
name|cr
operator|->
name|cr_mrep
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_threads
operator|--
expr_stmt|;
if|if
condition|(
name|ct
operator|->
name|ct_closing
condition|)
name|wakeup
argument_list|(
name|ct
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cr
argument_list|,
name|M_RPC
argument_list|)
expr_stmt|;
return|return
operator|(
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|clnt_vc_geterr
parameter_list|(
name|CLIENT
modifier|*
name|cl
parameter_list|,
name|struct
name|rpc_err
modifier|*
name|errp
parameter_list|)
block|{
name|struct
name|ct_data
modifier|*
name|ct
init|=
operator|(
expr|struct
name|ct_data
operator|*
operator|)
name|cl
operator|->
name|cl_private
decl_stmt|;
operator|*
name|errp
operator|=
name|ct
operator|->
name|ct_error
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|bool_t
name|clnt_vc_freeres
parameter_list|(
name|CLIENT
modifier|*
name|cl
parameter_list|,
name|xdrproc_t
name|xdr_res
parameter_list|,
name|void
modifier|*
name|res_ptr
parameter_list|)
block|{
name|XDR
name|xdrs
decl_stmt|;
name|bool_t
name|dummy
decl_stmt|;
name|xdrs
operator|.
name|x_op
operator|=
name|XDR_FREE
expr_stmt|;
name|dummy
operator|=
call|(
modifier|*
name|xdr_res
call|)
argument_list|(
operator|&
name|xdrs
argument_list|,
name|res_ptr
argument_list|)
expr_stmt|;
return|return
operator|(
name|dummy
operator|)
return|;
block|}
end_function

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|clnt_vc_abort
parameter_list|(
name|CLIENT
modifier|*
name|cl
parameter_list|)
block|{ }
end_function

begin_function
specifier|static
name|bool_t
name|clnt_vc_control
parameter_list|(
name|CLIENT
modifier|*
name|cl
parameter_list|,
name|u_int
name|request
parameter_list|,
name|void
modifier|*
name|info
parameter_list|)
block|{
name|struct
name|ct_data
modifier|*
name|ct
init|=
operator|(
expr|struct
name|ct_data
operator|*
operator|)
name|cl
operator|->
name|cl_private
decl_stmt|;
name|void
modifier|*
name|infop
init|=
name|info
decl_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|request
condition|)
block|{
case|case
name|CLSET_FD_CLOSE
case|:
name|ct
operator|->
name|ct_closeit
operator|=
name|TRUE
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
return|return
operator|(
name|TRUE
operator|)
return|;
case|case
name|CLSET_FD_NCLOSE
case|:
name|ct
operator|->
name|ct_closeit
operator|=
name|FALSE
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
return|return
operator|(
name|TRUE
operator|)
return|;
default|default:
break|break;
block|}
comment|/* for other requests which use info */
if|if
condition|(
name|info
operator|==
name|NULL
condition|)
block|{
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
return|return
operator|(
name|FALSE
operator|)
return|;
block|}
switch|switch
condition|(
name|request
condition|)
block|{
case|case
name|CLSET_TIMEOUT
case|:
if|if
condition|(
name|time_not_ok
argument_list|(
operator|(
expr|struct
name|timeval
operator|*
operator|)
name|info
argument_list|)
condition|)
block|{
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
return|return
operator|(
name|FALSE
operator|)
return|;
block|}
name|ct
operator|->
name|ct_wait
operator|=
operator|*
operator|(
expr|struct
name|timeval
operator|*
operator|)
name|infop
expr_stmt|;
break|break;
case|case
name|CLGET_TIMEOUT
case|:
operator|*
operator|(
expr|struct
name|timeval
operator|*
operator|)
name|infop
operator|=
name|ct
operator|->
name|ct_wait
expr_stmt|;
break|break;
case|case
name|CLGET_SERVER_ADDR
case|:
operator|(
name|void
operator|)
name|memcpy
argument_list|(
name|info
argument_list|,
operator|&
name|ct
operator|->
name|ct_addr
argument_list|,
operator|(
name|size_t
operator|)
name|ct
operator|->
name|ct_addr
operator|.
name|ss_len
argument_list|)
expr_stmt|;
break|break;
case|case
name|CLGET_SVC_ADDR
case|:
comment|/* 		 * Slightly different semantics to userland - we use 		 * sockaddr instead of netbuf. 		 */
name|memcpy
argument_list|(
name|info
argument_list|,
operator|&
name|ct
operator|->
name|ct_addr
argument_list|,
name|ct
operator|->
name|ct_addr
operator|.
name|ss_len
argument_list|)
expr_stmt|;
break|break;
case|case
name|CLSET_SVC_ADDR
case|:
comment|/* set to new address */
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
return|return
operator|(
name|FALSE
operator|)
return|;
case|case
name|CLGET_XID
case|:
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|info
operator|=
name|ct
operator|->
name|ct_xid
expr_stmt|;
break|break;
case|case
name|CLSET_XID
case|:
comment|/* This will set the xid of the NEXT call */
comment|/* decrement by 1 as clnt_vc_call() increments once */
name|ct
operator|->
name|ct_xid
operator|=
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|info
operator|-
literal|1
expr_stmt|;
break|break;
case|case
name|CLGET_VERS
case|:
comment|/* 		 * This RELIES on the information that, in the call body, 		 * the version number field is the fifth field from the 		 * begining of the RPC header. MUST be changed if the 		 * call_struct is changed 		 */
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|info
operator|=
name|ntohl
argument_list|(
operator|*
operator|(
name|uint32_t
operator|*
operator|)
operator|(
name|void
operator|*
operator|)
operator|(
name|ct
operator|->
name|ct_mcallc
operator|+
literal|4
operator|*
name|BYTES_PER_XDR_UNIT
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|CLSET_VERS
case|:
operator|*
operator|(
name|uint32_t
operator|*
operator|)
operator|(
name|void
operator|*
operator|)
operator|(
name|ct
operator|->
name|ct_mcallc
operator|+
literal|4
operator|*
name|BYTES_PER_XDR_UNIT
operator|)
operator|=
name|htonl
argument_list|(
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|info
argument_list|)
expr_stmt|;
break|break;
case|case
name|CLGET_PROG
case|:
comment|/* 		 * This RELIES on the information that, in the call body, 		 * the program number field is the fourth field from the 		 * begining of the RPC header. MUST be changed if the 		 * call_struct is changed 		 */
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|info
operator|=
name|ntohl
argument_list|(
operator|*
operator|(
name|uint32_t
operator|*
operator|)
operator|(
name|void
operator|*
operator|)
operator|(
name|ct
operator|->
name|ct_mcallc
operator|+
literal|3
operator|*
name|BYTES_PER_XDR_UNIT
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|CLSET_PROG
case|:
operator|*
operator|(
name|uint32_t
operator|*
operator|)
operator|(
name|void
operator|*
operator|)
operator|(
name|ct
operator|->
name|ct_mcallc
operator|+
literal|3
operator|*
name|BYTES_PER_XDR_UNIT
operator|)
operator|=
name|htonl
argument_list|(
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|info
argument_list|)
expr_stmt|;
break|break;
case|case
name|CLSET_WAITCHAN
case|:
name|ct
operator|->
name|ct_waitchan
operator|=
operator|*
operator|(
specifier|const
name|char
operator|*
operator|*
operator|)
name|info
expr_stmt|;
break|break;
case|case
name|CLGET_WAITCHAN
case|:
operator|*
operator|(
specifier|const
name|char
operator|*
operator|*
operator|)
name|info
operator|=
name|ct
operator|->
name|ct_waitchan
expr_stmt|;
break|break;
case|case
name|CLSET_INTERRUPTIBLE
case|:
if|if
condition|(
operator|*
operator|(
name|int
operator|*
operator|)
name|info
condition|)
name|ct
operator|->
name|ct_waitflag
operator|=
name|PCATCH
expr_stmt|;
else|else
name|ct
operator|->
name|ct_waitflag
operator|=
literal|0
expr_stmt|;
break|break;
case|case
name|CLGET_INTERRUPTIBLE
case|:
if|if
condition|(
name|ct
operator|->
name|ct_waitflag
condition|)
operator|*
operator|(
name|int
operator|*
operator|)
name|info
operator|=
name|TRUE
expr_stmt|;
else|else
operator|*
operator|(
name|int
operator|*
operator|)
name|info
operator|=
name|FALSE
expr_stmt|;
break|break;
default|default:
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
return|return
operator|(
name|FALSE
operator|)
return|;
block|}
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
return|return
operator|(
name|TRUE
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|clnt_vc_destroy
parameter_list|(
name|CLIENT
modifier|*
name|cl
parameter_list|)
block|{
name|struct
name|ct_data
modifier|*
name|ct
init|=
operator|(
expr|struct
name|ct_data
operator|*
operator|)
name|cl
operator|->
name|cl_private
decl_stmt|;
name|struct
name|ct_request
modifier|*
name|cr
decl_stmt|;
name|struct
name|socket
modifier|*
name|so
init|=
name|NULL
decl_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
if|if
condition|(
name|ct
operator|->
name|ct_socket
condition|)
block|{
name|SOCKBUF_LOCK
argument_list|(
operator|&
name|ct
operator|->
name|ct_socket
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_socket
operator|->
name|so_upcallarg
operator|=
name|NULL
expr_stmt|;
name|ct
operator|->
name|ct_socket
operator|->
name|so_upcall
operator|=
name|NULL
expr_stmt|;
name|ct
operator|->
name|ct_socket
operator|->
name|so_rcv
operator|.
name|sb_flags
operator|&=
operator|~
name|SB_UPCALL
expr_stmt|;
name|SOCKBUF_UNLOCK
argument_list|(
operator|&
name|ct
operator|->
name|ct_socket
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
comment|/* 		 * Abort any pending requests and wait until everyone 		 * has finished with clnt_vc_call. 		 */
name|ct
operator|->
name|ct_closing
operator|=
name|TRUE
expr_stmt|;
name|TAILQ_FOREACH
argument_list|(
argument|cr
argument_list|,
argument|&ct->ct_pending
argument_list|,
argument|cr_link
argument_list|)
block|{
name|cr
operator|->
name|cr_xid
operator|=
literal|0
expr_stmt|;
name|cr
operator|->
name|cr_error
operator|=
name|ESHUTDOWN
expr_stmt|;
name|wakeup
argument_list|(
name|cr
argument_list|)
expr_stmt|;
block|}
while|while
condition|(
name|ct
operator|->
name|ct_threads
condition|)
name|msleep
argument_list|(
name|ct
argument_list|,
operator|&
name|ct
operator|->
name|ct_lock
argument_list|,
literal|0
argument_list|,
literal|"rpcclose"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|ct
operator|->
name|ct_closeit
condition|)
block|{
name|so
operator|=
name|ct
operator|->
name|ct_socket
expr_stmt|;
block|}
block|}
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
name|mtx_destroy
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
condition|)
block|{
name|soshutdown
argument_list|(
name|so
argument_list|,
name|SHUT_WR
argument_list|)
expr_stmt|;
name|soclose
argument_list|(
name|so
argument_list|)
expr_stmt|;
block|}
name|mem_free
argument_list|(
name|ct
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|ct_data
argument_list|)
argument_list|)
expr_stmt|;
name|mem_free
argument_list|(
name|cl
argument_list|,
sizeof|sizeof
argument_list|(
name|CLIENT
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Make sure that the time is not garbage.   -1 value is disallowed.  * Note this is different from time_not_ok in clnt_dg.c  */
end_comment

begin_function
specifier|static
name|bool_t
name|time_not_ok
parameter_list|(
name|struct
name|timeval
modifier|*
name|t
parameter_list|)
block|{
return|return
operator|(
name|t
operator|->
name|tv_sec
operator|<=
operator|-
literal|1
operator|||
name|t
operator|->
name|tv_sec
operator|>
literal|100000000
operator|||
name|t
operator|->
name|tv_usec
operator|<=
operator|-
literal|1
operator|||
name|t
operator|->
name|tv_usec
operator|>
literal|1000000
operator|)
return|;
block|}
end_function

begin_function
name|void
name|clnt_vc_soupcall
parameter_list|(
name|struct
name|socket
modifier|*
name|so
parameter_list|,
name|void
modifier|*
name|arg
parameter_list|,
name|int
name|waitflag
parameter_list|)
block|{
name|struct
name|ct_data
modifier|*
name|ct
init|=
operator|(
expr|struct
name|ct_data
operator|*
operator|)
name|arg
decl_stmt|;
name|struct
name|uio
name|uio
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|struct
name|ct_request
modifier|*
name|cr
decl_stmt|;
name|int
name|error
decl_stmt|,
name|rcvflag
decl_stmt|,
name|foundreq
decl_stmt|;
name|uint32_t
name|xid
decl_stmt|,
name|header
decl_stmt|;
name|uio
operator|.
name|uio_td
operator|=
name|curthread
expr_stmt|;
do|do
block|{
comment|/* 		 * If ct_record_resid is zero, we are waiting for a 		 * record mark. 		 */
if|if
condition|(
name|ct
operator|->
name|ct_record_resid
operator|==
literal|0
condition|)
block|{
name|bool_t
name|do_read
decl_stmt|;
comment|/* 			 * Make sure there is either a whole record 			 * mark in the buffer or there is some other 			 * error condition 			 */
name|do_read
operator|=
name|FALSE
expr_stmt|;
name|SOCKBUF_LOCK
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_rcv
operator|.
name|sb_cc
operator|>=
sizeof|sizeof
argument_list|(
name|uint32_t
argument_list|)
operator|||
operator|(
name|so
operator|->
name|so_rcv
operator|.
name|sb_state
operator|&
name|SBS_CANTRCVMORE
operator|)
operator|||
name|so
operator|->
name|so_error
condition|)
name|do_read
operator|=
name|TRUE
expr_stmt|;
name|SOCKBUF_UNLOCK
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|do_read
condition|)
return|return;
name|uio
operator|.
name|uio_resid
operator|=
sizeof|sizeof
argument_list|(
name|uint32_t
argument_list|)
expr_stmt|;
name|m
operator|=
name|NULL
expr_stmt|;
name|rcvflag
operator|=
name|MSG_DONTWAIT
operator||
name|MSG_SOCALLBCK
expr_stmt|;
name|error
operator|=
name|soreceive
argument_list|(
name|so
argument_list|,
name|NULL
argument_list|,
operator|&
name|uio
argument_list|,
operator|&
name|m
argument_list|,
name|NULL
argument_list|,
operator|&
name|rcvflag
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|==
name|EWOULDBLOCK
condition|)
break|break;
comment|/* 			 * If there was an error, wake up all pending 			 * requests. 			 */
if|if
condition|(
name|error
operator|||
name|uio
operator|.
name|uio_resid
operator|>
literal|0
condition|)
block|{
name|wakeup_all
label|:
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|error
condition|)
block|{
comment|/* 					 * We must have got EOF trying 					 * to read from the stream. 					 */
name|error
operator|=
name|ECONNRESET
expr_stmt|;
block|}
name|ct
operator|->
name|ct_error
operator|.
name|re_status
operator|=
name|RPC_CANTRECV
expr_stmt|;
name|ct
operator|->
name|ct_error
operator|.
name|re_errno
operator|=
name|error
expr_stmt|;
name|TAILQ_FOREACH
argument_list|(
argument|cr
argument_list|,
argument|&ct->ct_pending
argument_list|,
argument|cr_link
argument_list|)
block|{
name|cr
operator|->
name|cr_error
operator|=
name|error
expr_stmt|;
name|wakeup
argument_list|(
name|cr
argument_list|)
expr_stmt|;
block|}
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
break|break;
block|}
name|memcpy
argument_list|(
operator|&
name|header
argument_list|,
name|mtod
argument_list|(
name|m
argument_list|,
name|uint32_t
operator|*
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
name|uint32_t
argument_list|)
argument_list|)
expr_stmt|;
name|header
operator|=
name|ntohl
argument_list|(
name|header
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_record
operator|=
name|NULL
expr_stmt|;
name|ct
operator|->
name|ct_record_resid
operator|=
name|header
operator|&
literal|0x7fffffff
expr_stmt|;
name|ct
operator|->
name|ct_record_eor
operator|=
operator|(
operator|(
name|header
operator|&
literal|0x80000000
operator|)
operator|!=
literal|0
operator|)
expr_stmt|;
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/* 			 * We have the record mark. Read as much as 			 * the socket has buffered up to the end of 			 * this record. 			 */
name|uio
operator|.
name|uio_resid
operator|=
name|ct
operator|->
name|ct_record_resid
expr_stmt|;
name|m
operator|=
name|NULL
expr_stmt|;
name|rcvflag
operator|=
name|MSG_DONTWAIT
operator||
name|MSG_SOCALLBCK
expr_stmt|;
name|error
operator|=
name|soreceive
argument_list|(
name|so
argument_list|,
name|NULL
argument_list|,
operator|&
name|uio
argument_list|,
operator|&
name|m
argument_list|,
name|NULL
argument_list|,
operator|&
name|rcvflag
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|==
name|EWOULDBLOCK
condition|)
break|break;
if|if
condition|(
name|error
operator|||
name|uio
operator|.
name|uio_resid
operator|==
name|ct
operator|->
name|ct_record_resid
condition|)
goto|goto
name|wakeup_all
goto|;
comment|/* 			 * If we have part of the record already, 			 * chain this bit onto the end. 			 */
if|if
condition|(
name|ct
operator|->
name|ct_record
condition|)
name|m_last
argument_list|(
name|ct
operator|->
name|ct_record
argument_list|)
operator|->
name|m_next
operator|=
name|m
expr_stmt|;
else|else
name|ct
operator|->
name|ct_record
operator|=
name|m
expr_stmt|;
name|ct
operator|->
name|ct_record_resid
operator|=
name|uio
operator|.
name|uio_resid
expr_stmt|;
comment|/* 			 * If we have the entire record, see if we can 			 * match it to a request. 			 */
if|if
condition|(
name|ct
operator|->
name|ct_record_resid
operator|==
literal|0
operator|&&
name|ct
operator|->
name|ct_record_eor
condition|)
block|{
comment|/* 				 * The XID is in the first uint32_t of 				 * the reply. 				 */
name|ct
operator|->
name|ct_record
operator|=
name|m_pullup
argument_list|(
name|ct
operator|->
name|ct_record
argument_list|,
sizeof|sizeof
argument_list|(
name|xid
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|ct
operator|->
name|ct_record
condition|)
break|break;
name|memcpy
argument_list|(
operator|&
name|xid
argument_list|,
name|mtod
argument_list|(
name|ct
operator|->
name|ct_record
argument_list|,
name|uint32_t
operator|*
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
name|uint32_t
argument_list|)
argument_list|)
expr_stmt|;
name|xid
operator|=
name|ntohl
argument_list|(
name|xid
argument_list|)
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
name|foundreq
operator|=
literal|0
expr_stmt|;
name|TAILQ_FOREACH
argument_list|(
argument|cr
argument_list|,
argument|&ct->ct_pending
argument_list|,
argument|cr_link
argument_list|)
block|{
if|if
condition|(
name|cr
operator|->
name|cr_xid
operator|==
name|xid
condition|)
block|{
comment|/* 						 * This one 						 * matches. We leave 						 * the reply mbuf in 						 * cr->cr_mrep. Set 						 * the XID to zero so 						 * that we will ignore 						 * any duplicaed 						 * replies. 						 */
name|cr
operator|->
name|cr_xid
operator|=
literal|0
expr_stmt|;
name|cr
operator|->
name|cr_mrep
operator|=
name|ct
operator|->
name|ct_record
expr_stmt|;
name|cr
operator|->
name|cr_error
operator|=
literal|0
expr_stmt|;
name|foundreq
operator|=
literal|1
expr_stmt|;
name|wakeup
argument_list|(
name|cr
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
name|mtx_unlock
argument_list|(
operator|&
name|ct
operator|->
name|ct_lock
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|foundreq
condition|)
name|m_freem
argument_list|(
name|ct
operator|->
name|ct_record
argument_list|)
expr_stmt|;
name|ct
operator|->
name|ct_record
operator|=
name|NULL
expr_stmt|;
block|}
block|}
block|}
do|while
condition|(
name|m
condition|)
do|;
block|}
end_function

end_unit

