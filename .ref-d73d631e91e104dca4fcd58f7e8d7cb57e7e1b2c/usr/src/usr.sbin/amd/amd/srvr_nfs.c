...
begin_comment
comment|/*  * $Id: srvr_nfs.c,v 5.2 90/06/23 22:20:02 jsp Rel $  *  * Copyright (c) 1990 Jan-Simon Pendry  * Copyright (c) 1990 Imperial College of Science, Technology& Medicine  * Copyright (c) 1990 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Jan-Simon Pendry at Imperial College, London.  *  * %sccs.include.redist.c%  *  *	@(#)srvr_nfs.c	5.1 (Berkeley) %G%  */
end_comment
...
name|np_mountd
expr_stmt|;
comment|/* 				 * Now go get it again in case it changed 				 */
name|np
operator|->
name|np_mountd_inval
operator|=
...
name|top
label|:
comment|/* 	 * Scan the list of known servers looking 	 * for one with the same name 	 */
name|ITER
argument_list|(
argument|fs
argument_list|,
...
block|}
block|}
comment|/* 	 * If the name is not known, it may be 	 * because it was an alternate name for 	 * the same machine.  So do a lookup and 	 * try again with the primary name if that 	 * is different. 	 * All that assuming it wasn't normalized 	 * earlier of course... 	 */
if|if
condition|(
name|hp
operator|==
