begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * $Id: amq.c,v 5.2 90/06/23 22:20:07 jsp Rel $  *  * Copyright (c) 1990 Jan-Simon Pendry  * Copyright (c) 1990 Imperial College of Science, Technology& Medicine  * Copyright (c) 1990 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Jan-Simon Pendry at Imperial College, London.  *  * %sccs.include.redist.c%  */
end_comment

begin_comment
comment|/*  * Automounter query tool  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"\ @(#)Copyright (c) 1990 Jan-Simon Pendry\n\ @(#)Copyright (c) 1990 Imperial College of Science, Technology& Medicine\n\ @(#)Copyright (c) 1990 The Regents of the University of California.\n\ @(#)All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|rcsid
index|[]
init|=
literal|"$Id: amq.c,v 5.2 90/06/23 22:20:07 jsp Rel $"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)amq.c	5.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|"am.h"
end_include

begin_include
include|#
directive|include
file|"amq.h"
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_decl_stmt
name|char
modifier|*
name|progname
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|flush_flag
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|minfo_flag
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|unmount_flag
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|stats_flag
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|debug_opts
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|logfile
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|xlog_opt
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
name|localhost
index|[]
init|=
literal|"localhost"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|def_server
init|=
name|localhost
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|int
name|optind
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|char
modifier|*
name|optarg
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|timeval
name|tmo
init|=
block|{
literal|10
block|,
literal|0
block|}
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|TIMEOUT
value|tmo
end_define

begin_enum
enum|enum
name|show_opt
block|{
name|Full
block|,
name|Stats
block|,
name|Calc
block|,
name|Short
block|,
name|ShowDone
block|}
enum|;
end_enum

begin_comment
comment|/*  * If (e) is Calc then just calculate the sizes  * Otherwise display the mount node on stdout  */
end_comment

begin_function
specifier|static
name|void
name|show_mti
parameter_list|(
name|mt
parameter_list|,
name|e
parameter_list|,
name|mwid
parameter_list|,
name|dwid
parameter_list|,
name|twid
parameter_list|)
name|amq_mount_tree
modifier|*
name|mt
decl_stmt|;
name|enum
name|show_opt
name|e
decl_stmt|;
name|int
modifier|*
name|mwid
decl_stmt|;
name|int
modifier|*
name|dwid
decl_stmt|;
name|int
modifier|*
name|twid
decl_stmt|;
block|{
switch|switch
condition|(
name|e
condition|)
block|{
case|case
name|Calc
case|:
block|{
name|int
name|mw
init|=
name|strlen
argument_list|(
name|mt
operator|->
name|mt_mountinfo
argument_list|)
decl_stmt|;
name|int
name|dw
init|=
name|strlen
argument_list|(
name|mt
operator|->
name|mt_directory
argument_list|)
decl_stmt|;
name|int
name|tw
init|=
name|strlen
argument_list|(
name|mt
operator|->
name|mt_type
argument_list|)
decl_stmt|;
if|if
condition|(
name|mw
operator|>
operator|*
name|mwid
condition|)
operator|*
name|mwid
operator|=
name|mw
expr_stmt|;
if|if
condition|(
name|dw
operator|>
operator|*
name|dwid
condition|)
operator|*
name|dwid
operator|=
name|dw
expr_stmt|;
if|if
condition|(
name|tw
operator|>
operator|*
name|twid
condition|)
operator|*
name|twid
operator|=
name|tw
expr_stmt|;
block|}
break|break;
case|case
name|Full
case|:
block|{
name|struct
name|tm
modifier|*
name|tp
init|=
name|localtime
argument_list|(
operator|&
name|mt
operator|->
name|mt_mounttime
argument_list|)
decl_stmt|;
name|printf
argument_list|(
literal|"%-*.*s %-*.*s %-*.*s %s\n\t%-5d %-7d %-6d %-7d %-7d %-6d %02d/%02d/%02d %02d:%02d:%02d\n"
argument_list|,
operator|*
name|dwid
argument_list|,
operator|*
name|dwid
argument_list|,
operator|*
name|mt
operator|->
name|mt_directory
condition|?
name|mt
operator|->
name|mt_directory
else|:
literal|"/"
argument_list|,
comment|/* XXX */
operator|*
name|twid
argument_list|,
operator|*
name|twid
argument_list|,
name|mt
operator|->
name|mt_type
argument_list|,
operator|*
name|mwid
argument_list|,
operator|*
name|mwid
argument_list|,
name|mt
operator|->
name|mt_mountinfo
argument_list|,
name|mt
operator|->
name|mt_mountpoint
argument_list|,
name|mt
operator|->
name|mt_mountuid
argument_list|,
name|mt
operator|->
name|mt_getattr
argument_list|,
name|mt
operator|->
name|mt_lookup
argument_list|,
name|mt
operator|->
name|mt_readdir
argument_list|,
name|mt
operator|->
name|mt_readlink
argument_list|,
name|mt
operator|->
name|mt_statfs
argument_list|,
name|tp
operator|->
name|tm_year
operator|>
literal|99
condition|?
name|tp
operator|->
name|tm_year
operator|-
literal|100
else|:
name|tp
operator|->
name|tm_year
argument_list|,
name|tp
operator|->
name|tm_mon
operator|+
literal|1
argument_list|,
name|tp
operator|->
name|tm_mday
argument_list|,
name|tp
operator|->
name|tm_hour
argument_list|,
name|tp
operator|->
name|tm_min
argument_list|,
name|tp
operator|->
name|tm_sec
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|Stats
case|:
block|{
name|struct
name|tm
modifier|*
name|tp
init|=
name|localtime
argument_list|(
operator|&
name|mt
operator|->
name|mt_mounttime
argument_list|)
decl_stmt|;
name|printf
argument_list|(
literal|"%-*.*s %-5d %-7d %-6d %-7d %-7d %-6d %02d/%02d/%02d %02d:%02d:%02d\n"
argument_list|,
operator|*
name|dwid
argument_list|,
operator|*
name|dwid
argument_list|,
operator|*
name|mt
operator|->
name|mt_directory
condition|?
name|mt
operator|->
name|mt_directory
else|:
literal|"/"
argument_list|,
comment|/* XXX */
name|mt
operator|->
name|mt_mountuid
argument_list|,
name|mt
operator|->
name|mt_getattr
argument_list|,
name|mt
operator|->
name|mt_lookup
argument_list|,
name|mt
operator|->
name|mt_readdir
argument_list|,
name|mt
operator|->
name|mt_readlink
argument_list|,
name|mt
operator|->
name|mt_statfs
argument_list|,
name|tp
operator|->
name|tm_year
operator|>
literal|99
condition|?
name|tp
operator|->
name|tm_year
operator|-
literal|100
else|:
name|tp
operator|->
name|tm_year
argument_list|,
name|tp
operator|->
name|tm_mon
operator|+
literal|1
argument_list|,
name|tp
operator|->
name|tm_mday
argument_list|,
name|tp
operator|->
name|tm_hour
argument_list|,
name|tp
operator|->
name|tm_min
argument_list|,
name|tp
operator|->
name|tm_sec
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|Short
case|:
block|{
name|printf
argument_list|(
literal|"%-*.*s %-*.*s %-*.*s %s\n"
argument_list|,
operator|*
name|dwid
argument_list|,
operator|*
name|dwid
argument_list|,
operator|*
name|mt
operator|->
name|mt_directory
condition|?
name|mt
operator|->
name|mt_directory
else|:
literal|"/"
argument_list|,
operator|*
name|twid
argument_list|,
operator|*
name|twid
argument_list|,
name|mt
operator|->
name|mt_type
argument_list|,
operator|*
name|mwid
argument_list|,
operator|*
name|mwid
argument_list|,
name|mt
operator|->
name|mt_mountinfo
argument_list|,
name|mt
operator|->
name|mt_mountpoint
argument_list|)
expr_stmt|;
block|}
break|break;
block|}
block|}
end_function

begin_comment
comment|/*  * Display a mount tree.  */
end_comment

begin_function
specifier|static
name|void
name|show_mt
parameter_list|(
name|mt
parameter_list|,
name|e
parameter_list|,
name|mwid
parameter_list|,
name|dwid
parameter_list|,
name|pwid
parameter_list|)
name|amq_mount_tree
modifier|*
name|mt
decl_stmt|;
name|enum
name|show_opt
name|e
decl_stmt|;
name|int
modifier|*
name|mwid
decl_stmt|;
name|int
modifier|*
name|dwid
decl_stmt|;
name|int
modifier|*
name|pwid
decl_stmt|;
block|{
while|while
condition|(
name|mt
condition|)
block|{
name|show_mti
argument_list|(
name|mt
argument_list|,
name|e
argument_list|,
name|mwid
argument_list|,
name|dwid
argument_list|,
name|pwid
argument_list|)
expr_stmt|;
name|show_mt
argument_list|(
name|mt
operator|->
name|mt_next
argument_list|,
name|e
argument_list|,
name|mwid
argument_list|,
name|dwid
argument_list|,
name|pwid
argument_list|)
expr_stmt|;
name|mt
operator|=
name|mt
operator|->
name|mt_child
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|show_mi
parameter_list|(
name|ml
parameter_list|,
name|e
parameter_list|,
name|mwid
parameter_list|,
name|dwid
parameter_list|,
name|twid
parameter_list|)
name|amq_mount_info_list
modifier|*
name|ml
decl_stmt|;
name|enum
name|show_opt
name|e
decl_stmt|;
name|int
modifier|*
name|mwid
decl_stmt|;
name|int
modifier|*
name|dwid
decl_stmt|;
name|int
modifier|*
name|twid
decl_stmt|;
block|{
name|int
name|i
decl_stmt|;
switch|switch
condition|(
name|e
condition|)
block|{
case|case
name|Calc
case|:
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|ml
operator|->
name|amq_mount_info_list_len
condition|;
name|i
operator|++
control|)
block|{
name|amq_mount_info
modifier|*
name|mi
init|=
operator|&
name|ml
operator|->
name|amq_mount_info_list_val
index|[
name|i
index|]
decl_stmt|;
name|int
name|mw
init|=
name|strlen
argument_list|(
name|mi
operator|->
name|mi_mountinfo
argument_list|)
decl_stmt|;
name|int
name|dw
init|=
name|strlen
argument_list|(
name|mi
operator|->
name|mi_mountpt
argument_list|)
decl_stmt|;
name|int
name|tw
init|=
name|strlen
argument_list|(
name|mi
operator|->
name|mi_type
argument_list|)
decl_stmt|;
if|if
condition|(
name|mw
operator|>
operator|*
name|mwid
condition|)
operator|*
name|mwid
operator|=
name|mw
expr_stmt|;
if|if
condition|(
name|dw
operator|>
operator|*
name|dwid
condition|)
operator|*
name|dwid
operator|=
name|dw
expr_stmt|;
if|if
condition|(
name|tw
operator|>
operator|*
name|twid
condition|)
operator|*
name|twid
operator|=
name|tw
expr_stmt|;
block|}
block|}
break|break;
case|case
name|Full
case|:
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|ml
operator|->
name|amq_mount_info_list_len
condition|;
name|i
operator|++
control|)
block|{
name|amq_mount_info
modifier|*
name|mi
init|=
operator|&
name|ml
operator|->
name|amq_mount_info_list_val
index|[
name|i
index|]
decl_stmt|;
name|printf
argument_list|(
literal|"%-*.*s %-*.*s %-*.*s %-3d %s is %s"
argument_list|,
operator|*
name|mwid
argument_list|,
operator|*
name|mwid
argument_list|,
name|mi
operator|->
name|mi_mountinfo
argument_list|,
operator|*
name|dwid
argument_list|,
operator|*
name|dwid
argument_list|,
name|mi
operator|->
name|mi_mountpt
argument_list|,
operator|*
name|twid
argument_list|,
operator|*
name|twid
argument_list|,
name|mi
operator|->
name|mi_type
argument_list|,
name|mi
operator|->
name|mi_refc
argument_list|,
name|mi
operator|->
name|mi_fserver
argument_list|,
name|mi
operator|->
name|mi_up
operator|>
literal|0
condition|?
literal|"up"
else|:
name|mi
operator|->
name|mi_up
operator|<
literal|0
condition|?
literal|"starting"
else|:
literal|"down"
argument_list|)
expr_stmt|;
if|if
condition|(
name|mi
operator|->
name|mi_error
operator|>
literal|0
condition|)
block|{
specifier|extern
name|char
modifier|*
name|sys_errlist
index|[]
decl_stmt|;
specifier|extern
name|int
name|sys_nerr
decl_stmt|;
if|if
condition|(
name|mi
operator|->
name|mi_error
operator|<
name|sys_nerr
condition|)
name|printf
argument_list|(
literal|" (%s)"
argument_list|,
name|sys_errlist
index|[
name|mi
operator|->
name|mi_error
index|]
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|" (Error %d)"
argument_list|,
name|mi
operator|->
name|mi_error
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|mi
operator|->
name|mi_error
operator|<
literal|0
condition|)
block|{
name|fputs
argument_list|(
literal|" (in progress)"
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
block|}
name|fputc
argument_list|(
literal|'\n'
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
block|}
block|}
break|break;
block|}
block|}
end_function

begin_comment
comment|/*  * Display general mount statistics  */
end_comment

begin_function
specifier|static
name|void
name|show_ms
parameter_list|(
name|ms
parameter_list|)
name|amq_mount_stats
modifier|*
name|ms
decl_stmt|;
block|{
name|printf
argument_list|(
literal|"\ requests  stale     mount     mount     unmount\n\ deferred  fhandles  ok        failed    failed\n\ %-9d %-9d %-9d %-9d %-9d\n"
argument_list|,
name|ms
operator|->
name|as_drops
argument_list|,
name|ms
operator|->
name|as_stale
argument_list|,
name|ms
operator|->
name|as_mok
argument_list|,
name|ms
operator|->
name|as_merr
argument_list|,
name|ms
operator|->
name|as_uerr
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|bool_t
name|xdr_pri_free
parameter_list|(
name|xdr_args
parameter_list|,
name|args_ptr
parameter_list|)
name|xdrproc_t
name|xdr_args
decl_stmt|;
name|caddr_t
name|args_ptr
decl_stmt|;
block|{
name|XDR
name|xdr
decl_stmt|;
name|xdr
operator|.
name|x_op
operator|=
name|XDR_FREE
expr_stmt|;
return|return
operator|(
call|(
modifier|*
name|xdr_args
call|)
argument_list|(
operator|&
name|xdr
argument_list|,
name|args_ptr
argument_list|)
operator|)
return|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|hpux
end_ifdef

begin_include
include|#
directive|include
file|<cluster.h>
end_include

begin_function
specifier|static
name|char
modifier|*
name|cluster_server
parameter_list|()
block|{
name|struct
name|cct_entry
modifier|*
name|cp
decl_stmt|;
if|if
condition|(
name|cnodeid
argument_list|()
operator|==
literal|0
condition|)
block|{
comment|/* 		 * Not clustered 		 */
return|return
name|def_server
return|;
block|}
while|while
condition|(
name|cp
operator|=
name|getccent
argument_list|()
condition|)
if|if
condition|(
name|cp
operator|->
name|cnode_type
operator|==
literal|'r'
condition|)
return|return
name|cp
operator|->
name|cnode_name
return|;
return|return
name|def_server
return|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* hpux */
end_comment

begin_comment
comment|/*  * MAIN  */
end_comment

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
name|int
name|opt_ch
decl_stmt|;
name|int
name|errs
init|=
literal|0
decl_stmt|;
name|char
modifier|*
name|server
decl_stmt|;
name|struct
name|sockaddr_in
name|server_addr
decl_stmt|;
name|int
name|s
init|=
name|RPC_ANYSOCK
decl_stmt|;
name|CLIENT
modifier|*
name|clnt
decl_stmt|;
name|struct
name|hostent
modifier|*
name|hp
decl_stmt|;
name|int
name|nodefault
init|=
literal|0
decl_stmt|;
comment|/* 	 * Compute program name 	 */
if|if
condition|(
name|argv
index|[
literal|0
index|]
condition|)
block|{
name|progname
operator|=
name|strrchr
argument_list|(
name|argv
index|[
literal|0
index|]
argument_list|,
literal|'/'
argument_list|)
expr_stmt|;
if|if
condition|(
name|progname
operator|&&
name|progname
index|[
literal|1
index|]
condition|)
name|progname
operator|++
expr_stmt|;
else|else
name|progname
operator|=
name|argv
index|[
literal|0
index|]
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|progname
condition|)
name|progname
operator|=
literal|"amq"
expr_stmt|;
comment|/* 	 * Parse arguments 	 */
while|while
condition|(
operator|(
name|opt_ch
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"fh:l:msux:D:"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|opt_ch
condition|)
block|{
case|case
literal|'f'
case|:
name|flush_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'h'
case|:
name|def_server
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'l'
case|:
name|logfile
operator|=
name|optarg
expr_stmt|;
name|nodefault
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'m'
case|:
name|minfo_flag
operator|=
literal|1
expr_stmt|;
name|nodefault
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'s'
case|:
name|stats_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'u'
case|:
name|unmount_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'x'
case|:
name|xlog_opt
operator|=
name|optarg
expr_stmt|;
name|nodefault
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'D'
case|:
name|debug_opts
operator|=
name|optarg
expr_stmt|;
name|nodefault
operator|=
literal|1
expr_stmt|;
break|break;
default|default:
name|errs
operator|=
literal|1
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|errs
condition|)
block|{
name|show_usage
label|:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\ Usage: %s [-h host] [[-f] [-m] | | [-s] | [[-u] directory ...]] |\n\ \t[-l logfile|\"syslog\"] [-x log_flags] [-D dbg_opts]\n"
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|hpux
comment|/* 	 * Figure out root server of cluster 	 */
if|if
condition|(
name|def_server
operator|==
name|localhost
condition|)
name|server
operator|=
name|cluster_server
argument_list|()
expr_stmt|;
else|else
endif|#
directive|endif
comment|/* hpux */
name|server
operator|=
name|def_server
expr_stmt|;
comment|/* 	 * Get address of server 	 */
if|if
condition|(
operator|(
name|hp
operator|=
name|gethostbyname
argument_list|(
name|server
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: Can't get address of %s\n"
argument_list|,
name|progname
argument_list|,
name|server
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|bzero
argument_list|(
operator|&
name|server_addr
argument_list|,
sizeof|sizeof
name|server_addr
argument_list|)
expr_stmt|;
name|server_addr
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|server_addr
operator|.
name|sin_addr
operator|=
operator|*
operator|(
expr|struct
name|in_addr
operator|*
operator|)
name|hp
operator|->
name|h_addr
expr_stmt|;
comment|/* 	 * Create RPC endpoint 	 */
name|clnt
operator|=
name|clntudp_create
argument_list|(
operator|&
name|server_addr
argument_list|,
name|AMQ_PROGRAM
argument_list|,
name|AMQ_VERSION
argument_list|,
name|TIMEOUT
argument_list|,
operator|&
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
name|clnt
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: "
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|clnt_pcreateerror
argument_list|(
name|server
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Control debugging 	 */
if|if
condition|(
name|debug_opts
condition|)
block|{
name|int
modifier|*
name|rc
decl_stmt|;
name|amq_setopt
name|opt
decl_stmt|;
name|opt
operator|.
name|as_opt
operator|=
name|AMOPT_DEBUG
expr_stmt|;
name|opt
operator|.
name|as_str
operator|=
name|debug_opts
expr_stmt|;
name|rc
operator|=
name|amqproc_setopt_1
argument_list|(
operator|&
name|opt
argument_list|,
name|clnt
argument_list|)
expr_stmt|;
if|if
condition|(
name|rc
operator|&&
operator|*
name|rc
operator|<
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: daemon not compiled for debug"
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|rc
operator|||
operator|*
name|rc
operator|>
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: debug setting for \"%s\" failed\n"
argument_list|,
name|progname
argument_list|,
name|debug_opts
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
block|}
comment|/* 	 * Control logging 	 */
if|if
condition|(
name|xlog_opt
condition|)
block|{
name|int
modifier|*
name|rc
decl_stmt|;
name|amq_setopt
name|opt
decl_stmt|;
name|opt
operator|.
name|as_opt
operator|=
name|AMOPT_XLOG
expr_stmt|;
name|opt
operator|.
name|as_str
operator|=
name|xlog_opt
expr_stmt|;
name|rc
operator|=
name|amqproc_setopt_1
argument_list|(
operator|&
name|opt
argument_list|,
name|clnt
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|rc
operator|||
operator|*
name|rc
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: setting log level to \"%s\" failed\n"
argument_list|,
name|progname
argument_list|,
name|xlog_opt
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
block|}
comment|/* 	 * Control log file 	 */
if|if
condition|(
name|logfile
condition|)
block|{
name|int
modifier|*
name|rc
decl_stmt|;
name|amq_setopt
name|opt
decl_stmt|;
name|opt
operator|.
name|as_opt
operator|=
name|AMOPT_LOGFILE
expr_stmt|;
name|opt
operator|.
name|as_str
operator|=
name|logfile
expr_stmt|;
name|rc
operator|=
name|amqproc_setopt_1
argument_list|(
operator|&
name|opt
argument_list|,
name|clnt
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|rc
operator|||
operator|*
name|rc
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: setting logfile to \"%s\" failed\n"
argument_list|,
name|progname
argument_list|,
name|logfile
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
block|}
comment|/* 	 * Flush map cache 	 */
if|if
condition|(
name|logfile
condition|)
block|{
name|int
modifier|*
name|rc
decl_stmt|;
name|amq_setopt
name|opt
decl_stmt|;
name|opt
operator|.
name|as_opt
operator|=
name|AMOPT_FLUSHMAPC
expr_stmt|;
name|opt
operator|.
name|as_str
operator|=
literal|""
expr_stmt|;
name|rc
operator|=
name|amqproc_setopt_1
argument_list|(
operator|&
name|opt
argument_list|,
name|clnt
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|rc
operator|||
operator|*
name|rc
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: amd on %s cannot flush the map cache\n"
argument_list|,
name|progname
argument_list|,
name|server
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
block|}
comment|/* 	 * Mount info 	 */
if|if
condition|(
name|minfo_flag
condition|)
block|{
name|int
name|dummy
decl_stmt|;
name|amq_mount_info_list
modifier|*
name|ml
init|=
name|amqproc_getmntfs_1
argument_list|(
operator|&
name|dummy
argument_list|,
name|clnt
argument_list|)
decl_stmt|;
if|if
condition|(
name|ml
condition|)
block|{
name|int
name|mwid
init|=
literal|0
decl_stmt|,
name|dwid
init|=
literal|0
decl_stmt|,
name|twid
init|=
literal|0
decl_stmt|;
name|show_mi
argument_list|(
name|ml
argument_list|,
name|Calc
argument_list|,
operator|&
name|mwid
argument_list|,
operator|&
name|dwid
argument_list|,
operator|&
name|twid
argument_list|)
expr_stmt|;
name|mwid
operator|++
expr_stmt|;
name|dwid
operator|++
expr_stmt|;
name|twid
operator|++
expr_stmt|;
name|show_mi
argument_list|(
name|ml
argument_list|,
name|Full
argument_list|,
operator|&
name|mwid
argument_list|,
operator|&
name|dwid
argument_list|,
operator|&
name|twid
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: amd on %s cannot provide mount info\n"
argument_list|,
name|progname
argument_list|,
name|server
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* 	 * Apply required operation to all remaining arguments 	 */
if|if
condition|(
name|optind
operator|<
name|argc
condition|)
block|{
do|do
block|{
name|char
modifier|*
name|fs
init|=
name|argv
index|[
name|optind
operator|++
index|]
decl_stmt|;
if|if
condition|(
name|unmount_flag
condition|)
block|{
comment|/* 				 * Unmount request 				 */
name|amqproc_umnt_1
argument_list|(
operator|&
name|fs
argument_list|,
name|clnt
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/* 				 * Stats request 				 */
name|amq_mount_tree_p
modifier|*
name|mtp
init|=
name|amqproc_mnttree_1
argument_list|(
operator|&
name|fs
argument_list|,
name|clnt
argument_list|)
decl_stmt|;
if|if
condition|(
name|mtp
condition|)
block|{
name|amq_mount_tree
modifier|*
name|mt
init|=
operator|*
name|mtp
decl_stmt|;
if|if
condition|(
name|mt
condition|)
block|{
name|int
name|mwid
init|=
literal|0
decl_stmt|,
name|dwid
init|=
literal|0
decl_stmt|,
name|twid
init|=
literal|0
decl_stmt|;
name|show_mt
argument_list|(
name|mt
argument_list|,
name|Calc
argument_list|,
operator|&
name|mwid
argument_list|,
operator|&
name|dwid
argument_list|,
operator|&
name|twid
argument_list|)
expr_stmt|;
name|mwid
operator|++
expr_stmt|;
name|dwid
operator|++
operator|,
name|twid
operator|++
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
name|printf
argument_list|(
literal|"\t%s\n%-*.*s %-*.*s %-*.*s %s\n"
argument_list|,
literal|"Uid   Getattr Lookup RdDir   RdLnk   Statfs Mounted@"
argument_list|,
name|dwid
argument_list|,
name|dwid
argument_list|,
literal|"What"
argument_list|,
name|twid
argument_list|,
name|twid
argument_list|,
literal|"Type"
argument_list|,
name|mwid
argument_list|,
name|mwid
argument_list|,
literal|"Info"
argument_list|,
literal|"Where"
argument_list|)
expr_stmt|;
name|show_mt
argument_list|(
name|mt
argument_list|,
name|Full
argument_list|,
operator|&
name|mwid
argument_list|,
operator|&
name|dwid
argument_list|,
operator|&
name|twid
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* notdef */
name|printf
argument_list|(
literal|"%-*.*s Uid   Getattr Lookup RdDir   RdLnk   Statfs Mounted@\n"
argument_list|,
name|dwid
argument_list|,
name|dwid
argument_list|,
literal|"What"
argument_list|)
expr_stmt|;
name|show_mt
argument_list|(
name|mt
argument_list|,
name|Stats
argument_list|,
operator|&
name|mwid
argument_list|,
operator|&
name|dwid
argument_list|,
operator|&
name|twid
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s not automounted\n"
argument_list|,
name|progname
argument_list|,
name|fs
argument_list|)
expr_stmt|;
block|}
name|xdr_pri_free
argument_list|(
name|xdr_amq_mount_tree_p
argument_list|,
operator|(
name|caddr_t
operator|)
name|mtp
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: "
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|clnt_perror
argument_list|(
name|clnt
argument_list|,
name|server
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
block|}
block|}
do|while
condition|(
name|optind
operator|<
name|argc
condition|)
do|;
block|}
elseif|else
if|if
condition|(
name|unmount_flag
condition|)
block|{
goto|goto
name|show_usage
goto|;
block|}
elseif|else
if|if
condition|(
name|stats_flag
condition|)
block|{
name|amq_mount_stats
modifier|*
name|ms
init|=
name|amqproc_stats_1
argument_list|(
operator|(
name|voidp
operator|)
literal|0
argument_list|,
name|clnt
argument_list|)
decl_stmt|;
if|if
condition|(
name|ms
condition|)
block|{
name|show_ms
argument_list|(
name|ms
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: "
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|clnt_perror
argument_list|(
name|clnt
argument_list|,
name|server
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|nodefault
condition|)
block|{
name|amq_mount_tree_list
modifier|*
name|mlp
init|=
name|amqproc_export_1
argument_list|(
operator|(
name|voidp
operator|)
literal|0
argument_list|,
name|clnt
argument_list|)
decl_stmt|;
if|if
condition|(
name|mlp
condition|)
block|{
name|enum
name|show_opt
name|e
init|=
name|Calc
decl_stmt|;
name|int
name|mwid
init|=
literal|0
decl_stmt|,
name|dwid
init|=
literal|0
decl_stmt|,
name|pwid
init|=
literal|0
decl_stmt|;
while|while
condition|(
name|e
operator|!=
name|ShowDone
condition|)
block|{
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|mlp
operator|->
name|amq_mount_tree_list_len
condition|;
name|i
operator|++
control|)
block|{
name|show_mt
argument_list|(
name|mlp
operator|->
name|amq_mount_tree_list_val
index|[
name|i
index|]
argument_list|,
name|e
argument_list|,
operator|&
name|mwid
argument_list|,
operator|&
name|dwid
argument_list|,
operator|&
name|pwid
argument_list|)
expr_stmt|;
block|}
name|mwid
operator|++
expr_stmt|;
name|dwid
operator|++
operator|,
name|pwid
operator|++
expr_stmt|;
if|if
condition|(
name|e
operator|==
name|Calc
condition|)
name|e
operator|=
name|Short
expr_stmt|;
elseif|else
if|if
condition|(
name|e
operator|==
name|Short
condition|)
name|e
operator|=
name|ShowDone
expr_stmt|;
block|}
block|}
else|else
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: "
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|clnt_perror
argument_list|(
name|clnt
argument_list|,
name|server
argument_list|)
expr_stmt|;
name|errs
operator|=
literal|1
expr_stmt|;
block|}
block|}
name|exit
argument_list|(
name|errs
argument_list|)
expr_stmt|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|DEBUG
end_ifdef

begin_macro
name|xfree
argument_list|(
argument|f
argument_list|,
argument|l
argument_list|,
argument|p
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|f
decl_stmt|,
modifier|*
name|l
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|voidp
name|p
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|free
argument_list|(
name|p
argument_list|)
expr_stmt|;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* DEBUG */
end_comment

end_unit

