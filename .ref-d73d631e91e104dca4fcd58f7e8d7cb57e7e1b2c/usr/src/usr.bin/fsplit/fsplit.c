begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1983 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Asa Romberger and Jerry Berkman.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1983 The Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)fsplit.c	5.4 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_comment
comment|/*  *	usage:		fsplit [-e efile] ... [file]  *  *	split single file containing source for several fortran programs  *		and/or subprograms into files each containing one  *		subprogram unit.  *	each separate file will be named using the corresponding subroutine,  *		function, block data or program name if one is found; otherwise  *		the name will be of the form mainNNN.f or blkdtaNNN.f .  *		If a file of that name exists, it is saved in a name of the  *		form zzz000.f .  *	If -e option is used, then only those subprograms named in the -e  *		option are split off; e.g.:  *			fsplit -esub1 -e sub2 prog.f  *		isolates sub1 and sub2 in sub1.f and sub2.f.  The space   *		after -e is optional.  *  *	Modified Feb., 1983 by Jerry Berkman, Computing Services, U.C. Berkeley.  *		- added comments  *		- more function types: double complex, character*(*), etc.  *		- fixed minor bugs  *		- instead of all unnamed going into zNNN.f, put mains in  *		  mainNNN.f, block datas in blkdtaNNN.f, dups in zzzNNN.f .  */
end_comment

begin_define
define|#
directive|define
name|BSZ
value|512
end_define

begin_decl_stmt
name|char
name|buf
index|[
name|BSZ
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|FILE
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|x
index|[]
init|=
literal|"zzz000.f"
decl_stmt|,
name|mainp
index|[]
init|=
literal|"main000.f"
decl_stmt|,
name|blkp
index|[]
init|=
literal|"blkdta000.f"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|look
argument_list|()
decl_stmt|,
modifier|*
name|skiplab
argument_list|()
decl_stmt|,
modifier|*
name|functs
argument_list|()
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|TRUE
value|1
end_define

begin_define
define|#
directive|define
name|FALSE
value|0
end_define

begin_decl_stmt
name|int
name|extr
init|=
name|FALSE
decl_stmt|,
name|extrknt
init|=
operator|-
literal|1
decl_stmt|,
name|extrfnd
index|[
literal|100
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|extrbuf
index|[
literal|1000
index|]
decl_stmt|,
modifier|*
name|extrnames
index|[
literal|100
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|stat
name|sbuf
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|trim
parameter_list|(
name|p
parameter_list|)
value|while (*p == ' ' || *p == '\t') p++
end_define

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
block|{
specifier|register
name|FILE
modifier|*
name|ofp
decl_stmt|;
comment|/* output file */
specifier|register
name|rv
expr_stmt|;
comment|/* 1 if got card in output file, 0 otherwise */
specifier|register
name|char
modifier|*
name|ptr
decl_stmt|;
name|int
name|nflag
decl_stmt|,
comment|/* 1 if got name of subprog., 0 otherwise */
name|retval
decl_stmt|,
name|i
decl_stmt|;
name|char
name|name
index|[
literal|20
index|]
decl_stmt|,
modifier|*
name|extrptr
init|=
name|extrbuf
decl_stmt|;
comment|/*  scan -e options */
while|while
condition|(
name|argc
operator|>
literal|1
operator|&&
name|argv
index|[
literal|1
index|]
index|[
literal|0
index|]
operator|==
literal|'-'
operator|&&
name|argv
index|[
literal|1
index|]
index|[
literal|1
index|]
operator|==
literal|'e'
condition|)
block|{
name|extr
operator|=
name|TRUE
expr_stmt|;
name|ptr
operator|=
name|argv
index|[
literal|1
index|]
operator|+
literal|2
expr_stmt|;
if|if
condition|(
operator|!
operator|*
name|ptr
condition|)
block|{
name|argc
operator|--
expr_stmt|;
name|argv
operator|++
expr_stmt|;
if|if
condition|(
name|argc
operator|<=
literal|1
condition|)
name|badparms
argument_list|()
expr_stmt|;
name|ptr
operator|=
name|argv
index|[
literal|1
index|]
expr_stmt|;
block|}
name|extrknt
operator|=
name|extrknt
operator|+
literal|1
expr_stmt|;
name|extrnames
index|[
name|extrknt
index|]
operator|=
name|extrptr
expr_stmt|;
name|extrfnd
index|[
name|extrknt
index|]
operator|=
name|FALSE
expr_stmt|;
while|while
condition|(
operator|*
name|ptr
condition|)
operator|*
name|extrptr
operator|++
operator|=
operator|*
name|ptr
operator|++
expr_stmt|;
operator|*
name|extrptr
operator|++
operator|=
literal|0
expr_stmt|;
name|argc
operator|--
expr_stmt|;
name|argv
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|argc
operator|>
literal|2
condition|)
name|badparms
argument_list|()
expr_stmt|;
elseif|else
if|if
condition|(
name|argc
operator|==
literal|2
condition|)
block|{
if|if
condition|(
operator|(
name|ifp
operator|=
name|fopen
argument_list|(
name|argv
index|[
literal|1
index|]
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"fsplit: cannot open %s\n"
argument_list|,
name|argv
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
else|else
name|ifp
operator|=
name|stdin
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
comment|/* look for a temp file that doesn't correspond to an existing file */
name|get_name
argument_list|(
name|x
argument_list|,
literal|3
argument_list|)
expr_stmt|;
name|ofp
operator|=
name|fopen
argument_list|(
name|x
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
name|nflag
operator|=
literal|0
expr_stmt|;
name|rv
operator|=
literal|0
expr_stmt|;
while|while
condition|(
name|getline
argument_list|()
operator|>
literal|0
condition|)
block|{
name|rv
operator|=
literal|1
expr_stmt|;
name|fprintf
argument_list|(
name|ofp
argument_list|,
literal|"%s"
argument_list|,
name|buf
argument_list|)
expr_stmt|;
if|if
condition|(
name|lend
argument_list|()
condition|)
comment|/* look for an 'end' statement */
break|break;
if|if
condition|(
name|nflag
operator|==
literal|0
condition|)
comment|/* if no name yet, try and find one */
name|nflag
operator|=
name|lname
argument_list|(
name|name
argument_list|)
expr_stmt|;
block|}
name|fclose
argument_list|(
name|ofp
argument_list|)
expr_stmt|;
if|if
condition|(
name|rv
operator|==
literal|0
condition|)
block|{
comment|/* no lines in file, forget the file */
name|unlink
argument_list|(
name|x
argument_list|)
expr_stmt|;
name|retval
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<=
name|extrknt
condition|;
name|i
operator|++
control|)
if|if
condition|(
operator|!
name|extrfnd
index|[
name|i
index|]
condition|)
block|{
name|retval
operator|=
literal|1
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"fsplit: %s not found\n"
argument_list|,
name|extrnames
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
name|retval
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nflag
condition|)
block|{
comment|/* rename the file */
if|if
condition|(
name|saveit
argument_list|(
name|name
argument_list|)
condition|)
block|{
if|if
condition|(
name|stat
argument_list|(
name|name
argument_list|,
operator|&
name|sbuf
argument_list|)
operator|<
literal|0
condition|)
block|{
name|link
argument_list|(
name|x
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|unlink
argument_list|(
name|x
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%s\n"
argument_list|,
name|name
argument_list|)
expr_stmt|;
continue|continue;
block|}
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|name
argument_list|,
name|x
argument_list|)
operator|==
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"%s\n"
argument_list|,
name|x
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|printf
argument_list|(
literal|"%s already exists, put in %s\n"
argument_list|,
name|name
argument_list|,
name|x
argument_list|)
expr_stmt|;
continue|continue;
block|}
else|else
name|unlink
argument_list|(
name|x
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
operator|!
name|extr
condition|)
name|printf
argument_list|(
literal|"%s\n"
argument_list|,
name|x
argument_list|)
expr_stmt|;
else|else
name|unlink
argument_list|(
name|x
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_macro
name|badparms
argument_list|()
end_macro

begin_block
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"fsplit: usage:  fsplit [-e efile] ... [file] \n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|saveit
argument_list|(
argument|name
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|name
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|i
decl_stmt|;
name|char
name|fname
index|[
literal|50
index|]
decl_stmt|,
modifier|*
name|fptr
init|=
name|fname
decl_stmt|;
if|if
condition|(
operator|!
name|extr
condition|)
return|return
operator|(
literal|1
operator|)
return|;
while|while
condition|(
operator|*
name|name
condition|)
operator|*
name|fptr
operator|++
operator|=
operator|*
name|name
operator|++
expr_stmt|;
operator|*
operator|--
name|fptr
operator|=
literal|0
expr_stmt|;
operator|*
operator|--
name|fptr
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<=
name|extrknt
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|strcmp
argument_list|(
name|fname
argument_list|,
name|extrnames
index|[
name|i
index|]
argument_list|)
operator|==
literal|0
condition|)
block|{
name|extrfnd
index|[
name|i
index|]
operator|=
name|TRUE
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_macro
name|get_name
argument_list|(
argument|name
argument_list|,
argument|letters
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|name
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|letters
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
modifier|*
name|ptr
decl_stmt|;
while|while
condition|(
name|stat
argument_list|(
name|name
argument_list|,
operator|&
name|sbuf
argument_list|)
operator|>=
literal|0
condition|)
block|{
for|for
control|(
name|ptr
operator|=
name|name
operator|+
name|letters
operator|+
literal|2
init|;
name|ptr
operator|>=
name|name
operator|+
name|letters
condition|;
name|ptr
operator|--
control|)
block|{
operator|(
operator|*
name|ptr
operator|)
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|ptr
operator|<=
literal|'9'
condition|)
break|break;
operator|*
name|ptr
operator|=
literal|'0'
expr_stmt|;
block|}
if|if
condition|(
name|ptr
operator|<
name|name
operator|+
name|letters
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"fsplit: ran out of file names\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_block

begin_macro
name|getline
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|char
modifier|*
name|ptr
decl_stmt|;
for|for
control|(
name|ptr
operator|=
name|buf
init|;
name|ptr
operator|<
operator|&
name|buf
index|[
name|BSZ
index|]
condition|;
control|)
block|{
operator|*
name|ptr
operator|=
name|getc
argument_list|(
name|ifp
argument_list|)
expr_stmt|;
if|if
condition|(
name|feof
argument_list|(
name|ifp
argument_list|)
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
if|if
condition|(
operator|*
name|ptr
operator|++
operator|==
literal|'\n'
condition|)
block|{
operator|*
name|ptr
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
block|}
while|while
condition|(
name|getc
argument_list|(
name|ifp
argument_list|)
operator|!=
literal|'\n'
operator|&&
name|feof
argument_list|(
name|ifp
argument_list|)
operator|==
literal|0
condition|)
empty_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"line truncated to %d characters\n"
argument_list|,
name|BSZ
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_comment
comment|/* return 1 for 'end' alone on card (up to col. 72),  0 otherwise */
end_comment

begin_macro
name|lend
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|char
modifier|*
name|p
decl_stmt|;
if|if
condition|(
operator|(
name|p
operator|=
name|skiplab
argument_list|(
name|buf
argument_list|)
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|trim
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|!=
literal|'e'
operator|&&
operator|*
name|p
operator|!=
literal|'E'
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|p
operator|++
expr_stmt|;
name|trim
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|!=
literal|'n'
operator|&&
operator|*
name|p
operator|!=
literal|'N'
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|p
operator|++
expr_stmt|;
name|trim
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|!=
literal|'d'
operator|&&
operator|*
name|p
operator|!=
literal|'D'
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|p
operator|++
expr_stmt|;
name|trim
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|-
name|buf
operator|>=
literal|72
operator|||
operator|*
name|p
operator|==
literal|'\n'
condition|)
return|return
operator|(
literal|1
operator|)
return|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*		check for keywords for subprograms	 		return 0 if comment card, 1 if found 		name and put in arg string. invent name for unnamed 		block datas and main programs.		*/
end_comment

begin_macro
name|lname
argument_list|(
argument|s
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|s
decl_stmt|;
end_decl_stmt

begin_block
block|{
define|#
directive|define
name|LINESIZE
value|80
specifier|register
name|char
modifier|*
name|ptr
decl_stmt|,
modifier|*
name|p
decl_stmt|,
modifier|*
name|sptr
decl_stmt|;
name|char
name|line
index|[
name|LINESIZE
index|]
decl_stmt|,
modifier|*
name|iptr
init|=
name|line
decl_stmt|;
comment|/* first check for comment cards */
if|if
condition|(
name|buf
index|[
literal|0
index|]
operator|==
literal|'c'
operator|||
name|buf
index|[
literal|0
index|]
operator|==
literal|'C'
operator|||
name|buf
index|[
literal|0
index|]
operator|==
literal|'*'
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|ptr
operator|=
name|buf
expr_stmt|;
while|while
condition|(
operator|*
name|ptr
operator|==
literal|' '
operator|||
operator|*
name|ptr
operator|==
literal|'\t'
condition|)
name|ptr
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|ptr
operator|==
literal|'\n'
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|ptr
operator|=
name|skiplab
argument_list|(
name|buf
argument_list|)
expr_stmt|;
comment|/*  copy to buffer and converting to lower case */
name|p
operator|=
name|ptr
expr_stmt|;
while|while
condition|(
operator|*
name|p
operator|&&
name|p
operator|<=
operator|&
name|buf
index|[
literal|71
index|]
condition|)
block|{
operator|*
name|iptr
operator|=
name|isupper
argument_list|(
operator|*
name|p
argument_list|)
condition|?
name|tolower
argument_list|(
operator|*
name|p
argument_list|)
else|:
operator|*
name|p
expr_stmt|;
name|iptr
operator|++
expr_stmt|;
name|p
operator|++
expr_stmt|;
block|}
operator|*
name|iptr
operator|=
literal|'\n'
expr_stmt|;
if|if
condition|(
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|line
argument_list|,
literal|"subroutine"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|line
argument_list|,
literal|"function"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|functs
argument_list|(
name|line
argument_list|)
operator|)
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|scan_name
argument_list|(
name|s
argument_list|,
name|ptr
argument_list|)
condition|)
return|return
operator|(
literal|1
operator|)
return|;
name|strcpy
argument_list|(
name|s
argument_list|,
name|x
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|line
argument_list|,
literal|"program"
argument_list|)
operator|)
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|scan_name
argument_list|(
name|s
argument_list|,
name|ptr
argument_list|)
condition|)
return|return
operator|(
literal|1
operator|)
return|;
name|get_name
argument_list|(
name|mainp
argument_list|,
literal|4
argument_list|)
expr_stmt|;
name|strcpy
argument_list|(
name|s
argument_list|,
name|mainp
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|line
argument_list|,
literal|"blockdata"
argument_list|)
operator|)
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|scan_name
argument_list|(
name|s
argument_list|,
name|ptr
argument_list|)
condition|)
return|return
operator|(
literal|1
operator|)
return|;
name|get_name
argument_list|(
name|blkp
argument_list|,
literal|6
argument_list|)
expr_stmt|;
name|strcpy
argument_list|(
name|s
argument_list|,
name|blkp
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|ptr
operator|=
name|functs
argument_list|(
name|line
argument_list|)
operator|)
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|scan_name
argument_list|(
name|s
argument_list|,
name|ptr
argument_list|)
condition|)
return|return
operator|(
literal|1
operator|)
return|;
name|strcpy
argument_list|(
name|s
argument_list|,
name|x
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|get_name
argument_list|(
name|mainp
argument_list|,
literal|4
argument_list|)
expr_stmt|;
name|strcpy
argument_list|(
name|s
argument_list|,
name|mainp
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_macro
name|scan_name
argument_list|(
argument|s
argument_list|,
argument|ptr
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|s
decl_stmt|,
modifier|*
name|ptr
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
modifier|*
name|sptr
decl_stmt|;
comment|/* scan off the name */
name|trim
argument_list|(
name|ptr
argument_list|)
expr_stmt|;
name|sptr
operator|=
name|s
expr_stmt|;
while|while
condition|(
operator|*
name|ptr
operator|!=
literal|'('
operator|&&
operator|*
name|ptr
operator|!=
literal|'\n'
condition|)
block|{
if|if
condition|(
operator|*
name|ptr
operator|!=
literal|' '
operator|&&
operator|*
name|ptr
operator|!=
literal|'\t'
condition|)
operator|*
name|sptr
operator|++
operator|=
operator|*
name|ptr
expr_stmt|;
name|ptr
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|sptr
operator|==
name|s
condition|)
return|return
operator|(
literal|0
operator|)
return|;
operator|*
name|sptr
operator|++
operator|=
literal|'.'
expr_stmt|;
operator|*
name|sptr
operator|++
operator|=
literal|'f'
expr_stmt|;
operator|*
name|sptr
operator|++
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_function
name|char
modifier|*
name|functs
parameter_list|(
name|p
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|ptr
decl_stmt|;
comment|/*      look for typed functions such as: real*8 function,                 character*16 function, character*(*) function  */
if|if
condition|(
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|p
argument_list|,
literal|"character"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|p
argument_list|,
literal|"logical"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|p
argument_list|,
literal|"real"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|p
argument_list|,
literal|"integer"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|p
argument_list|,
literal|"doubleprecision"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|p
argument_list|,
literal|"complex"
argument_list|)
operator|)
operator|!=
literal|0
operator|||
operator|(
name|ptr
operator|=
name|look
argument_list|(
name|p
argument_list|,
literal|"doublecomplex"
argument_list|)
operator|)
operator|!=
literal|0
condition|)
block|{
while|while
condition|(
operator|*
name|ptr
operator|==
literal|' '
operator|||
operator|*
name|ptr
operator|==
literal|'\t'
operator|||
operator|*
name|ptr
operator|==
literal|'*'
operator|||
operator|(
operator|*
name|ptr
operator|>=
literal|'0'
operator|&&
operator|*
name|ptr
operator|<=
literal|'9'
operator|)
operator|||
operator|*
name|ptr
operator|==
literal|'('
operator|||
operator|*
name|ptr
operator|==
literal|')'
condition|)
name|ptr
operator|++
expr_stmt|;
name|ptr
operator|=
name|look
argument_list|(
name|ptr
argument_list|,
literal|"function"
argument_list|)
expr_stmt|;
return|return
operator|(
name|ptr
operator|)
return|;
block|}
else|else
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/* 	if first 6 col. blank, return ptr to col. 7, 	if blanks and then tab, return ptr after tab, 	else return 0 (labelled statement, comment or continuation */
end_comment

begin_function
name|char
modifier|*
name|skiplab
parameter_list|(
name|p
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|ptr
decl_stmt|;
for|for
control|(
name|ptr
operator|=
name|p
init|;
name|ptr
operator|<
operator|&
name|p
index|[
literal|6
index|]
condition|;
name|ptr
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|ptr
operator|==
literal|' '
condition|)
continue|continue;
if|if
condition|(
operator|*
name|ptr
operator|==
literal|'\t'
condition|)
block|{
name|ptr
operator|++
expr_stmt|;
break|break;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
return|return
operator|(
name|ptr
operator|)
return|;
block|}
end_function

begin_comment
comment|/* 	return 0 if m doesn't match initial part of s; 	otherwise return ptr to next char after m in s */
end_comment

begin_function
name|char
modifier|*
name|look
parameter_list|(
name|s
parameter_list|,
name|m
parameter_list|)
name|char
modifier|*
name|s
decl_stmt|,
decl|*
name|m
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|char
modifier|*
name|sp
decl_stmt|,
modifier|*
name|mp
decl_stmt|;
name|sp
operator|=
name|s
expr_stmt|;
name|mp
operator|=
name|m
expr_stmt|;
while|while
condition|(
operator|*
name|mp
condition|)
block|{
name|trim
argument_list|(
name|sp
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|sp
operator|++
operator|!=
operator|*
name|mp
operator|++
condition|)
return|return
operator|(
literal|0
operator|)
return|;
block|}
return|return
operator|(
name|sp
operator|)
return|;
block|}
end_block

end_unit

