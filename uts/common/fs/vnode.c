...
begin_comment
comment|/*  * Copyright (c) 1988, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright 2017, Joyent, Inc.  * Copyright 2016 Nexenta Systems, Inc.  All rights reserved.  * Copyright (c) 2011, 2017 by Delphix. All rights reserved.  */
end_comment
...
begin_comment
comment|/* Global used for empty/invalid v_path */
end_comment
...
name|NULL
expr_stmt|;
comment|/* 	 * In a few specific instances, vn_reinit() is used to initialize 	 * locally defined vnode_t instances.  Lacking the construction offered 	 * by vn_alloc(), these vnodes require v_path initialization. 	 */
if|if
condition|(
name|vp
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * If the snapshot of v_path_stamp passed in via compare_stamp does not 	 * match the present value on the vnode, it indicates that subsequent 	 * changes have occurred.  The v_path value is not cleared in this case 	 * since the new value may be valid. 	 */
if|if
condition|(
name|compare_stamp
operator|!=
...
literal|0
decl_stmt|;
comment|/* Handle the vn_setpath_str case. */
if|if
condition|(
name|pvp
operator|==
...
return|return;
block|}
comment|/* Take snapshot of parent dir */
name|mutex_enter
argument_list|(
operator|&
name|pvp
...
condition|)
block|{
comment|/* 		 * Without v_path from the parent directory, generating a child 		 * path from the name is impossible. 		 */
if|if
condition|(
name|len
operator|>
...
return|return;
block|}
comment|/* 		 * The only feasible case here is where a NUL lookup is being 		 * performed on rootdir prior to its v_path being populated. 		 */
name|ASSERT
argument_list|(
name|pvp
operator|->
...
argument_list|)
expr_stmt|;
comment|/* ignore a trailing slash if present */
if|if
condition|(
name|pvp
operator|->
...
condition|)
block|{
comment|/* This should only the be case for rootdir */
name|ASSERT
argument_list|(
name|baselen
operator|==
...
condition|)
block|{
comment|/* Free the existing (mis-sized) buffer in case of retry */
name|kmem_free
argument_list|(
name|buf
argument_list|,
...
expr_stmt|;
block|}
comment|/* base, '/', name and trailing NUL */
name|buflen
operator|=
name|baselen
operator|+
...
name|vlen
decl_stmt|;
comment|/* 		 * Since v_path_stamp changed on the parent, it is likely that 		 * v_path has been altered as well.  If the length does not 		 * exactly match what was previously measured, the buffer 		 * allocation must be repeated for proper sizing. 		 */
if|if
condition|(
name|pvp
operator|->
...
condition|)
block|{
comment|/* Give up if parent lack v_path */
name|mutex_exit
argument_list|(
operator|&
name|pvp
...
condition|)
block|{
comment|/* never-visited vnode can inherit stamp from parent */
name|ASSERT
argument_list|(
name|vp
operator|->
...
condition|)
block|{
comment|/* 		 * Install the updated path and stamp, ensuring that the v_path 		 * pointer is valid at all times for dtrace. 		 */
name|oldbuf
operator|=
name|vp
operator|->
...
block|{
comment|/* 		 * If the timestamp matches or is greater, it means another 		 * thread performed the update first while locks were dropped 		 * here to make the allocation.  We defer to the newer value. 		 */
name|mutex_exit
argument_list|(
operator|&
name|vp
...
name|len
decl_stmt|;
comment|/* 	 * If the parent is older or empty, there's nothing further to do. 	 */
if|if
condition|(
name|pvp
operator|->
...
return|return;
block|}
comment|/* 	 * Given the lack of appropriate context, meaningful updates to v_path 	 * cannot be made for during lookups for the '.' or '..' entries. 	 */
name|len
operator|=
name|strlen
argument_list|(
