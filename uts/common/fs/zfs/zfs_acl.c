...
return|;
block|}
comment|/* 	 * close race where znode could be upgrade while trying to 	 * read the znode attributes. 	 * 	 * But this could only happen if the file isn't already an SA 	 * znode 	 */
if|if
condition|(
operator|!
name|zp
...
begin_comment
comment|/* See zfs_zaccess_delete() */
end_comment

begin_decl_stmt
name|int
name|zfs_write_implies_delete_child
init|=
literal|1
decl_stmt|;
end_decl_stmt
...
operator|)
return|;
comment|/* 	 * Case 2: 	 * If the containing directory grants ACE_DELETE_CHILD, 	 * or we're in backward compatibility mode and the 	 * containing directory has ACE_WRITE_DATA, allow. 	 * Case 2b is handled with wanted_dirperms. 	 */
name|wanted_dirperms
operator|=
name|ACE_DELETE_CHILD
expr_stmt|;
...
return|;
block|}
comment|/* 	 * Cases 2a, 2b (continued) 	 * 	 * Note: dzp_working_mode now contains any permissions 	 * that were NOT granted.  Therefore, if any of the 	 * wanted_dirperms WERE granted, we will have: 	 *   dzp_working_mode != wanted_dirperms 	 * We're really asking if ANY of those permissions 	 * were granted, and if so, grant delete access. 	 */
if|if
condition|(
name|dzp_working_mode
operator|!=
...
literal|0
expr_stmt|;
comment|/* 	 * dzp_error is 0 if the container granted us permissions to "modify". 	 * If we do not have permission via one or more ACEs, our current 	 * privileges may still permit us to modify the container. 	 * 	 * dzpcheck_privs is false when i.e. the FS is read-only. 	 * Otherwise, do privilege checks for the container. 	 */
if|if
condition|(
name|dzp_error
operator|!=
...
condition|)
block|{
comment|/* 		 * Note: We may have dzp_error = -1 here (from 		 * zfs_zacess_common).  Don't return that. 		 */
return|return
operator|(
name|SET_ERROR
argument_list|(
...
operator|)
return|;
comment|/* 	 * zfs_sticky_remove_access will succeed if: 	 * 1. The sticky bit is absent. 	 * 2. We pass the sticky bit restrictions. 	 * 3. We have privileges that always allow file removal. 	 */
return|return
operator|(
