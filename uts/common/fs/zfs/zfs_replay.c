...
name|length
expr_stmt|;
comment|/* end of data for this write */
comment|/* 	 * This may be a write from a dmu_sync() for a whole block, 	 * and may extend beyond the current end of the file. 	 * We can't just replay what was written for this TX_WRITE as 	 * a future TX_WRITE2 may extend the eof and the data for that 	 * write needs to be there. So we write the whole block and 	 * reduce the eof. This needs to be done within the single dmu 	 * transaction created within vn_rdwr -> zfs_write. So a possible 	 * new end of file is passed through in zfsvfs->z_replay_eof 	 */
name|zfsvfs
operator|->
name|z_replay_eof
operator|=
literal|0
expr_stmt|;
comment|/* 0 means don't change end of file */
...
begin_comment
comment|/*  * TX_WRITE2 are only generated when dmu_sync() returns EALREADY  * meaning the pool block is already being synced. So now that we always write  * out full blocks, all we have to do is expand the eof if  * the file is grown.  */
end_comment

begin_function
specifier|static
name|int
name|zfs_replay_write2
parameter_list|(
name|zfsvfs_t
modifier|*
name|zfsvfs
...
argument_list|)
expr_stmt|;
comment|/* Ensure the replayed seq is updated */
operator|(
name|void
operator|)
name|zil_replaying
