...
name|err
decl_stmt|;
comment|/* 	 * Account for the header block of the fatzap. 	 */
if|if
condition|(
operator|!
name|add
...
expr_stmt|;
block|}
comment|/* 	 * Account for the pointer table blocks. 	 * If we are adding we need to account for the following cases : 	 * - If the pointer table is embedded, this operation could force an 	 *   external pointer table. 	 * - If this already has an external pointer table this operation 	 *   could extend the table. 	 */
if|if
condition|(
name|add
condition|)
...
block|}
block|}
comment|/* 	 * Now, check if the block containing leaf is freeable 	 * and account accordingly. 	 */
name|err
operator|=
name|zap_deref_leaf
argument_list|(
...
block|{
comment|/* 		 * If this an add operation, the leaf block could split. 		 * Hence, we need to account for an additional leaf block. 		 */
operator|(
name|void
operator|)
name|refcount_add_many
