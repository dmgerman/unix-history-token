...
begin_comment
comment|/*  * Copyright (c) 2010, Oracle and/or its affiliates. All rights reserved.  * Portions Copyright 2011 iXsystems, Inc  * Copyright (c) 2013 by Delphix. All rights reserved.  * Copyright (c) 2014 Spectra Logic Corporation, All rights reserved.  */
end_comment
...
condition|)
block|{
comment|/* 				 * Account for header space used by array of 				 * optional sizes of variable-length attributes. 				 * Record the index in case this increase needs 				 * to be reversed due to spill-over. 				 */
name|hdrsize
operator|+=
sizeof|sizeof
argument_list|(
...
expr_stmt|;
block|}
comment|/* 	 * j holds the index of the last variable-sized attribute for 	 * which hdrsize was increased.  Reverse the increase if that 	 * attribute will be relocated to the spill block. 	 */
if|if
condition|(
operator|*
name|will_spill
...
control|)
block|{
comment|/* iterate over each attribute in layout */
for|for
control|(
name|i
operator|=
