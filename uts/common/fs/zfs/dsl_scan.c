...
begin_comment
comment|/* set to disable srub prefetching */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * XXX need to make sure all of these arc_read() prefetches are 	 * done before setting xlateall (similar to dsl_read()) 	 */
operator|(
name|void
operator|)
name|arc_read
...
name|bp
argument_list|,
literal|"visiting ds=%p/%llu zb=%llx/%llx/%llx/%llx buf=%p bp=%p"
argument_list|,
name|ds
argument_list|,
name|ds
...
name|B_TRUE
expr_stmt|;
comment|/* 	 * First process the free list.  If we pause the free, don't do 	 * any scanning.  This ensures that there is no free list when 	 * we are scanning, so the scan code doesn't have to worry about 	 * traversing it. 	 */
if|if
condition|(
name|spa_version
argument_list|(
...
condition|)
return|return;
comment|/* disable async destroy feature */
...
literal|"free_bpobj/bptree txg %llu"
argument_list|,
operator|(
name|longlong_t
operator|)
...
literal|0
expr_stmt|;
comment|/* 			 * Re-sync the ddt so that we can further modify 			 * it when doing bprewrite. 			 */
name|ddt_sync
argument_list|(
name|spa
argument_list|,
