...
begin_comment
comment|/*  * Copyright (c) 2008, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright 2016 Gary Mills  * Copyright (c) 2011, 2016 by Delphix. All rights reserved.  * Copyright 2017 Joyent, Inc.  * Copyright (c) 2017 Datto Inc.  */
end_comment
...
specifier|static
name|int
name|dsl_scrub_pause_resume_check
parameter_list|(
name|void
modifier|*
name|arg
...
condition|)
block|{
comment|/* can't pause a scrub when there is no in-progress scrub */
if|if
condition|(
operator|!
name|dsl_scan_scrubbing
...
operator|)
return|;
comment|/* can't pause a paused scrub */
if|if
condition|(
name|dsl_scan_is_paused_scrub
argument_list|(
...
specifier|static
name|void
name|dsl_scrub_pause_resume_sync
parameter_list|(
name|void
modifier|*
name|arg
...
condition|)
block|{
comment|/* 			 * We need to keep track of how much time we spend 			 * paused per pass so that we can adjust the scrub rate 			 * shown in the output of 'zpool status' 			 */
name|spa
operator|->
...
begin_comment
comment|/*  * Set scrub pause/resume state if it makes sense to do so  */
end_comment
...
operator|)
return|;
comment|/* we're already suspending */
if|if
condition|(
operator|!
name|ZB_IS_ZERO
...
name|dprintf
argument_list|(
literal|"suspending at bookmark %llx/%llx/%llx/%llx\n"
argument_list|,
operator|(
name|longlong_t
operator|)
...
name|dprintf
argument_list|(
literal|"suspending at DDT bookmark %llx/%llx/%llx/%llx\n"
argument_list|,
operator|(
name|longlong_t
operator|)
...
operator|)
return|;
comment|/* 		 * If we found the block we're trying to resume from, or 		 * we went past it to a different object, zero it out to 		 * indicate that it's OK to start checking for suspending 		 * again. 		 */
if|if
condition|(
name|bcmp
argument_list|(
...
condition|)
block|{
comment|/* 			 * We also always visit user/group accounting 			 * objects, and never skip them, even if we are 			 * suspending.  This is necessary so that the space 			 * deltas from this txg get integrated. 			 */
name|dsl_scan_visitdnode
argument_list|(
name|scn
argument_list|,
...
name|zfs_dbgmsg
argument_list|(
literal|"scanned %llu ddt entries with class_max = %u; "
literal|"suspending=%u"
argument_list|,
operator|(
name|longlong_t
...
condition|)
block|{
comment|/* 		 * If we were suspended, continue from here.  Note if the 		 * ds we were suspended on was deleted, the zb_objset may 		 * be -1, so we will skip this and find a new objset 		 * below. 		 */
name|dsl_scan_visitds
argument_list|(
name|scn
argument_list|,
...
return|return;
block|}
comment|/* 	 * In case we were suspended right at the end of the ds, zero the 	 * bookmark so we don't think that we're still trying to resume. 	 */
name|bzero
argument_list|(
operator|&
name|scn
...
specifier|static
name|boolean_t
name|dsl_scan_free_should_suspend
parameter_list|(
name|dsl_scan_t
modifier|*
name|scn
...
name|B_TRUE
expr_stmt|;
comment|/* 	 * First process the async destroys.  If we suspend, don't do 	 * any scrubbing or resilvering.  This ensures that there are no 	 * async destroys while we are scanning, so the scan code doesn't 	 * have to worry about traversing it.  It is also faster to free the 	 * blocks than to scrub them. 	 */
if|if
condition|(
name|zfs_free_bpobj_enabled
operator|&&
...
begin_comment
comment|/*  * Called by the ZFS_IOC_POOL_SCAN ioctl to start a scrub or resilver.  * Can also be called to resume a paused scrub.  */
end_comment
...
condition|)
block|{
comment|/* got scrub start cmd, resume paused scrub */
name|int
name|err
init|=
name|dsl_scrub_set_pause_resume
