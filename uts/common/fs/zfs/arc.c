...
condition|)
block|{
comment|/* 		 * XXX This happens from traverse callback funcs, for 		 * the objset_phys_t block. 		 */
return|return
operator|(
...
begin_comment
comment|/*  * Release this buffer.  If it does not match the provided BP, fill it  * with that block's contents.  */
end_comment
...
expr_stmt|;
block|}
comment|/* 	 * If the block to be written was all-zero, we may have 	 * compressed it away.  In this case no write was performed 	 * so there will be no dva/birth/checksum.  The buffer must 	 * therefore remain anonymous (and uncached). 	 */
if|if
condition|(
operator|!
name|BUF_EMPTY
