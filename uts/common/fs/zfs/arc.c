...
condition|)
block|{
comment|/* 		 * The arc_buf_hdr_t is either not compressed or is 		 * associated with an embedded block or a hole in which 		 * case they remain anonymous. 		 */
name|IMPLY
argument_list|(
name|HDR_COMPRESSION_ENABLED
argument_list|(
...
specifier|static
name|void
name|arc_evitable_space_decrement
parameter_list|(
name|arc_buf_hdr_t
modifier|*
name|hdr
...
begin_comment
comment|/*  * Allocate an initial buffer for this hdr, subsequent buffers will  * use arc_buf_clone().  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * If the hdr's data can be shared (no byteswapping, hdr is 	 * uncompressed, hdr's data is not currently being written to the 	 * L2ARC write) then we share the data buffer and set the appropriate 	 * bit in the hdr's b_flags to indicate the hdr is sharing it's 	 * b_pdata with the arc_buf_t. Otherwise, we allocate a new buffer to 	 * store the buf's data. 	 */
if|if
condition|(
name|hdr
operator|->
...
begin_comment
comment|/*  * Used when allocating additional buffers.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* 	 * Free up the data associated with the buf but only 	 * if we're not sharing this with the hdr. If we are sharing 	 * it with the hdr, then hdr will have performed the allocation 	 * so allow it to do the free. 	 */
if|if
condition|(
name|buf
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * If the current arc_buf_t is sharing its data 	 * buffer with the hdr, then reassign the hdr's 	 * b_pdata to share it with the new buffer at the end 	 * of the list. The shared buffer is always the last one 	 * on the hdr's buffer list. 	 */
if|if
condition|(
name|destroyed_buf_is_shared
operator|&&
...
condition|)
block|{
comment|/* 			 * If we're here, then this must be a demand read 			 * since prefetch requests don't have callbacks. 			 * If a read request has a callback (i.e. acb_done is 			 * not NULL), then we decompress the data for the 			 * first request and clone the rest. This avoids 			 * having to waste cpu resources decompressing data 			 * that nobody is explicitly waiting to read. 			 */
if|if
condition|(
name|abuf
operator|==
...
condition|)
block|{
comment|/* 		 * This buffer didn't have a callback so it must 		 * be a prefetch. 		 */
name|ASSERT
argument_list|(
name|HDR_PREFETCH
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * If the current arc_buf_t and the hdr are sharing their data 		 * buffer, then we must stop sharing that block, transfer 		 * ownership and setup sharing with a new arc_buf_t at the end 		 * of the hdr's b_buf list. 		 */
if|if
condition|(
name|arc_buf_is_shared
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 			 * First, sever the block sharing relationship between 			 * buf and the arc_buf_hdr_t. Then, setup a new 			 * block sharing relationship with the last buffer 			 * on the arc_buf_t list. 			 */
name|arc_unshare_buf
argument_list|(
name|hdr
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we're reexecuting this zio because the pool suspended, then 	 * cleanup any state that was previously set the first time the 	 * callback as invoked. 	 */
if|if
condition|(
name|zio
operator|->
