...
name|hdr
parameter_list|)
value|((enum zio_compress)BF32_GET(hdr->b_flags, \ 	    HDR_COMPRESS_OFFSET, HDR_COMPRESS_NBITS))
end_define
...
name|cmp
parameter_list|)
value|BF32_SET(hdr->b_flags, \ 	    HDR_COMPRESS_OFFSET, HDR_COMPRESS_NBITS, (cmp))
end_define
...
argument_list|)
expr_stmt|;
comment|/* 		 * We might be removing the L1hdr of a buffer which was just 		 * written out to L2ARC. If such a buffer is compressed then we 		 * need to free its b_tmp_cdata before destroying the header. 		 */
if|if
condition|(
name|hdr
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * Decide which "type" (data vs metadata) to recycle from. 	 * 	 * If we are over the metadata limit, recycle from metadata. 	 * If we are under the metadata minimum, recycle from data. 	 * Otherwise, recycle from whichever type has the oldest (least 	 * recently accessed) header. 	 */
if|if
condition|(
name|recycle
condition|)
...
condition|)
block|{
comment|/* 			 * This buffer misses out.  It may be in a stage 			 * of eviction.  Its ARC_FLAG_L2_WRITING flag will be 			 * left set, denying reads to this buffer. 			 */
name|ARCSTAT_BUMP
argument_list|(
name|arcstat_l2_writes_hdr_miss
argument_list|)
...
continue|continue;
block|}
comment|/* 		 * It's possible that this buffer got evicted from the L1 cache 		 * before we grabbed the vdev + hash locks, in which case 		 * arc_hdr_realloc freed b_tmp_cdata for us if it was allocated. 		 * Only free the buffer if we still have an L1 hdr. 		 */
if|if
condition|(
name|HDR_HAS_L1HDR
argument_list|(
