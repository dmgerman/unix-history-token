...
begin_comment
comment|/* shift of arc_c for calculating overflow limit in arc_get_data_impl */
end_comment
...
name|arcstat_size
decl_stmt|;
comment|/* 	 * Number of compressed bytes stored in the arc_buf_hdr_t's b_pabd. 	 * Note that the compressed bytes may match the uncompressed bytes 	 * if the block is either not compressed or compressed arc is disabled. 	 */
name|kstat_named_t
name|arcstat_compressed_size
decl_stmt|;
comment|/* 	 * Uncompressed size of the data stored in b_pabd. If compressed 	 * arc is disabled then this value will be identical to the stat 	 * above. 	 */
name|kstat_named_t
...
decl_stmt|;
name|kstat_named_t
name|arcstat_l2_lsize
decl_stmt|;
name|kstat_named_t
name|arcstat_l2_psize
decl_stmt|;
name|kstat_named_t
...
begin_comment
comment|/*  * Return the size of the block, b_pabd, that is stored in the arc_buf_hdr_t.  */
end_comment
...
begin_comment
comment|/*  * Given a hdr and a buf, returns whether that buf can share its b_data buffer  * with the hdr's b_pabd.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 		 * A buffer must not be moved into the arc_l2c_only 		 * state if it's not finished being written out to the 		 * l2arc device. Otherwise, the b_l1hdr.b_pabd field 		 * might try to be accessed, even though it was removed. 		 */
name|VERIFY
argument_list|(
operator|!
name|HDR_L2_WRITING
...
condition|)
block|{
comment|/* 		 * To ensure that the hdr has the correct data in it if we call 		 * arc_decompress() on this buf before it's been written to 		 * disk, it's easiest if we just set up sharing between the 		 * buf and the hdr. 		 */
name|ASSERT
argument_list|(
operator|!
name|abd_is_linear
...
argument_list|)
expr_stmt|;
comment|/* 		 * l2arc_write_buffers() relies on a header's L1 portion 		 * (i.e. its b_pabd field) during it's write phase. 		 * Thus, we cannot push a header onto the arc_l2c_only 		 * state (removing it's L1 piece) until the header is 		 * done being written to the l2arc. 		 */
if|if
condition|(
name|HDR_HAS_L2HDR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * If this hdr is being evicted and has a compressed 		 * buffer then we discard it here before we change states. 		 * This ensures that the accounting is updated correctly 		 * in arc_free_data_impl(). 		 */
name|arc_hdr_free_pabd
argument_list|(
name|hdr
argument_list|)
...
operator|++
expr_stmt|;
comment|/* 			 * If arc_size isn't overflowing, signal any 			 * threads that might happen to be waiting. 			 * 			 * For each header evicted, we wake up a single 			 * thread. If we used cv_broadcast, we could 			 * wake up "too many" threads causing arc_size 			 * to significantly overflow arc_c; since 			 * arc_get_data_impl() doesn't check for overflow 			 * when it's woken up (it doesn't because it's 			 * possible for the ARC to be overflowing while 			 * full of un-evictable buffers, and the 			 * function should proceed in this case). 			 * 			 * If threads are left sleeping, due to not 			 * using cv_broadcast, they will be woken up 			 * just before arc_reclaim_thread() sleeps. 			 */
name|mutex_enter
argument_list|(
operator|&
name|arc_reclaim_lock
...
begin_comment
comment|/*  * Threads can block in arc_get_data_impl() waiting for this thread to evict  * enough data and signal them to proceed. When this happens, the threads in  * arc_get_data_impl() are sleeping while holding the hash lock for their  * particular arc header. Thus, we must be careful to never sleep on a  * hash lock in this thread. This is to prevent the following deadlock:  *  *  - Thread A sleeps on CV in arc_get_data_impl() holding hash lock "L",  *    waiting for the reclaim thread to signal it.  *  *  - arc_reclaim_thread() tries to acquire hash lock "L" using mutex_enter,  *    fails, and goes to sleep forever.  *  * This possible deadlock is avoided by always acquiring a hash lock  * using mutex_tryenter() from arc_reclaim_thread().  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 		 * We call arc_adjust() before (possibly) calling 		 * arc_kmem_reap_now(), so that we can wake up 		 * arc_get_data_impl() sooner. 		 */
name|evicted
operator|=
name|arc_adjust
argument_list|()
...
argument_list|)
expr_stmt|;
comment|/* 			 * Now we need to recreate the hdr's b_pabd. Since we 			 * have lastbuf handy, we try to share with it, but if 			 * we can't then we allocate a new b_pabd and copy the 			 * data from buf into it. 			 */
if|if
condition|(
name|arc_can_share
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Allocate a new hdr. The new hdr will contain a b_pabd 		 * buffer which will be freed in arc_write(). 		 */
name|nhdr
operator|=
name|arc_hdr_alloc
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Ideally, we would always copy the io_abd into b_pabd, but the 		 * user may have disabled compressed ARC, thus we must check the 		 * hdr's compression setting rather than the io_bp's. 		 */
if|if
condition|(
name|HDR_GET_COMPRESS
argument_list|(
...
name|buf
expr_stmt|;
comment|/* 	 * The hdr's b_pabd is now stale, free it now. A new data block 	 * will be allocated when the zio pipeline calls arc_write_ready(). 	 */
if|if
condition|(
name|hdr
operator|->
...
goto|;
block|}
comment|/* 		 * A header can't be on this list if it doesn't have L2 header. 		 */
name|ASSERT
argument_list|(
name|HDR_HAS_L2HDR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 			 * This doesn't exist in the ARC.  Destroy. 			 * arc_hdr_destroy() will call list_remove() 			 * and decrement arcstat_l2_lsize. 			 */
name|arc_change_state
argument_list|(
name|arc_anon
argument_list|,
