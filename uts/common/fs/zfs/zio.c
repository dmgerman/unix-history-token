...
argument_list|)
decl_stmt|;
comment|/* 	 * Ensure that anyone expecting this zio to contain a linear ABD isn't 	 * going to get a nasty surprise when they try to access the data. 	 */
name|IMPLY
argument_list|(
name|abd_is_linear
argument_list|(
...
condition|)
block|{
comment|/* we are not doing a raw write */
name|ASSERT3U
argument_list|(
name|zio
operator|->
...
expr_stmt|;
block|}
comment|/* If it's a compressed write that is not raw, compress the buffer. */
if|if
condition|(
name|compress
operator|!=
...
condition|)
return|return;
comment|/* this ABD was created from a linear buf in zio_gang_tree_assemble */
if|if
condition|(
...
operator|)
decl_stmt|;
comment|/* We should never get a raw, override zio */
name|ASSERT
argument_list|(
operator|!
operator|(
