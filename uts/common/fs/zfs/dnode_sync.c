...
literal|0
decl_stmt|;
comment|/* 	 * There is a small possibility that this block will not be cached: 	 *   1 - if level> 1 and there are no children with level<= 1 	 *   2 - if we didn't get a dirty hold (because this block had just 	 *	 finished being written -- and so had no holds), and then this 	 *	 block got evicted before we got here. 	 */
if|if
condition|(
name|db
operator|->
...
argument_list|)
expr_stmt|;
comment|/* grab the mutex so we don't race with dnode_block_freed() */
name|mutex_enter
argument_list|(
operator|&
name|dn
