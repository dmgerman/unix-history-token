...
name|db
expr_stmt|;
comment|/* NULL db plus first field offset is NULL */
return|return
operator|(
name|err
operator|)
...
name|err
decl_stmt|;
comment|/* find next allocated L1 indirect */
name|err
operator|=
...
argument_list|)
expr_stmt|;
comment|/* if there are no more, then we are done */
if|if
condition|(
name|err
operator|==
...
literal|1
expr_stmt|;
comment|/* reset offset to end of "next" block back */
operator|*
name|start
operator|=
name|P2ALIGN
...
condition|)
block|{
comment|/* 		 * If we didn't allocate a new block (i.e. ZIO_FLAG_NOPWRITE) 		 * then there is nothing to do here. Otherwise, free the 		 * newly allocated block in this txg. 		 */
if|if
condition|(
name|zio
operator|->
...
expr_stmt|;
block|}
comment|/* 		 * Enable nopwrite if we have a cryptographically secure 		 * checksum that has no known collisions (i.e. SHA-256) 		 * and compression is enabled.  We don't enable nopwrite if 		 * dedup is enabled as the two features are mutually exclusive. 		 */
name|nopwrite
operator|=
operator|(
operator|!
