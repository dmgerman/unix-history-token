...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright (c) 2012, 2014 by Delphix. All rights reserved.  * Copyright (c) 2013 Martin Matuska. All rights reserved.  * Copyright 2015, Joyent, Inc.  */
end_comment
...
begin_function
specifier|static
name|dsl_prop_record_t
modifier|*
name|dsl_prop_record_find
parameter_list|(
name|dsl_dir_t
modifier|*
name|dd
...
begin_comment
comment|/*  * Unregister all callbacks that are registered with the  * given callback argument.  */
end_comment
...
name|value
decl_stmt|;
comment|/* 			 * Callback entries do not have holds on their 			 * datasets so that datasets with registered 			 * callbacks are still eligible for eviction. 			 * Unlike operations to update properties on a 			 * single dataset, we are performing a recursive 			 * descent of related head datasets.  The caller 			 * of this function only has a dataset hold on 			 * the passed in head dataset, not the snapshots 			 * associated with this dataset.  Without a hold, 			 * the dataset pointer within callback records 			 * for snapshots can be invalidated by eviction 			 * at any time. 			 * 			 * Use dsl_dataset_try_add_ref() to verify 			 * that the dataset for a snapshot has not 			 * begun eviction processing and to prevent 			 * eviction from occurring for the duration of 			 * the callback.  If the hold attempt fails, 			 * this object is already being evicted and the 			 * callback can be safely ignored. 			 */
if|if
condition|(
name|ds
operator|!=
...
name|propobj
decl_stmt|;
comment|/* 			 * cbr->cbr_ds may be invalidated due to eviction, 			 * requiring the use of dsl_dataset_try_add_ref(). 			 * See comment block in dsl_prop_notify_all_cb() 			 * for details. 			 */
if|if
condition|(
operator|!
name|dsl_dataset_try_add_ref
...
name|ds_props_obj
expr_stmt|;
comment|/* 			 * If the property is not set on this ds, then it is 			 * inherited here; call the callback. 			 */
if|if
condition|(
name|propobj
operator|==
