...
condition|)
return|return;
comment|/* 	 * The object contains more than one block so we can't adjust 	 * its size. 	 */
if|if
condition|(
name|sm
operator|->
...
name|newsz
decl_stmt|;
comment|/* 		 * Older software versions treat space map blocks as fixed 		 * entities. The DMU is capable of handling different block 		 * sizes making it possible for us to increase the 		 * block size and maintain backwards compatibility. The 		 * caveat is that the new block sizes must be a 		 * power of 2 so that old software can append to the file, 		 * adding more blocks. The block size can grow until it 		 * reaches space_map_max_blksz. 		 */
name|newsz
operator|=
name|ISP2
argument_list|(
...
name|zfs_dbgmsg
argument_list|(
literal|"txg %llu, spa %s, increasing blksz from %d to %d"
argument_list|,
name|dmu_tx_get_txg
argument_list|(
name|tx
...
argument_list|)
expr_stmt|;
comment|/* 	 * Calculate the new size for the space map on-disk and see if 	 * we can grow the block size to accommodate the new size. 	 */
name|newsz
operator|=
name|sm
operator|->
