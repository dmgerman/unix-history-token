...
parameter_list|)
define|\
value|if (addr& (size - 1)) {					\ 		*flags |= CPU_DTRACE_BADALIGN;				\ 		cpu_core[CPU->cpu_id].cpuc_dtrace_illval = addr;	\ 		return (0);						\ 	}
end_define
...
comment|/*CSTYLED*/
value|\ 	uint##bits##_t rval;						\ 	int i;								\ 	volatile uint16_t *flags = (volatile uint16_t *)		\&cpu_core[CPU->cpu_id].cpuc_dtrace_flags;			\ 									\ 	DTRACE_ALIGNCHECK(addr, size, flags);				\ 									\ 	for (i = 0; i< dtrace_toxranges; i++) {			\ 		if (addr>= dtrace_toxrange[i].dtt_limit)		\ 			continue;					\ 									\ 		if (addr + size<= dtrace_toxrange[i].dtt_base)		\ 			continue;					\ 									\
comment|/*							\ 		 * This address falls within a toxic region; return 0.	\ 		 */
value|\ 		*flags |= CPU_DTRACE_BADADDR;				\ 		cpu_core[CPU->cpu_id].cpuc_dtrace_illval = addr;	\ 		return (0);						\ 	}								\ 									\ 	*flags |= CPU_DTRACE_NOFAULT;					\
comment|/*CSTYLED*/
...
expr_stmt|;
block|}
comment|/* 	 * If the dte_cond bits indicate that this consumer is only allowed to 	 * see user-mode firings of this probe, check that the probe was fired 	 * while in a user context.  If that's not the case, use the policy 	 * specified by the provider to determine if we drop the probe or 	 * merely restrict operation. 	 */
if|if
condition|(
name|ecb
operator|->
...
block|}
block|}
comment|/* 	 * By merits of being in this code path at all, we have limited 	 * privileges.  If the provider has indicated that limited privileges 	 * are to denote restricted operation, strip off the ability to access 	 * arguments. 	 */
if|if
condition|(
name|mode
operator|&
...
break|break;
block|}
comment|/* 		 * This is safe because fi_nfiles only increases, and the 		 * fi_list array is not freed when the array size doubles. 		 * (See the comment in flist_grow() for details on the 		 * management of the u_finfo structure.) 		 */
name|fp
operator|=
name|fd
operator|<
...
continue|continue;
block|}
comment|/* 				 * Clear the string space, since there's no 				 * helper to do it for us. 				 */
if|if
condition|(
name|DTRACE_USTACK_STRSIZE
argument_list|(
...
literal|"dtps_mode() op for given privilege attributes"
argument_list|,
name|name
argument_list|)
expr_stmt|;
