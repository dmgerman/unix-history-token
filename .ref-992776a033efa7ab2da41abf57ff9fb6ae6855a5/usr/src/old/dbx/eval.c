...
name|fr
parameter_list|)
value|{ \     eval(p->value.arg[n]); \     if (isreal(p->op)) { \ 	fr = pop(double); \     } else if (isint(p->op)) { \ 	r = popsmall(p->value.arg[n]->nodetype); \     } \ }
end_define
...
expr_stmt|;
break|break;
comment|/* 	 * Effectively, we want to pop n bytes off for the evaluated subtree 	 * and push len bytes on for the new type of the same tree. 	 */
case|case
name|O_TYPERENAME
case|:
name|n
...
begin_comment
comment|/*  * Send some nasty mail to the current support person.  */
end_comment
