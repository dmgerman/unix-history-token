...
begin_comment
comment|/*  * Allocate 'size' units from the given map, starting at address 'addr'.  * Return 'addr' if successful, 0 if not.  * This may cause the creation or destruction of a resource map segment.  *  * This routine will return failure status if there is not enough room  * for a required additional map segment.  *  * An attempt to use this on 'swapmap' will result in  * a failure return.  This is due mainly to laziness and could be fixed  * to do the right thing, although it probably will never be used.  */
end_comment
...
operator|)
return|;
comment|/* 	 * Look for a map segment containing the requested address. 	 * If none found, return failure. 	 */
for|for
control|(
name|bp
operator|=
...
operator|)
return|;
comment|/* 	 * If segment is too small, return failure. 	 * If big enough, allocate the block, compressing or expanding 	 * the map as necessary. 	 */
if|if
condition|(
name|bp
operator|->
...
condition|)
block|{
comment|/* 			 * Allocate entire segment and compress map 			 */
name|bp2
operator|=
name|bp
expr_stmt|;
...
block|{
comment|/* 			 * Allocate first part of segment 			 */
name|bp
operator|->
name|m_addr
operator|+=
...
condition|)
block|{
comment|/* 			 * Allocate last part of segment 			 */
name|bp
operator|->
name|m_size
operator|-=
...
block|{
comment|/* 			 * Allocate from middle of segment, but only 			 * if table can be expanded. 			 */
for|for
control|(
name|bp2
operator|=
