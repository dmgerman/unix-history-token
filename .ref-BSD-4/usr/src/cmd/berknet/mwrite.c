...
literal|0
expr_stmt|;
comment|/* 	NO LONGER NEEDED 	strcpy(toaddress,argv[1]); 	strcpy(ttynamestr,argv[2]); 	sscanf(argv[3],"%lo",&lutmptime); 	sprintf(fromaddress,"%s:%s",longname(argv[4][0]),argv[5]); 	if(argc> 6)strcpy(cmdstr,argv[6]); 	else cmdstr[0] = 0; 	if(argc> 7)strcpy(stimeSent,argv[7]); 	else stimeSent[0] = 0; 	*/
