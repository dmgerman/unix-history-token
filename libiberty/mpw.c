...
literal|0
expr_stmt|;
comment|/* If you're going to end up with one or more colons in the middle of a      path after an all-Unix relative path is translated, you must add a      colon on the front, so that the first component is not thought to be      a disk name.  */
if|if
condition|(
name|unixname
index|[
...
block|{
comment|/* Don't copy the leading slash. */
block|}
block|}
elseif|else
if|if
...
literal|'\0'
expr_stmt|;
comment|/* Allow for getting the debug flag from an env var; quite useful. */
if|if
condition|(
name|DebugPI
operator|<
