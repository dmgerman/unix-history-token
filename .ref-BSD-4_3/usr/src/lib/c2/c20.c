...
index|[]
init|=
literal|"@(#)c20.c	4.10 (Berkeley) 8/22/85"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * Cheapo space allocator.  Works much like the old one but uses malloc()  * and doesn't clash with stdio.  Assumes that no one requests more than  * ALLOCSIZE bytes at a time.  */
end_comment
...
name|stderr
argument_list|,
literal|"c2: garbled 'case' instruction\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
