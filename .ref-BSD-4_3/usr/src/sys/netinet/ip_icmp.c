begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982, 1986 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)ip_icmp.c	7.2 (Berkeley) 1/10/87  */
end_comment

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"systm.h"
end_include

begin_include
include|#
directive|include
file|"mbuf.h"
end_include

begin_include
include|#
directive|include
file|"protosw.h"
end_include

begin_include
include|#
directive|include
file|"socket.h"
end_include

begin_include
include|#
directive|include
file|"time.h"
end_include

begin_include
include|#
directive|include
file|"kernel.h"
end_include

begin_include
include|#
directive|include
file|"../net/route.h"
end_include

begin_include
include|#
directive|include
file|"../net/if.h"
end_include

begin_include
include|#
directive|include
file|"in.h"
end_include

begin_include
include|#
directive|include
file|"in_systm.h"
end_include

begin_include
include|#
directive|include
file|"in_var.h"
end_include

begin_include
include|#
directive|include
file|"ip.h"
end_include

begin_include
include|#
directive|include
file|"ip_icmp.h"
end_include

begin_include
include|#
directive|include
file|"icmp_var.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|ICMPPRINTFS
end_ifdef

begin_comment
comment|/*  * ICMP routines: error generation, receive packet processing, and  * routines to turnaround packets back to the originator, and  * host table maintenance routines.  */
end_comment

begin_decl_stmt
name|int
name|icmpprintfs
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Generate an error packet of type error  * in response to bad packet ip.  */
end_comment

begin_comment
comment|/*VARARGS4*/
end_comment

begin_macro
name|icmp_error
argument_list|(
argument|oip
argument_list|,
argument|type
argument_list|,
argument|code
argument_list|,
argument|ifp
argument_list|,
argument|dest
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|ip
modifier|*
name|oip
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|type
decl_stmt|,
name|code
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|in_addr
name|dest
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|unsigned
name|oiplen
init|=
name|oip
operator|->
name|ip_hl
operator|<<
literal|2
decl_stmt|;
specifier|register
name|struct
name|icmp
modifier|*
name|icp
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|struct
name|ip
modifier|*
name|nip
decl_stmt|;
name|unsigned
name|icmplen
decl_stmt|;
ifdef|#
directive|ifdef
name|ICMPPRINTFS
if|if
condition|(
name|icmpprintfs
condition|)
name|printf
argument_list|(
literal|"icmp_error(%x, %d, %d)\n"
argument_list|,
name|oip
argument_list|,
name|type
argument_list|,
name|code
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|type
operator|!=
name|ICMP_REDIRECT
condition|)
name|icmpstat
operator|.
name|icps_error
operator|++
expr_stmt|;
comment|/* 	 * Don't send error if not the first fragment of message. 	 * Don't EVER error if the old packet protocol was ICMP. 	 * (Could do ECHO, etc, but not error indications.) 	 */
if|if
condition|(
name|oip
operator|->
name|ip_off
operator|&
operator|~
operator|(
name|IP_MF
operator||
name|IP_DF
operator|)
condition|)
goto|goto
name|free
goto|;
if|if
condition|(
name|oip
operator|->
name|ip_p
operator|==
name|IPPROTO_ICMP
operator|&&
name|type
operator|!=
name|ICMP_REDIRECT
condition|)
block|{
name|icmpstat
operator|.
name|icps_oldicmp
operator|++
expr_stmt|;
goto|goto
name|free
goto|;
block|}
comment|/* 	 * First, formulate icmp message 	 */
name|m
operator|=
name|m_get
argument_list|(
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
goto|goto
name|free
goto|;
name|icmplen
operator|=
name|oiplen
operator|+
name|MIN
argument_list|(
literal|8
argument_list|,
name|oip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
name|icmplen
operator|+
name|ICMP_MINLEN
expr_stmt|;
name|m
operator|->
name|m_off
operator|=
name|MMAXOFF
operator|-
name|m
operator|->
name|m_len
expr_stmt|;
name|icp
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|icmp
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|u_int
operator|)
name|type
operator|>
name|ICMP_MAXTYPE
condition|)
name|panic
argument_list|(
literal|"icmp_error"
argument_list|)
expr_stmt|;
name|icmpstat
operator|.
name|icps_outhist
index|[
name|type
index|]
operator|++
expr_stmt|;
name|icp
operator|->
name|icmp_type
operator|=
name|type
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|ICMP_REDIRECT
condition|)
name|icp
operator|->
name|icmp_gwaddr
operator|=
name|dest
expr_stmt|;
else|else
name|icp
operator|->
name|icmp_void
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|ICMP_PARAMPROB
condition|)
block|{
name|icp
operator|->
name|icmp_pptr
operator|=
name|code
expr_stmt|;
name|code
operator|=
literal|0
expr_stmt|;
block|}
name|icp
operator|->
name|icmp_code
operator|=
name|code
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|oip
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|icp
operator|->
name|icmp_ip
argument_list|,
name|icmplen
argument_list|)
expr_stmt|;
name|nip
operator|=
operator|&
name|icp
operator|->
name|icmp_ip
expr_stmt|;
name|nip
operator|->
name|ip_len
operator|+=
name|oiplen
expr_stmt|;
name|nip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|nip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
comment|/* 	 * Now, copy old ip header in front of icmp message. 	 */
if|if
condition|(
name|m
operator|->
name|m_len
operator|+
name|oiplen
operator|>
name|MLEN
condition|)
name|oiplen
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_len
operator|+
name|oiplen
operator|>
name|MLEN
condition|)
name|panic
argument_list|(
literal|"icmp len"
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_off
operator|-=
name|oiplen
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
name|oiplen
expr_stmt|;
name|nip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|oip
argument_list|,
operator|(
name|caddr_t
operator|)
name|nip
argument_list|,
name|oiplen
argument_list|)
expr_stmt|;
name|nip
operator|->
name|ip_len
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
name|nip
operator|->
name|ip_p
operator|=
name|IPPROTO_ICMP
expr_stmt|;
name|icmp_reflect
argument_list|(
name|nip
argument_list|,
name|ifp
argument_list|)
expr_stmt|;
name|free
label|:
name|m_freem
argument_list|(
name|dtom
argument_list|(
name|oip
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_decl_stmt
specifier|static
name|struct
name|sockproto
name|icmproto
init|=
block|{
name|AF_INET
block|,
name|IPPROTO_ICMP
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|sockaddr_in
name|icmpsrc
init|=
block|{
name|AF_INET
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|sockaddr_in
name|icmpdst
init|=
block|{
name|AF_INET
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|sockaddr_in
name|icmpgw
init|=
block|{
name|AF_INET
block|}
decl_stmt|;
end_decl_stmt

begin_function_decl
name|struct
name|in_ifaddr
modifier|*
name|ifptoia
parameter_list|()
function_decl|;
end_function_decl

begin_comment
comment|/*  * Process a received ICMP message.  */
end_comment

begin_expr_stmt
name|icmp_input
argument_list|(
name|m
argument_list|,
name|ifp
argument_list|)
specifier|register
expr|struct
name|mbuf
operator|*
name|m
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|icmp
modifier|*
name|icp
decl_stmt|;
specifier|register
name|struct
name|ip
modifier|*
name|ip
init|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
decl_stmt|;
name|int
name|icmplen
init|=
name|ip
operator|->
name|ip_len
decl_stmt|,
name|hlen
init|=
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
decl_stmt|;
specifier|register
name|int
name|i
decl_stmt|;
name|struct
name|in_ifaddr
modifier|*
name|ia
decl_stmt|;
name|int
argument_list|(
operator|*
name|ctlfunc
argument_list|)
argument_list|()
decl_stmt|,
name|code
decl_stmt|;
specifier|extern
name|u_char
name|ip_protox
index|[]
decl_stmt|;
specifier|extern
name|struct
name|in_addr
name|in_makeaddr
parameter_list|()
function_decl|;
comment|/* 	 * Locate icmp structure in mbuf, and check 	 * that not corrupted and of at least minimum length. 	 */
ifdef|#
directive|ifdef
name|ICMPPRINTFS
if|if
condition|(
name|icmpprintfs
condition|)
name|printf
argument_list|(
literal|"icmp_input from %x, len %d\n"
argument_list|,
name|ip
operator|->
name|ip_src
argument_list|,
name|icmplen
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|icmplen
operator|<
name|ICMP_MINLEN
condition|)
block|{
name|icmpstat
operator|.
name|icps_tooshort
operator|++
expr_stmt|;
goto|goto
name|free
goto|;
block|}
name|i
operator|=
name|hlen
operator|+
name|MIN
argument_list|(
name|icmplen
argument_list|,
name|ICMP_ADVLENMIN
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|m
operator|->
name|m_off
operator|>
name|MMAXOFF
operator|||
name|m
operator|->
name|m_len
operator|<
name|i
operator|)
operator|&&
operator|(
name|m
operator|=
name|m_pullup
argument_list|(
name|m
argument_list|,
name|i
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
name|icmpstat
operator|.
name|icps_tooshort
operator|++
expr_stmt|;
return|return;
block|}
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|-=
name|hlen
expr_stmt|;
name|m
operator|->
name|m_off
operator|+=
name|hlen
expr_stmt|;
name|icp
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|icmp
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
name|in_cksum
argument_list|(
name|m
argument_list|,
name|icmplen
argument_list|)
condition|)
block|{
name|icmpstat
operator|.
name|icps_checksum
operator|++
expr_stmt|;
goto|goto
name|free
goto|;
block|}
name|m
operator|->
name|m_len
operator|+=
name|hlen
expr_stmt|;
name|m
operator|->
name|m_off
operator|-=
name|hlen
expr_stmt|;
ifdef|#
directive|ifdef
name|ICMPPRINTFS
comment|/* 	 * Message type specific processing. 	 */
if|if
condition|(
name|icmpprintfs
condition|)
name|printf
argument_list|(
literal|"icmp_input, type %d code %d\n"
argument_list|,
name|icp
operator|->
name|icmp_type
argument_list|,
name|icp
operator|->
name|icmp_code
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|icp
operator|->
name|icmp_type
operator|>
name|ICMP_MAXTYPE
condition|)
goto|goto
name|raw
goto|;
name|icmpstat
operator|.
name|icps_inhist
index|[
name|icp
operator|->
name|icmp_type
index|]
operator|++
expr_stmt|;
name|code
operator|=
name|icp
operator|->
name|icmp_code
expr_stmt|;
switch|switch
condition|(
name|icp
operator|->
name|icmp_type
condition|)
block|{
case|case
name|ICMP_UNREACH
case|:
if|if
condition|(
name|code
operator|>
literal|5
condition|)
goto|goto
name|badcode
goto|;
name|code
operator|+=
name|PRC_UNREACH_NET
expr_stmt|;
goto|goto
name|deliver
goto|;
case|case
name|ICMP_TIMXCEED
case|:
if|if
condition|(
name|code
operator|>
literal|1
condition|)
goto|goto
name|badcode
goto|;
name|code
operator|+=
name|PRC_TIMXCEED_INTRANS
expr_stmt|;
goto|goto
name|deliver
goto|;
case|case
name|ICMP_PARAMPROB
case|:
if|if
condition|(
name|code
condition|)
goto|goto
name|badcode
goto|;
name|code
operator|=
name|PRC_PARAMPROB
expr_stmt|;
goto|goto
name|deliver
goto|;
case|case
name|ICMP_SOURCEQUENCH
case|:
if|if
condition|(
name|code
condition|)
goto|goto
name|badcode
goto|;
name|code
operator|=
name|PRC_QUENCH
expr_stmt|;
name|deliver
label|:
comment|/* 		 * Problem with datagram; advise higher level routines. 		 */
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_len
operator|=
name|ntohs
argument_list|(
operator|(
name|u_short
operator|)
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_len
argument_list|)
expr_stmt|;
if|if
condition|(
name|icmplen
operator|<
name|ICMP_ADVLENMIN
operator|||
name|icmplen
operator|<
name|ICMP_ADVLEN
argument_list|(
name|icp
argument_list|)
condition|)
block|{
name|icmpstat
operator|.
name|icps_badlen
operator|++
expr_stmt|;
goto|goto
name|free
goto|;
block|}
ifdef|#
directive|ifdef
name|ICMPPRINTFS
if|if
condition|(
name|icmpprintfs
condition|)
name|printf
argument_list|(
literal|"deliver to protocol %d\n"
argument_list|,
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_p
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|icmpsrc
operator|.
name|sin_addr
operator|=
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_dst
expr_stmt|;
if|if
condition|(
name|ctlfunc
operator|=
name|inetsw
index|[
name|ip_protox
index|[
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_p
index|]
index|]
operator|.
name|pr_ctlinput
condition|)
call|(
modifier|*
name|ctlfunc
call|)
argument_list|(
name|code
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpsrc
argument_list|)
expr_stmt|;
break|break;
name|badcode
label|:
name|icmpstat
operator|.
name|icps_badcode
operator|++
expr_stmt|;
break|break;
case|case
name|ICMP_ECHO
case|:
name|icp
operator|->
name|icmp_type
operator|=
name|ICMP_ECHOREPLY
expr_stmt|;
goto|goto
name|reflect
goto|;
case|case
name|ICMP_TSTAMP
case|:
if|if
condition|(
name|icmplen
operator|<
name|ICMP_TSLEN
condition|)
block|{
name|icmpstat
operator|.
name|icps_badlen
operator|++
expr_stmt|;
break|break;
block|}
name|icp
operator|->
name|icmp_type
operator|=
name|ICMP_TSTAMPREPLY
expr_stmt|;
name|icp
operator|->
name|icmp_rtime
operator|=
name|iptime
argument_list|()
expr_stmt|;
name|icp
operator|->
name|icmp_ttime
operator|=
name|icp
operator|->
name|icmp_rtime
expr_stmt|;
comment|/* bogus, do later! */
goto|goto
name|reflect
goto|;
case|case
name|ICMP_IREQ
case|:
define|#
directive|define
name|satosin
parameter_list|(
name|sa
parameter_list|)
value|((struct sockaddr_in *)(sa))
if|if
condition|(
name|in_netof
argument_list|(
name|ip
operator|->
name|ip_src
argument_list|)
operator|==
literal|0
operator|&&
operator|(
name|ia
operator|=
name|ifptoia
argument_list|(
name|ifp
argument_list|)
operator|)
condition|)
name|ip
operator|->
name|ip_src
operator|=
name|in_makeaddr
argument_list|(
name|in_netof
argument_list|(
name|IA_SIN
argument_list|(
name|ia
argument_list|)
operator|->
name|sin_addr
argument_list|)
argument_list|,
name|in_lnaof
argument_list|(
name|ip
operator|->
name|ip_src
argument_list|)
argument_list|)
expr_stmt|;
name|icp
operator|->
name|icmp_type
operator|=
name|ICMP_IREQREPLY
expr_stmt|;
goto|goto
name|reflect
goto|;
case|case
name|ICMP_MASKREQ
case|:
if|if
condition|(
name|icmplen
operator|<
name|ICMP_MASKLEN
operator|||
operator|(
name|ia
operator|=
name|ifptoia
argument_list|(
name|ifp
argument_list|)
operator|)
operator|==
literal|0
condition|)
break|break;
name|icp
operator|->
name|icmp_type
operator|=
name|ICMP_MASKREPLY
expr_stmt|;
name|icp
operator|->
name|icmp_mask
operator|=
name|ia
operator|->
name|ia_netmask
expr_stmt|;
if|if
condition|(
name|ip
operator|->
name|ip_src
operator|.
name|s_addr
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|ia
operator|->
name|ia_ifp
operator|->
name|if_flags
operator|&
name|IFF_BROADCAST
condition|)
name|ip
operator|->
name|ip_src
operator|=
name|satosin
argument_list|(
operator|&
name|ia
operator|->
name|ia_broadaddr
argument_list|)
operator|->
name|sin_addr
expr_stmt|;
elseif|else
if|if
condition|(
name|ia
operator|->
name|ia_ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
condition|)
name|ip
operator|->
name|ip_src
operator|=
name|satosin
argument_list|(
operator|&
name|ia
operator|->
name|ia_dstaddr
argument_list|)
operator|->
name|sin_addr
expr_stmt|;
block|}
name|reflect
label|:
name|ip
operator|->
name|ip_len
operator|+=
name|hlen
expr_stmt|;
comment|/* since ip_input deducts this */
name|icmpstat
operator|.
name|icps_reflect
operator|++
expr_stmt|;
name|icmpstat
operator|.
name|icps_outhist
index|[
name|icp
operator|->
name|icmp_type
index|]
operator|++
expr_stmt|;
name|icmp_reflect
argument_list|(
name|ip
argument_list|,
name|ifp
argument_list|)
expr_stmt|;
return|return;
case|case
name|ICMP_REDIRECT
case|:
if|if
condition|(
name|icmplen
operator|<
name|ICMP_ADVLENMIN
operator|||
name|icmplen
operator|<
name|ICMP_ADVLEN
argument_list|(
name|icp
argument_list|)
condition|)
block|{
name|icmpstat
operator|.
name|icps_badlen
operator|++
expr_stmt|;
break|break;
block|}
comment|/* 		 * Short circuit routing redirects to force 		 * immediate change in the kernel's routing 		 * tables.  The message is also handed to anyone 		 * listening on a raw socket (e.g. the routing 		 * daemon for use in updating its tables). 		 */
name|icmpgw
operator|.
name|sin_addr
operator|=
name|ip
operator|->
name|ip_src
expr_stmt|;
name|icmpdst
operator|.
name|sin_addr
operator|=
name|icp
operator|->
name|icmp_gwaddr
expr_stmt|;
ifdef|#
directive|ifdef
name|ICMPPRINTFS
if|if
condition|(
name|icmpprintfs
condition|)
name|printf
argument_list|(
literal|"redirect dst %x to %x\n"
argument_list|,
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_dst
argument_list|,
name|icp
operator|->
name|icmp_gwaddr
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|code
operator|==
name|ICMP_REDIRECT_NET
operator|||
name|code
operator|==
name|ICMP_REDIRECT_TOSNET
condition|)
block|{
name|icmpsrc
operator|.
name|sin_addr
operator|=
name|in_makeaddr
argument_list|(
name|in_netof
argument_list|(
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_dst
argument_list|)
argument_list|,
name|INADDR_ANY
argument_list|)
expr_stmt|;
name|rtredirect
argument_list|(
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpsrc
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpdst
argument_list|,
name|RTF_GATEWAY
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpgw
argument_list|)
expr_stmt|;
name|icmpsrc
operator|.
name|sin_addr
operator|=
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_dst
expr_stmt|;
name|pfctlinput
argument_list|(
name|PRC_REDIRECT_NET
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpsrc
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|icmpsrc
operator|.
name|sin_addr
operator|=
name|icp
operator|->
name|icmp_ip
operator|.
name|ip_dst
expr_stmt|;
name|rtredirect
argument_list|(
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpsrc
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpdst
argument_list|,
name|RTF_GATEWAY
operator||
name|RTF_HOST
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpgw
argument_list|)
expr_stmt|;
name|pfctlinput
argument_list|(
name|PRC_REDIRECT_HOST
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpsrc
argument_list|)
expr_stmt|;
block|}
break|break;
comment|/* 	 * No kernel processing for the following; 	 * just fall through to send to raw listener. 	 */
case|case
name|ICMP_ECHOREPLY
case|:
case|case
name|ICMP_TSTAMPREPLY
case|:
case|case
name|ICMP_IREQREPLY
case|:
case|case
name|ICMP_MASKREPLY
case|:
default|default:
break|break;
block|}
name|raw
label|:
name|icmpsrc
operator|.
name|sin_addr
operator|=
name|ip
operator|->
name|ip_src
expr_stmt|;
name|icmpdst
operator|.
name|sin_addr
operator|=
name|ip
operator|->
name|ip_dst
expr_stmt|;
name|raw_input
argument_list|(
name|m
argument_list|,
operator|&
name|icmproto
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpsrc
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|icmpdst
argument_list|)
expr_stmt|;
return|return;
name|free
label|:
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Reflect the ip packet back to the source  */
end_comment

begin_expr_stmt
name|icmp_reflect
argument_list|(
name|ip
argument_list|,
name|ifp
argument_list|)
specifier|register
expr|struct
name|ip
operator|*
name|ip
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|in_ifaddr
modifier|*
name|ia
decl_stmt|;
name|struct
name|in_addr
name|t
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|opts
init|=
literal|0
decl_stmt|,
modifier|*
name|ip_srcroute
argument_list|()
decl_stmt|;
name|int
name|optlen
init|=
operator|(
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
operator|)
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
decl_stmt|;
name|t
operator|=
name|ip
operator|->
name|ip_dst
expr_stmt|;
name|ip
operator|->
name|ip_dst
operator|=
name|ip
operator|->
name|ip_src
expr_stmt|;
comment|/* 	 * If the incoming packet was addressed directly to us, 	 * use dst as the src for the reply.  Otherwise (broadcast 	 * or anonymous), use the address which corresponds 	 * to the incoming interface. 	 */
for|for
control|(
name|ia
operator|=
name|in_ifaddr
init|;
name|ia
condition|;
name|ia
operator|=
name|ia
operator|->
name|ia_next
control|)
block|{
if|if
condition|(
name|t
operator|.
name|s_addr
operator|==
name|IA_SIN
argument_list|(
name|ia
argument_list|)
operator|->
name|sin_addr
operator|.
name|s_addr
condition|)
break|break;
if|if
condition|(
operator|(
name|ia
operator|->
name|ia_ifp
operator|->
name|if_flags
operator|&
name|IFF_BROADCAST
operator|)
operator|&&
name|t
operator|.
name|s_addr
operator|==
name|satosin
argument_list|(
operator|&
name|ia
operator|->
name|ia_broadaddr
argument_list|)
operator|->
name|sin_addr
operator|.
name|s_addr
condition|)
break|break;
block|}
if|if
condition|(
name|ia
operator|==
operator|(
expr|struct
name|in_ifaddr
operator|*
operator|)
literal|0
condition|)
name|ia
operator|=
name|ifptoia
argument_list|(
name|ifp
argument_list|)
expr_stmt|;
if|if
condition|(
name|ia
operator|==
operator|(
expr|struct
name|in_ifaddr
operator|*
operator|)
literal|0
condition|)
name|ia
operator|=
name|in_ifaddr
expr_stmt|;
name|t
operator|=
name|IA_SIN
argument_list|(
name|ia
argument_list|)
operator|->
name|sin_addr
expr_stmt|;
name|ip
operator|->
name|ip_src
operator|=
name|t
expr_stmt|;
if|if
condition|(
name|optlen
operator|>
literal|0
condition|)
block|{
comment|/* 		 * Retrieve any source routing from the incoming packet 		 * and strip out other options.  Adjust the IP length. 		 */
name|opts
operator|=
name|ip_srcroute
argument_list|()
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|-=
name|optlen
expr_stmt|;
name|ip_stripoptions
argument_list|(
name|ip
argument_list|,
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
block|}
name|icmp_send
argument_list|(
name|ip
argument_list|,
name|opts
argument_list|)
expr_stmt|;
if|if
condition|(
name|opts
condition|)
operator|(
name|void
operator|)
name|m_free
argument_list|(
name|opts
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
name|struct
name|in_ifaddr
modifier|*
name|ifptoia
parameter_list|(
name|ifp
parameter_list|)
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
block|{
specifier|register
name|struct
name|in_ifaddr
modifier|*
name|ia
decl_stmt|;
for|for
control|(
name|ia
operator|=
name|in_ifaddr
init|;
name|ia
condition|;
name|ia
operator|=
name|ia
operator|->
name|ia_next
control|)
if|if
condition|(
name|ia
operator|->
name|ia_ifp
operator|==
name|ifp
condition|)
return|return
operator|(
name|ia
operator|)
return|;
return|return
operator|(
operator|(
expr|struct
name|in_ifaddr
operator|*
operator|)
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Send an icmp packet back to the ip level,  * after supplying a checksum.  */
end_comment

begin_expr_stmt
name|icmp_send
argument_list|(
name|ip
argument_list|,
name|opts
argument_list|)
specifier|register
expr|struct
name|ip
operator|*
name|ip
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|opts
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|hlen
decl_stmt|;
specifier|register
name|struct
name|icmp
modifier|*
name|icp
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|m
operator|=
name|dtom
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|hlen
operator|=
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
expr_stmt|;
name|m
operator|->
name|m_off
operator|+=
name|hlen
expr_stmt|;
name|m
operator|->
name|m_len
operator|-=
name|hlen
expr_stmt|;
name|icp
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|icmp
operator|*
argument_list|)
expr_stmt|;
name|icp
operator|->
name|icmp_cksum
operator|=
literal|0
expr_stmt|;
name|icp
operator|->
name|icmp_cksum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|ip
operator|->
name|ip_len
operator|-
name|hlen
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_off
operator|-=
name|hlen
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
name|hlen
expr_stmt|;
ifdef|#
directive|ifdef
name|ICMPPRINTFS
if|if
condition|(
name|icmpprintfs
condition|)
name|printf
argument_list|(
literal|"icmp_send dst %x src %x\n"
argument_list|,
name|ip
operator|->
name|ip_dst
argument_list|,
name|ip
operator|->
name|ip_src
argument_list|)
expr_stmt|;
endif|#
directive|endif
operator|(
name|void
operator|)
name|ip_output
argument_list|(
name|m
argument_list|,
name|opts
argument_list|,
operator|(
expr|struct
name|route
operator|*
operator|)
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
name|n_time
name|iptime
parameter_list|()
block|{
name|struct
name|timeval
name|atv
decl_stmt|;
name|u_long
name|t
decl_stmt|;
name|microtime
argument_list|(
operator|&
name|atv
argument_list|)
expr_stmt|;
name|t
operator|=
operator|(
name|atv
operator|.
name|tv_sec
operator|%
operator|(
literal|24
operator|*
literal|60
operator|*
literal|60
operator|)
operator|)
operator|*
literal|1000
operator|+
name|atv
operator|.
name|tv_usec
operator|/
literal|1000
expr_stmt|;
return|return
operator|(
name|htonl
argument_list|(
name|t
argument_list|)
operator|)
return|;
block|}
end_function

end_unit

