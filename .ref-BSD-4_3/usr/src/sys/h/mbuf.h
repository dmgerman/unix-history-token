...
begin_comment
comment|/*  * Copyright (c) 1982, 1986 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)mbuf.h	7.3 (Berkeley) 9/11/86  */
end_comment
...
parameter_list|)
define|\
value|{ struct mbuf *p; \ 	  MCLALLOC(p, 1); \ 	  if (p) { \ 		(m)->m_off = (int)p - (int)(m); \ 		(m)->m_len = CLBYTES; \ 	  } \ 	}
end_define
