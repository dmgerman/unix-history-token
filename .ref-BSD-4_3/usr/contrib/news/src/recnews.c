...
name|SccsId
init|=
literal|"@(#)recnews.c	2.11	3/19/86"
decl_stmt|;
end_decl_stmt
...
argument_list|)
condition|)
comment|/* 		 * Some arpanet mail comes from "joe at mit-dms" 		 * instead of "joe@mit-dms", so handle it here. 		 */
name|sprintf
argument_list|(
name|fbuf
argument_list|,
...
directive|ifdef
name|fussy
comment|/* 	 * Default unknown "to" fields to "general".  This gives you 	 * tight control over which newsgroups exist. 	 */
if|if
condition|(
name|strcmp
argument_list|(
...
directive|else
comment|/* 	 * Allow any newsgroup.  This way you don't have to recompile 	 * recnews every time you add a newsgroup. 	 */
name|strcpy
argument_list|(
name|group
argument_list|,
