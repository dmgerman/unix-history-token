...
begin_comment
comment|/*  *  maharani - a czarina-like program that generates interpress files  *  *  Written for Xerox Corporation by William LeFebvre  *  *  Copyright (c)  1984, 1985 Xerox Corporation  *  *  HISTORY  * 13-Jan-86  lee at Xerox, WRC  *	Changed a call to strcpyn to strncpy.  *  *    8-apr-85  ed flint	conditional compilation for vax11-c (vms)  *    26-mar-85  ed @ Xerox, WRC  *	add fclose after do_file to prevent running out of open file descriptors  */
end_comment
...
name|ptr
decl_stmt|;
comment|/* this works with max_alloc = hi_water = NULL, although we probably        shouldn't depend on that! */
if|if
condition|(
name|max_alloc
operator|+
