...
operator|>=
literal|200080000
comment|/* 	 * ensure that mbufs are writable beforehand 	 * as it's assumed by ipf code. 	 * XXX inefficient 	 */
name|int
name|error
init|=
name|m_makewritable
...
directive|endif
comment|/* INET */
comment|/* 	 * Note, we don't need to update the checksum, because 	 * it has already been verified. 	 */
name|rv
operator|=
name|ipf_check
argument_list|(
...
decl_stmt|;
block|{
comment|/* 	 * The interface pointer is useless for create (we have nothing to 	 * compare it to) and at detach, the interface name is still in the 	 * list of active NICs (albeit, down, but that's not any real 	 * indicator) and doing ifunit() on the name will still return the 	 * pointer, so it's not much use then, either. 	 */
name|ipf_sync
argument_list|(
operator|&
name|ipfmain
...
begin_comment
comment|/* __NetBSD_Version__>= 105110000 */
end_comment
...
name|l
parameter_list|)
value|kauth_cred_getuid((l)->l_cred)
else|#
directive|else
define|#
directive|define
...
comment|/* XXX: no way to return error */
return|return
literal|0
return|;
return|return
