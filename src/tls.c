...
name|NOQID
argument_list|,
literal|"STARTTLS=init, CertFingerprintAlgorithm=%s, status=invalid"
argument_list|,
name|CertFingerprintAlgorithm
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* **  TLS_SET_VERIFY -- request client certificate? ** **	Parameters: **		ctx -- TLS context **		ssl -- TLS structure **		vrfy -- request certificate? ** **	Returns: **		none. ** **	Side Effects: **		Sets verification state for TLS ** # if TLS_VRFY_PER_CTX **	Notice: **		This is per TLS context, not per TLS structure; **		the former is global, the latter per connection. **		It would be nice to do this per connection, but this **		doesn't work in the current TLS libraries :-( # endif * TLS_VRFY_PER_CTX * */
end_comment
...
begin_comment
comment|/* **  LOAD_CERTKEY -- load cert/key for TLS session ** **	Parameters: **		ssl -- TLS session context **		certfile -- filename of certificate **		keyfile -- filename of private key ** **	Returns: **		succeeded? */
end_comment
...
parameter_list|)
define|\
value|SSL_use_certificate_file(ssl, certfile, SSL_FILETYPE_PEM)
define|#
directive|define
name|SSL_USE_CERT
value|"SSL_use_certificate_file"
if|if
condition|(
name|bitset
argument_list|(
...
name|NOQID
argument_list|,
literal|"STARTTLS=%s, error: %s(%s) failed"
argument_list|,
name|who
argument_list|,
name|SSL_USE_CERT
...
name|NOQID
argument_list|,
literal|"STARTTLS=%s, error: SSL_use_PrivateKey_file(%s) failed"
argument_list|,
name|who
argument_list|,
name|keyfile
...
name|NOQID
argument_list|,
literal|"STARTTLS=%s, error: SSL_check_private_key failed(%s): %d"
argument_list|,
name|who
argument_list|,
name|keyfile
...
expr_stmt|;
block|}
comment|/* 	**  valid values for dhparam are (only the first char is checked) 	**  none	no parameters: don't use DH 	**  i		use precomputed 2048 bit parameters 	**  512		use precomputed 512 bit parameters 	**  1024	generate 1024 bit parameters 	**  2048	generate 2048 bit parameters 	**  /file/name	read parameters from /file/name 	*/
define|#
directive|define
name|SET_DH_DFL
define|\
value|do {	\ 		dhparam = "I";	\ 		req |= TLS_I_DHFIXED;	\ 	} while (0)
if|if
condition|(
name|bitset
argument_list|(
...
name|NOQID
argument_list|,
literal|"STARTTLS=%s, error: illegal value '%s' for DHParameters"
argument_list|,
name|who
argument_list|,
name|dhparam
...
parameter_list|)
define|\
value|SSL_CTX_use_certificate_chain_file(ssl_ctx, certfile)
define|#
directive|define
name|SSL_CTX_USE_CERT
value|"SSL_CTX_use_certificate_chain_file"
else|#
directive|else
define|#
directive|define
...
parameter_list|)
define|\
value|SSL_CTX_use_certificate_file(ssl_ctx, certfile, SSL_FILETYPE_PEM)
define|#
directive|define
name|SSL_CTX_USE_CERT
value|"SSL_CTX_use_certificate_file"
endif|#
directive|endif
...
name|sm_dprintf
argument_list|(
literal|"inittls: req=%#lx, status=%#lx\n"
argument_list|,
name|req
argument_list|,
name|status
...
name|sm_dprintf
argument_list|(
literal|"inittls: Using precomputed 2048 bit DH parameters\n"
argument_list|)
expr_stmt|;
name|dh
operator|=
...
begin_comment
comment|/* **  CERT_FP -- get cert fingerprint ** **	Parameters: **		cert -- TLS cert **		mac -- macro storage **		macro -- where to store cert fp ** **	Returns: **<=0: cert fp calculation failed **>0: cert fp calculation ok */
end_comment
...
begin_comment
comment|/* **  TLS_GET_INFO -- get information about TLS connection ** **	Parameters: **		ssl -- TLS connection structure **		srv -- server or client **		host -- hostname of other side **		mac -- macro storage **		certreq -- did we ask for a cert? ** **	Returns: **		result of authentication. ** **	Side Effects: **		sets various TLS related macros. */
end_comment
...
directive|define
name|CERTFPMACRO
value|(CertFingerprintAlgorithm != NULL ? "{cert_fp}" : "{cert_md5}")
define|#
directive|define
...
begin_comment
comment|/* **  TLS_VERIFY_LOG -- log verify error for TLS certificates ** **	Parameters: **		ok -- verify ok? **		ctx -- x509 context **		name -- from where is this called? ** **	Returns: **		1 -- ok */
end_comment
