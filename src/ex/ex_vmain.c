...
name|reread
label|:
comment|/* 		 * Come to reread from below after some macro expansions. 		 * The call to map allows use of function key pads 		 * by performing a terminal dependent mapping of inputs, 		 * but notably not of two character inputs starting with 		 * an escape (as from a VT52 keypad).  This is useful for 		 * e.g. DM1520's however. 		 */
name|op
operator|=
name|c
operator|=
...
expr_stmt|;
continue|continue;
comment|/* 		 * $		Escape just cancels the current command 		 *		with a little feedback.  It would be 		 *		nice to handle function pads sending ESC-X; 		 *		here, using something else to replace ESC. 		 */
case|case
name|ESCAPE
case|:
name|beep
...
expr_stmt|;
continue|continue;
comment|/* 		 * ^^		Return to previous context.  Like a 't 		 *		if that mark is set since tag sets that 		 *		mark if it stays in same file.  Else 		 *		like a :e #, and thus can be used after a 		 *		"No Write" diagnostic. 		 */
case|case
name|CTRL
argument_list|(
operator|^
