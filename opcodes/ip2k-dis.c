...
define|#
directive|define
name|PRINT_FUNC_DECL
parameter_list|(
name|name
parameter_list|)
define|\
value|static void name PARAMS ((CGEN_CPU_DESC, PTR, long, unsigned int, bfd_vma, int))
end_define
...
literal|0x7F
expr_stmt|;
comment|/* Check to see if first two bits are 10 -> (DP) */
if|if
condition|(
name|offsettest
operator|==
...
return|return;
block|}
comment|/* Check to see if first two bits are 11 -> (SP) */
if|if
condition|(
name|offsettest
operator|==
...
return|return;
block|}
comment|/* Attempt to print as a register keyword. */
name|ke
operator|=
...
return|return;
block|}
comment|/* Print as an address literal. */
call|(
modifier|*
name|info
operator|->
...
name|dis_info
decl_stmt|;
comment|/* This is a loadh instruction. Shift the value to the left      */
comment|/* by 8 bits so that disassembled code will reassemble properly. */
name|value
operator|=
operator|(
operator|(
