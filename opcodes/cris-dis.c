...
begin_comment
comment|/* Disassembler code for CRIS.    Copyright 2000, 2001, 2002 Free Software Foundation, Inc.    Contributed by Axis Communications AB, Lund, Sweden.    Written by Hans-Peter Nilsson.  This file is part of the GNU binutils and GDB, the GNU debugger.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Return the descriptor of a special register.    FIXME: Depend on a CPU-version specific argument when all machinery    is in place.  */
end_comment
...
decl_stmt|;
block|{
comment|/* Obfuscate to avoid warning on 32-bit host, but properly truncate      negative numbers on>32-bit hosts.  */
if|if
condition|(
sizeof|sizeof
argument_list|(
...
literal|'!'
case|:
comment|/* Do not recognize "pop" if there's a prefix.  */
if|if
condition|(
name|prefix_insn
operator|!=
...
literal|2
return|;
comment|/* PC is incremented by two, not one, for a byte.  */
name|to_skip
operator|+=
operator|(
name|sregp
...
decl_stmt|;
block|{
comment|/* Use the v8 (Etrax 100) flag definitions for disassembly.      The differences with v0 (Etrax 1..4) vs. Svinto are:      v0 'd'<=> v8 'm'      v0 'e'<=> v8 'b'.  */
specifier|static
specifier|const
name|char
name|fnames
...
literal|'!'
case|:
comment|/* Ignore at this point; used at earlier stages to avoid recognition 	   if there's a prefixes at something that in other ways looks like 	   a "pop".  */
break|break;
case|case
literal|'B'
case|:
...
comment|/* PC is always incremented by a multiple of two.  */
name|nbytes
operator|=
operator|(
name|sregp
...
begin_comment
comment|/* Disassemble, prefixing register names with `$'.  */
end_comment
...
begin_comment
comment|/* Disassemble, no prefixes on register names.  */
end_comment
...
begin_comment
comment|/* Return a disassembler-function that prints registers with a `$' prefix,    or one that prints registers without a prefix.  */
end_comment
...
decl_stmt|;
block|{
comment|/* If there's no bfd in sight, we return what is valid as input in all      contexts if fed back to the assembler: disassembly *with* register      prefix.  */
if|if
condition|(
name|abfd
operator|==
