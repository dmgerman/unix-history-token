...
directive|if
literal|0
block|errmsg = cgen_parse_signed_integer (cd, strp, opindex, valuep);       if (errmsg != NULL)         fprintf (stderr, "parse_hi: %s\n", errmsg);       if (errmsg != NULL)
endif|#
directive|endif
name|errmsg
operator|=
...
directive|if
literal|0
block|errmsg = cgen_parse_signed_integer (cd, strp, opindex, valuep);       if (errmsg != NULL)         fprintf (stderr, "parse_lo: %s\n", errmsg);        if (errmsg != NULL)
endif|#
directive|endif
name|errmsg
operator|=
...
begin_if
unit|void openrisc_cgen_asm_hash_keywords (CGEN_CPU_DESC cd, CGEN_KEYWORD *opvals) {   CGEN_KEYWORD_SEARCH search = cgen_keyword_search_init (opvals, NULL);   const CGEN_KEYWORD_ENTRY * ke;    while ((ke = cgen_keyword_search_next (& search)) != NULL)     {
if|#
directive|if
literal|0
end_if
...
begin_endif
unit|if (! openrisc_cgen_opval_supported (ke)) 	continue;
endif|#
directive|endif
end_endif
