...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright (c) 2013, Joyent, Inc. All rights reserved.  * Copyright (c) 2014 by Delphix. All rights reserved.  */
end_comment
...
begin_comment
comment|/*  * zfs_get_hole_count retrieves the number of holes (blocks which are  * zero-filled) in the specified file using the _FIO_COUNT_FILLED ioctl. It  * also optionally fetches the block size when bs is non-NULL. With hole count  * and block size the full space consumed by the holes of a file can be  * calculated.  *  * On success, zero is returned, the count argument is set to the  * number of holes, and the bs argument is set to the block size (if it is  * not NULL). On error, a non-zero errno is returned and the values in count  * and bs are undefined.  */
end_comment
