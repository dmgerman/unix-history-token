...
condition|)
break|break;
comment|/* 		 * Search for the requested value. Special cases: 		 * 		 * - ZPOOL_CONFIG_PATH for whole disk entries. To support 		 *   UEFI boot, these end in "s0" or "s0/old" or "s1" or 		 *   "s1/old".   The "s0" or "s1" part is hidden from the user, 		 *   but included in the string, so this matches around it. 		 * - looking for a top-level vdev name (i.e. ZPOOL_CONFIG_TYPE). 		 * 		 * Otherwise, all other searches are simple string compares. 		 */
if|if
condition|(
name|strcmp
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* 			 * If it starts with c#, and ends with "s0" or "s1", 			 * chop the slice off, or if it ends with "s0/old" or 			 * "s1/old", remove the slice from the middle. 			 */
if|if
condition|(
name|CTD_CHECK
argument_list|(
...
begin_comment
comment|/*  * Read the EFI label from the config, if a label does not exist then  * pass back the error to the caller. If the caller has passed a non-NULL  * diskaddr argument then we set it to the starting address of the EFI  * partition. If the caller has passed a non-NULL boolean argument, then  * we set it to indicate if the disk does have efi system partition.  */
end_comment
...
comment|/* first fix the partition start block */
if|if
condition|(
name|start_block
operator|==
...
name|NEW_START_BLOCK
expr_stmt|;
comment|/* 	 * EFI System partition is using slice 0. 	 * ZFS is on slice 1 and slice 8 is reserved. 	 * We assume the GPT partition table without system 	 * partition has zfs p_start == NEW_START_BLOCK. 	 * If start_block != NEW_START_BLOCK, it means we have 	 * system partition. Correct solution would be to query/cache vtoc 	 * from existing vdev member. 	 */
if|if
condition|(
name|boot_type
operator|==
...
name|TEXT_DOMAIN
argument_list|,
literal|"boot partition size must be a multiple of %d"
argument_list|)
argument_list|,
name|vtoc
operator|->
...
return|;
block|}
comment|/* 		 * System partition size checks. 		 * Note the 1MB is quite arbitrary value, since we 		 * are creating dedicated pool, it should be enough 		 * to hold fat + efi bootloader. May need to be 		 * adjusted if the bootloader size will grow. 		 */
if|if
condition|(
name|boot_size
operator|<
...
name|TEXT_DOMAIN
argument_list|,
literal|"Specified size %s for EFI System partition is too "
literal|"small, the minimum size is 1MB."
argument_list|)
argument_list|,
name|buf
argument_list|)
...
return|;
block|}
comment|/* 33MB is tested with mkfs -F pcfs */
if|if
condition|(
name|hdl
operator|->
...
name|TEXT_DOMAIN
argument_list|,
literal|"Warning: EFI System partition size %s is "
literal|"not allowing to create FAT32 file\nsystem, which "
literal|"may result in unbootable system.\n"
argument_list|)
argument_list|,
name|buf
argument_list|)
expr_stmt|;
block|}
comment|/* Adjust zfs partition start by size of system partition. */
name|start_block
operator|+=
name|boot_size
operator|/
...
condition|)
block|{
comment|/* 		 * Use default layout. 		 * ZFS is on slice 0 and slice 8 is reserved. 		 */
name|slice_size
operator|=
name|vtoc
operator|->
