...
argument_list|)
return|;
comment|/* not compatible with static DCtx */
block|{
name|ZSTD_customMem
specifier|const
name|cMem
...
begin_comment
comment|/** ZSTD_getFrameHeader() : *   decode Frame Header, or require larger `srcSize`. *   @return : 0, `zfhPtr` is correctly filled, *>0, `srcSize` is too small, result is expected `srcSize`, *             or an error code, which can be tested using ZSTD_isError() */
end_comment
...
name|ZSTD_skippableHeaderSize
return|;
comment|/* magic number + frame length */
name|memset
argument_list|(
name|zfhPtr
argument_list|,
...
literal|6
argument_list|,
literal|"seq: litL=%u, matchL=%u, offset=%u"
argument_list|,
operator|(
name|U32
operator|)
...
literal|5
argument_list|,
literal|"ZSTD_decompressSequences"
argument_list|)
expr_stmt|;
...
literal|5
argument_list|,
literal|"ZSTD_decodeSeqHeaders: size=%u, nbSeq=%i"
argument_list|,
operator|(
name|U32
operator|)
...
literal|6
argument_list|,
literal|"regenerated sequence size : %u"
argument_list|,
operator|(
name|U32
operator|)
...
literal|5
argument_list|,
literal|"after decode loop, remaining nbSeq : %i"
argument_list|,
name|nbSeq
argument_list|)
expr_stmt|;
...
literal|5
argument_list|,
literal|"ZSTD_decompressBlock_internal"
argument_list|)
expr_stmt|;
if|if
condition|(
...
literal|5
argument_list|,
literal|"ZSTD_decodeLiteralsBlock : %u"
argument_list|,
operator|(
name|U32
operator|)
...
begin_comment
comment|/** ZSTD_decompressContinue() :  *  srcSize : must be the exact nb of bytes expected (see ZSTD_nextSrcSizeToDecompress())  *  @return : nb of bytes generated into `dst` (necessarily<= `dstCapacity)  *            or an error code, which can be tested using ZSTD_isError() */
end_comment
...
literal|5
argument_list|,
literal|"case ZSTDds_decompressBlock"
argument_list|)
expr_stmt|;
block|{
name|size_t
...
begin_comment
comment|/*! ZSTD_estimateDDictSize() :  *  Estimate amount of memory that will be needed to create a dictionary for decompression.  *  Note : dictionary created "byReference" are smaller */
end_comment
...
name|blockSize
decl_stmt|;
comment|/* no block can be larger */
name|size_t
specifier|const
name|outBuffSize
init|=
...
block|}
block|}
comment|/* Consume header (see ZSTDds_decodeFrameHeader) */
name|DEBUGLOG
argument_list|(
literal|4
argument_list|,
literal|"Consume header"
argument_list|)
expr_stmt|;
name|CHECK_F
argument_list|(
...
expr_stmt|;
block|}
comment|/* control buffer memory usage */
name|DEBUGLOG
argument_list|(
literal|4
argument_list|,
literal|"Control max buffer memory usage"
argument_list|)
expr_stmt|;
name|zds
operator|->
...
block|}
comment|/* zds->hostageByte */
return|return
literal|0
return|;
block|}
comment|/* zds->outEnd == zds->outStart */
if|if
condition|(
operator|!
name|zds
