...
begin_comment
comment|//===------- TreeTransform.h - Semantic Tree Transformation ---------------===/
end_comment
...
comment|/// appropriate TransformXXXType to build a new type, then applying
comment|/// the qualifiers on \p T to the resulting type with AddTypeQualifiers.
comment|/// Subclasses may override this function (to take over all type
comment|/// transformations), some set of the TransformXXXType functions, or
comment|/// the AddTypeQualifiers function to alter the transformation.
comment|///
...
parameter_list|)
function_decl|;
comment|/// \brief Transform the given type by adding the given set of qualifiers
comment|/// and returning the result.
comment|///
comment|/// FIXME: By default, this routine adds type qualifiers only to types that
comment|/// can have qualifiers, and silently suppresses those qualifiers that are
comment|/// not permitted (e.g., qualifiers on reference or function types). This
comment|/// is the right thing for template instantiation, but probably not for
...
parameter_list|)
define|\
value|QualType Transform##CLASS##Type(const CLASS##Type *T);
include|#
directive|include
...
decl_stmt|;
comment|/// \brief Build a new constant array type given the element type, size
comment|/// modifier, (known) size of the array, size expression, and index type
comment|/// qualifiers.
comment|///
comment|/// By default, performs semantic analysis when building the array type.
comment|/// Subclasses may override this routine to provide different behavior.
name|QualType
name|RebuildConstantArrayWithExprType
argument_list|(
name|QualType
name|ElementType
argument_list|,
...
comment|/// Subclasses may override this routine to provide different behavior.
name|QualType
name|RebuildConstantArrayWithoutExprType
argument_list|(
name|QualType
name|ElementType
argument_list|,
...
function_decl|;
comment|/// \brief Build a new template name given a nested name specifier, a flag
comment|/// indicating whether the "template" keyword was provided, and a set of
comment|/// overloaded function templates.
comment|///
comment|/// By default, builds the new template name directly. Subclasses may override
comment|/// this routine to provide different behavior.
name|TemplateName
...
return|;
block|}
comment|/// \brief Build a new implicit cast expression.
comment|///
comment|/// By default, builds a new implicit cast without any semantic analysis.
comment|/// Subclasses may override this routine to provide different behavior.
name|OwningExprResult
name|RebuildImplicitCastExpr
argument_list|(
name|QualType
name|T
argument_list|,
...
return|;
block|}
comment|/// \brief Build a new C++ conditional declaration expression.
comment|///
comment|/// By default, performs semantic analysis to build the new expression.
comment|/// Subclasses may override this routine to provide different behavior.
name|OwningExprResult
name|RebuildCXXConditionDeclExpr
parameter_list|(
name|SourceLocation
name|StartLoc
parameter_list|,
...
comment|/// Subclasses may override this routine to provide different behavior.
name|OwningExprResult
name|RebuildUnresolvedDeclRefExpr
parameter_list|(
name|NestedNameSpecifier
modifier|*
name|NNS
...
comment|/// Subclasses may override this routine to provide different behavior.
name|OwningExprResult
name|RebuildCXXUnresolvedMemberExpr
parameter_list|(
name|ExprArg
name|BaseE
parameter_list|,
...
return|;
block|}
comment|/// \brief Build a new member reference expression with explicit template
comment|/// arguments.
comment|///
comment|/// By default, performs semantic analysis to build the new expression.
comment|/// Subclasses may override this routine to provide different behavior.
...
name|Ovl
operator|&&
literal|"Not a template name or an overload set?"
argument_list|)
expr_stmt|;
end_expr_stmt
...
parameter_list|)
define|\
value|case Type::CLASS:                                                        \       Result = getDerived().Transform##CLASS##Type(                          \                                        static_cast<const CLASS##Type*>(Ty)); \       break;
include|#
directive|include
file|"clang/AST/TypeNodes.def"
block|}
...
argument_list|(
argument|const FixedWidthIntType *T
argument_list|)
block|{
...
argument_list|(
argument|const BlockPointerType *T
argument_list|)
block|{
name|QualType
name|PointeeType
...
argument_list|(
argument|const LValueReferenceType *T
argument_list|)
block|{
name|QualType
name|PointeeType
...
argument_list|(
argument|const RValueReferenceType *T
argument_list|)
block|{
name|QualType
name|PointeeType
...
argument_list|(
argument|const MemberPointerType *T
argument_list|)
block|{
name|QualType
name|PointeeType
...
argument_list|(
argument|const ConstantArrayType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
operator|>
operator|::
name|TransformConstantArrayWithExprType
argument_list|(
argument|const ConstantArrayWithExprType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
operator|>
operator|::
name|TransformConstantArrayWithoutExprType
argument_list|(
argument|const ConstantArrayWithoutExprType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
argument_list|(
argument|const IncompleteArrayType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
argument_list|(
argument|const VariableArrayType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
argument_list|(
argument|const DependentSizedArrayType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
argument_list|(
argument|const DependentSizedExtVectorType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
argument_list|(
argument|const ExtVectorType *T
argument_list|)
block|{
name|QualType
name|ElementType
...
argument_list|(
argument|const FunctionProtoType *T
argument_list|)
block|{
name|QualType
name|ResultType
...
argument_list|(
argument|const FunctionNoProtoType *T
argument_list|)
block|{
comment|// FIXME: Implement
return|return
...
argument_list|(
argument|const TypeOfExprType *T
argument_list|)
block|{
...
argument_list|(
argument|const ObjCInterfaceType *T
argument_list|)
block|{
comment|// FIXME: Implement
return|return
...
argument_list|(
argument|const ObjCObjectPointerType *T
argument_list|)
block|{
comment|// FIXME: Implement
return|return
...
operator|>
operator|::
name|TransformObjCProtocolListType
argument_list|(
argument|const ObjCProtocolListType *T
argument_list|)
block|{
name|assert
argument_list|(
name|false
operator|&&
literal|"Should not see ObjCProtocolList types"
argument_list|)
block|;
return|return
name|QualType
...
begin_comment
comment|// FIXME: Would like to avoid rebuilding if nothing changed, but we can't
end_comment

begin_comment
comment|// compare template arguments (yet).
end_comment

begin_comment
comment|// FIXME: It's possible that we'll find out now that the template name
end_comment

begin_comment
comment|// actually refers to a type, in which case the caller is actually dealing
end_comment

begin_comment
comment|// with a functional cast. Give a reasonable error message!
end_comment
...
begin_comment
comment|// FIXME: The first qualifier found might be a template type parameter,
end_comment

begin_comment
comment|// in which case there is no transformed declaration to refer to (it might
end_comment

begin_comment
comment|// refer to a built-in type!).
end_comment
...
begin_comment
comment|// FIXME: This is an ugly hack, which forces the same template name to
end_comment
...
argument|bool TemplateKW
argument_list|,
argument|OverloadedFunctionDecl *Ovl
argument_list|)
block|{
return|return
name|SemaRef
