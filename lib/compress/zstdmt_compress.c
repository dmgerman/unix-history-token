...
define|#
directive|define
name|ZSTDMT_OVERLAPLOG_DEFAULT
value|6
end_define
...
name|mutex
parameter_list|)
value|{          \     if (ZSTD_DEBUG>= MUTEX_WAIT_TIME_DLEVEL) {   \         unsigned long long const beforeTime = GetCurrentClockTimeMicroseconds(); \         ZSTD_pthread_mutex_lock(mutex);           \         {   unsigned long long const afterTime = GetCurrentClockTimeMicroseconds(); \             unsigned long long const elapsedTime = (afterTime-beforeTime); \             if (elapsedTime> 1000) {
...
name|m
parameter_list|)
value|ZSTD_pthread_mutex_lock(m)
end_define
...
begin_comment
comment|/* a single Buffer Pool can be invoked from multiple threads in parallel */
end_comment
...
literal|3
argument_list|,
literal|"ZSTDMT_freeBufferPool (address:%08X)"
argument_list|,
operator|(
name|U32
operator|)
...
literal|4
argument_list|,
literal|"free buffer %2u (address:%08X)"
argument_list|,
name|u
argument_list|,
operator|(
...
begin_comment
comment|/** ZSTDMT_getBuffer() :  *  assumption : bufPool must be valid */
end_comment
...
literal|5
argument_list|,
literal|"existing buffer does not meet size conditions => freeing"
argument_list|)
expr_stmt|;
name|ZSTD_free
argument_list|(
...
condition|)
return|return;
comment|/* compatible with release on NULL */
name|DEBUGLOG
argument_list|(
literal|5
argument_list|,
literal|"ZSTDMT_releaseBuffer"
argument_list|)
expr_stmt|;
name|ZSTD_pthread_mutex_lock
argument_list|(
...
literal|5
argument_list|,
literal|"buffer pool capacity reached => freeing "
argument_list|)
expr_stmt|;
name|ZSTD_free
argument_list|(
...
begin_comment
comment|/* Sets parameters relevant to the compression job, initializing others to  * default values. Notably, nbThreads should probably be zero. */
end_comment

begin_function
specifier|static
name|ZSTD_CCtx_params
name|ZSTDMT_makeJobCCtxParams
parameter_list|(
name|ZSTD_CCtx_params
specifier|const
name|params
...
begin_comment
comment|/* a single CCtx Pool can be invoked from multiple threads in parallel */
end_comment
...
literal|5
argument_list|,
literal|"CCtx pool overflow : free cctx"
argument_list|)
expr_stmt|;
name|ZSTD_freeCCtx
argument_list|(
...
literal|4
argument_list|,
literal|"job%02u: release dst address %08X"
argument_list|,
name|jobID
argument_list|,
operator|(
...
literal|4
argument_list|,
literal|"job%02u: release src address %08X"
argument_list|,
name|jobID
argument_list|,
operator|(
...
literal|4
argument_list|,
literal|"input: release address %08X"
argument_list|,
operator|(
name|U32
operator|)
...
argument_list|)
expr_stmt|;
comment|/* stop and free worker threads */
name|ZSTDMT_releaseAllJobResources
argument_list|(
name|mtctx
argument_list|)
...
specifier|static
name|size_t
name|ZSTDMT_compress_advanced_internal
parameter_list|(
name|ZSTDMT_CCtx
modifier|*
name|mtctx
...
argument_list|)
expr_stmt|;
comment|/* condition for ZSTD_compressBound(A) + ZSTD_compressBound(B)<= ZSTD_compressBound(A+B), which is required for compressWithinDst */
name|ZSTDMT_setBufferSize
argument_list|(
name|mtctx
operator|->
...
block|}
block|}
comment|/* for (chunkID=0; chunkID<nbChunks; chunkID++) */
name|DEBUGLOG
argument_list|(
literal|4
argument_list|,
...
begin_comment
comment|/** ZSTDMT_compressStream_generic() :  *  internal use only - exposed to be invoked from zstd_compress.c  *  assumption : output and input are valid (pos<= size)  * @return : minimum amount of data remaining to flush, 0 if none */
end_comment
...
condition|)
block|{
comment|/* current frame being ended. Only flush/end are allowed */
return|return
name|ERROR
argument_list|(
name|stage_wrong
...
condition|)
block|{
comment|/* delegate to single-thread (synchronous) */
return|return
...
return|;
block|}
comment|/* single-pass shortcut (note : synchronous-mode) */
if|if
condition|(
operator|(
name|mtctx
...
argument_list|)
expr_stmt|;
comment|/* note : may fail, in which case, no forward input progress */
name|mtctx
operator|->
name|inBuff
operator|.
...
argument_list|)
expr_stmt|;
comment|/* block if there was no forward input progress */
if|if
condition|(
name|input
operator|->
