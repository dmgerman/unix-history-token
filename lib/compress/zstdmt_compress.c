...
modifier|...
parameter_list|)
value|if (l<=g_debugLevel) { fprintf(stderr, __VA_ARGS__); }
end_define
...
modifier|...
parameter_list|)
value|if (l<=g_debugLevel) { fprintf(stderr, __FILE__ ": "); fprintf(stderr, __VA_ARGS__); fprintf(stderr, " \n"); }
end_define
...
begin_comment
comment|/* assumption : invocation from main thread only ! */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* size conditions not respected : scratch this buffer and create a new one */
block|}
...
index|]
decl_stmt|;
comment|/* variable size (must lies at the end) */
block|}
struct|;
end_struct
...
literal|5
argument_list|,
literal|"nbThreads : %u  ; minNbJobs : %u ;  nbJobsLog2 : %u ;  nbJobs : %u  \n"
argument_list|,
name|nbThreads
argument_list|,
name|minNbJobs
...
condition|)
block|{
comment|/* one object was not created */
...
begin_comment
comment|/* ZSTDMT_releaseAllJobResources() :  * Ensure all workers are killed first. */
end_comment
...
literal|3
argument_list|,
literal|"windowLog : %2u => chunkTargetSize : %u bytes  "
argument_list|,
name|params
operator|.
name|cParams
...
block|}
block|}
comment|/* note : since nbChunks<= nbThreads, all jobs should be running immediately in parallel */
block|{
name|unsigned
name|chunkID
decl_stmt|;
...
condition|)
block|{
comment|/* note : chunk 0 is already written directly into dst */
if|if
condition|(
operator|!
name|error
...
literal|3
argument_list|,
literal|"Started new compression, with windowLog : %u"
argument_list|,
name|params
operator|.
name|cParams
...
condition|)
block|{
comment|/* previous job not correctly finished */
name|ZSTDMT_waitForAllJobsCompleted
argument_list|(
name|zcs
argument_list|)
...
name|dictSize
expr_stmt|;
comment|/* note : zcs->inBuff.filled is presumed>= srcSize + dictSize */
name|zcs
operator|->
name|jobs
index|[
...
literal|0
expr_stmt|;
comment|/* do not calculate checksum within sections, just keep it in header for first section */
name|zcs
operator|->
name|jobs
index|[
...
literal|5
argument_list|,
literal|"new job : filled to %u, with %u dict and %u src"
argument_list|,
operator|(
name|U32
operator|)
...
literal|0
expr_stmt|;
comment|/* single chunk : checksum is calculated directly within worker thread */
block|}
name|DEBUGLOG
argument_list|(
literal|3
...
argument_list|)
return|;
comment|/* current frame being ended. Only flush is allowed. Restart with init */
if|if
condition|(
name|zcs
operator|->
...
expr_stmt|;
block|}
comment|/* check for data to flush */
name|CHECK_F
argument_list|(
name|ZSTDMT_flushNextJob
argument_list|(
...
literal|4
argument_list|,
literal|"flushing : %u bytes left to compress"
argument_list|,
operator|(
name|U32
operator|)
...
literal|5
argument_list|,
literal|"zcs->doneJobID : %u  ; zcs->nextJobID : %u "
argument_list|,
name|zcs
operator|->
name|doneJobID
