...
begin_comment
comment|/* minimum nb of apparition to be selected in dictionary */
end_comment
...
value|(ZDICT_CONTENTSIZE_MIN * MINRATIO)
end_define
...
control|)
block|{
comment|/* works because end of buffer is a noisy guard band */
if|if
condition|(
name|ip
index|[
...
modifier|...
parameter_list|)
value|if (notificationLevel>=l) { \             if (ZDICT_clockSpan(displayClock)> refreshRate)  \             { displayClock = clock(); DISPLAY(__VA_ARGS__); \             if (notificationLevel>=4) fflush(stderr); } }
comment|/* init */
name|DISPLAYLEVEL
argument_list|(
literal|2
...
literal|1
argument_list|,
literal|"error : ZSTD_compressBegin_advanced() failed : %s \n"
argument_list|,
...
name|HBUFFSIZE
value|256
comment|/* should prove large enough for all entropy headers */
name|BYTE
name|header
index|[
name|HBUFFSIZE
...
literal|8
decl_stmt|;
comment|/* calculate entropy tables */
name|DISPLAYLEVEL
argument_list|(
literal|2
argument_list|,
...
expr_stmt|;
block|}
comment|/* add dictionary header (after entropy tables) */
name|MEM_writeLE32
argument_list|(
name|dictBuffer
argument_list|,
...
return|;
block|}
comment|/* requested dictionary size is too small */
if|if
condition|(
name|samplesBuffSize
operator|<
...
return|;
block|}
comment|/* dictionary content too small */
if|if
condition|(
name|dictContentSize
operator|<
...
literal|2
argument_list|,
literal|"!  always test dictionary efficiency on real samples \n"
argument_list|)
expr_stmt|;
block|}
