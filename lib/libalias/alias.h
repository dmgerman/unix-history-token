...
begin_comment
comment|/* Alias link representativei (incomplete struct) */
end_comment
...
begin_comment
comment|/* InternetChecksum() is not specifically part of the    packet aliasing API, but is sometimes needed outside    the module. (~for instance, natd uses it to create    an ICMP error message when interface size is    exceeded.) */
end_comment
...
begin_comment
comment|/* If PKT_ALIAS_SAME_PORTS is set, packets will be attempted sent from 	the same port as they originated on.  This allow eg rsh to work 	*99% of the time*, but _not_ 100%.  (It will be slightly flakey 	instead of not working at all.) */
end_comment
