...
begin_comment
comment|/*  * A pointer is printed as a fixed-size integer.  This is used both for  * pointers and functions.  */
end_comment
...
name|dmp
decl_stmt|;
comment|/* 	 * Split the fully-qualified type ID (module`id).  This should 	 * always be the format, but if for some reason we don't find the 	 * expected value, return 0 to fall back to the generic trace() 	 * behavior. 	 */
for|for
control|(
name|s
operator|=
