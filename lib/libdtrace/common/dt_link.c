...
operator|)
return|;
comment|/* 	 * We may have already processed this object file in an earlier linker 	 * invocation. Check to see if the present instruction sequence matches 	 * the one we would install. 	 */
if|if
condition|(
name|isenabled
condition|)
...
block|{
comment|/* 		 * If the call is followed by a restore, it's a tail call so 		 * change the call to a ret. If the call if followed by a mov 		 * of a register into %o7, it's a tail call in leaf context 		 * so change the call to a retl-like instruction that returns 		 * to that register value + 8 (rather than the typical %o7 + 		 * 8); the delay slot instruction is left, but should have no 		 * effect. Otherwise we change the call to be a nop. In the 		 * first and the last case we adjust the offset to land on what 		 * was once the delay slot of the call so we correctly get all 		 * the arguments as they would have been passed in a normal 		 * function call. 		 */
if|if
condition|(
name|DT_IS_RESTORE
argument_list|(
