...
name|ISC_LOG_ERROR
argument_list|,
literal|"sdlz newversion on origin %s failed : %s"
argument_list|,
name|origin
argument_list|,
...
name|ISC_LOG_ERROR
argument_list|,
literal|"sdlz closeversion on origin %s failed"
argument_list|,
name|origin
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * getoriginnode() is used by the update code to find the  * dns_rdatatype_dnskey record for a zone  */
end_comment
...
name|ISC_LOG_ERROR
argument_list|,
literal|"sdlz getoriginnode failed : %s"
argument_list|,
name|isc_result_totext
argument_list|(
name|result
...
name|driverarg
expr_stmt|;
comment|/* Call SDLZ driver's configure method */
if|if
condition|(
name|imp
operator|->
...
operator|)
return|;
comment|/* 	 * Format the request elements. sdlz operates on strings, not 	 * structures 	 */
if|if
condition|(
name|signer
operator|!=
...
condition|)
block|{
comment|/* 			 * BIND9 doesn't enforce all RRs in an RRset 			 * having the same TTL, as per RFC 2136, 			 * section 7.12. If a DLZ backend has 			 * different TTLs, then the best 			 * we can do is return the lowest. 			*/
name|rdatalist
operator|->
name|ttl
operator|=
