...
condition|)
block|{
comment|/* 		 * We can now delete the node if the reference counter is 		 * zero.  This should be typically the case, but a different 		 * thread may still gain a (new) reference just before the 		 * current thread locks the tree (e.g., in findnode()). 		 */
...
name|dns_db_secure
expr_stmt|;
comment|/* 	 * Do we have a NSEC/NSEC3 chain under creation? 	 */
elseif|else
if|if
condition|(
name|hasoptbit
