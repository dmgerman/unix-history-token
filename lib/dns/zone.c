...
begin_comment
comment|/* $Id: zone.c,v 1.582.8.7 2011-02-18 23:23:08 each Exp $ */
end_comment
...
name|added
decl_stmt|;
comment|/*% 	 * whether a rpz radix was needed when last loaded 	 */
name|isc_boolean_t
name|rpz_zone
decl_stmt|;
block|}
...
begin_comment
comment|/* #define DNS_ZONEFLG_XXXXX	0x10000000U   XXXMPA unused. */
end_comment
...
sizeof|sizeof
argument_list|(
literal|"REMOVE|CREATE|NONSEC|OPTOUT"
argument_list|)
index|]
decl_stmt|;
name|int
...
begin_comment
comment|/*  * Add key to the security roots for all views.  */
end_comment
...
begin_comment
comment|/*  * Remove key from the security roots for all views.  */
end_comment
...
begin_comment
comment|/*  * Add a null key to the security roots for all views, so that all queries  * to the zone will fail.  */
end_comment
...
begin_comment
comment|/*  * Scan a set of KEYDATA records from the key zone.  The ones that are  * valid (i.e., the add holddown timer has expired) become trusted keys for  * all views.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* For each view, delete references to this key from secroots. */
for|for
control|(
name|view
operator|=
...
name|ISC_LOG_WARNING
argument_list|,
literal|"DNSKEY RRSIG(s) will expire at %u"
argument_list|,
name|when
argument_list|)
expr_stmt|;
comment|/* XXXMPA convert to date. */
name|delta
operator|=
name|when
operator|-
...
name|ISC_LOG_NOTICE
argument_list|,
comment|/* XXMPA ISC_LOG_DEBUG(1) */
literal|"setting keywarntime to %u - 7 days"
argument_list|,
name|when
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* 			 * Ensure that we don't loop resigning the SOA. 			 */
if|if
condition|(
name|covers
operator|==
...
condition|)
block|{
comment|/* Trust this key in all views */
name|dns_rdata_tostruct
argument_list|(
operator|&
name|dnskeyrr
...
name|ISC_LOG_ERROR
argument_list|,
literal|"NOTIFY to %s not "
literal|"sent. Peer TSIG key lookup failure."
argument_list|,
name|addrbuf
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Can't get keys for some reason; try again later. */
if|if
condition|(
name|result
operator|!=
...
goto|;
block|}
comment|/* See if any pre-existing keys have newly become active */
for|for
control|(
name|key
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 		 * Has a new key become active?  If so, is it for 		 * a new algorithm? 		 */
for|for
control|(
name|tuple
operator|=
...
block|}
block|}
comment|/* 		 * If we found a new algorithm, we need to sign the 		 * zone fully.  If there's a new key, but it's for an 		 * already-existing algorithm, then the zone signing 		 * can be handled incrementally. 		 */
if|if
condition|(
name|newkey
operator|&&
...
condition|)
block|{
comment|/* 			 * We haven't been told to sign fully, but a new 			 * algorithm was added to the DNSKEY.  We sign 			 * the full zone, but only with the newly-added 			 * keys. 			 */
for|for
control|(
name|tuple
operator|=
...
name|timethen
decl_stmt|;
comment|/* 		 * If we are doing automatic key maintenance and the 		 * key metadata indicates there is a key change event 		 * scheduled in the future, set the key refresh timer. 		 */
if|if
condition|(
operator|!
name|DNS_ZONEKEY_OPTION
...
expr_stmt|;
block|}
comment|/* 	 * If no key event is scheduled, we should still check the key 	 * repository for updates every so often.  (Currently this is 	 * hard-coded to 12 hours, but it could be configurable.) 	 */
if|if
condition|(
name|isc_time_isepoch
argument_list|(
