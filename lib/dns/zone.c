...
begin_comment
comment|/* $Id: zone.c,v 1.483.36.6 2009/03/26 22:57:07 marka Exp $ */
end_comment
...
name|level
argument_list|,
literal|"NS '%s' has no address records (A or AAAA)"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
comment|/* XXX950 Make fatal ISC_FALSE for 9.5.0. */
return|return
operator|(
name|ISC_TRUE
operator|)
...
name|level
argument_list|,
literal|"NS '%s' is a CNAME (illegal)"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
name|level
argument_list|,
literal|"NS '%s' is below a DNAME '%s' (illegal)"
argument_list|,
name|namebuf
argument_list|,
name|altbuf
...
condition|)
block|{
comment|/* 		 * Set the OPT bit to indicate that this is a 		 * partially secure zone. 		 */
name|isc_region_t
name|region
decl_stmt|;
...
name|NULL
decl_stmt|;
comment|/* 	 * Check to see if the OPT bit has already been cleared. 	 */
name|CHECK
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Find the NEXT name for building the new record. 	 */
name|CHECK
argument_list|(
name|dns_rdata_tostruct
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Delete the old NSEC record. 	 */
name|CHECK
argument_list|(
name|update_one_rr
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Add the new NSEC record. 	 */
name|CHECK
argument_list|(
name|dns_nsec_buildrdata
argument_list|(
...
condition|)
block|{
comment|/* 		 * We have a complete NSEC chain.  Signal to update 		 * the apex NSEC record. 		 */
operator|*
name|updatensec
operator|=
name|ISC_TRUE
...
control|)
block|{
comment|/* 		 * We have changed the NSEC3 RRset above so we need to update 		 * the signatures. 		 */
name|result
operator|=
name|del_sigs
argument_list|(
...
control|)
block|{
comment|/* 		 * We have changed the NSEC3PARAM RRset above so we need to 		 * update the signatures. 		 */
name|result
operator|=
name|del_sigs
argument_list|(
...
name|ISC_TRUE
expr_stmt|;
comment|/* 	 * See if we have a NSEC chain. 	 */
name|result
operator|=
name|dns_db_getoriginnode
argument_list|(
...
block|{
comment|/* 		 * No NSEC chain present. 		 * See if we need to build a NSEC3 chain? 		 */
name|result
operator|=
...
control|)
block|{
comment|/* 			 * Find the key we want to sign with. 			 */
if|if
condition|(
name|dst_key_alg
argument_list|(
...
condition|)
block|{
comment|/* 		 * We have changed the NSEC RRset above so we need to update 		 * the signatures. 		 */
name|result
operator|=
name|del_sigs
argument_list|(
...
condition|)
block|{
comment|/* 		 * We have changed the RRset above so we need to update 		 * the signatures. 		 */
name|result
operator|=
name|del_sigs
argument_list|(
