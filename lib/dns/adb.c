...
begin_comment
comment|/*%  * The period in seconds after which an ADB name entry is regarded as stale  * and forced to be cleaned up.  * TODO: This should probably be configurable at run-time.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|ADB_STALE_MARGIN
end_ifndef
...
name|name_refcnt
decl_stmt|;
comment|/*! 	 * Bucketized locks and lists for entries. 	 * 	 * XXXRTH  Have a per-bucket structure that contains all of these? 	 */
name|unsigned
name|int
name|nentries
decl_stmt|;
...
name|finds
decl_stmt|;
comment|/* for LRU-based management */
name|isc_stdtime_t
name|last_used
decl_stmt|;
name|ISC_LINK
...
begin_comment
comment|/*  * Private flag(s) for entries.  * MUST NOT overlap FCTX_ADDRINFO_xxx and DNS_FETCHOPT_NOEDNS0.  */
end_comment
...
begin_comment
comment|/*  * Hashing is most efficient if the number of buckets is prime.  * The sequence below is the closest previous primes to 2^n and  * 1.5 * 2^n, for values of n from 10 to 28.  (The tables will  * no longer grow beyond 2^28 entries.)  */
end_comment
...
name|ISC_LOG_INFO
argument_list|,
literal|"adb: grow_entries to %u starting"
argument_list|,
name|n
argument_list|)
expr_stmt|;
...
name|cleanup
goto|;
comment|/* 	 * Grab all the resources we need. 	 */
name|newentries
operator|=
name|isc_mem_get
argument_list|(
...
name|cleanup
goto|;
comment|/* 	 * Initialise the new resources. 	 */
name|result
operator|=
...
expr_stmt|;
block|}
comment|/* 	 * Move entries to new arrays. 	 */
for|for
control|(
name|i
operator|=
...
name|n
expr_stmt|;
comment|/* 	 * Only on success do we set adb->growentries_sent to ISC_FALSE. 	 * This will prevent us being continuously being called on error. 	 */
name|adb
operator|->
name|growentries_sent
operator|=
...
name|ISC_LOG_INFO
argument_list|,
literal|"adb: grow_entries finished"
argument_list|)
expr_stmt|;
block|}
end_function
...
name|ISC_LOG_INFO
argument_list|,
literal|"adb: grow_names to %u starting"
argument_list|,
name|n
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* 	 * Move names to new arrays. 	 */
for|for
control|(
name|i
operator|=
...
name|n
expr_stmt|;
comment|/* 	 * Only on success do we set adb->grownames_sent to ISC_FALSE. 	 * This will prevent us being continuously being called on error. 	 */
name|adb
operator|->
name|grownames_sent
operator|=
...
name|ISC_LOG_INFO
argument_list|,
literal|"adb: grow_names finished"
argument_list|)
expr_stmt|;
block|}
end_function
...
expr_stmt|;
block|}
comment|/* Search the list, while cleaning up expired entries. */
for|for
control|(
name|entry
operator|=
...
literal|1
expr_stmt|;
comment|/* 	 * We limit the number of scanned entries to 10 (arbitrary choice) 	 * in order to avoid examining too many entries when there are many 	 * tail entries that have fetches (this should be rare, but could 	 * happen). 	 */
name|victim
operator|=
name|ISC_LIST_TAIL
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* this is actually unused */
name|REQUIRE
argument_list|(
name|taskmgr
operator|!=
...
name|ISC_LOG_INFO
argument_list|,
literal|"adb: task-exclusive mode unavailable, "
literal|"intializing table sizes to %u\n"
argument_list|,
name|nbuckets
index|[
literal|11
...
parameter_list|)
define|\
value|do { \ 		(adb)->el = isc_mem_get((adb)->mctx, \ 				     sizeof(*(adb)->el) * (adb)->nentries); \ 		if ((adb)->el == NULL) { \ 			result = ISC_R_NOMEMORY; \ 			goto fail1; \ 		}\ 	} while (0)
name|ALLOCENTRY
argument_list|(
name|adb
argument_list|,
...
parameter_list|)
define|\
value|do { \ 		(adb)->el = isc_mem_get((adb)->mctx, \ 				     sizeof(*(adb)->el) * (adb)->nnames); \ 		if ((adb)->el == NULL) { \ 			result = ISC_R_NOMEMORY; \ 			goto fail1; \ 		}\ 	} while (0)
name|ALLOCNAME
argument_list|(
name|adb
argument_list|,
...
directive|undef
name|MPINIT
comment|/* 	 * Allocate an internal task. 	 */
name|result
operator|=
...
condition|)
block|{
comment|/* 		 * See if there is any stale name at the end of list, and purge 		 * it if so. 		 */
name|check_stale_name
argument_list|(
name|adb
argument_list|,
...
block|{
comment|/* Move this name forward in the LRU list */
name|ISC_LIST_UNLINK
argument_list|(
name|adb
operator|->
...
name|FIND_ERR_UNEXPECTED
expr_stmt|;
comment|/* 	 * We need to specify whether to search static-stub zones (if 	 * configured) depending on whether this is a "start at zone" lookup, 	 * i.e., whether it's a "bailiwick" glue.  If it's bailiwick (in which 	 * case NAME_STARTATZONE is set) we need to stop the search at any 	 * matching static-stub zone without looking into the cache to honor 	 * the configuration on which server we should send queries to. 	 */
name|result
operator|=
name|dns_view_find2
argument_list|(
