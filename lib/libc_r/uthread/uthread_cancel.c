...
name|PS_JOIN
case|:
comment|/* 				 * Threads in these states may be in queues. 				 * In order to preserve queue integrity, the 				 * cancelled thread must remove itself from the 				 * queue.  Mark the thread as interrupted and 				 * needing cancellation, and set the state to 				 * running.  When the thread resumes, it will 				 * exit after removing itself from the queue. 				 */
name|pthread
operator|->
name|interrupted
operator|=
