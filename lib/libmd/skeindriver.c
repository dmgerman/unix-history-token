begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/* SKEINDRIVER.C - test driver for SKEIN */
end_comment

begin_comment
comment|/* Copyright (C) 1990-2, RSA Data Security, Inc. Created 1990. All rights  * reserved.  *   * RSA Data Security, Inc. makes no representations concerning either the  * merchantability of this software or the suitability of this software for  * any particular purpose. It is provided "as is" without express or implied  * warranty of any kind.  *   * These notices must be retained in any copies of any part of this  * documentation and/or software. */
end_comment

begin_include
include|#
directive|include
file|<sys/cdefs.h>
end_include

begin_expr_stmt
name|__FBSDID
argument_list|(
literal|"$FreeBSD$"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<time.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|"skein.h"
end_include

begin_comment
comment|/* The following makes SKEIN default to SKEIN512 if it has not already been  * defined with C compiler flags. */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|SKEIN
end_ifndef

begin_define
define|#
directive|define
name|SKEIN
value|512
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|SKEIN
operator|==
literal|256
end_if

begin_undef
undef|#
directive|undef
name|SKEIN_Data
end_undef

begin_define
define|#
directive|define
name|SKEIN_Data
value|SKEIN256_Data
end_define

begin_elif
elif|#
directive|elif
name|SKEIN
operator|==
literal|512
end_elif

begin_undef
undef|#
directive|undef
name|SKEIN_Data
end_undef

begin_define
define|#
directive|define
name|SKEIN_Data
value|SKEIN512_Data
end_define

begin_elif
elif|#
directive|elif
name|SKEIN
operator|==
literal|1024
end_elif

begin_undef
undef|#
directive|undef
name|SKEIN_Data
end_undef

begin_define
define|#
directive|define
name|SKEIN_Data
value|SKEIN1024_Data
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* Digests a string and prints the result. */
end_comment

begin_function
specifier|static
name|void
name|SKEINString
parameter_list|(
name|char
modifier|*
name|string
parameter_list|)
block|{
name|char
name|buf
index|[
literal|2
operator|*
literal|128
operator|+
literal|1
index|]
decl_stmt|;
name|printf
argument_list|(
literal|"SKEIN%d (\"%s\") = %s\n"
argument_list|,
name|SKEIN
argument_list|,
name|string
argument_list|,
name|SKEIN_Data
argument_list|(
name|string
argument_list|,
name|strlen
argument_list|(
name|string
argument_list|)
argument_list|,
name|buf
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Digests a reference suite of strings and prints the results. */
end_comment

begin_function
name|int
name|main
parameter_list|(
name|void
parameter_list|)
block|{
name|printf
argument_list|(
literal|"SKEIN%d test suite:\n"
argument_list|,
name|SKEIN
argument_list|)
expr_stmt|;
name|SKEINString
argument_list|(
literal|""
argument_list|)
expr_stmt|;
name|SKEINString
argument_list|(
literal|"abc"
argument_list|)
expr_stmt|;
name|SKEINString
argument_list|(
literal|"message digest"
argument_list|)
expr_stmt|;
name|SKEINString
argument_list|(
literal|"abcdefghijklmnopqrstuvwxyz"
argument_list|)
expr_stmt|;
name|SKEINString
argument_list|(
literal|"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
literal|"abcdefghijklmnopqrstuvwxyz0123456789"
argument_list|)
expr_stmt|;
name|SKEINString
argument_list|(
literal|"1234567890123456789012345678901234567890"
literal|"1234567890123456789012345678901234567890"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

end_unit

