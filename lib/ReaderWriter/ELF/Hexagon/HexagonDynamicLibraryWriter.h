begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|//===- lib/ReaderWriter/ELF/Hexagon/HexagonDynamicLibraryWriter.h ---------===//
end_comment

begin_comment
comment|//
end_comment

begin_comment
comment|//                             The LLVM Linker
end_comment

begin_comment
comment|//
end_comment

begin_comment
comment|// This file is distributed under the University of Illinois Open Source
end_comment

begin_comment
comment|// License. See LICENSE.TXT for details.
end_comment

begin_comment
comment|//
end_comment

begin_comment
comment|//===----------------------------------------------------------------------===//
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|HEXAGON_DYNAMIC_LIBRARY_WRITER_H
end_ifndef

begin_define
define|#
directive|define
name|HEXAGON_DYNAMIC_LIBRARY_WRITER_H
end_define

begin_include
include|#
directive|include
file|"DynamicLibraryWriter.h"
end_include

begin_include
include|#
directive|include
file|"HexagonLinkingContext.h"
end_include

begin_decl_stmt
name|namespace
name|lld
block|{
name|namespace
name|elf
block|{
name|class
name|HexagonTargetLayout
decl_stmt|;
name|class
name|HexagonDynamicLibraryWriter
range|:
name|public
name|DynamicLibraryWriter
operator|<
name|ELF32LE
operator|>
block|{
name|public
operator|:
name|HexagonDynamicLibraryWriter
argument_list|(
name|HexagonLinkingContext
operator|&
name|ctx
argument_list|,
name|HexagonTargetLayout
operator|&
name|layout
argument_list|)
block|;
name|protected
operator|:
comment|// Add any runtime files and their atoms to the output
name|void
name|createImplicitFiles
argument_list|(
argument|std::vector<std::unique_ptr<File>>&
argument_list|)
name|override
block|;
name|void
name|finalizeDefaultAtomValues
argument_list|()
name|override
block|;
name|std
operator|::
name|error_code
name|setELFHeader
argument_list|()
name|override
block|{
name|DynamicLibraryWriter
operator|::
name|setELFHeader
argument_list|()
block|;
name|setHexagonELFHeader
argument_list|(
operator|*
name|_elfHeader
argument_list|)
block|;
return|return
name|std
operator|::
name|error_code
argument_list|()
return|;
block|}
name|private
operator|:
name|HexagonLinkingContext
operator|&
name|_ctx
block|;
name|HexagonTargetLayout
operator|&
name|_targetLayout
block|; }
decl_stmt|;
name|HexagonDynamicLibraryWriter
operator|::
name|HexagonDynamicLibraryWriter
argument_list|(
name|HexagonLinkingContext
operator|&
name|ctx
argument_list|,
name|HexagonTargetLayout
operator|&
name|layout
argument_list|)
operator|:
name|DynamicLibraryWriter
argument_list|(
name|ctx
argument_list|,
name|layout
argument_list|)
operator|,
name|_ctx
argument_list|(
name|ctx
argument_list|)
operator|,
name|_targetLayout
argument_list|(
argument|layout
argument_list|)
block|{}
name|void
name|HexagonDynamicLibraryWriter
operator|::
name|createImplicitFiles
argument_list|(
argument|std::vector<std::unique_ptr<File>>&result
argument_list|)
block|{
name|DynamicLibraryWriter
operator|::
name|createImplicitFiles
argument_list|(
name|result
argument_list|)
block|;
comment|// Add the default atoms as defined for hexagon
name|auto
name|file
operator|=
name|llvm
operator|::
name|make_unique
operator|<
name|RuntimeFile
operator|<
name|ELF32LE
operator|>>
operator|(
name|_ctx
operator|,
literal|"Hexagon runtime file"
operator|)
block|;
name|file
operator|->
name|addAbsoluteAtom
argument_list|(
literal|"_GLOBAL_OFFSET_TABLE_"
argument_list|)
block|;
name|file
operator|->
name|addAbsoluteAtom
argument_list|(
literal|"_DYNAMIC"
argument_list|)
block|;
name|result
operator|.
name|push_back
argument_list|(
name|std
operator|::
name|move
argument_list|(
name|file
argument_list|)
argument_list|)
block|; }
name|void
name|HexagonDynamicLibraryWriter
operator|::
name|finalizeDefaultAtomValues
argument_list|()
block|{
comment|// Finalize the atom values that are part of the parent.
name|DynamicLibraryWriter
operator|::
name|finalizeDefaultAtomValues
argument_list|()
block|;
if|if
condition|(
name|_ctx
operator|.
name|isDynamic
argument_list|()
condition|)
name|finalizeHexagonRuntimeAtomValues
argument_list|(
name|_targetLayout
argument_list|)
expr_stmt|;
block|}
block|}
comment|// namespace elf
block|}
end_decl_stmt

begin_comment
comment|// namespace lld
end_comment

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|// HEXAGON_DYNAMIC_LIBRARY_WRITER_H
end_comment

end_unit

