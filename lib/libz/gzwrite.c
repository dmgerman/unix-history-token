...
begin_comment
comment|/* gzwrite.c -- zlib functions for writing gzip files  * Copyright (C) 2004, 2005, 2010, 2011, 2012, 2013 Mark Adler  * For conditions of distribution and use, see copyright notice in zlib.h  */
end_comment
...
begin_comment
comment|/* Initialize state for writing a gzip file.  Mark initialization by setting    state->size to non-zero.  Return -1 on failure or 0 on success. */
end_comment
...
begin_comment
comment|/* Compress len zeros to output.  Return -1 on error, 0 on success. */
end_comment
...
return|;
block|}
comment|/* input was all buffered or compressed (put will fit in int) */
return|return
operator|(
name|int
operator|)
...
literal|0
return|;
comment|/* do the printf() into the input buffer, put length in len */
name|size
operator|=
call|(
name|int
...
literal|0
return|;
comment|/* update buffer and position, defer compression until needed */
name|strm
operator|->
name|avail_in
operator|=
