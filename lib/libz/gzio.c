...
name|in
decl_stmt|;
comment|/* bytes into deflate or inflate */
name|z_off_t
name|out
decl_stmt|;
comment|/* bytes out of deflate or inflate */
name|int
name|back
decl_stmt|;
comment|/* one character push-back */
name|int
name|last
decl_stmt|;
comment|/* true if push-back is last character */
block|}
name|gz_stream
typedef|;
end_typedef
...
begin_comment
comment|/* ===========================================================================      Opens a gzip (.gz) file for reading or writing. The mode parameter    is as in fopen ("rb" or "wb"). The file is given either by file descriptor    or path name (if fd == -1).      gz_open returns NULL if the file could not be opened or if there was    insufficient memory to allocate the (de)compression state; errno    can be checked to distinguish the two cases (if errno is zero, the    zlib error is Z_MEM_ERROR). */
end_comment
...
literal|10L
expr_stmt|;
comment|/* We use 10L instead of ftell(s->file) to because ftell causes an          * fflush on some systems. This version of the library doesn't use          * start anyway in write mode, so this initialization is not          * necessary.          */
block|}
else|else
block|{
name|check_header
...
name|c
decl_stmt|;
comment|/* Assure two bytes in the buffer so we can peek ahead -- handle case        where first byte of header is at the end of the buffer after the last        gzip segment */
name|len
operator|=
name|s
operator|->
...
argument_list|)
expr_stmt|;
comment|/* The uncompressed length returned by above getlong() may be                  * different from s->out in case of concatenated .gz files.                  * Check for such files:                  */
name|check_header
argument_list|(
name|s
argument_list|)
...
begin_comment
comment|/* ===========================================================================       Push one byte back onto the stream. */
end_comment
...
name|file
decl_stmt|;
comment|/* With concatenated compressed files that can have embedded      * crc trailers, z_eof is no longer the only/best indicator of EOF      * on a gz_stream. Handle end-of-stream error explicitly here.      */
if|if
condition|(
name|s
operator|==
...
begin_comment
comment|/* ===========================================================================      Clear the error and end-of-file flags, and do the same for the real file. */
end_comment
