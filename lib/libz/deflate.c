...
directive|if
literal|0
block|dest->state = (struct internal_state FAR *)         (*dest->zalloc)(1, sizeof(deflate_state));     if (dest->state == Z_NULL) return Z_MEM_ERROR;      *(dest->state) = *(source->state);     return Z_OK;
endif|#
directive|endif
block|}
end_function
...
literal|0
expr_stmt|;
comment|/* Stored blocks are limited to 0xffff bytes: */
if|if
condition|(
name|s
operator|->
...
expr_stmt|;
block|}
comment|/* Emit a stored block if it is large enough: */
if|if
condition|(
name|s
operator|->
