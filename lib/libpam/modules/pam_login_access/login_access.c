begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*   * This module implements a simple but effective form of login access   * control based on login names and on host (or domain) names, internet   * addresses (or network numbers), or on terminal line names in case of   * non-networked logins. Diagnostics are reported through syslog(3).   *   * Author: Wietse Venema, Eindhoven University of Technology, The Netherlands.   */
end_comment

begin_if
if|#
directive|if
literal|0
end_if

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_endif
unit|static char sccsid[] = "%Z% %M% %I% %E% %U%";
endif|#
directive|endif
end_endif

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/cdefs.h>
end_include

begin_expr_stmt
name|__FBSDID
argument_list|(
literal|"$FreeBSD$"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<grp.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<syslog.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|"pam_login_access.h"
end_include

begin_define
define|#
directive|define
name|_PATH_LOGACCESS
value|"/etc/login.access"
end_define

begin_comment
comment|/* Delimiters for fields and for lists of users, ttys or hosts. */
end_comment

begin_decl_stmt
specifier|static
name|char
name|fs
index|[]
init|=
literal|":"
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* field separator */
end_comment

begin_decl_stmt
specifier|static
name|char
name|sep
index|[]
init|=
literal|", \t"
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* list-element separator */
end_comment

begin_comment
comment|/* Constants to be used in assignments only, not in comparisons... */
end_comment

begin_define
define|#
directive|define
name|YES
value|1
end_define

begin_define
define|#
directive|define
name|NO
value|0
end_define

begin_function_decl
specifier|static
name|int
name|from_match
parameter_list|(
specifier|const
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|list_match
parameter_list|(
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|,
name|int
function_decl|(
modifier|*
function_decl|)
parameter_list|(
specifier|const
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|)
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|netgroup_match
parameter_list|(
specifier|const
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|string_match
parameter_list|(
specifier|const
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|user_match
parameter_list|(
specifier|const
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_comment
comment|/* login_access - match username/group and host/tty with access control file */
end_comment

begin_function
name|int
name|login_access
parameter_list|(
specifier|const
name|char
modifier|*
name|user
parameter_list|,
specifier|const
name|char
modifier|*
name|from
parameter_list|)
block|{
name|FILE
modifier|*
name|fp
decl_stmt|;
name|char
name|line
index|[
name|BUFSIZ
index|]
decl_stmt|;
name|char
modifier|*
name|perm
decl_stmt|;
comment|/* becomes permission field */
name|char
modifier|*
name|users
decl_stmt|;
comment|/* becomes list of login names */
name|char
modifier|*
name|froms
decl_stmt|;
comment|/* becomes list of terminals or hosts */
name|int
name|match
init|=
name|NO
decl_stmt|;
name|int
name|end
decl_stmt|;
name|int
name|lineno
init|=
literal|0
decl_stmt|;
comment|/* for diagnostics */
comment|/*      * Process the table one line at a time and stop at the first match.      * Blank lines and lines that begin with a '#' character are ignored.      * Non-comment lines are broken at the ':' character. All fields are      * mandatory. The first field should be a "+" or "-" character. A      * non-existing table means no access control.      */
if|if
condition|(
operator|(
name|fp
operator|=
name|fopen
argument_list|(
name|_PATH_LOGACCESS
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
while|while
condition|(
operator|!
name|match
operator|&&
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|fp
argument_list|)
condition|)
block|{
name|lineno
operator|++
expr_stmt|;
if|if
condition|(
name|line
index|[
name|end
operator|=
name|strlen
argument_list|(
name|line
argument_list|)
operator|-
literal|1
index|]
operator|!=
literal|'\n'
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: line %d: missing newline or line too long"
argument_list|,
name|_PATH_LOGACCESS
argument_list|,
name|lineno
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|line
index|[
literal|0
index|]
operator|==
literal|'#'
condition|)
continue|continue;
comment|/* comment line */
while|while
condition|(
name|end
operator|>
literal|0
operator|&&
name|isspace
argument_list|(
name|line
index|[
name|end
operator|-
literal|1
index|]
argument_list|)
condition|)
name|end
operator|--
expr_stmt|;
name|line
index|[
name|end
index|]
operator|=
literal|0
expr_stmt|;
comment|/* strip trailing whitespace */
if|if
condition|(
name|line
index|[
literal|0
index|]
operator|==
literal|0
condition|)
comment|/* skip blank lines */
continue|continue;
if|if
condition|(
operator|!
operator|(
name|perm
operator|=
name|strtok
argument_list|(
name|line
argument_list|,
name|fs
argument_list|)
operator|)
operator|||
operator|!
operator|(
name|users
operator|=
name|strtok
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
name|fs
argument_list|)
operator|)
operator|||
operator|!
operator|(
name|froms
operator|=
name|strtok
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
name|fs
argument_list|)
operator|)
operator|||
name|strtok
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
name|fs
argument_list|)
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: line %d: bad field count"
argument_list|,
name|_PATH_LOGACCESS
argument_list|,
name|lineno
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|perm
index|[
literal|0
index|]
operator|!=
literal|'+'
operator|&&
name|perm
index|[
literal|0
index|]
operator|!=
literal|'-'
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: line %d: bad first field"
argument_list|,
name|_PATH_LOGACCESS
argument_list|,
name|lineno
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|match
operator|=
operator|(
name|list_match
argument_list|(
name|froms
argument_list|,
name|from
argument_list|,
name|from_match
argument_list|)
operator|&&
name|list_match
argument_list|(
name|users
argument_list|,
name|user
argument_list|,
name|user_match
argument_list|)
operator|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|errno
operator|!=
name|ENOENT
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"cannot open %s: %m"
argument_list|,
name|_PATH_LOGACCESS
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|match
operator|==
literal|0
operator|||
operator|(
name|line
index|[
literal|0
index|]
operator|==
literal|'+'
operator|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/* list_match - match an item against a list of tokens with exceptions */
end_comment

begin_function
specifier|static
name|int
name|list_match
parameter_list|(
name|char
modifier|*
name|list
parameter_list|,
specifier|const
name|char
modifier|*
name|item
parameter_list|,
name|int
function_decl|(
modifier|*
name|match_fn
function_decl|)
parameter_list|(
specifier|const
name|char
modifier|*
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|)
parameter_list|)
block|{
name|char
modifier|*
name|tok
decl_stmt|;
name|int
name|match
init|=
name|NO
decl_stmt|;
comment|/*      * Process tokens one at a time. We have exhausted all possible matches      * when we reach an "EXCEPT" token or the end of the list. If we do find      * a match, look for an "EXCEPT" list and recurse to determine whether      * the match is affected by any exceptions.      */
for|for
control|(
name|tok
operator|=
name|strtok
argument_list|(
name|list
argument_list|,
name|sep
argument_list|)
init|;
name|tok
operator|!=
literal|0
condition|;
name|tok
operator|=
name|strtok
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
name|sep
argument_list|)
control|)
block|{
if|if
condition|(
name|strcasecmp
argument_list|(
name|tok
argument_list|,
literal|"EXCEPT"
argument_list|)
operator|==
literal|0
condition|)
comment|/* EXCEPT: give up */
break|break;
if|if
condition|(
operator|(
name|match
operator|=
call|(
modifier|*
name|match_fn
call|)
argument_list|(
name|tok
argument_list|,
name|item
argument_list|)
operator|)
operator|!=
literal|0
condition|)
comment|/* YES */
break|break;
block|}
comment|/* Process exceptions to matches. */
if|if
condition|(
name|match
operator|!=
name|NO
condition|)
block|{
while|while
condition|(
operator|(
name|tok
operator|=
name|strtok
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
name|sep
argument_list|)
operator|)
operator|&&
name|strcasecmp
argument_list|(
name|tok
argument_list|,
literal|"EXCEPT"
argument_list|)
condition|)
comment|/* VOID */
empty_stmt|;
if|if
condition|(
name|tok
operator|==
literal|0
operator|||
name|list_match
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
name|item
argument_list|,
name|match_fn
argument_list|)
operator|==
name|NO
condition|)
return|return
operator|(
name|match
operator|)
return|;
block|}
return|return
operator|(
name|NO
operator|)
return|;
block|}
end_function

begin_comment
comment|/* netgroup_match - match group against machine or user */
end_comment

begin_function
specifier|static
name|int
name|netgroup_match
parameter_list|(
specifier|const
name|char
modifier|*
name|group
parameter_list|,
specifier|const
name|char
modifier|*
name|machine
parameter_list|,
specifier|const
name|char
modifier|*
name|user
parameter_list|)
block|{
name|char
name|domain
index|[
literal|1024
index|]
decl_stmt|;
name|unsigned
name|int
name|i
decl_stmt|;
if|if
condition|(
name|getdomainname
argument_list|(
name|domain
argument_list|,
sizeof|sizeof
argument_list|(
name|domain
argument_list|)
argument_list|)
operator|!=
literal|0
operator|||
operator|*
name|domain
operator|==
literal|'\0'
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"NIS netgroup support disabled: no NIS domain"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NO
operator|)
return|;
block|}
comment|/* getdomainname() does not reliably terminate the string */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
sizeof|sizeof
argument_list|(
name|domain
argument_list|)
condition|;
operator|++
name|i
control|)
if|if
condition|(
name|domain
index|[
name|i
index|]
operator|==
literal|'\0'
condition|)
break|break;
if|if
condition|(
name|i
operator|==
sizeof|sizeof
argument_list|(
name|domain
argument_list|)
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"NIS netgroup support disabled: invalid NIS domain"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NO
operator|)
return|;
block|}
if|if
condition|(
name|innetgr
argument_list|(
name|group
argument_list|,
name|machine
argument_list|,
name|user
argument_list|,
name|domain
argument_list|)
operator|==
literal|1
condition|)
return|return
operator|(
name|YES
operator|)
return|;
return|return
operator|(
name|NO
operator|)
return|;
block|}
end_function

begin_comment
comment|/* user_match - match a username against one token */
end_comment

begin_function
specifier|static
name|int
name|user_match
parameter_list|(
specifier|const
name|char
modifier|*
name|tok
parameter_list|,
specifier|const
name|char
modifier|*
name|string
parameter_list|)
block|{
name|struct
name|group
modifier|*
name|group
decl_stmt|;
name|int
name|i
decl_stmt|;
comment|/*      * If a token has the magic value "ALL" the match always succeeds.      * Otherwise, return YES if the token fully matches the username, or if      * the token is a group that contains the username.      */
if|if
condition|(
name|tok
index|[
literal|0
index|]
operator|==
literal|'@'
condition|)
block|{
comment|/* netgroup */
return|return
operator|(
name|netgroup_match
argument_list|(
name|tok
operator|+
literal|1
argument_list|,
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
name|string
argument_list|)
operator|)
return|;
block|}
elseif|else
if|if
condition|(
name|string_match
argument_list|(
name|tok
argument_list|,
name|string
argument_list|)
condition|)
block|{
comment|/* ALL or exact match */
return|return
operator|(
name|YES
operator|)
return|;
block|}
elseif|else
if|if
condition|(
operator|(
name|group
operator|=
name|getgrnam
argument_list|(
name|tok
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
comment|/* try group membership */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|group
operator|->
name|gr_mem
index|[
name|i
index|]
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|strcasecmp
argument_list|(
name|string
argument_list|,
name|group
operator|->
name|gr_mem
index|[
name|i
index|]
argument_list|)
operator|==
literal|0
condition|)
return|return
operator|(
name|YES
operator|)
return|;
block|}
return|return
operator|(
name|NO
operator|)
return|;
block|}
end_function

begin_comment
comment|/* from_match - match a host or tty against a list of tokens */
end_comment

begin_function
specifier|static
name|int
name|from_match
parameter_list|(
specifier|const
name|char
modifier|*
name|tok
parameter_list|,
specifier|const
name|char
modifier|*
name|string
parameter_list|)
block|{
name|int
name|tok_len
decl_stmt|;
name|int
name|str_len
decl_stmt|;
comment|/*      * If a token has the magic value "ALL" the match always succeeds. Return      * YES if the token fully matches the string. If the token is a domain      * name, return YES if it matches the last fields of the string. If the      * token has the magic value "LOCAL", return YES if the string does not      * contain a "." character. If the token is a network number, return YES      * if it matches the head of the string.      */
if|if
condition|(
name|tok
index|[
literal|0
index|]
operator|==
literal|'@'
condition|)
block|{
comment|/* netgroup */
return|return
operator|(
name|netgroup_match
argument_list|(
name|tok
operator|+
literal|1
argument_list|,
name|string
argument_list|,
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|)
operator|)
return|;
block|}
elseif|else
if|if
condition|(
name|string_match
argument_list|(
name|tok
argument_list|,
name|string
argument_list|)
condition|)
block|{
comment|/* ALL or exact match */
return|return
operator|(
name|YES
operator|)
return|;
block|}
elseif|else
if|if
condition|(
name|tok
index|[
literal|0
index|]
operator|==
literal|'.'
condition|)
block|{
comment|/* domain: match last fields */
if|if
condition|(
operator|(
name|str_len
operator|=
name|strlen
argument_list|(
name|string
argument_list|)
operator|)
operator|>
operator|(
name|tok_len
operator|=
name|strlen
argument_list|(
name|tok
argument_list|)
operator|)
operator|&&
name|strcasecmp
argument_list|(
name|tok
argument_list|,
name|string
operator|+
name|str_len
operator|-
name|tok_len
argument_list|)
operator|==
literal|0
condition|)
return|return
operator|(
name|YES
operator|)
return|;
block|}
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|tok
argument_list|,
literal|"LOCAL"
argument_list|)
operator|==
literal|0
condition|)
block|{
comment|/* local: no dots */
if|if
condition|(
name|strchr
argument_list|(
name|string
argument_list|,
literal|'.'
argument_list|)
operator|==
literal|0
condition|)
return|return
operator|(
name|YES
operator|)
return|;
block|}
elseif|else
if|if
condition|(
name|tok
index|[
operator|(
name|tok_len
operator|=
name|strlen
argument_list|(
name|tok
argument_list|)
operator|)
operator|-
literal|1
index|]
operator|==
literal|'.'
comment|/* network */
operator|&&
name|strncmp
argument_list|(
name|tok
argument_list|,
name|string
argument_list|,
name|tok_len
argument_list|)
operator|==
literal|0
condition|)
block|{
return|return
operator|(
name|YES
operator|)
return|;
block|}
return|return
operator|(
name|NO
operator|)
return|;
block|}
end_function

begin_comment
comment|/* string_match - match a string against one token */
end_comment

begin_function
specifier|static
name|int
name|string_match
parameter_list|(
specifier|const
name|char
modifier|*
name|tok
parameter_list|,
specifier|const
name|char
modifier|*
name|string
parameter_list|)
block|{
comment|/*      * If the token has the magic value "ALL" the match always succeeds.      * Otherwise, return YES if the token fully matches the string.      */
if|if
condition|(
name|strcasecmp
argument_list|(
name|tok
argument_list|,
literal|"ALL"
argument_list|)
operator|==
literal|0
condition|)
block|{
comment|/* all: always matches */
return|return
operator|(
name|YES
operator|)
return|;
block|}
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|tok
argument_list|,
name|string
argument_list|)
operator|==
literal|0
condition|)
block|{
comment|/* try exact match */
return|return
operator|(
name|YES
operator|)
return|;
block|}
return|return
operator|(
name|NO
operator|)
return|;
block|}
end_function

end_unit

