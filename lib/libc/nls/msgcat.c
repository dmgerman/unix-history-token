...
name|e
parameter_list|)
value|{ WLOCK(NLERR);					\ 				  np = malloc(sizeof(struct catentry));		\ 				  if (np != NULL) {				\ 				  	np->name = strdup(n);			\ 					np->path = NULL;			\ 					np->refcount = 0;			\ 					np->lang = (l == NULL) ? NULL :		\ 					    strdup(l);				\ 					np->caterrno = e;			\ 				  	SLIST_INSERT_HEAD(&cache, np, list);	\ 				  }						\ 				  UNLOCK;					\ 				  errno = e;					\ 				}
end_define
