begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * getvfsent.c - get a listing of installed filesystems  * Written September 1994 by Garrett A. Wollman  * This file is in the public domain.  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/sysctl.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/wait.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<paths.h>
end_include

begin_comment
comment|/* XXX hide some compatibility problems. */
end_comment

begin_undef
undef|#
directive|undef
name|getvfsbyname
end_undef

begin_define
define|#
directive|define
name|vfsconf
value|ovfsconf
end_define

begin_define
define|#
directive|define
name|_PATH_MODLOAD
value|"/sbin/modload"
end_define

begin_comment
comment|/* XXX should be in header file */
end_comment

begin_decl_stmt
specifier|static
name|struct
name|vfsconf
modifier|*
name|_vfslist
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|vfsconf
name|_vfsconf
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|size_t
name|_vfslistlen
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|_vfs_keeplist
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|_vfs_index
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_function
specifier|static
name|int
name|initvfs
parameter_list|(
name|void
parameter_list|)
block|{
name|int
name|mib
index|[
literal|2
index|]
init|=
block|{
name|CTL_VFS
block|,
name|VFS_VFSCONF
block|}
decl_stmt|;
name|size_t
name|size
init|=
literal|0
decl_stmt|;
name|int
name|rv
decl_stmt|;
name|rv
operator|=
name|sysctl
argument_list|(
name|mib
argument_list|,
literal|2
argument_list|,
operator|(
name|void
operator|*
operator|)
literal|0
argument_list|,
operator|&
name|size
argument_list|,
operator|(
name|void
operator|*
operator|)
literal|0
argument_list|,
operator|(
name|size_t
operator|)
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|rv
operator|<
literal|0
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|_vfslist
condition|)
name|free
argument_list|(
name|_vfslist
argument_list|)
expr_stmt|;
name|_vfslist
operator|=
name|malloc
argument_list|(
name|size
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|_vfslist
condition|)
return|return
literal|0
return|;
name|rv
operator|=
name|sysctl
argument_list|(
name|mib
argument_list|,
literal|2
argument_list|,
name|_vfslist
argument_list|,
operator|&
name|size
argument_list|,
operator|(
name|void
operator|*
operator|)
literal|0
argument_list|,
operator|(
name|size_t
operator|)
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|rv
operator|<
literal|0
condition|)
block|{
name|free
argument_list|(
name|_vfslist
argument_list|)
expr_stmt|;
name|_vfslist
operator|=
literal|0
expr_stmt|;
return|return
literal|0
return|;
block|}
name|_vfslistlen
operator|=
name|size
operator|/
sizeof|sizeof
name|_vfslist
index|[
literal|0
index|]
expr_stmt|;
return|return
literal|1
return|;
block|}
end_function

begin_function
name|struct
name|vfsconf
modifier|*
name|getvfsent
parameter_list|(
name|void
parameter_list|)
block|{
if|if
condition|(
operator|!
name|_vfslist
operator|&&
operator|!
name|initvfs
argument_list|()
condition|)
block|{
return|return
literal|0
return|;
block|}
do|do
block|{
if|if
condition|(
name|_vfs_index
operator|>=
name|_vfslistlen
condition|)
block|{
return|return
literal|0
return|;
block|}
name|_vfsconf
operator|=
name|_vfslist
index|[
name|_vfs_index
operator|++
index|]
expr_stmt|;
block|}
do|while
condition|(
operator|!
name|_vfsconf
operator|.
name|vfc_vfsops
condition|)
do|;
if|if
condition|(
operator|!
name|_vfs_keeplist
condition|)
block|{
name|free
argument_list|(
name|_vfslist
argument_list|)
expr_stmt|;
name|_vfslist
operator|=
literal|0
expr_stmt|;
block|}
return|return
operator|&
name|_vfsconf
return|;
block|}
end_function

begin_function
name|struct
name|vfsconf
modifier|*
name|getvfsbyname
parameter_list|(
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
operator|!
name|_vfslist
operator|&&
operator|!
name|initvfs
argument_list|()
condition|)
block|{
return|return
literal|0
return|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|_vfslistlen
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|_vfslist
index|[
name|i
index|]
operator|.
name|vfc_name
argument_list|,
name|name
argument_list|)
condition|)
break|break;
block|}
if|if
condition|(
name|i
operator|<
name|_vfslistlen
condition|)
block|{
name|_vfsconf
operator|=
name|_vfslist
index|[
name|i
index|]
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|_vfs_keeplist
condition|)
block|{
name|free
argument_list|(
name|_vfslist
argument_list|)
expr_stmt|;
name|_vfslist
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|i
operator|<
name|_vfslistlen
condition|)
block|{
return|return
operator|&
name|_vfsconf
return|;
block|}
else|else
block|{
return|return
literal|0
return|;
block|}
block|}
end_function

begin_function
name|struct
name|vfsconf
modifier|*
name|getvfsbytype
parameter_list|(
name|int
name|type
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
operator|!
name|_vfslist
operator|&&
operator|!
name|initvfs
argument_list|()
condition|)
block|{
return|return
literal|0
return|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|_vfslistlen
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|_vfslist
index|[
name|i
index|]
operator|.
name|vfc_index
operator|==
name|type
condition|)
break|break;
block|}
if|if
condition|(
name|i
operator|<
name|_vfslistlen
condition|)
block|{
name|_vfsconf
operator|=
name|_vfslist
index|[
name|i
index|]
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|_vfs_keeplist
condition|)
block|{
name|free
argument_list|(
name|_vfslist
argument_list|)
expr_stmt|;
name|_vfslist
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|i
operator|<
name|_vfslistlen
condition|)
block|{
return|return
operator|&
name|_vfsconf
return|;
block|}
else|else
block|{
return|return
literal|0
return|;
block|}
block|}
end_function

begin_function
name|void
name|setvfsent
parameter_list|(
name|int
name|keep
parameter_list|)
block|{
if|if
condition|(
name|_vfslist
operator|&&
operator|!
name|keep
condition|)
block|{
name|free
argument_list|(
name|_vfslist
argument_list|)
expr_stmt|;
name|_vfslist
operator|=
literal|0
expr_stmt|;
block|}
name|_vfs_keeplist
operator|=
name|keep
expr_stmt|;
name|_vfs_index
operator|=
literal|0
expr_stmt|;
block|}
end_function

begin_function
name|void
name|endvfsent
parameter_list|(
name|void
parameter_list|)
block|{
if|if
condition|(
name|_vfslist
condition|)
block|{
name|free
argument_list|(
name|_vfslist
argument_list|)
expr_stmt|;
name|_vfslist
operator|=
literal|0
expr_stmt|;
block|}
name|_vfs_index
operator|=
literal|0
expr_stmt|;
block|}
end_function

begin_decl_stmt
specifier|static
specifier|const
name|char
modifier|*
name|vfs_lkmdirs
index|[]
init|=
block|{
literal|"/lkm"
block|,
literal|"/usr/lkm"
block|,
literal|0
block|,
literal|0
block|}
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|NLKMDIRS
value|((sizeof vfs_lkmdirs) / (sizeof vfs_lkmdirs[0]))
end_define

begin_function
specifier|static
specifier|const
name|char
modifier|*
name|vfspath
parameter_list|(
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
specifier|static
name|char
name|pnbuf
index|[
name|MAXPATHLEN
index|]
decl_stmt|;
name|char
modifier|*
name|userdir
init|=
name|getenv
argument_list|(
literal|"LKMDIR"
argument_list|)
decl_stmt|;
name|int
name|i
decl_stmt|;
if|if
condition|(
name|userdir
operator|&&
name|getuid
argument_list|()
operator|==
name|geteuid
argument_list|()
operator|&&
name|getuid
argument_list|()
operator|==
literal|0
condition|)
block|{
name|vfs_lkmdirs
index|[
name|NLKMDIRS
operator|-
literal|2
index|]
operator|=
name|userdir
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|vfs_lkmdirs
index|[
name|i
index|]
condition|;
name|i
operator|++
control|)
block|{
name|snprintf
argument_list|(
name|pnbuf
argument_list|,
sizeof|sizeof
name|pnbuf
argument_list|,
literal|"%s/%s_mod.o"
argument_list|,
name|vfs_lkmdirs
index|[
name|i
index|]
argument_list|,
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|access
argument_list|(
name|pnbuf
argument_list|,
name|R_OK
argument_list|)
condition|)
return|return
name|pnbuf
return|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_function
name|int
name|vfsisloadable
parameter_list|(
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
if|#
directive|if
literal|0
block|int fd;  	fd = open("/dev/lkm", O_RDWR, 0); 	if(fd< 0) { 		return 0; 	} 	close(fd);  	return !!vfspath(name);
else|#
directive|else
return|return
literal|1
return|;
endif|#
directive|endif
block|}
end_function

begin_function
name|int
name|vfsload
parameter_list|(
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|path
decl_stmt|;
name|char
name|name_mod
index|[
sizeof|sizeof
argument_list|(
literal|"_mod"
argument_list|)
operator|+
name|strlen
argument_list|(
name|name
argument_list|)
index|]
decl_stmt|;
name|pid_t
name|pid
decl_stmt|;
name|int
name|status
decl_stmt|;
name|status
operator|=
name|kldload
argument_list|(
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
name|status
operator|!=
operator|-
literal|1
condition|)
return|return
literal|0
return|;
name|path
operator|=
name|vfspath
argument_list|(
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|path
condition|)
block|{
name|errno
operator|=
name|ENOENT
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|pid
operator|=
name|fork
argument_list|()
expr_stmt|;
if|if
condition|(
name|pid
operator|<
literal|0
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|pid
operator|>
literal|0
condition|)
block|{
name|waitpid
argument_list|(
name|pid
argument_list|,
operator|&
name|status
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|WIFEXITED
argument_list|(
name|status
argument_list|)
condition|)
block|{
name|errno
operator|=
name|WEXITSTATUS
argument_list|(
name|status
argument_list|)
expr_stmt|;
return|return
name|errno
condition|?
operator|-
literal|1
else|:
literal|0
return|;
block|}
name|errno
operator|=
name|EINVAL
expr_stmt|;
comment|/* not enough errno values,>sigh< */
return|return
operator|-
literal|1
return|;
block|}
name|status
operator|=
operator|-
literal|1
expr_stmt|;
name|unsetenv
argument_list|(
literal|"TMPDIR"
argument_list|)
expr_stmt|;
if|if
condition|(
name|status
condition|)
block|{
name|status
operator|=
name|chdir
argument_list|(
name|_PATH_VARTMP
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|status
condition|)
block|{
name|status
operator|=
name|chdir
argument_list|(
name|_PATH_TMP
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|status
condition|)
block|{
name|exit
argument_list|(
name|errno
argument_list|)
expr_stmt|;
block|}
name|snprintf
argument_list|(
name|name_mod
argument_list|,
sizeof|sizeof
name|name_mod
argument_list|,
literal|"%s%s"
argument_list|,
name|name
argument_list|,
literal|"_mod"
argument_list|)
expr_stmt|;
name|status
operator|=
name|execl
argument_list|(
name|_PATH_MODLOAD
argument_list|,
literal|"modload"
argument_list|,
literal|"-e"
argument_list|,
name|name_mod
argument_list|,
literal|"-o"
argument_list|,
name|name_mod
argument_list|,
literal|"-u"
argument_list|,
literal|"-q"
argument_list|,
name|path
argument_list|,
operator|(
specifier|const
name|char
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
name|exit
argument_list|(
name|status
condition|?
name|errno
else|:
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

