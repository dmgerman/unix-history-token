...
parameter_list|)
define|\
value|mmap(NULL, (size), PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE, \ 	    MMAP_FD, (off_t)0);
end_define
...
operator|)
return|;
comment|/* ..don't do anything else. */
block|}
block|}
elseif|else
if|if
...
name|errno
expr_stmt|;
comment|/* Protect errno against changes in pubrealloc(). */
name|result
operator|=
name|pubrealloc
argument_list|(
