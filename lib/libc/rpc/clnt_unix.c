...
begin_comment
comment|/*static char *sccsid = "from: @(#)clnt_unix.c 1.37 87/10/05 Copyr 1984 Sun Micro";*/
end_comment

begin_comment
comment|/*static char *sccsid = "from: @(#)clnt_unix.c	2.2 88/08/01 4.0 RPCSRC";*/
end_comment
...
name|rcsid
init|=
literal|"$Id: clnt_unix.c,v 1.7 1996/12/30 14:36:17 peter Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * clnt_unix.c, Implements a AF_UNIX based, client side RPC.  *  * Copyright (C) 1984, Sun Microsystems, Inc.  *  * AF_UNIX based RPC supports 'batched calls'.  * A sequence of calls may be batched-up in a send buffer.  The rpc call  * return immediately to the client even though the call was not necessarily  * sent.  The batching occurs if the results' xdr routine is NULL (0) AND  * the rpc timeout value is zero (see clnt.h, rpc).  *  * Clients should NOT casually batch calls that in fact return results; that is,  * the server side should be aware that a call is batched and not produce any  * return message.  Batched calls that produce many result messages can  * deadlock (netlock) the client and the server....  *  * Now go hang yourself.  */
end_comment
...
begin_comment
comment|/*  * Create a client handle for a unix/ip connection.  * If *sockp<0, *sockp is set to a newly created TCP socket and it is  * connected to raddr.  If *sockp non-negative then  * raddr is ignored.  The rpc/unix package does buffering  * similar to stdio, so the client must pick send and receive buffer sizes,];  * 0 => use the default.  * If raddr->sin_port is 0, then a binder on the remote machine is  * consulted for the right port number.  * NB: *sockp is copied into a private area.  * NB: It is the clients responsibility to close *sockp.  * NB: The rpch->cl_auth is set null authentication.  Caller may wish to set this  * something more useful.  */
end_comment
...
name|stderr
argument_list|,
literal|"clntunix_create: out of memory\n"
argument_list|)
expr_stmt|;
name|rpc_createerr
operator|.
...
argument_list|)
expr_stmt|;
comment|/* decrement by 1 as clntunix_call() increments once */
case|case
name|CLGET_VERS
case|:
...
begin_comment
comment|/*  * Interface between xdr serializer and unix connection.  * Behaves like the system calls, read& write, but keeps some error state  * around for the rpc level.  */
end_comment
