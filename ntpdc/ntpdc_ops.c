begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * ntpdc_ops.c - subroutines which are called to perform operations by  *		 ntpdc  */
end_comment

begin_ifdef
ifdef|#
directive|ifdef
name|HAVE_CONFIG_H
end_ifdef

begin_include
include|#
directive|include
file|<config.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stddef.h>
end_include

begin_include
include|#
directive|include
file|"ntpdc.h"
end_include

begin_include
include|#
directive|include
file|"ntp_net.h"
end_include

begin_include
include|#
directive|include
file|"ntp_control.h"
end_include

begin_include
include|#
directive|include
file|"ntp_refclock.h"
end_include

begin_include
include|#
directive|include
file|"ntp_stdlib.h"
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|HAVE_SYS_TIMEX_H
end_ifdef

begin_include
include|#
directive|include
file|<sys/timex.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|__bsdi__
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|apollo
argument_list|)
end_if

begin_ifdef
ifdef|#
directive|ifdef
name|HAVE_NETINET_IN_H
end_ifdef

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<arpa/inet.h>
end_include

begin_comment
comment|/*  * utility functions  */
end_comment

begin_function_decl
specifier|static
name|int
name|checkitems
parameter_list|(
name|int
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|checkitemsize
parameter_list|(
name|int
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|check1item
parameter_list|(
name|int
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_comment
comment|/*  * Declarations for command handlers in here  */
end_comment

begin_function_decl
specifier|static
name|void
name|peerlist
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|peers
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|doconfig
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|mode
parameter_list|,
name|int
name|refc
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|dmpeers
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|dopeers
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|printpeer
parameter_list|(
name|struct
name|info_peer
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|showpeer
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|peerstats
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|loopinfo
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|sysinfo
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|sysstats
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|iostats
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|memstats
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|timerstats
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|addpeer
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|addserver
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|addrefclock
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|broadcast
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|doconfig
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|int
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|unconfig
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|set
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|sys_clear
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|doset
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|reslist
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|new_restrict
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|unrestrict
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|delrestrict
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|do_restrict
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|monlist
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|reset
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|preset
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|readkeys
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|trustkey
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|untrustkey
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|do_trustkey
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|authinfo
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|traps
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|addtrap
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|clrtrap
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|do_addclr_trap
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|requestkey
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|controlkey
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|do_changekey
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|ctlstats
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|clockstat
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|fudge
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|clkbug
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|kerninfo
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|get_if_stats
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|do_if_reload
parameter_list|(
name|struct
name|parse
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_comment
comment|/*  * Commands we understand.  Ntpdc imports this.  */
end_comment

begin_decl_stmt
name|struct
name|xcmd
name|opcmds
index|[]
init|=
block|{
block|{
literal|"listpeers"
block|,
name|peerlist
block|,
block|{
name|OPT
operator||
name|IP_VERSION
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"-4|-6"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display list of peers the server knows about [IP Version]"
block|}
block|,
block|{
literal|"peers"
block|,
name|peers
block|,
block|{
name|OPT
operator||
name|IP_VERSION
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"-4|-6"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display peer summary information [IP Version]"
block|}
block|,
block|{
literal|"dmpeers"
block|,
name|dmpeers
block|,
block|{
name|OPT
operator||
name|IP_VERSION
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"-4|-6"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display peer summary info the way Dave Mills likes it (IP Version)"
block|}
block|,
block|{
literal|"showpeer"
block|,
name|showpeer
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|}
block|,
block|{
literal|"peer_address"
block|,
literal|"peer2_addr"
block|,
literal|"peer3_addr"
block|,
literal|"peer4_addr"
block|}
block|,
literal|"display detailed information for one or more peers"
block|}
block|,
block|{
literal|"pstats"
block|,
name|peerstats
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|}
block|,
block|{
literal|"peer_address"
block|,
literal|"peer2_addr"
block|,
literal|"peer3_addr"
block|,
literal|"peer4_addr"
block|}
block|,
literal|"display statistical information for one or more peers"
block|}
block|,
block|{
literal|"loopinfo"
block|,
name|loopinfo
block|,
block|{
name|OPT
operator||
name|NTP_STR
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"oneline|multiline"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display loop filter information"
block|}
block|,
block|{
literal|"sysinfo"
block|,
name|sysinfo
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display local server information"
block|}
block|,
block|{
literal|"sysstats"
block|,
name|sysstats
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display local server statistics"
block|}
block|,
block|{
literal|"memstats"
block|,
name|memstats
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display peer memory usage statistics"
block|}
block|,
block|{
literal|"iostats"
block|,
name|iostats
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display I/O subsystem statistics"
block|}
block|,
block|{
literal|"timerstats"
block|,
name|timerstats
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display event timer subsystem statistics"
block|}
block|,
block|{
literal|"addpeer"
block|,
name|addpeer
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"addr"
block|,
literal|"keyid"
block|,
literal|"version"
block|,
literal|"minpoll#|prefer|burst|iburst|'minpoll N'|'maxpoll N'|'keyid N'|'version N' ..."
block|}
block|,
literal|"configure a new peer association"
block|}
block|,
block|{
literal|"addserver"
block|,
name|addserver
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"addr"
block|,
literal|"keyid"
block|,
literal|"version"
block|,
literal|"minpoll#|prefer|burst|iburst|'minpoll N'|'maxpoll N'|'keyid N'|'version N' ..."
block|}
block|,
literal|"configure a new server"
block|}
block|,
block|{
literal|"addrefclock"
block|,
name|addrefclock
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"addr"
block|,
literal|"mode"
block|,
literal|"minpoll|prefer"
block|,
literal|"minpoll|prefer"
block|}
block|,
literal|"configure a new server"
block|}
block|,
block|{
literal|"broadcast"
block|,
name|broadcast
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"addr"
block|,
literal|"keyid"
block|,
literal|"version"
block|,
literal|"minpoll"
block|}
block|,
literal|"configure broadcasting time service"
block|}
block|,
block|{
literal|"unconfig"
block|,
name|unconfig
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|}
block|,
block|{
literal|"peer_address"
block|,
literal|"peer2_addr"
block|,
literal|"peer3_addr"
block|,
literal|"peer4_addr"
block|}
block|,
literal|"unconfigure existing peer assocations"
block|}
block|,
block|{
literal|"enable"
block|,
name|set
block|,
block|{
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"auth|bclient|monitor|pll|kernel|stats"
block|,
literal|"..."
block|,
literal|"..."
block|,
literal|"..."
block|}
block|,
literal|"set a system flag (auth, bclient, monitor, pll, kernel, stats)"
block|}
block|,
block|{
literal|"disable"
block|,
name|sys_clear
block|,
block|{
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"auth|bclient|monitor|pll|kernel|stats"
block|,
literal|"..."
block|,
literal|"..."
block|,
literal|"..."
block|}
block|,
literal|"clear a system flag (auth, bclient, monitor, pll, kernel, stats)"
block|}
block|,
block|{
literal|"reslist"
block|,
name|reslist
block|,
block|{
name|OPT
operator||
name|IP_VERSION
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"-4|-6"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display the server's restrict list"
block|}
block|,
block|{
literal|"restrict"
block|,
name|new_restrict
block|,
block|{
name|NTP_ADD
block|,
name|NTP_ADD
block|,
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"address"
block|,
literal|"mask"
block|,
literal|"ntpport|ignore|noserve|notrust|noquery|nomodify|nopeer|version|kod"
block|,
literal|"..."
block|}
block|,
literal|"create restrict entry/add flags to entry"
block|}
block|,
block|{
literal|"unrestrict"
block|,
name|unrestrict
block|,
block|{
name|NTP_ADD
block|,
name|NTP_ADD
block|,
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"address"
block|,
literal|"mask"
block|,
literal|"ntpport|ignore|noserve|notrust|noquery|nomodify|nopeer|version|kod"
block|,
literal|"..."
block|}
block|,
literal|"remove flags from a restrict entry"
block|}
block|,
block|{
literal|"delrestrict"
block|,
name|delrestrict
block|,
block|{
name|NTP_ADD
block|,
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|NO
block|}
block|,
block|{
literal|"address"
block|,
literal|"mask"
block|,
literal|"ntpport"
block|,
literal|""
block|}
block|,
literal|"delete a restrict entry"
block|}
block|,
block|{
literal|"monlist"
block|,
name|monlist
block|,
block|{
name|OPT
operator||
name|NTP_INT
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"version"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display data the server's monitor routines have collected"
block|}
block|,
block|{
literal|"reset"
block|,
name|reset
block|,
block|{
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|,
name|OPT
operator||
name|NTP_STR
block|}
block|,
block|{
literal|"io|sys|mem|timer|auth|ctl|allpeers"
block|,
literal|"..."
block|,
literal|"..."
block|,
literal|"..."
block|}
block|,
literal|"reset various subsystem statistics counters"
block|}
block|,
block|{
literal|"preset"
block|,
name|preset
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|}
block|,
block|{
literal|"peer_address"
block|,
literal|"peer2_addr"
block|,
literal|"peer3_addr"
block|,
literal|"peer4_addr"
block|}
block|,
literal|"reset stat counters associated with particular peer(s)"
block|}
block|,
block|{
literal|"readkeys"
block|,
name|readkeys
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"request a reread of the keys file and re-init of system keys"
block|}
block|,
block|{
literal|"trustedkey"
block|,
name|trustkey
block|,
block|{
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_UINT
block|}
block|,
block|{
literal|"keyid"
block|,
literal|"keyid"
block|,
literal|"keyid"
block|,
literal|"keyid"
block|}
block|,
literal|"add one or more key ID's to the trusted list"
block|}
block|,
block|{
literal|"untrustedkey"
block|,
name|untrustkey
block|,
block|{
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_UINT
block|}
block|,
block|{
literal|"keyid"
block|,
literal|"keyid"
block|,
literal|"keyid"
block|,
literal|"keyid"
block|}
block|,
literal|"remove one or more key ID's from the trusted list"
block|}
block|,
block|{
literal|"authinfo"
block|,
name|authinfo
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display the state of the authentication code"
block|}
block|,
block|{
literal|"traps"
block|,
name|traps
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display the traps set in the server"
block|}
block|,
block|{
literal|"addtrap"
block|,
name|addtrap
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|NO
block|}
block|,
block|{
literal|"address"
block|,
literal|"port"
block|,
literal|"interface"
block|,
literal|""
block|}
block|,
literal|"configure a trap in the server"
block|}
block|,
block|{
literal|"clrtrap"
block|,
name|clrtrap
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_UINT
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|NO
block|}
block|,
block|{
literal|"address"
block|,
literal|"port"
block|,
literal|"interface"
block|,
literal|""
block|}
block|,
literal|"remove a trap (configured or otherwise) from the server"
block|}
block|,
block|{
literal|"requestkey"
block|,
name|requestkey
block|,
block|{
name|NTP_UINT
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"keyid"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"change the keyid the server uses to authenticate requests"
block|}
block|,
block|{
literal|"controlkey"
block|,
name|controlkey
block|,
block|{
name|NTP_UINT
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|"keyid"
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"change the keyid the server uses to authenticate control messages"
block|}
block|,
block|{
literal|"ctlstats"
block|,
name|ctlstats
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display packet count statistics from the control module"
block|}
block|,
block|{
literal|"clockstat"
block|,
name|clockstat
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|}
block|,
block|{
literal|"address"
block|,
literal|"address"
block|,
literal|"address"
block|,
literal|"address"
block|}
block|,
literal|"display clock status information"
block|}
block|,
block|{
literal|"fudge"
block|,
name|fudge
block|,
block|{
name|NTP_ADD
block|,
name|NTP_STR
block|,
name|NTP_STR
block|,
name|NO
block|}
block|,
block|{
literal|"address"
block|,
literal|"time1|time2|val1|val2|flags"
block|,
literal|"value"
block|,
literal|""
block|}
block|,
literal|"set/change one of a clock's fudge factors"
block|}
block|,
block|{
literal|"clkbug"
block|,
name|clkbug
block|,
block|{
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|,
name|OPT
operator||
name|NTP_ADD
block|}
block|,
block|{
literal|"address"
block|,
literal|"address"
block|,
literal|"address"
block|,
literal|"address"
block|}
block|,
literal|"display clock debugging information"
block|}
block|,
block|{
literal|"kerninfo"
block|,
name|kerninfo
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"display the kernel pll/pps variables"
block|}
block|,
block|{
literal|"ifstats"
block|,
name|get_if_stats
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"list interface statistics"
block|}
block|,
block|{
literal|"ifreload"
block|,
name|do_if_reload
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|"reload interface configuration"
block|}
block|,
block|{
literal|0
block|,
literal|0
block|,
block|{
name|NO
block|,
name|NO
block|,
name|NO
block|,
name|NO
block|}
block|,
block|{
literal|""
block|,
literal|""
block|,
literal|""
block|,
literal|""
block|}
block|,
literal|""
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * For quick string comparisons  */
end_comment

begin_define
define|#
directive|define
name|STREQ
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|(*(a) == *(b)&& strcmp((a), (b)) == 0)
end_define

begin_comment
comment|/*  * SET_SS_LEN_IF_PRESENT - used by SET_ADDR, SET_ADDRS macros  */
end_comment

begin_ifdef
ifdef|#
directive|ifdef
name|ISC_PLATFORM_HAVESALEN
end_ifdef

begin_define
define|#
directive|define
name|SET_SS_LEN_IF_PRESENT
parameter_list|(
name|psau
parameter_list|)
define|\
value|do {							\ 		(psau)->sa.sa_len = SOCKLEN(psau);		\ 	} while (0)
end_define

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|SET_SS_LEN_IF_PRESENT
parameter_list|(
name|psau
parameter_list|)
value|do { } while (0)
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * SET_ADDR - setup address for v4/v6 as needed  */
end_comment

begin_define
define|#
directive|define
name|SET_ADDR
parameter_list|(
name|address
parameter_list|,
name|v6flag
parameter_list|,
name|v4addr
parameter_list|,
name|v6addr
parameter_list|)
define|\
value|do {								\ 	ZERO(address);						\ 	if (v6flag) {						\ 		AF(&(address)) = AF_INET6;			\ 		SOCK_ADDR6(&(address)) = (v6addr);		\ 	} else {						\ 		AF(&(address)) = AF_INET;			\ 		NSRCADR(&(address)) = (v4addr);			\ 	}							\ 	SET_SS_LEN_IF_PRESENT(&(address));			\ } while (0)
end_define

begin_comment
comment|/*  * SET_ADDRS - setup source and destination addresses for   * v4/v6 as needed  */
end_comment

begin_define
define|#
directive|define
name|SET_ADDRS
parameter_list|(
name|a1
parameter_list|,
name|a2
parameter_list|,
name|info
parameter_list|,
name|a1prefix
parameter_list|,
name|a2prefix
parameter_list|)
define|\
value|do {								\ 	ZERO(a1);						\ 	ZERO(a2);						\ 	if ((info)->v6_flag) {					\ 		AF(&(a1)) = AF_INET6;				\ 		AF(&(a2)) = AF_INET6;				\ 		SOCK_ADDR6(&(a1)) = (info)->a1prefix##6;	\ 		SOCK_ADDR6(&(a2)) = (info)->a2prefix##6;	\ 	} else {						\ 		AF(&(a1)) = AF_INET;				\ 		AF(&(a2)) = AF_INET;				\ 		NSRCADR(&(a1)) = (info)->a1prefix;		\ 		NSRCADR(&(a2)) = (info)->a2prefix;		\ 	}							\ 	SET_SS_LEN_IF_PRESENT(&(a1));				\ 	SET_SS_LEN_IF_PRESENT(&(a2));				\ } while (0)
end_define

begin_comment
comment|/*  * checkitems - utility to print a message if no items were returned  */
end_comment

begin_function
specifier|static
name|int
name|checkitems
parameter_list|(
name|int
name|items
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
if|if
condition|(
name|items
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"No data returned in response to query\n"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
literal|1
return|;
block|}
end_function

begin_comment
comment|/*  * checkitemsize - utility to print a message if the item size is wrong  */
end_comment

begin_function
specifier|static
name|int
name|checkitemsize
parameter_list|(
name|int
name|itemsize
parameter_list|,
name|int
name|expected
parameter_list|)
block|{
if|if
condition|(
name|itemsize
operator|!=
name|expected
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Incorrect item size returned by remote host (%d should be %d)\n"
argument_list|,
name|itemsize
argument_list|,
name|expected
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
literal|1
return|;
block|}
end_function

begin_comment
comment|/*  * check1item - check to make sure we have exactly one item  */
end_comment

begin_function
specifier|static
name|int
name|check1item
parameter_list|(
name|int
name|items
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
if|if
condition|(
name|items
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"No data returned in response to query\n"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
name|items
operator|>
literal|1
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"Expected one item in response, got %d\n"
argument_list|,
name|items
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
literal|1
return|;
block|}
end_function

begin_comment
comment|/*  * peerlist - get a short list of peers  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|peerlist
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_peer_list
modifier|*
name|plist
decl_stmt|;
name|sockaddr_u
name|paddr
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_PEER_LIST
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|plist
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_list
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_list
argument_list|)
argument_list|)
operator|&&
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
name|v4sizeof
argument_list|(
expr|struct
name|info_peer_list
argument_list|)
argument_list|)
condition|)
return|return;
while|while
condition|(
name|items
operator|>
literal|0
condition|)
block|{
name|SET_ADDR
argument_list|(
name|paddr
argument_list|,
name|plist
operator|->
name|v6_flag
argument_list|,
name|plist
operator|->
name|addr
argument_list|,
name|plist
operator|->
name|addr6
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|pcmd
operator|->
name|nargs
operator|==
literal|0
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|6
operator|)
operator|&&
operator|(
name|plist
operator|->
name|v6_flag
operator|!=
literal|0
operator|)
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|4
operator|)
operator|&&
operator|(
name|plist
operator|->
name|v6_flag
operator|==
literal|0
operator|)
operator|)
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%-9s %s\n"
argument_list|,
name|modetoa
argument_list|(
name|plist
operator|->
name|hmode
argument_list|)
argument_list|,
name|nntohost
argument_list|(
operator|&
name|paddr
argument_list|)
argument_list|)
expr_stmt|;
name|plist
operator|++
expr_stmt|;
name|items
operator|--
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * peers - show peer summary  */
end_comment

begin_function
specifier|static
name|void
name|peers
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|dopeers
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * dmpeers - show peer summary, Dave Mills style  */
end_comment

begin_function
specifier|static
name|void
name|dmpeers
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|dopeers
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * peers - show peer summary  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|dopeers
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|dmstyle
parameter_list|)
block|{
name|struct
name|info_peer_summary
modifier|*
name|plist
decl_stmt|;
name|sockaddr_u
name|dstadr
decl_stmt|;
name|sockaddr_u
name|srcadr
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|ntp_poll
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|c
decl_stmt|;
name|l_fp
name|tempts
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_PEER_LIST_SUM
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|plist
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_summary
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_summary
argument_list|)
argument_list|)
operator|&&
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
name|v4sizeof
argument_list|(
expr|struct
name|info_peer_summary
argument_list|)
argument_list|)
condition|)
return|return;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"     remote           local      st poll reach  delay   offset    disp\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"=======================================================================\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|items
operator|>
literal|0
condition|)
block|{
if|if
condition|(
operator|!
name|dmstyle
condition|)
block|{
if|if
condition|(
name|plist
operator|->
name|flags
operator|&
name|INFO_FLAG_SYSPEER
condition|)
name|c
operator|=
literal|'*'
expr_stmt|;
elseif|else
if|if
condition|(
name|plist
operator|->
name|hmode
operator|==
name|MODE_ACTIVE
condition|)
name|c
operator|=
literal|'+'
expr_stmt|;
elseif|else
if|if
condition|(
name|plist
operator|->
name|hmode
operator|==
name|MODE_PASSIVE
condition|)
name|c
operator|=
literal|'-'
expr_stmt|;
elseif|else
if|if
condition|(
name|plist
operator|->
name|hmode
operator|==
name|MODE_CLIENT
condition|)
name|c
operator|=
literal|'='
expr_stmt|;
elseif|else
if|if
condition|(
name|plist
operator|->
name|hmode
operator|==
name|MODE_BROADCAST
condition|)
name|c
operator|=
literal|'^'
expr_stmt|;
elseif|else
if|if
condition|(
name|plist
operator|->
name|hmode
operator|==
name|MODE_BCLIENT
condition|)
name|c
operator|=
literal|'~'
expr_stmt|;
else|else
name|c
operator|=
literal|' '
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|plist
operator|->
name|flags
operator|&
name|INFO_FLAG_SYSPEER
condition|)
name|c
operator|=
literal|'*'
expr_stmt|;
elseif|else
if|if
condition|(
name|plist
operator|->
name|flags
operator|&
name|INFO_FLAG_SHORTLIST
condition|)
name|c
operator|=
literal|'+'
expr_stmt|;
elseif|else
if|if
condition|(
name|plist
operator|->
name|flags
operator|&
name|INFO_FLAG_SEL_CANDIDATE
condition|)
name|c
operator|=
literal|'.'
expr_stmt|;
else|else
name|c
operator|=
literal|' '
expr_stmt|;
block|}
name|NTOHL_FP
argument_list|(
operator|&
operator|(
name|plist
operator|->
name|offset
operator|)
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
name|ntp_poll
operator|=
literal|1
operator|<<
name|max
argument_list|(
name|min3
argument_list|(
name|plist
operator|->
name|ppoll
argument_list|,
name|plist
operator|->
name|hpoll
argument_list|,
name|NTP_MAXPOLL
argument_list|)
argument_list|,
name|NTP_MINPOLL
argument_list|)
expr_stmt|;
name|SET_ADDRS
argument_list|(
name|dstadr
argument_list|,
name|srcadr
argument_list|,
name|plist
argument_list|,
name|dstadr
argument_list|,
name|srcadr
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|pcmd
operator|->
name|nargs
operator|==
literal|0
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|6
operator|)
operator|&&
operator|(
name|plist
operator|->
name|v6_flag
operator|!=
literal|0
operator|)
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|4
operator|)
operator|&&
operator|(
name|plist
operator|->
name|v6_flag
operator|==
literal|0
operator|)
operator|)
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%c%-15.15s %-15.15s %2u %4d  %3o %7.7s %9.9s %7.7s\n"
argument_list|,
name|c
argument_list|,
name|nntohost
argument_list|(
operator|&
name|srcadr
argument_list|)
argument_list|,
name|stoa
argument_list|(
operator|&
name|dstadr
argument_list|)
argument_list|,
name|plist
operator|->
name|stratum
argument_list|,
name|ntp_poll
argument_list|,
name|plist
operator|->
name|reach
argument_list|,
name|fptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|plist
operator|->
name|delay
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|tempts
argument_list|,
literal|6
argument_list|)
argument_list|,
name|ufptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|plist
operator|->
name|dispersion
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|)
expr_stmt|;
name|plist
operator|++
expr_stmt|;
name|items
operator|--
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/* Convert a refid& stratum (in host order) to a string */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|refid_string
parameter_list|(
name|u_int32
name|refid
parameter_list|,
name|int
name|stratum
parameter_list|)
block|{
if|if
condition|(
name|stratum
operator|<=
literal|1
condition|)
block|{
specifier|static
name|char
name|junk
index|[
literal|5
index|]
decl_stmt|;
name|junk
index|[
literal|4
index|]
operator|=
literal|0
expr_stmt|;
name|memcpy
argument_list|(
name|junk
argument_list|,
operator|&
name|refid
argument_list|,
literal|4
argument_list|)
expr_stmt|;
return|return
name|junk
return|;
block|}
return|return
name|numtoa
argument_list|(
name|refid
argument_list|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|print_pflag
parameter_list|(
name|FILE
modifier|*
name|fp
parameter_list|,
name|u_int32
name|flags
parameter_list|)
block|{
specifier|static
specifier|const
name|char
name|none
index|[]
init|=
literal|""
decl_stmt|;
specifier|static
specifier|const
name|char
name|comma
index|[]
init|=
literal|","
decl_stmt|;
specifier|const
name|char
modifier|*
name|dlim
decl_stmt|;
if|if
condition|(
literal|0
operator|==
name|flags
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" none\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|dlim
operator|=
name|none
expr_stmt|;
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_SYSPEER
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" system_peer"
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_CONFIG
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s config"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_REFCLOCK
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s refclock"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_AUTHENABLE
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s auth"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_PREFER
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s prefer"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_IBURST
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s iburst"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_BURST
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s burst"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_SEL_CANDIDATE
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s candidate"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
if|if
condition|(
name|flags
operator|&
name|INFO_FLAG_SHORTLIST
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%s shortlist"
argument_list|,
name|dlim
argument_list|)
expr_stmt|;
name|dlim
operator|=
name|comma
expr_stmt|;
block|}
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * printpeer - print detail information for a peer  */
end_comment

begin_function
specifier|static
name|void
name|printpeer
parameter_list|(
specifier|register
name|struct
name|info_peer
modifier|*
name|pp
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
specifier|register
name|int
name|i
decl_stmt|;
name|l_fp
name|tempts
decl_stmt|;
name|sockaddr_u
name|srcadr
decl_stmt|,
name|dstadr
decl_stmt|;
name|SET_ADDRS
argument_list|(
name|dstadr
argument_list|,
name|srcadr
argument_list|,
name|pp
argument_list|,
name|dstadr
argument_list|,
name|srcadr
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"remote %s, local %s\n"
argument_list|,
name|stoa
argument_list|(
operator|&
name|srcadr
argument_list|)
argument_list|,
name|stoa
argument_list|(
operator|&
name|dstadr
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"hmode %s, pmode %s, stratum %d, precision %d\n"
argument_list|,
name|modetoa
argument_list|(
name|pp
operator|->
name|hmode
argument_list|)
argument_list|,
name|modetoa
argument_list|(
name|pp
operator|->
name|pmode
argument_list|)
argument_list|,
name|pp
operator|->
name|stratum
argument_list|,
name|pp
operator|->
name|precision
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"leap %c%c, refid [%s], rootdistance %s, rootdispersion %s\n"
argument_list|,
name|pp
operator|->
name|leap
operator|&
literal|0x2
condition|?
literal|'1'
else|:
literal|'0'
argument_list|,
name|pp
operator|->
name|leap
operator|&
literal|0x1
condition|?
literal|'1'
else|:
literal|'0'
argument_list|,
name|refid_string
argument_list|(
name|pp
operator|->
name|refid
argument_list|,
name|pp
operator|->
name|stratum
argument_list|)
argument_list|,
name|fptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|pp
operator|->
name|rootdelay
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|,
name|ufptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|pp
operator|->
name|rootdispersion
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"ppoll %d, hpoll %d, keyid %lu, version %d, association %u\n"
argument_list|,
name|pp
operator|->
name|ppoll
argument_list|,
name|pp
operator|->
name|hpoll
argument_list|,
operator|(
name|u_long
operator|)
name|pp
operator|->
name|keyid
argument_list|,
name|pp
operator|->
name|version
argument_list|,
name|ntohs
argument_list|(
name|pp
operator|->
name|associd
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"reach %03o, unreach %d, flash 0x%04x, "
argument_list|,
name|pp
operator|->
name|reach
argument_list|,
name|pp
operator|->
name|unreach
argument_list|,
name|pp
operator|->
name|flash2
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"boffset %s, ttl/mode %d\n"
argument_list|,
name|fptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|pp
operator|->
name|estbdelay
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|,
name|pp
operator|->
name|ttl
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"timer %lds, flags"
argument_list|,
operator|(
name|long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|timer
argument_list|)
argument_list|)
expr_stmt|;
name|print_pflag
argument_list|(
name|fp
argument_list|,
name|pp
operator|->
name|flags
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|pp
operator|->
name|reftime
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"reference time:      %s\n"
argument_list|,
name|prettydate
argument_list|(
operator|&
name|tempts
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|pp
operator|->
name|org
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"originate timestamp: %s\n"
argument_list|,
name|prettydate
argument_list|(
operator|&
name|tempts
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|pp
operator|->
name|rec
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"receive timestamp:   %s\n"
argument_list|,
name|prettydate
argument_list|(
operator|&
name|tempts
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|pp
operator|->
name|xmt
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"transmit timestamp:  %s\n"
argument_list|,
name|prettydate
argument_list|(
operator|&
name|tempts
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"filter delay: "
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NTP_SHIFT
condition|;
name|i
operator|++
control|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" %-8.8s"
argument_list|,
name|fptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|pp
operator|->
name|filtdelay
index|[
name|i
index|]
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|==
operator|(
name|NTP_SHIFT
operator|>>
literal|1
operator|)
operator|-
literal|1
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n              "
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"filter offset:"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NTP_SHIFT
condition|;
name|i
operator|++
control|)
block|{
name|NTOHL_FP
argument_list|(
operator|&
name|pp
operator|->
name|filtoffset
index|[
name|i
index|]
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" %-8.8s"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|tempts
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|==
operator|(
name|NTP_SHIFT
operator|>>
literal|1
operator|)
operator|-
literal|1
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n              "
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"filter order: "
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NTP_SHIFT
condition|;
name|i
operator|++
control|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" %-8d"
argument_list|,
name|pp
operator|->
name|order
index|[
name|i
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|==
operator|(
name|NTP_SHIFT
operator|>>
literal|1
operator|)
operator|-
literal|1
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n              "
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|pp
operator|->
name|offset
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"offset %s, delay %s, error bound %s, filter error %s\n"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|tempts
argument_list|,
literal|6
argument_list|)
argument_list|,
name|fptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|pp
operator|->
name|delay
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|,
name|ufptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|pp
operator|->
name|dispersion
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|,
name|ufptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|pp
operator|->
name|selectdisp
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * showpeer - show detailed information for a peer  */
end_comment

begin_function
specifier|static
name|void
name|showpeer
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_peer
modifier|*
name|pp
decl_stmt|;
comment|/* 4 is the maximum number of peers which will fit in a packet */
name|struct
name|info_peer_list
modifier|*
name|pl
decl_stmt|,
name|plist
index|[
name|min
argument_list|(
name|MAXARGS
argument_list|,
literal|4
argument_list|)
index|]
decl_stmt|;
name|int
name|qitemlim
decl_stmt|;
name|int
name|qitems
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|sendsize
decl_stmt|;
name|again
label|:
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|sendsize
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_list
argument_list|)
expr_stmt|;
else|else
name|sendsize
operator|=
name|v4sizeof
argument_list|(
expr|struct
name|info_peer_list
argument_list|)
expr_stmt|;
name|qitemlim
operator|=
name|min
argument_list|(
name|pcmd
operator|->
name|nargs
argument_list|,
name|COUNTOF
argument_list|(
name|plist
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|qitems
operator|=
literal|0
operator|,
name|pl
operator|=
name|plist
init|;
name|qitems
operator|<
name|qitemlim
condition|;
name|qitems
operator|++
control|)
block|{
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|pl
operator|->
name|addr
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|pl
operator|->
name|v6_flag
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|pl
operator|->
name|addr6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|pl
operator|->
name|v6_flag
operator|=
literal|1
expr_stmt|;
block|}
name|pl
operator|->
name|port
operator|=
operator|(
name|u_short
operator|)
name|s_port
expr_stmt|;
name|pl
operator|->
name|hmode
operator|=
name|pl
operator|->
name|flags
operator|=
literal|0
expr_stmt|;
name|pl
operator|=
operator|(
name|void
operator|*
operator|)
operator|(
operator|(
name|char
operator|*
operator|)
name|pl
operator|+
name|sendsize
operator|)
expr_stmt|;
block|}
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_PEER_INFO
argument_list|,
literal|0
argument_list|,
name|qitems
argument_list|,
name|sendsize
argument_list|,
operator|(
name|char
operator|*
operator|)
name|plist
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|pp
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer
argument_list|)
argument_list|)
operator|&&
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
name|v4sizeof
argument_list|(
expr|struct
name|info_peer
argument_list|)
argument_list|)
condition|)
return|return;
while|while
condition|(
name|items
operator|--
operator|>
literal|0
condition|)
block|{
name|printpeer
argument_list|(
name|pp
argument_list|,
name|fp
argument_list|)
expr_stmt|;
if|if
condition|(
name|items
operator|>
literal|0
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
name|pp
operator|++
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * peerstats - return statistics for a peer  */
end_comment

begin_function
specifier|static
name|void
name|peerstats
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_peer_stats
modifier|*
name|pp
decl_stmt|;
comment|/* 4 is the maximum number of peers which will fit in a packet */
name|struct
name|info_peer_list
modifier|*
name|pl
decl_stmt|,
name|plist
index|[
name|min
argument_list|(
name|MAXARGS
argument_list|,
literal|4
argument_list|)
index|]
decl_stmt|;
name|sockaddr_u
name|src
decl_stmt|,
name|dst
decl_stmt|;
name|int
name|qitemlim
decl_stmt|;
name|int
name|qitems
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|sendsize
decl_stmt|;
name|again
label|:
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|sendsize
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_list
argument_list|)
expr_stmt|;
else|else
name|sendsize
operator|=
name|v4sizeof
argument_list|(
expr|struct
name|info_peer_list
argument_list|)
expr_stmt|;
name|ZERO
argument_list|(
name|plist
argument_list|)
expr_stmt|;
name|qitemlim
operator|=
name|min
argument_list|(
name|pcmd
operator|->
name|nargs
argument_list|,
name|COUNTOF
argument_list|(
name|plist
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|qitems
operator|=
literal|0
operator|,
name|pl
operator|=
name|plist
init|;
name|qitems
operator|<
name|qitemlim
condition|;
name|qitems
operator|++
control|)
block|{
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|pl
operator|->
name|addr
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|pl
operator|->
name|v6_flag
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|pl
operator|->
name|addr6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|pl
operator|->
name|v6_flag
operator|=
literal|1
expr_stmt|;
block|}
name|pl
operator|->
name|port
operator|=
operator|(
name|u_short
operator|)
name|s_port
expr_stmt|;
name|pl
operator|->
name|hmode
operator|=
name|plist
index|[
name|qitems
index|]
operator|.
name|flags
operator|=
literal|0
expr_stmt|;
name|pl
operator|=
operator|(
name|void
operator|*
operator|)
operator|(
operator|(
name|char
operator|*
operator|)
name|pl
operator|+
name|sendsize
operator|)
expr_stmt|;
block|}
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_PEER_STATS
argument_list|,
literal|0
argument_list|,
name|qitems
argument_list|,
name|sendsize
argument_list|,
operator|(
name|char
operator|*
operator|)
name|plist
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|pp
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_stats
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_peer_stats
argument_list|)
argument_list|)
operator|&&
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
name|v4sizeof
argument_list|(
expr|struct
name|info_peer_stats
argument_list|)
argument_list|)
condition|)
return|return;
while|while
condition|(
name|items
operator|--
operator|>
literal|0
condition|)
block|{
name|ZERO_SOCK
argument_list|(
operator|&
name|dst
argument_list|)
expr_stmt|;
name|ZERO_SOCK
argument_list|(
operator|&
name|src
argument_list|)
expr_stmt|;
if|if
condition|(
name|pp
operator|->
name|v6_flag
operator|!=
literal|0
condition|)
block|{
name|AF
argument_list|(
operator|&
name|dst
argument_list|)
operator|=
name|AF_INET6
expr_stmt|;
name|AF
argument_list|(
operator|&
name|src
argument_list|)
operator|=
name|AF_INET6
expr_stmt|;
name|SOCK_ADDR6
argument_list|(
operator|&
name|dst
argument_list|)
operator|=
name|pp
operator|->
name|dstadr6
expr_stmt|;
name|SOCK_ADDR6
argument_list|(
operator|&
name|src
argument_list|)
operator|=
name|pp
operator|->
name|srcadr6
expr_stmt|;
block|}
else|else
block|{
name|AF
argument_list|(
operator|&
name|dst
argument_list|)
operator|=
name|AF_INET
expr_stmt|;
name|AF
argument_list|(
operator|&
name|src
argument_list|)
operator|=
name|AF_INET
expr_stmt|;
name|NSRCADR
argument_list|(
operator|&
name|dst
argument_list|)
operator|=
name|pp
operator|->
name|dstadr
expr_stmt|;
name|NSRCADR
argument_list|(
operator|&
name|src
argument_list|)
operator|=
name|pp
operator|->
name|srcadr
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|ISC_PLATFORM_HAVESALEN
name|src
operator|.
name|sa
operator|.
name|sa_len
operator|=
name|SOCKLEN
argument_list|(
operator|&
name|src
argument_list|)
expr_stmt|;
name|dst
operator|.
name|sa
operator|.
name|sa_len
operator|=
name|SOCKLEN
argument_list|(
operator|&
name|dst
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"remote host:          %s\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
name|src
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"local interface:      %s\n"
argument_list|,
name|stoa
argument_list|(
operator|&
name|dst
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time last received:   %lus\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|timereceived
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time until next send: %lus\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|timetosend
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"reachability change:  %lus\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|timereachable
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"packets sent:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|sent
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"packets received:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|processed
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad authentication:   %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|badauth
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bogus origin:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|bogusorg
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"duplicate:            %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|oldpkt
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad dispersion:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|seldisp
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad reference time:   %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|pp
operator|->
name|selbroken
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"candidate order:      %u\n"
argument_list|,
name|pp
operator|->
name|candidate
argument_list|)
expr_stmt|;
if|if
condition|(
name|items
operator|>
literal|0
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"flags:	"
argument_list|)
expr_stmt|;
name|print_pflag
argument_list|(
name|fp
argument_list|,
name|ntohs
argument_list|(
name|pp
operator|->
name|flags
argument_list|)
argument_list|)
expr_stmt|;
name|pp
operator|++
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * loopinfo - show loop filter information  */
end_comment

begin_function
specifier|static
name|void
name|loopinfo
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_loop
modifier|*
name|il
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|oneline
init|=
literal|0
decl_stmt|;
name|int
name|res
decl_stmt|;
name|l_fp
name|tempts
decl_stmt|;
if|if
condition|(
name|pcmd
operator|->
name|nargs
operator|>
literal|0
condition|)
block|{
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|string
argument_list|,
literal|"oneline"
argument_list|)
condition|)
name|oneline
operator|=
literal|1
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|string
argument_list|,
literal|"multiline"
argument_list|)
condition|)
name|oneline
operator|=
literal|0
expr_stmt|;
else|else
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"How many lines?\n"
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_LOOP_INFO
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|il
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_loop
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_loop
argument_list|)
argument_list|)
condition|)
return|return;
if|if
condition|(
name|oneline
condition|)
block|{
name|l_fp
name|temp2ts
decl_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|il
operator|->
name|last_offset
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|il
operator|->
name|drift_comp
argument_list|,
operator|&
name|temp2ts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"offset %s, frequency %s, time_const %ld, watchdog %ld\n"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|tempts
argument_list|,
literal|6
argument_list|)
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|temp2ts
argument_list|,
literal|3
argument_list|)
argument_list|,
operator|(
name|long
operator|)
operator|(
name|int32
operator|)
name|ntohl
argument_list|(
operator|(
name|u_long
operator|)
name|il
operator|->
name|compliance
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
operator|(
name|u_long
operator|)
name|il
operator|->
name|watchdog_timer
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|NTOHL_FP
argument_list|(
operator|&
name|il
operator|->
name|last_offset
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"offset:               %s s\n"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|tempts
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|il
operator|->
name|drift_comp
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"frequency:            %s ppm\n"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|tempts
argument_list|,
literal|3
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"poll adjust:          %ld\n"
argument_list|,
operator|(
name|long
operator|)
operator|(
name|int32
operator|)
name|ntohl
argument_list|(
name|il
operator|->
name|compliance
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"watchdog timer:       %ld s\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|il
operator|->
name|watchdog_timer
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * sysinfo - show current system state  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|sysinfo
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_sys
modifier|*
name|is
decl_stmt|;
name|sockaddr_u
name|peeraddr
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|l_fp
name|tempts
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_SYS_INFO
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|is
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_sys
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_sys
argument_list|)
argument_list|)
operator|&&
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
name|v4sizeof
argument_list|(
expr|struct
name|info_sys
argument_list|)
argument_list|)
condition|)
return|return;
name|SET_ADDR
argument_list|(
name|peeraddr
argument_list|,
name|is
operator|->
name|v6_flag
argument_list|,
name|is
operator|->
name|peer
argument_list|,
name|is
operator|->
name|peer6
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"system peer:          %s\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
name|peeraddr
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"system peer mode:     %s\n"
argument_list|,
name|modetoa
argument_list|(
name|is
operator|->
name|peer_mode
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"leap indicator:       %c%c\n"
argument_list|,
name|is
operator|->
name|leap
operator|&
literal|0x2
condition|?
literal|'1'
else|:
literal|'0'
argument_list|,
name|is
operator|->
name|leap
operator|&
literal|0x1
condition|?
literal|'1'
else|:
literal|'0'
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"stratum:              %d\n"
argument_list|,
operator|(
name|int
operator|)
name|is
operator|->
name|stratum
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"precision:            %d\n"
argument_list|,
operator|(
name|int
operator|)
name|is
operator|->
name|precision
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"root distance:        %s s\n"
argument_list|,
name|fptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|is
operator|->
name|rootdelay
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"root dispersion:      %s s\n"
argument_list|,
name|ufptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|is
operator|->
name|rootdispersion
argument_list|)
argument_list|,
literal|5
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"reference ID:         [%s]\n"
argument_list|,
name|refid_string
argument_list|(
name|is
operator|->
name|refid
argument_list|,
name|is
operator|->
name|stratum
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|is
operator|->
name|reftime
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"reference time:       %s\n"
argument_list|,
name|prettydate
argument_list|(
operator|&
name|tempts
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"system flags:         "
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|is
operator|->
name|flags
operator|&
operator|(
name|INFO_FLAG_BCLIENT
operator||
name|INFO_FLAG_AUTHENABLE
operator||
name|INFO_FLAG_NTP
operator||
name|INFO_FLAG_KERNEL
operator||
name|INFO_FLAG_CAL
operator||
name|INFO_FLAG_PPS_SYNC
operator||
name|INFO_FLAG_MONITOR
operator||
name|INFO_FLAG_FILEGEN
operator|)
operator|)
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"none\n"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_BCLIENT
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bclient "
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_AUTHENTICATE
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"auth "
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_MONITOR
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"monitor "
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_NTP
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"ntp "
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_KERNEL
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"kernel "
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_FILEGEN
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"stats "
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_CAL
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"calibrate "
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|flags
operator|&
name|INFO_FLAG_PPS_SYNC
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"pps "
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"jitter:               %s s\n"
argument_list|,
name|fptoa
argument_list|(
name|ntohl
argument_list|(
name|is
operator|->
name|frequency
argument_list|)
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"stability:            %s ppm\n"
argument_list|,
name|ufptoa
argument_list|(
name|ntohl
argument_list|(
name|is
operator|->
name|stability
argument_list|)
argument_list|,
literal|3
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"broadcastdelay:       %s s\n"
argument_list|,
name|fptoa
argument_list|(
name|NTOHS_FP
argument_list|(
name|is
operator|->
name|bdelay
argument_list|)
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|is
operator|->
name|authdelay
argument_list|,
operator|&
name|tempts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"authdelay:            %s s\n"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|tempts
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * sysstats - print system statistics  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|sysstats
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_sys_stats
modifier|*
name|ss
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_SYS_STATS
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|ss
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_sys_stats
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
name|itemsize
operator|!=
sizeof|sizeof
argument_list|(
expr|struct
name|info_sys_stats
argument_list|)
operator|&&
name|itemsize
operator|!=
sizeof|sizeof
argument_list|(
expr|struct
name|old_info_sys_stats
argument_list|)
condition|)
block|{
comment|/* issue warning according to new structure size */
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_sys_stats
argument_list|)
argument_list|)
expr_stmt|;
return|return;
block|}
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time since restart:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|timeup
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time since reset:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|timereset
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"packets received:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|received
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"packets processed:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|processed
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"current version:        %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|newversionpkt
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"previous version:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|oldversionpkt
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"declined:               %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|unknownversion
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"access denied:          %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|denied
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad length or format:   %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|badlength
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad authentication:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|badauth
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|itemsize
operator|!=
sizeof|sizeof
argument_list|(
expr|struct
name|info_sys_stats
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"rate exceeded:          %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ss
operator|->
name|limitrejected
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * iostats - print I/O statistics  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|iostats
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_io_stats
modifier|*
name|io
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_IO_STATS
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|io
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|io
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|io
argument_list|)
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time since reset:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|timereset
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"receive buffers:      %u\n"
argument_list|,
operator|(
name|u_int
operator|)
name|ntohs
argument_list|(
name|io
operator|->
name|totalrecvbufs
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"free receive buffers: %u\n"
argument_list|,
operator|(
name|u_int
operator|)
name|ntohs
argument_list|(
name|io
operator|->
name|freerecvbufs
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"used receive buffers: %u\n"
argument_list|,
operator|(
name|u_int
operator|)
name|ntohs
argument_list|(
name|io
operator|->
name|fullrecvbufs
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"low water refills:    %u\n"
argument_list|,
operator|(
name|u_int
operator|)
name|ntohs
argument_list|(
name|io
operator|->
name|lowwater
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"dropped packets:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|dropped
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"ignored packets:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|ignored
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"received packets:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|received
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"packets sent:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|sent
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"packets not sent:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|notsent
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"interrupts handled:   %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|interrupts
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"received by int:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|io
operator|->
name|int_received
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * memstats - print peer memory statistics  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|memstats
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_mem_stats
modifier|*
name|mem
decl_stmt|;
name|int
name|i
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_MEM_STATS
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|mem
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|mem
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|mem
argument_list|)
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time since reset:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|mem
operator|->
name|timereset
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"total peer memory:    %u\n"
argument_list|,
operator|(
name|u_int
operator|)
name|ntohs
argument_list|(
name|mem
operator|->
name|totalpeermem
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"free peer memory:     %u\n"
argument_list|,
operator|(
name|u_int
operator|)
name|ntohs
argument_list|(
name|mem
operator|->
name|freepeermem
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"calls to findpeer:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|mem
operator|->
name|findpeer_calls
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"new peer allocations: %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|mem
operator|->
name|allocations
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"peer demobilizations: %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|mem
operator|->
name|demobilizations
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"hash table counts:   "
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NTP_HASH_SIZE
condition|;
name|i
operator|++
control|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%4d"
argument_list|,
operator|(
name|int
operator|)
name|mem
operator|->
name|hashcount
index|[
name|i
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|i
operator|%
literal|8
operator|)
operator|==
literal|7
operator|&&
name|i
operator|!=
operator|(
name|NTP_HASH_SIZE
operator|-
literal|1
operator|)
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n                     "
argument_list|)
expr_stmt|;
block|}
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * timerstats - print timer statistics  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|timerstats
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_timer_stats
modifier|*
name|tim
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_TIMER_STATS
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|tim
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|tim
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|tim
argument_list|)
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time since reset:  %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|tim
operator|->
name|timereset
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"alarms handled:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|tim
operator|->
name|alarms
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"alarm overruns:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|tim
operator|->
name|overflows
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"calls to transmit: %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|tim
operator|->
name|xmtcalls
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * addpeer - configure an active mode association  */
end_comment

begin_function
specifier|static
name|void
name|addpeer
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|doconfig
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|MODE_ACTIVE
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * addserver - configure a client mode association  */
end_comment

begin_function
specifier|static
name|void
name|addserver
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|doconfig
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|MODE_CLIENT
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * addrefclock - configure a reference clock association  */
end_comment

begin_function
specifier|static
name|void
name|addrefclock
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|doconfig
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|MODE_CLIENT
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * broadcast - configure a broadcast mode association  */
end_comment

begin_function
specifier|static
name|void
name|broadcast
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|doconfig
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|MODE_BROADCAST
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * config - configure a new peer association  */
end_comment

begin_function
specifier|static
name|void
name|doconfig
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|mode
parameter_list|,
name|int
name|refc
parameter_list|)
block|{
name|struct
name|conf_peer
name|cpeer
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|u_long
name|keyid
decl_stmt|;
name|u_int
name|version
decl_stmt|;
name|u_char
name|minpoll
decl_stmt|;
name|u_char
name|maxpoll
decl_stmt|;
name|u_int
name|flags
decl_stmt|;
name|u_char
name|cmode
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|sendsize
decl_stmt|;
name|int
name|numtyp
decl_stmt|;
name|long
name|val
decl_stmt|;
name|again
label|:
name|keyid
operator|=
literal|0
expr_stmt|;
name|version
operator|=
literal|3
expr_stmt|;
name|flags
operator|=
literal|0
expr_stmt|;
name|res
operator|=
name|FALSE
expr_stmt|;
name|cmode
operator|=
literal|0
expr_stmt|;
name|minpoll
operator|=
name|NTP_MINDPOLL
expr_stmt|;
name|maxpoll
operator|=
name|NTP_MAXDPOLL
expr_stmt|;
name|numtyp
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|refc
condition|)
name|numtyp
operator|=
literal|5
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|sendsize
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|conf_peer
argument_list|)
expr_stmt|;
else|else
name|sendsize
operator|=
name|v4sizeof
argument_list|(
expr|struct
name|conf_peer
argument_list|)
expr_stmt|;
name|items
operator|=
literal|1
expr_stmt|;
while|while
condition|(
name|pcmd
operator|->
name|nargs
operator|>
name|items
condition|)
block|{
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"prefer"
argument_list|)
condition|)
name|flags
operator||=
name|CONF_FLAG_PREFER
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"burst"
argument_list|)
condition|)
name|flags
operator||=
name|CONF_FLAG_BURST
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"iburst"
argument_list|)
condition|)
name|flags
operator||=
name|CONF_FLAG_IBURST
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|refc
operator|&&
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"keyid"
argument_list|)
condition|)
name|numtyp
operator|=
literal|1
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|refc
operator|&&
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"version"
argument_list|)
condition|)
name|numtyp
operator|=
literal|2
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"minpoll"
argument_list|)
condition|)
name|numtyp
operator|=
literal|3
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"maxpoll"
argument_list|)
condition|)
name|numtyp
operator|=
literal|4
expr_stmt|;
else|else
block|{
if|if
condition|(
operator|!
name|atoint
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
operator|&
name|val
argument_list|)
condition|)
name|numtyp
operator|=
literal|0
expr_stmt|;
switch|switch
condition|(
name|numtyp
condition|)
block|{
case|case
literal|1
case|:
name|keyid
operator|=
name|val
expr_stmt|;
name|numtyp
operator|=
literal|2
expr_stmt|;
break|break;
case|case
literal|2
case|:
name|version
operator|=
operator|(
name|u_int
operator|)
name|val
expr_stmt|;
name|numtyp
operator|=
literal|0
expr_stmt|;
break|break;
case|case
literal|3
case|:
name|minpoll
operator|=
operator|(
name|u_char
operator|)
name|val
expr_stmt|;
name|numtyp
operator|=
literal|0
expr_stmt|;
break|break;
case|case
literal|4
case|:
name|maxpoll
operator|=
operator|(
name|u_char
operator|)
name|val
expr_stmt|;
name|numtyp
operator|=
literal|0
expr_stmt|;
break|break;
case|case
literal|5
case|:
name|cmode
operator|=
operator|(
name|u_char
operator|)
name|val
expr_stmt|;
name|numtyp
operator|=
literal|0
expr_stmt|;
break|break;
default|default:
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"*** '%s' not understood\n"
argument_list|,
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
name|res
operator|=
name|TRUE
expr_stmt|;
name|numtyp
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|val
operator|<
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"*** Value '%s' should be unsigned\n"
argument_list|,
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
name|res
operator|=
name|TRUE
expr_stmt|;
block|}
block|}
name|items
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|keyid
operator|>
literal|0
condition|)
name|flags
operator||=
name|CONF_FLAG_AUTHENABLE
expr_stmt|;
if|if
condition|(
name|version
operator|>
name|NTP_VERSION
operator|||
name|version
operator|<
name|NTP_OLDVERSION
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"***invalid version number: %u\n"
argument_list|,
name|version
argument_list|)
expr_stmt|;
name|res
operator|=
name|TRUE
expr_stmt|;
block|}
if|if
condition|(
name|minpoll
operator|<
name|NTP_MINPOLL
operator|||
name|minpoll
operator|>
name|NTP_MAXPOLL
operator|||
name|maxpoll
operator|<
name|NTP_MINPOLL
operator|||
name|maxpoll
operator|>
name|NTP_MAXPOLL
operator|||
name|minpoll
operator|>
name|maxpoll
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"***min/max-poll must be within %d..%d\n"
argument_list|,
name|NTP_MINPOLL
argument_list|,
name|NTP_MAXPOLL
argument_list|)
expr_stmt|;
name|res
operator|=
name|TRUE
expr_stmt|;
block|}
if|if
condition|(
name|res
condition|)
return|return;
name|ZERO
argument_list|(
name|cpeer
argument_list|)
expr_stmt|;
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|cpeer
operator|.
name|peeraddr
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|cpeer
operator|.
name|v6_flag
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|cpeer
operator|.
name|peeraddr6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|cpeer
operator|.
name|v6_flag
operator|=
literal|1
expr_stmt|;
block|}
name|cpeer
operator|.
name|hmode
operator|=
operator|(
name|u_char
operator|)
name|mode
expr_stmt|;
name|cpeer
operator|.
name|keyid
operator|=
name|keyid
expr_stmt|;
name|cpeer
operator|.
name|version
operator|=
operator|(
name|u_char
operator|)
name|version
expr_stmt|;
name|cpeer
operator|.
name|minpoll
operator|=
name|minpoll
expr_stmt|;
name|cpeer
operator|.
name|maxpoll
operator|=
name|maxpoll
expr_stmt|;
name|cpeer
operator|.
name|flags
operator|=
operator|(
name|u_char
operator|)
name|flags
expr_stmt|;
name|cpeer
operator|.
name|ttl
operator|=
name|cmode
expr_stmt|;
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_CONFIG
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
name|sendsize
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|cpeer
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_peer
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
name|INFO_ERR_FMT
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"***Retrying command with old conf_peer size\n"
argument_list|)
expr_stmt|;
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_CONFIG
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|old_conf_peer
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|cpeer
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_peer
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * unconfig - unconfigure some associations  */
end_comment

begin_function
specifier|static
name|void
name|unconfig
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
comment|/* 8 is the maximum number of peers which will fit in a packet */
name|struct
name|conf_unpeer
modifier|*
name|pl
decl_stmt|,
name|plist
index|[
name|min
argument_list|(
name|MAXARGS
argument_list|,
literal|8
argument_list|)
index|]
decl_stmt|;
name|int
name|qitemlim
decl_stmt|;
name|int
name|qitems
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|sendsize
decl_stmt|;
name|again
label|:
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|sendsize
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|conf_unpeer
argument_list|)
expr_stmt|;
else|else
name|sendsize
operator|=
name|v4sizeof
argument_list|(
expr|struct
name|conf_unpeer
argument_list|)
expr_stmt|;
name|qitemlim
operator|=
name|min
argument_list|(
name|pcmd
operator|->
name|nargs
argument_list|,
name|COUNTOF
argument_list|(
name|plist
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|qitems
operator|=
literal|0
operator|,
name|pl
operator|=
name|plist
init|;
name|qitems
operator|<
name|qitemlim
condition|;
name|qitems
operator|++
control|)
block|{
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|pl
operator|->
name|peeraddr
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|pl
operator|->
name|v6_flag
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|pl
operator|->
name|peeraddr6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|pl
operator|->
name|v6_flag
operator|=
literal|1
expr_stmt|;
block|}
name|pl
operator|=
operator|(
name|void
operator|*
operator|)
operator|(
operator|(
name|char
operator|*
operator|)
name|pl
operator|+
name|sendsize
operator|)
expr_stmt|;
block|}
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_UNCONFIG
argument_list|,
literal|1
argument_list|,
name|qitems
argument_list|,
name|sendsize
argument_list|,
operator|(
name|char
operator|*
operator|)
name|plist
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_unpeer
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * set - set some system flags  */
end_comment

begin_function
specifier|static
name|void
name|set
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|doset
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_SET_SYS_FLAG
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * clear - clear some system flags  */
end_comment

begin_function
specifier|static
name|void
name|sys_clear
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|doset
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_CLR_SYS_FLAG
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * doset - set/clear system flags  */
end_comment

begin_function
specifier|static
name|void
name|doset
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|req
parameter_list|)
block|{
name|struct
name|conf_sys_flags
name|sys
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|res
decl_stmt|;
name|sys
operator|.
name|flags
operator|=
literal|0
expr_stmt|;
name|res
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|items
operator|=
literal|0
init|;
name|items
operator|<
name|pcmd
operator|->
name|nargs
condition|;
name|items
operator|++
control|)
block|{
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"auth"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_AUTH
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"bclient"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_BCLIENT
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"calibrate"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_CAL
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"kernel"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_KERNEL
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"monitor"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_MONITOR
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"ntp"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_NTP
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"pps"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_PPS
expr_stmt|;
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|,
literal|"stats"
argument_list|)
condition|)
name|sys
operator|.
name|flags
operator||=
name|SYS_FLAG_FILEGEN
expr_stmt|;
else|else
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"Unknown flag %s\n"
argument_list|,
name|pcmd
operator|->
name|argval
index|[
name|items
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
name|res
operator|=
literal|1
expr_stmt|;
block|}
block|}
name|sys
operator|.
name|flags
operator|=
name|htonl
argument_list|(
name|sys
operator|.
name|flags
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|||
name|sys
operator|.
name|flags
operator|==
literal|0
condition|)
return|return;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|req
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_sys_flags
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|sys
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_sys_flags
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * data for printing/interrpreting the restrict flags  */
end_comment

begin_struct
struct|struct
name|resflags
block|{
specifier|const
name|char
modifier|*
name|str
decl_stmt|;
name|int
name|bit
decl_stmt|;
block|}
struct|;
end_struct

begin_comment
comment|/* XXX: HMS: we apparently don't report set bits we do not recognize. */
end_comment

begin_decl_stmt
specifier|static
name|struct
name|resflags
name|resflagsV2
index|[]
init|=
block|{
block|{
literal|"ignore"
block|,
literal|0x001
block|}
block|,
block|{
literal|"noserve"
block|,
literal|0x002
block|}
block|,
block|{
literal|"notrust"
block|,
literal|0x004
block|}
block|,
block|{
literal|"noquery"
block|,
literal|0x008
block|}
block|,
block|{
literal|"nomodify"
block|,
literal|0x010
block|}
block|,
block|{
literal|"nopeer"
block|,
literal|0x020
block|}
block|,
block|{
literal|"notrap"
block|,
literal|0x040
block|}
block|,
block|{
literal|"lptrap"
block|,
literal|0x080
block|}
block|,
block|{
literal|"limited"
block|,
literal|0x100
block|}
block|,
block|{
literal|""
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|resflags
name|resflagsV3
index|[]
init|=
block|{
block|{
literal|"ignore"
block|,
name|RES_IGNORE
block|}
block|,
block|{
literal|"noserve"
block|,
name|RES_DONTSERVE
block|}
block|,
block|{
literal|"notrust"
block|,
name|RES_DONTTRUST
block|}
block|,
block|{
literal|"noquery"
block|,
name|RES_NOQUERY
block|}
block|,
block|{
literal|"nomodify"
block|,
name|RES_NOMODIFY
block|}
block|,
block|{
literal|"nopeer"
block|,
name|RES_NOPEER
block|}
block|,
block|{
literal|"notrap"
block|,
name|RES_NOTRAP
block|}
block|,
block|{
literal|"lptrap"
block|,
name|RES_LPTRAP
block|}
block|,
block|{
literal|"limited"
block|,
name|RES_LIMITED
block|}
block|,
block|{
literal|"version"
block|,
name|RES_VERSION
block|}
block|,
block|{
literal|"kod"
block|,
name|RES_KOD
block|}
block|,
block|{
literal|"flake"
block|,
name|RES_FLAKE
block|}
block|,
block|{
literal|""
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|resflags
name|resmflags
index|[]
init|=
block|{
block|{
literal|"ntpport"
block|,
name|RESM_NTPONLY
block|}
block|,
block|{
literal|"interface"
block|,
name|RESM_INTERFACE
block|}
block|,
block|{
literal|"source"
block|,
name|RESM_SOURCE
block|}
block|,
block|{
literal|""
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * reslist - obtain and print the server's restrict list  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|reslist
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_restrict
modifier|*
name|rl
decl_stmt|;
name|sockaddr_u
name|resaddr
decl_stmt|;
name|sockaddr_u
name|maskaddr
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|skip
decl_stmt|;
specifier|const
name|char
modifier|*
name|addr
decl_stmt|;
specifier|const
name|char
modifier|*
name|mask
decl_stmt|;
name|struct
name|resflags
modifier|*
name|rf
decl_stmt|;
name|u_int32
name|count
decl_stmt|;
name|u_short
name|flags
decl_stmt|;
name|u_short
name|mflags
decl_stmt|;
name|char
name|flagstr
index|[
literal|300
index|]
decl_stmt|;
specifier|static
specifier|const
name|char
modifier|*
name|comma
init|=
literal|", "
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_GET_RESTRICT
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|rl
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_restrict
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_restrict
argument_list|)
argument_list|)
operator|&&
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
name|v4sizeof
argument_list|(
expr|struct
name|info_restrict
argument_list|)
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"   address          mask            count        flags\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"=====================================================================\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|items
operator|>
literal|0
condition|)
block|{
name|SET_ADDRS
argument_list|(
name|resaddr
argument_list|,
name|maskaddr
argument_list|,
name|rl
argument_list|,
name|addr
argument_list|,
name|mask
argument_list|)
expr_stmt|;
if|if
condition|(
name|rl
operator|->
name|v6_flag
operator|!=
literal|0
condition|)
block|{
name|addr
operator|=
name|nntohost
argument_list|(
operator|&
name|resaddr
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|rl
operator|->
name|mask
operator|==
operator|(
name|u_int32
operator|)
literal|0xffffffff
condition|)
name|addr
operator|=
name|nntohost
argument_list|(
operator|&
name|resaddr
argument_list|)
expr_stmt|;
else|else
name|addr
operator|=
name|stoa
argument_list|(
operator|&
name|resaddr
argument_list|)
expr_stmt|;
block|}
name|mask
operator|=
name|stoa
argument_list|(
operator|&
name|maskaddr
argument_list|)
expr_stmt|;
name|skip
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|(
name|pcmd
operator|->
name|nargs
operator|==
literal|0
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|6
operator|)
operator|&&
operator|(
name|rl
operator|->
name|v6_flag
operator|!=
literal|0
operator|)
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|4
operator|)
operator|&&
operator|(
name|rl
operator|->
name|v6_flag
operator|==
literal|0
operator|)
operator|)
condition|)
name|skip
operator|=
literal|0
expr_stmt|;
name|count
operator|=
name|ntohl
argument_list|(
name|rl
operator|->
name|count
argument_list|)
expr_stmt|;
name|flags
operator|=
name|ntohs
argument_list|(
name|rl
operator|->
name|flags
argument_list|)
expr_stmt|;
name|mflags
operator|=
name|ntohs
argument_list|(
name|rl
operator|->
name|mflags
argument_list|)
expr_stmt|;
name|flagstr
index|[
literal|0
index|]
operator|=
literal|'\0'
expr_stmt|;
name|res
operator|=
literal|1
expr_stmt|;
name|rf
operator|=
operator|&
name|resmflags
index|[
literal|0
index|]
expr_stmt|;
while|while
condition|(
name|rf
operator|->
name|bit
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|mflags
operator|&
name|rf
operator|->
name|bit
condition|)
block|{
if|if
condition|(
operator|!
name|res
condition|)
name|strlcat
argument_list|(
name|flagstr
argument_list|,
name|comma
argument_list|,
sizeof|sizeof
argument_list|(
name|flagstr
argument_list|)
argument_list|)
expr_stmt|;
name|res
operator|=
literal|0
expr_stmt|;
name|strlcat
argument_list|(
name|flagstr
argument_list|,
name|rf
operator|->
name|str
argument_list|,
sizeof|sizeof
argument_list|(
name|flagstr
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|rf
operator|++
expr_stmt|;
block|}
name|rf
operator|=
operator|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
operator|)
condition|?
operator|&
name|resflagsV2
index|[
literal|0
index|]
else|:
operator|&
name|resflagsV3
index|[
literal|0
index|]
expr_stmt|;
while|while
condition|(
name|rf
operator|->
name|bit
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|flags
operator|&
name|rf
operator|->
name|bit
condition|)
block|{
if|if
condition|(
operator|!
name|res
condition|)
name|strlcat
argument_list|(
name|flagstr
argument_list|,
name|comma
argument_list|,
sizeof|sizeof
argument_list|(
name|flagstr
argument_list|)
argument_list|)
expr_stmt|;
name|res
operator|=
literal|0
expr_stmt|;
name|strlcat
argument_list|(
name|flagstr
argument_list|,
name|rf
operator|->
name|str
argument_list|,
sizeof|sizeof
argument_list|(
name|flagstr
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|rf
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|flagstr
index|[
literal|0
index|]
operator|==
literal|'\0'
condition|)
name|strlcpy
argument_list|(
name|flagstr
argument_list|,
literal|"none"
argument_list|,
sizeof|sizeof
argument_list|(
name|flagstr
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|skip
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%-15.15s %-15.15s %9lu  %s\n"
argument_list|,
name|addr
argument_list|,
name|mask
argument_list|,
operator|(
name|u_long
operator|)
name|count
argument_list|,
name|flagstr
argument_list|)
expr_stmt|;
name|rl
operator|++
expr_stmt|;
name|items
operator|--
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * new_restrict - create/add a set of restrictions  */
end_comment

begin_function
specifier|static
name|void
name|new_restrict
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_restrict
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_RESADDFLAGS
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * unrestrict - remove restriction flags from existing entry  */
end_comment

begin_function
specifier|static
name|void
name|unrestrict
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_restrict
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_RESSUBFLAGS
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * delrestrict - delete an existing restriction  */
end_comment

begin_function
specifier|static
name|void
name|delrestrict
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_restrict
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_UNRESTRICT
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * do_restrict - decode commandline restrictions and make the request  */
end_comment

begin_function
specifier|static
name|void
name|do_restrict
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|req_code
parameter_list|)
block|{
name|struct
name|conf_restrict
name|cres
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|u_int32
name|num
decl_stmt|;
name|u_long
name|bit
decl_stmt|;
name|int
name|i
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|err
decl_stmt|;
name|int
name|sendsize
decl_stmt|;
comment|/* Initialize cres */
name|cres
operator|.
name|addr
operator|=
literal|0
expr_stmt|;
name|cres
operator|.
name|mask
operator|=
literal|0
expr_stmt|;
name|cres
operator|.
name|flags
operator|=
literal|0
expr_stmt|;
name|cres
operator|.
name|mflags
operator|=
literal|0
expr_stmt|;
name|cres
operator|.
name|v6_flag
operator|=
literal|0
expr_stmt|;
name|again
label|:
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|sendsize
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|conf_restrict
argument_list|)
expr_stmt|;
else|else
name|sendsize
operator|=
name|v4sizeof
argument_list|(
expr|struct
name|conf_restrict
argument_list|)
expr_stmt|;
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|cres
operator|.
name|addr
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|cres
operator|.
name|mask
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|cres
operator|.
name|v6_flag
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|cres
operator|.
name|addr6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|cres
operator|.
name|v6_flag
operator|=
literal|1
expr_stmt|;
block|}
name|cres
operator|.
name|flags
operator|=
literal|0
expr_stmt|;
name|cres
operator|.
name|mflags
operator|=
literal|0
expr_stmt|;
name|err
operator|=
name|FALSE
expr_stmt|;
for|for
control|(
name|res
operator|=
literal|2
init|;
name|res
operator|<
name|pcmd
operator|->
name|nargs
condition|;
name|res
operator|++
control|)
block|{
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|res
index|]
operator|.
name|string
argument_list|,
literal|"ntpport"
argument_list|)
condition|)
block|{
name|cres
operator|.
name|mflags
operator||=
name|RESM_NTPONLY
expr_stmt|;
block|}
else|else
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|resflagsV3
index|[
name|i
index|]
operator|.
name|bit
operator|!=
literal|0
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|res
index|]
operator|.
name|string
argument_list|,
name|resflagsV3
index|[
name|i
index|]
operator|.
name|str
argument_list|)
condition|)
break|break;
block|}
if|if
condition|(
name|resflagsV3
index|[
name|i
index|]
operator|.
name|bit
operator|!=
literal|0
condition|)
block|{
name|cres
operator|.
name|flags
operator||=
name|resflagsV3
index|[
name|i
index|]
operator|.
name|bit
expr_stmt|;
if|if
condition|(
name|req_code
operator|==
name|REQ_UNRESTRICT
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"Flag %s inappropriate\n"
argument_list|,
name|resflagsV3
index|[
name|i
index|]
operator|.
name|str
argument_list|)
expr_stmt|;
name|err
operator|=
name|TRUE
expr_stmt|;
block|}
block|}
else|else
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"Unknown flag %s\n"
argument_list|,
name|pcmd
operator|->
name|argval
index|[
name|res
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
name|err
operator|=
name|TRUE
expr_stmt|;
block|}
block|}
block|}
name|cres
operator|.
name|flags
operator|=
name|htons
argument_list|(
name|cres
operator|.
name|flags
argument_list|)
expr_stmt|;
name|cres
operator|.
name|mflags
operator|=
name|htons
argument_list|(
name|cres
operator|.
name|mflags
argument_list|)
expr_stmt|;
comment|/* 	 * Make sure mask for default address is zero.  Otherwise, 	 * make sure mask bits are contiguous. 	 */
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
if|if
condition|(
name|cres
operator|.
name|addr
operator|==
literal|0
condition|)
block|{
name|cres
operator|.
name|mask
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|num
operator|=
name|ntohl
argument_list|(
name|cres
operator|.
name|mask
argument_list|)
expr_stmt|;
for|for
control|(
name|bit
operator|=
literal|0x80000000
init|;
name|bit
operator|!=
literal|0
condition|;
name|bit
operator|>>=
literal|1
control|)
if|if
condition|(
operator|(
name|num
operator|&
name|bit
operator|)
operator|==
literal|0
condition|)
break|break;
for|for
control|(
init|;
name|bit
operator|!=
literal|0
condition|;
name|bit
operator|>>=
literal|1
control|)
if|if
condition|(
operator|(
name|num
operator|&
name|bit
operator|)
operator|!=
literal|0
condition|)
break|break;
if|if
condition|(
name|bit
operator|!=
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"Invalid mask %s\n"
argument_list|,
name|numtoa
argument_list|(
name|cres
operator|.
name|mask
argument_list|)
argument_list|)
expr_stmt|;
name|err
operator|=
name|TRUE
expr_stmt|;
block|}
block|}
block|}
else|else
block|{
comment|/* XXX IPv6 sanity checking stuff */
block|}
if|if
condition|(
name|err
condition|)
return|return;
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|req_code
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
name|sendsize
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|cres
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|cres
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * monlist - obtain and print the server's monitor data  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|monlist
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|char
modifier|*
name|struct_star
decl_stmt|;
name|struct
name|info_monitor
modifier|*
name|ml
decl_stmt|;
name|struct
name|info_monitor_1
modifier|*
name|m1
decl_stmt|;
name|struct
name|old_info_monitor
modifier|*
name|oml
decl_stmt|;
name|sockaddr_u
name|addr
decl_stmt|;
name|sockaddr_u
name|dstadr
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|version
init|=
operator|-
literal|1
decl_stmt|;
if|if
condition|(
name|pcmd
operator|->
name|nargs
operator|>
literal|0
condition|)
name|version
operator|=
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|ival
expr_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
operator|(
name|version
operator|==
literal|1
operator|||
name|version
operator|==
operator|-
literal|1
operator|)
condition|?
name|REQ_MON_GETLIST_1
else|:
name|REQ_MON_GETLIST
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|struct_star
argument_list|,
operator|(
name|version
operator|<
literal|0
operator|)
condition|?
operator|(
literal|1
operator|<<
name|INFO_ERR_REQ
operator|)
else|:
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_monitor_1
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
name|INFO_ERR_REQ
operator|&&
name|version
operator|<
literal|0
condition|)
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_MON_GETLIST
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|struct_star
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_monitor
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
name|itemsize
operator|==
sizeof|sizeof
argument_list|(
expr|struct
name|info_monitor_1
argument_list|)
operator|||
name|itemsize
operator|==
name|v4sizeof
argument_list|(
expr|struct
name|info_monitor_1
argument_list|)
condition|)
block|{
name|m1
operator|=
operator|(
name|void
operator|*
operator|)
name|struct_star
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"remote address          port local address      count m ver rstr avgint  lstint\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"===============================================================================\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|items
operator|>
literal|0
condition|)
block|{
name|SET_ADDRS
argument_list|(
name|dstadr
argument_list|,
name|addr
argument_list|,
name|m1
argument_list|,
name|daddr
argument_list|,
name|addr
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|pcmd
operator|->
name|nargs
operator|==
literal|0
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|6
operator|)
operator|&&
operator|(
name|m1
operator|->
name|v6_flag
operator|!=
literal|0
operator|)
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|4
operator|)
operator|&&
operator|(
name|m1
operator|->
name|v6_flag
operator|==
literal|0
operator|)
operator|)
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%-22.22s %5d %-15s %8lu %1u %1u %6lx %6lu %7lu\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
name|addr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|m1
operator|->
name|port
argument_list|)
argument_list|,
name|stoa
argument_list|(
operator|&
name|dstadr
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|m1
operator|->
name|count
argument_list|)
argument_list|,
name|m1
operator|->
name|mode
argument_list|,
name|m1
operator|->
name|version
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|m1
operator|->
name|restr
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|m1
operator|->
name|avg_int
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|m1
operator|->
name|last_int
argument_list|)
argument_list|)
expr_stmt|;
name|m1
operator|++
expr_stmt|;
name|items
operator|--
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|itemsize
operator|==
sizeof|sizeof
argument_list|(
expr|struct
name|info_monitor
argument_list|)
operator|||
name|itemsize
operator|==
name|v4sizeof
argument_list|(
expr|struct
name|info_monitor
argument_list|)
condition|)
block|{
name|ml
operator|=
operator|(
name|void
operator|*
operator|)
name|struct_star
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"     address               port     count mode ver rstr avgint  lstint\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"===============================================================================\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|items
operator|>
literal|0
condition|)
block|{
name|SET_ADDR
argument_list|(
name|dstadr
argument_list|,
name|ml
operator|->
name|v6_flag
argument_list|,
name|ml
operator|->
name|addr
argument_list|,
name|ml
operator|->
name|addr6
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|pcmd
operator|->
name|nargs
operator|==
literal|0
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|6
operator|)
operator|&&
operator|(
name|ml
operator|->
name|v6_flag
operator|!=
literal|0
operator|)
operator|)
operator|||
operator|(
operator|(
name|pcmd
operator|->
name|argval
operator|->
name|ival
operator|==
literal|4
operator|)
operator|&&
operator|(
name|ml
operator|->
name|v6_flag
operator|==
literal|0
operator|)
operator|)
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%-25.25s %5u %9lu %4u %2u %9lx %9lu %9lu\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
name|dstadr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|ml
operator|->
name|port
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ml
operator|->
name|count
argument_list|)
argument_list|,
name|ml
operator|->
name|mode
argument_list|,
name|ml
operator|->
name|version
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ml
operator|->
name|restr
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ml
operator|->
name|avg_int
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ml
operator|->
name|last_int
argument_list|)
argument_list|)
expr_stmt|;
name|ml
operator|++
expr_stmt|;
name|items
operator|--
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|itemsize
operator|==
sizeof|sizeof
argument_list|(
expr|struct
name|old_info_monitor
argument_list|)
condition|)
block|{
name|oml
operator|=
operator|(
name|void
operator|*
operator|)
name|struct_star
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"     address          port     count  mode version  lasttime firsttime\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"======================================================================\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|items
operator|>
literal|0
condition|)
block|{
name|SET_ADDR
argument_list|(
name|dstadr
argument_list|,
name|oml
operator|->
name|v6_flag
argument_list|,
name|oml
operator|->
name|addr
argument_list|,
name|oml
operator|->
name|addr6
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%-20.20s %5u %9lu %4u   %3u %9lu %9lu\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
name|dstadr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|oml
operator|->
name|port
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|oml
operator|->
name|count
argument_list|)
argument_list|,
name|oml
operator|->
name|mode
argument_list|,
name|oml
operator|->
name|version
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|oml
operator|->
name|lasttime
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|oml
operator|->
name|firsttime
argument_list|)
argument_list|)
expr_stmt|;
name|oml
operator|++
expr_stmt|;
name|items
operator|--
expr_stmt|;
block|}
block|}
else|else
block|{
comment|/* issue warning according to new info_monitor size */
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_monitor
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * Mapping between command line strings and stat reset flags  */
end_comment

begin_struct
struct|struct
name|statreset
block|{
specifier|const
name|char
modifier|*
specifier|const
name|str
decl_stmt|;
specifier|const
name|int
name|flag
decl_stmt|;
block|}
name|sreset
index|[]
init|=
block|{
block|{
literal|"allpeers"
block|,
name|RESET_FLAG_ALLPEERS
block|}
block|,
block|{
literal|"io"
block|,
name|RESET_FLAG_IO
block|}
block|,
block|{
literal|"sys"
block|,
name|RESET_FLAG_SYS
block|}
block|,
block|{
literal|"mem"
block|,
name|RESET_FLAG_MEM
block|}
block|,
block|{
literal|"timer"
block|,
name|RESET_FLAG_TIMER
block|}
block|,
block|{
literal|"auth"
block|,
name|RESET_FLAG_AUTH
block|}
block|,
block|{
literal|"ctl"
block|,
name|RESET_FLAG_CTL
block|}
block|,
block|{
literal|""
block|,
literal|0
block|}
block|}
struct|;
end_struct

begin_comment
comment|/*  * reset - reset statistic counters  */
end_comment

begin_function
specifier|static
name|void
name|reset
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|reset_flags
name|rflags
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|i
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|err
decl_stmt|;
name|err
operator|=
literal|0
expr_stmt|;
name|rflags
operator|.
name|flags
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|res
operator|=
literal|0
init|;
name|res
operator|<
name|pcmd
operator|->
name|nargs
condition|;
name|res
operator|++
control|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|sreset
index|[
name|i
index|]
operator|.
name|flag
operator|!=
literal|0
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
name|res
index|]
operator|.
name|string
argument_list|,
name|sreset
index|[
name|i
index|]
operator|.
name|str
argument_list|)
condition|)
break|break;
block|}
if|if
condition|(
name|sreset
index|[
name|i
index|]
operator|.
name|flag
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"Flag %s unknown\n"
argument_list|,
name|pcmd
operator|->
name|argval
index|[
name|res
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
name|err
operator|++
expr_stmt|;
block|}
else|else
block|{
name|rflags
operator|.
name|flags
operator||=
name|sreset
index|[
name|i
index|]
operator|.
name|flag
expr_stmt|;
block|}
block|}
name|rflags
operator|.
name|flags
operator|=
name|htonl
argument_list|(
name|rflags
operator|.
name|flags
argument_list|)
expr_stmt|;
if|if
condition|(
name|err
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"Not done due to errors\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_RESET_STATS
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|reset_flags
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|rflags
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|reset_flags
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * preset - reset stat counters for particular peers  */
end_comment

begin_function
specifier|static
name|void
name|preset
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
comment|/* 8 is the maximum number of peers which will fit in a packet */
name|struct
name|conf_unpeer
modifier|*
name|pl
decl_stmt|,
name|plist
index|[
name|min
argument_list|(
name|MAXARGS
argument_list|,
literal|8
argument_list|)
index|]
decl_stmt|;
name|int
name|qitemlim
decl_stmt|;
name|int
name|qitems
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|sendsize
decl_stmt|;
name|again
label|:
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|sendsize
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|conf_unpeer
argument_list|)
expr_stmt|;
else|else
name|sendsize
operator|=
name|v4sizeof
argument_list|(
expr|struct
name|conf_unpeer
argument_list|)
expr_stmt|;
name|qitemlim
operator|=
name|min
argument_list|(
name|pcmd
operator|->
name|nargs
argument_list|,
name|COUNTOF
argument_list|(
name|plist
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|qitems
operator|=
literal|0
operator|,
name|pl
operator|=
name|plist
init|;
name|qitems
operator|<
name|qitemlim
condition|;
name|qitems
operator|++
control|)
block|{
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|pl
operator|->
name|peeraddr
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|pl
operator|->
name|v6_flag
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|pl
operator|->
name|peeraddr6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|pl
operator|->
name|v6_flag
operator|=
literal|1
expr_stmt|;
block|}
name|pl
operator|=
operator|(
name|void
operator|*
operator|)
operator|(
operator|(
name|char
operator|*
operator|)
name|pl
operator|+
name|sendsize
operator|)
expr_stmt|;
block|}
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_RESET_PEER
argument_list|,
literal|1
argument_list|,
name|qitems
argument_list|,
name|sendsize
argument_list|,
operator|(
name|char
operator|*
operator|)
name|plist
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_unpeer
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * readkeys - request the server to reread the keys file  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|readkeys
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_REREAD_KEYS
argument_list|,
literal|1
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|dummy
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * trustkey - add some keys to the trusted key list  */
end_comment

begin_function
specifier|static
name|void
name|trustkey
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_trustkey
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_TRUSTKEY
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * untrustkey - remove some keys from the trusted key list  */
end_comment

begin_function
specifier|static
name|void
name|untrustkey
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_trustkey
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_UNTRUSTKEY
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * do_trustkey - do grunge work of adding/deleting keys  */
end_comment

begin_function
specifier|static
name|void
name|do_trustkey
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|req
parameter_list|)
block|{
name|u_long
name|keyids
index|[
name|MAXARGS
index|]
decl_stmt|;
name|int
name|i
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|ritems
decl_stmt|;
name|int
name|res
decl_stmt|;
name|ritems
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|pcmd
operator|->
name|nargs
condition|;
name|i
operator|++
control|)
block|{
name|keyids
index|[
name|ritems
operator|++
index|]
operator|=
name|pcmd
operator|->
name|argval
index|[
name|i
index|]
operator|.
name|uval
expr_stmt|;
block|}
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|req
argument_list|,
literal|1
argument_list|,
name|ritems
argument_list|,
sizeof|sizeof
argument_list|(
name|u_long
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
name|keyids
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|dummy
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * authinfo - obtain and print info about authentication  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|authinfo
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_auth
modifier|*
name|ia
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_AUTHINFO
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|ia
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ia
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ia
argument_list|)
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time since reset:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|timereset
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"stored keys:          %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|numkeys
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"free keys:            %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|numfreekeys
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"key lookups:          %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|keylookups
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"keys not found:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|keynotfound
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"uncached keys:        %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|keyuncached
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"encryptions:          %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|encryptions
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"decryptions:          %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|decryptions
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"expired keys:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ia
operator|->
name|expired
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * traps - obtain and print a list of traps  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|traps
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
name|struct
name|info_trap
modifier|*
name|it
decl_stmt|;
name|sockaddr_u
name|trap_addr
decl_stmt|,
name|local_addr
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_TRAPS
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|it
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|it
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_trap
argument_list|)
argument_list|)
operator|&&
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
name|v4sizeof
argument_list|(
expr|struct
name|info_trap
argument_list|)
argument_list|)
condition|)
return|return;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|items
condition|;
name|i
operator|++
control|)
block|{
name|SET_ADDRS
argument_list|(
name|trap_addr
argument_list|,
name|local_addr
argument_list|,
name|it
argument_list|,
name|trap_address
argument_list|,
name|local_address
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%saddress %s, port %d\n"
argument_list|,
operator|(
literal|0
operator|==
name|i
operator|)
condition|?
literal|""
else|:
literal|"\n"
argument_list|,
name|stoa
argument_list|(
operator|&
name|trap_addr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|it
operator|->
name|trap_port
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"interface: %s, "
argument_list|,
operator|(
literal|0
operator|==
name|it
operator|->
name|local_address
operator|)
condition|?
literal|"wildcard"
else|:
name|stoa
argument_list|(
operator|&
name|local_addr
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ntohl
argument_list|(
name|it
operator|->
name|flags
argument_list|)
operator|&
name|TRAP_CONFIGURED
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"configured\n"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|ntohl
argument_list|(
name|it
operator|->
name|flags
argument_list|)
operator|&
name|TRAP_NONPRIO
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"low priority\n"
argument_list|)
expr_stmt|;
else|else
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"normal priority\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"set for %ld secs, last set %ld secs ago\n"
argument_list|,
operator|(
name|long
operator|)
name|ntohl
argument_list|(
name|it
operator|->
name|origtime
argument_list|)
argument_list|,
operator|(
name|long
operator|)
name|ntohl
argument_list|(
name|it
operator|->
name|settime
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"sequence %d, number of resets %ld\n"
argument_list|,
name|ntohs
argument_list|(
name|it
operator|->
name|sequence
argument_list|)
argument_list|,
operator|(
name|long
operator|)
name|ntohl
argument_list|(
name|it
operator|->
name|resets
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * addtrap - configure a trap  */
end_comment

begin_function
specifier|static
name|void
name|addtrap
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_addclr_trap
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_ADD_TRAP
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * clrtrap - clear a trap from the server  */
end_comment

begin_function
specifier|static
name|void
name|clrtrap
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_addclr_trap
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_CLR_TRAP
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * do_addclr_trap - do grunge work of adding/deleting traps  */
end_comment

begin_function
specifier|static
name|void
name|do_addclr_trap
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|req
parameter_list|)
block|{
name|struct
name|conf_trap
name|ctrap
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|sendsize
decl_stmt|;
name|again
label|:
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|sendsize
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|conf_trap
argument_list|)
expr_stmt|;
else|else
name|sendsize
operator|=
name|v4sizeof
argument_list|(
expr|struct
name|conf_trap
argument_list|)
expr_stmt|;
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|ctrap
operator|.
name|trap_address
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
name|ctrap
operator|.
name|v6_flag
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|impl_ver
operator|==
name|IMPL_XNTPD_OLD
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|ctrap
operator|.
name|trap_address6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|ctrap
operator|.
name|v6_flag
operator|=
literal|1
expr_stmt|;
block|}
name|ctrap
operator|.
name|local_address
operator|=
literal|0
expr_stmt|;
name|ctrap
operator|.
name|trap_port
operator|=
name|htons
argument_list|(
name|TRAPPORT
argument_list|)
expr_stmt|;
name|ctrap
operator|.
name|unused
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|pcmd
operator|->
name|nargs
operator|>
literal|1
condition|)
block|{
name|ctrap
operator|.
name|trap_port
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|uval
argument_list|)
expr_stmt|;
if|if
condition|(
name|pcmd
operator|->
name|nargs
operator|>
literal|2
condition|)
block|{
if|if
condition|(
name|AF
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|netnum
argument_list|)
operator|!=
name|AF
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"***Cannot mix IPv4 and IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|IS_IPV4
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|netnum
argument_list|)
condition|)
name|ctrap
operator|.
name|local_address
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
else|else
name|ctrap
operator|.
name|local_address6
operator|=
name|SOCK_ADDR6
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
block|}
block|}
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|req
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
name|sendsize
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|ctrap
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_trap
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * requestkey - change the server's request key (a dangerous request)  */
end_comment

begin_function
specifier|static
name|void
name|requestkey
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_changekey
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_REQUEST_KEY
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * controlkey - change the server's control key  */
end_comment

begin_function
specifier|static
name|void
name|controlkey
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|do_changekey
argument_list|(
name|pcmd
argument_list|,
name|fp
argument_list|,
name|REQ_CONTROL_KEY
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * do_changekey - do grunge work of changing keys  */
end_comment

begin_function
specifier|static
name|void
name|do_changekey
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
name|req
parameter_list|)
block|{
name|u_long
name|key
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|int
name|res
decl_stmt|;
name|key
operator|=
name|htonl
argument_list|(
operator|(
name|u_int32
operator|)
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|uval
argument_list|)
expr_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|req
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
sizeof|sizeof
argument_list|(
name|u_int32
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|key
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|dummy
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * ctlstats - obtain and print info about authentication  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|ctlstats
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_control
modifier|*
name|ic
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_GET_CTLSTATS
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|ic
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ic
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ic
argument_list|)
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"time since reset:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|ctltimereset
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"requests received:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlreq
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"responses sent:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlresponses
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"fragments sent:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlfrags
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"async messages sent:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numasyncmsgs
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"error msgs sent:        %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlerrors
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"total bad pkts:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlbadpkts
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"packet too short:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctltooshort
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"response on input:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlinputresp
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"fragment on input:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlinputfrag
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"error set on input:     %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlinputerr
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad offset on input:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlbadoffset
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad version packets:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlbadversion
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"data in pkt too short:  %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctldatatooshort
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"unknown op codes:       %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ic
operator|->
name|numctlbadop
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * clockstat - get and print clock status information  */
end_comment

begin_function
specifier|static
name|void
name|clockstat
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_clock
modifier|*
name|cl
decl_stmt|;
comment|/* 8 is the maximum number of clocks which will fit in a packet */
name|u_long
name|clist
index|[
name|min
argument_list|(
name|MAXARGS
argument_list|,
literal|8
argument_list|)
index|]
decl_stmt|;
name|int
name|qitemlim
decl_stmt|;
name|int
name|qitems
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|l_fp
name|ts
decl_stmt|;
name|struct
name|clktype
modifier|*
name|clk
decl_stmt|;
name|qitemlim
operator|=
name|min
argument_list|(
name|pcmd
operator|->
name|nargs
argument_list|,
name|COUNTOF
argument_list|(
name|clist
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|qitems
operator|=
literal|0
init|;
name|qitems
operator|<
name|qitemlim
condition|;
name|qitems
operator|++
control|)
name|clist
index|[
name|qitems
index|]
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_GET_CLOCKINFO
argument_list|,
literal|0
argument_list|,
name|qitems
argument_list|,
sizeof|sizeof
argument_list|(
name|u_int32
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
name|clist
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|cl
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_clock
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_clock
argument_list|)
argument_list|)
condition|)
return|return;
while|while
condition|(
name|items
operator|--
operator|>
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"clock address:        %s\n"
argument_list|,
name|numtoa
argument_list|(
name|cl
operator|->
name|clockadr
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|clk
operator|=
name|clktypes
init|;
name|clk
operator|->
name|code
operator|>=
literal|0
condition|;
name|clk
operator|++
control|)
if|if
condition|(
name|clk
operator|->
name|code
operator|==
name|cl
operator|->
name|type
condition|)
break|break;
if|if
condition|(
name|clk
operator|->
name|code
operator|>=
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"clock type:           %s\n"
argument_list|,
name|clk
operator|->
name|clocktype
argument_list|)
expr_stmt|;
else|else
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"clock type:           unknown type (%d)\n"
argument_list|,
name|cl
operator|->
name|type
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"last event:           %d\n"
argument_list|,
name|cl
operator|->
name|lastevent
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"current status:       %d\n"
argument_list|,
name|cl
operator|->
name|currentstatus
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"number of polls:      %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|cl
operator|->
name|polls
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"no response to poll:  %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|cl
operator|->
name|noresponse
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad format responses: %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|cl
operator|->
name|badformat
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"bad data responses:   %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|cl
operator|->
name|baddata
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"running time:         %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|cl
operator|->
name|timestarted
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|cl
operator|->
name|fudgetime1
argument_list|,
operator|&
name|ts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"fudge time 1:         %s\n"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|ts
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
name|NTOHL_FP
argument_list|(
operator|&
name|cl
operator|->
name|fudgetime2
argument_list|,
operator|&
name|ts
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"fudge time 2:         %s\n"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|ts
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"stratum:              %ld\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|cl
operator|->
name|fudgeval1
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"reference ID:         %s\n"
argument_list|,
name|refid_string
argument_list|(
name|ntohl
argument_list|(
name|cl
operator|->
name|fudgeval2
argument_list|)
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"fudge flags:          0x%x\n"
argument_list|,
name|cl
operator|->
name|flags
argument_list|)
expr_stmt|;
if|if
condition|(
name|items
operator|>
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
name|cl
operator|++
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * fudge - set clock fudge factors  */
end_comment

begin_function
specifier|static
name|void
name|fudge
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|conf_fudge
name|fudgedata
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|char
modifier|*
name|dummy
decl_stmt|;
name|l_fp
name|ts
decl_stmt|;
name|int
name|res
decl_stmt|;
name|long
name|val
decl_stmt|;
name|u_long
name|u_val
decl_stmt|;
name|int
name|err
decl_stmt|;
name|err
operator|=
literal|0
expr_stmt|;
name|ZERO
argument_list|(
name|fudgedata
argument_list|)
expr_stmt|;
name|fudgedata
operator|.
name|clockadr
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
literal|0
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|string
argument_list|,
literal|"time1"
argument_list|)
condition|)
block|{
name|fudgedata
operator|.
name|which
operator|=
name|htonl
argument_list|(
name|FUDGE_TIME1
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|atolfp
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|string
argument_list|,
operator|&
name|ts
argument_list|)
condition|)
name|err
operator|=
literal|1
expr_stmt|;
else|else
name|NTOHL_FP
argument_list|(
operator|&
name|ts
argument_list|,
operator|&
name|fudgedata
operator|.
name|fudgetime
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|string
argument_list|,
literal|"time2"
argument_list|)
condition|)
block|{
name|fudgedata
operator|.
name|which
operator|=
name|htonl
argument_list|(
name|FUDGE_TIME2
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|atolfp
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|string
argument_list|,
operator|&
name|ts
argument_list|)
condition|)
name|err
operator|=
literal|1
expr_stmt|;
else|else
name|NTOHL_FP
argument_list|(
operator|&
name|ts
argument_list|,
operator|&
name|fudgedata
operator|.
name|fudgetime
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|string
argument_list|,
literal|"val1"
argument_list|)
condition|)
block|{
name|fudgedata
operator|.
name|which
operator|=
name|htonl
argument_list|(
name|FUDGE_VAL1
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|atoint
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|string
argument_list|,
operator|&
name|val
argument_list|)
condition|)
name|err
operator|=
literal|1
expr_stmt|;
else|else
name|fudgedata
operator|.
name|fudgeval_flags
operator|=
name|htonl
argument_list|(
name|val
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|string
argument_list|,
literal|"val2"
argument_list|)
condition|)
block|{
name|fudgedata
operator|.
name|which
operator|=
name|htonl
argument_list|(
name|FUDGE_VAL2
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|atoint
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|string
argument_list|,
operator|&
name|val
argument_list|)
condition|)
name|err
operator|=
literal|1
expr_stmt|;
else|else
name|fudgedata
operator|.
name|fudgeval_flags
operator|=
name|htonl
argument_list|(
operator|(
name|u_int32
operator|)
name|val
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|STREQ
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|string
argument_list|,
literal|"flags"
argument_list|)
condition|)
block|{
name|fudgedata
operator|.
name|which
operator|=
name|htonl
argument_list|(
name|FUDGE_FLAGS
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|hextoint
argument_list|(
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|string
argument_list|,
operator|&
name|u_val
argument_list|)
condition|)
name|err
operator|=
literal|1
expr_stmt|;
else|else
name|fudgedata
operator|.
name|fudgeval_flags
operator|=
name|htonl
argument_list|(
call|(
name|u_int32
call|)
argument_list|(
name|u_val
operator|&
literal|0xf
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"What fudge is %s?\n"
argument_list|,
name|pcmd
operator|->
name|argval
index|[
literal|1
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|err
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Unknown fudge parameter %s\n"
argument_list|,
name|pcmd
operator|->
name|argval
index|[
literal|2
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
return|return;
block|}
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_SET_CLKFUDGE
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|conf_fudge
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|fudgedata
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|&
name|dummy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|dummy
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"done!\n"
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_comment
comment|/*  * clkbug - get and print clock debugging information  */
end_comment

begin_function
specifier|static
name|void
name|clkbug
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
specifier|register
name|int
name|i
decl_stmt|;
specifier|register
name|int
name|n
decl_stmt|;
specifier|register
name|u_int32
name|s
decl_stmt|;
name|struct
name|info_clkbug
modifier|*
name|cl
decl_stmt|;
comment|/* 8 is the maximum number of clocks which will fit in a packet */
name|u_long
name|clist
index|[
name|min
argument_list|(
name|MAXARGS
argument_list|,
literal|8
argument_list|)
index|]
decl_stmt|;
name|u_int32
name|ltemp
decl_stmt|;
name|int
name|qitemlim
decl_stmt|;
name|int
name|qitems
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|int
name|needsp
decl_stmt|;
name|l_fp
name|ts
decl_stmt|;
name|qitemlim
operator|=
name|min
argument_list|(
name|pcmd
operator|->
name|nargs
argument_list|,
name|COUNTOF
argument_list|(
name|clist
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|qitems
operator|=
literal|0
init|;
name|qitems
operator|<
name|qitemlim
condition|;
name|qitems
operator|++
control|)
name|clist
index|[
name|qitems
index|]
operator|=
name|NSRCADR
argument_list|(
operator|&
name|pcmd
operator|->
name|argval
index|[
name|qitems
index|]
operator|.
name|netnum
argument_list|)
expr_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_GET_CLKBUGINFO
argument_list|,
literal|0
argument_list|,
name|qitems
argument_list|,
sizeof|sizeof
argument_list|(
name|u_int32
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
name|clist
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|cl
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_clkbug
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_clkbug
argument_list|)
argument_list|)
condition|)
return|return;
while|while
condition|(
name|items
operator|--
operator|>
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"clock address:        %s\n"
argument_list|,
name|numtoa
argument_list|(
name|cl
operator|->
name|clockadr
argument_list|)
argument_list|)
expr_stmt|;
name|n
operator|=
operator|(
name|int
operator|)
name|cl
operator|->
name|nvalues
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"values: %d"
argument_list|,
name|n
argument_list|)
expr_stmt|;
name|s
operator|=
name|ntohs
argument_list|(
name|cl
operator|->
name|svalues
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|>
name|NUMCBUGVALUES
condition|)
name|n
operator|=
name|NUMCBUGVALUES
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
block|{
name|ltemp
operator|=
name|ntohl
argument_list|(
name|cl
operator|->
name|values
index|[
name|i
index|]
argument_list|)
expr_stmt|;
name|ltemp
operator|&=
literal|0xffffffff
expr_stmt|;
comment|/* HMS: This does nothing now */
if|if
condition|(
operator|(
name|i
operator|&
literal|0x3
operator|)
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|&
operator|(
literal|1
operator|<<
name|i
operator|)
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%12ld"
argument_list|,
operator|(
name|u_long
operator|)
name|ltemp
argument_list|)
expr_stmt|;
else|else
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%12lu"
argument_list|,
operator|(
name|u_long
operator|)
name|ltemp
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
name|n
operator|=
operator|(
name|int
operator|)
name|cl
operator|->
name|ntimes
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"times: %d"
argument_list|,
name|n
argument_list|)
expr_stmt|;
name|s
operator|=
name|ntohl
argument_list|(
name|cl
operator|->
name|stimes
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|>
name|NUMCBUGTIMES
condition|)
name|n
operator|=
name|NUMCBUGTIMES
expr_stmt|;
name|needsp
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|(
name|i
operator|&
literal|0x1
operator|)
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
for|for
control|(
init|;
name|needsp
operator|>
literal|0
condition|;
name|needsp
operator|--
control|)
name|putc
argument_list|(
literal|' '
argument_list|,
name|fp
argument_list|)
expr_stmt|;
block|}
name|NTOHL_FP
argument_list|(
operator|&
name|cl
operator|->
name|times
index|[
name|i
index|]
argument_list|,
operator|&
name|ts
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|&
operator|(
literal|1
operator|<<
name|i
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%17s"
argument_list|,
name|lfptoa
argument_list|(
operator|&
name|ts
argument_list|,
literal|6
argument_list|)
argument_list|)
expr_stmt|;
name|needsp
operator|=
literal|22
expr_stmt|;
block|}
else|else
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%37s"
argument_list|,
name|uglydate
argument_list|(
operator|&
name|ts
argument_list|)
argument_list|)
expr_stmt|;
name|needsp
operator|=
literal|2
expr_stmt|;
block|}
block|}
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|items
operator|>
literal|0
condition|)
block|{
name|cl
operator|++
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_function

begin_comment
comment|/*  * kerninfo - display the kernel pll/pps variables  */
end_comment

begin_function
specifier|static
name|void
name|kerninfo
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_kernel
modifier|*
name|ik
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|unsigned
name|status
decl_stmt|;
name|double
name|tscale
init|=
literal|1e-6
decl_stmt|;
name|again
label|:
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_GET_KERNEL
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|ik
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_kernel
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|==
name|INFO_ERR_IMPL
operator|&&
name|impl_ver
operator|==
name|IMPL_XNTPD
condition|)
block|{
name|impl_ver
operator|=
name|IMPL_XNTPD_OLD
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|check1item
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_kernel
argument_list|)
argument_list|)
condition|)
return|return;
name|status
operator|=
name|ntohs
argument_list|(
name|ik
operator|->
name|status
argument_list|)
operator|&
literal|0xffff
expr_stmt|;
comment|/* 	 * pll variables. We know more than we should about the NANO bit. 	 */
ifdef|#
directive|ifdef
name|STA_NANO
if|if
condition|(
name|status
operator|&
name|STA_NANO
condition|)
name|tscale
operator|=
literal|1e-9
expr_stmt|;
endif|#
directive|endif
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"pll offset:           %g s\n"
argument_list|,
operator|(
name|int32
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|offset
argument_list|)
operator|*
name|tscale
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"pll frequency:        %s ppm\n"
argument_list|,
name|fptoa
argument_list|(
operator|(
name|s_fp
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|freq
argument_list|)
argument_list|,
literal|3
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"maximum error:        %g s\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|maxerror
argument_list|)
operator|*
name|tscale
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"estimated error:      %g s\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|esterror
argument_list|)
operator|*
name|tscale
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"status:               %04x "
argument_list|,
name|status
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|STA_PLL
if|if
condition|(
name|status
operator|&
name|STA_PLL
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" pll"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_PPSFREQ
if|if
condition|(
name|status
operator|&
name|STA_PPSFREQ
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" ppsfreq"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_PPSTIME
if|if
condition|(
name|status
operator|&
name|STA_PPSTIME
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" ppstime"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_FLL
if|if
condition|(
name|status
operator|&
name|STA_FLL
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" fll"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_INS
if|if
condition|(
name|status
operator|&
name|STA_INS
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" ins"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_DEL
if|if
condition|(
name|status
operator|&
name|STA_DEL
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" del"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_UNSYNC
if|if
condition|(
name|status
operator|&
name|STA_UNSYNC
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" unsync"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_FREQHOLD
if|if
condition|(
name|status
operator|&
name|STA_FREQHOLD
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" freqhold"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_PPSSIGNAL
if|if
condition|(
name|status
operator|&
name|STA_PPSSIGNAL
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" ppssignal"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_PPSJITTER
if|if
condition|(
name|status
operator|&
name|STA_PPSJITTER
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" ppsjitter"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_PPSWANDER
if|if
condition|(
name|status
operator|&
name|STA_PPSWANDER
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" ppswander"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_PPSERROR
if|if
condition|(
name|status
operator|&
name|STA_PPSERROR
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" ppserror"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_CLOCKERR
if|if
condition|(
name|status
operator|&
name|STA_CLOCKERR
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" clockerr"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_NANO
if|if
condition|(
name|status
operator|&
name|STA_NANO
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" nano"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_MODE
if|if
condition|(
name|status
operator|&
name|STA_MODE
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" mode=fll"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|STA_CLK
if|if
condition|(
name|status
operator|&
name|STA_CLK
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" src=B"
argument_list|)
expr_stmt|;
endif|#
directive|endif
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"pll time constant:    %ld\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|constant
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"precision:            %g s\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|precision
argument_list|)
operator|*
name|tscale
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"frequency tolerance:  %s ppm\n"
argument_list|,
name|fptoa
argument_list|(
operator|(
name|s_fp
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|tolerance
argument_list|)
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 	 * For backwards compatibility (ugh), we find the pps variables 	 * only if the shift member is nonzero. 	 */
if|if
condition|(
operator|!
name|ik
operator|->
name|shift
condition|)
return|return;
comment|/* 	 * pps variables 	 */
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"pps frequency:        %s ppm\n"
argument_list|,
name|fptoa
argument_list|(
operator|(
name|s_fp
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|ppsfreq
argument_list|)
argument_list|,
literal|3
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"pps stability:        %s ppm\n"
argument_list|,
name|fptoa
argument_list|(
operator|(
name|s_fp
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|stabil
argument_list|)
argument_list|,
literal|3
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"pps jitter:           %g s\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|jitter
argument_list|)
operator|*
name|tscale
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"calibration interval: %d s\n"
argument_list|,
literal|1
operator|<<
name|ntohs
argument_list|(
name|ik
operator|->
name|shift
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"calibration cycles:   %ld\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|calcnt
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"jitter exceeded:      %ld\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|jitcnt
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"stability exceeded:   %ld\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|stbcnt
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"calibration errors:   %ld\n"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ik
operator|->
name|errcnt
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_define
define|#
directive|define
name|IF_LIST_FMT
value|"%2d %c %48s %c %c %12.12s %03lx %3lu %2lu %5lu %5lu %5lu %2lu %3lu %7lu\n"
end_define

begin_define
define|#
directive|define
name|IF_LIST_FMT_STR
value|"%2s %c %48s %c %c %12.12s %3s %3s %2s %5s %5s %5s %2s %3s %7s\n"
end_define

begin_define
define|#
directive|define
name|IF_LIST_AFMT_STR
value|"     %48s %c\n"
end_define

begin_define
define|#
directive|define
name|IF_LIST_LABELS
value|"#", 'A', "Address/Mask/Broadcast", 'T', 'E', "IF name", "Flg", "TL", "#M", "recv", "sent", "drop", "S", "PC", "uptime"
end_define

begin_define
define|#
directive|define
name|IF_LIST_LINE
value|"==================================================================================================================\n"
end_define

begin_function
specifier|static
name|void
name|iflist
parameter_list|(
name|FILE
modifier|*
name|fp
parameter_list|,
name|struct
name|info_if_stats
modifier|*
name|ifs
parameter_list|,
name|int
name|items
parameter_list|,
name|int
name|itemsize
parameter_list|,
name|int
name|res
parameter_list|)
block|{
specifier|static
name|char
modifier|*
name|actions
init|=
literal|"?.+-"
decl_stmt|;
name|sockaddr_u
name|saddr
decl_stmt|;
if|if
condition|(
name|res
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitems
argument_list|(
name|items
argument_list|,
name|fp
argument_list|)
condition|)
return|return;
if|if
condition|(
operator|!
name|checkitemsize
argument_list|(
name|itemsize
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_if_stats
argument_list|)
argument_list|)
condition|)
return|return;
name|fprintf
argument_list|(
name|fp
argument_list|,
name|IF_LIST_FMT_STR
argument_list|,
name|IF_LIST_LABELS
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
name|IF_LIST_LINE
argument_list|)
expr_stmt|;
while|while
condition|(
name|items
operator|>
literal|0
condition|)
block|{
name|SET_ADDR
argument_list|(
name|saddr
argument_list|,
name|ntohl
argument_list|(
name|ifs
operator|->
name|v6_flag
argument_list|)
argument_list|,
name|ifs
operator|->
name|unaddr
operator|.
name|addr
operator|.
name|s_addr
argument_list|,
name|ifs
operator|->
name|unaddr
operator|.
name|addr6
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
name|IF_LIST_FMT
argument_list|,
name|ntohl
argument_list|(
name|ifs
operator|->
name|ifnum
argument_list|)
argument_list|,
name|actions
index|[
operator|(
name|ifs
operator|->
name|action
operator|>=
literal|1
operator|&&
name|ifs
operator|->
name|action
operator|<
literal|4
operator|)
condition|?
name|ifs
operator|->
name|action
else|:
literal|0
index|]
argument_list|,
name|stoa
argument_list|(
operator|(
operator|&
name|saddr
operator|)
argument_list|)
argument_list|,
literal|'A'
argument_list|,
name|ifs
operator|->
name|ignore_packets
condition|?
literal|'D'
else|:
literal|'E'
argument_list|,
name|ifs
operator|->
name|name
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|flags
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|last_ttl
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|num_mcast
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|received
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|sent
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|notsent
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|scopeid
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|peercnt
argument_list|)
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|ifs
operator|->
name|uptime
argument_list|)
argument_list|)
expr_stmt|;
name|SET_ADDR
argument_list|(
name|saddr
argument_list|,
name|ntohl
argument_list|(
name|ifs
operator|->
name|v6_flag
argument_list|)
argument_list|,
name|ifs
operator|->
name|unmask
operator|.
name|addr
operator|.
name|s_addr
argument_list|,
name|ifs
operator|->
name|unmask
operator|.
name|addr6
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
name|IF_LIST_AFMT_STR
argument_list|,
name|stoa
argument_list|(
operator|&
name|saddr
argument_list|)
argument_list|,
literal|'M'
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|ntohl
argument_list|(
name|ifs
operator|->
name|v6_flag
argument_list|)
operator|&&
name|ntohl
argument_list|(
name|ifs
operator|->
name|flags
argument_list|)
operator|&
operator|(
name|INT_BCASTOPEN
operator|)
condition|)
block|{
name|SET_ADDR
argument_list|(
name|saddr
argument_list|,
name|ntohl
argument_list|(
name|ifs
operator|->
name|v6_flag
argument_list|)
argument_list|,
name|ifs
operator|->
name|unbcast
operator|.
name|addr
operator|.
name|s_addr
argument_list|,
name|ifs
operator|->
name|unbcast
operator|.
name|addr6
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
name|IF_LIST_AFMT_STR
argument_list|,
name|stoa
argument_list|(
operator|&
name|saddr
argument_list|)
argument_list|,
literal|'B'
argument_list|)
expr_stmt|;
block|}
name|ifs
operator|++
expr_stmt|;
name|items
operator|--
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|get_if_stats
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_if_stats
modifier|*
name|ifs
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_IF_STATS
argument_list|,
literal|1
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|ifs
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_if_stats
argument_list|)
argument_list|)
expr_stmt|;
name|iflist
argument_list|(
name|fp
argument_list|,
name|ifs
argument_list|,
name|items
argument_list|,
name|itemsize
argument_list|,
name|res
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
specifier|static
name|void
name|do_if_reload
parameter_list|(
name|struct
name|parse
modifier|*
name|pcmd
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|)
block|{
name|struct
name|info_if_stats
modifier|*
name|ifs
decl_stmt|;
name|int
name|items
decl_stmt|;
name|int
name|itemsize
decl_stmt|;
name|int
name|res
decl_stmt|;
name|res
operator|=
name|doquery
argument_list|(
name|impl_ver
argument_list|,
name|REQ_IF_RELOAD
argument_list|,
literal|1
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|,
operator|&
name|items
argument_list|,
operator|&
name|itemsize
argument_list|,
operator|(
name|void
operator|*
operator|)
operator|&
name|ifs
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|info_if_stats
argument_list|)
argument_list|)
expr_stmt|;
name|iflist
argument_list|(
name|fp
argument_list|,
name|ifs
argument_list|,
name|items
argument_list|,
name|itemsize
argument_list|,
name|res
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

