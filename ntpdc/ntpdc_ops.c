...
block|}
block|,
literal|"display list of peers the server knows about [IP Version]"
block|}
block|,
block|{
literal|"peers"
...
block|}
block|,
literal|"display peer summary information [IP Version]"
block|}
block|,
block|{
literal|"dmpeers"
...
block|}
block|,
literal|"display peer summary info the way Dave Mills likes it (IP Version)"
block|}
block|,
block|{
literal|"showpeer"
...
literal|"version"
block|,
literal|"minpoll#|prefer|burst|iburst|'minpoll N'|'maxpoll N'|'keyid N'|'version N' ..."
block|}
block|,
...
block|}
block|,
literal|"list interface statistics"
block|}
block|,
block|{
literal|"ifreload"
...
block|}
block|,
literal|"reload interface configuration"
block|}
block|,
block|{
literal|0
...
begin_comment
comment|/*  * SET_SS_LEN_IF_PRESENT - used by SET_ADDR, SET_ADDRS macros  */
end_comment
...
define|#
directive|define
name|SET_SS_LEN_IF_PRESENT
parameter_list|(
name|psau
parameter_list|)
define|\
value|do {							\ 		(psau)->sa.sa_len = SOCKLEN(psau);		\ 	} while (0)
end_define
...
begin_comment
comment|/*  * SET_ADDR - setup address for v4/v6 as needed  */
end_comment
...
parameter_list|)
define|\
value|do {								\ 	memset(&(address), 0, sizeof(address));			\ 	if (v6flag) {						\ 		AF(&(address)) = AF_INET6;			\ 		SOCK_ADDR6(&(address)) = (v6addr);		\ 	} else {						\ 		AF(&(address)) = AF_INET;			\ 		NSRCADR(&(address)) = (v4addr);			\ 	}							\ 	SET_SS_LEN_IF_PRESENT(&(address));			\ } while (0)
end_define

begin_comment
comment|/*  * SET_ADDRS - setup source and destination addresses for   * v4/v6 as needed  */
end_comment
...
parameter_list|)
define|\
value|do {								\ 	memset(&(a1), 0, sizeof(a1));				\ 	memset(&(a2), 0, sizeof(a2));				\ 	if ((info)->v6_flag) {					\ 		AF(&(a1)) = AF_INET6;				\ 		AF(&(a2)) = AF_INET6;				\ 		SOCK_ADDR6(&(a1)) = (info)->a1prefix##6;	\ 		SOCK_ADDR6(&(a2)) = (info)->a2prefix##6;	\ 	} else {						\ 		AF(&(a1)) = AF_INET;				\ 		AF(&(a2)) = AF_INET;				\ 		NSRCADR(&(a1)) = (info)->a1prefix;		\ 		NSRCADR(&(a2)) = (info)->a2prefix;		\ 	}							\ 	SET_SS_LEN_IF_PRESENT(&(a1));				\ 	SET_SS_LEN_IF_PRESENT(&(a2));				\ } while (0)
end_define
...
parameter_list|)
define|\
value|do {								\ 	memset(&(address), 0, sizeof(address));			\ 	memset(&(mask), 0, sizeof(mask));			\ 	if ((info)->v6_flag) {					\ 		AF(&(address)) = AF_INET6;			\ 		AF(&(addrmask)) = AF_INET6;			\ 		SOCK_ADDR6(&(address)) = (info)->addr6;		\ 		SOCK_ADDR6(&(addrmask)) = (info)->mask6;	\ 	} else {						\ 		AF(&(address)) = AF_INET;			\ 		AF(&(addrmask)) = AF_INET;			\ 		NSRCADR(&(address)) = (info)->addr;		\ 		NSRCADR(&(addrmask)) = (info)->mask;		\ 	}							\ 	SET_SS_LEN_IF_PRESENT(&(address));			\ 	SET_SS_LEN_IF_PRESENT(&(addrmask));			\ } while (0)
end_define
...
name|fp
argument_list|,
literal|"%c%-15.15s %-15.15s %2u %4d  %3o %7.7s %9.9s %7.7s\n"
argument_list|,
name|c
argument_list|,
name|nntohost
...
name|stderr
argument_list|,
literal|"***Server doesn't understand IPv6 addresses\n"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|fp
argument_list|,
literal|"time last received:   %lus\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"time until next send: %lus\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"reachability change:  %lus\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"bad length or format:   %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"free receive buffers: %u\n"
argument_list|,
operator|(
name|u_int
operator|)
...
name|fp
argument_list|,
literal|"used receive buffers: %u\n"
argument_list|,
operator|(
name|u_int
operator|)
...
name|fp
argument_list|,
literal|"new peer allocations: %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"peer demobilizations: %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"***min/max-poll must be within %d..%d\n"
argument_list|,
name|NTP_MINPOLL
argument_list|,
name|NTP_MAXPOLL
...
name|fp
argument_list|,
literal|"***Retrying command with old conf_peer size\n"
argument_list|)
expr_stmt|;
name|res
operator|=
...
begin_comment
comment|/* XXX: HMS: we apparently don't report set bits we do not recognize. */
end_comment
...
block|{
comment|/* XXX IPv6 sanity checking stuff */
block|}
if|if
condition|(
name|err
...
name|fp
argument_list|,
literal|"remote address          port local address      count m ver rstr avgint  lstint\n"
argument_list|)
expr_stmt|;
operator|(
name|void
...
name|fp
argument_list|,
literal|"%-22.22s %5d %-15s %8lu %1u %1u %6lx %6lu %7lu\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
...
name|fp
argument_list|,
literal|"     address               port     count mode ver rstr avgint  lstint\n"
argument_list|)
expr_stmt|;
operator|(
name|void
...
name|fp
argument_list|,
literal|"%-25.25s %5u %9lu %4u %2u %9lx %9lu %9lu\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
...
name|fp
argument_list|,
literal|"%-20.20s %5u %9lu %4u   %3u %9lu %9lu\n"
argument_list|,
name|nntohost
argument_list|(
operator|&
...
name|fp
argument_list|,
literal|"async messages sent:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"bad offset on input:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"bad version packets:    %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
...
name|fp
argument_list|,
literal|"data in pkt too short:  %lu\n"
argument_list|,
operator|(
name|u_long
operator|)
...
directive|define
name|IF_LIST_FMT
value|"%2d %c %48s %c %c %12.12s %03lx %3lu %2lu %5lu %5lu %5lu %2lu %3lu %7lu\n"
end_define

begin_define
define|#
directive|define
name|IF_LIST_FMT_STR
value|"%2s %c %48s %c %c %12.12s %3s %3s %2s %5s %5s %5s %2s %3s %7s\n"
end_define

begin_define
define|#
directive|define
name|IF_LIST_AFMT_STR
value|"     %48s %c\n"
end_define
...
directive|define
name|IF_LIST_LABELS
value|"#", 'A', "Address/Mask/Broadcast", 'T', 'E', "IF name", "Flg", "TL", "#M", "recv", "sent", "drop", "S", "PC", "uptime"
end_define
