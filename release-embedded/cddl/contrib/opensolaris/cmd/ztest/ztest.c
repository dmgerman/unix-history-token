...
literal|0
argument_list|,
literal|"dsl_dataset_user_hold(%s)"
argument_list|,
name|fullname
argument_list|,
name|tag
...
name|bin
argument_list|,
literal|"/usr/sbin%.*s/zdb -bcc%s%s -U %s %s"
argument_list|,
name|isalen
argument_list|,
name|isa
...
argument_list|)
expr_stmt|;
comment|/* 	 * Run tests that generate log records but don't alter the pool config 	 * or depend on DSL sync tasks (snapshots, objset create/destroy, etc). 	 * We do a txg_wait_synced() after each iteration to force the txg 	 * to increase well beyond the last synced value in the uberblock. 	 * The ZIL should be OK with that. 	 */
while|while
condition|(
name|ztest_random
argument_list|(
