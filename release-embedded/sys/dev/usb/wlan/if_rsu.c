...
name|self
argument_list|,
literal|"cannot allocate interface\n"
argument_list|)
expr_stmt|;
goto|goto
name|fail_ifalloc
...
block|if (usb_lookup(rsu_devs_noht, uaa->vendor, uaa->product) == NULL) {
comment|/* Set HT capabilities. */
block|ic->ic_htcaps = 		    IEEE80211_HTCAP_CBW20_40 | 		    IEEE80211_HTCAP_DSSSCCK40;
comment|/* Set supported HT rates. */
block|for (i = 0; i< 2; i++) 			ic->ic_sup_mcs[i] = 0xff; 	}
endif|#
directive|endif
...
argument_list|)
expr_stmt|;
comment|/* rsu_init(ifp); XXX needs a process context! */
name|ifp
operator|->
name|if_oerrors
operator|++
...
argument_list|)
expr_stmt|;
comment|/* NB: it really takes that long for firmware to boot. */
name|usb_pause_mtx
argument_list|(
operator|&
name|sc
...
directive|if
literal|0
block|if (ic->ic_htcaps& IEEE80211_HTCAP_CBW20_40) {
comment|/* Enable 40MHz mode. */
block|error = rsu_fw_iocmd(sc, 		    SM(R92S_IOCMD_CLASS, 0xf4) | 		    SM(R92S_IOCMD_INDEX, 0x00) | 		    SM(R92S_IOCMD_VALUE, 0x0007)); 		if (error != 0) { 			device_printf(sc->sc_dev, 			    "could not enable 40MHz mode\n"); 			goto fail; 		} 	}
comment|/* Set default channel. */
block|ic->ic_bss->ni_chan = ic->ic_ibss_chan;
endif|#
directive|endif
name|sc
operator|->
