...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  *  * Copyright (c) 2006-2010 Pawel Jakub Dawidek<pjd@FreeBSD.org>  * All rights reserved.  * Copyright (c) 2013 by Delphix. All rights reserved.  * Copyright (c) 2013, Joyent, Inc. All rights reserved.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * Reinitialize the dump area to the new size. If we 	 * failed to resize the dump area then restore it back to 	 * its original size. 	 */
if|if
condition|(
name|zv
operator|&&
...
name|B_FALSE
decl_stmt|;
comment|/* 	 * Protect against recursively entering spa_namespace_lock 	 * when spa_open() is used for a pool on a (local) ZVOL(s). 	 * This is needed since we replaced upstream zfsdev_state_lock 	 * with spa_namespace_lock in the ZVOL code. 	 * We are using the same trick as spa_open(). 	 * Note that calls in zvol_first_open which need to resolve 	 * pool name to a spa object will enter spa_open() 	 * recursively, but that function already has all the 	 * necessary protection. 	 */
if|if
condition|(
operator|!
name|MUTEX_HELD
...
name|write_state
decl_stmt|;
comment|/* 		 * Unlike zfs_log_write() we can be called with 		 * upto DMU_MAX_ACCESS/2 (5MB) writes. 		 */
if|if
condition|(
name|blocksize
operator|>
...
comment|/*  * Dirtbag ioctls to support mkfs(1M) for UFS filesystems.  See dkio(7I).  */
comment|/*ARGSUSED*/
name|int
name|zvol_ioctl
parameter_list|(
