...
condition|)
break|break;
comment|/* Must be ZPL and not gzip compressed. */
if|if
condition|(
...
expr_stmt|;
block|}
comment|/* 	 * Create pool config based on the best vdev config. 	 */
name|nvlist_dup
argument_list|(
name|best_cfg
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * Drop vdev config elements that should not be present at pool level. 	 */
name|nvlist_remove
argument_list|(
name|config
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 		 * Set spa_ubsync.ub_version as it can be used in vdev_alloc() 		 * via spa_version(). 		 */
if|if
condition|(
name|nvlist_lookup_uint64
argument_list|(
...
name|CE_NOTE
argument_list|,
literal|"Cannot find the pool label for '%s'"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/* 	 * If anything has changed in this txg, or if someone is waiting 	 * for this txg to sync (eg, spa_vdev_remove()), push the 	 * deferred frees from the previous txg.  If not, leave them 	 * alone so that we don't generate work on an otherwise idle 	 * system. 	 */
if|if
condition|(
operator|!
name|txg_list_empty
