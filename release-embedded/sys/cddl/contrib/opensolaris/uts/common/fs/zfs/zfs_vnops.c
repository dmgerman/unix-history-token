...
argument_list|)
expr_stmt|;
comment|/* 		 * FreeBSD: In OpenSolaris they only check if rename source is 		 * ".." here, because "." is handled in their lookup. This is 		 * not the case for FreeBSD, so we check for "." explicitly. 		 */
if|if
condition|(
name|strcmp
argument_list|(
...
argument_list|,
operator|(
literal|"reqpage beyond [reqstart, reqstart + reqsize[ bounds"
operator|)
argument_list|)
expr_stmt|;
block|}
...
argument_list|,
operator|(
literal|"zfs_getpages: page %p is dirty"
operator|,
name|m
index|[
name|i
