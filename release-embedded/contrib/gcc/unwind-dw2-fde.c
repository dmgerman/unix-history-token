...
name|y
parameter_list|)
value|do { const fde * tmp = x; x = y; y = tmp; } while (0)
end_define

begin_comment
comment|/* Convert a semi-heap to a heap.  A semi-heap is a heap except possibly    for the first (root) node; push it down to its rightful place.  */
end_comment
...
name|m
decl_stmt|;
comment|/* Expand our heap incrementally from the end of the array, heapifying      each resulting semi-heap as we go.  After each step, a[m] is the top      of a heap.  */
for|for
control|(
name|m
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Shrink our heap incrementally from the end of the array, first      swapping out the largest element a[0] and then re-heapifying the      resulting semi-heap.  After each step, a[0..m) is a heap.  */
for|for
control|(
name|m
operator|=
