...
begin_comment
comment|/* Simple garbage collection for the GNU compiler.    Copyright (C) 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006    Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
specifier|const
name|size_t
name|mmap_offset_alignment
init|=
name|host_hooks
operator|.
name|gt_pch_alloc_granularity
argument_list|()
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Pad the PCH file so that the mmapped area starts on an allocation      granularity (usually page) boundary.  */
block|{
name|long
name|o
decl_stmt|;
...
begin_comment
comment|/* Default version of HOST_HOOKS_GT_PCH_GET_ADDRESS when mmap is not present.    Select no address whatsoever, and let gt_pch_save choose what it will with    malloc, presumably.  */
end_comment
...
begin_comment
comment|/* Default version of HOST_HOOKS_GT_PCH_GET_ADDRESS.   Return the    alignment required for allocating virtual memory. Usually this is the    same as pagesize.  */
end_comment
...
begin_comment
comment|/* Default version of HOST_HOOKS_GT_PCH_GET_ADDRESS when mmap is present.    We temporarily allocate SIZE bytes, and let the kernel place the data    wherever it will.  If it worked, that's our spot, if not we're likely    to be in trouble.  */
end_comment
...
begin_comment
comment|/* Modify the bound based on rlimits.  */
end_comment
...
name|RLIMIT_AS
argument_list|)
comment|/* RLIMIT_AS is what POSIX says is the limit on mmap.  Presumably      any OS which has RLIMIT_AS also has a working mmap that GCC will use.  */
if|if
condition|(
name|getrlimit
argument_list|(
...
name|RLIMIT_DATA
argument_list|)
comment|/* ... but some older OSs bound mmap based on RLIMIT_DATA, or we      might be on an OS that has a broken mmap.  (Others don't bound      mmap at all, apparently.)  */
if|if
condition|(
name|getrlimit
argument_list|(
...
operator|<
name|limit
comment|/* Darwin has this horribly bogus default setting of 	 RLIMIT_DATA, to 6144Kb.  No-one notices because RLIMIT_DATA 	 appears to be ignored.  Ignore such silliness.  If a limit 	 this small was actually effective for mmap, GCC wouldn't even 	 start up.  */
operator|&&
name|rlim
operator|.
name|rlim_cur
...
directive|endif
comment|/* RLIMIT_AS or RLIMIT_DATA */
endif|#
directive|endif
...
name|RLIMIT_RSS
argument_list|)
comment|/* Try not to overrun the RSS limit while doing garbage collection.        The RSS limit is only advisory, so no margin is subtracted.  */
block|{
name|struct
name|rlimit
name|rlim
...
directive|endif
comment|/* Don't blindly run over our data limit; do GC at least when the      *next* GC would be within 16Mb of the limit.  If GCC does hit the      data limit, compilation will fail, so this tries to be      conservative.  */
name|limit_kbytes
operator|=
name|MAX
argument_list|(
...
begin_comment
comment|/* Datastructure used to store per-call-site statistics.  */
end_comment
...
begin_comment
comment|/* Hashtable used for statistics.  */
end_comment
...
begin_comment
comment|/* Hash table helpers functions.  */
end_comment
...
begin_comment
comment|/* Hashtable converting address of allocated field to loc descriptor.  */
end_comment
...
begin_comment
comment|/* Return descriptor for given call site, create new one if needed.  */
end_comment
...
begin_comment
comment|/* Record ALLOCATED and OVERHEAD bytes to descriptor NAME:LINE (FUNCTION).  */
end_comment
...
begin_comment
comment|/* Helper function for prune_overhead_list.  See if SLOT is still marked and    remove it from hashtable if it is not.  */
end_comment
...
begin_comment
comment|/* After live values has been marked, walk all recorded pointers and see if    they are still live.  */
end_comment
...
begin_comment
comment|/* Notice that the pointer has been freed.  */
end_comment
...
begin_comment
comment|/* Helper for qsort; sort descriptors by amount of memory consumed.  */
end_comment
...
begin_comment
comment|/* Collect array of the descriptors from hashtable.  */
end_comment
...
begin_comment
comment|/* Dump per-site memory statistics.  */
end_comment
...
name|stderr
argument_list|,
literal|"%-48s %10li:%4.1f%% %10li:%4.1f%% %10li:%4.1f%% %10li:%4.1f%% %10li\n"
argument_list|,
name|s
argument_list|,
operator|(
...
name|stderr
argument_list|,
literal|"%-48s %10ld       %10ld       %10ld       %10ld       %10ld\n"
argument_list|,
literal|"Total"
argument_list|,
operator|(
...
name|stderr
argument_list|,
literal|"%-48s %10s       %10s       %10s       %10s       %10s\n"
argument_list|,
literal|"source location"
argument_list|,
literal|"Garbage"
