...
argument_list|)
decl_stmt|;
comment|/* You want to truncate to a _what_?  */
name|gcc_assert
argument_list|(
...
directive|else
comment|/* defined(POINTERS_EXTEND_UNSIGNED) */
name|enum
name|machine_mode
name|from_mode
decl_stmt|;
...
name|code
decl_stmt|;
comment|/* If X already has the right mode, just return it.  */
if|if
condition|(
name|GET_MODE
argument_list|(
...
name|MULT
case|:
comment|/* For addition we can safely permute the conversion and addition 	 operation if one operand is a constant and converting the constant 	 does not change it.  We can always safely permute them if we are 	 making the address narrower.  */
if|if
condition|(
name|GET_MODE_SIZE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* By passing constant addresses through registers      we get a chance to cse them.  */
if|if
condition|(
operator|!
...
begin_comment
comment|/* If X is a memory reference to a member of an object block, try rewriting    it to use an anchor instead.  Return the new memory reference on success    and the old one on failure.  */
end_comment
...
name|x
return|;
comment|/* Split the address into a base and offset.  */
name|base
operator|=
name|XEXP
argument_list|(
...
expr_stmt|;
block|}
comment|/* Check whether BASE is suitable for anchors.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|x
return|;
comment|/* Decide where BASE is going to be.  */
name|place_block_symbol
argument_list|(
name|base
argument_list|)
expr_stmt|;
comment|/* Get the anchor we need to use.  */
name|offset
operator|+=
...
argument_list|)
expr_stmt|;
comment|/* Work out the offset from the anchor.  */
name|offset
operator|-=
name|SYMBOL_REF_BLOCK_OFFSET
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If we're going to run a CSE pass, force the anchor into a register.      We will then be able to reuse registers for several accesses, if the      target costs say that that's worthwhile.  */
if|if
condition|(
operator|!
name|cse_not_expected
...
argument_list|)
expr_stmt|;
comment|/* Let optimizers know that TEMP is a pointer, and if so, the      known alignment of that pointer.  */
block|{
name|unsigned
name|align
init|=
...
begin_comment
comment|/* Return the mode to use to store a scalar of TYPE and MODE.    PUNSIGNEDP points to the signedness of the type and may be adjusted    to show what signedness to use on extension operations.     FOR_CALL is nonzero if this call is promoting args for a call.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* These clobbers prevent the scheduler from moving 	 references to variable arrays below the code 	 that deletes (pops) the arrays.  */
name|emit_insn
argument_list|(
...
begin_comment
comment|/* Invoke emit_stack_save on the nonlocal_goto_save_area for the current    function.  This function should be called whenever we allocate or    deallocate dynamic stack space.  */
end_comment
...
name|r_save
decl_stmt|;
comment|/* The nonlocal_goto_save_area object is an array of N pointers.  The      first one is used for the frame pointer save; the rest are sized by      STACK_SAVEAREA_MODE.  Create a reference to array index 1, the first      of the stack save area slots.  */
name|t_save
operator|=
name|build4
argument_list|(
...
name|tmpmode
decl_stmt|;
comment|/* int_size_in_bytes can return -1.  We don't need a check here 	 since the value of bytes will then be large enough that no 	 mode will match anyway.  */
for|for
control|(
name|tmpmode
operator|=
