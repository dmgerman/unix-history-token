...
begin_comment
comment|/*     LOCAL FUNCTION     elf_symtab_read -- read the symbol table of an ELF file     SYNOPSIS     void elf_symtab_read (struct objfile *objfile, int dynamic, 			 long number_of_symbols, asymbol **symbol_table)     DESCRIPTION     Given an objfile, a symbol table, and a flag indicating whether the    symbol table contains dynamic symbols, add all the global function    and data symbols to the minimal symbol table.     In stabs-in-ELF, as implemented by Sun, there are some local symbols    defined in the ELF symbol table, which can be used to locate    the beginnings of sections from each ".o" file that was linked to    form the executable objfile.  We gather any such info and record it    in data structures hung off the objfile's private data.   */
end_comment
...
expr_stmt|;
block|}
comment|/* Add synthetic symbols - for instance, names for any PLT entries.  */
name|synthcount
operator|=
...
name|i
expr_stmt|;
comment|/* Synthetic symbols are not, strictly speaking, either local 	     or global.  But we can treat them as global symbols, since 	     they are effectively dynamic symbols.  */
name|synth_symbol_table
index|[
name|i
index|]
