...
begin_comment
comment|/* Check whether the value VAL is printable.  Return 1 if it is;    return 0 and print an appropriate error message to STREAM if it    is not.  */
end_comment

begin_function
specifier|static
name|int
name|value_check_printable
parameter_list|(
name|struct
name|value
modifier|*
...
begin_comment
comment|/* Print the value VAL onto stream STREAM according to FORMAT (a    letter, or 0 for natural format using TYPE).     If DEREF_REF is nonzero, then dereference references, otherwise just print    them like pointers.     The PRETTY parameter controls prettyprinting.     If the data are a string pointer, returns the number of string characters    printed.     This is a preferable interface to val_print, above, because it uses    GDB's value mechanism.  */
end_comment
