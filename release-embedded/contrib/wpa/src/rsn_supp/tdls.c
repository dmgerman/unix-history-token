...
literal|"dialog_token=%u status_code=%u msg_len=%u"
argument_list|,
name|MAC2STR
argument_list|(
name|dest
...
argument_list|)
expr_stmt|;
comment|/* clear the Peerkey statemachine */
name|wpa_tdls_peer_free
argument_list|(
name|sm
argument_list|,
...
name|MSG_INFO
argument_list|,
literal|"TDLS: Failed to parse IEs in Teardown"
argument_list|)
expr_stmt|;
return|return
operator|-
...
begin_comment
comment|/**  * wpa_tdls_send_error - To send suitable TDLS status response with  *	appropriate status code mentioning reason for error/failure.  * @dst 	- MAC addr of Peer station  * @tdls_action - TDLS frame type for which error code is sent  * @status 	- status code mentioning reason  */
end_comment
...
name|MSG_INFO
argument_list|,
literal|"TDLS: Failed to parse IEs in TPK M1"
argument_list|)
expr_stmt|;
goto|goto
name|error
...
directive|if
literal|0
comment|/* TODO: Disabling the link would be more proper 			 * operation here, but it seems to trigger a race with 			 * some drivers handling the new request frame. */
block|wpa_sm_tdls_oper(sm, TDLS_DISABLE_LINK, src_addr);
else|#
directive|else
if|if
condition|(
...
comment|/* CONFIG_TDLS_TESTING */
block|}
comment|/* add supported rates and capabilities to the TDLS peer */
name|wpa_sm_tdls_peer_addset
argument_list|(
name|sm
argument_list|,
...
name|MSG_INFO
argument_list|,
literal|"TDLS: Failed to parse IEs in TPK M2"
argument_list|)
expr_stmt|;
goto|goto
name|error
...
name|MSG_INFO
argument_list|,
literal|"TDLS: Failed to parse KDEs in TPK M3"
argument_list|)
expr_stmt|;
return|return
operator|-
...
return|;
block|}
comment|/* Find existing entry and if found, use that instead of adding 	 * a new one */
for|for
control|(
name|peer
operator|=
