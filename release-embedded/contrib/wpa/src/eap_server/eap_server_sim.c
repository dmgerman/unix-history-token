...
begin_comment
comment|/*  * hostapd / EAP-SIM (RFC 4186)  * Copyright (c) 2005-2012, Jouni Malinen<j@w1.fi>  *  * This software may be distributed under the terms of the BSD license.  * See README for more details.  */
end_comment
...
condition|)
block|{
comment|/* Cannot use more than three rounds of Start messages */
...
condition|)
block|{
comment|/* 		 * This is a special case that is used to recover from 		 * AT_COUNTER_TOO_SMALL during re-authentication. Since we 		 * already know the identity of the peer, there is no need to 		 * request any identity in this case. 		 */
block|}
elseif|else
if|if
condition|(
...
name|wpabuf
modifier|*
name|eap_sim_build_notification
parameter_list|(
name|struct
name|eap_sm
modifier|*
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Generating Notification"
argument_list|)
expr_stmt|;
name|msg
operator|=
...
specifier|static
name|Boolean
name|eap_sim_unexpected_subtype
parameter_list|(
name|struct
name|eap_sim_data
modifier|*
...
condition|)
block|{
comment|/* 		 * Special case for AT_COUNTER_TOO_SMALL recovery - no identity 		 * was requested since we already know it. 		 */
goto|goto
name|skip_id_update
goto|;
block|}
comment|/* 	 * We always request identity in SIM/Start, so the peer is required to 	 * have replied with one. 	 */
if|if
condition|(
operator|!
name|attr
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Peer did not provide any "
literal|"identity"
argument_list|)
expr_stmt|;
goto|goto
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Reauth username '%s'"
argument_list|,
name|username
argument_list|)
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Unknown reauth "
literal|"identity - request full auth identity"
argument_list|)
expr_stmt|;
comment|/* Remain in START state for another round */
return|return;
block|}
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Using fast re-authentication"
argument_list|)
expr_stmt|;
name|os_strlcpy
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Pseudonym username '%s'"
argument_list|,
name|username
argument_list|)
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Unknown pseudonym "
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Permanent username '%s'"
argument_list|,
name|username
argument_list|)
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Unrecognized username '%s'"
argument_list|,
name|username
argument_list|)
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Client replied to notification"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|MSG_DEBUG
argument_list|,
literal|"EAP-SIM: Unrecognized or unexpected "
literal|"EAP-SIM Subtype in EAP Response"
argument_list|)
expr_stmt|;
name|data
operator|->
