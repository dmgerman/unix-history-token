...
begin_comment
comment|/* Fill in *INFO with the information it would contain if it were    obtained from svn_wc__db_read_children_info. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Maybe we have to get some shadowed lock from BASE to make our test suite      happy... (It might be completely unrelated, but...) */
if|if
condition|(
name|mtb
operator|->
...
decl_stmt|;
comment|/* NOTE: we can't use op-root-ness as a condition here since a base            * node can be the root of a move and still not be an explicit            * op-root (having a working node with op_depth == pathelements).            *            * Both these (almost identical) situations showcase this:            *   svn mv a/b bb            *   svn del a            * and            *   svn mv a aa            *   svn mv aa/b bb            * In both, 'bb' is moved from 'a/b', but 'a/b' has no op_depth>0            * node at all, as its parent 'a' is locally deleted. */
name|SVN_ERR
argument_list|(
...
expr_stmt|;
block|}
comment|/* ### ^^^ THIS SUCKS. For at least two reasons:            * 1) We scan the node deletion and that's technically not necessary.            *    We'd be fine to know if this is an actual root of a move.            * 2) From the elaborately calculated results, we backwards-guess            *    whether this is a root.            * It works ok, and this code only gets called when a node is an            * explicit target of a 'status'. But it would be better to do this            * differently.            * We could return moved-to via svn_wc__db_base_get_info() (called            * just above), but as moved-to is only intended to be returned for            * roots of a move, that doesn't fit too well. */
block|}
block|}
comment|/* ### svn_wc__db_read_info() could easily return the moved-here flag. But    * for now... (The per-dir query for recursive status is far more optimal.)    * Note that this actually scans around to get the full path, for a bool.    * This bool then gets returned, later is evaluated, and if true leads to    * the same paths being scanned again. We'd want to obtain this bool here as    * cheaply as svn_wc__db_read_children_info() does. */
if|if
condition|(
name|mtb
operator|->
