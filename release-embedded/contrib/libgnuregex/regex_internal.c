...
name|reg_errcode_t
name|internal_function
name|__attribute_warn_unused_result__
...
name|new_wcs
decl_stmt|;
comment|/* Avoid overflow in realloc.  */
specifier|const
name|size_t
...
condition|)
block|{
comment|/* It is an invalid character, an incomplete character 		 at the end of the string, or '\0'.  Just use the byte.  */
name|int
name|ch
init|=
name|pstr
...
condition|)
block|{
comment|/* We treat these cases as a single byte character.  */
if|if
condition|(
name|mbclen
operator|==
...
condition|)
block|{
comment|/* Should the already checked characters be kept?  */
if|if
condition|(
name|BE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* This can be quite complicated, so handle specially 		 only the common and easy case where the character with 		 different length representation of lower and upper 		 case is present at or after offset.  */
if|if
condition|(
name|pstr
operator|->
...
block|{
comment|/* Otherwise, just find out how long the partial multibyte 		     character at offset is and fill it with WEOF/255.  */
name|pstr
operator|->
name|len
operator|=
...
directive|ifdef
name|_LIBC
comment|/* We know the wchar_t encoding is UCS4, so for the simple 		     case, ASCII characters, skip the conversion step.  */
if|if
condition|(
name|isascii
argument_list|(
...
name|new_nodes
decl_stmt|;
comment|/* Avoid overflows in realloc.  */
specifier|const
name|size_t
name|max_object_size
init|=
