...
parameter_list|)
define|\
value|do{ trans1[state] = trans1[state] - MARKER_DIFFERENCE;} while(0)
define|#
directive|define
name|IS_MARKED
parameter_list|(
...
parameter_list|)
define|\
value|do{ \ nfaccnum = accptnum[state]; \ if ( nfaccnum != NIL ) \ accset[++nacc] = nfaccnum; \ }while(0)
define|#
directive|define
name|DO_REALLOCATION
parameter_list|()
define|\
value|do { \ current_max_dfa_size += MAX_DFA_SIZE_INCREMENT; \ ++num_reallocs; \ t = reallocate_integer_array( t, current_max_dfa_size ); \ stk = reallocate_integer_array( stk, current_max_dfa_size ); \ }while(0) \  #define PUT_ON_STACK(state) \ do { \ if ( ++stkend>= current_max_dfa_size ) \ DO_REALLOCATION(); \ stk[stkend] = state; \ MARK_STATE(state); \ }while(0)
define|#
directive|define
name|ADD_STATE
parameter_list|(
...
parameter_list|)
define|\
value|do { \ if ( ++numstates>= current_max_dfa_size ) \ DO_REALLOCATION(); \ t[numstates] = state; \ hashval += state; \ }while(0)
define|#
directive|define
name|STACK_STATE
parameter_list|(
...
parameter_list|)
define|\
value|do { \ PUT_ON_STACK(state); \ CHECK_ACCEPT(state); \ if ( nfaccnum != NIL || transchar[state] != SYM_EPSILON ) \ ADD_STATE(state); \ }while(0)
if|if
condition|(
operator|!
name|did_stk_init
...
index|]
decl_stmt|;
comment|/* accset needs to be large enough to hold all of the rules present 	 * in the input, *plus* their YY_TRAILING_HEAD_MASK variants. 	 */
name|accset
operator|=
name|allocate_integer_array
argument_list|(
...
literal|1
expr_stmt|;
comment|/* Begin generating yy_nxt[][] 		 * This spans the entire LONG function. 		 * This table is tricky because we don't know how big it will be. 		 * So we'll have to realloc() on the way... 		 * we'll wait until we can calculate yynxt_tbl->td_hilen. 		 */
name|yynxt_tbl
operator|=
operator|(
expr|struct
...
name|out_str
argument_list|(
literal|"static yyconst %s *yy_nxt =0;\n"
argument_list|,
name|long_align
condition|?
literal|"flex_int32_t"
...
condition|)
block|{
comment|/* Each time we hit here, it's another td_hilen, so we realloc. */
name|yynxt_tbl
operator|->
name|td_hilen
operator|++
...
condition|)
block|{
comment|/* We sort the states in sns so we 					 * can compare it to oldsns quickly. 					 */
name|qsort
argument_list|(
operator|&
name|sns
...
condition|)
block|{
comment|/* We sort the accepting set in increasing order so the 		 * disambiguating rule that the first rule listed is considered 		 * match in the event of ties will work. 		 */
name|qsort
argument_list|(
operator|&
name|accset
