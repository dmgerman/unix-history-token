...
name|SM_RCSID
argument_list|(
literal|"@(#)$Id: savemail.c,v 8.318 2013/03/12 15:24:54 ca Exp $"
argument_list|)
end_macro
...
argument_list|)
expr_stmt|;
comment|/* 				**  p == NULL shouldn't happen due to 				**  check in srvrsmtp.c 				**  we could log an error in this case. 				*/
if|if
condition|(
name|p
operator|!=
...
argument_list|)
argument_list|,
literal|"Original-Recipient: %.100s;%.700s"
argument_list|,
name|q
operator|->
name|q_orcpt
...
begin_comment
comment|/* **  ISATOM -- check if a string is an "atom" ** **	Parameters: **		s -- the string to check. ** **	Returns: **		true -- iff s is an atom */
end_comment
