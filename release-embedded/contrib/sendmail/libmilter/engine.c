...
begin_comment
comment|/* **  MI_CLR_SYMLIST -- clear list of macros ** **	Parameters: **		ctx -- context structure ** **	Returns: **		None. */
end_comment
...
begin_comment
comment|/* **  MI_CLR_CTX -- clear context ** **	Parameters: **		ctx -- context structure ** **	Returns: **		None. */
end_comment
...
condition|)
block|{
comment|/* 		**  Allow changing the size only if milter is compiled 		**  against a version that supports this. 		**  If a milter is dynamically linked against a newer 		**  libmilter version, we don't want to "surprise" 		**  it with a larger buffer as it may rely on it 		**  even though it is not documented as a limit. 		*/
if|if
condition|(
name|bitset
argument_list|(
...
directive|if
literal|0
comment|/* don't log this for now... */
block|else if (ctx->ctx_smfi->xxfi_version< SMFI_VERSION_MDS&& 		 bitset(SMFIP_MDS_1M|SMFIP_MDS_256K, v)) 	{ 		smi_log(SMI_LOG_WARN, 			"%s: st_optionneg[%ld]: milter version=%X, trying flags=%X", 			ctx->ctx_smfi->xxfi_name, 			(long) ctx->ctx_id, ctx->ctx_smfi->xxfi_version, v); 	}
endif|#
directive|endif
comment|/* 0 */
endif|#
...
comment|/* 	**  MTA protocol flags. 	**  We pass the internal flags to the milter as "read only", 	**  i.e., a milter can read them so it knows which size 	**  will be used, but any changes by a milter will be ignored 	**  (see below, search for SMFI_INTERNAL). 	*/
name|ctx
operator|->
name|ctx_mta_pflags
operator|=
...
argument_list|)
expr_stmt|;
comment|/* in test mode: take flags without further modifications */
if|if
condition|(
operator|!
name|testmode
condition|)
comment|/* Warning: check statement below! */
endif|#
directive|endif
comment|/* _FFR_MILTER_CHECK */
comment|/* 	**  Remove the internal flags that might have been set by a milter 	**  and set only those determined above. 	*/
name|ctx
operator|->
name|ctx_pflags2mta
operator|=
