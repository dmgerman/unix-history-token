...
condition|)
block|{
comment|/* PR binutils/4292 	 Guess the target's bitsize based on its name. 	 We assume here than any 64-bit format will include 	 "64" somewhere in its name.  The only known exception 	 is the MMO object file format.  */
if|if
condition|(
name|strstr
argument_list|(
...
name|_
argument_list|(
literal|"Print width has not been initialized (%d)"
argument_list|)
argument_list|,
name|print_width
argument_list|)
