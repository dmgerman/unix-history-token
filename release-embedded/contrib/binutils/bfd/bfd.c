...
begin_comment
comment|/* Generic BFD library interface and support routines.    Copyright 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999,    2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007    Free Software Foundation, Inc.    Written by Cygnus Support.     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* FUNCTION 	bfd_set_error  SYNOPSIS 	void bfd_set_error (bfd_error_type error_tag, ...);  DESCRIPTION 	Set the BFD error condition to be @var{error_tag}. 	If @var{error_tag} is bfd_error_on_input, then this function 	takes two more parameters, the input bfd where the error 	occurred, and the bfd_error_type error. */
end_comment
...
condition|)
block|{
comment|/* This is an error that occurred during bfd_close when 	 writing an archive, but on one of the input files.  */
name|va_list
name|ap
decl_stmt|;
name|va_start
...
name|buf
return|;
comment|/* Ick, what to do on out of memory?  */
return|return
name|msg
return|;
block|}
...
name|buf
expr_stmt|;
comment|/* Reserve enough space for the existing format string.  */
name|avail
operator|-=
name|strlen
argument_list|(
...
name|buf
expr_stmt|;
comment|/* If we run out of space, tough, you lose your ridiculously 	     long file or section name.  It's not safe to try to alloc 	     memory here;  We might be printing an out of memory message.  */
if|if
condition|(
name|avail
operator|==
...
name|NULL
condition|)
comment|/* Invoking %B with a null bfd pointer is an internal error.  */
name|abort
argument_list|()
expr_stmt|;
elseif|else
...
name|NULL
condition|)
comment|/* Invoking %A with a null section pointer is an internal error.  */
name|abort
argument_list|()
expr_stmt|;
name|abfd
...
literal|2
expr_stmt|;
comment|/* We need to replace any '%' we printed by "%%". 		 First count how many.  */
name|q
operator|=
name|bufp
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* If there isn't room, trim off the end of the string.  */
name|q
operator|=
name|bufp
expr_stmt|;
...
name|extra
expr_stmt|;
comment|/* Now double all '%' chars, shuffling the string as we go.  */
while|while
condition|(
name|extra
operator|!=
...
begin_comment
comment|/* SECTION 	Miscellaneous  SUBSECTION 	Miscellaneous functions */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Return a proper value for DJGPP& PE COFF.      This function is required for DWARF2 support, but there is      no place to store this information in the COFF back end.      Should enough other COFF targets add support for DWARF2,      a place will have to be found.  Until then, this hack will do.  */
if|if
condition|(
name|CONST_STRNEQ
argument_list|(
...
begin_comment
comment|/* FUNCTION 	bfd_copy_private_header_data  SYNOPSIS 	bfd_boolean bfd_copy_private_header_data (bfd *ibfd, bfd *obfd);  DESCRIPTION 	Copy private BFD header information from the BFD @var{ibfd} to the 	the BFD @var{obfd}.  This copies information that may require 	sections to exist, but does not require symbol tables.  Return<<true>> on success,<<false>> on error. 	Possible error returns are:  	o<<bfd_error_no_memory>> - 	Not enough memory exists to create private data for @var{obfd}.  .#define bfd_copy_private_header_data(ibfd, obfd) \ .     BFD_SEND (obfd, _bfd_copy_private_header_data, \ .		(ibfd, obfd))  */
end_comment
...
begin_comment
comment|/* CODE_FRAGMENT  .struct bfd_preserve .{ .  void *marker; .  void *tdata; .  flagword flags; .  const struct bfd_arch_info *arch_info; .  struct bfd_section *sections; .  struct bfd_section *section_last; .  unsigned int section_count; .  struct bfd_hash_table section_htab; .}; . */
end_comment
...
begin_comment
comment|/* FUNCTION 	bfd_emul_get_maxpagesize  SYNOPSIS  	bfd_vma bfd_emul_get_maxpagesize (const char *);  DESCRIPTION 	Returns the maximum page size, in bytes, as determined by 	emulation.  RETURNS 	Returns the maximum page size in bytes for ELF, abort 	otherwise. */
end_comment
...
begin_comment
comment|/* FUNCTION 	bfd_emul_set_maxpagesize  SYNOPSIS  	void bfd_emul_set_maxpagesize (const char *, bfd_vma);  DESCRIPTION 	For ELF, set the maximum page size for the emulation.  It is 	a no-op for other formats.  */
end_comment
...
begin_comment
comment|/* FUNCTION 	bfd_emul_get_commonpagesize  SYNOPSIS  	bfd_vma bfd_emul_get_commonpagesize (const char *);  DESCRIPTION 	Returns the common page size, in bytes, as determined by 	emulation.  RETURNS 	Returns the common page size in bytes for ELF, abort otherwise. */
end_comment
...
begin_comment
comment|/* FUNCTION 	bfd_emul_set_commonpagesize  SYNOPSIS  	void bfd_emul_set_commonpagesize (const char *, bfd_vma);  DESCRIPTION 	For ELF, set the common page size for the emulation.  It is 	a no-op for other formats.  */
end_comment
...
begin_comment
comment|/* FUNCTION 	bfd_demangle  SYNOPSIS 	char *bfd_demangle (bfd *, const char *, int);  DESCRIPTION 	Wrapper around cplus_demangle.  Strips leading underscores and 	other such chars that would otherwise confuse the demangler. 	If passed a g++ v3 ABI mangled name, returns a buffer allocated 	with malloc holding the demangled name.  Returns NULL otherwise 	and on memory alloc failure. */
end_comment
...
name|pre
expr_stmt|;
comment|/* Strip off @plt and suchlike too.  */
name|alloc
operator|=
name|NULL
expr_stmt|;
...
name|NULL
return|;
comment|/* Put back any prefix or suffix.  */
if|if
condition|(
name|pre_len
operator|!=
