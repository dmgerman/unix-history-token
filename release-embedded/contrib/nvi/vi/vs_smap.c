...
index|[]
init|=
literal|"$Id: vs_smap.c,v 10.31 2011/02/26 13:56:21 skimo Exp $"
decl_stmt|;
end_decl_stmt
...
block|{
comment|/* 			 * If number of lines HMAP->lno (top line) spans 			 * changed due to, say reformatting, and now is 			 * fewer than HMAP->soff, reset so the line is 			 * redrawn at the top of the screen. 			 */
name|cnt
operator|=
name|vs_screens
argument_list|(
...
name|gp
expr_stmt|;
comment|/* If the screen is vertically split, we can't scroll it. */
if|if
condition|(
name|IS_VSPLIT
argument_list|(
