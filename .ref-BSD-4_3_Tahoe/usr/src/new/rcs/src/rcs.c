...
index|[]
init|=
literal|"$Header: /usr/src/local/bin/rcs/src/RCS/rcs.c,v 4.7 87/12/18 11:37:17 narten Exp $ Purdue CS"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* filename for fcopy and fedit     */
end_comment
...
name|which
decl_stmt|;
comment|/*   Function:  remove lock to revision which->revno form newlocklst   */
block|{
name|struct
name|Lockrev
modifier|*
...
name|delta
decl_stmt|;
comment|/* function: Finds the lock held by who on delta,  * removes it, and returns a pointer to the delta.  * Sends mail if a lock different from the caller's is broken.  * Prints an error message and returns nil if there is no such lock or error.  */
block|{
specifier|register
name|struct
name|lock
