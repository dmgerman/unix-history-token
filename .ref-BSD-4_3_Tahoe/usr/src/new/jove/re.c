...
name|defchar
goto|;
comment|/* The * operator applies only to the previous 			   character.  If we were building a chr_cnt at 			   the time we got the *, we have to remove the 			   last character from the chr_cnt (by decrementing 			   *chr_cnt) and replacing it with a new STAR entry.  			   If we are decrementing the count to 0, we just 			   delete the chr_cnt entry altogether, replacing 			   it with the STAR entry. */
if|if
condition|(
name|chr_cnt
condition|)
