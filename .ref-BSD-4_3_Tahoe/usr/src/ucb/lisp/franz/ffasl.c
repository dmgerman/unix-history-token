...
name|rcsid
init|=
literal|"$Header: ffasl.c,v 1.11 87/12/14 18:48:06 sklower Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*					-[Mon Mar 21 19:37:21 1983 by jkf]-  * 	ffasl.c				$Locker:  $  * dynamically load C code  *  * (c) copyright 1982, Regents of the University of California  */
end_comment
...
name|cbuf
argument_list|,
literal|"%s/nld -N -x -A %s -T %x %s -e %s -o %s %s -lc"
argument_list|,
name|Ilibdir
argument_list|()
argument_list|,
...
operator|)
return|;
comment|/* 			 *	Try Xargv[0] with ".exe" concatenated 			 */
name|strcpy
argument_list|(
name|myname
argument_list|,
...
directive|else
comment|/* 			 *	Also try ".stb" and ".exe" in VMS 			 */
if|if
condition|(
operator|!
name|oktox
...
return|;
block|}
comment|/* one last try for dual systems */
name|strcpy
argument_list|(
name|myname
argument_list|,
...
parameter_list|()
block|{
comment|/*if(mypid==0) mypid = (getpid()& 0xffff); 	  fails if you do a dumplisp after doing a 	  cfasl */
name|sprintf
argument_list|(
name|mybuff
argument_list|,
...
comment|/* 		 *	Under VMS/EUNICE we have to try the 1st 512 byte 		 *	block and the 2nd 512 byte block (there may be 		 *	a VMS header in the 1st 512 bytes). 		 */
name|get_aout_header
argument_list|(
name|fileno
argument_list|(
name|f
...
name|error
argument_list|(
literal|"cfasl or load: lisp-library-directory not bound to string or atom"
argument_list|,
name|TRUE
argument_list|)
expr_stmt|;
