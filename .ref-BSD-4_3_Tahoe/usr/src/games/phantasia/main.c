...
literal|27
argument_list|,
literal|"Sorry, you can't play now.\n"
argument_list|)
expr_stmt|;
name|cleanup
argument_list|(
...
begin_comment
comment|/* used for counting users on system */
end_comment

begin_comment
comment|/************************************************************************ / / FUNCTION NAME: ok_to_play() / / FUNCTION: indicate whether playing is allowed / / AUTHOR: E. A. Estes, 12/4/85 / / ARGUMENTS: none / / RETURN VALUE: /	FALSE if playing is not allowed /	TRUE if playing is allowed / / MODULES CALLED: time(), fread(), fopen(), fclose(), localtime() / / GLOBAL INPUTS: Wizard, Okcount / / GLOBAL OUTPUTS: Okcount / / DESCRIPTION:  /	This function is provided to allow one to restrict access to the game. /	Tailor this routine as appropriate. /	Return FALSE if playing is not allowed at this time. / /************************************************************************/
end_comment
...
literal|0
decl_stmt|;
comment|/* number of users on system */
name|FILE
modifier|*
name|fp
decl_stmt|;
comment|/* to open files */
name|long
name|now
decl_stmt|;
...
name|ubuf
decl_stmt|;
comment|/* to read 'utmp' file */
if|if
condition|(
name|Wizard
condition|)
comment|/* wizard can always play */
return|return
operator|(
name|TRUE
operator|)
...
literal|0
condition|)
comment|/* only check every 5 times */
return|return
operator|(
name|TRUE
operator|)
return|;
comment|/* check time of day */
name|time
argument_list|(
operator|&
name|now
...
operator|)
condition|)
comment|/* not a weekend */
return|return
operator|(
name|FALSE
operator|)
return|;
comment|/* check # of users */
if|if
condition|(
operator|(
name|fp
