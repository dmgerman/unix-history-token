...
name|zdmap
expr_stmt|;
comment|/* 	 * make page-out daemon (process 2) 	 * the daemon has ctopt(NSWBUF*CLSIZE) pages of page 	 * table so that it can map dirty pages into 	 * its address space during asychronuous pushes. 	 */
name|mpid
operator|=
literal|1
expr_stmt|;
...
begin_comment
comment|/*  * Initialize the buffer I/O system by freeing  * all buffers and setting all device buffer lists to empty.  *  * MUST USE MEMALL HERE!!!  */
end_comment
