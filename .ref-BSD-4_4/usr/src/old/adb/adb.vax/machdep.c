...
index|[]
init|=
literal|"@(#)machdep.c	5.4 (Berkeley) 4/4/91"
decl_stmt|;
end_decl_stmt
...
name|fr
decl_stmt|;
comment|/* 	 * The magic constants below allow us to read just the part of 	 * the frame that we need. 	 */
if|if
condition|(
name|adbread
argument_list|(
...
begin_comment
comment|/*  * Check a float for `correctness' (reserved patterns, etc).  Return  * a pointer to a character string to be printed instead of the float,  * or NULL to print the float as-is.  *  * The string returned, if any, should be no longer than 16 characters.  *  * On the VAX, we can simply check the first two bytes.  Byte zero  * contains one bit of the exponent, and byte 1 has the remaining 7  * exponent bits and the sign bit.  If the sign bit is set and the  * exponent is zero, the value is reserved.  */
end_comment
...
name|adbprintf
argument_list|(
literal|"p0br %R p0lr %R p1br %R p1lr %R\n"
argument_list|,
name|pcb
operator|.
name|pcb_p0br
...
begin_comment
comment|/*  * Convert a kernel virtual address to a physical address,  * a la the VAX hardware.  Set *err if the resulting address  * is invalid.  */
end_comment
...
condition|)
block|{
comment|/* P1 space: must not be in shadow region */
if|if
condition|(
name|v
operator|<
...
return|;
block|}
comment|/* in either case, find system pte by recursing */
name|pteaddr
operator|=
name|vtophys
argument_list|(
...
name|s
decl_stmt|;
comment|/*	addr_t callpc;		/* pc that called this frame */
name|struct
name|activation
name|cur
decl_stmt|;
...
name|narg
decl_stmt|;
comment|/* number of int-args to this frame */
name|addr_t
name|dummy
decl_stmt|;
...
directive|endif
comment|/* fr_savpc==UNKNOWN implies fr is invalid */
name|fr
operator|.
name|fr_savpc
operator|=
...
condition|)
block|{
comment|/* was a `calls'; can figure out ap */
name|cur
operator|.
name|a_ap
operator|=
...
condition|)
block|{
comment|/*			GET(callpc, cur.a_fp + 92);	/* XXX magic 92 */
name|s
operator|=
literal|"sigtramp"
expr_stmt|;
...
name|UNKNOWN
expr_stmt|;
comment|/* until we read it again */
if|if
condition|(
operator|!
name|gavedot
