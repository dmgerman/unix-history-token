...
begin_comment
comment|/* Fundamental definitions for GNU Emacs Lisp interpreter.    Copyright (C) 1985, 1986, 1987 Free Software Foundation, Inc.  This file is part of GNU Emacs.  GNU Emacs is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GNU Emacs is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU Emacs; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|Lisp_Objfwd
block|,
comment|/* was Lisp_Internal */
name|Lisp_Object_Unused_2
block|,
...
name|Lisp_Window
block|,
comment|/* Used by save,set,restore-window-configuration */
...
begin_comment
comment|/* One need to override this if there must be high bits set in data space    (doing the result of the below& ((1<< (GCTYPE + 1)) - 1) would work     on all machines, but would penalise machines which don't need it)  */
end_comment
...
begin_comment
comment|/* Extract the value of a Lisp_Object as a signed integer.  */
end_comment
...
begin_comment
comment|/* Some machines need to do this differently.  */
end_comment
...
begin_comment
comment|/* Extract the value as an unsigned integer.  This is a basis    for exctacting it as a pointer to a structure in storage.  */
end_comment
...
begin_comment
comment|/* In this representation, data is found in two widely separated segments.  */
end_comment
...
parameter_list|)
define|\
value|(XUINT (a) | (XUINT (a)> PURESIZE ? DATA_SEG_BITS : PURE_SEG_BITS))
end_define
...
begin_comment
comment|/* This case is used for the rt-pc.    In the diffs I was given, it checked for ptr = 0    and did not adjust it in that case.    But I don't think that zero should ever be found    in a Lisp object whose data type says it points to something.  */
end_comment
...
begin_comment
comment|/* In version 19, try  #if VALBITS + GCTYPEBITS == INTBITS - 1 #define XMARKBIT(a) ((a)< 0) #define XSETMARKBIT(a,b) ((a) = ((a)& ~MARKBIT) | ((b) ? MARKBIT : 0)) */
end_comment
...
name|a
parameter_list|)
value|((struct Lisp_Cons *) XPNTR(a))
end_define
...
name|a
parameter_list|)
value|((struct Lisp_Vector *) XPNTR(a))
end_define
...
name|a
parameter_list|)
value|((struct Lisp_Subr *) XPNTR(a))
end_define
...
name|a
parameter_list|)
value|((struct Lisp_String *) XPNTR(a))
end_define
...
name|a
parameter_list|)
value|((struct Lisp_Symbol *) XPNTR(a))
end_define
...
name|a
parameter_list|)
value|((struct Lisp_Marker *) XPNTR(a))
end_define
...
name|a
parameter_list|)
value|((struct Lisp_Process *) XPNTR(a))
end_define
...
begin_comment
comment|/* #define LISTP(x) (XTYPE ((x)) == Lisp_Cons)*/
end_comment
...
parameter_list|)
define|\
value|{ if ((XTYPE ((x)) != Lisp_Cons)&& !NULL (x)) x = wrong_type_argument (Qlistp, (x)); }
end_define
...
parameter_list|)
define|\
value|{ if (XTYPE ((x)) != Lisp_Cons) x = wrong_type_argument (Qconsp, (x)); }
end_define
...
parameter_list|)
define|\
value|{ extern int pure[]; \     if ((PNTR_COMPARISON_TYPE) XPNTR (obj)< (PNTR_COMPARISON_TYPE) ((char *) pure + PURESIZE) \&& (PNTR_COMPARISON_TYPE) XPNTR (obj)>= (PNTR_COMPARISON_TYPE) pure) \       pure_write_error (); }
end_define
...
begin_comment
comment|/* when PNTR_COMPARISON_TYPE is not the default (unsigned int) */
end_comment
...
parameter_list|)
define|\
value|{ extern int my_edata; \     if ((PNTR_COMPARISON_TYPE) XPNTR (obj)< (PNTR_COMPARISON_TYPE)&my_edata) \       pure_write_error (); }
end_define
...
begin_comment
comment|/* not VIRT_ADDRESS_VARIES, not PNTR_COMPARISON_TYPE */
end_comment
...
parameter_list|)
define|\
value|{ extern int my_edata; \     if (XPNTR (obj)< (unsigned int)&my_edata) \       pure_write_error (); }
end_define
...
begin_comment
comment|/* Cast pointers to this type to compare them.  Some machines want int.  */
end_comment
...
begin_comment
comment|/* Macros we use to define forwarded Lisp variables.    These are used in the syms_of_FILENAME functions.  */
end_comment
...
name|doc
parameter_list|)
value|defvar_int (lname, vname)
end_define

begin_define
define|#
directive|define
name|DEFVARPERBUFFER
parameter_list|(
name|lname
parameter_list|,
name|vname
...
parameter_list|)
define|\
value|defvar_per_buffer (lname, vname)
end_define
...
name|doc
parameter_list|)
value|defvar_lisp_nopro (lname, vname)
end_define
...
name|unused
decl_stmt|;
comment|/* Dividing by 16 is faster than by 12 */
block|}
struct|;
end_struct
...
decl_stmt|;
comment|/* No error should exit a piece of code 				   in which polling is suppressed.  */
name|struct
name|catchtag
modifier|*
name|tag
...
begin_comment
comment|/* Nonzero if ought to quit now.  */
end_comment
...
directive|define
name|QUITP
value|(!NULL (Vquit_flag)&& NULL (Vinhibit_quit))
end_define
...
name|CH
parameter_list|)
value|(downcase_table[CH] != (CH))
end_define

begin_comment
comment|/* 1 if CH is lower case.  */
end_comment
...
parameter_list|)
define|\
value|(downcase_table[CH] == (CH)&& downcase_table[0400 + (CH)] != (CH))
end_define

begin_comment
comment|/* 1 if CH is neither upper nor lower case.  */
end_comment
...
name|CH
parameter_list|)
value|(downcase_table[0400 + (CH)] == (CH))
end_define

begin_comment
comment|/* Upcase a character, or make no change if that cannot be done.  */
end_comment
...
name|CH
parameter_list|)
value|(downcase_table[CH] == (CH) ? UPCASE1 (CH) : (CH))
end_define

begin_comment
comment|/* Upcase a character known to be not upper case.  */
end_comment
...
name|CH
parameter_list|)
value|downcase_table[0400 + (CH)]
end_define

begin_comment
comment|/* Downcase a character, or make no change if that cannot be done. */
end_comment
...
begin_comment
comment|/* Nonzero means don't do use window-system-specific display code */
end_comment
...
begin_comment
comment|/* extern void current_environ (); */
end_comment
...
begin_comment
comment|/* Nonzero means Emacs has already been initialized.    Used during startup to detect startup of dumped Emacs.  */
end_comment
