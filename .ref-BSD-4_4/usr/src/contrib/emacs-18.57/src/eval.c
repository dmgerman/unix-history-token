...
begin_comment
comment|/* Evaluator for GNU Emacs Lisp interpreter.    Copyright (C) 1985, 1986, 1987, 1990 Free Software Foundation, Inc.  This file is part of GNU Emacs.  GNU Emacs is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GNU Emacs is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU Emacs; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* Skip the frame of interactive-p itself (if interpreted)      or the frame of byte-code (if called from compiled function).  */
end_comment
...
literal|0
argument_list|,
literal|"(defconst SYMBOL INITVALUE DOCSTRING) defines SYMBOL as a constant variable.\n\ The intent is that programs do not change this value (but users may).\n\ Always sets the value of SYMBOL to the result of evalling INITVALUE.\n\ DOCSTRING is optional.\n\ If DOCSTRING starts with *, this variable is identified as a user option.\n\  This means that M-x set-variable and M-x edit-options recognize it."
argument_list|)
end_macro
...
begin_comment
comment|/* Set up a catch, then call C function FUNC on argument ARG.    FUNC should return a Lisp_Object.    This is how catches are done from within C code. */
end_comment
...
begin_block
block|{
comment|/* This structure is made part of the chain `catchlist'.  */
name|struct
name|catchtag
name|c
decl_stmt|;
comment|/* Fill in the components of c, and put it on the list.  */
name|c
operator|.
name|next
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Throw works by a longjmp that comes right here.  */
name|catchlist
operator|=
name|c
operator|.
...
condition|)
block|{
comment|/* Restore the polling-suppression count.  */
if|if
condition|(
name|c
operator|->
...
begin_comment
comment|/* Chain of condition handlers currently in effect.    The elements of this chain are contained in the stack frames    of Fcondition_case and internal_condition_case.    When an error is signaled (by calling Fsignal, below),    this chain is searched for an element that applies.  */
end_comment
...
name|error
argument_list|(
literal|"Invalid condition handler"
argument_list|,
name|tem
argument_list|)
expr_stmt|;
...
name|allhandlers
expr_stmt|;
comment|/* If no handler is present now, try to run the debugger,      and if that fails, throw to top level.  */
name|find_handler_clause
argument_list|(
name|Qerror
argument_list|,
...
literal|0
argument_list|,
literal|"Call FUNCTION, passing remaining arguments to it.  The last argument\n\ is a list of arguments to pass.\n\ Thus, (apply '+ 1 2 '(3 4)) returns 10."
argument_list|)
end_macro
...
condition|)
block|{
comment|/* Let funcall get the error */
name|fun
operator|=
name|args
index|[
...
condition|)
block|{
comment|/* Avoid making funcall cons up a yet another new vector of arguments 	   by explicitly supplying nil's for optional values */
name|funcall_args
operator|=
operator|(
name|Lisp_Object
...
name|funcall
label|:
comment|/* We add 1 to numargs because funcall_args includes the      function itself as well as its arguments.  */
if|if
condition|(
operator|!
name|funcall_args
...
argument_list|)
expr_stmt|;
comment|/* Spread the last arg we got.  Its first element goes in      the slot that it used to occupy, hence this value of I.  */
name|i
operator|=
name|nargs
operator|-
...
begin_comment
comment|/* Call function fn on no arguments */
end_comment
...
begin_comment
comment|/* Get the value of symbol's global binding, even if that binding  is not now dynamically visible.  */
end_comment

begin_endif
unit|Lisp_Object top_level_value (symbol)      Lisp_Object symbol; {   register struct specbinding *ptr = specpdl;    CHECK_SYMBOL (symbol, 0);   for (; ptr != specpdl_ptr; ptr++)     {       if (EQ (ptr->symbol, symbol)) 	return ptr->old_value;     }   return Fsymbol_value (symbol); }  Lisp_Object top_level_set (symbol, newval)      Lisp_Object symbol, newval; {   register struct specbinding *ptr = specpdl;    CHECK_SYMBOL (symbol, 0);   for (; ptr != specpdl_ptr; ptr++)     {       if (EQ (ptr->symbol, symbol)) 	{ 	  ptr->old_value = newval; 	  return newval; 	}     }   return Fset (symbol, newval); }
endif|#
directive|endif
end_endif
