...
begin_comment
comment|/* non-zero if inside `load' */
end_comment
...
name|c
decl_stmt|;
comment|/* This used to be return of a conditional expression, 	 but that truncated -1 to a char on VMS.  */
if|if
condition|(
name|read_from_string_index
operator|<
...
argument_list|)
condition|)
comment|/* If there are non-absolute elts in PATH (eg ".") */
comment|/* Of course, this could conceivably lose if luser sets 	   default-directory to be something non-absolute... */
block|{
name|filename
operator|=
name|Fexpand_file_name
...
argument_list|)
condition|)
comment|/* Give up on this path element! */
continue|continue;
block|}
comment|/* Calculate maximum size of any filename made from 	 this path element/specified file name and any possible suffix.  */
name|want_size
operator|=
name|strlen
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* Concatenate path element/specified name with the suffix.  */
name|strncpy
argument_list|(
name|fn
argument_list|,
...
literal|0
condition|)
comment|/* Bug happens on CCI if lsuffix is 0.  */
name|strncat
argument_list|(
name|fn
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Ignore file if it's a directory.  */
if|if
condition|(
name|stat
argument_list|(
...
condition|)
block|{
comment|/* Check that we can access or open it.  */
if|if
condition|(
name|exec_only
condition|)
...
condition|)
block|{
comment|/* We succeeded; return this descriptor and filename.  */
if|if
condition|(
name|storeptr
condition|)
...
block|}
block|}
comment|/* Advance to next suffix.  */
if|if
condition|(
name|esuffix
operator|==
...
argument_list|)
expr_stmt|;
comment|/* This both uses b and checks its type.  */
name|Fgoto_char
argument_list|(
name|b
argument_list|)
...
name|read_buffer
expr_stmt|;
comment|/*	      end = read_buffer + read_buffer_size;  */
block|}
operator|*
name|p
operator|=
...
condition|)
block|{
comment|/* If Vobarray is now invalid, force it to be valid.  */
if|if
condition|(
name|EQ
argument_list|(
...
expr_stmt|;
block|}
comment|/* Combining next two lines breaks VMS C 2.3.  */
name|hash
operator|=
name|hash_string
argument_list|(
...
name|error
argument_list|(
literal|"Bad data in guts of obarray"
argument_list|)
expr_stmt|;
comment|/* Like CADR error message */
else|else
for|for
control|(
name|tail
...
argument_list|)
expr_stmt|;
comment|/* Separate statement here to avoid VAXC bug. */
name|hash
operator|%=
name|OBARRAY_SIZE
expr_stmt|;
...
name|Qt
expr_stmt|;
comment|/* Qt is correct even if CANNOT_DUMP.  loadup.el will set to nil at end.  */
name|Vpurify_flag
operator|=
name|Qt
expr_stmt|;
...
begin_comment
comment|/* use fset in subr.el now */
end_comment
...
begin_comment
comment|/* New replacement for DefIntVar; it ignores the doc string argument    on the assumption that make-docfile will handle that.  */
end_comment

begin_comment
comment|/* Define an "integer variable"; a symbol whose value is forwarded  to a C variable of type int.  Sample call: */
end_comment

begin_comment
comment|/* DEFVARINT ("indent-tabs-mode",&indent_tabs_mode, "Documentation");  */
end_comment
...
begin_comment
comment|/* Similar but don't request gc-marking of the C variable.    Used when that variable will be gc-marked for some other reason,    since marking the same slot twice can cause trouble with strings.  */
end_comment
...
literal|0
condition|)
comment|/* Did a DEFVAR_PER_BUFFER without initializing the corresponding        slot of buffer_local_flags */
name|abort
argument_list|()
expr_stmt|;
block|}
...
name|normal_path
decl_stmt|;
comment|/* Warn if dirs in the *standard* path don't exist.  */
name|normal_path
operator|=
...
name|printf
argument_list|(
literal|"Warning: lisp library (%s) does not exist.\n"
argument_list|,
name|XSTRING
argument_list|(
name|Fcar
