...
begin_comment
comment|/* SUPPRESS 701 on free */
end_comment

begin_comment
comment|/* Conflicting declaration */
end_comment
...
begin_comment
comment|/*  * ANSI C says an offset into a file is a long, not an off_t, for some  * reason.  This actually does simplify life a bit, but it's still nice  * to have a distinctive name for it.  Beware, this is just for readability,  * don't try to change this.  */
end_comment
...
begin_comment
comment|/*  * We assume that unused areas of a binary file are zeros, and that the  * bit pattern of `(of_t)0' is all zeros.  The alternative is rather  * painful file initialization.  Note that okayvalue(), if OVERFLOW is  * defined, knows what value of an offset would cause overflow.  */
end_comment
...
begin_comment
comment|/* make any valid of_t non-VACANT */
end_comment
...
begin_comment
comment|/*  * In a Unix implementation, or indeed any in which an of_t is a byte  * count, there are a bunch of high bits free in an of_t.  There is a  * use for them.  Checking a possible hit by looking it up in the base  * file is relatively expensive, and the cost can be dramatically reduced  * by using some of those high bits to tag the value with a few more bits  * of the key's hash.  This detects most false hits without the overhead of  * seek+read+strcmp.  We use the top bit to indicate whether the value is  * tagged or not, and don't tag a value which is using the tag bits itself.  * We're in trouble if the of_t representation wants to use the top bit.  * The actual bitmasks and offset come from the configuration stuff,  * which permits fiddling with them as necessary, and also suppressing  * them completely (by defining the masks to 0).  We build pre-shifted  * versions of the masks for efficiency.  */
end_comment
...
begin_comment
comment|/*  * Using mmap() is a more efficent way of keeping the .pag file incore.  On  * average, it cuts the number of system calls and buffer copies in half.  * It also allows one copy to be shared among many processes without  * consuming any extra resources.  */
end_comment
...
begin_comment
comment|/*  * Write to filesystem even if incore?  This replaces a single multi-  * megabyte write when doing a dbzsync with a multi-byte write each  * time an article is added.  On most systems, this will give an overall  * performance boost.  */
end_comment
...
name|args
parameter_list|)
value|if (debug) { (void) printf args ; } else
end_define
...
begin_comment
unit|extern char *memcpy(); extern char *memchr(); extern char *malloc(); extern char *calloc(); extern void free();
...
return|;
block|}
comment|/* calculate tagging from old file */
if|if
condition|(
name|stat
argument_list|(
...
control|)
continue|continue;
comment|/* if we had more tags than the default, use the new data */
if|if
condition|(
operator|(
name|c
...
argument_list|(
operator|(
literal|"dbmclose: munmap failed\n"
operator|)
argument_list|)
expr_stmt|;
name|ret
...
begin_comment
comment|/*  - dbzwritethrough - write through the pag file in core  */
end_comment
...
begin_comment
comment|/*  - dbztagmask - calculate the correct tagmask for the given base file size  */
end_comment
...
argument_list|(
operator|(
literal|"getconf: wrong of_t size (%d)\n"
operator|,
name|cp
operator|->
name|valuesize
...
condition|)
block|{
comment|/* file too small; extend it */
if|if
condition|(
name|ftruncate
argument_list|(
...
argument_list|(
operator|(
literal|"getcore: ftruncate failed\n"
operator|)
argument_list|)
expr_stmt|;
return|return
...
directive|ifdef
name|MC_ADVISE
comment|/* not present in all versions of mmap() */
name|madvise
argument_list|(
name|it
argument_list|,
...
begin_comment
comment|/*  - mybytemap - determine this machine's byte map  *  * A byte map is an array of ints, sizeof(of_t) of them.  The 0th int  * is the byte number of the high-order byte in my of_t, and so forth.  */
end_comment
...
comment|/* Retrieving char where long is stored */
if|if
condition|(
name|u
operator|.
...
begin_comment
comment|/*  - bytemap - transform an of_t from byte ordering map1 to map2  */
end_comment
