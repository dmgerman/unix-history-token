...
begin_comment
comment|/* Optimize by combining instructions for GNU compiler.    Copyright (C) 1987, 1988, 1992 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|HOST_WIDE_INT
modifier|*
name|reg_significant
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Mode used to compute significance in reg_significant.  It is the largest    integer mode that can fit in HOST_BITS_PER_WIDE_INT.  */
end_comment
...
begin_comment
comment|/* Nonzero when reg_significant and reg_sign_bit_copies can be safely used.    It is zero while computing them and after combine has completed.  This    former test prevents propagating values based on previously set values,    which can be incorrect if a variable is modified in a loop.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|significant_valid
decl_stmt|;
end_decl_stmt
...
parameter_list|)
define|\
value|do { rtx _new = (NEWVAL);						\       if (undobuf.num_undo< MAX_UNDO)					\ 	{								\ 	  undobuf.undo[undobuf.num_undo].is_int = 0;			\ 	  undobuf.undo[undobuf.num_undo].where.rtx =&INTO;		\ 	  undobuf.undo[undobuf.num_undo].old_contents.rtx = INTO;	\ 	  INTO = _new;							\ 	  if (undobuf.undo[undobuf.num_undo].old_contents.rtx != INTO)	\ 	    undobuf.num_undo++; 					\ 	}								\     } while (0)
end_define

begin_comment
comment|/* Similar to SUBST, but NEWVAL is an int.  INTO will normally be an XINT    expression.    Note that substitution for the value of a CONST_INT is not safe.  */
end_comment
...
parameter_list|)
define|\
value|do { if (undobuf.num_undo< MAX_UNDO)					\ {									\ 	  undobuf.undo[undobuf.num_undo].is_int = 1;			\ 	  undobuf.undo[undobuf.num_undo].where.i = (int *)&INTO;	\ 	  undobuf.undo[undobuf.num_undo].old_contents.i = INTO;		\ 	  INTO = NEWVAL;						\ 	  if (undobuf.undo[undobuf.num_undo].old_contents.i != INTO)	\ 	    undobuf.num_undo++;						\ 	}								\      } while (0)
end_define
...
argument_list|)
expr_stmt|;
comment|/* Don't use reg_significant when computing it.  This can cause problems      when, for example, we have j<<= 1 in a loop.  */
name|significant_valid
operator|=
literal|0
expr_stmt|;
comment|/* Compute the mapping from uids to cuids.      Cuids are numbers assigned to insns, like uids,      except that cuids increase monotonically through the code.        Scan all SETs and see if we can deduce anything about what      bits are significant for some registers.  */
for|for
control|(
name|insn
operator|=
...
comment|/* Don't install a subreg involving two modes not tieable. 	 It can worsen register allocation, and can even make invalid reload 	 insns, since the reg inside may need to be copied from in the 	 outside mode, and that may be invalid if it is an fp reg copied in 	 integer mode.  As a special exception, we can allow this if 	 I3 is simply copying DEST, a REG,  to CC0.  */
operator|||
operator|(
name|GET_CODE
argument_list|(
...
comment|/* Don't extend the life of a hard register.  */
operator|||
name|REGNO
argument_list|(
name|src
...
literal|0
return|;
comment|/* If INSN contains anything volatile, or is an `asm' (whether volatile      or not), reject, unless nothing volatile comes between it and I3,      with the exception of SUCC.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
operator|)
operator|)
comment|/* This is the same test done in can_combine_p except that we 	     allow a hard register with SMALL_REGISTER_CLASSES if SRC is a 	     CALL operation.  */
operator|||
operator|(
name|GET_CODE
argument_list|(
...
literal|0
return|;
comment|/* If DEST is used in I3, it is being killed in this insn, 	 so record that for later.  */
if|if
condition|(
name|pi3dest_killed
operator|&&
...
decl_stmt|;
block|{
comment|/* New patterns for I3 and I3, respectively.  */
name|rtx
name|newpat
decl_stmt|,
name|newi2pat
...
name|i
decl_stmt|;
comment|/* If any of I1, I2, and I3 isn't really an insn, we can't do anything.      This can occur when flow deletes an insn that it has merged into an      auto-increment address.  We also can't do anything if I3 has a      REG_LIBCALL note since we don't want to disrupt the contiguity of a      libcall.  */
if|if
condition|(
name|GET_RTX_CLASS
argument_list|(
...
literal|0
expr_stmt|;
comment|/* Save the current high-water-mark so we can free storage if we didn't      accept this combination.  */
name|undobuf
operator|.
name|storage
operator|=
...
expr_stmt|;
block|}
comment|/* See if this is an XOR.  If so, perhaps the problem is that the      constant is out of range.  Replace it with a complemented XOR with      a complemented constant; it might be in range.  */
elseif|else
if|if
condition|(
name|insn_code_number
...
argument_list|)
expr_stmt|;
comment|/* If I2 or I3 has multiple SETs, we won't know how to track 	     register status, so don't use these insns.  */
if|if
condition|(
name|i2_code_number
operator|>=
...
block|}
block|}
comment|/* Check for a case where we loaded from memory in a narrow mode and      then sign extended it, but we need both registers.  In that case,      we have a PARALLEL with both loads from the same memory location.      We can split this into a load from memory followed by a register-register      copy.  This saves at least one insn, more if register allocation can      eliminate the copy.  */
elseif|else
if|if
condition|(
name|i1
...
name|regno
decl_stmt|;
comment|/* Compute which registers we expect to eliminate.  */
name|rtx
name|elim_i2
init|=
operator|(
...
name|other_code_number
expr_stmt|;
comment|/* We had one special case above where I2 had more than one set and        we replaced a destination of one of those sets with the destination        of I3.  In that case, we have to update LOG_LINKS of insns later        in this basic block.  Note that this (expensive) case is rare.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* If I3DEST was used in I3SRC, it really died in I3.  We may need to        put a REG_DEAD note for it somewhere.  Similarly for I2 and I1.        Show an additional death due to the REG_DEAD note we make here.  If        we discard it in distribute_notes, we will decrement it again.  */
if|if
condition|(
name|i3dest_killed
condition|)
...
expr_stmt|;
block|}
comment|/* For I2 and I1, we have to be careful.  If NEWI2PAT exists and sets        I2DEST or I1DEST, the death must be somewhere before I2, not I3.  If        we passed I3 in that case, it might delete I2.  */
if|if
condition|(
name|i2dest_in_i2src
condition|)
...
block|}
block|}
comment|/* Update reg_significant et al for any changes that may have been made        to this insn.  */
name|note_stores
argument_list|(
name|newpat
argument_list|,
...
directive|endif
comment|/* If we have a PLUS whose second operand is a constant and the 	 address is not valid, perhaps will can split it up using 	 the machine-specific way to split large constants.  We use 	 the first psuedo-reg (one of the virtual regs) as a placeholder; 	 it will not remain in the result.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
parameter_list|)
define|\
value|(GET_MODE_SIZE (MODE)<= UNITS_PER_WORD)
...
block|}
block|}
comment|/* We come back to here if we have replaced the expression with one of      a different code and it is likely that further simplification will be      possible.  */
name|restart
label|:
comment|/* If we have restarted more than 4 times, we are probably looping, so      give up.  */
if|if
condition|(
operator|++
name|n_restarts
...
name|x
return|;
comment|/* If we are restarting at all, it means that we no longer know the      original mode of operand 0 (since we have probably changed the      form of X).  */
if|if
condition|(
name|n_restarts
operator|>
...
expr_stmt|;
block|}
comment|/* If this is a simple operation applied to an IF_THEN_ELSE, try       applying it to the arms of the IF_THEN_ELSE.  This often simplifies      things.  Don't deal with operations that change modes here.  */
if|if
condition|(
operator|(
name|GET_RTX_CLASS
...
condition|)
block|{
comment|/* Don't do this by using SUBST inside X since we might be messing 	 up a shared expression.  */
name|rtx
name|cond
init|=
name|XEXP
...
name|SUBREG
case|:
comment|/* (subreg:A (mem:B X) N) becomes a modified MEM unless the SUBREG 	 is paradoxical.  If we can't do that safely, then it becomes 	 something nonsensical so that this combination won't take place.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
return|;
block|}
comment|/* If we are in a SET_DEST, these other cases can't apply.  */
if|if
condition|(
name|in_dest
condition|)
...
expr_stmt|;
block|}
comment|/* SUBREG of a hard register => just change the register number 	 and/or mode.  If the hard register is not valid in that mode, 	 suppress this combination.  If the hard register is the stack, 	 frame, or argument pointer, leave this as a SUBREG.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
return|;
block|}
comment|/* For a constant, try to pick up the part we want.  Handle a full 	 word and low-order part.  Only do this if we are narrowing 	 the constant; if it is being widened, we have no idea what 	 the extra bits will have been set to.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
argument_list|)
return|;
comment|/* If we are narrowing the object, we need to see if we can simplify 	 the expression for the object knowing that we only need the 	 low-order bits.  */
if|if
condition|(
name|GET_MODE_SIZE
argument_list|(
...
operator|-
literal|1
comment|/* (not (comparison foo bar)) can be done by reversing the comparison 	 code if valid.  */
if|if
condition|(
name|GET_RTX_CLASS
argument_list|(
...
argument_list|)
return|;
comment|/* (plus (xor (and<foo> (const_int pow2 - 1))<c>)<-c>) 	 when c is (const_int (pow2 + 1) / 2) is a sign extension of a 	 bit-field and can be replaced by either a sign_extend or a 	 sign_extract.  The `and' may be a zero_extend.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
goto|;
block|}
comment|/* If only the low-order bit of X is significant, (plus x -1) 	 can become (ashiftrt (ashift (xor x 1) C) C) where C is 	 the bitsize of the mode - 1.  This allows simplification of 	 "a = (b& 8) == 0;"  */
if|if
condition|(
name|XEXP
argument_list|(
...
goto|;
block|}
comment|/* If this is multiplication by a power of two and its first operand is 	 a shift, treat the multiply as a shift to allow the shifts to 	 possibly combine.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
goto|;
block|}
comment|/* Convert (mult (ashift (const_int 1) A) B) to (ashift B A).  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
operator|==
literal|1
comment|/* If STORE_FLAG_VALUE is 1, we can convert (ne x 0) to simply X 	     if only the low-order bit is significant in X (such as when 	     X is a ZERO_EXTRACT of one bit.  Similarly, we can convert 	     EQ to (xor X 1).  */
if|if
condition|(
name|new_code
operator|==
...
operator|-
literal|1
comment|/* If STORE_FLAG_VALUE is -1, we can convert (ne x 0) 	     to (neg x) if only the low-order bit of X is significant. 	     This converts (ne (zero_extract X 1 Y) 0) to 	     (sign_extract X 1 Y).  */
if|if
condition|(
name|new_code
operator|==
...
directive|endif
comment|/* If STORE_FLAG_VALUE says to just test the sign bit and X has just 	     one significant bit, we can convert (ne x 0) to (ashift x c) 	     where C puts the bit in the sign bit.  Remove any AND with 	     STORE_FLAG_VALUE when we are done, since we are only going to 	     test the sign bit.  */
if|if
condition|(
name|new_code
operator|==
...
expr_stmt|;
block|}
comment|/* If we are comparing against zero and the expression being tested 	     has only a single significant bit, that is its value when it is  	     not equal to zero.  Similarly if it is known to be -1 or 0.  */
if|if
condition|(
name|true_code
operator|==
...
name|constm1_rtx
expr_stmt|;
comment|/* Now simplify an arm if we know the value of the register 	     in the branch and it is used in the arm.  Be carefull due to 	     the potential of locally-shared RTL.  */
if|if
condition|(
name|reg_mentioned_p
argument_list|(
...
expr_stmt|;
block|}
comment|/* If we have (if_then_else FOO (pc) (label_ref BAR)) and FOO can be 	 reversed, do so to avoid needing two sets of patterns for 	 subtract-and-branch insns.  Similarly if we have a constant in that 	 position or if the third operand is the same as the first operand 	 of the comparison.  */
if|if
condition|(
name|GET_RTX_CLASS
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If the only change we made was to change an EQ into an 		 NE or vice versa, OP0 has only one significant bit, 		 and OP1 is zero, check if changing the user of the condition 		 code will produce a valid insn.  If it won't, we can keep 		 the original code in that insn by surrounding our operation 		 with an XOR.  */
if|if
condition|(
operator|(
operator|(
...
goto|;
block|}
comment|/* In the following group of tests (and those in case IOR below), 	 we start with some combination of logical operations and apply 	 the distributive law followed by the inverse distributive law. 	 Most of the time, this results in no change.  However, if some of 	 the operands are the same or inverses of each other, simplifications 	 will result.  	 For example, (and (ior A B) (not B)) can occur as the result of 	 expanding a bit field assignment.  When we apply the distributive 	 law to this, we get (ior (and (A (not B))) (and (B (not B)))), 	 which then simplifies to (and (A (not B))).  */
comment|/* If we have (and (ior A B) C), apply the distributive law and then 	 the inverse distributive law to see if things simplify.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|IOR
case|:
comment|/* (ior A C) is C if all significant bits of A are on in C.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
operator|==
literal|1
comment|/* (xor (comparison foo bar) (const_int 1)) can become the reversed 	 comparison.  */
if|if
condition|(
name|XEXP
argument_list|(
...
literal|0
return|;
comment|/* Get the mode to use should INNER be a MEM, the mode for the position,      and the mode for the result.  */
ifdef|#
directive|ifdef
name|HAVE_insv
if|if
...
argument_list|)
expr_stmt|;
comment|/* If this is not from memory or we have to change the mode of memory and      cannot, the desired mode is EXTRACTION_MODE.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
directive|if
name|BITS_BIG_ENDIAN
comment|/* If position is constant, compute new position.  Otherwise, build      subtraction.  */
if|if
condition|(
name|pos
operator|>=
...
block|}
block|}
comment|/* If INNER is not memory, we can always get it into the proper mode. */
elseif|else
if|if
condition|(
name|GET_CODE
...
argument_list|)
expr_stmt|;
comment|/* Make POS_RTX unless we already have it and it is correct.  */
if|if
condition|(
name|pos_rtx
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Similarly if we have (ashifrt (OP (ashift foo C1) C3) C2).  In these 	 cases, we are better off returning a SIGN_EXTEND of the operation.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Similarly for (ashiftrt (neg (ashift FOO C1)) C2).  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Rewrite X so that it is an expression in MODE.  We only care about the    low-order BITS bits so we can ignore AND operations that just clear    higher-order bits.     Also, if REG is non-zero and X is a register equal in value to REG,     replace X with REG.  */
end_comment
...
name|mode
decl_stmt|;
comment|/* If X is narrower than MODE or if BITS is larger than the size of MODE,      just get X in the proper mode.  */
if|if
condition|(
name|GET_MODE_SIZE
argument_list|(
...
name|SUBREG
case|:
comment|/* Ignore low-order SUBREGs. */
if|if
condition|(
name|subreg_lowpart_p
argument_list|(
...
name|AND
case|:
comment|/* If this is an AND with a constant.  Otherwise, we fall through to 	 do the general binary case.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* If this is masking some low-order bits, we may be able to 	     impose a stricter constraint on what bits of the operand are 	     required.  */
name|op
operator|=
name|force_to_mode
argument_list|(
...
literal|1
expr_stmt|;
comment|/* If we have no AND in MODE, use the original mode for the 	     operation.  */
if|if
condition|(
name|and_optab
operator|->
...
argument_list|)
expr_stmt|;
comment|/* If X is still an AND, see if it is an AND with a mask that 	     is just some low-order bits.  If so, and it is BITS wide (it 	     can't be wider), we don't need it.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|LSHIFT
case|:
comment|/* For left shifts, do the same, but just for the first operand. 	 If the shift count is a constant, we need even fewer bits of the 	 first operand.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|LSHIFTRT
case|:
comment|/* Here we can only do something if the shift count is a constant and 	 the count plus BITS is no larger than the width of MODE, we can do 	 the shift in MODE.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|ASHIFTRT
case|:
comment|/* If this is a sign-extension operation that just affects bits 	 we don't care about, remove it.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Handle these similarly to the way we handle most binary operations. */
name|x
operator|=
name|gen_unary
argument_list|(
...
return|;
block|}
comment|/* The other case we handle is assignments into a constant-position      field.  They look like (ior (and DEST C1) OTHER).  If C1 represents      a mask that has all one bits except for a group of zero bits and      OTHER is known to have zeros where C1 has ones, this is such an      assignment.  Compute the position and length from C1.  Shift OTHER      to the appropriate position, force it to the required mode, and      make the extraction.  Check for the AND in both operands.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|significant
decl_stmt|;
comment|/* There is a large class of optimizations based on the principle that      some operations produce results where certain bits are known to be zero,      and hence are not significant to the AND.  For example, if we have just      done a left shift of one bit, the low-order bit is known to be zero and      hence an AND with a mask of ~1 would not do anything.       At the end of the following loop, we set:       VAROP to be the item to be AND'ed with;      CONSTOP to the constant value to AND it with.  */
while|while
condition|(
literal|1
condition|)
block|{
comment|/* If we ever encounter a mode wider than the host machine's widest 	 integer size, we can't compute the masks accurately, so give up.  */
if|if
condition|(
name|GET_MODE_BITSIZE
argument_list|(
...
condition|)
break|break;
comment|/* Unless one of the cases below does a `continue', 	 a `break' will be executed to exit the loop.  */
switch|switch
condition|(
name|GET_CODE
argument_list|(
...
name|CLOBBER
case|:
comment|/* If VAROP is a (clobber (const_int)), return it since we know 	     we are generating something that won't match. */
return|return
name|varop
return|;
if|#
...
name|USE
case|:
comment|/* VAROP is a (use (mem ..)) that was made from a bit-field 	     extraction that spanned the boundary of the MEM.  If we are 	     now masking so it is within that boundary, we don't need the 	     USE any more.  */
if|if
condition|(
operator|(
name|constop
...
name|varop
argument_list|)
comment|/* We can ignore the effect this SUBREG if it narrows the mode 		 or, on machines where byte operations extend, if the 		 constant masks to zero all the bits the mode doesn't have.  */
operator|&&
operator|(
operator|(
name|GET_MODE_SIZE
...
name|SIGN_EXTEND
case|:
comment|/* Try to expand these into a series of shifts and then work 	     with that result.  If we can't, for example, if the extract 	     isn't at a fixed position, give up.  */
name|temp
operator|=
name|expand_compound_operation
argument_list|(
...
name|XOR
case|:
comment|/* If VAROP is (ior (lshiftrt FOO C1) C2), try to commute the IOR and 	     LSHIFT so we end up with an (and (lshiftrt (ior ...) ...) ...) 	     operation which may be a bitfield extraction.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
continue|continue;
block|}
comment|/* Apply the AND to both branches of the IOR or XOR, then try to 	     apply the distributive law.  This may eliminate operations  	     if either branch can be simplified because of the AND. 	     It may also make some cases more complex, but those cases 	     probably won't match a pattern either with or without this.  */
return|return
name|gen_lowpart_for_combine
argument_list|(
name|mode
...
name|NOT
case|:
comment|/* (and (not FOO)) is (and (xor FOO CONST_OP)) so if FOO is an 	     LSHIFTRT we can do the same as above.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
block|}
block|}
comment|/* If our mask is 1, convert this to a LSHIFTRT.  This can be done 	     even if the shift count isn't a constant.  */
if|if
condition|(
name|constop
operator|==
...
name|NE
case|:
comment|/* (and (ne FOO 0) CONST) can be (and FOO CONST) if CONST is 	     included in STORE_FLAG_VALUE and FOO has no significant bits 	     not in CONST.  */
if|if
condition|(
operator|(
name|constop
...
name|PLUS
case|:
comment|/* In (and (plus FOO C1) M), if M is a mask that just turns off 	     low-order bits (as in an alignment operation) and FOO is already 	     aligned to that boundary, we can convert remove this AND 	     and possibly the PLUS if it is now adding zero.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|MINUS
case|:
comment|/* In (and (plus (and FOO M1) BAR) M2), if M1 and M2 are one 	     less than powers of two and M2 is narrower than M1, we can 	     eliminate the inner AND.  This occurs when incrementing 	     bit fields.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
break|break;
block|}
comment|/* If we have reached a constant, this whole thing is constant.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
return|;
comment|/* See what bits are significant in VAROP.  */
name|significant
operator|=
name|significant_bits
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Turn off all bits in the constant that are known to already be zero.      Thus, if the AND isn't needed at all, we will have CONSTOP == SIGNIFICANT      which is tested below.  */
name|constop
operator|&=
name|significant
expr_stmt|;
...
name|varop
expr_stmt|;
comment|/* Otherwise, return an AND.  See how much, if any, of X we can use.  */
elseif|else
if|if
condition|(
name|x
...
directive|endif
comment|/* If X is a register whose value we can find, use that value.   	 Otherwise, use the previously-computed significant bits for this 	 register.  */
name|tem
operator|=
name|get_last_value
argument_list|(
...
literal|1
expr_stmt|;
comment|/* A comparison operation only sets the bits given by its mode.  The 	 rest are set undefined.  */
if|if
condition|(
name|GET_MODE_SIZE
argument_list|(
...
operator|)
expr_stmt|;
comment|/* If the inner mode is a single word for both the host and target 	 machines, we can compute this from which bits of the inner 	 object are known significant.  */
if|if
condition|(
name|GET_MODE_BITSIZE
argument_list|(
...
name|ROTATE
case|:
comment|/* The significant bits are in two classes: any bits within MODE 	 that aren't in GET_MODE (x) are always significant.  The rest of the 	 significant bits are those that are significant in the operand of 	 the shift when shifted the appropriate number of bits.  This 	 shows that high-order bits are cleared by the right shift and 	 low-order bits by left shifts.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|count
expr_stmt|;
comment|/* If the sign bit was significant at before the shift, we 		 need to mark all the places it could have been copied to 		 by the shift significant.  */
if|if
condition|(
name|inner
operator|&
...
begin_comment
comment|/* Return the number of bits at the high-order end of X that are known to    be equal to the sign bit.  This number will always be between 1 and    the number of bits in the mode of X.  MODE is the mode to be used    if X is VOIDmode.  */
end_comment
...
name|tem
decl_stmt|;
comment|/* If we weren't given a mode, use the mode of X.  If the mode is still      VOIDmode, we don't know anything.  */
if|if
condition|(
name|mode
operator|==
...
name|BYTE_LOADS_SIGN_EXTEND
argument_list|)
comment|/* For paradoxical SUBREGs, just look inside since, on machines with 	 one of these defined, we assume that operations are actually  	 performed on the full register.  Note that we are passing MODE 	 to the recursive call, so the number of sign bit copies will 	 remain relative to that mode, not the inner mode.  */
if|if
condition|(
name|GET_MODE_SIZE
argument_list|(
...
name|NEG
case|:
comment|/* In general, this subtracts one sign bit copy.  But if the value 	 is known to be positive, the number of sign bit copies is the 	 same as that of the input.  Finally, if the input has just one 	 significant bit, all the bits are copies of the sign bit.  */
name|sig
operator|=
name|significant_bits
argument_list|(
...
operator|==
name|constm1_rtx
comment|/* Don't do this if XEXP (x, 0) is a paradoxical subreg 	     because in principle we don't know what the high bits are.  */
operator|&&
operator|!
operator|(
name|GET_CODE
...
name|UDIV
case|:
comment|/* The result must be<= the first operand.  */
return|return
name|num_sign_bit_copies
argument_list|(
name|XEXP
...
name|UMOD
case|:
comment|/* The result must be<= the scond operand.  */
return|return
name|num_sign_bit_copies
argument_list|(
name|XEXP
...
condition|)
break|break;
comment|/* Convert ROTATETRT to ROTATE.  */
if|if
condition|(
name|code
operator|==
...
name|count
expr_stmt|;
comment|/* Canonicalize LSHIFT to ASHIFT.  */
if|if
condition|(
name|code
operator|==
...
name|ASHIFT
expr_stmt|;
comment|/* We need to determine what mode we will do the shift in.  If the 	 shift is a ASHIFTRT or ROTATE, we must always do it in the mode it 	 was originally done in.  Otherwise, we can do it in MODE, the widest 	 mode encountered. */
name|shift_mode
operator|=
operator|(
name|code
...
block|}
block|}
comment|/* Negative counts are invalid and should not have been made (a 	 programmer-specified negative count should have been handled 	 above). */
elseif|else
if|if
condition|(
name|count
...
condition|)
break|break;
comment|/* To compute the mask to apply after the shift, shift the 		 significant bits of the inner shift the same way the  		 outer shift will.  */
name|mask_rtx
operator|=
name|GEN_INT
argument_list|(
...
name|EQ
case|:
comment|/* convert (lshift (eq FOO 0) C) to (xor FOO 1) if STORE_FLAG_VALUE 	     says that the sign bit can be tested, FOO has mode MODE, C is 	     GET_MODE_BITSIZE (MODE) - 1, and FOO has only the low-order bit 	     significant.  */
if|if
condition|(
name|code
operator|==
...
continue|continue;
block|}
comment|/* If we have (xshiftrt (plus FOO BAR) C), and the only bits 	     significant in BAR are those being shifted out and those 	     bits are known zero in FOO, we can replace the PLUS with FOO. 	     Similarly in the other operand order.  This code occurs when 	     we are computing the size of a variable-size array.  */
if|if
condition|(
operator|(
name|code
...
break|break;
block|}
comment|/* We need to determine what mode to do the shift in.  If the shift is      a ASHIFTRT or ROTATE, we must always do it in the mode it was originally      done in.  Otherwise, we can do it in MODE, the widest mode encountered.      The code we care about is that of the shift that will actually be done,      not the shift that was originally requested.  */
name|shift_mode
operator|=
operator|(
name|code
...
begin_comment
comment|/* Make an rtx expression.  This is a subset of gen_rtx and only supports    expressions of 1, 2, or 3 operands, each of which are rtx expressions.     If the identical expression was previously in the insn (in the undobuf),    it will be returned.  Only if it is not found will a new expression    be made.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* Get each arg and verify that it is supposed to be an expression.  */
for|for
control|(
name|j
operator|=
...
expr_stmt|;
block|}
comment|/* See if this is in undobuf.  Be sure we don't use objects that came      from another insn; this could produce circular rtl structures.  */
for|for
control|(
name|i
operator|=
...
return|;
block|}
comment|/* Otherwise make a new rtx.  We know we have 1, 2, or 3 args.      Use rtx_alloc instead of gen_rtx because it's faster on RISC.  */
name|rt
operator|=
name|rtx_alloc
argument_list|(
...
condition|)
block|{
comment|/* If both operands are the same constant shift, see if we can ignore the 	 shift.  We can if the shift is a rotate or if the bits shifted out of 	 this shift are not significant for either input and if the type of 	 comparison is compatible with the shift.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
break|break;
block|}
comment|/* If the first operand is a constant, swap the operands and adjust the      comparison code appropriately.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
name|mask
expr_stmt|;
comment|/* If we are comparing against a constant power of two and the value 	 being compared has only that single significant bit (e.g., it was 	 `and'ed with that bit), we can replace this with a comparison 	 with zero.  */
if|if
condition|(
name|const_op
operator|&&
...
continue|continue;
block|}
comment|/* If we have NEG of something that is the result of a 	     SIGN_EXTEND, SIGN_EXTRACT, or ASHIFTRT, we know that the 	     two high-order bits must be the same and hence that 	     "(-a)< 0" is equivalent to "a> 0".  Otherwise, we can't 	     do this.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|SUBREG
case|:
comment|/* Check for the case where we are comparing A - C1 with C2, 	     both constants are smaller than 1/2 the maxium positive 	     value in MODE, and the comparison is equality or unsigned. 	     In that case, if A is either zero-extended to MODE or has 	     sufficient sign bits so that the high-order bit in MODE 	     is a copy of the sign in the inner mode, we can prove that it is 	     safe to do the operation in the wider mode.  This simplifies 	     many range checks.  */
if|if
condition|(
name|mode_width
operator|<=
...
name|PLUS
case|:
comment|/* (eq (plus X C1) C2) -> (eq X (minus C2 C1)).  We can only do 	     this for equality comparisons due to pathological cases involving 	     overflows.  */
if|if
condition|(
name|equality_comparison_p
operator|&&
...
name|LSHIFT
case|:
comment|/* If we have (compare (xshift FOO N) (const_int C)) and 	     the high order N bits of FOO (N+1 if an inequality comparison) 	     are not significant, we can do this by comparing FOO with C 	     shifted right N bits so long as the low-order N bits of C are 	     zero.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|LSHIFTRT
case|:
comment|/* If we have (compare (xshiftrt FOO N) (const_int C)) and 	     the low order N bits of FOO are not significant, we can do this 	     by comparing FOO with C shifted left N bits so long as no 	     overflow occurs.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
break|break;
block|}
comment|/* Now make any compound operations involved in this comparison.  Then,      check for an outmost SUBREG on OP0 that isn't doing anything or is      paradoxical.  The latter case can only occur when it is known that the      "extra" bits will be zero.  Therefore, it is safe to remove the SUBREG.      We can never remove a SUBREG for a non-equality comparison because the      sign bit is in a different place in the underlying object.  */
name|op0
operator|=
name|make_compound_operation
argument_list|(
...
condition|)
block|{
comment|/* If the only significant bits in OP0 and OP1 are those in the 	     narrower mode and this is an equality or unsigned comparison, 	     we can use the wider mode.  Similarly for sign-extended 	     values and equality or signed comparisons.  */
if|if
condition|(
operator|(
operator|(
...
begin_comment
comment|/* Return 1 if we know that X, a comparison operation, is not operating    on a floating-point value or is EQ or NE, meaning that we can safely    reverse it.  */
end_comment
...
begin_comment
comment|/* Record that REG is set to VALUE in insn INSN.  If VALUE is zero, we    are saying that the register is clobbered and we no longer know its    value.  If INSN is zero, don't update reg_last_set; this call is normally    done with VALUE also zero to invalidate the register.  */
end_comment
...
expr_stmt|;
block|}
comment|/* For each register modified, show we don't know its value, that      its value has been updated, and that we don't know the location of      the death of the register.  */
for|for
control|(
name|i
operator|=
...
expr_stmt|;
block|}
comment|/* For the main register being modified, update the value.  */
name|reg_last_set_value
index|[
name|regno
index|]
...
begin_comment
comment|/* Used for communication between the following two routines.  */
end_comment
...
begin_comment
comment|/* Called via note_stores from record_dead_and_set_regs to handle one    SET or CLOBBER in an insn.  */
end_comment
...
begin_comment
comment|/* Update the records of when each REG was most recently set or killed    for the things done by INSN.  This is the last thing done in processing    INSN in the combiner loop.     We update reg_last_set, reg_last_set_value, reg_last_death, and also the    similar information mem_last_set (which insn most recently modified memory)    and last_call_cuid (which insn was the most recent subroutine call).  */
end_comment
...
name|j
index|]
comment|/* If this is a pseudo-register that was only set once, it is 	       always valid.  */
operator|||
operator|(
operator|!
operator|(
...
index|]
expr_stmt|;
comment|/* If we don't have a value or if it isn't for this basic block, return 0. */
if|if
condition|(
name|value
operator|==
...
literal|0
return|;
comment|/* If the value was set in a later insn that the ones we are processing,      we can't use it even if the register was only set once, but make a quick      check to see if the previous insn set it to something.  This is commonly      the case when the same pseudo is used by repeated insns.  */
if|if
condition|(
name|INSN_CUID
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Make sure that VALUE doesn't reference X.  Replace any 	     expliit references with a CLOBBER.  If there are any remaining 	     references (rare), don't use the value.  */
if|if
condition|(
name|reg_mentioned_p
argument_list|(
...
begin_comment
comment|/* Return non-zero if REG is known to be dead at INSN.     We scan backwards from INSN.  If we hit a REG_DEAD note or a CLOBBER    referencing REG, it is dead.  If we hit a SET referencing REG, it is    live.  Otherwise, see if it is live or dead at the start of the basic    block we are in.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* It is possible for the call above to return 0.  This can occur 	     when reg_last_death points to I2 or I1 that we combined with. 	     In that case make a new note.  */
if|if
condition|(
name|note
condition|)
...
name|REG_WAS_0
case|:
comment|/* It is too much trouble to try to see if this note is still 	     correct in all situations.  It is better to simply delete it.  */
break|break;
case|case
name|REG_RETVAL
case|:
...
argument_list|)
decl_stmt|;
comment|/* Verify that it was the set, and not a clobber that 		       modified the register.  */
if|if
condition|(
name|set
operator|!=
...
name|p
decl_stmt|;
comment|/* See if we already placed a USE note for this 			   register in front of PLACE.  */
for|for
control|(
name|p
operator|=
...
condition|)
block|{
comment|/* Put only REG_DEAD notes for pieces that are 			 still used and that are not already dead or set.  */
for|for
control|(
name|i
operator|=
