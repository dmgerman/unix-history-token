...
begin_comment
comment|/* Functions related to invoking methods and overloaded functions.    Copyright (C) 1987, 1992 Free Software Foundation, Inc.    Contributed by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
condition|)
block|{
comment|/* Becuase the READONLY bits are not always in the type, 		     this extra check is necessary.  The problem should 		     be fixed someplace else, and this extra code removed.  		     Also, if type if a reference, the readonly bits could 		     either be in the outer type (with reference) or on the 		     inner type (the thing being referenced).  (mrs)  */
if|if
condition|(
name|TREE_READONLY
argument_list|(
...
condition|)
block|{
comment|/* We first try to look for a nesting in our current class context.  */
name|tree
name|rval
init|=
name|resolve_scope_to_name
...
directive|if
literal|0
block|if (all_virtual == 1&& (! strncmp (IDENTIFIER_POINTER (name), OPERATOR_METHOD_FORMAT, 		     OPERATOR_METHOD_LENGTH) 	  || instance_ptr == NULL_TREE 	  || (TYPE_OVERLOADS_METHOD_CALL_EXPR (basetype) == 0
comment|/*&& TYPE_NEEDS_WRAPPER (basetype) == 0*/
block|)))     all_virtual = 0;
endif|#
directive|endif
name|last
...
directive|endif
block|)     {       tree fn_as_int;
ifdef|#
directive|ifdef
name|ESKIT
block|register int bytecount = 0;        parms = tree_cons (NULL_TREE, build_int_2 (bytecount, 0), 			 TREE_CHAIN (parms));
else|#
directive|else
block|parms = TREE_CHAIN (parms);
endif|#
directive|endif
block|if (!all_virtual&& TREE_CODE (function) == FUNCTION_DECL) 	fn_as_int = build_unary_op (ADDR_EXPR, function, 0);       else 	fn_as_int = convert (TREE_TYPE (default_conversion (function)), DECL_VINDEX (function));       if (all_virtual == 1) 	fn_as_int = convert (integer_type_node, fn_as_int);        result = build_opfncall (METHOD_CALL_EXPR, LOOKUP_NORMAL, instance, fn_as_int, parms);        if (result == NULL_TREE) 	{ 	  compiler_error ("could not overload `operator->()(...)'"); 	  return error_mark_node; 	}       else if (result == error_mark_node) 	return error_mark_node;
if|#
directive|if
literal|0
...
name|ESKIT
block|{ 	int used, size;
comment|/* Count the number of bytes of arguments to operator->(), 	   not to the method itself.  In the tally, don't count bytes 	   for pointer to member function or for the bytecount.  */
block|parms = TREE_OPERAND (result, 1); 	bytecount = get_arglist_len_in_bytes (TREE_CHAIN (TREE_CHAIN (TREE_CHAIN (parms)))); 	used = size = GET_MODE_SIZE (TYPE_MODE (TREE_TYPE (TREE_VALUE (parms))));
ifdef|#
directive|ifdef
name|PUSH_ROUNDING
block|size = PUSH_ROUNDING (size);
endif|#
directive|endif
block|used = (((size + PARM_BOUNDARY / BITS_PER_UNIT - 1) 		 / (PARM_BOUNDARY / BITS_PER_UNIT)) 		* (PARM_BOUNDARY / BITS_PER_UNIT)); 	bytecount += used; 	TREE_CHAIN (TREE_CHAIN (parms)) 	  = tree_cons (NULL_TREE, build_int_2 (bytecount, 0), 		       TREE_CHAIN (TREE_CHAIN (TREE_CHAIN (parms))));       }
endif|#
directive|endif
block|return result;     }
endif|#
