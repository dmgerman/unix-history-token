...
begin_comment
comment|/* Max scale factor for scaled address in lea instruction.  */
end_comment
...
name|reg
argument_list|,
comment|/* Using a constant gives better 				     estimate of typical costs. 				     1 or 2 might have quirks.  */
name|GEN_INT
argument_list|(
literal|3
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Using 0 as second argument is not quite right, 	 but what else is there to do?  */
if|if
condition|(
name|recog
argument_list|(
...
begin_comment
comment|/* Compute and return the best algorithm for multiplying by T.    Assume that add insns cost ADD_COST and shifts cost SHIFT_COST.    Return cost -1 if would cost more than MAX_COST.  */
end_comment
...
name|cost
decl_stmt|;
comment|/* No matter what happens, we want to return a valid algorithm.  */
name|best_alg
operator|->
name|cost
operator|=
...
literal|0
expr_stmt|;
comment|/* Is t an exponent of 2, so we can just do a shift?  */
if|if
condition|(
operator|(
name|t
...
return|;
block|}
comment|/* If MAX_COST just permits as little as an addition (or less), we won't      succeed in synthesizing an algorithm for t.  Return immediately with      an indication of failure.  */
if|if
condition|(
name|max_cost
operator|<=
...
return|;
block|}
comment|/* Look for factors of t of the form      t = q(2**m +- 1), 2<= m<= floor(log2(t)) - 1.      If we find such a factor, we can multiply by t using an algorithm that      multiplies by q, shift the result by m and add/subtract it to itself.  */
for|for
control|(
name|m
operator|=
...
block|}
block|}
comment|/* Try load effective address instructions, i.e. do a*3, a*5, a*9.  */
block|{
name|HOST_WIDE_INT
name|q
decl_stmt|;
...
name|cost
expr_stmt|;
comment|/* Use<= to prefer this method to the factoring method 	       when the cost appears the same, because this method 	       uses fewer temporary registers.  */
if|if
condition|(
name|cost
operator|<=
...
block|}
block|}
comment|/* Now, use the good old method to add or subtract at the leftmost      1-bit.  */
block|{
name|HOST_WIDE_INT
name|q
decl_stmt|;
...
block|{
comment|/* There's only one bit at the left.  Make it by addition.  */
name|cost
operator|=
name|add_cost
expr_stmt|;
...
condition|)
block|{
comment|/* If we found something, it must be cheaper than multiply. 	     So use it.  */
name|int
name|opno
init|=
literal|0
...
block|{
comment|/* 1 if this is the last in a series of adds and subtracts.  */
name|int
name|last
init|=
operator|(
...
operator|)
decl_stmt|;
comment|/* If we have not yet seen any separate factors (alg_compound) 		 then turn op0<<a1 + op0<<a2 + op0<<a3... into 		 (op0<<(a1-a2) + op0)<<(a2-a3) + op0...  */
switch|switch
condition|(
name|alg
operator|.
...
block|}
block|}
comment|/* Write a REG_EQUAL note on the last insn so that we can cse  	     multiplication sequences.  We need not do this if we were 	     multiplying by a power of two, since only one insn would have 	     been generated.  	     ??? We could also write REG_EQUAL notes on the last insn of 	     each sequence that uses a single temporary, but it is not 	     clear how to calculate the partial product so far.  	     Torbjorn: Can you do this?  */
if|if
condition|(
name|exact_log2
argument_list|(
