...
comment|/* Also mark as reachable any blocks headed by labels that      must not be deleted.  */
block|{
specifier|register
name|RTX_CODE
name|prev_code
...
name|i
expr_stmt|;
comment|/* Don't separate a CALL_INSN from following CLOBBER insns.  This is 	   a kludge that will go away when each CALL_INSN records its 	   USE and CLOBBERs.  */
if|if
condition|(
name|code
operator|!=
...
literal|1
decl_stmt|;
comment|/* Find all indirect jump insns and mark them as possibly jumping 	 to all the labels whose addresses are explicitly used. 	 This is because, when there are computed gotos, 	 we can't tell which labels they jump to, of all the possibilities.  */
for|for
control|(
name|insn
operator|=
...
condition|)
block|{
comment|/* We have *p followed by q = p+size. 		 Both p and q must be live afterward, 		 and q must be dead before. 		 Change it to q = p, ...*q..., q = q+size. 		 Then fall into the usual case.  */
name|rtx
name|insns
decl_stmt|,
name|temp
...
name|addr
expr_stmt|;
comment|/* Indicate insn must be re-recognized.  */
name|INSN_CODE
argument_list|(
name|incr
argument_list|)
...
literal|1
expr_stmt|;
comment|/* If that makes it a no-op (copying the register into itself) 		 then delete it so it won't appear to be a "use" and a "set" 		 of this register.  */
if|if
condition|(
name|SET_DEST
argument_list|(
