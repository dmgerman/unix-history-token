...
begin_comment
comment|/* Common subexpression elimination for GNU compiler.    Copyright (C) 1987, 1988, 1989, 1992 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* Compute cost of X, as stored in the `cost' field of a table_elt.  Fixed    hard registers are the cheapest with a cost of 0.  Next come pseudos    with a cost of one and other hard registers with a cost of 2.  Aside    from these special cases, call `rtx_cost'.  */
end_comment
...
parameter_list|)
define|\
value|(GET_CODE (X) == REG					\    ? (REGNO (X)>= FIRST_PSEUDO_REGISTER ? 1		\       : (FIXED_REGNO_P (REGNO (X))			\&& REGNO_REG_CLASS (REGNO (X)) != NO_REGS) ? 0	\       : 2)						\    : rtx_cost (X, SET) * 2)
end_define
...
begin_comment
comment|/* Return 1 iff any subexpression of X refers to memory    at an address of BASE plus some offset    such that any of the bytes' offsets fall between START (inclusive)    and END (exclusive).     The value is undefined if X is a varying address.    This function is not used in such cases.     When used in the cse pass, `qty_const' is nonzero, and it is used    to treat an address that is a register with a known constant value    as if it were that constant value.    In the loop pass, `qty_const' is zero, so this is not done.  */
end_comment
...
literal|1
return|;
comment|/* The REG component of the LO_SUM is known by the 	     const value in the XEXP part.  */
name|addr
operator|=
name|XEXP
argument_list|(
...
name|op0
return|;
comment|/* Strip off any surrounding CONSTs.  They don't matter in any of  	     the cases below.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
return|;
block|}
comment|/* Don't use the associative law for floating point. 	     The inaccuracy makes it nonassociative, 	     and subtle programs can break if operations are associated.  */
if|if
condition|(
name|GET_MODE_CLASS
argument_list|(
...
condition|)
break|break;
comment|/* (a - b) + b -> a, similarly a + (b - a) -> a */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
return|;
comment|/* (c1 - a) + c2 becomes (c1 + c2) - a.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
directive|if
literal|0
comment|/* No good, because this can produce the sum of two relocatable 	 symbols, in an assembler instruction.  Most UNIX assemblers can't 	 handle that.  */
block|else 		return gen_rtx (CONST, mode, 				gen_rtx (PLUS, mode, 					 GET_CODE (op0) == CONST 					 ? XEXP (op0, 0) : op0, 					 GET_CODE (op1) == CONST 					 ? XEXP (op1, 0) : op1));
endif|#
directive|endif
block|}
elseif|else
...
argument_list|)
return|;
comment|/* (a +/- b) - (a +/- c) can be simplified.  Do variants of 	     this involving commutativity.  The most common case is 	     (a + C1) - (a + C2), but it's not hard to do all the cases.  */
if|if
condition|(
operator|(
name|GET_CODE
...
literal|0
decl_stmt|;
comment|/* Set LHS and RHS to the two different terms.  */
if|if
condition|(
name|rtx_equal_p
argument_list|(
...
name|rhs0
expr_stmt|;
comment|/* The RHS is the operand of a MINUS, so its negation 		 status should be complemented.  */
name|rhs_neg
operator|=
operator|!
name|rhs_neg
expr_stmt|;
comment|/* If we found two values equal, form the sum or difference 		 of the remaining two terms.   */
if|if
condition|(
name|lhs
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If both sides negated, negate result.  */
if|if
condition|(
name|lhs_neg
operator|&&
...
return|;
block|}
comment|/* c1 - (c2 - a) becomes (c1 - c2) + a.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|REAL_ARITHMETIC
argument_list|)
block|REAL_ARITHMETIC (d, RDIV_EXPR, dconst1, d); 	      return gen_rtx (MULT, mode, op0,  			      CONST_DOUBLE_FROM_REAL_VALUE (d, mode));
else|#
directive|else
...
begin_comment
comment|/* If X is a nontrivial arithmetic operation on an argument    for which a constant value can be determined, return    the result of operating on that value, as a constant.    Otherwise, return X, possibly with one or more operands    modified by recursive calls to this function.     If X is a register whose contents are known, we do NOT    return those contents.  This is because an instruction that    uses a register is usually faster than one that uses a constant.     INSN is the insn that we may be modifying.  If it is 0, make a copy    of X before modifying it.  */
end_comment
...
condition|)
break|break;
comment|/* If we are associating shift operations, don't let this 		 produce a shift of larger than the object.  This could 		 occur when we following a sign-extend by a right shift on 		 a machine that does a sign-extend as a pair of shifts.  */
if|if
condition|(
name|is_shift
operator|&&
