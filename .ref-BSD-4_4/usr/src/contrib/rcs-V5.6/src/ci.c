...
literal|"ci"
argument_list|,
literal|"$Id: ci.c,v 5.21 1991/11/20 17:58:07 eggert Exp $"
argument_list|)
end_macro
...
expr_stmt|;
block|}
comment|/* now RCSfilename contains the name of the RCS file, and          * workfilename contains the name of the working file. 	 * If the RCS file exists, finptr contains the file descriptor for the          * RCS file. The admin node is initialized. 	 * RCSstat is set.          */
name|diagnose
argument_list|(
literal|"%s<--  %s\n"
argument_list|,
...
name|error
argument_list|(
literal|"Date %s precedes %s in existing revision %s."
argument_list|,
name|date2str
argument_list|(
name|newdelta
...
directive|else
comment|/* 			     * Work around a common ftruncate() bug. 			     * We can't rely on has_truncate, because we might 			     * be using a filesystem exported to us via NFS. 			     */
name|bad_truncate
operator|=
name|ftruncate
argument_list|(
...
argument_list|,
argument|newhead ? workdiffname : expfilename
argument_list|,
argument|newhead ? expfilename : workdiffname
argument_list|,
argument|(char*)
literal|0
argument_list|)
...
name|error
argument_list|(
literal|"Branch point doesn't exist for %s."
argument_list|,
name|newdelnum
operator|.
name|string
...
name|error
argument_list|(
literal|"can't find branchpoint %s"
argument_list|,
name|newdelnum
operator|.
name|string
...
name|fchmod
argument_list|(
argument|Ifileno(workptr)
argument_list|,
argument|newworkmode
argument_list|)
else|#
directive|else
name|chmod
...
name|exfilename
decl_stmt|;
comment|/*  * Read unexfile and copy it to a  * file in dir, performing keyword substitution with data from delta.  * Return -1 if unsuccessful, 1 if expansion occurred, 0 otherwise.  * If successful, stores the stream descriptor into *EXFILEP  * and its name into *EXFILENAME.  */
block|{
name|char
specifier|const
modifier|*
...
name|error
argument_list|(
literal|"can't expand working file"
argument_list|)
expr_stmt|;
return|return
operator|-
