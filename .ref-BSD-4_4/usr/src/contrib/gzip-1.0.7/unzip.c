...
index|[]
init|=
literal|"$Id: unzip.c,v 0.11 1993/03/04 19:15:33 jloup Exp $"
decl_stmt|;
end_decl_stmt
...
name|error
argument_list|(
literal|"input not a zip file or empty"
argument_list|)
expr_stmt|;
block|}
name|method
...
name|error
argument_list|(
literal|"first entry not deflated or stored -- use unzip"
argument_list|)
expr_stmt|;
block|}
...
name|stderr
argument_list|,
literal|"encrypted file, not yet supported.\n"
argument_list|)
expr_stmt|;
name|exit_code
operator|=
...
name|stderr
argument_list|,
literal|"warning: zip file has more than one entry--rest ignored\n"
argument_list|)
expr_stmt|;
block|}
else|else
