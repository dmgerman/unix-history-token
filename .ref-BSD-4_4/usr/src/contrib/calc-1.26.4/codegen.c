...
begin_comment
comment|/*  * Copyright (c) 1993 David I. Bell  * Permission is granted to use, distribute, or modify this source,  * provided that this copyright notice remains intact.  *  * Module to generate opcodes from the input tokens.  */
end_comment
...
begin_comment
comment|/*  * Read all the commands from an input file.  * These are either declarations, or else are commands to execute now.  * In general, commands are terminated by newlines or semicolons.  * Exceptions are function definitions and escaped newlines.  * Commands are read and executed until the end of file.  */
end_comment
...
begin_comment
comment|/*  * Get a line of local or global variable declarations.  * declarations = { LOCAL | GLOBAL } name [ ',' name ] ... ';'.  */
end_comment
...
name|T_SEMICOLON
argument_list|,
literal|"Variable name expected for declaration statement"
argument_list|)
expr_stmt|;
return|return;
block|}
...
begin_comment
comment|/*  * Read in an object definition statement.  * This is of the following form:  *	OBJ type [ '{' id [ ',' id ] ... '}' ]  [ objlist ].  * The OBJ keyword has already been read.  */
end_comment
...
begin_comment
comment|/*  * Routine to collect a set of variables for the specified object type  * and initialize them as being that type of object.  * Here  *	objlist = name [ ',' name] ... ';'.  */
end_comment
...
begin_comment
comment|/*  * Read a matrix definition statment for a one or more dimensional matrix.  * The MAT keyword has already been read.  */
end_comment
...
name|T_SEMICOLON
argument_list|,
literal|"Only %d dimensions allowed"
argument_list|,
name|MAXDIM
argument_list|)
expr_stmt|;
...
name|stringindex
argument_list|(
literal|"builtins\0globals\0functions\0objfuncs\0memory\0"
argument_list|,
name|name
argument_list|)
condition|)
...
argument_list|()
expr_stmt|;
comment|/* 	 * Normal indexing with the indexes separated by commas. 	 */
name|dim
operator|=
literal|1
expr_stmt|;
