...
comment|/* It's possible that the buffer has been given another process 	   between the time CHILD dies and CHILD's death is noticed (via 	   list-processes).  So we only set it the buffer's process to 	   NULL if CHILD is still the controlling process. */
if|if
condition|(
name|child
operator|->
