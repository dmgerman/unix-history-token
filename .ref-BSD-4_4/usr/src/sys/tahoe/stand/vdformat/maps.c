...
index|[]
init|=
literal|"@(#)maps.c	1.8 (Berkeley/CCI) 6/24/90"
decl_stmt|;
end_decl_stmt
...
name|norm_bad_map
expr_stmt|;
comment|/* 		 * If this doesn't look like a new-style map, 		 * but (as an old-style map) bs_count and bs_max are sensible, 		 * munge pointer to prepend fields missing in old map. 		 */
if|if
condition|(
name|map
operator|->
...
name|print
argument_list|(
literal|"Bad-sector map magic number wrong! (%x != %x)\n"
argument_list|,
name|bad_map
operator|->
name|bs_magic
...
name|print
argument_list|(
literal|"The following %d sector%s known to be bad:\n"
argument_list|,
name|bad_map
operator|->
name|bs_count
...
name|print
argument_list|(
literal|"%c %d cn %d tn %d sn %d pos %d len %d "
argument_list|,
name|errloc
operator|.
name|err_stat
