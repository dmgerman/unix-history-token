begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * System call names.  *  * DO NOT EDIT-- this file is automatically generated.  * created from	@(#)syscalls.master	8.1 (Berkeley) 6/16/93  */
end_comment

begin_decl_stmt
name|char
modifier|*
name|syscallnames
index|[]
init|=
block|{
literal|"syscall"
block|,
comment|/* 0 = syscall */
literal|"exit"
block|,
comment|/* 1 = exit */
literal|"fork"
block|,
comment|/* 2 = fork */
literal|"read"
block|,
comment|/* 3 = read */
literal|"write"
block|,
comment|/* 4 = write */
literal|"open"
block|,
comment|/* 5 = open */
literal|"close"
block|,
comment|/* 6 = close */
literal|"wait4"
block|,
comment|/* 7 = wait4 */
literal|"old.creat"
block|,
comment|/* 8 = old creat */
literal|"link"
block|,
comment|/* 9 = link */
literal|"unlink"
block|,
comment|/* 10 = unlink */
literal|"obs_execv"
block|,
comment|/* 11 = obsolete execv */
literal|"chdir"
block|,
comment|/* 12 = chdir */
literal|"fchdir"
block|,
comment|/* 13 = fchdir */
literal|"mknod"
block|,
comment|/* 14 = mknod */
literal|"chmod"
block|,
comment|/* 15 = chmod */
literal|"chown"
block|,
comment|/* 16 = chown */
literal|"break"
block|,
comment|/* 17 = break */
literal|"getfsstat"
block|,
comment|/* 18 = getfsstat */
literal|"old.lseek"
block|,
comment|/* 19 = old lseek */
literal|"getpid"
block|,
comment|/* 20 = getpid */
literal|"mount"
block|,
comment|/* 21 = mount */
literal|"unmount"
block|,
comment|/* 22 = unmount */
literal|"setuid"
block|,
comment|/* 23 = setuid */
literal|"getuid"
block|,
comment|/* 24 = getuid */
literal|"geteuid"
block|,
comment|/* 25 = geteuid */
literal|"ptrace"
block|,
comment|/* 26 = ptrace */
literal|"recvmsg"
block|,
comment|/* 27 = recvmsg */
literal|"sendmsg"
block|,
comment|/* 28 = sendmsg */
literal|"recvfrom"
block|,
comment|/* 29 = recvfrom */
literal|"accept"
block|,
comment|/* 30 = accept */
literal|"getpeername"
block|,
comment|/* 31 = getpeername */
literal|"getsockname"
block|,
comment|/* 32 = getsockname */
literal|"access"
block|,
comment|/* 33 = access */
literal|"chflags"
block|,
comment|/* 34 = chflags */
literal|"fchflags"
block|,
comment|/* 35 = fchflags */
literal|"sync"
block|,
comment|/* 36 = sync */
literal|"kill"
block|,
comment|/* 37 = kill */
literal|"old.stat"
block|,
comment|/* 38 = old stat */
literal|"getppid"
block|,
comment|/* 39 = getppid */
literal|"old.lstat"
block|,
comment|/* 40 = old lstat */
literal|"dup"
block|,
comment|/* 41 = dup */
literal|"pipe"
block|,
comment|/* 42 = pipe */
literal|"getegid"
block|,
comment|/* 43 = getegid */
literal|"profil"
block|,
comment|/* 44 = profil */
ifdef|#
directive|ifdef
name|KTRACE
literal|"ktrace"
block|,
comment|/* 45 = ktrace */
else|#
directive|else
literal|"#45"
block|,
comment|/* 45 = ktrace */
endif|#
directive|endif
literal|"sigaction"
block|,
comment|/* 46 = sigaction */
literal|"getgid"
block|,
comment|/* 47 = getgid */
literal|"sigprocmask"
block|,
comment|/* 48 = sigprocmask */
literal|"getlogin"
block|,
comment|/* 49 = getlogin */
literal|"setlogin"
block|,
comment|/* 50 = setlogin */
literal|"acct"
block|,
comment|/* 51 = acct */
literal|"sigpending"
block|,
comment|/* 52 = sigpending */
literal|"sigaltstack"
block|,
comment|/* 53 = sigaltstack */
literal|"ioctl"
block|,
comment|/* 54 = ioctl */
literal|"reboot"
block|,
comment|/* 55 = reboot */
literal|"revoke"
block|,
comment|/* 56 = revoke */
literal|"symlink"
block|,
comment|/* 57 = symlink */
literal|"readlink"
block|,
comment|/* 58 = readlink */
literal|"execve"
block|,
comment|/* 59 = execve */
literal|"umask"
block|,
comment|/* 60 = umask */
literal|"chroot"
block|,
comment|/* 61 = chroot */
literal|"old.fstat"
block|,
comment|/* 62 = old fstat */
literal|"old.getkerninfo"
block|,
comment|/* 63 = old getkerninfo */
literal|"old.getpagesize"
block|,
comment|/* 64 = old getpagesize */
literal|"msync"
block|,
comment|/* 65 = msync */
literal|"vfork"
block|,
comment|/* 66 = vfork */
literal|"obs_vread"
block|,
comment|/* 67 = obsolete vread */
literal|"obs_vwrite"
block|,
comment|/* 68 = obsolete vwrite */
literal|"sbrk"
block|,
comment|/* 69 = sbrk */
literal|"sstk"
block|,
comment|/* 70 = sstk */
literal|"old.mmap"
block|,
comment|/* 71 = old mmap */
literal|"vadvise"
block|,
comment|/* 72 = vadvise */
literal|"munmap"
block|,
comment|/* 73 = munmap */
literal|"mprotect"
block|,
comment|/* 74 = mprotect */
literal|"madvise"
block|,
comment|/* 75 = madvise */
literal|"obs_vhangup"
block|,
comment|/* 76 = obsolete vhangup */
literal|"obs_vlimit"
block|,
comment|/* 77 = obsolete vlimit */
literal|"mincore"
block|,
comment|/* 78 = mincore */
literal|"getgroups"
block|,
comment|/* 79 = getgroups */
literal|"setgroups"
block|,
comment|/* 80 = setgroups */
literal|"getpgrp"
block|,
comment|/* 81 = getpgrp */
literal|"setpgid"
block|,
comment|/* 82 = setpgid */
literal|"setitimer"
block|,
comment|/* 83 = setitimer */
literal|"old.wait"
block|,
comment|/* 84 = old wait */
literal|"swapon"
block|,
comment|/* 85 = swapon */
literal|"getitimer"
block|,
comment|/* 86 = getitimer */
literal|"old.gethostname"
block|,
comment|/* 87 = old gethostname */
literal|"old.sethostname"
block|,
comment|/* 88 = old sethostname */
literal|"getdtablesize"
block|,
comment|/* 89 = getdtablesize */
literal|"dup2"
block|,
comment|/* 90 = dup2 */
literal|"#91"
block|,
comment|/* 91 = getdopt */
literal|"fcntl"
block|,
comment|/* 92 = fcntl */
literal|"select"
block|,
comment|/* 93 = select */
literal|"#94"
block|,
comment|/* 94 = setdopt */
literal|"fsync"
block|,
comment|/* 95 = fsync */
literal|"setpriority"
block|,
comment|/* 96 = setpriority */
literal|"socket"
block|,
comment|/* 97 = socket */
literal|"connect"
block|,
comment|/* 98 = connect */
literal|"old.accept"
block|,
comment|/* 99 = old accept */
literal|"getpriority"
block|,
comment|/* 100 = getpriority */
literal|"old.send"
block|,
comment|/* 101 = old send */
literal|"old.recv"
block|,
comment|/* 102 = old recv */
literal|"sigreturn"
block|,
comment|/* 103 = sigreturn */
literal|"bind"
block|,
comment|/* 104 = bind */
literal|"setsockopt"
block|,
comment|/* 105 = setsockopt */
literal|"listen"
block|,
comment|/* 106 = listen */
literal|"obs_vtimes"
block|,
comment|/* 107 = obsolete vtimes */
literal|"old.sigvec"
block|,
comment|/* 108 = old sigvec */
literal|"old.sigblock"
block|,
comment|/* 109 = old sigblock */
literal|"old.sigsetmask"
block|,
comment|/* 110 = old sigsetmask */
literal|"sigsuspend"
block|,
comment|/* 111 = sigsuspend */
literal|"old.sigstack"
block|,
comment|/* 112 = old sigstack */
literal|"old.recvmsg"
block|,
comment|/* 113 = old recvmsg */
literal|"old.sendmsg"
block|,
comment|/* 114 = old sendmsg */
ifdef|#
directive|ifdef
name|TRACE
literal|"vtrace"
block|,
comment|/* 115 = vtrace */
else|#
directive|else
literal|"obs_vtrace"
block|,
comment|/* 115 = obsolete vtrace */
endif|#
directive|endif
literal|"gettimeofday"
block|,
comment|/* 116 = gettimeofday */
literal|"getrusage"
block|,
comment|/* 117 = getrusage */
literal|"getsockopt"
block|,
comment|/* 118 = getsockopt */
ifdef|#
directive|ifdef
name|vax
literal|"resuba"
block|,
comment|/* 119 = resuba */
else|#
directive|else
literal|"#119"
block|,
comment|/* 119 = nosys */
endif|#
directive|endif
literal|"readv"
block|,
comment|/* 120 = readv */
literal|"writev"
block|,
comment|/* 121 = writev */
literal|"settimeofday"
block|,
comment|/* 122 = settimeofday */
literal|"fchown"
block|,
comment|/* 123 = fchown */
literal|"fchmod"
block|,
comment|/* 124 = fchmod */
literal|"old.recvfrom"
block|,
comment|/* 125 = old recvfrom */
literal|"old.setreuid"
block|,
comment|/* 126 = old setreuid */
literal|"old.setregid"
block|,
comment|/* 127 = old setregid */
literal|"rename"
block|,
comment|/* 128 = rename */
literal|"old.truncate"
block|,
comment|/* 129 = old truncate */
literal|"old.ftruncate"
block|,
comment|/* 130 = old ftruncate */
literal|"flock"
block|,
comment|/* 131 = flock */
literal|"mkfifo"
block|,
comment|/* 132 = mkfifo */
literal|"sendto"
block|,
comment|/* 133 = sendto */
literal|"shutdown"
block|,
comment|/* 134 = shutdown */
literal|"socketpair"
block|,
comment|/* 135 = socketpair */
literal|"mkdir"
block|,
comment|/* 136 = mkdir */
literal|"rmdir"
block|,
comment|/* 137 = rmdir */
literal|"utimes"
block|,
comment|/* 138 = utimes */
literal|"obs_4.2"
block|,
comment|/* 139 = obsolete 4.2 sigreturn */
literal|"adjtime"
block|,
comment|/* 140 = adjtime */
literal|"old.getpeername"
block|,
comment|/* 141 = old getpeername */
literal|"old.gethostid"
block|,
comment|/* 142 = old gethostid */
literal|"old.sethostid"
block|,
comment|/* 143 = old sethostid */
literal|"old.getrlimit"
block|,
comment|/* 144 = old getrlimit */
literal|"old.setrlimit"
block|,
comment|/* 145 = old setrlimit */
literal|"old.killpg"
block|,
comment|/* 146 = old killpg */
literal|"setsid"
block|,
comment|/* 147 = setsid */
literal|"quotactl"
block|,
comment|/* 148 = quotactl */
literal|"old.quota"
block|,
comment|/* 149 = old quota */
literal|"old.getsockname"
block|,
comment|/* 150 = old getsockname */
literal|"#151"
block|,
comment|/* 151 = nosys */
literal|"#152"
block|,
comment|/* 152 = nosys */
literal|"#153"
block|,
comment|/* 153 = nosys */
literal|"#154"
block|,
comment|/* 154 = nosys */
ifdef|#
directive|ifdef
name|NFS
literal|"nfssvc"
block|,
comment|/* 155 = nfssvc */
else|#
directive|else
literal|"#155"
block|,
comment|/* 155 = nosys */
endif|#
directive|endif
literal|"old.getdirentries"
block|,
comment|/* 156 = old getdirentries */
literal|"statfs"
block|,
comment|/* 157 = statfs */
literal|"fstatfs"
block|,
comment|/* 158 = fstatfs */
literal|"#159"
block|,
comment|/* 159 = nosys */
literal|"#160"
block|,
comment|/* 160 = nosys */
ifdef|#
directive|ifdef
name|NFS
literal|"getfh"
block|,
comment|/* 161 = getfh */
else|#
directive|else
literal|"#161"
block|,
comment|/* 161 = nosys */
endif|#
directive|endif
literal|"#162"
block|,
comment|/* 162 = nosys */
literal|"#163"
block|,
comment|/* 163 = nosys */
literal|"#164"
block|,
comment|/* 164 = nosys */
literal|"#165"
block|,
comment|/* 165 = nosys */
literal|"#166"
block|,
comment|/* 166 = nosys */
literal|"#167"
block|,
comment|/* 167 = nosys */
literal|"#168"
block|,
comment|/* 168 = nosys */
literal|"#169"
block|,
comment|/* 169 = nosys */
literal|"#170"
block|,
comment|/* 170 = nosys */
ifdef|#
directive|ifdef
name|SYSVSHM
literal|"shmsys"
block|,
comment|/* 171 = shmsys */
else|#
directive|else
literal|"#171"
block|,
comment|/* 171 = nosys */
endif|#
directive|endif
literal|"#172"
block|,
comment|/* 172 = nosys */
literal|"#173"
block|,
comment|/* 173 = nosys */
literal|"#174"
block|,
comment|/* 174 = nosys */
literal|"#175"
block|,
comment|/* 175 = nosys */
literal|"#176"
block|,
comment|/* 176 = nosys */
literal|"#177"
block|,
comment|/* 177 = nosys */
literal|"#178"
block|,
comment|/* 178 = nosys */
literal|"#179"
block|,
comment|/* 179 = nosys */
literal|"#180"
block|,
comment|/* 180 = nosys */
literal|"setgid"
block|,
comment|/* 181 = setgid */
literal|"setegid"
block|,
comment|/* 182 = setegid */
literal|"seteuid"
block|,
comment|/* 183 = seteuid */
ifdef|#
directive|ifdef
name|LFS
literal|"lfs_bmapv"
block|,
comment|/* 184 = lfs_bmapv */
literal|"lfs_markv"
block|,
comment|/* 185 = lfs_markv */
literal|"lfs_segclean"
block|,
comment|/* 186 = lfs_segclean */
literal|"lfs_segwait"
block|,
comment|/* 187 = lfs_segwait */
else|#
directive|else
literal|"#184"
block|,
comment|/* 184 = nosys */
literal|"#185"
block|,
comment|/* 185 = nosys */
literal|"#186"
block|,
comment|/* 186 = nosys */
literal|"#187"
block|,
comment|/* 187 = nosys */
endif|#
directive|endif
literal|"stat"
block|,
comment|/* 188 = stat */
literal|"fstat"
block|,
comment|/* 189 = fstat */
literal|"lstat"
block|,
comment|/* 190 = lstat */
literal|"pathconf"
block|,
comment|/* 191 = pathconf */
literal|"fpathconf"
block|,
comment|/* 192 = fpathconf */
literal|"#193"
block|,
comment|/* 193 = nosys */
literal|"getrlimit"
block|,
comment|/* 194 = getrlimit */
literal|"setrlimit"
block|,
comment|/* 195 = setrlimit */
literal|"getdirentries"
block|,
comment|/* 196 = getdirentries */
literal|"mmap"
block|,
comment|/* 197 = mmap */
literal|"__syscall"
block|,
comment|/* 198 = __syscall */
literal|"lseek"
block|,
comment|/* 199 = lseek */
literal|"truncate"
block|,
comment|/* 200 = truncate */
literal|"ftruncate"
block|,
comment|/* 201 = ftruncate */
literal|"__sysctl"
block|,
comment|/* 202 = __sysctl */
literal|"mlock"
block|,
comment|/* 203 = mlock */
literal|"munlock"
block|,
comment|/* 204 = munlock */
literal|"#205"
block|,
comment|/* 205 = nosys */
literal|"#206"
block|,
comment|/* 206 = nosys */
literal|"#207"
block|,
comment|/* 207 = nosys */
literal|"#208"
block|,
comment|/* 208 = nosys */
literal|"#209"
block|,
comment|/* 209 = nosys */
literal|"#210"
block|,
comment|/* 210 = nosys */
block|}
decl_stmt|;
end_decl_stmt

end_unit

