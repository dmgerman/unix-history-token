...
begin_comment
comment|/*  * Take a string of network machine id's and separators and  * optimize them.  We process these by pulling off maximal  * leading strings of the same type, ppassing these to the appropriate  * optimizer and concatenating the results.  */
end_comment
...
begin_comment
comment|/*  * Return the type of the separator --  *	IMPLICIT -- if for a smart, implicitly routed network  *	EXPLICIT -- if for a dumb, explicitly routed neetwork.  *	0 if we don't know.  */
end_comment
