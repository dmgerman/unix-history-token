...
argument|tpcb
argument_list|)
comment|/* tp_p_meas is a cluster : "copy" it */
name|mclrefcnt
index|[
name|mtocl
argument_list|(
...
name|sb_mb
argument_list|,
literal|"tp_ctloutput after sbappendrecord"
argument_list|)
expr_stmt|;
name|ENDDEBUG
name|IFTRACE
...
name|sb_cc
decl_stmt|;
comment|/* getsockopt() allocated an mbuf but it's a whole lot easier 			 * to do an m_copy than to explicitly copy from the socket buf 			 * into the buffer provided by getsockopt() 			 */
name|IFDEBUG
argument_list|(
argument|D_REQUEST
argument_list|)
...
name|sb_mb
argument_list|,
literal|"tp_ctlout: tpXrcv before sbdrop"
argument_list|)
expr_stmt|;
name|ENDDEBUG
if|if
...
name|sb_mb
argument_list|,
literal|"tp_ctlout: tpXrcv after sbdrop"
argument_list|)
expr_stmt|;
name|ENDDEBUG
comment|/* a potential problem here is that REAL expedited may have arrived 			 * after the data-on-connect  			 * however, this presently works because incoming XPD_TPDUs are 			 * dropped if tp_Xrcv.sb_cc != 0 			 */
if|if
condition|(
name|tpcb
operator|->
