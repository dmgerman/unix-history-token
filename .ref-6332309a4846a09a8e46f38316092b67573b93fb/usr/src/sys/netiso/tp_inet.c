...
begin_comment
comment|/*  * NAME:	in_getnetaddr()  *  * CALLED FROM:  *  pr_usrreq() PRU_SOCKADDR, PRU_ACCEPT, PRU_PEERADDR  * FUNCTION and ARGUMENTS:  * 	Copy a whole net addr from an inpcb (inp) into  * 	a struct sockaddr (name).  * 	The argument (which) takes values TP_LOCAL or TP_FOREIGN.  *  * RETURNS:		Nada  *  * SIDE EFFECTS:	  *  * NOTES:			  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* IP layer has already pulled up the IP header */
while|while
condition|(
name|m
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * now pull up the whole tp header : we stripped all leading mbufs 	 * w/o at least one byte, so we know we can read the tpdu_li field. 	 */
name|hdr
operator|=
operator|&
operator|(
...
name|printf
argument_list|(
literal|"tpip_route: ifp from route:0x%x ro_rt 0x%x\n"
argument_list|,
name|ro
argument_list|,
name|ro
