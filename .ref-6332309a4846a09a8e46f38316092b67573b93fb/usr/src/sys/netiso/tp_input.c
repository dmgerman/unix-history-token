...
condition|)
block|{
comment|/* align to a 4-byte boundary - sigh */
specifier|register
name|struct
name|mbuf
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* we KNOW that there is at least 1 byte in this mbuf */
name|hdr
operator|=
name|mtod
argument_list|(
...
parameter_list|)
define|\
value|{	register  caddr_t		P;\ 	P = (caddr_t)(hdr) +\ 	tpdu_info[(hdr)->tpdu_type][(format)];\ 	while( P< (caddr_t)(hdr) + (int)((hdr)->tpdu_li) ) {
end_define
...
parameter_list|)
define|\
value|P = P  + 2 + (int)((struct tp_vbp *)P)->tpv_len ;\ } }
end_define
...
parameter_list|)
define|\
value|if(Phrase) { error = (Erval); errloc = (caddr_t)(Loc); IncStat(Stat); \ 	goto Whattodo; }
end_define
...
condition|)
block|{
comment|/*  		 * Flags has already been copied, now copy the data  		 * -- these data are the connect- or disconnect- data. 		 */
name|struct
name|mbuf
modifier|*
name|conndata
...
name|sb_mb
argument_list|,
literal|"new (so->)sosnd before sbapndrec"
argument_list|)
expr_stmt|;
name|dump_mbuf
argument_list|(
name|conndata
argument_list|,
literal|"conndata before sbappendrec"
argument_list|)
expr_stmt|;
name|ENDDEBUG
name|sbappendrecord
...
name|CHECK
argument_list|(
argument|(vbval(P, u_char) != TP_VERSION )
argument_list|,
...
argument|(
literal|1
argument|+ (caddr_t)&vbptr(P)->tpv_val - P)
argument_list|)
break|break;
case|case
name|TPP_acktime
...
index|]
expr_stmt|;
comment|/* zero-th one is never open */
for|for
control|(
name|r
operator|=
...
name|printf
argument_list|(
literal|"MISMATCHED AF on CR! laddr.family 0x%x expected 0x%x\n"
argument_list|,
name|laddr
operator|->
name|sa_family
...
name|TPPTmisc
argument_list|,
literal|"tp_input: ref *lsufxloc refstate"
argument_list|,
name|r
argument_list|,
operator|*
...
comment|/* l suffix is already there */
name|bcopy
argument_list|(
name|fsufxloc
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 			 * in the AF_INET case, we need the l,f addrs to contain the ports 			 */
if|if
condition|(
name|tpcb
operator|->
...
name|CHECK
argument_list|(
argument|(fsufxlen != sizeof(short))||(lsufxlen != sizeof(short))
argument_list|,
argument|E_TP_ADDR_UNK
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 					this has already been done 'cause the fsufxlen is 					sizeof(short): 					bcopy (fsufxloc,&(satosin(faddr)->sin_port),  						sizeof(short)); 					(tpcb->tp_nlproto->nlp_putsufx)(so->so_pcb, faddr,  						TP_FOREIGN); 				*/
block|}
ifdef|#
directive|ifdef
name|TP_PERF_MEAS
...
name|CHECK
argument_list|(
argument|(dusize< TP_MIN_TPDUSIZE || dusize>  						TP_MAX_TPDUSIZE)
argument_list|,
argument|E_TP_INV_PVAL
argument_list|,
argument|ts_inv_pval
...
name|CHECK
argument_list|(
argument|iso_check_csum(m, hdr->tpdu_li +
literal|1
argument|)
argument_list|,
argument|E_TP_INV_PVAL
...
name|CHECK
argument_list|(
argument|(hdr->tpdu_DCsref != tpcb->tp_fref)
argument_list|,
argument|E_TP_MISM_REFS
argument_list|,
argument|ts_inv_sufx
...
name|CHECK
argument_list|(
argument|(hdr->tpdu_DRsref != tpcb->tp_fref)
argument_list|,
argument|E_TP_MISM_REFS
argument_list|,
argument|ts_inv_sufx
...
name|CHECK
argument_list|(
argument|(max&& e.ATTR(DT_TPDU).e_datalen> max)
argument_list|,
argument|E_TP_LENGTH_INVAL
argument_list|,
argument|ts_inv_length
argument_list|,
argument|respond
argument_list|,
argument|(max + hdr->tpdu_li +
literal|1
argument|)
argument_list|)
name|e
...
name|printf
argument_list|(
literal|"RESPOND: error 0x%x, errloc 0x%x\n"
argument_list|,
name|error
argument_list|,
name|errloc
