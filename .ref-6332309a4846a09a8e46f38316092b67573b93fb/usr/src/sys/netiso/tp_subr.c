...
begin_comment
comment|/* For xpd marks we use mbufs of a special type with length 0;  * the m_next field is really the seq number of the xpd tpdu that  * must be acked before more normal data may be sent  */
end_comment
...
name|n
decl_stmt|;
comment|/* link field we'll be modifying when we 								take mb-->m out of the socket buffer */
name|struct
name|mbuf
modifier|*
name|nextrecord
...
condition|)
block|{
comment|/* stop sending here because there are unacked XPD which were  				 * given to us before the next data were. Leave mark in place. 				 */
name|IncStat
argument_list|(
...
break|break;
block|}
comment|/* otherwise, mark is obsolete; delete it */
name|sbfree
argument_list|(
name|sb
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* have to do this to delete the sb_mbcnt */
name|sb
operator|->
name|sb_mb
operator|=
...
name|TPPTmisc
argument_list|,
literal|"tp_send XPD mark deleted mnext old_act new_act"
argument_list|,
name|mnext
argument_list|,
name|sb
...
parameter_list|(
function_decl|"tp_send XPD mark deleted mnext 0x%x old act 0x%x new act 0x%x\n"
operator|,
function_decl|mnext
operator|,
function_decl|sb->sb_mb
operator|,
function_decl|mnext->m_act
operator|,
function_decl|0
block|)
empty_stmt|;
...
name|m
expr_stmt|;
comment|/* meaningless first time through the loop */
name|len
operator|+=
name|m
operator|->
...
condition|)
block|{
comment|/*   				 * Won't use the whole mbuf - split into 2 mbufs.  				 */
name|int
name|amount
init|=
name|m
...
name|mx
decl_stmt|;
comment|/* copy the part we are NOT using and put that back in the 				 * socket buf; leave m with this tpdu chain; adjust its fields 				 */
name|IFTRACE
argument_list|(
argument|D_STASH
argument_list|)
...
name|TPPTmisc
argument_list|,
literal|"tp_send SPLIT len, amount, m->m_len, tpdusize"
argument_list|,
name|len
argument_list|,
name|amount
...
name|amount
expr_stmt|;
comment|/* would do an sbfree but don't want the mbcnt to be 				 * decremented since it was never sballoced  				 */
name|sb
operator|->
name|sb_cc
operator|-=
...
break|break;
block|}
comment|/* going to use the whole mbuf */
name|IFTRACE
argument_list|(
argument|D_STASH
argument_list|)
...
literal|0
expr_stmt|;
comment|/* unlink the to-be-sent stuff from 				the stuff still in the sb_mb so when we do the m_free 				it won't clobber part of the socket buffer */
block|}
end_while
...
comment|/* a kludgy optimization would be to take care of this in 		 * tp_input (oh, horrors!)  		 * BTW, don't set ack_reason here because we don't know if the 		 * sequence number is right 		 */
while|while
condition|(
name|n
operator|->
...
name|printf
argument_list|(
literal|"EOT! changing m_type of m 0x%x\n"
argument_list|,
name|n
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|/* 'cause sbappend won't append something of length zero */
block|}
else|else
block|{
name|IFDEBUG
...
name|printf
argument_list|(
literal|"stash EQ: plain old append\n"
argument_list|)
expr_stmt|;
name|ENDDEBUG
name|sbappend
