...
begin_comment
comment|/* 	The daemon program that runs the network.  	netdaemon mach readfd writefd  	Must be started by root. */
end_comment
...
name|addtopublic
argument_list|(
literal|"%s: sent %-8s to %c (%s, %ld b, wait %s)\n"
argument_list|,
name|sdate
argument_list|,
name|sn
...
name|diff
expr_stmt|;
comment|/* add this users name to /usr/net/usernames */
name|addtoname
argument_list|(
name|sn
argument_list|)
...
condition|)
block|{
comment|/* 		error("versions dont agree (%d,%d) vs. (%d,%d) remote", 			VMAJOR,VMINOR,vmajor,vminor); 		return(-1); 		*/
block|}
name|i
operator|+=
name|mgets
...
argument_list|)
expr_stmt|;
comment|/* keep variable parameter list in jobno slot */
name|parseparmlist
argument_list|(
name|parmlist
argument_list|)
...
goto|;
block|}
comment|/* allow larger files between the Ingres machines */
if|if
condition|(
name|machtype
index|[
...
begin_comment
comment|/*     check() -- verify login name and password    snTo    = login,    pass    = passwd,    verify  = 1 if password must check    Returns 1 if password is ok, 0 if not. */
end_comment
...
begin_comment
comment|/* errormsg- sends error message to user-    may be on local or remote machine.    Sends to addrfrom if fmach != local,    or to addrto if fmach=local, on the assumption    that addrfrom is where it came from and addrto is where it    was destined on this machine.  The only time fmach=local is    if the result file is screwed up.     Note that errormsg can be called by the netrcv subroutine    after the setuid() call to the specific user, so the     user must be able to get off an error msg back to him,    and to write in the two log files.    Can't use -w,-x,-y,-z for the net cmd because must be root for those. */
end_comment
...
condition|)
block|{
comment|/* send to local mach, thus send to toaddr. */
