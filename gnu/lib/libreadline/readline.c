...
name|CRAY
argument_list|)
comment|/* If you cast map[key].function to type (Keymap) on a Cray, 	     the compiler takes the value of may[key].function and 	     divides it by 4 to convert between pointer types (pointers 	     to functions and pointers to structs are different sizes). 	     This is not what is wanted. */
name|rl_dispatch
argument_list|(
name|newkey
argument_list|,
...
name|NULL
expr_stmt|;
comment|/* Does the _GO32_ have a meta key?  I don't know. */
name|term_has_meta
operator|=
literal|0
expr_stmt|;
...
directive|if
literal|0
block|if (rl_point&& the_line[rl_point - 1] != ' ')     rl_insert_text (" ");
endif|#
directive|endif
name|rl_insert_text
argument_list|(
