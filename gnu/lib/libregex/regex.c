...
begin_comment
comment|/* This is for multi byte string support.  */
end_comment
...
begin_comment
comment|/* unsigned character type */
end_comment
...
value|((__alignof__(wctype_t)+sizeof(wctype_t))/sizeof(CHAR_TYPE)+1)
end_define
...
parameter_list|)
define|\
value|do {									      \     if (MB_CUR_MAX == 1)						      \       putchar (c);							      \     else								      \       printf ("%C", (wint_t) c);
...
parameter_list|)
define|\
value|do {									\     *(destination) = (US_CHAR_TYPE)(number);				\   } while (0)
end_define
...
condition|)
block|{
comment|/* We don't need conversion.  */
for|for
control|(
init|;
name|wc_count
...
parameter_list|)
define|\
value|fail_stack.stack[fail_stack.avail++].pointer = (US_CHAR_TYPE *) (item)
end_define
...
parameter_list|)
define|\
value|{									\   DEBUG_STATEMENT (unsigned failure_id;)				\   active_reg_t this_reg;						\   const US_CHAR_TYPE *string_temp;					\ 									\   assert (!FAIL_STACK_EMPTY ());					\ 									\
...
value|\   string_temp = POP_FAILURE_POINTER ();					\   if (string_temp != NULL)						\     str = (const CHAR_TYPE *) string_temp;				\ 									\   DEBUG_PRINT2 ("  Popping string %p: `", str);				\   DEBUG_PRINT_DOUBLE_STRING (str, string1, size1, string2, size2);	\   DEBUG_PRINT1 ("'\n");							\ 									\   pat = (US_CHAR_TYPE *) POP_FAILURE_POINTER ();			\   DEBUG_PRINT2 ("  Popping pattern %p:\n", pat);			\   DEBUG_PRINT_COMPILED_PATTERN (bufp, pat, pend);			\ 									\
...
parameter_list|)
define|\
value|do {if (p == pend) return REG_EEND;					\     c = (US_CHAR_TYPE) *p++;						\     if (translate&& (c<= 0xff)) c = (US_CHAR_TYPE) translate[c];	\   } while (0)
end_define
...
parameter_list|)
define|\
value|do {if (p == pend) return REG_EEND;					\     c = (US_CHAR_TYPE) *p++; 						\   } while (0)
end_define
...
parameter_list|)
define|\
value|((translate&& ((US_CHAR_TYPE) (d))<= 0xff) \    ? (char) translate[(unsigned char) (d)] : (d))
end_define
...
parameter_list|)
define|\
value|while (((unsigned long)b - (unsigned long)COMPILED_BUFFER_VAR	\             + (n)*sizeof(CHAR_TYPE))> bufp->allocated)			\       EXTEND_BUFFER ()
end_define
...
parameter_list|)
define|\
value|do {									\     GET_BUFFER_SPACE (1);						\     *b++ = (US_CHAR_TYPE) (c);						\   } while (0)
end_define
...
parameter_list|)
define|\
value|do {									\     GET_BUFFER_SPACE (2);						\     *b++ = (US_CHAR_TYPE) (c1);					\     *b++ = (US_CHAR_TYPE) (c2);					\   } while (0)
end_define
...
parameter_list|)
define|\
value|do {									\     GET_BUFFER_SPACE (3);						\     *b++ = (US_CHAR_TYPE) (c1);					\     *b++ = (US_CHAR_TYPE) (c2);					\     *b++ = (US_CHAR_TYPE) (c3);					\   } while (0)
end_define
...
parameter_list|)
define|\
value|store_op1 (op, loc, (int) ((to) - (loc) - (1 + OFFSET_ADDRESS_SIZE)))
end_define
...
parameter_list|)
define|\
value|store_op2 (op, loc, (int) ((to) - (loc) - (1 + OFFSET_ADDRESS_SIZE)), arg)
end_define
...
parameter_list|)
define|\
value|insert_op1 (op, loc, (int) ((to) - (loc) - (1 + OFFSET_ADDRESS_SIZE)), b)
end_define
...
parameter_list|)
define|\
value|insert_op2 (op, loc, (int) ((to) - (loc) - (1 + OFFSET_ADDRESS_SIZE)),\ 	      arg, b)
end_define
...
parameter_list|()
define|\
value|do {									\     US_CHAR_TYPE *old_buffer = COMPILED_BUFFER_VAR;			\     int wchar_count;							\     if (bufp->allocated + sizeof(US_CHAR_TYPE)> MAX_BUF_SIZE)		\       return REG_ESIZE;							\     bufp->allocated<<= 1;						\     if (bufp->allocated> MAX_BUF_SIZE)					\       bufp->allocated = MAX_BUF_SIZE;					\
comment|/* How many characters the new buffer can have?  */
value|\     wchar_count = bufp->allocated / sizeof(US_CHAR_TYPE);		\     if (wchar_count == 0) wchar_count = 1;				\
comment|/* Truncate the buffer to CHAR_TYPE align.  */
value|\     bufp->allocated = wchar_count * sizeof(US_CHAR_TYPE);		\     RETALLOC (COMPILED_BUFFER_VAR, wchar_count, US_CHAR_TYPE);		\     bufp->buffer = (char*)COMPILED_BUFFER_VAR;				\     if (COMPILED_BUFFER_VAR == NULL)					\       return REG_ESPACE;						\
...
parameter_list|()
define|\
value|do {									\     US_CHAR_TYPE *old_buffer = COMPILED_BUFFER_VAR;			\     if (bufp->allocated == MAX_BUF_SIZE)				\       return REG_ESIZE;							\     bufp->allocated<<= 1;						\     if (bufp->allocated> MAX_BUF_SIZE)					\       bufp->allocated = MAX_BUF_SIZE;					\     bufp->buffer = (US_CHAR_TYPE *) REALLOC (COMPILED_BUFFER_VAR,	\ 						bufp->allocated);	\     if (COMPILED_BUFFER_VAR == NULL)					\       return REG_ESPACE;						\
comment|/* If the buffer moved, move all the pointers into it.  */
value|\     if (old_buffer != COMPILED_BUFFER_VAR)				\       {									\ 	int incr = COMPILED_BUFFER_VAR - old_buffer;			\ 	MOVE_BUFFER_POINTER (b);					\ 	MOVE_BUFFER_POINTER (begalt);					\ 	if (fixup_alt_jump)						\ 	  MOVE_BUFFER_POINTER (fixup_alt_jump);				\ 	if (laststart)							\ 	  MOVE_BUFFER_POINTER (laststart);				\ 	if (pending_exact)						\ 	  MOVE_BUFFER_POINTER (pending_exact);				\       }									\     ELSE_EXTEND_BUFFER_HIGH_BOUND					\   } while (0)
end_define
...
parameter_list|)
define|\
value|{									\     while (p != pend)							\       {									\ 	PATFETCH (c);							\ 	if (! ('0'<= c&& c<= '9'))					\ 	  break;							\ 	if (num<= RE_DUP_MAX)						\ 	  {								\ 	    if (num< 0)						\ 	      num = 0;							\ 	    num = num * 10 + c - '0';					\ 	  }								\       }									\   }
end_define
...
comment|/* ifdef MBS_SUPPORT, 'b + 1 + OFFSET_ADDRESS_SIZE' instead of 	       'b + 3'.  */
name|GET_BUFFER_SPACE
argument_list|(
literal|1
operator|+
...
comment|/* ifdef MBS_SUPPORT, The '1+2*OFFSET_ADDRESS_SIZE' 			is the 'set_number_at', plus '1+OFFSET_ADDRESS_SIZE' 			of the following `succeed_n'.  */
name|insert_op2
argument_list|(
name|set_number_at
argument_list|,
...
begin_comment
comment|/* ifdef MBS_SUPPORT, integer parameter is 1 wchar_t.  */
end_comment
...
parameter_list|)
define|\
value|(iswalnum ((wint_t)((d) == end1 ? *string2				\            : (d) == string2 - 1 ? *(end1 - 1) : *(d))) != 0)
end_define
...
parameter_list|()
define|\
value|do {									\     REGEX_FREE_STACK (fail_stack.stack);				\     FREE_VAR (regstart);						\     FREE_VAR (regend);							\     FREE_VAR (old_regstart);						\     FREE_VAR (old_regend);						\     FREE_VAR (best_regstart);						\     FREE_VAR (best_regend);						\     FREE_VAR (reg_info);						\     FREE_VAR (reg_dummy);						\     FREE_VAR (reg_info_dummy);						\     FREE_VAR (string1);							\     FREE_VAR (string2);							\     FREE_VAR (mbs_offset1);						\     FREE_VAR (mbs_offset2);						\   } while (0)
end_define
...
parameter_list|()
define|\
value|do {									\     FREE_VAR (string1);							\     FREE_VAR (string2);							\     FREE_VAR (mbs_offset1);						\     FREE_VAR (mbs_offset2);						\   } while (0)
end_define
...
name|length
condition|)
comment|/* It is a fragment of a wide character.  */
return|return
operator|-
literal|1
return|;
block|}
comment|/* We reached at the sentinel.  */
return|return
operator|-
literal|1
return|;
...
directive|ifdef
name|MBS_SUPPORT
comment|/* We need wchar_t* buffers correspond to string1, string2.  */
name|CHAR_TYPE
modifier|*
name|string1
init|=
