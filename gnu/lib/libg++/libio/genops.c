...
begin_comment
unit|int _IO_switch_to_put_mode(fp)      register _IO_FILE *fp; {   fp->_IO_write_base = fp->_IO_read_ptr;   fp->_IO_write_ptr = fp->_IO_read_ptr;
...
begin_else
unit|void _IO_set_column(fp, c)      register _IO_FILE *fp;      int c; {   if (c == -1)     fp->_collumn = -1;   else     fp->_collumn = c - (fp->_IO_write_ptr - fp->_IO_write_base); }
else|#
directive|else
end_else
