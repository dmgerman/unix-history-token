...
begin_comment
unit|class parsestream : public istream {     streammarker marks[2];     short _first;
comment|// of the two marks; either 0 or 1
end_comment

begin_endif
unit|int _lineno;     int first() { return _first; }     int second() { return 1-_first; }     int line_length() { marks[second].delta(marks[first]); }     int line_length() { marks[second].delta(marks[first]); }     int seek_in_line(int i);     int tell_in_line();     int line_number(); };
endif|#
directive|endif
end_endif
