...
argument_list|)
expr_stmt|;
comment|/* SUM might be identical to either operand, so don't store the      result there until we are finished with the input operands.  We      dare to overwrite the numerator of SUM when we are finished      with the numerators of A1 and A2.  */
name|mpz_gcd
argument_list|(
operator|&
name|gcd1
