...
goto|;
block|}
comment|/*        * Try harder than usual to fit the minor number.  to_oct() wastes        * one byte by adding 2 terminating bytes (a space and a NUL) where        * only 1 (either a space or a NUL) is required by POSIX.1.  We        * will discard the NUL if that is necessary and sufficient for the        * minor number to fit.  This is compatible with tar-1.13.        */
if|if
condition|(
name|checked_to_oct
argument_list|(
