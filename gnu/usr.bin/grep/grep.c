...
begin_comment
comment|/* Return the address of a new page-aligned buffer of size SIZE.  Set    *UP to the newly allocated (but possibly unaligned) buffer used to    *build the aligned buffer.  To free the buffer, free (*UP).  */
end_comment
...
decl_stmt|;
block|{
comment|/* HAVE_WORKING_VALLOC means that valloc is properly declared, and      you can free the result of valloc.  This symbol is not (yet)      autoconfigured.  It can be useful to define HAVE_WORKING_VALLOC      while debugging, since some debugging memory allocators might      catch more bugs if this symbol is enabled.  */
if|#
directive|if
name|HAVE_WORKING_VALLOC
operator|*
name|up
operator|=
name|valloc
...
condition|)
block|{
comment|/* This used to issue a warning, but on some hosts              (e.g. Solaris 2.5) mmap can fail merely because some              other process has an advisory read lock on the file.              There's no point alarming the user about this misfeature.  */
if|#
directive|if
literal|0
block|fprintf (stderr, _("%s: warning: %s: %s\n"), prog, filename, 		  strerror (errno));
endif|#
directive|endif
goto|goto
name|tryread
...
directive|if
name|O_BINARY
literal|"0123456789A:B:C::EFGHVX:abcd:e:f:hiLlnqrsvwxyUu"
argument_list|,
else|#
directive|else
