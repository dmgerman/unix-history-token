...
begin_comment
comment|/* Flag saying to print the full filename of libgcc.a    as found through our usual search mechanism.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|print_libgcc_file_name
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* The compiler version specified with -V */
end_comment
...
parameter_list|)
define|\
value|(!strcmp (STR, "Tdata") || !strcmp (STR, "Ttext")	\   || !strcmp (STR, "Tbss") || !strcmp (STR, "include")	\   || !strcmp (STR, "imacros") || !strcmp (STR, "aux-info") \   || !strcmp (STR, "idirafter") || !strcmp (STR, "iprefix") \   || !strcmp (STR, "iwithprefix"))
end_define
...
literal|"@assembler"
block|,
literal|"%{!S:as %{R} %{j} %{J} %{h} %{d2} %a %Y\             %{c:%W{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%u.o} %i %A\n }"
block|}
block|,
block|{
literal|".S"
...
literal|"@fortran-f2c"
block|,
literal|"cat %i %{!pipe:%eGNU Fortran has to have -pipe option}|\n"
block|,
literal|"f2c %{checksubscripts:-C} %{I2} %{onetrip} %{honorcase:-U} %{u} %{w}\         %{ANSIC:-A} %{a} %{C++}\         %{c} %{E} %{ec} %{ext} %{f} %{72} %{g} %{h} %{i2} %{kr}\         %{P} %{p} %{r} %{r8} %{s} %{w8} %{z}|\n"
block|,
literal|"gcpp -lang-c %{nostdinc*} %{C} %{v} %{A*} %{I*} %{P} %I\ 	%{C:%{!E:%eGNU C does not support -C without using -E}}\ 	%{M} %{MM} %{MD:-MD %b.d} %{MMD:-MMD %b.d}\         -undef -D__GNUC__=2 %{ansi:-trigraphs -$ -D__STRICT_ANSI__}\ 	%{!undef:%{!ansi:%p} %P} %{trigraphs} \         %c %{O*:%{!O0:-D__OPTIMIZE__}} %{traditional} %{ftraditional:-traditional}\         %{traditional-cpp:-traditional}\ 	%{g*} %{W*} %{w} %{pedantic*} %{H} %{d*} %C %{D*} %{U*} %{i*}\         - %{!M:%{!MM:%{!E:%{!pipe:%g.i}}}}%{E:%W{o*}}%{M:%W{o*}}%{MM:%W{o*}} |\n"
block|,
literal|"%{!M:%{!MM:%{!E:gcc1 %{!pipe:%g.i} %1 \ 		   %{!Q:-quiet} -dumpbase %b.c %{d*} %{m*} %{a}\ 		   %{g*} %{O*} %{W*} %{w} %{pedantic*} %{ansi} \ 		   %{traditional} %{v:-version} %{pg:-p} %{p} %{f*}\ 		   %{aux-info*}\ 		   %{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\ 		   %{S:%W{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\               %{!S:as %{R} %{j} %{J} %{h} %{d2} %a %Y\ 		      %{c:%W{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%u.o}\                       %{!pipe:%g.s} %A\n }}}}"
...
condition|)
block|{
comment|/* Delete only ordinary files.  */
if|if
condition|(
name|S_ISREG
argument_list|(
...
literal|0
expr_stmt|;
comment|/* Default for -V is our version number, ending at first space.  */
name|spec_version
operator|=
name|save_string
argument_list|(
