...
parameter_list|)
define|\
value|((CHAR) == 'D' || (CHAR) == 'U' || (CHAR) == 'o' \    || (CHAR) == 'e' || (CHAR) == 'T' || (CHAR) == 'u' \    || (CHAR) == 'I' || (CHAR) == 'm' \    || (CHAR) == 'L' || (CHAR) == 'A')
end_define
...
literal|"@assembler"
block|,
literal|"%{!M:%{!MM:%{!E:%{!S:as %{R} %{j} %{J} %{h} %{d2} %a %Y\ 		            %{c:%W{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%u.o}\ 			    %i %A\n }}}}"
block|}
block|,
block|{
literal|".S"
...
literal|"@ada"
block|,
literal|"%{!M:%{!MM:%{!E:gnat1 %{k8:-gnatk8} %{w:-gnatws} %{!Q:-quiet}\ 			   -dumpbase %b.ada %{g*} %{O*} %{p} %{pg:-p} %{f*}\ 			  %{d*}\ 			  %{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\ 			  %i %{S:%W{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\ 		    %{!S:%{!gnatc:%{!gnats:as %{R} %{j} %{J} %{h} %{d2} %a %Y\ 					      %{c:%W{o*}%{!o*:-o %w%b.o}}\ 					      %{!c:-o %d%w%u.o} %{!pipe:%g.s} %A\n}}}}}} "
block|}
block|,
comment|/***** ljo's Fortran rule *****/
block|{
literal|".f"
block|,
literal|"@f2c"
...
block|}
block|,
comment|/***** End of ljo's Fortran rule *****/
...
begin_comment
comment|/* Have gcc do the search for libgcc.a, but generate -L options as usual.  */
end_comment
...
init|=
literal|"\ %{!fsyntax-only: \  %{!c:%{!M:%{!MM:%{!E:%{!S:ld %l %X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} \ 			%{r} %{s} %{t} %{u*} %{x} %{z} %{Z}\ 			%{!A:%{!nostartfiles:%{!nostdlib:%S}}} %{static:}\ 			%{L*} %D %{T*} %o %{!nostdlib:libgcc.a%s %L libgcc.a%s %{!A:%E}}\n }}}}}}"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Have gcc do the search for libgcc.a, and don't generate -L options.  */
end_comment
...
name|link_command_spec
init|=
literal|"\ %{!fsyntax-only: \  %{!c:%{!M:%{!MM:%{!E:%{!S:ld %l %X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} \ 			%{r} %{s} %{t} %{u*} %{x} %{z} %{Z}\ 			%{!A:%{!nostartfiles:%{!nostdlib:%S}}} %{static:}\ 			%{L*} %{T*} %o %{!nostdlib:libgcc.a%s %L libgcc.a%s %{!A:%E}}\n }}}}}}"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Use -L and have the linker do the search for -lgcc.  */
end_comment
...
name|link_command_spec
init|=
literal|"\ %{!fsyntax-only: \  %{!c:%{!M:%{!MM:%{!E:%{!S:ld %l %X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} \ 			%{r} %{s} %{t} %{u*} %{x} %{z} %{Z}\ 			%{!A:%{!nostartfiles:%{!nostdlib:%S}}} %{static:}\ 			%{L*} %D %{T*} %o %{!nostdlib:-lgcc %L -lgcc %{!A:%E}}\n }}}}}}"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* A vector of options to give to the linker.    These options are accumulated by -Xlinker and -Wl,    and substituted into the linker command with %X.  */
end_comment
...
begin_comment
comment|/* A vector of options to give to the assembler.    These options are accumulated by -Wa,    and substituted into the assembler command with %X.  */
end_comment
...
name|equivalent
decl_stmt|;
comment|/* Argument info.  A string of flag chars; NULL equals no options.      a => argument required.      o => argument optional.      j => join argument to equivalent, making one word.      * => allow other text after NAME as an argument.  */
name|char
modifier|*
name|arg_info
decl_stmt|;
...
literal|1
expr_stmt|;
comment|/* If this mapping allows extra text at end of name, 			 accept that as "argument".  */
elseif|else
if|if
condition|(
name|index
...
begin_comment
comment|/* Declare these to avoid compilation error.  They won't be called.  */
end_comment
...
condition|)
block|{
comment|/* Pass the argument of this option to the linker when we link.  */
if|if
condition|(
name|i
operator|+
...
name|j
decl_stmt|;
comment|/* Pass the rest of this option to the linker when we link.  */
name|n_linker_options
operator|++
expr_stmt|;
if|if
...
argument_list|)
expr_stmt|;
comment|/* As a kludge, if the arg is "[foo/]stageN/", just add 		   "[foo/]stageN/../include" to the include prefix.  */
block|{
name|int
name|len
init|=
...
argument_list|)
expr_stmt|;
comment|/* If tooldir is relative, base it on exec_prefix.  A relative      tooldir lets us move the installed tree as a unit.       If GCC_EXEC_PREFIX is defined, then we want to add two relative      directories, so that we can search both the user specified directory      and the standard place.  */
if|if
condition|(
operator|*
name|tooldir_prefix
...
block|}
break|break;
comment|/* Dump out the options accumulated previously using %x, 	     -Xlinker and -Wl,.  */
case|case
literal|'X'
case|:
for|for
...
block|add_prefix (&startfile_prefix, "./", 0, 1, NULL_PTR);
endif|#
directive|endif
block|}
