...
comment|/* Do this, until we can undo any previous convertions.  See net35.C      for a testcase. */
name|fixed_type_p
operator|=
name|complete_type_p
argument_list|(
...
name|cp_error_at
argument_list|(
literal|"duplicate class scope type `%D'"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
name|cp_error_at
argument_list|(
literal|"cannot make protected `%D' public"
argument_list|,
name|fdecl
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Get the offset to the start of the original binfo that we derived this    binfo from.  */
end_comment
...
literal|0
expr_stmt|;
comment|/* Skip initial vtable length field and RTTI fake object. */
for|for
control|(
init|;
name|virtuals
...
directive|if
literal|0
block|my_friendly_assert (*binfo2_ptr == binfo, 999); 	    *binfo2_ptr = copy_binfo (binfo);
endif|#
directive|endif
block|}
if|if
...
begin_comment
comment|/* Fixup all the delta entries in this vtable that need updating.    This happens when we have non-overridden virtual functions from a    virtual base class, that are at a different offset, in the new    hierarchy, because the layout of the virtual bases has changed.  */
end_comment
...
block|}
block|{
comment|/* This MUST be overriden, or the class is ill-formed.  */
...
block|struct tinst_level *til = tinst_for_decl();        if (til) 	{ 	  DECL_SOURCE_FILE (name) = til->file; 	  if (DECL_SOURCE_LINE (name)) 	    DECL_SOURCE_LINE (name) = til->line; 	}       else
endif|#
directive|endif
block|{
specifier|extern
...
condition|)
block|{
comment|/* Make sure we set this up.  In find_scoped_type, it explicitly 		 looks for a TYPE_DECL in the TYPE_FIELDS list.  If we don't 		 do this here, we'll miss including this TYPE_DECL in the 		 list.  */
if|if
condition|(
operator|!
name|fields
...
name|cp_pedwarn_at
argument_list|(
literal|"member `%#D' with only non-default constructor"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
name|cp_pedwarn_at
argument_list|(
literal|"member with only non-default constructor"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* We might be able to shorten the ammount of work we do by 	     only doing this for vtables that come from virtual bases 	     that have differing offsets, but don't want to miss any 	     entries.  */
name|fixup_vtable_deltas
argument_list|(
name|vbases
argument_list|,
...
directive|if
literal|0
comment|/* What's wrong with using the decl the type already has? */
block|tree tag = build_decl (TYPE_DECL, TREE_PURPOSE (x), TREE_VALUE (x)); 	  DECL_CONTEXT (tag) = t;
else|#
directive|else
name|tree
name|tag
...
name|item
decl_stmt|;
comment|/* Hooray, our cacheing was successful, let's just install the 	     cached class_shadowed list, and walk through it to get the 	     IDENTIFIER_TYPE_VALUEs correct.  */
name|set_class_shadows
argument_list|(
name|previous_class_values
argument_list|)
