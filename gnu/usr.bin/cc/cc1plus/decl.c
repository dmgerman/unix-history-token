...
condition|)
block|{
comment|/* Since we're doing this before finish_struct can set the 	     line number on NEWDECL, we just do a regular error here.  */
if|if
condition|(
...
name|cp_error
argument_list|(
literal|"redeclaration of exception `%T'"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* A signature type will contain the fields of the signature 	     table.  Therefore, it's not only an interface.  */
if|if
condition|(
name|tag_code
operator|==
...
expr_stmt|;
block|}
comment|/* Signatures don't have a vtable.  As long as we don't have default 	     implementations, they behave as if `write_virtuals' were 3.  */
if|if
condition|(
name|tag_code
operator|==
...
argument_list|)
condition|)
comment|/* PUBLIC and EXTERNAL set by do_*_instantiation */
empty_stmt|;
else|else
block|{
