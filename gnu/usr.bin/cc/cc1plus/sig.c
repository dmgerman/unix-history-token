...
expr_stmt|;
block|}
comment|/* A signature pointer or reference to a signature `s' looks like this:         struct {          void * optr; 	 const s * sptr; 	 vtbl_type_node * vptr;        };       A `const' signature pointer/reference is a         struct {          const void * optr; 	 const s * sptr; 	 vtbl_type_node * vptr;        };       Similarly, for `volatile' and `const volatile'.    */
name|t
operator|=
name|make_lang_type
argument_list|(
...
condition|)
block|{
comment|/* We can share `sptr' and `vptr' among type variants.  */
name|sptr
operator|=
name|TREE_CHAIN
argument_list|(
...
name|optr
expr_stmt|;
comment|/* To make `build_vfn_ref' work when building a signature method call.  */
...
name|ambient_saveable_obstack
expr_stmt|;
comment|/* Ouput debug information for this type.  */
...
name|NULL_TREE
decl_stmt|;
comment|/* For signatures it should actually be only a list with one element.  */
for|for
control|(
name|l
operator|=
...
name|error
argument_list|(
literal|"class `%s' does not contain a method conforming to `%s'"
argument_list|,
name|TYPE_NAME_STRING
argument_list|(
name|rhstype
...
name|sorry
argument_list|(
literal|"class with multiple inheritance as implementation of signature"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
begin_comment
comment|/* Build a signature member function call.  Looks up the signature table    entry corresponding to FUNCTION.  Depending on the value of the CODE    field, either call the function in PFN directly, or use OFFSET to    index INSTANCE's virtual function table.  */
end_comment
...
begin_comment
comment|/* Create a COMPONENT_REF expression for referencing the VPTR field    of a signature pointer or reference.  */
end_comment
