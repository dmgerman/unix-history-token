...
directive|if
literal|0
else|my_friendly_abort (346);
else|#
directive|else
comment|/* Using ctype fixes the `X::Y::~Y()' crash.  The cname has no type when 	 it's defined out of the class definition, since poplevel_class wipes 	 it out.  */
name|dbuf
operator|=
name|build_overload_name
argument_list|(
...
begin_comment
comment|/* Generate errors possibly applicable for a given set of specifiers.  */
end_comment
...
name|error
argument_list|(
literal|"`const' and `volatile' function specifiers invalid in %s declaration"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|error
argument_list|(
literal|"argument list for `%s' does not match any in class `%s'"
argument_list|,
name|err_name
argument_list|,
name|IDENTIFIER_POINTER
...
name|error
argument_list|(
literal|"no `%s' member function declared in class `%s'"
argument_list|,
name|err_name
argument_list|,
name|IDENTIFIER_POINTER
...
name|error
argument_list|(
literal|"no %s declared in class `%s'"
argument_list|,
name|err_name
argument_list|,
name|IDENTIFIER_POINTER
...
begin_comment
comment|/* Note even though prev is never used in here, walk_vtables    expects this to have two arguments, so concede.  */
end_comment
