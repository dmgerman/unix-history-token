...
begin_comment
comment|/* Build expressions with type checking for C++ compiler.    Copyright (C) 1987, 1988, 1989, 1992, 1993 Free Software Foundation, Inc.    Hacked by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
directive|if
literal|0
block|case POINTER_TYPE:       return build_pointer_type (common_type (TREE_TYPE (t1), TREE_TYPE (t2)));      case REFERENCE_TYPE:       return build_reference_type (common_type (TREE_TYPE (t1), TREE_TYPE (t2)));
endif|#
directive|endif
case|case
name|ARRAY_TYPE
...
name|warning
argument_list|(
literal|"Internal error: t2 == error_mark_node in `comptypes'"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|basetype
argument_list|,
literal|"destructor specifier `%s::~%s' must have matching names"
argument_list|,
name|IDENTIFIER_POINTER
argument_list|(
name|TREE_OPERAND
...
name|error
argument_list|(
literal|"%s has no such type conversion operator"
argument_list|,
name|code
operator|==
name|RECORD_TYPE
...
name|error
argument_list|(
literal|"invalid use of type decl `%s' as expression"
argument_list|,
name|IDENTIFIER_POINTER
argument_list|(
name|DECL_NAME
...
name|pedwarn
argument_list|(
literal|"ANSI C++ forbids passing objects of type `%s' through `...'"
argument_list|,
...
name|rval
decl_stmt|;
comment|/* If there's any way we can call this function, do so.  */
if|if
condition|(
name|rval
operator|=
...
condition|)
block|{
comment|/* If it's accessible, we win.  */
if|if
condition|(
name|rval
operator|=
...
index|]
argument_list|,
literal|"no member function `%s::operator %s'"
argument_list|,
name|opname_tab
index|[
operator|(
...
name|rval
decl_stmt|;
comment|/* See comments in `build_x_unary_op'.  */
if|if
condition|(
name|rval
operator|=
...
name|pedwarn
argument_list|(
literal|"ANSI C++ forbids %sing a pointer of type `%s'"
argument_list|,
operator|(
operator|(
name|code
...
name|error
argument_list|(
literal|"attempt to take address of function `main'"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
return|;
block|}
comment|/* This possible setting of TREE_CONSTANT is what makes it possible 		 with an initializer list to emit the entire thing in the data 		 section, rather than a run-time initialization.  */
name|arg
operator|=
name|build1
argument_list|(
...
block|}
block|}
comment|/* When converting into a reference type, just convert into a pointer      to the new type and deference it.  While this is not exactly what ARM 5.4      calls for, it is pretty close for now.  (int&)ri  --->  *(int*)&ri  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
condition|)
block|{
comment|/* For C++, we must copy the constness of TYPE into VALUE.  */
if|if
condition|(
name|TREE_READONLY
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Make up for the fact that we do not always perform      `default_conversion' anymore.  */
if|if
condition|(
name|TREE_READONLY_DECL_P
argument_list|(
...
return|;
block|}
comment|/* It is now illegal to assign unions which contain members that      have non-default assignment operators.  */
if|if
condition|(
operator|!
name|flag_traditional
...
name|error
argument_list|(
literal|"invalid initialization of union containing members with X(X&) constructor"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
name|error
argument_list|(
literal|"invalid assignment of union containing members with non-default operator="
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
argument_list|,
literal|"float or double used for argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"negative value passed as argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"float or double passed as enumeral data type for argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"enumeral value passed as real data type for argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"passing integer as enumeral data type for argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"pointer to const given for argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"pointer to volatile given for argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
name|warn_for_assignment
argument_list|(
literal|"%s between function pointer and `void *'"
argument_list|,
literal|"function pointer and `void *' incompatible; argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"passing signed pointer to unsigned pointer argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"passing unsigned pointer to signed pointer argument %d of `%s'"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
name|error
argument_list|(
literal|"incompatible pointer types for argument %d of `%s'"
argument_list|,
name|parmnum
operator|+
literal|1
...
argument_list|,
literal|"passing integer to pointer argument %d of `%s' lacks a cast"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
argument_list|,
literal|"passing pointer to integer argument %d of `%s' lacks a cast"
argument_list|,
name|errtype
argument_list|,
name|fndecl
...
begin_comment
comment|/* Print a warning using either ANON_MSG or NAMED_MSG.    ANON_MSG is used if DECL and FUNCTION are 0; it gets one parameter, OPNAME.    NAMED_MSG is used if DECL is non-0;    it gets two parameters, the name of DECL and that of FUNCTION.    FUNCTION_MSG is used if DECL is 0 and FUNCTION is non-0;    it gets one parameter, the name FUNCTION.     If SEVERE is non-0, the report an error instead of a warning.     If FNDECL is nonzero, the message concerns an argument in a call    to that function.  ARGNUM is the number of the argument, origin 0.  */
end_comment
...
condition|)
block|{
comment|/* We have to check for template classes here, or else 		 build_method_call won't be able to see the constructor.  */
name|tree
name|id
init|=
name|TYPE_IDENTIFIER
...
block|if (TYPE_HAS_ASSIGNMENT (type)) 	    warning ("bitwise copy: `%s' defines operator=()", 		     TYPE_NAME_STRING (type)); 	  else if (TYPE_GETS_ASSIGNMENT (type)) 	    warning ("bitwise copy: `%s' has a member with operator=()", 		     TYPE_NAME_STRING (type));
endif|#
directive|endif
if|if
condition|(
...
literal|0
comment|/* ??? The following warnings are turned off because      this is another place where the default X(X&) constructor      is implemented.  */
block|if (TYPE_LANG_SPECIFIC (type))     {       if (TYPE_HAS_ASSIGNMENT (type)) 	warning ("bitwise copy: `%s' defines operator=()", 		 TYPE_NAME_STRING (type));       else if (TYPE_GETS_ASSIGNMENT (type)) 	warning ("bitwise copy: `%s' has a member with operator=()", 		 TYPE_NAME_STRING (type));     }
endif|#
directive|endif
if|if
condition|(
...
name|pedwarn
argument_list|(
literal|"ANSI C++ forbids `return' with a value, in function returning void"
argument_list|)
expr_stmt|;
name|expand_return
argument_list|(
...
name|whats_returned
argument_list|,
literal|"address of local variable `%s' returned"
argument_list|)
expr_stmt|;
block|}
...
begin_comment
comment|/* C++ does not yet support type checking of format strings.  */
end_comment
