...
operator|)
decl_stmt|;
comment|/* Anything converts to void *.  void * converts to anything. 	 Since these may be `const void *' (etc.) use VOID_TYPE 	 instead of void_type_node.  Otherwise, the targets must be the same, 	 except that we do allow (at some cost) conversion between signed and 	 unsigned pointer types.  */
if|if
condition|(
operator|(
name|TREE_CODE
...
argument_list|)
expr_stmt|;
comment|/* When passing a non-const argument into a const reference (or vice 	 versa), dig it a little, so a non-const reference is preferred 	 over this one. (mrs) */
if|if
condition|(
name|TYPE_READONLY
argument_list|(
...
name|TRIVIAL_CODE
expr_stmt|;
comment|/* We set this here so that build_overload_call_real will be 		 able to see the penalty we found, rather than just looking 		 at a TRIVIAL_CODE with no other information.  */
name|h
operator|.
name|int_penalty
operator|=
...
name|conv
decl_stmt|;
comment|/* Don't issue warnings since we're only groping 			 around for the right answer, we haven't yet 			 committed to going with this solution.  */
name|int
name|old_inhibit_warnings
init|=
name|inhibit_warnings
...
directive|if
literal|0
comment|/* This breaks initialization of a reference from a new              expression of a different type.  And it doesn't appear to              serve its original purpose any more, either.  jason 10/12/94 */
...
block|if (parms&& TREE_CODE (TREE_VALUE (parms)) == CALL_EXPR&& TREE_OPERAND (TREE_VALUE (parms), 1)&& TREE_CALLS_NEW (TREE_VALUE (TREE_OPERAND (TREE_VALUE (parms), 1)))) 	    parms = build_tree_list (NULL_TREE, instance_ptr); 	  else
endif|#
directive|endif
name|parms
operator|=
...
condition|)
block|{
comment|/* TREE_VALUE (parms) may have been modified by now;                      restore it to its original value. */
name|TREE_VALUE
argument_list|(
name|parms
argument_list|)
...
name|cp_error
argument_list|(
literal|"no matching function for call to `%T::%D (%A)'"
argument_list|,
name|TREE_TYPE
argument_list|(
name|TREE_TYPE
