...
begin_comment
comment|/* Separate lexical analyzer for GNU C++.    Copyright (C) 1987, 1989, 1992, 1993, 1994 Free Software Foundation, Inc.    Hacked by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* This obstack is used to hold information about methods to be    synthesized.  It should go away when synthesized methods are handled    properly (i.e. only when needed).  */
end_comment
...
specifier|static
name|void
name|default_copy_constructor_body
parameter_list|()
function_decl|;
end_function_decl
...
block|if (!flag_handle_exceptions)     {       UNSET_RESERVED_WORD ("throw");       UNSET_RESERVED_WORD ("try");       UNSET_RESERVED_WORD ("catch");     }
endif|#
directive|endif
if|if
condition|(
...
expr_stmt|;
block|}
comment|/* This kludge should go away when synthesized methods are handled        properly, i.e. only when needed.  */
for|for
control|(
name|t
operator|=
...
directive|if
literal|0
block|if (flag_default_inline&& t->fndecl
comment|/* If we're working from a template, don't change 	 the `inline' state.  */
block|&& t->parm_vec == NULL_TREE)     DECL_INLINE (t->fndecl) = 1;
endif|#
directive|endif
name|DECL_PENDING_INLINE_INFO
argument_list|(
...
directive|if
literal|0
block|if (flag_default_inline
comment|/* If we're working from a template, don't change 	     the `inline' state.  */
block|&& i->parm_vec == NULL_TREE) 	DECL_INLINE (i->fndecl) = 1;
endif|#
directive|endif
name|DECL_PENDING_INLINE_INFO
argument_list|(
...
begin_comment
comment|/* Used by default_copy_constructor_body.  For the anonymous union    in TYPE, return the member that is at least as large as the rest    of the members, so we can copy it.  */
end_comment

begin_comment
unit|static tree largest_union_member (type)      tree type; {   tree f, type_size = TYPE_SIZE (type);    for (f = TYPE_FIELDS (type); f; f = TREE_CHAIN (f))     if (simple_cst_equal (DECL_SIZE (f), type_size))       return f;
comment|/* We should always find one.  */
end_comment

begin_comment
unit|my_friendly_abort (323);   return NULL_TREE; }
comment|/* Construct the body of a default assignment operator.    Mostly copied directly from default_copy_constructor_body.  */
end_comment

begin_comment
unit|static void default_assign_ref_body (bufp, lenp, type, fields)      char **bufp;      int *lenp;      tree type, fields; {   static struct obstack body;   static int inited = FALSE;   int n_bases = CLASSTYPE_N_BASECLASSES (type);   char *tbuf;   int tgot, tneed;    if (!inited)     {       obstack_init (&body);       inited = TRUE;     }   body.next_free = body.object_base;    obstack_1grow (&body, '{');
comment|/* Small buffer for sprintf().  */
end_comment

begin_comment
unit|tgot = 100;   tbuf = (char *) alloca (tgot);
comment|/* If we don't need a real op=, just do a bitwise copy.  */
end_comment
...
begin_comment
unit|if (n_bases)     {       tree bases = TYPE_BINFO_BASETYPES (type);       int i = 0;        for (i = 0; i< n_bases; i++) 	{ 	  tree binfo = TREE_VEC_ELT (bases, i); 	  tree btype, name; 	  char *s;  	  btype = BINFO_TYPE (binfo); 	  name = TYPE_NESTED_NAME (btype); 	  s = IDENTIFIER_POINTER (name);  	  tneed = (2 * strlen (s)) + 42; 	  if (tgot< tneed) 	    { 	      tgot = tneed; 	      tbuf = (char *) alloca (tgot); 	    }  	  sprintf (tbuf, "%s::operator=((%s%s ::%s&)_ctor_arg);", s, 		   TYPE_READONLY (type) ? "const " : "", 		   CLASSTYPE_DECLARED_CLASS (btype) ? "class" : "struct", 		   s); 	  obstack_grow (&body, tbuf, strlen (tbuf)); 	}     }
comment|/* Construct fields.  */
end_comment

begin_comment
unit|if (fields)     {       tree f;        for (f = fields; f; f = TREE_CHAIN (f)) 	{ 	  if (TREE_CODE (f) == FIELD_DECL&& ! DECL_VIRTUAL_P (f)) 	    { 	      char *s; 	      tree x; 	      tree t = TREE_TYPE (f);  	      if (DECL_NAME (f)) 		x = f; 	      else if (t != NULL_TREE&& TREE_CODE (t) == UNION_TYPE&& ((TREE_CODE (TYPE_NAME (t)) == IDENTIFIER_NODE&& ANON_AGGRNAME_P (TYPE_NAME (t))) 			   || (TREE_CODE (TYPE_NAME (t)) == TYPE_DECL&& ANON_AGGRNAME_P (TYPE_IDENTIFIER (t))))&& TYPE_FIELDS (t) != NULL_TREE) 		x = largest_union_member (t); 	      else 		continue;  	      s = IDENTIFIER_POINTER (DECL_NAME (x)); 	      tneed = (2 * strlen (s)) + 13; 	      if (tgot< tneed) 		{ 		  tgot = tneed; 		  tbuf = (char *) alloca (tgot); 		}  	      sprintf (tbuf, "%s=_ctor_arg.%s;", s, s); 	      obstack_grow (&body, tbuf, strlen (tbuf)); 	    } 	}     }    obstack_grow (&body, "return *this;}", 15);    *lenp = obstack_object_size (&body) - 1;   *bufp = obstack_alloc (&inline_text_obstack, *lenp);    strcpy (*bufp, body.object_base); }
comment|/* Construct the body of a default copy constructor.  */
end_comment

begin_comment
unit|static void default_copy_constructor_body (bufp, lenp, type, fields)      char **bufp;      int *lenp;      tree type, fields; {   static struct obstack prologue;   static int inited = FALSE;   int n_bases = CLASSTYPE_N_BASECLASSES (type);   char sep = ':';   char *tbuf;   int tgot, tneed;
comment|/* Create a buffer to call base class constructors and construct members      (fields).  */
end_comment

begin_comment
unit|if (!inited)     {       obstack_init (&prologue);       inited = TRUE;     }   prologue.next_free = prologue.object_base;
comment|/* If we don't need a real copy ctor, just do a bitwise copy.  */
end_comment

begin_comment
unit|if (! TYPE_HAS_COMPLEX_INIT_REF (type))     {       tbuf = "{__builtin_memcpy(this,&_ctor_arg,sizeof(_ctor_arg));}";       *lenp = strlen (tbuf);       *bufp = obstack_alloc (&inline_text_obstack, *lenp + 1);       strcpy (*bufp, tbuf);       return;     }
comment|/* Small buffer for sprintf().  */
end_comment

begin_comment
unit|tgot = 100;   tbuf = (char *) alloca (tgot);    if (TREE_CODE (type) == UNION_TYPE)     {       if (fields) 	{ 	  tree main = fields; 	  char * s; 	  tree f;  	  for (f = TREE_CHAIN (fields); f; f = TREE_CHAIN (f)) 	    if (tree_int_cst_lt (TYPE_SIZE (TREE_TYPE (main)), 				 TYPE_SIZE (TREE_TYPE (f)))) 	      main = f;  	  s = IDENTIFIER_POINTER (DECL_NAME (main)); 	  tneed = (2 * strlen (s)) + 16; 	  if (tgot< tneed) 	    { 	      tgot = tneed; 	      tbuf = (char *) alloca (tgot); 	    }  	  sprintf (tbuf, ":%s(_ctor_arg.%s){}", s, s); 	}       else 	tbuf = "{}";        *lenp = strlen (tbuf);       *bufp = obstack_alloc (&inline_text_obstack, *lenp + 1);       strcpy (*bufp, tbuf);       return;     }
comment|/* Construct base classes... */
end_comment

begin_comment
unit|if (n_bases)     {
comment|/* Note that CLASSTYPE_VBASECLASSES isn't set yet... */
end_comment

begin_comment
unit|tree v = get_vbase_types (type);       tree bases = TYPE_BINFO_BASETYPES (type);       int i = 0;        for (;;) 	{ 	  tree binfo, btype, name; 	  char *s;  	  if (v) 	    { 	      binfo = v; 	      v = TREE_CHAIN (v); 	    } 	  else if (i< n_bases) 	    { 	      binfo = TREE_VEC_ELT (bases, i++); 	      if (TREE_VIA_VIRTUAL (binfo)) 		continue; 	    } 	  else 	    break;  	  btype = BINFO_TYPE (binfo); 	  name = TYPE_NESTED_NAME (btype); 	  s = IDENTIFIER_POINTER (name);  	  tneed = (2 * strlen (s)) + 39; 	  if (tgot< tneed) 	    { 	      tgot = tneed; 	      tbuf = (char *) alloca (tgot); 	    }  	  sprintf (tbuf, "%c%s((%s%s ::%s&)_ctor_arg)", sep, s, 		   TYPE_READONLY (type) ? "const " : "", 		   CLASSTYPE_DECLARED_CLASS (btype) ? "class" : "struct", 		   s); 	  sep = ','; 	  obstack_grow (&prologue, tbuf, strlen (tbuf)); 	}     }
comment|/* Construct fields.  */
end_comment

begin_comment
unit|if (fields)     {       tree f;        for (f = fields; f; f = TREE_CHAIN (f)) 	{ 	  if (TREE_CODE (f) == FIELD_DECL&& ! DECL_VIRTUAL_P (f)) 	    { 	      char *s; 	      tree x; 	      tree t = TREE_TYPE (f);  	      if (DECL_NAME (f)) 		x = f; 	      else if (t != NULL_TREE&& TREE_CODE (t) == UNION_TYPE&& ((TREE_CODE (TYPE_NAME (t)) == IDENTIFIER_NODE&& ANON_AGGRNAME_P (TYPE_NAME (t))) 			   || (TREE_CODE (TYPE_NAME (t)) == TYPE_DECL&& ANON_AGGRNAME_P (TYPE_IDENTIFIER (t))))&& TYPE_FIELDS (t) != NULL_TREE) 		x = largest_union_member (t); 	      else 		continue;  	      s = IDENTIFIER_POINTER (DECL_NAME (x)); 	      tneed = (2 * strlen (s)) + 30; 	      if (tgot< tneed) 		{ 		  tgot = tneed; 		  tbuf = (char *) alloca (tgot); 		}  	      sprintf (tbuf, "%c%s(_ctor_arg.%s)", sep, s, s); 	      sep = ','; 	      obstack_grow (&prologue, tbuf, strlen (tbuf)); 	    } 	}     }
comment|/* Concatenate constructor body to prologue.  */
end_comment

begin_endif
unit|*lenp = obstack_object_size (&prologue) + 2;   *bufp = obstack_alloc (&inline_text_obstack, *lenp + 1);    obstack_1grow (&prologue, '\0');    strcpy (*bufp, prologue.object_base);   strcat (*bufp, "{}"); }
endif|#
directive|endif
end_endif
...
block|}
block|}
comment|/* Do the actions implied by the preceeding numbers.  */
if|if
condition|(
name|action
operator|==
...
begin_comment
comment|/* NO LONGER USED: Value is -1 if we must not see a type name.  */
end_comment

begin_endif
unit|void dont_see_typename () {   looking_for_typename = -1;   if (yychar == TYPENAME || yychar == PTYPENAME)     {       yychar = IDENTIFIER;       lastiddecl = 0;     } }
endif|#
directive|endif
end_endif
...
name|cp_error
argument_list|(
literal|"invalid use of member `%D' from base class `%T'"
argument_list|,
name|field
argument_list|,
...
name|pedwarn
argument_list|(
literal|"`catch', `throw', and `try' are all C++ reserved words"
argument_list|)
expr_stmt|;
name|did_warn
operator|=
...
name|tmp
expr_stmt|;
comment|/* A user-invisible read-only initialized variable 	       should be replaced by its value.  We only handle strings 	       since that's the only case used in C (and C++).  */
comment|/* Note we go right after the local value for the identifier 	       (e.g., __FUNCTION__ or __PRETTY_FUNCTION__).  We used to 	       call lookup_name, but that could result in an error about 	       ambiguities.  */
name|tmp
operator|=
name|IDENTIFIER_LOCAL_VALUE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Note: garbage_chars is -1 if first char is *not* garbage.  */
while|while
condition|(
name|isalnum
argument_list|(
...
block|if (code == FUNCTION_DECL&& name != 0&& ! (IDENTIFIER_LENGTH (name) == 4&& IDENTIFIER_POINTER (name)[0] == 'm'&& strcmp (IDENTIFIER_POINTER (name), "main") == 0)&& ! (IDENTIFIER_LENGTH (name)> 10&& IDENTIFIER_POINTER (name)[0] == '_'&& IDENTIFIER_POINTER (name)[1] == '_'&& strncmp (IDENTIFIER_POINTER (name)+2, "builtin_", 8) == 0)) 	TREE_OVERLOADED (name) = 1;
endif|#
directive|endif
endif|#
directive|endif
