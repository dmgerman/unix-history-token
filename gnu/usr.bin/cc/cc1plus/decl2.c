...
begin_comment
comment|/* True if we want to implement vtbvales using "thunks".    The default is off now, but will be on later.     Also causes output of vtables to be controlled by whether    we seen the class's first non-inline virtual function. */
end_comment
...
begin_comment
comment|/* Non-zero means warn when sysnthesis behavior differs from Cfront's.  */
end_comment
...
name|warning
argument_list|(
literal|"templates that are built with -fexternal-templates should be in files that have #pragma interface/implementation"
argument_list|)
expr_stmt|;
block|}
end_function
...
argument_list|)
return|;
comment|/* Woops, looks like they did something like `5[a]' instead of `a[5]'.      We don't emit a warning or error for this, since it's allowed      by ARM $8.2.4.  */
name|type
operator|=
name|TREE_TYPE
argument_list|(
...
argument_list|)
return|;
comment|/* The expression E1[E2] is identical (by definition) to *((E1)+(E2)).  */
return|return
...
return|;
block|}
comment|/* Deleting a pointer with the value zero is legal and has no effect.  */
if|if
condition|(
name|integer_zerop
argument_list|(
...
name|cp_error
argument_list|(
literal|"argument list for `%#D' does not match any in class `%T'"
argument_list|,
name|function
argument_list|,
name|ctype
...
begin_comment
unit|return decl;    if (DECL_IN_AGGR_P (decl))     {       cp_error ("`%D' already defined in the class ", decl);       return void_type_node;     }    finish_decl (decl, NULL_TREE, NULL_TREE, 0);
...
name|rval
return|;
comment|/* We do need to push the scope in this case, since CTYPE helps      determine subsequent intializers (i.e., Foo::Bar x = foo_enum_1;).  */
...
block|finish_decl (current_vtable_decl, convert_force (TREE_TYPE (current_vtable_decl), vfield), 0, 0);       current_vtable_decl = build_indirect_ref (current_vtable_decl, NULL_PTR);     }   else
endif|#
directive|endif
...
condition|)
return|return;
comment|/* If an explicit instantiation doesn't have TREE_PUBLIC set, it was with      'extern'.  */
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* 9.5p5: The initializer of a static member of a class has 		 the same acess rights as a member function.  */
name|DECL_CLASS_CONTEXT
argument_list|(
name|current_function_decl
argument_list|)
...
name|NULL_TREE
decl_stmt|;
comment|/* These might be referenced later */
...
argument_list|)
expr_stmt|;
comment|/* Redefinition of a member function can cause DECL_SAVED_INSNS to be 	   0; don't crash.  */
if|if
condition|(
name|TREE_ASM_WRITTEN
argument_list|(
