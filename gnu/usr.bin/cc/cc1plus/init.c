...
begin_comment
comment|/* Handle initialization things in C++.    Copyright (C) 1987, 1989, 1992, 1993, 1994 Free Software Foundation, Inc.    Contributed by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|cp_warning
argument_list|(
literal|"caution, member `%D' may not be destroyed in the presense of an exception during construction"
argument_list|,
name|member
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* See if we already found an initializer for this field.  */
if|if
condition|(
name|found
condition|)
...
literal|0
expr_stmt|;
comment|/* XXX is this a memory leak? */
name|fields_to_unmark
operator|=
name|TREE_CHAIN
argument_list|(
...
expr_stmt|;
block|}
comment|/* We used to give an error if this wasn't true, saying that 		 there's no constructor for the initialization of basename. 		 This turned out to be incorrect---it should use the 		 default constructor, since a user could try to initialize 		 the class in a derived class's base initializer list.  */
if|if
condition|(
name|TYPE_HAS_CONSTRUCTOR
argument_list|(
...
condition|)
block|{
comment|/* ?? This should be fixed in RENO by forcing 		 default constructors to exist.  It is needed for symmetry 		 with code above.  */
name|base_binfo
operator|=
name|BINFO_BASETYPE
argument_list|(
...
name|cp_error
argument_list|(
literal|"in base initialization for class `%T'"
argument_list|,
name|binfo
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"invalid initializer to constructor for type `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"virtual baseclass `%T' does not have default initializer"
argument_list|,
name|binfo
argument_list|)
expr_stmt|;
...
decl_stmt|;
comment|/* This does not handle access checking yet.  */
if|if
condition|(
name|DECL_CHAIN
argument_list|(
...
name|cp_error_at
argument_list|(
literal|"member `%D' is non-static in static member function context"
argument_list|,
name|member
argument_list|)
expr_stmt|;
...
name|pedwarn
argument_list|(
literal|"const and volatile types cannot be created with operator new"
argument_list|)
expr_stmt|;
name|type
operator|=
...
name|LOOKUP_COMPLAIN
decl_stmt|;
comment|/* If a copy constructor might work, set things up so that we can 	 try that after this.  We deliberately don't clear LOOKUP_COMPLAIN 	 any more, since that would make it impossible to rationally use 	 the access of a constructor that matches perfectly.  */
if|#
directive|if
literal|0
block|if (rval != NULL_TREE) 	flags |= LOOKUP_SPECULATIVELY;
endif|#
directive|endif
if|if
condition|(
