...
begin_comment
comment|/* This is the newer recursive depth first one, the old one follows. */
end_comment

begin_function
specifier|static
name|tree
name|get_binfo_recursive
parameter_list|(
name|binfo
parameter_list|,
name|is_private
...
begin_comment
comment|/* This is the newer depth first get_base_distance, the older one follows.  */
end_comment
...
argument|is_private
argument_list|,
argument|basetype_path
argument_list|,
argument|rval
argument_list|,
argument|rval_private_ptr
argument_list|,
argument|new_binfo_ptr
argument_list|,
argument|parent
argument_list|,
argument|path_ptr
...
argument|protect
argument_list|,
argument|via_virtual_ptr
argument_list|,
argument|via_virtual
argument_list|)
name|tree
name|binfo
operator|,
...
condition|)
block|{
comment|/* Note, this should probably succeed to find, and 		 override the old one if the old one was private and 		 this one isn't.  */
return|return
name|rval
return|;
block|}
...
condition|)
block|{
comment|/* UNION_TYPEs do not participate in inheritance relationships.  */
return|return
operator|-
literal|1
return|;
...
comment|/* See PR 428 for a test case that can tickle this. */
name|warning
argument_list|(
...
begin_comment
comment|/* Look for a field named NAME in an inheritance lattice dominated by    XBASETYPE.  PROTECT is zero if we can avoid computing visibility    information, otherwise it is 1.  WANT_TYPE is 1 when we should only    return TYPE_DECLs, if no TYPE_DECL can be found return NULL_TREE.     It was not clear what should happen if WANT_TYPE is set, and an    ambiguity is found.  At least one use (lookup_name) to not see    the error.  */
end_comment
...
name|normal
decl_stmt|;
comment|/* This code is most likely wrong, and probably only works for single 	 inheritance or by accident. */
...
name|error
argument_list|(
literal|"inheritance lattice too complex below"
argument_list|)
expr_stmt|;
block|}
while|while
...
goto|;
comment|/* case 4.  */
comment|/* These two are not hard errors, but could be 			   symptoms of bad code.  The resultant code 			   the compiler generates needs to be checked. 			   (mrs) */
if|#
directive|if
literal|0
block|error_with_decl (MI_VMATRIX (row, col).decl, "ambiguous virtual function `%s'"); 			error_with_decl (this_decl, "ambiguating function `%s' (joined by type `%s')", IDENTIFIER_POINTER (current_class_name));
endif|#
directive|endif
block|}
name|ok
...
directive|if
literal|0
block|tree tags = CLASSTYPE_TAGS (type);    while (tags)     {       tree code_type_node;       tree tag;        switch (TREE_CODE (TREE_VALUE (tags))) 	{ 	case ENUMERAL_TYPE: 	  code_type_node = enum_type_node; 	  break; 	case RECORD_TYPE: 	  code_type_node = record_type_node; 	  break; 	case CLASS_TYPE: 	  code_type_node = class_type_node; 	  break; 	case UNION_TYPE: 	  code_type_node = union_type_node; 	  break; 	default: 	  my_friendly_abort (297); 	}       tag = xref_tag (code_type_node, TREE_PURPOSE (tags), 		      TYPE_BINFO_BASETYPE (TREE_VALUE (tags), 0));
if|#
directive|if
literal|0
