...
begin_comment
comment|/* Insert BLOCK at the end of the list of subblocks of the    current binding level.  This is used when a BIND_EXPR is expanded,    to handle the BLOCK node inside teh BIND_EXPR.  */
end_comment
...
condition|)
block|{
comment|/* If overriding decl is static, 	     optionally warn this was a bad choice of name.  */
if|if
condition|(
operator|!
name|TREE_PUBLIC
...
argument_list|)
expr_stmt|;
comment|/* It is nice to warn when a function is declared 	     global first and then static.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
block|}
block|}
comment|/* Multiple external decls of the same identifier ought to match. 	 Check against both global declarations and out of scope (limbo) block 	 level declarations.  	 We get warnings about inline functions where they are defined. 	 Avoid duplicate warnings where they are used.  */
if|if
condition|(
name|TREE_PUBLIC
argument_list|(
...
argument_list|)
condition|?
comment|/* A static variable with an incomplete type 		   is an error if it is initialized or `static'. 		   Otherwise, let it through, but if it is not `extern' 		   then it may cause an error message later.  */
operator|!
name|TREE_PUBLIC
argument_list|(
name|decl
...
directive|if
literal|0
block|if (DECL_NAME (decl))     {       olddecl = lookup_name (DECL_NAME (decl));       if (pedantic&& olddecl != 0&& TREE_CODE (olddecl) == TYPE_DECL) 	pedwarn_with_decl (decl, "ANSI C forbids parameter `%s' shadowing typedef");     }
endif|#
directive|endif
name|decl
operator|=
...
condition|)
block|{
comment|/* Compute number of bits to represent magnitude of a negative value. 	     Add one to MINVALUE since range of negative numbers 	     includes the power of two.  */
name|unsigned
name|negprecision
init|=
name|floor_log2
...
argument_list|)
expr_stmt|;
comment|/* If the enumerators might not fit in an int, change their type now.  */
comment|/* It seems more useful in the debugger to leave these as int      unless the enumerator is wider than int.  */
if|if
condition|(
name|TYPE_PRECISION
argument_list|(
