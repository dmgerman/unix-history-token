...
begin_comment
comment|/* Nonzero if we've already printed a "partly bracketed initializer"    message within this initializer.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|partial_bracket_mentioned
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
argument_list|)
expr_stmt|;
comment|/* If one type is complex, form the common type      of the non-complex components,      then make that complex.  */
if|if
condition|(
name|code1
operator|==
...
argument_list|)
decl_stmt|;
comment|/* We use NOP_EXPR rather than NON_LVALUE_EXPR 	 so that convert_for_assignment won't strip it. 	 That way, we get warnings for things like p = (1 - 1).  */
name|result
operator|=
name|build1
argument_list|(
...
operator|)
condition|)
comment|/* Don't use convert_and_check here.  If the input has type int        and did not overflow, and we are converting it here to a short,        we don't want an error.  A warning would be okay, but it's too risky now        to add an option to convert_and_check to get just warnings.  */
return|return
name|convert
argument_list|(
name|type
...
argument_list|)
return|;
comment|/* Conversion to a union from its member types.  */
elseif|else
if|if
condition|(
name|codel
...
name|init
decl_stmt|;
comment|/* Make sure there is just one "partially bracketed" message      per top-level initializer or constructor.  */
if|if
condition|(
name|ofwhat
operator|!=
...
name|SAVE_SPELLING_DEPTH
argument_list|(
argument|{ 	  if (ofwhat) 	    push_string (ofwhat); 	  if (!raw_constructor) 	    inside_init = init;
...
argument|inside_init 	    = convert_for_assignment (type, inside_init,&initialization_message, 				      NULL_TREE, NULL_TREE,
literal|0
argument|); 	}
argument_list|)
empty_stmt|;
...
name|error
argument_list|(
literal|"non-constant or non-integer array index in initializer"
argument_list|)
expr_stmt|;
elseif|else
if|if
