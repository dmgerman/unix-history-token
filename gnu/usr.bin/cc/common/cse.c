...
begin_comment
comment|/* Index by (pseudo) register number, gives the number of the next (or    previous) (pseudo) register in the chain of registers sharing the same    value.     Or -1 if this register is at the end of the chain.     If reg_qty[N] == N, reg_next_eqv[N] is undefined.  */
end_comment
...
begin_comment
comment|/* Two vectors of ints:    one containing max_reg -1's; the other max_reg + 500 (an approximation    for max_qty) elements where element i contains i.    These are used to initialize various other vectors fast.  */
end_comment
...
parameter_list|)
define|\
value|(GET_CODE (X) == REG&& REGNO (X)>= FIRST_PSEUDO_REGISTER	\   ? ((((int) REG<< 7) + reg_qty[REGNO (X)]) % NBUCKETS)	\   : canon_hash (X, M) % NBUCKETS)
end_define
...
parameter_list|)
define|\
value|(((N) == FRAME_POINTER_REGNUM || fixed_regs[N])	\&& ! OVERLAPPING_REGNO_P ((N)))
end_define
...
parameter_list|)
define|\
value|((N) == FRAME_POINTER_REGNUM || fixed_regs[N])
end_define
...
parameter_list|)
define|\
value|((N) == FRAME_POINTER_REGNUM || (N) == STACK_POINTER_REGNUM \    || (N) == ARG_POINTER_REGNUM				\    || ((N)>= FIRST_VIRTUAL_REGISTER&& (N)<= LAST_VIRTUAL_REGISTER) \    || ((N)< FIRST_PSEUDO_REGISTER			\&& FIXED_REGNO_P (N)&& REGNO_REG_CLASS (N) != NO_REGS))
end_define
...
parameter_list|)
define|\
value|(GET_CODE (X) == REG					\    ? (CHEAP_REG (REGNO (X)) ? 0				\       : REGNO (X)>= FIRST_PSEUDO_REGISTER ? 1		\       : 2)						\    : rtx_cost (X, SET) * 2)
end_define
...
literal|1
decl_stmt|;
comment|/* Invalidate stack pointer. */
name|int
name|var
range|:
literal|1
...
parameter_list|)
define|\
value|((X) == frame_pointer_rtx || (X) == arg_pointer_rtx		\    || (X) == virtual_stack_vars_rtx				\    || (X) == virtual_incoming_args_rtx				\    || (GET_CODE (X) == PLUS&& GET_CODE (XEXP (X, 1)) == CONST_INT \&& (XEXP (X, 0) == frame_pointer_rtx			\ 	   || XEXP (X, 0) == arg_pointer_rtx			\ 	   || XEXP (X, 0) == virtual_stack_vars_rtx		\ 	   || XEXP (X, 0) == virtual_incoming_args_rtx)))
end_define
...
index|]
decl_stmt|;
comment|/* If invalid, do nothing.  N and P above are undefined in that case.  */
if|if
condition|(
name|q
operator|==
...
argument_list|)
decl_stmt|;
comment|/* On some machines, we can't record any non-fixed hard register, 	   because extending its life will cause reload problems.  We 	   consider ap, fp, and sp to be fixed for this purpose. 	   On all machines, we can't record any global registers. */
if|if
condition|(
name|regno
operator|<
...
name|tem1
decl_stmt|;
comment|/* If the operand is a REG that is equivalent to a constant, hash 	     as if we were hashing the constant, since we will be comparing 	     that way.  */
if|if
condition|(
name|tem
operator|!=
...
begin_comment
comment|/* Given ADDR and SIZE (a memory address, and the size of the memory reference),    set PBASE, PSTART, and PEND which correspond to the base of the address,    the starting offset, and ending offset respectively.     ADDR is known to be a nonvarying address.      cse_address_varies_p returns zero for nonvarying addresses.  */
end_comment
...
begin_comment
comment|/* Return 1 iff any subexpression of X refers to memory    at an address of BASE plus some offset    such that any of the bytes' offsets fall between START (inclusive)    and END (exclusive).     The value is undefined if X is a varying address (as determined by    cse_rtx_addr_varies_p).  This function is not used in such cases.     When used in the cse pass, `qty_const' is nonzero, and it is used    to treat an address that is a register with a known constant value    as if it were that constant value.    In the loop pass, `qty_const' is zero, so this is not done.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* refers_to_mem_p is never called with varying addresses.  	 If the base addresses are not equal, there is no chance 	 of the memory addresses conflicting.  */
if|if
condition|(
operator|!
name|rtx_equal_p
...
argument_list|)
return|;
comment|/* Handle both-operands-constant cases.  We can only add 	     CONST_INTs to constants since the sum of relocatable symbols 	     can't be handled by most assemblers.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
condition|)
break|break;
comment|/* We can't assume x-x is 0 even with non-IEEE floating point.  */
if|if
condition|(
name|rtx_equal_p
argument_list|(
...
name|op0
return|;
comment|/* Convert multiply by constant power of two into shift.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
directive|if
literal|0
comment|/* Turned off till an expert says this is a safe thing to do.  */
if|#
directive|if
operator|!
name|defined
...
name|REAL_ARITHMETIC
argument_list|)
comment|/* Change division by a constant into multiplication.  */
block|else if (GET_CODE (op1) == CONST_DOUBLE&& GET_MODE_CLASS (GET_MODE (op1)) == MODE_FLOAT&& op1 != CONST0_RTX (mode)) 	    { 	      REAL_VALUE_TYPE d; 	      REAL_VALUE_FROM_CONST_DOUBLE (d, op1); 	      if (REAL_VALUES_EQUAL (d, dconst0)) 		abort();
if|#
directive|if
name|defined
argument_list|(
name|REAL_ARITHMETIC
argument_list|)
block|REAL_ARITHMETIC (d, (int) RDIV_EXPR, dconst1, d); 	      return gen_rtx (MULT, mode, op0,  			      CONST_DOUBLE_FROM_REAL_VALUE (d, mode));
else|#
directive|else
block|return gen_rtx (MULT, mode, op0,  			      CONST_DOUBLE_FROM_REAL_VALUE (1./d, mode)); 	    }
endif|#
directive|endif
endif|#
directive|endif
...
begin_comment
comment|/* Like simplify_binary_operation except used for relational operators.    MODE is the mode of the operands, not that of the result.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* What to do with MODE_CC isn't clear yet.      Let's make sure nothing erroneous is done.  */
if|if
condition|(
name|GET_MODE_CLASS
argument_list|(
...
literal|0
return|;
comment|/* Unlike the arithmetic operations, we can do the comparison whether      or not WIDTH is larger than HOST_BITS_PER_WIDE_INT because the      CONST_INTs are to be understood as being infinite precision as      is the comparison.  So there is no question of overflow.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
directive|if
literal|0
comment|/* We can't make this assumption due to #pragma weak */
block|if (CONSTANT_P (op0)&& op1 == const0_rtx) 	      return const0_rtx;
endif|#
directive|endif
if|if
condition|(
...
literal|0
comment|/* We can't make this assumption due to #pragma weak */
block|if (CONSTANT_P (op0)&& op1 == const0_rtx) 	    return const_true_rtx;
endif|#
directive|endif
if|if
condition|(
...
name|GEU
case|:
comment|/* Unsigned values are never negative, but we must be sure we are 	     actually comparing a value, not a CC operand.  */
if|if
condition|(
name|op1
operator|==
...
name|new
return|;
comment|/* If this is a paradoxical SUBREG, we have no idea what value the 	 extra bits would have.  However, if the operand is equivalent 	 to a SUBREG whose operand is the same as our mode, and all the 	 modes are within a word, we can just use the inner operand 	 because these SUBREGs just say how to treat the register.  */
if|if
condition|(
name|GET_MODE_SIZE
argument_list|(
...
name|constant
return|;
comment|/* If this actually isn't a constant (wierd!), we can't do 	       anything.  Otherwise, handle the two most common cases: 	       extracting a word from a multi-word constant, and extracting 	       the low-order bits.  Other cases don't seem common enough to 	       worry about.  */
if|if
condition|(
operator|!
name|CONSTANT_P
...
return|return;
block|}
comment|/* If both are equivalent, merge the two classes.  Save this class for      `cse_set_around_loop'.  */
if|if
condition|(
name|op0_elt
operator|&&
...
expr_stmt|;
block|}
comment|/* For whichever side doesn't have an equivalence, make one.  */
if|if
condition|(
name|op0_elt
operator|==
...
name|src_const
decl_stmt|;
comment|/* Hash code of constant equivalent for SET_SRC.  */
name|int
name|src_const_hash_code
decl_stmt|;
...
condition|)
block|{
comment|/* Store the equivalent value in SRC_EQV, if different.  */
name|rtx
name|tem
init|=
name|find_reg_note
...
expr_stmt|;
block|}
comment|/* If this is a STRICT_LOW_PART assignment, src_eqv corresponds to the 	 value of the INNER register, not the destination.  So it is not 	 a legal substitution for the source.  But save it for later.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|src_eqv_elt
expr_stmt|;
comment|/* Try to find a constant somewhere and record it in `src_const'. 	 Record its table element, if any, in `src_const_elt'.  Look in 	 any known equivalences first.  (If the constant is not in the 	 table, also set `sets[i].src_const_hash_code').  */
if|if
condition|(
name|elt
condition|)
...
expr_stmt|;
block|}
comment|/* If this is a single SET, we are setting a register, and we have an 	 equivalent constant, we want to add a REG_NOTE.   We don't want 	 to write a REG_EQUAL note for a constant pseudo since verifying that 	 that pseudo hasn't been eliminated is a pain.  Such a note also 	 won't help anything.  */
if|if
condition|(
name|n_sets
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Decide whether we invalidate everything in memory, 	     or just things at non-fixed places. 	     Writing a large aggregate must invalidate everything 	     because we don't know how long it is.  */
name|note_mem_written
argument_list|(
name|dest
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Process insns starting after LOOP_START until we hit a CALL_INSN or      a CODE_LABEL (we could handle a CALL_INSN, but it isn't worth it).       The only thing we do with SET_DEST is invalidate entries, so we      can safely process each SET in order.  It is slightly less efficient      to do so, but we only want to handle the most common cases.  */
for|for
control|(
name|insn
operator|=
...
begin_comment
comment|/* Variable used for communications between the next two routines.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Don't delete any insns that are part of a libcall block. 	 Flow or loop might get confused if we did that.  Remember 	 that we are scanning backwards.  */
if|if
condition|(
name|find_reg_note
argument_list|(
