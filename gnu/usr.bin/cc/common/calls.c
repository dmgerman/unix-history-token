...
begin_comment
comment|/* Convert function calls to rtl insns, for GNU C compiler.    Copyright (C) 1989, 1992 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|initial_value
decl_stmt|;
comment|/* Register to pass this argument in, 0 if passed on stack, or an      EXPR_LIST if the arg is to be copied into multiple different      registers.  */
name|rtx
name|reg
decl_stmt|;
...
begin_comment
comment|/* Force FUNEXP into a form suitable for the address of a CALL,    and return that as an rtx.  Also load the static chain register    if FNDECL is a nested function.     USE_INSNS points to a variable holding a chain of USE insns    to which a USE of the static chain    register should be added, if required.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Put the USE insn in the chain we were passed.  It will later be 	 output immediately in front of the CALL insn.  */
...
name|funexp
decl_stmt|;
comment|/* Tree node for the function to be called (not the address!).  */
name|tree
name|funtree
decl_stmt|;
...
condition|)
block|{
comment|/* May still be a `const' function if it is 	     a call through a pointer-to-const. 	     But we don't handle that.  */
name|fndecl
operator|=
literal|0
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* On some machines (such as the PA) indirect calls have a different      calling convention than normal calls.  FUNCTION_ARG in the target      description can look at current_call_is_indirect to determine which      calling convention to use.  */
name|current_call_is_indirect
operator|=
operator|(
name|fndecl
...
directive|if
literal|0
block|= TREE_CODE (TREE_OPERAND (exp, 0)) == NON_LVALUE_EXPR ? 1 : 0;
endif|#
directive|endif
if|#
directive|if
...
argument_list|)
expr_stmt|;
comment|/* Push the temporary stack slot level so that we can free temporaries used      by each of the arguments separately.  */
...
directive|endif
comment|/* FUNCTION_ARG_PASS_BY_REFERENCE */
name|mode
operator|=
name|TYPE_MODE
argument_list|(
...
comment|/* Compute the mode in which the arg is actually to be extended to.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If FUNCTION_ARG returned an (expr_list (nil) FOO), it means that 	 we are to pass this arg in the register(s) designated by FOO, but 	 also to pass it in the stack.  */
if|if
condition|(
name|args
index|[
...
name|reg
expr_stmt|;
comment|/* Clobber REG and move each partword into it.  Ensure we don't 	       go past the end of the structure.  Note that the loop below 	       works because we've already verified that padding 	       and endianness are compatible.  */
name|emit_insn
argument_list|(
name|gen_rtx
argument_list|(
...
block|}
block|}
comment|/* Now do the register loads required for any wholly-register parms or any      parms which are passed both on the stack and in a register.  Their      expressions were already evaluated.        Mark all register-parms as living through the call, putting these USE      insns in a list headed by USE_INSNS.  */
for|for
control|(
name|i
operator|=
...
name|nregs
decl_stmt|;
comment|/* Process each register that needs to get this arg.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
literal|0
expr_stmt|;
comment|/* Set to non-zero if must move a word at a time, even if just one 	     word (e.g, partial == 1&& mode == DFmode).  Set to zero if 	     we just use a normal move insn.  */
name|nregs
operator|=
operator|(
name|partial
...
argument_list|()
expr_stmt|;
comment|/* PARTIAL referred only to the first register, so clear it for the 	     next time.  */
name|partial
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* We used leave the value in the location that it is 	     returned in, but that causes problems if it is used more 	     than once in one expression.  Rather than trying to track 	     when a copy is required, we always copy when TARGET is 	     not specified.  This calling sequence is only used on 	     a few machines and TARGET is usually nonzero.  */
if|if
condition|(
name|TYPE_MODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* If we didn't promote as expected, something is wrong.  */
if|if
condition|(
name|mode
operator|!=
...
name|use_insns
decl_stmt|;
comment|/* library calls are never indirect calls.  */
name|int
name|current_call_is_indirect
init|=
literal|0
...
directive|ifdef
name|ACCUMULATE_OUTGOING_ARGS
comment|/* If this arg is actually passed on the stack, it might be 	 clobbering something we already put there (this library call might 	 be inside the evaluation of an argument to a function whose call 	 requires the stack).  This will only occur when the library call 	 has sufficient args to run out of argument registers.  Abort in 	 this case; if this ever occurs, code must be added to save and 	 restore the arg slot.  */
if|if
condition|(
name|argvec
index|[
...
begin_comment
comment|/* Like emit_library_call except that an extra argument, VALUE,    comes second and says where to store the result.    (If VALUE is zero, the result comes in the function value register.)  */
end_comment
...
begin_endif
unit|target = gen_rtx (PLUS, Pmode, args_addr, offset_rtx);       target = memory_address (QImode, target);     }    return gen_rtx (MEM, BLKmode, target); }
endif|#
directive|endif
end_endif
...
directive|ifdef
name|ARGS_GROW_DOWNWARD
comment|/* stack_slot is negative, but we want to index stack_usage_map */
comment|/* with positive values. */
if|if
condition|(
name|GET_CODE
argument_list|(
...
directive|endif
comment|/* If this is being partially passed in a register, but multiple locations      are specified, we assume that the one partially used is the one that is      listed first.  */
if|if
condition|(
name|reg
operator|&&
