...
block|}
block|}
comment|/* Now, use the simple method of adding or subtracting at the leftmost      1-bit.  */
block|{
name|unsigned
name|HOST_WIDE_INT
name|w
...
literal|1
operator|)
comment|/* Reject the case where t has only two bits. 	   Thus we prefer addition in that case.  */
operator|&&
operator|!
operator|(
name|t
...
condition|)
block|{
comment|/* There are many bits in a row.  Make 'em by subtraction.  */
name|m
operator|=
name|exact_log2
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Don't use shiftsub_cost here, this operation 	   scales wrong operand.  */
name|cost
operator|=
name|add_cost
operator|+
...
block|{
comment|/* There's only one or two bit at the left.  Make it by addition.  */
name|m
operator|=
name|exact_log2
argument_list|(
