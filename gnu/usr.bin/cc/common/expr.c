...
argument_list|()
expr_stmt|;
comment|/* If we aren't returning a result, just pass on what expand_expr 	 returned; it was probably const0_rtx.  Otherwise, convert RESULT 	 to the proper mode.  */
return|return
operator|(
name|want_value
condition|?
...
name|i
decl_stmt|;
comment|/* Optimize the special-case of a zero lower bound.  */
if|if
condition|(
operator|!
name|integer_zerop
...
name|op0
return|;
comment|/* If arg is a constant integer being extended from a narrower mode, 	 we must really truncate to get the extended bits right.  Otherwise 	 (unsigned long) (unsigned char) ("\377"[0]) 	 would come out as ffffffff.  */
if|if
condition|(
name|GET_MODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* We would like the object in memory.  If it is a constant, 	     we can have it be statically allocated into memory.  For 	     a non-constant (REG or SUBREG), we need to allocate some 	     memory and store the value into it.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
name|sub_optab
expr_stmt|;
comment|/* If OP0 is not the actual lvalue, but rather a copy in a register,      then we cannot just increment OP0.  We must therefore contrive to      increment the original value.  Then, for postincrement, we can return      OP0 since it is a copy of the old value.  For preincrement, we want      to always expand here, since this generates better or equivalent code.  */
if|if
condition|(
operator|!
name|post
