...
begin_comment
comment|/* Reload pseudo regs into hard regs for insns that require hard regs.    Copyright (C) 1987, 1988, 1989, 1992, 1993 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* Indexed by pseudo reg number N,    says may not delete stores into the real (memory) home of pseudo N.    This is set if we already substituted a memory equivalent in some uses,    which happens when we have to eliminate the fp from it.  */
end_comment
...
decl_stmt|;
name|int
name|max_total_input_groups
init|=
literal|0
decl_stmt|,
name|max_total_output_groups
init|=
literal|0
decl_stmt|;
...
comment|/* These just count RELOAD_OTHER.  */
name|int
name|insn_needs
index|[
name|N_REG_CLASSES
...
literal|0
decl_stmt|;
comment|/* Count RELOAD_FOR_INPUT reloads.  */
name|int
...
literal|0
decl_stmt|;
comment|/* Count RELOAD_FOR_OUTPUT reloads.  */
name|int
...
literal|0
decl_stmt|;
comment|/* Count RELOAD_FOR_INSN reloads.  */
name|int
name|insn_needs_for_insn
index|[
name|N_REG_CLASSES
index|]
decl_stmt|;
name|int
name|insn_groups_for_insn
index|[
name|N_REG_CLASSES
index|]
decl_stmt|;
name|int
name|insn_total_groups_for_insn
init|=
literal|0
decl_stmt|;
comment|/* Count RELOAD_FOR_OTHER_ADDRESS reloads.  */
name|int
name|insn_needs_for_other_addr
index|[
name|N_REG_CLASSES
index|]
decl_stmt|;
name|int
name|insn_groups_for_other_addr
index|[
name|N_REG_CLASSES
index|]
decl_stmt|;
name|int
name|insn_total_groups_for_other_addr
init|=
literal|0
decl_stmt|;
comment|/* Count RELOAD_FOR_INPUT_ADDRESS reloads.  */
name|int
name|insn_needs_for_in_addr
index|[
...
decl_stmt|;
name|int
name|insn_total_groups_for_in_addr
index|[
name|MAX_RECOG_OPERANDS
index|]
decl_stmt|;
comment|/* Count RELOAD_FOR_OUTPUT_ADDRESS reloads.  */
name|int
name|insn_needs_for_out_addr
index|[
name|MAX_RECOG_OPERANDS
index|]
index|[
...
decl_stmt|;
name|int
name|insn_total_groups_for_out_addr
index|[
name|MAX_RECOG_OPERANDS
index|]
decl_stmt|;
...
name|int
name|insn_needs_for_op_addr
index|[
name|N_REG_CLASSES
index|]
decl_stmt|;
name|int
name|insn_groups_for_op_addr
index|[
name|N_REG_CLASSES
index|]
decl_stmt|;
name|int
name|insn_total_groups_for_op_addr
init|=
literal|0
decl_stmt|;
if|#
...
expr_stmt|;
block|}
comment|/* Now count extra regs if there might be a conflict with 		     the return value register.  		     ??? This is not quite correct because we don't properly 		     handle the case of groups, but if we end up doing 		     something wrong, it either will end up not mattering or 		     we will abort elsewhere.  */
for|for
control|(
name|r
operator|=
...
directive|ifdef
name|SMALL_REGISTER_CLASSES
comment|/* This should be right for all machines, but only the 386 		 is known to need it, so this conditional plays safe. 		 ??? For 2.5, try making this unconditional.  */
...
argument_list|)
comment|/* On these machines we will be reloading what is 		 inside the SUBREG if it originally was a pseudo and 		 the inner and outer modes are both a word or 		 smaller.  So leave the SUBREG then.  */
operator|&&
operator|!
operator|(
name|GET_CODE
...
name|i
decl_stmt|;
comment|/* In use for a RELOAD_OTHER means it's not available for anything except      RELOAD_FOR_OTHER_ADDRESS.  Recall that RELOAD_FOR_OTHER_ADDRESS is known      to be used only for inputs.  */
if|if
condition|(
name|type
operator|!=
...
name|RELOAD_FOR_OUTPUT
case|:
comment|/* This can't be used in the output address for this operand and 	 anything that can't be used for it, except that we've already 	 tested for RELOAD_FOR_INSN objects.  */
if|if
condition|(
name|TEST_HARD_REG_BIT
argument_list|(
...
name|RELOAD_FOR_INSN
case|:
comment|/* These conflict with other outputs with with RELOAD_OTHER.  So 	 we need only check for output addresses.  */
name|opnum
operator|=
operator|-
literal|1
...
condition|)
continue|continue;
comment|/* If the reload regs aren't exactly the same (e.g, different modes) 	     or if the values are different, we can't merge anything with this 	     reload register.  */
if|if
condition|(
operator|!
name|rtx_equal_p
...
argument_list|)
expr_stmt|;
comment|/* If INSN is followed by any CLOBBER insns made by find_reloads,      put our reloads after them since they may otherwise be       misinterpreted.  */
while|while
condition|(
name|GET_CODE
argument_list|(
...
index|]
decl_stmt|;
comment|/* I is nonneg if this reload used one of the spill regs. 	 If reload_reg_rtx[r] is 0, this is an optional reload 	 that we opted to ignore.  	 Also ignore reloads that don't reach the end of the insn, 	 since we will eventually see the one that does.  */
if|if
condition|(
name|i
operator|>=
...
begin_comment
comment|/* Emit code to perform an input reload of IN to RELOADREG.  IN is from    operand OPNUM with reload type TYPE.      Returns first insn emitted.  */
end_comment
