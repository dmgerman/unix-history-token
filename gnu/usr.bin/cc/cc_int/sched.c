...
name|insn
expr_stmt|;
comment|/* Don't save away NOTE_INSN_SETJMPs, because they must remain 	 immediately after the call they follow.  We use a fake 	 (REG_DEAD (const_int -1)) note to remember them.  */
elseif|else
if|if
condition|(
name|NOTE_LINE_NUMBER
...
name|insn
expr_stmt|;
comment|/* Check to see if we need to re-emit a NOTE_INSN_SETJMP here.  */
if|if
condition|(
name|GET_CODE
argument_list|(
