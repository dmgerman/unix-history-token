...
begin_comment
comment|/* Indicate that NEW is an alternate way of refering to the temp slot    that previous was known by OLD.  */
end_comment
...
begin_comment
comment|/* If X could be a reference to a temporary slot, mark the fact that its    adddress was taken.  */
end_comment
...
condition|)
return|return;
comment|/* If we can find a match, move it to our level.  */
for|for
control|(
name|p
operator|=
...
begin_comment
comment|/* Subroutine of put_var_into_stack.  This puts a single pseudo reg REG    into the stack frame of FUNCTION (0 means the current function).    DECL_MODE is the machine mode of the user-level data type.    PROMOTED_MODE is the machine mode of the register.  */
end_comment
...
block|if (nominal_mode != BLKmode&& nominal_mode != passed_mode&& stack_parm != 0) 	{ 	  rtx offset_rtx;
if|#
directive|if
name|BYTES_BIG_ENDIAN
block|if (GET_MODE_SIZE (nominal_mode)< UNITS_PER_WORD) 	    stack_offset.constant += (GET_MODE_SIZE (passed_mode) 				      - GET_MODE_SIZE (nominal_mode));
endif|#
directive|endif
block|offset_rtx = ARGS_SIZE_RTX (stack_offset); 	  if (offset_rtx == const0_rtx) 	    stack_parm = gen_rtx (MEM, nominal_mode, internal_arg_pointer); 	  else 	    stack_parm = gen_rtx (MEM, nominal_mode, 				  gen_rtx (PLUS, Pmode, 					   internal_arg_pointer, offset_rtx));
comment|/* If this is a memory ref that contains aggregate components, 	     mark it as such for cse and loop optimize.  */
...
argument_list|)
expr_stmt|;
comment|/* This must be done after rounding sizetree, so that it will subtract      the same value that we explicitly add below.  */
if|if
condition|(
name|where_pad
operator|==
...
argument_list|)
expr_stmt|;
comment|/* If not doing stupid register allocation copy the static chain      pointer into a psuedo.  If we have small register classes, copy the      value from memory if static_chain_incoming_rtx is a REG.  If we do      stupid register allocation, we use the stack address generated above.  */
if|if
condition|(
name|tem
operator|&&
