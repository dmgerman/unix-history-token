...
parameter_list|)
define|\
value|(GET_CODE (X) == REG					\    ? (CHEAP_REG (X) ? 0					\       : REGNO (X)>= FIRST_PSEUDO_REGISTER ? 1		\       : 2)						\    : rtx_cost (X, SET) * 2)
end_define
...
parameter_list|)
define|\
value|((X) == frame_pointer_rtx || (X) == hard_frame_pointer_rtx	\    || (X) == arg_pointer_rtx					\    || (X) == virtual_stack_vars_rtx				\    || (X) == virtual_incoming_args_rtx				\    || (GET_CODE (X) == PLUS&& GET_CODE (XEXP (X, 1)) == CONST_INT \&& (XEXP (X, 0) == frame_pointer_rtx			\ 	   || XEXP (X, 0) == hard_frame_pointer_rtx		\ 	   || XEXP (X, 0) == arg_pointer_rtx			\ 	   || XEXP (X, 0) == virtual_stack_vars_rtx		\ 	   || XEXP (X, 0) == virtual_incoming_args_rtx)))
end_define
...
literal|0
return|;
comment|/* For integer comparisons of A and B maybe we can simplify A - B and can      then simplify a comparison of that with zero.  If A and B are both either      a register or a CONST_INT, this can't help; testing for these cases will      prevent infinite recursion here and speed things up.       If CODE is an unsigned comparison, we can only do this if A - B is a      constant integer, and then we have to compare that integer with zero as a      signed comparison.  Note that this will give the incorrect result from      comparisons that overflow.  Since these are undefined, this is probably      OK.  If it causes a problem, we can check for A or B being an address      (fp + const or SYMBOL_REF) and only do it in that case.  */
if|if
condition|(
name|INTEGRAL_MODE_P
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Indicate this is a constant.  This isn't a 		       valid form of CONST, but it will only be used 		       to fold the next insns and then discarded, so 		       it should be safe.  */
return|return
name|gen_rtx
argument_list|(
name|CONST
...
expr_stmt|;
block|}
comment|/* Store the equivalent value in SRC_EQV, if different, or if the DEST      is a STRICT_LOW_PART.  The latter condition is necessary because SRC_EQV      is handled specially for this case, and if it isn't set, then there will      be no equivalence for the destinatation.  */
if|if
condition|(
name|n_sets
operator|==
...
name|everything
expr_stmt|;
comment|/* (mem (scratch)) means clobber everything.  */
elseif|else
if|if
condition|(
name|GET_CODE
