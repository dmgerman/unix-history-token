...
begin_comment
comment|/* Compute register class preferences for pseudo-registers.    Copyright (C) 1987, 88, 91, 92, 93, 1994 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If we couldn't find a valid mode, fall back to `word_mode'. 	 ??? We assume `word_mode' has already been initialized.          ??? One situation in which we need to do this is on the mips where 	 HARD_REGNO_NREGS (fpreg, [SD]Fmode) returns 2.  Ideally we'd like 	 to use DF mode for the even registers and VOIDmode for the odd 	 (for the cpu models where the odd ones are inaccessable).  */
if|if
condition|(
name|reg_raw_mode
index|[
...
name|warning
argument_list|(
literal|"request to mark `%s' as %s ignored by bytecode compiler"
argument_list|,
name|name
argument_list|,
name|call_used
...
begin_comment
comment|/* Indexed by pseudo register number, gives uid of last insn using the reg    or mentioning it in a note (as of the time reg_scan is called).  */
end_comment
...
begin_comment
comment|/* X is the expression to scan.  INSN is the insn it appears in.    NOTE_FLAG is nonzero if X is from INSN's notes rather than its body.  */
end_comment
