...
block|,
comment|/* This is for GNAT and is temporary.  */
literal|"-gnat"
block|,
literal|0
block|}
...
literal|0
expr_stmt|;
comment|/* Ada will use extensions like .ada, .adb, and .ads, so just test      for "ad".  */
elseif|else
if|if
condition|(
name|len
...
argument_list|)
empty_stmt|;
comment|/* If function is inline, and we don't yet know whether to 	 compile it by itself, defer decision till end of compilation. 	 finish_compilation will call rest_of_compilation again 	 for those functions that need to be output.  Also defer those 	 functions that were marked inline but weren't inlined; they 	 may never be used.  */
if|if
condition|(
operator|(
name|specd
