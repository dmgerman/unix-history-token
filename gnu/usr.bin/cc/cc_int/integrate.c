...
operator|)
expr_stmt|;
comment|/* If we have made a nonlocal label local, it means that this 	 inlined call will be refering to our nonlocal goto handler. 	 So make sure we create one for this block; we normally would 	 not since this is not otherwise considered a "call".  */
if|if
condition|(
name|LABEL_REF_NONLOCAL_P
argument_list|(
...
name|warning
argument_list|(
literal|"`inline' ignored for bytecode output"
argument_list|)
expr_stmt|;
return|return;
block|}
