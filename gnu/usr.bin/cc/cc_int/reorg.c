...
begin_comment
comment|/* Perform instruction reorganizations for delay slot filling.    Copyright (C) 1992, 1993, 1994 Free Software Foundation, Inc.    Contributed by Richard Kenner (kenner@vlsi1.ultra.nyu.edu).    Hacked by Michael Tiemann (tiemann@cygnus.com).  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
parameter_list|)
define|\
value|do { (RES)->memory = (RES)->volatil = (RES)->cc = 0;	\       CLEAR_HARD_REG_SET ((RES)->regs); } while (0)
end_define
...
begin_comment
comment|/* Given X, some rtl, and RES, a pointer to a `struct resource', mark    which resources are references by the insn.  If INCLUDE_CALLED_ROUTINE    is TRUE, resources used by the called routine will be included for    CALL_INSNs.  */
end_comment
...
expr_stmt|;
block|}
comment|/* ... and also what it's RTL says it modifies, if anything.  */
case|case
name|JUMP_INSN
case|:
case|case
...
begin_comment
comment|/* Return TRUE if any resource marked in RES, a `struct resources', is    referenced by INSN.  If INCLUDE_CALLED_ROUTINE is set, return if the called    routine is using those resources.     We compute this by computing all the resources referenced by INSN and    seeing if this conflicts with RES.  It might be faster to directly check    ourselves, and this is the way it used to work, but it means duplicating    a large block of complex code.  */
end_comment
...
begin_comment
comment|/* Return TRUE if INSN modifies resources that are marked in RES.    INCLUDE_CALLED_ROUTINE is set if the actions of that routine should be    included.   CC0 is only modified if it is explicitly set; see comments    in front of mark_set_resources for details.  */
end_comment
...
decl_stmt|;
block|{
comment|/* If we have an empty list, just make a new list element.  If      INSN has it's block number recorded, clear it since we may      be moving the insn to a new block.  */
if|if
condition|(
name|delay_list
operator|==
...
begin_comment
comment|/* Delete INSN from the the delay slot of the insn that it is in.  This may    produce an insn without anything in its delay slots.  */
end_comment
...
return|;
block|}
comment|/* Look at the relative rarities of the fallthough and destination.  If      they differ, we can predict the branch that way. */
switch|switch
condition|(
name|rare_fallthrough
operator|-
...
name|i
decl_stmt|;
comment|/* We can't do anything if there are more delay slots in SEQ than we      can handle, or if we don't know that it will be a taken branch.       We know that it will be a taken branch if it is either an unconditional      branch or a conditional branch with a stricter branch condition.  */
if|if
condition|(
name|XVECLEN
argument_list|(
...
block|}
block|}
comment|/* If all insns in the delay slot have been matched and we were previously      annulling the branch, we need not any more.  In that case delete all the      merged insns.  Also clear the INSN_FROM_TARGET_P bit of each insn the      the delay list so that we know that it isn't only being used at the      target.  */
if|if
condition|(
name|slot_number
operator|==
...
literal|0
return|;
comment|/* If this this is an INSN or JUMP_INSN with delayed effects, it 	     is hard to track the resource needs properly, so give up.  */
ifdef|#
directive|ifdef
name|INSN_SETS_ARE_DELAYED
if|if
...
argument_list|)
expr_stmt|;
comment|/* Now step forward from TARGET looking for registers that are set before      they are used.  These are dead.  If we pass a label, any pending dead      registers that weren't yet used can be made dead.  Stop when we pass a      conditional JUMP_INSN; follow the first few unconditional branches.  */
name|CLEAR_RESOURCE
argument_list|(
operator|&
name|set
...
condition|)
block|{
comment|/* Get our target and show how many more uses we want to 		     see before we hit the label.  */
name|target
operator|=
name|JUMP_LABEL
argument_list|(
...
literal|1
expr_stmt|;
comment|/* If this is our target, see if we have seen all its uses. 		     If so, indicate we have passed our target and ignore it. 		     All other labels cause us to stop our search.  */
if|if
condition|(
name|trial
operator|==
...
expr_stmt|;
block|}
comment|/* If there are slots left to fill and our search was stopped by an 	     unconditional branch, try the insn at the branch target.  We can 	     redirect the branch if it works.  */
if|if
condition|(
name|slots_to_fill
operator|!=
...
condition|)
continue|continue;
comment|/* Get the next active fallthough and target insns and see if we own 	 them.  Then see whether the branch is likely true.  We don't need 	 to do a lot of this for unconditional branches.  */
name|insn_at_target
operator|=
name|next_active_insn
argument_list|(
