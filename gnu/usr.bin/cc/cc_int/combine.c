...
begin_comment
comment|/* Optimize by combining instructions for GNU compiler.    Copyright (C) 1987, 1988, 1992, 1993, 1994 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* Define a defulat value for REVERSIBLE_CC_MODE.    We can never assume that a condition code mode is safe to reverse unless    the md tells us so.  */
end_comment
...
begin_comment
comment|/* If nonzero, this is the insn that should be presumed to be    immediately in front of `subst_insn'.  */
end_comment
...
parameter_list|)
define|\
value|do { rtx _new = (NEWVAL);						\       if (undobuf.num_undo< MAX_UNDO)					\ 	{								\ 	  undobuf.undo[undobuf.num_undo].is_int = 0;			\ 	  undobuf.undo[undobuf.num_undo].where.r =&INTO;		\ 	  undobuf.undo[undobuf.num_undo].old_contents.r = INTO;	\ 	  INTO = _new;							\ 	  if (undobuf.undo[undobuf.num_undo].old_contents.r != INTO)	\ 	    undobuf.num_undo++; 					\ 	}								\     } while (0)
end_define
...
argument_list|)
operator|)
comment|/* Don't extend the life of a hard register unless it is 		 user variable (if we have few registers) or it can't 		 fit into the desired register (meaning something special 		 is going on).  */
operator|||
operator|(
name|REGNO
argument_list|(
...
argument_list|)
return|;
comment|/* A paradoxical SUBREG of a VOIDmode constant is the same constant, 	 since we are saying that the high bits don't matter.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
operator|==
literal|1
comment|/* (minus 1 (comparison foo bar)) can be done by reversing the comparison 	 code if valid.  */
if|if
condition|(
name|XEXP
argument_list|(
...
literal|0
return|;
comment|/* Unless we are allowed to span bytes, reject this if we would be      spanning bytes or if the position is not a constant and the length      is not 1.  In all other cases, we would only be going outside      out object in cases when an original shift would have been      undefined.  */
if|if
condition|(
operator|!
name|spans_byte
...
name|temp
decl_stmt|;
comment|/* If this is a CALL, don't do anything.  Some of the code below      will do the wrong thing since the mode of a CALL is VOIDmode.  */
if|if
condition|(
name|code
operator|==
...
argument_list|)
expr_stmt|;
comment|/* We can only change the mode of the shift if we can do arithmetic 	     in the mode of the shift and MASK is no wider than the width of 	     OP_MODE.  */
if|if
condition|(
name|GET_MODE_BITSIZE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If this is a sign-extension operation that just affects bits 	 we don't care about, remove it.  Be sure the call above returned 	 something that is still a shift.  */
if|if
condition|(
operator|(
name|GET_CODE
...
return|;
block|}
comment|/* If this is a COMPARE, do nothing, since the IF_THEN_ELSE we would      make can't possibly match and would supress other optimizations.  */
elseif|else
if|if
condition|(
name|code
...
operator|-
literal|1
comment|/* See if we have PLUS, IOR, XOR, MINUS or UMAX, where one of the 	 operands is zero when the other is non-zero, and vice-versa.  */
if|if
condition|(
operator|(
name|code
...
block|}
block|}
comment|/* Similarly for MULT, AND and UMIN, execpt that for these the result 	 is always zero.  */
if|if
condition|(
operator|(
name|code
...
return|;
block|}
comment|/* If X is a normal SUBREG with both inner and outer modes integral,      we can narrow both the true and false values of the inner expression,      if there is a condition.  */
elseif|else
if|if
condition|(
name|code
...
name|MODE_CC
case|:
comment|/* If the mode of the condition codes tells us that this is safe, 	 we need look no further.  */
if|if
condition|(
name|REVERSIBLE_CC_MODE
argument_list|(
...
name|set
decl_stmt|;
comment|/* If there is an insn that is supposed to be immediately 	 in front of subst_insn, use it.  */
if|if
condition|(
name|subst_prev_insn
operator|!=
