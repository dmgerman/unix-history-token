...
begin_comment
comment|/* Emit RTL for the GNU C-Compiler expander.    Copyright (C) 1987, 1988, 1992, 1993, 1994 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* (CONST_INT STORE_FLAG_VALUE) */
end_comment
...
begin_comment
comment|/* (REG:Pmode HARD_FRAME_POINTER_REGNUM) */
end_comment
...
begin_comment
comment|/* (REG:Pmode VIRTUAL_INCOMING_ARGS_REGNUM) */
end_comment
...
begin_comment
comment|/* (REG:Pmode VIRTUAL_STACK_VARS_REGNUM) */
end_comment
...
begin_comment
comment|/* (REG:Pmode VIRTUAL_STACK_DYNAMIC_REGNUM) */
end_comment
...
begin_comment
comment|/* (REG:Pmode VIRTUAL_OUTGOING_ARGS_REGNUM) */
end_comment
...
name|val
decl_stmt|;
comment|/* Don't let anything called by or after reload create new registers      (actually, registers can't be created after flow, but this is a good      approximation).  */
if|if
condition|(
name|reload_in_progress
operator|||
...
block|{
comment|/* MODE must be narrower than HOST_BITS_PER_INT.  */
name|int
name|width
init|=
name|GET_MODE_BITSIZE
...
argument_list|()
expr_stmt|;
comment|/* If OP is narrower than a word or if we want a word outside OP, fail.  */
if|if
condition|(
name|mode
operator|!=
...
condition|)
block|{
comment|/* If the register is not valid for MODE, return 0.  If we don't 	 do this, there is no way to fix up the resulting REG later.  */
if|if
condition|(
name|REGNO
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* We handle 32-bit and>= 64-bit words here.  Note that the order in 	 which the words are written depends on the word endianness.  	 ??? This is a potential portability problem and should 	 be fixed at some point.  */
if|if
condition|(
name|BITS_PER_WORD
operator|==
...
operator|)
expr_stmt|;
comment|/* If BITS_PER_WORD is smaller than an int, get the appropriate bits.  */
if|if
condition|(
name|BITS_PER_WORD
operator|<
...
argument_list|)
expr_stmt|;
comment|/* These types may be freely shared so we needn't do any reseting      for them.  */
switch|switch
condition|(
name|code
condition|)
...
begin_comment
comment|/* Add INSN into the doubly-linked list after insn AFTER.  This should be the    only function called to insert an insn once delay slots have been filled    since only it knows how to update a SEQUENCE.  */
end_comment
...
begin_comment
comment|/* Make an insn of code BARRIER    and output it before the insn AFTER.  */
end_comment
...
condition|)
block|{
comment|/* FIXME: for now we do nothing, but eventually we will have to deal with 	 debugging information.  */
return|return
literal|0
return|;
block|}
...
begin_comment
comment|/* Begin emitting insns to a sequence which can be packaged in an RTL_EXPR.  */
end_comment
...
begin_comment
comment|/* After emitting to a sequence, restore previous saved state.     To get the contents of the sequence just made,    you must call `gen_sequence' *before* calling here.  */
end_comment
...
block|{
comment|/* Ensure that this rtl goes in saveable_obstack, since we may be 	 caching it.  */
...
expr_stmt|;
block|}
comment|/* These four calls obtain some of the rtx expressions made above.  */
name|const0_rtx
operator|=
name|GEN_INT
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* This will usually be one of the above constants, but may be a new rtx.  */
name|const_true_rtx
operator|=
name|GEN_INT
argument_list|(
