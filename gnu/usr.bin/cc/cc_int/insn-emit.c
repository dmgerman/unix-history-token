...
argument_list|)
expr_stmt|;
comment|/* Don't generate memory->memory moves, go through a register */
elseif|else
if|if
condition|(
name|TARGET_MOVE
...
expr_stmt|;
block|{
comment|/* Special case memory->memory moves and pushes */
if|if
condition|(
name|TARGET_MOVE
operator|&&
...
expr_stmt|;
block|}
comment|/* If we are loading a floating point constant that isn't 0 or 1 into a register,      indicate we need the pic register loaded.  This could be optimized into stores      of constants if the target eventually moves to memory, but better safe than      sorry.  */
if|if
condition|(
name|flag_pic
operator|&&
...
expr_stmt|;
block|}
comment|/* The optimizer does not know that the call sets the function value      registers we stored in the result block.  We avoid problems by      claiming that all hard registers are used and clobbered at this      point.  */
name|emit_insn
argument_list|(
name|gen_blockage
argument_list|()
