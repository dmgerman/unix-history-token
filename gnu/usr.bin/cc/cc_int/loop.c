...
begin_comment
comment|/* Holds the number of loop iterations.  It is zero if the number could not be    calculated.  Must be unsigned since the number of iterations can    be as high as 2^wordsize-1.  For loops with a wider iterator, this number    will will be zero if the number of loop iterations is too large for an    unsigned integer to hold.  */
end_comment
...
condition|)
block|{
comment|/* Replace any usage in a REG_EQUAL note.  */
name|REG_NOTES
argument_list|(
name|reg_single_usage
index|[
...
comment|/* This code appears in three places, once in scan_loop, and twice 	 in strength_reduce.  */
elseif|else
if|if
condition|(
operator|(
...
directive|if
literal|0
comment|/* This isn't needed because REG_NOTES is copied 			 below and is wrong since P might be a PARALLEL.  */
block|if (REG_NOTES (i1) == 0&& ! m->partial
comment|/* But not if it's a zero-extend clr. */
block|&& ! m->global
comment|/* and not if used outside the loop 					    (since it might get set outside).  */
block|&& CONSTANT_P (SET_SRC (PATTERN (p)))) 			REG_NOTES (i1) 			  = gen_rtx (EXPR_LIST, REG_EQUAL, 				     SET_SRC (PATTERN (p)), REG_NOTES (i1));
endif|#
directive|endif
...
begin_endif
unit|if (XEXP (x, 0) != reg) 	abort ();       XEXP (x, 0) = addr;       return;     }    fmt = GET_RTX_FORMAT (code);   for (i = GET_RTX_LENGTH (code) - 1; i>= 0; i--)     {       if (fmt[i] == 'e') 	replace_call_address (XEXP (x, i), reg, addr);       if (fmt[i] == 'E') 	{ 	  register int j; 	  for (j = 0; j< XVECLEN (x, i); j++) 	    replace_call_address (XVECEXP (x, i, j), reg, addr); 	}     } }
endif|#
directive|endif
end_endif
...
return|return;
default|default:
comment|/* Treat anything else (such as a symbol_ref) 	 as a branch out of this loop, but not into any loop.  */
if|if
condition|(
name|loop_num
operator|!=
...
block|}
block|}
comment|/* Past CODE_LABEL, we get to insns that may be executed multiple 	 times.  The only way we can be sure that they can't is if every 	 every jump insn between here and the end of the loop either 	 returns, exits the loop, or is a forward jump.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
block|}
block|}
comment|/* Past a label or a jump, we get to insns for which we can't count 	 on whether or how many times they will be executed during each 	 iteration.  */
comment|/* This code appears in three places, once in scan_loop, and twice 	 in strength_reduce.  */
if|if
condition|(
operator|(
...
comment|/* ??? Perhaps attempt to guess whether autoincrement will handle 	     some of the new add insns; if so, can increase BENEFIT 	     (undo the subtraction of add_cost that was done above).  */
...
name|USE
case|:
comment|/* Both invariant.  Only valid if sum is machine operand. 	       First strip off possible USE on first operand.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
index|]
decl_stmt|;
comment|/* If there are no givs for this biv, and the only exit is the 	     fall through at the end of the the loop, then 	     see if perhaps there are no uses except to count.  */
name|no_use_except_counting
operator|=
literal|1
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Now check other conditions: 	     initial_value must be zero, 	     final_value % add_val == 0, so that when reversed, the 	     biv will be zero on the last iteration.  	     This test can probably be improved since +/- 1 in the constant 	     can be obtained by changing LT to LE and vice versa; this is 	     confusing.  */
if|if
condition|(
name|comparison
operator|&&
...
directive|if
literal|0
comment|/* XXX patch to correct strength-reduction problem from 	 * Richard Henderson<richard@atheist.tamu.edu> incorporated 	 * 3 Jan 1996 - jkh@FreeBSD.org 	 */
block|if (CONSTANT_P (arg)) 	{
...
block|for (v = bl->giv; v; v = v->next_iv) 	    if (CONSTANT_P (v->mult_val)&& INTVAL (v->mult_val)> 0&& CONSTANT_P (v->add_val)&& ! v->ignore&& ! v->maybe_dead&& v->always_computable&& v->mode == mode) 	      { 		if (! eliminate_p) 		  return 1;
...
block|if (GET_CODE (arg) == CONST_INT&& GET_CODE (v->mult_val) == CONST_INT&& GET_CODE (v->add_val) == CONST_INT&& validate_change (insn,&XEXP (x, arg_operand), 					GEN_INT (INTVAL (arg) 						 * INTVAL (v->mult_val) 						 + INTVAL (v->add_val)), 0)) 		  return 1;
comment|/* Otherwise, load it into a register.  */
block|tem = gen_reg_rtx (mode); 		emit_iv_add_mult (arg, v->mult_val, v->add_val, tem, where); 		if (validate_change (insn,&XEXP (x, arg_operand), tem, 0)) 		  return 1;
comment|/* If that failed, put back the change we made above.  */
block|XEXP (x, 1-arg_operand) = reg; 	      }
comment|/* Look for giv with positive constant mult_val and nonconst add_val. 	     Insert insns to calculate new compare value.  */
block|for (v = bl->giv; v; v = v->next_iv) 	    if (CONSTANT_P (v->mult_val)&& INTVAL (v->mult_val)> 0&& ! v->ignore&& ! v->maybe_dead&& v->always_computable&& v->mode == mode) 	      { 		rtx tem;  		if (! eliminate_p) 		  return 1;  		tem = gen_reg_rtx (mode);
comment|/* Replace biv with giv's reduced register.  */
block|validate_change (insn,&XEXP (x, 1 - arg_operand), 				 v->new_reg, 1);
comment|/* Compute value to compare against.  */
block|emit_iv_add_mult (arg, v->mult_val, v->add_val, tem, where);
comment|/* Use it in this insn.  */
block|validate_change (insn,&XEXP (x, arg_operand), tem, 1); 		if (apply_change_group ()) 		  return 1; 	      } 	}       else if (GET_CODE (arg) == REG || GET_CODE (arg) == MEM) 	{ 	  if (invariant_p (arg) == 1) 	    {
comment|/* Look for giv with constant positive mult_val and nonconst 		 add_val. Insert insns to compute new compare value.  */
block|for (v = bl->giv; v; v = v->next_iv) 		if (CONSTANT_P (v->mult_val)&& INTVAL (v->mult_val)> 0&& ! v->ignore&& ! v->maybe_dead&& v->always_computable&& v->mode == mode) 		  { 		    rtx tem;  		    if (! eliminate_p) 		      return 1;  		    tem = gen_reg_rtx (mode);
comment|/* Replace biv with giv's reduced register.  */
block|validate_change (insn,&XEXP (x, 1 - arg_operand), 				     v->new_reg, 1);
comment|/* Compute value to compare against.  */
block|emit_iv_add_mult (arg, v->mult_val, v->add_val, 				      tem, where); 		    validate_change (insn,&XEXP (x, arg_operand), tem, 1); 		    if (apply_change_group ()) 		      return 1; 		  } 	    }
...
block|for (v = bl->giv; v; v = v->next_iv) 	    { 	      struct induction *tv;  	      if (v->ignore || v->maybe_dead || v->mode != mode) 		continue;  	      for (tv = reg_biv_class[REGNO (arg)]->giv; tv; tv = tv->next_iv) 		if (! tv->ignore&& ! tv->maybe_dead&& rtx_equal_p (tv->mult_val, v->mult_val)&& rtx_equal_p (tv->add_val, v->add_val)&& tv->mode == mode) 		  { 		    if (! eliminate_p) 		      return 1;
