...
begin_function_decl
name|int
name|bc_expand_exit_loop_if_false
parameter_list|()
function_decl|;
end_function_decl
...
block|}
block|}
comment|/* Mark the cleanups of exited blocks so that they are executed      by the code above.  */
for|for
control|(
name|prev
operator|=
...
name|error
argument_list|(
literal|"`asm' is illegal when generating bytecode"
argument_list|)
expr_stmt|;
return|return;
block|}
...
return|return;
block|}
comment|/* If an output operand is not a variable or indirect ref, 	 or a part of one, 	 create a SAVE_EXPR which is a pseudo-reg 	 to act as an intermediate temporary. 	 Make the asm insn write into that, then copy it to 	 the real output operand.  */
while|while
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If it's a constant, print error now so don't crash later.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
return|return;
block|}
comment|/* The time complexity of this loop is currently O(N * M), with      N being the number of members in the enumerated type, and      M being the number of case expressions in the switch. */
for|for
control|(
name|chain
operator|=
...
control|)
block|{
comment|/* Find a match between enumeral and case expression, if possible. 	 Quit looking when we've gone too far (since case expressions 	 are kept sorted in ascending order).  Warn about enumerators not 	 handled in the switch statement case expression list. */
for|for
control|(
name|n
operator|=
...
directive|if
literal|0
comment|/* ??? This optimization is disabled because it causes valid programs to      fail.  ANSI C does not guarantee that an expression with enum type      will have a value that is the same as one of the enumation literals.  */
