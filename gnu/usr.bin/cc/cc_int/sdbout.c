...
begin_comment
comment|/* Mips systems use the SDB functions to dump out symbols, but    do not supply usable syms.h include files.  */
end_comment
...
begin_comment
comment|/* Use T_INT if we don't have T_VOID.  */
end_comment
...
begin_comment
comment|/* Define the default sizes for various types.  */
end_comment
...
name|a
parameter_list|)
value|fprintf (asm_out_file, "\t.val\t%d%s", (a), SDB_DELIM)
end_define
...
parameter_list|)
define|\
value|do { fprintf (asm_out_file, "\t.def\t");	\      ASM_OUTPUT_LABELREF (asm_out_file, a); 	\      fprintf (asm_out_file, SDB_DELIM); } while (0)
end_define
...
name|a
parameter_list|)
value|fprintf(asm_out_file, "\t.size\t%d%s", a, SDB_DELIM)
end_define
...
parameter_list|)
define|\
value|do { fprintf (asm_out_file, "\t.tag\t");	\      ASM_OUTPUT_LABELREF (asm_out_file, a);	\      fprintf (asm_out_file, SDB_DELIM); } while (0)
end_define
...
parameter_list|)
define|\
value|do { fprintf (asm_out_file, "\t.def\t");		\      ASM_OUTPUT_LABELREF (asm_out_file, NAME);		\      fprintf (asm_out_file,				\ 	      "%s\t.val\t.%s\t.scl\t-1%s\t.endef\n",	\ 	      SDB_DELIM, SDB_DELIM, SDB_DELIM); } while (0)
end_define
...
directive|if
literal|1
comment|/* As a temporary hack, use typedef names for C++ only.  */
elseif|else
if|if
condition|(
name|TREE_CODE
...
directive|if
name|BYTES_BIG_ENDIAN
block|if (offset != -1&& TREE_TYPE (parms) != DECL_ARG_TYPE (parms)) 	      offset += (GET_MODE_SIZE (TYPE_MODE (DECL_ARG_TYPE (parms))) 			 - GET_MODE_SIZE (GET_MODE (DECL_RTL (parms))));
endif|#
directive|endif
block|if (INTVAL (XEXP (XEXP (DECL_RTL (parms), 0), 1)) != offset) {...}
endif|#
directive|endif
block|{
if|if
