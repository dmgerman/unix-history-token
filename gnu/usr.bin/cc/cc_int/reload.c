...
name|PLUS
condition|)
comment|/* We can get a PLUS as an "operand" as a result of 	   register elimination.  See eliminate_regs and gen_reload.  */
name|substed_operand
index|[
name|i
index|]
...
argument_list|)
expr_stmt|;
comment|/* Force reload if this is a constant or PLUS or if there may may 		 be a problem accessing OPERAND in the outer mode.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
argument_list|)
operator|)
comment|/* If the ouput is an earlyclobber operand, this is 		 a conflict.  */
operator|||
operator|(
operator|(
name|GET_CODE
...
begin_comment
comment|/* This function is used to print the variables set by 'find_reloads' */
end_comment
