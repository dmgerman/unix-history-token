...
argument_list|()
expr_stmt|;
comment|/* If X is a CONCAT, we got insns like RD = RS, ID = IS, 	 each with a separate pseudo as destination. 	 It's not correct for flow to treat them as a unit.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If TEMP is a volatile MEM and we want a result value, make 	 the access now so it gets done only once.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Figure out how much is left in TARGET 		 that we have to clear.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If we are building a static constructor into a register, 	 set the initial value as zero so we can fold the value into 	 a constant.  */
elseif|else
if|if
condition|(
name|GET_CODE
...
condition|)
block|{
comment|/* We don't currently allow variable indices in a 		 C initializer, but let's try here to support them.  */
name|rtx
name|pos_rtx
decl_stmt|,
name|addr
...
literal|0
condition|)
comment|/* We don't know what this can modify.  */
return|return
literal|0
return|;
break|break;
...
argument_list|()
expr_stmt|;
comment|/* In some cases, we will be offsetting OP0's address by a constant. 	   So get it as a sum, if possible.  If we will be using it 	   directly in an insn, we validate it.  */
name|op0
operator|=
name|expand_expr
argument_list|(
...
expr_stmt|;
block|}
comment|/* If the result is to be Pmode and we are adding an integer to 	 something, we might be forming a constant.  So try to use 	 plus_constant.  If it produces a sum and we can't accept it, 	 use force_operand.  This allows P =&ARR[const] to generate 	 efficient code on machines where a SYMBOL_REF is not a valid 	 address.  	 If this is an EXPAND_SUM call, always return the sum.  */
if|if
condition|(
name|modifier
operator|==
...
argument_list|)
decl_stmt|;
comment|/* If one operand is a CONST_INT, put it last.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* For each register that may be used for calling a function, this    gives the offset of that register into the block returned by    __bultin_apply_args.  0 indicates that the register is not    used for calling a function. */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* Here we get the insn before what was just emitted. 	 On some machines, emitting the branch can discard 	 the previous compare insn and emit a replacement.  */
if|if
condition|(
name|prev
operator|==
literal|0
condition|)
comment|/* If there's only one preceding insn...  */
name|insn
operator|=
name|get_insns
argument_list|()
