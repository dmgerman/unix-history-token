...
block|{
comment|/* The args are chosen so that the last part 	     includes the lsb.  */
name|int
name|bit_offset
init|=
literal|0
decl_stmt|;
comment|/* If the value isn't in memory, then it must be right aligned 	     if a register, so skip past the padding on the left.  If it 	     is in memory, then there is no padding on the left.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
block|if (SHIFT_COUNT_TRUNCATED&& GET_CODE (op1) == CONST_INT&& (unsigned HOST_WIDE_INT) INTVAL (op1)>= GET_MODE_BITSIZE (mode))     op1 = GEN_INT ((unsigned HOST_WIDE_INT) INTVAL (op1) 		   % GET_MODE_BITSIZE (mode));
endif|#
directive|endif
if|if
condition|(
...
begin_comment
comment|/* Emit code to multiply OP0 and CNST1, putting the high half of the result    in TARGET if that is convenient, and return where the result is.  If the    operation can not be performed, 0 is returned.     MODE is the mode of operation and result.     UNSIGNEDP nonzero means unsigned multiply.  */
end_comment
...
argument_list|)
return|;
comment|/* Thirdly, we try to use a widening multiplication, or a wider mode      multiplication.  */
name|moptab
operator|=
name|unsignedp
condition|?
...
block|}
block|}
comment|/* As a last resort, try widening the mode and perform a 	 non-widening multiplication.  */
name|moptab
operator|=
name|smul_optab
expr_stmt|;
...
directive|if
literal|0
comment|/* It should be possible to restrict the precision to GET_MODE_BITSIZE      (mode), and thereby get better code when OP1 is a constant.  Do that for      GCC 2.7.  It will require going over all usages of SIZE below.  */
...
argument_list|()
expr_stmt|;
comment|/* Promote floor rouding to trunc rounding for unsigned operations.  */
if|if
condition|(
name|unsignedp
condition|)
...
condition|)
block|{
comment|/* Most significant bit of divisor is set, emit a scc insn. 		       emit_store_flag needs to be passed a place for the 		       result.  */
name|quotient
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Can emit_store_flag have failed? */
if|if
condition|(
name|quotient
operator|==
...
name|ROUND_MOD_EXPR
case|:
comment|/* The code that used to be here was wrong, and nothing really 	   depends on it.  */
name|abort
argument_list|()
expr_stmt|;
break|break;
