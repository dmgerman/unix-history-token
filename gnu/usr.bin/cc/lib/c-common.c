...
expr_stmt|;
block|}
comment|/* If the default size of char arrays isn't big enough for the name,      make a bigger one.  */
name|len
operator|=
name|strlen
argument_list|(
...
condition|)
block|{
comment|/* ??? This is a warning, not a pedwarn, in 2.4, 	 because it happens in contexts that are not 	 "constant expressions" in ANSI C. 	 Fix the problem differently in 2.5.  */
name|warning
argument_list|(
literal|"overflow in constant expression"
argument_list|)
expr_stmt|;
comment|/* Suppress duplicate warnings.  */
name|TREE_CONSTANT_OVERFLOW
argument_list|(
name|value
argument_list|)
