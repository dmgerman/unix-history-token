...
name|end
decl_stmt|;
comment|/* Endinf offset or register number.  */
block|}
struct|;
end_struct
...
directive|endif
comment|/* We can only record one secondary and one tertiary reload.  If both 	 IN and OUT need secondary reloads, we can only make an in-out 	 reload if neither need an insn and if the classes are compatible. 	 If they aren't, all we can do is abort since making two separate 	 reloads is invalid.  */
if|if
condition|(
name|secondary_class
operator|!=
...
directive|ifdef
name|SECONDARY_MEMORY_NEEDED
comment|/* Likewise for different secondary memory locations.  */
operator|&&
operator|(
name|secondary_memlocs_elim
index|[
...
operator|!
name|no_input_reloads
comment|/* Optional output reloads don't do anything and we mustn't 	       make in-out reloads on insns that are not permitted output 	       reloads.  */
operator|&&
operator|(
name|modified
index|[
...
argument_list|)
expr_stmt|;
comment|/* In these cases, we can't tell if the operand is an input 	 or an output, so be conservative.  In practice it won't be 	 problem.  */
if|if
condition|(
name|code
operator|==
...
begin_comment
comment|/* Scan X for memory references and scan the addresses for reloading.    Also checks for references to "constant" regs that we want to eliminate    and replaces them with the values they stand for.    We may alter X destructively if it contains a reference to such.    If X is just a constant reg, we return the equivalent value    instead of X.     IND_LEVELS says how many levels of indirect addressing this machine    supports.     OPNUM and TYPE identify the purpose of the reload.     IS_SET_DEST is true if X is the destination of a SET, which is not    appropriate to be replaced by a constant.  */
end_comment
...
return|;
block|}
comment|/* We can avoid a reload if the register's equivalent memory expression 	 is valid as an indirect memory address. */
elseif|else
if|if
condition|(
name|reg_equiv_mem
...
begin_comment
comment|/* Record the pseudo registers we must reload into hard registers    in a subexpression of a would-be memory address, X.    (This function is not called if the address we find is strictly valid.)    CONTEXT = 1 means we are considering regs as index regs,    = 0 means we are considering them as base regs.     OPNUM and TYPE specify the purpose of any reloads made.     IND_LEVELS says how many levels of indirect addressing are    supported at this point in the address.     We return nonzero if X, as a whole, is reloaded or replaced.  */
end_comment
...
literal|0
comment|/* This might screw code in reload1.c to delete prior output-reload 	 that feeds this insn.  */
block|if (reg_equiv_mem[regno] != 0) 	{ 	  push_reload (reg_equiv_mem[regno], NULL_RTX, loc, NULL_PTR, 		       context ? INDEX_REG_CLASS : BASE_REG_CLASS, 		       GET_MODE (x), VOIDmode, 0, 0, opnum, type); 	  return 1; 	}
endif|#
directive|endif
if|if
condition|(
...
begin_comment
comment|/* Substitute into the current INSN the registers into which we have reloaded    the things that need reloading.  The array `replacements'    says contains the locations of all pointers that must be changed    and says what to replace them with.     Return the rtx that X translates into; usually X, but modified.  */
end_comment
