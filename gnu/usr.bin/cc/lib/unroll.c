...
begin_comment
comment|/* Values describing the current loop's iteration variable.  These are set up    by loop_iterations, and used by precondition_loop_p.  */
end_comment

begin_decl_stmt
specifier|static
name|rtx
name|loop_iteration_var
decl_stmt|;
end_decl_stmt
...
comment|/* If there is no strength reduce info, then set loop_n_iterations to zero.      This can happen if strength_reduce can't find any bivs in the loop.      A value of zero indicates that the number of iterations could not be      calculated.  */
if|if
condition|(
operator|!
name|strength_reduce_p
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: %d iterations.\n"
argument_list|,
name|loop_n_iterations
argument_list|)
expr_stmt|;
...
directive|ifdef
name|HAVE_cc0
comment|/* The immediately preceding insn is a compare which must be 	     deleted.  */
name|delete_insn
argument_list|(
name|last_loop_insn
argument_list|)
...
directive|else
comment|/* The immediately preceding insn may not be the compare, so don't 	     delete it.  */
name|delete_insn
argument_list|(
name|last_loop_insn
argument_list|)
...
condition|)
block|{
comment|/* Loops of these types should never start with a jump down to 	 the exit condition test.  For now, check for this case just to 	 be sure.  UNROLL_NAIVE loops can be of this form, this case is 	 handled below.  */
name|insn
operator|=
name|loop_start
expr_stmt|;
...
directive|ifdef
name|HAVE_cc0
comment|/* The instruction immediately before the JUMP_INSN is a compare 	     instruction which we do not want to copy.  */
name|copy_end
operator|=
name|PREV_INSN
argument_list|(
...
directive|else
comment|/* The instruction immediately before the JUMP_INSN may not be the 	     compare, so we must copy it.  */
name|copy_end
operator|=
name|PREV_INSN
argument_list|(
...
directive|ifdef
name|HAVE_cc0
comment|/* The instruction immediately before the JUMP_INSN is a compare 	     instruction which we do not want to copy or delete.  */
name|insert_before
operator|=
name|PREV_INSN
argument_list|(
...
directive|ifdef
name|HAVE_cc0
comment|/* The immediately preceding insn is a compare which we do not 		 want to copy.  */
name|copy_end
operator|=
name|PREV_INSN
argument_list|(
...
directive|else
comment|/* The immediately preceding insn may not be a compare, so we 		 must copy it.  */
name|copy_end
operator|=
name|PREV_INSN
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Space is needed in some of the map for new registers, so new_maxregnum      is an (over)estimate of how many registers will exist at the end.  */
name|new_maxregnum
operator|=
name|maxregnum
operator|+
...
operator|)
expr_stmt|;
comment|/* Must realloc space for the constant maps, because the number of registers      may have changed.  */
name|map
operator|->
name|const_equiv_map
operator|=
...
directive|endif
block|}
comment|/* If the loop is being partially unrolled, and the iteration variables      are being split, and are being renamed for the split, then must fix up      the compare instruction at the end of the loop to refer to the new      registers.  This compare isn't copied, so the registers used in it      will never be replaced if it isn't done here.  */
if|if
condition|(
name|unroll_type
operator|==
...
directive|if
literal|0
comment|/* If non-reduced/final-value givs were split, then this would also 	     have to remap those givs.  */
endif|#
directive|endif
name|tem
operator|=
...
argument_list|)
expr_stmt|;
comment|/* The set source is a register.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
block|{
comment|/* The set source is a compare of some sort.  */
name|tem
operator|=
name|XEXP
argument_list|(
...
block|}
block|}
comment|/* For unroll_number - 1 times, make a copy of each instruction      between copy_start and copy_end, and insert these new instructions      before the end of the loop.  */
for|for
control|(
name|i
operator|=
...
expr_stmt|;
block|}
comment|/* Can now delete the 'safety' label emitted to protect us from runaway      delete_insn calls.  */
if|if
condition|(
name|INSN_DELETED_P
argument_list|(
...
begin_comment
comment|/* ??? If the loop is known to be executed very many times, or the machine    has a very cheap divide instruction, then preconditioning is a win even    when the increment is not a power of 2.  Use RTX_COST to compute    whether divide is cheap.  */
end_comment
...
name|loop_dump_stream
argument_list|,
literal|"Preconditioning: Success, number of iterations known, %d.\n"
argument_list|,
name|loop_n_iterations
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* Must ensure that final_value is invariant, so call invariant_p to      check.  Before doing so, must check regno against max_reg_before_loop      to make sure that the register is in the range covered by invariant_p.      If it isn't, then it is most likely a biv/giv which by definition are      not invariant.  */
if|if
condition|(
operator|(
name|GET_CODE
...
return|;
block|}
comment|/* Now set initial_value to be the iteration_var, since that may be a      simpler expression, and is guaranteed to be correct if all of the      above tests succeed.       We can not use the initial_value as calculated, because it will be      one too small for loops of the form "while (i--> 0)".  We can not      emit code before the loop_skip_over insns to fix this problem as this      will then give a number one too large for loops of the form      "while (--i> 0)".       Note that all loops that reach here are entered at the top, because      this function is not called if the loop starts with a jump.  */
comment|/* Fail if loop_iteration_var is not live before loop_start, since we need      to test its value in the preconditioning code.  */
if|if
condition|(
name|uid_luid
index|[
...
argument_list|()
expr_stmt|;
comment|/* The insn loading the constant into a register is not longer needed, 	 so delete it.  */
name|delete_insn
argument_list|(
name|get_last_insn
argument_list|()
...
name|NOTE
case|:
comment|/* VTOP notes are valid only before the loop exit test.  If placed 	     anywhere else, loop may generate bad code.  */
if|if
condition|(
name|NOTE_LINE_NUMBER
argument_list|(
...
begin_comment
comment|/* Searches the insns between INSN and LOOP_END.  Returns 1 if there    is a backward branch in that range that branches to somewhere between    LOOP_START and INSN.  Returns 0 otherwise.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Check for the case where insn is the last insn in the loop.  */
if|if
condition|(
name|insn
operator|==
...
name|result
decl_stmt|;
comment|/* For increment, must check every instruction that sets it.  Each      instruction must be executed only once each time through the loop.      To verify this, we check that the the insn is always executed, and that      there are no backward branches after the insn that branch to before it.      Also, the insn must have a mult_val of one (to make sure it really is      an increment).  */
name|result
operator|=
name|const0_rtx
expr_stmt|;
...
begin_comment
comment|/* Determine the initial value of the iteration variable, and the amount    that it is incremented each loop.  Use the tables constructed by    the strength reduction pass to calculate these values.     Initial_value and/or increment are set to zero if their values could not    be calculated.  */
end_comment
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: No reg_iv_type entry for iteration var.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* Reject iteration variables larger than the host long size, since they      could result in a number of iterations greater than the range of our      `unsigned long' variable loop_n_iterations.  */
elseif|else
if|if
condition|(
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Iteration var rejected because mode larger than host long.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Iteration var not an integer.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
...
directive|if
literal|1
comment|/* ??? The code below does not work because the incorrect number of 	 iterations is calculated when the biv is incremented after the giv 	 is set (which is the usual case).  This can probably be accounted 	 for by biasing the initial_value by subtracting the amount of the 	 increment that occurs between the giv set and the giv test.  However, 	 a giv as an iterator is very rare, so it does not seem worthwhile 	 to handle this.  */
comment|/* ??? An example failure is: i = 6; do {;} while (i++< 9).  */
if|if
condition|(
name|loop_dump_stream
condition|)
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Giv iterators are not handled.\n"
argument_list|)
expr_stmt|;
return|return;
else|#
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Not basic or general induction var.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
...
begin_comment
comment|/* Calculate the approximate final value of the iteration variable    which has an loop exit test with code COMPARISON_CODE and comparison value    of COMPARISON_VALUE.  Also returns an indication of whether the comparison    was signed or unsigned, and the direction of the comparison.  This info is    needed to calculate the number of loop iterations.  */
end_comment
...
decl_stmt|;
block|{
comment|/* Calculate the final value of the induction variable.      The exact final value depends on the branch operator, and increment sign.      This is only an approximate value.  It will be wrong if the iteration      variable is not incremented by one each time through the loop, and      approx final value - start value % increment != 0.  */
operator|*
name|unsigned_p
operator|=
literal|0
...
name|EQ
case|:
comment|/* Can not calculate a final value for this case.  */
operator|*
name|compare_dir
operator|=
literal|0
...
comment|/* loop_number_exit_labels is non-zero if the loop has an exit other than 	 a fall through at the end.  */
name|biv_splittable
operator|=
literal|1
expr_stmt|;
...
condition|)
block|{
comment|/* If the initial value of the biv is itself (i.e. it is too 		 complicated for strength_reduce to compute), or is a hard 		 register, then we must create a new pseudo reg to hold the 		 initial value of the biv.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* For every giv based on the biv BL, check to determine whether it is    splittable.  This is a subroutine to find_splittable_regs ().  */
end_comment
...
condition|)
block|{
comment|/* If value is not a constant, register, or register plus 		 constant, then compute its value into a register before 		 loop start.  This prevents illegal rtx sharing, and should 		 generate better code.  We can use bl->initial_value here 		 instead of splittable_regs[bl->regno] because this code 		 is going before the loop start.  */
if|if
condition|(
name|unroll_type
operator|==
...
name|tem
expr_stmt|;
comment|/* If the address hasn't been checked for validity yet, do so 		     now, and fail completely if either the first or the last 		     unrolled copy of the address is not a valid address.  */
if|if
condition|(
name|v
operator|->
...
name|loop_dump_stream
argument_list|,
literal|"Illegal address for giv at insn %d\n"
argument_list|,
name|INSN_UID
argument_list|(
name|v
...
name|loop_dump_stream
argument_list|,
literal|"Illegal init insn, rewritten.\n"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|value
expr_stmt|;
comment|/* Check the resulting address for validity, and fail 		     if the resulting address would be illegal.  */
if|if
condition|(
operator|!
name|memory_address_p
...
literal|0
decl_stmt|;
comment|/* HACK: Must also search the loop fall through exit, create a label_ref      here which points to the loop_end, and append the loop_number_exit_labels      list to it.  */
name|label
operator|=
name|gen_rtx
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Make sure loop_end is not the last insn.  */
if|if
condition|(
name|NEXT_INSN
argument_list|(
...
condition|)
block|{
comment|/* Can calculate the loop exit value of its biv as 	     (loop_n_iterations * increment) + initial_value */
...
begin_comment
comment|/* Calculate the number of loop iterations.  Returns the exact number of loop    iterations if it can be calculated, otherwise returns zero.  */
end_comment
...
name|last_loop_insn
decl_stmt|;
comment|/* First find the iteration variable.  If the last insn is a conditional      branch, and the insn before tests a register value, make that the      iteration variable.  */
name|loop_initial_value
operator|=
literal|0
expr_stmt|;
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: No final conditional branch found.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Comparison not against register.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
comment|/* Loop iterations is always called before any new registers are created      now, so this should never occur.  */
if|if
condition|(
name|REGNO
argument_list|(
...
literal|0
condition|)
comment|/* iteration_info already printed a message.  */
return|return
literal|0
return|;
if|if
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Increment value can't be calculated.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Increment value not constant.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Initial value not constant.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
...
argument_list|)
expr_stmt|;
comment|/* Save the calculated values describing this loop's bounds, in case      precondition_loop_p will need them later.  These values can not be      recalculated inside precondition_loop_p because strength reduction      optimizations may obscure the loop's structure.  */
name|loop_iteration_var
operator|=
name|iteration_var
expr_stmt|;
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: EQ comparison loop.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Final value not constant.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
comment|/* ?? Final value and initial value do not have to be constants.      Only their difference has to be constant.  When the iteration variable      is an array address, the final value and initial value might both      be addresses with the same base but different constant offsets.      Final value must be invariant for this to work.       To do this, need some way to find the values of registers which are      invariant.  */
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Not normal loop.\n"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
comment|/* Calculate the number of iterations, final_value is only an approximation,      so correct for that.  Note that tempu and loop_n_iterations are      unsigned, because they can be as large as 2^n - 1.  */
name|i
operator|=
name|INTVAL
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* For NE tests, make sure that the iteration variable won't miss the      final value.  If tempu mod i is not zero, then the iteration variable      will overflow before the loop exits, and we can not calculate the      number of iterations.  */
if|if
condition|(
name|compare_dir
operator|==
