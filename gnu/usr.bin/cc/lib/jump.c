...
begin_comment
comment|/* Optimize jump instructions, for GNU compiler.    Copyright (C) 1987, 1988, 1989, 1991, 1992 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
directive|endif
comment|/* HAVE_cc0 */
comment|/* We deal with four cases:  	     1) x = a; if (...) x = b; and either A or B is zero, 	     2) if (...) x = 0; and jumps are expensive, 	     3) x = a; if (...) x = b; and A and B are constants where all the 	        set bits in A are also set in B and jumps are expensive, and 	     4) x = a; if (...) x = b; and A and B non-zero, and jumps are 	        more expensive. 	     5) if (...) x = b; if jumps are even more expensive.  	     In each of these try to use a store-flag insn to avoid the jump. 	     (If the jump would be faster, the machine should not have 	     defined the scc insns!).  These cases are often made by the 	     previous optimization.  	     INSN here is the jump around the store.  We set:  	     TEMP to the "x = b;" insn. 	     TEMP1 to X. 	     TEMP2 to B (const0_rtx in the second case). 	     TEMP3 to A (X in the second case). 	     TEMP4 to the condition being tested. 	     TEMP5 to the earliest insn used to find the condition.  */
if|if
condition|(
...
name|temp2
expr_stmt|;
comment|/* See if we can do this with a store-flag insn. */
name|start_sequence
argument_list|()
expr_stmt|;
...
name|range2before
decl_stmt|;
comment|/* Include in each range any line number before it.  */
while|while
condition|(
name|PREV_INSN
argument_list|(
...
condition|)
block|{
comment|/* Remove this CALL_INSN from the range we can cross-jump.  */
name|last1
operator|=
name|next_real_insn
argument_list|(
...
expr_stmt|;
block|}
comment|/* Skip past CLOBBERS since they may be right after a CALL_INSN.  It      isn't worth checking for the CALL_INSN.  */
while|while
condition|(
name|last1
operator|!=
...
argument_list|)
decl_stmt|;
comment|/* Don't put a label between a CALL_INSN and USE insns that precede 	 it.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* Don't put a label between a CALL_INSN and CLOBBER insns 	 following it. */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* Don't record labels that refer to dispatch tables. 		   This is not necessary, since the tablejump 		   references the same label. 		   And if we did record them, flow.c would make worse code.  */
if|if
condition|(
name|next
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Record in the note whether label is nonlocal.  */
name|LABEL_REF_NONLOCAL_P
argument_list|(
name|REG_NOTES
argument_list|(
