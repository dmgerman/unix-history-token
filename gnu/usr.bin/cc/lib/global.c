...
begin_comment
comment|/* Allocate registers for pseudo-registers that span basic blocks.    Copyright (C) 1987, 1988, 1991 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* All register that can be eliminated.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If we know we will definitely not be eliminating the frame pointer,      don't allocate it.  */
if|if
condition|(
operator|!
name|flag_omit_frame_pointer
...
directive|if
literal|0
block|for (link = REG_NOTES (insn); 		   link&& i< NUM_NO_CONFLICT_PAIRS; 		   link = XEXP (link, 1)) 		if (REG_NOTE_KIND (link) == REG_NO_CONFLICT) 		  { 		    no_conflict_pairs[i].allocno1 		      = reg_allocno[REGNO (SET_DEST (PATTERN (insn)))]; 		    no_conflict_pairs[i].allocno2 		      = reg_allocno[REGNO (XEXP (link, 0))]; 		    i++; 		  }
endif|#
directive|endif
comment|/* 0 */
...
condition|)
block|{
comment|/* Hard reg I was used less in total by local regs 	       than it would be used by this one allocno!  */
name|int
name|k
decl_stmt|;
for|for
