...
control|)
block|{
comment|/* Note that this loop must select the same block boundaries 	   as code in find_blocks. */
if|if
condition|(
name|INSN_UID
argument_list|(
...
control|)
block|{
comment|/* Note that this loop must select the same block boundaries 	 as code in reg_to_stack. */
name|code
operator|=
name|GET_CODE
argument_list|(
