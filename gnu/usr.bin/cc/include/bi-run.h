...
begin_comment
comment|/* Given OFFSET:SIZE for  a bitfield, calculate:     [1] BYTE_OFFSET  = the byte offset of the bit field.    [2] BIT_OFFSET   = the bit offset of the bit field (less than INTERP_BPC).    [3] NBYTES       = the number of integral bytes in the bit field.    [4] TRAILING_BITS= the number of trailing bits (less than INTERP_BPC).      ,        ,        ,        ,        ,    (memory bytes)                     ----------------        (bitfield)    |        |       ||        |    |        (divisions)         ^         ^       ^      ^         |         |       |      |__ [4]  (bits)         |         |       |_________ [3]  (bytes)         |         |_________________ [2]  (bits)         |___________________________ [1]  (bytes)      The above applies to BYTE_LOW_ENDIAN machines. In BYTE_BIG_ENDIAN machines, the    bit numbering is reversed (i.e. bit 0 is the sign bit).     (Alright, so I drew this to keep my tongue in cheek while writing the code below,     not because I'm into ASCII art.) */
end_comment
...
parameter_list|)
define|\
value|(DEST = ((DEST)<< (NBITS))					\    | (LM ((NBITS))						\& ((SOURCE)>> (INTERP_BPC - (OFFSET) - (NBITS)))))
end_define
...
parameter_list|)
define|\
value|(DEST = ((DEST)& ~(LM ((NBITS))<< (INTERP_BPC - (OFFSET) - (NBITS))))	\    | (((VALUE)& LM ((NBITS)))<< (INTERP_BPC - (OFFSET) - (NBITS))))
end_define
...
parameter_list|)
define|\
value|(DEST = ((DEST)<< (NBITS))					\    | (LM ((NBITS))						\& ((SOURCE)>> (OFFSET))))
end_define
...
parameter_list|)
define|\
value|(DEST = ((DEST)& ~(LM ((NBITS))<< (OFFSET)))		\    | (((VALUE)& LM ((NBITS)))<< (OFFSET)))
end_define
