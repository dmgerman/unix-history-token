...
argument_list|(
literal|"Welcome to Info version %s.  Type \"\\[get-help-window]\" for help."
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Set WINDOW to show NODE.  Remember the new window in our list of Info    windows.  If we are doing automatic footnote display, also try to display    the footnotes for this window. */
end_comment
...
control|)
empty_stmt|;
comment|/* If at a line start alreay, do nothing. */
if|if
condition|(
name|point
operator|!=
...
begin_comment
comment|/* Here is a list of time counter names which correspond to ordinal numbers.    It is used to print "once" instead of "1". */
end_comment
...
begin_comment
comment|/* Buffer used to return values from times_description (). */
end_comment
...
begin_comment
comment|/* Function returns a static string fully describing the number of times    present in COUNT. */
end_comment
...
begin_comment
comment|/* Declaration only.  Special cased in info_dispatch_on_key (). */
end_comment
...
directive|endif
comment|/* !FIONREAD */
comment|/* Store the input characters just read into our input buffer. */
block|{
specifier|register
name|int
name|i
