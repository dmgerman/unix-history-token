...
begin_comment
comment|/* This function looks up additions. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* looks like we found something. */
if|if
condition|(
name|tmp
operator|->
...
begin_comment
comment|/* This function lets us add files from outside the standard file tree.    It is useful if we want to duplicate a cd, but add/replace things.    We should note that the real path will be used for exclusions. */
end_comment
...
name|addpath
expr_stmt|;
comment|/* Now we must create the directory entry. */
comment|/* fortuneately only the name seems to matter. */
comment|/*   de->d_ino = -1;   de->d_off = 0;   de->d_reclen = strlen (name);   */
name|strncpy
argument_list|(
name|de
operator|->
...
name|stderr
argument_list|,
literal|"Already cached directory seen (%s)\n"
argument_list|,
name|whole_path
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/*      * Add directories to the cache so that we don't waste space even      * if we are supposed to be following symlinks.      */
if|if
condition|(
name|follow_links
operator|&&
...
name|stderr
argument_list|,
literal|"Directory loop - fatal goof (%s %lx %lu).\n"
argument_list|,
name|whole_path
argument_list|,
operator|(
