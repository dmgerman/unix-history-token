...
index|[]
init|=
literal|"$CVSid: @(#)rcs.c 1.40 94/10/07 $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * Add an RCSNode to a list of them.  */
end_comment
...
comment|/* Some ports of RCS to Windows NT write RCS files with newline-        delimited lines.  We would need to pass fopen a "binary" flag.  */
name|abort
argument_list|()
expr_stmt|;
endif|#
...
begin_comment
comment|/*  * Do the real work of parsing an RCS file   *  * There are no allowances for error here.  */
end_comment
...
directive|ifdef
name|DEATH_SUPPORT
comment|/* Accept this regardless of DEATH_STATE, so that we can read 	   repositories created with different versions of CVS.  */
if|if
condition|(
name|strcmp
argument_list|(
...
directive|ifdef
name|DEATH_SUPPORT
comment|/* Enable use of repositories created with a CVS which defines 	       DEATH_SUPPORT and not DEATH_STATE.  */
if|if
condition|(
name|strcmp
argument_list|(
...
begin_comment
comment|/*  * Given a list of RCSNodes, returns non-zero if the specified  * revision number or symbolic tag resolves to a "branch" within the  * rcs file.  */
end_comment
