...
index|[]
init|=
literal|"$CVSid: @(#)diff.c 1.61 94/10/22 $"
decl_stmt|;
end_decl_stmt
...
comment|/* Idea: often times the changed region of a file is relatively small.    It would be cool if the client could just divide the file into 4k    blocks or whatever and send hash values for the blocks.  Send hash    values for blocks aligned with the beginning of the file and the    end of the file.  Then the server can tell how much of the head and    tail of the file is unchanged.  Well, hash collisions will screw    things up, but MD5 has 128 bits of hash value...  */
comment|/* Send the current files unless diffing two revs from the archive */
block|if (diff_rev2 == NULL&& diff_date2 == NULL) 	    send_files (argc, argv, local); 	else 	    send_file_names (argc, argv);
else|#
directive|else
name|send_files
argument_list|(
