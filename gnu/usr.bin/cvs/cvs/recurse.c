...
begin_comment
comment|/*  * Called to start a recursive command.  *  * Command line arguments dictate the directories and files on which  * we operate.  In the special case of no arguments, we default to  * ".".  *  * The general algorythm is as follows.  */
end_comment
