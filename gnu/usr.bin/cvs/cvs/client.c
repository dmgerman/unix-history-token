...
begin_comment
comment|/* Things shared with update.c */
end_comment
...
ifdef|#
directive|ifdef
name|EXECUTE_PERMISSION_LOSES
name|KFF_DEBUG
argument_list|(
name|printf
argument_list|(
literal|"*** S_IXUSR in change_mode().\n"
argument_list|)
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* ! EXECUTE_PERMISSION_LOSES */
name|mode
operator||=
name|S_IXUSR
expr_stmt|;
...
name|printf
argument_list|(
literal|"*** S_IXGRP in change_mode().\n"
argument_list|)
argument_list|)
expr_stmt|;
else|#
...
name|printf
argument_list|(
literal|"*** S_IXOTH in change_mode().\n"
argument_list|)
argument_list|)
expr_stmt|;
else|#
...
begin_comment
comment|/*  * Read a line from the server.  *  * Space for the result is malloc'd and should be freed by the caller.  *  * Returns number of bytes read.  If EOF_OK, then return 0 on end of file,  * else end of file is an error.  */
end_comment
...
literal|0
argument_list|,
literal|"premature end of file from server"
argument_list|)
expr_stmt|;
block|}
if|if
...
argument_list|)
condition|)
comment|/* FIXME: Should delete temp_filename.  */
name|error
argument_list|(
literal|1
argument_list|,
...
block|{
comment|/* short reads are ok if we keep trying */
name|buf2
operator|+=
name|size_read
expr_stmt|;
...
name|errno
argument_list|,
literal|"waiting for gzip process %d"
argument_list|,
name|gzip_pid
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * Unlike some requests this doesn't include the repository.  So we can't  * just call call_in_directory and have the right thing happen; we save up  * the requests and do them at the end.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * In this case, both sides of the net connection will use the          * same fd. 	 */
if|if
condition|(
name|fileno
argument_list|(
...
directive|endif
comment|/* HAVE_KERBEROS || AUTH_CLIENT_SUPPORT */
ifdef|#
directive|ifdef
...
expr_stmt|;
block|}
comment|/* Else authorization granted, so we can go on... */
block|}
comment|/* This was stolen straight from start_kerberos_server(). */
block|{
name|server_fd
operator|=
name|sock
...
argument_list|)
expr_stmt|;
comment|/*      * If we do any filtering, TOFD and FROMFD will be      * closed.  So make sure they're copies of SERVER_FD,      * and not the same fd number.      */
if|if
condition|(
name|log
condition|)
...
name|errno
argument_list|,
literal|"waiting for gzip proc %d"
argument_list|,
name|gzip_pid
argument_list|)
expr_stmt|;
