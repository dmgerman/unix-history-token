...
condition|)
block|{
comment|/* under NEXTSTEP errno is set to return EPERM if 	 * the file is a directory,or if the user is not 	 * allowed to read or write to the file. 	 * [This is probably a bug in the O/S] 	 * other systems will return EISDIR to indicate 	 * that the path is a directory. 	 */
if|if
condition|(
name|errno
operator|==
...
comment|/* The file wasn't a directory and some other 		 * error occured 		 */
return|return
operator|-
literal|1
return|;
...
block|{
comment|/* buf isn't a directory, or there are 		     * some sort of permision problems 		     */
name|closedir
argument_list|(
name|dirp
argument_list|)
