...
argument_list|)
condition|)
comment|/* Get the size out of the stat block. 	   Allocate enough room for appended newline and sentinel. 	   Allocate at least one block, to prevent overrunning the buffer 	   when comparing growing binary files.  */
name|current
operator|->
name|bufsize
operator|=
...
begin_comment
comment|/* Given a vector of two file_data objects, read the file associated    with each one, and build the table of equivalence classes.    Return 1 if either file appears to be a binary file.  */
end_comment
