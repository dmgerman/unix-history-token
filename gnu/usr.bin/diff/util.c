...
argument_list|)
expr_stmt|;
comment|/* Posix.2 section 4.17.6.1.1 specifies this format.  But there are some      bugs in the first printing (IEEE Std 1003.2-1992 p 251 l 3304):      it says that we must print only the last component of the pathnames,      and it requires two spaces after "diff" if there are no options.      These requirements are silly and do not match historical practice.  */
name|sprintf
argument_list|(
name|name
argument_list|,
...
index|]
decl_stmt|;
comment|/* Fork a `pr' and make OUTFILE a pipe to it.  */
if|if
condition|(
name|pipe
argument_list|(
...
begin_comment
comment|/* Compare two lines (typically one from each input file)    according to the command line options.    Return 1 if the lines differ, like `memcmp'.  */
end_comment
...
literal|0
return|;
comment|/* Not exactly identical, but perhaps they match anyway      when case or white space is ignored.  */
if|if
condition|(
