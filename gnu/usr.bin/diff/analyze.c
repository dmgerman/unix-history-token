...
name|changes
decl_stmt|;
comment|/* If we have detected that either file is binary,      compare the two files as binary.  This can happen      only when the first chunk is read.      Also, -q means treat all files as binary.  */
if|if
condition|(
name|read_files
argument_list|(
...
expr_stmt|;
block|}
comment|/* Set CHANGES if we had any diffs that were printed. 	 If some changes are ignored, we must scan the script to decide.  */
if|if
condition|(
