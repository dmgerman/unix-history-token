...
name|buf_size
decl_stmt|;
comment|/* Make sure buffers can always hold 2 blocks and that they      are big enough to hold 1 tar record (512 bytes) even if it      is not aligned on a block boundary.  The extra buffer space      is needed by process_copyin and peek_in_buf to automatically      figure out what kind of archive it is reading.  */
if|if
condition|(
name|io_block_size
operator|>=
...
literal|0
expr_stmt|;
comment|/* Leave space for an `int' sentinel for `empty_output_buffer',      in case we ever put back sparseness checking.  */
name|output_buffer
operator|=
operator|(
name|char
