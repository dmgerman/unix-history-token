...
begin_comment
comment|/* permanent repository of field nodes */
end_comment
...
begin_comment
comment|/* Someone assigned a value to $(something).  Fix up $0 to be right */
end_comment
...
begin_comment
comment|/*  * setup $0, but defer parsing rest of line until reference is made to $(>0)  * or to NF.  At that point, parse only as much as necessary.  */
end_comment
...
begin_comment
comment|/*  * this is called both from get_field() and from do_split()  * via (*parse_field)().  This variation is for when FS is a regular  * expression -- either user-defined or because RS=="" and FS==" "  */
end_comment
...
begin_comment
comment|/*  * this is called both from get_field() and from do_split()  * via (*parse_field)().  This variation is for when FS is a single space  * character.  */
end_comment
...
begin_comment
comment|/*  * this is called both from get_field() and from do_split()  * via (*parse_field)().  This variation is for when FS is a single character  * other than space.  */
end_comment
...
begin_comment
comment|/*  * this is called both from get_field() and from do_split()  * via (*parse_field)().  This variation is for fields are fixed widths.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* fill in fields that don't exist */
for|for
control|(
name|i
operator|=
...
name|NULL
decl_stmt|;
comment|/* 	 * do dupnode(), to avoid problems like 	 *	x = split(a[1], a, "blah") 	 * since we assoc_clear the array. gack. 	 * this also gives up complete call by value semantics. 	 */
name|tmp
operator|=
name|tree_eval
argument_list|(
...
name|fatal
argument_list|(
literal|"second argument of split is not a variable"
argument_list|)
expr_stmt|;
name|assoc_clear
argument_list|(
