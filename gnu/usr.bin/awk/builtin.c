...
begin_comment
comment|/*  * On the alpha, LONG_MAX is too big for doing rand().  * On the Cray (Y-MP, anyway), ints and longs are 64 bits, but  * random() does things in terms of 32 bits. So we have to chop  * LONG_MAX down.  */
end_comment
...
literal|'0'
expr_stmt|;
comment|/* with a negative precision *cur is already set  */
comment|/* to -1, so it will remain negative, but we have */
comment|/* to "eat" precision digits in any case          */
while|while
condition|(
name|n0
operator|>
...
literal|0
condition|)
comment|/* ignore '0' flag if */
name|fill
operator|=
name|sp
expr_stmt|;
comment|/* precision given    */
if|if
condition|(
name|prec
operator|>
...
name|warning
argument_list|(
literal|"substr: length %d at position %d exceeds length of first argument"
argument_list|,
name|length
argument_list|,
name|indx
