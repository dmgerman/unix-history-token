...
begin_comment
comment|/*  *	$Id: ld.c,v 1.14 1993/12/02 05:09:52 nate Exp $  */
end_comment
...
name|fatal_with_file
argument_list|(
literal|"internal error: cannot read string table for "
argument_list|,
name|entry
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * Determine where the sections of ENTRY go into the output file,  * whose total section sizes are already known.  * Also relocate the addresses of the file's local and debugger symbols  * and determine which of the local symbols will make it into the  * output symbol table.  */
end_comment
...
break|break;
block|}
comment|/* 		 * See if this symbol should be in the output symbol table. 		 */
if|if
condition|(
name|type
operator|==
...
name|fatal_with_file
argument_list|(
literal|"Unsupported relocation type in "
argument_list|,
name|entry
argument_list|)
expr_stmt|;
...
directive|if
literal|0
block|entry->local_syms_offset = *syms_written_addr * sizeof(struct nlist);
endif|#
directive|endif
block|}
