...
argument|revId
argument_list|,
literal|"$Id: rcsrev.c,v 1.1.1.1 1993/06/18 04:22:13 jkh Exp $"
argument_list|)
end_macro
...
name|branchno
decl_stmt|;
comment|/* Given a non-nil revision number revno, getbranchno copies the number of the branch  * on which revno is into branchno. If revno itself is a branch number,  * it is copied unchanged.  */
block|{
specifier|register
name|unsigned
name|numflds
...
begin_comment
comment|/* Function: finds the deltas needed for reconstructing the  * revision given by revno, date, author, and state, and stores pointers  * to these deltas into a list whose starting address is given by store.  * The last delta (target delta) is returned.  * If the proper delta could not be found, nil is returned.  */
end_comment
...
begin_comment
comment|/* Function: given a branchpoint, a revision number, date, author, and state,  * genbranch finds the deltas necessary to reconstruct the given revision  * from the branch point on.  * Pointers to the found deltas are stored in a list beginning with store.  * revno must be on a side branch.  * return nil on error  */
end_comment
