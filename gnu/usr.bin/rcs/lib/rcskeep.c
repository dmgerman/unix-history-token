...
argument|keepId
argument_list|,
literal|"$Id: rcskeep.c,v 1.1.1.1 1993/06/18 04:22:13 jkh Exp $"
argument_list|)
end_macro
...
name|fp
decl_stmt|;
comment|/* Function: Tries to read keyword values for author, date,  * revision number, and state out of the file fp.  * If FNAME is nonnull, it is opened and closed instead of using FP.  * The results are placed into  * prevauthor, prevdate, prevrev, prevstate.  * Aborts immediately if it finds an error and returns false.  * If it returns true, it doesn't mean that any of the  * values were found; instead, check to see whether the corresponding arrays  * contain the empty string.  */
block|{
specifier|register
name|int
name|c
...
name|optional
decl_stmt|;
comment|/* Reads a keyword value from FP into TARGET.  * Returns true if one is found, false otherwise.  * Does not modify target if it is nil.  * Do not report an error if OPTIONAL is set and KDELIM is found instead.  */
block|{
name|int
name|c
decl_stmt|;
...
name|error
argument_list|(
literal|"too much white space in keyword value"
argument_list|)
expr_stmt|;
return|return
name|got1
