...
begin_comment
comment|/* linker.c -- BFD linker routines    Copyright (C) 1993, 94 Free Software Foundation, Inc.    Written by Steve Chamberlain and Ian Lance Taylor, Cygnus Support  This file is part of BFD, the Binary File Descriptor library.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|bfd_hash_entry
modifier|*
name|generic_link_hash_newfunc
name|PARAMS
argument_list|(
operator|(
expr|struct
...
condition|)
continue|continue;
comment|/* We are only interested if we know something about this 	 symbol, and it is undefined or common.  An undefined weak 	 symbol (type bfd_link_hash_weak) is not considered to be a 	 reference when pulling files out of an archive.  See the SVR4 	 ABI, p. 4-27.  */
name|h
operator|=
name|bfd_link_hash_lookup
argument_list|(
...
condition|)
block|{
comment|/* The name of P is actually the warning string, and the 		 value is actually a pointer to the symbol to warn 		 about.  */
name|string
operator|=
name|name
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* Store a back pointer from the symbol to the hash 		 table entry for the benefit of relaxation code until 		 it gets rewritten to not use asymbol structures.  */
name|p
operator|->
name|udata
operator|=
...
init|=
block|{
comment|/* current\prev    new    undef  weak   def    com    indr   warn  */
comment|/* UNDEF_ROW 	*/
block|{
name|UND
block|,
...
name|CREF
case|:
comment|/* We have found a common definition for a symbol which was 	     already defined.  */
name|BFD_ASSERT
argument_list|(
name|h
operator|->
...
name|CWARN
case|:
comment|/* Warn if this symbol has been referenced already, 	     otherwise either add a warning or cycle.  A symbol has 	     been referenced if the next field is not NULL, or it is 	     the tail of the undefined symbol list.  The REF case 	     above helps to ensure this.  */
if|if
condition|(
name|h
operator|->
...
expr_stmt|;
block|}
comment|/* We do not set the section of the symbol to 		     h->root.u.c.section.  That value was saved so 		     that we would know where to allocate the symbol 		     if it was defined.  In this case the type is 		     still bfd_link_hash_common, so we did not define 		     it, so we do not want to use that section.  */
break|break;
block|}
block|}
block|}
...
expr_stmt|;
block|}
comment|/* Get the canonical symbols.  The generic linker will always have      retrieved them by this point, but we may be being called by a      specific linker when linking different types of object files      together.  */
if|if
condition|(
operator|!
name|generic_link_read_symbols
