...
decl_stmt|;
block|{
comment|/* For the .pdata section, which has a special meaning on the Alpha,      we set the alignment to 8.  We correct this later in      ecoff_compute_section_file_positions.  We do this hackery because      we need to know the exact unaligned size of the .pdata section in      order to set the lnnoptr field correctly.  */
if|if
condition|(
name|strcmp
argument_list|(
...
name|local
decl_stmt|;
comment|/* This function is selected based on the input vector.  We only      want to copy information over if the output BFD also uses ECOFF      format.  */
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* If this is a .lib section, bump the vma address so that it winds      up being the number of .lib sections output.  This is right for      Irix 4.  Ian Taylor<ian@cygnus.com>.  */
if|if
condition|(
name|strcmp
argument_list|(
...
name|data
decl_stmt|;
comment|/* FIXME: We should check if this symbol is being stripped.  */
if|if
condition|(
name|h
operator|->
...
block|{
comment|/* We can't set up a reloc against a symbol correctly, because 	 we have no asymbol structure.  Currently no adjust_reloc_out 	 routine cases.  */
name|rel
operator|.
name|sym_ptr_ptr
operator|=
