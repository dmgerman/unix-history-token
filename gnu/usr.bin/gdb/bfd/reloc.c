...
begin_comment
comment|/* FUNCTION 	bfd_get_reloc_size  SYNOPSIS 	int bfd_get_reloc_size (const reloc_howto_type *);  DESCRIPTION 	For a reloc_howto_type that operates on a fixed number of bytes, 	this returns the number of bytes operated on.  */
end_comment
...
begin_comment
comment|/* INTERNAL_FUNCTION 	bfd_default_reloc_type_lookup  SYNOPSIS 	const struct reloc_howto_struct *bfd_default_reloc_type_lookup 	(bfd *abfd, bfd_reloc_code_real_type  code);  DESCRIPTION 	Provides a default relocation lookup routine for any architecture.   */
end_comment
