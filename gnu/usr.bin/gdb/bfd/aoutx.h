...
name|EXTERNAL_NLIST_SIZE
expr_stmt|;
comment|/* We allocate using malloc to make the values easy to free 	 later on.  If we put them on the obstack it might not be 	 possible to free them.  */
name|syms
operator|=
operator|(
operator|(
...
condition|)
block|{
comment|/* Protect the bfd_is_com_section call.  This case occurs, e.g., 	 for the *DEBUG* section of a COFF file.  */
name|bfd_set_error
argument_list|(
...
begin_comment
comment|/* We use a hash table when writing out symbols so that we only write    out a particular string once.  This helps particularly when the    linker writes out stabs debugging entries, because each different    contributing object file tends to have many duplicate stabs    strings.     Possible improvements:    + look for strings matching trailing substrings of other strings    + better data structures?  balanced trees?    + look at reducing memory use elsewhere -- maybe if we didn't have      to construct the entire symbol table at once, we could get by      with smaller amounts of VM?  (What effect does that have on the      string table reductions?)     This hash table code breaks dbx on SunOS 4.1.3, so we don't do it    if BFD_TRADITIONAL_FORMAT is set.  */
end_comment
...
name|false
return|;
comment|/* Leave space for the size of the string table.  */
name|table
operator|->
name|size
operator|=
...
name|tab
parameter_list|)
value|bfd_hash_table_free (&(tab)->table)
end_define
...
return|;
block|}
comment|/* We keep around the symbols even if we aren't going to use this      object file, because we may want to reread it.  This doesn't      waste too much memory, because it isn't all that common to read      an archive element but not need it.  */
if|if
condition|(
operator|!
name|info
...
name|drsize
decl_stmt|;
comment|/* Count up the relocation sizes.  */
name|trsize
operator|=
literal|0
expr_stmt|;
...
return|;
block|}
comment|/* Write out the symbols and get a map of the new indices.  */
if|if
condition|(
operator|!
name|aout_link_write_symbols
...
name|error_return
goto|;
comment|/* Relocate and write out the sections.  */
if|if
condition|(
operator|!
name|aout_link_input_section
...
name|output_section
decl_stmt|;
comment|/* This case means a common symbol which was turned 		     into a defined symbol.  */
name|input_section
operator|=
name|hresolve
operator|->
...
block|}
comment|/* As described above, we must always adjust a PC relative 	     reloc by the change in VMA of the source.  */
if|if
condition|(
name|howto_table_ext
index|[
