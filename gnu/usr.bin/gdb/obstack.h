...
parameter_list|)
define|\
value|_obstack_begin ((h), 0, 0, obstack_chunk_alloc, obstack_chunk_free)
end_define
...
parameter_list|)
define|\
value|_obstack_begin ((h), (size), 0, obstack_chunk_alloc, obstack_chunk_free)
end_define
...
parameter_list|)
define|\
value|({ struct obstack *__o = (OBSTACK);					\    int __len = (length);						\    ((__o->next_free + __len> __o->chunk_limit)				\     ? _obstack_newchunk (__o, __len) : 0);				\    bcopy (where, __o->next_free, __len);				\    __o->next_free += __len;						\    (void) 0; })
end_define
...
parameter_list|)
define|\
value|({ struct obstack *__o = (OBSTACK);					\    int __len = (length);						\    ((__o->next_free + __len + 1> __o->chunk_limit)			\     ? _obstack_newchunk (__o, __len + 1) : 0),				\    bcopy (where, __o->next_free, __len),				\    __o->next_free += __len,						\    *(__o->next_free)++ = 0;						\    (void) 0; })
end_define
...
parameter_list|)
define|\
value|({ struct obstack *__o = (OBSTACK);					\    ((__o->next_free + 1> __o->chunk_limit)				\     ? _obstack_newchunk (__o, 1) : 0),					\    *(__o->next_free)++ = (datum);					\    (void) 0; })
end_define
...
parameter_list|)
define|\
value|({ struct obstack *__o = (OBSTACK);					\    int __len = (length);						\    ((__o->next_free + __len> __o->chunk_limit)				\     ? _obstack_newchunk (__o, __len) : 0);				\    __o->next_free += __len;						\    (void) 0; })
end_define
...
begin_comment
comment|/* The non-GNU macros copy the obstack-pointer into this global variable    to avoid multiple evaluation.  */
end_comment
...
parameter_list|)
define|\
value|(unsigned) (_obstack = (h), (h)->next_free - (h)->object_base)
end_define
...
parameter_list|)
define|\
value|(unsigned) (_obstack = (h), (h)->chunk_limit - (h)->next_free)
end_define
...
parameter_list|)
define|\
value|( (h)->temp = (length),							\   (((h)->next_free + (h)->temp> (h)->chunk_limit)			\    ? _obstack_newchunk ((h), (h)->temp) : 0),				\   bcopy (where, (h)->next_free, (h)->temp),				\   (h)->next_free += (h)->temp)
end_define
...
parameter_list|)
define|\
value|( (h)->temp = (length),							\   (((h)->next_free + (h)->temp + 1> (h)->chunk_limit)			\    ? _obstack_newchunk ((h), (h)->temp + 1) : 0),				\   bcopy (where, (h)->next_free, (h)->temp),				\   (h)->next_free += (h)->temp,						\   *((h)->next_free)++ = 0)
end_define
...
parameter_list|)
define|\
value|( (((h)->next_free + 1> (h)->chunk_limit)				\    ? _obstack_newchunk ((h), 1) : 0),					\   *((h)->next_free)++ = (datum))
end_define
...
parameter_list|)
define|\
value|( (h)->temp = (length),							\   (((h)->next_free + (h)->temp> (h)->chunk_limit)			\    ? _obstack_newchunk ((h), (h)->temp) : 0),				\   (h)->next_free += (h)->temp)
end_define
...
parameter_list|)
define|\
value|( (h)->temp = (char *)(obj) - (char *) (h)->chunk,			\   (((h)->temp>= 0&& (h)->temp< (h)->chunk_limit - (char *) (h)->chunk)\    ? (int) ((h)->next_free = (h)->object_base				\ 	    = (h)->temp + (char *) (h)->chunk)				\    : ((obstack_free) ((h), (h)->temp + (char *) (h)->chunk), 0)))
end_define
...
parameter_list|)
define|\
value|( (h)->temp = (char *)(obj) - (char *) (h)->chunk,			\   (((h)->temp>= 0&& (h)->temp< (h)->chunk_limit - (char *) (h)->chunk)\    ? (int) ((h)->next_free = (h)->object_base				\ 	    = (h)->temp + (char *) (h)->chunk)				\    : (int) _obstack_free ((h), (h)->temp + (char *) (h)->chunk)))
end_define
