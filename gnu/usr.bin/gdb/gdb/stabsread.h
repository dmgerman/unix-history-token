...
begin_comment
comment|/* Hash table of global symbols whose values are not known yet.    They are chained thru the SYMBOL_VALUE_CHAIN, since we don't    have the correct data for that slot yet.     The use of the LOC_BLOCK code in this chain is nonstandard--    it refers to a FORTRAN common block rather than the usual meaning.  */
end_comment
...
begin_comment
comment|/* Functions exported by dbxread.c.  These are not in stabsread.h because    they are only used by some stabs readers.  */
end_comment
