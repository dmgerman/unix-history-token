...
parameter_list|)
define|\
value|{ COERCE_REF(arg);							\   if (VALUE_REPEATED (arg)						\       || TYPE_CODE (VALUE_TYPE (arg)) == TYPE_CODE_ARRAY)		\     arg = value_coerce_array (arg);					\   if (TYPE_CODE (VALUE_TYPE (arg)) == TYPE_CODE_FUNC)                   \     arg = value_coerce_function (arg);                                  \   if (TYPE_CODE (VALUE_TYPE (arg)) == TYPE_CODE_ENUM)			\     arg = value_cast (builtin_type_unsigned_int, arg);			\ }
end_define
...
parameter_list|)
define|\
value|{ COERCE_REF (arg); \   if (TYPE_CODE (VALUE_TYPE (arg)) == TYPE_CODE_ENUM)			\     arg = value_cast (builtin_type_unsigned_int, arg);			\ }
end_define
...
begin_comment
comment|/* FIXME:  Assumes equivalence of "struct frame_info *" and "FRAME" */
end_comment
