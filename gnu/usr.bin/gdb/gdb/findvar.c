...
begin_comment
comment|/* Swap LEN bytes at BUFFER between target and host byte-order.  This is    the wrong way to do byte-swapping because it assumes that you have a way    to have a host variable of exactly the right size.  Once extract_floating    and store_floating have been fixed, this can go away.  */
end_comment
