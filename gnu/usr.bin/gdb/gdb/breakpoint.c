...
name|func_start
decl_stmt|;
comment|/* If caller_pc_valid is true, we are stepping through      a function prologue, which is bounded by callee_func_start      (inclusive) and callee_prologue_end (exclusive).      caller_pc is the pc of the caller.       Yes, this is hairy.  */
specifier|static
name|int
name|caller_pc_valid
init|=
literal|0
decl_stmt|;
specifier|static
...
specifier|static
name|CORE_ADDR
name|callee_func_start
decl_stmt|;
specifier|static
name|CORE_ADDR
name|callee_prologue_end
decl_stmt|;
...
condition|)
block|{
comment|/* We just called a function.  The only other case I 	 can think of where the pc would equal the pc of the 	 start of a function is a frameless function (i.e. 	 no prologue) where we branch back to the start 	 of the function.  In that case, SKIP_PROLOGUE won't 	 find one, and we'll clear caller_pc_valid a few lines 	 down.  */
name|caller_pc_valid
operator|=
literal|1
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* If any active frame is in the exp_valid_block, then it's      OK.  Note that this might not be the same invocation of      the exp_valid_block that we were watching a little while      ago, or the same one as when the watchpoint was set (e.g.      we are watching a local variable in a recursive function.      When we return from a recursive invocation, then we are      suddenly watching a different instance of the variable).       At least for now I am going to consider this a feature.  */
for|for
control|(
init|;
name|fr
...
begin_comment
comment|/* The watchpoint has been disabled.  */
end_comment
...
begin_comment
comment|/* Get a bpstat associated with having just stopped at address *PC    and frame address FRAME_ADDRESS.  Update *PC to point at the    breakpoint (if we hit a breakpoint).  NOT_A_BREAKPOINT is nonzero    if this is known to not be a real breakpoint (it could still be a    watchpoint, though).  */
end_comment
...
block|{
comment|/*                              old action */
comment|/*       keep_c  stop_s  stop_n  single  setlr   clrlr   clrlrs */
comment|/*no_effect*/
block|{
name|keep_c
block|,
...
directive|if
literal|0
comment|/* Need to temporarily disable this until we can fix the bug 	     with nexting over a breakpoint with ->stop clear causing 	     an infinite loop.  For now, treat the breakpoint as having 	     been hit even if the frame is wrong.  */
block|if (bs->stop) 	    {
endif|#
directive|endif
name|retval
...
literal|1
expr_stmt|;
comment|/* We don't handle this via the main_action.  */
name|bs_class
operator|=
name|no_effect
expr_stmt|;
...
begin_comment
comment|/* But for now, just kludge it based on the concept that before an    objfile is relocated the breakpoint is below 0x10000000, and afterwards    it is higher, so that way we only relocate each breakpoint once.  */
end_comment
