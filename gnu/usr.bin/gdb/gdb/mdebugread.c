...
begin_comment
comment|/* Each partial symbol table entry contains a pointer to private data    for the read_symtab() function to use when expanding a partial    symbol table entry to a full symbol table entry.     For mdebugread this structure contains the index of the FDR that this    psymtab represents and a pointer to the BFD that the psymtab was    created from.  */
end_comment
...
begin_comment
comment|/* Types corresponding to btComplex, btDComplex, etc.  These are here    rather than in gdbtypes.c or some such, because the meaning of codes    like btComplex is specific to the mdebug debug format.  FIXME:  We should    be using our own types thoughout this file, instead of sometimes using    builtin_type_*.  */
end_comment
...
comment|/* DEC c89 has the number of enumerators in the sh.value field, 	       not the type length, so we have to compensate for that 	       incompatibility quirk. 	       This might do the wrong thing for an enum with one or two 	       enumerators and gcc -gcoff -fshort-enums, but these cases 	       are hopefully rare enough.  */
if|if
condition|(
name|TYPE_LENGTH
argument_list|(
...
parameter_list|()
define|\
value|namestring = debug_info->ss + fh->issBase + sh.iss
define|#
directive|define
...
name|stBlock
case|:
comment|/* It is an enumerated type if the next symbol entry is a stMember 	 and its auxiliary index is indexNil or its auxiliary entry 	 is a plain btNil or btVoid.  */
call|(
modifier|*
name|swap_sym_in
call|)
...
begin_comment
comment|/* FIXME: This function is called only by mips-tdep.c.  It needs to be    here because it calls functions defined in this file, but perhaps    this could be handled in a better way.  */
end_comment
