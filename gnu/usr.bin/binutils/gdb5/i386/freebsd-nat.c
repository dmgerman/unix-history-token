...
operator|)
decl_stmt|;
comment|/* XXX extract pid from tid */
name|ptrace
argument_list|(
name|PT_GETFPREGS
argument_list|,
...
begin_comment
comment|/*  * 0: no trace output  * 1: trace watchpoint requests  * 2: trace `watchpoint hit?' tests, too  */
end_comment
...
argument_list|(
literal|"can_watch(%d, %d, %d) = %d (counts: w: %d, rw: %d)\n"
argument_list|,
name|type
argument_list|,
name|cnt
...
name|perror
argument_list|(
literal|"ptrace(PT_GETDBREGS) failed"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|printf_filtered
argument_list|(
literal|"stopped_by_watchpoint(): DR6 = %#x\n"
argument_list|,
name|dbr
operator|.
name|dr6
...
directive|endif
comment|/*        * If a hardware watchpoint was hit, one of the lower 4 bits in        * DR6 is set (the actual bit indicates which of DR0...DR3 triggered        * the trap).        */
return|return
name|dbr
operator|.
name|dr6
...
name|warning
argument_list|(
literal|"Can't set a watchpoint on a core file."
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|warning
argument_list|(
literal|"no more hardware watchpoints available"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|warning
argument_list|(
literal|"watchpoint length %d unsupported, using lenght = 4"
argument_list|,
name|len
argument_list|)
expr_stmt|;
...
name|warning
argument_list|(
literal|"weird watchpoint length 3, using 2"
argument_list|)
expr_stmt|;
name|len
operator|=
...
name|warning
argument_list|(
literal|"weird watchpoint length 0, using 1"
argument_list|)
expr_stmt|;
name|len
operator|=
...
break|break;
block|}
comment|/*        *  The `type' value is 0 for `watch on write', 1 for `watch on        * read', 2 for `watch on both'.  The i386 debug register        * breakpoint types are 0 for `execute' (not used in GDB), 1 for        * `write', and 4 for `read/write'.  Plain `read' trapping is        * not supported on i386, value 3 is illegal.        */
switch|switch
condition|(
name|type
condition|)
...
name|warning
argument_list|(
literal|"weird watchpoint type %d, using a write watchpoint"
argument_list|)
expr_stmt|;
comment|/* FALLTHROUGH */
case|case
...
name|printf_filtered
argument_list|(
literal|"insert_watchpoint(), inserting DR7 = %#x, DR%d = %#x\n"
argument_list|,
name|dbr
operator|.
name|dr7
...
name|perror
argument_list|(
literal|"ptrace(PT_SETDBREGS) failed"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|warning
argument_list|(
literal|"watchpoint for address %#x not found"
argument_list|,
name|addr
argument_list|)
expr_stmt|;
...
name|printf_filtered
argument_list|(
literal|"remove_watchpoint(): removing watchpoint for %#x, DR7 = %#x\n"
argument_list|,
name|addr
argument_list|,
name|dbr
