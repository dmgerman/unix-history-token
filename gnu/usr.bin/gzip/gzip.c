...
begin_comment
comment|/* Compress files with zip algorithm and 'compress' interface.  * See usage() and help() functions below for all options.  * Outputs:  *        file.gz:   compressed file with same mode, owner, and utimes  *     or stdout with -c option or if stdin used as input.  * If the OS does not support file names with multiple dots (MSDOS, VMS) or  * if the output file name had to be truncated, the original name is kept  * in the compressed file.  * On MSDOS, file.tmp -> file.tmz. On VMS, file.tmp -> file.tmp-gz.  *  * For the meaning of all compilation flags, see comments in Makefile.in.  */
end_comment
...
index|[]
init|=
literal|"$Id: gzip.c,v 0.19 1993/06/01 14:21:46 jloup Exp $"
decl_stmt|;
end_decl_stmt
...
comment|/* {"list",       0, 0, 'l'},    list .gz file contents */
block|{
literal|"license"
block|,
literal|0
...
name|stderr
argument_list|,
literal|"usage: %s [-%scdfhL%stvV19] [-S suffix] [file ...]\n"
argument_list|,
name|progname
argument_list|,
if|#
...
comment|/* -l --list        list .gz file contents */
...
directive|ifdef
name|MAX_EXT_CHARS
literal|" -S --suffix .gz  use suffix .gz instead of .z"
block|,
else|#
directive|else
literal|" -S --suffix .z   use suffix .z instead of .gz"
block|,
endif|#
directive|endif
...
name|argv
argument_list|,
literal|"ab:cdfhLqrS:tvVZ123456789"
argument_list|,
name|longopts
argument_list|,
operator|(
...
begin_comment
comment|/* ========================================================================  * Create the output file. Return OK or ERROR.  * Try several times if necessary to avoid truncating the z_suffix. For  * example, do not create a compressed file of name "1234567890123."  * IN assertions: the input file has already been open (ifd is set) and  *   ofname has already been updated if there was an original name.  * OUT assertions: ifd and ofd are closed in case of error.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 123456789012.gz -> 12345678901.gz */
endif|#
directive|endif
block|}
comment|/* decompress ? */
block|}
comment|/* while non null name */
name|close
argument_list|(
name|ifd
argument_list|)
...
return|;
block|}
comment|/* file.ext doesn't exist, try adding a suffix.      */
name|s
operator|=
name|get_suffix
argument_list|(
...
begin_comment
comment|/* ========================================================================  * Generate ofname given ifname. Return OK, or WARNING if file must be skipped.  * Initializes save_orig_name.  * IN assertion: this function is not called if to_stdout is true.  */
end_comment
