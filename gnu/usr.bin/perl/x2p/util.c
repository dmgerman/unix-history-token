begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/* $RCSfile: util.c,v $$Revision: 1.1.1.1 $$Date: 1993/08/23 21:30:10 $  *  *    Copyright (c) 1991, Larry Wall  *  *    You may distribute under the terms of either the GNU General Public  *    License or the Artistic License, as specified in the README file.  *  * $Log: util.c,v $  * Revision 1.1.1.1  1993/08/23  21:30:10  nate  * PERL!  *  * Revision 4.0.1.1  91/06/07  12:20:35  lwall  * patch4: new copyright notice  *   * Revision 4.0  91/03/20  01:58:25  lwall  * 4.0 baseline.  *   */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|"handy.h"
end_include

begin_include
include|#
directive|include
file|"EXTERN.h"
end_include

begin_include
include|#
directive|include
file|"a2p.h"
end_include

begin_include
include|#
directive|include
file|"INTERN.h"
end_include

begin_include
include|#
directive|include
file|"util.h"
end_include

begin_define
define|#
directive|define
name|FLUSH
end_define

begin_define
define|#
directive|define
name|MEM_SIZE
value|unsigned int
end_define

begin_decl_stmt
specifier|static
name|char
name|nomem
index|[]
init|=
literal|"Out of memory!\n"
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* paranoid version of malloc */
end_comment

begin_decl_stmt
specifier|static
name|int
name|an
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_function
name|char
modifier|*
name|safemalloc
parameter_list|(
name|size
parameter_list|)
name|MEM_SIZE
name|size
decl_stmt|;
block|{
name|char
modifier|*
name|ptr
decl_stmt|;
name|char
modifier|*
name|malloc
parameter_list|()
function_decl|;
name|ptr
operator|=
name|malloc
argument_list|(
name|size
condition|?
name|size
else|:
literal|1
argument_list|)
expr_stmt|;
comment|/* malloc(0) is NASTY on our system */
ifdef|#
directive|ifdef
name|DEBUGGING
if|if
condition|(
name|debug
operator|&
literal|128
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"0x%x: (%05d) malloc %d bytes\n"
argument_list|,
name|ptr
argument_list|,
name|an
operator|++
argument_list|,
name|size
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|ptr
operator|!=
name|Nullch
condition|)
return|return
name|ptr
return|;
else|else
block|{
name|fputs
argument_list|(
argument|nomem
argument_list|,
argument|stdout
argument_list|)
name|FLUSH
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/*NOTREACHED*/
block|}
end_function

begin_comment
comment|/* paranoid version of realloc */
end_comment

begin_function
name|char
modifier|*
name|saferealloc
parameter_list|(
name|where
parameter_list|,
name|size
parameter_list|)
name|char
modifier|*
name|where
decl_stmt|;
name|MEM_SIZE
name|size
decl_stmt|;
block|{
name|char
modifier|*
name|ptr
decl_stmt|;
name|char
modifier|*
name|realloc
parameter_list|()
function_decl|;
name|ptr
operator|=
name|realloc
argument_list|(
name|where
argument_list|,
name|size
condition|?
name|size
else|:
literal|1
argument_list|)
expr_stmt|;
comment|/* realloc(0) is NASTY on our system */
ifdef|#
directive|ifdef
name|DEBUGGING
if|if
condition|(
name|debug
operator|&
literal|128
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"0x%x: (%05d) rfree\n"
argument_list|,
name|where
argument_list|,
name|an
operator|++
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"0x%x: (%05d) realloc %d bytes\n"
argument_list|,
name|ptr
argument_list|,
name|an
operator|++
argument_list|,
name|size
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
if|if
condition|(
name|ptr
operator|!=
name|Nullch
condition|)
return|return
name|ptr
return|;
else|else
block|{
name|fputs
argument_list|(
argument|nomem
argument_list|,
argument|stdout
argument_list|)
name|FLUSH
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/*NOTREACHED*/
block|}
end_function

begin_comment
comment|/* safe version of free */
end_comment

begin_macro
name|safefree
argument_list|(
argument|where
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|where
decl_stmt|;
end_decl_stmt

begin_block
block|{
ifdef|#
directive|ifdef
name|DEBUGGING
if|if
condition|(
name|debug
operator|&
literal|128
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"0x%x: (%05d) free\n"
argument_list|,
name|where
argument_list|,
name|an
operator|++
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|free
argument_list|(
name|where
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* safe version of string copy */
end_comment

begin_function
name|char
modifier|*
name|safecpy
parameter_list|(
name|to
parameter_list|,
name|from
parameter_list|,
name|len
parameter_list|)
name|char
modifier|*
name|to
decl_stmt|;
specifier|register
name|char
modifier|*
name|from
decl_stmt|;
specifier|register
name|int
name|len
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|dest
init|=
name|to
decl_stmt|;
if|if
condition|(
name|from
operator|!=
name|Nullch
condition|)
for|for
control|(
name|len
operator|--
init|;
name|len
operator|&&
operator|(
operator|*
name|dest
operator|++
operator|=
operator|*
name|from
operator|++
operator|)
condition|;
name|len
operator|--
control|)
empty_stmt|;
operator|*
name|dest
operator|=
literal|'\0'
expr_stmt|;
return|return
name|to
return|;
block|}
end_function

begin_comment
comment|/* copy a string up to some (non-backslashed) delimiter, if any */
end_comment

begin_function
name|char
modifier|*
name|cpytill
parameter_list|(
name|to
parameter_list|,
name|from
parameter_list|,
name|delim
parameter_list|)
specifier|register
name|char
modifier|*
name|to
decl_stmt|,
decl|*
name|from
decl_stmt|;
end_function

begin_decl_stmt
specifier|register
name|int
name|delim
decl_stmt|;
end_decl_stmt

begin_block
block|{
for|for
control|(
init|;
operator|*
name|from
condition|;
name|from
operator|++
operator|,
name|to
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|from
operator|==
literal|'\\'
condition|)
block|{
if|if
condition|(
name|from
index|[
literal|1
index|]
operator|==
name|delim
condition|)
name|from
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|from
index|[
literal|1
index|]
operator|==
literal|'\\'
condition|)
operator|*
name|to
operator|++
operator|=
operator|*
name|from
operator|++
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|from
operator|==
name|delim
condition|)
break|break;
operator|*
name|to
operator|=
operator|*
name|from
expr_stmt|;
block|}
operator|*
name|to
operator|=
literal|'\0'
expr_stmt|;
return|return
name|from
return|;
block|}
end_block

begin_function
name|char
modifier|*
name|cpy2
parameter_list|(
name|to
parameter_list|,
name|from
parameter_list|,
name|delim
parameter_list|)
specifier|register
name|char
modifier|*
name|to
decl_stmt|,
decl|*
name|from
decl_stmt|;
end_function

begin_decl_stmt
specifier|register
name|int
name|delim
decl_stmt|;
end_decl_stmt

begin_block
block|{
for|for
control|(
init|;
operator|*
name|from
condition|;
name|from
operator|++
operator|,
name|to
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|from
operator|==
literal|'\\'
condition|)
operator|*
name|to
operator|++
operator|=
operator|*
name|from
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
operator|*
name|from
operator|==
literal|'$'
condition|)
operator|*
name|to
operator|++
operator|=
literal|'\\'
expr_stmt|;
elseif|else
if|if
condition|(
operator|*
name|from
operator|==
name|delim
condition|)
break|break;
operator|*
name|to
operator|=
operator|*
name|from
expr_stmt|;
block|}
operator|*
name|to
operator|=
literal|'\0'
expr_stmt|;
return|return
name|from
return|;
block|}
end_block

begin_comment
comment|/* return ptr to little string in big string, NULL if not found */
end_comment

begin_function
name|char
modifier|*
name|instr
parameter_list|(
name|big
parameter_list|,
name|little
parameter_list|)
name|char
modifier|*
name|big
decl_stmt|,
decl|*
name|little
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|char
modifier|*
name|t
decl_stmt|,
modifier|*
name|s
decl_stmt|,
modifier|*
name|x
decl_stmt|;
for|for
control|(
name|t
operator|=
name|big
init|;
operator|*
name|t
condition|;
name|t
operator|++
control|)
block|{
for|for
control|(
name|x
operator|=
name|t
operator|,
name|s
operator|=
name|little
init|;
operator|*
name|s
condition|;
name|x
operator|++
operator|,
name|s
operator|++
control|)
block|{
if|if
condition|(
operator|!
operator|*
name|x
condition|)
return|return
name|Nullch
return|;
if|if
condition|(
operator|*
name|s
operator|!=
operator|*
name|x
condition|)
break|break;
block|}
if|if
condition|(
operator|!
operator|*
name|s
condition|)
return|return
name|t
return|;
block|}
return|return
name|Nullch
return|;
block|}
end_block

begin_comment
comment|/* copy a string to a safe spot */
end_comment

begin_function
name|char
modifier|*
name|savestr
parameter_list|(
name|str
parameter_list|)
name|char
modifier|*
name|str
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|newaddr
init|=
name|safemalloc
argument_list|(
call|(
name|MEM_SIZE
call|)
argument_list|(
name|strlen
argument_list|(
name|str
argument_list|)
operator|+
literal|1
argument_list|)
argument_list|)
decl_stmt|;
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|newaddr
argument_list|,
name|str
argument_list|)
expr_stmt|;
return|return
name|newaddr
return|;
block|}
end_function

begin_comment
comment|/* grow a static string to at least a certain length */
end_comment

begin_function
name|void
name|growstr
parameter_list|(
name|strptr
parameter_list|,
name|curlen
parameter_list|,
name|newlen
parameter_list|)
name|char
modifier|*
modifier|*
name|strptr
decl_stmt|;
name|int
modifier|*
name|curlen
decl_stmt|;
name|int
name|newlen
decl_stmt|;
block|{
if|if
condition|(
name|newlen
operator|>
operator|*
name|curlen
condition|)
block|{
comment|/* need more room? */
if|if
condition|(
operator|*
name|curlen
condition|)
operator|*
name|strptr
operator|=
name|saferealloc
argument_list|(
operator|*
name|strptr
argument_list|,
operator|(
name|MEM_SIZE
operator|)
name|newlen
argument_list|)
expr_stmt|;
else|else
operator|*
name|strptr
operator|=
name|safemalloc
argument_list|(
operator|(
name|MEM_SIZE
operator|)
name|newlen
argument_list|)
expr_stmt|;
operator|*
name|curlen
operator|=
name|newlen
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*VARARGS1*/
end_comment

begin_macro
name|fatal
argument_list|(
argument|pat
argument_list|,
argument|a1
argument_list|,
argument|a2
argument_list|,
argument|a3
argument_list|,
argument|a4
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|pat
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
name|pat
argument_list|,
name|a1
argument_list|,
name|a2
argument_list|,
name|a3
argument_list|,
name|a4
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*VARARGS1*/
end_comment

begin_macro
name|warn
argument_list|(
argument|pat
argument_list|,
argument|a1
argument_list|,
argument|a2
argument_list|,
argument|a3
argument_list|,
argument|a4
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|pat
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
name|pat
argument_list|,
name|a1
argument_list|,
name|a2
argument_list|,
name|a3
argument_list|,
name|a4
argument_list|)
expr_stmt|;
block|}
end_block

begin_decl_stmt
specifier|static
name|bool
name|firstsetenv
init|=
name|TRUE
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|char
modifier|*
modifier|*
name|environ
decl_stmt|;
end_decl_stmt

begin_function
name|void
name|setenv
parameter_list|(
name|nam
parameter_list|,
name|val
parameter_list|)
name|char
modifier|*
name|nam
decl_stmt|,
decl|*
name|val
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|int
name|i
init|=
name|envix
argument_list|(
name|nam
argument_list|)
decl_stmt|;
comment|/* where does it go? */
if|if
condition|(
operator|!
name|environ
index|[
name|i
index|]
condition|)
block|{
comment|/* does not exist yet */
if|if
condition|(
name|firstsetenv
condition|)
block|{
comment|/* need we copy environment? */
name|int
name|j
decl_stmt|;
ifndef|#
directive|ifndef
name|lint
name|char
modifier|*
modifier|*
name|tmpenv
init|=
operator|(
name|char
operator|*
operator|*
operator|)
comment|/* point our wand at memory */
name|safemalloc
argument_list|(
operator|(
name|i
operator|+
literal|2
operator|)
operator|*
sizeof|sizeof
argument_list|(
name|char
operator|*
argument_list|)
argument_list|)
decl_stmt|;
else|#
directive|else
name|char
modifier|*
modifier|*
name|tmpenv
init|=
name|Null
argument_list|(
name|char
operator|*
operator|*
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* lint */
name|firstsetenv
operator|=
name|FALSE
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|i
condition|;
name|j
operator|++
control|)
comment|/* copy environment */
name|tmpenv
index|[
name|j
index|]
operator|=
name|environ
index|[
name|j
index|]
expr_stmt|;
name|environ
operator|=
name|tmpenv
expr_stmt|;
comment|/* tell exec where it is now */
block|}
ifndef|#
directive|ifndef
name|lint
else|else
name|environ
operator|=
operator|(
name|char
operator|*
operator|*
operator|)
name|saferealloc
argument_list|(
operator|(
name|char
operator|*
operator|)
name|environ
argument_list|,
operator|(
name|i
operator|+
literal|2
operator|)
operator|*
sizeof|sizeof
argument_list|(
name|char
operator|*
argument_list|)
argument_list|)
expr_stmt|;
comment|/* just expand it a bit */
endif|#
directive|endif
comment|/* lint */
name|environ
index|[
name|i
operator|+
literal|1
index|]
operator|=
name|Nullch
expr_stmt|;
comment|/* make sure it's null terminated */
block|}
name|environ
index|[
name|i
index|]
operator|=
name|safemalloc
argument_list|(
name|strlen
argument_list|(
name|nam
argument_list|)
operator|+
name|strlen
argument_list|(
name|val
argument_list|)
operator|+
literal|2
argument_list|)
expr_stmt|;
comment|/* this may or may not be in */
comment|/* the old environ structure */
name|sprintf
argument_list|(
name|environ
index|[
name|i
index|]
argument_list|,
literal|"%s=%s"
argument_list|,
name|nam
argument_list|,
name|val
argument_list|)
expr_stmt|;
comment|/* all that work just for this */
block|}
end_block

begin_function
name|int
name|envix
parameter_list|(
name|nam
parameter_list|)
name|char
modifier|*
name|nam
decl_stmt|;
block|{
specifier|register
name|int
name|i
decl_stmt|,
name|len
init|=
name|strlen
argument_list|(
name|nam
argument_list|)
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|environ
index|[
name|i
index|]
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|strnEQ
argument_list|(
name|environ
index|[
name|i
index|]
argument_list|,
name|nam
argument_list|,
name|len
argument_list|)
operator|&&
name|environ
index|[
name|i
index|]
index|[
name|len
index|]
operator|==
literal|'='
condition|)
break|break;
comment|/* strnEQ must come first to avoid */
block|}
comment|/* potential SEGV's */
return|return
name|i
return|;
block|}
end_function

end_unit

