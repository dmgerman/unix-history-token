...
index|[]
init|=
literal|"$Id: trans.c,v 1.1 1993/08/04 19:36:31 jtc Exp $"
decl_stmt|;
end_decl_stmt
...
expr_stmt|;
block|}
comment|/* We can read the file in a tight loop until qTremote 		 changes or until we have transferred the entire file. 		 We can disregard any changes to qTlocal since we 		 already have something to send anyhow.  */
while|while
condition|(
name|qTremote
operator|==
...
break|break;
block|}
comment|/* It is possible that this transfer has just been 		     cancelled.  */
if|if
condition|(
name|q
operator|!=
