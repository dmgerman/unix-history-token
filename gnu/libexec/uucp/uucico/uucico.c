...
index|[]
init|=
literal|"$Id: uucico.c,v 1.1 1993/08/04 19:36:34 jtc Exp $"
decl_stmt|;
end_decl_stmt
...
decl_stmt|;
block|{
comment|/* -c: Whether to warn if a call is attempted at a bad time.  */
name|boolean
name|ftimewarn
init|=
name|TRUE
...
name|argv
argument_list|,
literal|"cDefI:lp:qr:s:S:u:x:X:w"
argument_list|,
name|asLongopts
argument_list|,
operator|(
...
literal|'c'
case|:
comment|/* Don't warn if a call is attempted at a bad time.  */
name|ftimewarn
operator|=
name|FALSE
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|/* If a port was given, we loop forever.  */
name|fendless
operator|=
name|TRUE
expr_stmt|;
...
name|FALSE
return|;
comment|/* Make sure it's been long enough since the last failed call, and      that we haven't exceeded the maximum number of retries.  Even if      we are over the limit on retries, we permit a call to be made if      24 hours have passed.  This 24 hour limit is still controlled by      the retry time.  */
name|inow
operator|=
name|ixsysdep_time
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Get any jobs queued for the system, and turn on the selected      protocol.  */
if|if
condition|(
operator|!
name|fqueue
