...
index|[]
init|=
literal|"$Id: uux.c,v 1.2 1994/04/01 13:11:01 jkh Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* The name of the execute file.  */
end_comment
...
name|fexit
decl_stmt|;
comment|/* We need to be able to read a single - as an option, which getopt      won't do.  So that we can still use getopt, we run through the      options looking for an option "-"; if we find one we change it to      "-p", which is equivalent to "-".  */
for|for
control|(
name|i
operator|=
...
expr_stmt|;
block|}
comment|/* Figure out which system the command is to be executed on.  Some      mailers apparently pass local!rmail, so we must explicitly check      for that.  */
name|zexclam
operator|=
name|strchr
argument_list|(
...
name|zlocalname
expr_stmt|;
comment|/* We can send this as an E command if the execution is on a      different, directly connected, system and the only file used is      the standard input and comes from this system.  This is true of      the common cases of rmail and rnews.  We get an execute file name      here in case we need it.  */
if|if
condition|(
name|fxqtlocal
condition|)
...
name|LOG_FATAL
argument_list|,
literal|"!%s: Can't figure out where to get file"
argument_list|,
name|zfile
argument_list|)
expr_stmt|;
