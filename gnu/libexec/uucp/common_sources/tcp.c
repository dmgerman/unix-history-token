...
index|[]
init|=
literal|"$Id: tcp.c,v 1.1 1993/08/04 19:31:06 jtc Exp $"
decl_stmt|;
end_decl_stmt
...
comment|/* Set the effective user id to the real user id.  Since the 	 effective user id is the saved setuid we will able to set 	 back to it later.  If the real user id is root we will not be 	 able to switch back and forth, but that doesn't matter since 	 we only want to switch once.  */
if|if
condition|(
name|setuid
argument_list|(
...
comment|/* There's no way to switch between real permissions and 	 effective permissions.  Just try the bind with the uucp 	 permissions.  */
endif|#
directive|endif
comment|/* ! HAVE_SAVED_SETUID */
endif|#
