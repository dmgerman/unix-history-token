...
index|[]
init|=
literal|"$Id: conn.c,v 1.1 1993/08/04 19:30:39 jtc Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Create a new connection.  This relies on system dependent functions    to set the qcmds and psysdep fields.  If qport is NULL, it opens a    standard input port.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Make sure any signal reporting has been done before we set      fLog_sighup back to TRUE.  */
name|ulog
argument_list|(
name|LOG_ERROR
argument_list|,
...
name|DEBUG_PORT
argument_list|,
literal|"fconn_reset: Resetting connection"
argument_list|)
expr_stmt|;
return|return
call|(
...
name|ffirst
decl_stmt|;
comment|/* The pzdialer field is a sequence of dialer/token pairs.  The 	 dialer portion names a dialer to use.  The token portion is 	 what \D and \T in the chat script expand to.  If there is no 	 token for the last dialer, the phone number for the system is 	 used.  */
name|ffirst
operator|=
name|TRUE
expr_stmt|;
