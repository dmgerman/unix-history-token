...
index|[]
init|=
literal|"$Id: uuxqt.c,v 1.1 1993/08/04 19:37:19 jtc Exp $"
decl_stmt|;
end_decl_stmt
...
name|FALSE
expr_stmt|;
comment|/* If we were given a system name, canonicalize it, since the system      dependent layer will not be returning aliases.  */
if|if
condition|(
name|zdosys
operator|!=
...
block|}
block|,
comment|/* Some systems create execution files in which B takes an argument;      I don't know what it means, so I just ignore it.  */
block|{
literal|"B"
block|,
name|UUCONF_CMDTABTYPE_FN
