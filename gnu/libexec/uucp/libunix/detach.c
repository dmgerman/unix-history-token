...
name|igrp
decl_stmt|;
comment|/* First make sure we are not a process group leader.  If we have      TIOCNOTTY, this doesn't matter, since TIOCNOTTY sets our process      group to 0 anyhow.  */
if|#
directive|if
name|HAVE_BSD_PGRP
name|igrp
...
directive|endif
comment|/* ! HAVE_BSD_PGRP || ! HAVE_TIOCNOTTY */
if|#
directive|if
name|HAVE_TIOCNOTTY
comment|/* Lose the original controlling terminal.  If standard input has      been reopened to /dev/null, this will do no harm.  If another      port has been opened to become the controlling terminal, it      should have been detached when it was closed.  */
operator|(
name|void
operator|)
name|ioctl
...
directive|if
name|HAVE_SETPGRP
comment|/* Now we assume we have the System V setpgrp, which takes no      arguments, and we couldn't compile the HAVE_BSD_PGRP code above      because there was a prototype somewhere in scope.  On System V      setpgrp makes us the leader of a new process group and also      detaches the controlling terminal.  */
if|if
condition|(
name|setpgrp
argument_list|()
