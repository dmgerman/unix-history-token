...
index|[]
init|=
literal|"$Id: statsb.c,v 1.1 1993/08/04 19:33:06 jtc Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* See whether the user is permitted to kill arbitrary jobs.  This is    true only for root and uucp.  We check for uucp by seeing if the    real user ID and the effective user ID are the same; this works    because we should be suid to uucp, so our effective user ID will    always be uucp while our real user ID will be whoever ran the    program.  */
end_comment
