...
begin_comment
comment|// Define the timeout for EFI_DEBUGPORT_PROTOCOL.Write
end_comment
...
begin_comment
comment|/**   Send message to DebugPort Protocol.    If mDebugPort is NULL, i.e. EFI_DEBUGPORT_PROTOCOL is not located,   EFI_DEBUGPORT_PROTOCOL is located first.   Then, Buffer is sent via EFI_DEBUGPORT_PROTOCOL.Write.    @param  Buffer         The message to be sent.   @param  BufferLength   The byte length of Buffer. **/
end_comment

begin_function
name|VOID
name|UefiDebugLibDebugPortProtocolWrite
parameter_list|(
name|IN
name|CONST
name|CHAR8
...
decl_stmt|;
comment|//
comment|// If mDebugPort is NULL, initialize first.
comment|//
if|if
condition|(
name|mDebugPort
...
block|}
comment|//
comment|// EFI_DEBUGPORT_PROTOCOL.Write is called until all message is sent.
comment|//
while|while
condition|(
name|BufferLength
...
block|}
comment|//
comment|// Convert the DEBUG() message to an ASCII String
comment|//
name|VA_START
argument_list|(
name|Marker
...
expr_stmt|;
comment|//
comment|// Send the print string to EFI_DEBUGPORT_PROTOCOL.Write.
comment|//
name|UefiDebugLibDebugPortProtocolWrite
argument_list|(
name|Buffer
...
decl_stmt|;
comment|//
comment|// Generate the ASSERT() message in ASCII format
comment|//
name|AsciiSPrint
argument_list|(
name|Buffer
