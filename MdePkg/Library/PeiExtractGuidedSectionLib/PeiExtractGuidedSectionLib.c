...
begin_comment
comment|/** @file   Provide generic extract guided section functions for PEI phase.    Copyright (c) 2007 - 2012, Intel Corporation. All rights reserved.<BR>   This program and the accompanying materials   are licensed and made available under the terms and conditions of the BSD License   which accompanies this distribution.  The full text of the license may be found at   http://opensource.org/licenses/bsd-license.php.    THE PROGRAM IS DISTRIBUTED UNDER THE BSD LICENSE ON AN "AS IS" BASIS,   WITHOUT WARRANTIES OR REPRESENTATIONS OF ANY KIND, EITHER EXPRESS OR IMPLIED.  **/
end_comment
...
define|#
directive|define
name|PEI_EXTRACT_HANDLER_INFO_SIGNATURE
value|SIGNATURE_32 ('P', 'E', 'H', 'I')
end_define
...
decl_stmt|;
block|}
name|PEI_EXTRACT_GUIDED_SECTION_HANDLER_INFO
typedef|;
end_typedef

begin_comment
comment|/**   Build guid hob for the global memory to store the registered guid and Handler list.   If GuidHob exists, HandlerInfo will be directly got from Guid hob data.    @param[in, out]  InfoPointer   The pointer to pei handler information structure.    @retval  RETURN_SUCCESS            Build Guid hob for the global memory space to store guid and function tables.   @retval  RETURN_OUT_OF_RESOURCES   No enough memory to allocated. **/
end_comment

begin_function
name|RETURN_STATUS
name|PeiGetExtractGuidedSectionHandlerInfo
parameter_list|(
name|IN
name|OUT
name|PEI_EXTRACT_GUIDED_SECTION_HANDLER_INFO
...
decl_stmt|;
comment|//
comment|// First try to get handler information from guid hob specified by CallerId.
comment|//
name|Hob
operator|.
name|Raw
...
block|{
comment|//
comment|// Update Table Pointer when hob start address is changed.
comment|//
if|if
condition|(
name|HandlerInfo
...
block|}
comment|//
comment|// Return HandlerInfo pointer.
comment|//
operator|*
name|InfoPointer
operator|=
...
block|}
comment|//
comment|// If Guid Hob is not found, Build CallerId Guid hob to store Handler Info
comment|//
name|HandlerInfo
operator|=
name|BuildGuidHob
...
block|{
comment|//
comment|// No enough resource to build guid hob.
comment|//
operator|*
name|InfoPointer
operator|=
...
expr_stmt|;
comment|//
comment|// return the created HandlerInfo.
comment|//
operator|*
name|InfoPointer
operator|=
...
expr_stmt|;
comment|//
comment|// Build the Guided Section GUID HOB to record the GUID itself.
comment|// Then the content of the GUIDed HOB will be the same as the GUID value itself.
comment|//
