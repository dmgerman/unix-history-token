...
begin_comment
comment|/** @file   UEFI Runtime Library implementation for non IPF processor types.    This library hides the global variable for the EFI Runtime Services so the   caller does not need to deal with the possibility of being called from an   OS virtual address space. All pointer values are different for a virtual    mapping than from the normal physical mapping at boot services time.  Copyright (c) 2006 - 2010, Intel Corporation. All rights reserved.<BR> This program and the accompanying materials are licensed and made available under the terms and conditions of the BSD License which accompanies this distribution.  The full text of the license may be found at http://opensource.org/licenses/bsd-license.php.  THE PROGRAM IS DISTRIBUTED UNDER THE BSD LICENSE ON AN "AS IS" BASIS, WITHOUT WARRANTIES OR REPRESENTATIONS OF ANY KIND, EITHER EXPRESS OR IMPLIED.  **/
end_comment
...
begin_comment
comment|/// Driver Lib Module Globals
end_comment
...
begin_decl_stmt
name|EFI_EVENT
name|mEfiExitBootServicesEvent
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|BOOLEAN
name|mEfiGoneVirtual
init|=
name|FALSE
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/**   Set AtRuntime flag as TRUE after ExitBootServices.    @param[in]  Event   The Event that is being processed.   @param[in]  Context The Event Context.  **/
end_comment

begin_function
name|VOID
name|EFIAPI
name|RuntimeLibExitBootServicesEvent
parameter_list|(
name|IN
name|EFI_EVENT
name|Event
...
begin_comment
comment|/**   Fixup internal data so that EFI can be call in virtual mode.   Call the passed in Child Notify event and convert any pointers in   lib to virtual mode.    @param[in]    Event   The Event that is being processed.   @param[in]    Context The Event Context. **/
end_comment

begin_function
name|VOID
name|EFIAPI
name|RuntimeLibVirtualNotifyEvent
parameter_list|(
name|IN
name|EFI_EVENT
name|Event
...
block|{
comment|//
comment|// Update global for Runtime Services Table and IO
comment|//
...
begin_comment
comment|/**   Initialize runtime Driver Lib if it has not yet been initialized.   It will ASSERT() if gRT is NULL or gBS is NULL.   It will ASSERT() if that operation fails.    @param[in]  ImageHandle   The firmware allocated handle for the EFI image.   @param[in]  SystemTable   A pointer to the EFI System Table.    @return     EFI_STATUS    always returns EFI_SUCCESS except EFI_ALREADY_STARTED if already started. **/
end_comment

begin_function
name|EFI_STATUS
name|EFIAPI
name|RuntimeDriverLibConstruct
parameter_list|(
name|IN
name|EFI_HANDLE
name|ImageHandle
...
begin_comment
comment|/**   This function allows the caller to determine if UEFI ExitBootServices() has been called.    This function returns TRUE after all the EVT_SIGNAL_EXIT_BOOT_SERVICES functions have   executed as a result of the OS calling ExitBootServices().  Prior to this time FALSE   is returned. This function is used by runtime code to decide it is legal to access   services that go away after ExitBootServices().    @retval  TRUE  The system has finished executing the EVT_SIGNAL_EXIT_BOOT_SERVICES event.   @retval  FALSE The system has not finished executing the EVT_SIGNAL_EXIT_BOOT_SERVICES event.  **/
end_comment
...
decl_stmt|;
comment|//
comment|// For NULL List, return EFI_SUCCESS
comment|//
if|if
condition|(
name|ListHead
...
block|}
comment|//
comment|// Convert all the ForwardLink& BackLink pointers in the list
comment|//
name|Link
operator|=
name|ListHead
