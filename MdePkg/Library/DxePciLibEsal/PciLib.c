...
begin_comment
comment|/**   Converts a PCI Library Address to a ESAL PCI Service Address.   Based on SAL Spec 3.2, there are two SAL PCI Address:    If address type = 0   Bits 0..7 - Register address   Bits 8..10 - Function number   Bits 11..15 - Device number   Bits 16..23 - Bus number   Bits 24..31 - PCI segment group   Bits 32..63 - Reserved (0)    If address type = 1   Bits 0..7 - Register address   Bits 8..11 - Extended Register address   Bits 12..14 - Function number   Bits 15..19 - Device number   Bits 20..27 - Bus number   Bits 28..43 - PCI segment group   Bits 44..63 - Reserved (0)    @param  A The PCI Library Address to convert.  **/
end_comment

begin_define
define|#
directive|define
name|CONVERT_PCI_LIB_ADDRESS_TO_PCI_ESAL_ADDRESS0
parameter_list|(
name|Address
parameter_list|)
value|((((Address)>> 4)& 0x00ffff00) | ((Address)& 0xff))
end_define

begin_define
define|#
directive|define
name|CONVERT_PCI_LIB_ADDRESS_TO_PCI_ESAL_ADDRESS1
parameter_list|(
name|Address
parameter_list|)
value|(Address)
...
begin_comment
comment|/**   Check a PCI Library Address is a PCI Compatible Address or not. **/
end_comment

begin_define
define|#
directive|define
name|IS_PCI_COMPATIBLE_ADDRESS
parameter_list|(
name|Address
parameter_list|)
value|(((Address)& 0xf00) == 0)
end_define
