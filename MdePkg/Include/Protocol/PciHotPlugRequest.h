...
begin_comment
comment|/// Global ID for EFI_PCI_HOTPLUG_REQUEST_PROTOCOL
end_comment
...
define|\
value|{ \     0x19cb87ab, 0x2cb9, 0x4665, {0x83, 0x60, 0xdd, 0xcf, 0x60, 0x54, 0xf7, 0x9d} \   }
end_define
...
begin_comment
comment|/// Forward declaration for EFI_PCI_HOTPLUG_REQUEST_PROTOCOL
end_comment
...
typedef|typedef
name|struct
name|_EFI_PCI_HOTPLUG_REQUEST_PROTOCOL
name|EFI_PCI_HOTPLUG_REQUEST_PROTOCOL
typedef|;
end_typedef
...
begin_comment
comment|/// Enumeration of PCI hot plug operations
end_comment
...
name|EFIAPI
modifier|*
name|EFI_PCI_HOTPLUG_REQUEST_NOTIFY
function_decl|)
parameter_list|(
name|IN
name|EFI_PCI_HOTPLUG_REQUEST_PROTOCOL
...
begin_comment
comment|/// Provides services to notify PCI bus driver that some events have happened in
end_comment

begin_comment
comment|/// a hot-plug controller (for example, PC Card socket, or PHPC), and ask PCI bus
end_comment

begin_comment
comment|/// driver to create or destroy handles for the PCI-like devices.
end_comment
...
block|{
comment|///
comment|/// Notify the PCI bus driver that some events have happened in a hot-plug
comment|/// controller (for example, PC Card socket, or PHPC), and ask PCI bus driver
comment|/// to create or destroy handles for the PCI-like devices. See Section 0 for
...
specifier|extern
name|EFI_GUID
name|gEfiPciHotPlugRequestProtocolGuid
decl_stmt|;
end_decl_stmt
