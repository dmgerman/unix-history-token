...
define|#
directive|define
name|EFI_S3_SAVE_STATE_PROTOCOL_GUID
define|\
value|{ 0xe857caf6, 0xc046, 0x45dc, { 0xbe, 0x3f, 0xee, 0x7, 0x65, 0xfb, 0xa8, 0x87 }}
end_define
...
name|VOID
modifier|*
name|EFI_S3_BOOT_SCRIPT_POSITION
typedef|;
end_typedef
...
typedef|typedef
name|struct
name|_EFI_S3_SAVE_STATE_PROTOCOL
name|EFI_S3_SAVE_STATE_PROTOCOL
typedef|;
end_typedef

begin_comment
comment|/**   Record operations that need to be replayed during an S3 resume.      This function is used to store an OpCode to be replayed as part of the S3 resume boot path. It is   assumed this protocol has platform specific mechanism to store the OpCode set and replay them   during the S3 resume.      @param[in]    This    A pointer to the EFI_S3_SAVE_STATE_PROTOCOL instance.   @param[in]    OpCode  The operation code (opcode) number.   @param[in]    ...     Argument list that is specific to each opcode. See the following subsections for the                         definition of each opcode.                            @retval EFI_SUCCESS           The operation succeeded. A record was added into the specified                                 script table.                       @retval EFI_INVALID_PARAMETER The parameter is illegal or the given boot script is not supported.   @retval EFI_OUT_OF_RESOURCES  There is insufficient memory to store the boot script.                        **/
end_comment
...
name|EFIAPI
modifier|*
name|EFI_S3_SAVE_STATE_COMPARE
function_decl|)
parameter_list|(
name|IN
name|CONST
