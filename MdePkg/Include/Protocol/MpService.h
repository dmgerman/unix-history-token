...
begin_comment
comment|/// Global ID for the EFI_MP_SERVICES_PROTOCOL.
end_comment
...
define|#
directive|define
name|EFI_MP_SERVICES_PROTOCOL_GUID
define|\
value|{ \     0x3fdda605, 0xa76e, 0x4f46, {0xad, 0x29, 0x12, 0xf4, 0x53, 0x1b, 0x3d, 0x08} \   }
end_define
...
begin_comment
comment|/// Forward declaration for the EFI_MP_SERVICES_PROTOCOL.
end_comment
...
typedef|typedef
name|struct
name|_EFI_MP_SERVICES_PROTOCOL
name|EFI_MP_SERVICES_PROTOCOL
typedef|;
end_typedef
...
begin_comment
comment|/// Terminator for a list of failed CPUs returned by StartAllAPs().
end_comment
...
begin_comment
comment|/// This bit is used in the StatusFlag field of EFI_PROCESSOR_INFORMATION and
end_comment

begin_comment
comment|/// indicates whether the processor is playing the role of BSP. If the bit is 1,
end_comment

begin_comment
comment|/// then the processor is BSP. Otherwise, it is AP.
end_comment
...
begin_comment
comment|/// indicates whether the processor is enabled. If the bit is 1, then the
end_comment

begin_comment
comment|/// processor is enabled. Otherwise, it is disabled.
end_comment
...
begin_comment
comment|/// indicates whether the processor is healthy. If the bit is 1, then the
end_comment

begin_comment
comment|/// processor is healthy. Otherwise, some fault has been detected for the processor.
end_comment
...
define|#
directive|define
name|PROCESSOR_HEALTH_STATUS_BIT
value|0x00000004
end_define
...
begin_comment
comment|/// Structure that describes the pyhiscal location of a logical CPU.
end_comment
...
block|{
comment|///
comment|/// Zero-based physical package number that identifies the cartridge of the processor.
comment|///
name|UINT32
name|Package
decl_stmt|;
comment|///
comment|/// Zero-based physical core number within package of the processor.
comment|///
name|UINT32
name|Core
decl_stmt|;
comment|///
comment|/// Zero-based logical thread number within core of the processor.
comment|///
name|UINT32
name|Thread
decl_stmt|;
block|}
name|EFI_CPU_PHYSICAL_LOCATION
typedef|;
end_typedef
...
begin_comment
comment|/// Structure that describes information about a logical CPU.
end_comment
...
block|{
comment|///
comment|/// The unique processor ID determined by system hardware.  For IA32 and X64,
comment|/// the processor ID is the same as the Local APIC ID. Only the lower 8 bits
comment|/// are used, and higher bits are reserved.  For IPF, the lower 16 bits contains
comment|/// id/eid, and higher bits are reserved.
comment|///
name|UINT64
name|ProcessorId
decl_stmt|;
comment|///
comment|/// Flags indicating if the processor is BSP or AP, if the processor is enabled
comment|/// or disabled, and if the processor is healthy. Bits 3..31 are reserved and
comment|/// must be 0.
comment|///
comment|///<pre>
comment|/// BSP  ENABLED  HEALTH  Description
comment|/// ===  =======  ======  ===================================================
comment|///  0      0       0     Unhealthy Disabled AP.
comment|///  0      0       1     Healthy Disabled AP.
comment|///  0      1       0     Unhealthy Enabled AP.
comment|///  0      1       1     Healthy Enabled AP.
comment|///  1      0       0     Invalid. The BSP can never be in the disabled state.
comment|///  1      0       1     Invalid. The BSP can never be in the disabled state.
comment|///  1      1       0     Unhealthy Enabled BSP.
comment|///  1      1       1     Healthy Enabled BSP.
comment|///</pre>
comment|///
name|UINT32
name|StatusFlag
decl_stmt|;
comment|///
comment|/// The physical location of the processor, including the physical package number
comment|/// that identifies the cartridge, the physical core number within package, and
comment|/// logical thread number within core.
comment|///
name|EFI_CPU_PHYSICAL_LOCATION
name|Location
decl_stmt|;
...
name|EFIAPI
modifier|*
name|EFI_MP_SERVICES_GET_NUMBER_OF_PROCESSORS
function_decl|)
parameter_list|(
name|IN
name|EFI_MP_SERVICES_PROTOCOL
...
name|EFIAPI
modifier|*
name|EFI_MP_SERVICES_GET_PROCESSOR_INFO
function_decl|)
parameter_list|(
name|IN
name|EFI_MP_SERVICES_PROTOCOL
...
name|EFIAPI
modifier|*
name|EFI_MP_SERVICES_STARTUP_ALL_APS
function_decl|)
parameter_list|(
name|IN
name|EFI_MP_SERVICES_PROTOCOL
...
name|EFIAPI
modifier|*
name|EFI_MP_SERVICES_STARTUP_THIS_AP
function_decl|)
parameter_list|(
name|IN
name|EFI_MP_SERVICES_PROTOCOL
...
name|EFIAPI
modifier|*
name|EFI_MP_SERVICES_SWITCH_BSP
function_decl|)
parameter_list|(
name|IN
name|EFI_MP_SERVICES_PROTOCOL
...
name|EFIAPI
modifier|*
name|EFI_MP_SERVICES_ENABLEDISABLEAP
function_decl|)
parameter_list|(
name|IN
name|EFI_MP_SERVICES_PROTOCOL
...
begin_comment
comment|/// When installed, the MP Services Protocol produces a collection of services
end_comment

begin_comment
comment|/// that are needed for MP management.
end_comment
...
begin_comment
comment|/// Before the UEFI event EFI_EVENT_GROUP_READY_TO_BOOT is signaled, the module
end_comment

begin_comment
comment|/// that produces this protocol is required to place all APs into an idle state
end_comment

begin_comment
comment|/// whenever the APs are disabled or the APs are not executing code as requested
end_comment

begin_comment
comment|/// through the StartupAllAPs() or StartupThisAP() services. The idle state of
end_comment

begin_comment
comment|/// an AP before the UEFI event EFI_EVENT_GROUP_READY_TO_BOOT is signaled is
end_comment
...
begin_comment
comment|/// After the UEFI event EFI_EVENT_GROUP_READY_TO_BOOT is signaled, all the APs
end_comment

begin_comment
comment|/// must be placed in the OS compatible CPU state as defined by the UEFI
end_comment

begin_comment
comment|/// Specification. Implementations of this protocol may use the UEFI event
end_comment

begin_comment
comment|/// EFI_EVENT_GROUP_READY_TO_BOOT to force APs into the OS compatible state as
end_comment

begin_comment
comment|/// defined by the UEFI Specification. Modules that use this protocol must
end_comment

begin_comment
comment|/// guarantee that all non-blocking mode requests on all APs have been completed
end_comment

begin_comment
comment|/// before the UEFI event EFI_EVENT_GROUP_READY_TO_BOOT is signaled. Since the
end_comment

begin_comment
comment|/// order that event notification functions in the same event group are executed
end_comment

begin_comment
comment|/// is not deterministic, an event of type EFI_EVENT_GROUP_READY_TO_BOOT cannot
end_comment

begin_comment
comment|/// be used to guarantee that APs have completed their non-blocking mode requests.
end_comment
...
begin_comment
comment|/// When the UEFI event EFI_EVENT_GROUP_READY_TO_BOOT is signaled, the StartAllAPs()
end_comment

begin_comment
comment|/// and StartupThisAp() services must no longer support non-blocking mode requests.
end_comment

begin_comment
comment|/// The support for SwitchBSP() and EnableDisableAP() may no longer be supported
end_comment

begin_comment
comment|/// after this event is signaled. Since UEFI Applications and UEFI OS Loaders
end_comment

begin_comment
comment|/// execute after the UEFI event EFI_EVENT_GROUP_READY_TO_BOOT is signaled, these
end_comment

begin_comment
comment|/// UEFI images must be aware that the functionality of this protocol may be reduced.
end_comment
...
specifier|extern
name|EFI_GUID
name|gEfiMpServiceProtocolGuid
decl_stmt|;
end_decl_stmt
