...
define|\
value|{ \     0x7f1647c8, 0xb76e, 0x44b2, {0xa5, 0x65, 0xf7, 0xf, 0xf1, 0x9c, 0xd1, 0x9e } \   }
end_define

begin_define
define|#
directive|define
name|EFI_DNS6_PROTOCOL_GUID
define|\
value|{ \     0xca37bc1f, 0xa327, 0x4ae9, {0x82, 0x8a, 0x8c, 0x40, 0xd8, 0x50, 0x6a, 0x17 } \   }
end_define

begin_typedef
typedef|typedef
name|struct
name|_EFI_DNS6_PROTOCOL
name|EFI_DNS6_PROTOCOL
typedef|;
end_typedef
...
block|{
comment|///
comment|/// If TRUE, enable DNS cache function for this DNS instance. If FALSE, all DNS query
comment|/// will not lookup local DNS cache.
comment|///
name|BOOLEAN
name|EnableDnsCache
decl_stmt|;
comment|///
comment|/// Use the protocol number defined in
comment|/// http://www.iana.org/assignments/protocol-numbers. Beside TCP/UDP, Other protocol
comment|/// is invalid value. An implementation can choose to support UDP, or both TCP and UDP.
comment|///
name|UINT8
name|Protocol
decl_stmt|;
...
decl_stmt|;
comment|///
comment|/// Pointer to DNS server list containing DnsServerListCount
comment|/// entries or NULL if DnsServerListCount is 0. For Configure(),
comment|/// this will be NULL when there are no caller supplied server addresses
comment|/// and the DNS instance will retrieve DNS server from DHCP Server.
comment|/// The provided DNS server list is recommended to be filled up in the sequence
comment|/// of preference. When used with GetModeData(), the buffer containing the list
comment|/// will be allocated by the driver implementing this protocol and must be
...
block|{
comment|///
comment|/// Host name. This should be interpreted as Unicode characters.
comment|///
name|CHAR16
modifier|*
name|HostName
...
decl_stmt|;
comment|///
comment|/// Time in second unit that this entry will remain in DNS cache. A value of zero means
comment|/// that this entry is permanent. A nonzero value will override the existing one if
comment|/// this entry to be added is dynamic entry. Implementations may set its default
comment|/// timeout value for the dynamically created DNS cache entry after one DNS resolve
comment|/// succeeds.
name|UINT32
name|Timeout
decl_stmt|;
block|}
name|EFI_DNS6_CACHE_ENTRY
typedef|;
end_typedef
...
decl_stmt|;
comment|///
comment|/// Number of configured DNS6 servers.
comment|///
name|UINT32
name|DnsServerCount
decl_stmt|;
comment|///
comment|/// Pointer to common list of addresses of all configured DNS server used by EFI_DNS6_PROTOCOL
comment|/// instances. List will include DNS servers configured by this or any other EFI_DNS6_PROTOCOL
comment|/// instance. The storage for this list is allocated by the driver publishing this protocol,
comment|/// and must be freed by the caller.
comment|///
name|EFI_IPv6_ADDRESS
modifier|*
name|DnsServerList
...
comment|/// entry structures. The storage for thislist is allocated by the driver
...
begin_comment
comment|/// DNS6_HOST_TO_ADDR_DATA
end_comment
...
block|{
comment|///
comment|/// Number of the returned IP address.
comment|///
name|UINT32
name|IpCount
decl_stmt|;
comment|///
comment|/// Pointer to the all the returned IP address.
comment|///
name|EFI_IPv6_ADDRESS
modifier|*
name|IpList
...
begin_comment
comment|/// DNS6_ADDR_TO_HOST_DATA
end_comment
...
comment|/// this RR cannot be cached.
comment|///
name|UINT32
name|TTL
decl_stmt|;
...
begin_comment
comment|/// DNS6_GENERAL_LOOKUP_DATA
end_comment
...
begin_comment
comment|/// EFI_DNS6_COMPLETION_TOKEN
end_comment
...
block|{
comment|///
comment|/// This Event will be signaled after the Status field is updated by the EFI DNSv6
...
decl_stmt|;
comment|///
comment|/// The parameter configured through DNSv6.Configure() interface. Retry number if no
comment|/// response received after RetryInterval.
comment|///
name|UINT32
name|RetryCount
decl_stmt|;
comment|///
comment|/// The parameter configured through DNSv6.Configure() interface. Minimum interval of
comment|/// retry is 2 seconds. If the retry interval is less than 2 seconds, then use the 2
comment|/// seconds.
comment|///
name|UINT32
name|RetryInterval
...
comment|/// pointer to the DNS6_ADDR_TO_HOST_DATA.
comment|///
name|DNS6_ADDR_TO_HOST_DATA
modifier|*
name|A2HData
...
comment|/// the DNS6_GENERAL_LOOKUP_DATA.
comment|///
name|DNS6_GENERAL_LOOKUP_DATA
modifier|*
name|GLookupData
...
name|EFIAPI
modifier|*
name|EFI_DNS6_UPDATE_DNS_CACHE
function_decl|)
parameter_list|(
name|IN
name|EFI_DNS6_PROTOCOL
...
begin_comment
comment|/// The EFI_DNS6_PROTOCOL provides the function to get the host name and address
end_comment

begin_comment
comment|/// mapping, also provide pass through interface to retrieve arbitrary information from
end_comment
...
specifier|extern
name|EFI_GUID
name|gEfiDns6ServiceBindingProtocolGuid
decl_stmt|;
end_decl_stmt
