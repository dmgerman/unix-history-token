...
begin_comment
comment|/** @file   Provides services to create and parse HOBs. Only available for PEI   and DXE module types.    The HOB Library supports the efficient creation and searching of HOBs   defined in the PI Specification.   A HOB is a Hand-Off Block, defined in the Framework architecture, that   allows the PEI phase to pass information to the DXE phase. HOBs are position   independent and can be relocated easily to different memory memory locations.  Copyright (c) 2006 - 2016, Intel Corporation. All rights reserved.<BR> This program and the accompanying materials are licensed and made available under the terms and conditions of the BSD License which accompanies this distribution.  The full text of the license may be found at http://opensource.org/licenses/bsd-license.php  THE PROGRAM IS DISTRIBUTED UNDER THE BSD LICENSE ON AN "AS IS" BASIS, WITHOUT WARRANTIES OR REPRESENTATIONS OF ANY KIND, EITHER EXPRESS OR IMPLIED.  **/
end_comment
...
begin_comment
comment|/**   Returns the first instance of the matched GUID HOB among the whole HOB list.    This function searches the first instance of a HOB among the whole HOB list.   Such HOB should satisfy two conditions:   its HOB type is EFI_HOB_TYPE_GUID_EXTENSION and its GUID Name equals to the input Guid.   If there does not exist such HOB from the starting HOB pointer, it will return NULL.   Caller is required to apply GET_GUID_HOB_DATA () and GET_GUID_HOB_DATA_SIZE ()   to extract the data section and its size info respectively.    If the pointer to the HOB list is NULL, then ASSERT().   If Guid is NULL, then ASSERT().    @param  Guid          The GUID to match with in the HOB list.    @return The first instance of the matched GUID HOB among the whole HOB list.  **/
end_comment
...
begin_comment
comment|/**   Returns the type of a HOB.    This macro returns the HobType field from the HOB header for the   HOB specified by HobStart.    @param  HobStart   A pointer to a HOB.    @return HobType.  **/
end_comment
...
parameter_list|)
define|\
value|((*(EFI_HOB_GENERIC_HEADER **)&(HobStart))->HobType)
end_define

begin_comment
comment|/**   Returns the length, in bytes, of a HOB.    This macro returns the HobLength field from the HOB header for the   HOB specified by HobStart.    @param  HobStart   A pointer to a HOB.    @return HobLength.  **/
end_comment
...
parameter_list|)
define|\
value|((*(EFI_HOB_GENERIC_HEADER **)&(HobStart))->HobLength)
end_define

begin_comment
comment|/**   Returns a pointer to the next HOB in the HOB list.    This macro returns a pointer to HOB that follows the   HOB specified by HobStart in the HOB List.    @param  HobStart   A pointer to a HOB.    @return A pointer to the next HOB in the HOB list.  **/
end_comment
...
parameter_list|)
define|\
value|(VOID *)(*(UINT8 **)&(HobStart) + GET_HOB_LENGTH (HobStart))
end_define

begin_comment
comment|/**   Determines if a HOB is the last HOB in the HOB list.    This macro determine if the HOB specified by HobStart is the   last HOB in the HOB list.  If HobStart is last HOB in the HOB list,   then TRUE is returned.  Otherwise, FALSE is returned.    @param  HobStart   A pointer to a HOB.    @retval TRUE       The HOB specified by HobStart is the last HOB in the HOB list.   @retval FALSE      The HOB specified by HobStart is not the last HOB in the HOB list.  **/
end_comment
...
name|HobStart
parameter_list|)
value|(GET_HOB_TYPE (HobStart) == (UINT16)EFI_HOB_TYPE_END_OF_HOB_LIST)
end_define

begin_comment
comment|/**   Returns a pointer to data buffer from a HOB of type EFI_HOB_TYPE_GUID_EXTENSION.    This macro returns a pointer to the data buffer in a HOB specified by HobStart.   HobStart is assumed to be a HOB of type EFI_HOB_TYPE_GUID_EXTENSION.    @param   GuidHob   A pointer to a HOB.    @return  A pointer to the data buffer in a HOB.  **/
end_comment

begin_define
define|#
directive|define
name|GET_GUID_HOB_DATA
parameter_list|(
name|HobStart
parameter_list|)
define|\
value|(VOID *)(*(UINT8 **)&(HobStart) + sizeof (EFI_HOB_GUID_TYPE))
end_define

begin_comment
comment|/**   Returns the size of the data buffer from a HOB of type EFI_HOB_TYPE_GUID_EXTENSION.    This macro returns the size, in bytes, of the data buffer in a HOB specified by HobStart.   HobStart is assumed to be a HOB of type EFI_HOB_TYPE_GUID_EXTENSION.    @param   GuidHob   A pointer to a HOB.    @return  The size of the data buffer. **/
end_comment

begin_define
define|#
directive|define
name|GET_GUID_HOB_DATA_SIZE
parameter_list|(
name|HobStart
parameter_list|)
define|\
value|(UINT16)(GET_HOB_LENGTH (HobStart) - sizeof (EFI_HOB_GUID_TYPE))
end_define
