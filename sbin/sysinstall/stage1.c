...
argument_list|()
expr_stmt|;
comment|/*       * Try to be intelligent about this and assign some mountpoints      */
define|#
directive|define
name|LEGAL
parameter_list|(
...
name|part
parameter_list|)
value|( 						\ 	Dlbl[disk]->d_partitions[part].p_size&&			\ 	(Dlbl[disk]->d_partitions[part].p_offset>= 			\ 	    Dlbl[disk]->d_partitions[OURPART].p_offset)&&		\ 	((Dlbl[disk]->d_partitions[part].p_size + 			\ 		Dlbl[disk]->d_partitions[part].p_offset)<=		\ 	    (Dlbl[disk]->d_partitions[OURPART].p_offset + 		\ 		Dlbl[disk]->d_partitions[OURPART].p_size)))
for|for
control|(
name|i
operator|=
...
literal|0
argument_list|,
literal|"(H)elp  (F)disk  (D)isklabel  (P)roceed  (Q)uit"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|complaint
operator|=
literal|"You must assign something to mount on '/'"
expr_stmt|;
break|break;
block|}
if|if
...
name|complaint
operator|=
literal|"You must assign something to mount on 'swap'"
expr_stmt|;
break|break;
block|}
if|if
...
name|complaint
operator|=
literal|"You must assign something to mount on '/usr'"
expr_stmt|;
break|break;
block|}
name|ret
