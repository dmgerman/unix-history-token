begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1996 Alex Nash, Paul Traina, Poul-Henning Kamp  * Copyright (c) 1994 Ugen J.S.Antsilevich  *  * Idea and grammar partially left from:  * Copyright (c) 1993 Daniel Boulet  *  * Redistribution and use in source forms, with and without modification,  * are permitted provided that this entire comment appears intact.  *  * Redistribution in binary form may occur without any restrictions.  * Obviously, it would be nice if you gave credit where credit is due  * but requiring it would be too onerous.  *  * This software is provided ``AS IS'' without any warranties of any kind.  *  * NEW command line interface for IP firewall facility  *  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
specifier|const
name|char
name|rcsid
index|[]
init|=
literal|"$FreeBSD$"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/sockio.h>
end_include

begin_include
include|#
directive|include
file|<sys/sysctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/wait.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<err.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<grp.h>
end_include

begin_include
include|#
directive|include
file|<limits.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_include
include|#
directive|include
file|<pwd.h>
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<stdarg.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<sysexits.h>
end_include

begin_include
include|#
directive|include
file|<net/if.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_systm.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip_icmp.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip_fw.h>
end_include

begin_include
include|#
directive|include
file|<net/route.h>
end_include

begin_comment
comment|/* def. of struct route */
end_comment

begin_include
include|#
directive|include
file|<netinet/ip_dummynet.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp.h>
end_include

begin_include
include|#
directive|include
file|<arpa/inet.h>
end_include

begin_decl_stmt
name|int
name|s
decl_stmt|,
comment|/* main RAW socket */
name|do_resolv
decl_stmt|,
comment|/* Would try to resolve all */
name|do_acct
decl_stmt|,
comment|/* Show packet/byte count */
name|do_time
decl_stmt|,
comment|/* Show time stamps */
name|do_quiet
decl_stmt|,
comment|/* Be quiet in add and flush */
name|do_force
decl_stmt|,
comment|/* Don't ask for confirmation */
name|do_pipe
decl_stmt|,
comment|/* this cmd refers to a pipe */
name|do_sort
decl_stmt|,
comment|/* field to sort results (0 = no) */
name|do_dynamic
decl_stmt|,
comment|/* display dynamic rules */
name|do_expired
decl_stmt|,
comment|/* display expired dynamic rules */
name|verbose
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|icmpcode
block|{
name|int
name|code
decl_stmt|;
name|char
modifier|*
name|str
decl_stmt|;
block|}
struct|;
end_struct

begin_decl_stmt
specifier|static
name|struct
name|icmpcode
name|icmpcodes
index|[]
init|=
block|{
block|{
name|ICMP_UNREACH_NET
block|,
literal|"net"
block|}
block|,
block|{
name|ICMP_UNREACH_HOST
block|,
literal|"host"
block|}
block|,
block|{
name|ICMP_UNREACH_PROTOCOL
block|,
literal|"protocol"
block|}
block|,
block|{
name|ICMP_UNREACH_PORT
block|,
literal|"port"
block|}
block|,
block|{
name|ICMP_UNREACH_NEEDFRAG
block|,
literal|"needfrag"
block|}
block|,
block|{
name|ICMP_UNREACH_SRCFAIL
block|,
literal|"srcfail"
block|}
block|,
block|{
name|ICMP_UNREACH_NET_UNKNOWN
block|,
literal|"net-unknown"
block|}
block|,
block|{
name|ICMP_UNREACH_HOST_UNKNOWN
block|,
literal|"host-unknown"
block|}
block|,
block|{
name|ICMP_UNREACH_ISOLATED
block|,
literal|"isolated"
block|}
block|,
block|{
name|ICMP_UNREACH_NET_PROHIB
block|,
literal|"net-prohib"
block|}
block|,
block|{
name|ICMP_UNREACH_HOST_PROHIB
block|,
literal|"host-prohib"
block|}
block|,
block|{
name|ICMP_UNREACH_TOSNET
block|,
literal|"tosnet"
block|}
block|,
block|{
name|ICMP_UNREACH_TOSHOST
block|,
literal|"toshost"
block|}
block|,
block|{
name|ICMP_UNREACH_FILTER_PROHIB
block|,
literal|"filter-prohib"
block|}
block|,
block|{
name|ICMP_UNREACH_HOST_PRECEDENCE
block|,
literal|"host-precedence"
block|}
block|,
block|{
name|ICMP_UNREACH_PRECEDENCE_CUTOFF
block|,
literal|"precedence-cutoff"
block|}
block|,
block|{
literal|0
block|,
name|NULL
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * structure to hold flag names and associated values to be  * set in the appropriate masks.  * The last element has value=0 and the string is the error message.  */
end_comment

begin_struct
struct|struct
name|_flaglist
block|{
name|char
modifier|*
name|name
decl_stmt|;
name|u_char
name|value
decl_stmt|;
block|}
struct|;
end_struct

begin_decl_stmt
specifier|static
name|struct
name|_flaglist
name|f_tcpflags
index|[]
init|=
block|{
block|{
literal|"syn"
block|,
name|TH_SYN
block|}
block|,
block|{
literal|"fin"
block|,
name|TH_FIN
block|}
block|,
block|{
literal|"ack"
block|,
name|TH_ACK
block|}
block|,
block|{
literal|"psh"
block|,
name|TH_PUSH
block|}
block|,
block|{
literal|"rst"
block|,
name|TH_RST
block|}
block|,
block|{
literal|"urg"
block|,
name|TH_URG
block|}
block|,
block|{
literal|"tcp flag"
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|_flaglist
name|f_tcpopts
index|[]
init|=
block|{
block|{
literal|"mss"
block|,
name|IP_FW_TCPOPT_MSS
block|}
block|,
block|{
literal|"window"
block|,
name|IP_FW_TCPOPT_WINDOW
block|}
block|,
block|{
literal|"sack"
block|,
name|IP_FW_TCPOPT_SACK
block|}
block|,
block|{
literal|"ts"
block|,
name|IP_FW_TCPOPT_TS
block|}
block|,
block|{
literal|"cc"
block|,
name|IP_FW_TCPOPT_CC
block|}
block|,
block|{
literal|"tcp option"
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|_flaglist
name|f_ipopts
index|[]
init|=
block|{
block|{
literal|"ssrr"
block|,
name|IP_FW_IPOPT_SSRR
block|}
block|,
block|{
literal|"lsrr"
block|,
name|IP_FW_IPOPT_LSRR
block|}
block|,
block|{
literal|"rr"
block|,
name|IP_FW_IPOPT_RR
block|}
block|,
block|{
literal|"ts"
block|,
name|IP_FW_IPOPT_TS
block|}
block|,
block|{
literal|"ip option"
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|_flaglist
name|f_iptos
index|[]
init|=
block|{
block|{
literal|"lowdelay"
block|,
name|IPTOS_LOWDELAY
block|}
block|,
block|{
literal|"throughput"
block|,
name|IPTOS_THROUGHPUT
block|}
block|,
block|{
literal|"reliability"
block|,
name|IPTOS_RELIABILITY
block|}
block|,
block|{
literal|"mincost"
block|,
name|IPTOS_MINCOST
block|}
block|,
block|{
literal|"congestion"
block|,
name|IPTOS_CE
block|}
block|,
if|#
directive|if
literal|0
comment|/* conflicting */
block|{ "ecntransport", IPTOS_ECT},
endif|#
directive|endif
block|{
literal|"ip tos option"
block|,
literal|0
block|}
block|, }
decl_stmt|;
end_decl_stmt

begin_comment
comment|/**  * _s_x holds a string-int pair for various lookups. Same as _flaglist.  */
end_comment

begin_struct
struct|struct
name|_s_x
block|{
name|char
modifier|*
name|s
decl_stmt|;
name|int
name|x
decl_stmt|;
block|}
struct|;
end_struct

begin_decl_stmt
specifier|static
name|struct
name|_s_x
name|limit_masks
index|[]
init|=
block|{
block|{
literal|"src-addr"
block|,
name|DYN_SRC_ADDR
block|}
block|,
block|{
literal|"src-port"
block|,
name|DYN_SRC_PORT
block|}
block|,
block|{
literal|"dst-addr"
block|,
name|DYN_DST_ADDR
block|}
block|,
block|{
literal|"dst-port"
block|,
name|DYN_DST_PORT
block|}
block|,
block|{
name|NULL
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|_s_x
name|ether_types
index|[]
init|=
block|{
comment|/*      * Note, we cannot use "-:&/" in the names because they are field      * separators in the type specifications. Also, we use s = NULL as      * end-delimiter, because a type of 0 can be legal.      */
block|{
literal|"ip"
block|,
literal|0x0800
block|}
block|,
block|{
literal|"ipv4"
block|,
literal|0x0800
block|}
block|,
block|{
literal|"ipv6"
block|,
literal|0x86dd
block|}
block|,
block|{
literal|"arp"
block|,
literal|0x0806
block|}
block|,
block|{
literal|"rarp"
block|,
literal|0x8035
block|}
block|,
block|{
literal|"vlan"
block|,
literal|0x8100
block|}
block|,
block|{
literal|"loop"
block|,
literal|0x9000
block|}
block|,
block|{
literal|"trail"
block|,
literal|0x1000
block|}
block|,
block|{
literal|"at"
block|,
literal|0x809b
block|}
block|,
block|{
literal|"atalk"
block|,
literal|0x809b
block|}
block|,
block|{
literal|"aarp"
block|,
literal|0x80f3
block|}
block|,
block|{
literal|"pppoe_disc"
block|,
literal|0x8863
block|}
block|,
block|{
literal|"pppoe_sess"
block|,
literal|0x8864
block|}
block|,
block|{
literal|"ipx_8022"
block|,
literal|0x00E0
block|}
block|,
block|{
literal|"ipx_8023"
block|,
literal|0x0000
block|}
block|,
block|{
literal|"ipx_ii"
block|,
literal|0x8137
block|}
block|,
block|{
literal|"ipx_snap"
block|,
literal|0x8137
block|}
block|,
block|{
literal|"ipx"
block|,
literal|0x8137
block|}
block|,
block|{
literal|"ns"
block|,
literal|0x0600
block|}
block|,
block|{
name|NULL
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_function_decl
specifier|static
name|void
name|show_usage
parameter_list|(
name|void
parameter_list|)
function_decl|;
end_function_decl

begin_comment
comment|/*  * print the arrays of ports. The first two entries can be  * a range (a-b) or a port:mask pair, and they are processed  * accordingly if one of the two arguments range,mask is set.  * (they are not supposed to be both set).  */
end_comment

begin_function
specifier|static
name|void
name|print_ports
parameter_list|(
name|u_char
name|prot
parameter_list|,
name|int
name|n
parameter_list|,
name|u_short
modifier|*
name|ports
parameter_list|,
name|int
name|range
parameter_list|,
name|int
name|mask
parameter_list|)
block|{
name|int
name|i
init|=
literal|0
decl_stmt|;
name|char
name|comma
init|=
literal|' '
decl_stmt|;
if|if
condition|(
name|mask
condition|)
block|{
name|printf
argument_list|(
literal|" %04x:%04x"
argument_list|,
name|ports
index|[
literal|0
index|]
argument_list|,
name|ports
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|i
operator|=
literal|2
expr_stmt|;
name|comma
operator|=
literal|','
expr_stmt|;
block|}
for|for
control|(
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|servent
modifier|*
name|se
init|=
name|NULL
decl_stmt|;
if|if
condition|(
name|do_resolv
condition|)
block|{
name|struct
name|protoent
modifier|*
name|pe
init|=
name|getprotobynumber
argument_list|(
name|prot
argument_list|)
decl_stmt|;
name|se
operator|=
name|getservbyport
argument_list|(
name|htons
argument_list|(
name|ports
index|[
name|i
index|]
argument_list|)
argument_list|,
name|pe
condition|?
name|pe
operator|->
name|p_name
else|:
name|NULL
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|se
condition|)
name|printf
argument_list|(
literal|"%c%s"
argument_list|,
name|comma
argument_list|,
name|se
operator|->
name|s_name
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"%c%d"
argument_list|,
name|comma
argument_list|,
name|ports
index|[
name|i
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|==
literal|0
operator|&&
name|range
condition|)
name|comma
operator|=
literal|'-'
expr_stmt|;
else|else
name|comma
operator|=
literal|','
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|print_iface
parameter_list|(
name|char
modifier|*
name|key
parameter_list|,
name|union
name|ip_fw_if
modifier|*
name|un
parameter_list|,
name|int
name|byname
parameter_list|)
block|{
name|char
name|ifnb
index|[
name|FW_IFNLEN
operator|+
literal|1
index|]
decl_stmt|;
if|if
condition|(
name|byname
condition|)
block|{
name|strncpy
argument_list|(
name|ifnb
argument_list|,
name|un
operator|->
name|fu_via_if
operator|.
name|name
argument_list|,
name|FW_IFNLEN
argument_list|)
expr_stmt|;
name|ifnb
index|[
name|FW_IFNLEN
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|un
operator|->
name|fu_via_if
operator|.
name|unit
operator|==
operator|-
literal|1
condition|)
name|printf
argument_list|(
literal|" %s %s*"
argument_list|,
name|key
argument_list|,
name|ifnb
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|" %s %s%d"
argument_list|,
name|key
argument_list|,
name|ifnb
argument_list|,
name|un
operator|->
name|fu_via_if
operator|.
name|unit
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|un
operator|->
name|fu_via_ip
operator|.
name|s_addr
operator|!=
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|" %s %s"
argument_list|,
name|key
argument_list|,
name|inet_ntoa
argument_list|(
name|un
operator|->
name|fu_via_ip
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
name|printf
argument_list|(
literal|" %s any"
argument_list|,
name|key
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|print_reject_code
parameter_list|(
name|int
name|code
parameter_list|)
block|{
name|struct
name|icmpcode
modifier|*
name|ic
decl_stmt|;
for|for
control|(
name|ic
operator|=
name|icmpcodes
init|;
name|ic
operator|->
name|str
condition|;
name|ic
operator|++
control|)
if|if
condition|(
name|ic
operator|->
name|code
operator|==
name|code
condition|)
block|{
name|printf
argument_list|(
literal|"%s"
argument_list|,
name|ic
operator|->
name|str
argument_list|)
expr_stmt|;
return|return;
block|}
name|printf
argument_list|(
literal|"%u"
argument_list|,
name|code
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Returns the number of bits set (from left) in a contiguous bitmask,  * or -1 if the mask is not contiguous.  * This effectively works on masks in big-endian (network) format.  * First bit is bit 7 of the first byte -- note, for MAC addresses,  * the first bit on the wire is bit 0 of the first byte.  * len is the max length in bits.  */
end_comment

begin_function
specifier|static
name|int
name|contigmask
parameter_list|(
name|u_char
modifier|*
name|p
parameter_list|,
name|int
name|len
parameter_list|)
block|{
name|int
name|i
decl_stmt|,
name|n
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|len
condition|;
name|i
operator|++
control|)
if|if
condition|(
operator|(
name|p
index|[
name|i
operator|/
literal|8
index|]
operator|&
operator|(
literal|1
operator|<<
operator|(
literal|7
operator|-
operator|(
name|i
operator|%
literal|8
operator|)
operator|)
operator|)
operator|)
operator|==
literal|0
condition|)
comment|/* first bit unset */
break|break;
for|for
control|(
name|n
operator|=
name|i
operator|+
literal|1
init|;
name|n
operator|<
name|len
condition|;
name|n
operator|++
control|)
if|if
condition|(
operator|(
name|p
index|[
name|n
operator|/
literal|8
index|]
operator|&
operator|(
literal|1
operator|<<
operator|(
literal|7
operator|-
operator|(
name|n
operator|%
literal|8
operator|)
operator|)
operator|)
operator|)
operator|!=
literal|0
condition|)
comment|/* mask is not contiguous */
return|return
operator|-
literal|1
return|;
return|return
name|i
return|;
block|}
end_function

begin_comment
comment|/*  * print options set/clear in the two bitmasks passed as parameters.  */
end_comment

begin_function
specifier|static
name|void
name|printopts
parameter_list|(
name|char
modifier|*
name|name
parameter_list|,
name|u_char
name|set
parameter_list|,
name|u_char
name|clear
parameter_list|,
name|struct
name|_flaglist
modifier|*
name|list
parameter_list|)
block|{
name|char
modifier|*
name|comma
init|=
literal|""
decl_stmt|;
name|int
name|i
decl_stmt|;
name|printf
argument_list|(
literal|" %s "
argument_list|,
name|name
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|list
index|[
name|i
index|]
operator|.
name|value
operator|!=
literal|0
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|set
operator|&
name|list
index|[
name|i
index|]
operator|.
name|value
condition|)
block|{
name|printf
argument_list|(
literal|"%s%s"
argument_list|,
name|comma
argument_list|,
name|list
index|[
name|i
index|]
operator|.
name|name
argument_list|)
expr_stmt|;
name|comma
operator|=
literal|","
expr_stmt|;
block|}
if|if
condition|(
name|clear
operator|&
name|list
index|[
name|i
index|]
operator|.
name|value
condition|)
block|{
name|printf
argument_list|(
literal|"%s!%s"
argument_list|,
name|comma
argument_list|,
name|list
index|[
name|i
index|]
operator|.
name|name
argument_list|)
expr_stmt|;
name|comma
operator|=
literal|","
expr_stmt|;
block|}
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|print_ip
parameter_list|(
name|struct
name|in_addr
name|addr
parameter_list|,
name|struct
name|in_addr
name|mask
parameter_list|)
block|{
name|struct
name|hostent
modifier|*
name|he
init|=
name|NULL
decl_stmt|;
name|int
name|mb
init|=
name|contigmask
argument_list|(
operator|(
name|u_char
operator|*
operator|)
operator|&
name|mask
argument_list|,
literal|32
argument_list|)
decl_stmt|;
if|if
condition|(
name|mb
operator|==
literal|32
operator|&&
name|do_resolv
condition|)
name|he
operator|=
name|gethostbyaddr
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
operator|(
name|addr
operator|.
name|s_addr
operator|)
argument_list|,
sizeof|sizeof
argument_list|(
name|u_long
argument_list|)
argument_list|,
name|AF_INET
argument_list|)
expr_stmt|;
if|if
condition|(
name|he
operator|!=
name|NULL
condition|)
comment|/* resolved to name */
name|printf
argument_list|(
literal|"%s"
argument_list|,
name|he
operator|->
name|h_name
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|mb
operator|==
literal|0
condition|)
comment|/* any */
name|printf
argument_list|(
literal|"any"
argument_list|)
expr_stmt|;
else|else
block|{
comment|/* numeric IP followed by some kind of mask */
name|printf
argument_list|(
literal|"%s"
argument_list|,
name|inet_ntoa
argument_list|(
name|addr
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|mb
operator|<
literal|0
condition|)
name|printf
argument_list|(
literal|":%s"
argument_list|,
name|inet_ntoa
argument_list|(
name|mask
argument_list|)
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|mb
operator|<
literal|32
condition|)
name|printf
argument_list|(
literal|"/%d"
argument_list|,
name|mb
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * prints a MAC address/mask pair  */
end_comment

begin_function
specifier|static
name|void
name|print_mac
parameter_list|(
name|u_char
modifier|*
name|addr
parameter_list|,
name|u_char
modifier|*
name|mask
parameter_list|)
block|{
name|int
name|l
init|=
name|contigmask
argument_list|(
name|mask
argument_list|,
literal|48
argument_list|)
decl_stmt|;
if|if
condition|(
name|l
operator|==
literal|0
condition|)
name|printf
argument_list|(
literal|" any"
argument_list|)
expr_stmt|;
else|else
block|{
name|printf
argument_list|(
literal|" %02x:%02x:%02x:%02x:%02x:%02x"
argument_list|,
name|addr
index|[
literal|0
index|]
argument_list|,
name|addr
index|[
literal|1
index|]
argument_list|,
name|addr
index|[
literal|2
index|]
argument_list|,
name|addr
index|[
literal|3
index|]
argument_list|,
name|addr
index|[
literal|4
index|]
argument_list|,
name|addr
index|[
literal|5
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|l
operator|==
operator|-
literal|1
condition|)
name|printf
argument_list|(
literal|"&%02x:%02x:%02x:%02x:%02x:%02x"
argument_list|,
name|mask
index|[
literal|0
index|]
argument_list|,
name|mask
index|[
literal|1
index|]
argument_list|,
name|mask
index|[
literal|2
index|]
argument_list|,
name|mask
index|[
literal|3
index|]
argument_list|,
name|mask
index|[
literal|4
index|]
argument_list|,
name|mask
index|[
literal|5
index|]
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|l
operator|<
literal|48
condition|)
name|printf
argument_list|(
literal|"/%d"
argument_list|,
name|l
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|show_ipfw
parameter_list|(
name|struct
name|ip_fw
modifier|*
name|chain
parameter_list|)
block|{
name|struct
name|protoent
modifier|*
name|pe
decl_stmt|;
name|int
name|nsp
init|=
name|IP_FW_GETNSRCP
argument_list|(
name|chain
argument_list|)
decl_stmt|;
name|int
name|ndp
init|=
name|IP_FW_GETNDSTP
argument_list|(
name|chain
argument_list|)
decl_stmt|;
if|if
condition|(
name|do_resolv
condition|)
name|setservent
argument_list|(
literal|1
comment|/*stay open*/
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%05u "
argument_list|,
name|chain
operator|->
name|fw_number
argument_list|)
expr_stmt|;
if|if
condition|(
name|do_acct
condition|)
name|printf
argument_list|(
literal|"%10qu %10qu "
argument_list|,
name|chain
operator|->
name|fw_pcnt
argument_list|,
name|chain
operator|->
name|fw_bcnt
argument_list|)
expr_stmt|;
if|if
condition|(
name|do_time
condition|)
block|{
if|if
condition|(
name|chain
operator|->
name|timestamp
condition|)
block|{
name|char
name|timestr
index|[
literal|30
index|]
decl_stmt|;
name|time_t
name|t
init|=
name|_long_to_time
argument_list|(
name|chain
operator|->
name|timestamp
argument_list|)
decl_stmt|;
name|strcpy
argument_list|(
name|timestr
argument_list|,
name|ctime
argument_list|(
operator|&
name|t
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|strchr
argument_list|(
name|timestr
argument_list|,
literal|'\n'
argument_list|)
operator|=
literal|'\0'
expr_stmt|;
name|printf
argument_list|(
literal|"%s "
argument_list|,
name|timestr
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|printf
argument_list|(
literal|"			 "
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|==
name|IP_FW_F_CHECK_S
condition|)
block|{
name|printf
argument_list|(
literal|"check-state\n"
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_RND_MATCH
condition|)
block|{
name|double
name|d
init|=
literal|1.0
operator|*
name|chain
operator|->
name|dont_match_prob
decl_stmt|;
name|d
operator|=
literal|1
operator|-
operator|(
name|d
operator|/
literal|0x7fffffff
operator|)
expr_stmt|;
name|printf
argument_list|(
literal|"prob %f "
argument_list|,
name|d
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_COMMAND
condition|)
block|{
case|case
name|IP_FW_F_ACCEPT
case|:
name|printf
argument_list|(
literal|"allow"
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_DENY
case|:
name|printf
argument_list|(
literal|"deny"
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_COUNT
case|:
name|printf
argument_list|(
literal|"count"
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_DIVERT
case|:
name|printf
argument_list|(
literal|"divert %u"
argument_list|,
name|chain
operator|->
name|fw_divert_port
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_TEE
case|:
name|printf
argument_list|(
literal|"tee %u"
argument_list|,
name|chain
operator|->
name|fw_divert_port
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_SKIPTO
case|:
name|printf
argument_list|(
literal|"skipto %u"
argument_list|,
name|chain
operator|->
name|fw_skipto_rule
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_PIPE
case|:
name|printf
argument_list|(
literal|"pipe %u"
argument_list|,
name|chain
operator|->
name|fw_skipto_rule
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_QUEUE
case|:
name|printf
argument_list|(
literal|"queue %u"
argument_list|,
name|chain
operator|->
name|fw_skipto_rule
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_FW_F_REJECT
case|:
if|if
condition|(
name|chain
operator|->
name|fw_reject_code
operator|==
name|IP_FW_REJECT_RST
condition|)
name|printf
argument_list|(
literal|"reset"
argument_list|)
expr_stmt|;
else|else
block|{
name|printf
argument_list|(
literal|"unreach "
argument_list|)
expr_stmt|;
name|print_reject_code
argument_list|(
name|chain
operator|->
name|fw_reject_code
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|IP_FW_F_FWD
case|:
name|printf
argument_list|(
literal|"fwd %s"
argument_list|,
name|inet_ntoa
argument_list|(
name|chain
operator|->
name|fw_fwd_ip
operator|.
name|sin_addr
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_fwd_ip
operator|.
name|sin_port
condition|)
name|printf
argument_list|(
literal|",%d"
argument_list|,
name|chain
operator|->
name|fw_fwd_ip
operator|.
name|sin_port
argument_list|)
expr_stmt|;
break|break;
default|default:
name|errx
argument_list|(
name|EX_OSERR
argument_list|,
literal|"impossible"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_PRN
condition|)
block|{
name|printf
argument_list|(
literal|" log"
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_logamount
condition|)
name|printf
argument_list|(
literal|" logamount %d"
argument_list|,
name|chain
operator|->
name|fw_logamount
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_MAC
condition|)
block|{
name|u_char
modifier|*
name|addr
decl_stmt|,
modifier|*
name|mask
decl_stmt|;
name|u_short
modifier|*
name|type
decl_stmt|,
modifier|*
name|typemask
decl_stmt|;
name|int
name|l
decl_stmt|;
name|printf
argument_list|(
literal|" MAC"
argument_list|)
expr_stmt|;
name|addr
operator|=
operator|(
name|u_char
operator|*
operator|)
operator|&
operator|(
name|chain
operator|->
name|fw_mac_hdr
operator|)
expr_stmt|;
name|mask
operator|=
operator|(
name|u_char
operator|*
operator|)
operator|&
operator|(
name|chain
operator|->
name|fw_mac_mask
operator|)
expr_stmt|;
name|print_mac
argument_list|(
name|addr
argument_list|,
name|mask
argument_list|)
expr_stmt|;
comment|/* destination */
name|addr
operator|+=
literal|6
expr_stmt|;
name|mask
operator|+=
literal|6
expr_stmt|;
name|print_mac
argument_list|(
name|addr
argument_list|,
name|mask
argument_list|)
expr_stmt|;
comment|/* source */
name|type
operator|=
operator|(
name|u_short
operator|*
operator|)
operator|&
operator|(
name|chain
operator|->
name|fw_mac_type
operator|)
expr_stmt|;
name|typemask
operator|=
operator|(
name|u_short
operator|*
operator|)
operator|&
operator|(
name|chain
operator|->
name|fw_mac_mask_type
operator|)
expr_stmt|;
comment|/* type is in net format for all cases but range */
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_SRNG
condition|)
name|printf
argument_list|(
literal|" %04x-%04x"
argument_list|,
operator|*
name|type
argument_list|,
operator|*
name|typemask
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|ntohs
argument_list|(
operator|*
name|typemask
argument_list|)
operator|==
literal|0
condition|)
name|printf
argument_list|(
literal|" any"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|ntohs
argument_list|(
operator|*
name|typemask
argument_list|)
operator|!=
literal|0xffff
condition|)
name|printf
argument_list|(
literal|" %04x&%04x"
argument_list|,
name|ntohs
argument_list|(
operator|*
name|type
argument_list|)
argument_list|,
name|ntohs
argument_list|(
operator|*
name|typemask
argument_list|)
argument_list|)
expr_stmt|;
else|else
block|{
name|struct
name|_s_x
modifier|*
name|p
init|=
name|NULL
decl_stmt|;
name|u_int16_t
name|i
init|=
name|ntohs
argument_list|(
operator|*
name|type
argument_list|)
decl_stmt|;
if|if
condition|(
name|do_resolv
condition|)
for|for
control|(
name|p
operator|=
name|ether_types
init|;
name|p
operator|->
name|s
operator|!=
name|NULL
condition|;
name|p
operator|++
control|)
if|if
condition|(
name|p
operator|->
name|x
operator|==
name|i
condition|)
break|break;
if|if
condition|(
name|p
operator|&&
name|p
operator|->
name|s
operator|!=
name|NULL
condition|)
name|printf
argument_list|(
literal|" %s"
argument_list|,
name|p
operator|->
name|s
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|" %04x"
argument_list|,
name|i
argument_list|)
expr_stmt|;
block|}
goto|goto
name|do_options
goto|;
block|}
name|pe
operator|=
name|getprotobynumber
argument_list|(
name|chain
operator|->
name|fw_prot
argument_list|)
expr_stmt|;
if|if
condition|(
name|pe
condition|)
name|printf
argument_list|(
literal|" %s"
argument_list|,
name|pe
operator|->
name|p_name
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|" %u"
argument_list|,
name|chain
operator|->
name|fw_prot
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|" from %s"
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_INVSRC
condition|?
literal|"not "
else|:
literal|""
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_SME
condition|)
name|printf
argument_list|(
literal|"me"
argument_list|)
expr_stmt|;
else|else
name|print_ip
argument_list|(
name|chain
operator|->
name|fw_src
argument_list|,
name|chain
operator|->
name|fw_smsk
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_prot
operator|==
name|IPPROTO_TCP
operator|||
name|chain
operator|->
name|fw_prot
operator|==
name|IPPROTO_UDP
condition|)
name|print_ports
argument_list|(
name|chain
operator|->
name|fw_prot
argument_list|,
name|nsp
argument_list|,
name|chain
operator|->
name|fw_uar
operator|.
name|fw_pts
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_SRNG
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_SMSK
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|" to %s"
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_INVDST
condition|?
literal|"not "
else|:
literal|""
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_DME
condition|)
name|printf
argument_list|(
literal|"me"
argument_list|)
expr_stmt|;
else|else
name|print_ip
argument_list|(
name|chain
operator|->
name|fw_dst
argument_list|,
name|chain
operator|->
name|fw_dmsk
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_prot
operator|==
name|IPPROTO_TCP
operator|||
name|chain
operator|->
name|fw_prot
operator|==
name|IPPROTO_UDP
condition|)
name|print_ports
argument_list|(
name|chain
operator|->
name|fw_prot
argument_list|,
name|ndp
argument_list|,
name|chain
operator|->
name|fw_uar
operator|.
name|fw_pts
operator|+
name|nsp
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_DRNG
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_DMSK
argument_list|)
expr_stmt|;
name|do_options
label|:
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_UID
condition|)
block|{
name|struct
name|passwd
modifier|*
name|pwd
init|=
name|getpwuid
argument_list|(
name|chain
operator|->
name|fw_uid
argument_list|)
decl_stmt|;
if|if
condition|(
name|pwd
condition|)
name|printf
argument_list|(
literal|" uid %s"
argument_list|,
name|pwd
operator|->
name|pw_name
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|" uid %u"
argument_list|,
name|chain
operator|->
name|fw_uid
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_GID
condition|)
block|{
name|struct
name|group
modifier|*
name|grp
init|=
name|getgrgid
argument_list|(
name|chain
operator|->
name|fw_gid
argument_list|)
decl_stmt|;
if|if
condition|(
name|grp
condition|)
name|printf
argument_list|(
literal|" gid %s"
argument_list|,
name|grp
operator|->
name|gr_name
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|" gid %u"
argument_list|,
name|chain
operator|->
name|fw_gid
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_KEEP_S
condition|)
block|{
name|struct
name|_s_x
modifier|*
name|p
init|=
name|limit_masks
decl_stmt|;
switch|switch
condition|(
name|chain
operator|->
name|dyn_type
condition|)
block|{
default|default:
name|printf
argument_list|(
literal|" *** unknown type ***"
argument_list|)
expr_stmt|;
break|break ;
case|case
name|DYN_KEEP_STATE
case|:
name|printf
argument_list|(
literal|" keep-state"
argument_list|)
expr_stmt|;
break|break;
case|case
name|DYN_LIMIT
case|:
name|printf
argument_list|(
literal|" limit"
argument_list|)
expr_stmt|;
for|for
control|(
init|;
name|p
operator|->
name|x
operator|!=
literal|0
condition|;
name|p
operator|++
control|)
if|if
condition|(
name|chain
operator|->
name|limit_mask
operator|&
name|p
operator|->
name|x
condition|)
name|printf
argument_list|(
literal|" %s"
argument_list|,
name|p
operator|->
name|s
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|" %d"
argument_list|,
name|chain
operator|->
name|conn_limit
argument_list|)
expr_stmt|;
break|break ;
block|}
block|}
comment|/* Direction */
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_BRIDGED
condition|)
name|printf
argument_list|(
literal|" bridged"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_IN
operator|)
operator|&&
operator|!
operator|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_OUT
operator|)
condition|)
name|printf
argument_list|(
literal|" in"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_IN
operator|)
operator|&&
operator|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_OUT
operator|)
condition|)
name|printf
argument_list|(
literal|" out"
argument_list|)
expr_stmt|;
comment|/* Handle hack for "via" backwards compatibility */
if|if
condition|(
operator|(
name|chain
operator|->
name|fw_flg
operator|&
name|IF_FW_F_VIAHACK
operator|)
operator|==
name|IF_FW_F_VIAHACK
condition|)
block|{
name|print_iface
argument_list|(
literal|"via"
argument_list|,
operator|&
name|chain
operator|->
name|fw_in_if
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_IIFNAME
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/* Receive interface specified */
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_IIFACE
condition|)
name|print_iface
argument_list|(
literal|"recv"
argument_list|,
operator|&
name|chain
operator|->
name|fw_in_if
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_IIFNAME
argument_list|)
expr_stmt|;
comment|/* Transmit interface specified */
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_OIFACE
condition|)
name|print_iface
argument_list|(
literal|"xmit"
argument_list|,
operator|&
name|chain
operator|->
name|fw_out_if
argument_list|,
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_OIFNAME
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_FRAG
condition|)
name|printf
argument_list|(
literal|" frag"
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_IPOPT
condition|)
name|printopts
argument_list|(
literal|"ipopt"
argument_list|,
name|chain
operator|->
name|fw_ipopt
argument_list|,
name|chain
operator|->
name|fw_ipnopt
argument_list|,
name|f_ipopts
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_IPLEN
condition|)
name|printf
argument_list|(
literal|" iplen %u"
argument_list|,
name|chain
operator|->
name|fw_iplen
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_IPID
condition|)
name|printf
argument_list|(
literal|" ipid %#x"
argument_list|,
name|chain
operator|->
name|fw_ipid
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_IPPRE
condition|)
name|printf
argument_list|(
literal|" ipprecedence %u"
argument_list|,
operator|(
name|chain
operator|->
name|fw_iptos
operator|&
literal|0xe0
operator|)
operator|>>
literal|5
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_IPTOS
condition|)
name|printopts
argument_list|(
literal|"iptos"
argument_list|,
name|chain
operator|->
name|fw_iptos
argument_list|,
name|chain
operator|->
name|fw_ipntos
argument_list|,
name|f_iptos
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_IPTTL
condition|)
name|printf
argument_list|(
literal|" ipttl %u"
argument_list|,
name|chain
operator|->
name|fw_ipttl
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_IPVER
condition|)
name|printf
argument_list|(
literal|" ipversion %u"
argument_list|,
name|chain
operator|->
name|fw_ipver
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_TCPEST
condition|)
name|printf
argument_list|(
literal|" established"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|chain
operator|->
name|fw_tcpf
operator|==
name|TH_SYN
operator|&&
name|chain
operator|->
name|fw_tcpnf
operator|==
name|TH_ACK
condition|)
name|printf
argument_list|(
literal|" setup"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_TCPFLG
condition|)
name|printopts
argument_list|(
literal|"tcpflags"
argument_list|,
name|chain
operator|->
name|fw_tcpf
argument_list|,
name|chain
operator|->
name|fw_tcpnf
argument_list|,
name|f_tcpflags
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_TCPOPT
condition|)
name|printopts
argument_list|(
literal|"tcpoptions"
argument_list|,
name|chain
operator|->
name|fw_tcpopt
argument_list|,
name|chain
operator|->
name|fw_tcpnopt
argument_list|,
name|f_tcpopts
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_TCPSEQ
condition|)
name|printf
argument_list|(
literal|" tcpseq %lu"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|chain
operator|->
name|fw_tcpseq
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_TCPACK
condition|)
name|printf
argument_list|(
literal|" tcpack %lu"
argument_list|,
operator|(
name|u_long
operator|)
name|ntohl
argument_list|(
name|chain
operator|->
name|fw_tcpack
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_ipflg
operator|&
name|IP_FW_IF_TCPWIN
condition|)
name|printf
argument_list|(
literal|" tcpwin %hu"
argument_list|,
name|ntohs
argument_list|(
name|chain
operator|->
name|fw_tcpwin
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|chain
operator|->
name|fw_flg
operator|&
name|IP_FW_F_ICMPBIT
condition|)
block|{
name|int
name|i
decl_stmt|,
name|first
init|=
literal|1
decl_stmt|;
name|unsigned
name|j
decl_stmt|;
name|printf
argument_list|(
literal|" icmptype"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|IP_FW_ICMPTYPES_DIM
condition|;
operator|++
name|i
control|)
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
sizeof|sizeof
argument_list|(
name|unsigned
argument_list|)
operator|*
literal|8
condition|;
operator|++
name|j
control|)
if|if
condition|(
name|chain
operator|->
name|fw_uar
operator|.
name|fw_icmptypes
index|[
name|i
index|]
operator|&
operator|(
literal|1
operator|<<
name|j
operator|)
condition|)
block|{
name|printf
argument_list|(
literal|"%c%d"
argument_list|,
name|first
condition|?
literal|' '
else|:
literal|','
argument_list|,
name|i
operator|*
sizeof|sizeof
argument_list|(
name|unsigned
argument_list|)
operator|*
literal|8
operator|+
name|j
argument_list|)
expr_stmt|;
name|first
operator|=
literal|0
expr_stmt|;
block|}
block|}
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|done
label|:
if|if
condition|(
name|do_resolv
condition|)
name|endservent
argument_list|()
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|show_dyn_ipfw
parameter_list|(
name|struct
name|ipfw_dyn_rule
modifier|*
name|d
parameter_list|)
block|{
name|struct
name|protoent
modifier|*
name|pe
decl_stmt|;
name|struct
name|in_addr
name|a
decl_stmt|;
if|if
condition|(
operator|!
name|d
operator|->
name|expire
operator|&&
operator|!
name|do_expired
condition|)
return|return;
name|printf
argument_list|(
literal|"%05d %qu %qu (T %ds, slot %d)"
argument_list|,
call|(
name|int
call|)
argument_list|(
name|d
operator|->
name|rule
argument_list|)
argument_list|,
name|d
operator|->
name|pcnt
argument_list|,
name|d
operator|->
name|bcnt
argument_list|,
name|d
operator|->
name|expire
argument_list|,
name|d
operator|->
name|bucket
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|d
operator|->
name|dyn_type
condition|)
block|{
case|case
name|DYN_LIMIT_PARENT
case|:
name|printf
argument_list|(
literal|" PARENT %d"
argument_list|,
name|d
operator|->
name|count
argument_list|)
expr_stmt|;
break|break;
case|case
name|DYN_LIMIT
case|:
name|printf
argument_list|(
literal|" LIMIT"
argument_list|)
expr_stmt|;
break|break;
case|case
name|DYN_KEEP_STATE
case|:
comment|/* bidir, no mask */
name|printf
argument_list|(
literal|"<->"
argument_list|)
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|do_resolv
operator|&&
operator|(
name|pe
operator|=
name|getprotobynumber
argument_list|(
name|d
operator|->
name|id
operator|.
name|proto
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
name|printf
argument_list|(
literal|" %s,"
argument_list|,
name|pe
operator|->
name|p_name
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|" %u,"
argument_list|,
name|d
operator|->
name|id
operator|.
name|proto
argument_list|)
expr_stmt|;
name|a
operator|.
name|s_addr
operator|=
name|htonl
argument_list|(
name|d
operator|->
name|id
operator|.
name|src_ip
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|" %s %d"
argument_list|,
name|inet_ntoa
argument_list|(
name|a
argument_list|)
argument_list|,
name|d
operator|->
name|id
operator|.
name|src_port
argument_list|)
expr_stmt|;
name|a
operator|.
name|s_addr
operator|=
name|htonl
argument_list|(
name|d
operator|->
name|id
operator|.
name|dst_ip
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"<-> %s %d"
argument_list|,
name|inet_ntoa
argument_list|(
name|a
argument_list|)
argument_list|,
name|d
operator|->
name|id
operator|.
name|dst_port
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|int
name|sort_q
parameter_list|(
specifier|const
name|void
modifier|*
name|pa
parameter_list|,
specifier|const
name|void
modifier|*
name|pb
parameter_list|)
block|{
name|int
name|rev
init|=
operator|(
name|do_sort
operator|<
literal|0
operator|)
decl_stmt|;
name|int
name|field
init|=
name|rev
condition|?
operator|-
name|do_sort
else|:
name|do_sort
decl_stmt|;
name|long
name|long
name|res
init|=
literal|0
decl_stmt|;
specifier|const
name|struct
name|dn_flow_queue
modifier|*
name|a
init|=
name|pa
decl_stmt|;
specifier|const
name|struct
name|dn_flow_queue
modifier|*
name|b
init|=
name|pb
decl_stmt|;
switch|switch
condition|(
name|field
condition|)
block|{
case|case
literal|1
case|:
comment|/* pkts */
name|res
operator|=
name|a
operator|->
name|len
operator|-
name|b
operator|->
name|len
expr_stmt|;
break|break;
case|case
literal|2
case|:
comment|/* bytes */
name|res
operator|=
name|a
operator|->
name|len_bytes
operator|-
name|b
operator|->
name|len_bytes
expr_stmt|;
break|break;
case|case
literal|3
case|:
comment|/* tot pkts */
name|res
operator|=
name|a
operator|->
name|tot_pkts
operator|-
name|b
operator|->
name|tot_pkts
expr_stmt|;
break|break;
case|case
literal|4
case|:
comment|/* tot bytes */
name|res
operator|=
name|a
operator|->
name|tot_bytes
operator|-
name|b
operator|->
name|tot_bytes
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|res
operator|<
literal|0
condition|)
name|res
operator|=
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|res
operator|>
literal|0
condition|)
name|res
operator|=
literal|1
expr_stmt|;
return|return
call|(
name|int
call|)
argument_list|(
name|rev
condition|?
name|res
else|:
operator|-
name|res
argument_list|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|list_queues
parameter_list|(
name|struct
name|dn_flow_set
modifier|*
name|fs
parameter_list|,
name|struct
name|dn_flow_queue
modifier|*
name|q
parameter_list|)
block|{
name|int
name|l
decl_stmt|;
name|printf
argument_list|(
literal|"    mask: 0x%02x 0x%08x/0x%04x -> 0x%08x/0x%04x\n"
argument_list|,
name|fs
operator|->
name|flow_mask
operator|.
name|proto
argument_list|,
name|fs
operator|->
name|flow_mask
operator|.
name|src_ip
argument_list|,
name|fs
operator|->
name|flow_mask
operator|.
name|src_port
argument_list|,
name|fs
operator|->
name|flow_mask
operator|.
name|dst_ip
argument_list|,
name|fs
operator|->
name|flow_mask
operator|.
name|dst_port
argument_list|)
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|rq_elements
operator|==
literal|0
condition|)
return|return;
name|printf
argument_list|(
literal|"BKT Prot ___Source IP/port____ "
literal|"____Dest. IP/port____ Tot_pkt/bytes Pkt/Byte Drp\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|do_sort
operator|!=
literal|0
condition|)
name|heapsort
argument_list|(
name|q
argument_list|,
name|fs
operator|->
name|rq_elements
argument_list|,
sizeof|sizeof
expr|*
name|q
argument_list|,
name|sort_q
argument_list|)
expr_stmt|;
for|for
control|(
name|l
operator|=
literal|0
init|;
name|l
operator|<
name|fs
operator|->
name|rq_elements
condition|;
name|l
operator|++
control|)
block|{
name|struct
name|in_addr
name|ina
decl_stmt|;
name|struct
name|protoent
modifier|*
name|pe
decl_stmt|;
name|ina
operator|.
name|s_addr
operator|=
name|htonl
argument_list|(
name|q
index|[
name|l
index|]
operator|.
name|id
operator|.
name|src_ip
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%3d "
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|hash_slot
argument_list|)
expr_stmt|;
name|pe
operator|=
name|getprotobynumber
argument_list|(
name|q
index|[
name|l
index|]
operator|.
name|id
operator|.
name|proto
argument_list|)
expr_stmt|;
if|if
condition|(
name|pe
condition|)
name|printf
argument_list|(
literal|"%-4s "
argument_list|,
name|pe
operator|->
name|p_name
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"%4u "
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|id
operator|.
name|proto
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%15s/%-5d "
argument_list|,
name|inet_ntoa
argument_list|(
name|ina
argument_list|)
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|id
operator|.
name|src_port
argument_list|)
expr_stmt|;
name|ina
operator|.
name|s_addr
operator|=
name|htonl
argument_list|(
name|q
index|[
name|l
index|]
operator|.
name|id
operator|.
name|dst_ip
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%15s/%-5d "
argument_list|,
name|inet_ntoa
argument_list|(
name|ina
argument_list|)
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|id
operator|.
name|dst_port
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%4qu %8qu %2u %4u %3u\n"
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|tot_pkts
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|tot_bytes
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|len
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|len_bytes
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|drops
argument_list|)
expr_stmt|;
if|if
condition|(
name|verbose
condition|)
name|printf
argument_list|(
literal|"   S %20qd  F %20qd\n"
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|S
argument_list|,
name|q
index|[
name|l
index|]
operator|.
name|F
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|print_flowset_parms
parameter_list|(
name|struct
name|dn_flow_set
modifier|*
name|fs
parameter_list|,
name|char
modifier|*
name|prefix
parameter_list|)
block|{
name|int
name|l
decl_stmt|;
name|char
name|qs
index|[
literal|30
index|]
decl_stmt|;
name|char
name|plr
index|[
literal|30
index|]
decl_stmt|;
name|char
name|red
index|[
literal|90
index|]
decl_stmt|;
comment|/* Display RED parameters */
name|l
operator|=
name|fs
operator|->
name|qsize
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|flags_fs
operator|&
name|DN_QSIZE_IS_BYTES
condition|)
block|{
if|if
condition|(
name|l
operator|>=
literal|8192
condition|)
name|sprintf
argument_list|(
name|qs
argument_list|,
literal|"%d KB"
argument_list|,
name|l
operator|/
literal|1024
argument_list|)
expr_stmt|;
else|else
name|sprintf
argument_list|(
name|qs
argument_list|,
literal|"%d B"
argument_list|,
name|l
argument_list|)
expr_stmt|;
block|}
else|else
name|sprintf
argument_list|(
name|qs
argument_list|,
literal|"%3d sl."
argument_list|,
name|l
argument_list|)
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|plr
condition|)
name|sprintf
argument_list|(
name|plr
argument_list|,
literal|"plr %f"
argument_list|,
literal|1.0
operator|*
name|fs
operator|->
name|plr
operator|/
call|(
name|double
call|)
argument_list|(
literal|0x7fffffff
argument_list|)
argument_list|)
expr_stmt|;
else|else
name|plr
index|[
literal|0
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|flags_fs
operator|&
name|DN_IS_RED
condition|)
comment|/* RED parameters */
name|sprintf
argument_list|(
name|red
argument_list|,
literal|"\n\t  %cRED w_q %f min_th %d max_th %d max_p %f"
argument_list|,
operator|(
name|fs
operator|->
name|flags_fs
operator|&
name|DN_IS_GENTLE_RED
operator|)
condition|?
literal|'G'
else|:
literal|' '
argument_list|,
literal|1.0
operator|*
name|fs
operator|->
name|w_q
operator|/
call|(
name|double
call|)
argument_list|(
literal|1
operator|<<
name|SCALE_RED
argument_list|)
argument_list|,
name|SCALE_VAL
argument_list|(
name|fs
operator|->
name|min_th
argument_list|)
argument_list|,
name|SCALE_VAL
argument_list|(
name|fs
operator|->
name|max_th
argument_list|)
argument_list|,
literal|1.0
operator|*
name|fs
operator|->
name|max_p
operator|/
call|(
name|double
call|)
argument_list|(
literal|1
operator|<<
name|SCALE_RED
argument_list|)
argument_list|)
expr_stmt|;
else|else
name|sprintf
argument_list|(
name|red
argument_list|,
literal|"droptail"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%s %s%s %d queues (%d buckets) %s\n"
argument_list|,
name|prefix
argument_list|,
name|qs
argument_list|,
name|plr
argument_list|,
name|fs
operator|->
name|rq_elements
argument_list|,
name|fs
operator|->
name|rq_size
argument_list|,
name|red
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|list
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
name|struct
name|ip_fw
modifier|*
name|rules
decl_stmt|,
modifier|*
name|r
decl_stmt|;
name|struct
name|ipfw_dyn_rule
modifier|*
name|dynrules
decl_stmt|,
modifier|*
name|d
decl_stmt|;
name|struct
name|dn_pipe
modifier|*
name|pipes
decl_stmt|;
name|void
modifier|*
name|data
init|=
name|NULL
decl_stmt|;
name|int
name|n
decl_stmt|,
name|nbytes
decl_stmt|,
name|nstat
decl_stmt|,
name|ndyn
decl_stmt|;
name|int
name|exitval
init|=
name|EX_OK
decl_stmt|;
name|int
name|lac
decl_stmt|;
name|char
modifier|*
modifier|*
name|lav
decl_stmt|;
name|u_long
name|rnum
decl_stmt|;
name|char
modifier|*
name|endptr
decl_stmt|;
name|int
name|seen
init|=
literal|0
decl_stmt|;
comment|/* get rules or pipes from kernel, resizing array as necessary */
block|{
specifier|const
name|int
name|unit
init|=
name|do_pipe
condition|?
sizeof|sizeof
argument_list|(
operator|*
name|pipes
argument_list|)
else|:
sizeof|sizeof
argument_list|(
operator|*
name|rules
argument_list|)
decl_stmt|;
specifier|const
name|int
name|ocmd
init|=
name|do_pipe
condition|?
name|IP_DUMMYNET_GET
else|:
name|IP_FW_GET
decl_stmt|;
name|int
name|nalloc
init|=
name|unit
decl_stmt|;
name|nbytes
operator|=
name|nalloc
expr_stmt|;
while|while
condition|(
name|nbytes
operator|>=
name|nalloc
condition|)
block|{
name|nalloc
operator|=
name|nalloc
operator|*
literal|2
operator|+
literal|200
expr_stmt|;
name|nbytes
operator|=
name|nalloc
expr_stmt|;
if|if
condition|(
operator|(
name|data
operator|=
name|realloc
argument_list|(
name|data
argument_list|,
name|nbytes
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|EX_OSERR
argument_list|,
literal|"realloc"
argument_list|)
expr_stmt|;
if|if
condition|(
name|getsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|ocmd
argument_list|,
name|data
argument_list|,
operator|&
name|nbytes
argument_list|)
operator|<
literal|0
condition|)
name|err
argument_list|(
name|EX_OSERR
argument_list|,
literal|"getsockopt(IP_%s_GET)"
argument_list|,
name|do_pipe
condition|?
literal|"DUMMYNET"
else|:
literal|"FW"
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* display requested pipes */
if|if
condition|(
name|do_pipe
condition|)
block|{
name|u_long
name|rulenum
decl_stmt|;
name|void
modifier|*
name|next
init|=
name|data
decl_stmt|;
name|struct
name|dn_pipe
modifier|*
name|p
init|=
operator|(
expr|struct
name|dn_pipe
operator|*
operator|)
name|data
decl_stmt|;
name|struct
name|dn_flow_set
modifier|*
name|fs
decl_stmt|;
name|struct
name|dn_flow_queue
modifier|*
name|q
decl_stmt|;
name|int
name|l
decl_stmt|;
if|if
condition|(
name|ac
operator|>
literal|0
condition|)
name|rulenum
operator|=
name|strtoul
argument_list|(
operator|*
name|av
operator|++
argument_list|,
name|NULL
argument_list|,
literal|10
argument_list|)
expr_stmt|;
else|else
name|rulenum
operator|=
literal|0
expr_stmt|;
for|for
control|(
init|;
name|nbytes
operator|>=
sizeof|sizeof
expr|*
name|p
condition|;
name|p
operator|=
operator|(
expr|struct
name|dn_pipe
operator|*
operator|)
name|next
control|)
block|{
name|double
name|b
init|=
name|p
operator|->
name|bandwidth
decl_stmt|;
name|char
name|buf
index|[
literal|30
index|]
decl_stmt|;
name|char
name|prefix
index|[
literal|80
index|]
decl_stmt|;
if|if
condition|(
name|p
operator|->
name|next
operator|!=
operator|(
expr|struct
name|dn_pipe
operator|*
operator|)
name|DN_IS_PIPE
condition|)
break|break;
name|l
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|p
argument_list|)
operator|+
name|p
operator|->
name|fs
operator|.
name|rq_elements
operator|*
sizeof|sizeof
argument_list|(
operator|*
name|q
argument_list|)
expr_stmt|;
name|next
operator|=
operator|(
name|void
operator|*
operator|)
name|p
operator|+
name|l
expr_stmt|;
name|nbytes
operator|-=
name|l
expr_stmt|;
name|q
operator|=
operator|(
expr|struct
name|dn_flow_queue
operator|*
operator|)
operator|(
name|p
operator|+
literal|1
operator|)
expr_stmt|;
if|if
condition|(
name|rulenum
operator|!=
literal|0
operator|&&
name|rulenum
operator|!=
name|p
operator|->
name|pipe_nr
condition|)
continue|continue;
if|if
condition|(
name|p
operator|->
name|if_name
index|[
literal|0
index|]
operator|!=
literal|'\0'
condition|)
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"%s"
argument_list|,
name|p
operator|->
name|if_name
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|b
operator|==
literal|0
condition|)
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"unlimited"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|b
operator|>=
literal|1000000
condition|)
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"%7.3f Mbit/s"
argument_list|,
name|b
operator|/
literal|1000000
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|b
operator|>=
literal|1000
condition|)
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"%7.3f Kbit/s"
argument_list|,
name|b
operator|/
literal|1000
argument_list|)
expr_stmt|;
else|else
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"%7.3f bit/s "
argument_list|,
name|b
argument_list|)
expr_stmt|;
name|sprintf
argument_list|(
name|prefix
argument_list|,
literal|"%05d: %s %4d ms "
argument_list|,
name|p
operator|->
name|pipe_nr
argument_list|,
name|buf
argument_list|,
name|p
operator|->
name|delay
argument_list|)
expr_stmt|;
name|print_flowset_parms
argument_list|(
operator|&
operator|(
name|p
operator|->
name|fs
operator|)
argument_list|,
name|prefix
argument_list|)
expr_stmt|;
if|if
condition|(
name|verbose
condition|)
name|printf
argument_list|(
literal|"   V %20qd\n"
argument_list|,
name|p
operator|->
name|V
operator|>>
name|MY_M
argument_list|)
expr_stmt|;
name|list_queues
argument_list|(
operator|&
operator|(
name|p
operator|->
name|fs
operator|)
argument_list|,
name|q
argument_list|)
expr_stmt|;
block|}
name|fs
operator|=
operator|(
expr|struct
name|dn_flow_set
operator|*
operator|)
name|next
expr_stmt|;
for|for
control|(
init|;
name|nbytes
operator|>=
sizeof|sizeof
expr|*
name|fs
condition|;
name|fs
operator|=
operator|(
expr|struct
name|dn_flow_set
operator|*
operator|)
name|next
control|)
block|{
name|char
name|prefix
index|[
literal|80
index|]
decl_stmt|;
if|if
condition|(
name|fs
operator|->
name|next
operator|!=
operator|(
expr|struct
name|dn_flow_set
operator|*
operator|)
name|DN_IS_QUEUE
condition|)
break|break;
name|l
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|fs
argument_list|)
operator|+
name|fs
operator|->
name|rq_elements
operator|*
sizeof|sizeof
argument_list|(
operator|*
name|q
argument_list|)
expr_stmt|;
name|next
operator|=
operator|(
name|void
operator|*
operator|)
name|fs
operator|+
name|l
expr_stmt|;
name|nbytes
operator|-=
name|l
expr_stmt|;
name|q
operator|=
operator|(
expr|struct
name|dn_flow_queue
operator|*
operator|)
operator|(
name|fs
operator|+
literal|1
operator|)
expr_stmt|;
name|sprintf
argument_list|(
name|prefix
argument_list|,
literal|"q%05d: weight %d pipe %d "
argument_list|,
name|fs
operator|->
name|fs_nr
argument_list|,
name|fs
operator|->
name|weight
argument_list|,
name|fs
operator|->
name|parent_nr
argument_list|)
expr_stmt|;
name|print_flowset_parms
argument_list|(
name|fs
argument_list|,
name|prefix
argument_list|)
expr_stmt|;
name|list_queues
argument_list|(
name|fs
argument_list|,
name|q
argument_list|)
expr_stmt|;
block|}
name|free
argument_list|(
name|data
argument_list|)
expr_stmt|;
return|return;
block|}
name|rules
operator|=
operator|(
expr|struct
name|ip_fw
operator|*
operator|)
name|data
expr_stmt|;
for|for
control|(
name|nstat
operator|=
literal|0
init|;
name|rules
index|[
name|nstat
index|]
operator|.
name|fw_number
operator|<
literal|65535
condition|;
operator|++
name|nstat
control|)
comment|/* nothing */
empty_stmt|;
name|nstat
operator|++
expr_stmt|;
comment|/* counting starts from 0 ... */
name|dynrules
operator|=
operator|(
expr|struct
name|ipfw_dyn_rule
operator|*
operator|)
operator|&
name|rules
index|[
name|nstat
index|]
expr_stmt|;
name|ndyn
operator|=
operator|(
name|nbytes
operator|-
operator|(
name|nstat
operator|*
sizeof|sizeof
expr|*
name|rules
operator|)
operator|)
operator|/
sizeof|sizeof
expr|*
name|dynrules
expr_stmt|;
comment|/* if no rule numbers were specified, list all rules */
if|if
condition|(
name|ac
operator|==
literal|0
condition|)
block|{
for|for
control|(
name|n
operator|=
literal|0
init|;
name|n
operator|<
name|nstat
condition|;
name|n
operator|++
control|)
name|show_ipfw
argument_list|(
operator|&
name|rules
index|[
name|n
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|do_dynamic
operator|&&
name|ndyn
condition|)
block|{
name|printf
argument_list|(
literal|"## Dynamic rules:\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|n
operator|=
literal|0
operator|,
name|d
operator|=
name|dynrules
init|;
name|n
operator|<
name|ndyn
condition|;
name|n
operator|++
operator|,
name|d
operator|++
control|)
name|show_dyn_ipfw
argument_list|(
name|d
argument_list|)
expr_stmt|;
block|}
name|free
argument_list|(
name|data
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* display specific rules requested on command line */
for|for
control|(
name|lac
operator|=
name|ac
operator|,
name|lav
operator|=
name|av
init|;
name|lac
operator|!=
literal|0
condition|;
name|lac
operator|--
control|)
block|{
comment|/* convert command line rule # */
name|rnum
operator|=
name|strtoul
argument_list|(
operator|*
name|lav
operator|++
argument_list|,
operator|&
name|endptr
argument_list|,
literal|10
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|endptr
condition|)
block|{
name|exitval
operator|=
name|EX_USAGE
expr_stmt|;
name|warnx
argument_list|(
literal|"invalid rule number: %s"
argument_list|,
operator|*
operator|(
name|lav
operator|-
literal|1
operator|)
argument_list|)
expr_stmt|;
continue|continue;
block|}
for|for
control|(
name|n
operator|=
name|seen
operator|=
literal|0
operator|,
name|r
operator|=
name|rules
init|;
name|n
operator|<
name|nstat
condition|;
name|n
operator|++
operator|,
name|r
operator|++
control|)
block|{
if|if
condition|(
name|r
operator|->
name|fw_number
operator|>
name|rnum
condition|)
break|break;
if|if
condition|(
name|r
operator|->
name|fw_number
operator|==
name|rnum
condition|)
block|{
name|show_ipfw
argument_list|(
name|r
argument_list|)
expr_stmt|;
name|seen
operator|=
literal|1
expr_stmt|;
block|}
block|}
if|if
condition|(
operator|!
name|seen
condition|)
block|{
comment|/* give precedence to other error(s) */
if|if
condition|(
name|exitval
operator|==
name|EX_OK
condition|)
name|exitval
operator|=
name|EX_UNAVAILABLE
expr_stmt|;
name|warnx
argument_list|(
literal|"rule %lu does not exist"
argument_list|,
name|rnum
argument_list|)
expr_stmt|;
block|}
block|}
name|printf
argument_list|(
literal|"## Dynamic rules:\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|do_dynamic
operator|&&
name|ndyn
condition|)
block|{
for|for
control|(
name|lac
operator|=
name|ac
operator|,
name|lav
operator|=
name|av
init|;
name|lac
operator|!=
literal|0
condition|;
name|lac
operator|--
control|)
block|{
name|rnum
operator|=
name|strtoul
argument_list|(
operator|*
name|lav
operator|++
argument_list|,
operator|&
name|endptr
argument_list|,
literal|10
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|endptr
condition|)
comment|/* already warned */
continue|continue;
for|for
control|(
name|n
operator|=
literal|0
operator|,
name|d
operator|=
name|dynrules
init|;
name|n
operator|<
name|ndyn
condition|;
name|n
operator|++
operator|,
name|d
operator|++
control|)
block|{
if|if
condition|(
call|(
name|int
call|)
argument_list|(
name|d
operator|->
name|rule
argument_list|)
operator|>
name|rnum
condition|)
break|break;
if|if
condition|(
call|(
name|int
call|)
argument_list|(
name|d
operator|->
name|rule
argument_list|)
operator|==
name|rnum
condition|)
name|show_dyn_ipfw
argument_list|(
name|d
argument_list|)
expr_stmt|;
block|}
block|}
block|}
name|ac
operator|=
literal|0
expr_stmt|;
name|free
argument_list|(
name|data
argument_list|)
expr_stmt|;
if|if
condition|(
name|exitval
operator|!=
name|EX_OK
condition|)
name|exit
argument_list|(
name|exitval
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|show_usage
parameter_list|(
name|void
parameter_list|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: ipfw [options]\n"
literal|"    add [number] rule\n"
literal|"    pipe number config [pipeconfig]\n"
literal|"    queue number config [queueconfig]\n"
literal|"    [pipe] flush\n"
literal|"    [pipe] delete number ...\n"
literal|"    [pipe] {list|show} [number ...]\n"
literal|"    {zero|resetlog} [number ...]\n"
literal|"  rule: [prob<match_probability>] action (mac-hdr|ip-hdr) options...\n"
literal|"  action:\n"
literal|"      {allow|permit|accept|pass|deny|drop|reject|unreach code|\n"
literal|"	reset|count|skipto num|divert port|tee port|fwd ip|\n"
literal|"	pipe num} [log [logamount count]]\n"
literal|"    proto: {ip|tcp|udp|icmp|<number>}\n"
literal|"    src: from [not] {me|any|ip[{/bits|:mask}]} [{port[-port]}, [port], ...]\n"
literal|"    dst: to [not] {me|any|ip[{/bits|:mask}]} [{port[-port]}, [port], ...]\n"
literal|"  extras:\n"
literal|"    uid {user id}\n"
literal|"    gid {group id}\n"
literal|"    fragment	  (may not be used with ports or tcpflags)\n"
literal|"    in\n"
literal|"    out\n"
literal|"    {xmit|recv|via} {iface|ip|any}\n"
literal|"    {established|setup}\n"
literal|"    tcpflags [!]{syn|fin|rst|ack|psh|urg}, ...\n"
literal|"    ipoptions [!]{ssrr|lsrr|rr|ts}, ...\n"
literal|"    iplen {length}\n"
literal|"    ipid {identification number}\n"
literal|"    ipprecedence {precedence}\n"
literal|"    iptos [!]{lowdelay|throughput|reliability|mincost|congestion}, ...\n"
literal|"    ipttl {time to live}\n"
literal|"    ipversion {version number}\n"
literal|"    tcpoptions [!]{mss|window|sack|ts|cc}, ...\n"
literal|"    tcpseq {sequence number}\n"
literal|"    tcpack {acknowledgement number}\n"
literal|"    tcpwin {window size}\n"
literal|"    icmptypes {type[, type]}...\n"
literal|"    keep-state [method]\n"
literal|"  pipeconfig:\n"
literal|"    {bw|bandwidth}<number>{bit/s|Kbit/s|Mbit/s|Bytes/s|KBytes/s|MBytes/s}\n"
literal|"    {bw|bandwidth} interface_name\n"
literal|"    delay<milliseconds>\n"
literal|"    queue<size>{packets|Bytes|KBytes}\n"
literal|"    plr<fraction>\n"
literal|"    mask {all| [dst-ip|src-ip|dst-port|src-port|proto]<number>}\n"
literal|"    buckets<number>}\n"
literal|"    {red|gred}<fraction>/<number>/<number>/<fraction>\n"
literal|"    droptail\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
name|EX_USAGE
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|lookup_host
parameter_list|(
name|char
modifier|*
name|host
parameter_list|,
name|struct
name|in_addr
modifier|*
name|ipaddr
parameter_list|)
block|{
name|struct
name|hostent
modifier|*
name|he
decl_stmt|;
if|if
condition|(
operator|!
name|inet_aton
argument_list|(
name|host
argument_list|,
name|ipaddr
argument_list|)
condition|)
block|{
if|if
condition|(
operator|(
name|he
operator|=
name|gethostbyname
argument_list|(
name|host
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
operator|*
name|ipaddr
operator|=
operator|*
operator|(
expr|struct
name|in_addr
operator|*
operator|)
name|he
operator|->
name|h_addr_list
index|[
literal|0
index|]
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|fill_ip
parameter_list|(
name|struct
name|in_addr
modifier|*
name|ipno
parameter_list|,
name|struct
name|in_addr
modifier|*
name|mask
parameter_list|,
name|int
modifier|*
name|acp
parameter_list|,
name|char
modifier|*
modifier|*
modifier|*
name|avp
parameter_list|)
block|{
name|int
name|ac
init|=
operator|*
name|acp
decl_stmt|;
name|char
modifier|*
modifier|*
name|av
init|=
operator|*
name|avp
decl_stmt|;
name|char
modifier|*
name|p
init|=
literal|0
decl_stmt|,
name|md
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|ac
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"any"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|ipno
operator|->
name|s_addr
operator|=
name|mask
operator|->
name|s_addr
operator|=
literal|0
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
else|else
block|{
name|u_int32_t
name|i
decl_stmt|;
name|p
operator|=
name|strchr
argument_list|(
operator|*
name|av
argument_list|,
literal|'/'
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|p
condition|)
name|p
operator|=
name|strchr
argument_list|(
operator|*
name|av
argument_list|,
literal|':'
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
condition|)
block|{
name|md
operator|=
operator|*
name|p
expr_stmt|;
operator|*
name|p
operator|++
operator|=
literal|'\0'
expr_stmt|;
block|}
if|if
condition|(
name|lookup_host
argument_list|(
operator|*
name|av
argument_list|,
name|ipno
argument_list|)
operator|!=
literal|0
condition|)
name|errx
argument_list|(
name|EX_NOHOST
argument_list|,
literal|"hostname ``%s'' unknown"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|md
condition|)
block|{
case|case
literal|':'
case|:
if|if
condition|(
operator|!
name|inet_aton
argument_list|(
name|p
argument_list|,
name|mask
argument_list|)
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"bad netmask ``%s''"
argument_list|,
name|p
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'/'
case|:
name|i
operator|=
name|atoi
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|==
literal|0
condition|)
name|mask
operator|->
name|s_addr
operator|=
literal|0
expr_stmt|;
elseif|else
if|if
condition|(
name|i
operator|>
literal|32
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"bad width ``%s''"
argument_list|,
name|p
argument_list|)
expr_stmt|;
else|else
name|mask
operator|->
name|s_addr
operator|=
name|htonl
argument_list|(
operator|~
literal|0
operator|<<
operator|(
literal|32
operator|-
name|i
operator|)
argument_list|)
expr_stmt|;
break|break;
default|default:
name|mask
operator|->
name|s_addr
operator|=
name|htonl
argument_list|(
operator|~
literal|0
argument_list|)
expr_stmt|;
break|break;
block|}
name|ipno
operator|->
name|s_addr
operator|&=
name|mask
operator|->
name|s_addr
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
operator|*
name|acp
operator|=
name|ac
expr_stmt|;
operator|*
name|avp
operator|=
name|av
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|fill_reject_code
parameter_list|(
name|u_short
modifier|*
name|codep
parameter_list|,
name|char
modifier|*
name|str
parameter_list|)
block|{
name|struct
name|icmpcode
modifier|*
name|ic
decl_stmt|;
name|u_long
name|val
decl_stmt|;
name|char
modifier|*
name|s
decl_stmt|;
if|if
condition|(
name|str
operator|==
literal|'\0'
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"missing unreachable code"
argument_list|)
expr_stmt|;
name|val
operator|=
name|strtoul
argument_list|(
name|str
argument_list|,
operator|&
name|s
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|!=
name|str
operator|&&
operator|*
name|s
operator|==
literal|'\0'
operator|&&
name|val
operator|<
literal|0x100
condition|)
block|{
operator|*
name|codep
operator|=
name|val
expr_stmt|;
return|return;
block|}
for|for
control|(
name|ic
operator|=
name|icmpcodes
init|;
name|ic
operator|->
name|str
condition|;
name|ic
operator|++
control|)
if|if
condition|(
operator|!
name|strcasecmp
argument_list|(
name|str
argument_list|,
name|ic
operator|->
name|str
argument_list|)
condition|)
block|{
operator|*
name|codep
operator|=
name|ic
operator|->
name|code
expr_stmt|;
return|return;
block|}
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"unknown ICMP unreachable code ``%s''"
argument_list|,
name|str
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|add_port
parameter_list|(
name|u_short
modifier|*
name|cnt
parameter_list|,
name|u_short
modifier|*
name|ptr
parameter_list|,
name|u_short
name|off
parameter_list|,
name|u_short
name|port
parameter_list|)
block|{
if|if
condition|(
name|off
operator|+
operator|*
name|cnt
operator|>=
name|IP_FW_MAX_PORTS
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"too many ports (max is %d)"
argument_list|,
name|IP_FW_MAX_PORTS
argument_list|)
expr_stmt|;
name|ptr
index|[
name|off
operator|+
operator|*
name|cnt
index|]
operator|=
name|port
expr_stmt|;
operator|(
operator|*
name|cnt
operator|)
operator|++
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|lookup_port
parameter_list|(
specifier|const
name|char
modifier|*
name|arg
parameter_list|,
name|int
name|proto
parameter_list|,
name|int
name|test
parameter_list|,
name|int
name|nodash
parameter_list|)
block|{
name|int
name|val
decl_stmt|;
name|char
modifier|*
name|earg
decl_stmt|,
name|buf
index|[
literal|32
index|]
decl_stmt|;
name|struct
name|servent
modifier|*
name|s
decl_stmt|;
name|char
modifier|*
name|p
decl_stmt|,
modifier|*
name|q
decl_stmt|;
name|snprintf
argument_list|(
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|,
literal|"%s"
argument_list|,
name|arg
argument_list|)
expr_stmt|;
for|for
control|(
name|p
operator|=
name|q
operator|=
name|buf
init|;
operator|*
name|p
condition|;
operator|*
name|q
operator|++
operator|=
operator|*
name|p
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|p
operator|==
literal|'\\'
condition|)
block|{
if|if
condition|(
operator|*
operator|(
name|p
operator|+
literal|1
operator|)
condition|)
name|p
operator|++
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
operator|*
name|p
operator|==
literal|','
operator|||
operator|(
name|nodash
operator|&&
operator|*
name|p
operator|==
literal|'-'
operator|)
condition|)
break|break;
block|}
block|}
operator|*
name|q
operator|=
literal|'\0'
expr_stmt|;
name|val
operator|=
operator|(
name|int
operator|)
name|strtoul
argument_list|(
name|buf
argument_list|,
operator|&
name|earg
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
operator|*
name|buf
operator|||
operator|*
name|earg
condition|)
block|{
name|char
modifier|*
name|protocol
init|=
name|NULL
decl_stmt|;
if|if
condition|(
name|proto
operator|!=
literal|0
condition|)
block|{
name|struct
name|protoent
modifier|*
name|pe
init|=
name|getprotobynumber
argument_list|(
name|proto
argument_list|)
decl_stmt|;
if|if
condition|(
name|pe
condition|)
name|protocol
operator|=
name|pe
operator|->
name|p_name
expr_stmt|;
block|}
name|setservent
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|s
operator|=
name|getservbyname
argument_list|(
name|buf
argument_list|,
name|protocol
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|!=
name|NULL
condition|)
name|val
operator|=
name|htons
argument_list|(
name|s
operator|->
name|s_port
argument_list|)
expr_stmt|;
else|else
block|{
if|if
condition|(
operator|!
name|test
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"unknown port ``%s''"
argument_list|,
name|buf
argument_list|)
expr_stmt|;
name|val
operator|=
operator|-
literal|1
expr_stmt|;
block|}
block|}
else|else
block|{
if|if
condition|(
name|val
operator|<
literal|0
operator|||
name|val
operator|>
literal|0xffff
condition|)
block|{
if|if
condition|(
operator|!
name|test
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"port ``%s'' out of range"
argument_list|,
name|buf
argument_list|)
expr_stmt|;
name|val
operator|=
operator|-
literal|1
expr_stmt|;
block|}
block|}
return|return
operator|(
name|val
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * return: 0 normally, 1 if first pair is a range,  * 2 if first pair is a port+mask  */
end_comment

begin_function
specifier|static
name|int
name|fill_port
parameter_list|(
name|u_short
modifier|*
name|cnt
parameter_list|,
name|u_short
modifier|*
name|ptr
parameter_list|,
name|u_short
name|off
parameter_list|,
name|char
modifier|*
name|arg
parameter_list|,
name|int
name|proto
parameter_list|)
block|{
name|char
modifier|*
name|s
decl_stmt|;
name|int
name|initial_range
init|=
literal|0
decl_stmt|;
for|for
control|(
name|s
operator|=
name|arg
init|;
operator|*
name|s
operator|&&
operator|*
name|s
operator|!=
literal|','
operator|&&
operator|*
name|s
operator|!=
literal|'-'
operator|&&
operator|*
name|s
operator|!=
literal|':'
condition|;
name|s
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|s
operator|==
literal|'\\'
operator|&&
operator|*
operator|(
name|s
operator|+
literal|1
operator|)
condition|)
name|s
operator|++
expr_stmt|;
block|}
if|if
condition|(
operator|*
name|s
operator|==
literal|':'
condition|)
block|{
operator|*
name|s
operator|++
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|strchr
argument_list|(
name|arg
argument_list|,
literal|','
argument_list|)
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"port/mask must be first in list"
argument_list|)
expr_stmt|;
name|add_port
argument_list|(
name|cnt
argument_list|,
name|ptr
argument_list|,
name|off
argument_list|,
operator|*
name|arg
condition|?
name|lookup_port
argument_list|(
name|arg
argument_list|,
name|proto
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
else|:
literal|0x0000
argument_list|)
expr_stmt|;
name|arg
operator|=
name|s
expr_stmt|;
name|s
operator|=
name|strchr
argument_list|(
name|arg
argument_list|,
literal|','
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
condition|)
operator|*
name|s
operator|++
operator|=
literal|'\0'
expr_stmt|;
name|add_port
argument_list|(
name|cnt
argument_list|,
name|ptr
argument_list|,
name|off
argument_list|,
operator|*
name|arg
condition|?
name|lookup_port
argument_list|(
name|arg
argument_list|,
name|proto
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
else|:
literal|0xffff
argument_list|)
expr_stmt|;
name|arg
operator|=
name|s
expr_stmt|;
name|initial_range
operator|=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|s
operator|==
literal|'-'
condition|)
block|{
operator|*
name|s
operator|++
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|strchr
argument_list|(
name|arg
argument_list|,
literal|','
argument_list|)
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"port range must be first in list"
argument_list|)
expr_stmt|;
name|add_port
argument_list|(
name|cnt
argument_list|,
name|ptr
argument_list|,
name|off
argument_list|,
operator|*
name|arg
condition|?
name|lookup_port
argument_list|(
name|arg
argument_list|,
name|proto
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
else|:
literal|0x0000
argument_list|)
expr_stmt|;
name|arg
operator|=
name|s
expr_stmt|;
name|s
operator|=
name|strchr
argument_list|(
name|arg
argument_list|,
literal|','
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
condition|)
operator|*
name|s
operator|++
operator|=
literal|'\0'
expr_stmt|;
name|add_port
argument_list|(
name|cnt
argument_list|,
name|ptr
argument_list|,
name|off
argument_list|,
operator|*
name|arg
condition|?
name|lookup_port
argument_list|(
name|arg
argument_list|,
name|proto
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
else|:
literal|0xffff
argument_list|)
expr_stmt|;
name|arg
operator|=
name|s
expr_stmt|;
name|initial_range
operator|=
literal|1
expr_stmt|;
block|}
while|while
condition|(
name|arg
operator|!=
name|NULL
condition|)
block|{
name|s
operator|=
name|strchr
argument_list|(
name|arg
argument_list|,
literal|','
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
condition|)
operator|*
name|s
operator|++
operator|=
literal|'\0'
expr_stmt|;
name|add_port
argument_list|(
name|cnt
argument_list|,
name|ptr
argument_list|,
name|off
argument_list|,
name|lookup_port
argument_list|(
name|arg
argument_list|,
name|proto
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|arg
operator|=
name|s
expr_stmt|;
block|}
return|return
name|initial_range
return|;
block|}
end_function

begin_comment
comment|/*  * helper function to process a set of flags and set bits in the  * appropriate masks.  */
end_comment

begin_function
specifier|static
name|void
name|fill_flags
parameter_list|(
name|u_char
modifier|*
name|set
parameter_list|,
name|u_char
modifier|*
name|reset
parameter_list|,
name|struct
name|_flaglist
modifier|*
name|flags
parameter_list|,
name|char
modifier|*
modifier|*
name|vp
parameter_list|)
block|{
name|char
modifier|*
name|p
init|=
operator|*
name|vp
decl_stmt|,
modifier|*
name|q
decl_stmt|;
comment|/* parameter */
name|u_char
modifier|*
name|d
decl_stmt|;
comment|/* which mask we are working on */
while|while
condition|(
name|p
operator|&&
operator|*
name|p
condition|)
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|'!'
condition|)
block|{
name|p
operator|++
expr_stmt|;
name|d
operator|=
name|reset
expr_stmt|;
block|}
else|else
name|d
operator|=
name|set
expr_stmt|;
name|q
operator|=
name|strchr
argument_list|(
name|p
argument_list|,
literal|','
argument_list|)
expr_stmt|;
if|if
condition|(
name|q
condition|)
operator|*
name|q
operator|++
operator|=
literal|'\0'
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|flags
index|[
name|i
index|]
operator|.
name|value
operator|!=
literal|0
condition|;
operator|++
name|i
control|)
if|if
condition|(
operator|!
name|strncmp
argument_list|(
name|p
argument_list|,
name|flags
index|[
name|i
index|]
operator|.
name|name
argument_list|,
name|strlen
argument_list|(
name|p
argument_list|)
argument_list|)
condition|)
block|{
operator|*
name|d
operator||=
name|flags
index|[
name|i
index|]
operator|.
name|value
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|flags
index|[
name|i
index|]
operator|.
name|value
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"invalid %s ``%s''"
argument_list|,
name|flags
index|[
name|i
index|]
operator|.
name|name
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|p
operator|=
name|q
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|fill_icmptypes
parameter_list|(
name|unsigned
modifier|*
name|types
parameter_list|,
name|char
modifier|*
modifier|*
name|vp
parameter_list|,
name|u_int
modifier|*
name|fw_flg
parameter_list|)
block|{
name|unsigned
name|long
name|icmptype
decl_stmt|;
name|char
modifier|*
name|c
init|=
operator|*
name|vp
decl_stmt|;
while|while
condition|(
operator|*
name|c
condition|)
block|{
if|if
condition|(
operator|*
name|c
operator|==
literal|','
condition|)
operator|++
name|c
expr_stmt|;
name|icmptype
operator|=
name|strtoul
argument_list|(
name|c
argument_list|,
operator|&
name|c
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|c
operator|!=
literal|','
operator|&&
operator|*
name|c
operator|!=
literal|'\0'
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"invalid ICMP type"
argument_list|)
expr_stmt|;
if|if
condition|(
name|icmptype
operator|>=
name|IP_FW_ICMPTYPES_DIM
operator|*
sizeof|sizeof
argument_list|(
name|unsigned
argument_list|)
operator|*
literal|8
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"ICMP type out of range"
argument_list|)
expr_stmt|;
name|types
index|[
name|icmptype
operator|/
operator|(
sizeof|sizeof
argument_list|(
name|unsigned
argument_list|)
operator|*
literal|8
operator|)
index|]
operator||=
literal|1
operator|<<
operator|(
name|icmptype
operator|%
operator|(
sizeof|sizeof
argument_list|(
name|unsigned
argument_list|)
operator|*
literal|8
operator|)
operator|)
expr_stmt|;
operator|*
name|fw_flg
operator||=
name|IP_FW_F_ICMPBIT
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|delete
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
name|struct
name|ip_fw
name|rule
decl_stmt|;
name|struct
name|dn_pipe
name|pipe
decl_stmt|;
name|int
name|i
decl_stmt|;
name|int
name|exitval
init|=
name|EX_OK
decl_stmt|;
name|memset
argument_list|(
operator|&
name|rule
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
name|rule
argument_list|)
expr_stmt|;
name|memset
argument_list|(
operator|&
name|pipe
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
name|pipe
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
comment|/* Rule number */
while|while
condition|(
name|ac
operator|&&
name|isdigit
argument_list|(
operator|*
operator|*
name|av
argument_list|)
condition|)
block|{
name|i
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|do_pipe
condition|)
block|{
if|if
condition|(
name|do_pipe
operator|==
literal|1
condition|)
name|pipe
operator|.
name|pipe_nr
operator|=
name|i
expr_stmt|;
else|else
name|pipe
operator|.
name|fs
operator|.
name|fs_nr
operator|=
name|i
expr_stmt|;
name|i
operator|=
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_DUMMYNET_DEL
argument_list|,
operator|&
name|pipe
argument_list|,
sizeof|sizeof
name|pipe
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
condition|)
block|{
name|exitval
operator|=
literal|1
expr_stmt|;
name|warn
argument_list|(
literal|"rule %u: setsockopt(IP_DUMMYNET_DEL)"
argument_list|,
name|do_pipe
operator|==
literal|1
condition|?
name|pipe
operator|.
name|pipe_nr
else|:
name|pipe
operator|.
name|fs
operator|.
name|fs_nr
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|rule
operator|.
name|fw_number
operator|=
name|i
expr_stmt|;
name|i
operator|=
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_FW_DEL
argument_list|,
operator|&
name|rule
argument_list|,
sizeof|sizeof
name|rule
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
condition|)
block|{
name|exitval
operator|=
name|EX_UNAVAILABLE
expr_stmt|;
name|warn
argument_list|(
literal|"rule %u: setsockopt(IP_FW_DEL)"
argument_list|,
name|rule
operator|.
name|fw_number
argument_list|)
expr_stmt|;
block|}
block|}
block|}
if|if
condition|(
name|exitval
operator|!=
name|EX_OK
condition|)
name|exit
argument_list|(
name|exitval
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|verify_interface
parameter_list|(
name|union
name|ip_fw_if
modifier|*
name|ifu
parameter_list|)
block|{
name|struct
name|ifreq
name|ifr
decl_stmt|;
comment|/* 	 *	If a unit was specified, check for that exact interface. 	 *	If a wildcard was specified, check for unit 0. 	 */
name|snprintf
argument_list|(
name|ifr
operator|.
name|ifr_name
argument_list|,
sizeof|sizeof
argument_list|(
name|ifr
operator|.
name|ifr_name
argument_list|)
argument_list|,
literal|"%s%d"
argument_list|,
name|ifu
operator|->
name|fu_via_if
operator|.
name|name
argument_list|,
name|ifu
operator|->
name|fu_via_if
operator|.
name|unit
operator|==
operator|-
literal|1
condition|?
literal|0
else|:
name|ifu
operator|->
name|fu_via_if
operator|.
name|unit
argument_list|)
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|s
argument_list|,
name|SIOCGIFFLAGS
argument_list|,
operator|&
name|ifr
argument_list|)
operator|<
literal|0
condition|)
name|warnx
argument_list|(
literal|"warning: interface ``%s'' does not exist"
argument_list|,
name|ifr
operator|.
name|ifr_name
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|fill_iface
parameter_list|(
name|char
modifier|*
name|which
parameter_list|,
name|union
name|ip_fw_if
modifier|*
name|ifu
parameter_list|,
name|int
modifier|*
name|byname
parameter_list|,
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|arg
parameter_list|)
block|{
if|if
condition|(
operator|!
name|ac
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"missing argument for ``%s''"
argument_list|,
name|which
argument_list|)
expr_stmt|;
comment|/* Parse the interface or address */
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|arg
argument_list|,
literal|"any"
argument_list|)
condition|)
block|{
name|ifu
operator|->
name|fu_via_ip
operator|.
name|s_addr
operator|=
literal|0
expr_stmt|;
operator|*
name|byname
operator|=
literal|0
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|isdigit
argument_list|(
operator|*
name|arg
argument_list|)
condition|)
block|{
name|char
modifier|*
name|q
decl_stmt|;
operator|*
name|byname
operator|=
literal|1
expr_stmt|;
name|strncpy
argument_list|(
name|ifu
operator|->
name|fu_via_if
operator|.
name|name
argument_list|,
name|arg
argument_list|,
sizeof|sizeof
argument_list|(
name|ifu
operator|->
name|fu_via_if
operator|.
name|name
argument_list|)
argument_list|)
expr_stmt|;
name|ifu
operator|->
name|fu_via_if
operator|.
name|name
index|[
sizeof|sizeof
argument_list|(
name|ifu
operator|->
name|fu_via_if
operator|.
name|name
argument_list|)
operator|-
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
for|for
control|(
name|q
operator|=
name|ifu
operator|->
name|fu_via_if
operator|.
name|name
init|;
operator|*
name|q
operator|&&
operator|!
name|isdigit
argument_list|(
operator|*
name|q
argument_list|)
operator|&&
operator|*
name|q
operator|!=
literal|'*'
condition|;
name|q
operator|++
control|)
continue|continue;
name|ifu
operator|->
name|fu_via_if
operator|.
name|unit
operator|=
operator|(
operator|*
name|q
operator|==
literal|'*'
operator|)
condition|?
operator|-
literal|1
else|:
name|atoi
argument_list|(
name|q
argument_list|)
expr_stmt|;
operator|*
name|q
operator|=
literal|'\0'
expr_stmt|;
name|verify_interface
argument_list|(
name|ifu
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|inet_aton
argument_list|(
name|arg
argument_list|,
operator|&
name|ifu
operator|->
name|fu_via_ip
argument_list|)
condition|)
block|{
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"bad ip address ``%s''"
argument_list|,
name|arg
argument_list|)
expr_stmt|;
block|}
else|else
operator|*
name|byname
operator|=
literal|0
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|config_pipe
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
modifier|*
name|av
parameter_list|)
block|{
name|struct
name|dn_pipe
name|pipe
decl_stmt|;
name|int
name|i
decl_stmt|;
name|char
modifier|*
name|end
decl_stmt|;
name|memset
argument_list|(
operator|&
name|pipe
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
name|pipe
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
comment|/* Pipe number */
if|if
condition|(
name|ac
operator|&&
name|isdigit
argument_list|(
operator|*
operator|*
name|av
argument_list|)
condition|)
block|{
name|i
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|do_pipe
operator|==
literal|1
condition|)
name|pipe
operator|.
name|pipe_nr
operator|=
name|i
expr_stmt|;
else|else
name|pipe
operator|.
name|fs
operator|.
name|fs_nr
operator|=
name|i
expr_stmt|;
block|}
while|while
condition|(
name|ac
operator|>
literal|1
condition|)
block|{
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"plr"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|double
name|d
init|=
name|strtod
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
name|NULL
argument_list|)
decl_stmt|;
if|if
condition|(
name|d
operator|>
literal|1
condition|)
name|d
operator|=
literal|1
expr_stmt|;
elseif|else
if|if
condition|(
name|d
operator|<
literal|0
condition|)
name|d
operator|=
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|plr
operator|=
call|(
name|int
call|)
argument_list|(
name|d
operator|*
literal|0x7fffffff
argument_list|)
expr_stmt|;
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"queue"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|end
operator|=
name|NULL
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|qsize
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|end
operator|==
literal|'K'
operator|||
operator|*
name|end
operator|==
literal|'k'
condition|)
block|{
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator||=
name|DN_QSIZE_IS_BYTES
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|qsize
operator|*=
literal|1024
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|end
operator|==
literal|'B'
operator|||
operator|!
name|strncmp
argument_list|(
name|end
argument_list|,
literal|"by"
argument_list|,
literal|2
argument_list|)
condition|)
block|{
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator||=
name|DN_QSIZE_IS_BYTES
expr_stmt|;
block|}
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"buckets"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|pipe
operator|.
name|fs
operator|.
name|rq_size
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"mask"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
comment|/* per-flow queue, mask is dst_ip, dst_port, 			 * src_ip, src_port, proto measured in bits 			 */
name|u_int32_t
name|a
decl_stmt|;
name|void
modifier|*
name|par
init|=
name|NULL
decl_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|dst_ip
operator|=
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|src_ip
operator|=
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|dst_port
operator|=
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|src_port
operator|=
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|proto
operator|=
literal|0
expr_stmt|;
name|end
operator|=
name|NULL
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|ac
operator|>=
literal|1
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"all"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
comment|/* special case -- all bits are significant */
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|dst_ip
operator|=
operator|~
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|src_ip
operator|=
operator|~
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|dst_port
operator|=
operator|~
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|src_port
operator|=
operator|~
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|proto
operator|=
operator|~
literal|0
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator||=
name|DN_HAVE_FLOW_MASK
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
continue|continue;
block|}
while|while
condition|(
name|ac
operator|>=
literal|1
condition|)
block|{
name|int
name|len
init|=
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"dst-ip"
argument_list|,
name|len
argument_list|)
condition|)
name|par
operator|=
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|dst_ip
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"src-ip"
argument_list|,
name|len
argument_list|)
condition|)
name|par
operator|=
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|src_ip
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"dst-port"
argument_list|,
name|len
argument_list|)
condition|)
name|par
operator|=
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|dst_port
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"src-port"
argument_list|,
name|len
argument_list|)
condition|)
name|par
operator|=
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|src_port
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"proto"
argument_list|,
name|len
argument_list|)
condition|)
name|par
operator|=
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|proto
expr_stmt|;
else|else
break|break;
if|if
condition|(
name|ac
operator|<
literal|2
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"mask: %s value"
literal|" missing"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|av
index|[
literal|1
index|]
operator|==
literal|'/'
condition|)
block|{
name|a
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
operator|+
literal|1
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|a
operator|==
literal|32
condition|)
comment|/* special case... */
name|a
operator|=
operator|~
literal|0
expr_stmt|;
else|else
name|a
operator|=
operator|(
literal|1
operator|<<
name|a
operator|)
operator|-
literal|1
expr_stmt|;
block|}
else|else
block|{
name|a
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|par
operator|==
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|src_port
operator|||
name|par
operator|==
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|dst_port
condition|)
block|{
if|if
condition|(
name|a
operator|>=
operator|(
literal|1
operator|<<
literal|16
operator|)
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"mask: %s"
literal|" must be 16 bit, not"
literal|" 0x%08x"
argument_list|,
operator|*
name|av
argument_list|,
name|a
argument_list|)
expr_stmt|;
operator|*
operator|(
operator|(
name|u_int16_t
operator|*
operator|)
name|par
operator|)
operator|=
operator|(
name|u_int16_t
operator|)
name|a
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|par
operator|==
operator|&
name|pipe
operator|.
name|fs
operator|.
name|flow_mask
operator|.
name|proto
condition|)
block|{
if|if
condition|(
name|a
operator|>=
operator|(
literal|1
operator|<<
literal|8
operator|)
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"mask: %s"
literal|" must be"
literal|" 8 bit, not 0x%08x"
argument_list|,
operator|*
name|av
argument_list|,
name|a
argument_list|)
expr_stmt|;
operator|*
operator|(
operator|(
name|u_int8_t
operator|*
operator|)
name|par
operator|)
operator|=
operator|(
name|u_int8_t
operator|)
name|a
expr_stmt|;
block|}
else|else
operator|*
operator|(
operator|(
name|u_int32_t
operator|*
operator|)
name|par
operator|)
operator|=
name|a
expr_stmt|;
if|if
condition|(
name|a
operator|!=
literal|0
condition|)
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator||=
name|DN_HAVE_FLOW_MASK
expr_stmt|;
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
comment|/* end for */
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"red"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"gred"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
comment|/* RED enabled */
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator||=
name|DN_IS_RED
expr_stmt|;
if|if
condition|(
operator|*
name|av
index|[
literal|0
index|]
operator|==
literal|'g'
condition|)
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator||=
name|DN_IS_GENTLE_RED
expr_stmt|;
if|if
condition|(
operator|(
name|end
operator|=
name|strsep
argument_list|(
operator|&
name|av
index|[
literal|1
index|]
argument_list|,
literal|"/"
argument_list|)
operator|)
condition|)
block|{
name|double
name|w_q
init|=
name|strtod
argument_list|(
name|end
argument_list|,
name|NULL
argument_list|)
decl_stmt|;
if|if
condition|(
name|w_q
operator|>
literal|1
operator|||
name|w_q
operator|<=
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"w_q %f must be "
literal|"0< x<= 1"
argument_list|,
name|w_q
argument_list|)
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|w_q
operator|=
call|(
name|int
call|)
argument_list|(
name|w_q
operator|*
operator|(
literal|1
operator|<<
name|SCALE_RED
operator|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|end
operator|=
name|strsep
argument_list|(
operator|&
name|av
index|[
literal|1
index|]
argument_list|,
literal|"/"
argument_list|)
operator|)
condition|)
block|{
name|pipe
operator|.
name|fs
operator|.
name|min_th
operator|=
name|strtoul
argument_list|(
name|end
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|end
operator|==
literal|'K'
operator|||
operator|*
name|end
operator|==
literal|'k'
condition|)
name|pipe
operator|.
name|fs
operator|.
name|min_th
operator|*=
literal|1024
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|end
operator|=
name|strsep
argument_list|(
operator|&
name|av
index|[
literal|1
index|]
argument_list|,
literal|"/"
argument_list|)
operator|)
condition|)
block|{
name|pipe
operator|.
name|fs
operator|.
name|max_th
operator|=
name|strtoul
argument_list|(
name|end
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|end
operator|==
literal|'K'
operator|||
operator|*
name|end
operator|==
literal|'k'
condition|)
name|pipe
operator|.
name|fs
operator|.
name|max_th
operator|*=
literal|1024
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|end
operator|=
name|strsep
argument_list|(
operator|&
name|av
index|[
literal|1
index|]
argument_list|,
literal|"/"
argument_list|)
operator|)
condition|)
block|{
name|double
name|max_p
init|=
name|strtod
argument_list|(
name|end
argument_list|,
name|NULL
argument_list|)
decl_stmt|;
if|if
condition|(
name|max_p
operator|>
literal|1
operator|||
name|max_p
operator|<=
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"max_p %f must be "
literal|"0< x<= 1"
argument_list|,
name|max_p
argument_list|)
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|max_p
operator|=
call|(
name|int
call|)
argument_list|(
name|max_p
operator|*
operator|(
literal|1
operator|<<
name|SCALE_RED
operator|)
argument_list|)
expr_stmt|;
block|}
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"droptail"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
comment|/* DROPTAIL */
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator|&=
operator|~
operator|(
name|DN_IS_RED
operator||
name|DN_IS_GENTLE_RED
operator|)
expr_stmt|;
name|av
operator|+=
literal|1
expr_stmt|;
name|ac
operator|-=
literal|1
expr_stmt|;
block|}
else|else
block|{
name|int
name|len
init|=
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
decl_stmt|;
if|if
condition|(
name|do_pipe
operator|==
literal|1
condition|)
block|{
comment|/* some commands are only good for pipes. */
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"bw"
argument_list|,
name|len
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"bandwidth"
argument_list|,
name|len
argument_list|)
condition|)
block|{
if|if
condition|(
name|av
index|[
literal|1
index|]
index|[
literal|0
index|]
operator|>=
literal|'a'
operator|&&
name|av
index|[
literal|1
index|]
index|[
literal|0
index|]
operator|<=
literal|'z'
condition|)
block|{
name|int
name|l
init|=
sizeof|sizeof
argument_list|(
name|pipe
operator|.
name|if_name
argument_list|)
operator|-
literal|1
decl_stmt|;
comment|/* interface name */
name|strncpy
argument_list|(
name|pipe
operator|.
name|if_name
argument_list|,
name|av
index|[
literal|1
index|]
argument_list|,
name|l
argument_list|)
expr_stmt|;
name|pipe
operator|.
name|if_name
index|[
name|l
index|]
operator|=
literal|'\0'
expr_stmt|;
name|pipe
operator|.
name|bandwidth
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|pipe
operator|.
name|if_name
index|[
literal|0
index|]
operator|=
literal|'\0'
expr_stmt|;
name|pipe
operator|.
name|bandwidth
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|end
operator|==
literal|'K'
operator|||
operator|*
name|end
operator|==
literal|'k'
condition|)
block|{
name|end
operator|++
expr_stmt|;
name|pipe
operator|.
name|bandwidth
operator|*=
literal|1000
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|end
operator|==
literal|'M'
condition|)
block|{
name|end
operator|++
expr_stmt|;
name|pipe
operator|.
name|bandwidth
operator|*=
literal|1000000
expr_stmt|;
block|}
if|if
condition|(
operator|*
name|end
operator|==
literal|'B'
operator|||
operator|!
name|strncmp
argument_list|(
name|end
argument_list|,
literal|"by"
argument_list|,
literal|2
argument_list|)
condition|)
name|pipe
operator|.
name|bandwidth
operator|*=
literal|8
expr_stmt|;
block|}
if|if
condition|(
name|pipe
operator|.
name|bandwidth
operator|<
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"bandwidth too large"
argument_list|)
expr_stmt|;
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"delay"
argument_list|,
name|len
argument_list|)
condition|)
block|{
name|pipe
operator|.
name|delay
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
else|else
block|{
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"unrecognised pipe"
literal|" option ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
comment|/* this refers to a queue */
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"weight"
argument_list|,
name|len
argument_list|)
condition|)
block|{
name|pipe
operator|.
name|fs
operator|.
name|weight
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"pipe"
argument_list|,
name|len
argument_list|)
condition|)
block|{
name|pipe
operator|.
name|fs
operator|.
name|parent_nr
operator|=
name|strtoul
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
else|else
block|{
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"unrecognised option "
literal|"``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
if|if
condition|(
name|do_pipe
operator|==
literal|1
condition|)
block|{
if|if
condition|(
name|pipe
operator|.
name|pipe_nr
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"pipe_nr %d must be> 0"
argument_list|,
name|pipe
operator|.
name|pipe_nr
argument_list|)
expr_stmt|;
if|if
condition|(
name|pipe
operator|.
name|delay
operator|>
literal|10000
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"delay %d must be< 10000"
argument_list|,
name|pipe
operator|.
name|delay
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/* do_pipe == 2, queue */
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|parent_nr
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"pipe %d must be> 0"
argument_list|,
name|pipe
operator|.
name|fs
operator|.
name|parent_nr
argument_list|)
expr_stmt|;
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|weight
operator|>
literal|100
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"weight %d must be<= 100"
argument_list|,
name|pipe
operator|.
name|fs
operator|.
name|weight
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator|&
name|DN_QSIZE_IS_BYTES
condition|)
block|{
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|qsize
operator|>
literal|1024
operator|*
literal|1024
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"queue size %d, must be< 1MB"
argument_list|,
name|pipe
operator|.
name|fs
operator|.
name|qsize
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|qsize
operator|>
literal|100
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"queue size %d, must be"
literal|" 2<= x<= 100"
argument_list|,
name|pipe
operator|.
name|fs
operator|.
name|qsize
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|flags_fs
operator|&
name|DN_IS_RED
condition|)
block|{
name|size_t
name|len
decl_stmt|;
name|int
name|lookup_depth
decl_stmt|,
name|avg_pkt_size
decl_stmt|;
name|double
name|s
decl_stmt|,
name|idle
decl_stmt|,
name|weight
decl_stmt|,
name|w_q
decl_stmt|;
name|struct
name|clockinfo
name|clock
decl_stmt|;
name|int
name|t
decl_stmt|;
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|min_th
operator|>=
name|pipe
operator|.
name|fs
operator|.
name|max_th
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"min_th %d must be< than max_th %d"
argument_list|,
name|pipe
operator|.
name|fs
operator|.
name|min_th
argument_list|,
name|pipe
operator|.
name|fs
operator|.
name|max_th
argument_list|)
expr_stmt|;
if|if
condition|(
name|pipe
operator|.
name|fs
operator|.
name|max_th
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"max_th must be> 0"
argument_list|)
expr_stmt|;
name|len
operator|=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
expr_stmt|;
if|if
condition|(
name|sysctlbyname
argument_list|(
literal|"net.inet.ip.dummynet.red_lookup_depth"
argument_list|,
operator|&
name|lookup_depth
argument_list|,
operator|&
name|len
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"sysctlbyname(\"%s\")"
argument_list|,
literal|"net.inet.ip.dummynet.red_lookup_depth"
argument_list|)
expr_stmt|;
if|if
condition|(
name|lookup_depth
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"net.inet.ip.dummynet.red_lookup_depth"
literal|" must be greater than zero"
argument_list|)
expr_stmt|;
name|len
operator|=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
expr_stmt|;
if|if
condition|(
name|sysctlbyname
argument_list|(
literal|"net.inet.ip.dummynet.red_avg_pkt_size"
argument_list|,
operator|&
name|avg_pkt_size
argument_list|,
operator|&
name|len
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"sysctlbyname(\"%s\")"
argument_list|,
literal|"net.inet.ip.dummynet.red_avg_pkt_size"
argument_list|)
expr_stmt|;
if|if
condition|(
name|avg_pkt_size
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"net.inet.ip.dummynet.red_avg_pkt_size must"
literal|" be greater than zero"
argument_list|)
expr_stmt|;
name|len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|clockinfo
argument_list|)
expr_stmt|;
if|if
condition|(
name|sysctlbyname
argument_list|(
literal|"kern.clockrate"
argument_list|,
operator|&
name|clock
argument_list|,
operator|&
name|len
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"sysctlbyname(\"%s\")"
argument_list|,
literal|"kern.clockrate"
argument_list|)
expr_stmt|;
comment|/* 		 * Ticks needed for sending a medium-sized packet. 		 * Unfortunately, when we are configuring a WF2Q+ queue, we 		 * do not have bandwidth information, because that is stored 		 * in the parent pipe, and also we have multiple queues 		 * competing for it. So we set s=0, which is not very 		 * correct. But on the other hand, why do we want RED with 		 * WF2Q+ ? 		 */
if|if
condition|(
name|pipe
operator|.
name|bandwidth
operator|==
literal|0
condition|)
comment|/* this is a WF2Q+ queue */
name|s
operator|=
literal|0
expr_stmt|;
else|else
name|s
operator|=
name|clock
operator|.
name|hz
operator|*
name|avg_pkt_size
operator|*
literal|8
operator|/
name|pipe
operator|.
name|bandwidth
expr_stmt|;
comment|/* 		 * max idle time (in ticks) before avg queue size becomes 0. 		 * NOTA:  (3/w_q) is approx the value x so that 		 * (1-w_q)^x< 10^-3. 		 */
name|w_q
operator|=
operator|(
operator|(
name|double
operator|)
name|pipe
operator|.
name|fs
operator|.
name|w_q
operator|)
operator|/
operator|(
literal|1
operator|<<
name|SCALE_RED
operator|)
expr_stmt|;
name|idle
operator|=
name|s
operator|*
literal|3.
operator|/
name|w_q
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|lookup_step
operator|=
operator|(
name|int
operator|)
name|idle
operator|/
name|lookup_depth
expr_stmt|;
if|if
condition|(
operator|!
name|pipe
operator|.
name|fs
operator|.
name|lookup_step
condition|)
name|pipe
operator|.
name|fs
operator|.
name|lookup_step
operator|=
literal|1
expr_stmt|;
name|weight
operator|=
literal|1
operator|-
name|w_q
expr_stmt|;
for|for
control|(
name|t
operator|=
name|pipe
operator|.
name|fs
operator|.
name|lookup_step
init|;
name|t
operator|>
literal|0
condition|;
operator|--
name|t
control|)
name|weight
operator|*=
name|weight
expr_stmt|;
name|pipe
operator|.
name|fs
operator|.
name|lookup_weight
operator|=
call|(
name|int
call|)
argument_list|(
name|weight
operator|*
operator|(
literal|1
operator|<<
name|SCALE_RED
operator|)
argument_list|)
expr_stmt|;
block|}
name|i
operator|=
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_DUMMYNET_CONFIGURE
argument_list|,
operator|&
name|pipe
argument_list|,
sizeof|sizeof
name|pipe
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
condition|)
name|err
argument_list|(
literal|1
argument_list|,
literal|"setsockopt(%s)"
argument_list|,
literal|"IP_DUMMYNET_CONFIGURE"
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|get_mac_addr_mask
parameter_list|(
name|char
modifier|*
name|p
parameter_list|,
name|u_char
modifier|*
name|addr
parameter_list|,
name|u_char
modifier|*
name|mask
parameter_list|)
block|{
name|int
name|i
decl_stmt|,
name|l
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|6
condition|;
name|i
operator|++
control|)
name|addr
index|[
name|i
index|]
operator|=
name|mask
index|[
name|i
index|]
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|p
argument_list|,
literal|"any"
argument_list|)
condition|)
return|return;
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|*
name|p
operator|&&
name|i
operator|<
literal|6
condition|;
name|i
operator|++
operator|,
name|p
operator|++
control|)
block|{
name|addr
index|[
name|i
index|]
operator|=
name|strtol
argument_list|(
name|p
argument_list|,
operator|&
name|p
argument_list|,
literal|16
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|!=
literal|':'
condition|)
comment|/* we start with the mask */
break|break;
block|}
if|if
condition|(
operator|*
name|p
operator|==
literal|'/'
condition|)
block|{
comment|/* mask len */
name|l
operator|=
name|strtol
argument_list|(
name|p
operator|+
literal|1
argument_list|,
operator|&
name|p
argument_list|,
literal|0
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|l
operator|>
literal|0
condition|;
name|l
operator|-=
literal|8
operator|,
name|i
operator|++
control|)
name|mask
index|[
name|i
index|]
operator|=
operator|(
name|l
operator|>=
literal|8
operator|)
condition|?
literal|0xff
else|:
operator|(
operator|~
literal|0
operator|)
operator|<<
operator|(
literal|8
operator|-
name|l
operator|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
literal|'&'
condition|)
block|{
comment|/* mask */
for|for
control|(
name|i
operator|=
literal|0
operator|,
name|p
operator|++
init|;
operator|*
name|p
operator|&&
name|i
operator|<
literal|6
condition|;
name|i
operator|++
operator|,
name|p
operator|++
control|)
block|{
name|mask
index|[
name|i
index|]
operator|=
name|strtol
argument_list|(
name|p
argument_list|,
operator|&
name|p
argument_list|,
literal|16
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|!=
literal|':'
condition|)
break|break;
block|}
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
literal|'\0'
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|6
condition|;
name|i
operator|++
control|)
name|mask
index|[
name|i
index|]
operator|=
literal|0xff
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|6
condition|;
name|i
operator|++
control|)
name|addr
index|[
name|i
index|]
operator|&=
name|mask
index|[
name|i
index|]
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * fetch and add the MAC address and type, with masks  */
end_comment

begin_function
specifier|static
name|void
name|add_mac
parameter_list|(
name|struct
name|ip_fw
modifier|*
name|rule
parameter_list|,
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
name|u_char
modifier|*
name|addr
decl_stmt|,
modifier|*
name|mask
decl_stmt|;
name|u_short
modifier|*
name|type
decl_stmt|,
modifier|*
name|typemask
decl_stmt|;
name|int
name|i
decl_stmt|;
name|char
modifier|*
name|p
decl_stmt|;
name|struct
name|_s_x
modifier|*
name|pt
decl_stmt|;
if|if
condition|(
name|ac
operator|<
literal|3
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"MAC dst src type"
argument_list|)
expr_stmt|;
name|addr
operator|=
operator|(
name|u_char
operator|*
operator|)
operator|&
operator|(
name|rule
operator|->
name|fw_mac_hdr
operator|)
expr_stmt|;
name|mask
operator|=
operator|(
name|u_char
operator|*
operator|)
operator|&
operator|(
name|rule
operator|->
name|fw_mac_mask
operator|)
expr_stmt|;
name|get_mac_addr_mask
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
name|addr
argument_list|,
name|mask
argument_list|)
expr_stmt|;
name|addr
operator|+=
literal|6
expr_stmt|;
name|mask
operator|+=
literal|6
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|get_mac_addr_mask
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
name|addr
argument_list|,
name|mask
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|type
operator|=
operator|(
name|u_short
operator|*
operator|)
operator|&
operator|(
name|rule
operator|->
name|fw_mac_type
operator|)
expr_stmt|;
name|typemask
operator|=
operator|(
name|u_short
operator|*
operator|)
operator|&
operator|(
name|rule
operator|->
name|fw_mac_mask_type
operator|)
expr_stmt|;
name|rule
operator|->
name|fw_flg
operator||=
name|IP_FW_F_MAC
expr_stmt|;
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
literal|"any"
argument_list|)
condition|)
block|{
operator|*
name|type
operator|=
operator|*
name|typemask
operator|=
name|htons
argument_list|(
literal|0
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 	 * the match length is the string up to the first separator 	 * we know, i.e. any of "\0:/&". Note, we use bcmp instead of 	 * strcmp as we want an exact match. 	 */
name|p
operator|=
name|strpbrk
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
literal|"-:/&"
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
name|i
operator|=
name|strlen
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
else|else
name|i
operator|=
name|p
operator|-
name|av
index|[
literal|0
index|]
expr_stmt|;
for|for
control|(
name|pt
operator|=
name|ether_types
init|;
name|i
operator|&&
name|pt
operator|->
name|s
operator|!=
name|NULL
condition|;
name|pt
operator|++
control|)
if|if
condition|(
name|strlen
argument_list|(
name|pt
operator|->
name|s
argument_list|)
operator|==
name|i
operator|&&
operator|!
name|bcmp
argument_list|(
operator|*
name|av
argument_list|,
name|pt
operator|->
name|s
argument_list|,
name|i
argument_list|)
condition|)
break|break;
comment|/* store type in network format for all cases but range */
if|if
condition|(
name|pt
operator|->
name|s
operator|!=
name|NULL
condition|)
block|{
operator|*
name|type
operator|=
name|htons
argument_list|(
name|pt
operator|->
name|x
argument_list|)
expr_stmt|;
name|p
operator|=
name|av
index|[
literal|0
index|]
operator|+
name|i
expr_stmt|;
block|}
else|else
operator|*
name|type
operator|=
name|htons
argument_list|(
name|strtol
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
operator|&
name|p
argument_list|,
literal|16
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|typemask
operator|=
name|htons
argument_list|(
literal|0xffff
argument_list|)
expr_stmt|;
comment|/* default */
if|if
condition|(
operator|*
name|p
operator|==
literal|'-'
condition|)
block|{
name|rule
operator|->
name|fw_flg
operator||=
name|IP_FW_F_SRNG
expr_stmt|;
operator|*
name|type
operator|=
name|ntohs
argument_list|(
operator|*
name|type
argument_list|)
expr_stmt|;
comment|/* revert to host format */
name|p
operator|++
expr_stmt|;
name|i
operator|=
name|strlen
argument_list|(
name|p
argument_list|)
expr_stmt|;
for|for
control|(
name|pt
operator|=
name|ether_types
init|;
name|i
operator|&&
name|pt
operator|->
name|s
operator|!=
name|NULL
condition|;
name|pt
operator|++
control|)
if|if
condition|(
name|strlen
argument_list|(
name|pt
operator|->
name|s
argument_list|)
operator|==
name|i
operator|&&
operator|!
name|bcmp
argument_list|(
name|p
argument_list|,
name|pt
operator|->
name|s
argument_list|,
name|i
argument_list|)
condition|)
break|break;
if|if
condition|(
name|pt
operator|->
name|s
operator|!=
name|NULL
condition|)
block|{
operator|*
name|typemask
operator|=
name|pt
operator|->
name|x
expr_stmt|;
name|p
operator|+=
name|i
expr_stmt|;
block|}
else|else
operator|*
name|typemask
operator|=
name|strtol
argument_list|(
name|p
argument_list|,
operator|&
name|p
argument_list|,
literal|16
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
literal|'/'
condition|)
block|{
name|i
operator|=
name|strtol
argument_list|(
name|p
operator|+
literal|1
argument_list|,
operator|&
name|p
argument_list|,
literal|10
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|16
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"MAC: bad type %s\n"
argument_list|,
name|av
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
operator|*
name|typemask
operator|=
name|htons
argument_list|(
operator|(
operator|~
literal|0
operator|)
operator|<<
operator|(
literal|16
operator|-
name|i
operator|)
argument_list|)
expr_stmt|;
operator|*
name|type
operator|&=
operator|*
name|typemask
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
literal|':'
condition|)
block|{
operator|*
name|typemask
operator|=
name|htons
argument_list|(
name|strtol
argument_list|(
name|p
operator|+
literal|1
argument_list|,
operator|&
name|p
argument_list|,
literal|16
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|type
operator|&=
operator|*
name|typemask
expr_stmt|;
block|}
if|if
condition|(
operator|*
name|p
operator|!=
literal|'\0'
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"MAC: bad end type %s\n"
argument_list|,
name|av
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * the following macro returns an error message if we run out of  * arguments.  */
end_comment

begin_define
define|#
directive|define
name|NEED1
parameter_list|(
name|msg
parameter_list|)
value|{if (!ac) errx(EX_USAGE, msg);}
end_define

begin_function
specifier|static
name|void
name|add
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
name|struct
name|ip_fw
name|rule
decl_stmt|;
name|int
name|i
decl_stmt|;
name|u_char
name|proto
decl_stmt|;
name|struct
name|protoent
modifier|*
name|pe
decl_stmt|;
name|int
name|saw_xmrc
init|=
literal|0
decl_stmt|,
name|saw_via
init|=
literal|0
decl_stmt|;
name|memset
argument_list|(
operator|&
name|rule
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
name|rule
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
comment|/* [rule N]	-- Rule number optional */
if|if
condition|(
name|ac
operator|&&
name|isdigit
argument_list|(
operator|*
operator|*
name|av
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_number
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
comment|/* [prob D]	-- match probability, optional */
if|if
condition|(
name|ac
operator|>
literal|1
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"prob"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|double
name|d
init|=
name|strtod
argument_list|(
name|av
index|[
literal|1
index|]
argument_list|,
name|NULL
argument_list|)
decl_stmt|;
if|if
condition|(
name|d
operator|<=
literal|0
operator|||
name|d
operator|>
literal|1
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"illegal match prob. %s"
argument_list|,
name|av
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|d
operator|!=
literal|1
condition|)
block|{
comment|/* 1 means always match */
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_RND_MATCH
expr_stmt|;
name|rule
operator|.
name|dont_match_prob
operator|=
call|(
name|long
call|)
argument_list|(
operator|(
literal|1
operator|-
name|d
operator|)
operator|*
literal|0x7fffffff
argument_list|)
expr_stmt|;
block|}
name|av
operator|+=
literal|2
expr_stmt|;
name|ac
operator|-=
literal|2
expr_stmt|;
block|}
comment|/* action	-- mandatory */
name|NEED1
argument_list|(
literal|"missing action"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"accept"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"pass"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"allow"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"permit"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_ACCEPT
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"count"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_COUNT
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"pipe"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_PIPE
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing pipe number"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_divert_port
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"queue"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_QUEUE
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing queue number"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_divert_port
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"divert"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_DIVERT
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing divert port"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_divert_port
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|fw_divert_port
operator|==
literal|0
condition|)
block|{
name|struct
name|servent
modifier|*
name|s
decl_stmt|;
name|setservent
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|s
operator|=
name|getservbyname
argument_list|(
name|av
index|[
operator|-
literal|1
index|]
argument_list|,
literal|"divert"
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|!=
name|NULL
condition|)
name|rule
operator|.
name|fw_divert_port
operator|=
name|ntohs
argument_list|(
name|s
operator|->
name|s_port
argument_list|)
expr_stmt|;
else|else
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"illegal %s port"
argument_list|,
literal|"divert"
argument_list|)
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tee"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_TEE
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing tee divert port"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_divert_port
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|fw_divert_port
operator|==
literal|0
condition|)
block|{
name|struct
name|servent
modifier|*
name|s
decl_stmt|;
name|setservent
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|s
operator|=
name|getservbyname
argument_list|(
name|av
index|[
operator|-
literal|1
index|]
argument_list|,
literal|"divert"
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|!=
name|NULL
condition|)
name|rule
operator|.
name|fw_divert_port
operator|=
name|ntohs
argument_list|(
name|s
operator|->
name|s_port
argument_list|)
expr_stmt|;
else|else
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"illegal %s port"
argument_list|,
literal|"tee divert"
argument_list|)
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"fwd"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"forward"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|struct
name|in_addr
name|dummyip
decl_stmt|;
name|char
modifier|*
name|pp
decl_stmt|;
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_FWD
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing forwarding IP address"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_fwd_ip
operator|.
name|sin_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|sockaddr_in
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_fwd_ip
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|rule
operator|.
name|fw_fwd_ip
operator|.
name|sin_port
operator|=
literal|0
expr_stmt|;
name|pp
operator|=
name|strchr
argument_list|(
operator|*
name|av
argument_list|,
literal|':'
argument_list|)
expr_stmt|;
if|if
condition|(
name|pp
operator|==
name|NULL
condition|)
name|pp
operator|=
name|strchr
argument_list|(
operator|*
name|av
argument_list|,
literal|','
argument_list|)
expr_stmt|;
if|if
condition|(
name|pp
operator|!=
name|NULL
condition|)
block|{
operator|*
operator|(
name|pp
operator|++
operator|)
operator|=
literal|'\0'
expr_stmt|;
name|i
operator|=
name|lookup_port
argument_list|(
name|pp
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|==
operator|-
literal|1
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"illegal forwarding"
literal|" port ``%s''"
argument_list|,
name|pp
argument_list|)
expr_stmt|;
else|else
name|rule
operator|.
name|fw_fwd_ip
operator|.
name|sin_port
operator|=
operator|(
name|u_short
operator|)
name|i
expr_stmt|;
block|}
name|fill_ip
argument_list|(
operator|&
operator|(
name|rule
operator|.
name|fw_fwd_ip
operator|.
name|sin_addr
operator|)
argument_list|,
operator|&
name|dummyip
argument_list|,
operator|&
name|ac
argument_list|,
operator|&
name|av
argument_list|)
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|fw_fwd_ip
operator|.
name|sin_addr
operator|.
name|s_addr
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"illegal forwarding IP address"
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"skipto"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_SKIPTO
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing skipto rule number"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_skipto_rule
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|10
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"deny"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"drop"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_DENY
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"reject"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_REJECT
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|rule
operator|.
name|fw_reject_code
operator|=
name|ICMP_UNREACH_HOST
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"reset"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_REJECT
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|rule
operator|.
name|fw_reject_code
operator|=
name|IP_FW_REJECT_RST
expr_stmt|;
comment|/* check TCP later */
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"unreach"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_REJECT
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|fill_reject_code
argument_list|(
operator|&
name|rule
operator|.
name|fw_reject_code
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"check-state"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_CHECK_S
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
goto|goto
name|done
goto|;
block|}
else|else
block|{
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"invalid action ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
comment|/* [log [logamount N]]	-- log, optional */
if|if
condition|(
name|ac
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"log"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_PRN
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|ac
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"logamount"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|NEED1
argument_list|(
literal|"``logamount'' requires argument"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_logamount
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|fw_logamount
operator|<
literal|0
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"``logamount'' argument must be positive"
argument_list|)
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|fw_logamount
operator|==
literal|0
condition|)
name|rule
operator|.
name|fw_logamount
operator|=
operator|-
literal|1
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
block|}
block|}
comment|/* protocol	-- mandatory */
name|NEED1
argument_list|(
literal|"missing protocol"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"MAC"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
comment|/* need exactly 3 fields */
name|add_mac
argument_list|(
operator|&
name|rule
argument_list|,
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
comment|/* exits in case of errors */
name|ac
operator|-=
literal|3
expr_stmt|;
name|av
operator|+=
literal|3
expr_stmt|;
goto|goto
name|do_options
goto|;
block|}
elseif|else
if|if
condition|(
operator|(
name|proto
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|rule
operator|.
name|fw_prot
operator|=
name|proto
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"all"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_prot
operator|=
name|IPPROTO_IP
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|pe
operator|=
name|getprotobyname
argument_list|(
operator|*
name|av
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
name|rule
operator|.
name|fw_prot
operator|=
name|pe
operator|->
name|p_proto
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
else|else
block|{
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"invalid protocol ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|rule
operator|.
name|fw_prot
operator|!=
name|IPPROTO_TCP
operator|&&
operator|(
name|rule
operator|.
name|fw_flg
operator|&
name|IP_FW_F_COMMAND
operator|)
operator|==
name|IP_FW_F_REJECT
operator|&&
name|rule
operator|.
name|fw_reject_code
operator|==
name|IP_FW_REJECT_RST
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"``reset'' is only valid for tcp packets"
argument_list|)
expr_stmt|;
comment|/* from --	mandatory */
if|if
condition|(
operator|!
name|ac
operator|||
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"from"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"missing ``from''"
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
comment|/* not	--	optional */
if|if
condition|(
name|ac
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"not"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_INVSRC
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
name|NEED1
argument_list|(
literal|"missing source address"
argument_list|)
expr_stmt|;
comment|/* source	-- mandatory */
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"me"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_SME
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
else|else
block|{
name|fill_ip
argument_list|(
operator|&
name|rule
operator|.
name|fw_src
argument_list|,
operator|&
name|rule
operator|.
name|fw_smsk
argument_list|,
operator|&
name|ac
argument_list|,
operator|&
name|av
argument_list|)
expr_stmt|;
block|}
comment|/* ports	-- optional */
if|if
condition|(
name|ac
operator|&&
operator|(
name|isdigit
argument_list|(
operator|*
operator|*
name|av
argument_list|)
operator|||
name|lookup_port
argument_list|(
operator|*
name|av
argument_list|,
name|rule
operator|.
name|fw_prot
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|)
operator|>=
literal|0
operator|)
condition|)
block|{
name|u_short
name|nports
init|=
literal|0
decl_stmt|;
name|int
name|retval
decl_stmt|;
name|retval
operator|=
name|fill_port
argument_list|(
operator|&
name|nports
argument_list|,
name|rule
operator|.
name|fw_uar
operator|.
name|fw_pts
argument_list|,
literal|0
argument_list|,
operator|*
name|av
argument_list|,
name|rule
operator|.
name|fw_prot
argument_list|)
expr_stmt|;
if|if
condition|(
name|retval
operator|==
literal|1
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_SRNG
expr_stmt|;
elseif|else
if|if
condition|(
name|retval
operator|==
literal|2
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_SMSK
expr_stmt|;
name|IP_FW_SETNSRCP
argument_list|(
operator|&
name|rule
argument_list|,
name|nports
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
comment|/* to --	mandatory */
if|if
condition|(
operator|!
name|ac
operator|||
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"to"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"missing ``to''"
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
comment|/* not --	optional */
if|if
condition|(
name|ac
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"not"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_INVDST
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
name|NEED1
argument_list|(
literal|"missing dst address"
argument_list|)
expr_stmt|;
comment|/* destination	-- mandatory */
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"me"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_DME
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
else|else
block|{
name|fill_ip
argument_list|(
operator|&
name|rule
operator|.
name|fw_dst
argument_list|,
operator|&
name|rule
operator|.
name|fw_dmsk
argument_list|,
operator|&
name|ac
argument_list|,
operator|&
name|av
argument_list|)
expr_stmt|;
block|}
comment|/* dest.ports	-- optional */
if|if
condition|(
name|ac
operator|&&
operator|(
name|isdigit
argument_list|(
operator|*
operator|*
name|av
argument_list|)
operator|||
name|lookup_port
argument_list|(
operator|*
name|av
argument_list|,
name|rule
operator|.
name|fw_prot
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|)
operator|>=
literal|0
operator|)
condition|)
block|{
name|u_short
name|nports
init|=
literal|0
decl_stmt|;
name|int
name|retval
decl_stmt|;
name|retval
operator|=
name|fill_port
argument_list|(
operator|&
name|nports
argument_list|,
name|rule
operator|.
name|fw_uar
operator|.
name|fw_pts
argument_list|,
name|IP_FW_GETNSRCP
argument_list|(
operator|&
name|rule
argument_list|)
argument_list|,
operator|*
name|av
argument_list|,
name|rule
operator|.
name|fw_prot
argument_list|)
expr_stmt|;
if|if
condition|(
name|retval
operator|==
literal|1
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_DRNG
expr_stmt|;
elseif|else
if|if
condition|(
name|retval
operator|==
literal|2
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_DMSK
expr_stmt|;
name|IP_FW_SETNDSTP
argument_list|(
operator|&
name|rule
argument_list|,
name|nports
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|rule
operator|.
name|fw_prot
operator|!=
name|IPPROTO_TCP
operator|)
operator|&&
operator|(
name|rule
operator|.
name|fw_prot
operator|!=
name|IPPROTO_UDP
operator|)
operator|&&
operator|(
name|IP_FW_GETNSRCP
argument_list|(
operator|&
name|rule
argument_list|)
operator|||
name|IP_FW_GETNDSTP
argument_list|(
operator|&
name|rule
argument_list|)
operator|)
condition|)
block|{
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"only TCP and UDP protocols are valid"
literal|" with port specifications"
argument_list|)
expr_stmt|;
block|}
name|do_options
label|:
while|while
condition|(
name|ac
condition|)
block|{
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"uid"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|struct
name|passwd
modifier|*
name|pwd
decl_stmt|;
name|char
modifier|*
name|end
decl_stmt|;
name|uid_t
name|uid
decl_stmt|;
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_UID
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|NEED1
argument_list|(
literal|"``uid'' requires argument"
argument_list|)
expr_stmt|;
name|uid
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|end
operator|==
literal|'\0'
condition|)
name|pwd
operator|=
name|getpwuid
argument_list|(
name|uid
argument_list|)
expr_stmt|;
else|else
name|pwd
operator|=
name|getpwnam
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
if|if
condition|(
name|pwd
operator|==
name|NULL
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"uid \"%s\" is"
literal|" nonexistent"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_uid
operator|=
name|pwd
operator|->
name|pw_uid
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"gid"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|struct
name|group
modifier|*
name|grp
decl_stmt|;
name|char
modifier|*
name|end
decl_stmt|;
name|gid_t
name|gid
decl_stmt|;
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_GID
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|NEED1
argument_list|(
literal|"``gid'' requires argument"
argument_list|)
expr_stmt|;
name|gid
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
operator|&
name|end
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|end
operator|==
literal|'\0'
condition|)
name|grp
operator|=
name|getgrgid
argument_list|(
name|gid
argument_list|)
expr_stmt|;
else|else
name|grp
operator|=
name|getgrnam
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
if|if
condition|(
name|grp
operator|==
name|NULL
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"gid \"%s\" is"
literal|" nonexistent"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_gid
operator|=
name|grp
operator|->
name|gr_gid
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"in"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_IN
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"limit"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
comment|/* dyn. rule used to limit number of connections. */
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_KEEP_S
expr_stmt|;
name|rule
operator|.
name|dyn_type
operator|=
name|DYN_LIMIT
expr_stmt|;
name|rule
operator|.
name|limit_mask
operator|=
literal|0
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
for|for
control|(
init|;
name|ac
operator|>
literal|1
condition|;
control|)
block|{
name|struct
name|_s_x
modifier|*
name|p
init|=
name|limit_masks
decl_stmt|;
for|for
control|(
init|;
name|p
operator|->
name|x
operator|!=
literal|0
condition|;
name|p
operator|++
control|)
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
name|p
operator|->
name|s
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|limit_mask
operator||=
name|p
operator|->
name|x
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
break|break ;
block|}
if|if
condition|(
name|p
operator|->
name|s
operator|==
name|NULL
condition|)
break|break ;
block|}
name|NEED1
argument_list|(
literal|"limit needs mask and # of connections"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|conn_limit
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|conn_limit
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"limit: limit must be>0"
argument_list|)
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|limit_mask
operator|==
literal|0
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"missing limit mask"
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"keep-state"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|u_long
name|type
decl_stmt|;
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_KEEP_S
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|ac
operator|>
literal|0
operator|&&
operator|(
name|type
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
operator|)
operator|!=
literal|0
condition|)
block|{
name|rule
operator|.
name|dyn_type
operator|=
name|type
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"bridged"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_BRIDGED
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"out"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_OUT
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"xmit"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|union
name|ip_fw_if
name|ifu
decl_stmt|;
name|int
name|byname
decl_stmt|;
if|if
condition|(
name|saw_via
condition|)
block|{
name|badviacombo
label|:
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"``via'' is incompatible"
literal|" with ``xmit'' and ``recv''"
argument_list|)
expr_stmt|;
block|}
name|saw_xmrc
operator|=
literal|1
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|fill_iface
argument_list|(
literal|"xmit"
argument_list|,
operator|&
name|ifu
argument_list|,
operator|&
name|byname
argument_list|,
name|ac
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_out_if
operator|=
name|ifu
expr_stmt|;
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_OIFACE
expr_stmt|;
if|if
condition|(
name|byname
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_OIFNAME
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"recv"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|union
name|ip_fw_if
name|ifu
decl_stmt|;
name|int
name|byname
decl_stmt|;
if|if
condition|(
name|saw_via
condition|)
goto|goto
name|badviacombo
goto|;
name|saw_xmrc
operator|=
literal|1
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|fill_iface
argument_list|(
literal|"recv"
argument_list|,
operator|&
name|ifu
argument_list|,
operator|&
name|byname
argument_list|,
name|ac
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_in_if
operator|=
name|ifu
expr_stmt|;
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_IIFACE
expr_stmt|;
if|if
condition|(
name|byname
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_IIFNAME
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"via"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|union
name|ip_fw_if
name|ifu
decl_stmt|;
name|int
name|byname
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|saw_xmrc
condition|)
goto|goto
name|badviacombo
goto|;
name|saw_via
operator|=
literal|1
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|fill_iface
argument_list|(
literal|"via"
argument_list|,
operator|&
name|ifu
argument_list|,
operator|&
name|byname
argument_list|,
name|ac
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_out_if
operator|=
name|rule
operator|.
name|fw_in_if
operator|=
name|ifu
expr_stmt|;
if|if
condition|(
name|byname
condition|)
name|rule
operator|.
name|fw_flg
operator||=
operator|(
name|IP_FW_F_IIFNAME
operator||
name|IP_FW_F_OIFNAME
operator|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"fragment"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_FRAG
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"ipoptions"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"ipopts"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``ipoptions''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_IPOPT
expr_stmt|;
name|fill_flags
argument_list|(
operator|&
name|rule
operator|.
name|fw_ipopt
argument_list|,
operator|&
name|rule
operator|.
name|fw_ipnopt
argument_list|,
name|f_ipopts
argument_list|,
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"iplen"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``iplen''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_IPLEN
expr_stmt|;
name|rule
operator|.
name|fw_iplen
operator|=
operator|(
name|u_short
operator|)
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"ipid"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|unsigned
name|long
name|ipid
decl_stmt|;
name|char
modifier|*
name|c
decl_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``ipid''"
argument_list|)
expr_stmt|;
name|ipid
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
operator|&
name|c
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|c
operator|!=
literal|'\0'
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"argument to ipid must"
literal|" be numeric"
argument_list|)
expr_stmt|;
if|if
condition|(
name|ipid
operator|>
literal|65535
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"argument to ipid out"
literal|" of range"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_IPID
expr_stmt|;
name|rule
operator|.
name|fw_ipid
operator|=
operator|(
name|u_short
operator|)
name|ipid
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"ipprecedence"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|u_long
name|ippre
decl_stmt|;
name|char
modifier|*
name|c
decl_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``ipprecedence''"
argument_list|)
expr_stmt|;
name|ippre
operator|=
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
operator|&
name|c
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|c
operator|!=
literal|'\0'
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"argument to ipprecedence"
literal|" must be numeric"
argument_list|)
expr_stmt|;
if|if
condition|(
name|ippre
operator|>
literal|7
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"argument to ipprecedence"
literal|" out of range"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_IPPRE
expr_stmt|;
name|rule
operator|.
name|fw_iptos
operator||=
call|(
name|u_short
call|)
argument_list|(
name|ippre
operator|<<
literal|5
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"iptos"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``iptos''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_IPTOS
expr_stmt|;
name|fill_flags
argument_list|(
operator|&
name|rule
operator|.
name|fw_iptos
argument_list|,
operator|&
name|rule
operator|.
name|fw_ipntos
argument_list|,
name|f_iptos
argument_list|,
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"ipttl"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``ipttl''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_IPTTL
expr_stmt|;
name|rule
operator|.
name|fw_ipttl
operator|=
operator|(
name|u_short
operator|)
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"ipversion"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"ipver"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``ipversion''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_IPVER
expr_stmt|;
name|rule
operator|.
name|fw_ipver
operator|=
operator|(
name|u_short
operator|)
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|rule
operator|.
name|fw_prot
operator|==
name|IPPROTO_TCP
condition|)
block|{
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"established"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_TCPEST
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"setup"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_tcpf
operator||=
name|TH_SYN
expr_stmt|;
name|rule
operator|.
name|fw_tcpnf
operator||=
name|TH_ACK
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_TCPFLG
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tcpflags"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tcpflgs"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``tcpflags''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_TCPFLG
expr_stmt|;
name|fill_flags
argument_list|(
operator|&
name|rule
operator|.
name|fw_tcpf
argument_list|,
operator|&
name|rule
operator|.
name|fw_tcpnf
argument_list|,
name|f_tcpflags
argument_list|,
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tcpoptions"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
operator|||
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tcpopts"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``tcpoptions''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_TCPOPT
expr_stmt|;
name|fill_flags
argument_list|(
operator|&
name|rule
operator|.
name|fw_tcpopt
argument_list|,
operator|&
name|rule
operator|.
name|fw_tcpnopt
argument_list|,
name|f_tcpopts
argument_list|,
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tcpseq"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``tcpseq''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_TCPSEQ
expr_stmt|;
name|rule
operator|.
name|fw_tcpseq
operator|=
name|htonl
argument_list|(
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tcpack"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``tcpack''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_TCPACK
expr_stmt|;
name|rule
operator|.
name|fw_tcpack
operator|=
name|htonl
argument_list|(
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"tcpwin"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``tcpwin''"
argument_list|)
expr_stmt|;
name|rule
operator|.
name|fw_ipflg
operator||=
name|IP_FW_IF_TCPWIN
expr_stmt|;
name|rule
operator|.
name|fw_tcpwin
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|strtoul
argument_list|(
operator|*
name|av
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
else|else
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"unknown or out of order"
literal|" argument ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|rule
operator|.
name|fw_prot
operator|==
name|IPPROTO_ICMP
condition|)
block|{
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"icmptypes"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|NEED1
argument_list|(
literal|"missing argument for ``icmptypes''"
argument_list|)
expr_stmt|;
name|fill_icmptypes
argument_list|(
name|rule
operator|.
name|fw_uar
operator|.
name|fw_icmptypes
argument_list|,
name|av
argument_list|,
operator|&
name|rule
operator|.
name|fw_flg
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
block|}
else|else
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"unknown or out of"
literal|" order argument ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
else|else
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"unknown argument ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
comment|/* No direction specified -> do both directions */
if|if
condition|(
operator|!
operator|(
name|rule
operator|.
name|fw_flg
operator|&
operator|(
name|IP_FW_F_OUT
operator||
name|IP_FW_F_IN
operator|)
operator|)
condition|)
name|rule
operator|.
name|fw_flg
operator||=
operator|(
name|IP_FW_F_OUT
operator||
name|IP_FW_F_IN
operator|)
expr_stmt|;
comment|/* Sanity check interface check, but handle "via" case separately */
if|if
condition|(
name|saw_via
condition|)
block|{
if|if
condition|(
name|rule
operator|.
name|fw_flg
operator|&
name|IP_FW_F_IN
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_IIFACE
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|fw_flg
operator|&
name|IP_FW_F_OUT
condition|)
name|rule
operator|.
name|fw_flg
operator||=
name|IP_FW_F_OIFACE
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|rule
operator|.
name|fw_flg
operator|&
name|IP_FW_F_OIFACE
operator|)
operator|&&
operator|(
name|rule
operator|.
name|fw_flg
operator|&
name|IP_FW_F_IN
operator|)
condition|)
block|{
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"can't check xmit interface of incoming"
literal|" packets"
argument_list|)
expr_stmt|;
block|}
comment|/* frag may not be used in conjunction with ports or TCP flags */
if|if
condition|(
name|rule
operator|.
name|fw_flg
operator|&
name|IP_FW_F_FRAG
condition|)
block|{
if|if
condition|(
name|rule
operator|.
name|fw_tcpf
operator|||
name|rule
operator|.
name|fw_tcpnf
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"can't mix 'frag' and tcpflags"
argument_list|)
expr_stmt|;
if|if
condition|(
name|rule
operator|.
name|fw_nports
condition|)
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"can't mix 'frag' and port"
literal|" specifications"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|rule
operator|.
name|fw_flg
operator|&
name|IP_FW_F_PRN
condition|)
block|{
if|if
condition|(
operator|!
name|rule
operator|.
name|fw_logamount
condition|)
block|{
name|size_t
name|len
init|=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
decl_stmt|;
if|if
condition|(
name|sysctlbyname
argument_list|(
literal|"net.inet.ip.fw.verbose_limit"
argument_list|,
operator|&
name|rule
operator|.
name|fw_logamount
argument_list|,
operator|&
name|len
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"sysctlbyname(\"%s\")"
argument_list|,
literal|"net.inet.ip.fw.verbose_limit"
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|rule
operator|.
name|fw_logamount
operator|==
operator|-
literal|1
condition|)
name|rule
operator|.
name|fw_logamount
operator|=
literal|0
expr_stmt|;
name|rule
operator|.
name|fw_loghighest
operator|=
name|rule
operator|.
name|fw_logamount
expr_stmt|;
block|}
name|done
label|:
name|i
operator|=
sizeof|sizeof
argument_list|(
name|rule
argument_list|)
expr_stmt|;
if|if
condition|(
name|getsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_FW_ADD
argument_list|,
operator|&
name|rule
argument_list|,
operator|&
name|i
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|err
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"getsockopt(%s)"
argument_list|,
literal|"IP_FW_ADD"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|do_quiet
condition|)
name|show_ipfw
argument_list|(
operator|&
name|rule
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|zero
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
name|struct
name|ip_fw
name|rule
decl_stmt|;
name|int
name|failed
init|=
name|EX_OK
decl_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
operator|!
name|ac
condition|)
block|{
comment|/* clear all entries */
if|if
condition|(
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_FW_ZERO
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
name|err
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"setsockopt(%s)"
argument_list|,
literal|"IP_FW_ZERO"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|do_quiet
condition|)
name|printf
argument_list|(
literal|"Accounting cleared.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|memset
argument_list|(
operator|&
name|rule
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
name|rule
argument_list|)
expr_stmt|;
while|while
condition|(
name|ac
condition|)
block|{
comment|/* Rule number */
if|if
condition|(
name|isdigit
argument_list|(
operator|*
operator|*
name|av
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_number
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_FW_ZERO
argument_list|,
operator|&
name|rule
argument_list|,
sizeof|sizeof
name|rule
argument_list|)
condition|)
block|{
name|warn
argument_list|(
literal|"rule %u: setsockopt(IP_FW_ZERO)"
argument_list|,
name|rule
operator|.
name|fw_number
argument_list|)
expr_stmt|;
name|failed
operator|=
name|EX_UNAVAILABLE
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|do_quiet
condition|)
name|printf
argument_list|(
literal|"Entry %d cleared\n"
argument_list|,
name|rule
operator|.
name|fw_number
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"invalid rule number ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|failed
operator|!=
name|EX_OK
condition|)
name|exit
argument_list|(
name|failed
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|resetlog
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
name|struct
name|ip_fw
name|rule
decl_stmt|;
name|int
name|failed
init|=
name|EX_OK
decl_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
operator|!
name|ac
condition|)
block|{
comment|/* clear all entries */
if|if
condition|(
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_FW_RESETLOG
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
name|err
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"setsockopt(IP_FW_RESETLOG)"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|do_quiet
condition|)
name|printf
argument_list|(
literal|"Logging counts reset.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|memset
argument_list|(
operator|&
name|rule
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
name|rule
argument_list|)
expr_stmt|;
while|while
condition|(
name|ac
condition|)
block|{
comment|/* Rule number */
if|if
condition|(
name|isdigit
argument_list|(
operator|*
operator|*
name|av
argument_list|)
condition|)
block|{
name|rule
operator|.
name|fw_number
operator|=
name|atoi
argument_list|(
operator|*
name|av
argument_list|)
expr_stmt|;
name|av
operator|++
expr_stmt|;
name|ac
operator|--
expr_stmt|;
if|if
condition|(
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|IP_FW_RESETLOG
argument_list|,
operator|&
name|rule
argument_list|,
sizeof|sizeof
name|rule
argument_list|)
condition|)
block|{
name|warn
argument_list|(
literal|"rule %u: setsockopt(IP_FW_RESETLOG)"
argument_list|,
name|rule
operator|.
name|fw_number
argument_list|)
expr_stmt|;
name|failed
operator|=
name|EX_UNAVAILABLE
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|do_quiet
condition|)
name|printf
argument_list|(
literal|"Entry %d logging count reset\n"
argument_list|,
name|rule
operator|.
name|fw_number
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|errx
argument_list|(
name|EX_DATAERR
argument_list|,
literal|"invalid rule number ``%s''"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|failed
operator|!=
name|EX_OK
condition|)
name|exit
argument_list|(
name|failed
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|flush
parameter_list|()
block|{
if|if
condition|(
operator|!
name|do_force
operator|&&
operator|!
name|do_quiet
condition|)
block|{
comment|/* need to ask user */
name|int
name|c
decl_stmt|;
name|printf
argument_list|(
literal|"Are you sure? [yn] "
argument_list|)
expr_stmt|;
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
do|do
block|{
name|c
operator|=
name|toupper
argument_list|(
name|getc
argument_list|(
name|stdin
argument_list|)
argument_list|)
expr_stmt|;
while|while
condition|(
name|c
operator|!=
literal|'\n'
operator|&&
name|getc
argument_list|(
name|stdin
argument_list|)
operator|!=
literal|'\n'
condition|)
if|if
condition|(
name|feof
argument_list|(
name|stdin
argument_list|)
condition|)
return|return;
comment|/* and do not flush */
block|}
do|while
condition|(
name|c
operator|!=
literal|'Y'
operator|&&
name|c
operator|!=
literal|'N'
condition|)
do|;
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|c
operator|==
literal|'N'
condition|)
comment|/* user said no */
return|return;
block|}
if|if
condition|(
name|setsockopt
argument_list|(
name|s
argument_list|,
name|IPPROTO_IP
argument_list|,
name|do_pipe
condition|?
name|IP_DUMMYNET_FLUSH
else|:
name|IP_FW_FLUSH
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
name|err
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"setsockopt(IP_%s_FLUSH)"
argument_list|,
name|do_pipe
condition|?
literal|"DUMMYNET"
else|:
literal|"FW"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|do_quiet
condition|)
name|printf
argument_list|(
literal|"Flushed all %s.\n"
argument_list|,
name|do_pipe
condition|?
literal|"pipes"
else|:
literal|"rules"
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|ipfw_main
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
modifier|*
name|av
parameter_list|)
block|{
name|int
name|ch
decl_stmt|;
if|if
condition|(
name|ac
operator|==
literal|1
condition|)
name|show_usage
argument_list|()
expr_stmt|;
comment|/* Set the force flag for non-interactive processes */
name|do_force
operator|=
operator|!
name|isatty
argument_list|(
name|STDIN_FILENO
argument_list|)
expr_stmt|;
name|optind
operator|=
name|optreset
operator|=
literal|1
expr_stmt|;
while|while
condition|(
operator|(
name|ch
operator|=
name|getopt
argument_list|(
name|ac
argument_list|,
name|av
argument_list|,
literal|"s:adefNqtv"
argument_list|)
operator|)
operator|!=
operator|-
literal|1
condition|)
switch|switch
condition|(
name|ch
condition|)
block|{
case|case
literal|'s'
case|:
comment|/* sort */
name|do_sort
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'a'
case|:
name|do_acct
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'d'
case|:
name|do_dynamic
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'e'
case|:
name|do_expired
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'f'
case|:
name|do_force
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'N'
case|:
name|do_resolv
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'q'
case|:
name|do_quiet
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'t'
case|:
name|do_time
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'v'
case|:
comment|/* verbose */
name|verbose
operator|++
expr_stmt|;
break|break;
default|default:
name|show_usage
argument_list|()
expr_stmt|;
block|}
name|ac
operator|-=
name|optind
expr_stmt|;
name|av
operator|+=
name|optind
expr_stmt|;
if|if
condition|(
operator|*
name|av
operator|==
name|NULL
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"bad arguments, for usage summary ``ipfw''"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"pipe"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|do_pipe
operator|=
literal|1
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"queue"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|do_pipe
operator|=
literal|2
expr_stmt|;
name|ac
operator|--
expr_stmt|;
name|av
operator|++
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|ac
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"pipe requires arguments"
argument_list|)
expr_stmt|;
comment|/* allow argument swapping -- pipe config xxx or pipe xxx config */
if|if
condition|(
name|ac
operator|>
literal|1
operator|&&
operator|*
name|av
index|[
literal|0
index|]
operator|>=
literal|'0'
operator|&&
operator|*
name|av
index|[
literal|0
index|]
operator|<=
literal|'9'
condition|)
block|{
name|char
modifier|*
name|p
init|=
name|av
index|[
literal|0
index|]
decl_stmt|;
name|av
index|[
literal|0
index|]
operator|=
name|av
index|[
literal|1
index|]
expr_stmt|;
name|av
index|[
literal|1
index|]
operator|=
name|p
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"add"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|add
argument_list|(
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|do_pipe
operator|&&
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"config"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|config_pipe
argument_list|(
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"delete"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|delete
argument_list|(
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"flush"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|flush
argument_list|()
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"zero"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|zero
argument_list|(
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"resetlog"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|resetlog
argument_list|(
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"print"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|list
argument_list|(
operator|--
name|ac
argument_list|,
operator|++
name|av
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"list"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
name|list
argument_list|(
operator|--
name|ac
argument_list|,
operator|++
name|av
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strncmp
argument_list|(
operator|*
name|av
argument_list|,
literal|"show"
argument_list|,
name|strlen
argument_list|(
operator|*
name|av
argument_list|)
argument_list|)
condition|)
block|{
name|do_acct
operator|++
expr_stmt|;
name|list
argument_list|(
operator|--
name|ac
argument_list|,
operator|++
name|av
argument_list|)
expr_stmt|;
block|}
else|else
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"bad command `%s'"
argument_list|,
operator|*
name|av
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|ipfw_readfile
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
define|#
directive|define
name|MAX_ARGS
value|32
define|#
directive|define
name|WHITESP
value|" \t\f\v\n\r"
name|char
name|buf
index|[
name|BUFSIZ
index|]
decl_stmt|;
name|char
modifier|*
name|a
decl_stmt|,
modifier|*
name|p
decl_stmt|,
modifier|*
name|args
index|[
name|MAX_ARGS
index|]
decl_stmt|,
modifier|*
name|cmd
init|=
name|NULL
decl_stmt|;
name|char
name|linename
index|[
literal|10
index|]
decl_stmt|;
name|int
name|i
init|=
literal|0
decl_stmt|,
name|lineno
init|=
literal|0
decl_stmt|,
name|qflag
init|=
literal|0
decl_stmt|,
name|pflag
init|=
literal|0
decl_stmt|,
name|status
decl_stmt|;
name|FILE
modifier|*
name|f
init|=
name|NULL
decl_stmt|;
name|pid_t
name|preproc
init|=
literal|0
decl_stmt|;
name|int
name|c
decl_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
name|getopt
argument_list|(
name|ac
argument_list|,
name|av
argument_list|,
literal|"D:U:p:q"
argument_list|)
operator|)
operator|!=
operator|-
literal|1
condition|)
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'D'
case|:
if|if
condition|(
operator|!
name|pflag
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"-D requires -p"
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
name|MAX_ARGS
operator|-
literal|2
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"too many -D or -U options"
argument_list|)
expr_stmt|;
name|args
index|[
name|i
operator|++
index|]
operator|=
literal|"-D"
expr_stmt|;
name|args
index|[
name|i
operator|++
index|]
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'U'
case|:
if|if
condition|(
operator|!
name|pflag
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"-U requires -p"
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
name|MAX_ARGS
operator|-
literal|2
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"too many -D or -U options"
argument_list|)
expr_stmt|;
name|args
index|[
name|i
operator|++
index|]
operator|=
literal|"-U"
expr_stmt|;
name|args
index|[
name|i
operator|++
index|]
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'p'
case|:
name|pflag
operator|=
literal|1
expr_stmt|;
name|cmd
operator|=
name|optarg
expr_stmt|;
name|args
index|[
literal|0
index|]
operator|=
name|cmd
expr_stmt|;
name|i
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'q'
case|:
name|qflag
operator|=
literal|1
expr_stmt|;
break|break;
default|default:
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"bad arguments, for usage"
literal|" summary ``ipfw''"
argument_list|)
expr_stmt|;
block|}
name|av
operator|+=
name|optind
expr_stmt|;
name|ac
operator|-=
name|optind
expr_stmt|;
if|if
condition|(
name|ac
operator|!=
literal|1
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"extraneous filename arguments"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|f
operator|=
name|fopen
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"fopen: %s"
argument_list|,
name|av
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|pflag
condition|)
block|{
comment|/* pipe through preprocessor (cpp or m4) */
name|int
name|pipedes
index|[
literal|2
index|]
decl_stmt|;
name|args
index|[
name|i
index|]
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|pipe
argument_list|(
name|pipedes
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|err
argument_list|(
name|EX_OSERR
argument_list|,
literal|"cannot create pipe"
argument_list|)
expr_stmt|;
switch|switch
condition|(
operator|(
name|preproc
operator|=
name|fork
argument_list|()
operator|)
condition|)
block|{
case|case
operator|-
literal|1
case|:
name|err
argument_list|(
name|EX_OSERR
argument_list|,
literal|"cannot fork"
argument_list|)
expr_stmt|;
case|case
literal|0
case|:
comment|/* child */
if|if
condition|(
name|dup2
argument_list|(
name|fileno
argument_list|(
name|f
argument_list|)
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
operator|||
name|dup2
argument_list|(
name|pipedes
index|[
literal|1
index|]
argument_list|,
literal|1
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|err
argument_list|(
name|EX_OSERR
argument_list|,
literal|"dup2()"
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|pipedes
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|pipedes
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|execvp
argument_list|(
name|cmd
argument_list|,
name|args
argument_list|)
expr_stmt|;
name|err
argument_list|(
name|EX_OSERR
argument_list|,
literal|"execvp(%s) failed"
argument_list|,
name|cmd
argument_list|)
expr_stmt|;
default|default:
comment|/* parent */
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|pipedes
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|f
operator|=
name|fdopen
argument_list|(
name|pipedes
index|[
literal|0
index|]
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|int
name|savederrno
init|=
name|errno
decl_stmt|;
operator|(
name|void
operator|)
name|kill
argument_list|(
name|preproc
argument_list|,
name|SIGTERM
argument_list|)
expr_stmt|;
name|errno
operator|=
name|savederrno
expr_stmt|;
name|err
argument_list|(
name|EX_OSERR
argument_list|,
literal|"fdopen()"
argument_list|)
expr_stmt|;
block|}
block|}
block|}
while|while
condition|(
name|fgets
argument_list|(
name|buf
argument_list|,
name|BUFSIZ
argument_list|,
name|f
argument_list|)
condition|)
block|{
name|lineno
operator|++
expr_stmt|;
name|sprintf
argument_list|(
name|linename
argument_list|,
literal|"Line %d"
argument_list|,
name|lineno
argument_list|)
expr_stmt|;
name|args
index|[
literal|0
index|]
operator|=
name|linename
expr_stmt|;
if|if
condition|(
operator|*
name|buf
operator|==
literal|'#'
condition|)
continue|continue;
if|if
condition|(
operator|(
name|p
operator|=
name|strchr
argument_list|(
name|buf
argument_list|,
literal|'#'
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
operator|*
name|p
operator|=
literal|'\0'
expr_stmt|;
name|i
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|qflag
condition|)
name|args
index|[
name|i
operator|++
index|]
operator|=
literal|"-q"
expr_stmt|;
for|for
control|(
name|a
operator|=
name|strtok
argument_list|(
name|buf
argument_list|,
name|WHITESP
argument_list|)
init|;
name|a
operator|&&
name|i
operator|<
name|MAX_ARGS
condition|;
name|a
operator|=
name|strtok
argument_list|(
name|NULL
argument_list|,
name|WHITESP
argument_list|)
operator|,
name|i
operator|++
control|)
name|args
index|[
name|i
index|]
operator|=
name|a
expr_stmt|;
if|if
condition|(
name|i
operator|==
operator|(
name|qflag
condition|?
literal|2
else|:
literal|1
operator|)
condition|)
continue|continue;
if|if
condition|(
name|i
operator|==
name|MAX_ARGS
condition|)
name|errx
argument_list|(
name|EX_USAGE
argument_list|,
literal|"%s: too many arguments"
argument_list|,
name|linename
argument_list|)
expr_stmt|;
name|args
index|[
name|i
index|]
operator|=
name|NULL
expr_stmt|;
name|ipfw_main
argument_list|(
name|i
argument_list|,
name|args
argument_list|)
expr_stmt|;
block|}
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
if|if
condition|(
name|pflag
condition|)
block|{
if|if
condition|(
name|waitpid
argument_list|(
name|preproc
argument_list|,
operator|&
name|status
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|errx
argument_list|(
name|EX_OSERR
argument_list|,
literal|"waitpid()"
argument_list|)
expr_stmt|;
if|if
condition|(
name|WIFEXITED
argument_list|(
name|status
argument_list|)
operator|&&
name|WEXITSTATUS
argument_list|(
name|status
argument_list|)
operator|!=
name|EX_OK
condition|)
name|errx
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"preprocessor exited with status %d"
argument_list|,
name|WEXITSTATUS
argument_list|(
name|status
argument_list|)
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|WIFSIGNALED
argument_list|(
name|status
argument_list|)
condition|)
name|errx
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"preprocessor exited with signal %d"
argument_list|,
name|WTERMSIG
argument_list|(
name|status
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_function
name|int
name|main
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
name|av
index|[]
parameter_list|)
block|{
name|s
operator|=
name|socket
argument_list|(
name|AF_INET
argument_list|,
name|SOCK_RAW
argument_list|,
name|IPPROTO_RAW
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|<
literal|0
condition|)
name|err
argument_list|(
name|EX_UNAVAILABLE
argument_list|,
literal|"socket"
argument_list|)
expr_stmt|;
name|setbuf
argument_list|(
name|stdout
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* 	 * Only interpret the last command line argument as a file to 	 * be preprocessed if it is specified as an absolute pathname. 	 */
if|if
condition|(
name|ac
operator|>
literal|1
operator|&&
name|av
index|[
name|ac
operator|-
literal|1
index|]
index|[
literal|0
index|]
operator|==
literal|'/'
operator|&&
name|access
argument_list|(
name|av
index|[
name|ac
operator|-
literal|1
index|]
argument_list|,
name|R_OK
argument_list|)
operator|==
literal|0
condition|)
name|ipfw_readfile
argument_list|(
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
else|else
name|ipfw_main
argument_list|(
name|ac
argument_list|,
name|av
argument_list|)
expr_stmt|;
return|return
name|EX_OK
return|;
block|}
end_function

end_unit

