...
begin_comment
comment|/*  * show_ipfw() prints the body of an ipfw rule.  * Because the standard rule has at least proto src_ip dst_ip, we use  * a helper function to produce these entries if not provided explicitly.  *  * Special case: if we have provided a MAC header, and no IP specs,  * just leave it alone.  * Also, if we have providea a MAC header and no IP protocol, print it  * as "all" instead of "ip".  */
end_comment
