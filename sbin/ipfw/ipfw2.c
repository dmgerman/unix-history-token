...
begin_endif
unit|static struct _s_x f_iptos[] = { 	{ "lowdelay",	IPTOS_LOWDELAY}, 	{ "throughput",	IPTOS_THROUGHPUT}, 	{ "reliability", IPTOS_RELIABILITY}, 	{ "mincost",	IPTOS_MINCOST}, 	{ "congestion",	IPTOS_CE}, 	{ "ecntransport", IPTOS_ECT}, 	{ "ip tos option", 0}, 	{ NULL,	0 } };
endif|#
directive|endif
end_endif
...
begin_comment
comment|/*  * Like strtol, but also translates service names into port numbers  * for some protocols.  * In particular:  *	proto == -1 disables the protocol check;  *	proto == IPPROTO_ETHERTYPE looks up an internal table  *	proto ==<some value in /etc/protocols> matches the values there.  */
end_comment
...
argument_list|)
return|;
comment|/* 	 * find separator and replace with a '\0' 	 */
for|for
control|(
name|s1
operator|=
...
begin_endif
unit|static struct _s_x icmpcodes[] = {       { "net",			ICMP_UNREACH_NET },       { "host",			ICMP_UNREACH_HOST },       { "protocol",		ICMP_UNREACH_PROTOCOL },       { "port",			ICMP_UNREACH_PORT },       { "needfrag",		ICMP_UNREACH_NEEDFRAG },       { "srcfail",		ICMP_UNREACH_SRCFAIL },       { "net-unknown",		ICMP_UNREACH_NET_UNKNOWN },       { "host-unknown",		ICMP_UNREACH_HOST_UNKNOWN },       { "isolated",		ICMP_UNREACH_ISOLATED },       { "net-prohib",		ICMP_UNREACH_NET_PROHIB },       { "host-prohib",		ICMP_UNREACH_HOST_PROHIB },       { "tosnet",		ICMP_UNREACH_TOSNET },       { "toshost",		ICMP_UNREACH_TOSHOST },       { "filter-prohib",	ICMP_UNREACH_FILTER_PROHIB },       { "host-precedence",	ICMP_UNREACH_HOST_PRECEDENCE },       { "precedence-cutoff",	ICMP_UNREACH_PRECEDENCE_CUTOFF },       { NULL, 0 } };  static void fill_reject_code(u_short *codep, char *str) { 	int val; 	char *s;  	val = strtoul(str,&s, 0); 	if (s == str || *s != '\0' || val>= 0x100) 		val = match_token(icmpcodes, str); 	if (val<= 0) 		errx(EX_DATAERR, "unknown ICMP unreachable code ``%s''", str); 	*codep = val; 	return; }  static void print_reject_code(u_int32_t code) { 	char *s = match_value(icmpcodes, code);  	if (s != NULL) 		printf("%s", s); 	else 		printf("%u", code); }
endif|#
directive|endif
end_endif
...
begin_comment
comment|/*  * show_ipfw() prints the body of an ipfw rule.  * Because the standard rule has at least proto src_ip dst_ip, we use  * a helper function to produce these entries if not provided explicitly.  */
end_comment
...
begin_endif
unit|static void fill_icmptypes(unsigned *types, char **vp, u_int *fw_flg) { 	unsigned long icmptype; 	char *c = *vp;  	while (*c) { 		if (*c == ',') 			++c;  		icmptype = strtoul(c,&c, 0);  		if (*c != ','&& *c != '\0') 			errx(EX_DATAERR, "invalid ICMP type");  		if (icmptype>= IP_FW_ICMPTYPES_DIM * sizeof(unsigned) * 8) 			errx(EX_DATAERR, "ICMP type out of range");  		types[icmptype / (sizeof(unsigned) * 8)] |= 			1<< (icmptype % (sizeof(unsigned) * 8)); 		*fw_flg |= IP_FW_F_ICMPBIT; 	} }
endif|#
directive|endif
end_endif
...
condition|)
block|{
comment|/* we have a non-null port */
name|cmd
operator|+=
name|F_LEN
argument_list|(
...
directive|if
literal|0
block|} else if (!strncmp(*av, "reject", strlen(*av))) { 		rule.fw_flg |= IP_FW_F_REJECT; av++; ac--; 		rule.fw_reject_code = ICMP_UNREACH_HOST; 	} else if (!strncmp(*av, "reset", strlen(*av))) { 		rule.fw_flg |= IP_FW_F_REJECT; av++; ac--; 		rule.fw_reject_code = ICMP_REJECT_RST;
comment|/* check TCP later */
block|} else if (!strncmp(*av, "unreach", strlen(*av))) { 		rule.fw_flg |= IP_FW_F_REJECT; av++; ac--; 		fill_reject_code(&rule.fw_reject_code, *av); av++; ac--; 	}
endif|#
directive|endif
comment|/* XXX other actions */
...
argument_list|)
expr_stmt|;
comment|/* 	 * "from", mandatory (unless we have a MAC address) 	 */
if|if
condition|(
operator|!
name|ac
...
name|have_mac
condition|)
comment|/* we do not need a "to" address */
goto|goto
name|read_to
goto|;
name|errx
...
name|O_IP_SRC_MASK
expr_stmt|;
comment|/* otherwise len will be zero and the command skipped */
name|ac
operator|--
expr_stmt|;
name|av
...
name|cmd
expr_stmt|;
comment|/* in case we need to backtrack */
name|cmd
operator|=
name|next_cmd
argument_list|(
...
literal|0
comment|/* XXX todo */
block|do_options: 	while (ac) { 		} else if (!strncmp(*av, "ipprecedence", strlen(*av))) { 			u_long ippre; 			char *c;  			av++; ac--; 			NEED1("missing argument for ``ipprecedence''"); 			ippre = strtoul(*av,&c, 0); 			if (*c != '\0') 				errx(EX_DATAERR, "argument to ipprecedence" 					" must be numeric"); 			if (ippre> 7) 				errx(EX_DATAERR, "argument to ipprecedence" 					" out of range"); 			rule.fw_ipflg |= IP_FW_IF_IPPRE; 			rule.fw_iptos |= (u_short)(ippre<< 5); 			av++; ac--; 		} else if (!strncmp(*av, "iptos", strlen(*av))) { 			av++; ac--; 			NEED1("missing argument for ``iptos''"); 			rule.fw_ipflg |= IP_FW_IF_IPTOS; 			fill_flags(&rule.fw_iptos,&rule.fw_ipntos, 			    f_iptos, av); 			av++; ac--; 		} else if (rule.fw_prot == IPPROTO_ICMP) { 			if (!strncmp(*av, "icmptypes", strlen(*av))) { 				av++; ac--; 				NEED1("missing argument for ``icmptypes''"); 				fill_icmptypes(rule.fw_uar.fw_icmptypes, 				    av,&rule.fw_flg); 				av++; ac--; 			} 		} 	}
endif|#
directive|endif
comment|/* XXX todo */
name|done
