...
begin_comment
comment|/*  *  * ===================================  * HARP  |  Host ATM Research Platform  * ===================================  *  *  * This Host ATM Research Platform ("HARP") file (the "Software") is  * made available by Network Computing Services, Inc. ("NetworkCS")  * "AS IS".  NetworkCS does not provide maintenance, improvements or  * support of any kind.  *  * NETWORKCS MAKES NO WARRANTIES OR REPRESENTATIONS, EXPRESS OR IMPLIED,  * INCLUDING, BUT NOT LIMITED TO, IMPLIED WARRANTIES OF MERCHANTABILITY  * AND FITNESS FOR A PARTICULAR PURPOSE, AS TO ANY ELEMENT OF THE  * SOFTWARE OR ANY SUPPORT PROVIDED IN CONNECTION WITH THIS SOFTWARE.  * In no event shall NetworkCS be responsible for any damages, including  * but not limited to consequential damages, arising from or relating to  * any use of the Software or related support.  *  * Copyright 1994-1998 Network Computing Services, Inc.  *  * Copies of this Software may be made, however, the above copyright  * notice must be reproduced on all copies.  *  *	@(#) $Id: fore_dnld.c,v 1.15 1998/08/26 23:29:32 mks Exp $  *  */
end_comment
...
name|RCSid
init|=
literal|"@(#) $Id: fore_dnld.c,v 1.15 1998/08/26 23:29:32 mks Exp $"
decl_stmt|;
end_decl_stmt
...
argument|, dirname, 							base, 							    air->acp_bustype ); 						if ( stat ( filename,&sbuf ) != -
literal|1
argument|) 						    sndfile = filename; 					    } else 						sndfile = filename; 					} else 					    sndfile = filename; 				    } else 					sndfile = filename; 				    break; 				case DEV_FORE_PCA200E: 				    sprintf ( filename,
literal|"%s.bin"
argument|, base ); 				    if ( stat ( filename,&sbuf ) == -
literal|1
argument|) { 					sprintf ( filename,
literal|"%s/%s.bin"
argument|, 					    dirname, base ); 					if ( stat ( filename,&sbuf ) != -
literal|1
argument|) { 						sndfile = filename; 					} 				    } else 					sndfile = filename; 				    break; 			        } 			    } else 				sndfile = objfile;  			    if ( ext&& !binary ) 				err = sendfile ( sndfile ); 			    else 				err = sendbinfile ( sndfile, ram );  			    if ( err ) { 				fprintf(stderr,
literal|"%s download failed\n"
argument|, 					air->acp_intf); 				(void) close(fd); 				continue; 			    }
