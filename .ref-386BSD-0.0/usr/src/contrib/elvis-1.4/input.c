...
index|]
decl_stmt|;
comment|/* the abbreviated form (appended to struct) */
block|}
modifier|*
name|abbrev
struct|;
...
begin_comment
comment|/* This functions lists or defines abbreviations */
end_comment
...
name|smlen
decl_stmt|;
comment|/* length of the small form */
name|int
name|lrg
decl_stmt|;
comment|/* index of the start of the large form */
name|REG
name|struct
name|_AB
modifier|*
name|ab
decl_stmt|;
comment|/* used to move through the abbrev list */
name|struct
name|_AB
modifier|*
name|prev
...
condition|)
block|{
comment|/* list all current abbreviations */
for|for
control|(
name|ab
operator|=
...
return|return;
block|}
comment|/* else one or more arguments.  Parse the first& look up in abbrev[] */
for|for
control|(
name|smlen
operator|=
...
block|}
block|}
comment|/* locate the start of the large form, if any */
for|for
control|(
name|lrg
operator|=
...
condition|)
block|{
comment|/* trying to undo an abbreviation which doesn't exist? */
if|if
condition|(
operator|!
name|ab
...
return|return;
block|}
comment|/* multiple args - [re]define an abbreviation */
if|if
condition|(
name|ab
condition|)
block|{
comment|/* redefining - free the old large form */
name|free
argument_list|(
name|ab
operator|->
...
block|{
comment|/* adding a new definition - make a new struct */
name|ab
operator|=
operator|(
expr|struct
...
begin_comment
comment|/* This function is called from cmd_mkexrc() to save the abbreviations */
end_comment
...
name|fd
decl_stmt|;
comment|/* fd to which the :abbr commands should be written */
block|{
name|REG
name|struct
name|_AB
...
begin_comment
comment|/* This function should be called before each char is inserted.  If the next  * char is non-alphanumeric and we're at the end of a word, then that word  * is checked against the abbrev[] array and expanded, if appropriate.  Upon  * returning from this function, the new char still must be inserted.  */
end_comment
...
name|ch
decl_stmt|;
comment|/* the character to insert */
block|{
name|char
modifier|*
name|word
decl_stmt|;
comment|/* where the word starts */
name|int
name|len
decl_stmt|;
...
name|ab
decl_stmt|;
comment|/* if no abbreviations are in effect, or ch is aphanumeric, then 	 * don't do anything 	 */
if|if
condition|(
operator|!
name|abbrev
...
return|;
block|}
comment|/* see where the preceding word starts */
name|pfetch
argument_list|(
name|markline
argument_list|(
...
operator|++
expr_stmt|;
comment|/* if zero-length, then it isn't a word, really -- so nothing */
if|if
condition|(
name|len
operator|==
...
return|;
block|}
comment|/* look it up in the abbrev list */
for|for
control|(
name|ab
operator|=
...
block|}
block|}
comment|/* not an abbreviation? then do nothing */
if|if
condition|(
operator|!
name|ab
...
return|;
block|}
comment|/* else replace the small form with the large form */
name|add
argument_list|(
name|m
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* return with the cursor after the end of the large form */
return|return
name|m
operator|-
name|len
...
argument_list|)
expr_stmt|;
comment|/* if whitespace& wrapmargin is set& we're 				 * past the warpmargin, then change the 				 * whitespace character into a newline 				 */
if|if
condition|(
operator|(
operator|*
...
directive|ifndef
name|NO_SHOWMATCH
comment|/* show matching "({[" if neceesary */
if|if
condition|(
operator|*
name|o_showmatch
