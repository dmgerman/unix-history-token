...
index|[]
init|=
literal|"@(#)toplev.c	6.3 (Berkeley) 5/8/91"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Change by Bryan Boreham, Kewill, Sun Jul 23 14:41:26 1989.    Added some stuff for unexec-ing a part-done compilation. */
end_comment
...
begin_comment
comment|/* Is this an unexec'd program? */
end_comment
...
begin_comment
comment|/* Is this the process that *was* unexec'd?  */
end_comment
...
begin_comment
comment|/* Keep argv[0] for dumping. */
end_comment

begin_comment
comment|/* @@ Should there be a filename length limit?  */
end_comment
...
begin_comment
comment|/* asm_file_name from the first run. */
end_comment
...
begin_comment
comment|/* 1 => write gdb debugging output (using symout.c).  -g0    2 => write dbx debugging output (using dbxout.c).  -G0  */
end_comment
...
begin_comment
comment|/* Nonzero means schedule instructions within basic blocks.  */
end_comment
...
begin_comment
comment|/* Value of the -G nn switch if the machine needs it.  */
end_comment
...
begin_comment
comment|/* cross referencing flag	*/
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Initialize data in various passes,        checking if this is an unexec'd program - bryan@kewill.uucp.  */
if|if
condition|(
operator|!
name|been_here_before
...
block|{
comment|/* Copy what we output during the first run into this asm file. */
name|FILE
modifier|*
name|prev_file
decl_stmt|;
...
block|{
comment|/* Change by Bryan Boreham, Kewill, Sat Aug  5 19:05:44 1989. 	 We'll be coming back later, so skip the finish. 	 Modified by Michael Tiemann.  */
comment|/* Compilation is now finished except for now.  */
name|parse_time
operator|+=
name|gettime
argument_list|()
...
argument_list|)
expr_stmt|;
comment|/* Warn about any function or variable 	   declared static but not defined.  */
if|if
condition|(
operator|(
name|warn_unused
...
argument_list|)
operator|)
comment|/*&& TREE_CODE (decl) == FUNCTION_DECL  */
operator|&&
name|DECL_INITIAL
argument_list|(
name|decl
...
block|if (write_symbols == GDB_DEBUG)     TIMEVAR (symout_time, 	     { 	       struct stat statbuf; 	       fstat (fileno (finput),&statbuf); 	       symout_types (get_permanent_types ()); 	       symout_top_blocks (globals, gettags ()); 	       symout_finish (name, statbuf.st_ctime); 	     });
endif|#
directive|endif
comment|/* 0 */
...
name|error
argument_list|(
literal|"parameter cannot have special register declaration"
argument_list|)
expr_stmt|;
else|else
block|{
...
literal|0
comment|/* This mode is discontinued */
block|if (write_symbols == GDB_DEBUG) 	{ 	  TIMEVAR (symout_time, 		   {
...
block|symout_types (get_temporary_types ()); 		   }); 	}       else
endif|#
directive|endif
comment|/* 0 */
...
literal|"\n;; Function %s\n\n"
argument|, 			    IDENTIFIER_POINTER (DECL_NAME (decl))); 		   if (lose ==
literal|0
argument|) 		     fprintf (rtl_dump_file,
literal|";; (integrable)\n\n"
...
goto|;
block|}
comment|/* If we have to compile the function now, save its rtl 	 so that its compilation will not affect what others get.  */
if|if
condition|(
name|TREE_INLINE
argument_list|(
...
block|TIMEVAR (shorten_branch_time, 	     { 	       shorten_branches (get_insns ()); 	     });
endif|#
directive|endif
...
block|if (write_symbols == GDB_DEBUG)     {       TIMEVAR (symout_time, 	       { 		 symout_types (get_permanent_types ()); 		 symout_types (get_temporary_types ());  		 DECL_BLOCK_SYMTAB_ADDRESS (decl) 		   = symout_function (DECL_INITIAL (decl), 				      DECL_ARGUMENTS (decl), 0); 		 symout_function_end (); 	       });     }   else
endif|#
directive|endif
comment|/* 0 */
name|get_temporary_types
...
begin_comment
comment|/* Hacky routine to read back entire data segment saved earlier.     Michael Tiemann (tiemann@cygnus.com).  */
end_comment
...
begin_comment
comment|/* Hacky routine to read back entire data segment saved earlier.     Bryan Boreham (bryan@kewill.uucp)    */
end_comment
...
begin_comment
comment|/* Entry point of c++.  Decode command args, then call compile_file.    Exit code is 35 if can't open files, 34 if fatal error,    33 if had nonfatal errors, else success.  */
end_comment
...
name|p
expr_stmt|;
comment|/* Change by Bryan Boreham, Kewill, Sun Aug  6 13:42:47 1989.      Stuff for unexec'ing    */
name|dump_source_name
operator|=
name|argv
index|[
...
directive|if
literal|0
if|else if (!strcmp (str, "symout")) 	    { 	      if (write_symbols == NO_DEBUG) 		write_symbols = GDB_DEBUG; 	      sym_file_name = argv[++i]; 	    }
endif|#
directive|endif
comment|/* 0 */
elseif|else
