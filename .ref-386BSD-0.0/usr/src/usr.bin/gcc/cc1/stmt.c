...
name|cleanups
decl_stmt|;
comment|/* List of cleanup-lists of blocks containing this block, 	     as they were at the locus where this block appears. 	     There is an element for each containing block, 	     ordered innermost containing block first. 	     The element's TREE_VALUE is the cleanup-list of that block, 	     which may be null.  */
name|tree
name|outer_cleanups
decl_stmt|;
...
parameter_list|)
define|\
value|do { int initial_depth = nesting_stack->depth;		\      do { struct nesting *this = STACK;			\ 	  STACK = this->next;				\ 	  nesting_stack = this->all;			\ 	  nesting_depth = this->depth;			\ 	  free (this); }				\      while (nesting_depth> initial_depth); } while (0)
end_define
...
name|stack_level
decl_stmt|;
comment|/* List of lists of cleanup expressions to be run by this goto.      There is one element for each block that this goto is within.      The TREE_VALUE contains the cleanup list of that block as of the      time this goto was seen.      The TREE_ADDRESSABLE flag is 1 for a block that has been exited.  */
name|tree
...
decl_stmt|;
block|{
comment|/* If -W, warn about statements with no side effects,      except inside a ({...}) where they may be useful.  */
if|if
condition|(
name|expr_stmts_for_value
operator|==
...
condition|)
block|{
comment|/* This was a cleanup with no variable.  */
if|if
condition|(
name|cleanup
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Aside from that, only automatic variables need any expansion done.      Static and external variables, and external functions,      will be handled by `assemble_variable' (called from finish_decl).      TYPE_DECL and CONST_DECL require nothing.      PARM_DECLs are handled in `assign_parms'.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
operator|!=
name|BLKmode
comment|/* #error broken /* ??? This is incorrect if X is a MEM.    (SUBREG (MEM)) is not allowed at rtl generation time.  */
condition|?
name|gen_rtx
argument_list|(
name|SUBREG
...
directive|ifdef
name|FIRST_PARM_CALLER_OFFSET
comment|/* Make the right thing happen on the sparc 	 in a function with a struct value and struct arg.  */
if|if
condition|(
name|first_parm_caller_offset
operator|<
...
comment|/* If we arrive at a stack parm while still counting space for reg parms, 	 skip up to the offset for the first stack parm.  */
if|if
condition|(
name|entry_parm
operator|==
...
comment|/* Note that this is not necessarily a valid address. 	 We make it valid later when it is used. 	 It is necessary for the DECL_RTL to be an explicit stack slot, 	 but not necessary for it to be valid.  */
name|stack_parm
operator|=
name|gen_rtx
argument_list|(
...
directive|ifdef
name|PUSH_ROUNDING
comment|/* If this arg will be pushed with a push instruction, 	     note how that will add to its size.  */
if|if
condition|(
name|DECL_MODE
argument_list|(
...
directive|if
literal|0
comment|/* This was probably wrong, but save it just in case.  */
block|rtx unpadded_stack_parm;
comment|/* Determine parm's home in the stack.  */
block|if (stack_parm == 0) 		unpadded_stack_parm 		  = assign_stack_local (GET_MODE (entry_parm), 					int_size_in_bytes (TREE_TYPE (parm))); 	      else 		unpadded_stack_parm 		  = gen_rtx (MEM, passed_mode, 			     memory_address (passed_mode, 					     gen_rtx (PLUS, Pmode, 						      internal_arg_pointer, 						      ARGS_SIZE_RTX (unpadded_stack_offset))));
comment|/* Here we use unpadded_stack_parm because we assume 		 that downward padding is used on big-endian machines 		 where we would want to make the real data in the reg 		 (which is in the low bits) end up at the padded address.  */
endif|#
directive|endif
if|if
condition|(
...
begin_comment
comment|/* Push and pop the current structure value block.  */
end_comment
...
begin_comment
comment|/* ??? Nobody seems to emit the cleanup_label and the cleanups themselves.  */
end_comment
...
control|)
block|{
comment|/* ??? Tiemann thinks this does not work.  */
name|use_variable
argument_list|(
name|XEXP
argument_list|(
