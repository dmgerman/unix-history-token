...
begin_comment
comment|/*  *  Written by Julian Elischer (julian@DIALix.oz.au)  *  *	$Header: /home/ncvs/src/sys/miscfs/devfs/devfs_tree.c,v 1.3 1995/09/06 23:15:53 julian Exp $  */
end_comment
...
name|panic
argument_list|(
literal|"devfs_sinit: dev_add_name failed "
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
begin_comment
comment|/***********************************************************************\ * Add a new element to the devfs backing structure. 			* * If we're creating a root node, then dirname is NULL			* * If devnode is non zero, then we just want to create a link to it	* * This implies that we are not at base level and it's a not a DIR	* *									* * Creates a name node, and (optionally) a new dev_node to go with it	* \***********************************************************************/
end_comment
...
begin_comment
comment|/***************************************************************  * duplicate the backing tree into a tree of nodes hung off the  * mount point given as the argument. Do this by  * calling dev_dup_name which recurses all the way  * up the tree..  * If we are the first plane, just return the base root   **************************************************************/
end_comment
...
begin_comment
comment|/***************************************************************\ * Free a name node (and any below it of it's a directory node)	* * remember that if there are other names pointing to the	* * dev_node then it may not get freed yet			* \***************************************************************/
end_comment
