...
define|#
directive|define
name|ATTR_RMTVALUE_TRANSBLKS
value|8
end_define

begin_comment
comment|/* max # of blks in a transaction */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* length is stored in uint8 */
name|namelen
operator|=
name|strlen
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Do we answer them, or ignore them? 		 */
if|if
condition|(
operator|(
name|error
...
literal|1
expr_stmt|;
comment|/* Determine space new attribute will use, and if it will be inline 	 * or out of line. 	 */
name|size
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * If the attribute list is non-existant or a shortform list, 	 * upgrade it to a single-leaf-block attribute list. 	 */
if|if
condition|(
operator|(
name|dp
...
operator|)
return|;
comment|/* 	 * Do they have permission? 	 */
name|xfs_ilock
argument_list|(
name|dp
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* XXXsup - why on earth are we taking ILOCK_EXCL here??? */
name|xfs_ilock
argument_list|(
name|dp
argument_list|,
...
begin_comment
comment|/*  * Fill in the disk block numbers in the state structure for the buffers  * that are attached to the state structure.  * This is done so that we can quickly reattach ourselves to those buffers  * after some set of transaction commit's has released these buffers.  */
end_comment
...
begin_comment
comment|/*  * Reattach the buffers to the state structure based on the disk block  * numbers stored in the state structure.  * This is done after some set of transaction commit's has released those  * buffers from our grip.  */
end_comment
...
comment|/* bp = incore(mp->m_dev, dblkno, blkcnt, 1); */
name|bp
operator|=
name|xfs_incore
argument_list|(
