...
name|CE_WARN
argument_list|,
literal|"xfs_inotobp: inode number (%llu + %d) maps to a block outside the bounds "
...
name|i_mount
argument_list|,
literal|"corrupt inode %Lu "
literal|"(local format for regular file)."
argument_list|,
operator|(
name|unsigned
name|long
...
argument_list|,
literal|"corrupt inode %Lu "
literal|"(bad size %Ld for local inode)."
argument_list|,
operator|(
name|unsigned
name|long
...
argument_list|,
literal|"corrupt inode %Lu "
literal|"(bad size %d for local fork, size = %d)."
argument_list|,
operator|(
name|unsigned
name|long
...
name|i_mount
argument_list|,
literal|"corrupt inode %Lu ((a)extents = %d)."
argument_list|,
operator|(
name|unsigned
name|long
...
name|i_mount
argument_list|,
literal|"corrupt inode %Lu (btree)."
argument_list|,
operator|(
name|unsigned
name|long
...
begin_comment
comment|/*  * xfs_xlate_dinode_core - translate an xfs_inode_core_t between ondisk  * and native format  *  * buf  = on-disk representation  * dip  = native representation  * dir  = direction - +ve -> disk to native  *                    -ve -> native to disk  */
end_comment
...
begin_comment
comment|/*  * Given a mount structure and an inode number, return a pointer  * to a newly allocated in-core inode corresponding to the given  * inode number.  *  * Initialize the inode's attributes and extent pointers if it  * already has them (it will not if the inode has no links).  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* now that we have an i_mode  we can set Linux inode ops (& unlock) */
...
begin_comment
comment|/*  * Decrement the pin count of the given inode, and wake up  * anyone in xfs_iwait_unpin() if the count goes to 0.  The  * inode must have been previously pinned with a call to xfs_ipin().  */
end_comment
...
condition|)
block|{
comment|/* 		 * If the inode is currently being reclaimed, the 		 * linux inode _and_ the xfs vnode may have been 		 * freed so we cannot reference either of them safely. 		 * Hence we should not try to do anything to them 		 * if the xfs inode is currently in the reclaim 		 * path. 		 * 		 * However, we still need to issue the unpin wakeup 		 * call as the inode reclaim may be blocked waiting for 		 * the inode to become unpinned. 		 */
if|if
condition|(
operator|!
operator|(
...
name|printf
argument_list|(
literal|"xfs_iunpin: REC RECABLE ip %p\n"
argument_list|,
name|ip
argument_list|)
expr_stmt|;
...
name|printf
argument_list|(
literal|"xfs_iflush: ip %p i_ino %lld\n"
argument_list|,
name|ip
argument_list|,
name|ip
...
begin_comment
comment|/*  * Flush all inactive inodes in mp.  */
end_comment
...
begin_comment
comment|/*  * xfs_iaccess: check accessibility of inode for mode.  * This function is quite linuxy now  * probably should be move to a os specfic location  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* FreeBSD local change here */
name|imode
operator|=
operator|(
name|ip
...
argument_list|)
expr_stmt|;
comment|/* 	 * Verify that the MAC policy allows the requested access. 	 */
if|if
condition|(
operator|(
name|error
...
begin_comment
comment|/*  * Return a pointer to the extent record at file index idx.  */
end_comment
...
name|idx
parameter_list|)
comment|/* index of target extent */
block|{
name|ASSERT
argument_list|(
name|idx
...
name|idx
decl_stmt|;
comment|/* ext index in target list */
name|erp
operator|=
...
begin_comment
comment|/*  * Insert new item(s) into the extent records for incore inode  * fork 'ifp'.  'count' new items are inserted at index 'idx'.  */
end_comment
...
name|idx
parameter_list|,
comment|/* starting index of new items */
name|xfs_extnum_t
name|count
parameter_list|,
comment|/* number of inserted items */
name|xfs_bmbt_irec_t
modifier|*
name|new
parameter_list|)
comment|/* items to insert */
block|{
name|xfs_bmbt_rec_t
modifier|*
name|ep
...
name|idx
parameter_list|,
comment|/* index to begin adding exts */
name|int
name|ext_diff
parameter_list|)
comment|/* number of extents to add */
block|{
name|int
name|byte_diff
decl_stmt|;
comment|/* new bytes being added */
name|int
name|new_size
decl_stmt|;
comment|/* size of extents after adding */
name|xfs_extnum_t
name|nextents
decl_stmt|;
...
name|byte_diff
expr_stmt|;
comment|/* 	 * If the new number of extents (nextents + ext_diff) 	 * fits inside the inode, then continue to use the inline 	 * extent buffer. 	 */
if|if
condition|(
name|nextents
operator|+
...
expr_stmt|;
block|}
comment|/* 	 * Otherwise use a linear (direct) extent list. 	 * If the extents are currently inside the inode, 	 * xfs_iext_realloc_direct will switch us from 	 * inline to direct extent allocation mode. 	 */
elseif|else
if|if
condition|(
name|nextents
...
expr_stmt|;
block|}
comment|/* Extents fit in target extent page */
if|if
condition|(
name|erp
operator|&&
...
expr_stmt|;
block|}
comment|/* Insert a new extent page */
elseif|else
if|if
condition|(
name|erp
...
expr_stmt|;
block|}
comment|/* 		 * If extent(s) are being appended to the last page in 		 * the indirection array and the new extent(s) don't fit 		 * in the page, then erp is NULL and erp_idx is set to 		 * the next index needed in the indirection array. 		 */
else|else
block|{
name|int
name|count
...
name|erp_idx
parameter_list|,
comment|/* target extent irec index */
name|xfs_extnum_t
name|idx
parameter_list|,
comment|/* index within target list */
name|int
name|count
parameter_list|)
comment|/* new extents being added */
block|{
name|int
name|byte_diff
decl_stmt|;
...
name|ext_cnt
decl_stmt|;
comment|/* new extents still needed */
name|xfs_extnum_t
name|nex2
decl_stmt|;
comment|/* extents after idx + count */
name|xfs_bmbt_rec_t
modifier|*
name|nex2_ep
init|=
name|NULL
decl_stmt|;
comment|/* temp list for nex2 extents */
name|int
name|nlists
decl_stmt|;
comment|/* number of irec's (lists) */
name|ASSERT
argument_list|(
name|ifp
operator|->
...
name|XFS_IEXT_BUFSZ
expr_stmt|;
comment|/* 	 * Save second part of target extent list 	 * (all extents past */
if|if
condition|(
name|nex2
condition|)
...
expr_stmt|;
block|}
comment|/* 	 * Add the new extents to the end of the target 	 * list, then allocate new irec record(s) and 	 * extent buffer(s) as needed to store the rest 	 * of the new extents. 	 */
name|ext_cnt
operator|=
name|count
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* Add nex2 extents back to indirection array */
if|if
condition|(
name|nex2
condition|)
...
literal|0
expr_stmt|;
comment|/* 		 * If nex2 extents fit in the current page, append 		 * nex2_ep after the new extents. 		 */
if|if
condition|(
name|nex2
operator|<=
...
expr_stmt|;
block|}
comment|/* 		 * Otherwise, check if space is available in the 		 * next page. 		 */
elseif|else
if|if
condition|(
operator|(
...
operator|++
expr_stmt|;
comment|/* Create a hole for nex2 extents */
name|memmove
argument_list|(
operator|&
name|erp
...
expr_stmt|;
block|}
comment|/* 		 * Final choice, create a new extent page for 		 * nex2 extents. 		 */
else|else
block|{
name|erp_idx
operator|++
...
name|idx
parameter_list|,
comment|/* index to begin removing exts */
name|int
name|ext_diff
parameter_list|)
comment|/* number of extents to remove */
block|{
name|xfs_extnum_t
name|nextents
decl_stmt|;
...
name|new_size
decl_stmt|;
comment|/* size of extents after removal */
name|ASSERT
argument_list|(
name|ext_diff
operator|>
...
begin_comment
comment|/*  * This removes ext_diff extents from the inline buffer, beginning  * at extent index idx.  */
end_comment
...
return|return;
block|}
comment|/* Move extents up in the list (if needed) */
if|if
condition|(
name|idx
operator|+
...
argument_list|)
expr_stmt|;
comment|/* 	 * Reallocate the direct extent list. If the extents 	 * will fit inside the inode then xfs_iext_realloc_direct 	 * will switch from direct to inline extent allocation 	 * mode for us. 	 */
name|xfs_iext_realloc_direct
argument_list|(
name|ifp
argument_list|,
...
name|idx
parameter_list|,
comment|/* index to begin removing extents */
name|int
name|count
parameter_list|)
comment|/* number of extents to remove */
...
name|erp
decl_stmt|;
comment|/* indirection array pointer */
name|int
name|erp_idx
init|=
literal|0
decl_stmt|;
comment|/* indirection array index */
name|xfs_extnum_t
name|ext_cnt
decl_stmt|;
comment|/* extents left to remove */
name|xfs_extnum_t
name|ext_diff
decl_stmt|;
comment|/* extents to remove in current list */
name|xfs_extnum_t
name|nex1
decl_stmt|;
comment|/* number of extents before idx */
name|xfs_extnum_t
name|nex2
decl_stmt|;
comment|/* extents after idx + count */
...
name|nlists
decl_stmt|;
comment|/* entries in indirection array */
name|int
name|page_idx
init|=
name|idx
decl_stmt|;
comment|/* index in target extent list */
name|ASSERT
argument_list|(
name|ifp
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 		 * Check for deletion of entire list; 		 * xfs_iext_irec_remove() updates extent offsets. 		 */
if|if
condition|(
name|ext_diff
operator|==
...
block|}
block|}
comment|/* Move extents up (if needed) */
if|if
condition|(
name|nex2
condition|)
...
argument_list|)
expr_stmt|;
comment|/* Update remaining counters */
name|erp
operator|->
name|er_extcount
operator|-=
...
begin_comment
comment|/*  * Create, destroy, or resize a linear (direct) block of extents.  */
end_comment
...
name|new_size
parameter_list|)
comment|/* new size of extents */
block|{
name|int
name|rnew_size
decl_stmt|;
comment|/* real new size of extents */
name|rnew_size
operator|=
name|new_size
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* Resize direct extent list and zero any new bytes */
elseif|else
if|if
condition|(
name|ifp
...
condition|)
block|{
comment|/* Check if extents will fit inside the inode */
if|if
condition|(
name|new_size
operator|<=
...
block|}
block|}
comment|/* 	 * Switch from the inline extent buffer to a direct 	 * extent list. Be sure to include the inline extent 	 * bytes in new_size. 	 */
else|else
block|{
name|new_size
operator|+=
...
begin_comment
comment|/*  * Switch from linear (direct) extent records to inline buffer.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * The inline buffer was zeroed when we switched 	 * from inline to direct extent allocation mode, 	 * so we don't need to clear it here. 	 */
name|memcpy
argument_list|(
name|ifp
operator|->
...
begin_comment
comment|/*  * Resize an extent indirection array to new_size bytes.  */
end_comment
...
name|new_size
parameter_list|)
comment|/* new indirection array size */
block|{
name|int
name|nlists
decl_stmt|;
comment|/* number of irec's (ex lists) */
name|int
name|size
decl_stmt|;
comment|/* current indirection array size */
name|ASSERT
argument_list|(
name|ifp
operator|->
...
begin_comment
comment|/*  * Switch from indirection array to linear (direct) extent allocations.  */
end_comment
...
begin_comment
comment|/*  * Free incore file extents.  */
end_comment
...
begin_comment
comment|/*  * Return a pointer to the extent record for file system block bno.  */
end_comment

begin_function
name|xfs_bmbt_rec_t
modifier|*
comment|/* pointer to found extent record */
...
name|bno
parameter_list|,
comment|/* block number to search for */
name|xfs_extnum_t
modifier|*
name|idxp
parameter_list|)
...
name|base
decl_stmt|;
comment|/* pointer to first extent */
name|xfs_filblks_t
name|blockcount
init|=
literal|0
...
name|NULL
decl_stmt|;
comment|/* pointer to target extent */
name|xfs_ext_irec_t
modifier|*
name|erp
init|=
...
name|high
decl_stmt|;
comment|/* upper boundary in search */
name|xfs_extnum_t
name|idx
init|=
literal|0
...
name|low
decl_stmt|;
comment|/* lower boundary in search */
name|xfs_extnum_t
name|nextents
decl_stmt|;
...
literal|0
decl_stmt|;
comment|/* start offset of extent */
name|nextents
operator|=
name|ifp
operator|->
...
condition|)
block|{
comment|/* Find target extent list */
name|int
name|erp_idx
init|=
literal|0
...
expr_stmt|;
block|}
comment|/* Binary search extent records */
while|while
condition|(
name|low
operator|<=
...
block|{
comment|/* Convert back to file-based extent index */
if|if
condition|(
name|ifp
operator|->
...
begin_comment
comment|/*  * Return a pointer to the indirection array entry containing the  * extent record for filesystem block bno. Store the index of the  * target irec in *erp_idxp.  */
end_comment
...
name|erp_idxp
parameter_list|)
comment|/* irec index of target ext list */
block|{
name|xfs_ext_irec_t
modifier|*
name|erp
...
name|erp_next
decl_stmt|;
comment|/* next indirection array entry */
name|int
name|erp_idx
decl_stmt|;
comment|/* indirection array index */
...
name|nlists
decl_stmt|;
comment|/* number of extent irec's (lists) */
name|int
name|high
decl_stmt|;
comment|/* binary search upper limit */
name|int
name|low
decl_stmt|;
comment|/* binary search lower limit */
name|ASSERT
argument_list|(
name|ifp
operator|->
...
begin_comment
comment|/*  * Return a pointer to the indirection array entry containing the  * extent record at file extent index *idxp. Store the index of the  * target irec in *erp_idxp and store the page index of the target  * extent record in *idxp.  */
end_comment
...
name|idxp
parameter_list|,
comment|/* extent index (file -> page) */
name|int
modifier|*
name|erp_idxp
parameter_list|,
comment|/* pointer to target irec */
name|int
name|realloc
parameter_list|)
comment|/* new bytes were just added */
block|{
name|xfs_ext_irec_t
modifier|*
name|prev
decl_stmt|;
comment|/* pointer to previous irec */
name|xfs_ext_irec_t
modifier|*
name|erp
init|=
name|NULL
decl_stmt|;
comment|/* pointer to current irec */
name|int
name|erp_idx
decl_stmt|;
comment|/* indirection array index */
...
name|idxp
decl_stmt|;
comment|/* extent index in target list */
name|ASSERT
argument_list|(
name|ifp
operator|->
...
literal|1
expr_stmt|;
comment|/* Binary search extent irec's */
while|while
condition|(
name|low
operator|<=
...
begin_comment
comment|/*  * Allocate and initialize an indirection array once the space needed  * for incore extents increases above XFS_IEXT_BUFSZ.  */
end_comment
...
begin_comment
comment|/*  * Allocate and initialize a new entry in the indirection array.  */
end_comment
...
name|XFS_IEXT_BUFSZ
expr_stmt|;
comment|/* Resize indirection array */
name|xfs_iext_realloc_indirect
argument_list|(
name|ifp
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * Move records down in the array so the 	 * new page can use erp_idx. 	 */
name|erp
operator|=
name|ifp
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Initialize new extent record */
name|erp
operator|=
name|ifp
operator|->
...
begin_comment
comment|/*  * Remove a record from the indirection array.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Compact extent records */
name|erp
operator|=
name|ifp
operator|->
...
expr_stmt|;
block|}
comment|/* 	 * Manually free the last extent record from the indirection 	 * array.  A call to xfs_iext_realloc_indirect() with a size 	 * of zero would result in a call to xfs_iext_destroy() which 	 * would in turn call this function again, creating a nasty 	 * infinite loop. 	 */
if|if
condition|(
operator|--
name|nlists
...
begin_comment
comment|/*  * Combine extents from neighboring extent pages.  */
end_comment
...
name|erp_next
decl_stmt|;
comment|/* pointers to irec entries */
name|int
name|erp_idx
init|=
literal|0
...
name|er_extcount
expr_stmt|;
comment|/* 			 * Free page before removing extent record 			 * so er_extoffs don't get modified in 			 * xfs_iext_irec_remove. 			 */
name|kmem_free
argument_list|(
name|erp_next
operator|->
...
begin_comment
comment|/*  * Fully compact the extent records managed by the indirection array.  */
end_comment
...
name|ep_next
decl_stmt|;
comment|/* extent record pointers */
name|xfs_ext_irec_t
modifier|*
name|erp
decl_stmt|,
...
name|erp_next
decl_stmt|;
comment|/* extent irec pointers */
name|int
name|erp_idx
init|=
literal|0
decl_stmt|;
comment|/* extent irec index */
name|int
name|ext_avail
decl_stmt|;
comment|/* empty entries in ex list */
name|int
name|ext_diff
decl_stmt|;
comment|/* number of exts to add */
name|int
name|nlists
decl_stmt|;
comment|/* number of irec's (ex lists) */
...
name|XFS_IEXT_BUFSZ
expr_stmt|;
comment|/* Update next page */
block|}
else|else
block|{
comment|/* Move rest of page up to become next new page */
name|memmove
argument_list|(
name|erp_next
operator|->
...
begin_comment
comment|/*  * This is called to update the er_extoff field in the indirection  * array when extents have been added or removed from one of the  * extent lists. erp_idx contains the irec index to begin updating  * at and ext_diff contains the number of extents that were added  * or removed.  */
end_comment
...
name|erp_idx
parameter_list|,
comment|/* irec index to update */
name|int
name|ext_diff
parameter_list|)
comment|/* number of new extents */
block|{
name|int
name|i
decl_stmt|;
