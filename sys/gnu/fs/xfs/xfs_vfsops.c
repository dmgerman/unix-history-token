...
argument_list|)
expr_stmt|;
comment|/* 	 * Pull in the 'wsync' and 'ino64' mount options before we do the real 	 * work of mounting and recovery.  The arg pointer will 	 * be NULL when we are being called from the root mount code. 	 */
if|if
condition|(
name|ap
operator|->
...
return|;
block|}
comment|/* 	 * disallow mount attempts with (IRIX) project quota enabled 	 */
if|if
condition|(
...
name|CE_WARN
argument_list|,
literal|"XFS: cannot mount a filesystem with IRIX project quota enabled"
argument_list|)
expr_stmt|;
return|return
name|XFS_ERROR
...
define|#
directive|define
name|REMOUNT_READONLY_FLAGS
value|(SYNC_REMOUNT|SYNC_ATTR|SYNC_WAIT)
end_define
...
condition|)
do|;
comment|/* Ok now write out an unmount record */
...
begin_comment
comment|/*  * xfs sync routine for internal use  *  * This routine supports all of the flags defined for the generic VFS_SYNC  * interface as explained above under xfs_sync.  In the interests of not  * changing interfaces within the 6.5 family, additional internallly-  * required functions are specified within a separate xflags parameter,  * only available by calling this routine.  *  */
end_comment
...
return|;
block|}
comment|/* 		 * If this is just vfs_sync() or pflushd() calling 		 * then we can skip inodes for which it looks like 		 * there is nothing to do.  Since we don't have the 		 * inode locked this is racey, but these are periodic 		 * calls so it doesn't matter.  For the others we want 		 * to know for sure, so we at least try to lock them. 		 */
if|if
condition|(
name|flags
operator|&
...
continue|continue;
block|}
comment|/* 			 * We need to unlock the inode list lock in order 			 * to lock the inode. Insert a marker record into 			 * the inode list to remember our position, dropping 			 * the lock is now done inside the IPOINTER_INSERT 			 * macro. 			 * 			 * We also use the inode list lock to protect us 			 * in taking a snapshot of the vnode version number 			 * for use in calling vn_get(). 			 */
name|VMAP
argument_list|(
name|vp
argument_list|,
...
condition|)
block|{
comment|/* 				 * The vnode was reclaimed once we let go 				 * of the inode list lock.  Skip to the 				 * next list entry. Remove the marker. 				 */
name|XFS_MOUNT_ILOCK
argument_list|(
name|mp
argument_list|)
...
begin_comment
comment|/*  * xfs_vget - called by DMAPI to get vnode from file handle  */
end_comment
...
define|#
directive|define
name|MNTOPT_NOLOGFLUSH
value|"nologflush"
end_define

begin_comment
comment|/* don't hard flush on log writes */
end_comment
...
directive|if
literal|0
comment|/* XXX: off by default, until some remaining issues ironed out */
block|args->flags |= XFSMNT_IDELETE;
comment|/* default to on */
endif|#
directive|endif
if|if
condition|(
