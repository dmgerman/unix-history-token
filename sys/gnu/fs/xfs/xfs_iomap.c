...
name|STATIC
name|int
name|xfs_iomap_eof_align_last_fsb
parameter_list|(
name|xfs_mount_t
modifier|*
name|mp
...
condition|)
empty_stmt|;
comment|/* 	 * If mounted with the "-o swalloc" option, roundup the allocation 	 * request to a stripe width boundary if the file size is>= 	 * stripe width and we are allocating past the allocation eof. 	 */
elseif|else
if|if
condition|(
name|mp
...
argument_list|)
expr_stmt|;
comment|/* 	 * Roundup the allocation request to a stripe unit (m_dalign) boundary 	 * if the file size is>= stripe unit size, and we are allocating past 	 * the allocation eof. 	 */
elseif|else
if|if
condition|(
name|mp
...
argument_list|)
expr_stmt|;
comment|/* 	 * Always round up the allocation request to an extent boundary 	 * (when file on a real-time subvolume or has di_extsize hint). 	 */
if|if
condition|(
name|extsize
condition|)
...
argument_list|)
expr_stmt|;
comment|/* 	 * Check for running out of space, note: need lock to return 	 */
if|if
condition|(
name|error
condition|)
...
begin_comment
comment|/*  * If the caller is doing a write at the end of the file,  * then extend the allocation out to the file system's write  * iosize.  We clean up any extra space left over when the  * file is closed in xfs_inactive().  *  * For sync writes, we are flushing delayed allocate space to  * try to make additional space available for allocation near  * the filesystem full boundary - preallocation hurts in that  * situation, of course.  */
end_comment

begin_function
name|STATIC
name|int
name|xfs_iomap_eof_want_preallocate
parameter_list|(
name|xfs_mount_t
modifier|*
name|mp
...
literal|0
return|;
comment|/* 	 * If there are any real blocks past eof, then don't 	 * do any speculative allocation. 	 */
name|start_fsb
operator|=
name|XFS_B_TO_FSBT
argument_list|(
...
argument_list|)
return|;
comment|/* 	 * If bmapi returned us nothing, and if we didn't get back EDQUOT, 	 * then we must have run out of space - flush delalloc, and retry.. 	 */
if|if
condition|(
name|nimaps
operator|==
...
name|CE_PANIC
argument_list|,
literal|"Access to block zero:  "
literal|"fs<%s> inode: %lld "
literal|"start_block : %llx start_off : %llx "
literal|"blkcnt : %llx extent-state : %x \n"
argument_list|,
operator|(
name|ip
operator|->
...
name|CE_PANIC
argument_list|,
literal|"Access to block zero:  fs<%s> "
literal|"inode: %lld start_block : %llx start_off : "
literal|"%llx blkcnt : %llx extent-state : %x \n"
argument_list|,
operator|(
name|ip
operator|->
