...
begin_comment
comment|/*  * This is called to set the a callback to be called when the given  * transaction is committed to disk.  The transaction pointer and the  * argument pointer will be passed to the callback routine.  *  * Only one callback can be associated with any single transaction.  */
end_comment
...
case|:
comment|/* 		 * The allocation has already been applied to the 		 * in-core superblocks's counter.  This should only 		 * be applied to the on-disk superblock. 		 */
name|ASSERT
argument_list|(
name|delta
operator|<
...
name|whole
condition|)
comment|/* 		 * Log the whole thing, the fields are discontiguous. 		 */
...
expr_stmt|;
block|}
comment|/* 	 * Apply any superblock modifications to the in-core version. 	 * The t_res_fdblocks_delta and t_res_frextents_delta fields are 	 * explicity NOT applied to the in-core superblock. 	 * The idea is that that has already been done. 	 */
if|if
condition|(
name|tp
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * Ignore errors here. xfs_log_done would do the right thing. 	 * We need to put the ticket, etc. away. 	 */
name|error
operator|=
name|xfs_log_write
argument_list|(
...
directive|else
comment|/* 	 * This is the regular case.  At this point (after the call finishes), 	 * the transaction is committed incore and could go out to disk at 	 * any time.  However, all the items associated with the transaction 	 * are still locked and pinned in memory. 	 */
name|commit_lsn
operator|=
name|xfs_log_done
argument_list|(
