...
name|di_projid
expr_stmt|;
comment|/* 	 * Check vnode type block/char vs. everything else. 	 * Do it with bitmask because that's faster than looking 	 * for multiple values individually. 	 */
if|if
condition|(
operator|(
operator|(
...
directive|if
literal|0
comment|/* Large block sizes confuse various 			 * user space programs, so letting the 			 * stripe size through is not a good 			 * idea for now. 			 */
block|vap->va_blocksize = mp->m_swidth ?
comment|/* 				 * If the underlying volume is a stripe, then 				 * return the stripe width in bytes as the 				 * recommended I/O size. 				 */
block|(mp->m_swidth<< mp->m_sb.sb_blocklog) :
comment|/* 				 * Return the largest of the preferred buffer 				 * sizes since doing small I/Os into larger 				 * buffers causes buffers to be decommissioned. 				 * The value returned is in bytes. 				 */
block|(1<< (int)MAX(mp->m_readio_log, 					       mp->m_writeio_log));
else|#
directive|else
name|vap
operator|->
...
block|if ((vap->va_mode& S_ISVTX)&& vp->v_type != VDIR) 				m |= S_ISVTX;
endif|#
directive|endif
if|if
condition|(
...
goto|;
block|}
comment|/* 		 * Do a quota reservation only if uid or gid is actually 		 * going to change. 		 */
if|if
condition|(
operator|(
...
goto|;
block|}
comment|/* 		 * Can't set extent size unless the file is marked, or 		 * about to be marked as a realtime file. 		 * 		 * This check will be removed when fixed size extents 		 * with buffered data writes is implemented. 		 * 		 */
if|if
condition|(
operator|(
name|mask
...
name|XFS_DIFLAG_NODUMP
expr_stmt|;
comment|/* can't set PREALLOC this way, just ignore it */
block|}
name|xfs_trans_log_inode
argument_list|(
name|tp
...
expr_stmt|;
block|}
comment|/* 	 * If we're invalidating, always flush since we want to 	 * tear things down.  Otherwise, don't flush anything if 	 * we're not dirty. 	 */
if|if
condition|(
name|flag
operator|&
...
block|{
comment|/* 		 * In the non-invalidating case, calls to fsync() do not 		 * flush all the dirty mmap'd pages.  That requires a 		 * call to msync(). 		 */
...
argument_list|)
expr_stmt|;
comment|/* 	 * Commit the transaction containing extent freeing and EFD's. 	 * If we get an error on the commit here or on the reserve below, 	 * we need to unlock the inode since the new transaction doesn't 	 * have the inode attached. 	 */
name|error
operator|=
name|xfs_trans_commit
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Propogate the fact that the vnode changed after the 	 * xfs_inode locks have been released. 	 */
name|XVOP_VNODE_CHANGE
argument_list|(
name|vp
argument_list|,
...
argument_list|)
return|;
comment|/* 	 * For now, manually find the XFS behavior descriptor for 	 * the source vnode.  If it doesn't exist then something 	 * is wrong and we should just return an error. 	 * Eventually we need to figure out how link is going to 	 * work in the face of stacked vnodes. 	 */
name|src_bdp
operator|=
...
condition|)
block|{
comment|/* 			 * Flush and invalidate any data left around that is 			 * a part of this file. 			 * 			 * Get the inode's i/o lock so that buffers are pushed 			 * out while holding the proper lock.  We can't hold 			 * the inode lock here since flushing out buffers may 			 * cause us to try to get the lock in xfs_strategy(). 			 * 			 * We don't have to call remapf() here, because there 			 * cannot be any mapped file references to this vnode 			 * since it is being reclaimed. 			 */
name|xfs_ilock
argument_list|(
name|ip
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 			 * If we hit an IO error, we need to make sure that the 			 * buffer and page caches of file data for 			 * the file are tossed away. We don't want to use 			 * VOP_FLUSHINVAL_PAGES here because we don't want dirty 			 * pages to stay attached to the vnode, but be 			 * marked P_BAD. pdflush/vnode_pagebad 			 * hates that. 			 */
if|if
condition|(
operator|!
name|XFS_FORCED_SHUTDOWN
...
condition|)
block|{
comment|/* 			 * di_size field may not be quite accurate if we're 			 * shutting down. 			 */
name|XVOP_TOSS_PAGES
argument_list|(
name|vp
argument_list|,
...
argument_list|)
return|;
comment|/* 	 * determine if this is a realtime file 	 */
if|if
condition|(
operator|(
name|rt
