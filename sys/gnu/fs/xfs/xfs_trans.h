...
begin_comment
comment|/*  * This structure is passed as a parameter to xfs_trans_push_ail()  * and is used to track the what LSN the waiting processes are  * waiting to become unused.  */
end_comment
...
name|at_lsn
decl_stmt|;
comment|/* lsn waitin for */
name|struct
name|xfs_ail_ticket
modifier|*
name|at_forw
decl_stmt|;
comment|/* wait list ptr */
name|struct
name|xfs_ail_ticket
modifier|*
name|at_back
...
name|cp
parameter_list|)
value|((cp)->lic_free = XFS_LIC_FREEMASK)
end_define
...
parameter_list|)
define|\
value|((cp)->lic_descs[slot].lid_index = (unsigned char)(slot))
end_define
...
name|cp
parameter_list|)
value|(((cp)->lic_free& XFS_LIC_FREEMASK) ==\ 					XFS_LIC_FREEMASK)
end_define
...
name|dp
parameter_list|)
value|((xfs_log_item_chunk_t*) \ 					(((xfs_caddr_t)((dp) - (dp)->lid_index)) -\ 					(xfs_caddr_t)(((xfs_log_item_chunk_t*) \ 					0)->lic_descs)))
end_define
...
name|t_dqinfo
decl_stmt|;
comment|/* accting info for dquots */
name|xfs_trans_callback_t
name|t_callback
decl_stmt|;
...
parameter_list|)
define|\
value|(MAX( \ 	 ((mp)->m_sb.sb_inodesize + \ 	  XFS_FSB_TO_B((mp), XFS_BM_MAXLEVELS(mp, XFS_DATA_FORK) + 1) + \ 	  (128 * (2 + XFS_BM_MAXLEVELS(mp, XFS_DATA_FORK)))), \ 	 ((4 * (mp)->m_sb.sb_sectsize) + \ 	  (4 * (mp)->m_sb.sb_sectsize) + \ 	  (mp)->m_sb.sb_sectsize + \ 	  XFS_ALLOCFREE_LOG_RES(mp, 4) + \ 	  (128 * (9 + XFS_ALLOCFREE_LOG_COUNT(mp, 4))))))
end_define
...
parameter_list|)
define|\
value|((mp)->m_sb.sb_inodesize + \ 	 (mp)->m_sb.sb_sectsize + \ 	 (mp)->m_sb.sb_sectsize + \ 	 XFS_FSB_TO_B((mp), 1) + \ 	 MAX((__uint16_t)XFS_FSB_TO_B((mp), 1), XFS_INODE_CLUSTER_SIZE(mp)) + \ 	 (128 * 5) + \ 	  (128 * (2 + XFS_IALLOC_BLOCKS(mp) + XFS_IN_MAXLEVELS(mp) + \ 	   XFS_ALLOCFREE_LOG_COUNT(mp, 1))))
end_define
