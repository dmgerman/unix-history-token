...
begin_comment
comment|/*  * Called by xfs_bmapi to update extent list structure and the btree  * after allocating space (or doing a delayed allocation).  */
end_comment
...
name|new
parameter_list|,
comment|/* new data to put in extent list */
name|xfs_fsblock_t
modifier|*
name|first
parameter_list|,
...
begin_comment
comment|/*  * Transform a btree format file with only one leaf node, where the  * extents list will fit in the inode, into an extents format file.  * Since the extent list is already in-core, all we have to do is  * give up the space for the btree root and pitch the leaf block.  */
end_comment
...
begin_comment
comment|/*  * Called by xfs_bmapi to update extent list structure and the btree  * after removing space (or undoing a delayed allocation).  */
end_comment
...
begin_comment
comment|/*  * Remove count entries from the extents array for inode "ip", starting  * at index "idx".  Copies the remaining items down over the deleted ones,  * and gives back the excess memory.  */
end_comment

begin_function_decl
name|STATIC
name|void
name|xfs_bmap_delete_exlist
parameter_list|(
name|xfs_inode_t
modifier|*
name|ip
parameter_list|,
comment|/* incode inode pointer */
name|xfs_extnum_t
name|idx
parameter_list|,
comment|/* starting delete index */
name|xfs_extnum_t
name|count
parameter_list|,
comment|/* count of items to delete */
name|int
name|whichfork
parameter_list|)
function_decl|;
...
begin_comment
comment|/*  * Insert new item(s) in the extent list for inode "ip".  * Count new items are inserted at offset idx.  */
end_comment

begin_function_decl
name|STATIC
name|void
name|xfs_bmap_insert_exlist
parameter_list|(
name|xfs_inode_t
modifier|*
name|ip
...
begin_comment
comment|/*  * Add bmap trace entry prior to a call to xfs_bmap_delete_exlist.  */
end_comment
...
begin_comment
comment|/*  * Add bmap trace entry prior to a call to xfs_bmap_insert_exlist, or  * reading in the extents list from the disk (in the btree).  */
end_comment
...
begin_comment
comment|/*  * Add bmap trace entry after updating an extent list entry in place.  */
end_comment
...
begin_comment
comment|/*  * Add bmap trace entry prior to updating an extent list entry in place.  */
end_comment
...
name|base
decl_stmt|;
comment|/* base of extent entry list */
name|xfs_btree_cur_t
modifier|*
name|cur
decl_stmt|;
...
name|ep
decl_stmt|;
comment|/* extent list entry for idx */
ifdef|#
directive|ifdef
name|XFS_BMAP_TRACE
specifier|static
...
argument_list|)
case|:
comment|/* 		 * New allocation is contiguous with delayed allocations 		 * on the left and on the right. 		 * Merge all three into a single extent list entry. 		 */
name|temp
operator|=
name|left
operator|.
...
argument_list|)
case|:
comment|/* 		 * New allocation is contiguous with real allocations on the 		 * left and on the right. 		 * Merge all three into a single extent list entry. 		 */
name|xfs_bmap_trace_pre_update
argument_list|(
name|fname
argument_list|,
...
name|extsz
decl_stmt|;
comment|/* file extent size for rt */
name|xfs_fileoff_t
name|nexto
decl_stmt|;
...
name|orig_off
decl_stmt|;
comment|/* original ap->off */
name|xfs_extlen_t
name|mod_off
decl_stmt|;
comment|/* modulus calculations */
name|xfs_fileoff_t
name|prevo
decl_stmt|;
...
name|temp
decl_stmt|;
comment|/* temp for rt calculations */
comment|/* 		 * Set prod to match the realtime extent size. 		 */
if|if
condition|(
operator|!
operator|(
...
name|orig_alen
expr_stmt|;
comment|/* 		 * If the file offset is unaligned vs. the extent size 		 * we need to align it.  This will be possible unless 		 * the file was previously written with a kernel that didn't 		 * perform this alignment. 		 */
name|mod_off
operator|=
name|do_mod
argument_list|(
...
expr_stmt|;
block|}
comment|/* 		 * If the result isn't a multiple of rtextents we need to 		 * remove blocks until it is. 		 */
if|if
condition|(
operator|(
name|temp
...
name|del
parameter_list|,
comment|/* data to remove from extent list */
name|int
modifier|*
name|logflagsp
parameter_list|,
...
name|nextents
decl_stmt|;
comment|/* number of extents in list after */
name|ifp
operator|=
name|XFS_IFORK_PTR
argument_list|(
...
name|bp
decl_stmt|;
comment|/* buffer for extent list block */
name|xfs_bmbt_rec_t
modifier|*
name|ep
decl_stmt|;
...
name|ep
decl_stmt|;
comment|/* extent list entry pointer */
name|xfs_bmbt_irec_t
name|got
decl_stmt|;
comment|/* extent list entry, decoded */
name|int
name|high
decl_stmt|;
comment|/* high index of binary search */
name|int
name|low
decl_stmt|;
comment|/* low index of binary search */
if|if
condition|(
name|lastx
operator|!=
...
block|{
comment|/* binary search the extents array */
name|low
operator|=
literal|0
expr_stmt|;
...
name|rsvd
parameter_list|)
comment|/* OK to allocated reserved blocks in trans */
block|{
name|int
name|blks
decl_stmt|;
...
begin_comment
comment|/*  * Returns EINVAL if the specified file is not swappable.  */
end_comment
...
name|end_fsb
decl_stmt|;
comment|/* last block of file within size */
name|xfs_bmbt_irec_t
name|ext
decl_stmt|;
comment|/* extent list entry, decoded */
...
argument_list|)
expr_stmt|;
comment|/* 	 * Check for a zero length file. 	 */
if|if
condition|(
name|ip
operator|->
...
name|check_done
goto|;
comment|/* 	 * Scan extents until the file size is reached. Look for 	 * holes or unwritten extents, since I/O to these would cause 	 * a transaction. 	 */
name|end_fsb
operator|=
name|XFS_B_TO_FSB
argument_list|(
...
literal|0
expr_stmt|;
comment|/* 	 * Loop over all leaf nodes.  Copy information to the extent list. 	 */
for|for
control|(
init|;
condition|;
...
name|i_mount
argument_list|,
literal|"corrupt dinode %Lu, (btree extents).  Unmount and run xfs_repair."
argument_list|,
operator|(
name|unsigned
name|long
...
argument_list|)
expr_stmt|;
comment|/* 		 * Copy records into the extent list. 		 */
name|frp
operator|=
...
name|ep
decl_stmt|;
comment|/* current entry in extent list */
name|xfs_extnum_t
name|idx
decl_stmt|;
comment|/* extent list entry number */
name|xfs_ifork_t
modifier|*
name|ifp
decl_stmt|;
...
name|contig
decl_stmt|;
comment|/* allocation must be one extent */
name|xfs_btree_cur_t
modifier|*
name|cur
decl_stmt|;
...
name|delay
decl_stmt|;
comment|/* this request is for delayed alloc */
name|xfs_fileoff_t
name|end
decl_stmt|;
...
name|eof
decl_stmt|;
comment|/* we've hit the end of extent list */
name|xfs_bmbt_rec_t
modifier|*
name|ep
decl_stmt|;
...
name|exact
decl_stmt|;
comment|/* don't do all of wasdelayed extent */
name|xfs_bmbt_irec_t
name|got
decl_stmt|;
comment|/* current extent list record */
name|xfs_ifork_t
modifier|*
name|ifp
decl_stmt|;
...
name|prev
decl_stmt|;
comment|/* previous extent list record */
name|char
name|stateless
decl_stmt|;
comment|/* ignore state flag set */
name|int
name|tmp_logflags
decl_stmt|;
...
name|trim
decl_stmt|;
comment|/* output trimmed to match range */
name|char
name|userdata
decl_stmt|;
comment|/* allocating non-metadata */
name|char
name|wasdelay
decl_stmt|;
...
name|wr
condition|)
comment|/* if writing unwritten space, no */
name|wr
operator|=
literal|0
expr_stmt|;
comment|/* allocations are allowed */
name|ASSERT
argument_list|(
name|wr
operator|||
...
argument_list|)
expr_stmt|;
comment|/* 				 * Make a transaction-less quota reservation for 				 * delayed allocation blocks. This number gets 				 * adjusted later. 				 * We return EDQUOT if we haven't allocated 				 * blks already inside this loop; 				 */
if|if
condition|(
...
name|error0
label|:
comment|/* 	 * Log everything.  Do this after conversion, there's no point in 	 * logging the extent list if we've converted to btree format. 	 */
if|if
condition|(
operator|(
name|logflags
...
name|got
decl_stmt|;
comment|/* current extent list entry */
name|xfs_ifork_t
modifier|*
name|ifp
decl_stmt|;
...
name|nextents
decl_stmt|;
comment|/* size of extent list */
name|xfs_bmbt_irec_t
name|prev
decl_stmt|;
comment|/* previous extent list entry */
name|xfs_fileoff_t
name|start
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Unreserve our quota space */
name|XFS_TRANS_RESERVE_QUOTA_NBLKS
argument_list|(
name|mp
argument_list|,
...
condition|)
block|{
comment|/* 				 * came to hole at end of file 				 */
goto|goto
name|unlock_and_return
goto|;
block|}
...
name|s
decl_stmt|;
comment|/* expanded extent list entry */
name|ASSERT
argument_list|(
name|whichfork
operator|==
...
name|lastp
decl_stmt|;
comment|/* extent pointers in block entry */
name|int
name|bp_release
init|=
literal|0
...
literal|1
expr_stmt|;
comment|/* For the next iteration */
name|i
operator|+=
name|num_recs
expr_stmt|;
...
begin_comment
comment|/*  * Count leaf blocks given a pointer to an extent list.  */
end_comment
