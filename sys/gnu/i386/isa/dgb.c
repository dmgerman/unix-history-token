...
begin_comment
comment|/*-  *  dgb.c $Id: dgb.c,v 1.2.2.2 1995/10/13 03:41:22 davidg Exp $  *  *  Digiboard driver.  *  *  Stage 1. "Better than nothing".  *  *  Based on sio driver by Bruce Evans and on Linux driver by Troy  *  De Jongh<troyd@digibd.com> or<troyd@skypoint.com>  *  which is under GNU General Public License version 2 so this driver  *  is forced to be under GPL 2 too.  *  *  Written by Serge Babkin,  *      Joint Stock Commercial Bank "Chelindbank"  *      (Chelyabinsk, Russia)  *      babkin@hq.icb.chel.su  *  *  Modified by Oleg Panashchenko  *	    helg@tav.kiev.ua  *      - read optimization  *      - corrections in data flow mechanism  *      - restoring board memory window state while returning from  *		  reentrable functions  *    1996/03/16:  *      - now several boards can coexist and share the same memory window  *    1996/03/24:  *      - eliminated 'b_to_q to a clist..' message while turning off modem   *    1996/04/10  *      - 'static int nports'... - next attempt to drive several boards.  *        Reported that with this fix it works with 2 boards.  *    1996/04/20  *      - Overloading channel (ping -f host) caused 'No buffer space..'  *        state until slip unit restart. Fixed.  */
end_comment
...
block|{
block|{
comment|/* Helg: special case to supress error message because of hardware err */
comment|/*if(pnum!=0xff)*/
name|printf
argument_list|(
...
directive|if
literal|0
block|HPRINT4("dgb%d:port%d:wrapmask%d\n",unit,pnum,wrapmask);
endif|#
directive|endif
name|rhead
operator|=
...
block|}
block|}
comment|/* 					HPRINT5("pnum%d:sz=%d head=%d tail=%d\n",pnum,size,rhead,rtail); */
name|rtail
operator|=
operator|(
name|rtail
...
condition|)
block|{
comment|/* can't put all data to board now */
name|bc
operator|->
name|iempty
operator|=
...
argument_list|)
expr_stmt|;
comment|/*#define LEAVE_FREE_CHARS 16*/
ifdef|#
directive|ifdef
