...
begin_comment
comment|/* 0(no debug), 1(debug), 2(debug on each read) */
end_comment
...
name|last_track
decl_stmt|;
comment|/*	struct s_sony_subcode subcode; */
name|short
name|audio_status
decl_stmt|;
name|struct
...
name|printf
argument_list|(
literal|"scd%d: DEBUG: ********* ATTENTION = 0x%x\n"
argument_list|,
name|unit
argument_list|,
name|atten_code
...
condition|)
block|{
comment|/* Someone changed the CD.  Mark it as changed */
case|case
...
name|printf
argument_list|(
literal|"scd%d: unknown ATTENTION = 0x%x\n"
argument_list|,
name|unit
argument_list|,
name|atten_code
...
literal|1
expr_stmt|;
comment|/* Cannot take address of bitfield (above) */
if|if
condition|(
name|toc
operator|->
...
name|printf
argument_list|(
literal|"scd: WARNING: pointb0 != track0\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
begin_comment
comment|/* Return value same as do_cmd() */
end_comment
...
literal|0x03
expr_stmt|;
comment|/* Set auto spin up and auto eject */
if|if
condition|(
name|scd_data
index|[
...
literal|0x04
expr_stmt|;
comment|/* Set the drive to double speed if possible */
name|res
operator|=
name|do_cmd
argument_list|(
...
name|printf
argument_list|(
literal|"scd%d: Unable to set mechanical parameters. Errcode = 0x%x\n"
argument_list|,
name|res
argument_list|)
expr_stmt|;
...
literal|50
expr_stmt|;
comment|/* While probing tsleep doesn't sleep */
while|while
condition|(
operator|(
name|retry_count
...
begin_comment
comment|/* Returns 0 or (sony error code) or -(errno code) */
end_comment
...
name|CDIOCRESUME
case|:
comment|/*		return scd_resume(unit); */
case|case
name|CDIOCPAUSE
case|:
comment|/*		return scd_pause(unit); */
case|case
name|CDIOCSTART
case|:
return|return
...
name|printf
argument_list|(
literal|"scd%d: playmsf: %02d:%02d:%02d -> %02d:%02d:%02d\n"
argument_list|,
name|unit
argument_list|,
name|msf
...
begin_comment
comment|/* check to see if a Sony CD-ROM is attached to the ISA bus */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If I use a shorter delay, I get a timeout in get_result() */
comment|/* I guess there is a better way, but I don't have the manuals */
name|DELAY
argument_list|(
literal|500000
argument_list|)
...
begin_comment
comment|/* State machine copied from mcd.c /Micke */
end_comment
...
block|if (cd->status& SCDAUDIOBSY) { 			printf("scd%d: audio is active\n",unit); 			goto readerr; 		}
endif|#
directive|endif
name|mbx
operator|->
...
comment|/* that it is ok to send the SPIN_UP command below */
if|if
condition|(
operator|!
operator|(
