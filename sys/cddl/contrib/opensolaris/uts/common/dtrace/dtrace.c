...
begin_comment
comment|/* non-zero if executing a probe */
end_comment
...
begin_comment
comment|/* Address of invop when already in probe */
end_comment
...
begin_comment
comment|/*  * DTrace Helper Tracing Variables  */
end_comment
...
parameter_list|)
define|\
value|((testaddr) - (baseaddr)< (basesz)&& \ 	(testaddr) + (testsz) - (baseaddr)<= (basesz)&& \ 	(testaddr) + (testsz)>= (testaddr))
end_define
...
goto|;
block|}
comment|/* 	 * We have the space; copy the buffer across.  (Note that this is a 	 * highly subobtimal bcopy(); in the unlikely event that this becomes 	 * a serious performance issue, a high-performance DTrace-specific 	 * bcopy() should obviously be invented.) 	 */
name|daddr
operator|=
operator|(
name|uintptr_t
...
directive|else
comment|/*           * XXX - The following code works because mutex, rwlocks,& sxlocks          *       all have similar data structures in FreeBSD.  This may not be          *	 good if someone changes one of the lock data structures. 	 * 	 Ideally, it would be nice if all these shared a common lock  	 * 	 object.          */
case|case
name|DIF_SUBR_MUTEX_OWNED
case|:
comment|/* XXX - need to use dtrace_canload() and dtrace_loadptr() */
name|m
operator|.
name|mx
operator|=
...
literal|22
decl_stmt|;
comment|/* enough room for 2^64 in decimal */
name|char
modifier|*
name|end
init|=
...
name|size
decl_stmt|;
comment|/* address, num_elements, type_str, type_len */
name|typeref
index|[
literal|0
index|]
...
condition|)
block|{
comment|/* 			 * We seem to be dead.  Unless we (a) have kernel 			 * destructive permissions (b) have expicitly enabled 			 * destructive actions and (c) destructive actions have 			 * not been disabled, we're going to transition into 			 * the KILLED state, from which no further processing 			 * on this state will be performed. 			 */
if|if
condition|(
operator|!
name|dtrace_priv_kernel_destructive
...
case|:
block|{
comment|/* The DIF returns a 'typeref'. */
name|uintptr_t
modifier|*
name|typeref
init|=
...
name|s
decl_stmt|;
comment|/* 				 * Get the type string length and round it 				 * up so that the data that follows is 				 * aligned for easy access. 				 */
name|size_t
name|typs
init|=
name|strlen
...
argument_list|)
expr_stmt|;
comment|/* 				 *Get the size from the typeref using the 				 * number of elements and the type size. 				 */
name|size
operator|=
name|typeref
index|[
...
argument_list|)
expr_stmt|;
comment|/* Store the type size in the buffer. */
name|DTRACE_STORE
argument_list|(
name|uintptr_t
argument_list|,
...
expr_stmt|;
block|}
comment|/* 				 * Reset to the memory address rather than 				 * the typeref array, then let the BYREF 				 * code below do the work to store the  				 * memory data in the buffer. 				 */
name|val
operator|=
name|typeref
index|[
...
condition|)
continue|continue;
comment|/* 		 * We have at least one ECB; we can't remove this provider. 		 */
if|if
condition|(
operator|!
name|self
...
name|i
argument_list|,
literal|"oversized by-ref global\n"
argument_list|)
expr_stmt|;
break|break;
block|}
...
name|probe
expr_stmt|;
comment|/* 	 * The default size is the size of the default action: recording 	 * the epid. 	 */
name|ecb
operator|->
name|dte_size
operator|=
...
name|dte_state
decl_stmt|;
comment|/* 	 * If we record anything, we always record the epid.  (And we always 	 * record it first.) 	 */
name|offs
operator|=
sizeof|sizeof
argument_list|(
...
condition|)
block|{
comment|/* 			 * This is the first record in a tuple.  Align the 			 * offset to be at offset 4 in an 8-byte aligned 			 * block. 			 */
name|diff
operator|=
name|offs
operator|+
...
condition|)
block|{
comment|/* 			 * The current offset is not properly aligned; align it. 			 */
name|offs
operator|+=
name|align
operator|-
...
condition|)
block|{
comment|/* 		 * If the size is still sizeof (dtrace_epid_t), then all 		 * actions store no data; set the size to 0. 		 */
name|ecb
operator|->
name|dte_alignment
operator|=
...
literal|0
expr_stmt|;
comment|/* 		 * If the needed space is still sizeof (dtrace_epid_t), then 		 * all actions need no additional space; set the needed 		 * size to 0. 		 */
if|if
condition|(
name|ecb
operator|->
...
return|return;
block|}
comment|/* 	 * Set our alignment, and make sure that the dte_size and dte_needed 	 * are aligned to the size of an EPID. 	 */
name|ecb
operator|->
name|dte_alignment
operator|=
...
literal|0
return|;
comment|/* 	 * Devices are cloned, so if the DTrace state has already 	 * been allocated, that means this device belongs to a 	 * different client. Each client should open '/dev/dtrace' 	 * to get a cloned device. 	 */
if|if
condition|(
name|dev
operator|->
...
operator|<
literal|800039
comment|/* Destroy the cloned device. */
name|destroy_dev
argument_list|(
name|dev
argument_list|)
...
operator|<
literal|800039
comment|/* Schedule this cloned device to be destroyed. */
...
name|dtrace_clones
decl_stmt|;
comment|/* Ptr to the array of cloned devices. */
specifier|static
