...
begin_comment
comment|/*  * zfs_match_find() is used by zfs_dirent_lookup() to peform zap lookups  * of names after deciding which is the appropriate lookup interface.  */
end_comment
...
operator|)
return|;
comment|/* 	 * If we are a snapshot mounted under .zfs, return 	 * the snapshot directory. 	 */
if|if
condition|(
name|parent
operator|==
...
begin_comment
comment|/*  * Link zp into dzp.  Can only fail if zp has been unlinked.  */
end_comment
...
directive|if
literal|0
block|if (zp_is_dir) { 		error = 0; 		if (dzp->z_links>= LINK_MAX) 			error = SET_ERROR(EMLINK); 		return (error); 	}
endif|#
directive|endif
if|if
condition|(
...
directive|if
literal|0
block|if (zp->z_links>= LINK_MAX) { 			return (SET_ERROR(EMLINK)); 		}
endif|#
directive|endif
name|zp
operator|->
...
begin_comment
comment|/*  * Unlink zp from dzp, and mark zp for deletion if this was the last link.  * Can fail if zp is a mount point (EBUSY) or a non-empty directory (EEXIST).  * If 'unlinkedp' is NULL, we put unlinked znodes on the unlinked list.  * If it's non-NULL, we use it to indicate whether the znode needs deletion,  * and it's the caller's job to do it.  */
end_comment
...
begin_comment
comment|/*  * Indicate whether the directory is empty.  */
end_comment
