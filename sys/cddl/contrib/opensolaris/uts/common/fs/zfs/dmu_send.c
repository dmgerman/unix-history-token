...
begin_comment
comment|/*  * This function is a callback used by dmu_objset_find() (which  * enumerates the object sets) to build an avl tree that maps guids  * to datasets.  The resulting table is used when processing DRR_WRITE_BYREF  * send stream records.  These records, which are used in dedup'ed  * streams, do not contain data themselves, but refer to a copy  * of the data block that has already been written because it was  * earlier in the stream.  That previous copy is identified by the  * guid of the dataset with the referenced data.  */
end_comment

begin_function
name|int
name|find_ds_by_guid
parameter_list|(
specifier|const
name|char
modifier|*
...
condition|)
block|{
comment|/* 			 * Skip this snapshot and move on. It's not 			 * clear why this would ever happen, but the 			 * remainder of the snapshot streadm can be 			 * processed. 			 */
name|rw_exit
argument_list|(
operator|&
name|dp
