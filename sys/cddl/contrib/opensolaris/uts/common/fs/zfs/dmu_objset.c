...
name|ha
decl_stmt|;
comment|/* only needed in the temporary case */
name|dsl_dataset_t
modifier|*
name|newds
decl_stmt|;
...
name|error
decl_stmt|;
comment|/* The props have already been checked by zfs_check_userprops(). */
name|error
operator|=
...
condition|)
block|{
comment|/* 		 * Ideally we would just call 		 * dsl_dataset_user_hold_check() and 		 * dsl_dataset_destroy_check() here.  However the 		 * dataset we want to hold and destroy is the snapshot 		 * that we just confirmed we can create, but it won't 		 * exist until after these checks are run.  Do any 		 * checks we can here and if more checks are added to 		 * those routines in the future, similar checks may be 		 * necessary here. 		 */
if|if
condition|(
name|spa_version
argument_list|(
...
operator|)
return|;
comment|/* 		 * Not checking number of tags because the tag will be 		 * unique, as it will be the only tag. 		 */
if|if
condition|(
name|strlen
argument_list|(
...
name|cp
decl_stmt|;
comment|/* 	 * If the objset starts with a '%', then ignore it unless it was 	 * explicitly named (ie, not recursive).  These hidden datasets 	 * are always inconsistent, and by not opening them here, we can 	 * avoid a race with dsl_dir_destroy_check(). 	 */
name|cp
operator|=
name|strrchr
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Check permissions if we are doing a recursive snapshot.  The 	 * permission checks for the starting dataset have already been 	 * performed in zfs_secpolicy_snapshot() 	 */
if|if
condition|(
name|sn
operator|->
...
operator|)
return|;
comment|/* 	 * If the objset is in an inconsistent state (eg, in the process 	 * of being destroyed), don't snapshot it.  As with %hidden 	 * datasets, we return EBUSY if this name was explicitly 	 * requested (ie, not recursive), and otherwise ignore it. 	 */
if|if
condition|(
name|os
operator|->
