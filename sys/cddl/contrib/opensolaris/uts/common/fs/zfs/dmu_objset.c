...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright (c) 2012, 2017 by Delphix. All rights reserved.  * Copyright (c) 2013 by Saso Kiselkov. All rights reserved.  * Copyright (c) 2013, Joyent, Inc. All rights reserved.  * Copyright (c) 2014 Spectra Logic Corporation, All rights reserved.  * Copyright (c) 2015, STRATO AG, Inc. All rights reserved.  * Copyright (c) 2014 Integros [integros.com]  * Copyright 2017 Nexenta Systems, Inc.  */
end_comment
...
begin_comment
comment|/*  * The hash is a CRC-based hash of the objset_t pointer and the object number.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * The low 6 bits of the pointer don't have much entropy, because 	 * the objset_t is larger than 2^6 bytes long. 	 */
name|crc
operator|=
operator|(
name|crc
...
name|unsigned
name|int
name|dnode_multilist_index_func
parameter_list|(
name|multilist_t
modifier|*
name|ml
...
begin_comment
comment|/*  * Instantiates the objset_t in-memory structure corresponding to the  * objset_phys_t that's pointed to by the specified blkptr_t.  */
end_comment
...
condition|)
block|{
comment|/* 		 * We must create the list here because it uses the 		 * dn_dirty_link[] of this txg.  But it may already 		 * exist because we call dsl_dataset_sync() twice per txg. 		 */
if|if
condition|(
name|os
operator|->
...
condition|)
block|{
comment|/* 		 * os_userused_lock protects against concurrent calls to 		 * zap_increment_int().  It's needed because zap_increment_int() 		 * is not thread-safe (i.e. not atomic). 		 */
name|mutex_enter
argument_list|(
operator|&
name|os
...
name|tx
expr_stmt|;
comment|/* note: caller does taskq_wait() */
operator|(
name|void
operator|)
name|taskq_dispatch
...
begin_comment
comment|/*  * Call when we think we're going to write/free space in open context to track  * the amount of dirty data in the open txg, which is also the amount  * of memory that can not be evicted until this txg syncs.  */
end_comment
