...
argument_list|)
expr_stmt|;
comment|/* First round tries to get provider's ID without reading metadata. */
name|LIST_FOREACH
argument_list|(
argument|mp
argument_list|,
...
block|}
block|}
comment|/* Second round looks for ID by reading ZFS metadata. */
name|LIST_FOREACH
argument_list|(
argument|mp
argument_list|,
...
name|g_consumer
modifier|*
name|vdev_geom_open_by_path_and_devid
parameter_list|(
name|vdev_t
modifier|*
name|vd
...
name|len
decl_stmt|;
comment|/* 	 * We can't search by devid if it's missing. 	 */
if|if
condition|(
name|vd
operator|->
...
condition|)
block|{
comment|/* 		 * The device at vd->vdev_path doesn't have the right devid. 		 * The disks might have merely moved around so try all other 		 * geom providers to find one with the right devid. 		 */
name|cp
operator|=
name|vdev_geom_open_by_devid
argument_list|(
