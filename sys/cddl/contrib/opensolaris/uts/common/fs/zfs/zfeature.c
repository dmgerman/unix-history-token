...
begin_comment
comment|/*  * Use an in-memory cache of feature refcounts for quick retrieval.  *  * Note: well-designed features will not need to use this; they should  * use spa_feature_is_enabled() and spa_feature_is_active() instead.  * However, this is non-static for zdb and zhack.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * feature_sync is called directly from zhack, allowing the 	 * creation of arbitrary features whose fi_feature field may 	 * be greater than SPA_FEATURES. When called from zhack, the 	 * zfeature_info_t object's fi_feature field will be set to 	 * SPA_FEATURE_NONE. 	 */
if|if
condition|(
name|feature
operator|->
