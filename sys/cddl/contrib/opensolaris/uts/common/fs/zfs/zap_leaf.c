...
begin_comment
comment|/*  * Only to be used on 8-bit arrays.  * array_len is actual len in bytes (not encoded le_value_length).  * namenorm is null-terminated.  */
end_comment
...
return|;
block|}
comment|/* Fast path for exact matching */
while|while
condition|(
name|bseen
operator|<
...
block|}
block|}
comment|/* 		 * we would run out of space in a block before we could 		 * have ZAP_MAXCD entries 		 */
name|ASSERT3U
argument_list|(
name|cd
argument_list|,
