...
argument_list|)
expr_stmt|;
comment|/* 		 * Now that we've validate the config, check the state of the 		 * root vdev.  If it can't be opened, it indicates one or 		 * more toplevel vdevs are faulted. 		 */
if|if
condition|(
name|rvd
operator|->
...
name|CE_NOTE
argument_list|,
literal|"Can not read the pool label from '%s'"
argument_list|,
name|devpath
argument_list|)
expr_stmt|;
...
name|NULL
argument_list|,
literal|"split new pool %s from pool %s"
argument_list|,
name|newname
argument_list|,
name|spa_name
