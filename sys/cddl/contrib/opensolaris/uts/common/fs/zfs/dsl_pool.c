...
argument_list|)
control|)
name|dmu_objset_do_userquota_callbacks
argument_list|(
name|ds
operator|->
name|ds_user_ptr
...
argument_list|)
expr_stmt|;
comment|/* 	 * Sync the datasets again to push out the changes due to 	 * userquota updates.  This must be done before we process the 	 * sync tasks, because that could cause a snapshot of a dataset 	 * whose ds_bp will be rewritten when we do this 2nd sync. 	 */
name|zio
operator|=
name|zio_root
argument_list|(
