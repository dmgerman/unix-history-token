...
condition|)
return|return;
comment|/* 	 * If the vnode lock is held and we aren't the owner 	 * then just return since we don't want to deadlock 	 * trying to update the status of z_is_sa.  This 	 * file can then be upgraded at a later time. 	 * 	 * Otherwise, we know we are doing the 	 * sa_update() that caused us to enter this function. 	 */
if|if
condition|(
name|vn_lock
argument_list|(
