...
operator|)
return|;
comment|/* 	 * lzc_destroy_snaps() is documented to take an nvlist whose 	 * values "don't matter".  We need to convert that nvlist to 	 * one that we know can be converted to LUA. We also don't 	 * care about any duplicate entries because the nvlist will 	 * be converted to a LUA table which should take care of this. 	 */
name|nvlist_t
modifier|*
