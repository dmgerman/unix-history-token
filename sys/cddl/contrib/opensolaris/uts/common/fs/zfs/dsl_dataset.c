...
begin_comment
comment|/*  * Returns a string that represents the receive resume stats token. It should  * be freed with strfree().  */
end_comment
...
begin_comment
comment|/*  * Returns a string that represents the receive resume stats token of the  * dataset's child. It should be freed with strfree().  */
end_comment
...
begin_comment
comment|/*  * 'snap' should be a buffer of size ZFS_MAX_DATASET_NAME_LEN.  */
end_comment
...
begin_comment
comment|/*  * Returns the mountpoint property and source for the given dataset in the value  * and source buffers. The value buffer must be at least as large as MAXPATHLEN  * and the source buffer as least as large a ZFS_MAX_DATASET_NAME_LEN.  * Returns 0 on success and an error on failure.  */
end_comment
...
name|dd_pool
decl_stmt|;
comment|/* Retrieve the mountpoint value stored in the zap opbject */
name|error
operator|=
...
return|;
block|}
comment|/* Process the dsname and source to find the full mountpoint string */
if|if
condition|(
name|value
index|[
...
expr_stmt|;
block|}
comment|/* 	 * In order to return the full list of conflicting snapshots, we check 	 * whether there was a conflict after traversing all of them. 	 */
if|if
condition|(
name|conflicting_snaps
condition|)
...
argument_list|)
expr_stmt|;
comment|/* 	 * Return the first conflicting snapshot found. 	 */
name|snap_pair
operator|=
name|nvlist_next_nvpair
argument_list|(
