...
begin_comment
comment|/*  * FUID Domain table(s).  *  * The FUID table is stored as a packed nvlist of an array  * of nvlists which contain an index, domain string and offset  *  * During file system initialization the nvlist(s) are read and  * two AVL trees are created.  One tree is keyed by the index number  * and the other by the domain string.  Nodes are never removed from  * trees, but new entries may be added.  If a new entry is added then the  * on-disk packed nvlist will also be updated.  */
end_comment
...
condition|)
block|{
comment|/* first make sure we need to allocate object */
name|error
operator|=
name|zap_lookup
argument_list|(
...
begin_comment
comment|/*  * Query domain table for a given domain.  *  * If domain isn't found it is added to AVL trees and  * the results are pushed out to disk.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 		 * Now resync the on-disk nvlist. 		 */
name|VERIFY
argument_list|(
name|nvlist_alloc
argument_list|(
