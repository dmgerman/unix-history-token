...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright (c) 2011 Pawel Jakub Dawidek<pawel@dawidek.net>.  * All rights reserved.  * Copyright (c) 2012, 2015 by Delphix. All rights reserved.  * Copyright (c) 2014 Integros [integros.com]  */
end_comment
...
expr_stmt|;
block|}
comment|/* 	 * We need to enter pool configuration here, so that we can use 	 * dsl_prop_get_int_ds() to handle the special nbmand property below. 	 * dsl_prop_get_integer() can not be used, because it has to acquire 	 * spa_namespace_lock and we can not do that because we already hold 	 * z_teardown_lock.  The problem is that spa_config_sync() is called 	 * with spa_namespace_lock held and the function calls ZFS vnode 	 * operations to write the cache file and thus z_teardown_lock is 	 * acquired after spa_namespace_lock. 	 */
name|ds
operator|=
name|dmu_objset_ds
argument_list|(
...
operator|)
return|;
comment|/* 	 * Only use the name cache if we are looking for a 	 * name on a file system that does not require normalization 	 * or case folding.  We can also look there if we happen to be 	 * on a non-normalizing, mixed sensitivity file system IF we 	 * are looking for the exact name (which is always the case on 	 * FreeBSD). 	 */
name|zfsvfs
operator|->
...
name|vfs_data
decl_stmt|;
comment|/* 		 * Refresh mount options with z_teardown_lock blocking I/O while 		 * the filesystem is in an inconsistent state. 		 * The lock also serializes this code with filesystem 		 * manipulations between entry to zfs_suspend_fs() and return 		 * from zfs_resume_fs(). 		 */
name|rrm_enter
argument_list|(
operator|&
name|zfsvfs
...
goto|;
block|}
comment|/* Initial root mount: try hard to import the requested root pool. */
if|if
condition|(
operator|(
name|vfsp
...
return|;
block|}
comment|/* 	 * A zero fid_gen means we are in .zfs or the .zfs/snapshot 	 * directory tree. If the object == zfsvfs->z_shares_dir, then 	 * we are in the .zfs/shares directory tree. 	 */
if|if
condition|(
operator|(
name|fid_gen
...
condition|)
block|{
comment|/* 			 * XXX This branch must not be taken, 			 * if it is, then the lookup below will 			 * explode. 			 */
name|cn
operator|.
name|cn_nameptr
operator|=
