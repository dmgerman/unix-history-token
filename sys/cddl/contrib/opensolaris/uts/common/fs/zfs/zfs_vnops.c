...
argument_list|,
operator|(
literal|"unexpected offset in mappedread for sendfile"
operator|)
argument_list|)
expr_stmt|;
if|if
...
break|break;
block|}
comment|/* 		 * If dmu_assign_arcbuf() is expected to execute with minimum 		 * overhead loan an arc buffer and copy user data to it before 		 * we enter a txg.  This avoids holding a txg forever while we 		 * pagefault on a hanging NFS server mapping. 		 */
if|if
condition|(
name|abuf
operator|==
