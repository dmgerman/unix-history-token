...
argument_list|)
expr_stmt|;
comment|/* Drop an extra reference from zfs_dirent_lock(). */
name|VN_RELE
argument_list|(
name|vp
argument_list|)
...
goto|;
block|}
comment|/* 	 * POSIX: "If the old argument and the new argument 	 * both refer to links to the same existing file, 	 * the rename() function shall return successfully 	 * and perform no other action." 	 * This case is already handled in kern_rename(). 	 */
name|ASSERT
argument_list|(
name|svp
operator|!=
...
comment|/* if (sdzp->z_id> tdzp->z_id) */
block|{
name|terr
operator|=
name|zfs_dirent_lock
...
goto|;
block|}
comment|/* Remove an extra reference from zfs_dirent_lock(). */
name|vrele
argument_list|(
name|svp
argument_list|)
...
directive|if
literal|0
comment|/* 	 * We do it from zfs_inactive(), because after zfs_inactive() we can't 	 * VOP_WRITE() to the vnode. 	 */
comment|/* 	 * Destroy the vm object and flush associated pages. 	 */
block|vnode_destroy_vobject(vp);
endif|#
directive|endif
name|VI_LOCK
argument_list|(
...
directive|if
literal|0
block|vnode_t		*vp = ap->a_vp; 	znode_t		*zp, *xzp; 	zfsvfs_t	*zfsvfs; 	zfs_dirlock_t	*dl; 	int		error;
endif|#
directive|endif
switch|switch
condition|(
