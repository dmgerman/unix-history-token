...
begin_comment
comment|/*  * Returns the configuration from the label of the given vdev. If 'label' is  * VDEV_BEST_LABEL, each label of the vdev will be read until a valid  * configuration is found; otherwise, only the specified label will be read.  */
end_comment
...
name|ubl_label
decl_stmt|;
comment|/* Label associated with the above */
block|}
struct|;
end_struct
...
condition|)
block|{
comment|/* 			 * Keep track of the vdev and label in which this 			 * uberblock was found. We will use this information 			 * later to obtain the config nvlist associated with 			 * this uberblock. 			 */
operator|*
name|cbp
operator|->
name|ubl_ubbest
...
begin_comment
comment|/*  * Reads the 'best' uberblock from disk along with its associated  * configuration. First, we read the uberblock array of each label of each  * vdev, keeping track of the uberblock with the highest txg in each array.  * Then, we read the configuration from the same label as the best uberblock.  */
end_comment
