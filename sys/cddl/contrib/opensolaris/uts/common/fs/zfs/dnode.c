...
argument_list|)
expr_stmt|;
comment|/* XXX - for now, we can't make nblkptr smaller */
name|ASSERT3U
argument_list|(
name|dn
operator|->
...
argument_list|)
expr_stmt|;
comment|/* fix up the bonus db_size if dn_nblkptr has changed */
if|if
condition|(
name|dn
operator|->
...
name|children_dnodes
decl_stmt|;
comment|/* 	 * If you are holding the spa config lock as writer, you shouldn't 	 * be asking the DMU to do *anything*. 	 */
name|ASSERT
argument_list|(
...
condition|)
block|{
comment|/* Freeing the whole block; fast-track this request */
name|blkid
operator|=
literal|0
expr_stmt|;
...
literal|1
expr_stmt|;
comment|/* 	 * Read in and mark all the level-1 indirects dirty, 	 * so that they will stay in memory until syncing phase. 	 * Always dirty the first and last indirect to make sure 	 * we dirty all the partial indirects. 	 */
if|if
condition|(
name|dn
operator|->
