...
name|B_TRUE
expr_stmt|;
comment|/* 	 * Now that we've written this log block, we have a stable pointer 	 * to the next block in the chain, so it's OK to let the txg in 	 * which we allocated the next block sync. We still have the 	 * zl_lock to ensure zil_sync doesn't kmem free the lwb. 	 */
...
begin_comment
comment|/*  * Read in the data for the dmu_sync()ed block, and change the log  * record to write this whole block.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 		 * If the blksz is zero then we must be replaying a log 		 * from an version prior to setting the blksize of null blocks. 		 * So we just zero the actual write size reqeusted. 		 */
if|if
condition|(
name|blksz
operator|==
...
name|lrc_seq
expr_stmt|;
comment|/* Only byteswap (if needed) on the 1st pass.  */
name|error
operator|=
name|zr
operator|->
...
condition|)
return|return;
comment|/* 		 * The DMU's dnode layer doesn't see removes until the txg 		 * commits, so a subsequent claim can spuriously fail with 		 * EEXIST. So if we receive any error we try syncing out 		 * any removes then retry the transaction. 		 */
if|if
condition|(
name|pass
operator|==
