...
name|NULL
expr_stmt|;
comment|/* 	 * We shouldn't be doing any scrubbing while we're doing log 	 * replay, it's OK to not lock. 	 */
name|error
operator|=
...
name|zh_log
expr_stmt|;
comment|/* 	 * If we don't already have an initial log block or we have one 	 * but it's the wrong endianness then allocate one. 	 */
if|if
condition|(
name|BP_IS_HOLE
argument_list|(
...
begin_comment
comment|/*  * zil_rollback_destroy() is only called by the rollback code.  * We already have a syncing tx. Rollback has exclusive access to the  * dataset, so we don't have to worry about concurrent zil access.  * The actual freeing of any log blocks occurs in zil_sync() later in  * this txg syncing phase.  */
end_comment
...
name|B_FALSE
expr_stmt|;
comment|/* 	 * Ensure there's no outstanding ZIL IO.  No lwbs or just the 	 * unused one that allocated in advance is ok. 	 */
name|ASSERT
argument_list|(
name|zilog
operator|->
...
block|{
comment|/* 			 * On the first pass, arrange for the replay vector 			 * to fail its dmu_tx_assign().  That's the only way 			 * to ensure that those code paths remain well tested. 			 * 			 * Only byteswap (if needed) on the 1st pass. 			 */
operator|*
name|zr
operator|->
name|zr_txgp
