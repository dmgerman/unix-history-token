...
name|zfs_dbgmsg
argument_list|(
literal|"internal %s pool:%s txg:%llu %s"
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* spa_history_log_sync will free ha and strings */
return|return
operator|(
name|err
operator|)
...
name|NULL
argument_list|,
literal|"pool spa %llu; zfs spa %llu; zpl %d; uts %s %s %s %s"
argument_list|,
operator|(
name|u_longlong_t
operator|)
...
directive|if
literal|0
block|cmn_err(CE_CONT, "!%s version %llu pool %s using %llu", 	    event == LOG_POOL_IMPORT ? "imported" : 	    event == LOG_POOL_CREATE ? "created" : "accessed", 	    (u_longlong_t)current_vers, spa_name(spa), SPA_VERSION);
endif|#
directive|endif
endif|#
directive|endif
