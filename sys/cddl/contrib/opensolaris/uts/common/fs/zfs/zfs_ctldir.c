...
directive|else
comment|/* 		 * NB: can not use VFS_ROOT here as it would acquire 		 * the vnode lock of the parent (root) vnode while 		 * holding the child's (.zfs) lock. 		 */
name|znode_t
modifier|*
name|rootzp
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 	 * The vnode may get reclaimed between dropping sd_lock and 	 * getting the vnode lock. 	 * */
name|err
operator|=
name|vn_lock
argument_list|(
...
name|v_data
expr_stmt|;
comment|/* this may already have been unmounted */
if|if
condition|(
name|sdp
operator|==
