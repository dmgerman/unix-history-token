...
argument_list|)
expr_stmt|;
comment|/* Leave dir open until this callback is unregistered */
return|return
operator|(
literal|0
operator|)
...
name|err
decl_stmt|;
comment|/* 	 * Do these checks before the syncfunc, since it can't fail. 	 */
while|while
condition|(
operator|(
name|elem
...
literal|0
expr_stmt|;
comment|/* 		 * If we are just after the props that have been set 		 * locally, then we are done after the first iteration. 		 */
if|if
condition|(
name|local
condition|)
