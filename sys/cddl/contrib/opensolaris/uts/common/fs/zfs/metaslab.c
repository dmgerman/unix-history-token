...
literal|0
argument_list|,
literal|"Percentage of metaslab group size that should be free"
literal|" to make it eligible for allocation"
argument_list|)
expr_stmt|;
end_expr_stmt
...
name|TUNABLE_INT
argument_list|(
literal|"vfs.zfs.metaslab.fragmentation_threshold"
argument_list|,
operator|&
name|zfs_metaslab_fragmentation_threshold
argument_list|)
...
literal|0
argument_list|,
literal|"Load all metaslabs when pool is first opened"
argument_list|)
expr_stmt|;
end_expr_stmt
...
literal|0
argument_list|,
literal|"Prevent metaslabs from being unloaded"
argument_list|)
expr_stmt|;
end_expr_stmt
...
literal|0
argument_list|,
literal|"Percentage of cpus that can be used by the metaslab taskq"
argument_list|)
expr_stmt|;
end_expr_stmt
...
literal|0
argument_list|,
literal|"Number of TXGs that an unused metaslab can be kept in memory"
argument_list|)
expr_stmt|;
end_expr_stmt
...
literal|0
argument_list|,
literal|"Max number of metaslabs per group to preload"
argument_list|)
expr_stmt|;
end_expr_stmt
...
name|TUNABLE_INT
argument_list|(
literal|"vfs.zfs.metaslab_fragmentation_factor_enabled"
argument_list|,
operator|&
name|metaslab_fragmentation_factor_enabled
argument_list|)
...
name|OID_AUTO
argument_list|,
name|fragmentation_factor_enabled
argument_list|,
name|CTLFLAG_RWTUN
argument_list|,
operator|&
...
literal|0
argument_list|,
literal|"Enable fragmentation weighting on metaslabs"
argument_list|)
expr_stmt|;
end_expr_stmt
...
name|TUNABLE_INT
argument_list|(
literal|"vfs.zfs.metaslab.lba_weighting_enabled"
argument_list|,
operator|&
name|metaslab_lba_weighting_enabled
argument_list|)
...
literal|0
argument_list|,
literal|"Enable LBA weighting (i.e. outer tracks are given preference)"
argument_list|)
expr_stmt|;
end_expr_stmt
...
name|TUNABLE_INT
argument_list|(
literal|"vfs.zfs.metaslab.bias_enabled"
argument_list|,
operator|&
name|metaslab_bias_enabled
argument_list|)
...
literal|0
argument_list|,
literal|"Enable metaslab group biasing"
argument_list|)
expr_stmt|;
end_expr_stmt
...
name|mg_class
decl_stmt|;
comment|/* 	 * We use two key metrics to determine if a metaslab group is 	 * considered allocatable -- free space and fragmentation. If 	 * the free space is greater than the free space threshold and 	 * the fragmentation is less than the fragmentation threshold then 	 * consider the group allocatable. There are two case when we will 	 * not consider these key metrics. The first is if the group is 	 * associated with a slog device and the second is if all groups 	 * in this metaslab class have already been consider ineligible 	 * for allocations. 	 */
return|return
operator|(
operator|(
name|mg
...
operator|)
return|;
comment|/* 	 * If this metaslab's space_map has not been upgraded, flag it 	 * so that we upgrade next time we encounter it. 	 */
if|if
condition|(
name|msp
operator|->
