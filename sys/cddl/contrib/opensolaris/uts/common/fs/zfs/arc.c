...
name|DTRACE_PROBE
argument_list|(
name|arc__reclaim_aggr_no_grow
argument_list|)
expr_stmt|;
name|last_reclaim
operator|=
...
name|NULL
expr_stmt|;
comment|/* 				 * We are destroying l2hdr, so ensure that 				 * its compressed buffer, if any, is not leaked. 				 */
name|ASSERT
argument_list|(
name|abl2
operator|->
...
name|b_asize
expr_stmt|;
comment|/* 		 * If the data has not been compressed, then clear b_tmp_cdata 		 * to make sure that it points only to a temporary compression 		 * buffer. 		 */
if|if
condition|(
operator|!
name|L2ARC_IS_VALID_COMPRESS
