...
name|b_acb
decl_stmt|;
comment|/* temporary buffer holder for in-flight compressed or padded data */
name|void
modifier|*
name|b_tmp_cdata
decl_stmt|;
...
return|return;
block|}
comment|/* 	 * The bufer has been chosen for writing to L2ARC, but it's 	 * not being written just yet.  In other words, 	 * b_tmp_cdata points to exactly the same buffer as b_data, 	 * l2arc_transform_buf hasn't been called. 	 */
if|if
condition|(
name|hdr
operator|->
...
return|return;
block|}
comment|/* 	 * Nothing to do if the temporary buffer was not required. 	 */
if|if
condition|(
name|hdr
operator|->
...
name|l2ad_hand
expr_stmt|;
comment|/* 		 * Save a pointer to the original buffer data we had previously 		 * stashed away. 		 */
name|buf_data
operator|=
name|hdr
operator|->
...
expr_stmt|;
block|}
comment|/* 		 * Get the new buffer size that accounts for compression 		 * and padding. 		 */
name|buf_sz
operator|=
name|hdr
operator|->
...
condition|)
block|{
comment|/* 			 * If the data was padded or compressed, then it 			 * it is in a new buffer. 			 */
if|if
condition|(
name|hdr
operator|->
...
block|{
comment|/* 		 * Compression did not save space. 		 */
if|if
condition|(
name|P2PHASE
argument_list|(
...
condition|)
block|{
comment|/* 			 * Use compression buffer for a copy of data padded to 			 * the proper size.  Compression algorithm remains set 			 * to ZIO_COMPRESS_OFF. 			 */
name|ASSERT3U
argument_list|(
name|len
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 			 * The original buffer is good as is, 			 * release the compressed buffer. 			 * l2hdr will be left unmodified except for b_tmp_cdata. 			 */
name|zio_data_buf_free
argument_list|(
name|cdata
argument_list|,
...
begin_comment
comment|/*  * Releases the temporary b_tmp_cdata buffer in an l2arc header structure.  * This buffer serves as a temporary holder of compressed or padded data while  * the buffer entry is being written to an l2arc device. Once that is  * done, we can dispose of it.  */
end_comment
