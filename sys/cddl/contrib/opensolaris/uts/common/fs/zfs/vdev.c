...
argument_list|)
expr_stmt|;
comment|/* 		 * If spa->spa_load_verbatim is true, no need to check the 		 * state of the pool. 		 */
if|if
condition|(
operator|!
name|spa
...
return|;
block|}
comment|/* 	 * If we successfully offlined the log device then we need to 	 * sync out the current txg so that the "stubby" block can be 	 * removed by zil_sync(). 	 */
...
condition|)
block|{
comment|/* 		 * It would be nice to call vdev_offline() 		 * directly but the pool isn't fully loaded and 		 * the txg threads have not been started yet. 		 */
