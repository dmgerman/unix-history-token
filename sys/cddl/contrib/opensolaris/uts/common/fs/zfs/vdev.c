...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright 2011 Nexenta Systems, Inc.  All rights reserved.  * Copyright (c) 2011, 2014 by Delphix. All rights reserved.  * Copyright 2013 Martin Matuska<mm@FreeBSD.org>. All rights reserved.  */
end_comment
...
begin_comment
comment|/**  * The limit for ZFS to automatically increase a top-level vdev's ashift  * from logical ashift to physical ashift.  *  * Example: one or more 512B emulation child vdevs  *          child->vdev_ashift = 9 (512 bytes)  *          child->vdev_physical_ashift = 12 (4096 bytes)  *          zfs_max_auto_ashift = 11 (2048 bytes)  *  * On pool creation or the addition of a new top-leve vdev, ZFS will  * bump the ashift of the top-level vdev to 2048.  *  * Example: one or more 512B emulation child vdevs  *          child->vdev_ashift = 9 (512 bytes)  *          child->vdev_physical_ashift = 12 (4096 bytes)  *          zfs_max_auto_ashift = 13 (8192 bytes)  *  * On pool creation or the addition of a new top-leve vdev, ZFS will  * bump the ashift of the top-level vdev to 4096.  */
end_comment
...
literal|"QU"
argument_list|,
literal|"Cap on logical -> physical ashift adjustment on new top-level vdevs."
argument_list|)
expr_stmt|;
end_expr_stmt
...
begin_comment
comment|/*  * Maximize performance by inflating the configured ashift for  * top level vdevs to be as close to the physical ashift as  * possible without exceeding the administrator specified  * limit.  */
end_comment
