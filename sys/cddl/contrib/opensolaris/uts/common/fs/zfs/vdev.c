...
argument_list|)
expr_stmt|;
comment|/* 		 * When importing a pool, we want to ignore the persistent fault 		 * state, as the diagnosis made on another system may not be 		 * valid in the current context. 		 */
if|if
condition|(
name|spa
operator|->
...
return|;
block|}
comment|/* 		 * If this vdev just became a top-level vdev because its 		 * sibling was detached, it will have adopted the parent's 		 * vdev guid -- but the label may or may not be on disk yet. 		 * Fortunately, either version of the label will have the 		 * same top guid, so if we're a top-level vdev, we can 		 * safely compare to that instead. 		 */
if|if
condition|(
name|nvlist_lookup_uint64
argument_list|(
...
condition|)
block|{
comment|/* XXX should check scrub_done? */
comment|/* 			 * We completed a scrub up to scrub_txg.  If we 			 * did it without rebooting, then the scrub dtl 			 * will be valid, so excise the old region and 			 * fold in the scrub dtl.  Otherwise, leave the 			 * dtl as-is if there was an error. 			 */
...
argument_list|)
expr_stmt|;
comment|/* 	 * If marking the vdev as faulted cause the top-level vdev to become 	 * unavailable, then back off and simply mark the vdev as degraded 	 * instead. 	 */
if|if
condition|(
name|vdev_is_dead
argument_list|(
...
parameter_list|)
block|{
comment|/* 	 * We currently allow allocations from vdevs which maybe in the 	 * process of reopening (i.e. VDEV_STATE_CLOSED). If the device 	 * fails to reopen then we'll catch it later when we're holding 	 * the proper locks. 	 */
return|return
operator|(
operator|!
operator|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Don't count non-normal (e.g. intent log) space as part of 		 * the pool's capacity. 		 */
if|if
condition|(
name|vd
operator|->
...
name|c
decl_stmt|;
comment|/* 	 * If this is an aux vdev (as with l2cache devices), then we update the 	 * vdev config manually and set the sync flag. 	 */
if|if
condition|(
name|vd
operator|->
...
condition|)
block|{
comment|/* 		 * Indicate to the ZFS DE that this device has been removed, and 		 * any recent errors should be ignored. 		 */
