...
directive|ifndef
name|_KERNEL
argument|int zcmd_ioctl_compat(int fd, int request, zfs_cmd_t *zc, const int cflag) { 	int nc, ret; 	void *zc_c; 	unsigned long ncmd;  	switch (cflag) { 	case ZFS_CMD_COMPAT_NONE: 		ncmd = _IOWR(
literal|'Z'
argument|, request, struct zfs_cmd); 		ret = ioctl(fd, ncmd, zc); 		return (ret); 	case ZFS_CMD_COMPAT_DEADMAN: 		zc_c = malloc(sizeof(zfs_cmd_deadman_t)); 		ncmd = _IOWR(
literal|'Z'
