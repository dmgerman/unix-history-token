...
name|field
parameter_list|)
value|zc->field = inlanes_c->field
name|FIELD_COPY
argument_list|(
name|zc_nvlist_src
argument_list|)
...
name|field
parameter_list|)
value|inlanes_c->field = zc->field
name|FIELD_COPY
argument_list|(
name|zc_nvlist_src
argument_list|)
...
literal|'Z'
argument|, request, struct zfs_iocparm); 		zp.zfs_cmd = (uint64_t)zc; 		zp.zfs_cmd_size = sizeof(zfs_cmd_t); 		zp.zfs_ioctl_version = ZFS_IOCVER_CURRENT; 		return (ioctl(fd, ncmd,&zp)); 	case ZFS_CMD_COMPAT_INLANES: 		ncmd = _IOWR(
literal|'Z'
argument|, request, struct zfs_iocparm); 		zp.zfs_cmd = (uint64_t)zc; 		zp.zfs_cmd_size = sizeof(zfs_cmd_inlanes_t); 		zp.zfs_ioctl_version = ZFS_IOCVER_INLANES; 		return (ioctl(fd, ncmd,&zp)); 	case ZFS_CMD_COMPAT_RESUME: 		ncmd = _IOWR(
literal|'Z'
argument|, request, struct zfs_iocparm); 		zp.zfs_cmd = (uint64_t)zc; 		zp.zfs_cmd_size = sizeof(zfs_cmd_resume_t); 		zp.zfs_ioctl_version = ZFS_IOCVER_RESUME; 		return (ioctl(fd, ncmd,&zp)); 	case ZFS_CMD_COMPAT_EDBP: 		ncmd = _IOWR(
literal|'Z'
argument|, request, struct zfs_iocparm); 		zp.zfs_cmd = (uint64_t)zc; 		zp.zfs_cmd_size = sizeof(zfs_cmd_edbp_t); 		zp.zfs_ioctl_version = ZFS_IOCVER_EDBP; 		return (ioctl(fd, ncmd,&zp)); 	case ZFS_CMD_COMPAT_ZCMD: 		ncmd = _IOWR(
literal|'Z'
