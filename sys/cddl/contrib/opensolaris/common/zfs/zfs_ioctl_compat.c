...
begin_comment
comment|/*  * Copyright 2010 Martin Matuska<mm@FreeBSD.org>. All rights reserved.  * Use is subject to license terms.  */
end_comment
...
specifier|static
name|int
name|zfs_ioctl_compat_write_nvlist_dst
parameter_list|(
name|zfs_cmd_t
modifier|*
name|zc
...
directive|endif
argument|DATA_TYPE_UINT64_ARRAY); 	} }  static void zfs_ioctl_compat_fix_stats(zfs_cmd_t *zc, const int cflag) { 	nvlist_t *nv, *nvp = NULL; 	nvpair_t *elem; 	size_t nvsize; 	char *packed;  	if (nvlist_unpack((void *)(uintptr_t)zc->zc_nvlist_dst, 	    zc->zc_nvlist_dst_size,&nv,
literal|0
argument|) !=
literal|0
argument|) 		return;  	if (cflag ==
literal|5
argument|) {
comment|/* ZFS_IOC_POOL_STATS */
argument|elem = NULL; 		while ((elem = nvlist_next_nvpair(nv, elem)) != NULL) { 			if (nvpair_value_nvlist(elem,&nvp) ==
literal|0
argument|) 				zfs_ioctl_compat_fix_stats_nvlist(nvp); 		} 		elem = NULL; 	} else 		zfs_ioctl_compat_fix_stats_nvlist(nv);  	VERIFY(nvlist_size(nv,&nvsize, NV_ENCODE_NATIVE) ==
literal|0
argument|); 	zfs_ioctl_compat_write_nvlist_dst(zc, nv, nvsize);  	nvlist_free(nv); }  static void zfs_ioctl_compat_pool_get_props(zfs_cmd_t *zc) { 	nvlist_t *nv, *nva = NULL; 	size_t nvsize;  	if (nvlist_unpack((void *)(uintptr_t)zc->zc_nvlist_dst, 	    zc->zc_nvlist_dst_size,&nv,
literal|0
argument|) !=
literal|0
argument|) 		return;
...
directive|endif
argument|VERIFY(nvlist_size(nv,&nvsize, NV_ENCODE_NATIVE) ==
literal|0
argument|); 	zfs_ioctl_compat_write_nvlist_dst(zc, nv, nvsize);  	nvlist_free(nv); }
ifndef|#
directive|ifndef
name|_KERNEL
