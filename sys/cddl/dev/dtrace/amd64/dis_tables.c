...
directive|pragma
name|ident
literal|"@(#)dis_tables.c	1.11	06/03/02 SMI"
end_pragma
...
begin_comment
comment|/*  * Similar, but for 2 operands plus an immediate.  */
end_comment
...
name|immsize
parameter_list|)
value|{ \ 		dtrace_get_modrm(x,&mode,&reg,&r_m);			\ 		dtrace_rex_adjust(rex_prefix, mode,&reg,&r_m);	\ 		dtrace_get_operand(x, mode, r_m, wbit, 1);		\ 		dtrace_get_operand(x, REG_ONLY, reg, w2, 2);		\ 		dtrace_imm_opnd(x, wbit, immsize, 0);			\ }
end_define
...
name|opcode5
decl_stmt|;
comment|/* low nibble of 2ne byte */
name|uint_t
name|opcode6
decl_stmt|;
...
expr_stmt|;
break|break;
comment|/* 		 * movsbl movsbw movsbq (0x0FBE) or movswl movswq (0x0FBF) 		 * movzbl movzbw movzbq (0x0FB6) or mobzwl movzwq (0x0FB7) 		 * wbit lives in 2nd byte, note that operands 		 * are different sized 		 */
case|case
name|MOVZ
case|:
if|if
