...
begin_comment
comment|/*  * Copyright (c) 2003, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright 2016 Joyent, Inc.  */
end_comment
...
literal|1
decl_stmt|;
comment|/* VEX instructions that don't use XMM/YMM */
name|uint_t
name|it_avxsuf
range|:
literal|1
decl_stmt|;
comment|/* AVX suffix required */
block|}
name|instable_t
typedef|;
end_typedef
...
name|VEX_VRMrX
block|,
comment|/* VEX  mod_rm, VEX.vvvv               -> mod_rm */
name|VEX_RRX
block|,
...
name|VEX_RIM
block|,
comment|/* VEX  mod_reg, imm8                  -> mod_rm */
name|VEX_RRM
block|,
comment|/* VEX  VEX.vvvv, mod_reg              -> mod_rm */
name|VEX_RMX
...
name|VEX_SbVM
block|,
comment|/* VEX  SIB, VEX.vvvv                  -> mod_rm */
name|VMx
block|,
...
name|BLS
block|,
comment|/* BLSR, BLSMSK, BLSI */
name|FMA
block|,
comment|/* FMA instructions, all VEX_RMrX */
name|ADX
comment|/* ADX instructions, support REX.w, mod_rm->mod_reg */
block|}
enum|;
end_enum
...
name|sz
parameter_list|)
value|{TERM, amode, name, 0, sz, 0, 0, 0, 0, 1}
end_define
...
name|sz
parameter_list|)
value|{TERM, amode, name, 1, sz, 0, 0, 0, 0, 0, 1}
end_define
...
name|sz
parameter_list|)
value|{TERM, amode, name, 0, 0, 0, 0, 0, 1}
end_define
...
name|sz
parameter_list|)
value|{TERM, amode, name, 1, 0, 0, 0, 0, 0, 1}
end_define
...
name|sz
parameter_list|)
value|{TERM, amode, sz, 0, 0, 0, 0, 0, 1}
end_define
...
begin_comment
comment|/*  * decode for scale from VSIB byte, note that we always include the scale factor  * to match gas.  */
end_comment
...
begin_comment
comment|/*  * The following table is used to distinguish between adox and adcx which share  * the same opcodes.  */
end_comment
...
begin_comment
comment|/*  * 	Decode table for 0x0F0D which uses the first byte of the mod_rm to  * 	indicate a sub-code.  */
end_comment
...
begin_comment
comment|/*  * The AVX2 gather instructions are a bit of a mess. While there's a pattern,  * there's not really a consistent scheme that we can use to know what the mode  * is supposed to be for a given type. Various instructions, like VPGATHERDD,  * always match the value of VEX_L. Other instructions like VPGATHERDQ, have  * some registers match VEX_L, but the VSIB is always XMM.  *  * The simplest way to deal with this is to just define a table based on the  * instruction opcodes, which are 0x90-0x93, so we subtract 0x90 to index into  * them.  *  * We further have to subdivide this based on the value of VEX_W and the value  * of VEX_L. The array is constructed to be indexed as:  * 	[opcode - 0x90][VEX_W][VEX_L].  */
end_comment
...
name|dgr_suffix
decl_stmt|;
comment|/* suffix to append */
block|}
name|dis_gather_regs_t
typedef|;
end_typedef
...
directive|ifdef
name|DIS_TEXT
comment|/* Instruction name for BLS* family of instructions */
name|char
modifier|*
name|blsinstr
decl_stmt|;
...
block|}
block|}
comment|/* 			 * The adx family of instructions (adcx and adox) 			 * continue the classic Intel tradition of abusing 			 * arbitrary prefixes without actually meaning the 			 * prefix bit. Therefore, if we find either the 			 * opnd_size_prefix or rep_prefix we end up zeroing it 			 * out after making our determination so as to ensure 			 * that we don't get confused and accidentally print 			 * repz prefixes and the like on these instructions. 			 * 			 * In addition, these instructions are actually much 			 * closer to AVX instructions in semantics. Importantly, 			 * they always default to having 32-bit operands. 			 * However, if the CPU is in 64-bit mode, then and only 			 * then, does it use REX.w promotes things to 64-bits 			 * and REX.r allows 64-bit mode to use register r8-r15. 			 */
if|if
condition|(
name|dp
operator|->
...
name|error
goto|;
comment|/* 	 * Deal with MMX/SSE opcodes which are changed by prefixes. Note, we do 	 * need to include UNKNOWN below, as we may have instructions that 	 * actually have a prefix, but don't exist in any other form. 	 */
switch|switch
condition|(
name|dp
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 		 * In classic Intel fashion, the opcodes for all of the FMA 		 * instructions all have two possible mnemonics which vary by 		 * one letter, which is selected based on the value of the wbit. 		 * When wbit is one, they have the 'd' suffix and when 'wbit' is 		 * 0, they have the 's' suffix. Otherwise, the FMA instructions 		 * are all a standard VEX_RMrX. 		 */
ifdef|#
directive|ifdef
name|DIS_TEXT
if|if
...
name|VEX_VRMrX
case|:
comment|/* ModR/M.reg := op(MODR/M.r/m, VEX.vvvv) */
name|x
operator|->
name|d86_numopnds
operator|=
...
name|VEX_SbVM
case|:
comment|/* ModR/M.reg := op(MODR/M.r/m, VSIB, VEX.vvvv) */
name|x
operator|->
name|d86_numopnds
operator|=
...
literal|1
expr_stmt|;
comment|/* 		 * All instructions that use VSIB are currently a mess. See the 		 * comment around the dis_gather_regs_t structure definition. 		 */
name|vreg
operator|=
operator|&
name|dis_vgather
...
comment|/* or vcvtph2ps<xmm>,<ymm> */
comment|/* or vbroadcasts*<xmm>,<ymm> */
name|dtrace_get_operand
argument_list|(
name|x
argument_list|,
