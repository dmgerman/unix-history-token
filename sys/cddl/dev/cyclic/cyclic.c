...
name|cyx_cyclics
decl_stmt|;
comment|/* Disable preemption and interrupts. */
name|mtx_lock_spin
argument_list|(
operator|&
name|cpu
...
argument_list|)
expr_stmt|;
comment|/* 	 * Assert that the new size is a power of 2. 	 */
name|ASSERT
argument_list|(
operator|(
name|new_size
...
condition|)
block|{
comment|/* 		 * If a start time hasn't been explicitly specified, we'll 		 * start on the next interval boundary. 		 */
name|cyclic
operator|->
name|cy_expire
operator|=
...
condition|)
block|{
comment|/* 		 * This is expensive; it will cross call onto the other 		 * CPU to perform the expansion. 		 */
name|cyclic_expand
argument_list|(
name|cpu
argument_list|)
...
expr_stmt|;
block|}
comment|/* 	 * By now, we know that we're going to be able to successfully 	 * perform the add.  Now cross call over to the CPU of interest to 	 * actually add our cyclic. 	 */
name|arg
operator|.
name|cyx_cpu
operator|=
...
expr_stmt|;
block|}
comment|/* 	 * Now set the flags to CYF_FREE.  We don't need a membar_enter() 	 * between zeroing pend and setting the flags because we're at 	 * CY_HIGH_LEVEL (that is, the zeroing of pend and the setting 	 * of cy_flags appear atomic to softints). 	 */
name|cyclic
operator|->
name|cy_flags
operator|=
