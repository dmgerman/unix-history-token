...
argument_list|,
operator|(
literal|"evtchn_to_irq inconsistent"
operator|)
argument_list|)
expr_stmt|;
if|if
...
begin_endif
unit|if (HYPERVISOR_event_channel_op(&op)>= 0) 		bind_evtchn_to_cpu(evtchn, tcpu);  	mtx_unlock_spin(&irq_mapping_update_lock);  }  static void set_affinity_irq(unsigned irq, cpumask_t dest) { 	unsigned tcpu = ffs(dest) - 1; 	rebind_irq_to_cpu(irq, tcpu); }
endif|#
directive|endif
end_endif
...
argument_list|,
operator|(
literal|"pirq_to_irq inconsistent"
operator|)
argument_list|)
expr_stmt|;
block|}
...
argument_list|,
operator|(
literal|"virq_to_irq inconsistent"
operator|)
argument_list|)
expr_stmt|;
block|}
...
argument_list|,
operator|(
literal|"ipi_to_irq inconsistent"
operator|)
argument_list|)
expr_stmt|;
block|}
