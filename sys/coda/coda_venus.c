...
directive|if
literal|0
block|int cflag;     DECL(coda_open);
comment|/* sets Isize& Osize */
block|ALLOC(coda_open);
comment|/* sets inp& outp */
comment|/* send the open to venus. */
block|INIT_IN(&inp->ih, CODA_OPEN, cred, p);     inp->Fid = *fid;     CNV_OFLAG(cflag, flag);     inp->flags = cflag;      error = coda_call(mdp, Isize,&Osize, (char *)inp);     if (!error) { 	*dev =  findcdev(outp->dev); 	*inode = outp->inode;     }      CODA_FREE(inp, coda_open_size);     return error;
else|#
directive|else
return|return
operator|(
