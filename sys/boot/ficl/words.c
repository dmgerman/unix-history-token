...
argument_list|)
expr_stmt|;
comment|/* OK - now we have something in the text buffer - use it */
name|count
operator|=
name|stackPopUNS32
argument_list|(
...
operator|++
expr_stmt|;
comment|/* find next delimiter or end */
name|count
operator|=
name|pSrc
operator|-
...
condition|)
block|{
comment|/* add a local to the dict and update nLocals */
name|FICL_DICT
modifier|*
name|pLoc
init|=
...
begin_comment
comment|/*************** freebsd added memory-alloc handling words ******************/
end_comment
...
begin_comment
comment|/***************** freebsd added exception handling words *******************/
end_comment

begin_comment
comment|/*  * Catch, from ANS Forth standard. Installs a safety net, then EXECUTE  * the word in ToS. If an exception happens, restore the state to what  * it was before, and pushes the exception value on the stack. If not,  * push zero.  *  * Notice that Catch implements an inner interpreter. This is ugly,  * but given how ficl works, it cannot be helped. The problem is that  * colon definitions will be executed *after* the function returns,  * while "code" definitions will be executed immediately. I considered  * other solutions to this problem, but all of them shared the same  * basic problem (with added disadvantages): if ficl ever changes it's  * inner thread modus operandi, one would have to fix this word.  *  * More comments can be found throughout catch's code.  *  * BUGS: do not handle locals unnesting correctly... I think...  *  * Daniel C. Sobral	Jan 09/1999  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * And now, choose what to do depending on except. 	 */
comment|/* Things having gone wrong... */
if|if
condition|(
name|except
condition|)
...
name|ip
expr_stmt|;
comment|/* Execute the xt -- inline code for vmExecute */
name|pVM
operator|->
name|runningWord
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 		 * Run the inner loop until we get back to exitIP 		 */
for|for
control|(
init|;
name|pVM
...
operator|++
expr_stmt|;
comment|/* Inline code for vmExecute */
name|pVM
operator|->
name|runningWord
operator|=
