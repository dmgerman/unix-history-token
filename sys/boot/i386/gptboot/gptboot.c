begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1998 Robert Nordier  * All rights reserved.  *  * Redistribution and use in source and binary forms are freely  * permitted provided that the above copyright notice and this  * paragraph and the following disclaimer are duplicated in all  * such forms.  *  * This software is provided "AS IS" and without any express or  * implied warranties, including, without limitation, the implied  * warranties of merchantability and fitness for a particular  * purpose.  */
end_comment

begin_include
include|#
directive|include
file|<sys/cdefs.h>
end_include

begin_expr_stmt
name|__FBSDID
argument_list|(
literal|"$FreeBSD$"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/gpt.h>
end_include

begin_include
include|#
directive|include
file|<sys/dirent.h>
end_include

begin_include
include|#
directive|include
file|<sys/reboot.h>
end_include

begin_include
include|#
directive|include
file|<machine/bootinfo.h>
end_include

begin_include
include|#
directive|include
file|<machine/elf.h>
end_include

begin_include
include|#
directive|include
file|<machine/psl.h>
end_include

begin_include
include|#
directive|include
file|<stdarg.h>
end_include

begin_include
include|#
directive|include
file|<a.out.h>
end_include

begin_include
include|#
directive|include
file|<btxv86.h>
end_include

begin_include
include|#
directive|include
file|"lib.h"
end_include

begin_define
define|#
directive|define
name|IO_KEYBOARD
value|1
end_define

begin_define
define|#
directive|define
name|IO_SERIAL
value|2
end_define

begin_define
define|#
directive|define
name|SECOND
value|18
end_define

begin_comment
comment|/* Circa that many ticks in a second. */
end_comment

begin_define
define|#
directive|define
name|RBX_ASKNAME
value|0x0
end_define

begin_comment
comment|/* -a */
end_comment

begin_define
define|#
directive|define
name|RBX_SINGLE
value|0x1
end_define

begin_comment
comment|/* -s */
end_comment

begin_comment
comment|/* 0x2 is reserved for log2(RB_NOSYNC). */
end_comment

begin_comment
comment|/* 0x3 is reserved for log2(RB_HALT). */
end_comment

begin_comment
comment|/* 0x4 is reserved for log2(RB_INITNAME). */
end_comment

begin_define
define|#
directive|define
name|RBX_DFLTROOT
value|0x5
end_define

begin_comment
comment|/* -r */
end_comment

begin_define
define|#
directive|define
name|RBX_KDB
value|0x6
end_define

begin_comment
comment|/* -d */
end_comment

begin_comment
comment|/* 0x7 is reserved for log2(RB_RDONLY). */
end_comment

begin_comment
comment|/* 0x8 is reserved for log2(RB_DUMP). */
end_comment

begin_comment
comment|/* 0x9 is reserved for log2(RB_MINIROOT). */
end_comment

begin_define
define|#
directive|define
name|RBX_CONFIG
value|0xa
end_define

begin_comment
comment|/* -c */
end_comment

begin_define
define|#
directive|define
name|RBX_VERBOSE
value|0xb
end_define

begin_comment
comment|/* -v */
end_comment

begin_define
define|#
directive|define
name|RBX_SERIAL
value|0xc
end_define

begin_comment
comment|/* -h */
end_comment

begin_define
define|#
directive|define
name|RBX_CDROM
value|0xd
end_define

begin_comment
comment|/* -C */
end_comment

begin_comment
comment|/* 0xe is reserved for log2(RB_POWEROFF). */
end_comment

begin_define
define|#
directive|define
name|RBX_GDB
value|0xf
end_define

begin_comment
comment|/* -g */
end_comment

begin_define
define|#
directive|define
name|RBX_MUTE
value|0x10
end_define

begin_comment
comment|/* -m */
end_comment

begin_comment
comment|/* 0x11 is reserved for log2(RB_SELFTEST). */
end_comment

begin_comment
comment|/* 0x12 is reserved for boot programs. */
end_comment

begin_comment
comment|/* 0x13 is reserved for boot programs. */
end_comment

begin_define
define|#
directive|define
name|RBX_PAUSE
value|0x14
end_define

begin_comment
comment|/* -p */
end_comment

begin_define
define|#
directive|define
name|RBX_QUIET
value|0x15
end_define

begin_comment
comment|/* -q */
end_comment

begin_define
define|#
directive|define
name|RBX_NOINTR
value|0x1c
end_define

begin_comment
comment|/* -n */
end_comment

begin_comment
comment|/* 0x1d is reserved for log2(RB_MULTIPLE) and is just misnamed here. */
end_comment

begin_define
define|#
directive|define
name|RBX_DUAL
value|0x1d
end_define

begin_comment
comment|/* -D */
end_comment

begin_comment
comment|/* 0x1f is reserved for log2(RB_BOOTINFO). */
end_comment

begin_comment
comment|/* pass: -a, -s, -r, -d, -c, -v, -h, -C, -g, -m, -p, -D */
end_comment

begin_define
define|#
directive|define
name|RBX_MASK
value|(OPT_SET(RBX_ASKNAME) | OPT_SET(RBX_SINGLE) | \ 			OPT_SET(RBX_DFLTROOT) | OPT_SET(RBX_KDB ) | \ 			OPT_SET(RBX_CONFIG) | OPT_SET(RBX_VERBOSE) | \ 			OPT_SET(RBX_SERIAL) | OPT_SET(RBX_CDROM) | \ 			OPT_SET(RBX_GDB ) | OPT_SET(RBX_MUTE) | \ 			OPT_SET(RBX_PAUSE) | OPT_SET(RBX_DUAL))
end_define

begin_define
define|#
directive|define
name|PATH_CONFIG
value|"/boot.config"
end_define

begin_define
define|#
directive|define
name|PATH_BOOT3
value|"/boot/loader"
end_define

begin_define
define|#
directive|define
name|PATH_KERNEL
value|"/boot/kernel/kernel"
end_define

begin_define
define|#
directive|define
name|ARGS
value|0x900
end_define

begin_define
define|#
directive|define
name|NOPT
value|14
end_define

begin_define
define|#
directive|define
name|NDEV
value|3
end_define

begin_define
define|#
directive|define
name|MEM_BASE
value|0x12
end_define

begin_define
define|#
directive|define
name|MEM_EXT
value|0x15
end_define

begin_define
define|#
directive|define
name|V86_CY
parameter_list|(
name|x
parameter_list|)
value|((x)& PSL_C)
end_define

begin_define
define|#
directive|define
name|V86_ZR
parameter_list|(
name|x
parameter_list|)
value|((x)& PSL_Z)
end_define

begin_define
define|#
directive|define
name|DRV_HARD
value|0x80
end_define

begin_define
define|#
directive|define
name|DRV_MASK
value|0x7f
end_define

begin_define
define|#
directive|define
name|TYPE_AD
value|0
end_define

begin_define
define|#
directive|define
name|TYPE_DA
value|1
end_define

begin_define
define|#
directive|define
name|TYPE_MAXHARD
value|TYPE_DA
end_define

begin_define
define|#
directive|define
name|TYPE_FD
value|2
end_define

begin_define
define|#
directive|define
name|OPT_SET
parameter_list|(
name|opt
parameter_list|)
value|(1<< (opt))
end_define

begin_define
define|#
directive|define
name|OPT_CHECK
parameter_list|(
name|opt
parameter_list|)
value|((opts)& OPT_SET(opt))
end_define

begin_decl_stmt
specifier|extern
name|uint32_t
name|_end
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|uuid_t
name|freebsd_ufs_uuid
init|=
name|GPT_ENT_TYPE_FREEBSD_UFS
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|char
name|optstr
index|[
name|NOPT
index|]
init|=
literal|"DhaCcdgmnpqrsv"
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Also 'P', 'S' */
end_comment

begin_decl_stmt
specifier|static
specifier|const
name|unsigned
name|char
name|flags
index|[
name|NOPT
index|]
init|=
block|{
name|RBX_DUAL
block|,
name|RBX_SERIAL
block|,
name|RBX_ASKNAME
block|,
name|RBX_CDROM
block|,
name|RBX_CONFIG
block|,
name|RBX_KDB
block|,
name|RBX_GDB
block|,
name|RBX_MUTE
block|,
name|RBX_NOINTR
block|,
name|RBX_PAUSE
block|,
name|RBX_QUIET
block|,
name|RBX_DFLTROOT
block|,
name|RBX_SINGLE
block|,
name|RBX_VERBOSE
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|char
modifier|*
specifier|const
name|dev_nm
index|[
name|NDEV
index|]
init|=
block|{
literal|"ad"
block|,
literal|"da"
block|,
literal|"fd"
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|unsigned
name|char
name|dev_maj
index|[
name|NDEV
index|]
init|=
block|{
literal|30
block|,
literal|4
block|,
literal|2
block|}
decl_stmt|;
end_decl_stmt

begin_struct
specifier|static
struct|struct
name|dsk
block|{
name|unsigned
name|drive
decl_stmt|;
name|unsigned
name|type
decl_stmt|;
name|unsigned
name|unit
decl_stmt|;
name|int
name|part
decl_stmt|;
name|daddr_t
name|start
decl_stmt|;
name|int
name|init
decl_stmt|;
block|}
name|dsk
struct|;
end_struct

begin_decl_stmt
specifier|static
name|char
name|cmd
index|[
literal|512
index|]
decl_stmt|,
name|cmddup
index|[
literal|512
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
name|kname
index|[
literal|1024
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|uint32_t
name|opts
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|comspeed
init|=
name|SIOSPD
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|bootinfo
name|bootinfo
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|uint8_t
name|ioctrl
init|=
name|IO_KEYBOARD
decl_stmt|;
end_decl_stmt

begin_function_decl
name|void
name|exit
parameter_list|(
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|bcmp
parameter_list|(
specifier|const
name|void
modifier|*
parameter_list|,
specifier|const
name|void
modifier|*
parameter_list|,
name|size_t
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|load
parameter_list|(
name|void
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|parse
parameter_list|(
name|void
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|xfsread
parameter_list|(
name|ino_t
parameter_list|,
name|void
modifier|*
parameter_list|,
name|size_t
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|dskread
parameter_list|(
name|void
modifier|*
parameter_list|,
name|daddr_t
parameter_list|,
name|unsigned
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|printf
parameter_list|(
specifier|const
name|char
modifier|*
parameter_list|,
modifier|...
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|putchar
parameter_list|(
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|memcpy
parameter_list|(
name|void
modifier|*
parameter_list|,
specifier|const
name|void
modifier|*
parameter_list|,
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|uint32_t
name|memsize
parameter_list|(
name|void
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|drvread
parameter_list|(
name|void
modifier|*
parameter_list|,
name|daddr_t
parameter_list|,
name|unsigned
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|keyhit
parameter_list|(
name|unsigned
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|xputc
parameter_list|(
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|xgetc
parameter_list|(
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|getc
parameter_list|(
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function
specifier|static
name|void
name|memcpy
parameter_list|(
name|void
modifier|*
name|dst
parameter_list|,
specifier|const
name|void
modifier|*
name|src
parameter_list|,
name|int
name|len
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|s
init|=
name|src
decl_stmt|;
name|char
modifier|*
name|d
init|=
name|dst
decl_stmt|;
while|while
condition|(
name|len
operator|--
condition|)
operator|*
name|d
operator|++
operator|=
operator|*
name|s
operator|++
expr_stmt|;
block|}
end_function

begin_function
specifier|static
specifier|inline
name|int
name|strcmp
parameter_list|(
specifier|const
name|char
modifier|*
name|s1
parameter_list|,
specifier|const
name|char
modifier|*
name|s2
parameter_list|)
block|{
for|for
control|(
init|;
operator|*
name|s1
operator|==
operator|*
name|s2
operator|&&
operator|*
name|s1
condition|;
name|s1
operator|++
operator|,
name|s2
operator|++
control|)
empty_stmt|;
return|return
operator|(
name|unsigned
name|char
operator|)
operator|*
name|s1
operator|-
operator|(
name|unsigned
name|char
operator|)
operator|*
name|s2
return|;
block|}
end_function

begin_include
include|#
directive|include
file|"ufsread.c"
end_include

begin_function
specifier|static
specifier|inline
name|int
name|xfsread
parameter_list|(
name|ino_t
name|inode
parameter_list|,
name|void
modifier|*
name|buf
parameter_list|,
name|size_t
name|nbyte
parameter_list|)
block|{
if|if
condition|(
operator|(
name|size_t
operator|)
name|fsread
argument_list|(
name|inode
argument_list|,
name|buf
argument_list|,
name|nbyte
argument_list|)
operator|!=
name|nbyte
condition|)
block|{
name|printf
argument_list|(
literal|"Invalid %s\n"
argument_list|,
literal|"format"
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_function
specifier|static
specifier|inline
name|uint32_t
name|memsize
parameter_list|(
name|void
parameter_list|)
block|{
name|v86
operator|.
name|addr
operator|=
name|MEM_EXT
expr_stmt|;
name|v86
operator|.
name|eax
operator|=
literal|0x8800
expr_stmt|;
name|v86int
argument_list|()
expr_stmt|;
return|return
name|v86
operator|.
name|eax
return|;
block|}
end_function

begin_function
specifier|static
specifier|inline
name|void
name|getstr
parameter_list|(
name|void
parameter_list|)
block|{
name|char
modifier|*
name|s
decl_stmt|;
name|int
name|c
decl_stmt|;
name|s
operator|=
name|cmd
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
switch|switch
condition|(
name|c
operator|=
name|xgetc
argument_list|(
literal|0
argument_list|)
condition|)
block|{
case|case
literal|0
case|:
break|break;
case|case
literal|'\177'
case|:
case|case
literal|'\b'
case|:
if|if
condition|(
name|s
operator|>
name|cmd
condition|)
block|{
name|s
operator|--
expr_stmt|;
name|printf
argument_list|(
literal|"\b \b"
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
literal|'\n'
case|:
case|case
literal|'\r'
case|:
operator|*
name|s
operator|=
literal|0
expr_stmt|;
return|return;
default|default:
if|if
condition|(
name|s
operator|-
name|cmd
operator|<
sizeof|sizeof
argument_list|(
name|cmd
argument_list|)
operator|-
literal|1
condition|)
operator|*
name|s
operator|++
operator|=
name|c
expr_stmt|;
name|putchar
argument_list|(
name|c
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_function

begin_function
specifier|static
specifier|inline
name|void
name|putc
parameter_list|(
name|int
name|c
parameter_list|)
block|{
name|v86
operator|.
name|addr
operator|=
literal|0x10
expr_stmt|;
name|v86
operator|.
name|eax
operator|=
literal|0xe00
operator||
operator|(
name|c
operator|&
literal|0xff
operator|)
expr_stmt|;
name|v86
operator|.
name|ebx
operator|=
literal|0x7
expr_stmt|;
name|v86int
argument_list|()
expr_stmt|;
block|}
end_function

begin_function
name|int
name|main
parameter_list|(
name|void
parameter_list|)
block|{
name|int
name|autoboot
decl_stmt|;
name|ino_t
name|ino
decl_stmt|;
name|dmadat
operator|=
operator|(
name|void
operator|*
operator|)
operator|(
name|roundup2
argument_list|(
name|__base
operator|+
operator|(
name|int32_t
operator|)
operator|&
name|_end
argument_list|,
literal|0x10000
argument_list|)
operator|-
name|__base
operator|)
expr_stmt|;
name|v86
operator|.
name|ctl
operator|=
name|V86_FLAGS
expr_stmt|;
name|v86
operator|.
name|efl
operator|=
name|PSL_RESERVED_DEFAULT
operator||
name|PSL_I
expr_stmt|;
name|dsk
operator|.
name|drive
operator|=
operator|*
operator|(
name|uint8_t
operator|*
operator|)
name|PTOV
argument_list|(
name|ARGS
argument_list|)
expr_stmt|;
name|dsk
operator|.
name|type
operator|=
name|dsk
operator|.
name|drive
operator|&
name|DRV_HARD
condition|?
name|TYPE_AD
else|:
name|TYPE_FD
expr_stmt|;
name|dsk
operator|.
name|unit
operator|=
name|dsk
operator|.
name|drive
operator|&
name|DRV_MASK
expr_stmt|;
name|dsk
operator|.
name|part
operator|=
operator|-
literal|1
expr_stmt|;
name|bootinfo
operator|.
name|bi_version
operator|=
name|BOOTINFO_VERSION
expr_stmt|;
name|bootinfo
operator|.
name|bi_size
operator|=
sizeof|sizeof
argument_list|(
name|bootinfo
argument_list|)
expr_stmt|;
name|bootinfo
operator|.
name|bi_basemem
operator|=
literal|0
expr_stmt|;
comment|/* XXX will be filled by loader or kernel */
name|bootinfo
operator|.
name|bi_extmem
operator|=
name|memsize
argument_list|()
expr_stmt|;
name|bootinfo
operator|.
name|bi_memsizes_valid
operator|++
expr_stmt|;
comment|/* Process configuration file */
name|autoboot
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|(
name|ino
operator|=
name|lookup
argument_list|(
name|PATH_CONFIG
argument_list|)
operator|)
condition|)
name|fsread
argument_list|(
name|ino
argument_list|,
name|cmd
argument_list|,
sizeof|sizeof
argument_list|(
name|cmd
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|cmd
condition|)
block|{
name|memcpy
argument_list|(
name|cmddup
argument_list|,
name|cmd
argument_list|,
sizeof|sizeof
argument_list|(
name|cmd
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|parse
argument_list|()
condition|)
name|autoboot
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
name|OPT_CHECK
argument_list|(
name|RBX_QUIET
argument_list|)
condition|)
name|printf
argument_list|(
literal|"%s: %s"
argument_list|,
name|PATH_CONFIG
argument_list|,
name|cmddup
argument_list|)
expr_stmt|;
comment|/* Do not process this command twice */
operator|*
name|cmd
operator|=
literal|0
expr_stmt|;
block|}
comment|/*      * Try to exec stage 3 boot loader. If interrupted by a keypress,      * or in case of failure, try to load a kernel directly instead.      */
if|if
condition|(
name|autoboot
operator|&&
operator|!
operator|*
name|kname
condition|)
block|{
name|memcpy
argument_list|(
name|kname
argument_list|,
name|PATH_BOOT3
argument_list|,
sizeof|sizeof
argument_list|(
name|PATH_BOOT3
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|keyhit
argument_list|(
literal|3
operator|*
name|SECOND
argument_list|)
condition|)
block|{
name|load
argument_list|()
expr_stmt|;
name|memcpy
argument_list|(
name|kname
argument_list|,
name|PATH_KERNEL
argument_list|,
sizeof|sizeof
argument_list|(
name|PATH_KERNEL
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* Present the user with the boot2 prompt. */
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|!
name|autoboot
operator|||
operator|!
name|OPT_CHECK
argument_list|(
name|RBX_QUIET
argument_list|)
condition|)
name|printf
argument_list|(
literal|"\nFreeBSD/i386 boot\n"
literal|"Default: %u:%s(%up%u)%s\n"
literal|"boot: "
argument_list|,
name|dsk
operator|.
name|drive
operator|&
name|DRV_MASK
argument_list|,
name|dev_nm
index|[
name|dsk
operator|.
name|type
index|]
argument_list|,
name|dsk
operator|.
name|unit
argument_list|,
name|dsk
operator|.
name|part
argument_list|,
name|kname
argument_list|)
expr_stmt|;
if|if
condition|(
name|ioctrl
operator|&
name|IO_SERIAL
condition|)
name|sio_flush
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
name|autoboot
operator|||
name|keyhit
argument_list|(
literal|5
operator|*
name|SECOND
argument_list|)
condition|)
name|getstr
argument_list|()
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|autoboot
operator|||
operator|!
name|OPT_CHECK
argument_list|(
name|RBX_QUIET
argument_list|)
condition|)
name|putchar
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
name|autoboot
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|parse
argument_list|()
condition|)
name|putchar
argument_list|(
literal|'\a'
argument_list|)
expr_stmt|;
else|else
name|load
argument_list|()
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/* XXX - Needed for btxld to link the boot2 binary; do not remove. */
end_comment

begin_function
name|void
name|exit
parameter_list|(
name|int
name|x
parameter_list|)
block|{ }
end_function

begin_function
specifier|static
name|void
name|load
parameter_list|(
name|void
parameter_list|)
block|{
union|union
block|{
name|struct
name|exec
name|ex
decl_stmt|;
name|Elf32_Ehdr
name|eh
decl_stmt|;
block|}
name|hdr
union|;
specifier|static
name|Elf32_Phdr
name|ep
index|[
literal|2
index|]
decl_stmt|;
specifier|static
name|Elf32_Shdr
name|es
index|[
literal|2
index|]
decl_stmt|;
name|caddr_t
name|p
decl_stmt|;
name|ino_t
name|ino
decl_stmt|;
name|uint32_t
name|addr
decl_stmt|,
name|x
decl_stmt|;
name|int
name|fmt
decl_stmt|,
name|i
decl_stmt|,
name|j
decl_stmt|;
if|if
condition|(
operator|!
operator|(
name|ino
operator|=
name|lookup
argument_list|(
name|kname
argument_list|)
operator|)
condition|)
block|{
if|if
condition|(
operator|!
name|ls
condition|)
name|printf
argument_list|(
literal|"No %s\n"
argument_list|,
name|kname
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
operator|&
name|hdr
argument_list|,
sizeof|sizeof
argument_list|(
name|hdr
argument_list|)
argument_list|)
condition|)
return|return;
if|if
condition|(
name|N_GETMAGIC
argument_list|(
name|hdr
operator|.
name|ex
argument_list|)
operator|==
name|ZMAGIC
condition|)
name|fmt
operator|=
literal|0
expr_stmt|;
elseif|else
if|if
condition|(
name|IS_ELF
argument_list|(
name|hdr
operator|.
name|eh
argument_list|)
condition|)
name|fmt
operator|=
literal|1
expr_stmt|;
else|else
block|{
name|printf
argument_list|(
literal|"Invalid %s\n"
argument_list|,
literal|"format"
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|fmt
operator|==
literal|0
condition|)
block|{
name|addr
operator|=
name|hdr
operator|.
name|ex
operator|.
name|a_entry
operator|&
literal|0xffffff
expr_stmt|;
name|p
operator|=
name|PTOV
argument_list|(
name|addr
argument_list|)
expr_stmt|;
name|fs_off
operator|=
name|PAGE_SIZE
expr_stmt|;
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|p
argument_list|,
name|hdr
operator|.
name|ex
operator|.
name|a_text
argument_list|)
condition|)
return|return;
name|p
operator|+=
name|roundup2
argument_list|(
name|hdr
operator|.
name|ex
operator|.
name|a_text
argument_list|,
name|PAGE_SIZE
argument_list|)
expr_stmt|;
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|p
argument_list|,
name|hdr
operator|.
name|ex
operator|.
name|a_data
argument_list|)
condition|)
return|return;
name|p
operator|+=
name|hdr
operator|.
name|ex
operator|.
name|a_data
operator|+
name|roundup2
argument_list|(
name|hdr
operator|.
name|ex
operator|.
name|a_bss
argument_list|,
name|PAGE_SIZE
argument_list|)
expr_stmt|;
name|bootinfo
operator|.
name|bi_symtab
operator|=
name|VTOP
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|memcpy
argument_list|(
name|p
argument_list|,
operator|&
name|hdr
operator|.
name|ex
operator|.
name|a_syms
argument_list|,
sizeof|sizeof
argument_list|(
name|hdr
operator|.
name|ex
operator|.
name|a_syms
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|hdr
operator|.
name|ex
operator|.
name|a_syms
argument_list|)
expr_stmt|;
if|if
condition|(
name|hdr
operator|.
name|ex
operator|.
name|a_syms
condition|)
block|{
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|p
argument_list|,
name|hdr
operator|.
name|ex
operator|.
name|a_syms
argument_list|)
condition|)
return|return;
name|p
operator|+=
name|hdr
operator|.
name|ex
operator|.
name|a_syms
expr_stmt|;
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|p
argument_list|,
sizeof|sizeof
argument_list|(
name|int
argument_list|)
argument_list|)
condition|)
return|return;
name|x
operator|=
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|p
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
expr_stmt|;
name|x
operator|-=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
expr_stmt|;
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|p
argument_list|,
name|x
argument_list|)
condition|)
return|return;
name|p
operator|+=
name|x
expr_stmt|;
block|}
block|}
else|else
block|{
name|fs_off
operator|=
name|hdr
operator|.
name|eh
operator|.
name|e_phoff
expr_stmt|;
for|for
control|(
name|j
operator|=
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|hdr
operator|.
name|eh
operator|.
name|e_phnum
operator|&&
name|j
operator|<
literal|2
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|ep
operator|+
name|j
argument_list|,
sizeof|sizeof
argument_list|(
name|ep
index|[
literal|0
index|]
argument_list|)
argument_list|)
condition|)
return|return;
if|if
condition|(
name|ep
index|[
name|j
index|]
operator|.
name|p_type
operator|==
name|PT_LOAD
condition|)
name|j
operator|++
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|2
condition|;
name|i
operator|++
control|)
block|{
name|p
operator|=
name|PTOV
argument_list|(
name|ep
index|[
name|i
index|]
operator|.
name|p_paddr
operator|&
literal|0xffffff
argument_list|)
expr_stmt|;
name|fs_off
operator|=
name|ep
index|[
name|i
index|]
operator|.
name|p_offset
expr_stmt|;
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|p
argument_list|,
name|ep
index|[
name|i
index|]
operator|.
name|p_filesz
argument_list|)
condition|)
return|return;
block|}
name|p
operator|+=
name|roundup2
argument_list|(
name|ep
index|[
literal|1
index|]
operator|.
name|p_memsz
argument_list|,
name|PAGE_SIZE
argument_list|)
expr_stmt|;
name|bootinfo
operator|.
name|bi_symtab
operator|=
name|VTOP
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|hdr
operator|.
name|eh
operator|.
name|e_shnum
operator|==
name|hdr
operator|.
name|eh
operator|.
name|e_shstrndx
operator|+
literal|3
condition|)
block|{
name|fs_off
operator|=
name|hdr
operator|.
name|eh
operator|.
name|e_shoff
operator|+
sizeof|sizeof
argument_list|(
name|es
index|[
literal|0
index|]
argument_list|)
operator|*
operator|(
name|hdr
operator|.
name|eh
operator|.
name|e_shstrndx
operator|+
literal|1
operator|)
expr_stmt|;
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
operator|&
name|es
argument_list|,
sizeof|sizeof
argument_list|(
name|es
argument_list|)
argument_list|)
condition|)
return|return;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|2
condition|;
name|i
operator|++
control|)
block|{
name|memcpy
argument_list|(
name|p
argument_list|,
operator|&
name|es
index|[
name|i
index|]
operator|.
name|sh_size
argument_list|,
sizeof|sizeof
argument_list|(
name|es
index|[
name|i
index|]
operator|.
name|sh_size
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|es
index|[
name|i
index|]
operator|.
name|sh_size
argument_list|)
expr_stmt|;
name|fs_off
operator|=
name|es
index|[
name|i
index|]
operator|.
name|sh_offset
expr_stmt|;
if|if
condition|(
name|xfsread
argument_list|(
name|ino
argument_list|,
name|p
argument_list|,
name|es
index|[
name|i
index|]
operator|.
name|sh_size
argument_list|)
condition|)
return|return;
name|p
operator|+=
name|es
index|[
name|i
index|]
operator|.
name|sh_size
expr_stmt|;
block|}
block|}
name|addr
operator|=
name|hdr
operator|.
name|eh
operator|.
name|e_entry
operator|&
literal|0xffffff
expr_stmt|;
block|}
name|bootinfo
operator|.
name|bi_esymtab
operator|=
name|VTOP
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|bootinfo
operator|.
name|bi_kernelname
operator|=
name|VTOP
argument_list|(
name|kname
argument_list|)
expr_stmt|;
name|bootinfo
operator|.
name|bi_bios_dev
operator|=
name|dsk
operator|.
name|drive
expr_stmt|;
name|__exec
argument_list|(
operator|(
name|caddr_t
operator|)
name|addr
argument_list|,
name|RB_BOOTINFO
operator||
operator|(
name|opts
operator|&
name|RBX_MASK
operator|)
argument_list|,
name|MAKEBOOTDEV
argument_list|(
name|dev_maj
index|[
name|dsk
operator|.
name|type
index|]
argument_list|,
name|dsk
operator|.
name|part
operator|+
literal|1
argument_list|,
name|dsk
operator|.
name|unit
argument_list|,
literal|0xff
argument_list|)
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|VTOP
argument_list|(
operator|&
name|bootinfo
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|parse
parameter_list|(
name|void
parameter_list|)
block|{
name|char
modifier|*
name|arg
init|=
name|cmd
decl_stmt|;
name|char
modifier|*
name|ep
decl_stmt|,
modifier|*
name|p
decl_stmt|,
modifier|*
name|q
decl_stmt|;
specifier|const
name|char
modifier|*
name|cp
decl_stmt|;
name|unsigned
name|int
name|drv
decl_stmt|;
name|int
name|c
decl_stmt|,
name|i
decl_stmt|,
name|j
decl_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
operator|*
name|arg
operator|++
operator|)
condition|)
block|{
if|if
condition|(
name|c
operator|==
literal|' '
operator|||
name|c
operator|==
literal|'\t'
operator|||
name|c
operator|==
literal|'\n'
condition|)
continue|continue;
for|for
control|(
name|p
operator|=
name|arg
init|;
operator|*
name|p
operator|&&
operator|*
name|p
operator|!=
literal|'\n'
operator|&&
operator|*
name|p
operator|!=
literal|' '
operator|&&
operator|*
name|p
operator|!=
literal|'\t'
condition|;
name|p
operator|++
control|)
empty_stmt|;
name|ep
operator|=
name|p
expr_stmt|;
if|if
condition|(
operator|*
name|p
condition|)
operator|*
name|p
operator|++
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|c
operator|==
literal|'-'
condition|)
block|{
while|while
condition|(
operator|(
name|c
operator|=
operator|*
name|arg
operator|++
operator|)
condition|)
block|{
if|if
condition|(
name|c
operator|==
literal|'P'
condition|)
block|{
if|if
condition|(
operator|*
operator|(
name|uint8_t
operator|*
operator|)
name|PTOV
argument_list|(
literal|0x496
argument_list|)
operator|&
literal|0x10
condition|)
block|{
name|cp
operator|=
literal|"yes"
expr_stmt|;
block|}
else|else
block|{
name|opts
operator||=
name|OPT_SET
argument_list|(
name|RBX_DUAL
argument_list|)
operator||
name|OPT_SET
argument_list|(
name|RBX_SERIAL
argument_list|)
expr_stmt|;
name|cp
operator|=
literal|"no"
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"Keyboard: %s\n"
argument_list|,
name|cp
argument_list|)
expr_stmt|;
continue|continue;
block|}
elseif|else
if|if
condition|(
name|c
operator|==
literal|'S'
condition|)
block|{
name|j
operator|=
literal|0
expr_stmt|;
while|while
condition|(
call|(
name|unsigned
name|int
call|)
argument_list|(
name|i
operator|=
operator|*
name|arg
operator|++
operator|-
literal|'0'
argument_list|)
operator|<=
literal|9
condition|)
name|j
operator|=
name|j
operator|*
literal|10
operator|+
name|i
expr_stmt|;
if|if
condition|(
name|j
operator|>
literal|0
operator|&&
name|i
operator|==
operator|-
literal|'0'
condition|)
block|{
name|comspeed
operator|=
name|j
expr_stmt|;
break|break;
block|}
comment|/* Fall through to error below ('S' not in optstr[]). */
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|c
operator|!=
name|optstr
index|[
name|i
index|]
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|i
operator|==
name|NOPT
operator|-
literal|1
condition|)
return|return
operator|-
literal|1
return|;
name|opts
operator|^=
name|OPT_SET
argument_list|(
name|flags
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
name|ioctrl
operator|=
name|OPT_CHECK
argument_list|(
name|RBX_DUAL
argument_list|)
condition|?
operator|(
name|IO_SERIAL
operator||
name|IO_KEYBOARD
operator|)
else|:
name|OPT_CHECK
argument_list|(
name|RBX_SERIAL
argument_list|)
condition|?
name|IO_SERIAL
else|:
name|IO_KEYBOARD
expr_stmt|;
if|if
condition|(
name|ioctrl
operator|&
name|IO_SERIAL
condition|)
name|sio_init
argument_list|(
literal|115200
operator|/
name|comspeed
argument_list|)
expr_stmt|;
block|}
else|else
block|{
for|for
control|(
name|q
operator|=
name|arg
operator|--
init|;
operator|*
name|q
operator|&&
operator|*
name|q
operator|!=
literal|'('
condition|;
name|q
operator|++
control|)
empty_stmt|;
if|if
condition|(
operator|*
name|q
condition|)
block|{
name|drv
operator|=
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|arg
index|[
literal|1
index|]
operator|==
literal|':'
condition|)
block|{
name|drv
operator|=
operator|*
name|arg
operator|-
literal|'0'
expr_stmt|;
if|if
condition|(
name|drv
operator|>
literal|9
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
name|arg
operator|+=
literal|2
expr_stmt|;
block|}
if|if
condition|(
name|q
operator|-
name|arg
operator|!=
literal|2
condition|)
return|return
operator|-
literal|1
return|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|arg
index|[
literal|0
index|]
operator|!=
name|dev_nm
index|[
name|i
index|]
index|[
literal|0
index|]
operator|||
name|arg
index|[
literal|1
index|]
operator|!=
name|dev_nm
index|[
name|i
index|]
index|[
literal|1
index|]
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|i
operator|==
name|NDEV
operator|-
literal|1
condition|)
return|return
operator|-
literal|1
return|;
name|dsk
operator|.
name|type
operator|=
name|i
expr_stmt|;
name|arg
operator|+=
literal|3
expr_stmt|;
name|dsk
operator|.
name|unit
operator|=
operator|*
name|arg
operator|-
literal|'0'
expr_stmt|;
if|if
condition|(
name|arg
index|[
literal|1
index|]
operator|!=
literal|'p'
operator|||
name|dsk
operator|.
name|unit
operator|>
literal|9
condition|)
return|return
operator|-
literal|1
return|;
name|arg
operator|+=
literal|2
expr_stmt|;
name|dsk
operator|.
name|part
operator|=
operator|*
name|arg
operator|-
literal|'0'
expr_stmt|;
if|if
condition|(
name|dsk
operator|.
name|part
operator|<
literal|1
operator|||
name|dsk
operator|.
name|part
operator|>
literal|9
condition|)
return|return
operator|-
literal|1
return|;
name|arg
operator|++
expr_stmt|;
if|if
condition|(
name|arg
index|[
literal|0
index|]
operator|!=
literal|')'
condition|)
return|return
operator|-
literal|1
return|;
name|arg
operator|++
expr_stmt|;
if|if
condition|(
name|drv
operator|==
operator|-
literal|1
condition|)
name|drv
operator|=
name|dsk
operator|.
name|unit
expr_stmt|;
name|dsk
operator|.
name|drive
operator|=
operator|(
name|dsk
operator|.
name|type
operator|<=
name|TYPE_MAXHARD
condition|?
name|DRV_HARD
else|:
literal|0
operator|)
operator|+
name|drv
expr_stmt|;
name|dsk_meta
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|i
operator|=
name|ep
operator|-
name|arg
operator|)
condition|)
block|{
if|if
condition|(
operator|(
name|size_t
operator|)
name|i
operator|>=
sizeof|sizeof
argument_list|(
name|kname
argument_list|)
condition|)
return|return
operator|-
literal|1
return|;
name|memcpy
argument_list|(
name|kname
argument_list|,
name|arg
argument_list|,
name|i
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
name|arg
operator|=
name|p
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|dskread
parameter_list|(
name|void
modifier|*
name|buf
parameter_list|,
name|daddr_t
name|lba
parameter_list|,
name|unsigned
name|nblk
parameter_list|)
block|{
name|struct
name|gpt_hdr
name|hdr
decl_stmt|;
name|struct
name|gpt_ent
modifier|*
name|ent
decl_stmt|;
name|char
modifier|*
name|sec
decl_stmt|;
name|daddr_t
name|slba
decl_stmt|,
name|elba
decl_stmt|;
name|int
name|part
decl_stmt|,
name|entries_per_sec
decl_stmt|;
if|if
condition|(
operator|!
name|dsk_meta
condition|)
block|{
comment|/* Read and verify GPT. */
name|sec
operator|=
name|dmadat
operator|->
name|secbuf
expr_stmt|;
name|dsk
operator|.
name|start
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|drvread
argument_list|(
name|sec
argument_list|,
literal|1
argument_list|,
literal|1
argument_list|)
condition|)
return|return
operator|-
literal|1
return|;
name|memcpy
argument_list|(
operator|&
name|hdr
argument_list|,
name|sec
argument_list|,
sizeof|sizeof
argument_list|(
name|hdr
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|bcmp
argument_list|(
name|hdr
operator|.
name|hdr_sig
argument_list|,
name|GPT_HDR_SIG
argument_list|,
sizeof|sizeof
argument_list|(
name|hdr
operator|.
name|hdr_sig
argument_list|)
argument_list|)
operator|!=
literal|0
operator|||
name|hdr
operator|.
name|hdr_lba_self
operator|!=
literal|1
operator|||
name|hdr
operator|.
name|hdr_revision
operator|<
literal|0x00010000
operator|||
name|hdr
operator|.
name|hdr_entsz
operator|<
sizeof|sizeof
argument_list|(
operator|*
name|ent
argument_list|)
operator|||
name|DEV_BSIZE
operator|%
name|hdr
operator|.
name|hdr_entsz
operator|!=
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"Invalid GPT header\n"
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
comment|/* XXX: CRC check? */
comment|/* 	 * If the partition isn't specified, then search for the first UFS 	 * partition and hope it is /.  Perhaps we should be using an OS 	 * flag in the GPT entry to mark / partitions. 	 * 	 * If the partition is specified, then figure out the LBA for the 	 * sector containing that partition index and load it. 	 */
name|entries_per_sec
operator|=
name|DEV_BSIZE
operator|/
name|hdr
operator|.
name|hdr_entsz
expr_stmt|;
if|if
condition|(
name|dsk
operator|.
name|part
operator|==
operator|-
literal|1
condition|)
block|{
name|slba
operator|=
name|hdr
operator|.
name|hdr_lba_table
expr_stmt|;
name|elba
operator|=
name|slba
operator|+
name|hdr
operator|.
name|hdr_entries
operator|/
name|entries_per_sec
expr_stmt|;
while|while
condition|(
name|slba
operator|<
name|elba
operator|&&
name|dsk
operator|.
name|part
operator|==
operator|-
literal|1
condition|)
block|{
if|if
condition|(
name|drvread
argument_list|(
name|sec
argument_list|,
name|slba
argument_list|,
literal|1
argument_list|)
condition|)
return|return
operator|-
literal|1
return|;
for|for
control|(
name|part
operator|=
literal|0
init|;
name|part
operator|<
name|entries_per_sec
condition|;
name|part
operator|++
control|)
block|{
name|ent
operator|=
operator|(
expr|struct
name|gpt_ent
operator|*
operator|)
operator|(
name|sec
operator|+
name|part
operator|*
name|hdr
operator|.
name|hdr_entsz
operator|)
expr_stmt|;
if|if
condition|(
name|bcmp
argument_list|(
operator|&
name|ent
operator|->
name|ent_type
argument_list|,
operator|&
name|freebsd_ufs_uuid
argument_list|,
sizeof|sizeof
argument_list|(
name|uuid_t
argument_list|)
argument_list|)
operator|==
literal|0
condition|)
block|{
name|dsk
operator|.
name|part
operator|=
operator|(
name|slba
operator|-
name|hdr
operator|.
name|hdr_lba_table
operator|)
operator|*
name|entries_per_sec
operator|+
name|part
operator|+
literal|1
expr_stmt|;
name|dsk
operator|.
name|start
operator|=
name|ent
operator|->
name|ent_lba_start
expr_stmt|;
break|break;
block|}
block|}
name|slba
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|dsk
operator|.
name|part
operator|==
operator|-
literal|1
condition|)
block|{
name|printf
argument_list|(
literal|"No UFS partition was found\n"
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
block|}
else|else
block|{
if|if
condition|(
name|dsk
operator|.
name|part
operator|>
name|hdr
operator|.
name|hdr_entries
condition|)
block|{
name|printf
argument_list|(
literal|"Invalid partition index\n"
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|slba
operator|=
name|hdr
operator|.
name|hdr_lba_table
operator|+
operator|(
name|dsk
operator|.
name|part
operator|-
literal|1
operator|)
operator|/
name|entries_per_sec
expr_stmt|;
if|if
condition|(
name|drvread
argument_list|(
name|sec
argument_list|,
name|slba
argument_list|,
literal|1
argument_list|)
condition|)
return|return
operator|-
literal|1
return|;
name|part
operator|=
operator|(
name|dsk
operator|.
name|part
operator|-
literal|1
operator|)
operator|%
name|entries_per_sec
expr_stmt|;
name|ent
operator|=
operator|(
expr|struct
name|gpt_ent
operator|*
operator|)
operator|(
name|sec
operator|+
name|part
operator|*
name|hdr
operator|.
name|hdr_entsz
operator|)
expr_stmt|;
if|if
condition|(
name|bcmp
argument_list|(
operator|&
name|ent
operator|->
name|ent_type
argument_list|,
operator|&
name|freebsd_ufs_uuid
argument_list|,
sizeof|sizeof
argument_list|(
name|uuid_t
argument_list|)
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"Specified partition is not UFS\n"
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|dsk
operator|.
name|start
operator|=
name|ent
operator|->
name|ent_lba_start
expr_stmt|;
block|}
comment|/* 	 * XXX: No way to detect SCSI vs. ATA currently. 	 */
if|#
directive|if
literal|0
block|if (!dsk.init) { 	    if (d->d_type == DTYPE_SCSI) 		dsk.type = TYPE_DA; 	    dsk.init++; 	}
endif|#
directive|endif
block|}
return|return
name|drvread
argument_list|(
name|buf
argument_list|,
name|dsk
operator|.
name|start
operator|+
name|lba
argument_list|,
name|nblk
argument_list|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|printf
parameter_list|(
specifier|const
name|char
modifier|*
name|fmt
parameter_list|,
modifier|...
parameter_list|)
block|{
name|va_list
name|ap
decl_stmt|;
name|char
name|buf
index|[
literal|10
index|]
decl_stmt|;
name|char
modifier|*
name|s
decl_stmt|;
name|unsigned
name|u
decl_stmt|;
name|int
name|c
decl_stmt|;
name|va_start
argument_list|(
name|ap
argument_list|,
name|fmt
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
operator|*
name|fmt
operator|++
operator|)
condition|)
block|{
if|if
condition|(
name|c
operator|==
literal|'%'
condition|)
block|{
name|c
operator|=
operator|*
name|fmt
operator|++
expr_stmt|;
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'c'
case|:
name|putchar
argument_list|(
name|va_arg
argument_list|(
name|ap
argument_list|,
name|int
argument_list|)
argument_list|)
expr_stmt|;
continue|continue;
case|case
literal|'s'
case|:
for|for
control|(
name|s
operator|=
name|va_arg
argument_list|(
name|ap
argument_list|,
name|char
operator|*
argument_list|)
init|;
operator|*
name|s
condition|;
name|s
operator|++
control|)
name|putchar
argument_list|(
operator|*
name|s
argument_list|)
expr_stmt|;
continue|continue;
case|case
literal|'u'
case|:
name|u
operator|=
name|va_arg
argument_list|(
name|ap
argument_list|,
name|unsigned
argument_list|)
expr_stmt|;
name|s
operator|=
name|buf
expr_stmt|;
do|do
operator|*
name|s
operator|++
operator|=
literal|'0'
operator|+
name|u
operator|%
literal|10U
expr_stmt|;
do|while
condition|(
name|u
operator|/=
literal|10U
condition|)
do|;
while|while
condition|(
operator|--
name|s
operator|>=
name|buf
condition|)
name|putchar
argument_list|(
operator|*
name|s
argument_list|)
expr_stmt|;
continue|continue;
block|}
block|}
name|putchar
argument_list|(
name|c
argument_list|)
expr_stmt|;
block|}
name|va_end
argument_list|(
name|ap
argument_list|)
expr_stmt|;
return|return;
block|}
end_function

begin_function
specifier|static
name|void
name|putchar
parameter_list|(
name|int
name|c
parameter_list|)
block|{
if|if
condition|(
name|c
operator|==
literal|'\n'
condition|)
name|xputc
argument_list|(
literal|'\r'
argument_list|)
expr_stmt|;
name|xputc
argument_list|(
name|c
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|bcmp
parameter_list|(
specifier|const
name|void
modifier|*
name|b1
parameter_list|,
specifier|const
name|void
modifier|*
name|b2
parameter_list|,
name|size_t
name|length
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|p1
init|=
name|b1
decl_stmt|,
modifier|*
name|p2
init|=
name|b2
decl_stmt|;
if|if
condition|(
name|length
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
do|do
block|{
if|if
condition|(
operator|*
name|p1
operator|++
operator|!=
operator|*
name|p2
operator|++
condition|)
break|break;
block|}
do|while
condition|(
operator|--
name|length
condition|)
do|;
return|return
operator|(
name|length
operator|)
return|;
block|}
end_function

begin_struct
specifier|static
struct|struct
block|{
name|uint16_t
name|len
decl_stmt|;
name|uint16_t
name|count
decl_stmt|;
name|uint16_t
name|off
decl_stmt|;
name|uint16_t
name|seg
decl_stmt|;
name|uint64_t
name|lba
decl_stmt|;
block|}
name|packet
struct|;
end_struct

begin_function
specifier|static
name|int
name|drvread
parameter_list|(
name|void
modifier|*
name|buf
parameter_list|,
name|daddr_t
name|lba
parameter_list|,
name|unsigned
name|nblk
parameter_list|)
block|{
specifier|static
name|unsigned
name|c
init|=
literal|0x2d5c7c2f
decl_stmt|;
if|if
condition|(
operator|!
name|OPT_CHECK
argument_list|(
name|RBX_QUIET
argument_list|)
condition|)
name|printf
argument_list|(
literal|"%c\b"
argument_list|,
name|c
operator|=
name|c
operator|<<
literal|8
operator||
name|c
operator|>>
literal|24
argument_list|)
expr_stmt|;
name|packet
operator|.
name|len
operator|=
literal|0x10
expr_stmt|;
name|packet
operator|.
name|count
operator|=
name|nblk
expr_stmt|;
name|packet
operator|.
name|off
operator|=
name|VTOPOFF
argument_list|(
name|buf
argument_list|)
expr_stmt|;
name|packet
operator|.
name|seg
operator|=
name|VTOPSEG
argument_list|(
name|buf
argument_list|)
expr_stmt|;
name|packet
operator|.
name|lba
operator|=
name|lba
expr_stmt|;
name|v86
operator|.
name|ctl
operator|=
name|V86_FLAGS
expr_stmt|;
name|v86
operator|.
name|addr
operator|=
literal|0x13
expr_stmt|;
name|v86
operator|.
name|eax
operator|=
literal|0x4200
expr_stmt|;
name|v86
operator|.
name|edx
operator|=
name|dsk
operator|.
name|drive
expr_stmt|;
name|v86
operator|.
name|ds
operator|=
name|VTOPSEG
argument_list|(
operator|&
name|packet
argument_list|)
expr_stmt|;
name|v86
operator|.
name|esi
operator|=
name|VTOPOFF
argument_list|(
operator|&
name|packet
argument_list|)
expr_stmt|;
name|v86int
argument_list|()
expr_stmt|;
if|if
condition|(
name|V86_CY
argument_list|(
name|v86
operator|.
name|efl
argument_list|)
condition|)
block|{
name|printf
argument_list|(
literal|"error %u lba %u\n"
argument_list|,
name|v86
operator|.
name|eax
operator|>>
literal|8
operator|&
literal|0xff
argument_list|,
name|lba
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|keyhit
parameter_list|(
name|unsigned
name|ticks
parameter_list|)
block|{
name|uint32_t
name|t0
decl_stmt|,
name|t1
decl_stmt|;
if|if
condition|(
name|OPT_CHECK
argument_list|(
name|RBX_NOINTR
argument_list|)
condition|)
return|return
literal|0
return|;
name|t0
operator|=
literal|0
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
name|xgetc
argument_list|(
literal|1
argument_list|)
condition|)
return|return
literal|1
return|;
name|t1
operator|=
operator|*
operator|(
name|uint32_t
operator|*
operator|)
name|PTOV
argument_list|(
literal|0x46c
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|t0
condition|)
name|t0
operator|=
name|t1
expr_stmt|;
if|if
condition|(
name|t1
operator|<
name|t0
operator|||
name|t1
operator|>=
name|t0
operator|+
name|ticks
condition|)
return|return
literal|0
return|;
block|}
block|}
end_function

begin_function
specifier|static
name|int
name|xputc
parameter_list|(
name|int
name|c
parameter_list|)
block|{
if|if
condition|(
name|ioctrl
operator|&
name|IO_KEYBOARD
condition|)
name|putc
argument_list|(
name|c
argument_list|)
expr_stmt|;
if|if
condition|(
name|ioctrl
operator|&
name|IO_SERIAL
condition|)
name|sio_putc
argument_list|(
name|c
argument_list|)
expr_stmt|;
return|return
name|c
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|xgetc
parameter_list|(
name|int
name|fn
parameter_list|)
block|{
if|if
condition|(
name|OPT_CHECK
argument_list|(
name|RBX_NOINTR
argument_list|)
condition|)
return|return
literal|0
return|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
name|ioctrl
operator|&
name|IO_KEYBOARD
operator|&&
name|getc
argument_list|(
literal|1
argument_list|)
condition|)
return|return
name|fn
condition|?
literal|1
else|:
name|getc
argument_list|(
literal|0
argument_list|)
return|;
if|if
condition|(
name|ioctrl
operator|&
name|IO_SERIAL
operator|&&
name|sio_ischar
argument_list|()
condition|)
return|return
name|fn
condition|?
literal|1
else|:
name|sio_getc
argument_list|()
return|;
if|if
condition|(
name|fn
condition|)
return|return
literal|0
return|;
block|}
block|}
end_function

begin_function
specifier|static
name|int
name|getc
parameter_list|(
name|int
name|fn
parameter_list|)
block|{
name|v86
operator|.
name|addr
operator|=
literal|0x16
expr_stmt|;
name|v86
operator|.
name|eax
operator|=
name|fn
operator|<<
literal|8
expr_stmt|;
name|v86int
argument_list|()
expr_stmt|;
return|return
name|fn
operator|==
literal|0
condition|?
name|v86
operator|.
name|eax
operator|&
literal|0xff
else|:
operator|!
name|V86_ZR
argument_list|(
name|v86
operator|.
name|efl
argument_list|)
return|;
block|}
end_function

end_unit

