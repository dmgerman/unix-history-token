...
begin_comment
comment|/*-  * APM (Advanced Power Management) BIOS Device Driver  *  * Copyright (c) 1994 UKAI, Fumitoshi.  * Copyright (c) 1994-1995 by HOSOKAWA, Tatsumi<hosokawa@jp.FreeBSD.org>  * Copyright (c) 1996 Nate Williams<nate@FreeBSD.org>  * Copyright (c) 1997 Poul-Henning Kamp<phk@FreeBSD.org>  *  * This software may be used, modified, copied, and distributed, in  * both source and binary form provided that the above copyright and  * these terms are retained. Under no circumstances is the author  * responsible for the proper functioning of this software, nor does  * the author assume any responsibility for damages incurred with its  * use.  *  * Sep, 1994	Implemented on FreeBSD 1.1.5.1R (Toshiba AVS001WD)  */
end_comment
...
name|TUNABLE_INT
argument_list|(
literal|"machdep.apm_swab_batt_minutes"
argument_list|,
operator|&
name|apm_swab_batt_minutes
argument_list|)
...
literal|0
argument_list|,
literal|"Byte swap battery time value."
argument_list|)
expr_stmt|;
end_expr_stmt
...
begin_comment
comment|/*  * APM thread loop.  *  * This routine wakes up from time to time to deal with delaying the  * suspend of the system, or other events.  */
end_comment
...
name|panic
argument_list|(
literal|"Cannot create apm worker thread"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|printf
argument_list|(
literal|"apm: Other PM system enabled.\n"
argument_list|)
expr_stmt|;
return|return
name|ENXIO
...
argument_list|)
expr_stmt|;
comment|/* Register APM again to pass the correct argument of pm_func. */
