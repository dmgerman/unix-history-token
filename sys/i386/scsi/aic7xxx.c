...
begin_comment
comment|/*  * Driver for the 27/284X series adaptec SCSI controllers   * Copyright (c) 1994 Justin T. Gibbs.    * All rights reserved.  *  * Portions of this driver are based on the FreeBSD 1742 Driver:   *  * Written by Julian Elischer (julian@tfs.com)  * for TRW Financial Systems for use under the MACH(2.5) operating system.  *  * TRW Financial Systems, in accordance with their agreement with Carnegie  * Mellon University, makes this software available to CMU to distribute  * or use in any manner that they see fit as long as this message is kept with  * the software. For this reason TFS also grants any other persons or  * organisations permission to use or modify this software.  *  * TFS supplies this software to be publicly redistributed  * on the understanding that TFS is not responsible for the correct  * functioning of this software in any circumstances.  *  * commenced: Sun Sep 27 18:14:01 PDT 1992  *  *      $Id: aic7770.c,v 1.7 1994/11/29 23:06:54 gibbs Exp $  */
end_comment

begin_comment
comment|/*  * TODO:  * 	Add support for dual and wide busses  *	Implement Target Mode  *	Implement Tagged Queuing  * 	Add target reset capabilities  *	Add support for the 294X series cards  *  *	This driver is very stable, and seems to offer performance  *	comprable to the 1742 FreeBSD driver.  I have not experienced  *	any timeouts since the timeout code was written, so in that   *	sense, it is untested.  */
end_comment
...
parameter_list|)
define|\
value|do {                                    \                         outb( SEQCTL + ahc->baseport, SEQRESET|FASTMODE );     \                 } while (inw(SEQADDR0 + ahc->baseport) != 0);     \                                                         \                 UNPAUSE_SEQUENCER(ahc);
end_define
...
name|needsdtr
decl_stmt|;
comment|/* Targets we iniaitiate sync neg to */
name|int
name|numscbs
decl_stmt|;
name|u_char
...
argument_list|)
expr_stmt|;
comment|/* 	 * Clear the pending messages flag 	 */
name|outb
argument_list|(
name|HA_MSG_FLAGS
operator|+
