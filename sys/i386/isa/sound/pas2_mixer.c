begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_define
define|#
directive|define
name|_PAS2_MIXER_C_
end_define

begin_comment
comment|/*  * linux/kernel/chr_drv/sound/pas2_mixer.c  *   * Mixer routines for the Pro Audio Spectrum cards.  *   * (C) 1992  Hannu Savolainen (hsavolai@cs.helsinki.fi) Craig Metz  * (cmetz@thor.tjhsst.edu) See COPYING for further details. Should be  * distributed with this file.  */
end_comment

begin_include
include|#
directive|include
file|"sound_config.h"
end_include

begin_if
if|#
directive|if
name|defined
argument_list|(
name|CONFIGURE_SOUNDCARD
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|EXCLUDE_PAS
argument_list|)
end_if

begin_include
include|#
directive|include
file|"pas.h"
end_include

begin_define
define|#
directive|define
name|TRACE
parameter_list|(
name|what
parameter_list|)
end_define

begin_comment
comment|/* (what) */
end_comment

begin_decl_stmt
specifier|extern
name|int
name|translat_code
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|rec_devices
init|=
operator|(
name|SOUND_MASK_MIC
operator|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Default recording source */
end_comment

begin_decl_stmt
specifier|static
name|int
name|mode_control
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|POSSIBLE_RECORDING_DEVICES
value|(SOUND_MASK_SYNTH | SOUND_MASK_SPEAKER | SOUND_MASK_LINE | SOUND_MASK_MIC | \ 					 SOUND_MASK_CD | SOUND_MASK_ALTPCM)
end_define

begin_define
define|#
directive|define
name|SUPPORTED_MIXER_DEVICES
value|(SOUND_MASK_SYNTH | SOUND_MASK_PCM | SOUND_MASK_SPEAKER | SOUND_MASK_LINE | SOUND_MASK_MIC | \ 					 SOUND_MASK_CD
comment|/*|SOUND_MASK_ALTPCM*/
value|| SOUND_MASK_IMIX | \ 					 SOUND_MASK_VOLUME | SOUND_MASK_BASS | SOUND_MASK_TREBLE | SOUND_MASK_RECLEV | \ 					 SOUND_MASK_MUTE | SOUND_MASK_ENHANCE | SOUND_MASK_LOUD)
end_define

begin_decl_stmt
specifier|static
name|unsigned
name|short
name|levels
index|[
name|SOUND_MIXER_NRDEVICES
index|]
init|=
block|{
literal|0x3232
block|,
comment|/* Master Volume */
literal|0x3232
block|,
comment|/* Bass */
literal|0x3232
block|,
comment|/* Treble */
literal|0x5050
block|,
comment|/* FM */
literal|0x4b4b
block|,
comment|/* PCM */
literal|0x3232
block|,
comment|/* PC Speaker */
literal|0x4b4b
block|,
comment|/* Ext Line */
literal|0x4b4b
block|,
comment|/* Mic */
literal|0x4b4b
block|,
comment|/* CD */
literal|0x6464
block|,
comment|/* Recording monitor */
literal|0x4b4b
block|,
comment|/* SB PCM */
literal|0x6464
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Recording level */
end_comment

begin_function
specifier|static
name|int
name|mixer_output
parameter_list|(
name|int
name|right_vol
parameter_list|,
name|int
name|left_vol
parameter_list|,
name|int
name|div
parameter_list|,
name|int
name|bits
parameter_list|,
name|int
name|mixer
comment|/* Input or output mixer */
parameter_list|)
block|{
name|int
name|left
init|=
name|left_vol
operator|*
name|div
operator|/
literal|100
decl_stmt|;
name|int
name|right
init|=
name|right_vol
operator|*
name|div
operator|/
literal|100
decl_stmt|;
comment|/*    * The Revision D cards have a problem with their MVA508 interface. The    * kludge-o-rama fix is to make a 16-bit quantity with identical LSB and    * MSBs out of the output byte and to do a 16-bit out to the mixer port -    * 1. We don't need to do this because the call to pas_write more than    * compensates for the timing problems.    */
if|if
condition|(
name|bits
operator|&
name|P_M_MV508_MIXER
condition|)
block|{
comment|/* Select input or output mixer */
name|left
operator||=
name|mixer
expr_stmt|;
name|right
operator||=
name|mixer
expr_stmt|;
block|}
if|if
condition|(
name|bits
operator|==
name|P_M_MV508_BASS
operator|||
name|bits
operator|==
name|P_M_MV508_TREBLE
condition|)
block|{
comment|/* Bass and trebble are mono devices	 */
name|pas_write
argument_list|(
name|P_M_MV508_ADDRESS
operator||
name|bits
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
name|pas_write
argument_list|(
name|left
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
name|right_vol
operator|=
name|left_vol
expr_stmt|;
block|}
else|else
block|{
name|pas_write
argument_list|(
name|P_M_MV508_ADDRESS
operator||
name|P_M_MV508_LEFT
operator||
name|bits
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
name|pas_write
argument_list|(
name|left
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
name|pas_write
argument_list|(
name|P_M_MV508_ADDRESS
operator||
name|P_M_MV508_RIGHT
operator||
name|bits
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
name|pas_write
argument_list|(
name|right
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|left_vol
operator||
operator|(
name|right_vol
operator|<<
literal|8
operator|)
operator|)
return|;
block|}
end_function

begin_function
name|void
name|set_mode
parameter_list|(
name|int
name|new_mode
parameter_list|)
block|{
name|pas_write
argument_list|(
name|P_M_MV508_ADDRESS
operator||
name|P_M_MV508_MODE
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
name|pas_write
argument_list|(
name|new_mode
argument_list|,
name|PARALLEL_MIXER
argument_list|)
expr_stmt|;
name|mode_control
operator|=
name|new_mode
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|pas_mixer_set
parameter_list|(
name|int
name|whichDev
parameter_list|,
name|unsigned
name|int
name|level
parameter_list|)
block|{
name|int
name|left
decl_stmt|,
name|right
decl_stmt|,
name|devmask
decl_stmt|,
name|changed
decl_stmt|,
name|i
decl_stmt|,
name|mixer
init|=
literal|0
decl_stmt|;
name|TRACE
argument_list|(
name|printk
argument_list|(
literal|"static int pas_mixer_set(int whichDev = %d, unsigned int level = %X)\n"
argument_list|,
name|whichDev
argument_list|,
name|level
argument_list|)
argument_list|)
expr_stmt|;
name|left
operator|=
name|level
operator|&
literal|0x7f
expr_stmt|;
name|right
operator|=
operator|(
name|level
operator|&
literal|0x7f00
operator|)
operator|>>
literal|8
expr_stmt|;
if|if
condition|(
name|whichDev
operator|<
name|SOUND_MIXER_NRDEVICES
condition|)
if|if
condition|(
operator|(
literal|1
operator|<<
name|whichDev
operator|)
operator|&
name|rec_devices
condition|)
name|mixer
operator|=
name|P_M_MV508_INPUTMIX
expr_stmt|;
else|else
name|mixer
operator|=
name|P_M_MV508_OUTPUTMIX
expr_stmt|;
switch|switch
condition|(
name|whichDev
condition|)
block|{
case|case
name|SOUND_MIXER_VOLUME
case|:
comment|/* Master volume (0-63) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|63
argument_list|,
name|P_M_MV508_MASTER_A
argument_list|,
literal|0
argument_list|)
expr_stmt|;
break|break;
comment|/*        * Note! Bass and Treble are mono devices. Will use just the left        * channel.        */
case|case
name|SOUND_MIXER_BASS
case|:
comment|/* Bass (0-12) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|12
argument_list|,
name|P_M_MV508_BASS
argument_list|,
literal|0
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_TREBLE
case|:
comment|/* Treble (0-12) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|12
argument_list|,
name|P_M_MV508_TREBLE
argument_list|,
literal|0
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_SYNTH
case|:
comment|/* Internal synthesizer (0-31) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_FM
argument_list|,
name|mixer
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_PCM
case|:
comment|/* PAS PCM (0-31) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_PCM
argument_list|,
name|mixer
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_ALTPCM
case|:
comment|/* SB PCM (0-31) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_SB
argument_list|,
name|mixer
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_SPEAKER
case|:
comment|/* PC speaker (0-31) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_SPEAKER
argument_list|,
name|mixer
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_LINE
case|:
comment|/* External line (0-31) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_LINE
argument_list|,
name|mixer
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_CD
case|:
comment|/* CD (0-31) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_CDROM
argument_list|,
name|mixer
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_MIC
case|:
comment|/* External microphone (0-31) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_MIC
argument_list|,
name|mixer
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_IMIX
case|:
comment|/* Recording monitor (0-31) (Only available 				 * on the Output Mixer) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|31
argument_list|,
name|P_M_MV508_MIXER
operator||
name|P_M_MV508_IMIXER
argument_list|,
name|P_M_MV508_OUTPUTMIX
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_RECLEV
case|:
comment|/* Recording level (0-15) */
name|levels
index|[
name|whichDev
index|]
operator|=
name|mixer_output
argument_list|(
name|right
argument_list|,
name|left
argument_list|,
literal|15
argument_list|,
name|P_M_MV508_MASTER_B
argument_list|,
literal|0
argument_list|)
expr_stmt|;
break|break;
case|case
name|SOUND_MIXER_MUTE
case|:
return|return
literal|0
return|;
break|break;
case|case
name|SOUND_MIXER_ENHANCE
case|:
name|i
operator|=
literal|0
expr_stmt|;
name|level
operator|&=
literal|0x7f
expr_stmt|;
if|if
condition|(
name|level
condition|)
name|i
operator|=
operator|(
name|level
operator|/
literal|20
operator|)
operator|-
literal|1
expr_stmt|;
name|mode_control
operator|&=
operator|~
name|P_M_MV508_ENHANCE_BITS
expr_stmt|;
name|mode_control
operator||=
name|P_M_MV508_ENHANCE_BITS
expr_stmt|;
name|set_mode
argument_list|(
name|mode_control
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
condition|)
name|i
operator|=
operator|(
name|i
operator|+
literal|1
operator|)
operator|*
literal|20
expr_stmt|;
return|return
name|i
return|;
break|break;
case|case
name|SOUND_MIXER_LOUD
case|:
name|mode_control
operator|&=
operator|~
name|P_M_MV508_LOUDNESS
expr_stmt|;
if|if
condition|(
name|level
condition|)
name|mode_control
operator||=
name|P_M_MV508_LOUDNESS
expr_stmt|;
name|set_mode
argument_list|(
name|mode_control
argument_list|)
expr_stmt|;
return|return
operator|!
operator|!
name|level
return|;
comment|/* 0 or 1 */
break|break;
case|case
name|SOUND_MIXER_RECSRC
case|:
name|devmask
operator|=
name|level
operator|&
name|POSSIBLE_RECORDING_DEVICES
expr_stmt|;
name|changed
operator|=
name|devmask
operator|^
name|rec_devices
expr_stmt|;
name|rec_devices
operator|=
name|devmask
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|SOUND_MIXER_NRDEVICES
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|changed
operator|&
operator|(
literal|1
operator|<<
name|i
operator|)
condition|)
block|{
name|pas_mixer_set
argument_list|(
name|i
argument_list|,
name|levels
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
return|return
name|rec_devices
return|;
break|break;
default|default:
return|return
name|RET_ERROR
argument_list|(
name|EINVAL
argument_list|)
return|;
block|}
return|return
operator|(
name|levels
index|[
name|whichDev
index|]
operator|)
return|;
block|}
end_function

begin_comment
comment|/*****/
end_comment

begin_function
specifier|static
name|int
name|mixer_set_levels
parameter_list|(
name|struct
name|sb_mixer_levels
modifier|*
name|user_l
parameter_list|)
block|{
define|#
directive|define
name|cmix
parameter_list|(
name|v
parameter_list|)
value|((((v.r*100+7)/15)<<8)| ((v.l*100+7)/15))
name|struct
name|sb_mixer_levels
name|l
decl_stmt|;
name|IOCTL_FROM_USER
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|l
argument_list|,
operator|(
name|char
operator|*
operator|)
name|user_l
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|l
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|l
operator|.
name|master
operator|.
name|l
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|master
operator|.
name|r
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|line
operator|.
name|l
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|line
operator|.
name|r
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|voc
operator|.
name|l
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|voc
operator|.
name|r
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|fm
operator|.
name|l
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|fm
operator|.
name|r
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|cd
operator|.
name|l
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|cd
operator|.
name|r
operator|&
operator|~
literal|0xF
operator|||
name|l
operator|.
name|mic
operator|&
operator|~
literal|0x7
condition|)
return|return
operator|(
name|RET_ERROR
argument_list|(
name|EINVAL
argument_list|)
operator|)
return|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_VOLUME
argument_list|,
name|cmix
argument_list|(
name|l
operator|.
name|master
argument_list|)
argument_list|)
expr_stmt|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_LINE
argument_list|,
name|cmix
argument_list|(
name|l
operator|.
name|line
argument_list|)
argument_list|)
expr_stmt|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_PCM
argument_list|,
name|cmix
argument_list|(
name|l
operator|.
name|voc
argument_list|)
argument_list|)
expr_stmt|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_ALTPCM
argument_list|,
name|cmix
argument_list|(
name|l
operator|.
name|voc
argument_list|)
argument_list|)
expr_stmt|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_SYNTH
argument_list|,
name|cmix
argument_list|(
name|l
operator|.
name|fm
argument_list|)
argument_list|)
expr_stmt|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_CD
argument_list|,
name|cmix
argument_list|(
name|l
operator|.
name|cd
argument_list|)
argument_list|)
expr_stmt|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_MIC
argument_list|,
operator|(
operator|(
name|l
operator|.
name|mic
operator|*
literal|100
operator|+
literal|3
operator|)
operator|/
literal|7
operator|)
operator||
operator|(
operator|(
operator|(
name|l
operator|.
name|mic
operator|*
literal|100
operator|+
literal|3
operator|)
operator|/
literal|7
operator|)
operator|<<
literal|8
operator|)
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * This sets aspects of the Mixer that are not volume levels. (Recording  * source, filter level, I/O filtering, and stereo.)  */
end_comment

begin_function
specifier|static
name|int
name|mixer_set_params
parameter_list|(
name|struct
name|sb_mixer_params
modifier|*
name|user_p
parameter_list|)
block|{
name|struct
name|sb_mixer_params
name|p
decl_stmt|;
name|S_BYTE
name|val
decl_stmt|;
name|int
name|src
decl_stmt|;
name|unsigned
name|long
name|flags
decl_stmt|;
name|IOCTL_FROM_USER
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|p
argument_list|,
operator|(
name|char
operator|*
operator|)
name|user_p
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|p
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|.
name|record_source
operator|!=
name|SRC_MIC
operator|&&
name|p
operator|.
name|record_source
operator|!=
name|SRC_CD
operator|&&
name|p
operator|.
name|record_source
operator|!=
name|SRC_LINE
condition|)
return|return
operator|(
name|RET_ERROR
argument_list|(
name|EINVAL
argument_list|)
operator|)
return|;
comment|/*    * I'm not sure if this is The Right Thing.  Should stereo be entirely    * under control of DSP?  I like being able to toggle it while a sound is    * playing, so I do this... because I can.    */
name|DISABLE_INTR
argument_list|(
name|flags
argument_list|)
expr_stmt|;
name|val
operator|=
operator|(
name|pas_read
argument_list|(
name|PCM_CONTROL
argument_list|)
operator|&
operator|~
name|P_C_MIXER_CROSS_FIELD
operator|)
operator||
name|P_C_MIXER_CROSS_R_TO_R
operator||
name|P_C_MIXER_CROSS_L_TO_L
expr_stmt|;
if|if
condition|(
operator|!
name|p
operator|.
name|dsp_stereo
condition|)
name|val
operator||=
operator|(
name|P_C_MIXER_CROSS_R_TO_L
operator||
name|P_C_MIXER_CROSS_L_TO_R
operator|)
expr_stmt|;
comment|/* Mono */
name|pas_write
argument_list|(
name|val
argument_list|,
name|PCM_CONTROL
argument_list|)
expr_stmt|;
name|RESTORE_INTR
argument_list|(
name|flags
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|p
operator|.
name|record_source
condition|)
block|{
case|case
name|SRC_CD
case|:
name|src
operator|=
name|SOUND_MASK_CD
expr_stmt|;
break|break;
case|case
name|SRC_LINE
case|:
name|src
operator|=
name|SOUND_MASK_LINE
expr_stmt|;
break|break;
default|default:
name|src
operator|=
name|SOUND_MASK_MIC
expr_stmt|;
break|break;
block|}
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_RECSRC
argument_list|,
name|src
argument_list|)
expr_stmt|;
comment|/*    * setmixer (OUT_FILTER, ((dsp_stereo ? STEREO_DAC : MONO_DAC) |    * (p.filter_output ? FILT_ON : FILT_OFF)));    */
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|getmixer
parameter_list|(
name|int
name|dev
parameter_list|,
name|int
name|chn
parameter_list|)
block|{
if|if
condition|(
name|chn
operator|==
name|P_M_MV508_RIGHT
condition|)
block|{
return|return
operator|(
name|levels
index|[
name|dev
index|]
operator|>>
literal|8
operator|)
operator|&
literal|0x7f
return|;
block|}
else|else
block|{
return|return
name|levels
index|[
name|dev
index|]
operator|&
literal|0x7f
return|;
block|}
block|}
end_function

begin_comment
comment|/* Read the current mixer level settings into the user's struct. */
end_comment

begin_function
specifier|static
name|int
name|mixer_get_levels
parameter_list|(
name|struct
name|sb_mixer_levels
modifier|*
name|user_l
parameter_list|)
block|{
name|struct
name|sb_mixer_levels
name|l
decl_stmt|;
name|l
operator|.
name|master
operator|.
name|r
operator|=
operator|(
operator|(
operator|(
operator|(
name|levels
index|[
name|SOUND_MIXER_VOLUME
index|]
operator|>>
literal|8
operator|)
operator|&
literal|0x7f
operator|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
comment|/* Master */
name|l
operator|.
name|master
operator|.
name|l
operator|=
operator|(
operator|(
operator|(
name|levels
index|[
name|SOUND_MIXER_VOLUME
index|]
operator|&
literal|0x7f
operator|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
comment|/* Master */
name|l
operator|.
name|line
operator|.
name|r
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_LINE
argument_list|,
name|P_M_MV508_RIGHT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
comment|/* Line */
name|l
operator|.
name|line
operator|.
name|l
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_LINE
argument_list|,
name|P_M_MV508_LEFT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
name|l
operator|.
name|voc
operator|.
name|r
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_PCM
argument_list|,
name|P_M_MV508_RIGHT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
comment|/* DAC */
name|l
operator|.
name|voc
operator|.
name|l
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_PCM
argument_list|,
name|P_M_MV508_LEFT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
name|l
operator|.
name|fm
operator|.
name|r
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_SYNTH
argument_list|,
name|P_M_MV508_RIGHT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
comment|/* FM */
name|l
operator|.
name|fm
operator|.
name|l
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_SYNTH
argument_list|,
name|P_M_MV508_LEFT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
name|l
operator|.
name|cd
operator|.
name|r
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_CD
argument_list|,
name|P_M_MV508_RIGHT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
comment|/* CD */
name|l
operator|.
name|cd
operator|.
name|l
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_CD
argument_list|,
name|P_M_MV508_LEFT
argument_list|)
operator|*
literal|15
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
name|l
operator|.
name|mic
operator|=
operator|(
operator|(
name|getmixer
argument_list|(
name|SOUND_MIXER_MIC
argument_list|,
name|P_M_MV508_LEFT
argument_list|)
operator|*
literal|7
operator|)
operator|+
literal|50
operator|)
operator|/
literal|100
expr_stmt|;
comment|/* Microphone */
name|IOCTL_TO_USER
argument_list|(
operator|(
name|char
operator|*
operator|)
name|user_l
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|l
argument_list|,
sizeof|sizeof
argument_list|(
name|l
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/* Read the current mixer parameters into the user's struct. */
end_comment

begin_function
specifier|static
name|int
name|mixer_get_params
parameter_list|(
name|struct
name|sb_mixer_params
modifier|*
name|user_params
parameter_list|)
block|{
name|S_BYTE
name|val
decl_stmt|;
name|struct
name|sb_mixer_params
name|params
decl_stmt|;
switch|switch
condition|(
name|rec_devices
condition|)
block|{
case|case
name|SOUND_MASK_CD
case|:
name|params
operator|.
name|record_source
operator|=
name|SRC_CD
expr_stmt|;
break|break;
case|case
name|SOUND_MASK_LINE
case|:
name|params
operator|.
name|record_source
operator|=
name|SRC_LINE
expr_stmt|;
break|break;
case|case
name|SOUND_MASK_MIC
case|:
name|params
operator|.
name|record_source
operator|=
name|SRC_MIC
expr_stmt|;
break|break;
default|default:
name|params
operator|.
name|record_source
operator|=
name|SRC_MIC
expr_stmt|;
name|pas_mixer_set
argument_list|(
name|SOUND_MIXER_RECSRC
argument_list|,
name|SOUND_MASK_MIC
argument_list|)
expr_stmt|;
comment|/* Adjust */
block|}
name|params
operator|.
name|hifreq_filter
operator|=
name|OFF
expr_stmt|;
name|params
operator|.
name|filter_input
operator|=
name|OFF
expr_stmt|;
name|params
operator|.
name|filter_output
operator|=
name|OFF
expr_stmt|;
name|val
operator|=
name|INB
argument_list|(
name|PCM_CONTROL
argument_list|)
expr_stmt|;
name|params
operator|.
name|dsp_stereo
operator|=
operator|(
operator|(
name|val
operator|&
name|P_C_MIXER_CROSS_FIELD
operator|)
operator|==
operator|(
name|P_C_MIXER_CROSS_L_TO_L
operator||
name|P_C_MIXER_CROSS_R_TO_R
operator|)
operator|)
expr_stmt|;
name|IOCTL_TO_USER
argument_list|(
operator|(
name|char
operator|*
operator|)
name|user_params
argument_list|,
literal|0
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|params
argument_list|,
sizeof|sizeof
argument_list|(
name|params
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*****/
end_comment

begin_function
specifier|static
name|void
name|pas_mixer_reset
parameter_list|(
name|void
parameter_list|)
block|{
name|int
name|foo
decl_stmt|;
name|TRACE
argument_list|(
name|printk
argument_list|(
literal|"pas2_mixer.c: void pas_mixer_reset(void)\n"
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|foo
operator|=
literal|0
init|;
name|foo
operator|<
name|SOUND_MIXER_NRDEVICES
condition|;
name|foo
operator|++
control|)
name|pas_mixer_set
argument_list|(
name|foo
argument_list|,
name|levels
index|[
name|foo
index|]
argument_list|)
expr_stmt|;
name|set_mode
argument_list|(
name|P_M_MV508_LOUDNESS
operator||
name|P_M_MV508_ENHANCE_40
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|int
name|pas_mixer_ioctl
parameter_list|(
name|int
name|dev
parameter_list|,
name|unsigned
name|int
name|cmd
parameter_list|,
name|unsigned
name|int
name|arg
parameter_list|)
block|{
name|TRACE
argument_list|(
name|printk
argument_list|(
literal|"pas2_mixer.c: int pas_mixer_ioctl(unsigned int cmd = %X, unsigned int arg = %X)\n"
argument_list|,
name|cmd
argument_list|,
name|arg
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
operator|(
name|cmd
operator|>>
literal|8
operator|)
operator|&
literal|0xff
operator|)
operator|==
literal|'M'
condition|)
block|{
if|if
condition|(
name|cmd
operator|&
name|IOC_IN
condition|)
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
name|pas_mixer_set
argument_list|(
name|cmd
operator|&
literal|0xff
argument_list|,
name|IOCTL_IN
argument_list|(
name|arg
argument_list|)
argument_list|)
argument_list|)
return|;
else|else
block|{
comment|/* Read parameters */
switch|switch
condition|(
name|cmd
operator|&
literal|0xff
condition|)
block|{
case|case
name|SOUND_MIXER_RECSRC
case|:
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
name|rec_devices
argument_list|)
return|;
break|break;
case|case
name|SOUND_MIXER_STEREODEVS
case|:
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
name|SUPPORTED_MIXER_DEVICES
operator|&
operator|~
operator|(
name|SOUND_MASK_BASS
operator||
name|SOUND_MASK_TREBLE
operator|)
argument_list|)
return|;
break|break;
case|case
name|SOUND_MIXER_DEVMASK
case|:
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
name|SUPPORTED_MIXER_DEVICES
argument_list|)
return|;
break|break;
case|case
name|SOUND_MIXER_RECMASK
case|:
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
name|POSSIBLE_RECORDING_DEVICES
operator|&
name|SUPPORTED_MIXER_DEVICES
argument_list|)
return|;
break|break;
case|case
name|SOUND_MIXER_CAPS
case|:
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
literal|0
argument_list|)
return|;
comment|/* No special capabilities */
break|break;
case|case
name|SOUND_MIXER_MUTE
case|:
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
literal|0
argument_list|)
return|;
comment|/* No mute yet */
break|break;
case|case
name|SOUND_MIXER_ENHANCE
case|:
if|if
condition|(
operator|!
operator|(
name|mode_control
operator|&
name|P_M_MV508_ENHANCE_BITS
operator|)
condition|)
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
literal|0
argument_list|)
return|;
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
operator|(
operator|(
name|mode_control
operator|&
name|P_M_MV508_ENHANCE_BITS
operator|)
operator|+
literal|1
operator|)
operator|*
literal|20
argument_list|)
return|;
break|break;
case|case
name|SOUND_MIXER_LOUD
case|:
if|if
condition|(
name|mode_control
operator|&
name|P_M_MV508_LOUDNESS
condition|)
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
literal|1
argument_list|)
return|;
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
literal|0
argument_list|)
return|;
break|break;
default|default:
return|return
name|IOCTL_OUT
argument_list|(
name|arg
argument_list|,
name|levels
index|[
name|cmd
operator|&
literal|0xff
index|]
argument_list|)
return|;
block|}
block|}
block|}
else|else
block|{
switch|switch
condition|(
name|cmd
condition|)
block|{
case|case
name|MIXER_IOCTL_SET_LEVELS
case|:
name|mixer_set_levels
argument_list|(
operator|(
expr|struct
name|sb_mixer_levels
operator|*
operator|)
name|arg
argument_list|)
expr_stmt|;
return|return
name|mixer_get_levels
argument_list|(
operator|(
expr|struct
name|sb_mixer_levels
operator|*
operator|)
name|arg
argument_list|)
return|;
case|case
name|MIXER_IOCTL_SET_PARAMS
case|:
name|mixer_set_params
argument_list|(
operator|(
expr|struct
name|sb_mixer_params
operator|*
operator|)
name|arg
argument_list|)
expr_stmt|;
return|return
name|mixer_get_params
argument_list|(
operator|(
expr|struct
name|sb_mixer_params
operator|*
operator|)
name|arg
argument_list|)
return|;
case|case
name|MIXER_IOCTL_READ_LEVELS
case|:
return|return
name|mixer_get_levels
argument_list|(
operator|(
expr|struct
name|sb_mixer_levels
operator|*
operator|)
name|arg
argument_list|)
return|;
case|case
name|MIXER_IOCTL_READ_PARAMS
case|:
return|return
name|mixer_get_params
argument_list|(
operator|(
expr|struct
name|sb_mixer_params
operator|*
operator|)
name|arg
argument_list|)
return|;
case|case
name|MIXER_IOCTL_RESET
case|:
name|pas_mixer_reset
argument_list|()
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
default|default:
return|return
name|RET_ERROR
argument_list|(
name|EINVAL
argument_list|)
return|;
block|}
block|}
return|return
name|RET_ERROR
argument_list|(
name|EINVAL
argument_list|)
return|;
block|}
end_function

begin_decl_stmt
specifier|static
name|struct
name|mixer_operations
name|pas_mixer_operations
init|=
block|{
name|pas_mixer_ioctl
block|}
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|pas_init_mixer
parameter_list|(
name|void
parameter_list|)
block|{
name|pas_mixer_reset
argument_list|()
expr_stmt|;
name|mixer_devs
index|[
name|num_mixers
operator|++
index|]
operator|=
operator|&
name|pas_mixer_operations
expr_stmt|;
return|return
literal|1
return|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

end_unit

