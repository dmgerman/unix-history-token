begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * linux/kernel/chr_drv/sound/dsp.c  *   * Device file manager for /dev/dsp  *   * (C) 1992  Hannu Savolainen (hsavolai@cs.helsinki.fi) See COPYING for further  * details. Should be distributed with this file.  */
end_comment

begin_include
include|#
directive|include
file|"sound_config.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|CONFIGURE_SOUNDCARD
end_ifdef

begin_ifndef
ifndef|#
directive|ifndef
name|EXCLUDE_AUDIO
end_ifndef

begin_define
define|#
directive|define
name|ON
value|1
end_define

begin_define
define|#
directive|define
name|OFF
value|0
end_define

begin_decl_stmt
specifier|static
name|int
name|wr_buff_no
index|[
name|MAX_DSP_DEV
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* != -1, if there is a 						 * incomplete output block */
end_comment

begin_decl_stmt
specifier|static
name|int
name|wr_buff_size
index|[
name|MAX_DSP_DEV
index|]
decl_stmt|,
name|wr_buf_ptr
index|[
name|MAX_DSP_DEV
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|wr_dma_buf
index|[
name|MAX_DSP_DEV
index|]
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|dsp_open
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|int
name|bits
parameter_list|)
block|{
name|int
name|mode
decl_stmt|;
name|int
name|ret
decl_stmt|;
name|dev
operator|=
name|dev
operator|>>
literal|4
expr_stmt|;
name|mode
operator|=
name|file
operator|->
name|mode
operator|&
name|O_ACCMODE
expr_stmt|;
if|if
condition|(
operator|(
name|ret
operator|=
name|DMAbuf_open
argument_list|(
name|dev
argument_list|,
name|mode
argument_list|)
operator|)
operator|<
literal|0
condition|)
return|return
name|ret
return|;
if|if
condition|(
name|DMAbuf_ioctl
argument_list|(
name|dev
argument_list|,
name|SNDCTL_DSP_SAMPLESIZE
argument_list|,
name|bits
argument_list|,
literal|1
argument_list|)
operator|!=
name|bits
condition|)
block|{
name|dsp_release
argument_list|(
name|dev
argument_list|,
name|file
argument_list|)
expr_stmt|;
return|return
name|RET_ERROR
argument_list|(
name|ENXIO
argument_list|)
return|;
block|}
name|wr_buff_no
index|[
name|dev
index|]
operator|=
operator|-
literal|1
expr_stmt|;
return|return
name|ret
return|;
block|}
end_function

begin_function
name|void
name|dsp_release
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|)
block|{
name|int
name|mode
decl_stmt|;
name|dev
operator|=
name|dev
operator|>>
literal|4
expr_stmt|;
name|mode
operator|=
name|file
operator|->
name|mode
operator|&
name|O_ACCMODE
expr_stmt|;
if|if
condition|(
name|wr_buff_no
index|[
name|dev
index|]
operator|>=
literal|0
condition|)
block|{
name|DMAbuf_start_output
argument_list|(
name|dev
argument_list|,
name|wr_buff_no
index|[
name|dev
index|]
argument_list|,
name|wr_buf_ptr
index|[
name|dev
index|]
argument_list|)
expr_stmt|;
name|wr_buff_no
index|[
name|dev
index|]
operator|=
operator|-
literal|1
expr_stmt|;
block|}
name|DMAbuf_release
argument_list|(
name|dev
argument_list|,
name|mode
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|int
name|dsp_write
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|snd_rw_buf
modifier|*
name|buf
parameter_list|,
name|int
name|count
parameter_list|)
block|{
name|int
name|c
decl_stmt|,
name|p
decl_stmt|,
name|l
decl_stmt|;
name|int
name|err
decl_stmt|;
name|dev
operator|=
name|dev
operator|>>
literal|4
expr_stmt|;
name|p
operator|=
literal|0
expr_stmt|;
name|c
operator|=
name|count
expr_stmt|;
if|if
condition|(
operator|!
name|count
condition|)
comment|/* Flush output */
block|{
if|if
condition|(
name|wr_buff_no
index|[
name|dev
index|]
operator|>=
literal|0
condition|)
block|{
name|DMAbuf_start_output
argument_list|(
name|dev
argument_list|,
name|wr_buff_no
index|[
name|dev
index|]
argument_list|,
name|wr_buf_ptr
index|[
name|dev
index|]
argument_list|)
expr_stmt|;
name|wr_buff_no
index|[
name|dev
index|]
operator|=
operator|-
literal|1
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
while|while
condition|(
name|c
condition|)
block|{
comment|/* Perform output blocking */
if|if
condition|(
name|wr_buff_no
index|[
name|dev
index|]
operator|<
literal|0
condition|)
comment|/* There is no incomplete buffers */
block|{
if|if
condition|(
operator|(
name|wr_buff_no
index|[
name|dev
index|]
operator|=
name|DMAbuf_getwrbuffer
argument_list|(
name|dev
argument_list|,
operator|&
name|wr_dma_buf
index|[
name|dev
index|]
argument_list|,
operator|&
name|wr_buff_size
index|[
name|dev
index|]
argument_list|)
operator|)
operator|<
literal|0
condition|)
return|return
name|wr_buff_no
index|[
name|dev
index|]
return|;
name|wr_buf_ptr
index|[
name|dev
index|]
operator|=
literal|0
expr_stmt|;
block|}
name|l
operator|=
name|c
expr_stmt|;
if|if
condition|(
name|l
operator|>
operator|(
name|wr_buff_size
index|[
name|dev
index|]
operator|-
name|wr_buf_ptr
index|[
name|dev
index|]
operator|)
condition|)
name|l
operator|=
operator|(
name|wr_buff_size
index|[
name|dev
index|]
operator|-
name|wr_buf_ptr
index|[
name|dev
index|]
operator|)
expr_stmt|;
if|if
condition|(
operator|!
name|dsp_devs
index|[
name|dev
index|]
operator|->
name|copy_from_user
condition|)
block|{
comment|/* No device specific copy routine */
name|COPY_FROM_USER
argument_list|(
operator|&
name|wr_dma_buf
index|[
name|dev
index|]
index|[
name|wr_buf_ptr
index|[
name|dev
index|]
index|]
argument_list|,
name|buf
argument_list|,
name|p
argument_list|,
name|l
argument_list|)
expr_stmt|;
block|}
else|else
name|dsp_devs
index|[
name|dev
index|]
operator|->
name|copy_from_user
argument_list|(
name|dev
argument_list|,
name|wr_dma_buf
index|[
name|dev
index|]
argument_list|,
name|wr_buf_ptr
index|[
name|dev
index|]
argument_list|,
name|buf
argument_list|,
name|p
argument_list|,
name|l
argument_list|)
expr_stmt|;
name|c
operator|-=
name|l
expr_stmt|;
name|p
operator|+=
name|l
expr_stmt|;
name|wr_buf_ptr
index|[
name|dev
index|]
operator|+=
name|l
expr_stmt|;
if|if
condition|(
name|wr_buf_ptr
index|[
name|dev
index|]
operator|>=
name|wr_buff_size
index|[
name|dev
index|]
condition|)
block|{
if|if
condition|(
operator|(
name|err
operator|=
name|DMAbuf_start_output
argument_list|(
name|dev
argument_list|,
name|wr_buff_no
index|[
name|dev
index|]
argument_list|,
name|wr_buf_ptr
index|[
name|dev
index|]
argument_list|)
operator|)
operator|<
literal|0
condition|)
return|return
name|err
return|;
name|wr_buff_no
index|[
name|dev
index|]
operator|=
operator|-
literal|1
expr_stmt|;
block|}
block|}
return|return
name|count
return|;
block|}
end_function

begin_function
name|int
name|dsp_read
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|snd_rw_buf
modifier|*
name|buf
parameter_list|,
name|int
name|count
parameter_list|)
block|{
name|int
name|c
decl_stmt|,
name|p
decl_stmt|,
name|l
decl_stmt|;
name|char
modifier|*
name|dmabuf
decl_stmt|;
name|int
name|buff_no
decl_stmt|;
name|dev
operator|=
name|dev
operator|>>
literal|4
expr_stmt|;
name|p
operator|=
literal|0
expr_stmt|;
name|c
operator|=
name|count
expr_stmt|;
while|while
condition|(
name|c
condition|)
block|{
if|if
condition|(
operator|(
name|buff_no
operator|=
name|DMAbuf_getrdbuffer
argument_list|(
name|dev
argument_list|,
operator|&
name|dmabuf
argument_list|,
operator|&
name|l
argument_list|)
operator|)
operator|<
literal|0
condition|)
return|return
name|buff_no
return|;
if|if
condition|(
name|l
operator|>
name|c
condition|)
name|l
operator|=
name|c
expr_stmt|;
comment|/* Insert any local processing here. */
name|COPY_TO_USER
argument_list|(
name|buf
argument_list|,
literal|0
argument_list|,
name|dmabuf
argument_list|,
name|l
argument_list|)
expr_stmt|;
name|DMAbuf_rmchars
argument_list|(
name|dev
argument_list|,
name|buff_no
argument_list|,
name|l
argument_list|)
expr_stmt|;
name|p
operator|+=
name|l
expr_stmt|;
name|c
operator|-=
name|l
expr_stmt|;
block|}
return|return
name|count
operator|-
name|c
return|;
block|}
end_function

begin_function
name|int
name|dsp_ioctl
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|unsigned
name|int
name|cmd
parameter_list|,
name|unsigned
name|int
name|arg
parameter_list|)
block|{
name|dev
operator|=
name|dev
operator|>>
literal|4
expr_stmt|;
switch|switch
condition|(
name|cmd
condition|)
block|{
case|case
name|SNDCTL_DSP_SYNC
case|:
if|if
condition|(
name|wr_buff_no
index|[
name|dev
index|]
operator|>=
literal|0
condition|)
block|{
name|DMAbuf_start_output
argument_list|(
name|dev
argument_list|,
name|wr_buff_no
index|[
name|dev
index|]
argument_list|,
name|wr_buf_ptr
index|[
name|dev
index|]
argument_list|)
expr_stmt|;
name|wr_buff_no
index|[
name|dev
index|]
operator|=
operator|-
literal|1
expr_stmt|;
block|}
return|return
name|DMAbuf_ioctl
argument_list|(
name|dev
argument_list|,
name|cmd
argument_list|,
name|arg
argument_list|,
literal|0
argument_list|)
return|;
break|break;
case|case
name|SNDCTL_DSP_POST
case|:
if|if
condition|(
name|wr_buff_no
index|[
name|dev
index|]
operator|>=
literal|0
condition|)
block|{
name|DMAbuf_start_output
argument_list|(
name|dev
argument_list|,
name|wr_buff_no
index|[
name|dev
index|]
argument_list|,
name|wr_buf_ptr
index|[
name|dev
index|]
argument_list|)
expr_stmt|;
name|wr_buff_no
index|[
name|dev
index|]
operator|=
operator|-
literal|1
expr_stmt|;
block|}
return|return
literal|0
return|;
break|break;
case|case
name|SNDCTL_DSP_RESET
case|:
name|wr_buff_no
index|[
name|dev
index|]
operator|=
operator|-
literal|1
expr_stmt|;
return|return
name|DMAbuf_ioctl
argument_list|(
name|dev
argument_list|,
name|cmd
argument_list|,
name|arg
argument_list|,
literal|0
argument_list|)
return|;
break|break;
default|default:
return|return
name|DMAbuf_ioctl
argument_list|(
name|dev
argument_list|,
name|cmd
argument_list|,
name|arg
argument_list|,
literal|0
argument_list|)
return|;
block|}
block|}
end_function

begin_function
name|long
name|dsp_init
parameter_list|(
name|long
name|mem_start
parameter_list|)
block|{
return|return
name|mem_start
return|;
block|}
end_function

begin_else
else|#
directive|else
end_else

begin_comment
comment|/* Stub version */
end_comment

begin_function
name|int
name|dsp_read
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|snd_rw_buf
modifier|*
name|buf
parameter_list|,
name|int
name|count
parameter_list|)
block|{
return|return
name|RET_ERROR
argument_list|(
name|EIO
argument_list|)
return|;
block|}
end_function

begin_function
name|int
name|dsp_write
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|snd_rw_buf
modifier|*
name|buf
parameter_list|,
name|int
name|count
parameter_list|)
block|{
return|return
name|RET_ERROR
argument_list|(
name|EIO
argument_list|)
return|;
block|}
end_function

begin_function
name|int
name|dsp_open
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|int
name|bits
parameter_list|)
block|{
return|return
name|RET_ERROR
argument_list|(
name|ENXIO
argument_list|)
return|;
block|}
end_function

begin_function
name|void
name|dsp_release
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|)
block|{   }
end_function

begin_empty_stmt
empty_stmt|;
end_empty_stmt

begin_function
name|int
name|dsp_ioctl
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|unsigned
name|int
name|cmd
parameter_list|,
name|unsigned
name|int
name|arg
parameter_list|)
block|{
return|return
name|RET_ERROR
argument_list|(
name|EIO
argument_list|)
return|;
block|}
end_function

begin_function
name|int
name|dsp_lseek
parameter_list|(
name|int
name|dev
parameter_list|,
name|struct
name|fileinfo
modifier|*
name|file
parameter_list|,
name|off_t
name|offset
parameter_list|,
name|int
name|orig
parameter_list|)
block|{
return|return
name|RET_ERROR
argument_list|(
name|EIO
argument_list|)
return|;
block|}
end_function

begin_function
name|long
name|dsp_init
parameter_list|(
name|long
name|mem_start
parameter_list|)
block|{
return|return
name|mem_start
return|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

begin_endif
endif|#
directive|endif
end_endif

end_unit

