...
parameter_list|)
define|\
value|{ \ 	  int flag, chn; \ 	  f.mode = WK_SLEEP; \ 	  q =&chn; \ 	  flag=tsleep(&chn, (PRIBIO-5)|PCATCH, "sndint", time_limit); \ 	  if(flag == ERESTART) f.aborting = 1;\ 	  else f.aborting = 0;\ 	  f.mode&= ~WK_SLEEP; \ 	}
end_define
...
name|f
parameter_list|)
value|{f.mode = WK_WAKEUP;wakeup(q);}
end_define
