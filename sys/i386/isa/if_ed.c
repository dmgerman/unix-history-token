...
name|ether_delmulti
argument_list|(
argument|(ifr,&sc->arpcom);  		if (error == ENETRESET) {
comment|/* 			 * Multicast list has changed; set the hardware filter 			 * accordingly. 			 */
argument|ed_setrcr(ifp, sc); 			error =
literal|0
argument|; 		} 		break;
endif|#
directive|endif
...
argument|if (ifr->ifr_mtu> ETHERMTU) { 			error = EINVAL; 		} else { 			ifp->if_mtu = ifr->ifr_mtu; 		} 		break;  	default: 		error = EINVAL; 	} 	(void) splx(s); 	return (error); }
...
argument|static void ed_get_packet(sc, buf, len, multicast) 	struct ed_softc *sc; 	char   *buf; 	u_short len; 	int     multicast; { 	struct ether_header *eh; 	struct mbuf *m
argument_list|,
argument|*head =
literal|0
argument_list|,
argument|*ed_ring_to_mbuf(); 	u_short off; 	int     resid; 	u_short etype; 	struct trailer_header trailer_header;
comment|/* Allocate a header mbuf */
argument|MGETHDR(m, M_DONTWAIT, MT_DATA); 	if (m ==
literal|0
argument|) 		goto bad; 	m->m_pkthdr.rcvif =&sc->arpcom.ac_if; 	m->m_pkthdr.len = len; 	m->m_len =
literal|0
argument|; 	head = m;
...
argument|head->m_data += EOFF; 	eh = mtod(head, struct ether_header *);  	if (sc->mem_shared) 		bcopy(buf, mtod(head, caddr_t), sizeof(struct ether_header)); 	else 		ed_pio_readmem(sc, buf, mtod(head, caddr_t), 			       sizeof(struct ether_header)); 	buf += sizeof(struct ether_header); 	head->m_len += sizeof(struct ether_header); 	len -= sizeof(struct ether_header);  	etype = ntohs((u_short) eh->ether_type);
...
argument|if (etype>= ETHERTYPE_TRAIL&& 	    etype< ETHERTYPE_TRAIL + ETHERTYPE_NTRAILER) {  		off = (etype - ETHERTYPE_TRAIL)<<
literal|9
argument|; 		if ((off + sizeof(struct trailer_header))> len) 			goto bad;
comment|/* insanity */
...
argument|if (sc->mem_shared) { 			eh->ether_type = *ringoffset(sc, buf, off, u_short *); 			resid = ntohs(*ringoffset(sc, buf, off +
literal|2
argument|, u_short *)); 		} else { 			struct trailer_header trailer_header;  			ed_pio_readmem(sc, 				       ringoffset(sc, buf, off, caddr_t), 				       (char *)&trailer_header, 				       sizeof(trailer_header)); 			eh->ether_type = trailer_header.ether_type; 			resid = trailer_header.ether_residual; 		}  		if ((off + resid)> len) 			goto bad;
comment|/* insanity */
argument|resid -= sizeof(struct trailer_header); 		if (resid<
literal|0
argument|) 			goto bad;
comment|/* insanity */
argument|m = ed_ring_to_mbuf(sc, ringoffset(sc, buf, off +
literal|4
argument|, char *), 				    head, resid); 		if (m ==
literal|0
argument|) 			goto bad;  		len = off; 		head->m_pkthdr.len -=
literal|4
argument|;
...
argument|m = ed_ring_to_mbuf(sc, buf, m, len); 	if (m ==
literal|0
argument|) 		goto bad;
if|#
directive|if
...
argument|if (sc->bpf) { 		bpf_mtap(sc->bpf, head);
...
argument|if ((sc->arpcom.ac_if.if_flags& IFF_PROMISC)&& 		    bcmp(eh->ether_dhost, sc->arpcom.ac_enaddr, 			 sizeof(eh->ether_dhost)) !=
literal|0
argument|&& multicast ==
literal|0
argument|) { 			m_freem(head); 			return; 		} 	}
endif|#
directive|endif
comment|/* 	 * Fix up data start offset in mbuf to point past ether header 	 */
argument|m_adj(head, sizeof(struct ether_header));
comment|/* 	 * silly ether_input routine needs 'type' in host byte order 	 */
argument|eh->ether_type = ntohs(eh->ether_type);  	ether_input(&sc->arpcom.ac_if, eh, head); 	return;  bad:	if (head) 		m_freem(head); 	return; }
...
argument|void ed_pio_readmem(sc, src, dst, amount) 	struct ed_softc *sc; 	unsigned short src; 	unsigned char *dst; 	unsigned short amount; { 	unsigned short tmp_amount;
comment|/* select page 0 registers */
argument|outb(sc->nic_addr + ED_P0_CR, ED_CR_RD2 | ED_CR_STA);
comment|/* round up to a word */
argument|tmp_amount = amount; 	if (amount&
literal|1
argument|) 		++amount;
comment|/* set up DMA byte count */
argument|outb(sc->nic_addr + ED_P0_RBCR0, amount); 	outb(sc->nic_addr + ED_P0_RBCR1, amount>>
literal|8
argument|);
comment|/* set up source address in NIC mem */
argument|outb(sc->nic_addr + ED_P0_RSAR0, src); 	outb(sc->nic_addr + ED_P0_RSAR1, src>>
literal|8
argument|);  	outb(sc->nic_addr + ED_P0_CR, ED_CR_RD0 | ED_CR_STA);  	if (sc->isa16bit) { 		insw(sc->asic_addr + ED_NOVELL_DATA, dst, amount /
literal|2
argument|); 	} else 		insb(sc->asic_addr + ED_NOVELL_DATA, dst, amount);  }
...
argument|void ed_pio_writemem(sc, src, dst, len) 	struct ed_softc *sc; 	char   *src; 	unsigned short dst; 	unsigned short len; { 	int     maxwait =
literal|100
argument|;
comment|/* about 120us */
comment|/* select page 0 registers */
...
argument|outb(sc->nic_addr + ED_P0_ISR, ED_ISR_RDC);
comment|/* set up DMA byte count */
argument|outb(sc->nic_addr + ED_P0_RBCR0, len); 	outb(sc->nic_addr + ED_P0_RBCR1, len>>
literal|8
argument|);
comment|/* set up destination address in NIC mem */
argument|outb(sc->nic_addr + ED_P0_RSAR0, dst); 	outb(sc->nic_addr + ED_P0_RSAR1, dst>>
literal|8
argument|);
comment|/* set remote DMA write */
argument|outb(sc->nic_addr + ED_P0_CR, ED_CR_RD1 | ED_CR_STA);  	if (sc->isa16bit) 		outsw(sc->asic_addr + ED_NOVELL_DATA, src, len /
literal|2
argument|); 	else 		outsb(sc->asic_addr + ED_NOVELL_DATA, src, len);
...
argument|while (((inb(sc->nic_addr + ED_P0_ISR)& ED_ISR_RDC) != ED_ISR_RDC)&& --maxwait); }
comment|/*  * Write an mbuf chain to the destination NIC memory address using  *	programmed I/O.  */
argument|u_short ed_pio_write_mbufs(sc, m, dst) 	struct ed_softc *sc; 	struct mbuf *m; 	unsigned short dst; { 	unsigned short len
argument_list|,
argument|mb_offset; 	struct mbuf *mp; 	unsigned char residual[
literal|2
argument|]; 	int     maxwait =
literal|100
argument|;
comment|/* about 120us */
...
argument|for (len =
literal|0
argument|, mp = m; mp; mp = mp->m_next) 		len += mp->m_len;
comment|/* select page 0 registers */
argument|outb(sc->nic_addr + ED_P0_CR, ED_CR_RD2 | ED_CR_STA);
comment|/* reset remote DMA complete flag */
argument|outb(sc->nic_addr + ED_P0_ISR, ED_ISR_RDC);
...
argument|);
comment|/* set remote DMA write */
argument|outb(sc->nic_addr + ED_P0_CR, ED_CR_RD1 | ED_CR_STA);  	mb_offset =
literal|0
argument|;
...
argument|while (m) { 		if (m->m_len - mb_offset) { 			if (sc->isa16bit) { 				if ((m->m_len - mb_offset)>
literal|1
argument|) 					outsw(sc->asic_addr + ED_NOVELL_DATA, 					      mtod(m, caddr_t) + mb_offset, 					      (m->m_len - mb_offset) /
literal|2
argument|);
comment|/* 				 * if odd number of bytes, get the odd byte 				 * from the next mbuf with data 				 */
argument|if ((m->m_len - mb_offset)&
literal|1
argument|) {
comment|/* first the last byte in current mbuf */
argument|residual[
literal|0
argument|] = *(mtod(m, caddr_t) + 							m->m_len -
literal|1
argument|);
comment|/* advance past any empty mbufs */
argument|while (m->m_next&& (m->m_next->m_len ==
literal|0
argument|)) 						m = m->m_next;  					if (m->m_next) {
comment|/* 						 * remove first byte in next 						 * mbuf 						 */
argument|residual[
literal|1
argument|] = *(mtod(m->m_next, caddr_t)); 						mb_offset =
literal|1
argument|; 					} 					outw(sc->asic_addr + ED_NOVELL_DATA, 					     *((unsigned short *) residual)); 				} else 					mb_offset =
literal|0
argument|; 			} else 				outsb(sc->asic_addr + ED_NOVELL_DATA, m->m_data, m->m_len);  		} 		m = m->m_next; 	}
comment|/* 	 * Wait for remote DMA complete. This is necessary because on the 	 * transmit side, data is handled internally by the NIC in bursts and 	 * we can't start another remote DMA until this one completes. Not 	 * waiting causes really bad things to happen - like the NIC 	 * irrecoverably jamming the ISA bus. 	 */
argument|while (((inb(sc->nic_addr + ED_P0_ISR)& ED_ISR_RDC) != ED_ISR_RDC)&& --maxwait);  	if (!maxwait) { 		log(LOG_WARNING,
literal|"ed%d: remote transmit DMA failed to complete\n"
argument|, 		    sc->arpcom.ac_if.if_unit); 		ed_reset(sc->arpcom.ac_if.if_unit); 	} 	return (len); }
...
argument|static inline char * ed_ring_copy(sc, src, dst, amount) 	struct ed_softc *sc; 	char   *src; 	char   *dst; 	u_short amount; { 	u_short tmp_amount;
comment|/* does copy wrap to lower addr in ring buffer? */
argument|if (src + amount> sc->mem_end) { 		tmp_amount = sc->mem_end - src;
comment|/* copy amount up to end of NIC memory */
argument|if (sc->mem_shared) 			bcopy(src, dst, tmp_amount); 		else 			ed_pio_readmem(sc, src, dst, tmp_amount);  		amount -= tmp_amount; 		src = sc->mem_ring; 		dst += tmp_amount; 	} 	if (sc->mem_shared) 		bcopy(src, dst, amount); 	else 		ed_pio_readmem(sc, src, dst, amount);  	return (src + amount); }
...
argument|struct mbuf * ed_ring_to_mbuf(sc, src, dst, total_len) 	struct ed_softc *sc; 	char   *src; 	struct mbuf *dst; 	u_short total_len; { 	register struct mbuf *m = dst;  	while (total_len) { 		register u_short amount = min(total_len, M_TRAILINGSPACE(m));  		if (amount ==
literal|0
argument|) {
...
argument|dst = m; 			MGET(m, M_DONTWAIT, MT_DATA); 			if (m ==
literal|0
argument|) 				return (
literal|0
argument|);  			if (total_len>= MINCLSIZE) 				MCLGET(m, M_DONTWAIT);  			m->m_len =
literal|0
argument|; 			dst->m_next = m; 			amount = min(total_len, M_TRAILINGSPACE(m)); 		} 		src = ed_ring_copy(sc, src, mtod(m, caddr_t) + m->m_len, amount);  		m->m_len += amount; 		total_len -= amount;  	} 	return (m); }  void ed_setrcr(ifp, sc) 	struct ifnet *ifp; 	struct ed_softc *sc; { 	int     i;
comment|/* set page 1 registers */
argument|if (sc->is790) { 		outb(sc->nic_addr + ED_P0_CR, ED_CR_PAGE_1 | ED_CR_STP); 	} else { 		outb(sc->nic_addr + ED_P0_CR, ED_CR_PAGE_1 | ED_CR_RD2 | ED_CR_STP); 	}  	if (ifp->if_flags& IFF_PROMISC) {
comment|/* 		 * Reconfigure the multicast filter. 		 */
argument|for (i =
literal|0
argument|; i<
literal|8
argument|; i++) 			outb(sc->nic_addr + ED_P1_MAR0 + i,
literal|0xff
argument|);
...
argument|if (sc->is790) { 			outb(sc->nic_addr + ED_P0_CR, ED_CR_STP); 		} else { 			outb(sc->nic_addr + ED_P0_CR, ED_CR_RD2 | ED_CR_STP); 		} 		outb(sc->nic_addr + ED_P0_RCR, ED_RCR_PRO | ED_RCR_AM | 		     ED_RCR_AB | ED_RCR_AR | ED_RCR_SEP); 	} else {
ifndef|#
directive|ifndef
name|MULTICAST
...
argument|);
comment|/* Set page 0 registers */
argument|if (sc->is790) { 			outb(sc->nic_addr + ED_P0_CR, ED_CR_STP); 		} else { 			outb(sc->nic_addr + ED_P0_CR, ED_CR_RD2 | ED_CR_STP); 		}  		outb(sc->nic_addr + ED_P0_RCR, ED_RCR_AB);
else|#
directive|else
comment|/* set up multicast addresses and filter modes */
argument|if (ifp->if_flags& IFF_MULTICAST) { 			u_long  mcaf[
literal|2
argument|];  			if (ifp->if_flags& IFF_ALLMULTI) { 				mcaf[
literal|0
argument|] =
literal|0xffffffff
argument|; 				mcaf[
...
argument|] =
literal|0xffffffff
argument|; 			} else 				ds_getmcaf(sc, mcaf);
comment|/* 			 * Set multicast filter on chip. 			 */
argument|for (i =
literal|0
argument|; i<
literal|8
argument|; i++) 				outb(sc->nic_addr + ED_P1_MAR0 + i, ((u_char *) mcaf)[i]);
comment|/* Set page 0 registers */
argument|if (sc->is790) { 				outb(sc->nic_addr + ED_P0_CR, ED_CR_STP); 			} else { 				outb(sc->nic_addr + ED_P0_CR, ED_CR_RD2 | ED_CR_STP); 			}  			outb(sc->nic_addr + ED_P0_RCR, ED_RCR_AM | ED_RCR_AB); 		} else {
comment|/* 			 * Initialize multicast address hashing registers to 			 * not accept multicasts. 			 */
argument|for (i =
literal|0
argument|; i<
...
argument|u_long ds_crc(ep) 	u_char *ep; {
define|#
directive|define
name|POLYNOMIAL
value|0x04c11db6
argument|register u_long crc =
literal|0xffffffffL
argument|; 	register int carry
argument_list|,
argument|i
argument_list|,
argument|j; 	register u_char b;  	for (i =
literal|6
argument|; --i>=
literal|0
argument|;) { 		b = *ep++; 		for (j =
...
argument|; 			b>>=
literal|1
argument|; 			if (carry) 				crc = ((crc ^ POLYNOMIAL) | carry); 		} 	} 	return crc;
undef|#
directive|undef
name|POLYNOMIAL
argument|}
comment|/*  * Compute the multicast address filter from the  * list of multicast addresses we need to listen to.  */
argument|void ds_getmcaf(sc, mcaf) 	struct ed_softc *sc; 	u_long *mcaf; { 	register u_int index; 	register u_char *af = (u_char *) mcaf; 	register struct ether_multi *enm; 	register struct ether_multistep step;  	mcaf[
literal|0
argument|] =
literal|0
argument|; 	mcaf[
...
argument|] =
literal|0
argument|;  	ETHER_FIRST_MULTI(step,&sc->arpcom, enm); 	while (enm != NULL) { 		if (bcmp(enm->enm_addrlo, enm->enm_addrhi,
literal|6
argument|) !=
literal|0
argument|) { 			mcaf[
...
argument|] =
literal|0xffffffff
argument|; 			return; 		} 		index = ds_crc(enm->enm_addrlo,
literal|6
argument|)>>
literal|26
argument|; 		af[index>>
...
argument|<< (index&
literal|7
argument|);  		ETHER_NEXT_MULTI(step, enm); 	} }
end_function
