...
block|{
comment|/*              * Allow access to the field if it is within the region size              * rounded up to a multiple of the access byte width.  This              * overcomes "off-by-one" programming errors in the AML often              * found in Toshiba laptops.  These errors were allowed by              * the Microsoft ASL compiler.              */
name|UINT32
name|rounded_length
init|=
name|ACPI_ROUND_UP
...
begin_comment
comment|/*******************************************************************************  *  * FUNCTION:    AcpiExCommonBufferSetup  *  * PARAMETERS:  ObjDesc             - Field object  *              BufferLength        - Length of caller's buffer  *              DatumCount          - Where the DatumCount is returned  *  * RETURN:      Status, DatumCount  *  * DESCRIPTION: Common code to validate the incoming buffer size and compute  *              the number of field "datums" that must be read or written.  *              A "datum" is the smallest unit that can be read or written  *              to the field, it is either 1,2,4, or 8 bytes.  *  ******************************************************************************/
end_comment
...
name|ACPI_FUNCTION_TRACE
argument_list|(
literal|"ExCommonBufferSetup"
argument_list|)
expr_stmt|;
comment|/*      * Incoming buffer must be at least as long as the field, we do not      * allow "partial" field reads/writes.  We do not care if the buffer is      * larger than the field, this typically happens when an integer is      * read/written to a field that is actually smaller than an integer.      */
name|ByteFieldLength
operator|=
...
name|ACPI_DB_BFIELD
operator|,
literal|"Field size %X (bytes) is too large for buffer (%X)\n"
operator|,
name|ByteFieldLength
operator|,
name|BufferLength
...
name|ACPI_DB_BFIELD
operator|,
literal|"BufferBytes %X, ActualBytes %X, Datums %X, ByteGran %X\n"
operator|,
name|ByteFieldLength
operator|,
name|ActualByteFieldLength
...
argument_list|)
expr_stmt|;
comment|/* Validate buffer, compute number of datums */
name|Status
operator|=
name|AcpiExCommonBufferSetup
argument_list|(
