begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/******************************************************************************  *  * Module Name: psutils - Parser miscellaneous utilities (Parser only)  *              $Revision: 38 $  *  *****************************************************************************/
end_comment

begin_comment
comment|/******************************************************************************  *  * 1. Copyright Notice  *  * Some or all of this work - Copyright (c) 1999, 2000, 2001, Intel Corp.  * All rights reserved.  *  * 2. License  *  * 2.1. This is your license from Intel Corp. under its intellectual property  * rights.  You may have additional license terms from the party that provided  * you this software, covering your right to use that party's intellectual  * property rights.  *  * 2.2. Intel grants, free of charge, to any person ("Licensee") obtaining a  * copy of the source code appearing in this file ("Covered Code") an  * irrevocable, perpetual, worldwide license under Intel's copyrights in the  * base code distributed originally by Intel ("Original Intel Code") to copy,  * make derivatives, distribute, use and display any portion of the Covered  * Code in any form, with the right to sublicense such rights; and  *  * 2.3. Intel grants Licensee a non-exclusive and non-transferable patent  * license (with the right to sublicense), under only those claims of Intel  * patents that are infringed by the Original Intel Code, to make, use, sell,  * offer to sell, and import the Covered Code and derivative works thereof  * solely to the minimum extent necessary to exercise the above copyright  * license, and in no event shall the patent license extend to any additions  * to or modifications of the Original Intel Code.  No other license or right  * is granted directly or by implication, estoppel or otherwise;  *  * The above copyright and patent license is granted only if the following  * conditions are met:  *  * 3. Conditions  *  * 3.1. Redistribution of Source with Rights to Further Distribute Source.  * Redistribution of source code of any substantial portion of the Covered  * Code or modification with rights to further distribute source must include  * the above Copyright Notice, the above License, this list of Conditions,  * and the following Disclaimer and Export Compliance provision.  In addition,  * Licensee must cause all Covered Code to which Licensee contributes to  * contain a file documenting the changes Licensee made to create that Covered  * Code and the date of any change.  Licensee must include in that file the  * documentation of any changes made by any predecessor Licensee.  Licensee  * must include a prominent statement that the modification is derived,  * directly or indirectly, from Original Intel Code.  *  * 3.2. Redistribution of Source with no Rights to Further Distribute Source.  * Redistribution of source code of any substantial portion of the Covered  * Code or modification without rights to further distribute source must  * include the following Disclaimer and Export Compliance provision in the  * documentation and/or other materials provided with distribution.  In  * addition, Licensee may not authorize further sublicense of source of any  * portion of the Covered Code, and must include terms to the effect that the  * license from Licensee to its licensee is limited to the intellectual  * property embodied in the software Licensee provides to its licensee, and  * not to intellectual property embodied in modifications its licensee may  * make.  *  * 3.3. Redistribution of Executable. Redistribution in executable form of any  * substantial portion of the Covered Code or modification must reproduce the  * above Copyright Notice, and the following Disclaimer and Export Compliance  * provision in the documentation and/or other materials provided with the  * distribution.  *  * 3.4. Intel retains all right, title, and interest in and to the Original  * Intel Code.  *  * 3.5. Neither the name Intel nor any other trademark owned or controlled by  * Intel shall be used in advertising or otherwise to promote the sale, use or  * other dealings in products derived from or relating to the Covered Code  * without prior written authorization from Intel.  *  * 4. Disclaimer and Export Compliance  *  * 4.1. INTEL MAKES NO WARRANTY OF ANY KIND REGARDING ANY SOFTWARE PROVIDED  * HERE.  ANY SOFTWARE ORIGINATING FROM INTEL OR DERIVED FROM INTEL SOFTWARE  * IS PROVIDED "AS IS," AND INTEL WILL NOT PROVIDE ANY SUPPORT,  ASSISTANCE,  * INSTALLATION, TRAINING OR OTHER SERVICES.  INTEL WILL NOT PROVIDE ANY  * UPDATES, ENHANCEMENTS OR EXTENSIONS.  INTEL SPECIFICALLY DISCLAIMS ANY  * IMPLIED WARRANTIES OF MERCHANTABILITY, NONINFRINGEMENT AND FITNESS FOR A  * PARTICULAR PURPOSE.  *  * 4.2. IN NO EVENT SHALL INTEL HAVE ANY LIABILITY TO LICENSEE, ITS LICENSEES  * OR ANY OTHER THIRD PARTY, FOR ANY LOST PROFITS, LOST DATA, LOSS OF USE OR  * COSTS OF PROCUREMENT OF SUBSTITUTE GOODS OR SERVICES, OR FOR ANY INDIRECT,  * SPECIAL OR CONSEQUENTIAL DAMAGES ARISING OUT OF THIS AGREEMENT, UNDER ANY  * CAUSE OF ACTION OR THEORY OF LIABILITY, AND IRRESPECTIVE OF WHETHER INTEL  * HAS ADVANCE NOTICE OF THE POSSIBILITY OF SUCH DAMAGES.  THESE LIMITATIONS  * SHALL APPLY NOTWITHSTANDING THE FAILURE OF THE ESSENTIAL PURPOSE OF ANY  * LIMITED REMEDY.  *  * 4.3. Licensee shall not export, either directly or indirectly, any of this  * software or system incorporating such software without first obtaining any  * required license or other approval from the U. S. Department of Commerce or  * any other agency or department of the United States Government.  In the  * event Licensee exports any such software from the United States or  * re-exports any such software from a foreign destination, Licensee shall  * ensure that the distribution and export/re-export of the software is in  * compliance with all laws, regulations, orders, or other restrictions of the  * U.S. Export Administration Regulations. Licensee agrees that neither it nor  * any of its subsidiaries will export/re-export any technical data, process,  * software, or service, directly or indirectly, to any country for which the  * United States government or any agency thereof requires an export license,  * other governmental approval, or letter of assurance, without first obtaining  * such license, approval or letter.  *  *****************************************************************************/
end_comment

begin_include
include|#
directive|include
file|"acpi.h"
end_include

begin_include
include|#
directive|include
file|"acparser.h"
end_include

begin_include
include|#
directive|include
file|"amlcode.h"
end_include

begin_define
define|#
directive|define
name|_COMPONENT
value|ACPI_PARSER
end_define

begin_macro
name|MODULE_NAME
argument_list|(
literal|"psutils"
argument_list|)
end_macro

begin_define
define|#
directive|define
name|PARSEOP_GENERIC
value|0x01
end_define

begin_define
define|#
directive|define
name|PARSEOP_NAMED
value|0x02
end_define

begin_define
define|#
directive|define
name|PARSEOP_DEFERRED
value|0x03
end_define

begin_define
define|#
directive|define
name|PARSEOP_BYTELIST
value|0x04
end_define

begin_define
define|#
directive|define
name|PARSEOP_IN_CACHE
value|0x80
end_define

begin_comment
comment|/*******************************************************************************  *  * FUNCTION:    AcpiPsInitOp  *  * PARAMETERS:  Op              - A newly allocated Op object  *              Opcode          - Opcode to store in the Op  *  * RETURN:      Status  *  * DESCRIPTION: Allocate an acpi_op, choose op type (and thus size) based on  *              opcode  *  ******************************************************************************/
end_comment

begin_function
name|void
name|AcpiPsInitOp
parameter_list|(
name|ACPI_PARSE_OBJECT
modifier|*
name|Op
parameter_list|,
name|UINT16
name|Opcode
parameter_list|)
block|{
name|ACPI_OPCODE_INFO
modifier|*
name|AmlOp
decl_stmt|;
name|Op
operator|->
name|DataType
operator|=
name|ACPI_DESC_TYPE_PARSER
expr_stmt|;
name|Op
operator|->
name|Opcode
operator|=
name|Opcode
expr_stmt|;
name|AmlOp
operator|=
name|AcpiPsGetOpcodeInfo
argument_list|(
name|Opcode
argument_list|)
expr_stmt|;
name|DEBUG_ONLY_MEMBERS
argument_list|(
name|STRNCPY
argument_list|(
name|Op
operator|->
name|OpName
argument_list|,
name|AmlOp
operator|->
name|Name
argument_list|,
sizeof|sizeof
argument_list|(
name|Op
operator|->
name|OpName
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*******************************************************************************  *  * FUNCTION:    AcpiPsAllocOp  *  * PARAMETERS:  Opcode          - Opcode that will be stored in the new Op  *  * RETURN:      Pointer to the new Op.  *  * DESCRIPTION: Allocate an acpi_op, choose op type (and thus size) based on  *              opcode.  A cache of opcodes is available for the pure  *              GENERIC_OP, since this is by far the most commonly used.  *  ******************************************************************************/
end_comment

begin_function
name|ACPI_PARSE_OBJECT
modifier|*
name|AcpiPsAllocOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
name|ACPI_PARSE_OBJECT
modifier|*
name|Op
init|=
name|NULL
decl_stmt|;
name|UINT32
name|Size
decl_stmt|;
name|UINT8
name|Flags
decl_stmt|;
name|PROC_NAME
argument_list|(
literal|"PsAllocOp"
argument_list|)
expr_stmt|;
comment|/* Allocate the minimum required size object */
if|if
condition|(
name|AcpiPsIsDeferredOp
argument_list|(
name|Opcode
argument_list|)
condition|)
block|{
name|Size
operator|=
sizeof|sizeof
argument_list|(
name|ACPI_PARSE2_OBJECT
argument_list|)
expr_stmt|;
name|Flags
operator|=
name|PARSEOP_DEFERRED
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|AcpiPsIsNamedOp
argument_list|(
name|Opcode
argument_list|)
condition|)
block|{
name|Size
operator|=
sizeof|sizeof
argument_list|(
name|ACPI_PARSE2_OBJECT
argument_list|)
expr_stmt|;
name|Flags
operator|=
name|PARSEOP_NAMED
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|AcpiPsIsBytelistOp
argument_list|(
name|Opcode
argument_list|)
condition|)
block|{
name|Size
operator|=
sizeof|sizeof
argument_list|(
name|ACPI_PARSE2_OBJECT
argument_list|)
expr_stmt|;
name|Flags
operator|=
name|PARSEOP_BYTELIST
expr_stmt|;
block|}
else|else
block|{
name|Size
operator|=
sizeof|sizeof
argument_list|(
name|ACPI_PARSE_OBJECT
argument_list|)
expr_stmt|;
name|Flags
operator|=
name|PARSEOP_GENERIC
expr_stmt|;
block|}
if|if
condition|(
name|Size
operator|==
sizeof|sizeof
argument_list|(
name|ACPI_PARSE_OBJECT
argument_list|)
condition|)
block|{
comment|/*          * The generic op is by far the most common (16 to 1), and therefore          * the op cache is implemented with this type.          *          * Check if there is an Op already available in the cache          */
name|AcpiUtAcquireMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
name|AcpiGbl_ParseCacheRequests
operator|++
expr_stmt|;
if|if
condition|(
name|AcpiGbl_ParseCache
condition|)
block|{
comment|/* Extract an op from the front of the cache list */
name|AcpiGbl_ParseCacheDepth
operator|--
expr_stmt|;
name|AcpiGbl_ParseCacheHits
operator|++
expr_stmt|;
name|Op
operator|=
name|AcpiGbl_ParseCache
expr_stmt|;
name|AcpiGbl_ParseCache
operator|=
name|Op
operator|->
name|Next
expr_stmt|;
if|if
condition|(
name|Op
operator|->
name|DataType
operator|==
literal|0xFF
condition|)
block|{
name|DEBUG_PRINTP
argument_list|(
name|ACPI_ERROR
argument_list|,
operator|(
literal|"Op %p deleted while in cache!\n"
operator|,
name|Op
operator|)
argument_list|)
expr_stmt|;
block|}
comment|/* Clear the previously used Op */
name|MEMSET
argument_list|(
name|Op
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_PARSE_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
name|DEBUG_PRINTP
argument_list|(
name|TRACE_PARSE
argument_list|,
operator|(
literal|"Op %p from Parse Cache\n"
operator|,
name|Op
operator|)
argument_list|)
expr_stmt|;
block|}
name|AcpiUtReleaseMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/*          * The generic op is by far the most common (16 to 1), and therefore          * the op cache is implemented with this type.          *          * Check if there is an Op already available in the cache          */
name|AcpiUtAcquireMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
name|AcpiGbl_ExtParseCacheRequests
operator|++
expr_stmt|;
if|if
condition|(
name|AcpiGbl_ExtParseCache
condition|)
block|{
comment|/* Extract an op from the front of the cache list */
name|AcpiGbl_ExtParseCacheDepth
operator|--
expr_stmt|;
name|AcpiGbl_ExtParseCacheHits
operator|++
expr_stmt|;
name|Op
operator|=
operator|(
name|ACPI_PARSE_OBJECT
operator|*
operator|)
name|AcpiGbl_ExtParseCache
expr_stmt|;
name|AcpiGbl_ExtParseCache
operator|=
operator|(
name|ACPI_PARSE2_OBJECT
operator|*
operator|)
name|Op
operator|->
name|Next
expr_stmt|;
if|if
condition|(
name|Op
operator|->
name|DataType
operator|==
literal|0xFF
condition|)
block|{
name|DEBUG_PRINTP
argument_list|(
name|ACPI_ERROR
argument_list|,
operator|(
literal|"Op %p deleted while in cache!\n"
operator|,
name|Op
operator|)
argument_list|)
expr_stmt|;
block|}
comment|/* Clear the previously used Op */
name|MEMSET
argument_list|(
name|Op
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_PARSE2_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
name|DEBUG_PRINTP
argument_list|(
name|TRACE_PARSE
argument_list|,
operator|(
literal|"Op %p from ExtParse Cache\n"
operator|,
name|Op
operator|)
argument_list|)
expr_stmt|;
block|}
name|AcpiUtReleaseMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
block|}
comment|/* Allocate a new Op if necessary */
if|if
condition|(
operator|!
name|Op
condition|)
block|{
name|Op
operator|=
name|ACPI_MEM_CALLOCATE
argument_list|(
name|Size
argument_list|)
expr_stmt|;
block|}
comment|/* Initialize the Op */
if|if
condition|(
name|Op
condition|)
block|{
name|AcpiPsInitOp
argument_list|(
name|Op
argument_list|,
name|Opcode
argument_list|)
expr_stmt|;
name|Op
operator|->
name|Flags
operator|=
name|Flags
expr_stmt|;
block|}
return|return
operator|(
name|Op
operator|)
return|;
block|}
end_function

begin_comment
comment|/*******************************************************************************  *  * FUNCTION:    AcpiPsFreeOp  *  * PARAMETERS:  Op              - Op to be freed  *  * RETURN:      None.  *  * DESCRIPTION: Free an Op object.  Either put it on the GENERIC_OP cache list  *              or actually free it.  *  ******************************************************************************/
end_comment

begin_function
name|void
name|AcpiPsFreeOp
parameter_list|(
name|ACPI_PARSE_OBJECT
modifier|*
name|Op
parameter_list|)
block|{
name|PROC_NAME
argument_list|(
literal|"PsFreeOp"
argument_list|)
expr_stmt|;
if|if
condition|(
name|Op
operator|->
name|Opcode
operator|==
name|AML_INT_RETURN_VALUE_OP
condition|)
block|{
name|DEBUG_PRINTP
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Free retval op: %p\n"
operator|,
name|Op
operator|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|Op
operator|->
name|Flags
operator|==
name|PARSEOP_GENERIC
condition|)
block|{
comment|/* Is the cache full? */
if|if
condition|(
name|AcpiGbl_ParseCacheDepth
operator|<
name|MAX_PARSE_CACHE_DEPTH
condition|)
block|{
comment|/* Put a GENERIC_OP back into the cache */
comment|/* Clear the previously used Op */
name|MEMSET
argument_list|(
name|Op
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_PARSE_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
name|Op
operator|->
name|Flags
operator|=
name|PARSEOP_IN_CACHE
expr_stmt|;
name|AcpiUtAcquireMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
name|AcpiGbl_ParseCacheDepth
operator|++
expr_stmt|;
name|Op
operator|->
name|Next
operator|=
name|AcpiGbl_ParseCache
expr_stmt|;
name|AcpiGbl_ParseCache
operator|=
name|Op
expr_stmt|;
name|AcpiUtReleaseMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
else|else
block|{
comment|/* Is the cache full? */
if|if
condition|(
name|AcpiGbl_ExtParseCacheDepth
operator|<
name|MAX_EXTPARSE_CACHE_DEPTH
condition|)
block|{
comment|/* Put a GENERIC_OP back into the cache */
comment|/* Clear the previously used Op */
name|MEMSET
argument_list|(
name|Op
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_PARSE2_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
name|Op
operator|->
name|Flags
operator|=
name|PARSEOP_IN_CACHE
expr_stmt|;
name|AcpiUtAcquireMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
name|AcpiGbl_ExtParseCacheDepth
operator|++
expr_stmt|;
name|Op
operator|->
name|Next
operator|=
operator|(
name|ACPI_PARSE_OBJECT
operator|*
operator|)
name|AcpiGbl_ExtParseCache
expr_stmt|;
name|AcpiGbl_ExtParseCache
operator|=
operator|(
name|ACPI_PARSE2_OBJECT
operator|*
operator|)
name|Op
expr_stmt|;
name|AcpiUtReleaseMutex
argument_list|(
name|ACPI_MTX_CACHES
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
comment|/*      * Not a GENERIC OP, or the cache is full, just free the Op      */
name|ACPI_MEM_FREE
argument_list|(
name|Op
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*******************************************************************************  *  * FUNCTION:    AcpiPsDeleteParseCache  *  * PARAMETERS:  None  *  * RETURN:      None  *  * DESCRIPTION: Free all objects that are on the parse cache list.  *  ******************************************************************************/
end_comment

begin_function
name|void
name|AcpiPsDeleteParseCache
parameter_list|(
name|void
parameter_list|)
block|{
name|ACPI_PARSE_OBJECT
modifier|*
name|Next
decl_stmt|;
name|FUNCTION_TRACE
argument_list|(
literal|"PsDeleteParseCache"
argument_list|)
expr_stmt|;
comment|/* Traverse the global cache list */
while|while
condition|(
name|AcpiGbl_ParseCache
condition|)
block|{
comment|/* Delete one cached state object */
name|Next
operator|=
name|AcpiGbl_ParseCache
operator|->
name|Next
expr_stmt|;
name|ACPI_MEM_FREE
argument_list|(
name|AcpiGbl_ParseCache
argument_list|)
expr_stmt|;
name|AcpiGbl_ParseCache
operator|=
name|Next
expr_stmt|;
name|AcpiGbl_ParseCacheDepth
operator|--
expr_stmt|;
block|}
comment|/* Traverse the global cache list */
while|while
condition|(
name|AcpiGbl_ExtParseCache
condition|)
block|{
comment|/* Delete one cached state object */
name|Next
operator|=
name|AcpiGbl_ExtParseCache
operator|->
name|Next
expr_stmt|;
name|ACPI_MEM_FREE
argument_list|(
name|AcpiGbl_ExtParseCache
argument_list|)
expr_stmt|;
name|AcpiGbl_ExtParseCache
operator|=
operator|(
name|ACPI_PARSE2_OBJECT
operator|*
operator|)
name|Next
expr_stmt|;
name|AcpiGbl_ExtParseCacheDepth
operator|--
expr_stmt|;
block|}
name|return_VOID
expr_stmt|;
block|}
end_function

begin_comment
comment|/*******************************************************************************  *  * FUNCTION:    Utility functions  *  * DESCRIPTION: Low level functions  *  * TBD: [Restructure]  * 1) Some of these functions should be macros  * 2) Some can be simplified  *  ******************************************************************************/
end_comment

begin_comment
comment|/*  * Is "c" a namestring lead character?  */
end_comment

begin_function
name|BOOLEAN
name|AcpiPsIsLeadingChar
parameter_list|(
name|UINT32
name|c
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|c
operator|==
literal|'_'
operator|||
operator|(
name|c
operator|>=
literal|'A'
operator|&&
name|c
operator|<=
literal|'Z'
operator|)
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Is "c" a namestring prefix character?  */
end_comment

begin_function
name|BOOLEAN
name|AcpiPsIsPrefixChar
parameter_list|(
name|UINT32
name|c
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|c
operator|==
literal|'\\'
operator|||
name|c
operator|==
literal|'^'
argument_list|)
operator|)
return|;
block|}
end_function

begin_function
name|BOOLEAN
name|AcpiPsIsNamespaceObjectOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_SCOPE_OP
operator|||
name|Opcode
operator|==
name|AML_DEVICE_OP
operator|||
name|Opcode
operator|==
name|AML_THERMAL_ZONE_OP
operator|||
name|Opcode
operator|==
name|AML_METHOD_OP
operator|||
name|Opcode
operator|==
name|AML_POWER_RES_OP
operator|||
name|Opcode
operator|==
name|AML_PROCESSOR_OP
operator|||
name|Opcode
operator|==
name|AML_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_INDEX_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_BANK_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_INT_NAMEDFIELD_OP
operator|||
name|Opcode
operator|==
name|AML_NAME_OP
operator|||
name|Opcode
operator|==
name|AML_ALIAS_OP
operator|||
name|Opcode
operator|==
name|AML_MUTEX_OP
operator|||
name|Opcode
operator|==
name|AML_EVENT_OP
operator|||
name|Opcode
operator|==
name|AML_REGION_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BIT_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BYTE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_WORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_DWORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_QWORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_INT_METHODCALL_OP
operator|||
name|Opcode
operator|==
name|AML_INT_NAMEPATH_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_function
name|BOOLEAN
name|AcpiPsIsNamespaceOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_SCOPE_OP
operator|||
name|Opcode
operator|==
name|AML_DEVICE_OP
operator|||
name|Opcode
operator|==
name|AML_THERMAL_ZONE_OP
operator|||
name|Opcode
operator|==
name|AML_METHOD_OP
operator|||
name|Opcode
operator|==
name|AML_POWER_RES_OP
operator|||
name|Opcode
operator|==
name|AML_PROCESSOR_OP
operator|||
name|Opcode
operator|==
name|AML_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_INDEX_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_BANK_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_NAME_OP
operator|||
name|Opcode
operator|==
name|AML_ALIAS_OP
operator|||
name|Opcode
operator|==
name|AML_MUTEX_OP
operator|||
name|Opcode
operator|==
name|AML_EVENT_OP
operator|||
name|Opcode
operator|==
name|AML_REGION_OP
operator|||
name|Opcode
operator|==
name|AML_INT_NAMEDFIELD_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Is opcode for a named object Op?  * (Includes all named object opcodes)  *  * TBD: [Restructure] Need a better way than this brute force approach!  */
end_comment

begin_function
name|BOOLEAN
name|AcpiPsIsNodeOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_SCOPE_OP
operator|||
name|Opcode
operator|==
name|AML_DEVICE_OP
operator|||
name|Opcode
operator|==
name|AML_THERMAL_ZONE_OP
operator|||
name|Opcode
operator|==
name|AML_METHOD_OP
operator|||
name|Opcode
operator|==
name|AML_POWER_RES_OP
operator|||
name|Opcode
operator|==
name|AML_PROCESSOR_OP
operator|||
name|Opcode
operator|==
name|AML_INT_NAMEDFIELD_OP
operator|||
name|Opcode
operator|==
name|AML_NAME_OP
operator|||
name|Opcode
operator|==
name|AML_ALIAS_OP
operator|||
name|Opcode
operator|==
name|AML_MUTEX_OP
operator|||
name|Opcode
operator|==
name|AML_EVENT_OP
operator|||
name|Opcode
operator|==
name|AML_REGION_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BIT_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BYTE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_WORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_DWORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_QWORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_INT_METHODCALL_OP
operator|||
name|Opcode
operator|==
name|AML_INT_NAMEPATH_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Is opcode for a named Op?  */
end_comment

begin_function
name|BOOLEAN
name|AcpiPsIsNamedOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_SCOPE_OP
operator|||
name|Opcode
operator|==
name|AML_DEVICE_OP
operator|||
name|Opcode
operator|==
name|AML_THERMAL_ZONE_OP
operator|||
name|Opcode
operator|==
name|AML_METHOD_OP
operator|||
name|Opcode
operator|==
name|AML_POWER_RES_OP
operator|||
name|Opcode
operator|==
name|AML_PROCESSOR_OP
operator|||
name|Opcode
operator|==
name|AML_NAME_OP
operator|||
name|Opcode
operator|==
name|AML_ALIAS_OP
operator|||
name|Opcode
operator|==
name|AML_MUTEX_OP
operator|||
name|Opcode
operator|==
name|AML_EVENT_OP
operator|||
name|Opcode
operator|==
name|AML_REGION_OP
operator|||
name|Opcode
operator|==
name|AML_INT_NAMEDFIELD_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_function
name|BOOLEAN
name|AcpiPsIsDeferredOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_METHOD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BIT_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BYTE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_WORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_DWORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_QWORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_REGION_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Is opcode for a bytelist?  */
end_comment

begin_function
name|BOOLEAN
name|AcpiPsIsBytelistOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_INT_BYTELIST_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Is opcode for a Field, IndexField, or BankField  */
end_comment

begin_function
name|BOOLEAN
name|AcpiPsIsFieldOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_CREATE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_INDEX_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_BANK_FIELD_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Is field creation op  */
end_comment

begin_function
name|BOOLEAN
name|AcpiPsIsCreateFieldOp
parameter_list|(
name|UINT16
name|Opcode
parameter_list|)
block|{
return|return
operator|(
call|(
name|BOOLEAN
call|)
argument_list|(
name|Opcode
operator|==
name|AML_CREATE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BIT_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_BYTE_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_WORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_DWORD_FIELD_OP
operator|||
name|Opcode
operator|==
name|AML_CREATE_QWORD_FIELD_OP
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Cast an acpi_op to an acpi_extended_op if possible  */
end_comment

begin_comment
comment|/* TBD: This is very inefficient, fix */
end_comment

begin_function
name|ACPI_PARSE2_OBJECT
modifier|*
name|AcpiPsToExtendedOp
parameter_list|(
name|ACPI_PARSE_OBJECT
modifier|*
name|Op
parameter_list|)
block|{
return|return
operator|(
operator|(
name|AcpiPsIsDeferredOp
argument_list|(
name|Op
operator|->
name|Opcode
argument_list|)
operator|||
name|AcpiPsIsNamedOp
argument_list|(
name|Op
operator|->
name|Opcode
argument_list|)
operator|||
name|AcpiPsIsBytelistOp
argument_list|(
name|Op
operator|->
name|Opcode
argument_list|)
operator|)
condition|?
operator|(
operator|(
name|ACPI_PARSE2_OBJECT
operator|*
operator|)
name|Op
operator|)
else|:
name|NULL
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Get op's name (4-byte name segment) or 0 if unnamed  */
end_comment

begin_function
name|UINT32
name|AcpiPsGetName
parameter_list|(
name|ACPI_PARSE_OBJECT
modifier|*
name|Op
parameter_list|)
block|{
name|ACPI_PARSE2_OBJECT
modifier|*
name|Named
init|=
name|AcpiPsToExtendedOp
argument_list|(
name|Op
argument_list|)
decl_stmt|;
return|return
operator|(
name|Named
condition|?
name|Named
operator|->
name|Name
else|:
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Set op's name  */
end_comment

begin_function
name|void
name|AcpiPsSetName
parameter_list|(
name|ACPI_PARSE_OBJECT
modifier|*
name|Op
parameter_list|,
name|UINT32
name|name
parameter_list|)
block|{
name|ACPI_PARSE2_OBJECT
modifier|*
name|Named
init|=
name|AcpiPsToExtendedOp
argument_list|(
name|Op
argument_list|)
decl_stmt|;
if|if
condition|(
name|Named
condition|)
block|{
name|Named
operator|->
name|Name
operator|=
name|name
expr_stmt|;
block|}
block|}
end_function

end_unit

