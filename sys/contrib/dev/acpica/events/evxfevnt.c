...
condition|)
block|{
comment|/* The GPE must have the ability to wake the system */
if|if
condition|(
operator|!
operator|(
...
goto|;
block|}
comment|/*          * Update the enable mask on the first wakeup reference. Wake GPEs          * are only hardware-enabled just before sleeping.          */
name|GpeEventInfo
operator|->
name|WakeupCount
operator|++
...
goto|;
block|}
comment|/* Hardware-disable a runtime GPE on removal of the last reference */
if|if
condition|(
name|GpeType
operator|&
...
name|AE_LIMIT
expr_stmt|;
comment|/* There are no references to remove */
goto|goto
name|UnlockAndExit
goto|;
block|}
...
block|}
block|}
comment|/*      * Update masks for wake GPE on removal of the last reference.      * No need to hardware-disable wake GPEs here, they are not currently      * enabled.      */
if|if
condition|(
name|GpeType
operator|&
...
begin_comment
comment|/*******************************************************************************  *  * FUNCTION:    AcpiGetGpeStatus  *  * PARAMETERS:  GpeDevice       - Parent GPE Device. NULL for GPE0/GPE1  *              GpeNumber       - GPE level within the GPE block  *              EventStatus     - Where the current status of the event will  *                                be returned  *  * RETURN:      Status  *  * DESCRIPTION: Get status of an event (general purpose)  *  ******************************************************************************/
end_comment
...
name|GpeBlock
expr_stmt|;
comment|/* Run the _PRW methods and enable the runtime GPEs in the new block */
name|Status
operator|=
