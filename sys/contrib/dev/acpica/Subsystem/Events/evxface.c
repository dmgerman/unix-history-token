...
begin_comment
comment|/******************************************************************************  *  * Module Name: evxface - External interfaces for ACPI events  *              $Revision: 98 $  *  *****************************************************************************/
end_comment
...
goto|;
block|}
comment|/*      * Support for global notify handlers.  These handlers are invoked for      * every notifiy of the type specifiec      */
if|if
condition|(
name|Device
operator|==
...
block|}
block|}
comment|/*      *  If we get here, we know that there is no handler installed      *  so let's party      */
name|NotifyObj
operator|=
name|AcpiCmCreateInternalObject
argument_list|(
...
goto|;
block|}
comment|/*      *  The object exists.      *      *  Make sure the handler is installed.      */
if|if
condition|(
name|HandlerType
operator|==
...
goto|;
block|}
comment|/*      * Now we can remove the handler      */
if|if
condition|(
name|HandlerType
operator|==
