begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/******************************************************************************  *  * Module Name: amdump - Interpreter debug output routines  *              $Revision: 100 $  *  *****************************************************************************/
end_comment

begin_comment
comment|/******************************************************************************  *  * 1. Copyright Notice  *  * Some or all of this work - Copyright (c) 1999, 2000, 2001, Intel Corp.  * All rights reserved.  *  * 2. License  *  * 2.1. This is your license from Intel Corp. under its intellectual property  * rights.  You may have additional license terms from the party that provided  * you this software, covering your right to use that party's intellectual  * property rights.  *  * 2.2. Intel grants, free of charge, to any person ("Licensee") obtaining a  * copy of the source code appearing in this file ("Covered Code") an  * irrevocable, perpetual, worldwide license under Intel's copyrights in the  * base code distributed originally by Intel ("Original Intel Code") to copy,  * make derivatives, distribute, use and display any portion of the Covered  * Code in any form, with the right to sublicense such rights; and  *  * 2.3. Intel grants Licensee a non-exclusive and non-transferable patent  * license (with the right to sublicense), under only those claims of Intel  * patents that are infringed by the Original Intel Code, to make, use, sell,  * offer to sell, and import the Covered Code and derivative works thereof  * solely to the minimum extent necessary to exercise the above copyright  * license, and in no event shall the patent license extend to any additions  * to or modifications of the Original Intel Code.  No other license or right  * is granted directly or by implication, estoppel or otherwise;  *  * The above copyright and patent license is granted only if the following  * conditions are met:  *  * 3. Conditions  *  * 3.1. Redistribution of Source with Rights to Further Distribute Source.  * Redistribution of source code of any substantial portion of the Covered  * Code or modification with rights to further distribute source must include  * the above Copyright Notice, the above License, this list of Conditions,  * and the following Disclaimer and Export Compliance provision.  In addition,  * Licensee must cause all Covered Code to which Licensee contributes to  * contain a file documenting the changes Licensee made to create that Covered  * Code and the date of any change.  Licensee must include in that file the  * documentation of any changes made by any predecessor Licensee.  Licensee  * must include a prominent statement that the modification is derived,  * directly or indirectly, from Original Intel Code.  *  * 3.2. Redistribution of Source with no Rights to Further Distribute Source.  * Redistribution of source code of any substantial portion of the Covered  * Code or modification without rights to further distribute source must  * include the following Disclaimer and Export Compliance provision in the  * documentation and/or other materials provided with distribution.  In  * addition, Licensee may not authorize further sublicense of source of any  * portion of the Covered Code, and must include terms to the effect that the  * license from Licensee to its licensee is limited to the intellectual  * property embodied in the software Licensee provides to its licensee, and  * not to intellectual property embodied in modifications its licensee may  * make.  *  * 3.3. Redistribution of Executable. Redistribution in executable form of any  * substantial portion of the Covered Code or modification must reproduce the  * above Copyright Notice, and the following Disclaimer and Export Compliance  * provision in the documentation and/or other materials provided with the  * distribution.  *  * 3.4. Intel retains all right, title, and interest in and to the Original  * Intel Code.  *  * 3.5. Neither the name Intel nor any other trademark owned or controlled by  * Intel shall be used in advertising or otherwise to promote the sale, use or  * other dealings in products derived from or relating to the Covered Code  * without prior written authorization from Intel.  *  * 4. Disclaimer and Export Compliance  *  * 4.1. INTEL MAKES NO WARRANTY OF ANY KIND REGARDING ANY SOFTWARE PROVIDED  * HERE.  ANY SOFTWARE ORIGINATING FROM INTEL OR DERIVED FROM INTEL SOFTWARE  * IS PROVIDED "AS IS," AND INTEL WILL NOT PROVIDE ANY SUPPORT,  ASSISTANCE,  * INSTALLATION, TRAINING OR OTHER SERVICES.  INTEL WILL NOT PROVIDE ANY  * UPDATES, ENHANCEMENTS OR EXTENSIONS.  INTEL SPECIFICALLY DISCLAIMS ANY  * IMPLIED WARRANTIES OF MERCHANTABILITY, NONINFRINGEMENT AND FITNESS FOR A  * PARTICULAR PURPOSE.  *  * 4.2. IN NO EVENT SHALL INTEL HAVE ANY LIABILITY TO LICENSEE, ITS LICENSEES  * OR ANY OTHER THIRD PARTY, FOR ANY LOST PROFITS, LOST DATA, LOSS OF USE OR  * COSTS OF PROCUREMENT OF SUBSTITUTE GOODS OR SERVICES, OR FOR ANY INDIRECT,  * SPECIAL OR CONSEQUENTIAL DAMAGES ARISING OUT OF THIS AGREEMENT, UNDER ANY  * CAUSE OF ACTION OR THEORY OF LIABILITY, AND IRRESPECTIVE OF WHETHER INTEL  * HAS ADVANCE NOTICE OF THE POSSIBILITY OF SUCH DAMAGES.  THESE LIMITATIONS  * SHALL APPLY NOTWITHSTANDING THE FAILURE OF THE ESSENTIAL PURPOSE OF ANY  * LIMITED REMEDY.  *  * 4.3. Licensee shall not export, either directly or indirectly, any of this  * software or system incorporating such software without first obtaining any  * required license or other approval from the U. S. Department of Commerce or  * any other agency or department of the United States Government.  In the  * event Licensee exports any such software from the United States or  * re-exports any such software from a foreign destination, Licensee shall  * ensure that the distribution and export/re-export of the software is in  * compliance with all laws, regulations, orders, or other restrictions of the  * U.S. Export Administration Regulations. Licensee agrees that neither it nor  * any of its subsidiaries will export/re-export any technical data, process,  * software, or service, directly or indirectly, to any country for which the  * United States government or any agency thereof requires an export license,  * other governmental approval, or letter of assurance, without first obtaining  * such license, approval or letter.  *  *****************************************************************************/
end_comment

begin_define
define|#
directive|define
name|__AMDUMP_C__
end_define

begin_include
include|#
directive|include
file|"acpi.h"
end_include

begin_include
include|#
directive|include
file|"acinterp.h"
end_include

begin_include
include|#
directive|include
file|"amlcode.h"
end_include

begin_include
include|#
directive|include
file|"acnamesp.h"
end_include

begin_include
include|#
directive|include
file|"actables.h"
end_include

begin_define
define|#
directive|define
name|_COMPONENT
value|INTERPRETER
end_define

begin_macro
name|MODULE_NAME
argument_list|(
literal|"amdump"
argument_list|)
end_macro

begin_comment
comment|/*  * The following routines are used for debug output only  */
end_comment

begin_if
if|#
directive|if
name|defined
argument_list|(
name|ACPI_DEBUG
argument_list|)
operator|||
name|defined
argument_list|(
name|ENABLE_DEBUGGER
argument_list|)
end_if

begin_comment
comment|/*****************************************************************************  *  * FUNCTION:    AcpiAmlShowHexValue  *  * PARAMETERS:  ByteCount           - Number of bytes to print (1, 2, or 4)  *              *AmlPtr             - Address in AML stream of bytes to print  *              InterpreterMode     - Current running mode (load1/Load2/Exec)  *              LeadSpace           - # of spaces to print ahead of value  *                                    0 => none ahead but one behind  *  * DESCRIPTION: Print ByteCount byte(s) starting at AcpiAmlPtr as a single  *              value, in hex.  If ByteCount> 1 or the value printed is> 9, also  *              print in decimal.  *  ****************************************************************************/
end_comment

begin_function
name|void
name|AcpiAmlShowHexValue
parameter_list|(
name|UINT32
name|ByteCount
parameter_list|,
name|UINT8
modifier|*
name|AmlPtr
parameter_list|,
name|UINT32
name|LeadSpace
parameter_list|)
block|{
name|UINT32
name|Value
decl_stmt|;
comment|/*  Value retrieved from AML stream */
name|UINT32
name|ShowDecimalValue
decl_stmt|;
name|UINT32
name|Length
decl_stmt|;
comment|/*  Length of printed field */
name|UINT8
modifier|*
name|CurrentAmlPtr
init|=
name|NULL
decl_stmt|;
comment|/*  Pointer to current byte of AML value    */
name|FUNCTION_TRACE
argument_list|(
literal|"AmlShowHexValue"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|AmlPtr
condition|)
block|{
name|REPORT_ERROR
argument_list|(
operator|(
literal|"AmlShowHexValue: null pointer\n"
operator|)
argument_list|)
expr_stmt|;
block|}
comment|/*      * AML numbers are always stored little-endian,      * even if the processor is big-endian.      */
for|for
control|(
name|CurrentAmlPtr
operator|=
name|AmlPtr
operator|+
name|ByteCount
operator|,
name|Value
operator|=
literal|0
init|;
name|CurrentAmlPtr
operator|>
name|AmlPtr
condition|;
control|)
block|{
name|Value
operator|=
operator|(
name|Value
operator|<<
literal|8
operator|)
operator|+
operator|(
name|UINT32
operator|)
operator|*
operator|--
name|CurrentAmlPtr
expr_stmt|;
block|}
name|Length
operator|=
name|LeadSpace
operator|*
name|ByteCount
operator|+
literal|2
expr_stmt|;
if|if
condition|(
name|ByteCount
operator|>
literal|1
condition|)
block|{
name|Length
operator|+=
operator|(
name|ByteCount
operator|-
literal|1
operator|)
expr_stmt|;
block|}
name|ShowDecimalValue
operator|=
operator|(
name|ByteCount
operator|>
literal|1
operator|||
name|Value
operator|>
literal|9
operator|)
expr_stmt|;
if|if
condition|(
name|ShowDecimalValue
condition|)
block|{
name|Length
operator|+=
literal|3
operator|+
name|AcpiAmlDigitsNeeded
argument_list|(
name|Value
argument_list|,
literal|10
argument_list|)
expr_stmt|;
block|}
name|DEBUG_PRINT
argument_list|(
name|TRACE_LOAD
argument_list|,
operator|(
literal|""
operator|)
argument_list|)
expr_stmt|;
for|for
control|(
name|Length
operator|=
name|LeadSpace
init|;
name|Length
condition|;
operator|--
name|Length
control|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|TRACE_LOAD
argument_list|,
operator|(
literal|" "
operator|)
argument_list|)
expr_stmt|;
block|}
while|while
condition|(
name|ByteCount
operator|--
condition|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|TRACE_LOAD
argument_list|,
operator|(
literal|"%02x"
operator|,
operator|*
name|AmlPtr
operator|++
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ByteCount
condition|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|TRACE_LOAD
argument_list|,
operator|(
literal|" "
operator|)
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|ShowDecimalValue
condition|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|TRACE_LOAD
argument_list|,
operator|(
literal|" [%ld]"
operator|,
name|Value
operator|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
literal|0
operator|==
name|LeadSpace
condition|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|TRACE_LOAD
argument_list|,
operator|(
literal|" "
operator|)
argument_list|)
expr_stmt|;
block|}
name|DEBUG_PRINT_RAW
argument_list|(
name|TRACE_LOAD
argument_list|,
operator|(
literal|"\n"
operator|)
argument_list|)
expr_stmt|;
name|return_VOID
expr_stmt|;
block|}
end_function

begin_comment
comment|/*****************************************************************************  *  * FUNCTION:    AcpiAmlDumpOperand  *  * PARAMETERS:  *EntryDesc          - Pointer to entry to be dumped  *  * RETURN:      Status  *  * DESCRIPTION: Dump a stack entry  *  ****************************************************************************/
end_comment

begin_function
name|ACPI_STATUS
name|AcpiAmlDumpOperand
parameter_list|(
name|ACPI_OPERAND_OBJECT
modifier|*
name|EntryDesc
parameter_list|)
block|{
name|UINT8
modifier|*
name|Buf
init|=
name|NULL
decl_stmt|;
name|UINT32
name|Length
decl_stmt|;
name|UINT32
name|i
decl_stmt|;
if|if
condition|(
operator|!
name|EntryDesc
condition|)
block|{
comment|/*          * This usually indicates that something serious is wrong --          * since most (if not all)          * code that dumps the stack expects something to be there!          */
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"AmlDumpOperand: *** Possible error: Null stack entry ptr\n"
operator|)
argument_list|)
expr_stmt|;
return|return
operator|(
name|AE_OK
operator|)
return|;
block|}
if|if
condition|(
name|VALID_DESCRIPTOR_TYPE
argument_list|(
name|EntryDesc
argument_list|,
name|ACPI_DESC_TYPE_NAMED
argument_list|)
condition|)
block|{
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"AmlDumpOperand: Node: \n"
operator|)
argument_list|)
expr_stmt|;
name|DUMP_ENTRY
argument_list|(
name|EntryDesc
argument_list|,
name|ACPI_INFO
argument_list|)
expr_stmt|;
return|return
operator|(
name|AE_OK
operator|)
return|;
block|}
if|if
condition|(
name|AcpiTbSystemTablePointer
argument_list|(
name|EntryDesc
argument_list|)
condition|)
block|{
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"AmlDumpOperand: %p is a Pcode pointer\n"
operator|,
name|EntryDesc
operator|)
argument_list|)
expr_stmt|;
return|return
operator|(
name|AE_OK
operator|)
return|;
block|}
if|if
condition|(
operator|!
name|VALID_DESCRIPTOR_TYPE
argument_list|(
name|EntryDesc
argument_list|,
name|ACPI_DESC_TYPE_INTERNAL
argument_list|)
condition|)
block|{
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"AmlDumpOperand: %p Not a local object \n"
operator|,
name|EntryDesc
operator|)
argument_list|)
expr_stmt|;
name|DUMP_BUFFER
argument_list|(
name|EntryDesc
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_OPERAND_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
name|AE_OK
operator|)
return|;
block|}
comment|/*  EntryDesc is a valid object  */
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"AmlDumpOperand: %p "
operator|,
name|EntryDesc
operator|)
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|EntryDesc
operator|->
name|Common
operator|.
name|Type
condition|)
block|{
case|case
name|INTERNAL_TYPE_REFERENCE
case|:
switch|switch
condition|(
name|EntryDesc
operator|->
name|Reference
operator|.
name|OpCode
condition|)
block|{
case|case
name|AML_ZERO_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference: Zero\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_ONE_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference: One\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_ONES_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference: Ones\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_DEBUG_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference: Debug\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_NAME_OP
case|:
name|DUMP_PATHNAME
argument_list|(
name|EntryDesc
operator|->
name|Reference
operator|.
name|Object
argument_list|,
literal|"Reference: Name: "
argument_list|,
name|ACPI_INFO
argument_list|,
name|_COMPONENT
argument_list|)
expr_stmt|;
name|DUMP_ENTRY
argument_list|(
name|EntryDesc
operator|->
name|Reference
operator|.
name|Object
argument_list|,
name|ACPI_INFO
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_INDEX_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference: Index %p\n"
operator|,
name|EntryDesc
operator|->
name|Reference
operator|.
name|Object
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_ARG_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference: Arg%d"
operator|,
name|EntryDesc
operator|->
name|Reference
operator|.
name|Offset
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ACPI_TYPE_INTEGER
operator|==
name|EntryDesc
operator|->
name|Common
operator|.
name|Type
condition|)
block|{
comment|/* Value is a Number */
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|" value is [%ld]"
operator|,
name|EntryDesc
operator|->
name|Integer
operator|.
name|Value
operator|)
argument_list|)
expr_stmt|;
block|}
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_LOCAL_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference: Local%d"
operator|,
name|EntryDesc
operator|->
name|Reference
operator|.
name|Offset
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ACPI_TYPE_INTEGER
operator|==
name|EntryDesc
operator|->
name|Common
operator|.
name|Type
condition|)
block|{
comment|/* Value is a Number */
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|" value is [%ld]"
operator|,
name|EntryDesc
operator|->
name|Integer
operator|.
name|Value
operator|)
argument_list|)
expr_stmt|;
block|}
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|AML_NAMEPATH_OP
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Reference.Node->Name %x\n"
operator|,
name|EntryDesc
operator|->
name|Reference
operator|.
name|Node
operator|->
name|Name
operator|)
argument_list|)
expr_stmt|;
break|break;
default|default:
comment|/*  unknown opcode  */
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Unknown opcode=%X\n"
operator|,
name|EntryDesc
operator|->
name|Reference
operator|.
name|OpCode
operator|)
argument_list|)
expr_stmt|;
break|break;
block|}
break|break;
case|case
name|ACPI_TYPE_BUFFER
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Buffer len %X @ %p \n"
operator|,
name|EntryDesc
operator|->
name|Buffer
operator|.
name|Length
operator|,
name|EntryDesc
operator|->
name|Buffer
operator|.
name|Pointer
operator|)
argument_list|)
expr_stmt|;
name|Length
operator|=
name|EntryDesc
operator|->
name|Buffer
operator|.
name|Length
expr_stmt|;
if|if
condition|(
name|Length
operator|>
literal|64
condition|)
block|{
name|Length
operator|=
literal|64
expr_stmt|;
block|}
comment|/* Debug only -- dump the buffer contents */
if|if
condition|(
name|EntryDesc
operator|->
name|Buffer
operator|.
name|Pointer
condition|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Buffer Contents: "
operator|)
argument_list|)
expr_stmt|;
for|for
control|(
name|Buf
operator|=
name|EntryDesc
operator|->
name|Buffer
operator|.
name|Pointer
init|;
name|Length
operator|--
condition|;
operator|++
name|Buf
control|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
name|Length
condition|?
literal|" %02x"
else|:
literal|" %02x"
operator|,
operator|*
name|Buf
operator|)
argument_list|)
expr_stmt|;
block|}
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"\n"
operator|)
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|ACPI_TYPE_INTEGER
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Number %lX\n"
operator|,
name|EntryDesc
operator|->
name|Integer
operator|.
name|Value
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_IF
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"If [Number] %lX\n"
operator|,
name|EntryDesc
operator|->
name|Integer
operator|.
name|Value
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_WHILE
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"While [Number] %lX\n"
operator|,
name|EntryDesc
operator|->
name|Integer
operator|.
name|Value
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_PACKAGE
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Package count %X @ %p\n"
operator|,
name|EntryDesc
operator|->
name|Package
operator|.
name|Count
operator|,
name|EntryDesc
operator|->
name|Package
operator|.
name|Elements
operator|)
argument_list|)
expr_stmt|;
comment|/*          * If elements exist, package vector pointer is valid,          * and debug_level exceeds 1, dump package's elements.          */
if|if
condition|(
name|EntryDesc
operator|->
name|Package
operator|.
name|Count
operator|&&
name|EntryDesc
operator|->
name|Package
operator|.
name|Elements
operator|&&
name|GetDebugLevel
argument_list|()
operator|>
literal|1
condition|)
block|{
name|ACPI_OPERAND_OBJECT
modifier|*
modifier|*
name|Element
decl_stmt|;
name|UINT16
name|ElementIndex
decl_stmt|;
for|for
control|(
name|ElementIndex
operator|=
literal|0
operator|,
name|Element
operator|=
name|EntryDesc
operator|->
name|Package
operator|.
name|Elements
init|;
name|ElementIndex
operator|<
name|EntryDesc
operator|->
name|Package
operator|.
name|Count
condition|;
operator|++
name|ElementIndex
operator|,
operator|++
name|Element
control|)
block|{
name|AcpiAmlDumpOperand
argument_list|(
operator|*
name|Element
argument_list|)
expr_stmt|;
block|}
block|}
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_REGION
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Region %s (%X)"
operator|,
name|AcpiCmGetRegionName
argument_list|(
name|EntryDesc
operator|->
name|Region
operator|.
name|SpaceId
argument_list|)
operator|,
name|EntryDesc
operator|->
name|Region
operator|.
name|SpaceId
operator|)
argument_list|)
expr_stmt|;
comment|/*          * If the address and length have not been evaluated,          * don't print them.          */
if|if
condition|(
operator|!
operator|(
name|EntryDesc
operator|->
name|Region
operator|.
name|Flags
operator|&
name|AOPOBJ_DATA_VALID
operator|)
condition|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"\n"
operator|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|" base %p Length %X\n"
operator|,
name|EntryDesc
operator|->
name|Region
operator|.
name|Address
operator|,
name|EntryDesc
operator|->
name|Region
operator|.
name|Length
operator|)
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|ACPI_TYPE_STRING
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"String length %X @ %p\n\n"
operator|,
name|EntryDesc
operator|->
name|String
operator|.
name|Length
operator|,
name|EntryDesc
operator|->
name|String
operator|.
name|Pointer
operator|)
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|EntryDesc
operator|->
name|String
operator|.
name|Length
condition|;
name|i
operator|++
control|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"%c"
operator|,
name|EntryDesc
operator|->
name|String
operator|.
name|Pointer
index|[
name|i
index|]
operator|)
argument_list|)
expr_stmt|;
block|}
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"\n\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_BANK_FIELD
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"BankField\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_DEF_FIELD
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"DefField: bits=%X  acc=%X lock=%X update=%X at byte=%lX bit=%X of below:\n"
operator|,
name|EntryDesc
operator|->
name|Field
operator|.
name|Length
operator|,
name|EntryDesc
operator|->
name|Field
operator|.
name|Access
operator|,
name|EntryDesc
operator|->
name|Field
operator|.
name|LockRule
operator|,
name|EntryDesc
operator|->
name|Field
operator|.
name|UpdateRule
operator|,
name|EntryDesc
operator|->
name|Field
operator|.
name|Offset
operator|,
name|EntryDesc
operator|->
name|Field
operator|.
name|BitOffset
operator|)
argument_list|)
expr_stmt|;
name|DUMP_STACK_ENTRY
argument_list|(
name|EntryDesc
operator|->
name|Field
operator|.
name|Container
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_INDEX_FIELD
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"IndexField\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_FIELD_UNIT
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"FieldUnit: %X bits acc %X lock %X update %X at byte %lX bit %X of \n"
operator|,
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|Length
operator|,
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|Access
operator|,
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|LockRule
operator|,
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|UpdateRule
operator|,
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|Offset
operator|,
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|BitOffset
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|Container
condition|)
block|{
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"*NULL* \n"
operator|)
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|ACPI_TYPE_BUFFER
operator|!=
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|Container
operator|->
name|Common
operator|.
name|Type
condition|)
block|{
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"*not a Buffer* \n"
operator|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|DUMP_STACK_ENTRY
argument_list|(
name|EntryDesc
operator|->
name|FieldUnit
operator|.
name|Container
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|ACPI_TYPE_EVENT
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Event\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_METHOD
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Method(%X) @ %p:%lX\n"
operator|,
name|EntryDesc
operator|->
name|Method
operator|.
name|ParamCount
operator|,
name|EntryDesc
operator|->
name|Method
operator|.
name|Pcode
operator|,
name|EntryDesc
operator|->
name|Method
operator|.
name|PcodeLength
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_MUTEX
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Mutex\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_DEVICE
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Device\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_POWER
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Power\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_PROCESSOR
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Processor\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_THERMAL
case|:
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Thermal\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
default|default:
comment|/*  unknown EntryDesc->Common.Type value    */
name|DEBUG_PRINT_RAW
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"Unknown Type %X\n"
operator|,
name|EntryDesc
operator|->
name|Common
operator|.
name|Type
operator|)
argument_list|)
expr_stmt|;
comment|/* Back up to previous entry */
name|EntryDesc
operator|--
expr_stmt|;
comment|/* TBD: [Restructure]  Change to use dump object routine !! */
comment|/*       What is all of this?? */
name|DUMP_BUFFER
argument_list|(
name|EntryDesc
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_OPERAND_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
name|DUMP_BUFFER
argument_list|(
operator|++
name|EntryDesc
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_OPERAND_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
name|DUMP_BUFFER
argument_list|(
operator|++
name|EntryDesc
argument_list|,
sizeof|sizeof
argument_list|(
name|ACPI_OPERAND_OBJECT
argument_list|)
argument_list|)
expr_stmt|;
break|break;
block|}
return|return
operator|(
name|AE_OK
operator|)
return|;
block|}
end_function

begin_comment
comment|/*****************************************************************************  *  * FUNCTION:    AcpiAmlDumpOperands  *  * PARAMETERS:  InterpreterMode      - Load or Exec  *              *Ident              - Identification  *              NumLevels           - # of stack entries to dump above line  *              *Note               - Output notation  *  * DESCRIPTION: Dump the object stack  *  ****************************************************************************/
end_comment

begin_function
name|void
name|AcpiAmlDumpOperands
parameter_list|(
name|ACPI_OPERAND_OBJECT
modifier|*
modifier|*
name|Operands
parameter_list|,
name|OPERATING_MODE
name|InterpreterMode
parameter_list|,
name|NATIVE_CHAR
modifier|*
name|Ident
parameter_list|,
name|UINT32
name|NumLevels
parameter_list|,
name|NATIVE_CHAR
modifier|*
name|Note
parameter_list|,
name|NATIVE_CHAR
modifier|*
name|ModuleName
parameter_list|,
name|UINT32
name|LineNumber
parameter_list|)
block|{
name|NATIVE_UINT
name|i
decl_stmt|;
name|ACPI_OPERAND_OBJECT
modifier|*
modifier|*
name|EntryDesc
decl_stmt|;
if|if
condition|(
operator|!
name|Ident
condition|)
block|{
name|Ident
operator|=
literal|"?"
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|Note
condition|)
block|{
name|Note
operator|=
literal|"?"
expr_stmt|;
block|}
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"************* AcpiAmlDumpOperands  Mode=%X ******************\n"
operator|,
name|InterpreterMode
operator|)
argument_list|)
expr_stmt|;
name|DEBUG_PRINT
argument_list|(
name|ACPI_INFO
argument_list|,
operator|(
literal|"From %12s(%d)  %s: %s\n"
operator|,
name|ModuleName
operator|,
name|LineNumber
operator|,
name|Ident
operator|,
name|Note
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|NumLevels
operator|==
literal|0
condition|)
name|NumLevels
operator|=
literal|1
expr_stmt|;
comment|/* Dump the stack starting at the top, working down */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|NumLevels
operator|>
literal|0
condition|;
name|i
operator|--
operator|,
name|NumLevels
operator|--
control|)
block|{
name|EntryDesc
operator|=
operator|&
name|Operands
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|ACPI_FAILURE
argument_list|(
name|AcpiAmlDumpOperand
argument_list|(
operator|*
name|EntryDesc
argument_list|)
argument_list|)
condition|)
block|{
break|break;
block|}
block|}
return|return;
block|}
end_function

begin_comment
comment|/*****************************************************************************  *  * FUNCTION:    AcpiAmlDumpNode  *  * PARAMETERS:  *Node           - Descriptor to dump  *              Flags               - Force display  *  * DESCRIPTION: Dumps the members of the given.Node  *  ****************************************************************************/
end_comment

begin_function
name|void
name|AcpiAmlDumpNode
parameter_list|(
name|ACPI_NAMESPACE_NODE
modifier|*
name|Node
parameter_list|,
name|UINT32
name|Flags
parameter_list|)
block|{
if|if
condition|(
operator|!
name|Flags
condition|)
block|{
if|if
condition|(
operator|!
operator|(
operator|(
name|TRACE_OBJECTS
operator|&
name|AcpiDbgLevel
operator|)
operator|&&
operator|(
name|_COMPONENT
operator|&
name|AcpiDbgLayer
operator|)
operator|)
condition|)
block|{
return|return;
block|}
block|}
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %4.4s\n"
argument_list|,
literal|"Name"
argument_list|,
operator|&
name|Node
operator|->
name|Name
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
name|AcpiCmGetTypeName
argument_list|(
name|Node
operator|->
name|Type
argument_list|)
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Flags"
argument_list|,
name|Node
operator|->
name|Flags
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Owner Id"
argument_list|,
name|Node
operator|->
name|OwnerId
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Reference Count"
argument_list|,
name|Node
operator|->
name|ReferenceCount
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Attached Object"
argument_list|,
name|Node
operator|->
name|Object
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"ChildList"
argument_list|,
name|Node
operator|->
name|Child
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"NextPeer"
argument_list|,
name|Node
operator|->
name|Peer
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Parent"
argument_list|,
name|AcpiNsGetParentObject
argument_list|(
name|Node
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*****************************************************************************  *  * FUNCTION:    AcpiAmlDumpObjectDescriptor  *  * PARAMETERS:  *Object             - Descriptor to dump  *              Flags               - Force display  *  * DESCRIPTION: Dumps the members of the object descriptor given.  *  ****************************************************************************/
end_comment

begin_function
name|void
name|AcpiAmlDumpObjectDescriptor
parameter_list|(
name|ACPI_OPERAND_OBJECT
modifier|*
name|ObjDesc
parameter_list|,
name|UINT32
name|Flags
parameter_list|)
block|{
name|FUNCTION_TRACE
argument_list|(
literal|"AmlDumpObjectDescriptor"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|Flags
condition|)
block|{
if|if
condition|(
operator|!
operator|(
operator|(
name|TRACE_OBJECTS
operator|&
name|AcpiDbgLevel
operator|)
operator|&&
operator|(
name|_COMPONENT
operator|&
name|AcpiDbgLayer
operator|)
operator|)
condition|)
block|{
return|return;
block|}
block|}
if|if
condition|(
operator|!
operator|(
name|VALID_DESCRIPTOR_TYPE
argument_list|(
name|ObjDesc
argument_list|,
name|ACPI_DESC_TYPE_INTERNAL
argument_list|)
operator|)
condition|)
block|{
name|AcpiOsPrintf
argument_list|(
literal|"%p is not a valid ACPI object\n"
argument_list|,
name|ObjDesc
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* Common Fields */
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Reference Count"
argument_list|,
name|ObjDesc
operator|->
name|Common
operator|.
name|ReferenceCount
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Flags"
argument_list|,
name|ObjDesc
operator|->
name|Common
operator|.
name|Flags
argument_list|)
expr_stmt|;
comment|/* Object-specific Fields */
switch|switch
condition|(
name|ObjDesc
operator|->
name|Common
operator|.
name|Type
condition|)
block|{
case|case
name|ACPI_TYPE_INTEGER
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Number"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Value"
argument_list|,
name|ObjDesc
operator|->
name|Integer
operator|.
name|Value
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_STRING
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"String"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|String
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Pointer"
argument_list|,
name|ObjDesc
operator|->
name|String
operator|.
name|Pointer
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_BUFFER
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Buffer"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|Buffer
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Pointer"
argument_list|,
name|ObjDesc
operator|->
name|Buffer
operator|.
name|Pointer
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_PACKAGE
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Package"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Count"
argument_list|,
name|ObjDesc
operator|->
name|Package
operator|.
name|Count
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Elements"
argument_list|,
name|ObjDesc
operator|->
name|Package
operator|.
name|Elements
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"NextElement"
argument_list|,
name|ObjDesc
operator|->
name|Package
operator|.
name|NextElement
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_FIELD_UNIT
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"FieldUnit"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Access"
argument_list|,
name|ObjDesc
operator|->
name|FieldUnit
operator|.
name|Access
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"LockRule"
argument_list|,
name|ObjDesc
operator|->
name|FieldUnit
operator|.
name|LockRule
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"UpdateRule"
argument_list|,
name|ObjDesc
operator|->
name|FieldUnit
operator|.
name|UpdateRule
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|FieldUnit
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"BitOffset"
argument_list|,
name|ObjDesc
operator|->
name|FieldUnit
operator|.
name|BitOffset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Offset"
argument_list|,
name|ObjDesc
operator|->
name|FieldUnit
operator|.
name|Offset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Container"
argument_list|,
name|ObjDesc
operator|->
name|FieldUnit
operator|.
name|Container
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_DEVICE
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Device"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"AddrHandler"
argument_list|,
name|ObjDesc
operator|->
name|Device
operator|.
name|AddrHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"SysHandler"
argument_list|,
name|ObjDesc
operator|->
name|Device
operator|.
name|SysHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"DrvHandler"
argument_list|,
name|ObjDesc
operator|->
name|Device
operator|.
name|DrvHandler
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_EVENT
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Event"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Semaphore"
argument_list|,
name|ObjDesc
operator|->
name|Event
operator|.
name|Semaphore
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_METHOD
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Method"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"ParamCount"
argument_list|,
name|ObjDesc
operator|->
name|Method
operator|.
name|ParamCount
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Concurrency"
argument_list|,
name|ObjDesc
operator|->
name|Method
operator|.
name|Concurrency
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Semaphore"
argument_list|,
name|ObjDesc
operator|->
name|Method
operator|.
name|Semaphore
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"PcodeLength"
argument_list|,
name|ObjDesc
operator|->
name|Method
operator|.
name|PcodeLength
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Pcode"
argument_list|,
name|ObjDesc
operator|->
name|Method
operator|.
name|Pcode
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_MUTEX
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Mutex"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"SyncLevel"
argument_list|,
name|ObjDesc
operator|->
name|Mutex
operator|.
name|SyncLevel
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Semaphore"
argument_list|,
name|ObjDesc
operator|->
name|Mutex
operator|.
name|Semaphore
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_REGION
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Region"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"SpaceId"
argument_list|,
name|ObjDesc
operator|->
name|Region
operator|.
name|SpaceId
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Flags"
argument_list|,
name|ObjDesc
operator|->
name|Region
operator|.
name|Flags
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Address"
argument_list|,
name|ObjDesc
operator|->
name|Region
operator|.
name|Address
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|Region
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"AddrHandler"
argument_list|,
name|ObjDesc
operator|->
name|Region
operator|.
name|AddrHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Next"
argument_list|,
name|ObjDesc
operator|->
name|Region
operator|.
name|Next
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_POWER
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"PowerResource"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"SystemLevel"
argument_list|,
name|ObjDesc
operator|->
name|PowerResource
operator|.
name|SystemLevel
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"ResourceOrder"
argument_list|,
name|ObjDesc
operator|->
name|PowerResource
operator|.
name|ResourceOrder
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"SysHandler"
argument_list|,
name|ObjDesc
operator|->
name|PowerResource
operator|.
name|SysHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"DrvHandler"
argument_list|,
name|ObjDesc
operator|->
name|PowerResource
operator|.
name|DrvHandler
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_PROCESSOR
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Processor"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Processor ID"
argument_list|,
name|ObjDesc
operator|->
name|Processor
operator|.
name|ProcId
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|Processor
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Address"
argument_list|,
name|ObjDesc
operator|->
name|Processor
operator|.
name|Address
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"SysHandler"
argument_list|,
name|ObjDesc
operator|->
name|Processor
operator|.
name|SysHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"DrvHandler"
argument_list|,
name|ObjDesc
operator|->
name|Processor
operator|.
name|DrvHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"AddrHandler"
argument_list|,
name|ObjDesc
operator|->
name|Processor
operator|.
name|AddrHandler
argument_list|)
expr_stmt|;
break|break;
case|case
name|ACPI_TYPE_THERMAL
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"ThermalZone"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"SysHandler"
argument_list|,
name|ObjDesc
operator|->
name|ThermalZone
operator|.
name|SysHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"DrvHandler"
argument_list|,
name|ObjDesc
operator|->
name|ThermalZone
operator|.
name|DrvHandler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"AddrHandler"
argument_list|,
name|ObjDesc
operator|->
name|ThermalZone
operator|.
name|AddrHandler
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_BANK_FIELD
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"BankField"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Access"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|Access
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"LockRule"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|LockRule
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"UpdateRule"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|UpdateRule
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"BitOffset"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|BitOffset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Offset"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|Offset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Value"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|Value
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Container"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|Container
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"BankSelect"
argument_list|,
name|ObjDesc
operator|->
name|BankField
operator|.
name|BankSelect
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_INDEX_FIELD
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"IndexField"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Access"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|Access
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"LockRule"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|LockRule
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"UpdateRule"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|UpdateRule
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"BitOffset"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|BitOffset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Value"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|Value
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Index"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|Index
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Data"
argument_list|,
name|ObjDesc
operator|->
name|IndexField
operator|.
name|Data
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_REFERENCE
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Reference"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"TargetType"
argument_list|,
name|ObjDesc
operator|->
name|Reference
operator|.
name|TargetType
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"OpCode"
argument_list|,
name|ObjDesc
operator|->
name|Reference
operator|.
name|OpCode
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"Offset"
argument_list|,
name|ObjDesc
operator|->
name|Reference
operator|.
name|Offset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"ObjDesc"
argument_list|,
name|ObjDesc
operator|->
name|Reference
operator|.
name|Object
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Node"
argument_list|,
name|ObjDesc
operator|->
name|Reference
operator|.
name|Node
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Where"
argument_list|,
name|ObjDesc
operator|->
name|Reference
operator|.
name|Where
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_ADDRESS_HANDLER
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Address Handler"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %X\n"
argument_list|,
literal|"SpaceId"
argument_list|,
name|ObjDesc
operator|->
name|AddrHandler
operator|.
name|SpaceId
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Next"
argument_list|,
name|ObjDesc
operator|->
name|AddrHandler
operator|.
name|Next
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"RegionList"
argument_list|,
name|ObjDesc
operator|->
name|AddrHandler
operator|.
name|RegionList
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Node"
argument_list|,
name|ObjDesc
operator|->
name|AddrHandler
operator|.
name|Node
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Handler"
argument_list|,
name|ObjDesc
operator|->
name|AddrHandler
operator|.
name|Handler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Context"
argument_list|,
name|ObjDesc
operator|->
name|AddrHandler
operator|.
name|Context
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_NOTIFY
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %s\n"
argument_list|,
literal|"Type"
argument_list|,
literal|"Notify Handler"
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Node"
argument_list|,
name|ObjDesc
operator|->
name|NotifyHandler
operator|.
name|Node
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Handler"
argument_list|,
name|ObjDesc
operator|->
name|NotifyHandler
operator|.
name|Handler
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Context"
argument_list|,
name|ObjDesc
operator|->
name|NotifyHandler
operator|.
name|Context
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_DEF_FIELD
case|:
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Granularity"
argument_list|,
name|ObjDesc
operator|->
name|Field
operator|.
name|Granularity
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Length"
argument_list|,
name|ObjDesc
operator|->
name|Field
operator|.
name|Length
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Offset"
argument_list|,
name|ObjDesc
operator|->
name|Field
operator|.
name|Offset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"BitOffset"
argument_list|,
name|ObjDesc
operator|->
name|Field
operator|.
name|BitOffset
argument_list|)
expr_stmt|;
name|AcpiOsPrintf
argument_list|(
literal|"%20s : %p\n"
argument_list|,
literal|"Container"
argument_list|,
name|ObjDesc
operator|->
name|Field
operator|.
name|Container
argument_list|)
expr_stmt|;
break|break;
case|case
name|INTERNAL_TYPE_ALIAS
case|:
case|case
name|INTERNAL_TYPE_DEF_FIELD_DEFN
case|:
case|case
name|INTERNAL_TYPE_BANK_FIELD_DEFN
case|:
case|case
name|INTERNAL_TYPE_INDEX_FIELD_DEFN
case|:
case|case
name|INTERNAL_TYPE_IF
case|:
case|case
name|INTERNAL_TYPE_ELSE
case|:
case|case
name|INTERNAL_TYPE_WHILE
case|:
case|case
name|INTERNAL_TYPE_SCOPE
case|:
case|case
name|INTERNAL_TYPE_DEF_ANY
case|:
name|AcpiOsPrintf
argument_list|(
literal|"*** Structure display not implemented for type %X! ***\n"
argument_list|,
name|ObjDesc
operator|->
name|Common
operator|.
name|Type
argument_list|)
expr_stmt|;
break|break;
default|default:
name|AcpiOsPrintf
argument_list|(
literal|"*** Cannot display unknown type %X! ***\n"
argument_list|,
name|ObjDesc
operator|->
name|Common
operator|.
name|Type
argument_list|)
expr_stmt|;
break|break;
block|}
name|return_VOID
expr_stmt|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

end_unit

