...
begin_comment
comment|/******************************************************************************  *  * Module Name: nsinit - namespace initialization  *              $Revision: 4 $  *  *****************************************************************************/
end_comment
...
expr_stmt|;
block|}
comment|/*      * Examine the HID of the device.  _HID can be an executable      * control method -- it simply has to return a string or number      * containing the HID.      */
if|if
condition|(
name|RetObj
condition|)
...
condition|)
block|{
comment|/* No _HID --> Can't be a PCI root bridge */
name|return_ACPI_STATUS
argument_list|(
name|AE_OK
argument_list|)
...
operator|++
expr_stmt|;
comment|/*      * Found an _HID object.      * Check for a PCI Root Bridge.  We need to install the PCI_CONFIG space      * handler on all PCI Root Bridges found within the namespace      *      * A PCI Root Bridge has an HID with the value EISAID("PNP0A03")      * The HID can be either a number or a string.      */
switch|switch
condition|(
name|RetObj
operator|->
...
goto|;
block|}
comment|/*      * We found a valid PCI_ROOT_HID.      * The parent of the HID entry is the PCI device;  Install the default PCI      * handler for this PCI device.      */
name|Info
operator|->
name|Num_PCI
operator|++
