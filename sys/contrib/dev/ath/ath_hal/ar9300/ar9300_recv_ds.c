...
literal|0
expr_stmt|;
comment|/* TX beamforming; CSI for locationing */
name|rxs
operator|->
name|rs_flags
operator||=
...
directive|else
comment|/*              * If the locationing counter is enabled, Osprey always              * seems to put AR_position_bit in each frame.              * So, only do this if we also have a valid upload              * and it's type "1" (which I'm guessing is CSI.)              */
if|if
condition|(
operator|(
name|rxs
