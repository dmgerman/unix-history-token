...
decl_stmt|;
comment|//int16_t nf_buf[HAL_NUM_NF_READINGS];
if|if
condition|(
operator|(
name|AR_SREV_WASP
...
directive|if
literal|0
block|if (ss->ss_spectral_pri) {         for (i = 0; i< HAL_NUM_NF_READINGS; i++) {             nf_buf[i] = NOISE_PWR_DBM_2_INT(ss->ss_nf_cal[i]);         }         ar9300_load_nf(ah, nf_buf);
ifdef|#
directive|ifdef
name|DEMO_MODE
block|ar9300_disable_strong_signal(ah);         ar9300_disable_weak_signal(ah);         ar9300_set_radar_dc_thresh(ah);         ar9300_set_cca_threshold(ah, MAX_CCA_THRESH);
comment|/*ar9300_disable_restart(ah);*/
endif|#
directive|endif
block|}
