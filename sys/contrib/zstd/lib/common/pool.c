...
name|queueSize
decl_stmt|;
comment|/* The number of threads working on jobs */
name|size_t
name|numThreadsBusy
decl_stmt|;
comment|/* Indicates if the queue is empty */
name|int
name|queueEmpty
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* If the intended queue size was 0, signal after finishing job */
if|if
condition|(
name|ctx
operator|->
...
begin_comment
comment|/**  * Returns 1 if the queue is full and 0 otherwise.  *  * If the queueSize is 1 (the pool was created with an intended queueSize of 0),  * then a queue is empty if there is a thread free and no job is waiting.  */
end_comment
