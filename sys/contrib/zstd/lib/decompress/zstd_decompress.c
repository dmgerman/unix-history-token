...
directive|define
name|ZSTD_MAXWINDOWSIZE_DEFAULT
value|(((U32)1<< ZSTD_WINDOWLOG_DEFAULTMAX) + 1)
end_define
...
name|bType
decl_stmt|;
comment|/* used in ZSTD_decompressContinue(), store blockType between block header decoding and block decompression stages */
name|ZSTD_dStage
name|stage
decl_stmt|;
name|U32
...
argument_list|)
expr_stmt|;
comment|/* only supports formats ZSTD_f_zstd1 and ZSTD_f_zstd1_magicless */
name|assert
argument_list|(
operator|(
name|format
...
name|ZSTD_f_zstd1
expr_stmt|;
comment|/* ZSTD_decompressBegin() invokes ZSTD_startingInputLength() with argument dctx->format */
name|dctx
operator|->
name|staticSize
operator|=
...
begin_comment
comment|/** ZSTD_frameHeaderSize_internal() :  *  srcSize must be large enough to reach header size fields.  *  note : only works for formats ZSTD_f_zstd1 and ZSTD_f_zstd1_magicless  * @return : size of the Frame Header  *           or an error code, which can be tested with ZSTD_isError() */
end_comment

begin_function
specifier|static
name|size_t
name|ZSTD_frameHeaderSize_internal
parameter_list|(
specifier|const
name|void
modifier|*
...
begin_comment
comment|/** ZSTD_getFrameHeader_internal() :  *  decode Frame Header, or require larger `srcSize`.  *  note : only works for formats ZSTD_f_zstd1 and ZSTD_f_zstd1_magicless  * @return : 0, `zfhPtr` is correctly filled,  *>0, `srcSize` is too small, value is wanted `srcSize` amount,  *           or an error code, which can be tested using ZSTD_isError() */
end_comment

begin_function
specifier|static
name|size_t
name|ZSTD_getFrameHeader_internal
parameter_list|(
name|ZSTD_frameHeader
modifier|*
name|zfhPtr
...
begin_comment
comment|/** ZSTD_getFrameHeader() :  *  decode Frame Header, or require larger `srcSize`.  *  note : this function does not consume input, it only reads it.  * @return : 0, `zfhPtr` is correctly filled,  *>0, `srcSize` is too small, value is wanted `srcSize` amount,  *           or an error code, which can be tested using ZSTD_isError() */
end_comment
...
begin_comment
comment|/*! ZSTD_decodeLiteralsBlock() :  * @return : nb of bytes read from src (< srcSize )  *  note : symbol not declared but exposed for fullbench */
end_comment
...
literal|1
block|}
name|ZSTD_longOffset_e
typedef|;
end_typedef

begin_comment
comment|/* We need to add at most (ZSTD_WINDOWLOG_MAX_32 - 1) bits to read the maximum  * offset bits. But we can only read at most (STREAM_ACCUMULATOR_MIN_32 - 1)  * bits before reloading. This value is the maximum number of bytes we read  * after reloading when we are decoding long offets.  */
end_comment

begin_define
define|#
directive|define
name|LONG_OFFSETS_MAX_EXTRA_BITS_32
define|\
value|(ZSTD_WINDOWLOG_MAX_32> STREAM_ACCUMULATOR_MIN_32                         \         ? ZSTD_WINDOWLOG_MAX_32 - STREAM_ACCUMULATOR_MIN_32                    \         : 0)
end_define
...
argument_list|)
expr_stmt|;
comment|/* Verify that there is enough bits to read the rest of the data in 64-bit mode. */
name|ZSTD_STATIC_ASSERT
argument_list|(
literal|16
operator|+
...
name|src
decl_stmt|;
comment|/* isLongOffset must be true if there are long offsets.      * Offsets are long if they are larger than 2^STREAM_ACCUMULATOR_MIN.      * We don't expect that to be the case in 64-bit mode.      * If we are in block mode we don't know the window size, so we have to be      * conservative.      */
name|ZSTD_longOffset_e
specifier|const
name|isLongOffset
init|=
...
argument_list|)
decl_stmt|;
comment|/* windowSize could be any value at this point, since it is only validated      * in the streaming API.      */
name|DEBUGLOG
argument_list|(
literal|5
argument_list|,
literal|"ZSTD_decompressBlock_internal (size : %u)"
argument_list|,
operator|(
name|U32
operator|)
...
literal|4
argument_list|,
literal|"reading magic number %08X (expecting %08X)"
argument_list|,
operator|(
name|U32
operator|)
...
argument_list|)
expr_stmt|;
comment|/* to read skippable magic number */
if|if
condition|(
operator|(
name|MEM_readLE32
...
name|srcSize
expr_stmt|;
comment|/* remaining to load to get full skippable frame header */
name|dctx
operator|->
name|stage
operator|=
...
literal|5
argument_list|,
literal|"decoded size from block : %u"
argument_list|,
operator|(
name|U32
operator|)
...
literal|4
argument_list|,
literal|"decoded size from frame : %u"
argument_list|,
operator|(
name|U32
operator|)
...
argument_list|)
expr_stmt|;
comment|/* guaranteed by dctx->expected */
block|{
name|U32
specifier|const
name|h32
...
literal|4
argument_list|,
literal|"checksum : calculated %08X :: %08X read"
argument_list|,
name|h32
argument_list|,
name|check32
...
argument_list|)
expr_stmt|;
comment|/* complete skippable header */
name|dctx
operator|->
name|expected
operator|=
...
argument_list|)
expr_stmt|;
comment|/* note : dctx->expected can grow seriously large, beyond local buffer size */
name|dctx
operator|->
name|stage
operator|=
...
begin_comment
comment|/* Note : this function cannot fail */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* dctx->format must be properly set */
name|dctx
operator|->
name|stage
operator|=
...
begin_comment
comment|/*! ZSTD_estimateDDictSize() :  *  Estimate amount of memory that will be needed to create a dictionary for decompression.  *  Note : dictionary created by reference using ZSTD_dlm_byRef are smaller */
end_comment
...
begin_comment
comment|/* note : this variant can't fail */
end_comment
...
begin_comment
comment|/* ZSTD_initDStream_usingDDict() :  * ddict will just be referenced, and must outlive decompression session  * this function cannot fail */
end_comment
...
literal|4
argument_list|,
literal|"setting maxWindowSize = %u KB"
argument_list|,
name|paramValue
operator|>>
literal|10
...
name|ZSTD_WINDOWLOG_MAX
decl_stmt|;
comment|/* note : should be user-selectable */
name|ZSTD_frameHeader
name|zfh
decl_stmt|;
name|size_t
...
literal|5
argument_list|,
literal|"stage zdss_loadHeader (srcSize : %u)"
argument_list|,
call|(
name|U32
call|)
...
literal|4
argument_list|,
literal|"Control max buffer memory usage (max %u KB)"
argument_list|,
call|(
name|U32
call|)
...
argument_list|,
literal|4
comment|/* frame checksum */
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|neededOutBuffSize
init|=
name|ZSTD_decodingBufferSize_min
argument_list|(
name|zds
...
literal|5
argument_list|,
literal|"restart filling outBuff from beginning (left:%i, needed:%u)"
argument_list|,
call|(
name|int
call|)
