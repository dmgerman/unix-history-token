...
directive|define
name|MAX_NB_SEQ
value|((ZSTD_BLOCKSIZE_MAX + MIN_SEQ_LEN - 1) / MIN_SEQ_LEN)
end_define
...
literal|0
block|,
comment|/* generate frames */
name|gt_block
block|,
comment|/* generate compressed blocks without block/frame headers */
block|}
name|genType_e
typedef|;
end_typedef
...
begin_comment
comment|/*-******************************************************* *  Global variables (set from command line) *********************************************************/
end_comment

begin_decl_stmt
name|U32
name|g_maxDecompressedSizeLog
init|=
name|MAX_DECOMPRESSED_SIZE_LOG
decl_stmt|;
end_decl_stmt
...
expr_stmt|;
block|{
comment|/*          * fcsCode: 2-bit flag specifying how many bytes used to represent Frame_Content_Size (bits 7-6)          * singleSegment: 1-bit flag describing if data must be regenerated within a single continuous memory segment. (bit 5)          * contentChecksumFlag: 1-bit flag that is set if frame includes checksum at the end -- set to 1 below (bit 2)          * dictBits: 2-bit flag describing how many bytes Dictionary_ID uses -- set to 3 (bits 1-0)          * For more information: https://github.com/facebook/zstd/blob/dev/doc/zstd_compression_format.md#frame_header          */
name|int
specifier|const
name|dictBits
init|=
...
begin_comment
comment|/* Write a Huffman coded literals block and return the literals size */
end_comment
...
condition|)
block|{
comment|/* need to occasionally generate offsets that go past the start */
comment|/* including i+1 != numSequences because the last sequences has to adhere to predetermined contentSize */
name|U32
name|lenPastStart
init|=
operator|(
...
condition|)
block|{
comment|/* when offset> windowSize, matchLen bound by end of dictionary (lenPastStart) */
comment|/* this also means that lenPastStart must be greater than MIN_SEQ_LEN */
comment|/* make sure lenPastStart does not go past dictionary start though */
name|lenPastStart
operator|=
name|MIN
argument_list|(
...
condition|)
block|{
comment|/* copy from dictionary instead of literals */
name|size_t
specifier|const
name|dictOffset
init|=
...
condition|)
block|{
comment|/* compressed block must be strictly smaller than uncompressed one */
name|blockType
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* some variable size block */
name|blockContentSize
operator|=
name|RAND
argument_list|(
...
begin_comment
comment|/**  * Generated a single zstd compressed block with no block/frame header.  * Returns the final seed.  */
end_comment

begin_function
specifier|static
name|U32
name|generateCompressedBlock
parameter_list|(
name|U32
name|seed
parameter_list|,
...
condition|)
block|{
comment|/* some small blocks<= 128 bytes*/
name|blockContentSize
operator|=
name|RAND
argument_list|(
...
block|}
block|}
comment|/* try generating a compressed block */
name|frame
operator|->
name|oldStats
operator|=
...
condition|)
block|{
comment|/* compressed size must be strictly smaller than decompressed size : https://github.com/facebook/zstd/blob/dev/doc/zstd_compression_format.md#blocks */
comment|/* data doesn't compress -- try again */
name|frame
operator|->
name|stats
operator|=
...
literal|5
argument_list|,
literal|"   can't compress block : try again \n"
argument_list|)
expr_stmt|;
block|}
else|else
...
begin_comment
comment|/*_******************************************************* *  Dictionary Helper Functions *********************************************************/
end_comment

begin_comment
comment|/* returns 0 if successful, otherwise returns 1 upon error */
end_comment
...
parameter_list|)
block|{
comment|/* allocate space for samples */
name|int
name|ret
init|=
literal|0
...
name|DISPLAY
argument_list|(
literal|"Error: could not allocate space for samples\n"
argument_list|)
expr_stmt|;
return|return
literal|1
...
name|DISPLAY
argument_list|(
literal|"Error: dictionary size is too small\n"
argument_list|)
expr_stmt|;
name|ret
operator|=
...
expr_stmt|;
goto|goto
name|exitGenRandomDict
goto|;
block|}
comment|/* init dictionary params */
name|memset
argument_list|(
operator|&
name|zdictParams
...
literal|1
expr_stmt|;
comment|/* fill in dictionary content */
name|RAND_buffer
argument_list|(
operator|&
name|seed
...
argument_list|)
expr_stmt|;
comment|/* finalize dictionary with random samples */
name|dictWriteSize
operator|=
...
name|DISPLAY
argument_list|(
literal|"Could not finalize dictionary: %s\n"
argument_list|,
...
parameter_list|)
block|{
comment|/* allocate space statically */
name|dictInfo
name|dictOp
decl_stmt|;
name|memset
...
return|;
block|}
comment|/* generate random dictionary */
if|if
condition|(
name|genRandomDict
argument_list|(
...
expr_stmt|;
block|}
comment|/* manually decompress and check difference */
if|if
condition|(
name|genType
operator|==
...
name|DISPLAY
argument_list|(
literal|"Error in block mode on test seed %u: %s\n"
argument_list|,
name|seedCopy
argument_list|,
...
name|DISPLAY
argument_list|(
literal|"Error in block mode with dictionary on test seed %u: %s\n"
argument_list|,
name|seedCopy
argument_list|,
name|ZSTD_getErrorName
...
argument_list|)
decl_stmt|;
comment|/* avoid big dictionaries */
if|if
condition|(
name|ZSTD_isError
argument_list|(
...
name|DISPLAY
argument_list|(
literal|"Error in dictionary mode on test seed %u: %s\n"
argument_list|,
name|seedCopy
argument_list|,
name|ZSTD_getErrorName
...
return|;
block|}
comment|/* allocate space for the dictionary */
name|fullDict
operator|=
name|malloc
argument_list|(
...
name|DISPLAY
argument_list|(
literal|"Error: could not allocate space for full dictionary.\n"
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
comment|/* randomly generate the dictionary */
block|{
name|int
specifier|const
name|ret
...
name|DISPLAY
argument_list|(
literal|"Error: dictionary path too long\n"
argument_list|)
expr_stmt|;
name|errorDetected
operator|=
...
expr_stmt|;
block|}
comment|/* generate random compressed/decompressed files */
block|{
name|unsigned
name|fnum
decl_stmt|;
...
name|DISPLAY
argument_list|(
literal|" --use-dict=#             : include a dictionary used to decompress the corpus\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" --gen-blocks             : generate raw compressed blocks without block/frame headers\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" --max-block-size-log=#   : max block size log, must be in range [2, 17]\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" --max-content-size-log=# : max content size log, must be<= 20\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|"                            (this is ignored with gen-blocks)\n"
argument_list|)
expr_stmt|;
block|}
end_function
...
block|{
comment|/* should generate files with a dictionary */
return|return
name|generateCorpusWithDict
argument_list|(
name|seed
