...
name|f
parameter_list|)
value|{                            \     size_t const err = f;                       \     if (ZSTD_isError(err)) {                    \         fprintf(stderr,                         \                 "Error=> %s: %s",               \                 #f, ZSTD_getErrorName(err));    \         crash(1);                                \ }   }
end_define
...
begin_comment
comment|/** cctxParamRoundTripTest() :  *  Same as roundTripTest() except allows experimenting with ZSTD_CCtx_params. */
end_comment

begin_function
specifier|static
name|size_t
name|cctxParamRoundTripTest
parameter_list|(
name|void
modifier|*
name|resultBuff
