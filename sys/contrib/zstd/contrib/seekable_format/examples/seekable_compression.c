begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 2017-present, Facebook, Inc.  * All rights reserved.  *  * This source code is licensed under both the BSD-style license (found in the  * LICENSE file in the root directory of this source tree) and the GPLv2 (found  * in the COPYING file in the root directory of this source tree).  */
end_comment

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_comment
comment|// malloc, free, exit, atoi
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_comment
comment|// fprintf, perror, feof, fopen, etc.
end_comment

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_comment
comment|// strlen, memset, strcat
end_comment

begin_define
define|#
directive|define
name|ZSTD_STATIC_LINKING_ONLY
end_define

begin_include
include|#
directive|include
file|<zstd.h>
end_include

begin_comment
comment|// presumes zstd library is installed
end_comment

begin_include
include|#
directive|include
file|"zstd_seekable.h"
end_include

begin_function
specifier|static
name|void
modifier|*
name|malloc_orDie
parameter_list|(
name|size_t
name|size
parameter_list|)
block|{
name|void
modifier|*
specifier|const
name|buff
init|=
name|malloc
argument_list|(
name|size
argument_list|)
decl_stmt|;
if|if
condition|(
name|buff
condition|)
return|return
name|buff
return|;
comment|/* error */
name|perror
argument_list|(
literal|"malloc:"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|FILE
modifier|*
name|fopen_orDie
parameter_list|(
specifier|const
name|char
modifier|*
name|filename
parameter_list|,
specifier|const
name|char
modifier|*
name|instruction
parameter_list|)
block|{
name|FILE
modifier|*
specifier|const
name|inFile
init|=
name|fopen
argument_list|(
name|filename
argument_list|,
name|instruction
argument_list|)
decl_stmt|;
if|if
condition|(
name|inFile
condition|)
return|return
name|inFile
return|;
comment|/* error */
name|perror
argument_list|(
name|filename
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|3
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|size_t
name|fread_orDie
parameter_list|(
name|void
modifier|*
name|buffer
parameter_list|,
name|size_t
name|sizeToRead
parameter_list|,
name|FILE
modifier|*
name|file
parameter_list|)
block|{
name|size_t
specifier|const
name|readSize
init|=
name|fread
argument_list|(
name|buffer
argument_list|,
literal|1
argument_list|,
name|sizeToRead
argument_list|,
name|file
argument_list|)
decl_stmt|;
if|if
condition|(
name|readSize
operator|==
name|sizeToRead
condition|)
return|return
name|readSize
return|;
comment|/* good */
if|if
condition|(
name|feof
argument_list|(
name|file
argument_list|)
condition|)
return|return
name|readSize
return|;
comment|/* good, reached end of file */
comment|/* error */
name|perror
argument_list|(
literal|"fread"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|4
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|size_t
name|fwrite_orDie
parameter_list|(
specifier|const
name|void
modifier|*
name|buffer
parameter_list|,
name|size_t
name|sizeToWrite
parameter_list|,
name|FILE
modifier|*
name|file
parameter_list|)
block|{
name|size_t
specifier|const
name|writtenSize
init|=
name|fwrite
argument_list|(
name|buffer
argument_list|,
literal|1
argument_list|,
name|sizeToWrite
argument_list|,
name|file
argument_list|)
decl_stmt|;
if|if
condition|(
name|writtenSize
operator|==
name|sizeToWrite
condition|)
return|return
name|sizeToWrite
return|;
comment|/* good */
comment|/* error */
name|perror
argument_list|(
literal|"fwrite"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|5
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|size_t
name|fclose_orDie
parameter_list|(
name|FILE
modifier|*
name|file
parameter_list|)
block|{
if|if
condition|(
operator|!
name|fclose
argument_list|(
name|file
argument_list|)
condition|)
return|return
literal|0
return|;
comment|/* error */
name|perror
argument_list|(
literal|"fclose"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|6
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|compressFile_orDie
parameter_list|(
specifier|const
name|char
modifier|*
name|fname
parameter_list|,
specifier|const
name|char
modifier|*
name|outName
parameter_list|,
name|int
name|cLevel
parameter_list|,
name|unsigned
name|frameSize
parameter_list|)
block|{
name|FILE
modifier|*
specifier|const
name|fin
init|=
name|fopen_orDie
argument_list|(
name|fname
argument_list|,
literal|"rb"
argument_list|)
decl_stmt|;
name|FILE
modifier|*
specifier|const
name|fout
init|=
name|fopen_orDie
argument_list|(
name|outName
argument_list|,
literal|"wb"
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|buffInSize
init|=
name|ZSTD_CStreamInSize
argument_list|()
decl_stmt|;
comment|/* can always read one full block */
name|void
modifier|*
specifier|const
name|buffIn
init|=
name|malloc_orDie
argument_list|(
name|buffInSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|buffOutSize
init|=
name|ZSTD_CStreamOutSize
argument_list|()
decl_stmt|;
comment|/* can always flush a full block */
name|void
modifier|*
specifier|const
name|buffOut
init|=
name|malloc_orDie
argument_list|(
name|buffOutSize
argument_list|)
decl_stmt|;
name|ZSTD_seekable_CStream
modifier|*
specifier|const
name|cstream
init|=
name|ZSTD_seekable_createCStream
argument_list|()
decl_stmt|;
if|if
condition|(
name|cstream
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ZSTD_seekable_createCStream() error \n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|10
argument_list|)
expr_stmt|;
block|}
name|size_t
specifier|const
name|initResult
init|=
name|ZSTD_seekable_initCStream
argument_list|(
name|cstream
argument_list|,
name|cLevel
argument_list|,
literal|1
argument_list|,
name|frameSize
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|initResult
argument_list|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ZSTD_seekable_initCStream() error : %s \n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|initResult
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|11
argument_list|)
expr_stmt|;
block|}
name|size_t
name|read
decl_stmt|,
name|toRead
init|=
name|buffInSize
decl_stmt|;
while|while
condition|(
operator|(
name|read
operator|=
name|fread_orDie
argument_list|(
name|buffIn
argument_list|,
name|toRead
argument_list|,
name|fin
argument_list|)
operator|)
condition|)
block|{
name|ZSTD_inBuffer
name|input
init|=
block|{
name|buffIn
block|,
name|read
block|,
literal|0
block|}
decl_stmt|;
while|while
condition|(
name|input
operator|.
name|pos
operator|<
name|input
operator|.
name|size
condition|)
block|{
name|ZSTD_outBuffer
name|output
init|=
block|{
name|buffOut
block|,
name|buffOutSize
block|,
literal|0
block|}
decl_stmt|;
name|toRead
operator|=
name|ZSTD_seekable_compressStream
argument_list|(
name|cstream
argument_list|,
operator|&
name|output
argument_list|,
operator|&
name|input
argument_list|)
expr_stmt|;
comment|/* toRead is guaranteed to be<= ZSTD_CStreamInSize() */
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|toRead
argument_list|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ZSTD_seekable_compressStream() error : %s \n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|toRead
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|12
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|toRead
operator|>
name|buffInSize
condition|)
name|toRead
operator|=
name|buffInSize
expr_stmt|;
comment|/* Safely handle case when `buffInSize` is manually changed to a value< ZSTD_CStreamInSize()*/
name|fwrite_orDie
argument_list|(
name|buffOut
argument_list|,
name|output
operator|.
name|pos
argument_list|,
name|fout
argument_list|)
expr_stmt|;
block|}
block|}
while|while
condition|(
literal|1
condition|)
block|{
name|ZSTD_outBuffer
name|output
init|=
block|{
name|buffOut
block|,
name|buffOutSize
block|,
literal|0
block|}
decl_stmt|;
name|size_t
specifier|const
name|remainingToFlush
init|=
name|ZSTD_seekable_endStream
argument_list|(
name|cstream
argument_list|,
operator|&
name|output
argument_list|)
decl_stmt|;
comment|/* close stream */
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|remainingToFlush
argument_list|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ZSTD_seekable_endStream() error : %s \n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|remainingToFlush
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|13
argument_list|)
expr_stmt|;
block|}
name|fwrite_orDie
argument_list|(
name|buffOut
argument_list|,
name|output
operator|.
name|pos
argument_list|,
name|fout
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|remainingToFlush
condition|)
break|break;
block|}
name|ZSTD_seekable_freeCStream
argument_list|(
name|cstream
argument_list|)
expr_stmt|;
name|fclose_orDie
argument_list|(
name|fout
argument_list|)
expr_stmt|;
name|fclose_orDie
argument_list|(
name|fin
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buffIn
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buffOut
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
specifier|const
name|char
modifier|*
name|createOutFilename_orDie
parameter_list|(
specifier|const
name|char
modifier|*
name|filename
parameter_list|)
block|{
name|size_t
specifier|const
name|inL
init|=
name|strlen
argument_list|(
name|filename
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|outL
init|=
name|inL
operator|+
literal|5
decl_stmt|;
name|void
modifier|*
name|outSpace
init|=
name|malloc_orDie
argument_list|(
name|outL
argument_list|)
decl_stmt|;
name|memset
argument_list|(
name|outSpace
argument_list|,
literal|0
argument_list|,
name|outL
argument_list|)
expr_stmt|;
name|strcat
argument_list|(
name|outSpace
argument_list|,
name|filename
argument_list|)
expr_stmt|;
name|strcat
argument_list|(
name|outSpace
argument_list|,
literal|".zst"
argument_list|)
expr_stmt|;
return|return
operator|(
specifier|const
name|char
operator|*
operator|)
name|outSpace
return|;
block|}
end_function

begin_function
name|int
name|main
parameter_list|(
name|int
name|argc
parameter_list|,
specifier|const
name|char
modifier|*
modifier|*
name|argv
parameter_list|)
block|{
specifier|const
name|char
modifier|*
specifier|const
name|exeName
init|=
name|argv
index|[
literal|0
index|]
decl_stmt|;
if|if
condition|(
name|argc
operator|!=
literal|3
condition|)
block|{
name|printf
argument_list|(
literal|"wrong arguments\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"usage:\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%s FILE FRAME_SIZE\n"
argument_list|,
name|exeName
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
block|{
specifier|const
name|char
modifier|*
specifier|const
name|inFileName
init|=
name|argv
index|[
literal|1
index|]
decl_stmt|;
name|unsigned
specifier|const
name|frameSize
init|=
operator|(
name|unsigned
operator|)
name|atoi
argument_list|(
name|argv
index|[
literal|2
index|]
argument_list|)
decl_stmt|;
specifier|const
name|char
modifier|*
specifier|const
name|outFileName
init|=
name|createOutFilename_orDie
argument_list|(
name|inFileName
argument_list|)
decl_stmt|;
name|compressFile_orDie
argument_list|(
name|inFileName
argument_list|,
name|outFileName
argument_list|,
literal|5
argument_list|,
name|frameSize
argument_list|)
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function

end_unit

