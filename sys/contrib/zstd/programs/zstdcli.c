...
define|#
directive|define
name|LDM_PARAM_DEFAULT
value|9999
end_define

begin_comment
comment|/* Default for parameters where 0 is valid */
end_comment
...
modifier|...
parameter_list|)
value|fprintf(g_displayOut, __VA_ARGS__)
end_define
...
name|DISPLAY
argument_list|(
literal|" -f     : overwrite output without prompting and (de)compress links \n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
...
name|DISPLAY
argument_list|(
literal|" -v     : verbose mode; specify multiple times to increase verbosity\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
...
name|DISPLAY
argument_list|(
literal|"--long[=#]  : enable long distance matching with given window log (default : %u)\n"
argument_list|,
name|g_defaultMaxWindowLog
argument_list|)
expr_stmt|;
...
name|DISPLAY
argument_list|(
literal|" -B#    : select size of each job (default:0==automatic) \n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|DISPLAY
argument_list|(
literal|"--format=lz4 : compress files to the .lz4 format \n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|DISPLAY
argument_list|(
literal|"--[no-]sparse : sparse mode (default:disabled)\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|DISPLAY
argument_list|(
literal|"--train-cover[=k=#,d=#,steps=#] : use the cover algorithm with optional args\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|"--train-legacy[=s=#] : use the legacy algorithm with selectivity (default: %u)\n"
argument_list|,
name|g_defaultSelectivityLevel
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*! exeNameMatch() :     @return : a non-zero value if exeName matches test, excluding the extension    */
end_comment
...
begin_comment
comment|/**  * parseCoverParameters() :  * reads cover parameters from *stringPtr (e.g. "--train-cover=k=48,d=8,steps=32") into *params  * @return 1 means that cover parameters were correct  * @return 0 in case of malformed parameters  */
end_comment
...
begin_comment
comment|/**  * parseLegacyParameters() :  * reads legacy dictioanry builter parameters from *stringPtr (e.g. "--train-legacy=selectivity=8") into *selectivity  * @return 1 means that legacy dictionary builder parameters were correct  * @return 0 in case of malformed parameters  */
end_comment

begin_function
specifier|static
name|unsigned
name|parseLegacyParameters
parameter_list|(
specifier|const
name|char
modifier|*
...
continue|continue;
block|}
comment|/* kept available for compatibility with old syntax ; will be removed one day */
if|if
condition|(
operator|!
name|strcmp
...
literal|1
expr_stmt|;
comment|/* Parse optional window log */
if|if
condition|(
operator|*
name|argument
...
condition|)
block|{
comment|/* Invalid character following --long */
name|CLEAN_RETURN
argument_list|(
name|badusage
argument_list|(
...
expr_stmt|;
block|}
comment|/* Only set windowLog if not already set by --zstd */
if|if
condition|(
name|compressionParams
operator|.
...
expr_stmt|;
break|break;
comment|/* keep source file (default) */
case|case
literal|'k'
case|:
name|FIO_setRemoveSrcFile
...
argument_list|)
expr_stmt|;
name|BMK_setLdmMinMatch
argument_list|(
name|g_ldmMinMatch
argument_list|)
expr_stmt|;
name|BMK_setLdmHashLog
argument_list|(
name|g_ldmHashLog
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
name|FIO_setLdmHashLog
argument_list|(
name|g_ldmHashLog
argument_list|)
expr_stmt|;
name|FIO_setLdmMinMatch
argument_list|(
name|g_ldmMinMatch
argument_list|)
expr_stmt|;
