...
begin_comment
comment|/*  * Disable the "cloner" event handler;  we are getting interface  * events before the firewall is fully initiallized and also no vnet  * information thus leading to uninitialised memory accesses.  * In addition it is unclear why we need it in first place.  * If it turns out to be needed, well need a dedicated event handler  * for it to deal with the ifc and the correct vnet.  */
end_comment

begin_endif
unit|static eventhandler_tag ipf_clonetag;
endif|#
directive|endif
end_endif
...
name|DT2
argument_list|(
name|ipf_fi_bad_checkv4sum_csum_ip_checked
argument_list|,
name|fr_info_t
operator|*
argument_list|,
...
condition|)
block|{
comment|/* Depending on the driver, UDP may have zero checksum */
if|if
condition|(
name|fin
operator|->
...
condition|)
block|{
comment|/* 				 * we're good no matter what the hardware 				 * checksum flags and csum_data say (handling 				 * of csum_data for zero UDP checksum is not 				 * consistent across all drivers) 				 */
name|fin
operator|->
name|fin_cksum
operator|=
...
name|DT2
argument_list|(
name|ipf_fi_bad_checkv4sum_sum
argument_list|,
name|fr_info_t
operator|*
argument_list|,
...
name|DT2
argument_list|(
name|ipf_fi_bad_checkv4sum_manual
argument_list|,
name|fr_info_t
operator|*
argument_list|,
...
name|DT2
argument_list|(
name|ipf_fi_bad_checkv4sum_checkl4sum
argument_list|,
name|fr_info_t
operator|*
argument_list|,
...
name|DT2
argument_list|(
name|ipf_fi_bad_checkv6sum_checkl4sum
argument_list|,
name|fr_info_t
operator|*
argument_list|,
...
directive|if
literal|0
block|ipf_clonetag  = EVENTHANDLER_REGISTER(if_clone_event, ipf_ifevent, \ 					       NULL, EVENTHANDLER_PRI_ANY);
endif|#
directive|endif
block|}
end_function
...
directive|if
literal|0
block|if (ipf_clonetag != NULL) { 		EVENTHANDLER_DEREGISTER(if_clone_event, ipf_clonetag); 	}
endif|#
directive|endif
block|}
end_function
