...
begin_comment
comment|/*  * Copyright (C) 1995-2002 by Darren Reed.  *  * See the IPFILTER.LICENCE file for details on licencing.  */
end_comment
...
begin_comment
comment|/*  * flush state tables.  two actions currently defined:  * which == 0 : flush all state table entries  * which == 1 : flush TCP connections which have started to close but are  *	        stuck for some reason.  * which == 2 : flush TCP connections which have been idle for a long time,  *              starting at> 4 days idle and working back in successive half-  *              days to at most 12 hours old.  */
end_comment
...
expr_stmt|;
block|}
comment|/* 	 * Asked to remove inactive entries, try again if first attempt 	 * failed.  In this case, 86400 is half a day because the counter is 	 * activated every half second. 	 */
if|if
condition|(
operator|(
name|which
...
begin_comment
comment|/*  * Copy out state information from the kernel to a user space process.  */
end_comment
...
begin_comment
comment|/*  * Insert a state table entry manually.  */
end_comment
...
condition|)
block|{
comment|/* 			 * So you can do keep state with neighbour discovery. 			 */
name|flags
operator||=
name|FI_W_DADDR
expr_stmt|;
...
operator|)
expr_stmt|;
comment|/* If we don't find wscale here, we need to clear it */
name|wscale
operator|=
operator|-
literal|2
expr_stmt|;
comment|/* Termination condition picked such that opt[0 .. 2] exist */
while|while
condition|(
operator|(
name|opt
...
break|break;
default|default:
comment|/* Unknown options must be two bytes+ */
if|if
condition|(
name|opt
index|[
...
begin_comment
comment|/*  * Match a state table entry against an IP packet.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * We make an fin entry to be able to feed it to 	 * matchsrcdst.  Note that not all fields are encessary 	 * but this is the cleanest way. Note further that we 	 * fill in fin_mp such that if someone uses it we'll get 	 * a kernel panic. fr_matchsrcdst does not use this. 	 */
name|ofin
operator|.
name|fin_ifp
operator|=
...
literal|4
expr_stmt|;
comment|/* 	 * watch out here, as ip is in host order and oip in network 	 * order. Any change we make must be undone afterwards, like 	 * oip->ip_off - it is still in network byte order so fix it. 	 */
name|savelen
operator|=
name|oip
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * Reset the short flag here because in fr_matchsrcdst() the flags 	 * for the current packet (fin_fl) are compared against * those for 	 * the existing session. 	 */
name|ofin
operator|.
name|fin_fl
operator|&=
...
begin_comment
comment|/*  * Move a state hash table entry from its old location at is->is_hv to  * its new location, indexed by hv % fr_statesize.  */
end_comment
...
name|s_addr
expr_stmt|;
comment|/* 	 * Search the hash table for matching packet header info. 	 * At the bottom of this switch statement, the following is expected: 	 * is == NULL, no lock on ipf_state is held. 	 * is != NULL, a lock on ipf_state is held. 	 */
name|v
operator|=
name|fin
operator|->
...
begin_comment
comment|/*  * Sync. state entries.  If interfaces come or go or just change position,  * this is needed.  */
end_comment
...
expr_stmt|;
block|}
comment|/* 		 * The next piece of code makes it possible to get 		 * already established connections into the state table 		 * after a restart or reload of the filter rules; this 		 * does not work when a strict 'flags S keep state' is 		 * used for tcp connections of course, however, use a 		 * lower time-out so the state disappears quickly if 		 * the other side does not pick it up. 		 */
elseif|else
if|if
condition|(
operator|!
...
condition|)
block|{
comment|/* 			 * A retransmitted SYN packet.  We do not reset the 			 * timeout here to fr_tcptimeout because a connection 			 * connect timeout does not renew after every packet 			 * that is sent.  We need to set newage to something 			 * to indicate the packet has passed the check for its 			 * flags being valid in the TCP FSM. 			 */
name|newage
operator|=
operator|*
name|age
