...
index|[]
init|=
literal|"@(#)$Id: ip_state.c,v 2.186.2.36 2005/12/04 22:25:36 darrenr Exp $"
decl_stmt|;
end_decl_stmt
...
return|;
block|}
comment|/* 	 * If a "keep state" rule has reached the maximum number of references 	 * to it, then schedule an automatic flush in case we can clear out 	 * some "dead old wood". 	 */
name|MUTEX_ENTER
argument_list|(
operator|&
name|fr
...
directive|if
literal|0
comment|/* 	 * XXX - This is a kludge is here because IPFilter doesn't track SACK 	 * options in TCP packets.  This is not a trivial to do if one is to 	 * consider the performance impact of it.  So instead, if the 	 * receiver has said SACK is ok, double the allowed window size. 	 * This is disabled for testing of another workaround for a problem 	 * with Microsoft Windows - see below. 	 */
block|if ((tdata->td_winflags& TCP_SACK_PERMIT) != 0) 		win *= 2;
endif|#
directive|endif
