...
begin_comment
comment|/*  * Copyright (C) 1993-2001, 2003 by Darren Reed.  *  * See the IPFILTER.LICENCE file for details on licencing.  *  * @(#)ip_compat.h	1.8 1/14/96  * $FreeBSD$  * Id: ip_compat.h,v 2.142.2.57 2007/10/10 09:51:42 darrenr Exp $  */
end_comment
...
begin_comment
comment|/* These 5 are defined in<inet/ip.h> and<netinet/ip.h> */
end_comment
...
name|y
parameter_list|)
value|rw_init(&(x)->ipf_lk, (y),  \ 						RW_DRIVER, NULL)
end_define
...
name|y
parameter_list|)
value|mutex_init(&(x)->ipf_lk, (y), \ 						   MUTEX_DRIVER, NULL)
end_define
...
parameter_list|)
define|\
value|(void) strncpy(b, ((qif_t *)x)->qf_name, \ 					       LIFNAMSIZ)
end_define
...
name|x
parameter_list|)
value|uniqtime((struct timeval *)x)
end_define
...
name|y
parameter_list|)
value|if (x) { printf y; cmn_err(CE_PANIC, "ipf_panic"); }
end_define
...
include|#
directive|include
file|"../netinet/ip_info.h"
end_include

begin_comment
comment|/*  * According to /usr/include/sys/spinlock.h on HP-UX 11.00, these functions  * are available.  Attempting to use them actually results in unresolved  * symbols when it comes time to load the module.  * This has been fixed!  Yipee!  */
end_comment
...
name|c
parameter_list|)
value|bcopy((caddr_t)b, (caddr_t)a, c)
end_define
...
begin_comment
comment|/*  * The read-write lock implementation in HP-UX 11.0 is crippled - it can  * only be used by threads working in a user context!  * This has been fixed!  Yipee! (Or at least it does in 11.00, not 11.11..)  */
end_comment
...
name|y
parameter_list|)
value|rwlock_init4(&(x)->ipf_lk, 0, RWLCK_CANSLEEP, 0, y)
end_define
...
name|y
parameter_list|)
value|lock_init3(&(x)->ipf_lk, 0, 1, 0, 0, y)
end_define
...
name|n
parameter_list|)
value|{ lock_t *_l = get_sleep_lock((caddr_t)id); \ 				  sleep(id, PZERO+1); \ 				  spinunlock(_l); \ 				}
end_define
...
name|x
parameter_list|)
value|{ lock_t *_l = get_sleep_lock((caddr_t)id); \ 				  wakeup(id + x); \ 				  spinunlock(_l); \ 				}
end_define
...
name|b
parameter_list|)
value|MALLOC((a), b, sizeof(*(a)), M_IOSYS, M_NOWAIT)
end_define
...
name|c
parameter_list|)
value|MALLOC((a), b, (c), M_IOSYS, M_NOWAIT)
end_define
...
name|y
parameter_list|)
value|mutex_init(&(x)->ipf_lk,  \ 						   MUTEX_DEFAULT, y)
end_define
...
name|x
parameter_list|)
value|UNLOCK((x)->ipf_lk.l, (x)->ipf_lk.pl);
end_define
...
name|y
parameter_list|)
value|(x)->ipf_lk.l = LOCK_ALLOC((uchar_t)-1, IPF_LOCK_PL, (lkinfo_t *)-1, KM_NOSLEEP)
end_define
...
name|x
parameter_list|)
value|(x)->ipf_lk.pl = LOCK((x)->ipf_lk.l, \ 							      IPF_LOCK_PL);
end_define
...
name|x
parameter_list|)
value|lock_write_to_read(&(x)->ipf_lk)
end_define
...
name|y
parameter_list|)
value|simple_lock_init(&(x)->ipf_lk)
end_define
...
name|x
parameter_list|)
value|simple_lock_terminate(&(x)->ipf_lk)
end_define
...
name|b
parameter_list|)
value|MALLOC((a), b, sizeof(*(a)), M_PFILT, M_NOWAIT)
end_define
...
name|c
parameter_list|)
value|MALLOC((a), b, (c), M_PFILT, \ 					    ((c)> 4096) ? M_WAITOK : M_NOWAIT)
end_define
...
name|x
parameter_list|)
value|do { \ 					    if ((x)->ipf_lk.sx_cnt< 0) \ 						sx_xunlock(&(x)->ipf_lk); \ 					    else \ 						sx_sunlock(&(x)->ipf_lk); \ 					} while (0)
end_define
...
name|c
parameter_list|)
value|copy_from_user((caddr_t)(b), (caddr_t)(a), (c))
end_define
...
name|c
parameter_list|)
value|copy_to_user((caddr_t)(b), (caddr_t)(a), (c))
end_define
...
name|x
parameter_list|)
value|do_gettimeofday((struct timeval *)x)
end_define
...
name|x
parameter_list|)
value|bzero(&(x)->ipf_lk, sizeof((x)->ipf_lk))
end_define
...
name|m
parameter_list|)
value|skb_clone((m), in_interrupt() ? GFP_ATOMIC : \ 								GFP_KERNEL)
end_define
...
name|t
parameter_list|)
value|(x) = (t)kmalloc(sizeof(*(x)), \ 				    in_interrupt() ? GFP_ATOMIC : GFP_KERNEL)
end_define
...
name|s
parameter_list|)
value|(x) = (t)kmalloc((s), \ 				    in_interrupt() ? GFP_ATOMIC : GFP_KERNEL)
end_define
...
name|y
parameter_list|)
value|lock_alloc(&(x)->ipf_lk, \ 						   LOCK_ALLOC_PIN, \ 						   (u_short)y, 0); \ 					lock_init((x)->ipf_lk, TRUE)
end_define
...
name|y
parameter_list|)
value|lock_alloc(&(x)->ipf_lk, \ 						   LOCK_ALLOC_PIN, \ 						   (u_short)y, 0); \ 					simple_lock_init((x)->ipf_lk)
end_define
...
name|c
parameter_list|)
value|MALLOC((a), b, (c), M_TEMP, \ 					    ((c)> 4096) ? M_WAITOK : M_NOWAIT)
end_define
