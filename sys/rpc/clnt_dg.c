...
begin_comment
comment|/*  * A pending RPC request which awaits a reply.  */
end_comment
...
goto|;
block|}
comment|/* 	 * Check to see if we got an upcall while waiting for the 	 * lock. In both these cases, the request has been removed 	 * from cs->cs_pending. 	 */
if|if
condition|(
name|cr
operator|.
...
argument_list|,
operator|(
literal|"clnt_dg_destroy(): closing a socket "
literal|"shared with other clients"
operator|)
argument_list|)
expr_stmt|;
name|so
...
condition|)
block|{
comment|/* 				 * This one matches. We snip it out of 				 * the pending list and leave the 				 * reply mbuf in cr->cr_mrep. Set the 				 * XID to zero so that clnt_dg_call 				 * can know not to repeat the 				 * TAILQ_REMOVE. 				 */
name|TAILQ_REMOVE
argument_list|(
operator|&
name|cs
