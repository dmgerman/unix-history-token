...
name|LOG_DEBUG
argument_list|,
literal|"ip_mforward: src %x, dst %x, ifp %x (%s%d)\n"
argument_list|,
name|ntohl
argument_list|(
name|ip
...
condition|)
return|return;
comment|/*      * Make sure the header isn't in an cluster, because the sharing      * in clusters defeats the whole purpose of making the copy above.      */
name|mb_copy
operator|=
name|m_pullup
argument_list|(
...
directive|ifdef
name|MROUTE_LKM
name|X_multiencap_decap
parameter_list|(
name|m
parameter_list|)
else|#
directive|else
function|multiencap_decap
parameter_list|(
name|m
parameter_list|)
endif|#
