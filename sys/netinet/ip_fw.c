...
literal|0
argument_list|,
literal|"Set rule number, below which rules are permanent"
argument_list|)
expr_stmt|;
end_expr_stmt
...
condition|)
block|{
comment|/* bridged packets are as on the wire */
name|ip_off
operator|=
name|ntohs
argument_list|(
...
continue|continue;
block|}
comment|/* 		 * For packets which matched the MAC check, we do not need 		 * to continue, this is a valid match. 		 * For not-ip packets, the rule does not apply. 		 */
if|if
condition|(
name|f
operator|->
...
condition|)
continue|continue;
comment|/* 		 * For matching addresses, tif != NULL means we matched 		 * the address we requested (either "me" or addr/mask). 		 * Then the check for "xxx" or "not xxx" can be done 		 * with an XOR. 		 */
comment|/* source address --	mandatory */
if|if
condition|(
name|f
operator|->
...
condition|)
continue|continue;
comment|/* Check IP header values */
if|if
condition|(
name|f
operator|->
...
argument_list|(
operator|(
literal|"%s cannot use uid/gid logic on non-TCP/UDP\n"
operator|,
name|err_prefix
operator|)
argument_list|)
