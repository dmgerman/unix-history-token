...
control|)
block|{
comment|/* 	     * logic can become complex here, so we split tests. 	     * First, test if we match any chain, 	     * then make sure the rule is expired or we want to kill it, 	     * and possibly more in the future. 	     */
name|int
name|zap
init|=
operator|(
...
begin_comment
comment|/*  * Install dynamic state.  * There are different types of dynamic rules which can be installed.  * The type is in chain->dyn_type.  * Type 0 (default) is a bidirectional rule  *  * Returns 1 (failure) if state is not installed.  */
end_comment
