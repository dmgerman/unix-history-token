...
block|hash packet, lookup into dynamic session ids. 	If found, set rule to the parent rule, update timers, 	and goto rnd_then_got_match. rule != NULL means that this is 	a match of a dynamic rule.
endif|#
directive|endif
comment|/* stateful ipfw */
for|for
...
literal|0
comment|/* stateful ipfw */
block|if this is not a dynamic match and we have keep-state, 	install a new dynamic entry if needed. It uses proto, 	swap ip and ports. 		if (rule&& f->fw_flg& IP_FW_F_KEEP_S) { 			f = rule ; 			install_state(); 		}
endif|#
directive|endif
comment|/* stateful ipfw */
