...
name|chain
return|;
comment|/*      * note the above matches any rule>= the jump target. I      * don't think this is a particularly good idea, but it is      * how ipfw always worked.      */
return|return
name|me
operator|->
name|chain
...
condition|)
block|{
comment|/* 		 * If we've been asked to start at a given rule immediatly, 		 * do so. 		 */
if|if
condition|(
name|skipto
operator|>=
...
directive|if
literal|0
block|printf("-- ip_fw: rule %d(%d) for %6D<- %6D type 0x%04x\n", 			    f->fw_number, IP_FW_GETNSRCP(f), 			    eh->ether_dhost, ".", eh->ether_shost, ".", 			    ntohs(eh->ether_type) );
endif|#
directive|endif
