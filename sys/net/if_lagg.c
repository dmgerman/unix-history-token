...
begin_comment
comment|/*  * This action handler shall be called from taskqueue handler for each  * submitted operation  */
end_comment
...
begin_comment
comment|/*  * lagg llq action Table: Called at the taskqueue context for each  * submitted operations.  * Contents SHOULD be in sync with lagg_llq_idx index.  * New entries shall be appended.  */
end_comment
...
name|lagg_llq_action_lladdr
block|,
comment|/* Maps to LAGG_LLQ_LLADDR index */
name|lagg_llq_action_mtu
block|,
comment|/* Maps to LAGG_LLQ_MTU index */
block|}
decl_stmt|;
end_decl_stmt
...
argument|cmn_llq
argument_list|,
argument|&sc->sc_llq[LAGG_LLQ_LLADDR]
argument_list|,
...
begin_comment
comment|/*  * Set the interface MTU, MAC address from a taskqueue to avoid a LOR.  *  * Set noinline to be dtrace-friendly  */
end_comment
...
name|err
decl_stmt|;
comment|/* Set the new MTU on the lagg interface */
name|LAGG_WLOCK
argument_list|(
name|sc
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* 	 * Traverse the queue and set the mtu on each ifp. It is safe to do 	 * unlocked as we have the only reference to it. 	 */
name|err
operator|=
name|EIO
expr_stmt|;
comment|/* In case the list is empty. */
name|llq
operator|=
name|first
expr_stmt|;
...
name|llq
expr_stmt|;
comment|/* Set the new MTU on the physical interface */
name|err
operator|=
call|(
modifier|*
...
name|llq_ifp
argument_list|,
literal|"Failed to change MTU from %d to %d (err %d)\n"
argument_list|,
name|llq_ctxt
operator|->
name|llq_old_mtu
...
condition|)
block|{
comment|/* Restore the old MTU on the lagg interface */
name|LAGG_WLOCK
argument_list|(
name|sc
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Restore the old MTU on the physical interface */
name|llq
operator|=
name|first
expr_stmt|;
...
name|llq_ifp
argument_list|,
literal|"Failed to restore MTU to %d (err %d)\n"
argument_list|,
name|llq_ctxt
operator|->
name|llq_old_mtu
...
block|}
block|}
comment|/* Free the MTU LLQ entries */
name|_lagg_free_llq_entries
argument_list|(
name|first
argument_list|)
...
name|err
expr_stmt|;
comment|/* Signal for command completion */
name|cv_signal
argument_list|(
operator|&
name|sc
...
argument_list|)
expr_stmt|;
comment|/* Wait for the command completion */
name|cv_wait
argument_list|(
operator|&
name|sc
...
name|SIOCSIFCAP
case|:
comment|/* Do not allow the CAPs to be directly changed. */
name|error
operator|=
name|EINVAL
expr_stmt|;
