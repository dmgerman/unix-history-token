...
argument_list|)
expr_stmt|;
comment|/* Get common enabled capabilities for the lagg ports */
name|ena
operator|=
operator|~
literal|0
...
operator|)
expr_stmt|;
comment|/* 	 * Apply common enabled capabilities back to the lagg ports. 	 * May require several iterations if they are dependent. 	 */
do|do
block|{
name|pena
operator|=
...
condition|)
do|;
comment|/* Get other capabilities from the lagg ports */
name|cap
operator|=
operator|~
literal|0
...
argument_list|)
expr_stmt|;
comment|/* 		 * Update lladdr for each port (new primary needs update 		 * as well, to switch from old lladdr to its 'real' one) 		 */
name|SLIST_FOREACH
argument_list|(
argument|lp_ptr
argument_list|,
...
name|SIOCSIFMTU
case|:
comment|/* Do not allow the MTU to be directly changed */
name|error
operator|=
name|EINVAL
expr_stmt|;
...
name|lp
operator|=
name|lacp_select_tx_port_by_hash
argument_list|(
name|sc
argument_list|,
name|params
