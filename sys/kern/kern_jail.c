begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * ----------------------------------------------------------------------------  * "THE BEER-WARE LICENSE" (Revision 42):  *<phk@FreeBSD.ORG> wrote this file.  As long as you retain this notice you  * can do whatever you want with this stuff. If we meet some day, and you think  * this stuff is worth it, you can buy me a beer in return.   Poul-Henning Kamp  * ----------------------------------------------------------------------------  */
end_comment

begin_include
include|#
directive|include
file|<sys/cdefs.h>
end_include

begin_expr_stmt
name|__FBSDID
argument_list|(
literal|"$FreeBSD$"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/sysproto.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/taskqueue.h>
end_include

begin_include
include|#
directive|include
file|<sys/jail.h>
end_include

begin_include
include|#
directive|include
file|<sys/lock.h>
end_include

begin_include
include|#
directive|include
file|<sys/mutex.h>
end_include

begin_include
include|#
directive|include
file|<sys/namei.h>
end_include

begin_include
include|#
directive|include
file|<sys/queue.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/syscallsubr.h>
end_include

begin_include
include|#
directive|include
file|<sys/sysctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<net/if.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_expr_stmt
name|MALLOC_DEFINE
argument_list|(
name|M_PRISON
argument_list|,
literal|"prison"
argument_list|,
literal|"Prison structures"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|SYSCTL_DECL
argument_list|(
name|_security
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|SYSCTL_NODE
argument_list|(
name|_security
argument_list|,
name|OID_AUTO
argument_list|,
name|jail
argument_list|,
name|CTLFLAG_RW
argument_list|,
literal|0
argument_list|,
literal|"Jail rules"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_macro
name|mp_fixme
argument_list|(
literal|"these variables need a lock"
argument_list|)
end_macro

begin_decl_stmt
name|int
name|jail_set_hostname_allowed
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_expr_stmt
name|SYSCTL_INT
argument_list|(
name|_security_jail
argument_list|,
name|OID_AUTO
argument_list|,
name|set_hostname_allowed
argument_list|,
name|CTLFLAG_RW
argument_list|,
operator|&
name|jail_set_hostname_allowed
argument_list|,
literal|0
argument_list|,
literal|"Processes in jail can set their hostnames"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|jail_socket_unixiproute_only
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_expr_stmt
name|SYSCTL_INT
argument_list|(
name|_security_jail
argument_list|,
name|OID_AUTO
argument_list|,
name|socket_unixiproute_only
argument_list|,
name|CTLFLAG_RW
argument_list|,
operator|&
name|jail_socket_unixiproute_only
argument_list|,
literal|0
argument_list|,
literal|"Processes in jail are limited to creating UNIX/IPv4/route sockets only"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|jail_sysvipc_allowed
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_expr_stmt
name|SYSCTL_INT
argument_list|(
name|_security_jail
argument_list|,
name|OID_AUTO
argument_list|,
name|sysvipc_allowed
argument_list|,
name|CTLFLAG_RW
argument_list|,
operator|&
name|jail_sysvipc_allowed
argument_list|,
literal|0
argument_list|,
literal|"Processes in jail can use System V IPC primitives"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|jail_getfsstatroot_only
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_expr_stmt
name|SYSCTL_INT
argument_list|(
name|_security_jail
argument_list|,
name|OID_AUTO
argument_list|,
name|getfsstate_getfsstatroot_only
argument_list|,
name|CTLFLAG_RW
argument_list|,
operator|&
name|jail_getfsstatroot_only
argument_list|,
literal|0
argument_list|,
literal|"Processes see only their root file system in getfsstat()"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|jail_list_allowed
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_expr_stmt
name|SYSCTL_INT
argument_list|(
name|_security_jail
argument_list|,
name|OID_AUTO
argument_list|,
name|list_allowed
argument_list|,
name|CTLFLAG_RW
argument_list|,
operator|&
name|jail_list_allowed
argument_list|,
literal|0
argument_list|,
literal|"Processes in jail can access system jail list"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* allprison, lastprid, and prisoncount are protected by allprison_mtx. */
end_comment

begin_decl_stmt
name|struct
name|prisonlist
name|allprison
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mtx
name|allprison_mtx
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|lastprid
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|prisoncount
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_function_decl
specifier|static
name|void
name|init_prison
parameter_list|(
name|void
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|prison_complete
parameter_list|(
name|void
modifier|*
name|context
parameter_list|,
name|int
name|pending
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|struct
name|prison
modifier|*
name|prison_find
parameter_list|(
name|int
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|sysctl_jail_list
parameter_list|(
name|SYSCTL_HANDLER_ARGS
parameter_list|)
function_decl|;
end_function_decl

begin_function
specifier|static
name|void
name|init_prison
parameter_list|(
name|void
modifier|*
name|data
name|__unused
parameter_list|)
block|{
name|mtx_init
argument_list|(
operator|&
name|allprison_mtx
argument_list|,
literal|"allprison"
argument_list|,
name|NULL
argument_list|,
name|MTX_DEF
argument_list|)
expr_stmt|;
name|LIST_INIT
argument_list|(
operator|&
name|allprison
argument_list|)
expr_stmt|;
block|}
end_function

begin_expr_stmt
name|SYSINIT
argument_list|(
name|prison
argument_list|,
name|SI_SUB_INTRINSIC
argument_list|,
name|SI_ORDER_ANY
argument_list|,
name|init_prison
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
end_expr_stmt

begin_comment
comment|/*  * MPSAFE  *  * struct jail_args {  *	struct jail *jail;  * };  */
end_comment

begin_function
name|int
name|jail
parameter_list|(
name|struct
name|thread
modifier|*
name|td
parameter_list|,
name|struct
name|jail_args
modifier|*
name|uap
parameter_list|)
block|{
name|struct
name|nameidata
name|nd
decl_stmt|;
name|struct
name|prison
modifier|*
name|pr
decl_stmt|,
modifier|*
name|tpr
decl_stmt|;
name|struct
name|jail
name|j
decl_stmt|;
name|struct
name|jail_attach_args
name|jaa
decl_stmt|;
name|int
name|error
decl_stmt|,
name|tryprid
decl_stmt|;
name|error
operator|=
name|copyin
argument_list|(
name|uap
operator|->
name|jail
argument_list|,
operator|&
name|j
argument_list|,
sizeof|sizeof
argument_list|(
name|j
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|j
operator|.
name|version
operator|!=
literal|0
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|MALLOC
argument_list|(
name|pr
argument_list|,
expr|struct
name|prison
operator|*
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|pr
argument_list|)
argument_list|,
name|M_PRISON
argument_list|,
name|M_WAITOK
operator||
name|M_ZERO
argument_list|)
expr_stmt|;
name|mtx_init
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|,
literal|"jail mutex"
argument_list|,
name|NULL
argument_list|,
name|MTX_DEF
argument_list|)
expr_stmt|;
name|pr
operator|->
name|pr_ref
operator|=
literal|1
expr_stmt|;
name|error
operator|=
name|copyinstr
argument_list|(
name|j
operator|.
name|path
argument_list|,
operator|&
name|pr
operator|->
name|pr_path
argument_list|,
sizeof|sizeof
argument_list|(
name|pr
operator|->
name|pr_path
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
goto|goto
name|e_killmtx
goto|;
name|mtx_lock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|NDINIT
argument_list|(
operator|&
name|nd
argument_list|,
name|LOOKUP
argument_list|,
name|FOLLOW
operator||
name|LOCKLEAF
argument_list|,
name|UIO_SYSSPACE
argument_list|,
name|pr
operator|->
name|pr_path
argument_list|,
name|td
argument_list|)
expr_stmt|;
name|error
operator|=
name|namei
argument_list|(
operator|&
name|nd
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|mtx_unlock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
goto|goto
name|e_killmtx
goto|;
block|}
name|pr
operator|->
name|pr_root
operator|=
name|nd
operator|.
name|ni_vp
expr_stmt|;
name|VOP_UNLOCK
argument_list|(
name|nd
operator|.
name|ni_vp
argument_list|,
literal|0
argument_list|,
name|td
argument_list|)
expr_stmt|;
name|NDFREE
argument_list|(
operator|&
name|nd
argument_list|,
name|NDF_ONLY_PNBUF
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|error
operator|=
name|copyinstr
argument_list|(
name|j
operator|.
name|hostname
argument_list|,
operator|&
name|pr
operator|->
name|pr_host
argument_list|,
sizeof|sizeof
argument_list|(
name|pr
operator|->
name|pr_host
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
goto|goto
name|e_dropvnref
goto|;
name|pr
operator|->
name|pr_ip
operator|=
name|j
operator|.
name|ip_number
expr_stmt|;
name|pr
operator|->
name|pr_linux
operator|=
name|NULL
expr_stmt|;
name|pr
operator|->
name|pr_securelevel
operator|=
name|securelevel
expr_stmt|;
comment|/* Determine next pr_id and add prison to allprison list. */
name|mtx_lock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|tryprid
operator|=
name|lastprid
operator|+
literal|1
expr_stmt|;
if|if
condition|(
name|tryprid
operator|==
name|JAIL_MAX
condition|)
name|tryprid
operator|=
literal|1
expr_stmt|;
name|next
label|:
name|LIST_FOREACH
argument_list|(
argument|tpr
argument_list|,
argument|&allprison
argument_list|,
argument|pr_list
argument_list|)
block|{
if|if
condition|(
name|tpr
operator|->
name|pr_id
operator|==
name|tryprid
condition|)
block|{
name|tryprid
operator|++
expr_stmt|;
if|if
condition|(
name|tryprid
operator|==
name|JAIL_MAX
condition|)
block|{
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|error
operator|=
name|EAGAIN
expr_stmt|;
goto|goto
name|e_dropvnref
goto|;
block|}
goto|goto
name|next
goto|;
block|}
block|}
name|pr
operator|->
name|pr_id
operator|=
name|jaa
operator|.
name|jid
operator|=
name|lastprid
operator|=
name|tryprid
expr_stmt|;
name|LIST_INSERT_HEAD
argument_list|(
operator|&
name|allprison
argument_list|,
name|pr
argument_list|,
name|pr_list
argument_list|)
expr_stmt|;
name|prisoncount
operator|++
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|error
operator|=
name|jail_attach
argument_list|(
name|td
argument_list|,
operator|&
name|jaa
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
goto|goto
name|e_dropprref
goto|;
name|mtx_lock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|pr
operator|->
name|pr_ref
operator|--
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|td
operator|->
name|td_retval
index|[
literal|0
index|]
operator|=
name|jaa
operator|.
name|jid
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
name|e_dropprref
label|:
name|mtx_lock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|LIST_REMOVE
argument_list|(
name|pr
argument_list|,
name|pr_list
argument_list|)
expr_stmt|;
name|prisoncount
operator|--
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|e_dropvnref
label|:
name|mtx_lock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|vrele
argument_list|(
name|pr
operator|->
name|pr_root
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|e_killmtx
label|:
name|mtx_destroy
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|FREE
argument_list|(
name|pr
argument_list|,
name|M_PRISON
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * MPSAFE  *  * struct jail_attach_args {  *	int jid;  * };  */
end_comment

begin_function
name|int
name|jail_attach
parameter_list|(
name|struct
name|thread
modifier|*
name|td
parameter_list|,
name|struct
name|jail_attach_args
modifier|*
name|uap
parameter_list|)
block|{
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
name|struct
name|ucred
modifier|*
name|newcred
decl_stmt|,
modifier|*
name|oldcred
decl_stmt|;
name|struct
name|prison
modifier|*
name|pr
decl_stmt|;
name|int
name|error
decl_stmt|;
comment|/* 	 * XXX: Note that there is a slight race here if two threads 	 * in the same privileged process attempt to attach to two 	 * different jails at the same time.  It is important for 	 * user processes not to do this, or they might end up with 	 * a process root from one prison, but attached to the jail 	 * of another. 	 */
name|error
operator|=
name|suser
argument_list|(
name|td
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|p
operator|=
name|td
operator|->
name|td_proc
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|pr
operator|=
name|prison_find
argument_list|(
name|uap
operator|->
name|jid
argument_list|)
expr_stmt|;
if|if
condition|(
name|pr
operator|==
name|NULL
condition|)
block|{
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
return|return
operator|(
name|EINVAL
operator|)
return|;
block|}
name|pr
operator|->
name|pr_ref
operator|++
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|vn_lock
argument_list|(
name|pr
operator|->
name|pr_root
argument_list|,
name|LK_EXCLUSIVE
operator||
name|LK_RETRY
argument_list|,
name|td
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|error
operator|=
name|change_dir
argument_list|(
name|pr
operator|->
name|pr_root
argument_list|,
name|td
argument_list|)
operator|)
operator|!=
literal|0
condition|)
goto|goto
name|e_unlock
goto|;
ifdef|#
directive|ifdef
name|MAC
if|if
condition|(
operator|(
name|error
operator|=
name|mac_check_vnode_chroot
argument_list|(
name|td
operator|->
name|td_ucred
argument_list|,
name|pr
operator|->
name|pr_root
argument_list|)
operator|)
condition|)
goto|goto
name|e_unlock
goto|;
endif|#
directive|endif
name|VOP_UNLOCK
argument_list|(
name|pr
operator|->
name|pr_root
argument_list|,
literal|0
argument_list|,
name|td
argument_list|)
expr_stmt|;
name|change_root
argument_list|(
name|pr
operator|->
name|pr_root
argument_list|,
name|td
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|newcred
operator|=
name|crget
argument_list|()
expr_stmt|;
name|PROC_LOCK
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|oldcred
operator|=
name|p
operator|->
name|p_ucred
expr_stmt|;
name|setsugid
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|crcopy
argument_list|(
name|newcred
argument_list|,
name|oldcred
argument_list|)
expr_stmt|;
name|newcred
operator|->
name|cr_prison
operator|=
name|pr
expr_stmt|;
name|p
operator|->
name|p_ucred
operator|=
name|newcred
expr_stmt|;
name|PROC_UNLOCK
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|crfree
argument_list|(
name|oldcred
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
name|e_unlock
label|:
name|VOP_UNLOCK
argument_list|(
name|pr
operator|->
name|pr_root
argument_list|,
literal|0
argument_list|,
name|td
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|pr
operator|->
name|pr_ref
operator|--
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Returns a locked prison instance, or NULL on failure.  */
end_comment

begin_function
specifier|static
name|struct
name|prison
modifier|*
name|prison_find
parameter_list|(
name|int
name|prid
parameter_list|)
block|{
name|struct
name|prison
modifier|*
name|pr
decl_stmt|;
name|mtx_assert
argument_list|(
operator|&
name|allprison_mtx
argument_list|,
name|MA_OWNED
argument_list|)
expr_stmt|;
name|LIST_FOREACH
argument_list|(
argument|pr
argument_list|,
argument|&allprison
argument_list|,
argument|pr_list
argument_list|)
block|{
if|if
condition|(
name|pr
operator|->
name|pr_id
operator|==
name|prid
condition|)
block|{
name|mtx_lock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
return|return
operator|(
name|pr
operator|)
return|;
block|}
block|}
return|return
operator|(
name|NULL
operator|)
return|;
block|}
end_function

begin_function
name|void
name|prison_free
parameter_list|(
name|struct
name|prison
modifier|*
name|pr
parameter_list|)
block|{
name|mtx_lock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|pr
operator|->
name|pr_ref
operator|--
expr_stmt|;
if|if
condition|(
name|pr
operator|->
name|pr_ref
operator|==
literal|0
condition|)
block|{
name|LIST_REMOVE
argument_list|(
name|pr
argument_list|,
name|pr_list
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|prisoncount
operator|--
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|TASK_INIT
argument_list|(
operator|&
name|pr
operator|->
name|pr_task
argument_list|,
literal|0
argument_list|,
name|prison_complete
argument_list|,
name|pr
argument_list|)
expr_stmt|;
name|taskqueue_enqueue
argument_list|(
name|taskqueue_swi
argument_list|,
operator|&
name|pr
operator|->
name|pr_task
argument_list|)
expr_stmt|;
return|return;
block|}
name|mtx_unlock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|prison_complete
parameter_list|(
name|void
modifier|*
name|context
parameter_list|,
name|int
name|pending
parameter_list|)
block|{
name|struct
name|prison
modifier|*
name|pr
decl_stmt|;
name|pr
operator|=
operator|(
expr|struct
name|prison
operator|*
operator|)
name|context
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|vrele
argument_list|(
name|pr
operator|->
name|pr_root
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|Giant
argument_list|)
expr_stmt|;
name|mtx_destroy
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
if|if
condition|(
name|pr
operator|->
name|pr_linux
operator|!=
name|NULL
condition|)
name|FREE
argument_list|(
name|pr
operator|->
name|pr_linux
argument_list|,
name|M_PRISON
argument_list|)
expr_stmt|;
name|FREE
argument_list|(
name|pr
argument_list|,
name|M_PRISON
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|prison_hold
parameter_list|(
name|struct
name|prison
modifier|*
name|pr
parameter_list|)
block|{
name|mtx_lock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|pr
operator|->
name|pr_ref
operator|++
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|u_int32_t
name|prison_getip
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred
parameter_list|)
block|{
return|return
operator|(
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
operator|)
return|;
block|}
end_function

begin_function
name|int
name|prison_ip
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred
parameter_list|,
name|int
name|flag
parameter_list|,
name|u_int32_t
modifier|*
name|ip
parameter_list|)
block|{
name|u_int32_t
name|tmp
decl_stmt|;
if|if
condition|(
operator|!
name|jailed
argument_list|(
name|cred
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
if|if
condition|(
name|flag
condition|)
name|tmp
operator|=
operator|*
name|ip
expr_stmt|;
else|else
name|tmp
operator|=
name|ntohl
argument_list|(
operator|*
name|ip
argument_list|)
expr_stmt|;
if|if
condition|(
name|tmp
operator|==
name|INADDR_ANY
condition|)
block|{
if|if
condition|(
name|flag
condition|)
operator|*
name|ip
operator|=
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
expr_stmt|;
else|else
operator|*
name|ip
operator|=
name|htonl
argument_list|(
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
name|tmp
operator|==
name|INADDR_LOOPBACK
condition|)
block|{
if|if
condition|(
name|flag
condition|)
operator|*
name|ip
operator|=
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
expr_stmt|;
else|else
operator|*
name|ip
operator|=
name|htonl
argument_list|(
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
operator|!=
name|tmp
condition|)
return|return
operator|(
literal|1
operator|)
return|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_function
name|void
name|prison_remote_ip
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred
parameter_list|,
name|int
name|flag
parameter_list|,
name|u_int32_t
modifier|*
name|ip
parameter_list|)
block|{
name|u_int32_t
name|tmp
decl_stmt|;
if|if
condition|(
operator|!
name|jailed
argument_list|(
name|cred
argument_list|)
condition|)
return|return;
if|if
condition|(
name|flag
condition|)
name|tmp
operator|=
operator|*
name|ip
expr_stmt|;
else|else
name|tmp
operator|=
name|ntohl
argument_list|(
operator|*
name|ip
argument_list|)
expr_stmt|;
if|if
condition|(
name|tmp
operator|==
name|INADDR_LOOPBACK
condition|)
block|{
if|if
condition|(
name|flag
condition|)
operator|*
name|ip
operator|=
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
expr_stmt|;
else|else
operator|*
name|ip
operator|=
name|htonl
argument_list|(
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
argument_list|)
expr_stmt|;
return|return;
block|}
return|return;
block|}
end_function

begin_function
name|int
name|prison_if
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred
parameter_list|,
name|struct
name|sockaddr
modifier|*
name|sa
parameter_list|)
block|{
name|struct
name|sockaddr_in
modifier|*
name|sai
decl_stmt|;
name|int
name|ok
decl_stmt|;
name|sai
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|sa
expr_stmt|;
if|if
condition|(
operator|(
name|sai
operator|->
name|sin_family
operator|!=
name|AF_INET
operator|)
operator|&&
name|jail_socket_unixiproute_only
condition|)
name|ok
operator|=
literal|1
expr_stmt|;
elseif|else
if|if
condition|(
name|sai
operator|->
name|sin_family
operator|!=
name|AF_INET
condition|)
name|ok
operator|=
literal|0
expr_stmt|;
elseif|else
if|if
condition|(
name|cred
operator|->
name|cr_prison
operator|->
name|pr_ip
operator|!=
name|ntohl
argument_list|(
name|sai
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|)
condition|)
name|ok
operator|=
literal|1
expr_stmt|;
else|else
name|ok
operator|=
literal|0
expr_stmt|;
return|return
operator|(
name|ok
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Return 0 if jails permit p1 to frob p2, otherwise ESRCH.  */
end_comment

begin_function
name|int
name|prison_check
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred1
parameter_list|,
name|struct
name|ucred
modifier|*
name|cred2
parameter_list|)
block|{
if|if
condition|(
name|jailed
argument_list|(
name|cred1
argument_list|)
condition|)
block|{
if|if
condition|(
operator|!
name|jailed
argument_list|(
name|cred2
argument_list|)
condition|)
return|return
operator|(
name|ESRCH
operator|)
return|;
if|if
condition|(
name|cred2
operator|->
name|cr_prison
operator|!=
name|cred1
operator|->
name|cr_prison
condition|)
return|return
operator|(
name|ESRCH
operator|)
return|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Return 1 if the passed credential is in a jail, otherwise 0.  */
end_comment

begin_function
name|int
name|jailed
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred
parameter_list|)
block|{
return|return
operator|(
name|cred
operator|->
name|cr_prison
operator|!=
name|NULL
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Return the correct hostname for the passed credential.  */
end_comment

begin_function
name|void
name|getcredhostname
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred
parameter_list|,
name|char
modifier|*
name|buf
parameter_list|,
name|size_t
name|size
parameter_list|)
block|{
if|if
condition|(
name|jailed
argument_list|(
name|cred
argument_list|)
condition|)
block|{
name|mtx_lock
argument_list|(
operator|&
name|cred
operator|->
name|cr_prison
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|strlcpy
argument_list|(
name|buf
argument_list|,
name|cred
operator|->
name|cr_prison
operator|->
name|pr_host
argument_list|,
name|size
argument_list|)
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|cred
operator|->
name|cr_prison
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
block|}
else|else
name|strlcpy
argument_list|(
name|buf
argument_list|,
name|hostname
argument_list|,
name|size
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Return 1 if the passed credential can "see" the passed mountpoint  * when performing a getfsstat(); otherwise, 0.  */
end_comment

begin_function
name|int
name|prison_check_mount
parameter_list|(
name|struct
name|ucred
modifier|*
name|cred
parameter_list|,
name|struct
name|mount
modifier|*
name|mp
parameter_list|)
block|{
if|if
condition|(
name|jail_getfsstatroot_only
operator|&&
name|cred
operator|->
name|cr_prison
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
name|cred
operator|->
name|cr_prison
operator|->
name|pr_root
operator|->
name|v_mount
operator|!=
name|mp
condition|)
return|return
operator|(
literal|0
operator|)
return|;
block|}
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|sysctl_jail_list
parameter_list|(
name|SYSCTL_HANDLER_ARGS
parameter_list|)
block|{
name|struct
name|xprison
modifier|*
name|xp
decl_stmt|,
modifier|*
name|sxp
decl_stmt|;
name|struct
name|prison
modifier|*
name|pr
decl_stmt|;
name|int
name|count
decl_stmt|,
name|error
decl_stmt|;
name|mtx_assert
argument_list|(
operator|&
name|Giant
argument_list|,
name|MA_OWNED
argument_list|)
expr_stmt|;
if|if
condition|(
name|jailed
argument_list|(
name|req
operator|->
name|td
operator|->
name|td_ucred
argument_list|)
operator|&&
operator|!
name|jail_list_allowed
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|retry
label|:
name|mtx_lock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|count
operator|=
name|prisoncount
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
if|if
condition|(
name|count
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|sxp
operator|=
name|xp
operator|=
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|xp
argument_list|)
operator|*
name|count
argument_list|,
name|M_TEMP
argument_list|,
name|M_WAITOK
operator||
name|M_ZERO
argument_list|)
expr_stmt|;
name|mtx_lock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
if|if
condition|(
name|count
operator|!=
name|prisoncount
condition|)
block|{
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|sxp
argument_list|,
name|M_TEMP
argument_list|)
expr_stmt|;
goto|goto
name|retry
goto|;
block|}
name|LIST_FOREACH
argument_list|(
argument|pr
argument_list|,
argument|&allprison
argument_list|,
argument|pr_list
argument_list|)
block|{
name|mtx_lock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|xp
operator|->
name|pr_version
operator|=
name|XPRISON_VERSION
expr_stmt|;
name|xp
operator|->
name|pr_id
operator|=
name|pr
operator|->
name|pr_id
expr_stmt|;
name|strlcpy
argument_list|(
name|xp
operator|->
name|pr_path
argument_list|,
name|pr
operator|->
name|pr_path
argument_list|,
sizeof|sizeof
argument_list|(
name|xp
operator|->
name|pr_path
argument_list|)
argument_list|)
expr_stmt|;
name|strlcpy
argument_list|(
name|xp
operator|->
name|pr_host
argument_list|,
name|pr
operator|->
name|pr_host
argument_list|,
sizeof|sizeof
argument_list|(
name|xp
operator|->
name|pr_host
argument_list|)
argument_list|)
expr_stmt|;
name|xp
operator|->
name|pr_ip
operator|=
name|pr
operator|->
name|pr_ip
expr_stmt|;
name|mtx_unlock
argument_list|(
operator|&
name|pr
operator|->
name|pr_mtx
argument_list|)
expr_stmt|;
name|xp
operator|++
expr_stmt|;
block|}
name|mtx_unlock
argument_list|(
operator|&
name|allprison_mtx
argument_list|)
expr_stmt|;
name|error
operator|=
name|SYSCTL_OUT
argument_list|(
name|req
argument_list|,
name|sxp
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|sxp
argument_list|)
operator|*
name|count
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|sxp
argument_list|,
name|M_TEMP
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_expr_stmt
name|SYSCTL_OID
argument_list|(
name|_security_jail
argument_list|,
name|OID_AUTO
argument_list|,
name|list
argument_list|,
name|CTLTYPE_STRUCT
operator||
name|CTLFLAG_RD
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|sysctl_jail_list
argument_list|,
literal|"S"
argument_list|,
literal|"List of active jails"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_function
specifier|static
name|int
name|sysctl_jail_jailed
parameter_list|(
name|SYSCTL_HANDLER_ARGS
parameter_list|)
block|{
name|int
name|error
decl_stmt|,
name|injail
decl_stmt|;
name|injail
operator|=
name|jailed
argument_list|(
name|req
operator|->
name|td
operator|->
name|td_ucred
argument_list|)
expr_stmt|;
name|error
operator|=
name|SYSCTL_OUT
argument_list|(
name|req
argument_list|,
operator|&
name|injail
argument_list|,
sizeof|sizeof
argument_list|(
name|injail
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_expr_stmt
name|SYSCTL_PROC
argument_list|(
name|_security_jail
argument_list|,
name|OID_AUTO
argument_list|,
name|jailed
argument_list|,
name|CTLTYPE_INT
operator||
name|CTLFLAG_RD
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|sysctl_jail_jailed
argument_list|,
literal|"I"
argument_list|,
literal|"Process in jail?"
argument_list|)
expr_stmt|;
end_expr_stmt

end_unit

