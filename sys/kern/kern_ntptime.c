...
begin_comment
comment|/*  * Adapted from the original sources for FreeBSD and timecounters by:  * Poul-Henning Kamp<phk@FreeBSD.org>  *  * The 32bit version of the "LP" macros seems a bit past its "sell by"   * date so I have retained only the 64bit version and included it directly  * in this file.  *  * Only minor changes done to interface with the timecounters over in  * sys/kern/kern_clock.c.   Some of the comments below may be (even more)  * confusing and/or plain wrong in that context.  *  * The PPS_SYNC/hardpps() is currently not supported.  *  */
end_comment
...
begin_comment
comment|/* max PPS error (jitter) (ns) */
end_comment
...
begin_comment
comment|/* scaled time dispersion (ns) */
end_comment
...
begin_comment
comment|/* offset accumulator counter */
end_comment
...
expr_stmt|;
block|}
comment|/* 	 * Compute the total time adjustment for the next 	 * second in ns. The offset is reduced by a factor 	 * depending on FLL or PLL mode and whether the PPS 	 * signal is operating. Note that the value is in effect 	 * scaled by the clock frequency, since the adjustment 	 * is added at each tick interrupt. 	 */
name|ftemp
operator|=
name|time_offset
expr_stmt|;
...
block|}
block|}
comment|/* 	 * Nominal jitter is due to PPS signal noise and  interrupt 	 * latency. If it exceeds the jitter limit, the sample is 	 * discarded. otherwise, if so enabled, the time offset is 	 * updated. The offsets are accumulated over the phase averaging 	 * interval to improve accuracy. The jitter is averaged only for 	 * performance monitoring. We can tolerate a modest loss of data 	 * here without degrading time accuracy. 	 */
if|if
condition|(
name|u_nsec
operator|>
...
name|PPS_FAVG
expr_stmt|;
comment|/* 	 * The frequency offset is averaged into the PPS frequency. If 	 * enabled, the system clock frequency is updated as well. 	 */
name|L_RSHIFT
argument_list|(
name|ftemp
argument_list|,
