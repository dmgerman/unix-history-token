...
return|return;
block|}
comment|/* 	 * Go and handle errors now. 	 * If it returns -1 then we should RETRY 	 */
if|if
condition|(
operator|(
name|retval
...
begin_comment
comment|/*  * Look at the returned sense and act on the error, determining  * the unix error number to pass back.  (0 = report no error)  *  * THIS IS THE DEFAULT ERROR HANDLER  */
end_comment
...
directive|endif
comment|/*SCSIDEBUG */
comment|/* 	 * If the device has it's own error handler, call it first. 	 * If it returns a legit error value, return that, otherwise 	 * it wants us to continue with normal error processing. 	 */
if|if
condition|(
name|sc_link
operator|->
