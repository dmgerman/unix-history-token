...
argument_list|)
expr_stmt|;
comment|/* 	 * allocate the resources for another drive 	 * if we have already allocate a sd_data pointer we must 	 * copy the old pointers into a new region that is 	 * larger and release the old region, aka realloc 	 */
comment|/* XXX 	 * This if will always be true for now, but future code may 	 * preallocate more units to reduce overhead.  This would be 	 * done by changing the malloc to be (next_sd_unit * x) and 	 * the sd_driver.size++ to be +x 	 */
name|unit
operator|=
name|next_sd_unit
operator|++
...
name|printf
argument_list|(
literal|"sd%ld: malloc failed for sdrealloc\n"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|printf
argument_list|(
literal|"sd%ld: Already has storage!\n"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|printf
argument_list|(
literal|"sd%ld: malloc failed for sd_data\n"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 	 * make raw partition the whole disk in case of failure then get pdinfo 	 * for historical reasons, make part a same as raw part 	 */
name|sd
operator|->
name|disklabel
operator|.
