...
name|secsiz
expr_stmt|;
comment|/* need to cast to avoid overflow! */
name|sd
operator|->
name|limit
operator|.
...
block|sdsleep(du->dk_ctrlr, "wdioct");
endif|#
directive|endif
...
directive|endif
comment|/* ! SLICE */
comment|/* really only take this from the ctl device XXX */
return|return
operator|(
name|scsi_do_ioctl
argument_list|(
...
begin_comment
comment|/*  * Find out from the device what its capacity is. It turns  * out this is also the best way to find out the sector size.  */
end_comment
...
name|printf
argument_list|(
literal|" Using fictitious geometry\n"
argument_list|)
expr_stmt|;
comment|/* 		 * use adaptec standard fictitious geometry 		 * this depends on which controller (e.g. 1542C is 		 * different. but we have to put SOMETHING here..) 		 */
if|if
condition|(
name|error
operator|=
...
begin_comment
comment|/*  * dump all of physical memory into the partition specified, starting  * at offset 'dumplo' into the partition.  * XXX for SLICE starts at argument 'start'.  */
end_comment
...
begin_comment
comment|/*  * arguments, and schedules the transfer.  Does not wait for the transfer  * to complete.  Multi-page transfers are supported.  All I/O requests must  * be a multiple of a sector in length. scsi_strategy(bp,&sd_switch);  */
end_comment
...
block|}
comment|/* 	 * check it's not too big a transfer for our adapter 	 */
comment|/*scsi_minphys(bp,&sd_switch);*/
name|opri
operator|=
name|SPLSD
argument_list|()
