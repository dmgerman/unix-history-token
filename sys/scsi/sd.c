...
literal|528
case|:
comment|/* 			 * This is not the correct fix but will do till 			 * I get to my scsi docs. Some drives return this value 			 * for physical size when actually presenting 			 * a 512 byte logical sector size. The correct 			 * fix involves figuring out what hte correct 			 * command sequence is to get teh logical blocksize, 			 * and figuring out what to do if it doesn't 			 * work on an old drive. Possibly only do this 		 	 * if the value here is not a power of 2. 			 */
name|sd
operator|->
name|params
operator|.
...
name|printf
argument_list|(
literal|"sd%ld: Can't deal with %d byte blocks\n"
argument_list|,
name|unit
argument_list|,
name|sd
