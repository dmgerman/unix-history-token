...
begin_comment
comment|/*  * NB: normally `upgt_txbuf' value can be increased to maximum 6, mininum 1.  * However, we're using just 2 txbufs to protect packet losses in some cases  * so the performance was sacrificed that with this value its speed is about  * 2.1Mb/s.  *  * With setting txbuf value as 6, you can get full speed, 3.0Mb/s, of this  * device but sometimes you'd meet some packet losses then retransmision.  */
end_comment
...
directive|endif
comment|/* set configuration number */
if|if
condition|(
...
name|dev
argument_list|,
literal|"no endpoint descriptor for iface %d!\n"
argument_list|,
name|i
argument_list|)
expr_stmt|;
...
name|bEndpointAddress
expr_stmt|;
comment|/* 		 * 0x01 TX pipe 		 * 0x81 RX pipe 		 * 		 * Deprecated scheme (not used with fw version>2.5.6.x): 		 * 0x02 TX MGMT pipe 		 * 0x82 TX MGMT pipe 		 */
if|if
condition|(
name|sc
operator|->
...
name|dev
argument_list|,
literal|"could not open TX pipe: %s!\n"
argument_list|,
name|usbd_errstr
argument_list|(
name|error
...
goto|;
block|}
comment|/* Allocate TX, RX, and CMD xfers.  */
if|if
condition|(
name|upgt_alloc_tx
argument_list|(
...
name|fail
goto|;
comment|/* We need the firmware loaded to complete the attach.  */
return|return
name|upgt_attach_hook
argument_list|(
name|dev
...
argument_list|)
expr_stmt|;
comment|/* we do not need the mbuf anymore */
name|m_freem
argument_list|(
name|m
argument_list|)
...
name|UPGT_DEBUG_XMIT
argument_list|,
literal|"%s: TX start data sending\n"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
name|sc_dev
argument_list|,
literal|"could not transmit TX data URB!\n"
argument_list|)
expr_stmt|;
goto|goto
name|done
...
name|sc_dev
argument_list|,
literal|"could not transmit statistics CMD data URB!\n"
argument_list|)
expr_stmt|;
return|return
operator|(
...
name|sc_ifp
decl_stmt|;
comment|/* abort and close TX / RX pipes */
if|if
condition|(
name|sc
operator|->
...
name|sc_dev
argument_list|,
literal|"could not transmit led CMD URB!\n"
argument_list|)
expr_stmt|;
block|}
end_function
...
name|sc_dev
argument_list|,
literal|"could not transmit macfilter CMD data URB!\n"
argument_list|)
expr_stmt|;
name|UPGT_LOCK
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* process the TX queue in process context */
name|usb_rem_task
argument_list|(
name|sc
operator|->
...
operator|++
expr_stmt|;
comment|/* choose a unused buffer.  */
for|for
control|(
name|i
operator|=
...
literal|0
expr_stmt|;
comment|/* TODO: where to get from? */
name|tap
operator|->
name|wt_chan_freq
operator|=
...
name|sc_dev
argument_list|,
literal|"could not transmit channel CMD data URB!\n"
argument_list|)
expr_stmt|;
block|}
end_function
...
name|sc_dev
argument_list|,
literal|"could not transmit EEPROM data URB!\n"
argument_list|)
expr_stmt|;
return|return
operator|(
...
argument_list|)
expr_stmt|;
comment|/* EEPROM data has arrived in time, wakeup tsleep() */
name|wakeup
argument_list|(
name|sc
argument_list|)
...
condition|)
block|{
comment|/* TX queued was processed, continue */
name|sc
operator|->
name|sc_tx_timer
operator|=
...
name|sc_dev
argument_list|,
literal|"could not send start_firmware_load command!\n"
argument_list|)
expr_stmt|;
name|error
operator|=
...
name|sc_dev
argument_list|,
literal|"could not send firmware X2 header!\n"
argument_list|)
expr_stmt|;
name|error
operator|=
...
name|sc_dev
argument_list|,
literal|"error while downloading firmware block!\n"
argument_list|)
expr_stmt|;
name|error
operator|=
...
name|sc_dev
argument_list|,
literal|"could not send load_firmware command!\n"
argument_list|)
expr_stmt|;
name|error
operator|=
...
name|sc_dev
argument_list|,
literal|"could not read firmware response!\n"
argument_list|)
expr_stmt|;
name|error
operator|=
...
condition|)
break|break;
comment|/* firmware load was successful */
block|}
if|if
condition|(
name|i
...
name|sc_dev
argument_list|,
literal|"could not send device init string!\n"
argument_list|)
expr_stmt|;
return|return
operator|(
