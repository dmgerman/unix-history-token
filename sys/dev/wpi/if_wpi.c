...
begin_endif
unit|static void	wpi_radio_on(void *, int); static void	wpi_radio_off(void *, int);
endif|#
directive|endif
end_endif
...
error|#
directive|error
literal|"Sorry, this driver is not yet ready for FreeBSD< 7.0"
endif|#
directive|endif
...
directive|if
literal|0
block|TASK_INIT(&sc->sc_radioontask, 0, wpi_radio_on, sc); 	TASK_INIT(&sc->sc_radioofftask, 0, wpi_radio_off, sc);
endif|#
directive|endif
name|TASK_INIT
argument_list|(
...
goto|;
block|}
comment|/* 	 * Allocate the receive buffer pool. The recieve buffers are 	 * WPI_RBUF_SIZE in length (3k) this is bigger than MCLBYTES 	 * hence we can't simply use a cluster and used mapped dma memory 	 * instead. 	 */
if|if
condition|(
operator|(
name|error
...
argument_list|)
expr_stmt|;
comment|/* whilst ieee80211_ifattach will listen for ieee80211 frames, 	 * we also want to listen for the lower level radio frames 	 */
name|bpfattach2
argument_list|(
name|ifp
argument_list|,
...
goto|;
block|}
comment|/** 	 * Sadly FreeBSD can't always align on a 16k boundary, hence we give it 	 * 10 attempts increasing the size of the allocation by 4k each time. 	 * This should eventually align us on a 16k boundary at the cost 	 * of chewing up dma memory 	 */
if|if
condition|(
operator|(
operator|(
...
argument_list|,
operator|(
literal|"Memory Unaligned, trying again: %d\n"
operator|,
name|count
operator|++
operator|)
...
argument_list|,
operator|(
literal|"Memory, allocated& %s Aligned!\n"
operator|,
name|count
operator|==
literal|10
...
name|sc_dev
argument_list|,
literal|"Unable to align memory\n"
argument_list|)
expr_stmt|;
name|error
operator|=
...
begin_comment
comment|/*  * This is called automatically by the network stack when the mbuf to which our  * Rx buffer is attached is freed.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* put the buffer back in the free list */
...
name|sc_dev
argument_list|,
literal|"could not allocate Rx buffers DMA memory\n"
argument_list|)
expr_stmt|;
return|return
name|error
return|;
block|}
comment|/* ..and split it into 3KB chunks */
name|SLIST_INIT
argument_list|(
operator|&
name|ring
...
goto|;
block|}
comment|/* attach RxBuffer to mbuf */
name|MEXTADD
argument_list|(
name|data
operator|->
...
argument_list|(
operator|(
literal|"NEWSTATE:SCAN\n"
operator|)
argument_list|)
expr_stmt|;
comment|/* Scanning is handled in net80211 via the scan_start, 		 * scan_end, scan_curchan functions. Hence all we do when 		 * changing to the SCAN state is update the leds 		 */
...
directive|if
literal|0
block|if (ic->ic_opmode != IEEE80211_M_STA) { 			(void) wpi_auth(sc);
comment|/* XXX */
block|wpi_setup_beacon(sc, ic->ic_bss); 		}
endif|#
directive|endif
name|ni
operator|=
...
directive|if
literal|0
block|if (ic->ic_opmode != IEEE80211_M_STA) 			sc->config.filter |= htole32(WPI_FILTER_BEACON);
endif|#
directive|endif
...
return|return;
block|}
comment|/* attach Rx buffer to mbuf */
name|MEXTADD
argument_list|(
name|mnew
argument_list|,
...
block|{
comment|/* no free rbufs, copy frame */
name|m
operator|=
name|m_dup
argument_list|(
...
condition|)
block|{
comment|/* no free mbufs either, drop frame */
name|ifp
operator|->
name|if_ierrors
operator|++
...
argument_list|)
expr_stmt|;
comment|/* XXX frame length> sizeof(struct ieee80211_frame_min)? */
...
argument_list|)
expr_stmt|;
comment|/* release node reference */
...
argument_list|)
expr_stmt|;
comment|/* XXX beacon miss handling? */
switch|switch
condition|(
name|desc
operator|->
...
condition|)
block|{
comment|/* 		 * mgmt/multicast frames are sent at the lowest available 		 * bit-rate 		 */
name|rate
operator|=
name|ni
operator|->
...
argument_list|)
expr_stmt|;
comment|/*XXX Need to cater for multiple essids */
name|memset
argument_list|(
operator|&
name|hdr
...
return|return;
block|}
comment|/* At this point the firmware is up and running. If the hardware 	 * RF switch is turned off thermal calibration will fail, though 	 * the card is still happy to continue to accept commands, catch 	 * this case and record the hw is disabled. 	 */
name|wpi_mem_lock
argument_list|(
name|sc
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Clear any commands left in the command buffer */
name|memset
argument_list|(
name|sc
operator|->
...
name|WPI_LOCK_DECL
expr_stmt|;
comment|/* automatic rate control triggered every 500ms */
if|if
condition|(
name|ic
operator|->
...
directive|if
literal|0
comment|//XXX Used by OpenBSD Sensor Framework
block|sc->sensor.value = temp + 260;
endif|#
directive|endif
comment|/* automatic power calibration every 60s */
if|if
condition|(
operator|++
name|sc
...
directive|if
literal|0
block|static void wpi_radio_on(void *arg, int pending) { 	struct wpi_softc *sc = arg;  	device_printf(sc->sc_dev, "radio turned on\n"); }  static void wpi_radio_off(void *arg, int pending) { 	struct wpi_softc *sc = arg;  	device_printf(sc->sc_dev, "radio turned off\n"); }
endif|#
directive|endif
...
block|}
comment|/**  * The ops function is called to perform some actual work.  * because we can't sleep from any of the ic callbacks, we queue an  * op task with wpi_queue_cmd and have the taskqueue process that task.  * The task that gets cued is a op task, which ends up calling this function.  */
specifier|static
name|void
name|wpi_ops
parameter_list|(
...
argument_list|(
operator|(
literal|"Authenticating, not changing channel\n"
operator|)
argument_list|)
expr_stmt|;
break|break;
...
name|sc_dev
argument_list|,
literal|"could not send authentication request\n"
argument_list|)
expr_stmt|;
name|wpi_stop_locked
argument_list|(
...
goto|;
block|}
comment|/**  * queue a command for later execution in a different thread.  * This is needed as the net80211 callbacks do not allow  * sleeping, since we need to sleep to confirm commands have  * been processed by the firmware, we must defer execution to  * a sleep enabled thread.  */
specifier|static
name|int
name|wpi_queue_cmd
parameter_list|(
...
directive|if
literal|0
block|struct wpi_softc *sc = arg; 	struct ieee80211com *ic =&sc->sc_ic; 	WPI_LOCK_DECL;  	DPRINTF(("Device failed, restarting device\n")); 	WPI_LOCK(sc); 	wpi_stop(sc); 	wpi_init(sc); 	WPI_UNLOCK(sc); 	ieee80211_new_state(ic, IEEE80211_S_SCAN, -1);
endif|#
directive|endif
block|}
...
expr_stmt|;
block|}
comment|/**  * Called every second, wpi_tick used by the watch dog timer  * to check that the card is still alive  */
specifier|static
name|void
name|wpi_tick
parameter_list|(
