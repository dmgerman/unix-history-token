...
argument_list|)
expr_stmt|;
comment|/* Setup Tx status flags (constant). */
name|sc
operator|->
name|sc_txs
operator|.
...
name|IEEE80211_S_RUN
case|:
comment|/* 		 * RUN -> RUN transition: 		 * STA mode: Just restart the timers. 		 * IBSS mode: Process IBSS merge. 		 */
if|if
condition|(
name|vap
operator|->
...
block|{
comment|/* 				 * Drop the BSS_FILTER bit 				 * (there is no another way to change bssid). 				 */
name|WPI_RXON_LOCK
argument_list|(
name|sc
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Restore all what was lost. */
name|wpi_restore_node_table
argument_list|(
name|sc
argument_list|,
...
argument_list|,
literal|"beacon sent: rts %u, ack %u, btkill %u, rate %u, "
literal|"duration %u, status %x, tsf %ju, mode %x\n"
argument_list|,
name|stat
operator|->
name|rtsfailcnt
...
begin_comment
comment|/*  * This function prints firmware registers  */
end_comment
...
name|WPI_DEBUG_REGISTER
argument_list|,
literal|"Cannot access internal registers.\n"
argument_list|)
expr_stmt|;
block|}
block|}
...
condition|)
block|{
comment|/* XXX fix this (applicable to the iwn(4) too) */
...
operator|++
expr_stmt|;
comment|/* Check for available space. */
name|ac
operator|=
name|M_WME_GETAC
argument_list|(
...
name|WPI_DEBUG_CMD
argument_list|,
literal|"%s: cmd %s size %u async %d\n"
argument_list|,
name|__func__
argument_list|,
name|wpi_cmd_str
...
name|sc_dev
argument_list|,
literal|"%s: wpi_cmd() call failed with error code %d\n"
argument_list|,
name|__func__
argument_list|,
name|error
...
name|sc_dev
argument_list|,
literal|"%s: error while setting global keys\n"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
argument_list|,
operator|(
literal|"the node %d was added before"
operator|,
name|wn
operator|->
name|id
...
argument_list|,
operator|(
literal|"undefined node id passed"
operator|)
argument_list|)
expr_stmt|;
name|DPRINTF
...
name|sc_dev
argument_list|,
literal|"RXON_ASSOC command failed, error %d\n"
argument_list|,
name|error
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* XXX Force probe request transmission. */
name|memcpy
argument_list|(
name|chan
operator|+
...
operator|++
expr_stmt|;
comment|/* Reduce unnecessary delay. */
name|chan
operator|->
name|flags
operator|=
...
index|]
expr_stmt|;
comment|/* XXX seems to be unused */
if|if
condition|(
operator|*
operator|(
...
expr_stmt|;
block|}
comment|/* Necessary for recursion in ieee80211_beacon_update(). */
name|m
operator|=
name|bcn
operator|->
...
name|WPI_DEBUG_KEY
argument_list|,
literal|"%s: setting %s key id %d for node %d (%s)\n"
argument_list|,
name|__func__
argument_list|,
operator|(
...
name|sc_dev
argument_list|,
literal|"%s: error while setting key\n"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
name|WPI_DEBUG_KEY
argument_list|,
literal|"%s: deleting %s key %d for node %d (%s)\n"
argument_list|,
name|__func__
argument_list|,
operator|(
...
name|sc_dev
argument_list|,
literal|"%s: error while deleting key\n"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* XMIT keys are handled in wpi_tx_data(). */
return|return
literal|1
return|;
block|}
