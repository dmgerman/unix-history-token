...
argument_list|)
expr_stmt|;
comment|/* 	 * With the ForeThought 4.X image it appears that we need to 	 * busy wait on the initializisation command to complete. 	 * Otherwise the command queue address (the first word 	 * of the queue structure) will be mangled. 	 */
name|c
operator|=
literal|0
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"fore initialization failed: intf=%s%d, "
literal|"hbeat=0x%lx\n"
argument_list|,
name|fup
operator|->
...
name|LOG_ERR
argument_list|,
literal|"fore initialization timed out: intf=%s%d, "
literal|"hbeat=0x%lx\n"
argument_list|,
name|fup
operator|->
