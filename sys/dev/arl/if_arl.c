...
begin_comment
comment|/*  * $RISS: if_arl/dev/arl/if_arl.c,v 1.5 2004/01/22 12:49:05 frol Exp $  */
end_comment
...
directive|define
name|BROADCASTADDR
value|(sc->arpcom.ac_if.if_broadcastaddr)
end_define
...
name|printf
argument_list|(
literal|"arl%d: card is NOT configured\n"
argument_list|,
name|sc
operator|->
name|arl_unit
...
argument_list|)
expr_stmt|;
comment|/*	 	if (arl_command(sc) != 0) { 		int i;  		for (i = 0x168; ar->diagnosticInfo == 0&& i; i--) { 			DELAY(ARDELAY1); 		} 	 		if (i != 0&& ar->diagnosticInfo != 0xff) 			printf("arl%d: config error\n", sc->arl_unit);  		else if (i == 0)  			printf("arl%d: config timeout\n", sc->arl_unit);   	} else 		printf("arl%d: config failed\n", sc->arl_unit); */
if|if
condition|(
name|arl_command
argument_list|(
...
name|name
parameter_list|)
value|(arlan_io.cfg.name = ar->name)
define|#
directive|define
name|GET_COPY_PARAM
parameter_list|(
...
parameter_list|)
define|\
value|{								\ 		bzero(arlan_io.cfg.name, sizeof(arlan_io.cfg.name));	\ 		bcopy(ar->name, arlan_io.cfg.name, sizeof(arlan_io.cfg.name)); \ 	}
case|case
name|SIOCGARLALL
case|:
name|bzero
...
literal|0
expr_stmt|;
comment|/*		(sc->quality.txLevel)[ar->txAckQuality& 0x0f]++; 		(sc->quality.txQuality)[(ar->txAckQuality& 0xf0)>> 4]++;*/
block|}
if|if
condition|(
name|ar
...
condition|)
block|{
comment|/*		(sc->quality.rxLevel)[ar->rxQuality& 0x0f]++; 		(sc->quality.rxQuality)[(ar->rxQuality& 0xf0)>> 4]++; */
if|if
condition|(
name|ar
operator|->
