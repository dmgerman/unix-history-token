...
begin_comment
comment|/* Always enable for now - useful for queue hangs. */
end_comment

begin_define
define|#
directive|define
name|VMXNET3_DEBUG_SYSCTL
end_define
...
begin_comment
comment|/*  * NOTE: We only support the simple case of each Rx and Tx queue on its  * own MSIX vector. This is good enough until we support mulitqueue.  */
end_comment
...
begin_comment
comment|/*  * XXX BMV Should probably reorganize the attach and just do  * this in vmxnet3_init_shared_data().  */
end_comment
...
name|vtxrs_offload_failed
argument_list|,
literal|"Tx checksum offload failed"
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"Tx mbuf chains collapsed"
argument_list|)
expr_stmt|;
name|SYSCTL_ADD_UINT
argument_list|(
