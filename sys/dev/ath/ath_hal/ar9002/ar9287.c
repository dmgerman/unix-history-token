...
name|ah
parameter_list|)
value|((struct ar9287State *) AH5212(ah)->ah_rfHal)
end_define

begin_function_decl
specifier|static
name|HAL_BOOL
name|ar9287GetChannelMaxMinPower
parameter_list|(
name|struct
name|ath_hal
modifier|*
...
begin_comment
unit|static int16_t ar9287GetMinPower(struct ath_hal *ah, EXPN_DATA_PER_CHANNEL_5112 *data) {     int i, minIndex;     int16_t minGain,minPwr,minPcdac,retVal;
...
block|for (last=0,i=0;          (i<numChannels)&& (chan->channel> data[i].channelValue);          last=i++);     totalD = data[i].channelValue - data[last].channelValue;     if (totalD> 0) {         totalF = data[i].maxPower_t4 - data[last].maxPower_t4;         *maxPow = (int8_t) ((totalF*(chan->channel-data[last].channelValue) + data[last].maxPower_t4*totalD)/totalD);          totalMin = ar9287GetMinPower(ah,&data[i]) - ar9287GetMinPower(ah,&data[last]);         *minPow = (int8_t) ((totalMin*(chan->channel-data[last].channelValue) + ar9287GetMinPower(ah,&data[last])*totalD)/totalD);         return (AH_TRUE);     } else {         if (chan->channel == data[i].channelValue) {             *maxPow = data[i].maxPower_t4;             *minPow = ar9287GetMinPower(ah,&data[i]);             return(AH_TRUE);         } else             return(AH_FALSE);     }
else|#
directive|else
operator|*
name|maxPow
