...
begin_comment
comment|/*  * Check whether there's an in-progress NF completion.  *  * Returns AH_TRUE if there's a in-progress NF calibration, AH_FALSE  * otherwise.  */
end_comment
...
begin_comment
comment|/*  * Wait for an in-progress NF calibration to complete.  *  * The completion function waits "i" times 10uS.  * It returns AH_TRUE if the NF calibration completed (or was never  * in progress); AH_FALSE if it was still in progress after "i" checks.  */
end_comment
...
literal|1
expr_stmt|;
comment|/* it should run at least once */
for|for
control|(
name|j
operator|=
...
name|AR_PHY_RADAR_2
argument_list|,
name|AR_PHY_RADAR_2_BLOCKOFDMWEAK
argument_list|)
expr_stmt|;
else|else
name|OS_REG_CLR_BIT
...
name|AR_PHY_RADAR_2
argument_list|,
name|AR_PHY_RADAR_2_ENRELSTEPCHK
argument_list|)
expr_stmt|;
else|else
name|OS_REG_CLR_BIT
...
name|AR_PHY_RADAR_2
argument_list|,
name|AR_PHY_RADAR_2_ENRELPWRCHK
argument_list|)
expr_stmt|;
else|else
name|OS_REG_CLR_BIT
...
begin_comment
comment|/*  * Parameters for the AR5212 PHY.  */
end_comment
...
begin_comment
comment|/*  * Default parameters for the AR5413 PHY.  */
end_comment

begin_define
define|#
directive|define
name|AR5413_DFS_FIRPWR
value|-34
end_define
...
begin_comment
comment|/*  * Process the radar phy error and extract the pulse duration.  */
end_comment
...
name|AH_FALSE
return|;
comment|/* 	 * The first byte is the pulse width - if there's 	 * no data, simply set the duration to 0 	 */
if|if
condition|(
name|rxs
operator|->
...
literal|0
expr_stmt|;
comment|/* Pulse RSSI is the normal reported RSSI */
name|rssi
operator|=
operator|(
name|uint8_t
...
name|rs_rssi
expr_stmt|;
comment|/* 0 duration/rssi is not a valid radar event */
if|if
condition|(
name|dur
operator|==
...
begin_comment
comment|/*  * There's no channel survey support for the AR5212.  */
end_comment
