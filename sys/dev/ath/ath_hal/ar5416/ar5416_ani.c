...
begin_comment
comment|/*  * Cleanup any ANI state setup.  *  * This doesn't restore registers to their default settings!  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* These commands can't be disabled */
if|if
condition|(
name|cmd
operator|==
...
block|{
comment|/* Enable MIB Counters */
comment|/* 				 * XXX use 2.4ghz params if no channel is 				 * available 				 */
name|enableAniMIBCounters
argument_list|(
name|ah
argument_list|,
...
return|;
block|}
comment|/* Check whether the particular function is enabled */
if|if
condition|(
operator|(
operator|(
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: HAL_ANI_NOISE_IMMUNITY_LEVEL: set level = %d\n"
argument_list|,
name|__func__
argument_list|,
name|level
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: HAL_ANI_OFDM_WEAK_SIGNAL_DETECTION: %s\n"
argument_list|,
name|__func__
argument_list|,
name|on
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: HAL_ANI_CCK_WEAK_SIGNAL_THR: %s\n"
argument_list|,
name|__func__
argument_list|,
name|high
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: HAL_ANI_FIRSTEP_LEVEL: level = %d\n"
argument_list|,
name|__func__
argument_list|,
name|level
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: HAL_ANI_SPUR_IMMUNITY_LEVEL: level = %d\n"
argument_list|,
name|__func__
argument_list|,
name|level
...
return|return;
block|}
comment|/* 	 * In the case of AP mode operation, we cannot bucketize beacons 	 * according to RSSI.  Instead, raise Firstep level, up to max, and 	 * simply return. 	 */
if|if
condition|(
name|AH_PRIVATE
argument_list|(
...
goto|;
block|}
comment|/* 	 * Use a restrictive set of ANI parameters for hostap mode. 	 */
if|if
condition|(
name|opmode
operator|==
...
name|params
expr_stmt|;
comment|/* 	 * In the case of AP mode operation, we cannot bucketize beacons 	 * according to RSSI.  Instead, lower Firstep level, down to min, and 	 * simply return. 	 */
if|if
condition|(
name|AH_PRIVATE
argument_list|(
...
begin_comment
comment|/*   * Return an approximation of the time spent ``listening'' by  * deducting the cycles spent tx'ing and rx'ing from the total  * cycle count since our last call.  A return value<0 indicates  * an invalid/inconsistent time.  *  * This may be called with ANI disabled; in which case simply keep  * the statistics and don't write to the aniState pointer.  *  * XXX TODO: Make this cleaner!  */
end_comment
...
return|;
block|}
comment|/* 	 * Fetch the current statistics, squirrel away the current 	 * sample. 	 */
name|OS_MEMZERO
argument_list|(
operator|&
name|hs
