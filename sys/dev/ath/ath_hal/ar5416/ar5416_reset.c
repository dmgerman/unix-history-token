...
operator|)
expr_stmt|;
comment|/* For chips on which the RTC reset is done, save TSF before it gets cleared */
if|if
condition|(
name|AR_SREV_HOWL
argument_list|(
...
expr_stmt|;
block|}
comment|/* Override ini values (that can be overriden in this fashion) */
name|ar5416OverrideIni
argument_list|(
name|ah
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * Some AR91xx SoC devices frequently fail to accept TSF writes 	 * right after the chip reset. When that happens, write a new 	 * value after the initvals have been applied, with an offset 	 * based on measured time difference 	 */
if|if
condition|(
name|AR_SREV_HOWL
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Setup ah_tx_chainmask / ah_rx_chainmask before we fiddle 	 * with enabling the TX/RX radio chains. 	 */
name|ar5416UpdateChainMasks
argument_list|(
name|ah
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * This routine swaps the analog chains - it should be done 	 * before any radio register twiddling is done. 	 */
...
argument_list|)
expr_stmt|;
comment|/* XXX this won't work for AR9287! */
if|if
condition|(
...
directive|if
literal|0
comment|/* 			 * AR5413? 			 * Force window_length for 1/2 and 1/4 rate channels, 			 * the ini file sets this to zero otherwise. 			 */
block|OS_REG_RMW_FIELD(ah, AR_PHY_FRAME_CTL, 			    AR_PHY_FRAME_CTL_WINLEN, 3); 		}
endif|#
directive|endif
block|}
if|if
...
condition|)
block|{
comment|/* 		 * Enable ASYNC FIFO 		 * 		 * If Async FIFO is enabled, the following counters change 		 * as MAC now runs at 117 Mhz instead of 88/44MHz when 		 * async FIFO is disabled. 		 * 		 * Overwrite the delay/timeouts initialized in ProcessIni() 		 * above. 		 */
name|OS_REG_WRITE
argument_list|(
name|ah
argument_list|,
...
condition|)
block|{
comment|/* Enable AGGWEP to accelerate encryption engine */
name|OS_REG_SET_BIT
argument_list|(
name|ah
argument_list|,
...
condition|)
block|{
comment|/* 		 * Enable the MBSSID block-ack fix for HOWL. 		 * This feature is only supported on Howl 1.4, but it is safe to 		 * set bit 22 of STA_ID1 on other Howl revisions (1.1, 1.2, 1.3), 		 * since bit 22 is unused in those Howl revisions. 		 */
name|unsigned
name|int
name|reg
decl_stmt|;
...
name|ah
argument_list|,
literal|"MBSSID Set bit 22 of AR_STA_ID 0x%x\n"
argument_list|,
name|reg
argument_list|)
expr_stmt|;
...
directive|ifdef
name|ADRIAN_NOTYET
comment|/* This is straight from ath9k */
if|if
condition|(
operator|!
name|AR_SREV_HOWL
...
begin_comment
comment|/**************************************************************  * ar5416WriteTxPowerRateRegisters  *  * Write the TX power rate registers from the raw values given  * in ratesArray[].  *  * The CCK and HT40 rate registers are only written if needed.  * HT20 and 11g/11a OFDM rate registers are always written.  *  * The values written are raw values which should be written  * to the registers - so it's up to the caller to pre-adjust  * them (eg CCK power offset value, or Merlin TX power offset,  * etc.)  */
end_comment
...
directive|ifdef
name|AH_EEPROM_DUMP
comment|/*      * Dump the rate array whilst it represents the intended dBm*2      * values versus what's being adjusted before being programmed      * in. Keep this in mind if you code up this function and enable      * this debugging; the values won't necessarily be what's being      * programmed into the hardware.      */
...
directive|endif
comment|/*      * Merlin and later have a power offset, so subtract      * pwr_table_offset * 2 from each value. The default      * power offset is -5 dBm - ie, a register value of 0      * equates to a TX power of -5 dBm.      */
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* Underflow power gets clamped at raw value 0 */
comment|/* Overflow power gets camped at AR5416_MAX_RATE_POWER */
for|for
control|(
name|i
operator|=
...
control|)
block|{
comment|/* 		 * + pwr_table_offset is in dBm 		 * + ratesArray is in 1/2 dBm 		 */
name|ratesArray
index|[
name|i
index|]
...
comment|/* Because of the AR9130 specific registers */
if|if
condition|(
name|AR_SREV_HOWL
argument_list|(
...
name|HAL_DEBUG_ANY
argument_list|,
literal|"[ath] HOWL: Fiddling with derived clk!\n"
argument_list|)
expr_stmt|;
name|uint32_t
name|val
...
name|HAL_DEBUG_RESET
argument_list|,
literal|"CFG Byte Swap Set 0x%x\n"
argument_list|,
name|mask
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 	 * Workaround for OWL 1.0 calibration failure; enable multi-chain; 	 * then set true mask after calibration. 	 */
if|if
condition|(
name|IS_5416V1
argument_list|(
...
begin_comment
comment|/*  * Work-around for Owl 1.0 calibration failure.  *  * ar5416InitChainMasks sets the RX chainmask to 0x7 if it's Owl 1.0  * due to init calibration failures. ar5416RestoreChainMask restores  * these registers to the correct setting.  */
end_comment
...
begin_comment
comment|/*  * Update the chainmask based on the current channel configuration.  *  * XXX ath9k checks bluetooth co-existence here  * XXX ath9k checks whether the current state is "off-channel".  * XXX ath9k sticks the hardware into 1x1 mode for legacy;  *     we're going to leave multi-RX on for multi-path cancellation.  */
end_comment
...
begin_comment
comment|/*  * Get the register chain offset for the given chain.  *  * Take into account the register chain swapping with AR5416 v2.0.  *  * XXX make sure that the reg chain swapping is only done for  * XXX AR5416 v2.0 or greater, and not later chips?  */
end_comment
...
begin_comment
comment|/*  * Set the target power array "ratesArray" from the  * given set of target powers.  *  * This is used by the various chipset/EEPROM TX power  * setup routines.  */
end_comment
...
name|i
decl_stmt|;
comment|/* Blank the rates array, to be consistent */
for|for
control|(
name|i
operator|=
...
begin_comment
comment|/*  * Set the gain boundaries for the given radio chain.  *  * The gain boundaries tell the hardware at what point in the  * PDADC array to "switch over" from one PD gain setting  * to another. There's also a gain overlap between two  * PDADC array gain curves where there's valid PD values  * for 2 gain settings.  *  * The hardware uses the gain overlap and gain boundaries  * to determine which gain curve to use for the given  * target TX power.  */
end_comment
...
begin_comment
comment|/*  * Get the gain values and the number of gain levels given  * in xpdMask.  *  * The EEPROM xpdMask determines which power detector gain  * levels were used during calibration. Each of these mask  * bits maps to a fixed gain level in hardware.  */
end_comment
...
begin_comment
comment|/*  * Write the detector gain and biases.  *  * There are four power detector gain levels. The xpdMask in the EEPROM  * determines which power detector gain levels have TX power calibration  * data associated with them. This function writes the number of  * PD gain levels and their values into the hardware.  *  * This is valid for all TX chains - the calibration data itself however  * will likely differ per-chain.  */
end_comment
...
begin_comment
comment|/*  * The linux ath9k driver and (from what I've been told) the reference  * Atheros driver enables the 11n PHY by default whether or not it's  * configured.  */
end_comment
...
begin_comment
comment|/*  * Override the INI vals being programmed.  */
end_comment
...
name|val
decl_stmt|;
comment|/* 	 * Set the RX_ABORT and RX_DIS and clear if off only after 	 * RXE is set for MAC. This prevents frames with corrupted 	 * descriptor status. 	 */
name|OS_REG_SET_BIT
argument_list|(
name|ah
argument_list|,
...
begin_comment
comment|/*  * Override XPA bias level based on operating frequency.  * This is a v14 EEPROM specific thing for the AR9160.  */
end_comment
...
name|cnt
parameter_list|)
value|(pModal->xpaBiasLvlFreq[cnt])
name|MODAL_EEP_HEADER
modifier|*
name|pModal
decl_stmt|;
...
name|HAL_DEBUG_EEPROM
argument_list|,
literal|"%s: overriding XPA bias level = %d\n"
argument_list|,
name|__func__
argument_list|,
name|biaslevel
argument_list|)
expr_stmt|;
comment|/* 	 * This is a dirty workaround for the const initval data, 	 * which will upset multiple AR9160's on the same board. 	 * 	 * The HAL should likely just have a private copy of the addac 	 * data per instance. 	 */
if|if
condition|(
name|IEEE80211_IS_CHAN_2GHZ
argument_list|(
...
define|#
directive|define
name|AR5416_IFS_SLOT_FULL_RATE_40
value|0x168
end_define
...
define|#
directive|define
name|AR5416_IFS_SLOT_HALF_RATE_40
value|0x104
end_define
...
define|#
directive|define
name|AR5416_IFS_SLOT_QUARTER_RATE_40
value|0xD2
end_define
...
define|#
directive|define
name|AR5416_IFS_EIFS_FULL_RATE_40
value|0xE60
end_define
...
define|#
directive|define
name|AR5416_IFS_EIFS_HALF_RATE_40
value|0xDAC
end_define
...
define|#
directive|define
name|AR5416_IFS_EIFS_QUARTER_RATE_40
value|0xD48
end_define
...
define|#
directive|define
name|AR5416_IFS_SLOT_FULL_RATE_44
value|0x18c
end_define

begin_comment
comment|/* 9 us half, 44 MHz core clock (9*44) */
end_comment

begin_define
define|#
directive|define
name|AR5416_IFS_SLOT_HALF_RATE_44
value|0x11e
end_define

begin_comment
comment|/* 13 us half, 22 MHz core clock (13*22) */
end_comment

begin_define
define|#
directive|define
name|AR5416_IFS_SLOT_QUARTER_RATE_44
value|0xe7
end_define

begin_comment
comment|/* 21 us quarter, 11 MHz core clock (21*11) */
end_comment

begin_define
define|#
directive|define
name|AR5416_IFS_EIFS_FULL_RATE_44
value|0xfd0
end_define

begin_comment
comment|/* (74 + (2 * 9)) * 44MHz core clock */
end_comment

begin_define
define|#
directive|define
name|AR5416_IFS_EIFS_HALF_RATE_44
value|0xf0a
end_define

begin_comment
comment|/* (149 + (2 * 13)) * 22MHz core clock */
end_comment

begin_define
define|#
directive|define
name|AR5416_IFS_EIFS_QUARTER_RATE_44
value|0xe9c
end_define

begin_comment
comment|/* (298 + (2 * 21)) * 11MHz core clock */
end_comment

begin_define
define|#
directive|define
name|AR5416_INIT_USEC_40
value|40
end_define
...
define|#
directive|define
name|AR5416_QUARTER_RATE_USEC_40
value|9
end_define
...
define|#
directive|define
name|AR5416_QUARTER_RATE_USEC_44
value|10
end_define
...
begin_comment
comment|/* XXX What should these be for 40/44MHz clocks (and half/quarter) ? */
end_comment

begin_define
define|#
directive|define
name|AR5416_RX_NON_FULL_RATE_LATENCY
value|63
end_define
...
define|#
directive|define
name|AR5416_TX_HALF_RATE_LATENCY
value|108
end_define
...
argument_list|)
expr_stmt|;
comment|/* 2GHz and 5GHz fast clock - 44MHz; else 40MHz */
if|if
condition|(
name|IEEE80211_IS_CHAN_2GHZ
argument_list|(
...
name|AR_USEC_USEC32
expr_stmt|;
comment|/* 	 * XXX This really should calculate things, not use 	 * hard coded values! Ew. 	 */
if|if
condition|(
name|IEEE80211_IS_CHAN_HALF
argument_list|(
