...
comment|/* 	 * Blank the interrupt debugging area regardless. 	 */
name|bzero
argument_list|(
operator|&
name|ah
...
name|HAL_INT_TSFOOR
expr_stmt|;
comment|/* 			 * Don't mask out AR_BCNMISC; instead mask 			 * out what causes it. 			 */
name|OS_REG_WRITE
argument_list|(
name|ah
argument_list|,
...
name|HAL_INT_TX
expr_stmt|;
comment|/* 		 * When doing RX interrupt mitigation, the RXOK bit is set 		 * in AR_ISR even if the relevant bit in AR_IMR is clear. 		 * Since this interrupt may be due to another source, don't 		 * just automatically set HAL_INT_RX if it's set, otherwise 		 * we could prematurely service the RX queue. 		 * 		 * In some cases, the driver can even handle all the RX 		 * frames just before the mitigation interrupt fires. 		 * The subsequent RX processing trip will then end up 		 * processing 0 frames. 		 */
ifdef|#
directive|ifdef
...
argument_list|)
expr_stmt|;
comment|/* 			 * Don't clear the primary ISR TX bits, clear 			 * what causes them (S0/S1.) 			 */
name|isr
operator|&=
operator|~
operator|(
