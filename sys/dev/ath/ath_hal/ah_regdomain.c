...
name|HAL_DEBUG_REGDOMAIN
argument_list|,
literal|"%s: %d: freq=%d, flags=0x%08x\n"
argument_list|,
name|__func__
argument_list|,
operator|*
...
name|HAL_DEBUG_REGDOMAIN
argument_list|,
literal|"%s: freq=%d..%d, flags=0x%08x, step=%d\n"
argument_list|,
name|__func__
argument_list|,
operator|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Don't start the 5GHz channel list at 5120MHz. 		 * 		 * Unfortunately (sigh) the HT40 channel creation 		 * logic will create HT40U channels at 5120, 5160, 5200. 		 * This means that 36 (5180) isn't considered as a 		 * HT40 channel, and everything goes messed up from there. 		 */
if|if
condition|(
operator|(
name|cm
...
expr_stmt|;
block|}
comment|/* 		 * Same with HT40D - need to start at 5200 or the low 		 * channels are all wrong again. 		 */
if|if
condition|(
operator|(
name|cm
...
name|HAL_DEBUG_REGDOMAIN
argument_list|,
literal|"%s: freq_lo=%d, freq_hi=%d, low_adj=%d, hi_adj=%d, "
literal|"bandlo=%d, bandhi=%d, bfreqlo=%d, bfreqhi=%d, step=%d, "
