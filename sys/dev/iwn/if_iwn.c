...
specifier|static
name|int
name|iwn5000_send_calib_results
parameter_list|(
name|struct
name|iwn_softc
modifier|*
...
specifier|static
name|int
name|iwn5000_save_calib_result
parameter_list|(
name|struct
name|iwn_softc
modifier|*
...
specifier|static
name|void
name|iwn5000_free_calib_results
parameter_list|(
name|struct
name|iwn_softc
modifier|*
...
name|IWN_CALIB_LO
operator||
name|IWN_CALIB_TX_IQ
operator||
name|IWN_CALIB_TX_IQ_PERIODIC
operator||
name|IWN_CALIB_BASE_BAND
expr_stmt|;
break|break;
case|case
...
name|chan
decl_stmt|;
comment|/* Retrieve channel number. */
name|chan
operator|=
...
begin_comment
comment|/*  * Send calibration results to the runtime firmware.  These results were  * obtained on first boot from the initialization firmware, or by reading  * the EEPROM for crystal calibration.  */
end_comment
...
index|]
expr_stmt|;
comment|/* No support for this type of calibration. */
if|if
condition|(
operator|(
name|sc
...
condition|)
continue|continue;
comment|/* No calibration result available. */
if|if
condition|(
name|calib_result
operator|->
...
name|IWN_DEBUG_CALIBRATE
argument_list|,
literal|"%s: send calibration result idx=%d, len=%d\n"
argument_list|,
name|__func__
argument_list|,
name|idx
...
name|sc_dev
argument_list|,
literal|"%s: could not send calibration result "
literal|"idx=%d, error=%d\n"
argument_list|,
name|__func__
argument_list|,
name|idx
...
begin_comment
comment|/*  * Save calibration result at the given index.  The index determines  * in which order the results are sent to the runtime firmware.  */
end_comment
...
name|IWN_DEBUG_CALIBRATE
argument_list|,
literal|"%s: saving calibration result code=%d, idx=%d, len=%d\n"
argument_list|,
name|__func__
argument_list|,
name|calib
...
name|sc_dev
argument_list|,
literal|"%s: not enough memory for calibration result "
literal|"code=%d, len=%d\n"
argument_list|,
name|__func__
argument_list|,
name|calib
...
begin_comment
comment|/*  * Obtain the crystal calibration result from the EEPROM.  */
end_comment
...
name|IWN_DEBUG_CALIBRATE
argument_list|,
literal|"%s: crystal calibration=0x%08x\n"
argument_list|,
name|__func__
argument_list|,
name|le32toh
...
begin_comment
comment|/*  * Query calibration results from the initialization firmware.  We do this  * only once at first boot.  */
end_comment
...
name|IWN_DEBUG_CALIBRATE
argument_list|,
literal|"%s: query calibration results, cfg %x\n"
argument_list|,
name|__func__
argument_list|,
name|cfg
...
begin_comment
comment|/*  * Process a CALIBRATION_RESULT notification sent by the initialization  * firmware on response to a CMD_CALIB_CONFIG command.  */
end_comment
...
literal|4
expr_stmt|;
comment|/* 	 * Determine the order in which the results will be send to the 	 * runtime firmware. 	 */
switch|switch
condition|(
name|calib
operator|->
...
name|idx
operator|=
name|IWN_CALIB_IDX_TX_IQ_PERIODIC
expr_stmt|;
break|break;
case|case
...
name|IWN_DEBUG_CALIBRATE
argument_list|,
literal|"%s: unknown calibration code=%d\n"
argument_list|,
name|__func__
argument_list|,
name|calib
...
condition|)
block|{
comment|/* 		 * Start calibration by setting and sending the chrystal 		 * calibration first, this must be done before we are able 		 * to query the other calibration results. 		 */
name|error
operator|=
name|iwn5000_chrystal_calib
argument_list|(
...
name|sc_dev
argument_list|,
literal|"%s: could not set chrystal calibration, "
literal|"error=%d\n"
argument_list|,
name|__func__
argument_list|,
...
name|sc_dev
argument_list|,
literal|"%s: could not send chrystal calibration, "
literal|"error=%d\n"
argument_list|,
name|__func__
argument_list|,
...
return|;
block|}
comment|/* 		 * Query other calibration results from the initialization 		 * firmware. 		 */
name|error
operator|=
name|iwn5000_send_calib_query
argument_list|(
...
block|{
comment|/* 		 * Send calibration results obtained from the initialization 		 * firmware to the runtime firmware. 		 */
name|error
operator|=
name|iwn5000_send_calib_results
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Tell the runtime firmware to do certain calibration types. 		 */
if|if
condition|(
name|sc
operator|->
...
name|sc_dev
argument_list|,
literal|"%s: could not send query calibration, "
literal|"error=%d, cfg=%x\n"
argument_list|,
name|__func__
argument_list|,
name|error
