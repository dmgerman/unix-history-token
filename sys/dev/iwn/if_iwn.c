...
literal|0
comment|/* HT */
block|ops->ampdu_tx_start = iwn4965_ampdu_tx_start; 	ops->ampdu_tx_stop = iwn4965_ampdu_tx_stop;
endif|#
directive|endif
name|sc
operator|->
...
literal|0
comment|/* HT */
block|ops->ampdu_tx_start = iwn5000_ampdu_tx_start; 	ops->ampdu_tx_stop = iwn5000_ampdu_tx_stop;
endif|#
directive|endif
name|sc
operator|->
...
condition|)
block|{
comment|/* Set radio temperature sensor offset. */
name|error
operator|=
name|iwn5000_temp_offset_calib
argument_list|(
...
begin_comment
unit|static int iwn_ampdu_rx_start(struct ieee80211com *ic, struct ieee80211_node *ni,     uint8_t tid) { 	struct ieee80211_rx_ba *ba =&ni->ni_rx_ba[tid]; 	struct iwn_softc *sc = ic->ic_softc; 	struct iwn_ops *ops =&sc->ops; 	struct iwn_node *wn = (void *)ni; 	struct iwn_node_info node;  	memset(&node, 0, sizeof node); 	node.id = wn->id; 	node.control = IWN_NODE_UPDATE; 	node.flags = IWN_FLAG_SET_ADDBA; 	node.addba_tid = tid; 	node.addba_ssn = htole16(ba->ba_winstart); 	DPRINTF(sc, IWN_DEBUG_RECV, "ADDBA RA=%d TID=%d SSN=%d\n", 	    wn->id, tid, ba->ba_winstart); 	return ops->add_node(sc,&node, 1); }
...
begin_comment
unit|static void iwn_ampdu_rx_stop(struct ieee80211com *ic, struct ieee80211_node *ni,     uint8_t tid) { 	struct iwn_softc *sc = ic->ic_softc; 	struct iwn_ops *ops =&sc->ops; 	struct iwn_node *wn = (void *)ni; 	struct iwn_node_info node;  	memset(&node, 0, sizeof node); 	node.id = wn->id; 	node.control = IWN_NODE_UPDATE; 	node.flags = IWN_FLAG_SET_DELBA; 	node.delba_tid = tid; 	DPRINTF(sc, IWN_DEBUG_RECV, "DELBA RA=%d TID=%d\n", wn->id, tid); 	(void)ops->add_node(sc,&node, 1); }
comment|/*  * This function is called by upper layer when an ADDBA response is received  * from another STA.  */
end_comment

begin_comment
unit|static int iwn_ampdu_tx_start(struct ieee80211com *ic, struct ieee80211_node *ni,     uint8_t tid) { 	struct ieee80211_tx_ba *ba =&ni->ni_tx_ba[tid]; 	struct iwn_softc *sc = ic->ic_softc; 	struct iwn_ops *ops =&sc->ops; 	struct iwn_node *wn = (void *)ni; 	struct iwn_node_info node; 	int error;
