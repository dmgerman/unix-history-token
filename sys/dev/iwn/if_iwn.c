...
begin_comment
comment|/*  * Driver for Intel Wireless WiFi Link 4965 and Intel WiFi Link 5000 Series  * 802.11 network adapters.  */
end_comment
...
name|dev
argument_list|,
literal|"could not power ON adapter, error %d\n"
argument_list|,
name|error
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Initialization firmware has not been loaded yet. */
name|sc
operator|->
name|sc_flags
operator||=
...
literal|"iwn5150fw"
expr_stmt|;
comment|/* NB: critical temperature will be read from EEPROM. */
name|sc
operator|->
name|txantmsk
operator|=
...
condition|)
block|{
comment|/* Compute critical temperature (in Kelvin.) */
name|iwn_read_prom_data
argument_list|(
name|sc
argument_list|,
...
name|IWN_DEBUG_CALIBRATE
argument_list|,
literal|"temp=%d volt=%d delta=%dK\n"
argument_list|,
name|temp
argument_list|,
name|volt
...
begin_comment
comment|/*  * Set the critical temperature at which the firmware will notify us.  */
end_comment
...
expr_stmt|;
comment|/* Others: -5dB */
comment|/* Do not exceed channel's max TX power. */
if|if
condition|(
name|pwr
operator|>
...
name|INT_MAX
expr_stmt|;
comment|/* NB: There's at least one antennaiwn. */
for|for
control|(
name|i
operator|=
...
literal|1
expr_stmt|;
comment|/* Set differential gains for antennas B and C. */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* The delta is relative to antenna A. */
name|delta
operator|=
operator|(
operator|(
...
name|sc_dev
argument_list|,
literal|"%s: could not set power saving level, error %d\n"
argument_list|,
name|__func__
argument_list|,
name|error
...
name|sc_dev
argument_list|,
literal|"%s: configure command failed\n"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* 	 * Reconfiguring RXON clears the firmware's nodes table so we must 	 * add the broadcast node again. 	 */
name|error
operator|=
name|iwn_add_broadcast_node
argument_list|(
...
name|sc_dev
argument_list|,
literal|"%s: 1 could not add broadcast node, error %d\n"
argument_list|,
name|__func__
argument_list|,
name|error
...
name|error
return|;
comment|/* Clear TX scheduler's state in SRAM. */
name|sc
operator|->
name|sched_base
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Enable chain mode for all our 20 queues. */
name|iwn_prph_write
argument_list|(
name|sc
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Configure WiMAX (IEEE 802.16e) coexistence. */
name|memset
argument_list|(
operator|&
name|wimax
...
return|;
block|}
comment|/* 		 * We have the calibration results now so we can skip 		 * loading the initialization firmware next time. 		 */
name|sc
operator|->
name|sc_flags
operator|&=
...
name|IWN_FLAG_FIRST_BOOT
expr_stmt|;
comment|/* Reboot (call ourselves recursively!) */
name|iwn_hw_stop
argument_list|(
name|sc
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Set Flow Handler wait threshold to the maximum. */
name|IWN_SETBITS
argument_list|(
name|sc
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Enable HAP to move adapter from L1a to L0s. */
name|IWN_SETBITS
argument_list|(
name|sc
argument_list|,
...
name|sc_dev
argument_list|,
literal|"%s: radio is disabled by hardware switch\n"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
