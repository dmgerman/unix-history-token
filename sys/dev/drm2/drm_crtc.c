...
parameter_list|)
define|\
value|char *fnname(int val)					\ 	{							\ 		int i;						\ 		for (i = 0; i< ARRAY_SIZE(list); i++) {	\ 			if (list[i].type == val)		\ 				return list[i].name;		\ 		}						\ 		return "(unknown)";				\ 	}
end_define
...
begin_comment
comment|/**  * drm_framebuffer_unreference - unref a framebuffer  *  * LOCKING:  * Caller must hold mode config lock.  */
end_comment
...
name|DRM_WARNING
argument_list|(
literal|"%s: dev->mode_config.mutex not locked\n"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
begin_comment
comment|/**  * drm_framebuffer_reference - incr the fb refcnt  */
end_comment
...
name|dev
decl_stmt|;
comment|/* 	 * This could be moved to drm_framebuffer_remove(), but for 	 * debugging is nice to keep around the list of fb's that are 	 * no longer associated w/ a drm_file but are not unreferenced 	 * yet.  (i915 and omapdrm have debugfs files which will show 	 * this.) 	 */
name|drm_mode_object_put
argument_list|(
name|dev
argument_list|,
...
begin_comment
comment|/**  * drm_framebuffer_remove - remove and unreference a framebuffer object  * @fb: framebuffer to remove  *  * LOCKING:  * Caller must hold mode config lock.  *  * Scans all the CRTCs and planes in @dev's mode_config.  If they're  * using @fb, removes it, setting it to NULL.  */
end_comment
...
begin_comment
comment|/**  * drm_crtc_init - Initialise a new CRTC object  * @dev: DRM device  * @crtc: CRTC object to init  * @funcs: callbacks for the new CRTC  *  * LOCKING:  * Takes mode_config lock.  *  * Inits a new object created as base part of an driver crtc object.  *  * RETURNS:  * Zero on success, error code on failure.  */
end_comment
...
name|connector
decl_stmt|;
comment|/* taking the mode config mutex ends up in a clash with sysfs */
name|list_for_each_entry
argument_list|(
argument|connector
argument_list|,
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"out of memory when allocating plane\n"
argument_list|)
expr_stmt|;
name|drm_mode_object_put
argument_list|(
...
argument|bt
argument_list|,
argument|&dev->mode_config.property_blob_list
argument_list|,
argument|head
argument_list|)
block|{
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"Invalid fb size %ux%u for CRTC viewport %ux%u+%d+%d%s.\n"
argument_list|,
name|fb
operator|->
name|width
...
name|DRM_ERROR
argument_list|(
literal|"bad bpp, assuming x8r8g8b8 pixel format\n"
argument_list|)
expr_stmt|;
name|fmt
operator|=
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"could not create framebuffer\n"
argument_list|)
expr_stmt|;
goto|goto
name|out
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"bad framebuffer width %u\n"
argument_list|,
name|r
operator|->
name|height
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"bad framebuffer height %u\n"
argument_list|,
name|r
operator|->
name|height
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"no buffer object handle for plane %d\n"
argument_list|,
name|i
argument_list|)
expr_stmt|;
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"bad pitch %u for plane %d\n"
argument_list|,
name|r
operator|->
name|pitches
...
name|DRM_DEBUG_KMS
argument_list|(
literal|"bad framebuffer flags 0x%08x\n"
argument_list|,
name|r
operator|->
name|flags
...
name|EINVAL
return|;
comment|/* 	 * Bitmask enum properties have the additional constraint of values 	 * from 0 to 63 	 */
if|if
condition|(
operator|(
name|property
...
name|DRM_WARNING
argument_list|(
literal|"Failed to attach object property (type: 0x%x). Please "
literal|"increase DRM_OBJECT_MAX_PROPERTY by 1 for each time "
literal|"you see this message on the same object type.\n"
argument_list|,
name|obj
operator|->
name|type
...
specifier|static
name|bool
name|drm_property_change_is_valid
parameter_list|(
name|struct
name|drm_property
modifier|*
...
block|}
decl_stmt|;
comment|/* It does all the locking and checking we need */
return|return
...
specifier|static
name|int
name|drm_mode_connector_set_obj_prop
parameter_list|(
name|struct
name|drm_mode_object
modifier|*
...
begin_comment
comment|/**  * drm_format_num_planes - get the number of planes for format  * @format: pixel format (DRM_FORMAT_*)  *  * RETURNS:  * The number of planes used by the specified pixel format.  */
end_comment
...
begin_comment
comment|/**  * drm_format_plane_cpp - determine the bytes per pixel value  * @format: pixel format (DRM_FORMAT_*)  * @plane: plane index  *  * RETURNS:  * The bytes per pixel value for the specified plane.  */
end_comment
...
begin_comment
comment|/**  * drm_format_horz_chroma_subsampling - get the horizontal chroma subsampling factor  * @format: pixel format (DRM_FORMAT_*)  *  * RETURNS:  * The horizontal chroma subsampling factor for the  * specified pixel format.  */
end_comment
...
begin_comment
comment|/**  * drm_format_vert_chroma_subsampling - get the vertical chroma subsampling factor  * @format: pixel format (DRM_FORMAT_*)  *  * RETURNS:  * The vertical chroma subsampling factor for the  * specified pixel format.  */
end_comment
