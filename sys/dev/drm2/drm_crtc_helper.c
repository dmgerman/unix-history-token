...
begin_comment
comment|/**  * drm_helper_probe_single_connector_modes - get complete set of display modes  * @connector: connector to probe  * @maxX: max width for modes  * @maxY: max height for modes  *  * LOCKING:  * Caller must hold mode config lock.  *  * Based on the helper callbacks implemented by @connector try to detect all  * valid modes.  Modes will first be added to the connector's probed_modes list,  * then culled (based on validity and the @maxX, @maxY parameters) and put into  * the normal modes list.  *  * Intended to be use as a generic implementation of the ->probe() @connector  * callback for drivers that use the crtc helpers for output mode filtering and  * detection.  *  * RETURNS:  * Number of modes found on @connector.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Re-enable polling in case the global poll config changed. */
if|if
condition|(
name|drm_kms_helper_poll
operator|!=
...
ifdef|#
directive|ifdef
name|CONFIG_DRM_LOAD_EDID_FIRMWARE
name|count
operator|=
...
begin_comment
comment|/**  * drm_crtc_helper_set_config - set a new config from userspace  * @set: mode set configuration  *  * LOCKING:  * Caller must hold mode config lock.  *  * Setup a new configuration, provided by the upper layers (either an ioctl call  * from userspace or internally e.g. from the fbdev suppport code) in @set, and  * enable it. This is the main helper functions for drivers that implement  * kernel mode setting with the crtc helper functions and the assorted  * ->prepare(), ->modeset() and ->commit() helper callbacks.  *  * RETURNS:  * Returns 0 on success, -ERRNO on failure.  */
end_comment
...
begin_comment
comment|/**  * drm_helper_connector_dpms() - connector dpms helper implementation  * @connector: affected connector  * @mode: DPMS mode  *  * This is the main helper function provided by the crtc helper framework for  * implementing the DPMS connector attribute. It computes the new desired DPMS  * state for all encoders and crtcs in the output mesh and calls the ->dpms()  * callback provided by the driver appropriately.  */
end_comment
...
argument_list|)
block|{
comment|/* Ignore forced connectors. */
if|if
condition|(
name|connector
operator|->
...
condition|)
continue|continue;
comment|/* Ignore HPD capable connectors and connectors where we don't 		 * want any hotplug detection at all for polling. */
if|if
condition|(
operator|!
name|connector
...
argument_list|)
block|{
comment|/* Only handle HPD capable connectors. */
if|if
condition|(
operator|!
operator|(
