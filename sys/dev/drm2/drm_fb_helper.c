...
name|c
decl_stmt|;
comment|/* TODO handle difference between  */
switch|switch
condition|(
name|sc
operator|->
...
directive|if
literal|0
block|if (new_fb) { 		info->var.pixclock = 0; 		if (register_framebuffer(info)< 0) { 			return -EINVAL; 		}  		printf("fb%d: %s frame buffer device\n", info->node, 		       info->fix.id);  	} else { 		drm_fb_helper_set_par(info); 	}
comment|/* Switch back to kernel console on panic */
comment|/* multi card linked list maybe */
block|if (list_empty(&kernel_fb_helper_list)) { 		printf("drm: registered panic notifier\n"); 		atomic_notifier_chain_register(&panic_notifier_list,&paniced); 	} 	if (new_fb) 		list_add(&fb_helper->kernel_fb_list,&kernel_fb_helper_list);
endif|#
directive|endif
name|struct
name|vt_kms_softc
...
name|bits_per_pixel
expr_stmt|;
comment|/* XXX: fix depth in VT, bpp is pix size, depth is meaning bits size */
name|sc
operator|->
name|sc_height
operator|=
...
name|width
expr_stmt|;
comment|/* Save fb_helper (XXX Do we really need it?) */
name|sc
operator|->
name|fb_helper
operator|=
...
name|DRM_DEBUG
argument_list|(
literal|"Attach VT to %dx%d fb: P %#lx V %#lx\n"
argument_list|,
name|fb_helper
operator|->
name|fb
