...
directive|if
literal|0
block|time_t		last_recv; 	time_t		last_rxerr; 	time_t		last_xmit;  	u_long		rx_error;  	u_long		short_error; 	u_long		crc_error; 	u_long		dribble_error; 	u_long		long_error; 	u_long		abort_error; 	u_long		overflow_error;  	int		last_error; 	int		prev_error;  	u_long		tx_pending;
endif|#
directive|endif
block|}
struct|;
...
directive|if
literal|0
comment|/* XXX: not yet xmit */
block|sc->reg->srd = sc->last = 0x0b20 + ch; 	tsleep(&sc->last, PZERO + PCATCH, "con4", hz);
endif|#
directive|endif
...
literal|0
comment|/* XXX: not yet xmit */
block|sc->reg->srd = sc->last = 0x0820 + ch; 	tsleep(&sc->last, PZERO + PCATCH, "con4", hz);
endif|#
directive|endif
return|return
operator|(
...
literal|0
comment|/* XXX: not yet xmit */
block|sc->reg->srd = sc->last = 0x0920 + sch->chan; 	tsleep(&sc->last, PZERO + PCATCH, "con4", hz);
endif|#
directive|endif
for|for
control|(
...
directive|if
literal|0
comment|/* We don't really need to reread the Time Slot Map */
block|sc->reg->srd = sc->last = 0x1800; 	tsleep(&sc->last, PZERO + PCATCH, "con1", hz); 	sc->reg->srd = sc->last = 0x1820; 	tsleep(&sc->last, PZERO + PCATCH, "con2", hz);
endif|#
directive|endif
return|return
operator|(
