...
block|,
comment|/* OEM: Huawei */
comment|/* Handled separately. Do not add! 	{{ USB_VENDOR_HUAWEI, USB_PRODUCT_HUAWEI_MOBILE },		U3GFL_NONE }, 	{{ USB_VENDOR_HUAWEI, USB_PRODUCT_HUAWEI_E220 },		U3GFL_NONE }, 	 */
comment|/* OEM: Novatel */
block|{
block|{
...
condition|)
block|{
comment|/* We attach to the interface instead of the device as 			 * some devices have a built-in SD card reader on the 			 * second interface. If the interface class of the 			 * first interface is no longer mass storage it has 			 * changed appearance and we should attach it. 			 */
name|id
operator|=
...
name|id
decl_stmt|;
comment|/* These are 3G modem devices (E220, Mobile, etc.) with auto-install 	 * flash disks for Windows/MacOSX through the first interface. 	 */
if|if
condition|(
name|uaa
operator|->
...
directive|if
literal|0
block|if (!bootverbose) 		device_quiet(self);
endif|#
directive|endif
return|return
literal|0
