...
modifier|...
parameter_list|)
value|do { if (u3gdebug) device_printf(self, ##x); } while (0)
end_define
...
value|0x01
comment|// Requires init (Huawei cards)
define|#
directive|define
...
expr_stmt|;
block|}
comment|/* Avoid excessive buffer sizes. On modern fast machines this is 		 * not needed. 		 * XXX The values here should be checked. Lower them and see 		 * whether 'lost chars' messages appear. 		 */
if|if
condition|(
name|tp
operator|->
...
name|DPRINTF
argument_list|(
literal|"ispeedwat = %d, ospeedwat = %d\n"
argument_list|,
name|tp
operator|->
name|t_ispeedwat
...
name|DPRINTF
argument_list|(
literal|"ispeedwat = default, ospeedwat = default\n"
argument_list|)
expr_stmt|;
name|ttsetwater
argument_list|(
...
condition|)
block|{
comment|/* XXX Should we delay the kick? 		 */
name|DPRINTF
argument_list|(
