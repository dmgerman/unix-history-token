...
begin_endif
unit|static fb_blank_display_t udl_fb_blank_display;
endif|#
directive|endif
end_endif
...
begin_endif
unit|static int udl_power_save(struct udl_softc *, int, int);
endif|#
directive|endif
end_endif
...
directive|if
literal|0
block|DEVMETHOD(fb_blank_display, udl_fb_blank_display),
endif|#
directive|endif
name|DEVMETHOD_END
block|}
...
begin_endif
unit|static int udl_fb_blank_display(device_t dev, int mode) { 	struct udl_softc *sc = device_get_softc(dev);  	switch (mode) { 	case V_DISPLAY_ON: 		udl_power_save(sc, 1, M_WAITOK); 		break; 	case V_DISPLAY_BLANK: 		udl_power_save(sc, 1, M_WAITOK); 		if (sc->sc_fb_addr != 0) { 			const uint32_t max = udl_get_fb_size(sc);  			memset((void *)sc->sc_fb_addr, 0, max); 		} 		break; 	case V_DISPLAY_STAND_BY: 	case V_DISPLAY_SUSPEND: 		udl_power_save(sc, 0, M_WAITOK); 		break; 	} 	return (0); }
endif|#
directive|endif
end_endif
...
begin_comment
unit|static int udl_power_save(struct udl_softc *sc, int on, int flags) { 	struct udl_cmd_buf *cb;
comment|/* get new buffer */
end_comment

begin_endif
unit|cb = udl_cmd_buf_alloc(sc, flags); 	if (cb == NULL) 		return (EAGAIN);  	DPRINTF("screen %s\n", on ? "ON" : "OFF");  	sc->sc_power_save = on ? 0 : 1;  	if (on) 		udl_cmd_write_reg_1(cb, UDL_REG_SCREEN, UDL_REG_SCREEN_ON); 	else 		udl_cmd_write_reg_1(cb, UDL_REG_SCREEN, UDL_REG_SCREEN_OFF);  	udl_cmd_write_reg_1(cb, UDL_REG_SYNC, 0xff); 	udl_cmd_buf_send(sc, cb); 	return (0); }
endif|#
directive|endif
end_endif
