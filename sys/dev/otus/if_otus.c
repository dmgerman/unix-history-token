...
argument_list|)
expr_stmt|;
comment|/* XXX operating mode? filter? */
name|OTUS_UNLOCK
argument_list|(
name|sc
argument_list|)
...
begin_comment
comment|/*  * Set up operating mode, MAC/BSS address and RX filter.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we're in sniffer mode or we don't have a MAC 	 * address assigned, ensure it gets reset to all-zero. 	 */
name|IEEE80211_ADDR_COPY
argument_list|(
name|bssid
argument_list|,
...
name|IEEE80211_M_MONITOR
case|:
comment|/* 		 * Note: monitor mode ends up causing the MAC to 		 * generate ACK frames for everything it sees. 		 * So don't do that; instead just put it in STA mode 		 * and disable RX filters. 		 */
default|default:
name|cam_mode
operator||=
name|AR_MAC_CAM_STA
...
break|break;
block|}
comment|/* 	 * TODO: if/when we do hardware encryption, ensure it's 	 * disabled if the NIC is in monitor mode. 	 */
name|otus_write
argument_list|(
name|sc
argument_list|,
...
parameter_list|)
block|{
comment|//	struct ieee80211com *ic =&sc->sc_ic;
name|OTUS_LOCK_ASSERT
argument_list|(
name|sc
argument_list|)
...
directive|if
literal|0
block|if (ic->ic_allmulti> 0 || ic->ic_promisc> 0 || 	    ic->ic_opmode == IEEE80211_M_MONITOR) { 		otus_write(sc, AR_MAC_REG_FRAMETYPE_FILTER, 0xff00ffff); 	} else {
endif|#
directive|endif
