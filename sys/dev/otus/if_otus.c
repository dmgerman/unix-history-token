...
argument_list|)
expr_stmt|;
comment|/* 	 * XXX TODO: take temporary copy of EDCA information 	 * when scheduling this so we have a more time-correct view 	 * of things. 	 */
name|otus_updateedca
argument_list|(
name|sc
argument_list|)
...
begin_comment
comment|/*  * This is called by net80211 in RX packet context, so we  * can't sleep here.  *  * TODO: have net80211 schedule an update itself for its  * own internal taskqueue.  */
end_comment

begin_function
specifier|static
name|int
name|otus_wme_update
parameter_list|(
name|struct
name|ieee80211com
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* 	 * XXX TODO: has to reach into the cmd queue "waiting for 	 * an RX response" list, grab the head entry and check 	 */
if|if
condition|(
operator|(
name|hdr
