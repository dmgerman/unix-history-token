...
modifier|...
parameter_list|)
value|do {				\ 	if (sc->sc_debug& (m))					\ 		printf("%s: " fmt, __func__, ## __VA_ARGS__);	\ } while (0)
end_define
...
directive|if
literal|0
block|if (sc->sc_flags& ZYD_FLAG_WAIT_COMMAND) {
comment|/* 			 * don't send anything while a command is pending ! 			 */
block|DPRINTF(sc, ZYD_DEBUG_ANY, "wait command\n"); 			break; 		}
endif|#
directive|endif
name|data
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Stop now if the device has vanished or was never set up */
if|if
condition|(
name|sc
operator|->
