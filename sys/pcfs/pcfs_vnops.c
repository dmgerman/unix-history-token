...
begin_comment
comment|/*  *  Written by Paul Popelka (paulp@uts.amdahl.com)  *  *  You can do anything you want with this software,  *    just don't say you wrote it,  *    and don't remove this notice.  *  *  This software is provided "as is".  *  *  The author supplies this software to be publicly  *  redistributed on the understanding that the author  *  is not responsible for the correct functioning of  *  this software in any circumstances and is not liable  *  for any damages caused by this software.  *  *  October 1992  *  *	$Header: /usr/src/CVS/sys/pcfs/pcfs_vnops.c,v 1.1.2.2 1993/02/07 21:57:27 friedl Exp $  *  */
end_comment
...
begin_comment
comment|/* defines plimit structure in the proc struct */
end_comment
...
name|printf
argument_list|(
literal|"pcfs_create(ndp %08x, vap %08x, p %08x\n"
argument_list|,
name|ndp
argument_list|,
name|vap
...
expr_stmt|;
break|break;
comment|/*  *  pcfs_create() sets ndp->ni_vp.  */
case|case
name|VREG
case|:
name|error
...
name|de_dev
expr_stmt|;
comment|/* The following computation of the fileid must be the 	 * same as that used in pcfs_readdir() to compute d_fileno. 	 * If not, pwd doesn't work. */
if|if
condition|(
name|dep
operator|->
...
name|printf
argument_list|(
literal|"pcfs_setattr(): vp %08x, vap %08x, cred %08x, p %08x\n"
argument_list|,
name|vp
argument_list|,
name|vap
...
name|printf
argument_list|(
literal|"pcfs_setattr(): returning EINVAL\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
...
name|printf
argument_list|(
literal|"pcfs_write(vp %08x, uio %08x, ioflag %08x, cred %08x\n"
argument_list|,
name|vp
argument_list|,
name|uio
...
name|printf
argument_list|(
literal|"pcfs_write(): diroff %d, dirclust %d, startcluster %d\n"
argument_list|,
name|dep
operator|->
name|de_diroffset
...
name|panic
argument_list|(
literal|"pcfs_write(): non-sync directory update"
argument_list|)
expr_stmt|;
name|isadir
operator|=
...
name|panic
argument_list|(
literal|"pcfs_write(): bad file type"
argument_list|)
expr_stmt|;
break|break;
block|}
...
begin_comment
comment|/*  *  Since the dos filesystem does not allow files with  *  holes in them we must fill the file with zeroed  *  blocks when a seek past the end of file happens.  *  *  It seems that nothing in the kernel calls the filesystem  *  specific file seek functions.  And, someone on the  *  net told me that NFS never sends announcements of  *  seeks to the server.  So, if pcfs ever becomes  *  NFS mountable it will have to use other means to  *  fill in holes in what would be a sparse file.  */
end_comment
...
name|printf
argument_list|(
literal|"pcfs_seek(vp %08x, oldoff %d, newoff %d, cred %08x)\n"
argument_list|,
name|vp
argument_list|,
name|oldoff
...
name|printf
argument_list|(
literal|"pcfs_remove(), dep %08x, v_usecount %d\n"
argument_list|,
name|dep
argument_list|,
name|ndp
...
argument_list|)
expr_stmt|;
comment|/* causes pcfs_inactive() to be called via vrele() */
name|deput
argument_list|(
name|ddep
argument_list|)
...
name|printf
argument_list|(
literal|"pcfs_rename(fndp %08x, tndp %08x, p %08x\n"
argument_list|,
name|fndp
argument_list|,
name|tndp
...
name|panic
argument_list|(
literal|"pcfs_rename(): lost to startdir"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|panic
argument_list|(
literal|"pcfs_rename(): updating .. in root directory?\n"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|printf
argument_list|(
literal|"pcfs_mkdir(): deput(%08x), vnode %08x\n"
argument_list|,
name|pdep
argument_list|,
name|DETOV
...
name|printf
argument_list|(
literal|"pcfs_readdir(): vp %08x, uio %08x, cred %08x, eofflagp %08x\n"
argument_list|,
name|vp
argument_list|,
name|uio
...
directive|endif
comment|/* defined(PCFSDEBUG) */
comment|/*  *  pcfs_readdir() won't operate properly on regular files  *  since it does i/o only with the the filesystem vnode,  *  and hence can retrieve the wrong block from the buffer  *  cache for a plain file.  So, fail attempts to readdir()  *  on a plain file.  */
if|if
condition|(
operator|(
name|dep
...
condition|)
block|{
comment|/*printf("pcfs_readdir(): going after . or .. in root dir, offset %d\n", 	uio->uio_offset);*/
name|bias
operator|=
literal|2
operator|*
...
block|{
comment|/* this computation of d_fileno must match 				 * the computation of va_fileid in pcfs_getattr */
if|if
condition|(
name|dentp
operator|->
...
name|lost
expr_stmt|;
comment|/*  *  I don't know why we bother setting this eofflag, getdirentries()  *  in vfs_syscalls.c doesn't bother to look at it when we return.  */
if|if
condition|(
name|dep
operator|->
...
name|panic
argument_list|(
literal|"pcfs_unlock: denode not locked"
argument_list|)
expr_stmt|;
name|DEUNLOCK
argument_list|(
...
name|printf
argument_list|(
literal|"tag VT_PCFS, startcluster %d, dircluster %d, diroffset %d "
argument_list|,
name|dep
operator|->
name|de_StartCluster
