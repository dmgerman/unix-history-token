...
directive|if
literal|0
block|error = rtinit(&aa->aa_ifa, RTM_ADD, flags);
else|#
directive|else
name|struct
name|at_addr
...
begin_comment
comment|/*  * aa_addrangeroute()  *  * Add a route for a range of networks from bot to top .  * bot == top means a single net.  * Algorithm:  *  * Starting with the lowest net on it's own, keep doubling the subnet size  * until it overflows the netrange (either at the top or the bottom)  * then back off one bit (to the last subnet that fit), and use that.  * Repeat the operation for the left over space at the top of the  * netrange until the entire range is done. This produces a minimal  * spanning set of binary subnets that cover the netrange.  *  * May need to use this to clear the routes out too.. not sure yet. XXX  */
end_comment
