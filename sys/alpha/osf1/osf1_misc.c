...
begin_comment
comment|/*  * [ taken from the linux emulator ]  * Search an alternate path before passing pathname arguments on  * to system calls. Useful for keeping a seperate 'emulation tree'.  *  * If cflag is set, we check if an attempt can be made to create  * the named file, i.e. we check if the directory it should  * be in exists.  */
end_comment
...
begin_comment
unit|return (EINVAL); 	}  	error = fcntl(p,&a);  	if (error) 		return error;  	switch (SCARG(uap, cmd)) { 	case OSF1_F_GETFL:
comment|/* XXX */
end_comment
