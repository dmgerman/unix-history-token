...
literal|0
expr_stmt|;
comment|/* 		 * On input, fix ip_len which has been byte-swapped 		 * at ip_input(). 		 */
if|if
condition|(
operator|!
name|out
...
block|{
comment|/* Hash verification has already been done successfully. */
name|tc
operator|=
operator|(
expr|struct
...
return|;
block|}
comment|/* Only save information if crypto processing is needed. */
if|if
condition|(
name|mtag
operator|==
...
name|mtag
expr_stmt|;
comment|/* Save the mtag we've identified. */
if|if
condition|(
name|mtag
operator|==
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we have an mtag, we don't need to verify the authenticator -- 	 * it has been verified by an IPsec-aware NIC. 	 */
if|if
condition|(
name|mtag
operator|==
...
begin_comment
comment|/*  * AH output routine, called by ipsec[46]_process_packet().  */
end_comment
