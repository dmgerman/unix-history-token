...
begin_comment
comment|/* $Id: ifiter_ioctl.c,v 1.34 2002/08/16 00:05:57 marka Exp $ */
end_comment
...
literal|0
expr_stmt|;
comment|/* 	 * Create an unbound datagram socket to do the SIOCGLIFADDR ioctl on. 	 */
if|if
condition|(
operator|(
name|iter
...
condition|)
block|{
comment|/* 		 * XXX This should be looked at further since it looks strange. 		 * If we get an ENXIO then we ignore the error and not worry 		 * about the flags. 		 */
if|if
condition|(
name|errno
operator|!=
...
name|INTERFACE_F_LOOPBACK
expr_stmt|;
comment|/*  	 * Note that IPv6 broadcast does not exist 	 * so don't check for IPv6 broadcast flag 	 */
ifdef|#
directive|ifdef
name|IFF_MULTICAST
if|if
