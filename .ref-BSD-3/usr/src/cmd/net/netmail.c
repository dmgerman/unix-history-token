...
begin_comment
comment|/*   netmail [-l ...] [-p ...] [-f] [-n] [-r] [-c] ([mach] | [mach:username])     Read mail on remote machine "mach"    Sends a command to the remote machine to "mail" the mail    to this machine.    The -r option says remove any mail you find on the    remote machine after sending it back to this machine.    Uses the -q option of net, so is a quiet command.     If the -c option is specified, this command is a mail check command,    and in this mode it logs into the remote machine as "network"    and determines if mach:username has mail.    If so, it writes/mails a message to that effect.    If not, it should be silent.    This variant is intended to be used in .login files, silently    checking if you have mail on another machine.     Must duplicate effort that will be redone by the net command-    the calls to commandfile and promptlogin are necessary    to get a value for the login name to send to the prmail    command on the other machine.    May read the passwd file: 	1. Commandfile calls getenv(HOME) to get the home directory. 	   If not easily reached,.... 	2. SnCurrent() calls getlogin(). If no entry in utmp file, 	   will read passwd file.  */
end_comment
...
name|stderr
argument_list|,
literal|"Usage: netmail [-l login] [-p password] [-c] [-f] [-n] [-r] [mach]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
expr_stmt|;
block|}
comment|/* get local address to send to prmail, store in status.localname */
name|s
operator|=
name|SnCurrent
argument_list|()
