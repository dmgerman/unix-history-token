...
begin_comment
comment|/*  * It is important that these #define's only affect this .h file in here  * because we depend on the routing stuff getting the current names.  */
end_comment
...
name|in6
expr_stmt|;
comment|/* 		tcp2->th_sum = in6_cksum(m, IPPROTO_TCP, 					 sizeof(*ip6), sizeof(*tcp2)); */
return|return
name|ipf_send_ip
argument_list|(
name|fin
...
argument_list|)
expr_stmt|;
comment|/* 		icmp->icmp_cksum = in6_cksum(m, IPPROTO_ICMPV6, 					     sizeof(*ip6), iclen - hlen); */
block|}
else|else
endif|#
directive|endif
...
directive|if
literal|0
comment|/* ifdef	USE_INET6 */
block|if (fin->fin_v == 6) {                 dst6->sin6_family = AF_INET6;                  fr = fin->fin_fr;                 if (fdp != NULL)                         ifp = fdp->fd_ifp;                 else {                         ifp = fin->fin_ifp;                         dst->sin6_addr = fin->fin_daddr6;                 }                  ip6tx.tx_mbuf = *mpp;                 ip6tx.tx_ip6 = (ip6_t *)ip;                 ip6tx.tx_ro = ro;                 ip6tx.tx_if6 = NULL;                 ip6tx.tx_nexthop = dst6;                 ip6tx.tx_imo6 = NULL;                 ip6tx.tx_pmtudisc = 0;                 ip6tx.tx_dontroute = 0;                 ip6tx.tx_rawoutput = 0;                 ip6tx.tx_mtu = ifp->if_mtu;                 ip6tx.tx_opt = NULL;
comment|/*                  * currently "to<if>" and "to<if>:ip#" are not supported                  * for IPv6                  */
block|return ip6_output(&ip6tx); 	}
endif|#
directive|endif
