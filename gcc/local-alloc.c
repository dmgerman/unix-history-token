...
begin_comment
comment|/* Allocate registers within a basic block, for GNU compiler.    Copyright (C) 1987, 1988, 1991, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006 Free Software Foundation,    Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
name|init_insns
decl_stmt|;
comment|/* Nonzero if this had a preexisting REG_EQUIV note.  */
name|int
name|is_arg_equivalence
decl_stmt|;
block|}
struct|;
end_struct
...
literal|1
return|;
comment|/* Nonconst functions may access memory.  */
if|if
condition|(
name|CALL_P
argument_list|(
...
begin_comment
comment|/* Find registers that are equivalent to a single value throughout the    compilation (either because they can be referenced in memory or are set once    from a single constant).  Lower their priority for a register.     If such a register is only referenced once, try substituting its value    into the using insn.  If it succeeds, we can eliminate the register    completely.     Initialize the REG_EQUIV_INIT array of initializing insns.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* See if this is setting up the equivalence between an argument 	     register and its stack slot.  */
name|note
operator|=
name|find_reg_note
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Note that we don't want to clear reg_equiv_init even if there 		 are multiple sets of this register.  */
name|reg_equiv
index|[
name|regno
index|]
...
literal|1
expr_stmt|;
comment|/* Record for reload that this is an equivalencing insn.  */
if|if
condition|(
name|rtx_equal_p
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Continue normally in case this is a candidate for 		 replacements.  */
block|}
if|if
condition|(
operator|!
...
argument_list|)
decl_stmt|;
comment|/* If we haven't done so, record for reload that this is an 		 equivalencing insn.  */
if|if
condition|(
operator|!
name|reg_equiv
...
name|out
goto|;
comment|/* A second pass, to gather additional equivalences with memory.  This needs      to be done after we know which registers we are going to replace.  */
for|for
control|(
name|insn
operator|=
...
argument_list|)
expr_stmt|;
comment|/* This insn makes the equivalence, not the one initializing 		 the register.  */
name|reg_equiv_init
index|[
name|regno
index|]
...
literal|0
expr_stmt|;
comment|/* Make sure this insn is recognized before 			 reload begins, otherwise 			 eliminate_regs_in_insn will die.  */
name|INSN_CODE
argument_list|(
name|new_insn
argument_list|)
...
name|NULL_RTX
expr_stmt|;
comment|/* This doesn't matter for equivalences made for argument registers, we      should keep their initialization insns.  */
if|if
condition|(
name|reg_equiv
index|[
...
name|unsigned
name|int
name|rest_of_handle_local_alloc
parameter_list|(
name|void
parameter_list|)
block|{
