...
begin_comment
comment|/* Include output.h for dump_file.  */
end_comment
...
begin_comment
comment|/* When I2MOD is nonnull, this is a copy of the old right hand side.  */
end_comment
...
begin_comment
comment|/* When I2MOD is nonnull, this is a copy of the new right hand side.  */
end_comment
...
literal|8
expr_stmt|;
comment|/* Set nonzero if references to register n in expressions should not be      used.  last_set_invalid is set nonzero when this register is being      assigned to and last_set_table_tick == label_tick.  */
name|char
name|last_set_invalid
decl_stmt|;
...
name|unsigned
name|char
name|sign_bit_copies
decl_stmt|;
name|unsigned
name|HOST_WIDE_INT
name|nonzero_bits
decl_stmt|;
comment|/* Record the value of the label_tick when the last truncation      happened.  The field truncated_to_mode is only valid if      truncation_label == label_tick.  */
name|int
name|truncation_label
decl_stmt|;
comment|/* Record the last truncation seen for this register.  If truncation      is not a nop to this mode we might be able to save an explicit      truncation if we know that value already contains a truncated      value.  */
name|ENUM_BITFIELD
argument_list|(
argument|machine_mode
argument_list|)
...
begin_comment
comment|/* The following array records the insn_rtx_cost for every insn    in the instruction stream.  */
end_comment
...
begin_comment
comment|/* Length of the currently allocated uid_insn_cost array.  */
end_comment
...
begin_comment
comment|/* Mode used to compute significance in reg_stat[].nonzero_bits.  It is the    largest integer mode that can fit in HOST_BITS_PER_WIDE_INT.  */
end_comment
...
begin_comment
comment|/* Nonzero when reg_stat[].nonzero_bits and reg_stat[].sign_bit_copies can    be safely used.  It is zero while computing them and after combine has    completed.  This former test prevents propagating values based on    previously set values, which can be incorrect if a variable is modified    in a loop.  */
end_comment
...
begin_comment
comment|/* Record one modification to rtl structure    to be undone by storing old_contents into *where.  */
end_comment
...
specifier|static
name|rtx
name|reg_nonzero_bits_for_combine
parameter_list|(
name|rtx
parameter_list|,
name|enum
...
specifier|static
name|rtx
name|reg_num_sign_bit_copies_for_combine
parameter_list|(
name|rtx
parameter_list|,
name|enum
...
specifier|static
name|rtx
name|distribute_and_simplify_rtx
parameter_list|(
name|rtx
parameter_list|,
name|int
...
begin_comment
comment|/* It is not safe to use ordinary gen_lowpart in combine.    See comments in gen_lowpart_for_combine.  */
end_comment
...
directive|define
name|RTL_HOOKS_GEN_LOWPART
value|gen_lowpart_for_combine
end_define

begin_comment
comment|/* Our implementation of gen_lowpart never emits a new pseudo.  */
end_comment
...
directive|define
name|RTL_HOOKS_REG_NONZERO_REG_BITS
value|reg_nonzero_bits_for_combine
end_define
...
directive|define
name|RTL_HOOKS_REG_NUM_SIGN_BIT_COPIES
value|reg_num_sign_bit_copies_for_combine
end_define
...
begin_comment
comment|/* Similar to SUBST, but just substitute the mode.  This is used when    changing the mode of a pseudo-register, so that any other    references to the entry in the regno_reg_rtx array will change as    well.  */
end_comment
...
name|NEWVAL
parameter_list|)
value|do_SUBST_MODE(&(INTO), (NEWVAL))
end_define
...
begin_comment
comment|/* Subroutine of try_combine.  Determine whether the combine replacement    patterns NEWPAT and NEWI2PAT are cheaper according to insn_rtx_cost    that the original instruction sequence I1, I2 and I3.  Note that I1    and/or NEWI2PAT may be NULL_RTX.  This function returns false, if the    costs of all instructions can be estimated, and the replacements are    more expensive than the original sequence.  */
end_comment

begin_function
specifier|static
name|bool
name|combine_validate_cost
parameter_list|(
name|rtx
name|i1
parameter_list|,
...
name|new_cost
decl_stmt|;
comment|/* Lookup the original insn_rtx_costs.  */
name|i2_cost
operator|=
name|INSN_UID
argument_list|(
...
expr_stmt|;
block|}
comment|/* Calculate the replacement insn_rtx_costs.  */
name|new_i3_cost
operator|=
name|insn_rtx_cost
argument_list|(
...
expr_stmt|;
block|}
comment|/* Disallow this recombination if both new_cost and old_cost are      greater than zero, and new_cost is greater than old cost.  */
if|if
condition|(
name|old_cost
operator|>
...
name|dump_file
argument_list|,
literal|"rejecting combination of insns %d, %d and %d\n"
argument_list|,
name|INSN_UID
argument_list|(
name|i1
...
name|dump_file
argument_list|,
literal|"original costs %d + %d + %d = %d\n"
argument_list|,
name|i1_cost
argument_list|,
name|i2_cost
...
name|dump_file
argument_list|,
literal|"rejecting combination of insns %d and %d\n"
argument_list|,
name|INSN_UID
argument_list|(
name|i2
...
name|dump_file
argument_list|,
literal|"replacement costs %d + %d = %d\n"
argument_list|,
name|new_i2_cost
argument_list|,
name|new_i3_cost
...
return|;
block|}
comment|/* Update the uid_insn_cost array with the replacement costs.  */
name|uid_insn_cost
index|[
name|INSN_UID
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Don't use reg_stat[].nonzero_bits when computing it.  This can cause      problems when, for example, we have j<<= 1 in a loop.  */
name|nonzero_sign_valid
operator|=
literal|0
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Allocate array of current insn_rtx_costs.  */
name|uid_insn_cost
operator|=
name|XCNEWVEC
argument_list|(
...
directive|endif
comment|/* Record the current insn_rtx_cost of this instruction.  */
if|if
condition|(
name|NONJUMP_INSN_P
argument_list|(
...
name|retry
goto|;
comment|/* Try this insn with each REG_EQUAL note it links back to.  */
for|for
control|(
name|links
operator|=
...
operator|!=
name|EXPR_LIST
comment|/* Avoid using a register that may already been marked 			 dead by an earlier instruction.  */
operator|&&
operator|!
name|unmentioned_reg_p
argument_list|(
...
condition|)
block|{
comment|/* Temporarily replace the set's source with the 			 contents of the REG_EQUAL note.  The insn will 			 be deleted or recognized by try_combine.  */
name|rtx
name|orig
init|=
name|SET_SRC
...
begin_comment
comment|/* Wipe the last_xxx fields of reg_stat in preparation for another pass.  */
end_comment
...
argument_list|)
operator|)
comment|/* Don't substitute into a non-local goto, this confuses CFG.  */
operator|||
operator|(
name|JUMP_P
argument_list|(
...
argument_list|)
operator|)
comment|/* Make sure that the value that is to be substituted for the register 	 does not use any registers whose values alter in between.  However, 	 If the insns are adjacent, a use can't cross a set even though we 	 think it might (this can happen for a sequence of insns each setting 	 the same destination; last_set of that register might point to 	 a NOTE).  If INSN has a REG_EQUIV note, the register is always 	 equivalent to the memory so the substitution is valid even if there 	 are intervening stores.  Also, don't move a volatile asm or 	 UNSPEC_VOLATILE across any other insns.  */
operator|||
operator|(
operator|!
name|all_adjacent
...
literal|0
return|;
comment|/* If INSN contains an autoincrement or autodecrement, make sure that      register is not used between there and I3, and not already used in      I3 either.  Neither must it be used in PRED or SUCC, if they exist.      Also insist that I3 not be a jump; if it were one      and the incremented register were spilled, we would lose.  */
ifdef|#
directive|ifdef
name|AUTO_INC_DEC
for|for
...
argument_list|)
expr_stmt|;
comment|/* Check for the case where I3 modifies its output, as discussed 	 above.  We don't want to prevent pseudos from being combined 	 into the address of a MEM, so only prevent the combination if 	 i1 or i2 set the same MEM.  */
if|if
condition|(
operator|(
name|inner_dest
...
begin_struct
struct|struct
name|likely_spilled_retval_info
block|{
name|unsigned
name|regno
decl_stmt|,
...
begin_comment
comment|/* Called via note_stores by likely_spilled_retval_p.  Remove from info->mask    hard registers that are known to be written to / clobbered in full.  */
end_comment

begin_function
specifier|static
name|void
name|likely_spilled_retval_1
parameter_list|(
name|rtx
name|x
parameter_list|,
...
begin_comment
comment|/* Return nonzero iff part of the return value is live during INSN, and    it is likely spilled.  This can happen when more than one insn is needed    to copy the return value, e.g. when we consider to combine into the    second copy insn for a complex value.  */
end_comment

begin_function
specifier|static
name|int
name|likely_spilled_retval_p
parameter_list|(
name|rtx
name|insn
parameter_list|)
...
name|nregs
decl_stmt|;
comment|/* We assume here that no machine mode needs more than      32 hard registers when the value overlaps with a register      for which FUNCTION_VALUE_REGNO_P is true.  */
name|unsigned
name|mask
decl_stmt|;
name|struct
...
literal|1
expr_stmt|;
comment|/* Disregard parts of the return value that are set later.  */
name|info
operator|.
name|regno
operator|=
...
name|mask
expr_stmt|;
comment|/* Check if any of the (probably) live return value registers is      likely spilled.  */
name|nregs
operator|--
expr_stmt|;
do|do
...
begin_comment
comment|/* Return TRUE if combine can reuse reg X in mode MODE.    ADDED_SETS is nonzero if the original set is still required.  */
end_comment

begin_function
specifier|static
name|bool
name|can_change_dest_mode
parameter_list|(
name|rtx
name|x
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* Allow hard registers if the new mode is legal, and occupies no more      registers than the old mode.  */
if|if
condition|(
name|regno
operator|<
...
operator|)
return|;
comment|/* Or a pseudo that is only used once.  */
return|return
operator|(
name|REG_N_SETS
argument_list|(
...
begin_comment
comment|/* Check whether X, the destination of a set, refers to part of    the register specified by REG.  */
end_comment
...
parameter_list|)
block|{
comment|/* Check that reg is an integer mode register.  */
if|if
condition|(
operator|!
name|REG_P
...
name|total_sets
decl_stmt|;
comment|/* Nonzero if I2's body now appears in I3.  */
name|int
name|i2_is_used
decl_stmt|;
...
literal|0
decl_stmt|;
comment|/* PATTERN (I1) and PATTERN (I2), or a copy of it in certain cases.  */
name|rtx
name|i1pat
init|=
literal|0
...
block|}
block|}
comment|/* If I2 is setting a pseudo to a constant and I3 is setting some      sub-part of it to another constant, merge them by making a new      constant.  */
if|if
condition|(
name|i1
operator|==
...
condition|)
block|{
comment|/* If this is the low part, we're done.  */
if|if
condition|(
...
condition|)
empty_stmt|;
comment|/* Handle the case where inner is twice the size of outer.  */
elseif|else
if|if
condition|(
name|GET_MODE_BITSIZE
...
argument_list|)
expr_stmt|;
comment|/* Otherwise give up for now.  */
else|else
name|offset
operator|=
operator|-
...
directive|if
literal|0
block|if (!(GET_CODE (PATTERN (i3)) == SET&& REG_P (SET_SRC (PATTERN (i3)))&& MEM_P (SET_DEST (PATTERN (i3)))&& (GET_CODE (XEXP (SET_DEST (PATTERN (i3)), 0)) == POST_INC 	    || GET_CODE (XEXP (SET_DEST (PATTERN (i3)), 0)) == POST_DEC)))
...
condition|)
block|{
comment|/* Pass pc_rtx so no substitutions are done, just 	     simplifications.  */
if|if
condition|(
name|i1
condition|)
...
operator|)
operator|)
comment|/* Fail if we tried to make a new register.  */
operator|||
name|max_reg_num
argument_list|()
operator|!=
...
argument_list|)
expr_stmt|;
comment|/* If recog_for_combine fails, it strips existing clobbers.  If we'll      consider splitting this pattern, we might need these clobbers.  */
if|if
condition|(
name|i1
operator|&&
...
argument_list|)
decl_stmt|;
comment|/* First try to split using the original register as a 	     scratch register.  */
name|m_split
operator|=
name|split_insns
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If that didn't work, try changing the mode of I2DEST if 	     we can.  */
if|if
condition|(
name|m_split
operator|==
...
block|}
block|}
comment|/* If recog_for_combine has discarded clobbers, try to use them 	 again for the split.  */
if|if
condition|(
name|m_split
operator|==
...
directive|endif
comment|/* We need I2DEST in the proper mode.  If it is a hard register 	     or the only use of a pseudo, we can change its mode. 	     Make sure we don't change a hard register to have a mode that 	     isn't valid for it, or change the number of registers.  */
operator|&&
operator|(
name|GET_MODE
argument_list|(
...
directive|endif
comment|/* Attempt to split binary operators using arithmetic identities.  */
if|if
condition|(
name|BINARY_P
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* Split "X = Y op Y" as "Z = Y; X = Z op Z".  */
if|if
condition|(
name|rtx_equal_p
argument_list|(
...
expr_stmt|;
block|}
comment|/* Split "((P op Q) op R) op S" where op is PLUS or MULT.  */
elseif|else
if|if
condition|(
operator|(
...
name|src_op1
decl_stmt|;
comment|/* Split both "((X op Y) op X) op Y" and 		     "((X op Y) op Y) op X" as "T op T" where T is 		     "X op Y".  */
if|if
condition|(
operator|(
name|rtx_equal_p
...
expr_stmt|;
block|}
comment|/* Split "((X op X) op Y) op Y)" as "T op T" where 		     T is "X op Y".  */
elseif|else
if|if
condition|(
name|rtx_equal_p
...
argument_list|)
expr_stmt|;
comment|/* recog_for_combine might have added CLOBBERs to newi2pat. 	     Make sure NEWPAT does not depend on the clobbered regs.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
directive|ifdef
name|HAVE_cc0
comment|/* We cannot split the parallel into two sets if both sets 	      reference cc0.  */
operator|&&
operator|!
operator|(
name|reg_referenced_p
...
directive|ifdef
name|HAVE_cc0
comment|/* If I2 is the CC0 setter and I3 is the CC0 user then check whether      they are adjacent to each other or not.  */
block|{
name|rtx
name|p
init|=
...
directive|endif
comment|/* Only allow this combination if insn_rtx_costs reports that the      replacement instructions are cheaper than the originals.  */
if|if
condition|(
operator|!
name|combine_validate_cost
...
name|ni2dest
decl_stmt|;
comment|/* I3 now uses what used to be its destination and which is now 	 I2's destination.  This requires us to do a few adjustments.  */
name|PATTERN
argument_list|(
name|i3
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* We need a LOG_LINK from I3 to I2.  But we used to have one, 	 so we still will.  	 However, some later insn might be using I2's dest and have 	 a LOG_LINK pointing at I3.  We must remove this link. 	 The simplest way to remove the link is to point it at I1, 	 which we know will be a NOTE.  */
comment|/* newi2pat is usually a SET here; however, recog_for_combine might 	 have added some clobbers.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Update reg_stat[].nonzero_bits et al for any changes that may have        been made to this insn.  The order of        set_nonzero_bits_and_sign_copies() is important.  Because newi2pat        can affect nonzero_bits of newpat */
if|if
condition|(
name|newi2pat
condition|)
...
parameter_list|)
define|\
value|((X) == (Y)						\    || (REG_P (X)&& REG_P (Y)	\&& REGNO (X) == REGNO (Y)&& GET_MODE (X) == GET_MODE (Y)))
if|if
condition|(
operator|!
name|in_dest
...
begin_comment
comment|/* Simplify X, a piece of RTL.  We just operate on the expression at the    outer level; call `subst' to simplify recursively.  Return the new    expression.     OP0_MODE is the original mode of XEXP (x, 0).  IN_DEST is nonzero    if we are inside a SET_DEST.  */
end_comment
...
name|x
return|;
comment|/* Simplify the alternative arms; this may collapse the true and 	     false arms to store-flag values.  Be careful to use copy_rtx 	     here since true_rtx or false_rtx might share RTL with x as a 	     result of the if_then_else_cond call above.  */
name|true_rtx
operator|=
name|subst
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* See if this can be moved to simplify_subreg.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
name|x
argument_list|)
comment|/* Don't call gen_lowpart if the inner mode 		is VOIDmode and we cannot simplify it, as SUBREG without 		inner mode is invalid.  */
operator|&&
operator|(
name|GET_MODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Similarly to what we do in simplify-rtx.c, a truncate of a register 	 whose value is a comparison can be replaced with a subreg if 	 STORE_FLAG_VALUE permits.  */
if|if
condition|(
name|GET_MODE_BITSIZE
argument_list|(
...
argument_list|)
return|;
comment|/* (IF_THEN_ELSE (NE REG 0) (0) (8)) is REG for nonzero_bits (REG) == 8.  */
if|if
condition|(
name|true_code
operator|==
...
begin_comment
comment|/* Simplify, X, and AND, IOR, or XOR operation, and return the simplified    result.  */
end_comment
...
expr_stmt|;
block|}
comment|/* If we have any of (and (ior A B) C) or (and (xor A B) C), 	 apply the distributive law and then the inverse distributive 	 law to see if things simplify.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* This should stay within the object being extracted, fail otherwise.  */
if|if
condition|(
name|len
operator|+
...
argument_list|)
decl_stmt|;
comment|/* Make sure this is a profitable operation.  */
if|if
condition|(
name|rtx_cost
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* A constant position should stay within the width of INNER.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* We can't call gen_lowpart in a DEST since we 		 always want a SUBREG (see below) and it would sometimes 		 return a new hard register.  */
if|if
condition|(
name|pos
operator|||
...
literal|0
return|;
comment|/* Unless INNER is not MEM, reject this if we would be spanning bytes or      if the position is not a constant and the length is not 1.  In all      other cases, we would only be going outside our object in cases when      an original shift would have been undefined.  */
if|if
condition|(
name|MEM_P
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If this is not from memory, the desired mode is the preferred mode      for an extraction pattern's first input operand, or word_mode if there      is none.  */
if|if
condition|(
operator|!
name|MEM_P
...
block|{
comment|/* Be careful not to go beyond the extracted object and maintain the 	 natural alignment of the memory.  */
name|wanted_inner_mode
operator|=
...
expr_stmt|;
block|}
comment|/* If we have to change the mode of memory and cannot, the desired mode 	 is EXTRACTION_MODE.  */
if|if
condition|(
name|inner_mode
operator|!=
...
argument_list|)
expr_stmt|;
comment|/* POS may be less than 0 now, but we check for that below. 	 Note that it can only be less than 0 if !MEM_P (inner).  */
block|}
comment|/* If INNER has a wider mode, and this is a constant extraction, try to      make it smaller and adjust the byte to point to the byte containing      the value.  */
if|if
condition|(
name|wanted_inner_mode
operator|!=
...
argument_list|)
expr_stmt|;
comment|/* We can now move to the desired byte.  */
name|offset
operator|+=
operator|(
name|pos
...
expr_stmt|;
block|}
comment|/* If this is a commutative operation, the changes to the operands      may have made it noncanonical.  */
if|if
condition|(
name|COMMUTATIVE_ARITH_P
argument_list|(
...
begin_comment
comment|/* If X refers to a register that equals REG in value, replace these    references with REG.  */
end_comment
...
begin_comment
comment|/* Return X converted to MODE.  If the value is already truncated to    MODE we can just return a subreg even though in the general case we    would need an explicit truncation.  */
end_comment
...
name|temp
decl_stmt|;
comment|/* If this is a CALL or ASM_OPERANDS, don't do anything.  Some of the      code below will do the wrong thing since the mode of such an      expression is VOIDmode.       Also do nothing if X is a CLOBBER; this can happen if X was      the return value from a call to gen_lowpart.  */
if|if
condition|(
name|code
operator|==
...
argument_list|)
expr_stmt|;
comment|/* When we have an arithmetic operation, or a shift whose count we      do not know, we need to assume that all bits up to the highest-order      bit in MASK will be needed.  This is how we form such a mask.  */
if|if
condition|(
name|mask
operator|&
...
argument_list|)
expr_stmt|;
comment|/* We can only change the mode of the shift if we can do arithmetic 	     in the mode of the shift and INNER_MASK is no wider than the 	     width of X's mode.  */
if|if
condition|(
operator|(
name|inner_mask
...
return|;
block|}
comment|/* If DEST is already a field assignment, i.e. ZERO_EXTRACT, and the      SRC is an AND with all bits of that field set, then we can discard      the AND.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
literal|1
expr_stmt|;
comment|/* Complete overlap.  We can remove the source AND.  */
if|if
condition|(
operator|(
name|and_mask
...
argument_list|)
return|;
comment|/* Partial overlap.  We can reduce the source AND.  */
if|if
condition|(
operator|(
name|and_mask
...
argument_list|)
expr_stmt|;
comment|/* If SRC is masked by an AND that does not make a difference in      the value being stored, strip it.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|tem
decl_stmt|;
comment|/* Distributivity is not true for floating point as it can change the      value.  So we don't do it unless -funsafe-math-optimizations.  */
if|if
condition|(
name|FLOAT_MODE_P
argument_list|(
...
operator|>
name|UNITS_PER_WORD
comment|/* Result might need to be truncated.  Don't change mode if 	     explicit truncation is needed.  */
operator|||
operator|!
name|TRULY_NOOP_TRUNCATION
argument_list|(
...
return|;
block|}
comment|/* Set LHS and RHS to the inner operands (A and B in the example      above) and set OTHER to the common operand (C in the example).      There is only one way to do this unless the inner operation is      commutative.  */
if|if
condition|(
name|COMMUTATIVE_ARITH_P
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* There is one exception to the general way of distributing:      (a | c) ^ (b | c) -> (a ^ b)& ~c  */
if|if
condition|(
name|code
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Special case (and (xor B C) (not A)), which is equivalent to      (xor (ior A B) (ior A C))  */
if|if
condition|(
name|outer_code
operator|==
...
condition|)
block|{
comment|/* Distribute the second term.  */
name|new_op0
operator|=
name|simplify_gen_binary
argument_list|(
...
block|{
comment|/* Distribute the first term.  */
name|new_op0
operator|=
name|simplify_gen_binary
argument_list|(
...
begin_comment
comment|/* Simplify a logical `and' of VAROP with the constant CONSTOP, to be done    in MODE.  Return an equivalent form, if different from (and VAROP    (const_int CONSTOP)).  Otherwise, return NULL_RTX.  */
end_comment
...
argument_list|)
return|;
comment|/* If VAROP is PLUS, and the constant is a mask of low bits, distribute      the AND and see if one of the operands simplifies to zero.  If so, we      may eliminate it.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
return|;
block|}
comment|/* Make a SUBREG if necessary.  If we can't make it, fail.  */
name|varop
operator|=
name|gen_lowpart
argument_list|(
...
begin_comment
comment|/* Given a REG, X, compute which bits in X can be nonzero.    We don't care about bits outside of those defined in MODE.     For most X this is simply GET_MODE_MASK (GET_MODE (MODE)), but if X is    a shift, AND, or zero_extract, we can do better.  */
end_comment
...
name|const0
expr_stmt|;
comment|/* If OP0 or OP1 is UNKNOWN, this is easy.  Similarly if they are the same or      if OP0 is SET.  */
if|if
condition|(
name|op1
operator|==
...
condition|)
block|{
comment|/* If we have an operand of (clobber (const_int 0)), fail.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
operator|==
name|CONST_INT
comment|/* We can't do this if we have (ashiftrt (xor))  and the 		 constant has its sign bit set in shift_mode.  */
operator|&&
operator|!
operator|(
name|code
...
continue|continue;
block|}
comment|/* If we can't do that, try to simplify the shift in each arm of the 	     logical expression, make a new logical expression, and apply 	     the inverse distributive law.  This also can't be done 	     for some (ashiftrt (xor)).  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
continue|continue;
block|}
comment|/* Check for 'PLUS signbit', which is the canonical form of 'XOR 	     signbit', and attempt to change the PLUS to an XOR and move it to 	     the outer operation as is done above in the AND/IOR/XOR case 	     leg for shift(logical). See details in logical handling above 	     for reasoning in doing so.  */
if|if
condition|(
name|code
operator|==
...
begin_comment
comment|/* Like gen_lowpart_general but for use by combine.  In combine it    is not possible to create any new pseudoregs.  However, it is    safe to create invalid memory addresses, because combine will    try to recognize them and all they will do is make the combine    attempt fail.     If for some reason this cannot do its job, an rtx    (clobber (const_int 0)) is returned.    An insn containing that will not be recognized.  */
end_comment
...
name|x
return|;
comment|/* Return identity if this is a CONST or symbolic reference.  */
if|if
condition|(
name|omode
operator|==
...
argument_list|)
expr_stmt|;
comment|/* For use in case we fall down into the address adjustments 	 further below, we need to adjust the known mode and size of 	 x; imode and isize, since we just adjusted x.  */
name|imode
operator|=
name|GET_MODE
argument_list|(
...
name|fail
goto|;
comment|/* If we want to refer to something bigger than the original memref, 	 generate a paradoxical subreg instead.  That will force a reload 	 of the original memref X.  */
if|if
condition|(
name|isize
operator|<
...
comment|/* Except we can't if SHIFT_COUNT_TRUNCATED is set, since we might 	     have already reduced the shift count modulo the word size.  */
if|if
condition|(
operator|!
name|SHIFT_COUNT_TRUNCATED
...
name|unsigned_comparison_p
operator|)
comment|/* (A - C1) zero-extends if it is positive and sign-extends 		      if it is negative, C2 both zero- and sign-extends.  */
operator|&&
operator|(
operator|(
literal|0
...
literal|0
operator|)
comment|/* (A - C1) sign-extends if it is positive and 1-extends 			  if it is negative, C2 both sign- and 1-extends.  */
operator|||
operator|(
name|num_sign_bit_copies
argument_list|(
...
literal|2
operator|)
comment|/* (A - C1) always sign-extends, like C2.  */
operator|&&
name|num_sign_bit_copies
argument_list|(
name|a
...
continue|continue;
block|}
comment|/* If this AND operation is really a ZERO_EXTEND from a narrower 	     mode, the constant fits within that mode, and this is either an 	     equality or unsigned comparison, try to do this comparison in 	     the narrower mode.  	     Note that in:  	     (ne:DI (and:DI (reg:DI 4) (const_int 0xffffffff)) (const_int 0)) 	     -> (ne:DI (reg:SI 4) (const_int 0))  	     unless TRULY_NOOP_TRUNCATION allows it or the register is 	     known to hold a value of the required mode the 	     transformation is invalid.  */
if|if
condition|(
operator|(
name|equality_comparison_p
...
block|}
block|}
comment|/* Convert (ne (and (not X) 1) 0) to (eq (and X 1) 0).  */
if|if
condition|(
name|const_op
operator|==
...
continue|continue;
block|}
comment|/* Convert (ne (and (lshiftrt (not X)) 1) 0) to 	     (eq (and (lshiftrt X) 1) 0). 	     Also handle the case where (not X) is expressed using xor.  */
if|if
condition|(
name|const_op
operator|==
...
condition|)
block|{
comment|/* For paradoxical subregs, allow case 1 as above.  Case 3 isn't 	     implemented.  */
if|if
condition|(
name|REG_P
argument_list|(
...
begin_comment
comment|/* Utility function for record_value_for_reg.  Count number of    rtxs in X.  */
end_comment
...
begin_comment
comment|/* Utility function for following routine.  Called when X is part of a value    being stored into last_set_value.  Sets last_set_table_tick    for each register mentioned.  Similar to mention_regs in cse.c  */
end_comment
...
begin_comment
comment|/* Record that REG is set to VALUE in insn INSN.  If VALUE is zero, we    are saying that the register is clobbered and we no longer know its    value.  If INSN is zero, don't update reg_stat[].last_set; this is    only permitted with VALUE also zero and is used to invalidate the    register.  */
end_comment
...
condition|)
block|{
comment|/* If there are two or more occurrences of REG in VALUE, 		 prevent the value from growing too much.  */
if|if
condition|(
name|count_rtxs
argument_list|(
...
begin_comment
comment|/* Update the records of when each REG was most recently set or killed    for the things done by INSN.  This is the last thing done in processing    INSN in the combiner loop.     We update reg_stat[], in particular fields last_set, last_set_value,    last_set_mode, last_set_nonzero_bits, last_set_sign_bit_copies,    last_death, and also the similar information mem_last_set (which insn    most recently modified memory) and last_call_cuid (which insn was the    most recent subroutine call).  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* We can't combine into a call pattern.  Remember, though, that 	 the return value register is set at this CUID.  We could 	 still replace a register with the return value from the 	 wrong subroutine call!  */
name|note_stores
argument_list|(
name|PATTERN
argument_list|(
...
begin_comment
comment|/* Check if X, a register, is known to contain a value already    truncated to MODE.  In this case we can use a subreg to refer to    the truncated value even though in the generic case we would need    an explicit truncation.  */
end_comment
...
begin_comment
comment|/* X is a REG or a SUBREG.  If X is some sort of a truncation record    it.  For non-TRULY_NOOP_TRUNCATION targets we might be able to turn    a truncate into a subreg using this information.  */
end_comment
...
expr_stmt|;
block|}
comment|/* ??? For hard-regs we now record everything.  We might be able to      optimize this using last_set_mode.  */
elseif|else
if|if
condition|(
name|REG_P
...
begin_comment
comment|/* Scan X for promoted SUBREGs and truncated REGs.  For each one    found, note what it implies to the registers used in it.  */
end_comment
...
literal|0
expr_stmt|;
comment|/* Check that reg isn't mentioned in NEWPAT_USED_REGS.  For fixed registers      we allow the machine description to decide whether use-and-clobber      patterns are OK.  */
if|if
condition|(
name|reg_dead_regno
operator|<
...
case|:
comment|/* Just get rid of this note, as it is unused later anyway.  */
break|break;
case|case
name|REG_NON_LOCAL_GOTO
case|:
...
expr_stmt|;
block|}
comment|/* Don't attach REG_LABEL note to a JUMP_INSN.  Add 	     a JUMP_LABEL instead or decrement LABEL_NUSES.  */
if|if
condition|(
name|place
operator|&&
...
name|REG_NONNEG
case|:
comment|/* This note says something about the value of a register prior 	     to the execution of an insn.  It is too much trouble to see 	     if the note is still correct in all situations.  It is better 	     to simply delete it.  */
break|break;
case|case
name|REG_RETVAL
case|:
...
literal|0
expr_stmt|;
comment|/* Don't add the dangling REG_RETVAL note.  */
elseif|else
if|if
condition|(
operator|!
...
literal|0
expr_stmt|;
comment|/* Don't add the dangling REG_LIBCALL note.  */
elseif|else
if|if
condition|(
operator|!
...
continue|continue;
block|}
comment|/* If the register is being set at TEM, see if that is all 		     TEM is doing.  If so, delete TEM.  Otherwise, make this 		     into a REG_UNUSED note instead. Don't delete sets to 		     global register vars.  */
if|if
condition|(
operator|(
name|REGNO
...
argument_list|)
expr_stmt|;
comment|/*  If there isn't already a REG_UNUSED note, put one 			      here.  Do not place a REG_DEAD note, even if 			      the register is also used here; that would not 			      match the algorithm used in lifetime analysis 			      and can cause the consistency check in the 			      scheduler to fail.  */
if|if
condition|(
operator|!
name|find_regno_note
...
expr_stmt|;
block|}
comment|/* If the register is set or already dead at PLACE, we needn't do 	     anything with this note if it is still a REG_DEAD note. 	     We check here if it is set at all, not if is it totally replaced, 	     which is what `dead_or_set_p' checks, so also check for it being 	     set partially.  */
if|if
condition|(
name|place
operator|&&
...
condition|)
block|{
comment|/* Unless the register previously died in PLACE, clear 		     last_death.  [I no longer understand why this is 		     being done.] */
if|if
condition|(
name|reg_stat
index|[
...
begin_comment
comment|/* Subroutine of unmentioned_reg_p and callback from for_each_rtx.    Check whether the expression pointer to by LOC is a register or    memory, and if so return 1 if it isn't mentioned in the rtx EXPR.    Otherwise return zero.  */
end_comment
...
begin_comment
comment|/* Check for any register or memory mentioned in EQUIV that is not    mentioned in EXPR.  This is used to restrict EQUIV to "specializations"    of EXPR where some registers may have been replaced by constants.  */
end_comment
