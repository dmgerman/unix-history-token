...
begin_comment
comment|/* Return the type info for TYPE as used by EH machinery.  */
end_comment
...
begin_comment
comment|/* Build up a call to __cxa_get_exception_ptr so that we can build a    copy constructor for the thrown object.  */
end_comment

begin_function
specifier|static
name|tree
name|do_get_exception_ptr
parameter_list|(
name|void
parameter_list|)
block|{
...
name|get_identifier
argument_list|(
literal|"__cxa_get_exception_ptr"
argument_list|)
expr_stmt|;
if|if
condition|(
...
condition|)
block|{
comment|/* Declare void* __cxa_get_exception_ptr (void *).  */
name|tree
name|tmp
init|=
name|tree_cons
...
name|error
argument_list|(
literal|"type %qT is disallowed in Java %<throw%> or %<catch%>"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"type %qT is not derived from %<java::lang::Throwable%>"
argument_list|,
name|TREE_TYPE
argument_list|(
name|decl
...
return|;
block|}
comment|/* Call __cxa_end_catch at the end of processing the exception.  */
name|push_eh_cleanup
argument_list|(
name|type
argument_list|)
expr_stmt|;
comment|/* If there's no decl at all, then all we need to do is make sure      to tell the runtime that we've begun handling the exception.  */
if|if
condition|(
name|decl
operator|==
...
argument_list|)
expr_stmt|;
comment|/* If the C++ object needs constructing, we need to do that before      calling __cxa_begin_catch, so that std::uncaught_exception gets      the right value during the copy constructor.  */
elseif|else
if|if
condition|(
...
expr_stmt|;
block|}
comment|/* Otherwise the type uses a bitwise copy, and we don't have to worry      about the value of std::uncaught_exception and therefore can do the      copy with the return value of __cxa_end_catch instead.  */
else|else
block|{
name|tree
name|init
...
name|type
decl_stmt|;
comment|/* Pointers are passed by values, everything else by reference.  */
if|if
condition|(
operator|!
name|TYPE_PTR_P
...
begin_comment
comment|/* Call __cxa_free_exception from a cleanup.  This is never invoked    directly, but see the comment for stabilize_throw_expr.  */
end_comment

begin_function
specifier|static
name|tree
name|do_free_exception
parameter_list|(
name|tree
name|ptr
parameter_list|)
...
begin_comment
comment|/* Wrap all cleanups for TARGET_EXPRs in MUST_NOT_THROW_EXPR.    Called from build_throw via walk_tree_without_duplicates.  */
end_comment
...
operator|)
expr_stmt|;
comment|/* And initialize the exception object.  */
if|if
condition|(
name|CLASS_TYPE_P
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Wrap the initialization in a CLEANUP_POINT_EXPR so that cleanups 	 for temporaries within the initialization are run before the one 	 for the exception object, preserving LIFO order.  */
name|exp
operator|=
name|build1
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Prepend the allocation.  */
name|exp
operator|=
name|build2
argument_list|(
...
condition|)
block|{
comment|/* Prepend the calculation of the throw expression.  Also, force 	     any cleanups from the expression to be evaluated here so that 	     we don't have to do them during unwinding.  But first wrap 	     them in MUST_NOT_THROW_EXPR, since they are run after the 	     exception object is initialized.  */
...
argument_list|)
expr_stmt|;
comment|/* Tack on the initialization stuff.  */
name|exp
operator|=
name|build2
argument_list|(
...
expr_stmt|;
block|}
comment|/* ??? Indicate that this function call allows exceptions of the type 	 of the enclosing catch block (if known).  */
name|exp
operator|=
name|build_function_call
argument_list|(
...
begin_comment
comment|/* Make sure TYPE is complete, pointer to complete, reference to    complete, or pointer to cv void. Issue diagnostic on failure.    Return the zero on failure and nonzero on success. FROM can be    the expr or decl from whence TYPE came, if available.  */
end_comment
...
name|error
argument_list|(
literal|"expression %qE of abstract class type %qT cannot "
literal|"be used in throw-expression"
argument_list|,
name|expr
argument_list|,
name|type
...
literal|0
return|;
comment|/* Being a C library function, DECL_ASSEMBLER_NAME == DECL_NAME      unless the system headers are playing rename tricks, and if      they are, we don't want to be confused by them.  */
name|id
operator|=
name|DECL_NAME
argument_list|(
...
literal|0
argument_list|,
literal|"%Hexception of type %qT will be caught"
argument_list|,
name|EXPR_LOCUS
argument_list|(
name|handler
...
literal|0
argument_list|,
literal|"%H   by earlier handler for %qT"
argument_list|,
name|EXPR_LOCUS
argument_list|(
name|master
...
begin_comment
comment|/* Given a STATEMENT_LIST of HANDLERs, make sure that they're OK.  */
end_comment
...
name|i
decl_stmt|;
comment|/* If we don't have a STATEMENT_LIST, then we've just got one      handler, and thus nothing to warn about.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|pedwarn
argument_list|(
literal|"%H%<...%> handler must be the last handler for"
literal|" its try block"
argument_list|,
name|EXPR_LOCUS
argument_list|(
name|handler
