...
begin_comment
comment|/* Perform branch target register load optimizations.    Copyright (C) 2001, 2002, 2003, 2004, 2005, 2006    Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
decl_stmt|;
comment|/* We set own_end when we have moved a definition into a dominator.      Thus, when a later combination removes this definition again, we know      to clear out trs_live_at_end again.  */
name|char
name|own_end
decl_stmt|;
name|bitmap
...
begin_comment
comment|/* Array indexed by basic block number, giving the set of registers live at   the end of that block, including any uses by a final jump insn, if any.  */
end_comment
...
begin_comment
comment|/* Return an estimate of the frequency of execution of block bb.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Check for the blockage emitted by expand_nl_goto_receiver.  */
elseif|else
if|if
condition|(
...
name|user
decl_stmt|;
comment|/* Do the equivalent of calling note_other_use_this_block 		     for every target register.  */
for|for
control|(
name|user
operator|=
...
argument_list|)
expr_stmt|;
comment|/* If this block ends in a jump insn, add any uses or even clobbers 	 of branch target registers that it might have.  */
for|for
control|(
name|insn
operator|=
...
argument_list|)
expr_stmt|;
comment|/* ??? for the fall-through edge, it would make sense to insert the 	 btr set on the edge, but that would require to split the block 	 early on so that we can distinguish between dominance from the fall 	 through edge - which can use the call-clobbered registers - from 	 dominance by the throw edge.  */
if|if
condition|(
...
begin_comment
comment|/* We are adding the def/use web DEF.  Add the target register used    in this web to the live set of all of the basic blocks that contain    the live range of the web.    If OWN_END is set, also show that the register is live from our    definitions at the end of the basic block where it is defined.  */
end_comment
...
begin_comment
comment|/* Update a live range to contain the basic block NEW_BLOCK, and all    blocks on paths between the existing live range and NEW_BLOCK.    HEAD is a block contained in the existing live range that dominates    all other blocks in the existing live range.    Also add to the set BTRS_LIVE_IN_RANGE all target registers that    are live in the blocks that we add to the live range.    If FULL_RANGE is set, include the full live range of NEW_BB;    otherwise, if NEW_BB dominates HEAD_BB, only add registers that    are life at the end of NEW_BB for NEW_BB itself.    It is a precondition that either NEW_BLOCK dominates HEAD,or    HEAD dom NEW_BLOCK.  This is used to speed up the    implementation of this function.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* A previous btr migration could have caused a register to be 	live just at the end of new_block which we need in full, so 	use trs_live_at_end even if full_range is set.  */
name|IOR_HARD_REG_SET
argument_list|(
operator|*
name|btrs_live_in_range
...
name|dump_file
argument_list|,
literal|"Adding end of block %d and rest of %d to live range\n"
argument_list|,
name|new_block
argument_list|,
name|head_bb
...
argument_list|)
expr_stmt|;
comment|/* A previous btr migration could have caused a register to be 	     live just at the end of a block which we need in full.  */
name|IOR_HARD_REG_SET
argument_list|(
operator|*
name|btrs_live_in_range
...
name|e
decl_stmt|;
comment|/* If TRY has abnormal edges, skip it.  */
name|FOR_EACH_EDGE
argument_list|(
argument|e
argument_list|,
...
specifier|static
name|bool
name|gate_handle_branch_target_load_optimize
parameter_list|(
name|void
parameter_list|)
block|{
...
name|unsigned
name|int
name|rest_of_handle_branch_target_load_optimize
parameter_list|(
name|void
parameter_list|)
block|{
