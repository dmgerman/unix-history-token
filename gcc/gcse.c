...
begin_comment
comment|/* Global common subexpression elimination/Partial redundancy elimination    and global constant/copy propagation for GNU compiler.    Copyright (C) 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005    Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
parameter_list|)
define|\
value|(gcc_assert (INSN_UID (INSN)<= max_uid), uid_cuid[INSN_UID (INSN)])
end_define
...
name|next
decl_stmt|;
comment|/* The index of the block where it was set.  */
name|int
name|bb_index
decl_stmt|;
block|}
...
begin_comment
comment|/* This is a list of expressions which are MEMs and will be used by load    or store motion.    Load motion tracks MEMs which aren't killed by    anything except itself. (i.e., loads and stores to a single location).    We can then allow movement of these MEM refs with a little special    allowance. (all stores copy the same value to the reaching reg used    for the loads).  This means all values used to store into memory must have    no side effects so we can re-issue the setter value.    Store Motion uses this structure as an expression table to track stores    which look interesting, and might be moveable towards the exit block.  */
end_comment
...
begin_comment
comment|/* Hashtable for the load/store memory refs.  */
end_comment
...
begin_comment
comment|/* For each block, a bitmap of registers set in the block.    This is used by compute_transp.    It is computed during hash table computation and not by compute_sets    as it includes registers added since the last pass (or between cprop and    gcse) and it's currently not easy to realloc sbitmap vectors.  */
end_comment
...
begin_comment
comment|/* Bitmap indexed by block numbers to record which blocks contain    function calls.  */
end_comment

begin_decl_stmt
specifier|static
name|bitmap
name|blocks_with_calls
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Number of local constants propagated.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|local_const_prop_count
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Number of local copies propagated.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|local_copy_prop_count
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Number of global constants propagated.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|global_const_prop_count
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Number of global copies propagated.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|global_copy_prop_count
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Entry point for global common subexpression elimination.    F is the first instruction in the function.  Return nonzero if a    change is mode.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* It does not make sense to run code hoisting unless we are optimizing 	 for code size -- it rarely makes programs faster, and can make 	 them bigger if we did partial redundancy elimination (when optimizing 	 for space, we don't run the partial redundancy algorithms).  */
if|if
condition|(
name|optimize_size
condition|)
...
name|insn
decl_stmt|;
comment|/* Find the largest UID and create a mapping from UIDs to CUIDs.      CUIDs are like UIDs except they increase monotonically, have no gaps,      and only apply to real insns.      (Actually, there are gaps, for insn that are not inside a basic block.      but we should never see those anyway, so this is OK.)  */
name|max_uid
operator|=
name|get_max_uid
argument_list|()
...
directive|ifdef
name|STACK_REGS
comment|/* On register stack architectures, don't GCSE constants from the      constant pool, as the benefits are often swamped by the overhead      of shuffling the register stack between basic blocks.  */
if|if
condition|(
name|IS_STACK_MODE
argument_list|(
...
begin_comment
comment|/* Used internally by can_assign_to_reg_p.  */
end_comment
...
begin_comment
comment|/* Return true if we can assign X to a pseudo register.  */
end_comment
...
index|]
decl_stmt|;
comment|/* If this is a readonly then we aren't going to be changing it.  */
if|if
condition|(
name|MEM_READONLY_P
argument_list|(
...
begin_comment
comment|/* Hash expression X.     MODE is only used if X is a CONST_INT.  DO_NOT_RECORD_P is a boolean    indicating if a volatile operand is found or if the expression contains    something we don't want to insert in the table.  HASH_TABLE_SIZE is    the current size of the hash table to be probed.  */
end_comment
...
begin_comment
comment|/* Return nonzero if exp1 is equivalent to exp2.  */
end_comment
...
name|tmp
decl_stmt|;
comment|/* See if a REG_NOTE shows this equivalent to a simpler expression. 	 This allows us to do a single GCSE pass and still eliminate 	 redundant constants, addresses or other expressions that are 	 constructed with multiple instructions.  */
name|note
operator|=
...
begin_comment
comment|/* Release memory used by modify_mem_list_set.  */
end_comment
...
name|bb_index
decl_stmt|;
comment|/* First handle all the blocks with calls.  We don't need to 	     do any list walking for them.  */
name|EXECUTE_IF_SET_IN_BITMAP
argument_list|(
argument|blocks_with_calls
argument_list|,
literal|0
argument_list|,
argument|bb_index
...
expr_stmt|;
block|}
comment|/* Now iterate over the blocks which have memory modifications 	       but which do not have any calls.  */
...
expr_stmt|;
block|}
comment|/* If there is already a REG_EQUAL note, update the expression in it      with our replacement.  */
if|if
condition|(
name|note
operator|!=
...
condition|)
break|break;
comment|/* Follow the copy chain, i.e. start another iteration of the loop 	 and see if we have an available copy into SRC.  */
name|regno
operator|=
name|REGNO
argument_list|(
...
name|dump_file
argument_list|,
literal|"GLOBAL CONST-PROP: Replacing reg %d in jump_insn %d with constant "
argument_list|,
name|REGNO
argument_list|(
name|from
...
begin_comment
comment|/* Do local const/copy propagation (i.e. within each basic block).    If ALTER_JUMPS is true, allow propagating into jump insns, which    could modify the CFG.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Forget everything at the end of a basic block.  Make sure we are 	 not inside a libcall, they should never cross basic blocks.  */
name|cselib_clear_table
argument_list|()
expr_stmt|;
name|gcc_assert
argument_list|(
...
name|dump_file
argument_list|,
literal|"%d local const props, %d local copy props, "
argument_list|,
name|local_const_prop_count
argument_list|,
name|local_copy_prop_count
...
name|dump_file
argument_list|,
literal|"%d global const props, %d global copy props\n\n"
argument_list|,
name|global_const_prop_count
argument_list|,
name|global_copy_prop_count
...
name|dump_file
argument_list|,
literal|"JUMP-BYPASS: Proved reg %d "
literal|"in jump_insn %d equals constant "
argument_list|,
name|regno
argument_list|,
name|INSN_UID
...
expr_stmt|;
block|}
comment|/* Compute ae_kill for each basic block using:       ~(TRANSP | COMP)   */
name|FOR_EACH_BB
argument_list|(
argument|bb
argument_list|)
...
condition|)
continue|continue;
comment|/* Insert this expression on this edge if it would 		       reach the deleted occurrence in BB.  */
if|if
condition|(
operator|!
name|TEST_BIT
...
condition|)
block|{
comment|/* If the source was a REG_EQUAL or REG_EQUIV note, we 		 may not find an equivalent expression, but in this 		 case the PARALLEL will have a single set.  */
if|if
condition|(
name|first_set
operator|==
...
condition|)
return|return;
comment|/* Even if the destination cannot trap, the source may.  In this case we'd      need to handle updating the REG_EH_REGION note.  */
if|if
condition|(
name|find_reg_note
argument_list|(
...
condition|)
return|return;
comment|/* Make sure that the SET_SRC of this store insns can be assigned to      a register, or we will fail later on in replace_store_insn, which      assumes that we can do this.  But sometimes the target machine has      oddities like MEM read-modify-write instruction.  See for example      PR24257.  */
if|if
condition|(
operator|!
name|can_assign_to_reg_p
...
begin_comment
comment|/* Check if INSN kills the store pattern X (is aliased with it).    AFTER is true if we are checking the case when store X occurs    after the insn.  Return true if it does.  */
end_comment
...
return|;
block|}
comment|/* We can't put stores in the front of blocks pointed to by abnormal      edges since that may put a store where one didn't used to be.  */
name|gcc_assert
argument_list|(
operator|!
operator|(
...
control|)
block|{
comment|/* If any of the edges we have above are abnormal, we can't move this 	 store.  */
for|for
control|(
name|x
operator|=
...
name|dump_file
argument_list|,
literal|"Can't replace store %d: abnormal edge from %d to %d\n"
argument_list|,
name|ptr
operator|->
name|index
...
expr_stmt|;
block|}
comment|/* If gcse or cse altered any jumps, rerun jump optimizations to clean      things up.  */
if|if
condition|(
name|tem
operator|||
