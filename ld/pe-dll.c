...
specifier|const
name|autofilter_entry_type
name|autofilter_symbollist_generic
index|[]
init|=
block|{
block|{
...
specifier|const
name|autofilter_entry_type
name|autofilter_symbollist_i386
index|[]
init|=
block|{
block|{
...
block|}
block|,
comment|/* Entry point symbols, and entry hooks.  */
block|{
name|STRING_COMMA_LEN
argument_list|(
literal|"cygwin_crt0"
...
block|}
block|,
comment|/* Global vars that should not be exported.  */
block|{
name|STRING_COMMA_LEN
argument_list|(
literal|"impure_ptr"
...
literal|2
block|,
comment|/* ARM_RVA32 on Windows CE, see bfd/coff-arm.c.  */
name|PE_ARCH_arm_wince
block|,
name|bfd_arch_arm
block|,
...
init|=
block|{
comment|/* _imp_ is treated specially, as it is always underscored.  */
comment|/* { STRING_COMMA_LEN ("_imp_") },  */
comment|/* Don't export some c++ symbols.  */
block|{
name|STRING_COMMA_LEN
argument_list|(
literal|"__rtti_"
...
block|}
block|}
comment|/* Don't try to blindly exclude all symbols 	 that begin with '__'; this was tried and 	 it is too restrictive.  Instead we have 	 a target specific list to use:  */
name|afptr
operator|=
name|pe_details
operator|->
...
begin_comment
comment|/*	.section	.idata$4  	.long		0 	[.long		0] for PE+  	.section	.idata$5  	.long		0 	[.long		0] for PE+  	.section	idata$7  	.global		__my_dll_iname   __my_dll_iname:  	.asciz		"my.dll"       */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If len was odd, the above      strcpy leaves behind an undefined byte. That is harmless,      but we set it to 0 just so the binary dumps are pretty.  */
name|d7
index|[
name|len
operator|-
...
argument_list|)
expr_stmt|;
comment|/* We need space for the real thunk and for the null terminator.  */
name|bfd_set_section_size
argument_list|(
name|abfd
argument_list|,
...
begin_comment
comment|/*	.section	.idata$2   	.rva		__nm_thnk_SYM (singleton thunk with name of func)  	.long		0  	.long		0  	.rva		__my_dll_iname (name of dll)  	.rva		__fuNN_SYM (pointer to reference (address) in text)  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Include the jump stub for<sym> only if the<sym> 	       is undefined.  */
if|if
condition|(
operator|!
name|blhe
