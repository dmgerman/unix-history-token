...
specifier|static
name|bfd_boolean
name|xtensa_is_insntable_section
parameter_list|(
name|asection
modifier|*
parameter_list|)
...
specifier|static
name|bfd_boolean
name|xtensa_is_proptable_section
parameter_list|(
name|asection
modifier|*
parameter_list|)
...
argument_list|)
block|,
comment|/* Old relocations for backward compatibility.  */
name|HOWTO
argument_list|(
name|R_XTENSA_OP0
argument_list|,
...
name|reloc_howto_type
modifier|*
name|elf_xtensa_reloc_name_lookup
parameter_list|(
name|bfd
modifier|*
name|abfd
...
begin_comment
comment|/* Xtensa ELF linker hash table.  */
end_comment

begin_struct
struct|struct
name|elf_xtensa_link_hash_table
block|{
name|struct
...
begin_comment
comment|/* Get the Xtensa ELF linker hash table from a link_info structure.  */
end_comment

begin_define
define|#
directive|define
name|elf_xtensa_hash_table
parameter_list|(
name|p
parameter_list|)
define|\
value|((struct elf_xtensa_link_hash_table *) ((p)->hash))
end_define

begin_comment
comment|/* Create an Xtensa ELF linker hash table.  */
end_comment
...
modifier|*
name|elf_xtensa_link_hash_table_create
parameter_list|(
name|bfd
modifier|*
name|abfd
...
specifier|inline
name|bfd_boolean
name|elf_xtensa_dynamic_symbol_p
parameter_list|(
name|struct
name|elf_link_hash_entry
modifier|*
...
argument_list|)
decl_stmt|;
comment|/* Skip any relocations before the current offset.  This should help 	 avoid confusion caused by unexpected relocations for the preceding 	 table entry.  */
while|while
condition|(
name|irel
operator|&&
...
condition|)
block|{
comment|/* For shared objects, there's no need for PLT entries for local 	     symbols (use RELATIVE relocs instead of JMP_SLOT relocs).  */
if|if
condition|(
name|h
operator|->
...
parameter_list|)
block|{
comment|/* Property sections are marked "KEEP" in the linker scripts, but they      should not cause other sections to be marked.  (This approach relies      on elf_xtensa_discard_info to remove property table entries that      describe discarded sections.  Alternatively, it might be more      efficient to avoid using "KEEP" in the linker scripts and instead use      the gc_mark_extra_sections hook to mark only the property sections      that describe marked sections.  That alternative does not work well      with the current property table sections, which do not correspond      one-to-one with the sections they describe, but that should be fixed      someday.) */
if|if
condition|(
name|xtensa_is_property_section
argument_list|(
...
specifier|static
name|bfd_boolean
name|elf_xtensa_allocate_dynrelocs
parameter_list|(
name|struct
name|elf_link_hash_entry
modifier|*
...
literal|4
expr_stmt|;
comment|/* Allocate space in ".rela.got" for literals that reference global 	 symbols and space in ".rela.plt" for literals that have PLT 	 entries.  */
...
return|;
block|}
comment|/* Sort the relocations.  They should already be in order when      relaxation is enabled, but it might not be.  */
name|qsort
argument_list|(
name|cookie
operator|->
...
name|unsigned
name|int
name|elf_xtensa_action_discarded
parameter_list|(
name|asection
modifier|*
name|sec
...
begin_comment
comment|/* Attempt to narrow an instruction.  If the narrowing is valid, perform    the action in-place directly into the contents and return TRUE.  Otherwise,    the return value is FALSE and the contents are not modified.  */
end_comment
...
begin_comment
comment|/* Attempt to widen an instruction.  If the widening is valid, perform    the action in-place directly into the contents and return TRUE.  Otherwise,    the return value is FALSE and the contents are not modified.  */
end_comment
...
begin_comment
comment|/* Do not widen an instruction if it is preceeded by a    loop opcode.  It might cause misalignment.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|prev_instr_is_a_loop
parameter_list|(
name|bfd_byte
modifier|*
name|contents
...
argument_list|)
expr_stmt|;
comment|/* Walk over memory and relocations at the same time.          This REQUIRES that the internal_relocs be sorted by offset.  */
name|qsort
argument_list|(
name|internal_relocs
argument_list|,
...
decl_stmt|;
name|bfd_boolean
name|remove_this_rel
decl_stmt|;
name|flagword
name|flags
decl_stmt|;
comment|/* Find the first relocation for the entry at the current offset. 	     Adjust the offsets of any extra relocations for the previous 	     entry.  */
name|offset_rel
operator|=
name|NULL
expr_stmt|;
...
block|}
block|}
comment|/* Find the next relocation (if there are any left).  */
name|extra_rel
operator|=
name|NULL
expr_stmt|;
...
block|}
block|}
comment|/* Check if there are relocations on the current entry.  There 	     should usually be a relocation on the offset field.  If there 	     are relocations on the size or flags, then we can't optimize 	     this entry.  Also, find the next relocation to examine on the 	     next iteration.  */
if|if
condition|(
name|offset_rel
condition|)
...
name|offset_rel
expr_stmt|;
comment|/* There are no relocations on the current entry, but we 		     might still be able to remove it if the size is zero.  */
name|offset_rel
operator|=
name|NULL
expr_stmt|;
...
condition|)
block|{
comment|/* There is a relocation on the size or flags, so we can't 		     do anything with this entry.  Continue with the next.  */
name|next_rel
operator|=
name|offset_rel
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* In case this is the last entry, move the relocation offset 		 to the previous entry, if there is one.  */
if|if
condition|(
name|offset_rel
operator|->
...
condition|)
block|{
comment|/* Fix up any extra relocations on the last entry.  */
for|for
control|(
name|irel
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Check if the section already exists.  */
name|prop_sec
operator|=
...
define|\
value|elf_xtensa_reloc_name_lookup
end_define
...
value|elf_xtensa_link_hash_table_create
end_define
...
value|elf_xtensa_action_discarded
end_define
