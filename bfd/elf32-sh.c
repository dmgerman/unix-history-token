...
specifier|static
name|int
name|sh_elf_optimized_tls_reloc
parameter_list|(
name|struct
name|bfd_link_info
modifier|*
...
name|r
operator|<
name|R_SH_FIRST_INVALID_RELOC_5
operator|||
name|r
operator|>
name|R_SH_LAST_INVALID_RELOC_5
argument_list|)
expr_stmt|;
name|cache_ptr
operator|->
...
argument_list|)
expr_stmt|;
comment|/* We used to test (ELF32_R_SYM (irelfn->r_info)< symtab_hdr->sh_info) 	 here, but that only checks if the symbol is an external symbol, 	 not if the symbol is in a different section.  Besides, we need 	 a consistent meaning for the relocation, so we just assume here that 	 the value of the symbol is not available.  */
...
condition|)
block|{
comment|/* This has been made by previous relaxation.  Since the 		 relocation will be against an external symbol, the 		 final relocation will just do the right thing.  */
name|start
operator|=
name|stop
operator|=
...
argument_list|)
expr_stmt|;
comment|/* The addend will be against the section symbol, thus 		 for adjusting the addend, the relevant start is the 		 start of the section. 		 N.B. If we want to abandon in-place changes here and 		 test directly using symbol + addend, we have to take into 		 account that the addend has already been adjusted by -4.  */
if|if
condition|(
name|stop
operator|>
...
define|#
directive|define
name|sh_elf_hash_entry
parameter_list|(
name|ent
parameter_list|)
value|((struct elf_sh_link_hash_entry *)(ent))
end_define

begin_struct
struct|struct
name|sh_elf_obj_tdata
block|{
name|struct
name|elf_obj_tdata
name|root
...
parameter_list|)
define|\
value|((struct sh_elf_obj_tdata *) (abfd)->tdata.any)
end_define

begin_define
define|#
directive|define
name|sh_elf_local_got_tls_type
parameter_list|(
name|abfd
parameter_list|)
define|\
value|(sh_elf_tdata (abfd)->local_got_tls_type)
end_define

begin_comment
comment|/* Override the generic function because we need to store sh_elf_obj_tdata    as the specific tdata.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|sh_elf_mkobject
parameter_list|(
name|bfd
modifier|*
name|abfd
...
name|sym_sec
decl_stmt|;
comment|/* A counter or offset to track a TLS got entry.  */
union|union
block|{
name|bfd_signed_vma
name|refcount
...
literal|4
expr_stmt|;
comment|/* R_SH_TLS_GD needs 2 consecutive GOT slots.  */
if|if
condition|(
name|tls_type
operator|==
...
name|dynamic_sections_created
expr_stmt|;
comment|/* R_SH_TLS_IE_32 needs one dynamic relocation if dynamic, 	 R_SH_TLS_GD needs one if local symbol and two if global.  */
if|if
condition|(
operator|(
name|tls_type
...
condition|)
block|{
comment|/* Allocate 2 got entries and 1 dynamic reloc for R_SH_TLS_LD_32 	 relocs.  */
name|htab
operator|->
name|tls_ldm_got
operator|.
...
name|_
argument_list|(
literal|"%B: 0x%lx: fatal: unaligned branch target for relax-support relocation"
argument_list|)
argument_list|,
name|input_section
operator|->
...
name|_
argument_list|(
literal|"%B: 0x%lx: fatal: unaligned %s relocation 0x%lx"
argument_list|)
argument_list|,
name|input_section
operator|->
...
name|_
argument_list|(
literal|"%B: 0x%lx: fatal: R_SH_PSHA relocation %d not in range -32..32"
argument_list|)
argument_list|,
name|input_section
operator|->
...
name|_
argument_list|(
literal|"%B: 0x%lx: fatal: R_SH_PSHL relocation %d not in range -32..32"
argument_list|)
argument_list|,
name|input_section
operator|->
...
condition|)
block|{
comment|/* GD->LE transition: 		       mov.l 1f,r4; mova 2f,r0; mov.l 2f,r1; add r0,r1; 		       jsr @r1; add r12,r4; bra 3f; nop; .align 2; 		       1: .long x$TLSGD; 2: .long __tls_get_addr@PLT; 3: 		     We change it into: 		       mov.l 1f,r4; stc gbr,r0; add r4,r0; nop; 		       nop; nop; ... 		       1: .long x@TPOFF; 2: .long __tls_get_addr@PLT; 3:.  */
name|offset
operator|=
name|rel
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Size of GD instructions is 16 or 18.  */
name|offset
operator|-=
literal|16
expr_stmt|;
...
name|index
decl_stmt|;
comment|/* IE->LE transition: 		     mov.l 1f,r0; stc gbr,rN; mov.l @(r0,r12),rM; 		     bra 2f; add ...; .align 2; 1: x@GOTTPOFF; 2: 		     We change it into: 		     mov.l .Ln,rM; stc gbr,rN; nop; ...; 		     1: x@TPOFF; 2:.  */
name|offset
operator|=
name|rel
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Size of IE instructions is 10 or 12.  */
name|offset
operator|-=
literal|10
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* Relocate R_SH_TLS_IE_32 directly when statically linking.  */
if|if
condition|(
name|r_type
operator|==
...
name|insn
decl_stmt|;
comment|/* GD->IE transition: 		   mov.l 1f,r4; mova 2f,r0; mov.l 2f,r1; add r0,r1; 		   jsr @r1; add r12,r4; bra 3f; nop; .align 2; 		   1: .long x$TLSGD; 2: .long __tls_get_addr@PLT; 3: 		 We change it into: 		   mov.l 1f,r0; stc gbr,r4; mov.l @(r0,r12),r0; add r4,r0; 		   nop; nop; bra 3f; nop; .align 2; 		   1: .long x@TPOFF; 2:...; 3:.  */
name|offset
operator|=
name|rel
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Replace mov.l 1f,R4 with mov.l 1f,r0.  */
name|bfd_put_16
argument_list|(
name|output_bfd
argument_list|,
...
name|insn
decl_stmt|;
comment|/* LD->LE transition: 		   mov.l 1f,r4; mova 2f,r0; mov.l 2f,r1; add r0,r1; 		   jsr @r1; add r12,r4; bra 3f; nop; .align 2; 		   1: .long x$TLSLD; 2: .long __tls_get_addr@PLT; 3: 		 We change it into: 		   stc gbr,r0; nop; nop; nop; 		   nop; nop; bra 3f; ...; 3:.  */
name|offset
operator|=
name|rel
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Size of LD instructions is 16 or 18.  */
name|offset
operator|-=
literal|16
expr_stmt|;
...
begin_comment
comment|/* Return the relocation value for R_SH_TLS_TPOFF32..  */
end_comment
...
literal|0
return|;
comment|/* SH TLS ABI is variant I and static TLS block start just after tcbhead      structure which has 2 pointer fields.  */
return|return
operator|(
name|address
operator|-
...
directive|ifdef
name|INCLUDE_SHMEDIA
comment|/* Take care of both the datalabel and codelabel local 		     GOT offsets.  */
name|sh_elf_local_got_tls_type
argument_list|(
name|abfd
argument_list|)
...
begin_comment
comment|/* Reverse table lookup for sh_ef_bfd_table[].    Given a bfd MACH value from archures.c    return the equivalent ELF flags from the table.    Return -1 if no match is found.  */
end_comment
...
begin_comment
comment|/* This function returns the ELF architecture number that    corresponds to the given arch_sh* flags.  */
end_comment
...
begin_comment
comment|/* This routine initialises the elf flags when required and    calls sh_merge_bfd_arch() to check dsp/fpu compatibility.  */
end_comment
...
name|_bfd_error_handler
argument_list|(
literal|"%B: uses instructions which are incompatible "
literal|"with instructions used in previous modules"
argument_list|,
name|ibfd
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Override the generic function because we need to store sh_elf_obj_tdata    as the specific tdata.  We set also the machine architecture from flags    here.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|sh_elf_object_p
parameter_list|(
name|bfd
modifier|*
name|abfd
...
name|loc
decl_stmt|;
comment|/* This symbol has a datalabel entry in the global offset table. 	   Set it up.  */
name|sgot
operator|=
name|htab
operator|->
...
begin_comment
comment|/* Support for Linux core dump NOTE sections.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|elf32_shlin_grok_prstatus
parameter_list|(
name|bfd
modifier|*
name|abfd
...
begin_comment
comment|/* not SH_TARGET_ALREADY_DEFINED */
end_comment
...
directive|define
name|elf_backend_grok_prstatus
value|elf32_shlin_grok_prstatus
end_define
...
begin_comment
comment|/* neither INCLUDE_SHMEDIA nor SH_TARGET_ALREADY_DEFINED */
end_comment
