...
begin_struct
struct|struct
name|elf_m68k_plt_info
block|{
comment|/* The size of each PLT entry.  */
name|bfd_vma
name|size
decl_stmt|;
comment|/* The template for the first PLT entry.  */
specifier|const
name|bfd_byte
modifier|*
name|plt0_entry
decl_stmt|;
comment|/* Offsets of fields in PLT0_ENTRY that require R_68K_PC32 relocations.      The comments by each member indicate the value that the relocation      is against.  */
struct|struct
block|{
name|unsigned
name|int
...
name|symbol_entry
decl_stmt|;
comment|/* Offsets of fields in SYMBOL_ENTRY that require R_68K_PC32 relocations.      The comments by each member indicate the value that the relocation      is against.  */
struct|struct
block|{
name|unsigned
name|int
name|got
decl_stmt|;
comment|/* the symbol's .got.plt entry */
name|unsigned
name|int
name|plt
decl_stmt|;
...
name|symbol_relocs
struct|;
comment|/* The offset of the resolver stub from the start of SYMBOL_ENTRY.      The stub starts with "move.l #relocoffset,%d0".  */
name|bfd_vma
name|symbol_resolve_entry
decl_stmt|;
block|}
struct|;
end_struct
...
literal|0
block|,
comment|/* replaced with (.got entry) - . */
literal|0x20
block|,
literal|0x7b
block|,
...
name|sym_sec
decl_stmt|;
comment|/* The PLT format used by this link, or NULL if the format has not      yet been chosen.  */
specifier|const
name|struct
name|elf_m68k_plt_info
modifier|*
...
begin_comment
comment|/* Return the type of PLT associated with OUTPUT_BFD.  */
end_comment
...
specifier|static
name|bfd_boolean
name|elf_m68k_always_size_sections
parameter_list|(
name|bfd
modifier|*
name|output_bfd
...
begin_comment
comment|/* Install an M_68K_PC32 relocation against VALUE at offset OFFSET    into section SEC.  */
end_comment

begin_function
specifier|static
name|void
name|elf_m68k_install_pc32
parameter_list|(
name|asection
modifier|*
name|sec
...
parameter_list|)
block|{
comment|/* Make VALUE PC-relative.  */
name|value
operator|-=
name|sec
operator|->
...
name|offset
expr_stmt|;
comment|/* Apply any in-place addend.  */
name|value
operator|+=
name|bfd_get_32
argument_list|(
...
define|\
value|elf_m68k_always_size_sections
end_define
