...
begin_endif
unit|static bfd_boolean m32r_elf_relax_delete_bytes   PARAMS ((bfd *, asection *, bfd_vma, int));
endif|#
directive|endif
end_endif
...
name|BFD_ASSERT
argument_list|(
argument|ELF32_R_TYPE(dst->r_info)<= (unsigned int) R_M32R_GNU_VTENTRY
argument_list|)
name|cache_ptr
operator|->
name|howto
...
name|dyn_relocs
decl_stmt|;
comment|//  bfd_signed_vma gotplt_refcount;
comment|/* Number of PC relative relocs copied for this symbol.  */
comment|/* struct elf_m32r_pcrel_relocs_copied *pcrel_relocs_copied;  FIXME */
block|}
struct|;
end_struct
...
name|NULL
expr_stmt|;
comment|//      eh->gotplt_refcount = 0;
comment|/* eh->pcrel_relocs_copied = NULL; FIXME */
block|}
return|return
operator|(
expr|struct
...
expr_stmt|;
block|}
comment|//  if (ind->root.type == bfd_link_hash_indirect
comment|//&& dir->got.refcount<= 0)
comment|//    {
comment|//      edir->tls_type = eind->tls_type;
comment|//      eind->tls_type = GOT_UNKNOWN;
comment|//    }
...
name|h
expr_stmt|;
comment|//  if ((h->got.refcount> 0
comment|//      || (h->elf_link_hash_flags& ELF_LINK_FORCED_LOCAL))
comment|//&& eh->gotplt_refcount> 0)
comment|//    {
comment|//      /* The symbol has been forced local, or we have some direct got refs,
comment|//         so treat all the gotplt refs as got refs. */
comment|//      h->got.refcount += eh->gotplt_refcount;
comment|//      if (h->plt.refcount>= eh->gotplt_refcount)
comment|//        h->plt.refcount -= eh->gotplt_refcount;
comment|//    }
if|if
condition|(
name|htab
...
condition|)
break|break;
comment|//if (ELF_ST_VISIBILITY (h->other) == STV_INTERNAL
comment|//    || ELF_ST_VISIBILITY (h->other) == STV_HIDDEN)
comment|//  break;
if|if
condition|(
name|h
...
name|_
argument_list|(
literal|"%s: The target (%s) of an %s relocation is in the wrong section (%s)"
argument_list|)
argument_list|,
name|bfd_archive_filename
argument_list|(
...
begin_comment
comment|/* relaxing not supported yet */
end_comment

begin_comment
comment|/* This function handles relaxing for the m32r.    Relaxing on the m32r is tricky because of instruction alignment    requirements (4 byte instructions must be aligned on 4 byte boundaries).     The following relaxing opportunities are handled:     seth/add3/jl -> bl24 or bl8    seth/add3 -> ld24     It would be nice to handle bl24 -> bl8 but given:     - 4 byte insns must be on 4 byte boundaries    - branch instructions only branch to insns on 4 byte boundaries     this isn't much of a win because the insn in the 2 "deleted" bytes    must become a nop.  With some complexity some real relaxation could be    done but the frequency just wouldn't make it worth it; it's better to    try to do all the code compaction one can elsewhere.    When the chip supports parallel 16 bit insns, things may change. */
end_comment

begin_comment
unit|static bfd_boolean m32r_elf_relax_section (abfd, sec, link_info, again)      bfd *abfd;      asection *sec;      struct bfd_link_info *link_info;      bfd_boolean *again; {   Elf_Internal_Shdr *symtab_hdr;
comment|/* The Rela structures are used here because that's what      _bfd_elf_link_read_relocs uses [for convenience - it sets the addend      field to 0].  */
end_comment

begin_comment
unit|Elf_Internal_Rela *internal_relocs = NULL;   Elf_Internal_Rela *irel, *irelend;   bfd_byte *contents = NULL;   Elf_Internal_Sym *isymbuf = NULL;
...
begin_comment
unit|if (link_info->relocatable       || (sec->flags& SEC_RELOC) == 0       || sec->reloc_count == 0       || (sec->flags& SEC_CODE) == 0       || 0
comment|/* FIXME: check SHF_M32R_CAN_RELAX */
end_comment
...
begin_comment
unit|if (sec->_cooked_size == 0)     sec->_cooked_size = sec->_raw_size;    symtab_hdr =&elf_tdata (abfd)->symtab_hdr;
comment|/* Get a copy of the native relocations.  */
end_comment

begin_comment
unit|internal_relocs = (_bfd_elf_link_read_relocs 		     (abfd, sec, (PTR) NULL, (Elf_Internal_Rela *) NULL, 		      link_info->keep_memory));   if (internal_relocs == NULL)     goto error_return;
comment|/* Walk through them looking for relaxing opportunities.  */
end_comment

begin_comment
unit|irelend = internal_relocs + sec->reloc_count;   for (irel = internal_relocs; irel< irelend; irel++)     {       bfd_vma symval;
comment|/* If this isn't something that can be relaxed, then ignore 	 this reloc.  */
end_comment

begin_comment
unit|if (ELF32_R_TYPE (irel->r_info) != (int) R_M32R_HI16_SLO) 	continue;
comment|/* Get the section contents if we haven't done so already.  */
end_comment

begin_comment
unit|if (contents == NULL) 	{
comment|/* Get cached copy if it exists.  */
end_comment

begin_comment
unit|if (elf_section_data (sec)->this_hdr.contents != NULL) 	    contents = elf_section_data (sec)->this_hdr.contents; 	  else 	    {
comment|/* Go get them off disk.  */
end_comment

begin_comment
unit|contents = (bfd_byte *) bfd_malloc (sec->_raw_size); 	      if (contents == NULL) 		goto error_return;  	      if (! bfd_get_section_contents (abfd, sec, contents, 					      (file_ptr) 0, sec->_raw_size)) 		goto error_return; 	    } 	}
comment|/* Read this BFD's local symbols if we haven't done so already.  */
end_comment

begin_comment
unit|if (isymbuf == NULL&& symtab_hdr->sh_info != 0) 	{ 	  isymbuf = (Elf_Internal_Sym *) symtab_hdr->contents; 	  if (isymbuf == NULL) 	    isymbuf = bfd_elf_get_elf_syms (abfd, symtab_hdr, 					    symtab_hdr->sh_info, 0, 					    NULL, NULL, NULL); 	  if (isymbuf == NULL) 	    goto error_return; 	}
comment|/* Get the value of the symbol referred to by the reloc.  */
end_comment

begin_comment
unit|if (ELF32_R_SYM (irel->r_info)< symtab_hdr->sh_info) 	{
comment|/* A local symbol.  */
end_comment

begin_comment
unit|Elf_Internal_Sym *isym; 	  asection *sym_sec;  	  isym = isymbuf + ELF32_R_SYM (irel->r_info), 	  sym_sec = bfd_section_from_elf_index (abfd, isym->st_shndx); 	  symval = (isym->st_value 		    + sym_sec->output_section->vma 		    + sym_sec->output_offset); 	}       else 	{ 	  unsigned long indx; 	  struct elf_link_hash_entry *h;
comment|/* An external symbol.  */
end_comment

begin_comment
unit|indx = ELF32_R_SYM (irel->r_info) - symtab_hdr->sh_info; 	  h = elf_sym_hashes (abfd)[indx]; 	  BFD_ASSERT (h != NULL); 	  if (h->root.type != bfd_link_hash_defined&& h->root.type != bfd_link_hash_defweak) 	    {
...
begin_comment
unit|continue; 	    }  	  symval = (h->root.u.def.value 		    + h->root.u.def.section->output_section->vma 		    + h->root.u.def.section->output_offset); 	}
...
begin_comment
comment|/* Try to change a seth/add3/jl subroutine call to bl24 or bl8. 	 This sequence is generated by the compiler when compiling in 	 32 bit mode.  Also look for seth/add3 -> ld24.  */
end_comment

begin_comment
unit|if (ELF32_R_TYPE (irel->r_info) == (int) R_M32R_HI16_SLO) 	{ 	  Elf_Internal_Rela *nrel; 	  bfd_vma pc = (sec->output_section->vma + sec->output_offset 			+ irel->r_offset); 	  bfd_signed_vma pcrel_value = symval - pc; 	  unsigned int code,reg; 	  int addend,nop_p,bl8_p,to_delete;
comment|/* The tests are ordered so that we get out as quickly as possible 	     if this isn't something we can relax, taking into account that 	     we are looking for two separate possibilities (jl/ld24).  */
end_comment

begin_comment
comment|/* Do nothing if no room in the section for this to be what we're 	     looking for.  */
end_comment

begin_comment
unit|if (irel->r_offset> sec->_cooked_size - 8) 	    continue;
comment|/* Make sure the next relocation applies to the next 	     instruction and that it's the add3's reloc.  */
end_comment

begin_comment
unit|nrel = irel + 1; 	  if (nrel == irelend 	      || irel->r_offset + 4 != nrel->r_offset 	      || ELF32_R_TYPE (nrel->r_info) != (int) R_M32R_LO16) 	    continue;
comment|/* See if the instructions are seth/add3.  */
end_comment

begin_comment
comment|/* FIXME: This is where macros from cgen can come in.  */
end_comment

begin_comment
unit|code = bfd_get_16 (abfd, contents + irel->r_offset + 0); 	  if ((code& 0xf0ff) != 0xd0c0) 	    continue;
comment|/* not seth rN,foo */
end_comment

begin_comment
unit|reg = (code& 0x0f00)>> 8; 	  code = bfd_get_16 (abfd, contents + irel->r_offset + 4); 	  if (code != (0x80a0 | reg | (reg<< 8))) 	    continue;
comment|/* not add3 rN,rN,foo */
end_comment

begin_comment
comment|/* At this point we've confirmed we have seth/add3.  Now check 	     whether the next insn is a jl, in which case try to change this 	     to bl24 or bl8.  */
end_comment

begin_comment
comment|/* Ensure the branch target is in range. 	     The bl24 instruction has a 24 bit operand which is the target 	     address right shifted by 2, giving a signed range of 26 bits. 	     Note that 4 bytes are added to the high value because the target 	     will be at least 4 bytes closer if we can relax.  It'll actually 	     be 4 or 8 bytes closer, but we don't know which just yet and 	     the difference isn't significant enough to worry about.  */
end_comment
...
begin_comment
comment|/* put in for learning purposes */
end_comment

begin_else
unit|pcrel_value += irel->r_addend;
else|#
directive|else
end_else

begin_endif
unit|addend = bfd_get_signed_16 (abfd, contents + irel->r_offset + 2); 	  pcrel_value += addend;
endif|#
directive|endif
end_endif

begin_comment
unit|if (pcrel_value>= -(1<< 25)&& pcrel_value< (1<< 25) + 4
comment|/* Do nothing if no room in the section for this to be what we're 		 looking for.  */
end_comment

begin_comment
unit|&& (irel->r_offset<= sec->_cooked_size - 12)
comment|/* Ensure the next insn is "jl rN".  */
end_comment

begin_comment
unit|&& ((code = bfd_get_16 (abfd, contents + irel->r_offset + 8)), 		  code != (0x1ec0 | reg))) 	    {
comment|/* We can relax to bl24/bl8.  */
end_comment

begin_comment
comment|/* See if there's a nop following the jl. 		 Also see if we can use a bl8 insn.  */
end_comment

begin_comment
unit|code = bfd_get_16 (abfd, contents + irel->r_offset + 10); 	      nop_p = (code& 0x7fff) == NOP_INSN; 	      bl8_p = pcrel_value>= -0x200&& pcrel_value< 0x200;  	      if (bl8_p) 		{
comment|/* Change "seth rN,foo" to "bl8 foo || nop". 		     We OR in CODE just in case it's not a nop (technically, 		     CODE currently must be a nop, but for cleanness we 		     allow it to be anything).  */
end_comment
...
begin_else
unit|code = 0x7e000000 | MAKE_PARALLEL (code);
else|#
directive|else
end_else

begin_endif
unit|code = (0x7e000000 + (((addend>> 2)& 0xff)<< 16)) | MAKE_PARALLEL (code);
endif|#
directive|endif
end_endif

begin_comment
unit|to_delete = 8; 		} 	      else 		{
comment|/* Change the seth rN,foo to a bl24 foo.  */
end_comment
...
begin_else
unit|code = 0xfe000000;
else|#
directive|else
end_else

begin_endif
unit|code = 0xfe000000 + ((addend>> 2)& 0xffffff);
endif|#
directive|endif
end_endif

begin_comment
unit|to_delete = nop_p ? 8 : 4; 		}  	      bfd_put_32 (abfd, code, contents + irel->r_offset);
comment|/* Set the new reloc type.  */
end_comment

begin_comment
unit|irel->r_info = ELF32_R_INFO (ELF32_R_SYM (nrel->r_info), 					   bl8_p ? R_M32R_10_PCREL : R_M32R_26_PCREL);
comment|/* Delete the add3 reloc by making it a null reloc.  */
end_comment

begin_comment
unit|nrel->r_info = ELF32_R_INFO (ELF32_R_SYM (nrel->r_info), 					   R_M32R_NONE); 	    } 	  else if (addend>= 0&& symval + addend<= 0xffffff) 	    {
comment|/* We can relax to ld24.  */
end_comment

begin_comment
unit|code = 0xe0000000 | (reg<< 24) | (addend& 0xffffff); 	      bfd_put_32 (abfd, code, contents + irel->r_offset); 	      to_delete = 4;
comment|/* Tell the following code a nop filler isn't needed.  */
end_comment

begin_comment
unit|nop_p = 1; 	    } 	  else 	    {
comment|/* Can't do anything here.  */
end_comment
...
name|_
argument_list|(
literal|"%s: Instruction set mismatch with previous modules"
argument_list|)
argument_list|,
name|bfd_archive_filename
argument_list|(
...
define|\
value|m32r_elf_get_relocated_section_contents
end_define
