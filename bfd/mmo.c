...
specifier|static
name|bfd_boolean
name|mmo_write_section_description
parameter_list|(
name|bfd
modifier|*
parameter_list|,
...
specifier|static
name|bfd_boolean
name|mmo_has_leading_or_trailing_zero_tetra_p
parameter_list|(
name|bfd
modifier|*
parameter_list|,
...
name|_
argument_list|(
literal|"%s: unsupported wide character sequence"
literal|" 0x%02X 0x%02X after symbol name starting with `%s'\n"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
name|_
argument_list|(
literal|"%s: invalid mmo file: file number %d `%s',"
literal|" was already entered as `%s'\n"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
name|_
argument_list|(
literal|"%s: invalid mmo file: file name for number %d"
literal|" was not specified before use\n"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
name|_
argument_list|(
literal|"%s: invalid mmo file: fields y and z of lop_stab"
literal|" non-zero, y: %d, z: %d\n"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
name|_
argument_list|(
literal|"%s: invalid mmo file: lop_end not last item in"
literal|" file\n"
argument_list|)
argument_list|,
name|bfd_get_filename
...
name|_
argument_list|(
literal|"%s: invalid mmo file: YZ of lop_end (%ld)"
literal|" not equal to the number of tetras to the preceding"
literal|" lop_stab (%ld)\n"
argument_list|)
argument_list|,
name|bfd_get_filename
...
begin_comment
comment|/* Return TRUE iff the leading or trailing tetrabyte in SEC is defined and    is 0.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* Because leading and trailing zeros are omitted in output, we need to 	 specify the section boundaries so they're correct when the file 	 is read in again.  That's also the case if this section is 	 specified as not within its usual boundaries or alignments.  */
if|if
condition|(
name|sec
operator|->
...
argument_list|)
decl_stmt|;
comment|/* Same goes as for MMO_TEXT_SECTION_NAME above.  */
if|if
condition|(
name|sec
operator|->
...
return|;
block|}
comment|/* Some section without contents.  */
return|return
name|TRUE
return|;
block|}
...
begin_comment
comment|/* Write the description of a section, extended-mmo-style.  */
end_comment
...
parameter_list|)
block|{
comment|/* Keep the following document-comment formatted the way it is.  */
...
name|_
argument_list|(
literal|"%s: Bad symbol definition: `Main' set to %s rather"
literal|" than the start address %s\n"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Need to strip const here; strdup:ing would leak and the 	     existing string must be safe to reuse.  */
name|sym
operator|.
name|name
operator|=
...
name|_
argument_list|(
literal|"%s: warning: symbol table too large for mmo, larger than 65535"
literal|" 32-bit words: %d.  Only `Main' will be emitted.\n"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
name|_
argument_list|(
literal|"%s: internal error, symbol table changed size from %d to %d"
literal|" words\n"
argument_list|)
argument_list|,
name|bfd_get_filename
...
name|_
argument_list|(
literal|"%s: internal error, internal register section %s had"
literal|" contents\n"
argument_list|)
argument_list|,
name|bfd_get_filename
...
name|_
argument_list|(
literal|"%s: invalid start address for initialized registers of"
literal|" length %ld: 0x%lx%08lx\n"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
define|#
directive|define
name|mmo_bfd_is_target_special_symbol
define|\
...
define|#
directive|define
name|mmo_bfd_copy_private_header_data
