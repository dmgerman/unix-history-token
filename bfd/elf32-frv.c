...
begin_comment
comment|/* Map BFD reloc types to FRV ELF reloc types.  */
end_comment
...
comment|/* The condition below is an ugly hack to get .scommon data to 	  be regarded as local.  For some reason the 	  ELF_LINK_HASH_DEF_REGULAR bit is not set on such common 	  symbols, and the SEC_IS_COMMON bit is not set any longer 	  when we need to perform this test.  Hopefully this 	  approximation is good enough.  */
value|\        ((H)->root.type == bfd_link_hash_defined \ 	|| (H)->root.type == bfd_link_hash_defweak) \&& (H)->root.u.def.section->output_section \&& ((H)->root.u.def.section->flags& SEC_LINKER_CREATED)))
end_define
...
literal|1
decl_stmt|;
comment|/* The number of R_FRV_32, R_FRV_FUNCDESC and R_FRV_FUNCDESC_VALUE      relocations referencing the symbol.  */
name|unsigned
name|relocs32
decl_stmt|,
name|relocsfd
...
directive|if
literal|0
comment|/* These are set in _frvfdpic_count_got_plt_entries() or later, and this      function is only called in _frvfdpic_resolve_final_relocs_info(), that      runs just before it, so we don't have to worry about the fields      below.  */
block|e2->plt |= e1->plt;   e2->privfd |= e1->privfd;   e2->lazyplt |= e1->lazyplt;   e2->done |= e1->done;    e2->relocs32 += e1->relocs32;   e2->relocsfd += e1->relocsfd;   e2->relocsfdv += e1->relocsfdv;   e2->fixups += e1->fixups;   e2->dynrelocs += e1->dynrelocs;    if (abs (e1->got_entry)< abs (e2->got_entry))     e2->got_entry = e1->got_entry;   if (abs (e1->fdgot_entry)< abs (e2->fdgot_entry))     e2->fdgot_entry = e1->fdgot_entry;   if (abs (e1->fd_entry)< abs (e2->fd_entry))     e2->fd_entry = e1->fd_entry;    if (e1->plt_entry< e2->plt_entry)     e2->plt_entry = e1->plt_entry;   if (e1->lzplt_entry< e2->lzplt_entry)     e2->lzplt_entry = e1->lzplt_entry;
endif|#
directive|endif
block|}
end_function
...
name|_
argument_list|(
literal|"%s: relocation at `%s+0x%x' references symbol `%s' with nonzero addend"
argument_list|)
argument_list|,
...
name|s
expr_stmt|;
comment|/* FRV-specific: we want to create the GOT in the FRV way.  */
if|if
condition|(
operator|!
name|_frv_create_got_section
...
name|info
decl_stmt|;
comment|/* Total size needed for GOT entries within the 12-, 16- or 32-bit      ranges.  */
name|bfd_vma
name|got12
decl_stmt|,
name|gotlos
...
name|gothilo
decl_stmt|;
comment|/* Total size needed for function descriptor entries within the 12-,      16- or 32-bit ranges.  */
name|bfd_vma
name|fd12
decl_stmt|,
name|fdlos
...
name|_
argument_list|(
literal|"%s: unsupported relocation type %i"
argument_list|)
argument_list|,
name|bfd_archive_filename
argument_list|(
