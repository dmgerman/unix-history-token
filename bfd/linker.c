...
begin_comment
comment|/* linker.c -- BFD linker routines    Copyright 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002,    2003, 2004, 2005, 2006 Free Software Foundation, Inc.    Written by Steve Chamberlain and Ian Lance Taylor, Cygnus Support     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
comment|/* pun points at an u.undef.next field.  Go back to 		   the start of the link_hash_entry.  */
name|table
operator|->
name|undefs_tail
operator|=
...
begin_comment
comment|/* Routine to create an entry in a generic link hash table.  */
end_comment
...
begin_comment
comment|/* Create a generic link hash table.  */
end_comment
...
begin_comment
comment|/* Indicate that we are only retrieving symbol values from this    section.  We want the symbols to act as though the values in the    file are absolute.  */
end_comment
...
condition|)
block|{
comment|/* If we haven't found the exact symbol we're looking for, 	     let's look for its import thunk */
if|if
condition|(
name|info
operator|->
...
name|BIG
case|:
comment|/* We have found a common definition for a symbol which 	     already had a common definition.  Use the maximum of the 	     two sizes, and use the section required by the larger symbol.  */
name|BFD_ASSERT
argument_list|(
name|h
operator|->
...
name|power
expr_stmt|;
comment|/* Some systems have special treatment for small commons, 		 hence we want to select the section used by the larger 		 symbol.  This makes sure the symbol does not go in a 		 small common section if it is now too large.  */
if|if
condition|(
name|section
operator|==
...
name|_
argument_list|(
literal|"%B: indirect symbol `%s' to `%s' is a loop"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|CWARN
case|:
comment|/* Warn if this symbol has been referenced already, 	     otherwise add a warning.  A symbol has been referenced if 	     the u.undef.next field is not NULL, or it is the tail of the 	     undefined symbol list.  The REF case above helps to 	     ensure this.  */
if|if
condition|(
name|h
operator|->
...
argument_list|()
expr_stmt|;
comment|/* If this symbol is in a section which is not being included 	 in the output file, then we don't want to output the 	 symbol.  */
if|if
condition|(
operator|!
name|bfd_is_abs_section
...
begin_comment
comment|/* Default routine to handle a bfd_data_link_order.  */
end_comment
...
name|_
argument_list|(
literal|"Attempt to do relocatable link with %s input and %s output"
argument_list|)
argument_list|,
name|bfd_get_target
argument_list|(
...
begin_comment
comment|/* FUNCTION 	bfd_link_split_section  SYNOPSIS         bfd_boolean bfd_link_split_section (bfd *abfd, asection *sec);  DESCRIPTION 	Return nonzero if @var{sec} should be split during a 	reloceatable or final link.  .#define bfd_link_split_section(abfd, sec) \ .       BFD_SEND (abfd, _bfd_link_split_section, (abfd, sec)) .  */
end_comment
...
begin_comment
comment|/* FUNCTION 	bfd_section_already_linked  SYNOPSIS         void bfd_section_already_linked (bfd *abfd, asection *sec);  DESCRIPTION 	Check if @var{sec} has been already linked during a reloceatable 	or final link.  .#define bfd_section_already_linked(abfd, sec) \ .       BFD_SEND (abfd, _section_already_linked, (abfd, sec)) .  */
end_comment
...
name|struct
name|bfd_hash_table
name|_bfd_section_already_linked_table
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Support routines for the hash table used by section_already_linked,    initialize the table, traverse, lookup, fill in an entry and remove    the table.  */
end_comment
...
name|l
decl_stmt|;
comment|/* Allocate the memory from the same obstack as the hash table is      kept in.  */
name|l
operator|=
name|bfd_hash_allocate
argument_list|(
...
begin_comment
comment|/* This is used on non-ELF inputs.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* We may have 3 different sections on the list: group section, 	 comdat section and linkonce section. SEC may be a linkonce or 	 comdat section. We always ignore group section. For non-COFF 	 inputs, we also ignore comdat section.  	 FIXME: Is that safe to match a linkonce section with a comdat 	 section for COFF inputs?  */
if|if
condition|(
operator|(
name|l
...
name|_
argument_list|(
literal|"%B: warning: ignoring duplicate section `%A'\n"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: warning: duplicate section `%A' has different size\n"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
begin_comment
comment|/* Convert symbols in excluded output sections to absolute.  */
end_comment
