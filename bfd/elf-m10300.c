...
begin_struct
struct|struct
name|elf_mn10300_pcrel_relocs_copied
block|{
...
specifier|static
name|bfd_boolean
name|_bfd_mn10300_elf_discard_copies
name|PARAMS
argument_list|(
operator|(
expr|struct
...
argument_list|)
expr_stmt|;
comment|/* If we are linking with -Bsymbolic, and this is a 		 global symbol, we count the number of PC relative 		 relocations we have entered for this symbol, so that 		 we can discard them again if the symbol is later 		 defined by a regular object.  Note that this function 		 is only called if we are using an elf_sh linker 		 hash table, which means that h is really a pointer to 		 an elf32_mn10300_link_hash_entry.  */
if|if
condition|(
name|h
operator|!=
