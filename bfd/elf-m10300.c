...
name|_
argument_list|(
literal|"error: inappropriate relocation type for shared"
literal|" library (did you forget -fpic?)"
argument_list|)
expr_stmt|;
else|else
name|msg
...
name|_
argument_list|(
literal|"internal error: suspicious relocation type used"
...
begin_comment
comment|/* Used to count hash table entries.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|elf32_mn10300_count_hash_table_entries
parameter_list|(
name|struct
name|bfd_hash_entry
modifier|*
...
begin_comment
comment|/* Used to enumerate hash table entries into a linear array.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|elf32_mn10300_list_hash_table_entries
parameter_list|(
name|struct
name|bfd_hash_entry
modifier|*
...
begin_comment
comment|/* Used to sort the array created by the above.  */
end_comment
...
expr_stmt|;
block|{
comment|/* This section of code collects all our local symbols, sorts 	   them by value, and looks for multiple symbols referring to 	   the same address.  For those symbols, the flags are merged. 	   At this point, the only flag that can be set is 	   MN10300_CONVERT_CALL_TO_CALLS, so we simply OR the flags 	   together.  */
name|int
name|static_count
init|=
literal|0
