...
directive|if
literal|0
block|for (i = 0; i< mdata->header.ncmds; i++)     {       bfd_mach_o_load_command *cur =&mdata->commands[i];       if (cur->type != BFD_MACH_O_LC_SEGMENT) 	break;        { 	bfd_mach_o_segment_command *seg =&cur->command.segment; 	char buf[1024]; 	bfd_vma nbytes = seg->filesize; 	bfd_vma curoff = seg->fileoff;  	while (nbytes> 0) 	  { 	    bfd_vma thisread = nbytes;  	    if (thisread> 1024) 	      thisread = 1024;  	    bfd_seek (abfd, curoff, SEEK_SET); 	    if (bfd_bread ((PTR) buf, thisread, abfd) != thisread) 	      return FALSE;  	    bfd_seek (abfd, curoff, SEEK_SET); 	    if (bfd_bwrite ((PTR) buf, thisread, abfd) != thisread) 	      return FALSE;  	    nbytes -= thisread; 	    curoff += thisread; 	  }       }   }
endif|#
directive|endif
...
index|]
expr_stmt|;
comment|/* Don't set this from the symbol information; use stored values.  */
if|#
directive|if
literal|0
block|if (s->flags& BSF_GLOBAL) 	ntype |= N_EXT;       if (s->flags& BSF_DEBUGGING) 	ntype |= N_STAB;        if (s->section == bfd_und_section_ptr) 	ntype |= N_UNDF;       else if (s->section == bfd_abs_section_ptr) 	ntype |= N_ABS;       else 	ntype |= N_SECT;
endif|#
directive|endif
...
directive|if
literal|0
block|fprintf (stderr, "bfd_mach_o_scan: unable to scan start address: %s\n", 	       bfd_errmsg (bfd_get_error ()));       abfd->tdata.mach_o_data = NULL;       return -1;
endif|#
directive|endif
block|}
name|bfd_mach_o_flatten_sections
