...
begin_comment
comment|/* MMIX-specific support for 64-bit ELF.    Copyright 2001, 2002, 2003, 2004 Free Software Foundation, Inc.    Contributed by Hans-Peter Nilsson<hp@bitrange.com>  This file is part of BFD, the Binary File Descriptor library.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
specifier|static
name|bfd_boolean
name|mmix_elf_get_section_contents
name|PARAMS
argument_list|(
operator|(
name|bfd
...
begin_comment
comment|/* We need to include the maximum size of PUSHJ-stubs in the initial    section size.  This is expected to shrink during linker relaxation.     You might think that we should set *only* _cooked_size, but that won't    work: section contents allocation will be using _raw_size in mixed    format linking and not enough storage will be allocated.  FIXME: That's    a major bug, including the name bfd_get_section_size_before_reloc; it    should be bfd_get_section_size_before_relax.  The relaxation functions    set _cooked size.  Relaxation happens before relocation.  All functions    *after relaxation* should be using _cooked size.  */
end_comment
...
name|n_allocated_bpo_gregs
expr_stmt|;
comment|/* We need to have a _raw_size contents even though there's only      _cooked_size worth of data, since the generic relocation machinery      will allocate and copy that much temporarily.  */
name|bpo_gregs_section
operator|->
name|contents
operator|=
...
begin_comment
comment|/* Because we set _raw_size to include the max size of pushj stubs,    i.e. larger than the actual section input size (see    mmix_set_relaxable_raw_size), we have to take care of that when reading    the section.  */
end_comment
...
return|;
block|}
comment|/* Check bounds against the faked raw_size.  */
if|if
condition|(
name|offset
operator|+
...
condition|)
block|{
comment|/* Clear the part in the faked area.  */
name|memset
argument_list|(
name|location
operator|+
...
argument_list|)
expr_stmt|;
comment|/* If there's no initial part within the "real" contents, we're          done.  */
if|if
condition|(
operator|(
name|bfd_size_type
...
name|TRUE
return|;
comment|/* Else adjust the count and fall through to call the generic          function.  */
name|count
operator|=
name|raw_size
operator|-
...
define|#
directive|define
name|bfd_elf64_get_section_contents
value|mmix_elf_get_section_contents
end_define
