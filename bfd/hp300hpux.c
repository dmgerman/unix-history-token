...
begin_comment
comment|/* @@ Also, note that there wind up being two versions of some routines, with    different names, only one of which actually gets used.  For example: 	slurp_symbol_table 	swap_std_reloc_in 	slurp_reloc_table 	canonicalize_symtab 	get_symtab_upper_bound 	canonicalize_reloc 	mkobject    This should also be fixed.  */
end_comment
...
begin_comment
comment|/* these don't use MY because that causes problems within JUMP_TABLE    (CONCAT2 winds up being expanded recursively, which ANSI C compilers    will not do).  */
end_comment
...
value|hp300hpux_canonicalize_symtab
end_define
...
name|y
parameter_list|)
value|CONCAT3 (hp300hpux,_32_,y)
end_define
...
function_decl|(bfd *
operator|,
function_decl|struct hp300hpux_reloc *
operator|,
...
name|N_EXT
case|:
comment|/* If the value is nonzero, then just treat this as a                  common symbol.  I don't know if this is correct in                  all cases, but it is more correct than treating it as                  a weak undefined symbol.  */
if|if
condition|(
name|cache_ptr
operator|->
...
comment|/* this is detected rather bizarrely by requiring a bunch of   */
...
comment|/* OK, now walk the new symtable, caching symbol properties */
block|{
name|aout_symbol_type
modifier|*
name|cache_ptr
...
return|;
comment|/********************************************************/
comment|/* for hpux, the 'length' value indicates the length of */
...
begin_decl_stmt
name|long
name|aout_32_canonicalize_symtab
name|PARAMS
argument_list|(
operator|(
name|bfd
