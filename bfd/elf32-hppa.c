...
name|sym_sec
decl_stmt|;
comment|/* Data for LDM relocations.  */
union|union
block|{
name|bfd_signed_vma
name|refcount
...
define|#
directive|define
name|hppa_elf_hash_entry
parameter_list|(
name|ent
parameter_list|)
define|\
value|((struct elf32_hppa_link_hash_entry *)(ent))
end_define

begin_define
define|#
directive|define
name|hppa_stub_hash_entry
parameter_list|(
name|ent
parameter_list|)
define|\
value|((struct elf32_hppa_stub_hash_entry *)(ent))
end_define
...
define|#
directive|define
name|hppa_elf_local_got_tls_type
parameter_list|(
name|abfd
parameter_list|)
define|\
value|((char *)(elf_local_got_offsets (abfd) + (elf_tdata (abfd)->symtab_hdr.sh_info * 2)))
end_define
...
parameter_list|)
define|\
value|(hh ? hh->eh.root.root.string : "<undef>")
end_define
...
parameter_list|)
define|\
value|(eh ? eh->root.root.string : "<undef>")
end_define
...
name|_
argument_list|(
literal|"%B(%A+0x%lx): cannot reach %s, recompile with -ffunction-sections"
argument_list|)
argument_list|,
name|hsh
operator|->
...
argument_list|)
expr_stmt|;
comment|/* hppa-linux needs _GLOBAL_OFFSET_TABLE_ to be visible from the main      application, because __canonicalize_funcptr_for_compare needs it.  */
name|eh
operator|=
name|elf_hash_table
argument_list|(
...
specifier|static
name|int
name|elf32_hppa_optimized_tls_reloc
parameter_list|(
name|struct
name|bfd_link_info
modifier|*
...
parameter_list|)
block|{
comment|/* For now we don't support linker optimizations.  */
return|return
name|r_type
return|;
block|}
...
name|_
argument_list|(
literal|"%B: relocation %s can not be used when making a shared object; recompile with -fPIC"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Add in space to store the local GOT TLS types.  */
name|size
operator|+=
name|symtab_hdr
operator|->
...
name|GOT_ENTRY_SIZE
expr_stmt|;
comment|/* R_PARISC_TLS_GD* needs two GOT entries */
if|if
condition|(
operator|(
name|hh
...
condition|)
block|{
comment|/* Allocate 2 got entries and 1 dynamic reloc for           R_PARISC_TLS_DTPMOD32 relocs.  */
name|htab
operator|->
name|tls_ldm_got
operator|.
...
name|_
argument_list|(
literal|"%B: duplicate export stub %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
begin_comment
comment|/* Return the base vma address which should be subtracted from the real    address when resolving a dtpoff relocation.      This is PT_TLS segment p_vaddr.  */
end_comment
...
begin_comment
comment|/* Return the relocation value for R_PARISC_TLS_TPOFF*..  */
end_comment
...
literal|0
return|;
comment|/* hppa TLS ABI is variant I and static TLS block start just after       tcbhead structure which has 2 pointer fields.  */
return|return
operator|(
name|address
operator|-
...
name|_
argument_list|(
literal|"%B(%A+0x%lx): %s fixup for insn 0x%x is not supported in a non-shared link"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
case|:
case|case
name|R_PARISC_TLS_DTPMOD32
case|:
case|case
name|R_PARISC_TLS_DTPOFF32
case|:
case|case
...
name|contents
expr_stmt|;
comment|/* FIXME (CAO): Should this be reloc_count++ ? */
name|loc
operator|+=
name|htab
operator|->
...
name|_
argument_list|(
literal|"%B(%A+0x%lx): cannot handle %s for %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
parameter_list|)
block|{
comment|/* Handle TLS relocs first; we don't want them to be marked      relative by the "if (ELF32_R_SYM (rela->r_info) == 0)"      check below.  */
switch|switch
condition|(
operator|(
name|int
...
value|elf_hppa_reloc_name_lookup
end_define
...
value|elf32_hppa_grok_prstatus
end_define
