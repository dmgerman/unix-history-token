...
name|R_386_TLS_GD
condition|)
comment|/* If this is a GD->IE transition, we may use either 		     of R_386_TLS_TPOFF and R_386_TLS_TPOFF32.  But if 		     we may have both R_386_TLS_IE and R_386_TLS_GD, 		     we can't share the same R_386_TLS_TPOFF since 		     they require different offsets. So we remember 		     it comes from R_386_TLS_GD.  */
name|tls_type
operator|=
name|GOT_TLS_IE
operator||
...
condition|)
block|{
comment|/* We remember it comes from R_386_TLS_IE.  */
name|tls_type
operator|=
name|GOT_TLS_IE_POS
operator||
...
name|tls_type
decl_stmt|;
comment|/* If we have both R_386_TLS_IE and R_386_TLS_GD, GOT_TLS_IE_BOTH 	 should be used.  */
if|if
condition|(
operator|(
name|tls_type
...
condition|)
block|{
comment|/* r_symndx will be zero only for relocs against symbols from 	   removed linkonce sections, or sections discarded by a linker 	   script.  For these relocs, we just want the section contents 	   zeroed.  Avoid any special processing in the switch below.  */
name|r_type
operator|=
name|R_386_NONE
expr_stmt|;
