...
begin_comment
comment|/* Return the GOT offset for address VALUE, which was derived from    a symbol belonging to INPUT_SECTION.   If there is not yet a GOT    entry for this value, create one.  If R_SYMNDX refers to a TLS symbol,    create a TLS GOT entry instead.  Return -1 if no satisfactory GOT    offset can be found.  */
end_comment
...
begin_comment
comment|/* Find a local GOT entry for an R_MIPS_GOT16 relocation against VALUE,    which was calculated from a symbol belonging to INPUT_SECTION.    EXTERNAL is true if the relocation was against a global symbol    that has been forced local.  */
end_comment
...
expr_stmt|;
block|}
comment|/* In the unlikely event a special section is empty it has to lose its      special meaning.  This may happen e.g. when using `strip' with the      "--only-keep-debug" option.  */
if|if
condition|(
name|sec
operator|->
...
expr_stmt|;
block|}
comment|/* We need to figure out the alignment required for this symbol.  */
name|power_of_two
operator|=
name|bfd_log2
argument_list|(
...
begin_comment
comment|/* Ensure that the STO_OPTIONAL flag is copied into h->other,    even if this is not a defintion of the symbol.  */
end_comment
