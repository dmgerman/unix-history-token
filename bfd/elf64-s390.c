...
begin_comment
comment|/* IBM S/390-specific support for 64-bit ELF    Copyright 2000, 2001, 2002, 2003, 2004, 2005, 2006    Free Software Foundation, Inc.    Contributed Martin Schwidefsky (schwidefsky@de.ibm.com).     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA    02110-1301, USA.  */
end_comment
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_GD32.  */
name|HOWTO
argument_list|(
name|R_390_TLS_GD64
argument_list|,
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_GOTIE32.  */
name|HOWTO
argument_list|(
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_LDM32.  */
name|HOWTO
argument_list|(
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_IE32.  */
name|HOWTO
argument_list|(
name|R_390_TLS_IE64
argument_list|,
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_LE32.  */
name|HOWTO
argument_list|(
name|R_390_TLS_LE64
argument_list|,
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_LDO32.  */
name|HOWTO
argument_list|(
...
specifier|static
name|reloc_howto_type
name|elf64_s390_vtinherit_howto
init|=
name|HOWTO
argument_list|(
...
begin_comment
comment|/* We need to use ELF64_R_TYPE so we have our own copy of this function,    and elf64-s390.c has its own copy.  */
end_comment
...
begin_comment
comment|/* For the s390, simple addr offset can only be 0 - 4096.    To use the full 16777216 TB address space, several instructions    are needed to load an address in a register and execute    a branch( or just saving the address)     Furthermore, only r 0 and 1 are free to use!!!  */
end_comment

begin_comment
comment|/* The first 3 words in the GOT are then reserved.    Word 0 is the address of the dynamic table.    Word 1 is a pointer to a structure describing the object    Word 2 is used to point to the loader entry address.     The code for PLT entries looks like this:     The GOT holds the address in the PLT to be executed.    The loader then gets:    24(15) =  Pointer to the structure describing the object.    28(15) =  Offset in symbol table    The loader  must  then find the module where the function is    and insert the address in the GOT.     PLT1: LARL 1,<fn>@GOTENT # 6 bytes  Load address of GOT entry in r1          LG   1,0(1)      # 6 bytes  Load address from GOT in r1          BCR  15,1        # 2 bytes  Jump to address    RET1: BASR 1,0         # 2 bytes  Return from GOT 1st time          LGF  1,12(1)     # 6 bytes  Load offset in symbl table in r1          BRCL 15,-x       # 6 bytes  Jump to start of PLT          .long ?          # 4 bytes  offset into symbol table     Total = 32 bytes per PLT entry    Fixup at offset 2: relative address to GOT entry    Fixup at offset 22: relative branch to PLT0    Fixup at offset 28: 32 bit offset into symbol table     A 32 bit offset into the symbol table is enough. It allows for symbol    tables up to a size of 2 gigabyte. A single dynamic object (the main    program, any shared library) is limited to 4GB in size and I want to see    the program that manages to have a symbol table of more than 2 GB with a    total size of at max 4 GB.  */
end_comment
...
value|(bfd_vma) 0xe310100c
end_define

begin_define
define|#
directive|define
name|PLT_ENTRY_WORD5
value|(bfd_vma) 0x0014c0f4
end_define

begin_define
define|#
directive|define
name|PLT_ENTRY_WORD6
value|(bfd_vma) 0x00000000
end_define

begin_define
define|#
directive|define
name|PLT_ENTRY_WORD7
value|(bfd_vma) 0x00000000
end_define
...
begin_comment
comment|/* The first entry in the PLT:    PLT0:      STG  1,56(15)  # r1 contains the offset into the symbol table      LARL 1,_GLOBAL_OFFSET_TABLE # load address of global offset table      MVC  48(8,15),8(1) # move loader ino (object struct address) to stack      LG   1,16(1)   # get entry address of loader      BCR  15,1      # jump to loader       Fixup at offset 8: relative address to start of GOT.  */
end_comment
...
value|(bfd_vma) 0x10100004
end_define

begin_define
define|#
directive|define
name|PLT_FIRST_ENTRY_WORD6
value|(bfd_vma) 0x07f10700
end_define

begin_define
define|#
directive|define
name|PLT_FIRST_ENTRY_WORD7
value|(bfd_vma) 0x07000700
end_define
...
expr_stmt|;
block|}
comment|/* If R_390_TLS_{IE64,GOTIE64,GOTIE12,IEENT} symbol is now local to      the binary, we can optimize a bit. IE64 and GOTIE64 get converted      to R_390_TLS_LE64 requiring no TLS entry. For GOTIE12 and IEENT      we can save the dynamic TLS relocation.  */
if|if
condition|(
name|h
operator|->
...
name|GOT_ENTRY_SIZE
expr_stmt|;
comment|/* R_390_TLS_GD64 needs 2 consecutive GOT slots.  */
if|if
condition|(
name|tls_type
operator|==
...
name|dynamic_sections_created
expr_stmt|;
comment|/* R_390_TLS_IE64 needs one dynamic relocation, 	 R_390_TLS_GD64 needs one if local symbol and two if global.  */
if|if
condition|(
operator|(
name|tls_type
...
condition|)
block|{
comment|/* Allocate 2 got entries and 1 dynamic reloc for R_390_TLS_LDM64 	 relocs.  */
name|htab
operator|->
name|tls_ldm_got
operator|.
...
condition|)
block|{
comment|/* IE->LE transition. Four valid cases: 		     lg %rx,(0,%ry)    -> sllg %rx,%ry,0 		     lg %rx,(%ry,0)    -> sllg %rx,%ry,0 		     lg %rx,(%ry,%r12) -> sllg %rx,%ry,0 		     lg %rx,(%r12,%ry) -> sllg %rx,%ry,0  */
name|unsigned
name|int
name|insn0
decl_stmt|,
...
literal|0xe3000000
condition|)
comment|/* lg %rx,0(%ry,0) -> sllg %rx,%ry,0  */
name|ry
operator|=
operator|(
name|insn0
...
literal|0xe3000000
condition|)
comment|/* lg %rx,0(0,%ry) -> sllg %rx,%ry,0  */
name|ry
operator|=
operator|(
name|insn0
...
literal|0xe300c000
condition|)
comment|/* lg %rx,0(%ry,%r12) -> sllg %rx,%ry,0  */
name|ry
operator|=
operator|(
name|insn0
...
literal|0xe30c0000
condition|)
comment|/* lg %rx,0(%r12,%ry) -> sllg %rx,%ry,0  */
name|ry
operator|=
operator|(
name|insn0
...
condition|)
block|{
comment|/* GD->LE transition. 		     brasl %r14,__tls_get_addr@plt -> brcl 0,. */
name|insn0
operator|=
literal|0xc0040000
expr_stmt|;
...
block|{
comment|/* GD->IE transition. 		     brasl %r14,__tls_get_addr@plt -> lg %r2,0(%r2,%r12)  */
name|insn0
operator|=
literal|0xe322c000
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* LD->LE transition. 		     brasl %r14,__tls_get_addr@plt -> brcl 0,. */
name|insn0
operator|=
literal|0xc0040000
expr_stmt|;
...
name|PLT_ENTRY_SIZE
expr_stmt|;
comment|/* Offset in GOT is PLT index plus GOT headers(3) times 8, 	 addr& GOT addr.  */
name|got_offset
operator|=
operator|(
name|plt_index
...
name|GOT_ENTRY_SIZE
expr_stmt|;
comment|/* Fill in the blueprint of a PLT.  */
name|bfd_put_32
argument_list|(
name|output_bfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Fixup the relative address to the GOT entry */
name|bfd_put_32
argument_list|(
name|output_bfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Fixup the relative branch to PLT 0 */
name|bfd_put_32
argument_list|(
name|output_bfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Fixup offset into symbol table */
name|bfd_put_32
argument_list|(
name|output_bfd
argument_list|,
...
condition|)
block|{
comment|/* fill in blueprint for plt 0 entry */
name|bfd_put_32
argument_list|(
name|output_bfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Fixup relative address to start of GOT */
name|bfd_put_32
argument_list|(
name|output_bfd
argument_list|,
...
begin_comment
comment|/* Why was the hash table entry size definition changed from    ARCH_SIZE/8 to 4? This breaks the 64 bit dynamic linker and    this is the only reason for the s390_elf64_size_info structure.  */
end_comment
