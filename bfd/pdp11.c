...
begin_comment
comment|/* The header is not included in the text segment.  */
end_comment
...
begin_comment
comment|/* There are no shared libraries.  */
end_comment
...
begin_comment
comment|/* There is no flags field.  */
end_comment
...
name|x
parameter_list|)
value|(((x).a_info != OMAGIC)   \&& ((x).a_info != NMAGIC)   \&& ((x).a_info != A_MAGIC3) \&& ((x).a_info != A_MAGIC4) \&& ((x).a_info != A_MAGIC5) \&& ((x).a_info != A_MAGIC6))
end_define
...
directive|define
name|external_exec
value|pdp11_external_exec
end_define

begin_struct
struct|struct
name|pdp11_external_exec
block|{
name|bfd_byte
name|e_info
index|[
...
begin_comment
comment|/* Auto-overlay (nonseparate).  */
end_comment
...
directive|define
name|external_nlist
value|pdp11_external_nlist
end_define

begin_struct
struct|struct
name|pdp11_external_nlist
block|{
name|bfd_byte
name|e_unused
index|[
...
name|execp
parameter_list|)
value|pdp11_aout_write_headers (abfd, execp)
end_define
...
begin_comment
comment|/* Start of modified aoutx.h.  */
end_comment
...
begin_comment
comment|/* Swap the information in an executable header @var{raw_bytes} taken    from a raw byte stream memory image into the internal exec header    structure "execp".  */
end_comment
...
begin_comment
comment|/*  Swap the information in an internal exec header structure     "execp" into the buffer "bytes" ready for writing to disk.  */
end_comment
...
block|{
comment|/* TODO: print a proper warning message.  */
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"BFD:%s:%d: internal error\n"
argument_list|,
name|__FILE__
argument_list|,
name|__LINE__
...
begin_comment
comment|/* Some a.out variant thinks that the file open in ABFD    checking is an a.out file.  Do some more checking, and set up    for access if it really is.  Call back to the calling    environment's "finish up" function just before returning, to    handle any last-minute setup.  */
end_comment
...
begin_comment
comment|/* Initialize ABFD for use with a.out files.  */
end_comment
...
name|bfd_arch_pdp11
case|:
comment|/* TODO: arch_flags = M_PDP11; */
operator|*
name|unknown
operator|=
name|FALSE
...
begin_comment
comment|/* Set the architecture and the machine of the ABFD to the    values ARCH and MACHINE.  Verify that @ABFD's format    can support the architecture required.  */
end_comment
...
begin_comment
comment|/* Called by the BFD in response to a bfd_make_section request.  */
end_comment
...
call|)
argument_list|(
literal|"%B: can not represent section for symbol `%s' in a.out object file format"
argument_list|,
name|abfd
argument_list|,
name|cache_ptr
...
call|)
argument_list|(
literal|"%B: can not represent section `%A' in a.out object file format"
argument_list|,
name|abfd
argument_list|,
name|sec
...
argument_list|)
expr_stmt|;
comment|/* TODO: is 0 a safe value here?  */
name|in
operator|->
name|desc
operator|=
...
literal|1
condition|)
comment|/* Add BYTES_IN_LONG to the return value to account for the        space taken up by the string table size.  */
name|index
operator|+=
name|BYTES_IN_LONG
expr_stmt|;
...
specifier|static
name|void
name|pdp11_aout_swap_reloc_out
parameter_list|(
name|bfd
modifier|*
name|abfd
...
name|stderr
argument_list|,
literal|"BFD: can't do this reloc addend stuff\n"
argument_list|)
expr_stmt|;
name|r_pcrel
operator|=
...
argument_list|)
expr_stmt|;
comment|/* r_extern reflects whether the symbol the reloc is against is      local or global.  */
name|r_extern
operator|=
operator|(
name|reloc_entry
...
name|each_size
expr_stmt|;
comment|/* Count the number of NON-ZERO relocs, this is the count we want.  */
block|{
name|unsigned
name|int
name|real_count
...
name|done
label|:
comment|/* Just in case, if rptr>= relocs + reloc_size should happen      too early.  */
name|BFD_ASSERT
argument_list|(
name|counter
operator|==
...
operator|)
return|;
comment|/* TODO: why are there two if statements for obj_bsssec()? */
if|if
condition|(
name|asect
operator|==
...
begin_comment
comment|/* If we don't have to allocate more than 1MB to hold the generic    symbols, we use the generic minisymbol method: it's faster, since    it only translates the symbols once, not multiple times.  */
end_comment
...
name|stderr
argument_list|,
literal|"TODO: line %d in bfd/pdp11.c\n"
argument_list|,
name|__LINE__
argument_list|)
expr_stmt|;
...
name|asection
modifier|*
name|aout_reloc_type_to_section
parameter_list|(
name|bfd
modifier|*
name|abfd
...
specifier|static
name|bfd_boolean
name|pdp11_aout_link_input_section
parameter_list|(
name|struct
name|aout_final_link_info
modifier|*
...
name|stderr
argument_list|,
literal|"TODO: change the address of the relocation\n"
argument_list|)
expr_stmt|;
...
function_decl|HAS_LINENO | HAS_DEBUG |    HAS_SYMS | HAS_LOCALS | WP_TEXT
end_function_decl
