...
begin_struct
struct|struct
name|pdp11_aout_reloc_external
block|{
name|bfd_byte
name|e_reloc_entry
index|[
...
directive|if
literal|0
block|if ((cache_ptr->flags& BSF_CONSTRUCTOR) != 0)     {       int type = ((aout_symbol_type *) cache_ptr)->type;         switch (type) 	{ 	case N_ABS:	type = N_SETA; break; 	case N_TEXT:	type = N_SETT; break; 	case N_DATA:	type = N_SETD; break; 	case N_BSS:	type = N_SETB; break; 	}       sym_pointer->e_type[0] = type;     }
endif|#
directive|endif
if|#
directive|if
literal|0
block|if ((cache_ptr->flags& BSF_WEAK) != 0)     {       int type;        switch (sym_pointer->e_type[0]& N_TYPE) 	{ 	default: 	case N_ABS:	type = N_WEAKA; break; 	case N_TEXT:	type = N_WEAKT; break; 	case N_DATA:	type = N_WEAKD; break; 	case N_BSS:	type = N_WEAKB; break; 	case N_UNDF:	type = N_WEAKU; break; 	}       sym_pointer->e_type[0] = type;     }
endif|#
directive|endif
name|PUT_WORD
argument_list|(
...
directive|if
literal|0
block|if (bfd_is_abs_section (bfd_get_section (sym)))     {       r_extern = 0;       r_index = N_ABS;       r_type = RABS;     }   else if ((sym->flags& BSF_SECTION_SYM) == 0)     {       if (bfd_is_und_section (bfd_get_section (sym)) 	  || (sym->flags& BSF_GLOBAL) != 0) 	r_extern = 1;       else 	r_extern = 0;       r_index = (*(g->sym_ptr_ptr))->KEEPIT;     }   else     {
comment|/* Just an ordinary section */
block|r_extern = 0;       r_index = output_section->target_index;     }
endif|#
directive|endif
name|reloc_entry
operator|=
...
directive|if
literal|0
comment|/* If we're writing an .o file, we must write      relocation information, even if there is none. */
block|if ((count == 0 || section->orelocation == NULL)&&<writing_executable>)     return TRUE;
endif|#
directive|endif
name|natsize
operator|=
...
directive|if
literal|0
comment|/* not supported in PDP-11 a.out */
comment|/* Ignore debugging symbols.  */
block|if ((type& N_STAB) != 0) 	continue;
endif|#
directive|endif
name|name
operator|=
...
directive|if
literal|0
block|PUT_WORD (output_bfd, 		    r_addr + input_section->output_offset, 		    rel->r_address);
else|#
directive|else
name|fprintf
argument_list|(
...
begin_comment
comment|/* end of modified aoutx.h */
end_comment
