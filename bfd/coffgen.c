...
begin_comment
comment|/* Support for the generic parts of COFF, for BFD.    Copyright 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999,    2000, 2001, 2002, 2003, 2004, 2005    Free Software Foundation, Inc.    Written by Cygnus Support.     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
name|output_section
decl_stmt|;
comment|/* Do not try to update fields in read-only sections.  */
if|if
condition|(
operator|!
...
expr_stmt|;
block|}
comment|/* FIXME: Do we need to handle the absolute section here?  */
else|else
block|{
if|if
condition|(
...
name|vma
expr_stmt|;
comment|/* Copy the any flags from the file header into the symbol.          FIXME: Why?  */
block|{
name|coff_symbol_type
modifier|*
name|c
...
name|_
argument_list|(
literal|"%B: bad string table size %lu"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
block|{
comment|/* Ordinary short filename, put into memory anyway.  The                  Microsoft PE tools sometimes store a filename in                  multiple AUX entries.  */
if|if
condition|(
name|internal_ptr
operator|->
...
name|file
argument_list|,
literal|"(sec %2d)(fl 0x%02x)(ty %3x)(scl %3d) (nx %d) 0x"
argument_list|,
name|combined
operator|->
name|u
...
name|file
argument_list|,
literal|"AUX tagndx %ld ttlsiz 0x%lx lnnos %ld next %ld"
argument_list|,
name|tagndx
argument_list|,
name|auxp
...
block|}
block|}
comment|/* We use<= MAXDIFF here so that if we get a zero length              file, we actually use the next file entry.  */
if|if
condition|(
name|p2
operator|<
...
comment|/* If we have been called on this section before, and th. e offset we      want is further down then we can prime the lookup loop.  */
name|sec_data
operator|=
name|coff_section_data
argument_list|(
...
expr_stmt|;
block|}
comment|/* If we fell off the end of the loop, then assume that this 	 symbol has no line number info.  Otherwise, symbols with no 	 line number info get reported with the line number of the 	 last line of the last symbol which does have line number 	 info.  We use 0x100 as a slop to account for cases where the 	 last line has executable code.  */
if|if
condition|(
name|i
operator|>=
...
begin_comment
comment|/* Change the class of a coff symbol held by BFD.  */
end_comment
...
condition|)
block|{
comment|/* This is an alien symbol which no native coff backend data. 	 We cheat here by creating a fake native entry for it and 	 then filling in the class.  This code is based on that in 	 coff_write_alien_symbol().  */
name|combined_entry_type
modifier|*
name|native
decl_stmt|;
