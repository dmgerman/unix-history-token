...
name|EXTEND_BUFFER
define|\
value|{ old_buffer = bufp->buffer; \     if (bufp->allocated == (1<<16)) goto too_big; \     bufp->allocated *= 2; \     if (bufp->allocated> (1<<16)) bufp->allocated = (1<<16); \     if (!(bufp->buffer = (char *) realloc (bufp->buffer, bufp->allocated))) \       goto memory_exhausted; \     c = bufp->buffer - old_buffer; \     b += c; \     if (fixup_jump) \       fixup_jump += c; \     if (laststart) \       laststart += c; \     begalt += c; \     if (pending_exact) \       pending_exact += c; \   }
end_define
...
name|translate
decl_stmt|;
comment|/* Temporary used when buffer is made bigger. */
name|char
modifier|*
name|old_buffer
decl_stmt|;
...
literal|'$'
case|:
comment|/* $ means succeed if at end of line, but only in special contexts. 	    If randonly in the middle of a pattern, it is a normal character. */
if|if
condition|(
name|p
operator|==
...
expr_stmt|;
block|}
comment|/* Initialize \( and \) text positions to -1      to mark ones that no \( or \) has been seen for.  */
for|for
control|(
name|mcnt
operator|=
