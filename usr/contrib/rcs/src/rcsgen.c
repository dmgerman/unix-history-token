...
index|[]
init|=
literal|"$Header: rcsgen.c,v 3.4 86/05/15 02:18:42 lepreau Exp $ Purdue CS"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* $Log:	rcsgen.c,v $  * Revision 3.4  86/05/15  02:18:42  lepreau  * Fix immediate EOF from non-tty files: avoid 0377's in description.  *   * Revision 3.3  82/11/28  21:36:49  wft  * *** empty log message ***  *   * Revision 3.3  82/11/28  21:36:49  wft  * Replaced ferror() followed by fclose() with ffclose().  * Putdesc() now suppresses the prompts if stdin  * is not a terminal. A pointer to the current log message is now  * inserted into the corresponding delta, rather than leaving it in a  * global variable.  *   * Revision 3.2  82/10/18  21:11:26  wft  * I added checks for write errors during editing, and improved  * the prompt on putdesc().  *  * Revision 3.1  82/10/13  15:55:09  wft  * corrected type of variables assigned to by getc (char --> int)  */
end_comment
