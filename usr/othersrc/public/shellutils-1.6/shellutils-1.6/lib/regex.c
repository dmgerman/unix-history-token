...
begin_comment
comment|/* For interactive testing, compile with -Dtest.  Then this becomes    a self-contained program which reads a pattern, describes how it    compiles, then reads a string and searches for it.  If a command-line    argument is present, it is taken to be the value for obscure_syntax (in    decimal).  The default is 0 (Emacs-style syntax).        If DEBUG is defined, this prints many voluminous messages about what    it is doing (if the variable `debug' is nonzero).  */
end_comment
...
begin_comment
comment|/* USG or POSIX or STDC_HEADERS  */
end_comment
...
begin_comment
comment|/* Get the interface, including the syntax bits.  POSIX says<regex.h>    requires<sys/types.h>, and since<sys/types.h> is sometimes not    protected, it seems prudent to not include it in regex.h itself.  */
end_comment
...
begin_comment
comment|/* not __GNUC__ or sparc or _AIX */
end_comment
...
begin_comment
comment|/* Still not REGEX_MALLOC.  */
end_comment
...
begin_comment
comment|/* Requires a `char *destination' declared.  */
end_comment
...
parameter_list|)
define|\
value|(destination = alloca (size),						\    bcopy (source, destination, size),					\    destination)
end_define
...
name|no_pop_jump
block|,
comment|/* Same as no_pop_jump, but marks the end of an alternative.  */
name|jump_past_next_alt
block|,
...
block|,
comment|/* Change to pop_failure_jump if know won't have to backtrack to            match; otherwise change to no_pop_jump.  This is used to jump            back to the beginning of a repeat.  If what follows this jump            clearly won't match what the repeat does, such that we can be            sure that there is no use backtracking out of repetitions            already matched, then we change it to a pop_failure_jump.  */
name|maybe_pop_jump
block|,
comment|/* Jump, and push a dummy failure point. This failure point will            be thrown away if an attempt is made to use it for a failure.            A `+' construct makes this before the first repeat.  Also use            it as an intermediary kind of jump when compiling an            alternative.  */
name|dummy_failure_jump
block|,
comment|/* Used like on_failure_jump except has to succeed n times; The            two-byte relative address following it is useless until then.            The address is followed by two bytes containing n.  */
name|succeed_n
block|,
comment|/* Similar to no_pop_jump, but jump n times only; also the            relative address following is in turn followed by yet two            more bytes containing n.  */
name|no_pop_jump_n
block|,
comment|/* Set the following relative location (two bytes) to the            subsequent (two-byte) number.  */
name|set_number_at
block|,
name|wordchar
block|,
...
parameter_list|)
define|\
value|do { (destination) = *(source)& 0377;				\     (destination) += SIGN_EXTEND_CHAR (*(char *)((source) + 1))<< 8; 	\   } while (0)
end_define
...
begin_comment
comment|/* Print a compiled pattern buffer in human-readable form, starting at    the START pointer into it and ending just before the pointer END.  */
end_comment

begin_function
specifier|static
name|void
name|partial_compiled_pattern_printer
parameter_list|(
name|pbufp
parameter_list|,
name|start
...
return|return;
block|}
comment|/* This loop loops over pattern commands.  */
while|while
condition|(
name|p
operator|<
...
name|printf
argument_list|(
literal|"/jump_past_next_alt/0/%d"
argument_list|,
name|mcnt
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* It is useful to test things that must to be true when debugging.  */
end_comment
...
begin_comment
comment|/* We write debugging messages using stdio routines.  */
end_comment
...
define|#
directive|define
name|DEBUG_COMPILED_PATTERN_PRINTER
parameter_list|(
name|p
parameter_list|,
name|s
...
parameter_list|)
define|\
value|if (debug) partial_compiled_pattern_printer (p, s, e)
end_define

begin_define
define|#
directive|define
name|DEBUG_DOUBLE_STRING_PRINTER
parameter_list|(
name|w
parameter_list|,
name|s1
...
parameter_list|)
define|\
value|if (debug) double_string_printer (w, s1, sz1, s2, sz2)
end_define
...
begin_comment
comment|/* Set by re_set_syntax to the current regexp syntax to recognize.  Can    also be assigned to more or less arbitrarily.  Since we use this as a    collection of bits, declaring it unsigned maximizes portability.  */
end_comment
...
begin_comment
comment|/* Other subroutine declarations and macros for regex_compile.  */
end_comment
...
parameter_list|)
define|\
value|do {									\     GET_BUFFER_SPACE (1);						\     *b++ = (c);								\   } while (0)
end_define

begin_comment
comment|/* Make sure we have two more bytes of buffer space and then add C1 and     C2 to it.  */
end_comment
...
parameter_list|)
define|\
value|do {									\     GET_BUFFER_SPACE (2);						\     *b++ = (c1);							\     *b++ = (c2);							\   } while (0)
end_define

begin_comment
comment|/* Make sure we have two more bytes of buffer space and then add C1, C2    and C3 to it.  */
end_comment
...
parameter_list|)
define|\
value|do {									\     GET_BUFFER_SPACE (3);						\     *b++ = (c1);							\     *b++ = (c2);							\     *b++ = (c3);							\   } while (0)
end_define

begin_comment
comment|/* This is not an arbitrary limit: the arguments to the opcodes which    represent offsets into the pattern are two bytes long.  So if 2^16    bytes turns out to be too small, many things would have to change.  */
end_comment
...
begin_comment
comment|/* This type needs to be able to hold values from 0 to MAX_BUF_SIZE - 1.  */
end_comment
...
begin_comment
comment|/* Read the endpoint of a range from the uncompiled pattern and set the    corresponding bits in the compiled pattern.  */
end_comment
...
name|DO_RANGE
define|\
value|{									\     char end;								\     char this_char = p[-2];						\                                                                        	\     if (p == pend)							\       return REG_ERANGE;						\     PATFETCH (end);							\     if (syntax& RE_NO_EMPTY_RANGES&& this_char> end)		\       return REG_ERANGE;						\     while (this_char<= end)						\       {									\         SET_LIST_BIT (TRANSLATE (this_char));				\         this_char++;							\       }									\     }
end_define
...
name|char
modifier|*
name|following_left_brace
decl_stmt|;
comment|/* For a repeat, 1 means zero (many) matches is allowed.  */
name|char
name|zero_times_ok
decl_stmt|,
name|many_times_ok
...
condition|)
block|{
comment|/* EXTEND_BUFFER loses when bufp->allocated is 0.  This loses if              buffer's address is bogus, but that is the user's              responsibility.  */
name|RETALLOC
argument_list|(
...
condition|)
block|{
comment|/* ^ matches the empty string at the beginning of a string (or            possibly a line).  If RE_CONTEXT_INDEP_ANCHORS is set, ^ is            always an operator (and foo^bar is unmatchable).  If that bit            isn't set, it's an operator only at the beginning of the            pattern or after an alternation or open-group operator, or,            if RE_NEWLINE_ORDINARY is not set, after a newline (except it            can be preceded by other operators that match the empty            string); otherwise, it's a normal character.  */
case|case
literal|'^'
case|:
block|{
if|if
condition|(
comment|/* If at start of (sub)pattern, it's an operator.  */
name|laststart
operator|==
literal|0
...
comment|/* If after a newline, might be an operator.  (Since                       laststart is nonzero here, we know we have at                       least one byte before the ^.)  */
operator|||
operator|(
operator|!
operator|(
...
block|}
break|break;
comment|/* $ matches the empty string following the end of the string (or            possibly a line).  It follows rules dual to those for ^.  */
case|case
literal|'$'
case|:
block|{
...
condition|)
block|{
comment|/* More than one repetition is allowed, so put in at the                  end a backward relative jump from b to before the next                  jump we're going to put in below (which jumps from                  laststart to after this jump).  */
name|GET_BUFFER_SPACE
argument_list|(
literal|3
argument_list|)
...
literal|3
expr_stmt|;
comment|/* Because store_jump puts stuff here.  */
block|}
...
condition|)
block|{
comment|/* At least one repetition is required, so insert a                  dummy_failure before the initial on_failure_jump                  instruction of the loop. This effects a skip over that                  instruction the first time we hit that loop.  */
name|GET_BUFFER_SPACE
argument_list|(
literal|3
argument_list|)
...
name|REG_EBRACK
return|;
comment|/* Ensure that we have enough space to push an entire                charset: the opcode, the byte count, and the bitmap.  */
while|while
condition|(
name|b
operator|-
...
argument_list|)
expr_stmt|;
comment|/* charset_not can match newline by default.  */
if|if
condition|(
operator|(
name|re_opcode_t
...
block|}
block|}
comment|/* Discard any (non)matching list bytes that are all 0 at the                end of the map.  Decrement the map-length byte too.  */
while|while
condition|(
operator|(
name|int
...
operator|++
expr_stmt|;
comment|/* If found an empty group...  */
if|if
condition|(
operator|*
name|p1
...
name|regnum
expr_stmt|;
comment|/* We will eventually replace the 0 with the number of                  groups inner to this one.  */
if|if
condition|(
name|regnum
operator|<=
...
name|normal_char
goto|;
comment|/* Disallow empty alternatives if RE_NO_EMPTY_ALTS is set.                  Caveat: can't detect if the vbar is followed by a                  trailing '$' yet, unless it's the last thing in a                  pattern; the routine for verifying endlines has to do                  the rest.  */
if|if
condition|(
operator|(
name|syntax
...
label|:
block|{
comment|/* If got here, then intervals must be allowed.  */
comment|/* For intervals, at least (most) this many matches must                    be made.  */
name|int
name|lower_bound
init|=
operator|-
...
expr_stmt|;
block|}
comment|/* Otherwise, after lower_bound number of succeeds, jump                     to after the no_pop_jump_n which will be inserted at                     the end of the buffer, and insert that                     no_pop_jump_n.  */
else|else
block|{
comment|/* Set to 5 if only one repetition is allowed and                         hence no no_pop_jump_n is inserted at the current                         end of the buffer.  Otherwise, need 10 bytes total                         for the succeed_n and the no_pop_jump_n.  */
name|unsigned
name|slots_needed
init|=
name|upper_bound
...
argument_list|)
expr_stmt|;
comment|/* Initialize the succeed_n to n, even though it will                         be set by its attendant set_number_at, because                         re_compile_fastmap will need to know it.  Jump to                         what the end of buffer will be after inserting                         this succeed_n and possibly appending a                         no_pop_jump_n.  */
name|insert_jump_n
argument_list|(
name|succeed_n
argument_list|,
...
literal|5
expr_stmt|;
comment|/* When hit this when matching, reset the                             preceding no_pop_jump_n's n to upper_bound - 1.  */
name|PAT_PUSH
argument_list|(
name|set_number_at
argument_list|)
expr_stmt|;
comment|/* Only need to get space for the numbers.  */
name|GET_BUFFER_SPACE
argument_list|(
literal|4
argument_list|)
...
operator|)
decl_stmt|;
comment|/* Skip past operators that match the empty string.  (Except we don't      handle empty groups.)  */
while|while
condition|(
name|p
operator|<
...
comment|/* Not an empty string operator.  */
break|break;
block|}
comment|/* See what we're at now.  */
return|return
name|p
operator|<
name|pend
...
begin_comment
comment|/* Roughly the maximum number of failure points on the stack.  Would be    exactly that if always used MAX_FAILURE_SPACE each time we failed.  */
end_comment
...
define|#
directive|define
name|FAILURE_STACK_EMPTY
value|(failure_stack.avail == 0)
end_define

begin_define
define|#
directive|define
name|FAILURE_STACK_PTR_EMPTY
value|(failure_stack_ptr->avail == 0)
end_define

begin_define
define|#
directive|define
name|FAILURE_STACK_FULL
value|(failure_stack.avail == failure_stack.size)
end_define

begin_comment
comment|/* Initialize a failure stack.     Return 1 if was able to allocate the space for (FAILURE_STACK) and    0 if not.  */
end_comment

begin_define
define|#
directive|define
name|INIT_FAILURE_STACK
parameter_list|(
name|failure_stack
parameter_list|)
define|\
value|((failure_stack).stack = (failure_stack_elt_t *)			\     REGEX_ALLOCATE (INIT_FAILURE_ALLOC * sizeof (failure_stack_elt_t)),\ 									\   (failure_stack).stack == NULL						\   ? 0									\   : ((failure_stack).size = INIT_FAILURE_ALLOC,				\      (failure_stack).avail = 0,						\      1))
end_define

begin_comment
comment|/* Double the size of FAILURE_STACK, up to MAX_SIZE.       Return 1 if was able to double it, and 0 if either ran out of memory    allocating space for it or it was already MAX_SIZE large.          REGEX_REALLOCATE requires `destination' be declared.   */
end_comment

begin_define
define|#
directive|define
name|DOUBLE_FAILURE_STACK
parameter_list|(
name|failure_stack
parameter_list|,
name|max_size
parameter_list|)
define|\
value|((failure_stack).size> max_size					\    ? 0									\    : ((failure_stack).stack = (failure_stack_elt_t *)			\         REGEX_REALLOCATE ((failure_stack).stack, 			\           ((failure_stack).size<< 1) * sizeof (failure_stack_elt_t)),\ 									\       (failure_stack).stack == NULL					\       ? 0								\       : ((failure_stack).size<<= 1, 					\          1)))
end_define

begin_comment
comment|/* Push PATTERN_OP on FAILURE_STACK.      Return 1 if was able to do so and 0 if ran out of memory allocating    space to do so.     DOUBLE_FAILURE_STACK requires `destination' be declared.   */
end_comment
...
parameter_list|)
define|\
value|((FAILURE_STACK_FULL							\&& !DOUBLE_FAILURE_STACK (failure_stack, re_max_failures))		\     ? 0									\     : ((failure_stack).stack[(failure_stack).avail++] = pattern_op,	\        1))
end_define

begin_comment
comment|/* This pushes an item onto the failure stack.  Must be a four-byte    value.  Assumes the variable `failure_stack'.  Probably should only    be called from within `PUSH_FAILURE_POINT'.  */
end_comment
...
parameter_list|)
define|\
value|failure_stack.stack[failure_stack.avail++] = (failure_stack_elt_t) item
end_define

begin_comment
comment|/* The complement operation.  Assumes stack is nonempty, and pointed to    `failure_stack_ptr'.  */
end_comment
...
parameter_list|()
define|\
value|failure_stack_ptr->stack[--failure_stack_ptr->avail]
end_define
...
begin_comment
comment|/* Push the information about the state we will need    if we ever fail back to it.          Requires variables failure_stack, regstart, regend, reg_info, and    num_regs be declared.  DOUBLE_FAILURE_STACK requires `destination' be    declared.        Does `return FAILURE_CODE' if runs out of memory.  */
end_comment
...
define|\
value|((failure_stack).size - (failure_stack).avail)
end_define
...
begin_comment
comment|/* re_compile_fastmap computes a ``fastmap'' for the compiled pattern in    BUFP.  A fastmap records which of the (1<< BYTEWIDTH) possible    characters can start a string that matches the pattern.  This fastmap    is used by re_search to skip quickly over impossible starting points.     The caller must supply the address of a (1<< BYTEWIDTH)-byte data    area as BUFP->fastmap.  The other components of BUFP describe the    pattern to be used.        Returns 0 if it can compile a fastmap.  Returns -2 if there is an    internal error.   */
end_comment
...
condition|)
continue|continue;
comment|/* Jump backward reached implies we just went through              the body of a loop and matched nothing.  Opcode jumped to              should be an on_failure_jump or succeed_n.  Just treat it              like an ordinary jump.  For a * loop, it has pushed its              failure point already; if so, discard that as redundant.  */
if|if
condition|(
operator|(
name|re_opcode_t
...
block|}
comment|/* switch *p++ */
comment|/* Getting here means we have successfully found the possible starting          characters of one path of the pattern.  We need not follow this          path any farther.  Instead, look at the next alternative          remembered in the stack.  */
if|if
condition|(
operator|!
name|FAILURE_STACK_EMPTY
...
condition|)
block|{
comment|/* If a fastmap is supplied, skip quickly over characters that          cannot be the start of a match.  If the pattern can match the          null string, however, we don't want to skip over characters; we          want the first null string.  */
if|if
condition|(
name|fastmap
operator|&&
...
specifier|static
name|boolean
name|group_unmatchable
argument_list|()
decl_stmt|,
name|common_op_unmatchable
argument_list|()
decl_stmt|,
name|alternative_unmatchable
argument_list|()
decl_stmt|;
end_decl_stmt
...
struct|struct
block|{
comment|/* This field is one if this group can match the empty string,          zero if not.  If not yet determined,  `MATCH_NOTHING_UNSET_VALUE'.  */
define|#
directive|define
name|MATCH_NOTHING_UNSET_VALUE
value|3
name|unsigned
name|unmatchable
range|:
...
define|#
directive|define
name|CAN_MATCH_NOTHING
parameter_list|(
name|R
parameter_list|)
value|((R).bits.unmatchable)
end_define
...
begin_comment
comment|/* Call this when have matched something; it sets `matched' flags for the    registers corresponding to the group of which we currently are inside.      Also records whether this group ever matched something.  We only care    about this information at `stop_memory', and then only about the    previous time through the loop (if the group is starred or whatever).    So it is ok to clear all the nonactive registers here.  */
end_comment
...
parameter_list|()
define|\
value|do									\     {									\       unsigned this_reg;						\       for (this_reg = 1; this_reg< num_regs; this_reg++)	\         {								\           MATCHED_SOMETHING (reg_info[this_reg])			\             = EVER_MATCHED_SOMETHING (reg_info[this_reg])		\             = IS_ACTIVE (reg_info[this_reg]);				\         }								\     }									\   while (0)
end_define

begin_comment
comment|/* This converts a pointer into one or the other of the strings into an    offset from the beginning of that string.  */
end_comment
...
name|pointer
parameter_list|)
value|IS_IN_FIRST_STRING (pointer)		\                                 ? (pointer) - string1			\                                 : (pointer) - string2 + size1
end_define

begin_comment
comment|/* Registers are set to a sentinel value when they haven't yet matched    anything.  */
end_comment
...
begin_comment
comment|/* Test if D points to a character which is word-constituent.  We have    two special cases to check for: if past the end of string1, look at    the first character in string2; and if before the beginning of    string2, look at the last character in string1.        We assume there is a string1, so use this in conjunction with    AT_STRINGS_BEG.  */
end_comment
...
parameter_list|)
define|\
value|(AT_STRINGS_BEG || AT_STRINGS_END || LETTER_P (d - 1) != LETTER_P (d))
end_define
...
parameter_list|()
define|\
value|do {									\     free (failure_stack.stack);						\     free (regstart);							\     free (regend);							\     free (old_regstart);						\     free (old_regend);							\     free (reg_info);							\     free (best_regstart);						\     free (best_regend);							\     reg_info = NULL;							\     failure_stack.stack = NULL;						\     regstart = regend = old_regstart = old_regend			\       = best_regstart = best_regend = NULL;				\   } while (0)
end_define
...
begin_comment
comment|/* As nothing, since we use alloca. */
end_comment
...
begin_comment
comment|/* re_match_2 matches the compiled pattern in BUFP against the    the (virtual) concatenation of STRING1 and STRING2 (of length SIZE1    and SIZE2, respectively).  We start matching at POS, and stop    matching at STOP.        If REGS is non-null, and the `no_sub' field of BUFP is nonzero, we    store offsets for the substring each group matched in REGS.  (If    BUFP->caller_allocated_regs is nonzero, we fill REGS->num_regs    registers; if zero, we set REGS->num_regs to RE_NREGS and allocate the    space with malloc before filling.)     We return -1 if no match, -2 if an internal error (such as the    failure stack overflowing).  Otherwise, we return the length of the    matched substring.  */
end_comment
...
directive|endif
comment|/* We fill all the registers internally, independent of what we      return, for use in backreferences.  The number here includes      register zero.  */
name|unsigned
name|num_regs
init|=
name|bufp
...
return|;
block|}
comment|/* Initialize subexpression text positions to -1 to mark ones that no      \( or ( and \) or ) has been seen for. Also set all registers to      inactive and mark them as not having any inner groups, able to      match the empty string, matched anything so far, or ever failed.  */
for|for
control|(
name|mcnt
operator|=
...
condition|)
block|{
comment|/* If they haven't allocated it, we'll do it.  */
if|if
condition|(
operator|!
name|bufp
...
expr_stmt|;
break|break;
comment|/* Match anything but possibly a newline or a null.  */
case|case
name|anychar
case|:
name|DEBUG_PRINT1
...
name|p
expr_stmt|;
comment|/* To send to group_unmatchable.  */
if|if
condition|(
name|CAN_MATCH_NOTHING
argument_list|(
...
literal|1
decl_stmt|;
comment|/* This loop will always terminate, because register 0 is                  always active.  */
name|assert
argument_list|(
name|IS_ACTIVE
argument_list|(
...
block|}
break|break;
comment|/* begline matches the empty string at the beginning of the string            (unless `not_bol' is set in `bufp'), and before newlines (if            `posix_newline' is set).  */
case|case
name|begline
case|:
name|DEBUG_PRINT1
...
break|break;
block|}
comment|/* We are not at the beginning of the data.  If we are              `posix_newline', check for a newline just matched.  */
if|if
condition|(
name|bufp
operator|->
...
break|break;
block|}
comment|/* We're not at the end of data.  If we are `posix_newline',              check for a newline coming up.  */
if|if
condition|(
name|bufp
operator|->
...
block|{
comment|/* We have to use a new variable because we can't push                the failure point to `p + mcnt' until after we do this.  */
name|unsigned
name|char
modifier|*
name|p1
...
expr_stmt|;
break|break;
comment|/* A smart repeat ends with a maybe_pop_jump. 	   We change it either to a pop_failure_jump or a no_pop_jump.  */
case|case
name|maybe_pop_jump
case|:
name|EXTRACT_NUMBER_AND_INCR
...
name|p
decl_stmt|;
comment|/* Compare the beginning of the repeat with what in the                pattern follows its end. If we can establish that there                is nothing that they would both match, i.e., that we                would have to backtrack because of (as in, e.g., `a*a')                then we can change to pop_failure_jump, because we'll                never have to backtrack.  */
...
name|mcnt
decl_stmt|;
comment|/* p1[0] ... p1[2] are the on_failure_jump corresponding                    to the maybe_finalize_jump of this case. Examine what                     follows it.  */
if|if
condition|(
name|p1
index|[
...
case|:
block|{
comment|/* We need to pass separate storage for the lowest and                highest registers, even though we aren't interested.                Otherwise, we will restore only one register from the                stack, since lowest will equal highest in                pop_failure_point (since they'll be the same memory                location).  */
name|unsigned
name|dummy_low
decl_stmt|,
name|dummy_high
...
name|DEBUG_PRINT2
argument_list|(
literal|"EXECUTING no_pop_jump %d "
argument_list|,
name|mcnt
argument_list|)
expr_stmt|;
...
expr_stmt|;
break|break;
comment|/* We need this opcode so we can detect where alternatives end            in `group_unmatchable' et al.  */
case|case
name|jump_past_next_alt
case|:
name|DEBUG_PRINT1
argument_list|(
literal|"EXECUTING jump_past_next_alt.\n"
argument_list|)
expr_stmt|;
goto|goto
name|no_pop
...
name|stderr
argument_list|,
literal|"regex: negative n at succeed_n.\n"
argument_list|)
expr_stmt|;
name|abort
argument_list|()
...
name|DEBUG_PRINT2
argument_list|(
literal|"EXECUTING no_pop_jump_n %d.\n"
argument_list|,
name|mcnt
argument_list|)
expr_stmt|;
...
name|matchnotsyntax
label|:
comment|/* We goto here from notsyntaxspec.  */
name|PREFETCH
expr_stmt|;
if|if
condition|(
...
comment|/* We jump here if a matching operation fails. */
name|fail
label|:
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* If this failure point is a dummy_failure_point, skip it.  */
if|if
condition|(
operator|!
name|d
...
begin_comment
comment|/* Pops what PUSH_FAILURE_STACK pushes.  */
end_comment
...
directive|ifdef
name|DEBUG
comment|/* Type is really unsigned; it's declared this way just to avoid a      compiler warning.  */
name|unsigned
name|char
modifier|*
name|failure_id
...
name|DEBUG_PRINT1
argument_list|(
literal|"pop_failure_point:\n"
argument_list|)
expr_stmt|;
name|DEBUG_PRINT2
argument_list|(
literal|"  Before pop, next avail: %d\n"
argument_list|,
name|failure_stack_ptr
operator|->
name|avail
...
name|DEBUG_PRINT2
argument_list|(
literal|"  Popping high active reg: %d\n"
argument_list|,
operator|*
name|highest_active_reg
argument_list|)
...
name|DEBUG_PRINT2
argument_list|(
literal|"  Popping low active reg: %d\n"
argument_list|,
operator|*
name|lowest_active_reg
argument_list|)
...
begin_comment
comment|/* We are given P pointing to a register number after a start_memory.        Return true if the pattern up to the corresponding stop_memory can    match the empty string, and false otherwise.        If we find the matching stop_memory, sets P to point to one past its number.    Otherwise, sets P to an undefined byte less than or equal to END.     We don't handle duplicates properly (yet).  */
end_comment
...
comment|/* Deal with the first (n-1) alternatives, which start                  with an on_failure_jump (see above) that jumps to right                  past a jump_past_next_alt.  */
while|while
condition|(
operator|(
name|re_opcode_t
...
condition|)
block|{
comment|/* `mcnt' holds how many bytes long the alternative                      is, including the ending `jump_past_next_alt' and                      its number.  */
if|if
condition|(
operator|!
name|alternative_unmatchable
...
name|false
return|;
comment|/* Move to right after this alternative, including the 		     jump_past_next_alt.  */
name|p1
operator|+=
name|mcnt
expr_stmt|;
...
block|}
block|}
comment|/* Deal with the last alternative: go back and get number                  of the jump_past_next_alt just before it.  `mcnt'                  contains how many bytes long the alternative is.  */
name|EXTRACT_NUMBER
argument_list|(
name|mcnt
argument_list|,
...
begin_comment
comment|/* group_unmatchable */
end_comment

begin_comment
comment|/* Similar to group_unmatchable, but doesn't deal with alternatives:    It expects P to be the first byte of a single alternative and END one    byte past the last. The alternative can contain groups.  */
end_comment
...
block|}
block|}
comment|/* While not at the end of the alternative.  */
return|return
name|true
return|;
block|}
...
begin_comment
comment|/* Deals with the ops common to group_unmatchable and    alternative_unmatchable.          Sets P to one after the op and its arguments, if any.  */
end_comment
...
name|ret
decl_stmt|;
comment|/* The following is something of a kludge, but it is reasonably      simple.  Here is the story: POSIX defines a flag REG_NEWLINE for      regcomp, which if set tells us to do the following four things:        (1) . doesn't match newline;        (2) [^...] doesn't match newline;        (3) ^ matches after newline;        (4) $ matches before newline.      Emacs and the other GNU programs already did (1), (3), and (4),      unconditionally.  Therefore, (1) is implemented by a syntax bit      (RE_DOT_NEWLINE), and so the `posix_newline' field has nothing to      do with it.  (2) is implemented by regex_compile, if `posix_newline'      is set; since this function is the GNU compile function, we want      `posix_newline' to be false there.  (3) and (4) are implemented by      re_match_2, if `posix_newline' is set; so we want `posix_newline'      to be true during matching.  */
comment|/* Don't do special things with newline when we compile.  */
name|bufp
operator|->
name|posix_newline
operator|=
literal|0
expr_stmt|;
comment|/* GNU code is written to assume RE_NREGS registers will be set      (and extraneous ones will be filled with -1).  */
name|bufp
operator|->
name|caller_allocated_regs
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Do do special things with newline when we match.  */
name|bufp
operator|->
name|posix_newline
operator|=
...
return|;
block|}
comment|/* Don't do special things with newline when we compile (see      re_compile_pattern for the full details).  */
name|re_comp_buf
operator|.
name|posix_newline
operator|=
...
begin_comment
comment|/* Entry points compatible with POSIX regex library.  Only define these    when this is a POSIX compilation (and it's not Emacs).  */
end_comment
...
begin_comment
comment|/* regcomp takes a regular-expression string and compiles it.     PREG      is a regex_t * whose pertinent fields are mentioned in below:                               It has a char * field called BUFFER which points to the              space where this routine will put the compiled pattern; the              user can either allocate this using malloc (whereupon they              should set the long field ALLOCATED to the number of bytes              malloced) or set ALLOCATED to 0 and let the routine              allocate it.  The routine may use realloc to enlarge the              buffer space.                            If the user wants to translate all ordinary elements in the              compiled pattern, they should set the char * field              TRANSLATE to a translate table (and not set the REG_ICASE              bit of CFLAGS, which would override this translate table              with one that ignores case); otherwise, they should set              TRANSLATE to 0.                            The routine sets the int field SYNTAX to RE_SYNTAX_POSIX_EXTENDED              if the REG_EXTENDED bit in CFLAGS is set; otherwise, it sets it               to RE_SYNTAX_POSIX_BASIC.                            It returns in the long field USED how many bytes long the              compiled pattern is.                            It returns 0 in the char field FASTMAP_ACCURATE, on              the assumption that the user usually doesn't compile the              same pattern twice and that consequently any fastmap in the              pattern buffer is inaccurate.                     	     In the size_t field RE_NSUB, it returns the number of              subexpressions it found in PATTERN.     PATTERN   is the address of the pattern string.     CFLAGS    is a series of bits ORed together which affect compilation.              If the bit REG_EXTENDED is set, regcomp compiles the              pattern as an extended regular expression, otherwise it              compiles it as a basic one.  If the bit REG_NEWLINE is set,              then dot and nonmatching lists won't match a newline, but              anchors will match at them.  If the bit REG_ICASE is set,              then it considers upper- and lowercase versions of letters              to be equal when matching.  If the bit REG_NOSUB is set,              then when PREG is passed to regexec, that routine will only              report success or failure.     It returns 0 if it succeeds, nonzero if it doesn't.  (See regex.h for    POSIX return codes and their meanings.)  */
end_comment
...
name|REG_ESPACE
return|;
comment|/* Map any uppercase characters into corresponding lowercase ones.  */
for|for
control|(
name|i
operator|=
...
literal|0
expr_stmt|;
comment|/* If REG_NEWLINE is set, newlines are treated differently; see      regex.texinfo.  Some of the differences are implemented by the      compiler, some are already implemented via syntax bits (which are      needed for other reasons as well), and some by the matcher.  */
name|preg
operator|->
name|posix_newline
operator|=
...
operator|)
expr_stmt|;
comment|/* POSIX says a null character in the pattern terminates it, so we       use strlen here.  */
name|ret
operator|=
name|regex_compile
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* POSIX doesn't allow to distinguish between an unmatched open-group      and an unmatched close-group: both are REG_EPAREN.  */
if|if
condition|(
name|ret
operator|==
...
operator|)
expr_stmt|;
comment|/* The user has told us how many registers to return information      about, via `nmatch'.  We have to pass that on to the matching      routines.  */
name|private_preg
operator|.
name|caller_allocated_regs
operator|=
