...
name|fnsv
init|=
literal|"C-Kermit functions, 5A(073) 8 Feb 92"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Because getpkt always writes */
end_comment

begin_comment
comment|/* into "data", but when this */
end_comment

begin_comment
comment|/* function is called, "data" might */
end_comment

begin_comment
comment|/* might not be allocated. */
end_comment
...
parameter_list|()
function_decl|;
comment|/* For translating Kanji characters */
name|int
name|zkanjf
parameter_list|()
function_decl|;
...
begin_comment
comment|/* Filename buffer must be static */
end_comment
...
name|cps
decl_stmt|;
comment|/* The following bit is in case user's timeout is shorter than the amount */
comment|/* of time it would take to receive a packet of the negotiated size. */
if|if
condition|(
name|speed
operator|>
...
condition|)
block|{
comment|/* First recompute timeout */
name|cps
operator|=
call|(
name|int
...
condition|)
block|{
comment|/* Only if not SET-cmd override */
name|x
operator|=
operator|(
name|rln
...
comment|/*   During testing of windows, this code required the user to SET WINDOW n   on BOTH Kermits.  Now that windows appear to be working pretty well, let's   use whatever the other Kermit requests.  (Edit 177)   BUT THEN...  It seems that certain commercial Kermit implementations do not   "do windows" correctly, even though they claim to in their initialization   packets.  If the following line is not active, there is no way for the   C-Kermit user to override the use of windows when the other Kermit requests   them.  (Edit 179) */
if|if
condition|(
name|wslotn
operator|>
...
directive|ifdef
name|datageneral
comment|/* A kludge for now -- the real change needs to be done elsewhere... */
block|{
specifier|extern
name|char
modifier|*
