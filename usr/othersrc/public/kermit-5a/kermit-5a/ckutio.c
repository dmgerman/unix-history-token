...
name|ckxv
init|=
literal|"UNIX tty I/O, 5A(089), 8 Feb 92"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*   Author: Frank da Cruz (fdc@cunixc.cc.columbia.edu, FDCCU@CUVMA.BITNET),   Columbia University Center for Computing Activities.   First released January 1985.   Copyright (C) 1985, 1992, Trustees of Columbia University in the City of New   York.  Permission is granted to any individual or institution to use this   software as long as it is not sold for profit.  This copyright notice must be   retained.  This software may not be included in commercial products without   written permission of Columbia University. */
end_comment
...
begin_comment
comment|/* If you define _POSIX_SOURCE you cannot get ENOTCONN from sys/errno.h */
end_comment
...
begin_comment
comment|/*   Major change in 5A(174).  We used to use LPASS8, if it was defined, in all   versions except ENCORE, to allow 8-bit data and Xon/Xoff flow control at the   same time.  But this LPASS8 business seems to have been causing trouble for   everybody but me!  For example, Annex terminal servers, commonly used with   Encore computers, do not support LPASS8 even though the Encore itself does.   Now, reportedly, even vanilla 4.3 BSD systems can't do this.  So let's turn   it off for everybody.  That means we goes back to using   raw mode, with no flow control.  Phooey. */
end_comment
...
directive|endif
comment|/* SVORPOSIX */
comment|/*	ttflui();  This fails some reason  */
block|}
...
name|F110
argument_list|,
literal|"tthang HP-UX: modem lines lowerd DTR = "
argument_list|,
name|modem_state
argument_list|,
literal|0
...
directive|ifndef
name|_IBMR2
comment|/* Don't do this on RS/6000... */
name|z
operator||=
name|TIOCM_RTS
expr_stmt|;
...
directive|endif
comment|/* _IBMR2 */
comment|/* or DTR will never come up again. */
endif|#
directive|endif
comment|/* TIOCM_RTS */
name|debug
...
directive|ifndef
name|_IBMR2
comment|/* SVID says the argument must be a pointer to an int. */
if|if
condition|(
name|ioctl
argument_list|(
...
directive|else
comment|/* Reportedly, the RS/6000 requires an int, not a pointer. */
if|if
condition|(
name|ioctl
argument_list|(
...
directive|ifndef
name|CLSOPN
comment|/* This method is used by default. */
...
directive|else
comment|/* We still need to restore the flags, etc. */
name|ttyfd
operator|=
name|priv_opn
argument_list|(
...
condition|)
block|{
comment|/* Read pid from lockfile */
if|if
condition|(
name|fpid
operator|>
...
name|FLO_HARD
condition|)
comment|/* Something besides Xon/Xoff */
else|#
directive|else
if|if
condition|(
...
name|F100
argument_list|,
literal|"ttpkt turning TANDEM off, RAW on"
argument_list|,
literal|""
argument_list|,
literal|0
...
comment|/* servers, telnet, rlogin, etc, so it is not generally used. */
name|x
operator|=
name|LPASS8
expr_stmt|;
...
name|F101
argument_list|,
literal|"ttpkt setting rawmode, flow"
argument_list|,
literal|""
argument_list|,
name|flow
...
name|RAW
expr_stmt|;
comment|/* Can use 8-bit raw mode. */
endif|#
directive|endif
comment|/* LPASS8 */
block|}
...
name|ANYP
expr_stmt|;
comment|/* Must tell Tower no parity */
endif|#
directive|endif
comment|/* TOWER1 */
if|if
...
directive|ifdef
name|CRTSCTS
comment|/* Found this one in<termios.h> */
name|ttraw
operator|.
name|c_iflag
operator||=
...
directive|ifdef
name|DTRXOFF
comment|/* This is straight out of SVID R3 */
if|if
condition|(
name|ioctl
argument_list|(
...
begin_comment
comment|/* Buffer, including push back */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* debug(F101, "myfillbuf read", "", my_count); */
if|if
condition|(
name|my_count
operator|<=
...
literal|0
expr_stmt|;
comment|/* If not local, use stdin */
name|debug
argument_list|(
name|F101
argument_list|,
literal|"ttflui xlocal"
argument_list|,
literal|""
argument_list|,
name|xlocal
...
comment|/*    jc = sysconf(_SC_JOB_CONTROL);	/* Whatever system says */
name|jc
operator|=
literal|1
expr_stmt|;
...
literal|0
expr_stmt|;
comment|/* Assume job control allowed */
name|debug
argument_list|(
name|F111
argument_list|,
...
literal|0
condition|)
comment|/* Sys III, Sys V, Apollo Aegis, etc */
name|n
operator|+=
name|my_count
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|/* Yes, turn off the alarm. */
if|if
condition|(
operator|!
name|backgrd
...
argument_list|()
expr_stmt|;
comment|/* If error, turn off timer, */
if|if
condition|(
operator|!
name|backgrd
...
comment|/* Here's where we actually check and adjust the parity. */
comment|/* The major flaw here is if parity is NONE (ttprty = 0) and the packets */
comment|/* really do have no parity, then parchk() is called for every packet. */
comment|/* In practice, this doesn't really harm efficiency noticably, but it would */
comment|/* be better if ttinl() had a way of knowing to stop doing this once a */
comment|/* particular file transfer had been started and checked. */
if|if
condition|(
name|ttprty
operator|==
...
directive|ifdef
name|AIXRS
comment|/* Don't pretend we're BSD any more! */
undef|#
directive|undef
name|BSD4
endif|#
...
name|IEXTEN
expr_stmt|;
comment|/* pass CTRL-V though while connected */
endif|#
directive|endif
comment|/* __386BSD__ */
endif|#
...
literal|0
condition|)
comment|/* Return the character if read ok */
return|return
operator|(
name|ch
operator|)
...
directive|ifndef
name|CIE
comment|/* CIE Regulus has no such symbol */
if|if
condition|(
name|n
operator|<
...
operator|)
return|;
comment|/* Return -1 as error indication */
block|}
block|}
name|saval
operator|=
...
directive|ifdef
name|UTEK
comment|/* Turn on or off hardware flow control under UTek */
name|VOID
name|hardflow
parameter_list|(
name|flow
