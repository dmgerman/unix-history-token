...
name|ckzv
init|=
literal|"File support, 5A(060) 8 Feb 92"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*   C-Kermit's OS/2 support originally by Chris Adie<C.Adie@uk.ac.edinburgh>   for C-Kermit 4F, Copyright (C) 1988 Edinburgh University Computing Service,   Scotland.  Permission is granted to any individual or institution to use,   copy, or redistribute this software so long as it is not sold for profit,   provided this copyright notice is retained.    Adapted to C-Kermit 5A and integrated into the UNIX support module   by Kai Uwe Rommel<rommel@informatik.tu-muenchen.de>, Muenchen, Germany,   December 1991. */
end_comment

begin_comment
comment|/*   Directory Separator macros, to allow this module to work with both UNIX and   OS/2: Because of ambiguity with the command line editor escape \ character,   the directory separator is currently left as / for OS/2 too, because the   OS/2 kernel accepts / as directory separatore too. But this is subject to   change in future versions to conform with the normal OS/2 style. */
end_comment
...
begin_comment
comment|/* Same deal for Amdah UTSV */
end_comment
...
condition|)
block|{
comment|/* (unbuffered for debugging) */
name|write
argument_list|(
name|fileno
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/*  return((chdir(hd)> -1) ? 1 : 0); (===OS2===)  */
return|return
operator|(
operator|(
name|chdir
...
directive|ifdef
name|OS2
comment|/*   Well, OS/2 should be able to handle 8.3 FAT names still,   although HPFS is so much easier to live with ...   This code should be checked out with long names. */
name|char
modifier|*
name|bp
decl_stmt|,
...
comment|/* I don't care about that! */
name|isleapyear
operator|=
operator|(
operator|(
...
name|F110
argument_list|,
literal|"zstime entering switch"
argument_list|,
name|s
argument_list|,
literal|0
...
name|F111
argument_list|,
literal|"Attribute creation date ok "
argument_list|,
name|yy
operator|->
name|date
...
directive|endif
comment|/* NOFRILLS */
comment|/*   Wildcard expansion functions.  C-Kermit used to do this itself (see #else   part...).  New code (version 5A, 1990-91) just asks UNIX to do it.   This lets users use the wildcard expansion features of their favorite shell.   Operation is slower because of the forking& piping, but flexibility is   greater and program is smaller.  For OS/2, C-Kermit still does this itself. */
specifier|static
name|char
name|scratch
index|[
...
comment|/*   This is not specified in POSIX.1.  In POSIX.2, there will be   some kind of filename matching facility, but it will not be done this way.   Therefore, we should not even be looking in the directory at all.   Maybe POSIX implementations should force "set wildcard shell"? */
ifdef|#
directive|ifdef
name|sun
if|if
