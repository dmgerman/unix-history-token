...
name|stderr
argument_list|,
literal|"%s: must set the CVSROOT environment variable\n"
argument_list|,
name|program_name
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"cannot get current working directory: %s"
argument_list|,
name|cwd
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* null terminate the value  XXX - is this space ours? */
name|val
operator|.
name|dptr
index|[
...
name|slashp
decl_stmt|;
comment|/* put the ' ' in a copy so we don't mess up the original */
name|value
operator|=
name|strcpy
argument_list|(
...
name|found
label|:
comment|/* copy value to our own string since if we go recursive we'll be        really screwed if we do another dbm lookup */
name|zvalue
operator|=
name|xstrdup
argument_list|(
...
condition|)
block|{
comment|/* 	     * we had nothing but special options, so skip arg 	     * parsing and regular stuff entirely 	     * 	     * If there were only special ones though, we must 	     * make the appropriate directory and cd to it 	     */
name|char
modifier|*
name|dir
decl_stmt|;
...
condition|)
block|{
comment|/* 	     * If a relative pathname is specified as the checkout or 	     * tag proc, try to tack on the current "where" value. 	     * if we can't find a matching program, just punt and use 	     * whatever is specified in the modules file. 	     */
name|char
name|real_prog
index|[
name|PATH_MAX
...
name|def_status
expr_stmt|;
comment|/* Minor kluge, but general enough to maintain */
for|for
control|(
name|cp
operator|=
