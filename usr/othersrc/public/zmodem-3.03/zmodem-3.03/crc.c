...
begin_comment
comment|/*% cc -O -K -dos % -o crc.exe */
end_comment

begin_comment
comment|/*  *  Crc - 32 BIT ANSI X3.66 CRC checksum files  */
end_comment
...
begin_comment
comment|/**********************************************************************\ |*                                                                    *| |* Demonstration program to compute the 32-bit CRC used as the frame  *| |* check sequence in ADCCP (ANSI X3.66, also known as FIPS PUB 71     *| |* and FED-STD-1003, the U.S. versions of CCITT's X.25 link-level     *| |* protocol).  The 32-bit FCS was added via the Federal Register,     *| |* 1 June 1982, p.23798.  I presume but don't know for certain that   *| |* this polynomial is or will be included in CCITT V.41, which        *| |* defines the 16-bit CRC (often called CRC-CCITT) polynomial.  FIPS  *| |* PUB 78 says that the 32-bit FCS reduces otherwise undetected       *| |* errors by a factor of 10^-5 over 16-bit FCS.                       *| |*                                                                    *| \**********************************************************************/
end_comment

begin_comment
comment|/* Need an unsigned type capable of holding 32 bits; */
end_comment
...
begin_comment
comment|/*  1. The table can be generated at runtime if desired; code to do so */
end_comment
...
begin_comment
comment|/*  2. The CRC accumulation logic is the same for all CRC polynomials, */
end_comment

begin_comment
comment|/*     be they sixteen or thirty-two bits wide.  You simply choose the */
end_comment

begin_comment
comment|/*     appropriate table.  Alternatively, because the table can be     */
end_comment

begin_comment
comment|/*     generated at runtime, you can start by generating the table for */
end_comment

begin_comment
comment|/*     the polynomial in question and use exactly the same "updcrc",   */
end_comment

begin_comment
comment|/*     if your application needn't simultaneously handle two CRC       */
end_comment

begin_comment
comment|/*     polynomials.  (Note, however, that XMODEM is strange.)          */
end_comment
...
begin_comment
comment|/*  3. For 16-bit CRCs, the table entries need be only 16 bits wide;   */
end_comment

begin_comment
comment|/*     of course, 32-bit entries work OK if the high 16 bits are zero. */
end_comment
...
begin_comment
comment|/*  4. The values must be right-shifted by eight bits by the "updcrc"  */
end_comment
...
name|crc
parameter_list|)
value|(crc_32_tab[((crc) ^ (octet))& 0xff] ^ ((crc)>> 8))
end_define
...
name|oldcrc32
expr_stmt|;
comment|/* 	crc32 = UPDC32((oldcrc32& 0377), crc32);  oldcrc32>>=8; 	crc32 = UPDC32((oldcrc32& 0377), crc32);  oldcrc32>>=8; 	crc32 = UPDC32((oldcrc32& 0377), crc32);  oldcrc32>>=8; 	crc32 = UPDC32((oldcrc32& 0377), crc32);  oldcrc32>>=8; 	printf("%08lX ", crc32); */
name|printf
argument_list|(
literal|"%08lX %7ld %s\n"
argument_list|,
