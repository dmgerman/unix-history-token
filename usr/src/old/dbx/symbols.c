...
index|[]
init|=
literal|"@(#)symbols.c 1.10 8/10/83"
decl_stmt|;
end_decl_stmt
...
index|[]
init|=
literal|"$Header: symbols.c,v 1.4 84/03/27 10:24:18 linton Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * When necessary, compute the upper bound for an open array (Modula-2 style).  */
end_comment
...
name|panic
argument_list|(
literal|"unexpected array bound type"
argument_list|)
expr_stmt|;
block|}
block|}
...
operator|)
expr_stmt|;
comment|/* r = (t->symvalue.field.length + 7) div 8; */
break|break;
case|case
name|RECORD
case|:
...
begin_comment
comment|/*  * Test if a symbol is a var parameter, i.e. has class REF but  * is not an open array parameter (those are treated special).  */
end_comment
...
name|or
argument_list|(
argument|rt2->type == t_open and rt1->type == t_int
argument_list|)
name|or
name|rt1
operator|==
