...
name|fatal
argument_list|(
literal|"not linked for debugging, use \"cc -g ...\""
argument_list|)
expr_stmt|;
block|}
name|dispose
...
name|warning
argument_list|(
literal|"old style symbol information found in \"%s\""
argument_list|,
name|curfilename
argument_list|()
argument_list|)
...
name|and
argument_list|(
argument|t->class == PROC or t->class == FUNC
argument_list|)
name|endfind
argument_list|(
name|t
...
expr_stmt|;
break|break;
comment|/*  *  keep global BSS variables chained so can resolve when get the start  *  of common; keep the list in order so f77 can display all vars in a COMMON */
case|case
literal|'V'
case|:
...
begin_comment
comment|/*  * Construct a type out of a string encoding.  *  * The forms of the string are  *  *<number>  *<number>=<type>  *	r<type>;<number>;<number>		$ subrange  *	a<type>;<type>				$ array[index] of element  *	s{<name>:<type>;<number>;<number>}	$ record  *	S<type>					$ set  *	*<type>					$ pointer  */
end_comment
...
begin_comment
comment|/*  * Get a range type.  *  * Special letters indicate a dynamic bound, i.e. what follows  * is the offset from the fp which contains the bound.  * J is a special flag to handle fortran a(*) bounds.  */
end_comment
