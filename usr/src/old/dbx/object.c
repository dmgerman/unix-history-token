...
index|[]
init|=
literal|"@(#)object.c 1.14 10/22/83"
decl_stmt|;
end_decl_stmt
...
index|[]
init|=
literal|"$Header: object.c,v 1.4 84/03/27 10:22:25 linton Exp $"
decl_stmt|;
end_decl_stmt
...
name|panic
argument_list|(
literal|"continuation followed by empty stab"
argument_list|)
expr_stmt|;
block|}
name|name
...
begin_comment
comment|/*  * Try to find the symbol that is referred to by the given name.  * Since it's an external, we may want to follow a level of indirection.  */
end_comment
...
name|and
argument_list|(
argument|s->class == EXTREF or s->class == VAR or 	     s->class == PROC or s->class == FUNC
argument_list|)
name|endfind
argument_list|(
name|s
