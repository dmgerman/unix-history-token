...
index|[]
init|=
literal|"$Header: machine.c,v 1.5 84/12/26 10:40:05 linton Exp $"
decl_stmt|;
end_decl_stmt
...
parameter_list|)
define|\
value|((i>> ((s)->symvalue.field.offset mod BITSPERBYTE))& \ 	 ((1<< (s)->symvalue.field.length) - 1))
end_define

begin_comment
comment|/*  * Expand/contract the expression stack to reflect a type  * rename operation.  We pad with zeros when expanding,  * otherwise we just pull the stack pointer back.  */
end_comment
...
name|newsize
parameter_list|)
value|{ \ 	int len = newsize - oldsize; \ 	if (len> 0) \ 		bzero(sp, len); \ 	sp += len; \ }
end_define
