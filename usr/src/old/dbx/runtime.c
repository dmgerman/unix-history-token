...
index|[]
init|=
literal|"$Header: runtime.c,v 1.3 84/03/27 10:23:40 linton Exp $"
decl_stmt|;
end_decl_stmt
...
operator|)
return|;
comment|/*      * Starting at the current stack frame,      * walk backwards looking for a symbol      * match.  Beware of local blocks which      * have a back pointer but no stack frame.      */
name|p
operator|=
name|whatblock
argument_list|(
...
begin_comment
comment|/*  * Pass an expression to a particular parameter.  *  * Normally we pass either the address or value, but in some cases  * (such as C strings) we want to copy the value onto the stack and  * pass its address.  */
end_comment
...
argument|integer
argument_list|,
argument|paramsize div size(formal->type->type)
argument_list|)
empty_stmt|;
block|}
block|}
