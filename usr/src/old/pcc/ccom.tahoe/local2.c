...
index|[]
init|=
literal|"@(#)local2.c	1.8 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
condition|)
block|{
comment|/* 				 * Must mask if result is shorter than 				 * the width of a register (to account 				 * for register treatment). 				 */
name|printf
argument_list|(
literal|"\n\tandl2\t$%d,"
argument_list|,
...
expr_stmt|;
block|}
comment|/* 		 * If condition codes are required and the last thing 		 * we did was mask the result, then we must generate a 		 * test of the appropriate type. 		 */
if|if
condition|(
name|forcc
condition|)
