...
index|[]
init|=
literal|"@(#)local2.c	1.11 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * Generate code for integral scalar conversions.  * Many work-arounds for brain-damaged Tahoe register behavior.  */
end_comment
...
comment|/* if (p->in.op == SCONV || optype(p->in.op) == LTYPE) */
block|{
name|src
operator|=
name|getlr
