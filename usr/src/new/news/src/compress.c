...
name|SccsId
init|=
literal|"@(#)compress.c	1.14	9/24/87"
decl_stmt|;
end_decl_stmt
...
index|[]
init|=
literal|"Based on compress.c,v 4.0 85/07/30 12:50:00 joe Release"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * compress.c - File compression ala IEEE Computer, June 1984.  *  * Authors:	Spencer W. Thomas	(decvax!harpo!utah-cs!utah-gr!thomas)  *		Jim McKie		(decvax!mcvax!jim)  *		Steve Davies		(decvax!vax135!petsd!peora!srd)  *		Ken Turkowski		(decvax!decwrl!turtlevax!ken)  *		James A. Woods		(decvax!ihnp4!ames!jaw)  *		Joe Orost		(decvax!vax135!petsd!joe)  *  */
end_comment
...
begin_comment
comment|/* support gould base register problems */
end_comment
...
argument_list|)
expr_stmt|;
comment|/*		for(ent=htabof(stabof(i))& ((1<<maxbits)-1); */
name|mbshift
operator|=
operator|(
operator|(
...
literal|256
condition|;
comment|/* ent=htabof(stabof(ent))& ((1<<maxbits)-1)) { */
name|ent
operator|=
name|htabof
argument_list|(
