...
index|[]
init|=
literal|"$Header: /usr/wft/RCS/SRC/RCS/rcs.c,v 3.9 83/02/15 15:38:39 wft Exp $ Purdue CS"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* used for preserving mode of an exisiting RCS file*/
end_comment
...
argument_list|)
expr_stmt|;
comment|/* breaklock does it's own diagnose */
block|}
else|else
block|{
name|warn
...
name|error
argument_list|(
literal|"Can't set state %s of a nonexistent revision %s"
argument_list|,
name|curstate
operator|->
name|status
...
condition|)
block|{
comment|/*  state attribute of Head  */
name|chgheadstate
operator|=
name|true
expr_stmt|;
...
name|delta
decl_stmt|;
comment|/* function: Finds the lock held by who on delta,  * removes it, and returns a pointer to the delta.  * delta may be nil; then the first lock held by who is chosen.  * Prints an error message and returns nil if there is no such lock or error.  */
block|{
specifier|register
name|struct
name|lock
...
begin_comment
comment|/*    Function: remove locks which are stored in rmvlocklst,    */
end_comment

begin_comment
comment|/*              add new locks which are stored in newlocklst,   */
end_comment
...
name|lpt
decl_stmt|;
comment|/*  remove locks which stored in rmvlocklst   */
name|lockpt
operator|=
name|rmvlocklst
expr_stmt|;
...
name|error
argument_list|(
literal|"Can't unlock a nonexisting revision %s"
argument_list|,
name|lockpt
operator|->
name|revno
...
name|error
argument_list|(
literal|"Can't lock a nonexisting revision %s"
argument_list|,
name|lockpt
operator|->
name|revno
