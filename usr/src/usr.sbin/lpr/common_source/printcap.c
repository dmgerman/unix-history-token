...
begin_comment
comment|/*	printcap.c	1.1	81/05/09	*/
end_comment
...
begin_comment
comment|/*  * Get an entry for printer name in buffer bp,  * from the printcap file.  Parse is very rudimentary;  * we just notice escaped newlines.  */
end_comment
...
begin_comment
comment|/*  * Tnamatch deals with name matching.  The first field of the printcap  * entry is a sequence of names separated by |'s, so we compare  * against each such name.  The normal : terminator after the last  * name (before the first field) stops us.  */
end_comment
...
begin_comment
comment|/*  * Skip to the next field.  Notice that this is very dumb, not  * knowing about \: escapes or any such.  If necessary, :'s can be put  * into the printcap file in octal.  */
end_comment
