begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1983, 1991, 1993 The Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1983, 1991, 1993 Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)inetd.c	5.33 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_comment
comment|/*  * Inetd - Internet super-server  *  * This program invokes all internet services as needed.  Connection-oriented  * services are invoked each time a connection is made, by creating a process.  * This process is passed the connection as file descriptor 0 and is expected  * to do a getpeername to find out the source host and port.  *  * Datagram oriented services are invoked when a datagram  * arrives; a process is created and passed a pending message  * on file descriptor 0.  Datagram servers may either connect  * to their peer, freeing up the original socket for inetd  * to receive further messages on, or ``take over the socket'',  * processing all arriving datagrams and, eventually, timing  * out.	 The first type of server is said to be ``multi-threaded'';  * the second type of server ``single-threaded''.   *  * Inetd uses a configuration file which is read at startup  * and, possibly, at some later time in response to a hangup signal.  * The configuration file is ``free format'' with fields given in the  * order shown below.  Continuation lines for an entry must being with  * a space or tab.  All fields must be present in each entry.  *  *	service name			must be in /etc/services or must  *					name a tcpmux service  *	socket type			stream/dgram/raw/rdm/seqpacket  *	protocol			must be in /etc/protocols  *	wait/nowait			single-threaded/multi-threaded  *	user				user to run daemon as  *	server program			full path name  *	server program arguments	maximum of MAXARGS (20)  *  * TCP services without official port numbers are handled with the  * RFC1078-based tcpmux internal service. Tcpmux listens on port 1 for  * requests. When a connection is made from a foreign host, the service  * requested is passed to tcpmux, which looks it up in the servtab list  * and returns the proper entry for the service. Tcpmux returns a  * negative reply if the service doesn't exist, otherwise the invoked  * server is expected to return the positive reply if the service type in  * inetd.conf file has the prefix "tcpmux/". If the service type has the  * prefix "tcpmux/+", tcpmux will return the positive reply for the  * process; this is for compatibility with older server code, and also  * allows you to invoke programs that use stdin/stdout without putting any  * special server code in them. Services that use tcpmux are "nowait"  * because they do not have a well-known port and hence cannot listen  * for new requests.  *  * Comment lines are indicated by a `#' in column 1.  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/wait.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/resource.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<arpa/inet.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_include
include|#
directive|include
file|<pwd.h>
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<syslog.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|"pathnames.h"
end_include

begin_define
define|#
directive|define
name|TOOMANY
value|40
end_define

begin_comment
comment|/* don't start more than TOOMANY */
end_comment

begin_define
define|#
directive|define
name|CNT_INTVL
value|60
end_define

begin_comment
comment|/* servers in CNT_INTVL sec. */
end_comment

begin_define
define|#
directive|define
name|RETRYTIME
value|(60*10)
end_define

begin_comment
comment|/* retry after bind or server fail */
end_comment

begin_define
define|#
directive|define
name|SIGBLOCK
value|(sigmask(SIGCHLD)|sigmask(SIGHUP)|sigmask(SIGALRM))
end_define

begin_decl_stmt
name|void
name|config
argument_list|()
decl_stmt|,
name|reapchild
argument_list|()
decl_stmt|,
name|retry
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|debug
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nsock
decl_stmt|,
name|maxsock
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|fd_set
name|allsock
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|options
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|timingout
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|toomany
init|=
name|TOOMANY
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servent
modifier|*
name|sp
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|servtab
block|{
name|char
modifier|*
name|se_service
decl_stmt|;
comment|/* name of service */
name|int
name|se_socktype
decl_stmt|;
comment|/* type of socket to use */
name|char
modifier|*
name|se_proto
decl_stmt|;
comment|/* protocol used */
name|short
name|se_wait
decl_stmt|;
comment|/* single threaded server */
name|short
name|se_checked
decl_stmt|;
comment|/* looked at during merge */
name|char
modifier|*
name|se_user
decl_stmt|;
comment|/* user name to run as */
name|struct
name|biltin
modifier|*
name|se_bi
decl_stmt|;
comment|/* if built-in, description */
name|char
modifier|*
name|se_server
decl_stmt|;
comment|/* server program */
define|#
directive|define
name|MAXARGV
value|20
name|char
modifier|*
name|se_argv
index|[
name|MAXARGV
operator|+
literal|1
index|]
decl_stmt|;
comment|/* program arguments */
name|int
name|se_fd
decl_stmt|;
comment|/* open descriptor */
name|int
name|se_type
decl_stmt|;
comment|/* type */
name|struct
name|sockaddr_in
name|se_ctrladdr
decl_stmt|;
comment|/* bound address */
name|int
name|se_count
decl_stmt|;
comment|/* number started since se_time */
name|struct
name|timeval
name|se_time
decl_stmt|;
comment|/* start of se_count */
name|struct
name|servtab
modifier|*
name|se_next
decl_stmt|;
block|}
modifier|*
name|servtab
struct|;
end_struct

begin_define
define|#
directive|define
name|NORM_TYPE
value|0
end_define

begin_define
define|#
directive|define
name|MUX_TYPE
value|1
end_define

begin_define
define|#
directive|define
name|MUXPLUS_TYPE
value|2
end_define

begin_define
define|#
directive|define
name|ISMUX
parameter_list|(
name|sep
parameter_list|)
value|(((sep)->se_type == MUX_TYPE) || \ 			 ((sep)->se_type == MUXPLUS_TYPE))
end_define

begin_define
define|#
directive|define
name|ISMUXPLUS
parameter_list|(
name|sep
parameter_list|)
value|((sep)->se_type == MUXPLUS_TYPE)
end_define

begin_decl_stmt
name|int
name|echo_stream
argument_list|()
decl_stmt|,
name|discard_stream
argument_list|()
decl_stmt|,
name|machtime_stream
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|daytime_stream
argument_list|()
decl_stmt|,
name|chargen_stream
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|echo_dg
argument_list|()
decl_stmt|,
name|discard_dg
argument_list|()
decl_stmt|,
name|machtime_dg
argument_list|()
decl_stmt|,
name|daytime_dg
argument_list|()
decl_stmt|,
name|chargen_dg
argument_list|()
decl_stmt|;
end_decl_stmt

begin_function_decl
name|struct
name|servtab
modifier|*
name|tcpmux
parameter_list|()
function_decl|;
end_function_decl

begin_struct
struct|struct
name|biltin
block|{
name|char
modifier|*
name|bi_service
decl_stmt|;
comment|/* internally provided service name */
name|int
name|bi_socktype
decl_stmt|;
comment|/* type of socket supported */
name|short
name|bi_fork
decl_stmt|;
comment|/* 1 if should fork before call */
name|short
name|bi_wait
decl_stmt|;
comment|/* 1 if should wait for child */
name|int
function_decl|(
modifier|*
name|bi_fn
function_decl|)
parameter_list|()
function_decl|;
comment|/* function which performs it */
block|}
name|biltins
index|[]
init|=
block|{
comment|/* Echo received data */
literal|"echo"
block|,
name|SOCK_STREAM
block|,
literal|1
block|,
literal|0
block|,
name|echo_stream
block|,
literal|"echo"
block|,
name|SOCK_DGRAM
block|,
literal|0
block|,
literal|0
block|,
name|echo_dg
block|,
comment|/* Internet /dev/null */
literal|"discard"
block|,
name|SOCK_STREAM
block|,
literal|1
block|,
literal|0
block|,
name|discard_stream
block|,
literal|"discard"
block|,
name|SOCK_DGRAM
block|,
literal|0
block|,
literal|0
block|,
name|discard_dg
block|,
comment|/* Return 32 bit time since 1970 */
literal|"time"
block|,
name|SOCK_STREAM
block|,
literal|0
block|,
literal|0
block|,
name|machtime_stream
block|,
literal|"time"
block|,
name|SOCK_DGRAM
block|,
literal|0
block|,
literal|0
block|,
name|machtime_dg
block|,
comment|/* Return human-readable time */
literal|"daytime"
block|,
name|SOCK_STREAM
block|,
literal|0
block|,
literal|0
block|,
name|daytime_stream
block|,
literal|"daytime"
block|,
name|SOCK_DGRAM
block|,
literal|0
block|,
literal|0
block|,
name|daytime_dg
block|,
comment|/* Familiar character generator */
literal|"chargen"
block|,
name|SOCK_STREAM
block|,
literal|1
block|,
literal|0
block|,
name|chargen_stream
block|,
literal|"chargen"
block|,
name|SOCK_DGRAM
block|,
literal|0
block|,
literal|0
block|,
name|chargen_dg
block|,
literal|"tcpmux"
block|,
name|SOCK_STREAM
block|,
literal|1
block|,
literal|0
block|,
operator|(
name|int
argument_list|(
operator|*
argument_list|)
argument_list|()
operator|)
name|tcpmux
block|,
name|NULL
block|}
struct|;
end_struct

begin_define
define|#
directive|define
name|NUMINT
value|(sizeof(intab) / sizeof(struct inent))
end_define

begin_decl_stmt
name|char
modifier|*
name|CONFIG
init|=
name|_PATH_INETDCONF
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
modifier|*
name|Argv
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|LastArg
decl_stmt|;
end_decl_stmt

begin_expr_stmt
expr|main
operator|(
name|argc
operator|,
name|argv
operator|,
name|envp
operator|)
name|int
name|argc
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|char
modifier|*
name|argv
index|[]
decl_stmt|,
modifier|*
name|envp
index|[]
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
specifier|register
name|struct
name|passwd
modifier|*
name|pwd
decl_stmt|;
specifier|register
name|int
name|tmpint
decl_stmt|;
name|struct
name|sigvec
name|sv
decl_stmt|;
name|int
name|ch
decl_stmt|,
name|pid
decl_stmt|,
name|dofork
decl_stmt|;
name|char
name|buf
index|[
literal|50
index|]
decl_stmt|;
name|Argv
operator|=
name|argv
expr_stmt|;
if|if
condition|(
name|envp
operator|==
literal|0
operator|||
operator|*
name|envp
operator|==
literal|0
condition|)
name|envp
operator|=
name|argv
expr_stmt|;
while|while
condition|(
operator|*
name|envp
condition|)
name|envp
operator|++
expr_stmt|;
name|LastArg
operator|=
name|envp
index|[
operator|-
literal|1
index|]
operator|+
name|strlen
argument_list|(
name|envp
index|[
operator|-
literal|1
index|]
argument_list|)
expr_stmt|;
name|openlog
argument_list|(
literal|"inetd"
argument_list|,
name|LOG_PID
operator||
name|LOG_NOWAIT
argument_list|,
name|LOG_DAEMON
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|ch
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"dR:"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|ch
condition|)
block|{
case|case
literal|'d'
case|:
name|debug
operator|=
literal|1
expr_stmt|;
name|options
operator||=
name|SO_DEBUG
expr_stmt|;
break|break;
case|case
literal|'R'
case|:
block|{
comment|/* invocation rate */
name|char
modifier|*
name|p
decl_stmt|;
name|tmpint
operator|=
name|strtol
argument_list|(
name|optarg
argument_list|,
operator|&
name|p
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|tmpint
operator|<
literal|1
operator|||
operator|*
name|p
condition|)
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"-R %s: bad value for service invocation rate"
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
else|else
name|toomany
operator|=
name|tmpint
expr_stmt|;
break|break;
block|}
case|case
literal|'?'
case|:
default|default:
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"usage: inetd [-d] [-R rate] [conf-file]"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|argc
operator|-=
name|optind
expr_stmt|;
name|argv
operator|+=
name|optind
expr_stmt|;
if|if
condition|(
name|argc
operator|>
literal|0
condition|)
name|CONFIG
operator|=
name|argv
index|[
literal|0
index|]
expr_stmt|;
if|if
condition|(
name|debug
operator|==
literal|0
condition|)
block|{
name|daemon
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
name|bzero
argument_list|(
operator|&
name|sv
argument_list|,
sizeof|sizeof
argument_list|(
name|sv
argument_list|)
argument_list|)
expr_stmt|;
name|sv
operator|.
name|sv_mask
operator|=
name|SIGBLOCK
expr_stmt|;
name|sv
operator|.
name|sv_handler
operator|=
name|retry
expr_stmt|;
name|sigvec
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|sv
argument_list|,
operator|(
expr|struct
name|sigvec
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
name|config
argument_list|()
expr_stmt|;
name|sv
operator|.
name|sv_handler
operator|=
name|config
expr_stmt|;
name|sigvec
argument_list|(
name|SIGHUP
argument_list|,
operator|&
name|sv
argument_list|,
operator|(
expr|struct
name|sigvec
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
name|sv
operator|.
name|sv_handler
operator|=
name|reapchild
expr_stmt|;
name|sigvec
argument_list|(
name|SIGCHLD
argument_list|,
operator|&
name|sv
argument_list|,
operator|(
expr|struct
name|sigvec
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
block|{
comment|/* space for daemons to overwrite environment for ps */
define|#
directive|define
name|DUMMYSIZE
value|100
name|char
name|dummy
index|[
name|DUMMYSIZE
index|]
decl_stmt|;
operator|(
name|void
operator|)
name|memset
argument_list|(
name|dummy
argument_list|,
literal|'x'
argument_list|,
sizeof|sizeof
argument_list|(
name|DUMMYSIZE
argument_list|)
operator|-
literal|1
argument_list|)
expr_stmt|;
name|dummy
index|[
name|DUMMYSIZE
operator|-
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
operator|(
name|void
operator|)
name|setenv
argument_list|(
literal|"inetd_dummy"
argument_list|,
name|dummy
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
for|for
control|(
init|;
condition|;
control|)
block|{
name|int
name|n
decl_stmt|,
name|ctrl
decl_stmt|;
name|fd_set
name|readable
decl_stmt|;
if|if
condition|(
name|nsock
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|sigblock
argument_list|(
name|SIGBLOCK
argument_list|)
expr_stmt|;
while|while
condition|(
name|nsock
operator|==
literal|0
condition|)
name|sigpause
argument_list|(
literal|0L
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|sigsetmask
argument_list|(
literal|0L
argument_list|)
expr_stmt|;
block|}
name|readable
operator|=
name|allsock
expr_stmt|;
if|if
condition|(
operator|(
name|n
operator|=
name|select
argument_list|(
name|maxsock
operator|+
literal|1
argument_list|,
operator|&
name|readable
argument_list|,
operator|(
name|fd_set
operator|*
operator|)
literal|0
argument_list|,
operator|(
name|fd_set
operator|*
operator|)
literal|0
argument_list|,
operator|(
expr|struct
name|timeval
operator|*
operator|)
literal|0
argument_list|)
operator|)
operator|<=
literal|0
condition|)
block|{
if|if
condition|(
name|n
operator|<
literal|0
operator|&&
name|errno
operator|!=
name|EINTR
condition|)
name|syslog
argument_list|(
name|LOG_WARNING
argument_list|,
literal|"select: %m"
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
continue|continue;
block|}
for|for
control|(
name|sep
operator|=
name|servtab
init|;
name|n
operator|&&
name|sep
condition|;
name|sep
operator|=
name|sep
operator|->
name|se_next
control|)
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|!=
operator|-
literal|1
operator|&&
name|FD_ISSET
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
operator|&
name|readable
argument_list|)
condition|)
block|{
name|n
operator|--
expr_stmt|;
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"someone wants %s\n"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|)
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_socktype
operator|==
name|SOCK_STREAM
condition|)
block|{
name|ctrl
operator|=
name|accept
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
literal|0
argument_list|,
operator|(
name|int
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"accept, ctrl %d\n"
argument_list|,
name|ctrl
argument_list|)
expr_stmt|;
if|if
condition|(
name|ctrl
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|!=
name|EINTR
condition|)
name|syslog
argument_list|(
name|LOG_WARNING
argument_list|,
literal|"accept (for %s): %m"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|)
expr_stmt|;
continue|continue;
block|}
comment|/* 			     * Call tcpmux to find the real service to exec. 			     */
if|if
condition|(
name|sep
operator|->
name|se_bi
operator|&&
name|sep
operator|->
name|se_bi
operator|->
name|bi_fn
operator|==
operator|(
name|int
argument_list|(
operator|*
argument_list|)
argument_list|()
operator|)
name|tcpmux
condition|)
block|{
name|sep
operator|=
name|tcpmux
argument_list|(
name|ctrl
argument_list|)
expr_stmt|;
if|if
condition|(
name|sep
operator|==
name|NULL
condition|)
block|{
name|close
argument_list|(
name|ctrl
argument_list|)
expr_stmt|;
continue|continue;
block|}
block|}
block|}
else|else
name|ctrl
operator|=
name|sep
operator|->
name|se_fd
expr_stmt|;
operator|(
name|void
operator|)
name|sigblock
argument_list|(
name|SIGBLOCK
argument_list|)
expr_stmt|;
name|pid
operator|=
literal|0
expr_stmt|;
name|dofork
operator|=
operator|(
name|sep
operator|->
name|se_bi
operator|==
literal|0
operator|||
name|sep
operator|->
name|se_bi
operator|->
name|bi_fork
operator|)
expr_stmt|;
if|if
condition|(
name|dofork
condition|)
block|{
if|if
condition|(
name|sep
operator|->
name|se_count
operator|++
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|gettimeofday
argument_list|(
operator|&
name|sep
operator|->
name|se_time
argument_list|,
operator|(
expr|struct
name|timezone
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|sep
operator|->
name|se_count
operator|>=
name|toomany
condition|)
block|{
name|struct
name|timeval
name|now
decl_stmt|;
operator|(
name|void
operator|)
name|gettimeofday
argument_list|(
operator|&
name|now
argument_list|,
operator|(
expr|struct
name|timezone
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|now
operator|.
name|tv_sec
operator|-
name|sep
operator|->
name|se_time
operator|.
name|tv_sec
operator|>
name|CNT_INTVL
condition|)
block|{
name|sep
operator|->
name|se_time
operator|=
name|now
expr_stmt|;
name|sep
operator|->
name|se_count
operator|=
literal|1
expr_stmt|;
block|}
else|else
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s/%s server failing (looping), service terminated"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|)
expr_stmt|;
name|close_sep
argument_list|(
name|sep
argument_list|)
expr_stmt|;
name|sigsetmask
argument_list|(
literal|0L
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|timingout
condition|)
block|{
name|timingout
operator|=
literal|1
expr_stmt|;
name|alarm
argument_list|(
name|RETRYTIME
argument_list|)
expr_stmt|;
block|}
continue|continue;
block|}
block|}
name|pid
operator|=
name|fork
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
name|pid
operator|<
literal|0
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"fork: %m"
argument_list|)
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_socktype
operator|==
name|SOCK_STREAM
condition|)
name|close
argument_list|(
name|ctrl
argument_list|)
expr_stmt|;
name|sigsetmask
argument_list|(
literal|0L
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|pid
operator|&&
name|sep
operator|->
name|se_wait
condition|)
block|{
name|sep
operator|->
name|se_wait
operator|=
name|pid
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|>=
literal|0
condition|)
block|{
name|FD_CLR
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
operator|&
name|allsock
argument_list|)
expr_stmt|;
name|nsock
operator|--
expr_stmt|;
block|}
block|}
name|sigsetmask
argument_list|(
literal|0L
argument_list|)
expr_stmt|;
if|if
condition|(
name|pid
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|debug
operator|&&
name|dofork
condition|)
name|setsid
argument_list|()
expr_stmt|;
if|if
condition|(
name|dofork
condition|)
block|{
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"+ Closing from %d\n"
argument_list|,
name|maxsock
argument_list|)
expr_stmt|;
for|for
control|(
name|tmpint
operator|=
name|maxsock
init|;
name|tmpint
operator|>
literal|2
condition|;
name|tmpint
operator|--
control|)
if|if
condition|(
name|tmpint
operator|!=
name|ctrl
condition|)
name|close
argument_list|(
name|tmpint
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|sep
operator|->
name|se_bi
condition|)
call|(
modifier|*
name|sep
operator|->
name|se_bi
operator|->
name|bi_fn
call|)
argument_list|(
name|ctrl
argument_list|,
name|sep
argument_list|)
expr_stmt|;
else|else
block|{
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%d execl %s\n"
argument_list|,
name|getpid
argument_list|()
argument_list|,
name|sep
operator|->
name|se_server
argument_list|)
expr_stmt|;
name|dup2
argument_list|(
name|ctrl
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|ctrl
argument_list|)
expr_stmt|;
name|dup2
argument_list|(
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|dup2
argument_list|(
literal|0
argument_list|,
literal|2
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|pwd
operator|=
name|getpwnam
argument_list|(
name|sep
operator|->
name|se_user
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s/%s: %s: No such user"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|,
name|sep
operator|->
name|se_user
argument_list|)
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_socktype
operator|!=
name|SOCK_STREAM
condition|)
name|recv
argument_list|(
literal|0
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|_exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|pwd
operator|->
name|pw_uid
condition|)
block|{
if|if
condition|(
name|setgid
argument_list|(
name|pwd
operator|->
name|pw_gid
argument_list|)
operator|<
literal|0
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: can't set gid %d: %m"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|pwd
operator|->
name|pw_gid
argument_list|)
expr_stmt|;
name|_exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|initgroups
argument_list|(
name|pwd
operator|->
name|pw_name
argument_list|,
name|pwd
operator|->
name|pw_gid
argument_list|)
expr_stmt|;
if|if
condition|(
name|setuid
argument_list|(
name|pwd
operator|->
name|pw_uid
argument_list|)
operator|<
literal|0
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: can't set uid %d: %m"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|pwd
operator|->
name|pw_uid
argument_list|)
expr_stmt|;
name|_exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
name|execv
argument_list|(
name|sep
operator|->
name|se_server
argument_list|,
name|sep
operator|->
name|se_argv
argument_list|)
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_socktype
operator|!=
name|SOCK_STREAM
condition|)
name|recv
argument_list|(
literal|0
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"cannot execute %s: %m"
argument_list|,
name|sep
operator|->
name|se_server
argument_list|)
expr_stmt|;
name|_exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|sep
operator|->
name|se_socktype
operator|==
name|SOCK_STREAM
condition|)
name|close
argument_list|(
name|ctrl
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_block

begin_function
name|void
name|reapchild
parameter_list|()
block|{
name|int
name|status
decl_stmt|;
name|int
name|pid
decl_stmt|;
specifier|register
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|pid
operator|=
name|wait3
argument_list|(
operator|&
name|status
argument_list|,
name|WNOHANG
argument_list|,
operator|(
expr|struct
name|rusage
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|pid
operator|<=
literal|0
condition|)
break|break;
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%d reaped, status %#x\n"
argument_list|,
name|pid
argument_list|,
name|status
argument_list|)
expr_stmt|;
for|for
control|(
name|sep
operator|=
name|servtab
init|;
name|sep
condition|;
name|sep
operator|=
name|sep
operator|->
name|se_next
control|)
if|if
condition|(
name|sep
operator|->
name|se_wait
operator|==
name|pid
condition|)
block|{
if|if
condition|(
name|status
condition|)
name|syslog
argument_list|(
name|LOG_WARNING
argument_list|,
literal|"%s: exit status 0x%x"
argument_list|,
name|sep
operator|->
name|se_server
argument_list|,
name|status
argument_list|)
expr_stmt|;
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"restored %s, fd %d\n"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_fd
argument_list|)
expr_stmt|;
name|FD_SET
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
operator|&
name|allsock
argument_list|)
expr_stmt|;
name|nsock
operator|++
expr_stmt|;
name|sep
operator|->
name|se_wait
operator|=
literal|1
expr_stmt|;
block|}
block|}
block|}
end_function

begin_function
name|void
name|config
parameter_list|()
block|{
specifier|register
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|,
modifier|*
name|cp
decl_stmt|,
modifier|*
modifier|*
name|sepp
decl_stmt|;
name|struct
name|servtab
modifier|*
name|getconfigent
argument_list|()
decl_stmt|,
modifier|*
name|enter
argument_list|()
decl_stmt|;
name|struct
name|passwd
modifier|*
name|pwd
decl_stmt|;
name|long
name|omask
decl_stmt|;
if|if
condition|(
operator|!
name|setconfig
argument_list|()
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: %m"
argument_list|,
name|CONFIG
argument_list|)
expr_stmt|;
return|return;
block|}
for|for
control|(
name|sep
operator|=
name|servtab
init|;
name|sep
condition|;
name|sep
operator|=
name|sep
operator|->
name|se_next
control|)
name|sep
operator|->
name|se_checked
operator|=
literal|0
expr_stmt|;
while|while
condition|(
name|cp
operator|=
name|getconfigent
argument_list|()
condition|)
block|{
if|if
condition|(
operator|(
name|pwd
operator|=
name|getpwnam
argument_list|(
name|cp
operator|->
name|se_user
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s/%s: No such user '%s', service ignored"
argument_list|,
name|cp
operator|->
name|se_service
argument_list|,
name|cp
operator|->
name|se_proto
argument_list|,
name|cp
operator|->
name|se_user
argument_list|)
expr_stmt|;
continue|continue;
block|}
for|for
control|(
name|sep
operator|=
name|servtab
init|;
name|sep
condition|;
name|sep
operator|=
name|sep
operator|->
name|se_next
control|)
if|if
condition|(
name|strcmp
argument_list|(
name|sep
operator|->
name|se_service
argument_list|,
name|cp
operator|->
name|se_service
argument_list|)
operator|==
literal|0
operator|&&
name|strcmp
argument_list|(
name|sep
operator|->
name|se_proto
argument_list|,
name|cp
operator|->
name|se_proto
argument_list|)
operator|==
literal|0
condition|)
break|break;
if|if
condition|(
name|sep
operator|!=
literal|0
condition|)
block|{
name|int
name|i
decl_stmt|;
name|omask
operator|=
name|sigblock
argument_list|(
name|SIGBLOCK
argument_list|)
expr_stmt|;
comment|/* 			 * sep->se_wait may be holding the pid of a daemon 			 * that we're waiting for.  If so, don't overwrite 			 * it unless the config file explicitly says don't  			 * wait. 			 */
if|if
condition|(
name|cp
operator|->
name|se_bi
operator|==
literal|0
operator|&&
operator|(
name|sep
operator|->
name|se_wait
operator|==
literal|1
operator|||
name|cp
operator|->
name|se_wait
operator|==
literal|0
operator|)
condition|)
name|sep
operator|->
name|se_wait
operator|=
name|cp
operator|->
name|se_wait
expr_stmt|;
define|#
directive|define
name|SWAP
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|{ char *c = a; a = b; b = c; }
if|if
condition|(
name|cp
operator|->
name|se_user
condition|)
name|SWAP
argument_list|(
name|sep
operator|->
name|se_user
argument_list|,
name|cp
operator|->
name|se_user
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
operator|->
name|se_server
condition|)
name|SWAP
argument_list|(
name|sep
operator|->
name|se_server
argument_list|,
name|cp
operator|->
name|se_server
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|MAXARGV
condition|;
name|i
operator|++
control|)
name|SWAP
argument_list|(
name|sep
operator|->
name|se_argv
index|[
name|i
index|]
argument_list|,
name|cp
operator|->
name|se_argv
index|[
name|i
index|]
argument_list|)
expr_stmt|;
name|sigsetmask
argument_list|(
name|omask
argument_list|)
expr_stmt|;
name|freeconfig
argument_list|(
name|cp
argument_list|)
expr_stmt|;
if|if
condition|(
name|debug
condition|)
name|print_service
argument_list|(
literal|"REDO"
argument_list|,
name|sep
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|sep
operator|=
name|enter
argument_list|(
name|cp
argument_list|)
expr_stmt|;
if|if
condition|(
name|debug
condition|)
name|print_service
argument_list|(
literal|"ADD "
argument_list|,
name|sep
argument_list|)
expr_stmt|;
block|}
name|sep
operator|->
name|se_checked
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|ISMUX
argument_list|(
name|sep
argument_list|)
condition|)
block|{
name|sep
operator|->
name|se_fd
operator|=
operator|-
literal|1
expr_stmt|;
continue|continue;
block|}
name|sp
operator|=
name|getservbyname
argument_list|(
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|)
expr_stmt|;
if|if
condition|(
name|sp
operator|==
literal|0
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s/%s: unknown service"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|)
expr_stmt|;
name|sep
operator|->
name|se_checked
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|sp
operator|->
name|s_port
operator|!=
name|sep
operator|->
name|se_ctrladdr
operator|.
name|sin_port
condition|)
block|{
name|sep
operator|->
name|se_ctrladdr
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|sep
operator|->
name|se_ctrladdr
operator|.
name|sin_port
operator|=
name|sp
operator|->
name|s_port
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|>=
literal|0
condition|)
name|close_sep
argument_list|(
name|sep
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|==
operator|-
literal|1
condition|)
name|setup
argument_list|(
name|sep
argument_list|)
expr_stmt|;
block|}
name|endconfig
argument_list|()
expr_stmt|;
comment|/* 	 * Purge anything not looked at above. 	 */
name|omask
operator|=
name|sigblock
argument_list|(
name|SIGBLOCK
argument_list|)
expr_stmt|;
name|sepp
operator|=
operator|&
name|servtab
expr_stmt|;
while|while
condition|(
name|sep
operator|=
operator|*
name|sepp
condition|)
block|{
if|if
condition|(
name|sep
operator|->
name|se_checked
condition|)
block|{
name|sepp
operator|=
operator|&
name|sep
operator|->
name|se_next
expr_stmt|;
continue|continue;
block|}
operator|*
name|sepp
operator|=
name|sep
operator|->
name|se_next
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|>=
literal|0
condition|)
name|close_sep
argument_list|(
name|sep
argument_list|)
expr_stmt|;
if|if
condition|(
name|debug
condition|)
name|print_service
argument_list|(
literal|"FREE"
argument_list|,
name|sep
argument_list|)
expr_stmt|;
name|freeconfig
argument_list|(
name|sep
argument_list|)
expr_stmt|;
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
name|sep
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|sigsetmask
argument_list|(
name|omask
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|retry
parameter_list|()
block|{
specifier|register
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
name|timingout
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|sep
operator|=
name|servtab
init|;
name|sep
condition|;
name|sep
operator|=
name|sep
operator|->
name|se_next
control|)
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|==
operator|-
literal|1
condition|)
name|setup
argument_list|(
name|sep
argument_list|)
expr_stmt|;
block|}
end_function

begin_expr_stmt
name|setup
argument_list|(
name|sep
argument_list|)
specifier|register
expr|struct
name|servtab
operator|*
name|sep
expr_stmt|;
end_expr_stmt

begin_block
block|{
name|int
name|on
init|=
literal|1
decl_stmt|;
if|if
condition|(
operator|(
name|sep
operator|->
name|se_fd
operator|=
name|socket
argument_list|(
name|AF_INET
argument_list|,
name|sep
operator|->
name|se_socktype
argument_list|,
literal|0
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"socket failed on %s/%s: %s\n"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s/%s: socket: %m"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|)
expr_stmt|;
return|return;
block|}
define|#
directive|define
name|turnon
parameter_list|(
name|fd
parameter_list|,
name|opt
parameter_list|)
define|\
value|setsockopt(fd, SOL_SOCKET, opt, (char *)&on, sizeof (on))
if|if
condition|(
name|strcmp
argument_list|(
name|sep
operator|->
name|se_proto
argument_list|,
literal|"tcp"
argument_list|)
operator|==
literal|0
operator|&&
operator|(
name|options
operator|&
name|SO_DEBUG
operator|)
operator|&&
name|turnon
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
name|SO_DEBUG
argument_list|)
operator|<
literal|0
condition|)
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"setsockopt (SO_DEBUG): %m"
argument_list|)
expr_stmt|;
if|if
condition|(
name|turnon
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
name|SO_REUSEADDR
argument_list|)
operator|<
literal|0
condition|)
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"setsockopt (SO_REUSEADDR): %m"
argument_list|)
expr_stmt|;
undef|#
directive|undef
name|turnon
if|if
condition|(
name|bind
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|sep
operator|->
name|se_ctrladdr
argument_list|,
sizeof|sizeof
argument_list|(
name|sep
operator|->
name|se_ctrladdr
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bind failed on %s/%s: %s\n"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s/%s: bind: %m"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|)
expr_stmt|;
name|sep
operator|->
name|se_fd
operator|=
operator|-
literal|1
expr_stmt|;
if|if
condition|(
operator|!
name|timingout
condition|)
block|{
name|timingout
operator|=
literal|1
expr_stmt|;
name|alarm
argument_list|(
name|RETRYTIME
argument_list|)
expr_stmt|;
block|}
return|return;
block|}
if|if
condition|(
name|sep
operator|->
name|se_socktype
operator|==
name|SOCK_STREAM
condition|)
name|listen
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
literal|10
argument_list|)
expr_stmt|;
name|FD_SET
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
operator|&
name|allsock
argument_list|)
expr_stmt|;
name|nsock
operator|++
expr_stmt|;
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|>
name|maxsock
condition|)
name|maxsock
operator|=
name|sep
operator|->
name|se_fd
expr_stmt|;
if|if
condition|(
name|debug
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"registered %s on %d\n"
argument_list|,
name|sep
operator|->
name|se_server
argument_list|,
name|sep
operator|->
name|se_fd
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Finish with a service and its socket.  */
end_comment

begin_expr_stmt
name|close_sep
argument_list|(
name|sep
argument_list|)
specifier|register
expr|struct
name|servtab
operator|*
name|sep
expr_stmt|;
end_expr_stmt

begin_block
block|{
if|if
condition|(
name|sep
operator|->
name|se_fd
operator|>=
literal|0
condition|)
block|{
name|nsock
operator|--
expr_stmt|;
name|FD_CLR
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|,
operator|&
name|allsock
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
name|sep
operator|->
name|se_fd
argument_list|)
expr_stmt|;
name|sep
operator|->
name|se_fd
operator|=
operator|-
literal|1
expr_stmt|;
block|}
name|sep
operator|->
name|se_count
operator|=
literal|0
expr_stmt|;
comment|/* 	 * Don't keep the pid of this running deamon: when reapchild() 	 * reaps this pid, it would erroneously increment nsock. 	 */
if|if
condition|(
name|sep
operator|->
name|se_wait
operator|>
literal|1
condition|)
name|sep
operator|->
name|se_wait
operator|=
literal|1
expr_stmt|;
block|}
end_block

begin_function
name|struct
name|servtab
modifier|*
name|enter
parameter_list|(
name|cp
parameter_list|)
name|struct
name|servtab
modifier|*
name|cp
decl_stmt|;
block|{
specifier|register
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
name|long
name|omask
decl_stmt|;
name|sep
operator|=
operator|(
expr|struct
name|servtab
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|sep
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sep
operator|==
operator|(
expr|struct
name|servtab
operator|*
operator|)
literal|0
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"Out of memory."
argument_list|)
expr_stmt|;
name|exit
argument_list|(
operator|-
literal|1
argument_list|)
expr_stmt|;
block|}
operator|*
name|sep
operator|=
operator|*
name|cp
expr_stmt|;
name|sep
operator|->
name|se_fd
operator|=
operator|-
literal|1
expr_stmt|;
name|omask
operator|=
name|sigblock
argument_list|(
name|SIGBLOCK
argument_list|)
expr_stmt|;
name|sep
operator|->
name|se_next
operator|=
name|servtab
expr_stmt|;
name|servtab
operator|=
name|sep
expr_stmt|;
name|sigsetmask
argument_list|(
name|omask
argument_list|)
expr_stmt|;
return|return
operator|(
name|sep
operator|)
return|;
block|}
end_function

begin_decl_stmt
name|FILE
modifier|*
name|fconfig
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
name|serv
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|line
index|[
literal|256
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|sskip
argument_list|()
decl_stmt|,
modifier|*
name|skip
argument_list|()
decl_stmt|,
modifier|*
name|nextline
argument_list|()
decl_stmt|;
end_decl_stmt

begin_macro
name|setconfig
argument_list|()
end_macro

begin_block
block|{
if|if
condition|(
name|fconfig
operator|!=
name|NULL
condition|)
block|{
name|fseek
argument_list|(
name|fconfig
argument_list|,
literal|0L
argument_list|,
name|SEEK_SET
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
name|fconfig
operator|=
name|fopen
argument_list|(
name|CONFIG
argument_list|,
literal|"r"
argument_list|)
expr_stmt|;
return|return
operator|(
name|fconfig
operator|!=
name|NULL
operator|)
return|;
block|}
end_block

begin_macro
name|endconfig
argument_list|()
end_macro

begin_block
block|{
if|if
condition|(
name|fconfig
condition|)
block|{
operator|(
name|void
operator|)
name|fclose
argument_list|(
name|fconfig
argument_list|)
expr_stmt|;
name|fconfig
operator|=
name|NULL
expr_stmt|;
block|}
block|}
end_block

begin_function
name|struct
name|servtab
modifier|*
name|getconfigent
parameter_list|()
block|{
specifier|register
name|struct
name|servtab
modifier|*
name|sep
init|=
operator|&
name|serv
decl_stmt|;
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|cp
decl_stmt|,
modifier|*
name|arg
decl_stmt|,
modifier|*
name|newstr
argument_list|()
decl_stmt|;
specifier|static
name|char
name|TCPMUX_TOKEN
index|[]
init|=
literal|"tcpmux/"
decl_stmt|;
define|#
directive|define
name|MUX_LEN
value|(sizeof(TCPMUX_TOKEN)-1)
name|more
label|:
while|while
condition|(
operator|(
name|cp
operator|=
name|nextline
argument_list|(
name|fconfig
argument_list|)
operator|)
operator|&&
operator|(
operator|*
name|cp
operator|==
literal|'#'
operator|||
operator|*
name|cp
operator|==
literal|'\0'
operator|)
condition|)
empty_stmt|;
if|if
condition|(
name|cp
operator|==
name|NULL
condition|)
return|return
operator|(
operator|(
expr|struct
name|servtab
operator|*
operator|)
literal|0
operator|)
return|;
comment|/* 	 * clear the static buffer, since some fields (se_ctrladdr, 	 * for example) don't get initialized here. 	 */
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
name|sep
argument_list|,
sizeof|sizeof
expr|*
name|sep
argument_list|)
expr_stmt|;
name|arg
operator|=
name|skip
argument_list|(
operator|&
name|cp
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
operator|==
name|NULL
condition|)
block|{
comment|/* got an empty line containing just blanks/tabs. */
goto|goto
name|more
goto|;
block|}
if|if
condition|(
name|strncmp
argument_list|(
name|arg
argument_list|,
name|TCPMUX_TOKEN
argument_list|,
name|MUX_LEN
argument_list|)
operator|==
literal|0
condition|)
block|{
name|char
modifier|*
name|c
init|=
name|arg
operator|+
name|MUX_LEN
decl_stmt|;
if|if
condition|(
operator|*
name|c
operator|==
literal|'+'
condition|)
block|{
name|sep
operator|->
name|se_type
operator|=
name|MUXPLUS_TYPE
expr_stmt|;
name|c
operator|++
expr_stmt|;
block|}
else|else
name|sep
operator|->
name|se_type
operator|=
name|MUX_TYPE
expr_stmt|;
name|sep
operator|->
name|se_service
operator|=
name|newstr
argument_list|(
name|c
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|sep
operator|->
name|se_service
operator|=
name|newstr
argument_list|(
name|arg
argument_list|)
expr_stmt|;
name|sep
operator|->
name|se_type
operator|=
name|NORM_TYPE
expr_stmt|;
block|}
name|arg
operator|=
name|sskip
argument_list|(
operator|&
name|cp
argument_list|)
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|arg
argument_list|,
literal|"stream"
argument_list|)
operator|==
literal|0
condition|)
name|sep
operator|->
name|se_socktype
operator|=
name|SOCK_STREAM
expr_stmt|;
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|arg
argument_list|,
literal|"dgram"
argument_list|)
operator|==
literal|0
condition|)
name|sep
operator|->
name|se_socktype
operator|=
name|SOCK_DGRAM
expr_stmt|;
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|arg
argument_list|,
literal|"rdm"
argument_list|)
operator|==
literal|0
condition|)
name|sep
operator|->
name|se_socktype
operator|=
name|SOCK_RDM
expr_stmt|;
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|arg
argument_list|,
literal|"seqpacket"
argument_list|)
operator|==
literal|0
condition|)
name|sep
operator|->
name|se_socktype
operator|=
name|SOCK_SEQPACKET
expr_stmt|;
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|arg
argument_list|,
literal|"raw"
argument_list|)
operator|==
literal|0
condition|)
name|sep
operator|->
name|se_socktype
operator|=
name|SOCK_RAW
expr_stmt|;
else|else
name|sep
operator|->
name|se_socktype
operator|=
operator|-
literal|1
expr_stmt|;
name|sep
operator|->
name|se_proto
operator|=
name|newstr
argument_list|(
name|sskip
argument_list|(
operator|&
name|cp
argument_list|)
argument_list|)
expr_stmt|;
name|arg
operator|=
name|sskip
argument_list|(
operator|&
name|cp
argument_list|)
expr_stmt|;
name|sep
operator|->
name|se_wait
operator|=
name|strcmp
argument_list|(
name|arg
argument_list|,
literal|"wait"
argument_list|)
operator|==
literal|0
expr_stmt|;
if|if
condition|(
name|ISMUX
argument_list|(
name|sep
argument_list|)
condition|)
block|{
comment|/* 		 * Silently enforce "nowait" for TCPMUX services since 		 * they don't have an assigned port to listen on. 		 */
name|sep
operator|->
name|se_wait
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|sep
operator|->
name|se_proto
argument_list|,
literal|"tcp"
argument_list|)
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: bad protocol for tcpmux service %s"
argument_list|,
name|CONFIG
argument_list|,
name|sep
operator|->
name|se_service
argument_list|)
expr_stmt|;
goto|goto
name|more
goto|;
block|}
if|if
condition|(
name|sep
operator|->
name|se_socktype
operator|!=
name|SOCK_STREAM
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: bad socket type for tcpmux service %s"
argument_list|,
name|CONFIG
argument_list|,
name|sep
operator|->
name|se_service
argument_list|)
expr_stmt|;
goto|goto
name|more
goto|;
block|}
block|}
name|sep
operator|->
name|se_user
operator|=
name|newstr
argument_list|(
name|sskip
argument_list|(
operator|&
name|cp
argument_list|)
argument_list|)
expr_stmt|;
name|sep
operator|->
name|se_server
operator|=
name|newstr
argument_list|(
name|sskip
argument_list|(
operator|&
name|cp
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|sep
operator|->
name|se_server
argument_list|,
literal|"internal"
argument_list|)
operator|==
literal|0
condition|)
block|{
specifier|register
name|struct
name|biltin
modifier|*
name|bi
decl_stmt|;
for|for
control|(
name|bi
operator|=
name|biltins
init|;
name|bi
operator|->
name|bi_service
condition|;
name|bi
operator|++
control|)
if|if
condition|(
name|bi
operator|->
name|bi_socktype
operator|==
name|sep
operator|->
name|se_socktype
operator|&&
name|strcmp
argument_list|(
name|bi
operator|->
name|bi_service
argument_list|,
name|sep
operator|->
name|se_service
argument_list|)
operator|==
literal|0
condition|)
break|break;
if|if
condition|(
name|bi
operator|->
name|bi_service
operator|==
literal|0
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"internal service %s unknown"
argument_list|,
name|sep
operator|->
name|se_service
argument_list|)
expr_stmt|;
goto|goto
name|more
goto|;
block|}
name|sep
operator|->
name|se_bi
operator|=
name|bi
expr_stmt|;
name|sep
operator|->
name|se_wait
operator|=
name|bi
operator|->
name|bi_wait
expr_stmt|;
block|}
else|else
name|sep
operator|->
name|se_bi
operator|=
name|NULL
expr_stmt|;
name|argc
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|arg
operator|=
name|skip
argument_list|(
operator|&
name|cp
argument_list|)
init|;
name|cp
condition|;
name|arg
operator|=
name|skip
argument_list|(
operator|&
name|cp
argument_list|)
control|)
if|if
condition|(
name|argc
operator|<
name|MAXARGV
condition|)
name|sep
operator|->
name|se_argv
index|[
name|argc
operator|++
index|]
operator|=
name|newstr
argument_list|(
name|arg
argument_list|)
expr_stmt|;
while|while
condition|(
name|argc
operator|<=
name|MAXARGV
condition|)
name|sep
operator|->
name|se_argv
index|[
name|argc
operator|++
index|]
operator|=
name|NULL
expr_stmt|;
return|return
operator|(
name|sep
operator|)
return|;
block|}
end_function

begin_expr_stmt
name|freeconfig
argument_list|(
name|cp
argument_list|)
specifier|register
expr|struct
name|servtab
operator|*
name|cp
expr_stmt|;
end_expr_stmt

begin_block
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
name|cp
operator|->
name|se_service
condition|)
name|free
argument_list|(
name|cp
operator|->
name|se_service
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
operator|->
name|se_proto
condition|)
name|free
argument_list|(
name|cp
operator|->
name|se_proto
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
operator|->
name|se_user
condition|)
name|free
argument_list|(
name|cp
operator|->
name|se_user
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
operator|->
name|se_server
condition|)
name|free
argument_list|(
name|cp
operator|->
name|se_server
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|MAXARGV
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|cp
operator|->
name|se_argv
index|[
name|i
index|]
condition|)
name|free
argument_list|(
name|cp
operator|->
name|se_argv
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Safe skip - if skip returns null, log a syntax error in the  * configuration file and exit.  */
end_comment

begin_function
name|char
modifier|*
name|sskip
parameter_list|(
name|cpp
parameter_list|)
name|char
modifier|*
modifier|*
name|cpp
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|cp
decl_stmt|;
name|cp
operator|=
name|skip
argument_list|(
name|cpp
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
operator|==
name|NULL
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"%s: syntax error"
argument_list|,
name|CONFIG
argument_list|)
expr_stmt|;
name|exit
argument_list|(
operator|-
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|cp
operator|)
return|;
block|}
end_function

begin_function
name|char
modifier|*
name|skip
parameter_list|(
name|cpp
parameter_list|)
name|char
modifier|*
modifier|*
name|cpp
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|cp
init|=
operator|*
name|cpp
decl_stmt|;
name|char
modifier|*
name|start
decl_stmt|;
name|again
label|:
while|while
condition|(
operator|*
name|cp
operator|==
literal|' '
operator|||
operator|*
name|cp
operator|==
literal|'\t'
condition|)
name|cp
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|cp
operator|==
literal|'\0'
condition|)
block|{
name|int
name|c
decl_stmt|;
name|c
operator|=
name|getc
argument_list|(
name|fconfig
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|ungetc
argument_list|(
name|c
argument_list|,
name|fconfig
argument_list|)
expr_stmt|;
if|if
condition|(
name|c
operator|==
literal|' '
operator|||
name|c
operator|==
literal|'\t'
condition|)
if|if
condition|(
name|cp
operator|=
name|nextline
argument_list|(
name|fconfig
argument_list|)
condition|)
goto|goto
name|again
goto|;
operator|*
name|cpp
operator|=
operator|(
name|char
operator|*
operator|)
literal|0
expr_stmt|;
return|return
operator|(
operator|(
name|char
operator|*
operator|)
literal|0
operator|)
return|;
block|}
name|start
operator|=
name|cp
expr_stmt|;
while|while
condition|(
operator|*
name|cp
operator|&&
operator|*
name|cp
operator|!=
literal|' '
operator|&&
operator|*
name|cp
operator|!=
literal|'\t'
condition|)
name|cp
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|cp
operator|!=
literal|'\0'
condition|)
operator|*
name|cp
operator|++
operator|=
literal|'\0'
expr_stmt|;
operator|*
name|cpp
operator|=
name|cp
expr_stmt|;
return|return
operator|(
name|start
operator|)
return|;
block|}
end_function

begin_function
name|char
modifier|*
name|nextline
parameter_list|(
name|fd
parameter_list|)
name|FILE
modifier|*
name|fd
decl_stmt|;
block|{
name|char
modifier|*
name|cp
decl_stmt|;
if|if
condition|(
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|fd
argument_list|)
operator|==
name|NULL
condition|)
return|return
operator|(
operator|(
name|char
operator|*
operator|)
literal|0
operator|)
return|;
name|cp
operator|=
name|index
argument_list|(
name|line
argument_list|,
literal|'\n'
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
condition|)
operator|*
name|cp
operator|=
literal|'\0'
expr_stmt|;
return|return
operator|(
name|line
operator|)
return|;
block|}
end_function

begin_function
name|char
modifier|*
name|newstr
parameter_list|(
name|cp
parameter_list|)
name|char
modifier|*
name|cp
decl_stmt|;
block|{
if|if
condition|(
name|cp
operator|=
name|strdup
argument_list|(
name|cp
condition|?
name|cp
else|:
literal|""
argument_list|)
condition|)
return|return
operator|(
name|cp
operator|)
return|;
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"strdup: %m"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
operator|-
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_macro
name|setproctitle
argument_list|(
argument|a
argument_list|,
argument|s
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|a
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|size
decl_stmt|;
specifier|register
name|char
modifier|*
name|cp
decl_stmt|;
name|struct
name|sockaddr_in
name|sin
decl_stmt|;
name|char
name|buf
index|[
literal|80
index|]
decl_stmt|;
name|cp
operator|=
name|Argv
index|[
literal|0
index|]
expr_stmt|;
name|size
operator|=
sizeof|sizeof
argument_list|(
name|sin
argument_list|)
expr_stmt|;
if|if
condition|(
name|getpeername
argument_list|(
name|s
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|sin
argument_list|,
operator|&
name|size
argument_list|)
operator|==
literal|0
condition|)
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"-%s [%s]"
argument_list|,
name|a
argument_list|,
name|inet_ntoa
argument_list|(
name|sin
operator|.
name|sin_addr
argument_list|)
argument_list|)
expr_stmt|;
else|else
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"-%s"
argument_list|,
name|a
argument_list|)
expr_stmt|;
name|strncpy
argument_list|(
name|cp
argument_list|,
name|buf
argument_list|,
name|LastArg
operator|-
name|cp
argument_list|)
expr_stmt|;
name|cp
operator|+=
name|strlen
argument_list|(
name|cp
argument_list|)
expr_stmt|;
while|while
condition|(
name|cp
operator|<
name|LastArg
condition|)
operator|*
name|cp
operator|++
operator|=
literal|' '
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Internet services provided internally by inetd:  */
end_comment

begin_define
define|#
directive|define
name|BUFSIZE
value|8192
end_define

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|echo_stream
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Echo service -- echo data back */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
name|buffer
index|[
name|BUFSIZE
index|]
decl_stmt|;
name|int
name|i
decl_stmt|;
name|setproctitle
argument_list|(
name|sep
operator|->
name|se_service
argument_list|,
name|s
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|i
operator|=
name|read
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
sizeof|sizeof
argument_list|(
name|buffer
argument_list|)
argument_list|)
operator|)
operator|>
literal|0
operator|&&
name|write
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
name|i
argument_list|)
operator|>
literal|0
condition|)
empty_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|echo_dg
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Echo service -- echo data back */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
name|buffer
index|[
name|BUFSIZE
index|]
decl_stmt|;
name|int
name|i
decl_stmt|,
name|size
decl_stmt|;
name|struct
name|sockaddr
name|sa
decl_stmt|;
name|size
operator|=
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|i
operator|=
name|recvfrom
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
sizeof|sizeof
argument_list|(
name|buffer
argument_list|)
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
operator|&
name|size
argument_list|)
operator|)
operator|<
literal|0
condition|)
return|return;
operator|(
name|void
operator|)
name|sendto
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
name|i
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|discard_stream
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Discard service -- ignore data */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|ret
decl_stmt|;
name|char
name|buffer
index|[
name|BUFSIZE
index|]
decl_stmt|;
name|setproctitle
argument_list|(
name|sep
operator|->
name|se_service
argument_list|,
name|s
argument_list|)
expr_stmt|;
while|while
condition|(
literal|1
condition|)
block|{
while|while
condition|(
operator|(
name|ret
operator|=
name|read
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
sizeof|sizeof
argument_list|(
name|buffer
argument_list|)
argument_list|)
operator|)
operator|>
literal|0
condition|)
empty_stmt|;
if|if
condition|(
name|ret
operator|==
literal|0
operator|||
name|errno
operator|!=
name|EINTR
condition|)
break|break;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|discard_dg
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Discard service -- ignore data */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
name|buffer
index|[
name|BUFSIZE
index|]
decl_stmt|;
operator|(
name|void
operator|)
name|read
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
sizeof|sizeof
argument_list|(
name|buffer
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_define
define|#
directive|define
name|LINESIZ
value|72
end_define

begin_decl_stmt
name|char
name|ring
index|[
literal|128
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|endring
decl_stmt|;
end_decl_stmt

begin_macro
name|initring
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
name|endring
operator|=
name|ring
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<=
literal|128
condition|;
operator|++
name|i
control|)
if|if
condition|(
name|isprint
argument_list|(
name|i
argument_list|)
condition|)
operator|*
name|endring
operator|++
operator|=
name|i
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|chargen_stream
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Character generator */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
modifier|*
name|rs
decl_stmt|;
name|int
name|len
decl_stmt|;
name|char
name|text
index|[
name|LINESIZ
operator|+
literal|2
index|]
decl_stmt|;
name|setproctitle
argument_list|(
name|sep
operator|->
name|se_service
argument_list|,
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|endring
condition|)
block|{
name|initring
argument_list|()
expr_stmt|;
name|rs
operator|=
name|ring
expr_stmt|;
block|}
name|text
index|[
name|LINESIZ
index|]
operator|=
literal|'\r'
expr_stmt|;
name|text
index|[
name|LINESIZ
operator|+
literal|1
index|]
operator|=
literal|'\n'
expr_stmt|;
for|for
control|(
name|rs
operator|=
name|ring
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|(
name|len
operator|=
name|endring
operator|-
name|rs
operator|)
operator|>=
name|LINESIZ
condition|)
name|bcopy
argument_list|(
name|rs
argument_list|,
name|text
argument_list|,
name|LINESIZ
argument_list|)
expr_stmt|;
else|else
block|{
name|bcopy
argument_list|(
name|rs
argument_list|,
name|text
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
name|ring
argument_list|,
name|text
operator|+
name|len
argument_list|,
name|LINESIZ
operator|-
name|len
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|++
name|rs
operator|==
name|endring
condition|)
name|rs
operator|=
name|ring
expr_stmt|;
if|if
condition|(
name|write
argument_list|(
name|s
argument_list|,
name|text
argument_list|,
sizeof|sizeof
argument_list|(
name|text
argument_list|)
argument_list|)
operator|!=
sizeof|sizeof
argument_list|(
name|text
argument_list|)
condition|)
break|break;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|chargen_dg
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Character generator */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|sockaddr
name|sa
decl_stmt|;
specifier|static
name|char
modifier|*
name|rs
decl_stmt|;
name|int
name|len
decl_stmt|,
name|size
decl_stmt|;
name|char
name|text
index|[
name|LINESIZ
operator|+
literal|2
index|]
decl_stmt|;
if|if
condition|(
name|endring
operator|==
literal|0
condition|)
block|{
name|initring
argument_list|()
expr_stmt|;
name|rs
operator|=
name|ring
expr_stmt|;
block|}
name|size
operator|=
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
expr_stmt|;
if|if
condition|(
name|recvfrom
argument_list|(
name|s
argument_list|,
name|text
argument_list|,
sizeof|sizeof
argument_list|(
name|text
argument_list|)
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
operator|&
name|size
argument_list|)
operator|<
literal|0
condition|)
return|return;
if|if
condition|(
operator|(
name|len
operator|=
name|endring
operator|-
name|rs
operator|)
operator|>=
name|LINESIZ
condition|)
name|bcopy
argument_list|(
name|rs
argument_list|,
name|text
argument_list|,
name|LINESIZ
argument_list|)
expr_stmt|;
else|else
block|{
name|bcopy
argument_list|(
name|rs
argument_list|,
name|text
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
name|ring
argument_list|,
name|text
operator|+
name|len
argument_list|,
name|LINESIZ
operator|-
name|len
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|++
name|rs
operator|==
name|endring
condition|)
name|rs
operator|=
name|ring
expr_stmt|;
name|text
index|[
name|LINESIZ
index|]
operator|=
literal|'\r'
expr_stmt|;
name|text
index|[
name|LINESIZ
operator|+
literal|1
index|]
operator|=
literal|'\n'
expr_stmt|;
operator|(
name|void
operator|)
name|sendto
argument_list|(
name|s
argument_list|,
name|text
argument_list|,
sizeof|sizeof
argument_list|(
name|text
argument_list|)
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Return a machine readable date and time, in the form of the  * number of seconds since midnight, Jan 1, 1900.  Since gettimeofday  * returns the number of seconds since midnight, Jan 1, 1970,  * we must add 2208988800 seconds to this figure to make up for  * some seventy years Bell Labs was asleep.  */
end_comment

begin_function
name|long
name|machtime
parameter_list|()
block|{
name|struct
name|timeval
name|tv
decl_stmt|;
if|if
condition|(
name|gettimeofday
argument_list|(
operator|&
name|tv
argument_list|,
operator|(
expr|struct
name|timezone
operator|*
operator|)
literal|0
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Unable to get time of day\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0L
operator|)
return|;
block|}
define|#
directive|define
name|OFFSET
value|((u_long)25567 * 24*60*60)
return|return
operator|(
name|htonl
argument_list|(
call|(
name|long
call|)
argument_list|(
name|tv
operator|.
name|tv_sec
operator|+
name|OFFSET
argument_list|)
argument_list|)
operator|)
return|;
undef|#
directive|undef
name|OFFSET
block|}
end_function

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|machtime_stream
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|long
name|result
decl_stmt|;
name|result
operator|=
name|machtime
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|write
argument_list|(
name|s
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|result
argument_list|,
sizeof|sizeof
argument_list|(
name|result
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|machtime_dg
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|long
name|result
decl_stmt|;
name|struct
name|sockaddr
name|sa
decl_stmt|;
name|int
name|size
decl_stmt|;
name|size
operator|=
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
expr_stmt|;
if|if
condition|(
name|recvfrom
argument_list|(
name|s
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|result
argument_list|,
sizeof|sizeof
argument_list|(
name|result
argument_list|)
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
operator|&
name|size
argument_list|)
operator|<
literal|0
condition|)
return|return;
name|result
operator|=
name|machtime
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|sendto
argument_list|(
name|s
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|result
argument_list|,
sizeof|sizeof
argument_list|(
name|result
argument_list|)
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|daytime_stream
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Return human-readable time of day */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
name|buffer
index|[
literal|256
index|]
decl_stmt|;
name|time_t
name|clock
decl_stmt|;
name|clock
operator|=
name|time
argument_list|(
operator|(
name|time_t
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|buffer
argument_list|,
literal|"%.24s\r\n"
argument_list|,
name|ctime
argument_list|(
operator|&
name|clock
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|write
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
name|strlen
argument_list|(
name|buffer
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|daytime_dg
argument_list|(
argument|s
argument_list|,
argument|sep
argument_list|)
end_macro

begin_comment
comment|/* Return human-readable time of day */
end_comment

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
name|buffer
index|[
literal|256
index|]
decl_stmt|;
name|time_t
name|clock
decl_stmt|;
name|struct
name|sockaddr
name|sa
decl_stmt|;
name|int
name|size
decl_stmt|;
name|clock
operator|=
name|time
argument_list|(
operator|(
name|time_t
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
name|size
operator|=
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
expr_stmt|;
if|if
condition|(
name|recvfrom
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
sizeof|sizeof
argument_list|(
name|buffer
argument_list|)
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
operator|&
name|size
argument_list|)
operator|<
literal|0
condition|)
return|return;
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|buffer
argument_list|,
literal|"%.24s\r\n"
argument_list|,
name|ctime
argument_list|(
operator|&
name|clock
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|sendto
argument_list|(
name|s
argument_list|,
name|buffer
argument_list|,
name|strlen
argument_list|(
name|buffer
argument_list|)
argument_list|,
literal|0
argument_list|,
operator|&
name|sa
argument_list|,
sizeof|sizeof
argument_list|(
name|sa
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * print_service:  *	Dump relevant information to stderr  */
end_comment

begin_macro
name|print_service
argument_list|(
argument|action
argument_list|,
argument|sep
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|action
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s proto=%s, wait=%d, user=%s builtin=%x server=%s\n"
argument_list|,
name|action
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|sep
operator|->
name|se_proto
argument_list|,
name|sep
operator|->
name|se_wait
argument_list|,
name|sep
operator|->
name|se_user
argument_list|,
operator|(
name|int
operator|)
name|sep
operator|->
name|se_bi
argument_list|,
name|sep
operator|->
name|se_server
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  *  Based on TCPMUX.C by Mark K. Lottor November 1988  *  sri-nic::ps:<mkl>tcpmux.c  */
end_comment

begin_function
specifier|static
name|int
comment|/* # of characters upto \r,\n or \0 */
name|getline
parameter_list|(
name|fd
parameter_list|,
name|buf
parameter_list|,
name|len
parameter_list|)
name|int
name|fd
decl_stmt|;
name|char
modifier|*
name|buf
decl_stmt|;
name|int
name|len
decl_stmt|;
block|{
name|int
name|count
init|=
literal|0
decl_stmt|,
name|n
decl_stmt|;
do|do
block|{
name|n
operator|=
name|read
argument_list|(
name|fd
argument_list|,
name|buf
argument_list|,
name|len
operator|-
name|count
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
return|return
name|count
return|;
if|if
condition|(
name|n
operator|<
literal|0
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
while|while
condition|(
operator|--
name|n
operator|>=
literal|0
condition|)
block|{
if|if
condition|(
operator|*
name|buf
operator|==
literal|'\r'
operator|||
operator|*
name|buf
operator|==
literal|'\n'
operator|||
operator|*
name|buf
operator|==
literal|'\0'
condition|)
return|return
name|count
return|;
name|count
operator|++
expr_stmt|;
name|buf
operator|++
expr_stmt|;
block|}
block|}
do|while
condition|(
name|count
operator|<
name|len
condition|)
do|;
return|return
operator|(
name|count
operator|)
return|;
block|}
end_function

begin_define
define|#
directive|define
name|MAX_SERV_LEN
value|(256+2)
end_define

begin_comment
comment|/* 2 bytes for \r\n */
end_comment

begin_define
define|#
directive|define
name|strwrite
parameter_list|(
name|fd
parameter_list|,
name|buf
parameter_list|)
value|(void) write(fd, buf, sizeof(buf)-1)
end_define

begin_function
name|struct
name|servtab
modifier|*
name|tcpmux
parameter_list|(
name|s
parameter_list|)
name|int
name|s
decl_stmt|;
block|{
specifier|register
name|struct
name|servtab
modifier|*
name|sep
decl_stmt|;
name|char
name|service
index|[
name|MAX_SERV_LEN
operator|+
literal|1
index|]
decl_stmt|;
name|int
name|len
decl_stmt|;
comment|/* Get requested service name */
if|if
condition|(
operator|(
name|len
operator|=
name|getline
argument_list|(
name|s
argument_list|,
name|service
argument_list|,
name|MAX_SERV_LEN
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
name|strwrite
argument_list|(
name|s
argument_list|,
literal|"-Error reading service name\r\n"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
name|service
index|[
name|len
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|debug
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"tcpmux: someone wants %s\n"
argument_list|,
name|service
argument_list|)
expr_stmt|;
comment|/* 	 * Help is a required command, and lists available services, 	 * one per line. 	 */
if|if
condition|(
operator|!
name|strcasecmp
argument_list|(
name|service
argument_list|,
literal|"help"
argument_list|)
condition|)
block|{
for|for
control|(
name|sep
operator|=
name|servtab
init|;
name|sep
condition|;
name|sep
operator|=
name|sep
operator|->
name|se_next
control|)
block|{
if|if
condition|(
operator|!
name|ISMUX
argument_list|(
name|sep
argument_list|)
condition|)
continue|continue;
operator|(
name|void
operator|)
name|write
argument_list|(
name|s
argument_list|,
name|sep
operator|->
name|se_service
argument_list|,
name|strlen
argument_list|(
name|sep
operator|->
name|se_service
argument_list|)
argument_list|)
expr_stmt|;
name|strwrite
argument_list|(
name|s
argument_list|,
literal|"\r\n"
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|NULL
operator|)
return|;
block|}
comment|/* Try matching a service in inetd.conf with the request */
for|for
control|(
name|sep
operator|=
name|servtab
init|;
name|sep
condition|;
name|sep
operator|=
name|sep
operator|->
name|se_next
control|)
block|{
if|if
condition|(
operator|!
name|ISMUX
argument_list|(
name|sep
argument_list|)
condition|)
continue|continue;
if|if
condition|(
operator|!
name|strcasecmp
argument_list|(
name|service
argument_list|,
name|sep
operator|->
name|se_service
argument_list|)
condition|)
block|{
if|if
condition|(
name|ISMUXPLUS
argument_list|(
name|sep
argument_list|)
condition|)
block|{
name|strwrite
argument_list|(
name|s
argument_list|,
literal|"+Go\r\n"
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|sep
operator|)
return|;
block|}
block|}
name|strwrite
argument_list|(
name|s
argument_list|,
literal|"-Service not available\r\n"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
end_function

end_unit

