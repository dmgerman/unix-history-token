...
index|[]
init|=
literal|"@(#) Copyright (c) 1980, 1991, 1993 The Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt
...
index|[]
init|=
literal|"@(#)pstat.c	5.41 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
parameter_list|)
define|\
value|if (kvm_read(kd, (u_long)(addr), p, s) != s) \ 		error("cannot read %s: %s", msg, kvm_geterr(kd))
end_define
...
parameter_list|)
define|\
value|if (kvm_read(kd, (u_long)(addr), p, s) != s) { \ 		error("cannot read %s: %s", msg, kvm_geterr(kd)); \ 		return (0); \ 	}
end_define
...
name|stderr
argument_list|,
literal|"usage: pstat -Tfnstv [system] [core]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
begin_comment
comment|/*  * doswap is based on a program called swapinfo written  * by Kevin Lahey<kml@rokkaku.atl.ga.us>.  */
end_comment
