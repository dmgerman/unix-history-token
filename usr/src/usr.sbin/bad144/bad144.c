begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1993, 198019861988  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1993, 198019861988\n\ 	The Regents of the University of California.  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
endif|not lint
end_endif

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)bad144.c	8.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
endif|not lint
end_endif

begin_comment
comment|/*  * bad144  *  * This program prints and/or initializes a bad block record for a pack,  * in the format used by the DEC standard 144.  * It can also add bad sector(s) to the record, moving the sector  * replacements as necessary.  *  * It is preferable to write the bad information with a standard formatter,  * but this program will do.  *   * RP06 sectors are marked as bad by inverting the format bit in the  * header; on other drives the valid-sector bit is cleared.  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/dkbad.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/disklabel.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ffs/fs.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<paths.h>
end_include

begin_define
define|#
directive|define
name|RETRIES
value|10
end_define

begin_comment
comment|/* number of retries on reading old sectors */
end_comment

begin_define
define|#
directive|define
name|RAWPART
value|"c"
end_define

begin_comment
comment|/* disk partition containing badsector tables */
end_comment

begin_decl_stmt
name|int
name|fflag
decl_stmt|,
name|add
decl_stmt|,
name|copy
decl_stmt|,
name|verbose
decl_stmt|,
name|nflag
decl_stmt|;
end_decl_stmt

begin_function_decl
name|int
name|compare
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|int
name|dups
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|badfile
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* copy of badsector table to use, -1 if any */
end_comment

begin_define
define|#
directive|define
name|MAXSECSIZE
value|1024
end_define

begin_decl_stmt
name|struct
name|dkbad
name|curbad
decl_stmt|,
name|oldbad
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|DKBAD_MAGIC
value|0
end_define

begin_decl_stmt
name|char
name|label
index|[
name|BBSIZE
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|daddr_t
name|size
decl_stmt|,
name|getold
argument_list|()
decl_stmt|,
name|badsn
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|disklabel
modifier|*
name|dp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|name
index|[
name|BUFSIZ
index|]
decl_stmt|;
end_decl_stmt

begin_function_decl
name|char
modifier|*
name|malloc
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|off_t
name|lseek
parameter_list|()
function_decl|;
end_function_decl

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
specifier|register
name|struct
name|bt_bad
modifier|*
name|bt
decl_stmt|;
name|daddr_t
name|sn
decl_stmt|,
name|bn
index|[
literal|126
index|]
decl_stmt|;
name|int
name|i
decl_stmt|,
name|f
decl_stmt|,
name|nbad
decl_stmt|,
name|new
decl_stmt|,
name|bad
decl_stmt|,
name|errs
decl_stmt|;
name|argc
operator|--
operator|,
name|argv
operator|++
expr_stmt|;
while|while
condition|(
name|argc
operator|>
literal|0
operator|&&
operator|*
operator|*
name|argv
operator|==
literal|'-'
condition|)
block|{
operator|(
operator|*
name|argv
operator|)
operator|++
expr_stmt|;
while|while
condition|(
operator|*
operator|*
name|argv
condition|)
block|{
switch|switch
condition|(
operator|*
operator|*
name|argv
condition|)
block|{
if|#
directive|if
name|vax
case|case
literal|'f'
case|:
name|fflag
operator|++
expr_stmt|;
break|break;
endif|#
directive|endif
case|case
literal|'a'
case|:
name|add
operator|++
expr_stmt|;
break|break;
case|case
literal|'c'
case|:
name|copy
operator|++
expr_stmt|;
break|break;
case|case
literal|'v'
case|:
name|verbose
operator|++
expr_stmt|;
break|break;
case|case
literal|'n'
case|:
name|nflag
operator|++
expr_stmt|;
name|verbose
operator|++
expr_stmt|;
break|break;
default|default:
if|if
condition|(
operator|*
operator|*
name|argv
operator|>=
literal|'0'
operator|&&
operator|*
operator|*
name|argv
operator|<=
literal|'4'
condition|)
block|{
name|badfile
operator|=
operator|*
operator|*
name|argv
operator|-
literal|'0'
expr_stmt|;
break|break;
block|}
goto|goto
name|usage
goto|;
block|}
operator|(
operator|*
name|argv
operator|)
operator|++
expr_stmt|;
block|}
name|argc
operator|--
operator|,
name|argv
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|argc
operator|<
literal|1
condition|)
block|{
name|usage
label|:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: bad144 [ -f ] disk [ snum [ bn ... ] ]\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"to read or overwrite bad-sector table, e.g.: bad144 hp0\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"or bad144 -a [ -f ] [ -c ] disk  bn ...\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"where options are:\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\t-a  add new bad sectors to the table\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\t-f  reformat listed sectors as bad\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\t-c  copy original sector to replacement\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|argv
index|[
literal|0
index|]
index|[
literal|0
index|]
operator|!=
literal|'/'
condition|)
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|name
argument_list|,
literal|"%s/r%s%s"
argument_list|,
name|_PATH_DEV
argument_list|,
name|argv
index|[
literal|0
index|]
argument_list|,
name|RAWPART
argument_list|)
expr_stmt|;
else|else
name|strcpy
argument_list|(
name|name
argument_list|,
name|argv
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|f
operator|=
name|open
argument_list|(
name|name
argument_list|,
name|argc
operator|==
literal|1
condition|?
name|O_RDONLY
else|:
name|O_RDWR
argument_list|)
expr_stmt|;
if|if
condition|(
name|f
operator|<
literal|0
condition|)
name|Perror
argument_list|(
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
name|read
argument_list|(
name|f
argument_list|,
name|label
argument_list|,
sizeof|sizeof
argument_list|(
name|label
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
name|Perror
argument_list|(
literal|"read"
argument_list|)
expr_stmt|;
for|for
control|(
name|dp
operator|=
operator|(
expr|struct
name|disklabel
operator|*
operator|)
operator|(
name|label
operator|+
name|LABELOFFSET
operator|)
init|;
name|dp
operator|<
operator|(
expr|struct
name|disklabel
operator|*
operator|)
operator|(
name|label
operator|+
sizeof|sizeof
argument_list|(
name|label
argument_list|)
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|disklabel
argument_list|)
operator|)
condition|;
name|dp
operator|=
operator|(
expr|struct
name|disklabel
operator|*
operator|)
operator|(
operator|(
name|char
operator|*
operator|)
name|dp
operator|+
literal|64
operator|)
control|)
if|if
condition|(
name|dp
operator|->
name|d_magic
operator|==
name|DISKMAGIC
operator|&&
name|dp
operator|->
name|d_magic2
operator|==
name|DISKMAGIC
condition|)
break|break;
if|if
condition|(
name|dp
operator|->
name|d_magic
operator|!=
name|DISKMAGIC
operator|||
name|dp
operator|->
name|d_magic2
operator|!=
name|DISKMAGIC
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Bad pack magic number (pack is unlabeled)\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|dp
operator|->
name|d_secsize
operator|>
name|MAXSECSIZE
operator|||
name|dp
operator|->
name|d_secsize
operator|<=
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Disk sector size too large/small (%d)\n"
argument_list|,
name|dp
operator|->
name|d_secsize
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|7
argument_list|)
expr_stmt|;
block|}
name|size
operator|=
name|dp
operator|->
name|d_nsectors
operator|*
name|dp
operator|->
name|d_ntracks
operator|*
name|dp
operator|->
name|d_ncylinders
expr_stmt|;
name|argc
operator|--
expr_stmt|;
name|argv
operator|++
expr_stmt|;
if|if
condition|(
name|argc
operator|==
literal|0
condition|)
block|{
name|sn
operator|=
name|getold
argument_list|(
name|f
argument_list|,
operator|&
name|oldbad
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"bad block information at sector %d in %s:\n"
argument_list|,
name|sn
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"cartridge serial number: %d(10)\n"
argument_list|,
name|oldbad
operator|.
name|bt_csn
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|oldbad
operator|.
name|bt_flag
condition|)
block|{
case|case
operator|(
name|u_short
operator|)
operator|-
literal|1
case|:
name|printf
argument_list|(
literal|"alignment cartridge\n"
argument_list|)
expr_stmt|;
break|break;
case|case
name|DKBAD_MAGIC
case|:
break|break;
default|default:
name|printf
argument_list|(
literal|"bt_flag=%x(16)?\n"
argument_list|,
name|oldbad
operator|.
name|bt_flag
argument_list|)
expr_stmt|;
break|break;
block|}
name|bt
operator|=
name|oldbad
operator|.
name|bt_bad
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|126
condition|;
name|i
operator|++
control|)
block|{
name|bad
operator|=
operator|(
name|bt
operator|->
name|bt_cyl
operator|<<
literal|16
operator|)
operator|+
name|bt
operator|->
name|bt_trksec
expr_stmt|;
if|if
condition|(
name|bad
operator|<
literal|0
condition|)
break|break;
name|printf
argument_list|(
literal|"sn=%d, cn=%d, tn=%d, sn=%d\n"
argument_list|,
name|badsn
argument_list|(
name|bt
argument_list|)
argument_list|,
name|bt
operator|->
name|bt_cyl
argument_list|,
name|bt
operator|->
name|bt_trksec
operator|>>
literal|8
argument_list|,
name|bt
operator|->
name|bt_trksec
operator|&
literal|0xff
argument_list|)
expr_stmt|;
name|bt
operator|++
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|checkold
argument_list|(
operator|&
name|oldbad
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|add
condition|)
block|{
comment|/* 		 * Read in the old badsector table. 		 * Verify that it makes sense, and the bad sectors 		 * are in order.  Copy the old table to the new one. 		 */
operator|(
name|void
operator|)
name|getold
argument_list|(
name|f
argument_list|,
operator|&
name|oldbad
argument_list|)
expr_stmt|;
name|i
operator|=
name|checkold
argument_list|(
operator|&
name|oldbad
argument_list|)
expr_stmt|;
if|if
condition|(
name|verbose
condition|)
name|printf
argument_list|(
literal|"Had %d bad sectors, adding %d\n"
argument_list|,
name|i
argument_list|,
name|argc
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|+
name|argc
operator|>
literal|126
condition|)
block|{
name|printf
argument_list|(
literal|"bad144: not enough room for %d more sectors\n"
argument_list|,
name|argc
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"limited to 126 by information format\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|curbad
operator|=
name|oldbad
expr_stmt|;
block|}
else|else
block|{
name|curbad
operator|.
name|bt_csn
operator|=
name|atoi
argument_list|(
operator|*
name|argv
operator|++
argument_list|)
expr_stmt|;
name|argc
operator|--
expr_stmt|;
name|curbad
operator|.
name|bt_mbz
operator|=
literal|0
expr_stmt|;
name|curbad
operator|.
name|bt_flag
operator|=
name|DKBAD_MAGIC
expr_stmt|;
if|if
condition|(
name|argc
operator|>
literal|126
condition|)
block|{
name|printf
argument_list|(
literal|"bad144: too many bad sectors specified\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"limited to 126 by information format\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|i
operator|=
literal|0
expr_stmt|;
block|}
name|errs
operator|=
literal|0
expr_stmt|;
name|new
operator|=
name|argc
expr_stmt|;
while|while
condition|(
name|argc
operator|>
literal|0
condition|)
block|{
name|daddr_t
name|sn
init|=
name|atoi
argument_list|(
operator|*
name|argv
operator|++
argument_list|)
decl_stmt|;
name|argc
operator|--
expr_stmt|;
if|if
condition|(
name|sn
operator|<
literal|0
operator|||
name|sn
operator|>=
name|size
condition|)
block|{
name|printf
argument_list|(
literal|"%d: out of range [0,%d) for disk %s\n"
argument_list|,
name|sn
argument_list|,
name|size
argument_list|,
name|dp
operator|->
name|d_typename
argument_list|)
expr_stmt|;
name|errs
operator|++
expr_stmt|;
continue|continue;
block|}
name|bn
index|[
name|i
index|]
operator|=
name|sn
expr_stmt|;
name|curbad
operator|.
name|bt_bad
index|[
name|i
index|]
operator|.
name|bt_cyl
operator|=
name|sn
operator|/
operator|(
name|dp
operator|->
name|d_nsectors
operator|*
name|dp
operator|->
name|d_ntracks
operator|)
expr_stmt|;
name|sn
operator|%=
operator|(
name|dp
operator|->
name|d_nsectors
operator|*
name|dp
operator|->
name|d_ntracks
operator|)
expr_stmt|;
name|curbad
operator|.
name|bt_bad
index|[
name|i
index|]
operator|.
name|bt_trksec
operator|=
operator|(
operator|(
name|sn
operator|/
name|dp
operator|->
name|d_nsectors
operator|)
operator|<<
literal|8
operator|)
operator|+
operator|(
name|sn
operator|%
name|dp
operator|->
name|d_nsectors
operator|)
expr_stmt|;
name|i
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|errs
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|nbad
operator|=
name|i
expr_stmt|;
while|while
condition|(
name|i
operator|<
literal|126
condition|)
block|{
name|curbad
operator|.
name|bt_bad
index|[
name|i
index|]
operator|.
name|bt_trksec
operator|=
operator|-
literal|1
expr_stmt|;
name|curbad
operator|.
name|bt_bad
index|[
name|i
index|]
operator|.
name|bt_cyl
operator|=
operator|-
literal|1
expr_stmt|;
name|i
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|add
condition|)
block|{
comment|/* 		 * Sort the new bad sectors into the list. 		 * Then shuffle the replacement sectors so that 		 * the previous bad sectors get the same replacement data. 		 */
name|qsort
argument_list|(
operator|(
name|char
operator|*
operator|)
name|curbad
operator|.
name|bt_bad
argument_list|,
name|nbad
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|bt_bad
argument_list|)
argument_list|,
name|compare
argument_list|)
expr_stmt|;
if|if
condition|(
name|dups
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: bad sectors have been duplicated; can't add existing sectors\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|3
argument_list|)
expr_stmt|;
block|}
name|shift
argument_list|(
name|f
argument_list|,
name|nbad
argument_list|,
name|nbad
operator|-
name|new
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|badfile
operator|==
operator|-
literal|1
condition|)
name|i
operator|=
literal|0
expr_stmt|;
else|else
name|i
operator|=
name|badfile
operator|*
literal|2
expr_stmt|;
for|for
control|(
init|;
name|i
operator|<
literal|10
operator|&&
name|i
operator|<
name|dp
operator|->
name|d_nsectors
condition|;
name|i
operator|+=
literal|2
control|)
block|{
if|if
condition|(
name|lseek
argument_list|(
name|f
argument_list|,
name|dp
operator|->
name|d_secsize
operator|*
operator|(
name|size
operator|-
name|dp
operator|->
name|d_nsectors
operator|+
name|i
operator|)
argument_list|,
name|L_SET
argument_list|)
operator|<
literal|0
condition|)
name|Perror
argument_list|(
literal|"lseek"
argument_list|)
expr_stmt|;
if|if
condition|(
name|verbose
condition|)
name|printf
argument_list|(
literal|"write badsect file at %d\n"
argument_list|,
name|size
operator|-
name|dp
operator|->
name|d_nsectors
operator|+
name|i
argument_list|)
expr_stmt|;
if|if
condition|(
name|nflag
operator|==
literal|0
operator|&&
name|write
argument_list|(
name|f
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|curbad
argument_list|,
sizeof|sizeof
argument_list|(
name|curbad
argument_list|)
argument_list|)
operator|!=
sizeof|sizeof
argument_list|(
name|curbad
argument_list|)
condition|)
block|{
name|char
name|msg
index|[
literal|80
index|]
decl_stmt|;
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|msg
argument_list|,
literal|"bad144: write bad sector file %d"
argument_list|,
name|i
operator|/
literal|2
argument_list|)
expr_stmt|;
name|perror
argument_list|(
name|msg
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|badfile
operator|!=
operator|-
literal|1
condition|)
break|break;
block|}
ifdef|#
directive|ifdef
name|vax
if|if
condition|(
name|nflag
operator|==
literal|0
operator|&&
name|fflag
condition|)
for|for
control|(
name|i
operator|=
name|nbad
operator|-
name|new
init|;
name|i
operator|<
name|nbad
condition|;
name|i
operator|++
control|)
name|format
argument_list|(
name|f
argument_list|,
name|bn
index|[
name|i
index|]
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|DIOCSBAD
if|if
condition|(
name|nflag
operator|==
literal|0
operator|&&
name|ioctl
argument_list|(
name|f
argument_list|,
name|DIOCSBAD
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|curbad
argument_list|)
operator|<
literal|0
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Can't sync bad-sector file; reboot for changes to take effect\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|daddr_t
name|getold
parameter_list|(
name|f
parameter_list|,
name|bad
parameter_list|)
name|struct
name|dkbad
modifier|*
name|bad
decl_stmt|;
block|{
specifier|register
name|int
name|i
decl_stmt|;
name|daddr_t
name|sn
decl_stmt|;
name|char
name|msg
index|[
literal|80
index|]
decl_stmt|;
if|if
condition|(
name|badfile
operator|==
operator|-
literal|1
condition|)
name|i
operator|=
literal|0
expr_stmt|;
else|else
name|i
operator|=
name|badfile
operator|*
literal|2
expr_stmt|;
for|for
control|(
init|;
name|i
operator|<
literal|10
operator|&&
name|i
operator|<
name|dp
operator|->
name|d_nsectors
condition|;
name|i
operator|+=
literal|2
control|)
block|{
name|sn
operator|=
name|size
operator|-
name|dp
operator|->
name|d_nsectors
operator|+
name|i
expr_stmt|;
if|if
condition|(
name|lseek
argument_list|(
name|f
argument_list|,
name|sn
operator|*
name|dp
operator|->
name|d_secsize
argument_list|,
name|L_SET
argument_list|)
operator|<
literal|0
condition|)
name|Perror
argument_list|(
literal|"lseek"
argument_list|)
expr_stmt|;
if|if
condition|(
name|read
argument_list|(
name|f
argument_list|,
operator|(
name|char
operator|*
operator|)
name|bad
argument_list|,
name|dp
operator|->
name|d_secsize
argument_list|)
operator|==
name|dp
operator|->
name|d_secsize
condition|)
block|{
if|if
condition|(
name|i
operator|>
literal|0
condition|)
name|printf
argument_list|(
literal|"Using bad-sector file %d\n"
argument_list|,
name|i
operator|/
literal|2
argument_list|)
expr_stmt|;
return|return
operator|(
name|sn
operator|)
return|;
block|}
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|msg
argument_list|,
literal|"bad144: read bad sector file at sn %d"
argument_list|,
name|sn
argument_list|)
expr_stmt|;
name|perror
argument_list|(
name|msg
argument_list|)
expr_stmt|;
if|if
condition|(
name|badfile
operator|!=
operator|-
literal|1
condition|)
break|break;
block|}
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: %s: can't read bad block info\n"
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
comment|/*NOTREACHED*/
block|}
end_function

begin_macro
name|checkold
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
specifier|register
name|struct
name|bt_bad
modifier|*
name|bt
decl_stmt|;
name|daddr_t
name|sn
decl_stmt|,
name|lsn
decl_stmt|;
name|int
name|errors
init|=
literal|0
decl_stmt|,
name|warned
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|oldbad
operator|.
name|bt_flag
operator|!=
name|DKBAD_MAGIC
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: %s: bad flag in bad-sector table\n"
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|errors
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|oldbad
operator|.
name|bt_mbz
operator|!=
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: %s: bad magic number\n"
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|errors
operator|++
expr_stmt|;
block|}
name|bt
operator|=
name|oldbad
operator|.
name|bt_bad
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|126
condition|;
name|i
operator|++
operator|,
name|bt
operator|++
control|)
block|{
if|if
condition|(
name|bt
operator|->
name|bt_cyl
operator|==
literal|0xffff
operator|&&
name|bt
operator|->
name|bt_trksec
operator|==
literal|0xffff
condition|)
break|break;
if|if
condition|(
operator|(
name|bt
operator|->
name|bt_cyl
operator|>=
name|dp
operator|->
name|d_ncylinders
operator|)
operator|||
operator|(
operator|(
name|bt
operator|->
name|bt_trksec
operator|>>
literal|8
operator|)
operator|>=
name|dp
operator|->
name|d_ntracks
operator|)
operator|||
operator|(
operator|(
name|bt
operator|->
name|bt_trksec
operator|&
literal|0xff
operator|)
operator|>=
name|dp
operator|->
name|d_nsectors
operator|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: cyl/trk/sect out of range in existing entry: "
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"sn=%d, cn=%d, tn=%d, sn=%d\n"
argument_list|,
name|badsn
argument_list|(
name|bt
argument_list|)
argument_list|,
name|bt
operator|->
name|bt_cyl
argument_list|,
name|bt
operator|->
name|bt_trksec
operator|>>
literal|8
argument_list|,
name|bt
operator|->
name|bt_trksec
operator|&
literal|0xff
argument_list|)
expr_stmt|;
name|errors
operator|++
expr_stmt|;
block|}
name|sn
operator|=
operator|(
name|bt
operator|->
name|bt_cyl
operator|*
name|dp
operator|->
name|d_ntracks
operator|+
operator|(
name|bt
operator|->
name|bt_trksec
operator|>>
literal|8
operator|)
operator|)
operator|*
name|dp
operator|->
name|d_nsectors
operator|+
operator|(
name|bt
operator|->
name|bt_trksec
operator|&
literal|0xff
operator|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|0
operator|&&
name|sn
operator|<
name|lsn
operator|&&
operator|!
name|warned
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: bad sector file is out of order\n"
argument_list|)
expr_stmt|;
name|errors
operator|++
expr_stmt|;
name|warned
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|i
operator|>
literal|0
operator|&&
name|sn
operator|==
name|lsn
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: bad sector file contains duplicates (sn %d)\n"
argument_list|,
name|sn
argument_list|)
expr_stmt|;
name|errors
operator|++
expr_stmt|;
block|}
name|lsn
operator|=
name|sn
expr_stmt|;
block|}
if|if
condition|(
name|errors
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
return|return
operator|(
name|i
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Move the bad sector replacements  * to make room for the new bad sectors.  * new is the new number of bad sectors, old is the previous count.  */
end_comment

begin_macro
name|shift
argument_list|(
argument|f
argument_list|,
argument|new
argument_list|,
argument|old
argument_list|)
end_macro

begin_block
block|{
name|daddr_t
name|repl
decl_stmt|;
comment|/* 	 * First replacement is last sector of second-to-last track. 	 */
name|repl
operator|=
name|size
operator|-
name|dp
operator|->
name|d_nsectors
operator|-
literal|1
expr_stmt|;
name|new
operator|--
expr_stmt|;
name|old
operator|--
expr_stmt|;
while|while
condition|(
name|new
operator|>=
literal|0
operator|&&
name|new
operator|!=
name|old
condition|)
block|{
if|if
condition|(
name|old
operator|<
literal|0
operator|||
name|compare
argument_list|(
operator|&
name|curbad
operator|.
name|bt_bad
index|[
name|new
index|]
argument_list|,
operator|&
name|oldbad
operator|.
name|bt_bad
index|[
name|old
index|]
argument_list|)
operator|>
literal|0
condition|)
block|{
comment|/* 			 * Insert new replacement here-- copy original 			 * sector if requested and possible, 			 * otherwise write a zero block. 			 */
if|if
condition|(
operator|!
name|copy
operator|||
operator|!
name|blkcopy
argument_list|(
name|f
argument_list|,
name|badsn
argument_list|(
operator|&
name|curbad
operator|.
name|bt_bad
index|[
name|new
index|]
argument_list|)
argument_list|,
name|repl
operator|-
name|new
argument_list|)
condition|)
name|blkzero
argument_list|(
name|f
argument_list|,
name|repl
operator|-
name|new
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|blkcopy
argument_list|(
name|f
argument_list|,
name|repl
operator|-
name|old
argument_list|,
name|repl
operator|-
name|new
argument_list|)
operator|==
literal|0
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Can't copy replacement sector %d to %d\n"
argument_list|,
name|repl
operator|-
name|old
argument_list|,
name|repl
operator|-
name|new
argument_list|)
expr_stmt|;
name|old
operator|--
expr_stmt|;
block|}
name|new
operator|--
expr_stmt|;
block|}
block|}
end_block

begin_decl_stmt
name|char
modifier|*
name|buf
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  *  Copy disk sector s1 to s2.  */
end_comment

begin_macro
name|blkcopy
argument_list|(
argument|f
argument_list|,
argument|s1
argument_list|,
argument|s2
argument_list|)
end_macro

begin_decl_stmt
name|daddr_t
name|s1
decl_stmt|,
name|s2
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|tries
operator|,
name|n
expr_stmt|;
if|if
condition|(
name|buf
operator|==
operator|(
name|char
operator|*
operator|)
name|NULL
condition|)
block|{
name|buf
operator|=
name|malloc
argument_list|(
operator|(
name|unsigned
operator|)
name|dp
operator|->
name|d_secsize
argument_list|)
expr_stmt|;
if|if
condition|(
name|buf
operator|==
operator|(
name|char
operator|*
operator|)
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Out of memory\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|20
argument_list|)
expr_stmt|;
block|}
block|}
for|for
control|(
name|tries
operator|=
literal|0
init|;
name|tries
operator|<
name|RETRIES
condition|;
name|tries
operator|++
control|)
block|{
if|if
condition|(
name|lseek
argument_list|(
name|f
argument_list|,
name|dp
operator|->
name|d_secsize
operator|*
name|s1
argument_list|,
name|L_SET
argument_list|)
operator|<
literal|0
condition|)
name|Perror
argument_list|(
literal|"lseek"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|n
operator|=
name|read
argument_list|(
name|f
argument_list|,
name|buf
argument_list|,
name|dp
operator|->
name|d_secsize
argument_list|)
operator|)
operator|==
name|dp
operator|->
name|d_secsize
condition|)
break|break;
block|}
if|if
condition|(
name|n
operator|!=
name|dp
operator|->
name|d_secsize
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: can't read sector, %d: "
argument_list|,
name|s1
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|<
literal|0
condition|)
name|perror
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
name|lseek
argument_list|(
name|f
argument_list|,
name|dp
operator|->
name|d_secsize
operator|*
name|s2
argument_list|,
name|L_SET
argument_list|)
operator|<
literal|0
condition|)
name|Perror
argument_list|(
literal|"lseek"
argument_list|)
expr_stmt|;
if|if
condition|(
name|verbose
condition|)
name|printf
argument_list|(
literal|"copying %d to %d\n"
argument_list|,
name|s1
argument_list|,
name|s2
argument_list|)
expr_stmt|;
if|if
condition|(
name|nflag
operator|==
literal|0
operator|&&
name|write
argument_list|(
name|f
argument_list|,
name|buf
argument_list|,
name|dp
operator|->
name|d_secsize
argument_list|)
operator|!=
name|dp
operator|->
name|d_secsize
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: can't write replacement sector, %d: "
argument_list|,
name|s2
argument_list|)
expr_stmt|;
name|perror
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_decl_stmt
name|char
modifier|*
name|zbuf
decl_stmt|;
end_decl_stmt

begin_macro
name|blkzero
argument_list|(
argument|f
argument_list|,
argument|sn
argument_list|)
end_macro

begin_decl_stmt
name|daddr_t
name|sn
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|zbuf
operator|==
operator|(
name|char
operator|*
operator|)
name|NULL
condition|)
block|{
name|zbuf
operator|=
name|malloc
argument_list|(
operator|(
name|unsigned
operator|)
name|dp
operator|->
name|d_secsize
argument_list|)
expr_stmt|;
if|if
condition|(
name|zbuf
operator|==
operator|(
name|char
operator|*
operator|)
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Out of memory\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|20
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|lseek
argument_list|(
name|f
argument_list|,
name|dp
operator|->
name|d_secsize
operator|*
name|sn
argument_list|,
name|L_SET
argument_list|)
operator|<
literal|0
condition|)
name|Perror
argument_list|(
literal|"lseek"
argument_list|)
expr_stmt|;
if|if
condition|(
name|verbose
condition|)
name|printf
argument_list|(
literal|"zeroing %d\n"
argument_list|,
name|sn
argument_list|)
expr_stmt|;
if|if
condition|(
name|nflag
operator|==
literal|0
operator|&&
name|write
argument_list|(
name|f
argument_list|,
name|zbuf
argument_list|,
name|dp
operator|->
name|d_secsize
argument_list|)
operator|!=
name|dp
operator|->
name|d_secsize
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: can't write replacement sector, %d: "
argument_list|,
name|sn
argument_list|)
expr_stmt|;
name|perror
argument_list|(
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_expr_stmt
name|compare
argument_list|(
name|b1
argument_list|,
name|b2
argument_list|)
specifier|register
expr|struct
name|bt_bad
operator|*
name|b1
operator|,
operator|*
name|b2
expr_stmt|;
end_expr_stmt

begin_block
block|{
if|if
condition|(
name|b1
operator|->
name|bt_cyl
operator|>
name|b2
operator|->
name|bt_cyl
condition|)
return|return
operator|(
literal|1
operator|)
return|;
if|if
condition|(
name|b1
operator|->
name|bt_cyl
operator|<
name|b2
operator|->
name|bt_cyl
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
if|if
condition|(
name|b1
operator|->
name|bt_trksec
operator|==
name|b2
operator|->
name|bt_trksec
condition|)
name|dups
operator|++
expr_stmt|;
return|return
operator|(
name|b1
operator|->
name|bt_trksec
operator|-
name|b2
operator|->
name|bt_trksec
operator|)
return|;
block|}
end_block

begin_function
name|daddr_t
name|badsn
parameter_list|(
name|bt
parameter_list|)
specifier|register
name|struct
name|bt_bad
modifier|*
name|bt
decl_stmt|;
block|{
return|return
operator|(
operator|(
name|bt
operator|->
name|bt_cyl
operator|*
name|dp
operator|->
name|d_ntracks
operator|+
operator|(
name|bt
operator|->
name|bt_trksec
operator|>>
literal|8
operator|)
operator|)
operator|*
name|dp
operator|->
name|d_nsectors
operator|+
operator|(
name|bt
operator|->
name|bt_trksec
operator|&
literal|0xff
operator|)
operator|)
return|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|vax
end_ifdef

begin_struct
struct|struct
name|rp06hdr
block|{
name|short
name|h_cyl
decl_stmt|;
name|short
name|h_trksec
decl_stmt|;
name|short
name|h_key1
decl_stmt|;
name|short
name|h_key2
decl_stmt|;
name|char
name|h_data
index|[
literal|512
index|]
decl_stmt|;
define|#
directive|define
name|RP06_FMT
value|010000
comment|/* 1 == 16 bit, 0 == 18 bit */
block|}
struct|;
end_struct

begin_comment
comment|/*  * Most massbus and unibus drives  * have headers of this form  */
end_comment

begin_struct
struct|struct
name|hpuphdr
block|{
name|u_short
name|hpup_cyl
decl_stmt|;
name|u_char
name|hpup_sect
decl_stmt|;
name|u_char
name|hpup_track
decl_stmt|;
name|char
name|hpup_data
index|[
literal|512
index|]
decl_stmt|;
define|#
directive|define
name|HPUP_OKSECT
value|0xc000
comment|/* this normally means sector is good */
define|#
directive|define
name|HPUP_16BIT
value|0x1000
comment|/* 1 == 16 bit format */
block|}
struct|;
end_struct

begin_decl_stmt
name|int
name|rp06format
argument_list|()
decl_stmt|,
name|hpupformat
argument_list|()
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|formats
block|{
name|char
modifier|*
name|f_name
decl_stmt|;
comment|/* disk name */
name|int
name|f_bufsize
decl_stmt|;
comment|/* size of sector + header */
name|int
name|f_bic
decl_stmt|;
comment|/* value to bic in hpup_cyl */
name|int
function_decl|(
modifier|*
name|f_routine
function_decl|)
parameter_list|()
function_decl|;
comment|/* routine for special handling */
block|}
name|formats
index|[]
init|=
block|{
block|{
literal|"rp06"
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|rp06hdr
argument_list|)
block|,
name|RP06_FMT
block|,
name|rp06format
block|}
block|,
block|{
literal|"eagle"
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|hpuphdr
argument_list|)
block|,
name|HPUP_OKSECT
block|,
name|hpupformat
block|}
block|,
block|{
literal|"capricorn"
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|hpuphdr
argument_list|)
block|,
name|HPUP_OKSECT
block|,
name|hpupformat
block|}
block|,
block|{
literal|"rm03"
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|hpuphdr
argument_list|)
block|,
name|HPUP_OKSECT
block|,
name|hpupformat
block|}
block|,
block|{
literal|"rm05"
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|hpuphdr
argument_list|)
block|,
name|HPUP_OKSECT
block|,
name|hpupformat
block|}
block|,
block|{
literal|"9300"
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|hpuphdr
argument_list|)
block|,
name|HPUP_OKSECT
block|,
name|hpupformat
block|}
block|,
block|{
literal|"9766"
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|hpuphdr
argument_list|)
block|,
name|HPUP_OKSECT
block|,
name|hpupformat
block|}
block|,
block|{
literal|0
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|}
block|}
struct|;
end_struct

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|hpupformat
argument_list|(
argument|fp
argument_list|,
argument|dp
argument_list|,
argument|blk
argument_list|,
argument|buf
argument_list|,
argument|count
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|formats
modifier|*
name|fp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|disklabel
modifier|*
name|dp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|daddr_t
name|blk
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|buf
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|count
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|hpuphdr
modifier|*
name|hdr
init|=
operator|(
expr|struct
name|hpuphdr
operator|*
operator|)
name|buf
decl_stmt|;
name|int
name|sect
decl_stmt|;
if|if
condition|(
name|count
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|hpuphdr
argument_list|)
condition|)
block|{
name|hdr
operator|->
name|hpup_cyl
operator|=
operator|(
name|HPUP_OKSECT
operator||
name|HPUP_16BIT
operator|)
operator||
operator|(
name|blk
operator|/
operator|(
name|dp
operator|->
name|d_nsectors
operator|*
name|dp
operator|->
name|d_ntracks
operator|)
operator|)
expr_stmt|;
name|sect
operator|=
name|blk
operator|%
operator|(
name|dp
operator|->
name|d_nsectors
operator|*
name|dp
operator|->
name|d_ntracks
operator|)
expr_stmt|;
name|hdr
operator|->
name|hpup_track
operator|=
call|(
name|u_char
call|)
argument_list|(
name|sect
operator|/
name|dp
operator|->
name|d_nsectors
argument_list|)
expr_stmt|;
name|hdr
operator|->
name|hpup_sect
operator|=
call|(
name|u_char
call|)
argument_list|(
name|sect
operator|%
name|dp
operator|->
name|d_nsectors
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|rp06format
argument_list|(
argument|fp
argument_list|,
argument|dp
argument_list|,
argument|blk
argument_list|,
argument|buf
argument_list|,
argument|count
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|formats
modifier|*
name|fp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|disklabel
modifier|*
name|dp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|daddr_t
name|blk
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|buf
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|count
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|count
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|rp06hdr
argument_list|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Can't read header on blk %d, can't reformat\n"
argument_list|,
name|blk
argument_list|)
expr_stmt|;
return|return
operator|(
operator|-
literal|1
operator|)
return|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_macro
name|format
argument_list|(
argument|fd
argument_list|,
argument|blk
argument_list|)
end_macro

begin_decl_stmt
name|int
name|fd
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|daddr_t
name|blk
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|formats
modifier|*
name|fp
decl_stmt|;
specifier|static
name|char
modifier|*
name|buf
decl_stmt|;
specifier|static
name|char
name|bufsize
decl_stmt|;
name|struct
name|format_op
name|fop
decl_stmt|;
name|int
name|n
decl_stmt|;
for|for
control|(
name|fp
operator|=
name|formats
init|;
name|fp
operator|->
name|f_name
condition|;
name|fp
operator|++
control|)
if|if
condition|(
name|strcmp
argument_list|(
name|dp
operator|->
name|d_typename
argument_list|,
name|fp
operator|->
name|f_name
argument_list|)
operator|==
literal|0
condition|)
break|break;
if|if
condition|(
name|fp
operator|->
name|f_name
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: don't know how to format %s disks\n"
argument_list|,
name|dp
operator|->
name|d_typename
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|2
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|buf
operator|&&
name|bufsize
operator|<
name|fp
operator|->
name|f_bufsize
condition|)
block|{
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
name|buf
operator|=
name|NULL
expr_stmt|;
block|}
if|if
condition|(
name|buf
operator|==
name|NULL
condition|)
name|buf
operator|=
name|malloc
argument_list|(
operator|(
name|unsigned
operator|)
name|fp
operator|->
name|f_bufsize
argument_list|)
expr_stmt|;
if|if
condition|(
name|buf
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: can't allocate sector buffer\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|3
argument_list|)
expr_stmt|;
block|}
name|bufsize
operator|=
name|fp
operator|->
name|f_bufsize
expr_stmt|;
comment|/* 	 * Here we do the actual formatting.  All we really 	 * do is rewrite the sector header and flag the bad sector 	 * according to the format table description.  If a special 	 * purpose format routine is specified, we allow it to 	 * process the sector as well. 	 */
if|if
condition|(
name|verbose
condition|)
name|printf
argument_list|(
literal|"format blk %d\n"
argument_list|,
name|blk
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|fop
argument_list|,
sizeof|sizeof
argument_list|(
name|fop
argument_list|)
argument_list|)
expr_stmt|;
name|fop
operator|.
name|df_buf
operator|=
name|buf
expr_stmt|;
name|fop
operator|.
name|df_count
operator|=
name|fp
operator|->
name|f_bufsize
expr_stmt|;
name|fop
operator|.
name|df_startblk
operator|=
name|blk
expr_stmt|;
name|bzero
argument_list|(
name|buf
argument_list|,
name|fp
operator|->
name|f_bufsize
argument_list|)
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|DIOCRFORMAT
argument_list|,
operator|&
name|fop
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
literal|"bad144: read format"
argument_list|)
expr_stmt|;
if|if
condition|(
name|fp
operator|->
name|f_routine
operator|&&
call|(
modifier|*
name|fp
operator|->
name|f_routine
call|)
argument_list|(
name|fp
argument_list|,
name|dp
argument_list|,
name|blk
argument_list|,
name|buf
argument_list|,
name|fop
operator|.
name|df_count
argument_list|)
operator|!=
literal|0
condition|)
return|return;
if|if
condition|(
name|fp
operator|->
name|f_bic
condition|)
block|{
name|struct
name|hpuphdr
modifier|*
name|xp
init|=
operator|(
expr|struct
name|hpuphdr
operator|*
operator|)
name|buf
decl_stmt|;
name|xp
operator|->
name|hpup_cyl
operator|&=
operator|~
name|fp
operator|->
name|f_bic
expr_stmt|;
block|}
if|if
condition|(
name|nflag
condition|)
return|return;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|fop
argument_list|,
sizeof|sizeof
argument_list|(
name|fop
argument_list|)
argument_list|)
expr_stmt|;
name|fop
operator|.
name|df_buf
operator|=
name|buf
expr_stmt|;
name|fop
operator|.
name|df_count
operator|=
name|fp
operator|->
name|f_bufsize
expr_stmt|;
name|fop
operator|.
name|df_startblk
operator|=
name|blk
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|DIOCWFORMAT
argument_list|,
operator|&
name|fop
argument_list|)
operator|<
literal|0
condition|)
name|Perror
argument_list|(
literal|"write format"
argument_list|)
expr_stmt|;
if|if
condition|(
name|fop
operator|.
name|df_count
operator|!=
name|fp
operator|->
name|f_bufsize
condition|)
block|{
name|char
name|msg
index|[
literal|80
index|]
decl_stmt|;
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|msg
argument_list|,
literal|"bad144: write format %d"
argument_list|,
name|blk
argument_list|)
expr_stmt|;
name|perror
argument_list|(
name|msg
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_macro
name|Perror
argument_list|(
argument|op
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|op
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad144: "
argument_list|)
expr_stmt|;
name|perror
argument_list|(
name|op
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|4
argument_list|)
expr_stmt|;
block|}
end_block

end_unit

