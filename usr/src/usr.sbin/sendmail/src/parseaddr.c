...
index|[]
init|=
literal|"@(#)parseaddr.c	5.13.1.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
comment|/* test entry which should be nonzero */
if|if
condition|(
name|control_opts
index|[
...
condition|)
block|{
comment|/* First time through, initialize table */
name|int
name|c
decl_stmt|,
name|i
...
name|MATCHNMAP
case|:
comment|/* match any token not in a DBM map */
if|if
condition|(
operator|(
name|rp
...
argument_list|)
expr_stmt|;
comment|/* read database name if that's what we're up for */
if|if
condition|(
name|begintype
operator|==
...
return|;
block|}
comment|/* define tohost before running mailer rulesets */
name|define
argument_list|(
literal|'h'
argument_list|,
...
begin_comment
comment|/* **  REMOTENAME -- return the name relative to the current mailer ** **	Parameters: **		name -- the name to translate. **		m -- the mailer that we want to do rewriting relative **			to. **		senderaddress -- if set, uses the sender rewriting rules **			rather than the recipient rewriting rules. **		canonical -- if set, strip out any comment information, **			etc. **		headeraddress -- if set, use header specific rewriting **			rulesets and uurelativize if M_RELATIVIZE is set. ** **	Returns: **		the text string representing this address relative to **			the receiving mailer. ** **	Side Effects: **		none. ** **	Warnings: **		The text string returned is tucked away locally; **			copy it if you intend to save it. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	**  Check if we're supposed to do make the address 	**  UUCP !-relative to the rcpt host vs ourselves. 	*/
if|if
condition|(
name|headeraddress
operator|&&
