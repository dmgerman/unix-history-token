...
index|[]
init|=
literal|"@(#)alias.c	6.46 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* **  REBUILDALIASES -- rebuild the alias database. ** **	Parameters: **		map -- the database to rebuild. **		automatic -- set if this was automatically generated. **		e -- current envelope. ** **	Returns: **		none. ** **	Side Effects: **		Reads the text version of the database, builds the **		DBM or DB version. */
end_comment
...
begin_comment
comment|/* **  READALIASES -- read and process the alias file. ** **	This routine implements the part of initaliases that occurs **	when we are not going to use the DBM stuff. ** **	Parameters: **		map -- the alias database descriptor. **		af -- file to read the aliases from. **		automatic -- set if this was an automatic rebuild. **		e -- the current envelope. ** **	Returns: **		none. ** **	Side Effects: **		Reads aliasfile into the symbol table. **		Optionally, builds the .dir& .pag files. */
end_comment
