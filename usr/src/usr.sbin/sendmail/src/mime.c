...
expr_stmt|;
block|}
comment|/* flag subtypes that can't have any 8-bit data */
if|if
condition|(
name|strcasecmp
argument_list|(
...
comment|/* handle types that cannot have 8-bit data internally */
name|sprintf
argument_list|(
name|buf
argument_list|,
...
begin_comment
comment|/* **  MIME_GETCHAR -- get a character for MIME processing ** **	Treats boundaries as EOF. ** **	Parameters: **		fp -- the input file. **		boundaries -- the current MIME boundaries. ** **	Returns: **		The next character in the input stream. */
end_comment
