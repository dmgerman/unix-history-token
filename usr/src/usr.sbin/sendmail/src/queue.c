...
argument_list|()
expr_stmt|;
comment|/* arrange to reorder later */
name|ReorderQueue
operator|=
name|FALSE
expr_stmt|;
...
begin_comment
comment|/* **  REORDERSIG -- catch the reorder signal and tell sendmail to reorder queue. ** **	Parameters: **		parent -- if set, called from parent (i.e., not **			really doing the work). ** **	Returns: **		none. ** **	Side Effects: **		sets the "reorder work queue" flag. */
end_comment
...
block|{
comment|/* 		**  In parent.  If the child still exists, we want 		**  to do nothing.  If the child is gone, we will 		**  start up a new one. 		**  If the child exists, it is responsible for 		**  doing a queue reorder. 		**  This code really sucks. 		*/
if|if
condition|(
name|kill
argument_list|(
...
condition|)
block|{
comment|/* 		**  Collect any continuation lines... 		*/
name|i
operator|=
name|fgetc
argument_list|(
