...
begin_comment
comment|/*  * The user structure.  * One allocated per process.  * Contains all per process data  * that doesn't need to be referenced  * while the process is swapped.  * The user block is USIZE*512 bytes  * long; resides at virtual kernel  * loc 140000; contains the system  * stack per user; is cross referenced  * with the proc structure for the  * same process.  */
end_comment
...
index|]
decl_stmt|;
comment|/* kernel stack per user 					 * extends from u + USIZE*64 					 * backward not to reah here 					 */
block|}
struct|;
end_struct
