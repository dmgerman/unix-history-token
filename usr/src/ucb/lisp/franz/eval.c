...
name|rcsid
init|=
literal|"$Header: eval.c,v 1.6 83/09/07 17:54:42 sklower Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*					-[Thu Aug 18 10:07:22 1983 by jkf]-  * 	eval.c				$Locker:  $  * evaluator  *  * (c) copyright 1982, Regents of the University of California  */
end_comment
...
name|evalhcallsw
condition|)
comment|/*if we got here after calling evalhook, then 			  evalhcallsw will be TRUE, so we want to skip calling 			  the hook function, permitting one form to be 			  evaluated before the hook fires. 			 */
block|{
...
name|FALSE
expr_stmt|;
comment|/* clear indication that evalhook called */
switch|switch
condition|(
name|TYPE
argument_list|(
...
begin_comment
comment|/*  *    popnames  * removes from the name stack all entries above the first argument.     * routine should usually be used to clean up the name stack as it      * knows about the special cases.  bnp is returned pointing to the  * same place as the argument passed.  */
end_comment
...
name|funhcallsw
condition|)
comment|/*if we got here after calling funcallhook, then 			  funhcallsw will be TRUE, so we want to skip calling 			  the hook function, permitting one form to be 			  evaluated before the hook fires. 			 */
block|{
...
name|FALSE
expr_stmt|;
comment|/* so recursive calls to funcall will cause hook     			   to fire */
for|for
control|(
name|EVER
control|)
