...
name|rcsid
init|=
literal|"$Header: /na/franz/franz/RCS/eval.c,v 1.2 83/01/29 12:32:46 jkf Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*					-[Sat Jan 29 12:19:37 1983 by jkf]-  * 	eval.c				$Locker:  $  * evaluator  *  * (c) copyright 1982, Regents of the University of California  */
end_comment
...
begin_comment
comment|/*  *	eval  * returns the value of the pointer passed as the argument.		  *  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* bind funcallhook to nil*/
name|funhcallsw
operator|=
name|TRUE
expr_stmt|;
...
control|)
block|{
comment|/* short circuit evaluations of ATOM, INT, DOUB 		     * if not in debugging mode 		     */
if|if
condition|(
name|shortcircuit
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* save function definition in case function 				   is redefined */
name|lbot
operator|=
name|np
expr_stmt|;
...
begin_comment
comment|/*  *    popnames  * removes from the name stack all entries above the first argument.     * routine should usually be used to clean up the name stack as it      * knows about the special cases.  np is returned pointing to the  * same place as the argument passed.  */
end_comment
...
begin_comment
comment|/* dumpnamestack  * utility routine to dump out the namestack.  * from bottom to 5 above np  * should be put elsewhere  */
end_comment
...
name|np
decl_stmt|;
comment|/* First free on stack */
name|int
name|extrapush
decl_stmt|;
comment|/* if must save function value */
name|a
operator|=
name|lbot
operator|->
...
break|break;
comment|/* something is wrong - this isnt a proper function */
name|rebind
argument_list|(
name|a
operator|->
...
name|VECTOR
case|:
comment|/* certain vectors are valid (fclosures) */
if|if
condition|(
name|a
operator|->
...
block|}
empty_stmt|;
comment|/* pop off extra value if we pushed it before */
if|if
condition|(
name|extrapush
operator|==
...
begin_comment
comment|/* the argument to Lfuncal is now mandatory since nargs  * wont work on RISC. If it is given  then it is   * the name of the function to call and lbot points to the first arg.  * if it is not given, then lbot points to the function to call  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/*if(nargs()==1)			/* function I am evaling.    */
name|a
operator|=
name|fcncalled
operator|=
name|fcn
expr_stmt|;
comment|/*else { a = fcncalled = lbot->val; lbot++; }*/
...
literal|1
expr_stmt|;
comment|/* must protect this later */
block|}
name|vtemp
operator|=
name|CNIL
...
argument_list|)
expr_stmt|;
comment|/* since the actual arguments are bound to their formal params 	     * we can pop them off the stack.  However if we are doing 	     * debugging (that is if we've pushed a frame on the stack) 	     * then we must not pop off the actual args since they must 	     * be visible for evalframe to work 	     */
if|if
condition|(
operator|!
name|dopopframe
...
name|VECTOR
case|:
comment|/* A fclosure represented as a vector with the property 'fclosure' */
if|if
condition|(
name|a
operator|->
...
begin_function
name|lispval
comment|/* this version called from lisp */
name|Lfuncal
parameter_list|()
block|{
name|lispval
...
begin_comment
comment|/*  * Llexfun  :: lisp function lexpr-funcall  * lexpr-funcall is a cross between funcall and apply.  * the last argument is nil or a list of the rest of the arguments.  * we push those arguments on the stack and call funcall  *  */
end_comment
...
operator|)
return|;
comment|/* no args besides function */
block|}
comment|/* have at least one argument past the function to funcall */
name|handy
operator|=
name|np
index|[
...
name|val
expr_stmt|;
comment|/* get last value */
name|np
operator|--
expr_stmt|;
comment|/* pop it off stack */
while|while
condition|(
operator|(
name|handy
...
name|Vermisc
argument_list|,
literal|"lexpr-funcall: last argument is not a list "
argument_list|,
name|nil
argument_list|,
name|TRUE
...
begin_comment
comment|/* protect   * pushes the first argument onto namestack, thereby protecting from gc  */
end_comment
...
begin_comment
comment|/* unprot  * returns the top thing on the name stack.  Underflow had better not  * occur.  */
end_comment
...
begin_comment
comment|/* Undeff - called from qfuncl when it detects a call to a undefined     function from compiled code, we print out a message and     will continue only if returned a symbol (ATOM in C parlance). */
end_comment
...
begin_comment
comment|/* Ifclosure :: evaluate a fclosure  * the argument clos is a vector whose property is the atom fclosure  * the form of the vector is  *   0: function to run  * then for each symbol there are 3 vector entries  *   1: symbol  * either  *   2: nil  *   3: value to bind symbol to  * or  *   2: a vector (another fclosure)  *   3: fixnum which is the index into the fclosure of the  *      slot containing the value  *  .... and so on ...  *  * when entering a fclosure, the values of the globals are set to the  * saved values in the fclosure.  * When exiting a fclosure, the current values of those globals are stored  * back in.  * note: we don't do an unwind-protect to insure that we restore the values.  *  perhaps we should, it isn't clear though.  *  * clos is the fclosure, funcallp is TRUE if this is called from funcall,  * otherwise it is called from apply  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* vector length must be positive and 1+ a multiple of 3 */
if|if
condition|(
operator|(
name|vlength
...
condition|)
block|{
comment|/* this is a pointer to another fclosure */
name|vect
operator|=
name|vect
operator|->
...
expr_stmt|;
block|}
comment|/* this should never be an error (famous last words) */
if|if
condition|(
operator|(
name|bnp
...
name|Vermisc
argument_list|,
literal|"funcall: incorrect number of values after fclosure:"
argument_list|,
name|nil
argument_list|,
name|FALSE
...
argument_list|)
expr_stmt|;
comment|/* pop off current values and store current values in fclosure */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* store value in another fclosure */
name|vect
operator|->
name|v
operator|.
