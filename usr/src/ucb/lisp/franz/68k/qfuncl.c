...
begin_comment
comment|/*  *$Header: qfuncl.c,v 1.9 84/02/29 17:23:24 sklower Exp $  *$Locker:  $  *  * Copyright (c) 1982, by the Regents, University of California  *  *			-[Tue Mar 22 15:42:27 1983 by layer]-  *  * "quick" functions file.  *  * This is written in assembler but must be passed through the C preprocessor  * before being assembled.  *  */
end_comment
...
directive|ifdef
name|SPISFP
comment|/*  * This is necessary on the sun-II beta testing version since the C  * compiler makes refence to temporaries and restoring registers relative  * to the stack pointer.  See explicative comments in ../vax/qfuncl.c  * for Iretfrm and Ipushf  */
operator|.
name|globl
name|_Ipushf
name|_Ipushf
...
directive|endif
comment|/*  * Ancillary code for small thunks generated so that  * c routines can be passed the address of something  * to call which will pass onto lisp functions  */
operator|.
name|globl
name|_thcpy
name|_thcpy
...
directive|ifndef
name|SPISFP
comment|/* Copyright (c) 1982, Regents, University of California    This is here because for the sun II beta test version, you    can't do alloca */
operator|.
name|text
operator|.
name|globl
...
directive|endif
name|rts
comment|/*  * We want to be able to redefine read and write to check  * certain lisp values.  Rather than have 4 variants, we  * put the assembly language (obtained by adb rather than  * violating source) here under ifdef control.  */
operator|.
name|globl
name|__read
operator|.
