...
begin_comment
comment|/* **  MODIFY -- converts any relation to the specified **		storage structure ** **	arguments: **	0 - relation name **	1 - storage structure ("heap", "cheap", "hash", "chash", **		"isam", "cisam", "btree") **	2 - "name" for attribute names, or "num" for numbers **	3 - key1 **	4 - key2 **	    . **	    . **	i - null **	i+1 - option name (e.g., "fillfactor") **	i+2 - option value **	    . **	    . ** **	If all the options default, parameter i -> pc are omitted. **	If no keys are provided, parameter 2 is omitted. ** **	History: **		12/28/78 (rse) -- added descending sort option **		9/12/78 -- (marc) modified to give error message **			5519 when someone tries to modify a view. **			For this I split the openr mode 0 into 2, **			mode -1 then -2. **		9/25/78 -- (marc) 2 openr's replaced by one **			as view error code put in openr ** */
end_comment
...
name|PGPTRSIZ
expr_stmt|;
comment|/* determine fill factor for directory pages */
name|fill_leave
operator|=
operator|(
literal|100
...
name|syserr
argument_list|(
literal|"modify: bldbindex: get %d\n"
argument_list|,
name|tmp
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 				stuff_page(&temptup.childtid,&pageid); 				key = temptup.childtup; 				for (tmp = 0; keyx[tmp] != 0; tmp++) 				{ 					len = d->relfrml[keyx[tmp]]& I1MASK; 					bmove(&((char *)&tuple)[d->reloff[keyx[tmp]]], key, len); 					key += len; 				} 				*/
name|printf
argument_list|(
literal|"primary "
argument_list|)
