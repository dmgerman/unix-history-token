...
name|sccsid
init|=
literal|"@(#)mkfs.c	1.10 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * make file system for cylinder-group style file systems  *  * usage: mkfs fs proto  * or: mkfs size [ bsize frag nsect ntrak cpg ]  */
end_comment
...
name|printf
argument_list|(
literal|"usage: mkfs sblock proto/size [ bsize frag nsect ntrak cpg ]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
parameter_list|)
define|\
value|((fs).fs_nsect * (fs).fs_ntrak * (fs).fs_cpg/NSPB(&sblock)> MAXBPG(&sblock))
if|if
condition|(
name|argc
operator|>
