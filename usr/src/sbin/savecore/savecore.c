begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_decl_stmt
specifier|static
name|char
modifier|*
name|sccsid
init|=
literal|"@(#)savecore.c	4.7.1.1 (Berkeley) 82/10/24"
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * savecore  */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<nlist.h>
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/dir.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<sys/fs.h>
end_include

begin_include
include|#
directive|include
file|<time.h>
end_include

begin_define
define|#
directive|define
name|DAY
value|(60L*60L*24L)
end_define

begin_define
define|#
directive|define
name|LEEWAY
value|(3*DAY)
end_define

begin_define
define|#
directive|define
name|eq
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|(!strcmp(a,b))
end_define

begin_define
define|#
directive|define
name|ok
parameter_list|(
name|number
parameter_list|)
value|((number)&0x7fffffff)
end_define

begin_define
define|#
directive|define
name|SHUTDOWNLOG
value|"/usr/adm/shutdownlog"
end_define

begin_decl_stmt
name|struct
name|nlist
name|nl
index|[]
init|=
block|{
define|#
directive|define
name|X_DUMPDEV
value|0
block|{
literal|"_dumpdev"
block|}
block|,
define|#
directive|define
name|X_DUMPLO
value|1
block|{
literal|"_dumplo"
block|}
block|,
define|#
directive|define
name|X_TIME
value|2
block|{
literal|"_time"
block|}
block|,
define|#
directive|define
name|X_PHYSMEM
value|3
block|{
literal|"_physmem"
block|}
block|,
define|#
directive|define
name|X_VERSION
value|4
block|{
literal|"_version"
block|}
block|,
define|#
directive|define
name|X_PANICSTR
value|5
block|{
literal|"_panicstr"
block|}
block|,
define|#
directive|define
name|X_DOADUMP
value|6
block|{
literal|"_doadump"
block|}
block|,
block|{
literal|0
block|}
block|, }
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  *	this magic number is found in the kernel at label "doadump"  *  *	It is derived as follows:  *  *		doadump:	nop			01  *				nop			01  *				bicl2 $...		ca  *							8f  *  *	Thus, it is likely to be moderately stable, even across  *	operating system releases.  */
end_comment

begin_define
define|#
directive|define
name|DUMPMAG
value|0x8fca0101
end_define

begin_decl_stmt
name|char
modifier|*
name|system
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|dirname
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* directory to save dumps in */
end_comment

begin_decl_stmt
name|char
modifier|*
name|ddname
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* name of dump device */
end_comment

begin_function_decl
name|char
modifier|*
name|find_dev
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|dev_t
name|dumpdev
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* dump device */
end_comment

begin_decl_stmt
name|time_t
name|dumptime
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* time the dump was taken */
end_comment

begin_decl_stmt
name|int
name|dumplo
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* where dump starts on dumpdev */
end_comment

begin_decl_stmt
name|int
name|physmem
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* amount of memory in machine */
end_comment

begin_decl_stmt
name|time_t
name|now
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* current date */
end_comment

begin_function_decl
name|char
modifier|*
name|path
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|unsigned
name|malloc
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|char
modifier|*
name|ctime
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|char
name|vers
index|[
literal|80
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|core_vers
index|[
literal|80
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|panic_mesg
index|[
literal|80
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|panicstr
decl_stmt|;
end_decl_stmt

begin_function_decl
name|off_t
name|lseek
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|off_t
name|Lseek
parameter_list|()
function_decl|;
end_function_decl

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
name|int
name|argc
decl_stmt|;
block|{
if|if
condition|(
name|argc
operator|!=
literal|2
operator|&&
name|argc
operator|!=
literal|3
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: savecore dirname [ system ]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|dirname
operator|=
name|argv
index|[
literal|1
index|]
expr_stmt|;
if|if
condition|(
name|argc
operator|==
literal|3
condition|)
name|system
operator|=
name|argv
index|[
literal|2
index|]
expr_stmt|;
if|if
condition|(
name|access
argument_list|(
name|dirname
argument_list|,
literal|2
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|dirname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|read_kmem
argument_list|()
expr_stmt|;
if|if
condition|(
name|dump_exists
argument_list|()
condition|)
block|{
operator|(
name|void
operator|)
name|time
argument_list|(
operator|&
name|now
argument_list|)
expr_stmt|;
name|check_kmem
argument_list|()
expr_stmt|;
name|log_entry
argument_list|()
expr_stmt|;
if|if
condition|(
name|get_crashtime
argument_list|()
operator|&&
name|check_space
argument_list|()
condition|)
block|{
name|save_core
argument_list|()
expr_stmt|;
name|clear_dump
argument_list|()
expr_stmt|;
block|}
else|else
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_function
name|int
name|dump_exists
parameter_list|()
block|{
specifier|register
name|int
name|dumpfd
decl_stmt|;
name|int
name|word
decl_stmt|;
name|dumpfd
operator|=
name|Open
argument_list|(
name|ddname
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|dumpfd
argument_list|,
call|(
name|off_t
call|)
argument_list|(
name|dumplo
operator|+
name|ok
argument_list|(
name|nl
index|[
name|X_DOADUMP
index|]
operator|.
name|n_value
argument_list|)
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Read
argument_list|(
name|dumpfd
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|word
argument_list|,
sizeof|sizeof
name|word
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|dumpfd
argument_list|)
expr_stmt|;
return|return
operator|(
name|word
operator|==
name|DUMPMAG
operator|)
return|;
block|}
end_function

begin_macro
name|clear_dump
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|dumpfd
decl_stmt|;
name|int
name|zero
init|=
literal|0
decl_stmt|;
name|dumpfd
operator|=
name|Open
argument_list|(
name|ddname
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|dumpfd
argument_list|,
call|(
name|off_t
call|)
argument_list|(
name|dumplo
operator|+
name|ok
argument_list|(
name|nl
index|[
name|X_DOADUMP
index|]
operator|.
name|n_value
argument_list|)
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Write
argument_list|(
name|dumpfd
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|zero
argument_list|,
sizeof|sizeof
name|zero
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|dumpfd
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
name|char
modifier|*
name|find_dev
parameter_list|(
name|dev
parameter_list|,
name|type
parameter_list|)
specifier|register
name|dev_t
name|dev
decl_stmt|;
specifier|register
name|int
name|type
decl_stmt|;
block|{
specifier|register
name|DIR
modifier|*
name|dfd
init|=
name|opendir
argument_list|(
literal|"/dev"
argument_list|)
decl_stmt|;
name|struct
name|direct
modifier|*
name|dir
decl_stmt|;
name|struct
name|stat
name|statb
decl_stmt|;
specifier|static
name|char
name|devname
index|[
name|MAXPATHLEN
operator|+
literal|1
index|]
decl_stmt|;
name|char
modifier|*
name|dp
decl_stmt|;
name|strcpy
argument_list|(
name|devname
argument_list|,
literal|"/dev/"
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|dir
operator|=
name|readdir
argument_list|(
name|dfd
argument_list|)
operator|)
condition|)
block|{
name|strcpy
argument_list|(
name|devname
operator|+
literal|5
argument_list|,
name|dir
operator|->
name|d_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|devname
argument_list|,
operator|&
name|statb
argument_list|)
condition|)
block|{
name|perror
argument_list|(
name|devname
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
operator|(
name|statb
operator|.
name|st_mode
operator|&
name|S_IFMT
operator|)
operator|!=
name|type
condition|)
continue|continue;
if|if
condition|(
name|dev
operator|==
name|statb
operator|.
name|st_rdev
condition|)
block|{
name|closedir
argument_list|(
name|dfd
argument_list|)
expr_stmt|;
name|dp
operator|=
operator|(
name|char
operator|*
operator|)
name|malloc
argument_list|(
name|strlen
argument_list|(
name|devname
argument_list|)
operator|+
literal|1
argument_list|)
expr_stmt|;
name|strcpy
argument_list|(
name|dp
argument_list|,
name|devname
argument_list|)
expr_stmt|;
return|return
name|dp
return|;
block|}
block|}
name|closedir
argument_list|(
name|dfd
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Can't find device %d,%d\n"
argument_list|,
name|major
argument_list|(
name|dev
argument_list|)
argument_list|,
name|minor
argument_list|(
name|dev
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
comment|/*NOTREACHED*/
block|}
end_function

begin_macro
name|read_kmem
argument_list|()
end_macro

begin_block
block|{
name|int
name|kmem
decl_stmt|;
name|FILE
modifier|*
name|fp
decl_stmt|;
specifier|register
name|char
modifier|*
name|cp
decl_stmt|;
name|nlist
argument_list|(
literal|"/vmunix"
argument_list|,
name|nl
argument_list|)
expr_stmt|;
if|if
condition|(
name|nl
index|[
name|X_DUMPDEV
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"/vmunix: dumpdev not in namelist\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nl
index|[
name|X_DUMPLO
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"/vmunix: dumplo not in namelist\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nl
index|[
name|X_TIME
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"/vmunix: time not in namelist\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nl
index|[
name|X_PHYSMEM
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"/vmunix: physmem not in namelist\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nl
index|[
name|X_VERSION
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"/vmunix: version not in namelist\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nl
index|[
name|X_PANICSTR
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"/vmunix: panicstr not in namelist\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nl
index|[
name|X_DOADUMP
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"/vmunix: doadump not in namelist\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|kmem
operator|=
name|Open
argument_list|(
literal|"/dev/kmem"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|kmem
argument_list|,
operator|(
name|long
operator|)
name|nl
index|[
name|X_DUMPDEV
index|]
operator|.
name|n_value
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Read
argument_list|(
name|kmem
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|dumpdev
argument_list|,
sizeof|sizeof
name|dumpdev
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|kmem
argument_list|,
operator|(
name|long
operator|)
name|nl
index|[
name|X_DUMPLO
index|]
operator|.
name|n_value
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Read
argument_list|(
name|kmem
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|dumplo
argument_list|,
sizeof|sizeof
name|dumplo
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|kmem
argument_list|,
operator|(
name|long
operator|)
name|nl
index|[
name|X_PHYSMEM
index|]
operator|.
name|n_value
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Read
argument_list|(
name|kmem
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|physmem
argument_list|,
sizeof|sizeof
name|physmem
argument_list|)
expr_stmt|;
name|dumplo
operator|*=
literal|512L
expr_stmt|;
name|ddname
operator|=
name|find_dev
argument_list|(
name|dumpdev
argument_list|,
name|S_IFBLK
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|fp
operator|=
name|fdopen
argument_list|(
name|kmem
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Couldn't fdopen kmem\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|system
condition|)
return|return;
name|fseek
argument_list|(
name|fp
argument_list|,
operator|(
name|long
operator|)
name|nl
index|[
name|X_VERSION
index|]
operator|.
name|n_value
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|fgets
argument_list|(
name|vers
argument_list|,
sizeof|sizeof
name|vers
argument_list|,
name|fp
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|check_kmem
argument_list|()
end_macro

begin_block
block|{
name|FILE
modifier|*
name|fp
decl_stmt|;
specifier|register
name|char
modifier|*
name|cp
decl_stmt|;
if|if
condition|(
operator|(
name|fp
operator|=
name|fopen
argument_list|(
name|ddname
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|perror
argument_list|(
name|ddname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|fseek
argument_list|(
name|fp
argument_list|,
call|(
name|off_t
call|)
argument_list|(
name|dumplo
operator|+
name|ok
argument_list|(
name|nl
index|[
name|X_VERSION
index|]
operator|.
name|n_value
argument_list|)
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|fgets
argument_list|(
name|core_vers
argument_list|,
sizeof|sizeof
name|core_vers
argument_list|,
name|fp
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|eq
argument_list|(
name|vers
argument_list|,
name|core_vers
argument_list|)
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Warning: vmunix version mismatch:\n\t%sand\n\t%s"
argument_list|,
name|vers
argument_list|,
name|core_vers
argument_list|)
expr_stmt|;
name|fp
operator|=
name|fopen
argument_list|(
name|ddname
argument_list|,
literal|"r"
argument_list|)
expr_stmt|;
name|fseek
argument_list|(
name|fp
argument_list|,
call|(
name|off_t
call|)
argument_list|(
name|dumplo
operator|+
name|ok
argument_list|(
name|nl
index|[
name|X_PANICSTR
index|]
operator|.
name|n_value
argument_list|)
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|fread
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|panicstr
argument_list|,
sizeof|sizeof
name|panicstr
argument_list|,
literal|1
argument_list|,
name|fp
argument_list|)
expr_stmt|;
if|if
condition|(
name|panicstr
condition|)
block|{
name|fseek
argument_list|(
name|fp
argument_list|,
name|dumplo
operator|+
name|ok
argument_list|(
name|panicstr
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|cp
operator|=
name|panic_mesg
expr_stmt|;
do|do
operator|*
name|cp
operator|=
name|getc
argument_list|(
name|fp
argument_list|)
expr_stmt|;
do|while
condition|(
operator|*
name|cp
operator|++
condition|)
do|;
block|}
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|get_crashtime
argument_list|()
end_macro

begin_block
block|{
name|int
name|dumpfd
decl_stmt|;
name|time_t
name|clobber
init|=
operator|(
name|time_t
operator|)
literal|0
decl_stmt|;
if|if
condition|(
name|system
condition|)
return|return
operator|(
literal|1
operator|)
return|;
name|dumpfd
operator|=
name|Open
argument_list|(
name|ddname
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|dumpfd
argument_list|,
call|(
name|off_t
call|)
argument_list|(
name|dumplo
operator|+
name|ok
argument_list|(
name|nl
index|[
name|X_TIME
index|]
operator|.
name|n_value
argument_list|)
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Read
argument_list|(
name|dumpfd
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|dumptime
argument_list|,
sizeof|sizeof
name|dumptime
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|dumpfd
argument_list|)
expr_stmt|;
if|if
condition|(
name|dumptime
operator|==
literal|0
condition|)
block|{
ifdef|#
directive|ifdef
name|DEBUG
name|printf
argument_list|(
literal|"dump time is 0\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
literal|0
return|;
block|}
name|printf
argument_list|(
literal|"System went down at %s"
argument_list|,
name|ctime
argument_list|(
operator|&
name|dumptime
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|dumptime
operator|<
name|now
operator|-
name|LEEWAY
operator|||
name|dumptime
operator|>
name|now
operator|+
name|LEEWAY
condition|)
block|{
name|printf
argument_list|(
literal|"Dump time is unreasonable\n"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
literal|1
return|;
block|}
end_block

begin_function
name|char
modifier|*
name|path
parameter_list|(
name|file
parameter_list|)
name|char
modifier|*
name|file
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|cp
init|=
operator|(
name|char
operator|*
operator|)
name|malloc
argument_list|(
name|strlen
argument_list|(
name|file
argument_list|)
operator|+
name|strlen
argument_list|(
name|dirname
argument_list|)
operator|+
literal|2
argument_list|)
decl_stmt|;
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|cp
argument_list|,
name|dirname
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|strcat
argument_list|(
name|cp
argument_list|,
literal|"/"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|strcat
argument_list|(
name|cp
argument_list|,
name|file
argument_list|)
expr_stmt|;
return|return
name|cp
return|;
block|}
end_function

begin_macro
name|check_space
argument_list|()
end_macro

begin_block
block|{
name|struct
name|stat
name|dsb
decl_stmt|;
specifier|register
name|char
modifier|*
name|ddev
decl_stmt|;
name|int
name|dfd
decl_stmt|,
name|freespace
decl_stmt|;
name|struct
name|fs
name|fs
decl_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|dirname
argument_list|,
operator|&
name|dsb
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|dirname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|ddev
operator|=
name|find_dev
argument_list|(
name|dsb
operator|.
name|st_dev
argument_list|,
name|S_IFBLK
argument_list|)
expr_stmt|;
name|dfd
operator|=
name|Open
argument_list|(
name|ddev
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|dfd
argument_list|,
call|(
name|long
call|)
argument_list|(
name|SBLOCK
operator|*
name|DEV_BSIZE
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Read
argument_list|(
name|dfd
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|fs
argument_list|,
sizeof|sizeof
name|fs
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|dfd
argument_list|)
expr_stmt|;
comment|/* 	 * Minimum free space is in terms of kilobytes. 	 * We ignore the "percent free space to be maintained" 	 * parameter in the super block in the believe that a 	 * crash dump is more important than the (temporary) 	 * degradation in performance one might experience. 	 */
name|freespace
operator|=
operator|(
name|fs
operator|.
name|fs_cstotal
operator|.
name|cs_nbfree
operator|*
name|fs
operator|.
name|fs_bsize
operator|+
name|fs
operator|.
name|fs_cstotal
operator|.
name|cs_nffree
operator|*
name|fs
operator|.
name|fs_fsize
operator|)
operator|/
literal|1024
expr_stmt|;
if|if
condition|(
name|read_number
argument_list|(
literal|"minfree"
argument_list|)
operator|>
name|freespace
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Dump omitted, not enough space on device\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
name|fs
operator|.
name|fs_cstotal
operator|.
name|cs_nbfree
operator|*
name|fs
operator|.
name|fs_frag
operator|+
name|fs
operator|.
name|fs_cstotal
operator|.
name|cs_nffree
operator|<
name|fs
operator|.
name|fs_dsize
operator|*
name|fs
operator|.
name|fs_minfree
operator|/
literal|100
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Dump performed, but free space threshold crossed\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_macro
name|read_number
argument_list|(
argument|fn
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|fn
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
name|lin
index|[
literal|80
index|]
decl_stmt|;
specifier|register
name|FILE
modifier|*
name|fp
decl_stmt|;
if|if
condition|(
operator|(
name|fp
operator|=
name|fopen
argument_list|(
name|path
argument_list|(
name|fn
argument_list|)
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|fgets
argument_list|(
name|lin
argument_list|,
literal|80
argument_list|,
name|fp
argument_list|)
operator|==
name|NULL
condition|)
block|{
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
return|return
name|atoi
argument_list|(
name|lin
argument_list|)
return|;
block|}
end_block

begin_macro
name|save_core
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|n
decl_stmt|;
name|char
name|buffer
index|[
literal|32
operator|*
name|NBPG
index|]
decl_stmt|;
specifier|register
name|char
modifier|*
name|cp
init|=
name|buffer
decl_stmt|;
specifier|register
name|int
name|ifd
decl_stmt|,
name|ofd
decl_stmt|,
name|bounds
decl_stmt|;
specifier|register
name|FILE
modifier|*
name|fp
decl_stmt|;
name|bounds
operator|=
name|read_number
argument_list|(
literal|"bounds"
argument_list|)
expr_stmt|;
name|ifd
operator|=
name|Open
argument_list|(
name|system
condition|?
name|system
else|:
literal|"/vmunix"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|sprintf
argument_list|(
name|cp
argument_list|,
literal|"vmunix.%d"
argument_list|,
name|bounds
argument_list|)
expr_stmt|;
name|ofd
operator|=
name|Create
argument_list|(
name|path
argument_list|(
name|cp
argument_list|)
argument_list|,
literal|0644
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|n
operator|=
name|Read
argument_list|(
name|ifd
argument_list|,
name|cp
argument_list|,
name|BUFSIZ
argument_list|)
operator|)
operator|>
literal|0
condition|)
name|Write
argument_list|(
name|ofd
argument_list|,
name|cp
argument_list|,
name|n
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|ifd
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|ofd
argument_list|)
expr_stmt|;
name|ifd
operator|=
name|Open
argument_list|(
name|ddname
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|sprintf
argument_list|(
name|cp
argument_list|,
literal|"vmcore.%d"
argument_list|,
name|bounds
argument_list|)
expr_stmt|;
name|ofd
operator|=
name|Create
argument_list|(
name|path
argument_list|(
name|cp
argument_list|)
argument_list|,
literal|0644
argument_list|)
expr_stmt|;
name|Lseek
argument_list|(
name|ifd
argument_list|,
operator|(
name|off_t
operator|)
name|dumplo
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"Saving %d bytes of image in vmcore.%d\n"
argument_list|,
name|NBPG
operator|*
name|physmem
argument_list|,
name|bounds
argument_list|)
expr_stmt|;
while|while
condition|(
name|physmem
operator|>
literal|0
condition|)
block|{
name|n
operator|=
name|Read
argument_list|(
name|ifd
argument_list|,
name|cp
argument_list|,
operator|(
name|physmem
operator|>
literal|32
condition|?
literal|32
else|:
name|physmem
operator|)
operator|*
name|NBPG
argument_list|)
expr_stmt|;
name|Write
argument_list|(
name|ofd
argument_list|,
name|cp
argument_list|,
name|n
argument_list|)
expr_stmt|;
name|physmem
operator|-=
name|n
operator|/
name|NBPG
expr_stmt|;
block|}
name|close
argument_list|(
name|ifd
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|ofd
argument_list|)
expr_stmt|;
name|fp
operator|=
name|fopen
argument_list|(
name|path
argument_list|(
literal|"bounds"
argument_list|)
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%d\n"
argument_list|,
name|bounds
operator|+
literal|1
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
end_block

begin_decl_stmt
name|char
modifier|*
name|days
index|[]
init|=
block|{
literal|"Sun"
block|,
literal|"Mon"
block|,
literal|"Tue"
block|,
literal|"Wed"
block|,
literal|"Thu"
block|,
literal|"Fri"
block|,
literal|"Sat"
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|months
index|[]
init|=
block|{
literal|"Jan"
block|,
literal|"Feb"
block|,
literal|"Mar"
block|,
literal|"Apr"
block|,
literal|"May"
block|,
literal|"Jun"
block|,
literal|"Jul"
block|,
literal|"Aug"
block|,
literal|"Sep"
block|,
literal|"Oct"
block|,
literal|"Nov"
block|,
literal|"Dec"
block|}
decl_stmt|;
end_decl_stmt

begin_macro
name|log_entry
argument_list|()
end_macro

begin_block
block|{
name|FILE
modifier|*
name|fp
decl_stmt|;
name|struct
name|tm
modifier|*
name|tm
decl_stmt|,
modifier|*
name|localtime
argument_list|()
decl_stmt|;
name|tm
operator|=
name|localtime
argument_list|(
operator|&
name|now
argument_list|)
expr_stmt|;
name|fp
operator|=
name|fopen
argument_list|(
literal|"/usr/adm/shutdownlog"
argument_list|,
literal|"a"
argument_list|)
expr_stmt|;
if|if
condition|(
name|fp
operator|==
literal|0
condition|)
return|return;
name|fseek
argument_list|(
name|fp
argument_list|,
literal|0L
argument_list|,
literal|2
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|"%02d:%02d  %s %s %2d, %4d.  Reboot"
argument_list|,
name|tm
operator|->
name|tm_hour
argument_list|,
name|tm
operator|->
name|tm_min
argument_list|,
name|days
index|[
name|tm
operator|->
name|tm_wday
index|]
argument_list|,
name|months
index|[
name|tm
operator|->
name|tm_mon
index|]
argument_list|,
name|tm
operator|->
name|tm_mday
argument_list|,
name|tm
operator|->
name|tm_year
operator|+
literal|1900
argument_list|)
expr_stmt|;
if|if
condition|(
name|panicstr
condition|)
name|fprintf
argument_list|(
name|fp
argument_list|,
literal|" after panic: %s\n"
argument_list|,
name|panic_mesg
argument_list|)
expr_stmt|;
else|else
name|putc
argument_list|(
literal|'\n'
argument_list|,
name|fp
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Versions of std routines that exit on error.  */
end_comment

begin_macro
name|Open
argument_list|(
argument|name
argument_list|,
argument|rw
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|name
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|rw
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|fd
decl_stmt|;
if|if
condition|(
operator|(
name|fd
operator|=
name|open
argument_list|(
name|name
argument_list|,
name|rw
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|name
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
name|fd
return|;
block|}
end_block

begin_macro
name|Read
argument_list|(
argument|fd
argument_list|,
argument|buff
argument_list|,
argument|size
argument_list|)
end_macro

begin_decl_stmt
name|int
name|fd
decl_stmt|,
name|size
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|buff
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|ret
decl_stmt|;
if|if
condition|(
operator|(
name|ret
operator|=
name|read
argument_list|(
name|fd
argument_list|,
name|buff
argument_list|,
name|size
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
literal|"read"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
name|ret
return|;
block|}
end_block

begin_function
name|off_t
name|Lseek
parameter_list|(
name|fd
parameter_list|,
name|off
parameter_list|,
name|flag
parameter_list|)
name|int
name|fd
decl_stmt|,
name|flag
decl_stmt|;
name|long
name|off
decl_stmt|;
block|{
name|long
name|ret
decl_stmt|;
if|if
condition|(
operator|(
name|ret
operator|=
name|lseek
argument_list|(
name|fd
argument_list|,
name|off
argument_list|,
name|flag
argument_list|)
operator|)
operator|==
operator|-
literal|1L
condition|)
block|{
name|perror
argument_list|(
literal|"lseek"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
name|ret
return|;
block|}
end_function

begin_macro
name|Create
argument_list|(
argument|file
argument_list|,
argument|mode
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|file
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|mode
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|fd
decl_stmt|;
if|if
condition|(
operator|(
name|fd
operator|=
name|creat
argument_list|(
name|file
argument_list|,
name|mode
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
name|fd
return|;
block|}
end_block

begin_macro
name|Write
argument_list|(
argument|fd
argument_list|,
argument|buf
argument_list|,
argument|size
argument_list|)
end_macro

begin_decl_stmt
name|int
name|fd
decl_stmt|,
name|size
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|buf
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|write
argument_list|(
name|fd
argument_list|,
name|buf
argument_list|,
name|size
argument_list|)
operator|<
name|size
condition|)
block|{
name|perror
argument_list|(
literal|"write"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
end_block

end_unit

