begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1991 The Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1991 The Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)dumplfs.c	5.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<ufs/dinode.h>
end_include

begin_include
include|#
directive|include
file|"lfs.h"
end_include

begin_comment
comment|/* XXX fix this */
end_comment

begin_include
include|#
directive|include
file|<time.h>
end_include

begin_include
include|#
directive|include
file|<fstab.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|"extern.h"
end_include

begin_decl_stmt
specifier|static
name|void
name|dump_dinode
name|__P
argument_list|(
operator|(
expr|struct
name|dinode
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|dump_ifile
name|__P
argument_list|(
operator|(
name|int
operator|,
name|LFS_SUPER
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|dump_ipage_ifile
name|__P
argument_list|(
operator|(
name|int
operator|,
name|IFILE
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|dump_ipage_segusage
name|__P
argument_list|(
operator|(
name|LFS_SUPER
operator|*
operator|,
name|int
operator|,
name|IFILE
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|daddr_t
name|dump_segment
name|__P
argument_list|(
operator|(
name|int
operator|,
name|daddr_t
operator|,
name|LFS_SUPER
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|dump_sum
name|__P
argument_list|(
operator|(
name|SEGSUM
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|dump_super
name|__P
argument_list|(
operator|(
name|LFS_SUPER
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|usage
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|daddr_shift
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|special
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Segment Usage formats */
end_comment

begin_define
define|#
directive|define
name|print_suheader
define|\
value|(void)printf("segnum\tstatus\tnbytes\t\tlastmod\n")
end_define

begin_define
define|#
directive|define
name|print_suentry
parameter_list|(
name|i
parameter_list|,
name|sp
parameter_list|)
define|\
value|(void)printf("%d\t%s\t%d\t%s", \ 	    i, (((sp)->su_flags) ? "DIRTY" : "CLEAN"), \ 	    (sp)->su_nbytes, ctime((time_t *)&(sp)->su_lastmod))
end_define

begin_comment
comment|/* Ifile formats */
end_comment

begin_define
define|#
directive|define
name|print_iheader
define|\
value|(void)printf("inum\tstatus\tversion\tdaddr\t\tatime\tfreeptr\n")
end_define

begin_define
define|#
directive|define
name|print_ientry
parameter_list|(
name|i
parameter_list|,
name|ip
parameter_list|)
define|\
value|if (ip->if_daddr == LFS_UNUSED_DADDR) \ 		(void)printf("%d\tFREE\t%d\t \t\t \t%d\n", \ 		    i, ip->if_version, ip->if_nextfree); \ 	else \ 		(void)printf("%d\tINUSE\t%d\t%8X    \t%s\n", \ 		    i, ip->if_version, ip->if_daddr, \ 		    ctime((time_t *)&ip->if_st_atime))
end_define

begin_function
name|int
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
name|LFS_SUPER
name|lfs_sb1
decl_stmt|,
name|lfs_sb2
decl_stmt|,
modifier|*
name|lfs_master
decl_stmt|;
name|daddr_t
name|seg_addr
decl_stmt|,
name|next_seg
decl_stmt|;
name|int
name|ch
decl_stmt|,
name|do_allsb
decl_stmt|,
name|do_ientries
decl_stmt|,
name|fd
decl_stmt|;
name|do_allsb
operator|=
literal|0
expr_stmt|;
name|do_ientries
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
name|ch
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"is"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|ch
condition|)
block|{
case|case
literal|'i'
case|:
comment|/* Dump ifile entries */
name|do_ientries
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'s'
case|:
comment|/* Dump all superblocks */
name|do_allsb
operator|=
literal|1
expr_stmt|;
break|break;
default|default:
name|usage
argument_list|()
expr_stmt|;
block|}
name|argc
operator|-=
name|optind
expr_stmt|;
name|argv
operator|+=
name|optind
expr_stmt|;
if|if
condition|(
name|argc
operator|!=
literal|1
condition|)
name|usage
argument_list|()
expr_stmt|;
name|special
operator|=
name|argv
index|[
literal|0
index|]
expr_stmt|;
if|if
condition|(
operator|(
name|fd
operator|=
name|open
argument_list|(
name|special
argument_list|,
name|O_RDONLY
argument_list|,
literal|0
argument_list|)
operator|)
operator|<
literal|0
condition|)
name|err
argument_list|(
literal|"%s: %s"
argument_list|,
name|special
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Read the first superblock */
name|get
argument_list|(
name|fd
argument_list|,
name|LFS_LABELPAD
argument_list|,
operator|&
name|lfs_sb1
argument_list|,
sizeof|sizeof
argument_list|(
name|LFS_SUPER
argument_list|)
argument_list|)
expr_stmt|;
name|daddr_shift
operator|=
name|lfs_sb1
operator|.
name|lfs_bshift
operator|-
name|lfs_sb1
operator|.
name|lfs_fsbtodb
expr_stmt|;
comment|/* 	 * Read the second superblock and figure out which check point is 	 * most up to date. 	 */
name|get
argument_list|(
name|fd
argument_list|,
name|lfs_sb1
operator|.
name|lfs_sboffs
index|[
literal|1
index|]
operator|<<
name|daddr_shift
argument_list|,
operator|&
name|lfs_sb2
argument_list|,
sizeof|sizeof
argument_list|(
name|LFS_SUPER
argument_list|)
argument_list|)
expr_stmt|;
name|lfs_master
operator|=
operator|&
name|lfs_sb1
expr_stmt|;
if|if
condition|(
name|lfs_sb1
operator|.
name|lfs_tstamp
operator|<
name|lfs_sb2
operator|.
name|lfs_tstamp
condition|)
name|lfs_master
operator|=
operator|&
name|lfs_sb2
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Master Superblock:\n"
argument_list|)
expr_stmt|;
name|dump_super
argument_list|(
name|lfs_master
argument_list|)
expr_stmt|;
name|dump_ifile
argument_list|(
name|fd
argument_list|,
name|lfs_master
argument_list|,
name|do_ientries
argument_list|)
expr_stmt|;
name|next_seg
operator|=
name|LFS_UNUSED_DADDR
expr_stmt|;
for|for
control|(
name|seg_addr
operator|=
name|lfs_master
operator|->
name|lfs_lastseg
init|;
name|seg_addr
operator|!=
name|LFS_UNUSED_DADDR
operator|&&
name|next_seg
operator|!=
name|lfs_master
operator|->
name|lfs_lastseg
condition|;
name|seg_addr
operator|=
name|next_seg
control|)
name|next_seg
operator|=
name|dump_segment
argument_list|(
name|fd
argument_list|,
name|seg_addr
argument_list|,
name|lfs_master
argument_list|,
name|do_allsb
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * We are reading all the blocks of an inode and dumping out the ifile table.  * This code can be made tigher, but this is a first pass at getting the stuff  * printed out rather than making this code incredibly efficient.  */
end_comment

begin_function
specifier|static
name|void
name|dump_ifile
parameter_list|(
name|fd
parameter_list|,
name|lfsp
parameter_list|,
name|do_ientries
parameter_list|)
name|int
name|fd
decl_stmt|;
name|LFS_SUPER
modifier|*
name|lfsp
decl_stmt|;
name|int
name|do_ientries
decl_stmt|;
block|{
name|IFILE
modifier|*
name|ipage
decl_stmt|;
name|struct
name|dinode
modifier|*
name|dip
decl_stmt|,
modifier|*
name|dpage
decl_stmt|;
name|daddr_t
name|addr
decl_stmt|,
modifier|*
name|addrp
decl_stmt|,
modifier|*
name|iaddrp
decl_stmt|;
name|daddr_t
modifier|*
name|dindir
decl_stmt|;
name|daddr_t
modifier|*
name|indir
decl_stmt|;
name|int
name|block_limit
decl_stmt|,
name|nblocks
decl_stmt|;
name|int
name|i
decl_stmt|,
name|j
decl_stmt|,
name|inum
decl_stmt|,
name|nsupb
decl_stmt|,
name|psize
decl_stmt|;
name|psize
operator|=
name|lfsp
operator|->
name|lfs_bsize
expr_stmt|;
name|addr
operator|=
name|lfsp
operator|->
name|lfs_idaddr
expr_stmt|;
name|nsupb
operator|=
name|lfsp
operator|->
name|lfs_bsize
operator|/
sizeof|sizeof
argument_list|(
name|SEGUSAGE
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|dip
operator|=
name|dpage
operator|=
name|malloc
argument_list|(
name|psize
argument_list|)
operator|)
condition|)
name|err
argument_list|(
literal|"%s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|get
argument_list|(
name|fd
argument_list|,
name|addr
operator|<<
name|daddr_shift
argument_list|,
name|dip
argument_list|,
name|psize
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|lfsp
operator|->
name|lfs_inopb
condition|;
name|i
operator|++
operator|,
name|dip
operator|++
control|)
if|if
condition|(
name|dip
operator|->
name|di_inum
operator|==
name|LFS_IFILE_INUM
condition|)
break|break;
if|if
condition|(
name|i
operator|>=
name|lfsp
operator|->
name|lfs_inopb
condition|)
name|err
argument_list|(
literal|"unable to locate ifile inode"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\nIFILE inode\n"
argument_list|)
expr_stmt|;
name|dump_dinode
argument_list|(
name|dip
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\nIFILE contents\n"
argument_list|)
expr_stmt|;
name|print_suheader
expr_stmt|;
name|nblocks
operator|=
name|dip
operator|->
name|di_size
operator|>>
name|lfsp
operator|->
name|lfs_bshift
expr_stmt|;
name|block_limit
operator|=
name|MIN
argument_list|(
name|nblocks
argument_list|,
name|NDADDR
argument_list|)
expr_stmt|;
comment|/* Get the direct block */
if|if
condition|(
operator|(
name|ipage
operator|=
name|malloc
argument_list|(
name|psize
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|err
argument_list|(
literal|"%s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|inum
operator|=
literal|0
operator|,
name|addrp
operator|=
name|dip
operator|->
name|di_db
operator|,
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|block_limit
condition|;
name|i
operator|++
operator|,
name|addrp
operator|++
control|)
block|{
name|get
argument_list|(
name|fd
argument_list|,
operator|*
name|addrp
operator|<<
name|daddr_shift
argument_list|,
name|ipage
argument_list|,
name|psize
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|<
name|lfsp
operator|->
name|lfs_segtabsz
condition|)
block|{
name|inum
operator|=
name|dump_ipage_segusage
argument_list|(
name|lfsp
argument_list|,
name|inum
argument_list|,
name|ipage
argument_list|,
name|nsupb
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|inum
condition|)
if|if
condition|(
operator|!
name|do_ientries
condition|)
goto|goto
name|e0
goto|;
else|else
name|print_iheader
expr_stmt|;
block|}
else|else
name|inum
operator|=
name|dump_ipage_ifile
argument_list|(
name|inum
argument_list|,
name|ipage
argument_list|,
name|lfsp
operator|->
name|lfs_ifpb
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nblocks
operator|<=
name|NDADDR
condition|)
goto|goto
name|e0
goto|;
comment|/* Dump out blocks off of single indirect block */
if|if
condition|(
operator|!
operator|(
name|indir
operator|=
name|malloc
argument_list|(
name|psize
argument_list|)
operator|)
condition|)
name|err
argument_list|(
literal|"%s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|get
argument_list|(
name|fd
argument_list|,
name|dip
operator|->
name|di_ib
index|[
literal|0
index|]
operator|<<
name|daddr_shift
argument_list|,
name|indir
argument_list|,
name|psize
argument_list|)
expr_stmt|;
name|block_limit
operator|=
name|MIN
argument_list|(
name|i
operator|+
name|lfsp
operator|->
name|lfs_nindir
argument_list|,
name|nblocks
argument_list|)
expr_stmt|;
for|for
control|(
name|addrp
operator|=
name|indir
init|;
name|i
operator|<
name|block_limit
condition|;
name|i
operator|++
operator|,
name|addrp
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|addrp
operator|==
name|LFS_UNUSED_DADDR
condition|)
break|break;
name|get
argument_list|(
name|fd
argument_list|,
operator|*
name|addrp
operator|<<
name|daddr_shift
argument_list|,
name|ipage
argument_list|,
name|psize
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|<
name|lfsp
operator|->
name|lfs_segtabsz
condition|)
block|{
name|inum
operator|=
name|dump_ipage_segusage
argument_list|(
name|lfsp
argument_list|,
name|inum
argument_list|,
name|ipage
argument_list|,
name|nsupb
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|inum
condition|)
if|if
condition|(
operator|!
name|do_ientries
condition|)
goto|goto
name|e1
goto|;
else|else
name|print_iheader
expr_stmt|;
block|}
else|else
name|inum
operator|=
name|dump_ipage_ifile
argument_list|(
name|inum
argument_list|,
name|ipage
argument_list|,
name|lfsp
operator|->
name|lfs_ifpb
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|nblocks
operator|<=
name|lfsp
operator|->
name|lfs_nindir
operator|*
name|lfsp
operator|->
name|lfs_ifpb
condition|)
goto|goto
name|e1
goto|;
comment|/* Get the double indirect block */
if|if
condition|(
operator|!
operator|(
name|dindir
operator|=
name|malloc
argument_list|(
name|psize
argument_list|)
operator|)
condition|)
name|err
argument_list|(
literal|"%s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|get
argument_list|(
name|fd
argument_list|,
name|dip
operator|->
name|di_ib
index|[
literal|1
index|]
operator|<<
name|daddr_shift
argument_list|,
name|dindir
argument_list|,
name|psize
argument_list|)
expr_stmt|;
for|for
control|(
name|iaddrp
operator|=
name|dindir
operator|,
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|lfsp
operator|->
name|lfs_nindir
condition|;
name|j
operator|++
operator|,
name|iaddrp
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|iaddrp
operator|==
name|LFS_UNUSED_DADDR
condition|)
break|break;
name|get
argument_list|(
name|fd
argument_list|,
operator|*
name|iaddrp
operator|<<
name|daddr_shift
argument_list|,
name|indir
argument_list|,
name|psize
argument_list|)
expr_stmt|;
name|block_limit
operator|=
name|MIN
argument_list|(
name|i
operator|+
name|lfsp
operator|->
name|lfs_nindir
argument_list|,
name|nblocks
argument_list|)
expr_stmt|;
for|for
control|(
name|addrp
operator|=
name|indir
init|;
name|i
operator|<
name|block_limit
condition|;
name|i
operator|++
operator|,
name|addrp
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|addrp
operator|==
name|LFS_UNUSED_DADDR
condition|)
break|break;
name|get
argument_list|(
name|fd
argument_list|,
operator|*
name|addrp
operator|<<
name|daddr_shift
argument_list|,
name|ipage
argument_list|,
name|psize
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|<
name|lfsp
operator|->
name|lfs_segtabsz
condition|)
block|{
name|inum
operator|=
name|dump_ipage_segusage
argument_list|(
name|lfsp
argument_list|,
name|inum
argument_list|,
name|ipage
argument_list|,
name|nsupb
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|inum
condition|)
if|if
condition|(
operator|!
name|do_ientries
condition|)
goto|goto
name|e2
goto|;
else|else
name|print_iheader
expr_stmt|;
block|}
else|else
name|inum
operator|=
name|dump_ipage_ifile
argument_list|(
name|inum
argument_list|,
name|ipage
argument_list|,
name|lfsp
operator|->
name|lfs_ifpb
argument_list|)
expr_stmt|;
block|}
block|}
name|e2
label|:
name|free
argument_list|(
name|dindir
argument_list|)
expr_stmt|;
name|e1
label|:
name|free
argument_list|(
name|indir
argument_list|)
expr_stmt|;
name|e0
label|:
name|free
argument_list|(
name|dpage
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|ipage
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|dump_ipage_ifile
parameter_list|(
name|i
parameter_list|,
name|pp
parameter_list|,
name|tot
parameter_list|)
name|int
name|i
decl_stmt|;
name|IFILE
modifier|*
name|pp
decl_stmt|;
name|int
name|tot
decl_stmt|;
block|{
name|IFILE
modifier|*
name|ip
decl_stmt|;
name|int
name|cnt
decl_stmt|,
name|max
decl_stmt|;
name|max
operator|=
name|i
operator|+
name|tot
expr_stmt|;
for|for
control|(
name|ip
operator|=
name|pp
operator|,
name|cnt
operator|=
name|i
init|;
name|cnt
operator|<
name|max
condition|;
name|cnt
operator|++
operator|,
name|ip
operator|++
control|)
name|print_ientry
argument_list|(
name|cnt
argument_list|,
name|ip
argument_list|)
expr_stmt|;
return|return
operator|(
name|max
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|dump_ipage_segusage
parameter_list|(
name|lfsp
parameter_list|,
name|i
parameter_list|,
name|pp
parameter_list|,
name|tot
parameter_list|)
name|LFS_SUPER
modifier|*
name|lfsp
decl_stmt|;
name|int
name|i
decl_stmt|;
name|IFILE
modifier|*
name|pp
decl_stmt|;
name|int
name|tot
decl_stmt|;
block|{
name|SEGUSAGE
modifier|*
name|sp
decl_stmt|;
name|int
name|cnt
decl_stmt|,
name|max
decl_stmt|;
name|max
operator|=
name|i
operator|+
name|tot
expr_stmt|;
for|for
control|(
name|sp
operator|=
operator|(
name|SEGUSAGE
operator|*
operator|)
name|pp
operator|,
name|cnt
operator|=
name|i
init|;
name|cnt
operator|<
name|lfsp
operator|->
name|lfs_nseg
operator|&&
name|cnt
operator|<
name|max
condition|;
name|cnt
operator|++
operator|,
name|sp
operator|++
control|)
name|print_suentry
argument_list|(
name|cnt
argument_list|,
name|sp
argument_list|)
expr_stmt|;
if|if
condition|(
name|max
operator|>=
name|lfsp
operator|->
name|lfs_nseg
condition|)
return|return
operator|(
literal|0
operator|)
return|;
else|else
return|return
operator|(
name|max
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|dump_dinode
parameter_list|(
name|dip
parameter_list|)
name|struct
name|dinode
modifier|*
name|dip
decl_stmt|;
block|{
name|int
name|i
decl_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%d\t%s%d\t%s%d\t%s%d\t%s%d\n"
argument_list|,
literal|"mode  "
argument_list|,
name|dip
operator|->
name|di_mode
argument_list|,
literal|"nlink "
argument_list|,
name|dip
operator|->
name|di_nlink
argument_list|,
literal|"uid   "
argument_list|,
name|dip
operator|->
name|di_uid
argument_list|,
literal|"gid   "
argument_list|,
name|dip
operator|->
name|di_gid
argument_list|,
literal|"size  "
argument_list|,
name|dip
operator|->
name|di_size
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%s%s%s%s%s"
argument_list|,
literal|"atime "
argument_list|,
name|ctime
argument_list|(
operator|&
name|dip
operator|->
name|di_atime
argument_list|)
argument_list|,
literal|"mtime "
argument_list|,
name|ctime
argument_list|(
operator|&
name|dip
operator|->
name|di_mtime
argument_list|)
argument_list|,
literal|"ctime "
argument_list|,
name|ctime
argument_list|(
operator|&
name|dip
operator|->
name|di_ctime
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"inum  %d\n"
argument_list|,
name|dip
operator|->
name|di_inum
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Direct Addresses\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NDADDR
condition|;
name|i
operator|++
control|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\t%X"
argument_list|,
name|dip
operator|->
name|di_db
index|[
name|i
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|i
operator|%
literal|6
operator|)
operator|==
literal|5
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NIADDR
condition|;
name|i
operator|++
control|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\t%X"
argument_list|,
name|dip
operator|->
name|di_ib
index|[
name|i
index|]
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|dump_sum
parameter_list|(
name|sp
parameter_list|)
name|SEGSUM
modifier|*
name|sp
decl_stmt|;
block|{
name|FINFO
modifier|*
name|fp
decl_stmt|;
name|u_long
modifier|*
name|dp
decl_stmt|;
name|int
name|i
decl_stmt|,
name|j
decl_stmt|,
name|sb
decl_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Segment Summary Info\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\t%s%X   \t%s%X   \t%s%X   \n"
argument_list|,
literal|"next     "
argument_list|,
name|sp
operator|->
name|ss_next
argument_list|,
literal|"prev     "
argument_list|,
name|sp
operator|->
name|ss_prev
argument_list|,
literal|"nextsum  "
argument_list|,
name|sp
operator|->
name|ss_nextsum
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\t%s%d\t%s%d\t%s%X\n"
argument_list|,
literal|"nfinfo   "
argument_list|,
name|sp
operator|->
name|ss_nfinfo
argument_list|,
literal|"ninos    "
argument_list|,
name|sp
operator|->
name|ss_ninos
argument_list|,
literal|"cksum    "
argument_list|,
name|sp
operator|->
name|ss_cksum
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\tcreate   %s"
argument_list|,
name|ctime
argument_list|(
operator|(
name|time_t
operator|*
operator|)
operator|&
name|sp
operator|->
name|ss_create
argument_list|)
argument_list|)
expr_stmt|;
name|sb
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|fp
operator|=
operator|(
name|FINFO
operator|*
operator|)
operator|(
name|sp
operator|+
literal|1
operator|)
operator|,
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|sp
operator|->
name|ss_nfinfo
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|fp
operator|->
name|fi_ino
operator|==
name|LFS_UNUSED_INUM
condition|)
name|sb
operator|=
literal|1
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"File Info for file: %d version %d nblocks %d\n"
argument_list|,
name|fp
operator|->
name|fi_ino
argument_list|,
name|fp
operator|->
name|fi_version
argument_list|,
name|fp
operator|->
name|fi_nblocks
argument_list|)
expr_stmt|;
name|dp
operator|=
operator|&
operator|(
name|fp
operator|->
name|fi_blocks
index|[
literal|0
index|]
operator|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|fp
operator|->
name|fi_nblocks
condition|;
name|j
operator|++
operator|,
name|dp
operator|++
control|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\t%d"
argument_list|,
operator|*
name|dp
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|j
operator|%
literal|8
operator|)
operator|==
literal|7
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|j
operator|%
literal|8
operator|)
operator|!=
literal|0
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|fp
operator|=
operator|(
name|FINFO
operator|*
operator|)
name|dp
expr_stmt|;
block|}
return|return
operator|(
name|sb
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|daddr_t
name|dump_segment
parameter_list|(
name|fd
parameter_list|,
name|addr
parameter_list|,
name|lfsp
parameter_list|,
name|dump_sb
parameter_list|)
name|int
name|fd
decl_stmt|;
name|daddr_t
name|addr
decl_stmt|;
name|LFS_SUPER
modifier|*
name|lfsp
decl_stmt|;
name|int
name|dump_sb
decl_stmt|;
block|{
name|LFS_SUPER
name|lfs_sb
decl_stmt|;
name|SEGSUM
modifier|*
name|sump
decl_stmt|;
name|char
name|sumblock
index|[
name|LFS_SUMMARY_SIZE
index|]
decl_stmt|;
name|int
name|sum_offset
decl_stmt|;
name|int
name|sb
decl_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\nSegment Number %d (Disk Address %X)\n"
argument_list|,
name|addr
operator|>>
operator|(
name|lfsp
operator|->
name|lfs_segshift
operator|-
name|daddr_shift
operator|)
argument_list|,
name|addr
argument_list|)
expr_stmt|;
name|sum_offset
operator|=
operator|(
name|addr
operator|<<
operator|(
name|lfsp
operator|->
name|lfs_bshift
operator|-
name|lfsp
operator|->
name|lfs_fsbtodb
operator|)
operator|)
operator|+
operator|(
name|lfsp
operator|->
name|lfs_ssize
operator|<<
name|lfsp
operator|->
name|lfs_bshift
operator|)
operator|-
name|LFS_SUMMARY_SIZE
expr_stmt|;
name|sb
operator|=
literal|0
expr_stmt|;
do|do
block|{
name|get
argument_list|(
name|fd
argument_list|,
name|sum_offset
argument_list|,
name|sumblock
argument_list|,
name|LFS_SUMMARY_SIZE
argument_list|)
expr_stmt|;
name|sump
operator|=
operator|(
name|SEGSUM
operator|*
operator|)
name|sumblock
expr_stmt|;
name|sb
operator|=
name|sb
operator|||
name|dump_sum
argument_list|(
name|sump
argument_list|)
expr_stmt|;
name|sum_offset
operator|=
operator|(
name|sump
operator|->
name|ss_nextsum
operator|==
operator|-
literal|1
condition|?
literal|0
else|:
name|sump
operator|->
name|ss_nextsum
operator|<<
name|daddr_shift
operator|)
expr_stmt|;
block|}
do|while
condition|(
name|sum_offset
condition|)
do|;
if|if
condition|(
name|dump_sb
operator|&&
name|sb
condition|)
block|{
name|get
argument_list|(
name|fd
argument_list|,
name|addr
operator|<<
name|daddr_shift
argument_list|,
operator|&
name|lfs_sb
argument_list|,
sizeof|sizeof
argument_list|(
name|LFS_SUPER
argument_list|)
argument_list|)
expr_stmt|;
name|dump_super
argument_list|(
operator|&
name|lfs_sb
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|sump
operator|->
name|ss_prev
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|dump_super
parameter_list|(
name|lfsp
parameter_list|)
name|LFS_SUPER
modifier|*
name|lfsp
decl_stmt|;
block|{
name|int
name|i
decl_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%X\t%s%X\t%s%d\t%s%d\n"
argument_list|,
literal|"magic    "
argument_list|,
name|lfsp
operator|->
name|lfs_magic
argument_list|,
literal|"version  "
argument_list|,
name|lfsp
operator|->
name|lfs_version
argument_list|,
literal|"size     "
argument_list|,
name|lfsp
operator|->
name|lfs_size
argument_list|,
literal|"ssize    "
argument_list|,
name|lfsp
operator|->
name|lfs_ssize
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%d\t%s%d\t%s%d\t%s%d\n"
argument_list|,
literal|"dsize    "
argument_list|,
name|lfsp
operator|->
name|lfs_dsize
argument_list|,
literal|"bsize    "
argument_list|,
name|lfsp
operator|->
name|lfs_bsize
argument_list|,
literal|"fsize    "
argument_list|,
name|lfsp
operator|->
name|lfs_fsize
argument_list|,
literal|"frag     "
argument_list|,
name|lfsp
operator|->
name|lfs_frag
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%d\t%s%d\t%s%d\t%s%d\n"
argument_list|,
literal|"minfree  "
argument_list|,
name|lfsp
operator|->
name|lfs_minfree
argument_list|,
literal|"inopb    "
argument_list|,
name|lfsp
operator|->
name|lfs_inopb
argument_list|,
literal|"ifpb     "
argument_list|,
name|lfsp
operator|->
name|lfs_ifpb
argument_list|,
literal|"nindir   "
argument_list|,
name|lfsp
operator|->
name|lfs_nindir
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%d\t%s%d\t%s%d\n"
argument_list|,
literal|"nseg     "
argument_list|,
name|lfsp
operator|->
name|lfs_nseg
argument_list|,
literal|"nspf     "
argument_list|,
name|lfsp
operator|->
name|lfs_nspf
argument_list|,
literal|"segtabsz "
argument_list|,
name|lfsp
operator|->
name|lfs_segtabsz
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%X\t%s%d\t%s%X\t%s%d\n"
argument_list|,
literal|"segmask  "
argument_list|,
name|lfsp
operator|->
name|lfs_segmask
argument_list|,
literal|"segshift "
argument_list|,
name|lfsp
operator|->
name|lfs_segshift
argument_list|,
literal|"bmask    "
argument_list|,
name|lfsp
operator|->
name|lfs_bmask
argument_list|,
literal|"bshift   "
argument_list|,
name|lfsp
operator|->
name|lfs_bshift
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%X\t%s%d\t%s%X\t%s%d\n"
argument_list|,
literal|"ffmask   "
argument_list|,
name|lfsp
operator|->
name|lfs_ffmask
argument_list|,
literal|"ffshift  "
argument_list|,
name|lfsp
operator|->
name|lfs_ffshift
argument_list|,
literal|"fbmask   "
argument_list|,
name|lfsp
operator|->
name|lfs_fbmask
argument_list|,
literal|"fbshift  "
argument_list|,
name|lfsp
operator|->
name|lfs_fbshift
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%d\t%s%X\n"
argument_list|,
literal|"fsbtodb  "
argument_list|,
name|lfsp
operator|->
name|lfs_fsbtodb
argument_list|,
literal|"cksum    "
argument_list|,
name|lfsp
operator|->
name|lfs_cksum
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Superblock disk addresses:"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|LFS_MAXNUMSB
condition|;
name|i
operator|++
control|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|" %X"
argument_list|,
name|lfsp
operator|->
name|lfs_sboffs
index|[
name|i
index|]
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Checkpoint Info\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s%d\t%s%X\t%s%d\t%s%d\n"
argument_list|,
literal|"free     "
argument_list|,
name|lfsp
operator|->
name|lfs_free
argument_list|,
literal|"idaddr   "
argument_list|,
name|lfsp
operator|->
name|lfs_idaddr
argument_list|,
literal|"ifile    "
argument_list|,
name|lfsp
operator|->
name|lfs_ifile
argument_list|,
literal|"lastseg  "
argument_list|,
name|lfsp
operator|->
name|lfs_lastseg
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"tstamp   %s"
argument_list|,
name|ctime
argument_list|(
operator|(
name|time_t
operator|*
operator|)
operator|&
name|lfsp
operator|->
name|lfs_tstamp
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|usage
parameter_list|()
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: dumplfs [-is] file\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

