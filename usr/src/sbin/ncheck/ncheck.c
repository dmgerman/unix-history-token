begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1988, 1990, 1993  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.proprietary.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1988, 1990, 1993\n\ 	The Regents of the University of California.  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)ncheck.c	8.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_comment
comment|/*  * ncheck -- obtain file names from reading filesystem  */
end_comment

begin_define
define|#
directive|define
name|NB
value|500
end_define

begin_define
define|#
directive|define
name|MAXNINDIR
value|(MAXBSIZE / sizeof (daddr_t))
end_define

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/dir.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/dinode.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ffs/fs.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_decl_stmt
name|struct
name|fs
modifier|*
name|sblockp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|dinode
name|itab
index|[
name|MAXBSIZE
operator|/
sizeof|sizeof
argument_list|(
expr|struct
name|dinode
argument_list|)
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|dinode
modifier|*
name|gip
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|ilist
block|{
name|ino_t
name|ino
decl_stmt|;
name|u_short
name|mode
decl_stmt|;
name|short
name|uid
decl_stmt|;
name|short
name|gid
decl_stmt|;
block|}
name|ilist
index|[
name|NB
index|]
struct|;
end_struct

begin_struct
struct|struct
name|htab
block|{
name|ino_t
name|h_ino
decl_stmt|;
name|ino_t
name|h_pino
decl_stmt|;
name|char
modifier|*
name|h_name
decl_stmt|;
block|}
modifier|*
name|htab
struct|;
end_struct

begin_decl_stmt
name|char
modifier|*
name|strngtab
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
name|hsize
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|strngloc
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|dirstuff
block|{
name|int
name|loc
decl_stmt|;
name|struct
name|dinode
modifier|*
name|ip
decl_stmt|;
name|char
name|dbuf
index|[
name|MAXBSIZE
index|]
decl_stmt|;
block|}
struct|;
end_struct

begin_decl_stmt
name|int
name|aflg
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|sflg
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|iflg
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* number of inodes being searched for */
end_comment

begin_decl_stmt
name|int
name|mflg
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|fi
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|ino_t
name|ino
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nhent
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nxfile
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|dev_bsize
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nerror
decl_stmt|;
end_decl_stmt

begin_function_decl
name|daddr_t
name|bmap
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|long
name|atol
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|char
modifier|*
name|malloc
argument_list|()
decl_stmt|,
modifier|*
name|strcpy
argument_list|()
decl_stmt|;
end_decl_stmt

begin_function_decl
name|struct
name|htab
modifier|*
name|lookup
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|struct
name|direct
modifier|*
name|nreaddir
parameter_list|()
function_decl|;
end_function_decl

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
name|long
name|n
decl_stmt|;
while|while
condition|(
operator|--
name|argc
condition|)
block|{
name|argv
operator|++
expr_stmt|;
if|if
condition|(
operator|*
operator|*
name|argv
operator|==
literal|'-'
condition|)
switch|switch
condition|(
operator|(
operator|*
name|argv
operator|)
index|[
literal|1
index|]
condition|)
block|{
case|case
literal|'a'
case|:
name|aflg
operator|++
expr_stmt|;
continue|continue;
case|case
literal|'i'
case|:
for|for
control|(
name|iflg
operator|=
literal|0
init|;
name|iflg
operator|<
name|NB
operator|&&
name|argc
operator|>=
literal|2
condition|;
name|iflg
operator|++
control|)
block|{
name|n
operator|=
name|atol
argument_list|(
name|argv
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
break|break;
name|ilist
index|[
name|iflg
index|]
operator|.
name|ino
operator|=
name|n
expr_stmt|;
name|nxfile
operator|=
name|iflg
expr_stmt|;
name|argv
operator|++
expr_stmt|;
name|argc
operator|--
expr_stmt|;
block|}
continue|continue;
case|case
literal|'m'
case|:
name|mflg
operator|++
expr_stmt|;
continue|continue;
case|case
literal|'s'
case|:
name|sflg
operator|++
expr_stmt|;
continue|continue;
default|default:
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ncheck: bad flag %c\n"
argument_list|,
operator|(
operator|*
name|argv
operator|)
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|nerror
operator|++
expr_stmt|;
block|}
name|check
argument_list|(
operator|*
name|argv
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|nerror
operator|)
return|;
block|}
end_function

begin_macro
name|check
argument_list|(
argument|file
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|file
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|,
name|j
decl_stmt|,
name|c
decl_stmt|;
name|fi
operator|=
name|open
argument_list|(
name|file
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|fi
operator|<
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ncheck: cannot open %s\n"
argument_list|,
name|file
argument_list|)
expr_stmt|;
name|nerror
operator|++
expr_stmt|;
return|return;
block|}
name|nhent
operator|=
literal|0
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s:\n"
argument_list|,
name|file
argument_list|)
expr_stmt|;
name|sync
argument_list|()
expr_stmt|;
name|dev_bsize
operator|=
literal|1
expr_stmt|;
name|sblockp
operator|=
operator|(
expr|struct
name|fs
operator|*
operator|)
name|malloc
argument_list|(
operator|(
name|unsigned
operator|)
name|SBSIZE
argument_list|)
expr_stmt|;
if|if
condition|(
name|sblockp
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"icheck: couldn't malloc superblock memory\n"
argument_list|)
expr_stmt|;
name|nerror
operator|++
expr_stmt|;
return|return;
block|}
name|bread
argument_list|(
operator|(
name|daddr_t
operator|)
name|SBOFF
argument_list|,
operator|(
name|char
operator|*
operator|)
name|sblockp
argument_list|,
operator|(
name|long
operator|)
name|SBSIZE
argument_list|)
expr_stmt|;
if|if
condition|(
name|sblockp
operator|->
name|fs_magic
operator|!=
name|FS_MAGIC
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s: not a file system\n"
argument_list|,
name|file
argument_list|)
expr_stmt|;
name|nerror
operator|++
expr_stmt|;
return|return;
block|}
name|dev_bsize
operator|=
name|sblockp
operator|->
name|fs_fsize
operator|/
name|fsbtodb
argument_list|(
name|sblockp
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|hsize
operator|=
name|sblockp
operator|->
name|fs_ipg
operator|*
name|sblockp
operator|->
name|fs_ncg
operator|-
name|sblockp
operator|->
name|fs_cstotal
operator|.
name|cs_nifree
operator|+
literal|1
expr_stmt|;
name|htab
operator|=
operator|(
expr|struct
name|htab
operator|*
operator|)
name|malloc
argument_list|(
operator|(
name|unsigned
operator|)
name|hsize
operator|*
sizeof|sizeof
argument_list|(
expr|struct
name|htab
argument_list|)
argument_list|)
expr_stmt|;
name|strngtab
operator|=
name|malloc
argument_list|(
call|(
name|unsigned
call|)
argument_list|(
literal|30
operator|*
name|hsize
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|htab
operator|==
literal|0
operator|||
name|strngtab
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"not enough memory to allocate tables\n"
argument_list|)
expr_stmt|;
name|nerror
operator|++
expr_stmt|;
return|return;
block|}
name|ino
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|c
operator|=
literal|0
init|;
name|c
operator|<
name|sblockp
operator|->
name|fs_ncg
condition|;
name|c
operator|++
control|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|sblockp
operator|->
name|fs_ipg
operator|/
name|INOPF
argument_list|(
name|sblockp
argument_list|)
condition|;
name|i
operator|+=
name|sblockp
operator|->
name|fs_frag
control|)
block|{
name|bread
argument_list|(
name|fsbtodb
argument_list|(
name|sblockp
argument_list|,
name|cgimin
argument_list|(
name|sblockp
argument_list|,
name|c
argument_list|)
operator|+
name|i
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
name|itab
argument_list|,
name|sblockp
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|INOPB
argument_list|(
name|sblockp
argument_list|)
condition|;
name|j
operator|++
control|)
block|{
if|if
condition|(
name|itab
index|[
name|j
index|]
operator|.
name|di_mode
operator|!=
literal|0
condition|)
name|pass1
argument_list|(
operator|&
name|itab
index|[
name|j
index|]
argument_list|)
expr_stmt|;
name|ino
operator|++
expr_stmt|;
block|}
block|}
block|}
name|ilist
index|[
name|nxfile
operator|+
literal|1
index|]
operator|.
name|ino
operator|=
literal|0
expr_stmt|;
name|ino
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|c
operator|=
literal|0
init|;
name|c
operator|<
name|sblockp
operator|->
name|fs_ncg
condition|;
name|c
operator|++
control|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|sblockp
operator|->
name|fs_ipg
operator|/
name|INOPF
argument_list|(
name|sblockp
argument_list|)
condition|;
name|i
operator|+=
name|sblockp
operator|->
name|fs_frag
control|)
block|{
name|bread
argument_list|(
name|fsbtodb
argument_list|(
name|sblockp
argument_list|,
name|cgimin
argument_list|(
name|sblockp
argument_list|,
name|c
argument_list|)
operator|+
name|i
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
name|itab
argument_list|,
name|sblockp
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|INOPB
argument_list|(
name|sblockp
argument_list|)
condition|;
name|j
operator|++
control|)
block|{
if|if
condition|(
name|itab
index|[
name|j
index|]
operator|.
name|di_mode
operator|!=
literal|0
condition|)
name|pass2
argument_list|(
operator|&
name|itab
index|[
name|j
index|]
argument_list|)
expr_stmt|;
name|ino
operator|++
expr_stmt|;
block|}
block|}
block|}
name|ino
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|c
operator|=
literal|0
init|;
name|c
operator|<
name|sblockp
operator|->
name|fs_ncg
condition|;
name|c
operator|++
control|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|sblockp
operator|->
name|fs_ipg
operator|/
name|INOPF
argument_list|(
name|sblockp
argument_list|)
condition|;
name|i
operator|+=
name|sblockp
operator|->
name|fs_frag
control|)
block|{
name|bread
argument_list|(
name|fsbtodb
argument_list|(
name|sblockp
argument_list|,
name|cgimin
argument_list|(
name|sblockp
argument_list|,
name|c
argument_list|)
operator|+
name|i
argument_list|)
argument_list|,
operator|(
name|char
operator|*
operator|)
name|itab
argument_list|,
name|sblockp
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|INOPB
argument_list|(
name|sblockp
argument_list|)
condition|;
name|j
operator|++
control|)
block|{
if|if
condition|(
name|itab
index|[
name|j
index|]
operator|.
name|di_mode
operator|!=
literal|0
condition|)
name|pass3
argument_list|(
operator|&
name|itab
index|[
name|j
index|]
argument_list|)
expr_stmt|;
name|ino
operator|++
expr_stmt|;
block|}
block|}
block|}
operator|(
name|void
operator|)
name|close
argument_list|(
name|fi
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|hsize
condition|;
name|i
operator|++
control|)
name|htab
index|[
name|i
index|]
operator|.
name|h_ino
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
name|iflg
init|;
name|i
operator|<
name|NB
condition|;
name|i
operator|++
control|)
name|ilist
index|[
name|i
index|]
operator|.
name|ino
operator|=
literal|0
expr_stmt|;
name|nxfile
operator|=
name|iflg
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|pass1
argument_list|(
name|ip
argument_list|)
specifier|register
expr|struct
name|dinode
operator|*
name|ip
expr_stmt|;
end_expr_stmt

begin_block
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
name|mflg
condition|)
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|iflg
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|ino
operator|==
name|ilist
index|[
name|i
index|]
operator|.
name|ino
condition|)
block|{
name|ilist
index|[
name|i
index|]
operator|.
name|mode
operator|=
name|ip
operator|->
name|di_mode
expr_stmt|;
name|ilist
index|[
name|i
index|]
operator|.
name|uid
operator|=
name|ip
operator|->
name|di_uid
expr_stmt|;
name|ilist
index|[
name|i
index|]
operator|.
name|gid
operator|=
name|ip
operator|->
name|di_gid
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|ip
operator|->
name|di_mode
operator|&
name|IFMT
operator|)
operator|!=
name|IFDIR
condition|)
block|{
if|if
condition|(
name|sflg
operator|==
literal|0
operator|||
name|nxfile
operator|>=
name|NB
condition|)
return|return;
if|if
condition|(
operator|(
name|ip
operator|->
name|di_mode
operator|&
name|IFMT
operator|)
operator|==
name|IFBLK
operator|||
operator|(
name|ip
operator|->
name|di_mode
operator|&
name|IFMT
operator|)
operator|==
name|IFCHR
operator|||
name|ip
operator|->
name|di_mode
operator|&
operator|(
name|ISUID
operator||
name|ISGID
operator|)
condition|)
block|{
name|ilist
index|[
name|nxfile
index|]
operator|.
name|ino
operator|=
name|ino
expr_stmt|;
name|ilist
index|[
name|nxfile
index|]
operator|.
name|mode
operator|=
name|ip
operator|->
name|di_mode
expr_stmt|;
name|ilist
index|[
name|nxfile
index|]
operator|.
name|uid
operator|=
name|ip
operator|->
name|di_uid
expr_stmt|;
name|ilist
index|[
name|nxfile
operator|++
index|]
operator|.
name|gid
operator|=
name|ip
operator|->
name|di_gid
expr_stmt|;
return|return;
block|}
block|}
operator|(
name|void
operator|)
name|lookup
argument_list|(
name|ino
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|pass2
argument_list|(
name|ip
argument_list|)
specifier|register
expr|struct
name|dinode
operator|*
name|ip
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|direct
modifier|*
name|dp
decl_stmt|;
name|struct
name|dirstuff
name|dirp
decl_stmt|;
name|struct
name|htab
modifier|*
name|hp
decl_stmt|;
if|if
condition|(
operator|(
name|ip
operator|->
name|di_mode
operator|&
name|IFMT
operator|)
operator|!=
name|IFDIR
condition|)
return|return;
name|dirp
operator|.
name|loc
operator|=
literal|0
expr_stmt|;
name|dirp
operator|.
name|ip
operator|=
name|ip
expr_stmt|;
name|gip
operator|=
name|ip
expr_stmt|;
for|for
control|(
name|dp
operator|=
name|nreaddir
argument_list|(
operator|&
name|dirp
argument_list|)
init|;
name|dp
operator|!=
name|NULL
condition|;
name|dp
operator|=
name|nreaddir
argument_list|(
operator|&
name|dirp
argument_list|)
control|)
block|{
if|if
condition|(
name|dp
operator|->
name|d_ino
operator|==
literal|0
condition|)
continue|continue;
name|hp
operator|=
name|lookup
argument_list|(
name|dp
operator|->
name|d_ino
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|hp
operator|==
literal|0
condition|)
continue|continue;
if|if
condition|(
name|dotname
argument_list|(
name|dp
argument_list|)
condition|)
continue|continue;
name|hp
operator|->
name|h_pino
operator|=
name|ino
expr_stmt|;
name|hp
operator|->
name|h_name
operator|=
operator|&
name|strngtab
index|[
name|strngloc
index|]
expr_stmt|;
name|strngloc
operator|+=
name|strlen
argument_list|(
name|dp
operator|->
name|d_name
argument_list|)
operator|+
literal|1
expr_stmt|;
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|hp
operator|->
name|h_name
argument_list|,
name|dp
operator|->
name|d_name
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_expr_stmt
name|pass3
argument_list|(
name|ip
argument_list|)
specifier|register
expr|struct
name|dinode
operator|*
name|ip
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|direct
modifier|*
name|dp
decl_stmt|;
name|struct
name|dirstuff
name|dirp
decl_stmt|;
name|int
name|k
decl_stmt|;
if|if
condition|(
operator|(
name|ip
operator|->
name|di_mode
operator|&
name|IFMT
operator|)
operator|!=
name|IFDIR
condition|)
return|return;
name|dirp
operator|.
name|loc
operator|=
literal|0
expr_stmt|;
name|dirp
operator|.
name|ip
operator|=
name|ip
expr_stmt|;
name|gip
operator|=
name|ip
expr_stmt|;
for|for
control|(
name|dp
operator|=
name|nreaddir
argument_list|(
operator|&
name|dirp
argument_list|)
init|;
name|dp
operator|!=
name|NULL
condition|;
name|dp
operator|=
name|nreaddir
argument_list|(
operator|&
name|dirp
argument_list|)
control|)
block|{
if|if
condition|(
name|aflg
operator|==
literal|0
operator|&&
name|dotname
argument_list|(
name|dp
argument_list|)
condition|)
continue|continue;
if|if
condition|(
name|sflg
operator|==
literal|0
operator|&&
name|iflg
operator|==
literal|0
condition|)
goto|goto
name|pr
goto|;
for|for
control|(
name|k
operator|=
literal|0
init|;
name|ilist
index|[
name|k
index|]
operator|.
name|ino
operator|!=
literal|0
condition|;
name|k
operator|++
control|)
if|if
condition|(
name|ilist
index|[
name|k
index|]
operator|.
name|ino
operator|==
name|dp
operator|->
name|d_ino
condition|)
break|break;
if|if
condition|(
name|ilist
index|[
name|k
index|]
operator|.
name|ino
operator|==
literal|0
condition|)
continue|continue;
if|if
condition|(
name|mflg
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"mode %-6o uid %-5d gid %-5d ino "
argument_list|,
name|ilist
index|[
name|k
index|]
operator|.
name|mode
argument_list|,
name|ilist
index|[
name|k
index|]
operator|.
name|uid
argument_list|,
name|ilist
index|[
name|k
index|]
operator|.
name|gid
argument_list|)
expr_stmt|;
name|pr
label|:
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%-5lu\t"
argument_list|,
name|dp
operator|->
name|d_ino
argument_list|)
expr_stmt|;
name|pname
argument_list|(
name|ino
argument_list|,
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"/%s"
argument_list|,
name|dp
operator|->
name|d_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|lookup
argument_list|(
name|dp
operator|->
name|d_ino
argument_list|,
literal|0
argument_list|)
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"/."
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * get next entry in a directory.  */
end_comment

begin_function
name|struct
name|direct
modifier|*
name|nreaddir
parameter_list|(
name|dirp
parameter_list|)
specifier|register
name|struct
name|dirstuff
modifier|*
name|dirp
decl_stmt|;
block|{
specifier|register
name|struct
name|direct
modifier|*
name|dp
decl_stmt|;
name|daddr_t
name|lbn
decl_stmt|,
name|d
decl_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
name|dirp
operator|->
name|loc
operator|>=
name|dirp
operator|->
name|ip
operator|->
name|di_size
condition|)
return|return
name|NULL
return|;
if|if
condition|(
name|blkoff
argument_list|(
name|sblockp
argument_list|,
name|dirp
operator|->
name|loc
argument_list|)
operator|==
literal|0
condition|)
block|{
name|lbn
operator|=
name|lblkno
argument_list|(
name|sblockp
argument_list|,
name|dirp
operator|->
name|loc
argument_list|)
expr_stmt|;
name|d
operator|=
name|bmap
argument_list|(
name|lbn
argument_list|)
expr_stmt|;
if|if
condition|(
name|d
operator|==
literal|0
condition|)
return|return
name|NULL
return|;
name|bread
argument_list|(
name|fsbtodb
argument_list|(
name|sblockp
argument_list|,
name|d
argument_list|)
argument_list|,
name|dirp
operator|->
name|dbuf
argument_list|,
operator|(
name|long
operator|)
name|dblksize
argument_list|(
name|sblockp
argument_list|,
name|dirp
operator|->
name|ip
argument_list|,
name|lbn
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|dp
operator|=
operator|(
expr|struct
name|direct
operator|*
operator|)
operator|(
name|dirp
operator|->
name|dbuf
operator|+
name|blkoff
argument_list|(
name|sblockp
argument_list|,
name|dirp
operator|->
name|loc
argument_list|)
operator|)
expr_stmt|;
name|dirp
operator|->
name|loc
operator|+=
name|dp
operator|->
name|d_reclen
expr_stmt|;
if|if
condition|(
name|dp
operator|->
name|d_ino
operator|==
literal|0
condition|)
continue|continue;
return|return
operator|(
name|dp
operator|)
return|;
block|}
block|}
end_function

begin_expr_stmt
name|dotname
argument_list|(
name|dp
argument_list|)
specifier|register
expr|struct
name|direct
operator|*
name|dp
expr_stmt|;
end_expr_stmt

begin_block
block|{
if|if
condition|(
name|dp
operator|->
name|d_name
index|[
literal|0
index|]
operator|==
literal|'.'
condition|)
if|if
condition|(
name|dp
operator|->
name|d_name
index|[
literal|1
index|]
operator|==
literal|0
operator|||
operator|(
name|dp
operator|->
name|d_name
index|[
literal|1
index|]
operator|==
literal|'.'
operator|&&
name|dp
operator|->
name|d_name
index|[
literal|2
index|]
operator|==
literal|0
operator|)
condition|)
return|return
operator|(
literal|1
operator|)
return|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_macro
name|pname
argument_list|(
argument|i
argument_list|,
argument|lev
argument_list|)
end_macro

begin_decl_stmt
name|ino_t
name|i
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|lev
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|htab
modifier|*
name|hp
decl_stmt|;
if|if
condition|(
name|i
operator|==
name|ROOTINO
condition|)
return|return;
if|if
condition|(
operator|(
name|hp
operator|=
name|lookup
argument_list|(
name|i
argument_list|,
literal|0
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"???"
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|lev
operator|>
literal|10
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"..."
argument_list|)
expr_stmt|;
return|return;
block|}
name|pname
argument_list|(
name|hp
operator|->
name|h_pino
argument_list|,
operator|++
name|lev
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"/%s"
argument_list|,
name|hp
operator|->
name|h_name
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
name|struct
name|htab
modifier|*
name|lookup
parameter_list|(
name|i
parameter_list|,
name|ef
parameter_list|)
name|ino_t
name|i
decl_stmt|;
name|int
name|ef
decl_stmt|;
block|{
specifier|register
name|struct
name|htab
modifier|*
name|hp
decl_stmt|;
for|for
control|(
name|hp
operator|=
operator|&
name|htab
index|[
name|i
operator|%
name|hsize
index|]
init|;
name|hp
operator|->
name|h_ino
condition|;
control|)
block|{
if|if
condition|(
name|hp
operator|->
name|h_ino
operator|==
name|i
condition|)
return|return
operator|(
name|hp
operator|)
return|;
if|if
condition|(
operator|++
name|hp
operator|>=
operator|&
name|htab
index|[
name|hsize
index|]
condition|)
name|hp
operator|=
name|htab
expr_stmt|;
block|}
if|if
condition|(
name|ef
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
if|if
condition|(
operator|++
name|nhent
operator|>=
name|hsize
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ncheck: hsize of %ld is too small\n"
argument_list|,
name|hsize
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|hp
operator|->
name|h_ino
operator|=
name|i
expr_stmt|;
return|return
operator|(
name|hp
operator|)
return|;
block|}
end_function

begin_macro
name|bread
argument_list|(
argument|bno
argument_list|,
argument|buf
argument_list|,
argument|lcount
argument_list|)
end_macro

begin_decl_stmt
name|daddr_t
name|bno
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|char
modifier|*
name|buf
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
name|lcount
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|,
name|cnt
init|=
name|lcount
decl_stmt|;
specifier|register
name|off_t
name|off
init|=
name|bno
operator|*
name|dev_bsize
decl_stmt|;
operator|(
name|void
operator|)
name|lseek
argument_list|(
name|fi
argument_list|,
name|off
argument_list|,
name|SEEK_SET
argument_list|)
expr_stmt|;
if|if
condition|(
name|read
argument_list|(
name|fi
argument_list|,
name|buf
argument_list|,
name|cnt
argument_list|)
operator|!=
name|cnt
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ncheck: read error %ld\n"
argument_list|,
name|bno
argument_list|)
expr_stmt|;
if|if
condition|(
name|cnt
operator|%
name|dev_bsize
condition|)
block|{
comment|/* THIS INDICATES A SERIOUS BUG */
comment|/* bzero is probably not correct, but will do */
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ncheck: bread: cnt %d not multiple of %d\n"
argument_list|,
name|cnt
argument_list|,
name|dev_bsize
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|buf
argument_list|,
name|cnt
argument_list|)
expr_stmt|;
return|return;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|cnt
condition|;
name|i
operator|+=
name|dev_bsize
control|)
block|{
operator|(
name|void
operator|)
name|lseek
argument_list|(
name|fi
argument_list|,
name|off
argument_list|,
name|SEEK_SET
argument_list|)
expr_stmt|;
if|if
condition|(
name|read
argument_list|(
name|fi
argument_list|,
name|buf
argument_list|,
name|dev_bsize
argument_list|)
operator|!=
name|dev_bsize
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ncheck: re-read error %ld\n"
argument_list|,
name|bno
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|buf
argument_list|,
name|dev_bsize
argument_list|)
expr_stmt|;
block|}
name|off
operator|+=
name|dev_bsize
expr_stmt|;
name|buf
operator|+=
name|dev_bsize
expr_stmt|;
name|bno
operator|++
expr_stmt|;
block|}
block|}
block|}
end_block

begin_comment
comment|/*  * Swiped from standalone sys.c.  */
end_comment

begin_define
define|#
directive|define
name|NBUFS
value|4
end_define

begin_decl_stmt
name|char
name|b
index|[
name|NBUFS
index|]
index|[
name|MAXBSIZE
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|daddr_t
name|blknos
index|[
name|NBUFS
index|]
decl_stmt|;
end_decl_stmt

begin_function
name|daddr_t
name|bmap
parameter_list|(
name|bn
parameter_list|)
specifier|register
name|daddr_t
name|bn
decl_stmt|;
block|{
specifier|register
name|int
name|j
decl_stmt|;
name|int
name|i
decl_stmt|,
name|sh
decl_stmt|;
name|daddr_t
name|nb
decl_stmt|,
modifier|*
name|bap
decl_stmt|;
if|if
condition|(
name|bn
operator|<
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ncheck: bn %ld negative\n"
argument_list|,
name|bn
argument_list|)
expr_stmt|;
return|return
operator|(
operator|(
name|daddr_t
operator|)
literal|0
operator|)
return|;
block|}
comment|/* 	 * blocks 0..NDADDR are direct blocks 	 */
if|if
condition|(
name|bn
operator|<
name|NDADDR
condition|)
return|return
operator|(
name|gip
operator|->
name|di_db
index|[
name|bn
index|]
operator|)
return|;
comment|/* 	 * addresses NIADDR have single and double indirect blocks. 	 * the first step is to determine how many levels of indirection. 	 */
name|sh
operator|=
literal|1
expr_stmt|;
name|bn
operator|-=
name|NDADDR
expr_stmt|;
for|for
control|(
name|j
operator|=
name|NIADDR
init|;
name|j
operator|>
literal|0
condition|;
name|j
operator|--
control|)
block|{
name|sh
operator|*=
name|NINDIR
argument_list|(
name|sblockp
argument_list|)
expr_stmt|;
if|if
condition|(
name|bn
operator|<
name|sh
condition|)
break|break;
name|bn
operator|-=
name|sh
expr_stmt|;
block|}
if|if
condition|(
name|j
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"ncheck: bn %ld ovf, ino %lu\n"
argument_list|,
name|bn
argument_list|,
name|ino
argument_list|)
expr_stmt|;
return|return
operator|(
operator|(
name|daddr_t
operator|)
literal|0
operator|)
return|;
block|}
comment|/* 	 * fetch the first indirect block address from the inode 	 */
name|nb
operator|=
name|gip
operator|->
name|di_ib
index|[
name|NIADDR
operator|-
name|j
index|]
expr_stmt|;
if|if
condition|(
name|nb
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"ncheck: bn %ld void1, ino %lu\n"
argument_list|,
name|bn
argument_list|,
name|ino
argument_list|)
expr_stmt|;
return|return
operator|(
operator|(
name|daddr_t
operator|)
literal|0
operator|)
return|;
block|}
comment|/* 	 * fetch through the indirect blocks 	 */
for|for
control|(
init|;
name|j
operator|<=
name|NIADDR
condition|;
name|j
operator|++
control|)
block|{
if|if
condition|(
name|blknos
index|[
name|j
index|]
operator|!=
name|nb
condition|)
block|{
name|bread
argument_list|(
name|fsbtodb
argument_list|(
name|sblockp
argument_list|,
name|nb
argument_list|)
argument_list|,
name|b
index|[
name|j
index|]
argument_list|,
name|sblockp
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
name|blknos
index|[
name|j
index|]
operator|=
name|nb
expr_stmt|;
block|}
name|bap
operator|=
operator|(
name|daddr_t
operator|*
operator|)
name|b
index|[
name|j
index|]
expr_stmt|;
name|sh
operator|/=
name|NINDIR
argument_list|(
name|sblockp
argument_list|)
expr_stmt|;
name|i
operator|=
operator|(
name|bn
operator|/
name|sh
operator|)
operator|%
name|NINDIR
argument_list|(
name|sblockp
argument_list|)
expr_stmt|;
name|nb
operator|=
name|bap
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|nb
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"ncheck: bn %ld void2, ino %lu\n"
argument_list|,
name|bn
argument_list|,
name|ino
argument_list|)
expr_stmt|;
return|return
operator|(
operator|(
name|daddr_t
operator|)
literal|0
operator|)
return|;
block|}
block|}
return|return
operator|(
name|nb
operator|)
return|;
block|}
end_function

end_unit

