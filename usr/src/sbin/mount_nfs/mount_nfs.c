begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1992 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Rick Macklem at The University of Guelph.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1992 The Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)mount_nfs.c	5.4 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<strings.h>
end_include

begin_include
include|#
directive|include
file|<sys/syslog.h>
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/ucred.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/socketvar.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_include
include|#
directive|include
file|<rpc/rpc.h>
end_include

begin_include
include|#
directive|include
file|<rpc/pmap_clnt.h>
end_include

begin_include
include|#
directive|include
file|<rpc/pmap_prot.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|ISO
end_ifdef

begin_include
include|#
directive|include
file|<netiso/iso.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_ifdef
ifdef|#
directive|ifdef
name|KERBEROS
end_ifdef

begin_include
include|#
directive|include
file|<kerberosIV/krb.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<nfs/rpcv2.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nfsv2.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nfs.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nqnfs.h>
end_include

begin_decl_stmt
name|int
name|xdr_dir
argument_list|()
decl_stmt|,
name|xdr_fh
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|nfs_args
name|nfsdefargs
init|=
block|{
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
literal|0
block|,
sizeof|sizeof
argument_list|(
expr|struct
name|sockaddr_in
argument_list|)
block|,
name|SOCK_DGRAM
block|,
literal|0
block|,
operator|(
name|nfsv2fh_t
operator|*
operator|)
literal|0
block|,
literal|0
block|,
name|NFS_WSIZE
block|,
name|NFS_RSIZE
block|,
name|NFS_TIMEO
block|,
name|NFS_RETRANS
block|,
name|NFS_MAXGRPS
block|,
name|NFS_DEFRAHEAD
block|,
name|NQ_DEFLEASE
block|,
name|NQ_DEADTHRESH
block|,
operator|(
name|char
operator|*
operator|)
literal|0
block|, }
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|nfhret
block|{
name|u_long
name|stat
decl_stmt|;
name|nfsv2fh_t
name|nfh
decl_stmt|;
block|}
struct|;
end_struct

begin_define
define|#
directive|define
name|DEF_RETRY
value|10000
end_define

begin_define
define|#
directive|define
name|BGRND
value|1
end_define

begin_define
define|#
directive|define
name|ISBGRND
value|2
end_define

begin_decl_stmt
name|int
name|retrycnt
init|=
name|DEF_RETRY
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|opflags
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|int
name|errno
decl_stmt|;
end_decl_stmt

begin_ifdef
ifdef|#
directive|ifdef
name|ISO
end_ifdef

begin_function_decl
name|struct
name|iso_addr
modifier|*
name|iso_addr
parameter_list|()
function_decl|;
end_function_decl

begin_endif
endif|#
directive|endif
end_endif

begin_ifdef
ifdef|#
directive|ifdef
name|KERBEROS
end_ifdef

begin_decl_stmt
name|char
name|inst
index|[
name|INST_SZ
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|realm
index|[
name|REALM_SZ
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|KTEXT_ST
name|kt
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|,
name|arge
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
name|char
modifier|*
modifier|*
name|arge
decl_stmt|;
block|{
name|struct
name|nfs_args
name|nfsargs
decl_stmt|;
specifier|register
name|int
name|c
decl_stmt|;
specifier|register
name|struct
name|nfs_args
modifier|*
name|nfsargsp
init|=
operator|&
name|nfsargs
decl_stmt|;
name|struct
name|nfsd_cargs
name|ncd
decl_stmt|;
name|int
name|num
decl_stmt|,
name|flags
init|=
literal|0
decl_stmt|,
name|match
init|=
literal|1
decl_stmt|,
name|i
decl_stmt|,
name|nfssvc_flag
decl_stmt|;
name|char
modifier|*
name|spec
decl_stmt|,
modifier|*
name|name
decl_stmt|;
name|uid_t
name|last_ruid
init|=
operator|-
literal|1
decl_stmt|;
specifier|extern
name|int
name|optind
decl_stmt|;
specifier|extern
name|char
modifier|*
name|optarg
decl_stmt|;
ifdef|#
directive|ifdef
name|KERBEROS
name|strcpy
argument_list|(
name|realm
argument_list|,
name|KRB_REALM
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|nfsargs
operator|=
name|nfsdefargs
expr_stmt|;
name|retrycnt
operator|=
name|DEF_RETRY
expr_stmt|;
if|if
condition|(
name|argc
operator|<=
literal|1
condition|)
name|Usage
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"bsiTpMlqdckPF:R:r:w:t:x:g:a:L:D:Km:"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'b'
case|:
name|opflags
operator||=
name|BGRND
expr_stmt|;
break|break;
case|case
literal|'s'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_SOFT
expr_stmt|;
break|break;
case|case
literal|'i'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_INT
expr_stmt|;
break|break;
case|case
literal|'T'
case|:
name|nfsargsp
operator|->
name|sotype
operator|=
name|SOCK_STREAM
expr_stmt|;
break|break;
ifdef|#
directive|ifdef
name|ISO
case|case
literal|'p'
case|:
name|nfsargsp
operator|->
name|sotype
operator|=
name|SOCK_SEQPACKET
expr_stmt|;
break|break;
endif|#
directive|endif
case|case
literal|'M'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_MYWRITE
expr_stmt|;
break|break;
case|case
literal|'l'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_RDIRALOOK
expr_stmt|;
break|break;
case|case
literal|'q'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_NQNFS
expr_stmt|;
break|break;
case|case
literal|'d'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_DUMBTIMR
expr_stmt|;
break|break;
case|case
literal|'c'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_NOCONN
expr_stmt|;
break|break;
case|case
literal|'k'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_NQLOOKLEASE
expr_stmt|;
break|break;
case|case
literal|'P'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_RESVPORT
expr_stmt|;
break|break;
case|case
literal|'F'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|!=
literal|0
condition|)
name|flags
operator|=
name|num
expr_stmt|;
break|break;
case|case
literal|'R'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>
literal|0
condition|)
name|retrycnt
operator|=
name|num
expr_stmt|;
break|break;
case|case
literal|'r'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|nfsargsp
operator|->
name|rsize
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_RSIZE
expr_stmt|;
block|}
break|break;
case|case
literal|'w'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|nfsargsp
operator|->
name|wsize
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_WSIZE
expr_stmt|;
block|}
break|break;
case|case
literal|'t'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|nfsargsp
operator|->
name|timeo
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_TIMEO
expr_stmt|;
block|}
break|break;
case|case
literal|'x'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|nfsargsp
operator|->
name|retrans
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_RETRANS
expr_stmt|;
block|}
break|break;
case|case
literal|'g'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|set_rpc_maxgrouplist
argument_list|(
name|num
argument_list|)
expr_stmt|;
name|nfsargsp
operator|->
name|maxgrouplist
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_MAXGRPS
expr_stmt|;
block|}
break|break;
case|case
literal|'a'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>=
literal|0
condition|)
block|{
name|nfsargsp
operator|->
name|readahead
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_READAHEAD
expr_stmt|;
block|}
break|break;
case|case
literal|'L'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>=
literal|2
condition|)
block|{
name|nfsargsp
operator|->
name|leaseterm
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_LEASETERM
expr_stmt|;
block|}
break|break;
case|case
literal|'D'
case|:
if|if
condition|(
operator|(
name|num
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|nfsargsp
operator|->
name|deadthresh
operator|=
name|num
expr_stmt|;
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_DEADTHRESH
expr_stmt|;
block|}
break|break;
ifdef|#
directive|ifdef
name|KERBEROS
case|case
literal|'K'
case|:
name|nfsargsp
operator|->
name|flags
operator||=
name|NFSMNT_KERB
expr_stmt|;
break|break;
case|case
literal|'m'
case|:
name|strncpy
argument_list|(
name|realm
argument_list|,
name|optarg
argument_list|,
name|REALM_SZ
operator|-
literal|1
argument_list|)
expr_stmt|;
name|realm
index|[
name|REALM_SZ
operator|-
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
break|break;
endif|#
directive|endif
comment|/* KERBEROS */
default|default:
name|Usage
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|)
expr_stmt|;
block|}
empty_stmt|;
if|if
condition|(
operator|(
name|argc
operator|-
name|optind
operator|)
operator|==
literal|2
condition|)
block|{
name|spec
operator|=
name|argv
index|[
name|optind
index|]
expr_stmt|;
name|name
operator|=
name|argv
index|[
name|optind
operator|+
literal|1
index|]
expr_stmt|;
block|}
else|else
name|Usage
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|)
expr_stmt|;
if|if
condition|(
name|getnfsargs
argument_list|(
name|spec
argument_list|,
name|nfsargsp
argument_list|)
condition|)
block|{
if|if
condition|(
name|mount
argument_list|(
name|MOUNT_NFS
argument_list|,
name|name
argument_list|,
name|flags
argument_list|,
name|nfsargsp
argument_list|)
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|nfsargsp
operator|->
name|flags
operator|&
operator|(
name|NFSMNT_NQNFS
operator||
name|NFSMNT_KERB
operator|)
condition|)
block|{
if|if
condition|(
operator|(
name|opflags
operator|&
name|ISBGRND
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|i
operator|=
name|fork
argument_list|()
condition|)
block|{
if|if
condition|(
name|i
operator|==
operator|-
literal|1
condition|)
block|{
name|perror
argument_list|(
literal|"nqnfs"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|setsid
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
literal|1
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
literal|2
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|chdir
argument_list|(
literal|"/"
argument_list|)
expr_stmt|;
block|}
name|openlog
argument_list|(
literal|"mount_nfs:"
argument_list|,
name|LOG_PID
argument_list|,
name|LOG_DAEMON
argument_list|)
expr_stmt|;
name|nfssvc_flag
operator|=
name|NFSSVC_MNTD
expr_stmt|;
name|ncd
operator|.
name|ncd_dirp
operator|=
name|name
expr_stmt|;
while|while
condition|(
name|nfssvc
argument_list|(
name|nfssvc_flag
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|ncd
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|==
name|ENEEDAUTH
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"in eacces"
argument_list|)
expr_stmt|;
name|nfssvc_flag
operator|=
name|NFSSVC_MNTD
operator||
name|NFSSVC_GOTAUTH
operator||
name|NFSSVC_AUTHINFAIL
expr_stmt|;
ifdef|#
directive|ifdef
name|KERBEROS
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"Callin krb uid=%d inst=%s realm=%s"
argument_list|,
name|ncd
operator|.
name|ncd_authuid
argument_list|,
name|inst
argument_list|,
name|realm
argument_list|)
expr_stmt|;
comment|/* 				 * Set up as ncd_authuid for the kerberos call. 				 * Must set ruid to ncd_authuid and reset the 				 * ticket name iff ncd_authuid is not the same 				 * as last time, so that the right ticket file 				 * is found. 				 */
if|if
condition|(
name|ncd
operator|.
name|ncd_authuid
operator|!=
name|last_ruid
condition|)
block|{
name|krb_set_tkt_string
argument_list|(
literal|""
argument_list|)
expr_stmt|;
name|last_ruid
operator|=
name|ncd
operator|.
name|ncd_authuid
expr_stmt|;
block|}
name|setreuid
argument_list|(
name|ncd
operator|.
name|ncd_authuid
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|krb_mk_req
argument_list|(
operator|&
name|kt
argument_list|,
literal|"rcmd"
argument_list|,
name|inst
argument_list|,
name|realm
argument_list|,
literal|0
argument_list|)
operator|==
name|KSUCCESS
operator|&&
name|kt
operator|.
name|length
operator|<=
operator|(
name|RPCAUTH_MAXSIZ
operator|-
literal|2
operator|*
name|NFSX_UNSIGNED
operator|)
condition|)
block|{
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"Got it\n"
argument_list|)
expr_stmt|;
name|ncd
operator|.
name|ncd_authtype
operator|=
name|RPCAUTH_NQNFS
expr_stmt|;
name|ncd
operator|.
name|ncd_authlen
operator|=
name|kt
operator|.
name|length
expr_stmt|;
name|ncd
operator|.
name|ncd_authstr
operator|=
operator|(
name|char
operator|*
operator|)
name|kt
operator|.
name|dat
expr_stmt|;
name|nfssvc_flag
operator|=
name|NFSSVC_MNTD
operator||
name|NFSSVC_GOTAUTH
expr_stmt|;
block|}
name|setreuid
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"ktlen=%d\n"
argument_list|,
name|kt
operator|.
name|length
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* KERBEROS */
block|}
else|else
name|syslog
argument_list|(
name|LOG_ERR
argument_list|,
literal|"nfssvc err %m"
argument_list|)
expr_stmt|;
block|}
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_macro
name|getnfsargs
argument_list|(
argument|spec
argument_list|,
argument|nfsargsp
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|spec
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|nfs_args
modifier|*
name|nfsargsp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|CLIENT
modifier|*
name|clp
decl_stmt|;
name|struct
name|hostent
modifier|*
name|hp
decl_stmt|;
specifier|static
name|struct
name|sockaddr_in
name|saddr
decl_stmt|;
ifdef|#
directive|ifdef
name|ISO
specifier|static
name|struct
name|sockaddr_iso
name|isoaddr
decl_stmt|;
name|struct
name|iso_addr
modifier|*
name|isop
decl_stmt|;
endif|#
directive|endif
name|struct
name|timeval
name|pertry
decl_stmt|,
name|try
decl_stmt|;
name|enum
name|clnt_stat
name|clnt_stat
decl_stmt|;
name|int
name|so
init|=
name|RPC_ANYSOCK
decl_stmt|,
name|isoflag
init|=
literal|0
decl_stmt|,
name|i
decl_stmt|;
name|char
modifier|*
name|hostp
decl_stmt|,
modifier|*
name|delimp
decl_stmt|,
modifier|*
name|cp
decl_stmt|;
name|u_short
name|tport
decl_stmt|;
specifier|static
name|struct
name|nfhret
name|nfhret
decl_stmt|;
specifier|static
name|char
name|nam
index|[
name|MNAMELEN
operator|+
literal|1
index|]
decl_stmt|;
name|strncpy
argument_list|(
name|nam
argument_list|,
name|spec
argument_list|,
name|MNAMELEN
argument_list|)
expr_stmt|;
name|nam
index|[
name|MNAMELEN
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
operator|(
name|delimp
operator|=
name|index
argument_list|(
name|spec
argument_list|,
literal|'@'
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
name|hostp
operator|=
name|delimp
operator|+
literal|1
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|delimp
operator|=
name|index
argument_list|(
name|spec
argument_list|,
literal|':'
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
name|hostp
operator|=
name|spec
expr_stmt|;
name|spec
operator|=
name|delimp
operator|+
literal|1
expr_stmt|;
block|}
else|else
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"No<host>:<dirpath> or<dirpath>@<host> spec\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
operator|*
name|delimp
operator|=
literal|'\0'
expr_stmt|;
comment|/* 	 * DUMB!! Until the mount protocol works on iso transport, we must 	 * supply both an iso and an inet address for the host. 	 */
ifdef|#
directive|ifdef
name|ISO
if|if
condition|(
operator|!
name|strncmp
argument_list|(
name|hostp
argument_list|,
literal|"iso="
argument_list|,
literal|4
argument_list|)
condition|)
block|{
name|u_short
name|isoport
decl_stmt|;
name|hostp
operator|+=
literal|4
expr_stmt|;
name|isoflag
operator|++
expr_stmt|;
if|if
condition|(
operator|(
name|delimp
operator|=
name|index
argument_list|(
name|hostp
argument_list|,
literal|'+'
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"No iso+inet address\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
operator|*
name|delimp
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
operator|(
name|isop
operator|=
name|iso_addr
argument_list|(
name|hostp
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Bad iso address\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|isoaddr
argument_list|,
sizeof|sizeof
argument_list|(
name|isoaddr
argument_list|)
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|isop
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|isoaddr
operator|.
name|siso_addr
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|iso_addr
argument_list|)
argument_list|)
expr_stmt|;
name|isoaddr
operator|.
name|siso_len
operator|=
sizeof|sizeof
argument_list|(
name|isoaddr
argument_list|)
expr_stmt|;
name|isoaddr
operator|.
name|siso_family
operator|=
name|AF_ISO
expr_stmt|;
name|isoaddr
operator|.
name|siso_tlen
operator|=
literal|2
expr_stmt|;
name|isoport
operator|=
name|htons
argument_list|(
name|NFS_PORT
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|isoport
argument_list|,
name|TSEL
argument_list|(
operator|&
name|isoaddr
argument_list|)
argument_list|,
name|isoaddr
operator|.
name|siso_tlen
argument_list|)
expr_stmt|;
name|hostp
operator|=
name|delimp
operator|+
literal|1
expr_stmt|;
block|}
endif|#
directive|endif
comment|/* ISO */
comment|/* 	 * Handle an internet host address and reverse resolve it if 	 * doing Kerberos. 	 */
if|if
condition|(
name|isdigit
argument_list|(
operator|*
name|hostp
argument_list|)
condition|)
block|{
if|if
condition|(
operator|(
name|saddr
operator|.
name|sin_addr
operator|.
name|s_addr
operator|=
name|inet_addr
argument_list|(
name|hostp
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Bad net addr %s\n"
argument_list|,
name|hostp
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
operator|(
name|nfsargsp
operator|->
name|flags
operator|&
name|NFSMNT_KERB
operator|)
operator|&&
operator|(
name|hp
operator|=
name|gethostbyaddr
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|saddr
operator|.
name|sin_addr
operator|.
name|s_addr
argument_list|,
sizeof|sizeof
argument_list|(
name|u_long
argument_list|)
argument_list|,
name|AF_INET
argument_list|)
operator|)
operator|==
operator|(
expr|struct
name|hostent
operator|*
operator|)
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Can't reverse resolve net addr\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
block|}
elseif|else
if|if
condition|(
operator|(
name|hp
operator|=
name|gethostbyname
argument_list|(
name|hostp
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Can't get net id for host\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
ifdef|#
directive|ifdef
name|KERBEROS
if|if
condition|(
name|nfsargsp
operator|->
name|flags
operator|&
name|NFSMNT_KERB
condition|)
block|{
name|strncpy
argument_list|(
name|inst
argument_list|,
name|hp
operator|->
name|h_name
argument_list|,
name|INST_SZ
argument_list|)
expr_stmt|;
name|inst
index|[
name|INST_SZ
operator|-
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|cp
operator|=
name|index
argument_list|(
name|inst
argument_list|,
literal|'.'
argument_list|)
condition|)
operator|*
name|cp
operator|=
literal|'\0'
expr_stmt|;
block|}
endif|#
directive|endif
comment|/* KERBEROS */
name|bcopy
argument_list|(
name|hp
operator|->
name|h_addr
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|saddr
operator|.
name|sin_addr
argument_list|,
name|hp
operator|->
name|h_length
argument_list|)
expr_stmt|;
name|nfhret
operator|.
name|stat
operator|=
name|EACCES
expr_stmt|;
comment|/* Mark not yet successful */
while|while
condition|(
name|retrycnt
operator|>
literal|0
condition|)
block|{
name|saddr
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|saddr
operator|.
name|sin_port
operator|=
name|htons
argument_list|(
name|PMAPPORT
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|tport
operator|=
name|pmap_getport
argument_list|(
operator|&
name|saddr
argument_list|,
name|RPCPROG_NFS
argument_list|,
name|NFS_VER2
argument_list|,
name|IPPROTO_UDP
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
operator|(
name|opflags
operator|&
name|ISBGRND
operator|)
operator|==
literal|0
condition|)
name|clnt_pcreateerror
argument_list|(
literal|"NFS Portmap"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|saddr
operator|.
name|sin_port
operator|=
literal|0
expr_stmt|;
name|pertry
operator|.
name|tv_sec
operator|=
literal|10
expr_stmt|;
name|pertry
operator|.
name|tv_usec
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|(
name|clp
operator|=
name|clntudp_create
argument_list|(
operator|&
name|saddr
argument_list|,
name|RPCPROG_MNT
argument_list|,
name|RPCMNT_VER1
argument_list|,
name|pertry
argument_list|,
operator|&
name|so
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
operator|(
name|opflags
operator|&
name|ISBGRND
operator|)
operator|==
literal|0
condition|)
name|clnt_pcreateerror
argument_list|(
literal|"Cannot MNT PRC"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|clp
operator|->
name|cl_auth
operator|=
name|authunix_create_default
argument_list|()
expr_stmt|;
name|try
operator|.
name|tv_sec
operator|=
literal|10
expr_stmt|;
name|try
operator|.
name|tv_usec
operator|=
literal|0
expr_stmt|;
name|clnt_stat
operator|=
name|clnt_call
argument_list|(
name|clp
argument_list|,
name|RPCMNT_MOUNT
argument_list|,
name|xdr_dir
argument_list|,
name|spec
argument_list|,
name|xdr_fh
argument_list|,
operator|&
name|nfhret
argument_list|,
name|try
argument_list|)
expr_stmt|;
if|if
condition|(
name|clnt_stat
operator|!=
name|RPC_SUCCESS
condition|)
block|{
if|if
condition|(
operator|(
name|opflags
operator|&
name|ISBGRND
operator|)
operator|==
literal|0
condition|)
name|clnt_perror
argument_list|(
name|clp
argument_list|,
literal|"Bad MNT RPC"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|auth_destroy
argument_list|(
name|clp
operator|->
name|cl_auth
argument_list|)
expr_stmt|;
name|clnt_destroy
argument_list|(
name|clp
argument_list|)
expr_stmt|;
name|retrycnt
operator|=
literal|0
expr_stmt|;
block|}
block|}
block|}
if|if
condition|(
operator|--
name|retrycnt
operator|>
literal|0
condition|)
block|{
if|if
condition|(
name|opflags
operator|&
name|BGRND
condition|)
block|{
name|opflags
operator|&=
operator|~
name|BGRND
expr_stmt|;
if|if
condition|(
name|i
operator|=
name|fork
argument_list|()
condition|)
block|{
if|if
condition|(
name|i
operator|==
operator|-
literal|1
condition|)
block|{
name|perror
argument_list|(
literal|"nqnfs"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|setsid
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
literal|1
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
literal|2
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|chdir
argument_list|(
literal|"/"
argument_list|)
expr_stmt|;
name|opflags
operator||=
name|ISBGRND
expr_stmt|;
block|}
name|sleep
argument_list|(
literal|60
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|nfhret
operator|.
name|stat
condition|)
block|{
if|if
condition|(
name|opflags
operator|&
name|ISBGRND
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Can't access %s: "
argument_list|,
name|spec
argument_list|)
expr_stmt|;
name|errno
operator|=
name|nfhret
operator|.
name|stat
expr_stmt|;
name|perror
argument_list|(
name|NULL
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|saddr
operator|.
name|sin_port
operator|=
name|htons
argument_list|(
name|tport
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|ISO
if|if
condition|(
name|isoflag
condition|)
block|{
name|nfsargsp
operator|->
name|addr
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|isoaddr
expr_stmt|;
name|nfsargsp
operator|->
name|addrlen
operator|=
sizeof|sizeof
argument_list|(
name|isoaddr
argument_list|)
expr_stmt|;
block|}
else|else
endif|#
directive|endif
comment|/* ISO */
block|{
name|nfsargsp
operator|->
name|addr
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|saddr
expr_stmt|;
name|nfsargsp
operator|->
name|addrlen
operator|=
sizeof|sizeof
argument_list|(
name|saddr
argument_list|)
expr_stmt|;
block|}
name|nfsargsp
operator|->
name|fh
operator|=
operator|&
name|nfhret
operator|.
name|nfh
expr_stmt|;
name|nfsargsp
operator|->
name|hostname
operator|=
name|nam
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * xdr routines for mount rpc's  */
end_comment

begin_macro
name|xdr_dir
argument_list|(
argument|xdrsp
argument_list|,
argument|dirp
argument_list|)
end_macro

begin_decl_stmt
name|XDR
modifier|*
name|xdrsp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|dirp
decl_stmt|;
end_decl_stmt

begin_block
block|{
return|return
operator|(
name|xdr_string
argument_list|(
name|xdrsp
argument_list|,
operator|&
name|dirp
argument_list|,
name|RPCMNT_PATHLEN
argument_list|)
operator|)
return|;
block|}
end_block

begin_macro
name|xdr_fh
argument_list|(
argument|xdrsp
argument_list|,
argument|np
argument_list|)
end_macro

begin_decl_stmt
name|XDR
modifier|*
name|xdrsp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|nfhret
modifier|*
name|np
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
operator|!
name|xdr_u_long
argument_list|(
name|xdrsp
argument_list|,
operator|&
operator|(
name|np
operator|->
name|stat
operator|)
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
if|if
condition|(
name|np
operator|->
name|stat
condition|)
return|return
operator|(
literal|1
operator|)
return|;
return|return
operator|(
name|xdr_opaque
argument_list|(
name|xdrsp
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
operator|(
name|np
operator|->
name|nfh
operator|)
argument_list|,
name|NFSX_FH
argument_list|)
operator|)
return|;
block|}
end_block

begin_macro
name|Usage
argument_list|(
argument|argc
argument_list|,
argument|argv
argument_list|)
end_macro

begin_decl_stmt
name|int
name|argc
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|argc
condition|;
name|i
operator|++
control|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s "
argument_list|,
name|argv
index|[
name|i
index|]
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\nBad mount_nfs arg\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

end_unit

