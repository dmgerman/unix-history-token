...
name|lfs_ssize
expr_stmt|;
comment|/*  	 * The number of free blocks is set from the total data size (lfs_dsize) 	 * minus one block for each segment (for the segment summary).  Then  	 * we'll dubstract off the room for the superblocks, ifile entries and 	 * segment usage table  	 */
name|lfsp
operator|->
name|lfs_bfree
operator|=
...
index|]
expr_stmt|;
comment|/* 	 * Initialize the segment usage table.  The first segment will 	 * contain the superblock, the segusage table (segtabsz), 1 	 * block's worth of IFILE entries, the root directory, the lost+found 	 * directory and one block's worth of inodes (containing the ifile, 	 * root, and l+f inodes). 	 */
if|if
condition|(
operator|!
operator|(
...
argument_list|)
expr_stmt|;
comment|/* Create a block of the IFILES. */
if|if
condition|(
operator|!
operator|(
...
name|LFS_UNUSED_INUM
expr_stmt|;
comment|/* 	 * Make sure that we don't overflow a summary block. We have to 	 * record: FINFO structures for superblock, ifile, root, l+f. 	 */
name|sum_size
operator|=
literal|4
operator|*
...
argument_list|)
expr_stmt|;
comment|/*  	 * The first FINFO structure from above is good, just need to change 	 * the pointers 	 */
name|sp1
operator|=
operator|(
name|SEGSUM
