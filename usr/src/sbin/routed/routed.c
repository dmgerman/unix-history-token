begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)routed.c	4.15 82/06/09"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Routing Table Management Daemon  */
end_comment

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<net/in.h>
end_include

begin_include
include|#
directive|include
file|<net/if.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<nlist.h>
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<time.h>
end_include

begin_define
define|#
directive|define
name|RIPCMDS
end_define

begin_include
include|#
directive|include
file|"rip.h"
end_include

begin_include
include|#
directive|include
file|"router.h"
end_include

begin_define
define|#
directive|define
name|LOOPBACKNET
value|0177
end_define

begin_comment
comment|/* casts to keep lint happy */
end_comment

begin_define
define|#
directive|define
name|insque
parameter_list|(
name|q
parameter_list|,
name|p
parameter_list|)
value|_insque((caddr_t)q,(caddr_t)p)
end_define

begin_define
define|#
directive|define
name|remque
parameter_list|(
name|q
parameter_list|)
value|_remque((caddr_t)q)
end_define

begin_define
define|#
directive|define
name|equal
parameter_list|(
name|a1
parameter_list|,
name|a2
parameter_list|)
define|\
value|(bcmp((caddr_t)(a1), (caddr_t)(a2), sizeof (struct sockaddr)) == 0)
end_define

begin_define
define|#
directive|define
name|min
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|((a)>(b)?(b):(a))
end_define

begin_decl_stmt
name|struct
name|nlist
name|nl
index|[]
init|=
block|{
define|#
directive|define
name|N_IFNET
value|0
block|{
literal|"_ifnet"
block|}
block|,
literal|0
block|, }
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr_in
name|routingaddr
init|=
block|{
name|AF_INET
block|,
name|IPPORT_ROUTESERVER
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr_in
name|noroutingaddr
init|=
block|{
name|AF_INET
block|,
name|IPPORT_ROUTESERVER
operator|+
literal|1
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|snoroute
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* socket with no routing */
end_comment

begin_decl_stmt
name|int
name|kmem
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|supplier
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* process should supply updates */
end_comment

begin_decl_stmt
name|int
name|install
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* if 1 call kernel */
end_comment

begin_decl_stmt
name|int
name|timeval
init|=
operator|-
name|TIMER_RATE
decl_stmt|;
end_decl_stmt

begin_function_decl
name|int
name|timer
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|int
name|cleanup
parameter_list|()
function_decl|;
end_function_decl

begin_define
define|#
directive|define
name|tprintf
value|if (trace) printf
end_define

begin_decl_stmt
name|int
name|trace
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|FILE
modifier|*
name|ftrace
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|packet
index|[
name|MAXPACKETSIZE
operator|+
literal|1
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|rip
modifier|*
name|msg
init|=
operator|(
expr|struct
name|rip
operator|*
operator|)
name|packet
decl_stmt|;
end_decl_stmt

begin_function_decl
name|struct
name|in_addr
name|if_makeaddr
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|if_ifwithaddr
argument_list|()
decl_stmt|,
modifier|*
name|if_ifwithnet
argument_list|()
decl_stmt|;
end_decl_stmt

begin_function_decl
specifier|extern
name|char
modifier|*
name|malloc
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
specifier|extern
name|int
name|errno
decl_stmt|,
name|exit
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
modifier|*
name|argv0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|sendmsg
argument_list|()
decl_stmt|,
name|supply
argument_list|()
decl_stmt|;
end_decl_stmt

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
name|int
name|cc
decl_stmt|;
name|struct
name|sockaddr
name|from
decl_stmt|;
name|argv0
operator|=
name|argv
expr_stmt|;
ifndef|#
directive|ifndef
name|DEBUG
if|if
condition|(
name|fork
argument_list|()
condition|)
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
for|for
control|(
name|cc
operator|=
literal|0
init|;
name|cc
operator|<
literal|10
condition|;
name|cc
operator|++
control|)
operator|(
name|void
operator|)
name|close
argument_list|(
name|cc
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|open
argument_list|(
literal|"/"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|dup2
argument_list|(
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|dup2
argument_list|(
literal|0
argument_list|,
literal|2
argument_list|)
expr_stmt|;
block|{
name|int
name|t
init|=
name|open
argument_list|(
literal|"/dev/tty"
argument_list|,
literal|2
argument_list|)
decl_stmt|;
if|if
condition|(
name|t
operator|>=
literal|0
condition|)
block|{
name|ioctl
argument_list|(
name|t
argument_list|,
name|TIOCNOTTY
argument_list|,
operator|(
name|char
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
name|t
argument_list|)
expr_stmt|;
block|}
block|}
endif|#
directive|endif
if|if
condition|(
name|trace
condition|)
block|{
name|ftrace
operator|=
name|fopen
argument_list|(
literal|"/etc/routerlog"
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
name|dup2
argument_list|(
name|fileno
argument_list|(
name|ftrace
argument_list|)
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|dup2
argument_list|(
name|fileno
argument_list|(
name|ftrace
argument_list|)
argument_list|,
literal|2
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|vax
name|||
name|pdp11
name|routingaddr
operator|.
name|sin_port
operator|=
name|htons
argument_list|(
name|routingaddr
operator|.
name|sin_port
argument_list|)
expr_stmt|;
name|noroutingaddr
operator|.
name|sin_port
operator|=
name|htons
argument_list|(
name|noroutingaddr
operator|.
name|sin_port
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|again
label|:
name|s
operator|=
name|socket
argument_list|(
name|SOCK_DGRAM
argument_list|,
literal|0
argument_list|,
operator|&
name|routingaddr
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|s
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
literal|"socket"
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|30
argument_list|)
expr_stmt|;
goto|goto
name|again
goto|;
block|}
name|again2
label|:
name|snoroute
operator|=
name|socket
argument_list|(
name|SOCK_DGRAM
argument_list|,
literal|0
argument_list|,
operator|&
name|noroutingaddr
argument_list|,
name|SO_DONTROUTE
argument_list|)
expr_stmt|;
if|if
condition|(
name|snoroute
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
literal|"socket"
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|30
argument_list|)
expr_stmt|;
goto|goto
name|again2
goto|;
block|}
name|argv
operator|++
operator|,
name|argc
operator|--
expr_stmt|;
while|while
condition|(
name|argc
operator|>
literal|0
operator|&&
operator|*
operator|*
name|argv
operator|==
literal|'-'
condition|)
block|{
if|if
condition|(
operator|!
name|strcmp
argument_list|(
operator|*
name|argv
argument_list|,
literal|"-s"
argument_list|)
operator|==
literal|0
condition|)
block|{
name|supplier
operator|=
literal|1
expr_stmt|;
name|argv
operator|++
operator|,
name|argc
operator|--
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
operator|*
name|argv
argument_list|,
literal|"-q"
argument_list|)
operator|==
literal|0
condition|)
block|{
name|supplier
operator|=
literal|0
expr_stmt|;
name|argv
operator|++
operator|,
name|argc
operator|--
expr_stmt|;
continue|continue;
block|}
goto|goto
name|usage
goto|;
block|}
if|if
condition|(
name|argc
operator|>
literal|0
condition|)
block|{
name|usage
label|:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: routed [ -s ]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|rtinit
argument_list|()
expr_stmt|;
name|ifinit
argument_list|()
expr_stmt|;
if|if
condition|(
name|supplier
operator|<
literal|0
condition|)
name|supplier
operator|=
literal|0
expr_stmt|;
name|gwkludge
argument_list|()
expr_stmt|;
name|msg
operator|->
name|rip_cmd
operator|=
name|RIPCMD_REQUEST
expr_stmt|;
name|msg
operator|->
name|rip_nets
index|[
literal|0
index|]
operator|.
name|rip_dst
operator|.
name|sa_family
operator|=
name|AF_UNSPEC
expr_stmt|;
name|msg
operator|->
name|rip_nets
index|[
literal|0
index|]
operator|.
name|rip_metric
operator|=
name|HOPCNT_INFINITY
expr_stmt|;
name|toall
argument_list|(
name|sendmsg
argument_list|)
expr_stmt|;
name|sigset
argument_list|(
name|SIGALRM
argument_list|,
name|timer
argument_list|)
expr_stmt|;
name|timer
argument_list|()
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|cc
operator|=
name|receive
argument_list|(
name|s
argument_list|,
operator|&
name|from
argument_list|,
name|packet
argument_list|,
sizeof|sizeof
argument_list|(
name|packet
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|cc
operator|<=
literal|0
condition|)
block|{
if|if
condition|(
name|cc
operator|<
literal|0
operator|&&
name|errno
operator|!=
name|EINTR
condition|)
name|perror
argument_list|(
literal|"receive"
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|sighold
argument_list|(
name|SIGALRM
argument_list|)
expr_stmt|;
name|rip_input
argument_list|(
operator|&
name|from
argument_list|,
name|cc
argument_list|)
expr_stmt|;
name|sigrelse
argument_list|(
name|SIGALRM
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_macro
name|rtinit
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|struct
name|rthash
modifier|*
name|rh
decl_stmt|;
for|for
control|(
name|rh
operator|=
name|nethash
init|;
name|rh
operator|<
operator|&
name|nethash
index|[
name|ROUTEHASHSIZ
index|]
condition|;
name|rh
operator|++
control|)
name|rh
operator|->
name|rt_forw
operator|=
name|rh
operator|->
name|rt_back
operator|=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|rh
expr_stmt|;
for|for
control|(
name|rh
operator|=
name|hosthash
init|;
name|rh
operator|<
operator|&
name|hosthash
index|[
name|ROUTEHASHSIZ
index|]
condition|;
name|rh
operator|++
control|)
name|rh
operator|->
name|rt_forw
operator|=
name|rh
operator|->
name|rt_back
operator|=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|rh
expr_stmt|;
block|}
end_block

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|ifnet
decl_stmt|;
end_decl_stmt

begin_macro
name|ifinit
argument_list|()
end_macro

begin_block
block|{
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|,
modifier|*
name|next
decl_stmt|;
name|int
name|uniquemultihostinterfaces
init|=
literal|0
decl_stmt|;
name|nlist
argument_list|(
literal|"/vmunix"
argument_list|,
name|nl
argument_list|)
expr_stmt|;
if|if
condition|(
name|nl
index|[
name|N_IFNET
index|]
operator|.
name|n_value
operator|==
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"ifnet: not in namelist\n"
argument_list|)
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|kmem
operator|=
name|open
argument_list|(
literal|"/dev/kmem"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|kmem
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
literal|"/dev/kmem"
argument_list|)
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
if|if
condition|(
name|lseek
argument_list|(
name|kmem
argument_list|,
operator|(
name|long
operator|)
name|nl
index|[
name|N_IFNET
index|]
operator|.
name|n_value
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
operator|||
name|read
argument_list|(
name|kmem
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|next
argument_list|,
sizeof|sizeof
argument_list|(
name|next
argument_list|)
argument_list|)
operator|!=
sizeof|sizeof
argument_list|(
name|next
argument_list|)
condition|)
block|{
name|printf
argument_list|(
literal|"ifnet: error reading kmem\n"
argument_list|)
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
while|while
condition|(
name|next
condition|)
block|{
name|ifp
operator|=
operator|(
expr|struct
name|ifnet
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|ifnet
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifp
operator|==
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"routed: out of memory\n"
argument_list|)
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|lseek
argument_list|(
name|kmem
argument_list|,
operator|(
name|long
operator|)
name|next
argument_list|,
literal|0
argument_list|)
operator|==
operator|-
literal|1
operator|||
name|read
argument_list|(
name|kmem
argument_list|,
operator|(
name|char
operator|*
operator|)
name|ifp
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ifp
argument_list|)
argument_list|)
operator|!=
sizeof|sizeof
argument_list|(
operator|*
name|ifp
argument_list|)
condition|)
block|{
name|perror
argument_list|(
literal|"read"
argument_list|)
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|next
operator|=
name|ifp
operator|->
name|if_next
expr_stmt|;
if|if
condition|(
name|ifp
operator|->
name|if_addr
operator|.
name|sa_family
operator|!=
name|AF_INET
condition|)
continue|continue;
if|if
condition|(
name|ifp
operator|->
name|if_net
operator|==
name|LOOPBACKNET
condition|)
continue|continue;
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
operator|)
operator|==
literal|0
operator|||
name|if_ifwithaddr
argument_list|(
operator|&
name|ifp
operator|->
name|if_dstaddr
argument_list|)
operator|==
literal|0
condition|)
name|uniquemultihostinterfaces
operator|++
expr_stmt|;
name|ifp
operator|->
name|if_next
operator|=
name|ifnet
expr_stmt|;
name|ifnet
operator|=
name|ifp
expr_stmt|;
name|addrouteforif
argument_list|(
name|ifp
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|uniquemultihostinterfaces
operator|>
literal|1
operator|&&
name|supplier
operator|<
literal|0
condition|)
name|supplier
operator|=
literal|1
expr_stmt|;
return|return;
name|bad
label|:
name|sleep
argument_list|(
literal|60
argument_list|)
expr_stmt|;
name|execv
argument_list|(
literal|"/etc/routed"
argument_list|,
name|argv0
argument_list|)
expr_stmt|;
name|_exit
argument_list|(
literal|0177
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|addrouteforif
argument_list|(
argument|ifp
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|sockaddr_in
name|net
decl_stmt|;
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
if|if
condition|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
condition|)
name|dst
operator|=
operator|&
name|ifp
operator|->
name|if_dstaddr
expr_stmt|;
else|else
block|{
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|net
argument_list|,
sizeof|sizeof
argument_list|(
name|net
argument_list|)
argument_list|)
expr_stmt|;
name|net
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|net
operator|.
name|sin_addr
operator|=
name|if_makeaddr
argument_list|(
name|ifp
operator|->
name|if_net
argument_list|,
name|INADDR_ANY
argument_list|)
expr_stmt|;
name|dst
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|net
expr_stmt|;
block|}
name|rtadd
argument_list|(
name|dst
argument_list|,
operator|&
name|ifp
operator|->
name|if_addr
argument_list|,
literal|0
argument_list|,
name|RTS_DONTDELETE
operator||
name|RTS_DONTROUTE
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|gwkludge
argument_list|()
end_macro

begin_block
block|{
name|struct
name|sockaddr_in
name|dst
decl_stmt|,
name|gate
decl_stmt|;
name|FILE
modifier|*
name|fp
decl_stmt|;
name|char
name|buf
index|[
name|BUFSIZ
index|]
decl_stmt|;
name|fp
operator|=
name|fopen
argument_list|(
literal|"/etc/gateways"
argument_list|,
literal|"r"
argument_list|)
expr_stmt|;
if|if
condition|(
name|fp
operator|==
name|NULL
condition|)
return|return;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|dst
argument_list|,
sizeof|sizeof
argument_list|(
name|dst
argument_list|)
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|gate
argument_list|,
sizeof|sizeof
argument_list|(
name|gate
argument_list|)
argument_list|)
expr_stmt|;
name|dst
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|gate
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|buf
index|[
literal|0
index|]
operator|=
literal|'\0'
expr_stmt|;
comment|/* format is: dst XX gateway XX [passive]\n */
if|if
condition|(
name|fscanf
argument_list|(
name|fp
argument_list|,
literal|"dst %x gateway %x %s\n"
argument_list|,
operator|&
name|dst
operator|.
name|sin_addr
operator|.
name|s_addr
argument_list|,
operator|&
name|gate
operator|.
name|sin_addr
operator|.
name|s_addr
argument_list|,
name|buf
argument_list|)
operator|==
name|EOF
condition|)
break|break;
name|rtadd
argument_list|(
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|dst
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|gate
argument_list|,
literal|1
argument_list|,
name|strcmp
argument_list|(
name|buf
argument_list|,
literal|"passive"
argument_list|)
operator|==
literal|0
condition|?
name|RTS_PASSIVE
else|:
name|RTS_DONTDELETE
argument_list|)
expr_stmt|;
block|}
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|timer
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|struct
name|rthash
modifier|*
name|rh
decl_stmt|;
specifier|register
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
name|struct
name|rthash
modifier|*
name|base
init|=
name|hosthash
decl_stmt|;
name|int
name|doinghost
init|=
literal|1
decl_stmt|,
name|state
decl_stmt|;
name|timeval
operator|+=
name|TIMER_RATE
expr_stmt|;
name|tprintf
argument_list|(
literal|">>> time %d>>>\n"
argument_list|,
name|timeval
argument_list|)
expr_stmt|;
name|again
label|:
for|for
control|(
name|rh
operator|=
name|base
init|;
name|rh
operator|<
operator|&
name|base
index|[
name|ROUTEHASHSIZ
index|]
condition|;
name|rh
operator|++
control|)
block|{
name|rt
operator|=
name|rh
operator|->
name|rt_forw
expr_stmt|;
for|for
control|(
init|;
name|rt
operator|!=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|rh
condition|;
name|rt
operator|=
name|rt
operator|->
name|rt_forw
control|)
block|{
if|if
condition|(
operator|!
operator|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_PASSIVE
operator|)
condition|)
block|{
name|rt
operator|->
name|rt_timer
operator|+=
name|TIMER_RATE
expr_stmt|;
if|if
condition|(
name|rt
operator|->
name|rt_timer
operator|>
literal|9999
condition|)
name|rt
operator|->
name|rt_timer
operator|=
literal|9999
expr_stmt|;
block|}
name|log
argument_list|(
literal|""
argument_list|,
name|rt
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_HIDDEN
condition|)
continue|continue;
if|if
condition|(
name|rt
operator|->
name|rt_timer
operator|>=
name|EXPIRE_TIME
condition|)
name|rt
operator|->
name|rt_metric
operator|=
name|HOPCNT_INFINITY
expr_stmt|;
if|if
condition|(
operator|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_DELRT
operator|)
operator|||
name|rt
operator|->
name|rt_timer
operator|>=
name|GARBAGE_TIME
condition|)
block|{
name|rt
operator|=
name|rt
operator|->
name|rt_back
expr_stmt|;
name|rtdelete
argument_list|(
name|rt
operator|->
name|rt_forw
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|state
operator|=
name|rt
operator|->
name|rt_state
expr_stmt|;
if|if
condition|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_ADDRT
condition|)
block|{
if|if
condition|(
name|ioctl
argument_list|(
name|s
argument_list|,
name|SIOCADDRT
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|rt
operator|->
name|rt_rt
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
literal|"SIOCADDRT"
argument_list|)
expr_stmt|;
name|rt
operator|->
name|rt_state
operator|&=
operator|~
name|RTS_ADDRT
expr_stmt|;
block|}
if|if
condition|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_CHGRT
condition|)
block|{
name|struct
name|rtentry
name|oldroute
decl_stmt|;
name|oldroute
operator|=
name|rt
operator|->
name|rt_rt
expr_stmt|;
name|oldroute
operator|.
name|rt_gateway
operator|=
name|rt
operator|->
name|rt_oldrouter
expr_stmt|;
if|if
condition|(
name|install
operator|&&
name|ioctl
argument_list|(
name|s
argument_list|,
name|SIOCADDRT
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|rt
operator|->
name|rt_rt
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
literal|"SIOCADDRT"
argument_list|)
expr_stmt|;
if|if
condition|(
name|install
operator|&&
name|ioctl
argument_list|(
name|s
argument_list|,
name|SIOCDELRT
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|oldroute
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
literal|"SIOCDELRT"
argument_list|)
expr_stmt|;
name|rt
operator|->
name|rt_state
operator|&=
operator|~
name|RTS_CHGRT
expr_stmt|;
block|}
if|if
condition|(
name|supplier
operator|&&
operator|(
name|state
operator|&
operator|(
name|RTS_CHGRT
operator||
name|RTS_ADDRT
operator|)
operator|)
condition|)
block|{
name|log
argument_list|(
literal|"broadcast"
argument_list|,
name|rt
argument_list|)
expr_stmt|;
name|msg
operator|->
name|rip_cmd
operator|=
name|RIPCMD_RESPONSE
expr_stmt|;
name|msg
operator|->
name|rip_nets
index|[
literal|0
index|]
operator|.
name|rip_dst
operator|=
name|rt
operator|->
name|rt_dst
expr_stmt|;
name|msg
operator|->
name|rip_nets
index|[
literal|0
index|]
operator|.
name|rip_metric
operator|=
name|min
argument_list|(
name|rt
operator|->
name|rt_metric
operator|+
literal|1
argument_list|,
name|HOPCNT_INFINITY
argument_list|)
expr_stmt|;
name|toall
argument_list|(
name|sendmsg
argument_list|)
expr_stmt|;
block|}
block|}
block|}
if|if
condition|(
name|doinghost
condition|)
block|{
name|doinghost
operator|=
literal|0
expr_stmt|;
name|base
operator|=
name|nethash
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|supplier
operator|&&
operator|(
name|timeval
operator|%
name|SUPPLY_INTERVAL
operator|)
operator|==
literal|0
condition|)
name|toall
argument_list|(
name|supply
argument_list|)
expr_stmt|;
name|tprintf
argument_list|(
literal|"<<< time %d<<<\n"
argument_list|,
name|timeval
argument_list|)
expr_stmt|;
name|alarm
argument_list|(
name|TIMER_RATE
argument_list|)
expr_stmt|;
block|}
end_block

begin_function_decl
name|toall
function_decl|(
name|f
function_decl|)
name|int
argument_list|(
argument|*f
argument_list|)
parameter_list|()
function_decl|;
end_function_decl

begin_block
block|{
specifier|register
name|struct
name|rthash
modifier|*
name|rh
decl_stmt|;
specifier|register
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
specifier|register
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
name|struct
name|rthash
modifier|*
name|base
init|=
name|hosthash
decl_stmt|;
name|int
name|doinghost
init|=
literal|1
decl_stmt|;
name|again
label|:
for|for
control|(
name|rh
operator|=
name|base
init|;
name|rh
operator|<
operator|&
name|base
index|[
name|ROUTEHASHSIZ
index|]
condition|;
name|rh
operator|++
control|)
for|for
control|(
name|rt
operator|=
name|rh
operator|->
name|rt_forw
init|;
name|rt
operator|!=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|rh
condition|;
name|rt
operator|=
name|rt
operator|->
name|rt_forw
control|)
block|{
if|if
condition|(
operator|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_PASSIVE
operator|)
operator|||
name|rt
operator|->
name|rt_metric
operator|>
literal|0
condition|)
continue|continue;
if|if
condition|(
name|rt
operator|->
name|rt_ifp
operator|&&
operator|(
name|rt
operator|->
name|rt_ifp
operator|->
name|if_flags
operator|&
name|IFF_BROADCAST
operator|)
condition|)
name|dst
operator|=
operator|&
name|rt
operator|->
name|rt_ifp
operator|->
name|if_broadaddr
expr_stmt|;
else|else
name|dst
operator|=
operator|&
name|rt
operator|->
name|rt_router
expr_stmt|;
call|(
modifier|*
name|f
call|)
argument_list|(
name|dst
argument_list|,
name|rt
operator|->
name|rt_state
operator|&
name|RTS_DONTROUTE
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|doinghost
condition|)
block|{
name|base
operator|=
name|nethash
expr_stmt|;
name|doinghost
operator|=
literal|0
expr_stmt|;
goto|goto
name|again
goto|;
block|}
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|sendmsg
argument_list|(
argument|dst
argument_list|,
argument|dontroute
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|dontroute
decl_stmt|;
end_decl_stmt

begin_block
block|{
operator|(
operator|*
name|afswitch
index|[
name|dst
operator|->
name|sa_family
index|]
operator|.
name|af_output
operator|)
operator|(
name|s
operator|,
name|dst
operator|,
sizeof|sizeof
argument_list|(
expr|struct
name|rip
argument_list|)
operator|)
expr_stmt|;
block|}
end_block

begin_macro
name|supply
argument_list|(
argument|dst
argument_list|,
argument|dontroute
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|dontroute
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
name|struct
name|netinfo
modifier|*
name|n
init|=
name|msg
operator|->
name|rip_nets
decl_stmt|;
specifier|register
name|struct
name|rthash
modifier|*
name|rh
decl_stmt|;
name|struct
name|rthash
modifier|*
name|base
init|=
name|hosthash
decl_stmt|;
name|int
name|doinghost
init|=
literal|1
decl_stmt|,
name|size
decl_stmt|;
name|int
function_decl|(
modifier|*
name|output
function_decl|)
parameter_list|()
init|=
name|afswitch
index|[
name|dst
operator|->
name|sa_family
index|]
operator|.
name|af_output
function_decl|;
name|int
name|sto
init|=
name|dontroute
condition|?
name|snoroute
else|:
name|s
decl_stmt|;
name|again
label|:
for|for
control|(
name|rh
operator|=
name|base
init|;
name|rh
operator|<
operator|&
name|base
index|[
name|ROUTEHASHSIZ
index|]
condition|;
name|rh
operator|++
control|)
for|for
control|(
name|rt
operator|=
name|rh
operator|->
name|rt_forw
init|;
name|rt
operator|!=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|rh
condition|;
name|rt
operator|=
name|rt
operator|->
name|rt_forw
control|)
block|{
if|if
condition|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_HIDDEN
condition|)
continue|continue;
name|size
operator|=
operator|(
name|char
operator|*
operator|)
name|n
operator|-
name|packet
expr_stmt|;
if|if
condition|(
name|size
operator|>
name|MAXPACKETSIZE
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|netinfo
argument_list|)
condition|)
block|{
call|(
modifier|*
name|output
call|)
argument_list|(
name|sto
argument_list|,
name|dst
argument_list|,
name|size
argument_list|)
expr_stmt|;
name|n
operator|=
name|msg
operator|->
name|rip_nets
expr_stmt|;
block|}
name|n
operator|->
name|rip_dst
operator|=
name|rt
operator|->
name|rt_dst
expr_stmt|;
name|n
operator|->
name|rip_metric
operator|=
name|min
argument_list|(
name|rt
operator|->
name|rt_metric
operator|+
literal|1
argument_list|,
name|HOPCNT_INFINITY
argument_list|)
expr_stmt|;
name|n
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|doinghost
condition|)
block|{
name|doinghost
operator|=
literal|0
expr_stmt|;
name|base
operator|=
name|nethash
expr_stmt|;
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|n
operator|!=
name|msg
operator|->
name|rip_nets
condition|)
call|(
modifier|*
name|output
call|)
argument_list|(
name|sto
argument_list|,
name|dst
argument_list|,
operator|(
name|char
operator|*
operator|)
name|n
operator|-
name|packet
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Handle an incoming routing packet.  */
end_comment

begin_macro
name|rip_input
argument_list|(
argument|from
argument_list|,
argument|size
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|from
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|size
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
name|struct
name|netinfo
modifier|*
name|n
decl_stmt|;
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
name|time_t
name|t
decl_stmt|;
name|int
name|newsize
decl_stmt|;
name|struct
name|afswitch
modifier|*
name|afp
decl_stmt|;
if|if
condition|(
name|trace
condition|)
block|{
if|if
condition|(
name|msg
operator|->
name|rip_cmd
operator|<
name|RIPCMD_MAX
condition|)
name|printf
argument_list|(
literal|"%s from %x\n"
argument_list|,
name|ripcmds
index|[
name|msg
operator|->
name|rip_cmd
index|]
argument_list|,
operator|(
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|from
operator|)
operator|->
name|sin_addr
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"%x from %x\n"
argument_list|,
name|msg
operator|->
name|rip_cmd
argument_list|,
operator|(
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|from
operator|)
operator|->
name|sin_addr
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|from
operator|->
name|sa_family
operator|>=
name|AF_MAX
condition|)
return|return;
name|afp
operator|=
operator|&
name|afswitch
index|[
name|from
operator|->
name|sa_family
index|]
expr_stmt|;
switch|switch
condition|(
name|msg
operator|->
name|rip_cmd
condition|)
block|{
case|case
name|RIPCMD_REQUEST
case|:
name|newsize
operator|=
literal|0
expr_stmt|;
name|size
operator|-=
literal|4
operator|*
sizeof|sizeof
argument_list|(
name|char
argument_list|)
expr_stmt|;
name|n
operator|=
name|msg
operator|->
name|rip_nets
expr_stmt|;
while|while
condition|(
name|size
operator|>
literal|0
condition|)
block|{
if|if
condition|(
name|size
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|netinfo
argument_list|)
condition|)
break|break;
name|size
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|netinfo
argument_list|)
expr_stmt|;
comment|/*  			 * A single entry with sa_family == AF_UNSPEC and 			 * metric ``infinity'' means ``all routes''. 			 */
if|if
condition|(
name|n
operator|->
name|rip_dst
operator|.
name|sa_family
operator|==
name|AF_UNSPEC
operator|&&
name|n
operator|->
name|rip_metric
operator|==
name|HOPCNT_INFINITY
operator|&&
name|size
operator|==
literal|0
condition|)
block|{
name|supply
argument_list|(
name|from
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* don't route */
return|return;
block|}
name|rt
operator|=
name|rtlookup
argument_list|(
operator|&
name|n
operator|->
name|rip_dst
argument_list|)
expr_stmt|;
name|n
operator|->
name|rip_metric
operator|=
name|rt
operator|==
literal|0
condition|?
name|HOPCNT_INFINITY
else|:
name|min
argument_list|(
name|rt
operator|->
name|rt_metric
operator|+
literal|1
argument_list|,
name|HOPCNT_INFINITY
argument_list|)
expr_stmt|;
name|n
operator|++
operator|,
name|newsize
operator|+=
sizeof|sizeof
argument_list|(
expr|struct
name|netinfo
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|newsize
operator|>
literal|0
condition|)
block|{
name|msg
operator|->
name|rip_cmd
operator|=
name|RIPCMD_RESPONSE
expr_stmt|;
name|newsize
operator|+=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
expr_stmt|;
call|(
modifier|*
name|afp
operator|->
name|af_output
call|)
argument_list|(
name|s
argument_list|,
name|from
argument_list|,
name|newsize
argument_list|)
expr_stmt|;
block|}
return|return;
case|case
name|RIPCMD_TRACEON
case|:
if|if
condition|(
call|(
modifier|*
name|afp
operator|->
name|af_portcheck
call|)
argument_list|(
name|from
argument_list|)
operator|==
literal|0
condition|)
return|return;
if|if
condition|(
name|trace
condition|)
return|return;
name|packet
index|[
name|size
index|]
operator|=
literal|'\0'
expr_stmt|;
name|ftrace
operator|=
name|fopen
argument_list|(
name|msg
operator|->
name|rip_tracefile
argument_list|,
literal|"a"
argument_list|)
expr_stmt|;
if|if
condition|(
name|ftrace
operator|==
name|NULL
condition|)
return|return;
operator|(
name|void
operator|)
name|dup2
argument_list|(
name|fileno
argument_list|(
name|ftrace
argument_list|)
argument_list|,
literal|1
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|dup2
argument_list|(
name|fileno
argument_list|(
name|ftrace
argument_list|)
argument_list|,
literal|2
argument_list|)
expr_stmt|;
name|trace
operator|=
literal|1
expr_stmt|;
name|t
operator|=
name|time
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"*** Tracing turned on at %.24s ***\n"
argument_list|,
name|ctime
argument_list|(
operator|&
name|t
argument_list|)
argument_list|)
expr_stmt|;
return|return;
case|case
name|RIPCMD_TRACEOFF
case|:
comment|/* verify message came from a priviledged port */
if|if
condition|(
call|(
modifier|*
name|afp
operator|->
name|af_portcheck
call|)
argument_list|(
name|from
argument_list|)
operator|==
literal|0
condition|)
return|return;
if|if
condition|(
operator|!
name|trace
condition|)
return|return;
name|t
operator|=
name|time
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"*** Tracing turned off at %.24s ***\n"
argument_list|,
name|ctime
argument_list|(
operator|&
name|t
argument_list|)
argument_list|)
expr_stmt|;
name|fflush
argument_list|(
name|stdout
argument_list|)
operator|,
name|fflush
argument_list|(
name|stderr
argument_list|)
expr_stmt|;
if|if
condition|(
name|ftrace
condition|)
name|fclose
argument_list|(
name|ftrace
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
literal|1
argument_list|)
operator|,
operator|(
name|void
operator|)
name|close
argument_list|(
literal|2
argument_list|)
expr_stmt|;
name|trace
operator|=
literal|0
expr_stmt|;
return|return;
case|case
name|RIPCMD_RESPONSE
case|:
comment|/* verify message came from a router */
if|if
condition|(
call|(
modifier|*
name|afp
operator|->
name|af_portmatch
call|)
argument_list|(
name|from
argument_list|)
operator|==
literal|0
condition|)
return|return;
call|(
modifier|*
name|afp
operator|->
name|af_canon
call|)
argument_list|(
name|from
argument_list|)
expr_stmt|;
comment|/* are we talking to ourselves? */
name|ifp
operator|=
name|if_ifwithaddr
argument_list|(
name|from
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifp
condition|)
block|{
name|rt
operator|=
name|rtfind
argument_list|(
name|from
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|==
literal|0
condition|)
block|{
name|addrouteforif
argument_list|(
name|ifp
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* interface previously thought down? */
if|if
condition|(
name|rt
operator|->
name|rt_metric
operator|>
literal|0
condition|)
block|{
name|struct
name|rt_entry
modifier|*
name|downrt
init|=
name|rtfind
argument_list|(
name|from
argument_list|,
literal|1
argument_list|)
decl_stmt|;
if|if
condition|(
name|downrt
condition|)
block|{
comment|/* unhide old route and delete other */
name|downrt
operator|->
name|rt_state
operator|&=
operator|~
name|RTS_HIDDEN
expr_stmt|;
name|downrt
operator|->
name|rt_state
operator||=
name|RTS_ADDRT
expr_stmt|;
name|rt
operator|->
name|rt_state
operator||=
name|RTS_DELRT
expr_stmt|;
name|log
argument_list|(
literal|"unhide"
argument_list|,
name|downrt
argument_list|)
expr_stmt|;
block|}
block|}
name|rt
operator|->
name|rt_timer
operator|=
literal|0
expr_stmt|;
return|return;
block|}
name|size
operator|-=
literal|4
operator|*
sizeof|sizeof
argument_list|(
name|char
argument_list|)
expr_stmt|;
name|n
operator|=
name|msg
operator|->
name|rip_nets
expr_stmt|;
for|for
control|(
init|;
name|size
operator|>
literal|0
condition|;
name|size
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|netinfo
argument_list|)
operator|,
name|n
operator|++
control|)
block|{
if|if
condition|(
name|size
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|netinfo
argument_list|)
condition|)
break|break;
if|if
condition|(
name|n
operator|->
name|rip_metric
operator|>=
name|HOPCNT_INFINITY
condition|)
continue|continue;
name|tprintf
argument_list|(
literal|"dst %x hc %d..."
argument_list|,
operator|(
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|n
operator|->
name|rip_dst
operator|)
operator|->
name|sin_addr
argument_list|,
name|n
operator|->
name|rip_metric
argument_list|)
expr_stmt|;
name|rt
operator|=
name|rtlookup
argument_list|(
operator|&
name|n
operator|->
name|rip_dst
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|==
literal|0
condition|)
block|{
name|rtadd
argument_list|(
operator|&
name|n
operator|->
name|rip_dst
argument_list|,
name|from
argument_list|,
name|n
operator|->
name|rip_metric
argument_list|,
literal|0
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|tprintf
argument_list|(
literal|"ours: gate %x hc %d timer %d\n"
argument_list|,
operator|(
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|rt
operator|->
name|rt_router
operator|)
operator|->
name|sin_addr
argument_list|,
name|rt
operator|->
name|rt_metric
argument_list|,
name|rt
operator|->
name|rt_timer
argument_list|)
expr_stmt|;
comment|/* 			 * update if from gateway, shorter, or getting 			 * stale and equivalent. 			 */
if|if
condition|(
name|equal
argument_list|(
name|from
argument_list|,
operator|&
name|rt
operator|->
name|rt_router
argument_list|)
operator|||
name|n
operator|->
name|rip_metric
operator|<
name|rt
operator|->
name|rt_metric
operator|||
operator|(
name|rt
operator|->
name|rt_timer
operator|>
operator|(
name|EXPIRE_TIME
operator|/
literal|2
operator|)
operator|&&
name|rt
operator|->
name|rt_metric
operator|==
name|n
operator|->
name|rip_metric
operator|)
condition|)
block|{
name|rtchange
argument_list|(
name|rt
argument_list|,
name|from
argument_list|,
name|n
operator|->
name|rip_metric
argument_list|)
expr_stmt|;
name|rt
operator|->
name|rt_timer
operator|=
literal|0
expr_stmt|;
block|}
block|}
return|return;
block|}
name|printf
argument_list|(
literal|"bad packet, cmd=%x\n"
argument_list|,
name|msg
operator|->
name|rip_cmd
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
name|struct
name|rt_entry
modifier|*
name|rtlookup
parameter_list|(
name|dst
parameter_list|)
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
block|{
specifier|register
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
specifier|register
name|struct
name|rthash
modifier|*
name|rh
decl_stmt|;
specifier|register
name|int
name|hash
decl_stmt|;
name|struct
name|afhash
name|h
decl_stmt|;
name|int
name|doinghost
init|=
literal|1
decl_stmt|;
if|if
condition|(
name|dst
operator|->
name|sa_family
operator|>=
name|AF_MAX
condition|)
return|return
operator|(
literal|0
operator|)
return|;
operator|(
operator|*
name|afswitch
index|[
name|dst
operator|->
name|sa_family
index|]
operator|.
name|af_hash
operator|)
operator|(
name|dst
operator|,
operator|&
name|h
operator|)
expr_stmt|;
name|hash
operator|=
name|h
operator|.
name|afh_hosthash
expr_stmt|;
name|rh
operator|=
operator|&
name|hosthash
index|[
name|hash
operator|%
name|ROUTEHASHSIZ
index|]
expr_stmt|;
name|again
label|:
for|for
control|(
name|rt
operator|=
name|rh
operator|->
name|rt_forw
init|;
name|rt
operator|!=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|rh
condition|;
name|rt
operator|=
name|rt
operator|->
name|rt_forw
control|)
block|{
if|if
condition|(
name|rt
operator|->
name|rt_hash
operator|!=
name|hash
operator|||
operator|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_HIDDEN
operator|)
condition|)
continue|continue;
if|if
condition|(
name|equal
argument_list|(
operator|&
name|rt
operator|->
name|rt_dst
argument_list|,
name|dst
argument_list|)
condition|)
return|return
operator|(
name|rt
operator|)
return|;
block|}
if|if
condition|(
name|doinghost
condition|)
block|{
name|doinghost
operator|=
literal|0
expr_stmt|;
name|hash
operator|=
name|h
operator|.
name|afh_nethash
expr_stmt|;
name|rh
operator|=
operator|&
name|nethash
index|[
name|hash
operator|%
name|ROUTEHASHSIZ
index|]
expr_stmt|;
goto|goto
name|again
goto|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_function
name|struct
name|rt_entry
modifier|*
name|rtfind
parameter_list|(
name|dst
parameter_list|,
name|lookfordownif
parameter_list|)
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
name|int
name|lookfordownif
decl_stmt|;
block|{
specifier|register
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
specifier|register
name|struct
name|rthash
modifier|*
name|rh
decl_stmt|;
specifier|register
name|int
name|hash
decl_stmt|;
name|struct
name|afhash
name|h
decl_stmt|;
name|int
name|af
init|=
name|dst
operator|->
name|sa_family
decl_stmt|;
name|int
name|doinghost
init|=
literal|1
decl_stmt|,
argument_list|(
operator|*
name|match
argument_list|)
argument_list|()
decl_stmt|;
if|if
condition|(
name|af
operator|>=
name|AF_MAX
condition|)
return|return
operator|(
literal|0
operator|)
return|;
operator|(
operator|*
name|afswitch
index|[
name|af
index|]
operator|.
name|af_hash
operator|)
operator|(
name|dst
operator|,
operator|&
name|h
operator|)
expr_stmt|;
name|hash
operator|=
name|h
operator|.
name|afh_hosthash
expr_stmt|;
name|rh
operator|=
operator|&
name|hosthash
index|[
name|hash
operator|%
name|ROUTEHASHSIZ
index|]
expr_stmt|;
name|again
label|:
for|for
control|(
name|rt
operator|=
name|rh
operator|->
name|rt_forw
init|;
name|rt
operator|!=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|rh
condition|;
name|rt
operator|=
name|rt
operator|->
name|rt_forw
control|)
block|{
if|if
condition|(
name|rt
operator|->
name|rt_hash
operator|!=
name|hash
condition|)
continue|continue;
if|if
condition|(
name|lookfordownif
operator|^
operator|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_HIDDEN
operator|)
condition|)
continue|continue;
if|if
condition|(
name|doinghost
condition|)
block|{
if|if
condition|(
name|equal
argument_list|(
operator|&
name|rt
operator|->
name|rt_dst
argument_list|,
name|dst
argument_list|)
condition|)
return|return
operator|(
name|rt
operator|)
return|;
block|}
else|else
block|{
if|if
condition|(
name|rt
operator|->
name|rt_dst
operator|.
name|sa_family
operator|==
name|af
operator|&&
call|(
modifier|*
name|match
call|)
argument_list|(
operator|&
name|rt
operator|->
name|rt_dst
argument_list|,
name|dst
argument_list|)
condition|)
return|return
operator|(
name|rt
operator|)
return|;
block|}
block|}
if|if
condition|(
name|doinghost
condition|)
block|{
name|doinghost
operator|=
literal|0
expr_stmt|;
name|hash
operator|=
name|h
operator|.
name|afh_nethash
expr_stmt|;
name|rh
operator|=
operator|&
name|nethash
index|[
name|hash
operator|%
name|ROUTEHASHSIZ
index|]
expr_stmt|;
name|match
operator|=
name|afswitch
index|[
name|af
index|]
operator|.
name|af_netmatch
expr_stmt|;
goto|goto
name|again
goto|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_macro
name|rtadd
argument_list|(
argument|dst
argument_list|,
argument|gate
argument_list|,
argument|metric
argument_list|,
argument|state
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|,
modifier|*
name|gate
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|metric
decl_stmt|,
name|state
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|afhash
name|h
decl_stmt|;
specifier|register
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
name|struct
name|rthash
modifier|*
name|rh
decl_stmt|;
name|int
name|af
init|=
name|dst
operator|->
name|sa_family
decl_stmt|,
name|flags
decl_stmt|,
name|hash
decl_stmt|;
if|if
condition|(
name|af
operator|>=
name|AF_MAX
condition|)
return|return;
operator|(
operator|*
name|afswitch
index|[
name|af
index|]
operator|.
name|af_hash
operator|)
operator|(
name|dst
operator|,
operator|&
name|h
operator|)
expr_stmt|;
name|flags
operator|=
operator|(
operator|*
name|afswitch
index|[
name|af
index|]
operator|.
name|af_checkhost
operator|)
operator|(
name|dst
operator|)
condition|?
name|RTF_HOST
else|:
literal|0
expr_stmt|;
if|if
condition|(
name|flags
operator|&
name|RTF_HOST
condition|)
block|{
name|hash
operator|=
name|h
operator|.
name|afh_hosthash
expr_stmt|;
name|rh
operator|=
operator|&
name|hosthash
index|[
name|hash
operator|%
name|ROUTEHASHSIZ
index|]
expr_stmt|;
block|}
else|else
block|{
name|hash
operator|=
name|h
operator|.
name|afh_nethash
expr_stmt|;
name|rh
operator|=
operator|&
name|nethash
index|[
name|hash
operator|%
name|ROUTEHASHSIZ
index|]
expr_stmt|;
block|}
name|rt
operator|=
operator|(
expr|struct
name|rt_entry
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|rt
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|==
literal|0
condition|)
return|return;
name|rt
operator|->
name|rt_hash
operator|=
name|hash
expr_stmt|;
name|rt
operator|->
name|rt_dst
operator|=
operator|*
name|dst
expr_stmt|;
name|rt
operator|->
name|rt_router
operator|=
operator|*
name|gate
expr_stmt|;
name|rt
operator|->
name|rt_metric
operator|=
name|metric
expr_stmt|;
name|rt
operator|->
name|rt_timer
operator|=
literal|0
expr_stmt|;
name|rt
operator|->
name|rt_flags
operator|=
name|RTF_UP
operator||
name|flags
expr_stmt|;
name|rt
operator|->
name|rt_state
operator|=
name|state
expr_stmt|;
name|rt
operator|->
name|rt_ifp
operator|=
name|if_ifwithnet
argument_list|(
operator|&
name|rt
operator|->
name|rt_router
argument_list|)
expr_stmt|;
if|if
condition|(
name|metric
condition|)
name|rt
operator|->
name|rt_flags
operator||=
name|RTF_GATEWAY
expr_stmt|;
name|insque
argument_list|(
name|rt
argument_list|,
name|rh
argument_list|)
expr_stmt|;
name|log
argument_list|(
literal|"add"
argument_list|,
name|rt
argument_list|)
expr_stmt|;
if|if
condition|(
name|install
condition|)
name|rt
operator|->
name|rt_state
operator||=
name|RTS_ADDRT
expr_stmt|;
block|}
end_block

begin_macro
name|rtchange
argument_list|(
argument|rt
argument_list|,
argument|gate
argument_list|,
argument|metric
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|gate
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|short
name|metric
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|doioctl
init|=
literal|0
decl_stmt|,
name|metricchanged
init|=
literal|0
decl_stmt|;
if|if
condition|(
operator|!
name|equal
argument_list|(
operator|&
name|rt
operator|->
name|rt_router
argument_list|,
name|gate
argument_list|)
condition|)
block|{
if|if
condition|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_DONTDELETE
condition|)
block|{
name|rtadd
argument_list|(
operator|&
name|rt
operator|->
name|rt_dst
argument_list|,
name|gate
argument_list|,
name|metric
argument_list|,
name|rt
operator|->
name|rt_state
operator|&
operator|~
operator|(
name|RTS_DONTDELETE
operator||
name|RTS_DONTROUTE
operator|)
argument_list|)
expr_stmt|;
name|rt
operator|->
name|rt_state
operator||=
name|RTS_DELRT
expr_stmt|;
return|return;
block|}
name|doioctl
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|metric
operator|!=
name|rt
operator|->
name|rt_metric
condition|)
block|{
name|metricchanged
operator|++
expr_stmt|;
name|rt
operator|->
name|rt_metric
operator|=
name|metric
expr_stmt|;
block|}
if|if
condition|(
name|doioctl
condition|)
block|{
if|if
condition|(
operator|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_CHGRT
operator|)
operator|==
literal|0
condition|)
name|rt
operator|->
name|rt_oldrouter
operator|=
name|rt
operator|->
name|rt_router
expr_stmt|;
name|rt
operator|->
name|rt_router
operator|=
operator|*
name|gate
expr_stmt|;
name|rt
operator|->
name|rt_state
operator||=
name|RTS_CHGRT
expr_stmt|;
block|}
if|if
condition|(
name|doioctl
operator|||
name|metricchanged
condition|)
name|log
argument_list|(
literal|"change"
argument_list|,
name|rt
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|rtdelete
argument_list|(
argument|rt
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|log
argument_list|(
literal|"delete"
argument_list|,
name|rt
argument_list|)
expr_stmt|;
if|if
condition|(
name|install
operator|&&
name|ioctl
argument_list|(
name|s
argument_list|,
name|SIOCDELRT
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|rt
operator|->
name|rt_rt
argument_list|)
condition|)
name|perror
argument_list|(
literal|"SIOCDELRT"
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_DONTDELETE
condition|)
block|{
name|rt
operator|->
name|rt_state
operator||=
name|RTS_HIDDEN
expr_stmt|;
return|return;
block|}
name|remque
argument_list|(
name|rt
argument_list|)
expr_stmt|;
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
name|rt
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|log
argument_list|(
argument|operation
argument_list|,
argument|rt
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|operation
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|rt_entry
modifier|*
name|rt
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|sockaddr_in
modifier|*
name|dst
decl_stmt|,
modifier|*
name|gate
decl_stmt|;
specifier|static
struct|struct
name|bits
block|{
name|int
name|t_bits
decl_stmt|;
name|char
modifier|*
name|t_name
decl_stmt|;
block|}
name|flagbits
index|[]
init|=
block|{
block|{
name|RTF_UP
block|,
literal|"UP"
block|}
block|,
block|{
name|RTF_GATEWAY
block|,
literal|"GATEWAY"
block|}
block|,
block|{
name|RTF_HOST
block|,
literal|"HOST"
block|}
block|,
block|{
literal|0
block|}
block|}
struct|,
name|statebits
index|[]
init|=
block|{
block|{
name|RTS_ADDRT
block|,
literal|"ADD"
block|}
block|,
block|{
name|RTS_DELRT
block|,
literal|"DELETE"
block|}
block|,
block|{
name|RTS_CHGRT
block|,
literal|"CHANGE"
block|}
block|,
block|{
name|RTS_PASSIVE
block|,
literal|"PASSIVE"
block|}
block|,
block|{
name|RTS_DONTDELETE
block|,
literal|"DONTDELETE"
block|}
block|,
block|{
name|RTS_DONTROUTE
block|,
literal|"DONTROUTE"
block|}
block|,
block|{
name|RTS_HIDDEN
block|,
literal|"HIDDEN"
block|}
block|,
block|{
literal|0
block|}
block|}
struct|;
specifier|register
name|struct
name|bits
modifier|*
name|p
decl_stmt|;
specifier|register
name|int
name|first
decl_stmt|;
name|char
modifier|*
name|cp
decl_stmt|;
if|if
condition|(
name|trace
operator|==
literal|0
condition|)
return|return;
name|printf
argument_list|(
literal|"%s "
argument_list|,
name|operation
argument_list|)
expr_stmt|;
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|rt
operator|->
name|rt_dst
expr_stmt|;
name|gate
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|rt
operator|->
name|rt_router
expr_stmt|;
name|printf
argument_list|(
literal|"dst %x, router %x"
argument_list|,
name|dst
operator|->
name|sin_addr
argument_list|,
name|gate
operator|->
name|sin_addr
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|->
name|rt_state
operator|&
name|RTS_CHGRT
condition|)
name|printf
argument_list|(
literal|" (old %x)"
argument_list|,
operator|(
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|rt
operator|->
name|rt_oldrouter
operator|)
operator|->
name|sin_addr
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|", metric %d, flags"
argument_list|,
name|rt
operator|->
name|rt_metric
argument_list|)
expr_stmt|;
name|cp
operator|=
literal|" %s"
expr_stmt|;
for|for
control|(
name|first
operator|=
literal|1
operator|,
name|p
operator|=
name|flagbits
init|;
name|p
operator|->
name|t_bits
operator|>
literal|0
condition|;
name|p
operator|++
control|)
block|{
if|if
condition|(
operator|(
name|rt
operator|->
name|rt_flags
operator|&
name|p
operator|->
name|t_bits
operator|)
operator|==
literal|0
condition|)
continue|continue;
name|printf
argument_list|(
name|cp
argument_list|,
name|p
operator|->
name|t_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|first
condition|)
block|{
name|cp
operator|=
literal|"|%s"
expr_stmt|;
name|first
operator|=
literal|0
expr_stmt|;
block|}
block|}
name|printf
argument_list|(
literal|" state"
argument_list|)
expr_stmt|;
name|cp
operator|=
literal|" %s"
expr_stmt|;
for|for
control|(
name|first
operator|=
literal|1
operator|,
name|p
operator|=
name|statebits
init|;
name|p
operator|->
name|t_bits
operator|>
literal|0
condition|;
name|p
operator|++
control|)
block|{
if|if
condition|(
operator|(
name|rt
operator|->
name|rt_state
operator|&
name|p
operator|->
name|t_bits
operator|)
operator|==
literal|0
condition|)
continue|continue;
name|printf
argument_list|(
name|cp
argument_list|,
name|p
operator|->
name|t_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|first
condition|)
block|{
name|cp
operator|=
literal|"|%s"
expr_stmt|;
name|first
operator|=
literal|0
expr_stmt|;
block|}
block|}
name|putchar
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
name|struct
name|ifnet
modifier|*
name|if_ifwithaddr
parameter_list|(
name|addr
parameter_list|)
name|struct
name|sockaddr
modifier|*
name|addr
decl_stmt|;
block|{
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
define|#
directive|define
name|same
parameter_list|(
name|a1
parameter_list|,
name|a2
parameter_list|)
define|\
value|(bcmp((caddr_t)((a1)->sa_data), (caddr_t)((a2)->sa_data), 14) == 0)
for|for
control|(
name|ifp
operator|=
name|ifnet
init|;
name|ifp
condition|;
name|ifp
operator|=
name|ifp
operator|->
name|if_next
control|)
block|{
if|if
condition|(
name|ifp
operator|->
name|if_addr
operator|.
name|sa_family
operator|!=
name|addr
operator|->
name|sa_family
condition|)
continue|continue;
if|if
condition|(
name|same
argument_list|(
operator|&
name|ifp
operator|->
name|if_addr
argument_list|,
name|addr
argument_list|)
condition|)
break|break;
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_BROADCAST
operator|)
operator|&&
name|same
argument_list|(
operator|&
name|ifp
operator|->
name|if_broadaddr
argument_list|,
name|addr
argument_list|)
condition|)
break|break;
block|}
return|return
operator|(
name|ifp
operator|)
return|;
undef|#
directive|undef
name|same
block|}
end_function

begin_function
name|struct
name|ifnet
modifier|*
name|if_ifwithnet
parameter_list|(
name|addr
parameter_list|)
specifier|register
name|struct
name|sockaddr
modifier|*
name|addr
decl_stmt|;
block|{
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
specifier|register
name|int
name|af
init|=
name|addr
operator|->
name|sa_family
decl_stmt|;
specifier|register
name|int
function_decl|(
modifier|*
name|netmatch
function_decl|)
parameter_list|()
function_decl|;
if|if
condition|(
name|af
operator|>=
name|AF_MAX
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|netmatch
operator|=
name|afswitch
index|[
name|af
index|]
operator|.
name|af_netmatch
expr_stmt|;
for|for
control|(
name|ifp
operator|=
name|ifnet
init|;
name|ifp
condition|;
name|ifp
operator|=
name|ifp
operator|->
name|if_next
control|)
block|{
if|if
condition|(
name|af
operator|!=
name|ifp
operator|->
name|if_addr
operator|.
name|sa_family
condition|)
continue|continue;
if|if
condition|(
call|(
modifier|*
name|netmatch
call|)
argument_list|(
name|addr
argument_list|,
operator|&
name|ifp
operator|->
name|if_addr
argument_list|)
condition|)
break|break;
block|}
return|return
operator|(
name|ifp
operator|)
return|;
block|}
end_function

begin_function
name|struct
name|in_addr
name|if_makeaddr
parameter_list|(
name|net
parameter_list|,
name|host
parameter_list|)
name|int
name|net
decl_stmt|,
name|host
decl_stmt|;
block|{
name|u_long
name|addr
decl_stmt|;
if|if
condition|(
name|net
operator|<
literal|128
condition|)
name|addr
operator|=
operator|(
name|net
operator|<<
literal|24
operator|)
operator||
name|host
expr_stmt|;
elseif|else
if|if
condition|(
name|net
operator|<
literal|65536
condition|)
name|addr
operator|=
operator|(
name|net
operator|<<
literal|16
operator|)
operator||
name|host
expr_stmt|;
else|else
name|addr
operator|=
operator|(
name|net
operator|<<
literal|8
operator|)
operator||
name|host
expr_stmt|;
ifdef|#
directive|ifdef
name|vax
name|addr
operator|=
name|htonl
argument_list|(
name|addr
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
operator|(
operator|*
operator|(
expr|struct
name|in_addr
operator|*
operator|)
operator|&
name|addr
operator|)
return|;
block|}
end_function

end_unit

