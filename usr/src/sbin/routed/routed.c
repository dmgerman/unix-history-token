...
begin_comment
comment|/*  * Timer routine:  *  * o handle timers on table entries,  * o invalidate entries which haven't been updated in a while,  * o delete entries which are too old,  * o if we're an internetwork router, supply routing updates  *   periodically  */
end_comment
...
expr_stmt|;
comment|/* 			 * If a change or addition is to be made 			 * and this isn't time to broadcast an 			 * update, then broadcast the change. 			 */
if|if
condition|(
operator|(
name|rt
...
begin_comment
comment|/*  * Look in a file for any gateways we should configure  * outside the directly connected ones.  This is a kludge,  * but until we can find out about gateways on the "other side"  * of the ARPANET using GGP, it's a must.  *  * File format is one entry per line,   *	destination gateway flags	(all hex #'s)  *  * We don't really know the distance to the gateway, so we  * assume it's a neighbor.  */
end_comment
