...
expr_stmt|;
comment|/* Fall through and check whether a subnet */
block|}
else|else
return|return
operator|(
...
return|;
block|}
comment|/* 	 * Check whether network is a subnet of a `local' network; 	 * if so, return subnet number. 	 */
for|for
control|(
name|ifp
operator|=
...
return|;
block|}
comment|/* 	 * Check whether network is a subnet of a `local' network; 	 * if so, use the modified interpretation of `host'. 	 */
for|for
control|(
name|ifp
operator|=
