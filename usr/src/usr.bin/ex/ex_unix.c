...
operator|.
name|h
literal|"  /*  * Unix escapes, filtering  */  /*  * First part of a shell escape,  * parse the line, expanding # and % and ! and printing if implied.  */ unix0(warn) 	bool warn; { 	register char *up, *fp; 	register short c; 	char printub, puxb[UXBSIZE + sizeof (int)];  	printub = 0; 	CP(puxb, uxb); 	c = getchar(); 	if (c == '\n' || c == EOF) 		error("
name|Incomplete
name|shell
name|escape
name|command
...
name|a
name|shell
literal|"); 	up = uxb; 	do { 		switch (c) {  		case '\\': 			if (any(peekchar(), "
operator|%
operator|#
operator|!
literal|")) 				c = getchar(); 		default: 			if (up>=&uxb[UXBSIZE]) { tunix: 				uxb[0] = 0; 				error("
name|Command
name|too
name|long
literal|"); 			} 			*up++ = c; 			break;  		case '!': 			fp = puxb; 			if (*fp == 0) { 				uxb[0] = 0; 				error("
name|No
name|previous
name|command
expr|@
