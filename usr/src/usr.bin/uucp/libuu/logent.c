...
index|[]
init|=
literal|"@(#)logent.c	5.3 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* This logfile stuff was awful -- it did output to an  * unbuffered stream.  *  * This new version just open the single logfile and writes  * the record in the stdio buffer.  Once that's done, it  * positions itself at the end of the file (lseek), and  * writes the buffer out.  This could mangle things but  * it isn't likely. -- ittvax!swatt  *  * Under USG UNIX& 4.2BSD, the files are opened with "guaranteed append to end"  * and the lseeks are removed.  */
end_comment
