...
index|[]
init|=
literal|"@(#)condevs.c	5.7 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/***  *	tcpopn - establish a network connection to the remote system  *  *	return codes:  *>= 0	file descriptor for remote connection  *		FAIL	unable to establish remote connection  */
end_comment
...
literal|4
argument_list|,
literal|"netcall: Unknown host (%s)\n"
argument_list|,
name|flds
index|[
name|F_NAME
...
literal|4
argument_list|,
literal|"failed, no local sockets (%d)\n"
argument_list|,
name|errno
argument_list|)
expr_stmt|;
...
literal|4
argument_list|,
literal|"failed, connect error (%d)\n"
argument_list|,
name|errno
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * tcpcls -- close TCP connection.  */
end_comment
...
condition|)
block|{
comment|/* disable this until a timeout is put in 		if (ioctl(fd, UIOCCLOSE, STBNULL)) 			logent("TCP CLOSE", "FAILED"); 		 */
name|close
argument_list|(
name|fd
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* add a delay at the end of the number for the local phone switch */
name|sprintf
argument_list|(
name|ntelno
argument_list|,
