...
begin_comment
comment|/*  * Buffer management for tar.  *  * Written by John Gilmore, ihnp4!hoptoad!gnu, on 25 August 1985.  *  * @(#) buffer.c 1.28 11/6/87 - gnu  */
end_comment
...
name|stderr
argument_list|,
literal|"MSDOS %s can't use compressed or remote archives\n"
argument_list|,
name|tar
argument_list|)
expr_stmt|;
...
block|}
elseif|else
comment|/* This can't happen if (ar_reading==2) 				archive = rmtopen(ar_file, O_RDWR|O_CREAT|O_BINARY, 0666); 			else */
if|if
condition|(
name|ar_reading
condition|)
...
name|msg_perror
argument_list|(
literal|"can't write to compress"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
name|msg
argument_list|(
literal|"write to compress short %d bytes"
argument_list|,
name|count
operator|-
name|err
...
name|msg_perror
argument_list|(
literal|"can't read from compress"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
directive|if
literal|0
block|if(strcmp(ptr,head->header.name)) { 					msg("Volume mismatch! %s!=%s",ptr,head->header.name); 					--volno; 					free(ptr); 					goto try_volume; 				} 				free(ptr);
endif|#
directive|endif
block|}
if|if
...
directive|ifndef
name|MSDOS
comment|/* FIXME does ftruncate really take an INT?! */
operator|(
name|void
operator|)
name|ftruncate
...
name|msg_file
argument_list|,
literal|"\007Prepare volume #%d and hit return: "
argument_list|,
name|volno
argument_list|)
expr_stmt|;
