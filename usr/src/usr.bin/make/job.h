...
begin_comment
comment|/*-  * job.h --  *	Definitions pertaining to the running of jobs in parallel mode.  *	Exported from job.c for the use of remote-execution modules.  *  * Copyright (c) 1988, 1989 by the Regents of the University of California  * Copyright (c) 1988, 1989 by Adam de Boor  * Copyright (c) 1989 by Berkeley Softworks  *  * Permission to use, copy, modify, and distribute this  * software and its documentation for any non-commercial purpose  * and without fee is hereby granted, provided that the above copyright  * notice appears in all copies.  The University of California,  * Berkeley Softworks and Adam de Boor make no representations about  * the suitability of this software for any purpose.  It is provided  * "as is" without express or implied warranty.  *  *	"$Id: job.h,v 1.1 89/11/14 13:44:21 adam Exp $ SPRITE (Berkeley)"  */
end_comment
...
begin_comment
comment|/*  * These functions should be used only by an intelligent Rmt module, hence  * their names do *not* include an underscore as they are not fully exported,  * if you see what I mean.  */
end_comment
...
begin_comment
comment|/* Funnel output from 			     	    	    	 * job->outPipe to the screen, 						 * filtering out echo-off 						 * strings etc. */
end_comment
...
begin_comment
comment|/* Finish out a job. If 			    	    	    	 * status indicates job has 						 * just stopped, not finished, 						 * the descriptor is placed on 						 * the stoppedJobs list. */
end_comment
