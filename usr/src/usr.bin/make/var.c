...
name|v
decl_stmt|;
comment|/*      * If the variable name begins with a '.' and an upper-case letter,      * it could very well be one of the local ones. We check the name      * against all the local variables and substitute the short version in for      * 'name' if it matches one of them.      */
if|if
condition|(
operator|*
name|name
...
condition|)
block|{
comment|/* 	     * If the variable is found in the environment, we only duplicate 	     * its value (since eVarVal was allocated on the stack). The name 	     * doesn't need duplication since it's always in the environment 	     */
name|int
name|len
decl_stmt|;
name|v
...
name|cp
decl_stmt|;
comment|/* Pointer to end of current word */
name|char
name|endc
decl_stmt|;
comment|/* Character that ended the word */
name|Boolean
name|addSpace
decl_stmt|;
...
condition|)
block|{
comment|/* 	 * Skip to next word and place cp at its end. 	 */
while|while
condition|(
name|isspace
argument_list|(
...
condition|)
block|{
comment|/* 	     * If we didn't go anywhere, we must be done! 	     */
name|Buf_AddByte
argument_list|(
name|buf
argument_list|,
...
return|;
block|}
comment|/* 	 * Nuke terminating character, but save it in endc b/c if str was 	 * some variable's value, it would not be good to screw it 	 * over... 	 */
name|endc
operator|=
operator|*
name|cp
