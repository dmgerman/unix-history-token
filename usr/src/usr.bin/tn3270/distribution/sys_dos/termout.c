begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  *	Copyright (c) 1984, 1985, 1986 by the Regents of the  *	University of California and by Gregory Glenn Minshall.  *  *	Permission to use, copy, modify, and distribute these  *	programs and their documentation for any purpose and  *	without fee is hereby granted, provided that this  *	copyright and permission appear on all copies and  *	supporting documentation, the name of the Regents of  *	the University of California not be used in advertising  *	or publicity pertaining to distribution of the programs  *	without specific prior permission, and notice be given in  *	supporting documentation that copying and distribution is  *	by permission of the Regents of the University of California  *	and by Gregory Glenn Minshall.  Neither the Regents of the  *	University of California nor Gregory Glenn Minshall make  *	representations about the suitability of this software  *	for any purpose.  It is provided "as is" without  *	express or implied warranty.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)outbound.c	3.1  10/29/86"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* lint */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|"../general.h"
end_include

begin_include
include|#
directive|include
file|"../telnet.ext"
end_include

begin_include
include|#
directive|include
file|"../ctlr/hostctlr.h"
end_include

begin_include
include|#
directive|include
file|"../ctlr/inbound.ext"
end_include

begin_include
include|#
directive|include
file|"../ctlr/oia.h"
end_include

begin_include
include|#
directive|include
file|"../ctlr/options.ext"
end_include

begin_include
include|#
directive|include
file|"../ctlr/outbound.ext"
end_include

begin_include
include|#
directive|include
file|"../ctlr/screen.h"
end_include

begin_include
include|#
directive|include
file|"../keyboard/map3270.ext"
end_include

begin_include
include|#
directive|include
file|"../system/globals.h"
end_include

begin_function_decl
specifier|extern
name|void
name|EmptyTerminal
parameter_list|()
function_decl|;
end_function_decl

begin_define
define|#
directive|define
name|CorrectTerminalCursor
parameter_list|()
value|((TransparentClock == OutputClock)? \ 		terminalCursorAddress:UnLocked? CursorAddress: HighestScreen())
end_define

begin_decl_stmt
specifier|static
name|int
name|terminalCursorAddress
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* where the cursor is on term */
end_comment

begin_decl_stmt
specifier|static
name|int
name|screenInitd
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* the screen has been initialized */
end_comment

begin_decl_stmt
specifier|static
name|int
name|screenStopped
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* the screen has been stopped */
end_comment

begin_decl_stmt
specifier|static
name|int
name|needToRing
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* need to ring terinal bell */
end_comment

begin_typedef
typedef|typedef
struct|struct
block|{
name|char
name|data
decl_stmt|,
comment|/* The data for this position */
name|attr
decl_stmt|;
comment|/* The attributes for this position */
block|}
name|ScreenBuffer
typedef|;
end_typedef

begin_decl_stmt
name|ScreenBuffer
name|Screen
index|[
name|MAXNUMBERLINES
operator|*
name|MAXNUMBERCOLUMNS
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Variables for transparent mode */
end_comment

begin_include
include|#
directive|include
file|"disp_asc.out"
end_include

begin_escape
end_escape

begin_comment
comment|/* OurExitString - designed to keep us from going through infinite recursion */
end_comment

begin_function
specifier|static
name|void
name|OurExitString
parameter_list|(
name|file
parameter_list|,
name|string
parameter_list|,
name|value
parameter_list|)
name|FILE
modifier|*
name|file
decl_stmt|;
name|char
modifier|*
name|string
decl_stmt|;
name|int
name|value
decl_stmt|;
block|{
specifier|static
name|int
name|recursion
init|=
literal|0
decl_stmt|;
if|if
condition|(
operator|!
name|recursion
condition|)
block|{
name|recursion
operator|=
literal|1
expr_stmt|;
name|ExitString
argument_list|(
name|file
argument_list|,
name|string
argument_list|,
name|value
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|GoAway
parameter_list|(
name|from
parameter_list|,
name|where
parameter_list|)
name|char
modifier|*
name|from
decl_stmt|;
comment|/* routine that gave error */
name|int
name|where
decl_stmt|;
comment|/* cursor address */
block|{
name|char
name|foo
index|[
literal|100
index|]
decl_stmt|;
name|sprintf
argument_list|(
name|foo
argument_list|,
literal|"ERR from %s at %d (%d, %d)\n"
argument_list|,
name|from
argument_list|,
name|where
argument_list|,
name|ScreenLine
argument_list|(
name|where
argument_list|)
argument_list|,
name|ScreenLineOffset
argument_list|(
name|where
argument_list|)
argument_list|)
expr_stmt|;
name|OurExitString
argument_list|(
name|stderr
argument_list|,
name|foo
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|/* NOTREACHED */
block|}
end_function

begin_escape
end_escape

begin_function
specifier|static
name|void
name|TryToSend
parameter_list|()
block|{
define|#
directive|define
name|STANDOUT
value|0x0a
define|#
directive|define
name|NORMAL
value|0x02
comment|/* Normal mode */
define|#
directive|define
name|DoAttribute
parameter_list|(
name|a
parameter_list|)
value|if (IsHighlightedAttr(a)) { \ 				a = STANDOUT; \ 			    } else { \ 				a = NORMAL; \ 			    } \ 			    if (IsNonDisplayAttr(a)) { \ 				a = 0;
comment|/* zero == don't display */
value|\ 			    } \ 			    if (!FormattedScreen()) { \ 				a = 1;
comment|/* one ==> do display on unformatted */
value|\ 			    }
name|ScreenImage
modifier|*
name|p
decl_stmt|,
modifier|*
name|upper
decl_stmt|;
name|ScreenBuffer
modifier|*
name|sp
decl_stmt|;
name|int
name|fieldattr
decl_stmt|;
comment|/* spends most of its time == 0 or 1 */
comment|/* OK.  We want to do this a quickly as possible.  So, we assume we  * only need to go from Lowest to Highest.  However, if we find a  * field in the middle, we do the whole screen.  *  * In particular, we separate out the two cases from the beginning.  */
if|if
condition|(
operator|(
name|Highest
operator|!=
name|HighestScreen
argument_list|()
operator|)
operator|||
operator|(
name|Lowest
operator|!=
name|LowestScreen
argument_list|()
operator|)
condition|)
block|{
specifier|register
name|int
name|columnsleft
decl_stmt|;
name|sp
operator|=
operator|&
name|Screen
index|[
name|Lowest
index|]
expr_stmt|;
name|p
operator|=
operator|&
name|Host
index|[
name|Lowest
index|]
expr_stmt|;
name|upper
operator|=
operator|&
name|Host
index|[
name|Highest
index|]
expr_stmt|;
name|fieldattr
operator|=
name|FieldAttributes
argument_list|(
name|Lowest
argument_list|)
expr_stmt|;
name|DoAttribute
argument_list|(
name|fieldattr
argument_list|)
expr_stmt|;
comment|/* Set standout, non-display status */
name|columnsleft
operator|=
name|NumberColumns
operator|-
name|ScreenLineOffset
argument_list|(
name|p
operator|-
name|Host
argument_list|)
expr_stmt|;
while|while
condition|(
name|p
operator|<=
name|upper
condition|)
block|{
if|if
condition|(
name|IsStartFieldPointer
argument_list|(
name|p
argument_list|)
condition|)
block|{
comment|/* New field? */
name|Highest
operator|=
name|HighestScreen
argument_list|()
expr_stmt|;
name|Lowest
operator|=
name|LowestScreen
argument_list|()
expr_stmt|;
name|TryToSend
argument_list|()
expr_stmt|;
comment|/* Recurse */
return|return;
block|}
elseif|else
if|if
condition|(
name|fieldattr
condition|)
block|{
comment|/* Should we display? */
name|sp
operator|->
name|data
operator|=
name|disp_asc
index|[
name|p
operator|->
name|data
index|]
expr_stmt|;
comment|/* Display translated data */
name|sp
operator|->
name|attr
operator|=
name|fieldattr
expr_stmt|;
block|}
else|else
block|{
name|sp
operator|->
name|data
operator|=
literal|' '
expr_stmt|;
name|sp
operator|->
name|attr
operator|=
name|NORMAL
expr_stmt|;
block|}
comment|/* If the physical screen is larger than what we 			 * are using, we need to make sure that each line 			 * starts at the beginning of the line.  Otherwise, 			 * we will just string all the lines together. 			 */
name|p
operator|++
expr_stmt|;
name|sp
operator|++
expr_stmt|;
block|}
block|}
else|else
block|{
comment|/* Going from Lowest to Highest */
name|ScreenImage
modifier|*
name|End
init|=
operator|&
name|Host
index|[
name|ScreenSize
index|]
operator|-
literal|1
decl_stmt|;
name|sp
operator|=
name|Screen
expr_stmt|;
name|p
operator|=
name|Host
expr_stmt|;
name|fieldattr
operator|=
name|FieldAttributes
argument_list|(
name|LowestScreen
argument_list|()
argument_list|)
expr_stmt|;
name|DoAttribute
argument_list|(
name|fieldattr
argument_list|)
expr_stmt|;
comment|/* Set standout, non-display status */
while|while
condition|(
name|p
operator|<=
name|End
condition|)
block|{
if|if
condition|(
name|IsStartFieldPointer
argument_list|(
name|p
argument_list|)
condition|)
block|{
comment|/* New field? */
name|fieldattr
operator|=
name|FieldAttributesPointer
argument_list|(
name|p
argument_list|)
expr_stmt|;
comment|/* Get attributes */
name|DoAttribute
argument_list|(
name|fieldattr
argument_list|)
expr_stmt|;
comment|/* Set standout, non-display */
block|}
else|else
block|{
if|if
condition|(
name|fieldattr
condition|)
block|{
comment|/* Should we display? */
comment|/* Display translated data */
name|sp
operator|->
name|data
operator|=
name|disp_asc
index|[
name|p
operator|->
name|data
index|]
expr_stmt|;
name|sp
operator|->
name|attr
operator|=
name|fieldattr
expr_stmt|;
block|}
else|else
block|{
name|sp
operator|->
name|data
operator|=
literal|' '
expr_stmt|;
name|sp
operator|->
name|attr
operator|=
name|NORMAL
expr_stmt|;
block|}
block|}
comment|/* If the physical screen is larger than what we 			 * are using, we need to make sure that each line 			 * starts at the beginning of the line.  Otherwise, 			 * we will just string all the lines together. 			 */
name|p
operator|++
expr_stmt|;
name|sp
operator|++
expr_stmt|;
block|}
block|}
name|terminalCursorAddress
operator|=
name|CorrectTerminalCursor
argument_list|()
expr_stmt|;
name|scrwrite
argument_list|(
name|Screen
operator|+
name|Lowest
argument_list|,
sizeof|sizeof
name|Screen
index|[
literal|0
index|]
operator|*
operator|(
name|Highest
operator|-
name|Lowest
operator|)
argument_list|,
name|Lowest
argument_list|)
expr_stmt|;
name|VideoSetCursorPosition
argument_list|(
name|ScreenLine
argument_list|(
name|terminalCursorAddress
argument_list|)
argument_list|,
name|ScreenLineOffset
argument_list|(
name|terminalCursorAddress
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|Lowest
operator|=
name|HighestScreen
argument_list|()
operator|+
literal|1
expr_stmt|;
name|Highest
operator|=
name|LowestScreen
argument_list|()
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|needToRing
condition|)
block|{
name|DataToTerminal
argument_list|(
literal|"\7"
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|needToRing
operator|=
literal|0
expr_stmt|;
block|}
return|return;
block|}
end_function

begin_escape
end_escape

begin_comment
comment|/* InitTerminal - called to initialize the screen, etc. */
end_comment

begin_function
name|void
name|InitTerminal
parameter_list|()
block|{
name|InitMapping
argument_list|()
expr_stmt|;
comment|/* Go do mapping file (MAP3270) first */
if|if
condition|(
operator|!
name|screenInitd
condition|)
block|{
comment|/* not initialized */
name|NumberLines
operator|=
literal|24
expr_stmt|;
comment|/* XXX */
name|NumberColumns
operator|=
literal|80
expr_stmt|;
comment|/* XXX */
name|scrini
argument_list|()
expr_stmt|;
name|savescr
argument_list|()
expr_stmt|;
comment|/* Save the screen buffer away */
name|ClearArray
argument_list|(
name|Screen
argument_list|)
expr_stmt|;
name|terminalCursorAddress
operator|=
name|SetBufferAddress
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|screenInitd
operator|=
literal|1
expr_stmt|;
name|screenStopped
operator|=
literal|0
expr_stmt|;
comment|/* Not stopped */
block|}
name|Initialized
operator|=
literal|1
expr_stmt|;
block|}
end_function

begin_comment
comment|/* StopScreen - called when we are going away... */
end_comment

begin_function
name|void
name|StopScreen
parameter_list|(
name|doNewLine
parameter_list|)
name|int
name|doNewLine
decl_stmt|;
block|{
if|if
condition|(
name|screenInitd
operator|&&
operator|!
name|screenStopped
condition|)
block|{
name|scrrest
argument_list|()
expr_stmt|;
name|VideoSetCursorPosition
argument_list|(
name|NumberLines
operator|-
literal|1
argument_list|,
name|NumberColumns
operator|-
literal|1
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/* RefreshScreen - called to cause the screen to be refreshed */
end_comment

begin_function
name|void
name|RefreshScreen
parameter_list|()
block|{
name|Highest
operator|=
name|HighestScreen
argument_list|()
expr_stmt|;
name|Lowest
operator|=
name|LowestScreen
argument_list|()
expr_stmt|;
name|TryToSend
argument_list|()
expr_stmt|;
block|}
end_function

begin_comment
comment|/* ConnectScreen - called to reconnect to the screen */
end_comment

begin_function
name|void
name|ConnectScreen
parameter_list|()
block|{
if|if
condition|(
name|screenInitd
condition|)
block|{
name|RefreshScreen
argument_list|()
expr_stmt|;
name|screenStopped
operator|=
literal|0
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/* LocalClearScreen() - clear the whole ball of wax, cheaply */
end_comment

begin_function
name|void
name|LocalClearScreen
parameter_list|()
block|{
name|Clear3270
argument_list|()
expr_stmt|;
name|Lowest
operator|=
name|LowestScreen
argument_list|()
expr_stmt|;
comment|/* everything in sync... */
name|Highest
operator|=
name|HighestScreen
argument_list|()
expr_stmt|;
name|TryToSend
argument_list|()
expr_stmt|;
block|}
end_function

begin_escape
end_escape

begin_comment
comment|/*  * Implement the bell/error message function.  */
end_comment

begin_decl_stmt
name|int
name|bellwinup
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* If != 0, length of bell message */
end_comment

begin_decl_stmt
specifier|static
name|int
name|bellpos0
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Where error message goes */
end_comment

begin_decl_stmt
specifier|static
name|char
name|bellstring
index|[
literal|100
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Where message goes */
end_comment

begin_define
define|#
directive|define
name|BELL_SPACES
value|2
end_define

begin_comment
comment|/* 2 spaces between border and bell */
end_comment

begin_define
define|#
directive|define
name|BELL_HIGH_LOW
parameter_list|(
name|h
parameter_list|,
name|l
parameter_list|)
value|{ \ 	    h = bellpos0+2*NumberColumns+bellwinup+BELL_SPACES*2; \ 	    l = bellpos0; \ 	}
end_define

begin_function
name|void
name|BellOff
parameter_list|()
block|{
if|if
condition|(
name|bellwinup
condition|)
block|{
name|BELL_HIGH_LOW
argument_list|(
name|Highest
argument_list|,
name|Lowest
argument_list|)
expr_stmt|;
name|TryToSend
argument_list|()
expr_stmt|;
block|}
block|}
end_function

begin_function
name|void
name|RingBell
parameter_list|(
name|s
parameter_list|)
name|char
modifier|*
name|s
decl_stmt|;
block|{
name|needToRing
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|s
condition|)
block|{
name|int
name|len
init|=
name|strlen
argument_list|(
name|s
argument_list|)
decl_stmt|;
if|if
condition|(
name|len
operator|>
sizeof|sizeof
name|bellstring
operator|-
literal|1
condition|)
block|{
name|OurExitString
argument_list|(
name|stderr
argument_list|,
literal|"Bell string too long."
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
name|memcpy
argument_list|(
name|bellstring
argument_list|,
name|s
argument_list|,
name|len
operator|+
literal|1
argument_list|)
expr_stmt|;
name|BELL_HIGH_LOW
argument_list|(
name|Highest
argument_list|,
name|Lowest
argument_list|)
expr_stmt|;
name|TryToSend
argument_list|()
expr_stmt|;
block|}
block|}
end_function

begin_escape
end_escape

begin_comment
comment|/*  * Update the OIA area.  */
end_comment

begin_function
name|void
name|ScreenOIA
parameter_list|(
name|oia
parameter_list|)
name|OIA
modifier|*
name|oia
decl_stmt|;
block|{ }
end_function

begin_escape
end_escape

begin_comment
comment|/* returns a 1 if no more output available (so, go ahead and block),     or a 0 if there is more output available (so, just poll the other     sources/destinations, don't block).  */
end_comment

begin_function
name|int
name|DoTerminalOutput
parameter_list|()
block|{
comment|/* called just before a select to conserve IO to terminal */
if|if
condition|(
operator|!
name|Initialized
condition|)
block|{
return|return
literal|1
return|;
comment|/* No output if not initialized */
block|}
if|if
condition|(
operator|(
name|Lowest
operator|<=
name|Highest
operator|)
operator|||
name|needToRing
operator|||
operator|(
name|terminalCursorAddress
operator|!=
name|CorrectTerminalCursor
argument_list|()
operator|)
condition|)
block|{
name|TryToSend
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
name|Lowest
operator|>
name|Highest
condition|)
block|{
return|return
literal|1
return|;
comment|/* no more output now */
block|}
else|else
block|{
return|return
literal|0
return|;
comment|/* more output for future */
block|}
block|}
end_function

begin_escape
end_escape

begin_comment
comment|/*  * The following are defined to handle transparent data.  */
end_comment

begin_function
name|void
name|TransStop
parameter_list|()
block|{
name|RefreshScreen
argument_list|()
expr_stmt|;
block|}
end_function

begin_function
name|void
name|TransOut
parameter_list|(
name|buffer
parameter_list|,
name|count
parameter_list|)
name|unsigned
name|char
modifier|*
name|buffer
decl_stmt|;
name|int
name|count
decl_stmt|;
block|{
while|while
condition|(
name|DoTerminalOutput
argument_list|()
operator|==
literal|0
condition|)
block|{
empty_stmt|;
block|}
operator|(
name|void
operator|)
name|DataToTerminal
argument_list|(
name|buffer
argument_list|,
name|count
argument_list|)
expr_stmt|;
block|}
end_function

begin_escape
end_escape

begin_comment
comment|/*  * init_screen()  *  * Initialize variables used by screen.  */
end_comment

begin_function
name|void
name|init_screen
parameter_list|()
block|{
name|bellwinup
operator|=
literal|0
expr_stmt|;
block|}
end_function

end_unit

