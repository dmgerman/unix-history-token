...
begin_comment
comment|/* See tm-m68k.h.  0 means 680[01]0 with no 68881.  */
end_comment
...
directive|define
name|ASM_SPEC
value|"%{m68020:-68020}%{!m68020:-68010}"
end_define
...
begin_comment
comment|/* Override parts of tm-m68k.h to fit the SGS-3b1 assembler.  */
end_comment
...
parameter_list|)
define|\
value|if (RTX_INTEGRATED_P (TABLE))				\     fprintf (FILE, "\tset LD%%%d,L%%%d-LI%%%d\n", (NUM), (NUM), (NUM))
end_define
...
parameter_list|)
define|\
value|{ if ((PTR)[0] == 'j'&& (PTR)[1] == 'b')		\     { ++(PTR);						\       while (*(PTR) != ' ')				\ 	{ putc (*(PTR), (FILE)); ++(PTR); }		\       fprintf ((FILE), ".w"); }				\   else if ((PTR)[0] == 'f')				\     {							\       if (!strncmp ((PTR), "fmove", 5))			\ 	{ fprintf ((FILE), "fmov"); (PTR) += 5; }	\       else if (!strncmp ((PTR), "ftst", 4))		\ 	{ fprintf ((FILE), "ftest"); (PTR) += 4; }	\     }							\
