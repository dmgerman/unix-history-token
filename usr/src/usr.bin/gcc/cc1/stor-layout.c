...
directive|if
literal|0
comment|/* If there are basetypes, the caller should already have      laid them out.  Leave space at the beginning for them.  */
block|if (TYPE_SIZE (rec) != 0)     {       if (TREE_CODE (TYPE_SIZE (rec)) == INTEGER_CST) 	const_size = TREE_INT_CST_LOW (TYPE_SIZE (rec));       else 	var_size = TYPE_SIZE (rec);       size_unit = TYPE_SIZE_UNIT (rec);     }
endif|#
directive|endif
comment|/* 0 */
for|for
...
continue|continue;
block|}
comment|/* Enumerators and enum types which are local to this class need not 	 be laid out.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* A bit field may not span the unit of alignment of its type. 	     Advance to next boundary if necessary. 	     If the type's alignment is less than its size, 	     then the bitfield may span more than one alignment unit, 	     but only up to the number that the type itself occupies. 	     Thus, a bitfield declared int, if int is 4 bytes but aligned 	     to the halfword, must fit within a 4 byte group that is 	     halfword aligned.  */
if|if
condition|(
name|const_size
operator|/
...
directive|if
literal|0
comment|/* Should be in a language-specific file, since message is lang spec.  */
block|if (TREE_STATIC (field)) 	{ 	  error_with_decl (field, "field `%s' declared static in union"); 	  TREE_STATIC (field) = 0; 	}
endif|#
directive|endif
comment|/* Ignore enumerators and enum types local to the union.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
