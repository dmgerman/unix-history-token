...
index|[]
init|=
literal|"@(#)cap_mkdb.c	1.7 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
argument_list|)
expr_stmt|;
comment|/* Create the backup: outname.db.BAK */
if|if
condition|(
operator|(
name|bak
...
argument_list|)
expr_stmt|;
comment|/*  	 * We want to avoid the confusion of where the capability record  	 * is being read from.  Since we intend to read the ascii file, we 	 * should make sure that the corresponding .db file is not present to 	 * override.          */
if|if
condition|(
name|unlink
argument_list|(
