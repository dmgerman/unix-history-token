...
index|[]
init|=
literal|"@(#)telnet.c	5.10 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* the user has set "localsigs" */
end_comment
...
begin_comment
comment|/* in line mode recognize echo toggle */
end_comment
...
directive|if
literal|0
comment|/*      * There is a question here.  Should we send a TM to flush the stream?      * Should we also send a TELNET SYNCH also?      */
block|*nfrontp++ = IAC;     *nfrontp++ = DO;     *nfrontp++ = TELOPT_TM;     flushout = 1;     printoption("SENT", doopt, TELOPT_TM);
endif|#
directive|endif
comment|/* 0 */
name|NET2ADD
