...
condition|)
block|{
comment|/* 		 * Advance local so further uses just yield file name 		 * thus later references for error messages need not check 		 * for '|' special case. 	 	 */
name|local
operator|+=
literal|1
expr_stmt|;
...
literal|0
expr_stmt|;
comment|/* so we don't print a message if the transfer was aborted */
goto|goto
name|done
goto|;
block|}
...
condition|)
block|{
comment|/* 		 * Advance local over '|' so don't need to check for 		 * '|' special case any further. 		 */
name|local
operator|+=
literal|1
expr_stmt|;
