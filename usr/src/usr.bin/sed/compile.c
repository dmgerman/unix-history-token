...
index|[]
init|=
literal|"@(#)compile.c	5.7 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * Convert goto label names to addresses.  Detect unused and duplicate labels.  * Set appendnum to the number of a and r commands in the script.  Free the  * memory used by labels in b and t commands (but not by :).  Root is a pointer  * to the script linked list; cp points to the search start.  *  * TODO: Remove } nodes  */
end_comment
...
begin_comment
comment|/*   * Find any unused labels.  This is because we want to warn the user if they  * accidentally put whitespace on a label name causing it be a different label  * than they intended.  */
end_comment
