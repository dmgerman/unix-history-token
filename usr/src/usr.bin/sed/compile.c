begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1992 Diomidis Spinellis.  * Copyright (c) 1992 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Diomidis Spinellis of Imperial College, University of London.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)compile.c	5.2 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<limits.h>
end_include

begin_include
include|#
directive|include
file|<regex.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|"defs.h"
end_include

begin_include
include|#
directive|include
file|"extern.h"
end_include

begin_decl_stmt
specifier|static
name|char
modifier|*
name|compile_addr
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|,
expr|struct
name|s_addr
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|compile_delimited
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|,
name|char
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|compile_flags
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|,
expr|struct
name|s_subst
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|compile_re
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|,
name|regex_t
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|compile_subst
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|,
name|char
operator|*
operator|*
operator|,
name|size_t
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|compile_text
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|compile_tr
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|,
name|char
operator|*
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|s_command
modifier|*
modifier|*
name|compile_stream
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|,
expr|struct
name|s_command
operator|*
operator|*
operator|,
name|char
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
name|duptoeol
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|s_command
modifier|*
name|findlabel
name|__P
argument_list|(
operator|(
expr|struct
name|s_command
operator|*
operator|,
expr|struct
name|s_command
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|fixuplabel
name|__P
argument_list|(
operator|(
expr|struct
name|s_command
operator|*
operator|,
expr|struct
name|s_command
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Command specification.  This is used to drive the command parser.  */
end_comment

begin_struct
struct|struct
name|s_format
block|{
name|char
name|code
decl_stmt|;
comment|/* Command code */
name|int
name|naddr
decl_stmt|;
comment|/* Number of address args */
name|enum
name|e_args
name|args
decl_stmt|;
comment|/* Argument type */
block|}
struct|;
end_struct

begin_decl_stmt
specifier|static
name|struct
name|s_format
name|cmd_fmts
index|[]
init|=
block|{
block|{
literal|'{'
block|,
literal|2
block|,
name|GROUP
block|}
block|,
block|{
literal|'a'
block|,
literal|1
block|,
name|TEXT
block|}
block|,
block|{
literal|'b'
block|,
literal|2
block|,
name|BRANCH
block|}
block|,
block|{
literal|'c'
block|,
literal|2
block|,
name|TEXT
block|}
block|,
block|{
literal|'d'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'D'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'g'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'G'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'h'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'H'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'i'
block|,
literal|1
block|,
name|TEXT
block|}
block|,
block|{
literal|'l'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'n'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'N'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'p'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'P'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'q'
block|,
literal|1
block|,
name|EMPTY
block|}
block|,
block|{
literal|'r'
block|,
literal|1
block|,
name|RFILE
block|}
block|,
block|{
literal|'s'
block|,
literal|2
block|,
name|SUBST
block|}
block|,
block|{
literal|'t'
block|,
literal|2
block|,
name|BRANCH
block|}
block|,
block|{
literal|'w'
block|,
literal|2
block|,
name|WFILE
block|}
block|,
block|{
literal|'x'
block|,
literal|2
block|,
name|EMPTY
block|}
block|,
block|{
literal|'y'
block|,
literal|2
block|,
name|TR
block|}
block|,
block|{
literal|'!'
block|,
literal|2
block|,
name|NONSEL
block|}
block|,
block|{
literal|':'
block|,
literal|0
block|,
name|LABEL
block|}
block|,
block|{
literal|'#'
block|,
literal|0
block|,
name|COMMENT
block|}
block|,
block|{
literal|'='
block|,
literal|1
block|,
name|EMPTY
block|}
block|,
block|{
literal|'\0'
block|,
literal|0
block|,
name|COMMENT
block|}
block|, }
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* The compiled program */
end_comment

begin_decl_stmt
name|struct
name|s_command
modifier|*
name|prog
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Compile the program into prog.  * Initialise appends  */
end_comment

begin_function
name|void
name|compile
parameter_list|()
block|{
operator|*
name|compile_stream
argument_list|(
name|NULL
argument_list|,
operator|&
name|prog
argument_list|,
name|NULL
argument_list|)
operator|=
name|NULL
expr_stmt|;
name|fixuplabel
argument_list|(
name|prog
argument_list|,
name|prog
argument_list|)
expr_stmt|;
name|appends
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|s_appends
argument_list|)
operator|*
name|appendnum
argument_list|)
expr_stmt|;
block|}
end_function

begin_define
define|#
directive|define
name|EATSPACE
parameter_list|()
value|do {							\ 	if (p)								\ 		while (*p&& isascii(*p)&& isspace(*p))		\ 			p++;						\ 	} while (0)
end_define

begin_function
specifier|static
name|struct
name|s_command
modifier|*
modifier|*
name|compile_stream
parameter_list|(
name|terminator
parameter_list|,
name|link
parameter_list|,
name|p
parameter_list|)
name|char
modifier|*
name|terminator
decl_stmt|;
name|struct
name|s_command
modifier|*
modifier|*
name|link
decl_stmt|;
specifier|register
name|char
modifier|*
name|p
decl_stmt|;
block|{
specifier|static
name|char
name|lbuf
index|[
name|_POSIX2_LINE_MAX
operator|+
literal|1
index|]
decl_stmt|;
comment|/* To save stack */
name|struct
name|s_command
modifier|*
name|cmd
decl_stmt|,
modifier|*
name|cmd2
decl_stmt|;
name|struct
name|s_format
modifier|*
name|fp
decl_stmt|;
name|int
name|naddr
decl_stmt|;
comment|/* Number of addresses */
if|if
condition|(
name|p
operator|!=
name|NULL
condition|)
goto|goto
name|semicolon
goto|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|(
name|p
operator|=
name|cu_fgets
argument_list|(
name|lbuf
argument_list|,
sizeof|sizeof
argument_list|(
name|lbuf
argument_list|)
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
name|terminator
operator|!=
name|NULL
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unexpected EOF (pending }'s)"
argument_list|)
expr_stmt|;
return|return
operator|(
name|link
operator|)
return|;
block|}
name|semicolon
label|:
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
name|p
operator|&&
operator|(
operator|*
name|p
operator|==
literal|'#'
operator|||
operator|*
name|p
operator|==
literal|'\0'
operator|)
condition|)
continue|continue;
if|if
condition|(
operator|*
name|p
operator|==
literal|'}'
condition|)
block|{
if|if
condition|(
name|terminator
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unexpected }"
argument_list|)
expr_stmt|;
return|return
operator|(
name|link
operator|)
return|;
block|}
operator|*
name|link
operator|=
name|cmd
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|s_command
argument_list|)
argument_list|)
expr_stmt|;
name|link
operator|=
operator|&
name|cmd
operator|->
name|next
expr_stmt|;
name|cmd
operator|->
name|nonsel
operator|=
name|cmd
operator|->
name|inrange
operator|=
literal|0
expr_stmt|;
comment|/* First parse the addresses */
name|naddr
operator|=
literal|0
expr_stmt|;
name|cmd
operator|->
name|a1
operator|=
name|cmd
operator|->
name|a2
operator|=
name|NULL
expr_stmt|;
comment|/* Valid characters to start an address */
define|#
directive|define
name|addrchar
parameter_list|(
name|c
parameter_list|)
value|(strchr("0123456789/\\$", (c)))
if|if
condition|(
name|addrchar
argument_list|(
operator|*
name|p
argument_list|)
condition|)
block|{
name|naddr
operator|++
expr_stmt|;
name|cmd
operator|->
name|a1
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|s_addr
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|=
name|compile_addr
argument_list|(
name|p
argument_list|,
name|cmd
operator|->
name|a1
argument_list|)
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
comment|/* EXTENSION */
if|if
condition|(
operator|*
name|p
operator|==
literal|','
condition|)
block|{
name|naddr
operator|++
expr_stmt|;
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
comment|/* EXTENSION */
name|cmd
operator|->
name|a2
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|s_addr
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|=
name|compile_addr
argument_list|(
name|p
argument_list|,
name|cmd
operator|->
name|a2
argument_list|)
expr_stmt|;
block|}
block|}
name|nonsel
label|:
comment|/* Now parse the command */
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
operator|*
name|p
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"command expected"
argument_list|)
expr_stmt|;
name|cmd
operator|->
name|code
operator|=
operator|*
name|p
expr_stmt|;
for|for
control|(
name|fp
operator|=
name|cmd_fmts
init|;
name|fp
operator|->
name|code
condition|;
name|fp
operator|++
control|)
if|if
condition|(
name|fp
operator|->
name|code
operator|==
operator|*
name|p
condition|)
break|break;
if|if
condition|(
operator|!
name|fp
operator|->
name|code
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"invalid command code %c"
argument_list|,
operator|*
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|naddr
operator|>
name|fp
operator|->
name|naddr
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"command %c expects up to %d address(es), found %d"
argument_list|,
operator|*
name|p
argument_list|,
name|fp
operator|->
name|naddr
argument_list|,
name|naddr
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|fp
operator|->
name|args
condition|)
block|{
case|case
name|NONSEL
case|:
comment|/* ! */
name|cmd
operator|->
name|nonsel
operator|=
operator|!
name|cmd
operator|->
name|nonsel
expr_stmt|;
name|p
operator|++
expr_stmt|;
goto|goto
name|nonsel
goto|;
case|case
name|GROUP
case|:
comment|/* { */
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
operator|*
name|p
condition|)
name|p
operator|=
name|NULL
expr_stmt|;
name|cmd2
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|s_command
argument_list|)
argument_list|)
expr_stmt|;
name|cmd2
operator|->
name|code
operator|=
literal|'}'
expr_stmt|;
operator|*
name|compile_stream
argument_list|(
literal|"}"
argument_list|,
operator|&
name|cmd
operator|->
name|u
operator|.
name|c
argument_list|,
name|p
argument_list|)
operator|=
name|cmd2
expr_stmt|;
name|cmd
operator|->
name|next
operator|=
name|cmd2
expr_stmt|;
name|link
operator|=
operator|&
name|cmd2
operator|->
name|next
expr_stmt|;
break|break;
case|case
name|EMPTY
case|:
comment|/* d D g G h H l n N p P q x = \0 */
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|';'
condition|)
block|{
name|p
operator|++
expr_stmt|;
name|link
operator|=
operator|&
name|cmd
operator|->
name|next
expr_stmt|;
goto|goto
name|semicolon
goto|;
block|}
if|if
condition|(
operator|*
name|p
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"extra characters at the end of %c command"
argument_list|,
name|cmd
operator|->
name|code
argument_list|)
expr_stmt|;
break|break;
case|case
name|TEXT
case|:
comment|/* a c i */
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|!=
literal|'\\'
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"command %c expects \\ followed by text"
argument_list|,
name|cmd
operator|->
name|code
argument_list|)
expr_stmt|;
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"extra characters after \\ at the end of %c command"
argument_list|,
name|cmd
operator|->
name|code
argument_list|)
expr_stmt|;
name|cmd
operator|->
name|t
operator|=
name|compile_text
argument_list|()
expr_stmt|;
break|break;
case|case
name|COMMENT
case|:
comment|/* \0 # */
break|break;
case|case
name|WFILE
case|:
comment|/* w */
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|'\0'
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"filename expected"
argument_list|)
expr_stmt|;
name|cmd
operator|->
name|t
operator|=
name|duptoeol
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|aflag
condition|)
name|cmd
operator|->
name|u
operator|.
name|fd
operator|=
operator|-
literal|1
expr_stmt|;
elseif|else
if|if
condition|(
operator|(
name|cmd
operator|->
name|u
operator|.
name|fd
operator|=
name|open
argument_list|(
name|p
argument_list|,
name|O_WRONLY
operator||
name|O_APPEND
operator||
name|O_CREAT
operator||
name|O_TRUNC
argument_list|,
name|DEFFILEMODE
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
name|err
argument_list|(
name|FATAL
argument_list|,
literal|"%s: %s\n"
argument_list|,
name|p
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
break|break;
case|case
name|RFILE
case|:
comment|/* r */
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|'\0'
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"filename expected"
argument_list|)
expr_stmt|;
else|else
name|cmd
operator|->
name|t
operator|=
name|duptoeol
argument_list|(
name|p
argument_list|)
expr_stmt|;
break|break;
case|case
name|BRANCH
case|:
comment|/* b t */
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|'\0'
condition|)
name|cmd
operator|->
name|t
operator|=
name|NULL
expr_stmt|;
else|else
name|cmd
operator|->
name|t
operator|=
name|duptoeol
argument_list|(
name|p
argument_list|)
expr_stmt|;
break|break;
case|case
name|LABEL
case|:
comment|/* : */
name|p
operator|++
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
name|cmd
operator|->
name|t
operator|=
name|duptoeol
argument_list|(
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|strlen
argument_list|(
name|p
argument_list|)
operator|==
literal|0
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"empty label"
argument_list|)
expr_stmt|;
break|break;
case|case
name|SUBST
case|:
comment|/* s */
name|p
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|'\0'
operator|||
operator|*
name|p
operator|==
literal|'\\'
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"substitute pattern can not be delimited by newline or backslash"
argument_list|)
expr_stmt|;
name|cmd
operator|->
name|u
operator|.
name|s
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|s_subst
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|=
name|compile_re
argument_list|(
name|p
argument_list|,
operator|&
name|cmd
operator|->
name|u
operator|.
name|s
operator|->
name|re
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unterminated substitute pattern"
argument_list|)
expr_stmt|;
name|cmd
operator|->
name|u
operator|.
name|s
operator|->
name|pmatch
operator|=
name|xmalloc
argument_list|(
operator|(
name|cmd
operator|->
name|u
operator|.
name|s
operator|->
name|re
operator|.
name|re_nsub
operator|+
literal|1
operator|)
operator|*
sizeof|sizeof
argument_list|(
name|regmatch_t
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|--
expr_stmt|;
name|p
operator|=
name|compile_subst
argument_list|(
name|p
argument_list|,
operator|&
name|cmd
operator|->
name|u
operator|.
name|s
operator|->
name|new
argument_list|,
name|cmd
operator|->
name|u
operator|.
name|s
operator|->
name|re
operator|.
name|re_nsub
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unterminated substitute replace in regular expression"
argument_list|)
expr_stmt|;
name|p
operator|=
name|compile_flags
argument_list|(
name|p
argument_list|,
name|cmd
operator|->
name|u
operator|.
name|s
argument_list|)
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|';'
condition|)
block|{
name|p
operator|++
expr_stmt|;
name|link
operator|=
operator|&
name|cmd
operator|->
name|next
expr_stmt|;
goto|goto
name|semicolon
goto|;
block|}
break|break;
case|case
name|TR
case|:
comment|/* y */
name|p
operator|++
expr_stmt|;
name|p
operator|=
name|compile_tr
argument_list|(
name|p
argument_list|,
operator|(
name|char
operator|*
operator|*
operator|)
operator|&
name|cmd
operator|->
name|u
operator|.
name|y
argument_list|)
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|';'
condition|)
block|{
name|p
operator|++
expr_stmt|;
name|link
operator|=
operator|&
name|cmd
operator|->
name|next
expr_stmt|;
goto|goto
name|semicolon
goto|;
block|}
if|if
condition|(
operator|*
name|p
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"extra text at the end of a transform command"
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
block|}
end_function

begin_comment
comment|/*  * Get a delimited string.  P points to the delimeter of the string; d points  * to a buffer area.  Newline and delimiter escapes are processed; other  * escapes are ignored.  *  * Returns a pointer to the first character after the final delimiter or NULL  * in the case of a non-terminated string.  The character array d is filled  * with the processed string.  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|compile_delimited
parameter_list|(
name|p
parameter_list|,
name|d
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|,
decl|*
name|d
decl_stmt|;
end_function

begin_block
block|{
name|char
name|c
decl_stmt|;
name|c
operator|=
operator|*
name|p
operator|++
expr_stmt|;
if|if
condition|(
name|c
operator|==
literal|'\0'
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
elseif|else
if|if
condition|(
name|c
operator|==
literal|'\\'
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"\\ can not be used as a string delimiter"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|c
operator|==
literal|'\n'
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"newline can not be used as a string delimiter"
argument_list|)
expr_stmt|;
while|while
condition|(
operator|*
name|p
condition|)
block|{
if|if
condition|(
operator|*
name|p
operator|==
literal|'\\'
operator|&&
name|p
index|[
literal|1
index|]
operator|==
name|c
condition|)
name|p
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
literal|'\\'
operator|&&
name|p
index|[
literal|1
index|]
operator|==
literal|'n'
condition|)
block|{
operator|*
name|d
operator|++
operator|=
literal|'\n'
expr_stmt|;
name|p
operator|+=
literal|2
expr_stmt|;
continue|continue;
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
name|c
condition|)
block|{
operator|*
name|d
operator|=
literal|'\0'
expr_stmt|;
return|return
operator|(
name|p
operator|+
literal|1
operator|)
return|;
block|}
operator|*
name|d
operator|++
operator|=
operator|*
name|p
operator|++
expr_stmt|;
block|}
return|return
operator|(
name|NULL
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Get a regular expression.  P points to the delimeter of the regular  * expression; d points a regexp pointer.  Newline and delimiter escapes  * are processed; other escapes are ignored.  * Returns a pointer to the first character after the final delimiter  * or NULL in the case of a non terminated regular expression.  * The regexp pointer is set to the compiled regular expression.  * Cflags are passed to regcomp.  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|compile_re
parameter_list|(
name|p
parameter_list|,
name|rep
parameter_list|,
name|cflags
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|;
name|regex_t
modifier|*
name|rep
decl_stmt|;
name|int
name|cflags
decl_stmt|;
block|{
name|int
name|eval
decl_stmt|;
name|char
name|re
index|[
name|_POSIX2_LINE_MAX
operator|+
literal|1
index|]
decl_stmt|;
name|p
operator|=
name|compile_delimited
argument_list|(
name|p
argument_list|,
name|re
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|&&
operator|(
name|eval
operator|=
name|regcomp
argument_list|(
name|rep
argument_list|,
name|re
argument_list|,
name|cflags
argument_list|)
operator|)
operator|!=
literal|0
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"RE error: %s"
argument_list|,
name|strregerror
argument_list|(
name|eval
argument_list|,
name|rep
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Compile the substitution string of a regular expression and set res to  * point to a saved copy of it.  Nsub is the number of parenthesized regular  * expressions.  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|compile_subst
parameter_list|(
name|p
parameter_list|,
name|res
parameter_list|,
name|nsub
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|,
decl|*
modifier|*
name|res
decl_stmt|;
end_function

begin_decl_stmt
name|size_t
name|nsub
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|static
name|char
name|lbuf
index|[
name|_POSIX2_LINE_MAX
operator|+
literal|1
index|]
decl_stmt|;
name|int
name|asize
decl_stmt|,
name|size
decl_stmt|;
name|char
name|c
decl_stmt|,
modifier|*
name|text
decl_stmt|,
modifier|*
name|op
decl_stmt|,
modifier|*
name|s
decl_stmt|;
name|c
operator|=
operator|*
name|p
operator|++
expr_stmt|;
comment|/* Terminator character */
if|if
condition|(
name|c
operator|==
literal|'\0'
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
name|asize
operator|=
literal|2
operator|*
name|_POSIX2_LINE_MAX
operator|+
literal|1
expr_stmt|;
name|text
operator|=
name|xmalloc
argument_list|(
name|asize
argument_list|)
expr_stmt|;
name|size
operator|=
literal|0
expr_stmt|;
do|do
block|{
name|op
operator|=
name|s
operator|=
name|text
operator|+
name|size
expr_stmt|;
for|for
control|(
init|;
operator|*
name|p
condition|;
name|p
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|p
operator|==
literal|'\\'
condition|)
block|{
name|p
operator|++
expr_stmt|;
if|if
condition|(
name|strchr
argument_list|(
literal|"123456789"
argument_list|,
operator|*
name|p
argument_list|)
operator|!=
name|NULL
condition|)
block|{
operator|*
name|s
operator|++
operator|=
literal|'\\'
expr_stmt|;
if|if
condition|(
operator|*
name|p
operator|-
literal|'1'
operator|>
name|nsub
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"\\%c not defined in regular expression (use \\1-\\%d)"
argument_list|,
operator|*
name|p
argument_list|,
name|nsub
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
literal|'&'
condition|)
operator|*
name|s
operator|++
operator|=
literal|'\\'
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
name|c
condition|)
block|{
name|p
operator|++
expr_stmt|;
operator|*
name|s
operator|++
operator|=
literal|'\0'
expr_stmt|;
name|size
operator|+=
name|s
operator|-
name|op
expr_stmt|;
operator|*
name|res
operator|=
name|xrealloc
argument_list|(
name|text
argument_list|,
name|size
argument_list|)
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
block|}
elseif|else
if|if
condition|(
operator|*
name|p
operator|==
literal|'\n'
condition|)
block|{
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unescaped newline inside substitute pattern"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
operator|*
name|s
operator|++
operator|=
operator|*
name|p
expr_stmt|;
block|}
name|size
operator|+=
name|s
operator|-
name|op
expr_stmt|;
if|if
condition|(
name|asize
operator|-
name|size
operator|<
name|_POSIX2_LINE_MAX
operator|+
literal|1
condition|)
block|{
name|asize
operator|*=
literal|2
expr_stmt|;
name|text
operator|=
name|xmalloc
argument_list|(
name|asize
argument_list|)
expr_stmt|;
block|}
block|}
do|while
condition|(
name|cu_fgets
argument_list|(
name|p
operator|=
name|lbuf
argument_list|,
sizeof|sizeof
argument_list|(
name|lbuf
argument_list|)
argument_list|)
condition|)
do|;
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unterminated substitute pattern"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Compile the flags of the s command  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|compile_flags
parameter_list|(
name|p
parameter_list|,
name|s
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|;
name|struct
name|s_subst
modifier|*
name|s
decl_stmt|;
block|{
name|int
name|gn
decl_stmt|;
comment|/* True if we have seen g or n */
name|char
name|wfile
index|[
name|_POSIX2_LINE_MAX
operator|+
literal|1
index|]
decl_stmt|,
modifier|*
name|q
decl_stmt|;
name|s
operator|->
name|n
operator|=
literal|1
expr_stmt|;
comment|/* Default */
name|s
operator|->
name|p
operator|=
literal|0
expr_stmt|;
name|s
operator|->
name|wfile
operator|=
name|NULL
expr_stmt|;
name|s
operator|->
name|wfd
operator|=
operator|-
literal|1
expr_stmt|;
for|for
control|(
name|gn
operator|=
literal|0
init|;
condition|;
control|)
block|{
name|EATSPACE
argument_list|()
expr_stmt|;
comment|/* EXTENSION */
switch|switch
condition|(
operator|*
name|p
condition|)
block|{
case|case
literal|'g'
case|:
if|if
condition|(
name|gn
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"more than one number or 'g' in substitute flags"
argument_list|)
expr_stmt|;
name|gn
operator|=
literal|1
expr_stmt|;
name|s
operator|->
name|n
operator|=
literal|0
expr_stmt|;
break|break;
case|case
literal|'\0'
case|:
case|case
literal|'\n'
case|:
case|case
literal|';'
case|:
return|return
operator|(
name|p
operator|)
return|;
case|case
literal|'p'
case|:
name|s
operator|->
name|p
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'1'
case|:
case|case
literal|'2'
case|:
case|case
literal|'3'
case|:
case|case
literal|'4'
case|:
case|case
literal|'5'
case|:
case|case
literal|'6'
case|:
case|case
literal|'7'
case|:
case|case
literal|'8'
case|:
case|case
literal|'9'
case|:
if|if
condition|(
name|gn
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"more than one number or 'g' in substitute flags"
argument_list|)
expr_stmt|;
name|gn
operator|=
literal|1
expr_stmt|;
comment|/* XXX Check for overflow */
name|s
operator|->
name|n
operator|=
operator|(
name|int
operator|)
name|strtol
argument_list|(
name|p
argument_list|,
operator|&
name|p
argument_list|,
literal|10
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'w'
case|:
name|p
operator|++
expr_stmt|;
ifdef|#
directive|ifdef
name|HISTORIC_PRACTICE
if|if
condition|(
operator|*
name|p
operator|!=
literal|' '
condition|)
block|{
name|err
argument_list|(
name|WARNING
argument_list|,
literal|"space missing before w wfile"
argument_list|)
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
block|}
endif|#
directive|endif
name|EATSPACE
argument_list|()
expr_stmt|;
name|q
operator|=
name|wfile
expr_stmt|;
while|while
condition|(
operator|*
name|p
condition|)
block|{
if|if
condition|(
operator|*
name|p
operator|==
literal|'\n'
condition|)
break|break;
operator|*
name|q
operator|++
operator|=
operator|*
name|p
operator|++
expr_stmt|;
block|}
operator|*
name|q
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|q
operator|==
name|wfile
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"no wfile specified"
argument_list|)
expr_stmt|;
name|s
operator|->
name|wfile
operator|=
name|strdup
argument_list|(
name|wfile
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|aflag
operator|&&
operator|(
name|s
operator|->
name|wfd
operator|=
name|open
argument_list|(
name|wfile
argument_list|,
name|O_WRONLY
operator||
name|O_APPEND
operator||
name|O_CREAT
operator||
name|O_TRUNC
argument_list|,
name|DEFFILEMODE
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
name|err
argument_list|(
name|FATAL
argument_list|,
literal|"%s: %s\n"
argument_list|,
name|wfile
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
default|default:
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"bad flag in substitute command: '%c'"
argument_list|,
operator|*
name|p
argument_list|)
expr_stmt|;
break|break;
block|}
name|p
operator|++
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * Compile a translation set of strings into a lookup table.  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|compile_tr
parameter_list|(
name|p
parameter_list|,
name|transtab
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|;
name|char
modifier|*
modifier|*
name|transtab
decl_stmt|;
block|{
name|int
name|i
decl_stmt|;
name|char
modifier|*
name|lt
decl_stmt|,
modifier|*
name|op
decl_stmt|,
modifier|*
name|np
decl_stmt|;
name|char
name|old
index|[
name|_POSIX2_LINE_MAX
operator|+
literal|1
index|]
decl_stmt|;
name|char
name|new
index|[
name|_POSIX2_LINE_MAX
operator|+
literal|1
index|]
decl_stmt|;
if|if
condition|(
operator|*
name|p
operator|==
literal|'\0'
operator|||
operator|*
name|p
operator|==
literal|'\\'
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"transform pattern can not be delimited by newline or backslash"
argument_list|)
expr_stmt|;
name|p
operator|=
name|compile_delimited
argument_list|(
name|p
argument_list|,
name|old
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
block|{
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unterminated transform source string"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
name|p
operator|=
name|compile_delimited
argument_list|(
operator|--
name|p
argument_list|,
name|new
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
block|{
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unterminated transform target string"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
name|EATSPACE
argument_list|()
expr_stmt|;
if|if
condition|(
name|strlen
argument_list|(
name|new
argument_list|)
operator|!=
name|strlen
argument_list|(
name|old
argument_list|)
condition|)
block|{
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"transform strings are not the same length"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
comment|/* We assume characters are 8 bits */
name|lt
operator|=
name|xmalloc
argument_list|(
name|UCHAR_MAX
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<=
name|UCHAR_MAX
condition|;
name|i
operator|++
control|)
name|lt
index|[
name|i
index|]
operator|=
operator|(
name|char
operator|)
name|i
expr_stmt|;
for|for
control|(
name|op
operator|=
name|old
operator|,
name|np
operator|=
name|new
init|;
operator|*
name|op
condition|;
name|op
operator|++
operator|,
name|np
operator|++
control|)
name|lt
index|[
operator|(
name|u_char
operator|)
operator|*
name|op
index|]
operator|=
operator|*
name|np
expr_stmt|;
operator|*
name|transtab
operator|=
name|lt
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Compile the text following an a or i command.  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|compile_text
parameter_list|()
block|{
name|int
name|asize
decl_stmt|,
name|size
decl_stmt|;
name|char
modifier|*
name|text
decl_stmt|,
modifier|*
name|p
decl_stmt|,
modifier|*
name|op
decl_stmt|,
modifier|*
name|s
decl_stmt|;
name|char
name|lbuf
index|[
name|_POSIX2_LINE_MAX
operator|+
literal|1
index|]
decl_stmt|;
name|asize
operator|=
literal|2
operator|*
name|_POSIX2_LINE_MAX
operator|+
literal|1
expr_stmt|;
name|text
operator|=
name|xmalloc
argument_list|(
name|asize
argument_list|)
expr_stmt|;
name|size
operator|=
literal|0
expr_stmt|;
while|while
condition|(
name|cu_fgets
argument_list|(
name|lbuf
argument_list|,
sizeof|sizeof
argument_list|(
name|lbuf
argument_list|)
argument_list|)
condition|)
block|{
name|op
operator|=
name|s
operator|=
name|text
operator|+
name|size
expr_stmt|;
name|p
operator|=
name|lbuf
expr_stmt|;
name|EATSPACE
argument_list|()
expr_stmt|;
for|for
control|(
init|;
operator|*
name|p
condition|;
name|p
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|p
operator|==
literal|'\\'
condition|)
name|p
operator|++
expr_stmt|;
operator|*
name|s
operator|++
operator|=
operator|*
name|p
expr_stmt|;
block|}
name|size
operator|+=
name|s
operator|-
name|op
expr_stmt|;
if|if
condition|(
name|p
index|[
operator|-
literal|2
index|]
operator|!=
literal|'\\'
condition|)
block|{
operator|*
name|s
operator|=
literal|'\0'
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|asize
operator|-
name|size
operator|<
name|_POSIX2_LINE_MAX
operator|+
literal|1
condition|)
block|{
name|asize
operator|*=
literal|2
expr_stmt|;
name|text
operator|=
name|xmalloc
argument_list|(
name|asize
argument_list|)
expr_stmt|;
block|}
block|}
return|return
operator|(
name|xrealloc
argument_list|(
name|text
argument_list|,
name|size
operator|+
literal|1
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Get an address and return a pointer to the first character after  * it.  Fill the structure pointed to according to the address.  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|compile_addr
parameter_list|(
name|p
parameter_list|,
name|a
parameter_list|)
name|char
modifier|*
name|p
decl_stmt|;
name|struct
name|s_addr
modifier|*
name|a
decl_stmt|;
block|{
name|regex_t
modifier|*
name|re
decl_stmt|;
name|char
modifier|*
name|end
decl_stmt|;
switch|switch
condition|(
operator|*
name|p
condition|)
block|{
case|case
literal|'\\'
case|:
comment|/* Context address */
name|re
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
name|regex_t
argument_list|)
argument_list|)
expr_stmt|;
name|a
operator|->
name|u
operator|.
name|r
operator|=
name|re
expr_stmt|;
name|p
operator|=
name|compile_re
argument_list|(
name|p
operator|+
literal|1
argument_list|,
name|re
argument_list|,
name|REG_NOSUB
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unterminated regular expression"
argument_list|)
expr_stmt|;
name|a
operator|->
name|type
operator|=
name|AT_RE
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
case|case
literal|'/'
case|:
comment|/* Context address */
name|re
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
name|regex_t
argument_list|)
argument_list|)
expr_stmt|;
name|a
operator|->
name|u
operator|.
name|r
operator|=
name|re
expr_stmt|;
name|p
operator|=
name|compile_re
argument_list|(
name|p
argument_list|,
name|a
operator|->
name|u
operator|.
name|r
argument_list|,
name|REG_NOSUB
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"unterminated regular expression"
argument_list|)
expr_stmt|;
name|a
operator|->
name|type
operator|=
name|AT_RE
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
case|case
literal|'$'
case|:
comment|/* Last line */
name|a
operator|->
name|type
operator|=
name|AT_LAST
expr_stmt|;
return|return
operator|(
name|p
operator|+
literal|1
operator|)
return|;
comment|/* Line number */
case|case
literal|'0'
case|:
case|case
literal|'1'
case|:
case|case
literal|'2'
case|:
case|case
literal|'3'
case|:
case|case
literal|'4'
case|:
case|case
literal|'5'
case|:
case|case
literal|'6'
case|:
case|case
literal|'7'
case|:
case|case
literal|'8'
case|:
case|case
literal|'9'
case|:
name|a
operator|->
name|type
operator|=
name|AT_LINE
expr_stmt|;
name|a
operator|->
name|u
operator|.
name|l
operator|=
name|strtol
argument_list|(
name|p
argument_list|,
operator|&
name|end
argument_list|,
literal|10
argument_list|)
expr_stmt|;
return|return
operator|(
name|end
operator|)
return|;
default|default:
name|err
argument_list|(
name|COMPILE
argument_list|,
literal|"expected context address"
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
block|}
end_function

begin_comment
comment|/*  * Return a copy of all the characters up to \n or \0  */
end_comment

begin_function
specifier|static
name|char
modifier|*
name|duptoeol
parameter_list|(
name|s
parameter_list|)
specifier|register
name|char
modifier|*
name|s
decl_stmt|;
block|{
name|size_t
name|len
decl_stmt|;
name|char
modifier|*
name|start
decl_stmt|;
for|for
control|(
name|start
operator|=
name|s
init|;
operator|*
name|s
operator|!=
literal|'\0'
operator|&&
operator|*
name|s
operator|!=
literal|'\n'
condition|;
operator|++
name|s
control|)
empty_stmt|;
operator|*
name|s
operator|=
literal|'\0'
expr_stmt|;
name|len
operator|=
name|s
operator|-
name|start
operator|+
literal|1
expr_stmt|;
return|return
operator|(
name|memmove
argument_list|(
name|xmalloc
argument_list|(
name|len
argument_list|)
argument_list|,
name|start
argument_list|,
name|len
argument_list|)
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Find the label contained in the command l in the command linked list cp.  * L is excluded from the search.  Return NULL if not found.  */
end_comment

begin_function
specifier|static
name|struct
name|s_command
modifier|*
name|findlabel
parameter_list|(
name|l
parameter_list|,
name|cp
parameter_list|)
name|struct
name|s_command
modifier|*
name|l
decl_stmt|,
decl|*
name|cp
decl_stmt|;
end_function

begin_block
block|{
name|struct
name|s_command
modifier|*
name|r
decl_stmt|;
for|for
control|(
init|;
name|cp
condition|;
name|cp
operator|=
name|cp
operator|->
name|next
control|)
if|if
condition|(
name|cp
operator|->
name|code
operator|==
literal|':'
operator|&&
name|cp
operator|!=
name|l
operator|&&
name|strcmp
argument_list|(
name|l
operator|->
name|t
argument_list|,
name|cp
operator|->
name|t
argument_list|)
operator|==
literal|0
condition|)
return|return
operator|(
name|cp
operator|)
return|;
elseif|else
if|if
condition|(
name|cp
operator|->
name|code
operator|==
literal|'{'
operator|&&
operator|(
name|r
operator|=
name|findlabel
argument_list|(
name|l
argument_list|,
name|cp
operator|->
name|u
operator|.
name|c
argument_list|)
operator|)
condition|)
return|return
operator|(
name|r
operator|)
return|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Convert goto label names to addresses.  * Detect duplicate labels.  * Set appendnum to the number of a and r commands in the script.  * Free the memory used by labels in b and t commands (but not by :)  * Root is a pointer to the script linked list; cp points to the  * search start.  * TODO: Remove } nodes  */
end_comment

begin_function
specifier|static
name|void
name|fixuplabel
parameter_list|(
name|root
parameter_list|,
name|cp
parameter_list|)
name|struct
name|s_command
modifier|*
name|root
decl_stmt|,
decl|*
name|cp
decl_stmt|;
end_function

begin_block
block|{
name|struct
name|s_command
modifier|*
name|cp2
decl_stmt|;
for|for
control|(
init|;
name|cp
condition|;
name|cp
operator|=
name|cp
operator|->
name|next
control|)
switch|switch
condition|(
name|cp
operator|->
name|code
condition|)
block|{
case|case
literal|'a'
case|:
case|case
literal|'r'
case|:
name|appendnum
operator|++
expr_stmt|;
break|break;
case|case
literal|'b'
case|:
case|case
literal|'t'
case|:
if|if
condition|(
name|cp
operator|->
name|t
operator|==
name|NULL
condition|)
block|{
name|cp
operator|->
name|u
operator|.
name|c
operator|=
name|NULL
expr_stmt|;
break|break;
block|}
if|if
condition|(
operator|(
name|cp2
operator|=
name|findlabel
argument_list|(
name|cp
argument_list|,
name|root
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|err
argument_list|(
name|COMPILE2
argument_list|,
literal|"undefined label '%s'"
argument_list|,
name|cp
operator|->
name|t
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cp
operator|->
name|t
argument_list|)
expr_stmt|;
name|cp
operator|->
name|u
operator|.
name|c
operator|=
name|cp2
expr_stmt|;
break|break;
case|case
literal|'{'
case|:
name|fixuplabel
argument_list|(
name|root
argument_list|,
name|cp
operator|->
name|u
operator|.
name|c
argument_list|)
expr_stmt|;
break|break;
case|case
literal|':'
case|:
if|if
condition|(
name|findlabel
argument_list|(
name|cp
argument_list|,
name|root
argument_list|)
condition|)
name|err
argument_list|(
name|COMPILE2
argument_list|,
literal|"duplicate label %s"
argument_list|,
name|cp
operator|->
name|t
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
end_block

end_unit

