...
begin_comment
comment|/* Use Sun's TARGET convention.  */
end_comment
...
begin_comment
comment|/* locate the correct a.out.h file */
end_comment
...
begin_comment
comment|/* Always use the GNU version of debugging symbol type codes, if possible.  */
end_comment
...
begin_comment
comment|/* Determine whether we should attempt to handle (minimally)    N_BINCL and N_EINCL.  */
end_comment
...
begin_comment
comment|/* Define this if names etext, edata and end should not start with `_'.  */
end_comment

begin_comment
comment|/* #define nounderscore 1 */
end_comment

begin_comment
comment|/* Define NON_NATIVE if using BSD or pseudo-BSD file format on a system    whose native format is different.  */
end_comment

begin_comment
comment|/* #define NON_NATIVE */
end_comment
...
begin_comment
comment|/* hpux bugs screw ZMAGIC */
end_comment
...
parameter_list|)
define|\
value|(((x).a_magic==Omagic)? (N_TXTADDR(x)+(x).a_text) \  : (SEGMENT_SIZE + ((N_TXTADDR(x)+(x).a_text-1)& ~(SEGMENT_SIZE-1))))
end_define
...
parameter_list|)
define|\
value|(((x).a_magic==OMAGIC)? (N_TXTADDR(x)+(x).a_text) \     : (page_size+(((x).a_text-1)& ~(page_size-1))))
end_define
...
define|\
value|{outheader.a_machtype = M_SPARC; outheader.a_toolversion = 1;}
end_define
...
directive|define
name|INITIALIZE_HEADER
value|outheader.a_machtype = M_68010
end_define
...
directive|define
name|INITIALIZE_HEADER
value|outheader.a_machtype = M_68020
end_define
...
directive|define
name|INITIALIZE_HEADER
value|N_SET_MACHTYPE (outheader, M_68020)
end_define
...
begin_comment
comment|/* ISI rel 4.0D doesn't use it, and rel 3.05 doesn't have an    a_machtype field and so won't recognize the magic number.  To keep    binary compatibility for now, just ignore it */
end_comment
...
directive|define
name|INITIALIZE_HEADER
value|N_SET_MACHTYPE (outheader, HP9000S200_ID)
end_define
...
directive|define
name|INITIALIZE_HEADER
value|N_SET_MACHTYPE (outheader, M_386)
end_define
...
begin_comment
comment|/* This enables code to take care of an ugly hack in the ISI OS.    If a symbol beings with _$, then the object file is included only    if the rest of the symbol name has been referenced. */
end_comment
