...
name|sccsid
init|=
literal|"@(#)checknr.c	4.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * All commands known to nroff, plus ms and me.  * Used so we can complain about unrecognized commands.  */
end_comment
...
name|printf
argument_list|(
literal|"Usage: nrc -s -f -a.xx.yy.xx.yy... (.xx, .yy)\n"
argument_list|)
expr_stmt|;
break|break;
block|}
...
begin_block
block|{
comment|/* First time figure out ncmds. */
if|if
condition|(
name|ncmds
operator|==
