...
condition|)
block|{
comment|/* A special hack for read access to (MEM (QUEUED ...)) 	 to facilitate use of autoincrement. 	 Make a copy of the contents of the memory location 	 rather than a copy of the address, but not 	 if the value is of mode BLKmode.  */
if|if
condition|(
name|code
operator|==
...
begin_comment
comment|/* Push a block of length SIZE (perhaps variable)    and return an rtx to address the beginning of the block.    Note that it is not possible for the value returned to be a QUEUED.    The value may be stack_pointer_rtx.     The value we return does take account of STACK_POINTER_OFFSET.  */
end_comment
...
begin_comment
comment|/* Store the value of EXP (an expression tree)    into a subfield of TARGET which has mode MODE and occupies    BITSIZE bits, starting BITPOS bits from the start of TARGET.     If VALUE_MODE is VOIDmode, return nothing in particular.    UNSIGNEDP is not used in this case.     Otherwise, return an rtx for the value stored.  This rtx    has mode VALUE_MODE if that is convenient to do.    In this case, UNSIGNEDP must be nonzero if the value is an unsigned type.     ALIGN is the alignment that TARGET is known to have, measured in bytes.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* Must mark EXP used because it might be a compiler-generated 	 variable used by a compiler-generated expression.  */
name|TREE_USED
argument_list|(
name|exp
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Exit flag is 0 for now.  */
...
condition|)
block|{
comment|/* Looks like a `while-do' loop.  */
...
block|{
comment|/* Looks like a `do-while' loop.  */
...
argument_list|)
expr_stmt|;
comment|/* Have to initialize these vars whereever this LET_STMT 	       is expanded.  */
name|expand_decl_init
argument_list|(
name|vars
argument_list|)
...
argument_list|)
control|)
comment|/* TYPE_DECLs do not have rtl.  */
if|if
condition|(
name|DECL_RTL
argument_list|(
...
comment|/* If an arm of the branch requires a cleanup, that 	   only that cleanup is performed.  */
name|tree
name|old_cleanups
init|=
name|cleanups_this_call
...
name|sorry
argument_list|(
literal|"aggreage value in COND_EXPR"
argument_list|)
expr_stmt|;
name|cleanups_this_call
operator|=
...
case|:
block|{
comment|/* Something needs to be initialized, but we didn't know 	   where that thing was when building the tree.  For example, 	   it could be the return value of a function, or a parameter 	   to a function which lays down in the stack, or a temporary 	   variable which must be passed by reference.  */
name|tree
name|slot
init|=
name|TREE_OPERAND
...
expr_stmt|;
block|}
comment|/* Run the initialization expression now with the target.  */
return|return
name|expand_expr
argument_list|(
name|TREE_OPERAND
...
argument_list|)
expr_stmt|;
comment|/* Check for |= or&= of a bitfield of size one into another bitfield 	   of size 1.  In this case, (unless we need the result of the 	   assignment) we can do this more efficiently with a 	   test followed by an assignment, if necessary.  */
if|if
condition|(
name|ignore
operator|&&
...
name|saving_insns
decl_stmt|,
name|last_saving_insn
decl_stmt|;
comment|/* If there are parameters which must be saved from this function, 	   save them now.  */
name|saving_insns
operator|=
...
name|is_setjmp
decl_stmt|;
comment|/* Nonzero if this is a call to longjmp or a related function.  */
name|int
name|is_longjmp
decl_stmt|;
...
condition|)
block|{
comment|/* May still be a `const' function if it is 	     a call through a const function.  */
name|fndecl
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* In case this function later becomes inlineable, 		 record that there was already a non-inline call to it.  		 Use abstraction instead of setting TREE_ADDRESSABLE 		 directly.  */
if|if
condition|(
name|TREE_INLINE
argument_list|(
...
name|fndecl
argument_list|,
literal|"can't inline call to `%s' which was declared inline"
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* Perform all cleanups needed for the arguments of this call 	     (i.e. destructors in C++).  It is ok if these destructors 	     clobber RETURN_VALUE_REG, because the only time we care about 	     this is when TARGET is that register.  But in C++, we take 	     care to never return that register directly.  */
name|expand_cleanups_to
argument_list|(
name|old_cleanups
argument_list|)
...
return|;
block|}
comment|/* If inlining failed, mark FNDECL as needing to be compiled 	 separately after all.  	 Use abstraction instead of setting TREE_ADDRESSABLE directly.  */
name|mark_addressable
argument_list|(
name|fndecl
argument_list|)
...
expr_stmt|;
block|}
comment|/* Don't let pending stack adjusts add up to too much.      Also, do all pending adjustments now      if there is any chance this might be a call to alloca (or longjmp).  */
if|if
condition|(
name|pending_stack_adjust
operator|>=
...
argument_list|)
expr_stmt|;
comment|/* Once this space is used, we cannot give other calls 	     (which might be part of the args to this call) 	     access to this space.  This is because there is 	     no way to say that we are building arguments 	     in a place which should not be deallocated by 	     `emit_call_1', which, by the way, is the way this 	     this space gets deallocated.  */
name|do_pending_stack_adjust
argument_list|()
expr_stmt|;
block|}
...
name|stack_offset
decl_stmt|;
comment|/* Hack for C++: see assign_parms for symmetric code.  */
name|int
name|extra
init|=
literal|0
