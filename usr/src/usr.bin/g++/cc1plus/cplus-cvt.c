...
begin_comment
comment|/* Language-level data type conversion for GNU C++.    Copyright (C) 1987, 1988 Free Software Foundation, Inc.    Hacked by Michael Tiemann (tiemann@mcc.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* We are passing something to a function which requires a reference.    The type we are interested in is in TYPE. The initial    value we have to begin with is in ARG.     FLAGS controls how we manage visibility checking.  */
end_comment
...
directive|if
literal|0
block|goto done_but_maybe_warn;
else|#
directive|else
goto|goto
name|done
...
name|error
argument_list|(
literal|"cannot convert to a reference type"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
directive|if
literal|0
block|if (TREE_CODE (val) == REFERENCE_EXPR)     {       val = build1 (NOP_EXPR, build_pointer_type (TREE_TYPE (type)), 		    TREE_OPERAND (val, 0));       return val;     }
endif|#
directive|endif
if|if
condition|(
...
argument_list|)
decl_stmt|;
comment|/* This function shouldn't be called with 	 unqualified arguments.  */
name|assert
argument_list|(
name|distance
operator|>=
...
argument_list|)
expr_stmt|;
comment|/* Conversion between aggregate types.  New C++ semantics allow 	 objects of derived type to be cast to objects of base type. 	 Old semantics only allowed this bwteen pointers.  	 There may be some ambiguity between using a constructor 	 vs. using a type conversion operator when both apply.  */
if|if
condition|(
name|IS_AGGR_TYPE
argument_list|(
...
directive|if
literal|0
block|if (TYPE_VIRTUAL_P (type)) 		warning ("assignment to virtual aggregate type");
endif|#
directive|endif
return|return
name|build
...
block|}
block|}
comment|/* Use the longer or shorter conversion that is appropriate.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"type conversion nonexistant for type `%s'"
argument_list|,
name|IDENTIFIER_POINTER
argument_list|(
name|DECL_NAME
...
name|error
argument_list|(
literal|"type conversion nonexistant for non-C++ type"
argument_list|)
expr_stmt|;
return|return
literal|0
