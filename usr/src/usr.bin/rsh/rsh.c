begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1983, 1990, 1993  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1983, 1990, 1993\n\ 	The Regents of the University of California.  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)rsh.c	8.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_comment
comment|/*  * $Source: mit/rsh/RCS/rsh.c,v $  * $Header: mit/rsh/RCS/rsh.c,v 5.1 89/07/31 19:28:59 kfall Exp Locker: kfall $  */
end_comment

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/signal.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_include
include|#
directive|include
file|<pwd.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<varargs.h>
end_include

begin_include
include|#
directive|include
file|"pathnames.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|KERBEROS
end_ifdef

begin_include
include|#
directive|include
file|<kerberosIV/des.h>
end_include

begin_include
include|#
directive|include
file|<kerberosIV/krb.h>
end_include

begin_decl_stmt
name|CREDENTIALS
name|cred
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|Key_schedule
name|schedule
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|use_kerberos
init|=
literal|1
decl_stmt|,
name|doencrypt
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|dst_realm_buf
index|[
name|REALM_SZ
index|]
decl_stmt|,
modifier|*
name|dest_realm
decl_stmt|;
end_decl_stmt

begin_function_decl
specifier|extern
name|char
modifier|*
name|krb_realmofhost
parameter_list|()
function_decl|;
end_function_decl

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * rsh - remote shell  */
end_comment

begin_decl_stmt
specifier|extern
name|int
name|errno
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|rfd2
decl_stmt|;
end_decl_stmt

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
block|{
specifier|extern
name|char
modifier|*
name|optarg
decl_stmt|;
specifier|extern
name|int
name|optind
decl_stmt|;
name|struct
name|passwd
modifier|*
name|pw
decl_stmt|;
name|struct
name|servent
modifier|*
name|sp
decl_stmt|;
name|struct
name|hostent
modifier|*
name|hp
decl_stmt|;
name|long
name|omask
decl_stmt|;
name|int
name|argoff
decl_stmt|,
name|asrsh
decl_stmt|,
name|ch
decl_stmt|,
name|dflag
decl_stmt|,
name|nflag
decl_stmt|,
name|one
decl_stmt|,
name|pid
decl_stmt|,
name|rem
decl_stmt|,
name|uid
decl_stmt|;
specifier|register
name|char
modifier|*
name|p
decl_stmt|;
name|char
modifier|*
name|args
decl_stmt|,
modifier|*
name|host
decl_stmt|,
modifier|*
name|user
decl_stmt|,
modifier|*
name|copyargs
argument_list|()
decl_stmt|;
name|void
name|sendsig
parameter_list|()
function_decl|;
name|argoff
operator|=
name|asrsh
operator|=
name|dflag
operator|=
name|nflag
operator|=
literal|0
expr_stmt|;
name|one
operator|=
literal|1
expr_stmt|;
name|host
operator|=
name|user
operator|=
name|NULL
expr_stmt|;
comment|/* if called as something other than "rsh", use it as the host name */
if|if
condition|(
name|p
operator|=
name|rindex
argument_list|(
name|argv
index|[
literal|0
index|]
argument_list|,
literal|'/'
argument_list|)
condition|)
operator|++
name|p
expr_stmt|;
else|else
name|p
operator|=
name|argv
index|[
literal|0
index|]
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|p
argument_list|,
literal|"rsh"
argument_list|)
condition|)
name|host
operator|=
name|p
expr_stmt|;
else|else
name|asrsh
operator|=
literal|1
expr_stmt|;
comment|/* handle "rsh host flags" */
if|if
condition|(
operator|!
name|host
operator|&&
name|argc
operator|>
literal|2
operator|&&
name|argv
index|[
literal|1
index|]
index|[
literal|0
index|]
operator|!=
literal|'-'
condition|)
block|{
name|host
operator|=
name|argv
index|[
literal|1
index|]
expr_stmt|;
name|argoff
operator|=
literal|1
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|KERBEROS
define|#
directive|define
name|OPTIONS
value|"8KLdek:l:nw"
else|#
directive|else
define|#
directive|define
name|OPTIONS
value|"8KLdel:nw"
endif|#
directive|endif
while|while
condition|(
operator|(
name|ch
operator|=
name|getopt
argument_list|(
name|argc
operator|-
name|argoff
argument_list|,
name|argv
operator|+
name|argoff
argument_list|,
name|OPTIONS
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|ch
condition|)
block|{
case|case
literal|'K'
case|:
ifdef|#
directive|ifdef
name|KERBEROS
name|use_kerberos
operator|=
literal|0
expr_stmt|;
endif|#
directive|endif
break|break;
case|case
literal|'L'
case|:
comment|/* -8Lew are ignored to allow rlogin aliases */
case|case
literal|'e'
case|:
case|case
literal|'w'
case|:
case|case
literal|'8'
case|:
break|break;
case|case
literal|'d'
case|:
name|dflag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'l'
case|:
name|user
operator|=
name|optarg
expr_stmt|;
break|break;
ifdef|#
directive|ifdef
name|KERBEROS
case|case
literal|'k'
case|:
name|dest_realm
operator|=
name|dst_realm_buf
expr_stmt|;
name|strncpy
argument_list|(
name|dest_realm
argument_list|,
name|optarg
argument_list|,
name|REALM_SZ
argument_list|)
expr_stmt|;
break|break;
endif|#
directive|endif
case|case
literal|'n'
case|:
name|nflag
operator|=
literal|1
expr_stmt|;
break|break;
ifdef|#
directive|ifdef
name|KERBEROS
endif|#
directive|endif
case|case
literal|'?'
case|:
default|default:
name|usage
argument_list|()
expr_stmt|;
block|}
name|optind
operator|+=
name|argoff
expr_stmt|;
comment|/* if haven't gotten a host yet, do so */
if|if
condition|(
operator|!
name|host
operator|&&
operator|!
operator|(
name|host
operator|=
name|argv
index|[
name|optind
operator|++
index|]
operator|)
condition|)
name|usage
argument_list|()
expr_stmt|;
comment|/* if no further arguments, must have been called as rlogin. */
if|if
condition|(
operator|!
name|argv
index|[
name|optind
index|]
condition|)
block|{
if|if
condition|(
name|asrsh
condition|)
operator|*
name|argv
operator|=
literal|"rlogin"
expr_stmt|;
name|execv
argument_list|(
name|_PATH_RLOGIN
argument_list|,
name|argv
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: can't exec %s.\n"
argument_list|,
name|_PATH_RLOGIN
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|argc
operator|-=
name|optind
expr_stmt|;
name|argv
operator|+=
name|optind
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|pw
operator|=
name|getpwuid
argument_list|(
name|uid
operator|=
name|getuid
argument_list|()
argument_list|)
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: unknown user id.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|user
condition|)
name|user
operator|=
name|pw
operator|->
name|pw_name
expr_stmt|;
ifdef|#
directive|ifdef
name|KERBEROS
endif|#
directive|endif
name|args
operator|=
name|copyargs
argument_list|(
name|argv
argument_list|)
expr_stmt|;
name|sp
operator|=
name|NULL
expr_stmt|;
ifdef|#
directive|ifdef
name|KERBEROS
if|if
condition|(
name|use_kerberos
condition|)
block|{
name|sp
operator|=
name|getservbyname
argument_list|(
operator|(
name|doencrypt
condition|?
literal|"ekshell"
else|:
literal|"kshell"
operator|)
argument_list|,
literal|"tcp"
argument_list|)
expr_stmt|;
if|if
condition|(
name|sp
operator|==
name|NULL
condition|)
block|{
name|use_kerberos
operator|=
literal|0
expr_stmt|;
name|warning
argument_list|(
literal|"can't get entry for %s/tcp service"
argument_list|,
name|doencrypt
condition|?
literal|"ekshell"
else|:
literal|"kshell"
argument_list|)
expr_stmt|;
block|}
block|}
endif|#
directive|endif
if|if
condition|(
name|sp
operator|==
name|NULL
condition|)
name|sp
operator|=
name|getservbyname
argument_list|(
literal|"shell"
argument_list|,
literal|"tcp"
argument_list|)
expr_stmt|;
if|if
condition|(
name|sp
operator|==
name|NULL
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: shell/tcp: unknown service.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|KERBEROS
name|try_connect
label|:
if|if
condition|(
name|use_kerberos
condition|)
block|{
comment|/* fully qualify hostname (needed for krb_realmofhost) */
name|hp
operator|=
name|gethostbyname
argument_list|(
name|host
argument_list|)
expr_stmt|;
if|if
condition|(
name|hp
operator|!=
name|NULL
operator|&&
operator|!
operator|(
name|host
operator|=
name|strdup
argument_list|(
name|hp
operator|->
name|h_name
argument_list|)
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|ENOMEM
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|rem
operator|=
name|KSUCCESS
expr_stmt|;
name|errno
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|dest_realm
operator|==
name|NULL
condition|)
name|dest_realm
operator|=
name|krb_realmofhost
argument_list|(
name|host
argument_list|)
expr_stmt|;
name|rem
operator|=
name|krcmd
argument_list|(
operator|&
name|host
argument_list|,
name|sp
operator|->
name|s_port
argument_list|,
name|user
argument_list|,
name|args
argument_list|,
operator|&
name|rfd2
argument_list|,
name|dest_realm
argument_list|)
expr_stmt|;
if|if
condition|(
name|rem
operator|<
literal|0
condition|)
block|{
name|use_kerberos
operator|=
literal|0
expr_stmt|;
name|sp
operator|=
name|getservbyname
argument_list|(
literal|"shell"
argument_list|,
literal|"tcp"
argument_list|)
expr_stmt|;
if|if
condition|(
name|sp
operator|==
name|NULL
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: unknown service shell/tcp.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|errno
operator|==
name|ECONNREFUSED
condition|)
name|warning
argument_list|(
literal|"remote host doesn't support Kerberos"
argument_list|)
expr_stmt|;
if|if
condition|(
name|errno
operator|==
name|ENOENT
condition|)
name|warning
argument_list|(
literal|"can't provide Kerberos auth data"
argument_list|)
expr_stmt|;
goto|goto
name|try_connect
goto|;
block|}
block|}
else|else
block|{
if|if
condition|(
name|doencrypt
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: the -x flag requires Kerberos authentication.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|rem
operator|=
name|rcmd
argument_list|(
operator|&
name|host
argument_list|,
name|sp
operator|->
name|s_port
argument_list|,
name|pw
operator|->
name|pw_name
argument_list|,
name|user
argument_list|,
name|args
argument_list|,
operator|&
name|rfd2
argument_list|)
expr_stmt|;
block|}
else|#
directive|else
name|rem
operator|=
name|rcmd
argument_list|(
operator|&
name|host
argument_list|,
name|sp
operator|->
name|s_port
argument_list|,
name|pw
operator|->
name|pw_name
argument_list|,
name|user
argument_list|,
name|args
argument_list|,
operator|&
name|rfd2
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|rem
operator|<
literal|0
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|rfd2
operator|<
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: can't establish stderr.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|dflag
condition|)
block|{
if|if
condition|(
name|setsockopt
argument_list|(
name|rem
argument_list|,
name|SOL_SOCKET
argument_list|,
name|SO_DEBUG
argument_list|,
operator|&
name|one
argument_list|,
sizeof|sizeof
argument_list|(
name|one
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: setsockopt: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|setsockopt
argument_list|(
name|rfd2
argument_list|,
name|SOL_SOCKET
argument_list|,
name|SO_DEBUG
argument_list|,
operator|&
name|one
argument_list|,
sizeof|sizeof
argument_list|(
name|one
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: setsockopt: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|setuid
argument_list|(
name|uid
argument_list|)
expr_stmt|;
name|omask
operator|=
name|sigblock
argument_list|(
name|sigmask
argument_list|(
name|SIGINT
argument_list|)
operator||
name|sigmask
argument_list|(
name|SIGQUIT
argument_list|)
operator||
name|sigmask
argument_list|(
name|SIGTERM
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|SIG_IGN
argument_list|)
operator|!=
name|SIG_IGN
condition|)
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|sendsig
argument_list|)
expr_stmt|;
if|if
condition|(
name|signal
argument_list|(
name|SIGQUIT
argument_list|,
name|SIG_IGN
argument_list|)
operator|!=
name|SIG_IGN
condition|)
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGQUIT
argument_list|,
name|sendsig
argument_list|)
expr_stmt|;
if|if
condition|(
name|signal
argument_list|(
name|SIGTERM
argument_list|,
name|SIG_IGN
argument_list|)
operator|!=
name|SIG_IGN
condition|)
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGTERM
argument_list|,
name|sendsig
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|nflag
condition|)
block|{
name|pid
operator|=
name|fork
argument_list|()
expr_stmt|;
if|if
condition|(
name|pid
operator|<
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: fork: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
ifdef|#
directive|ifdef
name|KERBEROS
endif|#
directive|endif
block|{
operator|(
name|void
operator|)
name|ioctl
argument_list|(
name|rfd2
argument_list|,
name|FIONBIO
argument_list|,
operator|&
name|one
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|ioctl
argument_list|(
name|rem
argument_list|,
name|FIONBIO
argument_list|,
operator|&
name|one
argument_list|)
expr_stmt|;
block|}
name|talk
argument_list|(
name|nflag
argument_list|,
name|omask
argument_list|,
name|pid
argument_list|,
name|rem
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|nflag
condition|)
operator|(
name|void
operator|)
name|kill
argument_list|(
name|pid
argument_list|,
name|SIGKILL
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_macro
name|talk
argument_list|(
argument|nflag
argument_list|,
argument|omask
argument_list|,
argument|pid
argument_list|,
argument|rem
argument_list|)
end_macro

begin_decl_stmt
name|int
name|nflag
decl_stmt|,
name|pid
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
name|omask
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|int
name|rem
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|cc
decl_stmt|,
name|wc
decl_stmt|;
specifier|register
name|char
modifier|*
name|bp
decl_stmt|;
name|int
name|readfrom
decl_stmt|,
name|ready
decl_stmt|,
name|rembits
decl_stmt|;
name|char
name|buf
index|[
name|BUFSIZ
index|]
decl_stmt|;
if|if
condition|(
operator|!
name|nflag
operator|&&
name|pid
operator|==
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|close
argument_list|(
name|rfd2
argument_list|)
expr_stmt|;
name|reread
label|:
name|errno
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|(
name|cc
operator|=
name|read
argument_list|(
literal|0
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
name|buf
argument_list|)
operator|)
operator|<=
literal|0
condition|)
goto|goto
name|done
goto|;
name|bp
operator|=
name|buf
expr_stmt|;
name|rewrite
label|:
name|rembits
operator|=
literal|1
operator|<<
name|rem
expr_stmt|;
if|if
condition|(
name|select
argument_list|(
literal|16
argument_list|,
literal|0
argument_list|,
operator|&
name|rembits
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|!=
name|EINTR
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: select: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
goto|goto
name|rewrite
goto|;
block|}
if|if
condition|(
operator|(
name|rembits
operator|&
operator|(
literal|1
operator|<<
name|rem
operator|)
operator|)
operator|==
literal|0
condition|)
goto|goto
name|rewrite
goto|;
ifdef|#
directive|ifdef
name|KERBEROS
endif|#
directive|endif
name|wc
operator|=
name|write
argument_list|(
name|rem
argument_list|,
name|bp
argument_list|,
name|cc
argument_list|)
expr_stmt|;
if|if
condition|(
name|wc
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|==
name|EWOULDBLOCK
condition|)
goto|goto
name|rewrite
goto|;
goto|goto
name|done
goto|;
block|}
name|bp
operator|+=
name|wc
expr_stmt|;
name|cc
operator|-=
name|wc
expr_stmt|;
if|if
condition|(
name|cc
operator|==
literal|0
condition|)
goto|goto
name|reread
goto|;
goto|goto
name|rewrite
goto|;
name|done
label|:
operator|(
name|void
operator|)
name|shutdown
argument_list|(
name|rem
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|sigsetmask
argument_list|(
name|omask
argument_list|)
expr_stmt|;
name|readfrom
operator|=
operator|(
literal|1
operator|<<
name|rfd2
operator|)
operator||
operator|(
literal|1
operator|<<
name|rem
operator|)
expr_stmt|;
do|do
block|{
name|ready
operator|=
name|readfrom
expr_stmt|;
if|if
condition|(
name|select
argument_list|(
literal|16
argument_list|,
operator|&
name|ready
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|!=
name|EINTR
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: select: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
continue|continue;
block|}
if|if
condition|(
name|ready
operator|&
operator|(
literal|1
operator|<<
name|rfd2
operator|)
condition|)
block|{
name|errno
operator|=
literal|0
expr_stmt|;
ifdef|#
directive|ifdef
name|KERBEROS
endif|#
directive|endif
name|cc
operator|=
name|read
argument_list|(
name|rfd2
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
name|buf
argument_list|)
expr_stmt|;
if|if
condition|(
name|cc
operator|<=
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|!=
name|EWOULDBLOCK
condition|)
name|readfrom
operator|&=
operator|~
operator|(
literal|1
operator|<<
name|rfd2
operator|)
expr_stmt|;
block|}
else|else
operator|(
name|void
operator|)
name|write
argument_list|(
literal|2
argument_list|,
name|buf
argument_list|,
name|cc
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|ready
operator|&
operator|(
literal|1
operator|<<
name|rem
operator|)
condition|)
block|{
name|errno
operator|=
literal|0
expr_stmt|;
ifdef|#
directive|ifdef
name|KERBEROS
endif|#
directive|endif
name|cc
operator|=
name|read
argument_list|(
name|rem
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
name|buf
argument_list|)
expr_stmt|;
if|if
condition|(
name|cc
operator|<=
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|!=
name|EWOULDBLOCK
condition|)
name|readfrom
operator|&=
operator|~
operator|(
literal|1
operator|<<
name|rem
operator|)
expr_stmt|;
block|}
else|else
operator|(
name|void
operator|)
name|write
argument_list|(
literal|1
argument_list|,
name|buf
argument_list|,
name|cc
argument_list|)
expr_stmt|;
block|}
block|}
do|while
condition|(
name|readfrom
condition|)
do|;
block|}
end_block

begin_function
name|void
name|sendsig
parameter_list|(
name|signo
parameter_list|)
name|char
name|signo
decl_stmt|;
block|{
ifdef|#
directive|ifdef
name|KERBEROS
endif|#
directive|endif
operator|(
name|void
operator|)
name|write
argument_list|(
name|rfd2
argument_list|,
operator|&
name|signo
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|KERBEROS
end_ifdef

begin_comment
comment|/* VARARGS */
end_comment

begin_macro
name|warning
argument_list|(
argument|va_alist
argument_list|)
end_macro

begin_macro
name|va_dcl
end_macro

begin_block
block|{
name|va_list
name|ap
decl_stmt|;
name|char
modifier|*
name|fmt
decl_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: warning, using standard rsh: "
argument_list|)
expr_stmt|;
name|va_start
argument_list|(
name|ap
argument_list|)
expr_stmt|;
name|fmt
operator|=
name|va_arg
argument_list|(
name|ap
argument_list|,
name|char
operator|*
argument_list|)
expr_stmt|;
name|vfprintf
argument_list|(
name|stderr
argument_list|,
name|fmt
argument_list|,
name|ap
argument_list|)
expr_stmt|;
name|va_end
argument_list|(
name|ap
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|".\n"
argument_list|)
expr_stmt|;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_function
name|char
modifier|*
name|copyargs
parameter_list|(
name|argv
parameter_list|)
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
block|{
specifier|register
name|int
name|cc
decl_stmt|;
specifier|register
name|char
modifier|*
modifier|*
name|ap
decl_stmt|,
modifier|*
name|p
decl_stmt|;
name|char
modifier|*
name|args
decl_stmt|,
modifier|*
name|malloc
argument_list|()
decl_stmt|;
name|cc
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|ap
operator|=
name|argv
init|;
operator|*
name|ap
condition|;
operator|++
name|ap
control|)
name|cc
operator|+=
name|strlen
argument_list|(
operator|*
name|ap
argument_list|)
operator|+
literal|1
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|args
operator|=
name|malloc
argument_list|(
operator|(
name|u_int
operator|)
name|cc
argument_list|)
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"rsh: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|ENOMEM
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
for|for
control|(
name|p
operator|=
name|args
operator|,
name|ap
operator|=
name|argv
init|;
operator|*
name|ap
condition|;
operator|++
name|ap
control|)
block|{
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|p
argument_list|,
operator|*
name|ap
argument_list|)
expr_stmt|;
for|for
control|(
name|p
operator|=
name|strcpy
argument_list|(
name|p
argument_list|,
operator|*
name|ap
argument_list|)
init|;
operator|*
name|p
condition|;
operator|++
name|p
control|)
empty_stmt|;
if|if
condition|(
name|ap
index|[
literal|1
index|]
condition|)
operator|*
name|p
operator|++
operator|=
literal|' '
expr_stmt|;
block|}
return|return
operator|(
name|args
operator|)
return|;
block|}
end_function

begin_macro
name|usage
argument_list|()
end_macro

begin_block
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: rsh [-nd%s]%s[-l login] host [command]\n"
argument_list|,
ifdef|#
directive|ifdef
name|KERBEROS
literal|""
argument_list|,
literal|" [-k realm] "
argument_list|)
expr_stmt|;
else|#
directive|else
literal|""
operator|,
literal|" "
block|)
end_block

begin_empty_stmt
empty_stmt|;
end_empty_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_expr_stmt
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
end_expr_stmt

unit|}
end_unit

