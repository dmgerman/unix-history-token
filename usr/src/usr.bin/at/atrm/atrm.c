...
begin_comment
comment|/*  * Remove an entry from the queue. The access of the file is checked for  * write permission (since all jobs are mode 644). If access is granted,  * unlink the file. If the fflag (suppress announcements) is not set,  * print the job number that we are removing and the result of the access  * check (either "permission denied" or "removed"). If the super-user is  * removing jobs, inform him/her who owns each file after it is removed.  * If we are running interactively (iflag), prompt the user before we   * unlink the file.  */
end_comment
...
parameter_list|()
function_decl|;
comment|/* get a user name using a uid */
if|if
condition|(
operator|!
name|fflag
...
name|printf
argument_list|(
literal|"unlink: Permission denied.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
