begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1980 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)dumptape.c	1.3 (UKC) %G%	5.8 (Berkeley) 2/23/87"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
endif|not lint
end_endif

begin_include
include|#
directive|include
file|"dump.h"
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/mtio.h>
end_include

begin_expr_stmt
name|char
argument_list|(
operator|*
name|tblock
argument_list|)
index|[
name|TP_BSIZE
index|]
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* pointer to malloc()ed buffer for tape */
end_comment

begin_decl_stmt
name|int
name|writesize
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* size of malloc()ed buffer for tape */
end_comment

begin_decl_stmt
name|long
name|lastspclrec
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* tape block number of last written header */
end_comment

begin_decl_stmt
name|int
name|trecno
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* next record to write in current block */
end_comment

begin_decl_stmt
specifier|extern
name|int
name|ntrec
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* blocking factor on tape */
end_comment

begin_decl_stmt
specifier|extern
name|int
name|cartridge
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|int
name|rewindoffline
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|int
name|read
argument_list|()
decl_stmt|,
name|write
argument_list|()
decl_stmt|;
end_decl_stmt

begin_ifdef
ifdef|#
directive|ifdef
name|RDUMP
end_ifdef

begin_decl_stmt
specifier|extern
name|char
modifier|*
name|host
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
endif|RDUMP
end_endif

begin_comment
comment|/*  * Concurrent dump mods (Caltech) - disk block reading and tape writing  * are exported to several slave processes.  While one slave writes the  * tape, the others read disk blocks; they pass control of the tape in  * a ring via flock().	The parent process traverses the filesystem and  * sends spclrec()'s and lists of daddr's to the slaves via pipes.  */
end_comment

begin_struct
struct|struct
name|req
block|{
comment|/* instruction packets sent to slaves */
name|daddr_t
name|dblk
decl_stmt|;
name|int
name|count
decl_stmt|;
block|}
modifier|*
name|req
struct|;
end_struct

begin_decl_stmt
name|int
name|reqsiz
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|SLAVES
value|3
end_define

begin_comment
comment|/* 1 slave writing, 1 reading, 1 for slack */
end_comment

begin_decl_stmt
name|int
name|slavefd
index|[
name|SLAVES
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* pipes from master to each slave */
end_comment

begin_decl_stmt
name|int
name|slavepid
index|[
name|SLAVES
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* used by killall() */
end_comment

begin_decl_stmt
name|int
name|rotor
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* next slave to be instructed */
end_comment

begin_decl_stmt
name|int
name|master
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* pid of master, for sending error signals */
end_comment

begin_decl_stmt
name|int
name|tenths
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* length of tape used per block written */
end_comment

begin_macro
name|alloctape
argument_list|()
end_macro

begin_block
block|{
name|int
name|pgoff
init|=
name|getpagesize
argument_list|()
operator|-
literal|1
decl_stmt|;
name|writesize
operator|=
name|ntrec
operator|*
name|TP_BSIZE
expr_stmt|;
name|reqsiz
operator|=
name|ntrec
operator|*
sizeof|sizeof
argument_list|(
expr|struct
name|req
argument_list|)
expr_stmt|;
comment|/* 	 * CDC 92181's and 92185's make 0.8" gaps in 1600-bpi start/stop mode 	 * (see DEC TU80 User's Guide).  The shorter gaps of 6250-bpi require 	 * repositioning after stopping, i.e, streaming mode, where the gap is 	 * variable, 0.30" to 0.45".  The gap is maximal when the tape stops. 	 */
name|tenths
operator|=
name|writesize
operator|/
name|density
operator|+
operator|(
name|cartridge
condition|?
literal|16
else|:
name|density
operator|==
literal|625
condition|?
literal|5
else|:
literal|8
operator|)
expr_stmt|;
comment|/* 	 * Allocate tape buffer contiguous with the array of instruction 	 * packets, so flusht() can write them together with one write(). 	 * Align tape buffer on page boundary to speed up tape write(). 	 */
name|req
operator|=
operator|(
expr|struct
name|req
operator|*
operator|)
name|malloc
argument_list|(
name|reqsiz
operator|+
name|writesize
operator|+
name|pgoff
argument_list|)
expr_stmt|;
if|if
condition|(
name|req
operator|==
name|NULL
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|tblock
operator|=
operator|(
name|char
argument_list|(
operator|*
argument_list|)
index|[
name|TP_BSIZE
index|]
operator|)
operator|(
operator|(
operator|(
name|long
operator|)
operator|&
name|req
index|[
name|ntrec
index|]
operator|+
name|pgoff
operator|)
operator|&
operator|~
name|pgoff
operator|)
expr_stmt|;
name|req
operator|=
operator|(
expr|struct
name|req
operator|*
operator|)
name|tblock
operator|-
name|ntrec
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_macro
name|taprec
argument_list|(
argument|dp
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|dp
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|req
index|[
name|trecno
index|]
operator|.
name|dblk
operator|=
operator|(
name|daddr_t
operator|)
literal|0
expr_stmt|;
name|req
index|[
name|trecno
index|]
operator|.
name|count
operator|=
literal|1
expr_stmt|;
operator|*
operator|(
expr|union
name|u_spcl
operator|*
operator|)
operator|(
operator|*
name|tblock
operator|++
operator|)
operator|=
operator|*
operator|(
expr|union
name|u_spcl
operator|*
operator|)
name|dp
expr_stmt|;
comment|/* movc3 */
name|lastspclrec
operator|=
name|spcl
operator|.
name|c_tapea
expr_stmt|;
name|trecno
operator|++
expr_stmt|;
name|spcl
operator|.
name|c_tapea
operator|++
expr_stmt|;
if|if
condition|(
name|trecno
operator|>=
name|ntrec
condition|)
name|flusht
argument_list|()
expr_stmt|;
block|}
end_block

begin_macro
name|dmpblk
argument_list|(
argument|blkno
argument_list|,
argument|size
argument_list|)
end_macro

begin_decl_stmt
name|daddr_t
name|blkno
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|size
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|avail
decl_stmt|,
name|tpblks
decl_stmt|,
name|dblkno
decl_stmt|;
name|dblkno
operator|=
name|fsbtodb
argument_list|(
name|sblock
argument_list|,
name|blkno
argument_list|)
expr_stmt|;
name|tpblks
operator|=
name|size
operator|/
name|TP_BSIZE
expr_stmt|;
while|while
condition|(
operator|(
name|avail
operator|=
name|MIN
argument_list|(
name|tpblks
argument_list|,
name|ntrec
operator|-
name|trecno
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|req
index|[
name|trecno
index|]
operator|.
name|dblk
operator|=
name|dblkno
expr_stmt|;
name|req
index|[
name|trecno
index|]
operator|.
name|count
operator|=
name|avail
expr_stmt|;
name|trecno
operator|+=
name|avail
expr_stmt|;
name|spcl
operator|.
name|c_tapea
operator|+=
name|avail
expr_stmt|;
if|if
condition|(
name|trecno
operator|>=
name|ntrec
condition|)
name|flusht
argument_list|()
expr_stmt|;
name|dblkno
operator|+=
name|avail
operator|*
operator|(
name|TP_BSIZE
operator|/
name|DEV_BSIZE
operator|)
expr_stmt|;
name|tpblks
operator|-=
name|avail
expr_stmt|;
block|}
block|}
end_block

begin_decl_stmt
name|int
name|nogripe
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_macro
name|tperror
argument_list|()
end_macro

begin_block
block|{
if|if
condition|(
name|pipeout
condition|)
block|{
name|msg
argument_list|(
literal|"Tape write error on %s\n"
argument_list|,
name|tape
argument_list|)
expr_stmt|;
name|msg
argument_list|(
literal|"Cannot recover\n"
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
comment|/* NOTREACHED */
block|}
name|msg
argument_list|(
literal|"Tape write error %d feet into tape %d\n"
argument_list|,
name|asize
operator|/
literal|120L
argument_list|,
name|tapeno
argument_list|)
expr_stmt|;
name|broadcast
argument_list|(
literal|"TAPE ERROR!\n"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|query
argument_list|(
literal|"Do you want to restart?"
argument_list|)
condition|)
name|dumpabort
argument_list|()
expr_stmt|;
name|msg
argument_list|(
literal|"This tape will rewind.  After it is rewound,\n"
argument_list|)
expr_stmt|;
name|msg
argument_list|(
literal|"replace the faulty tape with a new one;\n"
argument_list|)
expr_stmt|;
name|msg
argument_list|(
literal|"this dump volume will be rewritten.\n"
argument_list|)
expr_stmt|;
name|killall
argument_list|()
expr_stmt|;
name|nogripe
operator|=
literal|1
expr_stmt|;
name|close_rewind
argument_list|()
expr_stmt|;
name|Exit
argument_list|(
name|X_REWRITE
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|sigpipe
argument_list|()
end_macro

begin_block
block|{
name|msg
argument_list|(
literal|"Broken pipe\n"
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
end_block

begin_ifdef
ifdef|#
directive|ifdef
name|RDUMP
end_ifdef

begin_comment
comment|/*  * compatibility routine  */
end_comment

begin_macro
name|tflush
argument_list|(
argument|i
argument_list|)
end_macro

begin_decl_stmt
name|int
name|i
decl_stmt|;
end_decl_stmt

begin_block
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|ntrec
condition|;
name|i
operator|++
control|)
name|spclrec
argument_list|()
expr_stmt|;
block|}
end_block

begin_endif
endif|#
directive|endif
endif|RDUMP
end_endif

begin_macro
name|flusht
argument_list|()
end_macro

begin_block
block|{
name|int
name|siz
init|=
operator|(
name|char
operator|*
operator|)
name|tblock
operator|-
operator|(
name|char
operator|*
operator|)
name|req
decl_stmt|;
if|if
condition|(
name|atomic
argument_list|(
name|write
argument_list|,
name|slavefd
index|[
name|rotor
index|]
argument_list|,
name|req
argument_list|,
name|siz
argument_list|)
operator|!=
name|siz
condition|)
block|{
name|perror
argument_list|(
literal|"  DUMP: error writing command pipe"
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
operator|++
name|rotor
operator|>=
name|SLAVES
condition|)
name|rotor
operator|=
literal|0
expr_stmt|;
name|tblock
operator|=
operator|(
name|char
argument_list|(
operator|*
argument_list|)
index|[
name|TP_BSIZE
index|]
operator|)
operator|&
name|req
index|[
name|ntrec
index|]
expr_stmt|;
name|trecno
operator|=
literal|0
expr_stmt|;
name|asize
operator|+=
name|tenths
expr_stmt|;
name|blockswritten
operator|+=
name|ntrec
expr_stmt|;
if|if
condition|(
operator|!
name|pipeout
operator|&&
name|asize
operator|>
name|tsize
condition|)
block|{
name|close_rewind
argument_list|()
expr_stmt|;
name|otape
argument_list|()
expr_stmt|;
block|}
name|timeest
argument_list|()
expr_stmt|;
block|}
end_block

begin_macro
name|rewind
argument_list|()
end_macro

begin_block
block|{
name|int
name|f
decl_stmt|;
if|if
condition|(
name|pipeout
condition|)
return|return;
for|for
control|(
name|f
operator|=
literal|0
init|;
name|f
operator|<
name|SLAVES
condition|;
name|f
operator|++
control|)
name|close
argument_list|(
name|slavefd
index|[
name|f
index|]
argument_list|)
expr_stmt|;
while|while
condition|(
name|wait
argument_list|(
name|NULL
argument_list|)
operator|>=
literal|0
condition|)
empty_stmt|;
comment|/* wait for any signals from slaves */
name|msg
argument_list|(
literal|"Tape rewinding\n"
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|RDUMP
if|if
condition|(
name|host
condition|)
block|{
if|if
condition|(
name|rewindoffline
condition|)
block|{
name|rewind_offline
argument_list|(
name|to
argument_list|)
expr_stmt|;
name|rmtclose
argument_list|()
expr_stmt|;
return|return;
block|}
name|rmtclose
argument_list|()
expr_stmt|;
while|while
condition|(
name|rmtopen
argument_list|(
name|tape
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
name|sleep
argument_list|(
literal|10
argument_list|)
expr_stmt|;
name|rmtclose
argument_list|()
expr_stmt|;
return|return;
block|}
endif|#
directive|endif
endif|RDUMP
if|if
condition|(
name|rewindoffline
condition|)
block|{
name|rewind_offline
argument_list|(
name|to
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|to
argument_list|)
expr_stmt|;
return|return;
block|}
name|close
argument_list|(
name|to
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|f
operator|=
name|open
argument_list|(
name|tape
argument_list|,
literal|0
argument_list|)
operator|)
operator|<
literal|0
condition|)
name|sleep
argument_list|(
literal|10
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|f
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|close_rewind
argument_list|()
end_macro

begin_block
block|{
specifier|extern
name|int
name|userlabel
decl_stmt|;
name|char
modifier|*
name|label
decl_stmt|;
name|char
modifier|*
name|createlabel
parameter_list|()
function_decl|;
name|rewind
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
name|nogripe
condition|)
block|{
if|if
condition|(
name|userlabel
condition|)
block|{
name|label
operator|=
name|createlabel
argument_list|(
name|tapeno
operator|+
literal|1
argument_list|)
expr_stmt|;
name|msg
argument_list|(
literal|"Change Tapes: Mount tape labelled `%s' reel %d of this dump\n"
argument_list|,
name|label
argument_list|,
name|tapeno
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
else|else
name|msg
argument_list|(
literal|"Change Tapes: Mount tape #%d\n"
argument_list|,
name|tapeno
operator|+
literal|1
argument_list|)
expr_stmt|;
name|broadcast
argument_list|(
literal|"CHANGE TAPES!\7\7\n"
argument_list|)
expr_stmt|;
block|}
while|while
condition|(
operator|!
name|query
argument_list|(
literal|"Is the new tape mounted and ready to go?"
argument_list|)
condition|)
if|if
condition|(
name|query
argument_list|(
literal|"Do you want to abort?"
argument_list|)
condition|)
block|{
name|dumpabort
argument_list|()
expr_stmt|;
comment|/*NOTREACHED*/
block|}
block|}
end_block

begin_comment
comment|/*  *	We implement taking and restoring checkpoints on the tape level.  *	When each tape is opened, a new process is created by forking; this  *	saves all of the necessary context in the parent.  The child  *	continues the dump; the parent waits around, saving the context.  *	If the child returns X_REWRITE, then it had problems writing that tape;  *	this causes the parent to fork again, duplicating the context, and  *	everything continues as if nothing had happened.  */
end_comment

begin_macro
name|otape
argument_list|()
end_macro

begin_block
block|{
name|int
name|parentpid
decl_stmt|;
name|int
name|childpid
decl_stmt|;
name|int
name|status
decl_stmt|;
name|int
name|waitpid
decl_stmt|;
name|int
function_decl|(
modifier|*
name|interrupt
function_decl|)
parameter_list|()
init|=
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|SIG_IGN
argument_list|)
function_decl|;
name|int
name|blks
decl_stmt|,
name|i
decl_stmt|;
name|parentpid
operator|=
name|getpid
argument_list|()
expr_stmt|;
name|restore_check_point
label|:
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|interrupt
argument_list|)
expr_stmt|;
comment|/* 	 *	All signals are inherited... 	 */
name|childpid
operator|=
name|fork
argument_list|()
expr_stmt|;
if|if
condition|(
name|childpid
operator|<
literal|0
condition|)
block|{
name|msg
argument_list|(
literal|"Context save fork fails in parent %d\n"
argument_list|,
name|parentpid
argument_list|)
expr_stmt|;
name|Exit
argument_list|(
name|X_ABORT
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|childpid
operator|!=
literal|0
condition|)
block|{
comment|/* 		 *	PARENT: 		 *	save the context by waiting 		 *	until the child doing all of the work returns. 		 *	don't catch the interrupt 		 */
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|SIG_IGN
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|TDEBUG
name|msg
argument_list|(
literal|"Tape: %d; parent process: %d child process %d\n"
argument_list|,
name|tapeno
operator|+
literal|1
argument_list|,
name|parentpid
argument_list|,
name|childpid
argument_list|)
expr_stmt|;
endif|#
directive|endif
endif|TDEBUG
while|while
condition|(
operator|(
name|waitpid
operator|=
name|wait
argument_list|(
operator|&
name|status
argument_list|)
operator|)
operator|!=
name|childpid
condition|)
name|msg
argument_list|(
literal|"Parent %d waiting for child %d has another child %d return\n"
argument_list|,
name|parentpid
argument_list|,
name|childpid
argument_list|,
name|waitpid
argument_list|)
expr_stmt|;
if|if
condition|(
name|status
operator|&
literal|0xFF
condition|)
block|{
name|msg
argument_list|(
literal|"Child %d returns LOB status %o\n"
argument_list|,
name|childpid
argument_list|,
name|status
operator|&
literal|0xFF
argument_list|)
expr_stmt|;
block|}
name|status
operator|=
operator|(
name|status
operator|>>
literal|8
operator|)
operator|&
literal|0xFF
expr_stmt|;
ifdef|#
directive|ifdef
name|TDEBUG
switch|switch
condition|(
name|status
condition|)
block|{
case|case
name|X_FINOK
case|:
name|msg
argument_list|(
literal|"Child %d finishes X_FINOK\n"
argument_list|,
name|childpid
argument_list|)
expr_stmt|;
break|break;
case|case
name|X_ABORT
case|:
name|msg
argument_list|(
literal|"Child %d finishes X_ABORT\n"
argument_list|,
name|childpid
argument_list|)
expr_stmt|;
break|break;
case|case
name|X_REWRITE
case|:
name|msg
argument_list|(
literal|"Child %d finishes X_REWRITE\n"
argument_list|,
name|childpid
argument_list|)
expr_stmt|;
break|break;
default|default:
name|msg
argument_list|(
literal|"Child %d finishes unknown %d\n"
argument_list|,
name|childpid
argument_list|,
name|status
argument_list|)
expr_stmt|;
break|break;
block|}
endif|#
directive|endif
endif|TDEBUG
switch|switch
condition|(
name|status
condition|)
block|{
case|case
name|X_FINOK
case|:
name|Exit
argument_list|(
name|X_FINOK
argument_list|)
expr_stmt|;
case|case
name|X_ABORT
case|:
name|Exit
argument_list|(
name|X_ABORT
argument_list|)
expr_stmt|;
case|case
name|X_REWRITE
case|:
goto|goto
name|restore_check_point
goto|;
default|default:
name|msg
argument_list|(
literal|"Bad return code from dump: %d\n"
argument_list|,
name|status
argument_list|)
expr_stmt|;
name|Exit
argument_list|(
name|X_ABORT
argument_list|)
expr_stmt|;
block|}
comment|/*NOTREACHED*/
block|}
else|else
block|{
comment|/* we are the child; just continue */
ifdef|#
directive|ifdef
name|TDEBUG
name|sleep
argument_list|(
literal|4
argument_list|)
expr_stmt|;
comment|/* allow time for parent's message to get out */
name|msg
argument_list|(
literal|"Child on Tape %d has parent %d, my pid = %d\n"
argument_list|,
name|tapeno
operator|+
literal|1
argument_list|,
name|parentpid
argument_list|,
name|getpid
argument_list|()
argument_list|)
expr_stmt|;
endif|#
directive|endif
endif|TDEBUG
ifdef|#
directive|ifdef
name|RDUMP
while|while
condition|(
operator|(
name|to
operator|=
operator|(
name|host
condition|?
name|rmtopen
argument_list|(
name|tape
argument_list|,
literal|2
argument_list|)
else|:
name|pipeout
condition|?
literal|1
else|:
name|creat
argument_list|(
name|tape
argument_list|,
literal|0666
argument_list|)
operator|)
operator|)
operator|<
literal|0
condition|)
else|#
directive|else
else|RDUMP
while|while
condition|(
operator|(
name|to
operator|=
name|pipeout
condition|?
literal|1
else|:
name|creat
argument_list|(
name|tape
argument_list|,
literal|0666
argument_list|)
operator|)
operator|<
literal|0
condition|)
endif|#
directive|endif
endif|RDUMP
if|if
condition|(
operator|!
name|query
argument_list|(
literal|"Cannot open tape.  Do you want to retry the open?"
argument_list|)
condition|)
name|dumpabort
argument_list|()
expr_stmt|;
if|if
condition|(
name|labelcheck
argument_list|(
name|to
argument_list|,
name|tapeno
operator|+
literal|1
argument_list|)
operator|<
literal|0
condition|)
if|if
condition|(
operator|!
name|query
argument_list|(
literal|"Problem with tape label.  Do you want to retry the open?"
argument_list|)
condition|)
name|dumpabort
argument_list|()
expr_stmt|;
name|enslave
argument_list|()
expr_stmt|;
comment|/* Share open tape file descriptor with slaves */
name|asize
operator|=
literal|0
expr_stmt|;
name|tapeno
operator|++
expr_stmt|;
comment|/* current tape sequence */
name|newtape
operator|++
expr_stmt|;
comment|/* new tape signal */
name|blks
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|spcl
operator|.
name|c_type
operator|!=
name|TS_END
condition|)
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|spcl
operator|.
name|c_count
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|spcl
operator|.
name|c_addr
index|[
name|i
index|]
operator|!=
literal|0
condition|)
name|blks
operator|++
expr_stmt|;
name|spcl
operator|.
name|c_count
operator|=
name|blks
operator|+
literal|1
operator|-
name|spcl
operator|.
name|c_tapea
operator|+
name|lastspclrec
expr_stmt|;
name|spcl
operator|.
name|c_volume
operator|++
expr_stmt|;
name|spcl
operator|.
name|c_type
operator|=
name|TS_TAPE
expr_stmt|;
name|spcl
operator|.
name|c_flags
operator||=
name|DR_NEWHEADER
expr_stmt|;
name|spclrec
argument_list|()
expr_stmt|;
name|spcl
operator|.
name|c_flags
operator|&=
operator|~
name|DR_NEWHEADER
expr_stmt|;
comment|/* 		 *	It may seem that the logging ought to be done 		 *	at the end of the write but 		 *	a)	for cyclic dumps the tape has been destroyed 		 *	b)	if this dump fails due to a bad tape 		 *		then the new tape ought to be relabelled 		 *	c)	if this dump fails for other reasons 		 *		then the successful dump will also be logged 		 */
name|log_volume
argument_list|(
name|spcl
operator|.
name|c_label
argument_list|)
expr_stmt|;
if|if
condition|(
name|tapeno
operator|>
literal|1
condition|)
block|{
if|if
condition|(
name|userlabel
condition|)
name|msg
argument_list|(
literal|"Tape `%s' (reel %d) begins with blocks from ino %d\n"
argument_list|,
name|spcl
operator|.
name|c_label
argument_list|,
name|tapeno
argument_list|,
name|ino
argument_list|)
expr_stmt|;
else|else
name|msg
argument_list|(
literal|"Tape %d begins with blocks from ino %d\n"
argument_list|,
name|tapeno
argument_list|,
name|ino
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_block

begin_macro
name|dumpabort
argument_list|()
end_macro

begin_block
block|{
if|if
condition|(
name|master
operator|!=
literal|0
operator|&&
name|master
operator|!=
name|getpid
argument_list|()
condition|)
name|kill
argument_list|(
name|master
argument_list|,
name|SIGTERM
argument_list|)
expr_stmt|;
comment|/* Signals master to call dumpabort */
else|else
block|{
name|killall
argument_list|()
expr_stmt|;
name|msg
argument_list|(
literal|"The ENTIRE dump is aborted.\n"
argument_list|)
expr_stmt|;
block|}
name|Exit
argument_list|(
name|X_ABORT
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|Exit
argument_list|(
argument|status
argument_list|)
end_macro

begin_block
block|{
ifdef|#
directive|ifdef
name|TDEBUG
name|msg
argument_list|(
literal|"pid = %d exits with status %d\n"
argument_list|,
name|getpid
argument_list|()
argument_list|,
name|status
argument_list|)
expr_stmt|;
endif|#
directive|endif
endif|TDEBUG
name|exit
argument_list|(
name|status
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * could use pipe() for this if flock() worked on pipes  */
end_comment

begin_macro
name|lockfile
argument_list|(
argument|fd
argument_list|)
end_macro

begin_decl_stmt
name|int
name|fd
index|[
literal|2
index|]
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
name|tmpname
index|[
literal|20
index|]
decl_stmt|;
name|strcpy
argument_list|(
name|tmpname
argument_list|,
literal|"/tmp/dumplockXXXXXX"
argument_list|)
expr_stmt|;
name|mktemp
argument_list|(
name|tmpname
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|fd
index|[
literal|1
index|]
operator|=
name|creat
argument_list|(
name|tmpname
argument_list|,
literal|0400
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
name|msg
argument_list|(
literal|"Could not create lockfile "
argument_list|)
expr_stmt|;
name|perror
argument_list|(
name|tmpname
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|fd
index|[
literal|0
index|]
operator|=
name|open
argument_list|(
name|tmpname
argument_list|,
literal|0
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
name|msg
argument_list|(
literal|"Could not reopen lockfile "
argument_list|)
expr_stmt|;
name|perror
argument_list|(
name|tmpname
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
name|unlink
argument_list|(
name|tmpname
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|enslave
argument_list|()
end_macro

begin_block
block|{
name|int
name|first
index|[
literal|2
index|]
decl_stmt|,
name|prev
index|[
literal|2
index|]
decl_stmt|,
name|next
index|[
literal|2
index|]
decl_stmt|,
name|cmd
index|[
literal|2
index|]
decl_stmt|;
comment|/* file descriptors */
specifier|register
name|int
name|i
decl_stmt|,
name|j
decl_stmt|;
name|master
operator|=
name|getpid
argument_list|()
expr_stmt|;
name|signal
argument_list|(
name|SIGTERM
argument_list|,
name|dumpabort
argument_list|)
expr_stmt|;
comment|/* Slave sends SIGTERM on dumpabort() */
name|signal
argument_list|(
name|SIGPIPE
argument_list|,
name|sigpipe
argument_list|)
expr_stmt|;
name|signal
argument_list|(
name|SIGUSR1
argument_list|,
name|tperror
argument_list|)
expr_stmt|;
comment|/* Slave sends SIGUSR1 on tape errors */
name|lockfile
argument_list|(
name|first
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|SLAVES
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|i
operator|==
literal|0
condition|)
block|{
name|prev
index|[
literal|0
index|]
operator|=
name|first
index|[
literal|1
index|]
expr_stmt|;
name|prev
index|[
literal|1
index|]
operator|=
name|first
index|[
literal|0
index|]
expr_stmt|;
block|}
else|else
block|{
name|prev
index|[
literal|0
index|]
operator|=
name|next
index|[
literal|0
index|]
expr_stmt|;
name|prev
index|[
literal|1
index|]
operator|=
name|next
index|[
literal|1
index|]
expr_stmt|;
name|flock
argument_list|(
name|prev
index|[
literal|1
index|]
argument_list|,
name|LOCK_EX
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|i
operator|<
name|SLAVES
operator|-
literal|1
condition|)
block|{
name|lockfile
argument_list|(
name|next
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|next
index|[
literal|0
index|]
operator|=
name|first
index|[
literal|0
index|]
expr_stmt|;
name|next
index|[
literal|1
index|]
operator|=
name|first
index|[
literal|1
index|]
expr_stmt|;
comment|/* Last slave loops back */
block|}
if|if
condition|(
name|pipe
argument_list|(
name|cmd
argument_list|)
operator|<
literal|0
operator|||
operator|(
name|slavepid
index|[
name|i
index|]
operator|=
name|fork
argument_list|()
operator|)
operator|<
literal|0
condition|)
block|{
name|msg
argument_list|(
literal|"too many slaves, %d (recompile smaller) "
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|perror
argument_list|(
literal|""
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
name|slavefd
index|[
name|i
index|]
operator|=
name|cmd
index|[
literal|1
index|]
expr_stmt|;
if|if
condition|(
name|slavepid
index|[
name|i
index|]
operator|==
literal|0
condition|)
block|{
comment|/* Slave starts up here */
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<=
name|i
condition|;
name|j
operator|++
control|)
name|close
argument_list|(
name|slavefd
index|[
name|j
index|]
argument_list|)
expr_stmt|;
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|SIG_IGN
argument_list|)
expr_stmt|;
comment|/* Master handles this */
name|doslave
argument_list|(
name|cmd
index|[
literal|0
index|]
argument_list|,
name|prev
argument_list|,
name|next
argument_list|)
expr_stmt|;
name|Exit
argument_list|(
name|X_FINOK
argument_list|)
expr_stmt|;
block|}
name|close
argument_list|(
name|cmd
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|0
condition|)
block|{
name|close
argument_list|(
name|prev
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|prev
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
block|}
block|}
name|close
argument_list|(
name|first
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|first
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|master
operator|=
literal|0
expr_stmt|;
name|rotor
operator|=
literal|0
expr_stmt|;
block|}
end_block

begin_macro
name|killall
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|SLAVES
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|slavepid
index|[
name|i
index|]
operator|>
literal|0
condition|)
name|kill
argument_list|(
name|slavepid
index|[
name|i
index|]
argument_list|,
name|SIGKILL
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Synchronization - each process has a lockfile, and shares file  * descriptors to the following process's lockfile.  When our write  * completes, we release our lock on the following process's lock-  * file, allowing the following process to lock it and proceed. We  * get the lock back for the next cycle by swapping descriptors.  */
end_comment

begin_expr_stmt
name|doslave
argument_list|(
name|cmd
argument_list|,
name|prev
argument_list|,
name|next
argument_list|)
specifier|register
name|int
name|cmd
operator|,
name|prev
index|[
literal|2
index|]
operator|,
name|next
index|[
literal|2
index|]
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|int
name|nread
decl_stmt|,
name|toggle
init|=
literal|0
decl_stmt|;
name|close
argument_list|(
name|fi
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|fi
operator|=
name|open
argument_list|(
name|disk
argument_list|,
literal|0
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
comment|/* Need our own seek pointer */
name|perror
argument_list|(
literal|"  DUMP: slave couldn't reopen disk"
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
comment|/* 	 * Get list of blocks to dump, read the blocks into tape buffer 	 */
while|while
condition|(
operator|(
name|nread
operator|=
name|atomic
argument_list|(
name|read
argument_list|,
name|cmd
argument_list|,
name|req
argument_list|,
name|reqsiz
argument_list|)
operator|)
operator|==
name|reqsiz
condition|)
block|{
specifier|register
name|struct
name|req
modifier|*
name|p
init|=
name|req
decl_stmt|;
for|for
control|(
name|trecno
operator|=
literal|0
init|;
name|trecno
operator|<
name|ntrec
condition|;
name|trecno
operator|+=
name|p
operator|->
name|count
operator|,
name|p
operator|+=
name|p
operator|->
name|count
control|)
block|{
if|if
condition|(
name|p
operator|->
name|dblk
condition|)
block|{
name|bread
argument_list|(
name|p
operator|->
name|dblk
argument_list|,
name|tblock
index|[
name|trecno
index|]
argument_list|,
name|p
operator|->
name|count
operator|*
name|TP_BSIZE
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|p
operator|->
name|count
operator|!=
literal|1
operator|||
name|atomic
argument_list|(
name|read
argument_list|,
name|cmd
argument_list|,
name|tblock
index|[
name|trecno
index|]
argument_list|,
name|TP_BSIZE
argument_list|)
operator|!=
name|TP_BSIZE
condition|)
block|{
name|msg
argument_list|(
literal|"Master/slave protocol botched.\n"
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
block|}
block|}
name|flock
argument_list|(
name|prev
index|[
name|toggle
index|]
argument_list|,
name|LOCK_EX
argument_list|)
expr_stmt|;
comment|/* Wait our turn */
ifdef|#
directive|ifdef
name|RDUMP
if|if
condition|(
operator|(
name|host
condition|?
name|rmtwrite
argument_list|(
name|tblock
index|[
literal|0
index|]
argument_list|,
name|writesize
argument_list|)
else|:
name|write
argument_list|(
name|to
argument_list|,
name|tblock
index|[
literal|0
index|]
argument_list|,
name|writesize
argument_list|)
operator|)
operator|!=
name|writesize
condition|)
block|{
else|#
directive|else
else|RDUMP
if|if
condition|(
name|write
argument_list|(
name|to
argument_list|,
name|tblock
index|[
literal|0
index|]
argument_list|,
name|writesize
argument_list|)
operator|!=
name|writesize
condition|)
block|{
endif|#
directive|endif
endif|RDUMP
name|kill
argument_list|(
name|master
argument_list|,
name|SIGUSR1
argument_list|)
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
name|sigpause
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
name|toggle
operator|^=
literal|1
expr_stmt|;
name|flock
argument_list|(
name|next
index|[
name|toggle
index|]
argument_list|,
name|LOCK_UN
argument_list|)
expr_stmt|;
comment|/* Next slave's turn */
block|}
comment|/* Also jolts him awake */
if|if
condition|(
name|nread
operator|!=
literal|0
condition|)
block|{
name|perror
argument_list|(
literal|"  DUMP: error reading command pipe"
argument_list|)
expr_stmt|;
name|dumpabort
argument_list|()
expr_stmt|;
block|}
block|}
comment|/*  * Since a read from a pipe may not return all we asked for,  * or a write may not write all we ask if we get a signal,  * loop until the count is satisfied (or error).  */
name|atomic
argument_list|(
argument|func
argument_list|,
argument|fd
argument_list|,
argument|buf
argument_list|,
argument|count
argument_list|)
name|int
argument_list|(
operator|*
name|func
argument_list|)
argument_list|()
decl_stmt|,
name|fd
decl_stmt|,
name|count
decl_stmt|;
name|char
modifier|*
name|buf
decl_stmt|;
block|{
name|int
name|got
decl_stmt|,
name|need
init|=
name|count
decl_stmt|;
while|while
condition|(
operator|(
name|got
operator|=
call|(
modifier|*
name|func
call|)
argument_list|(
name|fd
argument_list|,
name|buf
argument_list|,
name|need
argument_list|)
operator|)
operator|>
literal|0
operator|&&
operator|(
name|need
operator|-=
name|got
operator|)
operator|>
literal|0
condition|)
name|buf
operator|+=
name|got
expr_stmt|;
return|return
operator|(
name|got
operator|<
literal|0
condition|?
name|got
else|:
name|count
operator|-
name|need
operator|)
return|;
block|}
comment|/*  *	Added by Peter C to backspace one record after a label read  */
name|backone
argument_list|(
argument|fd
argument_list|)
block|{
name|struct
name|mtop
name|mtop
decl_stmt|;
name|mtop
operator|.
name|mt_op
operator|=
name|MTBSR
expr_stmt|;
name|mtop
operator|.
name|mt_count
operator|=
literal|1
expr_stmt|;
ifdef|#
directive|ifdef
name|RDUMP
if|if
condition|(
name|host
condition|)
return|return
operator|(
name|rmtioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|)
return|;
endif|#
directive|endif
endif|RDUMP
return|return
operator|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|)
return|;
block|}
comment|/*  *	Added by Peter C  *	put a tape drive offline  */
name|rewind_offline
argument_list|(
argument|fd
argument_list|)
block|{
name|struct
name|mtop
name|mtop
decl_stmt|;
name|char
modifier|*
name|eofr
decl_stmt|;
name|char
modifier|*
name|eoff
decl_stmt|;
name|eofr
operator|=
literal|"Cannot write end of file record"
expr_stmt|;
name|eoff
operator|=
literal|"Cannot put the tape offline"
expr_stmt|;
ifdef|#
directive|ifdef
name|RDUMP
if|if
condition|(
name|host
condition|)
block|{
name|mtop
operator|.
name|mt_op
operator|=
name|MTWEOF
expr_stmt|;
name|mtop
operator|.
name|mt_count
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|rmtioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
name|eofr
argument_list|)
expr_stmt|;
name|mtop
operator|.
name|mt_op
operator|=
name|MTWEOF
expr_stmt|;
name|mtop
operator|.
name|mt_count
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|rmtioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
name|eofr
argument_list|)
expr_stmt|;
name|mtop
operator|.
name|mt_op
operator|=
name|MTOFFL
expr_stmt|;
name|mtop
operator|.
name|mt_count
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|rmtioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
name|eoff
argument_list|)
expr_stmt|;
return|return;
block|}
endif|#
directive|endif
endif|RDUMP
name|mtop
operator|.
name|mt_op
operator|=
name|MTWEOF
expr_stmt|;
name|mtop
operator|.
name|mt_count
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
name|eofr
argument_list|)
expr_stmt|;
name|mtop
operator|.
name|mt_op
operator|=
name|MTWEOF
expr_stmt|;
name|mtop
operator|.
name|mt_count
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
name|eofr
argument_list|)
expr_stmt|;
name|mtop
operator|.
name|mt_op
operator|=
name|MTOFFL
expr_stmt|;
name|mtop
operator|.
name|mt_count
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|MTIOCTOP
argument_list|,
operator|&
name|mtop
argument_list|)
operator|<
literal|0
condition|)
name|perror
argument_list|(
name|eoff
argument_list|)
expr_stmt|;
block|}
end_block

end_unit

