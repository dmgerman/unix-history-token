...
begin_comment
comment|/* Need 1 extra for the fontcache */
end_comment
...
literal|0
decl_stmt|;
comment|/* 	 * Here comes the kludgy bug fix itself 	 */
if|if
condition|(
name|xfont
operator|>
...
comment|/* search through search list of xfont 		/* to see what font it ought to be on. 		/* for now, searches S, then remaining fonts in wraparound 		 * order (but only when a special font is mounted!!) 		*/
if|if
condition|(
name|smnt
condition|)
...
name|smnt
condition|)
comment|/* Oops, only set bd 						 * when it is a special 						 * font!! 						 */
name|bd
operator|=
name|bdtab
index|[
...
name|stderr
argument_list|,
literal|"troff: no \\C yet (How do you know about it?)\n"
argument_list|)
expr_stmt|;
return|return
operator|(
...
begin_comment
comment|/*****  * find the font name i, where i also can be a number.  *  * Installs the font i when not present  *  * returns -1 on error  */
end_comment
...
condition|)
block|{
comment|/* 				 * when .ft<zero argument or the 				 	     argument is the non-documented P,> 				 * pop previous font. 				 * Alas, if the current font is in the 				 * font cache already, we have to be 				 * be very careful. No solution yet 				 * for this bug. 				 */
name|j
operator|=
name|font1
expr_stmt|;
...
condition|)
return|return;
comment|/* unable to find or mount font */
name|s0
label|:
name|font1
operator|=
...
block|{
comment|/* should check for third argument, (directory) 		 * doesn't work at all */
name|skip
argument_list|()
expr_stmt|;
name|setfp
...
name|f
condition|)
comment|/* f already mounted at pos, */
return|return
operator|(
name|pos
operator|)
...
literal|0
expr_stmt|;
comment|/* Small bug here, we should test for 				 * other special fonts mounted, needs 				 * to be fixed in future 				 */
name|sbold
operator|=
literal|0
expr_stmt|;
...
comment|/* Trying to fix this problem: 		 * 		 * I expand fontlab to the maximum of fonts troff can 		 * handle. The maximum number i, due to the two chars 		 * fontname limit, is 99. 		 * If we don't use the (named) font in one of the 		 * standard position, we install the name in the next 		 * free slot. Whenever we need info about the font, we 		 * read in the data at position zero, and secretly use 		 * the data (actually only necessary for the width 		 *  and ligature info). The ptfont() routine will tell 		 * the device filter to put the font always at position 		 * zero, so no need to change these filters. 		 * Yes, this is a bit kludgy. 		 * 		 * BTW, I removed the directory stuff completly since 		 * setfp is always called as setfp(x, y, 0). 		 * 		 * TODO: It must be possible to do this with all the 		 * font positions. Dynamically switch the ``mounted'' 		 * fonts in a LRU style, whithout bothering the user 		 * with it. Then they never have to use the .fp 		 * request. For compatibilty reason we could leave it 		 * in (it can shuffle the date in the fontlab table), 		 * or make it a NOOP. 		 * Of course we do not touch positions with a ``Special'' font. 		 * 		 * Simple things TODO 		 * 	A routine to extract two characters troff 		 *	  names is handy. 		 *	A routine to find out whether a font discription 		 *	  file is available, currently setfp(...) 		 *	  handle this and read in the font when 		 *	  appropriate. (Proposing ftstat() here). 		 *	Fix the ``smnt info''. 		 */
comment|/* 	 * In case we are mounting a font to a standard position ... 	 */
if|if
condition|(
name|pos
operator|>
...
name|k
expr_stmt|;
comment|/* Undocumented feature here, you can only do .bd S F N, 		 * for one font F. 		 */
block|}
name|bd1
label|:
name|skip
