...
begin_comment
comment|/* Font description file producer:  David Slattengren  * Taken from vfontinfo by Andy Hertzfeld  4/79  *  *	Use:  mkfnt [-s] [-m] [-p#] [-r#] [-ddirectory] font  *  *	Mkfnt takes the font named "font" and produces a ditroff description  *	file from it.  The -s option tells mkfnt that this is a special font,  *	and sould substitute special character names for the normal ones.  The  *	-m option switches to the math font character map.  -m and -s together  *	will get the math font at the moment.  The  *	-p# option tells what point size the DESC file has as it's "unitwidth"  *	argument (default: 36).  The -r# option is the resolution of the device  *	(default: 240, in units/inch).  The -d option tells where to look for  *	fonts (default: /usr/src/local/imagen/fonts/raster).  */
end_comment
...
index|[]
init|=
literal|"@(#)makeifont.c	1.1	(Berkeley)	%G%"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* flag "is this a math font?"; */
end_comment
...
begin_comment
comment|/* flag "is this a special font?";  used to 				   determine which mapping array to look in. */
end_comment
