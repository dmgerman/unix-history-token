...
begin_comment
comment|/*	makeifont.c	(Berkeley)	1.8	86/03/04  *  * Font description file producer for imagen fonts:  David Slattengren  * Taken from vfontinfo by Andy Hertzfeld  4/79  *  *  Use:  makeifont [ -nNAME ]  [ -i -s -a -o -l -c ]  [ "-xs1,s2[;s1,s2...]" ]  *		[ "-ys1,s2[;s1,s2...]" ]  [ -p# ]  [ -r# ]  [ -ddir ]  font  *  *	Mkfnt takes the font named "font" and produces a ditroff description  *	file from it.  The -n option takes the 1 or 2 letter troff name to put  *	the description (default = XX).  The -s, -o, -i, -a options select a  *	different character mapping than for a "roman" font.  s = special;  *	o = math;  i = italics;  a = ascii.  The -l option tells if the font  *	has ligatures.  The -c option tells makeifont that the font is a  *	constant width one and sets parameters appropriately.  *  *	Both -x and -y options allow character name mapping.  A semi-colon  *	separated list of comma-separated character-name pairs follows the  *	x or y.  Notice that there are no spaces in the -x or -y command.  It  *	is also IMPORTANT to enclose these arguments in single quotes to stop  *	the cshell from interpretting the contents.  A -x pair REPLACES the  *	definition for s1 by s2.  A -y pair creates a synonym for s1 and calls  *	it s2.  -x and -y MUST be sent after -s, -m, -i, or -a  if one of them  *	is used.  Some synonyms are defaulted.  To remove a synonym or char-  *	acter, leave out s2.  *  *	The -p# option tells what point size the DESC file has  *	as it's "unitwidth" argument (default: 40).  The -r# option is the  *	resolution of the device (default: 240, in units/inch).  The -d option  *	tells where to find fonts (default: /usr/src/local/imagen/fonts/raster).  */
end_comment
...
index|[]
init|=
literal|"@(#)makeifont.c	1.8	(Berkeley)	%G%"
decl_stmt|;
end_decl_stmt
...
name|NULL
decl_stmt|;
comment|/* list of character-name replacements */
name|char
modifier|*
name|synonymlist
init|=
name|NULL
decl_stmt|;
comment|/* list of synonym entries */
name|char
name|tostring
parameter_list|()
function_decl|;
...
parameter_list|()
function_decl|;
comment|/* reads operands from commandline */
name|charmap
operator|=
name|iregular
expr_stmt|;
...
name|error
argument_list|(
literal|"An RST font filename must be the last option"
argument_list|)
expr_stmt|;
for|for
control|(
...
comment|/* purposefully OUT of the range of characters (128) */
name|printf
argument_list|(
literal|"\\|	%4d	 0	0%o\n\\^	%4d	 0	0%o\n"
argument_list|,
