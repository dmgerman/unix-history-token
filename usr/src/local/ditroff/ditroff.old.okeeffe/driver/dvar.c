...
begin_comment
comment|/* current stipple font - not using any to start with */
end_comment
...
begin_comment
comment|/* current member to use from stipple font */
end_comment
...
begin_comment
comment|/* Zero origin in emptying buffer */
end_comment
...
begin_comment
comment|/* current position in emptying buffer */
end_comment
...
begin_comment
comment|/* amount of emptying done (initially "done") */
end_comment
...
name|fnum
decl_stmt|;
comment|/* if == -1, then this position is empty */
name|int
name|psize
decl_stmt|;
name|struct
...
begin_comment
comment|/* initialized at program start */
end_comment
...
begin_comment
comment|/* flag:  "do polygons get bordered?" */
end_comment
...
condition|)
block|{
comment|/* for each input operation */
name|slop_lines
argument_list|(
literal|1
argument_list|)
expr_stmt|;
comment|/* put out an output line */
ifdef|#
directive|ifdef
name|DRIVER
if|if
...
condition|)
block|{
comment|/* finish off last page, if */
name|slop_lines
argument_list|(
name|NLINES
operator|-
...
literal|0
expr_stmt|;
comment|/* set emptying to be started */
name|elevel
operator|=
name|fill
expr_stmt|;
comment|/* swap buffer pointers */
name|fill
operator|=
name|empty
expr_stmt|;
...
begin_comment
comment|/* buffer.	Also updates the pointers to the emptying buffer */
end_comment
...
begin_comment
comment|/*----------------------------------------------------------------------------*  | Routine:	polygon ( x_coordinates, y_coordinates, num_of_points )  |  | Results:	draws a polygon starting at (x[1], y[1]) going through  |		each of (x_coordinates, y_coordinates), and fills it  |		with a stipple pattern from stip_disp and stip_bits,  |		which point to the stipple font.  The pattern is defined  |		by "stip" and "stipmem".  |  |		The scan-line algorithm implemented scans from left to  |		right (low x to high x).  It also scans, within a line,  |		from bottom to top (high y to low y).  |  |		polygons are clipped to page boundary.  |  | Bugs:	stipple pattern MUST be a power of two bytes "wide" and  |		square.  The square restriction comes from the fact that  |		the varian and versatec are respectively rotated.  *----------------------------------------------------------------------------*/
end_comment
