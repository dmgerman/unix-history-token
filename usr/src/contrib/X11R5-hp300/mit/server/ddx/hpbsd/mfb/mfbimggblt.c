...
begin_comment
comment|/* $XConsortium: mfbimggblt.c,v 5.6 91/05/26 09:21:19 rws Exp $ */
end_comment
...
begin_comment
comment|/*     we should eventually special-case fixed-width fonts for ImageText.      this works for fonts with glyphs<= 32 bits wide.      the clipping calculations are done for worst-case fonts. we make no assumptions about the heights, widths, or bearings of the glyphs.  if we knew that the glyphs are all the same height, we could clip the tops and bottoms per clipping box, rather than per character per clipping box.  if we knew that the glyphs' left and right bearings were wlle-behaved, we could clip a single character at the start, output until the last unclipped character, and then clip the last one.  this is all straightforward to determine based on max-bounds and min-bounds from the font.     there is some inefficiency introduced in the per-character clipping to make what's going on clearer.      (it is possible, for example, for a font to be defined in which the next-to-last character in a font would be clipped out, but the last one wouldn't.  the code below deals with this.)      Image text looks at the bits in the glyph and the fg and bg in the GC.  it paints a rectangle, as defined in the protocol dcoument, and the paints the characters.     to avoid source proliferation, this file is compiled three times: 	MFBIMAGEGLYPHBLT	OPEQ 	mfbImageGlyphBltWhite	|= 	mfbImageGlyphBltBlack&=~      the register allocations for startmask and endmask may not be the right thing.  are there two other deserving candidates? xoff, pdst, pglyph, and tmpSrc seem like the right things, though. */
end_comment

begin_function
name|void
name|MFBIMAGEGLYPHBLT
parameter_list|(
name|pDrawable
parameter_list|,
name|pGC
...
name|backrect
decl_stmt|;
comment|/* backing rectangle to paint. 			   in the general case, NOT necessarily 			   the same as the string's bounding box 			*/
name|CharInfoPtr
name|pci
decl_stmt|;
name|int
...
parameter_list|()
function_decl|;
comment|/* we might temporarily usurp this 			   field in devPriv */
if|if
condition|(
operator|!
operator|(
...
name|FillArea
expr_stmt|;
comment|/* pcc doesn't like this.  why?     ((mfbPrivGC *)(pGC->devPrivates[mfbGCPrivateIndex].ptr))->FillArea =  			((pGC->bgPixel& 1) ? mfbSolidWhiteArea : mfbSolidBlackArea); */
if|if
condition|(
name|pGC
operator|->
...
name|oldFillArea
expr_stmt|;
comment|/* the faint-hearted can open their eyes now */
switch|switch
condition|(
call|(
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* HACK ALERT 	   since we continue out of the loop below so often, it 	   is easier to increment pbox at the  top than at the end. 	   don't try this at home. 	*/
name|pbox
operator|--
expr_stmt|;
while|while
