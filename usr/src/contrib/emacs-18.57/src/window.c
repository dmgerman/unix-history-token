...
begin_comment
comment|/* Non-nil means it is the window for C-M-v to scroll    when the minibuffer is selected.  */
end_comment
...
begin_comment
comment|/* Non-nil means it's function to call to display temp buffers.  */
end_comment

begin_decl_stmt
name|Lisp_Object
name|Vtemp_buffer_show_hook
decl_stmt|;
end_decl_stmt
...
name|p
decl_stmt|;
comment|/* Add sizeof (Lisp_Object) here because sizeof (struct Lisp_Vector)      includes the first element.  */
name|val
operator|=
name|Fmake_vector
argument_list|(
...
literal|0
argument_list|,
literal|"Return the window that the cursor now appears in and commands apply to."
argument_list|)
end_macro
...
name|DEFUN
argument_list|(
literal|"minibuffer-window"
argument_list|,
argument|Fminibuffer_window
argument_list|,
argument|Sminibuffer_window
argument_list|,
literal|0
argument_list|,
literal|0
...
literal|0
argument_list|,
literal|"Return the window used for minibuffers."
argument_list|)
end_macro
...
name|width
decl_stmt|;
comment|/* If this window does not end at the right margin,      must deduct one column for the border */
if|if
condition|(
name|w
operator|+
...
argument_list|)
expr_stmt|;
comment|/* this is not right, but much easier than doing what is right.  */
name|w
operator|->
...
name|DEFUN
argument_list|(
literal|"set-window-buffer"
argument_list|,
name|Fset_window_buffer
argument_list|,
name|Sset_window_buffer
argument_list|,
literal|2
argument_list|,
literal|2
...
argument_list|)
condition|)
comment|/* Do this except when the selected window's buffer        is being removed from some other window.  */
name|XBUFFER
argument_list|(
name|buf
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Point in the selected window's buffer      is actually stored in that buffer, and the window's pointm isn't used.      So don't clobber point in that buffer.  */
if|if
condition|(
operator|!
name|EQ
...
argument_list|)
expr_stmt|;
comment|/* Go to the point recorded in the window.      This is important when the buffer is in more      than one window.  It also matters when      redisplay_window has altered point after scrolling,      because it makes the change only in the window.  */
name|SET_PT
argument_list|(
name|marker_position
argument_list|(
...
argument_list|)
condition|)
comment|/* Add 1 so we round up rather than down. 	   This puts an excess column into the left-hand window, 	   which is the one that certainly contains a border line.  */
name|size
operator|=
operator|(
literal|1
...
argument_list|)
decl_stmt|;
comment|/* If trying to grow this window to or beyond size of the parent, 	 make delta1 so big that, on shrinking back down, 	 all the siblings end up with less than one line and are deleted.  */
if|if
condition|(
name|opht
operator|<=
...
literal|2
expr_stmt|;
comment|/* Otherwise, make delta1 just right so that if we add delta1 	 lines to this window and to the parent, and then shrink 	 the parent back to its original size, the new proportional 	 size of this window will increase by delta.  */
else|else
name|delta1
operator|=
operator|(
...
operator|)
expr_stmt|;
comment|/* Add delta1 lines or columns to this window, and to the parent, 	 keeping things consistent while not affecting siblings.  */
name|CURSIZE
argument_list|(
name|XWINDOW
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Squeeze out delta1 lines or columns from our parent, 	 shriking this window and siblings proportionately. 	 This brings parent back to correct size. 	 Delta1 was calculated so this makes this window the desired size, 	 taking it all out of the siblings.  */
call|(
modifier|*
name|setsizefun
call|)
...
block|{
name|int
name|size_from_Lisp_Vector_struct
decl_stmt|;
name|struct
name|Lisp_Vector
modifier|*
name|next_from_Lisp_Vector_struct
decl_stmt|;
name|Lisp_Object
name|screen_width
decl_stmt|,
...
decl_stmt|;
name|Lisp_Object
name|minibuf_scroll_window
decl_stmt|;
comment|/* A vector, interpreted as a struct saved_window */
name|Lisp_Object
name|saved_windows
decl_stmt|;
block|}
...
define|#
directive|define
name|SAVE_WINDOW_DATA_SIZE
value|6
end_define

begin_comment
comment|/* Arg to Fmake_vector */
end_comment

begin_comment
comment|/* This is saved as a Lisp_Vector */
end_comment
...
name|saved_window
block|{
comment|/* these first two must agree with struct Lisp_Vector in lisp.h */
name|int
name|size_from_Lisp_Vector_struct
decl_stmt|;
name|struct
...
parameter_list|)
define|\
value|((struct saved_window *) (XVECTOR ((swv)->contents[(n)])))
end_define

begin_macro
name|DEFUN
argument_list|(
literal|"set-window-configuration"
argument_list|,
argument|Fset_window_configuration
argument_list|,
argument|Sset_window_configuration
argument_list|,
literal|1
argument_list|,
literal|1
...
decl_stmt|;
name|Lisp_Object
name|new_current_buffer
decl_stmt|;
name|int
name|k
decl_stmt|;
comment|/* Save screen height here so we can go back to it at the end.  */
name|int
name|previous_screen_height
init|=
name|screen_height
decl_stmt|;
name|int
name|previous_screen_width
init|=
name|screen_width
decl_stmt|;
name|int
name|screen_size_change
init|=
literal|0
decl_stmt|;
while|while
...
condition|)
block|{
comment|/* the function window-configuration-p isn't actually defined 	 at present --- is there a need for it? */
name|arg
operator|=
...
name|intern
argument_list|(
literal|"window-configuration-p"
argument_list|)
argument_list|,
name|arg
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Set the screen height to the value it had at save time.  */
if|if
condition|(
name|XFASTINT
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Set window markers at start of buffer. 		 Rely on set_marker_restricted to put them 		 within the restriction.  */
name|set_marker_restricted
argument_list|(
name|w
operator|->
...
comment|/* Keeping window's old buffer; make sure the markers are real.  */
comment|/* Else if window's old buffer is dead too, get a live one.  */
block|{
comment|/* Set window markers at start of buffer. 		 Rely on set_marker_restricted to put them within the restriction.  */
if|if
...
block|}
block|}
comment|/* Set the screen height to the value it had before this function.  */
if|if
condition|(
name|screen_size_change
condition|)
...
name|DEFUN
argument_list|(
literal|"current-window-configuration"
argument_list|,
argument|Fcurrent_window_configuration
argument_list|,
argument|Scurrent_window_configuration
argument_list|,
literal|0
argument_list|,
literal|0
...
name|DEFVAR_LISP
argument_list|(
literal|"temp-buffer-show-hook"
argument_list|,
operator|&
name|Vtemp_buffer_show_hook
argument_list|,
literal|"Non-nil means call as function to display a help buffer.\n\ Used by with-output-to-temp-buffer."
argument_list|)
expr_stmt|;
name|Vtemp_buffer_show_hook
operator|=
...
name|DEFVAR_LISP
argument_list|(
literal|"minibuffer-scroll-window"
argument_list|,
operator|&
name|Vminibuf_scroll_window
argument_list|,
literal|"Non-nil means it is the window that C-M-v in minibuffer should scroll."
argument_list|)
expr_stmt|;
name|Vminibuf_scroll_window
operator|=
...
argument_list|(
operator|&
name|Sset_window_configuration
argument_list|)
expr_stmt|;
name|defsubr
argument_list|(
operator|&
name|Scurrent_window_configuration
argument_list|)
expr_stmt|;
name|defsubr
argument_list|(
