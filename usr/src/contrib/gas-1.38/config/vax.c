...
begin_comment
comment|/* vax.c - vax-specific -    Copyright (C) 1987 Free Software Foundation, Inc.  This file is part of GAS, the GNU Assembler.  GAS is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GAS is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GAS; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|as_warn
argument_list|(
literal|"Displacement (%ld) too long for instruction field length (%d)."
argument_list|,
name|v
argument_list|,
name|n
...
name|goofed
decl_stmt|;
comment|/* TRUE: instruction_string bad for all passes. */
specifier|register
name|struct
name|vop
modifier|*
...
expr_stmt|;
block|}
comment|/*    * Now we try to find as many as_warn()s as we can. If we do any as_warn()s    * then goofed=TRUE. Notice that we don't make any frags yet.    * Should goofed be TRUE, then this instruction will wedge in any pass,    * and we can safely flush it, without causing interpass symbol phase    * errors. That is, without changing label values in different passes.    */
if|if
condition|(
name|goofed
operator|=
...
name|can_be_short
decl_stmt|;
comment|/* TRUE if a bignum can be reduced to a short literal. */
name|input_line_pointer
operator|=
name|operandP
operator|->
...
literal|'\0'
expr_stmt|;
comment|/* If to_seg == SEG_PASS1, expression() will have set need_pass_2 = TRUE. */
switch|switch
condition|(
name|to_seg
operator|=
...
comment|/* But, we will program defensively. Since this situation occurs */
comment|/* rarely so it costs us little to do, and stops Dean */
comment|/* worrying about the origin of random bits in expressionS's. */
name|expP
operator|->
name|X_add_symbol
operator|=
...
name|is_undefined
decl_stmt|;
comment|/* True if operand expression's */
...
begin_comment
comment|/* on vax: first 4 bytes are normal unsigned long, next three bytes    are symbolnum, least sig. byte first.  Last byte is broken up with    the upper nibble as nuthin, bit 3 as extern, bits 2& 1 as length, and    bit 0 as pcrel. */
end_comment
...
begin_comment
comment|/* JF this used to be a separate file */
end_comment

begin_comment
comment|/* vax_ins_parse.c - a part for a VAX assembler */
end_comment

begin_comment
comment|/* Copyright (C) 1987 Free Software Foundtation, Inc */
end_comment
...
begin_comment
comment|/* #include<stdio.h> JF for one big happy file */
end_comment

begin_comment
comment|/* JF #include "vax-inst.h"		/* define the tree we parse it into */
end_comment
...
name|synthetic_too
decl_stmt|;
comment|/* TRUE means include jXXX op-codes. */
name|char
modifier|*
name|immediate
decl_stmt|,
...
begin_comment
comment|/* TRUE means want synthetic opcodes. */
end_comment
...
begin_comment
comment|/* end: vax_reg_parse.c */
end_comment

begin_comment
comment|/* JF this was another separate prog */
end_comment

begin_comment
comment|/* vip_op.c - parse 1 VAX instr's operand.(C)1986 Free Software Foundation. */
end_comment

begin_comment
comment|/* JF #include<ctype.h> */
end_comment

begin_comment
comment|/* #include "vax_inst.h" */
end_comment
...
begin_comment
comment|/* JF #define SP      (14) /* JF for one big happy file #define PC      (15) */
end_comment

begin_comment
comment|/* useful ideas */
end_comment
...
name|paren
decl_stmt|;
comment|/* TRUE if () surround register */
name|int
name|reg
decl_stmt|;
...
block|}
comment|/* 	   * If err == "..." then we lost. 	   * Otherwise paren==TRUE and reg = register in "()". 	   */
block|}
else|else
name|paren
operator|=
name|FALSE
expr_stmt|;
comment|/*        * If err == "..." then we lost.        * Otherwise, q points just before "(Rn)", if any.        * If there was a "(...)" then paren==TRUE, and reg is the register.        */
comment|/*        * We should only seek '-' of "-(...)" if:        *   we saw "(...)"                    paren == TRUE        *   we have no errors so far          ! *err        *   we did not see '+' of "(...)+"    sign< 1        * We don't check len. We want a specific error message later if        * user tries "x^...-(Rn)". This is a feature not a bug.        */
if|if
condition|(
operator|!
operator|*
...
directive|endif
comment|/*#Ifdef NEVER*/
comment|/*    * Case of S^#.    *    * in:  at       FALSE    *      len      's'               definition    *      hash     TRUE              demand    *      p:q                        demand not empty    *      sign     0                 by paren==FALSE    *      paren    FALSE             by "()" scan logic because "S^" seen    *      reg      -1                or nn by mistake    *      ndx      -1    *    * out: mode     0    *      reg      -1    *      len      's'    *      exp    *      ndx      -1    */
if|if
condition|(
operator|!
operator|*
...
block|}
block|}
comment|/*    * Case of -(Rn), which is weird case.    *    * in:  at       FALSE    *      len      '    *      hash     FALSE    *      p:q      q<p    *      sign     -1                by definition    *      paren    TRUE              by definition    *      reg      present           by definition    *      ndx      optional    *    * out: mode     7    *      reg      present    *      len      ' '    *      exp      ""                enforce empty expression    *      ndx      optional          warn if same as reg    */
if|if
condition|(
operator|!
operator|*
...
expr_stmt|;
block|}
comment|/*    * Case of (Rn)+, which is slightly different.    *    * in:  at    *      len      ' '    *      hash     FALSE    *      p:q      q<p    *      sign     +1                by definition    *      paren    TRUE              by definition    *      reg      present           by definition    *      ndx      optional    *    * out: mode     8+@    *      reg      present    *      len      ' '    *      exp      ""                enforce empty expression    *      ndx      optional          warn if same as reg    */
if|if
condition|(
operator|!
operator|*
...
block|}
block|}
comment|/*    * Case of #, without S^.    *    * in:  at    *      len      ' ' or 'i'    *      hash     TRUE              by definition    *      p:q    *      sign     0    *      paren    FALSE    *      reg      absent    *      ndx      optional    *    * out: mode     8+@    *      reg      PC    *      len      ' ' or 'i'    *      exp    *      ndx      optional    */
if|if
condition|(
operator|!
operator|*
...
block|}
block|}
comment|/*    * If !*err, then        sign == 0    *                       hash == FALSE    */
...
block|}
block|}
comment|/*    * If !*err,        sign  == 0    *                  hash  == FALSE    *                  paren == TRUE  OR reg==-1    */
comment|/*    * Rest of cases fit into one bunch.    *    * in:  at       optional    *      len      ' ' or 'b' or 'w' or 'l'    *      hash     FALSE             by program logic    *      p:q      expected          (empty is not an error)    *      sign     0                 by program logic    *      paren    optional    *      reg      optional    *      ndx      optional    *    * out: mode     10 + @ + len    *      reg      optional    *      len      ' ' or 'b' or 'w' or 'l'    *      exp                        maybe empty    *      ndx      optional          warn if same as reg    */
if|if
condition|(
operator|!
operator|*
