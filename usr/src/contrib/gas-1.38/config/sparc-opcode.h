begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * This code is derived from software copyrighted by the Free Software  * Foundation.  *  * Modified 1993 by Chris Torek at Lawrence Berkeley Laboratory.  *  *	@(#)sparc-opcode.h	5.2 (Berkeley) %G%  */
end_comment

begin_comment
comment|/* Table of opcodes for the sparc.    Copyright (C) 1989 Free Software Foundation, Inc.  This file is part of GAS, the GNU Assembler, and GDB, the GNU disassembler.  GAS/GDB is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GAS/GDB is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GAS or GDB; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|__STDC__
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
specifier|const
argument_list|)
end_if

begin_define
define|#
directive|define
name|const
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Structure of an opcode table entry.  */
end_comment

begin_struct
struct|struct
name|sparc_opcode
block|{
specifier|const
name|char
modifier|*
name|name
decl_stmt|;
name|unsigned
name|long
name|int
name|match
decl_stmt|;
comment|/* Bits that must be set.  */
name|unsigned
name|long
name|int
name|lose
decl_stmt|;
comment|/* Bits that must not be set.  */
specifier|const
name|char
modifier|*
name|args
decl_stmt|;
comment|/* Nonzero if this is a delayed branch instruction.  */
name|char
name|delayed
decl_stmt|;
block|}
struct|;
end_struct

begin_comment
comment|/*    All sparc opcodes are 32 bits, except for the `set' instruction (really    a macro), which is 64 bits.  It is handled as a special case.     The match component is a mask saying which bits must match a    particular opcode in order for an instruction to be an instance    of that opcode.     The args component is a string containing one character    for each operand of the instruction.  Kinds of operands:    #    Number used by optimizer.  It is ignored.    1    rs1 register.    2    rs2 register.    d    rd register.    e    frs1 floating point register.    f    frs2 floating point register.    g    frsd floating point register.    b    crs1 coprocessor register    c    crs2 coprocessor register    D    crsd coprocessor register    h    22 high bits.    i    13 bit Immediate.    l    22 bit PC relative immediate.    L    30 bit PC relative immediate.    a    Annul.  The annul bit is set.    A    Alternate address space.  Stored as 8 bits.    C    Coprocessor state register.    F    floating point state register.    p    Processor state register.    q    Floating point queue.    r    Single register that is both rs1 and rsd.    R	Single register that is both rs2 and rsd.    Q    Coprocessor queue.    S    Special case.    t    Trap base register.    w    Window invalid mask register.    y    Y register.  */
end_comment

begin_comment
comment|/* The order of the opcodes in this table is significant:        * The assembler requires that all instances of the same mnemonic must be    consecutive.  If they aren't, the assembler will bomb at runtime.     * The disassembler should not care about the order of the opcodes.  */
end_comment

begin_decl_stmt
specifier|static
name|struct
name|sparc_opcode
name|sparc_opcodes
index|[]
init|=
block|{
block|{
literal|"ldd"
block|,
literal|0xc1980000
block|,
literal|0x0060201f
block|,
literal|"[1],D"
block|,
literal|0
block|}
block|,
comment|/* ldd [rs1+%g0],d */
block|{
literal|"ldd"
block|,
literal|0xc1982000
block|,
literal|0x00601fff
block|,
literal|"[1],D"
block|,
literal|0
block|}
block|,
comment|/* ldd [rs1+0],d */
block|{
literal|"ldd"
block|,
literal|0xc1982000
block|,
literal|0x00600000
block|,
literal|"[1+i],D"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc1982000
block|,
literal|0x00600000
block|,
literal|"[i+1],D"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc1980000
block|,
literal|0x00602000
block|,
literal|"[1+2],D"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc1180000
block|,
literal|0x00e0201f
block|,
literal|"[1],g"
block|,
literal|0
block|}
block|,
comment|/* ldd [rs1+%g0],d */
block|{
literal|"ldd"
block|,
literal|0xc1182000
block|,
literal|0x00e01fff
block|,
literal|"[1],g"
block|,
literal|0
block|}
block|,
comment|/* ldd [rs1+0],d */
block|{
literal|"ldd"
block|,
literal|0xc1182000
block|,
literal|0x00e00000
block|,
literal|"[1+i],g"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc1182000
block|,
literal|0x00e00000
block|,
literal|"[i+1],g"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc1180000
block|,
literal|0x00e02000
block|,
literal|"[1+2],g"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc0180000
block|,
literal|0x01e0201f
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldd [rs1+%g0],d */
block|{
literal|"ldd"
block|,
literal|0xc0182000
block|,
literal|0x01e01fff
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldd [rs1+0],d */
block|{
literal|"ldd"
block|,
literal|0xc0182000
block|,
literal|0x01e00000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc0182000
block|,
literal|0x01e00000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldd"
block|,
literal|0xc0180000
block|,
literal|0x01e02000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1880000
block|,
literal|0x0070201f
block|,
literal|"[1],C"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+%g0],d */
block|{
literal|"ld"
block|,
literal|0xc1882000
block|,
literal|0x00701fff
block|,
literal|"[1],C"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+0],d */
block|{
literal|"ld"
block|,
literal|0xc1882000
block|,
literal|0x00700000
block|,
literal|"[1+i],C"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1882000
block|,
literal|0x00700000
block|,
literal|"[i+1],C"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1880000
block|,
literal|0x00702000
block|,
literal|"[1+2],C"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1800000
block|,
literal|0x0078201f
block|,
literal|"[1],D"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+%g0],d */
block|{
literal|"ld"
block|,
literal|0xc1802000
block|,
literal|0x00781fff
block|,
literal|"[1],D"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+0],d */
block|{
literal|"ld"
block|,
literal|0xc1802000
block|,
literal|0x00780000
block|,
literal|"[1+i],D"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1802000
block|,
literal|0x00780000
block|,
literal|"[i+1],D"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1800000
block|,
literal|0x00782000
block|,
literal|"[1+2],D"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1080000
block|,
literal|0x00f0201f
block|,
literal|"[1],F"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+%g0],d */
block|{
literal|"ld"
block|,
literal|0xc1082000
block|,
literal|0x00f01fff
block|,
literal|"[1],F"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+0],d */
block|{
literal|"ld"
block|,
literal|0xc1082000
block|,
literal|0x00f00000
block|,
literal|"[1+i],F"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1082000
block|,
literal|0x00f00000
block|,
literal|"[i+1],F"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1080000
block|,
literal|0x00f02000
block|,
literal|"[1+2],F"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1000000
block|,
literal|0x00f8201f
block|,
literal|"[1],g"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+%g0],d */
block|{
literal|"ld"
block|,
literal|0xc1002000
block|,
literal|0x00f81fff
block|,
literal|"[1],g"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+0],d */
block|{
literal|"ld"
block|,
literal|0xc1002000
block|,
literal|0x00f80000
block|,
literal|"[1+i],g"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1002000
block|,
literal|0x00f80000
block|,
literal|"[i+1],g"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc1000000
block|,
literal|0x00f82000
block|,
literal|"[1+2],g"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc0000000
block|,
literal|0x01f8201f
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+%g0],d */
block|{
literal|"ld"
block|,
literal|0xc0002000
block|,
literal|0x01f81fff
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ld [rs1+0],d */
block|{
literal|"ld"
block|,
literal|0xc0002000
block|,
literal|0x01f80000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc0002000
block|,
literal|0x01f80000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ld"
block|,
literal|0xc0000000
block|,
literal|0x01f82000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldstuba"
block|,
literal|0xc0d80000
block|,
literal|0x0100201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* ldstuba [rs1+%g0],d */
block|{
literal|"ldstuba"
block|,
literal|0xc0d80000
block|,
literal|0x01002000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsha"
block|,
literal|0xc0d00000
block|,
literal|0x0128201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* ldsha [rs1+%g0],d */
block|{
literal|"ldsha"
block|,
literal|0xc0d00000
block|,
literal|0x01282000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsba"
block|,
literal|0xc0c80000
block|,
literal|0x0130201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* ldsba [rs1+%g0],d */
block|{
literal|"ldsba"
block|,
literal|0xc0c80000
block|,
literal|0x01302000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldda"
block|,
literal|0xc0980000
block|,
literal|0x0160201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* ldda [rs1+%g0],d */
block|{
literal|"ldda"
block|,
literal|0xc0980000
block|,
literal|0x01602000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"lduha"
block|,
literal|0xc0900000
block|,
literal|0x0168201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* lduha [rs1+%g0],d */
block|{
literal|"lduha"
block|,
literal|0xc0900000
block|,
literal|0x01682000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldstub"
block|,
literal|0xc0680000
block|,
literal|0x0190201f
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldstub [rs1+%g0],d */
block|{
literal|"ldstub"
block|,
literal|0xc0682000
block|,
literal|0x01900000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldstub"
block|,
literal|0xc0682000
block|,
literal|0x01900000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldstub"
block|,
literal|0xc0680000
block|,
literal|0x01902000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"lda"
block|,
literal|0xc0800000
block|,
literal|0x0178201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* lda [rs1+%g0],d */
block|{
literal|"lda"
block|,
literal|0xc0800000
block|,
literal|0x01782000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsh"
block|,
literal|0xc0500000
block|,
literal|0x0000000d
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldsh [rs1+%g0],d */
block|{
literal|"ldsh"
block|,
literal|0xc0502000
block|,
literal|0x01a81fff
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldsh [rs1+0],d */
block|{
literal|"ldsh"
block|,
literal|0xc0502000
block|,
literal|0x01a80000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsh"
block|,
literal|0xc0502000
block|,
literal|0x01a80000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsh"
block|,
literal|0xc0500000
block|,
literal|0x01a82000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsb"
block|,
literal|0xc0480000
block|,
literal|0x01b0201f
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldsb [rs1+%g0],d */
block|{
literal|"ldsb"
block|,
literal|0xc0482000
block|,
literal|0x01b01fff
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldsb [rs1+0],d */
block|{
literal|"ldsb"
block|,
literal|0xc0482000
block|,
literal|0x01b00000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsb"
block|,
literal|0xc0482000
block|,
literal|0x01b00000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldsb"
block|,
literal|0xc0480000
block|,
literal|0x01b02000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldub"
block|,
literal|0xc0080000
block|,
literal|0x01f0201f
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldub [rs1+%g0],d */
block|{
literal|"ldub"
block|,
literal|0xc0082000
block|,
literal|0x01f01fff
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* ldub [rs1+0],d */
block|{
literal|"ldub"
block|,
literal|0xc0082000
block|,
literal|0x01f00000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldub"
block|,
literal|0xc0082000
block|,
literal|0x01f00000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"ldub"
block|,
literal|0xc0080000
block|,
literal|0x01f02000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"lduba"
block|,
literal|0xc0880000
block|,
literal|0x0170201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* lduba [rs1+%g0],d */
block|{
literal|"lduba"
block|,
literal|0xc0880000
block|,
literal|0x01702000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"lduh"
block|,
literal|0xc0102000
block|,
literal|0x01e80000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"lduh"
block|,
literal|0xc0102000
block|,
literal|0x01e80000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"lduh"
block|,
literal|0xc0100000
block|,
literal|0x01e8201f
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* lduh [rs1+%g0],d */
block|{
literal|"lduh"
block|,
literal|0xc0102000
block|,
literal|0x01e81fff
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* lduh [rs1+0],d */
block|{
literal|"lduh"
block|,
literal|0xc0100000
block|,
literal|0x01e82000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc0200000
block|,
literal|0x01d8201f
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+%g0] */
block|{
literal|"st"
block|,
literal|0xc0202000
block|,
literal|0x01d81fff
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+0] */
block|{
literal|"st"
block|,
literal|0xc0202000
block|,
literal|0x01d80000
block|,
literal|"d,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc0202000
block|,
literal|0x01d80000
block|,
literal|"d,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc0200000
block|,
literal|0x01d82000
block|,
literal|"d,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1200000
block|,
literal|0x00d8201f
block|,
literal|"g,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d[rs1+%g0] */
block|{
literal|"st"
block|,
literal|0xc1202000
block|,
literal|0x00d81fff
block|,
literal|"g,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+0] */
block|{
literal|"st"
block|,
literal|0xc1202000
block|,
literal|0x00d80000
block|,
literal|"g,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1202000
block|,
literal|0x00d80000
block|,
literal|"g,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1200000
block|,
literal|0x00d82000
block|,
literal|"g,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1280000
block|,
literal|0x00c0d01f
block|,
literal|"F,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+%g0] */
block|{
literal|"st"
block|,
literal|0xc1282000
block|,
literal|0x00c0dfff
block|,
literal|"F,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+0] */
block|{
literal|"st"
block|,
literal|0xc1282000
block|,
literal|0x00c0d000
block|,
literal|"F,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1282000
block|,
literal|0x00c0d000
block|,
literal|"F,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1280000
block|,
literal|0x00c0d000
block|,
literal|"F,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1a00000
block|,
literal|0x0058201f
block|,
literal|"D,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+%g0] */
block|{
literal|"st"
block|,
literal|0xc1a02000
block|,
literal|0x00581fff
block|,
literal|"D,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+0] */
block|{
literal|"st"
block|,
literal|0xc1a02000
block|,
literal|0x00580000
block|,
literal|"D,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1a02000
block|,
literal|0x00580000
block|,
literal|"D,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1a00000
block|,
literal|0x00582000
block|,
literal|"D,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1a80000
block|,
literal|0x0050201f
block|,
literal|"C,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+%g0] */
block|{
literal|"st"
block|,
literal|0xc1a82000
block|,
literal|0x00501fff
block|,
literal|"C,[1]"
block|,
literal|0
block|}
block|,
comment|/* st d,[rs1+0] */
block|{
literal|"st"
block|,
literal|0xc1a82000
block|,
literal|0x00500000
block|,
literal|"C,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1a82000
block|,
literal|0x00500000
block|,
literal|"C,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"st"
block|,
literal|0xc1a80000
block|,
literal|0x00502000
block|,
literal|"C,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"sta"
block|,
literal|0xc0a00000
block|,
literal|0x0108201f
block|,
literal|"d,[1]A"
block|,
literal|0
block|}
block|,
comment|/* sta d,[rs1+%g0] */
block|{
literal|"sta"
block|,
literal|0xc0a00000
block|,
literal|0x01082000
block|,
literal|"d,[1+2]A"
block|,
literal|0
block|}
block|,
block|{
literal|"stb"
block|,
literal|0xc0280000
block|,
literal|0x01d0201f
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* stb d,[rs1+%g0] */
block|{
literal|"stb"
block|,
literal|0xc0282000
block|,
literal|0x01d01fff
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* stb d,[rs1+0] */
block|{
literal|"stb"
block|,
literal|0xc0282000
block|,
literal|0x01d00000
block|,
literal|"d,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"stb"
block|,
literal|0xc0282000
block|,
literal|0x01d00000
block|,
literal|"d,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"stb"
block|,
literal|0xc0280000
block|,
literal|0x01d02000
block|,
literal|"d,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"stba"
block|,
literal|0xc0a80000
block|,
literal|0x01002000
block|,
literal|"d,[1+2]A"
block|,
literal|0
block|}
block|,
block|{
literal|"stba"
block|,
literal|0xc0a80000
block|,
literal|0x0100201f
block|,
literal|"d,[1]A"
block|,
literal|0
block|}
block|,
comment|/* stba d,[rs1+%g0] */
block|{
literal|"std"
block|,
literal|0xc0380000
block|,
literal|0x01c0201f
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+%g0] */
block|{
literal|"std"
block|,
literal|0xc0382000
block|,
literal|0x01c01fff
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+0] */
block|{
literal|"std"
block|,
literal|0xc0382000
block|,
literal|0x01c00000
block|,
literal|"d,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc0382000
block|,
literal|0x01c00000
block|,
literal|"d,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc0380000
block|,
literal|0x01c02000
block|,
literal|"d,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1380000
block|,
literal|0x00c0201f
block|,
literal|"g,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+%g0] */
block|{
literal|"std"
block|,
literal|0xc1382000
block|,
literal|0x00c01fff
block|,
literal|"g,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+0] */
block|{
literal|"std"
block|,
literal|0xc1382000
block|,
literal|0x00c00000
block|,
literal|"g,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1382000
block|,
literal|0x00c00000
block|,
literal|"g,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1380000
block|,
literal|0x00c02000
block|,
literal|"g,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1300000
block|,
literal|0x00c8201f
block|,
literal|"q,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+%g0] */
block|{
literal|"std"
block|,
literal|0xc1302000
block|,
literal|0x00c81fff
block|,
literal|"q,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+0] */
block|{
literal|"std"
block|,
literal|0xc1302000
block|,
literal|0x00c80000
block|,
literal|"q,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1302000
block|,
literal|0x00c80000
block|,
literal|"q,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1300000
block|,
literal|0x00c82000
block|,
literal|"q,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1b80000
block|,
literal|0x0040201f
block|,
literal|"D,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+%g0] */
block|{
literal|"std"
block|,
literal|0xc1b82000
block|,
literal|0x00401fff
block|,
literal|"D,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+0] */
block|{
literal|"std"
block|,
literal|0xc1b82000
block|,
literal|0x00400000
block|,
literal|"D,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1b82000
block|,
literal|0x00400000
block|,
literal|"D,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1b80000
block|,
literal|0x00402000
block|,
literal|"D,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1b00000
block|,
literal|0x0048201f
block|,
literal|"Q,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+%g0] */
block|{
literal|"std"
block|,
literal|0xc1b02000
block|,
literal|0x00481fff
block|,
literal|"Q,[1]"
block|,
literal|0
block|}
block|,
comment|/* std d,[rs1+0] */
block|{
literal|"std"
block|,
literal|0xc1b02000
block|,
literal|0x00480000
block|,
literal|"Q,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1b02000
block|,
literal|0x00480000
block|,
literal|"Q,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"std"
block|,
literal|0xc1b00000
block|,
literal|0x00482000
block|,
literal|"Q,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"stda"
block|,
literal|0xc0b80000
block|,
literal|0x01402000
block|,
literal|"d,[1+2]A"
block|,
literal|0
block|}
block|,
block|{
literal|"stda"
block|,
literal|0xc0b80000
block|,
literal|0x0140201f
block|,
literal|"d,[1]A"
block|,
literal|0
block|}
block|,
comment|/* stda d,[rs1+%g0] */
block|{
literal|"sth"
block|,
literal|0xc0300000
block|,
literal|0x01c8201f
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* sth d,[rs1+%g0] */
block|{
literal|"sth"
block|,
literal|0xc0302000
block|,
literal|0x01c81fff
block|,
literal|"d,[1]"
block|,
literal|0
block|}
block|,
comment|/* sth d,[rs1+0] */
block|{
literal|"sth"
block|,
literal|0xc0300000
block|,
literal|0x01c82000
block|,
literal|"d,[1+2]"
block|,
literal|0
block|}
block|,
block|{
literal|"sth"
block|,
literal|0xc0302000
block|,
literal|0x01c80000
block|,
literal|"d,[1+i]"
block|,
literal|0
block|}
block|,
block|{
literal|"sth"
block|,
literal|0xc0302000
block|,
literal|0x01c80000
block|,
literal|"d,[i+1]"
block|,
literal|0
block|}
block|,
block|{
literal|"stha"
block|,
literal|0xc0b00000
block|,
literal|0x0148201f
block|,
literal|"d,[1]A"
block|,
literal|0
block|}
block|,
comment|/* stha d,[rs1+%g0] */
block|{
literal|"stha"
block|,
literal|0xc0b00000
block|,
literal|0x01482000
block|,
literal|"d,[1+2]A"
block|,
literal|0
block|}
block|,
block|{
literal|"swap"
block|,
literal|0xc0780000
block|,
literal|0x0180201f
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* swap [rs1+%g0],d */
block|{
literal|"swap"
block|,
literal|0xc0782000
block|,
literal|0x01801fff
block|,
literal|"[1],d"
block|,
literal|0
block|}
block|,
comment|/* swap [rs1+0],d */
block|{
literal|"swap"
block|,
literal|0xc0782000
block|,
literal|0x01800000
block|,
literal|"[1+i],d"
block|,
literal|0
block|}
block|,
block|{
literal|"swap"
block|,
literal|0xc0782000
block|,
literal|0x01800000
block|,
literal|"[i+1],d"
block|,
literal|0
block|}
block|,
block|{
literal|"swap"
block|,
literal|0xc0780000
block|,
literal|0x01802000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"swapa"
block|,
literal|0xc0f80000
block|,
literal|0x01002000
block|,
literal|"[1+2]A,d"
block|,
literal|0
block|}
block|,
block|{
literal|"swapa"
block|,
literal|0xc0f80000
block|,
literal|0x0100201f
block|,
literal|"[1]A,d"
block|,
literal|0
block|}
block|,
comment|/* swapa [rs1+%g0],d */
block|{
literal|"restore"
block|,
literal|0x81e80000
block|,
literal|0x7e17e01f
block|,
literal|""
block|,
literal|0
block|}
block|,
comment|/* restore %g0,%g0,%g0 */
block|{
literal|"restore"
block|,
literal|0x81e82000
block|,
literal|0x7e14dfff
block|,
literal|""
block|,
literal|0
block|}
block|,
comment|/* restore %g0,0,%g0 */
block|{
literal|"restore"
block|,
literal|0x81e82000
block|,
literal|0x00000000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"restore"
block|,
literal|0x81e80000
block|,
literal|0x00000000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"rett"
block|,
literal|0x81c82000
block|,
literal|0x40300000
block|,
literal|"1+i"
block|,
literal|1
block|}
block|,
block|{
literal|"rett"
block|,
literal|0x81c82000
block|,
literal|0x40300000
block|,
literal|"i+1"
block|,
literal|1
block|}
block|,
block|{
literal|"rett"
block|,
literal|0x81c80000
block|,
literal|0x40302000
block|,
literal|"1+2"
block|,
literal|1
block|}
block|,
block|{
literal|"rett"
block|,
literal|0x81c82000
block|,
literal|0x40300000
block|,
literal|"1"
block|,
literal|1
block|}
block|,
block|{
literal|"save"
block|,
literal|0x81e02000
block|,
literal|0x40180000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"save"
block|,
literal|0x81e00000
block|,
literal|0x40180000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"ret"
block|,
literal|0x81c7e008
block|,
literal|0x00001ff7
block|,
literal|""
block|,
literal|1
block|}
block|,
comment|/* jmpl %i7+8,%g0 */
block|{
literal|"retl"
block|,
literal|0x81c3e008
block|,
literal|0x00001ff7
block|,
literal|""
block|,
literal|1
block|}
block|,
comment|/* jmpl %o7+8,%g0 */
block|{
literal|"jmpl"
block|,
literal|0x81c00000
block|,
literal|0x4038201f
block|,
literal|"1,d"
block|,
literal|1
block|}
block|,
comment|/* jmpl rs1+%g0,d */
block|{
literal|"jmpl"
block|,
literal|0x81c02000
block|,
literal|0x4037c000
block|,
literal|"i,d"
block|,
literal|1
block|}
block|,
comment|/* jmpl %g0+i,d */
block|{
literal|"jmpl"
block|,
literal|0x81c02000
block|,
literal|0x40380000
block|,
literal|"1+i,d"
block|,
literal|1
block|}
block|,
block|{
literal|"jmpl"
block|,
literal|0x81c02000
block|,
literal|0x40380000
block|,
literal|"i+1,d"
block|,
literal|1
block|}
block|,
block|{
literal|"jmpl"
block|,
literal|0x81c00000
block|,
literal|0x40382000
block|,
literal|"1+2,d"
block|,
literal|1
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81982000
block|,
literal|0x40600000
block|,
literal|"1,i,t"
block|,
literal|0
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81980000
block|,
literal|0x40600000
block|,
literal|"1,2,t"
block|,
literal|0
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81902000
block|,
literal|0x40680000
block|,
literal|"1,i,w"
block|,
literal|0
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81900000
block|,
literal|0x40680000
block|,
literal|"1,2,w"
block|,
literal|0
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81882000
block|,
literal|0x40700000
block|,
literal|"1,i,p"
block|,
literal|0
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81880000
block|,
literal|0x40700000
block|,
literal|"1,2,p"
block|,
literal|0
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81802000
block|,
literal|0x40780000
block|,
literal|"1,i,y"
block|,
literal|0
block|}
block|,
block|{
literal|"wr"
block|,
literal|0x81800000
block|,
literal|0x40780000
block|,
literal|"1,2,y"
block|,
literal|0
block|}
block|,
block|{
literal|"rd"
block|,
literal|0x81580000
block|,
literal|0x40a00000
block|,
literal|"t,d"
block|,
literal|0
block|}
block|,
block|{
literal|"rd"
block|,
literal|0x81500000
block|,
literal|0x40a80000
block|,
literal|"w,d"
block|,
literal|0
block|}
block|,
block|{
literal|"rd"
block|,
literal|0x81480000
block|,
literal|0x40b00000
block|,
literal|"p,d"
block|,
literal|0
block|}
block|,
block|{
literal|"rd"
block|,
literal|0x81400000
block|,
literal|0x40b80000
block|,
literal|"y,d"
block|,
literal|0
block|}
block|,
block|{
literal|"sra"
block|,
literal|0x81382000
block|,
literal|0x00000000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"sra"
block|,
literal|0x81380000
block|,
literal|0x00000000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"srl"
block|,
literal|0x81302000
block|,
literal|0x40c80000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"srl"
block|,
literal|0x81300000
block|,
literal|0x40c80000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"sll"
block|,
literal|0x81282000
block|,
literal|0x40d00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"sll"
block|,
literal|0x81280000
block|,
literal|0x40d00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"mulscc"
block|,
literal|0x81202000
block|,
literal|0x40d80000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"mulscc"
block|,
literal|0x81200000
block|,
literal|0x40d80000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"clr"
block|,
literal|0x80100000
block|,
literal|0x4e87e01f
block|,
literal|"d"
block|,
literal|0
block|}
block|,
comment|/* or %g0,%g0,d */
block|{
literal|"clr"
block|,
literal|0x80102000
block|,
literal|0x41efdfff
block|,
literal|"d"
block|,
literal|0
block|}
block|,
comment|/* or %g0,0,d   */
block|{
literal|"orncc"
block|,
literal|0x80b02000
block|,
literal|0x04048000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"orncc"
block|,
literal|0x80b02000
block|,
literal|0x04048000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"orncc"
block|,
literal|0x80b00000
block|,
literal|0x04048000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"tst"
block|,
literal|0x80900000
block|,
literal|0x7f6fe000
block|,
literal|"2"
block|,
literal|0
block|}
block|,
comment|/* orcc %g0, rs2, %g0 */
block|{
literal|"tst"
block|,
literal|0x80900000
block|,
literal|0x7f68201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* orcc rs1, %g0, %g0 */
block|{
literal|"tst"
block|,
literal|0x80902000
block|,
literal|0x7f681fff
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* orcc rs1, 0, %g0 */
block|{
literal|"orcc"
block|,
literal|0x80902000
block|,
literal|0x41680000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"orcc"
block|,
literal|0x80902000
block|,
literal|0x41680000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"orcc"
block|,
literal|0x80900000
block|,
literal|0x41680000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"orn"
block|,
literal|0x80302000
block|,
literal|0x41c80000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"orn"
block|,
literal|0x80302000
block|,
literal|0x41c80000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"orn"
block|,
literal|0x80300000
block|,
literal|0x41c80000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"mov"
block|,
literal|0x81800000
block|,
literal|0x4078201f
block|,
literal|"1,y"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,%g0,%y */
block|{
literal|"mov"
block|,
literal|0x81802000
block|,
literal|0x40781fff
block|,
literal|"1,y"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,0,%y */
block|{
literal|"mov"
block|,
literal|0x81400000
block|,
literal|0x40b80000
block|,
literal|"y,d"
block|,
literal|0
block|}
block|,
comment|/* rd %y,d */
block|{
literal|"mov"
block|,
literal|0x81980000
block|,
literal|0x4060201f
block|,
literal|"1,t"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,%g0,%tbr */
block|{
literal|"mov"
block|,
literal|0x81982000
block|,
literal|0x40601fff
block|,
literal|"1,t"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,0,%tbr */
block|{
literal|"mov"
block|,
literal|0x81580000
block|,
literal|0x40a00000
block|,
literal|"t,d"
block|,
literal|0
block|}
block|,
comment|/* rd %tbr,d */
block|{
literal|"mov"
block|,
literal|0x81900000
block|,
literal|0x4068201f
block|,
literal|"1,w"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,%g0,%wim */
block|{
literal|"mov"
block|,
literal|0x81902000
block|,
literal|0x40681fff
block|,
literal|"1,w"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,0,%wim */
block|{
literal|"mov"
block|,
literal|0x81500000
block|,
literal|0x40a80000
block|,
literal|"w,d"
block|,
literal|0
block|}
block|,
comment|/* rd %wim,d */
block|{
literal|"mov"
block|,
literal|0x81880000
block|,
literal|0x4070201f
block|,
literal|"1,p"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,%g0,%psr */
block|{
literal|"mov"
block|,
literal|0x81882000
block|,
literal|0x40701fff
block|,
literal|"1,p"
block|,
literal|0
block|}
block|,
comment|/* wr rs1,0,%psr */
block|{
literal|"mov"
block|,
literal|0x81480000
block|,
literal|0x40b00000
block|,
literal|"p,d"
block|,
literal|0
block|}
block|,
comment|/* rd %psr,d */
block|{
literal|"mov"
block|,
literal|0x80102000
block|,
literal|0x41efc000
block|,
literal|"i,d"
block|,
literal|0
block|}
block|,
comment|/* or %g0,i,d   */
block|{
literal|"mov"
block|,
literal|0x80100000
block|,
literal|0x41efe000
block|,
literal|"2,d"
block|,
literal|0
block|}
block|,
comment|/* or %g0,rs2,d */
block|{
literal|"or"
block|,
literal|0x80102000
block|,
literal|0x40800000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"or"
block|,
literal|0x80102000
block|,
literal|0x40800000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"or"
block|,
literal|0x80100000
block|,
literal|0x40800000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"bset"
block|,
literal|0x80100000
block|,
literal|0x40800000
block|,
literal|"2,r"
block|,
literal|0
block|}
block|,
comment|/* or rsd,rs2,rsd */
block|{
literal|"bset"
block|,
literal|0x80102000
block|,
literal|0x40800000
block|,
literal|"i,r"
block|,
literal|0
block|}
block|,
comment|/* or rsd,i,rsd */
block|{
literal|"andncc"
block|,
literal|0x80a82000
block|,
literal|0x41500000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andncc"
block|,
literal|0x80a82000
block|,
literal|0x41500000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andncc"
block|,
literal|0x80a80000
block|,
literal|0x41500000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andn"
block|,
literal|0x80282000
block|,
literal|0x41d00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andn"
block|,
literal|0x80282000
block|,
literal|0x41d00000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andn"
block|,
literal|0x80280000
block|,
literal|0x41d00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"bclr"
block|,
literal|0x80280000
block|,
literal|0x41d00000
block|,
literal|"2,r"
block|,
literal|0
block|}
block|,
comment|/* andn rsd,rs2,rsd */
block|{
literal|"bclr"
block|,
literal|0x80282000
block|,
literal|0x41d00000
block|,
literal|"i,r"
block|,
literal|0
block|}
block|,
comment|/* andn rsd,i,rsd */
block|{
literal|"cmp"
block|,
literal|0x80a02000
block|,
literal|0x7d580000
block|,
literal|"1,i"
block|,
literal|0
block|}
block|,
comment|/* subcc rs1,i,%g0 */
block|{
literal|"cmp"
block|,
literal|0x80a00000
block|,
literal|0x7d580000
block|,
literal|"1,2"
block|,
literal|0
block|}
block|,
comment|/* subcc rs1,rs2,%g0 */
block|{
literal|"deccc"
block|,
literal|0x80a02001
block|,
literal|0x41581ffe
block|,
literal|"r"
block|,
literal|0
block|}
block|,
comment|/* subcc rs1,1,rsd */
block|{
literal|"deccc"
block|,
literal|0x80a02000
block|,
literal|0x41581fff
block|,
literal|"i,r"
block|,
literal|0
block|}
block|,
comment|/* subcc rs1,i,rsd */
block|{
literal|"dec"
block|,
literal|0x80202001
block|,
literal|0x41d81ffe
block|,
literal|"r"
block|,
literal|0
block|}
block|,
comment|/* sub rs1,1,rsd */
block|{
literal|"dec"
block|,
literal|0x80202000
block|,
literal|0x41d81fff
block|,
literal|"i,r"
block|,
literal|0
block|}
block|,
comment|/* etc */
block|{
literal|"neg"
block|,
literal|0x80200000
block|,
literal|0x41d87800
block|,
literal|"2,d"
block|,
literal|0
block|}
block|,
comment|/* sub %g0,rs2,rsd */
block|{
literal|"neg"
block|,
literal|0x80200000
block|,
literal|0x41d87800
block|,
literal|"R"
block|,
literal|0
block|}
block|,
comment|/* same, but rsd=rs2 */
block|{
literal|"subcc"
block|,
literal|0x80a02000
block|,
literal|0x41580000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"subcc"
block|,
literal|0x80a00000
block|,
literal|0x41580000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"sub"
block|,
literal|0x80202000
block|,
literal|0x41d80000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"sub"
block|,
literal|0x80200000
block|,
literal|0x41d80000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"subx"
block|,
literal|0x80602000
block|,
literal|0x41980000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"subx"
block|,
literal|0x80600000
block|,
literal|0x41980000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"subxcc"
block|,
literal|0x80e02000
block|,
literal|0x41180000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"subxcc"
block|,
literal|0x80e00000
block|,
literal|0x41180000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andcc"
block|,
literal|0x80882000
block|,
literal|0x41700000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andcc"
block|,
literal|0x80882000
block|,
literal|0x41700000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"andcc"
block|,
literal|0x80880000
block|,
literal|0x41700000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"and"
block|,
literal|0x80082000
block|,
literal|0x41f00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"and"
block|,
literal|0x80082000
block|,
literal|0x41f00000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"and"
block|,
literal|0x80080000
block|,
literal|0x41f00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"btst"
block|,
literal|0x80880000
block|,
literal|0x41700000
block|,
literal|"1,2"
block|,
literal|0
block|}
block|,
comment|/* andcc rs1,rs2,%g0 */
block|{
literal|"btst"
block|,
literal|0x80882000
block|,
literal|0x41700000
block|,
literal|"i,1"
block|,
literal|0
block|}
block|,
comment|/* andcc rs1,i,%g0 */
block|{
literal|"inccc"
block|,
literal|0x80802001
block|,
literal|0x41781ffe
block|,
literal|"r"
block|,
literal|0
block|}
block|,
comment|/* addcc rs1,1,rsd */
block|{
literal|"inccc"
block|,
literal|0x80802000
block|,
literal|0x41781fff
block|,
literal|"i,r"
block|,
literal|0
block|}
block|,
comment|/* addcc rs1,i,rsd */
block|{
literal|"inc"
block|,
literal|0x80002001
block|,
literal|0x41f81ffe
block|,
literal|"r"
block|,
literal|0
block|}
block|,
comment|/* add rs1,1,rsd */
block|{
literal|"inc"
block|,
literal|0x80002000
block|,
literal|0x41f81fff
block|,
literal|"i,r"
block|,
literal|0
block|}
block|,
comment|/* add rs1,i,rsd */
block|{
literal|"addxcc"
block|,
literal|0x80c02000
block|,
literal|0x41380000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addxcc"
block|,
literal|0x80c02000
block|,
literal|0x41380000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addxcc"
block|,
literal|0x80c00000
block|,
literal|0x41380000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addcc"
block|,
literal|0x80802000
block|,
literal|0x41780000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addcc"
block|,
literal|0x80802000
block|,
literal|0x41780000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addcc"
block|,
literal|0x80800000
block|,
literal|0x41780000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addx"
block|,
literal|0x80402000
block|,
literal|0x41b80000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addx"
block|,
literal|0x80402000
block|,
literal|0x41b80000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"addx"
block|,
literal|0x80400000
block|,
literal|0x41b80000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"add"
block|,
literal|0x80002000
block|,
literal|0x41f80000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"add"
block|,
literal|0x80002000
block|,
literal|0x41f80000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"add"
block|,
literal|0x80000000
block|,
literal|0x41f80000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"call"
block|,
literal|0x9fc00000
block|,
literal|0x4038201f
block|,
literal|"1"
block|,
literal|1
block|}
block|,
comment|/* jmpl rs1+%g0, %o7 */
block|{
literal|"call"
block|,
literal|0x9fc00000
block|,
literal|0x4038201f
block|,
literal|"1,#"
block|,
literal|1
block|}
block|,
block|{
literal|"call"
block|,
literal|0x40000000
block|,
literal|0x80000000
block|,
literal|"L"
block|,
literal|1
block|}
block|,
block|{
literal|"call"
block|,
literal|0x40000000
block|,
literal|0x80000000
block|,
literal|"L,#"
block|,
literal|1
block|}
block|,
block|{
literal|"bvc"
block|,
literal|0x3e800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bvc"
block|,
literal|0x1e800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bvs"
block|,
literal|0x2e800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bvs"
block|,
literal|0x0e800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bpos"
block|,
literal|0x3c800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bpos"
block|,
literal|0x1c800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bneg"
block|,
literal|0x2c800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bneg"
block|,
literal|0x0c800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bcc"
block|,
literal|0x3a800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bcc"
block|,
literal|0x1a800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bcs"
block|,
literal|0x2a800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bcs"
block|,
literal|0x0a800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"blu"
block|,
literal|0x2a800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"blu"
block|,
literal|0x0a800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
comment|/* same as bcs */
block|{
literal|"bgeu"
block|,
literal|0x3a800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bgeu"
block|,
literal|0x1a800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
comment|/* same as bcc */
block|{
literal|"bgu"
block|,
literal|0x38800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bgu"
block|,
literal|0x18800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bleu"
block|,
literal|0x28800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bleu"
block|,
literal|0x08800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bge"
block|,
literal|0x36800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bge"
block|,
literal|0x16800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bl"
block|,
literal|0x26800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bl"
block|,
literal|0x06800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bg"
block|,
literal|0x34800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bg"
block|,
literal|0x14800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"ble"
block|,
literal|0x24800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"ble"
block|,
literal|0x04800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"be"
block|,
literal|0x22800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"be"
block|,
literal|0x02800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bz"
block|,
literal|0x22800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bz"
block|,
literal|0x02800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bne"
block|,
literal|0x32800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bne"
block|,
literal|0x12800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bnz"
block|,
literal|0x32800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bnz"
block|,
literal|0x12800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"b"
block|,
literal|0x30800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"b"
block|,
literal|0x10800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"ba"
block|,
literal|0x30800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"ba"
block|,
literal|0x10800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"bn"
block|,
literal|0x20800000
block|,
literal|0xc1400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"bn"
block|,
literal|0x00800000
block|,
literal|0xc1400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"jmp"
block|,
literal|0x81c00000
block|,
literal|0x7e38201f
block|,
literal|"1"
block|,
literal|1
block|}
block|,
comment|/* jmpl rs1+%g0,%g0 */
block|{
literal|"jmp"
block|,
literal|0x81c02000
block|,
literal|0x7e3fc000
block|,
literal|"i"
block|,
literal|1
block|}
block|,
comment|/* jmpl %g0+i,%g0 */
block|{
literal|"jmp"
block|,
literal|0x81c00000
block|,
literal|0x7e382000
block|,
literal|"1+2"
block|,
literal|1
block|}
block|,
comment|/* jmpl rs1+rs2,%g0 */
block|{
literal|"jmp"
block|,
literal|0x81c02000
block|,
literal|0x7e380000
block|,
literal|"1+i"
block|,
literal|1
block|}
block|,
comment|/* jmpl rs1+i,%g0 */
block|{
literal|"jmp"
block|,
literal|0x81c02000
block|,
literal|0x7e380000
block|,
literal|"i+1"
block|,
literal|1
block|}
block|,
comment|/* jmpl i+rs1,%g0 */
block|{
literal|"nop"
block|,
literal|0x01000000
block|,
literal|0xfe3fffff
block|,
literal|""
block|,
literal|0
block|}
block|,
comment|/* sethi 0, %g0 */
block|{
literal|"set"
block|,
literal|0x01000000
block|,
literal|0xc0c00000
block|,
literal|"Sh,d"
block|,
literal|0
block|}
block|,
block|{
literal|"sethi"
block|,
literal|0x01000000
block|,
literal|0xc0c00000
block|,
literal|"h,d"
block|,
literal|0
block|}
block|,
block|{
literal|"taddcctv"
block|,
literal|0x81102000
block|,
literal|0x40e00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"taddcctv"
block|,
literal|0x81100000
block|,
literal|0x40e00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"taddcc"
block|,
literal|0x81002000
block|,
literal|0x40f80000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"taddcc"
block|,
literal|0x81000000
block|,
literal|0x40f80000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"tvc"
block|,
literal|0x9fd02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tvc %g0+i */
block|{
literal|"tvc"
block|,
literal|0x9fd02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tvc"
block|,
literal|0x9fd00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tvc"
block|,
literal|0x9fd00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tvc rs1+%g0 */
block|{
literal|"tpos"
block|,
literal|0x9dd02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tpos %g0+i */
block|{
literal|"tpos"
block|,
literal|0x9dd02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tpos"
block|,
literal|0x9dd00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tpos"
block|,
literal|0x9dd00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tpos rs1+%g0 */
block|{
literal|"tcc"
block|,
literal|0x9bd02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tcc %g0+i */
block|{
literal|"tcc"
block|,
literal|0x9bd02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tcc"
block|,
literal|0x9bd00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tcc"
block|,
literal|0x9bd00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tcc rs1+%g0 */
block|{
literal|"tgu"
block|,
literal|0x99d02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tgu %g0+i */
block|{
literal|"tgu"
block|,
literal|0x99d02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tgu"
block|,
literal|0x99d00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tgu"
block|,
literal|0x99d00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tgu rs1+%g0 */
block|{
literal|"tge"
block|,
literal|0x97d02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tge %g0+i */
block|{
literal|"tge"
block|,
literal|0x97d02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tge"
block|,
literal|0x97d00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tge"
block|,
literal|0x97d00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tge rs1+%g0 */
block|{
literal|"tg"
block|,
literal|0x95d02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tg %g0+i */
block|{
literal|"tg"
block|,
literal|0x95d02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tg"
block|,
literal|0x95d00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tg"
block|,
literal|0x95d00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tg rs1+%g0 */
block|{
literal|"tne"
block|,
literal|0x93d02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tne %g0+i */
block|{
literal|"tne"
block|,
literal|0x93d02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tne"
block|,
literal|0x93d00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tne"
block|,
literal|0x93d00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tne rs1+%g0 */
block|{
literal|"tnz"
block|,
literal|0x93d02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tne %g0+i */
block|{
literal|"tnz"
block|,
literal|0x93d02000
block|,
literal|0x40280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tnz"
block|,
literal|0x93d00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tnz"
block|,
literal|0x93d00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tne rs1+%g0 */
block|{
literal|"tleu"
block|,
literal|0x8bd02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tleu %g0+i */
block|{
literal|"tleu"
block|,
literal|0x8bd02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tleu"
block|,
literal|0x8bd00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tleu"
block|,
literal|0x8bd00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tleu rs1+%g0 */
block|{
literal|"ta"
block|,
literal|0x91d02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* ta %g0+i */
block|{
literal|"ta"
block|,
literal|0x91d02000
block|,
literal|0x402d0000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"ta"
block|,
literal|0x91d00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"ta"
block|,
literal|0x91d00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* ta rs1+%g0 */
block|{
literal|"t"
block|,
literal|0x91d02000
block|,
literal|0x402fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* ta %g0+i */
block|{
literal|"t"
block|,
literal|0x91d02000
block|,
literal|0x402d0000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"t"
block|,
literal|0x91d00000
block|,
literal|0x40282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"t"
block|,
literal|0x91d00000
block|,
literal|0x4028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* ta rs1+%g0 */
block|{
literal|"tvs"
block|,
literal|0x8fd02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tvs %g0+i */
block|{
literal|"tvs"
block|,
literal|0x8fd02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tvs"
block|,
literal|0x8fd00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tvs"
block|,
literal|0x8fd00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tvs rs1+%g0 */
block|{
literal|"tneg"
block|,
literal|0x8dd02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tneg %g0+i */
block|{
literal|"tneg"
block|,
literal|0x8dd02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tneg"
block|,
literal|0x8dd00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tneg"
block|,
literal|0x8dd00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tneg rs1+%g0 */
block|{
literal|"tcs"
block|,
literal|0x8bd02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tcs %g0+i */
block|{
literal|"tcs"
block|,
literal|0x8bd02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tcs"
block|,
literal|0x8bd00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tcs"
block|,
literal|0x8bd00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tcs rs1+%g0 */
block|{
literal|"tl"
block|,
literal|0x87d02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tl %g0+i */
block|{
literal|"tl"
block|,
literal|0x87d02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tl"
block|,
literal|0x87d00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tl"
block|,
literal|0x87d00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tl rs1+%g0 */
block|{
literal|"tle"
block|,
literal|0x85d02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tle %g0+i */
block|{
literal|"tle"
block|,
literal|0x85d02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tle"
block|,
literal|0x85d00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tle"
block|,
literal|0x85d00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tle rs1+%g0 */
block|{
literal|"te"
block|,
literal|0x83d02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* te %g0+i */
block|{
literal|"te"
block|,
literal|0x83d02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"te"
block|,
literal|0x83d00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"te"
block|,
literal|0x83d00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* te rs1+%g0 */
block|{
literal|"tz"
block|,
literal|0x83d02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* te %g0+i */
block|{
literal|"tz"
block|,
literal|0x83d02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tz"
block|,
literal|0x83d00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tz"
block|,
literal|0x83d00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* te rs1+%g0 */
block|{
literal|"tn"
block|,
literal|0x81d02000
block|,
literal|0x502fc000
block|,
literal|"i"
block|,
literal|0
block|}
block|,
comment|/* tn %g0+i */
block|{
literal|"tn"
block|,
literal|0x81d02000
block|,
literal|0x50280000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"tn"
block|,
literal|0x81d00000
block|,
literal|0x50282000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"tn"
block|,
literal|0x81d00000
block|,
literal|0x5028201f
block|,
literal|"1"
block|,
literal|0
block|}
block|,
comment|/* tn rs1+%g0 */
block|{
literal|"tsubcc"
block|,
literal|0x81080000
block|,
literal|0x40f00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"tsubcc"
block|,
literal|0x81082000
block|,
literal|0x40f00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"tsubcctv"
block|,
literal|0x80580000
block|,
literal|0x40a00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"tsubcctv"
block|,
literal|0x80582000
block|,
literal|0x40a00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"unimp"
block|,
literal|0x00000000
block|,
literal|0x00000000
block|,
literal|"l"
block|,
literal|0
block|}
block|,
block|{
literal|"iflush"
block|,
literal|0x81d80000
block|,
literal|0x40202000
block|,
literal|"1+2"
block|,
literal|0
block|}
block|,
block|{
literal|"iflush"
block|,
literal|0x81d82000
block|,
literal|0x40200000
block|,
literal|"1+i"
block|,
literal|0
block|}
block|,
block|{
literal|"xnorcc"
block|,
literal|0x80b80000
block|,
literal|0x41400000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xnorcc"
block|,
literal|0x80b82000
block|,
literal|0x41400000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xnorcc"
block|,
literal|0x80b82000
block|,
literal|0x41400000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xorcc"
block|,
literal|0x80980000
block|,
literal|0x41600000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xorcc"
block|,
literal|0x80982000
block|,
literal|0x41600000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xorcc"
block|,
literal|0x80982000
block|,
literal|0x41600000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xnor"
block|,
literal|0x80380000
block|,
literal|0x41c00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xnor"
block|,
literal|0x80382000
block|,
literal|0x41c00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xnor"
block|,
literal|0x80382000
block|,
literal|0x41c00000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xor"
block|,
literal|0x80180000
block|,
literal|0x41e00000
block|,
literal|"1,2,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xor"
block|,
literal|0x80182000
block|,
literal|0x41e00000
block|,
literal|"1,i,d"
block|,
literal|0
block|}
block|,
block|{
literal|"xor"
block|,
literal|0x80182000
block|,
literal|0x41e00000
block|,
literal|"i,1,d"
block|,
literal|0
block|}
block|,
block|{
literal|"btog"
block|,
literal|0x80180000
block|,
literal|0x41e00000
block|,
literal|"2,r"
block|,
literal|0
block|}
block|,
comment|/* xor rsd,rs2,rsd */
block|{
literal|"btog"
block|,
literal|0x80182000
block|,
literal|0x41e00000
block|,
literal|"i,r"
block|,
literal|0
block|}
block|,
comment|/* xor rsd,i,rsd */
block|{
literal|"not"
block|,
literal|0x80380000
block|,
literal|0x41c00000
block|,
literal|"1,d"
block|,
literal|0
block|}
block|,
comment|/* xnor rs1,%g0,rsd */
block|{
literal|"not"
block|,
literal|0x80380000
block|,
literal|0x41c00000
block|,
literal|"r"
block|,
literal|0
block|}
block|,
comment|/* xnor rs1,%g0,rsd */
block|{
literal|"fpop1"
block|,
literal|0x81a00000
block|,
literal|0x40580000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"fpop2"
block|,
literal|0x81a80000
block|,
literal|0x40500000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"fb"
block|,
literal|0x31800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fb"
block|,
literal|0x11800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fba"
block|,
literal|0x31800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fba"
block|,
literal|0x11800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbn"
block|,
literal|0x21800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbn"
block|,
literal|0x01800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbu"
block|,
literal|0x2f800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbu"
block|,
literal|0x0f800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbg"
block|,
literal|0x2d800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbg"
block|,
literal|0x0d800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbug"
block|,
literal|0x2b800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbug"
block|,
literal|0x0b800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbl"
block|,
literal|0x29800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbl"
block|,
literal|0x09800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbul"
block|,
literal|0x27800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbul"
block|,
literal|0x07800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fblg"
block|,
literal|0x25800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fblg"
block|,
literal|0x05800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbne"
block|,
literal|0x23800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbne"
block|,
literal|0x03800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbe"
block|,
literal|0x33800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbe"
block|,
literal|0x13800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbue"
block|,
literal|0x35800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbue"
block|,
literal|0x15800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbge"
block|,
literal|0x37800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbge"
block|,
literal|0x17800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbuge"
block|,
literal|0x39800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbuge"
block|,
literal|0x19800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fble"
block|,
literal|0x3b800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fble"
block|,
literal|0x1b800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbule"
block|,
literal|0x3d800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbule"
block|,
literal|0x1d800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fbo"
block|,
literal|0x3f800000
block|,
literal|0xc0400000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"fbo"
block|,
literal|0x1f800000
block|,
literal|0xc0400000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cba"
block|,
literal|0x31c00000
block|,
literal|0xce000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cba"
block|,
literal|0x11c00000
block|,
literal|0xce000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cbn"
block|,
literal|0x21c00000
block|,
literal|0xde000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cbn"
block|,
literal|0x01c00000
block|,
literal|0xde000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb3"
block|,
literal|0x2fc00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb3"
block|,
literal|0x0fc00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb2"
block|,
literal|0x2dc00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb2"
block|,
literal|0x0dc00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb23"
block|,
literal|0x2bc00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb23"
block|,
literal|0x0bc00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb1"
block|,
literal|0x29c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb1"
block|,
literal|0x09c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb13"
block|,
literal|0x27c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb13"
block|,
literal|0x07c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb12"
block|,
literal|0x25c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb12"
block|,
literal|0x05c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb123"
block|,
literal|0x23c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb123"
block|,
literal|0x03c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb0"
block|,
literal|0x33c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb0"
block|,
literal|0x13c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb03"
block|,
literal|0x35c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb03"
block|,
literal|0x15c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb02"
block|,
literal|0x37c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb02"
block|,
literal|0x17c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb023"
block|,
literal|0x39c00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb023"
block|,
literal|0x19c00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb013"
block|,
literal|0x3dc00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb013"
block|,
literal|0x1dc00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"cb012"
block|,
literal|0x3fc00000
block|,
literal|0xc0000000
block|,
literal|",al"
block|,
literal|1
block|}
block|,
block|{
literal|"cb012"
block|,
literal|0x1fc00000
block|,
literal|0xc0000000
block|,
literal|"l"
block|,
literal|1
block|}
block|,
block|{
literal|"fstoi"
block|,
literal|0x81a01a20
block|,
literal|0x400025c0
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fdtoi"
block|,
literal|0x81a01a40
block|,
literal|0x400025a0
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fxtoi"
block|,
literal|0x81a01a60
block|,
literal|0x40002580
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fitox"
block|,
literal|0x81a01980
block|,
literal|0x40002660
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fitod"
block|,
literal|0x81a01900
block|,
literal|0x400026e0
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fitos"
block|,
literal|0x81a01880
block|,
literal|0x40002660
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fstod"
block|,
literal|0x81a01920
block|,
literal|0x400026c0
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fstox"
block|,
literal|0x81a019a0
block|,
literal|0x40002640
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fdtos"
block|,
literal|0x81a018c0
block|,
literal|0x40002720
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fdtox"
block|,
literal|0x81a019c0
block|,
literal|0x40002620
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fxtos"
block|,
literal|0x81a018e0
block|,
literal|0x40002700
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fxtod"
block|,
literal|0x81a01960
block|,
literal|0x40002680
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fdivx"
block|,
literal|0x81a009e0
block|,
literal|0x40083600
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fdivd"
block|,
literal|0x81a009c0
block|,
literal|0x40003620
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fdivs"
block|,
literal|0x81a009a0
block|,
literal|0x40003640
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fmuls"
block|,
literal|0x81a00920
block|,
literal|0x400036c0
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fmuld"
block|,
literal|0x81a00940
block|,
literal|0x400036a0
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fmulx"
block|,
literal|0x81a00960
block|,
literal|0x40003680
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fsqrts"
block|,
literal|0x81a00520
block|,
literal|0x40003ac0
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fsqrtd"
block|,
literal|0x81a00540
block|,
literal|0x40003aa8
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fsqrtx"
block|,
literal|0x81a00560
block|,
literal|0x40003a80
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fabss"
block|,
literal|0x81a00120
block|,
literal|0x40003ec0
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fnegs"
block|,
literal|0x81a000a0
block|,
literal|0x40003f40
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fmovs"
block|,
literal|0x81a00020
block|,
literal|0x40003fc0
block|,
literal|"f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fsubx"
block|,
literal|0x81a008e0
block|,
literal|0x40003700
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fsubd"
block|,
literal|0x81a008c0
block|,
literal|0x40003720
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fsubs"
block|,
literal|0x81a008a0
block|,
literal|0x40003740
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"faddx"
block|,
literal|0x81a00860
block|,
literal|0x40003780
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"faddd"
block|,
literal|0x81a00840
block|,
literal|0x400037a0
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fadds"
block|,
literal|0x81a00820
block|,
literal|0x400037c0
block|,
literal|"e,f,g"
block|,
literal|0
block|}
block|,
block|{
literal|"fcmpex"
block|,
literal|0x81a80ae0
block|,
literal|0x40003500
block|,
literal|"e,f"
block|,
literal|0
block|}
block|,
block|{
literal|"fcmped"
block|,
literal|0x81a80ac0
block|,
literal|0x40003520
block|,
literal|"e,f"
block|,
literal|0
block|}
block|,
block|{
literal|"fcmpes"
block|,
literal|0x81a80aa0
block|,
literal|0x40003540
block|,
literal|"e,f"
block|,
literal|0
block|}
block|,
block|{
literal|"fcmpx"
block|,
literal|0x81a80a60
block|,
literal|0x40003580
block|,
literal|"e,f"
block|,
literal|0
block|}
block|,
block|{
literal|"fcmpd"
block|,
literal|0x81a80a40
block|,
literal|0x400035a0
block|,
literal|"e,f"
block|,
literal|0
block|}
block|,
block|{
literal|"fcmps"
block|,
literal|0x81a80a20
block|,
literal|0x400035c0
block|,
literal|"e,f"
block|,
literal|0
block|}
block|,
block|{
literal|"cpop1"
block|,
literal|0x81b00000
block|,
literal|0x40480000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|,
block|{
literal|"cpop2"
block|,
literal|0x81b80000
block|,
literal|0x40400000
block|,
literal|"[1+2],d"
block|,
literal|0
block|}
block|, }
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|NUMOPCODES
value|((sizeof sparc_opcodes)/(sizeof sparc_opcodes[0]))
end_define

end_unit

