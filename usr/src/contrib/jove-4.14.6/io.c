...
condition|)
block|{
comment|/* ??? Is this correct? DHR 			 * Current path on this drive might not be the root. 			 */
return|return
literal|0
return|;
block|}
...
argument_list|)
expr_stmt|;
comment|/* just guessing that if this is moved here */
comment|/* then the bug on SunOS that seems to alter */
comment|/* mtime under our feet will disappear */
if|if
condition|(
operator|!
name|is_insert
...
directive|else
comment|/* System Vr4, and who else? */
specifier|extern
name|char
modifier|*
name|getcwd
...
condition|)
block|{
comment|/* allow quoting of ~ (but \ is a path separator in MSDOS) */
name|name
operator|+=
literal|1
expr_stmt|;
...
name|complain
argument_list|(
literal|"Warning: cannot create tmp file! %s"
argument_list|,
name|strerror
argument_list|(
name|errno
...
begin_comment
comment|/* Close tempfile before execing a child process.  * Since we might be vforking, we must not change any variables  * (in particular tmpfd).  */
end_comment
...
begin_comment
comment|/* Close and remove tempfile before exiting. */
end_comment
