...
begin_comment
comment|/* Make a new line after "after" in buffer "buf", unless "after" is NULL,    in which case we insert the new line before first line. */
end_comment
...
expr_stmt|;
block|}
comment|/* Make sure that there are no dangling references to lines in the free list,  * then move them to the end of the avail list.  */
name|private
name|void
name|RecycleLines
parameter_list|()
...
argument_list|()
expr_stmt|;
comment|/* ChkWindowLines(); -- nothing needs attention */
comment|/* ChkBufLines(); -- nothing needs attention */
if|if
condition|(
name|faline
operator|==
...
comment|/* ??? I think that this WILL be called when we are looking for a new  * buffer line: nbufline() => newchunk() => GCchunks() -- DHR  */
name|void
name|GCchunks
parameter_list|()
block|{
...
condition|)
block|{
comment|/* Empty: unlink and free it!!! */
if|if
condition|(
name|prev
operator|==
