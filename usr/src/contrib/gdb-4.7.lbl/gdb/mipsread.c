...
begin_comment
comment|/* External forms of ecoff sym structures */
end_comment
...
begin_comment
comment|/* These must match the corresponding definition in gcc/config/xm-mips.h.    At some point, these should probably go into a shared include file,    but currently gcc and gdb do not share any directories. */
end_comment
...
init|=
block|{
literal|"cannot map MIPS basic type 0x%x"
block|,
literal|0
block|,
literal|0
...
name|struct
name|complaint
name|array_bitsize_complaint
init|=
block|{
literal|"size of array target type not known, assuming %d bits"
block|,
literal|0
block|,
literal|0
...
init|=
block|{
literal|"can't handle PDR for static proc at 0x%x"
block|,
literal|0
block|,
literal|0
...
begin_comment
comment|/* Already-parsed symbols are marked specially */
end_comment
...
begin_comment
comment|/* When looking at .o files, avoid tripping over zero pointers.    FIXME; places that use this should be fixed to convert from    external to internal format, rather than examining in-place. */
end_comment

begin_define
define|#
directive|define
name|UNSAFE_DATA_ADDR
parameter_list|(
name|p
parameter_list|)
value|((p) == 0)
...
begin_comment
comment|/* MIPS symtab header for the current file */
end_comment
...
begin_endif
unit|static struct partial_symtab * parse_fdr PARAMS ((int, int, struct objfile *));
endif|#
directive|endif
end_endif
...
expr_stmt|;
block|}
comment|/* If we have a file symbol header lying around, blow it away.  */
if|if
condition|(
name|cur_hdr
condition|)
...
expr_stmt|;
block|}
comment|/* Restore the header and list of pending typedefs */
name|cur_hdr
operator|=
name|CUR_HDR
argument_list|(
...
begin_comment
comment|/* Read the symtab information from file ABFD into memory.  Also,    return address just past end of our text segment in *END_OF_TEXT_SEGP.  */
end_comment
...
name|st_hdr
decl_stmt|;
comment|/* Header for executable/object file we read symbols from */
name|struct
name|coff_exec
name|filhdr
decl_stmt|;
...
name|val
decl_stmt|;
comment|/* We need some info from the initial headers */
name|val
operator|=
name|bfd_seek
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Find and read the symbol table header */
name|st_hdrsize
operator|=
name|bfd_h_get_32
argument_list|(
...
condition|)
block|{
comment|/* Profanity check */
name|error
argument_list|(
literal|"Wrong header size: %d, not %d"
argument_list|,
name|st_hdrsize
argument_list|,
sizeof|sizeof
...
argument_list|)
expr_stmt|;
comment|/* Find out how large the symbol table is */
name|stsize
operator|=
operator|(
name|st_hdr
...
name|cbEXTR
expr_stmt|;
comment|/* Allocate space for the symbol table.  Read it in.  */
name|cur_hdr
operator|=
operator|(
name|HDRR
...
name|readerr
goto|;
comment|/* Fixup file_pointers in it */
name|fixup_symtab
argument_list|(
name|cur_hdr
argument_list|,
...
name|rbase
decl_stmt|;
comment|/* This function depends on the external and internal forms 	   of the MIPS symbol table taking identical space.  Check this 	   assumption at compile-time.   	   DO NOT DELETE THESE ENTRIES, OR COMMENT THEM OUT, JUST BECAUSE SOME 	   "LINT" OR COMPILER THINKS THEY ARE UNUSED!  Thank you.  */
specifier|static
name|check_hdr1
index|[
literal|1
...
block|}
expr_stmt|;
comment|/* Swap in the header record.  */
name|ecoff_swap_hdr_in
argument_list|(
name|abfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * These fields are useless (and empty) by now: 	 *	hdr->cbDnOffset, hdr->cbOptOffset 	 * We use them for other internal purposes. 	 */
name|hdr
operator|->
name|cbDnOffset
operator|=
...
parameter_list|)
define|\
value|if (hdr->off) hdr->off = (unsigned int)data + (hdr->off - f_ptr);
name|FIX
argument_list|(
name|cbLineOffset
argument_list|)
...
expr_stmt|;
block|}
comment|/* Fix all string pointers inside the symtab, and 	   the FDR records.  Also fix other miscellany.  */
for|for
control|(
name|f_idx
operator|=
...
name|code_offset
decl_stmt|;
comment|/* Header itself, and strings */
name|fh
operator|=
operator|(
name|FDR
...
argument_list|)
expr_stmt|;
comment|/* FIXME! Probably don't want to do this here! */
for|for
control|(
name|s_idx
operator|=
...
name|f_idx
expr_stmt|;
comment|/* cannot fix fh->ipdFirst because it is a short */
define|#
directive|define
name|IPDFIRST
parameter_list|(
...
parameter_list|)
define|\
value|((long)h->cbPdOffset + fh->ipdFirst * sizeof(PDR))
comment|/* Optional symbols (actually used for partial_symtabs) */
name|fh
operator|->
name|ioptBase
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Relative file descriptor table */
name|fh
operator|->
name|rfdBase
operator|=
...
name|cbLineOffset
expr_stmt|;
comment|/* Procedure symbols.  (XXX This should be done later) */
name|code_offset
operator|=
name|fh
operator|->
...
expr_stmt|;
block|}
comment|/* Turn index into a pointer */
name|pr
operator|->
name|isym
operator|=
...
block|}
block|}
comment|/* External symbols: swap in, and fix string */
for|for
control|(
name|s_idx
operator|=
...
begin_comment
comment|/* Check whether we already saw symbol SH in file FH as undefined */
end_comment
...
begin_comment
comment|/* Add a new undef symbol SH of type T */
end_comment
...
literal|1
expr_stmt|;
comment|/* for quick check */
block|}
end_function

begin_comment
comment|/* Throw away undef entries when done with file index F_IDX */
end_comment

begin_comment
comment|/* FIXME -- storage leak.  This is never called!!!   --gnu */
end_comment
...
begin_endif
unit|static void free_pending(f_idx) 	int f_idx; { 	register struct mips_pending *p, *q;  	for (p = pending_list[f_idx]; p; p = q) { 		q = p->next; 		free((PTR)p); 	} 	pending_list[f_idx] = 0; }
endif|#
directive|endif
end_endif
...
literal|1
decl_stmt|;
comment|/* When a symbol is cross-referenced from other files/symbols 	   we mark it explicitly */
name|int
name|pend
init|=
operator|(
...
block|bcopy(SYMBOL_TYPE(s),lookup_function_type(t),sizeof(struct type));
else|#
directive|else
name|SYMBOL_TYPE
argument_list|(
...
name|TYPE_CODE_UNDEF
condition|)
comment|/* If the type of the member is Nil (or Void), 			       assume the tag is an enumeration. */
if|if
condition|(
name|tsym
operator|->
...
name|TYPE_CODE_STRUCT
expr_stmt|;
comment|/* If this type was expected, use its partial definition */
if|if
condition|(
name|pend
condition|)
...
literal|0
expr_stmt|;
comment|/* Mark that symbol has a type, and say which one */
name|sh
operator|->
name|value
operator|=
...
block|}
block|}
comment|/* Skip over any further type qualifiers (FIXME).  */
if|if
condition|(
name|t
operator|->
...
condition|)
block|{
comment|/* This is the way it would work if the compiler worked */
name|TIR
name|t1
index|[
literal|1
...
name|pn
decl_stmt|;
comment|/* Try to cross reference this type */
name|ax
operator|+=
name|cross_ref
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* SOMEONE OUGHT TO FIX DBXREAD TO DROP "STRUCT" */
name|sprintf
argument_list|(
name|name
argument_list|,
...
parameter_list|)
define|\
value|if (t->tq != tqNil) ax += upgrade_type(&tp, t->tq, ax, bigend);
name|again
label|:
name|PARSE_TQ
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Fields are kept in an array */
...
comment|/* Check whether supplied array element bit size matches 		   the known size of the element type.  If this complaint 		   ends up not happening, we can remove this code.  It's 		   here because we aren't sure we understand this *&%&$ 		   symbol format.  */
name|id
operator|=
name|TYPE_LENGTH
argument_list|(
...
condition|)
block|{
comment|/* Most likely an undefined type */
name|id
operator|=
name|rf
expr_stmt|;
...
condition|)
continue|continue;
comment|/* 		 *	Aurgh! To know where to stop expanding we 		 *	must look-ahead. 		 */
for|for
control|(
name|l
operator|=
...
name|iline
expr_stmt|;
comment|/* 		 * When procedures are moved around the linenumbers 		 * are attributed to the next procedure up 		 */
if|if
condition|(
name|pr
operator|->
...
begin_comment
comment|/* Master parsing procedure for first-pass reading of file symbols    into a partial_symtab.     Parses the symtab described by the global symbolic header CUR_HDR.    END_OF_TEXT_SEG gives the address just after the text segment for    the symtab we are reading.  */
end_comment
...
name|pst
expr_stmt|;
comment|/* Make everything point to everything. */
name|FDR_IDX
argument_list|(
name|pst
argument_list|)
...
block|}
block|}
comment|/* Mark the last code address, and remember it for later */
name|hdr
operator|->
name|cbDnOffset
operator|=
...
condition|)
block|{
comment|/* If there are no functions defined here ... */
comment|/* ...then presumably a .h file: drop reverse depends .h->.c */
for|for
control|(
init|;
name|s_id0
...
begin_comment
comment|/* Do the initial analisys of the F_IDX-th file descriptor.    Allocates a partial symtab for it, and builds the list    of dependent files by recursion. LEV says at which level    of recursion we are called (to pretty up debug traces) */
end_comment

begin_comment
unit|static struct partial_symtab * parse_fdr(f_idx, lev, objfile) 	int f_idx; 	int lev; 	struct objfile *objfile; { 	register FDR *fh; 	register struct partial_symtab *pst; 	int s_idx, s_id0;  	fh = (FDR *) (cur_hdr->cbFdOffset) + f_idx;
comment|/* Use this to indicate into which symtab this file was parsed */
end_comment

begin_comment
unit|if (fh->ioptBase) 		return (struct partial_symtab *) fh->ioptBase;
comment|/* Debuggability level */
end_comment

begin_comment
unit|if (compare_glevel(max_glevel, fh->glevel)< 0) 		max_glevel = fh->glevel;
comment|/* Make a new partial_symtab */
end_comment

begin_comment
unit|pst = new_psymtab(fh->rss, objfile); 	if (fh->cpd == 0){ 		pst->textlow = 0; 		pst->texthigh = 0; 	} else { 		pst->textlow = fh->adr; 		pst->texthigh = fh->cpd;
...
begin_comment
unit|FDR_IDX(pst) = f_idx; 	fdr_to_pst[f_idx].pst = pst; 	fh->ioptBase = (int)pst;
comment|/* Analyze its dependencies */
end_comment

begin_comment
unit|if (fh->crfd<= 1) 		return pst;  	s_id0 = 0; 	if (fh->cpd == 0) {
comment|/* If there are no functions defined here ... */
end_comment
...
begin_comment
unit|for (; s_id0< fh->crfd; s_id0++) { 			RFDT *rh = (RFDT *) (fh->rfdBase) + s_id0; 			if (*rh == f_idx) { 				s_id0++;
comment|/* Skip self-dependency */
end_comment

begin_endif
unit|break; 			} 		} 	} 	pst->number_of_dependencies = fh->crfd - s_id0; 	pst->dependencies = (struct partial_symtab **) 		obstack_alloc (&objfile->psymbol_obstack, 				 pst->number_of_dependencies * 				   sizeof (struct partial_symtab *)); 	for (s_idx = s_id0; s_idx< fh->crfd; s_idx++) { 		RFDT *rh = (RFDT *) (fh->rfdBase) + s_idx;  		pst->dependencies[s_idx-s_id0] = parse_fdr(*rh, lev+1, objfile); 	}  	return pst; }
endif|#
directive|endif
end_endif
...
name|fh
expr_stmt|;
comment|/* BOUND is the highest core address of this file's procedures */
name|bound
operator|=
operator|(
name|cur_fd
...
argument_list|)
expr_stmt|;
comment|/* Fill in procedure info next.  We need to look-ahead to 	   find out where each procedure's code ends.  */
for|for
control|(
name|i
operator|=
...
expr_stmt|;
block|}
comment|/* Linenumbers.  At the end, check if we can save memory */
name|parse_lines
argument_list|(
name|fh
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* If there are undefined, tell the user */
if|if
condition|(
name|n_undef_symbols
condition|)
...
name|t
decl_stmt|;
comment|/* 		 * If we have processed this symbol then we left a forwarding 		 * pointer to the corresponding GDB symbol.  If not, we`ll put 		 * it in a list of pending symbols, to be processed later when 		 * the file f will be.  In any event, we collect the name for 		 * the type here. Which is why we made a first pass at 		 * strings. 		 */
name|sh
operator|=
operator|(
name|SYMR
...
name|index
expr_stmt|;
comment|/* Careful, we might be looking at .o files */
operator|*
name|pname
operator|=
operator|(
...
begin_comment
comment|/* Add a new linenumber entry (LINENO,ADR) to a linevector LT.    MIPS' linenumber encoding might need more than one byte    to describe it, LAST is used to detect these continuation lines */
end_comment
...
name|psymtab
decl_stmt|;
comment|/* FIXME -- why (char *) -1 rather than NULL? */
name|psymtab
operator|=
...
literal|1
expr_stmt|;
comment|/* We know it is sold as sigvec */
name|s
operator|=
name|lookup_symbol
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Most programs do not play with signals */
if|if
condition|(
name|s
operator|==
...
argument_list|)
expr_stmt|;
comment|/* A label of sigvec, to be more precise */
name|s
operator|=
name|lookup_symbol
argument_list|(
