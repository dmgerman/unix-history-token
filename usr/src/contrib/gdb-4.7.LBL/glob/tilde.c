...
begin_comment
comment|/* Return a new string which is the result of tilde expanding FILENAME. */
end_comment
...
name|NULL
expr_stmt|;
comment|/* Scan through FILENAME expanding tildes as we come to them. */
while|while
condition|(
literal|1
condition|)
...
comment|/* This test is always true the first time, since result_index 	 is 0, result_size is 0, and start is>= 0.  So we malloc here.  */
if|if
condition|(
operator|(
name|result_index
...
name|start
expr_stmt|;
comment|/* Advance FILENAME upto the starting tilde. */
name|filename
operator|+=
name|start
expr_stmt|;
