...
begin_comment
comment|/* Return a typename for a struct/union/enum type    without the tag qualifier.  If the type has a NULL name,    NULL is returned.  */
end_comment
...
break|break;
default|default:
comment|/* To avoid -Wall warnings */
break|break;
block|}
block|}
return|return
...
begin_comment
comment|/* Given a type TYPE, lookup the type of the component of type named    NAME.      If NOERR is nonzero, return zero if NAME is not suitably defined.  */
end_comment
...
begin_comment
comment|/* This function is really horrible, but to avoid it, there would need    to be more filling in of forward references.  */
end_comment
...
block|{
comment|/* If this is the first time we */
if|if
condition|(
name|objfile
operator|->
...
name|error
argument_list|(
literal|"internal error: unhandled type id %d"
argument_list|,
name|typeid
argument_list|)
expr_stmt|;
...
break|break;
block|}
comment|/* Install the newly created type in the objfile's fundamental_types 	     vector. */
operator|*
name|typep
operator|=
name|type
...
literal|2
argument_list|,
literal|"[%d] name '%s' (0x%x) length %d\n"
argument_list|,
name|method_idx
argument_list|,
...
name|spaces
argument_list|,
literal|"virtual_field_bits (%d bits at *0x%x)"
argument_list|,
name|TYPE_N_BASECLASSES
argument_list|(
name|type
...
name|spaces
argument_list|,
literal|"private_field_bits (%d bits at *0x%x)"
argument_list|,
name|TYPE_NFIELDS
argument_list|(
name|type
...
name|spaces
argument_list|,
literal|"protected_field_bits (%d bits at *0x%x)"
argument_list|,
name|TYPE_NFIELDS
argument_list|(
name|type
...
name|printf_filtered
argument_list|(
literal|"(TYPE_CODE_PASCAL_ARRAY)"
argument_list|)
expr_stmt|;
break|break;
case|case
...
literal|2
argument_list|,
literal|"[%d] bitpos %d bitsize %d type 0x%x name '%s' (0x%x)\n"
argument_list|,
name|idx
argument_list|,
name|TYPE_FIELD_BITPOS
