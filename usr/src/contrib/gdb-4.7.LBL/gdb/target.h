...
begin_comment
comment|/* The ops structure for our "current" target process.  */
end_comment
...
parameter_list|)
define|\
value|(*current_target->to_detach) (args, from_tty)
end_define
...
parameter_list|)
define|\
value|(*current_target->to_resume) (step, siggnal)
end_define

begin_comment
comment|/* Wait for inferior process to do something.  Return pid of child,    or -1 in case of error; store status through argument pointer STATUS.  */
end_comment
...
parameter_list|)
define|\
value|(*current_target->to_wait) (status)
end_define
...
begin_comment
comment|/* Store at least register REGNO, or all regs if REGNO == -1.    It can store as many registers as it wants to, so the entire registers    array must be valid.  Result is 0 for success, -1 for problems.  */
end_comment
...
begin_comment
comment|/* Reading and writing memory actually happens through a glue    function which iterates across the various targets.  Result is    0 for success, or an errno value.  */
end_comment
...
parameter_list|()
define|\
value|(*current_target->to_notice_signals) ()
end_define
...
begin_comment
comment|/* Does the target have execution?  Can we make it jump (through hoops),    or pop its stack a few times?  */
end_comment
