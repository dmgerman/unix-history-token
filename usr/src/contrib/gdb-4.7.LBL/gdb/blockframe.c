...
begin_comment
comment|/* Is ADDR inside the startup file?  Note that if your machine    has a way to detect the bottom of the stack, there is no need    to call this function from FRAME_CHAIN_VALID; the reason for    doing so is that some machines have no way of detecting bottom    of stack.  */
end_comment
...
begin_comment
comment|/* Test a specified PC value to see if it is in the range of addresses    that correspond to the main() function.  See comments above for why    we might want to do this.     Typically called from FRAME_CHAIN_VALID. */
end_comment
...
begin_comment
comment|/* Test a specified PC value to see if it is in the range of addresses    that correspond to the process entry point function.  See comments    in objfiles.h for why we might want to do this.     Typically called from FRAME_CHAIN_VALID. */
end_comment
...
begin_comment
comment|/* Return nonzero if the function for this frame has a prologue.  Many    machines can define FRAMELESS_FUNCTION_INVOCATION to just call this    function.  */
end_comment
...
name|error
argument_list|(
literal|"You haven't set up a process's stack to examine."
argument_list|)
expr_stmt|;
block|}
return|return
...
return|;
block|}
comment|/* Get the information from a combination of the pst 	 (static symbols), and the minimal symbol table (extern 	 symbols).  */
name|msymbol
operator|=
name|lookup_minimal_symbol_by_pc
argument_list|(
...
name|name
expr_stmt|;
comment|/* FIXME:  Deal with bumping into end of minimal symbols for a given      objfile, and what about testing for mst_text again? */
if|if
condition|(
operator|(
name|msymbol
...
begin_endif
unit|FRAME block_innermost_frame (block)      struct block *block; {   struct frame_info *fi;   register FRAME frame;   register CORE_ADDR start = BLOCK_START (block);   register CORE_ADDR end = BLOCK_END (block);    frame = 0;   while (1)     {       frame = get_prev_frame (frame);       if (frame == 0) 	return 0;       fi = get_frame_info (frame);       if (fi->pc>= start&& fi->pc< end) 	return frame;     } }
endif|#
directive|endif
end_endif
