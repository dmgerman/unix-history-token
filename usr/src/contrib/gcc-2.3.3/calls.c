...
begin_comment
comment|/* Return 1 if EXP contains a call to the built-in function `alloca'.  */
end_comment
...
comment|/* If we promoted this return value, make the proper SUBREG.  */
if|if
condition|(
name|GET_MODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If this is being passes partially in a register, we can't evaluate      it directly into its stack slot.  Otherwise, we can.  */
if|if
condition|(
name|arg
operator|->
