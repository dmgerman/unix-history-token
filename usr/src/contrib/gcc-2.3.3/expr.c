...
argument_list|)
return|;
comment|/* We can do this with a gen_lowpart if both desired and current modes      are integer, and this is either a constant integer, a register, or a      non-volatile MEM.  Except for the constant case, we must be narrowing      the operand.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
operator|!=
name|CODE_FOR_nothing
comment|/* We don't need MODE to be narrower than BITS_PER_HOST_WIDE_INT 		 here because if SIZE is less than the mode mask, as it is 		 returned by the macro, it will definately be less than the 		 actual mode mask.  */
operator|&&
operator|(
name|unsigned
operator|)
...
begin_comment
comment|/* Mark the instructions since PREV as a libcall block.    Add REG_LIBCALL to PREV and add a REG_RETVAL to the most recent insn.  */
end_comment
...
name|insn_last
decl_stmt|;
comment|/* Find the instructions to mark */
if|if
condition|(
name|prev
condition|)
...
argument_list|)
return|;
comment|/* Expand complex moves by moving real part and imag part, if posible.  */
elseif|else
if|if
condition|(
operator|(
...
argument_list|()
decl_stmt|;
comment|/* Tell flow that the whole of the destination is being set.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Mark these insns as a libcall block.  */
name|group_insns
argument_list|(
name|prev_insn
argument_list|)
...
name|INTEGER_CST
condition|)
comment|/* ??? This case remains to be written.  */
name|abort
argument_list|()
expr_stmt|;
name|bitpos
...
directive|if
literal|0
comment|/* We aren't finished fixing the callers to really handle nonzero offset.  */
block|if (offset != 0)     abort ();
endif|#
directive|endif
return|return
name|exp
...
argument_list|()
expr_stmt|;
comment|/* Ensure variable marked as used 	 even if it doesn't go through a parser.  */
name|TREE_USED
argument_list|(
name|exp
argument_list|)
...
expr_stmt|;
block|}
comment|/* If the mode of SAVE_EXPR_RTL does not match that of the expression, it 	 must be a promoted value.  We return a SUBREG of the wanted mode, 	 but mark it so that we know that it was already extended.  Note 	 that `unsignedp' was modified above in this case.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|EXIT_EXPR
case|:
comment|/* Exit the current loop if the body-expression is true.  */
block|{
name|rtx
name|label
init|=
...
name|CONSTRUCTOR
case|:
comment|/* All elts simple constants => refer to a constant in memory.  But 	 if this is a non-BLKmode mode, let it store a field at a time 	 since that should make a CONST_INT or CONST_DOUBLE when we 	 fold.  */
if|if
condition|(
name|TREE_STATIC
argument_list|(
...
directive|if
literal|0
comment|/* It is incorrectto set RTX_UNCHANGING_P here, because the fact that 	 a location is accessed through a pointer to const does not mean 	 that the value there can never change.  */
...
argument_list|)
expr_stmt|;
comment|/* If this is a constant, put it into a register if it is a 	   legimate constant and memory if it isn't.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* IN_EXPR: Inlined pascal set IN expression.         Algorithm:          rlo       = set_low - (set_low%bits_per_word); 	 the_word  = set [ (index - rlo)/bits_per_word ]; 	 bit_index = index % bits_per_word; 	 bitmask   = 1<< bit_index; 	 return !!(the_word& bitmask);  */
case|case
name|IN_EXPR
case|:
name|preexpand_calls
...
argument_list|)
expr_stmt|;
comment|/* If domain is empty, answer is no.  */
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* Compare index against bounds, if they are constant.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|sorry
argument_list|(
literal|"aggregate value in COND_EXPR"
argument_list|)
expr_stmt|;
name|cleanups_this_call
operator|=
...
if|else if (temp&& GET_MODE (temp) != BLKmode&& (TREE_CONSTANT (TREE_OPERAND (exp, 1)) 		     || ((TREE_CODE (TREE_OPERAND (exp, 1)) == PARM_DECL 			  || TREE_CODE (TREE_OPERAND (exp, 1)) == VAR_DECL)&& DECL_RTL (TREE_OPERAND (exp, 1))&& GET_CODE (DECL_RTL (TREE_OPERAND (exp, 1))) == REG&& DECL_RTL (TREE_OPERAND (exp, 1)) != temp))&& (TREE_CONSTANT (TREE_OPERAND (exp, 2)) 		     || ((TREE_CODE (TREE_OPERAND (exp, 2)) == PARM_DECL 			  || TREE_CODE (TREE_OPERAND (exp, 2)) == VAR_DECL)&& DECL_RTL (TREE_OPERAND (exp, 2))&& GET_CODE (DECL_RTL (TREE_OPERAND (exp, 2))) == REG&& DECL_RTL (TREE_OPERAND (exp, 2)) != temp))) 	  { 	    if (GET_CODE (temp) == REG&& REGNO (temp)< FIRST_PSEUDO_REGISTER) 	      temp = gen_reg_rtx (mode); 	    store_expr (TREE_OPERAND (exp, 2), temp, 0); 	    jumpifnot (TREE_OPERAND (exp, 0), op0); 	    store_expr (TREE_OPERAND (exp, 1), temp, 0); 	    op1 = op0; 	  }
endif|#
directive|endif
...
directive|if
literal|0
comment|/* I bet this needs to be done, and I bet that it needs to 	       be above, inside the else clause.  The reason is 	       simple, how else is it going to get cleaned up? (mrs)  	       The reason is probably did not work before, and was 	       commented out is because this was re-expanding already 	       expanded target_exprs (target == 0 and DECL_RTL (slot) 	       != 0) also cleaning them up many times as well.  :-( */
...
block|if (TREE_OPERAND (exp, 2) == 0) 	      TREE_OPERAND (exp, 2) = maybe_build_cleanup (slot); 	    if (TREE_OPERAND (exp, 2)) 	      cleanups_this_call = tree_cons (NULL_TREE, TREE_OPERAND (exp, 2), 					      cleanups_this_call);
endif|#
directive|endif
block|}
else|else
...
argument_list|)
expr_stmt|;
comment|/* Conjugate should appear as a single unit */
name|group_insns
argument_list|(
name|prev
argument_list|)
...
name|temp
expr_stmt|;
comment|/* This won't work inside a SEQUENCE--it really has to be 	   at the start of the function.  */
if|if
condition|(
name|in_sequence_p
argument_list|()
condition|)
block|{
comment|/* Better to do this than to crash.  */
name|error
argument_list|(
literal|"`va_start' used within `({...})'"
argument_list|)
expr_stmt|;
return|return
name|temp
return|;
block|}
comment|/* Put the sequence after the NOTE that starts the function.  */
...
decl_stmt|;
block|{
comment|/* If one operand is constant, make it the second one.  */
if|if
condition|(
name|GET_CODE
argument_list|(
