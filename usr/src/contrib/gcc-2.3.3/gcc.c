...
parameter_list|)
define|\
value|(!strcmp (STR, "Tdata") || !strcmp (STR, "Ttext")	\   || !strcmp (STR, "Tbss") || !strcmp (STR, "include")	\   || !strcmp (STR, "imacros") || !strcmp (STR, "aux-info"))
end_define
...
begin_comment
comment|/* List of prefixes to try when looking for libraries. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Make separate list of dirs that came from LIBRARY_PATH.  */
name|add_prefix
argument_list|(
operator|&
name|library_prefix
...
literal|'e'
condition|)
comment|/* Compensate for the +e options to the C++ front-end.  */
name|n_switches
operator|++
expr_stmt|;
elseif|else
...
condition|)
block|{
comment|/* Compensate for the +e options to the C++ front-end; 	     they're there simply for cfront call-compatability.  We do 	     some magic in default_compilers to pass them down properly. 	     Note we deliberately start at the `+' here, to avoid passing 	     -e0 or -e1 down into the linker.  */
name|switches
index|[
name|n_switches
index|]
...
block|}
break|break;
comment|/* Here are digits and numbers that just process 	       a certain constant string as a spec. 	    /* Here are digits and numbers that just process 	       a certain constant string as a spec.  */
case|case
literal|'1'
case|:
name|do_spec_1
...
begin_comment
comment|/* Determine whether a -L option is relevant.  Not required for certain    fixed names and for directories that don't exist.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Set up to remember the pathname of gcc and any options      needed for collect.  */
name|obstack_init
argument_list|(
operator|&
name|collect_obstack
