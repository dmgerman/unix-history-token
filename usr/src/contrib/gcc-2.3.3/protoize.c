...
begin_comment
comment|/* Make a duplicate of a given string in a newly allocated area.  */
end_comment
...
begin_comment
comment|/* Return true if the given character is a legal identifier character.  */
end_comment
...
begin_comment
comment|/* Given a pointer to a character in the cleaned text buffer, return a pointer    to the next non-whitepace character which follows it.  */
end_comment
...
begin_comment
comment|/* Clean up the clean text buffer.  Do this by converting comments and    preprocessor directives into spaces.   Also convert line continuations    into whitespace.  Also, whiteout string and character literals.  */
end_comment
...
block|strcpy (clean_filename, convert_filename);     strcat (clean_filename, ".clean");     if ((clean_file = creat (clean_filename, 0666)) == -1)       {         fprintf (stderr, "%s: can't create/open clean file `%s': %s\n", 		 pname, shortpath (NULL, clean_filename), 		 sys_errlist[errno]);         return;       }
comment|/* Write the clean file.  */
block|if (write (clean_file, new_clean_text_base, clean_size) != clean_size)       fprintf (stderr, "%s: error writing file `%s': %s\n", 	       pname, shortpath (NULL, clean_filename), sys_errlist[errno]);        close (clean_file);   }
endif|#
directive|endif
comment|/* 0 */
