...
begin_comment
comment|/* Output xcoff-format symbol table information from GNU compiler.    Copyright (C) 1992 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* These are GNU extensions we need to reference in this file.  */
end_comment
...
parameter_list|)
define|\
value|if (LINE<= xcoff_begin_function_line)	\     LINE = xcoff_begin_function_line + 1	\  #define ASM_OUTPUT_LFB(FILE,LINENUM) \ {						\   if (xcoff_begin_function_line == -1)		\     {						\       xcoff_begin_function_line = (LINENUM) - 1;\       fprintf (FILE, "\t.bf\t%d\n", (LINENUM));	\     }						\   xcoff_current_function_file			\     = (xcoff_current_include_file		\        ? xcoff_current_include_file : main_input_filename); \ }
end_define
...
parameter_list|)
define|\
value|do {						\     int linenum = LINENUM;				\     MAKE_LINE_SAFE (linenum);			\     fprintf (FILE, "\t.ef\t%d\n", ABS_OR_RELATIVE_LINENO (linenum)); \     xcoff_begin_function_line = -1;		\   } while (0)
end_define
...
parameter_list|)
define|\
value|do {						\     int linenum = LINENUM;				\     MAKE_LINE_SAFE (linenum);			\     fprintf (FILE, "\t.bb\t%d\n", ABS_OR_RELATIVE_LINENO (linenum)); \   } while (0)
end_define
...
parameter_list|)
define|\
value|do {						\     int linenum = LINENUM;				\     MAKE_LINE_SAFE (linenum);			\     fprintf (FILE, "\t.eb\t%d\n", ABS_OR_RELATIVE_LINENO (linenum)); \   } while (0)
end_define
...
comment|/* No such type "wchar".  */
comment|/* "long long int", and "long long unsigned int", are not handled here,      because there are no predefined types that match them.  */
...
parameter_list|)
define|\
value|do { \      fprintf(stderr, "Error, unknown stab %s: : 0x%x\n", STR, stab); \      fflush (stderr);	\    } while (0)
end_define
...
begin_comment
comment|/* In XCOFF, we have to have this .bf before the function prologue.    Rely on the value of `dbx_begin_function_line' not to duplicate .bf.  */
end_comment

begin_function
name|void
name|xcoffout_output_first_source_line
parameter_list|(
name|file
parameter_list|,
name|last_linenum
