...
argument_list|)
expr_stmt|;
comment|/* Handle a special case.  If we are about to output a location descriptor      for a variable or parameter which has been optimized out of existence,      don't do that.  Instead we output a zero-length location descriptor      value as part of the location attribute.       A variable which has been optimized out of existance will have a      DECL_RTL value which denotes a pseudo-reg.       Currently, in some rare cases, variables can have DECL_RTL values      which look like (MEM (REG pseudo-reg#)).  These cases are due to      bugs elsewhere in the compiler.  We treat such cases      as if the variable(s) in question had been optimized out of existance.       Note that in all cases where we wish to express the fact that a      variable has been optimized out of existance, we do not simply      suppress the generation of the entire location attribute because      the absence of a location attribute in certain kinds of DIEs is      used to indicate something else entirely... i.e. that the DIE      represents an object declaration, but not a definition.  So sayeth      the PLSIG.   */
if|if
condition|(
operator|!
name|is_pseudo_reg
...
name|PARM_DECL
condition|)
comment|/*  If this decl is not a formal parameter, just use DECL_RTL.  */
name|rtl
operator|=
name|DECL_RTL
argument_list|(
...
name|MEM
condition|)
comment|/* Parameter was passed in memory, so say that's where it lives.  */
name|rtl
operator|=
name|DECL_INCOMING_RTL
argument_list|(
...
block|{
comment|/* Parameter was passed in a register, so say it lives in the 	     register it will be copied to during the prologue.  */
name|rtl
operator|=
name|DECL_RTL
argument_list|(
...
block|}
block|}
comment|/* RECORD_TYPEs and UNION_TYPEs are themselves scopes (at least 	       in C++) so we must now output any nested pending types which 	       are local just to this RECORD_TYPE or UNION_TYPE.  */
name|output_pending_types_for_scope
argument_list|(
name|type
argument_list|)
...
name|FUNCTION_DECL
case|:
comment|/* If we are in terse mode, don't output any DIEs to represent 	 mere external function declarations.  Also, if we are conforming 	 to the DWARF version 1 specification, don't output DIEs for 	 mere external function declarations.  */
if|if
condition|(
name|DECL_EXTERNAL
argument_list|(
...
comment|/* In the case where we are describing an external function, all 	 we need to do here (and all we *can* do here) is to describe 	 the *types* of its formal parameters.  */
if|if
condition|(
name|DECL_EXTERNAL
argument_list|(
...
condition|)
return|return;
comment|/* Ignore this FUNCTION_DECL if it refers to a file-scope extern 	 function declaration and if the declaration was never even 	 referenced from within this entire compilation unit.  We 	 suppress these DIEs in order to save space in the .debug section 	 (by eliminating entries which are probably useless).  Note that 	 we must not suppress block-local extern declarations (whether 	 used or not) because that would screw-up the debugger's name 	 lookup mechanism and cause it to miss things which really ought 	 to be in scope at a given point.  */
if|if
condition|(
name|DECL_EXTERNAL
argument_list|(
