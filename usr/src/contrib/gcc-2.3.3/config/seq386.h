...
begin_comment
comment|/* By default, don't use IEEE compatible arithmetic comparisons    because the assembler can't handle the fucom insn.    Return float values in the 387.    (TARGET_80387 | TARGET_FLOAT_RETURNS_IN_80387) */
end_comment
...
parameter_list|)
define|\
value|(((REGNO)< 2 ? 1							\     : (REGNO)< 4 ? 1							\     : (REGNO)>= 8 ? (GET_MODE_CLASS (MODE) == MODE_FLOAT		\ 		    || GET_MODE_CLASS (MODE) == MODE_COMPLEX_FLOAT)	\     : (MODE) != QImode)							\&& ! (REGNO == 2&& GET_MODE_UNIT_SIZE (MODE)> 4))
end_define
