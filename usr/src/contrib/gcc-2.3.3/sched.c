...
literal|0
condition|)
comment|/* In theory, we should not end up with more REG_DEAD reg notes than we        started with.  In practice, this can occur as the result of bugs in        flow, combine and/or sched.  */
block|{
if|#
directive|if
literal|1
...
expr_stmt|;
block|}
comment|/* In theory, there should be no REG_DEAD notes leftover at the end.      In practice, this can occur as the result of bugs in flow, combine.c,      and/or sched.c.  The values of the REG_DEAD notes remaining are      meaningless, because dead_notes is just used as a free list.  */
if|#
directive|if
literal|1
if|if
...
argument_list|)
expr_stmt|;
comment|/* ??? Could add check here to see whether, the hard register is referenced      in the same mode as in the original insn.  If so, then it has not been      split, and the rest of the code below is unnecessary.  */
for|for
control|(
name|i
operator|=
...
block|}
block|}
comment|/* If this note refers to a multiple word hard register, it may 	     have been split into several smaller hard register references. 	     Check to see if there are any new register references that 	     need REG_NOTES added for them.  */
name|temp
operator|=
name|XEXP
argument_list|(
