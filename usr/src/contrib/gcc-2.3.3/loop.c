...
expr_stmt|;
block|}
comment|/* Now scan all JUMP_INSN's in the function.  If any branches into a loop      that it is not contained within, that loop is marked invalid.       Also look for blocks of code ending in an unconditional branch that      exits the loop.  If such a block is surrounded by a conditional       branch around the block, move the block elsewhere (see below) and      invert the jump to point to the code block.  This may eliminate a      label in our loop and will simplify processing by both us and a      possible second cse pass.  */
for|for
control|(
name|insn
operator|=
...
return|return;
default|default:
comment|/* Nothing else should occur in a JUMP_INSN.  */
name|abort
argument_list|()
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* We are storing at the same address as previously noted.  Save the 	   wider reference, treating BLKmode as wider.  */
if|if
condition|(
name|GET_MODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Save insn immediately after the loop_end.  Insns inserted after loop_end      must be put before this insn, so that they will appear in the right      order (i.e. loop order).  */
name|end_insert_before
operator|=
name|NEXT_INSN
argument_list|(
