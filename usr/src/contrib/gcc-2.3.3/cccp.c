...
begin_comment
comment|/* Nonzero means never to include precompiled files. */
end_comment
...
expr_stmt|;
block|}
comment|/* OK, now bring us back to the state we were in before we entered 	     this branch.  We need #line b/c the newline for the pragma 	     could fuck things up. */
name|output_line_command
argument_list|(
name|ip
argument_list|,
...
operator|++
expr_stmt|;
comment|/* ".." terminates a preprocessing number.  This is useless for C 	     code but useful for preprocessing other things.  */
if|if
condition|(
operator|!
name|isalnum
...
name|error
argument_list|(
literal|"`#import' expects \"fname\" or<fname>"
argument_list|)
expr_stmt|;
else|else
name|error
...
condition|)
block|{
comment|/* Break the line before this.  */
name|deps_output
argument_list|(
literal|""
argument_list|,
...
begin_comment
comment|/* Process the contents of include file FNAME, already open on descriptor F,    with output to OP.    SYSTEM_HEADER_P is 1 if this file was specified using<...>.    DIRPTR is the link in the dir path through which this file was found,    or 0 if the file name was absolute.  */
end_comment
...
name|basep
expr_stmt|;
comment|/* So it will get freed, on error.  */
name|bufp
operator|=
name|basep
expr_stmt|;
...
comment|/* NOTE: This works correctly even if HOST_WIDE_INT        is narrower than a pointer.        Do not try risky measures here to get another type to use!        Do not include gstddef.h or stddef.h--either one will fail!  */
if|if
condition|(
operator|(
name|HOST_WIDE_INT
...
literal|'\\'
case|:
comment|/* \# quotes a # even outside of strings.  */
if|if
condition|(
name|p
operator|<
...
block|}
block|}
comment|/* If we get here, this #endif ends a #ifndef 	 that contains all of the file (aside from whitespace). 	 Arrange not to include the file again 	 if the macro that was tested is defined.  */
if|if
condition|(
name|indepth
operator|!=
...
index|]
decl_stmt|;
comment|/* add chars to XBUF unless rest_args was zero with concatenation */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* Don't bother doing delete_newlines for subsequent 	       uses of arg.  */
name|arg
operator|->
name|use_count
operator|=
...
begin_comment
comment|/* Delete newlines in the string of length LENGTH at START, except inside    of string constants.  The string is copied into itself with its beginning    staying fixed.  */
end_comment
...
literal|'\n'
case|:
comment|/* If this is a NEWLINE NEWLINE, then this is a real newline in the 	 output.  Skip past the newline and its duplicate.  */
if|if
condition|(
operator|*
name|ibp
...
begin_comment
comment|/* Add output to `deps_buffer' for the -M switch.    STRING points to the text to be output.    SIZE is the number of bytes, or 0 meaning output until a null.    Outputting the empty string breaks the line if it is long enough.  */
end_comment
