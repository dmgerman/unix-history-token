...
begin_comment
comment|/*  A Bison parser, made from awk.y  */
end_comment
...
name|x
parameter_list|)
value|((unsigned)(x)<= 294 ? yytranslate[x] : 106)
end_define
...
name|x
parameter_list|)
value|(*token++ = (x), token == tokend ? tokexpand() : token)
end_define
...
name|nextc
parameter_list|()
value|((lexptr&& lexptr< lexend) ? \ 			*lexptr++ : \ 			(get_src_buf() ? *lexptr++ : '\0') \ 		)
end_define
...
begin_comment
comment|/*  * Read the input and turn it into tokens.  */
end_comment
...
name|yyerror
argument_list|(
literal|"inappropriate use of backslash"
argument_list|)
expr_stmt|;
break|break;
case|case
...
begin_comment
comment|/*  * This allocates a node with defined lnode and rnode.   */
end_comment
...
begin_comment
comment|/*  * This allocates a node with defined subnode and proc for builtin functions  * Checks for arg. count and supplies defaults where possible.  */
end_comment
...
begin_comment
comment|/*  * This allocates a Node_line_range node with defined condpair and  * zeroes the trigger word to avoid the temptation of assuming that calling  * 'node( foo, Node_line_range, 0)' will properly initialize 'triggered'.   */
end_comment

begin_comment
comment|/* Otherwise like node() */
end_comment
...
begin_comment
comment|/*  * Install a name in the symbol table, even if it is already there.  * Caller must check against redefinition if that is desired.   */
end_comment
...
begin_comment
comment|/* find the most recent hash node for name installed by install */
end_comment
...
begin_comment
comment|/*  * Add new to the rightmost branch of LIST.  This uses n^2 time, so we make  * a simple attempt at optimizing it.  */
end_comment
...
begin_comment
comment|/*  * check if name is already installed;  if so, it had better have Null value,  * in which case def is added as the value. Otherwise, install name with def  * as value.   */
end_comment
...
begin_comment
comment|/* Name points to a variable name.  Make sure its in the symbol table */
end_comment
