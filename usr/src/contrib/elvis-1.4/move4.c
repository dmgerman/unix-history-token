...
expr_stmt|;
block|}
comment|/* allow a "window size" number to be entered, but ignore it */
while|while
condition|(
name|key
operator|>=
...
literal|1
expr_stmt|;
comment|/* keeps one old line on screen */
break|break;
case|case
name|ctrl
argument_list|(
...
break|break;
block|}
comment|/* arrange for ctrl-B and ctrl-F to redraw the smart line */
if|if
condition|(
name|key
operator|==
...
operator|++
expr_stmt|;
comment|/* also, erase the statusline.  This happens naturally for 		 * the scrolling commands, but the paging commands need to 		 * explicitly clear the statusline. 		 */
name|msg
argument_list|(
literal|""
argument_list|)
