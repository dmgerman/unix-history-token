...
index|[]
init|=
literal|"@(#)$Id: m_getfld.c,v 1.1.1.1 1993/01/30 04:41:26 jtc Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* defined in sbr/m_msgdef.c = 0 			 * disgusting hack for "inc" so it can  			 * know how many characters were stuffed 			 * in the buffer on the last call (see 			 * comments in uip/scansbr.c) */
end_comment
...
begin_comment
comment|/* defined in sbr/m_msgdef.c = MS_DEFAULT */
end_comment
...
begin_comment
comment|/*  defined in sbr/m_msgdef.c = "" */
end_comment
...
name|bp
condition|)
comment|/* 				     * ep< bp means that all the buffer 				     * contains is a prefix of delim. 				     * If this prefix is really a delim, the 				     * m_eom call at entry should have found 				     * it.  Thus it's not a delim and we can 				     * take all of it. 				     */
name|c
operator|=
operator|(
name|ep
...
break|break;
block|}
comment|/* try matching one less char of delim string */
name|ep
operator|=
name|bp
operator|+
