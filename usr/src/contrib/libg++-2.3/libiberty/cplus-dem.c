...
begin_comment
comment|/* This is for g++ 1.95.03 (November 13 version).  */
end_comment
...
begin_comment
comment|/* Lucid/cfront virtual table prefix */
end_comment
...
begin_endif
unit|static void string_prepends PARAMS ((string *, string *));
endif|#
directive|endif
end_endif

begin_comment
comment|/*  Translate count to integer, consuming tokens in the process.     Conversion terminates on the first non-digit character. */
end_comment
...
operator|++
expr_stmt|;
comment|/* For lucid/cfront style we have to forget any types we might 	       have remembered up to this point, since they were not argument 	       types.  GNU style considers all types seen as available for 	       back references.  See comment in demangle_args() */
if|if
condition|(
name|LUCID_DEMANGLING
operator|||
name|CFRONT_DEMANGLING
condition|)
block|{
name|forget_types
argument_list|(
...
condition|)
block|{
comment|/* A GNU style constructor starts with "__<digit>" or "__Q". */
name|work
operator|->
name|constructor
operator|=
...
condition|)
block|{
comment|/* Found a cfront style virtual table, get past ARM_VTABLE_STRING          and create the decl.  Note that we consume the entire mangled 	 input string, which means that demangle_signature has no work 	 to do. */
operator|(
operator|*
name|mangled
operator|)
...
begin_comment
comment|/*  LOCAL FUNCTION  	demangle_qualified -- demangle 'Q' qualified name strings  SYNOPSIS  	static int 	demangle_qualified (struct work_stuff *, const char *mangled, 			    string *result, int isfuncname);  DESCRIPTION  	Demangle a qualified name, such as "Q25Outer5Inner" which is 	the mangled form of "Outer::Inner".  The demangled output is 	appended to the result string.  	If isfuncname is nonzero, then the qualified name we are building 	is going to be used as a member function name, so if it is a 	constructor or destructor function, append an appropriate 	constructor or destructor name.  I.E. for the above example, 	the result for use as a constructor is "Outer::Inner::Inner" 	and the result for use as a destructor is "Outer::Inner::~Inner".  BUGS  	Numeric conversion is ASCII dependent (FIXME).   */
end_comment
...
condition|)
block|{
comment|/* Assume success until we discover otherwise.  Skip over the 'Q', the 	 qualifier count, and any '_' between the qualifier count and the 	 first name (cfront qualified names). */
name|success
operator|=
literal|1
expr_stmt|;
...
literal|2
expr_stmt|;
comment|/* Pick off the names and append them to the result string as they 	 are found, separated by '::'. */
while|while
condition|(
name|qualifiers
operator|--
...
begin_endif
unit|static void string_prepends (p, s)      string *p, *s; {   if (s->b != s->p)     {       string_prependn (p, s->b, s->p - s->b);     } }
endif|#
directive|endif
end_endif
...
name|stderr
argument_list|,
literal|"usage: demangle [-s style] [arg1 [arg2]] ...\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"style = { gnu, lucid, cfront }\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"reads args from stdin if none supplied\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
