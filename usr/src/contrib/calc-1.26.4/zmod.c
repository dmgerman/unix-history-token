...
begin_comment
unit|extern void zaddmod proto((ZVALUE z1, ZVALUE z2, ZVALUE z3, ZVALUE *res)); extern void znegmod proto((ZVALUE z1, ZVALUE z2, ZVALUE *res));
...
begin_comment
unit|{ 	ZVALUE tmp; 	FULL prod; 	FULL digit; 	BOOL neg;  	if (iszero(z3) || isneg(z3)) 		error("Mod of non-positive integer"); 	if (iszero(z1) || iszero(z2) || isunit(z3)) { 		*res = _zero_; 		return; 	}
comment|/* 	 * If the modulus is a single digit number, then do the result 	 * cheaply.  Check especially for a small power of two. 	 */
end_comment

begin_comment
unit|if (istiny(z3)) { 		neg = (z1.sign != z2.sign); 		digit = z3.v[0]; 		if ((digit& -digit) == digit) {
...
begin_comment
unit|zmul(z1, z2,&tmp); 	if (ispos(tmp)&& ((tmp.len< z3.len) || ((tmp.len == z3.len)&& 		(tmp.v[tmp.len-1]< z2.v[z3.len-1])))) 	{ 		*res = tmp; 		return; 	} 	zmod(tmp, z3, res); 	freeh(tmp.v); }
...
begin_comment
unit|{ 	ZVALUE tmp; 	FULL prod; 	FULL digit;  	if (iszero(z2) || isneg(z2)) 		error("Mod of non-positive integer"); 	if (iszero(z1) || isunit(z2)) { 		*res = _zero_; 		return; 	}
comment|/* 	 * If the modulus is a single digit number, then do the result 	 * cheaply.  Check especially for a small power of two. 	 */
end_comment

begin_comment
unit|if (istiny(z2)) { 		digit = z2.v[0]; 		if ((digit& -digit) == digit) {
comment|/* NEEDS 2'S COMP */
end_comment
...
begin_comment
unit|zsquare(z1,&tmp); 	if ((tmp.len< z2.len) || 		((tmp.len == z2.len)&& (tmp.v[tmp.len-1]< z2.v[z2.len-1]))) { 			*res = tmp; 			return; 	} 	zmod(tmp, z2, res); 	freeh(tmp.v); }
...
begin_comment
unit|{ 	ZVALUE tmp; 	FULL sumdigit; 	FULL moddigit;  	if (iszero(z3) || isneg(z3)) 		error("Mod of non-positive integer"); 	if ((iszero(z1)&& iszero(z2)) || isunit(z3)) { 		*res = _zero_; 		return; 	} 	if (istwo(z2)) { 		if ((z1.v[0] + z2.v[0])& 0x1) 			*res = _one_; 		else 			*res = _zero_; 		return; 	} 	zadd(z1, z2,&tmp); 	if (isneg(tmp) || (tmp.len> z3.len)) { 		zmod(tmp, z3, res); 		freeh(tmp.v); 		return; 	} 	sumdigit = tmp.v[tmp.len - 1]; 	moddigit = z3.v[z3.len - 1]; 	if ((tmp.len< z3.len) || (sumdigit< moddigit)) { 		*res = tmp; 		return; 	} 	if (sumdigit< 2 * moddigit) { 		zsub(tmp, z3, res); 		freeh(tmp.v); 		return; 	} 	zmod(tmp, z2, res); 	freeh(tmp.v); }
...
begin_comment
unit|{ 	if (iszero(z3) || isneg(z3)) 		error("Mod of non-positive integer"); 	if (iszero(z2)) { 		zmod(z1, z3, res); 		return; 	} 	if (iszero(z1)) { 		znegmod(z2, z3, res); 		return; 	} 	if ((z1.sign == z2.sign)&& (z1.len == z2.len)&& 		(z1.v[0] == z2.v[0])&& (zcmp(z1, z2) == 0)) { 			*res = _zero_; 			return; 	} 	z2.sign = !z2.sign; 	zaddmod(z1, z2, z3, res); }
...
begin_comment
unit|{ 	int sign; 	int cv;  	if (iszero(z2) || isneg(z2)) 		error("Mod of non-positive integer"); 	if (iszero(z1) || isunit(z2)) { 		*res = _zero_; 		return; 	} 	if (istwo(z2)) { 		if (z1.v[0]& 0x1) 			*res = _one_; 		else 			*res = _zero_; 		return; 	}
