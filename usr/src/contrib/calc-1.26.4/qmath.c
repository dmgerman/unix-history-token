...
begin_comment
comment|/*  * Copyright (c) 1993 David I. Bell  * Permission is granted to use, distribute, or modify this source,  * provided that this copyright notice remains intact.  *  * Extended precision rational arithmetic primitive routines  */
end_comment
...
begin_endif
unit|static char *abortmsg = "Calculation aborted";
endif|#
directive|endif
end_endif
...
begin_comment
unit|NUMBER * qmuli(q, n) 	NUMBER *q; 	long n; { 	NUMBER *r; 	long d;
comment|/* gcd of multiplier and denominator */
end_comment

begin_endif
unit|int sign;  	if ((n == 0) || qiszero(q)) 		return qlink(&_qzero_); 	if (n == 1) 		return qlink(q); 	r = qalloc(); 	if (qisint(q)) { 		zmuli(q->num, n,&r->num); 		return r; 	} 	sign = 1; 	if (n< 0) { 		n = -n; 		sign = -1; 	} 	d = zmodi(q->den, n); 	d = iigcd(d, n); 	zmuli(q->num, (n * sign) / d,&r->num); 	(void) zdivi(q->den, d,&r->den); 	return r; }
endif|#
directive|endif
end_endif
