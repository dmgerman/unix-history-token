...
begin_comment
comment|/*  * Copyright (c) 1993 David I. Bell  * Permission is granted to use, distribute, or modify this source,  * provided that this copyright notice remains intact.  *  * Extended precision rational arithmetic matrix functions.  * Matrices can contain arbitrary types of elements.  */
end_comment
...
begin_comment
comment|/*  * Local utility routine to add one row to another one.  * No checks are made to verify the legality of the arguments.  */
end_comment

begin_comment
unit|static void mataddrow(m, r1, r2) 	MATRIX *m; 	long r1;
comment|/* row to be added into */
end_comment
...
begin_endif
unit|{ 	register VALUE *v1, *v2; 	register long rows; 	VALUE tmp;  	rows = (m->m_max[0] - m->m_min[0] + 1); 	v1 =&m->m_table[r1 * rows]; 	v2 =&m->m_table[r2 * rows]; 	while (rows--> 0) { 		addvalue(v1, v2,&tmp); 		freevalue(v1); 		*v1 = tmp; 		v1++; 		v2++; 	} }
endif|#
directive|endif
end_endif
