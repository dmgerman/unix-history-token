...
name|datagram
name|socket
literal|"); 		exit(-1); 	} 	/* Create name. */ 	name.sun_family = AF_UNIX; 	strcpy(name.sun_path, NAME); 	if (bind(sock,&name, sizeof(struct sockaddr_un))) { 		close(sock); 		perror("
name|binding
name|name
name|to
name|datagram
...
unit|, buf); 	unlink(NAME); 	close(sock); }
end_unit
