...
name|stream
name|socket
literal|"); 		exit(0); 	} 	/* Connect socket using name specified by command line. */ 	server.sun_family = AF_UNIX; 	strcpy(server.sun_path, argv[1]);  	if (connect(sock,&server, sizeof(struct sockaddr_un))< 0) { 		close(sock); 		perror("
name|connecting
name|stream
name|socket
