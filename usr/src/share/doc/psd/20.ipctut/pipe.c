...
operator|.
operator|\
literal|" Copyright (c) 1986 Regents of the University of California. .\" All rights reserved.  The Berkeley software License Agreement .\" specifies the terms and conditions for redistribution. .\" .\"	@(#)pipe.c	6.1 (Berkeley) %G% .\" #include<stdio.h>  #define DATA "
name|Bright
name|star
operator|,
name|would
...
name|socket
name|pair
literal|");  	if (child = fork()) { 		char            buf[1024];  		/* This is still the parent.  It reads the child's message. */ 		close(sockets[1]); 		if (read(sockets[0], buf, 1024)< 0) 			perror("
name|reading
name|message
literal|"); 		printf("
operator|--
