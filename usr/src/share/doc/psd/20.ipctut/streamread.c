...
operator|\
name|en
literal|", ntohs(server.sin_port));  	/* Start accepting connections */ 	listen(sock, 5); 	do { 		msgsock = accept(sock, 0, 0); 		do { 			for (i = 0; i< 1024; i++) 				buf[i] = '\e0'; 			if ((rval = read(msgsock, buf, 1024))< 0) 				perror("
name|reading
name|stream
name|message
