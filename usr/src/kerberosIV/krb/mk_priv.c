begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * $Source: /mit/kerberos/src/lib/krb/RCS/mk_priv.c,v $  * $Author: jtkohl $  *  * Copyright 1986, 1987, 1988 by the Massachusetts Institute  * of Technology.  *  * For copying and distribution information, please see the file  *<mit-copyright.h>.  *  * This routine constructs a Kerberos 'private msg', i.e.  * cryptographically sealed with a private session key.  *  * Note-- bcopy is used to avoid alignment problems on IBM RT.  *  * Note-- It's too bad that it did a long int compare on the RT before.  *  * Returns either< 0 ===> error, or resulting size of message  *  * Steve Miller    Project Athena  MIT/DEC  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
modifier|*
name|rcsid_mk_priv_c
init|=
literal|"$Header: mk_priv.c,v 4.13 89/03/22 14:48:59 jtkohl Exp $"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* lint */
end_comment

begin_include
include|#
directive|include
file|<mit-copyright.h>
end_include

begin_comment
comment|/* system include files */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_comment
comment|/* application include files */
end_comment

begin_include
include|#
directive|include
file|<des.h>
end_include

begin_include
include|#
directive|include
file|<krb.h>
end_include

begin_include
include|#
directive|include
file|<prot.h>
end_include

begin_include
include|#
directive|include
file|"lsb_addr_comp.h"
end_include

begin_function_decl
specifier|extern
name|char
modifier|*
name|errmsg
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
specifier|extern
name|int
name|errno
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|int
name|krb_debug
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* static storage */
end_comment

begin_decl_stmt
specifier|static
name|u_long
name|c_length
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|struct
name|timeval
name|msg_time
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|u_char
name|msg_time_5ms
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|long
name|msg_time_sec
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * krb_mk_priv() constructs an AUTH_MSG_PRIVATE message.  It takes  * some user data "in" of "length" bytes and creates a packet in "out"  * consisting of the user data, a timestamp, and the sender's network  * address. #ifndef NOENCRYTION  * The packet is encrypted by pcbc_encrypt(), using the given  * "key" and "schedule". #endif  * The length of the resulting packet "out" is  * returned.  *  * It is similar to krb_mk_safe() except for the additional key  * schedule argument "schedule" and the fact that the data is encrypted  * rather than appended with a checksum.  Also, the protocol version  * number is "private_msg_ver", defined in krb_rd_priv.c, rather than  * KRB_PROT_VERSION, defined in "krb.h".  *  * The "out" packet consists of:  *  * Size			Variable		Field  * ----			--------		-----  *  * 1 byte		private_msg_ver		protocol version number  * 1 byte		AUTH_MSG_PRIVATE |	message type plus local  *			HOST_BYTE_ORDER		byte order in low bit  * #ifdef NOENCRYPTION  * 4 bytes		c_length		length of data #else  * 4 bytes		c_length		length of encrypted data  *  * ===================== begin encrypt ================================ #endif  *   * 4 bytes		length			length of user data  * length		in			user data  * 1 byte		msg_time_5ms		timestamp milliseconds  * 4 bytes		sender->sin.addr.s_addr	sender's IP address  *  * 4 bytes		msg_time_sec or		timestamp seconds with  *			-msg_time_sec		direction in sign bit  *  * 0<=n<=7  bytes	pad to 8 byte multiple	zeroes #ifndef NOENCRYPTION  *			(done by pcbc_encrypt())  *  * ======================= end encrypt ================================ #endif  */
end_comment

begin_function
name|long
name|krb_mk_priv
parameter_list|(
name|in
parameter_list|,
name|out
parameter_list|,
name|length
parameter_list|,
name|schedule
parameter_list|,
name|key
parameter_list|,
name|sender
parameter_list|,
name|receiver
parameter_list|)
name|u_char
modifier|*
name|in
decl_stmt|;
comment|/* application data */
name|u_char
modifier|*
name|out
decl_stmt|;
comment|/* put msg here, leave room for                                  * header! breaks if in and out                                  * (header stuff) overlap */
name|u_long
name|length
decl_stmt|;
comment|/* of in data */
name|Key_schedule
name|schedule
decl_stmt|;
comment|/* precomputed key schedule */
name|C_Block
name|key
decl_stmt|;
comment|/* encryption key for seed and ivec */
name|struct
name|sockaddr_in
modifier|*
name|sender
decl_stmt|;
comment|/* sender address */
name|struct
name|sockaddr_in
modifier|*
name|receiver
decl_stmt|;
comment|/* receiver address */
block|{
specifier|register
name|u_char
modifier|*
name|p
decl_stmt|,
modifier|*
name|q
decl_stmt|;
specifier|static
name|u_char
modifier|*
name|c_length_ptr
decl_stmt|;
specifier|extern
name|int
name|private_msg_ver
decl_stmt|;
comment|/* in krb_rd_priv.c */
comment|/*      * get the current time to use instead of a sequence #, since      * process lifetime may be shorter than the lifetime of a session      * key.      */
if|if
condition|(
name|gettimeofday
argument_list|(
operator|&
name|msg_time
argument_list|,
operator|(
expr|struct
name|timezone
operator|*
operator|)
literal|0
argument_list|)
condition|)
block|{
return|return
operator|-
literal|1
return|;
block|}
name|msg_time_sec
operator|=
operator|(
name|long
operator|)
name|msg_time
operator|.
name|tv_sec
expr_stmt|;
name|msg_time_5ms
operator|=
name|msg_time
operator|.
name|tv_usec
operator|/
literal|5000
expr_stmt|;
comment|/* 5ms quanta */
name|p
operator|=
name|out
expr_stmt|;
operator|*
name|p
operator|++
operator|=
name|private_msg_ver
expr_stmt|;
operator|*
name|p
operator|++
operator|=
name|AUTH_MSG_PRIVATE
operator||
name|HOST_BYTE_ORDER
expr_stmt|;
comment|/* calculate cipher length */
name|c_length_ptr
operator|=
name|p
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|c_length
argument_list|)
expr_stmt|;
ifndef|#
directive|ifndef
name|NOENCRYPTION
comment|/* start for encrypted stuff */
endif|#
directive|endif
name|q
operator|=
name|p
expr_stmt|;
comment|/* stuff input length */
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|length
argument_list|,
operator|(
name|char
operator|*
operator|)
name|p
argument_list|,
sizeof|sizeof
argument_list|(
name|length
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|length
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|NOENCRYPTION
comment|/* make all the stuff contiguous for checksum */
else|#
directive|else
comment|/* make all the stuff contiguous for checksum and encryption */
endif|#
directive|endif
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
name|in
argument_list|,
operator|(
name|char
operator|*
operator|)
name|p
argument_list|,
operator|(
name|int
operator|)
name|length
argument_list|)
expr_stmt|;
name|p
operator|+=
name|length
expr_stmt|;
comment|/* stuff time 5ms */
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|msg_time_5ms
argument_list|,
operator|(
name|char
operator|*
operator|)
name|p
argument_list|,
sizeof|sizeof
argument_list|(
name|msg_time_5ms
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|msg_time_5ms
argument_list|)
expr_stmt|;
comment|/* stuff source address */
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|sender
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|,
operator|(
name|char
operator|*
operator|)
name|p
argument_list|,
sizeof|sizeof
argument_list|(
name|sender
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|sender
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|)
expr_stmt|;
comment|/*      * direction bit is the sign bit of the timestamp.  Ok      * until 2038??      */
comment|/* For compatibility with broken old code, compares are done in VAX         byte order (LSBFIRST) */
if|if
condition|(
name|lsb_net_ulong_less
argument_list|(
name|sender
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|,
comment|/* src< recv */
name|receiver
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|msg_time_sec
operator|=
operator|-
name|msg_time_sec
expr_stmt|;
elseif|else
if|if
condition|(
name|lsb_net_ulong_less
argument_list|(
name|sender
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|,
name|receiver
operator|->
name|sin_addr
operator|.
name|s_addr
argument_list|)
operator|==
literal|0
condition|)
if|if
condition|(
name|lsb_net_ushort_less
argument_list|(
name|sender
operator|->
name|sin_port
argument_list|,
name|receiver
operator|->
name|sin_port
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|msg_time_sec
operator|=
operator|-
name|msg_time_sec
expr_stmt|;
comment|/* stuff time sec */
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|msg_time_sec
argument_list|,
operator|(
name|char
operator|*
operator|)
name|p
argument_list|,
sizeof|sizeof
argument_list|(
name|msg_time_sec
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|msg_time_sec
argument_list|)
expr_stmt|;
comment|/*      * All that for one tiny bit!  Heaven help those that talk to      * themselves.      */
ifdef|#
directive|ifdef
name|notdef
comment|/*      * calculate the checksum of the length, address, sequence, and      * inp data      */
name|cksum
operator|=
name|quad_cksum
argument_list|(
name|q
argument_list|,
name|NULL
argument_list|,
name|p
operator|-
name|q
argument_list|,
literal|0
argument_list|,
name|key
argument_list|)
expr_stmt|;
if|if
condition|(
name|krb_debug
condition|)
name|printf
argument_list|(
literal|"\ncksum = %u"
argument_list|,
name|cksum
argument_list|)
expr_stmt|;
comment|/* stuff checksum */
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|cksum
argument_list|,
operator|(
name|char
operator|*
operator|)
name|p
argument_list|,
sizeof|sizeof
argument_list|(
name|cksum
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|+=
sizeof|sizeof
argument_list|(
name|cksum
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|NOENCRYPTION
comment|/*      * All the data have been assembled, compute length      */
else|#
directive|else
comment|/*      * All the data have been assembled, compute length and encrypt      * starting with the length, data, and timestamps use the key as      * an ivec.      */
endif|#
directive|endif
name|c_length
operator|=
name|p
operator|-
name|q
expr_stmt|;
name|c_length
operator|=
operator|(
operator|(
name|c_length
operator|+
sizeof|sizeof
argument_list|(
name|C_Block
argument_list|)
operator|-
literal|1
operator|)
operator|/
sizeof|sizeof
argument_list|(
name|C_Block
argument_list|)
operator|)
operator|*
sizeof|sizeof
argument_list|(
name|C_Block
argument_list|)
expr_stmt|;
comment|/* stuff the length */
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|c_length
argument_list|,
operator|(
name|char
operator|*
operator|)
name|c_length_ptr
argument_list|,
sizeof|sizeof
argument_list|(
name|c_length
argument_list|)
argument_list|)
expr_stmt|;
ifndef|#
directive|ifndef
name|NOENCRYPTION
comment|/* pcbc encrypt, pad as needed, use key as ivec */
name|pcbc_encrypt
argument_list|(
operator|(
name|C_Block
operator|*
operator|)
name|q
argument_list|,
operator|(
name|C_Block
operator|*
operator|)
name|q
argument_list|,
call|(
name|long
call|)
argument_list|(
name|p
operator|-
name|q
argument_list|)
argument_list|,
name|schedule
argument_list|,
name|key
argument_list|,
name|ENCRYPT
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* NOENCRYPTION */
return|return
operator|(
name|q
operator|-
name|out
operator|+
name|c_length
operator|)
return|;
comment|/* resulting size */
block|}
end_function

end_unit

