...
block|struct vnode tvp; 	int error;  	ifhp = (struct ifid *)fhp; 	imp = VFSTOISOFS (mp);  	if (ifhp->ifid_lbn>= imp->volume_space_size) 		return (EINVAL);  	if (ifhp->ifid_offset + sizeof (struct iso_directory_record)>= imp->im_bsize) 		return (EINVAL);  	bread (isomp->im_devvp, ifhp->ifid_lbn, imp->im_bsize, NOCRED,&bp);  	dirp = bp->b_un.b_addr + ifhp->ifid_offset;  	if (ifhp->ifid_offset + isonum_711 (dirp)>= imp->im_bsize) { 		brelse (bp); 		return (EINVAL); 	}  	tvp.v_mount = mp; 	ip = VTOI(&tvp); 	ip->i_vnode =&tvp; 	ip->i_dev = imp->im_dev; 	if (error = iso_iget(ip, ifhp->ifid_ino,&nip, dirp)) { 		*vpp = NULLVP; 		brelse (bp); 		return (error); 	} 	ip = nip; 	*vpp = ITOV(ip); 	brelse (bp); 	return (0);
endif|#
directive|endif
block|}
end_block
...
directive|if
literal|0
block|register struct inode *ip = VTOI(vp); 	register struct ufid *ufhp;  	ufhp = (struct ufid *)fhp; 	ufhp->ufid_len = sizeof(struct ufid); 	ufhp->ufid_ino = ip->i_number; 	ufhp->ufid_gen = ip->i_gen; 	return (0);
endif|#
directive|endif
block|}
end_block
