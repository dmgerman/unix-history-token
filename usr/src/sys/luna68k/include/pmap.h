...
parameter_list|)
define|\
value|if ((pmapp) != NULL&& (pmapp)->pm_stchanged) { \ 		(pcbp)->pcb_ustp = \ 		    luna_btop(pmap_extract(kernel_pmap, \ 			((vm_offset_t)(pmapp)->pm_stab))); \ 		if (iscurproc) \ 			loadustp((pcbp)->pcb_ustp); \ 		(pmapp)->pm_stchanged = FALSE; \ 	}
end_define
