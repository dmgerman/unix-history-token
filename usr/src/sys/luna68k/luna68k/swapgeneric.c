begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1992 OMRON Corporation.  * Copyright (c) 1982, 1986, 1992, 1993  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  *  * from: hp300/hp300/swapgeneric.c	7.9 (Berkeley) 6/16/93  *  *	@(#)swapgeneric.c	8.2 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/conf.h>
end_include

begin_include
include|#
directive|include
file|<sys/buf.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/reboot.h>
end_include

begin_include
include|#
directive|include
file|<luna68k/dev/device.h>
end_include

begin_comment
comment|/*  * Generic configuration;  all in one  */
end_comment

begin_decl_stmt
name|dev_t
name|rootdev
init|=
name|NODEV
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|dev_t
name|argdev
init|=
name|NODEV
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|dev_t
name|dumpdev
init|=
name|NODEV
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nswap
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|swdevt
name|swdevt
index|[]
init|=
block|{
block|{
operator|-
literal|1
block|,
literal|1
block|,
literal|0
block|}
block|,
block|{
name|NODEV
block|,
literal|0
block|,
literal|0
block|}
block|, }
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|dmmin
decl_stmt|,
name|dmmax
decl_stmt|,
name|dmtext
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|struct
name|driver
name|sddriver
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|struct
name|hp_ctlr
name|hp_cinit
index|[]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|struct
name|hp_device
name|hp_dinit
index|[]
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|genericconf
block|{
name|caddr_t
name|gc_driver
decl_stmt|;
name|char
modifier|*
name|gc_name
decl_stmt|;
name|dev_t
name|gc_root
decl_stmt|;
block|}
name|genericconf
index|[]
init|=
block|{
block|{
operator|(
name|caddr_t
operator|)
operator|&
name|sddriver
block|,
literal|"sd"
block|,
name|makedev
argument_list|(
literal|4
argument_list|,
literal|0
argument_list|)
block|}
block|,
block|{
literal|0
block|}
block|, }
struct|;
end_struct

begin_macro
name|setconf
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|struct
name|hp_ctlr
modifier|*
name|hc
decl_stmt|;
specifier|register
name|struct
name|hp_device
modifier|*
name|hd
decl_stmt|;
specifier|register
name|struct
name|genericconf
modifier|*
name|gc
decl_stmt|;
specifier|register
name|char
modifier|*
name|cp
decl_stmt|;
name|int
name|unit
decl_stmt|,
name|part
decl_stmt|,
name|swaponroot
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|rootdev
operator|!=
name|NODEV
condition|)
goto|goto
name|doswap
goto|;
name|unit
operator|=
literal|0
expr_stmt|;
name|part
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|boothowto
operator|&
name|RB_ASKNAME
condition|)
block|{
name|char
name|name
index|[
literal|128
index|]
decl_stmt|;
name|retry
label|:
name|printf
argument_list|(
literal|"root device? "
argument_list|)
expr_stmt|;
name|gets
argument_list|(
name|name
argument_list|)
expr_stmt|;
for|for
control|(
name|gc
operator|=
name|genericconf
init|;
name|gc
operator|->
name|gc_driver
condition|;
name|gc
operator|++
control|)
if|if
condition|(
name|gc
operator|->
name|gc_name
index|[
literal|0
index|]
operator|==
name|name
index|[
literal|0
index|]
operator|&&
name|gc
operator|->
name|gc_name
index|[
literal|1
index|]
operator|==
name|name
index|[
literal|1
index|]
condition|)
goto|goto
name|gotit
goto|;
name|printf
argument_list|(
literal|"use one of:"
argument_list|)
expr_stmt|;
for|for
control|(
name|gc
operator|=
name|genericconf
init|;
name|gc
operator|->
name|gc_driver
condition|;
name|gc
operator|++
control|)
name|printf
argument_list|(
literal|" %s%%d"
argument_list|,
name|gc
operator|->
name|gc_name
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
goto|goto
name|retry
goto|;
name|gotit
label|:
name|cp
operator|=
operator|&
name|name
index|[
literal|2
index|]
expr_stmt|;
if|if
condition|(
operator|*
name|cp
operator|<
literal|'0'
operator|||
operator|*
name|cp
operator|>
literal|'9'
condition|)
block|{
name|printf
argument_list|(
literal|"bad/missing unit number\n"
argument_list|)
expr_stmt|;
goto|goto
name|retry
goto|;
block|}
while|while
condition|(
operator|*
name|cp
operator|>=
literal|'0'
operator|&&
operator|*
name|cp
operator|<=
literal|'9'
condition|)
name|unit
operator|=
literal|10
operator|*
name|unit
operator|+
operator|*
name|cp
operator|++
operator|-
literal|'0'
expr_stmt|;
if|if
condition|(
operator|*
name|cp
operator|<
literal|'a'
operator|||
operator|*
name|cp
operator|>
literal|'h'
condition|)
block|{
name|printf
argument_list|(
literal|"bad/missing partiiton number\n"
argument_list|)
expr_stmt|;
goto|goto
name|retry
goto|;
block|}
name|part
operator|=
operator|*
name|cp
operator|++
operator|-
literal|'a'
expr_stmt|;
comment|/* 		if (*cp == '*') 			swaponroot++;  */
goto|goto
name|found
goto|;
block|}
for|for
control|(
name|gc
operator|=
name|genericconf
init|;
name|gc
operator|->
name|gc_driver
condition|;
name|gc
operator|++
control|)
block|{
for|for
control|(
name|hd
operator|=
name|hp_dinit
init|;
name|hd
operator|->
name|hp_driver
condition|;
name|hd
operator|++
control|)
block|{
if|if
condition|(
name|hd
operator|->
name|hp_alive
operator|==
literal|0
condition|)
continue|continue;
if|if
condition|(
name|hd
operator|->
name|hp_unit
operator|==
literal|0
operator|&&
name|hd
operator|->
name|hp_driver
operator|==
operator|(
expr|struct
name|driver
operator|*
operator|)
name|gc
operator|->
name|gc_driver
condition|)
block|{
name|printf
argument_list|(
literal|"root on %s0\n"
argument_list|,
name|hd
operator|->
name|hp_driver
operator|->
name|d_name
argument_list|)
expr_stmt|;
goto|goto
name|found
goto|;
block|}
block|}
block|}
name|printf
argument_list|(
literal|"no suitable root\n"
argument_list|)
expr_stmt|;
asm|asm("stop #0x2700");
name|found
label|:
name|gc
operator|->
name|gc_root
operator|=
name|makedev
argument_list|(
name|major
argument_list|(
name|gc
operator|->
name|gc_root
argument_list|)
argument_list|,
operator|(
name|unit
operator|*
literal|8
operator|)
operator|+
name|part
argument_list|)
expr_stmt|;
name|rootdev
operator|=
name|gc
operator|->
name|gc_root
expr_stmt|;
comment|/* 	printf("using root device: %s%d%c\n", gc->gc_name, unit, 'a' + part);  */
name|doswap
label|:
name|swdevt
index|[
literal|0
index|]
operator|.
name|sw_dev
operator|=
name|argdev
operator|=
name|dumpdev
operator|=
name|makedev
argument_list|(
name|major
argument_list|(
name|rootdev
argument_list|)
argument_list|,
operator|(
name|minor
argument_list|(
name|rootdev
argument_list|)
operator|&
operator|~
literal|0x7
operator|)
operator|+
literal|1
argument_list|)
expr_stmt|;
comment|/* 	printf("using swap device: %s%d%c\n", 	       gc->gc_name, unit, 'a' + (minor(swdevt[0].sw_dev)& 0x7));  */
comment|/* swap size and dumplo set during autoconfigure */
if|if
condition|(
name|swaponroot
condition|)
name|rootdev
operator|=
name|dumpdev
expr_stmt|;
block|}
end_block

begin_macro
name|gets
argument_list|(
argument|cp
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|cp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
modifier|*
name|lp
decl_stmt|;
specifier|register
name|c
expr_stmt|;
name|lp
operator|=
name|cp
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|cnputc
argument_list|(
name|c
operator|=
name|cngetc
argument_list|()
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'\n'
case|:
case|case
literal|'\r'
case|:
operator|*
name|lp
operator|++
operator|=
literal|'\0'
expr_stmt|;
return|return;
case|case
literal|'\b'
case|:
case|case
literal|'\177'
case|:
if|if
condition|(
name|lp
operator|>
name|cp
condition|)
block|{
name|lp
operator|--
expr_stmt|;
name|cnputc
argument_list|(
literal|' '
argument_list|)
expr_stmt|;
name|cnputc
argument_list|(
literal|'\b'
argument_list|)
expr_stmt|;
block|}
continue|continue;
case|case
literal|'#'
case|:
name|lp
operator|--
expr_stmt|;
if|if
condition|(
name|lp
operator|<
name|cp
condition|)
name|lp
operator|=
name|cp
expr_stmt|;
continue|continue;
case|case
literal|'@'
case|:
case|case
literal|'u'
operator|&
literal|037
case|:
name|lp
operator|=
name|cp
expr_stmt|;
name|cnputc
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
continue|continue;
default|default:
operator|*
name|lp
operator|++
operator|=
name|c
expr_stmt|;
block|}
block|}
block|}
end_block

end_unit

