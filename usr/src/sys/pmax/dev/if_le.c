...
begin_comment
comment|/*  * This is exceptionally ugly, but since the lance buffers are not always  * contiguous in cpu address space, this was the best I could think of.  * Essentially build an mbuf list with the entire raw packet in it and  * then dismantle it again if it is a local packet. I can't believe I am  * rebuilding the trailer encapsulation, but...  * Return true if the packet has been thrown away.  */
end_comment
