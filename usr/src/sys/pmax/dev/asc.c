...
begin_comment
comment|/*-  * Copyright (c) 1992 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Ralph Campbell.  *  * %sccs.include.redist.c%  *  *	@(#)asc.c	7.7 (Berkeley) %G%  */
end_comment
...
comment|/* XXX */
block|state->flags |= DID_SYNC; 		regs->asc_cmd = ASC_CMD_MSG_ACPT; 		status = asc_wait(regs, ASC_CSR_INT); 		ir = regs->asc_intr;
comment|/* some just break out here, some dont */
block|if (ASC_PHASE(status) == ASC_PHASE_MSG_OUT) { 			regs->asc_fifo = SCSI_ABORT; 			regs->asc_cmd = ASC_CMD_XFER_INFO; 			status = asc_wait(regs, ASC_CSR_INT); 			ir = regs->asc_intr; 		} 		if (ir& ASC_INT_DISC) { 			asc_end(asc, status, 0, ir); 			return (0); 		} 		goto status;
endif|#
directive|endif
case|case
name|SCSI_EXTENDED_MSG
