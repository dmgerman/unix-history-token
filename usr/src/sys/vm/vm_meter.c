begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	vm_meter.c	4.6	81/04/23	*/
end_comment

begin_include
include|#
directive|include
file|"../h/param.h"
end_include

begin_include
include|#
directive|include
file|"../h/systm.h"
end_include

begin_include
include|#
directive|include
file|"../h/seg.h"
end_include

begin_include
include|#
directive|include
file|"../h/dir.h"
end_include

begin_include
include|#
directive|include
file|"../h/user.h"
end_include

begin_include
include|#
directive|include
file|"../h/proc.h"
end_include

begin_include
include|#
directive|include
file|"../h/text.h"
end_include

begin_include
include|#
directive|include
file|"../h/vm.h"
end_include

begin_include
include|#
directive|include
file|"../h/cmap.h"
end_include

begin_decl_stmt
name|int
name|maxslp
init|=
name|MAXSLP
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|saferss
init|=
name|SAFERSS
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * The following parameters control operation of the page replacement  * algorithm.  They are initialized to 0, and then computed at boot time  * based on the size of the system.  If they are patched non-zero in  * a loaded vmunix they are left alone and may thus be changed per system  * using adb on the loaded system.  */
end_comment

begin_decl_stmt
name|int
name|maxpgio
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|minfree
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|desfree
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|lotsfree
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|slowscan
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|fastscan
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|klin
init|=
name|KLIN
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|klseql
init|=
name|KLSEQL
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|kltxt
init|=
name|KLTXT
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|klout
init|=
name|KLOUT
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|multprog
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* so we don't count process 2 */
end_comment

begin_decl_stmt
name|double
name|avenrun
index|[
literal|3
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* load average, of runnable procs */
end_comment

begin_comment
comment|/*  * Setup the paging constants for the clock algorithm.  * Called after the system is initialized and the amount of memory  * and number of paging devices is known.  */
end_comment

begin_macro
name|setupclock
argument_list|()
end_macro

begin_block
block|{
name|int
name|nclust
decl_stmt|,
name|nkb
decl_stmt|;
comment|/* 	 * Setup thresholds for paging: 	 *	lotsfree	is threshold where paging daemon turns on 	 *	desfree		is amount of memory desired free.  if less 	 *			than this for extended period, do swapping 	 *	minfree		is minimal amount of free memory which is 	 *			tolerable. 	 * 	 * Strategy of 4/22/81: 	 *	lotsfree is 1/4 of memory free. 	 *	desfree is 200k bytes, but at most 1/8 of memory 	 *	minfree is 32k bytes. 	 */
if|if
condition|(
name|lotsfree
operator|==
literal|0
condition|)
name|lotsfree
operator|=
name|LOOPPAGES
operator|/
literal|4
expr_stmt|;
if|if
condition|(
name|desfree
operator|==
literal|0
condition|)
block|{
name|desfree
operator|=
operator|(
literal|200
operator|*
literal|1024
operator|)
operator|/
name|NBPG
expr_stmt|;
if|if
condition|(
name|desfree
operator|>
name|LOOPPAGES
operator|/
literal|8
condition|)
name|desfree
operator|=
name|LOOPPAGES
operator|/
literal|8
expr_stmt|;
block|}
if|if
condition|(
name|minfree
operator|==
literal|0
condition|)
name|minfree
operator|=
operator|(
literal|32
operator|*
literal|1024
operator|)
operator|/
name|NBPG
expr_stmt|;
comment|/* 	 * Maxpgio thresholds how much paging is acceptable. 	 * This figures that 2/3 busy on an arm is all that is 	 * tolerable for paging.  We assume one operation per disk rev. 	 */
if|if
condition|(
name|maxpgio
operator|==
literal|0
condition|)
name|maxpgio
operator|=
operator|(
name|DISKRPM
operator|*
literal|2
operator|)
operator|/
literal|3
expr_stmt|;
comment|/* 	 * Clock to scan using max of 10% of processor time for sampling, 	 *     this estimated to allow maximum of 400 samples per second. 	 * Allow slighly higher angular velocity if 2 or more swap devices, 	 *     allow max of 600 samples per second (but only>= 2m) 	 * Basic scan time for ``fastscan'', the time for a clock rev 	 * with given memory and CLSIZE=2: 	 *	swap ilv<=1m	2m	3m	4m	6m	8m 	 * 	one-way		4s	5s	7s	XXX	XXX	XXX 	 * 	two-way		4s	4s	5s	6s	10s	13s 	 * XXXs here are situations we should not be in. 	 */
if|if
condition|(
name|fastscan
operator|==
literal|0
condition|)
block|{
name|nclust
operator|=
name|LOOPPAGES
operator|/
name|CLSIZE
expr_stmt|;
name|nkb
operator|=
operator|(
name|LOOPPAGES
operator|*
name|NBPG
operator|)
operator|/
literal|1024
expr_stmt|;
if|if
condition|(
name|nswdev
operator|==
literal|1
operator|&&
name|nkb
operator|>=
literal|2
operator|*
literal|1024
condition|)
name|printf
argument_list|(
literal|"WARNING: should run interleaved swap with>= 2Mb\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|nswdev
operator|==
literal|1
operator|||
name|nkb
operator|<
literal|2
operator|*
literal|1024
condition|)
name|fastscan
operator|=
name|nclust
operator|/
literal|400
expr_stmt|;
else|else
block|{
name|maxpgio
operator|=
operator|(
name|maxpgio
operator|*
literal|3
operator|)
operator|/
literal|2
expr_stmt|;
name|fastscan
operator|=
name|nclust
operator|/
literal|600
expr_stmt|;
block|}
block|}
if|if
condition|(
name|fastscan
operator|<
literal|4
condition|)
name|fastscan
operator|=
literal|4
expr_stmt|;
if|if
condition|(
name|fastscan
operator|>
name|maxslp
condition|)
name|fastscan
operator|=
name|maxslp
expr_stmt|;
comment|/* 	 * Set slow scan time to 1/3 the fast scan time but at most 	 * maxslp (a macroscopic slow). 	 */
if|if
condition|(
name|slowscan
operator|==
literal|0
condition|)
name|slowscan
operator|=
literal|3
operator|*
name|fastscan
expr_stmt|;
if|if
condition|(
name|slowscan
operator|>
name|maxslp
condition|)
name|slowscan
operator|=
name|maxslp
expr_stmt|;
ifdef|#
directive|ifdef
name|defined
name|(
name|BERT
name|)
name|||
name|defined
name|(
name|ERNIE
name|)
name|printf
argument_list|(
literal|"slowscan %d, fastscan %d, maxpgio %d\n"
argument_list|,
name|slowscan
argument_list|,
name|fastscan
argument_list|,
name|maxpgio
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"lotsfree %d, desfree %d, minfree %d\n"
argument_list|,
name|lotsfree
argument_list|,
name|desfree
argument_list|,
name|minfree
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
end_block

begin_comment
comment|/*  * The main loop of the scheduling (swapping) process.  *  * The basic idea is:  *	see if anyone wants to be swapped in;  *	swap out processes until there is room;  *	swap him in;  *	repeat.  * If the paging rate is too high, or the average free memory  * is very low, then we do not consider swapping anyone in,  * but rather look for someone to swap out.  *  * The runout flag is set whenever someone is swapped out.  * Sched sleeps on it awaiting work.  *  * Sched sleeps on runin whenever it cannot find enough  * core (by swapping out or otherwise) to fit the  * selected swapped process.  It is awakened when the  * core situation changes and in any case once per second.  *  * sched DOESN'T ACCOUNT FOR PAGE TABLE SIZE IN CALCULATIONS.  */
end_comment

begin_define
define|#
directive|define
name|swappable
parameter_list|(
name|p
parameter_list|)
define|\
value|(((p)->p_flag&(SSYS|SLOCK|SULOCK|SLOAD|SPAGE|SKEEP|SWEXIT|SPHYSIO))==SLOAD)
end_define

begin_comment
comment|/* insure non-zero */
end_comment

begin_define
define|#
directive|define
name|nz
parameter_list|(
name|x
parameter_list|)
value|(x != 0 ? x : 1)
end_define

begin_define
define|#
directive|define
name|NBIG
value|4
end_define

begin_define
define|#
directive|define
name|MAXNBIG
value|10
end_define

begin_decl_stmt
name|int
name|nbig
init|=
name|NBIG
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|bigp
block|{
name|struct
name|proc
modifier|*
name|bp_proc
decl_stmt|;
name|int
name|bp_pri
decl_stmt|;
name|struct
name|bigp
modifier|*
name|bp_link
decl_stmt|;
block|}
name|bigp
index|[
name|MAXNBIG
index|]
struct|,
name|bplist
struct|;
end_struct

begin_macro
name|sched
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|struct
name|proc
modifier|*
name|rp
decl_stmt|,
modifier|*
name|p
decl_stmt|,
modifier|*
name|inp
decl_stmt|;
name|int
name|outpri
decl_stmt|,
name|inpri
decl_stmt|,
name|rppri
decl_stmt|;
name|int
name|sleeper
decl_stmt|,
name|desperate
decl_stmt|,
name|deservin
decl_stmt|,
name|needs
decl_stmt|,
name|divisor
decl_stmt|;
specifier|register
name|struct
name|bigp
modifier|*
name|bp
decl_stmt|,
modifier|*
name|nbp
decl_stmt|;
name|int
name|biggot
decl_stmt|,
name|gives
decl_stmt|;
comment|/* 	 * Check if paging rate is too high, or average of 	 * free list very low and if so, adjust multiprogramming 	 * load by swapping someone out. 	 */
name|loop
label|:
name|wantin
operator|=
literal|0
expr_stmt|;
name|deservin
operator|=
literal|0
expr_stmt|;
name|sleeper
operator|=
literal|0
expr_stmt|;
name|p
operator|=
literal|0
expr_stmt|;
comment|/* 	 * Conditions for hard outswap are: 	 *	if need kernel map (mix it up). 	 * or 	 *	1. if there are at least 2 runnable processes (on the average) 	 * and	2. the paging rate is excessive or memory is now VERY low. 	 * and	3. the short (5-second) and longer (30-second) average 	 *	   memory is less than desirable. 	 */
if|if
condition|(
name|kmapwnt
operator|||
operator|(
name|avenrun
index|[
literal|0
index|]
operator|>=
literal|2
operator|&&
name|max
argument_list|(
name|avefree
argument_list|,
name|avefree30
argument_list|)
operator|<
name|desfree
operator|&&
operator|(
name|rate
operator|.
name|v_pgin
operator|+
name|rate
operator|.
name|v_pgout
operator|>
name|maxpgio
operator|||
name|avefree
operator|<
name|minfree
operator|)
operator|)
condition|)
block|{
name|desperate
operator|=
literal|1
expr_stmt|;
goto|goto
name|hardswap
goto|;
block|}
name|desperate
operator|=
literal|0
expr_stmt|;
comment|/* 	 * Not desperate for core, 	 * look for someone who deserves to be brought in. 	 */
name|outpri
operator|=
operator|-
literal|20000
expr_stmt|;
for|for
control|(
name|rp
operator|=
name|proc
init|;
name|rp
operator|<
name|procNPROC
condition|;
name|rp
operator|++
control|)
switch|switch
condition|(
name|rp
operator|->
name|p_stat
condition|)
block|{
case|case
name|SRUN
case|:
if|if
condition|(
operator|(
name|rp
operator|->
name|p_flag
operator|&
name|SLOAD
operator|)
operator|==
literal|0
condition|)
block|{
name|rppri
operator|=
name|rp
operator|->
name|p_time
operator|-
name|rp
operator|->
name|p_swrss
operator|/
name|nz
argument_list|(
operator|(
name|maxpgio
operator|/
literal|2
operator|)
operator|*
operator|(
name|klin
operator|*
name|CLSIZE
operator|)
argument_list|)
operator|+
name|rp
operator|->
name|p_slptime
operator|-
operator|(
name|rp
operator|->
name|p_nice
operator|-
name|NZERO
operator|)
operator|*
literal|8
expr_stmt|;
if|if
condition|(
name|rppri
operator|>
name|outpri
condition|)
block|{
if|if
condition|(
name|rp
operator|->
name|p_poip
condition|)
continue|continue;
if|if
condition|(
name|rp
operator|->
name|p_textp
operator|&&
name|rp
operator|->
name|p_textp
operator|->
name|x_poip
condition|)
continue|continue;
name|p
operator|=
name|rp
expr_stmt|;
name|outpri
operator|=
name|rppri
expr_stmt|;
block|}
block|}
continue|continue;
case|case
name|SSLEEP
case|:
case|case
name|SSTOP
case|:
if|if
condition|(
operator|(
name|freemem
operator|<
name|desfree
operator|||
name|rp
operator|->
name|p_rssize
operator|==
literal|0
operator|)
operator|&&
name|rp
operator|->
name|p_slptime
operator|>
name|maxslp
operator|&&
operator|(
operator|!
name|rp
operator|->
name|p_textp
operator|||
operator|(
name|rp
operator|->
name|p_textp
operator|->
name|x_flag
operator|&
name|XLOCK
operator|)
operator|==
literal|0
operator|)
operator|&&
name|swappable
argument_list|(
name|rp
argument_list|)
condition|)
block|{
comment|/* 			 * Kick out deadwood. 			 */
operator|(
name|void
operator|)
name|spl6
argument_list|()
expr_stmt|;
name|rp
operator|->
name|p_flag
operator|&=
operator|~
name|SLOAD
expr_stmt|;
if|if
condition|(
name|rp
operator|->
name|p_stat
operator|==
name|SRUN
condition|)
name|remrq
argument_list|(
name|rp
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|spl0
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|swapout
argument_list|(
name|rp
argument_list|,
name|rp
operator|->
name|p_dsize
argument_list|,
name|rp
operator|->
name|p_ssize
argument_list|)
expr_stmt|;
goto|goto
name|loop
goto|;
block|}
continue|continue;
block|}
comment|/* 	 * No one wants in, so nothing to do. 	 */
if|if
condition|(
name|outpri
operator|==
operator|-
literal|20000
condition|)
block|{
operator|(
name|void
operator|)
name|spl6
argument_list|()
expr_stmt|;
if|if
condition|(
name|wantin
condition|)
block|{
name|wantin
operator|=
literal|0
expr_stmt|;
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|lbolt
argument_list|,
name|PSWP
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|runout
operator|++
expr_stmt|;
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|runout
argument_list|,
name|PSWP
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|spl0
argument_list|()
expr_stmt|;
goto|goto
name|loop
goto|;
block|}
comment|/* 	 * Decide how deserving this guy is.  If he is deserving 	 * we will be willing to work harder to bring him in. 	 * Needs is an estimate of how much core he will need. 	 * If he has been out for a while, then we will 	 * bring him in with 1/2 the core he will need, otherwise 	 * we are conservative. 	 */
name|deservin
operator|=
literal|0
expr_stmt|;
name|divisor
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|outpri
operator|>
name|maxslp
operator|/
literal|2
condition|)
block|{
name|deservin
operator|=
literal|1
expr_stmt|;
name|divisor
operator|=
literal|2
expr_stmt|;
block|}
name|needs
operator|=
name|p
operator|->
name|p_swrss
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_textp
operator|&&
name|p
operator|->
name|p_textp
operator|->
name|x_ccount
operator|==
literal|0
condition|)
name|needs
operator|+=
name|p
operator|->
name|p_textp
operator|->
name|x_swrss
expr_stmt|;
name|needs
operator|=
name|imin
argument_list|(
name|needs
argument_list|,
name|lotsfree
argument_list|)
expr_stmt|;
if|if
condition|(
name|freemem
operator|-
name|deficit
operator|>
name|needs
operator|/
name|divisor
condition|)
block|{
name|deficit
operator|+=
name|needs
expr_stmt|;
if|if
condition|(
name|swapin
argument_list|(
name|p
argument_list|)
condition|)
goto|goto
name|loop
goto|;
name|deficit
operator|-=
name|imin
argument_list|(
name|needs
argument_list|,
name|deficit
argument_list|)
expr_stmt|;
block|}
name|hardswap
label|:
comment|/* 	 * Need resources (kernel map or memory), swap someone out. 	 * Select the nbig largest jobs, then the oldest of these 	 * is ``most likely to get booted.'' 	 */
name|inp
operator|=
name|p
expr_stmt|;
name|sleeper
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|nbig
operator|>
name|MAXNBIG
condition|)
name|nbig
operator|=
name|MAXNBIG
expr_stmt|;
if|if
condition|(
name|nbig
operator|<
literal|1
condition|)
name|nbig
operator|=
literal|1
expr_stmt|;
name|biggot
operator|=
literal|0
expr_stmt|;
name|bplist
operator|.
name|bp_link
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|rp
operator|=
name|proc
init|;
name|rp
operator|<
name|procNPROC
condition|;
name|rp
operator|++
control|)
block|{
if|if
condition|(
operator|!
name|swappable
argument_list|(
name|rp
argument_list|)
condition|)
continue|continue;
if|if
condition|(
name|rp
operator|->
name|p_stat
operator|==
name|SZOMB
condition|)
continue|continue;
if|if
condition|(
name|rp
operator|==
name|inp
condition|)
continue|continue;
if|if
condition|(
name|rp
operator|->
name|p_textp
operator|&&
name|rp
operator|->
name|p_textp
operator|->
name|x_flag
operator|&
name|XLOCK
condition|)
continue|continue;
if|if
condition|(
name|rp
operator|->
name|p_slptime
operator|>
name|maxslp
operator|&&
operator|(
name|rp
operator|->
name|p_stat
operator|==
name|SSLEEP
operator|&&
name|rp
operator|->
name|p_pri
operator|>
name|PZERO
operator|||
name|rp
operator|->
name|p_stat
operator|==
name|SSTOP
operator|)
condition|)
block|{
if|if
condition|(
name|sleeper
operator|<
name|rp
operator|->
name|p_slptime
condition|)
block|{
name|p
operator|=
name|rp
expr_stmt|;
name|sleeper
operator|=
name|rp
operator|->
name|p_slptime
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|sleeper
operator|&&
operator|(
name|rp
operator|->
name|p_stat
operator|==
name|SRUN
operator|||
name|rp
operator|->
name|p_stat
operator|==
name|SSLEEP
operator|)
condition|)
block|{
name|rppri
operator|=
name|rp
operator|->
name|p_rssize
expr_stmt|;
if|if
condition|(
name|rp
operator|->
name|p_textp
condition|)
name|rppri
operator|+=
name|rp
operator|->
name|p_textp
operator|->
name|x_rssize
operator|/
name|rp
operator|->
name|p_textp
operator|->
name|x_ccount
expr_stmt|;
if|if
condition|(
name|biggot
operator|<
name|nbig
condition|)
name|nbp
operator|=
operator|&
name|bigp
index|[
name|biggot
operator|++
index|]
expr_stmt|;
else|else
block|{
name|nbp
operator|=
name|bplist
operator|.
name|bp_link
expr_stmt|;
if|if
condition|(
name|nbp
operator|->
name|bp_pri
operator|>
name|rppri
condition|)
continue|continue;
name|bplist
operator|.
name|bp_link
operator|=
name|nbp
operator|->
name|bp_link
expr_stmt|;
block|}
for|for
control|(
name|bp
operator|=
operator|&
name|bplist
init|;
name|bp
operator|->
name|bp_link
condition|;
name|bp
operator|=
name|bp
operator|->
name|bp_link
control|)
if|if
condition|(
name|rppri
operator|<
name|bp
operator|->
name|bp_link
operator|->
name|bp_pri
condition|)
break|break;
name|nbp
operator|->
name|bp_link
operator|=
name|bp
operator|->
name|bp_link
expr_stmt|;
name|bp
operator|->
name|bp_link
operator|=
name|nbp
expr_stmt|;
name|nbp
operator|->
name|bp_pri
operator|=
name|rppri
expr_stmt|;
name|nbp
operator|->
name|bp_proc
operator|=
name|rp
expr_stmt|;
block|}
block|}
if|if
condition|(
operator|!
name|sleeper
condition|)
block|{
name|p
operator|=
name|NULL
expr_stmt|;
name|inpri
operator|=
operator|-
literal|1000
expr_stmt|;
for|for
control|(
name|bp
operator|=
name|bplist
operator|.
name|bp_link
init|;
name|bp
condition|;
name|bp
operator|=
name|bp
operator|->
name|bp_link
control|)
block|{
name|rp
operator|=
name|bp
operator|->
name|bp_proc
expr_stmt|;
name|rppri
operator|=
name|rp
operator|->
name|p_time
operator|+
name|rp
operator|->
name|p_nice
operator|-
name|NZERO
expr_stmt|;
if|if
condition|(
name|rppri
operator|>=
name|inpri
condition|)
block|{
name|p
operator|=
name|rp
expr_stmt|;
name|inpri
operator|=
name|rppri
expr_stmt|;
block|}
block|}
block|}
comment|/* 	 * If we found a long-time sleeper, or we are desperate and 	 * found anyone to swap out, or if someone deserves to come 	 * in and we didn't find a sleeper, but found someone who 	 * has been in core for a reasonable length of time, then 	 * we kick the poor luser out. 	 */
if|if
condition|(
name|sleeper
operator|||
name|desperate
operator|&&
name|p
operator|||
name|deservin
operator|&&
name|inpri
operator|>
name|maxslp
condition|)
block|{
operator|(
name|void
operator|)
name|spl6
argument_list|()
expr_stmt|;
name|p
operator|->
name|p_flag
operator|&=
operator|~
name|SLOAD
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_stat
operator|==
name|SRUN
condition|)
name|remrq
argument_list|(
name|p
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|spl0
argument_list|()
expr_stmt|;
if|if
condition|(
name|desperate
condition|)
block|{
comment|/* 			 * Want to give this space to the rest of 			 * the processes in core so give them a chance 			 * by increasing the deficit. 			 */
name|gives
operator|=
name|p
operator|->
name|p_rssize
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_textp
condition|)
name|gives
operator|+=
name|p
operator|->
name|p_textp
operator|->
name|x_rssize
operator|/
name|p
operator|->
name|p_textp
operator|->
name|x_ccount
expr_stmt|;
name|gives
operator|=
name|min
argument_list|(
name|gives
argument_list|,
name|lotsfree
argument_list|)
expr_stmt|;
name|deficit
operator|+=
name|gives
expr_stmt|;
block|}
else|else
name|gives
operator|=
literal|0
expr_stmt|;
comment|/* someone else taketh away */
if|if
condition|(
name|swapout
argument_list|(
name|p
argument_list|,
name|p
operator|->
name|p_dsize
argument_list|,
name|p
operator|->
name|p_ssize
argument_list|)
operator|==
literal|0
condition|)
name|deficit
operator|-=
name|imin
argument_list|(
name|gives
argument_list|,
name|deficit
argument_list|)
expr_stmt|;
goto|goto
name|loop
goto|;
block|}
comment|/* 	 * Want to swap someone in, but can't 	 * so wait on runin. 	 */
operator|(
name|void
operator|)
name|spl6
argument_list|()
expr_stmt|;
name|runin
operator|++
expr_stmt|;
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|runin
argument_list|,
name|PSWP
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|spl0
argument_list|()
expr_stmt|;
goto|goto
name|loop
goto|;
block|}
end_block

begin_macro
name|vmmeter
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|unsigned
modifier|*
name|cp
decl_stmt|,
modifier|*
name|rp
decl_stmt|,
modifier|*
name|sp
decl_stmt|;
name|deficit
operator|-=
name|imin
argument_list|(
name|deficit
argument_list|,
name|imax
argument_list|(
name|deficit
operator|/
literal|10
argument_list|,
operator|(
operator|(
name|klin
operator|*
name|CLSIZE
operator|)
operator|/
literal|2
operator|)
operator|*
name|maxpgio
operator|/
literal|2
argument_list|)
argument_list|)
expr_stmt|;
name|ave
argument_list|(
name|avefree
argument_list|,
name|freemem
argument_list|,
literal|5
argument_list|)
expr_stmt|;
name|ave
argument_list|(
name|avefree30
argument_list|,
name|freemem
argument_list|,
literal|30
argument_list|)
expr_stmt|;
comment|/* v_pgin is maintained by clock.c */
name|cp
operator|=
operator|&
name|cnt
operator|.
name|v_first
expr_stmt|;
name|rp
operator|=
operator|&
name|rate
operator|.
name|v_first
expr_stmt|;
name|sp
operator|=
operator|&
name|sum
operator|.
name|v_first
expr_stmt|;
while|while
condition|(
name|cp
operator|<=
operator|&
name|cnt
operator|.
name|v_last
condition|)
block|{
name|ave
argument_list|(
operator|*
name|rp
argument_list|,
operator|*
name|cp
argument_list|,
literal|5
argument_list|)
expr_stmt|;
operator|*
name|sp
operator|+=
operator|*
name|cp
expr_stmt|;
operator|*
name|cp
operator|=
literal|0
expr_stmt|;
name|rp
operator|++
operator|,
name|cp
operator|++
operator|,
name|sp
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|time
operator|%
literal|5
operator|==
literal|0
condition|)
block|{
name|vmtotal
argument_list|()
expr_stmt|;
name|rate
operator|.
name|v_swpin
operator|=
name|cnt
operator|.
name|v_swpin
expr_stmt|;
name|sum
operator|.
name|v_swpin
operator|+=
name|cnt
operator|.
name|v_swpin
expr_stmt|;
name|cnt
operator|.
name|v_swpin
operator|=
literal|0
expr_stmt|;
name|rate
operator|.
name|v_swpout
operator|=
name|cnt
operator|.
name|v_swpout
expr_stmt|;
name|sum
operator|.
name|v_swpout
operator|+=
name|cnt
operator|.
name|v_swpout
expr_stmt|;
name|cnt
operator|.
name|v_swpout
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|avefree
operator|<
name|minfree
operator|&&
name|runout
operator|||
name|proc
index|[
literal|0
index|]
operator|.
name|p_slptime
operator|>
name|maxslp
operator|/
literal|2
condition|)
block|{
name|runout
operator|=
literal|0
expr_stmt|;
name|runin
operator|=
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|runin
argument_list|)
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|runout
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_macro
name|vmpago
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|vavail
decl_stmt|;
specifier|register
name|int
name|scanrate
decl_stmt|;
comment|/* 	 * Compute new rate for clock; if 	 * nonzero, restart clock. 	 * Rate ranges linearly from one rev per 	 * slowscan seconds when there is lotsfree memory 	 * available to one rev per fastscan seconds when 	 * there is no memory available. 	 */
name|nscan
operator|=
name|desscan
operator|=
literal|0
expr_stmt|;
name|vavail
operator|=
name|freemem
operator|-
name|deficit
expr_stmt|;
if|if
condition|(
name|vavail
operator|<
literal|0
condition|)
name|vavail
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|freemem
operator|>=
name|lotsfree
condition|)
return|return;
name|scanrate
operator|=
operator|(
name|slowscan
operator|*
name|vavail
operator|+
name|fastscan
operator|*
operator|(
name|lotsfree
operator|-
name|vavail
operator|)
operator|)
operator|/
name|nz
argument_list|(
name|lotsfree
argument_list|)
expr_stmt|;
name|desscan
operator|=
operator|(
name|LOOPPAGES
operator|/
name|CLSIZE
operator|)
operator|/
name|nz
argument_list|(
name|scanrate
argument_list|)
expr_stmt|;
comment|/* 	 * DIVIDE BY 4 TO ACCOUNT FOR RUNNING 4* A SECOND (see clock.c) 	 */
name|desscan
operator|/=
literal|4
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|proc
index|[
literal|2
index|]
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|vmtotal
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
specifier|register
name|struct
name|text
modifier|*
name|xp
decl_stmt|;
name|int
name|nrun
init|=
literal|0
decl_stmt|;
name|total
operator|.
name|t_vmtxt
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_avmtxt
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_rmtxt
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_armtxt
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|xp
operator|=
name|text
init|;
name|xp
operator|<
name|textNTEXT
condition|;
name|xp
operator|++
control|)
if|if
condition|(
name|xp
operator|->
name|x_iptr
condition|)
block|{
name|total
operator|.
name|t_vmtxt
operator|+=
name|xp
operator|->
name|x_size
expr_stmt|;
name|total
operator|.
name|t_rmtxt
operator|+=
name|xp
operator|->
name|x_rssize
expr_stmt|;
for|for
control|(
name|p
operator|=
name|xp
operator|->
name|x_caddr
init|;
name|p
condition|;
name|p
operator|=
name|p
operator|->
name|p_xlink
control|)
switch|switch
condition|(
name|p
operator|->
name|p_stat
condition|)
block|{
case|case
name|SSTOP
case|:
case|case
name|SSLEEP
case|:
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|>=
name|maxslp
condition|)
continue|continue;
comment|/* fall into... */
case|case
name|SRUN
case|:
case|case
name|SIDL
case|:
name|total
operator|.
name|t_avmtxt
operator|+=
name|xp
operator|->
name|x_size
expr_stmt|;
name|total
operator|.
name|t_armtxt
operator|+=
name|xp
operator|->
name|x_rssize
expr_stmt|;
goto|goto
name|next
goto|;
block|}
name|next
label|:
empty_stmt|;
block|}
name|total
operator|.
name|t_vm
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_avm
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_rm
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_arm
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_rq
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_dw
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_pw
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_sl
operator|=
literal|0
expr_stmt|;
name|total
operator|.
name|t_sw
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|p
operator|=
name|proc
init|;
name|p
operator|<
name|procNPROC
condition|;
name|p
operator|++
control|)
block|{
if|if
condition|(
name|p
operator|->
name|p_flag
operator|&
name|SSYS
condition|)
continue|continue;
if|if
condition|(
name|p
operator|->
name|p_stat
condition|)
block|{
name|total
operator|.
name|t_vm
operator|+=
name|p
operator|->
name|p_dsize
operator|+
name|p
operator|->
name|p_ssize
expr_stmt|;
name|total
operator|.
name|t_rm
operator|+=
name|p
operator|->
name|p_rssize
expr_stmt|;
switch|switch
condition|(
name|p
operator|->
name|p_stat
condition|)
block|{
case|case
name|SSLEEP
case|:
case|case
name|SSTOP
case|:
if|if
condition|(
name|p
operator|->
name|p_pri
operator|<=
name|PZERO
condition|)
name|nrun
operator|++
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_flag
operator|&
name|SPAGE
condition|)
name|total
operator|.
name|t_pw
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|p
operator|->
name|p_flag
operator|&
name|SLOAD
condition|)
block|{
if|if
condition|(
name|p
operator|->
name|p_pri
operator|<=
name|PZERO
condition|)
name|total
operator|.
name|t_dw
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|<
name|maxslp
condition|)
name|total
operator|.
name|t_sl
operator|++
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|<
name|maxslp
condition|)
name|total
operator|.
name|t_sw
operator|++
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|<
name|maxslp
condition|)
goto|goto
name|active
goto|;
break|break;
case|case
name|SRUN
case|:
case|case
name|SIDL
case|:
name|nrun
operator|++
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_flag
operator|&
name|SLOAD
condition|)
name|total
operator|.
name|t_rq
operator|++
expr_stmt|;
else|else
name|total
operator|.
name|t_sw
operator|++
expr_stmt|;
name|active
label|:
name|total
operator|.
name|t_avm
operator|+=
name|p
operator|->
name|p_dsize
operator|+
name|p
operator|->
name|p_ssize
expr_stmt|;
name|total
operator|.
name|t_arm
operator|+=
name|p
operator|->
name|p_rssize
expr_stmt|;
break|break;
block|}
block|}
block|}
name|total
operator|.
name|t_vm
operator|+=
name|total
operator|.
name|t_vmtxt
expr_stmt|;
name|total
operator|.
name|t_avm
operator|+=
name|total
operator|.
name|t_avmtxt
expr_stmt|;
name|total
operator|.
name|t_rm
operator|+=
name|total
operator|.
name|t_rmtxt
expr_stmt|;
name|total
operator|.
name|t_arm
operator|+=
name|total
operator|.
name|t_armtxt
expr_stmt|;
name|total
operator|.
name|t_free
operator|=
name|avefree
expr_stmt|;
name|loadav
argument_list|(
name|avenrun
argument_list|,
name|nrun
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Constants for averages over 1, 5, and 15 minutes  * when sampling at 5 second intervals.  */
end_comment

begin_decl_stmt
name|double
name|cexp
index|[
literal|3
index|]
init|=
block|{
literal|0.9200444146293232
block|,
comment|/* exp(-1/12) */
literal|0.9834714538216174
block|,
comment|/* exp(-1/60) */
literal|0.9944598480048967
block|,
comment|/* exp(-1/180) */
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Compute a tenex style load average of a quantity on  * 1, 5 and 15 minute intervals.  */
end_comment

begin_expr_stmt
name|loadav
argument_list|(
name|avg
argument_list|,
name|n
argument_list|)
specifier|register
name|double
operator|*
name|avg
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|n
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|3
condition|;
name|i
operator|++
control|)
name|avg
index|[
name|i
index|]
operator|=
name|cexp
index|[
name|i
index|]
operator|*
name|avg
index|[
name|i
index|]
operator|+
name|n
operator|*
operator|(
literal|1.0
operator|-
name|cexp
index|[
name|i
index|]
operator|)
expr_stmt|;
block|}
end_block

end_unit

