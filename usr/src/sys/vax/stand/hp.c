...
begin_comment
comment|/*  * Copyright (c) 1982 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)hp.c	6.7 (Berkeley) %G%  */
end_comment
...
begin_comment
comment|/* max # bits allow in ecc error w/ F_ECCLM */
end_comment
...
condition|)
block|{
comment|/* 		 * The last ditch effort to bad sector forward 		 * below will probably fail since mba byte ctr 		 * (bcr) is different for BSE and ECC errors and 		 * the wrong block will be revectored to if one 		 * has 2 contiguous bad blocks and reads the second. 		 * For now, we can probably just let a header CRC 		 * error be handled like a BSE since no data will 		 * have been transferred and the bcr should the same 		 * as it would with a BSE error. 		 * --ghg. 		 */
if|if
condition|(
name|er1
operator|&
...
name|restart
goto|;
comment|/* retry whole transfer  --ghg */
name|success
label|:
