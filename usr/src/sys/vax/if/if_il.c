begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	if_il.c	4.6	82/06/13	*/
end_comment

begin_include
include|#
directive|include
file|"il.h"
end_include

begin_comment
comment|/*  * Interlan Ethernet Communications Controller interface  */
end_comment

begin_include
include|#
directive|include
file|"../h/param.h"
end_include

begin_include
include|#
directive|include
file|"../h/systm.h"
end_include

begin_include
include|#
directive|include
file|"../h/mbuf.h"
end_include

begin_include
include|#
directive|include
file|"../h/pte.h"
end_include

begin_include
include|#
directive|include
file|"../h/buf.h"
end_include

begin_include
include|#
directive|include
file|"../h/protosw.h"
end_include

begin_include
include|#
directive|include
file|"../h/socket.h"
end_include

begin_include
include|#
directive|include
file|"../h/ubareg.h"
end_include

begin_include
include|#
directive|include
file|"../h/ubavar.h"
end_include

begin_include
include|#
directive|include
file|"../h/ilreg.h"
end_include

begin_include
include|#
directive|include
file|"../h/cpu.h"
end_include

begin_include
include|#
directive|include
file|"../h/mtpr.h"
end_include

begin_include
include|#
directive|include
file|"../h/vmmac.h"
end_include

begin_include
include|#
directive|include
file|"../net/in.h"
end_include

begin_include
include|#
directive|include
file|"../net/in_systm.h"
end_include

begin_include
include|#
directive|include
file|"../net/if.h"
end_include

begin_include
include|#
directive|include
file|"../net/if_il.h"
end_include

begin_include
include|#
directive|include
file|"../net/if_uba.h"
end_include

begin_include
include|#
directive|include
file|"../net/ip.h"
end_include

begin_include
include|#
directive|include
file|"../net/ip_var.h"
end_include

begin_include
include|#
directive|include
file|"../net/pup.h"
end_include

begin_include
include|#
directive|include
file|"../net/route.h"
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_define
define|#
directive|define
name|ILMTU
value|1500
end_define

begin_decl_stmt
name|int
name|ilprobe
argument_list|()
decl_stmt|,
name|ilattach
argument_list|()
decl_stmt|,
name|ilrint
argument_list|()
decl_stmt|,
name|ilcint
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|uba_device
modifier|*
name|ilinfo
index|[
name|NIL
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|u_short
name|ilstd
index|[]
init|=
block|{
literal|0
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|uba_driver
name|ildriver
init|=
block|{
name|ilprobe
block|,
literal|0
block|,
name|ilattach
block|,
literal|0
block|,
name|ilstd
block|,
literal|"il"
block|,
name|ilinfo
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|u_char
name|il_ectop
index|[
literal|3
index|]
init|=
block|{
literal|0x02
block|,
literal|0x60
block|,
literal|0x8c
block|}
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|ILUNIT
parameter_list|(
name|x
parameter_list|)
value|minor(x)
end_define

begin_decl_stmt
name|int
name|ilinit
argument_list|()
decl_stmt|,
name|iloutput
argument_list|()
decl_stmt|,
name|ilreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Ethernet software status per interface.  *  * Each interface is referenced by a network interface structure,  * is_if, which the routing code uses to locate the interface.  * This structure contains the output queue for the interface, its address, ...  * We also have, for each interface, a UBA interface structure, which  * contains information about the UNIBUS resources held by the interface:  * map registers, buffered data paths, etc.  Information is cached in this  * structure for use by the if_uba.c routines in running the interface  * efficiently.  */
end_comment

begin_struct
struct|struct
name|il_softc
block|{
name|struct
name|ifnet
name|is_if
decl_stmt|;
comment|/* network-visible interface */
name|struct
name|ifuba
name|is_ifuba
decl_stmt|;
comment|/* UNIBUS resources */
name|short
name|is_oactive
decl_stmt|;
comment|/* is output active? */
name|short
name|is_startrcv
decl_stmt|;
comment|/* hang receive next chance */
name|u_char
name|is_enaddr
index|[
literal|6
index|]
decl_stmt|;
comment|/* board's ethernet address */
block|}
name|il_softc
index|[
name|NIL
index|]
struct|;
end_struct

begin_comment
comment|/*  * Do an OFFLINE command.  This will cause an interrupt for the  * autoconfigure stuff.  */
end_comment

begin_macro
name|ilprobe
argument_list|(
argument|reg
argument_list|)
end_macro

begin_decl_stmt
name|caddr_t
name|reg
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|br
decl_stmt|,
name|cvec
decl_stmt|;
comment|/* r11, r10 value-result */
specifier|register
name|struct
name|ildevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|ildevice
operator|*
operator|)
name|reg
decl_stmt|;
specifier|register
name|i
expr_stmt|;
name|COUNT
argument_list|(
name|ILPROBE
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|lint
name|br
operator|=
literal|0
expr_stmt|;
name|cvec
operator|=
name|br
expr_stmt|;
name|br
operator|=
name|cvec
expr_stmt|;
name|ilrint
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|ilcint
argument_list|(
literal|0
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|addr
operator|->
name|il_csr
operator|=
name|ILC_OFFLINE
operator||
name|IL_CIE
expr_stmt|;
name|DELAY
argument_list|(
literal|100000
argument_list|)
expr_stmt|;
name|i
operator|=
name|addr
operator|->
name|il_csr
expr_stmt|;
comment|/* Clear CDONE */
if|if
condition|(
name|cvec
operator|>
literal|0
operator|&&
name|cvec
operator|!=
literal|0x200
condition|)
name|cvec
operator|-=
literal|4
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_decl_stmt
name|struct
name|il_stat
name|ilbuf
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Interface exists: make available by filling in network interface  * record.  System will initialize the interface when it is ready  * to accept packets.  A STATUS command is done to get the ethernet  * address and other interesting data.  */
end_comment

begin_macro
name|ilattach
argument_list|(
argument|ui
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|uba_device
modifier|*
name|ui
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|il_softc
modifier|*
name|is
init|=
operator|&
name|il_softc
index|[
name|ui
operator|->
name|ui_unit
index|]
decl_stmt|;
specifier|register
name|struct
name|sockaddr_in
modifier|*
name|sin
decl_stmt|;
specifier|register
name|struct
name|ildevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|ildevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
decl_stmt|;
specifier|register
name|int
name|i
decl_stmt|;
name|int
name|s
decl_stmt|;
name|int
name|ubaddr
decl_stmt|;
name|COUNT
argument_list|(
name|ILATTACH
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_unit
operator|=
name|ui
operator|->
name|ui_unit
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_name
operator|=
literal|"il"
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_mtu
operator|=
name|ILMTU
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_net
operator|=
name|ui
operator|->
name|ui_flags
expr_stmt|;
comment|/* 	 * Reset the board 	 */
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
name|addr
operator|->
name|il_csr
operator|=
operator|(
operator|(
name|ubaddr
operator|>>
literal|2
operator|)
operator|&
literal|0xc000
operator|)
operator||
name|ILC_RESET
expr_stmt|;
while|while
condition|(
operator|!
operator|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_CDONE
operator|)
condition|)
comment|/* Busy wait */
empty_stmt|;
if|if
condition|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_STATUS
condition|)
name|printf
argument_list|(
literal|"il%d: %s\n"
argument_list|,
name|ui
operator|->
name|ui_unit
argument_list|,
name|ildiag
index|[
name|addr
operator|->
name|il_csr
operator|&
name|IL_STATUS
index|]
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
comment|/* 	 * Map the status buffer to the Unibus, do the status command, 	 * and unmap the buffer. 	 */
name|ubaddr
operator|=
name|uballoc
argument_list|(
name|ui
operator|->
name|ui_ubanum
argument_list|,
operator|&
name|ilbuf
argument_list|,
sizeof|sizeof
argument_list|(
name|ilbuf
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
name|addr
operator|->
name|il_bar
operator|=
name|ubaddr
operator|&
literal|0xffff
expr_stmt|;
name|addr
operator|->
name|il_bcr
operator|=
sizeof|sizeof
argument_list|(
name|ilbuf
argument_list|)
expr_stmt|;
name|addr
operator|->
name|il_csr
operator|=
operator|(
operator|(
name|ubaddr
operator|>>
literal|2
operator|)
operator|&
literal|0xc000
operator|)
operator||
name|ILC_STAT
expr_stmt|;
while|while
condition|(
operator|!
operator|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_CDONE
operator|)
condition|)
comment|/* Busy wait */
empty_stmt|;
if|if
condition|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_STATUS
condition|)
name|printf
argument_list|(
literal|"il%d: %s\n"
argument_list|,
name|ui
operator|->
name|ui_unit
argument_list|,
name|ilerrs
index|[
name|addr
operator|->
name|il_csr
operator|&
name|IL_STATUS
index|]
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|ubarelse
argument_list|(
name|ui
operator|->
name|ui_ubanum
argument_list|,
operator|&
name|ubaddr
argument_list|)
expr_stmt|;
comment|/* 	 * Fill in the Ethernet address from the status buffer 	 */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|6
condition|;
name|i
operator|++
control|)
name|is
operator|->
name|is_enaddr
index|[
name|i
index|]
operator|=
name|ilbuf
operator|.
name|ils_addr
index|[
name|i
index|]
expr_stmt|;
name|printf
argument_list|(
literal|"il%d: addr=%x:%x:%x:%x:%x:%x module=%s firmware=%s\n"
argument_list|,
name|ui
operator|->
name|ui_unit
argument_list|,
name|is
operator|->
name|is_enaddr
index|[
literal|0
index|]
operator|&
literal|0xff
argument_list|,
name|is
operator|->
name|is_enaddr
index|[
literal|1
index|]
operator|&
literal|0xff
argument_list|,
name|is
operator|->
name|is_enaddr
index|[
literal|2
index|]
operator|&
literal|0xff
argument_list|,
name|is
operator|->
name|is_enaddr
index|[
literal|3
index|]
operator|&
literal|0xff
argument_list|,
name|is
operator|->
name|is_enaddr
index|[
literal|4
index|]
operator|&
literal|0xff
argument_list|,
name|is
operator|->
name|is_enaddr
index|[
literal|5
index|]
operator|&
literal|0xff
argument_list|,
name|ilbuf
operator|.
name|ils_module
argument_list|,
name|ilbuf
operator|.
name|ils_firmware
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_host
index|[
literal|0
index|]
operator|=
operator|(
operator|(
name|is
operator|->
name|is_enaddr
index|[
literal|3
index|]
operator|&
literal|0xff
operator|)
operator|<<
literal|16
operator|)
operator||
literal|0x800000
operator||
operator|(
operator|(
name|is
operator|->
name|is_enaddr
index|[
literal|4
index|]
operator|&
literal|0xff
operator|)
operator|<<
literal|8
operator|)
operator||
operator|(
name|is
operator|->
name|is_enaddr
index|[
literal|5
index|]
operator|&
literal|0xff
operator|)
expr_stmt|;
name|sin
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|is
operator|->
name|is_if
operator|.
name|if_addr
expr_stmt|;
name|sin
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|sin
operator|->
name|sin_addr
operator|=
name|if_makeaddr
argument_list|(
name|is
operator|->
name|is_if
operator|.
name|if_net
argument_list|,
name|is
operator|->
name|is_if
operator|.
name|if_host
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|sin
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|is
operator|->
name|is_if
operator|.
name|if_broadaddr
expr_stmt|;
name|sin
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|sin
operator|->
name|sin_addr
operator|=
name|if_makeaddr
argument_list|(
name|is
operator|->
name|is_if
operator|.
name|if_net
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_flags
operator|=
name|IFF_BROADCAST
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_init
operator|=
name|ilinit
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_output
operator|=
name|iloutput
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_ubareset
operator|=
name|ilreset
expr_stmt|;
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_flags
operator|=
name|UBA_CANTWAIT
expr_stmt|;
name|if_attach
argument_list|(
operator|&
name|is
operator|->
name|is_if
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Reset of interface after UNIBUS reset.  * If interface is on specified uba, reset its state.  */
end_comment

begin_macro
name|ilreset
argument_list|(
argument|unit
argument_list|,
argument|uban
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|,
name|uban
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|uba_device
modifier|*
name|ui
decl_stmt|;
name|COUNT
argument_list|(
name|ILRESET
argument_list|)
expr_stmt|;
if|if
condition|(
name|unit
operator|>=
name|NIL
operator|||
operator|(
name|ui
operator|=
name|ilinfo
index|[
name|unit
index|]
operator|)
operator|==
literal|0
operator|||
name|ui
operator|->
name|ui_alive
operator|==
literal|0
operator|||
name|ui
operator|->
name|ui_ubanum
operator|!=
name|uban
condition|)
return|return;
name|printf
argument_list|(
literal|" il%d"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
name|ilinit
argument_list|(
name|unit
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Initialization of interface; clear recorded pending  * operations, and reinitialize UNIBUS usage.  */
end_comment

begin_macro
name|ilinit
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|il_softc
modifier|*
name|is
init|=
operator|&
name|il_softc
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|uba_device
modifier|*
name|ui
init|=
name|ilinfo
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|ildevice
modifier|*
name|addr
decl_stmt|;
specifier|register
name|i
expr_stmt|;
name|int
name|s
decl_stmt|;
if|if
condition|(
name|if_ubainit
argument_list|(
operator|&
name|is
operator|->
name|is_ifuba
argument_list|,
name|ui
operator|->
name|ui_ubanum
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|il_rheader
argument_list|)
argument_list|,
operator|(
name|int
operator|)
name|btoc
argument_list|(
name|ILMTU
argument_list|)
argument_list|)
operator|==
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"il%d: can't initialize\n"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_flags
operator|&=
operator|~
name|IFF_UP
expr_stmt|;
return|return;
block|}
name|addr
operator|=
operator|(
expr|struct
name|ildevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
expr_stmt|;
comment|/* 	 * Set board online. 	 * Hang receive buffer and start any pending 	 * writes by faking a transmit complete. 	 * Receive bcr is not a muliple of 4 so buffer 	 * chaining can't happen. 	 */
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
name|addr
operator|->
name|il_csr
operator|=
name|ILC_ONLINE
expr_stmt|;
while|while
condition|(
operator|!
operator|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_CDONE
operator|)
condition|)
comment|/* Busy wait */
empty_stmt|;
name|addr
operator|->
name|il_bar
operator|=
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_info
operator|&
literal|0xffff
expr_stmt|;
name|addr
operator|->
name|il_bcr
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|il_rheader
argument_list|)
operator|+
name|ILMTU
operator|+
literal|6
expr_stmt|;
name|addr
operator|->
name|il_csr
operator|=
operator|(
operator|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_info
operator|>>
literal|2
operator|)
operator|&
literal|0xc000
operator|)
operator||
name|ILC_RCV
operator||
name|IL_RIE
expr_stmt|;
while|while
condition|(
operator|!
operator|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_CDONE
operator|)
condition|)
comment|/* Busy wait */
empty_stmt|;
name|is
operator|->
name|is_startrcv
operator|=
literal|0
expr_stmt|;
name|is
operator|->
name|is_oactive
operator|=
literal|1
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_flags
operator||=
name|IFF_UP
expr_stmt|;
name|ilcint
argument_list|(
name|unit
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|if_rtinit
argument_list|(
operator|&
name|is
operator|->
name|is_if
argument_list|,
name|RTF_UP
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Start output on interface.  * Get another datagram to send off of the interface queue,  * and map it to the interface before starting the output.  */
end_comment

begin_macro
name|ilstart
argument_list|(
argument|dev
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|unit
init|=
name|ILUNIT
argument_list|(
name|dev
argument_list|)
decl_stmt|;
name|struct
name|uba_device
modifier|*
name|ui
init|=
name|ilinfo
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|il_softc
modifier|*
name|is
init|=
operator|&
name|il_softc
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|ildevice
modifier|*
name|addr
decl_stmt|;
specifier|register
name|len
expr_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|int
name|dest
decl_stmt|;
name|COUNT
argument_list|(
name|ILSTART
argument_list|)
expr_stmt|;
comment|/* 	 * Dequeue another request and copy it into the buffer. 	 * If no more requests, just return. 	 */
name|IF_DEQUEUE
argument_list|(
operator|&
name|is
operator|->
name|is_if
operator|.
name|if_snd
argument_list|,
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
return|return;
name|len
operator|=
name|if_wubaput
argument_list|(
operator|&
name|is
operator|->
name|is_ifuba
argument_list|,
name|m
argument_list|)
expr_stmt|;
comment|/* 	 * Flush BDP, then start the output. 	 */
if|if
condition|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_flags
operator|&
name|UBA_NEEDBDP
condition|)
name|UBAPURGE
argument_list|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_uba
argument_list|,
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_w
operator|.
name|ifrw_bdp
argument_list|)
expr_stmt|;
name|addr
operator|=
operator|(
expr|struct
name|ildevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
expr_stmt|;
name|addr
operator|->
name|il_bar
operator|=
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_w
operator|.
name|ifrw_info
operator|&
literal|0xffff
expr_stmt|;
name|addr
operator|->
name|il_bcr
operator|=
name|len
expr_stmt|;
name|addr
operator|->
name|il_csr
operator|=
operator|(
operator|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_w
operator|.
name|ifrw_info
operator|>>
literal|2
operator|)
operator|&
literal|0xc000
operator|)
operator||
name|ILC_XMIT
operator||
name|IL_CIE
operator||
name|IL_RIE
expr_stmt|;
name|is
operator|->
name|is_oactive
operator|=
literal|1
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Command done interrupt.  * This should only happen after a transmit command,  * so it is equivalent to a transmit interrupt.  * Start another output if more data to send.  */
end_comment

begin_macro
name|ilcint
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|uba_device
modifier|*
name|ui
init|=
name|ilinfo
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|il_softc
modifier|*
name|is
init|=
operator|&
name|il_softc
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|ildevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|ildevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
decl_stmt|;
specifier|register
name|int
name|err
decl_stmt|;
name|COUNT
argument_list|(
name|ILCINT
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|is_oactive
operator|==
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"il%d: strange xmit interrupt!\n"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
return|return;
block|}
name|is
operator|->
name|is_if
operator|.
name|if_opackets
operator|++
expr_stmt|;
name|is
operator|->
name|is_oactive
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|err
operator|=
operator|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_STATUS
operator|)
condition|)
block|{
name|is
operator|->
name|is_if
operator|.
name|if_oerrors
operator|++
expr_stmt|;
name|printf
argument_list|(
literal|"il%d: output error %d\n"
argument_list|,
name|unit
argument_list|,
name|err
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Hang receive buffer if it couldn't be done earlier (in ilrint). 	 */
if|if
condition|(
name|is
operator|->
name|is_startrcv
condition|)
block|{
name|addr
operator|->
name|il_bar
operator|=
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_info
operator|&
literal|0xffff
expr_stmt|;
name|addr
operator|->
name|il_bcr
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|il_rheader
argument_list|)
operator|+
name|ILMTU
operator|+
literal|6
expr_stmt|;
name|addr
operator|->
name|il_csr
operator|=
operator|(
operator|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_info
operator|>>
literal|2
operator|)
operator|&
literal|0xc000
operator|)
operator||
name|ILC_RCV
operator||
name|IL_RIE
expr_stmt|;
while|while
condition|(
operator|!
operator|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_CDONE
operator|)
condition|)
comment|/* Busy wait */
empty_stmt|;
name|is
operator|->
name|is_startrcv
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_xtofree
condition|)
block|{
name|m_freem
argument_list|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_xtofree
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_xtofree
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|is
operator|->
name|is_if
operator|.
name|if_snd
operator|.
name|ifq_head
operator|==
literal|0
condition|)
block|{
return|return;
block|}
name|ilstart
argument_list|(
name|unit
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Ethernet interface receiver interrupt.  * If input error just drop packet.  * Otherwise purge input buffered data path and examine   * packet to determine type.  If can't determine length  * from type, then have to drop packet.  Othewise decapsulate  * packet based on type and pass to type specific higher-level  * input routine.  */
end_comment

begin_macro
name|ilrint
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|il_softc
modifier|*
name|is
init|=
operator|&
name|il_softc
index|[
name|unit
index|]
decl_stmt|;
name|struct
name|ildevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|ildevice
operator|*
operator|)
name|ilinfo
index|[
name|unit
index|]
operator|->
name|ui_addr
decl_stmt|;
specifier|register
name|struct
name|il_rheader
modifier|*
name|il
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|int
name|len
decl_stmt|,
name|off
decl_stmt|,
name|resid
decl_stmt|;
specifier|register
name|struct
name|ifqueue
modifier|*
name|inq
decl_stmt|;
name|COUNT
argument_list|(
name|ILRINT
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_ipackets
operator|++
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_flags
operator|&
name|UBA_NEEDBDP
condition|)
name|UBAPURGE
argument_list|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_uba
argument_list|,
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_bdp
argument_list|)
expr_stmt|;
name|il
operator|=
operator|(
expr|struct
name|il_rheader
operator|*
operator|)
operator|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_addr
operator|)
expr_stmt|;
name|len
operator|=
name|il
operator|->
name|ilr_length
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|il_rheader
argument_list|)
expr_stmt|;
if|if
condition|(
name|il
operator|->
name|ilr_status
operator|&
literal|0x3
operator|||
name|len
operator|<
literal|46
operator|||
name|len
operator|>
name|ILMTU
condition|)
block|{
name|is
operator|->
name|is_if
operator|.
name|if_ierrors
operator|++
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
if|if
condition|(
name|is
operator|->
name|is_if
operator|.
name|if_ierrors
operator|%
literal|100
operator|==
literal|0
condition|)
name|printf
argument_list|(
literal|"il%d: += 100 input errors\n"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|printf
argument_list|(
literal|"il%d: input error (status=%x, len=%d)\n"
argument_list|,
name|unit
argument_list|,
name|il
operator|->
name|ilr_status
argument_list|,
name|len
argument_list|)
expr_stmt|;
goto|goto
name|setup
goto|;
block|}
comment|/* 	 * Deal with trailer protocol: if type is PUP trailer 	 * get true type from first 16-bit word past data. 	 * Remember that type was trailer by setting off. 	 */
define|#
directive|define
name|ildataaddr
parameter_list|(
name|il
parameter_list|,
name|off
parameter_list|,
name|type
parameter_list|)
value|((type)(((caddr_t)((il)+1)+(off))))
if|if
condition|(
name|il
operator|->
name|ilr_type
operator|>=
name|ILPUP_TRAIL
operator|&&
name|il
operator|->
name|ilr_type
operator|<
name|ILPUP_TRAIL
operator|+
name|ILPUP_NTRAILER
condition|)
block|{
name|off
operator|=
operator|(
name|il
operator|->
name|ilr_type
operator|-
name|ILPUP_TRAIL
operator|)
operator|*
literal|512
expr_stmt|;
if|if
condition|(
name|off
operator|>=
name|ILMTU
condition|)
goto|goto
name|setup
goto|;
comment|/* sanity */
name|il
operator|->
name|ilr_type
operator|=
operator|*
name|ildataaddr
argument_list|(
name|il
argument_list|,
name|off
argument_list|,
name|u_short
operator|*
argument_list|)
expr_stmt|;
name|resid
operator|=
operator|*
operator|(
name|ildataaddr
argument_list|(
name|il
argument_list|,
name|off
operator|+
literal|2
argument_list|,
name|u_short
operator|*
argument_list|)
operator|)
expr_stmt|;
if|if
condition|(
name|off
operator|+
name|resid
operator|>
name|len
condition|)
goto|goto
name|setup
goto|;
comment|/* sanity */
name|len
operator|=
name|off
operator|+
name|resid
expr_stmt|;
block|}
else|else
name|off
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|len
operator|==
literal|0
condition|)
goto|goto
name|setup
goto|;
comment|/* 	 * Pull packet off interface.  Off is nonzero if packet 	 * has trailing header; ilget will then force this header 	 * information to be at the front, but we still have to drop 	 * the type and length which are at the front of any trailer data. 	 */
name|m
operator|=
name|if_rubaget
argument_list|(
operator|&
name|is
operator|->
name|is_ifuba
argument_list|,
name|len
argument_list|,
name|off
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
goto|goto
name|setup
goto|;
if|if
condition|(
name|off
condition|)
block|{
name|m
operator|->
name|m_off
operator|+=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|-=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|il
operator|->
name|ilr_type
condition|)
block|{
ifdef|#
directive|ifdef
name|INET
case|case
name|ILPUP_IPTYPE
case|:
name|schednetisr
argument_list|(
name|NETISR_IP
argument_list|)
expr_stmt|;
name|inq
operator|=
operator|&
name|ipintrq
expr_stmt|;
break|break;
endif|#
directive|endif
default|default:
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
goto|goto
name|setup
goto|;
block|}
if|if
condition|(
name|IF_QFULL
argument_list|(
name|inq
argument_list|)
condition|)
block|{
name|IF_DROP
argument_list|(
name|inq
argument_list|)
expr_stmt|;
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
else|else
name|IF_ENQUEUE
argument_list|(
name|inq
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|setup
label|:
comment|/* 	 * Reset for next packet if possible. 	 * If waiting for transmit command completion, set flag 	 * and wait until command completes. 	 */
if|if
condition|(
operator|!
name|is
operator|->
name|is_oactive
condition|)
block|{
name|addr
operator|->
name|il_bar
operator|=
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_info
operator|&
literal|0xffff
expr_stmt|;
name|addr
operator|->
name|il_bcr
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|il_rheader
argument_list|)
operator|+
name|ILMTU
operator|+
literal|6
expr_stmt|;
name|addr
operator|->
name|il_csr
operator|=
operator|(
operator|(
name|is
operator|->
name|is_ifuba
operator|.
name|ifu_r
operator|.
name|ifrw_info
operator|>>
literal|2
operator|)
operator|&
literal|0xc000
operator|)
operator||
name|ILC_RCV
operator||
name|IL_RIE
expr_stmt|;
while|while
condition|(
operator|!
operator|(
name|addr
operator|->
name|il_csr
operator|&
name|IL_CDONE
operator|)
condition|)
comment|/* Busy wait */
empty_stmt|;
block|}
else|else
name|is
operator|->
name|is_startrcv
operator|=
literal|1
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Ethernet output routine.  * Encapsulate a packet of type family for the local net.  * Use trailer local net encapsulation if enough data in first  * packet leaves a multiple of 512 bytes of data in remainder.  */
end_comment

begin_macro
name|iloutput
argument_list|(
argument|ifp
argument_list|,
argument|m0
argument_list|,
argument|dst
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|type
decl_stmt|,
name|dest
decl_stmt|,
name|s
decl_stmt|,
name|error
decl_stmt|;
specifier|register
name|struct
name|il_softc
modifier|*
name|is
init|=
operator|&
name|il_softc
index|[
name|ifp
operator|->
name|if_unit
index|]
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
init|=
name|m0
decl_stmt|;
specifier|register
name|struct
name|il_xheader
modifier|*
name|il
decl_stmt|;
specifier|register
name|int
name|off
decl_stmt|;
specifier|register
name|int
name|i
decl_stmt|;
name|COUNT
argument_list|(
name|ILOUTPUT
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|dst
operator|->
name|sa_family
condition|)
block|{
ifdef|#
directive|ifdef
name|INET
case|case
name|AF_INET
case|:
name|dest
operator|=
operator|(
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|dst
operator|)
operator|->
name|sin_addr
operator|.
name|s_addr
expr_stmt|;
name|off
operator|=
name|ntohs
argument_list|(
operator|(
name|u_short
operator|)
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
operator|->
name|ip_len
argument_list|)
operator|-
name|m
operator|->
name|m_len
expr_stmt|;
if|if
condition|(
name|off
operator|>
literal|0
operator|&&
operator|(
name|off
operator|&
literal|0x1ff
operator|)
operator|==
literal|0
operator|&&
name|m
operator|->
name|m_off
operator|>=
name|MMINOFF
operator|+
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
condition|)
block|{
name|type
operator|=
name|ILPUP_TRAIL
operator|+
operator|(
name|off
operator|>>
literal|9
operator|)
expr_stmt|;
name|m
operator|->
name|m_off
operator|-=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
operator|*
name|mtod
argument_list|(
name|m
argument_list|,
name|u_short
operator|*
argument_list|)
operator|=
name|ILPUP_IPTYPE
expr_stmt|;
operator|*
operator|(
name|mtod
argument_list|(
name|m
argument_list|,
name|u_short
operator|*
argument_list|)
operator|+
literal|1
operator|)
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
goto|goto
name|gottrailertype
goto|;
block|}
name|type
operator|=
name|ILPUP_IPTYPE
expr_stmt|;
name|off
operator|=
literal|0
expr_stmt|;
goto|goto
name|gottype
goto|;
endif|#
directive|endif
default|default:
name|printf
argument_list|(
literal|"il%d: can't handle af%d\n"
argument_list|,
name|ifp
operator|->
name|if_unit
argument_list|,
name|dst
operator|->
name|sa_family
argument_list|)
expr_stmt|;
name|error
operator|=
name|EAFNOSUPPORT
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|gottrailertype
label|:
comment|/* 	 * Packet to be sent as trailer: move first packet 	 * (control information) to end of chain. 	 */
while|while
condition|(
name|m
operator|->
name|m_next
condition|)
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
name|m
operator|->
name|m_next
operator|=
name|m0
expr_stmt|;
name|m
operator|=
name|m0
operator|->
name|m_next
expr_stmt|;
name|m0
operator|->
name|m_next
operator|=
literal|0
expr_stmt|;
name|m0
operator|=
name|m
expr_stmt|;
name|gottype
label|:
comment|/* 	 * Add local net header.  If no space in first mbuf, 	 * allocate another. 	 */
if|if
condition|(
name|m
operator|->
name|m_off
operator|>
name|MMAXOFF
operator|||
name|MMINOFF
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|il_xheader
argument_list|)
operator|>
name|m
operator|->
name|m_off
condition|)
block|{
name|m
operator|=
name|m_get
argument_list|(
name|M_DONTWAIT
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|m
operator|->
name|m_next
operator|=
name|m0
expr_stmt|;
name|m
operator|->
name|m_off
operator|=
name|MMINOFF
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|il_xheader
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|m
operator|->
name|m_off
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|il_xheader
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
sizeof|sizeof
argument_list|(
expr|struct
name|il_xheader
argument_list|)
expr_stmt|;
block|}
name|il
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|il_xheader
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|dest
operator|&
operator|~
literal|0xff
operator|)
operator|==
literal|0
condition|)
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|6
condition|;
name|i
operator|++
control|)
name|il
operator|->
name|ilx_dhost
index|[
name|i
index|]
operator|=
literal|0xff
expr_stmt|;
else|else
block|{
if|if
condition|(
name|dest
operator|&
literal|0x8000
condition|)
block|{
name|il
operator|->
name|ilx_dhost
index|[
literal|0
index|]
operator|=
name|is
operator|->
name|is_enaddr
index|[
literal|0
index|]
expr_stmt|;
name|il
operator|->
name|ilx_dhost
index|[
literal|1
index|]
operator|=
name|is
operator|->
name|is_enaddr
index|[
literal|1
index|]
expr_stmt|;
name|il
operator|->
name|ilx_dhost
index|[
literal|2
index|]
operator|=
name|is
operator|->
name|is_enaddr
index|[
literal|2
index|]
expr_stmt|;
block|}
else|else
block|{
name|il
operator|->
name|ilx_dhost
index|[
literal|0
index|]
operator|=
name|il_ectop
index|[
literal|0
index|]
expr_stmt|;
name|il
operator|->
name|ilx_dhost
index|[
literal|1
index|]
operator|=
name|il_ectop
index|[
literal|1
index|]
expr_stmt|;
name|il
operator|->
name|ilx_dhost
index|[
literal|2
index|]
operator|=
name|il_ectop
index|[
literal|2
index|]
expr_stmt|;
block|}
name|il
operator|->
name|ilx_dhost
index|[
literal|3
index|]
operator|=
operator|(
name|dest
operator|>>
literal|8
operator|)
operator|&
literal|0x7f
expr_stmt|;
name|il
operator|->
name|ilx_dhost
index|[
literal|4
index|]
operator|=
operator|(
name|dest
operator|>>
literal|16
operator|)
operator|&
literal|0xff
expr_stmt|;
name|il
operator|->
name|ilx_dhost
index|[
literal|5
index|]
operator|=
operator|(
name|dest
operator|>>
literal|24
operator|)
operator|&
literal|0xff
expr_stmt|;
block|}
name|il
operator|->
name|ilx_type
operator|=
name|type
expr_stmt|;
comment|/* 	 * Queue message on interface, and start output if interface 	 * not yet active. 	 */
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
if|if
condition|(
name|IF_QFULL
argument_list|(
operator|&
name|ifp
operator|->
name|if_snd
argument_list|)
condition|)
block|{
name|IF_DROP
argument_list|(
operator|&
name|ifp
operator|->
name|if_snd
argument_list|)
expr_stmt|;
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|qfull
goto|;
block|}
name|IF_ENQUEUE
argument_list|(
operator|&
name|ifp
operator|->
name|if_snd
argument_list|,
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|is
operator|->
name|is_oactive
operator|==
literal|0
condition|)
name|ilstart
argument_list|(
name|ifp
operator|->
name|if_unit
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
name|qfull
label|:
name|m0
operator|=
name|m
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|bad
label|:
name|m_freem
argument_list|(
name|m0
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

end_unit

