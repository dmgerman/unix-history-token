...
begin_comment
comment|/*  * Copyright (c) 1982 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)if_ec.c	6.13 (Berkeley) %G%  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* 		 * write our ethernet address into the address recognition ROM  		 * so we can always use the same EC_READ bits (referencing ROM), 		 * in case we change the address sometime. 		 * Note that this is safe here as the reciever is NOT armed. 		 */
name|ec_setaddr
argument_list|(
name|es
operator|->
...
argument_list|)
expr_stmt|;
comment|/* before, so we can send the ARP packet */
operator|(
operator|(
expr|struct
name|arpcom
...
block|{
comment|/*  				 * The manual says we can't change the address  				 * while the reciever is armed so reset everything 				 */
name|struct
name|ec_softc
modifier|*
name|es
...
name|j
decl_stmt|;
comment|/* 	 * Use the ethernet address supplied 	 * NOte that we do a UECLR here, so the recieve buffers 	 * must be requeued. 	 */
ifdef|#
directive|ifdef
name|DEBUG
name|printf
