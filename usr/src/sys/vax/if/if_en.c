...
literal|0
expr_stmt|;
comment|/* 	 * Hang pending read, start any writes. 	 */
name|s
operator|=
name|splimp
argument_list|()
...
begin_comment
comment|/*  * Collision on ethernet interface.  Do exponential  * backoff, and retransmit.  If have backed off all  * the way printing warning diagnostic, and drop packet.  */
end_comment
...
name|ui_addr
expr_stmt|;
comment|/* 	 * Purge BDP; drop error packets. 	 */
name|UBAPURGE
argument_list|(
name|es
operator|->
...
goto|;
block|}
comment|/* 	 * Get pointer to ethernet header (in input buffer). 	 * Deal with trailer protocol: if type is PUP trailer 	 * get true type from first 16-bit word past data. 	 * Remember that type was trailer by setting off. 	 */
name|en
operator|=
operator|(
expr|struct
...
name|setup
goto|;
comment|/* 	 * Pull packet off interface.  Off is nonzero if packet 	 * has trailing header; if_rubaget will then force this header 	 * information to be at the front, but we still have to drop 	 * the two-byte type which is at the front of the trailer data 	 * (which we ``consumed'' above). 	 */
name|m
operator|=
name|if_rubaget
argument_list|(
