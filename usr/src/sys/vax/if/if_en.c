...
begin_endif
unit|enreset(uban) 	int uban; { 	register int unit; 	struct uba_device *ui;  	for (unit = 0; unit< NEN; unit++) { 		ui = eninfo[unit]; 		if (ui == 0 || ui->ui_ubanum != uban || ui->ui_alive == 0) 			continue; 		if (imp_stat.iaddr) 			ubarelse(uban,&imp_stat.iaddr); 		if (imp_stat.oaddr) 			ubarelse(uban,&imp_stat.oaddr); 		eninit(unit); 		printf("en%d ", unit); 	} }
endif|#
directive|endif
end_endif
