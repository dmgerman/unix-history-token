...
begin_comment
comment|/*  * Return our host address.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* about 2 SECONDS on a 780!! */
name|addr
operator|->
name|vvocsr
operator|=
...
index|]
expr_stmt|;
comment|/* 	 * Check to see if the input side has wedged. 	 * 	 * We are lower than device ipl when we enter this routine, 	 * so if the interface is ready with an input packet then 	 * an input interrupt must have slipped through the cracks. 	 * 	 * Avoid the race with an input interrupt by watching to see 	 * if any packets come in. 	 */
name|s
operator|=
name|vs
operator|->
