...
name|vs_flush
decl_stmt|;
comment|/* number of flushed packets */
name|short
name|vs_nottaken
decl_stmt|;
...
name|VV_LPB
expr_stmt|;
comment|/* map xmit message into uba if not already there */
if|if
condition|(
operator|!
name|retrying
...
begin_comment
comment|/*  * Start or restart output on interface.  * If interface is not already active, get another datagram  * to send off of the interface queue, and map it to the interface  * before starting the output.  */
end_comment
...
operator|)
expr_stmt|;
comment|/* 	 * Demultiplex on packet type and deal with oddities of 	 * trailer protocol format 	 */
switch|switch
condition|(
name|vv
operator|->
...
name|setup
goto|;
comment|/* 	 * Pull packet off interface.  Off is nonzero if packet 	 * has trailing header; if_rubaget will then force this header 	 * information to be at the front, but we still have to drop 	 * the two-byte type which is at the front of any trailer data. 	 */
name|m
operator|=
name|if_rubaget
argument_list|(
...
begin_comment
comment|/*  * Logical host interface driver.  * Allows host to appear as an ARPAnet  * logical host.  Must also have routing  * table entry set up to forward packets  * to appropriate geteway on localnet.  */
end_comment
...
begin_comment
comment|/*  * Called by localnet interface to allow logical  * host interface to "attach".    */
end_comment
