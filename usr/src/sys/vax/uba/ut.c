...
condition|)
block|{
comment|/* 			 * On write error retries erase the 			 * inter-record gap 			 */
if|if
condition|(
name|um
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * Check for stray attentions from slaves going online, offline, 	 * or a completing rewind.  (The rewind started interrupt 	 * satisfied the requestor of the rewind.) 	 */
if|if
condition|(
operator|(
operator|(
...
literal|0xff
expr_stmt|;
comment|/* writing a 1 clears attention */
comment|/* 		 * If we're doing a rewind and we're at the beginning 		 * of tape, then the attention and the rewind  		 * command may complete at the same time -- resulting in only 		 * one interrupt.  In this case, simulate things to look like 		 * the attention was the command complete. 		 */
if|if
condition|(
name|bp
operator|->
...
directive|ifdef
name|notdef
comment|/* 		 * if this bit were emulated, it would allow us to wait 		 * for the transport to settle 		 */
while|while
condition|(
name|addr
operator|->
...
directive|endif
comment|/* 		 * If we hit the end of tape, update our position 		 */
if|if
condition|(
name|addr
operator|->
...
condition|)
block|{
comment|/* 			 * Set blkno and nxrec on sensing end of tape. 			 */
if|if
condition|(
name|bp
operator|==
...
name|UT_GO
expr_stmt|;
comment|/* 			 * Stuff fc so that it will be unstuffed correctly 			 * later to get the residual. 			 */
name|addr
operator|->
name|utfc
operator|=
...
name|printf
argument_list|(
literal|"ut%d: hard error bn%d er=%b cs2=%b\n"
argument_list|,
name|tjunit
argument_list|,
name|bp
...
name|ui_physaddr
expr_stmt|;
comment|/* do it at 1600 bpi so tape can be read on other machines */
name|utaddr
operator|->
name|uttc
operator|=
