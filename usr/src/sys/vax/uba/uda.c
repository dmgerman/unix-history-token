...
name|copenpart
decl_stmt|;
comment|/* characters partitions open */
block|}
name|ra_info
index|[
name|NRA
...
expr_stmt|;
block|}
comment|/* 	 * Try to read pack label. 	 */
if|if
condition|(
name|rainit
argument_list|(
...
begin_comment
comment|/*  * Initialize a drive:  * bring on line and read in pack label.  */
end_comment
...
name|RDLABEL
expr_stmt|;
comment|/* 	 * Set up default sizes until we've read the label, 	 * or longer if there isn't one there. 	 * Set secpercyl, as readdisklabel wants to compute b_cylin 	 * (although we don't need it). 	 */
name|lp
operator|->
name|d_secpercyl
operator|=
...
name|printf
argument_list|(
literal|"Don't have a partition table for an ra%d\n"
argument_list|,
name|type
argument_list|)
expr_stmt|;
