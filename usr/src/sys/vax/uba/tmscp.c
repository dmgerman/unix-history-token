...
name|tmscpaddr
decl_stmt|;
comment|/* ptr to tmscpdevice struct (IP& SA) * / 	struct uba_ctlr *um;	/* UNUSED ptr to uba_ctlr (controller) struct * / 	int count;		/* for probe delay time out */
ifdef|#
directive|ifdef
name|lint
name|br
...
argument_list|,
argument|tms_info[ui->ui_unit].tms_status& M _ST_MASK
argument_list|)
empty_stmt|;
endif|#
directive|endif
...
name|ui_mi
decl_stmt|;
comment|/* ptr to controller struct * / 	struct tmscpdevice *tmscpaddr = (struct tmscpdevice *) um->um_addr; /*  IP& SA */
name|struct
name|mscp
modifier|*
name|mp
...
name|t
decl_stmt|;
comment|/* communications area; cmd& resp packets * / 	struct tmscpdevice *tmscpaddr; 	struct uba_ctlr *um;   	sc =&tmscp_softc[d]; 	um = tmscpminfo[d]; 	um->um_tab.b_active++; 	t =&tmscp[d]; 	tmscpaddr = (struct tmscpdevice *)um->um_addr; 	if (sc->sc_mapped == 0) 		{ 		/* 		 * Map the communications area and command 		 * and response packets into Unibus address 		 * space. 		 */
name|sc
operator|->
name|sc_ubainfo
operator|=
...
comment|/* frees a bdp tmscpstart will be called * / 			if ((mp = tmscpgetcp(um)) == NULL) 				break; #			ifdef DEBUG 			printd("tmscpstart: GTUNT %d ubasetup = %d\n",ui->ui_un it, i); 			if(tmscpdebug)DELAY(10000); #			endif 			mp->mscp_opcode = M_OP_GTUNT; 			mp->mscp_unit = ui->ui_slave; 			*((long *)mp->mscp_dscptr) |= TMSCP_OWN|TMSCP_INT; 			if (tmscpaddr->tmscpsa&TMSCP_ERR) 				printf("tmscp%d: fatal error (0%o)\n",um->um_ct lr, 					    tmscpaddr->tmscpsa&0xffff); 			i = tmscpaddr->tmscpip;	/* initiate polling */
break|break;
block|}
if|#
directive|if
...
literal|0
expr_stmt|;
comment|/* default device dependant par ameters */
if|if
condition|(
operator|(
name|tms_info
...
name|M_MD_CLSEX
expr_stmt|;
comment|/*  clear serious exc * / 		tms_info[ui->ui_unit].tms_serex = 0; 		} 	*((long *)mp->mscp_dscptr) |= TMSCP_OWN|TMSCP_INT; #	ifdef DEBUG 	printd("tmscpstart: opcode 0%o mod %o unit %d cnt %d\n",mp->mscp_opcode ,mp->mscp_modifier,mp->mscp_unit,mp->mscp_bytecnt); 	if(tmscpdebug)DELAY(100000); #	endif 	i = tmscpaddr->tmscpip;              /* initiate polling */
name|dp
operator|->
name|b_qsize
operator|++
...
literal|0xf
expr_stmt|;
comment|/* low 4 bits * / 	if ((mp->mscp_header.tmscp_credits& 0xf0)> 0x10)	/* Check */
return|return;
ifdef|#
directive|ifdef
name|DEBUG
...
argument_list|,
argument|mp->mscp_opcode
argument_list|,
argument|mp->mscp_sta tus&M_ST_MASK
argument_list|)
empty_stmt|;
endif|#
directive|endif
...
argument_list|,
argument|mp->mscp_header.tmscp_c redits&
literal|0xf
argument_list|)
empty_stmt|;
name|printd
...
name|M_OP_END
case|:
comment|/* 	 * The AVAILABLE message occurs when the mt ioctl "rewoffl" is 	 * issued.  For the ioctl, "rewoffl", a tmscp AVAILABLE command is 	 * done with the UNLOAD modifier.  This performs a rewind, followed 	 * by marking the unit offline.  So mark the unit offline 	 * software wise as well (ui_flags = 0 and  	 * tms_info[ui->ui_unit].tms_openf = 0). 	 */
case|case
name|M_OP_AVAIL
operator||
name|M_OP_END
...
argument_list|)
expr_stmt|;
comment|/* produces more info * / #				ifdef DEBUG 				printd("tmscprsp: error; status sub-code = 0%o,  flags = 0%o\n", 					(mp->mscp_status& 177740)>>5, mp->mscp _flags); #				endif 				bp->b_flags |= B_ERROR; 				} 			else 				/* Hit a tape mark - Set serex flag to 				 * a special value so we can clear the 				 * serious exception on the next command. 				 */
name|tms_info
index|[
name|ui
operator|->
...
argument|op
argument_list|,
argument|tmscpp->tmscp_Rsp.mscp_opcode
argument_list|,
argument|tmscpp->tmscp_Rsp.mscp_s tatus
argument_list|)
empty_stmt|;
return|return
operator|(
...
name|physio
argument_list|(
argument|tmscpstrategy
argument_list|,
argument|&rtmsbuf[unit]
argument_list|,
argument|dev
argument_list|,
argument|B_WRITE
