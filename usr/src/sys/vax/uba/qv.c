...
name|sccsid
init|=
literal|"@(#)qv.c	1.8 (ULTRIX) 8/21/85"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * qv_def_scrn is used to select the appropriate tables. 0=15 inch 1=19 inch,  * 2 = new display (untested).  */
end_comment
...
name|tvec
expr_stmt|;
comment|/* can't do this until we know we have memory to write in */
name|qv_scn
operator|->
name|qvaddr
operator|=
...
argument_list|)
expr_stmt|;
comment|/*          * Map the qvss memory for use by users.          */
name|qvssmem
operator|=
operator|(
name|char
...
operator|)
return|;
comment|/*          * Found an entry for this cpu. Because this device is Microvax specific          * we assume that there is a single q-bus and don't have to worry about          * multiple adapters.          *          * Map the bus memory and device registers.          */
name|ubaaccess
argument_list|(
name|pcpu
operator|->
