...
begin_comment
comment|/*  * TU58 DECtape II/DL11 device driver  * (based on a driver written by Mike Obrien @ RAND)  *  * The TU58 * is treated as a block device (only).  Error detection and  * recovery is almost non-existant.  It is assumed that the  * TU58 will follow the RSP protocol exactly, very few protocol  * errors are checked for.    */
end_comment
