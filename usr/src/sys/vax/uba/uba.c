begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	uba.c	4.39	81/12/09	*/
end_comment

begin_include
include|#
directive|include
file|"../h/param.h"
end_include

begin_include
include|#
directive|include
file|"../h/systm.h"
end_include

begin_include
include|#
directive|include
file|"../h/cpu.h"
end_include

begin_include
include|#
directive|include
file|"../h/map.h"
end_include

begin_include
include|#
directive|include
file|"../h/pte.h"
end_include

begin_include
include|#
directive|include
file|"../h/buf.h"
end_include

begin_include
include|#
directive|include
file|"../h/vm.h"
end_include

begin_include
include|#
directive|include
file|"../h/ubareg.h"
end_include

begin_include
include|#
directive|include
file|"../h/ubavar.h"
end_include

begin_include
include|#
directive|include
file|"../h/dir.h"
end_include

begin_include
include|#
directive|include
file|"../h/user.h"
end_include

begin_include
include|#
directive|include
file|"../h/proc.h"
end_include

begin_include
include|#
directive|include
file|"../h/conf.h"
end_include

begin_include
include|#
directive|include
file|"../h/mtpr.h"
end_include

begin_include
include|#
directive|include
file|"../h/nexus.h"
end_include

begin_include
include|#
directive|include
file|"../h/dk.h"
end_include

begin_if
if|#
directive|if
name|VAX780
end_if

begin_decl_stmt
name|char
name|ubasr_bits
index|[]
init|=
name|UBASR_BITS
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Do transfer on device argument.  The controller  * and uba involved are implied by the device.  * We queue for resource wait in the uba code if necessary.  * We return 1 if the transfer was started, 0 if it was not.  * If you call this routine with the head of the queue for a  * UBA, it will automatically remove the device from the UBA  * queue before it returns.  If some other device is given  * as argument, it will be added to the request queue if the  * request cannot be started immediately.  This means that  * passing a device which is on the queue but not at the head  * of the request queue is likely to be a disaster.  */
end_comment

begin_expr_stmt
name|ubago
argument_list|(
name|ui
argument_list|)
specifier|register
expr|struct
name|uba_device
operator|*
name|ui
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|uba_ctlr
modifier|*
name|um
init|=
name|ui
operator|->
name|ui_mi
decl_stmt|;
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
decl_stmt|;
specifier|register
name|int
name|s
decl_stmt|,
name|unit
decl_stmt|;
name|uh
operator|=
operator|&
name|uba_hd
index|[
name|um
operator|->
name|um_ubanum
index|]
expr_stmt|;
name|s
operator|=
name|spl6
argument_list|()
expr_stmt|;
if|if
condition|(
name|um
operator|->
name|um_driver
operator|->
name|ud_xclu
operator|&&
name|uh
operator|->
name|uh_users
operator|>
literal|0
operator|||
name|uh
operator|->
name|uh_xclu
condition|)
goto|goto
name|rwait
goto|;
name|um
operator|->
name|um_ubinfo
operator|=
name|ubasetup
argument_list|(
name|um
operator|->
name|um_ubanum
argument_list|,
name|um
operator|->
name|um_tab
operator|.
name|b_actf
operator|->
name|b_actf
argument_list|,
name|UBA_NEEDBDP
operator||
name|UBA_CANTWAIT
argument_list|)
expr_stmt|;
if|if
condition|(
name|um
operator|->
name|um_ubinfo
operator|==
literal|0
condition|)
goto|goto
name|rwait
goto|;
name|uh
operator|->
name|uh_users
operator|++
expr_stmt|;
if|if
condition|(
name|um
operator|->
name|um_driver
operator|->
name|ud_xclu
condition|)
name|uh
operator|->
name|uh_xclu
operator|=
literal|1
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
name|ui
operator|->
name|ui_dk
operator|>=
literal|0
condition|)
block|{
name|unit
operator|=
name|ui
operator|->
name|ui_dk
expr_stmt|;
name|dk_busy
operator||=
literal|1
operator|<<
name|unit
expr_stmt|;
block|}
if|if
condition|(
name|uh
operator|->
name|uh_actf
operator|==
name|ui
condition|)
name|uh
operator|->
name|uh_actf
operator|=
name|ui
operator|->
name|ui_forw
expr_stmt|;
call|(
modifier|*
name|um
operator|->
name|um_driver
operator|->
name|ud_dgo
call|)
argument_list|(
name|um
argument_list|)
expr_stmt|;
if|if
condition|(
name|ui
operator|->
name|ui_dk
operator|>=
literal|0
condition|)
block|{
name|dk_xfer
index|[
name|unit
index|]
operator|++
expr_stmt|;
name|dk_wds
index|[
name|unit
index|]
operator|+=
name|um
operator|->
name|um_tab
operator|.
name|b_actf
operator|->
name|b_actf
operator|->
name|b_bcount
operator|>>
literal|6
expr_stmt|;
block|}
return|return
operator|(
literal|1
operator|)
return|;
name|rwait
label|:
if|if
condition|(
name|uh
operator|->
name|uh_actf
operator|!=
name|ui
condition|)
block|{
name|ui
operator|->
name|ui_forw
operator|=
name|NULL
expr_stmt|;
if|if
condition|(
name|uh
operator|->
name|uh_actf
operator|==
name|NULL
condition|)
name|uh
operator|->
name|uh_actf
operator|=
name|ui
expr_stmt|;
else|else
name|uh
operator|->
name|uh_actl
operator|->
name|ui_forw
operator|=
name|ui
expr_stmt|;
name|uh
operator|->
name|uh_actl
operator|=
name|ui
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_expr_stmt
name|ubadone
argument_list|(
name|um
argument_list|)
specifier|register
expr|struct
name|uba_ctlr
operator|*
name|um
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
init|=
operator|&
name|uba_hd
index|[
name|um
operator|->
name|um_ubanum
index|]
decl_stmt|;
if|if
condition|(
name|um
operator|->
name|um_driver
operator|->
name|ud_xclu
condition|)
name|uh
operator|->
name|uh_xclu
operator|=
literal|0
expr_stmt|;
name|uh
operator|->
name|uh_users
operator|--
expr_stmt|;
name|ubarelse
argument_list|(
name|um
operator|->
name|um_ubanum
argument_list|,
operator|&
name|um
operator|->
name|um_ubinfo
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Allocate and setup UBA map registers, and bdp's  * Flags says whether bdp is needed, whether the caller can't  * wait (e.g. if the caller is at interrupt level).  *  * Return value:  *	Bits 0-8	Byte offset  *	Bits 9-17	Start map reg. no.  *	Bits 18-27	No. mapping reg's  *	Bits 28-31	BDP no.  */
end_comment

begin_macro
name|ubasetup
argument_list|(
argument|uban
argument_list|,
argument|bp
argument_list|,
argument|flags
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
init|=
operator|&
name|uba_hd
index|[
name|uban
index|]
decl_stmt|;
specifier|register
name|int
name|temp
decl_stmt|,
name|i
decl_stmt|;
name|int
name|npf
decl_stmt|,
name|reg
decl_stmt|,
name|bdp
decl_stmt|;
name|unsigned
name|v
decl_stmt|;
specifier|register
name|struct
name|pte
modifier|*
name|pte
decl_stmt|,
modifier|*
name|io
decl_stmt|;
name|struct
name|proc
modifier|*
name|rp
decl_stmt|;
name|int
name|a
decl_stmt|,
name|o
decl_stmt|,
name|ubinfo
decl_stmt|;
if|#
directive|if
name|VAX7ZZ
if|if
condition|(
name|cpu
operator|==
name|VAX_7ZZ
condition|)
name|flags
operator|&=
operator|~
name|UBA_NEEDBDP
expr_stmt|;
endif|#
directive|endif
name|v
operator|=
name|btop
argument_list|(
name|bp
operator|->
name|b_un
operator|.
name|b_addr
argument_list|)
expr_stmt|;
name|o
operator|=
operator|(
name|int
operator|)
name|bp
operator|->
name|b_un
operator|.
name|b_addr
operator|&
name|PGOFSET
expr_stmt|;
name|npf
operator|=
name|btoc
argument_list|(
name|bp
operator|->
name|b_bcount
operator|+
name|o
argument_list|)
operator|+
literal|1
expr_stmt|;
name|a
operator|=
name|spl6
argument_list|()
expr_stmt|;
while|while
condition|(
operator|(
name|reg
operator|=
name|rmalloc
argument_list|(
name|uh
operator|->
name|uh_map
argument_list|,
name|npf
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|flags
operator|&
name|UBA_CANTWAIT
condition|)
block|{
name|splx
argument_list|(
name|a
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|uh
operator|->
name|uh_mrwant
operator|++
expr_stmt|;
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
name|uh
operator|->
name|uh_map
argument_list|,
name|PSWP
argument_list|)
expr_stmt|;
block|}
name|bdp
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|flags
operator|&
name|UBA_NEEDBDP
condition|)
block|{
while|while
condition|(
operator|(
name|bdp
operator|=
name|ffs
argument_list|(
name|uh
operator|->
name|uh_bdpfree
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|flags
operator|&
name|UBA_CANTWAIT
condition|)
block|{
name|rmfree
argument_list|(
name|uh
operator|->
name|uh_map
argument_list|,
name|npf
argument_list|,
name|reg
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|a
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|uh
operator|->
name|uh_bdpwant
operator|++
expr_stmt|;
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
name|uh
operator|->
name|uh_map
argument_list|,
name|PSWP
argument_list|)
expr_stmt|;
block|}
name|uh
operator|->
name|uh_bdpfree
operator|&=
operator|~
operator|(
literal|1
operator|<<
operator|(
name|bdp
operator|-
literal|1
operator|)
operator|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|flags
operator|&
name|UBA_HAVEBDP
condition|)
name|bdp
operator|=
operator|(
name|flags
operator|>>
literal|28
operator|)
operator|&
literal|0xf
expr_stmt|;
name|splx
argument_list|(
name|a
argument_list|)
expr_stmt|;
name|reg
operator|--
expr_stmt|;
name|ubinfo
operator|=
operator|(
name|bdp
operator|<<
literal|28
operator|)
operator||
operator|(
name|npf
operator|<<
literal|18
operator|)
operator||
operator|(
name|reg
operator|<<
literal|9
operator|)
operator||
name|o
expr_stmt|;
name|io
operator|=
operator|&
name|uh
operator|->
name|uh_uba
operator|->
name|uba_map
index|[
name|reg
index|]
expr_stmt|;
name|temp
operator|=
operator|(
name|bdp
operator|<<
literal|21
operator|)
operator||
name|UBAMR_MRV
expr_stmt|;
name|rp
operator|=
name|bp
operator|->
name|b_flags
operator|&
name|B_DIRTY
condition|?
operator|&
name|proc
index|[
literal|2
index|]
else|:
name|bp
operator|->
name|b_proc
expr_stmt|;
if|if
condition|(
name|bdp
operator|&&
operator|(
name|o
operator|&
literal|01
operator|)
condition|)
name|temp
operator||=
name|UBAMR_BO
expr_stmt|;
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
name|B_UAREA
condition|)
block|{
for|for
control|(
name|i
operator|=
name|UPAGES
operator|-
name|bp
operator|->
name|b_bcount
operator|/
name|NBPG
init|;
name|i
operator|<
name|UPAGES
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|rp
operator|->
name|p_addr
index|[
name|i
index|]
operator|.
name|pg_pfnum
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"uba: zero upage"
argument_list|)
expr_stmt|;
operator|*
operator|(
name|int
operator|*
operator|)
name|io
operator|++
operator|=
name|rp
operator|->
name|p_addr
index|[
name|i
index|]
operator|.
name|pg_pfnum
operator||
name|temp
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|(
name|bp
operator|->
name|b_flags
operator|&
name|B_PHYS
operator|)
operator|==
literal|0
condition|)
block|{
name|pte
operator|=
operator|&
name|Sysmap
index|[
name|btop
argument_list|(
operator|(
operator|(
name|int
operator|)
name|bp
operator|->
name|b_un
operator|.
name|b_addr
operator|)
operator|&
literal|0x7fffffff
argument_list|)
index|]
expr_stmt|;
while|while
condition|(
operator|--
name|npf
operator|!=
literal|0
condition|)
operator|*
operator|(
name|int
operator|*
operator|)
name|io
operator|++
operator|=
name|pte
operator|++
operator|->
name|pg_pfnum
operator||
name|temp
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
name|B_PAGET
condition|)
name|pte
operator|=
operator|&
name|Usrptmap
index|[
name|btokmx
argument_list|(
operator|(
expr|struct
name|pte
operator|*
block|)
name|bp
operator|->
name|b_un
operator|.
name|b_addr
block|)
end_block

begin_empty_stmt
unit|]
empty_stmt|;
end_empty_stmt

begin_else
else|else
name|pte
operator|=
name|vtopte
argument_list|(
name|rp
argument_list|,
name|v
argument_list|)
expr_stmt|;
end_else

begin_while
while|while
condition|(
operator|--
name|npf
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|pte
operator|->
name|pg_pfnum
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"uba zero uentry"
argument_list|)
expr_stmt|;
operator|*
operator|(
name|int
operator|*
operator|)
name|io
operator|++
operator|=
name|pte
operator|++
operator|->
name|pg_pfnum
operator||
name|temp
expr_stmt|;
block|}
end_while

begin_expr_stmt
unit|} 	*
operator|(
name|int
operator|*
operator|)
name|io
operator|++
operator|=
literal|0
expr_stmt|;
end_expr_stmt

begin_return
return|return
operator|(
name|ubinfo
operator|)
return|;
end_return

begin_comment
unit|}
comment|/*  * Non buffer setup interface... set up a buffer and call ubasetup.  */
end_comment

begin_expr_stmt
unit|uballoc
operator|(
name|uban
operator|,
name|addr
operator|,
name|bcnt
operator|,
name|flags
operator|)
name|int
name|uban
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|caddr_t
name|addr
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|bcnt
decl_stmt|,
name|flags
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|buf
name|ubabuf
decl_stmt|;
name|ubabuf
operator|.
name|b_un
operator|.
name|b_addr
operator|=
name|addr
expr_stmt|;
name|ubabuf
operator|.
name|b_flags
operator|=
name|B_BUSY
expr_stmt|;
name|ubabuf
operator|.
name|b_bcount
operator|=
name|bcnt
expr_stmt|;
comment|/* that's all the fields ubasetup() needs */
return|return
operator|(
name|ubasetup
argument_list|(
name|uban
argument_list|,
operator|&
name|ubabuf
argument_list|,
name|flags
argument_list|)
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Release resources on uba uban, and then unblock resource waiters.  * The map register parameter is by value since we need to block  * against uba resets on 11/780's.  */
end_comment

begin_macro
name|ubarelse
argument_list|(
argument|uban
argument_list|,
argument|amr
argument_list|)
end_macro

begin_decl_stmt
name|int
modifier|*
name|amr
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
init|=
operator|&
name|uba_hd
index|[
name|uban
index|]
decl_stmt|;
specifier|register
name|int
name|bdp
decl_stmt|,
name|reg
decl_stmt|,
name|npf
decl_stmt|,
name|s
decl_stmt|;
name|int
name|mr
decl_stmt|;
comment|/* 	 * Carefully see if we should release the space, since 	 * it may be released asynchronously at uba reset time. 	 */
name|s
operator|=
name|spl6
argument_list|()
expr_stmt|;
name|mr
operator|=
operator|*
name|amr
expr_stmt|;
if|if
condition|(
name|mr
operator|==
literal|0
condition|)
block|{
comment|/* 		 * A ubareset() occurred before we got around 		 * to releasing the space... no need to bother. 		 */
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return;
block|}
operator|*
name|amr
operator|=
literal|0
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
comment|/* let interrupts in, we're safe for a while */
name|bdp
operator|=
operator|(
name|mr
operator|>>
literal|28
operator|)
operator|&
literal|0x0f
expr_stmt|;
if|if
condition|(
name|bdp
condition|)
block|{
switch|switch
condition|(
name|cpu
condition|)
block|{
if|#
directive|if
name|VAX780
case|case
name|VAX_780
case|:
name|uh
operator|->
name|uh_uba
operator|->
name|uba_dpr
index|[
name|bdp
index|]
operator||=
name|UBADPR_BNE
expr_stmt|;
break|break;
endif|#
directive|endif
if|#
directive|if
name|VAX750
case|case
name|VAX_750
case|:
name|uh
operator|->
name|uh_uba
operator|->
name|uba_dpr
index|[
name|bdp
index|]
operator||=
name|UBADPR_PURGE
operator||
name|UBADPR_NXM
operator||
name|UBADPR_UCE
expr_stmt|;
break|break;
endif|#
directive|endif
block|}
name|uh
operator|->
name|uh_bdpfree
operator||=
literal|1
operator|<<
operator|(
name|bdp
operator|-
literal|1
operator|)
expr_stmt|;
comment|/* atomic */
if|if
condition|(
name|uh
operator|->
name|uh_bdpwant
condition|)
block|{
name|uh
operator|->
name|uh_bdpwant
operator|=
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
name|uh
operator|->
name|uh_map
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* 	 * Put back the registers in the resource map. 	 * The map code must not be reentered, so we do this 	 * at high ipl. 	 */
name|npf
operator|=
operator|(
name|mr
operator|>>
literal|18
operator|)
operator|&
literal|0x3ff
expr_stmt|;
name|reg
operator|=
operator|(
operator|(
name|mr
operator|>>
literal|9
operator|)
operator|&
literal|0x1ff
operator|)
operator|+
literal|1
expr_stmt|;
name|s
operator|=
name|spl6
argument_list|()
expr_stmt|;
name|rmfree
argument_list|(
name|uh
operator|->
name|uh_map
argument_list|,
name|npf
argument_list|,
name|reg
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
comment|/* 	 * Wakeup sleepers for map registers, 	 * and also, if there are processes blocked in dgo(), 	 * give them a chance at the UNIBUS. 	 */
if|if
condition|(
name|uh
operator|->
name|uh_mrwant
condition|)
block|{
name|uh
operator|->
name|uh_mrwant
operator|=
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
name|uh
operator|->
name|uh_map
argument_list|)
expr_stmt|;
block|}
while|while
condition|(
name|uh
operator|->
name|uh_actf
operator|&&
name|ubago
argument_list|(
name|uh
operator|->
name|uh_actf
argument_list|)
condition|)
empty_stmt|;
block|}
end_block

begin_expr_stmt
name|ubapurge
argument_list|(
name|um
argument_list|)
specifier|register
expr|struct
name|uba_ctlr
operator|*
name|um
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
init|=
name|um
operator|->
name|um_hd
decl_stmt|;
specifier|register
name|int
name|bdp
init|=
operator|(
name|um
operator|->
name|um_ubinfo
operator|>>
literal|28
operator|)
operator|&
literal|0x0f
decl_stmt|;
switch|switch
condition|(
name|cpu
condition|)
block|{
if|#
directive|if
name|VAX780
case|case
name|VAX_780
case|:
name|uh
operator|->
name|uh_uba
operator|->
name|uba_dpr
index|[
name|bdp
index|]
operator||=
name|UBADPR_BNE
expr_stmt|;
break|break;
endif|#
directive|endif
if|#
directive|if
name|VAX750
case|case
name|VAX_750
case|:
name|uh
operator|->
name|uh_uba
operator|->
name|uba_dpr
index|[
name|bdp
index|]
operator||=
name|UBADPR_PURGE
operator||
name|UBADPR_NXM
operator||
name|UBADPR_UCE
expr_stmt|;
break|break;
endif|#
directive|endif
block|}
block|}
end_block

begin_comment
comment|/*  * Generate a reset on uba number uban.  Then  * call each device in the character device table,  * giving it a chance to clean up so as to be able to continue.  */
end_comment

begin_macro
name|ubareset
argument_list|(
argument|uban
argument_list|)
end_macro

begin_decl_stmt
name|int
name|uban
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|cdevsw
modifier|*
name|cdp
decl_stmt|;
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
init|=
operator|&
name|uba_hd
index|[
name|uban
index|]
decl_stmt|;
name|int
name|s
decl_stmt|;
name|s
operator|=
name|spl6
argument_list|()
expr_stmt|;
name|uh
operator|->
name|uh_users
operator|=
literal|0
expr_stmt|;
name|uh
operator|->
name|uh_zvcnt
operator|=
literal|0
expr_stmt|;
name|uh
operator|->
name|uh_xclu
operator|=
literal|0
expr_stmt|;
name|uh
operator|->
name|uh_hangcnt
operator|=
literal|0
expr_stmt|;
name|uh
operator|->
name|uh_actf
operator|=
name|uh
operator|->
name|uh_actl
operator|=
literal|0
expr_stmt|;
name|uh
operator|->
name|uh_bdpwant
operator|=
literal|0
expr_stmt|;
name|uh
operator|->
name|uh_mrwant
operator|=
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|uh
operator|->
name|uh_bdpwant
argument_list|)
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|uh
operator|->
name|uh_mrwant
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"uba%d: reset"
argument_list|,
name|uban
argument_list|)
expr_stmt|;
name|ubainit
argument_list|(
name|uh
operator|->
name|uh_uba
argument_list|)
expr_stmt|;
for|for
control|(
name|cdp
operator|=
name|cdevsw
init|;
name|cdp
operator|->
name|d_open
condition|;
name|cdp
operator|++
control|)
call|(
modifier|*
name|cdp
operator|->
name|d_reset
call|)
argument_list|(
name|uban
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|INET
name|ifubareset
argument_list|(
name|uban
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Init a uba.  This is called with a pointer  * rather than a virtual address since it is called  * by code which runs with memory mapping disabled.  * In these cases we really don't need the interrupts  * enabled, but since we run with ipl high, we don't care  * if they are, they will never happen anyways.  */
end_comment

begin_expr_stmt
name|ubainit
argument_list|(
name|uba
argument_list|)
specifier|register
expr|struct
name|uba_regs
operator|*
name|uba
expr_stmt|;
end_expr_stmt

begin_block
block|{
switch|switch
condition|(
name|cpu
condition|)
block|{
if|#
directive|if
name|VAX780
case|case
name|VAX_780
case|:
name|uba
operator|->
name|uba_cr
operator|=
name|UBACR_ADINIT
expr_stmt|;
name|uba
operator|->
name|uba_cr
operator|=
name|UBACR_IFS
operator||
name|UBACR_BRIE
operator||
name|UBACR_USEFIE
operator||
name|UBACR_SUEFIE
expr_stmt|;
while|while
condition|(
operator|(
name|uba
operator|->
name|uba_cnfgr
operator|&
name|UBACNFGR_UBIC
operator|)
operator|==
literal|0
condition|)
empty_stmt|;
break|break;
endif|#
directive|endif
if|#
directive|if
name|VAX750
case|case
name|VAX_750
case|:
endif|#
directive|endif
if|#
directive|if
name|VAX7ZZ
case|case
name|VAX_7ZZ
case|:
endif|#
directive|endif
if|#
directive|if
name|defined
argument_list|(
name|VAX750
argument_list|)
operator|||
name|defined
argument_list|(
name|VAX7ZZ
argument_list|)
name|mtpr
argument_list|(
name|IUR
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* give devices time to recover from power fail */
comment|/* THIS IS PROBABLY UNNECESSARY */
name|DELAY
argument_list|(
literal|500000
argument_list|)
expr_stmt|;
comment|/* END PROBABLY UNNECESSARY */
break|break;
endif|#
directive|endif
block|}
block|}
end_block

begin_if
if|#
directive|if
name|VAX780
end_if

begin_comment
comment|/*  * Check to make sure the UNIBUS adaptor is not hung,  * with an interrupt in the register to be presented,  * but not presenting it for an extended period (5 seconds).  */
end_comment

begin_macro
name|unhang
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|uban
decl_stmt|;
for|for
control|(
name|uban
operator|=
literal|0
init|;
name|uban
operator|<
name|numuba
condition|;
name|uban
operator|++
control|)
block|{
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
init|=
operator|&
name|uba_hd
index|[
name|uban
index|]
decl_stmt|;
specifier|register
name|struct
name|uba_regs
modifier|*
name|up
init|=
name|uh
operator|->
name|uh_uba
decl_stmt|;
if|if
condition|(
name|up
operator|->
name|uba_sr
operator|==
literal|0
condition|)
return|return;
name|up
operator|->
name|uba_sr
operator|=
name|UBASR_CRD
operator||
name|UBASR_LEB
expr_stmt|;
name|uh
operator|->
name|uh_hangcnt
operator|++
expr_stmt|;
if|if
condition|(
name|uh
operator|->
name|uh_hangcnt
operator|>
literal|5
operator|*
name|hz
condition|)
block|{
name|uh
operator|->
name|uh_hangcnt
operator|=
literal|0
expr_stmt|;
name|printf
argument_list|(
literal|"uba%d: hung\n"
argument_list|,
name|uban
argument_list|)
expr_stmt|;
name|ubareset
argument_list|(
name|uban
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_block

begin_comment
comment|/*  * This is a timeout routine which decrements the ``i forgot to  * interrupt'' counts, on an 11/780.  This prevents slowly growing  * counts from causing a UBA reset since we are interested only  * in hang situations.  */
end_comment

begin_macro
name|ubawatch
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
decl_stmt|;
specifier|register
name|int
name|uban
decl_stmt|;
if|if
condition|(
name|panicstr
condition|)
return|return;
for|for
control|(
name|uban
operator|=
literal|0
init|;
name|uban
operator|<
name|numuba
condition|;
name|uban
operator|++
control|)
block|{
name|uh
operator|=
operator|&
name|uba_hd
index|[
name|uban
index|]
expr_stmt|;
if|if
condition|(
name|uh
operator|->
name|uh_hangcnt
condition|)
name|uh
operator|->
name|uh_hangcnt
operator|--
expr_stmt|;
block|}
block|}
end_block

begin_decl_stmt
name|int
name|ubawedgecnt
init|=
literal|10
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ubacrazy
init|=
literal|500
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * This routine is called by the locore code to  * process a UBA error on an 11/780.  The arguments are passed  * on the stack, and value-result (through some trickery).  * In particular, the uvec argument is used for further  * uba processing so the result aspect of it is very important.  * It must not be declared register.  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_expr_stmt
name|ubaerror
argument_list|(
name|uban
argument_list|,
name|uh
argument_list|,
name|xx
argument_list|,
name|uvec
argument_list|,
name|uba
argument_list|)
specifier|register
name|int
name|uban
expr_stmt|;
end_expr_stmt

begin_decl_stmt
specifier|register
name|struct
name|uba_hd
modifier|*
name|uh
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|uvec
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|struct
name|uba_regs
modifier|*
name|uba
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|sr
operator|,
name|s
expr_stmt|;
if|if
condition|(
name|uvec
operator|==
literal|0
condition|)
block|{
name|uh
operator|->
name|uh_zvcnt
operator|++
expr_stmt|;
if|if
condition|(
name|uh
operator|->
name|uh_zvcnt
operator|>
literal|250000
condition|)
block|{
name|printf
argument_list|(
literal|"uba%d: too many zero vectors\n"
argument_list|)
expr_stmt|;
name|ubareset
argument_list|(
name|uban
argument_list|)
expr_stmt|;
block|}
name|uvec
operator|=
literal|0
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|uba
operator|->
name|uba_cnfgr
operator|&
name|NEX_CFGFLT
condition|)
block|{
name|printf
argument_list|(
literal|"uba%d: sbi fault sr=%b cnfgr=%b\n"
argument_list|,
name|uban
argument_list|,
name|uba
operator|->
name|uba_sr
argument_list|,
name|ubasr_bits
argument_list|,
name|uba
operator|->
name|uba_cnfgr
argument_list|,
name|NEXFLT_BITS
argument_list|)
expr_stmt|;
name|ubareset
argument_list|(
name|uban
argument_list|)
expr_stmt|;
name|uvec
operator|=
literal|0
expr_stmt|;
return|return;
block|}
name|sr
operator|=
name|uba
operator|->
name|uba_sr
expr_stmt|;
name|s
operator|=
name|spl7
argument_list|()
expr_stmt|;
name|printf
argument_list|(
literal|"uba%d: uba error sr=%b fmer=%x fubar=%o\n"
argument_list|,
name|uban
argument_list|,
name|uba
operator|->
name|uba_sr
argument_list|,
name|ubasr_bits
argument_list|,
name|uba
operator|->
name|uba_fmer
argument_list|,
literal|4
operator|*
name|uba
operator|->
name|uba_fubar
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|uba
operator|->
name|uba_sr
operator|=
name|sr
expr_stmt|;
name|uvec
operator|&=
name|UBABRRVR_DIV
expr_stmt|;
if|if
condition|(
operator|++
name|uh
operator|->
name|uh_errcnt
operator|%
name|ubawedgecnt
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|uh
operator|->
name|uh_errcnt
operator|>
name|ubacrazy
condition|)
name|panic
argument_list|(
literal|"uba crazy"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"ERROR LIMIT "
argument_list|)
expr_stmt|;
name|ubareset
argument_list|(
name|uban
argument_list|)
expr_stmt|;
name|uvec
operator|=
literal|0
expr_stmt|;
return|return;
block|}
return|return;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
literal|0
end_if

begin_comment
comment|/*  * This routine allows remapping of previously  * allocated UNIBUS bdp and map resources  * onto different memory addresses.  * It should only be used by routines which need  * small fixed length mappings for long periods of time  * (like the ARPANET ACC IMP interface).  * It only maps kernel addresses.  */
end_comment

begin_comment
unit|ubaremap(uban, ubinfo, addr) 	int uban; 	register unsigned ubinfo; 	caddr_t addr; { 	register struct uba_hd *uh =&uba_hd[uban]; 	register struct pte *pte, *io; 	register int temp, bdp; 	int npf, o;  	o = (int)addr& PGOFSET; 	bdp = (ubinfo>> 28)& 0xf; 	npf = (ubinfo>> 18)& 0x3ff; 	io =&uh->uh_uba->uba_map[(ubinfo>> 9)& 0x1ff]; 	temp = (bdp<< 21) | UBAMR_MRV;
comment|/* 	 * If using buffered data path initiate purge 	 * of old data and set byte offset bit if next 	 * transfer will be from odd address. 	 */
end_comment

begin_if
unit|if (bdp) { 		switch (cpu) {
if|#
directive|if
name|VAX780
end_if

begin_endif
unit|case VAX_780: 			uh->uh_uba->uba_dpr[bdp] |= UBADPR_BNE; 			break;
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|VAX750
end_if

begin_endif
unit|case VAX_750: 			uh->uh_uba->uba_dpr[bdp] |= 			    UBADPR_PURGE|UBADPR_NXM|UBADPR_UCE; 			break;
endif|#
directive|endif
end_endif

begin_comment
unit|} 		if (o& 1) 			temp |= UBAMR_BO; 	}
comment|/* 	 * Set up the map registers, leaving an invalid reg 	 * at the end to guard against wild unibus transfers. 	 */
end_comment

begin_comment
unit|pte =&Sysmap[btop(((int)addr)&0x7fffffff)]; 	while (--npf != 0) 		*(int *)io++ = pte++->pg_pfnum | temp; 	*(int *)io = 0;
comment|/* 	 * Return effective UNIBUS address. 	 */
end_comment

begin_endif
unit|return (ubinfo | o); }
endif|#
directive|endif
end_endif

end_unit

