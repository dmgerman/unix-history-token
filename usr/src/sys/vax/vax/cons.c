begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	cons.c	3.1	%H%	*/
end_comment

begin_comment
comment|/*  *   Vax console driver and floppy interface  */
end_comment

begin_include
include|#
directive|include
file|"../h/param.h"
end_include

begin_include
include|#
directive|include
file|"../h/conf.h"
end_include

begin_include
include|#
directive|include
file|"../h/dir.h"
end_include

begin_include
include|#
directive|include
file|"../h/user.h"
end_include

begin_include
include|#
directive|include
file|"../h/tty.h"
end_include

begin_include
include|#
directive|include
file|"../h/systm.h"
end_include

begin_include
include|#
directive|include
file|"../h/cons.h"
end_include

begin_include
include|#
directive|include
file|"../h/mtpr.h"
end_include

begin_define
define|#
directive|define
name|NL1
value|000400
end_define

begin_define
define|#
directive|define
name|NL2
value|001000
end_define

begin_define
define|#
directive|define
name|CR2
value|020000
end_define

begin_define
define|#
directive|define
name|FF1
value|040000
end_define

begin_define
define|#
directive|define
name|TAB1
value|002000
end_define

begin_decl_stmt
name|struct
name|tty
name|cons
decl_stmt|;
end_decl_stmt

begin_function_decl
name|int
name|cnstart
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|int
name|ttrstrt
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|char
name|partab
index|[]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|cnopen
argument_list|(
argument|dev
argument_list|,
argument|flag
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|tty
modifier|*
name|tp
decl_stmt|;
name|tp
operator|=
operator|&
name|cons
expr_stmt|;
name|tp
operator|->
name|t_oproc
operator|=
name|cnstart
expr_stmt|;
name|tp
operator|->
name|t_iproc
operator|=
name|NULL
expr_stmt|;
if|if
condition|(
operator|(
name|tp
operator|->
name|t_state
operator|&
name|ISOPEN
operator|)
operator|==
literal|0
condition|)
block|{
name|ttychars
argument_list|(
name|tp
argument_list|)
expr_stmt|;
name|tp
operator|->
name|t_state
operator|=
name|ISOPEN
operator||
name|CARR_ON
expr_stmt|;
name|tp
operator|->
name|t_flags
operator|=
name|EVENP
operator||
name|ECHO
operator||
name|XTABS
operator||
name|CRMOD
expr_stmt|;
block|}
name|mtpr
argument_list|(
name|RXCS
argument_list|,
name|mfpr
argument_list|(
name|RXCS
argument_list|)
operator||
name|RXCS_IE
argument_list|)
expr_stmt|;
name|mtpr
argument_list|(
name|TXCS
argument_list|,
name|mfpr
argument_list|(
name|TXCS
argument_list|)
operator||
name|TXCS_IE
argument_list|)
expr_stmt|;
name|ttyopen
argument_list|(
name|dev
argument_list|,
name|tp
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|cnclose
argument_list|(
argument|dev
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|tty
modifier|*
name|tp
decl_stmt|;
name|tp
operator|=
operator|&
name|cons
expr_stmt|;
name|wflushtty
argument_list|(
name|tp
argument_list|)
expr_stmt|;
name|tp
operator|->
name|t_state
operator|=
literal|0
expr_stmt|;
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|cnread
argument_list|(
argument|dev
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|ttread
argument_list|(
operator|&
name|cons
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|cnwrite
argument_list|(
argument|dev
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|ttwrite
argument_list|(
operator|&
name|cons
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Got a level-20 transmission interrupt -  * the LSI wants another character.  First,  * see if we can send something to the typewriter.  * If not, try the floppy.  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|cnxint
argument_list|(
argument|dev
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|cnxtty
argument_list|()
operator|==
literal|0
condition|)
name|conxfl
argument_list|()
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Formerly cnxint -  * called by new cnxint to see if there are  * any characters to write to the console, and,  * if there are, to do it.  */
end_comment

begin_macro
name|cnxtty
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|struct
name|tty
modifier|*
name|tp
decl_stmt|;
name|tp
operator|=
operator|&
name|cons
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|t_outq
operator|.
name|c_cc
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|ttstart
argument_list|(
name|tp
argument_list|)
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|t_outq
operator|.
name|c_cc
operator|==
literal|0
operator|||
name|tp
operator|->
name|t_outq
operator|.
name|c_cc
operator|==
name|TTLOWAT
condition|)
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|tp
operator|->
name|t_outq
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Got a level-20 receive interrupt -  * the LSI wants to give us a character.  * Catch the character, and see who it goes to.  */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|cnrint
argument_list|(
argument|dev
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|c
decl_stmt|;
name|c
operator|=
name|mfpr
argument_list|(
name|RXDB
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|c
operator|&
name|RXDB_ID
operator|)
operator|==
literal|0
condition|)
comment|/* look at source byte */
name|ttyinput
argument_list|(
name|c
argument_list|,
operator|&
name|cons
argument_list|)
expr_stmt|;
comment|/* character from typewriter */
else|else
name|cnrfl
argument_list|(
name|c
argument_list|)
expr_stmt|;
comment|/* character from floppy */
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|cnioctl
argument_list|(
argument|dev
argument_list|,
argument|cmd
argument_list|,
argument|addr
argument_list|,
argument|flag
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|caddr_t
name|addr
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|tty
modifier|*
name|tp
decl_stmt|;
name|tp
operator|=
operator|&
name|cons
expr_stmt|;
if|if
condition|(
name|ttioccom
argument_list|(
name|cmd
argument_list|,
name|tp
argument_list|,
name|addr
argument_list|,
name|dev
argument_list|)
operator|==
literal|0
condition|)
name|u
operator|.
name|u_error
operator|=
name|ENOTTY
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|cnstart
argument_list|(
name|tp
argument_list|)
specifier|register
expr|struct
name|tty
operator|*
name|tp
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|c
expr_stmt|;
if|if
condition|(
operator|(
name|mfpr
argument_list|(
name|TXCS
argument_list|)
operator|&
name|TXCS_RDY
operator|)
operator|==
literal|0
condition|)
return|return;
if|if
condition|(
operator|(
name|c
operator|=
name|getc
argument_list|(
operator|&
name|tp
operator|->
name|t_outq
argument_list|)
operator|)
operator|>=
literal|0
condition|)
block|{
if|if
condition|(
name|tp
operator|->
name|t_flags
operator|&
name|RAW
condition|)
name|mtpr
argument_list|(
name|TXDB
argument_list|,
name|c
operator|&
literal|0xff
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|c
operator|<=
literal|0177
condition|)
name|mtpr
argument_list|(
name|TXDB
argument_list|,
operator|(
name|c
operator||
operator|(
name|partab
index|[
name|c
index|]
operator|&
literal|0200
operator|)
operator|)
operator|&
literal|0xff
argument_list|)
expr_stmt|;
else|else
block|{
name|timeout
argument_list|(
name|ttrstrt
argument_list|,
operator|(
name|caddr_t
operator|)
name|tp
argument_list|,
operator|(
name|c
operator|&
literal|0177
operator|)
argument_list|)
expr_stmt|;
name|tp
operator|->
name|t_state
operator||=
name|TIMEOUT
expr_stmt|;
block|}
block|}
block|}
end_block

begin_decl_stmt
name|char
modifier|*
name|msgbufp
init|=
name|msgbuf
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Next saved printf character */
end_comment

begin_comment
comment|/*  * Print a character on console.  * Attempts to save and restore device  * status.  * If the switches are 0, all  * printing is inhibited.  *  * Whether or not printing is inhibited,  * the last MSGBUFS characters  * are saved in msgbuf for inspection later.  */
end_comment

begin_expr_stmt
name|putchar
argument_list|(
name|c
argument_list|)
specifier|register
name|c
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|s
operator|,
name|timo
expr_stmt|;
if|if
condition|(
name|c
operator|!=
literal|'\0'
operator|&&
name|c
operator|!=
literal|'\r'
operator|&&
name|c
operator|!=
literal|0177
condition|)
block|{
operator|*
name|msgbufp
operator|++
operator|=
name|c
expr_stmt|;
if|if
condition|(
name|msgbufp
operator|>=
operator|&
name|msgbuf
index|[
name|MSGBUFS
index|]
condition|)
name|msgbufp
operator|=
name|msgbuf
expr_stmt|;
block|}
name|timo
operator|=
literal|30000
expr_stmt|;
comment|/* 	 * Try waiting for the console tty to come ready, 	 * otherwise give up after a reasonable time. 	 */
while|while
condition|(
operator|(
name|mfpr
argument_list|(
name|TXCS
argument_list|)
operator|&
name|TXCS_RDY
operator|)
operator|==
literal|0
condition|)
if|if
condition|(
operator|--
name|timo
operator|==
literal|0
condition|)
break|break;
if|if
condition|(
name|c
operator|==
literal|0
condition|)
return|return;
name|s
operator|=
name|mfpr
argument_list|(
name|TXCS
argument_list|)
expr_stmt|;
name|mtpr
argument_list|(
name|TXCS
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|mtpr
argument_list|(
name|TXDB
argument_list|,
name|c
operator|&
literal|0xff
argument_list|)
expr_stmt|;
if|if
condition|(
name|c
operator|==
literal|'\n'
condition|)
name|putchar
argument_list|(
literal|'\r'
argument_list|)
expr_stmt|;
name|putchar
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|mtpr
argument_list|(
name|TXCS
argument_list|,
name|s
argument_list|)
expr_stmt|;
block|}
end_block

end_unit

