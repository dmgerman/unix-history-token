begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)conf.c	6.11 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"systm.h"
end_include

begin_include
include|#
directive|include
file|"buf.h"
end_include

begin_include
include|#
directive|include
file|"ioctl.h"
end_include

begin_include
include|#
directive|include
file|"tty.h"
end_include

begin_include
include|#
directive|include
file|"conf.h"
end_include

begin_function_decl
name|int
name|nulldev
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|int
name|nodev
parameter_list|()
function_decl|;
end_function_decl

begin_include
include|#
directive|include
file|"hp.h"
end_include

begin_if
if|#
directive|if
name|NHP
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|hpopen
argument_list|()
decl_stmt|,
name|hpstrategy
argument_list|()
decl_stmt|,
name|hpread
argument_list|()
decl_stmt|,
name|hpwrite
argument_list|()
decl_stmt|,
name|hpdump
argument_list|()
decl_stmt|,
name|hpioctl
argument_list|()
decl_stmt|,
name|hpsize
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|hpopen
value|nodev
end_define

begin_define
define|#
directive|define
name|hpstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|hpread
value|nodev
end_define

begin_define
define|#
directive|define
name|hpwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|hpdump
value|nodev
end_define

begin_define
define|#
directive|define
name|hpioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|hpsize
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"tu.h"
end_include

begin_if
if|#
directive|if
name|NHT
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|htopen
argument_list|()
decl_stmt|,
name|htclose
argument_list|()
decl_stmt|,
name|htstrategy
argument_list|()
decl_stmt|,
name|htread
argument_list|()
decl_stmt|,
name|htwrite
argument_list|()
decl_stmt|,
name|htdump
argument_list|()
decl_stmt|,
name|htioctl
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|htopen
value|nodev
end_define

begin_define
define|#
directive|define
name|htclose
value|nodev
end_define

begin_define
define|#
directive|define
name|htstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|htread
value|nodev
end_define

begin_define
define|#
directive|define
name|htwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|htdump
value|nodev
end_define

begin_define
define|#
directive|define
name|htioctl
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"rk.h"
end_include

begin_if
if|#
directive|if
name|NHK
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|rkopen
argument_list|()
decl_stmt|,
name|rkstrategy
argument_list|()
decl_stmt|,
name|rkread
argument_list|()
decl_stmt|,
name|rkwrite
argument_list|()
decl_stmt|,
name|rkintr
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|rkdump
argument_list|()
decl_stmt|,
name|rkreset
argument_list|()
decl_stmt|,
name|rksize
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|rkopen
value|nodev
end_define

begin_define
define|#
directive|define
name|rkstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|rkread
value|nodev
end_define

begin_define
define|#
directive|define
name|rkwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|rkintr
value|nodev
end_define

begin_define
define|#
directive|define
name|rkdump
value|nodev
end_define

begin_define
define|#
directive|define
name|rkreset
value|nodev
end_define

begin_define
define|#
directive|define
name|rksize
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"te.h"
end_include

begin_if
if|#
directive|if
name|NTE
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|tmopen
argument_list|()
decl_stmt|,
name|tmclose
argument_list|()
decl_stmt|,
name|tmstrategy
argument_list|()
decl_stmt|,
name|tmread
argument_list|()
decl_stmt|,
name|tmwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|tmioctl
argument_list|()
decl_stmt|,
name|tmdump
argument_list|()
decl_stmt|,
name|tmreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|tmopen
value|nodev
end_define

begin_define
define|#
directive|define
name|tmclose
value|nodev
end_define

begin_define
define|#
directive|define
name|tmstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|tmread
value|nodev
end_define

begin_define
define|#
directive|define
name|tmwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|tmioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|tmdump
value|nodev
end_define

begin_define
define|#
directive|define
name|tmreset
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"ts.h"
end_include

begin_if
if|#
directive|if
name|NTS
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|tsopen
argument_list|()
decl_stmt|,
name|tsclose
argument_list|()
decl_stmt|,
name|tsstrategy
argument_list|()
decl_stmt|,
name|tsread
argument_list|()
decl_stmt|,
name|tswrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|tsioctl
argument_list|()
decl_stmt|,
name|tsdump
argument_list|()
decl_stmt|,
name|tsreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|tsopen
value|nodev
end_define

begin_define
define|#
directive|define
name|tsclose
value|nodev
end_define

begin_define
define|#
directive|define
name|tsstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|tsread
value|nodev
end_define

begin_define
define|#
directive|define
name|tswrite
value|nodev
end_define

begin_define
define|#
directive|define
name|tsioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|tsdump
value|nodev
end_define

begin_define
define|#
directive|define
name|tsreset
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"mu.h"
end_include

begin_if
if|#
directive|if
name|NMT
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|mtopen
argument_list|()
decl_stmt|,
name|mtclose
argument_list|()
decl_stmt|,
name|mtstrategy
argument_list|()
decl_stmt|,
name|mtread
argument_list|()
decl_stmt|,
name|mtwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|mtioctl
argument_list|()
decl_stmt|,
name|mtdump
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|mtopen
value|nodev
end_define

begin_define
define|#
directive|define
name|mtclose
value|nodev
end_define

begin_define
define|#
directive|define
name|mtstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|mtread
value|nodev
end_define

begin_define
define|#
directive|define
name|mtwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|mtioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|mtdump
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"ra.h"
end_include

begin_if
if|#
directive|if
name|NUDA
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|udopen
argument_list|()
decl_stmt|,
name|udstrategy
argument_list|()
decl_stmt|,
name|udread
argument_list|()
decl_stmt|,
name|udwrite
argument_list|()
decl_stmt|,
name|udreset
argument_list|()
decl_stmt|,
name|uddump
argument_list|()
decl_stmt|,
name|udsize
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|udopen
value|nodev
end_define

begin_define
define|#
directive|define
name|udstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|udread
value|nodev
end_define

begin_define
define|#
directive|define
name|udwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|udreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|uddump
value|nodev
end_define

begin_define
define|#
directive|define
name|udsize
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"up.h"
end_include

begin_if
if|#
directive|if
name|NSC
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|upopen
argument_list|()
decl_stmt|,
name|upstrategy
argument_list|()
decl_stmt|,
name|upread
argument_list|()
decl_stmt|,
name|upwrite
argument_list|()
decl_stmt|,
name|upreset
argument_list|()
decl_stmt|,
name|updump
argument_list|()
decl_stmt|,
name|upsize
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|upopen
value|nodev
end_define

begin_define
define|#
directive|define
name|upstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|upread
value|nodev
end_define

begin_define
define|#
directive|define
name|upwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|upreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|updump
value|nodev
end_define

begin_define
define|#
directive|define
name|upsize
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"tj.h"
end_include

begin_if
if|#
directive|if
name|NUT
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|utopen
argument_list|()
decl_stmt|,
name|utclose
argument_list|()
decl_stmt|,
name|utstrategy
argument_list|()
decl_stmt|,
name|utread
argument_list|()
decl_stmt|,
name|utwrite
argument_list|()
decl_stmt|,
name|utioctl
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|utreset
argument_list|()
decl_stmt|,
name|utdump
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|utopen
value|nodev
end_define

begin_define
define|#
directive|define
name|utclose
value|nodev
end_define

begin_define
define|#
directive|define
name|utread
value|nodev
end_define

begin_define
define|#
directive|define
name|utstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|utwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|utreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|utioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|utdump
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"rb.h"
end_include

begin_if
if|#
directive|if
name|NIDC
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|idcopen
argument_list|()
decl_stmt|,
name|idcstrategy
argument_list|()
decl_stmt|,
name|idcread
argument_list|()
decl_stmt|,
name|idcwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|idcreset
argument_list|()
decl_stmt|,
name|idcdump
argument_list|()
decl_stmt|,
name|idcsize
argument_list|()
decl_stmt|;
end_decl_stmt

begin_empty_stmt
empty_stmt|;
end_empty_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|idcopen
value|nodev
end_define

begin_define
define|#
directive|define
name|idcstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|idcread
value|nodev
end_define

begin_define
define|#
directive|define
name|idcwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|idcreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|idcdump
value|nodev
end_define

begin_define
define|#
directive|define
name|idcsize
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|defined
argument_list|(
name|VAX750
argument_list|)
operator|||
name|defined
argument_list|(
name|VAX730
argument_list|)
end_if

begin_decl_stmt
name|int
name|tuopen
argument_list|()
decl_stmt|,
name|tuclose
argument_list|()
decl_stmt|,
name|tustrategy
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|tuopen
value|nodev
end_define

begin_define
define|#
directive|define
name|tuclose
value|nodev
end_define

begin_define
define|#
directive|define
name|tustrategy
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"rx.h"
end_include

begin_if
if|#
directive|if
name|NFX
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|rxopen
argument_list|()
decl_stmt|,
name|rxstrategy
argument_list|()
decl_stmt|,
name|rxclose
argument_list|()
decl_stmt|,
name|rxread
argument_list|()
decl_stmt|,
name|rxwrite
argument_list|()
decl_stmt|,
name|rxreset
argument_list|()
decl_stmt|,
name|rxioctl
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|rxopen
value|nodev
end_define

begin_define
define|#
directive|define
name|rxstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|rxclose
value|nodev
end_define

begin_define
define|#
directive|define
name|rxread
value|nodev
end_define

begin_define
define|#
directive|define
name|rxwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|rxreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|rxioctl
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"uu.h"
end_include

begin_if
if|#
directive|if
name|NUU
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|uuopen
argument_list|()
decl_stmt|,
name|uustrategy
argument_list|()
decl_stmt|,
name|uuclose
argument_list|()
decl_stmt|,
name|uureset
argument_list|()
decl_stmt|,
name|uuioctl
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|uuopen
value|nodev
end_define

begin_define
define|#
directive|define
name|uustrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|uuclose
value|nodev
end_define

begin_define
define|#
directive|define
name|uureset
value|nulldev
end_define

begin_define
define|#
directive|define
name|uuioctl
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"rl.h"
end_include

begin_if
if|#
directive|if
name|NRL
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|rlopen
argument_list|()
decl_stmt|,
name|rlstrategy
argument_list|()
decl_stmt|,
name|rlread
argument_list|()
decl_stmt|,
name|rlwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|rlreset
argument_list|()
decl_stmt|,
name|rldump
argument_list|()
decl_stmt|,
name|rlsize
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|rlopen
value|nodev
end_define

begin_define
define|#
directive|define
name|rlstrategy
value|nodev
end_define

begin_define
define|#
directive|define
name|rlread
value|nodev
end_define

begin_define
define|#
directive|define
name|rlwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|rlreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|rldump
value|nodev
end_define

begin_define
define|#
directive|define
name|rlsize
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|int
name|swstrategy
argument_list|()
decl_stmt|,
name|swread
argument_list|()
decl_stmt|,
name|swwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|bdevsw
name|bdevsw
index|[]
init|=
block|{
block|{
name|hpopen
block|,
name|nulldev
block|,
name|hpstrategy
block|,
name|hpdump
block|,
comment|/*0*/
name|hpsize
block|,
literal|0
block|}
block|,
block|{
name|htopen
block|,
name|htclose
block|,
name|htstrategy
block|,
name|htdump
block|,
comment|/*1*/
literal|0
block|,
name|B_TAPE
block|}
block|,
block|{
name|upopen
block|,
name|nulldev
block|,
name|upstrategy
block|,
name|updump
block|,
comment|/*2*/
name|upsize
block|,
literal|0
block|}
block|,
block|{
name|rkopen
block|,
name|nulldev
block|,
name|rkstrategy
block|,
name|rkdump
block|,
comment|/*3*/
name|rksize
block|,
literal|0
block|}
block|,
block|{
name|nodev
block|,
name|nodev
block|,
name|swstrategy
block|,
name|nodev
block|,
comment|/*4*/
literal|0
block|,
literal|0
block|}
block|,
block|{
name|tmopen
block|,
name|tmclose
block|,
name|tmstrategy
block|,
name|tmdump
block|,
comment|/*5*/
literal|0
block|,
name|B_TAPE
block|}
block|,
block|{
name|tsopen
block|,
name|tsclose
block|,
name|tsstrategy
block|,
name|tsdump
block|,
comment|/*6*/
literal|0
block|,
name|B_TAPE
block|}
block|,
block|{
name|mtopen
block|,
name|mtclose
block|,
name|mtstrategy
block|,
name|mtdump
block|,
comment|/*7*/
literal|0
block|,
name|B_TAPE
block|}
block|,
block|{
name|tuopen
block|,
name|tuclose
block|,
name|tustrategy
block|,
name|nodev
block|,
comment|/*8*/
literal|0
block|,
name|B_TAPE
block|}
block|,
block|{
name|udopen
block|,
name|nulldev
block|,
name|udstrategy
block|,
name|uddump
block|,
comment|/*9*/
name|udsize
block|,
literal|0
block|}
block|,
block|{
name|utopen
block|,
name|utclose
block|,
name|utstrategy
block|,
name|utdump
block|,
comment|/*10*/
literal|0
block|,
name|B_TAPE
block|}
block|,
block|{
name|idcopen
block|,
name|nodev
block|,
name|idcstrategy
block|,
name|idcdump
block|,
comment|/*11*/
name|idcsize
block|,
literal|0
block|}
block|,
block|{
name|rxopen
block|,
name|rxclose
block|,
name|rxstrategy
block|,
name|nodev
block|,
comment|/*12*/
literal|0
block|,
literal|0
block|}
block|,
block|{
name|uuopen
block|,
name|uuclose
block|,
name|uustrategy
block|,
name|nodev
block|,
comment|/*13*/
literal|0
block|,
literal|0
block|}
block|,
block|{
name|rlopen
block|,
name|nodev
block|,
name|rlstrategy
block|,
name|rldump
block|,
comment|/*14*/
name|rlsize
block|,
literal|0
block|}
block|, }
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nblkdev
init|=
sizeof|sizeof
argument_list|(
name|bdevsw
argument_list|)
operator|/
sizeof|sizeof
argument_list|(
name|bdevsw
index|[
literal|0
index|]
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|cnopen
argument_list|()
decl_stmt|,
name|cnclose
argument_list|()
decl_stmt|,
name|cnread
argument_list|()
decl_stmt|,
name|cnwrite
argument_list|()
decl_stmt|,
name|cnioctl
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|tty
name|cons
decl_stmt|;
end_decl_stmt

begin_include
include|#
directive|include
file|"acc.h"
end_include

begin_if
if|#
directive|if
name|NACC
operator|>
literal|0
end_if

begin_function_decl
name|int
name|accreset
parameter_list|()
function_decl|;
end_function_decl

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|accreset
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"ct.h"
end_include

begin_if
if|#
directive|if
name|NCT
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|ctopen
argument_list|()
decl_stmt|,
name|ctclose
argument_list|()
decl_stmt|,
name|ctwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|ctopen
value|nulldev
end_define

begin_define
define|#
directive|define
name|ctclose
value|nulldev
end_define

begin_define
define|#
directive|define
name|ctwrite
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"dh.h"
end_include

begin_if
if|#
directive|if
name|NDH
operator|==
literal|0
end_if

begin_define
define|#
directive|define
name|dhopen
value|nodev
end_define

begin_define
define|#
directive|define
name|dhclose
value|nodev
end_define

begin_define
define|#
directive|define
name|dhread
value|nodev
end_define

begin_define
define|#
directive|define
name|dhwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|dhioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|dhstop
value|nodev
end_define

begin_define
define|#
directive|define
name|dhreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|dh11
value|0
end_define

begin_else
else|#
directive|else
end_else

begin_decl_stmt
name|int
name|dhopen
argument_list|()
decl_stmt|,
name|dhclose
argument_list|()
decl_stmt|,
name|dhread
argument_list|()
decl_stmt|,
name|dhwrite
argument_list|()
decl_stmt|,
name|dhioctl
argument_list|()
decl_stmt|,
name|dhstop
argument_list|()
decl_stmt|,
name|dhreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|tty
name|dh11
index|[]
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"dmf.h"
end_include

begin_if
if|#
directive|if
name|NDMF
operator|==
literal|0
end_if

begin_define
define|#
directive|define
name|dmfopen
value|nodev
end_define

begin_define
define|#
directive|define
name|dmfclose
value|nodev
end_define

begin_define
define|#
directive|define
name|dmfread
value|nodev
end_define

begin_define
define|#
directive|define
name|dmfwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|dmfioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|dmfstop
value|nodev
end_define

begin_define
define|#
directive|define
name|dmfreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|dmf_tty
value|0
end_define

begin_else
else|#
directive|else
end_else

begin_decl_stmt
name|int
name|dmfopen
argument_list|()
decl_stmt|,
name|dmfclose
argument_list|()
decl_stmt|,
name|dmfread
argument_list|()
decl_stmt|,
name|dmfwrite
argument_list|()
decl_stmt|,
name|dmfioctl
argument_list|()
decl_stmt|,
name|dmfstop
argument_list|()
decl_stmt|,
name|dmfreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|tty
name|dmf_tty
index|[]
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|VAX8600
end_if

begin_decl_stmt
name|int
name|crlopen
argument_list|()
decl_stmt|,
name|crlclose
argument_list|()
decl_stmt|,
name|crlread
argument_list|()
decl_stmt|,
name|crlwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|crlopen
value|nodev
end_define

begin_define
define|#
directive|define
name|crlclose
value|nodev
end_define

begin_define
define|#
directive|define
name|crlread
value|nodev
end_define

begin_define
define|#
directive|define
name|crlwrite
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|VAX780
end_if

begin_decl_stmt
name|int
name|flopen
argument_list|()
decl_stmt|,
name|flclose
argument_list|()
decl_stmt|,
name|flread
argument_list|()
decl_stmt|,
name|flwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|flopen
value|nodev
end_define

begin_define
define|#
directive|define
name|flclose
value|nodev
end_define

begin_define
define|#
directive|define
name|flread
value|nodev
end_define

begin_define
define|#
directive|define
name|flwrite
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"dz.h"
end_include

begin_if
if|#
directive|if
name|NDZ
operator|==
literal|0
end_if

begin_define
define|#
directive|define
name|dzopen
value|nodev
end_define

begin_define
define|#
directive|define
name|dzclose
value|nodev
end_define

begin_define
define|#
directive|define
name|dzread
value|nodev
end_define

begin_define
define|#
directive|define
name|dzwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|dzioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|dzstop
value|nodev
end_define

begin_define
define|#
directive|define
name|dzreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|dz_tty
value|0
end_define

begin_else
else|#
directive|else
end_else

begin_decl_stmt
name|int
name|dzopen
argument_list|()
decl_stmt|,
name|dzclose
argument_list|()
decl_stmt|,
name|dzread
argument_list|()
decl_stmt|,
name|dzwrite
argument_list|()
decl_stmt|,
name|dzioctl
argument_list|()
decl_stmt|,
name|dzstop
argument_list|()
decl_stmt|,
name|dzreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|tty
name|dz_tty
index|[]
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"lp.h"
end_include

begin_if
if|#
directive|if
name|NLP
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|lpopen
argument_list|()
decl_stmt|,
name|lpclose
argument_list|()
decl_stmt|,
name|lpwrite
argument_list|()
decl_stmt|,
name|lpreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|lpopen
value|nodev
end_define

begin_define
define|#
directive|define
name|lpclose
value|nodev
end_define

begin_define
define|#
directive|define
name|lpwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|lpreset
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|int
name|syopen
argument_list|()
decl_stmt|,
name|syread
argument_list|()
decl_stmt|,
name|sywrite
argument_list|()
decl_stmt|,
name|syioctl
argument_list|()
decl_stmt|,
name|syselect
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|mmread
argument_list|()
decl_stmt|,
name|mmwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|mmselect
value|seltrue
end_define

begin_include
include|#
directive|include
file|"va.h"
end_include

begin_if
if|#
directive|if
name|NVA
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|vaopen
argument_list|()
decl_stmt|,
name|vaclose
argument_list|()
decl_stmt|,
name|vawrite
argument_list|()
decl_stmt|,
name|vaioctl
argument_list|()
decl_stmt|,
name|vareset
argument_list|()
decl_stmt|,
name|vaselect
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|vaopen
value|nodev
end_define

begin_define
define|#
directive|define
name|vaclose
value|nodev
end_define

begin_define
define|#
directive|define
name|vawrite
value|nodev
end_define

begin_define
define|#
directive|define
name|vaopen
value|nodev
end_define

begin_define
define|#
directive|define
name|vaioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|vareset
value|nulldev
end_define

begin_define
define|#
directive|define
name|vaselect
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"vp.h"
end_include

begin_if
if|#
directive|if
name|NVP
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|vpopen
argument_list|()
decl_stmt|,
name|vpclose
argument_list|()
decl_stmt|,
name|vpwrite
argument_list|()
decl_stmt|,
name|vpioctl
argument_list|()
decl_stmt|,
name|vpreset
argument_list|()
decl_stmt|,
name|vpselect
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|vpopen
value|nodev
end_define

begin_define
define|#
directive|define
name|vpclose
value|nodev
end_define

begin_define
define|#
directive|define
name|vpwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|vpioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|vpreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|vpselect
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"pty.h"
end_include

begin_if
if|#
directive|if
name|NPTY
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|ptsopen
argument_list|()
decl_stmt|,
name|ptsclose
argument_list|()
decl_stmt|,
name|ptsread
argument_list|()
decl_stmt|,
name|ptswrite
argument_list|()
decl_stmt|,
name|ptsstop
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ptcopen
argument_list|()
decl_stmt|,
name|ptcclose
argument_list|()
decl_stmt|,
name|ptcread
argument_list|()
decl_stmt|,
name|ptcwrite
argument_list|()
decl_stmt|,
name|ptcselect
argument_list|()
decl_stmt|;
end_decl_stmt

begin_function_decl
name|int
name|ptyioctl
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|struct
name|tty
name|pt_tty
index|[]
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|ptsopen
value|nodev
end_define

begin_define
define|#
directive|define
name|ptsclose
value|nodev
end_define

begin_define
define|#
directive|define
name|ptsread
value|nodev
end_define

begin_define
define|#
directive|define
name|ptswrite
value|nodev
end_define

begin_define
define|#
directive|define
name|ptcopen
value|nodev
end_define

begin_define
define|#
directive|define
name|ptcclose
value|nodev
end_define

begin_define
define|#
directive|define
name|ptcread
value|nodev
end_define

begin_define
define|#
directive|define
name|ptcwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|ptyioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|pt_tty
value|0
end_define

begin_define
define|#
directive|define
name|ptcselect
value|nodev
end_define

begin_define
define|#
directive|define
name|ptsstop
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"lpa.h"
end_include

begin_if
if|#
directive|if
name|NLPA
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|lpaopen
argument_list|()
decl_stmt|,
name|lpaclose
argument_list|()
decl_stmt|,
name|lparead
argument_list|()
decl_stmt|,
name|lpawrite
argument_list|()
decl_stmt|,
name|lpaioctl
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|lpaopen
value|nodev
end_define

begin_define
define|#
directive|define
name|lpaclose
value|nodev
end_define

begin_define
define|#
directive|define
name|lparead
value|nodev
end_define

begin_define
define|#
directive|define
name|lpawrite
value|nodev
end_define

begin_define
define|#
directive|define
name|lpaioctl
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"dn.h"
end_include

begin_if
if|#
directive|if
name|NDN
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|dnopen
argument_list|()
decl_stmt|,
name|dnclose
argument_list|()
decl_stmt|,
name|dnwrite
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|dnopen
value|nodev
end_define

begin_define
define|#
directive|define
name|dnclose
value|nodev
end_define

begin_define
define|#
directive|define
name|dnwrite
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"ik.h"
end_include

begin_if
if|#
directive|if
name|NIK
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|ikopen
argument_list|()
decl_stmt|,
name|ikclose
argument_list|()
decl_stmt|,
name|ikread
argument_list|()
decl_stmt|,
name|ikwrite
argument_list|()
decl_stmt|,
name|ikioctl
argument_list|()
decl_stmt|,
name|ikreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|ikopen
value|nodev
end_define

begin_define
define|#
directive|define
name|ikclose
value|nodev
end_define

begin_define
define|#
directive|define
name|ikread
value|nodev
end_define

begin_define
define|#
directive|define
name|ikwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|ikioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|ikreset
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"ps.h"
end_include

begin_if
if|#
directive|if
name|NPS
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|psopen
argument_list|()
decl_stmt|,
name|psclose
argument_list|()
decl_stmt|,
name|psread
argument_list|()
decl_stmt|,
name|pswrite
argument_list|()
decl_stmt|,
name|psioctl
argument_list|()
decl_stmt|,
name|psreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|psopen
value|nodev
end_define

begin_define
define|#
directive|define
name|psclose
value|nodev
end_define

begin_define
define|#
directive|define
name|psread
value|nodev
end_define

begin_define
define|#
directive|define
name|pswrite
value|nodev
end_define

begin_define
define|#
directive|define
name|psopen
value|nodev
end_define

begin_define
define|#
directive|define
name|psioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|psreset
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"ad.h"
end_include

begin_if
if|#
directive|if
name|NAD
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|adopen
argument_list|()
decl_stmt|,
name|adclose
argument_list|()
decl_stmt|,
name|adioctl
argument_list|()
decl_stmt|,
name|adreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|adopen
value|nodev
end_define

begin_define
define|#
directive|define
name|adclose
value|nodev
end_define

begin_define
define|#
directive|define
name|adioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|adreset
value|nulldev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|int
name|logopen
argument_list|()
decl_stmt|,
name|logclose
argument_list|()
decl_stmt|,
name|logread
argument_list|()
decl_stmt|,
name|logioctl
argument_list|()
decl_stmt|,
name|logselect
argument_list|()
decl_stmt|;
end_decl_stmt

begin_include
include|#
directive|include
file|"dhu.h"
end_include

begin_if
if|#
directive|if
name|NDHU
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|dhuopen
argument_list|()
decl_stmt|,
name|dhuclose
argument_list|()
decl_stmt|,
name|dhuread
argument_list|()
decl_stmt|,
name|dhuwrite
argument_list|()
decl_stmt|,
name|dhuioctl
argument_list|()
decl_stmt|,
name|dhustop
argument_list|()
decl_stmt|,
name|dhureset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|tty
name|dhu_tty
index|[]
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|dhuopen
value|nodev
end_define

begin_define
define|#
directive|define
name|dhuclose
value|nodev
end_define

begin_define
define|#
directive|define
name|dhuread
value|nodev
end_define

begin_define
define|#
directive|define
name|dhuwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|dhuioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|dhustop
value|nodev
end_define

begin_define
define|#
directive|define
name|dhureset
value|nulldev
end_define

begin_define
define|#
directive|define
name|dhu_tty
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"vs.h"
end_include

begin_if
if|#
directive|if
name|NVS
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|vsopen
argument_list|()
decl_stmt|,
name|vsclose
argument_list|()
decl_stmt|,
name|vsioctl
argument_list|()
decl_stmt|,
name|vsreset
argument_list|()
decl_stmt|,
name|vsselect
argument_list|()
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|vsopen
value|nodev
end_define

begin_define
define|#
directive|define
name|vsclose
value|nodev
end_define

begin_define
define|#
directive|define
name|vsioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|vsreset
value|nodev
end_define

begin_define
define|#
directive|define
name|vsselect
value|nodev
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"dmz.h"
end_include

begin_if
if|#
directive|if
name|NDMZ
operator|>
literal|0
end_if

begin_decl_stmt
name|int
name|dmzopen
argument_list|()
decl_stmt|,
name|dmzclose
argument_list|()
decl_stmt|,
name|dmzread
argument_list|()
decl_stmt|,
name|dmzwrite
argument_list|()
decl_stmt|,
name|dmzioctl
argument_list|()
decl_stmt|,
name|dmzstop
argument_list|()
decl_stmt|,
name|dmzreset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|tty
name|dmz_tty
index|[]
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|dmzopen
value|nodev
end_define

begin_define
define|#
directive|define
name|dmzclose
value|nodev
end_define

begin_define
define|#
directive|define
name|dmzread
value|nodev
end_define

begin_define
define|#
directive|define
name|dmzwrite
value|nodev
end_define

begin_define
define|#
directive|define
name|dmzioctl
value|nodev
end_define

begin_define
define|#
directive|define
name|dmzstop
value|nodev
end_define

begin_define
define|#
directive|define
name|dmzreset
value|nulldev
end_define

begin_define
define|#
directive|define
name|dmz_tty
value|0
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|int
name|ttselect
argument_list|()
decl_stmt|,
name|seltrue
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|cdevsw
name|cdevsw
index|[]
init|=
block|{
name|cnopen
block|,
name|cnclose
block|,
name|cnread
block|,
name|cnwrite
block|,
comment|/*0*/
name|cnioctl
block|,
name|nulldev
block|,
name|nulldev
block|,
operator|&
name|cons
block|,
name|ttselect
block|,
name|nodev
block|,
name|dzopen
block|,
name|dzclose
block|,
name|dzread
block|,
name|dzwrite
block|,
comment|/*1*/
name|dzioctl
block|,
name|dzstop
block|,
name|dzreset
block|,
name|dz_tty
block|,
name|ttselect
block|,
name|nodev
block|,
name|syopen
block|,
name|nulldev
block|,
name|syread
block|,
name|sywrite
block|,
comment|/*2*/
name|syioctl
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|syselect
block|,
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
name|mmread
block|,
name|mmwrite
block|,
comment|/*3*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|mmselect
block|,
name|nodev
block|,
name|hpopen
block|,
name|nulldev
block|,
name|hpread
block|,
name|hpwrite
block|,
comment|/*4*/
name|hpioctl
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|htopen
block|,
name|htclose
block|,
name|htread
block|,
name|htwrite
block|,
comment|/*5*/
name|htioctl
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|vpopen
block|,
name|vpclose
block|,
name|nodev
block|,
name|vpwrite
block|,
comment|/*6*/
name|vpioctl
block|,
name|nulldev
block|,
name|vpreset
block|,
literal|0
block|,
name|vpselect
block|,
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
name|swread
block|,
name|swwrite
block|,
comment|/*7*/
name|nodev
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|,
name|flopen
block|,
name|flclose
block|,
name|flread
block|,
name|flwrite
block|,
comment|/*8*/
name|nodev
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|udopen
block|,
name|nulldev
block|,
name|udread
block|,
name|udwrite
block|,
comment|/*9*/
name|nodev
block|,
name|nodev
block|,
name|udreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|vaopen
block|,
name|vaclose
block|,
name|nodev
block|,
name|vawrite
block|,
comment|/*10*/
name|vaioctl
block|,
name|nulldev
block|,
name|vareset
block|,
literal|0
block|,
name|vaselect
block|,
name|nodev
block|,
name|rkopen
block|,
name|nulldev
block|,
name|rkread
block|,
name|rkwrite
block|,
comment|/*11*/
name|nodev
block|,
name|nodev
block|,
name|rkreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|dhopen
block|,
name|dhclose
block|,
name|dhread
block|,
name|dhwrite
block|,
comment|/*12*/
name|dhioctl
block|,
name|dhstop
block|,
name|dhreset
block|,
name|dh11
block|,
name|ttselect
block|,
name|nodev
block|,
name|upopen
block|,
name|nulldev
block|,
name|upread
block|,
name|upwrite
block|,
comment|/*13*/
name|nodev
block|,
name|nodev
block|,
name|upreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|tmopen
block|,
name|tmclose
block|,
name|tmread
block|,
name|tmwrite
block|,
comment|/*14*/
name|tmioctl
block|,
name|nodev
block|,
name|tmreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|lpopen
block|,
name|lpclose
block|,
name|nodev
block|,
name|lpwrite
block|,
comment|/*15*/
name|nodev
block|,
name|nodev
block|,
name|lpreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|tsopen
block|,
name|tsclose
block|,
name|tsread
block|,
name|tswrite
block|,
comment|/*16*/
name|tsioctl
block|,
name|nodev
block|,
name|tsreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|utopen
block|,
name|utclose
block|,
name|utread
block|,
name|utwrite
block|,
comment|/*17*/
name|utioctl
block|,
name|nodev
block|,
name|utreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|ctopen
block|,
name|ctclose
block|,
name|nodev
block|,
name|ctwrite
block|,
comment|/*18*/
name|nodev
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|mtopen
block|,
name|mtclose
block|,
name|mtread
block|,
name|mtwrite
block|,
comment|/*19*/
name|mtioctl
block|,
name|nodev
block|,
name|nodev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|ptsopen
block|,
name|ptsclose
block|,
name|ptsread
block|,
name|ptswrite
block|,
comment|/*20*/
name|ptyioctl
block|,
name|ptsstop
block|,
name|nulldev
block|,
name|pt_tty
block|,
name|ttselect
block|,
name|nodev
block|,
name|ptcopen
block|,
name|ptcclose
block|,
name|ptcread
block|,
name|ptcwrite
block|,
comment|/*21*/
name|ptyioctl
block|,
name|nulldev
block|,
name|nulldev
block|,
name|pt_tty
block|,
name|ptcselect
block|,
name|nodev
block|,
name|dmfopen
block|,
name|dmfclose
block|,
name|dmfread
block|,
name|dmfwrite
block|,
comment|/*22*/
name|dmfioctl
block|,
name|dmfstop
block|,
name|dmfreset
block|,
name|dmf_tty
block|,
name|ttselect
block|,
name|nodev
block|,
name|idcopen
block|,
name|nulldev
block|,
name|idcread
block|,
name|idcwrite
block|,
comment|/*23*/
name|nodev
block|,
name|nodev
block|,
name|idcreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|dnopen
block|,
name|dnclose
block|,
name|nodev
block|,
name|dnwrite
block|,
comment|/*24*/
name|nodev
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
comment|/* 25-29 reserved to local sites */
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*25*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|,
name|lpaopen
block|,
name|lpaclose
block|,
name|lparead
block|,
name|lpawrite
block|,
comment|/*26*/
name|lpaioctl
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|psopen
block|,
name|psclose
block|,
name|psread
block|,
name|pswrite
block|,
comment|/*27*/
name|psioctl
block|,
name|nodev
block|,
name|psreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*28*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|,
name|adopen
block|,
name|adclose
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*29*/
name|adioctl
block|,
name|nodev
block|,
name|adreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|rxopen
block|,
name|rxclose
block|,
name|rxread
block|,
name|rxwrite
block|,
comment|/*30*/
name|rxioctl
block|,
name|nodev
block|,
name|rxreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|ikopen
block|,
name|ikclose
block|,
name|ikread
block|,
name|ikwrite
block|,
comment|/*31*/
name|ikioctl
block|,
name|nodev
block|,
name|ikreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|rlopen
block|,
name|nodev
block|,
name|rlread
block|,
name|rlwrite
block|,
comment|/*32*/
name|nodev
block|,
name|nodev
block|,
name|rlreset
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|logopen
block|,
name|logclose
block|,
name|logread
block|,
name|nodev
block|,
comment|/*33*/
name|logioctl
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|logselect
block|,
name|nodev
block|,
name|dhuopen
block|,
name|dhuclose
block|,
name|dhuread
block|,
name|dhuwrite
block|,
comment|/*34*/
name|dhuioctl
block|,
name|dhustop
block|,
name|dhureset
block|,
name|dhu_tty
block|,
name|ttselect
block|,
name|nodev
block|,
name|crlopen
block|,
name|crlclose
block|,
name|crlread
block|,
name|crlwrite
block|,
comment|/*35*/
name|nodev
block|,
name|nodev
block|,
name|nulldev
block|,
literal|0
block|,
name|seltrue
block|,
name|nodev
block|,
name|vsopen
block|,
name|vsclose
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*36*/
name|vsioctl
block|,
name|nodev
block|,
name|vsreset
block|,
literal|0
block|,
name|vsselect
block|,
name|nodev
block|,
name|dmzopen
block|,
name|dmzclose
block|,
name|dmzread
block|,
name|dmzwrite
block|,
comment|/*37*/
name|dmzioctl
block|,
name|dmzstop
block|,
name|dmzreset
block|,
name|dmz_tty
block|,
name|ttselect
block|,
name|nodev
block|,
comment|/* 38-42 reserved to local sites */
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*38*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*39*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*40*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*41*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
name|nodev
block|,
comment|/*42*/
name|nodev
block|,
name|nulldev
block|,
name|nulldev
block|,
literal|0
block|,
name|nodev
block|,
name|nodev
block|, }
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nchrdev
init|=
sizeof|sizeof
argument_list|(
name|cdevsw
argument_list|)
operator|/
sizeof|sizeof
argument_list|(
name|cdevsw
index|[
literal|0
index|]
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|mem_no
init|=
literal|3
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* major device number of memory special file */
end_comment

begin_comment
comment|/*  * Swapdev is a fake device implemented  * in sw.c used only internally to get to swstrategy.  * It cannot be provided to the users, because the  * swstrategy routine munches the b_dev and b_blkno entries  * before calling the appropriate driver.  This would horribly  * confuse, e.g. the hashing routines. Instead, /dev/drum is  * provided as a character (raw) device.  */
end_comment

begin_decl_stmt
name|dev_t
name|swapdev
init|=
name|makedev
argument_list|(
literal|4
argument_list|,
literal|0
argument_list|)
decl_stmt|;
end_decl_stmt

end_unit

