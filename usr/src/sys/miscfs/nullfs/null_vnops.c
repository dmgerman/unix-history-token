...
condition|)
block|{
comment|/* 		 * XXX - even though some ops have vpp returned vp's, 		 * several ops actually vrele this before returning. 		 * We must avoid these ops. 		 * (This should go away.) 		 */
if|if
condition|(
name|descp
operator|->
...
name|printf
argument_list|(
literal|"null_bypass (%s), lowervpp->usecount = %d\n"
argument_list|,
name|vdesc
operator|->
name|vdesc_name
...
name|VBAD
expr_stmt|;
comment|/* The node is clean (no reclaim needed). */
name|vrele
argument_list|(
name|lowervp
argument_list|)
...
directive|else
comment|/* 	 * Note: at this point, vp->v_op == dead_vnodeop_p, 	 * so we can't call VOPs on ourself. 	 */
...
name|panic
argument_list|(
literal|"null_reclaim: lowervp is locked but must go away."
argument_list|)
expr_stmt|;
block|}
empty_stmt|;
