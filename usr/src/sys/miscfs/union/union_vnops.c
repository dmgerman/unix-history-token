...
begin_comment
comment|/*  * Copyright (c) 1992, 1993, 1994 The Regents of the University of California.  * Copyright (c) 1992, 1993, 1994 Jan-Simon Pendry.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Jan-Simon Pendry.  *  * %sccs.include.redist.c%  *  *	@(#)union_vnops.c	1.5 (Berkeley) %G%  */
end_comment
...
begin_comment
comment|/*  * Create a shadow directory in the upper layer.  * The new vnode is returned locked.  */
end_comment
...
comment|/* 			NDINIT(&nd, CREATE, 0, UIO_SYSSPACE, un->un_path, p); 			fmode = (O_CREAT|O_TRUNC|O_EXCL); 			*/
name|cmode
operator|=
name|UN_FILEMODE
operator|&
...
block|{
comment|/* 		 * XXX should do a copyfile (perhaps only if 		 * the file permission change, which would not 		 * track va_ctime correctly). 		 */
name|error
operator|=
name|EROFS
expr_stmt|;
