...
begin_comment
comment|/*  * Copyright (c) 1994 The Regents of the University of California.  * Copyright (c) 1994 Jan-Simon Pendry.  * All rights reserved.  *  * This code is derived from software donated to Berkeley by  * Jan-Simon Pendry.  *  * %sccs.include.redist.c%  *  *	@(#)union_vfsops.c	8.2 (Berkeley) %G%  */
end_comment
...
name|p_ruid
expr_stmt|;
comment|/* 	 * Ensure the *real* user has write permission on the 	 * mounted-on directory.  This allows the mount_union 	 * command to be made setuid root so allowing anyone 	 * to do union mounts onto any directory on which they 	 * have write permission. 	 */
name|error
operator|=
name|VOP_ACCESS
argument_list|(
