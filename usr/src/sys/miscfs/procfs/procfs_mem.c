begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1993 The Regents of the University of California.  * Copyright (c) 1993 Jan-Simon Pendry  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Jan-Simon Pendry and Sean Eric Fagan.  *  * %sccs.include.redist.c%  *  *	@(#)procfs_mem.c	8.2 (Berkeley) %G%  *  * From:  *	$Id: procfs_mem.c,v 3.2 1993/12/15 09:40:17 jsp Exp $  */
end_comment

begin_comment
comment|/*  * This is a lightly hacked and merged version  * of sef's pread/pwrite functions  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<miscfs/procfs/procfs.h>
end_include

begin_include
include|#
directive|include
file|<vm/vm.h>
end_include

begin_include
include|#
directive|include
file|<vm/vm_kern.h>
end_include

begin_include
include|#
directive|include
file|<vm/vm_page.h>
end_include

begin_function
specifier|static
name|int
name|procfs_rwmem
parameter_list|(
name|p
parameter_list|,
name|uio
parameter_list|)
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
name|struct
name|uio
modifier|*
name|uio
decl_stmt|;
block|{
name|int
name|error
decl_stmt|;
name|int
name|writing
decl_stmt|;
name|writing
operator|=
name|uio
operator|->
name|uio_rw
operator|==
name|UIO_WRITE
expr_stmt|;
comment|/* 	 * Only map in one page at a time.  We don't have to, but it 	 * makes things easier.  This way is trivial - right? 	 */
do|do
block|{
name|vm_map_t
name|map
decl_stmt|,
name|tmap
decl_stmt|;
name|vm_object_t
name|object
decl_stmt|;
name|vm_offset_t
name|kva
decl_stmt|;
name|vm_offset_t
name|uva
decl_stmt|;
name|int
name|page_offset
decl_stmt|;
comment|/* offset into page */
name|vm_offset_t
name|pageno
decl_stmt|;
comment|/* page number */
name|vm_map_entry_t
name|out_entry
decl_stmt|;
name|vm_prot_t
name|out_prot
decl_stmt|;
name|vm_page_t
name|m
decl_stmt|;
name|boolean_t
name|wired
decl_stmt|,
name|single_use
decl_stmt|;
name|vm_offset_t
name|off
decl_stmt|;
name|u_int
name|len
decl_stmt|;
name|int
name|fix_prot
decl_stmt|;
name|uva
operator|=
operator|(
name|vm_offset_t
operator|)
name|uio
operator|->
name|uio_offset
expr_stmt|;
if|if
condition|(
name|uva
operator|>
name|VM_MAXUSER_ADDRESS
condition|)
block|{
name|error
operator|=
literal|0
expr_stmt|;
break|break;
block|}
comment|/* 		 * Get the page number of this segment. 		 */
name|pageno
operator|=
name|trunc_page
argument_list|(
name|uva
argument_list|)
expr_stmt|;
name|page_offset
operator|=
name|uva
operator|-
name|pageno
expr_stmt|;
comment|/* 		 * How many bytes to copy 		 */
name|len
operator|=
name|min
argument_list|(
name|PAGE_SIZE
operator|-
name|page_offset
argument_list|,
name|uio
operator|->
name|uio_resid
argument_list|)
expr_stmt|;
comment|/* 		 * The map we want... 		 */
name|map
operator|=
operator|&
name|p
operator|->
name|p_vmspace
operator|->
name|vm_map
expr_stmt|;
comment|/* 		 * Check the permissions for the area we're interested 		 * in. 		 */
name|fix_prot
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|writing
condition|)
name|fix_prot
operator|=
operator|!
name|vm_map_check_protection
argument_list|(
name|map
argument_list|,
name|pageno
argument_list|,
name|pageno
operator|+
name|PAGE_SIZE
argument_list|,
name|VM_PROT_WRITE
argument_list|)
expr_stmt|;
if|if
condition|(
name|fix_prot
condition|)
block|{
comment|/* 			 * If the page is not writable, we make it so. 			 * XXX It is possible that a page may *not* be 			 * read/executable, if a process changes that! 			 * We will assume, for now, that a page is either 			 * VM_PROT_ALL, or VM_PROT_READ|VM_PROT_EXECUTE. 			 */
name|error
operator|=
name|vm_map_protect
argument_list|(
name|map
argument_list|,
name|pageno
argument_list|,
name|pageno
operator|+
name|PAGE_SIZE
argument_list|,
name|VM_PROT_ALL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
break|break;
block|}
comment|/* 		 * Now we need to get the page.  out_entry, out_prot, wired, 		 * and single_use aren't used.  One would think the vm code 		 * would be a *bit* nicer...  We use tmap because 		 * vm_map_lookup() can change the map argument. 		 */
name|tmap
operator|=
name|map
expr_stmt|;
name|error
operator|=
name|vm_map_lookup
argument_list|(
operator|&
name|tmap
argument_list|,
name|pageno
argument_list|,
name|writing
condition|?
name|VM_PROT_WRITE
else|:
name|VM_PROT_READ
argument_list|,
operator|&
name|out_entry
argument_list|,
operator|&
name|object
argument_list|,
operator|&
name|off
argument_list|,
operator|&
name|out_prot
argument_list|,
operator|&
name|wired
argument_list|,
operator|&
name|single_use
argument_list|)
expr_stmt|;
comment|/* 		 * We're done with tmap now. 		 */
if|if
condition|(
operator|!
name|error
condition|)
name|vm_map_lookup_done
argument_list|(
name|tmap
argument_list|,
name|out_entry
argument_list|)
expr_stmt|;
comment|/* 		 * Fault the page in... 		 */
if|if
condition|(
operator|!
name|error
operator|&&
name|writing
operator|&&
name|object
operator|->
name|shadow
condition|)
block|{
name|m
operator|=
name|vm_page_lookup
argument_list|(
name|object
argument_list|,
name|off
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
operator|||
operator|(
name|m
operator|->
name|flags
operator|&
name|PG_COPYONWRITE
operator|)
condition|)
name|error
operator|=
name|vm_fault
argument_list|(
name|map
argument_list|,
name|pageno
argument_list|,
name|VM_PROT_WRITE
argument_list|,
name|FALSE
argument_list|)
expr_stmt|;
block|}
comment|/* Find space in kernel_map for the page we're interested in */
if|if
condition|(
operator|!
name|error
condition|)
name|error
operator|=
name|vm_map_find
argument_list|(
name|kernel_map
argument_list|,
name|object
argument_list|,
name|off
argument_list|,
operator|&
name|kva
argument_list|,
name|PAGE_SIZE
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|error
condition|)
block|{
comment|/* 			 * Neither vm_map_lookup() nor vm_map_find() appear 			 * to add a reference count to the object, so we do 			 * that here and now. 			 */
name|vm_object_reference
argument_list|(
name|object
argument_list|)
expr_stmt|;
comment|/* 			 * Mark the page we just found as pageable. 			 */
name|error
operator|=
name|vm_map_pageable
argument_list|(
name|kernel_map
argument_list|,
name|kva
argument_list|,
name|kva
operator|+
name|PAGE_SIZE
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* 			 * Now do the i/o move. 			 */
if|if
condition|(
operator|!
name|error
condition|)
name|error
operator|=
name|uiomove
argument_list|(
name|kva
operator|+
name|page_offset
argument_list|,
name|len
argument_list|,
name|uio
argument_list|)
expr_stmt|;
name|vm_map_remove
argument_list|(
name|kernel_map
argument_list|,
name|kva
argument_list|,
name|kva
operator|+
name|PAGE_SIZE
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|fix_prot
condition|)
name|vm_map_protect
argument_list|(
name|map
argument_list|,
name|pageno
argument_list|,
name|pageno
operator|+
name|PAGE_SIZE
argument_list|,
name|VM_PROT_READ
operator||
name|VM_PROT_EXECUTE
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
do|while
condition|(
name|error
operator|==
literal|0
operator|&&
name|uio
operator|->
name|uio_resid
operator|>
literal|0
condition|)
do|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Copy data in and out of the target process.  * We do this by mapping the process's page into  * the kernel and then doing a uiomove direct  * from the kernel address space.  */
end_comment

begin_function
name|int
name|procfs_domem
parameter_list|(
name|curp
parameter_list|,
name|p
parameter_list|,
name|pfs
parameter_list|,
name|uio
parameter_list|)
name|struct
name|proc
modifier|*
name|curp
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
name|struct
name|pfsnode
modifier|*
name|pfs
decl_stmt|;
name|struct
name|uio
modifier|*
name|uio
decl_stmt|;
block|{
name|int
name|error
decl_stmt|;
if|if
condition|(
name|uio
operator|->
name|uio_resid
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|error
operator|=
name|procfs_rwmem
argument_list|(
name|p
argument_list|,
name|uio
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Given process (p), find the vnode from which  * it's text segment is being executed.  *  * It would be nice to grab this information from  * the VM system, however, there is no sure-fire  * way of doing that.  Instead, fork(), exec() and  * wait() all maintain the p_textvp field in the  * process proc structure which contains a held  * reference to the exec'ed vnode.  */
end_comment

begin_function
name|struct
name|vnode
modifier|*
name|procfs_findtextvp
parameter_list|(
name|p
parameter_list|)
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
return|return
operator|(
name|p
operator|->
name|p_textvp
operator|)
return|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|probably_never
end_ifdef

begin_comment
comment|/*  * Given process (p), find the vnode from which  * it's text segment is being mapped.  *  * (This is here, rather than in procfs_subr in order  * to keep all the VM related code in one place.)  */
end_comment

begin_function
name|struct
name|vnode
modifier|*
name|procfs_findtextvp
parameter_list|(
name|p
parameter_list|)
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
name|int
name|error
decl_stmt|;
name|vm_object_t
name|object
decl_stmt|;
name|vm_offset_t
name|pageno
decl_stmt|;
comment|/* page number */
comment|/* find a vnode pager for the user address space */
for|for
control|(
name|pageno
operator|=
name|VM_MIN_ADDRESS
init|;
name|pageno
operator|<
name|VM_MAXUSER_ADDRESS
condition|;
name|pageno
operator|+=
name|PAGE_SIZE
control|)
block|{
name|vm_map_t
name|map
decl_stmt|;
name|vm_map_entry_t
name|out_entry
decl_stmt|;
name|vm_prot_t
name|out_prot
decl_stmt|;
name|boolean_t
name|wired
decl_stmt|,
name|single_use
decl_stmt|;
name|vm_offset_t
name|off
decl_stmt|;
name|map
operator|=
operator|&
name|p
operator|->
name|p_vmspace
operator|->
name|vm_map
expr_stmt|;
name|error
operator|=
name|vm_map_lookup
argument_list|(
operator|&
name|map
argument_list|,
name|pageno
argument_list|,
name|VM_PROT_READ
argument_list|,
operator|&
name|out_entry
argument_list|,
operator|&
name|object
argument_list|,
operator|&
name|off
argument_list|,
operator|&
name|out_prot
argument_list|,
operator|&
name|wired
argument_list|,
operator|&
name|single_use
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|error
condition|)
block|{
name|vm_pager_t
name|pager
decl_stmt|;
name|printf
argument_list|(
literal|"procfs: found vm object\n"
argument_list|)
expr_stmt|;
name|vm_map_lookup_done
argument_list|(
name|map
argument_list|,
name|out_entry
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"procfs: vm object = %x\n"
argument_list|,
name|object
argument_list|)
expr_stmt|;
comment|/* 			 * At this point, assuming no errors, object 			 * is the VM object mapping UVA (pageno). 			 * Ensure it has a vnode pager, then grab 			 * the vnode from that pager's handle. 			 */
name|pager
operator|=
name|object
operator|->
name|pager
expr_stmt|;
name|printf
argument_list|(
literal|"procfs: pager = %x\n"
argument_list|,
name|pager
argument_list|)
expr_stmt|;
if|if
condition|(
name|pager
condition|)
name|printf
argument_list|(
literal|"procfs: found pager, type = %d\n"
argument_list|,
name|pager
operator|->
name|pg_type
argument_list|)
expr_stmt|;
if|if
condition|(
name|pager
operator|&&
name|pager
operator|->
name|pg_type
operator|==
name|PG_VNODE
condition|)
block|{
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|vp
operator|=
operator|(
expr|struct
name|vnode
operator|*
operator|)
name|pager
operator|->
name|pg_handle
expr_stmt|;
name|printf
argument_list|(
literal|"procfs: vp = 0x%x\n"
argument_list|,
name|vp
argument_list|)
expr_stmt|;
return|return
operator|(
name|vp
operator|)
return|;
block|}
block|}
block|}
name|printf
argument_list|(
literal|"procfs: text object not found\n"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* probably_never */
end_comment

end_unit

