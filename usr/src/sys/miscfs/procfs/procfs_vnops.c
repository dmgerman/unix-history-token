begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1993 Jan-Simon Pendry  * Copyright (c) 1993  *	The Regents of the University of California.  All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Jan-Simon Pendry.  *  * %sccs.include.redist.c%  *  *	@(#)procfs_vnops.c	8.8 (Berkeley) %G%  *  * From:  *	$Id: procfs_vnops.c,v 3.2 1993/12/15 09:40:17 jsp Exp $  */
end_comment

begin_comment
comment|/*  * procfs vnode interface  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<sys/namei.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/dirent.h>
end_include

begin_include
include|#
directive|include
file|<sys/resourcevar.h>
end_include

begin_include
include|#
directive|include
file|<vm/vm.h>
end_include

begin_comment
comment|/* for PAGE_SIZE */
end_comment

begin_include
include|#
directive|include
file|<machine/reg.h>
end_include

begin_include
include|#
directive|include
file|<miscfs/procfs/procfs.h>
end_include

begin_comment
comment|/*  * Vnode Operations.  *  */
end_comment

begin_comment
comment|/*  * This is a list of the valid names in the  * process-specific sub-directories.  It is  * used in procfs_lookup and procfs_readdir  */
end_comment

begin_struct
specifier|static
struct|struct
name|pfsnames
block|{
name|u_char
name|d_type
decl_stmt|;
name|u_char
name|d_namlen
decl_stmt|;
name|char
name|d_name
index|[
name|PROCFS_NAMELEN
index|]
decl_stmt|;
name|pfstype
name|d_pfstype
decl_stmt|;
name|int
argument_list|(
argument|*d_valid
argument_list|)
name|__P
argument_list|(
operator|(
expr|struct
name|proc
operator|*
name|p
operator|)
argument_list|)
expr_stmt|;
block|}
name|procent
index|[]
init|=
block|{
define|#
directive|define
name|N
parameter_list|(
name|s
parameter_list|)
value|sizeof(s)-1, s
comment|/* namlen, nam, type */
block|{
name|DT_DIR
block|,
name|N
argument_list|(
literal|"."
argument_list|)
block|,
name|Pproc
block|,
name|NULL
block|}
block|,
block|{
name|DT_DIR
block|,
name|N
argument_list|(
literal|".."
argument_list|)
block|,
name|Proot
block|,
name|NULL
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"file"
argument_list|)
block|,
name|Pfile
block|,
name|procfs_validfile
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"mem"
argument_list|)
block|,
name|Pmem
block|,
name|NULL
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"regs"
argument_list|)
block|,
name|Pregs
block|,
name|procfs_validregs
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"fpregs"
argument_list|)
block|,
name|Pfpregs
block|,
name|procfs_validfpregs
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"ctl"
argument_list|)
block|,
name|Pctl
block|,
name|NULL
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"status"
argument_list|)
block|,
name|Pstatus
block|,
name|NULL
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"note"
argument_list|)
block|,
name|Pnote
block|,
name|NULL
block|}
block|,
block|{
name|DT_REG
block|,
name|N
argument_list|(
literal|"notepg"
argument_list|)
block|,
name|Pnotepg
block|,
name|NULL
block|}
block|,
undef|#
directive|undef
name|N
block|}
struct|;
end_struct

begin_define
define|#
directive|define
name|Nprocent
value|(sizeof(procent)/sizeof(procent[0]))
end_define

begin_decl_stmt
specifier|static
name|pid_t
name|atopid
name|__P
argument_list|(
operator|(
specifier|const
name|char
operator|*
operator|,
name|u_int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * set things up for doing i/o on  * the pfsnode (vp).  (vp) is locked  * on entry, and should be left locked  * on exit.  *  * for procfs we don't need to do anything  * in particular for i/o.  all that is done  * is to support exclusive open on process  * memory images.  */
end_comment

begin_macro
name|procfs_open
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_open_args
comment|/* { 		struct vnode *a_vp; 		int  a_mode; 		struct ucred *a_cred; 		struct proc *a_p; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|pfsnode
modifier|*
name|pfs
init|=
name|VTOPFS
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
decl_stmt|;
switch|switch
condition|(
name|pfs
operator|->
name|pfs_type
condition|)
block|{
case|case
name|Pmem
case|:
if|if
condition|(
name|PFIND
argument_list|(
name|pfs
operator|->
name|pfs_pid
argument_list|)
operator|==
literal|0
condition|)
return|return
operator|(
name|ENOENT
operator|)
return|;
comment|/* was ESRCH, jsp */
if|if
condition|(
operator|(
name|pfs
operator|->
name|pfs_flags
operator|&
name|FWRITE
operator|)
operator|&&
operator|(
name|ap
operator|->
name|a_mode
operator|&
name|O_EXCL
operator|)
operator|||
operator|(
name|pfs
operator|->
name|pfs_flags
operator|&
name|O_EXCL
operator|)
operator|&&
operator|(
name|ap
operator|->
name|a_mode
operator|&
name|FWRITE
operator|)
condition|)
return|return
operator|(
name|EBUSY
operator|)
return|;
if|if
condition|(
name|ap
operator|->
name|a_mode
operator|&
name|FWRITE
condition|)
name|pfs
operator|->
name|pfs_flags
operator|=
name|ap
operator|->
name|a_mode
operator|&
operator|(
name|FWRITE
operator||
name|O_EXCL
operator|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
default|default:
break|break;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * close the pfsnode (vp) after doing i/o.  * (vp) is not locked on entry or exit.  *  * nothing to do for procfs other than undo  * any exclusive open flag (see _open above).  */
end_comment

begin_macro
name|procfs_close
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_close_args
comment|/* { 		struct vnode *a_vp; 		int  a_fflag; 		struct ucred *a_cred; 		struct proc *a_p; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|pfsnode
modifier|*
name|pfs
init|=
name|VTOPFS
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
decl_stmt|;
switch|switch
condition|(
name|pfs
operator|->
name|pfs_type
condition|)
block|{
case|case
name|Pmem
case|:
if|if
condition|(
operator|(
name|ap
operator|->
name|a_fflag
operator|&
name|FWRITE
operator|)
operator|&&
operator|(
name|pfs
operator|->
name|pfs_flags
operator|&
name|O_EXCL
operator|)
condition|)
name|pfs
operator|->
name|pfs_flags
operator|&=
operator|~
operator|(
name|FWRITE
operator||
name|O_EXCL
operator|)
expr_stmt|;
break|break;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * do an ioctl operation on pfsnode (vp).  * (vp) is not locked on entry or exit.  */
end_comment

begin_macro
name|procfs_ioctl
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_ioctl_args
comment|/* { 		struct vnode *a_vp; 		int a_command; 		caddr_t a_data; 		int a_fflag; 		struct ucred *a_cred; 		struct proc *a_p; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
return|return
operator|(
name|ENOTTY
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * do block mapping for pfsnode (vp).  * since we don't use the buffer cache  * for procfs this function should never  * be called.  in any case, it's not clear  * what part of the kernel ever makes use  * of this function.  for sanity, this is the  * usual no-op bmap, although returning  * (EIO) would be a reasonable alternative.  */
end_comment

begin_macro
name|procfs_bmap
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_bmap_args
comment|/* { 		struct vnode *a_vp; 		daddr_t  a_bn; 		struct vnode **a_vpp; 		daddr_t *a_bnp; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|ap
operator|->
name|a_vpp
operator|!=
name|NULL
condition|)
operator|*
name|ap
operator|->
name|a_vpp
operator|=
name|ap
operator|->
name|a_vp
expr_stmt|;
if|if
condition|(
name|ap
operator|->
name|a_bnp
operator|!=
name|NULL
condition|)
operator|*
name|ap
operator|->
name|a_bnp
operator|=
name|ap
operator|->
name|a_bn
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * _inactive is called when the pfsnode  * is vrele'd and the reference count goes  * to zero.  (vp) will be on the vnode free  * list, so to get it back vget() must be  * used.  *  * for procfs, check if the process is still  * alive and if it isn't then just throw away  * the vnode by calling vgone().  this may  * be overkill and a waste of time since the  * chances are that the process will still be  * there and PFIND is not free.  *  * (vp) is not locked on entry or exit.  */
end_comment

begin_macro
name|procfs_inactive
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_inactive_args
comment|/* { 		struct vnode *a_vp; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|pfsnode
modifier|*
name|pfs
init|=
name|VTOPFS
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
decl_stmt|;
if|if
condition|(
name|PFIND
argument_list|(
name|pfs
operator|->
name|pfs_pid
argument_list|)
operator|==
literal|0
condition|)
name|vgone
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * _reclaim is called when getnewvnode()  * wants to make use of an entry on the vnode  * free list.  at this time the filesystem needs  * to free any private data and remove the node  * from any private lists.  */
end_comment

begin_macro
name|procfs_reclaim
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_reclaim_args
comment|/* { 		struct vnode *a_vp; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
return|return
operator|(
name|procfs_freevp
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Return POSIX pathconf information applicable to special devices.  */
end_comment

begin_macro
name|procfs_pathconf
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_pathconf_args
comment|/* { 		struct vnode *a_vp; 		int a_name; 		int *a_retval; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
switch|switch
condition|(
name|ap
operator|->
name|a_name
condition|)
block|{
case|case
name|_PC_LINK_MAX
case|:
operator|*
name|ap
operator|->
name|a_retval
operator|=
name|LINK_MAX
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|_PC_MAX_CANON
case|:
operator|*
name|ap
operator|->
name|a_retval
operator|=
name|MAX_CANON
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|_PC_MAX_INPUT
case|:
operator|*
name|ap
operator|->
name|a_retval
operator|=
name|MAX_INPUT
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|_PC_PIPE_BUF
case|:
operator|*
name|ap
operator|->
name|a_retval
operator|=
name|PIPE_BUF
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|_PC_CHOWN_RESTRICTED
case|:
operator|*
name|ap
operator|->
name|a_retval
operator|=
literal|1
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|_PC_VDISABLE
case|:
operator|*
name|ap
operator|->
name|a_retval
operator|=
name|_POSIX_VDISABLE
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
default|default:
return|return
operator|(
name|EINVAL
operator|)
return|;
block|}
comment|/* NOTREACHED */
block|}
end_block

begin_comment
comment|/*  * _print is used for debugging.  * just print a readable description  * of (vp).  */
end_comment

begin_macro
name|procfs_print
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_print_args
comment|/* { 		struct vnode *a_vp; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|pfsnode
modifier|*
name|pfs
init|=
name|VTOPFS
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
decl_stmt|;
name|printf
argument_list|(
literal|"tag VT_PROCFS, type %s, pid %d, mode %x, flags %x\n"
argument_list|,
name|pfs
operator|->
name|pfs_type
argument_list|,
name|pfs
operator|->
name|pfs_pid
argument_list|,
name|pfs
operator|->
name|pfs_mode
argument_list|,
name|pfs
operator|->
name|pfs_flags
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * _abortop is called when operations such as  * rename and create fail.  this entry is responsible  * for undoing any side-effects caused by the lookup.  * this will always include freeing the pathname buffer.  */
end_comment

begin_macro
name|procfs_abortop
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_abortop_args
comment|/* { 		struct vnode *a_dvp; 		struct componentname *a_cnp; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
operator|(
name|ap
operator|->
name|a_cnp
operator|->
name|cn_flags
operator|&
operator|(
name|HASBUF
operator||
name|SAVESTART
operator|)
operator|)
operator|==
name|HASBUF
condition|)
name|FREE
argument_list|(
name|ap
operator|->
name|a_cnp
operator|->
name|cn_pnbuf
argument_list|,
name|M_NAMEI
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * generic entry point for unsupported operations  */
end_comment

begin_macro
name|procfs_badop
argument_list|()
end_macro

begin_block
block|{
return|return
operator|(
name|EIO
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Invent attributes for pfsnode (vp) and store  * them in (vap).  * Directories lengths are returned as zero since  * any real length would require the genuine size  * to be computed, and nothing cares anyway.  *  * this is relatively minimal for procfs.  */
end_comment

begin_macro
name|procfs_getattr
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_getattr_args
comment|/* { 		struct vnode *a_vp; 		struct vattr *a_vap; 		struct ucred *a_cred; 		struct proc *a_p; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|pfsnode
modifier|*
name|pfs
init|=
name|VTOPFS
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
decl_stmt|;
name|struct
name|vattr
modifier|*
name|vap
init|=
name|ap
operator|->
name|a_vap
decl_stmt|;
name|struct
name|proc
modifier|*
name|procp
decl_stmt|;
name|int
name|error
decl_stmt|;
comment|/* first check the process still exists */
switch|switch
condition|(
name|pfs
operator|->
name|pfs_type
condition|)
block|{
case|case
name|Proot
case|:
case|case
name|Pcurproc
case|:
name|procp
operator|=
literal|0
expr_stmt|;
break|break;
default|default:
name|procp
operator|=
name|PFIND
argument_list|(
name|pfs
operator|->
name|pfs_pid
argument_list|)
expr_stmt|;
if|if
condition|(
name|procp
operator|==
literal|0
condition|)
return|return
operator|(
name|ENOENT
operator|)
return|;
block|}
name|error
operator|=
literal|0
expr_stmt|;
comment|/* start by zeroing out the attributes */
name|VATTR_NULL
argument_list|(
name|vap
argument_list|)
expr_stmt|;
comment|/* next do all the common fields */
name|vap
operator|->
name|va_type
operator|=
name|ap
operator|->
name|a_vp
operator|->
name|v_type
expr_stmt|;
name|vap
operator|->
name|va_mode
operator|=
name|pfs
operator|->
name|pfs_mode
expr_stmt|;
name|vap
operator|->
name|va_fileid
operator|=
name|pfs
operator|->
name|pfs_fileno
expr_stmt|;
name|vap
operator|->
name|va_flags
operator|=
literal|0
expr_stmt|;
name|vap
operator|->
name|va_blocksize
operator|=
name|PAGE_SIZE
expr_stmt|;
name|vap
operator|->
name|va_bytes
operator|=
name|vap
operator|->
name|va_size
operator|=
literal|0
expr_stmt|;
comment|/* 	 * Make all times be current TOD. 	 * It would be possible to get the process start 	 * time from the p_stat structure, but there's 	 * no "file creation" time stamp anyway, and the 	 * p_stat structure is not addressible if u. gets 	 * swapped out for that process. 	 * 	 * XXX 	 * Note that microtime() returns a timeval, not a timespec. 	 */
name|microtime
argument_list|(
operator|&
name|vap
operator|->
name|va_ctime
argument_list|)
expr_stmt|;
name|vap
operator|->
name|va_atime
operator|=
name|vap
operator|->
name|va_mtime
operator|=
name|vap
operator|->
name|va_ctime
expr_stmt|;
comment|/* 	 * If the process has exercised some setuid or setgid 	 * privilege, then rip away read/write permission so 	 * that only root can gain access. 	 */
switch|switch
condition|(
name|pfs
operator|->
name|pfs_type
condition|)
block|{
case|case
name|Pmem
case|:
case|case
name|Pregs
case|:
case|case
name|Pfpregs
case|:
if|if
condition|(
name|procp
operator|->
name|p_flag
operator|&
name|P_SUGID
condition|)
name|vap
operator|->
name|va_mode
operator|&=
operator|~
operator|(
operator|(
name|VREAD
operator||
name|VWRITE
operator|)
operator||
operator|(
operator|(
name|VREAD
operator||
name|VWRITE
operator|)
operator|>>
literal|3
operator|)
operator||
operator|(
operator|(
name|VREAD
operator||
name|VWRITE
operator|)
operator|>>
literal|6
operator|)
operator|)
expr_stmt|;
case|case
name|Pctl
case|:
case|case
name|Pstatus
case|:
case|case
name|Pnote
case|:
case|case
name|Pnotepg
case|:
name|vap
operator|->
name|va_nlink
operator|=
literal|1
expr_stmt|;
name|vap
operator|->
name|va_uid
operator|=
name|procp
operator|->
name|p_ucred
operator|->
name|cr_uid
expr_stmt|;
name|vap
operator|->
name|va_gid
operator|=
name|procp
operator|->
name|p_ucred
operator|->
name|cr_gid
expr_stmt|;
break|break;
block|}
comment|/* 	 * now do the object specific fields 	 * 	 * The size could be set from struct reg, but it's hardly 	 * worth the trouble, and it puts some (potentially) machine 	 * dependent data into this machine-independent code.  If it 	 * becomes important then this function should break out into 	 * a per-file stat function in the corresponding .c file. 	 */
switch|switch
condition|(
name|pfs
operator|->
name|pfs_type
condition|)
block|{
case|case
name|Proot
case|:
comment|/* 		 * Set nlink to 1 to tell fts(3) we don't actually know. 		 */
name|vap
operator|->
name|va_nlink
operator|=
literal|1
expr_stmt|;
name|vap
operator|->
name|va_uid
operator|=
literal|0
expr_stmt|;
name|vap
operator|->
name|va_gid
operator|=
literal|0
expr_stmt|;
name|vap
operator|->
name|va_size
operator|=
name|vap
operator|->
name|va_bytes
operator|=
name|DEV_BSIZE
expr_stmt|;
break|break;
case|case
name|Pcurproc
case|:
block|{
name|char
name|buf
index|[
literal|16
index|]
decl_stmt|;
comment|/* should be enough */
name|vap
operator|->
name|va_nlink
operator|=
literal|1
expr_stmt|;
name|vap
operator|->
name|va_uid
operator|=
literal|0
expr_stmt|;
name|vap
operator|->
name|va_gid
operator|=
literal|0
expr_stmt|;
name|vap
operator|->
name|va_size
operator|=
name|vap
operator|->
name|va_bytes
operator|=
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"%ld"
argument_list|,
operator|(
name|long
operator|)
name|curproc
operator|->
name|p_pid
argument_list|)
expr_stmt|;
break|break;
block|}
case|case
name|Pproc
case|:
name|vap
operator|->
name|va_nlink
operator|=
literal|2
expr_stmt|;
name|vap
operator|->
name|va_uid
operator|=
name|procp
operator|->
name|p_ucred
operator|->
name|cr_uid
expr_stmt|;
name|vap
operator|->
name|va_gid
operator|=
name|procp
operator|->
name|p_ucred
operator|->
name|cr_gid
expr_stmt|;
name|vap
operator|->
name|va_size
operator|=
name|vap
operator|->
name|va_bytes
operator|=
name|DEV_BSIZE
expr_stmt|;
break|break;
case|case
name|Pfile
case|:
name|error
operator|=
name|EOPNOTSUPP
expr_stmt|;
break|break;
case|case
name|Pmem
case|:
name|vap
operator|->
name|va_bytes
operator|=
name|vap
operator|->
name|va_size
operator|=
name|ctob
argument_list|(
name|procp
operator|->
name|p_vmspace
operator|->
name|vm_tsize
operator|+
name|procp
operator|->
name|p_vmspace
operator|->
name|vm_dsize
operator|+
name|procp
operator|->
name|p_vmspace
operator|->
name|vm_ssize
argument_list|)
expr_stmt|;
break|break;
case|case
name|Pregs
case|:
name|vap
operator|->
name|va_bytes
operator|=
name|vap
operator|->
name|va_size
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|reg
argument_list|)
expr_stmt|;
break|break;
case|case
name|Pfpregs
case|:
name|vap
operator|->
name|va_bytes
operator|=
name|vap
operator|->
name|va_size
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|fpreg
argument_list|)
expr_stmt|;
break|break;
case|case
name|Pctl
case|:
case|case
name|Pstatus
case|:
case|case
name|Pnote
case|:
case|case
name|Pnotepg
case|:
break|break;
default|default:
name|panic
argument_list|(
literal|"procfs_getattr"
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_macro
name|procfs_setattr
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_setattr_args
comment|/* { 		struct vnode *a_vp; 		struct vattr *a_vap; 		struct ucred *a_cred; 		struct proc *a_p; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
comment|/* 	 * just fake out attribute setting 	 * it's not good to generate an error 	 * return, otherwise things like creat() 	 * will fail when they try to set the 	 * file length to 0.  worse, this means 	 * that echo $note> /proc/$pid/note will fail. 	 */
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * implement access checking.  *  * something very similar to this code is duplicated  * throughout the 4bsd kernel and should be moved  * into kern/vfs_subr.c sometime.  *  * actually, the check for super-user is slightly  * broken since it will allow read access to write-only  * objects.  this doesn't cause any particular trouble  * but does mean that the i/o entry points need to check  * that the operation really does make sense.  */
end_comment

begin_macro
name|procfs_access
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_access_args
comment|/* { 		struct vnode *a_vp; 		int a_mode; 		struct ucred *a_cred; 		struct proc *a_p; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|vattr
modifier|*
name|vap
decl_stmt|;
name|struct
name|vattr
name|vattr
decl_stmt|;
name|int
name|error
decl_stmt|;
comment|/* 	 * If you're the super-user, 	 * you always get access. 	 */
if|if
condition|(
name|ap
operator|->
name|a_cred
operator|->
name|cr_uid
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|vap
operator|=
operator|&
name|vattr
expr_stmt|;
if|if
condition|(
name|error
operator|=
name|VOP_GETATTR
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|,
name|vap
argument_list|,
name|ap
operator|->
name|a_cred
argument_list|,
name|ap
operator|->
name|a_p
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
comment|/* 	 * Access check is based on only one of owner, group, public. 	 * If not owner, then check group. If not a member of the 	 * group, then check public access. 	 */
if|if
condition|(
name|ap
operator|->
name|a_cred
operator|->
name|cr_uid
operator|!=
name|vap
operator|->
name|va_uid
condition|)
block|{
name|gid_t
modifier|*
name|gp
decl_stmt|;
name|int
name|i
decl_stmt|;
name|ap
operator|->
name|a_mode
operator|>>=
literal|3
expr_stmt|;
name|gp
operator|=
name|ap
operator|->
name|a_cred
operator|->
name|cr_groups
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|ap
operator|->
name|a_cred
operator|->
name|cr_ngroups
condition|;
name|i
operator|++
operator|,
name|gp
operator|++
control|)
if|if
condition|(
name|vap
operator|->
name|va_gid
operator|==
operator|*
name|gp
condition|)
goto|goto
name|found
goto|;
name|ap
operator|->
name|a_mode
operator|>>=
literal|3
expr_stmt|;
name|found
label|:
empty_stmt|;
block|}
if|if
condition|(
operator|(
name|vap
operator|->
name|va_mode
operator|&
name|ap
operator|->
name|a_mode
operator|)
operator|==
name|ap
operator|->
name|a_mode
condition|)
return|return
operator|(
literal|0
operator|)
return|;
return|return
operator|(
name|EACCES
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * lookup.  this is incredibly complicated in the  * general case, however for most pseudo-filesystems  * very little needs to be done.  *  * unless you want to get a migraine, just make sure your  * filesystem doesn't do any locking of its own.  otherwise  * read and inwardly digest ufs_lookup().  */
end_comment

begin_macro
name|procfs_lookup
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_lookup_args
comment|/* { 		struct vnode * a_dvp; 		struct vnode ** a_vpp; 		struct componentname * a_cnp; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|componentname
modifier|*
name|cnp
init|=
name|ap
operator|->
name|a_cnp
decl_stmt|;
name|struct
name|vnode
modifier|*
modifier|*
name|vpp
init|=
name|ap
operator|->
name|a_vpp
decl_stmt|;
name|struct
name|vnode
modifier|*
name|dvp
init|=
name|ap
operator|->
name|a_dvp
decl_stmt|;
name|char
modifier|*
name|pname
init|=
name|cnp
operator|->
name|cn_nameptr
decl_stmt|;
name|int
name|error
init|=
literal|0
decl_stmt|;
name|pid_t
name|pid
decl_stmt|;
name|struct
name|vnode
modifier|*
name|nvp
decl_stmt|;
name|struct
name|pfsnode
modifier|*
name|pfs
decl_stmt|;
name|struct
name|proc
modifier|*
name|procp
decl_stmt|;
name|pfstype
name|pfs_type
decl_stmt|;
name|int
name|i
decl_stmt|;
if|if
condition|(
name|cnp
operator|->
name|cn_namelen
operator|==
literal|1
operator|&&
operator|*
name|pname
operator|==
literal|'.'
condition|)
block|{
operator|*
name|vpp
operator|=
name|dvp
expr_stmt|;
name|VREF
argument_list|(
name|dvp
argument_list|)
expr_stmt|;
comment|/*VOP_LOCK(dvp);*/
return|return
operator|(
literal|0
operator|)
return|;
block|}
operator|*
name|vpp
operator|=
name|NULL
expr_stmt|;
name|pfs
operator|=
name|VTOPFS
argument_list|(
name|dvp
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|pfs
operator|->
name|pfs_type
condition|)
block|{
case|case
name|Proot
case|:
if|if
condition|(
name|cnp
operator|->
name|cn_flags
operator|&
name|ISDOTDOT
condition|)
return|return
operator|(
name|EIO
operator|)
return|;
if|if
condition|(
name|CNEQ
argument_list|(
name|cnp
argument_list|,
literal|"curproc"
argument_list|,
literal|7
argument_list|)
condition|)
return|return
operator|(
name|procfs_allocvp
argument_list|(
name|dvp
operator|->
name|v_mount
argument_list|,
name|vpp
argument_list|,
literal|0
argument_list|,
name|Pcurproc
argument_list|)
operator|)
return|;
name|pid
operator|=
name|atopid
argument_list|(
name|pname
argument_list|,
name|cnp
operator|->
name|cn_namelen
argument_list|)
expr_stmt|;
if|if
condition|(
name|pid
operator|==
name|NO_PID
condition|)
return|return
operator|(
name|ENOENT
operator|)
return|;
name|procp
operator|=
name|PFIND
argument_list|(
name|pid
argument_list|)
expr_stmt|;
if|if
condition|(
name|procp
operator|==
literal|0
condition|)
return|return
operator|(
name|ENOENT
operator|)
return|;
return|return
operator|(
name|procfs_allocvp
argument_list|(
name|dvp
operator|->
name|v_mount
argument_list|,
name|vpp
argument_list|,
name|pid
argument_list|,
name|Pproc
argument_list|)
operator|)
return|;
case|case
name|Pproc
case|:
if|if
condition|(
name|cnp
operator|->
name|cn_flags
operator|&
name|ISDOTDOT
condition|)
block|{
name|error
operator|=
name|procfs_root
argument_list|(
name|dvp
operator|->
name|v_mount
argument_list|,
name|vpp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|procp
operator|=
name|PFIND
argument_list|(
name|pfs
operator|->
name|pfs_pid
argument_list|)
expr_stmt|;
if|if
condition|(
name|procp
operator|==
literal|0
condition|)
return|return
operator|(
name|ENOENT
operator|)
return|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|Nprocent
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|pfsnames
modifier|*
name|dp
init|=
operator|&
name|procent
index|[
name|i
index|]
decl_stmt|;
if|if
condition|(
name|cnp
operator|->
name|cn_namelen
operator|==
name|dp
operator|->
name|d_namlen
operator|&&
name|bcmp
argument_list|(
name|pname
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
name|dp
operator|->
name|d_namlen
argument_list|)
operator|==
literal|0
operator|&&
operator|(
name|dp
operator|->
name|d_valid
operator|==
name|NULL
operator|||
call|(
modifier|*
name|dp
operator|->
name|d_valid
call|)
argument_list|(
name|procp
argument_list|)
operator|)
condition|)
block|{
name|pfs_type
operator|=
name|dp
operator|->
name|d_pfstype
expr_stmt|;
goto|goto
name|found
goto|;
block|}
block|}
return|return
operator|(
name|ENOENT
operator|)
return|;
name|found
label|:
if|if
condition|(
name|pfs_type
operator|==
name|Pfile
condition|)
block|{
name|nvp
operator|=
name|procfs_findtextvp
argument_list|(
name|procp
argument_list|)
expr_stmt|;
if|if
condition|(
name|nvp
operator|==
name|NULLVP
condition|)
return|return
operator|(
name|ENXIO
operator|)
return|;
name|VREF
argument_list|(
name|nvp
argument_list|)
expr_stmt|;
name|VOP_LOCK
argument_list|(
name|nvp
argument_list|)
expr_stmt|;
operator|*
name|vpp
operator|=
name|nvp
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
return|return
operator|(
name|procfs_allocvp
argument_list|(
name|dvp
operator|->
name|v_mount
argument_list|,
name|vpp
argument_list|,
name|pfs
operator|->
name|pfs_pid
argument_list|,
name|pfs_type
argument_list|)
operator|)
return|;
default|default:
return|return
operator|(
name|ENOTDIR
operator|)
return|;
block|}
block|}
end_block

begin_function
name|int
name|procfs_validfile
parameter_list|(
name|p
parameter_list|)
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
return|return
operator|(
name|procfs_findtextvp
argument_list|(
name|p
argument_list|)
operator|!=
name|NULLVP
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * readdir returns directory entries from pfsnode (vp).  *  * the strategy here with procfs is to generate a single  * directory entry at a time (struct pfsdent) and then  * copy that out to userland using uiomove.  a more efficent  * though more complex implementation, would try to minimize  * the number of calls to uiomove().  for procfs, this is  * hardly worth the added code complexity.  *  * this should just be done through read()  */
end_comment

begin_macro
name|procfs_readdir
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_readdir_args
comment|/* { 		struct vnode *a_vp; 		struct uio *a_uio; 		struct ucred *a_cred; 		int *a_eofflag; 		u_long *a_cookies; 		int a_ncookies; 	} */
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|uio
modifier|*
name|uio
init|=
name|ap
operator|->
name|a_uio
decl_stmt|;
name|struct
name|pfsdent
name|d
decl_stmt|;
name|struct
name|pfsdent
modifier|*
name|dp
init|=
operator|&
name|d
decl_stmt|;
name|struct
name|pfsnode
modifier|*
name|pfs
decl_stmt|;
name|int
name|error
decl_stmt|;
name|int
name|count
decl_stmt|;
name|int
name|i
decl_stmt|;
comment|/* 	 * We don't allow exporting procfs mounts, and currently local 	 * requests do not need cookies. 	 */
if|if
condition|(
name|ap
operator|->
name|a_ncookies
condition|)
name|panic
argument_list|(
literal|"procfs_readdir: not hungry"
argument_list|)
expr_stmt|;
name|pfs
operator|=
name|VTOPFS
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
expr_stmt|;
if|if
condition|(
name|uio
operator|->
name|uio_resid
operator|<
name|UIO_MX
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
if|if
condition|(
name|uio
operator|->
name|uio_offset
operator|&
operator|(
name|UIO_MX
operator|-
literal|1
operator|)
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
if|if
condition|(
name|uio
operator|->
name|uio_offset
operator|<
literal|0
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|error
operator|=
literal|0
expr_stmt|;
name|count
operator|=
literal|0
expr_stmt|;
name|i
operator|=
name|uio
operator|->
name|uio_offset
operator|/
name|UIO_MX
expr_stmt|;
switch|switch
condition|(
name|pfs
operator|->
name|pfs_type
condition|)
block|{
comment|/* 	 * this is for the process-specific sub-directories. 	 * all that is needed to is copy out all the entries 	 * from the procent[] table (top of this file). 	 */
case|case
name|Pproc
case|:
block|{
name|pid_t
name|pid
init|=
name|pfs
operator|->
name|pfs_pid
decl_stmt|;
name|struct
name|pfsnames
modifier|*
name|dt
decl_stmt|;
for|for
control|(
name|dt
operator|=
operator|&
name|procent
index|[
name|i
index|]
init|;
name|i
operator|<
name|Nprocent
operator|&&
name|uio
operator|->
name|uio_resid
operator|>=
name|UIO_MX
condition|;
name|dt
operator|++
operator|,
name|i
operator|++
control|)
block|{
name|struct
name|proc
modifier|*
name|p
init|=
name|PFIND
argument_list|(
name|pid
argument_list|)
decl_stmt|;
if|if
condition|(
name|p
operator|==
name|NULL
condition|)
break|break;
if|if
condition|(
name|dt
operator|->
name|d_valid
operator|&&
call|(
modifier|*
name|dt
operator|->
name|d_valid
call|)
argument_list|(
name|p
argument_list|)
operator|==
literal|0
condition|)
continue|continue;
name|dp
operator|->
name|d_reclen
operator|=
name|UIO_MX
expr_stmt|;
name|dp
operator|->
name|d_fileno
operator|=
name|PROCFS_FILENO
argument_list|(
name|pid
argument_list|,
name|dt
operator|->
name|d_pfstype
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_namlen
operator|=
name|dt
operator|->
name|d_namlen
expr_stmt|;
name|bcopy
argument_list|(
name|dt
operator|->
name|d_name
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
name|dt
operator|->
name|d_namlen
operator|+
literal|1
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_type
operator|=
name|dt
operator|->
name|d_type
expr_stmt|;
if|if
condition|(
name|error
operator|=
name|uiomove
argument_list|(
operator|(
name|caddr_t
operator|)
name|dp
argument_list|,
name|UIO_MX
argument_list|,
name|uio
argument_list|)
condition|)
break|break;
block|}
break|break;
block|}
comment|/* 	 * this is for the root of the procfs filesystem 	 * what is needed is a special entry for "curproc" 	 * followed by an entry for each process on allproc #ifdef PROCFS_ZOMBIE 	 * and zombproc. #endif 	 */
case|case
name|Proot
case|:
block|{
ifdef|#
directive|ifdef
name|PROCFS_ZOMBIE
name|int
name|doingzomb
init|=
literal|0
decl_stmt|;
endif|#
directive|endif
name|int
name|pcnt
init|=
literal|0
decl_stmt|;
specifier|volatile
name|struct
name|proc
modifier|*
name|p
init|=
name|allproc
decl_stmt|;
name|again
label|:
for|for
control|(
init|;
name|p
operator|&&
name|uio
operator|->
name|uio_resid
operator|>=
name|UIO_MX
condition|;
name|i
operator|++
operator|,
name|pcnt
operator|++
control|)
block|{
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|dp
argument_list|,
name|UIO_MX
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_reclen
operator|=
name|UIO_MX
expr_stmt|;
switch|switch
condition|(
name|i
condition|)
block|{
case|case
literal|0
case|:
comment|/* `.' */
case|case
literal|1
case|:
comment|/* `..' */
name|dp
operator|->
name|d_fileno
operator|=
name|PROCFS_FILENO
argument_list|(
literal|0
argument_list|,
name|Proot
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_namlen
operator|=
name|i
operator|+
literal|1
expr_stmt|;
name|bcopy
argument_list|(
literal|".."
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
name|dp
operator|->
name|d_namlen
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_name
index|[
name|i
operator|+
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
name|dp
operator|->
name|d_type
operator|=
name|DT_DIR
expr_stmt|;
break|break;
case|case
literal|2
case|:
name|dp
operator|->
name|d_fileno
operator|=
name|PROCFS_FILENO
argument_list|(
literal|0
argument_list|,
name|Pcurproc
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_namlen
operator|=
literal|7
expr_stmt|;
name|bcopy
argument_list|(
literal|"curproc"
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
literal|8
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_type
operator|=
name|DT_LNK
expr_stmt|;
break|break;
default|default:
while|while
condition|(
name|pcnt
operator|<
name|i
condition|)
block|{
name|pcnt
operator|++
expr_stmt|;
name|p
operator|=
name|p
operator|->
name|p_next
expr_stmt|;
if|if
condition|(
operator|!
name|p
condition|)
goto|goto
name|done
goto|;
block|}
name|dp
operator|->
name|d_fileno
operator|=
name|PROCFS_FILENO
argument_list|(
name|p
operator|->
name|p_pid
argument_list|,
name|Pproc
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_namlen
operator|=
name|sprintf
argument_list|(
name|dp
operator|->
name|d_name
argument_list|,
literal|"%ld"
argument_list|,
operator|(
name|long
operator|)
name|p
operator|->
name|p_pid
argument_list|)
expr_stmt|;
name|dp
operator|->
name|d_type
operator|=
name|DT_REG
expr_stmt|;
name|p
operator|=
name|p
operator|->
name|p_next
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|error
operator|=
name|uiomove
argument_list|(
operator|(
name|caddr_t
operator|)
name|dp
argument_list|,
name|UIO_MX
argument_list|,
name|uio
argument_list|)
condition|)
break|break;
block|}
name|done
label|:
ifdef|#
directive|ifdef
name|PROCFS_ZOMBIE
if|if
condition|(
name|p
operator|==
literal|0
operator|&&
name|doingzomb
operator|==
literal|0
condition|)
block|{
name|doingzomb
operator|=
literal|1
expr_stmt|;
name|p
operator|=
name|zombproc
expr_stmt|;
goto|goto
name|again
goto|;
block|}
endif|#
directive|endif
break|break;
block|}
default|default:
name|error
operator|=
name|ENOTDIR
expr_stmt|;
break|break;
block|}
name|uio
operator|->
name|uio_offset
operator|=
name|i
operator|*
name|UIO_MX
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * readlink reads the link of `curproc'  */
end_comment

begin_macro
name|procfs_readlink
argument_list|(
argument|ap
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vop_readlink_args
modifier|*
name|ap
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|uio
modifier|*
name|uio
init|=
name|ap
operator|->
name|a_uio
decl_stmt|;
name|char
name|buf
index|[
literal|16
index|]
decl_stmt|;
comment|/* should be enough */
name|int
name|len
decl_stmt|;
if|if
condition|(
name|VTOPFS
argument_list|(
name|ap
operator|->
name|a_vp
argument_list|)
operator|->
name|pfs_fileno
operator|!=
name|PROCFS_FILENO
argument_list|(
literal|0
argument_list|,
name|Pcurproc
argument_list|)
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|len
operator|=
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"%ld"
argument_list|,
operator|(
name|long
operator|)
name|curproc
operator|->
name|p_pid
argument_list|)
expr_stmt|;
return|return
operator|(
name|uiomove
argument_list|(
operator|(
name|caddr_t
operator|)
name|buf
argument_list|,
name|len
argument_list|,
name|ap
operator|->
name|a_uio
argument_list|)
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * convert decimal ascii to pid_t  */
end_comment

begin_function
specifier|static
name|pid_t
name|atopid
parameter_list|(
name|b
parameter_list|,
name|len
parameter_list|)
specifier|const
name|char
modifier|*
name|b
decl_stmt|;
name|u_int
name|len
decl_stmt|;
block|{
name|pid_t
name|p
init|=
literal|0
decl_stmt|;
while|while
condition|(
name|len
operator|--
condition|)
block|{
name|char
name|c
init|=
operator|*
name|b
operator|++
decl_stmt|;
if|if
condition|(
name|c
operator|<
literal|'0'
operator|||
name|c
operator|>
literal|'9'
condition|)
return|return
operator|(
name|NO_PID
operator|)
return|;
name|p
operator|=
literal|10
operator|*
name|p
operator|+
operator|(
name|c
operator|-
literal|'0'
operator|)
expr_stmt|;
if|if
condition|(
name|p
operator|>
name|PID_MAX
condition|)
return|return
operator|(
name|NO_PID
operator|)
return|;
block|}
return|return
operator|(
name|p
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * procfs vnode operations.  */
end_comment

begin_function_decl
name|int
function_decl|(
modifier|*
modifier|*
name|procfs_vnodeop_p
function_decl|)
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|struct
name|vnodeopv_entry_desc
name|procfs_vnodeop_entries
index|[]
init|=
block|{
block|{
operator|&
name|vop_default_desc
block|,
name|vn_default_error
block|}
block|,
block|{
operator|&
name|vop_lookup_desc
block|,
name|procfs_lookup
block|}
block|,
comment|/* lookup */
block|{
operator|&
name|vop_create_desc
block|,
name|procfs_create
block|}
block|,
comment|/* create */
block|{
operator|&
name|vop_mknod_desc
block|,
name|procfs_mknod
block|}
block|,
comment|/* mknod */
block|{
operator|&
name|vop_open_desc
block|,
name|procfs_open
block|}
block|,
comment|/* open */
block|{
operator|&
name|vop_close_desc
block|,
name|procfs_close
block|}
block|,
comment|/* close */
block|{
operator|&
name|vop_access_desc
block|,
name|procfs_access
block|}
block|,
comment|/* access */
block|{
operator|&
name|vop_getattr_desc
block|,
name|procfs_getattr
block|}
block|,
comment|/* getattr */
block|{
operator|&
name|vop_setattr_desc
block|,
name|procfs_setattr
block|}
block|,
comment|/* setattr */
block|{
operator|&
name|vop_read_desc
block|,
name|procfs_read
block|}
block|,
comment|/* read */
block|{
operator|&
name|vop_write_desc
block|,
name|procfs_write
block|}
block|,
comment|/* write */
block|{
operator|&
name|vop_ioctl_desc
block|,
name|procfs_ioctl
block|}
block|,
comment|/* ioctl */
block|{
operator|&
name|vop_select_desc
block|,
name|procfs_select
block|}
block|,
comment|/* select */
block|{
operator|&
name|vop_mmap_desc
block|,
name|procfs_mmap
block|}
block|,
comment|/* mmap */
block|{
operator|&
name|vop_fsync_desc
block|,
name|procfs_fsync
block|}
block|,
comment|/* fsync */
block|{
operator|&
name|vop_seek_desc
block|,
name|procfs_seek
block|}
block|,
comment|/* seek */
block|{
operator|&
name|vop_remove_desc
block|,
name|procfs_remove
block|}
block|,
comment|/* remove */
block|{
operator|&
name|vop_link_desc
block|,
name|procfs_link
block|}
block|,
comment|/* link */
block|{
operator|&
name|vop_rename_desc
block|,
name|procfs_rename
block|}
block|,
comment|/* rename */
block|{
operator|&
name|vop_mkdir_desc
block|,
name|procfs_mkdir
block|}
block|,
comment|/* mkdir */
block|{
operator|&
name|vop_rmdir_desc
block|,
name|procfs_rmdir
block|}
block|,
comment|/* rmdir */
block|{
operator|&
name|vop_symlink_desc
block|,
name|procfs_symlink
block|}
block|,
comment|/* symlink */
block|{
operator|&
name|vop_readdir_desc
block|,
name|procfs_readdir
block|}
block|,
comment|/* readdir */
block|{
operator|&
name|vop_readlink_desc
block|,
name|procfs_readlink
block|}
block|,
comment|/* readlink */
block|{
operator|&
name|vop_abortop_desc
block|,
name|procfs_abortop
block|}
block|,
comment|/* abortop */
block|{
operator|&
name|vop_inactive_desc
block|,
name|procfs_inactive
block|}
block|,
comment|/* inactive */
block|{
operator|&
name|vop_reclaim_desc
block|,
name|procfs_reclaim
block|}
block|,
comment|/* reclaim */
block|{
operator|&
name|vop_lock_desc
block|,
name|procfs_lock
block|}
block|,
comment|/* lock */
block|{
operator|&
name|vop_unlock_desc
block|,
name|procfs_unlock
block|}
block|,
comment|/* unlock */
block|{
operator|&
name|vop_bmap_desc
block|,
name|procfs_bmap
block|}
block|,
comment|/* bmap */
block|{
operator|&
name|vop_strategy_desc
block|,
name|procfs_strategy
block|}
block|,
comment|/* strategy */
block|{
operator|&
name|vop_print_desc
block|,
name|procfs_print
block|}
block|,
comment|/* print */
block|{
operator|&
name|vop_islocked_desc
block|,
name|procfs_islocked
block|}
block|,
comment|/* islocked */
block|{
operator|&
name|vop_pathconf_desc
block|,
name|procfs_pathconf
block|}
block|,
comment|/* pathconf */
block|{
operator|&
name|vop_advlock_desc
block|,
name|procfs_advlock
block|}
block|,
comment|/* advlock */
block|{
operator|&
name|vop_blkatoff_desc
block|,
name|procfs_blkatoff
block|}
block|,
comment|/* blkatoff */
block|{
operator|&
name|vop_valloc_desc
block|,
name|procfs_valloc
block|}
block|,
comment|/* valloc */
block|{
operator|&
name|vop_vfree_desc
block|,
name|procfs_vfree
block|}
block|,
comment|/* vfree */
block|{
operator|&
name|vop_truncate_desc
block|,
name|procfs_truncate
block|}
block|,
comment|/* truncate */
block|{
operator|&
name|vop_update_desc
block|,
name|procfs_update
block|}
block|,
comment|/* update */
block|{
operator|(
expr|struct
name|vnodeop_desc
operator|*
operator|)
name|NULL
block|,
operator|(
name|int
argument_list|(
operator|*
argument_list|)
argument_list|()
operator|)
name|NULL
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|vnodeopv_desc
name|procfs_vnodeop_opv_desc
init|=
block|{
operator|&
name|procfs_vnodeop_p
block|,
name|procfs_vnodeop_entries
block|}
decl_stmt|;
end_decl_stmt

end_unit

