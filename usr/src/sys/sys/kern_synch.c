begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982, 1986 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)kern_synch.c	7.1 (Berkeley) 6/5/86  */
end_comment

begin_include
include|#
directive|include
file|"../machine/pte.h"
end_include

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"systm.h"
end_include

begin_include
include|#
directive|include
file|"dir.h"
end_include

begin_include
include|#
directive|include
file|"user.h"
end_include

begin_include
include|#
directive|include
file|"proc.h"
end_include

begin_include
include|#
directive|include
file|"file.h"
end_include

begin_include
include|#
directive|include
file|"inode.h"
end_include

begin_include
include|#
directive|include
file|"vm.h"
end_include

begin_include
include|#
directive|include
file|"kernel.h"
end_include

begin_include
include|#
directive|include
file|"buf.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|vax
end_ifdef

begin_include
include|#
directive|include
file|"../vax/mtpr.h"
end_include

begin_comment
comment|/* XXX */
end_comment

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Force switch among equal priority processes every 100ms.  */
end_comment

begin_macro
name|roundrobin
argument_list|()
end_macro

begin_block
block|{
name|runrun
operator|++
expr_stmt|;
name|aston
argument_list|()
expr_stmt|;
name|timeout
argument_list|(
name|roundrobin
argument_list|,
operator|(
name|caddr_t
operator|)
literal|0
argument_list|,
name|hz
operator|/
literal|10
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/* fraction for digital decay to forget 90% of usage in 5*loadav sec */
end_comment

begin_define
define|#
directive|define
name|filter
parameter_list|(
name|loadav
parameter_list|)
value|((2 * (loadav)) / (2 * (loadav) + 1))
end_define

begin_decl_stmt
name|double
name|ccpu
init|=
literal|0.95122942450071400909
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* exp(-1/20) */
end_comment

begin_comment
comment|/*  * Recompute process priorities, once a second  */
end_comment

begin_macro
name|schedcpu
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|double
name|ccpu1
init|=
operator|(
literal|1.0
operator|-
name|ccpu
operator|)
operator|/
operator|(
name|double
operator|)
name|hz
decl_stmt|;
specifier|register
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
specifier|register
name|int
name|s
decl_stmt|,
name|a
decl_stmt|;
name|float
name|scale
init|=
name|filter
argument_list|(
name|avenrun
index|[
literal|0
index|]
argument_list|)
decl_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|lbolt
argument_list|)
expr_stmt|;
for|for
control|(
name|p
operator|=
name|allproc
init|;
name|p
operator|!=
name|NULL
condition|;
name|p
operator|=
name|p
operator|->
name|p_nxt
control|)
block|{
if|if
condition|(
name|p
operator|->
name|p_time
operator|!=
literal|127
condition|)
name|p
operator|->
name|p_time
operator|++
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_stat
operator|==
name|SSLEEP
operator|||
name|p
operator|->
name|p_stat
operator|==
name|SSTOP
condition|)
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|!=
literal|127
condition|)
name|p
operator|->
name|p_slptime
operator|++
expr_stmt|;
comment|/* 		 * If the process has slept the entire second, 		 * stop recalculating its priority until it wakes up. 		 */
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|>
literal|1
condition|)
block|{
name|p
operator|->
name|p_pctcpu
operator|*=
name|ccpu
expr_stmt|;
continue|continue;
block|}
comment|/* 		 * p_pctcpu is only for ps. 		 */
name|p
operator|->
name|p_pctcpu
operator|=
name|ccpu
operator|*
name|p
operator|->
name|p_pctcpu
operator|+
name|ccpu1
operator|*
name|p
operator|->
name|p_cpticks
expr_stmt|;
name|p
operator|->
name|p_cpticks
operator|=
literal|0
expr_stmt|;
name|a
operator|=
call|(
name|int
call|)
argument_list|(
name|scale
operator|*
operator|(
name|p
operator|->
name|p_cpu
operator|&
literal|0377
operator|)
argument_list|)
operator|+
name|p
operator|->
name|p_nice
expr_stmt|;
if|if
condition|(
name|a
operator|<
literal|0
condition|)
name|a
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|a
operator|>
literal|255
condition|)
name|a
operator|=
literal|255
expr_stmt|;
name|p
operator|->
name|p_cpu
operator|=
name|a
expr_stmt|;
operator|(
name|void
operator|)
name|setpri
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|s
operator|=
name|splhigh
argument_list|()
expr_stmt|;
comment|/* prevent state changes */
if|if
condition|(
name|p
operator|->
name|p_pri
operator|>=
name|PUSER
condition|)
block|{
define|#
directive|define
name|PPQ
value|(128 / NQS)
if|if
condition|(
operator|(
name|p
operator|!=
name|u
operator|.
name|u_procp
operator|||
name|noproc
operator|)
operator|&&
name|p
operator|->
name|p_stat
operator|==
name|SRUN
operator|&&
operator|(
name|p
operator|->
name|p_flag
operator|&
name|SLOAD
operator|)
operator|&&
operator|(
name|p
operator|->
name|p_pri
operator|/
name|PPQ
operator|)
operator|!=
operator|(
name|p
operator|->
name|p_usrpri
operator|/
name|PPQ
operator|)
condition|)
block|{
name|remrq
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|p
operator|->
name|p_pri
operator|=
name|p
operator|->
name|p_usrpri
expr_stmt|;
name|setrq
argument_list|(
name|p
argument_list|)
expr_stmt|;
block|}
else|else
name|p
operator|->
name|p_pri
operator|=
name|p
operator|->
name|p_usrpri
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
name|vmmeter
argument_list|()
expr_stmt|;
if|if
condition|(
name|runin
operator|!=
literal|0
condition|)
block|{
name|runin
operator|=
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|runin
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|bclnlist
operator|!=
name|NULL
condition|)
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|proc
index|[
literal|2
index|]
argument_list|)
expr_stmt|;
name|timeout
argument_list|(
name|schedcpu
argument_list|,
operator|(
name|caddr_t
operator|)
literal|0
argument_list|,
name|hz
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Recalculate the priority of a process after it has slept for a while.  */
end_comment

begin_expr_stmt
name|updatepri
argument_list|(
name|p
argument_list|)
specifier|register
expr|struct
name|proc
operator|*
name|p
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|int
name|a
init|=
name|p
operator|->
name|p_cpu
operator|&
literal|0377
decl_stmt|;
name|float
name|scale
init|=
name|filter
argument_list|(
name|avenrun
index|[
literal|0
index|]
argument_list|)
decl_stmt|;
name|p
operator|->
name|p_slptime
operator|--
expr_stmt|;
comment|/* the first time was done in schedcpu */
while|while
condition|(
name|a
operator|&&
operator|--
name|p
operator|->
name|p_slptime
condition|)
name|a
operator|=
call|(
name|int
call|)
argument_list|(
name|scale
operator|*
name|a
argument_list|)
comment|/* + p->p_nice */
expr_stmt|;
if|if
condition|(
name|a
operator|<
literal|0
condition|)
name|a
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|a
operator|>
literal|255
condition|)
name|a
operator|=
literal|255
expr_stmt|;
name|p
operator|->
name|p_cpu
operator|=
name|a
expr_stmt|;
operator|(
name|void
operator|)
name|setpri
argument_list|(
name|p
argument_list|)
expr_stmt|;
block|}
end_block

begin_define
define|#
directive|define
name|SQSIZE
value|0100
end_define

begin_comment
comment|/* Must be power of 2 */
end_comment

begin_define
define|#
directive|define
name|HASH
parameter_list|(
name|x
parameter_list|)
value|(( (int) x>> 5)& (SQSIZE-1))
end_define

begin_struct
struct|struct
name|slpque
block|{
name|struct
name|proc
modifier|*
name|sq_head
decl_stmt|;
name|struct
name|proc
modifier|*
modifier|*
name|sq_tailp
decl_stmt|;
block|}
name|slpque
index|[
name|SQSIZE
index|]
struct|;
end_struct

begin_comment
comment|/*  * Give up the processor till a wakeup occurs  * on chan, at which time the process  * enters the scheduling queue at priority pri.  * The most important effect of pri is that when  * pri<=PZERO a signal cannot disturb the sleep;  * if pri>PZERO signals will be processed.  * Callers of this routine must be prepared for  * premature return, and check that the reason for  * sleeping has gone away.  */
end_comment

begin_macro
name|sleep
argument_list|(
argument|chan
argument_list|,
argument|pri
argument_list|)
end_macro

begin_decl_stmt
name|caddr_t
name|chan
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|pri
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|proc
modifier|*
name|rp
decl_stmt|;
specifier|register
name|struct
name|slpque
modifier|*
name|qp
decl_stmt|;
specifier|register
name|s
expr_stmt|;
name|rp
operator|=
name|u
operator|.
name|u_procp
expr_stmt|;
name|s
operator|=
name|splhigh
argument_list|()
expr_stmt|;
if|if
condition|(
name|panicstr
condition|)
block|{
comment|/* 		 * After a panic, just give interrupts a chance, 		 * then just return; don't run any other procs  		 * or panic below, in case this is the idle process 		 * and already asleep. 		 * The splnet should be spl0 if the network was being used 		 * by the filesystem, but for now avoid network interrupts 		 * that might cause another panic. 		 */
operator|(
name|void
operator|)
name|splnet
argument_list|()
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|chan
operator|==
literal|0
operator|||
name|rp
operator|->
name|p_stat
operator|!=
name|SRUN
operator|||
name|rp
operator|->
name|p_rlink
condition|)
name|panic
argument_list|(
literal|"sleep"
argument_list|)
expr_stmt|;
name|rp
operator|->
name|p_wchan
operator|=
name|chan
expr_stmt|;
name|rp
operator|->
name|p_slptime
operator|=
literal|0
expr_stmt|;
name|rp
operator|->
name|p_pri
operator|=
name|pri
expr_stmt|;
name|qp
operator|=
operator|&
name|slpque
index|[
name|HASH
argument_list|(
name|chan
argument_list|)
index|]
expr_stmt|;
if|if
condition|(
name|qp
operator|->
name|sq_head
operator|==
literal|0
condition|)
name|qp
operator|->
name|sq_head
operator|=
name|rp
expr_stmt|;
else|else
operator|*
name|qp
operator|->
name|sq_tailp
operator|=
name|rp
expr_stmt|;
operator|*
operator|(
name|qp
operator|->
name|sq_tailp
operator|=
operator|&
name|rp
operator|->
name|p_link
operator|)
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|pri
operator|>
name|PZERO
condition|)
block|{
comment|/* 		 * If we stop in issig(), wakeup may already have happened 		 * when we return (rp->p_wchan will then be 0). 		 */
if|if
condition|(
name|ISSIG
argument_list|(
name|rp
argument_list|)
condition|)
block|{
if|if
condition|(
name|rp
operator|->
name|p_wchan
condition|)
name|unsleep
argument_list|(
name|rp
argument_list|)
expr_stmt|;
name|rp
operator|->
name|p_stat
operator|=
name|SRUN
expr_stmt|;
operator|(
name|void
operator|)
name|spl0
argument_list|()
expr_stmt|;
goto|goto
name|psig
goto|;
block|}
if|if
condition|(
name|rp
operator|->
name|p_wchan
operator|==
literal|0
condition|)
goto|goto
name|out
goto|;
name|rp
operator|->
name|p_stat
operator|=
name|SSLEEP
expr_stmt|;
operator|(
name|void
operator|)
name|spl0
argument_list|()
expr_stmt|;
name|u
operator|.
name|u_ru
operator|.
name|ru_nvcsw
operator|++
expr_stmt|;
name|swtch
argument_list|()
expr_stmt|;
if|if
condition|(
name|ISSIG
argument_list|(
name|rp
argument_list|)
condition|)
goto|goto
name|psig
goto|;
block|}
else|else
block|{
name|rp
operator|->
name|p_stat
operator|=
name|SSLEEP
expr_stmt|;
operator|(
name|void
operator|)
name|spl0
argument_list|()
expr_stmt|;
name|u
operator|.
name|u_ru
operator|.
name|ru_nvcsw
operator|++
expr_stmt|;
name|swtch
argument_list|()
expr_stmt|;
block|}
name|curpri
operator|=
name|rp
operator|->
name|p_usrpri
expr_stmt|;
name|out
label|:
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return;
comment|/* 	 * If priority was low (>PZERO) and 	 * there has been a signal, execute non-local goto through 	 * u.u_qsave, aborting the system call in progress (see trap.c) 	 */
name|psig
label|:
name|longjmp
argument_list|(
operator|&
name|u
operator|.
name|u_qsave
argument_list|)
expr_stmt|;
comment|/*NOTREACHED*/
block|}
end_block

begin_comment
comment|/*  * Remove a process from its wait queue  */
end_comment

begin_expr_stmt
name|unsleep
argument_list|(
name|p
argument_list|)
specifier|register
expr|struct
name|proc
operator|*
name|p
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|slpque
modifier|*
name|qp
decl_stmt|;
specifier|register
name|struct
name|proc
modifier|*
modifier|*
name|hp
decl_stmt|;
name|int
name|s
decl_stmt|;
name|s
operator|=
name|splhigh
argument_list|()
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_wchan
condition|)
block|{
name|hp
operator|=
operator|&
operator|(
name|qp
operator|=
operator|&
name|slpque
index|[
name|HASH
argument_list|(
name|p
operator|->
name|p_wchan
argument_list|)
index|]
operator|)
operator|->
name|sq_head
expr_stmt|;
while|while
condition|(
operator|*
name|hp
operator|!=
name|p
condition|)
name|hp
operator|=
operator|&
operator|(
operator|*
name|hp
operator|)
operator|->
name|p_link
expr_stmt|;
operator|*
name|hp
operator|=
name|p
operator|->
name|p_link
expr_stmt|;
if|if
condition|(
name|qp
operator|->
name|sq_tailp
operator|==
operator|&
name|p
operator|->
name|p_link
condition|)
name|qp
operator|->
name|sq_tailp
operator|=
name|hp
expr_stmt|;
name|p
operator|->
name|p_wchan
operator|=
literal|0
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Wake up all processes sleeping on chan.  */
end_comment

begin_expr_stmt
name|wakeup
argument_list|(
name|chan
argument_list|)
specifier|register
name|caddr_t
name|chan
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|slpque
modifier|*
name|qp
decl_stmt|;
specifier|register
name|struct
name|proc
modifier|*
name|p
decl_stmt|,
modifier|*
modifier|*
name|q
decl_stmt|;
name|int
name|s
decl_stmt|;
name|s
operator|=
name|splhigh
argument_list|()
expr_stmt|;
name|qp
operator|=
operator|&
name|slpque
index|[
name|HASH
argument_list|(
name|chan
argument_list|)
index|]
expr_stmt|;
name|restart
label|:
for|for
control|(
name|q
operator|=
operator|&
name|qp
operator|->
name|sq_head
init|;
name|p
operator|=
operator|*
name|q
condition|;
control|)
block|{
if|if
condition|(
name|p
operator|->
name|p_rlink
operator|||
name|p
operator|->
name|p_stat
operator|!=
name|SSLEEP
operator|&&
name|p
operator|->
name|p_stat
operator|!=
name|SSTOP
condition|)
name|panic
argument_list|(
literal|"wakeup"
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_wchan
operator|==
name|chan
condition|)
block|{
name|p
operator|->
name|p_wchan
operator|=
literal|0
expr_stmt|;
operator|*
name|q
operator|=
name|p
operator|->
name|p_link
expr_stmt|;
if|if
condition|(
name|qp
operator|->
name|sq_tailp
operator|==
operator|&
name|p
operator|->
name|p_link
condition|)
name|qp
operator|->
name|sq_tailp
operator|=
name|q
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_stat
operator|==
name|SSLEEP
condition|)
block|{
comment|/* OPTIMIZED INLINE EXPANSION OF setrun(p) */
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|>
literal|1
condition|)
name|updatepri
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|p
operator|->
name|p_slptime
operator|=
literal|0
expr_stmt|;
name|p
operator|->
name|p_stat
operator|=
name|SRUN
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_flag
operator|&
name|SLOAD
condition|)
name|setrq
argument_list|(
name|p
argument_list|)
expr_stmt|;
comment|/* 				 * Since curpri is a usrpri, 				 * p->p_pri is always better than curpri. 				 */
name|runrun
operator|++
expr_stmt|;
name|aston
argument_list|()
expr_stmt|;
if|if
condition|(
operator|(
name|p
operator|->
name|p_flag
operator|&
name|SLOAD
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|runout
operator|!=
literal|0
condition|)
block|{
name|runout
operator|=
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|runout
argument_list|)
expr_stmt|;
block|}
name|wantin
operator|++
expr_stmt|;
block|}
comment|/* END INLINE EXPANSION */
goto|goto
name|restart
goto|;
block|}
name|p
operator|->
name|p_slptime
operator|=
literal|0
expr_stmt|;
block|}
else|else
name|q
operator|=
operator|&
name|p
operator|->
name|p_link
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Initialize the (doubly-linked) run queues  * to be empty.  */
end_comment

begin_macro
name|rqinit
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NQS
condition|;
name|i
operator|++
control|)
name|qs
index|[
name|i
index|]
operator|.
name|ph_link
operator|=
name|qs
index|[
name|i
index|]
operator|.
name|ph_rlink
operator|=
operator|(
expr|struct
name|proc
operator|*
operator|)
operator|&
name|qs
index|[
name|i
index|]
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Set the process running;  * arrange for it to be swapped in if necessary.  */
end_comment

begin_expr_stmt
name|setrun
argument_list|(
name|p
argument_list|)
specifier|register
expr|struct
name|proc
operator|*
name|p
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|int
name|s
decl_stmt|;
name|s
operator|=
name|splhigh
argument_list|()
expr_stmt|;
switch|switch
condition|(
name|p
operator|->
name|p_stat
condition|)
block|{
case|case
literal|0
case|:
case|case
name|SWAIT
case|:
case|case
name|SRUN
case|:
case|case
name|SZOMB
case|:
default|default:
name|panic
argument_list|(
literal|"setrun"
argument_list|)
expr_stmt|;
case|case
name|SSTOP
case|:
case|case
name|SSLEEP
case|:
name|unsleep
argument_list|(
name|p
argument_list|)
expr_stmt|;
comment|/* e.g. when sending signals */
break|break;
case|case
name|SIDL
case|:
break|break;
block|}
if|if
condition|(
name|p
operator|->
name|p_slptime
operator|>
literal|1
condition|)
name|updatepri
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|p
operator|->
name|p_stat
operator|=
name|SRUN
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_flag
operator|&
name|SLOAD
condition|)
name|setrq
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_pri
operator|<
name|curpri
condition|)
block|{
name|runrun
operator|++
expr_stmt|;
name|aston
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|p
operator|->
name|p_flag
operator|&
name|SLOAD
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|runout
operator|!=
literal|0
condition|)
block|{
name|runout
operator|=
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|runout
argument_list|)
expr_stmt|;
block|}
name|wantin
operator|++
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Set user priority.  * The rescheduling flag (runrun)  * is set if the priority is better  * than the currently running process.  */
end_comment

begin_expr_stmt
name|setpri
argument_list|(
name|pp
argument_list|)
specifier|register
expr|struct
name|proc
operator|*
name|pp
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|int
name|p
decl_stmt|;
name|p
operator|=
operator|(
name|pp
operator|->
name|p_cpu
operator|&
literal|0377
operator|)
operator|/
literal|4
expr_stmt|;
name|p
operator|+=
name|PUSER
operator|+
literal|2
operator|*
name|pp
operator|->
name|p_nice
expr_stmt|;
if|if
condition|(
name|pp
operator|->
name|p_rssize
operator|>
name|pp
operator|->
name|p_maxrss
operator|&&
name|freemem
operator|<
name|desfree
condition|)
name|p
operator|+=
literal|2
operator|*
literal|4
expr_stmt|;
comment|/* effectively, nice(4) */
if|if
condition|(
name|p
operator|>
literal|127
condition|)
name|p
operator|=
literal|127
expr_stmt|;
if|if
condition|(
name|p
operator|<
name|curpri
condition|)
block|{
name|runrun
operator|++
expr_stmt|;
name|aston
argument_list|()
expr_stmt|;
block|}
name|pp
operator|->
name|p_usrpri
operator|=
name|p
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
block|}
end_block

end_unit

