...
name|pte
expr_stmt|;
comment|/* 	 * Check for text detached but in free list. 	 * This can happen only if the page is filling 	 * from a inode or from the swap device, (e.g. not when reading 	 * in 407/410 execs to a zero fill page.) 	 */
if|if
condition|(
name|type
operator|==
...
block|{
comment|/* 			 * Vreading block... update summary information. 			 */
if|if
condition|(
name|u
operator|.
...
expr_stmt|;
block|}
comment|/* 		 * Check that block is not in file system buffer cache. 		 * The way the cache is handled now, this 		 * happens only once every 2 days. 		 */
if|if
condition|(
name|bp
operator|=
...
condition|)
block|{
comment|/* 				 * Limit pushes to avoid saturating 				 * pageout device. 				 * 				 * MAGIC 4 BECAUSE WE RUN EVERY 1/4 SEC (clock) 				 */
if|if
condition|(
name|pushes
operator|>
