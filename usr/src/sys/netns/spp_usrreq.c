...
begin_comment
comment|/*  * Copyright (c) 1982 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)spp_usrreq.c	6.2 (Berkeley) %G%  */
end_comment
...
block|}
break|break;
comment|/* 	 * This state means that we have gotten a response 	 * to our attempt to establish a connection. 	 * We fill in the data from the other side 	 * (Telling which port to send to instead of the well- 	 * known one we might have to in the first place ) 	 * We also require that this is a response to our 	 * connection id, and that it should be a system packet, 	 * containing no data. 	 */
case|case
name|TCPS_SYN_SENT
case|:
if|if
