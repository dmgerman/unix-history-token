begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1984, 1985 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)ns.c	6.6 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"mbuf.h"
end_include

begin_include
include|#
directive|include
file|"ioctl.h"
end_include

begin_include
include|#
directive|include
file|"protosw.h"
end_include

begin_include
include|#
directive|include
file|"socket.h"
end_include

begin_include
include|#
directive|include
file|"socketvar.h"
end_include

begin_include
include|#
directive|include
file|"uio.h"
end_include

begin_include
include|#
directive|include
file|"dir.h"
end_include

begin_include
include|#
directive|include
file|"user.h"
end_include

begin_include
include|#
directive|include
file|"../net/if.h"
end_include

begin_include
include|#
directive|include
file|"../net/route.h"
end_include

begin_include
include|#
directive|include
file|"../net/af.h"
end_include

begin_include
include|#
directive|include
file|"ns.h"
end_include

begin_include
include|#
directive|include
file|"ns_if.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|NS
end_ifdef

begin_decl_stmt
name|struct
name|ns_ifaddr
modifier|*
name|ns_ifaddr
decl_stmt|;
end_decl_stmt

begin_expr_stmt
name|ns_hash
argument_list|(
name|sns
argument_list|,
name|hp
argument_list|)
specifier|register
expr|struct
name|sockaddr_ns
operator|*
name|sns
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|afhash
modifier|*
name|hp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|long
name|hash
init|=
literal|0
decl_stmt|;
specifier|register
name|u_short
modifier|*
name|s
init|=
name|sns
operator|->
name|sns_addr
operator|.
name|x_host
operator|.
name|s_host
decl_stmt|;
name|hp
operator|->
name|afh_nethash
operator|=
name|ns_netof
argument_list|(
name|sns
operator|->
name|sns_addr
argument_list|)
expr_stmt|;
name|hash
operator|=
operator|*
name|s
operator|++
expr_stmt|;
name|hash
operator|<<=
literal|8
expr_stmt|;
name|hash
operator|+=
operator|*
name|s
operator|++
expr_stmt|;
name|hash
operator|<<=
literal|8
expr_stmt|;
name|hash
operator|+=
operator|*
name|s
expr_stmt|;
name|hp
operator|->
name|afh_hosthash
operator|=
name|hash
expr_stmt|;
block|}
end_block

begin_macro
name|ns_netmatch
argument_list|(
argument|sns1
argument_list|,
argument|sns2
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockaddr_ns
modifier|*
name|sns1
decl_stmt|,
modifier|*
name|sns2
decl_stmt|;
end_decl_stmt

begin_block
block|{
return|return
operator|(
name|ns_netof
argument_list|(
name|sns1
operator|->
name|sns_addr
argument_list|)
operator|==
name|ns_netof
argument_list|(
name|sns2
operator|->
name|sns_addr
argument_list|)
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Generic internet control operations (ioctl's).  */
end_comment

begin_comment
comment|/* ARGSUSED */
end_comment

begin_macro
name|ns_control
argument_list|(
argument|so
argument_list|,
argument|cmd
argument_list|,
argument|data
argument_list|,
argument|ifp
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|cmd
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|caddr_t
name|data
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ifreq
modifier|*
name|ifr
init|=
operator|(
expr|struct
name|ifreq
operator|*
operator|)
name|data
decl_stmt|;
specifier|register
name|struct
name|ns_ifaddr
modifier|*
name|ia
decl_stmt|;
name|struct
name|ifaddr
modifier|*
name|ifa
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
comment|/* 	 * Find address for this interface, if it exists. 	 */
if|if
condition|(
name|ifp
operator|==
literal|0
condition|)
return|return
operator|(
name|EADDRNOTAVAIL
operator|)
return|;
for|for
control|(
name|ia
operator|=
name|ns_ifaddr
init|;
name|ia
condition|;
name|ia
operator|=
name|ia
operator|->
name|ia_next
control|)
if|if
condition|(
name|ia
operator|->
name|ia_ifp
operator|==
name|ifp
condition|)
break|break;
switch|switch
condition|(
name|cmd
condition|)
block|{
case|case
name|SIOCGIFADDR
case|:
if|if
condition|(
name|ia
operator|==
operator|(
expr|struct
name|ns_ifaddr
operator|*
operator|)
literal|0
condition|)
return|return
operator|(
name|EADDRNOTAVAIL
operator|)
return|;
name|ifr
operator|->
name|ifr_addr
operator|=
name|ia
operator|->
name|ia_addr
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|SIOCGIFBRDADDR
case|:
if|if
condition|(
name|ia
operator|==
operator|(
expr|struct
name|ns_ifaddr
operator|*
operator|)
literal|0
condition|)
return|return
operator|(
name|EADDRNOTAVAIL
operator|)
return|;
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_BROADCAST
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|ifr
operator|->
name|ifr_dstaddr
operator|=
name|ia
operator|->
name|ia_broadaddr
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|SIOCGIFDSTADDR
case|:
if|if
condition|(
name|ia
operator|==
operator|(
expr|struct
name|ns_ifaddr
operator|*
operator|)
literal|0
condition|)
return|return
operator|(
name|EADDRNOTAVAIL
operator|)
return|;
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|ifr
operator|->
name|ifr_dstaddr
operator|=
name|ia
operator|->
name|ia_dstaddr
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
operator|!
name|suser
argument_list|()
condition|)
return|return
operator|(
name|u
operator|.
name|u_error
operator|)
return|;
switch|switch
condition|(
name|cmd
condition|)
block|{
case|case
name|SIOCSIFADDR
case|:
case|case
name|SIOCSIFDSTADDR
case|:
if|if
condition|(
name|ia
operator|==
operator|(
expr|struct
name|ns_ifaddr
operator|*
operator|)
literal|0
condition|)
block|{
name|m
operator|=
name|m_getclr
argument_list|(
name|M_WAIT
argument_list|,
name|MT_IFADDR
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|NULL
condition|)
return|return
operator|(
name|ENOBUFS
operator|)
return|;
if|if
condition|(
name|ia
operator|=
name|ns_ifaddr
condition|)
block|{
for|for
control|(
init|;
name|ia
operator|->
name|ia_next
condition|;
name|ia
operator|=
name|ia
operator|->
name|ia_next
control|)
empty_stmt|;
name|ia
operator|->
name|ia_next
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ns_ifaddr
operator|*
argument_list|)
expr_stmt|;
block|}
else|else
name|ns_ifaddr
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ns_ifaddr
operator|*
argument_list|)
expr_stmt|;
name|ia
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ns_ifaddr
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifa
operator|=
name|ifp
operator|->
name|if_addrlist
condition|)
block|{
for|for
control|(
init|;
name|ifa
operator|->
name|ifa_next
condition|;
name|ifa
operator|=
name|ifa
operator|->
name|ifa_next
control|)
empty_stmt|;
name|ifa
operator|->
name|ifa_next
operator|=
operator|(
expr|struct
name|ifaddr
operator|*
operator|)
name|ia
expr_stmt|;
block|}
else|else
name|ifp
operator|->
name|if_addrlist
operator|=
operator|(
expr|struct
name|ifaddr
operator|*
operator|)
name|ia
expr_stmt|;
name|ia
operator|->
name|ia_ifp
operator|=
name|ifp
expr_stmt|;
name|IA_SNS
argument_list|(
name|ia
argument_list|)
operator|->
name|sns_family
operator|=
name|AF_NS
expr_stmt|;
block|}
block|}
switch|switch
condition|(
name|cmd
condition|)
block|{
case|case
name|SIOCSIFDSTADDR
case|:
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
if|if
condition|(
name|ia
operator|->
name|ia_flags
operator|&
name|IFA_ROUTE
condition|)
block|{
name|rtinit
argument_list|(
operator|&
name|ia
operator|->
name|ia_dstaddr
argument_list|,
operator|&
name|ia
operator|->
name|ia_addr
argument_list|,
operator|(
name|int
operator|)
name|SIOCDELRT
argument_list|,
name|RTF_HOST
argument_list|)
expr_stmt|;
name|ia
operator|->
name|ia_flags
operator|&=
operator|~
name|IFA_ROUTE
expr_stmt|;
block|}
if|if
condition|(
name|ifp
operator|->
name|if_ioctl
condition|)
block|{
name|int
name|error
init|=
call|(
modifier|*
name|ifp
operator|->
name|if_ioctl
call|)
argument_list|(
name|ifp
argument_list|,
name|SIOCSIFDSTADDR
argument_list|,
name|ia
argument_list|)
decl_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ia
operator|->
name|ia_dstaddr
operator|=
name|ifr
operator|->
name|ifr_dstaddr
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|SIOCSIFADDR
case|:
return|return
operator|(
name|ns_ifinit
argument_list|(
name|ifp
argument_list|,
name|ia
argument_list|,
operator|(
expr|struct
name|sockaddr_ns
operator|*
operator|)
operator|&
name|ifr
operator|->
name|ifr_addr
argument_list|)
operator|)
return|;
default|default:
if|if
condition|(
name|ifp
operator|->
name|if_ioctl
operator|==
literal|0
condition|)
return|return
operator|(
name|EOPNOTSUPP
operator|)
return|;
return|return
operator|(
call|(
modifier|*
name|ifp
operator|->
name|if_ioctl
call|)
argument_list|(
name|ifp
argument_list|,
name|cmd
argument_list|,
name|data
argument_list|)
operator|)
return|;
block|}
block|}
end_block

begin_comment
comment|/*  * Initialize an interface's internet address  * and routing table entry.  */
end_comment

begin_expr_stmt
name|ns_ifinit
argument_list|(
name|ifp
argument_list|,
name|ia
argument_list|,
name|sns
argument_list|)
specifier|register
expr|struct
name|ifnet
operator|*
name|ifp
expr_stmt|;
end_expr_stmt

begin_decl_stmt
specifier|register
name|struct
name|ns_ifaddr
modifier|*
name|ia
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr_ns
modifier|*
name|sns
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|sockaddr_ns
name|netaddr
decl_stmt|;
specifier|register
name|union
name|ns_host
modifier|*
name|h
init|=
operator|&
operator|(
name|IA_SNS
argument_list|(
name|ia
argument_list|)
operator|->
name|sns_addr
operator|.
name|x_host
operator|)
decl_stmt|;
name|int
name|s
init|=
name|splimp
argument_list|()
decl_stmt|,
name|error
decl_stmt|;
comment|/* 	 * The convention we shall adopt for naming is that 	 * a supplied address of zero means that "we don't care". 	 * if there is a single interface, use the address of that 	 * interface as our 6 byte host address. 	 * if there are multiple interfaces, use any address already 	 * used. 	 * 	 * If we have gotten into trouble and want to reset back to 	 * virginity, we recognize a request of the broadcast address. 	 */
if|if
condition|(
name|ns_hosteqnh
argument_list|(
name|sns
operator|->
name|sns_addr
operator|.
name|x_host
argument_list|,
name|ns_broadhost
argument_list|)
condition|)
block|{
name|ns_thishost
operator|=
name|ns_zerohost
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/* 	 * Delete any previous route for an old address. 	 */
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|netaddr
argument_list|,
sizeof|sizeof
argument_list|(
name|netaddr
argument_list|)
argument_list|)
expr_stmt|;
name|netaddr
operator|.
name|sns_family
operator|=
name|AF_NS
expr_stmt|;
name|netaddr
operator|.
name|sns_addr
operator|.
name|x_host
operator|=
name|ns_broadhost
expr_stmt|;
name|netaddr
operator|.
name|sns_addr
operator|.
name|x_net
operator|=
name|ia
operator|->
name|ia_net
expr_stmt|;
if|if
condition|(
name|ia
operator|->
name|ia_flags
operator|&
name|IFA_ROUTE
condition|)
block|{
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
operator|)
operator|==
literal|0
condition|)
block|{
name|rtinit
argument_list|(
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|netaddr
argument_list|,
operator|&
name|ia
operator|->
name|ia_addr
argument_list|,
operator|(
name|int
operator|)
name|SIOCDELRT
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
name|rtinit
argument_list|(
operator|&
name|ia
operator|->
name|ia_dstaddr
argument_list|,
operator|&
name|ia
operator|->
name|ia_addr
argument_list|,
operator|(
name|int
operator|)
name|SIOCDELRT
argument_list|,
name|RTF_HOST
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Set up new addresses. 	 */
name|ia
operator|->
name|ia_addr
operator|=
operator|*
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|sns
expr_stmt|;
name|ia
operator|->
name|ia_net
operator|=
name|sns
operator|->
name|sns_addr
operator|.
name|x_net
expr_stmt|;
name|netaddr
operator|.
name|sns_addr
operator|.
name|x_net
operator|=
name|ia
operator|->
name|ia_net
expr_stmt|;
if|if
condition|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_BROADCAST
condition|)
block|{
name|ia
operator|->
name|ia_broadaddr
operator|=
operator|*
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|netaddr
expr_stmt|;
block|}
comment|/* 	 * Give the interface a chance to initialize 	 * if this is its first address, 	 * and to validate the address if necessary. 	 */
if|if
condition|(
name|ns_hosteqnh
argument_list|(
name|ns_thishost
argument_list|,
name|ns_zerohost
argument_list|)
condition|)
block|{
if|if
condition|(
name|ifp
operator|->
name|if_ioctl
operator|&&
operator|(
name|error
operator|=
call|(
modifier|*
name|ifp
operator|->
name|if_ioctl
call|)
argument_list|(
name|ifp
argument_list|,
name|SIOCSIFADDR
argument_list|,
name|ia
argument_list|)
operator|)
condition|)
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ns_thishost
operator|=
operator|*
name|h
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|ns_hosteqnh
argument_list|(
name|sns
operator|->
name|sns_addr
operator|.
name|x_host
argument_list|,
name|ns_zerohost
argument_list|)
operator|||
name|ns_hosteqnh
argument_list|(
name|sns
operator|->
name|sns_addr
operator|.
name|x_host
argument_list|,
name|ns_thishost
argument_list|)
condition|)
block|{
operator|*
name|h
operator|=
name|ns_thishost
expr_stmt|;
if|if
condition|(
name|ifp
operator|->
name|if_ioctl
operator|&&
operator|(
name|error
operator|=
call|(
modifier|*
name|ifp
operator|->
name|if_ioctl
call|)
argument_list|(
name|ifp
argument_list|,
name|SIOCSIFADDR
argument_list|,
name|ia
argument_list|)
operator|)
condition|)
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
if|if
condition|(
operator|!
name|ns_hosteqnh
argument_list|(
name|ns_thishost
argument_list|,
operator|*
name|h
argument_list|)
condition|)
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|EINVAL
operator|)
return|;
block|}
block|}
else|else
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|EINVAL
operator|)
return|;
block|}
comment|/* 	 * Add route for the network. 	 */
if|if
condition|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
condition|)
name|rtinit
argument_list|(
operator|&
name|ia
operator|->
name|ia_dstaddr
argument_list|,
operator|&
name|ia
operator|->
name|ia_addr
argument_list|,
operator|(
name|int
operator|)
name|SIOCADDRT
argument_list|,
name|RTF_HOST
operator||
name|RTF_UP
argument_list|)
expr_stmt|;
else|else
name|rtinit
argument_list|(
operator|&
name|ia
operator|->
name|ia_broadaddr
argument_list|,
operator|&
name|ia
operator|->
name|ia_addr
argument_list|,
operator|(
name|int
operator|)
name|SIOCADDRT
argument_list|,
name|RTF_UP
argument_list|)
expr_stmt|;
name|ia
operator|->
name|ia_flags
operator||=
name|IFA_ROUTE
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Return address info for specified internet network.  */
end_comment

begin_function
name|struct
name|ns_ifaddr
modifier|*
name|ns_iaonnetof
parameter_list|(
name|dst
parameter_list|)
specifier|register
name|struct
name|ns_addr
modifier|*
name|dst
decl_stmt|;
block|{
specifier|register
name|struct
name|ns_ifaddr
modifier|*
name|ia
decl_stmt|;
specifier|register
name|struct
name|ns_addr
modifier|*
name|compare
decl_stmt|;
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
name|struct
name|ns_ifaddr
modifier|*
name|ia_maybe
init|=
literal|0
decl_stmt|;
name|long
name|net
init|=
name|ns_netof
argument_list|(
operator|*
name|dst
argument_list|)
decl_stmt|;
specifier|static
name|struct
name|ns_addr
name|laddr
decl_stmt|;
for|for
control|(
name|ia
operator|=
name|ns_ifaddr
init|;
name|ia
condition|;
name|ia
operator|=
name|ia
operator|->
name|ia_next
control|)
block|{
if|if
condition|(
name|ifp
operator|=
name|ia
operator|->
name|ia_ifp
condition|)
block|{
if|if
condition|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_POINTOPOINT
condition|)
block|{
name|compare
operator|=
operator|&
name|satons_addr
argument_list|(
name|ia
operator|->
name|ia_dstaddr
argument_list|)
expr_stmt|;
if|if
condition|(
name|ns_hosteq
argument_list|(
operator|*
name|dst
argument_list|,
operator|*
name|compare
argument_list|)
condition|)
return|return
operator|(
name|ia
operator|)
return|;
name|laddr
operator|.
name|x_net
operator|=
name|ia
operator|->
name|ia_net
expr_stmt|;
if|if
condition|(
name|ns_netof
argument_list|(
name|laddr
argument_list|)
operator|==
name|net
condition|)
name|ia_maybe
operator|=
name|ia
expr_stmt|;
block|}
else|else
block|{
name|laddr
operator|.
name|x_net
operator|=
name|ia
operator|->
name|ia_net
expr_stmt|;
if|if
condition|(
name|ns_netof
argument_list|(
name|laddr
argument_list|)
operator|==
name|net
condition|)
return|return
operator|(
name|ia
operator|)
return|;
block|}
block|}
block|}
return|return
operator|(
name|ia_maybe
operator|)
return|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

end_unit

