...
begin_comment
comment|/*	tcp_input.c	1.48	82/01/17	*/
end_comment
...
expr_stmt|;
block|}
comment|/* 	 * If an URG bit is set and in the segment and is greater than the 	 * current known urgent pointer, then signal the user that the 	 * remote side has out of band data.  This should not happen 	 * in CLOSE_WAIT, CLOSING, LAST-ACK or TIME_WAIT STATES since 	 * a FIN has been received from the remote side.  In these states 	 * we ignore the URG. 	 */
if|if
condition|(
operator|(
name|tiflags
...
begin_comment
comment|/*  * Pull the character before the urgent pointer into  * the TCP control block for presentation as out-of-band data.  * We leave ti->ti_len reflecting the out-of-band data,  * so that sequencing will continue to work.  */
end_comment
