begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/* tcp_input.c 1.21 81/11/08 */
end_comment

begin_include
include|#
directive|include
file|"../h/param.h"
end_include

begin_include
include|#
directive|include
file|"../h/systm.h"
end_include

begin_include
include|#
directive|include
file|"../h/mbuf.h"
end_include

begin_include
include|#
directive|include
file|"../h/socket.h"
end_include

begin_include
include|#
directive|include
file|"../h/socketvar.h"
end_include

begin_include
include|#
directive|include
file|"../net/inet_cksum.h"
end_include

begin_include
include|#
directive|include
file|"../net/inet.h"
end_include

begin_include
include|#
directive|include
file|"../net/inet_systm.h"
end_include

begin_include
include|#
directive|include
file|"../net/imp.h"
end_include

begin_include
include|#
directive|include
file|"../net/inet_host.h"
end_include

begin_include
include|#
directive|include
file|"../net/ip.h"
end_include

begin_include
include|#
directive|include
file|"../net/tcp.h"
end_include

begin_include
include|#
directive|include
file|"../net/tcp_fsm.h"
end_include

begin_include
include|#
directive|include
file|"../net/tcp_var.h"
end_include

begin_include
include|#
directive|include
file|"/usr/include/errno.h"
end_include

begin_decl_stmt
name|int
name|tcpcksum
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_macro
name|tcp_advise
argument_list|(
argument|mp
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|mp
decl_stmt|;
end_decl_stmt

begin_block
block|{  }
end_block

begin_expr_stmt
name|tcp_input
argument_list|(
name|mp
argument_list|)
specifier|register
expr|struct
name|mbuf
operator|*
name|mp
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|th
modifier|*
name|n
decl_stmt|;
comment|/* known to be r10 */
specifier|register
name|int
name|j
decl_stmt|;
specifier|register
name|struct
name|tcb
modifier|*
name|tp
decl_stmt|;
specifier|register
name|int
name|thflags
decl_stmt|;
name|int
name|nstate
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
name|int
name|hlen
decl_stmt|,
name|tlen
decl_stmt|;
name|u_short
name|lport
decl_stmt|,
name|fport
decl_stmt|;
ifdef|#
directive|ifdef
name|TCPDEBUG
name|struct
name|tcp_debug
name|tdb
decl_stmt|;
endif|#
directive|endif
name|COUNT
argument_list|(
name|TCP_INPUT
argument_list|)
expr_stmt|;
comment|/* 	 * Build extended tcp header 	 */
name|n
operator|=
name|mtod
argument_list|(
name|mp
argument_list|,
expr|struct
name|th
operator|*
argument_list|)
expr_stmt|;
name|thflags
operator|=
name|n
operator|->
name|th_flags
expr_stmt|;
name|tlen
operator|=
operator|(
operator|(
expr|struct
name|ip
operator|*
operator|)
name|n
operator|)
operator|->
name|ip_len
expr_stmt|;
name|n
operator|->
name|t_len
operator|=
name|htons
argument_list|(
name|tlen
argument_list|)
expr_stmt|;
name|n
operator|->
name|t_next
operator|=
name|NULL
expr_stmt|;
name|n
operator|->
name|t_prev
operator|=
name|NULL
expr_stmt|;
name|n
operator|->
name|t_x1
operator|=
literal|0
expr_stmt|;
name|lport
operator|=
name|ntohs
argument_list|(
name|n
operator|->
name|t_dst
argument_list|)
expr_stmt|;
name|fport
operator|=
name|ntohs
argument_list|(
name|n
operator|->
name|t_src
argument_list|)
expr_stmt|;
comment|/* WONT BE POSSIBLE WHEN MBUFS ARE 256 BYTES */
if|if
condition|(
operator|(
name|hlen
operator|=
name|n
operator|->
name|t_off
operator|<<
literal|2
operator|)
operator|>
name|mp
operator|->
name|m_len
condition|)
block|{
name|printf
argument_list|(
literal|"tcp header overflow\n"
argument_list|)
expr_stmt|;
name|m_freem
argument_list|(
name|mp
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|tcpcksum
condition|)
block|{
comment|/* 		 * Checksum extended header and data 		 */
name|CKSUM_TCPCHK
argument_list|(
name|mp
argument_list|,
name|n
argument_list|,
name|r10
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
operator|+
name|tlen
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|->
name|t_sum
operator|!=
literal|0
condition|)
block|{
name|netstat
operator|.
name|t_badsum
operator|++
expr_stmt|;
name|m_freem
argument_list|(
name|mp
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
comment|/* 	 * Find tcb for message (SHOULDN'T USE LINEAR SEARCH!) 	 */
name|tp
operator|=
name|tcb
operator|.
name|tcb_next
expr_stmt|;
for|for
control|(
init|;
name|tp
operator|!=
operator|(
expr|struct
name|tcb
operator|*
operator|)
operator|&
name|tcb
condition|;
name|tp
operator|=
name|tp
operator|->
name|tcb_hd
operator|.
name|tcb_next
control|)
if|if
condition|(
name|tp
operator|->
name|t_lport
operator|==
name|lport
operator|&&
name|tp
operator|->
name|t_fport
operator|==
name|fport
operator|&&
name|tp
operator|->
name|t_host
operator|->
name|h_addr
operator|.
name|s_addr
operator|==
name|n
operator|->
name|t_s
operator|.
name|s_addr
condition|)
goto|goto
name|found
goto|;
name|tp
operator|=
name|tcb
operator|.
name|tcb_next
expr_stmt|;
for|for
control|(
init|;
name|tp
operator|!=
operator|(
expr|struct
name|tcb
operator|*
operator|)
operator|&
name|tcb
condition|;
name|tp
operator|=
name|tp
operator|->
name|tcb_hd
operator|.
name|tcb_next
control|)
if|if
condition|(
name|tp
operator|->
name|t_lport
operator|==
name|lport
operator|&&
operator|(
name|tp
operator|->
name|t_fport
operator|==
name|fport
operator|||
name|tp
operator|->
name|t_fport
operator|==
literal|0
operator|)
operator|&&
operator|(
name|tp
operator|->
name|t_host
operator|->
name|h_addr
operator|.
name|s_addr
operator|==
name|n
operator|->
name|t_s
operator|.
name|s_addr
operator|||
name|tp
operator|->
name|t_host
operator|->
name|h_addr
operator|.
name|s_addr
operator|==
literal|0
operator|)
condition|)
goto|goto
name|found
goto|;
goto|goto
name|notwanted
goto|;
name|found
label|:
comment|/* 	 * Byte swap header 	 */
name|n
operator|->
name|t_len
operator|=
name|tlen
operator|-
name|hlen
expr_stmt|;
name|n
operator|->
name|t_src
operator|=
name|fport
expr_stmt|;
name|n
operator|->
name|t_dst
operator|=
name|lport
expr_stmt|;
name|n
operator|->
name|t_seq
operator|=
name|ntohl
argument_list|(
name|n
operator|->
name|t_seq
argument_list|)
expr_stmt|;
name|n
operator|->
name|t_ackno
operator|=
name|ntohl
argument_list|(
name|n
operator|->
name|t_ackno
argument_list|)
expr_stmt|;
name|n
operator|->
name|t_win
operator|=
name|ntohs
argument_list|(
name|n
operator|->
name|t_win
argument_list|)
expr_stmt|;
name|n
operator|->
name|t_urp
operator|=
name|ntohs
argument_list|(
name|n
operator|->
name|t_urp
argument_list|)
expr_stmt|;
comment|/* 	 * Check segment seq # and do rst processing 	 */
switch|switch
condition|(
name|tp
operator|->
name|t_state
condition|)
block|{
case|case
name|LISTEN
case|:
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
operator|||
operator|!
name|syn_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
condition|)
block|{
name|tcp_sndrst
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
expr_stmt|;
goto|goto
name|badseg
goto|;
block|}
if|if
condition|(
name|thflags
operator|&
name|TH_RST
condition|)
goto|goto
name|badseg
goto|;
goto|goto
name|goodseg
goto|;
case|case
name|SYN_SENT
case|:
if|if
condition|(
operator|!
name|ack_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
operator|||
operator|!
name|syn_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
condition|)
block|{
name|tcp_sndrst
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
expr_stmt|;
comment|/* 71,72,75 */
goto|goto
name|badseg
goto|;
block|}
if|if
condition|(
name|thflags
operator|&
name|TH_RST
condition|)
block|{
name|tcp_error
argument_list|(
name|tp
argument_list|,
name|ENETRESET
argument_list|)
expr_stmt|;
name|tcp_detach
argument_list|(
name|tp
argument_list|)
expr_stmt|;
comment|/* 70 */
name|tp
operator|->
name|t_state
operator|=
name|CLOSED
expr_stmt|;
goto|goto
name|badseg
goto|;
block|}
goto|goto
name|goodseg
goto|;
default|default:
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_RST
operator|)
operator|==
literal|0
condition|)
goto|goto
name|common
goto|;
if|if
condition|(
name|n
operator|->
name|t_seq
operator|<
name|tp
operator|->
name|rcv_nxt
condition|)
comment|/* bad rst */
goto|goto
name|badseg
goto|;
comment|/* 69 */
switch|switch
condition|(
name|tp
operator|->
name|t_state
condition|)
block|{
case|case
name|L_SYN_RCVD
case|:
if|if
condition|(
name|ack_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
operator|==
literal|0
condition|)
goto|goto
name|badseg
goto|;
comment|/* 69 */
name|tp
operator|->
name|t_rexmt
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|t_rexmttl
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|t_persist
operator|=
literal|0
expr_stmt|;
name|h_free
argument_list|(
name|tp
operator|->
name|t_host
argument_list|)
expr_stmt|;
name|tp
operator|->
name|t_host
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|t_state
operator|=
name|LISTEN
expr_stmt|;
goto|goto
name|badseg
goto|;
default|default:
name|tcp_error
argument_list|(
name|tp
argument_list|,
name|ENETRESET
argument_list|)
expr_stmt|;
name|tcp_detach
argument_list|(
name|tp
argument_list|)
expr_stmt|;
comment|/* 66 */
name|tp
operator|->
name|t_state
operator|=
name|CLOSED
expr_stmt|;
goto|goto
name|badseg
goto|;
block|}
comment|/*NOTREACHED*/
case|case
name|SYN_RCVD
case|:
name|common
label|:
if|if
condition|(
name|ack_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
operator|==
literal|0
condition|)
block|{
name|tcp_sndrst
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
expr_stmt|;
comment|/* 74 */
goto|goto
name|badseg
goto|;
block|}
if|if
condition|(
name|syn_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
operator|&&
name|n
operator|->
name|t_seq
operator|!=
name|tp
operator|->
name|irs
condition|)
block|{
name|tcp_sndnull
argument_list|(
name|tp
argument_list|)
expr_stmt|;
comment|/* 74 */
goto|goto
name|badseg
goto|;
block|}
goto|goto
name|goodseg
goto|;
block|}
name|badseg
label|:
name|m_freem
argument_list|(
name|mp
argument_list|)
expr_stmt|;
return|return;
name|goodseg
label|:
comment|/* 	 * Defer processing if no buffer space for this connection. 	 */
name|so
operator|=
name|tp
operator|->
name|t_socket
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_rcv
operator|.
name|sb_cc
operator|>=
name|so
operator|->
name|so_rcv
operator|.
name|sb_hiwat
operator|&&
name|n
operator|->
name|t_len
operator|!=
literal|0
operator|&&
name|mbstat
operator|.
name|m_bufs
operator|<
name|mbstat
operator|.
name|m_lowat
condition|)
block|{
comment|/* 		mp->m_act = (struct mbuf *)0; 		if ((m = tp->seg_unack) != NULL) { 			while (m->m_act != NULL) 				m = m->m_act; 			m->m_act = mp; 		} else 			tp->seg_unack = mp; */
name|m_freem
argument_list|(
name|mp
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 	 * Discard ip header, and do tcp input processing. 	 */
name|hlen
operator|+=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
name|mp
operator|->
name|m_off
operator|+=
name|hlen
expr_stmt|;
name|mp
operator|->
name|m_len
operator|-=
name|hlen
expr_stmt|;
name|nstate
operator|=
name|tp
operator|->
name|t_state
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_NET_KEEP
expr_stmt|;
ifdef|#
directive|ifdef
name|KPROF
name|acounts
index|[
name|tp
operator|->
name|t_state
index|]
index|[
name|INRECV
index|]
operator|++
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|TCPDEBUG
if|if
condition|(
operator|(
name|tp
operator|->
name|t_socket
operator|->
name|so_options
operator|&
name|SO_DEBUG
operator|)
operator|||
name|tcpconsdebug
condition|)
block|{
name|tdb_setup
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
name|INRECV
argument_list|,
operator|&
name|tdb
argument_list|)
expr_stmt|;
block|}
else|else
name|tdb
operator|.
name|td_tod
operator|=
literal|0
expr_stmt|;
endif|#
directive|endif
switch|switch
condition|(
name|tp
operator|->
name|t_state
condition|)
block|{
case|case
name|LISTEN
case|:
if|if
condition|(
operator|!
name|syn_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
operator|||
operator|(
operator|(
name|tp
operator|->
name|t_host
operator|=
name|h_make
argument_list|(
operator|&
name|n
operator|->
name|t_s
argument_list|)
operator|)
operator|==
literal|0
operator|)
condition|)
block|{
name|nstate
operator|=
name|EFAILEC
expr_stmt|;
goto|goto
name|done
goto|;
block|}
name|tp
operator|->
name|t_fport
operator|=
name|n
operator|->
name|t_src
expr_stmt|;
name|tp
operator|->
name|t_template
operator|=
name|tcp_template
argument_list|(
name|tp
argument_list|)
expr_stmt|;
name|tcp_ctldat
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
condition|)
block|{
name|tp
operator|->
name|t_finack
operator|=
name|T_2ML
expr_stmt|;
comment|/* 3 */
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_WAITED_2_ML
expr_stmt|;
name|nstate
operator|=
name|CLOSE_WAIT
expr_stmt|;
block|}
else|else
block|{
comment|/* XXX */
comment|/* tp->t_init = T_INIT / 2; */
comment|/* 4 */
name|nstate
operator|=
name|L_SYN_RCVD
expr_stmt|;
block|}
goto|goto
name|done
goto|;
case|case
name|SYN_SENT
case|:
if|if
condition|(
operator|!
name|syn_ok
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
condition|)
block|{
name|nstate
operator|=
name|EFAILEC
expr_stmt|;
goto|goto
name|done
goto|;
block|}
name|tcp_ctldat
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
condition|)
block|{
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
operator|==
literal|0
condition|)
block|{
name|tp
operator|->
name|t_finack
operator|=
name|T_2ML
expr_stmt|;
comment|/* 9 */
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_WAITED_2_ML
expr_stmt|;
block|}
name|nstate
operator|=
name|CLOSE_WAIT
expr_stmt|;
goto|goto
name|done
goto|;
block|}
name|nstate
operator|=
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
condition|?
name|ESTAB
else|:
name|SYN_RCVD
expr_stmt|;
comment|/* 11:8 */
goto|goto
name|done
goto|;
case|case
name|SYN_RCVD
case|:
case|case
name|L_SYN_RCVD
case|:
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
operator|==
literal|0
operator|||
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
operator|&&
name|n
operator|->
name|t_ackno
operator|<=
name|tp
operator|->
name|iss
condition|)
block|{
name|nstate
operator|=
name|EFAILEC
expr_stmt|;
goto|goto
name|done
goto|;
block|}
goto|goto
name|input
goto|;
case|case
name|ESTAB
case|:
case|case
name|FIN_W1
case|:
case|case
name|FIN_W2
case|:
case|case
name|TIME_WAIT
case|:
name|input
label|:
name|tcp_ctldat
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|/* 39 */
switch|switch
condition|(
name|tp
operator|->
name|t_state
condition|)
block|{
case|case
name|ESTAB
case|:
if|if
condition|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
condition|)
name|nstate
operator|=
name|CLOSE_WAIT
expr_stmt|;
break|break;
case|case
name|SYN_RCVD
case|:
case|case
name|L_SYN_RCVD
case|:
name|nstate
operator|=
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
operator|)
condition|?
name|CLOSE_WAIT
else|:
name|ESTAB
expr_stmt|;
comment|/* 33:5 */
break|break;
case|case
name|FIN_W1
case|:
name|j
operator|=
name|ack_fin
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|j
condition|)
name|nstate
operator|=
name|FIN_W2
expr_stmt|;
comment|/* 27 */
break|break;
block|}
name|tp
operator|->
name|t_finack
operator|=
name|T_2ML
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_WAITED_2_ML
expr_stmt|;
name|nstate
operator|=
name|j
condition|?
name|TIME_WAIT
else|:
name|CLOSING
expr_stmt|;
comment|/* 28:26 */
break|break;
case|case
name|FIN_W2
case|:
if|if
condition|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
condition|)
block|{
name|tp
operator|->
name|t_finack
operator|=
name|T_2ML
expr_stmt|;
comment|/* 29 */
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_WAITED_2_ML
expr_stmt|;
name|nstate
operator|=
name|TIME_WAIT
expr_stmt|;
break|break;
block|}
break|break;
block|}
goto|goto
name|done
goto|;
case|case
name|CLOSE_WAIT
case|:
if|if
condition|(
name|thflags
operator|&
name|TH_FIN
condition|)
block|{
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
operator|&&
name|n
operator|->
name|t_ackno
operator|<=
name|tp
operator|->
name|seq_fin
condition|)
block|{
name|tcp_ctldat
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* 30 */
name|tp
operator|->
name|t_finack
operator|=
name|T_2ML
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_WAITED_2_ML
expr_stmt|;
block|}
else|else
name|tcp_sndctl
argument_list|(
name|tp
argument_list|)
expr_stmt|;
comment|/* 31 */
goto|goto
name|done
goto|;
block|}
goto|goto
name|input
goto|;
case|case
name|CLOSING
case|:
name|j
operator|=
name|ack_fin
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
expr_stmt|;
if|if
condition|(
name|thflags
operator|&
name|TH_FIN
condition|)
block|{
name|tcp_ctldat
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|tp
operator|->
name|t_finack
operator|=
name|T_2ML
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_WAITED_2_ML
expr_stmt|;
if|if
condition|(
name|j
condition|)
name|nstate
operator|=
name|TIME_WAIT
expr_stmt|;
comment|/* 23 */
goto|goto
name|done
goto|;
block|}
if|if
condition|(
name|j
condition|)
block|{
if|if
condition|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_WAITED_2_ML
condition|)
if|if
condition|(
name|rcv_empty
argument_list|(
name|tp
argument_list|)
condition|)
block|{
name|sowakeup
argument_list|(
name|tp
operator|->
name|t_socket
argument_list|)
expr_stmt|;
comment|/* ### */
name|nstate
operator|=
name|CLOSED
expr_stmt|;
comment|/* 15 */
block|}
else|else
name|nstate
operator|=
name|RCV_WAIT
expr_stmt|;
comment|/* 18 */
else|else
name|nstate
operator|=
name|TIME_WAIT
expr_stmt|;
goto|goto
name|done
goto|;
block|}
goto|goto
name|input
goto|;
case|case
name|LAST_ACK
case|:
if|if
condition|(
name|ack_fin
argument_list|(
name|tp
argument_list|,
name|n
argument_list|)
condition|)
block|{
if|if
condition|(
name|rcv_empty
argument_list|(
name|tp
argument_list|)
condition|)
block|{
comment|/* 16 */
name|sowakeup
argument_list|(
name|tp
operator|->
name|t_socket
argument_list|)
expr_stmt|;
comment|/* ### */
comment|/* XXX */
comment|/* tcp_close(tp, UCLOSED); */
name|nstate
operator|=
name|CLOSED
expr_stmt|;
block|}
else|else
name|nstate
operator|=
name|RCV_WAIT
expr_stmt|;
comment|/* 19 */
goto|goto
name|done
goto|;
block|}
if|if
condition|(
name|thflags
operator|&
name|TH_FIN
condition|)
block|{
name|tcp_sndctl
argument_list|(
name|tp
argument_list|)
expr_stmt|;
comment|/* 31 */
goto|goto
name|done
goto|;
block|}
goto|goto
name|input
goto|;
case|case
name|RCV_WAIT
case|:
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_FIN
operator|)
operator|&&
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
operator|&&
name|n
operator|->
name|t_ackno
operator|<=
name|tp
operator|->
name|seq_fin
condition|)
block|{
name|tcp_ctldat
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|tp
operator|->
name|t_finack
operator|=
name|T_2ML
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_WAITED_2_ML
expr_stmt|;
comment|/* 30 */
block|}
goto|goto
name|done
goto|;
block|}
name|panic
argument_list|(
literal|"tcp_input"
argument_list|)
expr_stmt|;
name|done
label|:
comment|/* 	 * Done with state*input specific processing. 	 * Form trace records, free input if not needed, 	 * and enter new state. 	 */
ifdef|#
directive|ifdef
name|TCPDEBUG
if|if
condition|(
name|tdb
operator|.
name|td_tod
condition|)
name|tdb_stuff
argument_list|(
operator|&
name|tdb
argument_list|,
name|nstate
argument_list|)
expr_stmt|;
endif|#
directive|endif
switch|switch
condition|(
name|nstate
condition|)
block|{
case|case
name|EFAILEC
case|:
name|m_freem
argument_list|(
name|mp
argument_list|)
expr_stmt|;
return|return;
default|default:
name|tp
operator|->
name|t_state
operator|=
name|nstate
expr_stmt|;
comment|/* fall into ... */
case|case
name|CLOSED
case|:
comment|/* IF CLOSED CANT LOOK AT tc_flags */
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_NET_KEEP
operator|)
operator|==
literal|0
condition|)
comment|/* inline expansion of m_freem */
while|while
condition|(
name|mp
condition|)
block|{
name|MFREE
argument_list|(
name|mp
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|mp
operator|=
name|m
expr_stmt|;
block|}
return|return;
block|}
comment|/* NOTREACHED */
comment|/* 	 * Unwanted packed; free everything 	 * but the header and return an rst. 	 */
name|notwanted
label|:
name|m_freem
argument_list|(
name|mp
operator|->
name|m_next
argument_list|)
expr_stmt|;
name|mp
operator|->
name|m_next
operator|=
name|NULL
expr_stmt|;
name|mp
operator|->
name|m_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|th
argument_list|)
expr_stmt|;
define|#
directive|define
name|xchg
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|j=a; a=b; b=j
name|xchg
argument_list|(
name|n
operator|->
name|t_d
operator|.
name|s_addr
argument_list|,
name|n
operator|->
name|t_s
operator|.
name|s_addr
argument_list|)
expr_stmt|;
name|xchg
argument_list|(
name|n
operator|->
name|t_dst
argument_list|,
name|n
operator|->
name|t_src
argument_list|)
expr_stmt|;
undef|#
directive|undef
name|xchg
if|if
condition|(
name|thflags
operator|&
name|TH_ACK
condition|)
name|n
operator|->
name|t_seq
operator|=
name|n
operator|->
name|t_ackno
expr_stmt|;
else|else
block|{
name|n
operator|->
name|t_ackno
operator|=
name|htonl
argument_list|(
name|ntohl
argument_list|(
name|n
operator|->
name|t_seq
argument_list|)
operator|+
name|tlen
operator|-
name|hlen
argument_list|)
expr_stmt|;
name|n
operator|->
name|t_seq
operator|=
literal|0
expr_stmt|;
block|}
name|n
operator|->
name|th_flags
operator|=
operator|(
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
condition|?
literal|0
else|:
name|TH_ACK
operator|)
operator||
name|TH_RST
expr_stmt|;
name|n
operator|->
name|t_len
operator|=
name|htons
argument_list|(
name|TCPSIZE
argument_list|)
expr_stmt|;
name|n
operator|->
name|t_off
operator|=
literal|5
expr_stmt|;
name|n
operator|->
name|t_sum
operator|=
name|inet_cksum
argument_list|(
name|mp
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|th
argument_list|)
argument_list|)
expr_stmt|;
operator|(
operator|(
expr|struct
name|ip
operator|*
operator|)
name|n
operator|)
operator|->
name|ip_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|th
argument_list|)
expr_stmt|;
name|ip_output
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|netstat
operator|.
name|t_badsegs
operator|++
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|tcp_ctldat
argument_list|(
name|tp
argument_list|,
name|n
argument_list|,
name|dataok
argument_list|)
specifier|register
expr|struct
name|tcb
operator|*
name|tp
expr_stmt|;
end_expr_stmt

begin_decl_stmt
specifier|register
name|struct
name|th
modifier|*
name|n
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
specifier|register
name|int
name|thflags
init|=
name|n
operator|->
name|th_flags
decl_stmt|;
name|int
name|sent
decl_stmt|;
name|COUNT
argument_list|(
name|TCP_CTLDAT
argument_list|)
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
operator|(
name|TC_DROPPED_TXT
operator||
name|TC_ACK_DUE
operator||
name|TC_NEW_WINDOW
operator|)
expr_stmt|;
comment|/* syn */
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_SYN_RCVD
operator|)
operator|==
literal|0
operator|&&
operator|(
name|thflags
operator|&
name|TH_SYN
operator|)
condition|)
block|{
name|tp
operator|->
name|irs
operator|=
name|n
operator|->
name|t_seq
expr_stmt|;
name|tp
operator|->
name|rcv_nxt
operator|=
name|n
operator|->
name|t_seq
operator|+
literal|1
expr_stmt|;
name|tp
operator|->
name|snd_wl
operator|=
name|tp
operator|->
name|rcv_urp
operator|=
name|tp
operator|->
name|irs
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator||=
operator|(
name|TC_SYN_RCVD
operator||
name|TC_ACK_DUE
operator|)
expr_stmt|;
block|}
comment|/* ack */
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_ACK
operator|)
operator|&&
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_SYN_RCVD
operator|)
operator|&&
name|n
operator|->
name|t_ackno
operator|>
name|tp
operator|->
name|snd_una
condition|)
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|mn
decl_stmt|;
specifier|register
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
name|int
name|len
decl_stmt|;
name|so
operator|=
name|tp
operator|->
name|t_socket
expr_stmt|;
comment|/* update snd_una and snd_nxt */
name|tp
operator|->
name|snd_una
operator|=
name|n
operator|->
name|t_ackno
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|snd_una
operator|>
name|tp
operator|->
name|snd_nxt
condition|)
name|tp
operator|->
name|snd_nxt
operator|=
name|tp
operator|->
name|snd_una
expr_stmt|;
comment|/* if timed msg acked, set retrans time value */
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_SYN_ACKED
operator|)
operator|&&
name|tp
operator|->
name|snd_una
operator|>
name|tp
operator|->
name|t_xmt_val
condition|)
block|{
name|tp
operator|->
name|t_xmtime
operator|=
operator|(
name|tp
operator|->
name|t_xmt
operator|!=
literal|0
condition|?
name|tp
operator|->
name|t_xmt
else|:
name|T_REXMT
operator|)
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|t_xmtime
operator|>
name|T_REMAX
condition|)
name|tp
operator|->
name|t_xmtime
operator|=
name|T_REMAX
expr_stmt|;
block|}
comment|/* remove acked data from send buf */
name|len
operator|=
name|tp
operator|->
name|snd_una
operator|-
name|tp
operator|->
name|snd_off
expr_stmt|;
name|m
operator|=
name|so
operator|->
name|so_snd
operator|.
name|sb_mb
expr_stmt|;
while|while
condition|(
name|len
operator|>
literal|0
operator|&&
name|m
operator|!=
name|NULL
condition|)
if|if
condition|(
name|m
operator|->
name|m_len
operator|<=
name|len
condition|)
block|{
name|len
operator|-=
name|m
operator|->
name|m_len
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_off
operator|>
name|MMAXOFF
condition|)
name|so
operator|->
name|so_snd
operator|.
name|sb_mbcnt
operator|-=
name|NMBPG
expr_stmt|;
name|MFREE
argument_list|(
name|m
argument_list|,
name|mn
argument_list|)
expr_stmt|;
name|m
operator|=
name|mn
expr_stmt|;
name|so
operator|->
name|so_snd
operator|.
name|sb_mbcnt
operator|--
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_snd
operator|.
name|sb_mbcnt
operator|<=
literal|0
condition|)
name|panic
argument_list|(
literal|"tcp_ctldat"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|m
operator|->
name|m_len
operator|-=
name|len
expr_stmt|;
name|m
operator|->
name|m_off
operator|+=
name|len
expr_stmt|;
break|break;
block|}
name|so
operator|->
name|so_snd
operator|.
name|sb_mb
operator|=
name|m
expr_stmt|;
name|tp
operator|->
name|snd_off
operator|=
name|tp
operator|->
name|snd_una
expr_stmt|;
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_SYN_ACKED
operator|)
operator|==
literal|0
operator|&&
operator|(
name|tp
operator|->
name|snd_una
operator|>
name|tp
operator|->
name|iss
operator|)
condition|)
block|{
name|tp
operator|->
name|tc_flags
operator||=
name|TC_SYN_ACKED
expr_stmt|;
name|tp
operator|->
name|t_init
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|tp
operator|->
name|seq_fin
operator|!=
name|tp
operator|->
name|iss
operator|&&
name|tp
operator|->
name|snd_una
operator|>
name|tp
operator|->
name|seq_fin
condition|)
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_SND_FIN
expr_stmt|;
name|tp
operator|->
name|t_rexmt
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|t_rexmttl
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator||=
name|TC_CANCELLED
expr_stmt|;
name|sowakeup
argument_list|(
name|tp
operator|->
name|t_socket
argument_list|)
expr_stmt|;
comment|/* wasteful */
block|}
comment|/* win */
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_SYN_RCVD
operator|)
operator|&&
name|n
operator|->
name|t_seq
operator|>=
name|tp
operator|->
name|snd_wl
condition|)
block|{
name|tp
operator|->
name|snd_wl
operator|=
name|n
operator|->
name|t_seq
expr_stmt|;
name|tp
operator|->
name|snd_wnd
operator|=
name|n
operator|->
name|t_win
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator||=
name|TC_NEW_WINDOW
expr_stmt|;
name|tp
operator|->
name|t_persist
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|dataok
operator|==
literal|0
condition|)
goto|goto
name|ctlonly
goto|;
comment|/* text */
if|if
condition|(
name|n
operator|->
name|t_len
operator|==
literal|0
condition|)
goto|goto
name|notext
goto|;
block|{
specifier|register
name|int
name|i
decl_stmt|;
specifier|register
name|struct
name|th
modifier|*
name|p
decl_stmt|,
modifier|*
name|q
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|int
name|overage
decl_stmt|;
comment|/* 	 * Discard duplicate data already passed to user. 	 */
if|if
condition|(
name|SEQ_LT
argument_list|(
name|n
operator|->
name|t_seq
argument_list|,
name|tp
operator|->
name|rcv_nxt
argument_list|)
condition|)
block|{
name|i
operator|=
name|tp
operator|->
name|rcv_nxt
operator|-
name|n
operator|->
name|t_seq
expr_stmt|;
if|if
condition|(
name|i
operator|>=
name|n
operator|->
name|t_len
condition|)
goto|goto
name|notext
goto|;
name|n
operator|->
name|t_seq
operator|+=
name|i
expr_stmt|;
name|n
operator|->
name|t_len
operator|-=
name|i
expr_stmt|;
name|m_adj
argument_list|(
name|dtom
argument_list|(
name|n
argument_list|)
argument_list|,
name|i
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Find a segment which begins after this one does. 	 */
for|for
control|(
name|q
operator|=
name|tp
operator|->
name|tcb_hd
operator|.
name|seg_next
init|;
name|q
operator|!=
operator|(
expr|struct
name|th
operator|*
operator|)
name|tp
condition|;
name|q
operator|=
name|q
operator|->
name|t_next
control|)
if|if
condition|(
name|SEQ_GT
argument_list|(
name|q
operator|->
name|t_seq
argument_list|,
name|n
operator|->
name|t_seq
argument_list|)
condition|)
break|break;
comment|/* 	 * If there is a preceding segment, it may provide some of 	 * our data already.  If so, drop the data from the incoming 	 * segment.  If it provides all of our data, drop us. 	 */
if|if
condition|(
name|q
operator|->
name|t_prev
operator|!=
operator|(
expr|struct
name|th
operator|*
operator|)
name|tp
condition|)
block|{
comment|/* conversion to int (in i) handles seq wraparound */
name|i
operator|=
name|q
operator|->
name|t_prev
operator|->
name|t_seq
operator|+
name|q
operator|->
name|t_prev
operator|->
name|t_len
operator|-
name|n
operator|->
name|t_seq
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|0
condition|)
block|{
if|if
condition|(
name|i
operator|>=
name|n
operator|->
name|t_len
condition|)
goto|goto
name|notext
goto|;
comment|/* w/o setting TC_NET_KEEP */
name|m_adj
argument_list|(
name|dtom
argument_list|(
name|tp
argument_list|)
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|n
operator|->
name|t_len
operator|-=
name|i
expr_stmt|;
name|n
operator|->
name|t_seq
operator|+=
name|i
expr_stmt|;
block|}
block|}
comment|/* 	 * While we overlap succeeding segments trim them or, 	 * if they are completely covered, dequeue them. 	 */
while|while
condition|(
name|q
operator|!=
operator|(
expr|struct
name|th
operator|*
operator|)
name|tp
operator|&&
name|SEQ_GT
argument_list|(
name|n
operator|->
name|t_seq
operator|+
name|n
operator|->
name|t_len
argument_list|,
name|q
operator|->
name|t_seq
argument_list|)
condition|)
block|{
name|i
operator|=
operator|(
name|n
operator|->
name|t_seq
operator|+
name|n
operator|->
name|t_len
operator|)
operator|-
name|q
operator|->
name|t_seq
expr_stmt|;
if|if
condition|(
name|i
operator|<
name|q
operator|->
name|t_len
condition|)
block|{
name|q
operator|->
name|t_len
operator|-=
name|i
expr_stmt|;
name|m_adj
argument_list|(
name|dtom
argument_list|(
name|q
argument_list|)
argument_list|,
name|i
argument_list|)
expr_stmt|;
break|break;
block|}
name|q
operator|=
name|q
operator|->
name|t_next
expr_stmt|;
name|m_freem
argument_list|(
name|dtom
argument_list|(
name|q
operator|->
name|t_prev
argument_list|)
argument_list|)
expr_stmt|;
name|remque
argument_list|(
name|q
operator|->
name|t_prev
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Stick new segment in its place. 	 */
name|insque
argument_list|(
name|n
argument_list|,
name|q
operator|->
name|t_prev
argument_list|)
expr_stmt|;
name|tp
operator|->
name|seqcnt
operator|+=
name|n
operator|->
name|t_len
expr_stmt|;
comment|/* 	 * Calculate available space and discard segments for 	 * which there is too much. 	 */
name|q
operator|=
name|tp
operator|->
name|tcb_hd
operator|.
name|seg_prev
expr_stmt|;
name|overage
operator|=
operator|(
name|tp
operator|->
name|t_socket
operator|->
name|so_rcv
operator|.
name|sb_cc
comment|/* + tp->rcv_seqcnt XXX */
operator|)
operator|-
name|tp
operator|->
name|t_socket
operator|->
name|so_rcv
operator|.
name|sb_hiwat
expr_stmt|;
if|if
condition|(
name|overage
operator|>
literal|0
condition|)
for|for
control|(
init|;
condition|;
control|)
block|{
name|i
operator|=
name|MIN
argument_list|(
name|q
operator|->
name|t_len
argument_list|,
name|overage
argument_list|)
expr_stmt|;
name|overage
operator|-=
name|i
expr_stmt|;
name|q
operator|->
name|t_len
operator|-=
name|i
expr_stmt|;
name|m_adj
argument_list|(
name|q
argument_list|,
operator|-
name|i
argument_list|)
expr_stmt|;
if|if
condition|(
name|q
operator|==
name|n
condition|)
name|tp
operator|->
name|tc_flags
operator||=
name|TC_DROPPED_TXT
expr_stmt|;
if|if
condition|(
name|q
operator|->
name|t_len
condition|)
break|break;
if|if
condition|(
name|q
operator|==
name|n
condition|)
name|panic
argument_list|(
literal|"tcp_text dropall"
argument_list|)
expr_stmt|;
name|q
operator|=
name|q
operator|->
name|t_prev
expr_stmt|;
name|remque
argument_list|(
name|q
operator|->
name|t_next
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Advance rcv_next through 	 * newly completed sequence space 	 * and return forcing an ack. 	 */
while|while
condition|(
name|n
operator|->
name|t_seq
operator|==
name|tp
operator|->
name|rcv_nxt
condition|)
block|{
comment|/* present data belongs here */
name|tp
operator|->
name|rcv_nxt
operator|+=
name|n
operator|->
name|t_len
expr_stmt|;
name|n
operator|=
name|n
operator|->
name|t_next
expr_stmt|;
if|if
condition|(
name|n
operator|==
operator|(
expr|struct
name|th
operator|*
operator|)
name|tp
condition|)
break|break;
block|}
name|tp
operator|->
name|tc_flags
operator||=
operator|(
name|TC_ACK_DUE
operator||
name|TC_NET_KEEP
operator|)
expr_stmt|;
block|}
name|notext
label|:
comment|/* urg */
if|if
condition|(
name|thflags
operator|&
operator|(
name|TH_URG
operator||
name|TH_EOL
operator||
name|TH_FIN
operator|)
condition|)
block|{
if|if
condition|(
name|thflags
operator|&
name|TH_URG
condition|)
block|{
name|unsigned
name|urgent
decl_stmt|;
name|urgent
operator|=
name|n
operator|->
name|t_urp
operator|+
name|n
operator|->
name|t_seq
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|rcv_nxt
operator|<
name|urgent
condition|)
block|{
if|if
condition|(
name|tp
operator|->
name|rcv_urp
operator|<=
name|tp
operator|->
name|rcv_nxt
condition|)
block|{
comment|/* DO SOMETHING WITH URGENT!!! ### */
block|}
name|tp
operator|->
name|rcv_urp
operator|=
name|urgent
expr_stmt|;
block|}
block|}
comment|/* eol */
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_EOL
operator|)
operator|&&
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_DROPPED_TXT
operator|)
operator|==
literal|0
operator|&&
name|tp
operator|->
name|tcb_hd
operator|.
name|seg_prev
operator|!=
operator|(
expr|struct
name|th
operator|*
operator|)
name|tp
condition|)
block|{
comment|/* mark last mbuf */
name|m
operator|=
name|dtom
argument_list|(
name|tp
operator|->
name|tcb_hd
operator|.
name|seg_prev
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|!=
name|NULL
condition|)
block|{
while|while
condition|(
name|m
operator|->
name|m_next
operator|!=
name|NULL
condition|)
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
name|m
operator|->
name|m_act
operator|=
operator|(
expr|struct
name|mbuf
operator|*
operator|)
operator|(
name|m
operator|->
name|m_off
operator|+
name|m
operator|->
name|m_len
operator|-
literal|1
operator|)
expr_stmt|;
block|}
block|}
name|ctlonly
label|:
comment|/* fin */
if|if
condition|(
operator|(
name|thflags
operator|&
name|TH_FIN
operator|)
operator|&&
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_DROPPED_TXT
operator|)
operator|==
literal|0
condition|)
block|{
name|seq_t
name|last
decl_stmt|;
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
operator|)
operator|==
literal|0
condition|)
block|{
comment|/* do we really have fin ? */
name|last
operator|=
name|firstempty
argument_list|(
name|tp
argument_list|)
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|tcb_hd
operator|.
name|seg_prev
operator|==
operator|(
expr|struct
name|th
operator|*
operator|)
name|tp
operator|||
name|last
operator|==
name|t_end
argument_list|(
name|tp
operator|->
name|tcb_hd
operator|.
name|seg_prev
argument_list|)
condition|)
block|{
name|tp
operator|->
name|tc_flags
operator||=
name|TC_FIN_RCVD
expr_stmt|;
name|sowakeup
argument_list|(
name|tp
operator|->
name|t_socket
argument_list|)
expr_stmt|;
comment|/* ### */
block|}
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_FIN_RCVD
operator|)
operator|&&
name|tp
operator|->
name|rcv_nxt
operator|>=
name|last
condition|)
block|{
name|tp
operator|->
name|rcv_nxt
operator|=
name|last
operator|+
literal|1
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator||=
name|TC_ACK_DUE
expr_stmt|;
block|}
block|}
else|else
name|tp
operator|->
name|tc_flags
operator||=
name|TC_ACK_DUE
expr_stmt|;
block|}
block|}
comment|/* respond */
name|sent
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_ACK_DUE
condition|)
name|sent
operator|=
name|tcp_sndctl
argument_list|(
name|tp
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_NEW_WINDOW
operator|)
condition|)
if|if
condition|(
name|tp
operator|->
name|snd_nxt
operator|<=
name|tp
operator|->
name|snd_off
operator|+
name|tp
operator|->
name|t_socket
operator|->
name|so_snd
operator|.
name|sb_cc
operator|||
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_SND_FIN
operator|)
condition|)
name|sent
operator|=
name|tcp_send
argument_list|(
name|tp
argument_list|)
expr_stmt|;
comment|/* set for retrans */
if|if
condition|(
operator|!
name|sent
operator|&&
name|tp
operator|->
name|snd_una
operator|<
name|tp
operator|->
name|snd_nxt
operator|&&
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_CANCELLED
operator|)
condition|)
block|{
name|tp
operator|->
name|t_rexmt
operator|=
name|tp
operator|->
name|t_xmtime
expr_stmt|;
name|tp
operator|->
name|t_rexmttl
operator|=
name|T_REXMTTL
expr_stmt|;
name|tp
operator|->
name|t_rexmt_val
operator|=
name|tp
operator|->
name|t_rtl_val
operator|=
name|tp
operator|->
name|snd_lst
expr_stmt|;
name|tp
operator|->
name|tc_flags
operator|&=
operator|~
name|TC_CANCELLED
expr_stmt|;
block|}
comment|/* present data to user */
block|{
specifier|register
name|struct
name|mbuf
modifier|*
modifier|*
name|mp
decl_stmt|;
specifier|register
name|struct
name|socket
modifier|*
name|so
init|=
name|tp
operator|->
name|t_socket
decl_stmt|;
name|seq_t
name|ready
decl_stmt|;
comment|/* connection must be synced and data available for user */
if|if
condition|(
operator|(
name|tp
operator|->
name|tc_flags
operator|&
name|TC_SYN_ACKED
operator|)
operator|==
literal|0
condition|)
return|return;
name|so
operator|=
name|tp
operator|->
name|t_socket
expr_stmt|;
name|mp
operator|=
operator|&
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
expr_stmt|;
while|while
condition|(
operator|*
name|mp
condition|)
name|mp
operator|=
operator|&
operator|(
operator|*
name|mp
operator|)
operator|->
name|m_next
expr_stmt|;
name|n
operator|=
name|tp
operator|->
name|tcb_hd
operator|.
name|seg_next
expr_stmt|;
comment|/* SHOULD PACK DATA IN HERE */
while|while
condition|(
name|n
operator|!=
operator|(
expr|struct
name|th
operator|*
operator|)
name|tp
operator|&&
name|n
operator|->
name|t_seq
operator|<
name|tp
operator|->
name|rcv_nxt
condition|)
block|{
name|remque
argument_list|(
name|n
argument_list|)
expr_stmt|;
name|m
operator|=
name|dtom
argument_list|(
name|n
argument_list|)
expr_stmt|;
name|so
operator|->
name|so_rcv
operator|.
name|sb_cc
operator|+=
name|n
operator|->
name|t_len
expr_stmt|;
name|tp
operator|->
name|seqcnt
operator|-=
name|n
operator|->
name|t_len
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|seqcnt
operator|<
literal|0
condition|)
name|panic
argument_list|(
literal|"present_data"
argument_list|)
expr_stmt|;
name|n
operator|=
name|n
operator|->
name|t_next
expr_stmt|;
while|while
condition|(
name|m
condition|)
block|{
if|if
condition|(
name|m
operator|->
name|m_len
operator|==
literal|0
condition|)
block|{
name|MFREE
argument_list|(
name|m
argument_list|,
operator|*
name|mp
argument_list|)
expr_stmt|;
block|}
else|else
block|{
operator|*
name|mp
operator|=
name|m
expr_stmt|;
name|mp
operator|=
operator|&
name|m
operator|->
name|m_next
expr_stmt|;
block|}
name|m
operator|=
operator|*
name|mp
expr_stmt|;
block|}
block|}
name|sowakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
comment|/* should be macro/conditional */
block|}
block|}
end_block

begin_macro
name|tcp_drain
argument_list|()
end_macro

begin_block
block|{  }
end_block

end_unit

