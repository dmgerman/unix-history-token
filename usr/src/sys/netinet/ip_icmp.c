...
name|t
expr_stmt|;
comment|/* 	 * If the incoming packet was source-routed, 	 * we need to reverse the route and set the next-hop destination. 	 * We can dispense with the error checking 	 * as ip_dooptions has been through here before. 	 */
if|if
condition|(
operator|(
name|ip
...
operator|)
expr_stmt|;
comment|/* 				 * First switch the last route entry 				 * (first hop for return) with ip_dst 				 * (final hop on return). 				 */
name|bcopy
argument_list|(
operator|(
name|caddr_t
...
operator|--
expr_stmt|;
comment|/* 				 * Then reverse remaining route entries. 				 */
while|while
condition|(
name|p
operator|>
