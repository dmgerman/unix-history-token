begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982, 1986, 1988, 1990, 1993 Regents of the University  * of California.  All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)ip_output.c	7.32 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_include
include|#
directive|include
file|<sys/errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/protosw.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/socketvar.h>
end_include

begin_include
include|#
directive|include
file|<net/if.h>
end_include

begin_include
include|#
directive|include
file|<net/route.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_systm.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_pcb.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_var.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip_var.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|vax
end_ifdef

begin_include
include|#
directive|include
file|<machine/mtpr.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|ip_insertoptions
name|__P
argument_list|(
operator|(
expr|struct
name|mbuf
operator|*
operator|,
expr|struct
name|mbuf
operator|*
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|ip_mloopback
name|__P
argument_list|(
operator|(
expr|struct
name|ifnet
operator|*
operator|,
expr|struct
name|mbuf
operator|*
operator|,
expr|struct
name|sockaddr_in
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * IP output.  The packet in mbuf chain m contains a skeletal IP  * header (with len, off, ttl, proto, tos, src, dst).  * The mbuf chain containing the packet will be freed.  * The mbuf opt, if present, will not be freed.  */
end_comment

begin_function
name|int
name|ip_output
parameter_list|(
name|m0
parameter_list|,
name|opt
parameter_list|,
name|ro
parameter_list|,
name|flags
parameter_list|,
name|imo
parameter_list|)
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|opt
decl_stmt|;
name|struct
name|route
modifier|*
name|ro
decl_stmt|;
name|int
name|flags
decl_stmt|;
name|struct
name|ip_moptions
modifier|*
name|imo
decl_stmt|;
block|{
specifier|register
name|struct
name|ip
modifier|*
name|ip
decl_stmt|,
modifier|*
name|mhip
decl_stmt|;
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
init|=
name|m0
decl_stmt|;
specifier|register
name|int
name|hlen
init|=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
decl_stmt|;
name|int
name|len
decl_stmt|,
name|off
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|;
name|struct
name|route
name|iproute
decl_stmt|;
name|struct
name|sockaddr_in
modifier|*
name|dst
decl_stmt|;
name|struct
name|in_ifaddr
modifier|*
name|ia
decl_stmt|;
ifdef|#
directive|ifdef
name|DIAGNOSTIC
if|if
condition|(
operator|(
name|m
operator|->
name|m_flags
operator|&
name|M_PKTHDR
operator|)
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"ip_output no HDR"
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|opt
condition|)
block|{
name|m
operator|=
name|ip_insertoptions
argument_list|(
name|m
argument_list|,
name|opt
argument_list|,
operator|&
name|len
argument_list|)
expr_stmt|;
name|hlen
operator|=
name|len
expr_stmt|;
block|}
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
comment|/* 	 * Fill in IP header. 	 */
if|if
condition|(
operator|(
name|flags
operator|&
operator|(
name|IP_FORWARDING
operator||
name|IP_RAWOUTPUT
operator|)
operator|)
operator|==
literal|0
condition|)
block|{
name|ip
operator|->
name|ip_v
operator|=
name|IPVERSION
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|&=
name|IP_DF
expr_stmt|;
name|ip
operator|->
name|ip_id
operator|=
name|htons
argument_list|(
name|ip_id
operator|++
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_hl
operator|=
name|hlen
operator|>>
literal|2
expr_stmt|;
name|ipstat
operator|.
name|ips_localout
operator|++
expr_stmt|;
block|}
else|else
block|{
name|hlen
operator|=
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
expr_stmt|;
block|}
comment|/* 	 * Route packet. 	 */
if|if
condition|(
name|ro
operator|==
literal|0
condition|)
block|{
name|ro
operator|=
operator|&
name|iproute
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
name|ro
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ro
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ro
operator|->
name|ro_dst
expr_stmt|;
comment|/* 	 * If there is a cached route, 	 * check that it is to the same destination 	 * and is still up.  If not, free it and try again. 	 */
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|&&
operator|(
operator|(
name|ro
operator|->
name|ro_rt
operator|->
name|rt_flags
operator|&
name|RTF_UP
operator|)
operator|==
literal|0
operator|||
name|dst
operator|->
name|sin_addr
operator|.
name|s_addr
operator|!=
name|ip
operator|->
name|ip_dst
operator|.
name|s_addr
operator|)
condition|)
block|{
name|RTFREE
argument_list|(
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
name|ro
operator|->
name|ro_rt
operator|=
operator|(
expr|struct
name|rtentry
operator|*
operator|)
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|==
literal|0
condition|)
block|{
name|dst
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|dst
operator|->
name|sin_len
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|dst
argument_list|)
expr_stmt|;
name|dst
operator|->
name|sin_addr
operator|=
name|ip
operator|->
name|ip_dst
expr_stmt|;
block|}
comment|/* 	 * If routing to interface only, 	 * short circuit routing lookup. 	 */
define|#
directive|define
name|ifatoia
parameter_list|(
name|ifa
parameter_list|)
value|((struct in_ifaddr *)(ifa))
define|#
directive|define
name|sintosa
parameter_list|(
name|sin
parameter_list|)
value|((struct sockaddr *)(sin))
if|if
condition|(
name|flags
operator|&
name|IP_ROUTETOIF
condition|)
block|{
if|if
condition|(
operator|(
name|ia
operator|=
name|ifatoia
argument_list|(
name|ifa_ifwithdstaddr
argument_list|(
name|sintosa
argument_list|(
name|dst
argument_list|)
argument_list|)
argument_list|)
operator|)
operator|==
literal|0
operator|&&
operator|(
name|ia
operator|=
name|ifatoia
argument_list|(
name|ifa_ifwithnet
argument_list|(
name|sintosa
argument_list|(
name|dst
argument_list|)
argument_list|)
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
name|ipstat
operator|.
name|ips_noroute
operator|++
expr_stmt|;
name|error
operator|=
name|ENETUNREACH
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|ifp
operator|=
name|ia
operator|->
name|ia_ifp
expr_stmt|;
name|ip
operator|->
name|ip_ttl
operator|=
literal|1
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|==
literal|0
condition|)
name|rtalloc
argument_list|(
name|ro
argument_list|)
expr_stmt|;
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|==
literal|0
condition|)
block|{
name|ipstat
operator|.
name|ips_noroute
operator|++
expr_stmt|;
name|error
operator|=
name|EHOSTUNREACH
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|ia
operator|=
name|ifatoia
argument_list|(
name|ro
operator|->
name|ro_rt
operator|->
name|rt_ifa
argument_list|)
expr_stmt|;
name|ifp
operator|=
name|ro
operator|->
name|ro_rt
operator|->
name|rt_ifp
expr_stmt|;
name|ro
operator|->
name|ro_rt
operator|->
name|rt_use
operator|++
expr_stmt|;
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|->
name|rt_flags
operator|&
name|RTF_GATEWAY
condition|)
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|ro
operator|->
name|ro_rt
operator|->
name|rt_gateway
expr_stmt|;
block|}
if|if
condition|(
name|IN_MULTICAST
argument_list|(
name|ntohl
argument_list|(
name|ip
operator|->
name|ip_dst
operator|.
name|s_addr
argument_list|)
argument_list|)
condition|)
block|{
name|struct
name|in_multi
modifier|*
name|inm
decl_stmt|;
specifier|extern
name|struct
name|ifnet
name|loif
decl_stmt|;
name|m
operator|->
name|m_flags
operator||=
name|M_MCAST
expr_stmt|;
comment|/* 		 * IP destination address is multicast.  Make sure "dst" 		 * still points to the address in "ro".  (It may have been 		 * changed to point to a gateway address, above.) 		 */
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ro
operator|->
name|ro_dst
expr_stmt|;
comment|/* 		 * See if the caller provided any multicast options 		 */
if|if
condition|(
name|imo
operator|!=
name|NULL
condition|)
block|{
name|ip
operator|->
name|ip_ttl
operator|=
name|imo
operator|->
name|imo_multicast_ttl
expr_stmt|;
if|if
condition|(
name|imo
operator|->
name|imo_multicast_ifp
operator|!=
name|NULL
condition|)
name|ifp
operator|=
name|imo
operator|->
name|imo_multicast_ifp
expr_stmt|;
block|}
else|else
name|ip
operator|->
name|ip_ttl
operator|=
name|IP_DEFAULT_MULTICAST_TTL
expr_stmt|;
comment|/* 		 * Confirm that the outgoing interface supports multicast. 		 */
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_MULTICAST
operator|)
operator|==
literal|0
condition|)
block|{
name|ipstat
operator|.
name|ips_noroute
operator|++
expr_stmt|;
name|error
operator|=
name|ENETUNREACH
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
comment|/* 		 * If source address not specified yet, use address 		 * of outgoing interface. 		 */
if|if
condition|(
name|ip
operator|->
name|ip_src
operator|.
name|s_addr
operator|==
name|INADDR_ANY
condition|)
block|{
specifier|register
name|struct
name|in_ifaddr
modifier|*
name|ia
decl_stmt|;
for|for
control|(
name|ia
operator|=
name|in_ifaddr
init|;
name|ia
condition|;
name|ia
operator|=
name|ia
operator|->
name|ia_next
control|)
if|if
condition|(
name|ia
operator|->
name|ia_ifp
operator|==
name|ifp
condition|)
block|{
name|ip
operator|->
name|ip_src
operator|=
name|IA_SIN
argument_list|(
name|ia
argument_list|)
operator|->
name|sin_addr
expr_stmt|;
break|break;
block|}
block|}
name|IN_LOOKUP_MULTI
argument_list|(
name|ip
operator|->
name|ip_dst
argument_list|,
name|ifp
argument_list|,
name|inm
argument_list|)
expr_stmt|;
if|if
condition|(
name|inm
operator|!=
name|NULL
operator|&&
operator|(
name|imo
operator|==
name|NULL
operator|||
name|imo
operator|->
name|imo_multicast_loop
operator|)
condition|)
block|{
comment|/* 			 * If we belong to the destination multicast group 			 * on the outgoing interface, and the caller did not 			 * forbid loopback, loop back a copy. 			 */
name|ip_mloopback
argument_list|(
name|ifp
argument_list|,
name|m
argument_list|,
name|dst
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|MROUTING
else|else
block|{
comment|/* 			 * If we are acting as a multicast router, perform 			 * multicast forwarding as if the packet had just 			 * arrived on the interface to which we are about 			 * to send.  The multicast forwarding function 			 * recursively calls this function, using the 			 * IP_FORWARDING flag to prevent infinite recursion. 			 * 			 * Multicasts that are looped back by ip_mloopback(), 			 * above, will be forwarded by the ip_input() routine, 			 * if necessary. 			 */
specifier|extern
name|struct
name|socket
modifier|*
name|ip_mrouter
decl_stmt|;
if|if
condition|(
name|ip_mrouter
operator|&&
operator|(
name|flags
operator|&
name|IP_FORWARDING
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|ip_mforward
argument_list|(
name|m
argument_list|,
name|ifp
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
block|}
block|}
endif|#
directive|endif
comment|/* 		 * Multicasts with a time-to-live of zero may be looped- 		 * back, above, but must not be transmitted on a network. 		 * Also, multicasts addressed to the loopback interface 		 * are not sent -- the above call to ip_mloopback() will 		 * loop back a copy if this host actually belongs to the 		 * destination group on the loopback interface. 		 */
if|if
condition|(
name|ip
operator|->
name|ip_ttl
operator|==
literal|0
operator|||
name|ifp
operator|==
operator|&
name|loif
condition|)
block|{
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
goto|goto
name|sendit
goto|;
block|}
ifndef|#
directive|ifndef
name|notdef
comment|/* 	 * If source address not specified yet, use address 	 * of outgoing interface. 	 */
if|if
condition|(
name|ip
operator|->
name|ip_src
operator|.
name|s_addr
operator|==
name|INADDR_ANY
condition|)
name|ip
operator|->
name|ip_src
operator|=
name|IA_SIN
argument_list|(
name|ia
argument_list|)
operator|->
name|sin_addr
expr_stmt|;
endif|#
directive|endif
comment|/* 	 * Look for broadcast address and 	 * and verify user is allowed to send 	 * such a packet. 	 */
if|if
condition|(
name|in_broadcast
argument_list|(
name|dst
operator|->
name|sin_addr
argument_list|,
name|ifp
argument_list|)
condition|)
block|{
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_BROADCAST
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|EADDRNOTAVAIL
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
if|if
condition|(
operator|(
name|flags
operator|&
name|IP_ALLOWBROADCAST
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|EACCES
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
comment|/* don't allow broadcast messages to be fragmented */
if|if
condition|(
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
operator|>
name|ifp
operator|->
name|if_mtu
condition|)
block|{
name|error
operator|=
name|EMSGSIZE
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|m
operator|->
name|m_flags
operator||=
name|M_BCAST
expr_stmt|;
block|}
else|else
name|m
operator|->
name|m_flags
operator|&=
operator|~
name|M_BCAST
expr_stmt|;
name|sendit
label|:
comment|/* 	 * If small enough for interface, can just send directly. 	 */
if|if
condition|(
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
operator|<=
name|ifp
operator|->
name|if_mtu
condition|)
block|{
name|ip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_off
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|hlen
argument_list|)
expr_stmt|;
name|error
operator|=
call|(
modifier|*
name|ifp
operator|->
name|if_output
call|)
argument_list|(
name|ifp
argument_list|,
name|m
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|dst
argument_list|,
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
comment|/* 	 * Too large for interface; fragment if possible. 	 * Must be able to put at least 8 bytes per fragment. 	 */
if|if
condition|(
name|ip
operator|->
name|ip_off
operator|&
name|IP_DF
condition|)
block|{
name|error
operator|=
name|EMSGSIZE
expr_stmt|;
name|ipstat
operator|.
name|ips_cantfrag
operator|++
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|len
operator|=
operator|(
name|ifp
operator|->
name|if_mtu
operator|-
name|hlen
operator|)
operator|&
operator|~
literal|7
expr_stmt|;
if|if
condition|(
name|len
operator|<
literal|8
condition|)
block|{
name|error
operator|=
name|EMSGSIZE
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
block|{
name|int
name|mhlen
decl_stmt|,
name|firstlen
init|=
name|len
decl_stmt|;
name|struct
name|mbuf
modifier|*
modifier|*
name|mnext
init|=
operator|&
name|m
operator|->
name|m_nextpkt
decl_stmt|;
comment|/* 	 * Loop through length of segment after first fragment, 	 * make new header and copy data of each part and link onto chain. 	 */
name|m0
operator|=
name|m
expr_stmt|;
name|mhlen
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
for|for
control|(
name|off
operator|=
name|hlen
operator|+
name|len
init|;
name|off
operator|<
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
condition|;
name|off
operator|+=
name|len
control|)
block|{
name|MGETHDR
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
name|ipstat
operator|.
name|ips_odropped
operator|++
expr_stmt|;
goto|goto
name|sendorfree
goto|;
block|}
name|m
operator|->
name|m_data
operator|+=
name|max_linkhdr
expr_stmt|;
name|mhip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
operator|*
name|mhip
operator|=
operator|*
name|ip
expr_stmt|;
if|if
condition|(
name|hlen
operator|>
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
condition|)
block|{
name|mhlen
operator|=
name|ip_optcopy
argument_list|(
name|ip
argument_list|,
name|mhip
argument_list|)
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
name|mhip
operator|->
name|ip_hl
operator|=
name|mhlen
operator|>>
literal|2
expr_stmt|;
block|}
name|m
operator|->
name|m_len
operator|=
name|mhlen
expr_stmt|;
name|mhip
operator|->
name|ip_off
operator|=
operator|(
operator|(
name|off
operator|-
name|hlen
operator|)
operator|>>
literal|3
operator|)
operator|+
operator|(
name|ip
operator|->
name|ip_off
operator|&
operator|~
name|IP_MF
operator|)
expr_stmt|;
if|if
condition|(
name|ip
operator|->
name|ip_off
operator|&
name|IP_MF
condition|)
name|mhip
operator|->
name|ip_off
operator||=
name|IP_MF
expr_stmt|;
if|if
condition|(
name|off
operator|+
name|len
operator|>=
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
condition|)
name|len
operator|=
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
operator|-
name|off
expr_stmt|;
else|else
name|mhip
operator|->
name|ip_off
operator||=
name|IP_MF
expr_stmt|;
name|mhip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
call|(
name|u_short
call|)
argument_list|(
name|len
operator|+
name|mhlen
argument_list|)
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_next
operator|=
name|m_copy
argument_list|(
name|m0
argument_list|,
name|off
argument_list|,
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_next
operator|==
literal|0
condition|)
block|{
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
name|error
operator|=
name|ENOBUFS
expr_stmt|;
comment|/* ??? */
name|ipstat
operator|.
name|ips_odropped
operator|++
expr_stmt|;
goto|goto
name|sendorfree
goto|;
block|}
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|mhlen
operator|+
name|len
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|rcvif
operator|=
operator|(
expr|struct
name|ifnet
operator|*
operator|)
literal|0
expr_stmt|;
name|mhip
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|mhip
operator|->
name|ip_off
argument_list|)
expr_stmt|;
name|mhip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
name|mhip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|mhlen
argument_list|)
expr_stmt|;
operator|*
name|mnext
operator|=
name|m
expr_stmt|;
name|mnext
operator|=
operator|&
name|m
operator|->
name|m_nextpkt
expr_stmt|;
name|ipstat
operator|.
name|ips_ofragments
operator|++
expr_stmt|;
block|}
comment|/* 	 * Update first fragment by trimming what's been copied out 	 * and updating header, then send each fragment (in order). 	 */
name|m
operator|=
name|m0
expr_stmt|;
name|m_adj
argument_list|(
name|m
argument_list|,
name|hlen
operator|+
name|firstlen
operator|-
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|hlen
operator|+
name|firstlen
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|m
operator|->
name|m_pkthdr
operator|.
name|len
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
call|(
name|u_short
call|)
argument_list|(
name|ip
operator|->
name|ip_off
operator||
name|IP_MF
argument_list|)
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|hlen
argument_list|)
expr_stmt|;
name|sendorfree
label|:
for|for
control|(
name|m
operator|=
name|m0
init|;
name|m
condition|;
name|m
operator|=
name|m0
control|)
block|{
name|m0
operator|=
name|m
operator|->
name|m_nextpkt
expr_stmt|;
name|m
operator|->
name|m_nextpkt
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|error
operator|==
literal|0
condition|)
name|error
operator|=
call|(
modifier|*
name|ifp
operator|->
name|if_output
call|)
argument_list|(
name|ifp
argument_list|,
name|m
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|dst
argument_list|,
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
else|else
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|error
operator|==
literal|0
condition|)
name|ipstat
operator|.
name|ips_fragmented
operator|++
expr_stmt|;
block|}
name|done
label|:
if|if
condition|(
name|ro
operator|==
operator|&
name|iproute
operator|&&
operator|(
name|flags
operator|&
name|IP_ROUTETOIF
operator|)
operator|==
literal|0
operator|&&
name|ro
operator|->
name|ro_rt
condition|)
name|RTFREE
argument_list|(
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
name|bad
label|:
name|m_freem
argument_list|(
name|m0
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
end_function

begin_comment
comment|/*  * Insert IP options into preformed packet.  * Adjust IP destination as required for IP source routing,  * as indicated by a non-zero in_addr at the start of the options.  */
end_comment

begin_function
name|struct
name|mbuf
modifier|*
name|ip_insertoptions
parameter_list|(
name|m
parameter_list|,
name|opt
parameter_list|,
name|phlen
parameter_list|)
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|opt
decl_stmt|;
name|int
modifier|*
name|phlen
decl_stmt|;
block|{
specifier|register
name|struct
name|ipoption
modifier|*
name|p
init|=
name|mtod
argument_list|(
name|opt
argument_list|,
expr|struct
name|ipoption
operator|*
argument_list|)
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|n
decl_stmt|;
specifier|register
name|struct
name|ip
modifier|*
name|ip
init|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
decl_stmt|;
name|unsigned
name|optlen
decl_stmt|;
name|optlen
operator|=
name|opt
operator|->
name|m_len
operator|-
sizeof|sizeof
argument_list|(
name|p
operator|->
name|ipopt_dst
argument_list|)
expr_stmt|;
if|if
condition|(
name|optlen
operator|+
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
operator|>
name|IP_MAXPACKET
condition|)
return|return
operator|(
name|m
operator|)
return|;
comment|/* XXX should fail */
if|if
condition|(
name|p
operator|->
name|ipopt_dst
operator|.
name|s_addr
condition|)
name|ip
operator|->
name|ip_dst
operator|=
name|p
operator|->
name|ipopt_dst
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_flags
operator|&
name|M_EXT
operator|||
name|m
operator|->
name|m_data
operator|-
name|optlen
operator|<
name|m
operator|->
name|m_pktdat
condition|)
block|{
name|MGETHDR
argument_list|(
name|n
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
return|return
operator|(
name|m
operator|)
return|;
name|n
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|+
name|optlen
expr_stmt|;
name|m
operator|->
name|m_len
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_data
operator|+=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
name|n
operator|->
name|m_next
operator|=
name|m
expr_stmt|;
name|m
operator|=
name|n
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
name|optlen
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_data
operator|+=
name|max_linkhdr
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|ip
argument_list|,
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|m
operator|->
name|m_data
operator|-=
name|optlen
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
name|optlen
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|+=
name|optlen
expr_stmt|;
name|ovbcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|ip
argument_list|,
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|p
operator|->
name|ipopt_list
argument_list|,
call|(
name|caddr_t
call|)
argument_list|(
name|ip
operator|+
literal|1
argument_list|)
argument_list|,
operator|(
name|unsigned
operator|)
name|optlen
argument_list|)
expr_stmt|;
operator|*
name|phlen
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
operator|+
name|optlen
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|+=
name|optlen
expr_stmt|;
return|return
operator|(
name|m
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Copy options from ip to jp,  * omitting those not copied during fragmentation.  */
end_comment

begin_function
name|int
name|ip_optcopy
parameter_list|(
name|ip
parameter_list|,
name|jp
parameter_list|)
name|struct
name|ip
modifier|*
name|ip
decl_stmt|,
decl|*
name|jp
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|u_char
modifier|*
name|cp
decl_stmt|,
modifier|*
name|dp
decl_stmt|;
name|int
name|opt
decl_stmt|,
name|optlen
decl_stmt|,
name|cnt
decl_stmt|;
name|cp
operator|=
operator|(
name|u_char
operator|*
operator|)
operator|(
name|ip
operator|+
literal|1
operator|)
expr_stmt|;
name|dp
operator|=
operator|(
name|u_char
operator|*
operator|)
operator|(
name|jp
operator|+
literal|1
operator|)
expr_stmt|;
name|cnt
operator|=
operator|(
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
operator|)
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
for|for
control|(
init|;
name|cnt
operator|>
literal|0
condition|;
name|cnt
operator|-=
name|optlen
operator|,
name|cp
operator|+=
name|optlen
control|)
block|{
name|opt
operator|=
name|cp
index|[
literal|0
index|]
expr_stmt|;
if|if
condition|(
name|opt
operator|==
name|IPOPT_EOL
condition|)
break|break;
if|if
condition|(
name|opt
operator|==
name|IPOPT_NOP
condition|)
block|{
comment|/* Preserve for IP mcast tunnel's LSRR alignment. */
operator|*
name|dp
operator|++
operator|=
name|IPOPT_NOP
expr_stmt|;
name|optlen
operator|=
literal|1
expr_stmt|;
continue|continue;
block|}
else|else
name|optlen
operator|=
name|cp
index|[
name|IPOPT_OLEN
index|]
expr_stmt|;
comment|/* bogus lengths should have been caught by ip_dooptions */
if|if
condition|(
name|optlen
operator|>
name|cnt
condition|)
name|optlen
operator|=
name|cnt
expr_stmt|;
if|if
condition|(
name|IPOPT_COPIED
argument_list|(
name|opt
argument_list|)
condition|)
block|{
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|cp
argument_list|,
operator|(
name|caddr_t
operator|)
name|dp
argument_list|,
operator|(
name|unsigned
operator|)
name|optlen
argument_list|)
expr_stmt|;
name|dp
operator|+=
name|optlen
expr_stmt|;
block|}
block|}
for|for
control|(
name|optlen
operator|=
name|dp
operator|-
operator|(
name|u_char
operator|*
operator|)
operator|(
name|jp
operator|+
literal|1
operator|)
init|;
name|optlen
operator|&
literal|0x3
condition|;
name|optlen
operator|++
control|)
operator|*
name|dp
operator|++
operator|=
name|IPOPT_EOL
expr_stmt|;
return|return
operator|(
name|optlen
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * IP socket option processing.  */
end_comment

begin_function
name|int
name|ip_ctloutput
parameter_list|(
name|op
parameter_list|,
name|so
parameter_list|,
name|level
parameter_list|,
name|optname
parameter_list|,
name|mp
parameter_list|)
name|int
name|op
decl_stmt|;
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
name|int
name|level
decl_stmt|,
name|optname
decl_stmt|;
name|struct
name|mbuf
modifier|*
modifier|*
name|mp
decl_stmt|;
block|{
specifier|register
name|struct
name|inpcb
modifier|*
name|inp
init|=
name|sotoinpcb
argument_list|(
name|so
argument_list|)
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
init|=
operator|*
name|mp
decl_stmt|;
specifier|register
name|int
name|optval
decl_stmt|;
name|int
name|error
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|level
operator|!=
name|IPPROTO_IP
condition|)
goto|goto
name|freeit
goto|;
else|else
switch|switch
condition|(
name|op
condition|)
block|{
case|case
name|PRCO_SETOPT
case|:
switch|switch
condition|(
name|optname
condition|)
block|{
case|case
name|IP_OPTIONS
case|:
ifdef|#
directive|ifdef
name|notyet
case|case
name|IP_RETOPTS
case|:
return|return
operator|(
name|ip_pcbopts
argument_list|(
name|optname
argument_list|,
operator|&
name|inp
operator|->
name|inp_options
argument_list|,
name|m
argument_list|)
operator|)
return|;
else|#
directive|else
return|return
operator|(
name|ip_pcbopts
argument_list|(
operator|&
name|inp
operator|->
name|inp_options
argument_list|,
name|m
argument_list|)
operator|)
return|;
endif|#
directive|endif
case|case
name|IP_TOS
case|:
case|case
name|IP_TTL
case|:
case|case
name|IP_RECVOPTS
case|:
case|case
name|IP_RECVRETOPTS
case|:
case|case
name|IP_RECVDSTADDR
case|:
if|if
condition|(
name|m
operator|->
name|m_len
operator|!=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
condition|)
name|error
operator|=
name|EINVAL
expr_stmt|;
else|else
block|{
name|optval
operator|=
operator|*
name|mtod
argument_list|(
name|m
argument_list|,
name|int
operator|*
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|optname
condition|)
block|{
case|case
name|IP_TOS
case|:
name|inp
operator|->
name|inp_ip
operator|.
name|ip_tos
operator|=
name|optval
expr_stmt|;
break|break;
case|case
name|IP_TTL
case|:
name|inp
operator|->
name|inp_ip
operator|.
name|ip_ttl
operator|=
name|optval
expr_stmt|;
break|break;
define|#
directive|define
name|OPTSET
parameter_list|(
name|bit
parameter_list|)
define|\
value|if (optval) \ 		inp->inp_flags |= bit; \ 	else \ 		inp->inp_flags&= ~bit;
case|case
name|IP_RECVOPTS
case|:
name|OPTSET
argument_list|(
name|INP_RECVOPTS
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_RECVRETOPTS
case|:
name|OPTSET
argument_list|(
name|INP_RECVRETOPTS
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_RECVDSTADDR
case|:
name|OPTSET
argument_list|(
name|INP_RECVDSTADDR
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
break|break;
undef|#
directive|undef
name|OPTSET
case|case
name|IP_MULTICAST_IF
case|:
case|case
name|IP_MULTICAST_TTL
case|:
case|case
name|IP_MULTICAST_LOOP
case|:
case|case
name|IP_ADD_MEMBERSHIP
case|:
case|case
name|IP_DROP_MEMBERSHIP
case|:
name|error
operator|=
name|ip_setmoptions
argument_list|(
name|optname
argument_list|,
operator|&
name|inp
operator|->
name|inp_moptions
argument_list|,
name|m
argument_list|)
expr_stmt|;
break|break;
name|freeit
label|:
default|default:
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|m
condition|)
operator|(
name|void
operator|)
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
break|break;
case|case
name|PRCO_GETOPT
case|:
switch|switch
condition|(
name|optname
condition|)
block|{
case|case
name|IP_OPTIONS
case|:
case|case
name|IP_RETOPTS
case|:
operator|*
name|mp
operator|=
name|m
operator|=
name|m_get
argument_list|(
name|M_WAIT
argument_list|,
name|MT_SOOPTS
argument_list|)
expr_stmt|;
if|if
condition|(
name|inp
operator|->
name|inp_options
condition|)
block|{
name|m
operator|->
name|m_len
operator|=
name|inp
operator|->
name|inp_options
operator|->
name|m_len
expr_stmt|;
name|bcopy
argument_list|(
name|mtod
argument_list|(
name|inp
operator|->
name|inp_options
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
operator|(
name|unsigned
operator|)
name|m
operator|->
name|m_len
argument_list|)
expr_stmt|;
block|}
else|else
name|m
operator|->
name|m_len
operator|=
literal|0
expr_stmt|;
break|break;
case|case
name|IP_TOS
case|:
case|case
name|IP_TTL
case|:
case|case
name|IP_RECVOPTS
case|:
case|case
name|IP_RECVRETOPTS
case|:
case|case
name|IP_RECVDSTADDR
case|:
operator|*
name|mp
operator|=
name|m
operator|=
name|m_get
argument_list|(
name|M_WAIT
argument_list|,
name|MT_SOOPTS
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
sizeof|sizeof
argument_list|(
name|int
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|optname
condition|)
block|{
case|case
name|IP_TOS
case|:
name|optval
operator|=
name|inp
operator|->
name|inp_ip
operator|.
name|ip_tos
expr_stmt|;
break|break;
case|case
name|IP_TTL
case|:
name|optval
operator|=
name|inp
operator|->
name|inp_ip
operator|.
name|ip_ttl
expr_stmt|;
break|break;
define|#
directive|define
name|OPTBIT
parameter_list|(
name|bit
parameter_list|)
value|(inp->inp_flags& bit ? 1 : 0)
case|case
name|IP_RECVOPTS
case|:
name|optval
operator|=
name|OPTBIT
argument_list|(
name|INP_RECVOPTS
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_RECVRETOPTS
case|:
name|optval
operator|=
name|OPTBIT
argument_list|(
name|INP_RECVRETOPTS
argument_list|)
expr_stmt|;
break|break;
case|case
name|IP_RECVDSTADDR
case|:
name|optval
operator|=
name|OPTBIT
argument_list|(
name|INP_RECVDSTADDR
argument_list|)
expr_stmt|;
break|break;
block|}
operator|*
name|mtod
argument_list|(
name|m
argument_list|,
name|int
operator|*
argument_list|)
operator|=
name|optval
expr_stmt|;
break|break;
case|case
name|IP_MULTICAST_IF
case|:
case|case
name|IP_MULTICAST_TTL
case|:
case|case
name|IP_MULTICAST_LOOP
case|:
case|case
name|IP_ADD_MEMBERSHIP
case|:
case|case
name|IP_DROP_MEMBERSHIP
case|:
name|error
operator|=
name|ip_getmoptions
argument_list|(
name|optname
argument_list|,
name|inp
operator|->
name|inp_moptions
argument_list|,
name|mp
argument_list|)
expr_stmt|;
break|break;
default|default:
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
break|break;
block|}
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Set up IP options in pcb for insertion in output packets.  * Store in mbuf with pointer in pcbopt, adding pseudo-option  * with destination address if source routed.  */
end_comment

begin_function
name|int
ifdef|#
directive|ifdef
name|notyet
name|ip_pcbopts
parameter_list|(
name|optname
parameter_list|,
name|pcbopt
parameter_list|,
name|m
parameter_list|)
name|int
name|optname
decl_stmt|;
else|#
directive|else
function|ip_pcbopts
parameter_list|(
name|pcbopt
parameter_list|,
name|m
parameter_list|)
endif|#
directive|endif
name|struct
name|mbuf
modifier|*
modifier|*
name|pcbopt
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
block|{
specifier|register
name|cnt
operator|,
name|optlen
expr_stmt|;
specifier|register
name|u_char
modifier|*
name|cp
decl_stmt|;
name|u_char
name|opt
decl_stmt|;
comment|/* turn off any old options */
if|if
condition|(
operator|*
name|pcbopt
condition|)
operator|(
name|void
operator|)
name|m_free
argument_list|(
operator|*
name|pcbopt
argument_list|)
expr_stmt|;
operator|*
name|pcbopt
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|m
operator|==
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
operator|||
name|m
operator|->
name|m_len
operator|==
literal|0
condition|)
block|{
comment|/* 		 * Only turning off any previous options. 		 */
if|if
condition|(
name|m
condition|)
operator|(
name|void
operator|)
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
ifndef|#
directive|ifndef
name|vax
if|if
condition|(
name|m
operator|->
name|m_len
operator|%
sizeof|sizeof
argument_list|(
name|long
argument_list|)
condition|)
goto|goto
name|bad
goto|;
endif|#
directive|endif
comment|/* 	 * IP first-hop destination address will be stored before 	 * actual options; move other options back 	 * and clear it when none present. 	 */
if|if
condition|(
name|m
operator|->
name|m_data
operator|+
name|m
operator|->
name|m_len
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
operator|>=
operator|&
name|m
operator|->
name|m_dat
index|[
name|MLEN
index|]
condition|)
goto|goto
name|bad
goto|;
name|cnt
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
expr_stmt|;
name|cp
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|u_char
operator|*
argument_list|)
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
expr_stmt|;
name|ovbcopy
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
operator|(
name|caddr_t
operator|)
name|cp
argument_list|,
operator|(
name|unsigned
operator|)
name|cnt
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
init|;
name|cnt
operator|>
literal|0
condition|;
name|cnt
operator|-=
name|optlen
operator|,
name|cp
operator|+=
name|optlen
control|)
block|{
name|opt
operator|=
name|cp
index|[
name|IPOPT_OPTVAL
index|]
expr_stmt|;
if|if
condition|(
name|opt
operator|==
name|IPOPT_EOL
condition|)
break|break;
if|if
condition|(
name|opt
operator|==
name|IPOPT_NOP
condition|)
name|optlen
operator|=
literal|1
expr_stmt|;
else|else
block|{
name|optlen
operator|=
name|cp
index|[
name|IPOPT_OLEN
index|]
expr_stmt|;
if|if
condition|(
name|optlen
operator|<=
name|IPOPT_OLEN
operator|||
name|optlen
operator|>
name|cnt
condition|)
goto|goto
name|bad
goto|;
block|}
switch|switch
condition|(
name|opt
condition|)
block|{
default|default:
break|break;
case|case
name|IPOPT_LSRR
case|:
case|case
name|IPOPT_SSRR
case|:
comment|/* 			 * user process specifies route as: 			 *	->A->B->C->D 			 * D must be our final destination (but we can't 			 * check that since we may not have connected yet). 			 * A is first hop destination, which doesn't appear in 			 * actual IP option, but is stored before the options. 			 */
if|if
condition|(
name|optlen
operator|<
name|IPOPT_MINOFF
operator|-
literal|1
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
condition|)
goto|goto
name|bad
goto|;
name|m
operator|->
name|m_len
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
expr_stmt|;
name|cnt
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
expr_stmt|;
name|optlen
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
expr_stmt|;
name|cp
index|[
name|IPOPT_OLEN
index|]
operator|=
name|optlen
expr_stmt|;
comment|/* 			 * Move first hop before start of options. 			 */
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|cp
index|[
name|IPOPT_OFFSET
operator|+
literal|1
index|]
argument_list|,
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 			 * Then copy rest of options back 			 * to close up the deleted entry. 			 */
name|ovbcopy
argument_list|(
call|(
name|caddr_t
call|)
argument_list|(
operator|&
name|cp
index|[
name|IPOPT_OFFSET
operator|+
literal|1
index|]
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
argument_list|)
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|cp
index|[
name|IPOPT_OFFSET
operator|+
literal|1
index|]
argument_list|,
operator|(
name|unsigned
operator|)
name|cnt
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
if|if
condition|(
name|m
operator|->
name|m_len
operator|>
name|MAX_IPOPTLEN
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
condition|)
goto|goto
name|bad
goto|;
operator|*
name|pcbopt
operator|=
name|m
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
name|bad
label|:
operator|(
name|void
operator|)
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|(
name|EINVAL
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Set the IP multicast options in response to user setsockopt().  */
end_comment

begin_function
name|int
name|ip_setmoptions
parameter_list|(
name|optname
parameter_list|,
name|imop
parameter_list|,
name|m
parameter_list|)
name|int
name|optname
decl_stmt|;
name|struct
name|ip_moptions
modifier|*
modifier|*
name|imop
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
block|{
specifier|register
name|int
name|error
init|=
literal|0
decl_stmt|;
name|u_char
name|loop
decl_stmt|;
specifier|register
name|int
name|i
decl_stmt|;
name|struct
name|in_addr
name|addr
decl_stmt|;
specifier|register
name|struct
name|ip_mreq
modifier|*
name|mreq
decl_stmt|;
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
specifier|register
name|struct
name|ip_moptions
modifier|*
name|imo
init|=
operator|*
name|imop
decl_stmt|;
name|struct
name|route
name|ro
decl_stmt|;
specifier|register
name|struct
name|sockaddr_in
modifier|*
name|dst
decl_stmt|;
if|if
condition|(
name|imo
operator|==
name|NULL
condition|)
block|{
comment|/* 		 * No multicast option buffer attached to the pcb; 		 * allocate one and initialize to default values. 		 */
name|imo
operator|=
operator|(
expr|struct
name|ip_moptions
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|imo
argument_list|)
argument_list|,
name|M_IPMOPTS
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
if|if
condition|(
name|imo
operator|==
name|NULL
condition|)
return|return
operator|(
name|ENOBUFS
operator|)
return|;
operator|*
name|imop
operator|=
name|imo
expr_stmt|;
name|imo
operator|->
name|imo_multicast_ifp
operator|=
name|NULL
expr_stmt|;
name|imo
operator|->
name|imo_multicast_ttl
operator|=
name|IP_DEFAULT_MULTICAST_TTL
expr_stmt|;
name|imo
operator|->
name|imo_multicast_loop
operator|=
name|IP_DEFAULT_MULTICAST_LOOP
expr_stmt|;
name|imo
operator|->
name|imo_num_memberships
operator|=
literal|0
expr_stmt|;
block|}
switch|switch
condition|(
name|optname
condition|)
block|{
case|case
name|IP_MULTICAST_IF
case|:
comment|/* 		 * Select the interface for outgoing multicast packets. 		 */
if|if
condition|(
name|m
operator|==
name|NULL
operator|||
name|m
operator|->
name|m_len
operator|!=
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
name|addr
operator|=
operator|*
operator|(
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|in_addr
operator|*
argument_list|)
operator|)
expr_stmt|;
comment|/* 		 * INADDR_ANY is used to remove a previous selection. 		 * When no interface is selected, a default one is 		 * chosen every time a multicast packet is sent. 		 */
if|if
condition|(
name|addr
operator|.
name|s_addr
operator|==
name|INADDR_ANY
condition|)
block|{
name|imo
operator|->
name|imo_multicast_ifp
operator|=
name|NULL
expr_stmt|;
break|break;
block|}
comment|/* 		 * The selected interface is identified by its local 		 * IP address.  Find the interface and confirm that 		 * it supports multicasting. 		 */
name|INADDR_TO_IFP
argument_list|(
name|addr
argument_list|,
name|ifp
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifp
operator|==
name|NULL
operator|||
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_MULTICAST
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|EADDRNOTAVAIL
expr_stmt|;
break|break;
block|}
name|imo
operator|->
name|imo_multicast_ifp
operator|=
name|ifp
expr_stmt|;
break|break;
case|case
name|IP_MULTICAST_TTL
case|:
comment|/* 		 * Set the IP time-to-live for outgoing multicast packets. 		 */
if|if
condition|(
name|m
operator|==
name|NULL
operator|||
name|m
operator|->
name|m_len
operator|!=
literal|1
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
name|imo
operator|->
name|imo_multicast_ttl
operator|=
operator|*
operator|(
name|mtod
argument_list|(
name|m
argument_list|,
name|u_char
operator|*
argument_list|)
operator|)
expr_stmt|;
break|break;
case|case
name|IP_MULTICAST_LOOP
case|:
comment|/* 		 * Set the loopback flag for outgoing multicast packets. 		 * Must be zero or one. 		 */
if|if
condition|(
name|m
operator|==
name|NULL
operator|||
name|m
operator|->
name|m_len
operator|!=
literal|1
operator|||
operator|(
name|loop
operator|=
operator|*
operator|(
name|mtod
argument_list|(
name|m
argument_list|,
name|u_char
operator|*
argument_list|)
operator|)
operator|)
operator|>
literal|1
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
name|imo
operator|->
name|imo_multicast_loop
operator|=
name|loop
expr_stmt|;
break|break;
case|case
name|IP_ADD_MEMBERSHIP
case|:
comment|/* 		 * Add a multicast group membership. 		 * Group must be a valid IP multicast address. 		 */
if|if
condition|(
name|m
operator|==
name|NULL
operator|||
name|m
operator|->
name|m_len
operator|!=
sizeof|sizeof
argument_list|(
expr|struct
name|ip_mreq
argument_list|)
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
name|mreq
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip_mreq
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|IN_MULTICAST
argument_list|(
name|ntohl
argument_list|(
name|mreq
operator|->
name|imr_multiaddr
operator|.
name|s_addr
argument_list|)
argument_list|)
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
comment|/* 		 * If no interface address was provided, use the interface of 		 * the route to the given multicast address. 		 */
if|if
condition|(
name|mreq
operator|->
name|imr_interface
operator|.
name|s_addr
operator|==
name|INADDR_ANY
condition|)
block|{
name|ro
operator|.
name|ro_rt
operator|=
name|NULL
expr_stmt|;
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ro
operator|.
name|ro_dst
expr_stmt|;
name|dst
operator|->
name|sin_len
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|dst
argument_list|)
expr_stmt|;
name|dst
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|dst
operator|->
name|sin_addr
operator|=
name|mreq
operator|->
name|imr_multiaddr
expr_stmt|;
name|rtalloc
argument_list|(
operator|&
name|ro
argument_list|)
expr_stmt|;
if|if
condition|(
name|ro
operator|.
name|ro_rt
operator|==
name|NULL
condition|)
block|{
name|error
operator|=
name|EADDRNOTAVAIL
expr_stmt|;
break|break;
block|}
name|ifp
operator|=
name|ro
operator|.
name|ro_rt
operator|->
name|rt_ifp
expr_stmt|;
name|rtfree
argument_list|(
name|ro
operator|.
name|ro_rt
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|INADDR_TO_IFP
argument_list|(
name|mreq
operator|->
name|imr_interface
argument_list|,
name|ifp
argument_list|)
expr_stmt|;
block|}
comment|/* 		 * See if we found an interface, and confirm that it 		 * supports multicast. 		 */
if|if
condition|(
name|ifp
operator|==
name|NULL
operator|||
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_MULTICAST
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|EADDRNOTAVAIL
expr_stmt|;
break|break;
block|}
comment|/* 		 * See if the membership already exists or if all the 		 * membership slots are full. 		 */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|imo
operator|->
name|imo_num_memberships
condition|;
operator|++
name|i
control|)
block|{
if|if
condition|(
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
operator|->
name|inm_ifp
operator|==
name|ifp
operator|&&
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
operator|->
name|inm_addr
operator|.
name|s_addr
operator|==
name|mreq
operator|->
name|imr_multiaddr
operator|.
name|s_addr
condition|)
break|break;
block|}
if|if
condition|(
name|i
operator|<
name|imo
operator|->
name|imo_num_memberships
condition|)
block|{
name|error
operator|=
name|EADDRINUSE
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|i
operator|==
name|IP_MAX_MEMBERSHIPS
condition|)
block|{
name|error
operator|=
name|ETOOMANYREFS
expr_stmt|;
break|break;
block|}
comment|/* 		 * Everything looks good; add a new record to the multicast 		 * address list for the given interface. 		 */
if|if
condition|(
operator|(
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
operator|=
name|in_addmulti
argument_list|(
operator|&
name|mreq
operator|->
name|imr_multiaddr
argument_list|,
name|ifp
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
break|break;
block|}
operator|++
name|imo
operator|->
name|imo_num_memberships
expr_stmt|;
break|break;
case|case
name|IP_DROP_MEMBERSHIP
case|:
comment|/* 		 * Drop a multicast group membership. 		 * Group must be a valid IP multicast address. 		 */
if|if
condition|(
name|m
operator|==
name|NULL
operator|||
name|m
operator|->
name|m_len
operator|!=
sizeof|sizeof
argument_list|(
expr|struct
name|ip_mreq
argument_list|)
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
name|mreq
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip_mreq
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|IN_MULTICAST
argument_list|(
name|ntohl
argument_list|(
name|mreq
operator|->
name|imr_multiaddr
operator|.
name|s_addr
argument_list|)
argument_list|)
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
break|break;
block|}
comment|/* 		 * If an interface address was specified, get a pointer 		 * to its ifnet structure. 		 */
if|if
condition|(
name|mreq
operator|->
name|imr_interface
operator|.
name|s_addr
operator|==
name|INADDR_ANY
condition|)
name|ifp
operator|=
name|NULL
expr_stmt|;
else|else
block|{
name|INADDR_TO_IFP
argument_list|(
name|mreq
operator|->
name|imr_interface
argument_list|,
name|ifp
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifp
operator|==
name|NULL
condition|)
block|{
name|error
operator|=
name|EADDRNOTAVAIL
expr_stmt|;
break|break;
block|}
block|}
comment|/* 		 * Find the membership in the membership array. 		 */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|imo
operator|->
name|imo_num_memberships
condition|;
operator|++
name|i
control|)
block|{
if|if
condition|(
operator|(
name|ifp
operator|==
name|NULL
operator|||
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
operator|->
name|inm_ifp
operator|==
name|ifp
operator|)
operator|&&
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
operator|->
name|inm_addr
operator|.
name|s_addr
operator|==
name|mreq
operator|->
name|imr_multiaddr
operator|.
name|s_addr
condition|)
break|break;
block|}
if|if
condition|(
name|i
operator|==
name|imo
operator|->
name|imo_num_memberships
condition|)
block|{
name|error
operator|=
name|EADDRNOTAVAIL
expr_stmt|;
break|break;
block|}
comment|/* 		 * Give up the multicast address record to which the 		 * membership points. 		 */
name|in_delmulti
argument_list|(
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
argument_list|)
expr_stmt|;
comment|/* 		 * Remove the gap in the membership array. 		 */
for|for
control|(
operator|++
name|i
init|;
name|i
operator|<
name|imo
operator|->
name|imo_num_memberships
condition|;
operator|++
name|i
control|)
name|imo
operator|->
name|imo_membership
index|[
name|i
operator|-
literal|1
index|]
operator|=
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
expr_stmt|;
operator|--
name|imo
operator|->
name|imo_num_memberships
expr_stmt|;
break|break;
default|default:
name|error
operator|=
name|EOPNOTSUPP
expr_stmt|;
break|break;
block|}
comment|/* 	 * If all options have default values, no need to keep the mbuf. 	 */
if|if
condition|(
name|imo
operator|->
name|imo_multicast_ifp
operator|==
name|NULL
operator|&&
name|imo
operator|->
name|imo_multicast_ttl
operator|==
name|IP_DEFAULT_MULTICAST_TTL
operator|&&
name|imo
operator|->
name|imo_multicast_loop
operator|==
name|IP_DEFAULT_MULTICAST_LOOP
operator|&&
name|imo
operator|->
name|imo_num_memberships
operator|==
literal|0
condition|)
block|{
name|free
argument_list|(
operator|*
name|imop
argument_list|,
name|M_IPMOPTS
argument_list|)
expr_stmt|;
operator|*
name|imop
operator|=
name|NULL
expr_stmt|;
block|}
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Return the IP multicast options in response to user getsockopt().  */
end_comment

begin_function
name|int
name|ip_getmoptions
parameter_list|(
name|optname
parameter_list|,
name|imo
parameter_list|,
name|mp
parameter_list|)
name|int
name|optname
decl_stmt|;
specifier|register
name|struct
name|ip_moptions
modifier|*
name|imo
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
modifier|*
name|mp
decl_stmt|;
block|{
name|u_char
modifier|*
name|ttl
decl_stmt|;
name|u_char
modifier|*
name|loop
decl_stmt|;
name|struct
name|in_addr
modifier|*
name|addr
decl_stmt|;
name|struct
name|in_ifaddr
modifier|*
name|ia
decl_stmt|;
operator|*
name|mp
operator|=
name|m_get
argument_list|(
name|M_WAIT
argument_list|,
name|MT_SOOPTS
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|optname
condition|)
block|{
case|case
name|IP_MULTICAST_IF
case|:
name|addr
operator|=
name|mtod
argument_list|(
operator|*
name|mp
argument_list|,
expr|struct
name|in_addr
operator|*
argument_list|)
expr_stmt|;
operator|(
operator|*
name|mp
operator|)
operator|->
name|m_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|in_addr
argument_list|)
expr_stmt|;
if|if
condition|(
name|imo
operator|==
name|NULL
operator|||
name|imo
operator|->
name|imo_multicast_ifp
operator|==
name|NULL
condition|)
name|addr
operator|->
name|s_addr
operator|=
name|INADDR_ANY
expr_stmt|;
else|else
block|{
name|IFP_TO_IA
argument_list|(
name|imo
operator|->
name|imo_multicast_ifp
argument_list|,
name|ia
argument_list|)
expr_stmt|;
name|addr
operator|->
name|s_addr
operator|=
operator|(
name|ia
operator|==
name|NULL
operator|)
condition|?
name|INADDR_ANY
else|:
name|IA_SIN
argument_list|(
name|ia
argument_list|)
operator|->
name|sin_addr
operator|.
name|s_addr
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|IP_MULTICAST_TTL
case|:
name|ttl
operator|=
name|mtod
argument_list|(
operator|*
name|mp
argument_list|,
name|u_char
operator|*
argument_list|)
expr_stmt|;
operator|(
operator|*
name|mp
operator|)
operator|->
name|m_len
operator|=
literal|1
expr_stmt|;
operator|*
name|ttl
operator|=
operator|(
name|imo
operator|==
name|NULL
operator|)
condition|?
name|IP_DEFAULT_MULTICAST_TTL
else|:
name|imo
operator|->
name|imo_multicast_ttl
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|IP_MULTICAST_LOOP
case|:
name|loop
operator|=
name|mtod
argument_list|(
operator|*
name|mp
argument_list|,
name|u_char
operator|*
argument_list|)
expr_stmt|;
operator|(
operator|*
name|mp
operator|)
operator|->
name|m_len
operator|=
literal|1
expr_stmt|;
operator|*
name|loop
operator|=
operator|(
name|imo
operator|==
name|NULL
operator|)
condition|?
name|IP_DEFAULT_MULTICAST_LOOP
else|:
name|imo
operator|->
name|imo_multicast_loop
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
default|default:
return|return
operator|(
name|EOPNOTSUPP
operator|)
return|;
block|}
block|}
end_function

begin_comment
comment|/*  * Discard the IP multicast options.  */
end_comment

begin_function
name|void
name|ip_freemoptions
parameter_list|(
name|imo
parameter_list|)
specifier|register
name|struct
name|ip_moptions
modifier|*
name|imo
decl_stmt|;
block|{
specifier|register
name|int
name|i
decl_stmt|;
if|if
condition|(
name|imo
operator|!=
name|NULL
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|imo
operator|->
name|imo_num_memberships
condition|;
operator|++
name|i
control|)
name|in_delmulti
argument_list|(
name|imo
operator|->
name|imo_membership
index|[
name|i
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|imo
argument_list|,
name|M_IPMOPTS
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * Routine called from ip_output() to loop back a copy of an IP multicast  * packet to the input queue of a specified interface.  Note that this  * calls the output routine of the loopback "driver", but with an interface  * pointer that might NOT be&loif -- easier than replicating that code here.  */
end_comment

begin_function
specifier|static
name|void
name|ip_mloopback
parameter_list|(
name|ifp
parameter_list|,
name|m
parameter_list|,
name|dst
parameter_list|)
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
specifier|register
name|struct
name|sockaddr_in
modifier|*
name|dst
decl_stmt|;
block|{
specifier|register
name|struct
name|ip
modifier|*
name|ip
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|copym
decl_stmt|;
name|copym
operator|=
name|m_copy
argument_list|(
name|m
argument_list|,
literal|0
argument_list|,
name|M_COPYALL
argument_list|)
expr_stmt|;
if|if
condition|(
name|copym
operator|!=
name|NULL
condition|)
block|{
comment|/* 		 * We don't bother to fragment if the IP length is greater 		 * than the interface's MTU.  Can this possibly matter? 		 */
name|ip
operator|=
name|mtod
argument_list|(
name|copym
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_off
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|copym
argument_list|,
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|looutput
argument_list|(
name|ifp
argument_list|,
name|copym
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|dst
argument_list|)
expr_stmt|;
block|}
block|}
end_function

end_unit

