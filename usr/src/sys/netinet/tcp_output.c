begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982, 1986, 1988, 1990 Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)tcp_output.c	7.25 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_include
include|#
directive|include
file|<sys/protosw.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/socketvar.h>
end_include

begin_include
include|#
directive|include
file|<sys/errno.h>
end_include

begin_include
include|#
directive|include
file|<net/route.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_systm.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_pcb.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip_var.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp.h>
end_include

begin_define
define|#
directive|define
name|TCPOUTFLAGS
end_define

begin_include
include|#
directive|include
file|<netinet/tcp_fsm.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp_seq.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp_timer.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp_var.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcpip.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp_debug.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|notyet
end_ifdef

begin_function_decl
specifier|extern
name|struct
name|mbuf
modifier|*
name|m_copypack
parameter_list|()
function_decl|;
end_function_decl

begin_endif
endif|#
directive|endif
end_endif

begin_define
define|#
directive|define
name|MAX_TCPOPTLEN
value|32
end_define

begin_comment
comment|/* max # bytes that go in options */
end_comment

begin_comment
comment|/*  * Tcp output routine: figure out what should be sent and send it.  */
end_comment

begin_expr_stmt
name|tcp_output
argument_list|(
name|tp
argument_list|)
specifier|register
expr|struct
name|tcpcb
operator|*
name|tp
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|socket
modifier|*
name|so
init|=
name|tp
operator|->
name|t_inpcb
operator|->
name|inp_socket
decl_stmt|;
specifier|register
name|long
name|len
decl_stmt|,
name|win
decl_stmt|;
name|int
name|off
decl_stmt|,
name|flags
decl_stmt|,
name|error
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
specifier|register
name|struct
name|tcpiphdr
modifier|*
name|ti
decl_stmt|;
name|u_char
name|opt
index|[
name|MAX_TCPOPTLEN
index|]
decl_stmt|;
name|unsigned
name|optlen
decl_stmt|,
name|hdrlen
decl_stmt|;
name|int
name|idle
decl_stmt|,
name|sendalot
decl_stmt|;
comment|/* 	 * Determine length of data that should be transmitted, 	 * and flags that will be used. 	 * If there is some data or critical controls (SYN, RST) 	 * to send, then transmit; otherwise, investigate further. 	 */
name|idle
operator|=
operator|(
name|tp
operator|->
name|snd_max
operator|==
name|tp
operator|->
name|snd_una
operator|)
expr_stmt|;
if|if
condition|(
name|idle
operator|&&
name|tp
operator|->
name|t_idle
operator|>=
name|tp
operator|->
name|t_rxtcur
condition|)
comment|/* 		 * We have been idle for "a while" and no acks are 		 * expected to clock out any data we send -- 		 * slow start to get ack "clock" running again. 		 */
name|tp
operator|->
name|snd_cwnd
operator|=
name|tp
operator|->
name|t_maxseg
expr_stmt|;
name|again
label|:
name|sendalot
operator|=
literal|0
expr_stmt|;
name|off
operator|=
name|tp
operator|->
name|snd_nxt
operator|-
name|tp
operator|->
name|snd_una
expr_stmt|;
name|win
operator|=
name|min
argument_list|(
name|tp
operator|->
name|snd_wnd
argument_list|,
name|tp
operator|->
name|snd_cwnd
argument_list|)
expr_stmt|;
comment|/* 	 * If in persist timeout with window of 0, send 1 byte. 	 * Otherwise, if window is small but nonzero 	 * and timer expired, we will send what we can 	 * and go to transmit state. 	 */
if|if
condition|(
name|tp
operator|->
name|t_force
condition|)
block|{
if|if
condition|(
name|win
operator|==
literal|0
condition|)
name|win
operator|=
literal|1
expr_stmt|;
else|else
block|{
name|tp
operator|->
name|t_timer
index|[
name|TCPT_PERSIST
index|]
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|t_rxtshift
operator|=
literal|0
expr_stmt|;
block|}
block|}
name|flags
operator|=
name|tcp_outflags
index|[
name|tp
operator|->
name|t_state
index|]
expr_stmt|;
name|len
operator|=
name|min
argument_list|(
name|so
operator|->
name|so_snd
operator|.
name|sb_cc
argument_list|,
name|win
argument_list|)
operator|-
name|off
expr_stmt|;
if|if
condition|(
name|len
operator|<
literal|0
condition|)
block|{
comment|/* 		 * If FIN has been sent but not acked, 		 * but we haven't been called to retransmit, 		 * len will be -1.  Otherwise, window shrank 		 * after we sent into it.  If window shrank to 0, 		 * cancel pending retransmit and pull snd_nxt 		 * back to (closed) window.  We will enter persist 		 * state below.  If the window didn't close completely, 		 * just wait for an ACK. 		 */
name|len
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|win
operator|==
literal|0
condition|)
block|{
name|tp
operator|->
name|t_timer
index|[
name|TCPT_REXMT
index|]
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|snd_nxt
operator|=
name|tp
operator|->
name|snd_una
expr_stmt|;
block|}
block|}
if|if
condition|(
name|len
operator|>
name|tp
operator|->
name|t_maxseg
condition|)
block|{
name|len
operator|=
name|tp
operator|->
name|t_maxseg
expr_stmt|;
name|sendalot
operator|=
literal|1
expr_stmt|;
block|}
if|if
condition|(
name|SEQ_LT
argument_list|(
name|tp
operator|->
name|snd_nxt
operator|+
name|len
argument_list|,
name|tp
operator|->
name|snd_una
operator|+
name|so
operator|->
name|so_snd
operator|.
name|sb_cc
argument_list|)
condition|)
name|flags
operator|&=
operator|~
name|TH_FIN
expr_stmt|;
name|win
operator|=
name|sbspace
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
comment|/* 	 * Sender silly window avoidance.  If connection is idle 	 * and can send all data, a maximum segment, 	 * at least a maximum default-size segment do it, 	 * or are forced, do it; otherwise don't bother. 	 * If peer's buffer is tiny, then send 	 * when window is at least half open. 	 * If retransmitting (possibly after persist timer forced us 	 * to send into a small window), then must resend. 	 */
if|if
condition|(
name|len
condition|)
block|{
if|if
condition|(
name|len
operator|==
name|tp
operator|->
name|t_maxseg
condition|)
goto|goto
name|send
goto|;
if|if
condition|(
operator|(
name|idle
operator|||
name|tp
operator|->
name|t_flags
operator|&
name|TF_NODELAY
operator|)
operator|&&
name|len
operator|+
name|off
operator|>=
name|so
operator|->
name|so_snd
operator|.
name|sb_cc
condition|)
goto|goto
name|send
goto|;
if|if
condition|(
name|tp
operator|->
name|t_force
condition|)
goto|goto
name|send
goto|;
if|if
condition|(
name|len
operator|>=
name|tp
operator|->
name|max_sndwnd
operator|/
literal|2
condition|)
goto|goto
name|send
goto|;
if|if
condition|(
name|SEQ_LT
argument_list|(
name|tp
operator|->
name|snd_nxt
argument_list|,
name|tp
operator|->
name|snd_max
argument_list|)
condition|)
goto|goto
name|send
goto|;
block|}
comment|/* 	 * Compare available window to amount of window 	 * known to peer (as advertised window less 	 * next expected input).  If the difference is at least two 	 * max size segments, or at least 50% of the maximum possible 	 * window, then want to send a window update to peer. 	 */
if|if
condition|(
name|win
operator|>
literal|0
condition|)
block|{
comment|/*  		 * "adv" is the amount we can increase the window, 		 * taking into account that we are limited by 		 * TCP_MAXWIN<< tp->rcv_scale. 		 */
name|long
name|adv
init|=
name|min
argument_list|(
name|win
argument_list|,
operator|(
name|long
operator|)
name|TCP_MAXWIN
operator|<<
name|tp
operator|->
name|rcv_scale
argument_list|)
operator|-
operator|(
name|tp
operator|->
name|rcv_adv
operator|-
name|tp
operator|->
name|rcv_nxt
operator|)
decl_stmt|;
if|if
condition|(
name|adv
operator|>=
call|(
name|long
call|)
argument_list|(
literal|2
operator|*
name|tp
operator|->
name|t_maxseg
argument_list|)
condition|)
goto|goto
name|send
goto|;
if|if
condition|(
literal|2
operator|*
name|adv
operator|>=
operator|(
name|long
operator|)
name|so
operator|->
name|so_rcv
operator|.
name|sb_hiwat
condition|)
goto|goto
name|send
goto|;
block|}
comment|/* 	 * Send if we owe peer an ACK. 	 */
if|if
condition|(
name|tp
operator|->
name|t_flags
operator|&
name|TF_ACKNOW
condition|)
goto|goto
name|send
goto|;
if|if
condition|(
name|flags
operator|&
operator|(
name|TH_SYN
operator||
name|TH_RST
operator|)
condition|)
goto|goto
name|send
goto|;
if|if
condition|(
name|SEQ_GT
argument_list|(
name|tp
operator|->
name|snd_up
argument_list|,
name|tp
operator|->
name|snd_una
argument_list|)
condition|)
goto|goto
name|send
goto|;
comment|/* 	 * If our state indicates that FIN should be sent 	 * and we have not yet done so, or we're retransmitting the FIN, 	 * then we need to send. 	 */
if|if
condition|(
name|flags
operator|&
name|TH_FIN
operator|&&
operator|(
operator|(
name|tp
operator|->
name|t_flags
operator|&
name|TF_SENTFIN
operator|)
operator|==
literal|0
operator|||
name|tp
operator|->
name|snd_nxt
operator|==
name|tp
operator|->
name|snd_una
operator|)
condition|)
goto|goto
name|send
goto|;
comment|/* 	 * TCP window updates are not reliable, rather a polling protocol 	 * using ``persist'' packets is used to insure receipt of window 	 * updates.  The three ``states'' for the output side are: 	 *	idle			not doing retransmits or persists 	 *	persisting		to move a small or zero window 	 *	(re)transmitting	and thereby not persisting 	 * 	 * tp->t_timer[TCPT_PERSIST] 	 *	is set when we are in persist state. 	 * tp->t_force 	 *	is set when we are called to send a persist packet. 	 * tp->t_timer[TCPT_REXMT] 	 *	is set when we are retransmitting 	 * The output side is idle when both timers are zero. 	 * 	 * If send window is too small, there is data to transmit, and no 	 * retransmit or persist is pending, then go to persist state. 	 * If nothing happens soon, send when timer expires: 	 * if window is nonzero, transmit what we can, 	 * otherwise force out a byte. 	 */
if|if
condition|(
name|so
operator|->
name|so_snd
operator|.
name|sb_cc
operator|&&
name|tp
operator|->
name|t_timer
index|[
name|TCPT_REXMT
index|]
operator|==
literal|0
operator|&&
name|tp
operator|->
name|t_timer
index|[
name|TCPT_PERSIST
index|]
operator|==
literal|0
condition|)
block|{
name|tp
operator|->
name|t_rxtshift
operator|=
literal|0
expr_stmt|;
name|tcp_setpersist
argument_list|(
name|tp
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * No reason to send a segment, just return. 	 */
return|return
operator|(
literal|0
operator|)
return|;
name|send
label|:
comment|/* 	 * Before ESTABLISHED, force sending of initial options 	 * unless TCP set not to do any options. 	 * NOTE: we assume that the IP/TCP header plus TCP options 	 * always fit in a single mbuf, leaving room for a maximum 	 * link header, i.e. 	 *	max_linkhdr + sizeof (struct tcpiphdr) + optlen<= MHLEN 	 */
name|optlen
operator|=
literal|0
expr_stmt|;
name|hdrlen
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|tcpiphdr
argument_list|)
expr_stmt|;
if|if
condition|(
name|flags
operator|&
name|TH_SYN
operator|&&
operator|(
name|tp
operator|->
name|t_flags
operator|&
name|TF_NOOPT
operator|)
operator|==
literal|0
condition|)
block|{
name|u_short
name|mss
decl_stmt|;
name|opt
index|[
literal|0
index|]
operator|=
name|TCPOPT_MAXSEG
expr_stmt|;
name|opt
index|[
literal|1
index|]
operator|=
literal|4
expr_stmt|;
name|mss
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|tcp_mss
argument_list|(
name|tp
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|mss
argument_list|,
call|(
name|caddr_t
call|)
argument_list|(
name|opt
operator|+
literal|2
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
name|mss
argument_list|)
argument_list|)
expr_stmt|;
name|optlen
operator|=
literal|4
expr_stmt|;
if|if
condition|(
operator|(
name|tp
operator|->
name|t_flags
operator|&
name|TF_REQ_SCALE
operator|)
operator|&&
operator|(
operator|(
name|flags
operator|&
name|TH_ACK
operator|)
operator|==
literal|0
operator|||
operator|(
name|tp
operator|->
name|t_flags
operator|&
name|TF_RCVD_SCALE
operator|)
operator|)
condition|)
block|{
operator|*
operator|(
operator|(
name|u_long
operator|*
operator|)
operator|(
name|opt
operator|+
name|optlen
operator|)
operator|)
operator|=
name|htonl
argument_list|(
name|TCPOPT_NOP
operator|<<
literal|24
operator||
name|TCPOPT_WINDOW
operator|<<
literal|16
operator||
name|TCPOLEN_WINDOW
operator|<<
literal|8
operator||
name|tp
operator|->
name|request_r_scale
argument_list|)
expr_stmt|;
name|optlen
operator|+=
literal|4
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|DO_SACK
comment|/* Send a SACK_PERMITTED option in the SYN segment. */
operator|*
operator|(
operator|(
name|u_long
operator|*
operator|)
operator|(
name|opt
operator|+
name|optlen
operator|)
operator|)
operator|=
name|htonl
argument_list|(
name|TCPOPT_NOP
operator|<<
literal|24
operator||
name|TCPOPT_NOP
operator|<<
literal|16
operator||
name|TCPOPT_SACK_PERMITTED
operator|<<
literal|8
operator||
name|TCPOLEN_SACK_PERMITTED
argument_list|)
expr_stmt|;
name|optlen
operator|+=
literal|4
expr_stmt|;
endif|#
directive|endif
block|}
comment|/* 	 * Send a timestamp and echo-reply if this is a SYN and our side  	 * wants to use timestamps (TF_REQ_TSTMP is set) or both our side 	 * and our peer have sent timestamps in our SYN's.  	 */
if|if
condition|(
operator|(
name|tp
operator|->
name|t_flags
operator|&
operator|(
name|TF_REQ_TSTMP
operator||
name|TF_NOOPT
operator|)
operator|)
operator|==
name|TF_REQ_TSTMP
operator|&&
operator|(
name|flags
operator|&
name|TH_RST
operator|)
operator|==
literal|0
operator|&&
operator|(
operator|(
name|flags
operator|&
operator|(
name|TH_SYN
operator||
name|TH_ACK
operator|)
operator|)
operator|==
name|TH_SYN
operator|||
operator|(
name|tp
operator|->
name|t_flags
operator|&
name|TF_RCVD_TSTMP
operator|)
operator|)
condition|)
block|{
name|u_long
modifier|*
name|lp
init|=
operator|(
name|u_long
operator|*
operator|)
operator|(
name|opt
operator|+
name|optlen
operator|)
decl_stmt|;
comment|/* Form timestamp option as shown in appendix A of RFC 1323. */
operator|*
name|lp
operator|++
operator|=
name|htonl
argument_list|(
name|TCPOPT_TSTAMP_HDR
argument_list|)
expr_stmt|;
operator|*
name|lp
operator|++
operator|=
name|htonl
argument_list|(
name|tcp_now
argument_list|)
expr_stmt|;
operator|*
name|lp
operator|=
name|htonl
argument_list|(
name|tp
operator|->
name|ts_recent
argument_list|)
expr_stmt|;
name|optlen
operator|+=
name|TCPOLEN_TSTAMP_APPA
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|DO_SACK
comment|/* Send SACK if needed. Don't tack a SACK onto a non-empty segment. */
if|if
condition|(
name|tp
operator|->
name|seg_next
operator|!=
operator|(
expr|struct
name|tcpiphdr
operator|*
operator|)
name|tp
operator|&&
name|len
operator|==
literal|0
operator|&&
operator|(
name|tp
operator|->
name|t_flags
operator|&
operator|(
name|TF_SACK_PERMIT
operator||
name|TF_NOOPT
operator|)
operator|)
operator|==
name|TF_SACK_PERMIT
condition|)
block|{
specifier|register
name|struct
name|tcpiphdr
modifier|*
name|q
init|=
name|tp
operator|->
name|seg_next
decl_stmt|;
specifier|register
name|u_long
modifier|*
name|optl
init|=
operator|(
name|u_long
operator|*
operator|)
operator|(
name|opt
operator|+
name|optlen
operator|)
operator|+
literal|1
decl_stmt|;
name|tcp_seq
name|block_start
decl_stmt|;
name|u_long
name|block_size
decl_stmt|;
name|int
name|sack_len
init|=
literal|2
decl_stmt|;
comment|/* 		 * Use these to gather runs of received segments.  		 * The first segment in the queue becomes the initial run.  		 */
name|block_start
operator|=
name|q
operator|->
name|ti_seq
expr_stmt|;
name|block_size
operator|=
name|q
operator|->
name|ti_len
expr_stmt|;
do|do
block|{
name|q
operator|=
operator|(
expr|struct
name|tcpiphdr
operator|*
operator|)
name|q
operator|->
name|ti_next
expr_stmt|;
if|if
condition|(
name|q
operator|==
operator|(
expr|struct
name|tcpiphdr
operator|*
operator|)
name|tp
operator|||
name|block_start
operator|+
name|block_size
operator|!=
name|q
operator|->
name|ti_seq
condition|)
block|{
comment|/* 				 * Stick the relative origin and block size  				 * in the SACK option.  				 */
operator|*
name|optl
operator|++
operator|=
name|htonl
argument_list|(
name|block_start
operator|-
name|tp
operator|->
name|rcv_nxt
argument_list|)
expr_stmt|;
operator|*
name|optl
operator|++
operator|=
name|htonl
argument_list|(
name|block_size
argument_list|)
expr_stmt|;
name|sack_len
operator|+=
literal|8
expr_stmt|;
comment|/* If no more will fit into options, quit. */
if|if
condition|(
name|sack_len
operator|+
name|optlen
operator|>
name|MAX_TCPOPTLEN
operator|-
literal|8
condition|)
break|break;
if|if
condition|(
name|q
operator|!=
operator|(
expr|struct
name|tcpiphdr
operator|*
operator|)
name|tp
condition|)
block|{
name|block_start
operator|=
name|q
operator|->
name|ti_seq
expr_stmt|;
name|block_size
operator|=
name|q
operator|->
name|ti_len
expr_stmt|;
block|}
block|}
else|else
block|{
comment|/*  				 * This segment just accumulates into previous  				 * run.  				 */
name|block_size
operator|+=
name|q
operator|->
name|ti_len
expr_stmt|;
block|}
block|}
do|while
condition|(
name|q
operator|!=
operator|(
expr|struct
name|tcpiphdr
operator|*
operator|)
name|tp
condition|)
do|;
operator|*
operator|(
operator|(
name|u_long
operator|*
operator|)
operator|(
name|opt
operator|+
name|optlen
operator|)
operator|)
operator|=
name|htonl
argument_list|(
name|TCPOPT_NOP
operator|<<
literal|24
operator||
name|TCPOPT_NOP
operator|<<
literal|16
operator||
name|TCPOPT_SACK
operator|<<
literal|8
operator||
name|sack_len
argument_list|)
expr_stmt|;
name|optlen
operator|+=
name|sack_len
operator|+
literal|2
expr_stmt|;
block|}
endif|#
directive|endif
name|hdrlen
operator|+=
name|optlen
expr_stmt|;
comment|/* 	 * Adjust data length if insertion of options will 	 * bump the packet length beyond the t_maxseg length. 	 */
if|if
condition|(
name|len
operator|>
name|tp
operator|->
name|t_maxseg
operator|-
name|optlen
condition|)
block|{
name|len
operator|=
name|tp
operator|->
name|t_maxseg
operator|-
name|optlen
expr_stmt|;
name|sendalot
operator|=
literal|1
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|DIAGNOSTIC
if|if
condition|(
name|max_linkhdr
operator|+
name|hdrlen
operator|>
name|MHLEN
condition|)
name|panic
argument_list|(
literal|"tcphdr too big"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* 	 * Grab a header mbuf, attaching a copy of data to 	 * be transmitted, and initialize the header from 	 * the template for sends on this connection. 	 */
if|if
condition|(
name|len
condition|)
block|{
if|if
condition|(
name|tp
operator|->
name|t_force
operator|&&
name|len
operator|==
literal|1
condition|)
name|tcpstat
operator|.
name|tcps_sndprobe
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|SEQ_LT
argument_list|(
name|tp
operator|->
name|snd_nxt
argument_list|,
name|tp
operator|->
name|snd_max
argument_list|)
condition|)
block|{
name|tcpstat
operator|.
name|tcps_sndrexmitpack
operator|++
expr_stmt|;
name|tcpstat
operator|.
name|tcps_sndrexmitbyte
operator|+=
name|len
expr_stmt|;
block|}
else|else
block|{
name|tcpstat
operator|.
name|tcps_sndpack
operator|++
expr_stmt|;
name|tcpstat
operator|.
name|tcps_sndbyte
operator|+=
name|len
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|notyet
if|if
condition|(
operator|(
name|m
operator|=
name|m_copypack
argument_list|(
name|so
operator|->
name|so_snd
operator|.
name|sb_mb
argument_list|,
name|off
argument_list|,
operator|(
name|int
operator|)
name|len
argument_list|,
name|max_linkhdr
operator|+
name|hdrlen
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|out
goto|;
block|}
comment|/* 		 * m_copypack left space for our hdr; use it. 		 */
name|m
operator|->
name|m_len
operator|+=
name|hdrlen
expr_stmt|;
name|m
operator|->
name|m_data
operator|-=
name|hdrlen
expr_stmt|;
else|#
directive|else
name|MGETHDR
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|out
goto|;
block|}
name|m
operator|->
name|m_data
operator|+=
name|max_linkhdr
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
name|hdrlen
expr_stmt|;
if|if
condition|(
name|len
operator|<=
name|MHLEN
operator|-
name|hdrlen
operator|-
name|max_linkhdr
condition|)
block|{
if|if
condition|(
name|m
operator|->
name|m_next
operator|==
literal|0
condition|)
name|len
operator|=
literal|0
expr_stmt|;
block|}
endif|#
directive|endif
comment|/* 		 * If we're sending everything we've got, set PUSH. 		 * (This will keep happy those implementations which only 		 * give data to the user when a buffer fills or 		 * a PUSH comes in.) 		 */
if|if
condition|(
name|off
operator|+
name|len
operator|==
name|so
operator|->
name|so_snd
operator|.
name|sb_cc
condition|)
name|flags
operator||=
name|TH_PUSH
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|tp
operator|->
name|t_flags
operator|&
name|TF_ACKNOW
condition|)
name|tcpstat
operator|.
name|tcps_sndacks
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|flags
operator|&
operator|(
name|TH_SYN
operator||
name|TH_FIN
operator||
name|TH_RST
operator|)
condition|)
name|tcpstat
operator|.
name|tcps_sndctrl
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|SEQ_GT
argument_list|(
name|tp
operator|->
name|snd_up
argument_list|,
name|tp
operator|->
name|snd_una
argument_list|)
condition|)
name|tcpstat
operator|.
name|tcps_sndurg
operator|++
expr_stmt|;
else|else
name|tcpstat
operator|.
name|tcps_sndwinup
operator|++
expr_stmt|;
name|MGETHDR
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|out
goto|;
block|}
name|m
operator|->
name|m_data
operator|+=
name|max_linkhdr
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
name|hdrlen
expr_stmt|;
block|}
name|m
operator|->
name|m_pkthdr
operator|.
name|rcvif
operator|=
operator|(
expr|struct
name|ifnet
operator|*
operator|)
literal|0
expr_stmt|;
name|ti
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|tcpiphdr
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|t_template
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"tcp_output"
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|tp
operator|->
name|t_template
argument_list|,
operator|(
name|caddr_t
operator|)
name|ti
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|tcpiphdr
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 	 * Fill in fields, remembering maximum advertised 	 * window for use in delaying messages about window sizes. 	 * If resending a FIN, be sure not to use a new sequence number. 	 */
if|if
condition|(
name|flags
operator|&
name|TH_FIN
operator|&&
name|tp
operator|->
name|t_flags
operator|&
name|TF_SENTFIN
operator|&&
name|tp
operator|->
name|snd_nxt
operator|==
name|tp
operator|->
name|snd_max
condition|)
name|tp
operator|->
name|snd_nxt
operator|--
expr_stmt|;
name|ti
operator|->
name|ti_seq
operator|=
name|htonl
argument_list|(
name|tp
operator|->
name|snd_nxt
argument_list|)
expr_stmt|;
name|ti
operator|->
name|ti_ack
operator|=
name|htonl
argument_list|(
name|tp
operator|->
name|rcv_nxt
argument_list|)
expr_stmt|;
if|if
condition|(
name|optlen
condition|)
block|{
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|opt
argument_list|,
call|(
name|caddr_t
call|)
argument_list|(
name|ti
operator|+
literal|1
argument_list|)
argument_list|,
name|optlen
argument_list|)
expr_stmt|;
name|ti
operator|->
name|ti_off
operator|=
operator|(
sizeof|sizeof
argument_list|(
expr|struct
name|tcphdr
argument_list|)
operator|+
name|optlen
operator|)
operator|>>
literal|2
expr_stmt|;
block|}
name|ti
operator|->
name|ti_flags
operator|=
name|flags
expr_stmt|;
comment|/* 	 * Calculate receive window.  Don't shrink window, 	 * but avoid silly window syndrome. 	 */
if|if
condition|(
name|win
operator|<
call|(
name|long
call|)
argument_list|(
name|so
operator|->
name|so_rcv
operator|.
name|sb_hiwat
operator|/
literal|4
argument_list|)
operator|&&
name|win
operator|<
operator|(
name|long
operator|)
name|tp
operator|->
name|t_maxseg
condition|)
name|win
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|win
operator|>
operator|(
name|long
operator|)
name|TCP_MAXWIN
operator|<<
name|tp
operator|->
name|rcv_scale
condition|)
name|win
operator|=
operator|(
name|long
operator|)
name|TCP_MAXWIN
operator|<<
name|tp
operator|->
name|rcv_scale
expr_stmt|;
if|if
condition|(
name|win
operator|<
call|(
name|long
call|)
argument_list|(
name|tp
operator|->
name|rcv_adv
operator|-
name|tp
operator|->
name|rcv_nxt
argument_list|)
condition|)
name|win
operator|=
call|(
name|long
call|)
argument_list|(
name|tp
operator|->
name|rcv_adv
operator|-
name|tp
operator|->
name|rcv_nxt
argument_list|)
expr_stmt|;
if|if
condition|(
name|win
operator|>
name|IP_MAXPACKET
condition|)
name|win
operator|=
name|IP_MAXPACKET
expr_stmt|;
name|ti
operator|->
name|ti_win
operator|=
name|htons
argument_list|(
call|(
name|u_short
call|)
argument_list|(
name|win
operator|>>
name|tp
operator|->
name|rcv_scale
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|SEQ_GT
argument_list|(
name|tp
operator|->
name|snd_up
argument_list|,
name|tp
operator|->
name|snd_nxt
argument_list|)
condition|)
block|{
name|ti
operator|->
name|ti_urp
operator|=
name|htons
argument_list|(
call|(
name|u_short
call|)
argument_list|(
name|tp
operator|->
name|snd_up
operator|-
name|tp
operator|->
name|snd_nxt
argument_list|)
argument_list|)
expr_stmt|;
name|ti
operator|->
name|ti_flags
operator||=
name|TH_URG
expr_stmt|;
block|}
else|else
comment|/* 		 * If no urgent pointer to send, then we pull 		 * the urgent pointer to the left edge of the send window 		 * so that it doesn't drift into the send window on sequence 		 * number wraparound. 		 */
name|tp
operator|->
name|snd_up
operator|=
name|tp
operator|->
name|snd_una
expr_stmt|;
comment|/* drag it along */
comment|/* 	 * Put TCP length in extended header, and then 	 * checksum extended header and data. 	 */
if|if
condition|(
name|len
operator|+
name|optlen
condition|)
name|ti
operator|->
name|ti_len
operator|=
name|htons
argument_list|(
call|(
name|u_short
call|)
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|tcphdr
argument_list|)
operator|+
name|optlen
operator|+
name|len
argument_list|)
argument_list|)
expr_stmt|;
name|ti
operator|->
name|ti_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
call|(
name|int
call|)
argument_list|(
name|hdrlen
operator|+
name|len
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 	 * In transmit state, time the transmission and arrange for 	 * the retransmit.  In persist state, just set snd_max. 	 */
if|if
condition|(
name|tp
operator|->
name|t_force
operator|==
literal|0
operator|||
name|tp
operator|->
name|t_timer
index|[
name|TCPT_PERSIST
index|]
operator|==
literal|0
condition|)
block|{
name|tcp_seq
name|startseq
init|=
name|tp
operator|->
name|snd_nxt
decl_stmt|;
comment|/* 		 * Advance snd_nxt over sequence space of this segment. 		 */
if|if
condition|(
name|flags
operator|&
operator|(
name|TH_SYN
operator||
name|TH_FIN
operator|)
condition|)
block|{
if|if
condition|(
name|flags
operator|&
name|TH_SYN
condition|)
name|tp
operator|->
name|snd_nxt
operator|++
expr_stmt|;
if|if
condition|(
name|flags
operator|&
name|TH_FIN
condition|)
block|{
name|tp
operator|->
name|snd_nxt
operator|++
expr_stmt|;
name|tp
operator|->
name|t_flags
operator||=
name|TF_SENTFIN
expr_stmt|;
block|}
block|}
name|tp
operator|->
name|snd_nxt
operator|+=
name|len
expr_stmt|;
if|if
condition|(
name|SEQ_GT
argument_list|(
name|tp
operator|->
name|snd_nxt
argument_list|,
name|tp
operator|->
name|snd_max
argument_list|)
condition|)
block|{
name|tp
operator|->
name|snd_max
operator|=
name|tp
operator|->
name|snd_nxt
expr_stmt|;
comment|/* 			 * Time this transmission if not a retransmission and 			 * not currently timing anything. 			 */
if|if
condition|(
name|tp
operator|->
name|t_rtt
operator|==
literal|0
condition|)
block|{
name|tp
operator|->
name|t_rtt
operator|=
literal|1
expr_stmt|;
name|tp
operator|->
name|t_rtseq
operator|=
name|startseq
expr_stmt|;
name|tcpstat
operator|.
name|tcps_segstimed
operator|++
expr_stmt|;
block|}
block|}
comment|/* 		 * Set retransmit timer if not currently set, 		 * and not doing an ack or a keep-alive probe. 		 * Initial value for retransmit timer is smoothed 		 * round-trip time + 2 * round-trip time variance. 		 * Initialize shift counter which is used for backoff 		 * of retransmit time. 		 */
if|if
condition|(
name|tp
operator|->
name|t_timer
index|[
name|TCPT_REXMT
index|]
operator|==
literal|0
operator|&&
name|tp
operator|->
name|snd_nxt
operator|!=
name|tp
operator|->
name|snd_una
condition|)
block|{
name|tp
operator|->
name|t_timer
index|[
name|TCPT_REXMT
index|]
operator|=
name|tp
operator|->
name|t_rxtcur
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|t_timer
index|[
name|TCPT_PERSIST
index|]
condition|)
block|{
name|tp
operator|->
name|t_timer
index|[
name|TCPT_PERSIST
index|]
operator|=
literal|0
expr_stmt|;
name|tp
operator|->
name|t_rxtshift
operator|=
literal|0
expr_stmt|;
block|}
block|}
block|}
elseif|else
if|if
condition|(
name|SEQ_GT
argument_list|(
name|tp
operator|->
name|snd_nxt
operator|+
name|len
argument_list|,
name|tp
operator|->
name|snd_max
argument_list|)
condition|)
name|tp
operator|->
name|snd_max
operator|=
name|tp
operator|->
name|snd_nxt
operator|+
name|len
expr_stmt|;
comment|/* 	 * Trace. 	 */
if|if
condition|(
name|so
operator|->
name|so_options
operator|&
name|SO_DEBUG
condition|)
name|tcp_trace
argument_list|(
name|TA_OUTPUT
argument_list|,
name|tp
operator|->
name|t_state
argument_list|,
name|tp
argument_list|,
name|ti
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* 	 * Fill in IP length and desired time to live and 	 * send to IP level.  There should be a better way 	 * to handle ttl and tos; we could keep them in 	 * the template, but need a way to checksum without them. 	 */
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|hdrlen
operator|+
name|len
expr_stmt|;
operator|(
operator|(
expr|struct
name|ip
operator|*
operator|)
name|ti
operator|)
operator|->
name|ip_len
operator|=
name|m
operator|->
name|m_pkthdr
operator|.
name|len
expr_stmt|;
operator|(
operator|(
expr|struct
name|ip
operator|*
operator|)
name|ti
operator|)
operator|->
name|ip_ttl
operator|=
name|tp
operator|->
name|t_inpcb
operator|->
name|inp_ip
operator|.
name|ip_ttl
expr_stmt|;
comment|/* XXX */
operator|(
operator|(
expr|struct
name|ip
operator|*
operator|)
name|ti
operator|)
operator|->
name|ip_tos
operator|=
name|tp
operator|->
name|t_inpcb
operator|->
name|inp_ip
operator|.
name|ip_tos
expr_stmt|;
comment|/* XXX */
if|#
directive|if
name|BSD
operator|>=
literal|43
if|#
directive|if
name|BSD
operator|>=
literal|43
name|error
operator|=
name|ip_output
argument_list|(
name|m
argument_list|,
name|tp
operator|->
name|t_inpcb
operator|->
name|inp_options
argument_list|,
operator|&
name|tp
operator|->
name|t_inpcb
operator|->
name|inp_route
argument_list|,
name|so
operator|->
name|so_options
operator|&
name|SO_DONTROUTE
argument_list|,
literal|0
argument_list|)
expr_stmt|;
else|#
directive|else
name|error
operator|=
name|ip_output
argument_list|(
name|m
argument_list|,
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
argument_list|,
operator|&
name|tp
operator|->
name|t_inpcb
operator|->
name|inp_route
argument_list|,
name|so
operator|->
name|so_options
operator|&
name|SO_DONTROUTE
argument_list|)
expr_stmt|;
endif|#
directive|endif
else|#
directive|else
name|error
operator|=
name|ip_output
argument_list|(
name|m
argument_list|,
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
argument_list|,
operator|&
name|tp
operator|->
name|t_inpcb
operator|->
name|inp_route
argument_list|,
name|so
operator|->
name|so_options
operator|&
name|SO_DONTROUTE
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|error
condition|)
block|{
name|out
label|:
if|if
condition|(
name|error
operator|==
name|ENOBUFS
condition|)
block|{
name|tcp_quench
argument_list|(
name|tp
operator|->
name|t_inpcb
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
operator|(
name|error
operator|==
name|EHOSTUNREACH
operator|||
name|error
operator|==
name|ENETDOWN
operator|)
operator|&&
name|TCPS_HAVERCVDSYN
argument_list|(
name|tp
operator|->
name|t_state
argument_list|)
condition|)
block|{
name|tp
operator|->
name|t_softerror
operator|=
name|error
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
return|return
operator|(
name|error
operator|)
return|;
block|}
name|tcpstat
operator|.
name|tcps_sndtotal
operator|++
expr_stmt|;
comment|/* 	 * Data sent (as far as we can tell). 	 * If this advertises a larger window than any other segment, 	 * then remember the size of the advertised window. 	 * Any pending ACK has now been sent. 	 */
if|if
condition|(
name|win
operator|>
literal|0
operator|&&
name|SEQ_GT
argument_list|(
name|tp
operator|->
name|rcv_nxt
operator|+
name|win
argument_list|,
name|tp
operator|->
name|rcv_adv
argument_list|)
condition|)
name|tp
operator|->
name|rcv_adv
operator|=
name|tp
operator|->
name|rcv_nxt
operator|+
name|win
expr_stmt|;
name|tp
operator|->
name|last_ack_sent
operator|=
name|tp
operator|->
name|rcv_nxt
expr_stmt|;
name|tp
operator|->
name|t_flags
operator|&=
operator|~
operator|(
name|TF_ACKNOW
operator||
name|TF_DELACK
operator|)
expr_stmt|;
if|if
condition|(
name|sendalot
condition|)
goto|goto
name|again
goto|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_expr_stmt
name|tcp_setpersist
argument_list|(
name|tp
argument_list|)
specifier|register
expr|struct
name|tcpcb
operator|*
name|tp
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|t
operator|=
operator|(
operator|(
name|tp
operator|->
name|t_srtt
operator|>>
literal|2
operator|)
operator|+
name|tp
operator|->
name|t_rttvar
operator|)
operator|>>
literal|1
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|t_timer
index|[
name|TCPT_REXMT
index|]
condition|)
name|panic
argument_list|(
literal|"tcp_output REXMT"
argument_list|)
expr_stmt|;
comment|/* 	 * Start/restart persistance timer. 	 */
name|TCPT_RANGESET
argument_list|(
name|tp
operator|->
name|t_timer
index|[
name|TCPT_PERSIST
index|]
argument_list|,
name|t
operator|*
name|tcp_backoff
index|[
name|tp
operator|->
name|t_rxtshift
index|]
argument_list|,
name|TCPTV_PERSMIN
argument_list|,
name|TCPTV_PERSMAX
argument_list|)
expr_stmt|;
if|if
condition|(
name|tp
operator|->
name|t_rxtshift
operator|<
name|TCP_MAXRXTSHIFT
condition|)
name|tp
operator|->
name|t_rxtshift
operator|++
expr_stmt|;
block|}
end_block

end_unit

