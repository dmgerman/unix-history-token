...
literal|0
expr_stmt|;
comment|/* 	 * Handle end of file first.  If we are appending, we need to check 	 * if the current block was allocated contiguously.  If it wasn't, 	 * then we need to fire off a previous cluster if it existed. 	 * Additionally, when we're appending, we need to figure out how 	 * to initialize vp->v_clen. 	 */
if|if
condition|(
operator|(
name|lbn
...
condition|)
block|{
comment|/* This block was not allocated contiguously */
if|if
condition|(
name|vp
operator|->
...
literal|1
expr_stmt|;
comment|/* 			 * Next cluster started. Write this buffer and return. 			 */
name|vp
operator|->
name|v_lastw
operator|=
