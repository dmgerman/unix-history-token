...
begin_comment
comment|/*  * Copyright (c) 1982, 1986 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)kern_descrip.c	7.3 (Berkeley) %G%  */
end_comment
...
name|rwmode
decl_stmt|;
comment|/* 	 * Note the horrid kludge here: u.u_r.r_val1 contains the value 	 * of the new file descriptor, which has not been disturbed since 	 * it was allocated. 	 */
if|if
condition|(
operator|(
name|fp
...
operator|)
return|;
comment|/* 	 * Delete references to this pseudo-device. 	 * Note that fp->f_count is guaranteed == 1, and 	 * that fp references the inode for this driver. 	 */
if|if
condition|(
name|fp
operator|->
