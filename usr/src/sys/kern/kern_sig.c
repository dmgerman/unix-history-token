...
begin_comment
comment|/*  * Copyright (c) 1982, 1986, 1989 Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)kern_sig.c	7.25 (Berkeley) %G%  */
end_comment
...
begin_comment
comment|/*  * Can process p send the signal signo to process q?  */
end_comment
...
parameter_list|)
define|\
value|((p)->p_uid == 0 || \ 	    (p)->p_ruid == (q)->p_ruid || (p)->p_uid == (q)->p_ruid || \ 	    (p)->p_ruid == (q)->p_uid || (p)->p_uid == (q)->p_uid || \ 	    ((signo) == SIGCONT&& (q)->p_session == (p)->p_session))
end_define
