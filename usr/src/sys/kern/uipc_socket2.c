begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)uipc_socket2.c	6.10 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"systm.h"
end_include

begin_include
include|#
directive|include
file|"dir.h"
end_include

begin_include
include|#
directive|include
file|"user.h"
end_include

begin_include
include|#
directive|include
file|"proc.h"
end_include

begin_include
include|#
directive|include
file|"file.h"
end_include

begin_include
include|#
directive|include
file|"inode.h"
end_include

begin_include
include|#
directive|include
file|"buf.h"
end_include

begin_include
include|#
directive|include
file|"mbuf.h"
end_include

begin_include
include|#
directive|include
file|"protosw.h"
end_include

begin_include
include|#
directive|include
file|"socket.h"
end_include

begin_include
include|#
directive|include
file|"socketvar.h"
end_include

begin_comment
comment|/*  * Primitive routines for operating on sockets and socket buffers  */
end_comment

begin_comment
comment|/*  * Procedures to manipulate state flags of socket  * and do appropriate wakeups.  Normal sequence from the  * active (originating) side is that soisconnecting() is  * called during processing of connect() call,  * resulting in an eventual call to soisconnected() if/when the  * connection is established.  When the connection is torn down  * soisdisconnecting() is called during processing of disconnect() call,  * and soisdisconnected() is called when the connection to the peer  * is totally severed.  The semantics of these routines are such that  * connectionless protocols can call soisconnected() and soisdisconnected()  * only, bypassing the in-progress calls when setting up a ``connection''  * takes no time.  *  * From the passive side, a socket is created with  * two queues of sockets: so_q0 for connections in progress  * and so_q for connections already made and awaiting user acceptance.  * As a protocol is preparing incoming connections, it creates a socket  * structure queued on so_q0 by calling sonewconn().  When the connection  * is established, soisconnected() is called, and transfers the  * socket structure to so_q, making it available to accept().  *   * If a socket is closed with sockets on either  * so_q0 or so_q, these sockets are dropped.  *  * If higher level protocols are implemented in  * the kernel, the wakeups done here will sometimes  * cause software-interrupt process scheduling.  */
end_comment

begin_expr_stmt
name|soisconnecting
argument_list|(
name|so
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_block
block|{
name|so
operator|->
name|so_state
operator|&=
operator|~
operator|(
name|SS_ISCONNECTED
operator||
name|SS_ISDISCONNECTING
operator|)
expr_stmt|;
name|so
operator|->
name|so_state
operator||=
name|SS_ISCONNECTING
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|so
operator|->
name|so_timeo
argument_list|)
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|soisconnected
argument_list|(
name|so
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|struct
name|socket
modifier|*
name|head
init|=
name|so
operator|->
name|so_head
decl_stmt|;
if|if
condition|(
name|head
condition|)
block|{
if|if
condition|(
name|soqremque
argument_list|(
name|so
argument_list|,
literal|0
argument_list|)
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"soisconnected"
argument_list|)
expr_stmt|;
name|soqinsque
argument_list|(
name|head
argument_list|,
name|so
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|sorwakeup
argument_list|(
name|head
argument_list|)
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|head
operator|->
name|so_timeo
argument_list|)
expr_stmt|;
block|}
name|so
operator|->
name|so_state
operator|&=
operator|~
operator|(
name|SS_ISCONNECTING
operator||
name|SS_ISDISCONNECTING
operator|)
expr_stmt|;
name|so
operator|->
name|so_state
operator||=
name|SS_ISCONNECTED
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|so
operator|->
name|so_timeo
argument_list|)
expr_stmt|;
name|sorwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
name|sowwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|soisdisconnecting
argument_list|(
name|so
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_block
block|{
name|so
operator|->
name|so_state
operator|&=
operator|~
name|SS_ISCONNECTING
expr_stmt|;
name|so
operator|->
name|so_state
operator||=
operator|(
name|SS_ISDISCONNECTING
operator||
name|SS_CANTRCVMORE
operator||
name|SS_CANTSENDMORE
operator|)
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|so
operator|->
name|so_timeo
argument_list|)
expr_stmt|;
name|sowwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
name|sorwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|soisdisconnected
argument_list|(
name|so
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_block
block|{
name|so
operator|->
name|so_state
operator|&=
operator|~
operator|(
name|SS_ISCONNECTING
operator||
name|SS_ISCONNECTED
operator||
name|SS_ISDISCONNECTING
operator|)
expr_stmt|;
name|so
operator|->
name|so_state
operator||=
operator|(
name|SS_CANTRCVMORE
operator||
name|SS_CANTSENDMORE
operator|)
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|so
operator|->
name|so_timeo
argument_list|)
expr_stmt|;
name|sowwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
name|sorwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * When an attempt at a new connection is noted on a socket  * which accepts connections, sonewconn is called.  If the  * connection is possible (subject to space constraints, etc.)  * then we allocate a new structure, propoerly linked into the  * data structure of the original socket, and return this.  */
end_comment

begin_function
name|struct
name|socket
modifier|*
name|sonewconn
parameter_list|(
name|head
parameter_list|)
specifier|register
name|struct
name|socket
modifier|*
name|head
decl_stmt|;
block|{
specifier|register
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
if|if
condition|(
name|head
operator|->
name|so_qlen
operator|+
name|head
operator|->
name|so_q0len
operator|>
literal|3
operator|*
name|head
operator|->
name|so_qlimit
operator|/
literal|2
condition|)
goto|goto
name|bad
goto|;
name|m
operator|=
name|m_getclr
argument_list|(
name|M_DONTWAIT
argument_list|,
name|MT_SOCKET
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
goto|goto
name|bad
goto|;
name|so
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|socket
operator|*
argument_list|)
expr_stmt|;
name|so
operator|->
name|so_type
operator|=
name|head
operator|->
name|so_type
expr_stmt|;
name|so
operator|->
name|so_options
operator|=
name|head
operator|->
name|so_options
operator|&
operator|~
name|SO_ACCEPTCONN
expr_stmt|;
name|so
operator|->
name|so_linger
operator|=
name|head
operator|->
name|so_linger
expr_stmt|;
name|so
operator|->
name|so_state
operator|=
name|head
operator|->
name|so_state
operator||
name|SS_NOFDREF
expr_stmt|;
name|so
operator|->
name|so_proto
operator|=
name|head
operator|->
name|so_proto
expr_stmt|;
name|so
operator|->
name|so_timeo
operator|=
name|head
operator|->
name|so_timeo
expr_stmt|;
name|so
operator|->
name|so_pgrp
operator|=
name|head
operator|->
name|so_pgrp
expr_stmt|;
name|soqinsque
argument_list|(
name|head
argument_list|,
name|so
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
call|(
modifier|*
name|so
operator|->
name|so_proto
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_ATTACH
argument_list|,
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
argument_list|,
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
argument_list|,
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
argument_list|)
condition|)
block|{
operator|(
name|void
operator|)
name|soqremque
argument_list|(
name|so
argument_list|,
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
return|return
operator|(
name|so
operator|)
return|;
name|bad
label|:
return|return
operator|(
operator|(
expr|struct
name|socket
operator|*
operator|)
literal|0
operator|)
return|;
block|}
end_function

begin_expr_stmt
name|soqinsque
argument_list|(
name|head
argument_list|,
name|so
argument_list|,
name|q
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|head
operator|,
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|q
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|so
operator|->
name|so_head
operator|=
name|head
expr_stmt|;
if|if
condition|(
name|q
operator|==
literal|0
condition|)
block|{
name|head
operator|->
name|so_q0len
operator|++
expr_stmt|;
name|so
operator|->
name|so_q0
operator|=
name|head
operator|->
name|so_q0
expr_stmt|;
name|head
operator|->
name|so_q0
operator|=
name|so
expr_stmt|;
block|}
else|else
block|{
name|head
operator|->
name|so_qlen
operator|++
expr_stmt|;
name|so
operator|->
name|so_q
operator|=
name|head
operator|->
name|so_q
expr_stmt|;
name|head
operator|->
name|so_q
operator|=
name|so
expr_stmt|;
block|}
block|}
end_block

begin_expr_stmt
name|soqremque
argument_list|(
name|so
argument_list|,
name|q
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|q
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|socket
modifier|*
name|head
decl_stmt|,
modifier|*
name|prev
decl_stmt|,
modifier|*
name|next
decl_stmt|;
name|head
operator|=
name|so
operator|->
name|so_head
expr_stmt|;
name|prev
operator|=
name|head
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|next
operator|=
name|q
condition|?
name|prev
operator|->
name|so_q
else|:
name|prev
operator|->
name|so_q0
expr_stmt|;
if|if
condition|(
name|next
operator|==
name|so
condition|)
break|break;
if|if
condition|(
name|next
operator|==
name|head
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|prev
operator|=
name|next
expr_stmt|;
block|}
if|if
condition|(
name|q
operator|==
literal|0
condition|)
block|{
name|prev
operator|->
name|so_q0
operator|=
name|next
operator|->
name|so_q0
expr_stmt|;
name|head
operator|->
name|so_q0len
operator|--
expr_stmt|;
block|}
else|else
block|{
name|prev
operator|->
name|so_q
operator|=
name|next
operator|->
name|so_q
expr_stmt|;
name|head
operator|->
name|so_qlen
operator|--
expr_stmt|;
block|}
name|next
operator|->
name|so_q0
operator|=
name|next
operator|->
name|so_q
operator|=
literal|0
expr_stmt|;
name|next
operator|->
name|so_head
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Socantsendmore indicates that no more data will be sent on the  * socket; it would normally be applied to a socket when the user  * informs the system that no more data is to be sent, by the protocol  * code (in case PRU_SHUTDOWN).  Socantrcvmore indicates that no more data  * will be received, and will normally be applied to the socket by a  * protocol when it detects that the peer will send no more data.  * Data queued for reading in the socket may yet be read.  */
end_comment

begin_macro
name|socantsendmore
argument_list|(
argument|so
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|so
operator|->
name|so_state
operator||=
name|SS_CANTSENDMORE
expr_stmt|;
name|sowwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|socantrcvmore
argument_list|(
argument|so
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|so
operator|->
name|so_state
operator||=
name|SS_CANTRCVMORE
expr_stmt|;
name|sorwakeup
argument_list|(
name|so
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Socket select/wakeup routines.  */
end_comment

begin_comment
comment|/*  * Queue a process for a select on a socket buffer.  */
end_comment

begin_macro
name|sbselqueue
argument_list|(
argument|sb
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
if|if
condition|(
operator|(
name|p
operator|=
name|sb
operator|->
name|sb_sel
operator|)
operator|&&
name|p
operator|->
name|p_wchan
operator|==
operator|(
name|caddr_t
operator|)
operator|&
name|selwait
condition|)
name|sb
operator|->
name|sb_flags
operator||=
name|SB_COLL
expr_stmt|;
else|else
name|sb
operator|->
name|sb_sel
operator|=
name|u
operator|.
name|u_procp
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Wait for data to arrive at/drain from a socket buffer.  */
end_comment

begin_macro
name|sbwait
argument_list|(
argument|sb
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|sb
operator|->
name|sb_flags
operator||=
name|SB_WAIT
expr_stmt|;
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|sb
operator|->
name|sb_cc
argument_list|,
name|PZERO
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Wakeup processes waiting on a socket buffer.  */
end_comment

begin_expr_stmt
name|sbwakeup
argument_list|(
name|sb
argument_list|)
specifier|register
expr|struct
name|sockbuf
operator|*
name|sb
expr_stmt|;
end_expr_stmt

begin_block
block|{
if|if
condition|(
name|sb
operator|->
name|sb_sel
condition|)
block|{
name|selwakeup
argument_list|(
name|sb
operator|->
name|sb_sel
argument_list|,
name|sb
operator|->
name|sb_flags
operator|&
name|SB_COLL
argument_list|)
expr_stmt|;
name|sb
operator|->
name|sb_sel
operator|=
literal|0
expr_stmt|;
name|sb
operator|->
name|sb_flags
operator|&=
operator|~
name|SB_COLL
expr_stmt|;
block|}
if|if
condition|(
name|sb
operator|->
name|sb_flags
operator|&
name|SB_WAIT
condition|)
block|{
name|sb
operator|->
name|sb_flags
operator|&=
operator|~
name|SB_WAIT
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|sb
operator|->
name|sb_cc
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Wakeup socket readers and writers.  * Do asynchronous notification via SIGIO  * if the socket has the SS_ASYNC flag set.  */
end_comment

begin_expr_stmt
name|sowakeup
argument_list|(
name|so
argument_list|,
name|sb
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
name|sbwakeup
argument_list|(
name|sb
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_state
operator|&
name|SS_ASYNC
condition|)
block|{
if|if
condition|(
name|so
operator|->
name|so_pgrp
operator|<
literal|0
condition|)
name|gsignal
argument_list|(
operator|-
name|so
operator|->
name|so_pgrp
argument_list|,
name|SIGIO
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|so
operator|->
name|so_pgrp
operator|>
literal|0
operator|&&
operator|(
name|p
operator|=
name|pfind
argument_list|(
name|so
operator|->
name|so_pgrp
argument_list|)
operator|)
operator|!=
literal|0
condition|)
name|psignal
argument_list|(
name|p
argument_list|,
name|SIGIO
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Socket buffer (struct sockbuf) utility routines.  *  * Each socket contains two socket buffers: one for sending data and  * one for receiving data.  Each buffer contains a queue of mbufs,  * information about the number of mbufs and amount of data in the  * queue, and other fields allowing select() statements and notification  * on data availability to be implemented.  *  * Data stored in a socket buffer is maintained as a list of records.  * Each record is a list of mbufs chained together with the m_next  * field.  Records are chained together with the m_act field. The upper  * level routine soreceive() expects the following conventions to be  * observed when placing information in the receive buffer:  *  * 1. If the protocol requires each message be preceded by the sender's  *    name, then a record containing that name must be present before  *    any associated data (mbuf's must be of type MT_SONAME).  * 2. If the protocol supports the exchange of ``access rights'' (really  *    just additional data associated with the message), and there are  *    ``rights'' to be received, then a record containing this data  *    should be present (mbuf's must be of type MT_RIGHTS).  * 3. If a name or rights record exists, then it must be followed by  *    a data record, perhaps of zero length.  *  * Before using a new socket structure it is first necessary to reserve  * buffer space to the socket, by calling sbreserve().  This commits  * some of the available buffer space in the system buffer pool for the  * socket.  The space should be released by calling sbrelease() when the  * socket is destroyed.  *  * The routines sbappend() or sbappendrecord() are normally called to  * append new mbufs to a socket buffer, after checking that adequate  * space is available, comparing the function sbspace() with the amount  * of data to be added.  sbappendrecord() differs from sbappend() in  * that data supplied is treated as the beginning of a new record.  * Data is normally removed from a socket buffer in a protocol by  * first calling m_copy on the socket buffer mbuf chain and sending this  * to a peer, and then removing the data from the socket buffer with  * sbdrop() or sbdroprecord() when the data is acknowledged by the peer  * (or immediately in the case of unreliable protocols.)  *  * To place a sender's name, optionally, access rights, and data in a  * socket buffer sbappendaddr() should be used.  To place access rights  * and data in a socket buffer sbappendrights() should be used.  Note  * that unlike sbappend() and sbappendrecord(), these routines check  * for the caller that there will be enough space to store the data.  * Each fails if there is not enough space, or if it cannot find mbufs  * to store additional information in.  */
end_comment

begin_expr_stmt
name|soreserve
argument_list|(
name|so
argument_list|,
name|sndcc
argument_list|,
name|rcvcc
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|sndcc
decl_stmt|,
name|rcvcc
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|sbreserve
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|,
name|sndcc
argument_list|)
operator|==
literal|0
condition|)
goto|goto
name|bad
goto|;
if|if
condition|(
name|sbreserve
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|,
name|rcvcc
argument_list|)
operator|==
literal|0
condition|)
goto|goto
name|bad2
goto|;
return|return
operator|(
literal|0
operator|)
return|;
name|bad2
label|:
name|sbrelease
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
name|bad
label|:
return|return
operator|(
name|ENOBUFS
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Allot mbufs to a sockbuf.  */
end_comment

begin_macro
name|sbreserve
argument_list|(
argument|sb
argument_list|,
argument|cc
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
operator|(
name|unsigned
operator|)
name|cc
operator|>
name|SB_MAX
condition|)
return|return
operator|(
literal|0
operator|)
return|;
comment|/* someday maybe this routine will fail... */
name|sb
operator|->
name|sb_hiwat
operator|=
name|cc
expr_stmt|;
comment|/* * 2 implies names can be no more than 1 mbuf each */
name|sb
operator|->
name|sb_mbmax
operator|=
name|MIN
argument_list|(
name|cc
operator|*
literal|2
argument_list|,
name|SB_MAX
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Free mbufs held by a socket, and reserved mbuf space.  */
end_comment

begin_macro
name|sbrelease
argument_list|(
argument|sb
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|sbflush
argument_list|(
name|sb
argument_list|)
expr_stmt|;
name|sb
operator|->
name|sb_hiwat
operator|=
name|sb
operator|->
name|sb_mbmax
operator|=
literal|0
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Routines to add and remove  * data from an mbuf queue.  */
end_comment

begin_comment
comment|/*  * Append mbuf chain m to the last record in the  * socket buffer sb.  The additional space associated  * the mbuf chain is recorded in sb.  Empty mbufs are  * discarded and mbufs are compacted where possible.  */
end_comment

begin_macro
name|sbappend
argument_list|(
argument|sb
argument_list|,
argument|m
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|n
decl_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
return|return;
if|if
condition|(
name|n
operator|=
name|sb
operator|->
name|sb_mb
condition|)
block|{
while|while
condition|(
name|n
operator|->
name|m_act
condition|)
name|n
operator|=
name|n
operator|->
name|m_act
expr_stmt|;
while|while
condition|(
name|n
operator|->
name|m_next
condition|)
name|n
operator|=
name|n
operator|->
name|m_next
expr_stmt|;
block|}
name|sbcompress
argument_list|(
name|sb
argument_list|,
name|m
argument_list|,
name|n
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * As above, except the mbuf chain  * begins a new record.  */
end_comment

begin_expr_stmt
name|sbappendrecord
argument_list|(
name|sb
argument_list|,
name|m0
argument_list|)
specifier|register
expr|struct
name|sockbuf
operator|*
name|sb
expr_stmt|;
end_expr_stmt

begin_decl_stmt
specifier|register
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
if|if
condition|(
name|m0
operator|==
literal|0
condition|)
return|return;
if|if
condition|(
name|m
operator|=
name|sb
operator|->
name|sb_mb
condition|)
while|while
condition|(
name|m
operator|->
name|m_act
condition|)
name|m
operator|=
name|m
operator|->
name|m_act
expr_stmt|;
comment|/* 	 * Put the first mbuf on the queue. 	 * Note this permits zero length records. 	 */
name|sballoc
argument_list|(
name|sb
argument_list|,
name|m0
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
condition|)
name|m
operator|->
name|m_act
operator|=
name|m0
expr_stmt|;
else|else
name|sb
operator|->
name|sb_mb
operator|=
name|m0
expr_stmt|;
name|m
operator|=
name|m0
operator|->
name|m_next
expr_stmt|;
name|m0
operator|->
name|m_next
operator|=
literal|0
expr_stmt|;
name|sbcompress
argument_list|(
name|sb
argument_list|,
name|m
argument_list|,
name|m0
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Append address and data, and optionally, rights  * to the receive queue of a socket.  Return 0 if  * no space in sockbuf or insufficient mbufs.  */
end_comment

begin_expr_stmt
name|sbappendaddr
argument_list|(
name|sb
argument_list|,
name|asa
argument_list|,
name|m0
argument_list|,
name|rights0
argument_list|)
comment|/* XXX */
specifier|register
expr|struct
name|sockbuf
operator|*
name|sb
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|asa
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|rights0
decl_stmt|,
modifier|*
name|m0
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|n
decl_stmt|;
name|int
name|space
init|=
sizeof|sizeof
argument_list|(
operator|*
name|asa
argument_list|)
decl_stmt|;
name|m
operator|=
name|m0
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"sbappendaddr"
argument_list|)
expr_stmt|;
do|do
block|{
name|space
operator|+=
name|m
operator|->
name|m_len
expr_stmt|;
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
block|}
do|while
condition|(
name|m
condition|)
do|;
if|if
condition|(
name|rights0
condition|)
name|space
operator|+=
name|rights0
operator|->
name|m_len
expr_stmt|;
if|if
condition|(
name|space
operator|>
name|sbspace
argument_list|(
name|sb
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|m
operator|=
name|m_get
argument_list|(
name|M_DONTWAIT
argument_list|,
name|MT_SONAME
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
operator|*
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|sockaddr
operator|*
argument_list|)
operator|=
operator|*
name|asa
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|asa
argument_list|)
expr_stmt|;
if|if
condition|(
name|rights0
condition|)
block|{
name|m
operator|->
name|m_act
operator|=
name|m_copy
argument_list|(
name|rights0
argument_list|,
literal|0
argument_list|,
name|rights0
operator|->
name|m_len
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_act
operator|==
literal|0
condition|)
block|{
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|sballoc
argument_list|(
name|sb
argument_list|,
name|m
operator|->
name|m_act
argument_list|)
expr_stmt|;
block|}
name|sballoc
argument_list|(
name|sb
argument_list|,
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|=
name|sb
operator|->
name|sb_mb
condition|)
block|{
while|while
condition|(
name|n
operator|->
name|m_act
condition|)
name|n
operator|=
name|n
operator|->
name|m_act
expr_stmt|;
name|n
operator|->
name|m_act
operator|=
name|m
expr_stmt|;
block|}
else|else
name|sb
operator|->
name|sb_mb
operator|=
name|m
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_act
condition|)
name|m
operator|=
name|m
operator|->
name|m_act
expr_stmt|;
name|sballoc
argument_list|(
name|sb
argument_list|,
name|m0
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_act
operator|=
name|m0
expr_stmt|;
name|m
operator|=
name|m0
operator|->
name|m_next
expr_stmt|;
name|m0
operator|->
name|m_next
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|m
condition|)
name|sbcompress
argument_list|(
name|sb
argument_list|,
name|m
argument_list|,
name|m0
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_ifdef
ifdef|#
directive|ifdef
name|notdef
end_ifdef

begin_macro
name|sbappendrights
argument_list|(
argument|sb
argument_list|,
argument|rights
argument_list|,
argument|m0
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|rights
decl_stmt|,
modifier|*
name|m
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|n
decl_stmt|;
name|int
name|space
init|=
literal|0
decl_stmt|;
name|m
operator|=
name|m0
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
operator|||
name|rights
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"sbappendrights"
argument_list|)
expr_stmt|;
do|do
block|{
name|space
operator|+=
name|m
operator|->
name|m_len
expr_stmt|;
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
block|}
do|while
condition|(
name|m
condition|)
do|;
name|space
operator|+=
name|rights
operator|->
name|m_len
expr_stmt|;
if|if
condition|(
name|space
operator|>
name|sbspace
argument_list|(
name|sb
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|m
operator|=
name|m_copy
argument_list|(
name|rights
argument_list|,
literal|0
argument_list|,
name|rights
operator|->
name|m_len
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|sballoc
argument_list|(
name|sb
argument_list|,
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|=
name|sb
operator|->
name|sb_mb
condition|)
block|{
while|while
condition|(
name|n
operator|->
name|m_act
condition|)
name|n
operator|=
name|n
operator|->
name|m_act
expr_stmt|;
name|n
operator|->
name|m_act
operator|=
name|m
expr_stmt|;
block|}
else|else
name|n
operator|->
name|m_act
operator|=
name|m
expr_stmt|;
name|sballoc
argument_list|(
name|sb
argument_list|,
name|m0
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_act
operator|=
name|m0
expr_stmt|;
name|m
operator|=
name|m0
operator|->
name|m_next
expr_stmt|;
name|m0
operator|->
name|m_next
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|m
condition|)
name|sbcompress
argument_list|(
name|sb
argument_list|,
name|m
argument_list|,
name|m0
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Compress mbuf chain m into the socket  * buffer sb following mbuf n.  If n  * is null, the buffer is presumed empty.  */
end_comment

begin_expr_stmt
name|sbcompress
argument_list|(
name|sb
argument_list|,
name|m
argument_list|,
name|n
argument_list|)
specifier|register
expr|struct
name|sockbuf
operator|*
name|sb
expr_stmt|;
end_expr_stmt

begin_decl_stmt
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|n
decl_stmt|;
end_decl_stmt

begin_block
block|{
while|while
condition|(
name|m
condition|)
block|{
if|if
condition|(
name|m
operator|->
name|m_len
operator|==
literal|0
condition|)
block|{
name|m
operator|=
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|n
operator|&&
name|n
operator|->
name|m_off
operator|<=
name|MMAXOFF
operator|&&
name|m
operator|->
name|m_off
operator|<=
name|MMAXOFF
operator|&&
operator|(
name|n
operator|->
name|m_off
operator|+
name|n
operator|->
name|m_len
operator|+
name|m
operator|->
name|m_len
operator|)
operator|<=
name|MMAXOFF
condition|)
block|{
name|bcopy
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
name|mtod
argument_list|(
name|n
argument_list|,
name|caddr_t
argument_list|)
operator|+
name|n
operator|->
name|m_len
argument_list|,
operator|(
name|unsigned
operator|)
name|m
operator|->
name|m_len
argument_list|)
expr_stmt|;
name|n
operator|->
name|m_len
operator|+=
name|m
operator|->
name|m_len
expr_stmt|;
name|sb
operator|->
name|sb_cc
operator|+=
name|m
operator|->
name|m_len
expr_stmt|;
name|m
operator|=
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|sballoc
argument_list|(
name|sb
argument_list|,
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
condition|)
name|n
operator|->
name|m_next
operator|=
name|m
expr_stmt|;
else|else
name|sb
operator|->
name|sb_mb
operator|=
name|m
expr_stmt|;
name|n
operator|=
name|m
expr_stmt|;
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
name|n
operator|->
name|m_next
operator|=
literal|0
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Free all mbufs in a sockbuf.  * Check that all resources are reclaimed.  */
end_comment

begin_expr_stmt
name|sbflush
argument_list|(
name|sb
argument_list|)
specifier|register
expr|struct
name|sockbuf
operator|*
name|sb
expr_stmt|;
end_expr_stmt

begin_block
block|{
if|if
condition|(
name|sb
operator|->
name|sb_flags
operator|&
name|SB_LOCK
condition|)
name|panic
argument_list|(
literal|"sbflush"
argument_list|)
expr_stmt|;
if|if
condition|(
name|sb
operator|->
name|sb_cc
condition|)
name|sbdrop
argument_list|(
name|sb
argument_list|,
name|sb
operator|->
name|sb_cc
argument_list|)
expr_stmt|;
if|if
condition|(
name|sb
operator|->
name|sb_cc
operator|||
name|sb
operator|->
name|sb_mbcnt
operator|||
name|sb
operator|->
name|sb_mb
condition|)
name|panic
argument_list|(
literal|"sbflush 2"
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Drop data from (the front of) a sockbuf.  */
end_comment

begin_function
name|struct
name|mbuf
modifier|*
name|sbdrop
parameter_list|(
name|sb
parameter_list|,
name|len
parameter_list|)
specifier|register
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
specifier|register
name|int
name|len
decl_stmt|;
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|mn
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|next
decl_stmt|;
name|next
operator|=
operator|(
name|m
operator|=
name|sb
operator|->
name|sb_mb
operator|)
condition|?
name|m
operator|->
name|m_act
else|:
literal|0
expr_stmt|;
while|while
condition|(
name|len
operator|>
literal|0
condition|)
block|{
if|if
condition|(
name|m
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|next
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"sbdrop"
argument_list|)
expr_stmt|;
name|m
operator|=
name|next
expr_stmt|;
name|next
operator|=
name|m
operator|->
name|m_act
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|m
operator|->
name|m_len
operator|>
name|len
condition|)
block|{
name|m
operator|->
name|m_len
operator|-=
name|len
expr_stmt|;
name|m
operator|->
name|m_off
operator|+=
name|len
expr_stmt|;
name|sb
operator|->
name|sb_cc
operator|-=
name|len
expr_stmt|;
break|break;
block|}
name|len
operator|-=
name|m
operator|->
name|m_len
expr_stmt|;
name|sbfree
argument_list|(
name|sb
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|MFREE
argument_list|(
name|m
argument_list|,
name|mn
argument_list|)
expr_stmt|;
name|m
operator|=
name|mn
expr_stmt|;
block|}
while|while
condition|(
name|m
operator|&&
name|m
operator|->
name|m_len
operator|==
literal|0
condition|)
block|{
name|sbfree
argument_list|(
name|sb
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|MFREE
argument_list|(
name|m
argument_list|,
name|mn
argument_list|)
expr_stmt|;
name|m
operator|=
name|mn
expr_stmt|;
block|}
if|if
condition|(
name|m
condition|)
block|{
name|sb
operator|->
name|sb_mb
operator|=
name|m
expr_stmt|;
name|m
operator|->
name|m_act
operator|=
name|next
expr_stmt|;
block|}
else|else
name|sb
operator|->
name|sb_mb
operator|=
name|next
expr_stmt|;
return|return
operator|(
name|sb
operator|->
name|sb_mb
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Drop a record off the front of a sockbuf  * and move the next record to the front.  */
end_comment

begin_function
name|struct
name|mbuf
modifier|*
name|sbdroprecord
parameter_list|(
name|sb
parameter_list|)
specifier|register
name|struct
name|sockbuf
modifier|*
name|sb
decl_stmt|;
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|mn
decl_stmt|;
name|m
operator|=
name|sb
operator|->
name|sb_mb
expr_stmt|;
if|if
condition|(
name|m
condition|)
block|{
name|sb
operator|->
name|sb_mb
operator|=
name|m
operator|->
name|m_act
expr_stmt|;
do|do
block|{
name|sbfree
argument_list|(
name|sb
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|MFREE
argument_list|(
name|m
argument_list|,
name|mn
argument_list|)
expr_stmt|;
block|}
do|while
condition|(
name|m
operator|=
name|mn
condition|)
do|;
block|}
return|return
operator|(
name|sb
operator|->
name|sb_mb
operator|)
return|;
block|}
end_function

end_unit

