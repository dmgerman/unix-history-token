begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	uipc_socket.c	4.13	81/11/22	*/
end_comment

begin_include
include|#
directive|include
file|"../h/param.h"
end_include

begin_include
include|#
directive|include
file|"../h/systm.h"
end_include

begin_include
include|#
directive|include
file|"../h/dir.h"
end_include

begin_include
include|#
directive|include
file|"../h/user.h"
end_include

begin_include
include|#
directive|include
file|"../h/proc.h"
end_include

begin_include
include|#
directive|include
file|"../h/file.h"
end_include

begin_include
include|#
directive|include
file|"../h/inode.h"
end_include

begin_include
include|#
directive|include
file|"../h/buf.h"
end_include

begin_include
include|#
directive|include
file|"../h/mbuf.h"
end_include

begin_include
include|#
directive|include
file|"../h/protosw.h"
end_include

begin_include
include|#
directive|include
file|"../h/socket.h"
end_include

begin_include
include|#
directive|include
file|"../h/socketvar.h"
end_include

begin_include
include|#
directive|include
file|"../h/stat.h"
end_include

begin_include
include|#
directive|include
file|"../net/inet.h"
end_include

begin_include
include|#
directive|include
file|"../net/inet_systm.h"
end_include

begin_comment
comment|/*  * Socket support routines.  *  * DEAL WITH INTERRUPT NOTIFICATION.  */
end_comment

begin_comment
comment|/*  * Create a socket.  */
end_comment

begin_macro
name|socreate
argument_list|(
argument|aso
argument_list|,
argument|type
argument_list|,
argument|asp
argument_list|,
argument|asa
argument_list|,
argument|options
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
modifier|*
name|aso
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|type
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockproto
modifier|*
name|asp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|asa
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|options
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|protosw
modifier|*
name|prp
decl_stmt|;
specifier|register
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|int
name|pf
decl_stmt|,
name|proto
decl_stmt|,
name|error
decl_stmt|;
name|COUNT
argument_list|(
name|SOCREATE
argument_list|)
expr_stmt|;
comment|/* 	 * Use process standard protocol/protocol family if none 	 * specified by address argument. 	 */
if|if
condition|(
name|asp
operator|==
literal|0
condition|)
block|{
name|pf
operator|=
name|PF_INET
expr_stmt|;
comment|/* should be u.u_protof */
name|proto
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|pf
operator|=
name|asp
operator|->
name|sp_family
expr_stmt|;
name|proto
operator|=
name|asp
operator|->
name|sp_protocol
expr_stmt|;
block|}
comment|/* 	 * If protocol specified, look for it, otherwise 	 * for a protocol of the correct type in the right family. 	 */
if|if
condition|(
name|proto
condition|)
name|prp
operator|=
name|pffindproto
argument_list|(
name|pf
argument_list|,
name|proto
argument_list|)
expr_stmt|;
else|else
name|prp
operator|=
name|pffindtype
argument_list|(
name|pf
argument_list|,
name|type
argument_list|)
expr_stmt|;
if|if
condition|(
name|prp
operator|==
literal|0
condition|)
return|return
operator|(
name|EPROTONOSUPPORT
operator|)
return|;
comment|/* 	 * Get a socket structure. 	 */
name|m
operator|=
name|m_getclr
argument_list|(
name|M_WAIT
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
return|return
operator|(
name|ENOBUFS
operator|)
return|;
name|so
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|socket
operator|*
argument_list|)
expr_stmt|;
name|so
operator|->
name|so_options
operator|=
name|options
expr_stmt|;
comment|/* 	 * Attach protocol to socket, initializing 	 * and reserving resources. 	 */
name|so
operator|->
name|so_proto
operator|=
name|prp
expr_stmt|;
name|error
operator|=
call|(
modifier|*
name|prp
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_ATTACH
argument_list|,
literal|0
argument_list|,
name|asa
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
operator|(
name|void
operator|)
name|m_free
argument_list|(
name|dtom
argument_list|(
name|so
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
operator|*
name|aso
operator|=
name|so
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_macro
name|sofree
argument_list|(
argument|so
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|COUNT
argument_list|(
name|SOFREE
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_pcb
operator|||
operator|(
name|so
operator|->
name|so_state
operator|&
name|SS_USERGONE
operator|)
operator|==
literal|0
condition|)
return|return;
name|sbrelease
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
name|sbrelease
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|m_free
argument_list|(
name|dtom
argument_list|(
name|so
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Close a socket on last file table reference removal.  * Initiate disconnect if connected.  * Free socket when disconnect complete.  */
end_comment

begin_expr_stmt
name|soclose
argument_list|(
name|so
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_block
block|{
name|int
name|s
init|=
name|splnet
argument_list|()
decl_stmt|;
comment|/* conservative */
name|COUNT
argument_list|(
name|SOCLOSE
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_pcb
operator|==
literal|0
condition|)
goto|goto
name|discard
goto|;
if|if
condition|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISCONNECTED
condition|)
block|{
if|if
condition|(
operator|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISDISCONNECTING
operator|)
operator|==
literal|0
condition|)
block|{
name|u
operator|.
name|u_error
operator|=
name|sodisconnect
argument_list|(
name|so
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|u
operator|.
name|u_error
condition|)
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
if|if
condition|(
operator|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISDISCONNECTING
operator|)
operator|&&
operator|(
name|so
operator|->
name|so_options
operator|&
name|SO_NBIO
operator|)
condition|)
block|{
name|u
operator|.
name|u_error
operator|=
name|EINPROGRESS
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return;
block|}
while|while
condition|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISCONNECTED
condition|)
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|so
operator|->
name|so_timeo
argument_list|,
name|PZERO
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
name|u
operator|.
name|u_error
operator|=
call|(
modifier|*
name|so
operator|->
name|so_proto
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_DETACH
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|discard
label|:
name|so
operator|->
name|so_state
operator||=
name|SS_USERGONE
expr_stmt|;
name|sofree
argument_list|(
name|so
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|sosplice
argument_list|(
argument|pso
argument_list|,
argument|so
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|pso
decl_stmt|,
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|COUNT
argument_list|(
name|SOSPLICE
argument_list|)
expr_stmt|;
if|if
condition|(
name|pso
operator|->
name|so_proto
operator|->
name|pr_family
operator|!=
name|PF_LOCAL
condition|)
block|{
name|struct
name|socket
modifier|*
name|tso
decl_stmt|;
name|tso
operator|=
name|pso
expr_stmt|;
name|pso
operator|=
name|so
expr_stmt|;
name|so
operator|=
name|tso
expr_stmt|;
block|}
if|if
condition|(
name|pso
operator|->
name|so_proto
operator|->
name|pr_family
operator|!=
name|PF_LOCAL
condition|)
return|return
operator|(
name|EOPNOTSUPP
operator|)
return|;
comment|/* check types and buffer space */
comment|/* merge buffers */
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_macro
name|sostat
argument_list|(
argument|so
argument_list|,
argument|sb
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|stat
modifier|*
name|sb
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|COUNT
argument_list|(
name|SOSTAT
argument_list|)
expr_stmt|;
return|return
operator|(
name|EOPNOTSUPP
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Accept connection on a socket.  */
end_comment

begin_macro
name|soaccept
argument_list|(
argument|so
argument_list|,
argument|asa
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|asa
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|s
init|=
name|splnet
argument_list|()
decl_stmt|;
name|int
name|error
decl_stmt|;
name|COUNT
argument_list|(
name|SOACCEPT
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_state
operator|&
operator|(
name|SS_ISCONNECTED
operator||
name|SS_ISCONNECTING
operator|)
condition|)
block|{
name|error
operator|=
name|EISCONN
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
if|if
condition|(
operator|(
name|so
operator|->
name|so_options
operator|&
name|SO_ACCEPTCONN
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|EINVAL
expr_stmt|;
comment|/* XXX */
goto|goto
name|bad
goto|;
block|}
name|error
operator|=
call|(
modifier|*
name|so
operator|->
name|so_proto
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_ACCEPT
argument_list|,
literal|0
argument_list|,
operator|(
name|caddr_t
operator|)
name|asa
argument_list|)
expr_stmt|;
name|bad
label|:
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Connect socket to a specified address.  * If already connected or connecting, then avoid  * the protocol entry, to keep its job simpler.  */
end_comment

begin_macro
name|soconnect
argument_list|(
argument|so
argument_list|,
argument|asa
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|asa
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|s
init|=
name|splnet
argument_list|()
decl_stmt|;
name|int
name|error
decl_stmt|;
name|COUNT
argument_list|(
name|SOCONNECT
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_state
operator|&
operator|(
name|SS_ISCONNECTED
operator||
name|SS_ISCONNECTING
operator|)
condition|)
block|{
name|error
operator|=
name|EISCONN
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|error
operator|=
call|(
modifier|*
name|so
operator|->
name|so_proto
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_CONNECT
argument_list|,
literal|0
argument_list|,
operator|(
name|caddr_t
operator|)
name|asa
argument_list|)
expr_stmt|;
name|bad
label|:
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Disconnect from a socket.  * Address parameter is from system call for later multicast  * protocols.  Check to make sure that connected and no disconnect  * in progress (for protocol's sake), and then invoke protocol.  */
end_comment

begin_macro
name|sodisconnect
argument_list|(
argument|so
argument_list|,
argument|asa
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|asa
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|s
init|=
name|splnet
argument_list|()
decl_stmt|;
name|int
name|error
decl_stmt|;
name|COUNT
argument_list|(
name|SODISCONNECT
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISCONNECTED
operator|)
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|ENOTCONN
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
if|if
condition|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISDISCONNECTING
condition|)
block|{
name|error
operator|=
name|EALREADY
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|error
operator|=
call|(
modifier|*
name|so
operator|->
name|so_proto
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_DISCONNECT
argument_list|,
literal|0
argument_list|,
name|asa
argument_list|)
expr_stmt|;
name|bad
label|:
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Send on a socket.  * If send must go all at once and message is larger than  * send buffering, then hard error.  * Lock against other senders.  * If must go all at once and not enough room now, then  * inform user that this would block and do nothing.  */
end_comment

begin_expr_stmt
name|sosend
argument_list|(
name|so
argument_list|,
name|asa
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|asa
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|mbuf
modifier|*
name|top
init|=
literal|0
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
modifier|*
name|mp
init|=
operator|&
name|top
decl_stmt|;
specifier|register
name|u_int
name|len
decl_stmt|;
name|int
name|error
init|=
literal|0
decl_stmt|,
name|space
decl_stmt|,
name|s
decl_stmt|;
name|COUNT
argument_list|(
name|SOSEND
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_state
operator|&
name|SS_CANTSENDMORE
condition|)
return|return
operator|(
name|EPIPE
operator|)
return|;
if|if
condition|(
name|sosendallatonce
argument_list|(
name|so
argument_list|)
operator|&&
name|u
operator|.
name|u_count
operator|>
name|so
operator|->
name|so_snd
operator|.
name|sb_hiwat
condition|)
return|return
operator|(
name|EMSGSIZE
operator|)
return|;
if|if
condition|(
operator|(
name|so
operator|->
name|so_snd
operator|.
name|sb_flags
operator|&
name|SB_LOCK
operator|)
operator|&&
operator|(
name|so
operator|->
name|so_options
operator|&
name|SO_NBIO
operator|)
condition|)
return|return
operator|(
name|EWOULDBLOCK
operator|)
return|;
name|sblock
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
define|#
directive|define
name|snderr
parameter_list|(
name|errno
parameter_list|)
value|{ error = errno; splx(s); goto release; }
name|s
operator|=
name|splnet
argument_list|()
expr_stmt|;
name|nullchk
argument_list|(
literal|"sosend in"
argument_list|,
name|so
operator|->
name|so_snd
operator|.
name|sb_mb
argument_list|)
expr_stmt|;
name|again
label|:
if|if
condition|(
operator|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISCONNECTED
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|so
operator|->
name|so_proto
operator|->
name|pr_flags
operator|&
name|PR_CONNREQUIRED
condition|)
name|snderr
argument_list|(
name|ENOTCONN
argument_list|)
expr_stmt|;
if|if
condition|(
name|asa
operator|==
literal|0
condition|)
name|snderr
argument_list|(
name|EDESTADDRREQ
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|so
operator|->
name|so_error
condition|)
name|snderr
argument_list|(
name|so
operator|->
name|so_error
argument_list|)
expr_stmt|;
if|if
condition|(
name|top
condition|)
block|{
name|error
operator|=
call|(
modifier|*
name|so
operator|->
name|so_proto
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_SEND
argument_list|,
name|top
argument_list|,
name|asa
argument_list|)
expr_stmt|;
name|nullchk
argument_list|(
literal|"sosend after PRU_SEND"
argument_list|,
name|so
operator|->
name|so_snd
operator|.
name|sb_mb
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
goto|goto
name|release
goto|;
block|}
name|top
operator|=
literal|0
expr_stmt|;
name|mp
operator|=
operator|&
name|top
expr_stmt|;
block|}
if|if
condition|(
name|u
operator|.
name|u_count
operator|==
literal|0
condition|)
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
goto|goto
name|release
goto|;
block|}
name|space
operator|=
name|sbspace
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
if|if
condition|(
name|space
operator|==
literal|0
operator|||
name|sosendallatonce
argument_list|(
name|so
argument_list|)
operator|&&
name|space
operator|<
name|u
operator|.
name|u_count
condition|)
block|{
if|if
condition|(
name|so
operator|->
name|so_options
operator|&
name|SO_NBIO
condition|)
name|snderr
argument_list|(
name|EWOULDBLOCK
argument_list|)
expr_stmt|;
name|sbunlock
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
name|sbwait
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
goto|goto
name|again
goto|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
while|while
condition|(
name|u
operator|.
name|u_count
operator|&&
name|space
operator|>
literal|0
condition|)
block|{
name|MGET
argument_list|(
name|m
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
name|m_freem
argument_list|(
name|top
argument_list|)
expr_stmt|;
goto|goto
name|release
goto|;
block|}
if|if
condition|(
name|u
operator|.
name|u_count
operator|>=
name|PGSIZE
operator|&&
name|space
operator|>=
name|NMBPG
condition|)
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|p
decl_stmt|;
name|MPGET
argument_list|(
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|==
literal|0
condition|)
goto|goto
name|nopages
goto|;
name|m
operator|->
name|m_off
operator|=
operator|(
name|int
operator|)
name|p
operator|-
operator|(
name|int
operator|)
name|m
expr_stmt|;
name|len
operator|=
name|PGSIZE
expr_stmt|;
block|}
else|else
block|{
name|nopages
label|:
name|m
operator|->
name|m_off
operator|=
name|MMINOFF
expr_stmt|;
name|len
operator|=
name|MIN
argument_list|(
name|MLEN
argument_list|,
name|u
operator|.
name|u_count
argument_list|)
expr_stmt|;
block|}
name|iomove
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
name|len
argument_list|,
name|B_WRITE
argument_list|)
expr_stmt|;
name|nullblk
argument_list|(
literal|"sosend"
argument_list|,
name|m
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
name|len
expr_stmt|;
operator|*
name|mp
operator|=
name|m
expr_stmt|;
name|mp
operator|=
operator|&
name|m
operator|->
name|m_next
expr_stmt|;
name|space
operator|=
name|sbspace
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
block|}
name|nullchk
argument_list|(
literal|"sosend top"
argument_list|,
name|top
argument_list|)
expr_stmt|;
name|s
operator|=
name|splnet
argument_list|()
expr_stmt|;
goto|goto
name|again
goto|;
name|release
label|:
name|sbunlock
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_expr_stmt
name|soreceive
argument_list|(
name|so
argument_list|,
name|asa
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|asa
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|n
decl_stmt|;
name|u_int
name|len
decl_stmt|;
name|int
name|eor
decl_stmt|,
name|s
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|;
name|COUNT
argument_list|(
name|SORECEIVE
argument_list|)
expr_stmt|;
name|restart
label|:
name|nullchk
argument_list|(
literal|"soreceive restart"
argument_list|,
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
argument_list|)
expr_stmt|;
for|for
control|(
name|m
operator|=
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
init|;
name|m
condition|;
name|m
operator|=
name|m
operator|->
name|m_next
control|)
name|printf
argument_list|(
literal|"%d "
argument_list|,
name|m
operator|->
name|m_len
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|sblock
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|s
operator|=
name|splnet
argument_list|()
expr_stmt|;
define|#
directive|define
name|rcverr
parameter_list|(
name|errno
parameter_list|)
value|{ error = errno; splx(s); goto release; }
if|if
condition|(
name|so
operator|->
name|so_rcv
operator|.
name|sb_cc
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|so
operator|->
name|so_state
operator|&
name|SS_CANTRCVMORE
condition|)
block|{
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
goto|goto
name|release
goto|;
block|}
if|if
condition|(
operator|(
name|so
operator|->
name|so_state
operator|&
name|SS_ISCONNECTED
operator|)
operator|==
literal|0
operator|&&
operator|(
name|so
operator|->
name|so_proto
operator|->
name|pr_flags
operator|&
name|PR_CONNREQUIRED
operator|)
condition|)
name|rcverr
argument_list|(
name|ENOTCONN
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_options
operator|&
name|SO_NBIO
condition|)
name|rcverr
argument_list|(
name|EWOULDBLOCK
argument_list|)
expr_stmt|;
name|sbunlock
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|sbwait
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
goto|goto
name|restart
goto|;
block|}
name|printf
argument_list|(
literal|"soreceive about to\n"
argument_list|)
expr_stmt|;
name|psndrcv
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|,
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|m
operator|=
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"receive"
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_proto
operator|->
name|pr_flags
operator|&
name|PR_ADDR
condition|)
block|{
name|printf
argument_list|(
literal|"m_len %d\n"
argument_list|,
name|m
operator|->
name|m_len
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_len
operator|!=
sizeof|sizeof
argument_list|(
expr|struct
name|sockaddr
argument_list|)
condition|)
name|panic
argument_list|(
literal|"soreceive addr"
argument_list|)
expr_stmt|;
if|if
condition|(
name|asa
condition|)
name|bcopy
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
operator|(
name|caddr_t
operator|)
name|asa
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|asa
argument_list|)
argument_list|)
expr_stmt|;
else|else
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
name|asa
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|asa
argument_list|)
argument_list|)
expr_stmt|;
name|so
operator|->
name|so_rcv
operator|.
name|sb_cc
operator|-=
name|m
operator|->
name|m_len
expr_stmt|;
name|so
operator|->
name|so_rcv
operator|.
name|sb_mbcnt
operator|-=
name|MSIZE
expr_stmt|;
name|m
operator|=
name|m_free
argument_list|(
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"receive 2"
argument_list|)
expr_stmt|;
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
operator|=
name|m
expr_stmt|;
block|}
name|eor
operator|=
literal|0
expr_stmt|;
name|printf
argument_list|(
literal|"soreceive before receive loop\n"
argument_list|)
expr_stmt|;
name|psndrcv
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|,
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
do|do
block|{
name|len
operator|=
name|MIN
argument_list|(
name|m
operator|->
name|m_len
argument_list|,
name|u
operator|.
name|u_count
argument_list|)
expr_stmt|;
if|if
condition|(
name|len
operator|==
name|m
operator|->
name|m_len
condition|)
block|{
name|eor
operator|=
operator|(
name|int
operator|)
name|m
operator|->
name|m_act
expr_stmt|;
name|sbfree
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|,
name|m
argument_list|)
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|nullblk
argument_list|(
literal|"soreceive"
argument_list|,
name|m
argument_list|,
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
name|len
condition|)
name|printf
argument_list|(
literal|"%o\n"
argument_list|,
operator|*
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|)
expr_stmt|;
name|iomove
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
name|len
argument_list|,
name|B_READ
argument_list|)
expr_stmt|;
name|s
operator|=
name|splnet
argument_list|()
expr_stmt|;
if|if
condition|(
name|len
operator|==
name|m
operator|->
name|m_len
condition|)
block|{
name|MFREE
argument_list|(
name|m
argument_list|,
name|n
argument_list|)
expr_stmt|;
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
operator|=
name|n
expr_stmt|;
block|}
else|else
block|{
name|m
operator|->
name|m_off
operator|+=
name|len
expr_stmt|;
name|m
operator|->
name|m_len
operator|-=
name|len
expr_stmt|;
name|so
operator|->
name|so_rcv
operator|.
name|sb_cc
operator|-=
name|len
expr_stmt|;
block|}
block|}
do|while
condition|(
operator|(
name|m
operator|=
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
operator|)
operator|&&
name|u
operator|.
name|u_count
operator|&&
operator|!
name|eor
condition|)
do|;
name|printf
argument_list|(
literal|"after receive loop\n"
argument_list|)
expr_stmt|;
name|psndrcv
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|,
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|so
operator|->
name|so_proto
operator|->
name|pr_flags
operator|&
name|PR_ATOMIC
operator|)
operator|&&
name|eor
operator|==
literal|0
condition|)
do|do
block|{
if|if
condition|(
name|m
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"receive 3"
argument_list|)
expr_stmt|;
name|sbfree
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|eor
operator|=
operator|(
name|int
operator|)
name|m
operator|->
name|m_act
expr_stmt|;
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
name|MFREE
argument_list|(
name|m
argument_list|,
name|n
argument_list|)
expr_stmt|;
name|m
operator|=
name|n
expr_stmt|;
block|}
do|while
condition|(
name|eor
operator|==
literal|0
condition|)
do|;
name|printf
argument_list|(
literal|"soreceive after drop remnants\n"
argument_list|)
expr_stmt|;
name|psndrcv
argument_list|(
operator|&
name|so
operator|->
name|so_snd
argument_list|,
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|so
operator|->
name|so_proto
operator|->
name|pr_flags
operator|&
name|PR_WANTRCVD
operator|)
operator|&&
name|so
operator|->
name|so_pcb
condition|)
call|(
modifier|*
name|so
operator|->
name|so_proto
operator|->
name|pr_usrreq
call|)
argument_list|(
name|so
argument_list|,
name|PRU_RCVD
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|nullchk
argument_list|(
literal|"receive after PRU_RCVD"
argument_list|,
name|so
operator|->
name|so_rcv
operator|.
name|sb_mb
argument_list|)
expr_stmt|;
name|release
label|:
name|sbunlock
argument_list|(
operator|&
name|so
operator|->
name|so_rcv
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_expr_stmt
name|soioctl
argument_list|(
name|so
argument_list|,
name|cmd
argument_list|,
name|cmdp
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|cmd
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|caddr_t
name|cmdp
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|COUNT
argument_list|(
name|SOIOCTL
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|cmdp
condition|)
block|{  	}
switch|switch
condition|(
name|so
operator|->
name|so_type
condition|)
block|{
case|case
name|SOCK_STREAM
case|:
break|break;
case|case
name|SOCK_DGRAM
case|:
break|break;
case|case
name|SOCK_RDM
case|:
break|break;
case|case
name|SOCK_RAW
case|:
break|break;
block|}
block|}
end_block

begin_macro
name|nullchk
argument_list|(
argument|where
argument_list|,
argument|m0
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|where
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
for|for
control|(
name|m
operator|=
name|m0
init|;
name|m
condition|;
name|m
operator|=
name|m
operator|->
name|m_next
control|)
if|if
condition|(
name|nullany
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
name|m
operator|->
name|m_len
argument_list|)
condition|)
goto|goto
name|bad
goto|;
return|return;
name|bad
label|:
name|printf
argument_list|(
literal|"nullchk: %s\n"
argument_list|,
name|where
argument_list|)
expr_stmt|;
for|for
control|(
name|m
operator|=
name|m0
init|;
name|m
condition|;
name|m
operator|=
name|m
operator|->
name|m_next
control|)
name|printf
argument_list|(
literal|"\t%x len %d: %s\n"
argument_list|,
name|m
argument_list|,
name|m
operator|->
name|m_len
argument_list|,
name|nullany
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
name|m
operator|->
name|m_len
argument_list|)
condition|?
literal|"BAD"
else|:
literal|"OK"
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|nullblk
argument_list|(
argument|where
argument_list|,
argument|m
argument_list|,
argument|len
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|where
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|len
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|nullany
argument_list|(
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
name|len
argument_list|)
condition|)
name|printf
argument_list|(
literal|"nullblk: %s m=%x len=%d\n"
argument_list|,
name|where
argument_list|,
name|m
argument_list|,
name|len
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|nullany
argument_list|(
argument|cp
argument_list|,
argument|len
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|cp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|len
decl_stmt|;
end_decl_stmt

begin_block
block|{
for|for
control|(
init|;
name|len
operator|>
literal|0
condition|;
name|len
operator|--
control|)
if|if
condition|(
operator|*
name|cp
operator|++
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
comment|/* XXX */
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

end_unit

