...
begin_comment
comment|/*  * Copyright (c) 1992 The Regents of the University of California.  * All rights reserved.  *  * This software was developed by the Computer Systems Engineering group  * at Lawrence Berkeley Laboratory under DARPA contract BG 91-66 and  * contributed to Berkeley.  *  * %sccs.include.redist.c%  *  *	@(#)zs.c	7.1 (Berkeley) %G%  *  * from: $Header: zs.c,v 1.24 92/06/30 02:24:21 torek Exp $  */
end_comment
...
directive|endif
comment|/* store receive character and status into ring */
name|i
operator|=
name|cs
operator|->
...
name|LOG_WARNING
argument_list|,
literal|"zs%d%c: input %s overrun\n"
argument_list|,
name|unit
operator|>>
literal|1
...
begin_comment
comment|/*  * ZS software interrupt.  Scan both ZS chips.  */
end_comment
...
name|cs_ttyp
expr_stmt|;
comment|/* 		 * Scan receive ring.  This involves calling ttyinput(), 		 * which is quite slow, so we loop until we have caught 		 * up with the receiver.  (XXX should test effectiveness) 		 * If we are not interested, discard the input right away. 		 * 		 * XXX this will have to be broken up so that we can get 		 * ^S stops out reasonably quickly.... 		 */
for|for
control|(
init|;
condition|;
...
condition|)
block|{
comment|/* race to keep ahead of incoming data */
name|c
operator|=
name|cs
operator|->
...
name|check_xmit
label|:
comment|/* 		 * Atomically get and clear transmit and status change 		 * interrupts. 		 */
name|s
operator|=
name|splzs
argument_list|()
...
argument_list|)
expr_stmt|;
comment|/* 		 * Check for status changes.  If carrier has changed, 		 * and we want CTS output flow control, we have to fiddle 		 * the HFC bit (see zsparam).  If carrier is gone, and 		 * linesw l_modem returns 0, drop DTR. 		 */
if|if
condition|(
name|rr0
operator|&
