...
operator|)
return|;
comment|/* 	 * Allocate and clear memory for the queue 	 */
if|if
condition|(
name|hilp
operator|->
...
begin_comment
comment|/*  * Fork hook.  * Unmap queue from child's address space  */
end_comment
...
begin_comment
comment|/*  * Vfork hook.  * Associate queue with child when VM resources are passed.  */
end_comment
...
name|printf
argument_list|(
literal|"hilqvfork(%d): from %x to %x qnum %d, qprocp %x\n"
argument_list|,
name|u
operator|.
name|u_procp
...
begin_comment
comment|/*  * Exit hook.  * Unmap all devices and free all queues.  */
end_comment
...
directive|endif
comment|/* 	 * Atomically take all devices off the queue 	 */
name|mask
operator|=
operator|~
name|hilqmask
...
argument_list|)
expr_stmt|;
comment|/* 	 * Now unmap from user address space and free queue 	 */
name|i
operator|=
name|mp
operator|->
...
begin_comment
comment|/*  * This is just a copy of the virgin q_to_b routine with minor  * optimizations for HIL use.  It is used for two reasons:  * 1. If we have PAGE mode defined, the normal q_to_b processes  *    chars one at a time and breaks on newlines.  * 2. We don't have to raise the priority to spltty() for most  *    of the clist manipulations.  */
end_comment
