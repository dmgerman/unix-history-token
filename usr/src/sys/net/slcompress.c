...
begin_comment
comment|/*  * Routines to compress and uncompess tcp packets (for transmission  * over low speed serial lines.  *  * Copyright (c) 1988, 1989 by Van Jacobson, Lawrence Berkeley Laboratory  * All rights reserved.  */
end_comment
...
index|[]
init|=
literal|"$Header: slcompress.c,v 1.7 89/03/19 18:10:19 van Locked $"
decl_stmt|;
end_decl_stmt
...
argument_list|)
expr_stmt|;
comment|/* 	 * At this point, cp points to the first byte of data in the 	 * packet (if any).  Toss the compressed header from the 	 * original packet, allocatate a new mbuf for the uncompressed 	 * header (to make sure it's aligned correctly), then chain it 	 * in front of the original.  Set up the ip length& ip checksum then 	 * return the rebuilt packet. 	 */
name|changes
operator|=
name|cp
operator|-
...
argument_list|)
expr_stmt|;
comment|/*MGET(m0, M_DONTWAIT, MT_DATA);*/
name|MGETHDR
argument_list|(
name|m0
argument_list|,
