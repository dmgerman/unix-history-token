begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1991 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from the Stanford/CMU enet packet filter,  * (net/enet.c) distributed as part of 4.3BSD, and code contributed  * to Berkeley by Steven McCanne of Lawrence Berkeley Laboratory.  *  * %sccs.include.redist.c%  *  *	@(#)bpf_filter.c	7.2 (Berkeley) %G%  *  * static char rcsid[] =  * "@(#) $Header: bpf_filter.c,v 1.10 91/04/24 22:07:07 mccanne Locked $ (LBL)";  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<net/bpf.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|sun
end_ifdef

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|defined
argument_list|(
name|sparc
argument_list|)
operator|||
name|defined
argument_list|(
name|mips
argument_list|)
end_if

begin_define
define|#
directive|define
name|ALIGN
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_ifndef
ifndef|#
directive|ifndef
name|ALIGN
end_ifndef

begin_define
define|#
directive|define
name|EXTRACT_SHORT
parameter_list|(
name|p
parameter_list|)
value|(ntohs(*(u_short *)p))
end_define

begin_define
define|#
directive|define
name|EXTRACT_LONG
parameter_list|(
name|p
parameter_list|)
value|(ntohl(*(u_long *)p))
end_define

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|EXTRACT_SHORT
parameter_list|(
name|p
parameter_list|)
define|\
value|((u_short)\ 		(*((u_char *)(p)+0)<<8|\ 		 *((u_char *)(p)+1)<<0))
end_define

begin_define
define|#
directive|define
name|EXTRACT_LONG
parameter_list|(
name|p
parameter_list|)
define|\
value|(*((u_char *)(p)+0)<<24|\ 		 *((u_char *)(p)+1)<<16|\ 		 *((u_char *)(p)+2)<<8|\ 		 *((u_char *)(p)+3)<<0)
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_ifdef
ifdef|#
directive|ifdef
name|KERNEL
end_ifdef

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_define
define|#
directive|define
name|MINDEX
parameter_list|(
name|m
parameter_list|,
name|k
parameter_list|)
define|\
value|{ \ 	register int len = m->m_len; \  \ 	while (k>= len) { \ 		k -= len; \ 		m = m->m_next; \ 		if (m == 0) \ 			return 0; \ 		len = m->m_len; \ 	} \ }
end_define

begin_function
specifier|static
name|int
name|m_xword
parameter_list|(
name|m
parameter_list|,
name|k
parameter_list|,
name|err
parameter_list|)
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
specifier|register
name|int
name|k
decl_stmt|,
decl|*
name|err
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|int
name|len
decl_stmt|;
specifier|register
name|u_char
modifier|*
name|cp
decl_stmt|,
modifier|*
name|np
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
name|len
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
while|while
condition|(
name|k
operator|>=
name|len
condition|)
block|{
name|k
operator|-=
name|len
expr_stmt|;
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
goto|goto
name|bad
goto|;
name|len
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
block|}
name|cp
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|u_char
operator|*
argument_list|)
operator|+
name|k
expr_stmt|;
if|if
condition|(
name|len
operator|-
name|k
operator|>=
literal|4
condition|)
block|{
operator|*
name|err
operator|=
literal|0
expr_stmt|;
return|return
name|EXTRACT_LONG
argument_list|(
name|cp
argument_list|)
return|;
block|}
name|m0
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
if|if
condition|(
name|m0
operator|==
literal|0
operator|||
name|m0
operator|->
name|m_len
operator|+
name|len
operator|-
name|k
operator|<
literal|4
condition|)
goto|goto
name|bad
goto|;
operator|*
name|err
operator|=
literal|0
expr_stmt|;
name|np
operator|=
name|mtod
argument_list|(
name|m0
argument_list|,
name|u_char
operator|*
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|len
operator|-
name|k
condition|)
block|{
case|case
literal|1
case|:
return|return
operator|(
name|cp
index|[
name|k
index|]
operator|<<
literal|24
operator|)
operator||
operator|(
name|np
index|[
literal|0
index|]
operator|<<
literal|16
operator|)
operator||
operator|(
name|np
index|[
literal|1
index|]
operator|<<
literal|8
operator|)
operator||
name|np
index|[
literal|2
index|]
return|;
case|case
literal|2
case|:
return|return
operator|(
name|cp
index|[
name|k
index|]
operator|<<
literal|24
operator|)
operator||
operator|(
name|cp
index|[
name|k
operator|+
literal|1
index|]
operator|<<
literal|16
operator|)
operator||
operator|(
name|np
index|[
literal|0
index|]
operator|<<
literal|8
operator|)
operator||
name|np
index|[
literal|1
index|]
return|;
default|default:
return|return
operator|(
name|cp
index|[
name|k
index|]
operator|<<
literal|24
operator|)
operator||
operator|(
name|cp
index|[
name|k
operator|+
literal|1
index|]
operator|<<
literal|16
operator|)
operator||
operator|(
name|cp
index|[
name|k
operator|+
literal|2
index|]
operator|<<
literal|8
operator|)
operator||
name|np
index|[
literal|0
index|]
return|;
block|}
name|bad
label|:
operator|*
name|err
operator|=
literal|1
expr_stmt|;
return|return
literal|0
return|;
block|}
end_block

begin_function
specifier|static
name|int
name|m_xhalf
parameter_list|(
name|m
parameter_list|,
name|k
parameter_list|,
name|err
parameter_list|)
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
specifier|register
name|int
name|k
decl_stmt|,
decl|*
name|err
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|int
name|len
decl_stmt|;
specifier|register
name|u_char
modifier|*
name|cp
decl_stmt|,
modifier|*
name|np
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
name|len
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
while|while
condition|(
name|k
operator|>=
name|len
condition|)
block|{
name|k
operator|-=
name|len
expr_stmt|;
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
goto|goto
name|bad
goto|;
name|len
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
block|}
name|cp
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|u_char
operator|*
argument_list|)
operator|+
name|k
expr_stmt|;
if|if
condition|(
name|len
operator|-
name|k
operator|>=
literal|2
condition|)
block|{
operator|*
name|err
operator|=
literal|0
expr_stmt|;
return|return
name|EXTRACT_SHORT
argument_list|(
name|cp
argument_list|)
return|;
block|}
name|m0
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
if|if
condition|(
name|m0
operator|==
literal|0
condition|)
goto|goto
name|bad
goto|;
operator|*
name|err
operator|=
literal|0
expr_stmt|;
return|return
operator|(
name|cp
index|[
name|k
index|]
operator|<<
literal|8
operator|)
operator||
name|mtod
argument_list|(
name|m0
argument_list|,
name|u_char
operator|*
argument_list|)
index|[
literal|0
index|]
return|;
name|bad
label|:
operator|*
name|err
operator|=
literal|1
expr_stmt|;
return|return
literal|0
return|;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Execute the filter program starting at pc on the packet p  * wirelen is the length of the original packet  * buflen is the amount of data present  */
end_comment

begin_function
name|u_int
name|bpf_filter
parameter_list|(
name|pc
parameter_list|,
name|p
parameter_list|,
name|wirelen
parameter_list|,
name|buflen
parameter_list|)
specifier|register
name|struct
name|bpf_insn
modifier|*
name|pc
decl_stmt|;
specifier|register
name|u_char
modifier|*
name|p
decl_stmt|;
name|u_int
name|wirelen
decl_stmt|;
specifier|register
name|u_int
name|buflen
decl_stmt|;
block|{
specifier|register
name|long
name|A
decl_stmt|,
name|X
decl_stmt|;
specifier|register
name|int
name|k
decl_stmt|;
name|long
name|mem
index|[
name|BPF_MEMWORDS
index|]
decl_stmt|;
if|if
condition|(
name|pc
operator|==
literal|0
condition|)
comment|/* 		 * No filter means accept all. 		 */
return|return
operator|(
name|u_int
operator|)
operator|-
literal|1
return|;
ifdef|#
directive|ifdef
name|lint
name|A
operator|=
literal|0
expr_stmt|;
name|X
operator|=
literal|0
expr_stmt|;
endif|#
directive|endif
operator|--
name|pc
expr_stmt|;
while|while
condition|(
literal|1
condition|)
block|{
operator|++
name|pc
expr_stmt|;
switch|switch
condition|(
name|pc
operator|->
name|code
condition|)
block|{
default|default:
ifdef|#
directive|ifdef
name|KERNEL
return|return
literal|0
return|;
else|#
directive|else
name|abort
argument_list|()
expr_stmt|;
endif|#
directive|endif
case|case
name|BPF_RET
operator||
name|BPF_K
case|:
return|return
operator|(
name|u_int
operator|)
name|pc
operator|->
name|k
return|;
case|case
name|BPF_RET
operator||
name|BPF_A
case|:
return|return
operator|(
name|u_int
operator|)
name|A
return|;
case|case
name|BPF_LD
operator||
name|BPF_W
operator||
name|BPF_ABS
case|:
name|k
operator|=
name|pc
operator|->
name|k
expr_stmt|;
if|if
condition|(
name|k
operator|+
sizeof|sizeof
argument_list|(
name|long
argument_list|)
operator|>
name|buflen
condition|)
block|{
ifdef|#
directive|ifdef
name|KERNEL
name|int
name|merr
decl_stmt|;
if|if
condition|(
name|buflen
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
name|A
operator|=
name|m_xword
argument_list|(
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|p
argument_list|,
name|k
argument_list|,
operator|&
name|merr
argument_list|)
expr_stmt|;
if|if
condition|(
name|merr
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
continue|continue;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
ifdef|#
directive|ifdef
name|ALIGN
if|if
condition|(
operator|(
call|(
name|int
call|)
argument_list|(
name|p
operator|+
name|k
argument_list|)
operator|&
literal|3
operator|)
operator|!=
literal|0
condition|)
name|A
operator|=
name|EXTRACT_LONG
argument_list|(
operator|&
name|p
index|[
name|k
index|]
argument_list|)
expr_stmt|;
else|else
endif|#
directive|endif
name|A
operator|=
operator|*
operator|(
name|long
operator|*
operator|)
operator|(
name|p
operator|+
name|k
operator|)
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_H
operator||
name|BPF_ABS
case|:
name|k
operator|=
name|pc
operator|->
name|k
expr_stmt|;
if|if
condition|(
name|k
operator|+
sizeof|sizeof
argument_list|(
name|short
argument_list|)
operator|>
name|buflen
condition|)
block|{
ifdef|#
directive|ifdef
name|KERNEL
name|int
name|merr
decl_stmt|;
if|if
condition|(
name|buflen
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
name|A
operator|=
name|m_xhalf
argument_list|(
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|p
argument_list|,
name|k
argument_list|,
operator|&
name|merr
argument_list|)
expr_stmt|;
continue|continue;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
name|A
operator|=
name|EXTRACT_SHORT
argument_list|(
operator|&
name|p
index|[
name|k
index|]
argument_list|)
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_B
operator||
name|BPF_ABS
case|:
name|k
operator|=
name|pc
operator|->
name|k
expr_stmt|;
if|if
condition|(
name|k
operator|>=
name|buflen
condition|)
block|{
ifdef|#
directive|ifdef
name|KERNEL
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
if|if
condition|(
name|buflen
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
name|m
operator|=
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|p
expr_stmt|;
name|MINDEX
argument_list|(
name|m
argument_list|,
name|k
argument_list|)
expr_stmt|;
name|A
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|u_char
operator|*
argument_list|)
index|[
name|k
index|]
expr_stmt|;
continue|continue;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
name|A
operator|=
name|p
index|[
name|k
index|]
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_W
operator||
name|BPF_LEN
case|:
name|A
operator|=
name|wirelen
expr_stmt|;
continue|continue;
case|case
name|BPF_LDX
operator||
name|BPF_W
operator||
name|BPF_LEN
case|:
name|X
operator|=
name|wirelen
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_W
operator||
name|BPF_IND
case|:
name|k
operator|=
name|X
operator|+
name|pc
operator|->
name|k
expr_stmt|;
if|if
condition|(
name|k
operator|+
sizeof|sizeof
argument_list|(
name|long
argument_list|)
operator|>
name|buflen
condition|)
block|{
ifdef|#
directive|ifdef
name|KERNEL
name|int
name|merr
decl_stmt|;
if|if
condition|(
name|buflen
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
name|A
operator|=
name|m_xword
argument_list|(
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|p
argument_list|,
name|k
argument_list|,
operator|&
name|merr
argument_list|)
expr_stmt|;
if|if
condition|(
name|merr
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
continue|continue;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
ifdef|#
directive|ifdef
name|ALIGN
if|if
condition|(
operator|(
call|(
name|int
call|)
argument_list|(
name|p
operator|+
name|k
argument_list|)
operator|&
literal|3
operator|)
operator|!=
literal|0
condition|)
name|A
operator|=
name|EXTRACT_LONG
argument_list|(
operator|&
name|p
index|[
name|k
index|]
argument_list|)
expr_stmt|;
else|else
endif|#
directive|endif
name|A
operator|=
operator|*
operator|(
name|long
operator|*
operator|)
operator|(
name|p
operator|+
name|k
operator|)
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_H
operator||
name|BPF_IND
case|:
name|k
operator|=
name|X
operator|+
name|pc
operator|->
name|k
expr_stmt|;
if|if
condition|(
name|k
operator|+
sizeof|sizeof
argument_list|(
name|short
argument_list|)
operator|>
name|buflen
condition|)
block|{
ifdef|#
directive|ifdef
name|KERNEL
name|int
name|merr
decl_stmt|;
if|if
condition|(
name|buflen
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
name|A
operator|=
name|m_xhalf
argument_list|(
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|p
argument_list|,
name|k
argument_list|,
operator|&
name|merr
argument_list|)
expr_stmt|;
if|if
condition|(
name|merr
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
continue|continue;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
name|A
operator|=
name|EXTRACT_SHORT
argument_list|(
operator|&
name|p
index|[
name|k
index|]
argument_list|)
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_B
operator||
name|BPF_IND
case|:
name|k
operator|=
name|X
operator|+
name|pc
operator|->
name|k
expr_stmt|;
if|if
condition|(
name|k
operator|>=
name|buflen
condition|)
block|{
ifdef|#
directive|ifdef
name|KERNEL
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
if|if
condition|(
name|buflen
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
name|m
operator|=
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|p
expr_stmt|;
name|MINDEX
argument_list|(
name|m
argument_list|,
name|k
argument_list|)
expr_stmt|;
name|A
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|char
operator|*
argument_list|)
index|[
name|k
index|]
expr_stmt|;
continue|continue;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
name|A
operator|=
name|p
index|[
name|k
index|]
expr_stmt|;
continue|continue;
case|case
name|BPF_LDX
operator||
name|BPF_MSH
operator||
name|BPF_B
case|:
name|k
operator|=
name|pc
operator|->
name|k
expr_stmt|;
if|if
condition|(
name|k
operator|>=
name|buflen
condition|)
block|{
ifdef|#
directive|ifdef
name|KERNEL
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
if|if
condition|(
name|buflen
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
name|m
operator|=
operator|(
expr|struct
name|mbuf
operator|*
operator|)
name|p
expr_stmt|;
name|MINDEX
argument_list|(
name|m
argument_list|,
name|k
argument_list|)
expr_stmt|;
name|X
operator|=
operator|(
name|mtod
argument_list|(
name|m
argument_list|,
name|char
operator|*
argument_list|)
index|[
name|k
index|]
operator|&
literal|0xf
operator|)
operator|<<
literal|2
expr_stmt|;
continue|continue;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
name|X
operator|=
operator|(
name|p
index|[
name|pc
operator|->
name|k
index|]
operator|&
literal|0xf
operator|)
operator|<<
literal|2
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_IMM
case|:
name|A
operator|=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_LDX
operator||
name|BPF_IMM
case|:
name|X
operator|=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_LD
operator||
name|BPF_MEM
case|:
name|A
operator|=
name|mem
index|[
name|pc
operator|->
name|k
index|]
expr_stmt|;
continue|continue;
case|case
name|BPF_LDX
operator||
name|BPF_MEM
case|:
name|X
operator|=
name|mem
index|[
name|pc
operator|->
name|k
index|]
expr_stmt|;
continue|continue;
case|case
name|BPF_ST
case|:
name|mem
index|[
name|pc
operator|->
name|k
index|]
operator|=
name|A
expr_stmt|;
continue|continue;
case|case
name|BPF_STX
case|:
name|mem
index|[
name|pc
operator|->
name|k
index|]
operator|=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JA
case|:
name|pc
operator|+=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JGT
operator||
name|BPF_K
case|:
name|pc
operator|+=
operator|(
name|A
operator|>
name|pc
operator|->
name|k
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JGE
operator||
name|BPF_K
case|:
name|pc
operator|+=
operator|(
name|A
operator|>=
name|pc
operator|->
name|k
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JEQ
operator||
name|BPF_K
case|:
name|pc
operator|+=
operator|(
name|A
operator|==
name|pc
operator|->
name|k
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JSET
operator||
name|BPF_K
case|:
name|pc
operator|+=
operator|(
name|A
operator|&
name|pc
operator|->
name|k
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JGT
operator||
name|BPF_X
case|:
name|pc
operator|+=
operator|(
name|A
operator|>
name|X
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JGE
operator||
name|BPF_X
case|:
name|pc
operator|+=
operator|(
name|A
operator|>=
name|X
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JEQ
operator||
name|BPF_X
case|:
name|pc
operator|+=
operator|(
name|A
operator|==
name|X
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_JMP
operator||
name|BPF_JSET
operator||
name|BPF_X
case|:
name|pc
operator|+=
operator|(
name|A
operator|&
name|X
operator|)
condition|?
name|pc
operator|->
name|jt
else|:
name|pc
operator|->
name|jf
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_ADD
operator||
name|BPF_X
case|:
name|A
operator|+=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_SUB
operator||
name|BPF_X
case|:
name|A
operator|-=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_MUL
operator||
name|BPF_X
case|:
name|A
operator|*=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_DIV
operator||
name|BPF_X
case|:
if|if
condition|(
name|X
operator|==
literal|0
condition|)
return|return
literal|0
return|;
name|A
operator|/=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_AND
operator||
name|BPF_X
case|:
name|A
operator|&=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_OR
operator||
name|BPF_X
case|:
name|A
operator||=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_LSH
operator||
name|BPF_X
case|:
name|A
operator|<<=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_RSH
operator||
name|BPF_X
case|:
name|A
operator|>>=
name|X
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_ADD
operator||
name|BPF_K
case|:
name|A
operator|+=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_SUB
operator||
name|BPF_K
case|:
name|A
operator|-=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_MUL
operator||
name|BPF_K
case|:
name|A
operator|*=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_DIV
operator||
name|BPF_K
case|:
name|A
operator|/=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_AND
operator||
name|BPF_K
case|:
name|A
operator|&=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_OR
operator||
name|BPF_K
case|:
name|A
operator||=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_LSH
operator||
name|BPF_K
case|:
name|A
operator|<<=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_RSH
operator||
name|BPF_K
case|:
name|A
operator|>>=
name|pc
operator|->
name|k
expr_stmt|;
continue|continue;
case|case
name|BPF_ALU
operator||
name|BPF_NEG
case|:
name|A
operator|=
operator|-
name|A
expr_stmt|;
continue|continue;
case|case
name|BPF_MISC
operator||
name|BPF_TAX
case|:
name|X
operator|=
name|A
expr_stmt|;
continue|continue;
case|case
name|BPF_MISC
operator||
name|BPF_TXA
case|:
name|A
operator|=
name|X
expr_stmt|;
continue|continue;
block|}
block|}
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|KERNEL
end_ifdef

begin_comment
comment|/*  * Return true if the 'fcode' is a valid filter program.  * The constraints are that each jump be forward and to a valid  * code.  The code must terminate with either an accept or reject.   * 'valid' is an array for use by the routine (it must be at least  * 'len' bytes long).    *  * The kernel needs to be able to verify an application's filter code.  * Otherwise, a bogus program could easily crash the system.  */
end_comment

begin_function
name|int
name|bpf_validate
parameter_list|(
name|f
parameter_list|,
name|len
parameter_list|)
name|struct
name|bpf_insn
modifier|*
name|f
decl_stmt|;
name|int
name|len
decl_stmt|;
block|{
specifier|register
name|int
name|i
decl_stmt|;
specifier|register
name|struct
name|bpf_insn
modifier|*
name|p
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|len
condition|;
operator|++
name|i
control|)
block|{
comment|/* 		 * Check that that jumps are forward, and within  		 * the code block. 		 */
name|p
operator|=
operator|&
name|f
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|BPF_CLASS
argument_list|(
name|p
operator|->
name|code
argument_list|)
operator|==
name|BPF_JMP
condition|)
block|{
specifier|register
name|int
name|from
init|=
name|i
operator|+
literal|1
decl_stmt|;
if|if
condition|(
name|BPF_OP
argument_list|(
name|p
operator|->
name|code
argument_list|)
operator|==
name|BPF_JA
condition|)
block|{
if|if
condition|(
name|from
operator|+
name|p
operator|->
name|k
operator|>=
name|len
condition|)
return|return
literal|0
return|;
block|}
elseif|else
if|if
condition|(
name|from
operator|+
name|p
operator|->
name|jt
operator|>=
name|len
operator|||
name|from
operator|+
name|p
operator|->
name|jf
operator|>=
name|len
condition|)
return|return
literal|0
return|;
block|}
comment|/* 		 * Check that memory operations use valid addresses. 		 */
if|if
condition|(
operator|(
name|BPF_CLASS
argument_list|(
name|p
operator|->
name|code
argument_list|)
operator|==
name|BPF_ST
operator|||
operator|(
name|BPF_CLASS
argument_list|(
name|p
operator|->
name|code
argument_list|)
operator|==
name|BPF_LD
operator|&&
operator|(
name|p
operator|->
name|code
operator|&
literal|0xe0
operator|)
operator|==
name|BPF_MEM
operator|)
operator|)
operator|&&
operator|(
name|p
operator|->
name|k
operator|>=
name|BPF_MEMWORDS
operator|||
name|p
operator|->
name|k
operator|<
literal|0
operator|)
condition|)
return|return
literal|0
return|;
comment|/* 		 * Check for constant division by 0. 		 */
if|if
condition|(
name|p
operator|->
name|code
operator|==
name|BPF_ALU
operator||
name|BPF_DIV
operator||
name|BPF_K
operator|&&
name|p
operator|->
name|k
operator|==
literal|0
condition|)
return|return;
block|}
return|return
name|BPF_CLASS
argument_list|(
name|f
index|[
name|len
operator|-
literal|1
index|]
operator|.
name|code
argument_list|)
operator|==
name|BPF_RET
return|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

end_unit

