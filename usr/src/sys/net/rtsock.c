begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1988 Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)rtsock.c	7.14 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"mbuf.h"
end_include

begin_include
include|#
directive|include
file|"user.h"
end_include

begin_include
include|#
directive|include
file|"proc.h"
end_include

begin_include
include|#
directive|include
file|"socket.h"
end_include

begin_include
include|#
directive|include
file|"socketvar.h"
end_include

begin_include
include|#
directive|include
file|"domain.h"
end_include

begin_include
include|#
directive|include
file|"protosw.h"
end_include

begin_include
include|#
directive|include
file|"errno.h"
end_include

begin_include
include|#
directive|include
file|"af.h"
end_include

begin_include
include|#
directive|include
file|"if.h"
end_include

begin_include
include|#
directive|include
file|"route.h"
end_include

begin_include
include|#
directive|include
file|"raw_cb.h"
end_include

begin_include
include|#
directive|include
file|"machine/mtpr.h"
end_include

begin_decl_stmt
name|struct
name|sockaddr
name|route_dst
init|=
block|{
literal|2
block|,
name|PF_ROUTE
block|, }
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
name|route_src
init|=
block|{
literal|2
block|,
name|PF_ROUTE
block|, }
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockproto
name|route_proto
init|=
block|{
name|PF_ROUTE
block|, }
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_expr_stmt
name|route_usrreq
argument_list|(
name|so
argument_list|,
name|req
argument_list|,
name|m
argument_list|,
name|nam
argument_list|,
name|control
argument_list|)
specifier|register
expr|struct
name|socket
operator|*
name|so
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|req
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|nam
decl_stmt|,
modifier|*
name|control
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|error
init|=
literal|0
decl_stmt|;
specifier|register
name|struct
name|rawcb
modifier|*
name|rp
init|=
name|sotorawcb
argument_list|(
name|so
argument_list|)
decl_stmt|;
name|int
name|s
decl_stmt|;
if|if
condition|(
name|req
operator|==
name|PRU_ATTACH
condition|)
block|{
name|MALLOC
argument_list|(
name|rp
argument_list|,
expr|struct
name|rawcb
operator|*
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|rp
argument_list|)
argument_list|,
name|M_PCB
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
if|if
condition|(
name|so
operator|->
name|so_pcb
operator|=
operator|(
name|caddr_t
operator|)
name|rp
condition|)
name|bzero
argument_list|(
name|so
operator|->
name|so_pcb
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|rp
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|req
operator|==
name|PRU_DETACH
operator|&&
name|rp
condition|)
block|{
name|int
name|af
init|=
name|rp
operator|->
name|rcb_proto
operator|.
name|sp_protocol
decl_stmt|;
if|if
condition|(
name|af
operator|==
name|AF_INET
condition|)
name|route_cb
operator|.
name|ip_count
operator|--
expr_stmt|;
elseif|else
if|if
condition|(
name|af
operator|==
name|AF_NS
condition|)
name|route_cb
operator|.
name|ns_count
operator|--
expr_stmt|;
elseif|else
if|if
condition|(
name|af
operator|==
name|AF_ISO
condition|)
name|route_cb
operator|.
name|iso_count
operator|--
expr_stmt|;
name|route_cb
operator|.
name|any_count
operator|--
expr_stmt|;
block|}
name|s
operator|=
name|splnet
argument_list|()
expr_stmt|;
name|error
operator|=
name|raw_usrreq
argument_list|(
name|so
argument_list|,
name|req
argument_list|,
name|m
argument_list|,
name|nam
argument_list|,
name|control
argument_list|)
expr_stmt|;
name|rp
operator|=
name|sotorawcb
argument_list|(
name|so
argument_list|)
expr_stmt|;
if|if
condition|(
name|req
operator|==
name|PRU_ATTACH
operator|&&
name|rp
condition|)
block|{
name|int
name|af
init|=
name|rp
operator|->
name|rcb_proto
operator|.
name|sp_protocol
decl_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|free
argument_list|(
operator|(
name|caddr_t
operator|)
name|rp
argument_list|,
name|M_PCB
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
if|if
condition|(
name|af
operator|==
name|AF_INET
condition|)
name|route_cb
operator|.
name|ip_count
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|af
operator|==
name|AF_NS
condition|)
name|route_cb
operator|.
name|ns_count
operator|++
expr_stmt|;
elseif|else
if|if
condition|(
name|af
operator|==
name|AF_ISO
condition|)
name|route_cb
operator|.
name|iso_count
operator|++
expr_stmt|;
name|rp
operator|->
name|rcb_faddr
operator|=
operator|&
name|route_src
expr_stmt|;
name|route_cb
operator|.
name|any_count
operator|++
expr_stmt|;
name|soisconnected
argument_list|(
name|so
argument_list|)
expr_stmt|;
name|so
operator|->
name|so_options
operator||=
name|SO_USELOOPBACK
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_define
define|#
directive|define
name|ROUNDUP
parameter_list|(
name|a
parameter_list|)
value|(1 + (((a) - 1) | (sizeof(long) - 1)))
end_define

begin_define
define|#
directive|define
name|ADVANCE
parameter_list|(
name|x
parameter_list|,
name|n
parameter_list|)
value|(x += ((n)> 0 ? ROUNDUP(n) : sizeof(long)))
end_define

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_expr_stmt
name|route_output
argument_list|(
name|m
argument_list|,
name|so
argument_list|)
specifier|register
expr|struct
name|mbuf
operator|*
name|m
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|socket
modifier|*
name|so
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|rt_msghdr
modifier|*
name|rtm
init|=
literal|0
decl_stmt|;
specifier|register
name|struct
name|rtentry
modifier|*
name|rt
init|=
literal|0
decl_stmt|;
name|struct
name|rtentry
modifier|*
name|saved_nrt
init|=
literal|0
decl_stmt|;
name|struct
name|sockaddr
modifier|*
name|dst
init|=
literal|0
decl_stmt|,
modifier|*
name|gate
init|=
literal|0
decl_stmt|,
modifier|*
name|netmask
init|=
literal|0
decl_stmt|,
modifier|*
name|genmask
init|=
literal|0
decl_stmt|;
name|struct
name|sockaddr
modifier|*
name|ifpaddr
init|=
literal|0
decl_stmt|;
name|caddr_t
name|cp
decl_stmt|,
name|lim
decl_stmt|;
name|int
name|len
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|;
name|struct
name|ifnet
modifier|*
name|ifp
init|=
literal|0
decl_stmt|;
name|struct
name|ifaddr
modifier|*
name|ifa
decl_stmt|;
specifier|extern
name|struct
name|ifaddr
modifier|*
name|ifaof_ifpforaddr
argument_list|()
decl_stmt|,
modifier|*
name|ifa_ifwithroute
argument_list|()
decl_stmt|;
define|#
directive|define
name|senderr
parameter_list|(
name|e
parameter_list|)
value|{ error = e; goto flush;}
if|if
condition|(
name|m
operator|==
literal|0
operator|||
name|m
operator|->
name|m_len
operator|<
sizeof|sizeof
argument_list|(
name|long
argument_list|)
condition|)
return|return
operator|(
name|ENOBUFS
operator|)
return|;
if|if
condition|(
operator|(
name|m
operator|=
name|m_pullup
argument_list|(
name|m
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
name|ENOBUFS
operator|)
return|;
if|if
condition|(
operator|(
name|m
operator|->
name|m_flags
operator|&
name|M_PKTHDR
operator|)
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"route_output"
argument_list|)
expr_stmt|;
name|len
operator|=
name|m
operator|->
name|m_pkthdr
operator|.
name|len
expr_stmt|;
if|if
condition|(
name|len
operator|<
sizeof|sizeof
argument_list|(
operator|*
name|rtm
argument_list|)
operator|||
name|len
operator|!=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|rt_msghdr
operator|*
argument_list|)
operator|->
name|rtm_msglen
condition|)
name|senderr
argument_list|(
name|EINVAL
argument_list|)
expr_stmt|;
name|R_Malloc
argument_list|(
name|rtm
argument_list|,
expr|struct
name|rt_msghdr
operator|*
argument_list|,
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
name|rtm
operator|==
literal|0
condition|)
name|senderr
argument_list|(
name|ENOBUFS
argument_list|)
expr_stmt|;
name|m_copydata
argument_list|(
name|m
argument_list|,
literal|0
argument_list|,
name|len
argument_list|,
operator|(
name|caddr_t
operator|)
name|rtm
argument_list|)
expr_stmt|;
if|if
condition|(
name|rtm
operator|->
name|rtm_version
operator|!=
name|RTM_VERSION
condition|)
name|senderr
argument_list|(
name|EPROTONOSUPPORT
argument_list|)
expr_stmt|;
name|rtm
operator|->
name|rtm_pid
operator|=
name|u
operator|.
name|u_procp
operator|->
name|p_pid
expr_stmt|;
name|lim
operator|=
name|len
operator|+
operator|(
name|caddr_t
operator|)
name|rtm
expr_stmt|;
name|cp
operator|=
call|(
name|caddr_t
call|)
argument_list|(
name|rtm
operator|+
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|rtm
operator|->
name|rtm_addrs
operator|&
name|RTA_DST
condition|)
block|{
name|dst
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|cp
expr_stmt|;
name|ADVANCE
argument_list|(
name|cp
argument_list|,
name|dst
operator|->
name|sa_len
argument_list|)
expr_stmt|;
block|}
else|else
name|senderr
argument_list|(
name|EINVAL
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|rtm
operator|->
name|rtm_addrs
operator|&
name|RTA_GATEWAY
operator|)
operator|&&
name|cp
operator|<
name|lim
condition|)
block|{
name|gate
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|cp
expr_stmt|;
name|cp
operator|+=
name|ROUNDUP
argument_list|(
name|gate
operator|->
name|sa_len
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|rtm
operator|->
name|rtm_addrs
operator|&
name|RTA_NETMASK
operator|)
operator|&&
name|cp
operator|<
name|lim
condition|)
block|{
name|netmask
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|cp
expr_stmt|;
name|ADVANCE
argument_list|(
name|cp
argument_list|,
name|netmask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|rtm
operator|->
name|rtm_addrs
operator|&
name|RTA_GENMASK
operator|)
operator|&&
name|cp
operator|<
name|lim
condition|)
block|{
name|struct
name|radix_node
modifier|*
name|t
decl_stmt|,
modifier|*
name|rn_addmask
argument_list|()
decl_stmt|;
name|genmask
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|cp
expr_stmt|;
name|ADVANCE
argument_list|(
name|cp
argument_list|,
name|genmask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|t
operator|=
name|rn_addmask
argument_list|(
name|genmask
argument_list|,
literal|1
argument_list|,
literal|2
argument_list|)
expr_stmt|;
if|if
condition|(
name|t
operator|&&
name|Bcmp
argument_list|(
name|genmask
argument_list|,
name|t
operator|->
name|rn_key
argument_list|,
operator|*
operator|(
name|u_char
operator|*
operator|)
name|genmask
argument_list|)
operator|==
literal|0
condition|)
name|genmask
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|(
name|t
operator|->
name|rn_key
operator|)
expr_stmt|;
else|else
name|senderr
argument_list|(
name|ENOBUFS
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|rtm
operator|->
name|rtm_addrs
operator|&
name|RTA_IFP
operator|)
operator|&&
name|cp
operator|<
name|lim
condition|)
block|{
name|ifpaddr
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|cp
expr_stmt|;
block|}
switch|switch
condition|(
name|rtm
operator|->
name|rtm_type
condition|)
block|{
case|case
name|RTM_ADD
case|:
if|if
condition|(
name|gate
operator|==
literal|0
condition|)
name|senderr
argument_list|(
name|EINVAL
argument_list|)
expr_stmt|;
name|error
operator|=
name|rtrequest
argument_list|(
name|RTM_ADD
argument_list|,
name|dst
argument_list|,
name|gate
argument_list|,
name|netmask
argument_list|,
name|rtm
operator|->
name|rtm_flags
argument_list|,
operator|&
name|saved_nrt
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|==
literal|0
operator|&&
name|saved_nrt
condition|)
block|{
name|rt_setmetrics
argument_list|(
name|rtm
operator|->
name|rtm_inits
argument_list|,
operator|&
name|rtm
operator|->
name|rtm_rmx
argument_list|,
operator|&
name|saved_nrt
operator|->
name|rt_rmx
argument_list|)
expr_stmt|;
name|saved_nrt
operator|->
name|rt_refcnt
operator|--
expr_stmt|;
name|saved_nrt
operator|->
name|rt_genmask
operator|=
name|genmask
expr_stmt|;
block|}
break|break;
case|case
name|RTM_DELETE
case|:
name|error
operator|=
name|rtrequest
argument_list|(
name|RTM_DELETE
argument_list|,
name|dst
argument_list|,
name|gate
argument_list|,
name|netmask
argument_list|,
name|rtm
operator|->
name|rtm_flags
argument_list|,
operator|(
expr|struct
name|rtentry
operator|*
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
break|break;
case|case
name|RTM_GET
case|:
case|case
name|RTM_CHANGE
case|:
case|case
name|RTM_LOCK
case|:
name|rt
operator|=
name|rtalloc1
argument_list|(
name|dst
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|==
literal|0
condition|)
name|senderr
argument_list|(
name|ESRCH
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
operator|(
expr|struct
name|radix_node
operator|*
operator|)
name|rt
operator|)
operator|->
name|rn_dupedkey
condition|)
block|{
if|if
condition|(
name|netmask
operator|==
literal|0
operator|&&
name|rtm
operator|->
name|rtm_type
operator|!=
name|RTM_GET
condition|)
name|senderr
argument_list|(
name|ETOOMANYREFS
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|rtm
operator|->
name|rtm_type
condition|)
block|{
case|case
name|RTM_GET
case|:
name|dst
operator|=
name|rt_key
argument_list|(
name|rt
argument_list|)
expr_stmt|;
name|len
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|rtm
argument_list|)
operator|+
operator|(
name|dst
operator|->
name|sa_len
operator|>
literal|0
condition|?
name|ROUNDUP
argument_list|(
name|dst
operator|->
name|sa_len
argument_list|)
else|:
sizeof|sizeof
argument_list|(
name|long
argument_list|)
operator|)
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator||=
name|RTA_DST
expr_stmt|;
if|if
condition|(
name|gate
operator|=
name|rt
operator|->
name|rt_gateway
condition|)
block|{
name|len
operator|+=
name|ROUNDUP
argument_list|(
name|rt
operator|->
name|rt_gateway
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator||=
name|RTA_GATEWAY
expr_stmt|;
block|}
else|else
name|rtm
operator|->
name|rtm_addrs
operator|&=
operator|~
name|RTA_GATEWAY
expr_stmt|;
if|if
condition|(
name|netmask
operator|=
name|rt_mask
argument_list|(
name|rt
argument_list|)
condition|)
block|{
name|len
operator|+=
name|netmask
operator|->
name|sa_len
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator||=
name|RTA_NETMASK
expr_stmt|;
block|}
else|else
name|rtm
operator|->
name|rtm_addrs
operator|&=
operator|~
name|RTA_NETMASK
expr_stmt|;
if|if
condition|(
name|rt
operator|->
name|rt_genmask
condition|)
block|{
name|len
operator|+=
name|ROUNDUP
argument_list|(
name|rt
operator|->
name|rt_genmask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator||=
name|RTA_GENMASK
expr_stmt|;
block|}
else|else
name|rtm
operator|->
name|rtm_addrs
operator|&=
operator|~
name|RTA_GENMASK
expr_stmt|;
if|if
condition|(
name|len
operator|>
name|rtm
operator|->
name|rtm_msglen
condition|)
block|{
name|struct
name|rt_msghdr
modifier|*
name|new_rtm
decl_stmt|;
name|R_Malloc
argument_list|(
name|new_rtm
argument_list|,
expr|struct
name|rt_msghdr
operator|*
argument_list|,
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
name|new_rtm
operator|==
literal|0
condition|)
name|senderr
argument_list|(
name|ENOBUFS
argument_list|)
expr_stmt|;
name|Bcopy
argument_list|(
name|rtm
argument_list|,
name|new_rtm
argument_list|,
name|rtm
operator|->
name|rtm_msglen
argument_list|)
expr_stmt|;
name|Free
argument_list|(
name|rtm
argument_list|)
expr_stmt|;
name|rtm
operator|=
name|new_rtm
expr_stmt|;
block|}
name|rtm
operator|->
name|rtm_msglen
operator|=
name|len
expr_stmt|;
name|rtm
operator|->
name|rtm_flags
operator|=
name|rt
operator|->
name|rt_flags
expr_stmt|;
name|rtm
operator|->
name|rtm_rmx
operator|=
name|rt
operator|->
name|rt_rmx
expr_stmt|;
name|cp
operator|=
call|(
name|caddr_t
call|)
argument_list|(
literal|1
operator|+
name|rtm
argument_list|)
expr_stmt|;
name|Bcopy
argument_list|(
name|dst
argument_list|,
name|cp
argument_list|,
name|dst
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|ADVANCE
argument_list|(
name|cp
argument_list|,
name|dst
operator|->
name|sa_len
argument_list|)
expr_stmt|;
if|if
condition|(
name|gate
condition|)
block|{
name|Bcopy
argument_list|(
name|gate
argument_list|,
name|cp
argument_list|,
name|gate
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|ADVANCE
argument_list|(
name|cp
argument_list|,
name|gate
operator|->
name|sa_len
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|netmask
condition|)
block|{
name|Bcopy
argument_list|(
name|netmask
argument_list|,
name|cp
argument_list|,
name|netmask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|ADVANCE
argument_list|(
name|cp
argument_list|,
name|netmask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|rt
operator|->
name|rt_genmask
condition|)
block|{
name|Bcopy
argument_list|(
name|rt
operator|->
name|rt_genmask
argument_list|,
name|cp
argument_list|,
name|rt
operator|->
name|rt_genmask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|ADVANCE
argument_list|(
name|cp
argument_list|,
name|rt
operator|->
name|rt_genmask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|RTM_CHANGE
case|:
if|if
condition|(
name|gate
operator|&&
operator|(
name|gate
operator|->
name|sa_len
operator|>
operator|(
name|len
operator|=
name|rt
operator|->
name|rt_gateway
operator|->
name|sa_len
operator|)
operator|)
condition|)
name|senderr
argument_list|(
name|EDQUOT
argument_list|)
expr_stmt|;
if|if
condition|(
name|rt
operator|->
name|rt_ifa
operator|&&
name|rt
operator|->
name|rt_ifa
operator|->
name|ifa_rtrequest
condition|)
name|rt
operator|->
name|rt_ifa
operator|->
name|ifa_rtrequest
argument_list|(
name|RTM_DELETE
argument_list|,
name|rt
argument_list|,
name|gate
argument_list|)
expr_stmt|;
comment|/* new gateway could require new ifaddr, ifp; 			   flags may also be different; ifp may be specified 			   by ll sockaddr when protocol address is ambiguous */
if|if
condition|(
name|ifpaddr
operator|&&
operator|(
name|ifa
operator|=
name|ifa_ifwithnet
argument_list|(
name|ifpaddr
argument_list|)
operator|)
operator|&&
operator|(
name|ifp
operator|=
name|ifa
operator|->
name|ifa_ifp
operator|)
operator|&&
operator|(
name|ifa
operator|=
name|ifaof_ifpforaddr
argument_list|(
name|gate
argument_list|,
name|ifp
argument_list|)
operator|)
condition|)
block|{
comment|/* We got it */
block|}
else|else
block|{
name|ifa
operator|=
literal|0
expr_stmt|;
name|ifp
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|gate
condition|)
name|Bcopy
argument_list|(
name|gate
argument_list|,
name|rt
operator|->
name|rt_gateway
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|rt_setmetrics
argument_list|(
name|rtm
operator|->
name|rtm_inits
argument_list|,
operator|&
name|rtm
operator|->
name|rtm_rmx
argument_list|,
operator|&
name|rt
operator|->
name|rt_rmx
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifa
operator|==
literal|0
condition|)
name|ifa
operator|=
name|ifa_ifwithroute
argument_list|(
name|rt
operator|->
name|rt_flags
argument_list|,
name|rt_key
argument_list|(
name|rt
argument_list|)
argument_list|,
name|gate
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifa
condition|)
block|{
if|if
condition|(
name|rt
operator|->
name|rt_ifa
operator|!=
name|ifa
condition|)
block|{
name|rt
operator|->
name|rt_ifa
operator|=
name|ifa
expr_stmt|;
name|rt
operator|->
name|rt_ifp
operator|=
name|ifa
operator|->
name|ifa_ifp
expr_stmt|;
block|}
block|}
if|if
condition|(
name|rt
operator|->
name|rt_ifa
operator|&&
name|rt
operator|->
name|rt_ifa
operator|->
name|ifa_rtrequest
condition|)
name|rt
operator|->
name|rt_ifa
operator|->
name|ifa_rtrequest
argument_list|(
name|RTM_ADD
argument_list|,
name|rt
argument_list|,
name|gate
argument_list|)
expr_stmt|;
if|if
condition|(
name|genmask
condition|)
name|rt
operator|->
name|rt_genmask
operator|=
name|genmask
expr_stmt|;
comment|/* 			 * Fall into 			 */
case|case
name|RTM_LOCK
case|:
name|rt
operator|->
name|rt_rmx
operator|.
name|rmx_locks
operator||=
operator|(
name|rtm
operator|->
name|rtm_inits
operator|&
name|rtm
operator|->
name|rtm_rmx
operator|.
name|rmx_locks
operator|)
expr_stmt|;
name|rt
operator|->
name|rt_rmx
operator|.
name|rmx_locks
operator|&=
operator|~
operator|(
name|rtm
operator|->
name|rtm_inits
operator|)
expr_stmt|;
break|break;
block|}
goto|goto
name|cleanup
goto|;
default|default:
name|senderr
argument_list|(
name|EOPNOTSUPP
argument_list|)
expr_stmt|;
block|}
name|flush
label|:
if|if
condition|(
name|rtm
condition|)
block|{
if|if
condition|(
name|error
condition|)
name|rtm
operator|->
name|rtm_errno
operator|=
name|error
expr_stmt|;
else|else
name|rtm
operator|->
name|rtm_flags
operator||=
name|RTF_DONE
expr_stmt|;
block|}
name|cleanup
label|:
if|if
condition|(
name|rt
condition|)
name|rtfree
argument_list|(
name|rt
argument_list|)
expr_stmt|;
block|{
specifier|register
name|struct
name|rawcb
modifier|*
name|rp
init|=
literal|0
decl_stmt|;
comment|/* 	 * Check to see if we don't want our own messages. 	 */
if|if
condition|(
operator|(
name|so
operator|->
name|so_options
operator|&
name|SO_USELOOPBACK
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|route_cb
operator|.
name|any_count
operator|<=
literal|1
condition|)
block|{
if|if
condition|(
name|rtm
condition|)
name|Free
argument_list|(
name|rtm
argument_list|)
expr_stmt|;
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
comment|/* There is another listener, so construct message */
name|rp
operator|=
name|sotorawcb
argument_list|(
name|so
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|cp
operator|=
operator|(
name|caddr_t
operator|)
name|rtm
condition|)
block|{
name|m_copyback
argument_list|(
name|m
argument_list|,
literal|0
argument_list|,
name|len
argument_list|,
name|cp
argument_list|)
expr_stmt|;
name|Free
argument_list|(
name|rtm
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|rp
condition|)
name|rp
operator|->
name|rcb_proto
operator|.
name|sp_family
operator|=
literal|0
expr_stmt|;
comment|/* Avoid us */
if|if
condition|(
name|dst
condition|)
name|route_proto
operator|.
name|sp_protocol
operator|=
name|dst
operator|->
name|sa_family
expr_stmt|;
name|raw_input
argument_list|(
name|m
argument_list|,
operator|&
name|route_proto
argument_list|,
operator|&
name|route_src
argument_list|,
operator|&
name|route_dst
argument_list|)
expr_stmt|;
if|if
condition|(
name|rp
condition|)
name|rp
operator|->
name|rcb_proto
operator|.
name|sp_family
operator|=
name|PF_ROUTE
expr_stmt|;
block|}
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_macro
name|rt_setmetrics
argument_list|(
argument|which
argument_list|,
argument|in
argument_list|,
argument|out
argument_list|)
end_macro

begin_decl_stmt
name|u_long
name|which
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|struct
name|rt_metrics
modifier|*
name|in
decl_stmt|,
modifier|*
name|out
decl_stmt|;
end_decl_stmt

begin_block
block|{
define|#
directive|define
name|metric
parameter_list|(
name|f
parameter_list|,
name|e
parameter_list|)
value|if (which& (f)) out->e = in->e;
name|metric
argument_list|(
name|RTV_RPIPE
argument_list|,
name|rmx_recvpipe
argument_list|)
expr_stmt|;
name|metric
argument_list|(
name|RTV_SPIPE
argument_list|,
name|rmx_sendpipe
argument_list|)
expr_stmt|;
name|metric
argument_list|(
name|RTV_SSTHRESH
argument_list|,
name|rmx_ssthresh
argument_list|)
expr_stmt|;
name|metric
argument_list|(
name|RTV_RTT
argument_list|,
name|rmx_rtt
argument_list|)
expr_stmt|;
name|metric
argument_list|(
name|RTV_RTTVAR
argument_list|,
name|rmx_rttvar
argument_list|)
expr_stmt|;
name|metric
argument_list|(
name|RTV_HOPCOUNT
argument_list|,
name|rmx_hopcount
argument_list|)
expr_stmt|;
name|metric
argument_list|(
name|RTV_MTU
argument_list|,
name|rmx_mtu
argument_list|)
expr_stmt|;
undef|#
directive|undef
name|metric
block|}
end_block

begin_comment
comment|/*  * Copy data from a buffer back into the indicated mbuf chain,  * starting "off" bytes from the beginning, extending the mbuf  * chain if necessary.  */
end_comment

begin_macro
name|m_copyback
argument_list|(
argument|m0
argument_list|,
argument|off
argument_list|,
argument|len
argument_list|,
argument|cp
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|int
name|off
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|int
name|len
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|caddr_t
name|cp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|mlen
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
init|=
name|m0
decl_stmt|,
modifier|*
name|n
decl_stmt|;
name|int
name|totlen
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|m0
operator|==
literal|0
condition|)
return|return;
while|while
condition|(
name|off
operator|>=
operator|(
name|mlen
operator|=
name|m
operator|->
name|m_len
operator|)
condition|)
block|{
name|off
operator|-=
name|mlen
expr_stmt|;
name|totlen
operator|+=
name|mlen
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_next
operator|==
literal|0
condition|)
block|{
name|n
operator|=
name|m_getclr
argument_list|(
name|M_DONTWAIT
argument_list|,
name|m
operator|->
name|m_type
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
goto|goto
name|out
goto|;
name|n
operator|->
name|m_len
operator|=
name|min
argument_list|(
name|MLEN
argument_list|,
name|len
operator|+
name|off
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_next
operator|=
name|n
expr_stmt|;
block|}
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
block|}
while|while
condition|(
name|len
operator|>
literal|0
condition|)
block|{
name|mlen
operator|=
name|min
argument_list|(
name|m
operator|->
name|m_len
operator|-
name|off
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
name|cp
argument_list|,
name|off
operator|+
name|mtod
argument_list|(
name|m
argument_list|,
name|caddr_t
argument_list|)
argument_list|,
operator|(
name|unsigned
operator|)
name|mlen
argument_list|)
expr_stmt|;
name|cp
operator|+=
name|mlen
expr_stmt|;
name|len
operator|-=
name|mlen
expr_stmt|;
name|mlen
operator|+=
name|off
expr_stmt|;
name|off
operator|=
literal|0
expr_stmt|;
name|totlen
operator|+=
name|mlen
expr_stmt|;
if|if
condition|(
name|len
operator|==
literal|0
condition|)
break|break;
if|if
condition|(
name|m
operator|->
name|m_next
operator|==
literal|0
condition|)
block|{
name|n
operator|=
name|m_get
argument_list|(
name|M_DONTWAIT
argument_list|,
name|m
operator|->
name|m_type
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
break|break;
name|n
operator|->
name|m_len
operator|=
name|min
argument_list|(
name|MLEN
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_next
operator|=
name|n
expr_stmt|;
block|}
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
block|}
name|out
label|:
if|if
condition|(
operator|(
operator|(
name|m
operator|=
name|m0
operator|)
operator|->
name|m_flags
operator|&
name|M_PKTHDR
operator|)
operator|&&
operator|(
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|<
name|totlen
operator|)
condition|)
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|totlen
expr_stmt|;
block|}
end_block

begin_comment
comment|/*   * The miss message and losing message are very similar.  */
end_comment

begin_expr_stmt
name|rt_missmsg
argument_list|(
name|type
argument_list|,
name|dst
argument_list|,
name|gate
argument_list|,
name|mask
argument_list|,
name|src
argument_list|,
name|flags
argument_list|,
name|error
argument_list|)
specifier|register
expr|struct
name|sockaddr
operator|*
name|dst
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|gate
decl_stmt|,
modifier|*
name|mask
decl_stmt|,
modifier|*
name|src
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|rt_msghdr
modifier|*
name|rtm
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|int
name|dlen
init|=
name|ROUNDUP
argument_list|(
name|dst
operator|->
name|sa_len
argument_list|)
decl_stmt|;
name|int
name|len
init|=
name|dlen
operator|+
sizeof|sizeof
argument_list|(
operator|*
name|rtm
argument_list|)
decl_stmt|;
if|if
condition|(
name|route_cb
operator|.
name|any_count
operator|==
literal|0
condition|)
return|return;
name|m
operator|=
name|m_gethdr
argument_list|(
name|M_DONTWAIT
argument_list|,
name|MT_DATA
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
return|return;
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|m
operator|->
name|m_len
operator|=
name|min
argument_list|(
name|len
argument_list|,
name|MHLEN
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|rcvif
operator|=
literal|0
expr_stmt|;
name|rtm
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|rt_msghdr
operator|*
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
name|rtm
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|rtm
argument_list|)
argument_list|)
expr_stmt|;
comment|/*XXX assumes sizeof(*rtm)< MHLEN*/
name|rtm
operator|->
name|rtm_flags
operator|=
name|RTF_DONE
operator||
name|flags
expr_stmt|;
name|rtm
operator|->
name|rtm_msglen
operator|=
name|len
expr_stmt|;
name|rtm
operator|->
name|rtm_version
operator|=
name|RTM_VERSION
expr_stmt|;
name|rtm
operator|->
name|rtm_type
operator|=
name|type
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator|=
name|RTA_DST
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|RTM_OLDADD
operator|||
name|type
operator|==
name|RTM_OLDDEL
condition|)
block|{
name|rtm
operator|->
name|rtm_pid
operator|=
name|u
operator|.
name|u_procp
operator|->
name|p_pid
expr_stmt|;
block|}
name|m_copyback
argument_list|(
name|m
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|rtm
argument_list|)
argument_list|,
name|dlen
argument_list|,
operator|(
name|caddr_t
operator|)
name|dst
argument_list|)
expr_stmt|;
if|if
condition|(
name|gate
condition|)
block|{
name|dlen
operator|=
name|ROUNDUP
argument_list|(
name|gate
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|m_copyback
argument_list|(
name|m
argument_list|,
name|len
argument_list|,
name|dlen
argument_list|,
operator|(
name|caddr_t
operator|)
name|gate
argument_list|)
expr_stmt|;
name|len
operator|+=
name|dlen
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator||=
name|RTA_GATEWAY
expr_stmt|;
block|}
if|if
condition|(
name|mask
condition|)
block|{
if|if
condition|(
name|mask
operator|->
name|sa_len
condition|)
name|dlen
operator|=
name|ROUNDUP
argument_list|(
name|mask
operator|->
name|sa_len
argument_list|)
expr_stmt|;
else|else
name|dlen
operator|=
sizeof|sizeof
argument_list|(
name|long
argument_list|)
expr_stmt|;
name|m_copyback
argument_list|(
name|m
argument_list|,
name|len
argument_list|,
name|dlen
argument_list|,
operator|(
name|caddr_t
operator|)
name|mask
argument_list|)
expr_stmt|;
name|len
operator|+=
name|dlen
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator||=
name|RTA_NETMASK
expr_stmt|;
block|}
if|if
condition|(
name|src
condition|)
block|{
name|dlen
operator|=
name|ROUNDUP
argument_list|(
name|src
operator|->
name|sa_len
argument_list|)
expr_stmt|;
name|m_copyback
argument_list|(
name|m
argument_list|,
name|len
argument_list|,
name|dlen
argument_list|,
operator|(
name|caddr_t
operator|)
name|src
argument_list|)
expr_stmt|;
name|len
operator|+=
name|dlen
expr_stmt|;
name|rtm
operator|->
name|rtm_addrs
operator||=
name|RTA_AUTHOR
expr_stmt|;
block|}
if|if
condition|(
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|!=
name|len
condition|)
block|{
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return;
block|}
name|rtm
operator|->
name|rtm_errno
operator|=
name|error
expr_stmt|;
name|rtm
operator|->
name|rtm_msglen
operator|=
name|len
expr_stmt|;
name|route_proto
operator|.
name|sp_protocol
operator|=
name|dst
operator|->
name|sa_family
expr_stmt|;
name|raw_input
argument_list|(
name|m
argument_list|,
operator|&
name|route_proto
argument_list|,
operator|&
name|route_src
argument_list|,
operator|&
name|route_dst
argument_list|)
expr_stmt|;
block|}
end_block

begin_include
include|#
directive|include
file|"kinfo.h"
end_include

begin_struct
struct|struct
name|walkarg
block|{
name|int
name|w_op
decl_stmt|,
name|w_arg
decl_stmt|;
name|int
name|w_given
decl_stmt|,
name|w_needed
decl_stmt|;
name|caddr_t
name|w_where
decl_stmt|;
struct|struct
block|{
name|struct
name|rt_msghdr
name|m_rtm
decl_stmt|;
name|char
name|m_sabuf
index|[
literal|128
index|]
decl_stmt|;
block|}
name|w_m
struct|;
define|#
directive|define
name|w_rtm
value|w_m.m_rtm
block|}
struct|;
end_struct

begin_comment
comment|/*  * This is used in dumping the kernel table via getkinfo().  */
end_comment

begin_macro
name|rt_dumpentry
argument_list|(
argument|rn
argument_list|,
argument|w
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|radix_node
modifier|*
name|rn
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|struct
name|walkarg
modifier|*
name|w
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|sockaddr
modifier|*
name|sa
decl_stmt|;
name|int
name|n
decl_stmt|,
name|error
decl_stmt|;
for|for
control|(
init|;
name|rn
condition|;
name|rn
operator|=
name|rn
operator|->
name|rn_dupedkey
control|)
block|{
name|int
name|count
init|=
literal|0
decl_stmt|,
name|size
init|=
sizeof|sizeof
argument_list|(
name|w
operator|->
name|w_rtm
argument_list|)
decl_stmt|;
specifier|register
name|struct
name|rtentry
modifier|*
name|rt
init|=
operator|(
expr|struct
name|rtentry
operator|*
operator|)
name|rn
decl_stmt|;
if|if
condition|(
name|rn
operator|->
name|rn_flags
operator|&
name|RNF_ROOT
condition|)
continue|continue;
if|if
condition|(
name|w
operator|->
name|w_op
operator|==
name|KINFO_RT_FLAGS
operator|&&
operator|!
operator|(
name|rt
operator|->
name|rt_flags
operator|&
name|w
operator|->
name|w_arg
operator|)
condition|)
continue|continue;
define|#
directive|define
name|next
parameter_list|(
name|a
parameter_list|,
name|l
parameter_list|)
value|{size += (l); w->w_rtm.rtm_addrs |= (a); }
name|w
operator|->
name|w_rtm
operator|.
name|rtm_addrs
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt_key
argument_list|(
name|rt
argument_list|)
condition|)
name|next
argument_list|(
name|RTA_DST
argument_list|,
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt
operator|->
name|rt_gateway
condition|)
name|next
argument_list|(
name|RTA_GATEWAY
argument_list|,
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt_mask
argument_list|(
name|rt
argument_list|)
condition|)
name|next
argument_list|(
name|RTA_NETMASK
argument_list|,
name|sa
operator|->
name|sa_len
condition|?
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
else|:
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt
operator|->
name|rt_genmask
condition|)
name|next
argument_list|(
name|RTA_GENMASK
argument_list|,
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
name|w
operator|->
name|w_needed
operator|+=
name|size
expr_stmt|;
if|if
condition|(
name|w
operator|->
name|w_where
operator|==
name|NULL
operator|||
name|w
operator|->
name|w_needed
operator|>
literal|0
condition|)
continue|continue;
name|w
operator|->
name|w_rtm
operator|.
name|rtm_msglen
operator|=
name|size
expr_stmt|;
name|w
operator|->
name|w_rtm
operator|.
name|rtm_flags
operator|=
name|rt
operator|->
name|rt_flags
expr_stmt|;
name|w
operator|->
name|w_rtm
operator|.
name|rtm_use
operator|=
name|rt
operator|->
name|rt_use
expr_stmt|;
name|w
operator|->
name|w_rtm
operator|.
name|rtm_rmx
operator|=
name|rt
operator|->
name|rt_rmx
expr_stmt|;
name|w
operator|->
name|w_rtm
operator|.
name|rtm_index
operator|=
name|rt
operator|->
name|rt_ifp
operator|->
name|if_index
expr_stmt|;
undef|#
directive|undef
name|next
define|#
directive|define
name|next
parameter_list|(
name|l
parameter_list|)
value|{n = (l); Bcopy(sa, cp, n); cp += n;}
if|if
condition|(
name|size
operator|<=
sizeof|sizeof
argument_list|(
name|w
operator|->
name|w_m
argument_list|)
condition|)
block|{
specifier|register
name|caddr_t
name|cp
init|=
call|(
name|caddr_t
call|)
argument_list|(
name|w
operator|->
name|w_m
operator|.
name|m_sabuf
argument_list|)
decl_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt_key
argument_list|(
name|rt
argument_list|)
condition|)
name|next
argument_list|(
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt
operator|->
name|rt_gateway
condition|)
name|next
argument_list|(
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt_mask
argument_list|(
name|rt
argument_list|)
condition|)
name|next
argument_list|(
name|sa
operator|->
name|sa_len
condition|?
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
else|:
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt
operator|->
name|rt_genmask
condition|)
name|next
argument_list|(
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
undef|#
directive|undef
name|next
define|#
directive|define
name|next
parameter_list|(
name|s
parameter_list|,
name|l
parameter_list|)
value|{n = (l); \     if (error = copyout((caddr_t)(s), w->w_where, n)) return (error); \     w->w_where += n;}
name|next
argument_list|(
operator|&
name|w
operator|->
name|w_m
argument_list|,
name|size
argument_list|)
expr_stmt|;
comment|/* Copy rtmsg and sockaddrs back */
continue|continue;
block|}
name|next
argument_list|(
operator|&
name|w
operator|->
name|w_rtm
argument_list|,
sizeof|sizeof
argument_list|(
name|w
operator|->
name|w_rtm
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt_key
argument_list|(
name|rt
argument_list|)
condition|)
name|next
argument_list|(
name|sa
argument_list|,
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt
operator|->
name|rt_gateway
condition|)
name|next
argument_list|(
name|sa
argument_list|,
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt_mask
argument_list|(
name|rt
argument_list|)
condition|)
name|next
argument_list|(
name|sa
argument_list|,
name|sa
operator|->
name|sa_len
condition|?
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
else|:
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sa
operator|=
name|rt
operator|->
name|rt_genmask
condition|)
name|next
argument_list|(
name|sa
argument_list|,
name|ROUNDUP
argument_list|(
name|sa
operator|->
name|sa_len
argument_list|)
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
undef|#
directive|undef
name|next
block|}
end_block

begin_macro
name|kinfo_rtable
argument_list|(
argument|op
argument_list|,
argument|where
argument_list|,
argument|given
argument_list|,
argument|arg
argument_list|,
argument|needed
argument_list|)
end_macro

begin_decl_stmt
name|int
name|op
decl_stmt|,
name|arg
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|caddr_t
name|where
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
modifier|*
name|given
decl_stmt|,
modifier|*
name|needed
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|radix_node_head
modifier|*
name|rnh
decl_stmt|;
name|int
name|s
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|;
name|u_char
name|af
init|=
name|ki_af
argument_list|(
name|op
argument_list|)
decl_stmt|;
name|struct
name|walkarg
name|w
decl_stmt|;
name|op
operator|&=
literal|0xffff
expr_stmt|;
if|if
condition|(
name|op
operator|!=
name|KINFO_RT_DUMP
operator|&&
name|op
operator|!=
name|KINFO_RT_FLAGS
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|Bzero
argument_list|(
operator|&
name|w
argument_list|,
sizeof|sizeof
argument_list|(
name|w
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|w
operator|.
name|w_where
operator|=
name|where
operator|)
operator|&&
name|given
condition|)
name|w
operator|.
name|w_given
operator|=
operator|*
name|given
expr_stmt|;
name|w
operator|.
name|w_needed
operator|=
literal|0
operator|-
name|w
operator|.
name|w_given
expr_stmt|;
name|w
operator|.
name|w_arg
operator|=
name|arg
expr_stmt|;
name|w
operator|.
name|w_op
operator|=
name|op
expr_stmt|;
name|w
operator|.
name|w_rtm
operator|.
name|rtm_version
operator|=
name|RTM_VERSION
expr_stmt|;
name|w
operator|.
name|w_rtm
operator|.
name|rtm_type
operator|=
name|RTM_GET
expr_stmt|;
name|s
operator|=
name|splnet
argument_list|()
expr_stmt|;
for|for
control|(
name|rnh
operator|=
name|radix_node_head
init|;
name|rnh
condition|;
name|rnh
operator|=
name|rnh
operator|->
name|rnh_next
control|)
block|{
if|if
condition|(
name|rnh
operator|->
name|rnh_af
operator|==
literal|0
condition|)
continue|continue;
if|if
condition|(
name|af
operator|&&
name|af
operator|!=
name|rnh
operator|->
name|rnh_af
condition|)
continue|continue;
name|error
operator|=
name|rt_walk
argument_list|(
name|rnh
operator|->
name|rnh_treetop
argument_list|,
name|rt_dumpentry
argument_list|,
operator|&
name|w
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
break|break;
block|}
name|w
operator|.
name|w_needed
operator|+=
name|w
operator|.
name|w_given
expr_stmt|;
if|if
condition|(
name|where
operator|&&
name|given
condition|)
operator|*
name|given
operator|=
name|w
operator|.
name|w_where
operator|-
name|where
expr_stmt|;
else|else
name|w
operator|.
name|w_needed
operator|=
operator|(
literal|11
operator|*
name|w
operator|.
name|w_needed
operator|)
operator|/
literal|10
expr_stmt|;
operator|*
name|needed
operator|=
name|w
operator|.
name|w_needed
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_expr_stmt
name|rt_walk
argument_list|(
name|rn
argument_list|,
name|f
argument_list|,
name|w
argument_list|)
specifier|register
expr|struct
name|radix_node
operator|*
name|rn
expr_stmt|;
end_expr_stmt

begin_function_decl
specifier|register
name|int
function_decl|(
modifier|*
name|f
function_decl|)
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|struct
name|walkarg
modifier|*
name|w
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|error
decl_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
while|while
condition|(
name|rn
operator|->
name|rn_b
operator|>=
literal|0
condition|)
name|rn
operator|=
name|rn
operator|->
name|rn_l
expr_stmt|;
comment|/* First time through node, go left */
if|if
condition|(
name|error
operator|=
call|(
modifier|*
name|f
call|)
argument_list|(
name|rn
argument_list|,
name|w
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
comment|/* Process Leaf */
while|while
condition|(
name|rn
operator|->
name|rn_p
operator|->
name|rn_r
operator|==
name|rn
condition|)
block|{
comment|/* if coming back from right */
name|rn
operator|=
name|rn
operator|->
name|rn_p
expr_stmt|;
comment|/* go back up */
if|if
condition|(
name|rn
operator|->
name|rn_flags
operator|&
name|RNF_ROOT
condition|)
return|return
literal|0
return|;
block|}
name|rn
operator|=
name|rn
operator|->
name|rn_p
operator|->
name|rn_r
expr_stmt|;
comment|/* otherwise, go right*/
block|}
block|}
end_block

begin_comment
comment|/*  * Definitions of protocols supported in the ROUTE domain.  */
end_comment

begin_decl_stmt
name|int
name|raw_init
argument_list|()
decl_stmt|,
name|raw_usrreq
argument_list|()
decl_stmt|,
name|raw_input
argument_list|()
decl_stmt|,
name|raw_ctlinput
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|struct
name|domain
name|routedomain
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* or at least forward */
end_comment

begin_decl_stmt
name|struct
name|protosw
name|routesw
index|[]
init|=
block|{
block|{
name|SOCK_RAW
block|,
operator|&
name|routedomain
block|,
literal|0
block|,
name|PR_ATOMIC
operator||
name|PR_ADDR
block|,
name|raw_input
block|,
name|route_output
block|,
name|raw_ctlinput
block|,
literal|0
block|,
name|route_usrreq
block|,
name|raw_init
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|, }
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|unp_externalize
argument_list|()
decl_stmt|,
name|unp_dispose
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|domain
name|routedomain
init|=
block|{
name|PF_ROUTE
block|,
literal|"route"
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|,
name|routesw
block|,
operator|&
name|routesw
index|[
sizeof|sizeof
argument_list|(
name|routesw
argument_list|)
operator|/
sizeof|sizeof
argument_list|(
name|routesw
index|[
literal|0
index|]
argument_list|)
index|]
block|}
decl_stmt|;
end_decl_stmt

end_unit

