begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1988, 1989  Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)radix.c	7.13 (Berkeley) %G%  */
end_comment

begin_comment
comment|/*  * Routines to build and maintain radix trees for routing lookups.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|RNF_NORMAL
end_ifndef

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"radix.h"
end_include

begin_include
include|#
directive|include
file|"malloc.h"
end_include

begin_define
define|#
directive|define
name|M_DONTWAIT
value|M_NOWAIT
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|struct
name|radix_mask
modifier|*
name|rn_mkfreelist
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|radix_node_head
modifier|*
name|mask_rnhead
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|rn_maskhead
value|(mask_rnhead->rnh_treetop)
end_define

begin_undef
undef|#
directive|undef
name|Bcmp
end_undef

begin_define
define|#
directive|define
name|Bcmp
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|,
name|l
parameter_list|)
value|(l == 0 ? 0 : bcmp((caddr_t)(a), (caddr_t)(b), (u_long)l))
end_define

begin_comment
comment|/*  * The data structure for the keys is a radix tree with one way  * branching removed.  The index rn_b at an internal node n represents a bit  * position to be tested.  The tree is arranged so that all descendants  * of a node n have keys whose bits all agree up to position rn_b - 1.  * (We say the index of n is rn_b.)  *  * There is at least one descendant which has a one bit at position rn_b,  * and at least one with a zero there.  *  * A route is determined by a pair of key and mask.  We require that the  * bit-wise logical and of the key and mask to be the key.  * We define the index of a route to associated with the mask to be  * the first bit number in the mask where 0 occurs (with bit number 0  * representing the highest order bit).  *   * We say a mask is normal if every bit is 0, past the index of the mask.  * If a node n has a descendant (k, m) with index(m) == index(n) == rn_b,  * and m is a normal mask, then the route applies to every descendant of n.  * If the index(m)< rn_b, this implies the trailing last few bits of k  * before bit b are all 0, (and hence consequently true of every descendant  * of n), so the route applies to all descendants of the node as well.  *  * The present version of the code makes no use of normal routes,  * but similar logic shows that a non-normal mask m such that  * index(m)<= index(n) could potentially apply to many children of n.  * Thus, for each non-host route, we attach its mask to a list at an internal  * node as high in the tree as we can go.   */
end_comment

begin_function
name|struct
name|radix_node
modifier|*
name|rn_search
parameter_list|(
name|v
parameter_list|,
name|head
parameter_list|)
name|struct
name|radix_node
modifier|*
name|head
decl_stmt|;
specifier|register
name|caddr_t
name|v
decl_stmt|;
block|{
specifier|register
name|struct
name|radix_node
modifier|*
name|x
decl_stmt|;
for|for
control|(
name|x
operator|=
name|head
init|;
name|x
operator|->
name|rn_b
operator|>=
literal|0
condition|;
control|)
block|{
if|if
condition|(
name|x
operator|->
name|rn_bmask
operator|&
name|v
index|[
name|x
operator|->
name|rn_off
index|]
condition|)
name|x
operator|=
name|x
operator|->
name|rn_r
expr_stmt|;
else|else
name|x
operator|=
name|x
operator|->
name|rn_l
expr_stmt|;
block|}
return|return
name|x
return|;
block|}
end_function

begin_empty_stmt
empty_stmt|;
end_empty_stmt

begin_function
name|struct
name|radix_node
modifier|*
name|rn_search_m
parameter_list|(
name|v
parameter_list|,
name|head
parameter_list|,
name|m
parameter_list|)
name|struct
name|radix_node
modifier|*
name|head
decl_stmt|;
specifier|register
name|caddr_t
name|v
decl_stmt|,
name|m
decl_stmt|;
block|{
specifier|register
name|struct
name|radix_node
modifier|*
name|x
decl_stmt|;
for|for
control|(
name|x
operator|=
name|head
init|;
name|x
operator|->
name|rn_b
operator|>=
literal|0
condition|;
control|)
block|{
if|if
condition|(
operator|(
name|x
operator|->
name|rn_bmask
operator|&
name|m
index|[
name|x
operator|->
name|rn_off
index|]
operator|)
operator|&&
operator|(
name|x
operator|->
name|rn_bmask
operator|&
name|v
index|[
name|x
operator|->
name|rn_off
index|]
operator|)
condition|)
name|x
operator|=
name|x
operator|->
name|rn_r
expr_stmt|;
else|else
name|x
operator|=
name|x
operator|->
name|rn_l
expr_stmt|;
block|}
return|return
name|x
return|;
block|}
end_function

begin_empty_stmt
empty_stmt|;
end_empty_stmt

begin_expr_stmt
name|rn_refines
argument_list|(
name|m
argument_list|,
name|n
argument_list|)
specifier|register
name|caddr_t
name|m
operator|,
name|n
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|caddr_t
name|lim
decl_stmt|,
name|lim2
init|=
name|lim
operator|=
name|n
operator|+
operator|*
operator|(
name|u_char
operator|*
operator|)
name|n
decl_stmt|;
name|int
name|longer
init|=
operator|(
operator|*
operator|(
name|u_char
operator|*
operator|)
name|n
operator|++
operator|)
operator|-
call|(
name|int
call|)
argument_list|(
operator|*
operator|(
name|u_char
operator|*
operator|)
name|m
operator|++
argument_list|)
decl_stmt|;
if|if
condition|(
name|longer
operator|>
literal|0
condition|)
name|lim
operator|-=
name|longer
expr_stmt|;
while|while
condition|(
name|n
operator|<
name|lim
condition|)
if|if
condition|(
operator|*
name|n
operator|++
operator|&
operator|~
operator|(
operator|*
name|m
operator|++
operator|)
condition|)
return|return
literal|0
return|;
while|while
condition|(
name|n
operator|<
name|lim2
condition|)
if|if
condition|(
operator|*
name|n
operator|++
condition|)
return|return
literal|0
return|;
return|return
literal|1
return|;
block|}
end_block

begin_define
define|#
directive|define
name|MAXKEYLEN
value|52
end_define

begin_decl_stmt
specifier|static
name|int
name|gotOddMasks
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
name|maskedKey
index|[
name|MAXKEYLEN
index|]
decl_stmt|;
end_decl_stmt

begin_function
name|struct
name|radix_node
modifier|*
name|rn_match
parameter_list|(
name|v
parameter_list|,
name|head
parameter_list|)
name|struct
name|radix_node
modifier|*
name|head
decl_stmt|;
name|caddr_t
name|v
decl_stmt|;
block|{
specifier|register
name|struct
name|radix_node
modifier|*
name|t
init|=
name|head
decl_stmt|,
modifier|*
name|x
decl_stmt|;
specifier|register
name|caddr_t
name|cp
init|=
name|v
decl_stmt|,
name|cp2
decl_stmt|,
name|cp3
decl_stmt|;
name|caddr_t
name|cplim
decl_stmt|,
name|mstart
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|saved_t
decl_stmt|;
name|int
name|off
init|=
name|t
operator|->
name|rn_off
decl_stmt|,
name|vlen
init|=
operator|*
operator|(
name|u_char
operator|*
operator|)
name|cp
decl_stmt|,
name|matched_off
decl_stmt|;
comment|/* 	 * Open code rn_search(v, head) to avoid overhead of extra 	 * subroutine call. 	 */
for|for
control|(
init|;
name|t
operator|->
name|rn_b
operator|>=
literal|0
condition|;
control|)
block|{
if|if
condition|(
name|t
operator|->
name|rn_bmask
operator|&
name|cp
index|[
name|t
operator|->
name|rn_off
index|]
condition|)
name|t
operator|=
name|t
operator|->
name|rn_r
expr_stmt|;
else|else
name|t
operator|=
name|t
operator|->
name|rn_l
expr_stmt|;
block|}
comment|/* 	 * See if we match exactly as a host destination 	 */
name|cp
operator|+=
name|off
expr_stmt|;
name|cp2
operator|=
name|t
operator|->
name|rn_key
operator|+
name|off
expr_stmt|;
name|cplim
operator|=
name|v
operator|+
name|vlen
expr_stmt|;
for|for
control|(
init|;
name|cp
operator|<
name|cplim
condition|;
name|cp
operator|++
operator|,
name|cp2
operator|++
control|)
if|if
condition|(
operator|*
name|cp
operator|!=
operator|*
name|cp2
condition|)
goto|goto
name|on1
goto|;
comment|/* 	 * This extra grot is in case we are explicitly asked 	 * to look up the default.  Ugh! 	 */
if|if
condition|(
operator|(
name|t
operator|->
name|rn_flags
operator|&
name|RNF_ROOT
operator|)
operator|&&
name|t
operator|->
name|rn_dupedkey
condition|)
name|t
operator|=
name|t
operator|->
name|rn_dupedkey
expr_stmt|;
return|return
name|t
return|;
name|on1
label|:
name|matched_off
operator|=
name|cp
operator|-
name|v
expr_stmt|;
name|saved_t
operator|=
name|t
expr_stmt|;
do|do
block|{
if|if
condition|(
name|t
operator|->
name|rn_mask
condition|)
block|{
comment|/* 		 * Even if we don't match exactly as a hosts; 		 * we may match if the leaf we wound up at is 		 * a route to a net. 		 */
name|cp3
operator|=
name|matched_off
operator|+
name|t
operator|->
name|rn_mask
expr_stmt|;
name|cp2
operator|=
name|matched_off
operator|+
name|t
operator|->
name|rn_key
expr_stmt|;
for|for
control|(
init|;
name|cp
operator|<
name|cplim
condition|;
name|cp
operator|++
control|)
if|if
condition|(
operator|(
operator|*
name|cp2
operator|++
operator|^
operator|*
name|cp
operator|)
operator|&
operator|*
name|cp3
operator|++
condition|)
break|break;
if|if
condition|(
name|cp
operator|==
name|cplim
condition|)
return|return
name|t
return|;
name|cp
operator|=
name|matched_off
operator|+
name|v
expr_stmt|;
block|}
block|}
do|while
condition|(
name|t
operator|=
name|t
operator|->
name|rn_dupedkey
condition|)
do|;
name|t
operator|=
name|saved_t
expr_stmt|;
comment|/* start searching up the tree */
do|do
block|{
specifier|register
name|struct
name|radix_mask
modifier|*
name|m
decl_stmt|;
name|t
operator|=
name|t
operator|->
name|rn_p
expr_stmt|;
if|if
condition|(
name|m
operator|=
name|t
operator|->
name|rn_mklist
condition|)
block|{
comment|/* 			 * After doing measurements here, it may 			 * turn out to be faster to open code 			 * rn_search_m here instead of always 			 * copying and masking. 			 */
name|off
operator|=
name|min
argument_list|(
name|t
operator|->
name|rn_off
argument_list|,
name|matched_off
argument_list|)
expr_stmt|;
name|mstart
operator|=
name|maskedKey
operator|+
name|off
expr_stmt|;
do|do
block|{
name|cp2
operator|=
name|mstart
expr_stmt|;
name|cp3
operator|=
name|m
operator|->
name|rm_mask
operator|+
name|off
expr_stmt|;
for|for
control|(
name|cp
operator|=
name|v
operator|+
name|off
init|;
name|cp
operator|<
name|cplim
condition|;
control|)
operator|*
name|cp2
operator|++
operator|=
operator|*
name|cp
operator|++
operator|&
operator|*
name|cp3
operator|++
expr_stmt|;
name|x
operator|=
name|rn_search
argument_list|(
name|maskedKey
argument_list|,
name|t
argument_list|)
expr_stmt|;
while|while
condition|(
name|x
operator|&&
name|x
operator|->
name|rn_mask
operator|!=
name|m
operator|->
name|rm_mask
condition|)
name|x
operator|=
name|x
operator|->
name|rn_dupedkey
expr_stmt|;
if|if
condition|(
name|x
operator|&&
operator|(
name|Bcmp
argument_list|(
name|mstart
argument_list|,
name|x
operator|->
name|rn_key
operator|+
name|off
argument_list|,
name|vlen
operator|-
name|off
argument_list|)
operator|==
literal|0
operator|)
condition|)
return|return
name|x
return|;
block|}
do|while
condition|(
name|m
operator|=
name|m
operator|->
name|rm_mklist
condition|)
do|;
block|}
block|}
do|while
condition|(
name|t
operator|!=
name|head
condition|)
do|;
return|return
literal|0
return|;
block|}
end_function

begin_empty_stmt
empty_stmt|;
end_empty_stmt

begin_ifdef
ifdef|#
directive|ifdef
name|RN_DEBUG
end_ifdef

begin_decl_stmt
name|int
name|rn_nodenum
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|radix_node
modifier|*
name|rn_clist
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|rn_saveinfo
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_function
name|struct
name|radix_node
modifier|*
name|rn_newpair
parameter_list|(
name|v
parameter_list|,
name|b
parameter_list|,
name|nodes
parameter_list|)
name|caddr_t
name|v
decl_stmt|;
name|int
name|b
decl_stmt|;
name|struct
name|radix_node
name|nodes
index|[
literal|2
index|]
decl_stmt|;
block|{
specifier|register
name|struct
name|radix_node
modifier|*
name|tt
init|=
name|nodes
decl_stmt|,
modifier|*
name|t
init|=
name|tt
operator|+
literal|1
decl_stmt|;
name|t
operator|->
name|rn_b
operator|=
name|b
expr_stmt|;
name|t
operator|->
name|rn_bmask
operator|=
literal|0x80
operator|>>
operator|(
name|b
operator|&
literal|7
operator|)
expr_stmt|;
name|t
operator|->
name|rn_l
operator|=
name|tt
expr_stmt|;
name|t
operator|->
name|rn_off
operator|=
name|b
operator|>>
literal|3
expr_stmt|;
name|tt
operator|->
name|rn_b
operator|=
operator|-
literal|1
expr_stmt|;
name|tt
operator|->
name|rn_key
operator|=
name|v
expr_stmt|;
name|tt
operator|->
name|rn_p
operator|=
name|t
expr_stmt|;
name|tt
operator|->
name|rn_flags
operator|=
name|t
operator|->
name|rn_flags
operator|=
name|RNF_ACTIVE
expr_stmt|;
ifdef|#
directive|ifdef
name|RN_DEBUG
name|tt
operator|->
name|rn_info
operator|=
name|rn_nodenum
operator|++
expr_stmt|;
name|t
operator|->
name|rn_info
operator|=
name|rn_nodenum
operator|++
expr_stmt|;
name|tt
operator|->
name|rn_twin
operator|=
name|t
expr_stmt|;
name|tt
operator|->
name|rn_ybro
operator|=
name|rn_clist
expr_stmt|;
name|rn_clist
operator|=
name|tt
expr_stmt|;
endif|#
directive|endif
return|return
name|t
return|;
block|}
end_function

begin_decl_stmt
name|int
name|rn_debug
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_function
name|struct
name|radix_node
modifier|*
name|rn_insert
parameter_list|(
name|v
parameter_list|,
name|head
parameter_list|,
name|dupentry
parameter_list|,
name|nodes
parameter_list|)
name|caddr_t
name|v
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|head
decl_stmt|;
name|int
modifier|*
name|dupentry
decl_stmt|;
name|struct
name|radix_node
name|nodes
index|[
literal|2
index|]
decl_stmt|;
block|{
name|int
name|head_off
init|=
name|head
operator|->
name|rn_off
decl_stmt|,
name|vlen
init|=
operator|(
name|int
operator|)
operator|*
operator|(
operator|(
name|u_char
operator|*
operator|)
name|v
operator|)
decl_stmt|;
specifier|register
name|struct
name|radix_node
modifier|*
name|t
init|=
name|rn_search
argument_list|(
name|v
argument_list|,
name|head
argument_list|)
decl_stmt|;
specifier|register
name|caddr_t
name|cp
init|=
name|v
operator|+
name|head_off
decl_stmt|;
specifier|register
name|int
name|b
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|tt
decl_stmt|;
comment|/* 	 *find first bit at which v and t->rn_key differ 	 */
block|{
specifier|register
name|caddr_t
name|cp2
init|=
name|t
operator|->
name|rn_key
operator|+
name|head_off
decl_stmt|;
specifier|register
name|int
name|cmp_res
decl_stmt|;
name|caddr_t
name|cplim
init|=
name|v
operator|+
name|vlen
decl_stmt|;
while|while
condition|(
name|cp
operator|<
name|cplim
condition|)
if|if
condition|(
operator|*
name|cp2
operator|++
operator|!=
operator|*
name|cp
operator|++
condition|)
goto|goto
name|on1
goto|;
operator|*
name|dupentry
operator|=
literal|1
expr_stmt|;
return|return
name|t
return|;
name|on1
label|:
operator|*
name|dupentry
operator|=
literal|0
expr_stmt|;
name|cmp_res
operator|=
operator|(
name|cp
index|[
operator|-
literal|1
index|]
operator|^
name|cp2
index|[
operator|-
literal|1
index|]
operator|)
operator|&
literal|0xff
expr_stmt|;
for|for
control|(
name|b
operator|=
operator|(
name|cp
operator|-
name|v
operator|)
operator|<<
literal|3
init|;
name|cmp_res
condition|;
name|b
operator|--
control|)
name|cmp_res
operator|>>=
literal|1
expr_stmt|;
block|}
block|{
specifier|register
name|struct
name|radix_node
modifier|*
name|p
decl_stmt|,
modifier|*
name|x
init|=
name|head
decl_stmt|;
name|cp
operator|=
name|v
expr_stmt|;
do|do
block|{
name|p
operator|=
name|x
expr_stmt|;
if|if
condition|(
name|cp
index|[
name|x
operator|->
name|rn_off
index|]
operator|&
name|x
operator|->
name|rn_bmask
condition|)
name|x
operator|=
name|x
operator|->
name|rn_r
expr_stmt|;
else|else
name|x
operator|=
name|x
operator|->
name|rn_l
expr_stmt|;
block|}
do|while
condition|(
name|b
operator|>
operator|(
name|unsigned
operator|)
name|x
operator|->
name|rn_b
condition|)
do|;
comment|/* x->rn_b< b&& x->rn_b>= 0 */
ifdef|#
directive|ifdef
name|RN_DEBUG
if|if
condition|(
name|rn_debug
condition|)
name|printf
argument_list|(
literal|"Going In:\n"
argument_list|)
operator|,
name|traverse
argument_list|(
name|p
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|t
operator|=
name|rn_newpair
argument_list|(
name|v
argument_list|,
name|b
argument_list|,
name|nodes
argument_list|)
expr_stmt|;
name|tt
operator|=
name|t
operator|->
name|rn_l
expr_stmt|;
if|if
condition|(
operator|(
name|cp
index|[
name|p
operator|->
name|rn_off
index|]
operator|&
name|p
operator|->
name|rn_bmask
operator|)
operator|==
literal|0
condition|)
name|p
operator|->
name|rn_l
operator|=
name|t
expr_stmt|;
else|else
name|p
operator|->
name|rn_r
operator|=
name|t
expr_stmt|;
name|x
operator|->
name|rn_p
operator|=
name|t
expr_stmt|;
name|t
operator|->
name|rn_p
operator|=
name|p
expr_stmt|;
comment|/* frees x, p as temp vars below */
if|if
condition|(
operator|(
name|cp
index|[
name|t
operator|->
name|rn_off
index|]
operator|&
name|t
operator|->
name|rn_bmask
operator|)
operator|==
literal|0
condition|)
block|{
name|t
operator|->
name|rn_r
operator|=
name|x
expr_stmt|;
block|}
else|else
block|{
name|t
operator|->
name|rn_r
operator|=
name|tt
expr_stmt|;
name|t
operator|->
name|rn_l
operator|=
name|x
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|RN_DEBUG
if|if
condition|(
name|rn_debug
condition|)
name|printf
argument_list|(
literal|"Coming out:\n"
argument_list|)
operator|,
name|traverse
argument_list|(
name|p
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
return|return
operator|(
name|tt
operator|)
return|;
block|}
end_function

begin_function
name|struct
name|radix_node
modifier|*
name|rn_addmask
parameter_list|(
name|netmask
parameter_list|,
name|search
parameter_list|,
name|skip
parameter_list|)
name|caddr_t
name|netmask
decl_stmt|;
name|int
name|search
decl_stmt|,
name|skip
decl_stmt|;
block|{
specifier|register
name|struct
name|radix_node
modifier|*
name|x
decl_stmt|;
specifier|register
name|caddr_t
name|cp
decl_stmt|,
name|cplim
decl_stmt|;
specifier|register
name|int
name|b
decl_stmt|,
name|mlen
decl_stmt|,
name|j
decl_stmt|;
name|int
name|maskduplicated
decl_stmt|;
name|mlen
operator|=
operator|*
operator|(
name|u_char
operator|*
operator|)
name|netmask
expr_stmt|;
if|if
condition|(
name|search
condition|)
block|{
name|x
operator|=
name|rn_search
argument_list|(
name|netmask
argument_list|,
name|rn_maskhead
argument_list|)
expr_stmt|;
name|mlen
operator|=
operator|*
operator|(
name|u_char
operator|*
operator|)
name|netmask
expr_stmt|;
if|if
condition|(
name|Bcmp
argument_list|(
name|netmask
argument_list|,
name|x
operator|->
name|rn_key
argument_list|,
name|mlen
argument_list|)
operator|==
literal|0
condition|)
return|return
operator|(
name|x
operator|)
return|;
block|}
name|R_Malloc
argument_list|(
name|x
argument_list|,
expr|struct
name|radix_node
operator|*
argument_list|,
name|MAXKEYLEN
operator|+
literal|2
operator|*
sizeof|sizeof
argument_list|(
operator|*
name|x
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|x
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|Bzero
argument_list|(
name|x
argument_list|,
name|MAXKEYLEN
operator|+
literal|2
operator|*
sizeof|sizeof
argument_list|(
operator|*
name|x
argument_list|)
argument_list|)
expr_stmt|;
name|cp
operator|=
call|(
name|caddr_t
call|)
argument_list|(
name|x
operator|+
literal|2
argument_list|)
expr_stmt|;
name|Bcopy
argument_list|(
name|netmask
argument_list|,
name|cp
argument_list|,
name|mlen
argument_list|)
expr_stmt|;
name|netmask
operator|=
name|cp
expr_stmt|;
name|x
operator|=
name|rn_insert
argument_list|(
name|netmask
argument_list|,
name|rn_maskhead
argument_list|,
operator|&
name|maskduplicated
argument_list|,
name|x
argument_list|)
expr_stmt|;
comment|/* 	 * Calculate index of mask. 	 */
name|cplim
operator|=
name|netmask
operator|+
name|mlen
expr_stmt|;
for|for
control|(
name|cp
operator|=
name|netmask
operator|+
name|skip
init|;
name|cp
operator|<
name|cplim
condition|;
name|cp
operator|++
control|)
if|if
condition|(
operator|*
operator|(
name|u_char
operator|*
operator|)
name|cp
operator|!=
literal|0xff
condition|)
break|break;
name|b
operator|=
operator|(
name|cp
operator|-
name|netmask
operator|)
operator|<<
literal|3
expr_stmt|;
if|if
condition|(
name|cp
operator|!=
name|cplim
condition|)
block|{
if|if
condition|(
operator|*
name|cp
operator|!=
literal|0
condition|)
block|{
name|gotOddMasks
operator|=
literal|1
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0x80
init|;
name|j
condition|;
name|b
operator|++
operator|,
name|j
operator|>>=
literal|1
control|)
if|if
condition|(
operator|(
name|j
operator|&
operator|*
name|cp
operator|)
operator|==
literal|0
condition|)
break|break;
block|}
block|}
name|x
operator|->
name|rn_b
operator|=
operator|-
literal|1
operator|-
name|b
expr_stmt|;
return|return
operator|(
name|x
operator|)
return|;
block|}
end_function

begin_function
name|struct
name|radix_node
modifier|*
name|rn_addroute
parameter_list|(
name|v
parameter_list|,
name|netmask
parameter_list|,
name|head
parameter_list|,
name|treenodes
parameter_list|)
name|caddr_t
name|v
decl_stmt|,
name|netmask
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|head
decl_stmt|;
name|struct
name|radix_node
name|treenodes
index|[
literal|2
index|]
decl_stmt|;
block|{
specifier|register
name|int
name|j
decl_stmt|;
specifier|register
name|caddr_t
name|cp
decl_stmt|;
specifier|register
name|struct
name|radix_node
modifier|*
name|t
decl_stmt|,
modifier|*
name|x
decl_stmt|,
modifier|*
name|tt
decl_stmt|;
name|short
name|b
init|=
literal|0
decl_stmt|,
name|b_leaf
decl_stmt|;
name|int
name|vlen
init|=
operator|*
operator|(
name|u_char
operator|*
operator|)
name|v
decl_stmt|,
name|mlen
decl_stmt|,
name|keyduplicated
decl_stmt|;
name|caddr_t
name|cplim
decl_stmt|;
name|unsigned
name|char
modifier|*
name|maskp
decl_stmt|;
name|struct
name|radix_mask
modifier|*
name|m
decl_stmt|,
modifier|*
modifier|*
name|mp
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|saved_tt
decl_stmt|;
comment|/* 	 * In dealing with non-contiguous masks, there may be 	 * many different routes which have the same mask. 	 * We will find it useful to have a unique pointer to 	 * the mask to speed avoiding duplicate references at 	 * nodes and possibly save time in calculating indices. 	 */
if|if
condition|(
name|netmask
condition|)
block|{
name|x
operator|=
name|rn_search
argument_list|(
name|netmask
argument_list|,
name|rn_maskhead
argument_list|)
expr_stmt|;
name|mlen
operator|=
operator|*
operator|(
name|u_char
operator|*
operator|)
name|netmask
expr_stmt|;
if|if
condition|(
name|Bcmp
argument_list|(
name|netmask
argument_list|,
name|x
operator|->
name|rn_key
argument_list|,
name|mlen
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|x
operator|=
name|rn_addmask
argument_list|(
name|netmask
argument_list|,
literal|0
argument_list|,
name|head
operator|->
name|rn_off
argument_list|)
expr_stmt|;
if|if
condition|(
name|x
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|netmask
operator|=
name|x
operator|->
name|rn_key
expr_stmt|;
name|b
operator|=
operator|-
literal|1
operator|-
name|x
operator|->
name|rn_b
expr_stmt|;
block|}
comment|/* 	 * Deal with duplicated keys: attach node to previous instance 	 */
name|saved_tt
operator|=
name|tt
operator|=
name|rn_insert
argument_list|(
name|v
argument_list|,
name|head
argument_list|,
operator|&
name|keyduplicated
argument_list|,
name|treenodes
argument_list|)
expr_stmt|;
if|if
condition|(
name|keyduplicated
condition|)
block|{
do|do
block|{
if|if
condition|(
name|tt
operator|->
name|rn_mask
operator|==
name|netmask
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|t
operator|=
name|tt
expr_stmt|;
if|if
condition|(
name|netmask
operator|==
literal|0
operator|||
operator|(
name|tt
operator|->
name|rn_mask
operator|&&
name|rn_refines
argument_list|(
name|netmask
argument_list|,
name|tt
operator|->
name|rn_mask
argument_list|)
operator|)
condition|)
break|break;
block|}
do|while
condition|(
name|tt
operator|=
name|tt
operator|->
name|rn_dupedkey
condition|)
do|;
comment|/* 		 * If the mask is not duplicated, we wouldn't 		 * find it among possible duplicate key entries 		 * anyway, so the above test doesn't hurt. 		 * 		 * We sort the masks 		 * for a duplicated key the same way as in a masklist. 		 * It is an unfortunate pain having to relocate 		 * the head of the list. 		 */
if|if
condition|(
name|tt
operator|&&
name|t
operator|==
name|saved_tt
condition|)
block|{
name|struct
name|radix_node
modifier|*
name|xx
init|=
name|x
decl_stmt|;
comment|/* link in at head of list */
operator|(
name|tt
operator|=
name|treenodes
operator|)
operator|->
name|rn_dupedkey
operator|=
name|t
expr_stmt|;
name|tt
operator|->
name|rn_flags
operator|=
name|t
operator|->
name|rn_flags
expr_stmt|;
name|tt
operator|->
name|rn_p
operator|=
name|x
operator|=
name|t
operator|->
name|rn_p
expr_stmt|;
if|if
condition|(
name|x
operator|->
name|rn_l
operator|==
name|t
condition|)
name|x
operator|->
name|rn_l
operator|=
name|tt
expr_stmt|;
else|else
name|x
operator|->
name|rn_r
operator|=
name|tt
expr_stmt|;
name|saved_tt
operator|=
name|tt
expr_stmt|;
name|x
operator|=
name|xx
expr_stmt|;
block|}
else|else
block|{
operator|(
name|tt
operator|=
name|treenodes
operator|)
operator|->
name|rn_dupedkey
operator|=
name|t
operator|->
name|rn_dupedkey
expr_stmt|;
name|t
operator|->
name|rn_dupedkey
operator|=
name|tt
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|RN_DEBUG
name|t
operator|=
name|tt
operator|+
literal|1
expr_stmt|;
name|tt
operator|->
name|rn_info
operator|=
name|rn_nodenum
operator|++
expr_stmt|;
name|t
operator|->
name|rn_info
operator|=
name|rn_nodenum
operator|++
expr_stmt|;
name|tt
operator|->
name|rn_twin
operator|=
name|t
expr_stmt|;
name|tt
operator|->
name|rn_ybro
operator|=
name|rn_clist
expr_stmt|;
name|rn_clist
operator|=
name|tt
expr_stmt|;
endif|#
directive|endif
name|t
operator|=
name|saved_tt
expr_stmt|;
name|tt
operator|->
name|rn_key
operator|=
operator|(
name|caddr_t
operator|)
name|v
expr_stmt|;
name|tt
operator|->
name|rn_b
operator|=
operator|-
literal|1
expr_stmt|;
name|tt
operator|->
name|rn_flags
operator|=
name|t
operator|->
name|rn_flags
operator|&
operator|~
name|RNF_ROOT
expr_stmt|;
block|}
comment|/* 	 * Put mask in tree. 	 */
if|if
condition|(
name|netmask
condition|)
block|{
name|tt
operator|->
name|rn_mask
operator|=
name|netmask
expr_stmt|;
name|tt
operator|->
name|rn_b
operator|=
name|x
operator|->
name|rn_b
expr_stmt|;
block|}
name|t
operator|=
name|saved_tt
operator|->
name|rn_p
expr_stmt|;
name|b_leaf
operator|=
operator|-
literal|1
operator|-
name|t
operator|->
name|rn_b
expr_stmt|;
if|if
condition|(
name|t
operator|->
name|rn_r
operator|==
name|saved_tt
condition|)
name|x
operator|=
name|t
operator|->
name|rn_l
expr_stmt|;
else|else
name|x
operator|=
name|t
operator|->
name|rn_r
expr_stmt|;
comment|/* Promote general routes from below */
if|if
condition|(
name|x
operator|->
name|rn_b
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|x
operator|->
name|rn_mask
operator|&&
operator|(
name|x
operator|->
name|rn_b
operator|>=
name|b_leaf
operator|)
operator|&&
name|x
operator|->
name|rn_mklist
operator|==
literal|0
condition|)
block|{
name|MKGet
argument_list|(
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
condition|)
block|{
name|Bzero
argument_list|(
name|m
argument_list|,
sizeof|sizeof
expr|*
name|m
argument_list|)
expr_stmt|;
name|m
operator|->
name|rm_b
operator|=
name|x
operator|->
name|rn_b
expr_stmt|;
name|m
operator|->
name|rm_mask
operator|=
name|x
operator|->
name|rn_mask
expr_stmt|;
name|x
operator|->
name|rn_mklist
operator|=
name|t
operator|->
name|rn_mklist
operator|=
name|m
expr_stmt|;
block|}
block|}
block|}
elseif|else
if|if
condition|(
name|x
operator|->
name|rn_mklist
condition|)
block|{
comment|/* 		 * Skip over masks whose index is> that of new node 		 */
for|for
control|(
name|mp
operator|=
operator|&
name|x
operator|->
name|rn_mklist
init|;
name|m
operator|=
operator|*
name|mp
condition|;
name|mp
operator|=
operator|&
name|m
operator|->
name|rm_mklist
control|)
if|if
condition|(
name|m
operator|->
name|rm_b
operator|>=
name|b_leaf
condition|)
break|break;
name|t
operator|->
name|rn_mklist
operator|=
name|m
expr_stmt|;
operator|*
name|mp
operator|=
literal|0
expr_stmt|;
block|}
comment|/* Add new route to highest possible ancestor's list */
if|if
condition|(
operator|(
name|netmask
operator|==
literal|0
operator|)
operator|||
operator|(
name|b
operator|>
name|t
operator|->
name|rn_b
operator|)
condition|)
return|return
name|tt
return|;
comment|/* can't lift at all */
name|b_leaf
operator|=
name|tt
operator|->
name|rn_b
expr_stmt|;
do|do
block|{
name|x
operator|=
name|t
expr_stmt|;
name|t
operator|=
name|t
operator|->
name|rn_p
expr_stmt|;
block|}
do|while
condition|(
name|b
operator|<=
name|t
operator|->
name|rn_b
operator|&&
name|x
operator|!=
name|head
condition|)
do|;
comment|/* 	 * Search through routes associated with node to 	 * insert new route according to index. 	 * For nodes of equal index, place more specific 	 * masks first. 	 */
name|cplim
operator|=
name|netmask
operator|+
name|mlen
expr_stmt|;
for|for
control|(
name|mp
operator|=
operator|&
name|x
operator|->
name|rn_mklist
init|;
name|m
operator|=
operator|*
name|mp
condition|;
name|mp
operator|=
operator|&
name|m
operator|->
name|rm_mklist
control|)
block|{
if|if
condition|(
name|m
operator|->
name|rm_b
operator|<
name|b_leaf
condition|)
continue|continue;
if|if
condition|(
name|m
operator|->
name|rm_b
operator|>
name|b_leaf
condition|)
break|break;
if|if
condition|(
name|m
operator|->
name|rm_mask
operator|==
name|netmask
condition|)
block|{
name|m
operator|->
name|rm_refs
operator|++
expr_stmt|;
name|tt
operator|->
name|rn_mklist
operator|=
name|m
expr_stmt|;
return|return
name|tt
return|;
block|}
if|if
condition|(
name|rn_refines
argument_list|(
name|netmask
argument_list|,
name|m
operator|->
name|rm_mask
argument_list|)
condition|)
break|break;
block|}
name|MKGet
argument_list|(
name|m
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"Mask for route not entered\n"
argument_list|)
expr_stmt|;
return|return
operator|(
name|tt
operator|)
return|;
block|}
name|Bzero
argument_list|(
name|m
argument_list|,
sizeof|sizeof
expr|*
name|m
argument_list|)
expr_stmt|;
name|m
operator|->
name|rm_b
operator|=
name|b_leaf
expr_stmt|;
name|m
operator|->
name|rm_mask
operator|=
name|netmask
expr_stmt|;
name|m
operator|->
name|rm_mklist
operator|=
operator|*
name|mp
expr_stmt|;
operator|*
name|mp
operator|=
name|m
expr_stmt|;
name|tt
operator|->
name|rn_mklist
operator|=
name|m
expr_stmt|;
return|return
name|tt
return|;
block|}
end_function

begin_function
name|struct
name|radix_node
modifier|*
name|rn_delete
parameter_list|(
name|v
parameter_list|,
name|netmask
parameter_list|,
name|head
parameter_list|)
name|caddr_t
name|v
decl_stmt|,
name|netmask
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|head
decl_stmt|;
block|{
specifier|register
name|struct
name|radix_node
modifier|*
name|t
decl_stmt|,
modifier|*
name|p
decl_stmt|,
modifier|*
name|x
init|=
name|head
decl_stmt|;
specifier|register
name|struct
name|radix_node
modifier|*
name|tt
init|=
name|rn_search
argument_list|(
name|v
argument_list|,
name|x
argument_list|)
decl_stmt|;
name|int
name|b
decl_stmt|,
name|head_off
init|=
name|x
operator|->
name|rn_off
decl_stmt|,
name|vlen
init|=
operator|*
operator|(
name|u_char
operator|*
operator|)
name|v
decl_stmt|;
name|struct
name|radix_mask
modifier|*
name|m
decl_stmt|,
modifier|*
name|saved_m
decl_stmt|,
modifier|*
modifier|*
name|mp
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|dupedkey
decl_stmt|,
modifier|*
name|saved_tt
init|=
name|tt
decl_stmt|;
if|if
condition|(
name|tt
operator|==
literal|0
operator|||
name|Bcmp
argument_list|(
name|v
operator|+
name|head_off
argument_list|,
name|tt
operator|->
name|rn_key
operator|+
name|head_off
argument_list|,
name|vlen
operator|-
name|head_off
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
comment|/* 	 * Delete our route from mask lists. 	 */
if|if
condition|(
name|dupedkey
operator|=
name|tt
operator|->
name|rn_dupedkey
condition|)
block|{
if|if
condition|(
name|netmask
condition|)
name|netmask
operator|=
name|rn_search
argument_list|(
name|netmask
argument_list|,
name|rn_maskhead
argument_list|)
operator|->
name|rn_key
expr_stmt|;
while|while
condition|(
name|tt
operator|->
name|rn_mask
operator|!=
name|netmask
condition|)
if|if
condition|(
operator|(
name|tt
operator|=
name|tt
operator|->
name|rn_dupedkey
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
name|tt
operator|->
name|rn_mask
operator|==
literal|0
operator|||
operator|(
name|saved_m
operator|=
name|m
operator|=
name|tt
operator|->
name|rn_mklist
operator|)
operator|==
literal|0
condition|)
goto|goto
name|on1
goto|;
if|if
condition|(
name|m
operator|->
name|rm_mask
operator|!=
name|tt
operator|->
name|rn_mask
condition|)
block|{
name|printf
argument_list|(
literal|"rn_delete: inconsistent annotation\n"
argument_list|)
expr_stmt|;
goto|goto
name|on1
goto|;
block|}
if|if
condition|(
operator|--
name|m
operator|->
name|rm_refs
operator|>=
literal|0
condition|)
goto|goto
name|on1
goto|;
name|b
operator|=
operator|-
literal|1
operator|-
name|tt
operator|->
name|rn_b
expr_stmt|;
name|t
operator|=
name|saved_tt
operator|->
name|rn_p
expr_stmt|;
if|if
condition|(
name|b
operator|>
name|t
operator|->
name|rn_b
condition|)
goto|goto
name|on1
goto|;
comment|/* Wasn't lifted at all */
do|do
block|{
name|x
operator|=
name|t
expr_stmt|;
name|t
operator|=
name|t
operator|->
name|rn_p
expr_stmt|;
block|}
do|while
condition|(
name|b
operator|<=
name|t
operator|->
name|rn_b
operator|&&
name|x
operator|!=
name|head
condition|)
do|;
for|for
control|(
name|mp
operator|=
operator|&
name|x
operator|->
name|rn_mklist
init|;
name|m
operator|=
operator|*
name|mp
condition|;
name|mp
operator|=
operator|&
name|m
operator|->
name|rm_mklist
control|)
if|if
condition|(
name|m
operator|==
name|saved_m
condition|)
block|{
operator|*
name|mp
operator|=
name|m
operator|->
name|rm_mklist
expr_stmt|;
name|MKFree
argument_list|(
name|m
argument_list|)
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|m
operator|==
literal|0
condition|)
name|printf
argument_list|(
literal|"rn_delete: couldn't find our annotation\n"
argument_list|)
expr_stmt|;
name|on1
label|:
comment|/* 	 * Eliminate us from tree 	 */
if|if
condition|(
name|tt
operator|->
name|rn_flags
operator|&
name|RNF_ROOT
condition|)
return|return
operator|(
literal|0
operator|)
return|;
ifdef|#
directive|ifdef
name|RN_DEBUG
comment|/* Get us out of the creation list */
for|for
control|(
name|t
operator|=
name|rn_clist
init|;
name|t
operator|&&
name|t
operator|->
name|rn_ybro
operator|!=
name|tt
condition|;
name|t
operator|=
name|t
operator|->
name|rn_ybro
control|)
block|{}
if|if
condition|(
name|t
condition|)
name|t
operator|->
name|rn_ybro
operator|=
name|tt
operator|->
name|rn_ybro
expr_stmt|;
endif|#
directive|endif
endif|RN_DEBUG
name|t
operator|=
name|tt
operator|->
name|rn_p
expr_stmt|;
if|if
condition|(
name|dupedkey
condition|)
block|{
if|if
condition|(
name|tt
operator|==
name|saved_tt
condition|)
block|{
name|x
operator|=
name|dupedkey
expr_stmt|;
name|x
operator|->
name|rn_p
operator|=
name|t
expr_stmt|;
if|if
condition|(
name|t
operator|->
name|rn_l
operator|==
name|tt
condition|)
name|t
operator|->
name|rn_l
operator|=
name|x
expr_stmt|;
else|else
name|t
operator|->
name|rn_r
operator|=
name|x
expr_stmt|;
block|}
else|else
block|{
for|for
control|(
name|x
operator|=
name|p
operator|=
name|saved_tt
init|;
name|p
operator|&&
name|p
operator|->
name|rn_dupedkey
operator|!=
name|tt
condition|;
control|)
name|p
operator|=
name|p
operator|->
name|rn_dupedkey
expr_stmt|;
if|if
condition|(
name|p
condition|)
name|p
operator|->
name|rn_dupedkey
operator|=
name|tt
operator|->
name|rn_dupedkey
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"rn_delete: couldn't find us\n"
argument_list|)
expr_stmt|;
block|}
name|t
operator|=
name|tt
operator|+
literal|1
expr_stmt|;
if|if
condition|(
name|t
operator|->
name|rn_flags
operator|&
name|RNF_ACTIVE
condition|)
block|{
ifndef|#
directive|ifndef
name|RN_DEBUG
operator|*
operator|++
name|x
operator|=
operator|*
name|t
expr_stmt|;
name|p
operator|=
name|t
operator|->
name|rn_p
expr_stmt|;
else|#
directive|else
name|b
operator|=
name|t
operator|->
name|rn_info
expr_stmt|;
operator|*
operator|++
name|x
operator|=
operator|*
name|t
expr_stmt|;
name|t
operator|->
name|rn_info
operator|=
name|b
expr_stmt|;
name|p
operator|=
name|t
operator|->
name|rn_p
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|p
operator|->
name|rn_l
operator|==
name|t
condition|)
name|p
operator|->
name|rn_l
operator|=
name|x
expr_stmt|;
else|else
name|p
operator|->
name|rn_r
operator|=
name|x
expr_stmt|;
name|x
operator|->
name|rn_l
operator|->
name|rn_p
operator|=
name|x
expr_stmt|;
name|x
operator|->
name|rn_r
operator|->
name|rn_p
operator|=
name|x
expr_stmt|;
block|}
goto|goto
name|out
goto|;
block|}
if|if
condition|(
name|t
operator|->
name|rn_l
operator|==
name|tt
condition|)
name|x
operator|=
name|t
operator|->
name|rn_r
expr_stmt|;
else|else
name|x
operator|=
name|t
operator|->
name|rn_l
expr_stmt|;
name|p
operator|=
name|t
operator|->
name|rn_p
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|rn_r
operator|==
name|t
condition|)
name|p
operator|->
name|rn_r
operator|=
name|x
expr_stmt|;
else|else
name|p
operator|->
name|rn_l
operator|=
name|x
expr_stmt|;
name|x
operator|->
name|rn_p
operator|=
name|p
expr_stmt|;
comment|/* 	 * Demote routes attached to us. 	 */
if|if
condition|(
name|t
operator|->
name|rn_mklist
condition|)
block|{
if|if
condition|(
name|x
operator|->
name|rn_b
operator|>=
literal|0
condition|)
block|{
for|for
control|(
name|mp
operator|=
operator|&
name|x
operator|->
name|rn_mklist
init|;
name|m
operator|=
operator|*
name|mp
condition|;
control|)
name|mp
operator|=
operator|&
name|m
operator|->
name|rm_mklist
expr_stmt|;
operator|*
name|mp
operator|=
name|t
operator|->
name|rn_mklist
expr_stmt|;
block|}
else|else
block|{
for|for
control|(
name|m
operator|=
name|t
operator|->
name|rn_mklist
init|;
name|m
condition|;
control|)
block|{
name|struct
name|radix_mask
modifier|*
name|mm
init|=
name|m
operator|->
name|rm_mklist
decl_stmt|;
if|if
condition|(
name|m
operator|==
name|x
operator|->
name|rn_mklist
operator|&&
operator|(
operator|--
operator|(
name|m
operator|->
name|rm_refs
operator|)
operator|<
literal|0
operator|)
condition|)
block|{
name|x
operator|->
name|rn_mklist
operator|=
literal|0
expr_stmt|;
name|MKFree
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
else|else
name|printf
argument_list|(
literal|"%s %x at %x\n"
argument_list|,
literal|"rn_delete: Orphaned Mask"
argument_list|,
name|m
argument_list|,
name|x
argument_list|)
expr_stmt|;
name|m
operator|=
name|mm
expr_stmt|;
block|}
block|}
block|}
comment|/* 	 * We may be holding an active internal node in the tree. 	 */
name|x
operator|=
name|tt
operator|+
literal|1
expr_stmt|;
if|if
condition|(
name|t
operator|!=
name|x
condition|)
block|{
ifndef|#
directive|ifndef
name|RN_DEBUG
operator|*
name|t
operator|=
operator|*
name|x
expr_stmt|;
else|#
directive|else
name|b
operator|=
name|t
operator|->
name|rn_info
expr_stmt|;
operator|*
name|t
operator|=
operator|*
name|x
expr_stmt|;
name|t
operator|->
name|rn_info
operator|=
name|b
expr_stmt|;
endif|#
directive|endif
name|t
operator|->
name|rn_l
operator|->
name|rn_p
operator|=
name|t
expr_stmt|;
name|t
operator|->
name|rn_r
operator|->
name|rn_p
operator|=
name|t
expr_stmt|;
name|p
operator|=
name|x
operator|->
name|rn_p
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|rn_l
operator|==
name|x
condition|)
name|p
operator|->
name|rn_l
operator|=
name|t
expr_stmt|;
else|else
name|p
operator|->
name|rn_r
operator|=
name|t
expr_stmt|;
block|}
name|out
label|:
name|tt
operator|->
name|rn_flags
operator|&=
operator|~
name|RNF_ACTIVE
expr_stmt|;
name|tt
index|[
literal|1
index|]
operator|.
name|rn_flags
operator|&=
operator|~
name|RNF_ACTIVE
expr_stmt|;
return|return
operator|(
name|tt
operator|)
return|;
block|}
end_function

begin_expr_stmt
name|rn_walk
argument_list|(
name|rn
argument_list|,
name|f
argument_list|,
name|w
argument_list|)
specifier|register
expr|struct
name|radix_node
operator|*
name|rn
expr_stmt|;
end_expr_stmt

begin_function_decl
specifier|register
name|int
function_decl|(
modifier|*
name|f
function_decl|)
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|caddr_t
name|w
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|error
decl_stmt|;
name|struct
name|radix_node
modifier|*
name|orn
decl_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
while|while
condition|(
name|rn
operator|->
name|rn_b
operator|>=
literal|0
condition|)
name|rn
operator|=
name|rn
operator|->
name|rn_l
expr_stmt|;
comment|/* First time through node, go left */
for|for
control|(
name|orn
operator|=
name|rn
init|;
name|rn
condition|;
name|rn
operator|=
name|rn
operator|->
name|rn_dupedkey
control|)
comment|/* Process Leaves */
if|if
condition|(
operator|!
operator|(
name|rn
operator|->
name|rn_flags
operator|&
name|RNF_ROOT
operator|)
operator|&&
operator|(
name|error
operator|=
call|(
modifier|*
name|f
call|)
argument_list|(
name|rn
argument_list|,
name|w
argument_list|)
operator|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
for|for
control|(
name|rn
operator|=
name|orn
init|;
name|rn
operator|->
name|rn_p
operator|->
name|rn_r
operator|==
name|rn
condition|;
control|)
block|{
comment|/* If at right child */
name|rn
operator|=
name|rn
operator|->
name|rn_p
expr_stmt|;
comment|/* go back up */
if|if
condition|(
name|rn
operator|->
name|rn_flags
operator|&
name|RNF_ROOT
condition|)
return|return
literal|0
return|;
block|}
name|rn
operator|=
name|rn
operator|->
name|rn_p
operator|->
name|rn_r
expr_stmt|;
comment|/* otherwhise, go right*/
block|}
block|}
end_block

begin_decl_stmt
name|char
name|rn_zeros
index|[
name|MAXKEYLEN
index|]
decl_stmt|,
name|rn_ones
index|[
name|MAXKEYLEN
index|]
decl_stmt|;
end_decl_stmt

begin_macro
name|rn_inithead
argument_list|(
argument|head
argument_list|,
argument|off
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|radix_node_head
modifier|*
modifier|*
name|head
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|off
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|radix_node_head
modifier|*
name|rnh
decl_stmt|;
specifier|register
name|struct
name|radix_node
modifier|*
name|t
decl_stmt|,
modifier|*
name|tt
decl_stmt|,
modifier|*
name|ttt
decl_stmt|;
if|if
condition|(
operator|*
name|head
condition|)
return|return
operator|(
literal|1
operator|)
return|;
name|R_Malloc
argument_list|(
name|rnh
argument_list|,
expr|struct
name|radix_node_head
operator|*
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|rnh
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|rnh
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|Bzero
argument_list|(
name|rnh
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|rnh
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|head
operator|=
name|rnh
expr_stmt|;
name|t
operator|=
name|rn_newpair
argument_list|(
name|rn_zeros
argument_list|,
name|off
argument_list|,
name|rnh
operator|->
name|rnh_nodes
argument_list|)
expr_stmt|;
name|ttt
operator|=
name|rnh
operator|->
name|rnh_nodes
operator|+
literal|2
expr_stmt|;
name|t
operator|->
name|rn_r
operator|=
name|ttt
expr_stmt|;
name|t
operator|->
name|rn_p
operator|=
name|t
expr_stmt|;
name|tt
operator|=
name|t
operator|->
name|rn_l
expr_stmt|;
name|tt
operator|->
name|rn_flags
operator|=
name|t
operator|->
name|rn_flags
operator|=
name|RNF_ROOT
operator||
name|RNF_ACTIVE
expr_stmt|;
name|tt
operator|->
name|rn_b
operator|=
operator|-
literal|1
operator|-
name|off
expr_stmt|;
operator|*
name|ttt
operator|=
operator|*
name|tt
expr_stmt|;
name|ttt
operator|->
name|rn_key
operator|=
name|rn_ones
expr_stmt|;
name|rnh
operator|->
name|rnh_add
operator|=
name|rn_addroute
expr_stmt|;
name|rnh
operator|->
name|rnh_delete
operator|=
name|rn_delete
expr_stmt|;
name|rnh
operator|->
name|rnh_match
operator|=
name|rn_match
expr_stmt|;
name|rnh
operator|->
name|rnh_walk
operator|=
name|rn_walk
expr_stmt|;
name|rnh
operator|->
name|rnh_treetop
operator|=
name|t
expr_stmt|;
if|if
condition|(
name|mask_rnhead
operator|==
literal|0
condition|)
block|{
name|caddr_t
name|cp
init|=
name|rn_ones
decl_stmt|,
name|cplim
init|=
name|rn_ones
operator|+
name|MAXKEYLEN
decl_stmt|;
while|while
condition|(
name|cp
operator|<
name|cplim
condition|)
operator|*
name|cp
operator|++
operator|=
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|rn_inithead
argument_list|(
operator|&
name|mask_rnhead
argument_list|,
literal|0
argument_list|)
operator|==
literal|0
condition|)
block|{
name|Free
argument_list|(
name|rnh
argument_list|)
expr_stmt|;
operator|*
name|head
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
block|}
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

end_unit

