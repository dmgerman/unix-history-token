...
begin_comment
comment|/*	ffs_vnops.c	4.44	82/12/19	*/
end_comment
...
goto|;
block|}
comment|/* 		 * Target must be empty if a directory. 		 * Also, insure source and target are 		 * compatible (both directories, or both 		 * not directories). 		 */
if|if
condition|(
operator|(
name|xp
...
name|bad1
goto|;
comment|/* 		 * If this is a directory we know it is 		 * empty and we can squash the inode and 		 * any space associated with it.  Otherwise, 		 * we've got a plain file and the link count 		 * simply needs to be adjusted. 		 */
if|if
condition|(
name|doingdirectory
condition|)
