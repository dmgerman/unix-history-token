...
begin_comment
comment|/*  * Copyright (c) 1989, 1991 The Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)ffs_vfsops.c	7.61 (Berkeley) %G%  */
end_comment
...
return|;
block|}
comment|/*  * File handle to vnode  *  * Have to be really careful about stale file handles:  * - check that the inode number is valid  * - call ffs_vget() to get the locked inode  * - check for an unallocated inode (i_mode == 0)  * - check that the generation number matches unless setgen true  */
name|int
name|ffs_fhtovp
parameter_list|(
name|mp
