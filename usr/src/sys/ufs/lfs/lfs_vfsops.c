begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1989, 1991 The Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)lfs_vfsops.c	7.78 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/namei.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/buf.h>
end_include

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/disklabel.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<miscfs/specfs/specdev.h>
end_include

begin_include
include|#
directive|include
file|"ioctl.h"
end_include

begin_include
include|#
directive|include
file|"disklabel.h"
end_include

begin_include
include|#
directive|include
file|"stat.h"
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/quota.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/inode.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/ufsmount.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/ufs_extern.h>
end_include

begin_include
include|#
directive|include
file|<ufs/lfs/lfs.h>
end_include

begin_include
include|#
directive|include
file|<ufs/lfs/lfs_extern.h>
end_include

begin_decl_stmt
name|int
name|lfs_mountfs
name|__P
argument_list|(
operator|(
expr|struct
name|vnode
operator|*
operator|,
expr|struct
name|mount
operator|*
operator|,
expr|struct
name|proc
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|vfsops
name|lfs_vfsops
init|=
block|{
name|lfs_mount
block|,
name|ufs_start
block|,
name|lfs_unmount
block|,
name|lfs_root
block|,
name|ufs_quotactl
block|,
name|lfs_statfs
block|,
name|lfs_sync
block|,
name|lfs_vget
block|,
name|lfs_fhtovp
block|,
name|lfs_vptofh
block|,
name|lfs_init
block|, }
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|lfs_mountroot
parameter_list|()
block|{
name|panic
argument_list|(
literal|"lfs_mountroot"
argument_list|)
expr_stmt|;
comment|/* XXX -- implement */
block|}
end_function

begin_comment
comment|/*  * VFS Operations.  *  * mount system call  */
end_comment

begin_expr_stmt
name|lfs_mount
argument_list|(
name|mp
argument_list|,
name|path
argument_list|,
name|data
argument_list|,
name|ndp
argument_list|,
name|p
argument_list|)
specifier|register
expr|struct
name|mount
operator|*
name|mp
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|char
modifier|*
name|path
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|caddr_t
name|data
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|nameidata
modifier|*
name|ndp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|vnode
modifier|*
name|devvp
decl_stmt|;
name|struct
name|ufs_args
name|args
decl_stmt|;
name|struct
name|ufsmount
modifier|*
name|ump
decl_stmt|;
specifier|register
name|struct
name|lfs
modifier|*
name|fs
decl_stmt|;
comment|/* LFS */
name|u_int
name|size
decl_stmt|;
name|int
name|error
decl_stmt|;
if|if
condition|(
name|error
operator|=
name|copyin
argument_list|(
name|data
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|args
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|ufs_args
argument_list|)
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
comment|/* Until LFS can do NFS right.		XXX */
if|if
condition|(
name|args
operator|.
name|exflags
operator|&
name|MNT_EXPORTED
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
comment|/* 	 * If updating, check whether changing from read-only to 	 * read/write; if there is no device name, that's all we do. 	 */
if|if
condition|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_UPDATE
condition|)
block|{
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|NOTLFS
comment|/* LFS */
name|fs
operator|=
name|ump
operator|->
name|um_fs
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|fs_ronly
operator|&&
operator|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_RDONLY
operator|)
operator|==
literal|0
condition|)
name|fs
operator|->
name|fs_ronly
operator|=
literal|0
expr_stmt|;
else|#
directive|else
name|fs
operator|=
name|ump
operator|->
name|um_lfs
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|lfs_ronly
operator|&&
operator|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_RDONLY
operator|)
operator|==
literal|0
condition|)
name|fs
operator|->
name|lfs_ronly
operator|=
literal|0
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|args
operator|.
name|fspec
operator|==
literal|0
condition|)
block|{
comment|/* 			 * Process export requests. 			 */
if|if
condition|(
name|args
operator|.
name|exflags
operator|&
name|MNT_EXPORTED
condition|)
block|{
if|if
condition|(
name|error
operator|=
name|ufs_hang_addrlist
argument_list|(
name|mp
argument_list|,
operator|&
name|args
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|mp
operator|->
name|mnt_flag
operator||=
name|MNT_EXPORTED
expr_stmt|;
block|}
if|if
condition|(
name|args
operator|.
name|exflags
operator|&
name|MNT_DELEXPORT
condition|)
block|{
name|ufs_free_addrlist
argument_list|(
name|ump
argument_list|)
expr_stmt|;
name|mp
operator|->
name|mnt_flag
operator|&=
operator|~
operator|(
name|MNT_EXPORTED
operator||
name|MNT_DEFEXPORTED
operator|)
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
block|}
comment|/* 	 * Not an update, or updating the name: look up the name 	 * and verify that it refers to a sensible block device. 	 */
name|NDINIT
argument_list|(
name|ndp
argument_list|,
name|LOOKUP
argument_list|,
name|FOLLOW
argument_list|,
name|UIO_USERSPACE
argument_list|,
name|args
operator|.
name|fspec
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|=
name|namei
argument_list|(
name|ndp
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|devvp
operator|=
name|ndp
operator|->
name|ni_vp
expr_stmt|;
if|if
condition|(
name|devvp
operator|->
name|v_type
operator|!=
name|VBLK
condition|)
block|{
name|vrele
argument_list|(
name|devvp
argument_list|)
expr_stmt|;
return|return
operator|(
name|ENOTBLK
operator|)
return|;
block|}
if|if
condition|(
name|major
argument_list|(
name|devvp
operator|->
name|v_rdev
argument_list|)
operator|>=
name|nblkdev
condition|)
block|{
name|vrele
argument_list|(
name|devvp
argument_list|)
expr_stmt|;
return|return
operator|(
name|ENXIO
operator|)
return|;
block|}
if|if
condition|(
operator|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_UPDATE
operator|)
operator|==
literal|0
condition|)
name|error
operator|=
name|lfs_mountfs
argument_list|(
name|devvp
argument_list|,
name|mp
argument_list|,
name|p
argument_list|)
expr_stmt|;
comment|/* LFS */
else|else
block|{
if|if
condition|(
name|devvp
operator|!=
name|ump
operator|->
name|um_devvp
condition|)
name|error
operator|=
name|EINVAL
expr_stmt|;
comment|/* needs translation */
else|else
name|vrele
argument_list|(
name|devvp
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|error
condition|)
block|{
name|vrele
argument_list|(
name|devvp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|fs
operator|=
name|ump
operator|->
name|um_lfs
expr_stmt|;
comment|/* LFS */
ifdef|#
directive|ifdef
name|NOTLFS
comment|/* LFS */
operator|(
name|void
operator|)
name|copyinstr
argument_list|(
name|path
argument_list|,
name|fs
operator|->
name|fs_fsmnt
argument_list|,
sizeof|sizeof
argument_list|(
name|fs
operator|->
name|fs_fsmnt
argument_list|)
operator|-
literal|1
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|fs
operator|->
name|fs_fsmnt
operator|+
name|size
argument_list|,
sizeof|sizeof
argument_list|(
name|fs
operator|->
name|fs_fsmnt
argument_list|)
operator|-
name|size
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|fs
operator|->
name|fs_fsmnt
argument_list|,
operator|(
name|caddr_t
operator|)
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntonname
argument_list|,
name|MNAMELEN
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|copyinstr
argument_list|(
name|args
operator|.
name|fspec
argument_list|,
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
argument_list|,
name|MNAMELEN
operator|-
literal|1
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
operator|+
name|size
argument_list|,
name|MNAMELEN
operator|-
name|size
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|ufs_statfs
argument_list|(
name|mp
argument_list|,
operator|&
name|mp
operator|->
name|mnt_stat
argument_list|,
name|p
argument_list|)
expr_stmt|;
else|#
directive|else
operator|(
name|void
operator|)
name|copyinstr
argument_list|(
name|path
argument_list|,
name|fs
operator|->
name|lfs_fsmnt
argument_list|,
sizeof|sizeof
argument_list|(
name|fs
operator|->
name|lfs_fsmnt
argument_list|)
operator|-
literal|1
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|fs
operator|->
name|lfs_fsmnt
operator|+
name|size
argument_list|,
sizeof|sizeof
argument_list|(
name|fs
operator|->
name|lfs_fsmnt
argument_list|)
operator|-
name|size
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|fs
operator|->
name|lfs_fsmnt
argument_list|,
operator|(
name|caddr_t
operator|)
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntonname
argument_list|,
name|MNAMELEN
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|copyinstr
argument_list|(
name|args
operator|.
name|fspec
argument_list|,
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
argument_list|,
name|MNAMELEN
operator|-
literal|1
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
operator|+
name|size
argument_list|,
name|MNAMELEN
operator|-
name|size
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|lfs_statfs
argument_list|(
name|mp
argument_list|,
operator|&
name|mp
operator|->
name|mnt_stat
argument_list|,
name|p
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Common code for mount and mountroot  * LFS specific  */
end_comment

begin_function
name|int
name|lfs_mountfs
parameter_list|(
name|devvp
parameter_list|,
name|mp
parameter_list|,
name|p
parameter_list|)
specifier|register
name|struct
name|vnode
modifier|*
name|devvp
decl_stmt|;
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
specifier|extern
name|struct
name|vnode
modifier|*
name|rootvp
decl_stmt|;
specifier|register
name|struct
name|lfs
modifier|*
name|fs
decl_stmt|;
specifier|register
name|struct
name|ufsmount
modifier|*
name|ump
decl_stmt|;
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|struct
name|partinfo
name|dpart
decl_stmt|;
name|dev_t
name|dev
decl_stmt|;
name|int
name|error
decl_stmt|,
name|i
decl_stmt|,
name|ronly
decl_stmt|,
name|size
decl_stmt|;
if|if
condition|(
name|error
operator|=
name|VOP_OPEN
argument_list|(
name|devvp
argument_list|,
name|ronly
condition|?
name|FREAD
else|:
name|FREAD
operator||
name|FWRITE
argument_list|,
name|NOCRED
argument_list|,
name|p
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|VOP_IOCTL
argument_list|(
name|devvp
argument_list|,
name|DIOCGPART
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|dpart
argument_list|,
name|FREAD
argument_list|,
name|NOCRED
argument_list|,
name|p
argument_list|)
operator|!=
literal|0
condition|)
name|size
operator|=
name|DEV_BSIZE
expr_stmt|;
else|else
block|{
name|size
operator|=
name|dpart
operator|.
name|disklab
operator|->
name|d_secsize
expr_stmt|;
ifdef|#
directive|ifdef
name|NEVER_USED
name|dpart
operator|.
name|part
operator|->
name|p_fstype
operator|=
name|FS_LFS
expr_stmt|;
name|dpart
operator|.
name|part
operator|->
name|p_fsize
operator|=
name|fs
operator|->
name|lfs_fsize
expr_stmt|;
comment|/* frag size */
name|dpart
operator|.
name|part
operator|->
name|p_frag
operator|=
name|fs
operator|->
name|lfs_frag
expr_stmt|;
comment|/* frags per block */
name|dpart
operator|.
name|part
operator|->
name|p_cpg
operator|=
name|fs
operator|->
name|lfs_segshift
expr_stmt|;
comment|/* segment shift */
endif|#
directive|endif
block|}
comment|/* Don't free random space on error. */
name|bp
operator|=
name|NULL
expr_stmt|;
name|ump
operator|=
name|NULL
expr_stmt|;
comment|/* Read in the superblock. */
if|if
condition|(
name|error
operator|=
name|bread
argument_list|(
name|devvp
argument_list|,
name|LFS_LABELPAD
operator|/
name|size
argument_list|,
name|LFS_SBPAD
argument_list|,
name|NOCRED
argument_list|,
operator|&
name|bp
argument_list|)
condition|)
goto|goto
name|out
goto|;
name|error
operator|=
name|EINVAL
expr_stmt|;
comment|/* XXX needs translation */
goto|goto
name|out
goto|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|DEBUG
end_ifdef

begin_expr_stmt
name|lfs_dump_super
argument_list|(
name|fs
argument_list|)
expr_stmt|;
end_expr_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* Allocate the mount structure, copy the superblock into it. */
end_comment

begin_expr_stmt
name|ump
operator|=
operator|(
expr|struct
name|ufsmount
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
expr|*
name|ump
argument_list|,
name|M_UFSMNT
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|ump
operator|->
name|um_lfs
operator|=
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|lfs
argument_list|)
argument_list|,
name|M_UFSMNT
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|bcopy
argument_list|(
name|bp
operator|->
name|b_un
operator|.
name|b_addr
argument_list|,
name|ump
operator|->
name|um_lfs
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|lfs
argument_list|)
argument_list|)
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
sizeof|sizeof
argument_list|(
expr|struct
name|lfs
argument_list|)
operator|<
name|LFS_SBPAD
condition|)
comment|/* XXX why? */
name|bp
operator|->
name|b_flags
operator||=
name|B_INVAL
expr_stmt|;
end_if

begin_expr_stmt
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|bp
operator|=
name|NULL
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* Set up the I/O information */
end_comment

begin_expr_stmt
name|fs
operator|->
name|lfs_iocount
operator|=
literal|0
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* Set up the ifile flags */
end_comment

begin_expr_stmt
name|fs
operator|->
name|lfs_doifile
operator|=
literal|0
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|fs
operator|->
name|lfs_writer
operator|=
literal|0
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|fs
operator|->
name|lfs_dirops
operator|=
literal|0
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* Set the file system readonly/modify bits. */
end_comment

begin_expr_stmt
name|fs
operator|=
name|ump
operator|->
name|um_lfs
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|fs
operator|->
name|lfs_ronly
operator|=
name|ronly
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
name|ronly
operator|==
literal|0
condition|)
name|fs
operator|->
name|lfs_fmod
operator|=
literal|1
expr_stmt|;
end_if

begin_comment
comment|/* Initialize the mount structure. */
end_comment

begin_expr_stmt
name|dev
operator|=
name|devvp
operator|->
name|v_rdev
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|mp
operator|->
name|mnt_data
operator|=
operator|(
name|qaddr_t
operator|)
name|ump
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|mp
operator|->
name|mnt_stat
operator|.
name|f_fsid
operator|.
name|val
index|[
literal|0
index|]
operator|=
operator|(
name|long
operator|)
name|dev
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|mp
operator|->
name|mnt_stat
operator|.
name|f_fsid
operator|.
name|val
index|[
literal|1
index|]
operator|=
name|MOUNT_LFS
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|mp
operator|->
name|mnt_flag
operator||=
name|MNT_LOCAL
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|ump
operator|->
name|um_mountp
operator|=
name|mp
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|ump
operator|->
name|um_dev
operator|=
name|dev
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|ump
operator|->
name|um_devvp
operator|=
name|devvp
expr_stmt|;
end_expr_stmt

begin_for
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|MAXQUOTAS
condition|;
name|i
operator|++
control|)
name|ump
operator|->
name|um_quotas
index|[
name|i
index|]
operator|=
name|NULLVP
expr_stmt|;
end_for

begin_expr_stmt
name|devvp
operator|->
name|v_specflags
operator||=
name|SI_MOUNTEDON
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* 	 * We use the ifile vnode for almost every operation.  Instead of 	 * retrieving it from the hash table each time we retrieve it here, 	 * artificially increment the reference count and keep a pointer 	 * to it in the incore copy of the superblock. 	 */
end_comment

begin_if
if|if
condition|(
name|error
operator|=
name|VFS_VGET
argument_list|(
name|mp
argument_list|,
name|LFS_IFILE_INUM
argument_list|,
operator|&
name|vp
argument_list|)
condition|)
goto|goto
name|out
goto|;
end_if

begin_expr_stmt
name|fs
operator|->
name|lfs_ivnode
operator|=
name|vp
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|VREF
argument_list|(
name|vp
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|vput
argument_list|(
name|vp
argument_list|)
expr_stmt|;
end_expr_stmt

begin_return
return|return
operator|(
literal|0
operator|)
return|;
end_return

begin_label
name|out
label|:
end_label

begin_if
if|if
condition|(
name|bp
condition|)
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
end_if

begin_expr_stmt
operator|(
name|void
operator|)
name|VOP_CLOSE
argument_list|(
name|devvp
argument_list|,
name|ronly
condition|?
name|FREAD
else|:
name|FREAD
operator||
name|FWRITE
argument_list|,
name|NOCRED
argument_list|,
name|p
argument_list|)
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
name|ump
condition|)
block|{
name|free
argument_list|(
name|ump
operator|->
name|um_lfs
argument_list|,
name|M_UFSMNT
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|ump
argument_list|,
name|M_UFSMNT
argument_list|)
expr_stmt|;
name|mp
operator|->
name|mnt_data
operator|=
operator|(
name|qaddr_t
operator|)
literal|0
expr_stmt|;
block|}
end_if

begin_return
return|return
operator|(
name|error
operator|)
return|;
end_return

begin_comment
unit|}
comment|/*  * unmount system call  */
end_comment

begin_expr_stmt
unit|lfs_unmount
operator|(
name|mp
operator|,
name|mntflags
operator|,
name|p
operator|)
expr|struct
name|mount
operator|*
name|mp
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|mntflags
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|extern
name|int
name|doforce
decl_stmt|;
specifier|register
name|struct
name|ufsmount
modifier|*
name|ump
decl_stmt|;
specifier|register
name|struct
name|lfs
modifier|*
name|fs
decl_stmt|;
comment|/* LFS */
name|int
name|i
decl_stmt|,
name|error
decl_stmt|,
name|flags
decl_stmt|,
name|ronly
decl_stmt|;
ifdef|#
directive|ifdef
name|VERBOSE
name|printf
argument_list|(
literal|"lfs_unmount\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|flags
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|mntflags
operator|&
name|MNT_FORCE
condition|)
block|{
if|if
condition|(
operator|!
name|doforce
operator|||
name|mp
operator|==
name|rootfs
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|flags
operator||=
name|FORCECLOSE
expr_stmt|;
block|}
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|fs
operator|=
name|ump
operator|->
name|um_lfs
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
ifdef|#
directive|ifdef
name|QUOTA
if|if
condition|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_QUOTA
condition|)
block|{
if|if
condition|(
name|error
operator|=
name|vflush
argument_list|(
name|mp
argument_list|,
name|fs
operator|->
name|lfs_ivnode
argument_list|,
name|SKIPSYSTEM
operator||
name|flags
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|MAXQUOTAS
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|ump
operator|->
name|um_quotas
index|[
name|i
index|]
operator|==
name|NULLVP
condition|)
continue|continue;
name|quotaoff
argument_list|(
name|p
argument_list|,
name|mp
argument_list|,
name|i
argument_list|)
expr_stmt|;
block|}
comment|/* 		 * Here we fall through to vflush again to ensure 		 * that we have gotten rid of all the system vnodes. 		 */
block|}
endif|#
directive|endif
name|vrele
argument_list|(
name|fs
operator|->
name|lfs_ivnode
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|=
name|vflush
argument_list|(
name|mp
argument_list|,
name|fs
operator|->
name|lfs_ivnode
argument_list|,
name|flags
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|error
operator|=
name|VFS_SYNC
argument_list|(
name|mp
argument_list|,
literal|1
argument_list|,
name|p
operator|->
name|p_ucred
argument_list|,
name|p
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|fs
operator|->
name|lfs_ivnode
operator|->
name|v_dirtyblkhd
condition|)
name|panic
argument_list|(
literal|"lfs_unmount: still dirty blocks on ifile vnode\n"
argument_list|)
expr_stmt|;
name|vgone
argument_list|(
name|fs
operator|->
name|lfs_ivnode
argument_list|)
expr_stmt|;
name|ronly
operator|=
operator|!
name|fs
operator|->
name|lfs_ronly
expr_stmt|;
operator|*
name|Get
name|file
name|system
name|statistics
operator|.
modifier|*
expr|/
name|lfs_statfs
argument_list|(
argument|mp
argument_list|,
argument|sbp
argument_list|,
argument|p
argument_list|)
expr|struct
name|mount
operator|*
name|mp
expr_stmt|;
specifier|register
name|struct
name|statfs
modifier|*
name|sbp
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
specifier|register
name|struct
name|lfs
modifier|*
name|fs
decl_stmt|;
specifier|register
name|struct
name|ufsmount
modifier|*
name|ump
decl_stmt|;
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|fs
operator|=
name|ump
operator|->
name|um_lfs
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|lfs_magic
operator|!=
name|LFS_MAGIC
condition|)
name|panic
argument_list|(
literal|"lfs_statfs: magic"
argument_list|)
expr_stmt|;
name|sbp
operator|->
name|f_type
operator|=
name|MOUNT_LFS
expr_stmt|;
name|sbp
operator|->
name|f_bsize
operator|=
name|fs
operator|->
name|lfs_bsize
expr_stmt|;
name|sbp
operator|->
name|f_iosize
operator|=
name|fs
operator|->
name|lfs_bsize
expr_stmt|;
name|sbp
operator|->
name|f_blocks
operator|=
name|fs
operator|->
name|lfs_dsize
expr_stmt|;
name|sbp
operator|->
name|f_bfree
operator|=
name|fs
operator|->
name|lfs_bfree
expr_stmt|;
name|sbp
operator|->
name|f_bavail
operator|=
operator|(
name|fs
operator|->
name|lfs_dsize
operator|*
operator|(
literal|100
operator|-
name|fs
operator|->
name|lfs_minfree
operator|)
operator|/
literal|100
operator|)
operator|-
operator|(
name|fs
operator|->
name|lfs_dsize
operator|-
name|sbp
operator|->
name|f_bfree
operator|)
expr_stmt|;
name|sbp
operator|->
name|f_files
operator|=
name|fs
operator|->
name|lfs_nfiles
expr_stmt|;
name|sbp
operator|->
name|f_ffree
operator|=
name|fs
operator|->
name|lfs_bfree
operator|*
name|INOPB
argument_list|(
name|fs
argument_list|)
expr_stmt|;
if|if
condition|(
name|sbp
operator|!=
operator|&
name|mp
operator|->
name|mnt_stat
condition|)
block|{
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntonname
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|sbp
operator|->
name|f_mntonname
index|[
literal|0
index|]
argument_list|,
name|MNAMELEN
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|sbp
operator|->
name|f_mntfromname
index|[
literal|0
index|]
argument_list|,
name|MNAMELEN
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/*  * Go through the disk queues to initiate sandbagged IO;  * go through the inodes to write those that have been modified;  * initiate the writing of the super block if it has been modified.  *  * Note: we are always called with the filesystem marked `MPBUSY'.  */
name|lfs_sync
argument_list|(
argument|mp
argument_list|,
argument|waitfor
argument_list|,
argument|cred
argument_list|,
argument|p
argument_list|)
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
name|int
name|waitfor
decl_stmt|;
name|struct
name|ucred
modifier|*
name|cred
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
specifier|extern
name|int
name|crashandburn
decl_stmt|,
name|syncprt
decl_stmt|;
name|int
name|error
decl_stmt|;
ifdef|#
directive|ifdef
name|VERBOSE
name|printf
argument_list|(
literal|"lfs_sync\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|DIAGNOSTIC
if|if
condition|(
name|crashandburn
condition|)
return|return
operator|(
literal|0
operator|)
return|;
endif|#
directive|endif
comment|/* All syncs must be checkpoints until roll-forward is implemented. */
name|error
operator|=
name|lfs_segwrite
argument_list|(
name|mp
argument_list|,
literal|1
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|QUOTA
name|qsync
argument_list|(
name|mp
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
operator|(
name|error
operator|)
return|;
block|}
comment|/*  * Look up an LFS dinode number to find its incore vnode.  If not already  * in core, read it in from the specified device.  Return the inode locked.  * Detection and handling of mount points must be done by the calling routine.  */
name|int
name|lfs_vget
parameter_list|(
name|mp
parameter_list|,
name|ino
parameter_list|,
name|vpp
parameter_list|)
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
name|ino_t
name|ino
decl_stmt|;
name|struct
name|vnode
modifier|*
modifier|*
name|vpp
decl_stmt|;
block|{
specifier|register
name|struct
name|lfs
modifier|*
name|fs
decl_stmt|;
specifier|register
name|struct
name|inode
modifier|*
name|ip
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|struct
name|ifile
modifier|*
name|ifp
decl_stmt|;
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|struct
name|ufsmount
modifier|*
name|ump
decl_stmt|;
name|daddr_t
name|daddr
decl_stmt|;
name|dev_t
name|dev
decl_stmt|;
name|int
name|error
decl_stmt|;
ifdef|#
directive|ifdef
name|VERBOSE
name|printf
argument_list|(
literal|"lfs_vget\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|dev
operator|=
name|ump
operator|->
name|um_dev
expr_stmt|;
if|if
condition|(
operator|(
operator|*
name|vpp
operator|=
name|ufs_ihashget
argument_list|(
name|dev
argument_list|,
name|ino
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
return|return
operator|(
literal|0
operator|)
return|;
comment|/* Translate the inode number to a disk address. */
name|fs
operator|=
name|ump
operator|->
name|um_lfs
expr_stmt|;
if|if
condition|(
name|ino
operator|==
name|LFS_IFILE_INUM
condition|)
name|daddr
operator|=
name|fs
operator|->
name|lfs_idaddr
expr_stmt|;
else|else
block|{
name|LFS_IENTRY
argument_list|(
name|ifp
argument_list|,
name|fs
argument_list|,
name|ino
argument_list|,
name|bp
argument_list|)
expr_stmt|;
name|daddr
operator|=
name|ifp
operator|->
name|if_daddr
expr_stmt|;
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
if|if
condition|(
name|daddr
operator|==
name|LFS_UNUSED_DADDR
condition|)
return|return
operator|(
name|ENOENT
operator|)
return|;
block|}
comment|/* Allocate new vnode/inode. */
if|if
condition|(
name|error
operator|=
name|lfs_vcreate
argument_list|(
name|mp
argument_list|,
name|ino
argument_list|,
operator|&
name|vp
argument_list|)
condition|)
block|{
operator|*
name|vpp
operator|=
name|NULL
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
comment|/* 	 * Put it onto its hash chain and lock it so that other requests for 	 * this inode will block if they arrive while we are sleeping waiting 	 * for old data structures to be purged or for the contents of the 	 * disk portion of this inode to be read. 	 */
name|ip
operator|=
name|VTOI
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|ufs_ihashins
argument_list|(
name|ip
argument_list|)
expr_stmt|;
comment|/* 	 * XXX 	 * This may not need to be here, logically it should go down with 	 * the i_devvp initialization. 	 * Ask Kirk. 	 */
name|ip
operator|->
name|i_lfs
operator|=
name|ump
operator|->
name|um_lfs
expr_stmt|;
comment|/* Read in the disk contents for the inode, copy into the inode. */
if|if
condition|(
name|error
operator|=
name|bread
argument_list|(
name|ump
operator|->
name|um_devvp
argument_list|,
name|daddr
argument_list|,
operator|(
name|int
operator|)
name|fs
operator|->
name|lfs_bsize
argument_list|,
name|NOCRED
argument_list|,
operator|&
name|bp
argument_list|)
condition|)
block|{
comment|/* 		 * The inode does not contain anything useful, so it 		 * would be misleading to leave it on its hash chain. 		 * Iput() will return it to the free list. 		 */
name|remque
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|ip
operator|->
name|i_forw
operator|=
name|ip
expr_stmt|;
name|ip
operator|->
name|i_back
operator|=
name|ip
expr_stmt|;
comment|/* Unlock and discard unneeded inode. */
name|ufs_iput
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
operator|*
name|vpp
operator|=
name|NULL
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ip
operator|->
name|i_din
operator|=
operator|*
name|lfs_ifind
argument_list|(
name|fs
argument_list|,
name|ino
argument_list|,
name|bp
operator|->
name|b_un
operator|.
name|b_dino
argument_list|)
expr_stmt|;
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
comment|/* 	 * Initialize the vnode from the inode, check for aliases.  In all 	 * cases re-init ip, the underlying vnode/inode may have changed. 	 */
if|if
condition|(
name|error
operator|=
name|ufs_vinit
argument_list|(
name|mp
argument_list|,
name|lfs_specop_p
argument_list|,
name|LFS_FIFOOPS
argument_list|,
operator|&
name|vp
argument_list|)
condition|)
block|{
name|ufs_iput
argument_list|(
name|ip
argument_list|)
expr_stmt|;
operator|*
name|vpp
operator|=
name|NULL
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
comment|/* 	 * Finish inode initialization now that aliasing has been resolved. 	 */
name|ip
operator|->
name|i_devvp
operator|=
name|ump
operator|->
name|um_devvp
expr_stmt|;
name|VREF
argument_list|(
name|ip
operator|->
name|i_devvp
argument_list|)
expr_stmt|;
operator|*
name|vpp
operator|=
name|vp
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/*  * File handle to vnode  *  * Have to be really careful about stale file handles:  * - check that the inode number is valid  * - call lfs_vget() to get the locked inode  * - check for an unallocated inode (i_mode == 0)  * - check that the generation number matches  *  * XXX  * use ifile to see if inode is allocated instead of reading off disk  * what is the relationship between my generational number and the NFS  * generational number.  */
name|int
name|lfs_fhtovp
parameter_list|(
name|mp
parameter_list|,
name|fhp
parameter_list|,
name|nam
parameter_list|,
name|vpp
parameter_list|,
name|exflagsp
parameter_list|,
name|credanonp
parameter_list|)
specifier|register
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
name|struct
name|fid
modifier|*
name|fhp
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|nam
decl_stmt|;
name|struct
name|vnode
modifier|*
modifier|*
name|vpp
decl_stmt|;
name|int
modifier|*
name|exflagsp
decl_stmt|;
name|struct
name|ucred
modifier|*
modifier|*
name|credanonp
decl_stmt|;
block|{
specifier|register
name|struct
name|inode
modifier|*
name|ip
decl_stmt|;
specifier|register
name|struct
name|ufid
modifier|*
name|ufhp
decl_stmt|;
specifier|register
name|struct
name|netaddrhash
modifier|*
name|np
decl_stmt|;
specifier|register
name|struct
name|ufsmount
modifier|*
name|ump
init|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
decl_stmt|;
name|struct
name|vnode
modifier|*
name|nvp
decl_stmt|;
name|struct
name|sockaddr
modifier|*
name|saddr
decl_stmt|;
name|int
name|error
decl_stmt|;
name|ufhp
operator|=
operator|(
expr|struct
name|ufid
operator|*
operator|)
name|fhp
expr_stmt|;
if|if
condition|(
name|ufhp
operator|->
name|ufid_ino
operator|<
name|ROOTINO
condition|)
return|return
operator|(
name|ESTALE
operator|)
return|;
comment|/* 	 * Get the export permission structure for this<mp, client> tuple. 	 */
if|if
condition|(
operator|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_EXPORTED
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
name|EACCES
operator|)
return|;
if|if
condition|(
name|nam
operator|==
name|NULL
condition|)
block|{
name|np
operator|=
operator|(
expr|struct
name|netaddrhash
operator|*
operator|)
literal|0
expr_stmt|;
block|}
else|else
block|{
comment|/* 		 * First search for a network match. 		 */
name|np
operator|=
name|ump
operator|->
name|um_netaddr
index|[
name|NETMASK_HASH
index|]
expr_stmt|;
while|while
condition|(
name|np
condition|)
block|{
if|if
condition|(
name|netaddr_match
argument_list|(
name|np
operator|->
name|neth_family
argument_list|,
operator|&
name|np
operator|->
name|neth_haddr
argument_list|,
operator|&
name|np
operator|->
name|neth_hmask
argument_list|,
name|nam
argument_list|)
condition|)
break|break;
name|np
operator|=
name|np
operator|->
name|neth_next
expr_stmt|;
block|}
comment|/* 		 * If not found, try for an address match. 		 */
if|if
condition|(
name|np
operator|==
operator|(
expr|struct
name|netaddrhash
operator|*
operator|)
literal|0
condition|)
block|{
name|saddr
operator|=
name|mtod
argument_list|(
name|nam
argument_list|,
expr|struct
name|sockaddr
operator|*
argument_list|)
expr_stmt|;
name|np
operator|=
name|ump
operator|->
name|um_netaddr
index|[
name|NETADDRHASH
argument_list|(
name|saddr
argument_list|)
index|]
expr_stmt|;
while|while
condition|(
name|np
condition|)
block|{
if|if
condition|(
name|netaddr_match
argument_list|(
name|np
operator|->
name|neth_family
argument_list|,
operator|&
name|np
operator|->
name|neth_haddr
argument_list|,
operator|(
expr|struct
name|netaddrhash
operator|*
operator|)
literal|0
argument_list|,
name|nam
argument_list|)
condition|)
break|break;
name|np
operator|=
name|np
operator|->
name|neth_next
expr_stmt|;
block|}
block|}
block|}
if|if
condition|(
name|np
operator|==
operator|(
expr|struct
name|netaddrhash
operator|*
operator|)
literal|0
condition|)
block|{
comment|/* 		 * If no address match, use the default if it exists. 		 */
if|if
condition|(
operator|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_DEFEXPORTED
operator|)
operator|==
literal|0
condition|)
return|return
operator|(
name|EACCES
operator|)
return|;
name|np
operator|=
operator|&
name|ump
operator|->
name|um_defexported
expr_stmt|;
block|}
if|if
condition|(
name|error
operator|=
name|VFS_VGET
argument_list|(
name|mp
argument_list|,
name|ufhp
operator|->
name|ufid_ino
argument_list|,
operator|&
name|nvp
argument_list|)
condition|)
block|{
operator|*
name|vpp
operator|=
name|NULLVP
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ip
operator|=
name|VTOI
argument_list|(
name|nvp
argument_list|)
expr_stmt|;
if|if
condition|(
name|ip
operator|->
name|i_mode
operator|==
literal|0
operator|||
name|ip
operator|->
name|i_gen
operator|!=
name|ufhp
operator|->
name|ufid_gen
condition|)
block|{
name|ufs_iput
argument_list|(
name|ip
argument_list|)
expr_stmt|;
operator|*
name|vpp
operator|=
name|NULLVP
expr_stmt|;
return|return
operator|(
name|ESTALE
operator|)
return|;
block|}
operator|*
name|vpp
operator|=
name|nvp
expr_stmt|;
operator|*
name|exflagsp
operator|=
name|np
operator|->
name|neth_exflags
expr_stmt|;
operator|*
name|credanonp
operator|=
operator|&
name|np
operator|->
name|neth_anon
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/*  * Vnode pointer to File handle  */
comment|/* ARGSUSED */
name|lfs_vptofh
argument_list|(
argument|vp
argument_list|,
argument|fhp
argument_list|)
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|struct
name|fid
modifier|*
name|fhp
decl_stmt|;
block|{
specifier|register
name|struct
name|inode
modifier|*
name|ip
decl_stmt|;
specifier|register
name|struct
name|ufid
modifier|*
name|ufhp
decl_stmt|;
name|ip
operator|=
name|VTOI
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|ufhp
operator|=
operator|(
expr|struct
name|ufid
operator|*
operator|)
name|fhp
expr_stmt|;
name|ufhp
operator|->
name|ufid_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|ufid
argument_list|)
expr_stmt|;
name|ufhp
operator|->
name|ufid_ino
operator|=
name|ip
operator|->
name|i_number
expr_stmt|;
name|ufhp
operator|->
name|ufid_gen
operator|=
name|ip
operator|->
name|i_gen
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

end_unit

