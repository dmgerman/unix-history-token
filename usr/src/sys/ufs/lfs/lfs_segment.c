...
begin_comment
comment|/*  * Copyright (c) 1991 Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)lfs_segment.c	5.2 (Berkeley) %G%  */
end_comment
...
name|devvp
decl_stmt|;
comment|/* 	 * Grab the mount point for later (used to find the file system and 	 * block device) and, if the contents are valid, move the buffer back 	 * onto the clean list. 	 */
name|printf
argument_list|(
literal|"lfs_biocallback: buffer %x\n"
argument_list|,
name|bp
argument_list|,
name|bp
...
condition|)
block|{
comment|/* Wake up any other syncs waiting on this file system. */
return|return;
block|}
operator|--
name|fs
...
condition|)
block|{
comment|/* 		 * This buffer is now full.  Compute the next address if appropriate 		 * and the checksum, and close the buffer by setting sp->sbp NULL. 		 */
if|if
condition|(
name|calc_next
condition|)
...
name|cbpp
expr_stmt|;
comment|/* You know that you have a new FINFO either way */
name|start_lbp
operator|=
name|lbp
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * XXX 	 * Need a devvp, but this isn't a particularly clean way to get one. 	 * devvp = VTOI(fs->lfs_ivnode)->i_devvp; 	 */
ifdef|#
directive|ifdef
name|NOTWORKING
name|devvp
...
name|printf
argument_list|(
literal|"lfs_newseg: About to write segment %lx\n"
argument_list|,
name|sp
operator|->
name|saddr
...
name|printf
argument_list|(
literal|"Inserting summary block, address %x at index %d\n"
argument_list|,
name|sp
operator|->
name|sbp
...
condition|)
return|return;
comment|/* First sort the blocks and add disk addresses */
name|shellsort
argument_list|(
name|bpp
argument_list|,
...
name|panic
argument_list|(
literal|"lfs_updatemeta: lfs_bmap returned error"
argument_list|)
expr_stmt|;
if|if
condition|(
...
expr_stmt|;
block|}
comment|/* 		 * Now change whoever points to lbn.  We could start with the 		 * smallest (most negative) block number in these if clauses, 		 * but we assume that indirect blocks are least common, and 		 * handle them separately. 		 */
name|bp
operator|=
name|NULL
expr_stmt|;
...
name|panic
argument_list|(
literal|"lfs_updatemeta: error on bread"
argument_list|)
expr_stmt|;
name|bp
operator|->
...
name|panic
argument_list|(
literal|"lfs_updatemeta: bread returned error"
argument_list|)
expr_stmt|;
name|bp
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * Make sure that the segment usage table and the ifile inode will 	 * fit in this segment.  If they won't, put them in the next segment 	 */
name|bytes_needed
operator|=
name|fs
operator|->
...
directive|endif
comment|/* 	 * Now, update the segment usage information and the ifile inode and 	 * and write it out 	 */
name|sup
operator|=
name|fs
operator|->
...
name|SEGUSE_DIRTY
expr_stmt|;
comment|/* Get buffers for the segusage table and write it out */
name|ip
operator|=
name|VTOI
argument_list|(
...
begin_comment
comment|/*  * XXX -- I think we need to figure out what to do if we write  * the segment and find more dirty blocks when we're done.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/*  	 * Update the inode for this file and reflect new inode 	 * address in the ifile.  If this is the ifile, don't update 	 * the inode, because we're checkpointing and will update the 	 * inode with the segment usage information (so we musn't 	 * bump the finfo pointer either). 	 */
if|if
condition|(
name|ip
operator|->
...
name|printf
argument_list|(
literal|"alloc inode block @ daddr %x, bp = %x inserted at %d\n"
argument_list|,
name|next_addr
argument_list|,
name|sp
...
name|SEGUSE_DIRTY
expr_stmt|;
comment|/* 	 * Since we need to guarantee that the summary block gets written last, 	 * we issue the writes in two sets.  The first n-1 buffers first, and 	 * then, after they've completed, the last 1 buffer.  Only when that 	 * final write completes is the segment valid. 	 */
name|devvp
operator|=
name|VFSTOUFS
argument_list|(
...
name|um_devvp
expr_stmt|;
comment|/* 	 * Since no writes are yet scheduled, no need to block here; if we 	 * scheduled the writes at multiple points, we'd need an splbio() 	 * here. 	 */
name|fs
operator|->
name|lfs_iocount
operator|=
...
name|bpp
control|)
comment|/* (*(devvp->v_op->vop_strategy)) */
name|sdstrategy
argument_list|(
operator|*
name|bpp
...
argument_list|)
expr_stmt|;
comment|/* Wait for segment write to complete */
comment|/* XXX probably should do this biowait(*(sp->cbpp - 1)); */
comment|/* Get a buffer for the super block */
name|fs
operator|->
name|lfs_cksum
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Now, write the second one for which we don't have to wait */
name|bp
operator|->
name|b_flags
operator|&=
...
begin_comment
comment|/* Block match routines used when traversing the dirty block chain. */
end_comment
