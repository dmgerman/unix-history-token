...
condition|)
continue|continue;
comment|/* 		 * XXX 		 * Up the ref count so we don't get tossed out of 		 * memory. 		 */
name|VREF
argument_list|(
name|vp
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* 	 * Keep a cumulative count of the outstanding I/O operations.  If the 	 * disk drive catches up with us it could go to zero before we finish, 	 * so we artificially increment it by one until we've scheduled all of 	 * the writes we intend to do.  If not a checkpoint, we never do the 	 * final decrement, avoiding the wakeup in the callback routine. 	 */
name|s
operator|=
name|splbio
argument_list|()
...
argument_list|)
expr_stmt|;
comment|/* 	 * If this is a checkpoint, we need to loop on both the ifile and 	 * the writeseg to make sure that we don't end up with any dirty 	 * buffers left when this is all over. 	 */
if|if
condition|(
name|do_ckp
operator|||
...
directive|endif
comment|/* Checkpoint always writes superblock, even if no data blocks. */
if|if
condition|(
operator|(
name|nblocks
