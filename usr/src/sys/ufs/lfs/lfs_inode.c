begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982, 1986, 1989 Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)lfs_inode.c	7.42 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|"param.h"
end_include

begin_include
include|#
directive|include
file|"systm.h"
end_include

begin_include
include|#
directive|include
file|"mount.h"
end_include

begin_include
include|#
directive|include
file|"proc.h"
end_include

begin_include
include|#
directive|include
file|"file.h"
end_include

begin_include
include|#
directive|include
file|"buf.h"
end_include

begin_include
include|#
directive|include
file|"vnode.h"
end_include

begin_include
include|#
directive|include
file|"kernel.h"
end_include

begin_include
include|#
directive|include
file|"malloc.h"
end_include

begin_include
include|#
directive|include
file|"../ufs/quota.h"
end_include

begin_include
include|#
directive|include
file|"../ufs/inode.h"
end_include

begin_include
include|#
directive|include
file|"../ufs/ufsmount.h"
end_include

begin_include
include|#
directive|include
file|"lfs.h"
end_include

begin_include
include|#
directive|include
file|"lfs_extern.h"
end_include

begin_define
define|#
directive|define
name|INOHSZ
value|512
end_define

begin_if
if|#
directive|if
operator|(
operator|(
name|INOHSZ
operator|&
operator|(
name|INOHSZ
operator|-
literal|1
operator|)
operator|)
operator|==
literal|0
operator|)
end_if

begin_define
define|#
directive|define
name|INOHASH
parameter_list|(
name|dev
parameter_list|,
name|ino
parameter_list|)
value|(((dev)+(ino))&(INOHSZ-1))
end_define

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|INOHASH
parameter_list|(
name|dev
parameter_list|,
name|ino
parameter_list|)
value|(((unsigned)((dev)+(ino)))%INOHSZ)
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_union
union|union
name|lfsihead
block|{
comment|/* LFS */
name|union
name|lfsihead
modifier|*
name|ih_head
index|[
literal|2
index|]
decl_stmt|;
name|struct
name|inode
modifier|*
name|ih_chain
index|[
literal|2
index|]
decl_stmt|;
block|}
name|lfsihead
index|[
name|INOHSZ
index|]
union|;
end_union

begin_comment
comment|/* LFS */
end_comment

begin_decl_stmt
specifier|extern
name|int
name|prtactive
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* 1 => print out reclaim of active vnodes */
end_comment

begin_comment
comment|/*  * Initialize hash links for inodes.  */
end_comment

begin_macro
name|lfs_init
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
specifier|register
name|union
name|lfsihead
modifier|*
name|ih
init|=
name|lfsihead
decl_stmt|;
name|printf
argument_list|(
literal|"lfs_init\n"
argument_list|)
expr_stmt|;
ifndef|#
directive|ifndef
name|lint
if|if
condition|(
name|VN_MAXPRIVATE
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|inode
argument_list|)
condition|)
name|panic
argument_list|(
literal|"ihinit: too small"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* not lint */
for|for
control|(
name|i
operator|=
name|INOHSZ
init|;
operator|--
name|i
operator|>=
literal|0
condition|;
name|ih
operator|++
control|)
block|{
name|ih
operator|->
name|ih_head
index|[
literal|0
index|]
operator|=
name|ih
expr_stmt|;
name|ih
operator|->
name|ih_head
index|[
literal|1
index|]
operator|=
name|ih
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|NOTLFS
comment|/* LFS */
ifdef|#
directive|ifdef
name|QUOTA
name|dqinit
argument_list|()
expr_stmt|;
endif|#
directive|endif
comment|/* QUOTA */
endif|#
directive|endif
block|}
end_block

begin_macro
name|lfs_hqueue
argument_list|(
argument|ip
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|inode
modifier|*
name|ip
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|union
name|lfsihead
modifier|*
name|ih
decl_stmt|;
name|printf
argument_list|(
literal|"lfs_hqueue ino %d\n"
argument_list|,
name|ip
operator|->
name|i_number
argument_list|)
expr_stmt|;
name|ih
operator|=
operator|&
name|lfsihead
index|[
name|INOHASH
argument_list|(
name|ip
operator|->
name|i_dev
argument_list|,
name|ip
operator|->
name|i_number
argument_list|)
index|]
expr_stmt|;
name|insque
argument_list|(
name|ip
argument_list|,
name|ih
argument_list|)
expr_stmt|;
name|ILOCK
argument_list|(
name|ip
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Look up a UFS dinode number to find its incore vnode.  * If it is not in core, read it in from the specified device.  * If it is in core, wait for the lock bit to clear, then  * return the inode locked. Detection and handling of mount  * points must be done by the calling routine.  */
end_comment

begin_macro
name|lfs_iget
argument_list|(
argument|xp
argument_list|,
argument|ino
argument_list|,
argument|ipp
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|inode
modifier|*
name|xp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|ino_t
name|ino
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|inode
modifier|*
modifier|*
name|ipp
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|dev_t
name|dev
init|=
name|xp
operator|->
name|i_dev
decl_stmt|;
name|struct
name|mount
modifier|*
name|mntp
init|=
name|ITOV
argument_list|(
name|xp
argument_list|)
operator|->
name|v_mount
decl_stmt|;
specifier|register
name|LFS
modifier|*
name|fs
init|=
name|VFSTOUFS
argument_list|(
name|mntp
argument_list|)
operator|->
name|um_lfs
decl_stmt|;
comment|/* LFS */
specifier|extern
name|struct
name|vnodeops
name|ufs_vnodeops
decl_stmt|,
name|spec_inodeops
decl_stmt|;
specifier|register
name|struct
name|inode
modifier|*
name|ip
decl_stmt|,
modifier|*
name|iq
decl_stmt|;
specifier|register
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|struct
name|vnode
modifier|*
name|nvp
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|union
name|lfsihead
modifier|*
name|ih
decl_stmt|;
name|int
name|i
decl_stmt|,
name|error
decl_stmt|;
name|printf
argument_list|(
literal|"lfs_iget ino %d\n"
argument_list|,
name|ino
argument_list|)
expr_stmt|;
name|ih
operator|=
operator|&
name|lfsihead
index|[
name|INOHASH
argument_list|(
name|dev
argument_list|,
name|ino
argument_list|)
index|]
expr_stmt|;
name|loop
label|:
for|for
control|(
name|ip
operator|=
name|ih
operator|->
name|ih_chain
index|[
literal|0
index|]
init|;
name|ip
operator|!=
operator|(
expr|struct
name|inode
operator|*
operator|)
name|ih
condition|;
name|ip
operator|=
name|ip
operator|->
name|i_forw
control|)
block|{
if|if
condition|(
name|ino
operator|!=
name|ip
operator|->
name|i_number
operator|||
name|dev
operator|!=
name|ip
operator|->
name|i_dev
condition|)
continue|continue;
if|if
condition|(
operator|(
name|ip
operator|->
name|i_flag
operator|&
name|ILOCKED
operator|)
operator|!=
literal|0
condition|)
block|{
name|ip
operator|->
name|i_flag
operator||=
name|IWANT
expr_stmt|;
name|sleep
argument_list|(
operator|(
name|caddr_t
operator|)
name|ip
argument_list|,
name|PINOD
argument_list|)
expr_stmt|;
goto|goto
name|loop
goto|;
block|}
if|if
condition|(
name|vget
argument_list|(
name|ITOV
argument_list|(
name|ip
argument_list|)
argument_list|)
condition|)
goto|goto
name|loop
goto|;
operator|*
name|ipp
operator|=
name|ip
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/* Allocate new vnode/inode. */
name|error
operator|=
name|lfs_vcreate
argument_list|(
name|mntp
argument_list|,
name|ino
argument_list|,
operator|&
name|nvp
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
operator|*
name|ipp
operator|=
literal|0
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ip
operator|=
name|VTOI
argument_list|(
name|nvp
argument_list|)
expr_stmt|;
comment|/* 	 * Put it onto its hash chain and lock it so that other requests for 	 * this inode will block if they arrive while we are sleeping waiting 	 * for old data structures to be purged or for the contents of the 	 * disk portion of this inode to be read. 	 */
name|insque
argument_list|(
name|ip
argument_list|,
name|ih
argument_list|)
expr_stmt|;
name|ILOCK
argument_list|(
name|ip
argument_list|)
expr_stmt|;
comment|/* Read in the disk contents for the inode, copy into the vnode. */
if|if
condition|(
name|error
operator|=
name|bread
argument_list|(
name|VFSTOUFS
argument_list|(
name|mntp
argument_list|)
operator|->
name|um_devvp
argument_list|,
name|itod
argument_list|(
name|fs
argument_list|,
name|ino
argument_list|)
argument_list|,
operator|(
name|int
operator|)
name|fs
operator|->
name|lfs_bsize
argument_list|,
name|NOCRED
argument_list|,
operator|&
name|bp
argument_list|)
condition|)
block|{
comment|/* LFS */
comment|/* 		 * The inode does not contain anything useful, so it would 		 * be misleading to leave it on its hash chain. 		 * Iput() will take care of putting it back on the free list. 		 */
name|remque
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|ip
operator|->
name|i_forw
operator|=
name|ip
expr_stmt|;
name|ip
operator|->
name|i_back
operator|=
name|ip
expr_stmt|;
comment|/* 		 * Unlock and discard unneeded inode. 		 */
name|iput
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
operator|*
name|ipp
operator|=
literal|0
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ip
operator|->
name|i_din
operator|=
operator|*
name|lfs_ifind
argument_list|(
name|fs
argument_list|,
name|ino
argument_list|,
name|bp
operator|->
name|b_un
operator|.
name|b_dino
argument_list|)
expr_stmt|;
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
comment|/* 	 * Initialize the associated vnode 	 */
name|vp
operator|=
name|ITOV
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|vp
operator|->
name|v_type
operator|=
name|IFTOVT
argument_list|(
name|ip
operator|->
name|i_mode
argument_list|)
expr_stmt|;
if|if
condition|(
name|vp
operator|->
name|v_type
operator|==
name|VFIFO
condition|)
block|{
ifdef|#
directive|ifdef
name|FIFO
specifier|extern
name|struct
name|vnodeops
name|fifo_inodeops
decl_stmt|;
name|vp
operator|->
name|v_op
operator|=
operator|&
name|fifo_inodeops
expr_stmt|;
else|#
directive|else
name|iput
argument_list|(
name|ip
argument_list|)
expr_stmt|;
operator|*
name|ipp
operator|=
literal|0
expr_stmt|;
return|return
operator|(
name|EOPNOTSUPP
operator|)
return|;
endif|#
directive|endif
comment|/* FIFO */
block|}
if|if
condition|(
name|vp
operator|->
name|v_type
operator|==
name|VCHR
operator|||
name|vp
operator|->
name|v_type
operator|==
name|VBLK
condition|)
block|{
name|vp
operator|->
name|v_op
operator|=
operator|&
name|spec_inodeops
expr_stmt|;
if|if
condition|(
name|nvp
operator|=
name|checkalias
argument_list|(
name|vp
argument_list|,
name|ip
operator|->
name|i_rdev
argument_list|,
name|mntp
argument_list|)
condition|)
block|{
comment|/* 			 * Reinitialize aliased inode. 			 */
name|vp
operator|=
name|nvp
expr_stmt|;
name|iq
operator|=
name|VTOI
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|iq
operator|->
name|i_vnode
operator|=
name|vp
expr_stmt|;
name|iq
operator|->
name|i_flag
operator|=
literal|0
expr_stmt|;
name|ILOCK
argument_list|(
name|iq
argument_list|)
expr_stmt|;
name|iq
operator|->
name|i_din
operator|=
name|ip
operator|->
name|i_din
expr_stmt|;
name|iq
operator|->
name|i_dev
operator|=
name|dev
expr_stmt|;
name|iq
operator|->
name|i_number
operator|=
name|ino
expr_stmt|;
name|insque
argument_list|(
name|iq
argument_list|,
name|ih
argument_list|)
expr_stmt|;
comment|/* 			 * Discard unneeded vnode 			 */
name|ip
operator|->
name|i_mode
operator|=
literal|0
expr_stmt|;
name|iput
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|ip
operator|=
name|iq
expr_stmt|;
block|}
block|}
if|if
condition|(
name|ino
operator|==
name|ROOTINO
condition|)
name|vp
operator|->
name|v_flag
operator||=
name|VROOT
expr_stmt|;
name|VREF
argument_list|(
name|ip
operator|->
name|i_devvp
argument_list|)
expr_stmt|;
operator|*
name|ipp
operator|=
name|ip
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Last reference to an inode, write the inode out and if necessary,  * truncate and deallocate the file.  */
end_comment

begin_macro
name|lfs_inactive
argument_list|(
argument|vp
argument_list|,
argument|p
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|inode
modifier|*
name|ip
init|=
name|VTOI
argument_list|(
name|vp
argument_list|)
decl_stmt|;
name|int
name|mode
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|;
name|printf
argument_list|(
literal|"lfs_inactive: ino %d mode %d nlink %d\n"
argument_list|,
name|ip
operator|->
name|i_number
argument_list|,
name|ip
operator|->
name|i_mode
argument_list|,
name|ip
operator|->
name|i_nlink
argument_list|)
expr_stmt|;
if|if
condition|(
name|prtactive
operator|&&
name|vp
operator|->
name|v_usecount
operator|!=
literal|0
condition|)
name|vprint
argument_list|(
literal|"ufs_inactive: pushing active"
argument_list|,
name|vp
argument_list|)
expr_stmt|;
comment|/* 	 * Get rid of inodes related to stale file handles. 	 */
if|if
condition|(
name|ip
operator|->
name|i_mode
operator|==
literal|0
condition|)
block|{
if|if
condition|(
operator|(
name|vp
operator|->
name|v_flag
operator|&
name|VXLOCK
operator|)
operator|==
literal|0
condition|)
name|vgone
argument_list|(
name|vp
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|ILOCK
argument_list|(
name|ip
argument_list|)
expr_stmt|;
if|if
condition|(
name|ip
operator|->
name|i_nlink
operator|<=
literal|0
operator|&&
operator|(
name|vp
operator|->
name|v_mount
operator|->
name|mnt_flag
operator|&
name|MNT_RDONLY
operator|)
operator|==
literal|0
condition|)
block|{
ifdef|#
directive|ifdef
name|QUOTA
if|if
condition|(
operator|!
name|getinoquota
argument_list|(
name|ip
argument_list|)
condition|)
operator|(
name|void
operator|)
name|chkiq
argument_list|(
name|ip
argument_list|,
operator|-
literal|1
argument_list|,
name|NOCRED
argument_list|,
literal|0
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|error
operator|=
name|lfs_itrunc
argument_list|(
name|ip
argument_list|,
operator|(
name|u_long
operator|)
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* LFS */
name|mode
operator|=
name|ip
operator|->
name|i_mode
expr_stmt|;
name|ip
operator|->
name|i_mode
operator|=
literal|0
expr_stmt|;
name|ip
operator|->
name|i_rdev
operator|=
literal|0
expr_stmt|;
name|ip
operator|->
name|i_flag
operator||=
name|IUPD
operator||
name|ICHG
expr_stmt|;
ifdef|#
directive|ifdef
name|NOTLFS
comment|/* LFS */
name|ifree
argument_list|(
name|ip
argument_list|,
name|ip
operator|->
name|i_number
argument_list|,
name|mode
argument_list|)
expr_stmt|;
else|#
directive|else
name|lfs_ifree
argument_list|(
name|ip
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
name|ITIMES
argument_list|(
name|ip
argument_list|,
operator|&
name|time
argument_list|,
operator|&
name|time
argument_list|)
expr_stmt|;
name|IUNLOCK
argument_list|(
name|ip
argument_list|)
expr_stmt|;
name|ip
operator|->
name|i_flag
operator|=
literal|0
expr_stmt|;
comment|/* 	 * If we are done with the inode, reclaim it 	 * so that it can be reused immediately. 	 */
if|if
condition|(
name|vp
operator|->
name|v_usecount
operator|==
literal|0
operator|&&
name|ip
operator|->
name|i_mode
operator|==
literal|0
condition|)
name|vgone
argument_list|(
name|vp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_define
define|#
directive|define
name|SINGLE
value|0
end_define

begin_comment
comment|/* index of single indirect block */
end_comment

begin_define
define|#
directive|define
name|DOUBLE
value|1
end_define

begin_comment
comment|/* index of double indirect block */
end_comment

begin_define
define|#
directive|define
name|TRIPLE
value|2
end_define

begin_comment
comment|/* index of triple indirect block */
end_comment

begin_comment
comment|/*  * Truncate the inode ip to at most length size.  Free affected disk  * blocks -- the blocks of the file are removed in reverse order.  *  * NB: triple indirect blocks are untested.  */
end_comment

begin_expr_stmt
name|lfs_itrunc
argument_list|(
name|oip
argument_list|,
name|length
argument_list|,
name|flags
argument_list|)
specifier|register
expr|struct
name|inode
operator|*
name|oip
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|u_long
name|length
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|flags
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|daddr_t
name|lastblock
decl_stmt|;
name|daddr_t
name|bn
decl_stmt|,
name|lbn
decl_stmt|,
name|lastiblock
index|[
name|NIADDR
index|]
decl_stmt|;
specifier|register
name|LFS
modifier|*
name|fs
decl_stmt|;
comment|/* LFS */
specifier|register
name|struct
name|inode
modifier|*
name|ip
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|int
name|offset
decl_stmt|,
name|osize
decl_stmt|,
name|size
decl_stmt|,
name|level
decl_stmt|;
name|long
name|count
decl_stmt|,
name|nblocks
decl_stmt|,
name|blocksreleased
init|=
literal|0
decl_stmt|;
specifier|register
name|int
name|i
decl_stmt|;
name|int
name|aflags
decl_stmt|,
name|error
decl_stmt|,
name|allerror
decl_stmt|;
name|struct
name|inode
name|tip
decl_stmt|;
name|vnode_pager_setsize
argument_list|(
name|ITOV
argument_list|(
name|oip
argument_list|)
argument_list|,
name|length
argument_list|)
expr_stmt|;
if|if
condition|(
name|oip
operator|->
name|i_size
operator|<=
name|length
condition|)
block|{
name|oip
operator|->
name|i_flag
operator||=
name|ICHG
operator||
name|IUPD
expr_stmt|;
name|ITIMES
argument_list|(
name|oip
argument_list|,
operator|&
name|time
argument_list|,
operator|&
name|time
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/* 	 * Calculate index into inode's block list of 	 * last direct and indirect blocks (if any) 	 * which we want to keep.  Lastblock is -1 when 	 * the file is truncated to 0. 	 */
name|fs
operator|=
name|oip
operator|->
name|i_lfs
expr_stmt|;
comment|/* LFS */
name|lastblock
operator|=
name|lblkno
argument_list|(
name|fs
argument_list|,
name|length
operator|+
name|fs
operator|->
name|lfs_bsize
operator|-
literal|1
argument_list|)
operator|-
literal|1
expr_stmt|;
comment|/* LFS */
name|lastiblock
index|[
name|SINGLE
index|]
operator|=
name|lastblock
operator|-
name|NDADDR
expr_stmt|;
name|lastiblock
index|[
name|DOUBLE
index|]
operator|=
name|lastiblock
index|[
name|SINGLE
index|]
operator|-
name|NINDIR
argument_list|(
name|fs
argument_list|)
expr_stmt|;
name|lastiblock
index|[
name|TRIPLE
index|]
operator|=
name|lastiblock
index|[
name|DOUBLE
index|]
operator|-
name|NINDIR
argument_list|(
name|fs
argument_list|)
operator|*
name|NINDIR
argument_list|(
name|fs
argument_list|)
expr_stmt|;
name|nblocks
operator|=
name|btodb
argument_list|(
name|fs
operator|->
name|lfs_bsize
argument_list|)
expr_stmt|;
comment|/* LFS */
comment|/* 	 * Update the size of the file. If the file is not being 	 * truncated to a block boundry, the contents of the 	 * partial block following the end of the file must be 	 * zero'ed in case it ever become accessable again because 	 * of subsequent file growth. 	 */
name|osize
operator|=
name|oip
operator|->
name|i_size
expr_stmt|;
name|offset
operator|=
name|blkoff
argument_list|(
name|fs
argument_list|,
name|length
argument_list|)
expr_stmt|;
if|if
condition|(
name|offset
operator|==
literal|0
condition|)
block|{
name|oip
operator|->
name|i_size
operator|=
name|length
expr_stmt|;
block|}
else|else
block|{
name|lbn
operator|=
name|lblkno
argument_list|(
name|fs
argument_list|,
name|length
argument_list|)
expr_stmt|;
name|aflags
operator|=
name|B_CLRBUF
expr_stmt|;
if|if
condition|(
name|flags
operator|&
name|IO_SYNC
condition|)
name|aflags
operator||=
name|B_SYNC
expr_stmt|;
ifdef|#
directive|ifdef
name|QUOTA
if|if
condition|(
name|error
operator|=
name|getinoquota
argument_list|(
name|oip
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
endif|#
directive|endif
if|if
condition|(
name|error
operator|=
name|bread
argument_list|(
name|ITOV
argument_list|(
name|oip
argument_list|)
argument_list|,
name|lbn
argument_list|,
name|fs
operator|->
name|lfs_bsize
argument_list|,
name|NOCRED
argument_list|,
operator|&
name|bp
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|oip
operator|->
name|i_size
operator|=
name|length
expr_stmt|;
name|size
operator|=
name|blksize
argument_list|(
name|fs
argument_list|)
expr_stmt|;
comment|/* LFS */
operator|(
name|void
operator|)
name|vnode_pager_uncache
argument_list|(
name|ITOV
argument_list|(
name|oip
argument_list|)
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|bp
operator|->
name|b_un
operator|.
name|b_addr
operator|+
name|offset
argument_list|,
call|(
name|unsigned
call|)
argument_list|(
name|size
operator|-
name|offset
argument_list|)
argument_list|)
expr_stmt|;
name|allocbuf
argument_list|(
name|bp
argument_list|,
name|size
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|NOTLFS
if|if
condition|(
name|flags
operator|&
name|IO_SYNC
condition|)
comment|/* LFS */
name|bwrite
argument_list|(
name|bp
argument_list|)
expr_stmt|;
else|else
name|bdwrite
argument_list|(
name|bp
argument_list|)
expr_stmt|;
else|#
directive|else
name|lfs_bwrite
argument_list|(
name|bp
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
comment|/* 	 * Update file and block pointers 	 * on disk before we start freeing blocks. 	 * If we crash before free'ing blocks below, 	 * the blocks will be returned to the free list. 	 * lastiblock values are also normalized to -1 	 * for calls to indirtrunc below. 	 */
comment|/* Will need to modify the segment usage information */
comment|/* LFS */
name|tip
operator|=
operator|*
name|oip
expr_stmt|;
name|tip
operator|.
name|i_size
operator|=
name|osize
expr_stmt|;
for|for
control|(
name|level
operator|=
name|TRIPLE
init|;
name|level
operator|>=
name|SINGLE
condition|;
name|level
operator|--
control|)
if|if
condition|(
name|lastiblock
index|[
name|level
index|]
operator|<
literal|0
condition|)
block|{
name|oip
operator|->
name|i_ib
index|[
name|level
index|]
operator|=
literal|0
expr_stmt|;
name|lastiblock
index|[
name|level
index|]
operator|=
operator|-
literal|1
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
name|NDADDR
operator|-
literal|1
init|;
name|i
operator|>
name|lastblock
condition|;
name|i
operator|--
control|)
name|oip
operator|->
name|i_db
index|[
name|i
index|]
operator|=
literal|0
expr_stmt|;
name|oip
operator|->
name|i_flag
operator||=
name|ICHG
operator||
name|IUPD
expr_stmt|;
ifdef|#
directive|ifdef
name|NOTLFS
name|vinvalbuf
argument_list|(
name|ITOV
argument_list|(
name|oip
argument_list|)
argument_list|,
operator|(
name|length
operator|>
literal|0
operator|)
argument_list|)
expr_stmt|;
name|allerror
operator|=
name|ITIMES
argument_list|(
name|oip
argument_list|,
operator|&
name|time
argument_list|,
operator|&
name|time
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* Need lfs_vinvalbuf to get rid of invalid buffers in the cache */
name|ITIMES
argument_list|(
name|oip
argument_list|,
operator|&
name|time
argument_list|,
operator|&
name|time
argument_list|)
expr_stmt|;
name|allerror
operator|=
literal|0
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|NOTLFS
comment|/* 	 * Indirect blocks first. 	 */
name|ip
operator|=
operator|&
name|tip
expr_stmt|;
for|for
control|(
name|level
operator|=
name|TRIPLE
init|;
name|level
operator|>=
name|SINGLE
condition|;
name|level
operator|--
control|)
block|{
name|bn
operator|=
name|ip
operator|->
name|i_ib
index|[
name|level
index|]
expr_stmt|;
if|if
condition|(
name|bn
operator|!=
literal|0
condition|)
block|{
name|error
operator|=
name|indirtrunc
argument_list|(
name|ip
argument_list|,
name|bn
argument_list|,
name|lastiblock
index|[
name|level
index|]
argument_list|,
name|level
argument_list|,
operator|&
name|count
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
name|allerror
operator|=
name|error
expr_stmt|;
name|blocksreleased
operator|+=
name|count
expr_stmt|;
if|if
condition|(
name|lastiblock
index|[
name|level
index|]
operator|<
literal|0
condition|)
block|{
name|ip
operator|->
name|i_ib
index|[
name|level
index|]
operator|=
literal|0
expr_stmt|;
name|blkfree
argument_list|(
name|ip
argument_list|,
name|bn
argument_list|,
operator|(
name|off_t
operator|)
name|fs
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
name|blocksreleased
operator|+=
name|nblocks
expr_stmt|;
block|}
block|}
if|if
condition|(
name|lastiblock
index|[
name|level
index|]
operator|>=
literal|0
condition|)
goto|goto
name|done
goto|;
block|}
else|#
directive|else
comment|/* LFS -- not yet implemented.  Need to rewrite indirect blocks */
name|panic
argument_list|(
literal|"lfs_itrunc: not yet implemented"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* 	 * All whole direct blocks or frags. 	 */
for|for
control|(
name|i
operator|=
name|NDADDR
operator|-
literal|1
init|;
name|i
operator|>
name|lastblock
condition|;
name|i
operator|--
control|)
block|{
specifier|register
name|off_t
name|bsize
decl_stmt|;
name|bn
operator|=
name|ip
operator|->
name|i_db
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|bn
operator|==
literal|0
condition|)
continue|continue;
name|ip
operator|->
name|i_db
index|[
name|i
index|]
operator|=
literal|0
expr_stmt|;
name|bsize
operator|=
operator|(
name|off_t
operator|)
name|blksize
argument_list|(
name|fs
argument_list|)
expr_stmt|;
comment|/* LFS */
ifdef|#
directive|ifdef
name|NOTLFS
name|blkfree
argument_list|(
name|ip
argument_list|,
name|bn
argument_list|,
name|bsize
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* LFS Update segment usage information */
endif|#
directive|endif
name|blocksreleased
operator|+=
name|btodb
argument_list|(
name|bsize
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|lastblock
operator|<
literal|0
condition|)
goto|goto
name|done
goto|;
comment|/* 	 * Finally, look for a change in size of the 	 * last direct block; release any frags. 	 */
name|bn
operator|=
name|ip
operator|->
name|i_db
index|[
name|lastblock
index|]
expr_stmt|;
if|if
condition|(
name|bn
operator|!=
literal|0
condition|)
block|{
name|off_t
name|oldspace
decl_stmt|,
name|newspace
decl_stmt|;
comment|/* 		 * Calculate amount of space we're giving 		 * back as old block size minus new block size. 		 */
name|oldspace
operator|=
name|blksize
argument_list|(
name|fs
argument_list|)
expr_stmt|;
comment|/* LFS */
name|ip
operator|->
name|i_size
operator|=
name|length
expr_stmt|;
name|newspace
operator|=
name|blksize
argument_list|(
name|fs
argument_list|)
expr_stmt|;
comment|/* LFS */
if|if
condition|(
name|newspace
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"lfs_itrunc: newspace"
argument_list|)
expr_stmt|;
if|if
condition|(
name|oldspace
operator|-
name|newspace
operator|>
literal|0
condition|)
block|{
comment|/* 			 * Block number of space to be free'd is 			 * the old block # plus the number of frags 			 * required for the storage we're keeping. 			 */
name|bn
operator|+=
name|numfrags
argument_list|(
name|fs
argument_list|,
name|newspace
argument_list|)
expr_stmt|;
name|blkfree
argument_list|(
name|ip
argument_list|,
name|bn
argument_list|,
name|oldspace
operator|-
name|newspace
argument_list|)
expr_stmt|;
name|blocksreleased
operator|+=
name|btodb
argument_list|(
name|oldspace
operator|-
name|newspace
argument_list|)
expr_stmt|;
block|}
block|}
name|done
label|:
comment|/* BEGIN PARANOIA */
for|for
control|(
name|level
operator|=
name|SINGLE
init|;
name|level
operator|<=
name|TRIPLE
condition|;
name|level
operator|++
control|)
if|if
condition|(
name|ip
operator|->
name|i_ib
index|[
name|level
index|]
operator|!=
name|oip
operator|->
name|i_ib
index|[
name|level
index|]
condition|)
name|panic
argument_list|(
literal|"lfs_itrunc1"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NDADDR
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|ip
operator|->
name|i_db
index|[
name|i
index|]
operator|!=
name|oip
operator|->
name|i_db
index|[
name|i
index|]
condition|)
name|panic
argument_list|(
literal|"lfs_itrunc2"
argument_list|)
expr_stmt|;
comment|/* END PARANOIA */
name|oip
operator|->
name|i_blocks
operator|-=
name|blocksreleased
expr_stmt|;
if|if
condition|(
name|oip
operator|->
name|i_blocks
operator|<
literal|0
condition|)
comment|/* sanity */
name|oip
operator|->
name|i_blocks
operator|=
literal|0
expr_stmt|;
name|oip
operator|->
name|i_flag
operator||=
name|ICHG
expr_stmt|;
ifdef|#
directive|ifdef
name|QUOTA
if|if
condition|(
operator|!
name|getinoquota
argument_list|(
name|oip
argument_list|)
condition|)
operator|(
name|void
operator|)
name|chkdq
argument_list|(
name|oip
argument_list|,
operator|-
name|blocksreleased
argument_list|,
name|NOCRED
argument_list|,
literal|0
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
operator|(
name|allerror
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Release blocks associated with the inode ip and  * stored in the indirect block bn.  Blocks are free'd  * in LIFO order up to (but not including) lastbn.  If  * level is greater than SINGLE, the block is an indirect  * block and recursive calls to indirtrunc must be used to  * cleanse other indirect blocks.  *  * NB: triple indirect blocks are untested.  */
end_comment

begin_expr_stmt
name|lfs_indirtrunc
argument_list|(
name|ip
argument_list|,
name|bn
argument_list|,
name|lastbn
argument_list|,
name|level
argument_list|,
name|countp
argument_list|)
specifier|register
expr|struct
name|inode
operator|*
name|ip
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|daddr_t
name|bn
decl_stmt|,
name|lastbn
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|level
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
modifier|*
name|countp
decl_stmt|;
end_decl_stmt

begin_block
block|{
ifdef|#
directive|ifdef
name|NOTLFS
specifier|register
name|int
name|i
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
specifier|register
name|struct
name|fs
modifier|*
name|fs
init|=
name|ip
operator|->
name|i_fs
decl_stmt|;
specifier|register
name|daddr_t
modifier|*
name|bap
decl_stmt|;
name|daddr_t
modifier|*
name|copy
decl_stmt|,
name|nb
decl_stmt|,
name|last
decl_stmt|;
name|long
name|blkcount
decl_stmt|,
name|factor
decl_stmt|;
name|int
name|nblocks
decl_stmt|,
name|blocksreleased
init|=
literal|0
decl_stmt|;
name|int
name|error
decl_stmt|,
name|allerror
init|=
literal|0
decl_stmt|;
comment|/* 	 * Calculate index in current block of last 	 * block to be kept.  -1 indicates the entire 	 * block so we need not calculate the index. 	 */
name|factor
operator|=
literal|1
expr_stmt|;
for|for
control|(
name|i
operator|=
name|SINGLE
init|;
name|i
operator|<
name|level
condition|;
name|i
operator|++
control|)
name|factor
operator|*=
name|NINDIR
argument_list|(
name|fs
argument_list|)
expr_stmt|;
name|last
operator|=
name|lastbn
expr_stmt|;
if|if
condition|(
name|lastbn
operator|>
literal|0
condition|)
name|last
operator|/=
name|factor
expr_stmt|;
name|nblocks
operator|=
name|btodb
argument_list|(
name|fs
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
comment|/* 	 * Get buffer of block pointers, zero those  	 * entries corresponding to blocks to be free'd, 	 * and update on disk copy first. 	 */
ifdef|#
directive|ifdef
name|SECSIZE
name|bp
operator|=
name|bread
argument_list|(
name|ip
operator|->
name|i_dev
argument_list|,
name|fsbtodb
argument_list|(
name|fs
argument_list|,
name|bn
argument_list|)
argument_list|,
operator|(
name|int
operator|)
name|fs
operator|->
name|fs_bsize
argument_list|,
name|fs
operator|->
name|fs_dbsize
argument_list|)
expr_stmt|;
else|#
directive|else
else|SECSIZE
name|error
operator|=
name|bread
argument_list|(
name|ip
operator|->
name|i_devvp
argument_list|,
name|fsbtodb
argument_list|(
name|fs
argument_list|,
name|bn
argument_list|)
argument_list|,
operator|(
name|int
operator|)
name|fs
operator|->
name|fs_bsize
argument_list|,
name|NOCRED
argument_list|,
operator|&
name|bp
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
operator|*
name|countp
operator|=
literal|0
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|bap
operator|=
name|bp
operator|->
name|b_un
operator|.
name|b_daddr
expr_stmt|;
name|MALLOC
argument_list|(
name|copy
argument_list|,
name|daddr_t
operator|*
argument_list|,
name|fs
operator|->
name|fs_bsize
argument_list|,
name|M_TEMP
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|bap
argument_list|,
operator|(
name|caddr_t
operator|)
name|copy
argument_list|,
operator|(
name|u_int
operator|)
name|fs
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|bap
index|[
name|last
operator|+
literal|1
index|]
argument_list|,
call|(
name|u_int
call|)
argument_list|(
name|NINDIR
argument_list|(
name|fs
argument_list|)
operator|-
operator|(
name|last
operator|+
literal|1
operator|)
argument_list|)
operator|*
sizeof|sizeof
argument_list|(
name|daddr_t
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|last
operator|==
operator|-
literal|1
condition|)
name|bp
operator|->
name|b_flags
operator||=
name|B_INVAL
expr_stmt|;
name|error
operator|=
name|bwrite
argument_list|(
name|bp
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
name|allerror
operator|=
name|error
expr_stmt|;
name|bap
operator|=
name|copy
expr_stmt|;
comment|/* 	 * Recursively free totally unused blocks. 	 */
for|for
control|(
name|i
operator|=
name|NINDIR
argument_list|(
name|fs
argument_list|)
operator|-
literal|1
init|;
name|i
operator|>
name|last
condition|;
name|i
operator|--
control|)
block|{
name|nb
operator|=
name|bap
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|nb
operator|==
literal|0
condition|)
continue|continue;
if|if
condition|(
name|level
operator|>
name|SINGLE
condition|)
block|{
name|error
operator|=
name|indirtrunc
argument_list|(
name|ip
argument_list|,
name|nb
argument_list|,
operator|(
name|daddr_t
operator|)
operator|-
literal|1
argument_list|,
name|level
operator|-
literal|1
argument_list|,
operator|&
name|blkcount
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
name|allerror
operator|=
name|error
expr_stmt|;
name|blocksreleased
operator|+=
name|blkcount
expr_stmt|;
block|}
name|blkfree
argument_list|(
name|ip
argument_list|,
name|nb
argument_list|,
operator|(
name|off_t
operator|)
name|fs
operator|->
name|fs_bsize
argument_list|)
expr_stmt|;
name|blocksreleased
operator|+=
name|nblocks
expr_stmt|;
block|}
comment|/* 	 * Recursively free last partial block. 	 */
if|if
condition|(
name|level
operator|>
name|SINGLE
operator|&&
name|lastbn
operator|>=
literal|0
condition|)
block|{
name|last
operator|=
name|lastbn
operator|%
name|factor
expr_stmt|;
name|nb
operator|=
name|bap
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|nb
operator|!=
literal|0
condition|)
block|{
name|error
operator|=
name|indirtrunc
argument_list|(
name|ip
argument_list|,
name|nb
argument_list|,
name|last
argument_list|,
name|level
operator|-
literal|1
argument_list|,
operator|&
name|blkcount
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
name|allerror
operator|=
name|error
expr_stmt|;
name|blocksreleased
operator|+=
name|blkcount
expr_stmt|;
block|}
block|}
name|FREE
argument_list|(
name|copy
argument_list|,
name|M_TEMP
argument_list|)
expr_stmt|;
operator|*
name|countp
operator|=
name|blocksreleased
expr_stmt|;
return|return
operator|(
name|allerror
operator|)
return|;
else|#
directive|else
comment|/* LFS IMPLEMENT -- lfs_indirtrunc */
name|panic
argument_list|(
literal|"lfs_indirtrunc not implemented"
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
end_block

end_unit

