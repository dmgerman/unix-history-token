begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	if_ace.c	1.1	85/07/21	*/
end_comment

begin_comment
comment|/*  * ACC VERSAbus Ethernet controller  */
end_comment

begin_include
include|#
directive|include
file|"ace.h"
end_include

begin_if
if|#
directive|if
name|NACE
operator|>
literal|0
end_if

begin_include
include|#
directive|include
file|"../machine/pte.h"
end_include

begin_include
include|#
directive|include
file|"../h/param.h"
end_include

begin_include
include|#
directive|include
file|"../h/systm.h"
end_include

begin_include
include|#
directive|include
file|"../h/mbuf.h"
end_include

begin_include
include|#
directive|include
file|"../h/buf.h"
end_include

begin_include
include|#
directive|include
file|"../h/protosw.h"
end_include

begin_include
include|#
directive|include
file|"../h/socket.h"
end_include

begin_include
include|#
directive|include
file|"../h/vmmac.h"
end_include

begin_include
include|#
directive|include
file|"../h/ioctl.h"
end_include

begin_include
include|#
directive|include
file|"../h/errno.h"
end_include

begin_include
include|#
directive|include
file|"../h/vmparam.h"
end_include

begin_include
include|#
directive|include
file|"../net/if.h"
end_include

begin_include
include|#
directive|include
file|"../net/netisr.h"
end_include

begin_include
include|#
directive|include
file|"../net/route.h"
end_include

begin_include
include|#
directive|include
file|"../netinet/in.h"
end_include

begin_include
include|#
directive|include
file|"../netinet/in_systm.h"
end_include

begin_include
include|#
directive|include
file|"../netinet/ip.h"
end_include

begin_include
include|#
directive|include
file|"../netinet/ip_var.h"
end_include

begin_include
include|#
directive|include
file|"../netinet/if_ether.h"
end_include

begin_include
include|#
directive|include
file|"../netpup/pup.h"
end_include

begin_include
include|#
directive|include
file|"../tahoe/mtpr.h"
end_include

begin_include
include|#
directive|include
file|"../tahoeif/if_acereg.h"
end_include

begin_include
include|#
directive|include
file|"../vba/vbavar.h"
end_include

begin_define
define|#
directive|define
name|LONET
value|124
end_define

begin_comment
comment|/*  * Configuration table, for 2 units (should be defined by config)  */
end_comment

begin_define
define|#
directive|define
name|ACEVECTOR
value|0x90
end_define

begin_decl_stmt
name|long
name|acestd
index|[]
init|=
block|{
literal|0x0ff0000
block|,
literal|0xff0100
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* controller */
end_comment

begin_decl_stmt
specifier|extern
name|char
name|ace0utl
index|[]
decl_stmt|,
name|ace1utl
index|[]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* dpm */
end_comment

begin_decl_stmt
name|char
modifier|*
name|acemap
index|[]
init|=
block|{
name|ace0utl
block|,
name|ace1utl
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|long
name|ACE0map
index|[]
decl_stmt|,
name|ACE1map
index|[]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
modifier|*
name|ACEmap
index|[]
init|=
block|{
name|ACE0map
block|,
name|ACE1map
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
name|ACEmapa
index|[]
init|=
block|{
literal|0xfff80000
block|,
literal|0xfff90000
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* station address */
end_comment

begin_decl_stmt
name|char
name|ace_station
index|[
literal|6
index|]
init|=
block|{
operator|~
literal|0x8
block|,
operator|~
literal|0x0
block|,
operator|~
literal|0x3
block|,
operator|~
literal|0x0
block|,
operator|~
literal|0x0
block|,
operator|~
literal|0x1
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* multicast hash table initializer */
end_comment

begin_decl_stmt
name|char
name|ace_hash
index|[
literal|8
index|]
init|=
block|{
operator|~
literal|0xF
block|,
operator|~
literal|0xF
block|,
operator|~
literal|0xF
block|,
operator|~
literal|0xF
block|,
operator|~
literal|0xF
block|,
operator|~
literal|0xF
block|,
operator|~
literal|0xF
block|,
operator|~
literal|0xF
block|}
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* backoff table masks */
end_comment

begin_decl_stmt
name|short
name|random_mask_tbl
index|[
literal|16
index|]
init|=
block|{
literal|0x0040
block|,
literal|0x00C0
block|,
literal|0x01C0
block|,
literal|0x03C0
block|,
literal|0x07C0
block|,
literal|0x0FC0
block|,
literal|0x1FC0
block|,
literal|0x3FC0
block|,
literal|0x7FC0
block|,
literal|0xFFC0
block|,
literal|0xFFC0
block|,
literal|0xFFC0
block|,
literal|0xFFC0
block|,
literal|0xFFC0
block|,
literal|0xFFC0
block|,
literal|0xFFC0
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|aceprobe
argument_list|()
decl_stmt|,
name|aceattach
argument_list|()
decl_stmt|,
name|acerint
argument_list|()
decl_stmt|,
name|acecint
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|vba_device
modifier|*
name|aceinfo
index|[
name|NACE
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|vba_driver
name|acedriver
init|=
block|{
name|aceprobe
block|,
literal|0
block|,
name|aceattach
block|,
literal|0
block|,
name|acestd
block|,
literal|"ace"
block|,
name|aceinfo
block|,
literal|"v/eiu"
block|,
literal|0
block|}
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|ACEUNIT
parameter_list|(
name|x
parameter_list|)
value|minor(x)
end_define

begin_decl_stmt
name|int
name|aceinit
argument_list|()
decl_stmt|,
name|aceoutput
argument_list|()
decl_stmt|,
name|aceioctl
argument_list|()
decl_stmt|,
name|acereset
argument_list|()
decl_stmt|;
end_decl_stmt

begin_function_decl
name|struct
name|mbuf
modifier|*
name|aceget
parameter_list|()
function_decl|;
end_function_decl

begin_comment
comment|/*  * Ethernet software status per interface.  *  * Each interface is referenced by a network interface structure,  * is_if, which the routing code uses to locate the interface.  * This structure contains the output queue for the interface, its address, ...  */
end_comment

begin_struct
struct|struct
name|ace_softc
block|{
name|struct
name|arpcom
name|is_ac
decl_stmt|;
comment|/* Ethernet common part	*/
define|#
directive|define
name|is_if
value|is_ac.ac_if
comment|/* network-visible interface */
define|#
directive|define
name|is_addr
value|is_ac.ac_enaddr
comment|/* hardware Ethernet address */
name|char
modifier|*
name|is_dpm
decl_stmt|;
name|short
name|is_flags
decl_stmt|;
define|#
directive|define
name|ACEF_OACTIVE
value|0x1
comment|/* output is active */
define|#
directive|define
name|ACEF_RCVPENDING
value|0x2
comment|/* start rcv in acecint	*/
name|short
name|is_promiscuous
decl_stmt|;
comment|/* true is enabled */
name|short
name|is_segboundry
decl_stmt|;
comment|/* first TX Seg in dpm */
name|short
name|is_eictr
decl_stmt|;
comment|/* Rx segment tracking ctr */
name|short
name|is_eoctr
decl_stmt|;
comment|/* Tx segment tracking ctr */
name|short
name|is_txnext
decl_stmt|;
comment|/* Next available Tx segment */
name|short
name|is_currnd
decl_stmt|;
comment|/* current random backoff */
name|struct
name|ace_stats
name|is_stats
decl_stmt|;
comment|/* holds board statistics */
name|short
name|is_xcnt
decl_stmt|;
comment|/* count xmitted segments to be acked  					   by the controller */
block|}
name|ace_softc
index|[
name|NACE
index|]
struct|;
end_struct

begin_decl_stmt
specifier|extern
name|struct
name|ifnet
name|loif
decl_stmt|;
end_decl_stmt

begin_macro
name|aceprobe
argument_list|(
argument|reg
argument_list|)
end_macro

begin_decl_stmt
name|caddr_t
name|reg
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|acedevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|reg
decl_stmt|;
ifdef|#
directive|ifdef
name|lint
name|acerint
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|acecint
argument_list|(
literal|0
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|badaddr
argument_list|(
name|reg
argument_list|,
literal|2
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|movew
argument_list|(
operator|(
name|short
operator|)
name|CSR_RESET
argument_list|,
operator|&
name|addr
operator|->
name|csr
argument_list|)
expr_stmt|;
name|DELAY
argument_list|(
literal|10000
argument_list|)
expr_stmt|;
return|return
operator|(
sizeof|sizeof
argument_list|(
expr|struct
name|acedevice
argument_list|)
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Interface exists: make available by filling in network interface  * record.  System will initialize the interface when it is ready  * to accept packets.  */
end_comment

begin_macro
name|aceattach
argument_list|(
argument|ui
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|vba_device
modifier|*
name|ui
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|short
name|unit
init|=
name|ui
operator|->
name|ui_unit
decl_stmt|;
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
init|=
operator|&
name|is
operator|->
name|is_if
decl_stmt|;
specifier|register
name|struct
name|acedevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
decl_stmt|;
specifier|register
name|short
modifier|*
name|wp
decl_stmt|,
name|i
decl_stmt|;
name|struct
name|sockaddr_in
modifier|*
name|sin
decl_stmt|;
name|ifp
operator|->
name|if_unit
operator|=
name|unit
expr_stmt|;
name|ifp
operator|->
name|if_name
operator|=
literal|"ace"
expr_stmt|;
name|ifp
operator|->
name|if_mtu
operator|=
name|ETHERMTU
expr_stmt|;
comment|/* 	 * Set station's addresses, multicast 	 * hash table, and initialize dual ported memory. 	 */
name|ace_station
index|[
literal|5
index|]
operator|=
operator|~
operator|(
name|unit
operator|+
literal|1
operator|)
expr_stmt|;
name|acesetetaddr
argument_list|(
name|unit
argument_list|,
name|addr
argument_list|,
name|ace_station
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_promiscuous
operator|=
literal|0
expr_stmt|;
name|wp
operator|=
operator|(
name|short
operator|*
operator|)
name|addr
operator|->
name|hash
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|8
condition|;
name|i
operator|++
control|)
name|movew
argument_list|(
operator|(
name|short
operator|)
name|ace_hash
index|[
name|i
index|]
argument_list|,
name|wp
operator|++
argument_list|)
expr_stmt|;
name|movew
argument_list|(
operator|(
name|short
operator|)
operator|~
literal|0xffff
argument_list|,
operator|&
name|addr
operator|->
name|bcastena
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|movew
argument_list|(
operator|(
name|short
operator|)
operator|~
literal|0xffff
argument_list|,
operator|&
name|addr
operator|->
name|bcastena
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|aceclean
argument_list|(
name|unit
argument_list|)
expr_stmt|;
name|sin
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ifp
operator|->
name|if_addr
expr_stmt|;
name|sin
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|ifp
operator|->
name|if_init
operator|=
name|aceinit
expr_stmt|;
name|ifp
operator|->
name|if_output
operator|=
name|aceoutput
expr_stmt|;
name|ifp
operator|->
name|if_ioctl
operator|=
name|aceioctl
expr_stmt|;
name|ifp
operator|->
name|if_reset
operator|=
name|acereset
expr_stmt|;
name|if_attach
argument_list|(
name|ifp
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|acesetetaddr
argument_list|(
argument|unit
argument_list|,
argument|addr
argument_list|,
argument|station_addr
argument_list|)
end_macro

begin_decl_stmt
name|short
name|unit
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|acedevice
modifier|*
name|addr
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|station_addr
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|short
modifier|*
name|wp
decl_stmt|,
name|i
decl_stmt|;
specifier|register
name|char
modifier|*
name|cp
decl_stmt|;
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
name|wp
operator|=
operator|(
name|short
operator|*
operator|)
name|addr
operator|->
name|station
expr_stmt|;
name|cp
operator|=
name|station_addr
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|6
condition|;
name|i
operator|++
control|)
name|movew
argument_list|(
operator|(
name|short
operator|)
operator|*
name|cp
operator|++
argument_list|,
name|wp
operator|++
argument_list|)
expr_stmt|;
name|wp
operator|=
operator|(
name|short
operator|*
operator|)
name|addr
operator|->
name|station
expr_stmt|;
name|cp
operator|=
operator|(
name|char
operator|*
operator|)
operator|&
name|is
operator|->
name|is_addr
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|6
condition|;
name|i
operator|++
control|)
operator|*
name|cp
operator|++
operator|=
operator|~
operator|(
operator|*
name|wp
operator|++
operator|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Reset of interface after "system" reset.  */
end_comment

begin_macro
name|acereset
argument_list|(
argument|unit
argument_list|,
argument|vban
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|,
name|vban
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|vba_device
modifier|*
name|ui
decl_stmt|;
if|if
condition|(
name|unit
operator|>=
name|NACE
operator|||
operator|(
name|ui
operator|=
name|aceinfo
index|[
name|unit
index|]
operator|)
operator|==
literal|0
operator|||
name|ui
operator|->
name|ui_alive
operator|==
literal|0
operator|||
name|ui
operator|->
name|ui_vbanum
operator|!=
name|vban
condition|)
return|return;
name|printf
argument_list|(
literal|" ace%d"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
name|aceinit
argument_list|(
name|unit
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Initialization of interface; clear recorded pending operations  */
end_comment

begin_macro
name|aceinit
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|vba_device
modifier|*
name|ui
init|=
name|aceinfo
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|acedevice
modifier|*
name|addr
decl_stmt|;
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
init|=
operator|&
name|is
operator|->
name|is_if
decl_stmt|;
specifier|register
name|struct
name|sockaddr_in
modifier|*
name|sin
decl_stmt|;
specifier|register
name|short
name|Csr
decl_stmt|;
specifier|register
name|int
name|i
decl_stmt|,
name|s
decl_stmt|;
name|sin
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ifp
operator|->
name|if_addr
expr_stmt|;
if|if
condition|(
name|sin
operator|->
name|sin_addr
operator|.
name|s_addr
operator|==
literal|0
condition|)
comment|/* address still unknown */
return|return;
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_RUNNING
operator|)
operator|==
literal|0
condition|)
block|{
comment|/* 		 * Reset the controller, initialize the recieve buffers, 		 * and turn the controller on again and set board online. 		 */
name|addr
operator|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
expr_stmt|;
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
name|movew
argument_list|(
operator|(
name|short
operator|)
name|CSR_RESET
argument_list|,
operator|&
name|addr
operator|->
name|csr
argument_list|)
expr_stmt|;
name|DELAY
argument_list|(
literal|10000
argument_list|)
expr_stmt|;
comment|/* 		 * clean up dpm since the controller might 		 * jumble dpm after reset 		 */
name|aceclean
argument_list|(
name|unit
argument_list|)
expr_stmt|;
name|movew
argument_list|(
operator|(
name|short
operator|)
name|CSR_GO
argument_list|,
operator|&
name|addr
operator|->
name|csr
argument_list|)
expr_stmt|;
name|Csr
operator|=
name|addr
operator|->
name|csr
expr_stmt|;
if|if
condition|(
name|Csr
operator|&
name|CSR_ACTIVE
condition|)
block|{
name|movew
argument_list|(
call|(
name|short
call|)
argument_list|(
name|ACEVECTOR
operator|+
name|unit
operator|*
literal|8
argument_list|)
argument_list|,
operator|&
name|addr
operator|->
name|ivct
argument_list|)
expr_stmt|;
name|Csr
operator||=
name|CSR_IENA
operator||
name|is
operator|->
name|is_promiscuous
expr_stmt|;
if|if
condition|(
name|ifp
operator|->
name|if_net
operator|==
name|LONET
condition|)
name|Csr
operator||=
name|CSR_LOOP3
expr_stmt|;
name|movew
argument_list|(
name|Csr
argument_list|,
operator|&
name|addr
operator|->
name|csr
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_flags
operator|=
literal|0
expr_stmt|;
name|is
operator|->
name|is_xcnt
operator|=
literal|0
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_flags
operator||=
name|IFF_UP
operator||
name|IFF_RUNNING
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|is
operator|->
name|is_if
operator|.
name|if_flags
operator|&
name|IFF_UP
condition|)
block|{
name|if_rtinit
argument_list|(
operator|&
name|is
operator|->
name|is_if
argument_list|,
name|RTF_UP
argument_list|)
expr_stmt|;
name|aceStart
argument_list|(
name|unit
argument_list|)
expr_stmt|;
block|}
name|arpwhohas
argument_list|(
operator|&
name|is
operator|->
name|is_ac
argument_list|,
operator|&
name|sin
operator|->
name|sin_addr
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Start output on interface.  * Get another datagram to send off of the interface queue,  * and map it to the interface before starting the output.  *  */
end_comment

begin_macro
name|acestart
argument_list|(
argument|dev
argument_list|)
end_macro

begin_decl_stmt
name|dev_t
name|dev
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|tx_segment
modifier|*
name|txs
decl_stmt|;
specifier|register
name|long
name|len
decl_stmt|,
name|x
decl_stmt|;
name|int
name|unit
init|=
name|ACEUNIT
argument_list|(
name|dev
argument_list|)
decl_stmt|;
name|struct
name|vba_device
modifier|*
name|ui
init|=
name|aceinfo
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|acedevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
decl_stmt|;
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|short
name|retries
decl_stmt|,
name|idx
decl_stmt|;
name|again
label|:
name|txs
operator|=
operator|(
expr|struct
name|tx_segment
operator|*
operator|)
operator|(
name|is
operator|->
name|is_dpm
operator|+
operator|(
name|is
operator|->
name|is_txnext
operator|<<
literal|11
operator|)
operator|)
expr_stmt|;
if|if
condition|(
name|txs
operator|->
name|tx_csr
operator|&
name|TCS_TBFULL
condition|)
block|{
name|is
operator|->
name|is_stats
operator|.
name|tx_busy
operator|++
expr_stmt|;
return|return;
block|}
name|x
operator|=
name|splimp
argument_list|()
expr_stmt|;
name|IF_DEQUEUE
argument_list|(
operator|&
name|is
operator|->
name|is_if
operator|.
name|if_snd
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|x
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
return|return;
name|len
operator|=
name|aceput
argument_list|(
name|unit
argument_list|,
name|txs
operator|->
name|tx_data
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|retries
operator|=
name|txs
operator|->
name|tx_csr
operator|&
name|TCS_RTC
expr_stmt|;
if|if
condition|(
name|retries
operator|>
literal|0
condition|)
name|acebakoff
argument_list|(
name|is
argument_list|,
name|txs
argument_list|,
name|retries
argument_list|)
expr_stmt|;
comment|/* 	 * Ensure minimum packet length. 	 * This makes the safe assumtion that there are no virtual holes 	 * after the data. 	 * For security, it might be wise to zero out the added bytes, 	 * but we're mainly interested in speed at the moment. 	 */
ifdef|#
directive|ifdef
name|notdef
if|if
condition|(
name|len
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
operator|<
name|ETHERMIN
condition|)
name|len
operator|=
name|ETHERMIN
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
expr_stmt|;
else|#
directive|else
if|if
condition|(
name|len
operator|-
literal|14
operator|<
name|ETHERMIN
condition|)
name|len
operator|=
name|ETHERMIN
operator|+
literal|14
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
operator|++
name|is
operator|->
name|is_txnext
operator|>
name|SEG_MAX
condition|)
name|is
operator|->
name|is_txnext
operator|=
name|is
operator|->
name|is_segboundry
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_opackets
operator|++
expr_stmt|;
name|is
operator|->
name|is_xcnt
operator|++
expr_stmt|;
name|len
operator|=
operator|(
name|len
operator|&
literal|0x7fff
operator|)
operator||
name|TCS_TBFULL
expr_stmt|;
name|movew
argument_list|(
operator|(
name|short
operator|)
name|len
argument_list|,
name|txs
argument_list|)
expr_stmt|;
goto|goto
name|again
goto|;
block|}
end_block

begin_comment
comment|/*  * Transmit done interrupt.  */
end_comment

begin_macro
name|acecint
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
name|struct
name|vba_device
modifier|*
name|ui
init|=
name|aceinfo
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|acedevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
decl_stmt|;
specifier|register
name|struct
name|tx_segment
modifier|*
name|txseg
decl_stmt|;
name|short
name|txidx
decl_stmt|,
name|eostat
decl_stmt|;
if|if
condition|(
name|is
operator|->
name|is_xcnt
operator|<=
literal|0
condition|)
block|{
name|txidx
operator|=
operator|(
name|addr
operator|->
name|tseg
operator|>>
literal|11
operator|)
operator|&
literal|0xf
expr_stmt|;
name|printf
argument_list|(
literal|"ace%d: stray xmit interrupt, xcnt %d\n"
argument_list|,
name|unit
argument_list|,
name|is
operator|->
name|is_xcnt
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_xcnt
operator|=
literal|0
expr_stmt|;
name|aceStart
argument_list|(
name|unit
argument_list|)
expr_stmt|;
return|return;
block|}
name|is
operator|->
name|is_xcnt
operator|--
expr_stmt|;
name|txseg
operator|=
operator|(
expr|struct
name|tx_segment
operator|*
operator|)
operator|(
operator|(
name|is
operator|->
name|is_eoctr
operator|<<
literal|11
operator|)
operator|+
name|is
operator|->
name|is_dpm
operator|)
expr_stmt|;
name|eostat
operator|=
name|txseg
operator|->
name|tx_csr
expr_stmt|;
if|if
condition|(
operator|(
name|eostat
operator|&
name|TCS_TBFULL
operator|)
operator|==
literal|0
condition|)
block|{
name|is
operator|->
name|is_stats
operator|.
name|tx_retries
operator|+=
name|eostat
operator|&
name|TCS_RTC
expr_stmt|;
if|if
condition|(
name|eostat
operator|&
name|TCS_RTFAIL
condition|)
block|{
name|is
operator|->
name|is_stats
operator|.
name|tx_discarded
operator|++
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_oerrors
operator|++
expr_stmt|;
block|}
else|else
name|is
operator|->
name|is_stats
operator|.
name|tx_datagrams
operator|++
expr_stmt|;
if|if
condition|(
operator|++
name|is
operator|->
name|is_eoctr
operator|>=
literal|16
condition|)
name|is
operator|->
name|is_eoctr
operator|=
name|is
operator|->
name|is_segboundry
expr_stmt|;
block|}
name|aceStart
argument_list|(
name|unit
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Ethernet interface receiver interrupt.  * If input error just drop packet.  * Otherwise purge input buffered data path and examine   * packet to determine type.  If can't determine length  * from type, then have to drop packet.  Othewise decapsulate  * packet based on type and pass to type specific higher-level  * input routine.  */
end_comment

begin_macro
name|acerint
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|ifqueue
modifier|*
name|inq
decl_stmt|;
specifier|register
name|struct
name|ether_header
modifier|*
name|ace
decl_stmt|;
specifier|register
name|struct
name|rx_segment
modifier|*
name|rxseg
decl_stmt|;
name|struct
name|acedevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|aceinfo
index|[
name|unit
index|]
operator|->
name|ui_addr
decl_stmt|;
name|int
name|len
decl_stmt|,
name|s
decl_stmt|,
name|off
decl_stmt|,
name|resid
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|short
name|eistat
decl_stmt|;
name|again
label|:
name|rxseg
operator|=
operator|(
expr|struct
name|rx_segment
operator|*
operator|)
operator|(
operator|(
name|is
operator|->
name|is_eictr
operator|<<
literal|11
operator|)
operator|+
name|is
operator|->
name|is_dpm
operator|)
expr_stmt|;
name|eistat
operator|=
name|rxseg
operator|->
name|rx_csr
expr_stmt|;
if|if
condition|(
operator|(
name|eistat
operator|&
name|RCS_RBFULL
operator|)
operator|==
literal|0
condition|)
return|return;
name|is
operator|->
name|is_if
operator|.
name|if_ipackets
operator|++
expr_stmt|;
if|if
condition|(
operator|++
name|is
operator|->
name|is_eictr
operator|>=
name|is
operator|->
name|is_segboundry
condition|)
name|is
operator|->
name|is_eictr
operator|=
literal|0
expr_stmt|;
name|len
operator|=
name|eistat
operator|&
name|RCS_RBC
expr_stmt|;
if|if
condition|(
operator|(
name|eistat
operator|&
operator|(
name|RCS_ROVRN
operator||
name|RCS_RCRC
operator||
name|RCS_RODD
operator|)
operator|)
operator|||
name|len
operator|<
name|ET_MINLEN
operator|||
name|len
operator|>
name|ET_MAXLEN
operator|+
name|CRC_SIZE
condition|)
block|{
if|if
condition|(
name|eistat
operator|&
name|RCS_ROVRN
condition|)
name|is
operator|->
name|is_stats
operator|.
name|rx_overruns
operator|++
expr_stmt|;
if|if
condition|(
name|eistat
operator|&
name|RCS_RCRC
condition|)
name|is
operator|->
name|is_stats
operator|.
name|rx_crc_errors
operator|++
expr_stmt|;
if|if
condition|(
name|eistat
operator|&
name|RCS_RODD
condition|)
name|is
operator|->
name|is_stats
operator|.
name|rx_align_errors
operator|++
expr_stmt|;
if|if
condition|(
name|len
operator|<
name|ET_MINLEN
condition|)
name|is
operator|->
name|is_stats
operator|.
name|rx_underruns
operator|++
expr_stmt|;
if|if
condition|(
name|len
operator|>
name|ET_MAXLEN
operator|+
name|CRC_SIZE
condition|)
name|is
operator|->
name|is_stats
operator|.
name|rx_overruns
operator|++
expr_stmt|;
name|is
operator|->
name|is_if
operator|.
name|if_ierrors
operator|++
expr_stmt|;
name|rxseg
operator|->
name|rx_csr
operator|=
literal|0
expr_stmt|;
return|return;
block|}
else|else
name|is
operator|->
name|is_stats
operator|.
name|rx_datagrams
operator|++
expr_stmt|;
name|ace
operator|=
operator|(
expr|struct
name|ether_header
operator|*
operator|)
name|rxseg
operator|->
name|rx_data
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
name|len
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
expr_stmt|;
else|#
directive|else
name|len
operator|-=
literal|14
expr_stmt|;
endif|#
directive|endif
comment|/* 	 * Deal with trailer protocol: if type is PUP trailer 	 * get true type from first 16-bit word past data. 	 * Remember that type was trailer by setting off. 	 */
name|ace
operator|->
name|ether_type
operator|=
name|ntohs
argument_list|(
operator|(
name|u_short
operator|)
name|ace
operator|->
name|ether_type
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
define|#
directive|define
name|acedataaddr
parameter_list|(
name|ace
parameter_list|,
name|off
parameter_list|,
name|type
parameter_list|)
define|\
value|((type)(((caddr_t)(((char *)ace)+sizeof (struct ether_header))+(off))))
else|#
directive|else
define|#
directive|define
name|acedataaddr
parameter_list|(
name|ace
parameter_list|,
name|off
parameter_list|,
name|type
parameter_list|)
define|\
value|((type)(((caddr_t)(((char *)ace)+14)+(off))))
endif|#
directive|endif
if|if
condition|(
name|ace
operator|->
name|ether_type
operator|>=
name|ETHERPUP_TRAIL
operator|&&
name|ace
operator|->
name|ether_type
operator|<
name|ETHERPUP_TRAIL
operator|+
name|ETHERPUP_NTRAILER
condition|)
block|{
name|off
operator|=
operator|(
name|ace
operator|->
name|ether_type
operator|-
name|ETHERPUP_TRAIL
operator|)
operator|*
literal|512
expr_stmt|;
if|if
condition|(
name|off
operator|>=
name|ETHERMTU
condition|)
goto|goto
name|setup
goto|;
comment|/* sanity */
name|ace
operator|->
name|ether_type
operator|=
name|ntohs
argument_list|(
operator|*
name|acedataaddr
argument_list|(
name|ace
argument_list|,
name|off
argument_list|,
name|u_short
operator|*
argument_list|)
argument_list|)
expr_stmt|;
name|resid
operator|=
name|ntohs
argument_list|(
operator|*
operator|(
name|acedataaddr
argument_list|(
name|ace
argument_list|,
name|off
operator|+
literal|2
argument_list|,
name|u_short
operator|*
argument_list|)
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|off
operator|+
name|resid
operator|>
name|len
condition|)
goto|goto
name|setup
goto|;
comment|/* sanity */
name|len
operator|=
name|off
operator|+
name|resid
expr_stmt|;
block|}
else|else
name|off
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|len
operator|==
literal|0
condition|)
goto|goto
name|setup
goto|;
comment|/* 	 * Pull packet off interface.  Off is nonzero if packet 	 * has trailing header; aceget will then force this header 	 * information to be at the front, but we still have to drop 	 * the type and length which are at the front of any trailer data. 	 */
name|m
operator|=
name|aceget
argument_list|(
name|unit
argument_list|,
name|rxseg
operator|->
name|rx_data
argument_list|,
name|len
argument_list|,
name|off
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
goto|goto
name|setup
goto|;
if|if
condition|(
name|off
condition|)
block|{
name|m
operator|->
name|m_off
operator|+=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|-=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|ace
operator|->
name|ether_type
condition|)
block|{
ifdef|#
directive|ifdef
name|INET
case|case
name|ETHERPUP_IPTYPE
case|:
name|schednetisr
argument_list|(
name|NETISR_IP
argument_list|)
expr_stmt|;
name|inq
operator|=
operator|&
name|ipintrq
expr_stmt|;
break|break;
case|case
name|ETHERPUP_ARPTYPE
case|:
name|arpinput
argument_list|(
operator|&
name|is
operator|->
name|is_ac
argument_list|,
name|m
argument_list|)
expr_stmt|;
goto|goto
name|setup
goto|;
endif|#
directive|endif
default|default:
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
goto|goto
name|setup
goto|;
block|}
if|if
condition|(
name|IF_QFULL
argument_list|(
name|inq
argument_list|)
condition|)
block|{
name|IF_DROP
argument_list|(
name|inq
argument_list|)
expr_stmt|;
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
goto|goto
name|setup
goto|;
block|}
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
name|IF_ENQUEUE
argument_list|(
name|inq
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|setup
label|:
name|rxseg
operator|->
name|rx_csr
operator|=
literal|0
expr_stmt|;
goto|goto
name|again
goto|;
block|}
end_block

begin_comment
comment|/*  * Ethernet output routine.  * Encapsulate a packet of type family for the local net.  * Use trailer local net encapsulation if enough data in first  * packet leaves a multiple of 512 bytes of data in remainder.  */
end_comment

begin_macro
name|aceoutput
argument_list|(
argument|ifp
argument_list|,
argument|m0
argument_list|,
argument|dst
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sockaddr
modifier|*
name|dst
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|ifp
operator|->
name|if_unit
index|]
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
init|=
name|m0
decl_stmt|;
specifier|register
name|struct
name|ether_header
modifier|*
name|ace
decl_stmt|;
specifier|register
name|int
name|off
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|mcopy
init|=
operator|(
expr|struct
name|mbuf
operator|*
operator|)
literal|0
decl_stmt|;
name|int
name|type
decl_stmt|,
name|s
decl_stmt|,
name|error
decl_stmt|;
name|struct
name|ether_addr
name|edst
decl_stmt|;
name|struct
name|in_addr
name|idst
decl_stmt|;
switch|switch
condition|(
name|dst
operator|->
name|sa_family
condition|)
block|{
ifdef|#
directive|ifdef
name|INET
case|case
name|AF_INET
case|:
name|idst
operator|=
operator|(
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|dst
operator|)
operator|->
name|sin_addr
expr_stmt|;
if|if
condition|(
operator|!
name|arpresolve
argument_list|(
operator|&
name|is
operator|->
name|is_ac
argument_list|,
name|m
argument_list|,
operator|&
name|idst
argument_list|,
operator|&
name|edst
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
comment|/* if not yet resolved */
if|if
condition|(
name|in_lnaof
argument_list|(
name|idst
argument_list|)
operator|==
name|INADDR_ANY
condition|)
name|mcopy
operator|=
name|m_copy
argument_list|(
name|m
argument_list|,
literal|0
argument_list|,
operator|(
name|int
operator|)
name|M_COPYALL
argument_list|)
expr_stmt|;
name|off
operator|=
name|ntohs
argument_list|(
operator|(
name|u_short
operator|)
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
operator|->
name|ip_len
argument_list|)
operator|-
name|m
operator|->
name|m_len
expr_stmt|;
comment|/* need per host negotiation */
if|if
condition|(
operator|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_NOTRAILERS
operator|)
operator|==
literal|0
operator|&&
name|off
operator|>
literal|0
operator|&&
operator|(
name|off
operator|&
literal|0x1ff
operator|)
operator|==
literal|0
operator|&&
name|m
operator|->
name|m_off
operator|>=
name|MMINOFF
operator|+
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
condition|)
block|{
name|type
operator|=
name|ETHERPUP_TRAIL
operator|+
operator|(
name|off
operator|>>
literal|9
operator|)
expr_stmt|;
name|m
operator|->
name|m_off
operator|-=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
literal|2
operator|*
sizeof|sizeof
argument_list|(
name|u_short
argument_list|)
expr_stmt|;
operator|*
name|mtod
argument_list|(
name|m
argument_list|,
name|u_short
operator|*
argument_list|)
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|ETHERPUP_IPTYPE
argument_list|)
expr_stmt|;
operator|*
operator|(
name|mtod
argument_list|(
name|m
argument_list|,
name|u_short
operator|*
argument_list|)
operator|+
literal|1
operator|)
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|m
operator|->
name|m_len
argument_list|)
expr_stmt|;
goto|goto
name|gottrailertype
goto|;
block|}
name|type
operator|=
name|ETHERPUP_IPTYPE
expr_stmt|;
name|off
operator|=
literal|0
expr_stmt|;
goto|goto
name|gottype
goto|;
endif|#
directive|endif
case|case
name|AF_UNSPEC
case|:
name|ace
operator|=
operator|(
expr|struct
name|ether_header
operator|*
operator|)
name|dst
operator|->
name|sa_data
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
name|edst
operator|=
name|ace
operator|->
name|ether_dhost
expr_stmt|;
else|#
directive|else
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|ace
operator|->
name|ether_dhost
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|edst
argument_list|,
literal|6
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|type
operator|=
name|ace
operator|->
name|ether_type
expr_stmt|;
goto|goto
name|gottype
goto|;
default|default:
name|printf
argument_list|(
literal|"ace%d: can't handle af%d\n"
argument_list|,
name|ifp
operator|->
name|if_unit
argument_list|,
name|dst
operator|->
name|sa_family
argument_list|)
expr_stmt|;
name|error
operator|=
name|EAFNOSUPPORT
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|gottrailertype
label|:
comment|/* 	 * Packet to be sent as trailer: move first packet 	 * (control information) to end of chain. 	 */
while|while
condition|(
name|m
operator|->
name|m_next
condition|)
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
name|m
operator|->
name|m_next
operator|=
name|m0
expr_stmt|;
name|m
operator|=
name|m0
operator|->
name|m_next
expr_stmt|;
name|m0
operator|->
name|m_next
operator|=
literal|0
expr_stmt|;
name|m0
operator|=
name|m
expr_stmt|;
name|gottype
label|:
comment|/* 	 * Add local net header.  If no space in first mbuf, 	 * allocate another. 	 */
if|if
condition|(
name|m
operator|->
name|m_off
operator|>
name|MMAXOFF
operator|||
ifdef|#
directive|ifdef
name|notdef
name|MMINOFF
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
operator|>
name|m
operator|->
name|m_off
condition|)
block|{
else|#
directive|else
name|MMINOFF
operator|+
literal|14
operator|>
name|m
operator|->
name|m_off
block|)
block|{
endif|#
directive|endif
name|m
operator|=
name|m_get
argument_list|(
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|m
operator|->
name|m_next
operator|=
name|m0
expr_stmt|;
name|m
operator|->
name|m_off
operator|=
name|MMINOFF
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
name|m
operator|->
name|m_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
expr_stmt|;
else|#
directive|else
name|m
operator|->
name|m_len
operator|=
literal|14
expr_stmt|;
endif|#
directive|endif
block|}
else|else
block|{
ifdef|#
directive|ifdef
name|notdef
name|m
operator|->
name|m_off
operator|-=
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
expr_stmt|;
else|#
directive|else
name|m
operator|->
name|m_off
operator|-=
literal|14
expr_stmt|;
name|m
operator|->
name|m_len
operator|+=
literal|14
expr_stmt|;
endif|#
directive|endif
block|}
name|ace
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ether_header
operator|*
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
name|ace
operator|->
name|ether_dhost
operator|=
name|edst
expr_stmt|;
name|ace
operator|->
name|ether_shost
operator|=
name|is
operator|->
name|is_addr
expr_stmt|;
else|#
directive|else
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|edst
argument_list|,
operator|(
name|caddr_t
operator|)
name|ace
operator|->
name|ether_dhost
argument_list|,
literal|6
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|is
operator|->
name|is_addr
argument_list|,
operator|(
name|caddr_t
operator|)
name|ace
operator|->
name|ether_shost
argument_list|,
literal|6
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|ace
operator|->
name|ether_type
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|type
argument_list|)
expr_stmt|;
comment|/* 	 * Queue message on interface, and start output if interface 	 * not yet active. 	 */
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
if|if
condition|(
name|IF_QFULL
argument_list|(
operator|&
name|ifp
operator|->
name|if_snd
argument_list|)
condition|)
block|{
name|IF_DROP
argument_list|(
operator|&
name|ifp
operator|->
name|if_snd
argument_list|)
expr_stmt|;
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|qfull
goto|;
block|}
name|IF_ENQUEUE
argument_list|(
operator|&
name|ifp
operator|->
name|if_snd
argument_list|,
name|m
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|aceStart
argument_list|(
name|ifp
operator|->
name|if_unit
argument_list|)
expr_stmt|;
return|return
operator|(
name|mcopy
condition|?
name|looutput
argument_list|(
operator|&
name|loif
argument_list|,
name|mcopy
argument_list|,
name|dst
argument_list|)
else|:
literal|0
operator|)
return|;
name|qfull
label|:
name|m0
operator|=
name|m
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|bad
label|:
name|m_freem
argument_list|(
name|m0
argument_list|)
expr_stmt|;
if|if
condition|(
name|mcopy
condition|)
name|m_freem
argument_list|(
name|mcopy
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_macro
name|aceStart
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
if|if
condition|(
name|is
operator|->
name|is_flags
operator|&
name|ACEF_OACTIVE
condition|)
return|return;
name|is
operator|->
name|is_flags
operator||=
name|ACEF_OACTIVE
expr_stmt|;
name|acestart
argument_list|(
operator|(
name|dev_t
operator|)
name|unit
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_flags
operator|&=
operator|~
name|ACEF_OACTIVE
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Routine to copy from mbuf chain to transmit buffer on the VERSAbus  * If packet size is less than the minimum legal size,  * the buffer is expanded.  We probably should zero out the extra  * bytes for security, but that would slow things down.  */
end_comment

begin_macro
name|aceput
argument_list|(
argument|unit
argument_list|,
argument|txbuf
argument_list|,
argument|m
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* for statistics collection */
end_comment

begin_decl_stmt
name|u_char
modifier|*
name|txbuf
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|u_char
modifier|*
name|bp
decl_stmt|,
modifier|*
name|mcp
decl_stmt|;
comment|/* known to be r12, r11 */
specifier|register
name|short
modifier|*
name|s1
decl_stmt|,
modifier|*
name|s2
decl_stmt|;
comment|/* known to be r10, r9 */
specifier|register
name|unsigned
name|len
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|mp
decl_stmt|;
name|int
name|total
decl_stmt|,
name|idx
decl_stmt|;
name|total
operator|=
literal|0
expr_stmt|;
name|bp
operator|=
name|txbuf
expr_stmt|;
for|for
control|(
name|mp
operator|=
name|m
init|;
operator|(
name|mp
operator|)
condition|;
name|mp
operator|=
name|mp
operator|->
name|m_next
control|)
block|{
name|len
operator|=
name|mp
operator|->
name|m_len
expr_stmt|;
if|if
condition|(
name|len
operator|==
literal|0
condition|)
continue|continue;
name|total
operator|+=
name|len
expr_stmt|;
name|mcp
operator|=
name|mtod
argument_list|(
name|mp
argument_list|,
name|u_char
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
operator|(
name|int
operator|)
name|mcp
operator|&
literal|01
operator|)
operator|&&
operator|(
operator|(
name|int
operator|)
name|bp
operator|&
literal|01
operator|)
condition|)
block|{
comment|/* source& destination at odd addresses */
comment|/* *bp++ = *mcp++; */
asm|asm("movob (r11),(r12)");
name|bp
operator|++
operator|,
name|mcp
operator|++
expr_stmt|;
operator|--
name|len
expr_stmt|;
block|}
if|if
condition|(
name|len
operator|>
literal|1
operator|&&
operator|(
operator|(
operator|(
name|int
operator|)
name|mcp
operator|&
literal|01
operator|)
operator|==
literal|0
operator|)
operator|&&
operator|(
operator|(
operator|(
name|int
operator|)
name|bp
operator|&
literal|01
operator|)
operator|==
literal|0
operator|)
condition|)
block|{
specifier|register
name|int
name|l
decl_stmt|;
name|s1
operator|=
operator|(
name|short
operator|*
operator|)
name|bp
expr_stmt|;
name|s2
operator|=
operator|(
name|short
operator|*
operator|)
name|mcp
expr_stmt|;
name|l
operator|=
name|len
operator|>>
literal|1
expr_stmt|;
comment|/* count # of shorts */
while|while
condition|(
name|l
operator|--
operator|>
literal|0
condition|)
block|{
comment|/* *s1++ = *s2++; */
asm|asm("movow (r9),(r10)");
name|s1
operator|++
operator|,
name|s2
operator|++
expr_stmt|;
block|}
name|len
operator|&=
literal|1
expr_stmt|;
comment|/* # remaining bytes */
name|bp
operator|=
operator|(
name|u_char
operator|*
operator|)
name|s1
expr_stmt|;
name|mcp
operator|=
operator|(
name|u_char
operator|*
operator|)
name|s2
expr_stmt|;
block|}
while|while
condition|(
name|len
operator|--
operator|>
literal|0
condition|)
block|{
comment|/* *bp++ = *mcp++;  */
asm|asm("movob (r11),(r12)");
name|bp
operator|++
operator|,
name|mcp
operator|++
expr_stmt|;
block|}
block|}
name|m_freem
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|(
name|total
operator|)
return|;
block|}
end_block

begin_macro
name|movew
argument_list|(
argument|data
argument_list|,
argument|to
argument_list|)
end_macro

begin_decl_stmt
name|short
name|data
decl_stmt|,
modifier|*
name|to
decl_stmt|;
end_decl_stmt

begin_block
block|{
asm|asm("movow 6(fp),*8(fp)");
block|}
end_block

begin_comment
comment|/*  * Routine to copy from VERSAbus memory into mbufs.  *  * Warning: This makes the fairly safe assumption that  * mbufs have even lengths.  */
end_comment

begin_function
name|struct
name|mbuf
modifier|*
name|aceget
parameter_list|(
name|unit
parameter_list|,
name|rxbuf
parameter_list|,
name|totlen
parameter_list|,
name|off0
parameter_list|)
name|int
name|unit
decl_stmt|;
comment|/* for statistics collection */
name|u_char
modifier|*
name|rxbuf
decl_stmt|;
name|int
name|totlen
decl_stmt|,
name|off0
decl_stmt|;
block|{
specifier|register
name|u_char
modifier|*
name|cp
decl_stmt|,
modifier|*
name|mcp
decl_stmt|;
comment|/* known to be r12, r11 */
specifier|register
name|int
name|tlen
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|top
init|=
literal|0
decl_stmt|,
modifier|*
modifier|*
name|mp
init|=
operator|&
name|top
decl_stmt|;
name|int
name|len
decl_stmt|,
name|off
init|=
name|off0
decl_stmt|;
ifdef|#
directive|ifdef
name|notdef
name|cp
operator|=
name|rxbuf
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
expr_stmt|;
else|#
directive|else
name|cp
operator|=
name|rxbuf
operator|+
literal|14
expr_stmt|;
endif|#
directive|endif
while|while
condition|(
name|totlen
operator|>
literal|0
condition|)
block|{
specifier|register
name|int
name|words
decl_stmt|;
name|MGET
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_DATA
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
literal|0
condition|)
goto|goto
name|bad
goto|;
if|if
condition|(
name|off
condition|)
block|{
name|len
operator|=
name|totlen
operator|-
name|off
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
name|cp
operator|=
name|rxbuf
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
operator|+
name|off
expr_stmt|;
else|#
directive|else
name|cp
operator|=
name|rxbuf
operator|+
literal|14
operator|+
name|off
expr_stmt|;
endif|#
directive|endif
block|}
else|else
name|len
operator|=
name|totlen
expr_stmt|;
if|if
condition|(
name|len
operator|>=
name|CLBYTES
condition|)
block|{
name|struct
name|mbuf
modifier|*
name|p
decl_stmt|;
name|MCLGET
argument_list|(
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|!=
literal|0
condition|)
block|{
name|m
operator|->
name|m_len
operator|=
name|len
operator|=
name|CLBYTES
expr_stmt|;
name|m
operator|->
name|m_off
operator|=
operator|(
name|int
operator|)
name|p
operator|-
operator|(
name|int
operator|)
name|m
expr_stmt|;
block|}
else|else
block|{
name|m
operator|->
name|m_len
operator|=
name|len
operator|=
name|MIN
argument_list|(
name|MLEN
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_off
operator|=
name|MMINOFF
expr_stmt|;
block|}
block|}
else|else
block|{
name|m
operator|->
name|m_len
operator|=
name|len
operator|=
name|MIN
argument_list|(
name|MLEN
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_off
operator|=
name|MMINOFF
expr_stmt|;
block|}
name|mcp
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|u_char
operator|*
argument_list|)
expr_stmt|;
comment|/*bcopy((caddr_t)cp, (caddr_t)mcp, len);*/
comment|/*cp += len; mcp += len;*/
name|tlen
operator|=
name|len
expr_stmt|;
if|if
condition|(
operator|(
operator|(
name|int
operator|)
name|mcp
operator|&
literal|01
operator|)
operator|&&
operator|(
operator|(
name|int
operator|)
name|cp
operator|&
literal|01
operator|)
condition|)
block|{
comment|/* source& destination at odd addresses */
operator|*
name|mcp
operator|++
operator|=
operator|*
name|cp
operator|++
expr_stmt|;
operator|--
name|tlen
expr_stmt|;
block|}
if|if
condition|(
name|tlen
operator|>
literal|1
operator|&&
operator|(
operator|(
operator|(
name|int
operator|)
name|mcp
operator|&
literal|01
operator|)
operator|==
literal|0
operator|)
operator|&&
operator|(
operator|(
operator|(
name|int
operator|)
name|cp
operator|&
literal|01
operator|)
operator|==
literal|0
operator|)
condition|)
block|{
specifier|register
name|short
modifier|*
name|s1
decl_stmt|,
modifier|*
name|s2
decl_stmt|;
specifier|register
name|int
name|l
decl_stmt|;
name|s1
operator|=
operator|(
name|short
operator|*
operator|)
name|mcp
expr_stmt|;
name|s2
operator|=
operator|(
name|short
operator|*
operator|)
name|cp
expr_stmt|;
name|l
operator|=
name|tlen
operator|>>
literal|1
expr_stmt|;
comment|/* count # of shorts */
while|while
condition|(
name|l
operator|--
operator|>
literal|0
condition|)
comment|/* copy shorts */
operator|*
name|s1
operator|++
operator|=
operator|*
name|s2
operator|++
expr_stmt|;
name|tlen
operator|&=
literal|1
expr_stmt|;
comment|/* # remaining bytes */
name|mcp
operator|=
operator|(
name|u_char
operator|*
operator|)
name|s1
expr_stmt|;
name|cp
operator|=
operator|(
name|u_char
operator|*
operator|)
name|s2
expr_stmt|;
block|}
while|while
condition|(
name|tlen
operator|--
operator|>
literal|0
condition|)
operator|*
name|mcp
operator|++
operator|=
operator|*
name|cp
operator|++
expr_stmt|;
operator|*
name|mp
operator|=
name|m
expr_stmt|;
name|mp
operator|=
operator|&
name|m
operator|->
name|m_next
expr_stmt|;
if|if
condition|(
name|off
operator|==
literal|0
condition|)
block|{
name|totlen
operator|-=
name|len
expr_stmt|;
continue|continue;
block|}
name|off
operator|+=
name|len
expr_stmt|;
if|if
condition|(
name|off
operator|==
name|totlen
condition|)
block|{
ifdef|#
directive|ifdef
name|notdef
name|cp
operator|=
name|rxbuf
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ether_header
argument_list|)
expr_stmt|;
else|#
directive|else
name|cp
operator|=
name|rxbuf
operator|+
literal|14
expr_stmt|;
endif|#
directive|endif
name|off
operator|=
literal|0
expr_stmt|;
name|totlen
operator|=
name|off0
expr_stmt|;
block|}
block|}
return|return
operator|(
name|top
operator|)
return|;
name|bad
label|:
name|m_freem
argument_list|(
name|top
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_macro
name|acebakoff
argument_list|(
argument|is
argument_list|,
argument|txseg
argument_list|,
argument|retries
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|ace_softc
modifier|*
name|is
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|tx_segment
modifier|*
name|txseg
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|int
name|retries
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|short
modifier|*
name|pBakNum
decl_stmt|,
name|random_num
decl_stmt|;
name|short
modifier|*
name|pMask
decl_stmt|;
name|pMask
operator|=
operator|&
name|random_mask_tbl
index|[
literal|0
index|]
expr_stmt|;
name|pBakNum
operator|=
operator|&
name|txseg
operator|->
name|tx_backoff
index|[
literal|0
index|]
expr_stmt|;
while|while
condition|(
operator|--
name|retries
operator|>=
literal|0
condition|)
block|{
name|random_num
operator|=
operator|(
name|is
operator|->
name|is_currnd
operator|=
operator|(
name|is
operator|->
name|is_currnd
operator|*
literal|18741
operator|)
operator|-
literal|13849
operator|)
expr_stmt|;
name|random_num
operator|&=
operator|*
name|pMask
operator|++
expr_stmt|;
operator|*
name|pBakNum
operator|++
operator|=
name|random_num
operator|^
call|(
name|short
call|)
argument_list|(
literal|0xFF00
operator||
literal|0x00FC
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Process an ioctl request.  */
end_comment

begin_expr_stmt
name|aceioctl
argument_list|(
name|ifp
argument_list|,
name|cmd
argument_list|,
name|data
argument_list|)
specifier|register
expr|struct
name|ifnet
operator|*
name|ifp
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|cmd
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|caddr_t
name|data
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ifreq
modifier|*
name|ifr
init|=
operator|(
expr|struct
name|ifreq
operator|*
operator|)
name|data
decl_stmt|;
name|int
name|s
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|;
name|s
operator|=
name|splimp
argument_list|()
expr_stmt|;
switch|switch
condition|(
name|cmd
condition|)
block|{
case|case
name|SIOCSIFADDR
case|:
if|if
condition|(
name|ifp
operator|->
name|if_flags
operator|&
name|IFF_RUNNING
condition|)
name|if_rtinit
argument_list|(
name|ifp
argument_list|,
operator|-
literal|1
argument_list|)
expr_stmt|;
comment|/* delete previous route */
name|acesetaddr
argument_list|(
name|ifp
argument_list|,
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ifr
operator|->
name|ifr_addr
argument_list|)
expr_stmt|;
name|aceinit
argument_list|(
name|ifp
operator|->
name|if_unit
argument_list|)
expr_stmt|;
break|break;
ifdef|#
directive|ifdef
name|notdef
case|case
name|SIOCSETETADDR
case|:
block|{
comment|/* set Ethernet station address */
name|struct
name|vba_device
modifier|*
name|ui
decl_stmt|;
name|struct
name|acedevice
modifier|*
name|addr
decl_stmt|;
name|struct
name|sockaddr_in
modifier|*
name|sin
decl_stmt|;
name|ifp
operator|->
name|if_flags
operator|&=
operator|~
name|IFF_RUNNING
operator||
name|IFF_UP
expr_stmt|;
name|sin
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ifr
operator|->
name|ifr_addr
expr_stmt|;
name|ui
operator|=
name|aceinfo
index|[
name|ifp
operator|->
name|if_unit
index|]
expr_stmt|;
name|addr
operator|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
expr_stmt|;
name|movew
argument_list|(
operator|(
name|short
operator|)
name|CSR_RESET
argument_list|,
operator|&
name|addr
operator|->
name|csr
argument_list|)
expr_stmt|;
name|DELAY
argument_list|(
literal|10000
argument_list|)
expr_stmt|;
comment|/* set station address and copy addr to arp struct */
name|acesetetaddr
argument_list|(
name|ifp
operator|->
name|if_unit
argument_list|,
name|addr
argument_list|,
operator|&
name|sin
operator|->
name|sin_zero
index|[
literal|2
index|]
argument_list|)
expr_stmt|;
name|aceinit
argument_list|(
name|ifp
operator|->
name|if_unit
argument_list|)
expr_stmt|;
comment|/* Re-initialize */
break|break;
block|}
endif|#
directive|endif
default|default:
name|error
operator|=
name|EINVAL
expr_stmt|;
block|}
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

begin_expr_stmt
name|acesetaddr
argument_list|(
name|ifp
argument_list|,
name|sin
argument_list|)
specifier|register
expr|struct
name|ifnet
operator|*
name|ifp
expr_stmt|;
end_expr_stmt

begin_decl_stmt
specifier|register
name|struct
name|sockaddr_in
modifier|*
name|sin
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|ifp
operator|->
name|if_addr
operator|=
operator|*
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|sin
expr_stmt|;
name|ifp
operator|->
name|if_net
operator|=
name|in_netof
argument_list|(
name|sin
operator|->
name|sin_addr
argument_list|)
expr_stmt|;
name|ifp
operator|->
name|if_host
index|[
literal|0
index|]
operator|=
name|in_lnaof
argument_list|(
name|sin
operator|->
name|sin_addr
argument_list|)
expr_stmt|;
name|sin
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ifp
operator|->
name|if_broadaddr
expr_stmt|;
name|sin
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|sin
operator|->
name|sin_addr
operator|=
name|if_makeaddr
argument_list|(
name|ifp
operator|->
name|if_net
argument_list|,
name|INADDR_ANY
argument_list|)
expr_stmt|;
name|ifp
operator|->
name|if_flags
operator||=
name|IFF_BROADCAST
expr_stmt|;
block|}
end_block

begin_macro
name|aceclean
argument_list|(
argument|unit
argument_list|)
end_macro

begin_decl_stmt
name|int
name|unit
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ace_softc
modifier|*
name|is
init|=
operator|&
name|ace_softc
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|ifnet
modifier|*
name|ifp
init|=
operator|&
name|is
operator|->
name|is_if
decl_stmt|;
specifier|register
name|struct
name|vba_device
modifier|*
name|ui
init|=
name|aceinfo
index|[
name|unit
index|]
decl_stmt|;
specifier|register
name|struct
name|acedevice
modifier|*
name|addr
init|=
operator|(
expr|struct
name|acedevice
operator|*
operator|)
name|ui
operator|->
name|ui_addr
decl_stmt|;
specifier|register
name|short
name|i
decl_stmt|,
name|data
decl_stmt|;
specifier|register
name|char
modifier|*
name|pData1
decl_stmt|;
name|ioaccess
argument_list|(
name|ACEmap
index|[
name|unit
index|]
argument_list|,
name|ACEmapa
index|[
name|unit
index|]
argument_list|,
name|ACEBPTE
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_dpm
operator|=
name|acemap
index|[
name|unit
index|]
expr_stmt|;
comment|/* init dpm */
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|is
operator|->
name|is_dpm
argument_list|,
literal|16384
operator|*
literal|2
argument_list|)
expr_stmt|;
name|is
operator|->
name|is_currnd
operator|=
literal|49123
expr_stmt|;
name|is
operator|->
name|is_segboundry
operator|=
operator|(
name|addr
operator|->
name|segb
operator|>>
literal|11
operator|)
operator|&
literal|0xf
expr_stmt|;
name|pData1
operator|=
operator|(
name|char
operator|*
operator|)
operator|(
operator|(
name|int
operator|)
name|is
operator|->
name|is_dpm
operator|+
operator|(
name|is
operator|->
name|is_segboundry
operator|<<
literal|11
operator|)
operator|)
expr_stmt|;
for|for
control|(
name|i
operator|=
name|SEG_MAX
operator|+
literal|1
operator|-
name|is
operator|->
name|is_segboundry
init|;
operator|--
name|i
operator|>=
literal|0
condition|;
control|)
block|{
name|acebakoff
argument_list|(
name|is
argument_list|,
operator|(
expr|struct
name|tx_segment
operator|*
operator|)
name|pData1
argument_list|,
literal|15
argument_list|)
expr_stmt|;
name|pData1
operator|+=
sizeof|sizeof
argument_list|(
expr|struct
name|tx_segment
argument_list|)
expr_stmt|;
block|}
name|is
operator|->
name|is_eictr
operator|=
literal|0
expr_stmt|;
name|is
operator|->
name|is_eoctr
operator|=
name|is
operator|->
name|is_txnext
operator|=
name|is
operator|->
name|is_segboundry
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|is
operator|->
name|is_stats
argument_list|,
sizeof|sizeof
argument_list|(
name|is
operator|->
name|is_stats
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

end_unit

