...
name|cmd
decl_stmt|;
comment|/* 		 * Choose the appropriate i/o command based on the 		 * transfer size and the controller's internal buffer. 		 * If we're retrying a read on a raw device because 		 * the original try was a buffer request which failed 		 * due to a record length error, then we force the use 		 * of the raw controller read (YECH!!!!). 		 */
if|if
condition|(
name|bp
operator|->
...
condition|)
block|{
comment|/* 			 * Retry reads once with the command changed to 			 * a raw read (if possible).  Setting b_errcnt 			 * here causes cystart (above) to force a CY_RCOM. 			 */
if|if
condition|(
name|bp
operator|->
...
name|yc_ttyp
argument_list|,
literal|"yc%d: hard error bn%d status=%b"
argument_list|,
name|YCUNIT
argument_list|(
name|bp
...
begin_comment
comment|/*  * Load a 20 bit pointer into an i/o register.  */
end_comment
