...
begin_comment
comment|/* does this port? getw(fdesc) 	int fdesc; { 	register w,i; 	int val;  	for (i = 0, val = 0; i< sizeof(val); i++) { 		w = getc(fdesc); 		if (w< 0) { 			if (i == 0) 				return (-1); 			else 				return (val); 		} 		val = (val<< 8) | (w& 0xff); 	} 	return (val); } */
end_comment
