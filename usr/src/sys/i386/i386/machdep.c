...
begin_comment
comment|/*-  * Copyright (c) 1982, 1987, 1990 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * William Jolitz.  *  * %sccs.include.386.c%  *  *	@(#)machdep.c	5.7 (Berkeley) %G%  */
end_comment
...
literal|1
expr_stmt|;
comment|/* 	if(biosmem != 640) 		panic("does not have 640K of base memory");  	biosmem = 1024; 	biosmem += rtcin(RTC_EXTLO) + (rtcin(RTC_EXTHI)<<8); 	biosmem = biosmem/4 - 1 ; 	if (biosmem< maxmem) maxmem=biosmem; */
ifdef|#
directive|ifdef
name|SMALL
if|if
...
name|code
expr_stmt|;
comment|/*if (sig == SIGILL || sig == SIGFPE) { 		fp->sf_code = u.u_code; 		u.u_code = 0; 	} else 		fp->sf_code = 0; */
