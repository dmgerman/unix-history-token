...
name|printf
argument_list|(
literal|"FP exception: pid %d(%s): no busy frame, ft=%x pc=%x vec=%x\n"
argument_list|,
name|curproc
operator|->
name|p_pid
...
name|HP380
argument_list|)
comment|/* 	 * Deal with writebacks when returning from sigreturn. 	 * They may generate another signal to be processed. 	 * Again, we don't attempt the writeback if we have already 	 * tried and failed (see comment in trap). 	 */
if|if
condition|(
name|mmutype
operator|==
