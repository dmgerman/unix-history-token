...
begin_comment
comment|/*  * Copyright (c) 1988 University of Utah.  * Copyright (c) 1982, 1986, 1990 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * the Systems Programming Group of the University of Utah Computer  * Science Department.  *  * %sccs.include.redist.c%  *  * from: Utah $Hdr: trap.c 1.28 89/09/25$  *  *	@(#)trap.c	7.1 (Berkeley) %G%  */
end_comment
...
directive|endif
comment|/* 	 * We assign&u to a local variable for GCC.  This ensures that 	 * we can explicitly reload it after the call to qsetjmp below. 	 * If we don't do this, GCC may itself have assigned&u to a 	 * register variable which will not be properly reloaded, since 	 * GCC knows nothing of the funky semantics of qsetjmp. 	 */
name|up
operator|=
operator|&
name|u
...
name|NBPW
expr_stmt|;
comment|/* 	 * We use entry 0 instead of 63 to signify an invalid syscall because 	 * HPUX uses 63 and 0 works just as well for our purposes. 	 */
if|if
condition|(
name|code
operator|==
...
index|]
expr_stmt|;
comment|/* 	 * qsetjmp only saves a6/a7.  This speeds things up in the common 	 * case (where saved values are never used).  There is a side effect 	 * however.  Namely, if we do return via longjmp() we must restore 	 * our own register variables. 	 */
if|if
condition|(
name|qsetjmp
argument_list|(
...
expr_stmt|;
block|}
comment|/* 	 * Need to reinit p for two reason.  One, it is a register var 	 * and is not saved in the qsetjmp so a EINTR return will leave 	 * it with garbage.  Two, even on a normal return, it will be 	 * wrong for the child process of a fork (it will point to the 	 * parent). 	 */
name|p
operator|=
name|up
operator|->
...
name|opc
expr_stmt|;
comment|/* else if (up->u_eosys == JUSTRETURN) */
