...
begin_comment
comment|/*   * Copyright (c) 1991 Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * the Systems Programming Group of the University of Utah Computer  * Science Department.  *  * %sccs.include.redist.c%  *  *	@(#)pmap.c	7.14 (Berkeley) %G%  */
end_comment
...
parameter_list|)
define|\
value|(mmutype == MMU_68040 \ 	 ? (pmap_ste1(m, v)->sg_v&& pmap_ste2(m, v)->sg_v) \ 	 : (pmap_ste(m, v)->sg_v))
end_define
...
do|do
block|{
comment|/* 				 * Clear caches as necessary if making RO. 				 * XXX clear VAC?  Doesn't seem to be needed. 				 */
if|#
directive|if
name|defined
argument_list|(
...
name|HP380
argument_list|)
comment|/* 	 * If we are changing caching status or protection 	 * make sure the caches are flushed. 	 */
if|if
condition|(
name|mmutype
operator|==
