begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1982, 1986, 1989, 1994  *	The Regents of the University of California.  All rights reserved.  *  * This code is derived from software contributed to Berkeley  * by Pace Willisson (pace@blitz.com).  The Rock Ridge Extension  * Support code is derived from software contributed to Berkeley  * by Atsushi Murai (amurai@spec.co.jp).  *  * %sccs.include.redist.c%  *  *	@(#)cd9660_node.c	8.5 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/buf.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<isofs/cd9660/iso.h>
end_include

begin_include
include|#
directive|include
file|<isofs/cd9660/cd9660_node.h>
end_include

begin_include
include|#
directive|include
file|<isofs/cd9660/iso_rrip.h>
end_include

begin_comment
comment|/*  * Structures associated with iso_node caching.  */
end_comment

begin_decl_stmt
name|struct
name|iso_node
modifier|*
modifier|*
name|isohashtbl
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|u_long
name|isohash
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|INOHASH
parameter_list|(
name|device
parameter_list|,
name|inum
parameter_list|)
value|(((device) + ((inum)>>12))& isohash)
end_define

begin_ifdef
ifdef|#
directive|ifdef
name|ISODEVMAP
end_ifdef

begin_decl_stmt
name|struct
name|iso_node
modifier|*
modifier|*
name|idvhashtbl
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|u_long
name|idvhash
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|DNOHASH
parameter_list|(
name|device
parameter_list|,
name|inum
parameter_list|)
value|(((device) + ((inum)>>12))& idvhash)
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|int
name|prtactive
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* 1 => print out reclaim of active vnodes */
end_comment

begin_comment
comment|/*  * Initialize hash links for inodes and dnodes.  */
end_comment

begin_macro
name|cd9660_init
argument_list|()
end_macro

begin_block
block|{
name|isohashtbl
operator|=
name|hashinit
argument_list|(
name|desiredvnodes
argument_list|,
name|M_ISOFSMNT
argument_list|,
operator|&
name|isohash
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|ISODEVMAP
name|idvhashtbl
operator|=
name|hashinit
argument_list|(
name|desiredvnodes
operator|/
literal|8
argument_list|,
name|M_ISOFSMNT
argument_list|,
operator|&
name|idvhash
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
end_block

begin_ifdef
ifdef|#
directive|ifdef
name|ISODEVMAP
end_ifdef

begin_comment
comment|/*  * Enter a new node into the device hash list  */
end_comment

begin_function
name|struct
name|iso_dnode
modifier|*
name|iso_dmap
parameter_list|(
name|device
parameter_list|,
name|inum
parameter_list|,
name|create
parameter_list|)
name|dev_t
name|device
decl_stmt|;
name|ino_t
name|inum
decl_stmt|;
name|int
name|create
decl_stmt|;
block|{
specifier|register
name|struct
name|iso_dnode
modifier|*
modifier|*
name|dpp
decl_stmt|,
modifier|*
name|dp
decl_stmt|,
modifier|*
name|dq
decl_stmt|;
name|dpp
operator|=
operator|&
name|idvhashtbl
index|[
name|DNOHASH
argument_list|(
name|device
argument_list|,
name|inum
argument_list|)
index|]
expr_stmt|;
for|for
control|(
name|dp
operator|=
operator|*
name|dpp
init|;
condition|;
name|dp
operator|=
name|dp
operator|->
name|d_next
control|)
block|{
if|if
condition|(
name|dp
operator|==
name|NULL
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
if|if
condition|(
name|inum
operator|==
name|dp
operator|->
name|i_number
operator|&&
name|device
operator|==
name|dp
operator|->
name|i_dev
condition|)
return|return
operator|(
name|dp
operator|)
return|;
if|if
condition|(
operator|!
name|create
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
name|MALLOC
argument_list|(
name|dp
argument_list|,
expr|struct
name|iso_dnode
operator|*
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|iso_dnode
argument_list|)
argument_list|,
name|M_CACHE
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
name|dp
operator|->
name|i_dev
operator|=
name|dev
expr_stmt|;
name|dp
operator|->
name|i_number
operator|=
name|ino
expr_stmt|;
if|if
condition|(
name|dq
operator|=
operator|*
name|dpp
condition|)
name|dq
operator|->
name|d_prev
operator|=
name|dp
operator|->
name|d_next
expr_stmt|;
name|dp
operator|->
name|d_next
operator|=
name|dq
expr_stmt|;
name|dp
operator|->
name|d_prev
operator|=
name|dpp
expr_stmt|;
operator|*
name|dpp
operator|=
name|dp
expr_stmt|;
return|return
operator|(
name|dp
operator|)
return|;
block|}
name|void
name|iso_dunmap
parameter_list|(
name|device
parameter_list|)
name|dev_t
name|device
decl_stmt|;
block|{
name|struct
name|iso_dnode
modifier|*
modifier|*
name|dpp
decl_stmt|,
modifier|*
name|dp
decl_stmt|,
modifier|*
name|dq
decl_stmt|;
for|for
control|(
name|dpp
operator|=
name|idvhashtbl
init|;
name|dpp
operator|<=
name|idvhashtbl
operator|+
name|idvhash
condition|;
name|dpp
operator|++
control|)
block|{
for|for
control|(
name|dp
operator|=
operator|*
name|dpp
init|;
name|dp
operator|!=
name|NULL
condition|;
name|dp
operator|=
name|dq
control|)
name|dq
operator|=
name|dp
operator|->
name|d_next
expr_stmt|;
if|if
condition|(
name|device
operator|==
name|dp
operator|->
name|i_dev
condition|)
block|{
if|if
condition|(
name|dq
condition|)
name|dq
operator|->
name|d_prev
operator|=
name|dp
operator|->
name|d_prev
expr_stmt|;
operator|*
name|dp
operator|->
name|d_prev
operator|=
name|dq
expr_stmt|;
name|FREE
argument_list|(
name|dp
argument_list|,
name|M_CACHE
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Use the device/inum pair to find the incore inode, and return a pointer  * to it. If it is in core, but locked, wait for it.  */
end_comment

begin_function
name|struct
name|vnode
modifier|*
name|cd9660_ihashget
parameter_list|(
name|device
parameter_list|,
name|inum
parameter_list|)
name|dev_t
name|device
decl_stmt|;
name|ino_t
name|inum
decl_stmt|;
block|{
specifier|register
name|struct
name|iso_node
modifier|*
name|ip
decl_stmt|;
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
for|for
control|(
init|;
condition|;
control|)
for|for
control|(
name|ip
operator|=
name|isohashtbl
index|[
name|INOHASH
argument_list|(
name|device
argument_list|,
name|inum
argument_list|)
index|]
init|;
condition|;
name|ip
operator|=
name|ip
operator|->
name|i_next
control|)
block|{
if|if
condition|(
name|ip
operator|==
name|NULL
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
if|if
condition|(
name|inum
operator|==
name|ip
operator|->
name|i_number
operator|&&
name|device
operator|==
name|ip
operator|->
name|i_dev
condition|)
block|{
if|if
condition|(
name|ip
operator|->
name|i_flag
operator|&
name|IN_LOCKED
condition|)
block|{
name|ip
operator|->
name|i_flag
operator||=
name|IN_WANTED
expr_stmt|;
name|sleep
argument_list|(
name|ip
argument_list|,
name|PINOD
argument_list|)
expr_stmt|;
break|break;
block|}
name|vp
operator|=
name|ITOV
argument_list|(
name|ip
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|vget
argument_list|(
name|vp
argument_list|,
literal|1
argument_list|)
condition|)
return|return
operator|(
name|vp
operator|)
return|;
break|break;
block|}
block|}
comment|/* NOTREACHED */
block|}
end_function

begin_comment
comment|/*  * Insert the inode into the hash table, and return it locked.  */
end_comment

begin_function
name|void
name|cd9660_ihashins
parameter_list|(
name|ip
parameter_list|)
name|struct
name|iso_node
modifier|*
name|ip
decl_stmt|;
block|{
name|struct
name|iso_node
modifier|*
modifier|*
name|ipp
decl_stmt|,
modifier|*
name|iq
decl_stmt|;
name|ipp
operator|=
operator|&
name|isohashtbl
index|[
name|INOHASH
argument_list|(
name|ip
operator|->
name|i_dev
argument_list|,
name|ip
operator|->
name|i_number
argument_list|)
index|]
expr_stmt|;
if|if
condition|(
name|iq
operator|=
operator|*
name|ipp
condition|)
name|iq
operator|->
name|i_prev
operator|=
operator|&
name|ip
operator|->
name|i_next
expr_stmt|;
name|ip
operator|->
name|i_next
operator|=
name|iq
expr_stmt|;
name|ip
operator|->
name|i_prev
operator|=
name|ipp
expr_stmt|;
operator|*
name|ipp
operator|=
name|ip
expr_stmt|;
if|if
condition|(
name|ip
operator|->
name|i_flag
operator|&
name|IN_LOCKED
condition|)
name|panic
argument_list|(
literal|"cd9660_ihashins: already locked"
argument_list|)
expr_stmt|;
if|if
condition|(
name|curproc
condition|)
name|ip
operator|->
name|i_lockholder
operator|=
name|curproc
operator|->
name|p_pid
expr_stmt|;
else|else
name|ip
operator|->
name|i_lockholder
operator|=
operator|-
literal|1
expr_stmt|;
name|ip
operator|->
name|i_flag
operator||=
name|IN_LOCKED
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Remove the inode from the hash table.  */
end_comment

begin_function
name|void
name|cd9660_ihashrem
parameter_list|(
name|ip
parameter_list|)
specifier|register
name|struct
name|iso_node
modifier|*
name|ip
decl_stmt|;
block|{
specifier|register
name|struct
name|iso_node
modifier|*
name|iq
decl_stmt|;
if|if
condition|(
name|iq
operator|=
name|ip
operator|->
name|i_next
condition|)
name|iq
operator|->
name|i_prev
operator|=
name|ip
operator|->
name|i_prev
expr_stmt|;
operator|*
name|ip
operator|->
name|i_prev
operator|=
name|iq
expr_stmt|;
ifdef|#
directive|ifdef
name|DIAGNOSTIC
name|ip
operator|->
name|i_next
operator|=
name|NULL
expr_stmt|;
name|ip
operator|->
name|i_prev
operator|=
name|NULL
expr_stmt|;
endif|#
directive|endif
block|}
end_function

begin_comment
comment|/*  * Last reference to an inode, write the inode out and if necessary,  * truncate and deallocate the file.  */
end_comment

begin_function
name|int
name|cd9660_inactive
parameter_list|(
name|ap
parameter_list|)
name|struct
name|vop_inactive_args
comment|/* { 		struct vnode *a_vp; 	} */
modifier|*
name|ap
decl_stmt|;
block|{
name|struct
name|vnode
modifier|*
name|vp
init|=
name|ap
operator|->
name|a_vp
decl_stmt|;
specifier|register
name|struct
name|iso_node
modifier|*
name|ip
init|=
name|VTOI
argument_list|(
name|vp
argument_list|)
decl_stmt|;
name|int
name|mode
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|prtactive
operator|&&
name|vp
operator|->
name|v_usecount
operator|!=
literal|0
condition|)
name|vprint
argument_list|(
literal|"cd9660_inactive: pushing active"
argument_list|,
name|vp
argument_list|)
expr_stmt|;
name|ip
operator|->
name|i_flag
operator|=
literal|0
expr_stmt|;
comment|/* 	 * If we are done with the inode, reclaim it 	 * so that it can be reused immediately. 	 */
if|if
condition|(
name|vp
operator|->
name|v_usecount
operator|==
literal|0
operator|&&
name|ip
operator|->
name|inode
operator|.
name|iso_mode
operator|==
literal|0
condition|)
name|vgone
argument_list|(
name|vp
argument_list|)
expr_stmt|;
return|return
name|error
return|;
block|}
end_function

begin_comment
comment|/*  * Reclaim an inode so that it can be used for other purposes.  */
end_comment

begin_function
name|int
name|cd9660_reclaim
parameter_list|(
name|ap
parameter_list|)
name|struct
name|vop_reclaim_args
comment|/* { 		struct vnode *a_vp; 	} */
modifier|*
name|ap
decl_stmt|;
block|{
specifier|register
name|struct
name|vnode
modifier|*
name|vp
init|=
name|ap
operator|->
name|a_vp
decl_stmt|;
specifier|register
name|struct
name|iso_node
modifier|*
name|ip
init|=
name|VTOI
argument_list|(
name|vp
argument_list|)
decl_stmt|;
name|int
name|i
decl_stmt|;
if|if
condition|(
name|prtactive
operator|&&
name|vp
operator|->
name|v_usecount
operator|!=
literal|0
condition|)
name|vprint
argument_list|(
literal|"cd9660_reclaim: pushing active"
argument_list|,
name|vp
argument_list|)
expr_stmt|;
comment|/* 	 * Remove the inode from its hash chain. 	 */
name|cd9660_ihashrem
argument_list|(
name|ip
argument_list|)
expr_stmt|;
comment|/* 	 * Purge old data structures associated with the inode. 	 */
name|cache_purge
argument_list|(
name|vp
argument_list|)
expr_stmt|;
if|if
condition|(
name|ip
operator|->
name|i_devvp
condition|)
block|{
name|vrele
argument_list|(
name|ip
operator|->
name|i_devvp
argument_list|)
expr_stmt|;
name|ip
operator|->
name|i_devvp
operator|=
literal|0
expr_stmt|;
block|}
name|FREE
argument_list|(
name|vp
operator|->
name|v_data
argument_list|,
name|M_ISOFSNODE
argument_list|)
expr_stmt|;
name|vp
operator|->
name|v_data
operator|=
name|NULL
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * File attributes  */
end_comment

begin_function
name|void
name|cd9660_defattr
parameter_list|(
name|isodir
parameter_list|,
name|inop
parameter_list|,
name|bp
parameter_list|)
name|struct
name|iso_directory_record
modifier|*
name|isodir
decl_stmt|;
name|struct
name|iso_node
modifier|*
name|inop
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
block|{
name|struct
name|buf
modifier|*
name|bp2
init|=
name|NULL
decl_stmt|;
name|struct
name|iso_mnt
modifier|*
name|imp
decl_stmt|;
name|struct
name|iso_extended_attributes
modifier|*
name|ap
init|=
name|NULL
decl_stmt|;
name|int
name|off
decl_stmt|;
if|if
condition|(
name|isonum_711
argument_list|(
name|isodir
operator|->
name|flags
argument_list|)
operator|&
literal|2
condition|)
block|{
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator|=
name|S_IFDIR
expr_stmt|;
comment|/* 		 * If we return 2, fts() will assume there are no subdirectories 		 * (just links for the path and .), so instead we return 1. 		 */
name|inop
operator|->
name|inode
operator|.
name|iso_links
operator|=
literal|1
expr_stmt|;
block|}
else|else
block|{
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator|=
name|S_IFREG
expr_stmt|;
name|inop
operator|->
name|inode
operator|.
name|iso_links
operator|=
literal|1
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|bp
operator|&&
operator|(
operator|(
name|imp
operator|=
name|inop
operator|->
name|i_mnt
operator|)
operator|->
name|im_flags
operator|&
name|ISOFSMNT_EXTATT
operator|)
operator|&&
operator|(
name|off
operator|=
name|isonum_711
argument_list|(
name|isodir
operator|->
name|ext_attr_length
argument_list|)
operator|)
condition|)
block|{
name|VOP_BLKATOFF
argument_list|(
name|ITOV
argument_list|(
name|inop
argument_list|)
argument_list|,
operator|(
name|off_t
operator|)
operator|-
operator|(
name|off
operator|<<
name|imp
operator|->
name|im_bshift
operator|)
argument_list|,
name|NULL
argument_list|,
operator|&
name|bp2
argument_list|)
expr_stmt|;
name|bp
operator|=
name|bp2
expr_stmt|;
block|}
if|if
condition|(
name|bp
condition|)
block|{
name|ap
operator|=
operator|(
expr|struct
name|iso_extended_attributes
operator|*
operator|)
name|bp
operator|->
name|b_data
expr_stmt|;
if|if
condition|(
name|isonum_711
argument_list|(
name|ap
operator|->
name|version
argument_list|)
operator|==
literal|1
condition|)
block|{
if|if
condition|(
operator|!
operator|(
name|ap
operator|->
name|perm
index|[
literal|0
index|]
operator|&
literal|0x40
operator|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator||=
name|VEXEC
operator|>>
literal|6
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|ap
operator|->
name|perm
index|[
literal|0
index|]
operator|&
literal|0x10
operator|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator||=
name|VREAD
operator|>>
literal|6
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|ap
operator|->
name|perm
index|[
literal|0
index|]
operator|&
literal|4
operator|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator||=
name|VEXEC
operator|>>
literal|3
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|ap
operator|->
name|perm
index|[
literal|0
index|]
operator|&
literal|1
operator|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator||=
name|VREAD
operator|>>
literal|3
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|ap
operator|->
name|perm
index|[
literal|1
index|]
operator|&
literal|0x40
operator|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator||=
name|VEXEC
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|ap
operator|->
name|perm
index|[
literal|1
index|]
operator|&
literal|0x10
operator|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator||=
name|VREAD
expr_stmt|;
name|inop
operator|->
name|inode
operator|.
name|iso_uid
operator|=
name|isonum_723
argument_list|(
name|ap
operator|->
name|owner
argument_list|)
expr_stmt|;
comment|/* what about 0? */
name|inop
operator|->
name|inode
operator|.
name|iso_gid
operator|=
name|isonum_723
argument_list|(
name|ap
operator|->
name|group
argument_list|)
expr_stmt|;
comment|/* what about 0? */
block|}
else|else
name|ap
operator|=
name|NULL
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|ap
condition|)
block|{
name|inop
operator|->
name|inode
operator|.
name|iso_mode
operator||=
name|VREAD
operator||
name|VEXEC
operator||
operator|(
name|VREAD
operator||
name|VEXEC
operator|)
operator|>>
literal|3
operator||
operator|(
name|VREAD
operator||
name|VEXEC
operator|)
operator|>>
literal|6
expr_stmt|;
name|inop
operator|->
name|inode
operator|.
name|iso_uid
operator|=
operator|(
name|uid_t
operator|)
literal|0
expr_stmt|;
name|inop
operator|->
name|inode
operator|.
name|iso_gid
operator|=
operator|(
name|gid_t
operator|)
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|bp2
condition|)
name|brelse
argument_list|(
name|bp2
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Time stamps  */
end_comment

begin_function
name|void
name|cd9660_deftstamp
parameter_list|(
name|isodir
parameter_list|,
name|inop
parameter_list|,
name|bp
parameter_list|)
name|struct
name|iso_directory_record
modifier|*
name|isodir
decl_stmt|;
name|struct
name|iso_node
modifier|*
name|inop
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
block|{
name|struct
name|buf
modifier|*
name|bp2
init|=
name|NULL
decl_stmt|;
name|struct
name|iso_mnt
modifier|*
name|imp
decl_stmt|;
name|struct
name|iso_extended_attributes
modifier|*
name|ap
init|=
name|NULL
decl_stmt|;
name|int
name|off
decl_stmt|;
if|if
condition|(
operator|!
name|bp
operator|&&
operator|(
operator|(
name|imp
operator|=
name|inop
operator|->
name|i_mnt
operator|)
operator|->
name|im_flags
operator|&
name|ISOFSMNT_EXTATT
operator|)
operator|&&
operator|(
name|off
operator|=
name|isonum_711
argument_list|(
name|isodir
operator|->
name|ext_attr_length
argument_list|)
operator|)
condition|)
block|{
name|VOP_BLKATOFF
argument_list|(
name|ITOV
argument_list|(
name|inop
argument_list|)
argument_list|,
operator|(
name|off_t
operator|)
operator|-
operator|(
name|off
operator|<<
name|imp
operator|->
name|im_bshift
operator|)
argument_list|,
name|NULL
argument_list|,
operator|&
name|bp2
argument_list|)
expr_stmt|;
name|bp
operator|=
name|bp2
expr_stmt|;
block|}
if|if
condition|(
name|bp
condition|)
block|{
name|ap
operator|=
operator|(
expr|struct
name|iso_extended_attributes
operator|*
operator|)
name|bp
operator|->
name|b_data
expr_stmt|;
if|if
condition|(
name|isonum_711
argument_list|(
name|ap
operator|->
name|version
argument_list|)
operator|==
literal|1
condition|)
block|{
if|if
condition|(
operator|!
name|cd9660_tstamp_conv17
argument_list|(
name|ap
operator|->
name|ftime
argument_list|,
operator|&
name|inop
operator|->
name|inode
operator|.
name|iso_atime
argument_list|)
condition|)
name|cd9660_tstamp_conv17
argument_list|(
name|ap
operator|->
name|ctime
argument_list|,
operator|&
name|inop
operator|->
name|inode
operator|.
name|iso_atime
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|cd9660_tstamp_conv17
argument_list|(
name|ap
operator|->
name|ctime
argument_list|,
operator|&
name|inop
operator|->
name|inode
operator|.
name|iso_ctime
argument_list|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_ctime
operator|=
name|inop
operator|->
name|inode
operator|.
name|iso_atime
expr_stmt|;
if|if
condition|(
operator|!
name|cd9660_tstamp_conv17
argument_list|(
name|ap
operator|->
name|mtime
argument_list|,
operator|&
name|inop
operator|->
name|inode
operator|.
name|iso_mtime
argument_list|)
condition|)
name|inop
operator|->
name|inode
operator|.
name|iso_mtime
operator|=
name|inop
operator|->
name|inode
operator|.
name|iso_ctime
expr_stmt|;
block|}
else|else
name|ap
operator|=
name|NULL
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|ap
condition|)
block|{
name|cd9660_tstamp_conv7
argument_list|(
name|isodir
operator|->
name|date
argument_list|,
operator|&
name|inop
operator|->
name|inode
operator|.
name|iso_ctime
argument_list|)
expr_stmt|;
name|inop
operator|->
name|inode
operator|.
name|iso_atime
operator|=
name|inop
operator|->
name|inode
operator|.
name|iso_ctime
expr_stmt|;
name|inop
operator|->
name|inode
operator|.
name|iso_mtime
operator|=
name|inop
operator|->
name|inode
operator|.
name|iso_ctime
expr_stmt|;
block|}
if|if
condition|(
name|bp2
condition|)
name|brelse
argument_list|(
name|bp2
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|int
name|cd9660_tstamp_conv7
parameter_list|(
name|pi
parameter_list|,
name|pu
parameter_list|)
name|u_char
modifier|*
name|pi
decl_stmt|;
name|struct
name|timespec
modifier|*
name|pu
decl_stmt|;
block|{
name|int
name|i
decl_stmt|;
name|int
name|crtime
decl_stmt|,
name|days
decl_stmt|;
name|int
name|y
decl_stmt|,
name|m
decl_stmt|,
name|d
decl_stmt|,
name|hour
decl_stmt|,
name|minute
decl_stmt|,
name|second
decl_stmt|,
name|tz
decl_stmt|;
name|y
operator|=
name|pi
index|[
literal|0
index|]
operator|+
literal|1900
expr_stmt|;
name|m
operator|=
name|pi
index|[
literal|1
index|]
expr_stmt|;
name|d
operator|=
name|pi
index|[
literal|2
index|]
expr_stmt|;
name|hour
operator|=
name|pi
index|[
literal|3
index|]
expr_stmt|;
name|minute
operator|=
name|pi
index|[
literal|4
index|]
expr_stmt|;
name|second
operator|=
name|pi
index|[
literal|5
index|]
expr_stmt|;
name|tz
operator|=
name|pi
index|[
literal|6
index|]
expr_stmt|;
if|if
condition|(
name|y
operator|<
literal|1970
condition|)
block|{
name|pu
operator|->
name|ts_sec
operator|=
literal|0
expr_stmt|;
name|pu
operator|->
name|ts_nsec
operator|=
literal|0
expr_stmt|;
return|return
literal|0
return|;
block|}
else|else
block|{
ifdef|#
directive|ifdef
name|ORIGINAL
comment|/* computes day number relative to Sept. 19th,1989 */
comment|/* don't even *THINK* about changing formula. It works! */
name|days
operator|=
literal|367
operator|*
operator|(
name|y
operator|-
literal|1980
operator|)
operator|-
literal|7
operator|*
operator|(
name|y
operator|+
operator|(
name|m
operator|+
literal|9
operator|)
operator|/
literal|12
operator|)
operator|/
literal|4
operator|-
literal|3
operator|*
operator|(
operator|(
name|y
operator|+
operator|(
name|m
operator|-
literal|9
operator|)
operator|/
literal|7
operator|)
operator|/
literal|100
operator|+
literal|1
operator|)
operator|/
literal|4
operator|+
literal|275
operator|*
name|m
operator|/
literal|9
operator|+
name|d
operator|-
literal|100
expr_stmt|;
else|#
directive|else
comment|/* 		 * Changed :-) to make it relative to Jan. 1st, 1970 		 * and to disambiguate negative division 		 */
name|days
operator|=
literal|367
operator|*
operator|(
name|y
operator|-
literal|1960
operator|)
operator|-
literal|7
operator|*
operator|(
name|y
operator|+
operator|(
name|m
operator|+
literal|9
operator|)
operator|/
literal|12
operator|)
operator|/
literal|4
operator|-
literal|3
operator|*
operator|(
operator|(
name|y
operator|+
operator|(
name|m
operator|+
literal|9
operator|)
operator|/
literal|12
operator|-
literal|1
operator|)
operator|/
literal|100
operator|+
literal|1
operator|)
operator|/
literal|4
operator|+
literal|275
operator|*
name|m
operator|/
literal|9
operator|+
name|d
operator|-
literal|239
expr_stmt|;
endif|#
directive|endif
name|crtime
operator|=
operator|(
operator|(
operator|(
operator|(
name|days
operator|*
literal|24
operator|)
operator|+
name|hour
operator|)
operator|*
literal|60
operator|+
name|minute
operator|)
operator|*
literal|60
operator|)
operator|+
name|second
expr_stmt|;
comment|/* timezone offset is unreliable on some disks */
if|if
condition|(
operator|-
literal|48
operator|<=
name|tz
operator|&&
name|tz
operator|<=
literal|52
condition|)
name|crtime
operator|-=
name|tz
operator|*
literal|15
operator|*
literal|60
expr_stmt|;
block|}
name|pu
operator|->
name|ts_sec
operator|=
name|crtime
expr_stmt|;
name|pu
operator|->
name|ts_nsec
operator|=
literal|0
expr_stmt|;
return|return
literal|1
return|;
block|}
end_function

begin_function
specifier|static
name|u_int
name|cd9660_chars2ui
parameter_list|(
name|begin
parameter_list|,
name|len
parameter_list|)
name|u_char
modifier|*
name|begin
decl_stmt|;
name|int
name|len
decl_stmt|;
block|{
name|u_int
name|rc
decl_stmt|;
for|for
control|(
name|rc
operator|=
literal|0
init|;
operator|--
name|len
operator|>=
literal|0
condition|;
control|)
block|{
name|rc
operator|*=
literal|10
expr_stmt|;
name|rc
operator|+=
operator|*
name|begin
operator|++
operator|-
literal|'0'
expr_stmt|;
block|}
return|return
name|rc
return|;
block|}
end_function

begin_function
name|int
name|cd9660_tstamp_conv17
parameter_list|(
name|pi
parameter_list|,
name|pu
parameter_list|)
name|u_char
modifier|*
name|pi
decl_stmt|;
name|struct
name|timespec
modifier|*
name|pu
decl_stmt|;
block|{
name|u_char
name|buf
index|[
literal|7
index|]
decl_stmt|;
comment|/* year:"0001"-"9999" -> -1900  */
name|buf
index|[
literal|0
index|]
operator|=
name|cd9660_chars2ui
argument_list|(
name|pi
argument_list|,
literal|4
argument_list|)
operator|-
literal|1900
expr_stmt|;
comment|/* month: " 1"-"12"      -> 1 - 12 */
name|buf
index|[
literal|1
index|]
operator|=
name|cd9660_chars2ui
argument_list|(
name|pi
operator|+
literal|4
argument_list|,
literal|2
argument_list|)
expr_stmt|;
comment|/* day:   " 1"-"31"      -> 1 - 31 */
name|buf
index|[
literal|2
index|]
operator|=
name|cd9660_chars2ui
argument_list|(
name|pi
operator|+
literal|6
argument_list|,
literal|2
argument_list|)
expr_stmt|;
comment|/* hour:  " 0"-"23"      -> 0 - 23 */
name|buf
index|[
literal|3
index|]
operator|=
name|cd9660_chars2ui
argument_list|(
name|pi
operator|+
literal|8
argument_list|,
literal|2
argument_list|)
expr_stmt|;
comment|/* minute:" 0"-"59"      -> 0 - 59 */
name|buf
index|[
literal|4
index|]
operator|=
name|cd9660_chars2ui
argument_list|(
name|pi
operator|+
literal|10
argument_list|,
literal|2
argument_list|)
expr_stmt|;
comment|/* second:" 0"-"59"      -> 0 - 59 */
name|buf
index|[
literal|5
index|]
operator|=
name|cd9660_chars2ui
argument_list|(
name|pi
operator|+
literal|12
argument_list|,
literal|2
argument_list|)
expr_stmt|;
comment|/* difference of GMT */
name|buf
index|[
literal|6
index|]
operator|=
name|pi
index|[
literal|16
index|]
expr_stmt|;
return|return
name|cd9660_tstamp_conv7
argument_list|(
name|buf
argument_list|,
name|pu
argument_list|)
return|;
block|}
end_function

begin_function
name|ino_t
name|isodirino
parameter_list|(
name|isodir
parameter_list|,
name|imp
parameter_list|)
name|struct
name|iso_directory_record
modifier|*
name|isodir
decl_stmt|;
name|struct
name|iso_mnt
modifier|*
name|imp
decl_stmt|;
block|{
name|ino_t
name|ino
decl_stmt|;
name|ino
operator|=
operator|(
name|isonum_733
argument_list|(
name|isodir
operator|->
name|extent
argument_list|)
operator|+
name|isonum_711
argument_list|(
name|isodir
operator|->
name|ext_attr_length
argument_list|)
operator|)
operator|<<
name|imp
operator|->
name|im_bshift
expr_stmt|;
return|return
operator|(
name|ino
operator|)
return|;
block|}
end_function

end_unit

