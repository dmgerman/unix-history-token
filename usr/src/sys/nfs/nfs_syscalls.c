...
begin_comment
comment|/*  * Copyright (c) 1989 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Rick Macklem at The University of Guelph.  *  * %sccs.include.redist.c%  *  *	@(#)nfs_syscalls.c	7.35 (Berkeley) %G%  */
end_comment
...
begin_comment
comment|/*  * Asynchronous I/O daemons for client nfs.  * These babies just pretend to be disk interrupt service routines.  * They are mainly here for read ahead/write behind.  * Never returns unless it fails or gets killed.  */
end_comment
