begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  *  *	@(#)in_var.h	6.2 (Berkeley) 6/8/85  */
end_comment

begin_comment
comment|/*  * Interface address, internet version.  One of these structures  * is allocated for each interface with an internet address.  * The ifaddr structure contains the protocol-independent part  * of the structure and is assumed to be first.  */
end_comment

begin_struct
struct|struct
name|in_ifaddr
block|{
name|struct
name|ifaddr
name|ia_ifa
decl_stmt|;
comment|/* protocol-independent info */
define|#
directive|define
name|ia_addr
value|ia_ifa.ifa_addr
define|#
directive|define
name|ia_broadaddr
value|ia_ifa.ifa_broadaddr
define|#
directive|define
name|ia_dstaddr
value|ia_ifa.ifa_dstaddr
define|#
directive|define
name|ia_ifp
value|ia_ifa.ifa_ifp
name|u_long
name|ia_net
decl_stmt|;
comment|/* network number of interface */
name|u_long
name|ia_netmask
decl_stmt|;
comment|/* mask of net part */
name|u_long
name|ia_subnet
decl_stmt|;
comment|/* subnet number, including net */
name|u_long
name|ia_subnetmask
decl_stmt|;
comment|/* mask of net + subnet */
name|int
name|ia_flags
decl_stmt|;
name|struct
name|in_ifaddr
modifier|*
name|ia_next
decl_stmt|;
comment|/* next in list of internet addresses */
block|}
struct|;
end_struct

begin_comment
comment|/*  * Given a pointer to an in_ifaddr (ifaddr),  * return a pointer to the addr as a sockadd_in.  */
end_comment

begin_define
define|#
directive|define
name|IA_SIN
parameter_list|(
name|ia
parameter_list|)
value|((struct sockaddr_in *)(&((struct in_ifaddr *)ia)->ia_addr))
end_define

begin_define
define|#
directive|define
name|IA_B_SIN
parameter_list|(
name|ia
parameter_list|)
value|((struct sockaddr_in *)(&((struct in_ifaddr *)ia)->ia_broadaddr))
end_define

begin_comment
comment|/*  * in_ifaddr to in_addr  */
end_comment

begin_define
define|#
directive|define
name|IA_INADDR
parameter_list|(
name|ia
parameter_list|)
value|(IA_SIN(ia)->sin_addr)
end_define

begin_define
define|#
directive|define
name|IA_B_INADDR
parameter_list|(
name|ia
parameter_list|)
value|(IA_B_SIN(ia)->sin_addr)
end_define

begin_comment
comment|/*  * ia_flags  */
end_comment

begin_define
define|#
directive|define
name|IFA_ROUTE
value|0x01
end_define

begin_comment
comment|/* routing entry installed */
end_comment

begin_comment
comment|/*  * protocol switch table built by IP at initialization time  */
end_comment

begin_struct
struct|struct
name|ipswitch
block|{
name|struct
name|protosw
modifier|*
name|ipsw_user
decl_stmt|;
comment|/* for normal user packets passed up */
name|struct
name|protosw
modifier|*
name|ipsw_raw
decl_stmt|;
comment|/* for raw packets passed up */
name|int
name|ipsw_hlen
decl_stmt|;
block|}
struct|;
end_struct

begin_decl_stmt
specifier|extern
name|struct
name|ipswitch
name|ipsw
index|[]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|u_char
name|inetctlerrmap
index|[]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Internet protocol statistics structure (should be at the head  * of protocol dependent structures).  */
end_comment

begin_struct
struct|struct
name|in_stat
block|{
name|int
name|in_total
decl_stmt|;
comment|/* total packets seen */
name|int
name|in_badsum
decl_stmt|;
comment|/* packets with bad cksums */
name|int
name|in_tooshort
decl_stmt|;
comment|/* bad sizes */
name|int
name|in_drops
decl_stmt|;
comment|/* packets no one (except raw) wanted */
block|}
struct|;
end_struct

begin_ifdef
ifdef|#
directive|ifdef
name|KERNEL
end_ifdef

begin_decl_stmt
specifier|extern
name|struct
name|in_ifaddr
modifier|*
name|in_ifaddr
decl_stmt|;
end_decl_stmt

begin_function_decl
specifier|extern
name|struct
name|in_ifaddr
modifier|*
name|in_iawithaddr
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
specifier|extern
name|struct
name|in_ifaddr
modifier|*
name|in_iawithnet
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
specifier|extern
name|struct
name|in_ifaddr
modifier|*
name|in_iafromif
parameter_list|()
function_decl|;
end_function_decl

begin_endif
endif|#
directive|endif
end_endif

end_unit

