...
begin_comment
comment|/*  * Timer routine to keep priming the IMP until it sends  * us the noops we need.  Since we depend on the host and  * imp values returned in the noop messages, we must wait  * for them before we allow any outgoing traffic.  */
end_comment
