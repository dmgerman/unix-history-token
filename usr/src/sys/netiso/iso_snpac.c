...
begin_comment
comment|/*	@(#)iso_snpac.c	7.7 (Berkeley) %G% */
end_comment
...
condition|)
block|{
comment|/* We have just cloned a route to a host presumed to be on 			   our local net */
name|gate
operator|->
name|sdl
operator|.
...
condition|)
block|{
comment|/* 			 * Case 1: This route may come from a route to iface with mask 			 * or from a default IS. 			 */
name|rt
operator|->
name|rt_gateway
operator|=
...
condition|)
block|{
comment|/* 			 * Case 2:  This route may come from ESIS or a manual route 			 * add with a LL address. 			 */
name|insque
argument_list|(
name|lc
argument_list|,
...
block|{
comment|/* 			 * Case 3:  Told to add route via a gateway; 			 * try to provoke LL route to exist for gateway by cloning. 			 */
if|if
condition|(
name|gate
operator|->
