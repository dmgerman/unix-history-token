...
name|printf
argument_list|(
literal|"%x : Len %x Of %x A %x Nx %x Tp %x\n"
argument_list|,
name|n
argument_list|,
name|n
...
argument_list|)
expr_stmt|;
comment|/*  	 * mtod doesn't work for cluster-type mbufs, hence this disaster check:  	 */
if|if
condition|(
name|n
operator|->
...
name|panic
argument_list|(
literal|"tp_rcvoob: unexpected cluster "
argument_list|)
expr_stmt|;
name|m
operator|->
...
name|MMINOFF
expr_stmt|;
comment|/* restore it to its proper value */
name|IFDEBUG
argument_list|(
argument|D_XPD
argument_list|)
...
condition|)
block|{
comment|/* anything already in this sockbuf? */
if|if
condition|(
name|so
operator|->
...
expr_stmt|;
name|ENDDEBUG
comment|/* stick an xpd mark in the normal data queue 	 * make sure we have enough mbufs before we stick anything into any queues 	 */
name|MGET
argument_list|(
name|xmark
argument_list|,
...
expr_stmt|;
block|{
comment|/* stash the xpd sequence number in the mark */
name|SeqNum
modifier|*
name|Xuna
init|=
...
begin_comment
comment|/*  * CALLED FROM:  *  the socket routines  * FUNCTION and ARGUMENTS:  * 	Handles all "user requests" except the [gs]ockopts() requests.  * 	The argument (req) is the request type (PRU*),   * 	(m) is an mbuf chain, generally used for send and  * 	receive type requests only.  * 	(nam) is used for addresses usually, in particular for the bind request.  *   * 	The last argument (rights in most usrreq()s) has been stolen for   * 	returning flags values.  Since rights can't be passed around w/ tp,  * 	this field is used only for RCVOOB user requests, and is assumed  * 	to be either 0 (as soreceive() would have it) or a ptr to the int flags  * 	(as recvv()'s version of soreceive() would have it  */
end_comment
...
name|printf
argument_list|(
literal|"enter usrreq %d Xrcv sb_mb 0x%x is on freelist!\n"
argument_list|,
name|req
argument_list|,
name|n
...
condition|)
block|{
comment|/* note: this suffix is independent of the extended suffix */
if|if
condition|(
name|error
operator|=
...
name|TPPTmisc
argument_list|,
literal|"PRU_CONNECT: so *SHORT_LSUFXP(tpcb) 0x%x lsuflen 0x%x, class 0x%x"
argument_list|,
name|tpcb
operator|->
name|tp_sock
...
condition|)
block|{
comment|/* didn't set an extended suffix */
name|tpcb
operator|->
name|tp_lsuffixlen
operator|=
...
begin_comment
comment|/* all this garbage is to keep accept from returning 		 * before the 3-way handshake is done in class 4. 		 * it'll have to be modified for other classes  		 */
end_comment
...
name|printf
argument_list|(
literal|"PRU_ACCEPT so_error 0x%x\n"
argument_list|,
name|so
operator|->
name|so_error
...
argument_list|)
expr_stmt|;
comment|/* doesn't cover the case where the suffix is extended - 				 * grotesque - the user *has* to do the getsockopt */
break|break;
block|}
name|IFDEBUG
argument_list|(
...
begin_comment
comment|/* The last mbuf has type TPMT_EOT so it will never be compressed 			 * with TPMT_DATA mbufs, but if this was an EOTSDU request w/o 			 * any data, the only way to keep this mbuf from being thrown 			 * away is to link it through the m_act field 			 * We are ASSUMING that if there are any data at all with this 			 * request, the last mbuf will be non-empty!!! 			 */
end_comment
...
name|TPMT_EOT
condition|)
comment|/* first mbuf in chain is EOT? */
name|sbappendrecord
argument_list|(
name|sb
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* to keep 2 TPMT_EOTs from being 												compressed */
else|else
name|sbappend
argument_list|(
name|sb
...
name|printf
argument_list|(
literal|"returning from tp_usrreq(so 0x%x) error 0x%x\n"
argument_list|,
name|so
argument_list|,
name|error
