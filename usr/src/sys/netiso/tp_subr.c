...
begin_comment
comment|/*-  * Copyright (c) 1991 The Regents of the University of California.  * All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)tp_subr.c	7.14 (Berkeley) %G%  */
end_comment
...
expr_stmt|;
block|}
comment|/* Compute smoothed round trip time. 		 * Only measure rtt for tp_snduna if acked and the data 		 * were not retransmitted. 		 */
if|if
condition|(
name|tpcb
operator|->
...
name|ENDTRACE
block|{
comment|/* Update data retransmission timer based on the smoothed 				 * round trip time, peer ack time, and the pseudo-arbitrary 				 * number 2. 				 * new ticks: (avg rtt + 4*dev) 				 * rtt, rtv are in hz-ticks, 				 * and slowtimo-ticks are hz / 2; 				 * We want no less than peer ack time and no less than 2 				 */
name|int
name|rtt
init|=
name|tpcb
...
name|printf
argument_list|(
literal|"tp_sbdroping %d up through seq 0x%x\n"
argument_list|,
name|i
argument_list|,
name|seq
...
begin_comment
comment|/*  * CALLED FROM:  * 	tp.trans on user send request, arrival of AK and arrival of XAK  * FUNCTION and ARGUMENTS:  * 	Emits tpdus starting at sequence number (lowseq).  * 	Emits until a) runs out of data, or  b) runs into an XPD mark, or  * 			c) it hits seq number (highseq)  *  * 	If you want XPD to buffer> 1 du per socket buffer, you can  * 	modifiy this to issue XPD tpdus also, but then it'll have  * 	to take some argument(s) to distinguish between the type of DU to  * 	hand tp_emit.  *  * 	When something is sent for the first time, its time-of-send  * 	is stashed (the last RTT_NUM of them are stashed).  When the  * 	ack arrives, the smoothed round-trip time is figured using this value.  * RETURN VALUE:  * 	the highest seq # sent successfully.  */
end_comment
...
expr_stmt|;
name|ENDTRACE
comment|/* stop sending here because there are unacked XPD present 		 */
name|IncStat
parameter_list|(
...
condition|)
block|{
comment|/*  		 * In all cases, m points to mbuf containing first octet to be 		 * sent in the tpdu AFTER the one we're going to send now, 		 * or else m is null. 		 * 		 * The chain we're working on now begins at mb and has length<len>. 		 */
name|eotsdu_reached
operator|=
operator|(
name|mb
...
name|TPPTmisc
argument_list|,
literal|"SEND BF: maxsize totlen eotsdu"
argument_list|,
name|maxsize
argument_list|,
name|totlen
