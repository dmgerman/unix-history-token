...
begin_comment
comment|/* Deroff command -- strip troff, eqn, and Tbl sequences from a file.  Has one flag argument, -w, to cause output one word per line rather than in the original format. Deroff follows .so and .nx commands, removes contents of macro definitions, equations (both .EQ ... .EN and $...$), Tbl command sequences, and Troff backslash constructions.  All input is through the C macro; the most recently read character is in c. */
end_comment
...
begin_block
block|{
comment|/* do { SKIP; } 	while(C!='.' || C!='.');	/* look for  .EN */
name|SKIP
expr_stmt|;
name|inmacro
operator|=
