...
directive|define
name|ftstbit
value|asm("	ashl	$-2,r11,r3");\ 			asm("	bbcs	r3,_bitmapi,$1");\ 			asm("	.byte	4");
end_define
...
name|v
parameter_list|)
value|if((v)>= (lispval)beginsweep) markdp(v);
end_define
...
begin_comment
comment|/** get_more_space(type_struct,purep) ***********************************/
end_comment
...
begin_comment
comment|/*  If purep is TRUE, then pure space is allocated.			*/
end_comment
...
begin_comment
comment|/*  get_more_space(type_struct,FALSE)						*/
end_comment
...
begin_block
block|{
comment|/* the error  messages in this routine shouldn't print out the 	 * value since it is probably junk 	 */
specifier|register
name|lispval
name|temp
decl_stmt|,
...
argument_list|)
expr_stmt|;
comment|/* 	enddat = (int)datalim>> 8; 	for(bvalue=0; bvalue< (int)enddat ; ++bvalue) 	{ 	     bitmapq[bvalue] = zeroq;  	} 	*/
...
comment|/* dont mark these, they are pure 	 *point = bind_lists; 	 *while((start = point) != (int *)CNIL) { 	 *	if(debugin) ERDB("once "); 	 *	while( *start != -1 ) 	 *	{ 	 *		markdp(*start); 	 *		start++; 	 *	} 	 *	point = (int *)*(point-1); 	 *} 	 */
