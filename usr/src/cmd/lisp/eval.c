...
name|np
decl_stmt|;
comment|/*  struct argent *poplbot;  Why this here? - M. Marcus */
name|struct
name|nament
modifier|*
name|oldbnp
...
argument_list|()
decl_stmt|;
comment|/*debugging      if (rsetsw&& rsetatom->a.clb != nil) { 	printf("Eval:"); 	printr(a,stdout); 	printf("\nrsetsw: %d evalhsw: %d\n", rsetsw, evalhsw); 	printf("*rset: "); 	printr(rsetatom->a.clb,stdout); 	printf(" evalhook: "); 	printr(evalhatom->a.clb,stdout); 	printf(" evalhook call flag^G: "); 	printr(evalhcall->a.clb, stdout); 	fflush(stdout);      };  */
...
name|tatom
condition|)
comment|/*if this is a call to evalhook, don't call evalhook 			  function, but clobber evalhcall atom, so recursive  			  calls to eval cause evalhook function to fire. */
name|evalhcall
operator|->
name|a
operator|.
...
comment|/* atrocity to avoid call to nargs() */
comment|/*define nargs() (* (char *) (&fcn -1))*/
name|oldlbot
operator|=
name|lbot
expr_stmt|;
