...
index|[]
init|=
literal|"$Id: protg.c,v 1.31 1992/03/31 19:39:08 ian Rel $"
decl_stmt|;
end_decl_stmt
...
name|LOG_ERROR
argument_list|,
literal|"Illegal packet size %d for 'g' protocol"
argument_list|,
name|iGrequest_packsize
argument_list|)
expr_stmt|;
...
name|DEBUG_PROTO
argument_list|,
literal|"fgstart: Protocol started; packsize %d, winsize %d"
argument_list|,
name|iGremote_packsize
argument_list|,
name|iGremote_winsize
...
name|LOG_NORMAL
argument_list|,
literal|"Protocol 'g' packets: sent %ld, resent %ld, received %ld"
argument_list|,
name|cGsent_packets
argument_list|,
name|cGresent_packets
...
begin_comment
comment|/* Send out a data packet.  This computes the checksum, sets up the    header, and sends the packet out.  The argument should point to the    return value of zggetspace.  */
end_comment
...
name|cshort
decl_stmt|;
comment|/* We have to move the data within the packet, 	     unfortunately.  It's tough to see any way around this 	     without going to some sort of iovec structure.  It only 	     happens once per file transfer.  It would also be nice if 	     we computed the checksum as we move.  We zero out the 	     unused bytes, since it makes people happy.  */
name|itt
operator|=
name|SHORTDATA
expr_stmt|;
...
begin_comment
comment|/* Recompute the control byte and checksum of a packet so that it    includes the correct packet acknowledgement.  This is called    when a packet is retransmitted to make sure the retransmission    does not confuse the other side.  */
end_comment
...
begin_comment
comment|/* Process existing data.  Set *pfexit to TRUE if a file or a command    has been completely received.  Return FALSE on error.  */
end_comment
...
decl_stmt|;
block|{
comment|/* Don't ack incoming data (since this is called when a file is      being sent and the acks can be merged onto the outgoing data),      don't return after receiving a control packet, and don't bother      to report how much more data is needed or whether any packets      were found.  */
return|return
name|fgprocess_data
argument_list|(
name|FALSE
...
name|DEBUG_PROTO
argument_list|,
literal|"fgwait_for_packet: Need %d bytes"
argument_list|,
name|cneed
argument_list|)
expr_stmt|;
...
block|{
comment|/* The read timed out.  If we're looking for a control 	     packet, assume we're looking for an ack and send the last 	     unacknowledged packet again.  Otherwise, send an RJ with 	     the last packet we received correctly.  */
operator|++
name|ctimeouts
expr_stmt|;
if|if
...
name|LOG_ERROR
argument_list|,
literal|"Too many 'g' protocol errors"
argument_list|)
expr_stmt|;
return|return
name|FALSE
...
directive|endif
block|}
comment|/* If *pfexit gets set by the first batch of data, and there 	     is still more data, it must be the case that we are 	     accumulating a command and encountered a null byte, so we 	     can ignore the second batch of data.  */
if|if
condition|(
operator|!
name|fgot_data
...
name|RR
case|:
comment|/* Acknowledge receipt of a packet.  This was already handled 	     above.  */
break|break;
case|case
name|INITC
case|:
...
expr_stmt|;
block|}
comment|/* Add the next character to ichk1.  */
name|b
operator|=
operator|*
name|z
...
name|b
expr_stmt|;
comment|/* Add ichk1 xor the character position in the buffer counting from 	 the back to ichk2.  */
name|ichk2
operator|+=
name|ichk1
operator|^
name|c
expr_stmt|;
comment|/* If the character was zero, or adding it to ichk1 caused an 	 overflow, xor ichk2 to ichk1.  */
if|if
condition|(
name|b
operator|==
