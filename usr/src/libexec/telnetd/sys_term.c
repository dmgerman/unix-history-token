...
directive|ifdef
name|LINEMODE
comment|/* 	 * Opening the slave side may cause initilization of the 	 * kernel tty structure.  We need remember whether or not 	 * linemode was turned on, so that we can re-set it if we 	 * need to. 	 */
name|int
name|waslm
init|=
name|tty_linemode
