...
begin_comment
unit|netflush() { 	int n;  	if ((n = nfrontp - nbackp)> 0) 		n = write(net, nbackp, n); 	if (n< 0) { 		if (errno == EWOULDBLOCK) 			return;
comment|/* should blow this guy away... */
end_comment

begin_else
unit|return; 	} 	nbackp += n; 	if (nbackp == nfrontp) 		nbackp = nfrontp = netobuf; }
else|#
directive|else
end_else
