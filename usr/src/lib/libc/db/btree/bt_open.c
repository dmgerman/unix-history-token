begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1990 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Mike Olson.  *  * %sccs.include.redist.c%  */
end_comment

begin_if
if|#
directive|if
name|defined
argument_list|(
name|LIBC_SCCS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
end_if

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)bt_open.c	5.10 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* LIBC_SCCS and not lint */
end_comment

begin_comment
comment|/*  * Implementation of btree access method for 4.4BSD.  *  * The design here was originally based on that of the btree access method  * used in the Postgres database system at UC Berkeley.  This implementation  * is wholly independent of the Postgres code.  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<limits.h>
end_include

begin_define
define|#
directive|define
name|__DBINTERFACE_PRIVATE
end_define

begin_include
include|#
directive|include
file|<db.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|"btree.h"
end_include

begin_decl_stmt
specifier|static
name|int
name|nroot
name|__P
argument_list|(
operator|(
name|BTREE
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|tmp
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * __BT_OPEN -- Open a btree.  *  * Creates and fills a DB struct, and calls the routine that actually  * opens the btree.  *  * Parameters:  *	fname:	filename (NULL for in-memory trees)  *	flags:	open flag bits  *	mode:	open permission bits  *	b:	BTREEINFO pointer  *  * Returns:  *	NULL on failure, pointer to DB on success.  *  */
end_comment

begin_function
name|DB
modifier|*
name|__bt_open
parameter_list|(
name|fname
parameter_list|,
name|flags
parameter_list|,
name|mode
parameter_list|,
name|openinfo
parameter_list|)
specifier|const
name|char
modifier|*
name|fname
decl_stmt|;
name|int
name|flags
decl_stmt|,
name|mode
decl_stmt|;
specifier|const
name|BTREEINFO
modifier|*
name|openinfo
decl_stmt|;
block|{
name|BTREE
modifier|*
name|t
decl_stmt|;
name|DB
modifier|*
name|dbp
decl_stmt|;
name|BTMETA
name|m
decl_stmt|;
name|BTREEINFO
name|b
decl_stmt|;
name|pgno_t
name|ncache
decl_stmt|;
name|struct
name|stat
name|sb
decl_stmt|;
name|int
name|nr
decl_stmt|;
comment|/* 	 * Intention is to make sure all of the user's selections are okay 	 * here and then use them without checking.  Can't be complete, since 	 * we don't know the right page size, lorder or flags until the backing 	 * file is opened.  Also, the file's page size can cause the cachesize 	 * to change. 	 */
if|if
condition|(
name|openinfo
condition|)
block|{
name|b
operator|=
operator|*
name|openinfo
expr_stmt|;
comment|/* Flags: R_DUP. */
if|if
condition|(
name|b
operator|.
name|flags
operator|&&
name|b
operator|.
name|flags
operator|!=
name|R_DUP
condition|)
goto|goto
name|einval
goto|;
comment|/* 		 * Page size must be index_t aligned and>= MINPSIZE.  Default 		 * page size is set farther on, based on the underlying file 		 * transfer size. 		 */
if|if
condition|(
name|b
operator|.
name|psize
operator|&&
operator|(
name|b
operator|.
name|psize
operator|<
name|MINPSIZE
operator|||
name|b
operator|.
name|psize
operator|>
name|MAX_PAGE_OFFSET
operator|||
name|b
operator|.
name|psize
operator|&
sizeof|sizeof
argument_list|(
name|index_t
argument_list|)
operator|-
literal|1
operator|)
condition|)
goto|goto
name|einval
goto|;
ifdef|#
directive|ifdef
name|notdef
if|if
condition|(
name|b
operator|.
name|maxkeypage
operator|&&
name|b
operator|.
name|maxkeypage
operator|<
literal|1
condition|)
goto|goto
name|einval
goto|;
if|if
condition|(
name|b
operator|.
name|minkeypage
condition|)
block|{
if|if
condition|(
name|b
operator|.
name|minkeypage
operator|<
literal|2
condition|)
goto|goto
name|einval
goto|;
block|}
else|else
name|b
operator|.
name|minkeypage
operator|=
literal|2
expr_stmt|;
else|#
directive|else
name|b
operator|.
name|maxkeypage
operator|=
name|DEFMAXKEYPAGE
expr_stmt|;
name|b
operator|.
name|minkeypage
operator|=
name|DEFMINKEYPAGE
expr_stmt|;
endif|#
directive|endif
comment|/* If no comparison, use default comparison and prefix. */
if|if
condition|(
name|b
operator|.
name|compare
operator|==
name|NULL
condition|)
block|{
name|b
operator|.
name|compare
operator|=
name|__bt_defcmp
expr_stmt|;
if|if
condition|(
name|b
operator|.
name|prefix
operator|==
name|NULL
condition|)
name|b
operator|.
name|prefix
operator|=
name|__bt_defpfx
expr_stmt|;
block|}
if|if
condition|(
name|b
operator|.
name|lorder
operator|==
literal|0
condition|)
name|b
operator|.
name|lorder
operator|=
name|BYTE_ORDER
expr_stmt|;
elseif|else
if|if
condition|(
name|b
operator|.
name|lorder
operator|!=
name|BIG_ENDIAN
operator|&&
name|b
operator|.
name|lorder
operator|!=
name|LITTLE_ENDIAN
condition|)
goto|goto
name|einval
goto|;
block|}
else|else
block|{
name|b
operator|.
name|flags
operator|=
literal|0
expr_stmt|;
name|b
operator|.
name|maxkeypage
operator|=
name|DEFMAXKEYPAGE
expr_stmt|;
name|b
operator|.
name|minkeypage
operator|=
name|DEFMINKEYPAGE
expr_stmt|;
name|b
operator|.
name|compare
operator|=
name|__bt_defcmp
expr_stmt|;
name|b
operator|.
name|prefix
operator|=
name|__bt_defpfx
expr_stmt|;
name|b
operator|.
name|lorder
operator|=
name|BYTE_ORDER
expr_stmt|;
block|}
comment|/* Allocate and initialize DB and BTREE structures. */
if|if
condition|(
operator|(
name|t
operator|=
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
name|BTREE
argument_list|)
argument_list|)
operator|)
operator|==
name|NULL
condition|)
goto|goto
name|err
goto|;
name|t
operator|->
name|bt_fd
operator|=
operator|-
literal|1
expr_stmt|;
comment|/* Don't close unopened fd on error. */
if|if
condition|(
operator|(
name|t
operator|->
name|bt_dbp
operator|=
name|dbp
operator|=
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
name|DB
argument_list|)
argument_list|)
operator|)
operator|==
name|NULL
condition|)
goto|goto
name|err
goto|;
name|t
operator|->
name|bt_bcursor
operator|.
name|pgno
operator|=
name|P_INVALID
expr_stmt|;
name|t
operator|->
name|bt_bcursor
operator|.
name|index
operator|=
literal|0
expr_stmt|;
name|t
operator|->
name|bt_stack
operator|=
name|NULL
expr_stmt|;
name|t
operator|->
name|bt_sp
operator|=
name|t
operator|->
name|bt_maxstack
operator|=
literal|0
expr_stmt|;
name|t
operator|->
name|bt_kbuf
operator|=
name|t
operator|->
name|bt_dbuf
operator|=
name|NULL
expr_stmt|;
name|t
operator|->
name|bt_kbufsz
operator|=
name|t
operator|->
name|bt_dbufsz
operator|=
literal|0
expr_stmt|;
name|t
operator|->
name|bt_maxkeypage
operator|=
name|b
operator|.
name|maxkeypage
expr_stmt|;
name|t
operator|->
name|bt_minkeypage
operator|=
name|b
operator|.
name|minkeypage
expr_stmt|;
name|t
operator|->
name|bt_order
operator|=
name|NOT
expr_stmt|;
name|t
operator|->
name|bt_cmp
operator|=
name|b
operator|.
name|compare
expr_stmt|;
name|t
operator|->
name|bt_pfx
operator|=
name|b
operator|.
name|prefix
expr_stmt|;
name|dbp
operator|->
name|type
operator|=
name|DB_BTREE
expr_stmt|;
name|dbp
operator|->
name|internal
operator|=
name|t
expr_stmt|;
name|dbp
operator|->
name|close
operator|=
name|__bt_close
expr_stmt|;
name|dbp
operator|->
name|del
operator|=
name|__bt_delete
expr_stmt|;
name|dbp
operator|->
name|get
operator|=
name|__bt_get
expr_stmt|;
name|dbp
operator|->
name|put
operator|=
name|__bt_put
expr_stmt|;
name|dbp
operator|->
name|seq
operator|=
name|__bt_seq
expr_stmt|;
name|dbp
operator|->
name|sync
operator|=
name|__bt_sync
expr_stmt|;
comment|/* 	 * If no file name was supplied, this is an in-memory btree and we 	 * open a backing temporary file.  Otherwise, it's a disk-based tree. 	 */
if|if
condition|(
name|fname
condition|)
block|{
define|#
directive|define
name|USEFLAGS
value|(O_CREAT|O_EXCL|O_RDONLY|O_RDWR|O_TRUNC|O_WRONLY)
if|if
condition|(
operator|(
name|t
operator|->
name|bt_fd
operator|=
name|open
argument_list|(
name|fname
argument_list|,
name|flags
operator|&
name|USEFLAGS
argument_list|,
name|mode
argument_list|)
operator|)
operator|<
literal|0
condition|)
goto|goto
name|err
goto|;
if|if
condition|(
operator|(
name|flags
operator|&
name|O_ACCMODE
operator|)
operator|==
name|O_RDONLY
condition|)
name|SET
argument_list|(
name|t
argument_list|,
name|BTF_RDONLY
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
operator|(
name|t
operator|->
name|bt_fd
operator|=
name|tmp
argument_list|()
operator|)
operator|==
operator|-
literal|1
condition|)
goto|goto
name|err
goto|;
name|SET
argument_list|(
name|t
argument_list|,
name|BTF_INMEM
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|fcntl
argument_list|(
name|t
operator|->
name|bt_fd
argument_list|,
name|F_SETFL
argument_list|,
literal|1
argument_list|)
operator|==
operator|-
literal|1
condition|)
goto|goto
name|err
goto|;
if|if
condition|(
name|fstat
argument_list|(
name|t
operator|->
name|bt_fd
argument_list|,
operator|&
name|sb
argument_list|)
condition|)
goto|goto
name|err
goto|;
if|if
condition|(
name|sb
operator|.
name|st_size
condition|)
block|{
name|nr
operator|=
name|read
argument_list|(
name|t
operator|->
name|bt_fd
argument_list|,
operator|&
name|m
argument_list|,
sizeof|sizeof
argument_list|(
name|BTMETA
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|nr
operator|<
literal|0
condition|)
goto|goto
name|err
goto|;
if|if
condition|(
name|nr
operator|!=
sizeof|sizeof
argument_list|(
name|BTMETA
argument_list|)
condition|)
goto|goto
name|eftype
goto|;
comment|/* 		 * Read in the meta-data.  This can change the notion of what 		 * the lorder, page size and flags are, and, when the page size 		 * changes the cachesize value can change as well. 		 * 		 * Lorder is always stored in host-independent format. 		 */
name|NTOHL
argument_list|(
name|m
operator|.
name|m_lorder
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|.
name|m_lorder
operator|!=
name|BIG_ENDIAN
operator|&&
name|m
operator|.
name|m_lorder
operator|!=
name|LITTLE_ENDIAN
condition|)
goto|goto
name|eftype
goto|;
if|if
condition|(
name|m
operator|.
name|m_lorder
operator|!=
name|BYTE_ORDER
condition|)
block|{
name|BLSWAP
argument_list|(
name|m
operator|.
name|m_magic
argument_list|)
expr_stmt|;
name|BLSWAP
argument_list|(
name|m
operator|.
name|m_version
argument_list|)
expr_stmt|;
name|BLSWAP
argument_list|(
name|m
operator|.
name|m_psize
argument_list|)
expr_stmt|;
name|BLSWAP
argument_list|(
name|m
operator|.
name|m_free
argument_list|)
expr_stmt|;
name|BLSWAP
argument_list|(
name|m
operator|.
name|m_nrecs
argument_list|)
expr_stmt|;
name|BLSWAP
argument_list|(
name|m
operator|.
name|m_flags
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|m
operator|.
name|m_magic
operator|!=
name|BTREEMAGIC
operator|||
name|m
operator|.
name|m_version
operator|!=
name|BTREEVERSION
condition|)
goto|goto
name|eftype
goto|;
if|if
condition|(
name|m
operator|.
name|m_psize
operator|<
name|MINPSIZE
operator|||
name|m
operator|.
name|m_psize
operator|>
name|MAX_PAGE_OFFSET
operator|||
name|m
operator|.
name|m_psize
operator|&
sizeof|sizeof
argument_list|(
name|index_t
argument_list|)
operator|-
literal|1
condition|)
goto|goto
name|eftype
goto|;
if|if
condition|(
name|m
operator|.
name|m_flags
operator||
operator|~
name|SAVEMETA
condition|)
goto|goto
name|eftype
goto|;
name|b
operator|.
name|psize
operator|=
name|m
operator|.
name|m_psize
expr_stmt|;
name|t
operator|->
name|bt_flags
operator|=
name|m
operator|.
name|m_flags
expr_stmt|;
name|t
operator|->
name|bt_free
operator|=
name|m
operator|.
name|m_free
expr_stmt|;
name|t
operator|->
name|bt_lorder
operator|=
name|m
operator|.
name|m_lorder
expr_stmt|;
name|t
operator|->
name|bt_nrecs
operator|=
name|m
operator|.
name|m_nrecs
expr_stmt|;
block|}
else|else
block|{
comment|/* 		 * Set the page size to the best value for I/O to this file. 		 * Don't overflow the page offset type. 		 */
if|if
condition|(
name|b
operator|.
name|psize
operator|==
literal|0
condition|)
block|{
name|b
operator|.
name|psize
operator|=
name|sb
operator|.
name|st_blksize
expr_stmt|;
if|if
condition|(
name|b
operator|.
name|psize
operator|<
name|MINPSIZE
condition|)
name|b
operator|.
name|psize
operator|=
name|MINPSIZE
expr_stmt|;
if|if
condition|(
name|b
operator|.
name|psize
operator|>
name|MAX_PAGE_OFFSET
condition|)
name|b
operator|.
name|psize
operator|=
name|MAX_PAGE_OFFSET
expr_stmt|;
block|}
name|t
operator|->
name|bt_flags
operator|=
name|b
operator|.
name|flags
operator|&
name|R_DUP
condition|?
literal|0
else|:
name|BTF_NODUPS
expr_stmt|;
name|t
operator|->
name|bt_free
operator|=
name|P_INVALID
expr_stmt|;
name|t
operator|->
name|bt_lorder
operator|=
name|b
operator|.
name|lorder
expr_stmt|;
name|t
operator|->
name|bt_nrecs
operator|=
literal|0
expr_stmt|;
name|SET
argument_list|(
name|t
argument_list|,
name|BTF_METADIRTY
argument_list|)
expr_stmt|;
block|}
name|t
operator|->
name|bt_psize
operator|=
name|b
operator|.
name|psize
expr_stmt|;
comment|/* Set the cache size; must be a multiple of the page size. */
if|if
condition|(
name|b
operator|.
name|cachesize
operator|&&
name|b
operator|.
name|cachesize
operator|&
name|b
operator|.
name|psize
operator|-
literal|1
condition|)
name|b
operator|.
name|cachesize
operator|+=
operator|(
operator|~
name|b
operator|.
name|cachesize
operator|&
name|b
operator|.
name|psize
operator|-
literal|1
operator|)
operator|+
literal|1
expr_stmt|;
if|if
condition|(
name|b
operator|.
name|cachesize
operator|<
name|b
operator|.
name|psize
operator|*
name|MINCACHE
condition|)
name|b
operator|.
name|cachesize
operator|=
name|b
operator|.
name|psize
operator|*
name|MINCACHE
expr_stmt|;
comment|/* Calculate number of pages to cache. */
name|ncache
operator|=
operator|(
name|b
operator|.
name|cachesize
operator|+
name|t
operator|->
name|bt_psize
operator|-
literal|1
operator|)
operator|/
name|t
operator|->
name|bt_psize
expr_stmt|;
comment|/* 	 * The btree data structure requires that at least two keys can fit 	 * on a page, but other than that there's no fixed requirement.  The 	 * user can specify the minimum number per page, and we translate 	 * that into the maximum number of bytes a key can use before being 	 * placed on an overflow page. 	 */
name|t
operator|->
name|bt_minkeypage
operator|=
operator|(
name|t
operator|->
name|bt_psize
operator|-
name|BTDATAOFF
operator|)
operator|/
name|b
operator|.
name|minkeypage
expr_stmt|;
comment|/* Initialize the buffer pool. */
if|if
condition|(
operator|(
name|t
operator|->
name|bt_mp
operator|=
name|mpool_open
argument_list|(
name|NULL
argument_list|,
name|t
operator|->
name|bt_fd
argument_list|,
name|t
operator|->
name|bt_psize
argument_list|,
name|ncache
argument_list|)
operator|)
operator|==
name|NULL
condition|)
goto|goto
name|err
goto|;
name|mpool_filter
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|__bt_pgin
argument_list|,
name|__bt_pgout
argument_list|,
name|t
argument_list|)
expr_stmt|;
comment|/* Create a root page if new tree. */
if|if
condition|(
name|nroot
argument_list|(
name|t
argument_list|)
operator|==
name|RET_ERROR
condition|)
goto|goto
name|err
goto|;
return|return
operator|(
name|dbp
operator|)
return|;
name|einval
label|:
name|errno
operator|=
name|EINVAL
expr_stmt|;
goto|goto
name|err
goto|;
name|eftype
label|:
name|errno
operator|=
name|EFTYPE
expr_stmt|;
goto|goto
name|err
goto|;
name|err
label|:
if|if
condition|(
name|t
condition|)
block|{
if|if
condition|(
name|t
operator|->
name|bt_dbp
condition|)
name|free
argument_list|(
name|t
operator|->
name|bt_dbp
argument_list|)
expr_stmt|;
if|if
condition|(
name|t
operator|->
name|bt_fd
operator|!=
operator|-
literal|1
condition|)
operator|(
name|void
operator|)
name|close
argument_list|(
name|t
operator|->
name|bt_fd
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|t
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|NULL
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * NROOT -- Create the root of a new tree.  *  * Parameters:  *	t:	tree  *  * Returns:  *	RET_ERROR, RET_SUCCESS  */
end_comment

begin_function
specifier|static
name|int
name|nroot
parameter_list|(
name|t
parameter_list|)
name|BTREE
modifier|*
name|t
decl_stmt|;
block|{
name|PAGE
modifier|*
name|meta
decl_stmt|,
modifier|*
name|root
decl_stmt|;
name|pgno_t
name|npg
decl_stmt|;
if|if
condition|(
operator|(
name|meta
operator|=
name|mpool_get
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|meta
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
operator|(
name|RET_SUCCESS
operator|)
return|;
block|}
if|if
condition|(
name|errno
operator|!=
name|EINVAL
condition|)
return|return
operator|(
name|RET_ERROR
operator|)
return|;
if|if
condition|(
operator|(
name|meta
operator|=
name|mpool_new
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
operator|&
name|npg
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
name|RET_ERROR
operator|)
return|;
if|if
condition|(
operator|(
name|root
operator|=
name|mpool_new
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
operator|&
name|npg
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
name|RET_ERROR
operator|)
return|;
if|if
condition|(
name|npg
operator|!=
name|P_ROOT
condition|)
return|return
operator|(
name|RET_ERROR
operator|)
return|;
name|root
operator|->
name|pgno
operator|=
name|npg
expr_stmt|;
name|root
operator|->
name|prevpg
operator|=
name|root
operator|->
name|nextpg
operator|=
name|P_INVALID
expr_stmt|;
name|root
operator|->
name|lower
operator|=
name|BTDATAOFF
expr_stmt|;
name|root
operator|->
name|upper
operator|=
name|t
operator|->
name|bt_psize
expr_stmt|;
name|root
operator|->
name|flags
operator|=
name|P_BLEAF
expr_stmt|;
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|meta
argument_list|,
name|MPOOL_DIRTY
argument_list|)
expr_stmt|;
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|root
argument_list|,
name|MPOOL_DIRTY
argument_list|)
expr_stmt|;
return|return
operator|(
name|RET_SUCCESS
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|tmp
parameter_list|()
block|{
name|sigset_t
name|set
decl_stmt|,
name|oset
decl_stmt|;
name|int
name|fd
decl_stmt|;
name|char
modifier|*
name|envtmp
decl_stmt|;
name|char
name|path
index|[
name|MAXPATHLEN
index|]
decl_stmt|;
name|envtmp
operator|=
name|getenv
argument_list|(
literal|"TMPDIR"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|snprintf
argument_list|(
name|path
argument_list|,
sizeof|sizeof
argument_list|(
name|path
argument_list|)
argument_list|,
literal|"%s/bt.XXXXXX"
argument_list|,
name|envtmp
condition|?
name|envtmp
else|:
literal|"/tmp"
argument_list|)
expr_stmt|;
name|sigfillset
argument_list|(
operator|&
name|set
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|sigprocmask
argument_list|(
name|SIG_BLOCK
argument_list|,
operator|&
name|set
argument_list|,
operator|&
name|oset
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|fd
operator|=
name|mkstemp
argument_list|(
name|path
argument_list|)
operator|)
operator|!=
operator|-
literal|1
condition|)
operator|(
name|void
operator|)
name|unlink
argument_list|(
name|path
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|sigprocmask
argument_list|(
name|SIG_SETMASK
argument_list|,
operator|&
name|oset
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
return|return
operator|(
name|fd
operator|)
return|;
block|}
end_function

end_unit

