begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1990, 1993  *	The Regents of the University of California.  All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Mike Olson.  *  * %sccs.include.redist.c%  */
end_comment

begin_if
if|#
directive|if
name|defined
argument_list|(
name|LIBC_SCCS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
end_if

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)bt_search.c	8.5 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* LIBC_SCCS and not lint */
end_comment

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<db.h>
end_include

begin_include
include|#
directive|include
file|"btree.h"
end_include

begin_decl_stmt
specifier|static
name|int
name|bt_snext
name|__P
argument_list|(
operator|(
name|BTREE
operator|*
operator|,
name|PAGE
operator|*
operator|,
specifier|const
name|DBT
operator|*
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|bt_sprev
name|__P
argument_list|(
operator|(
name|BTREE
operator|*
operator|,
name|PAGE
operator|*
operator|,
specifier|const
name|DBT
operator|*
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * __BT_SEARCH -- Search a btree for a key.  *  * Parameters:  *	t:	tree to search  *	key:	key to find  *	exactp:	pointer to exact match flag  *  * Returns:  *	The EPG for matching record, if any, or the EPG for the location  *	of the key, if it were inserted into the tree, is entered into  *	the bt_cur field of the tree.  A pointer to the field is returned.  */
end_comment

begin_function
name|EPG
modifier|*
name|__bt_search
parameter_list|(
name|t
parameter_list|,
name|key
parameter_list|,
name|exactp
parameter_list|)
name|BTREE
modifier|*
name|t
decl_stmt|;
specifier|const
name|DBT
modifier|*
name|key
decl_stmt|;
name|int
modifier|*
name|exactp
decl_stmt|;
block|{
name|PAGE
modifier|*
name|h
decl_stmt|,
modifier|*
name|n
decl_stmt|;
name|indx_t
name|base
decl_stmt|,
name|index
decl_stmt|,
name|lim
decl_stmt|;
name|pgno_t
name|pg
decl_stmt|;
name|int
name|cmp
decl_stmt|;
name|BT_CLR
argument_list|(
name|t
argument_list|)
expr_stmt|;
for|for
control|(
name|pg
operator|=
name|P_ROOT
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|(
name|h
operator|=
name|mpool_get
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|pg
argument_list|,
literal|0
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
comment|/* Do a binary search on the current page. */
name|t
operator|->
name|bt_cur
operator|.
name|page
operator|=
name|h
expr_stmt|;
for|for
control|(
name|base
operator|=
literal|0
operator|,
name|lim
operator|=
name|NEXTINDEX
argument_list|(
name|h
argument_list|)
init|;
name|lim
condition|;
name|lim
operator|>>=
literal|1
control|)
block|{
name|t
operator|->
name|bt_cur
operator|.
name|index
operator|=
name|index
operator|=
name|base
operator|+
operator|(
name|lim
operator|>>
literal|1
operator|)
expr_stmt|;
if|if
condition|(
operator|(
name|cmp
operator|=
name|__bt_cmp
argument_list|(
name|t
argument_list|,
name|key
argument_list|,
operator|&
name|t
operator|->
name|bt_cur
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|h
operator|->
name|flags
operator|&
name|P_BLEAF
condition|)
block|{
operator|*
name|exactp
operator|=
literal|1
expr_stmt|;
return|return
operator|(
operator|&
name|t
operator|->
name|bt_cur
operator|)
return|;
block|}
goto|goto
name|next
goto|;
block|}
if|if
condition|(
name|cmp
operator|>
literal|0
condition|)
block|{
name|base
operator|=
name|index
operator|+
literal|1
expr_stmt|;
operator|--
name|lim
expr_stmt|;
block|}
block|}
comment|/* 		 * If it's a leaf page, and duplicates aren't allowed, we're 		 * done.  If duplicates are allowed, it's possible that there 		 * were duplicate keys on duplicate pages, and they were later 		 * deleted, so we could be on a page with no matches while 		 * there are matches on other pages.  If we're at the start or 		 * end of a page, check on both sides. 		 */
if|if
condition|(
name|h
operator|->
name|flags
operator|&
name|P_BLEAF
condition|)
block|{
name|t
operator|->
name|bt_cur
operator|.
name|index
operator|=
name|base
expr_stmt|;
operator|*
name|exactp
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
name|ISSET
argument_list|(
name|t
argument_list|,
name|B_NODUPS
argument_list|)
condition|)
block|{
if|if
condition|(
name|base
operator|==
literal|0
operator|&&
name|bt_sprev
argument_list|(
name|t
argument_list|,
name|h
argument_list|,
name|key
argument_list|,
name|exactp
argument_list|)
condition|)
return|return
operator|(
operator|&
name|t
operator|->
name|bt_cur
operator|)
return|;
if|if
condition|(
name|base
operator|==
name|NEXTINDEX
argument_list|(
name|h
argument_list|)
operator|&&
name|bt_snext
argument_list|(
name|t
argument_list|,
name|h
argument_list|,
name|key
argument_list|,
name|exactp
argument_list|)
condition|)
return|return
operator|(
operator|&
name|t
operator|->
name|bt_cur
operator|)
return|;
block|}
return|return
operator|(
operator|&
name|t
operator|->
name|bt_cur
operator|)
return|;
block|}
comment|/* 		 * No match found.  Base is the smallest index greater than 		 * key and may be zero or a last + 1 index.  If it's non-zero, 		 * decrement by one, and record the internal page which should 		 * be a parent page for the key.  If a split later occurs, the 		 * inserted page will be to the right of the saved page. 		 */
name|index
operator|=
name|base
condition|?
name|base
operator|-
literal|1
else|:
name|base
expr_stmt|;
name|next
label|:
if|if
condition|(
name|__bt_push
argument_list|(
name|t
argument_list|,
name|h
operator|->
name|pgno
argument_list|,
name|index
argument_list|)
operator|==
name|RET_ERROR
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
name|pg
operator|=
name|GETBINTERNAL
argument_list|(
name|h
argument_list|,
name|index
argument_list|)
operator|->
name|pgno
expr_stmt|;
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|h
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * BT_SNEXT -- Check for an exact match after the key.  *  * Parameters:  *	t:	tree to search  *	h:	current page.  *	key:	key to find  *	exactp:	pointer to exact match flag  *  * Returns:  *	If an exact match found.  */
end_comment

begin_function
specifier|static
name|int
name|bt_snext
parameter_list|(
name|t
parameter_list|,
name|h
parameter_list|,
name|key
parameter_list|,
name|exactp
parameter_list|)
name|BTREE
modifier|*
name|t
decl_stmt|;
name|PAGE
modifier|*
name|h
decl_stmt|;
specifier|const
name|DBT
modifier|*
name|key
decl_stmt|;
name|int
modifier|*
name|exactp
decl_stmt|;
block|{
name|EPG
name|e
decl_stmt|;
name|PAGE
modifier|*
name|tp
decl_stmt|;
name|pgno_t
name|pg
decl_stmt|;
comment|/* Skip until reach the end of the tree or a key. */
for|for
control|(
name|pg
operator|=
name|h
operator|->
name|nextpg
init|;
name|pg
operator|!=
name|P_INVALID
condition|;
control|)
block|{
if|if
condition|(
operator|(
name|tp
operator|=
name|mpool_get
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|pg
argument_list|,
literal|0
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|h
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
if|if
condition|(
name|NEXTINDEX
argument_list|(
name|tp
argument_list|)
operator|!=
literal|0
condition|)
break|break;
name|pg
operator|=
name|tp
operator|->
name|prevpg
expr_stmt|;
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|tp
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * The key is either an exact match, or not as good as 	 * the one we already have. 	 */
if|if
condition|(
name|pg
operator|!=
name|P_INVALID
condition|)
block|{
name|e
operator|.
name|page
operator|=
name|tp
expr_stmt|;
name|e
operator|.
name|index
operator|=
name|NEXTINDEX
argument_list|(
name|tp
argument_list|)
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|__bt_cmp
argument_list|(
name|t
argument_list|,
name|key
argument_list|,
operator|&
name|e
argument_list|)
operator|==
literal|0
condition|)
block|{
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|h
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|t
operator|->
name|bt_cur
operator|=
name|e
expr_stmt|;
operator|*
name|exactp
operator|=
literal|1
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * BT_SPREV -- Check for an exact match before the key.  *  * Parameters:  *	t:	tree to search  *	h:	current page.  *	key:	key to find  *	exactp:	pointer to exact match flag  *  * Returns:  *	If an exact match found.  */
end_comment

begin_function
specifier|static
name|int
name|bt_sprev
parameter_list|(
name|t
parameter_list|,
name|h
parameter_list|,
name|key
parameter_list|,
name|exactp
parameter_list|)
name|BTREE
modifier|*
name|t
decl_stmt|;
name|PAGE
modifier|*
name|h
decl_stmt|;
specifier|const
name|DBT
modifier|*
name|key
decl_stmt|;
name|int
modifier|*
name|exactp
decl_stmt|;
block|{
name|EPG
name|e
decl_stmt|;
name|PAGE
modifier|*
name|tp
decl_stmt|;
name|pgno_t
name|pg
decl_stmt|;
comment|/* Skip until reach the beginning of the tree or a key. */
for|for
control|(
name|pg
operator|=
name|h
operator|->
name|prevpg
init|;
name|pg
operator|!=
name|P_INVALID
condition|;
control|)
block|{
if|if
condition|(
operator|(
name|tp
operator|=
name|mpool_get
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|pg
argument_list|,
literal|0
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|h
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
operator|(
name|NULL
operator|)
return|;
block|}
if|if
condition|(
name|NEXTINDEX
argument_list|(
name|tp
argument_list|)
operator|!=
literal|0
condition|)
break|break;
name|pg
operator|=
name|tp
operator|->
name|prevpg
expr_stmt|;
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|tp
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * The key is either an exact match, or not as good as 	 * the one we already have. 	 */
if|if
condition|(
name|pg
operator|!=
name|P_INVALID
condition|)
block|{
name|e
operator|.
name|page
operator|=
name|tp
expr_stmt|;
name|e
operator|.
name|index
operator|=
name|NEXTINDEX
argument_list|(
name|tp
argument_list|)
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|__bt_cmp
argument_list|(
name|t
argument_list|,
name|key
argument_list|,
operator|&
name|e
argument_list|)
operator|==
literal|0
condition|)
block|{
name|mpool_put
argument_list|(
name|t
operator|->
name|bt_mp
argument_list|,
name|h
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|t
operator|->
name|bt_cur
operator|=
name|e
expr_stmt|;
operator|*
name|exactp
operator|=
literal|1
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

end_unit

