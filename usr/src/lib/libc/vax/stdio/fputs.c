...
operator|.
name|asciz
literal|"@(#)fputs.c	5.4 (Berkeley) 4/1/86"
endif|#
directive|endif
endif|LIBC_SCCS
comment|/*  * fputs(s, iop);  * char *s;  * FILE *iop;  *  * arguments: a source string and a file pointer.  * side effects: writes to the file indicated by iop using the data in  *	the null-terminated source string.  * result: technically void; for compatibility we return 0 for the null  *	string, non-zero otherwise.  We return zero for errors too.  */
include|#
directive|include
file|"DEFS.h"
define|#
...
name|COUNT
value|r9
comment|/* 	 * For compatibility (sigh). 	 */
name|tstb
argument_list|(
argument|S
argument_list|)
...
name|pushl
name|IOP
comment|/* Get _flsbuf() to do the work */
name|pushl
name|$0
name|calls
name|$2
...
literal|2
operator|:
comment|/* 	 * Search for the terminating null. 	 * We only need to look at _BUFSIZ bytes or less on each pass. 	 */
name|Lloop
operator|:
name|addl3
name|_BASE
