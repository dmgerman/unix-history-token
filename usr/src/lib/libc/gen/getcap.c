begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1992 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Casey Leedom of Lawrence Livermore National Laboratory.  *  * %sccs.include.redist.c%  */
end_comment

begin_if
if|#
directive|if
name|defined
argument_list|(
name|LIBC_SCCS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
end_if

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)getcap.c	5.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* LIBC_SCCS and not lint */
end_comment

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_define
define|#
directive|define
name|BFRAG
value|1024
end_define

begin_define
define|#
directive|define
name|BSIZE
value|80
end_define

begin_define
define|#
directive|define
name|ESC
value|('['& 037)
end_define

begin_comment
comment|/* ASCII ESC */
end_comment

begin_define
define|#
directive|define
name|MAX_RECURSION
value|32
end_define

begin_comment
comment|/* maximum getent recursion */
end_comment

begin_define
define|#
directive|define
name|SFRAG
value|100
end_define

begin_comment
comment|/* cgetstr mallocs in SFRAG chunks */
end_comment

begin_decl_stmt
specifier|static
name|size_t
name|topreclen
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* toprec length */
end_comment

begin_decl_stmt
specifier|static
name|char
modifier|*
name|toprec
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Additional record specified by cgetset() */
end_comment

begin_decl_stmt
specifier|static
name|int
name|getent
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|*
operator|,
name|u_int
operator|*
operator|,
name|char
operator|*
operator|*
operator|,
name|int
operator|,
name|char
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Cgetset() allows the addition of a user specified buffer to be added  * to the database array, in effect "pushing" the buffer on top of the  * virtual database. 0 is returned on success, -1 on failure.  */
end_comment

begin_function
name|int
name|cgetset
parameter_list|(
name|ent
parameter_list|)
name|char
modifier|*
name|ent
decl_stmt|;
block|{
if|if
condition|(
name|ent
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
name|toprec
condition|)
name|free
argument_list|(
name|toprec
argument_list|)
expr_stmt|;
name|toprec
operator|=
name|NULL
expr_stmt|;
name|topreclen
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|topreclen
operator|=
name|strlen
argument_list|(
name|ent
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|toprec
operator|=
name|malloc
argument_list|(
name|topreclen
operator|+
literal|1
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|errno
operator|=
name|ENOMEM
expr_stmt|;
return|return
operator|(
operator|-
literal|1
operator|)
return|;
block|}
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|toprec
argument_list|,
name|ent
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Cgetcap searches the capability record buf for the capability cap with  * type `type'.  A pointer to the value of cap is returned on success, NULL  * if the requested capability couldn't be found.  *  * Specifying a type of ':' means that nothing should follow cap (:cap:).  * In this case a pointer to the terminating ':' or NUL will be returned if  * cap is found.  *  * If (cap, '@') or (cap, terminator, '@') is found before (cap, terminator)  * return NULL.  */
end_comment

begin_function
name|char
modifier|*
name|cgetcap
parameter_list|(
name|buf
parameter_list|,
name|cap
parameter_list|,
name|type
parameter_list|)
name|char
modifier|*
name|buf
decl_stmt|,
decl|*
name|cap
decl_stmt|;
end_function

begin_decl_stmt
name|int
name|type
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
modifier|*
name|bp
decl_stmt|,
modifier|*
name|cp
decl_stmt|;
name|bp
operator|=
name|buf
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
comment|/* 		 * Skip past the current capability field - it's either the 		 * name field if this is the first time through the loop, or 		 * the remainder of a field whose name failed to match cap. 		 */
for|for
control|(
init|;
condition|;
control|)
if|if
condition|(
operator|*
name|bp
operator|==
literal|'\0'
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
elseif|else
if|if
condition|(
operator|*
name|bp
operator|++
operator|==
literal|':'
condition|)
break|break;
comment|/* 		 * Try to match (cap, type) in buf. 		 */
for|for
control|(
name|cp
operator|=
name|cap
init|;
operator|*
name|cp
operator|==
operator|*
name|bp
operator|&&
operator|*
name|bp
operator|!=
literal|'\0'
condition|;
name|cp
operator|++
operator|,
name|bp
operator|++
control|)
continue|continue;
if|if
condition|(
operator|*
name|cp
operator|!=
literal|'\0'
condition|)
continue|continue;
if|if
condition|(
operator|*
name|bp
operator|==
literal|'@'
condition|)
return|return
operator|(
name|NULL
operator|)
return|;
if|if
condition|(
name|type
operator|==
literal|':'
condition|)
block|{
if|if
condition|(
operator|*
name|bp
operator|!=
literal|'\0'
operator|&&
operator|*
name|bp
operator|!=
literal|':'
condition|)
continue|continue;
return|return
operator|(
name|bp
operator|)
return|;
block|}
if|if
condition|(
operator|*
name|bp
operator|!=
name|type
condition|)
continue|continue;
name|bp
operator|++
expr_stmt|;
return|return
operator|(
operator|*
name|bp
operator|==
literal|'@'
condition|?
name|NULL
else|:
name|bp
operator|)
return|;
block|}
comment|/* NOTREACHED */
block|}
end_block

begin_comment
comment|/*  * Cgetent extracts the capability record name from the NULL terminated file  * array db_array and returns a pointer to a malloc'd copy of it in buf.  * Buf must be retained through all subsequent calls to cgetcap, cgetnum,  * cgetflag, and cgetstr, but may then be free'd.  0 is returned on success,  * -1 if the requested record couldn't be found, -2 if a system error was  * encountered (couldn't open/read a file, etc.), and -3 if a potential  * reference loop is detected.  */
end_comment

begin_function
name|int
name|cgetent
parameter_list|(
name|buf
parameter_list|,
name|db_array
parameter_list|,
name|name
parameter_list|)
name|char
modifier|*
modifier|*
name|buf
decl_stmt|,
decl|*
modifier|*
name|db_array
decl_stmt|,
modifier|*
name|name
decl_stmt|;
end_function

begin_block
block|{
name|u_int
name|dummy
decl_stmt|;
return|return
operator|(
name|getent
argument_list|(
name|buf
argument_list|,
operator|&
name|dummy
argument_list|,
name|db_array
argument_list|,
operator|-
literal|1
argument_list|,
name|name
argument_list|,
literal|0
argument_list|)
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Getent implements the functions of cgetent.  If fd is non-negative,  * *db_array has already been opened and fd is the open file descriptor.  We  * do this to save time and avoid using up file descriptors for tc=  * recursions.  *  * Getent returns the same success/failure codes as cgetent.  On success, a  * pointer to a malloc'ed capability record with all tc= capabilities fully  * expanded and its length (not including trailing ASCII NUL) are left in  * *cap and *len.  *  * Basic algorithm:  *	+ Allocate memory incrementally as needed in chunks of size BFRAG  *	  for capability buffer.  *	+ Recurse for each tc=name and interpolate result.  Stop when all  *	  names interpolated, a name can't be found, or depth exceeds  *	  MAX_RECURSION.  */
end_comment

begin_function
specifier|static
name|int
name|getent
parameter_list|(
name|cap
parameter_list|,
name|len
parameter_list|,
name|db_array
parameter_list|,
name|fd
parameter_list|,
name|name
parameter_list|,
name|depth
parameter_list|)
name|char
modifier|*
modifier|*
name|cap
decl_stmt|,
decl|*
modifier|*
name|db_array
decl_stmt|,
modifier|*
name|name
decl_stmt|;
end_function

begin_decl_stmt
name|u_int
modifier|*
name|len
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|fd
decl_stmt|,
name|depth
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
modifier|*
name|r_end
decl_stmt|,
modifier|*
name|rp
decl_stmt|,
modifier|*
modifier|*
name|db_p
decl_stmt|;
name|int
name|myfd
decl_stmt|,
name|eof
decl_stmt|,
name|foundit
decl_stmt|;
name|char
modifier|*
name|record
decl_stmt|;
comment|/* 	 * Return with ``loop detected'' error if we've recursed more than 	 * MAX_RECURSION times. 	 */
if|if
condition|(
name|depth
operator|>
name|MAX_RECURSION
condition|)
return|return
operator|(
operator|-
literal|3
operator|)
return|;
comment|/* 	 * Check if we have a top record from cgetset().          */
if|if
condition|(
name|depth
operator|==
literal|0
operator|&&
name|toprec
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
operator|(
name|record
operator|=
name|malloc
argument_list|(
name|topreclen
operator|+
name|BFRAG
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|errno
operator|=
name|ENOMEM
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
block|}
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|record
argument_list|,
name|toprec
argument_list|)
expr_stmt|;
name|myfd
operator|=
literal|0
expr_stmt|;
name|db_p
operator|=
name|db_array
expr_stmt|;
name|rp
operator|=
name|record
operator|+
name|topreclen
operator|+
literal|1
expr_stmt|;
name|r_end
operator|=
name|rp
operator|+
name|BFRAG
expr_stmt|;
goto|goto
name|tc_exp
goto|;
block|}
comment|/* 	 * Allocate first chunk of memory. 	 */
if|if
condition|(
operator|(
name|record
operator|=
name|malloc
argument_list|(
name|BFRAG
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|errno
operator|=
name|ENOMEM
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
block|}
name|r_end
operator|=
name|record
operator|+
name|BFRAG
expr_stmt|;
name|foundit
operator|=
literal|0
expr_stmt|;
comment|/* 	 * Loop through database array until finding the record. 	 */
for|for
control|(
name|db_p
operator|=
name|db_array
init|;
operator|*
name|db_p
operator|!=
name|NULL
condition|;
name|db_p
operator|++
control|)
block|{
name|eof
operator|=
literal|0
expr_stmt|;
comment|/* 		 * Open database if not already open. 		 */
if|if
condition|(
name|fd
operator|>=
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|lseek
argument_list|(
name|fd
argument_list|,
literal|0L
argument_list|,
name|L_SET
argument_list|)
expr_stmt|;
name|myfd
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|fd
operator|=
name|open
argument_list|(
operator|*
name|db_p
argument_list|,
name|O_RDONLY
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|<
literal|0
condition|)
block|{
name|free
argument_list|(
name|record
argument_list|)
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
block|}
name|myfd
operator|=
literal|1
expr_stmt|;
block|}
comment|/* 		 * Find the requested capability record ... 		 */
block|{
name|char
name|buf
index|[
name|BUFSIZ
index|]
decl_stmt|;
specifier|register
name|char
modifier|*
name|b_end
decl_stmt|,
modifier|*
name|bp
decl_stmt|;
specifier|register
name|int
name|c
decl_stmt|;
comment|/* 		 * Loop invariants: 		 *	There is always room for one more character in record. 		 *	R_end always points just past end of record. 		 *	Rp always points just past last character in record. 		 *	B_end always points just past last character in buf. 		 *	Bp always points at next character in buf. 		 */
name|b_end
operator|=
name|buf
expr_stmt|;
name|bp
operator|=
name|buf
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
comment|/* 			 * Read in a line implementing (\, newline) 			 * line continuation. 			 */
name|rp
operator|=
name|record
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
name|bp
operator|>=
name|b_end
condition|)
block|{
name|int
name|n
decl_stmt|;
name|n
operator|=
name|read
argument_list|(
name|fd
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|<=
literal|0
condition|)
block|{
if|if
condition|(
name|myfd
condition|)
operator|(
name|void
operator|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|<
literal|0
condition|)
block|{
name|free
argument_list|(
name|record
argument_list|)
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
block|}
else|else
block|{
name|fd
operator|=
operator|-
literal|1
expr_stmt|;
name|eof
operator|=
literal|1
expr_stmt|;
break|break;
block|}
block|}
name|b_end
operator|=
name|buf
operator|+
name|n
expr_stmt|;
name|bp
operator|=
name|buf
expr_stmt|;
block|}
name|c
operator|=
operator|*
name|bp
operator|++
expr_stmt|;
if|if
condition|(
name|c
operator|==
literal|'\n'
condition|)
block|{
if|if
condition|(
name|rp
operator|>
name|record
operator|&&
operator|*
operator|(
name|rp
operator|-
literal|1
operator|)
operator|==
literal|'\\'
condition|)
block|{
name|rp
operator|--
expr_stmt|;
continue|continue;
block|}
else|else
break|break;
block|}
operator|*
name|rp
operator|++
operator|=
name|c
expr_stmt|;
comment|/* 				 * Enforce loop invariant: if no room  				 * left in record buffer, try to get 				 * some more. 				 */
if|if
condition|(
name|rp
operator|>=
name|r_end
condition|)
block|{
name|u_int
name|pos
decl_stmt|,
name|newsize
decl_stmt|;
name|pos
operator|=
name|rp
operator|-
name|record
expr_stmt|;
name|newsize
operator|=
name|r_end
operator|-
name|record
operator|+
name|BFRAG
expr_stmt|;
name|record
operator|=
name|realloc
argument_list|(
name|record
argument_list|,
name|newsize
argument_list|)
expr_stmt|;
if|if
condition|(
name|record
operator|==
name|NULL
condition|)
block|{
name|errno
operator|=
name|ENOMEM
expr_stmt|;
if|if
condition|(
name|myfd
condition|)
operator|(
name|void
operator|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
block|}
name|r_end
operator|=
name|record
operator|+
name|newsize
expr_stmt|;
name|rp
operator|=
name|record
operator|+
name|pos
expr_stmt|;
block|}
block|}
comment|/* loop invariant let's us do this */
operator|*
name|rp
operator|++
operator|=
literal|'\0'
expr_stmt|;
comment|/* 			 * If encountered eof check next file. 			 */
if|if
condition|(
name|eof
condition|)
break|break;
comment|/* 			 * Toss blank lines and comments. 			 */
if|if
condition|(
operator|*
name|record
operator|==
literal|'\0'
operator|||
operator|*
name|record
operator|==
literal|'#'
condition|)
continue|continue;
comment|/* 			 * See if this is the record we want ... 			 */
if|if
condition|(
name|cgetmatch
argument_list|(
name|record
argument_list|,
name|name
argument_list|)
operator|==
literal|0
condition|)
block|{
name|foundit
operator|=
literal|1
expr_stmt|;
break|break;
comment|/* found it! */
block|}
block|}
block|}
if|if
condition|(
name|foundit
condition|)
break|break;
block|}
if|if
condition|(
operator|!
name|foundit
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
comment|/* 	 * Got the capability record, but now we have to expand all tc=name 	 * references in it ... 	 */
name|tc_exp
label|:
block|{
specifier|register
name|char
modifier|*
name|newicap
decl_stmt|,
modifier|*
name|s
decl_stmt|;
specifier|register
name|int
name|newilen
decl_stmt|;
name|u_int
name|ilen
decl_stmt|;
name|int
name|diff
decl_stmt|,
name|iret
decl_stmt|,
name|tclen
decl_stmt|;
name|char
modifier|*
name|icap
decl_stmt|,
modifier|*
name|scan
decl_stmt|,
modifier|*
name|tc
decl_stmt|,
modifier|*
name|tcstart
decl_stmt|,
modifier|*
name|tcend
decl_stmt|;
comment|/* 		 * Loop invariants: 		 *	There is room for one more character in record. 		 *	R_end points just past end of record. 		 *	Rp points just past last character in record. 		 *	Scan points at remainder of record that needs to be 		 *	scanned for tc=name constructs. 		 */
name|scan
operator|=
name|record
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|(
name|tc
operator|=
name|cgetcap
argument_list|(
name|scan
argument_list|,
literal|"tc"
argument_list|,
literal|'='
argument_list|)
operator|)
operator|==
name|NULL
condition|)
break|break;
comment|/* 			 * Find end of tc=name and stomp on the trailing `:' 			 * (if present) so we can use it to call ourselves. 			 */
name|s
operator|=
name|tc
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
if|if
condition|(
operator|*
name|s
operator|==
literal|'\0'
condition|)
break|break;
elseif|else
if|if
condition|(
operator|*
name|s
operator|++
operator|==
literal|':'
condition|)
block|{
operator|*
operator|(
name|s
operator|-
literal|1
operator|)
operator|=
literal|'\0'
expr_stmt|;
break|break;
block|}
name|tcstart
operator|=
name|tc
operator|-
literal|3
expr_stmt|;
name|tclen
operator|=
name|s
operator|-
name|tcstart
expr_stmt|;
name|tcend
operator|=
name|s
expr_stmt|;
name|iret
operator|=
name|getent
argument_list|(
operator|&
name|icap
argument_list|,
operator|&
name|ilen
argument_list|,
name|db_p
argument_list|,
name|fd
argument_list|,
name|tc
argument_list|,
name|depth
operator|+
literal|1
argument_list|)
expr_stmt|;
name|newicap
operator|=
name|icap
expr_stmt|;
comment|/* Put into a register. */
name|newilen
operator|=
name|ilen
expr_stmt|;
if|if
condition|(
name|iret
operator|!=
literal|0
condition|)
block|{
comment|/* an error or couldn't resolve tc= */
if|if
condition|(
name|myfd
condition|)
operator|(
name|void
operator|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|record
argument_list|)
expr_stmt|;
return|return
operator|(
name|iret
operator|)
return|;
block|}
comment|/* not interested in name field of tc'ed record */
name|s
operator|=
name|newicap
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
if|if
condition|(
operator|*
name|s
operator|==
literal|'\0'
condition|)
break|break;
elseif|else
if|if
condition|(
operator|*
name|s
operator|++
operator|==
literal|':'
condition|)
break|break;
name|newilen
operator|-=
name|s
operator|-
name|newicap
expr_stmt|;
name|newicap
operator|=
name|s
expr_stmt|;
comment|/* make sure interpolated record is `:'-terminated */
name|s
operator|+=
name|newilen
expr_stmt|;
if|if
condition|(
operator|*
operator|(
name|s
operator|-
literal|1
operator|)
operator|!=
literal|':'
condition|)
block|{
operator|*
name|s
operator|=
literal|':'
expr_stmt|;
comment|/* overwrite NUL with : */
name|newilen
operator|++
expr_stmt|;
block|}
comment|/* 			 * Make sure there's enough room to insert the 			 * new record. 			 */
name|diff
operator|=
name|newilen
operator|-
name|tclen
expr_stmt|;
if|if
condition|(
name|diff
operator|>=
name|r_end
operator|-
name|rp
condition|)
block|{
name|u_int
name|pos
decl_stmt|,
name|newsize
decl_stmt|,
name|tcpos
decl_stmt|,
name|tcposend
decl_stmt|;
name|pos
operator|=
name|rp
operator|-
name|record
expr_stmt|;
name|newsize
operator|=
name|r_end
operator|-
name|record
operator|+
name|diff
operator|+
name|BFRAG
expr_stmt|;
name|tcpos
operator|=
name|tcstart
operator|-
name|record
expr_stmt|;
name|tcposend
operator|=
name|tcend
operator|-
name|record
expr_stmt|;
name|record
operator|=
name|realloc
argument_list|(
name|record
argument_list|,
name|newsize
argument_list|)
expr_stmt|;
if|if
condition|(
name|record
operator|==
name|NULL
condition|)
block|{
name|errno
operator|=
name|ENOMEM
expr_stmt|;
if|if
condition|(
name|myfd
condition|)
operator|(
name|void
operator|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|icap
argument_list|)
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
block|}
name|r_end
operator|=
name|record
operator|+
name|newsize
expr_stmt|;
name|rp
operator|=
name|record
operator|+
name|pos
expr_stmt|;
name|tcstart
operator|=
name|record
operator|+
name|tcpos
expr_stmt|;
name|tcend
operator|=
name|record
operator|+
name|tcposend
expr_stmt|;
block|}
comment|/* 			 * Insert tc'ed record into our record. 			 */
name|s
operator|=
name|tcstart
operator|+
name|newilen
expr_stmt|;
name|bcopy
argument_list|(
name|tcend
argument_list|,
name|s
argument_list|,
name|rp
operator|-
name|tcend
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
name|newicap
argument_list|,
name|tcstart
argument_list|,
name|newilen
argument_list|)
expr_stmt|;
name|rp
operator|+=
name|diff
expr_stmt|;
name|free
argument_list|(
name|icap
argument_list|)
expr_stmt|;
comment|/* 			 * Start scan on `:' so next cgetcap works properly 			 * (cgetcap always skips first field). 			 */
name|scan
operator|=
name|s
operator|-
literal|1
expr_stmt|;
block|}
block|}
comment|/* 	 * Close file (if we opened it), give back any extra memory, and 	 * return capability, length and success. 	 */
if|if
condition|(
name|myfd
condition|)
operator|(
name|void
operator|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
operator|*
name|len
operator|=
name|rp
operator|-
name|record
operator|-
literal|1
expr_stmt|;
comment|/* don't count NUL */
if|if
condition|(
name|r_end
operator|>
name|rp
condition|)
name|record
operator|=
name|realloc
argument_list|(
name|record
argument_list|,
call|(
name|u_int
call|)
argument_list|(
name|rp
operator|-
name|record
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|cap
operator|=
name|record
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Cgetmatch will return 0 if name is one of the names of the capability  * record buf, -1 if not.  */
end_comment

begin_function
name|int
name|cgetmatch
parameter_list|(
name|buf
parameter_list|,
name|name
parameter_list|)
name|char
modifier|*
name|buf
decl_stmt|,
decl|*
name|name
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|char
modifier|*
name|np
decl_stmt|,
modifier|*
name|bp
decl_stmt|;
comment|/* 	 * Start search at beginning of record. 	 */
name|bp
operator|=
name|buf
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
comment|/* 		 * Try to match a record name. 		 */
name|np
operator|=
name|name
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
if|if
condition|(
operator|*
name|np
operator|==
literal|'\0'
condition|)
if|if
condition|(
operator|*
name|bp
operator|==
literal|'|'
operator|||
operator|*
name|bp
operator|==
literal|':'
operator|||
operator|*
name|bp
operator|==
literal|'\0'
condition|)
return|return
operator|(
literal|0
operator|)
return|;
else|else
break|break;
elseif|else
if|if
condition|(
operator|*
name|bp
operator|++
operator|!=
operator|*
name|np
operator|++
condition|)
break|break;
comment|/* 		 * Match failed, skip to next name in record. 		 */
name|bp
operator|--
expr_stmt|;
comment|/* a '|' or ':' may have stopped the match */
for|for
control|(
init|;
condition|;
control|)
if|if
condition|(
operator|*
name|bp
operator|==
literal|'\0'
operator|||
operator|*
name|bp
operator|==
literal|':'
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
comment|/* match failed totally */
elseif|else
if|if
condition|(
operator|*
name|bp
operator|++
operator|==
literal|'|'
condition|)
break|break;
comment|/* found next name */
block|}
block|}
end_block

begin_function
name|int
name|cgetfirst
parameter_list|(
name|buf
parameter_list|,
name|db_array
parameter_list|)
name|char
modifier|*
modifier|*
name|buf
decl_stmt|,
decl|*
modifier|*
name|db_array
decl_stmt|;
end_function

begin_block
block|{
operator|(
name|void
operator|)
name|cgetclose
argument_list|()
expr_stmt|;
return|return
operator|(
name|cgetnext
argument_list|(
name|buf
argument_list|,
name|db_array
argument_list|)
operator|)
return|;
block|}
end_block

begin_decl_stmt
specifier|static
name|FILE
modifier|*
name|pfp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|slash
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|char
modifier|*
modifier|*
name|dbp
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|cgetclose
parameter_list|()
block|{
if|if
condition|(
name|pfp
operator|!=
name|NULL
condition|)
block|{
operator|(
name|void
operator|)
name|fclose
argument_list|(
name|pfp
argument_list|)
expr_stmt|;
name|pfp
operator|=
name|NULL
expr_stmt|;
block|}
name|dbp
operator|=
name|NULL
expr_stmt|;
name|slash
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Cgetnext() gets either the first or next entry in the logical database   * specified by db_array.  It returns 0 upon completion of the database, 1  * upon returning an entry with more remaining, and -1 if an error occurs.  */
end_comment

begin_function
name|int
name|cgetnext
parameter_list|(
name|bp
parameter_list|,
name|db_array
parameter_list|)
specifier|register
name|char
modifier|*
modifier|*
name|bp
decl_stmt|;
name|char
modifier|*
modifier|*
name|db_array
decl_stmt|;
block|{
name|size_t
name|len
decl_stmt|;
name|int
name|status
decl_stmt|;
name|char
modifier|*
name|cp
decl_stmt|,
modifier|*
name|line
decl_stmt|,
modifier|*
name|rp
decl_stmt|,
name|buf
index|[
name|BSIZE
index|]
decl_stmt|;
if|if
condition|(
name|dbp
operator|==
name|NULL
condition|)
name|dbp
operator|=
name|db_array
expr_stmt|;
if|if
condition|(
name|pfp
operator|==
name|NULL
operator|&&
operator|(
name|pfp
operator|=
name|fopen
argument_list|(
operator|*
name|dbp
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|line
operator|=
name|fgetline
argument_list|(
name|pfp
argument_list|,
operator|&
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
name|line
operator|==
name|NULL
condition|)
block|{
operator|(
name|void
operator|)
name|fclose
argument_list|(
name|pfp
argument_list|)
expr_stmt|;
if|if
condition|(
name|ferror
argument_list|(
name|pfp
argument_list|)
condition|)
block|{
name|pfp
operator|=
name|NULL
expr_stmt|;
name|dbp
operator|=
name|NULL
expr_stmt|;
name|slash
operator|=
literal|0
expr_stmt|;
return|return
operator|(
operator|-
literal|1
operator|)
return|;
block|}
else|else
block|{
name|dbp
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|dbp
operator|==
name|NULL
condition|)
block|{
name|pfp
operator|=
name|NULL
expr_stmt|;
name|dbp
operator|=
name|NULL
expr_stmt|;
name|slash
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
elseif|else
if|if
condition|(
operator|(
name|pfp
operator|=
name|fopen
argument_list|(
operator|*
name|dbp
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|pfp
operator|=
name|NULL
expr_stmt|;
name|dbp
operator|=
name|NULL
expr_stmt|;
name|slash
operator|=
literal|0
expr_stmt|;
return|return
operator|(
operator|-
literal|1
operator|)
return|;
block|}
else|else
continue|continue;
block|}
block|}
if|if
condition|(
name|isspace
argument_list|(
operator|*
name|line
argument_list|)
operator|||
operator|*
name|line
operator|==
literal|':'
operator|||
operator|*
name|line
operator|==
literal|'#'
operator|||
name|len
operator|==
literal|0
operator|||
name|slash
condition|)
block|{
if|if
condition|(
name|len
operator|>
literal|0
operator|&&
name|line
index|[
name|len
operator|-
literal|1
index|]
operator|==
literal|'\\'
condition|)
name|slash
operator|=
literal|1
expr_stmt|;
else|else
name|slash
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|len
operator|>
literal|0
operator|&&
name|line
index|[
name|len
operator|-
literal|1
index|]
operator|==
literal|'\\'
condition|)
name|slash
operator|=
literal|1
expr_stmt|;
else|else
name|slash
operator|=
literal|0
expr_stmt|;
comment|/* line points to a name line */
name|rp
operator|=
name|buf
expr_stmt|;
for|for
control|(
name|cp
operator|=
name|line
init|;
operator|*
name|cp
operator|!=
name|NULL
condition|;
name|cp
operator|++
control|)
if|if
condition|(
operator|*
name|cp
operator|==
literal|'|'
operator|||
operator|*
name|cp
operator|==
literal|':'
condition|)
break|break;
else|else
operator|*
name|rp
operator|++
operator|=
operator|*
name|cp
expr_stmt|;
operator|*
name|rp
operator|=
literal|'\0'
expr_stmt|;
name|status
operator|=
name|cgetent
argument_list|(
name|bp
argument_list|,
name|db_array
argument_list|,
operator|&
name|buf
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|status
operator|==
literal|0
condition|)
return|return
operator|(
literal|1
operator|)
return|;
if|if
condition|(
name|status
operator|==
operator|-
literal|2
operator|||
name|status
operator|==
operator|-
literal|3
condition|)
block|{
name|pfp
operator|=
name|NULL
expr_stmt|;
name|dbp
operator|=
name|NULL
expr_stmt|;
name|slash
operator|=
literal|0
expr_stmt|;
return|return
operator|(
name|status
operator|+
literal|1
operator|)
return|;
block|}
block|}
comment|/* NOTREACHED */
block|}
end_function

begin_comment
comment|/*  * Cgetstr retrieves the value of the string capability cap from the  * capability record pointed to by buf.  A pointer to a decoded, NUL  * terminated, malloc'd copy of the string is returned in the char *  * pointed to by str.  The length of the string not including the trailing  * NUL is returned on success, -1 if the requested string capability  * couldn't be found, -2 if a system error was encountered (storage  * allocation failure).  */
end_comment

begin_function
name|int
name|cgetstr
parameter_list|(
name|buf
parameter_list|,
name|cap
parameter_list|,
name|str
parameter_list|)
name|char
modifier|*
name|buf
decl_stmt|,
decl|*
name|cap
decl_stmt|;
end_function

begin_decl_stmt
name|char
modifier|*
modifier|*
name|str
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|u_int
name|m_room
decl_stmt|;
specifier|register
name|char
modifier|*
name|bp
decl_stmt|,
modifier|*
name|mp
decl_stmt|;
name|int
name|len
decl_stmt|;
name|char
modifier|*
name|mem
decl_stmt|;
comment|/* 	 * Find string capability cap 	 */
name|bp
operator|=
name|cgetcap
argument_list|(
name|buf
argument_list|,
name|cap
argument_list|,
literal|'='
argument_list|)
expr_stmt|;
if|if
condition|(
name|bp
operator|==
name|NULL
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
comment|/* 	 * Conversion / storage allocation loop ...  Allocate memory in 	 * chunks SFRAG in size. 	 */
if|if
condition|(
operator|(
name|mem
operator|=
name|malloc
argument_list|(
name|SFRAG
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|errno
operator|=
name|ENOMEM
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
comment|/* couldn't even allocate the first fragment */
block|}
name|m_room
operator|=
name|SFRAG
expr_stmt|;
name|mp
operator|=
name|mem
expr_stmt|;
while|while
condition|(
operator|*
name|bp
operator|!=
literal|':'
operator|&&
operator|*
name|bp
operator|!=
literal|'\0'
condition|)
block|{
comment|/* 		 * Loop invariants: 		 *	There is always room for one more character in mem. 		 *	Mp always points just past last character in mem. 		 *	Bp always points at next character in buf. 		 */
if|if
condition|(
operator|*
name|bp
operator|==
literal|'^'
condition|)
block|{
name|bp
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|bp
operator|==
literal|':'
operator|||
operator|*
name|bp
operator|==
literal|'\0'
condition|)
break|break;
comment|/* drop unfinished escape */
operator|*
name|mp
operator|++
operator|=
operator|*
name|bp
operator|++
operator|&
literal|037
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|*
name|bp
operator|==
literal|'\\'
condition|)
block|{
name|bp
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|bp
operator|==
literal|':'
operator|||
operator|*
name|bp
operator|==
literal|'\0'
condition|)
break|break;
comment|/* drop unfinished escape */
if|if
condition|(
literal|'0'
operator|<=
operator|*
name|bp
operator|&&
operator|*
name|bp
operator|<=
literal|'7'
condition|)
block|{
specifier|register
name|int
name|n
decl_stmt|,
name|i
decl_stmt|;
name|n
operator|=
literal|0
expr_stmt|;
name|i
operator|=
literal|3
expr_stmt|;
comment|/* maximum of three octal digits */
do|do
block|{
name|n
operator|=
name|n
operator|*
literal|8
operator|+
operator|(
operator|*
name|bp
operator|++
operator|-
literal|'0'
operator|)
expr_stmt|;
block|}
do|while
condition|(
operator|--
name|i
operator|&&
literal|'0'
operator|<=
operator|*
name|bp
operator|&&
operator|*
name|bp
operator|<=
literal|'7'
condition|)
do|;
operator|*
name|mp
operator|++
operator|=
name|n
expr_stmt|;
block|}
else|else
switch|switch
condition|(
operator|*
name|bp
operator|++
condition|)
block|{
case|case
literal|'b'
case|:
case|case
literal|'B'
case|:
operator|*
name|mp
operator|++
operator|=
literal|'\b'
expr_stmt|;
break|break;
case|case
literal|'t'
case|:
case|case
literal|'T'
case|:
operator|*
name|mp
operator|++
operator|=
literal|'\t'
expr_stmt|;
break|break;
case|case
literal|'n'
case|:
case|case
literal|'N'
case|:
operator|*
name|mp
operator|++
operator|=
literal|'\n'
expr_stmt|;
break|break;
case|case
literal|'f'
case|:
case|case
literal|'F'
case|:
operator|*
name|mp
operator|++
operator|=
literal|'\f'
expr_stmt|;
break|break;
case|case
literal|'r'
case|:
case|case
literal|'R'
case|:
operator|*
name|mp
operator|++
operator|=
literal|'\r'
expr_stmt|;
break|break;
case|case
literal|'e'
case|:
case|case
literal|'E'
case|:
operator|*
name|mp
operator|++
operator|=
name|ESC
expr_stmt|;
break|break;
case|case
literal|'c'
case|:
case|case
literal|'C'
case|:
operator|*
name|mp
operator|++
operator|=
literal|':'
expr_stmt|;
break|break;
default|default:
comment|/* 					 * Catches '\', '^', and 					 *  everything else. 					 */
operator|*
name|mp
operator|++
operator|=
operator|*
operator|(
name|bp
operator|-
literal|1
operator|)
expr_stmt|;
break|break;
block|}
block|}
else|else
operator|*
name|mp
operator|++
operator|=
operator|*
name|bp
operator|++
expr_stmt|;
name|m_room
operator|--
expr_stmt|;
comment|/* 		 * Enforce loop invariant: if no room left in current 		 * buffer, try to get some more. 		 */
if|if
condition|(
name|m_room
operator|==
literal|0
condition|)
block|{
name|u_int
name|size
init|=
name|mp
operator|-
name|mem
decl_stmt|;
if|if
condition|(
operator|(
name|mem
operator|=
name|realloc
argument_list|(
name|mem
argument_list|,
name|size
operator|+
name|SFRAG
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
operator|-
literal|2
operator|)
return|;
name|m_room
operator|=
name|SFRAG
expr_stmt|;
name|mp
operator|=
name|mem
operator|+
name|size
expr_stmt|;
block|}
block|}
operator|*
name|mp
operator|++
operator|=
literal|'\0'
expr_stmt|;
comment|/* loop invariant let's us do this */
name|m_room
operator|--
expr_stmt|;
name|len
operator|=
name|mp
operator|-
name|mem
operator|-
literal|1
expr_stmt|;
comment|/* 	 * Give back any extra memory and return value and success. 	 */
if|if
condition|(
name|m_room
operator|!=
literal|0
condition|)
name|mem
operator|=
name|realloc
argument_list|(
name|mem
argument_list|,
call|(
name|u_int
call|)
argument_list|(
name|mp
operator|-
name|mem
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|str
operator|=
name|mem
expr_stmt|;
return|return
operator|(
name|len
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Cgetustr retrieves the value of the string capability cap from the  * capability record pointed to by buf.  The difference between cgetustr()  * and cgetstr() is that cgetustr does not decode escapes but rather treats  * all characters literally.  A pointer to a  NUL terminated malloc'd   * copy of the string is returned in the char pointed to by str.  The   * length of the string not including the trailing NUL is returned on success,  * -1 if the requested string capability couldn't be found, -2 if a system   * error was encountered (storage allocation failure).  */
end_comment

begin_function
name|int
name|cgetustr
parameter_list|(
name|buf
parameter_list|,
name|cap
parameter_list|,
name|str
parameter_list|)
name|char
modifier|*
name|buf
decl_stmt|,
decl|*
name|cap
decl_stmt|,
modifier|*
modifier|*
name|str
decl_stmt|;
end_function

begin_block
block|{
specifier|register
name|u_int
name|m_room
decl_stmt|;
specifier|register
name|char
modifier|*
name|bp
decl_stmt|,
modifier|*
name|mp
decl_stmt|;
name|int
name|len
decl_stmt|;
name|char
modifier|*
name|mem
decl_stmt|;
comment|/* 	 * Find string capability cap 	 */
if|if
condition|(
operator|(
name|bp
operator|=
name|cgetcap
argument_list|(
name|buf
argument_list|,
name|cap
argument_list|,
literal|'='
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
comment|/* 	 * Conversion / storage allocation loop ...  Allocate memory in 	 * chunks SFRAG in size. 	 */
if|if
condition|(
operator|(
name|mem
operator|=
name|malloc
argument_list|(
name|SFRAG
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|errno
operator|=
name|ENOMEM
expr_stmt|;
return|return
operator|(
operator|-
literal|2
operator|)
return|;
comment|/* couldn't even allocate the first fragment */
block|}
name|m_room
operator|=
name|SFRAG
expr_stmt|;
name|mp
operator|=
name|mem
expr_stmt|;
while|while
condition|(
operator|*
name|bp
operator|!=
literal|':'
operator|&&
operator|*
name|bp
operator|!=
literal|'\0'
condition|)
block|{
comment|/* 		 * Loop invariants: 		 *	There is always room for one more character in mem. 		 *	Mp always points just past last character in mem. 		 *	Bp always points at next character in buf. 		 */
operator|*
name|mp
operator|++
operator|=
operator|*
name|bp
operator|++
expr_stmt|;
name|m_room
operator|--
expr_stmt|;
comment|/* 		 * Enforce loop invariant: if no room left in current 		 * buffer, try to get some more. 		 */
if|if
condition|(
name|m_room
operator|==
literal|0
condition|)
block|{
name|u_int
name|size
init|=
name|mp
operator|-
name|mem
decl_stmt|;
if|if
condition|(
operator|(
name|mem
operator|=
name|realloc
argument_list|(
name|mem
argument_list|,
name|size
operator|+
name|SFRAG
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return
operator|(
operator|-
literal|2
operator|)
return|;
name|m_room
operator|=
name|SFRAG
expr_stmt|;
name|mp
operator|=
name|mem
operator|+
name|size
expr_stmt|;
block|}
block|}
operator|*
name|mp
operator|++
operator|=
literal|'\0'
expr_stmt|;
comment|/* loop invariant let's us do this */
name|m_room
operator|--
expr_stmt|;
name|len
operator|=
name|mp
operator|-
name|mem
operator|-
literal|1
expr_stmt|;
comment|/* 	 * Give back any extra memory and return value and success. 	 */
if|if
condition|(
name|m_room
operator|!=
literal|0
condition|)
name|mem
operator|=
name|realloc
argument_list|(
name|mem
argument_list|,
call|(
name|u_int
call|)
argument_list|(
name|mp
operator|-
name|mem
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|str
operator|=
name|mem
expr_stmt|;
return|return
operator|(
name|len
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Cgetnum retrieves the value of the numeric capability cap from the  * capability record pointed to by buf.  The numeric value is returned in  * the long pointed to by num.  0 is returned on success, -1 if the requested  * numeric capability couldn't be found.  */
end_comment

begin_function
name|int
name|cgetnum
parameter_list|(
name|buf
parameter_list|,
name|cap
parameter_list|,
name|num
parameter_list|)
name|char
modifier|*
name|buf
decl_stmt|,
decl|*
name|cap
decl_stmt|;
end_function

begin_decl_stmt
name|long
modifier|*
name|num
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|long
name|n
decl_stmt|;
specifier|register
name|int
name|base
decl_stmt|,
name|digit
decl_stmt|;
specifier|register
name|char
modifier|*
name|bp
decl_stmt|;
comment|/* 	 * Find numeric capability cap 	 */
name|bp
operator|=
name|cgetcap
argument_list|(
name|buf
argument_list|,
name|cap
argument_list|,
literal|'#'
argument_list|)
expr_stmt|;
if|if
condition|(
name|bp
operator|==
name|NULL
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
comment|/* 	 * Look at value and determine numeric base: 	 *	0x... or 0X...	hexadecimal, 	 * else	0...		octal, 	 * else			decimal. 	 */
if|if
condition|(
operator|*
name|bp
operator|==
literal|'0'
condition|)
block|{
name|bp
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|bp
operator|==
literal|'x'
operator|||
operator|*
name|bp
operator|==
literal|'X'
condition|)
block|{
name|bp
operator|++
expr_stmt|;
name|base
operator|=
literal|16
expr_stmt|;
block|}
else|else
name|base
operator|=
literal|8
expr_stmt|;
block|}
else|else
name|base
operator|=
literal|10
expr_stmt|;
comment|/* 	 * Conversion loop ... 	 */
name|n
operator|=
literal|0
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
literal|'0'
operator|<=
operator|*
name|bp
operator|&&
operator|*
name|bp
operator|<=
literal|'9'
condition|)
name|digit
operator|=
operator|*
name|bp
operator|-
literal|'0'
expr_stmt|;
elseif|else
if|if
condition|(
operator|(
literal|'a'
operator|<=
operator|*
name|bp
operator|&&
operator|*
name|bp
operator|<=
literal|'f'
operator|)
operator|||
operator|(
literal|'A'
operator|<=
operator|*
name|bp
operator|&&
operator|*
name|bp
operator|<=
literal|'F'
operator|)
condition|)
name|digit
operator|=
literal|10
operator|+
operator|*
name|bp
operator|-
literal|'a'
expr_stmt|;
else|else
break|break;
if|if
condition|(
name|digit
operator|>=
name|base
condition|)
break|break;
name|n
operator|=
name|n
operator|*
name|base
operator|+
name|digit
expr_stmt|;
name|bp
operator|++
expr_stmt|;
block|}
comment|/* 	 * Return value and success. 	 */
operator|*
name|num
operator|=
name|n
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

end_unit

