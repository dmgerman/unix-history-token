begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1981, 1993, 1994  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)refresh.c	8.3 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|"curses.h"
end_include

begin_decl_stmt
specifier|static
name|int
name|curwin
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|short
name|ly
decl_stmt|,
name|lx
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|domvcur
name|__P
argument_list|(
operator|(
name|int
operator|,
name|int
operator|,
name|int
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|makech
name|__P
argument_list|(
operator|(
name|WINDOW
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|quickch
name|__P
argument_list|(
operator|(
name|WINDOW
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|scrolln
name|__P
argument_list|(
operator|(
name|WINDOW
operator|*
operator|,
name|int
operator|,
name|int
operator|,
name|int
operator|,
name|int
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * wrefresh --  *	Make the current screen look like "win" over the area coverd by  *	win.  */
end_comment

begin_function
name|int
name|wrefresh
parameter_list|(
name|win
parameter_list|)
specifier|register
name|WINDOW
modifier|*
name|win
decl_stmt|;
block|{
specifier|register
name|__LINE
modifier|*
name|wlp
decl_stmt|;
specifier|register
name|int
name|retval
decl_stmt|;
specifier|register
name|short
name|wy
decl_stmt|;
name|int
name|dnum
decl_stmt|;
comment|/* Initialize loop parameters. */
name|ly
operator|=
name|curscr
operator|->
name|cury
expr_stmt|;
name|lx
operator|=
name|curscr
operator|->
name|curx
expr_stmt|;
name|wy
operator|=
literal|0
expr_stmt|;
name|curwin
operator|=
operator|(
name|win
operator|==
name|curscr
operator|)
expr_stmt|;
if|if
condition|(
operator|!
name|curwin
condition|)
for|for
control|(
name|wy
operator|=
literal|0
init|;
name|wy
operator|<
name|win
operator|->
name|maxy
condition|;
name|wy
operator|++
control|)
block|{
name|wlp
operator|=
name|win
operator|->
name|lines
index|[
name|wy
index|]
expr_stmt|;
if|if
condition|(
name|wlp
operator|->
name|flags
operator|&
name|__ISDIRTY
condition|)
name|wlp
operator|->
name|hash
operator|=
name|__hash
argument_list|(
operator|(
name|char
operator|*
operator|)
name|wlp
operator|->
name|line
argument_list|,
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|win
operator|->
name|flags
operator|&
name|__CLEAROK
operator|||
name|curscr
operator|->
name|flags
operator|&
name|__CLEAROK
operator|||
name|curwin
condition|)
block|{
if|if
condition|(
operator|(
name|win
operator|->
name|flags
operator|&
name|__FULLWIN
operator|)
operator|||
name|curscr
operator|->
name|flags
operator|&
name|__CLEAROK
condition|)
block|{
name|tputs
argument_list|(
name|CL
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|ly
operator|=
literal|0
expr_stmt|;
name|lx
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
name|curwin
condition|)
block|{
name|curscr
operator|->
name|flags
operator|&=
operator|~
name|__CLEAROK
expr_stmt|;
name|curscr
operator|->
name|cury
operator|=
literal|0
expr_stmt|;
name|curscr
operator|->
name|curx
operator|=
literal|0
expr_stmt|;
name|werase
argument_list|(
name|curscr
argument_list|)
expr_stmt|;
block|}
name|__touchwin
argument_list|(
name|win
argument_list|)
expr_stmt|;
block|}
name|win
operator|->
name|flags
operator|&=
operator|~
name|__CLEAROK
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|CA
condition|)
block|{
if|if
condition|(
name|win
operator|->
name|curx
operator|!=
literal|0
condition|)
name|putchar
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|curwin
condition|)
name|werase
argument_list|(
name|curscr
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"wrefresh: (%0.2o): curwin = %d\n"
argument_list|,
name|win
argument_list|,
name|curwin
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"wrefresh: \tfirstch\tlastch\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifndef|#
directive|ifndef
name|NOQCH
if|if
condition|(
operator|(
name|win
operator|->
name|flags
operator|&
name|__FULLWIN
operator|)
operator|&&
operator|!
name|curwin
condition|)
block|{
comment|/* 		 * Invoke quickch() only if more than a quarter of the lines 		 * in the window are dirty. 		 */
for|for
control|(
name|wy
operator|=
literal|0
operator|,
name|dnum
operator|=
literal|0
init|;
name|wy
operator|<
name|win
operator|->
name|maxy
condition|;
name|wy
operator|++
control|)
if|if
condition|(
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|flags
operator|&
operator|(
name|__ISDIRTY
operator||
name|__FORCEPAINT
operator|)
condition|)
name|dnum
operator|++
expr_stmt|;
if|if
condition|(
operator|!
name|__noqch
operator|&&
name|dnum
operator|>
operator|(
name|int
operator|)
name|win
operator|->
name|maxy
operator|/
literal|4
condition|)
name|quickch
argument_list|(
name|win
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
ifdef|#
directive|ifdef
name|DEBUG
block|{
name|int
name|i
decl_stmt|,
name|j
decl_stmt|;
name|__CTRACE
argument_list|(
literal|"#####################################\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|curscr
operator|->
name|maxy
condition|;
name|i
operator|++
control|)
block|{
name|__CTRACE
argument_list|(
literal|"C: %d:"
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|" 0x%x \n"
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|hash
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|curscr
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%c"
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|ch
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|curscr
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%x"
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|attr
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"W: %d:"
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|" 0x%x \n"
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|hash
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|" 0x%x "
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|flags
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|win
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%c"
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|ch
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|win
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%x"
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|attr
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
block|}
endif|#
directive|endif
comment|/* DEBUG */
for|for
control|(
name|wy
operator|=
literal|0
init|;
name|wy
operator|<
name|win
operator|->
name|maxy
condition|;
name|wy
operator|++
control|)
block|{
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"%d\t%d\t%d\n"
argument_list|,
name|wy
argument_list|,
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|firstchp
argument_list|,
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|lastchp
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
operator|!
name|curwin
condition|)
name|curscr
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|hash
operator|=
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|hash
expr_stmt|;
if|if
condition|(
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|flags
operator|&
operator|(
name|__ISDIRTY
operator||
name|__FORCEPAINT
operator|)
condition|)
block|{
if|if
condition|(
name|makech
argument_list|(
name|win
argument_list|,
name|wy
argument_list|)
operator|==
name|ERR
condition|)
return|return
operator|(
name|ERR
operator|)
return|;
else|else
block|{
if|if
condition|(
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|firstchp
operator|>=
name|win
operator|->
name|ch_off
condition|)
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|firstchp
operator|=
name|win
operator|->
name|maxx
operator|+
name|win
operator|->
name|ch_off
expr_stmt|;
if|if
condition|(
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|lastchp
operator|<
name|win
operator|->
name|maxx
operator|+
name|win
operator|->
name|ch_off
condition|)
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|lastchp
operator|=
name|win
operator|->
name|ch_off
expr_stmt|;
if|if
condition|(
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|lastchp
operator|<
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|firstchp
condition|)
block|{
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"wrefresh: line %d notdirty \n"
argument_list|,
name|wy
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|flags
operator|&=
operator|~
name|__ISDIRTY
expr_stmt|;
block|}
block|}
block|}
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"\t%d\t%d\n"
argument_list|,
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|firstchp
argument_list|,
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|lastchp
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"refresh: ly=%d, lx=%d\n"
argument_list|,
name|ly
argument_list|,
name|lx
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|win
operator|==
name|curscr
condition|)
name|domvcur
argument_list|(
name|ly
argument_list|,
name|lx
argument_list|,
name|win
operator|->
name|cury
argument_list|,
name|win
operator|->
name|curx
argument_list|)
expr_stmt|;
else|else
block|{
if|if
condition|(
name|win
operator|->
name|flags
operator|&
name|__LEAVEOK
condition|)
block|{
name|curscr
operator|->
name|cury
operator|=
name|ly
expr_stmt|;
name|curscr
operator|->
name|curx
operator|=
name|lx
expr_stmt|;
name|ly
operator|-=
name|win
operator|->
name|begy
expr_stmt|;
name|lx
operator|-=
name|win
operator|->
name|begx
expr_stmt|;
if|if
condition|(
name|ly
operator|>=
literal|0
operator|&&
name|ly
operator|<
name|win
operator|->
name|maxy
operator|&&
name|lx
operator|>=
literal|0
operator|&&
name|lx
operator|<
name|win
operator|->
name|maxx
condition|)
block|{
name|win
operator|->
name|cury
operator|=
name|ly
expr_stmt|;
name|win
operator|->
name|curx
operator|=
name|lx
expr_stmt|;
block|}
else|else
name|win
operator|->
name|cury
operator|=
name|win
operator|->
name|curx
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|domvcur
argument_list|(
name|ly
argument_list|,
name|lx
argument_list|,
name|win
operator|->
name|cury
operator|+
name|win
operator|->
name|begy
argument_list|,
name|win
operator|->
name|curx
operator|+
name|win
operator|->
name|begx
argument_list|)
expr_stmt|;
name|curscr
operator|->
name|cury
operator|=
name|win
operator|->
name|cury
operator|+
name|win
operator|->
name|begy
expr_stmt|;
name|curscr
operator|->
name|curx
operator|=
name|win
operator|->
name|curx
operator|+
name|win
operator|->
name|begx
expr_stmt|;
block|}
block|}
name|retval
operator|=
name|OK
expr_stmt|;
operator|(
name|void
operator|)
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
return|return
operator|(
name|retval
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * makech --  *	Make a change on the screen.  */
end_comment

begin_function
specifier|static
name|int
name|makech
parameter_list|(
name|win
parameter_list|,
name|wy
parameter_list|)
specifier|register
name|WINDOW
modifier|*
name|win
decl_stmt|;
name|int
name|wy
decl_stmt|;
block|{
specifier|static
name|__LDATA
name|blank
init|=
block|{
literal|' '
block|,
literal|0
block|}
decl_stmt|;
specifier|register
name|int
name|nlsp
decl_stmt|,
name|clsp
decl_stmt|;
comment|/* Last space in lines. */
specifier|register
name|int
name|wx
decl_stmt|,
name|lch
decl_stmt|,
name|y
decl_stmt|;
specifier|register
name|__LDATA
modifier|*
name|nsp
decl_stmt|,
modifier|*
name|csp
decl_stmt|,
modifier|*
name|cp
decl_stmt|,
modifier|*
name|cep
decl_stmt|;
name|u_int
name|force
decl_stmt|;
name|char
modifier|*
name|ce
decl_stmt|;
comment|/* Is the cursor still on the end of the last line? */
if|if
condition|(
name|wy
operator|>
literal|0
operator|&&
name|win
operator|->
name|lines
index|[
name|wy
operator|-
literal|1
index|]
operator|->
name|flags
operator|&
name|__ISPASTEOL
condition|)
block|{
name|domvcur
argument_list|(
name|ly
argument_list|,
name|lx
argument_list|,
name|ly
operator|+
literal|1
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|ly
operator|++
expr_stmt|;
name|lx
operator|=
literal|0
expr_stmt|;
block|}
name|wx
operator|=
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|firstchp
operator|-
name|win
operator|->
name|ch_off
expr_stmt|;
if|if
condition|(
name|wx
operator|<
literal|0
condition|)
name|wx
operator|=
literal|0
expr_stmt|;
elseif|else
if|if
condition|(
name|wx
operator|>=
name|win
operator|->
name|maxx
condition|)
return|return
operator|(
name|OK
operator|)
return|;
name|lch
operator|=
operator|*
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|lastchp
operator|-
name|win
operator|->
name|ch_off
expr_stmt|;
if|if
condition|(
name|lch
operator|<
literal|0
condition|)
return|return
operator|(
name|OK
operator|)
return|;
elseif|else
if|if
condition|(
name|lch
operator|>=
operator|(
name|int
operator|)
name|win
operator|->
name|maxx
condition|)
name|lch
operator|=
name|win
operator|->
name|maxx
operator|-
literal|1
expr_stmt|;
name|y
operator|=
name|wy
operator|+
name|win
operator|->
name|begy
expr_stmt|;
if|if
condition|(
name|curwin
condition|)
name|csp
operator|=
operator|&
name|blank
expr_stmt|;
else|else
name|csp
operator|=
operator|&
name|curscr
operator|->
name|lines
index|[
name|wy
operator|+
name|win
operator|->
name|begy
index|]
operator|->
name|line
index|[
name|wx
operator|+
name|win
operator|->
name|begx
index|]
expr_stmt|;
name|nsp
operator|=
operator|&
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|line
index|[
name|wx
index|]
expr_stmt|;
name|force
operator|=
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|flags
operator|&
name|__FORCEPAINT
expr_stmt|;
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|flags
operator|&=
operator|~
name|__FORCEPAINT
expr_stmt|;
if|if
condition|(
name|CE
operator|&&
operator|!
name|curwin
condition|)
block|{
for|for
control|(
name|cp
operator|=
operator|&
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|line
index|[
name|win
operator|->
name|maxx
operator|-
literal|1
index|]
init|;
name|cp
operator|->
name|ch
operator|==
literal|' '
operator|&&
name|cp
operator|->
name|attr
operator|==
literal|0
condition|;
name|cp
operator|--
control|)
if|if
condition|(
name|cp
operator|<=
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|line
condition|)
break|break;
name|nlsp
operator|=
name|cp
operator|-
name|win
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|line
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|curwin
condition|)
name|ce
operator|=
name|CE
expr_stmt|;
else|else
name|ce
operator|=
name|NULL
expr_stmt|;
if|if
condition|(
name|force
condition|)
block|{
if|if
condition|(
name|CM
condition|)
name|tputs
argument_list|(
name|tgoto
argument_list|(
name|CM
argument_list|,
name|lx
argument_list|,
name|ly
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
block|{
name|tputs
argument_list|(
name|HO
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|,
name|ly
argument_list|,
name|lx
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
while|while
condition|(
name|wx
operator|<=
name|lch
condition|)
block|{
if|if
condition|(
operator|!
name|force
operator|&&
name|memcmp
argument_list|(
name|nsp
argument_list|,
name|csp
argument_list|,
sizeof|sizeof
argument_list|(
name|__LDATA
argument_list|)
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|wx
operator|<=
name|lch
condition|)
block|{
while|while
condition|(
name|wx
operator|<=
name|lch
operator|&&
name|memcmp
argument_list|(
name|nsp
argument_list|,
name|csp
argument_list|,
sizeof|sizeof
argument_list|(
name|__LDATA
argument_list|)
argument_list|)
operator|==
literal|0
condition|)
block|{
name|nsp
operator|++
expr_stmt|;
if|if
condition|(
operator|!
name|curwin
condition|)
name|csp
operator|++
expr_stmt|;
operator|++
name|wx
expr_stmt|;
block|}
continue|continue;
block|}
break|break;
block|}
name|domvcur
argument_list|(
name|ly
argument_list|,
name|lx
argument_list|,
name|y
argument_list|,
name|wx
operator|+
name|win
operator|->
name|begx
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"makech: 1: wx = %d, ly= %d, lx = %d, newy = %d, newx = %d, force =%d\n"
argument_list|,
name|wx
argument_list|,
name|ly
argument_list|,
name|lx
argument_list|,
name|y
argument_list|,
name|wx
operator|+
name|win
operator|->
name|begx
argument_list|,
name|force
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|ly
operator|=
name|y
expr_stmt|;
name|lx
operator|=
name|wx
operator|+
name|win
operator|->
name|begx
expr_stmt|;
while|while
condition|(
operator|(
name|force
operator|||
name|memcmp
argument_list|(
name|nsp
argument_list|,
name|csp
argument_list|,
sizeof|sizeof
argument_list|(
name|__LDATA
argument_list|)
argument_list|)
operator|!=
literal|0
operator|)
operator|&&
name|wx
operator|<=
name|lch
condition|)
block|{
if|if
condition|(
name|ce
operator|!=
name|NULL
operator|&&
name|win
operator|->
name|maxx
operator|+
name|win
operator|->
name|begx
operator|==
name|curscr
operator|->
name|maxx
operator|&&
name|wx
operator|>=
name|nlsp
operator|&&
name|nsp
operator|->
name|ch
operator|==
literal|' '
condition|)
block|{
comment|/* Check for clear to end-of-line. */
name|cep
operator|=
operator|&
name|curscr
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|line
index|[
name|win
operator|->
name|maxx
operator|-
literal|1
index|]
expr_stmt|;
while|while
condition|(
name|cep
operator|->
name|ch
operator|==
literal|' '
operator|&&
name|cep
operator|->
name|attr
operator|==
literal|0
condition|)
if|if
condition|(
name|cep
operator|--
operator|<=
name|csp
condition|)
break|break;
name|clsp
operator|=
name|cep
operator|-
name|curscr
operator|->
name|lines
index|[
name|wy
index|]
operator|->
name|line
operator|-
name|win
operator|->
name|begx
operator|*
name|__LDATASIZE
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"makech: clsp = %d, nlsp = %d\n"
argument_list|,
name|clsp
argument_list|,
name|nlsp
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
operator|(
name|clsp
operator|-
name|nlsp
operator|>=
name|strlen
argument_list|(
name|CE
argument_list|)
operator|&&
name|clsp
operator|<
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
operator|)
operator|||
name|wy
operator|==
name|win
operator|->
name|maxy
operator|-
literal|1
condition|)
block|{
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"makech: using CE\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|tputs
argument_list|(
name|CE
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|lx
operator|=
name|wx
operator|+
name|win
operator|->
name|begx
expr_stmt|;
while|while
condition|(
name|wx
operator|++
operator|<=
name|clsp
condition|)
block|{
name|csp
operator|->
name|ch
operator|=
literal|' '
expr_stmt|;
name|csp
operator|->
name|attr
operator|=
literal|0
expr_stmt|;
name|csp
operator|++
expr_stmt|;
block|}
return|return
operator|(
name|OK
operator|)
return|;
block|}
name|ce
operator|=
name|NULL
expr_stmt|;
block|}
comment|/* Enter/exit standout mode as appropriate. */
if|if
condition|(
name|SO
operator|&&
operator|(
name|nsp
operator|->
name|attr
operator|&
name|__STANDOUT
operator|)
operator|!=
operator|(
name|curscr
operator|->
name|flags
operator|&
name|__WSTANDOUT
operator|)
condition|)
block|{
if|if
condition|(
name|nsp
operator|->
name|attr
operator|&
name|__STANDOUT
condition|)
block|{
name|tputs
argument_list|(
name|SO
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|curscr
operator|->
name|flags
operator||=
name|__WSTANDOUT
expr_stmt|;
block|}
else|else
block|{
name|tputs
argument_list|(
name|SE
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|curscr
operator|->
name|flags
operator|&=
operator|~
name|__WSTANDOUT
expr_stmt|;
block|}
block|}
name|wx
operator|++
expr_stmt|;
if|if
condition|(
name|wx
operator|>=
name|win
operator|->
name|maxx
operator|&&
name|wy
operator|==
name|win
operator|->
name|maxy
operator|-
literal|1
operator|&&
operator|!
name|curwin
condition|)
if|if
condition|(
name|win
operator|->
name|flags
operator|&
name|__SCROLLOK
condition|)
block|{
if|if
condition|(
name|curscr
operator|->
name|flags
operator|&
name|__WSTANDOUT
operator|&&
name|win
operator|->
name|flags
operator|&
name|__ENDLINE
condition|)
if|if
condition|(
operator|!
name|MS
condition|)
block|{
name|tputs
argument_list|(
name|SE
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|curscr
operator|->
name|flags
operator|&=
operator|~
name|__WSTANDOUT
expr_stmt|;
block|}
if|if
condition|(
operator|!
operator|(
name|win
operator|->
name|flags
operator|&
name|__SCROLLWIN
operator|)
condition|)
block|{
if|if
condition|(
operator|!
name|curwin
condition|)
block|{
name|csp
operator|->
name|attr
operator|=
name|nsp
operator|->
name|attr
expr_stmt|;
name|putchar
argument_list|(
name|csp
operator|->
name|ch
operator|=
name|nsp
operator|->
name|ch
argument_list|)
expr_stmt|;
block|}
else|else
name|putchar
argument_list|(
name|nsp
operator|->
name|ch
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|wx
operator|+
name|win
operator|->
name|begx
operator|<
name|curscr
operator|->
name|maxx
condition|)
block|{
name|domvcur
argument_list|(
name|ly
argument_list|,
name|wx
operator|+
name|win
operator|->
name|begx
argument_list|,
name|win
operator|->
name|begy
operator|+
name|win
operator|->
name|maxy
operator|-
literal|1
argument_list|,
name|win
operator|->
name|begx
operator|+
name|win
operator|->
name|maxx
operator|-
literal|1
argument_list|)
expr_stmt|;
block|}
name|ly
operator|=
name|win
operator|->
name|begy
operator|+
name|win
operator|->
name|maxy
operator|-
literal|1
expr_stmt|;
name|lx
operator|=
name|win
operator|->
name|begx
operator|+
name|win
operator|->
name|maxx
operator|-
literal|1
expr_stmt|;
return|return
operator|(
name|OK
operator|)
return|;
block|}
if|if
condition|(
name|wx
operator|<
name|win
operator|->
name|maxx
operator|||
name|wy
operator|<
name|win
operator|->
name|maxy
operator|-
literal|1
operator|||
operator|!
operator|(
name|win
operator|->
name|flags
operator|&
name|__SCROLLWIN
operator|)
condition|)
block|{
if|if
condition|(
operator|!
name|curwin
condition|)
block|{
name|csp
operator|->
name|attr
operator|=
name|nsp
operator|->
name|attr
expr_stmt|;
name|putchar
argument_list|(
name|csp
operator|->
name|ch
operator|=
name|nsp
operator|->
name|ch
argument_list|)
expr_stmt|;
name|csp
operator|++
expr_stmt|;
block|}
else|else
name|putchar
argument_list|(
name|nsp
operator|->
name|ch
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"makech: putchar(%c)\n"
argument_list|,
name|nsp
operator|->
name|ch
operator|&
literal|0177
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|UC
operator|&&
operator|(
name|nsp
operator|->
name|attr
operator|&
name|__STANDOUT
operator|)
condition|)
block|{
name|putchar
argument_list|(
literal|'\b'
argument_list|)
expr_stmt|;
name|tputs
argument_list|(
name|UC
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
block|}
name|nsp
operator|++
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"makech: 2: wx = %d, lx = %d\n"
argument_list|,
name|wx
argument_list|,
name|lx
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
if|if
condition|(
name|lx
operator|==
name|wx
operator|+
name|win
operator|->
name|begx
condition|)
comment|/* If no change. */
break|break;
name|lx
operator|=
name|wx
operator|+
name|win
operator|->
name|begx
expr_stmt|;
if|if
condition|(
name|lx
operator|>=
name|COLS
operator|&&
name|AM
condition|)
name|lx
operator|=
name|COLS
operator|-
literal|1
expr_stmt|;
elseif|else
if|if
condition|(
name|wx
operator|>=
name|win
operator|->
name|maxx
condition|)
block|{
name|domvcur
argument_list|(
name|ly
argument_list|,
name|lx
argument_list|,
name|ly
argument_list|,
name|win
operator|->
name|maxx
operator|+
name|win
operator|->
name|begx
operator|-
literal|1
argument_list|)
expr_stmt|;
name|lx
operator|=
name|win
operator|->
name|maxx
operator|+
name|win
operator|->
name|begx
operator|-
literal|1
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"makech: 3: wx = %d, lx = %d\n"
argument_list|,
name|wx
argument_list|,
name|lx
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
return|return
operator|(
name|OK
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * domvcur --  *	Do a mvcur, leaving standout mode if necessary.  */
end_comment

begin_function
specifier|static
name|void
name|domvcur
parameter_list|(
name|oy
parameter_list|,
name|ox
parameter_list|,
name|ny
parameter_list|,
name|nx
parameter_list|)
name|int
name|oy
decl_stmt|,
name|ox
decl_stmt|,
name|ny
decl_stmt|,
name|nx
decl_stmt|;
block|{
if|if
condition|(
name|curscr
operator|->
name|flags
operator|&
name|__WSTANDOUT
operator|&&
operator|!
name|MS
condition|)
block|{
name|tputs
argument_list|(
name|SE
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|curscr
operator|->
name|flags
operator|&=
operator|~
name|__WSTANDOUT
expr_stmt|;
block|}
name|__mvcur
argument_list|(
name|oy
argument_list|,
name|ox
argument_list|,
name|ny
argument_list|,
name|nx
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Quickch() attempts to detect a pattern in the change of the window  * in order to optimize the change, e.g., scroll n lines as opposed to  * repainting the screen line by line.  */
end_comment

begin_function
specifier|static
name|void
name|quickch
parameter_list|(
name|win
parameter_list|)
name|WINDOW
modifier|*
name|win
decl_stmt|;
block|{
define|#
directive|define
name|THRESH
value|(int) win->maxy / 4
specifier|register
name|__LINE
modifier|*
name|clp
decl_stmt|,
modifier|*
name|tmp1
decl_stmt|,
modifier|*
name|tmp2
decl_stmt|;
specifier|register
name|int
name|bsize
decl_stmt|,
name|curs
decl_stmt|,
name|curw
decl_stmt|,
name|starts
decl_stmt|,
name|startw
decl_stmt|,
name|i
decl_stmt|,
name|j
decl_stmt|;
name|int
name|n
decl_stmt|,
name|target
decl_stmt|,
name|cur_period
decl_stmt|,
name|bot
decl_stmt|,
name|top
decl_stmt|,
name|sc_region
decl_stmt|;
name|__LDATA
name|buf
index|[
literal|1024
index|]
decl_stmt|;
name|u_int
name|blank_hash
decl_stmt|;
comment|/* 	 * Find how many lines from the top of the screen are unchanged. 	 */
for|for
control|(
name|top
operator|=
literal|0
init|;
name|top
operator|<
name|win
operator|->
name|maxy
condition|;
name|top
operator|++
control|)
if|if
condition|(
name|win
operator|->
name|lines
index|[
name|top
index|]
operator|->
name|flags
operator|&
name|__FORCEPAINT
operator|||
name|win
operator|->
name|lines
index|[
name|top
index|]
operator|->
name|hash
operator|!=
name|curscr
operator|->
name|lines
index|[
name|top
index|]
operator|->
name|hash
operator|||
name|memcmp
argument_list|(
name|win
operator|->
name|lines
index|[
name|top
index|]
operator|->
name|line
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|top
index|]
operator|->
name|line
argument_list|,
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
argument_list|)
operator|!=
literal|0
condition|)
break|break;
else|else
name|win
operator|->
name|lines
index|[
name|top
index|]
operator|->
name|flags
operator|&=
operator|~
name|__ISDIRTY
expr_stmt|;
comment|/* 	* Find how many lines from bottom of screen are unchanged. 	*/
for|for
control|(
name|bot
operator|=
name|win
operator|->
name|maxy
operator|-
literal|1
init|;
name|bot
operator|>=
literal|0
condition|;
name|bot
operator|--
control|)
if|if
condition|(
name|win
operator|->
name|lines
index|[
name|bot
index|]
operator|->
name|flags
operator|&
name|__FORCEPAINT
operator|||
name|win
operator|->
name|lines
index|[
name|bot
index|]
operator|->
name|hash
operator|!=
name|curscr
operator|->
name|lines
index|[
name|bot
index|]
operator|->
name|hash
operator|||
name|memcmp
argument_list|(
name|win
operator|->
name|lines
index|[
name|bot
index|]
operator|->
name|line
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|bot
index|]
operator|->
name|line
argument_list|,
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
argument_list|)
operator|!=
literal|0
condition|)
break|break;
else|else
name|win
operator|->
name|lines
index|[
name|bot
index|]
operator|->
name|flags
operator|&=
operator|~
name|__ISDIRTY
expr_stmt|;
ifdef|#
directive|ifdef
name|NO_JERKINESS
comment|/* 	 * If we have a bottom unchanged region return.  Scrolling the 	 * bottom region up and then back down causes a screen jitter. 	 * This will increase the number of characters sent to the screen 	 * but it looks better. 	 */
if|if
condition|(
name|bot
operator|<
name|win
operator|->
name|maxy
operator|-
literal|1
condition|)
return|return;
endif|#
directive|endif
comment|/* NO_JERKINESS */
comment|/* 	 * Search for the largest block of text not changed. 	 * Invariants of the loop: 	 * - Startw is the index of the beginning of the examined block in win.          * - Starts is the index of the beginning of the examined block in 	 *    curscr. 	 * - Curs is the index of one past the end of the exmined block in win. 	 * - Curw is the index of one past the end of the exmined block in 	 *   curscr. 	 * - bsize is the current size of the examined block.          */
for|for
control|(
name|bsize
operator|=
name|bot
operator|-
name|top
init|;
name|bsize
operator|>=
name|THRESH
condition|;
name|bsize
operator|--
control|)
block|{
for|for
control|(
name|startw
operator|=
name|top
init|;
name|startw
operator|<=
name|bot
operator|-
name|bsize
condition|;
name|startw
operator|++
control|)
for|for
control|(
name|starts
operator|=
name|top
init|;
name|starts
operator|<=
name|bot
operator|-
name|bsize
condition|;
name|starts
operator|++
control|)
block|{
for|for
control|(
name|curw
operator|=
name|startw
operator|,
name|curs
operator|=
name|starts
init|;
name|curs
operator|<
name|starts
operator|+
name|bsize
condition|;
name|curw
operator|++
operator|,
name|curs
operator|++
control|)
if|if
condition|(
name|win
operator|->
name|lines
index|[
name|curw
index|]
operator|->
name|flags
operator|&
name|__FORCEPAINT
operator|||
operator|(
name|win
operator|->
name|lines
index|[
name|curw
index|]
operator|->
name|hash
operator|!=
name|curscr
operator|->
name|lines
index|[
name|curs
index|]
operator|->
name|hash
operator|||
name|memcmp
argument_list|(
name|win
operator|->
name|lines
index|[
name|curw
index|]
operator|->
name|line
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|curs
index|]
operator|->
name|line
argument_list|,
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
argument_list|)
operator|!=
literal|0
operator|)
condition|)
break|break;
if|if
condition|(
name|curs
operator|==
name|starts
operator|+
name|bsize
condition|)
goto|goto
name|done
goto|;
block|}
block|}
name|done
label|:
comment|/* Did not find anything */
if|if
condition|(
name|bsize
operator|<
name|THRESH
condition|)
return|return;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"quickch:bsize=%d,starts=%d,startw=%d,curw=%d,curs=%d,top=%d,bot=%d\n"
argument_list|,
name|bsize
argument_list|,
name|starts
argument_list|,
name|startw
argument_list|,
name|curw
argument_list|,
name|curs
argument_list|,
name|top
argument_list|,
name|bot
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* 	 * Make sure that there is no overlap between the bottom and top 	 * regions and the middle scrolled block. 	 */
if|if
condition|(
name|bot
operator|<
name|curs
condition|)
name|bot
operator|=
name|curs
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|top
operator|>
name|starts
condition|)
name|top
operator|=
name|starts
expr_stmt|;
name|n
operator|=
name|startw
operator|-
name|starts
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"#####################################\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|curscr
operator|->
name|maxy
condition|;
name|i
operator|++
control|)
block|{
name|__CTRACE
argument_list|(
literal|"C: %d:"
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|" 0x%x \n"
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|hash
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|curscr
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%c"
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|ch
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|curscr
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%x"
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|attr
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"W: %d:"
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|" 0x%x \n"
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|hash
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|" 0x%x "
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|flags
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|win
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%c"
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|ch
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|win
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%x"
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|attr
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
comment|/* So we don't have to call __hash() each time */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|win
operator|->
name|maxx
condition|;
name|i
operator|++
control|)
block|{
name|buf
index|[
name|i
index|]
operator|.
name|ch
operator|=
literal|' '
expr_stmt|;
name|buf
index|[
name|i
index|]
operator|.
name|attr
operator|=
literal|0
expr_stmt|;
block|}
name|blank_hash
operator|=
name|__hash
argument_list|(
operator|(
name|char
operator|*
operator|)
name|buf
argument_list|,
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
argument_list|)
expr_stmt|;
comment|/* 	 * Perform the rotation to maintain the consistency of curscr. 	 * This is hairy since we are doing an *in place* rotation. 	 * Invariants of the loop: 	 * - I is the index of the current line. 	 * - Target is the index of the target of line i. 	 * - Tmp1 points to current line (i). 	 * - Tmp2 and points to target line (target); 	 * - Cur_period is the index of the end of the current period. 	 *   (see below). 	 * 	 * There are 2 major issues here that make this rotation non-trivial: 	 * 1.  Scrolling in a scrolling region bounded by the top 	 *     and bottom regions determined (whose size is sc_region). 	 * 2.  As a result of the use of the mod function, there may be a 	 *     period introduced, i.e., 2 maps to 4, 4 to 6, n-2 to 0, and 	 *     0 to 2, which then causes all odd lines not to be rotated. 	 *     To remedy this, an index of the end ( = beginning) of the 	 *     current 'period' is kept, cur_period, and when it is reached, 	 *     the next period is started from cur_period + 1 which is 	 *     guaranteed not to have been reached since that would mean that 	 *     all records would have been reached. (think about it...). 	 * 	 * Lines in the rotation can have 3 attributes which are marked on the 	 * line so that curscr is consistent with the visual screen. 	 * 1.  Not dirty -- lines inside the scrolled block, top region or 	 *                  bottom region. 	 * 2.  Blank lines -- lines in the differential of the scrolling 	 *		      region adjacent to top and bot regions 	 *                    depending on scrolling direction. 	 * 3.  Dirty line -- all other lines are marked dirty. 	 */
name|sc_region
operator|=
name|bot
operator|-
name|top
operator|+
literal|1
expr_stmt|;
name|i
operator|=
name|top
expr_stmt|;
name|tmp1
operator|=
name|curscr
operator|->
name|lines
index|[
name|top
index|]
expr_stmt|;
name|cur_period
operator|=
name|top
expr_stmt|;
for|for
control|(
name|j
operator|=
name|top
init|;
name|j
operator|<=
name|bot
condition|;
name|j
operator|++
control|)
block|{
name|target
operator|=
operator|(
name|i
operator|-
name|top
operator|+
name|n
operator|+
name|sc_region
operator|)
operator|%
name|sc_region
operator|+
name|top
expr_stmt|;
name|tmp2
operator|=
name|curscr
operator|->
name|lines
index|[
name|target
index|]
expr_stmt|;
name|curscr
operator|->
name|lines
index|[
name|target
index|]
operator|=
name|tmp1
expr_stmt|;
comment|/* Mark block as clean and blank out scrolled lines. */
name|clp
operator|=
name|curscr
operator|->
name|lines
index|[
name|target
index|]
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"quickch: n=%d startw=%d curw=%d i = %d target=%d "
argument_list|,
name|n
argument_list|,
name|startw
argument_list|,
name|curw
argument_list|,
name|i
argument_list|,
name|target
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
operator|(
name|target
operator|>=
name|startw
operator|&&
name|target
operator|<
name|curw
operator|)
operator|||
name|target
operator|<
name|top
operator|||
name|target
operator|>
name|bot
condition|)
block|{
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"-- notdirty"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|win
operator|->
name|lines
index|[
name|target
index|]
operator|->
name|flags
operator|&=
operator|~
name|__ISDIRTY
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|n
operator|>
literal|0
operator|&&
name|target
operator|>=
name|top
operator|&&
name|target
operator|<
name|top
operator|+
name|n
operator|)
operator|||
operator|(
name|n
operator|<
literal|0
operator|&&
name|target
operator|<=
name|bot
operator|&&
name|target
operator|>
name|bot
operator|+
name|n
operator|)
condition|)
block|{
if|if
condition|(
name|clp
operator|->
name|hash
operator|!=
name|blank_hash
operator|||
name|memcmp
argument_list|(
name|clp
operator|->
name|line
argument_list|,
name|buf
argument_list|,
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
argument_list|)
operator|!=
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|memcpy
argument_list|(
name|clp
operator|->
name|line
argument_list|,
name|buf
argument_list|,
name|win
operator|->
name|maxx
operator|*
name|__LDATASIZE
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"-- blanked out: dirty"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|clp
operator|->
name|hash
operator|=
name|blank_hash
expr_stmt|;
name|__touchline
argument_list|(
name|win
argument_list|,
name|target
argument_list|,
literal|0
argument_list|,
name|win
operator|->
name|maxx
operator|-
literal|1
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|__touchline
argument_list|(
name|win
argument_list|,
name|target
argument_list|,
literal|0
argument_list|,
name|win
operator|->
name|maxx
operator|-
literal|1
argument_list|,
literal|0
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|" -- blank line already: dirty"
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
block|}
else|else
block|{
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|" -- dirty"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|__touchline
argument_list|(
name|win
argument_list|,
name|target
argument_list|,
literal|0
argument_list|,
name|win
operator|->
name|maxx
operator|-
literal|1
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|target
operator|==
name|cur_period
condition|)
block|{
name|i
operator|=
name|target
operator|+
literal|1
expr_stmt|;
name|tmp1
operator|=
name|curscr
operator|->
name|lines
index|[
name|i
index|]
expr_stmt|;
name|cur_period
operator|=
name|i
expr_stmt|;
block|}
else|else
block|{
name|tmp1
operator|=
name|tmp2
expr_stmt|;
name|i
operator|=
name|target
expr_stmt|;
block|}
block|}
ifdef|#
directive|ifdef
name|DEBUG
name|__CTRACE
argument_list|(
literal|"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|curscr
operator|->
name|maxy
condition|;
name|i
operator|++
control|)
block|{
name|__CTRACE
argument_list|(
literal|"C: %d:"
argument_list|,
name|i
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|curscr
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%c"
argument_list|,
name|curscr
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|ch
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"W: %d:"
argument_list|,
name|i
argument_list|)
expr_stmt|;
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|win
operator|->
name|maxx
condition|;
name|j
operator|++
control|)
name|__CTRACE
argument_list|(
literal|"%c"
argument_list|,
name|win
operator|->
name|lines
index|[
name|i
index|]
operator|->
name|line
index|[
name|j
index|]
operator|.
name|ch
argument_list|)
expr_stmt|;
name|__CTRACE
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
if|if
condition|(
name|n
operator|!=
literal|0
condition|)
block|{
name|WINDOW
modifier|*
name|wp
decl_stmt|;
name|scrolln
argument_list|(
name|win
argument_list|,
name|starts
argument_list|,
name|startw
argument_list|,
name|curs
argument_list|,
name|bot
argument_list|,
name|top
argument_list|)
expr_stmt|;
comment|/* 		 * Need to repoint any subwindow lines to the rotated 		 * line structured. 		 */
for|for
control|(
name|wp
operator|=
name|win
operator|->
name|nextp
init|;
name|wp
operator|!=
name|win
condition|;
name|wp
operator|=
name|wp
operator|->
name|nextp
control|)
name|__set_subwin
argument_list|(
name|win
argument_list|,
name|wp
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * scrolln --  *	Scroll n lines, where n is starts - startw.  *  * XXX  * The initial tests that set __noqch don't let us reach here unless we  * have either CS + HO + SF/sf/SR/sr, or AL + DL.  SF/sf/SR/sr scrolling  * can only scroll the entire screen, not just a part of it, which means  * that the quickch() routine is going to be sadly disappointed in us.  * I've left the code here, as it would be useful to be able to scroll  * part of the screen and repaint the rest, instead painting the entire  * screen, on a terminal that didn't have CS or AL/DL.  */
end_comment

begin_function
specifier|static
name|void
name|scrolln
parameter_list|(
name|win
parameter_list|,
name|starts
parameter_list|,
name|startw
parameter_list|,
name|curs
parameter_list|,
name|bot
parameter_list|,
name|top
parameter_list|)
name|WINDOW
modifier|*
name|win
decl_stmt|;
name|int
name|starts
decl_stmt|,
name|startw
decl_stmt|,
name|curs
decl_stmt|,
name|bot
decl_stmt|,
name|top
decl_stmt|;
block|{
name|int
name|i
decl_stmt|,
name|oy
decl_stmt|,
name|ox
decl_stmt|,
name|n
decl_stmt|;
name|oy
operator|=
name|curscr
operator|->
name|cury
expr_stmt|;
name|ox
operator|=
name|curscr
operator|->
name|curx
expr_stmt|;
name|n
operator|=
name|starts
operator|-
name|startw
expr_stmt|;
if|if
condition|(
name|n
operator|>
literal|0
condition|)
block|{
comment|/* If scrolling the entire screen, SF/sf is all we need. */
if|if
condition|(
name|top
operator|==
literal|0
operator|&&
name|bot
operator|==
name|win
operator|->
name|maxy
operator|&&
operator|(
name|SF
operator|!=
name|NULL
operator|||
name|sf
operator|!=
name|NULL
operator|)
condition|)
block|{
name|__mvcur
argument_list|(
name|oy
argument_list|,
name|ox
argument_list|,
name|win
operator|->
name|maxy
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|SF
operator|!=
name|NULL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|SF
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|sf
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|win
operator|->
name|maxy
argument_list|,
literal|0
argument_list|,
name|oy
argument_list|,
name|ox
argument_list|,
literal|1
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 		 * If CS, HO and SF/sf are set, use the scrolling region.  We 		 * need HO because the cursor position after CS is undefined, 		 * so we need the ability to move to a fixed place without any 		 * knowledge of the current location.  Note, we call __mvcur() 		 * anyway, to update its idea of where the cursor is. 		 * 		 * When the scrolling region has been set, the cursor has to be 		 * at the last line of that region to make the scroll happen. 		 */
if|if
condition|(
name|CS
operator|!=
name|NULL
operator|&&
name|HO
operator|!=
name|NULL
operator|&&
operator|(
name|SF
operator|!=
name|NULL
operator|||
name|sf
operator|!=
name|NULL
operator|)
condition|)
block|{
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|CS
argument_list|,
name|top
argument_list|,
name|bot
operator|+
literal|1
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|oy
argument_list|,
name|ox
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|tputs
argument_list|(
name|HO
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|,
name|bot
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|SF
operator|!=
name|NULL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|SF
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|sf
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|CS
argument_list|,
literal|0
argument_list|,
name|win
operator|->
name|maxy
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|bot
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|tputs
argument_list|(
name|HO
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|,
name|oy
argument_list|,
name|ox
argument_list|,
literal|1
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 		 * Otherwise, we depend on having AL/al and DL/dl. 		 * 		 * Scroll up the block. 		 */
name|__mvcur
argument_list|(
name|oy
argument_list|,
name|ox
argument_list|,
name|top
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|DL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|DL
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|dl
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
comment|/* Push down the bottom region. */
name|__mvcur
argument_list|(
name|top
argument_list|,
literal|0
argument_list|,
name|bot
operator|-
name|n
operator|+
literal|1
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|AL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|AL
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|al
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|bot
operator|-
name|n
operator|+
literal|1
argument_list|,
literal|0
argument_list|,
name|oy
argument_list|,
name|ox
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/* If scrolling the entire screen, SR/sr is all we need. */
if|if
condition|(
name|top
operator|==
literal|0
operator|&&
name|bot
operator|==
name|win
operator|->
name|maxy
operator|&&
operator|(
name|SR
operator|!=
name|NULL
operator|||
name|sr
operator|!=
name|NULL
operator|)
condition|)
block|{
name|__mvcur
argument_list|(
name|oy
argument_list|,
name|ox
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|n
operator|=
operator|-
name|n
expr_stmt|;
if|if
condition|(
name|SR
operator|!=
name|NULL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|SR
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|sr
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|,
name|oy
argument_list|,
name|ox
argument_list|,
literal|1
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 		 * If CS, HO and SR/sr are set, use the scrolling region.  See 		 * the above comment for details. 		 */
if|if
condition|(
name|CS
operator|!=
name|NULL
operator|&&
name|HO
operator|!=
name|NULL
operator|&&
operator|(
name|SR
operator|!=
name|NULL
operator|||
name|sr
operator|!=
name|NULL
operator|)
condition|)
block|{
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|CS
argument_list|,
name|top
argument_list|,
name|bot
operator|+
literal|1
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|oy
argument_list|,
name|ox
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|tputs
argument_list|(
name|HO
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|,
name|top
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|n
operator|=
operator|-
name|n
expr_stmt|;
if|if
condition|(
name|SR
operator|!=
name|NULL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|SR
argument_list|,
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|sr
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|CS
argument_list|,
literal|0
argument_list|,
name|win
operator|->
name|maxy
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|top
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|tputs
argument_list|(
name|HO
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|,
name|oy
argument_list|,
name|ox
argument_list|,
literal|1
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 		 * Otherwise, we depend on having AL/al and DL/dl. 		 * 		 * Preserve the bottom lines. 		 */
name|__mvcur
argument_list|(
name|oy
argument_list|,
name|ox
argument_list|,
name|bot
operator|+
name|n
operator|+
literal|1
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|/* n< 0 */
if|if
condition|(
name|DL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|DL
argument_list|,
operator|-
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
name|n
init|;
name|i
operator|<
literal|0
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|dl
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|bot
operator|+
name|n
operator|+
literal|1
argument_list|,
literal|0
argument_list|,
name|top
argument_list|,
literal|0
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|/* Scroll the block down. */
if|if
condition|(
name|AL
condition|)
name|tputs
argument_list|(
name|__tscroll
argument_list|(
name|AL
argument_list|,
operator|-
name|n
argument_list|,
literal|0
argument_list|)
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
else|else
for|for
control|(
name|i
operator|=
name|n
init|;
name|i
operator|<
literal|0
condition|;
name|i
operator|++
control|)
name|tputs
argument_list|(
name|al
argument_list|,
literal|0
argument_list|,
name|__cputchar
argument_list|)
expr_stmt|;
name|__mvcur
argument_list|(
name|top
argument_list|,
literal|0
argument_list|,
name|oy
argument_list|,
name|ox
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
end_function

end_unit

