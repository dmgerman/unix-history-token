...
operator|.
operator|\
literal|" Copyright (c) 1980 The Regents of the University of California. .\" All rights reserved. .\" .\" %sccs.include.redist.roff% .\" .\"	@(#)twinkle2.c	6.3 (Berkeley) %G% .\" extern int	_putchar();  main() { 	reg char	*sp;  	srand(getpid());		/* initialize random sequence */  	if (isatty(0)) { 	       gettmode(); 	       if ((sp = getenv("
name|TERM
