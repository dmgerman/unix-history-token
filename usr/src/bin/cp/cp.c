begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1988 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * David Hitz of Auspex Systems Inc.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1988 The Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)cp.c	5.17 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_comment
comment|/*  * cp copies source files to target files.  *   * The global PATH_T structures "to" and "from" always contain paths to the  * current source and target files, respectively.  Since cp does not change  * directories, these paths can be either absolute or dot-realative.  *   * The basic algorithm is to initialize "to" and "from", and then call the  * recursive copy() function to do the actual work.  If "from" is a file,  * copy copies the data.  If "from" is a directory, copy creates the  * corresponding "to" directory, and calls itself recursively on all of  * the entries in the "from" directory.  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/dir.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_define
define|#
directive|define
name|type
parameter_list|(
name|st
parameter_list|)
value|((st).st_mode& S_IFMT)
end_define

begin_typedef
typedef|typedef
struct|struct
block|{
name|char
name|p_path
index|[
name|MAXPATHLEN
operator|+
literal|1
index|]
decl_stmt|;
comment|/* pointer to the start of a path. */
name|char
modifier|*
name|p_end
decl_stmt|;
comment|/* pointer to NULL at end of path. */
block|}
name|PATH_T
typedef|;
end_typedef

begin_decl_stmt
name|PATH_T
name|from
init|=
block|{
literal|""
block|,
name|from
operator|.
name|p_path
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|PATH_T
name|to
init|=
block|{
literal|""
block|,
name|to
operator|.
name|p_path
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|int
name|errno
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|uid_t
name|myuid
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|exit_val
decl_stmt|,
name|myumask
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|interactive_flag
decl_stmt|,
name|preserve_flag
decl_stmt|,
name|recursive_flag
decl_stmt|;
end_decl_stmt

begin_function_decl
name|int
function_decl|(
modifier|*
name|statfcn
function_decl|)
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|char
modifier|*
name|buf
decl_stmt|,
modifier|*
name|pname
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|path_append
argument_list|()
decl_stmt|,
modifier|*
name|path_basename
argument_list|()
decl_stmt|;
end_decl_stmt

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
block|{
specifier|extern
name|int
name|optind
decl_stmt|;
name|struct
name|stat
name|to_stat
decl_stmt|;
specifier|register
name|int
name|c
decl_stmt|,
name|r
decl_stmt|;
name|int
name|force_flag
decl_stmt|,
name|symfollow
decl_stmt|,
name|lstat
argument_list|()
decl_stmt|,
name|stat
argument_list|()
decl_stmt|;
name|char
modifier|*
name|old_to
decl_stmt|,
modifier|*
name|p
decl_stmt|,
modifier|*
name|malloc
argument_list|()
decl_stmt|;
comment|/* 	 * cp is used by mv(1) -- except for usage statements, print 	 * the "called as" program name. 	 */
name|pname
operator|=
operator|(
name|p
operator|=
name|rindex
argument_list|(
operator|*
name|argv
argument_list|,
literal|'/'
argument_list|)
operator|)
condition|?
operator|++
name|p
else|:
operator|*
name|argv
expr_stmt|;
name|force_flag
operator|=
name|symfollow
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"Rfhipr"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
block|{
switch|switch
condition|(
operator|(
name|char
operator|)
name|c
condition|)
block|{
case|case
literal|'f'
case|:
name|force_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'h'
case|:
name|symfollow
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'i'
case|:
name|interactive_flag
operator|=
name|isatty
argument_list|(
name|fileno
argument_list|(
name|stdin
argument_list|)
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'p'
case|:
name|preserve_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'r'
case|:
case|case
literal|'R'
case|:
name|recursive_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'?'
case|:
default|default:
name|usage
argument_list|()
expr_stmt|;
break|break;
block|}
block|}
name|argc
operator|-=
name|optind
expr_stmt|;
name|argv
operator|+=
name|optind
expr_stmt|;
if|if
condition|(
name|argc
operator|<
literal|2
condition|)
name|usage
argument_list|()
expr_stmt|;
if|if
condition|(
name|force_flag
condition|)
name|interactive_flag
operator|=
literal|0
expr_stmt|;
name|buf
operator|=
operator|(
name|char
operator|*
operator|)
name|malloc
argument_list|(
name|MAXBSIZE
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|buf
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: out of space.\n"
argument_list|,
name|pname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|myuid
operator|=
name|getuid
argument_list|()
expr_stmt|;
comment|/* copy the umask for explicit mode setting */
name|myumask
operator|=
name|umask
argument_list|(
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|umask
argument_list|(
name|myumask
argument_list|)
expr_stmt|;
comment|/* consume last argument first. */
if|if
condition|(
operator|!
name|path_set
argument_list|(
operator|&
name|to
argument_list|,
name|argv
index|[
operator|--
name|argc
index|]
argument_list|)
condition|)
name|exit
argument_list|(
name|exit_val
argument_list|)
expr_stmt|;
name|statfcn
operator|=
name|symfollow
operator|||
operator|!
name|recursive_flag
condition|?
name|stat
else|:
name|lstat
expr_stmt|;
comment|/* 	 * Cp has two distinct cases: 	 * 	 * Case (1)	  $ cp [-rip] source target 	 * 	 * Case (2)	  $ cp [-rip] source1 ... directory 	 * 	 * In both cases, source can be either a file or a directory. 	 * 	 * In (1), the target becomes a copy of the source. That is, if the 	 * source is a file, the target will be a file, and likewise for 	 * directories. 	 * 	 * In (2), the real target is not directory, but "directory/source". 	 */
name|r
operator|=
name|stat
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
operator|&
name|to_stat
argument_list|)
expr_stmt|;
if|if
condition|(
name|r
operator|==
operator|-
literal|1
operator|&&
name|errno
operator|!=
name|ENOENT
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|r
operator|==
operator|-
literal|1
operator|||
name|type
argument_list|(
name|to_stat
argument_list|)
operator|!=
name|S_IFDIR
condition|)
block|{
comment|/* 		 * Case (1).  Target is not a directory. 		 */
if|if
condition|(
name|argc
operator|>
literal|1
condition|)
block|{
name|usage
argument_list|()
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|path_set
argument_list|(
operator|&
name|from
argument_list|,
operator|*
name|argv
argument_list|)
condition|)
name|exit
argument_list|(
name|exit_val
argument_list|)
expr_stmt|;
name|copy
argument_list|()
expr_stmt|;
block|}
else|else
block|{
comment|/* 		 * Case (2).  Target is a directory. 		 */
for|for
control|(
init|;
condition|;
operator|++
name|argv
control|)
block|{
if|if
condition|(
operator|!
name|path_set
argument_list|(
operator|&
name|from
argument_list|,
operator|*
name|argv
argument_list|)
condition|)
continue|continue;
name|old_to
operator|=
name|path_append
argument_list|(
operator|&
name|to
argument_list|,
name|path_basename
argument_list|(
operator|&
name|from
argument_list|)
argument_list|,
operator|-
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|old_to
condition|)
continue|continue;
name|copy
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
operator|--
name|argc
condition|)
break|break;
name|path_restore
argument_list|(
operator|&
name|to
argument_list|,
name|old_to
argument_list|)
expr_stmt|;
block|}
block|}
name|exit
argument_list|(
name|exit_val
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* copy file or directory at "from" to "to". */
end_comment

begin_macro
name|copy
argument_list|()
end_macro

begin_block
block|{
name|struct
name|stat
name|from_stat
decl_stmt|,
name|to_stat
decl_stmt|;
name|int
name|dne
decl_stmt|,
name|statval
decl_stmt|;
name|statval
operator|=
name|statfcn
argument_list|(
name|from
operator|.
name|p_path
argument_list|,
operator|&
name|from_stat
argument_list|)
expr_stmt|;
if|if
condition|(
name|statval
operator|==
operator|-
literal|1
condition|)
block|{
name|error
argument_list|(
name|from
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* not an error, but need to remember it happened */
if|if
condition|(
name|stat
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
operator|&
name|to_stat
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|dne
operator|=
literal|1
expr_stmt|;
else|else
block|{
if|if
condition|(
name|to_stat
operator|.
name|st_dev
operator|==
name|from_stat
operator|.
name|st_dev
operator|&&
name|to_stat
operator|.
name|st_ino
operator|==
name|from_stat
operator|.
name|st_ino
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s and %s are identical (not copied).\n"
argument_list|,
name|pname
argument_list|,
name|to
operator|.
name|p_path
argument_list|,
name|from
operator|.
name|p_path
argument_list|)
expr_stmt|;
name|exit_val
operator|=
literal|1
expr_stmt|;
return|return;
block|}
name|dne
operator|=
literal|0
expr_stmt|;
block|}
switch|switch
condition|(
name|type
argument_list|(
name|from_stat
argument_list|)
condition|)
block|{
case|case
name|S_IFLNK
case|:
name|copy_link
argument_list|(
operator|!
name|dne
argument_list|)
expr_stmt|;
return|return;
case|case
name|S_IFDIR
case|:
if|if
condition|(
operator|!
name|recursive_flag
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s is a directory (not copied).\n"
argument_list|,
name|pname
argument_list|,
name|from
operator|.
name|p_path
argument_list|)
expr_stmt|;
name|exit_val
operator|=
literal|1
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|dne
condition|)
block|{
comment|/* 			 * If the directory doesn't exist, create the new 			 * one with the from file mode plus owner RWX bits, 			 * modified by the umask.  Trade-off between being 			 * able to write the directory (if from directory is 			 * 555) and not causing a permissions race.  If the 			 * umask blocks owner writes cp fails. 			 */
if|if
condition|(
name|mkdir
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|from_stat
operator|.
name|st_mode
operator||
name|S_IRWXU
argument_list|)
operator|<
literal|0
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
elseif|else
if|if
condition|(
name|type
argument_list|(
name|to_stat
argument_list|)
operator|!=
name|S_IFDIR
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s: not a directory.\n"
argument_list|,
name|pname
argument_list|,
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
name|copy_dir
argument_list|()
expr_stmt|;
comment|/* 		 * If not -p and directory didn't exist, set it to be the 		 * same as the from directory, umodified by the umask; 		 * arguably wrong, but it's been that way forever. 		 */
if|if
condition|(
name|preserve_flag
condition|)
name|setfile
argument_list|(
operator|&
name|from_stat
argument_list|,
literal|0
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|dne
condition|)
operator|(
name|void
operator|)
name|chmod
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|from_stat
operator|.
name|st_mode
argument_list|)
expr_stmt|;
break|break;
case|case
name|S_IFCHR
case|:
case|case
name|S_IFBLK
case|:
comment|/* 		 * if recursive flag on, try and create the special device 		 * otherwise copy the contents. 		 */
if|if
condition|(
name|recursive_flag
condition|)
block|{
name|copy_special
argument_list|(
operator|&
name|from_stat
argument_list|,
operator|!
name|dne
argument_list|)
expr_stmt|;
if|if
condition|(
name|preserve_flag
condition|)
name|setfile
argument_list|(
operator|&
name|from_stat
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* FALLTHROUGH */
default|default:
name|copy_file
argument_list|(
operator|&
name|from_stat
argument_list|,
name|dne
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_macro
name|copy_file
argument_list|(
argument|fs
argument_list|,
argument|dne
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|stat
modifier|*
name|fs
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|dne
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|from_fd
decl_stmt|,
name|to_fd
decl_stmt|,
name|rcount
decl_stmt|,
name|wcount
decl_stmt|;
name|struct
name|stat
name|to_stat
decl_stmt|;
if|if
condition|(
operator|(
name|from_fd
operator|=
name|open
argument_list|(
name|from
operator|.
name|p_path
argument_list|,
name|O_RDONLY
argument_list|,
literal|0
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
block|{
name|error
argument_list|(
name|from
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 	 * If the file exists and we're interactive, verify with the user. 	 * If the file DNE, set the mode to be the from file, minus setuid 	 * bits, modified by the umask; arguably wrong, but it makes copying 	 * executables work right and it's been that way forever.  (The 	 * other choice is 666 or'ed with the execute bits on the from file 	 * modified by the umask.) 	 */
if|if
condition|(
operator|!
name|dne
condition|)
block|{
if|if
condition|(
name|interactive_flag
condition|)
block|{
name|int
name|checkch
decl_stmt|,
name|ch
decl_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"overwrite %s? "
argument_list|,
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
name|checkch
operator|=
name|ch
operator|=
name|getchar
argument_list|()
expr_stmt|;
while|while
condition|(
name|ch
operator|!=
literal|'\n'
operator|&&
name|ch
operator|!=
name|EOF
condition|)
name|ch
operator|=
name|getchar
argument_list|()
expr_stmt|;
if|if
condition|(
name|checkch
operator|!=
literal|'y'
condition|)
block|{
operator|(
name|void
operator|)
name|close
argument_list|(
name|from_fd
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
name|to_fd
operator|=
name|open
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|O_WRONLY
operator||
name|O_TRUNC
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
name|to_fd
operator|=
name|open
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|O_WRONLY
operator||
name|O_CREAT
operator||
name|O_TRUNC
argument_list|,
name|fs
operator|->
name|st_mode
operator|&
operator|~
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|to_fd
operator|==
operator|-
literal|1
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
name|from_fd
argument_list|)
expr_stmt|;
return|return;
block|}
while|while
condition|(
operator|(
name|rcount
operator|=
name|read
argument_list|(
name|from_fd
argument_list|,
name|buf
argument_list|,
name|MAXBSIZE
argument_list|)
operator|)
operator|>
literal|0
condition|)
block|{
name|wcount
operator|=
name|write
argument_list|(
name|to_fd
argument_list|,
name|buf
argument_list|,
name|rcount
argument_list|)
expr_stmt|;
if|if
condition|(
name|rcount
operator|!=
name|wcount
operator|||
name|wcount
operator|==
operator|-
literal|1
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
if|if
condition|(
name|rcount
operator|<
literal|0
condition|)
name|error
argument_list|(
name|from
operator|.
name|p_path
argument_list|)
expr_stmt|;
if|if
condition|(
name|preserve_flag
condition|)
name|setfile
argument_list|(
name|fs
argument_list|,
name|to_fd
argument_list|)
expr_stmt|;
comment|/* 	 * If the source was setuid, set the bits on the copy if the copy 	 * was created and is owned by the same uid.  If the source was 	 * setgid, set the bits on the copy if the copy was created and is 	 * owned by the same gid and the user is a member of that group. 	 * If both setuid and setgid, lose both bits unless all the above 	 * conditions are met. 	 */
elseif|else
if|if
condition|(
name|fs
operator|->
name|st_mode
operator|&
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
condition|)
block|{
if|if
condition|(
name|fs
operator|->
name|st_mode
operator|&
name|S_ISUID
operator|&&
name|myuid
operator|!=
name|fs
operator|->
name|st_uid
condition|)
name|fs
operator|->
name|st_mode
operator|&=
operator|~
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|st_mode
operator|&
name|S_ISGID
condition|)
block|{
if|if
condition|(
name|fstat
argument_list|(
name|to_fd
argument_list|,
operator|&
name|to_stat
argument_list|)
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
name|fs
operator|->
name|st_mode
operator|&=
operator|~
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|fs
operator|->
name|st_gid
operator|!=
name|to_stat
operator|.
name|st_gid
operator|||
operator|!
name|ismember
argument_list|(
name|fs
operator|->
name|st_gid
argument_list|)
condition|)
name|fs
operator|->
name|st_mode
operator|&=
operator|~
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
expr_stmt|;
block|}
if|if
condition|(
name|fs
operator|->
name|st_mode
operator|&
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
operator|&&
name|fchmod
argument_list|(
name|to_fd
argument_list|,
name|fs
operator|->
name|st_mode
operator|&
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator||
name|S_IRWXU
operator||
name|S_IRWXG
operator||
name|S_IRWXO
operator|)
operator|&
operator|~
name|myumask
argument_list|)
condition|)
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|close
argument_list|(
name|from_fd
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|close
argument_list|(
name|to_fd
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|copy_dir
argument_list|()
end_macro

begin_block
block|{
name|struct
name|stat
name|from_stat
decl_stmt|;
name|struct
name|direct
modifier|*
name|dp
decl_stmt|,
modifier|*
modifier|*
name|dir_list
decl_stmt|;
specifier|register
name|int
name|dir_cnt
decl_stmt|,
name|i
decl_stmt|;
name|char
modifier|*
name|old_from
decl_stmt|,
modifier|*
name|old_to
decl_stmt|;
name|dir_cnt
operator|=
name|scandir
argument_list|(
name|from
operator|.
name|p_path
argument_list|,
operator|&
name|dir_list
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
if|if
condition|(
name|dir_cnt
operator|==
operator|-
literal|1
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: can't read directory %s.\n"
argument_list|,
name|pname
argument_list|,
name|from
operator|.
name|p_path
argument_list|)
expr_stmt|;
name|exit_val
operator|=
literal|1
expr_stmt|;
block|}
comment|/* 	 * Instead of handling directory entries in the order they appear 	 * on disk, do non-directory files before directory files. 	 * There are two reasons to do directories last.  The first is 	 * efficiency.  Files tend to be in the same cylinder group as 	 * their parent, whereas directories tend not to be.  Copying files 	 * all at once reduces seeking.  Second, deeply nested tree's 	 * could use up all the file descriptors if we didn't close one 	 * directory before recursivly starting on the next. 	 */
comment|/* copy files */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dir_cnt
condition|;
operator|++
name|i
control|)
block|{
name|dp
operator|=
name|dir_list
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|dp
operator|->
name|d_namlen
operator|<=
literal|2
operator|&&
name|dp
operator|->
name|d_name
index|[
literal|0
index|]
operator|==
literal|'.'
operator|&&
operator|(
name|dp
operator|->
name|d_name
index|[
literal|1
index|]
operator|==
name|NULL
operator|||
name|dp
operator|->
name|d_name
index|[
literal|1
index|]
operator|==
literal|'.'
operator|)
condition|)
goto|goto
name|done
goto|;
name|old_from
operator|=
name|path_append
argument_list|(
operator|&
name|from
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
operator|(
name|int
operator|)
name|dp
operator|->
name|d_namlen
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|old_from
condition|)
goto|goto
name|done
goto|;
if|if
condition|(
name|statfcn
argument_list|(
name|from
operator|.
name|p_path
argument_list|,
operator|&
name|from_stat
argument_list|)
operator|<
literal|0
condition|)
block|{
name|error
argument_list|(
name|dp
operator|->
name|d_name
argument_list|)
expr_stmt|;
name|path_restore
argument_list|(
operator|&
name|from
argument_list|,
name|old_from
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
if|if
condition|(
name|type
argument_list|(
name|from_stat
argument_list|)
operator|==
name|S_IFDIR
condition|)
block|{
name|path_restore
argument_list|(
operator|&
name|from
argument_list|,
name|old_from
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|old_to
operator|=
name|path_append
argument_list|(
operator|&
name|to
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
operator|(
name|int
operator|)
name|dp
operator|->
name|d_namlen
argument_list|)
expr_stmt|;
if|if
condition|(
name|old_to
condition|)
block|{
name|copy
argument_list|()
expr_stmt|;
name|path_restore
argument_list|(
operator|&
name|to
argument_list|,
name|old_to
argument_list|)
expr_stmt|;
block|}
name|path_restore
argument_list|(
operator|&
name|from
argument_list|,
name|old_from
argument_list|)
expr_stmt|;
name|done
label|:
name|dir_list
index|[
name|i
index|]
operator|=
name|NULL
expr_stmt|;
operator|(
name|void
operator|)
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
name|dp
argument_list|)
expr_stmt|;
block|}
comment|/* copy directories */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dir_cnt
condition|;
operator|++
name|i
control|)
block|{
name|dp
operator|=
name|dir_list
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
operator|!
name|dp
condition|)
continue|continue;
name|old_from
operator|=
name|path_append
argument_list|(
operator|&
name|from
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
operator|(
name|int
operator|)
name|dp
operator|->
name|d_namlen
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|old_from
condition|)
block|{
operator|(
name|void
operator|)
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
name|dp
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|old_to
operator|=
name|path_append
argument_list|(
operator|&
name|to
argument_list|,
name|dp
operator|->
name|d_name
argument_list|,
operator|(
name|int
operator|)
name|dp
operator|->
name|d_namlen
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|old_to
condition|)
block|{
operator|(
name|void
operator|)
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
name|dp
argument_list|)
expr_stmt|;
name|path_restore
argument_list|(
operator|&
name|from
argument_list|,
name|old_from
argument_list|)
expr_stmt|;
continue|continue;
block|}
name|copy
argument_list|()
expr_stmt|;
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
name|dp
argument_list|)
expr_stmt|;
name|path_restore
argument_list|(
operator|&
name|from
argument_list|,
name|old_from
argument_list|)
expr_stmt|;
name|path_restore
argument_list|(
operator|&
name|to
argument_list|,
name|old_to
argument_list|)
expr_stmt|;
block|}
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
name|dir_list
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|copy_link
argument_list|(
argument|exists
argument_list|)
end_macro

begin_decl_stmt
name|int
name|exists
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|len
decl_stmt|;
name|char
name|link
index|[
name|MAXPATHLEN
index|]
decl_stmt|;
if|if
condition|(
operator|(
name|len
operator|=
name|readlink
argument_list|(
name|from
operator|.
name|p_path
argument_list|,
name|link
argument_list|,
sizeof|sizeof
argument_list|(
name|link
argument_list|)
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
block|{
name|error
argument_list|(
name|from
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
name|link
index|[
name|len
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|exists
operator|&&
name|unlink
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|symlink
argument_list|(
name|link
argument_list|,
name|to
operator|.
name|p_path
argument_list|)
condition|)
block|{
name|error
argument_list|(
name|link
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
end_block

begin_macro
name|copy_special
argument_list|(
argument|from_stat
argument_list|,
argument|exists
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|stat
modifier|*
name|from_stat
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|exists
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|exists
operator|&&
name|unlink
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|mknod
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|from_stat
operator|->
name|st_mode
argument_list|,
name|from_stat
operator|->
name|st_rdev
argument_list|)
condition|)
block|{
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
end_block

begin_expr_stmt
name|setfile
argument_list|(
name|fs
argument_list|,
name|fd
argument_list|)
specifier|register
expr|struct
name|stat
operator|*
name|fs
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|int
name|fd
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|static
name|struct
name|timeval
name|tv
index|[
literal|2
index|]
decl_stmt|;
name|tv
index|[
literal|0
index|]
operator|.
name|tv_sec
operator|=
name|fs
operator|->
name|st_atime
expr_stmt|;
name|tv
index|[
literal|1
index|]
operator|.
name|tv_sec
operator|=
name|fs
operator|->
name|st_mtime
expr_stmt|;
if|if
condition|(
name|utimes
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|tv
argument_list|)
condition|)
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
comment|/* 	 * Changing the ownership probably won't succeed, unless we're root 	 * or POSIX_CHOWN_RESTRICTED is not set.  Set uid before setting the 	 * mode; current BSD behavior is to remove all setuid bits on chown. 	 * If setuid, lose the bits if chown fails. 	 * If setgid, lose the bits if chgrp fails. 	 * If both, lose the bits if either fails. 	 */
if|if
condition|(
operator|(
name|fd
condition|?
name|fchown
argument_list|(
name|fd
argument_list|,
name|fs
operator|->
name|st_uid
argument_list|,
operator|-
literal|1
argument_list|)
else|:
name|chown
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|fs
operator|->
name|st_uid
argument_list|,
operator|-
literal|1
argument_list|)
operator|)
condition|)
block|{
if|if
condition|(
name|errno
operator|!=
name|EPERM
condition|)
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|st_mode
operator|&
name|S_ISUID
condition|)
name|fs
operator|->
name|st_mode
operator|&=
operator|~
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|fd
condition|?
name|fchown
argument_list|(
name|fd
argument_list|,
operator|-
literal|1
argument_list|,
name|fs
operator|->
name|st_gid
argument_list|)
else|:
name|chown
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
operator|-
literal|1
argument_list|,
name|fs
operator|->
name|st_gid
argument_list|)
operator|)
condition|)
block|{
if|if
condition|(
name|errno
operator|!=
name|EPERM
condition|)
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|st_mode
operator|&
name|S_ISGID
condition|)
name|fs
operator|->
name|st_mode
operator|&=
operator|~
operator|(
name|S_ISUID
operator||
name|S_ISGID
operator|)
expr_stmt|;
block|}
name|fs
operator|->
name|st_mode
operator|&=
name|S_ISUID
operator||
name|S_ISGID
operator||
name|S_IRWXU
operator||
name|S_IRWXG
operator||
name|S_IRWXO
expr_stmt|;
if|if
condition|(
name|fd
condition|?
name|fchmod
argument_list|(
name|fd
argument_list|,
name|fs
operator|->
name|st_mode
argument_list|)
else|:
name|chmod
argument_list|(
name|to
operator|.
name|p_path
argument_list|,
name|fs
operator|->
name|st_mode
argument_list|)
condition|)
name|error
argument_list|(
name|to
operator|.
name|p_path
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|ismember
argument_list|(
argument|gid
argument_list|)
end_macro

begin_decl_stmt
name|gid_t
name|gid
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|cnt
decl_stmt|;
specifier|static
name|int
name|ngroups
decl_stmt|,
name|groups
index|[
name|NGROUPS
index|]
decl_stmt|;
if|if
condition|(
operator|!
name|ngroups
condition|)
block|{
name|ngroups
operator|=
name|getgroups
argument_list|(
name|NGROUPS
argument_list|,
name|groups
argument_list|)
expr_stmt|;
if|if
condition|(
name|ngroups
operator|==
operator|-
literal|1
condition|)
block|{
name|ngroups
operator|=
literal|0
expr_stmt|;
name|exit_val
operator|=
literal|1
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s\n"
argument_list|,
name|pname
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
block|}
for|for
control|(
name|cnt
operator|=
name|ngroups
init|;
name|cnt
operator|--
condition|;
control|)
if|if
condition|(
name|gid
operator|==
name|groups
index|[
name|cnt
index|]
condition|)
return|return
operator|(
literal|1
operator|)
return|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_macro
name|error
argument_list|(
argument|s
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|s
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|exit_val
operator|=
literal|1
expr_stmt|;
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s: %s\n"
argument_list|,
name|pname
argument_list|,
name|s
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/********************************************************************  * Path Manipulation Routines.  ********************************************************************/
end_comment

begin_comment
comment|/*  * These functions manipulate paths in PATH_T structures.  *   * They eliminate multiple slashes in paths when they notice them, and keep  * the path non-slash terminated.  *  * Both path_set() and path_append() return 0 if the requested name  * would be too long.  */
end_comment

begin_define
define|#
directive|define
name|STRIP_TRAILING_SLASH
parameter_list|(
name|p
parameter_list|)
value|{ \ 	while ((p)->p_end> (p)->p_path&& (p)->p_end[-1] == '/') \ 		*--(p)->p_end = 0; \ 	}
end_define

begin_comment
comment|/*  * Move specified string into path.  Convert "" to "." to handle BSD  * semantics for a null path.  Strip trailing slashes.  */
end_comment

begin_expr_stmt
name|path_set
argument_list|(
name|p
argument_list|,
name|string
argument_list|)
specifier|register
name|PATH_T
operator|*
name|p
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|char
modifier|*
name|string
decl_stmt|;
end_decl_stmt

begin_block
block|{
if|if
condition|(
name|strlen
argument_list|(
name|string
argument_list|)
operator|>
name|MAXPATHLEN
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s: name too long.\n"
argument_list|,
name|pname
argument_list|,
name|string
argument_list|)
expr_stmt|;
name|exit_val
operator|=
literal|1
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|p
operator|->
name|p_path
argument_list|,
name|string
argument_list|)
expr_stmt|;
name|p
operator|->
name|p_end
operator|=
name|p
operator|->
name|p_path
operator|+
name|strlen
argument_list|(
name|p
operator|->
name|p_path
argument_list|)
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|p_path
operator|==
name|p
operator|->
name|p_end
condition|)
block|{
operator|*
name|p
operator|->
name|p_end
operator|++
operator|=
literal|'.'
expr_stmt|;
operator|*
name|p
operator|->
name|p_end
operator|=
literal|0
expr_stmt|;
block|}
name|STRIP_TRAILING_SLASH
argument_list|(
name|p
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Append specified string to path, inserting '/' if necessary.  Return a  * pointer to the old end of path for restoration.  */
end_comment

begin_function
name|char
modifier|*
name|path_append
parameter_list|(
name|p
parameter_list|,
name|name
parameter_list|,
name|len
parameter_list|)
specifier|register
name|PATH_T
modifier|*
name|p
decl_stmt|;
name|char
modifier|*
name|name
decl_stmt|;
name|int
name|len
decl_stmt|;
block|{
name|char
modifier|*
name|old
decl_stmt|;
name|old
operator|=
name|p
operator|->
name|p_end
expr_stmt|;
if|if
condition|(
name|len
operator|==
operator|-
literal|1
condition|)
name|len
operator|=
name|strlen
argument_list|(
name|name
argument_list|)
expr_stmt|;
comment|/* 	 * The final "+ 1" accounts for the '/' between old path and name. 	 */
if|if
condition|(
operator|(
name|len
operator|+
name|p
operator|->
name|p_end
operator|-
name|p
operator|->
name|p_path
operator|+
literal|1
operator|)
operator|>
name|MAXPATHLEN
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: %s/%s: name too long.\n"
argument_list|,
name|pname
argument_list|,
name|p
operator|->
name|p_path
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|exit_val
operator|=
literal|1
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/* 	 * This code should always be executed, since paths shouldn't 	 * end in '/'. 	 */
if|if
condition|(
name|p
operator|->
name|p_end
index|[
operator|-
literal|1
index|]
operator|!=
literal|'/'
condition|)
block|{
operator|*
name|p
operator|->
name|p_end
operator|++
operator|=
literal|'/'
expr_stmt|;
operator|*
name|p
operator|->
name|p_end
operator|=
literal|0
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|strncat
argument_list|(
name|p
operator|->
name|p_end
argument_list|,
name|name
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|p
operator|->
name|p_end
operator|+=
name|len
expr_stmt|;
operator|*
name|p
operator|->
name|p_end
operator|=
literal|0
expr_stmt|;
name|STRIP_TRAILING_SLASH
argument_list|(
name|p
argument_list|)
expr_stmt|;
return|return
operator|(
name|old
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Restore path to previous value.  (As returned by path_append.)  */
end_comment

begin_macro
name|path_restore
argument_list|(
argument|p
argument_list|,
argument|old
argument_list|)
end_macro

begin_decl_stmt
name|PATH_T
modifier|*
name|p
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|old
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|p
operator|->
name|p_end
operator|=
name|old
expr_stmt|;
operator|*
name|p
operator|->
name|p_end
operator|=
literal|0
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Return basename of path.  (Like basename(1).)  */
end_comment

begin_function
name|char
modifier|*
name|path_basename
parameter_list|(
name|p
parameter_list|)
name|PATH_T
modifier|*
name|p
decl_stmt|;
block|{
name|char
modifier|*
name|basename
decl_stmt|;
name|basename
operator|=
name|rindex
argument_list|(
name|p
operator|->
name|p_path
argument_list|,
literal|'/'
argument_list|)
expr_stmt|;
return|return
operator|(
name|basename
condition|?
operator|++
name|basename
else|:
name|p
operator|->
name|p_path
operator|)
return|;
block|}
end_function

begin_macro
name|usage
argument_list|()
end_macro

begin_block
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: cp [-fhipr] src target;\n   or: cp [-fhipr] src1 ... srcN directory\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

end_unit

