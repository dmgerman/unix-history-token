begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1991 The Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Keith Muller of the University of California, San Diego.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1991 The Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)dd.c	5.4 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/mtio.h>
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|"dd.h"
end_include

begin_include
include|#
directive|include
file|"extern.h"
end_include

begin_decl_stmt
specifier|static
name|void
name|dd_close
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|dd_in
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|void
name|setup
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|io_desc
name|in
decl_stmt|,
name|out
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* input/output state */
end_comment

begin_function_decl
name|void
function_decl|(
modifier|*
name|cfunc
function_decl|)
parameter_list|()
function_decl|;
end_function_decl

begin_comment
comment|/* conversion function */
end_comment

begin_decl_stmt
name|u_long
name|cpy_cnt
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* # of blocks to copy */
end_comment

begin_decl_stmt
name|u_int
name|ddflags
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* conversion options */
end_comment

begin_decl_stmt
name|u_int
name|cbsz
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* conversion block size */
end_comment

begin_decl_stmt
name|u_int
name|files_cnt
init|=
literal|1
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* # of files to copy */
end_comment

begin_decl_stmt
name|u_char
modifier|*
name|ctab
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* conversion table */
end_comment

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
block|{
name|jcl
argument_list|(
name|argv
argument_list|)
expr_stmt|;
name|setup
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGINFO
argument_list|,
name|summary
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|terminate
argument_list|)
expr_stmt|;
while|while
condition|(
name|files_cnt
operator|--
condition|)
name|dd_in
argument_list|()
expr_stmt|;
name|dd_close
argument_list|()
expr_stmt|;
name|summary
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|setup
parameter_list|()
block|{
specifier|register
name|u_int
name|cnt
decl_stmt|;
name|struct
name|stat
name|sb
decl_stmt|;
name|struct
name|mtget
name|mt
decl_stmt|;
if|if
condition|(
name|in
operator|.
name|name
operator|==
name|NULL
condition|)
block|{
name|in
operator|.
name|name
operator|=
literal|"stdin"
expr_stmt|;
name|in
operator|.
name|fd
operator|=
name|STDIN_FILENO
expr_stmt|;
block|}
else|else
block|{
name|in
operator|.
name|fd
operator|=
name|open
argument_list|(
name|in
operator|.
name|name
argument_list|,
name|O_RDONLY
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|in
operator|.
name|fd
operator|<
literal|0
condition|)
name|err
argument_list|(
literal|"%s: %s"
argument_list|,
name|in
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|fstat
argument_list|(
name|in
operator|.
name|fd
argument_list|,
operator|&
name|sb
argument_list|)
condition|)
name|err
argument_list|(
literal|"%s: %s"
argument_list|,
name|in
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|S_ISCHR
argument_list|(
name|sb
operator|.
name|st_mode
argument_list|)
condition|)
name|in
operator|.
name|flags
operator||=
name|ioctl
argument_list|(
name|in
operator|.
name|fd
argument_list|,
name|MTIOCGET
argument_list|,
operator|&
name|mt
argument_list|)
condition|?
name|ISCHR
else|:
name|ISTAPE
expr_stmt|;
elseif|else
if|if
condition|(
name|lseek
argument_list|(
name|in
operator|.
name|fd
argument_list|,
literal|0L
argument_list|,
name|SEEK_CUR
argument_list|)
operator|==
operator|-
literal|1
operator|&&
name|errno
operator|==
name|ESPIPE
condition|)
name|in
operator|.
name|flags
operator||=
name|ISPIPE
expr_stmt|;
comment|/* XXX fixed in 4.4BSD */
if|if
condition|(
name|files_cnt
operator|>
literal|1
operator|&&
operator|!
operator|(
name|in
operator|.
name|flags
operator|&
name|ISTAPE
operator|)
condition|)
name|err
argument_list|(
literal|"files is not supported for non-tape devices"
argument_list|)
expr_stmt|;
if|if
condition|(
name|out
operator|.
name|name
operator|==
name|NULL
condition|)
block|{
comment|/* No way to check for read access here. */
name|out
operator|.
name|fd
operator|=
name|STDOUT_FILENO
expr_stmt|;
name|out
operator|.
name|name
operator|=
literal|"stdout"
expr_stmt|;
block|}
else|else
block|{
name|out
operator|.
name|fd
operator|=
name|open
argument_list|(
name|out
operator|.
name|name
argument_list|,
name|O_RDWR
operator||
name|O_CREAT
argument_list|,
name|DEFFILEMODE
argument_list|)
expr_stmt|;
comment|/* 		 * May not have read access, so try again with write only. 		 * Without read we may have a problem if output also does 		 * not support seeks. 		 */
if|if
condition|(
name|out
operator|.
name|fd
operator|<
literal|0
condition|)
block|{
name|out
operator|.
name|fd
operator|=
name|open
argument_list|(
name|out
operator|.
name|name
argument_list|,
name|O_WRONLY
operator||
name|O_CREAT
argument_list|,
name|DEFFILEMODE
argument_list|)
expr_stmt|;
name|out
operator|.
name|flags
operator||=
name|NOREAD
expr_stmt|;
block|}
if|if
condition|(
name|out
operator|.
name|fd
operator|<
literal|0
condition|)
name|err
argument_list|(
literal|"%s: %s"
argument_list|,
name|out
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|fstat
argument_list|(
name|out
operator|.
name|fd
argument_list|,
operator|&
name|sb
argument_list|)
condition|)
name|err
argument_list|(
literal|"%s: %s"
argument_list|,
name|out
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|S_ISCHR
argument_list|(
name|sb
operator|.
name|st_mode
argument_list|)
condition|)
name|out
operator|.
name|flags
operator||=
name|ioctl
argument_list|(
name|out
operator|.
name|fd
argument_list|,
name|MTIOCGET
argument_list|,
operator|&
name|mt
argument_list|)
condition|?
name|ISCHR
else|:
name|ISTAPE
expr_stmt|;
elseif|else
if|if
condition|(
name|lseek
argument_list|(
name|out
operator|.
name|fd
argument_list|,
literal|0L
argument_list|,
name|SEEK_CUR
argument_list|)
operator|==
operator|-
literal|1
operator|&&
name|errno
operator|==
name|ESPIPE
condition|)
name|out
operator|.
name|flags
operator||=
name|ISPIPE
expr_stmt|;
comment|/* XXX fixed in 4.4BSD */
comment|/* 	 * Allocate space for the input and output buffers.  If not doing 	 * record oriented I/O, only need a single buffer. 	 */
if|if
condition|(
operator|!
operator|(
name|ddflags
operator|&
operator|(
name|C_BLOCK
operator||
name|C_UNBLOCK
operator|)
operator|)
condition|)
block|{
if|if
condition|(
name|in
operator|.
name|dbsz
operator|>
name|out
operator|.
name|dbsz
condition|)
name|cnt
operator|=
name|in
operator|.
name|dbsz
operator|-
literal|1
operator|-
name|in
operator|.
name|dbsz
operator|/
literal|2
operator|+
name|in
operator|.
name|dbsz
expr_stmt|;
elseif|else
if|if
condition|(
name|in
operator|.
name|dbsz
operator|<
name|out
operator|.
name|dbsz
condition|)
name|cnt
operator|=
name|out
operator|.
name|dbsz
operator|+
name|in
operator|.
name|dbsz
operator|-
literal|1
expr_stmt|;
else|else
name|cnt
operator|=
name|in
operator|.
name|dbsz
expr_stmt|;
if|if
condition|(
operator|(
name|in
operator|.
name|db
operator|=
name|malloc
argument_list|(
name|cnt
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|err
argument_list|(
literal|"%s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|out
operator|.
name|db
operator|=
name|in
operator|.
name|db
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|in
operator|.
name|db
operator|=
name|malloc
argument_list|(
call|(
name|u_int
call|)
argument_list|(
name|MAX
argument_list|(
name|in
operator|.
name|dbsz
argument_list|,
name|cbsz
argument_list|)
operator|+
name|cbsz
argument_list|)
argument_list|)
operator|)
operator|==
name|NULL
operator|||
operator|(
name|out
operator|.
name|db
operator|=
name|malloc
argument_list|(
call|(
name|u_int
call|)
argument_list|(
name|out
operator|.
name|dbsz
operator|+
name|cbsz
argument_list|)
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|err
argument_list|(
literal|"%s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|in
operator|.
name|dbp
operator|=
name|in
operator|.
name|db
expr_stmt|;
name|out
operator|.
name|dbp
operator|=
name|out
operator|.
name|db
expr_stmt|;
comment|/* Position the input/output streams. */
if|if
condition|(
name|in
operator|.
name|offset
condition|)
name|pos_in
argument_list|()
expr_stmt|;
if|if
condition|(
name|out
operator|.
name|offset
condition|)
name|pos_out
argument_list|()
expr_stmt|;
comment|/* Truncate the output file. */
if|if
condition|(
name|ddflags
operator|&
name|C_NOTRUNC
condition|)
block|{
if|if
condition|(
name|out
operator|.
name|flags
operator|&
name|ISTAPE
condition|)
name|err
argument_list|(
literal|"notrunc is not supported for tape devices"
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|S_ISREG
argument_list|(
name|sb
operator|.
name|st_mode
argument_list|)
operator|&&
name|ftruncate
argument_list|(
name|out
operator|.
name|fd
argument_list|,
operator|(
name|off_t
operator|)
name|out
operator|.
name|offset
operator|*
name|out
operator|.
name|dbsz
argument_list|)
condition|)
name|err
argument_list|(
literal|"%s: truncate: %s"
argument_list|,
name|out
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 	 * If converting case at the same time as another conversion, build a 	 * table that does both at once.  If just converting case, use the 	 * built-in tables. 	 */
if|if
condition|(
name|ddflags
operator|&
operator|(
name|C_LCASE
operator||
name|C_UCASE
operator|)
condition|)
if|if
condition|(
name|ddflags
operator|&
name|C_BLOCK
condition|)
if|if
condition|(
name|ddflags
operator|&
name|C_LCASE
condition|)
block|{
for|for
control|(
name|cnt
operator|=
literal|0
init|;
name|cnt
operator|<
literal|0377
condition|;
operator|++
name|cnt
control|)
if|if
condition|(
name|isupper
argument_list|(
name|cnt
argument_list|)
condition|)
name|ctab
index|[
name|cnt
index|]
operator|=
name|ctab
index|[
name|tolower
argument_list|(
name|cnt
argument_list|)
index|]
expr_stmt|;
block|}
else|else
block|{
for|for
control|(
name|cnt
operator|=
literal|0
init|;
name|cnt
operator|<
literal|0377
condition|;
operator|++
name|cnt
control|)
if|if
condition|(
name|islower
argument_list|(
name|cnt
argument_list|)
condition|)
name|ctab
index|[
name|cnt
index|]
operator|=
name|ctab
index|[
name|toupper
argument_list|(
name|cnt
argument_list|)
index|]
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|ddflags
operator|&
name|C_UNBLOCK
condition|)
if|if
condition|(
name|ddflags
operator|&
name|C_LCASE
condition|)
block|{
for|for
control|(
name|cnt
operator|=
literal|0
init|;
name|cnt
operator|<
literal|0377
condition|;
operator|++
name|cnt
control|)
if|if
condition|(
name|isupper
argument_list|(
name|ctab
index|[
name|cnt
index|]
argument_list|)
condition|)
name|ctab
index|[
name|cnt
index|]
operator|=
name|tolower
argument_list|(
name|ctab
index|[
name|cnt
index|]
argument_list|)
expr_stmt|;
block|}
else|else
block|{
for|for
control|(
name|cnt
operator|=
literal|0
init|;
name|cnt
operator|<
literal|0377
condition|;
operator|++
name|cnt
control|)
if|if
condition|(
name|islower
argument_list|(
name|ctab
index|[
name|cnt
index|]
argument_list|)
condition|)
name|ctab
index|[
name|cnt
index|]
operator|=
name|toupper
argument_list|(
name|ctab
index|[
name|cnt
index|]
argument_list|)
expr_stmt|;
block|}
else|else
block|{
specifier|extern
name|u_char
name|l2u
index|[]
decl_stmt|,
name|u2l
index|[]
decl_stmt|;
name|ctab
operator|=
name|ddflags
operator|&
name|C_LCASE
condition|?
name|u2l
else|:
name|l2u
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|dd_in
parameter_list|()
block|{
specifier|register
name|int
name|flags
decl_stmt|,
name|n
decl_stmt|;
for|for
control|(
name|flags
operator|=
name|ddflags
init|;
condition|;
control|)
block|{
if|if
condition|(
name|cpy_cnt
operator|&&
operator|(
name|in
operator|.
name|f_stats
operator|+
name|in
operator|.
name|p_stats
operator|)
operator|>=
name|cpy_cnt
condition|)
return|return;
comment|/* 		 * Zero the buffer first if trying to recover from errors so 		 * lose the minimum amount of data.  If doing block operations 		 * use spaces. 		 */
if|if
condition|(
name|flags
operator|&
operator|(
name|C_NOERROR
operator||
name|C_SYNC
operator|)
condition|)
if|if
condition|(
name|flags
operator|&
operator|(
name|C_BLOCK
operator||
name|C_UNBLOCK
operator|)
condition|)
name|memset
argument_list|(
name|in
operator|.
name|dbp
argument_list|,
literal|' '
argument_list|,
name|in
operator|.
name|dbsz
argument_list|)
expr_stmt|;
else|else
name|bzero
argument_list|(
name|in
operator|.
name|dbp
argument_list|,
name|in
operator|.
name|dbsz
argument_list|)
expr_stmt|;
name|n
operator|=
name|read
argument_list|(
name|in
operator|.
name|fd
argument_list|,
name|in
operator|.
name|dbp
argument_list|,
name|in
operator|.
name|dbsz
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
block|{
name|in
operator|.
name|dbrcnt
operator|=
literal|0
expr_stmt|;
return|return;
block|}
comment|/* Read error. */
if|if
condition|(
name|n
operator|<
literal|0
condition|)
block|{
comment|/* 			 * If noerror not specified, die.  POSIX requires that 			 * the warning message be followed by an I/O display. 			 */
if|if
condition|(
operator|!
operator|(
name|flags
operator|&
name|C_NOERROR
operator|)
condition|)
name|err
argument_list|(
literal|"%s: %s"
argument_list|,
name|in
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|warn
argument_list|(
literal|"%s: %s"
argument_list|,
name|in
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|summary
argument_list|(
literal|0
argument_list|)
expr_stmt|;
comment|/* 			 * If it's not a tape drive or a pipe, seek past the 			 * error.  If your OS doesn't do the right thing for 			 * raw disks this section should be modified to re-read 			 * in sector size chunks. 			 */
if|if
condition|(
operator|!
operator|(
name|in
operator|.
name|flags
operator|&
operator|(
name|ISPIPE
operator||
name|ISTAPE
operator|)
operator|)
operator|&&
name|lseek
argument_list|(
name|in
operator|.
name|fd
argument_list|,
operator|(
name|off_t
operator|)
name|in
operator|.
name|dbsz
argument_list|,
name|SEEK_CUR
argument_list|)
condition|)
name|warn
argument_list|(
literal|"%s: %s"
argument_list|,
name|in
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/* If sync not specified, omit block and continue. */
if|if
condition|(
operator|!
operator|(
name|ddflags
operator|&
name|C_SYNC
operator|)
condition|)
continue|continue;
comment|/* Read errors count as full blocks. */
name|in
operator|.
name|dbcnt
operator|+=
name|in
operator|.
name|dbrcnt
operator|=
name|in
operator|.
name|dbsz
expr_stmt|;
operator|++
name|in
operator|.
name|f_stats
expr_stmt|;
comment|/* Handle full input blocks. */
block|}
elseif|else
if|if
condition|(
name|n
operator|==
name|in
operator|.
name|dbsz
condition|)
block|{
name|in
operator|.
name|dbcnt
operator|+=
name|in
operator|.
name|dbrcnt
operator|=
name|n
expr_stmt|;
operator|++
name|in
operator|.
name|f_stats
expr_stmt|;
comment|/* Handle partial input blocks. */
block|}
elseif|else
if|if
condition|(
name|n
operator|!=
name|in
operator|.
name|dbsz
condition|)
block|{
comment|/* If sync, fill the block with NULs. */
if|if
condition|(
name|ddflags
operator|&
name|C_SYNC
condition|)
block|{
name|bzero
argument_list|(
name|in
operator|.
name|dbp
operator|+
name|n
argument_list|,
name|in
operator|.
name|dbsz
operator|-
name|n
argument_list|)
expr_stmt|;
name|in
operator|.
name|dbcnt
operator|+=
name|in
operator|.
name|dbrcnt
operator|=
name|in
operator|.
name|dbsz
expr_stmt|;
block|}
else|else
name|in
operator|.
name|dbcnt
operator|+=
name|in
operator|.
name|dbrcnt
operator|=
name|n
expr_stmt|;
operator|++
name|in
operator|.
name|p_stats
expr_stmt|;
block|}
comment|/* 		 * POSIX states that if bs is set and no other conversions 		 * are specified, the block is output as it is read, without 		 * buffering. 		 */
if|if
condition|(
name|ddflags
operator|&
name|C_BS
condition|)
block|{
name|out
operator|.
name|dbcnt
operator|=
name|in
operator|.
name|dbcnt
expr_stmt|;
name|dd_out
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|in
operator|.
name|dbcnt
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|ddflags
operator|&
name|C_SWAB
condition|)
block|{
if|if
condition|(
operator|(
name|n
operator|=
name|in
operator|.
name|dbcnt
operator|)
operator|&
literal|1
condition|)
block|{
name|warn
argument_list|(
literal|"%s: odd swab count"
argument_list|,
name|in
operator|.
name|name
argument_list|)
expr_stmt|;
operator|--
name|n
expr_stmt|;
block|}
name|swab
argument_list|(
name|in
operator|.
name|dbp
argument_list|,
name|in
operator|.
name|dbp
argument_list|,
name|n
argument_list|)
expr_stmt|;
block|}
name|in
operator|.
name|dbp
operator|+=
name|in
operator|.
name|dbrcnt
expr_stmt|;
call|(
modifier|*
name|cfunc
call|)
argument_list|()
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * Cleanup any remaining I/O and flush output.  If necesssary, output file  * is truncated.  */
end_comment

begin_function
specifier|static
name|void
name|dd_close
parameter_list|()
block|{
if|if
condition|(
name|cfunc
operator|==
name|def
condition|)
name|def_close
argument_list|()
expr_stmt|;
elseif|else
if|if
condition|(
name|cfunc
operator|==
name|block
condition|)
name|block_close
argument_list|()
expr_stmt|;
elseif|else
if|if
condition|(
name|cfunc
operator|==
name|unblock
condition|)
name|unblock_close
argument_list|()
expr_stmt|;
if|if
condition|(
name|out
operator|.
name|dbcnt
condition|)
name|dd_out
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|dd_out
parameter_list|(
name|force
parameter_list|)
name|int
name|force
decl_stmt|;
block|{
specifier|static
name|int
name|warned
decl_stmt|;
specifier|register
name|int
name|cnt
decl_stmt|,
name|n
decl_stmt|,
name|nw
decl_stmt|;
specifier|register
name|u_char
modifier|*
name|outp
decl_stmt|;
comment|/* 	 * Write one or more blocks out.  The common case is writing a full 	 * output block in a single write; increment the full block stats. 	 * Otherwise, we're into partial block writes.  If a partial write, 	 * and it's a character device, just warn.  If a tape device, quit. 	 * 	 * The partial writes represent two cases.  1: Where the input block 	 * was less than expected so the output block was less than expected. 	 * 2: Where the input block was the right size but we were forced to 	 * write the block in multiple chunks.  The original versions of dd(1) 	 * never wrote a block in more than a single write, so the latter case 	 * never happened. 	 * 	 * One special case is if we're forced to do the write -- in that case 	 * we play games with the buffer size, and it's usually a partial write. 	 */
name|outp
operator|=
name|out
operator|.
name|db
expr_stmt|;
for|for
control|(
name|n
operator|=
name|force
condition|?
name|out
operator|.
name|dbcnt
else|:
name|out
operator|.
name|dbsz
init|;
condition|;
name|n
operator|=
name|out
operator|.
name|dbsz
control|)
block|{
for|for
control|(
name|cnt
operator|=
name|n
init|;
condition|;
name|cnt
operator|-=
name|nw
control|)
block|{
name|outp
operator|+=
name|nw
operator|=
name|write
argument_list|(
name|out
operator|.
name|fd
argument_list|,
name|outp
argument_list|,
name|cnt
argument_list|)
expr_stmt|;
if|if
condition|(
name|nw
operator|==
name|n
condition|)
block|{
if|if
condition|(
name|n
operator|!=
name|out
operator|.
name|dbsz
condition|)
operator|++
name|out
operator|.
name|p_stats
expr_stmt|;
else|else
operator|++
name|out
operator|.
name|f_stats
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|nw
operator|<
literal|0
condition|)
name|err
argument_list|(
literal|"%s: %s"
argument_list|,
name|out
operator|.
name|name
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
operator|++
name|out
operator|.
name|p_stats
expr_stmt|;
if|if
condition|(
name|nw
operator|==
name|cnt
condition|)
break|break;
if|if
condition|(
name|out
operator|.
name|flags
operator|&
name|ISCHR
operator|&&
operator|!
name|warned
condition|)
block|{
name|warned
operator|=
literal|1
expr_stmt|;
name|warn
argument_list|(
literal|"%s: short write on character device"
argument_list|,
name|out
operator|.
name|name
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|out
operator|.
name|flags
operator|&
name|ISTAPE
condition|)
name|err
argument_list|(
literal|"%s: short write on tape device"
argument_list|,
name|out
operator|.
name|name
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|out
operator|.
name|dbcnt
operator|-=
name|n
operator|)
operator|<
name|out
operator|.
name|dbsz
condition|)
break|break;
block|}
comment|/* Reassemble the output block. */
if|if
condition|(
name|out
operator|.
name|dbcnt
condition|)
name|bcopy
argument_list|(
name|out
operator|.
name|dbp
operator|-
name|out
operator|.
name|dbcnt
argument_list|,
name|out
operator|.
name|db
argument_list|,
name|out
operator|.
name|dbcnt
argument_list|)
expr_stmt|;
name|out
operator|.
name|dbp
operator|=
name|out
operator|.
name|db
operator|+
name|out
operator|.
name|dbcnt
expr_stmt|;
block|}
end_function

end_unit

