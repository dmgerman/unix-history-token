...
index|[]
init|=
literal|"$Id: trans.c,v 1.31 1994/01/30 20:59:40 ian Rel $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* A structure used to charge time to file transfers.  */
end_comment
...
name|scharge
block|{
comment|/* The transfer we are currently charging.  */
name|struct
name|stransfer
modifier|*
name|qtrans
decl_stmt|;
comment|/* The time at the last update.  */
name|long
name|isecs
decl_stmt|;
name|long
...
begin_comment
comment|/* We are always charging one send and one receive.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* We just received data for this transfer, so start charging.  */
return|return
name|ftcharge
argument_list|(
name|qdaemon
...
expr_stmt|;
block|}
comment|/* Since we're now going to wait to send data, don't charge this      transfer for receive time.  */
if|if
condition|(
name|qtrans
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Since we are now going to wait to receive data, don't charge this      transfer for send time.  */
if|if
condition|(
name|qtrans
operator|==
...
directive|endif
comment|/* Don't try to charge time to this structure any longer.  */
if|if
condition|(
name|sTsend
operator|.
...
name|imicros
expr_stmt|;
comment|/* If we are charging the same structure for both send and 	 receive, update the time we are not currently charging so 	 that we don't charge twice for the same time.  */
if|if
condition|(
name|qcharge
operator|->
...
name|inextmicros
expr_stmt|;
comment|/* If enough time has elapsed since the last time we checked the      queue, check it again.  We do this here because we have already      gone to the trouble of getting the time.  */
if|if
condition|(
name|inextsecs
operator|-
...
name|qsys
expr_stmt|;
comment|/* If we are not the caller, the grade will be set during the      initial handshake.  */
if|if
condition|(
operator|!
name|qdaemon
...
name|q
decl_stmt|;
comment|/* Update the transfer times, but avoid looking in the queue.  */
name|iTchecktime
operator|=
name|ixsysdep_process_time
argument_list|(
