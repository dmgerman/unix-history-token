...
condition|)
block|{
comment|/* There may be more than one function with the 			       same name but in different files.  In order to 			       set breakpoints on all of them, we must give 			       both the file name and the function name to 			       break_command.  */
name|char
modifier|*
name|string
init|=
...
block|char buf[1024]; 			c_type_print_base (TYPE_FN_FIELD_TYPE(t, i), 					   stdout, 0, 0);  			c_type_print_varspec_prefix (TYPE_FN_FIELD_TYPE(t, i), 						     stdout, 0);  			sprintf (buf, " %s::", type_name_no_tag (t)); 			cp_type_print_method_args (TYPE_FN_FIELD_ARGS (t, i), 						   buf, name, stdout);
endif|#
directive|endif
block|}
block|}
