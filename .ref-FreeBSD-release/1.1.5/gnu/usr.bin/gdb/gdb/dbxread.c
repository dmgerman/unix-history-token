...
begin_comment
comment|/* Read dbx symbol tables and convert to internal format, for GDB.    Copyright 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993    Free Software Foundation, Inc.  This file is part of GDB.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
init|=
block|{
literal|"unknown symbol type character `%c'"
block|,
literal|0
block|,
literal|0
...
parameter_list|()
define|\
value|if (((unsigned)bufp->n_strx + file_string_table_offset)>=		\       DBX_STRINGTAB_SIZE (objfile)) {					\     complain (&string_table_offset_complaint, symnum);			\     namestring = "foo";							\   } else								\     namestring = bufp->n_strx + file_string_table_offset +		\ 		 DBX_STRINGTAB (objfile)
define|#
directive|define
...
literal|0
decl_stmt|;
comment|/* This is true for Solaris (and all other stabs-in-elf systems, hopefully,      since it would be silly to do things differently from Solaris), and      false for SunOS4 and other a.out file formats.  */
name|block_address_function_relative
operator|=
literal|0
operator|==
...
condition|)
block|{
comment|/* The linker relocated it.  There used to be a kludge here 	       to add the text offset, but that will break if we ever 	       start using the text offset (currently it is always zero).  */
goto|goto
name|define_a_symbol
goto|;
block|}
...
begin_comment
comment|/* FIXME: The only difference between this and elfstab_build_psymtabs is    the call to install_minimal_symbols for elf.  If the differences are    really that small, the code should be shared.  */
end_comment
...
begin_comment
comment|/* Scan and build partial symbols for a PA symbol file.    This PA file has already been processed to get its minimal symbols.     OBJFILE is the object file we are reading symbols from.    ADDR is the address relative to which the symbols are (e.g.    the base address of the text segment).    MAINLINE is true if we are reading the main symbol    table (as opposed to a shared lib or dynamically loaded file).     */
end_comment

begin_function
name|void
name|pastab_build_psymtabs
parameter_list|(
name|objfile
parameter_list|,
name|section_offsets
...
argument_list|()
expr_stmt|;
comment|/* In a PA file, we've already installed the minimal symbols that came      from the PA (non-stab) symbol table, so always act like an      incremental load here. */
name|dbx_symfile_read
argument_list|(
name|objfile
argument_list|,
...
begin_comment
comment|/* Register our willingness to decode symbols for SunOS and a.out and    NetBSD and b.out files handled by BFD... */
end_comment
