...
begin_comment
comment|/* Optimize by combining instructions for GNU compiler.    Copyright (C) 1987, 1988, 1992, 1993 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* If byte loads either zero- or sign- extend, define BYTE_LOADS_EXTEND    for cases when we don't care which is true.  Define LOAD_EXTEND to    be ZERO_EXTEND or SIGN_EXTEND, depending on which was defined.  */
end_comment
...
block|}
block|}
comment|/* Update reg_nonzero_bits et al for any changes that may have been made        to this insn.  */
name|note_stores
argument_list|(
name|newpat
argument_list|,
...
return|;
block|}
comment|/* If we want a subreg of a constant, at offset 0, 	 take the low bits.  On a little-endian machine, that's 	 always valid.  On a big-endian machine, it's valid 	 only if the constant's mode fits in one word.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
argument_list|)
return|;
comment|/* (ashiftrt foo C) where C is the number of bits in FOO minus 1 	 is (lt foo (const_int 0)), so we can perform the above 	 simplification.  */
if|if
condition|(
name|XEXP
argument_list|(
...
goto|;
block|}
comment|/* If only the low-order bit of X is possible nonzero, (plus x -1) 	 can become (ashiftrt (ashift (xor x 1) C) C) where C is 	 the bitsize of the mode - 1.  This allows simplification of 	 "a = (b& 8) == 0;"  */
if|if
condition|(
name|XEXP
argument_list|(
...
operator|==
literal|1
comment|/* If STORE_FLAG_VALUE is 1, we can convert (ne x 0) to simply X 	     if only the low-order bit is possibly nonzero in X (such as when 	     X is a ZERO_EXTRACT of one bit.  Similarly, we can convert 	     EQ to (xor X 1).  Remove any ZERO_EXTRACT we made when thinking 	     this was a comparison.  It may now be simpler to use, e.g., an 	     AND.  If a ZERO_EXTRACT is indeed appropriate, it will 	     be placed back by the call to make_compound_operation in the 	     SET case.  */
if|if
condition|(
name|new_code
operator|==
...
operator|-
literal|1
comment|/* If STORE_FLAG_VALUE is -1, we can convert (ne x 0) 	     to (neg x) if only the low-order bit of X can be nonzero. 	     This converts (ne (zero_extract X 1 Y) 0) to 	     (sign_extract X 1 Y).  */
if|if
condition|(
name|new_code
operator|==
...
argument_list|)
return|;
comment|/* (lshiftrt foo C) where C is the number of bits in FOO minus 1 	 is (lt foo (const_int 0)), so we can perform the above 	 simplification.  */
if|if
condition|(
name|XEXP
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* If C1> C2, INNER needs to have the shift performed on it 	     for C1-C2 bits.  */
if|if
condition|(
name|INTVAL
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* If we have something other than a SUBREG, we might have 	     done an expansion, so rerun outselves.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|LSHIFT
case|:
comment|/* For left shifts, do the same, but just for the first operand. 	 However, we cannot do anything with shifts where we cannot 	 guarantee that the counts are smaller than the size of the mode 	 because such a count will have a different meaning in a 	 wider mode.  	 If we can narrow the shift and know the count, we need even fewer 	 bits of the first operand.  */
if|if
condition|(
operator|!
operator|(
...
name|LSHIFTRT
case|:
comment|/* Here we can only do something if the shift count is a constant and 	 the count plus BITS is no larger than the width of MODE.  In that 	 case, we can do the shift in MODE.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|XOR
case|:
comment|/* If VAROP is (ior (lshiftrt FOO C1) C2), try to commute the IOR and 	     LSHIFT so we end up with an (and (lshiftrt (ior ...) ...) ...) 	     operation which may be a bitfield extraction.  Ensure 	     that the constant we form is not wider than the mode of 	     VAROP.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|NOT
case|:
comment|/* (and (not FOO)) is (and (xor FOO CONST)), so if FOO is an 	     LSHIFTRT, we can do the same as above.  Ensure that the constant 	     we form is not wider than the mode of VAROP.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|NE
case|:
comment|/* (and (ne FOO 0) CONST) can be (and FOO CONST) if CONST is 	     included in STORE_FLAG_VALUE and FOO has no bits that might be 	     nonzero not in CONST.  */
if|if
condition|(
operator|(
name|constop
...
name|EQ
case|:
comment|/* convert (lshift (eq FOO 0) C) to (xor FOO 1) if STORE_FLAG_VALUE 	     says that the sign bit can be tested, FOO has mode MODE, C is 	     GET_MODE_BITSIZE (MODE) - 1, and FOO has only the low-order bit 	     may be nonzero.  */
if|if
condition|(
name|code
operator|==
...
name|LSHIFT
case|:
comment|/* If we have (compare (xshift FOO N) (const_int C)) and 	     the high order N bits of FOO (N+1 if an inequality comparison) 	     are known to be zero, we can do this by comparing FOO with C 	     shifted right N bits so long as the low-order N bits of C are 	     zero.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* If the only nonzero bits in OP0 and OP1 are those in the 	     narrower mode and this is an equality or unsigned comparison, 	     we can use the wider mode.  Similarly for sign-extended 	     values and equality or signed comparisons.  */
if|if
condition|(
operator|(
operator|(
