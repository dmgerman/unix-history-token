...
begin_comment
comment|/* Make the integer constant T valid for its type    by setting to 0 or 1 all the bits in the constant    that don't belong in the type.    Yield 1 if a signed overflow occurs, 0 otherwise.    If OVERFLOW is nonzero, a signed overflow has already occurred    in calculating T, so propagate it.  */
end_comment
...
name|type
expr_stmt|;
comment|/* Indicate an overflow if (1) ARG1 already overflowed, 	     or (2) ARG1 is a too-large unsigned value and T is signed, 	     or (3) force_fit_type indicates an overflow. 	     force_fit_type can't detect (2), since it sees only T's type.  */
name|TREE_CONSTANT_OVERFLOW
argument_list|(
name|t
argument_list|)
