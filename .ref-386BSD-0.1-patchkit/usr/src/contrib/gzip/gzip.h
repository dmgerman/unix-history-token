...
name|size
parameter_list|)
value|{ \       array = (type*)fcalloc((unsigned)(((size)+1L)/2), 2*sizeof(type)); \       if (array == NULL) error("insufficient memory"); \    }
end_define
...
begin_comment
comment|/* input filename or "stdin" */
end_comment
...
begin_comment
comment|/* output filename or "stdout" */
end_comment
...
begin_comment
comment|/* put_byte is used for the compressed output, put_char for the  * uncompressed output. However unlzw() uses window for its  * suffix table instead of its output buffer, so it does not use put_char.  * (to be cleaned up).  */
end_comment
