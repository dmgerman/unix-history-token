...
begin_comment
comment|/*  * Different files within a diff  */
end_comment
...
begin_comment
comment|/*  * Three way diffs are build out of two two-way diffs; the file which  * the two two-way diffs share is:  */
end_comment
...
begin_comment
comment|/*  * Options variables for flags set on command line.  *  * ALWAYS_TEXT: Treat all files as text files; never treat as binary.  *  * EDSCRIPT: Write out an ed script instead of the standard diff3 format.  *  * FLAGGING: Indicates that in the case of overlapping diffs (type  * DIFF_ALL), the lines which would normally be deleted from file 1  * should be preserved with a special flagging mechanism.  *  * DONT_WRITE_OVERLAP: 1 if information for overlapping diffs should  * not be output.  *  * DONT_WRITE_SIMPLE: 1 if information for non-overlapping diffs  * should not be output.   *  * FINALWRITE: 1 if a :wq should be included at the end of the script  * to write out the file being edited.  *  * MERGE: output a merged file.  */
end_comment
...
begin_decl_stmt
name|int
name|dont_write_overlap
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|dont_write_simple
decl_stmt|;
end_decl_stmt
...
literal|0
decl_stmt|;
comment|/* Element 0 is for file 0, element 1 is for file 2.  */
name|char
modifier|*
name|tag_strings
index|[
...
literal|'L'
case|:
comment|/* Handle one or two -L arguments.  */
if|if
condition|(
name|tag_count
operator|<
...
name|merge
expr_stmt|;
comment|/* -eExX3 without -m implies ed script.  */
name|flagging
operator||=
operator|~
name|incompat
...
name|merge
expr_stmt|;
comment|/* -m without -eExX3 implies -E.  */
if|if
condition|(
name|incompat
operator|>
literal|1
comment|/* Ensure at most one of -eExX3.  */
operator|||
name|finalwrite
operator|&
operator|(
...
name|merge
operator|)
comment|/* -i needs one of -eExX3; -i -m would rewrite input file.  */
operator|||
name|tag_count
operator|&&
operator|!
name|flagging
comment|/* -L requires one of -EX.  */
operator|||
name|argc
operator|-
name|optind
...
condition|)
block|{
comment|/* Sigh.  We've got standard input as the first arg. We can't */
comment|/* call diff twice on stdin */
if|if
condition|(
operator|!
name|strcmp
...
name|stderr
argument_list|,
literal|"Usage:\t%s [-exEX3 [-i | -m] [-L label1 -L label3]] file1 file2 file3\n"
argument_list|,
name|argv0
argument_list|)
expr_stmt|;
...
name|stderr
argument_list|,
literal|"\tOnly one of [exEX3] allowed\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
block|base_water_mark = D_LOWLINE (high_water_diff, FILE0);
endif|#
directive|endif
name|high_water_mark
operator|=
...
name|fatal
argument_list|(
literal|"internal: screwup in format of diff blocks"
argument_list|)
expr_stmt|;
...
name|tmpblock
expr_stmt|;
comment|/* Setup corresponding lines correctly */
name|last_diff
operator|=
name|tmpblock
expr_stmt|;
...
name|current0line
decl_stmt|;
comment|/* Find the range in file0 */
name|lowc
operator|=
name|using
index|[
...
index|]
expr_stmt|;
comment|/* Find the ranges in the other files.      If using[x] is null, that means that the file to which that diff      refers is equivalent to file 0 over this range */
if|if
condition|(
name|using
index|[
...
expr_stmt|;
block|}
comment|/*    * Note that in the following, we use file 1 relative to the diff,    * and file 2 relative to the corresponding lines struct.    */
if|if
condition|(
name|using
index|[
...
argument_list|)
expr_stmt|;
comment|/* Copy over all of the information for File 0.  Return with a zero      if any of the compares failed. */
for|for
control|(
name|ptr
operator|=
...
return|;
block|}
comment|/* Copy stuff for file 1.  First deal with anything that might be      before the first diff. */
for|for
control|(
name|i
operator|=
...
block|}
block|}
comment|/* Copy stuff for file 2.  First deal with anything that might be      before the first diff. */
for|for
control|(
name|i
operator|=
...
name|fatal
argument_list|(
literal|"internal: Bad diff type in process_diff"
argument_list|)
expr_stmt|;
break|break;
block|}
...
name|fatal
argument_list|(
literal|"Bad diff format: bad change separator"
argument_list|)
expr_stmt|;
name|scan_diff
operator|+=
...
parameter_list|)
define|\
value|{ if (!isdigit (*s)) return ERROR; holdnum = 0;	\ 	  do { holdnum = (*s++ - '0' + holdnum * 10); }	\ 	  while (isdigit (*s)); (num) = holdnum; }
...
argument_list|)
expr_stmt|;
comment|/* Avoid stdio, because the parent process's buffers are inherited. */
name|write
argument_list|(
name|fileno
argument_list|(
...
name|fatal
argument_list|(
literal|"bad diff format; incomplete last line"
argument_list|)
expr_stmt|;
operator|*
name|output_placement
...
name|fatal
argument_list|(
literal|"Bad diff format; incorrect leading line chars"
argument_list|)
expr_stmt|;
operator|*
name|set_start
...
name|fatal
argument_list|(
literal|"internal: Bad diff type passed to output"
argument_list|)
expr_stmt|;
block|}
name|fprintf
...
name|DIFF_ALL
condition|)
comment|/* Do special flagging */
block|{
comment|/* Put in lines from FILE2 with bracket */
name|fprintf
argument_list|(
name|outputfile
argument_list|,
...
literal|1
expr_stmt|;
comment|/* Add in code to take care of leading dots, if necessary. */
if|if
condition|(
name|leading_dot
condition|)
...
expr_stmt|;
block|}
comment|/* Put in code to do initial bracket of lines from FILE0  */
name|fprintf
argument_list|(
name|outputfile
argument_list|,
...
condition|)
continue|continue;
comment|/* Copy I lines from common file.  */
name|i
operator|=
name|D_LOWLINE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Skip I lines in common file.  */
name|i
operator|=
name|D_NUMLINES
argument_list|(
