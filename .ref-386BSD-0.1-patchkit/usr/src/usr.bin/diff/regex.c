...
value|\     for (last_used_reg = RE_NREGS - 1; last_used_reg> 0; last_used_reg--)\       if (regstart[last_used_reg] != (unsigned char *) -1)		\         break;								\ 									\     if (stacke - stackp< NUM_FAILURE_ITEMS)				\       {									\ 	unsigned char **stackx;						\ 	if (stacke - stackb> re_max_failures * MAX_NUM_FAILURE_ITEMS)	\ 	  return -2;							\ 									\
comment|/* Roughly double the size of the stack.  */
value|\         stackx = (unsigned char **) alloca (2 * MAX_NUM_FAILURE_ITEMS	\ 				            * (stacke - stackb)		\                                             * sizeof (unsigned char *));\
comment|/* Only copy what is in use.  */
value|\         bcopy (stackb, stackx, (stackp - stackb) * sizeof (char *));	\ 	stackp = stackx + (stackp - stackb);				\ 	stackb = stackx;						\ 	stacke = stackb + 2 * MAX_NUM_FAILURE_ITEMS * (stacke - stackb);\       }									\ 									\
