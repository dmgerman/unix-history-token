...
index|[]
init|=
literal|"@(#)g++.c	6.5 (Berkeley) 5/8/91"
decl_stmt|;
end_decl_stmt
...
directive|define
name|LIB_SPEC
value|"%{!p:%{!pg:-lm -lc}}%{p:-lm_p -lc_p}%{pg:-lm_p -lc_p}"
end_define
...
parameter_list|)
define|\
value|((CHAR) == 'D' || (CHAR) == 'U' || (CHAR) == 'o' \    || (CHAR) == 'e' || (CHAR) == 'T' || (CHAR) == 'u' \    || (CHAR) == 'I' || (CHAR) == 'Y' || (CHAR) == 'm' \    || (CHAR) == 'L' || (CHAR) == 'i')
end_define
...
define|#
directive|define
name|cplusplus_string
define|\
value|"cpp -+ %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{i*} \        -undef -D__GNUC__ -D__GNUG__ -D__cplusplus %p %P\        %c %{O:-D__OPTIMIZE__} %{traditional} %{pedantic}\        %{Wcomment*} %{Wtrigraphs} %{Wall} %C\        %i %{!M*:%{!E:%{!pipe:%g.cpp}}}%{E:%{o*}}%{M*:%{o*}} |\n\    %{!M*:%{!E:cc1plus %{!pipe:%g.cpp} %1\ 	     %{!Q:-quiet} -dumpbase %i %{Y*} %{d*} %{m*} %{f*} %{+e*} %{a}\ 	     %{g} %{g0} %{O} %{W*} %{w} %{pedantic} %{traditional}\ 	     %{v:-version} %{gg:-symout %g.sym} %{pg:-p} %{p} %{xref}\ 	     %{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\ 	     %{S:%{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\ 	     %{!S:as %{R} %{j} %{J} %{h} %{d2} %a %{gg:-G %g.sym}\ 		%{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o}\                    %{!pipe:%g.s}\n }}}"
end_define
...
directive|if
literal|0
block|"cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{i*} %{trigraphs} -undef \         -D__GNUC__ %{ansi:-trigraphs -$ -D__STRICT_ANSI__} %{!ansi:%p} %P\         %c %{O:-D__OPTIMIZE__} %{traditional} %{pedantic}\ 	%{Wcomment*} %{Wtrigraphs} %{Wall} %C\         %i %{!M*:%{!E:%{!pipe:%g.cpp}}}%{E:%{o*}}%{M*:%{o*}} |\n\     %{!M*:%{!E:cc1 %{!pipe:%g.cpp} %1 \ 		   %{!Q:-quiet} -dumpbase %i %{Y*} %{d*} %{m*} %{f*} %{a}\ 		   %{g} %{O} %{W*} %{w} %{pedantic} %{ansi} %{traditional}\ 		   %{v:-version} %{gg:-symout %g.sym} %{pg:-p} %{p} %{xref}\ 		   %{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\ 		   %{S:%{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\               %{!S:as %{R} %{j} %{J} %{h} %{d2} %a %{gg:-G %g.sym}\ 		      %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o}\                       %{!pipe:%g.s}\n }}}"
else|#
directive|else
name|cplusplus_string
endif|#
...
literal|".i"
block|,
literal|"cc1 %i %1 %{!Q:-quiet} %{Y*} %{d*} %{m*} %{f*} %{a}\ 	%{g} %{O} %{W*} %{w} %{pedantic} %{ansi} %{traditional}\ 	%{v:-version} %{gg:-symout %g.sym} %{pg:-p} %{p} %{xref}\ 	%{S:%{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\     %{!S:as %{R} %{j} %{J} %{h} %{d2} %a %{gg:-G %g.sym}\             %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o} %{!pipe:%g.s}\n }"
block|}
block|,
block|{
literal|".s"
block|,
literal|"%{!S:as %{R} %{j} %{J} %{h} %{d2} %a \             %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o} %i\n }"
block|}
block|,
block|{
literal|".S"
block|,
literal|"cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{i*} %{M*} %{trigraphs} \         -undef -D__GNUC__ -$ %p %P\         %c %{O:-D__OPTIMIZE__} %{traditional} %{pedantic}\ 	%{Wcomment*} %{Wtrigraphs} %{Wall} %C\         %i %{!M*:%{!E:%{!pipe:%g.s}}}%{E:%{o*}}%{M*:%{o*}} |\n\     %{!M*:%{!E:%{!S:as %{R} %{j} %{J} %{h} %{d2} %a \                     %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o}\ 		    %{!pipe:%g.s}\n }}}"
block|}
block|,
...
begin_comment
comment|/* C++: Here is the spec for collecting global ctor and dtor    requirements.  */
end_comment
...
name|collect_spec
init|=
literal|"%{!c:%{!M*:%{!E:%{!S:collect -o %g.s %g.R\n\ as %g.s -o %g.O\n\ ld %{o*} %g.R %g.O\n\ }}}}"
decl_stmt|;
end_decl_stmt
...
name|link_spec
init|=
literal|"%{!c:%{!M*:%{!E:%{!S:ld -r -o %g.R %l\  %{A} %{d} %{e*} %{N} %{n} %{r} %{s} %{S} %{T*} %{t} %{u*} %{X} %{x} %{z}\  %{y*} %{!nostdlib:%S} \  %{L*} %o %{!nostdlib:-lg++ %L -lgnulib}\n }}}}"
decl_stmt|;
end_decl_stmt
...
name|link_spec
init|=
literal|"%{!c:%{!M*:%{!E:%{!S:ld %{o*} %l\  %{A} %{d} %{e*} %{N} %{n} %{r} %{s} %{S} %{T*} %{t} %{u*} %{X} %{x} %{z}\  %{y*} %{!nostdlib:%S} \  %{L*} %o %{!nostdlib:-lg++ %L -lgnulib}\n }}}}"
decl_stmt|;
end_decl_stmt
...
name|stderr
argument_list|,
literal|"`-B%s' not used by compiler\n"
argument_list|,
name|user_exec_prefix
argument_list|)
expr_stmt|;
...
directive|ifdef
name|USE_COLLECT
comment|/* C++: now collect all the requirements for      calling global constructors and destructors,      and write them to a file.  We will link this file      in when we run.  */
if|if
condition|(
name|linker_was_run
operator|&&
