...
literal|0
condition|)
comment|/* Default size is what GNU malloc can fit in a 4096-byte block.        Pick a number small enough that when rounded up to DEFAULT_ROUNDING        it is still smaller than 4096 - 4.  */
block|{
name|int
name|extra
init|=
...
name|new_size
expr_stmt|;
comment|/* Move the existing object to the new chunk.      Word at a time is fast and is safe because these      structures are aligned at least that much.  */
for|for
control|(
name|i
operator|=
