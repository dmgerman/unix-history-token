...
begin_comment
comment|/* Vector mapping INSN_UIDs to suids.    The suids are like uids but increase monononically always.    We use them to see whether a subroutine call came    between a variable's birth and its death.  */
end_comment
...
name|where_born
expr_stmt|;
comment|/* The reg must live at least one insn even 		 in it is never again used--because it has to go 		 in SOME hard reg.  */
if|if
condition|(
name|reg_where_dead
index|[
