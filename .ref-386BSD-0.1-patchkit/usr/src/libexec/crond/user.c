begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|LINT
argument_list|)
end_if

begin_decl_stmt
specifier|static
name|char
name|rcsid
index|[]
init|=
literal|"$Header: user.c,v 2.1 90/07/18 00:23:45 vixie Exp $"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* vix 26jan87 [log is in RCS file]  *  * PATCHES MAGIC                LEVEL   PATCH THAT GOT US HERE  * --------------------         -----   ----------------------  * CURRENT PATCH LEVEL:         1       00131  * --------------------         -----   ----------------------  *  * 06 Apr 93	Adam Glass	Fixes so it compiles quitely  *  */
end_comment

begin_comment
comment|/* Copyright 1988,1990 by Paul Vixie  * All rights reserved  *  * Distribute freely, except: don't remove my name from the source or  * documentation (don't take credit for my work), mark your changes (don't  * get me blamed for your possible bugs), don't alter or remove this  * notice.  May be sold if buildable source is provided to buyer.  No  * warrantee of any kind, express or implied, is included with this  * software; use at your own risk, responsibility for damages (if any) to  * anyone resulting from the use of this software rests entirely with the  * user.  *  * Send bug reports, bug fixes, enhancements, requests, flames, etc., and  * I'll try to keep a version up to date.  I can be reached as follows:  * Paul Vixie, 329 Noe Street, San Francisco, CA, 94114, (415) 864-7013,  * paul@vixie.sf.ca.us || {hoptoad,pacbell,decwrl,crash}!vixie!paul  */
end_comment

begin_include
include|#
directive|include
file|"cron.h"
end_include

begin_function
name|void
name|free_user
parameter_list|(
name|u
parameter_list|)
name|user
modifier|*
name|u
decl_stmt|;
block|{
name|void
name|free_entry
parameter_list|()
function_decl|;
name|int
name|free
parameter_list|()
function_decl|;
name|entry
modifier|*
name|e
decl_stmt|;
name|char
modifier|*
modifier|*
name|env
decl_stmt|;
for|for
control|(
name|e
operator|=
name|u
operator|->
name|crontab
init|;
name|e
operator|!=
name|NULL
condition|;
name|e
operator|=
name|e
operator|->
name|next
control|)
name|free_entry
argument_list|(
name|e
argument_list|)
expr_stmt|;
for|for
control|(
name|env
operator|=
name|u
operator|->
name|envp
init|;
operator|*
name|env
condition|;
name|env
operator|++
control|)
operator|(
name|void
operator|)
name|free
argument_list|(
operator|*
name|env
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|free
argument_list|(
name|u
operator|->
name|envp
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|free
argument_list|(
name|u
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|user
modifier|*
name|load_user
parameter_list|(
name|crontab_fd
parameter_list|,
name|name
parameter_list|,
name|uid
parameter_list|,
name|gid
parameter_list|,
name|dir
parameter_list|,
name|shell
parameter_list|)
name|int
name|crontab_fd
decl_stmt|;
name|char
modifier|*
name|name
decl_stmt|;
name|int
name|uid
decl_stmt|;
name|int
name|gid
decl_stmt|;
name|char
modifier|*
name|dir
decl_stmt|;
name|char
modifier|*
name|shell
decl_stmt|;
block|{
name|char
modifier|*
modifier|*
name|env_init
argument_list|()
decl_stmt|,
modifier|*
modifier|*
name|env_set
argument_list|()
decl_stmt|;
name|int
name|load_env
parameter_list|()
function_decl|;
name|entry
modifier|*
name|load_entry
parameter_list|()
function_decl|;
name|char
name|envstr
index|[
name|MAX_ENVSTR
index|]
decl_stmt|;
name|FILE
modifier|*
name|file
decl_stmt|;
name|user
modifier|*
name|u
decl_stmt|;
name|entry
modifier|*
name|e
decl_stmt|;
name|int
name|status
decl_stmt|;
if|if
condition|(
operator|!
operator|(
name|file
operator|=
name|fdopen
argument_list|(
name|crontab_fd
argument_list|,
literal|"r"
argument_list|)
operator|)
condition|)
block|{
name|perror
argument_list|(
literal|"fdopen on crontab_fd in load_user"
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|Debug
argument_list|(
argument|DPARS
argument_list|,
argument|(
literal|"load_user()\n"
argument|)
argument_list|)
comment|/* file is open.  build user entry, then read the crontab file. 	 */
name|u
operator|=
operator|(
name|user
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
name|user
argument_list|)
argument_list|)
expr_stmt|;
name|u
operator|->
name|uid
operator|=
name|uid
expr_stmt|;
name|u
operator|->
name|gid
operator|=
name|gid
expr_stmt|;
name|u
operator|->
name|envp
operator|=
name|env_init
argument_list|()
expr_stmt|;
name|u
operator|->
name|crontab
operator|=
name|NULL
expr_stmt|;
comment|/* 	 * do auto env settings that the user could reset in the cron tab 	 */
name|sprintf
argument_list|(
name|envstr
argument_list|,
literal|"SHELL=%s"
argument_list|,
operator|(
operator|*
name|shell
operator|)
condition|?
name|shell
else|:
literal|"/bin/sh"
argument_list|)
expr_stmt|;
name|u
operator|->
name|envp
operator|=
name|env_set
argument_list|(
name|u
operator|->
name|envp
argument_list|,
name|envstr
argument_list|)
expr_stmt|;
name|sprintf
argument_list|(
name|envstr
argument_list|,
literal|"HOME=%s"
argument_list|,
name|dir
argument_list|)
expr_stmt|;
name|u
operator|->
name|envp
operator|=
name|env_set
argument_list|(
name|u
operator|->
name|envp
argument_list|,
name|envstr
argument_list|)
expr_stmt|;
comment|/* load the crontab 	 */
while|while
condition|(
operator|(
name|status
operator|=
name|load_env
argument_list|(
name|envstr
argument_list|,
name|file
argument_list|)
operator|)
operator|>=
name|OK
condition|)
block|{
if|if
condition|(
name|status
operator|==
name|TRUE
condition|)
block|{
name|u
operator|->
name|envp
operator|=
name|env_set
argument_list|(
name|u
operator|->
name|envp
argument_list|,
name|envstr
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|NULL
operator|!=
operator|(
name|e
operator|=
name|load_entry
argument_list|(
name|file
argument_list|,
name|NULL
argument_list|)
operator|)
condition|)
block|{
name|e
operator|->
name|next
operator|=
name|u
operator|->
name|crontab
expr_stmt|;
name|u
operator|->
name|crontab
operator|=
name|e
expr_stmt|;
block|}
block|}
block|}
comment|/* 	 * do automatic env settings that should have precedence over any 	 * set in the cron tab. 	 */
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|envstr
argument_list|,
literal|"%s=%s"
argument_list|,
name|USERENV
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|u
operator|->
name|envp
operator|=
name|env_set
argument_list|(
name|u
operator|->
name|envp
argument_list|,
name|envstr
argument_list|)
expr_stmt|;
comment|/* 	 * done. close file, return pointer to 'user' structure 	 */
name|fclose
argument_list|(
name|file
argument_list|)
expr_stmt|;
name|Debug
argument_list|(
argument|DPARS
argument_list|,
argument|(
literal|"...load_user() done\n"
argument|)
argument_list|)
return|return
name|u
return|;
block|}
end_function

end_unit

