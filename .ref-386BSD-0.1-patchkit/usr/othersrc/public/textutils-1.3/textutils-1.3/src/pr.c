...
name|fp
decl_stmt|;
comment|/* File pointer for this column. */
enum|enum
block|{
name|OPEN
block|,
...
begin_comment
comment|/* (-[0-9]+) True when we're given an option explicitly specifying    number of columns.  Used to detect when this option is used with -m. */
end_comment
...
begin_comment
comment|/* True means we haven't encountered any filenames in the argument list. */
end_comment
...
begin_comment
comment|/* This is used to keep track of white space.  When we hit a    nonspace character after some whitespace, we print whitespace, tabbing    if necessary to get to output_position + spaces_not_printed. */
end_comment
...
begin_comment
comment|/* Number of columns with either an open file or stored lines. */
end_comment
...
begin_comment
comment|/* True means print a number as soon as we know we'll be printing    from the current column. */
end_comment
...
begin_comment
comment|/* Number of files opened initially in init_files().  Should be one    unless we're printing multiple files in parallel. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Where's that filename kept? */
if|if
condition|(
name|p
operator|->
...
name|p
decl_stmt|;
comment|/* Used as an accumulator (with | operator) of successive values of      print_vertical_padding.  The trick is to set print_vertical_padding      to zero before each run through the inner loop, then after that      loop, it tells us whether a line was actually printed (whether a      newline needs to be output -- or two for double spacing).  But those      values have to be accumulated (in pvp) so we can invoke pad_down      properly after the outer loop completes. */
name|int
name|pvp
decl_stmt|;
name|init_page
...
begin_comment
comment|/* Print a line from buff.     If this function has been called, we know we have something to    print.  Therefore we set print_vertical_padding to TRUE, print    a header if necessary, pad across if necessary, and print    separators if necessary.     Return TRUE, meaning there is no need to call read_rest_of_line. */
end_comment
