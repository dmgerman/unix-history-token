...
name|userv
init|=
literal|"User Interface 5A(082), 8 Feb 92"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Milliseconds to pause during xmit */
end_comment
...
directive|ifdef
name|COMMENT
comment|/* The ERROR command is badly broken.  Uncomment it when it's fixed! */
literal|"error"
block|,
name|XXERR
block|,
...
init|=
block|{
comment|/*  "decnet",        NET_DEC,  0, */
literal|"tcp/ip"
block|,
name|NET_TCPB
block|,
literal|0
comment|/*  "stream-tcp-ip", NET_TCPA, 0  */
ifdef|#
directive|ifdef
name|SUNX25
block|,
...
break|break;
block|}
comment|/* And later, even the smaller ones were removed, so now we just have ifs. */
comment|/* (And even later, many of these were moved to separate modules...) */
ifndef|#
directive|ifndef
name|NOSPL
...
operator|++
control|)
comment|/* and our local copy too */
name|inpbuf
index|[
name|x
index|]
...
name|printf
argument_list|(
literal|"?You must tell which file or log\n"
argument_list|)
expr_stmt|;
return|return
operator|(
...
directive|ifdef
name|datageneral
comment|/* The DG can take an argument after its "space" command. */
if|if
condition|(
operator|(
name|x
