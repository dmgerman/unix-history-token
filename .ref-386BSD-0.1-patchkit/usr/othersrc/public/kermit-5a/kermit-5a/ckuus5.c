...
init|=
block|{
literal|"if def _wloop ass \\%9 \\fdef(_wloop),"
block|,
literal|"ass _wloop {_getargs,:wtest,\\%1,\\%2,goto wtest,:wbot,_putargs,return},"
block|,
...
init|=
block|{
literal|"ass \\%9 \\fdef(_ify),ass _ify {_getargs, \\%1, _putargs},"
block|,
...
name|fatal
argument_list|(
literal|"cmdini: no memory for m_arg"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
comment|/* (NOTE - should really use zkermini for this!)    */
name|homdir
operator|=
name|zhome
argument_list|()
...
name|ic
decl_stmt|;
comment|/* Interruption character */
name|int
name|icn
decl_stmt|;
comment|/* Interruption character counter */
ifndef|#
directive|ifndef
name|NOSPL
name|int
...
directive|ifndef
name|NOSPL
comment|/* If macro, check for interruption and then get next command */
if|if
condition|(
name|cmdstk
index|[
...
name|lvl
expr_stmt|;
comment|/* No interrupt, get current level */
name|cbp
operator|=
name|cmdbuf
expr_stmt|;
...
control|)
block|{
comment|/* Allow braces around macro def to prevent */
comment|/* commas from being turned to end-of-lines */
comment|/* and also treat any commas within parens  */
comment|/* as text so that multiple-argument functions */
comment|/* won't cause the command to break prematurely. */
operator|*
name|cbp
operator|=
operator|*
...
block|}
block|}
comment|/* If TAKE file, get next line */
block|}
elseif|else
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* Look for carriage return */
if|if
condition|(
name|ic
operator|==
...
name|line
expr_stmt|;
comment|/* Got a line, copy it. */
name|debug
argument_list|(
name|F110
argument_list|,
literal|"from TAKE file"
argument_list|,
name|line
argument_list|,
literal|0
...
argument_list|)
expr_stmt|;
comment|/* if it's already there, delete it. */
name|debug
argument_list|(
name|F111
argument_list|,
...
name|printf
argument_list|(
literal|" Kanji character-set translation\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|s
expr_stmt|;
comment|/* Pointer to beginning of arg */
name|b
operator|=
literal|0
expr_stmt|;
...
literal|0
expr_stmt|;
comment|/* Argument counter, 0 thru 9 */
while|while
condition|(
literal|1
condition|)
...
comment|/* we have hit a space, then we have an argument to assign. */
if|if
condition|(
operator|(
name|y
...
literal|1
expr_stmt|;
comment|/* For ARGC variable */
return|return
operator|(
literal|1
operator|)
return|;
comment|/* DO command succeeded */
block|}
end_function
...
operator|)
return|;
comment|/* if no name, just print directory name */
if|if
condition|(
operator|*
name|s
...
name|printf
argument_list|(
literal|"Current directory name not available.\n"
argument_list|)
expr_stmt|;
elseif|else
if|if
...
condition|)
block|{
comment|/* Change disk successful */
if|if
condition|(
operator|(
name|int
