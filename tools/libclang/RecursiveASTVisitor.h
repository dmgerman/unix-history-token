...
ifndef|#
directive|ifndef
name|LLVM_CLANG_LIBCLANG_RECURSIVEASTVISITOR_H
end_ifndef
...
comment|///
comment|/// Stmts are traversed internally using a data queue to avoid a stack overflow
comment|/// with hugely nested ASTs.
comment|///
...
argument_list|()
operator|&&
literal|"base TraverseStmt was never called?"
argument_list|)
expr_stmt|;
return|return
operator|*
...
parameter_list|)
define|\
value|TRY_TO(Traverse##NAME(static_cast<CLASS*>(VAR))); break
...
literal|0
argument|, E = S->getNumInputs(); I< E; ++I) {       StmtQueue.queue(S->getInputConstraintLiteral(I));     }     for (unsigned I =
literal|0
...
argument_list|,
argument|{     StmtQueue.queue(S->getQueriedExpression());   }
argument_list|)
name|DEF_TRAVERSE_STMT
argument_list|(
argument|VAArgExpr
...
begin_comment
comment|// LLVM_CLANG_LIBCLANG_RECURSIVEASTVISITOR_H
end_comment
