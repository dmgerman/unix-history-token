...
begin_comment
comment|// XXX: acks don't work for now... too slow!
end_comment
...
directive|if
literal|0
block|{ 	struct timeval tv; 	gettimeofday(&tv, NULL);  	printf("sent ack %lu.%lu\n", tv.tv_sec, tv.tv_usec); 	}
endif|#
directive|endif
block|}
end_function
...
name|time_print
argument_list|(
literal|"Warning: caplen=%d datalen=%d\n"
argument_list|,
name|bpfh
operator|->
name|bh_caplen
...
literal|0
comment|// XXX
block|printf("Error: rd=%d caplen=%d\n", rd, bpfh->bh_caplen); 		hexdump(buf, rd+bpfh->bh_hdrlen);
comment|//		exit(1);
block|return;
endif|#
directive|endif
comment|// XXX what's going on
name|assert
argument_list|(
name|rd
operator|>
...
name|args
expr_stmt|;
comment|/* NB: try local copy first; then installed dir */
if|if
condition|(
name|execve
argument_list|(
