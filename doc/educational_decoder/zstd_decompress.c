...
begin_comment
comment|// Copies literals and returns the total literal length that was copied
end_comment
...
begin_comment
comment|// Given an offset code from a sequence command (either an actual offset value
end_comment

begin_comment
comment|// or an index for previous offset), computes the correct offset and udpates
end_comment

begin_comment
comment|// the offset history
end_comment
...
begin_comment
comment|// Given an offset, match length, and total output, as well as the frame
end_comment

begin_comment
comment|// context for the dictionary, determines if the dictionary is used and
end_comment

begin_comment
comment|// executes the copy operation
end_comment

begin_function_decl
specifier|static
name|void
name|execute_match_copy
parameter_list|(
name|frame_context_t
modifier|*
specifier|const
...
comment|/* this decoder assumes decompression of a single frame */
name|decode_frame
argument_list|(
operator|&
name|out
...
return|return;
block|}
comment|// not a real frame or a skippable frame
name|ERROR
argument_list|(
literal|"Tried to decode non-ZSTD frame"
argument_list|)
expr_stmt|;
block|}
end_function
...
begin_comment
comment|/// This implementation assumes `src` points to a single ZSTD-compressed frame
end_comment
...
argument_list|)
decl_stmt|;
comment|// get decompressed size from ZSTD frame header
block|{
specifier|const
name|u32
name|magic_number
...
block|{
comment|// not a real frame or skippable frame
name|ERROR
argument_list|(
literal|"ZSTD frame magic number did not match"
argument_list|)
expr_stmt|;
block|}
block|}
...
define|#
directive|define
name|DICT_SIZE_ERROR
parameter_list|()
value|ERROR("Dictionary size cannot be less than 8 bytes")
end_define
...
name|NULL_SRC
parameter_list|()
value|ERROR("Tried to create dictionary with pointer to null src");
end_define
...
condition|)
block|{
comment|/* cannot initialize dictionary with null src */
name|NULL_SRC
argument_list|()
expr_stmt|;
block|}
