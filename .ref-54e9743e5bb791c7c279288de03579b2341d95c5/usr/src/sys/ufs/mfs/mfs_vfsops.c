begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1989, 1990, 1993  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)mfs_vfsops.c	8.3 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/buf.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/signalvar.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/quota.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/inode.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/ufsmount.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/ufs_extern.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ffs/fs.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ffs/ffs_extern.h>
end_include

begin_include
include|#
directive|include
file|<ufs/mfs/mfsnode.h>
end_include

begin_include
include|#
directive|include
file|<ufs/mfs/mfs_extern.h>
end_include

begin_decl_stmt
name|caddr_t
name|mfs_rootbase
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* address of mini-root in kernel virtual memory */
end_comment

begin_decl_stmt
name|u_long
name|mfs_rootsize
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* size of mini-root in bytes */
end_comment

begin_decl_stmt
specifier|static
name|int
name|mfs_minor
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* used for building internal dev_t */
end_comment

begin_function_decl
specifier|extern
name|int
function_decl|(
modifier|*
modifier|*
name|mfs_vnodeop_p
function_decl|)
parameter_list|()
function_decl|;
end_function_decl

begin_comment
comment|/*  * mfs vfs operations.  */
end_comment

begin_decl_stmt
name|struct
name|vfsops
name|mfs_vfsops
init|=
block|{
name|mfs_mount
block|,
name|mfs_start
block|,
name|ffs_unmount
block|,
name|ffs_root
block|,
name|ufs_quotactl
block|,
name|mfs_statfs
block|,
name|ffs_sync
block|,
name|ffs_vget
block|,
name|ffs_fhtovp
block|,
name|ffs_vptofh
block|,
name|mfs_init
block|, }
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Called by main() when mfs is going to be mounted as root.  *  * Name is updated by mount(8) after booting.  */
end_comment

begin_define
define|#
directive|define
name|ROOTNAME
value|"mfs_root"
end_define

begin_macro
name|mfs_mountroot
argument_list|()
end_macro

begin_block
block|{
specifier|extern
name|struct
name|vnode
modifier|*
name|rootvp
decl_stmt|;
specifier|register
name|struct
name|fs
modifier|*
name|fs
decl_stmt|;
specifier|register
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
init|=
name|curproc
decl_stmt|;
comment|/* XXX */
name|struct
name|ufsmount
modifier|*
name|ump
decl_stmt|;
name|struct
name|mfsnode
modifier|*
name|mfsp
decl_stmt|;
name|u_int
name|size
decl_stmt|;
name|int
name|error
decl_stmt|;
comment|/* 	 * Get vnodes for swapdev and rootdev. 	 */
if|if
condition|(
name|bdevvp
argument_list|(
name|swapdev
argument_list|,
operator|&
name|swapdev_vp
argument_list|)
operator|||
name|bdevvp
argument_list|(
name|rootdev
argument_list|,
operator|&
name|rootvp
argument_list|)
condition|)
name|panic
argument_list|(
literal|"mfs_mountroot: can't setup bdevvp's"
argument_list|)
expr_stmt|;
name|mp
operator|=
name|malloc
argument_list|(
operator|(
name|u_long
operator|)
sizeof|sizeof
argument_list|(
expr|struct
name|mount
argument_list|)
argument_list|,
name|M_MOUNT
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|mp
argument_list|,
operator|(
name|u_long
operator|)
sizeof|sizeof
argument_list|(
expr|struct
name|mount
argument_list|)
argument_list|)
expr_stmt|;
name|mp
operator|->
name|mnt_op
operator|=
operator|&
name|mfs_vfsops
expr_stmt|;
name|mp
operator|->
name|mnt_flag
operator|=
name|MNT_RDONLY
expr_stmt|;
name|mfsp
operator|=
name|malloc
argument_list|(
sizeof|sizeof
expr|*
name|mfsp
argument_list|,
name|M_MFSNODE
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
name|rootvp
operator|->
name|v_data
operator|=
name|mfsp
expr_stmt|;
name|rootvp
operator|->
name|v_op
operator|=
name|mfs_vnodeop_p
expr_stmt|;
name|rootvp
operator|->
name|v_tag
operator|=
name|VT_MFS
expr_stmt|;
name|mfsp
operator|->
name|mfs_baseoff
operator|=
name|mfs_rootbase
expr_stmt|;
name|mfsp
operator|->
name|mfs_size
operator|=
name|mfs_rootsize
expr_stmt|;
name|mfsp
operator|->
name|mfs_vnode
operator|=
name|rootvp
expr_stmt|;
name|mfsp
operator|->
name|mfs_pid
operator|=
name|p
operator|->
name|p_pid
expr_stmt|;
name|mfsp
operator|->
name|mfs_buflist
operator|=
operator|(
expr|struct
name|buf
operator|*
operator|)
literal|0
expr_stmt|;
if|if
condition|(
name|error
operator|=
name|ffs_mountfs
argument_list|(
name|rootvp
argument_list|,
name|mp
argument_list|,
name|p
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|mp
argument_list|,
name|M_MOUNT
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|mfsp
argument_list|,
name|M_MFSNODE
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
if|if
condition|(
name|error
operator|=
name|vfs_lock
argument_list|(
name|mp
argument_list|)
condition|)
block|{
operator|(
name|void
operator|)
name|ffs_unmount
argument_list|(
name|mp
argument_list|,
literal|0
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|mp
argument_list|,
name|M_MOUNT
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|mfsp
argument_list|,
name|M_MFSNODE
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|TAILQ_INSERT_TAIL
argument_list|(
operator|&
name|mountlist
argument_list|,
name|mp
argument_list|,
name|mnt_list
argument_list|)
expr_stmt|;
name|mp
operator|->
name|mnt_flag
operator||=
name|MNT_ROOTFS
expr_stmt|;
name|mp
operator|->
name|mnt_vnodecovered
operator|=
name|NULLVP
expr_stmt|;
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|fs
operator|=
name|ump
operator|->
name|um_fs
expr_stmt|;
name|bzero
argument_list|(
name|fs
operator|->
name|fs_fsmnt
argument_list|,
sizeof|sizeof
argument_list|(
name|fs
operator|->
name|fs_fsmnt
argument_list|)
argument_list|)
expr_stmt|;
name|fs
operator|->
name|fs_fsmnt
index|[
literal|0
index|]
operator|=
literal|'/'
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|fs
operator|->
name|fs_fsmnt
argument_list|,
operator|(
name|caddr_t
operator|)
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntonname
argument_list|,
name|MNAMELEN
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|copystr
argument_list|(
name|ROOTNAME
argument_list|,
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
argument_list|,
name|MNAMELEN
operator|-
literal|1
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
operator|+
name|size
argument_list|,
name|MNAMELEN
operator|-
name|size
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|ffs_statfs
argument_list|(
name|mp
argument_list|,
operator|&
name|mp
operator|->
name|mnt_stat
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|vfs_unlock
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|inittodr
argument_list|(
operator|(
name|time_t
operator|)
literal|0
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * This is called early in boot to set the base address and size  * of the mini-root.  */
end_comment

begin_macro
name|mfs_initminiroot
argument_list|(
argument|base
argument_list|)
end_macro

begin_decl_stmt
name|caddr_t
name|base
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|fs
modifier|*
name|fs
init|=
operator|(
expr|struct
name|fs
operator|*
operator|)
operator|(
name|base
operator|+
name|SBOFF
operator|)
decl_stmt|;
specifier|extern
name|int
function_decl|(
modifier|*
name|mountroot
function_decl|)
parameter_list|()
function_decl|;
comment|/* check for valid super block */
if|if
condition|(
name|fs
operator|->
name|fs_magic
operator|!=
name|FS_MAGIC
operator|||
name|fs
operator|->
name|fs_bsize
operator|>
name|MAXBSIZE
operator|||
name|fs
operator|->
name|fs_bsize
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|fs
argument_list|)
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|mountroot
operator|=
name|mfs_mountroot
expr_stmt|;
name|mfs_rootbase
operator|=
name|base
expr_stmt|;
name|mfs_rootsize
operator|=
name|fs
operator|->
name|fs_fsize
operator|*
name|fs
operator|->
name|fs_size
expr_stmt|;
name|rootdev
operator|=
name|makedev
argument_list|(
literal|255
argument_list|,
name|mfs_minor
operator|++
argument_list|)
expr_stmt|;
return|return
operator|(
name|mfs_rootsize
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * VFS Operations.  *  * mount system call  */
end_comment

begin_comment
comment|/* ARGSUSED */
end_comment

begin_function
name|int
name|mfs_mount
parameter_list|(
name|mp
parameter_list|,
name|path
parameter_list|,
name|data
parameter_list|,
name|ndp
parameter_list|,
name|p
parameter_list|)
specifier|register
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
name|char
modifier|*
name|path
decl_stmt|;
name|caddr_t
name|data
decl_stmt|;
name|struct
name|nameidata
modifier|*
name|ndp
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
name|struct
name|vnode
modifier|*
name|devvp
decl_stmt|;
name|struct
name|mfs_args
name|args
decl_stmt|;
name|struct
name|ufsmount
modifier|*
name|ump
decl_stmt|;
specifier|register
name|struct
name|fs
modifier|*
name|fs
decl_stmt|;
specifier|register
name|struct
name|mfsnode
modifier|*
name|mfsp
decl_stmt|;
name|u_int
name|size
decl_stmt|;
name|int
name|flags
decl_stmt|,
name|error
decl_stmt|;
if|if
condition|(
name|error
operator|=
name|copyin
argument_list|(
name|data
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|args
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|mfs_args
argument_list|)
argument_list|)
condition|)
return|return
operator|(
name|error
operator|)
return|;
comment|/* 	 * If updating, check whether changing from read-only to 	 * read/write; if there is no device name, that's all we do. 	 */
if|if
condition|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_UPDATE
condition|)
block|{
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|fs
operator|=
name|ump
operator|->
name|um_fs
expr_stmt|;
if|if
condition|(
name|fs
operator|->
name|fs_ronly
operator|==
literal|0
operator|&&
operator|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_RDONLY
operator|)
condition|)
block|{
name|flags
operator|=
name|WRITECLOSE
expr_stmt|;
if|if
condition|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_FORCE
condition|)
name|flags
operator||=
name|FORCECLOSE
expr_stmt|;
if|if
condition|(
name|vfs_busy
argument_list|(
name|mp
argument_list|)
condition|)
return|return
operator|(
name|EBUSY
operator|)
return|;
name|error
operator|=
name|ffs_flushfiles
argument_list|(
name|mp
argument_list|,
name|flags
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|vfs_unbusy
argument_list|(
name|mp
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
block|}
if|if
condition|(
name|fs
operator|->
name|fs_ronly
operator|&&
operator|(
name|mp
operator|->
name|mnt_flag
operator|&
name|MNT_WANTRDWR
operator|)
condition|)
name|fs
operator|->
name|fs_ronly
operator|=
literal|0
expr_stmt|;
ifdef|#
directive|ifdef
name|EXPORTMFS
if|if
condition|(
name|args
operator|.
name|fspec
operator|==
literal|0
condition|)
return|return
operator|(
name|vfs_export
argument_list|(
name|mp
argument_list|,
operator|&
name|ump
operator|->
name|um_export
argument_list|,
operator|&
name|args
operator|.
name|export
argument_list|)
operator|)
return|;
endif|#
directive|endif
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|error
operator|=
name|getnewvnode
argument_list|(
name|VT_MFS
argument_list|,
operator|(
expr|struct
name|mount
operator|*
operator|)
literal|0
argument_list|,
name|mfs_vnodeop_p
argument_list|,
operator|&
name|devvp
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|devvp
operator|->
name|v_type
operator|=
name|VBLK
expr_stmt|;
if|if
condition|(
name|checkalias
argument_list|(
name|devvp
argument_list|,
name|makedev
argument_list|(
literal|255
argument_list|,
name|mfs_minor
operator|++
argument_list|)
argument_list|,
operator|(
expr|struct
name|mount
operator|*
operator|)
literal|0
argument_list|)
condition|)
name|panic
argument_list|(
literal|"mfs_mount: dup dev"
argument_list|)
expr_stmt|;
name|mfsp
operator|=
operator|(
expr|struct
name|mfsnode
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
expr|*
name|mfsp
argument_list|,
name|M_MFSNODE
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
name|devvp
operator|->
name|v_data
operator|=
name|mfsp
expr_stmt|;
name|mfsp
operator|->
name|mfs_baseoff
operator|=
name|args
operator|.
name|base
expr_stmt|;
name|mfsp
operator|->
name|mfs_size
operator|=
name|args
operator|.
name|size
expr_stmt|;
name|mfsp
operator|->
name|mfs_vnode
operator|=
name|devvp
expr_stmt|;
name|mfsp
operator|->
name|mfs_pid
operator|=
name|p
operator|->
name|p_pid
expr_stmt|;
name|mfsp
operator|->
name|mfs_buflist
operator|=
operator|(
expr|struct
name|buf
operator|*
operator|)
literal|0
expr_stmt|;
if|if
condition|(
name|error
operator|=
name|ffs_mountfs
argument_list|(
name|devvp
argument_list|,
name|mp
argument_list|,
name|p
argument_list|)
condition|)
block|{
name|mfsp
operator|->
name|mfs_buflist
operator|=
operator|(
expr|struct
name|buf
operator|*
operator|)
operator|-
literal|1
expr_stmt|;
name|vrele
argument_list|(
name|devvp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|ump
operator|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
expr_stmt|;
name|fs
operator|=
name|ump
operator|->
name|um_fs
expr_stmt|;
operator|(
name|void
operator|)
name|copyinstr
argument_list|(
name|path
argument_list|,
name|fs
operator|->
name|fs_fsmnt
argument_list|,
sizeof|sizeof
argument_list|(
name|fs
operator|->
name|fs_fsmnt
argument_list|)
operator|-
literal|1
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|fs
operator|->
name|fs_fsmnt
operator|+
name|size
argument_list|,
sizeof|sizeof
argument_list|(
name|fs
operator|->
name|fs_fsmnt
argument_list|)
operator|-
name|size
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|fs
operator|->
name|fs_fsmnt
argument_list|,
operator|(
name|caddr_t
operator|)
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntonname
argument_list|,
name|MNAMELEN
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|copyinstr
argument_list|(
name|args
operator|.
name|fspec
argument_list|,
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
argument_list|,
name|MNAMELEN
operator|-
literal|1
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|mp
operator|->
name|mnt_stat
operator|.
name|f_mntfromname
operator|+
name|size
argument_list|,
name|MNAMELEN
operator|-
name|size
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|mfs_statfs
argument_list|(
name|mp
argument_list|,
operator|&
name|mp
operator|->
name|mnt_stat
argument_list|,
name|p
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_decl_stmt
name|int
name|mfs_pri
init|=
name|PWAIT
operator||
name|PCATCH
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* XXX prob. temp */
end_comment

begin_comment
comment|/*  * Used to grab the process and keep it in the kernel to service  * memory filesystem I/O requests.  *  * Loop servicing I/O requests.  * Copy the requested data into or out of the memory filesystem  * address space.  */
end_comment

begin_comment
comment|/* ARGSUSED */
end_comment

begin_function
name|int
name|mfs_start
parameter_list|(
name|mp
parameter_list|,
name|flags
parameter_list|,
name|p
parameter_list|)
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
name|int
name|flags
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
specifier|register
name|struct
name|vnode
modifier|*
name|vp
init|=
name|VFSTOUFS
argument_list|(
name|mp
argument_list|)
operator|->
name|um_devvp
decl_stmt|;
specifier|register
name|struct
name|mfsnode
modifier|*
name|mfsp
init|=
name|VTOMFS
argument_list|(
name|vp
argument_list|)
decl_stmt|;
specifier|register
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
specifier|register
name|caddr_t
name|base
decl_stmt|;
name|int
name|error
init|=
literal|0
decl_stmt|;
name|base
operator|=
name|mfsp
operator|->
name|mfs_baseoff
expr_stmt|;
while|while
condition|(
name|mfsp
operator|->
name|mfs_buflist
operator|!=
operator|(
expr|struct
name|buf
operator|*
operator|)
operator|(
operator|-
literal|1
operator|)
condition|)
block|{
while|while
condition|(
name|bp
operator|=
name|mfsp
operator|->
name|mfs_buflist
condition|)
block|{
name|mfsp
operator|->
name|mfs_buflist
operator|=
name|bp
operator|->
name|b_actf
expr_stmt|;
name|mfs_doio
argument_list|(
name|bp
argument_list|,
name|base
argument_list|)
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
name|bp
argument_list|)
expr_stmt|;
block|}
comment|/* 		 * If a non-ignored signal is received, try to unmount. 		 * If that fails, clear the signal (it has been "processed"), 		 * otherwise we will loop here, as tsleep will always return 		 * EINTR/ERESTART. 		 */
if|if
condition|(
name|error
operator|=
name|tsleep
argument_list|(
operator|(
name|caddr_t
operator|)
name|vp
argument_list|,
name|mfs_pri
argument_list|,
literal|"mfsidl"
argument_list|,
literal|0
argument_list|)
condition|)
if|if
condition|(
name|dounmount
argument_list|(
name|mp
argument_list|,
literal|0
argument_list|,
name|p
argument_list|)
operator|!=
literal|0
condition|)
name|CLRSIG
argument_list|(
name|p
argument_list|,
name|CURSIG
argument_list|(
name|p
argument_list|)
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Get file system statistics.  */
end_comment

begin_macro
name|mfs_statfs
argument_list|(
argument|mp
argument_list|,
argument|sbp
argument_list|,
argument|p
argument_list|)
end_macro

begin_decl_stmt
name|struct
name|mount
modifier|*
name|mp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|statfs
modifier|*
name|sbp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|error
decl_stmt|;
name|error
operator|=
name|ffs_statfs
argument_list|(
name|mp
argument_list|,
name|sbp
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|sbp
operator|->
name|f_type
operator|=
name|MOUNT_MFS
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_block

end_unit

