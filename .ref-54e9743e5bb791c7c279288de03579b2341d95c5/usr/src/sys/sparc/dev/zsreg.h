...
name|zc_csr
decl_stmt|;
comment|/* control and status, and indirect access */
name|u_char
name|zc_xxx0
decl_stmt|;
name|u_char
...
begin_comment
comment|/*  * N.B.: the keyboard is channel 1, the mouse channel 0; ttyb is 1, ttya  * is 0.  In other words, the things are BACKWARDS.  */
end_comment
...
begin_comment
comment|/*  * Some of the names in this files were chosen to make the hsis driver  * work unchanged (which means that they will match some in SunOS).  *  * `S.C.' stands for Special Condition, which is any of these:  *	receiver overrun	(aka silo overflow)  *	framing error		(missing stop bit, etc)  *	end of frame		(in synchronous modes)  *	parity error		(when `parity error is S.C.' is set)  */
end_comment

begin_comment
comment|/*  * Registers with only a single `numeric value' get a name.  * Other registers hold bits and are only numbered; the bit  * definitions imply the register number (see below).  *  * We never use the receive and transmit data registers as  * indirects (choosing instead the zc_data register), so they  * are not defined here.  */
end_comment
...
begin_comment
comment|/* one of the above sync modes */
end_comment
...
begin_comment
comment|/*  * Bits in Write Register 15 (`External/Status Interrupt Control').  * Most of these cause status interrupts whenever the corresponding  * bit or pin changes state (i.e., any rising or falling edge).  */
end_comment
...
begin_comment
comment|/*	0x04	   unused, must be zero */
end_comment
...
begin_comment
comment|/*	0x01	   unused, must be zero */
end_comment
