begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1992, 1993  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)t3000.c	8.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_comment
comment|/*  * Routines for calling up on a Telebit T3000 modem.  * Derived from Courier driver.  */
end_comment

begin_include
include|#
directive|include
file|"tip.h"
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_define
define|#
directive|define
name|MAXRETRY
value|5
end_define

begin_function_decl
specifier|static
name|void
name|sigALRM
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
specifier|static
name|int
name|timeout
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|connected
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|jmp_buf
name|timeoutbuf
decl_stmt|,
name|intbuf
decl_stmt|;
end_decl_stmt

begin_function_decl
specifier|static
name|int
name|t3000_sync
parameter_list|()
function_decl|;
end_function_decl

begin_expr_stmt
name|t3000_dialer
argument_list|(
name|num
argument_list|,
name|acu
argument_list|)
specifier|register
name|char
operator|*
name|num
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|char
modifier|*
name|acu
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
modifier|*
name|cp
decl_stmt|;
ifdef|#
directive|ifdef
name|ACULOG
name|char
name|line
index|[
literal|80
index|]
decl_stmt|;
endif|#
directive|endif
specifier|static
name|int
name|t3000_connect
argument_list|()
decl_stmt|,
name|t3000_swallow
argument_list|()
decl_stmt|;
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
name|printf
argument_list|(
literal|"Using \"%s\"\n"
argument_list|,
name|acu
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCHPCL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* 	 * Get in synch. 	 */
if|if
condition|(
operator|!
name|t3000_sync
argument_list|()
condition|)
block|{
name|badsynch
label|:
name|printf
argument_list|(
literal|"can't synchronize with t3000\n"
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|ACULOG
name|logent
argument_list|(
name|value
argument_list|(
name|HOST
argument_list|)
argument_list|,
name|num
argument_list|,
literal|"t3000"
argument_list|,
literal|"can't synch up"
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"AT E0\r"
argument_list|,
literal|6
argument_list|)
expr_stmt|;
comment|/* turn off echoing */
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
name|t3000_verbose_read
argument_list|()
expr_stmt|;
endif|#
directive|endif
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCFLUSH
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* flush any clutter */
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"AT E0 H0 Q0 X4 V1\r"
argument_list|,
literal|18
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|t3000_swallow
argument_list|(
literal|"\r\nOK\r\n"
argument_list|)
condition|)
goto|goto
name|badsynch
goto|;
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"AT D"
argument_list|,
literal|4
argument_list|)
expr_stmt|;
for|for
control|(
name|cp
operator|=
name|num
init|;
operator|*
name|cp
condition|;
name|cp
operator|++
control|)
if|if
condition|(
operator|*
name|cp
operator|==
literal|'='
condition|)
operator|*
name|cp
operator|=
literal|','
expr_stmt|;
name|t3000_write
argument_list|(
name|FD
argument_list|,
name|num
argument_list|,
name|strlen
argument_list|(
name|num
argument_list|)
argument_list|)
expr_stmt|;
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"\r"
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|connected
operator|=
name|t3000_connect
argument_list|()
expr_stmt|;
ifdef|#
directive|ifdef
name|ACULOG
if|if
condition|(
name|timeout
condition|)
block|{
name|sprintf
argument_list|(
name|line
argument_list|,
literal|"%d second dial timeout"
argument_list|,
name|number
argument_list|(
name|value
argument_list|(
name|DIALTIMEOUT
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
name|logent
argument_list|(
name|value
argument_list|(
name|HOST
argument_list|)
argument_list|,
name|num
argument_list|,
literal|"t3000"
argument_list|,
name|line
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
if|if
condition|(
name|timeout
condition|)
name|t3000_disconnect
argument_list|()
expr_stmt|;
return|return
operator|(
name|connected
operator|)
return|;
block|}
end_block

begin_macro
name|t3000_disconnect
argument_list|()
end_macro

begin_block
block|{
comment|/* first hang up the modem*/
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCCDTR
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCSDTR
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|t3000_sync
argument_list|()
expr_stmt|;
comment|/* reset */
name|close
argument_list|(
name|FD
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|t3000_abort
argument_list|()
end_macro

begin_block
block|{
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"\r"
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|/* send anything to abort the call */
name|t3000_disconnect
argument_list|()
expr_stmt|;
block|}
end_block

begin_function
specifier|static
name|void
name|sigALRM
parameter_list|()
block|{
name|printf
argument_list|(
literal|"\07timeout waiting for reply\n"
argument_list|)
expr_stmt|;
name|timeout
operator|=
literal|1
expr_stmt|;
name|longjmp
argument_list|(
name|timeoutbuf
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|t3000_swallow
parameter_list|(
name|match
parameter_list|)
specifier|register
name|char
modifier|*
name|match
decl_stmt|;
block|{
name|sig_t
name|f
decl_stmt|;
name|char
name|c
decl_stmt|;
name|f
operator|=
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|sigALRM
argument_list|)
expr_stmt|;
name|timeout
operator|=
literal|0
expr_stmt|;
do|do
block|{
if|if
condition|(
operator|*
name|match
operator|==
literal|'\0'
condition|)
block|{
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|f
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
if|if
condition|(
name|setjmp
argument_list|(
name|timeoutbuf
argument_list|)
condition|)
block|{
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|f
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|alarm
argument_list|(
name|number
argument_list|(
name|value
argument_list|(
name|DIALTIMEOUT
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
name|read
argument_list|(
name|FD
argument_list|,
operator|&
name|c
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|alarm
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|c
operator|&=
literal|0177
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
name|putchar
argument_list|(
name|c
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
do|while
condition|(
name|c
operator|==
operator|*
name|match
operator|++
condition|)
do|;
ifdef|#
directive|ifdef
name|DEBUG
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|SIG_DFL
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_ifndef
ifndef|#
directive|ifndef
name|B19200
end_ifndef

begin_comment
comment|/* XXX */
end_comment

begin_define
define|#
directive|define
name|B19200
value|EXTA
end_define

begin_define
define|#
directive|define
name|B38400
value|EXTB
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_struct
struct|struct
name|tbaud_msg
block|{
name|char
modifier|*
name|msg
decl_stmt|;
name|int
name|baud
decl_stmt|;
name|int
name|baud2
decl_stmt|;
block|}
name|tbaud_msg
index|[]
init|=
block|{
literal|""
block|,
name|B300
block|,
literal|0
block|,
literal|" 1200"
block|,
name|B1200
block|,
literal|0
block|,
literal|" 2400"
block|,
name|B2400
block|,
literal|0
block|,
literal|" 4800"
block|,
name|B4800
block|,
literal|0
block|,
literal|" 9600"
block|,
name|B9600
block|,
literal|0
block|,
literal|" 14400"
block|,
name|B19200
block|,
name|B9600
block|,
literal|" 19200"
block|,
name|B19200
block|,
name|B9600
block|,
literal|" 38400"
block|,
name|B38400
block|,
name|B9600
block|,
literal|" 57600"
block|,
name|B38400
block|,
name|B9600
block|,
literal|" 7512"
block|,
name|B9600
block|,
literal|0
block|,
literal|" 1275"
block|,
name|B2400
block|,
literal|0
block|,
literal|" 7200"
block|,
name|B9600
block|,
literal|0
block|,
literal|" 12000"
block|,
name|B19200
block|,
name|B9600
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|, }
struct|;
end_struct

begin_function
specifier|static
name|int
name|t3000_connect
parameter_list|()
block|{
name|char
name|c
decl_stmt|;
name|int
name|nc
decl_stmt|,
name|nl
decl_stmt|,
name|n
decl_stmt|;
name|struct
name|sgttyb
name|sb
decl_stmt|;
name|char
name|dialer_buf
index|[
literal|64
index|]
decl_stmt|;
name|struct
name|tbaud_msg
modifier|*
name|bm
decl_stmt|;
name|sig_t
name|f
decl_stmt|;
if|if
condition|(
name|t3000_swallow
argument_list|(
literal|"\r\n"
argument_list|)
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
name|f
operator|=
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|sigALRM
argument_list|)
expr_stmt|;
name|again
label|:
name|nc
operator|=
literal|0
expr_stmt|;
name|nl
operator|=
sizeof|sizeof
argument_list|(
name|dialer_buf
argument_list|)
operator|-
literal|1
expr_stmt|;
name|bzero
argument_list|(
name|dialer_buf
argument_list|,
sizeof|sizeof
argument_list|(
name|dialer_buf
argument_list|)
argument_list|)
expr_stmt|;
name|timeout
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|nc
operator|=
literal|0
operator|,
name|nl
operator|=
sizeof|sizeof
argument_list|(
name|dialer_buf
argument_list|)
operator|-
literal|1
init|;
name|nl
operator|>
literal|0
condition|;
name|nc
operator|++
operator|,
name|nl
operator|--
control|)
block|{
if|if
condition|(
name|setjmp
argument_list|(
name|timeoutbuf
argument_list|)
condition|)
break|break;
name|alarm
argument_list|(
name|number
argument_list|(
name|value
argument_list|(
name|DIALTIMEOUT
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
name|n
operator|=
name|read
argument_list|(
name|FD
argument_list|,
operator|&
name|c
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|alarm
argument_list|(
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|<=
literal|0
condition|)
break|break;
name|c
operator|&=
literal|0x7f
expr_stmt|;
if|if
condition|(
name|c
operator|==
literal|'\r'
condition|)
block|{
if|if
condition|(
name|t3000_swallow
argument_list|(
literal|"\n"
argument_list|)
operator|==
literal|0
condition|)
break|break;
if|if
condition|(
operator|!
name|dialer_buf
index|[
literal|0
index|]
condition|)
goto|goto
name|again
goto|;
if|if
condition|(
name|strcmp
argument_list|(
name|dialer_buf
argument_list|,
literal|"RINGING"
argument_list|)
operator|==
literal|0
operator|&&
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
block|{
ifdef|#
directive|ifdef
name|DEBUG
name|printf
argument_list|(
literal|"%s\r\n"
argument_list|,
name|dialer_buf
argument_list|)
expr_stmt|;
endif|#
directive|endif
goto|goto
name|again
goto|;
block|}
if|if
condition|(
name|strncmp
argument_list|(
name|dialer_buf
argument_list|,
literal|"CONNECT"
argument_list|,
sizeof|sizeof
argument_list|(
literal|"CONNECT"
argument_list|)
operator|-
literal|1
argument_list|)
operator|!=
literal|0
condition|)
break|break;
for|for
control|(
name|bm
operator|=
name|tbaud_msg
init|;
name|bm
operator|->
name|msg
condition|;
name|bm
operator|++
control|)
if|if
condition|(
name|strcmp
argument_list|(
name|bm
operator|->
name|msg
argument_list|,
name|dialer_buf
operator|+
sizeof|sizeof
argument_list|(
literal|"CONNECT"
argument_list|)
operator|-
literal|1
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCGETP
argument_list|,
operator|&
name|sb
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
literal|"TIOCGETP"
argument_list|)
expr_stmt|;
goto|goto
name|error
goto|;
block|}
name|sb
operator|.
name|sg_ispeed
operator|=
name|sb
operator|.
name|sg_ospeed
operator|=
name|bm
operator|->
name|baud
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|sb
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|bm
operator|->
name|baud2
condition|)
block|{
name|sb
operator|.
name|sg_ispeed
operator|=
name|sb
operator|.
name|sg_ospeed
operator|=
name|bm
operator|->
name|baud2
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|sb
argument_list|)
operator|>=
literal|0
condition|)
goto|goto
name|isok
goto|;
block|}
name|perror
argument_list|(
literal|"TIOCSETP"
argument_list|)
expr_stmt|;
goto|goto
name|error
goto|;
block|}
name|isok
label|:
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|f
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
name|printf
argument_list|(
literal|"%s\r\n"
argument_list|,
name|dialer_buf
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
operator|(
literal|1
operator|)
return|;
block|}
break|break;
block|}
name|dialer_buf
index|[
name|nc
index|]
operator|=
name|c
expr_stmt|;
ifdef|#
directive|ifdef
name|notdef
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
name|putchar
argument_list|(
name|c
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
name|error1
label|:
name|printf
argument_list|(
literal|"%s\r\n"
argument_list|,
name|dialer_buf
argument_list|)
expr_stmt|;
name|error
label|:
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|f
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * This convoluted piece of code attempts to get  * the t3000 in sync.  */
end_comment

begin_function
specifier|static
name|int
name|t3000_sync
parameter_list|()
block|{
name|int
name|already
init|=
literal|0
decl_stmt|;
name|int
name|len
decl_stmt|;
name|char
name|buf
index|[
literal|40
index|]
decl_stmt|;
while|while
condition|(
name|already
operator|++
operator|<
name|MAXRETRY
condition|)
block|{
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCFLUSH
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* flush any clutter */
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"\rAT Z\r"
argument_list|,
literal|6
argument_list|)
expr_stmt|;
comment|/* reset modem */
name|bzero
argument_list|(
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|2
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
name|FD
argument_list|,
name|FIONREAD
argument_list|,
operator|&
name|len
argument_list|)
expr_stmt|;
if|#
directive|if
literal|1
if|if
condition|(
name|len
operator|==
literal|0
condition|)
name|len
operator|=
literal|1
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|len
condition|)
block|{
name|len
operator|=
name|read
argument_list|(
name|FD
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|DEBUG
name|buf
index|[
name|len
index|]
operator|=
literal|'\0'
expr_stmt|;
name|printf
argument_list|(
literal|"t3000_sync: (\"%s\")\n\r"
argument_list|,
name|buf
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|index
argument_list|(
name|buf
argument_list|,
literal|'0'
argument_list|)
operator|||
operator|(
name|index
argument_list|(
name|buf
argument_list|,
literal|'O'
argument_list|)
operator|&&
name|index
argument_list|(
name|buf
argument_list|,
literal|'K'
argument_list|)
operator|)
condition|)
return|return
operator|(
literal|1
operator|)
return|;
block|}
comment|/* 		 * If not strapped for DTR control, 		 * try to get command mode. 		 */
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"+++"
argument_list|,
literal|3
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
comment|/* 		 * Toggle DTR to force anyone off that might have left 		 * the modem connected. 		 */
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCCDTR
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
name|FD
argument_list|,
name|TIOCSDTR
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
name|t3000_write
argument_list|(
name|FD
argument_list|,
literal|"\rAT Z\r"
argument_list|,
literal|6
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_macro
name|t3000_write
argument_list|(
argument|fd
argument_list|,
argument|cp
argument_list|,
argument|n
argument_list|)
end_macro

begin_decl_stmt
name|int
name|fd
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|cp
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|n
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|sgttyb
name|sb
decl_stmt|;
ifdef|#
directive|ifdef
name|notdef
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|VERBOSE
argument_list|)
argument_list|)
condition|)
name|write
argument_list|(
literal|1
argument_list|,
name|cp
argument_list|,
name|n
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|ioctl
argument_list|(
name|fd
argument_list|,
name|TIOCGETP
argument_list|,
operator|&
name|sb
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
name|fd
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|sb
argument_list|)
expr_stmt|;
name|t3000_nap
argument_list|()
expr_stmt|;
for|for
control|(
init|;
name|n
operator|--
condition|;
name|cp
operator|++
control|)
block|{
name|write
argument_list|(
name|fd
argument_list|,
name|cp
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
name|fd
argument_list|,
name|TIOCGETP
argument_list|,
operator|&
name|sb
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
name|fd
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|sb
argument_list|)
expr_stmt|;
name|t3000_nap
argument_list|()
expr_stmt|;
block|}
block|}
end_block

begin_ifdef
ifdef|#
directive|ifdef
name|DEBUG
end_ifdef

begin_macro
name|t3000_verbose_read
argument_list|()
end_macro

begin_block
block|{
name|int
name|n
init|=
literal|0
decl_stmt|;
name|char
name|buf
index|[
name|BUFSIZ
index|]
decl_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|FD
argument_list|,
name|FIONREAD
argument_list|,
operator|&
name|n
argument_list|)
operator|<
literal|0
condition|)
return|return;
if|if
condition|(
name|n
operator|<=
literal|0
condition|)
return|return;
if|if
condition|(
name|read
argument_list|(
name|FD
argument_list|,
name|buf
argument_list|,
name|n
argument_list|)
operator|!=
name|n
condition|)
return|return;
name|write
argument_list|(
literal|1
argument_list|,
name|buf
argument_list|,
name|n
argument_list|)
expr_stmt|;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*  * Code stolen from /usr/src/lib/libc/gen/sleep.c  */
end_comment

begin_define
define|#
directive|define
name|mask
parameter_list|(
name|s
parameter_list|)
value|(1<<((s)-1))
end_define

begin_define
define|#
directive|define
name|setvec
parameter_list|(
name|vec
parameter_list|,
name|a
parameter_list|)
define|\
value|vec.sv_handler = a; vec.sv_mask = vec.sv_onstack = 0
end_define

begin_expr_stmt
specifier|static
name|napms
operator|=
literal|50
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* Give the t3000 50 milliseconds between characters */
end_comment

begin_decl_stmt
specifier|static
name|int
name|ringring
decl_stmt|;
end_decl_stmt

begin_macro
name|t3000_nap
argument_list|()
end_macro

begin_block
block|{
specifier|static
name|void
name|t3000_napx
parameter_list|()
function_decl|;
name|int
name|omask
decl_stmt|;
name|struct
name|itimerval
name|itv
decl_stmt|,
name|oitv
decl_stmt|;
specifier|register
name|struct
name|itimerval
modifier|*
name|itp
init|=
operator|&
name|itv
decl_stmt|;
name|struct
name|sigvec
name|vec
decl_stmt|,
name|ovec
decl_stmt|;
name|timerclear
argument_list|(
operator|&
name|itp
operator|->
name|it_interval
argument_list|)
expr_stmt|;
name|timerclear
argument_list|(
operator|&
name|itp
operator|->
name|it_value
argument_list|)
expr_stmt|;
if|if
condition|(
name|setitimer
argument_list|(
name|ITIMER_REAL
argument_list|,
name|itp
argument_list|,
operator|&
name|oitv
argument_list|)
operator|<
literal|0
condition|)
return|return;
name|setvec
argument_list|(
name|ovec
argument_list|,
name|SIG_DFL
argument_list|)
expr_stmt|;
name|omask
operator|=
name|sigblock
argument_list|(
name|mask
argument_list|(
name|SIGALRM
argument_list|)
argument_list|)
expr_stmt|;
name|itp
operator|->
name|it_value
operator|.
name|tv_sec
operator|=
name|napms
operator|/
literal|1000
expr_stmt|;
name|itp
operator|->
name|it_value
operator|.
name|tv_usec
operator|=
operator|(
operator|(
name|napms
operator|%
literal|1000
operator|)
operator|*
literal|1000
operator|)
expr_stmt|;
name|setvec
argument_list|(
name|vec
argument_list|,
name|t3000_napx
argument_list|)
expr_stmt|;
name|ringring
operator|=
literal|0
expr_stmt|;
operator|(
name|void
operator|)
name|sigvec
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|vec
argument_list|,
operator|&
name|ovec
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|setitimer
argument_list|(
name|ITIMER_REAL
argument_list|,
name|itp
argument_list|,
operator|(
expr|struct
name|itimerval
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
while|while
condition|(
operator|!
name|ringring
condition|)
name|sigpause
argument_list|(
name|omask
operator|&
operator|~
name|mask
argument_list|(
name|SIGALRM
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|sigvec
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|ovec
argument_list|,
operator|(
expr|struct
name|sigvec
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|setitimer
argument_list|(
name|ITIMER_REAL
argument_list|,
operator|&
name|oitv
argument_list|,
operator|(
expr|struct
name|itimerval
operator|*
operator|)
literal|0
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|sigsetmask
argument_list|(
name|omask
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
specifier|static
name|void
name|t3000_napx
parameter_list|()
block|{
name|ringring
operator|=
literal|1
expr_stmt|;
block|}
end_function

end_unit

