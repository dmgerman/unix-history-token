...
define|\
value|do { \ 		while ((x).tv_nsec>= PPS_NANOSECOND) { \ 			(x).tv_nsec -= PPS_NANOSECOND; \ 			(x).tv_sec++; \ 		} \ 		while ((x).tv_nsec< 0) { \ 			(x).tv_nsec += PPS_NANOSECOND; \ 			(x).tv_sec--; \ 		} \ 	} while (0)
end_define
...
value|\ 		d_frac = (double)((ntp_fp_t *)&(x))->F.u		\ 			* PPS_NANOSECOND / PPS_FRAC;			\ 		(x).tv_sec = ((ntp_fp_t *)&(x))->I.u			\ 			     - (time_t)PPS_JAN_1970;			\ 		(x).tv_nsec = (long)d_frac;				\ 	} while (0)
end_define
