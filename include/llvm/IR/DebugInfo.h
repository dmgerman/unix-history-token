...
argument_list|(
argument|LLVMContext&VMContext
argument_list|,
...
return|;
block|}
comment|/// Return true if this subprogram is a C++11 reference-qualified
...
return|;
block|}
comment|/// Return true if this subprogram is a C++11
comment|/// rvalue-reference-qualified non-static member function
comment|/// (void foo()&&).
name|unsigned
name|isRValueReference
argument_list|()
...
begin_comment
comment|/// DIUnspecifiedParameter - This is a wrapper for unspecified parameters.
end_comment

begin_decl_stmt
name|class
name|DIUnspecifiedParameter
range|:
name|public
name|DIDescriptor
block|{
...
return|;
block|}
comment|/// \brief Return the size of this variable's complex address or
comment|/// zero if there is none.
name|unsigned
name|getNumAddrElements
argument_list|()
specifier|const
...
name|atSameLineAs
argument_list|(
argument|const DILocation&Other
argument_list|)
specifier|const
block|{
return|return
...
return|;
block|}
comment|/// getDiscriminator - DWARF discriminators are used to distinguish
comment|/// identical file locations for instructions that are on different
comment|/// basic blocks. If two instructions are inside the same lexical block
comment|/// and are in different basic blocks, we create a new lexical block
comment|/// with identical location as the original but with a different
comment|/// discriminator value (lib/Transforms/Util/AddDiscriminators.cpp
comment|/// for details).
name|unsigned
name|getDiscriminator
argument_list|()
...
argument_list|,
argument|DILexicalBlock NewScope
argument_list|)
block|; }
decl_stmt|;
end_decl_stmt
...
return|;
block|}
comment|/// Objective-C doesn't have an ODR, so there is no benefit in storing
