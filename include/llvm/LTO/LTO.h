...
decl_stmt|;
comment|/// An input file. This is a wrapper for ModuleSymbolTable that exposes only the
...
expr|default
expr_stmt|;
comment|// FIXME: Remove the LLVMContext once we have bitcode symbol tables.
name|LLVMContext
name|Ctx
decl_stmt|;
struct_decl|struct
...
decl_stmt|;
comment|/// This is a wrapper for ArrayRef<ModuleSymbolTable::Symbol>::iterator that
comment|/// exposes only the information that an LTO client should need in order to do
comment|/// symbol resolution.
comment|///
comment|/// This object is ephemeral; it is only valid as long as an iterator obtained
comment|/// from symbols() refers to it.
name|class
name|Symbol
block|{
name|friend
...
name|Symbol
argument_list|(
argument|ArrayRef<ModuleSymbolTable::Symbol>::iterator I
argument_list|,
argument|const ModuleSymbolTable&SymTab
argument_list|,
argument|const InputFile *File
argument_list|)
block|:
name|I
argument_list|(
...
argument_list|()
block|;     }
comment|/// Returns the mangled name of the global.
name|StringRef
name|getName
argument_list|()
specifier|const
...
specifier|const
block|{
comment|// FIXME: Expose a thread-local flag for module asm symbols.
return|return
name|isGV
argument_list|()
operator|&&
...
return|;
block|}
comment|// Returns the index of the comdat this symbol is in or -1 if the symbol
comment|// is not in a comdat.
comment|// FIXME: We have to return Expected<int> because aliases point to an
comment|// arbitrary ConstantExpr and that might not actually be a constant. That
comment|// means we might not be able to find what an alias is aliased to and
comment|// so find its comdat.
name|Expected
operator|<
name|int
operator|>
