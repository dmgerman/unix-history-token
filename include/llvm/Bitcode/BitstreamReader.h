...
expr_stmt|;
block|}
comment|/// Reset the stream to the bit pointed at by the specified pointer.
comment|///
comment|/// The pointer must be a dereferenceable pointer into the bytes in the
comment|/// underlying memory object.
name|void
name|jumpToPointer
parameter_list|(
specifier|const
...
name|Pointer
operator|&&
literal|"Expected pointer into bitstream"
argument_list|)
expr_stmt|;
name|JumpToBit
argument_list|(
...
name|Pointer
operator|&&
literal|"Expected to reach pointer"
argument_list|)
expr_stmt|;
block|}
name|void
...
expr_stmt|;
block|}
comment|/// Prevent the cursor from reading past a byte boundary.
comment|///
comment|/// Prevent the cursor from requesting byte reads past \c Limit.  This is
comment|/// useful when working with a cursor on a StreamingMemoryObject, when it's
comment|/// desirable to avoid invalidating the result of getPointerToByte().
comment|///
comment|/// If \c Limit is on a word boundary, AtEndOfStream() will return true if
comment|/// the cursor position reaches or exceeds \c Limit, regardless of the true
comment|/// number of available bytes.  Otherwise, AtEndOfStream() returns true when
comment|/// it reaches or exceeds the next word boundary.
name|void
name|setArtificialByteLimit
parameter_list|(
name|uint64_t
name|Limit
parameter_list|)
...
name|Limit
operator|&&
literal|"Move cursor before lowering limit"
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Only change size if the new one is lower.
if|if
condition|(
operator|!
name|Size
...
expr_stmt|;
block|}
comment|/// Return the Size, if known.
name|uint64_t
name|getSizeIfKnown
argument_list|()
specifier|const
