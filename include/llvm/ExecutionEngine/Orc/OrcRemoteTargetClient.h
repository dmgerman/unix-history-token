...
argument_list|(
argument|std::move(Other.IndirectStubOwnerIds)
argument_list|)
block|{}
name|OrcRemoteTargetClient
operator|&
...
name|CallbackManager
operator|&&
literal|"No calback manager. enableCompileCallbacks must be called first"
argument_list|)
expr_stmt|;
comment|// Check for an 'out-of-band' error, e.g. from an MM destructor.
if|if
...
argument_list|)
return|;
comment|// FIXME: CompileCallback could be an anonymous lambda defined at the use
comment|//        site below, but that triggers a GCC 4.7 ICE. When we move off
comment|//        GCC 4.7, tidy this up.
name|auto
