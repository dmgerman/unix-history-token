...
begin_comment
comment|/* Special version of<a.out.h> for use under hp-ux.    Copyright 1988, 1993, 1995, 2001 Free Software Foundation, Inc. */
end_comment
...
parameter_list|)
define|\
value|((N_MAGIC (x) == OMAGIC || N_MAGIC (x) == HPUX_DOT_O_MAGIC)		\    ? (N_TXTADDR (x) + N_TXTSIZE (x))					\    : (N_SEGSIZE (x) + ((N_TXTADDR (x) + N_TXTSIZE (x) - 1)		\& ~ (bfd_vma) (N_SEGSIZE (x) - 1))))
end_define
