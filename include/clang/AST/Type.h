...
comment|/// QualType - For efficiency, we don't store CVR-qualified types as nodes on
...
comment|///
comment|/// As an added efficiency bonus, instead of making this a pair, we just store
comment|/// the three bits we care about in the low bits of the pointer.  To handle the
comment|/// packing/unpacking, we make QualType be a simple wrapper class that acts like
...
decl|const
decl_stmt|;
comment|/// addConst/addVolatile/addRestrict - add the specified type qual to this
comment|/// QualType.
name|void
name|addConst
parameter_list|()
...
block|}
name|QualType
name|getWithAdditionalQualifiers
argument_list|(
name|unsigned
name|TQs
argument_list|)
...
return|;
block|}
comment|// CVR qualifiers go in low bits.
enum|enum
block|{
name|NumLowBitsAvailable
init|=
...
operator|*
name|getAsTemplateTypeParmType
argument_list|()
specifier|const
expr_stmt|;
specifier|const
...
operator|*
name|getAsTemplateSpecializationType
argument_list|()
specifier|const
expr_stmt|;
...
block|}
empty_stmt|;
comment|/// ExtQualType - TR18037 (C embedded extensions) 6.2.5p26
comment|/// This supports all kinds of type attributes; including,
comment|/// address space qualified types, objective-c's __weak and
comment|/// __strong attributes.
comment|///
name|class
name|ExtQualType
range|:
...
name|FoldingSetNode
block|{
comment|/// BaseType - This is the underlying type that this qualifies.  All CVR
comment|/// qualifiers are stored on the QualType that references this type, so we
comment|/// can't have any here.
name|Type
modifier|*
name|BaseType
decl_stmt|;
comment|/// Address Space ID - The address space ID this type is qualified with.
name|unsigned
name|AddressSpace
decl_stmt|;
comment|/// GC __weak/__strong attributes
name|QualType
operator|::
name|GCAttrTypes
name|GCAttrType
...
argument_list|,
argument|QualType::GCAttrTypes gcAttr
argument_list|)
block|:
name|Type
argument_list|(
...
operator|)
operator|&&
literal|"Cannot have ExtQualType of ExtQualType"
argument_list|)
block|;   }
name|friend
name|class
...
expr|}
block|;
comment|/// ConstantArrayType - This class represents C arrays with a specified constant
comment|/// size.  For example 'int A[100]' has ConstantArrayType where the element type
comment|/// is 'int' and the size is 100.
name|class
...
literal|0
operator|&&
literal|"Cannnot unique DependentSizedArrayTypes."
argument_list|)
block|;   }
block|}
block|;
...
comment|/// "NSString<NSCopyable, NSAmazing>".  Qualified interface types are instances
comment|/// of ObjCQualifiedInterfaceType, which is a subclass of ObjCInterfaceType.
name|class
name|ObjCInterfaceType
operator|:
name|public
...
block|}
comment|/// qual_iterator and friends: this provides access to the (potentially empty)
comment|/// list of protocols qualifying this interface.  If this is an instance of
comment|/// ObjCQualifiedInterfaceType it returns the list, otherwise it returns an
comment|/// empty list if there are no qualifying protocols.
typedef|typedef
name|llvm
operator|::
name|SmallVector
...
expr|}
block|;
comment|/// ObjCQualifiedInterfaceType - This class represents interface types
comment|/// conforming to a list of protocols, such as INTF<Proto1, Proto2, Proto1>.
comment|///
comment|/// Duplicate protocols are removed and protocol list is canonicalized to be in
comment|/// alphabetical order.
name|class
...
name|classof
argument_list|(
argument|const ObjCQualifiedInterfaceType *
argument_list|)
block|{
return|return
name|true
