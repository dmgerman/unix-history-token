...
block|; }
block|;
comment|/// \brief Wrapper base class for a wrapping matcher.
comment|///
comment|/// This is just a container for a DynTypedMatcher that can be used as a base
comment|/// class for another matcher.
name|template
operator|<
name|typename
name|T
...
name|InnerMatcher
argument_list|(
argument|std::move(InnerMatcher)
argument_list|)
block|{}
specifier|const
name|DynTypedMatcher
...
argument_list|,
argument|typename std::enable_if<std::is_base_of<From
argument_list|,
argument|T>::value&&                                   !std::is_same<From
argument_list|,
...
argument_list|,
argument|typename std::enable_if<             std::is_same<T
argument_list|,
argument|QualType>::value&&             std::is_same<TypeT
argument_list|,
...
name|value
argument_list|,
literal|"unsupported class for matcher"
argument_list|)
block|;
name|static_assert
argument_list|(
...
name|value
argument_list|,
literal|"argument type must be StringRef"
argument_list|)
block|;
name|public
operator|:
...
name|value
argument_list|,
literal|"instantiated with wrong types"
argument_list|)
block|;
name|public
operator|:
...
argument|BoundNodesTreeBuilder *Builder
argument_list|,
argument|typename std::enable_if<has_getDecl<U>::value
argument_list|,
argument|int>::type =
literal|0
argument_list|)
...
name|value
argument_list|,
literal|"unsupported type for recursive matching"
argument_list|)
block|;
return|return
name|matchesChildOf
...
comment|///
comment|/// A "linked list" of types, accessible by using the ::head and ::tail
comment|/// typedefs.
name|template
operator|<
name|typename
...
block|{}
expr_stmt|;
comment|// Empty sentinel type list.
name|template
operator|<
name|typename
name|T1
...
comment|///
comment|/// See AST_POLYMORPHIC_SUPPORTED_TYPES for details.
name|template
operator|<
name|class
name|T
...
name|value
argument_list|,
literal|"right polymorphic conversion"
argument_list|)
block|;
return|return
name|Matcher
...
name|value
argument_list|,
literal|"has only accepts base type matcher"
argument_list|)
block|;
name|public
operator|:
...
name|value
argument_list|,
literal|"for each only accepts base type matcher"
argument_list|)
block|;
name|public
operator|:
...
name|Params
argument_list|(
argument|std::forward<Ps>(Params)...
argument_list|)
block|{}
name|template
operator|<
...
name|private
operator|:
comment|// Helper method to unpack the tuple into a vector.
name|template
operator|<
name|typename
name|T
...
name|MaxCount
argument_list|,
literal|"invalid number of parameters for variadic matcher"
argument_list|)
block|;
return|return
name|VariadicOperatorMatcher
...
name|value
argument_list|,
literal|"has descendant only accepts base type matcher"
argument_list|)
block|;
name|public
operator|:
...
name|value
argument_list|,
literal|"has parent only accepts base type matcher"
argument_list|)
block|;
name|public
operator|:
...
name|value
argument_list|,
literal|"has ancestor only accepts base type matcher"
argument_list|)
block|;
name|public
operator|:
...
name|value
argument_list|,
literal|"for each descendant only accepts base type matcher"
argument_list|)
block|;
name|public
operator|:
...
name|value
argument_list|,
literal|"the node must have a getValue method"
argument_list|)
block|;
name|public
operator|:
...
begin_comment
unit|};
comment|/// \brief Template specializations to easily write matchers for floating point
end_comment
...
name|matchesNode
argument_list|(
argument|const FloatingLiteral&Node
argument_list|)
specifier|const
block|{
if|if
...
begin_comment
comment|// FIXME: unify ClassTemplateSpecializationDecl and TemplateSpecializationType's
end_comment

begin_comment
comment|// APIs for accessing the template argument list.
end_comment
...
argument_list|(
argument|const ClassTemplateSpecializationDecl&D
argument_list|)
block|{
return|return
name|D
