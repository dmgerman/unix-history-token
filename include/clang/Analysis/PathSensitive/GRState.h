...
argument_list|)
block|{
comment|// At this point states have already been uniqued.  Just
comment|// add the pointer.
name|profile
operator|.
name|AddPointer
argument_list|(
...
name|CurrentStmt
decl_stmt|;
comment|/// cfg - The CFG for the analyzed function/method.
name|CFG
modifier|&
name|cfg
decl_stmt|;
comment|/// codedecl - The Decl representing the function/method being analyzed.
specifier|const
name|Decl
modifier|&
name|codedecl
...
name|TF
decl_stmt|;
comment|/// Liveness - live-variables information of the ValueDecl* and block-level
comment|/// Expr* in the CFG. Used to get initial store and prune out dead state.
name|LiveVariables
modifier|&
name|Liveness
decl_stmt|;
...
return|;
block|}
comment|// FIXME: Remove when we do lazy initializaton of variable bindings.
comment|//   const GRState* BindVar(const GRState* St, VarDecl* D, SVal V) {
comment|//     return SetSVal(St, getLoc(D), V);
comment|//   }
name|public
label|:
name|GRStateManager
...
parameter_list|)
block|{
comment|// Store manager should return a persistent state.
return|return
name|StoreMgr
operator|->
name|BindDecl
...
return|;
block|}
comment|// Methods that query& manipulate the Environment.
name|SVal
name|GetSVal
parameter_list|(
specifier|const
...
condition|)
block|{
comment|// FIXME: Should this just be an assertion?  When would we want to set
comment|// the value of a block-level expression if it wasn't CurrentStmt?
name|isBlkExpr
operator|=
name|cfg
operator|.
...
parameter_list|)
function_decl|;
comment|// Methods that query or create regions.
name|bool
...
parameter_list|)
block|{
comment|// We only want to do fetches from regions that we can actually bind
comment|// values.  For example, SymbolicRegions of type 'id<...>' cannot
comment|// have direct bindings (but their can be bindings on their subregions).
if|if
condition|(
operator|!
name|R
...
parameter_list|)
function_decl|;
comment|// MakeStateWithStore - get a persistent state with the new store.
specifier|const
name|GRState
modifier|*
name|MakeStateWithStore
parameter_list|(
specifier|const
name|GRState
modifier|*
...
begin_comment
comment|// GRStateRef - A "fat" reference to GRState that also bundles GRStateManager.
end_comment
