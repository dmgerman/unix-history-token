...
begin_comment
comment|/* Remote target system call callback support.    Copyright 1997, 2007 Free Software Foundation, Inc.    Contributed by Cygnus Solutions.  This file is part of GDB.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Needed for enum bfd_endian.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Called by the framework when a read call has emptied a pipe buffer.  */
name|void
argument_list|(
argument|*pipe_empty
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Called by the framework when a write call makes a pipe buffer      non-empty.  */
name|void
argument_list|(
argument|*pipe_nonempty
argument_list|)
name|PARAMS
argument_list|(
operator|(
...
comment|/* ??? We don't have a callback entry for dup, although it is trival to      implement now.  */
name|short
name|fd_buddy
index|[
name|MAX_CALLBACK_FDS
...
index|]
decl_stmt|;
comment|/* 0 = none,>0 = reader (index of writer),<0 = writer (negative index of reader).      If abs (ispipe[N]) == N, then N is an end of a pipe whose other      end is closed.  */
name|short
name|ispipe
index|[
name|MAX_CALLBACK_FDS
index|]
decl_stmt|;
comment|/* A writer stores the buffer at its index.  Consecutive writes      realloc the buffer and add to the size.  The reader indicates the      read part in its .size, until it has consumed it all, at which      point it deallocates the buffer and zeroes out both sizes.  */
struct|struct
name|pipe_write_buffer
block|{
name|int
name|size
decl_stmt|;
...
name|target_endian
decl_stmt|;
comment|/* Size of an "int" on the target (for syscalls whose ABI uses "int").      This must include padding, and only padding-at-higher-address is      supported.  For example, a 64-bit target with 32-bit int:s which      are padded to 64 bits when in an array, should supposedly set this      to 8.  The default is 4 which matches ILP32 targets and 64-bit      targets with 32-bit ints and no padding.  */
name|int
name|target_sizeof_int
decl_stmt|;
...
begin_comment
comment|/* More standard syscalls.  */
end_comment
...
begin_comment
comment|/* Translate a value to target endian.  */
end_comment

begin_decl_stmt
name|void
name|cb_store_target_endian
name|PARAMS
argument_list|(
operator|(
name|host_callback
