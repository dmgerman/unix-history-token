...
begin_comment
comment|/* Checks for validity the macro name in #define, #undef, #ifdef and    #ifndef directives.  IS_DEF_OR_UNDEF is true if this call is    processing a #define or #undefine directive, and false    otherwise.  */
end_comment
...
condition|)
block|{
comment|/* This pragma allows extra tokens after the file name.  */
block|}
elseif|else
if|if
condition|(
...
name|CPP_DL_ERROR
argument_list|,
literal|"unexpected end of file after #line"
argument_list|)
expr_stmt|;
else|else
name|cpp_error
...
condition|)
block|{
comment|/* Unlike #line, there does not seem to be a way to get an EOF 	 here.  So, it should be safe to always spell the token.  */
name|cpp_error
argument_list|(
name|pfile
argument_list|,
...
begin_comment
comment|/* Handle the _Pragma operator.  Return 0 on error, 1 if ok.  */
end_comment
...
begin_comment
comment|/* Like lex_macro_node, but read the input from STR.  */
end_comment
...
begin_comment
comment|/* If STR is a defined macro, return its definition node, else return NULL.  */
end_comment
...
begin_comment
comment|/* Replace a previous definition DFN of the macro STR.  If DFN is NULL,    then the macro should be undefined.  */
end_comment
