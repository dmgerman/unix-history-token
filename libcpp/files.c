...
begin_comment
comment|/* Number of entries to put in a file_hash_entry pool.  */
end_comment

begin_define
define|#
directive|define
name|FILE_HASH_POOL_SIZE
value|127
end_define

begin_comment
comment|/* A file hash entry pool.  We allocate file_hash_entry object from    one of these.  */
end_comment
...
block|{
comment|/* Number of entries used from this pool.  */
name|unsigned
name|int
name|file_hash_entries_used
decl_stmt|;
comment|/* Next pool in the chain; used when freeing.  */
name|struct
name|file_hash_entry_pool
modifier|*
name|next
decl_stmt|;
comment|/* The memory pool.  */
name|struct
name|file_hash_entry
name|pool
index|[
...
condition|)
block|{
comment|/* On most UNIX systems, open succeeds on a directory.  Above,          we check if we have opened a directory and if so, set errno          to ENOENT.  However, on Windows, opening a directory          fails with EACCES.  We want to return ENOENT in that          case too.  */
if|if
condition|(
name|stat
argument_list|(
...
comment|/* The call to stat may have reset errno.  */
name|errno
operator|=
name|EACCES
expr_stmt|;
...
begin_comment
comment|/* Release all the files allocated by this reader.  */
end_comment

begin_function
specifier|static
name|void
name|destroy_all_cpp_files
parameter_list|(
name|cpp_reader
modifier|*
name|pfile
...
begin_comment
comment|/* Free the file hash entry pools.  */
end_comment

begin_function
specifier|static
name|void
name|free_file_hash_entries
parameter_list|(
name|cpp_reader
modifier|*
name|pfile
...
begin_comment
comment|/* Returns TRUE if a file FNAME has ever been successfully opened    before LOCATION.  This routine is not intended to correctly handle    filenames aliased by links or redundant . or .. traversals etc.  */
end_comment
...
begin_comment
comment|/* Make the parser forget about files it has seen.  This can be useful    for resetting the parser to start another run.  */
end_comment
...
begin_struct
struct|struct
name|report_missing_guard_data
block|{
specifier|const
name|char
modifier|*
...
block|}
block|}
comment|/* Keep traversing the hash table.  */
return|return
literal|1
return|;
block|}
...
begin_comment
comment|/* Comparison function for qsort.  */
end_comment

begin_function
specifier|static
name|int
name|report_missing_guard_cmp
parameter_list|(
specifier|const
name|void
modifier|*
...
name|i
decl_stmt|;
comment|/* Sort the paths to avoid outputting them in hash table 	 order.  */
name|qsort
argument_list|(
name|data
operator|.
