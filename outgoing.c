...
literal|1
expr_stmt|;
comment|/* ### we should avoid looking for writability for a while so                    ### that (hopefully) something will appear in the bucket so                    ### we can actually write something. otherwise, we could                    ### end up in a CPU spin: socket wants something, but we                    ### don't have anything (and keep returning EAGAIN)                 */
block|}
elseif|else
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* Setup server authorization headers */
if|if
condition|(
name|ctx
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Setup proxy authorization headers */
if|if
condition|(
name|ctx
operator|->
