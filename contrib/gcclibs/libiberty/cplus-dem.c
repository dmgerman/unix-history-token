...
begin_endif
unit|static int demangle_method_args (struct work_stuff *, const char **, string *);
endif|#
directive|endif
end_endif
...
begin_endif
unit|static int string_empty (string *);
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* This function performs most of what cplus_demangle use to do, but    to be able to demangle a name with a B, K or n code, we need to    have a longer term memory of what types have been seen. The original    now initializes and cleans up the squangle code info, while internal    calls go directly to this routine to avoid resetting that info. */
end_comment
...
begin_endif
unit|static int demangle_method_args (struct work_stuff *work, const char **mangled,                       string *declp) {   int success = 0;    if (work -> static_type)     {       string_append (declp, *mangled + 1);       *mangled += strlen (*mangled);       success = 1;     }   else     {       success = demangle_args (work, mangled, declp);     }   return (success); }
endif|#
directive|endif
end_endif
...
block|{
comment|/* Do not consume a following underscore; 	         consume_count_with_underscores will consume what 	         should be consumed.  */
name|leave_following_underscore
operator|=
literal|1
expr_stmt|;
...
name|b
expr_stmt|;
comment|/* We want to unconditionally demangle parameter types in 	 template parameters.  */
name|hold_options
operator|=
name|work
operator|->
...
begin_endif
unit|static int string_empty (string *s) {   return (s->b == s->p); }
endif|#
directive|endif
end_endif
