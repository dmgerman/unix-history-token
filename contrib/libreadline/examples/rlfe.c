...
name|FMT
parameter_list|)
value|(fprintf(debugfile, FMT), fflush(debugfile))
end_define
...
name|V1
parameter_list|)
value|(fprintf(debugfile, FMT, V1), fflush(debugfile))
end_define
...
name|V2
parameter_list|)
value|(fprintf(debugfile, FMT, V1, V2), fflush(debugfile))
end_define
...
specifier|static
name|int
name|rlfe_directory_completion_hook
name|__P
argument_list|(
operator|(
name|char
...
specifier|static
name|int
name|rlfe_directory_rewrite_hook
name|__P
argument_list|(
operator|(
name|char
...
name|char
modifier|*
name|rlfe_filename_completion_function
name|__P
argument_list|(
operator|(
specifier|const
...
name|stderr
argument_list|,
literal|"%s: usage: %s [-l filename] [-a] [-n appname] [-hv] [command [arguments...]]\n"
argument_list|,
name|progname
argument_list|,
name|progname
...
name|stderr
argument_list|,
literal|"%s: warning: could not open log file %s: %s\n"
argument_list|,
name|progname
argument_list|,
name|logfname
...
name|buf_count
expr_stmt|;
comment|/* Do some minimal carriage return translation and backspace 	     processing before logging the input line. */
if|if
condition|(
name|logfile
condition|)
...
begin_comment
comment|/*  *  * FILENAME COMPLETION FOR RLFE  *  */
end_comment
...
parameter_list|)
define|\
value|((x)[0] == '.'&& (PATHSEP((x)[1]) || \ 			  ((x)[1] == '.'&& PATHSEP((x)[2]))))
end_define
...
name|x
parameter_list|)
value|do { \ 			  s = strdup (x);\ 			  if (s == 0) \ 			    return ((char *)NULL); \ 			} while (0)
end_define

begin_function
specifier|static
name|int
name|get_inferior_cwd
parameter_list|(
name|path
parameter_list|,
name|psize
...
expr_stmt|;
block|}
comment|/* Minimally canonicalize ldirname by removing leading `./' */
for|for
control|(
name|ld
operator|=
...
name|DPRINT1
argument_list|(
literal|"rl_directory_rewrite_hook returns %s\n"
argument_list|,
name|retdir
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Translate *DIRNAMEP to be relative to the inferior's CWD.  Leave a trailing    slash on the result. */
end_comment
...
name|DPRINT1
argument_list|(
literal|"rl_directory_completion_hook returns %s\n"
argument_list|,
name|retdir
argument_list|)
expr_stmt|;
