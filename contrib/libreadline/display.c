...
name|INVIS_FIRST
parameter_list|()
value|(prompt_physical_chars> _rl_screenwidth ? prompt_invis_chars_first_line : wrap_offset)
define|#
directive|define
name|WRAP_OFFSET
parameter_list|(
...
name|offset
parameter_list|)
value|((line == 0) \ 					? (offset ? INVIS_FIRST() : 0) \ 					: ((line == prompt_last_screen_line) ? wrap_offset-prompt_invis_chars_first_line : 0))
define|#
directive|define
name|W_OFFSET
parameter_list|(
...
name|_rl_vis_botlin
decl_stmt|;
comment|/* If col_lendiff is> 0, implying that the new string takes up more 	 screen real estate than the old, but lendiff is< 0, meaning that it 	 takes fewer bytes, we need to just output the characters starting 	 from the first difference.  These will overwrite what is on the 	 display, so there's no reason to do a smart update.  This can really 	 only happen in a multibyte environment. */
if|if
condition|(
name|lendiff
operator|<
...
argument_list|)
expr_stmt|;
comment|/* If nfd begins before any invisible characters in the prompt, 	     adjust _rl_last_c_pos to account for wrap_offset and set 	     cpos_adjusted to let the caller know. */
if|if
condition|(
name|current_line
operator|==
...
condition|)
block|{
comment|/* If nfd begins at the prompt, or before the invisible 		 characters in the prompt, we need to adjust _rl_last_c_pos 		 in a multibyte locale to account for the wrap offset and 		 set cpos_adjusted accordingly. */
name|_rl_output_some_chars
argument_list|(
name|nfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Use NEW when comparing against the last invisible character in the 	 prompt string, since they're both buffer indices and DPOS is a 	 desired display position. */
if|if
condition|(
operator|(
name|new
