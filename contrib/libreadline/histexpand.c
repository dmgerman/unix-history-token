...
index|]
expr_stmt|;
comment|/* The history_comment_char, if set, appearing that the beginning 	     of a word signifies that the rest of the line should not have 	     history expansion performed on it. 	     Skip the rest of the line and break out of the loop. */
if|if
condition|(
name|history_comment_char
operator|&&
...
break|break;
block|}
comment|/* XXX - at some point, might want to extend this to handle 		   double quotes as well. */
elseif|else
if|if
condition|(
name|history_quotes_inhibit_expansion
