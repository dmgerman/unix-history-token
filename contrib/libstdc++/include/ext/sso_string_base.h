begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|// Short-string-optimized versatile string base -*- C++ -*-
end_comment

begin_comment
comment|// Copyright (C) 2005, 2006 Free Software Foundation, Inc.
end_comment

begin_comment
comment|//
end_comment

begin_comment
comment|// This file is part of the GNU ISO C++ Library.  This library is free
end_comment

begin_comment
comment|// software; you can redistribute it and/or modify it under the
end_comment

begin_comment
comment|// terms of the GNU General Public License as published by the
end_comment

begin_comment
comment|// Free Software Foundation; either version 2, or (at your option)
end_comment

begin_comment
comment|// any later version.
end_comment

begin_comment
comment|// This library is distributed in the hope that it will be useful,
end_comment

begin_comment
comment|// but WITHOUT ANY WARRANTY; without even the implied warranty of
end_comment

begin_comment
comment|// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
end_comment

begin_comment
comment|// GNU General Public License for more details.
end_comment

begin_comment
comment|// You should have received a copy of the GNU General Public License along
end_comment

begin_comment
comment|// with this library; see the file COPYING.  If not, write to the Free
end_comment

begin_comment
comment|// Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301,
end_comment

begin_comment
comment|// USA.
end_comment

begin_comment
comment|// As a special exception, you may use this file as part of a free software
end_comment

begin_comment
comment|// library without restriction.  Specifically, if other files instantiate
end_comment

begin_comment
comment|// templates or use macros or inline functions from this file, or you compile
end_comment

begin_comment
comment|// this file and link it with other files to produce an executable, this
end_comment

begin_comment
comment|// file does not by itself cause the resulting executable to be covered by
end_comment

begin_comment
comment|// the GNU General Public License.  This exception does not however
end_comment

begin_comment
comment|// invalidate any other reasons why the executable file might be covered by
end_comment

begin_comment
comment|// the GNU General Public License.
end_comment

begin_comment
comment|/** @file ext/sso_string_base.h  *  This file is a GNU extension to the Standard C++ Library.  *  This is an internal header file, included by other library headers.  *  You should not attempt to use it directly.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|_SSO_STRING_BASE_H
end_ifndef

begin_define
define|#
directive|define
name|_SSO_STRING_BASE_H
value|1
end_define

begin_macro
name|_GLIBCXX_BEGIN_NAMESPACE
argument_list|(
argument|__gnu_cxx
argument_list|)
end_macro

begin_expr_stmt
name|template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|class
name|__sso_string_base
operator|:
name|protected
name|__vstring_utility
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
block|{
name|public
operator|:
typedef|typedef
name|_Traits
name|traits_type
typedef|;
end_expr_stmt

begin_typedef
typedef|typedef
name|typename
name|_Traits
operator|::
name|char_type
name|value_type
expr_stmt|;
end_typedef

begin_typedef
typedef|typedef
name|__vstring_utility
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
name|_Util_Base
expr_stmt|;
end_typedef

begin_typedef
typedef|typedef
name|typename
name|_Util_Base
operator|::
name|_CharT_alloc_type
name|_CharT_alloc_type
expr_stmt|;
end_typedef

begin_typedef
typedef|typedef
name|typename
name|_CharT_alloc_type
operator|::
name|size_type
name|size_type
expr_stmt|;
end_typedef

begin_label
name|private
label|:
end_label

begin_comment
comment|// Data Members:
end_comment

begin_expr_stmt
name|typename
name|_Util_Base
operator|::
name|template
name|_Alloc_hider
operator|<
name|_CharT_alloc_type
operator|>
name|_M_dataplus
expr_stmt|;
end_expr_stmt

begin_decl_stmt
name|size_type
name|_M_string_length
decl_stmt|;
end_decl_stmt

begin_enum
enum|enum
block|{
name|_S_local_capacity
init|=
literal|15
block|}
enum|;
end_enum

begin_union
union|union
block|{
name|_CharT
name|_M_local_data
index|[
name|_S_local_capacity
operator|+
literal|1
index|]
decl_stmt|;
name|size_type
name|_M_allocated_capacity
decl_stmt|;
block|}
union|;
end_union

begin_function
name|void
name|_M_data
parameter_list|(
name|_CharT
modifier|*
name|__p
parameter_list|)
block|{
name|_M_dataplus
operator|.
name|_M_p
operator|=
name|__p
expr_stmt|;
block|}
end_function

begin_function
name|void
name|_M_length
parameter_list|(
name|size_type
name|__length
parameter_list|)
block|{
name|_M_string_length
operator|=
name|__length
expr_stmt|;
block|}
end_function

begin_function
name|void
name|_M_capacity
parameter_list|(
name|size_type
name|__capacity
parameter_list|)
block|{
name|_M_allocated_capacity
operator|=
name|__capacity
expr_stmt|;
block|}
end_function

begin_expr_stmt
name|bool
name|_M_is_local
argument_list|()
specifier|const
block|{
return|return
name|_M_data
argument_list|()
operator|==
name|_M_local_data
return|;
block|}
end_expr_stmt

begin_comment
comment|// Create& Destroy
end_comment

begin_function_decl
name|_CharT
modifier|*
name|_M_create
parameter_list|(
name|size_type
modifier|&
parameter_list|,
name|size_type
parameter_list|)
function_decl|;
end_function_decl

begin_function
name|void
name|_M_dispose
parameter_list|()
block|{
if|if
condition|(
operator|!
name|_M_is_local
argument_list|()
condition|)
name|_M_destroy
argument_list|(
name|_M_allocated_capacity
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|_M_destroy
parameter_list|(
name|size_type
name|__size
parameter_list|)
function|throw
parameter_list|()
block|{
name|_M_get_allocator
argument_list|()
operator|.
name|deallocate
argument_list|(
name|_M_data
argument_list|()
argument_list|,
name|__size
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|// _M_construct_aux is used to implement the 21.3.1 para 15 which
end_comment

begin_comment
comment|// requires special behaviour if _InIterator is an integral type
end_comment

begin_expr_stmt
name|template
operator|<
name|typename
name|_InIterator
operator|>
name|void
name|_M_construct_aux
argument_list|(
argument|_InIterator __beg
argument_list|,
argument|_InIterator __end
argument_list|,
argument|std::__false_type
argument_list|)
block|{
typedef|typedef
name|typename
name|iterator_traits
operator|<
name|_InIterator
operator|>
operator|::
name|iterator_category
name|_Tag
expr_stmt|;
name|_M_construct
argument_list|(
name|__beg
argument_list|,
name|__end
argument_list|,
name|_Tag
argument_list|()
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}        template
operator|<
name|typename
name|_InIterator
operator|>
name|void
name|_M_construct_aux
argument_list|(
argument|_InIterator __beg
argument_list|,
argument|_InIterator __end
argument_list|,
argument|std::__true_type
argument_list|)
block|{
name|_M_construct
argument_list|(
name|static_cast
operator|<
name|size_type
operator|>
operator|(
name|__beg
operator|)
argument_list|,
name|static_cast
operator|<
name|value_type
operator|>
operator|(
name|__end
operator|)
argument_list|)
block|; }
name|template
operator|<
name|typename
name|_InIterator
operator|>
name|void
name|_M_construct
argument_list|(
argument|_InIterator __beg
argument_list|,
argument|_InIterator __end
argument_list|)
block|{
typedef|typedef
name|typename
name|std
operator|::
name|__is_integer
operator|<
name|_InIterator
operator|>
operator|::
name|__type
name|_Integral
expr_stmt|;
name|_M_construct_aux
argument_list|(
name|__beg
argument_list|,
name|__end
argument_list|,
name|_Integral
argument_list|()
argument_list|)
expr_stmt|;
end_expr_stmt

begin_comment
unit|}
comment|// For Input Iterators, used in istreambuf_iterators, etc.
end_comment

begin_expr_stmt
unit|template
operator|<
name|typename
name|_InIterator
operator|>
name|void
name|_M_construct
argument_list|(
argument|_InIterator __beg
argument_list|,
argument|_InIterator __end
argument_list|,
argument|std::input_iterator_tag
argument_list|)
expr_stmt|;
end_expr_stmt

begin_comment
comment|// For forward_iterators up to random_access_iterators, used for
end_comment

begin_comment
comment|// string::iterator, _CharT*, etc.
end_comment

begin_expr_stmt
name|template
operator|<
name|typename
name|_FwdIterator
operator|>
name|void
name|_M_construct
argument_list|(
argument|_FwdIterator __beg
argument_list|,
argument|_FwdIterator __end
argument_list|,
argument|std::forward_iterator_tag
argument_list|)
expr_stmt|;
end_expr_stmt

begin_function_decl
name|void
name|_M_construct
parameter_list|(
name|size_type
name|__req
parameter_list|,
name|_CharT
name|__c
parameter_list|)
function_decl|;
end_function_decl

begin_label
name|public
label|:
end_label

begin_expr_stmt
name|size_type
name|_M_max_size
argument_list|()
specifier|const
block|{
return|return
operator|(
name|_M_get_allocator
argument_list|()
operator|.
name|max_size
argument_list|()
operator|-
literal|1
operator|)
operator|/
literal|2
return|;
block|}
end_expr_stmt

begin_expr_stmt
name|_CharT
operator|*
name|_M_data
argument_list|()
specifier|const
block|{
return|return
name|_M_dataplus
operator|.
name|_M_p
return|;
block|}
end_expr_stmt

begin_expr_stmt
name|size_type
name|_M_length
argument_list|()
specifier|const
block|{
return|return
name|_M_string_length
return|;
block|}
end_expr_stmt

begin_expr_stmt
name|size_type
name|_M_capacity
argument_list|()
specifier|const
block|{
return|return
name|_M_is_local
argument_list|()
operator|?
name|size_type
argument_list|(
name|_S_local_capacity
argument_list|)
operator|:
name|_M_allocated_capacity
return|;
block|}
end_expr_stmt

begin_expr_stmt
name|bool
name|_M_is_shared
argument_list|()
specifier|const
block|{
return|return
name|false
return|;
block|}
end_expr_stmt

begin_function
name|void
name|_M_set_leaked
parameter_list|()
block|{ }
end_function

begin_function
name|void
name|_M_leak
parameter_list|()
block|{ }
end_function

begin_function
name|void
name|_M_set_length
parameter_list|(
name|size_type
name|__n
parameter_list|)
block|{
name|_M_length
argument_list|(
name|__n
argument_list|)
expr_stmt|;
name|traits_type
operator|::
name|assign
argument_list|(
name|_M_data
argument_list|()
index|[
name|__n
index|]
argument_list|,
name|_CharT
argument_list|()
argument_list|)
expr_stmt|;
block|}
end_function

begin_expr_stmt
name|__sso_string_base
argument_list|()
operator|:
name|_M_dataplus
argument_list|(
argument|_Alloc()
argument_list|,
argument|_M_local_data
argument_list|)
block|{
name|_M_set_length
argument_list|(
literal|0
argument_list|)
block|; }
name|__sso_string_base
argument_list|(
specifier|const
name|_Alloc
operator|&
name|__a
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|__sso_string_base
argument_list|(
specifier|const
name|__sso_string_base
operator|&
name|__rcs
argument_list|)
expr_stmt|;
end_expr_stmt

begin_macro
name|__sso_string_base
argument_list|(
argument|size_type __n
argument_list|,
argument|_CharT __c
argument_list|,
argument|const _Alloc& __a
argument_list|)
end_macro

begin_empty_stmt
empty_stmt|;
end_empty_stmt

begin_expr_stmt
name|template
operator|<
name|typename
name|_InputIterator
operator|>
name|__sso_string_base
argument_list|(
argument|_InputIterator __beg
argument_list|,
argument|_InputIterator __end
argument_list|,
argument|const _Alloc& __a
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
operator|~
name|__sso_string_base
argument_list|()
block|{
name|_M_dispose
argument_list|()
block|; }
name|_CharT_alloc_type
operator|&
name|_M_get_allocator
argument_list|()
block|{
return|return
name|_M_dataplus
return|;
block|}
end_expr_stmt

begin_expr_stmt
specifier|const
name|_CharT_alloc_type
operator|&
name|_M_get_allocator
argument_list|()
specifier|const
block|{
return|return
name|_M_dataplus
return|;
block|}
end_expr_stmt

begin_function_decl
name|void
name|_M_swap
parameter_list|(
name|__sso_string_base
modifier|&
name|__rcs
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
name|void
name|_M_assign
parameter_list|(
specifier|const
name|__sso_string_base
modifier|&
name|__rcs
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
name|void
name|_M_reserve
parameter_list|(
name|size_type
name|__res
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
name|void
name|_M_mutate
parameter_list|(
name|size_type
name|__pos
parameter_list|,
name|size_type
name|__len1
parameter_list|,
specifier|const
name|_CharT
modifier|*
name|__s
parameter_list|,
name|size_type
name|__len2
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
name|void
name|_M_erase
parameter_list|(
name|size_type
name|__pos
parameter_list|,
name|size_type
name|__n
parameter_list|)
function_decl|;
end_function_decl

begin_function
name|void
name|_M_clear
parameter_list|()
block|{
name|_M_set_length
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_decl_stmt
name|bool
name|_M_compare
argument_list|(
specifier|const
name|__sso_string_base
operator|&
argument_list|)
decl|const
block|{
return|return
name|false
return|;
block|}
end_decl_stmt

begin_expr_stmt
unit|};
name|template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_swap
argument_list|(
argument|__sso_string_base& __rcs
argument_list|)
block|{
comment|// _GLIBCXX_RESOLVE_LIB_DEFECTS
comment|// 431. Swapping containers with unequal allocators.
name|std
operator|::
name|__alloc_swap
operator|<
name|_CharT_alloc_type
operator|>
operator|::
name|_S_do_it
argument_list|(
name|_M_get_allocator
argument_list|()
argument_list|,
name|__rcs
operator|.
name|_M_get_allocator
argument_list|()
argument_list|)
block|;
if|if
condition|(
name|_M_is_local
argument_list|()
condition|)
if|if
condition|(
name|__rcs
operator|.
name|_M_is_local
argument_list|()
condition|)
block|{
if|if
condition|(
name|_M_length
argument_list|()
operator|&&
name|__rcs
operator|.
name|_M_length
argument_list|()
condition|)
block|{
name|_CharT
name|__tmp_data
index|[
name|_S_local_capacity
operator|+
literal|1
index|]
decl_stmt|;
name|traits_type
operator|::
name|copy
argument_list|(
name|__tmp_data
argument_list|,
name|__rcs
operator|.
name|_M_local_data
argument_list|,
name|_S_local_capacity
operator|+
literal|1
argument_list|)
expr_stmt|;
name|traits_type
operator|::
name|copy
argument_list|(
name|__rcs
operator|.
name|_M_local_data
argument_list|,
name|_M_local_data
argument_list|,
name|_S_local_capacity
operator|+
literal|1
argument_list|)
expr_stmt|;
name|traits_type
operator|::
name|copy
argument_list|(
name|_M_local_data
argument_list|,
name|__tmp_data
argument_list|,
name|_S_local_capacity
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
end_expr_stmt

begin_elseif
elseif|else
if|if
condition|(
name|__rcs
operator|.
name|_M_length
argument_list|()
condition|)
block|{
name|traits_type
operator|::
name|copy
argument_list|(
name|_M_local_data
argument_list|,
name|__rcs
operator|.
name|_M_local_data
argument_list|,
name|_S_local_capacity
operator|+
literal|1
argument_list|)
expr_stmt|;
name|_M_length
argument_list|(
name|__rcs
operator|.
name|_M_length
argument_list|()
argument_list|)
expr_stmt|;
name|__rcs
operator|.
name|_M_set_length
argument_list|(
literal|0
argument_list|)
expr_stmt|;
return|return;
block|}
end_elseif

begin_elseif
elseif|else
if|if
condition|(
name|_M_length
argument_list|()
condition|)
block|{
name|traits_type
operator|::
name|copy
argument_list|(
name|__rcs
operator|.
name|_M_local_data
argument_list|,
name|_M_local_data
argument_list|,
name|_S_local_capacity
operator|+
literal|1
argument_list|)
expr_stmt|;
name|__rcs
operator|.
name|_M_length
argument_list|(
name|_M_length
argument_list|()
argument_list|)
expr_stmt|;
name|_M_set_length
argument_list|(
literal|0
argument_list|)
expr_stmt|;
return|return;
block|}
end_elseif

begin_block
unit|} 	else
block|{
specifier|const
name|size_type
name|__tmp_capacity
init|=
name|__rcs
operator|.
name|_M_allocated_capacity
decl_stmt|;
name|traits_type
operator|::
name|copy
argument_list|(
name|__rcs
operator|.
name|_M_local_data
argument_list|,
name|_M_local_data
argument_list|,
name|_S_local_capacity
operator|+
literal|1
argument_list|)
expr_stmt|;
name|_M_data
argument_list|(
name|__rcs
operator|.
name|_M_data
argument_list|()
argument_list|)
expr_stmt|;
name|__rcs
operator|.
name|_M_data
argument_list|(
name|__rcs
operator|.
name|_M_local_data
argument_list|)
expr_stmt|;
name|_M_capacity
argument_list|(
name|__tmp_capacity
argument_list|)
expr_stmt|;
block|}
end_block

begin_else
else|else
block|{
specifier|const
name|size_type
name|__tmp_capacity
init|=
name|_M_allocated_capacity
decl_stmt|;
if|if
condition|(
name|__rcs
operator|.
name|_M_is_local
argument_list|()
condition|)
block|{
name|traits_type
operator|::
name|copy
argument_list|(
name|_M_local_data
argument_list|,
name|__rcs
operator|.
name|_M_local_data
argument_list|,
name|_S_local_capacity
operator|+
literal|1
argument_list|)
expr_stmt|;
name|__rcs
operator|.
name|_M_data
argument_list|(
name|_M_data
argument_list|()
argument_list|)
expr_stmt|;
name|_M_data
argument_list|(
name|_M_local_data
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|_CharT
modifier|*
name|__tmp_ptr
init|=
name|_M_data
argument_list|()
decl_stmt|;
name|_M_data
argument_list|(
name|__rcs
operator|.
name|_M_data
argument_list|()
argument_list|)
expr_stmt|;
name|__rcs
operator|.
name|_M_data
argument_list|(
name|__tmp_ptr
argument_list|)
expr_stmt|;
name|_M_capacity
argument_list|(
name|__rcs
operator|.
name|_M_allocated_capacity
argument_list|)
expr_stmt|;
block|}
name|__rcs
operator|.
name|_M_capacity
argument_list|(
name|__tmp_capacity
argument_list|)
expr_stmt|;
block|}
end_else

begin_decl_stmt
specifier|const
name|size_type
name|__tmp_length
init|=
name|_M_length
argument_list|()
decl_stmt|;
end_decl_stmt

begin_expr_stmt
name|_M_length
argument_list|(
name|__rcs
operator|.
name|_M_length
argument_list|()
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|__rcs
operator|.
name|_M_length
argument_list|(
name|__tmp_length
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}    template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|_CharT
operator|*
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_create
argument_list|(
argument|size_type& __capacity
argument_list|,
argument|size_type __old_capacity
argument_list|)
block|{
comment|// _GLIBCXX_RESOLVE_LIB_DEFECTS
comment|// 83.  String::npos vs. string::max_size()
if|if
condition|(
name|__capacity
operator|>
name|_M_max_size
argument_list|()
condition|)
name|std
operator|::
name|__throw_length_error
argument_list|(
name|__N
argument_list|(
literal|"__sso_string_base::_M_create"
argument_list|)
argument_list|)
expr_stmt|;
end_expr_stmt

begin_comment
comment|// The below implements an exponential growth policy, necessary to
end_comment

begin_comment
comment|// meet amortized linear time requirements of the library: see
end_comment

begin_comment
comment|// http://gcc.gnu.org/ml/libstdc++/2001-07/msg00085.html.
end_comment

begin_if
if|if
condition|(
name|__capacity
operator|>
name|__old_capacity
operator|&&
name|__capacity
operator|<
literal|2
operator|*
name|__old_capacity
condition|)
block|{
name|__capacity
operator|=
literal|2
operator|*
name|__old_capacity
expr_stmt|;
comment|// Never allocate a string bigger than max_size.
if|if
condition|(
name|__capacity
operator|>
name|_M_max_size
argument_list|()
condition|)
name|__capacity
operator|=
name|_M_max_size
argument_list|()
expr_stmt|;
block|}
end_if

begin_comment
comment|// NB: Need an array of char_type[__capacity], plus a terminating
end_comment

begin_comment
comment|// null char_type() element.
end_comment

begin_return
return|return
name|_M_get_allocator
argument_list|()
operator|.
name|allocate
argument_list|(
name|__capacity
operator|+
literal|1
argument_list|)
return|;
end_return

begin_expr_stmt
unit|}    template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|__sso_string_base
argument_list|(
specifier|const
name|_Alloc
operator|&
name|__a
argument_list|)
operator|:
name|_M_dataplus
argument_list|(
argument|__a
argument_list|,
argument|_M_local_data
argument_list|)
block|{
name|_M_set_length
argument_list|(
literal|0
argument_list|)
block|; }
name|template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|__sso_string_base
argument_list|(
specifier|const
name|__sso_string_base
operator|&
name|__rcs
argument_list|)
operator|:
name|_M_dataplus
argument_list|(
argument|__rcs._M_get_allocator()
argument_list|,
argument|_M_local_data
argument_list|)
block|{
name|_M_construct
argument_list|(
name|__rcs
operator|.
name|_M_data
argument_list|()
argument_list|,
name|__rcs
operator|.
name|_M_data
argument_list|()
operator|+
name|__rcs
operator|.
name|_M_length
argument_list|()
argument_list|)
block|; }
name|template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|__sso_string_base
argument_list|(
argument|size_type __n
argument_list|,
argument|_CharT __c
argument_list|,
argument|const _Alloc& __a
argument_list|)
operator|:
name|_M_dataplus
argument_list|(
argument|__a
argument_list|,
argument|_M_local_data
argument_list|)
block|{
name|_M_construct
argument_list|(
name|__n
argument_list|,
name|__c
argument_list|)
block|; }
name|template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|template
operator|<
name|typename
name|_InputIterator
operator|>
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|__sso_string_base
argument_list|(
argument|_InputIterator __beg
argument_list|,
argument|_InputIterator __end
argument_list|,
argument|const _Alloc& __a
argument_list|)
operator|:
name|_M_dataplus
argument_list|(
argument|__a
argument_list|,
argument|_M_local_data
argument_list|)
block|{
name|_M_construct
argument_list|(
name|__beg
argument_list|,
name|__end
argument_list|)
block|; }
comment|// NB: This is the special case for Input Iterators, used in
comment|// istreambuf_iterators, etc.
comment|// Input Iterators have a cost structure very different from
comment|// pointers, calling for a different coding style.
name|template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|template
operator|<
name|typename
name|_InIterator
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_construct
argument_list|(
argument|_InIterator __beg
argument_list|,
argument|_InIterator __end
argument_list|,
argument|std::input_iterator_tag
argument_list|)
block|{
name|size_type
name|__len
operator|=
literal|0
block|;
name|size_type
name|__capacity
operator|=
name|size_type
argument_list|(
name|_S_local_capacity
argument_list|)
block|;
while|while
condition|(
name|__beg
operator|!=
name|__end
operator|&&
name|__len
operator|<
name|__capacity
condition|)
block|{
name|_M_data
argument_list|()
index|[
name|__len
operator|++
index|]
operator|=
operator|*
name|__beg
expr_stmt|;
operator|++
name|__beg
expr_stmt|;
block|}
name|try
block|{
while|while
condition|(
name|__beg
operator|!=
name|__end
condition|)
block|{
if|if
condition|(
name|__len
operator|==
name|__capacity
condition|)
block|{
comment|// Allocate more space.
name|__capacity
operator|=
name|__len
operator|+
literal|1
expr_stmt|;
name|_CharT
modifier|*
name|__another
init|=
name|_M_create
argument_list|(
name|__capacity
argument_list|,
name|__len
argument_list|)
decl_stmt|;
name|_S_copy
argument_list|(
name|__another
argument_list|,
name|_M_data
argument_list|()
argument_list|,
name|__len
argument_list|)
expr_stmt|;
name|_M_dispose
argument_list|()
expr_stmt|;
name|_M_data
argument_list|(
name|__another
argument_list|)
expr_stmt|;
name|_M_capacity
argument_list|(
name|__capacity
argument_list|)
expr_stmt|;
block|}
name|_M_data
argument_list|()
index|[
name|__len
operator|++
index|]
operator|=
operator|*
name|__beg
expr_stmt|;
end_expr_stmt

begin_expr_stmt
operator|++
name|__beg
expr_stmt|;
end_expr_stmt

begin_macro
unit|} 	  }
name|catch
argument_list|(
argument|...
argument_list|)
end_macro

begin_block
block|{
name|_M_dispose
argument_list|()
expr_stmt|;
name|__throw_exception_again
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|_M_set_length
argument_list|(
name|__len
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}    template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|template
operator|<
name|typename
name|_InIterator
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_construct
argument_list|(
argument|_InIterator __beg
argument_list|,
argument|_InIterator __end
argument_list|,
argument|std::forward_iterator_tag
argument_list|)
block|{
comment|// NB: Not required, but considered best practice.
if|if
condition|(
name|__builtin_expect
argument_list|(
name|_S_is_null_pointer
argument_list|(
name|__beg
argument_list|)
operator|&&
name|__beg
operator|!=
name|__end
argument_list|,
literal|0
argument_list|)
condition|)
name|std
operator|::
name|__throw_logic_error
argument_list|(
name|__N
argument_list|(
literal|"__sso_string_base::"
literal|"_M_construct NULL not valid"
argument_list|)
argument_list|)
expr_stmt|;
name|size_type
name|__dnew
operator|=
name|static_cast
operator|<
name|size_type
operator|>
operator|(
name|std
operator|::
name|distance
argument_list|(
name|__beg
argument_list|,
name|__end
argument_list|)
operator|)
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
name|__dnew
operator|>
name|size_type
argument_list|(
name|_S_local_capacity
argument_list|)
condition|)
block|{
name|_M_data
argument_list|(
name|_M_create
argument_list|(
name|__dnew
argument_list|,
name|size_type
argument_list|(
literal|0
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
name|_M_capacity
argument_list|(
name|__dnew
argument_list|)
expr_stmt|;
block|}
end_if

begin_comment
comment|// Check for out_of_range and length_error exceptions.
end_comment

begin_macro
name|try
end_macro

begin_block
block|{
name|_S_copy_chars
argument_list|(
name|_M_data
argument_list|()
argument_list|,
name|__beg
argument_list|,
name|__end
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|catch
argument_list|(
argument|...
argument_list|)
end_macro

begin_block
block|{
name|_M_dispose
argument_list|()
expr_stmt|;
name|__throw_exception_again
expr_stmt|;
block|}
end_block

begin_expr_stmt
name|_M_set_length
argument_list|(
name|__dnew
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}    template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_construct
argument_list|(
argument|size_type __n
argument_list|,
argument|_CharT __c
argument_list|)
block|{
if|if
condition|(
name|__n
operator|>
name|size_type
argument_list|(
name|_S_local_capacity
argument_list|)
condition|)
block|{
name|_M_data
argument_list|(
name|_M_create
argument_list|(
name|__n
argument_list|,
name|size_type
argument_list|(
literal|0
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
name|_M_capacity
argument_list|(
name|__n
argument_list|)
expr_stmt|;
block|}
end_expr_stmt

begin_if
if|if
condition|(
name|__n
condition|)
name|_S_assign
argument_list|(
name|_M_data
argument_list|()
argument_list|,
name|__n
argument_list|,
name|__c
argument_list|)
expr_stmt|;
end_if

begin_expr_stmt
name|_M_set_length
argument_list|(
name|__n
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}    template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_assign
argument_list|(
argument|const __sso_string_base& __rcs
argument_list|)
block|{
if|if
condition|(
name|this
operator|!=
operator|&
name|__rcs
condition|)
block|{
specifier|const
name|size_type
name|__rsize
init|=
name|__rcs
operator|.
name|_M_length
argument_list|()
decl_stmt|;
specifier|const
name|size_type
name|__capacity
init|=
name|_M_capacity
argument_list|()
decl_stmt|;
if|if
condition|(
name|__rsize
operator|>
name|__capacity
condition|)
block|{
name|size_type
name|__new_capacity
init|=
name|__rsize
decl_stmt|;
name|_CharT
modifier|*
name|__tmp
init|=
name|_M_create
argument_list|(
name|__new_capacity
argument_list|,
name|__capacity
argument_list|)
decl_stmt|;
name|_M_dispose
argument_list|()
expr_stmt|;
name|_M_data
argument_list|(
name|__tmp
argument_list|)
expr_stmt|;
name|_M_capacity
argument_list|(
name|__new_capacity
argument_list|)
expr_stmt|;
block|}
end_expr_stmt

begin_if
if|if
condition|(
name|__rsize
condition|)
name|_S_copy
argument_list|(
name|_M_data
argument_list|()
argument_list|,
name|__rcs
operator|.
name|_M_data
argument_list|()
argument_list|,
name|__rsize
argument_list|)
expr_stmt|;
end_if

begin_expr_stmt
name|_M_set_length
argument_list|(
name|__rsize
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}     }
name|template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_reserve
argument_list|(
argument|size_type __res
argument_list|)
block|{
comment|// Make sure we don't shrink below the current size.
if|if
condition|(
name|__res
operator|<
name|_M_length
argument_list|()
condition|)
name|__res
operator|=
name|_M_length
argument_list|()
expr_stmt|;
specifier|const
name|size_type
name|__capacity
operator|=
name|_M_capacity
argument_list|()
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
name|__res
operator|!=
name|__capacity
condition|)
block|{
if|if
condition|(
name|__res
operator|>
name|__capacity
operator|||
name|__res
operator|>
name|size_type
argument_list|(
name|_S_local_capacity
argument_list|)
condition|)
block|{
name|_CharT
modifier|*
name|__tmp
init|=
name|_M_create
argument_list|(
name|__res
argument_list|,
name|__capacity
argument_list|)
decl_stmt|;
name|_S_copy
argument_list|(
name|__tmp
argument_list|,
name|_M_data
argument_list|()
argument_list|,
name|_M_length
argument_list|()
operator|+
literal|1
argument_list|)
expr_stmt|;
name|_M_dispose
argument_list|()
expr_stmt|;
name|_M_data
argument_list|(
name|__tmp
argument_list|)
expr_stmt|;
name|_M_capacity
argument_list|(
name|__res
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|_M_is_local
argument_list|()
condition|)
block|{
name|_S_copy
argument_list|(
name|_M_local_data
argument_list|,
name|_M_data
argument_list|()
argument_list|,
name|_M_length
argument_list|()
operator|+
literal|1
argument_list|)
expr_stmt|;
name|_M_destroy
argument_list|(
name|__capacity
argument_list|)
expr_stmt|;
name|_M_data
argument_list|(
name|_M_local_data
argument_list|)
expr_stmt|;
block|}
block|}
end_if

begin_expr_stmt
unit|}    template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_mutate
argument_list|(
argument|size_type __pos
argument_list|,
argument|size_type __len1
argument_list|,
argument|const _CharT* __s
argument_list|,
argument|const size_type __len2
argument_list|)
block|{
specifier|const
name|size_type
name|__how_much
operator|=
name|_M_length
argument_list|()
operator|-
name|__pos
operator|-
name|__len1
block|;
name|size_type
name|__new_capacity
operator|=
name|_M_length
argument_list|()
operator|+
name|__len2
operator|-
name|__len1
block|;
name|_CharT
operator|*
name|__r
operator|=
name|_M_create
argument_list|(
name|__new_capacity
argument_list|,
name|_M_capacity
argument_list|()
argument_list|)
block|;
if|if
condition|(
name|__pos
condition|)
name|_S_copy
argument_list|(
name|__r
argument_list|,
name|_M_data
argument_list|()
argument_list|,
name|__pos
argument_list|)
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
name|__s
operator|&&
name|__len2
condition|)
name|_S_copy
argument_list|(
name|__r
operator|+
name|__pos
argument_list|,
name|__s
argument_list|,
name|__len2
argument_list|)
expr_stmt|;
end_if

begin_if
if|if
condition|(
name|__how_much
condition|)
name|_S_copy
argument_list|(
name|__r
operator|+
name|__pos
operator|+
name|__len2
argument_list|,
name|_M_data
argument_list|()
operator|+
name|__pos
operator|+
name|__len1
argument_list|,
name|__how_much
argument_list|)
expr_stmt|;
end_if

begin_expr_stmt
name|_M_dispose
argument_list|()
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|_M_data
argument_list|(
name|__r
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|_M_capacity
argument_list|(
name|__new_capacity
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}    template
operator|<
name|typename
name|_CharT
operator|,
name|typename
name|_Traits
operator|,
name|typename
name|_Alloc
operator|>
name|void
name|__sso_string_base
operator|<
name|_CharT
operator|,
name|_Traits
operator|,
name|_Alloc
operator|>
operator|::
name|_M_erase
argument_list|(
argument|size_type __pos
argument_list|,
argument|size_type __n
argument_list|)
block|{
specifier|const
name|size_type
name|__how_much
operator|=
name|_M_length
argument_list|()
operator|-
name|__pos
operator|-
name|__n
block|;
if|if
condition|(
name|__how_much
operator|&&
name|__n
condition|)
name|_S_move
argument_list|(
name|_M_data
argument_list|()
operator|+
name|__pos
argument_list|,
name|_M_data
argument_list|()
operator|+
name|__pos
operator|+
name|__n
argument_list|,
name|__how_much
argument_list|)
expr_stmt|;
name|_M_set_length
argument_list|(
name|_M_length
argument_list|()
operator|-
name|__n
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
unit|}    template
operator|<
operator|>
specifier|inline
name|bool
name|__sso_string_base
operator|<
name|char
operator|,
name|std
operator|::
name|char_traits
operator|<
name|char
operator|>
operator|,
name|std
operator|::
name|allocator
operator|<
name|char
operator|>
expr|>
operator|::
name|_M_compare
argument_list|(
argument|const __sso_string_base& __rcs
argument_list|)
specifier|const
block|{
if|if
condition|(
name|this
operator|==
operator|&
name|__rcs
condition|)
return|return
name|true
return|;
end_expr_stmt

begin_return
return|return
name|false
return|;
end_return

begin_ifdef
unit|}
ifdef|#
directive|ifdef
name|_GLIBCXX_USE_WCHAR_T
end_ifdef

begin_expr_stmt
unit|template
operator|<
operator|>
specifier|inline
name|bool
name|__sso_string_base
operator|<
name|wchar_t
operator|,
name|std
operator|::
name|char_traits
operator|<
name|wchar_t
operator|>
operator|,
name|std
operator|::
name|allocator
operator|<
name|wchar_t
operator|>
expr|>
operator|::
name|_M_compare
argument_list|(
argument|const __sso_string_base& __rcs
argument_list|)
specifier|const
block|{
if|if
condition|(
name|this
operator|==
operator|&
name|__rcs
condition|)
return|return
name|true
return|;
end_expr_stmt

begin_return
return|return
name|false
return|;
end_return

begin_endif
unit|}
endif|#
directive|endif
end_endif

begin_endif
unit|_GLIBCXX_END_NAMESPACE
endif|#
directive|endif
end_endif

begin_comment
comment|/* _SSO_STRING_BASE_H */
end_comment

end_unit

