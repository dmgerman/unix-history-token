...
directive|if
name|_LIBC
comment|/* Test whether the mode string specifies the conversion.  */
name|cs
operator|=
name|strstr
argument_list|(
...
condition|)
block|{
comment|/* Yep.  Load the appropriate conversions and set the orientation 	 to wide.  */
name|struct
name|gconv_fcts
name|fcts
decl_stmt|;
...
condition|)
block|{
comment|/* Something went wrong, we cannot load the conversion modules. 	       This means we cannot proceed since the user explicitly asked 	       for these.  */
name|_IO_new_fclose
argument_list|(
name|result
argument_list|)
...
expr_stmt|;
block|}
comment|/* Check for backup and repeat */
if|if
condition|(
name|_IO_in_backup
argument_list|(
...
continue|continue;
block|}
comment|/* If we now want less than a buffer, underflow and repeat 	     the copy.  Otherwise, _IO_SYSREAD directly to 	     the user buffer. */
if|if
condition|(
name|fp
operator|->
...
continue|continue;
block|}
comment|/* These must be set before the sysread as we might longjmp out 	     waiting for input. */
name|_IO_setg
argument_list|(
name|fp
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Try to maintain alignment: read a whole number of blocks.  */
name|count
operator|=
name|want
expr_stmt|;
