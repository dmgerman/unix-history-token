...
index|[]
init|=
literal|"$Id: ns_xfr.c,v 8.25 1998/03/25 18:47:34 halley Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * void  * ns_xfr(qsp, znp, zone, class, type, opcode, id)  *	Initiate a concurrent (event driven) outgoing zone transfer.  */
end_comment
...
argument_list|,
literal|"zone transfer of \"%s\" (%s) to %s"
argument_list|,
name|zones
index|[
name|zone
...
begin_comment
comment|/*  * u_char *  * renew_msg(msg)  *	init the header of a message, reset the compression pointers, and  *	reset the write pointer to the first byte following the header.  */
end_comment
...
condition|)
continue|continue;
comment|/* XXXRTH I presume this is still relevant and that 		   this is the right place... */
if|#
directive|if
literal|0
...
condition|)
return|return;
comment|/* 			 * NS RRs other than those at the 			 * zone top are zone cuts. 			 */
if|if
condition|(
name|rrcount
operator|>
