...
index|[]
init|=
literal|"$Id: ns_ixfr.c,v 8.19 2000/04/18 20:47:27 vixie Exp $"
decl_stmt|;
end_decl_stmt
...
argument_list|)
expr_stmt|;
comment|/* 		 * The current SOA has been emited already. 		 * It would be cleaner if the first one was emited here... 		 * 		 * if (sx_addrr(qsp, zp->z_origin, soa_dp)< 0) 		 *	goto cleanup; 		 */
name|qsp
operator|->
name|xfr
operator|.
...
begin_comment
comment|/*  * int ixfr_log_maint(struct zoneinfo *zp, int fast_trim)  *  * zp - pointer to the zone information  * fast_trim  - is used to denote that this is not called on the regular  *              maintaince cycle.  * 		  */
end_comment
