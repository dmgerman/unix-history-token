...
define|\
value|do \ 		{ \
comment|/* Undo effects of setting up null_text. */
...
comment|/* set up null_text again */
value|\ 		} \ 	while ( 0 )
end_define
...
begin_comment
comment|/* yy_hold_char holds the character lost when null_text is formed. */
end_comment
...
begin_comment
comment|/* Flag which is used to allow null_wrap()'s to do buffer switches  * instead of setting up a fresh null_in.  A bit of a hack ...  */
end_comment
...
directive|define
name|YY_FLUSH_BUFFER
value|null__flush_buffer(YY_CURRENT_BUFFER )
end_define

begin_function_decl
name|YY_BUFFER_STATE
name|null__scan_buffer
parameter_list|(
name|char
modifier|*
name|base
...
parameter_list|)
define|\
value|{ \ 	if ( ! YY_CURRENT_BUFFER ){ \         null_ensure_buffer_stack (); \ 		YY_CURRENT_BUFFER_LVALUE =    \             null__create_buffer(null_in,YY_BUF_SIZE ); \ 	} \ 	YY_CURRENT_BUFFER_LVALUE->yy_is_interactive = is_interactive; \ 	}
end_define
...
parameter_list|)
define|\
value|{ \ 	if ( ! YY_CURRENT_BUFFER ){\         null_ensure_buffer_stack (); \ 		YY_CURRENT_BUFFER_LVALUE =    \             null__create_buffer(null_in,YY_BUF_SIZE ); \ 	} \ 	YY_CURRENT_BUFFER_LVALUE->yy_at_bol = at_bol; \ 	}
end_define
...
begin_comment
comment|/* Done after the current pattern has been matched and before the  * corresponding action - sets up null_text.  */
end_comment
...
define|\
value|(yytext_ptr) = yy_bp; \ 	null_leng = (size_t) (yy_cp - yy_bp); \ 	(yy_hold_char) = *yy_cp; \ 	*yy_cp = '\0'; \ 	(yy_c_buf_p) = yy_cp;
end_define
...
directive|define
name|ECHO
value|do { if (fwrite( null_text, null_leng, 1, null_out )) {} } while (0)
end_define
...
parameter_list|)
define|\
value|if ( YY_CURRENT_BUFFER_LVALUE->yy_is_interactive ) \ 		{ \ 		int c = '*'; \ 		size_t n; \ 		for ( n = 0; n< max_size&& \ 			     (c = getc( null_in )) != EOF&& c != '\n'; ++n ) \ 			buf[n] = (char) c; \ 		if ( c == '\n' ) \ 			buf[n++] = (char) c; \ 		if ( c == EOF&& ferror( null_in ) ) \ 			YY_FATAL_ERROR( "input in flex scanner failed" ); \ 		result = n; \ 		} \ 	else \ 		{ \ 		errno=0; \ 		while ( (result = fread(buf, 1, max_size, null_in))==0&& ferror(null_in)) \ 			{ \ 			if( errno != EINTR) \ 				{ \ 				YY_FATAL_ERROR( "input in flex scanner failed" ); \ 				break; \ 				} \ 			errno=0; \ 			clearerr(null_in); \ 			} \ 		}\ \  #endif
end_define
...
begin_comment
comment|/* Code executed at the beginning of each rule, after null_text and null_leng  * have been set up.  */
end_comment
...
condition|)
block|{
comment|/* Note: because we've taken care in 					 * yy_get_next_buffer() to have set up 					 * null_text, we can now set up 					 * yy_c_buf_p so that if some total 					 * hoser (like flex itself) wants to 					 * call the scanner after we return the 					 * YY_NULL, it'll still work - another 					 * YY_NULL will get returned. 					 */
operator|(
name|yy_c_buf_p
operator|)
operator|=
...
parameter_list|)
block|{
comment|/* TODO. We should be able to replace this entire function body 	 * with 	 *		null_pop_buffer_state(); 	 *		null_push_buffer_state(new_buffer);      */
name|null_ensure_buffer_stack
argument_list|()
expr_stmt|;
if|if
...
argument_list|( )
expr_stmt|;
comment|/* We don't actually know whether we did this switch during 	 * EOF (null_wrap()) processing, but the only time this flag 	 * is looked at is after null_wrap() is called, so it's safe 	 * to go ahead and always set it. 	 */
operator|(
name|yy_did_buffer_switch_on_eof
operator|)
operator|=
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in null__create_buffer()"
argument_list|)
expr_stmt|;
name|b
operator|->
...
begin_comment
comment|/** Destroy the buffer.  * @param b a buffer created with null__create_buffer()  *   */
end_comment
...
begin_comment
comment|/* Initializes or reinitializes a buffer.  * This function is sometimes called more than once on the same buffer,  * such as during a null_restart() or at EOF.  */
end_comment
...
literal|1
expr_stmt|;
comment|/* If b is the current buffer, then null__init_buffer was _probably_      * called from null_restart() or through yy_get_next_buffer.      * In that case, we don't want to reset the lineno or column.      */
if|if
condition|(
name|b
operator|!=
...
argument_list|()
expr_stmt|;
comment|/* This block is copied from null__switch_to_buffer. */
if|if
condition|(
name|YY_CURRENT_BUFFER
condition|)
...
name|new_buffer
expr_stmt|;
comment|/* copied from null__switch_to_buffer. */
name|null__load_buffer_state
argument_list|( )
expr_stmt|;
operator|(
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in null_ensure_buffer_stack()"
argument_list|)
expr_stmt|;
name|memset
argument_list|(
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in null__scan_buffer()"
argument_list|)
expr_stmt|;
name|b
operator|->
...
begin_comment
comment|/** Setup the input buffer state to scan a string. The next call to null_lex() will  * scan from a @e copy of @a str.  * @param yystr a NUL-terminated string to scan  *   * @return the newly allocated buffer state object.  * @note If you want to scan bytes that may contain NUL values, then use  *       null__scan_bytes() instead.  */
end_comment
...
begin_comment
comment|/** Setup the input buffer state to scan the given bytes. The next call to null_lex() will  * scan from a @e copy of @a bytes.  * @param yybytes the byte buffer to scan  * @param _yybytes_len the number of bytes in the buffer pointed to by @a bytes.  *   * @return the newly allocated buffer state object.  */
end_comment
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in null__scan_bytes()"
argument_list|)
expr_stmt|;
for|for
control|(
...
name|YY_FATAL_ERROR
argument_list|(
literal|"bad buffer in null__scan_bytes()"
argument_list|)
expr_stmt|;
...
value|do \ 		{ \
comment|/* Undo effects of setting up null_text. */
value|\         int yyless_macro_arg = (n); \         YY_LESS_LINENO(yyless_macro_arg);\ 		null_text[null_leng] = (yy_hold_char); \ 		(yy_c_buf_p) = null_text + yyless_macro_arg; \ 		(yy_hold_char) = *(yy_c_buf_p); \ 		*(yy_c_buf_p) = '\0'; \ 		null_leng = yyless_macro_arg; \ 		} \ 	while ( 0 )
end_define
...
begin_comment
comment|/** Set the input stream. This does not discard the current  * input buffer.  * @param in_str A readable stream.  *   * @see null__switch_to_buffer  */
end_comment
...
parameter_list|)
block|{
comment|/* Initialization is the same as for the non-reentrant scanner.      * This function is called from null_lex_destroy(), so don't allocate here.      */
operator|(
name|yy_buffer_stack
operator|)
operator|=
...
directive|endif
comment|/* For future reference: Set errno on error, since we are called by      * null_lex_init()      */
return|return
literal|0
return|;
block|}
...
begin_comment
comment|/* null_lex_destroy is for both reentrant and non-reentrant scanners. */
end_comment
...
name|NULL
expr_stmt|;
comment|/* Reset the globals. This is important in a non-reentrant scanner so the next time      * null_lex() is called, initialization will occur. */
name|yy_init_globals
argument_list|( )
expr_stmt|;
return|return
...
argument_list|)
expr_stmt|;
comment|/* see null_realloc() for (char *) cast */
block|}
end_function
