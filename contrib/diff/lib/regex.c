...
begin_comment
comment|/* Support for bounded pointers.  */
end_comment
...
parameter_list|)
define|\
value|do {									\     (destination) = *(source)& 0377;					\     (destination) += (signed char) (*((source) + 1))<< 8;		\   } while (0)
end_define
...
name|len
parameter_list|,
comment|/* Correspondences between src(char string) and 		       dest(wchar_t string) for optimization.  E.g.: 		       src  = "xxxyzz" 		       dest = {'X', 'Y', 'Z'} 			 (each "xxx", "y" and "zz" represent one 			  multibyte character corresponding to 'X', 			  'Y' and 'Z'.) 		       offset_buffer = {0, 0+3("xxx"), 0+3+1("y"), 					0+3+1+2("zz")} 				     = {0, 3, 4, 6} */
name|int
modifier|*
name|offset_buffer
parameter_list|,
...
parameter_list|()
define|\
value|do {									\     UCHAR_T *old_buffer = COMPILED_BUFFER_VAR;				\     if (bufp->allocated == MAX_BUF_SIZE)				\       return REG_ESIZE;							\     bufp->allocated<<= 1;						\     if (bufp->allocated> MAX_BUF_SIZE)					\       bufp->allocated = MAX_BUF_SIZE;					\     bufp->buffer = REALLOC (COMPILED_BUFFER_VAR, bufp->allocated);	\     if (COMPILED_BUFFER_VAR == NULL)					\       return REG_ESPACE;						\
comment|/* If the buffer moved, move all the pointers into it.  */
value|\     if (old_buffer != COMPILED_BUFFER_VAR)				\       {									\ 	int incr = COMPILED_BUFFER_VAR - old_buffer;			\ 	MOVE_BUFFER_POINTER (b);					\ 	MOVE_BUFFER_POINTER (begalt);					\ 	if (fixup_alt_jump)						\ 	  MOVE_BUFFER_POINTER (fixup_alt_jump);				\ 	if (laststart)							\ 	  MOVE_BUFFER_POINTER (laststart);				\ 	if (pending_exact)						\ 	  MOVE_BUFFER_POINTER (pending_exact);				\       }									\     ELSE_EXTEND_BUFFER_HIGH_BOUND					\   } while (0)
end_define
...
name|stop
argument_list|,
comment|/* string1 == string2 == NULL means 			    string1/2, size1/2 and mbs_offset1/2 need 			    setting up in this function.  */
comment|/* We need wchar_t * buffers corresponding to 			    cstring1, cstring2.  */
name|wchar_t
operator|*
name|string1
argument_list|,
...
directive|endif
comment|/* Definitions for state transitions.  More efficiently for gcc.  */
ifdef|#
directive|ifdef
name|__GNUC__
if|#
directive|if
name|defined
name|HAVE_SUBTRACT_LOCAL_LABELS
operator|&&
name|defined
name|SHARED
define|#
...
name|NEXT
define|\
value|do								      \ 	{								      \ 	  int offset;							      \ 	  const void *__unbounded ptr;					      \ 	  offset = (p == pend						      \ 		    ? 0 : jmptable[SWITCH_ENUM_CAST ((re_opcode_t) *p++)]);   \ 	  ptr =&&end_of_pattern + offset;				      \ 	  goto *ptr;							      \ 	}								      \       while (0)
define|#
directive|define
name|REF
parameter_list|(
...
parameter_list|)
define|\
value|&&label_##x -&&end_of_pattern
define|#
directive|define
name|JUMP_TABLE_TYPE
value|const int
else|#
directive|else
define|#
...
name|NEXT
define|\
value|do								      \ 	{								      \ 	  const void *__unbounded ptr;					      \ 	  ptr = (p == pend ?&&end_of_pattern				      \ 		 : jmptable[SWITCH_ENUM_CAST ((re_opcode_t) *p++)]);	      \ 	  goto *ptr;							      \ 	}								      \       while (0)
define|#
directive|define
name|REF
parameter_list|(
