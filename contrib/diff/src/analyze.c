...
begin_comment
comment|/* Compare in detail contiguous subsequences of the two files    which are known, as a whole, to match each other.     The results are recorded in the vectors files[N].changed, by    storing 1 in the element for each line that is an insertion or deletion.     The subsequence of file 0 is [XOFF, XLIM) and likewise for file 1.     Note that XLIM, YLIM are exclusive bounds.    All line numbers are origin-0 and discarded lines are not counted.     If FIND_MINIMAL, find a minimal difference no matter how    expensive it is.  */
end_comment
...
name|buffered_lines
decl_stmt|;
comment|/* Note that changedN[len0] does exist, and is 0.  */
name|lin
name|i0
init|=
literal|0
...
name|buffered_lines
decl_stmt|;
comment|/* Note that changedN[-1] does exist, and is 0.  */
while|while
condition|(
name|i0
operator|>=
...
begin_comment
comment|/* If CHANGES, briefly report that two files differed.    Return 2 if trouble, CHANGES otherwise.  */
end_comment
...
begin_comment
comment|/* Report the differences of two files.  */
end_comment
...
block|{
comment|/* Allocate vectors for the results of comparison: 	 a flag for each line of each file, saying whether that line 	 is an insertion or deletion. 	 Allocate an extra element, always 0, at each end of each vector.  */
name|size_t
name|s
init|=
name|cmp
