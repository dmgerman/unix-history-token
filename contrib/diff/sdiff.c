...
begin_comment
comment|/* Return zero if we know that FILE does not exist.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Remove trailing slashes from the directory name.  */
while|while
condition|(
name|dlen
operator|&&
...
control|)
block|{
comment|/* Construct a file name and see if it already exists.  	     We use a single counter in INDEX to cycle each of three 	     character positions through each of 62 possible letters.  */
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"%.*s/dif%.5lu.%c%c%c"
argument_list|,
operator|(
name|int
operator|)
...
expr_stmt|;
block|}
comment|/* Don't free buf; `free' might change errno.  We'll exit soon anyway.  */
return|return
literal|0
return|;
block|}
