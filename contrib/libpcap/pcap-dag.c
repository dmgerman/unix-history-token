...
name|_U_
init|=
literal|"@(#) $Header: /tcpdump/master/libpcap/pcap-dag.c,v 1.21.2.7 2007/06/22 06:43:58 guy Exp $ (LBL)"
decl_stmt|;
end_decl_stmt
...
directive|ifdef
name|TYPE_COLOR_HDLC_POS
comment|/* in this type the color value overwrites the lctr */
case|case
name|TYPE_COLOR_HDLC_POS
case|:
break|break;
...
literal|65536
expr_stmt|;
comment|/* Obey to_ms if supplied. This is a good idea! 	 * Recommend 10-100ms. Calls will time out even if no data arrived. 	 */
name|maxwait
operator|.
name|tv_sec
operator|=
...
block|if (snaplen == 0 || snaplen> MAX_DAG_SNAPLEN) { 		snaplen = MAX_DAG_SNAPLEN; 	} else if (snaplen< MIN_DAG_SNAPLEN) { 		snaplen = MIN_DAG_SNAPLEN; 	}
comment|/* snap len has to be a multiple of 4 */
block|snprintf(conf, 30, "varlen slen=%d", (snaplen + 3)& ~3);   	if(dag_configure(handle->fd, conf)< 0) { 		snprintf(ebuf, PCAP_ERRBUF_SIZE,"dag_configure %s: %s\n", device, pcap_strerror(errno)); 		goto faildetach; 	}
endif|#
directive|endif
ifdef|#
directive|ifdef
...
begin_comment
comment|/*  * Simply submit all possible dag names as candidates.  * pcap_add_if() internally tests each candidate with pcap_open_live(),  * so any non-existent devices are dropped.  * For 2.5 try all rx stream names as well.  */
end_comment
