...
begin_comment
comment|/*	$OpenBSD: authpf.c,v 1.112 2009/01/10 19:08:53 miod Exp $	*/
end_comment
...
begin_comment
comment|/* controls whether $user_ip is set */
end_comment
...
block|}
block|}
comment|/* Make our entry in /var/authpf as ipaddr or username */
name|n
operator|=
name|snprintf
argument_list|(
...
begin_comment
comment|/*  * allowed_luser checks to see if user "luser" is allowed to  * use this gateway by virtue of being listed in an allowed  * users file, namely /etc/authpf/authpf.allow .  * Users may be listed by<username>, %<group>, or @<login_class>.  *  * If /etc/authpf/authpf.allow does not exist, then we assume that  * all users who are allowed in by sshd(8) are permitted to  * use this gateway. If /etc/authpf/authpf.allow does exist, then a  * user must be listed if the connection is to continue, else  * the session terminates in the same manner as being banned.  */
end_comment
...
condition|)
block|{
comment|/* check group membership */
name|int
name|cnt
decl_stmt|;
name|struct
...
name|LOG_ERR
argument_list|,
literal|"invalid group '%s' in %s (%s)"
argument_list|,
name|buf
operator|+
literal|1
...
block|{
comment|/* check username and wildcard */
name|matched
operator|=
name|strcmp
argument_list|(
...
operator|)
return|;
comment|/* matched an allowed user/group */
block|}
name|syslog
argument_list|(
name|LOG_INFO
...
name|LOG_INFO
argument_list|,
literal|"removed %s, user %s - duration %ju seconds"
argument_list|,
name|ip_src
argument_list|,
name|l_user
