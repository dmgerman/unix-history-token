...
case|:
block|{
comment|// This is a bug in patching, so we'll report it once and move on.
specifier|static
name|bool
name|Once
init|=
...
name|Report
argument_list|(
literal|"Internal error: patched an XRay custom event call as a function; "
literal|"func id = %d\n"
argument_list|,
name|FuncId
argument_list|)
...
condition|)
block|{
comment|// This means this is the first CPU this thread has ever run on. We set
comment|// the current CPU and record this as the first TSC we've seen.
name|CurrentCPU
operator|=
name|CPU
expr_stmt|;
...
return|return;
block|}
comment|// In case the reference has been cleaned up before, we make sure we
comment|// initialize it to the provided BufferQueue.
if|if
condition|(
name|LocalBQ
operator|==
