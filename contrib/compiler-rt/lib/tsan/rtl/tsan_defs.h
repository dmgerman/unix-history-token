...
literal|13
decl_stmt|;
comment|// Reduce kMaxTid by kClockCount because one slot in ClockBlock table is
comment|// occupied by reference counter, so total number of elements we can store
comment|// in SyncClock is kClockCount * (kTableSize - 1).
specifier|const
name|unsigned
name|kMaxTid
init|=
