...
name|PROF_WARN
argument_list|(
literal|"Unable to parse filename pattern %s. Using the default name."
argument_list|,
name|FilenamePat
argument_list|)
expr_stmt|;
...
name|DefaultProfileName
expr_stmt|;
comment|/* When -fprofile-instr-generate=<path> is specified on the    * command line, each module will be instrumented with runtime    * init call to __llvm_profile_init function which calls    * __llvm_profile_override_default_filename. In most of the cases,    * the path will be identical, so bypass the parsing completely.    */
if|if
condition|(
name|OldFilenamePat
operator|&&
...
begin_comment
comment|/*  * This API is invoked by the global initializers emitted by Clang/LLVM when  * -fprofile-instr-generate=<..> is specified (vs -fprofile-instr-generate  *  without an argument). This option has lower precedence than the  *  LLVM_PROFILE_FILE environment variable.  */
end_comment
