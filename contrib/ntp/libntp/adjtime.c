...
begin_comment
comment|/*  * Emulate adjtime() using QNX ClockAdjust().  * Chris Burghart<burghart@atd.ucar.edu>, 11/2001  * Miroslaw Pabich<miroslaw_pabich@o2.pl>, 09/2005  *  * This is an implementation of adjtime() for QNX.    * ClockAdjust() is used to tweak the system clock for about  * 1 second period until the desired delta is achieved.  * Time correction slew is limited to reasonable value.  * Internal rounding and relative errors are reduced.  */
end_comment
...
begin_comment
comment|/*  * Time correction slew limit. QNX is a hard real-time system,  * so don't adjust system clock too fast.  */
end_comment

begin_define
define|#
directive|define
name|CORR_SLEW_LIMIT
value|0.02
end_define
...
begin_comment
comment|/*  * Period of system clock adjustment. It should be equal to adjtime  * execution period (1s). If slightly less than 1s (0.95-0.99), then olddelta  * residual error (introduced by execution period jitter) will be reduced.  */
end_comment
...
literal|0
decl_stmt|;
comment|/* 	 * Convert to absolute value for future processing 	 */
if|if
condition|(
name|delta_nsec
operator|<
...
literal|1
return|;
comment|/* 	 * Compute count and nanoseconds increment 	 */
name|count
operator|=
literal|1e9
operator|*
...
expr_stmt|;
block|}
comment|/* 	 * Limit the adjust increment to appropriate value 	 */
name|increment_limit
operator|=
name|CORR_SLEW_LIMIT
operator|*
