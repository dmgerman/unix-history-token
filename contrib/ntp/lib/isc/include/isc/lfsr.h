...
begin_comment
comment|/* $Id: lfsr.h,v 1.17 2007/06/19 23:47:18 tbox Exp $ */
end_comment
...
begin_comment
comment|/*%<  * Initialize an LFSR.  *  * Note:  *  *\li	Putting untrusted values into this function will cause the LFSR to  *	generate (perhaps) non-maximal length sequences.  *  * Requires:  *  *\li	lfsr != NULL  *  *\li	8<= bits<= 32  *  *\li	tap != 0  */
end_comment
...
begin_comment
comment|/*%<  * Returns "count" bytes of data from the LFSR.  *  * Requires:  *  *\li	lfsr be valid.  *  *\li	data != NULL.  *  *\li	count> 0.  */
end_comment
...
begin_comment
comment|/*%<  * Skip "skip" states.  *  * Requires:  *  *\li	lfsr be valid.  */
end_comment
...
begin_comment
comment|/*%<  * Given two LFSRs, use the current state from each to skip entries in the  * other.  The next states are then xor'd together and returned.  *  * WARNING:  *  *\li	This function is used only for very, very low security data, such  *	as DNS message IDs where it is desired to have an unpredictable  *	stream of bytes that are harder to predict than a simple flooding  *	attack.  *  * Notes:  *  *\li	Since the current state from each of the LFSRs is used to skip  *	state in the other, it is important that no state be leaked  *	from either LFSR.  *  * Requires:  *  *\li	lfsr1 and lfsr2 be valid.  *  *\li	1<= skipbits<= 31  */
end_comment
