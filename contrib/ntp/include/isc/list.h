...
begin_comment
comment|/* $Id: list.h,v 1.19 2002/05/09 07:09:30 marka Exp $ */
end_comment
...
parameter_list|)
define|\
value|do { \ 		if ((elt)->link.next != NULL) \ 			(elt)->link.next->link.prev = (elt)->link.prev; \ 		else \ 			(list).tail = (elt)->link.prev; \ 		if ((elt)->link.prev != NULL) \ 			(elt)->link.prev->link.next = (elt)->link.next; \ 		else \ 			(list).head = (elt)->link.next; \ 		(elt)->link.prev = (type *)(-1); \ 		(elt)->link.next = (type *)(-1); \ 	} while (0)
end_define
