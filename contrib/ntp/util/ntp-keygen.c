...
argument_list|)
expr_stmt|;
comment|/* XXX: This modifies the private key and should probably use a 		 * copy of it instead. */
name|rsa
operator|=
name|EVP_PKEY_get0_RSA
argument_list|(
...
return|;
block|}
comment|/* setting 'u' and 'v' into a RSA object takes over ownership. 	 * Since we use these values again, we have to pass in dupes, 	 * or we'll corrupt the program! 	 */
name|RSA_set0_factors
argument_list|(
name|rsa
argument_list|,
...
block|}
block|}
comment|/* 	 * Generate gs[i] = g^a[i] mod p for all i and the generator g. 	 */
for|for
control|(
name|i
operator|=
...
expr_stmt|;
block|}
comment|/* 	 * Verify prod(gs[i]^(a[i] x[j]^i)) = 1 for all i, j. Note the 	 * a[i] x[j]^i exponent is computed mod q, but the gs[i] is 	 * computed mod p. also note the expression given in the paper 	 * is incorrect. 	 */
name|temp
operator|=
literal|1
expr_stmt|;
...
name|stderr
argument_list|,
literal|"Confirm prod(gs[i]^(x[j]^i)) = 1 for all i, j: %s\n"
argument_list|,
name|temp
condition|?
literal|"yes"
