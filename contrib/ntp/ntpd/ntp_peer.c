...
name|statstr
argument_list|,
literal|"unpeer %d flash %x reach %03o flags %04x"
argument_list|,
name|peer_to_remove
operator|->
name|associd
...
return|;
block|}
comment|/* 	 * Here no match has been found, so presumably this is a new 	 * persistent association. Mobilize the thing and initialize its 	 * variables. If emulating ntpdate, force iburst. 	 */
if|if
condition|(
name|mode_ntpdate
condition|)
...
argument_list|)
expr_stmt|;
comment|/* 		 * If it was a multicast packet, findbcastinter() may not 		 * find it, so try a little harder. 		 */
if|if
condition|(
name|peer
operator|->
