...
name|LOG_EMERG
argument_list|,
literal|"cannot register interface with IO engine -- will exit now"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
name|EADDRINUSE
case|:
comment|/* Some systems cannot bind a socket to a broadcast 			 * address, as that is not a valid host address. */
case|case
name|EADDRNOTAVAIL
case|:
ifdef|#
directive|ifdef
name|SYS_WINNT
comment|/*TODO: use for other systems, too? */
comment|/* avoid recurrence here -- if we already have a 			 * regular socket, it's quite useless to try this 			 * again. 			 */
if|if
condition|(
name|interf
operator|->
...
name|LOG_ERR
argument_list|,
literal|"recvfrom(%s) fd=%d: refclock srcadr on a network interface!"
argument_list|,
name|stoa
argument_list|(
operator|&
...
argument_list|,
operator|(
literal|"read_network_packet: fd=%d dropped (refclock srcadr))\n"
operator|,
name|fd
operator|)
argument_list|)
...
directive|ifdef
name|HAVE_SIGNALED_IO
comment|/*  * input_handler - receive packets asynchronously  *  * ALWAYS IN SIGNAL HANDLER CONTEXT -- only async-safe functions allowed!  */
specifier|static
name|RETSIGTYPE
name|input_handler
parameter_list|(
...
directive|endif
comment|/* HAVE_SIGNALED_IO */
comment|/*  * Try to sanitize the global FD set  *  * SIGNAL HANDLER CONTEXT if HAVE_SIGNALED_IO, ordinary userspace otherwise  */
specifier|static
name|int
comment|/*BOOL*/
name|sanitize_fdset
...
name|FALSE
return|;
comment|/* if we have oviously bad FDs, try to sanitize the FD set. */
for|for
control|(
name|j
operator|=
...
return|;
block|}
comment|/*  * scan the known FDs (clocks, servers, ...) for presence in a 'fd_set'.   *  * SIGNAL HANDLER CONTEXT if HAVE_SIGNALED_IO, ordinary userspace otherwise  */
specifier|static
name|void
name|input_handler_scan
parameter_list|(
