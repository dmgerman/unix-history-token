...
begin_comment
comment|/* static  void hopfserial_io		(struct recvbuf *); */
end_comment
...
directive|if
literal|0
block|snprintf(pp->a_lastcode, sizeof(pp->a_lastcode), 		 "STATUS: %1X%1X, DATE: %02d.%02d.%04d  TIME: %02d:%02d:%02d", 		 synch, 		 DoW, 		 day, 		 month, 		 pp->year, 		 pp->hour, 		 pp->minute, 		 pp->second);  	pp->lencode = strlen(pp->a_lastcode); 	if ((synch&& 0xc) == 0 ){
comment|/* time ok? */
