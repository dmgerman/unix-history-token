...
name|LOG_ERR
argument_list|,
literal|"'ttl' does not apply for refclocks"
argument_list|)
expr_stmt|;
name|errflag
operator|=
...
name|LOG_ERR
argument_list|,
literal|"'mode' does not apply for network peers"
argument_list|)
expr_stmt|;
name|errflag
operator|=
...
control|)
block|{
comment|/* accumulate with overflow retention */
name|u
operator|=
operator|(
literal|10
...
name|fatal_error
argument_list|(
literal|"config_auth: attr-token=%d"
argument_list|,
name|my_val
operator|->
name|attr
...
name|val
decl_stmt|;
comment|/* [Bug 2896] For the daemon to work properly it is essential 	 * that minsane< minclock<= maxclock. 	 * 	 * If either constraint is violated, the daemon will be or might 	 * become dysfunctional. Fixing the values is too fragile here, 	 * since three variables with interdependecies are involved. We 	 * just log an error but do not stop: This might be caused by 	 * remote config, and it might be fixed by remote config, too. 	 */
name|int
name|l_maxclock
init|=
name|sys_maxclock
...
name|sys_minsane
decl_stmt|;
comment|/* -*- phase one: inspect / sanitize the values */
name|tos
operator|=
name|HEAD_PFIFO
argument_list|(
...
name|LOG_WARNING
argument_list|,
literal|"Using maximum bcpollbstep ceiling %d, %d requested"
argument_list|,
literal|4
argument_list|,
operator|(
...
name|LOG_WARNING
argument_list|,
literal|"Using minimum bcpollbstep floor %d, %d requested"
argument_list|,
literal|0
argument_list|,
operator|(
...
name|LOG_WARNING
argument_list|,
literal|"Using maximum tos ceiling %d, %d requested"
argument_list|,
name|STRATUM_UNSPEC
operator|-
literal|1
...
name|LOG_WARNING
argument_list|,
literal|"Using minimum tos floor %d, %d requested"
argument_list|,
literal|1
argument_list|,
operator|(
...
name|LOG_ERR
argument_list|,
literal|"tos error: must have minsane (%d)< minclock (%d)<= maxclock (%d)"
literal|" - daemon will not operate properly!"
argument_list|,
name|l_minsane
argument_list|,
name|l_minclock
...
expr_stmt|;
block|}
comment|/* -*- phase two: forward the values to the protocol machinery */
name|tos
operator|=
name|HEAD_PFIFO
argument_list|(
...
name|fatal_error
argument_list|(
literal|"config-tos: attr-token=%d"
argument_list|,
name|tos
operator|->
name|attr
...
name|fatal_error
argument_list|(
literal|"config-monitor: type-token=%d"
argument_list|,
name|my_opts
operator|->
name|value
...
name|fatal_error
argument_list|(
literal|"config-access: flag-type-token=%d"
argument_list|,
name|curr_flag
operator|->
name|i
...
name|fatal_error
argument_list|(
literal|"config-rlimit: value-token=%d"
argument_list|,
name|rlimit_av
operator|->
name|attr
...
name|fatal_error
argument_list|(
literal|"config_tinker: attr-token=%d"
argument_list|,
name|tinker
operator|->
name|attr
...
name|fatal_error
argument_list|(
literal|"config_nic_rules: match-class-token=%d"
argument_list|,
name|curr_node
operator|->
name|match_class
...
name|fatal_error
argument_list|(
literal|"config_nic_rules: action-token=%d"
argument_list|,
name|curr_node
operator|->
name|action
...
name|fatal_error
argument_list|(
literal|"config-logconfig: modifier='%c'"
argument_list|,
name|my_lc
operator|->
name|attr
...
name|fatal_error
argument_list|(
literal|"peerflag_bits: option-token=%d"
argument_list|,
name|option
operator|->
name|value
...
control|)
block|{
comment|/* 		 * If we have no address attached, assume we have to 		 * unpeer by AssocID. 		 */
if|if
condition|(
operator|!
name|curr_unpeer
...
argument_list|()
expr_stmt|;
comment|/* [bug 2837] dep. on config_vars() */
name|config_trap
argument_list|(
name|ptree
argument_list|)
expr_stmt|;
comment|/* [bug 2923] dep. on io_open_sockets() */
name|config_other_modes
argument_list|(
name|ptree
argument_list|)
...
begin_comment
comment|/* Hack to disambiguate 'server' statements for refclocks and network peers.  * Please note the qualification 'hack'. It's just that.  */
end_comment
...
name|fatal_error
argument_list|(
literal|"ntp_rlimit: unexpected RLIMIT case: %d"
argument_list|,
name|rl_what
argument_list|)
expr_stmt|;
