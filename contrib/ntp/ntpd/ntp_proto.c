...
begin_comment
comment|/* [Bug 3031] define automatic broadcastdelay cutoff preset */
end_comment
...
name|INVALIDNAK
block|,
comment|/* NAK cannot be used */
name|VALIDNAK
comment|/* NAK is valid */
block|}
enum|;
end_enum
...
begin_comment
comment|/* system leap indicator, use set_sys_leap() to change this */
end_comment
...
begin_comment
comment|/* leap indicator sent in client requests, set up by set_sys_leap() */
end_comment
...
begin_comment
comment|/* bad crypto (TEST9) */
end_comment

begin_decl_stmt
name|int
name|unpeer_crypto_nak_early
init|=
literal|1
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* crypto_NAK (TEST5) */
end_comment

begin_decl_stmt
name|int
name|unpeer_digest_early
init|=
literal|1
decl_stmt|;
end_decl_stmt
...
name|sys_leap
expr_stmt|;
comment|/* 	 * Under certain conditions we send faked leap bits to clients, so 	 * eventually change xmt_leap below, but never change LEAP_NOTINSYNC. 	 */
if|if
condition|(
name|xmt_leap
operator|!=
...
block|{
comment|/* 			 * If leap smear is enabled in general we must 			 * never send a leap second warning to clients, 			 * so make sure we only send "in sync". 			 */
if|if
condition|(
name|leap_smear
operator|.
...
begin_comment
comment|/*   * Check that NAK is valid  */
end_comment
...
name|base_packet_length
expr_stmt|;
comment|/* 	 * Is this a potential NAK? 	 */
if|if
condition|(
name|remainder_size
operator|!=
...
return|;
block|}
comment|/* 	 * Only server responses can contain NAK's 	 */
if|if
condition|(
name|hismode
operator|!=
...
name|hm_str
decl_stmt|;
comment|/* hismode string */
specifier|const
name|char
modifier|*
name|am_str
decl_stmt|;
comment|/* association match string */
name|int
name|kissCode
init|=
name|NOKISS
...
name|int
name|crypto_nak_test
decl_stmt|;
comment|/* result of crypto-NAK check */
name|int
name|retcode
init|=
name|AM_NOMATCH
...
argument_list|,
operator|(
literal|"receive: at %ld %s<-%s flags %x restrict %03x org %#010x.%08x xmt %#010x.%08x\n"
operator|,
name|current_time
operator|,
name|stoa
...
argument_list|)
expr_stmt|;
comment|/* 	 * Drop any invalid crypto-NAKs 	 */
if|if
condition|(
name|crypto_nak_test
operator|==
...
name|LOG_ERR
argument_list|,
literal|"Invalid-NAK error at %ld %s<-%s"
argument_list|,
name|current_time
argument_list|,
name|stoa
...
argument_list|,
operator|(
literal|"receive: at %ld %s<-%s mode %d/%s:%s len %d org %#010x.%08x xmt %#010x.%08x NOMAC\n"
operator|,
name|current_time
operator|,
name|stoa
...
argument_list|,
operator|(
literal|"receive: at %ld %s<-%s mode %d/%s:%s keyid %08x len %d auth %d org %#010x.%08x xmt %#010x.%08x MAC4\n"
operator|,
name|current_time
operator|,
name|stoa
...
argument_list|,
operator|(
literal|"receive: at %ld %s<-%s mode %d/%s:%s keyid %08x len %d auth %d org %#010x.%08x xmt %#010x.%08x\n"
operator|,
name|current_time
operator|,
name|stoa
...
literal|" with unknown peer %s mode %d/%s:%s keyid %08x len %d auth %d\n"
operator|,
name|current_time
operator|,
name|stoa
...
argument_list|,
operator|(
literal|"receive: PROCPKT/BROADCAST: prev pkt %ld seconds ago, ppoll: %d, %d secs\n"
operator|,
operator|(
name|current_time
operator|-
...
argument_list|)
expr_stmt|;
comment|/* Things we can check: 			 * 			 * Did the poll interval change? 			 * Is the poll interval in the packet in-range? 			 * Did this packet arrive too soon? 			 * Is the timestamp in this packet monotonic 			 *  with respect to the previous packet? 			 */
comment|/* This is noteworthy, not error-worthy */
if|if
condition|(
name|pkt
operator|->
...
name|LOG_INFO
argument_list|,
literal|"receive: broadcast poll from %s changed from %ud to %ud"
argument_list|,
name|stoa
argument_list|(
operator|&
...
name|LOG_INFO
argument_list|,
literal|"receive: broadcast poll of %ud from %s is out-of-range (%d to %d)!"
argument_list|,
name|pkt
operator|->
name|ppoll
...
literal|3
expr_stmt|;
comment|/* allow greater fuzz after volley */
if|if
condition|(
operator|(
name|current_time
...
name|LOG_INFO
argument_list|,
literal|"receive: broadcast packet from %s arrived after %lu, not %lu seconds!"
argument_list|,
name|stoa
argument_list|(
operator|&
...
name|LOG_INFO
argument_list|,
literal|"receive: broadcast packet from %s contains non-monotonic timestamp: %#010x.%08x -> %#010x.%08x"
argument_list|,
name|stoa
argument_list|(
operator|&
...
expr_stmt|;
block|}
comment|/* 	 * Next comes a rigorous schedule of timestamp checking. If the 	 * transmit timestamp is zero, the server has not initialized in 	 * interleaved modes or is horribly broken. 	 * 	 * A KoD packet we pay attention to cannot have a 0 transmit 	 * timestamp. 	 */
if|if
condition|(
name|L_ISZERO
argument_list|(
...
operator|++
expr_stmt|;
comment|/* for TEST2 or TEST3 */
name|msyslog
argument_list|(
name|LOG_INFO
argument_list|,
literal|"receive: Unexpected zero transmit timestamp in KoD from %s"
argument_list|,
name|ntoa
argument_list|(
operator|&
...
return|return;
block|}
comment|/* 	 * If the transmit timestamp duplicates our previous one, the 	 * packet is a replay. This prevents the bad guys from replaying 	 * the most recent packet, authenticated or not. 	 */
block|}
elseif|else
if|if
condition|(
...
name|p_org
argument_list|)
comment|/* We checked p_xmt above */
operator|||
name|L_ISZERO
argument_list|(
operator|&
...
name|LOG_INFO
argument_list|,
literal|"receive: KoD packet from %s has a zero org or rec timestamp.  Ignoring."
argument_list|,
name|ntoa
argument_list|(
operator|&
...
name|LOG_INFO
argument_list|,
literal|"receive: KoD packet from %s has inconsistent xmt/org/rec timestamps.  Ignoring."
argument_list|,
name|ntoa
argument_list|(
operator|&
...
name|LOG_INFO
argument_list|,
literal|"receive: flip 0 KoD origin timestamp %#010x.%08x from %s does not match %#010x.%08x - ignoring."
argument_list|,
name|p_org
operator|.
name|l_ui
...
name|LOG_INFO
argument_list|,
literal|"receive: flip 1 KoD origin timestamp %#010x.%08x from %s does not match interleave %#010x.%08x - ignoring."
argument_list|,
name|p_org
operator|.
name|l_ui
...
condition|)
block|{
comment|/* are there cases here where we should bail? */
comment|/* Should we set TEST2 if we decide to try xleave? */
name|peer
operator|->
name|bogusorg
operator|++
...
name|LOG_INFO
argument_list|,
literal|"receive: Unexpected origin timestamp %#010x.%08x does not match aorg %#010x.%08x from %s@%s xmt %#010x.%08x"
argument_list|,
name|ntohl
argument_list|(
name|pkt
...
condition|)
block|{
comment|/* Might be the start of an interleave */
if|if
condition|(
name|dynamic_interleave
condition|)
...
name|LOG_INFO
argument_list|,
literal|"receive: Dynamic interleave from %s@%s denied"
argument_list|,
name|hm_str
argument_list|,
name|ntoa
...
directive|if
literal|0
comment|/* At this point, MODE_CONTROL is overloaded by MODE_BCLIENT */
block|case MODE_CONTROL:
...
name|LOG_INFO
argument_list|,
literal|"receive: Unexpected mode (%d) in packet from %s"
argument_list|,
name|hismode
argument_list|,
name|ntoa
...
condition|)
block|{
comment|/* drop all if in the initial volley */
if|if
condition|(
name|FLAG_BC_VOL
operator|&
...
expr_stmt|;
block|}
comment|/* 		 * When calibration is complete and the clock is 		 * synchronized, the bias is calculated as the difference 		 * between the unicast timestamp and the broadcast 		 * timestamp. This works for both basic and interleaved 		 * modes. 		 * [Bug 3031] Don't keep this peer when the delay  		 * calculation gives reason to suspect clock steps. 		 * This is assumed for delays> 50ms. 		 */
if|if
condition|(
name|FLAG_BC_VOL
operator|&
...
argument_list|,
operator|(
literal|"broadcast volley: initial delay=%.6f\n"
operator|,
name|peer
operator|->
name|delay
...
literal|"%s"
operator|,
literal|"broadcast volley: initial delay exceeds limit\n"
operator|)
argument_list|)
expr_stmt|;
name|unpeer
...
argument_list|,
operator|(
literal|"peer_xmit: at %ld %s->%s mode %d len %zu xmt %#010x.%08x\n"
operator|,
name|current_time
operator|,
name|peer
...
name|LOG_ERR
argument_list|,
literal|"peer_xmit: buffer overflow %zu"
argument_list|,
name|sendlen
argument_list|)
expr_stmt|;
...
argument_list|,
operator|(
literal|"peer_xmit: at %ld %s->%s mode %d keyid %08x len %zu index %d\n"
operator|,
name|current_time
operator|,
name|latoa
...
directive|ifdef
name|LEAP_SMEAR
comment|/* 		 * Make copies of the variables which can be affected by smearing. 		 */
name|l_fp
name|this_ref_time
decl_stmt|;
name|l_fp
...
directive|endif
comment|/* 		 * If we are inside the leap smear interval we add the current smear offset to 		 * the packet receive time, to the packet transmit time, and eventually to the 		 * reftime to make sure the reftime isn't later than the transmit/receive times. 		 */
name|xpkt
operator|.
name|li_vn_mode
operator|=
...
argument_list|,
operator|(
literal|"fast_xmit: leap_smear.in_progress: refid %8x, smear %s\n"
operator|,
name|ntohl
argument_list|(
name|xpkt
...
argument_list|,
operator|(
literal|"fast_xmit: at %ld %s->%s mode %d len %lu\n"
operator|,
name|current_time
operator|,
name|stoa
...
argument_list|,
operator|(
literal|"fast_xmit: at %ld %s->%s mode %d keyid %08x len %lu\n"
operator|,
name|current_time
operator|,
name|ntoa
...
argument_list|,
operator|(
literal|"pool_xmit: at %ld %s->%s pool\n"
operator|,
name|current_time
operator|,
name|latoa
...
name|LOG_WARNING
argument_list|,
literal|"restrict: 'monitor' cannot be disabled while 'limited' is enabled"
argument_list|)
expr_stmt|;
block|}
break|break;
