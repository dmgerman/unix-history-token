...
name|LOG_INFO
argument_list|,
literal|"receive: broadcast packet from %s arrived after %ld, not %d seconds!"
argument_list|,
name|stoa
argument_list|(
operator|&
...
name|LOG_INFO
argument_list|,
literal|"receive: Unexpected origin timestamp %#010x.%08x from %s xmt %#010x.%08x"
argument_list|,
name|ntohl
argument_list|(
name|pkt
...
block|}
return|return;
comment|/* Bogus or possible interleave packet */
block|}
else|else
block|{
name|L_CLR
...
block|}
comment|/* 	 * If: 	 *	- this is a *cast (uni-, broad-, or m-) server packet 	 *	- and it's authenticated 	 * then see if the sender's IP is trusted for this keyid. 	 * If it is, great - nothing special to do here. 	 * Otherwise, we should report and bail. 	 */
switch|switch
condition|(
name|hismode
condition|)
