...
begin_comment
comment|/*  * Set the processing not to be in the forground  */
end_comment
...
directive|ifdef
name|HAVE_CLOCKCTL
comment|/*  	 * Drop super-user privileges and chroot now if the OS supports 	 * non root clock control (only NetBSD for now). 	 */
if|if
condition|(
name|user
operator|!=
...
name|LOG_ERR
argument_list|,
literal|"ntpdc: WaitForMultipleObjectsEx Failed: Error: %m"
argument_list|)
expr_stmt|;
break|break;
comment|/* For now do nothing if not expected */
default|default:
break|break;
block|}
comment|/* switch */
