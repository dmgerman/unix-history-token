...
directive|endif
comment|/* check mode value: 0<= hmode<= 6 		 * 		 * There's no good global define for that limit, and 		 * using a magic define is as good (or bad, actually) as 		 * a magic number. So we use the highest possible peer 		 * mode, and that is MODE_BCLIENT. 		 * 		 * [Bug 3009] claims that a problem occurs for hmode> 7, 		 * but the code in ntp_peer.c indicates trouble for any 		 * hmode> 6 ( --> MODE_BCLIENT). 		 */
if|if
condition|(
name|temp_cp
operator|.
...
return|return;
block|}
comment|/* Any more checks on the values? Unchecked at this 		 * point: 		 *   - version 		 *   - ttl 		 *   - keyid 		 * 		 *   - minpoll/maxpoll, but they are treated properly 		 *     for all cases internally. Checking not necessary. 		 */
comment|/* finally create the peer */
if|if
condition|(
name|peer_config
argument_list|(
...
return|return;
block|}
comment|/* now do two runs: first a dry run, then a busy one */
for|for
control|(
name|loops
operator|=
...
condition|)
block|{
comment|/* copy from request to local */
name|ZERO
argument_list|(
name|temp_cp
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* search for matching configred(!) peer */
name|p
operator|=
name|NULL
expr_stmt|;
...
condition|)
block|{
comment|/* Item not found in dry run -- bail! */
name|req_ack
argument_list|(
name|srcadr
argument_list|,
...
condition|)
block|{
comment|/* Item found in busy run -- remove! */
name|peer_clear
argument_list|(
name|p
argument_list|,
...
begin_comment
comment|/* There have been some issues with the restrict list processing,  * ranging from problems with deep recursion (resulting in stack  * overflows) and overfull reply buffers.  *  * To avoid this trouble the list reversal is done iteratively using a  * scratch pad.  */
end_comment
...
condition|)
block|{
comment|/* need another sheet in the scratch pad */
name|sp
operator|=
name|emalloc
argument_list|(
...
condition|)
block|{
comment|/* discard sheet from scratch pad */
operator|*
name|spp
operator|=
name|sp
...
begin_comment
comment|/*  * list_restrict4 - iterative helper for list_restrict dumps IPv4  *		    restriction list in reverse order.  */
end_comment
...
begin_comment
comment|/*  * list_restrict6 - iterative helper for list_restrict dumps IPv6  *		    restriction list in reverse order.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * The restriction lists are kept sorted in the reverse order 	 * than they were originally.  To preserve the output semantics, 	 * dump each list in reverse order. The workers take care of that. 	 */
name|list_restrict4
argument_list|(
name|restrictlist4
argument_list|,
...
begin_comment
comment|/*  * Validate a request packet for a new request or control key:  *  - only one item allowed  *  - key must be valid (that is, known, and not in the autokey range)  */
end_comment

begin_function
specifier|static
name|void
name|set_keyid_checked
parameter_list|(
name|keyid_t
modifier|*
name|into
...
name|LOG_ERR
argument_list|,
literal|"set_keyid_checked[%s]: err_nitems> 1"
argument_list|,
name|what
argument_list|)
expr_stmt|;
...
return|return;
block|}
comment|/* plug the new key from the packet */
name|pkeyid
operator|=
operator|(
name|keyid_t
...
argument_list|)
expr_stmt|;
comment|/* validate the new key id, claim data error on failure */
if|if
condition|(
name|tmpkey
operator|<
...
name|LOG_ERR
argument_list|,
literal|"set_keyid_checked[%s]: invalid key id: %ld"
argument_list|,
name|what
argument_list|,
operator|(
...
return|return;
block|}
comment|/* if we arrive here, the key is good -- use it */
operator|*
name|into
operator|=
name|tmpkey
