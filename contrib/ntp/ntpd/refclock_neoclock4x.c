...
begin_comment
comment|/*  *  * refclock_neoclock4x.c  * - NeoClock4X driver for DCF77 or FIA Timecode  *  * Date: 2002-04-27 1.0  *  * see http://www.linum.com/redir/jump/id=neoclock4x&action=redir  * for details about the NeoClock4X device  *  * Copyright (C) 2002 by Linum Software GmbH<support@linum.com>  *                                      * This program is distributed in the hope that it will be useful,  * but WITHOUT ANY WARRANTY; without even the implied warranty of  * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  *  *  */
end_comment
...
name|LOG_CRIT
argument_list|,
literal|"NeoClock4X(%d): can't query serial port settings: %m"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
block|}
comment|/* 2400 Baud mit 8N2 */
name|termsettings
operator|.
name|c_cflag
operator|&=
...
name|LOG_CRIT
argument_list|,
literal|"NeoClock4X(%d): can't set serial port to 2400 8N2: %m"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|LOG_EMERG
argument_list|,
literal|"NeoClock4X(%d): OS interface is incapable of setting DTR/RTS to power NeoClock4X"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|fd
expr_stmt|;
comment|/* no time is given by user! use 169.583333 ms to compensate the serial line delay    * formula is:    * 2400 Baud / 11 bit = 218.18 charaters per second    *  (NeoClock4X timecode len)    */
name|pp
operator|->
name|fudgetime1
operator|=
...
control|)
block|{
comment|/*        * Wait 3 second for receiver to power up        */
name|NLOG
argument_list|(
...
name|LOG_INFO
argument_list|,
literal|"NeoClock4X(%d): try query NeoClock4X firmware version (%d/5)"
argument_list|,
name|unit
argument_list|,
name|tries
...
name|LOG_DEBUG
argument_list|,
literal|"NeoClock4X(%d): calculated UTC date/time: %04d-%02d-%02d %02d:%02d:%02d.%03d"
argument_list|,
name|up
operator|->
name|unit
...
continue|continue;
block|}
comment|//msyslog(LOG_NOTICE, "NeoClock4X(%d): firmware %c = %02Xh", unit, c, c);
if|if
condition|(
literal|0x0A
operator|==
...
begin_comment
comment|/*  * History:  * refclock_neoclock4x.c  *  * 2002/04/27 cjh  * Revision 1.0  first release  *  * 2002/0715 cjh  * preparing for bitkeeper reposity  *  */
end_comment
