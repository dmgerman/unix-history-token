...
begin_comment
comment|/*  * save_config - Implements ntpq -c "saveconfig<filename>"  *		 Writes current configuration including any runtime  *		 changes by ntpq's :config or config-from-file  *  * Note: There should be no buffer overflow or truncation in the  * processing of file names -- both cause security problems. This is bit  * painful to code but essential here.  */
end_comment
...
literal|"savedconfig="
decl_stmt|;
comment|/* Build a safe open mode from the available mode flags. We want 	 * to create a new file and write it in text mode (when 	 * applicable -- only Windows does this...) 	 */
specifier|static
specifier|const
name|int
name|openmode
...
name|_O_EXCL
argument_list|)
comment|/* windows is alway very special... */
operator||
name|_O_EXCL
endif|#
directive|endif
...
name|ctl_printf
argument_list|(
literal|"saveconfig exceeded maximum raw name length (%u)"
argument_list|,
operator|(
name|u_int
operator|)
...
name|LOG_NOTICE
argument_list|,
literal|"saveconfig exceeded maximum raw name length from %s"
argument_list|,
name|stoa
argument_list|(
operator|&
...
return|return;
block|}
comment|/* copy data directly as we exactly know the size */
name|memcpy
argument_list|(
name|filespec
argument_list|,
...
literal|'\0'
expr_stmt|;
comment|/* 	 * allow timestamping of the saved config filename with 	 * strftime() format such as: 	 *   ntpq -c "saveconfig ntp-%Y%m%d-%H%M%S.conf" 	 * XXX: Nice feature, but not too safe. 	 * YYY: The check for permitted characters in file names should 	 *      weed out the worst. Let's hope 'strftime()' does not 	 *      develop pathological problems. 	 */
name|time
argument_list|(
operator|&
name|now
...
expr_stmt|;
block|}
comment|/* 	 * Check the file name for sanity. This might/will rule out file 	 * names that would be legal but problematic, and it blocks 	 * directory traversal. 	 */
if|if
condition|(
operator|!
name|is_safe_filename
...
name|ctl_printf
argument_list|(
literal|"saveconfig rejects unsafe file name '%s'"
argument_list|,
name|filename
argument_list|)
expr_stmt|;
...
name|LOG_NOTICE
argument_list|,
literal|"saveconfig rejects unsafe file name from %s"
argument_list|,
name|stoa
argument_list|(
operator|&
...
return|return;
block|}
comment|/* 	 * XXX: This next test may not be needed with is_safe_filename() 	 */
...
return|return;
block|}
comment|/* concatenation of directory and path can cause another 	 * truncation... 	 */
name|prc
operator|=
name|snprintf
argument_list|(
...
name|ctl_printf
argument_list|(
literal|"saveconfig exceeded maximum path length (%u)"
argument_list|,
operator|(
name|u_int
operator|)
...
name|LOG_NOTICE
argument_list|,
literal|"saveconfig exceeded maximum path length from %s"
argument_list|,
name|stoa
argument_list|(
operator|&
...
name|ctl_printf
argument_list|(
literal|"Unable to save configuration to file '%s': %m"
argument_list|,
name|filename
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 		 * save the output filename in system variable 		 * savedconfig, retrieved with: 		 *   ntpq -c "rv 0 savedconfig" 		 * Note: the way 'savedconfig' is defined makes overflow 		 * checks unnecessary here. 		 */
name|snprintf
argument_list|(
name|savedconfig
argument_list|,
...
begin_comment
comment|/*  * ctl_printf - put a formatted string into the data buffer  */
end_comment
...
parameter_list|)
block|{
comment|/* [Bug 3008] First check the packet data sanity, then search 	 * the key. This improves the consistency of result values: If 	 * the result is NULL once, it will never be EOV again for this 	 * packet; If it's EOV, it will never be NULL again until the 	 * variable is found and processed in a given 'var_list'. (That 	 * is, a result is returned that is neither NULL nor EOV). 	 */
specifier|static
specifier|const
name|struct
name|ctl_var
...
name|tp
decl_stmt|;
comment|/* 	 * Part One: Validate the packet state 	 */
...
name|NULL
return|;
comment|/* Scan the string in the packet until we hit comma or 	 * EoB. Register position of first '=' on the fly. */
for|for
control|(
name|tp
operator|=
...
break|break;
block|}
comment|/* Process payload, if any. */
operator|*
name|data
operator|=
name|NULL
...
condition|)
block|{
comment|/* eventually strip white space from argument. */
specifier|const
name|char
modifier|*
name|plhead
...
name|pltail
expr_stmt|;
comment|/* check payload size, terminate packet on overflow */
name|plsize
operator|=
call|(
name|size_t
...
name|badpacket
goto|;
comment|/* copy data, NUL terminate, and set result data ptr */
name|memcpy
argument_list|(
name|buf
argument_list|,
...
break|break;
block|}
comment|/* See if we have found a valid entry or not. If found, advance 	 * the request pointer for the next round; if not, clear the 	 * data pointer so we have no dangling garbage here. 	 */
if|if
condition|(
name|EOV
operator|&
...
name|badpacket
label|:
comment|/*TODO? somehow indicate this packet was bad, apart from syslog? */
name|numctlbadpkts
operator|++
expr_stmt|;
name|NLOG
...
name|reqend
expr_stmt|;
comment|/* never again for this packet! */
return|return
name|NULL
return|;
block|}
...
return|return;
block|}
comment|/* Bug 2853 -- check if all characters were acceptable */
if|if
condition|(
name|data_count
operator|!=
...
argument_list|)
argument_list|,
literal|"runtime configuration failed: request contains an unprintable character"
argument_list|)
expr_stmt|;
name|ctl_putdata
argument_list|(
...
name|LOG_NOTICE
argument_list|,
literal|"runtime config from %s rejected: request contains an unprintable character: %0x"
argument_list|,
name|stoa
argument_list|(
operator|&
...
argument_list|)
expr_stmt|;
comment|/* The buffer has no trailing linefeed or NUL right now. For 	 * logging, we do not want a newline, so we do that first after 	 * adding the necessary NUL byte. 	 */
name|remote_config
operator|.
name|buffer
index|[
...
argument_list|)
expr_stmt|;
comment|/* Now we have to make sure there is a NL/NUL sequence at the 	 * end of the buffer before we parse it. 	 */
name|remote_config
operator|.
name|buffer
index|[
