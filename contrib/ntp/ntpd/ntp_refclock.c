...
argument_list|)
expr_stmt|;
comment|/* [Bug 2689] defined INIT state */
block|}
return|return
operator|(
literal|1
...
name|ts
expr_stmt|;
comment|/* save old timestamp for check */
if|if
condition|(
name|ap
operator|->
...
operator|)
return|;
comment|/* If the PPS source fails to deliver a new sample between 	 * polls, it regurgitates the last sample. We do not want to 	 * process the same sample multiple times. 	 */
if|if
condition|(
literal|0
operator|==
...
operator|)
return|;
comment|/* 	 * Convert to signed fraction offset, apply fudge and properly 	 * fold the correction into the [-0.5s,0.5s] range. Handle 	 * excessive fudge times, too. 	 */
name|dtemp
operator|=
name|ap
operator|->
...
literal|1.0
expr_stmt|;
comment|/* phase gate check: avoid wobbling by +/-1s when too close to 	 * the switch-over point. We allow +/-400ms max phase deviation. 	 * The trade-off is clear: The smaller the limit, the less 	 * sensitive to sampling noise the clock becomes. OTOH the 	 * system must get into phase gate range by other means for the 	 * PPS clock to lock in. 	 */
if|if
condition|(
name|fabs
argument_list|(
...
operator|)
return|;
comment|/* 	 * record this time stamp and stuff in median filter 	 */
name|pp
operator|->
name|lastrec
operator|.
