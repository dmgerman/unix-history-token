...
condition|)
block|{
comment|/* don't prune everything -- permit the last 10yrs 		 * before build. 		 */
name|build
operator|.
name|year
operator|-=
...
condition|)
block|{
comment|/* that was likely a query after a step */
name|reload_limits
argument_list|(
name|pt
argument_list|,
...
block|{
comment|/* close enough for deeper examination */
name|last
operator|=
name|pt
operator|->
...
name|fired
return|;
comment|/* now start to collect the remaining data */
name|due32
operator|=
name|pt
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Bail out if the basic offset is not zero and the putative 	 * offset is bigger than 10s. That was in 1972 -- we don't want 	 * to go back that far! 	 */
if|if
condition|(
name|pt
operator|->
...
name|FALSE
return|;
comment|/* If there's already data in the table, check if an update is 	 * possible. Update is impossible if there are static entries 	 * (since this indicates a valid leapsecond file) or if we're 	 * too close to a leapsecond transition: We do not know on what 	 * side the transition the sender might have been, so we use a 	 * dead zone around the transition. 	 */
comment|/* Check for static entries */
for|for
control|(
name|idx
operator|=
...
name|FALSE
return|;
comment|/* get the fulll time stamp and leap era for it */
name|now64
operator|=
name|ntpcal_ntp_to_ntp
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* check the limits with 20s dead band */
name|era
operator|.
name|ebase
operator|=
...
name|FALSE
return|;
comment|/* Here we can proceed. Calculate the delta update. */
name|tai_offset
operator|-=
name|era
operator|.
name|taiof
expr_stmt|;
comment|/* Shift the header info offsets. */
name|pt
operator|->
name|head
operator|.
...
name|tai_offset
expr_stmt|;
comment|/* Shift table entry offsets (if any) */
for|for
control|(
name|idx
operator|=
...
begin_comment
comment|/* [internal] check if a strtoXYZ ended at EOL or whitespace and  * converted something at all. Return TRUE if something went wrong.  */
end_comment
...
begin_comment
comment|/* [internal] fetch the leap era for a given time stamp.  * This is a cut-down version the algorithm used to reload the table  * limits, but it does not update any global state and provides just the  * era information for a given time stamp.  */
end_comment
...
name|idx
decl_stmt|;
comment|/* Simple search loop, also works with empty table. */
for|for
control|(
name|idx
operator|=
...
condition|)
break|break;
comment|/* fetch era data, keeping an eye on boundary conditions */
if|if
condition|(
name|idx
operator|>=
...
name|li
decl_stmt|;
comment|/* Check against the table expiration and the latest available 	 * leap entry. Do not permit inserts, only appends, and only if 	 * the extend the table beyond the expiration! 	 */
if|if
condition|(
name|ucmpv64
argument_list|(
...
name|li
decl_stmt|;
comment|/* Check that we either extend the table or get a duplicate of 	 * the latest entry. The latter is a benevolent overwrite with 	 * identical data and could happen if we get an autokey message 	 * that extends the lifetime of the current leapsecond table. 	 * Otherwise paranoia rulez! 	 */
if|if
condition|(
name|pt
operator|->
...
begin_comment
comment|/* reset the global state for unit tests */
end_comment
