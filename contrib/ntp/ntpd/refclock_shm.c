...
name|LOG_ERR
argument_list|,
literal|"SHM name too long (unit %d)"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
argument_list|(
operator|!
literal|"getShmTime(): Not reached."
argument_list|)
expr_stmt|;
block|}
end_function
...
directive|endif
comment|/* HAVE_ATOMIC_THREAD_FENCE */
block|}
end_function
...
block|}
comment|/*@-type@*/
comment|/* splint is confused about struct timespec */
name|shm_stat
operator|->
name|tvc
operator|.
...
expr_stmt|;
block|}
comment|/* relying on word access to be atomic here */
if|if
condition|(
name|shm
operator|->
...
name|count
expr_stmt|;
comment|/*      * This is proof against concurrency issues if either      * (a) the memory_barrier() call works on this host, or      * (b) memset compiles to an uninterruptible single-instruction bitblt.      */
name|memory_barrier
argument_list|()
expr_stmt|;
name|memcpy
...
argument_list|()
expr_stmt|;
comment|/*       * Clash detection in case neither (a) nor (b) was true.      * Not supported in mode 0, and word access to the count field       * must be atomic for this to work.      */
if|if
condition|(
name|shmcopy
operator|.
...
expr_stmt|;
block|}
comment|/* At this point shm_stat->tvr and shm_stat->tvt contain valid ns-level 	** timestamps, possibly generated by extending the old 	** us-level timestamps 	*/
break|break;
case|case
literal|1
case|:
...
expr_stmt|;
block|}
comment|/* At this point shm_stat->tvr and shm_stat->tvt contains valid ns-level 	** timestamps, possibly generated by extending the old 	** us-level timestamps 	*/
break|break;
default|default:
name|shm_stat
operator|->
...
block|}
comment|/*@-type@*/
comment|/*      * leap field is not a leap offset but a leap notification code.      * The values are magic numbers used by NTP and set by GPSD, if at all, in      * the subframe code.      */
name|shm_stat
operator|->
name|leap
operator|=
...
begin_comment
comment|/*  * shm_timer - called once every second.  *  * This tries to grab a sample from the SHM segment, filtering bad ones  */
end_comment
...
block|}
block|}
comment|/* query the segment, atomically */
name|status
operator|=
name|shm_query
argument_list|(
...
name|NO_SEGMENT
case|:
comment|/* should never happen, but is harmless */
return|return;
case|case
name|NOT_READY
case|:
...
argument_list|,
operator|(
literal|"%s: internal error, unknown SHM fetch status\n"
operator|,
name|refnumtoa
argument_list|(
operator|&
...
name|LOG_NOTICE
argument_list|,
literal|"internal error, unknown SHM fetch status"
argument_list|)
expr_stmt|;
name|up
operator|->
