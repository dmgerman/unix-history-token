...
parameter_list|)
block|{
comment|/* every line shows at most 16 bytes, so we need a buffer of 	 *   4 * 16 (2 xdigits, 1 char, one sep for xdigits) 	 * + 2 * 1  (block separators) 	 * +<LF> +<NUL> 	 *--------------- 	 *  68 bytes 	 */
specifier|static
specifier|const
name|char
name|s_xdig
...
name|n
expr_stmt|;
comment|/* faked timeout return from 'select()', 				 * execute RMW cycle on 'n' 				 */
block|}
if|if
condition|(
name|n
