...
directive|define
name|SQLITE_STDCALL
value|SQLITE_APICALL
endif|#
directive|endif
ifndef|#
directive|ifndef
name|SQLITE_CALLBACK
define|#
directive|define
name|SQLITE_CALLBACK
endif|#
...
value|"2017-08-01 13:24:15 9501e22dfeebdcefa783575e47c60b514d7c2e0cad73b2a496c0bc4b680900a8"
...
define|#
directive|define
name|SQLITE_OK_LOAD_PERMANENTLY
value|(SQLITE_OK | (1<<8))
...
define|#
directive|define
name|SQLITE_FCNTL_WIN32_GET_HANDLE
value|29
define|#
directive|define
name|SQLITE_FCNTL_PDB
value|30
...
name|sqlite3_mutex
typedef|;
comment|/* ** CAPI3REF: Loadable Extension Thunk ** ** A pointer to the opaque sqlite3_api_routines structure is passed as ** the third parameter to entry points of [loadable extensions].  This ** structure must be typedefed in order to work around compiler warnings ** on some platforms. */
typedef|typedef
name|struct
...
define|#
directive|define
name|SQLITE_DBCONFIG_MAINDBNAME
value|1000
comment|/* const char* */
define|#
directive|define
...
define|#
directive|define
name|SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION
value|1005
comment|/* int int* */
define|#
directive|define
name|SQLITE_DBCONFIG_NO_CKPT_ON_CLOSE
value|1006
comment|/* int int* */
define|#
directive|define
name|SQLITE_DBCONFIG_ENABLE_QPSG
value|1007
comment|/* int int* */
...
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Set the Last Insert Rowid value. ** METHOD: sqlite3 ** ** The sqlite3_set_last_insert_rowid(D, R) method allows the application to ** set the value returned by calling sqlite3_last_insert_rowid(D) to R  ** without inserting a row into the database. */
name|SQLITE_API
name|void
...
name|prepFlags
parameter_list|,
comment|/* Zero or more SQLITE_PREPARE_ flags */
name|sqlite3_stmt
modifier|*
modifier|*
name|ppStmt
...
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Retrieving Statement SQL ** METHOD: sqlite3_stmt ** ** ^The sqlite3_sql(P) interface returns a pointer to a copy of the UTF-8 ** SQL text used to create [prepared statement] P if P was ** created by [sqlite3_prepare_v2()], [sqlite3_prepare_v3()], ** [sqlite3_prepare16_v2()], or [sqlite3_prepare16_v3()]. ** ^The sqlite3_expanded_sql(P) interface returns a pointer to a UTF-8 ** string containing the SQL text of prepared statement P with ** [bound parameters] expanded. ** ** ^(For example, if a prepared statement is created using the SQL ** text "SELECT $abc,:xyz" and if parameter $abc is bound to integer 2345 ** and parameter :xyz is unbound, then sqlite3_sql() will return ** the original string, "SELECT $abc,:xyz" but sqlite3_expanded_sql() ** will return "SELECT 2345,NULL".)^ ** ** ^The sqlite3_expanded_sql() interface returns NULL if insufficient memory ** is available to hold the result, or if the result would exceed the ** the maximum string length determined by the [SQLITE_LIMIT_LENGTH]. ** ** ^The [SQLITE_TRACE_SIZE_LIMIT] compile-time option limits the size of ** bound parameter expansions.  ^The [SQLITE_OMIT_TRACE] compile-time ** option causes sqlite3_expanded_sql() to always return NULL. ** ** ^The string returned by sqlite3_sql(P) is managed by SQLite and is ** automatically freed when the prepared statement is finalized. ** ^The string returned by sqlite3_expanded_sql(P), on the other hand, ** is obtained from [sqlite3_malloc()] and must be free by the application ** by passing it to [sqlite3_free()]. */
name|SQLITE_API
specifier|const
name|char
modifier|*
...
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Number Of Columns In A Result Set ** METHOD: sqlite3_stmt ** ** ^Return the number of columns in the result set returned by the ** [prepared statement]. ^If this routine returns 0, that means the  ** [prepared statement] returns no data (for example an [UPDATE]). ** ^However, just because this routine returns a positive number does not ** mean that one or more rows of data will be returned.  ^A SELECT statement ** will always have a positive sqlite3_column_count() but depending on the ** WHERE clause constraints and the table content, it might return no rows. ** ** See also: [sqlite3_data_count()] */
name|SQLITE_API
name|int
...
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Enable Or Disable Shared Pager Cache ** ** ^(This routine enables or disables the sharing of the database cache ** and schema data structures between [database connection | connections] ** to the same database. Sharing is enabled if the argument is true ** and disabled if the argument is false.)^ ** ** ^Cache sharing is enabled and disabled for an entire process. ** This is a change as of SQLite [version 3.5.0] ([dateof:3.5.0]).  ** In prior versions of SQLite, ** sharing was enabled or disabled for each thread separately. ** ** ^(The cache sharing mode set by this interface effects all subsequent ** calls to [sqlite3_open()], [sqlite3_open_v2()], and [sqlite3_open16()]. ** Existing database connections continue use the sharing mode ** that was in effect at the time they were opened.)^ ** ** ^(This routine returns [SQLITE_OK] if shared cache was enabled or disabled ** successfully.  An [error code] is returned otherwise.)^ ** ** ^Shared cache is disabled by default. But this might change in ** future releases of SQLite.  Applications that care about shared ** cache setting should set it explicitly. ** ** Note: This method is disabled on MacOS X 10.7 and iOS version 5.0 ** and will always return SQLITE_MISUSE. On those systems,  ** shared cache mode should be enabled per-database connection via  ** [sqlite3_open_v2()] with [SQLITE_OPEN_SHAREDCACHE]. ** ** This interface is threadsafe on processors where writing a ** 32-bit integer is atomic. ** ** See Also:  [SQLite Shared-Cache Mode] */
name|SQLITE_API
name|int
...
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Move a BLOB Handle to a New Row ** METHOD: sqlite3_blob ** ** ^This function is used to move an existing [BLOB handle] so that it points ** to a different row of the same database table. ^The new row is identified ** by the rowid value passed as the second argument. Only the row can be ** changed. ^The database, table and column on which the blob handle is open ** remain the same. Moving an existing [BLOB handle] to a new row is ** faster than closing the existing handle and opening a new one. ** ** ^(The new row must meet the same criteria as for [sqlite3_blob_open()] - ** it must exist and there must be either a blob or text value stored in ** the nominated column.)^ ^If the new row is not present in the table, or if ** it does not contain a blob or text value, or if another error occurs, an ** SQLite error code is returned and the blob handle is considered aborted. ** ^All subsequent calls to [sqlite3_blob_read()], [sqlite3_blob_write()] or ** [sqlite3_blob_reopen()] on an aborted blob handle immediately return ** SQLITE_ABORT. ^Calling [sqlite3_blob_bytes()] on an aborted blob handle ** always returns zero. ** ** ^This function sets the database handle error code and message. */
name|SQLITE_API
name|int
...
define|#
directive|define
name|SQLITE_TESTCTRL_ONCE_RESET_THRESHOLD
value|19
define|#
directive|define
...
define|#
directive|define
name|SQLITE_DBSTATUS_CACHE_USED_SHARED
value|11
define|#
directive|define
...
define|#
directive|define
name|SQLITE_STMTSTATUS_REPREPARE
value|5
define|#
directive|define
name|SQLITE_STMTSTATUS_RUN
value|6
define|#
directive|define
name|SQLITE_STMTSTATUS_MEMUSED
value|99
...
name|pCtx
parameter_list|,
comment|/* Copy of third arg to preupdate_hook() */
name|sqlite3
modifier|*
name|db
parameter_list|,
...
name|op
parameter_list|,
comment|/* SQLITE_UPDATE, DELETE or INSERT */
name|char
specifier|const
modifier|*
name|zDb
...
name|iKey1
parameter_list|,
comment|/* Rowid of row about to be deleted/updated */
name|sqlite3_int64
name|iKey2
comment|/* New rowid value (for a rowid UPDATE) */
parameter_list|)
parameter_list|,
name|void
modifier|*
...
directive|endif
comment|/* ** CAPI3REF: Low-level system error code ** ** ^Attempt to return the underlying operating system error code or error ** number that caused the most recent I/O error or failure to open a file. ** The return value is OS-dependent.  For example, on unix systems, after ** [sqlite3_open_v2()] returns [SQLITE_CANTOPEN], this interface could be ** called to get back the underlying "errno" that caused the problem, such ** as ENOSPC, EAUTH, EISDIR, and so forth.   */
name|SQLITE_API
name|int
...
begin_comment
comment|/******** Begin file sqlite3rtree.h *********/
end_comment
...
begin_comment
comment|/******** End of sqlite3rtree.h *********/
end_comment

begin_comment
comment|/******** Begin file sqlite3session.h *********/
end_comment
...
name|zDb
parameter_list|,
comment|/* Name of db (e.g. "main") */
name|sqlite3_session
modifier|*
modifier|*
name|ppSession
comment|/* OUT: New session object */
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Delete A Session Object ** ** Delete a session object previously allocated using  ** [sqlite3session_create()]. Once a session object has been deleted, the ** results of attempting to use pSession with any other session module ** function are undefined. ** ** Session objects must be deleted before the database handle to which they ** are attached is closed. Refer to the documentation for  ** [sqlite3session_create()] for details. */
name|SQLITE_API
name|void
...
name|pCtx
parameter_list|,
comment|/* Copy of third arg to _filter_table() */
specifier|const
name|char
modifier|*
name|zTab
...
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Test if a changeset has recorded any changes. ** ** Return non-zero if no changes to attached tables have been recorded by  ** the session object passed as the first argument. Otherwise, if one or  ** more changes have been recorded, return zero. ** ** Even if this function returns zero, it is possible that calling ** [sqlite3session_changeset()] on the session handle may still return a ** changeset that contains no changes. This can happen when a row in  ** an attached table is modified and then later on the original values  ** are restored. However, if this function returns non-zero, then it is ** guaranteed that a call to sqlite3session_changeset() will return a  ** changeset containing zero changes. */
name|SQLITE_API
name|int
...
modifier|*
name|ppValue
comment|/* OUT: Old value (or NULL pointer) */
parameter_list|)
function_decl|;
...
modifier|*
name|ppValue
comment|/* OUT: New value (or NULL pointer) */
parameter_list|)
function_decl|;
...
name|SQLITE_API
name|int
name|sqlite3changeset_conflict
parameter_list|(
name|sqlite3_changeset_iter
modifier|*
name|pIter
...
modifier|*
name|ppOut
comment|/* OUT: Inverse of input */
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Concatenate Two Changeset Objects ** ** This function is used to concatenate two changesets, A and B, into a  ** single changeset. The result is a changeset equivalent to applying ** changeset A followed by changeset B.  ** ** This function combines the two input changesets using an  ** sqlite3_changegroup object. Calling it produces similar results as the ** following code fragment: ** **   sqlite3_changegroup *pGrp; **   rc = sqlite3_changegroup_new(&pGrp); **   if( rc==SQLITE_OK ) rc = sqlite3changegroup_add(pGrp, nA, pA); **   if( rc==SQLITE_OK ) rc = sqlite3changegroup_add(pGrp, nB, pB); **   if( rc==SQLITE_OK ){ **     rc = sqlite3changegroup_output(pGrp, pnOut, ppOut); **   }else{ **     *ppOut = 0; **     *pnOut = 0; **   } ** ** Refer to the sqlite3_changegroup documentation below for details. */
name|SQLITE_API
name|int
name|sqlite3changeset_concat
parameter_list|(
name|int
name|nA
parameter_list|,
comment|/* Number of bytes in buffer pA */
name|void
modifier|*
name|pA
parameter_list|,
comment|/* Pointer to buffer containing changeset A */
name|int
name|nB
parameter_list|,
comment|/* Number of bytes in buffer pB */
name|void
modifier|*
name|pB
parameter_list|,
comment|/* Pointer to buffer containing changeset B */
name|int
modifier|*
name|pnOut
parameter_list|,
comment|/* OUT: Number of bytes in output changeset */
name|void
modifier|*
modifier|*
name|ppOut
comment|/* OUT: Buffer containing output changeset */
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Changegroup Handle */
typedef|typedef
name|struct
name|sqlite3_changegroup
name|sqlite3_changegroup
typedef|;
comment|/* ** CAPI3REF: Create A New Changegroup Object ** ** An sqlite3_changegroup object is used to combine two or more changesets ** (or patchsets) into a single changeset (or patchset). A single changegroup ** object may combine changesets or patchsets, but not both. The output is ** always in the same format as the input. ** ** If successful, this function returns SQLITE_OK and populates (*pp) with ** a pointer to a new sqlite3_changegroup object before returning. The caller ** should eventually free the returned object using a call to  ** sqlite3changegroup_delete(). If an error occurs, an SQLite error code ** (i.e. SQLITE_NOMEM) is returned and *pp is set to NULL. ** ** The usual usage pattern for an sqlite3_changegroup object is as follows: ** **<ul> **<li> It is created using a call to sqlite3changegroup_new(). ** **<li> Zero or more changesets (or patchsets) are added to the object **        by calling sqlite3changegroup_add(). ** **<li> The result of combining all input changesets together is obtained  **        by the application via a call to sqlite3changegroup_output(). ** **<li> The object is deleted using a call to sqlite3changegroup_delete(). **</ul> ** ** Any number of calls to add() and output() may be made between the calls to ** new() and delete(), and in any order. ** ** As well as the regular sqlite3changegroup_add() and  ** sqlite3changegroup_output() functions, also available are the streaming ** versions sqlite3changegroup_add_strm() and sqlite3changegroup_output_strm(). */
name|SQLITE_API
name|int
name|sqlite3changegroup_new
parameter_list|(
name|sqlite3_changegroup
modifier|*
modifier|*
...
parameter_list|)
function_decl|;
comment|/* ** CAPI3REF: Obtain A Composite Changeset From A Changegroup ** ** Obtain a buffer containing a changeset (or patchset) representing the ** current contents of the changegroup. If the inputs to the changegroup ** were themselves changesets, the output is a changeset. Or, if the ** inputs were patchsets, the output is also a patchset. ** ** As with the output of the sqlite3session_changeset() and ** sqlite3session_patchset() functions, all changes related to a single ** table are grouped together in the output of this function. Tables appear ** in the same order as for the very first changeset added to the changegroup. ** If the second or subsequent changesets added to the changegroup contain ** changes for tables that do not appear in the first changeset, they are ** appended onto the end of the output changeset, again in the order in ** which they are first encountered. ** ** If an error occurs, an SQLite error code is returned and the output ** variables (*pnData) and (*ppData) are set to 0. Otherwise, SQLITE_OK ** is returned and the output variables are set to the size of and a  ** pointer to the output buffer, respectively. In this case it is the ** responsibility of the caller to eventually free the buffer using a ** call to sqlite3_free(). */
name|SQLITE_API
name|int
name|sqlite3changegroup_output
parameter_list|(
name|sqlite3_changegroup
modifier|*
parameter_list|,
...
name|pCtx
parameter_list|,
comment|/* Copy of sixth arg to _apply() */
specifier|const
name|char
modifier|*
name|zTab
...
name|eConflict
parameter_list|,
comment|/* DATA, MISSING, CONFLICT, CONSTRAINT */
name|sqlite3_changeset_iter
modifier|*
name|p
comment|/* Handle describing change and conflict */
parameter_list|)
parameter_list|,
name|void
modifier|*
name|pCtx
comment|/* First argument passed to xConflict */
parameter_list|)
function_decl|;
...
name|SQLITE_API
name|int
name|sqlite3changeset_concat_strm
parameter_list|(
name|int
function_decl|(
modifier|*
...
name|SQLITE_API
name|int
name|sqlite3changeset_invert_strm
parameter_list|(
name|int
function_decl|(
modifier|*
...
name|SQLITE_API
name|int
name|sqlite3changeset_start_strm
parameter_list|(
name|sqlite3_changeset_iter
modifier|*
modifier|*
...
name|SQLITE_API
name|int
name|sqlite3session_changeset_strm
parameter_list|(
name|sqlite3_session
modifier|*
name|pSession
...
name|SQLITE_API
name|int
name|sqlite3session_patchset_strm
parameter_list|(
name|sqlite3_session
modifier|*
name|pSession
...
name|SQLITE_API
name|int
name|sqlite3changegroup_add_strm
parameter_list|(
name|sqlite3_changegroup
modifier|*
parameter_list|,
...
name|SQLITE_API
name|int
name|sqlite3changegroup_output_strm
parameter_list|(
name|sqlite3_changegroup
modifier|*
parameter_list|,
...
begin_comment
comment|/* !defined(__SQLITESESSION_H_)&& defined(SQLITE_ENABLE_SESSION) */
end_comment

begin_comment
comment|/******** End of sqlite3session.h *********/
end_comment

begin_comment
comment|/******** Begin file fts5.h *********/
end_comment
