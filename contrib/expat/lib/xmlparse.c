...
name|s
parameter_list|)
value|(!(enc)->isUtf16 || (((unsigned long)s)& 1))
end_define
...
name|keepProcessing
decl_stmt|;
comment|/* true once an internal or external PE reference has been encountered;      any external subset is considered an external PE reference */
name|XML_Bool
...
name|XML_Error
name|FASTCALL
name|processInternalParamEntity
parameter_list|(
name|XML_Parser
name|parser
parameter_list|,
...
directive|define
name|parsing
value|(processor != prologInitProcessor)
end_define
...
name|XML_L
argument_list|(
literal|"xml declaration not at start of external entity"
argument_list|)
block|,
name|XML_L
argument_list|(
...
begin_comment
comment|/* If tagNamePtr is non-null, build a real list of attributes,    otherwise just check the attributes for well-formedness. */
end_comment
...
name|XML_ERROR_NO_MEMORY
return|;
comment|/* detect duplicate attributes */
if|if
condition|(
operator|(
name|attId
...
condition|)
block|{
comment|/* expand prefixed attribute names */
for|for
control|(
init|;
name|i
...
block|}
block|}
comment|/* clear the flags that say whether attributes were specified */
for|for
control|(
init|;
name|i
...
name|XML_TRUE
expr_stmt|;
comment|/* when docTypeSysid == NULL */
if|if
condition|(
name|paramEntityParsing
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* first, determine if a check for an existing declaration is needed;            if yes, check that the entity exists, and that it is internal,            otherwise call the default handler (if called from content)         */
if|if
condition|(
name|pool
operator|==
