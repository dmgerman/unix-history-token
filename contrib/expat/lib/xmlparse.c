...
name|unsigned
name|long
name|generate_hash_secret_salt
parameter_list|(
name|XML_Parser
name|parser
parameter_list|)
...
define|#
directive|define
name|hash_secret_salt
value|(parser->m_hash_secret_salt)
end_define
...
argument_list|)
expr_stmt|;
comment|/* Microseconds time is<20 bits entropy */
return|return
name|tv
operator|.
name|tv_usec
...
parameter_list|)
block|{
comment|/* Process ID is 0 bits entropy if attacker has local access    * XML_Parser address is few bits of entropy if attacker has local access */
specifier|const
name|unsigned
name|long
name|entropy
...
name|parser
decl_stmt|;
comment|/* Factors are 2^31-1 and 2^61-1 (Mersenne primes M31 and M61) */
if|if
condition|(
expr|sizeof
operator|(
...
specifier|static
name|XML_Bool
comment|/* only valid for root parser */
name|startParsing
parameter_list|(
name|XML_Parser
name|parser
parameter_list|)
block|{
comment|/* hash functions must be initialized before setContext() is called */
if|if
condition|(
name|hash_secret_salt
operator|==
...
name|ns_triplets
decl_stmt|;
comment|/* Note that the new parser shares the same hash secret as the old      parser, so that dtdCopy and copyEntityTable can lookup values      from hash tables associated with either parser without us having      to worry which hash secrets each table has.   */
name|unsigned
name|long
name|oldhash_secret_salt
init|=
name|hash_secret_salt
decl_stmt|;
ifdef|#
...
comment|/* Do not invoke signed arithmetic overflow: */
name|int
name|neededSize
init|=
call|(
...
do|;
block|{
comment|/* Check hash table for duplicate of expanded name (uriName).              Derived from code in lookup(parser, HASH_TABLE *table, ...).           */
name|unsigned
name|char
name|step
init|=
