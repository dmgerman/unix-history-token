begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (C) 1997-2000 by Darren Reed.  *  * Redistribution and use in source and binary forms are permitted  * provided that this notice is preserved and due credit is given  * to the original author and the contributors.  */
end_comment

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
end_if

begin_decl_stmt
specifier|static
specifier|const
name|char
name|rcsid
index|[]
init|=
literal|"@(#)$Id: ip_proxy.c,v 2.9.2.1 2000/05/06 12:30:50 darrenr Exp $"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|defined
argument_list|(
name|__FreeBSD__
argument_list|)
operator|&&
name|defined
argument_list|(
name|KERNEL
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|_KERNEL
argument_list|)
end_if

begin_define
define|#
directive|define
name|_KERNEL
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|__FreeBSD_version
argument_list|)
end_if

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/fcntl.h>
end_include

begin_include
include|#
directive|include
file|<sys/uio.h>
end_include

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|_KERNEL
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|KERNEL
argument_list|)
end_if

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_ifndef
ifndef|#
directive|ifndef
name|linux
end_ifndef

begin_include
include|#
directive|include
file|<sys/protosw.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_if
if|#
directive|if
name|defined
argument_list|(
name|_KERNEL
argument_list|)
end_if

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|linux
argument_list|)
end_if

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_else
else|#
directive|else
end_else

begin_include
include|#
directive|include
file|<linux/string.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|__SVR4
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|__svr4__
argument_list|)
end_if

begin_ifndef
ifndef|#
directive|ifndef
name|linux
end_ifndef

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_else
else|#
directive|else
end_else

begin_include
include|#
directive|include
file|<sys/byteorder.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|_KERNEL
end_ifdef

begin_include
include|#
directive|include
file|<sys/dditypes.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/stream.h>
end_include

begin_include
include|#
directive|include
file|<sys/kmem.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|__FreeBSD__
operator|>
literal|2
end_if

begin_include
include|#
directive|include
file|<sys/queue.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<net/if.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|sun
end_ifdef

begin_include
include|#
directive|include
file|<net/af.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<net/route.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_systm.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip.h>
end_include

begin_ifndef
ifndef|#
directive|ifndef
name|linux
end_ifndef

begin_include
include|#
directive|include
file|<netinet/ip_var.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<netinet/tcp.h>
end_include

begin_include
include|#
directive|include
file|<netinet/udp.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip_icmp.h>
end_include

begin_include
include|#
directive|include
file|"netinet/ip_compat.h"
end_include

begin_include
include|#
directive|include
file|<netinet/tcpip.h>
end_include

begin_include
include|#
directive|include
file|"netinet/ip_fil.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_proxy.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_nat.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_state.h"
end_include

begin_if
if|#
directive|if
operator|(
name|__FreeBSD_version
operator|>=
literal|300000
operator|)
end_if

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_ifndef
ifndef|#
directive|ifndef
name|MIN
end_ifndef

begin_define
define|#
directive|define
name|MIN
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|(((a)<(b))?(a):(b))
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
specifier|static
name|ap_session_t
modifier|*
name|appr_new_session
name|__P
argument_list|(
operator|(
name|aproxy_t
operator|*
operator|,
name|ip_t
operator|*
operator|,
name|fr_info_t
operator|*
operator|,
name|nat_t
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|int
name|appr_fixseqack
name|__P
argument_list|(
operator|(
name|fr_info_t
operator|*
operator|,
name|ip_t
operator|*
operator|,
name|ap_session_t
operator|*
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|AP_SESS_SIZE
value|53
end_define

begin_if
if|#
directive|if
name|defined
argument_list|(
name|_KERNEL
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|linux
argument_list|)
end_if

begin_include
include|#
directive|include
file|"netinet/ip_ftp_pxy.c"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_rcmd_pxy.c"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_raudio_pxy.c"
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|ap_session_t
modifier|*
name|ap_sess_tab
index|[
name|AP_SESS_SIZE
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|ap_session_t
modifier|*
name|ap_sess_list
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|aproxy_t
modifier|*
name|ap_proxylist
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|aproxy_t
name|ap_proxies
index|[]
init|=
block|{
ifdef|#
directive|ifdef
name|IPF_FTP_PROXY
block|{
name|NULL
block|,
literal|"ftp"
block|,
operator|(
name|char
operator|)
name|IPPROTO_TCP
block|,
literal|0
block|,
literal|0
block|,
name|ippr_ftp_init
block|,
name|NULL
block|,
name|ippr_ftp_new
block|,
name|ippr_ftp_in
block|,
name|ippr_ftp_out
block|}
block|,
endif|#
directive|endif
ifdef|#
directive|ifdef
name|IPF_RCMD_PROXY
block|{
name|NULL
block|,
literal|"rcmd"
block|,
operator|(
name|char
operator|)
name|IPPROTO_TCP
block|,
literal|0
block|,
literal|0
block|,
name|ippr_rcmd_init
block|,
name|NULL
block|,
name|ippr_rcmd_new
block|,
name|NULL
block|,
name|ippr_rcmd_out
block|}
block|,
endif|#
directive|endif
ifdef|#
directive|ifdef
name|IPF_RAUDIO_PROXY
block|{
name|NULL
block|,
literal|"raudio"
block|,
operator|(
name|char
operator|)
name|IPPROTO_TCP
block|,
literal|0
block|,
literal|0
block|,
name|ippr_raudio_init
block|,
name|NULL
block|,
name|ippr_raudio_new
block|,
name|ippr_raudio_in
block|,
name|ippr_raudio_out
block|}
block|,
endif|#
directive|endif
block|{
name|NULL
block|,
literal|""
block|,
literal|'\0'
block|,
literal|0
block|,
literal|0
block|,
name|NULL
block|,
name|NULL
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|appr_add
parameter_list|(
name|ap
parameter_list|)
name|aproxy_t
modifier|*
name|ap
decl_stmt|;
block|{
name|aproxy_t
modifier|*
name|a
decl_stmt|;
for|for
control|(
name|a
operator|=
name|ap_proxies
init|;
name|a
operator|->
name|apr_p
condition|;
name|a
operator|++
control|)
if|if
condition|(
operator|(
name|a
operator|->
name|apr_p
operator|==
name|ap
operator|->
name|apr_p
operator|)
operator|&&
operator|!
name|strncmp
argument_list|(
name|a
operator|->
name|apr_label
argument_list|,
name|ap
operator|->
name|apr_label
argument_list|,
sizeof|sizeof
argument_list|(
name|ap
operator|->
name|apr_label
argument_list|)
argument_list|)
condition|)
return|return
operator|-
literal|1
return|;
for|for
control|(
name|a
operator|=
name|ap_proxylist
init|;
name|a
operator|->
name|apr_p
condition|;
name|a
operator|=
name|a
operator|->
name|apr_next
control|)
if|if
condition|(
operator|(
name|a
operator|->
name|apr_p
operator|==
name|ap
operator|->
name|apr_p
operator|)
operator|&&
operator|!
name|strncmp
argument_list|(
name|a
operator|->
name|apr_label
argument_list|,
name|ap
operator|->
name|apr_label
argument_list|,
sizeof|sizeof
argument_list|(
name|ap
operator|->
name|apr_label
argument_list|)
argument_list|)
condition|)
return|return
operator|-
literal|1
return|;
name|ap
operator|->
name|apr_next
operator|=
name|ap_proxylist
expr_stmt|;
name|ap_proxylist
operator|=
name|ap
expr_stmt|;
return|return
call|(
modifier|*
name|ap
operator|->
name|apr_init
call|)
argument_list|()
return|;
block|}
end_function

begin_function
name|int
name|appr_del
parameter_list|(
name|ap
parameter_list|)
name|aproxy_t
modifier|*
name|ap
decl_stmt|;
block|{
name|aproxy_t
modifier|*
name|a
decl_stmt|,
modifier|*
modifier|*
name|app
decl_stmt|;
for|for
control|(
name|app
operator|=
operator|&
name|ap_proxylist
init|;
operator|(
name|a
operator|=
operator|*
name|app
operator|)
condition|;
name|app
operator|=
operator|&
name|a
operator|->
name|apr_next
control|)
if|if
condition|(
name|a
operator|==
name|ap
condition|)
block|{
if|if
condition|(
name|ap
operator|->
name|apr_ref
operator|!=
literal|0
condition|)
return|return
literal|1
return|;
operator|*
name|app
operator|=
name|a
operator|->
name|apr_next
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
operator|-
literal|1
return|;
block|}
end_function

begin_function
name|int
name|appr_ok
parameter_list|(
name|ip
parameter_list|,
name|tcp
parameter_list|,
name|nat
parameter_list|)
name|ip_t
modifier|*
name|ip
decl_stmt|;
name|tcphdr_t
modifier|*
name|tcp
decl_stmt|;
name|ipnat_t
modifier|*
name|nat
decl_stmt|;
block|{
name|aproxy_t
modifier|*
name|apr
init|=
name|nat
operator|->
name|in_apr
decl_stmt|;
name|u_short
name|dport
init|=
name|nat
operator|->
name|in_dport
decl_stmt|;
if|if
condition|(
operator|!
name|apr
operator|||
operator|(
name|apr
operator|->
name|apr_flags
operator|&
name|APR_DELETE
operator|)
operator|||
operator|(
name|ip
operator|->
name|ip_p
operator|!=
name|apr
operator|->
name|apr_p
operator|)
condition|)
return|return
literal|0
return|;
if|if
condition|(
operator|(
name|tcp
operator|&&
operator|(
name|tcp
operator|->
name|th_dport
operator|!=
name|dport
operator|)
operator|)
operator|||
operator|(
operator|!
name|tcp
operator|&&
name|dport
operator|)
condition|)
return|return
literal|0
return|;
return|return
literal|1
return|;
block|}
end_function

begin_comment
comment|/*  * Allocate a new application proxy structure and fill it in with the  * relevant details.  call the init function once complete, prior to  * returning.  */
end_comment

begin_function
specifier|static
name|ap_session_t
modifier|*
name|appr_new_session
parameter_list|(
name|apr
parameter_list|,
name|ip
parameter_list|,
name|fin
parameter_list|,
name|nat
parameter_list|)
name|aproxy_t
modifier|*
name|apr
decl_stmt|;
name|ip_t
modifier|*
name|ip
decl_stmt|;
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
name|nat_t
modifier|*
name|nat
decl_stmt|;
block|{
specifier|register
name|ap_session_t
modifier|*
name|aps
decl_stmt|;
if|if
condition|(
operator|!
name|apr
operator|||
operator|(
name|apr
operator|->
name|apr_flags
operator|&
name|APR_DELETE
operator|)
operator|||
operator|(
name|ip
operator|->
name|ip_p
operator|!=
name|apr
operator|->
name|apr_p
operator|)
condition|)
return|return
name|NULL
return|;
name|KMALLOC
argument_list|(
name|aps
argument_list|,
name|ap_session_t
operator|*
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|aps
condition|)
return|return
name|NULL
return|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|aps
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|aps
argument_list|)
argument_list|)
expr_stmt|;
name|aps
operator|->
name|aps_p
operator|=
name|ip
operator|->
name|ip_p
expr_stmt|;
name|aps
operator|->
name|aps_data
operator|=
name|NULL
expr_stmt|;
name|aps
operator|->
name|aps_apr
operator|=
name|apr
expr_stmt|;
name|aps
operator|->
name|aps_psiz
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|apr
operator|->
name|apr_new
operator|!=
name|NULL
condition|)
if|if
condition|(
call|(
modifier|*
name|apr
operator|->
name|apr_new
call|)
argument_list|(
name|fin
argument_list|,
name|ip
argument_list|,
name|aps
argument_list|,
name|nat
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|KFREE
argument_list|(
name|aps
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|aps
operator|->
name|aps_nat
operator|=
name|nat
expr_stmt|;
name|aps
operator|->
name|aps_next
operator|=
name|ap_sess_list
expr_stmt|;
name|ap_sess_list
operator|=
name|aps
expr_stmt|;
return|return
name|aps
return|;
block|}
end_function

begin_comment
comment|/*  * check to see if a packet should be passed through an active proxy routine  * if one has been setup for it.  */
end_comment

begin_function
name|int
name|appr_check
parameter_list|(
name|ip
parameter_list|,
name|fin
parameter_list|,
name|nat
parameter_list|)
name|ip_t
modifier|*
name|ip
decl_stmt|;
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
name|nat_t
modifier|*
name|nat
decl_stmt|;
block|{
name|ap_session_t
modifier|*
name|aps
decl_stmt|;
name|aproxy_t
modifier|*
name|apr
decl_stmt|;
name|tcphdr_t
modifier|*
name|tcp
init|=
name|NULL
decl_stmt|;
name|u_32_t
name|sum
decl_stmt|;
name|short
name|rv
decl_stmt|;
name|int
name|err
decl_stmt|;
if|if
condition|(
name|nat
operator|->
name|nat_aps
operator|==
name|NULL
condition|)
name|nat
operator|->
name|nat_aps
operator|=
name|appr_new_session
argument_list|(
name|nat
operator|->
name|nat_ptr
operator|->
name|in_apr
argument_list|,
name|ip
argument_list|,
name|fin
argument_list|,
name|nat
argument_list|)
expr_stmt|;
name|aps
operator|=
name|nat
operator|->
name|nat_aps
expr_stmt|;
if|if
condition|(
operator|(
name|aps
operator|!=
name|NULL
operator|)
operator|&&
operator|(
name|aps
operator|->
name|aps_p
operator|==
name|ip
operator|->
name|ip_p
operator|)
condition|)
block|{
if|if
condition|(
name|ip
operator|->
name|ip_p
operator|==
name|IPPROTO_TCP
condition|)
block|{
name|tcp
operator|=
operator|(
name|tcphdr_t
operator|*
operator|)
name|fin
operator|->
name|fin_dp
expr_stmt|;
comment|/* 			 * verify that the checksum is correct.  If not, then 			 * don't do anything with this packet. 			 */
if|#
directive|if
name|SOLARIS
operator|&&
name|defined
argument_list|(
name|_KERNEL
argument_list|)
name|sum
operator|=
name|fr_tcpsum
argument_list|(
name|fin
operator|->
name|fin_qfm
argument_list|,
name|ip
argument_list|,
name|tcp
argument_list|)
expr_stmt|;
else|#
directive|else
name|sum
operator|=
name|fr_tcpsum
argument_list|(
operator|*
operator|(
name|mb_t
operator|*
operator|*
operator|)
name|fin
operator|->
name|fin_mp
argument_list|,
name|ip
argument_list|,
name|tcp
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|sum
operator|!=
name|tcp
operator|->
name|th_sum
condition|)
block|{
name|frstats
index|[
name|fin
operator|->
name|fin_out
index|]
operator|.
name|fr_tcpbad
operator|++
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
block|}
name|apr
operator|=
name|aps
operator|->
name|aps_apr
expr_stmt|;
name|err
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_out
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|apr
operator|->
name|apr_outpkt
operator|!=
name|NULL
condition|)
name|err
operator|=
call|(
modifier|*
name|apr
operator|->
name|apr_outpkt
call|)
argument_list|(
name|fin
argument_list|,
name|ip
argument_list|,
name|aps
argument_list|,
name|nat
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|apr
operator|->
name|apr_inpkt
operator|!=
name|NULL
condition|)
name|err
operator|=
call|(
modifier|*
name|apr
operator|->
name|apr_inpkt
call|)
argument_list|(
name|fin
argument_list|,
name|ip
argument_list|,
name|aps
argument_list|,
name|nat
argument_list|)
expr_stmt|;
block|}
name|rv
operator|=
name|APR_EXIT
argument_list|(
name|err
argument_list|)
expr_stmt|;
if|if
condition|(
name|rv
operator|==
operator|-
literal|1
condition|)
return|return
name|rv
return|;
if|if
condition|(
name|tcp
operator|!=
name|NULL
condition|)
block|{
name|err
operator|=
name|appr_fixseqack
argument_list|(
name|fin
argument_list|,
name|ip
argument_list|,
name|aps
argument_list|,
name|APR_INC
argument_list|(
name|err
argument_list|)
argument_list|)
expr_stmt|;
if|#
directive|if
name|SOLARIS
operator|&&
name|defined
argument_list|(
name|_KERNEL
argument_list|)
name|tcp
operator|->
name|th_sum
operator|=
name|fr_tcpsum
argument_list|(
name|fin
operator|->
name|fin_qfm
argument_list|,
name|ip
argument_list|,
name|tcp
argument_list|)
expr_stmt|;
else|#
directive|else
name|tcp
operator|->
name|th_sum
operator|=
name|fr_tcpsum
argument_list|(
operator|*
operator|(
name|mb_t
operator|*
operator|*
operator|)
name|fin
operator|->
name|fin_mp
argument_list|,
name|ip
argument_list|,
name|tcp
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
name|aps
operator|->
name|aps_bytes
operator|+=
name|ip
operator|->
name|ip_len
expr_stmt|;
name|aps
operator|->
name|aps_pkts
operator|++
expr_stmt|;
return|return
literal|1
return|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_function
name|aproxy_t
modifier|*
name|appr_match
parameter_list|(
name|pr
parameter_list|,
name|name
parameter_list|)
name|u_int
name|pr
decl_stmt|;
name|char
modifier|*
name|name
decl_stmt|;
block|{
name|aproxy_t
modifier|*
name|ap
decl_stmt|;
for|for
control|(
name|ap
operator|=
name|ap_proxies
init|;
name|ap
operator|->
name|apr_p
condition|;
name|ap
operator|++
control|)
if|if
condition|(
operator|(
name|ap
operator|->
name|apr_p
operator|==
name|pr
operator|)
operator|&&
operator|!
name|strncmp
argument_list|(
name|name
argument_list|,
name|ap
operator|->
name|apr_label
argument_list|,
sizeof|sizeof
argument_list|(
name|ap
operator|->
name|apr_label
argument_list|)
argument_list|)
condition|)
block|{
name|ap
operator|->
name|apr_ref
operator|++
expr_stmt|;
return|return
name|ap
return|;
block|}
for|for
control|(
name|ap
operator|=
name|ap_proxylist
init|;
name|ap
condition|;
name|ap
operator|=
name|ap
operator|->
name|apr_next
control|)
if|if
condition|(
operator|(
name|ap
operator|->
name|apr_p
operator|==
name|pr
operator|)
operator|&&
operator|!
name|strncmp
argument_list|(
name|name
argument_list|,
name|ap
operator|->
name|apr_label
argument_list|,
sizeof|sizeof
argument_list|(
name|ap
operator|->
name|apr_label
argument_list|)
argument_list|)
condition|)
block|{
name|ap
operator|->
name|apr_ref
operator|++
expr_stmt|;
return|return
name|ap
return|;
block|}
return|return
name|NULL
return|;
block|}
end_function

begin_function
name|void
name|appr_free
parameter_list|(
name|ap
parameter_list|)
name|aproxy_t
modifier|*
name|ap
decl_stmt|;
block|{
name|ap
operator|->
name|apr_ref
operator|--
expr_stmt|;
block|}
end_function

begin_function
name|void
name|aps_free
parameter_list|(
name|aps
parameter_list|)
name|ap_session_t
modifier|*
name|aps
decl_stmt|;
block|{
name|ap_session_t
modifier|*
name|a
decl_stmt|,
modifier|*
modifier|*
name|ap
decl_stmt|;
if|if
condition|(
operator|!
name|aps
condition|)
return|return;
for|for
control|(
name|ap
operator|=
operator|&
name|ap_sess_list
init|;
operator|(
name|a
operator|=
operator|*
name|ap
operator|)
condition|;
name|ap
operator|=
operator|&
name|a
operator|->
name|aps_next
control|)
if|if
condition|(
name|a
operator|==
name|aps
condition|)
block|{
operator|*
name|ap
operator|=
name|a
operator|->
name|aps_next
expr_stmt|;
break|break;
block|}
if|if
condition|(
operator|(
name|aps
operator|->
name|aps_data
operator|!=
name|NULL
operator|)
operator|&&
operator|(
name|aps
operator|->
name|aps_psiz
operator|!=
literal|0
operator|)
condition|)
name|KFREES
argument_list|(
name|aps
operator|->
name|aps_data
argument_list|,
name|aps
operator|->
name|aps_psiz
argument_list|)
expr_stmt|;
name|KFREE
argument_list|(
name|aps
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|appr_fixseqack
parameter_list|(
name|fin
parameter_list|,
name|ip
parameter_list|,
name|aps
parameter_list|,
name|inc
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
name|ip_t
modifier|*
name|ip
decl_stmt|;
name|ap_session_t
modifier|*
name|aps
decl_stmt|;
name|int
name|inc
decl_stmt|;
block|{
name|int
name|sel
decl_stmt|,
name|ch
init|=
literal|0
decl_stmt|,
name|out
decl_stmt|,
name|nlen
decl_stmt|;
name|u_32_t
name|seq1
decl_stmt|,
name|seq2
decl_stmt|;
name|tcphdr_t
modifier|*
name|tcp
decl_stmt|;
name|tcp
operator|=
operator|(
name|tcphdr_t
operator|*
operator|)
name|fin
operator|->
name|fin_dp
expr_stmt|;
name|out
operator|=
name|fin
operator|->
name|fin_out
expr_stmt|;
name|nlen
operator|=
name|ip
operator|->
name|ip_len
expr_stmt|;
name|nlen
operator|-=
operator|(
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
operator|)
operator|+
operator|(
name|tcp
operator|->
name|th_off
operator|<<
literal|2
operator|)
expr_stmt|;
if|if
condition|(
name|out
operator|!=
literal|0
condition|)
block|{
name|seq1
operator|=
operator|(
name|u_32_t
operator|)
name|ntohl
argument_list|(
name|tcp
operator|->
name|th_seq
argument_list|)
expr_stmt|;
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
name|out
index|]
expr_stmt|;
comment|/* switch to other set ? */
if|if
condition|(
operator|(
name|aps
operator|->
name|aps_seqmin
index|[
operator|!
name|sel
index|]
operator|>
name|aps
operator|->
name|aps_seqmin
index|[
name|sel
index|]
operator|)
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_seqmin
index|[
operator|!
name|sel
index|]
operator|)
condition|)
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
name|out
index|]
operator|=
operator|!
name|sel
expr_stmt|;
if|if
condition|(
name|aps
operator|->
name|aps_seqoff
index|[
name|sel
index|]
condition|)
block|{
name|seq2
operator|=
name|aps
operator|->
name|aps_seqmin
index|[
name|sel
index|]
operator|-
name|aps
operator|->
name|aps_seqoff
index|[
name|sel
index|]
expr_stmt|;
if|if
condition|(
name|seq1
operator|>
name|seq2
condition|)
block|{
name|seq2
operator|=
name|aps
operator|->
name|aps_seqoff
index|[
name|sel
index|]
expr_stmt|;
name|seq1
operator|+=
name|seq2
expr_stmt|;
name|tcp
operator|->
name|th_seq
operator|=
name|htonl
argument_list|(
name|seq1
argument_list|)
expr_stmt|;
name|ch
operator|=
literal|1
expr_stmt|;
block|}
block|}
if|if
condition|(
name|inc
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_seqmin
index|[
operator|!
name|sel
index|]
operator|)
condition|)
block|{
name|aps
operator|->
name|aps_seqmin
index|[
operator|!
name|sel
index|]
operator|=
name|seq1
operator|+
name|nlen
operator|-
literal|1
expr_stmt|;
name|aps
operator|->
name|aps_seqoff
index|[
operator|!
name|sel
index|]
operator|=
name|aps
operator|->
name|aps_seqoff
index|[
name|sel
index|]
operator|+
name|inc
expr_stmt|;
block|}
comment|/***/
name|seq1
operator|=
name|ntohl
argument_list|(
name|tcp
operator|->
name|th_ack
argument_list|)
expr_stmt|;
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
literal|1
operator|-
name|out
index|]
expr_stmt|;
comment|/* switch to other set ? */
if|if
condition|(
operator|(
name|aps
operator|->
name|aps_ackmin
index|[
operator|!
name|sel
index|]
operator|>
name|aps
operator|->
name|aps_ackmin
index|[
name|sel
index|]
operator|)
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_ackmin
index|[
operator|!
name|sel
index|]
operator|)
condition|)
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
literal|1
operator|-
name|out
index|]
operator|=
operator|!
name|sel
expr_stmt|;
if|if
condition|(
name|aps
operator|->
name|aps_ackoff
index|[
name|sel
index|]
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_ackmin
index|[
name|sel
index|]
operator|)
condition|)
block|{
name|seq2
operator|=
name|aps
operator|->
name|aps_ackoff
index|[
name|sel
index|]
expr_stmt|;
name|tcp
operator|->
name|th_ack
operator|=
name|htonl
argument_list|(
name|seq1
operator|-
name|seq2
argument_list|)
expr_stmt|;
name|ch
operator|=
literal|1
expr_stmt|;
block|}
block|}
else|else
block|{
name|seq1
operator|=
name|ntohl
argument_list|(
name|tcp
operator|->
name|th_seq
argument_list|)
expr_stmt|;
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
name|out
index|]
expr_stmt|;
comment|/* switch to other set ? */
if|if
condition|(
operator|(
name|aps
operator|->
name|aps_ackmin
index|[
operator|!
name|sel
index|]
operator|>
name|aps
operator|->
name|aps_ackmin
index|[
name|sel
index|]
operator|)
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_ackmin
index|[
operator|!
name|sel
index|]
operator|)
condition|)
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
name|out
index|]
operator|=
operator|!
name|sel
expr_stmt|;
if|if
condition|(
name|aps
operator|->
name|aps_ackoff
index|[
name|sel
index|]
condition|)
block|{
name|seq2
operator|=
name|aps
operator|->
name|aps_ackmin
index|[
name|sel
index|]
operator|-
name|aps
operator|->
name|aps_ackoff
index|[
name|sel
index|]
expr_stmt|;
if|if
condition|(
name|seq1
operator|>
name|seq2
condition|)
block|{
name|seq2
operator|=
name|aps
operator|->
name|aps_ackoff
index|[
name|sel
index|]
expr_stmt|;
name|seq1
operator|+=
name|seq2
expr_stmt|;
name|tcp
operator|->
name|th_seq
operator|=
name|htonl
argument_list|(
name|seq1
argument_list|)
expr_stmt|;
name|ch
operator|=
literal|1
expr_stmt|;
block|}
block|}
if|if
condition|(
name|inc
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_ackmin
index|[
operator|!
name|sel
index|]
operator|)
condition|)
block|{
name|aps
operator|->
name|aps_ackmin
index|[
operator|!
name|sel
index|]
operator|=
name|seq1
operator|+
name|nlen
operator|-
literal|1
expr_stmt|;
name|aps
operator|->
name|aps_ackoff
index|[
operator|!
name|sel
index|]
operator|=
name|aps
operator|->
name|aps_ackoff
index|[
name|sel
index|]
operator|+
name|inc
expr_stmt|;
block|}
comment|/***/
name|seq1
operator|=
name|ntohl
argument_list|(
name|tcp
operator|->
name|th_ack
argument_list|)
expr_stmt|;
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
literal|1
operator|-
name|out
index|]
expr_stmt|;
comment|/* switch to other set ? */
if|if
condition|(
operator|(
name|aps
operator|->
name|aps_seqmin
index|[
operator|!
name|sel
index|]
operator|>
name|aps
operator|->
name|aps_seqmin
index|[
name|sel
index|]
operator|)
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_seqmin
index|[
operator|!
name|sel
index|]
operator|)
condition|)
name|sel
operator|=
name|aps
operator|->
name|aps_sel
index|[
literal|1
operator|-
name|out
index|]
operator|=
operator|!
name|sel
expr_stmt|;
if|if
condition|(
name|aps
operator|->
name|aps_seqoff
index|[
name|sel
index|]
operator|&&
operator|(
name|seq1
operator|>
name|aps
operator|->
name|aps_seqmin
index|[
name|sel
index|]
operator|)
condition|)
block|{
name|seq2
operator|=
name|aps
operator|->
name|aps_seqoff
index|[
name|sel
index|]
expr_stmt|;
name|tcp
operator|->
name|th_ack
operator|=
name|htonl
argument_list|(
name|seq1
operator|-
name|seq2
argument_list|)
expr_stmt|;
name|ch
operator|=
literal|1
expr_stmt|;
block|}
block|}
return|return
name|ch
condition|?
literal|2
else|:
literal|0
return|;
block|}
end_function

begin_function
name|int
name|appr_init
parameter_list|()
block|{
name|aproxy_t
modifier|*
name|ap
decl_stmt|;
name|int
name|err
init|=
literal|0
decl_stmt|;
for|for
control|(
name|ap
operator|=
name|ap_proxies
init|;
name|ap
operator|->
name|apr_p
condition|;
name|ap
operator|++
control|)
block|{
name|err
operator|=
call|(
modifier|*
name|ap
operator|->
name|apr_init
call|)
argument_list|()
expr_stmt|;
if|if
condition|(
name|err
operator|!=
literal|0
condition|)
break|break;
block|}
return|return
name|err
return|;
block|}
end_function

begin_function
name|void
name|appr_unload
parameter_list|()
block|{
name|aproxy_t
modifier|*
name|ap
decl_stmt|;
for|for
control|(
name|ap
operator|=
name|ap_proxies
init|;
name|ap
operator|->
name|apr_p
condition|;
name|ap
operator|++
control|)
if|if
condition|(
name|ap
operator|->
name|apr_fini
condition|)
call|(
modifier|*
name|ap
operator|->
name|apr_fini
call|)
argument_list|()
expr_stmt|;
for|for
control|(
name|ap
operator|=
name|ap_proxylist
init|;
name|ap
condition|;
name|ap
operator|=
name|ap
operator|->
name|apr_next
control|)
if|if
condition|(
name|ap
operator|->
name|apr_fini
condition|)
call|(
modifier|*
name|ap
operator|->
name|apr_fini
call|)
argument_list|()
expr_stmt|;
block|}
end_function

end_unit

