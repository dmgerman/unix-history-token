begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*	$FreeBSD$	*/
end_comment

begin_comment
comment|/*  * Copyright (C) 2012 by Darren Reed.  *  * See the IPFILTER.LICENCE file for details on licencing.  *  * Added redirect stuff and a variety of bug fixes. (mcn@EnGarde.com)  */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|__SVR4
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|__svr4__
argument_list|)
end_if

begin_include
include|#
directive|include
file|<strings.h>
end_include

begin_else
else|#
directive|else
end_else

begin_include
include|#
directive|include
file|<sys/byteorder.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<stddef.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_define
define|#
directive|define
name|_KERNEL
end_define

begin_include
include|#
directive|include
file|<sys/uio.h>
end_include

begin_undef
undef|#
directive|undef
name|_KERNEL
end_undef

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_if
if|#
directive|if
name|defined
argument_list|(
name|sun
argument_list|)
operator|&&
operator|(
name|defined
argument_list|(
name|__svr4__
argument_list|)
operator|||
name|defined
argument_list|(
name|__SVR4
argument_list|)
operator|)
end_if

begin_include
include|#
directive|include
file|<sys/ioccom.h>
end_include

begin_include
include|#
directive|include
file|<sys/sysmacros.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_systm.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp.h>
end_include

begin_include
include|#
directive|include
file|<net/if.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_include
include|#
directive|include
file|<arpa/nameser.h>
end_include

begin_include
include|#
directive|include
file|<arpa/inet.h>
end_include

begin_include
include|#
directive|include
file|<resolv.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_if
if|#
directive|if
name|defined
argument_list|(
name|linux
argument_list|)
end_if

begin_include
include|#
directive|include
file|<linux/a.out.h>
end_include

begin_else
else|#
directive|else
end_else

begin_include
include|#
directive|include
file|<nlist.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"ipf.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ipl.h"
end_include

begin_include
include|#
directive|include
file|"kmem.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|__hpux
end_ifdef

begin_define
define|#
directive|define
name|nlist
value|nlist64
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|defined
argument_list|(
name|sun
argument_list|)
operator|&&
operator|!
name|SOLARIS2
end_if

begin_define
define|#
directive|define
name|STRERROR
parameter_list|(
name|x
parameter_list|)
value|sys_errlist[x]
end_define

begin_decl_stmt
specifier|extern
name|char
modifier|*
name|sys_errlist
index|[]
decl_stmt|;
end_decl_stmt

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|STRERROR
parameter_list|(
name|x
parameter_list|)
value|strerror(x)
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
end_if

begin_decl_stmt
specifier|static
specifier|const
name|char
name|sccsid
index|[]
init|=
literal|"@(#)ipnat.c	1.9 6/5/96 (C) 1993 Darren Reed"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|char
name|rcsid
index|[]
init|=
literal|"@(#)$Id$"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|SOLARIS
end_if

begin_define
define|#
directive|define
name|bzero
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|memset(a,0,b)
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|int
name|use_inet6
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|thishost
index|[
name|MAXHOSTNAMELEN
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|char
modifier|*
name|optarg
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|dostats
name|__P
argument_list|(
operator|(
name|int
operator|,
name|natstat_t
operator|*
operator|,
name|int
operator|,
name|int
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|dotable
name|__P
argument_list|(
operator|(
name|natstat_t
operator|*
operator|,
name|int
operator|,
name|int
operator|,
name|int
operator|,
name|char
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|flushtable
name|__P
argument_list|(
operator|(
name|int
operator|,
name|int
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|usage
name|__P
argument_list|(
operator|(
name|char
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
decl|main
name|__P
argument_list|(
operator|(
name|int
operator|,
name|char
operator|*
index|[]
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|showhostmap
name|__P
argument_list|(
operator|(
name|natstat_t
operator|*
name|nsp
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|natstat_dead
name|__P
argument_list|(
operator|(
name|natstat_t
operator|*
operator|,
name|char
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|dostats_live
name|__P
argument_list|(
operator|(
name|int
operator|,
name|natstat_t
operator|*
operator|,
name|int
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|showhostmap_dead
name|__P
argument_list|(
operator|(
name|natstat_t
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|showhostmap_live
name|__P
argument_list|(
operator|(
name|int
operator|,
name|natstat_t
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|dostats_dead
name|__P
argument_list|(
operator|(
name|natstat_t
operator|*
operator|,
name|int
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nat_matcharray
name|__P
argument_list|(
operator|(
name|nat_t
operator|*
operator|,
name|int
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|opts
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nohdrfields
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|wordtab_t
modifier|*
name|nat_fields
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_function
name|void
name|usage
parameter_list|(
name|name
parameter_list|)
name|char
modifier|*
name|name
decl_stmt|;
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Usage: %s [-CFhlnrRsv] [-f filename]\n"
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|int
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
name|int
name|fd
decl_stmt|,
name|c
decl_stmt|,
name|mode
decl_stmt|,
modifier|*
name|natfilter
decl_stmt|;
name|char
modifier|*
name|file
decl_stmt|,
modifier|*
name|core
decl_stmt|,
modifier|*
name|kernel
decl_stmt|;
name|natstat_t
name|ns
decl_stmt|,
modifier|*
name|nsp
decl_stmt|;
name|ipfobj_t
name|obj
decl_stmt|;
name|fd
operator|=
operator|-
literal|1
expr_stmt|;
name|opts
operator|=
literal|0
expr_stmt|;
name|nsp
operator|=
operator|&
name|ns
expr_stmt|;
name|file
operator|=
name|NULL
expr_stmt|;
name|core
operator|=
name|NULL
expr_stmt|;
name|kernel
operator|=
name|NULL
expr_stmt|;
name|mode
operator|=
name|O_RDWR
expr_stmt|;
name|natfilter
operator|=
name|NULL
expr_stmt|;
name|assigndefined
argument_list|(
name|getenv
argument_list|(
literal|"IPNAT_PREDEFINED"
argument_list|)
argument_list|)
expr_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"CdFf:hlm:M:N:nO:prRsv"
argument_list|)
operator|)
operator|!=
operator|-
literal|1
condition|)
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'C'
case|:
name|opts
operator||=
name|OPT_CLEAR
expr_stmt|;
break|break;
case|case
literal|'d'
case|:
name|opts
operator||=
name|OPT_DEBUG
expr_stmt|;
break|break;
case|case
literal|'f'
case|:
name|file
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'F'
case|:
name|opts
operator||=
name|OPT_FLUSH
expr_stmt|;
break|break;
case|case
literal|'h'
case|:
name|opts
operator||=
name|OPT_HITS
expr_stmt|;
break|break;
case|case
literal|'l'
case|:
name|opts
operator||=
name|OPT_LIST
expr_stmt|;
name|mode
operator|=
name|O_RDONLY
expr_stmt|;
break|break;
case|case
literal|'m'
case|:
name|natfilter
operator|=
name|parseipfexpr
argument_list|(
name|optarg
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'M'
case|:
name|core
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'N'
case|:
name|kernel
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'n'
case|:
name|opts
operator||=
name|OPT_DONOTHING
operator||
name|OPT_DONTOPEN
expr_stmt|;
name|mode
operator|=
name|O_RDONLY
expr_stmt|;
break|break;
case|case
literal|'O'
case|:
name|nat_fields
operator|=
name|parsefields
argument_list|(
name|natfields
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'p'
case|:
name|opts
operator||=
name|OPT_PURGE
expr_stmt|;
break|break;
case|case
literal|'R'
case|:
name|opts
operator||=
name|OPT_NORESOLVE
expr_stmt|;
break|break;
case|case
literal|'r'
case|:
name|opts
operator||=
name|OPT_REMOVE
expr_stmt|;
break|break;
case|case
literal|'s'
case|:
name|opts
operator||=
name|OPT_STAT
expr_stmt|;
name|mode
operator|=
name|O_RDONLY
expr_stmt|;
break|break;
case|case
literal|'v'
case|:
name|opts
operator||=
name|OPT_VERBOSE
expr_stmt|;
break|break;
default|default :
name|usage
argument_list|(
name|argv
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
operator|(
name|opts
operator|&
name|OPT_PURGE
operator|)
operator|!=
literal|0
operator|)
operator|&&
operator|(
operator|(
name|opts
operator|&
name|OPT_REMOVE
operator|)
operator|==
literal|0
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: -p must be used with -r\n"
argument_list|,
name|argv
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|initparse
argument_list|()
expr_stmt|;
if|if
condition|(
operator|(
name|kernel
operator|!=
name|NULL
operator|)
operator|||
operator|(
name|core
operator|!=
name|NULL
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|setgid
argument_list|(
name|getgid
argument_list|()
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|setuid
argument_list|(
name|getuid
argument_list|()
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
operator|(
name|opts
operator|&
name|OPT_DONOTHING
operator|)
condition|)
block|{
if|if
condition|(
operator|(
operator|(
name|fd
operator|=
name|open
argument_list|(
name|IPNAT_NAME
argument_list|,
name|mode
argument_list|)
operator|)
operator|==
operator|-
literal|1
operator|)
operator|&&
operator|(
operator|(
name|fd
operator|=
name|open
argument_list|(
name|IPNAT_NAME
argument_list|,
name|O_RDONLY
argument_list|)
operator|)
operator|==
operator|-
literal|1
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: open: %s\n"
argument_list|,
name|IPNAT_NAME
argument_list|,
name|STRERROR
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|ns
argument_list|,
sizeof|sizeof
argument_list|(
name|ns
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|opts
operator|&
name|OPT_DONOTHING
operator|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|checkrev
argument_list|(
name|IPL_NAME
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"User/kernel version check failed\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
operator|!
operator|(
name|opts
operator|&
name|OPT_DONOTHING
operator|)
operator|&&
operator|(
name|kernel
operator|==
name|NULL
operator|)
operator|&&
operator|(
name|core
operator|==
name|NULL
operator|)
condition|)
block|{
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|obj
argument_list|,
sizeof|sizeof
argument_list|(
name|obj
argument_list|)
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_rev
operator|=
name|IPFILTER_VERSION
expr_stmt|;
name|obj
operator|.
name|ipfo_type
operator|=
name|IPFOBJ_NATSTAT
expr_stmt|;
name|obj
operator|.
name|ipfo_size
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|nsp
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_ptr
operator|=
operator|(
name|void
operator|*
operator|)
name|nsp
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCGNATS
argument_list|,
operator|&
name|obj
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|ipferror
argument_list|(
name|fd
argument_list|,
literal|"ioctl(SIOCGNATS)"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|setgid
argument_list|(
name|getgid
argument_list|()
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|setuid
argument_list|(
name|getuid
argument_list|()
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|(
name|kernel
operator|!=
name|NULL
operator|)
operator|||
operator|(
name|core
operator|!=
name|NULL
operator|)
condition|)
block|{
if|if
condition|(
name|openkmem
argument_list|(
name|kernel
argument_list|,
name|core
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|natstat_dead
argument_list|(
name|nsp
argument_list|,
name|kernel
argument_list|)
expr_stmt|;
if|if
condition|(
name|opts
operator|&
operator|(
name|OPT_LIST
operator||
name|OPT_STAT
operator|)
condition|)
name|dostats
argument_list|(
name|fd
argument_list|,
name|nsp
argument_list|,
name|opts
argument_list|,
literal|0
argument_list|,
name|natfilter
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|opts
operator|&
operator|(
name|OPT_FLUSH
operator||
name|OPT_CLEAR
operator|)
condition|)
name|flushtable
argument_list|(
name|fd
argument_list|,
name|opts
argument_list|,
name|natfilter
argument_list|)
expr_stmt|;
if|if
condition|(
name|file
condition|)
block|{
return|return
name|ipnat_parsefile
argument_list|(
name|fd
argument_list|,
name|ipnat_addrule
argument_list|,
name|ioctl
argument_list|,
name|file
argument_list|)
return|;
block|}
if|if
condition|(
name|opts
operator|&
operator|(
name|OPT_LIST
operator||
name|OPT_STAT
operator|)
condition|)
name|dostats
argument_list|(
name|fd
argument_list|,
name|nsp
argument_list|,
name|opts
argument_list|,
literal|1
argument_list|,
name|natfilter
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

begin_comment
comment|/*  * Read NAT statistic information in using a symbol table and memory file  * rather than doing ioctl's.  */
end_comment

begin_function
name|void
name|natstat_dead
parameter_list|(
name|nsp
parameter_list|,
name|kernel
parameter_list|)
name|natstat_t
modifier|*
name|nsp
decl_stmt|;
name|char
modifier|*
name|kernel
decl_stmt|;
block|{
name|struct
name|nlist
name|nat_nlist
index|[
literal|10
index|]
init|=
block|{
block|{
literal|"nat_table"
block|}
block|,
comment|/* 0 */
block|{
literal|"nat_list"
block|}
block|,
block|{
literal|"maptable"
block|}
block|,
block|{
literal|"ipf_nattable_sz"
block|}
block|,
block|{
literal|"ipf_natrules_sz"
block|}
block|,
block|{
literal|"ipf_rdrrules_sz"
block|}
block|,
comment|/* 5 */
block|{
literal|"ipf_hostmap_sz"
block|}
block|,
block|{
literal|"nat_instances"
block|}
block|,
block|{
name|NULL
block|}
block|}
decl_stmt|;
name|void
modifier|*
name|tables
index|[
literal|2
index|]
decl_stmt|;
if|if
condition|(
name|nlist
argument_list|(
name|kernel
argument_list|,
name|nat_nlist
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"nlist error\n"
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 	 * Normally the ioctl copies all of these values into the structure 	 * for us, before returning it to userland, so here we must copy each 	 * one in individually. 	 */
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|tables
argument_list|,
name|nat_nlist
index|[
literal|0
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|tables
argument_list|)
argument_list|)
expr_stmt|;
name|nsp
operator|->
name|ns_side
index|[
literal|0
index|]
operator|.
name|ns_table
operator|=
name|tables
index|[
literal|0
index|]
expr_stmt|;
name|nsp
operator|->
name|ns_side
index|[
literal|1
index|]
operator|.
name|ns_table
operator|=
name|tables
index|[
literal|1
index|]
expr_stmt|;
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nsp
operator|->
name|ns_list
argument_list|,
name|nat_nlist
index|[
literal|1
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|nsp
operator|->
name|ns_list
argument_list|)
argument_list|)
expr_stmt|;
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nsp
operator|->
name|ns_maptable
argument_list|,
name|nat_nlist
index|[
literal|2
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|nsp
operator|->
name|ns_maptable
argument_list|)
argument_list|)
expr_stmt|;
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nsp
operator|->
name|ns_nattab_sz
argument_list|,
name|nat_nlist
index|[
literal|3
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|nsp
operator|->
name|ns_nattab_sz
argument_list|)
argument_list|)
expr_stmt|;
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nsp
operator|->
name|ns_rultab_sz
argument_list|,
name|nat_nlist
index|[
literal|4
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|nsp
operator|->
name|ns_rultab_sz
argument_list|)
argument_list|)
expr_stmt|;
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nsp
operator|->
name|ns_rdrtab_sz
argument_list|,
name|nat_nlist
index|[
literal|5
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|nsp
operator|->
name|ns_rdrtab_sz
argument_list|)
argument_list|)
expr_stmt|;
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nsp
operator|->
name|ns_hostmap_sz
argument_list|,
name|nat_nlist
index|[
literal|6
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|nsp
operator|->
name|ns_hostmap_sz
argument_list|)
argument_list|)
expr_stmt|;
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nsp
operator|->
name|ns_instances
argument_list|,
name|nat_nlist
index|[
literal|7
index|]
operator|.
name|n_value
argument_list|,
sizeof|sizeof
argument_list|(
name|nsp
operator|->
name|ns_instances
argument_list|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Issue an ioctl to flush either the NAT rules table or the active mapping  * table or both.  */
end_comment

begin_function
name|void
name|flushtable
parameter_list|(
name|fd
parameter_list|,
name|opts
parameter_list|,
name|match
parameter_list|)
name|int
name|fd
decl_stmt|,
name|opts
decl_stmt|,
decl|*
name|match
decl_stmt|;
end_function

begin_block
block|{
name|int
name|n
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|opts
operator|&
name|OPT_FLUSH
condition|)
block|{
name|n
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|opts
operator|&
name|OPT_DONOTHING
operator|)
condition|)
block|{
if|if
condition|(
name|match
operator|!=
name|NULL
condition|)
block|{
name|ipfobj_t
name|obj
decl_stmt|;
name|obj
operator|.
name|ipfo_rev
operator|=
name|IPFILTER_VERSION
expr_stmt|;
name|obj
operator|.
name|ipfo_size
operator|=
name|match
index|[
literal|0
index|]
operator|*
sizeof|sizeof
argument_list|(
name|int
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_type
operator|=
name|IPFOBJ_IPFEXPR
expr_stmt|;
name|obj
operator|.
name|ipfo_ptr
operator|=
name|match
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCMATCHFLUSH
argument_list|,
operator|&
name|obj
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|ipferror
argument_list|(
name|fd
argument_list|,
literal|"ioctl(SIOCMATCHFLUSH)"
argument_list|)
expr_stmt|;
name|n
operator|=
operator|-
literal|1
expr_stmt|;
block|}
else|else
block|{
name|n
operator|=
name|obj
operator|.
name|ipfo_retval
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCIPFFL
argument_list|,
operator|&
name|n
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|ipferror
argument_list|(
name|fd
argument_list|,
literal|"ioctl(SIOCIPFFL)"
argument_list|)
expr_stmt|;
name|n
operator|=
operator|-
literal|1
expr_stmt|;
block|}
block|}
if|if
condition|(
name|n
operator|>=
literal|0
condition|)
name|printf
argument_list|(
literal|"%d entries flushed from NAT table\n"
argument_list|,
name|n
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|opts
operator|&
name|OPT_CLEAR
condition|)
block|{
name|n
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|opts
operator|&
name|OPT_DONOTHING
operator|)
operator|&&
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCIPFFL
argument_list|,
operator|&
name|n
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|ipferror
argument_list|(
name|fd
argument_list|,
literal|"ioctl(SIOCCNATL)"
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"%d entries flushed from NAT list\n"
argument_list|,
name|n
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Display NAT statistics.  */
end_comment

begin_function
name|void
name|dostats_dead
parameter_list|(
name|nsp
parameter_list|,
name|opts
parameter_list|,
name|filter
parameter_list|)
name|natstat_t
modifier|*
name|nsp
decl_stmt|;
name|int
name|opts
decl_stmt|,
decl|*
name|filter
decl_stmt|;
end_function

begin_block
block|{
name|nat_t
modifier|*
name|np
decl_stmt|,
name|nat
decl_stmt|;
name|ipnat_t
name|ipn
decl_stmt|;
name|int
name|i
decl_stmt|;
if|if
condition|(
name|nat_fields
operator|==
name|NULL
condition|)
block|{
name|printf
argument_list|(
literal|"List of active MAP/Redirect filters:\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|nsp
operator|->
name|ns_list
condition|)
block|{
if|if
condition|(
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|ipn
argument_list|,
operator|(
name|long
operator|)
name|nsp
operator|->
name|ns_list
argument_list|,
sizeof|sizeof
argument_list|(
name|ipn
argument_list|)
argument_list|)
condition|)
block|{
name|perror
argument_list|(
literal|"kmemcpy"
argument_list|)
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|opts
operator|&
name|OPT_HITS
condition|)
name|printf
argument_list|(
literal|"%lu "
argument_list|,
name|ipn
operator|.
name|in_hits
argument_list|)
expr_stmt|;
name|printnat
argument_list|(
operator|&
name|ipn
argument_list|,
name|opts
operator|&
operator|(
name|OPT_DEBUG
operator||
name|OPT_VERBOSE
operator|)
argument_list|)
expr_stmt|;
name|nsp
operator|->
name|ns_list
operator|=
name|ipn
operator|.
name|in_next
expr_stmt|;
block|}
block|}
if|if
condition|(
name|nat_fields
operator|==
name|NULL
condition|)
block|{
name|printf
argument_list|(
literal|"\nList of active sessions:\n"
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|nohdrfields
operator|==
literal|0
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|nat_fields
index|[
name|i
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|;
name|i
operator|++
control|)
block|{
name|printfieldhdr
argument_list|(
name|natfields
argument_list|,
name|nat_fields
operator|+
name|i
argument_list|)
expr_stmt|;
if|if
condition|(
name|nat_fields
index|[
name|i
operator|+
literal|1
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|)
name|printf
argument_list|(
literal|"\t"
argument_list|)
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
for|for
control|(
name|np
operator|=
name|nsp
operator|->
name|ns_instances
init|;
name|np
condition|;
name|np
operator|=
name|nat
operator|.
name|nat_next
control|)
block|{
if|if
condition|(
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|nat
argument_list|,
operator|(
name|long
operator|)
name|np
argument_list|,
sizeof|sizeof
argument_list|(
name|nat
argument_list|)
argument_list|)
condition|)
break|break;
if|if
condition|(
operator|(
name|filter
operator|!=
name|NULL
operator|)
operator|&&
operator|(
name|nat_matcharray
argument_list|(
operator|&
name|nat
argument_list|,
name|filter
argument_list|)
operator|==
literal|0
operator|)
condition|)
continue|continue;
if|if
condition|(
name|nat_fields
operator|!=
name|NULL
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|nat_fields
index|[
name|i
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|;
name|i
operator|++
control|)
block|{
name|printnatfield
argument_list|(
operator|&
name|nat
argument_list|,
name|nat_fields
index|[
name|i
index|]
operator|.
name|w_value
argument_list|)
expr_stmt|;
if|if
condition|(
name|nat_fields
index|[
name|i
operator|+
literal|1
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|)
name|printf
argument_list|(
literal|"\t"
argument_list|)
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|printactivenat
argument_list|(
operator|&
name|nat
argument_list|,
name|opts
argument_list|,
name|nsp
operator|->
name|ns_ticks
argument_list|)
expr_stmt|;
if|if
condition|(
name|nat
operator|.
name|nat_aps
condition|)
block|{
name|int
name|proto
decl_stmt|;
if|if
condition|(
name|nat
operator|.
name|nat_dir
operator|&
name|NAT_OUTBOUND
condition|)
name|proto
operator|=
name|nat
operator|.
name|nat_pr
index|[
literal|1
index|]
expr_stmt|;
else|else
name|proto
operator|=
name|nat
operator|.
name|nat_pr
index|[
literal|0
index|]
expr_stmt|;
name|printaps
argument_list|(
name|nat
operator|.
name|nat_aps
argument_list|,
name|opts
argument_list|,
name|proto
argument_list|)
expr_stmt|;
block|}
block|}
block|}
if|if
condition|(
name|opts
operator|&
name|OPT_VERBOSE
condition|)
name|showhostmap_dead
argument_list|(
name|nsp
argument_list|)
expr_stmt|;
block|}
end_block

begin_function
name|void
name|dotable
parameter_list|(
name|nsp
parameter_list|,
name|fd
parameter_list|,
name|alive
parameter_list|,
name|which
parameter_list|,
name|side
parameter_list|)
name|natstat_t
modifier|*
name|nsp
decl_stmt|;
name|int
name|fd
decl_stmt|,
name|alive
decl_stmt|,
name|which
decl_stmt|;
name|char
modifier|*
name|side
decl_stmt|;
block|{
name|int
name|sz
decl_stmt|,
name|i
decl_stmt|,
name|used
decl_stmt|,
name|maxlen
decl_stmt|,
name|minlen
decl_stmt|,
name|totallen
decl_stmt|;
name|ipftable_t
name|table
decl_stmt|;
name|u_int
modifier|*
name|buckets
decl_stmt|;
name|ipfobj_t
name|obj
decl_stmt|;
name|sz
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|buckets
argument_list|)
operator|*
name|nsp
operator|->
name|ns_nattab_sz
expr_stmt|;
name|buckets
operator|=
operator|(
name|u_int
operator|*
operator|)
name|malloc
argument_list|(
name|sz
argument_list|)
expr_stmt|;
if|if
condition|(
name|buckets
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"cannot allocate memory (%d) for buckets\n"
argument_list|,
name|sz
argument_list|)
expr_stmt|;
return|return;
block|}
name|obj
operator|.
name|ipfo_rev
operator|=
name|IPFILTER_VERSION
expr_stmt|;
name|obj
operator|.
name|ipfo_type
operator|=
name|IPFOBJ_GTABLE
expr_stmt|;
name|obj
operator|.
name|ipfo_size
operator|=
sizeof|sizeof
argument_list|(
name|table
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_ptr
operator|=
operator|&
name|table
expr_stmt|;
if|if
condition|(
name|which
operator|==
literal|0
condition|)
block|{
name|table
operator|.
name|ita_type
operator|=
name|IPFTABLE_BUCKETS_NATIN
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|which
operator|==
literal|1
condition|)
block|{
name|table
operator|.
name|ita_type
operator|=
name|IPFTABLE_BUCKETS_NATOUT
expr_stmt|;
block|}
name|table
operator|.
name|ita_table
operator|=
name|buckets
expr_stmt|;
if|if
condition|(
name|alive
condition|)
block|{
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCGTABL
argument_list|,
operator|&
name|obj
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|ipferror
argument_list|(
name|fd
argument_list|,
literal|"SIOCFTABL"
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buckets
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
else|else
block|{
if|if
condition|(
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
name|buckets
argument_list|,
operator|(
name|u_long
operator|)
name|nsp
operator|->
name|ns_nattab_sz
argument_list|,
name|sz
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|buckets
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
name|minlen
operator|=
name|nsp
operator|->
name|ns_side
index|[
name|which
index|]
operator|.
name|ns_inuse
expr_stmt|;
name|totallen
operator|=
literal|0
expr_stmt|;
name|maxlen
operator|=
literal|0
expr_stmt|;
name|used
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|nsp
operator|->
name|ns_nattab_sz
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|buckets
index|[
name|i
index|]
operator|>
name|maxlen
condition|)
name|maxlen
operator|=
name|buckets
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|buckets
index|[
name|i
index|]
operator|<
name|minlen
condition|)
name|minlen
operator|=
name|buckets
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|buckets
index|[
name|i
index|]
operator|!=
literal|0
condition|)
name|used
operator|++
expr_stmt|;
name|totallen
operator|+=
name|buckets
index|[
name|i
index|]
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"%d%%\thash efficiency %s\n"
argument_list|,
name|totallen
condition|?
name|used
operator|*
literal|100
operator|/
name|totallen
else|:
literal|0
argument_list|,
name|side
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%2.2f%%\tbucket usage %s\n"
argument_list|,
operator|(
operator|(
name|float
operator|)
name|used
operator|/
name|nsp
operator|->
name|ns_nattab_sz
operator|)
operator|*
literal|100.0
argument_list|,
name|side
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%d\tminimal length %s\n"
argument_list|,
name|minlen
argument_list|,
name|side
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%d\tmaximal length %s\n"
argument_list|,
name|maxlen
argument_list|,
name|side
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%.3f\taverage length %s\n"
argument_list|,
name|used
condition|?
operator|(
operator|(
name|float
operator|)
name|totallen
operator|/
name|used
operator|)
else|:
literal|0.0
argument_list|,
name|side
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buckets
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|dostats
parameter_list|(
name|fd
parameter_list|,
name|nsp
parameter_list|,
name|opts
parameter_list|,
name|alive
parameter_list|,
name|filter
parameter_list|)
name|natstat_t
modifier|*
name|nsp
decl_stmt|;
name|int
name|fd
decl_stmt|,
name|opts
decl_stmt|,
name|alive
decl_stmt|,
decl|*
name|filter
decl_stmt|;
end_function

begin_block
block|{
comment|/* 	 * Show statistics ? 	 */
if|if
condition|(
name|opts
operator|&
name|OPT_STAT
condition|)
block|{
name|printnatside
argument_list|(
literal|"in"
argument_list|,
operator|&
name|nsp
operator|->
name|ns_side
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|dotable
argument_list|(
name|nsp
argument_list|,
name|fd
argument_list|,
name|alive
argument_list|,
literal|0
argument_list|,
literal|"in"
argument_list|)
expr_stmt|;
name|printnatside
argument_list|(
literal|"out"
argument_list|,
operator|&
name|nsp
operator|->
name|ns_side
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|dotable
argument_list|(
name|nsp
argument_list|,
name|fd
argument_list|,
name|alive
argument_list|,
literal|1
argument_list|,
literal|"out"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tlog successes\n"
argument_list|,
name|nsp
operator|->
name|ns_side
index|[
literal|0
index|]
operator|.
name|ns_log
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tlog failures\n"
argument_list|,
name|nsp
operator|->
name|ns_side
index|[
literal|1
index|]
operator|.
name|ns_log
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tadded in\n%lu\tadded out\n"
argument_list|,
name|nsp
operator|->
name|ns_side
index|[
literal|0
index|]
operator|.
name|ns_added
argument_list|,
name|nsp
operator|->
name|ns_side
index|[
literal|1
index|]
operator|.
name|ns_added
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u\tactive\n"
argument_list|,
name|nsp
operator|->
name|ns_active
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\ttransparent adds\n"
argument_list|,
name|nsp
operator|->
name|ns_addtrpnt
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tdivert build\n"
argument_list|,
name|nsp
operator|->
name|ns_divert_build
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\texpired\n"
argument_list|,
name|nsp
operator|->
name|ns_expire
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tflush all\n"
argument_list|,
name|nsp
operator|->
name|ns_flush_all
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tflush closing\n"
argument_list|,
name|nsp
operator|->
name|ns_flush_closing
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tflush queue\n"
argument_list|,
name|nsp
operator|->
name|ns_flush_queue
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tflush state\n"
argument_list|,
name|nsp
operator|->
name|ns_flush_state
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tflush timeout\n"
argument_list|,
name|nsp
operator|->
name|ns_flush_timeout
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\thostmap new\n"
argument_list|,
name|nsp
operator|->
name|ns_hm_new
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\thostmap fails\n"
argument_list|,
name|nsp
operator|->
name|ns_hm_newfail
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\thostmap add\n"
argument_list|,
name|nsp
operator|->
name|ns_hm_addref
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\thostmap NULL rule\n"
argument_list|,
name|nsp
operator|->
name|ns_hm_nullnp
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tlog ok\n"
argument_list|,
name|nsp
operator|->
name|ns_log_ok
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%lu\tlog fail\n"
argument_list|,
name|nsp
operator|->
name|ns_log_fail
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u\torphan count\n"
argument_list|,
name|nsp
operator|->
name|ns_orphans
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u\trule count\n"
argument_list|,
name|nsp
operator|->
name|ns_rules
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u\tmap rules\n"
argument_list|,
name|nsp
operator|->
name|ns_rules_map
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u\trdr rules\n"
argument_list|,
name|nsp
operator|->
name|ns_rules_rdr
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u\twilds\n"
argument_list|,
name|nsp
operator|->
name|ns_wilds
argument_list|)
expr_stmt|;
if|if
condition|(
name|opts
operator|&
name|OPT_VERBOSE
condition|)
name|printf
argument_list|(
literal|"list %p\n"
argument_list|,
name|nsp
operator|->
name|ns_list
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|opts
operator|&
name|OPT_LIST
condition|)
block|{
if|if
condition|(
name|alive
condition|)
name|dostats_live
argument_list|(
name|fd
argument_list|,
name|nsp
argument_list|,
name|opts
argument_list|,
name|filter
argument_list|)
expr_stmt|;
else|else
name|dostats_dead
argument_list|(
name|nsp
argument_list|,
name|opts
argument_list|,
name|filter
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Display NAT statistics.  */
end_comment

begin_function
name|void
name|dostats_live
parameter_list|(
name|fd
parameter_list|,
name|nsp
parameter_list|,
name|opts
parameter_list|,
name|filter
parameter_list|)
name|natstat_t
modifier|*
name|nsp
decl_stmt|;
name|int
name|fd
decl_stmt|,
name|opts
decl_stmt|,
decl|*
name|filter
decl_stmt|;
end_function

begin_block
block|{
name|ipfgeniter_t
name|iter
decl_stmt|;
name|char
name|buffer
index|[
literal|2000
index|]
decl_stmt|;
name|ipfobj_t
name|obj
decl_stmt|;
name|ipnat_t
modifier|*
name|ipn
decl_stmt|;
name|nat_t
name|nat
decl_stmt|;
name|int
name|i
decl_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|obj
argument_list|,
sizeof|sizeof
argument_list|(
name|obj
argument_list|)
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_rev
operator|=
name|IPFILTER_VERSION
expr_stmt|;
name|obj
operator|.
name|ipfo_type
operator|=
name|IPFOBJ_GENITER
expr_stmt|;
name|obj
operator|.
name|ipfo_size
operator|=
sizeof|sizeof
argument_list|(
name|iter
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_ptr
operator|=
operator|&
name|iter
expr_stmt|;
name|iter
operator|.
name|igi_type
operator|=
name|IPFGENITER_IPNAT
expr_stmt|;
name|iter
operator|.
name|igi_nitems
operator|=
literal|1
expr_stmt|;
name|iter
operator|.
name|igi_data
operator|=
name|buffer
expr_stmt|;
name|ipn
operator|=
operator|(
name|ipnat_t
operator|*
operator|)
name|buffer
expr_stmt|;
comment|/* 	 * Show list of NAT rules and NAT sessions ? 	 */
if|if
condition|(
name|nat_fields
operator|==
name|NULL
condition|)
block|{
name|printf
argument_list|(
literal|"List of active MAP/Redirect filters:\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|nsp
operator|->
name|ns_list
condition|)
block|{
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCGENITER
argument_list|,
operator|&
name|obj
argument_list|)
operator|==
operator|-
literal|1
condition|)
break|break;
if|if
condition|(
name|opts
operator|&
name|OPT_HITS
condition|)
name|printf
argument_list|(
literal|"%lu "
argument_list|,
name|ipn
operator|->
name|in_hits
argument_list|)
expr_stmt|;
name|printnat
argument_list|(
name|ipn
argument_list|,
name|opts
operator|&
operator|(
name|OPT_DEBUG
operator||
name|OPT_VERBOSE
operator|)
argument_list|)
expr_stmt|;
name|nsp
operator|->
name|ns_list
operator|=
name|ipn
operator|->
name|in_next
expr_stmt|;
block|}
block|}
if|if
condition|(
name|nat_fields
operator|==
name|NULL
condition|)
block|{
name|printf
argument_list|(
literal|"\nList of active sessions:\n"
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|nohdrfields
operator|==
literal|0
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|nat_fields
index|[
name|i
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|;
name|i
operator|++
control|)
block|{
name|printfieldhdr
argument_list|(
name|natfields
argument_list|,
name|nat_fields
operator|+
name|i
argument_list|)
expr_stmt|;
if|if
condition|(
name|nat_fields
index|[
name|i
operator|+
literal|1
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|)
name|printf
argument_list|(
literal|"\t"
argument_list|)
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
name|i
operator|=
name|IPFGENITER_IPNAT
expr_stmt|;
operator|(
name|void
operator|)
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCIPFDELTOK
argument_list|,
operator|&
name|i
argument_list|)
expr_stmt|;
name|iter
operator|.
name|igi_type
operator|=
name|IPFGENITER_NAT
expr_stmt|;
name|iter
operator|.
name|igi_nitems
operator|=
literal|1
expr_stmt|;
name|iter
operator|.
name|igi_data
operator|=
operator|&
name|nat
expr_stmt|;
while|while
condition|(
name|nsp
operator|->
name|ns_instances
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCGENITER
argument_list|,
operator|&
name|obj
argument_list|)
operator|==
operator|-
literal|1
condition|)
break|break;
if|if
condition|(
operator|(
name|filter
operator|!=
name|NULL
operator|)
operator|&&
operator|(
name|nat_matcharray
argument_list|(
operator|&
name|nat
argument_list|,
name|filter
argument_list|)
operator|==
literal|0
operator|)
condition|)
continue|continue;
if|if
condition|(
name|nat_fields
operator|!=
name|NULL
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|nat_fields
index|[
name|i
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|;
name|i
operator|++
control|)
block|{
name|printnatfield
argument_list|(
operator|&
name|nat
argument_list|,
name|nat_fields
index|[
name|i
index|]
operator|.
name|w_value
argument_list|)
expr_stmt|;
if|if
condition|(
name|nat_fields
index|[
name|i
operator|+
literal|1
index|]
operator|.
name|w_value
operator|!=
literal|0
condition|)
name|printf
argument_list|(
literal|"\t"
argument_list|)
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|printactivenat
argument_list|(
operator|&
name|nat
argument_list|,
name|opts
argument_list|,
name|nsp
operator|->
name|ns_ticks
argument_list|)
expr_stmt|;
if|if
condition|(
name|nat
operator|.
name|nat_aps
condition|)
block|{
name|int
name|proto
decl_stmt|;
if|if
condition|(
name|nat
operator|.
name|nat_dir
operator|&
name|NAT_OUTBOUND
condition|)
name|proto
operator|=
name|nat
operator|.
name|nat_pr
index|[
literal|1
index|]
expr_stmt|;
else|else
name|proto
operator|=
name|nat
operator|.
name|nat_pr
index|[
literal|0
index|]
expr_stmt|;
name|printaps
argument_list|(
name|nat
operator|.
name|nat_aps
argument_list|,
name|opts
argument_list|,
name|proto
argument_list|)
expr_stmt|;
block|}
block|}
name|nsp
operator|->
name|ns_instances
operator|=
name|nat
operator|.
name|nat_next
expr_stmt|;
block|}
if|if
condition|(
name|opts
operator|&
name|OPT_VERBOSE
condition|)
name|showhostmap_live
argument_list|(
name|fd
argument_list|,
name|nsp
argument_list|)
expr_stmt|;
name|i
operator|=
name|IPFGENITER_NAT
expr_stmt|;
operator|(
name|void
operator|)
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCIPFDELTOK
argument_list|,
operator|&
name|i
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Display the active host mapping table.  */
end_comment

begin_function
name|void
name|showhostmap_dead
parameter_list|(
name|nsp
parameter_list|)
name|natstat_t
modifier|*
name|nsp
decl_stmt|;
block|{
name|hostmap_t
name|hm
decl_stmt|,
modifier|*
name|hmp
decl_stmt|,
modifier|*
modifier|*
name|maptable
decl_stmt|;
name|u_int
name|hv
decl_stmt|;
name|printf
argument_list|(
literal|"\nList of active host mappings:\n"
argument_list|)
expr_stmt|;
name|maptable
operator|=
operator|(
name|hostmap_t
operator|*
operator|*
operator|)
name|malloc
argument_list|(
sizeof|sizeof
argument_list|(
name|hostmap_t
operator|*
argument_list|)
operator|*
name|nsp
operator|->
name|ns_hostmap_sz
argument_list|)
expr_stmt|;
if|if
condition|(
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
name|maptable
argument_list|,
operator|(
name|u_long
operator|)
name|nsp
operator|->
name|ns_maptable
argument_list|,
sizeof|sizeof
argument_list|(
name|hostmap_t
operator|*
argument_list|)
operator|*
name|nsp
operator|->
name|ns_hostmap_sz
argument_list|)
condition|)
block|{
name|perror
argument_list|(
literal|"kmemcpy (maptable)"
argument_list|)
expr_stmt|;
return|return;
block|}
for|for
control|(
name|hv
operator|=
literal|0
init|;
name|hv
operator|<
name|nsp
operator|->
name|ns_hostmap_sz
condition|;
name|hv
operator|++
control|)
block|{
name|hmp
operator|=
name|maptable
index|[
name|hv
index|]
expr_stmt|;
while|while
condition|(
name|hmp
condition|)
block|{
if|if
condition|(
name|kmemcpy
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|hm
argument_list|,
operator|(
name|u_long
operator|)
name|hmp
argument_list|,
sizeof|sizeof
argument_list|(
name|hm
argument_list|)
argument_list|)
condition|)
block|{
name|perror
argument_list|(
literal|"kmemcpy (hostmap)"
argument_list|)
expr_stmt|;
return|return;
block|}
name|printhostmap
argument_list|(
operator|&
name|hm
argument_list|,
name|hv
argument_list|)
expr_stmt|;
name|hmp
operator|=
name|hm
operator|.
name|hm_next
expr_stmt|;
block|}
block|}
name|free
argument_list|(
name|maptable
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Display the active host mapping table.  */
end_comment

begin_function
name|void
name|showhostmap_live
parameter_list|(
name|fd
parameter_list|,
name|nsp
parameter_list|)
name|int
name|fd
decl_stmt|;
name|natstat_t
modifier|*
name|nsp
decl_stmt|;
block|{
name|ipfgeniter_t
name|iter
decl_stmt|;
name|hostmap_t
name|hm
decl_stmt|;
name|ipfobj_t
name|obj
decl_stmt|;
name|int
name|i
decl_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|obj
argument_list|,
sizeof|sizeof
argument_list|(
name|obj
argument_list|)
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_rev
operator|=
name|IPFILTER_VERSION
expr_stmt|;
name|obj
operator|.
name|ipfo_type
operator|=
name|IPFOBJ_GENITER
expr_stmt|;
name|obj
operator|.
name|ipfo_size
operator|=
sizeof|sizeof
argument_list|(
name|iter
argument_list|)
expr_stmt|;
name|obj
operator|.
name|ipfo_ptr
operator|=
operator|&
name|iter
expr_stmt|;
name|iter
operator|.
name|igi_type
operator|=
name|IPFGENITER_HOSTMAP
expr_stmt|;
name|iter
operator|.
name|igi_nitems
operator|=
literal|1
expr_stmt|;
name|iter
operator|.
name|igi_data
operator|=
operator|&
name|hm
expr_stmt|;
name|printf
argument_list|(
literal|"\nList of active host mappings:\n"
argument_list|)
expr_stmt|;
while|while
condition|(
name|nsp
operator|->
name|ns_maplist
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCGENITER
argument_list|,
operator|&
name|obj
argument_list|)
operator|==
operator|-
literal|1
condition|)
break|break;
name|printhostmap
argument_list|(
operator|&
name|hm
argument_list|,
name|hm
operator|.
name|hm_hv
argument_list|)
expr_stmt|;
name|nsp
operator|->
name|ns_maplist
operator|=
name|hm
operator|.
name|hm_next
expr_stmt|;
block|}
name|i
operator|=
name|IPFGENITER_HOSTMAP
expr_stmt|;
operator|(
name|void
operator|)
name|ioctl
argument_list|(
name|fd
argument_list|,
name|SIOCIPFDELTOK
argument_list|,
operator|&
name|i
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|int
name|nat_matcharray
parameter_list|(
name|nat
parameter_list|,
name|array
parameter_list|)
name|nat_t
modifier|*
name|nat
decl_stmt|;
name|int
modifier|*
name|array
decl_stmt|;
block|{
name|int
name|i
decl_stmt|,
name|n
decl_stmt|,
modifier|*
name|x
decl_stmt|,
name|rv
decl_stmt|,
name|p
decl_stmt|;
name|ipfexp_t
modifier|*
name|e
decl_stmt|;
name|rv
operator|=
literal|0
expr_stmt|;
name|n
operator|=
name|array
index|[
literal|0
index|]
expr_stmt|;
name|x
operator|=
name|array
operator|+
literal|1
expr_stmt|;
for|for
control|(
init|;
name|n
operator|>
literal|0
condition|;
name|x
operator|+=
literal|3
operator|+
name|x
index|[
literal|3
index|]
operator|,
name|rv
operator|=
literal|0
control|)
block|{
name|e
operator|=
operator|(
name|ipfexp_t
operator|*
operator|)
name|x
expr_stmt|;
if|if
condition|(
name|e
operator|->
name|ipfe_cmd
operator|==
name|IPF_EXP_END
condition|)
break|break;
name|n
operator|-=
name|e
operator|->
name|ipfe_size
expr_stmt|;
name|p
operator|=
name|e
operator|->
name|ipfe_cmd
operator|>>
literal|16
expr_stmt|;
if|if
condition|(
operator|(
name|p
operator|!=
literal|0
operator|)
operator|&&
operator|(
name|p
operator|!=
name|nat
operator|->
name|nat_pr
index|[
literal|1
index|]
operator|)
condition|)
break|break;
switch|switch
condition|(
name|e
operator|->
name|ipfe_cmd
condition|)
block|{
case|case
name|IPF_EXP_IP_PR
case|:
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
operator|(
name|nat
operator|->
name|nat_pr
index|[
literal|1
index|]
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
expr_stmt|;
block|}
break|break;
case|case
name|IPF_EXP_IP_SRCADDR
case|:
if|if
condition|(
name|nat
operator|->
name|nat_v
index|[
literal|0
index|]
operator|!=
literal|4
condition|)
break|break;
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
operator|(
operator|(
name|nat
operator|->
name|nat_osrcaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
operator|||
operator|(
operator|(
name|nat
operator|->
name|nat_nsrcaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
expr_stmt|;
block|}
break|break;
case|case
name|IPF_EXP_IP_DSTADDR
case|:
if|if
condition|(
name|nat
operator|->
name|nat_v
index|[
literal|0
index|]
operator|!=
literal|4
condition|)
break|break;
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
operator|(
operator|(
name|nat
operator|->
name|nat_odstaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
operator|||
operator|(
operator|(
name|nat
operator|->
name|nat_ndstaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
expr_stmt|;
block|}
break|break;
case|case
name|IPF_EXP_IP_ADDR
case|:
if|if
condition|(
name|nat
operator|->
name|nat_v
index|[
literal|0
index|]
operator|!=
literal|4
condition|)
break|break;
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
operator|(
operator|(
name|nat
operator|->
name|nat_osrcaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
operator|||
operator|(
operator|(
name|nat
operator|->
name|nat_nsrcaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
operator|||
operator|(
operator|(
name|nat
operator|->
name|nat_odstaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
operator|||
operator|(
operator|(
name|nat
operator|->
name|nat_ndstaddr
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
operator|+
literal|1
index|]
operator|)
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|2
index|]
operator|)
expr_stmt|;
block|}
break|break;
ifdef|#
directive|ifdef
name|USE_INET6
case|case
name|IPF_EXP_IP6_SRCADDR
case|:
if|if
condition|(
name|nat
operator|->
name|nat_v
index|[
literal|0
index|]
operator|!=
literal|6
condition|)
break|break;
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_osrc6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
operator|||
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_nsrc6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|IPF_EXP_IP6_DSTADDR
case|:
if|if
condition|(
name|nat
operator|->
name|nat_v
index|[
literal|0
index|]
operator|!=
literal|6
condition|)
break|break;
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_odst6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
operator|||
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_ndst6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|IPF_EXP_IP6_ADDR
case|:
if|if
condition|(
name|nat
operator|->
name|nat_v
index|[
literal|0
index|]
operator|!=
literal|6
condition|)
break|break;
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_osrc6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
operator|||
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_nsrc6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
operator|||
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_odst6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
operator|||
name|IP6_MASKEQ
argument_list|(
operator|&
name|nat
operator|->
name|nat_ndst6
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
operator|+
literal|4
index|]
argument_list|,
operator|&
name|e
operator|->
name|ipfe_arg0
index|[
name|i
operator|*
literal|8
index|]
argument_list|)
expr_stmt|;
block|}
break|break;
endif|#
directive|endif
case|case
name|IPF_EXP_UDP_PORT
case|:
case|case
name|IPF_EXP_TCP_PORT
case|:
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
operator|(
name|nat
operator|->
name|nat_osport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
operator|||
operator|(
name|nat
operator|->
name|nat_nsport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
operator|||
operator|(
name|nat
operator|->
name|nat_odport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
operator|||
operator|(
name|nat
operator|->
name|nat_ndport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
expr_stmt|;
block|}
break|break;
case|case
name|IPF_EXP_UDP_SPORT
case|:
case|case
name|IPF_EXP_TCP_SPORT
case|:
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
operator|(
name|nat
operator|->
name|nat_osport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
operator|||
operator|(
name|nat
operator|->
name|nat_nsport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
expr_stmt|;
block|}
break|break;
case|case
name|IPF_EXP_UDP_DPORT
case|:
case|case
name|IPF_EXP_TCP_DPORT
case|:
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|rv
operator|&&
name|i
operator|<
name|e
operator|->
name|ipfe_narg
condition|;
name|i
operator|++
control|)
block|{
name|rv
operator||=
operator|(
name|nat
operator|->
name|nat_odport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
operator|||
operator|(
name|nat
operator|->
name|nat_ndport
operator|==
name|e
operator|->
name|ipfe_arg0
index|[
name|i
index|]
operator|)
expr_stmt|;
block|}
break|break;
block|}
name|rv
operator|^=
name|e
operator|->
name|ipfe_not
expr_stmt|;
if|if
condition|(
name|rv
operator|==
literal|0
condition|)
break|break;
block|}
return|return
name|rv
return|;
block|}
end_function

end_unit

