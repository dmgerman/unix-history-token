...
begin_comment
comment|/*    perl.h  *  *    Copyright (c) 1987-2000, Larry Wall  *  *    You may distribute under the terms of either the GNU General Public  *    License or the Artistic License, as specified in the README file.  *  * $FreeBSD$  */
end_comment
...
begin_comment
comment|/* libsunmath doesn't have modfl and frexpl as of mid-March 2000 */
end_comment

begin_comment
comment|/* XXX Configure probe for modfl and frexpl needed XXX */
end_comment
...
name|x
parameter_list|)
value|((long double)frexp((double)(x)))
end_define
...
directive|define
name|Perl_error_log
value|(PL_stderrgv			\&& IoOFP(GvIOp(PL_stderrgv))	\ 				 ? IoOFP(GvIOp(PL_stderrgv))	\ 				 : PerlIO_stderr())
end_define
...
parameter_list|()
define|\
value|STMT_START {				\ 	if (! PL_numeric_standard)		\ 	    set_numeric_standard();		\     } STMT_END
end_define
...
parameter_list|()
define|\
value|STMT_START {				\ 	if (! PL_numeric_local)			\ 	    set_numeric_local();		\     } STMT_END
end_define
...
parameter_list|)
define|\
value|((PL_hints& HINT_LOCALE)&& \ 	  PL_numeric_radix&& (c) == PL_numeric_radix)
end_define
...
parameter_list|()
value|if ((PL_hints& HINT_LOCALE)&& PL_numeric_standard) SET_NUMERIC_LOCAL()
end_define
...
parameter_list|()
value|if ((PL_hints& HINT_LOCALE)&& PL_numeric_local) SET_NUMERIC_STANDARD()
end_define
...
begin_comment
comment|/* Mention        NV_PRESERVES_UV     HAS_ICONV    I_ICONV     HAS_MKSTEMP    HAS_MKSTEMPS    HAS_MKDTEMP     HAS_GETCWD     HAS_MMAP    HAS_MPROTECT    HAS_MSYNC    HAS_MADVISE    HAS_MUNMAP    I_SYSMMAN    Mmap_t     so that Configure picks them up. */
end_comment
