...
begin_comment
comment|/*    pp_ctl.c  *  *    Copyright (c) 1991-2000, Larry Wall  *  *    You may distribute under the terms of either the GNU General Public  *    License or the Artistic License, as specified in the README file.  *  */
end_comment
...
begin_comment
comment|/*  * Closures mentioned at top level of eval cannot be referenced  * again, and their presence indirectly causes a memory leak.  * (Note that the fact that compcv and friends are still set here  * is, AFAIK, an accident.)  --Chip  *  * XXX need to get comppad et al from eval's cv rather than  * relying on the incidental global values.  */
end_comment
...
expr_stmt|;
block|}
comment|/* try blocks have old_namesv == 0 */
elseif|else
if|if
condition|(
name|cx
...
argument_list|)
expr_stmt|;
comment|/* clean scope, but only if there's no continue block */
if|if
condition|(
operator|!
operator|(
