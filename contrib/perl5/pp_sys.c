...
begin_comment
comment|/*    pp_sys.c  *  *    Copyright (c) 1991-1999, Larry Wall  *  *    You may distribute under the terms of either the GNU General Public  *    License or the Artistic License, as specified in the README file.  *  */
end_comment
...
operator|>
literal|1
comment|/* If SELECT_MIN_BITS is greater than one we most probably will want      * to align the sizes with SELECT_MIN_BITS/8 because for example      * in many little-endian (Intel, Alpha) systems (Linux, OS/2, Digital      * UNIX, Solaris, NeXT) the smallest quantum select() operates on      * (sets bit) is 32 bits.  */
name|growsize
operator|=
name|maxlen
operator|+
