...
begin_comment
comment|/*    toke.c  *  *    Copyright (c) 1991-2000, Larry Wall  *  *    You may distribute under the terms of either the GNU General Public  *    License or the Artistic License, as specified in the README file.  *  */
end_comment
...
value|yylval_pointer,yychar_pointer
end_define
...
name|yylex
parameter_list|()
value|Perl_yylex(aTHX_ yylval_pointer, yychar_pointer)
end_define
...
condition|)
block|{
comment|/* illegal UTF8, make it valid */
name|char
modifier|*
name|old_pvx
init|=
...
name|NUM_ESCAPE_INSERT
label|:
comment|/* Insert oct or hex escaped character. 		 * There will always enough room in sv since such escapes will 		 * be longer than any utf8 sequence they can end up as 		 */
if|if
condition|(
name|uv
operator|>
...
condition|)
block|{
comment|/* might need to recode whatever we have accumulated so far 			 * if it contains any hibit chars 			 */
name|int
name|hicount
init|=
literal|0
...
argument_list|(
name|aTHX_
literal|"Too many /C and /U options"
argument_list|)
expr_stmt|;
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|/* unfortunately this monster needs to be on one line or 	   makedepend will be confused. */
if|#
directive|if
operator|(
name|defined
...
operator|)
operator|)
comment|/* 	   No working strto[u]l[l]. Since atoi() doesn't do range checks, 	   we need to do this the hard way. 	 */
name|value
operator|=
name|Atof
argument_list|(
...
argument_list|(
name|aTHX_
literal|"%_%s has too many errors.\n"
argument_list|,
name|ERRSV
argument_list|,
name|CopFILE
