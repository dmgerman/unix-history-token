...
begin_comment
comment|/*    util.c  *  *    Copyright (c) 1991-2000, Larry Wall  *  *    You may distribute under the terms of either the GNU General Public  *    License or the Artistic License, as specified in the README file.  *  */
end_comment
...
name|decimal_point
condition|)
comment|/* We assume that decimal separator aka the radix 	 * character is always a single character.  If it 	 * ever is a string, this needs to be rethunk. */
...
begin_comment
comment|/* =for apidoc croak  This is the XSUB-writer's interface to Perl's C<die> function.  Use this function the same way you use the C C<printf> function.  See C<warn>.  =cut */
end_comment
...
begin_comment
comment|/*  * Save environ of perl.exe, currently Cygwin links in separate environ's  * for each exe/dll.  Probably should be a member of impure_ptr.  */
end_comment
...
parameter_list|)
block|{
comment|/* You can not directly manipulate the environ[] array because      * the routines do some additional work that syncs the Cygwin      * environment with the Windows environment.      */
name|char
modifier|*
name|oldstr
init|=
...
name|Perl_main_environ
expr_stmt|;
comment|/* environ realloc can occur in setenv */
if|if
condition|(
name|oldstr
operator|&&
...
condition|)
continue|continue;
comment|/* Note: does not check for __ and the like. */
if|if
condition|(
name|seenb
operator|==
...
name|EVAL_NULL
expr_stmt|;
comment|/* ~(EVAL_INEVAL|EVAL_WARNONLY|EVAL_KEEPERR) */
name|PL_restartop
operator|=
literal|0
expr_stmt|;
...
name|Perl_warner
argument_list|(
argument|aTHX_ WARN_CLOSED
argument_list|,
literal|"%s() on closed %s %s"
argument_list|,
argument|func
argument_list|,
argument|obj
...
argument|aTHX_ WARN_CLOSED
argument_list|,
literal|"\t(Are you trying to call %s() on dirhandle %s?)\n"
argument_list|,
argument|func
argument_list|,
argument|name
