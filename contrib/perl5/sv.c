...
begin_comment
comment|/*    sv.c  *  *    Copyright (c) 1991-2000, Larry Wall  *  *    You may distribute under the terms of either the GNU General Public  *    License or the Artistic License, as specified in the README file.  *  */
end_comment
...
name|DEBUG_c
argument_list|(
argument|{ 	    RESTORE_NUMERIC_STANDARD(); 	    PerlIO_printf(Perl_debug_log,
literal|"0x%"
argument|UVxf
literal|" num(%"
...
begin_comment
comment|/*  * Returns a combination of (advisory only - can get false negatives)  * 	IS_NUMBER_TO_INT_BY_ATOL, IS_NUMBER_TO_INT_BY_ATOF, IS_NUMBER_NOT_IV,  *	IS_NUMBER_NEG  * 0 if does not look like number.  *  * In fact possible values are 0 and  * IS_NUMBER_TO_INT_BY_ATOL				123  * IS_NUMBER_TO_INT_BY_ATOL | IS_NUMBER_NOT_IV		123.1  * IS_NUMBER_TO_INT_BY_ATOF | IS_NUMBER_NOT_IV		123e0  * with a possible addition of IS_NUMBER_NEG.  */
end_comment
...
comment|/* next must be digit or the radix separator */
if|if
condition|(
name|isDIGIT
argument_list|(
...
comment|/* I tried changing this for to be 64-bit-aware and 	 * the t/op/numconvert.t became very, very, angry. 	 * --jhi Sep 1999 */
if|if
condition|(
name|SvTYPE
argument_list|(
...
condition|)
return|return;
comment|/* This function could be much more efficient if we had a FLAG      * to signal if there are any hibit chars in the string      */
name|hicount
operator|=
literal|0
expr_stmt|;
...
literal|0
decl_stmt|;
comment|/* need to figure out if this is possible at all first */
while|while
condition|(
name|c
operator|<
...
block|{
comment|/* XXX might want to make a callback here instead */
name|Perl_croak
argument_list|(
name|aTHX_
literal|"Big byte"
...
begin_comment
comment|/* =for apidoc sv_catsv  Concatenates the string from SV C<ssv> onto the end of the string in SV C<dsv>.  Handles 'get' magic, but not 'set' magic.  See C<sv_catsv_mg>.  =cut */
end_comment
...
condition|)
block|{
comment|/* must upgrade other to UTF8 first */
if|if
condition|(
name|SvUTF8
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* refresh pointer and length */
name|pv1
operator|=
name|SvPVX
argument_list|(
