...
name|log_info
argument_list|(
literal|"Incorrect polling interval %d"
argument_list|,
name|polling_interval
argument_list|)
expr_stmt|;
name|log_info
argument_list|(
literal|"Using a default of 5 seconds"
argument_list|)
expr_stmt|;
name|polling_interval
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * It's now possible that state_reboot can be called 	 * after a interface link went down and is up again. 	 * To prevent tons of equal leases saved on disk, we rewrite 	 * them. 	 */
name|read_client_leases
argument_list|()
expr_stmt|;
name|rewrite_client_leases
...
expr_stmt|;
block|}
comment|/* 			     * Set the link status back to nolink, even 			     * if we have media settings. 			     */
name|client
operator|->
name|interface
operator|->
...
expr_stmt|;
block|}
comment|/* 	 * Only try the first ten seconds to renew a lease from a 	 * given dhcp-server adress. After that, fall back to use 	 * state_reboot with INADDR_BROADCAST. 	 */
if|if
condition|(
name|destination
operator|.
...
argument_list|)
expr_stmt|;
comment|/* 	 * If sendto() for a direct request fails, fall back to use 	 * state_reboot with INADDR_BROADCAST. 	 */
if|if
condition|(
name|result
operator|==
...
begin_comment
comment|/* Check to see if there's a wire plugged in */
end_comment
...
name|log_fatal
argument_list|(
literal|"Can't create interface_active socket"
argument_list|)
expr_stmt|;
operator|(
name|void
...
condition|)
block|{
comment|/* 		 * Interface doesn't support SIOCGIFMEDIA, presume okay 		 */
name|close
argument_list|(
name|sock
argument_list|)
...
condition|)
block|{
comment|/* 		 * Assume that this means interface 		 * does not support SIOCGIFMEDIA 		 */
name|log_fatal
argument_list|(
...
condition|)
block|{
comment|/* 			 * Wavelan devices need to be checked if they are 			 * associated. 			 */
if|if
condition|(
operator|(
name|IFM_TYPE
...
begin_comment
comment|/* Go to background after some time */
end_comment
...
begin_comment
comment|/* Check the state of the NICs if we have link */
end_comment
...
name|printf
argument_list|(
literal|"%s: Trying media settings on interface\n"
argument_list|,
name|ip
operator|->
name|name
...
name|printf
argument_list|(
literal|"%s: Found Link on interface\n"
argument_list|,
name|ip
operator|->
name|name
...
directive|endif
comment|/* 				 * Set the interface to state_bound. We assume that we have 				 * a working link. If we cannot reach the server directly, 				 * INADDR_BROADCAST is used. 				 */
for|for
control|(
name|client
operator|=
...
control|)
block|{
comment|/* 					 * Without this add_timout(), dhclient does 					 * not poll on a interface if there 					 * is no cable plugged in at startup 					 * time. Because we add one additional second  					 * to the time of a normal timeout, we always 					 * skip and block a running one. This prevents 					 * that polling is done twice at the same time. 					 */
if|if
condition|(
name|client
operator|->
...
name|NOLINK
expr_stmt|;
comment|/* 				 * Automatically go into the background after 				 * some time. Do this only if there are no 				 * media options available for a interface. 				 */
if|if
condition|(
operator|!
name|ip
...
block|}
block|}
comment|/* 		 * The last status of the interface tells us 		 * the we previously had link. 		 */
if|if
condition|(
name|ip
operator|->
...
condition|)
block|{
comment|/* 				 * We lost link on the interface, or it isn't 				 * associated anymore. 				 */
ifdef|#
directive|ifdef
name|DEBUG
name|printf
argument_list|(
literal|"%s: Lost Link on interface\n"
argument_list|,
name|ip
operator|->
name|name
...
directive|endif
comment|/* 				 * After we lost link, cycle again through the 				 * different media settings if available. Else 				 * set NOLINK. 				 */
if|if
condition|(
name|ip
operator|->
...
expr_stmt|;
block|}
comment|/* 			 * If we happen to have a real link, but no 			 * active lease, force the interface into 			 * state_reboot. Do the same if media settings 			 * are available. 			 */
if|if
condition|(
name|ip
operator|->
...
expr_stmt|;
block|}
comment|/* We still have link, do nothing. */
block|}
block|}
name|doinitcheck
operator|=
