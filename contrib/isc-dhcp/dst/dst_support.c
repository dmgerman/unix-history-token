...
index|[]
init|=
literal|"$Header: /rc0/proj/cvs/isc/DHCP/dst/dst_support.c,v 1.1 2001/02/22 07:22:08 mellon Exp $"
decl_stmt|;
end_decl_stmt
...
begin_ifdef
unit|void dst_s_dump(const int mode, const u_char *data, const int size,  	    const char *msg) { 	if (size> 0) {
ifdef|#
directive|ifdef
name|LONG_TEST
end_ifdef
...
begin_else
unit|static u_char scratch[1000]; 		int n ; 		n = b64_ntop(data, scratch, size, sizeof(scratch)); 		printf("%s: %x %d %s\n", msg, mode, n, scratch);
else|#
directive|else
end_else
