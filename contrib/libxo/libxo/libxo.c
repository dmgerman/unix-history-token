...
begin_comment
comment|/*  * A word about states:  We're moving to a finite state machine (FMS)  * approach to help remove fragility from the caller's code.  Instead  * of requiring a specific order of calls, we'll allow the caller more  * flexibility and make the library responsible for recovering from  * missed steps.  The goal is that the library should not be capable of  * emitting invalid xml or json, but the developer shouldn't need  * to know or understand all the details about these encodings.  *  * You can think of states as either states or event, since they  * function rather like both.  None of the XO_CLOSE_* events will  * persist as states, since their stack frame will be popped.  * Same is true of XSS_EMIT, which is an event that asks us to  * prep for emitting output fields.  */
end_comment
...
begin_comment
comment|/* "colors" refers to fancy ansi codes */
end_comment
...
begin_comment
comment|/* "effects" refers to fancy ansi codes */
end_comment

begin_comment
comment|/*  * Yes, there's no blink.  We're civilized.  We like users.  Blink  * isn't something one does to someone you like.  Friends don't let  * friends use blink.  On friends.  You know what I mean.  Blink is  * like, well, it's like bursting into show tunes at a funeral.  It's  * just not done.  Not something anyone wants.  And on those rare  * instances where it might actually be appropriate, it's still wrong.  * It's likely done my the wrong person for the wrong reason.  Just  * like blink.  And if I implemented blink, I'd be like a funeral  * director who adds "Would you like us to burst into show tunes?" on  * the list of questions asking while making funeral arrangements.  * It's formalizing wrongness in the wrong way.  And we're just too  * civilized to do that.   Hhhmph!  */
end_comment
...
begin_comment
comment|/* Only emit for encoding formats (xml, json) */
end_comment
...
condition|)
block|{
comment|/* Default format for value fields is '%s' */
name|format
operator|=
literal|"%s"
expr_stmt|;
...
condition|)
block|{
comment|/* Need either content or format */
if|if
condition|(
name|format
operator|==
...
case|:
block|{
comment|/* 	     * For XML, we record the version string in our handle, and emit 	     * it in xo_emit_top. 	     */
name|int
name|len
init|=
name|strlen
