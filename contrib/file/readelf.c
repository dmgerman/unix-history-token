...
argument_list|)
expr_stmt|;
comment|/* 					 * Version number is stuck at 199905, 					 * and hence is basically content-free. 					 */
block|}
if|if
condition|(
name|nh_namesz
...
argument_list|)
expr_stmt|;
comment|/* 					 * Contents is __FreeBSD_version, 					 * whose relation to OS versions is 					 * defined by a huge table in the 					 * Porters' Handbook.  Happily, the 					 * first three digits are the version 					 * number, at least in versions of 					 * FreeBSD that use this note. 					 */
name|printf
argument_list|(
literal|" %d.%d"
argument_list|,
...
name|nh_size
expr_stmt|;
comment|/* 			 * Check whether this note has the name "CORE" or 			 * "FreeBSD", or "NetBSD-CORE". 			 */
if|if
condition|(
name|offset
operator|+
...
name|swap
decl_stmt|;
comment|/* 	 * If we can't seek, it must be a pipe, socket or fifo. 	 */
if|if
condition|(
operator|(
name|lseek
