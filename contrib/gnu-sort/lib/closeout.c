...
begin_comment
comment|/* Set the file name to be reported in the event an error is detected    by close_stdout.  */
end_comment
...
literal|1
decl_stmt|;
comment|/* If the stream's error bit is clear and there is nothing to flush,      then return right away.  */
if|if
condition|(
name|e
operator|&&
