...
begin_comment
comment|/* Note that PARAMS is just internal to the diff library; diffrun.h    has its own mechanism, which will hopefully be less likely to    conflict with the library's caller's namespace.  */
end_comment
...
begin_comment
comment|/* I believe that all relevant systems have    time.h.  It is in ANSI, for example.  The    code below looks quite bogus as I don't think    sys/time.h is ever a substitute for time.h;    it is something different.  */
end_comment
...
begin_comment
comment|/* these come from CVS's lib/system.h, but I wasn't sure how to include that  * properly or even if I really should  */
end_comment
