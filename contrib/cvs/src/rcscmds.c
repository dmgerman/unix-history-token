...
begin_comment
comment|/*  * Copyright (c) 1992, Brian Berliner and Jeff Polk  * Copyright (c) 1989-1992, Brian Berliner  *   * You may distribute under the terms of the GNU General Public License as  * specified in the README file that comes with the CVS source distribution.  *   * The functions in this file provide an interface for performing   * operations directly on RCS files.   */
end_comment
...
begin_comment
comment|/* diff_run is imported from libdiff.a. */
end_comment
...
begin_comment
comment|/* Some 4.4BSD Glue to hack stdout and stderr to call cvs_output */
end_comment
...
name|SERVER_SUPPORT
argument_list|)
comment|/* only do this on the server if it's in protocol mode */
if|if
condition|(
...
directive|else
comment|/* avoid that sleep "by any means necessary".. */
name|void
modifier|*
name|save_out
decl_stmt|,
...
name|rv
decl_stmt|;
comment|/* XXX: the cvs_out*() funcs call the buf routines which can call        cvs_outerr(), and also calls malloc which might printf something.        FreeBSD's malloc doesn't do this at the moment, so recursion should        be avoided.  No guarantees for other BSD4.4-Lite* systems though. */
if|if
condition|(
name|error_use_protocol
operator|||
