...
return|return;
block|}
comment|/* We need to check :ext: server here, :pserver: checks happen below. */
if|if
condition|(
...
literal|'+'
condition|)
comment|/* Skip this for entries with conflict markers.  */
operator|*
name|timefield
operator|=
literal|'='
...
begin_comment
comment|/*  * Set buffer FD to blocking I/O.  Returns 0 for success or errno code.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* Don't call server_cleanup - the parent will handle that.  */
ifdef|#
directive|ifdef
name|SYSTEM_CLEANUP
...
literal|1
expr_stmt|;
comment|/* Do not try to close it again. */
name|err_exit
operator|=
literal|1
expr_stmt|;
...
name|pending_error_text
argument_list|,
literal|"E init may not be run remotely"
argument_list|)
expr_stmt|;
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* It's okay to skip rcs_cleanup() and Lock_Cleanup() here.  */
ifdef|#
directive|ifdef
name|SYSTEM_CLEANUP
comment|/* Hook for OS-specific behavior, for example socket subsystems on 	   NT and OS2 or dealing with windows and arguments on Mac.  */
...
name|str
decl_stmt|;
comment|/* Local users that do 'cvs status 2>&1' on a local repository 	   may see the informational messages out-of-order with the 	   status messages unless we use the fflush (stderr) here. */
name|fflush
argument_list|(
name|stderr
argument_list|)
