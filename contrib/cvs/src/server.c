...
condition|)
block|{
comment|/* I think isabsolute (repos) should always be true, and that 	   any RELATIVE_REPOS stuff should only be in CVS/Repository 	   files, not the protocol (for compatibility), but I'm putting 	   in the in isabsolute check just in case.  */
if|if
condition|(
name|isabsolute
argument_list|(
...
name|second
decl_stmt|;
comment|/* Note that these strings are specified in RFC822 and do not vary        according to locale.  */
specifier|static
specifier|const
name|char
modifier|*
...
condition|)
return|return;
comment|/* The only hard part about this routine is converting the date        formats.  In terms of functionality it all boils down to the        call to RCS_getrevtime.  */
if|if
condition|(
name|RCS_getrevtime
argument_list|(
