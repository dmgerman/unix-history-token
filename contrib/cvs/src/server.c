...
argument_list|)
condition|)
comment|/* At least for the moment I'm going to do the paranoid 	   security thing and not tell them how it failed.  I'm not 	   sure that is a good idea; it is a real pain when one needs 	   to track down what is going on for legitimate reasons. 	   The other issue is that the protocol doesn't really have 	   a good way for anything other than I HATE YOU.  */
goto|goto
name|i_hate_you
goto|;
