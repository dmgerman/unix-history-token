...
name|valid_usage
decl_stmt|;
comment|/* initialize static options */
name|force_tag_match
operator|=
literal|1
expr_stmt|;
...
name|errno
argument_list|,
literal|"could not change directory to `%s'"
argument_list|,
name|current
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* where - last_component( where ) */
name|char
modifier|*
name|parent
decl_stmt|;
...
name|errno
argument_list|,
literal|"could not get working directory (nominally `%s')"
argument_list|,
name|where_location
argument_list|)
expr_stmt|;
...
name|errno
argument_list|,
literal|"could not change directory to requested checkout directory `%s'"
argument_list|,
name|where_location
argument_list|)
expr_stmt|;
block|}
comment|/* else: ERRNO == ENOENT& last_component(where) == where 	     * for example, 'cvs co -d newdir module', where newdir hasn't 	     * been created yet, so leave current set to '.' and check that 	     */
block|}
else|else
comment|/* fail */
name|error
