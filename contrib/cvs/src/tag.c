...
begin_comment
comment|/*  * Copyright (C) 1986-2005 The Free Software Foundation, Inc.  *  * Portions Copyright (C) 1998-2005 Derek Price, Ximbiot<http://ximbiot.com>,  *                                  and others.  *  * Portions Copyright (C) 1992, Brian Berliner and Jeff Polk  * Portions Copyright (C) 1989-1992, Brian Berliner  *   * You may distribute under the terms of the GNU General Public License as  * specified in the README file that comes with the CVS source distribution.  *   * Tag and Rtag  *   * Add or delete a symbolic name to an RCS file, or a collection of RCS files.  * Tag uses the checked out revision in the current directory, rtag uses  * the modules database, if necessary.  *  * $FreeBSD$  */
end_comment
...
begin_comment
comment|/* allow -F,-d to disturb branch tags */
end_comment
...
block|,
literal|"\t-B\tAllows -F and -d to disturb branch tags.  Use with extreme care.\n"
block|,
...
expr_stmt|;
block|}
comment|/* It would be nice to provide consistency with respect to        commits; however CVS lacks the infrastructure to do that (see        Concurrency in cvs.texinfo and comment in do_recursion).  */
...
block|}
elseif|else
comment|/* force_tag_move is set and... */
if|if
condition|(
operator|(
name|isbranch
...
literal|0
argument_list|,
literal|"%s: Not moving %s tag `%s' from %s to %s%s."
argument_list|,
name|finfo
operator|->
name|fullname
...
literal|0
argument_list|,
literal|"Not removing %s tag `%s' from `%s'%s."
argument_list|,
name|isbranch
condition|?
literal|"branch"
...
name|status
decl_stmt|;
comment|/* Casting out const should be safe here - input datums are not      * written to by the myndbm functions.      */
name|mytag
operator|.
name|dptr
operator|=
...
begin_comment
comment|/* Add a tag to the CVSROOT/val-tags cache.  Establishes a write lock and  * reverifies that the tag does not exist before adding it.  */
end_comment

begin_decl_stmt
specifier|static
name|void
name|add_to_val_tags
name|PROTO
argument_list|(
operator|(
specifier|const
...
argument_list|)
expr_stmt|;
comment|/* Check for presence again since we have a lock now.  */
if|if
condition|(
name|is_in_val_tags
argument_list|(
...
name|errno
argument_list|,
literal|"failed to store %s into val-tags"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
condition|)
return|return;
comment|/* Verify that the tag is valid syntactically.  Some later code once made      * assumptions about this.      */
name|RCS_check_tag
argument_list|(
name|name
argument_list|)
