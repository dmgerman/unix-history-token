...
name|printf
argument_list|(
literal|"no memory for local tag\n"
argument_list|)
expr_stmt|;
return|return;
block|}
...
directive|if
literal|0
comment|/* not yet.. gotta rethink the implications */
block|else if (!strncmp(buf, "umask=", 6)) { 		mode_t mode;  		cvsumask = (mode_t)(strtol(buf+6, NULL, 8)& 0777); 	    } 	    else if (!strncmp(buf, "dlimit=", 7)) {
ifdef|#
directive|ifdef
name|BSD
include|#
directive|include
file|<sys/resource.h>
block|struct rlimit rl;  		if (getrlimit(RLIMIT_DATA,&rl) != -1) { 			rl.rlim_cur = atoi(buf+7); 			rl.rlim_cur *= 1024;  			(void) setrlimit(RLIMIT_DATA,&rl); 		}
endif|#
directive|endif
comment|/* BSD */
block|}
