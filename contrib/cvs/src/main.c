...
begin_comment
comment|/* We step through the above values.  This variable is set to reflect    the currently active value. */
end_comment
...
expr_stmt|;
block|}
comment|/* The following commands do not use a checked-out working        directory.  We conservatively assume that everything else does.        Feel free to add to this list if you are _certain_ something        something doesn't use the WD. */
if|if
condition|(
operator|(
name|strcmp
...
literal|0
argument_list|,
literal|"notice: main loop with CVSROOT=%s"
argument_list|,
name|current_root
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Always send a four digit year.  */
if|if
condition|(
name|year
operator|<
