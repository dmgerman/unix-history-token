...
name|printf
argument_list|(
literal|"no memory for tag expand mode\n"
argument_list|)
expr_stmt|;
return|return;
block|}
...
expr_stmt|;
block|}
comment|/* 	     * OpenBSD has a "umask=" and "dlimit=" command, we silently 	     * ignore them here since they are not much use to us.  cvsumask 	     * defaults to 002 already, and the dlimit (data size limit) 	     * should really be handled elsewhere. 	     */
block|}
name|fclose
argument_list|(
name|fp
