...
begin_comment
comment|/*  * Copyright (C) 1986-2005 The Free Software Foundation, Inc.  *  * Portions Copyright (C) 1998-2005 Derek Price, Ximbiot<http://ximbiot.com>,  *                                  and others.  *  * Portions Copyright (C) 1992, Brian Berliner and Jeff Polk  * Portions Copyright (C) 1989-1992, Brian Berliner  *   * You may distribute under the terms of the GNU General Public License as  * specified in the README file that comes with the CVS source distribution.  *   * Difference  *   * Run diff against versions in the repository.  Options that are specified are  * passed on directly to "rcsdiff".  *   * Without any file arguments, runs diff against all the currently modified  * files.  *  * $FreeBSD$  */
end_comment
...
name|tmp
decl_stmt|;
comment|/* Add opt or longopt to diff_arv.  */
name|assert
argument_list|(
name|opt
operator|||
...
argument_list|)
expr_stmt|;
comment|/* When present, add ARGUMENT to DIFF_ARGV.  */
if|if
condition|(
name|argument
condition|)
...
directive|endif
block|{
comment|/* FreeBSD addition - warning idention not changed til matching-} */
if|if
condition|(
name|diff_rev1
operator|!=
