...
begin_comment
comment|/*  * Copyright (C) 1986-2005 The Free Software Foundation, Inc.  *  * Portions Copyright (C) 1998-2005 Derek Price, Ximbiot<http://ximbiot.com>,  *                                  and others.  *  * Portions Copyright (C) 1992, Brian Berliner and Jeff Polk  * Portions Copyright (C) 1989-1992, Brian Berliner  *   * You may distribute under the terms of the GNU General Public License as  * specified in the README file that comes with the CVS source distribution.  *   * General recursion handler  *   * $FreeBSD$  */
end_comment
...
name|dosrcs
expr_stmt|;
comment|/* If our repository_in has a trailing "/.", remove it before storing it      * for do_recursion().      *      * FIXME: This is somewhat of a hack in the sense that many of our callers      * painstakingly compute and add the trailing '.' we now remove.      */
while|while
condition|(
name|repository_in
operator|&&
...
condition|)
block|{
comment|/* Beware the case where the string is exactly "/." or "//.". 	 * Paths with a leading "//" are special on some early UNIXes. 	 */
if|if
condition|(
name|strlen
argument_list|(
...
argument_list|(
operator|!
literal|"Not reached.  Please report this problem to<"
