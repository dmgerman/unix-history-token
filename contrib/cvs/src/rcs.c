...
begin_comment
comment|/*  * Copyright (C) 1986-2005 The Free Software Foundation, Inc.  *  * Portions Copyright (C) 1998-2005 Derek Price, Ximbiot<http://ximbiot.com>,  *                                  and others.  *  * Portions Copyright (C) 1992, Brian Berliner and Jeff Polk  * Portions Copyright (C) 1989-1992, Brian Berliner  *   * You may distribute under the terms of the GNU General Public License as  * specified in the README file that comes with the CVS source distribution.  *   * The routines contained in this file do all the rcs file parsing and  * manipulation  *  * $FreeBSD$  */
end_comment
...
name|embedded_at
decl_stmt|;
comment|/* Whether the buffer has been mmap'ed or not.  */
name|int
name|mmapped
decl_stmt|;
block|}
...
name|b
parameter_list|)
value|(*(char *)(a) == *(char *)(b)&& strcmp ((a), (b)) == 0)
end_define
...
name|retval
decl_stmt|;
comment|/* First, try to find the file as cased. */
name|retval
operator|=
name|xmalloc
argument_list|(
...
condition|)
block|{
comment|/* Handle the error cases */
block|}
elseif|else
if|if
condition|(
...
literal|0
argument_list|,
literal|"Duplicate `access' keyword found in RCS file."
argument_list|)
expr_stmt|;
name|free
argument_list|(
...
literal|0
argument_list|,
literal|"Duplicate `locks' keyword found in RCS file."
argument_list|)
expr_stmt|;
name|free
argument_list|(
...
literal|0
argument_list|,
literal|"Duplicate `%s' keyword found in RCS file."
argument_list|,
name|RCSSYMBOLS
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"Multiple %s revision deltas found in `%s'"
argument_list|,
name|q
operator|->
name|key
...
literal|0
argument_list|,
literal|"Delta text %s without revision information in `%s'."
argument_list|,
name|key
argument_list|,
name|rcs
...
literal|0
argument_list|,
literal|"\ unrecognized operation '\\x%x' in %s revision %s"
argument_list|,
name|op
argument_list|,
name|rcs
...
name|pa
condition|)
comment|/* This might happen, for instance, if an RCS file only contained 	     * revisions 2.x and higher, and REV == "1". 	     */
name|error
argument_list|(
literal|1
argument_list|,
...
name|rev
return|;
comment|/* Trust the caller to print warnings. */
return|return
name|NULL
return|;
block|}
...
argument_list|)
expr_stmt|;
comment|/* If symtag is "HEAD", special case to get head RCS revision */
if|if
condition|(
name|symtag
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* tag is always allocated and numeric now.  */
...
literal|0
argument_list|,
literal|"%s: invalid date for revision %s (%s)"
argument_list|,
name|rcs
operator|->
name|path
...
literal|0
argument_list|,
literal|"%s: non-standard date format for revision %s (%s)"
argument_list|,
name|rcs
operator|->
name|path
...
name|symbols_data
expr_stmt|;
comment|/* Keeping track of LAST below isn't strictly necessary, now that tags 	 * should be parsed for validity before they are accepted, but tags 	 * with spaces used to cause the code below to loop indefintely, so 	 * I have corrected for that.  Now, in the event that I missed 	 * something, the server cannot be hung.  -DRP 	 */
name|last
operator|=
name|NULL
expr_stmt|;
...
name|free_path
condition|)
comment|/* If free_path is set then we know we allocated path 			 * and we can discard the const. 			 */
name|free
argument_list|(
operator|(
name|char
...
name|cnl
expr_stmt|;
comment|/* If the log message did not end in a newline, increment 	     * the newline count so we have space for the extra leader. 	     * Failure to do so results in a buffer overrun. 	     */
if|if
condition|(
name|loglen
operator|&&
...
operator|+
name|loglen
comment|/* Use CNL + 2 below:  One leader for each log 				* line, plus the Revision/Author/Date line, 				* plus a trailing blank line. 				*/
operator|+
operator|(
name|cnl
operator|+
...
condition|)
block|{
comment|/* There was no EOL at the end of the log message.  Add 			 * one. 			 */
name|sub
index|[
name|sublen
index|]
...
name|stderr
argument_list|,
literal|"%s-> RCS_checkout (%s, %s, %s, %s, %s)\n"
argument_list|,
ifdef|#
directive|ifdef
name|SERVER_SUPPORT
...
literal|0
argument_list|,
literal|"Expected head revision %s, found %s."
argument_list|,
name|rcs
operator|->
name|head
...
literal|0
argument_list|,
literal|"Duplicate log keyword found for head revision in RCS file."
argument_list|)
expr_stmt|;
name|free
argument_list|(
...
name|free_rev
condition|)
comment|/* It's okay to discard the const when free_rev is set, because 		 * we know we allocated it in this function. 		 */
name|free
argument_list|(
operator|(
name|char
...
literal|0
argument_list|,
literal|"RCS file `%s' does not contain its default revision."
argument_list|,
name|rcs
operator|->
name|path
...
condition|)
block|{
comment|/* If delta has not been added to a List, then freeing the Node key 	 * won't free delta->version. 	 */
if|if
condition|(
name|delta
operator|->
...
begin_comment
comment|/* Compare the contents of revision REV1 of RCS file RCS with the    contents of REV2 if given, otherwise, compare with the contents of    the file FILENAME.  OPTIONS is a string for the keyword    expansion options.  Return 0 if the contents of the revision are    the same as the contents of the file, 1 if they are different.  */
end_comment
...
begin_comment
comment|/* Return a nonzero value if a magic tag rooted at ARG is found.  */
end_comment
...
name|nlines
expr_stmt|;
comment|/* start with init # of lines */
for|for
control|(
name|p
operator|=
...
block|}
block|}
comment|/* New temp data structure to hold new org before        copy back into original structure. */
name|lines
operator|.
name|nlines
operator|=
...
argument_list|)
expr_stmt|;
comment|/* We changed the list order to first to last -- so the        list never gets larger than the size numlines. */
name|lastmodline
operator|=
literal|0
expr_stmt|;
comment|/* offset created when adding/removing lines        between new and original structure */
name|offset
operator|=
literal|0
expr_stmt|;
...
literal|1
expr_stmt|;
comment|/* On error, just free the rest of the list.  */
if|if
condition|(
operator|!
name|err
condition|)
block|{
comment|/* Here we need to get to the line where the next insert will 	       begin, which is DF->pos in ORIG_LINES.  We will fill up to 	       DF->pos - OFFSET in LINES with original items.  */
for|for
control|(
name|deltaend
operator|=
...
control|)
block|{
comment|/* we need to copy from the orig structure into new one */
name|lines
operator|.
name|vector
index|[
...
name|q
expr_stmt|;
comment|/* For each line we add the offset between the #'s 		       decreases. */
name|offset
operator|--
expr_stmt|;
break|break;
...
name|FRAG_DELETE
case|:
comment|/* we are removing this many lines from the source. */
name|offset
operator|+=
name|df
operator|->
...
literal|1
condition|)
comment|/* Annotate needs this but, since the original 				 * vector is disposed of before returning from 				 * this function, we only need keep track if 				 * there are multiple references. 				 */
name|orig_lines
operator|->
name|vector
index|[
...
condition|)
block|{
comment|/* No reason to try and move a half-mutated and known invalid 	 * text into the output buffer. 	 */
name|linevector_free
argument_list|(
operator|&
name|lines
...
block|{
comment|/* add the rest of the remaining lines to the data vector */
for|for
control|(
init|;
name|lastmodline
...
expr_stmt|;
block|}
comment|/* Move the lines vector to the original structure for output, 	 * first deleting the old. 	 */
name|linevector_free
argument_list|(
name|orig_lines
argument_list|)
...
literal|0
argument_list|,
literal|"Duplicate `log' keyword in RCS file (`%s')."
argument_list|,
name|rcs
operator|->
name|path
...
literal|0
argument_list|,
literal|"Invalid revision number in `%s'."
argument_list|,
name|rcs
operator|->
name|path
...
name|branch
decl_stmt|;
comment|/* Previously, this function used a recursive implementation, but        if the trunk has a huge number of revisions and the program        stack is not big, a stack overflow could occur, so this        nonrecursive version was developed to be more safe. */
name|Node
modifier|*
name|branchlist
decl_stmt|,
...
name|data
expr_stmt|;
comment|/* Print the delta node and go for its `next' node.  This 	   prints the trunk. If there are any branches printed on this 	   revision, mark we have some. */
name|putdelta
argument_list|(
name|versp
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Store branch information into branch list so to write its 	   trunk afterwards */
if|if
condition|(
name|versp
operator|->
...
expr_stmt|;
block|}
comment|/* If there are any branches printed on this revision,        print those trunks as well. */
name|branchlist
operator|=
name|branches
operator|->
...
name|NULL
expr_stmt|;
comment|/* so to prevent its freeing on dellist */
block|}
name|dellist
argument_list|(
operator|&
...
argument_list|)
condition|)
comment|/* Using errno here may well be misleanding since the most recent 	   call that set errno may not have anything whatsoever to do with 	   the error that set the flag, but it's better than nothing.  The 	   real solution is to check each call to fprintf rather than waiting 	   until the end like this.  */
name|error
argument_list|(
literal|1
argument_list|,
...
name|errno
argument_list|,
literal|"error flushing file `%s' to kernel buffers"
argument_list|,
name|rcs_lockfile
argument_list|)
expr_stmt|;
...
index|]
decl_stmt|;
comment|/* revs cannot be attached to /dev/null ... duh. */
name|assert
argument_list|(
name|strcmp
argument_list|(
...
literal|0
condition|)
comment|/* Assume that if the stat fails,then the later read for the 		 * diff will too. 		 */
name|error
argument_list|(
literal|1
argument_list|,
