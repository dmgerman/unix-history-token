...
expr_stmt|;
block|}
comment|/* Don't need to rcsbuf_valcopy `value' because 	   getdelta already did that. */
name|rdata
operator|->
name|desc
operator|=
...
block|}
block|}
comment|/* Here *IDP points to the id in the buffer, C is the character        we found at the end of the key, and PTR points to the location in        the buffer where we found C.  We may not set *PTR to \0, because        it may overwrite a terminating semicolon.  The calling function        must copy and terminate the id on its own. */
name|rcsbuf
operator|->
name|ptr
operator|=
...
begin_comment
comment|/* Read an RCS @-delimited string.  Store the result in STRP. */
end_comment

begin_function
specifier|static
name|int
name|rcsbuf_getstring
parameter_list|(
name|rcsbuf
parameter_list|,
name|strp
...
expr_stmt|;
block|}
comment|/* PTR should now point to the start of a string. */
if|if
condition|(
name|c
operator|!=
...
literal|0
argument_list|,
literal|"expected @-string at '\\x%x' in %s"
argument_list|,
name|c
argument_list|,
name|rcsbuf
...
begin_comment
comment|/* Read an RCS `word', in the sense of rcsfile(5) (an id, a num, a    @-delimited string, or `:').  Store the result in WORDP.  If a    `;' is reached without reading any text, the result is NULL. */
end_comment
...
expr_stmt|;
block|}
comment|/* If we have reached `;', there is no value. */
if|if
condition|(
name|c
operator|==
...
return|;
block|}
comment|/* C is neither `:', `;' nor `@', so it should be the start of a num        or an id.  Make sure it is not another special character. */
if|if
condition|(
name|c
operator|==
...
break|break;
block|}
comment|/* PTR points to the last non-id character in this word, and C is        the character in its memory cell.  Check to make sure that it        is a legitimate word delimiter -- whitespace or semicolon. */
if|if
condition|(
name|c
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Shut up compiler warnings.  */
return|return
literal|0
return|;
undef|#
...
argument_list|)
expr_stmt|;
comment|/* rcsbuf_getid did not terminate the key, so copy it to new space. */
name|len
operator|=
name|rcsbuf
operator|->
...
literal|'\0'
expr_stmt|;
comment|/* The `desc' keyword has only a single string value, with no 	   trailing semicolon, so it must be handled specially. */
if|if
condition|(
name|STREQ
argument_list|(
...
condition|)
break|break;
comment|/* Copy valbuf to new space so we can polish it, then 	       append it to value. */
if|if
condition|(
name|value
operator|==
