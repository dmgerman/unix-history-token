...
block|}
block|}
comment|/* Skip head and branch tags; we already have them. */
if|if
condition|(
name|strcmp
argument_list|(
...
comment|/* First, seek back to the start of the delta block. */
operator|(
name|void
operator|)
name|fseek
...
condition|)
do|;
comment|/* Do not include any trailing whitespace in the value. */
if|if
condition|(
name|c
operator|!=
...
begin_comment
comment|/* Like getrcsrev, but don't die on error.  Return the last character    read (last call to getc, which may be EOF).  TODO: implement getrcsrev    in terms of this function. */
end_comment
...
begin_comment
comment|/* TODO: RCS_checkin always unlinks the working file after checkin --    then RCS_checkout checks it out again.  The logic should probably    be reversed here. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Removing the file here is an RCS user-visible behavior which 	   we almost surely do not need in the CVS case.  In fact, getting 	   rid of it should clean up link_file and friends in import.c.  */
if|if
condition|(
name|unlink_file
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Get revision number. This uses getrcskey because it doesn't        croak when encountering unexpected input.  As a result, we have        to play unholy games with `key' and `value'. */
name|fpos
operator|=
name|ftell
argument_list|(
...
expr_stmt|;
block|}
comment|/* Note that "branches" and "next" are in fact mandatory, according        to doc/RCSFILES.  We perhaps should be giving an error if they        are not there.  */
...
block|}
block|}
comment|/* We got here because we read beyond the end of a delta.  Seek back        to the beginning of the erroneous read. */
operator|(
name|void
operator|)
name|fseek
...
condition|)
block|{
comment|/* If n == EOF and num == NULL, it means we reached EOF 	   naturally.  That's fine. */
if|if
condition|(
name|num
operator|==
...
name|errno
argument_list|,
literal|"cannot open RCS file `%s' for reading"
argument_list|,
name|rcs
operator|->
name|path
...
name|errno
argument_list|,
literal|"cannot fseek in RCS file %s"
argument_list|,
name|rcs
operator|->
name|path
