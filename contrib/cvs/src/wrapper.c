...
begin_comment
comment|/* FIXME: wrap_saved_count is never set to any non-zero value.    wrap_name_has and wrap_matching_entry should be using    wrap_tempcount instead.  I believe the consequence of this is that    .cvswrappers files are ignored (that was my own experience when I    tried to use one).  If this bug is fixed, would be nice to write a    sanity.sh testcase for .cvswrappers files.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Then add entries found in home dir, (if user has one) and file        exists.  (FIXME: I think this probably should be using        get_homedir, i.e. $HOME).  */
if|if
condition|(
operator|(
name|pw
