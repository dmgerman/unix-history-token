...
block|}
block|}
comment|/* Always look up the RCS keyword mode when we have an RCS archive.  It      * will either be needed as a default or to avoid allowing the -k options      * specified on the command line from overriding binary mode (-kb).      */
if|if
condition|(
name|finfo
operator|->
...
argument_list|)
expr_stmt|;
comment|/*      * -k options specified on the command line override (and overwrite)      * options stored in the entries file and default options from the RCS      * archive, except for binary mode (-kb).      */
if|if
condition|(
name|options
operator|&&
...
condition|)
block|{
comment|/* These just need matching content.  Might as well minimize it.  */
name|vers_ts
operator|->
name|ts_user
operator|=
