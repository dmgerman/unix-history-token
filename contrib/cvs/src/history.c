...
block|,
literal|"        -x [TOFWUCGMAR] Extract by record type\n"
block|,
literal|"   Flags:\n"
block|,
...
literal|0
argument_list|,
literal|"backto truncated to %d bytes"
argument_list|,
sizeof|sizeof
argument_list|(
name|backto
...
name|optind
expr_stmt|;
comment|/* Save the handled option count */
...
literal|0
argument_list|,
literal|"Only one report type allowed from: \"-Tcomx\"."
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
...
name|errno
argument_list|,
literal|"can't getwd in history"
argument_list|)
expr_stmt|;
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* This returns ptr to next line or final '\0' */
return|return
operator|(
name|rtn
operator|)
return|;
comment|/* If it falls through, go on to next record */
block|}
end_function

begin_comment
comment|/* read_hrecs's job is to read the history file and fill in all the "hrec"  * (history record) array elements with the ones we need to print.  *  * Logic:  * - Read the whole history file into a single buffer.  * - Walk through the buffer, parsing lines out of the buffer.  *   1. Split line into pointer and integer fields in the "next" hrec.  *   2. Apply tests to the hrec to see if it is wanted.  *   3. If it *is* wanted, bump the hrec pointer down by one.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Exactly enough space for lines data */
if|if
condition|(
operator|!
operator|(
...
literal|'\n'
expr_stmt|;
comment|/* Make sure last line ends in '\n' */
name|i
operator|++
expr_stmt|;
block|}
...
argument_list|)
expr_stmt|;
comment|/* cp == next line or '\0' at end of buffer */
if|if
condition|(
name|select_hrec
argument_list|(
...
argument_list|)
condition|)
comment|/* Don't bother with "file" if "TFO" */
block|{
if|if
condition|(
name|file_list
