...
name|zstatus
decl_stmt|;
comment|/* Don't make any attempt to pick up trailing data since we are shutting      * down.  If the client doesn't know we are shutting down, we might not      * see the EOF we are expecting.      */
name|zstatus
operator|=
name|inflateEnd
argument_list|(
...
literal|0
argument_list|,
literal|"gzipped data too small - lacks complete header"
argument_list|)
expr_stmt|;
return|return
literal|1
...
return|;
block|}
comment|/* Skip over the fixed header, and then skip any of the variable-length        fields.  As we skip each field, we keep pos<= size. The checks        on positions and lengths are really checks for malformed or         incomplete gzip data.  */
name|pos
operator|=
literal|10
expr_stmt|;
...
literal|0
argument_list|,
literal|"%s lacks proper gzip XLEN field"
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"%s lacks proper gzip \"extra field\""
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"%s has bad gzip filename field"
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"%s has bad gzip comment field"
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"%s has bad gzip CRC16 field"
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/* There could be no data to decompress - check and short circuit.  */
if|if
condition|(
name|pos
operator|>=
...
literal|0
argument_list|,
literal|"gzip data incomplete for %s (no data)"
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Check that there is still 8 trailer bytes remaining (CRC32        and ISIZE).  Check total decomp. data, plus header len (pos)        against input buffer total size.  */
name|pos
operator|+=
name|zstr
operator|.
...
literal|0
argument_list|,
literal|"gzip data incomplete for %s (no trailer)"
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* Adjust for 10-byte output header (filled in above) */
name|zstr
operator|.
name|total_out
operator|=
...
name|done
label|:
comment|/* Need to add the CRC information (8 bytes)        to the end of the gzip'd output.        Ensure there is enough space in the output buffer        to do so.  */
if|if
condition|(
name|zstr
operator|.
