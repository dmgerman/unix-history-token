...
begin_comment
comment|/* JT thinks BeOS is worth the trouble. */
end_comment
...
decl_stmt|;
comment|/* For security reasons, if PATHNAME is absolute or attemps to ascend      * outside of the current sanbbox, we abort.  The server should not send us      * anything but relative paths which remain inside the sandbox here.      * Anything less means a trojan CVS server could create and edit arbitrary      * files on the client.      */
if|if
condition|(
name|isabsolute
argument_list|(
...
name|file_timestamp
expr_stmt|;
comment|/* Checking for command_name of "commit" doesn't seem like 	       the cleanest way to handle this, but it seem to roughly 	       parallel what the :local: code which calls 	       mark_up_to_date ends up amounting to.  Some day, should 	       think more about what the Checked-in response means 	       vis-a-vis both Entries and Base and clarify 	       cvsclient.texi accordingly.  */
if|if
condition|(
operator|!
name|strcmp
...
begin_comment
comment|/* Connect to a forked server process. */
end_comment
