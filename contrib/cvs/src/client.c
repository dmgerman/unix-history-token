...
decl_stmt|;
block|{
comment|/* Decide if we should send this directory name to the server.  We        should always send argv[i] if:         1) the list of directories sent to the server is empty (as it        will be for checkout, etc.).         2) the argument is "."         3) the argument is a file in the cwd and the cwd is checked out        from the current root         4) the argument lies within one of the paths in        dirs_sent_to_server.         4) */
if|if
condition|(
name|list_isempty
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Try to remove the path components in UPDATE_DIR                    from REPOSITORY.  If the path elements don't exist                    in REPOSITORY, or the removal of those path                    elements mean that we "step above"                    CVSroot_directory, set toplevel_repos to                    CVSroot_directory. */
if|if
condition|(
operator|(
name|repository_len
...
literal|0
operator|)
comment|/* TOPLEVEL_REPOS shouldn't be above CVSroot_directory */
operator|&&
operator|(
operator|(
name|repository_len
