...
decl_stmt|;
block|{
comment|/* In the case where stdout and stderr point to the same place,        fflushing stderr will make output happen in the correct order.        Often stderr will be line-buffered and this won't be needed,        but not always.  */
name|fflush
argument_list|(
name|stderr
argument_list|)
...
name|ch
decl_stmt|;
comment|/* Long enough to hold I LOVE YOU or I HATE YOU.  Using a fixed-size 	   buffer seems better than letting an apeshit server chew up our 	   memory with illegal responses, and the value comes from 	   the protocol itself; it is not an arbitrary limit on data sent.  */
define|#
directive|define
name|LARGEST_RESPONSE
value|80
name|char
name|read_buf
index|[
...
begin_comment
comment|/*  * FIXME: this function has not been changed to deal with  * NO_SOCKET_TO_FD (i.e., systems on which sockets cannot be converted  * to file descriptors) or with SOCK_ERRNO/SOCK_STRERROR.  The first  * person to try building a kerberos client on such a system (OS/2,  * Windows 95, and maybe others) will have to make take care of this.  */
end_comment
...
literal|0
argument_list|,
literal|"Kerberos connect failed"
argument_list|)
expr_stmt|;
else|#
directive|else
...
literal|0
argument_list|,
literal|"Direct TCP connect failed"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
literal|0
argument_list|,
literal|"couldn't connect to remote host %s"
argument_list|,
name|CVSroot_hostname
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"Encryption is only supported when using Kerberos"
argument_list|)
expr_stmt|;
else|#
directive|else
