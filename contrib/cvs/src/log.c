...
block|,
literal|"\t-r[revisions]\tSpecify revision(s)s to list.\n"
block|,
literal|"\t-d dates\tSpecify dates (D1<D2 for range, D for latest before).\n"
block|,
...
name|ret
expr_stmt|;
comment|/* Copy the argument into memory so that we can change it.  We        don't want to change the argument because, at least as of this        writing, we will use it if we send the arguments to the server.        We never bother to free up our copy.  */
name|copy
operator|=
name|xstrdup
argument_list|(
...
literal|0
argument_list|,
literal|"warning: bad locks field in RCS file `%s'"
argument_list|,
name|finfo
operator|->
name|fullname
...
literal|0
argument_list|,
literal|"warning: lock for missing version `%s' in `%s'"
argument_list|,
name|version
argument_list|,
name|finfo
...
literal|0
argument_list|,
literal|"warning: duplicate lock for `%s' in `%s'"
argument_list|,
name|version
argument_list|,
name|finfo
...
literal|0
argument_list|,
literal|"warning: `%s' is not a branch in `%s'"
argument_list|,
name|r
operator|->
name|first
