...
name|printf
argument_list|(
literal|"is new; current revision %s\n"
argument_list|,
name|vers_head
argument_list|)
expr_stmt|;
...
name|printf
argument_list|(
literal|"changed from revision %s to %s\n"
argument_list|,
name|vers_tag
argument_list|,
name|vers_head
...
condition|)
block|{
comment|/* FIXME: should be printing a proper error message, with errno-based 	   message, and the filename which we could not create.  */
name|error
argument_list|(
literal|0
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Note that this prints "diff" not DIFF.  The format of a diff 	       does not depend on the name of the program which happens to 	       have produced it.  */
if|if
condition|(
name|unidiff
condition|)
