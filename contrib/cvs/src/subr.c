...
name|buf
argument_list|,
literal|"out of memory; can not reallocate %lu bytes"
argument_list|,
operator|(
name|unsigned
name|long
...
begin_comment
comment|/* Remove trailing newlines from STRING, destructively.  *  * RETURNS  *  *   True if any newlines were removed, false otherwise.  */
end_comment
...
do|do
block|{
comment|/* Now look for pathname level-ups.  */
if|if
condition|(
name|p
index|[
...
name|level
expr_stmt|;
comment|/* q = strchr (p, '/'); but sub ISDIRSEP() for '/': */
while|while
condition|(
operator|*
name|p
...
begin_comment
comment|/* Increment a revision number.  Working on the string is a bit awkward,    but it avoid problems with integer overflow should the revision numbers    get really big.  */
end_comment
...
expr_stmt|;
block|}
comment|/* The number was all 9s, so change the first character to 1 and add        a 0 to the end.  */
operator|*
name|p
operator|=
literal|'1'
...
begin_comment
comment|/* Given some revision, REV, return the first prior revision that exists in the  * RCS file, RCS.  *  * ASSUMPTIONS  *   REV exists.  *  * INPUTS  *   RCS	The RCS node pointer.  *   REV	An existing revision in the RCS file referred to by RCS.  *  * RETURNS  *   The first prior revision that exists in the RCS file, or NULL if no prior  *   revision exists.  The caller is responsible for disposing of this string.  *  * NOTES  *   This function currently neglects the case where we are on the trunk with  *   rev = X.1, where X != 1.  If rev = X.Y, where X != 1 and Y> 1, then this  *   function should work fine, as revision X.1 must exist, due to RCS rules.  */
end_comment
...
name|retval
decl_stmt|;
comment|/* Our retval can have no more digits and dots than our input revision.  */
name|retval
operator|=
name|xmalloc
argument_list|(
...
condition|)
block|{
comment|/* If r1 == 0, then we must be on a branch and our parent must 		 * exist, or we must be on the trunk with a REV like X.1. 		 * We are neglecting the X.1 with X != 1 case by assuming that 		 * there is no previous revision when we discover we were on 		 * the trunk. 		 */
name|p
operator|=
name|strrchr
argument_list|(
...
begin_comment
comment|/*  * file_has_conflict  *  * This function compares the timestamp of a file with ts_conflict set  * to the timestamp on the actual file and returns TRUE or FALSE based  * on the results.  *  * This function does not check for actual markers in the file and  * file_has_markers() function should be called when that is interesting.  *  * ASSUMPTIONS  *  The ts_conflict field is not NULL.  *  * RETURNS  *  TRUE	ts_conflict matches the current timestamp.  *  FALSE	The ts_conflict field does not match the file's  *		timestamp.  */
end_comment
...
name|retcode
decl_stmt|;
comment|/* If ts_conflict is NULL, there was no merge since the last      * commit and there can be no conflict.      */
name|assert
argument_list|(
name|ts_conflict
argument_list|)
expr_stmt|;
comment|/*      * If the timestamp has changed and no      * conflict indicators are found, it isn't a      * conflict any more.      */
ifdef|#
directive|ifdef
name|SERVER_SUPPORT
if|if
...
block|{
comment|/* Although it would be cleaner in some ways to just read 	   until end of file, reallocating the buffer, this function 	   does get called on files in the working directory which can 	   be of arbitrary size, so I think we better do all that 	   extra allocation.  */
if|if
condition|(
name|CVS_STAT
argument_list|(
...
condition|)
break|break;
comment|/* Allocate more space if needed.  */
if|if
condition|(
name|tobuf
operator|==
...
begin_comment
comment|/* Follow a chain of symbolic links to its destination.  FILENAME    should be a handle to a malloc'd block of memory which contains the    beginning of the chain.  This routine will replace the contents of    FILENAME with the destination (a real file).  */
end_comment
...
directive|ifdef
name|HAVE_READLINK
comment|/* The clean thing to do is probably to have each filesubr.c 	   implement this (with an error if not supported by the 	   platform, in which case islink would presumably return 0). 	   But that would require editing each filesubr.c and so the 	   expedient hack seems to be looking at HAVE_READLINK.  */
name|char
modifier|*
name|newname
init|=
...
begin_comment
comment|/*  * Rename a file to an appropriate backup name based on BAKPREFIX.  * If suffix non-null, then ".<suffix>" is appended to the new name.  *  * Returns the new name, which caller may free() if desired.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* one for dot, one for trailing '\0' */
name|sprintf
argument_list|(
name|backup_name
argument_list|,
...
begin_comment
comment|/*  * Copy a string into a buffer escaping any shell metacharacters.  The  * buffer should be at least twice as long as the string.  *  * Returns a pointer to the terminating NUL byte in buffer.  */
end_comment
...
begin_comment
comment|/*  * We can only travel forwards in time, not backwards.  :)  */
end_comment
...
directive|else
comment|/* default to 20 ms increments */
name|s
operator|=
name|desttime
operator|-
...
argument_list|)
block|{
comment|/* use select instead of sleep since it is a fairly portable way of 	     * sleeping for ms. 	     */
name|struct
name|timeval
name|tv
decl_stmt|;
