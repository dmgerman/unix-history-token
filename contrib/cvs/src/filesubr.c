...
begin_comment
comment|/* char *  * xresolvepath ( const char *path )  *  * Like xreadlink(), but resolve all links in a path.  *  * INPUTS  *  path	The original path.  *  * RETURNS  *  The path with any symbolic links expanded.  *  * ERRORS  *  This function exits with a fatal error if it fails to read the link for  *  any reason.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* FIXME - If HAVE_READLINK is defined, we should probably walk the path      * bit by bit calling xreadlink().      */
name|owd
operator|=
name|xgetwd
argument_list|()
...
literal|0
argument_list|,
literal|"expand_wild: too many arguments"
argument_list|)
expr_stmt|;
return|return;
block|}
