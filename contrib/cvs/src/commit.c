...
init|=
block|{
literal|"Usage: %s %s [-Rlf] [-m msg | -F logfile] [-r rev] files...\n"
block|,
...
literal|0
argument_list|,
literal|"`%s' should be removed and is still there (or is back"
literal|" again)"
argument_list|,
name|finfo
operator|->
...
condition|)
block|{
comment|/* This happens when one has `cvs add'ed a file, but it no 	       longer exists in the working directory at commit time. 	       FIXME: What classify_file does in this case is print 	       "new-born %s has disappeared" and removes the entry. 	       We probably should do the same.  */
if|if
condition|(
operator|!
name|really_quiet
...
condition|)
block|{
comment|/* FIXME: What classify_file does in this case is print 	   "%s was lost".  We probably should do the same.  */
name|freevers_ts
argument_list|(
operator|&
name|vers
...
literal|'n'
case|:
comment|/* Silently ignore -n for compatibility with old 		 * clients. 		 */
if|if
condition|(
operator|!
name|server_active
...
literal|0
argument_list|,
literal|"the `-n' option is obsolete"
argument_list|)
expr_stmt|;
break|break;
endif|#
...
literal|1
expr_stmt|;
comment|/* strip trailing dots and leading zeros */
while|while
condition|(
operator|*
operator|--
...
literal|0
argument_list|,
literal|"`%s' should be removed and is still there (or is"
literal|" back again)"
argument_list|,
name|finfo
operator|->
...
literal|0
argument_list|,
literal|"cannot remove file `%s' which has a numeric sticky"
literal|" tag of `%s'"
argument_list|,
name|finfo
operator|->
...
operator|->
name|tag
comment|/* If numeric, it is on the trunk; check_fileproc enforced 	       this.  */
operator|&&
operator|!
name|isdigit
argument_list|(
...
begin_comment
comment|/*  * remove a partially added file.  if we can parse it, leave it alone.  *  * FIXME: Every caller that calls this function can access finfo->rcs (the  * parsed RCSNode data), so we should be able to detect that the file needs  * to be removed without reparsing the file as we do below.  */
end_comment
...
begin_comment
comment|/*  * do the initial part of a file add for the named file.  if adding  * with a tag, put the file in the Attic and point the symbolic tag  * at the committed revision.  *  * INPUTS  *   file	The name of the file in the workspace.  *   repository	The repository directory to expect to find FILE,v in.  *   tag	The name or rev num of the branch being added to, if any.  *   options	Any RCS keyword expansion options specified by the user.  *   rcsnode	A pointer to the pre-parsed RCSNode for this file, if the file  *		exists in the repository.  If this is NULL, assume the file  *		does not yet exist.  *  * RETURNS  *   0 on success.  *   1 on errors, after printing any appropriate error messages.  *  * ERRORS  *   This function will return an error when any of the following functions do:  *     add_rcs_file  *     RCS_setattic  *     lock_RCS  *     RCS_checkin  *     RCS_parse (called to verify the newly created archive file)  *     RCS_settag  */
end_comment
...
literal|0
decl_stmt|;
comment|/* Set to 1 if we created a new RCS archive. */
name|int
name|retval
init|=
literal|1
...
argument_list|)
expr_stmt|;
comment|/* Callers expect to be able to use either "" or NULL to mean the        default keyword expansion.  */
if|if
condition|(
name|options
operator|!=
...
condition|)
block|{
comment|/* We tell the user about this, because it means that the 	       old revisions will no longer retrieve the way that they 	       used to.  */
name|error
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|,
literal|"changing keyword expansion mode to %s"
argument_list|,
name|options
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* Begin a critical section around the code that spans the 	       first commit on the trunk of a file that's already been 	       committed on a branch.  */
...
literal|0
argument_list|,
literal|"cannot lock revision %s in `%s'."
argument_list|,
name|rev
condition|?
name|rev
...
literal|0
argument_list|,
literal|"cannot lock initial revision in `%s'."
argument_list|,
name|rcs
operator|->
name|path
...
argument_list|)
expr_stmt|;
comment|/* If this is not a new branch, then we will want a dead 	       version created before this one. */
if|if
condition|(
operator|!
name|newfile
...
goto|;
block|}
comment|/* We need to add a dead version here to avoid -rtag -Dtime 	       checkout problems between when the head version was 	       created and now. */
if|if
condition|(
operator|!
name|newfile
...
argument_list|)
expr_stmt|;
comment|/* As we will be hacking the delta date, put the time 		   this was added into the log message. */
name|t
operator|=
name|time
argument_list|(
...
name|tmp
argument_list|,
literal|"file %s was added on branch %s on %d-%02d-%02d %02d:%02d:%02d +0000"
argument_list|,
name|file
argument_list|,
name|tag
...
literal|0
argument_list|,
literal|"could not created dead stub %s for %s"
argument_list|,
name|tag
argument_list|,
name|rcs
...
literal|0
argument_list|,
literal|"cannot lock head revision in `%s'."
argument_list|,
name|rcs
operator|->
name|path
...
argument_list|)
expr_stmt|;
comment|/* At this point, we used to set the file mode of the RCS file        based on the mode of the file in the working directory.  If we        are creating the RCS file for the first time, add_rcs_file does        this already.  If we are re-adding the file, then perhaps it is        consistent to preserve the old file mode, just as we preserve        the old keyword expansion mode.         If we decide that we should change the modes, then we can't do        it here anyhow.  At this point, the RCS file may be owned by        somebody else, so a chmod will fail.  We need to instead do the        chmod after rewriting it.         FIXME: In general, I think the file mode (and the keyword        expansion mode) should be associated with a particular revision        of the file, so that it is possible to have different revisions        of a file have different modes.  */
name|retval
operator|=
literal|0
expr_stmt|;
