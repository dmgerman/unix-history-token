...
control|)
block|{
comment|/*  	     * Recursion check: if an alias module calls itself or a module 	     * which causes the first to be called again, print an error 	     * message and stop recursing. 	     * 	     * Algorithm: 	     * 	     *   1. Check that MNAME isn't in the stack. 	     *   2. Push MNAME onto the stack. 	     *   3. Call do_module(). 	     *   4. Pop MNAME from the stack. 	     */
if|if
condition|(
name|stack
operator|&&
...
begin_comment
comment|/* External face of do_module so that we can have an internal version which  * accepts a stack argument to track alias recursion.  */
end_comment
