...
begin_comment
comment|/* Given a directory DIR and a subdirectory within it, SUBDIR, combine    the two into a new directory name.  Returns a newly malloc'd string.    For now this is a fairly simple affair, but perhaps it will want    to have grander ambitions in the context of VMS or others (or perhaps    not, perhaps that should all be hidden inside CVS_FOPEN and libc and so    on, and CVS should just see foo/bar/baz style pathnames).  */
end_comment
...
condition|)
block|{
comment|/* The dir name has an extraneous "." at the end. 	   I'm not completely sure that this is the best place 	   to strip it off--it is possible that Name_Repository 	   should do so, or it shouldn't be in the CVS/Repository 	   file in the first place.  Fixing it here seems like 	   a safe, small change, but I'm not sure it catches 	   all the cases.  */
name|strncpy
argument_list|(
name|retval
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* FIXME: should be able to pass SEND_NO_CONTENTS, I think.  */
name|send_files
argument_list|(
name|argc
argument_list|,
