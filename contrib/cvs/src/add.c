...
condition|)
block|{
comment|/* Need to check whether there is a directory with the 	       same name but different case.  We'll check for files 	       with the same name later (when Version_TS calls 	       RCS_parse which calls fopen_case).  If CVS some day 	       records directories in the RCS files, then we should be 	       able to skip the separate check here, which would be 	       cleaner.  */
name|DIR
modifier|*
name|dirp
decl_stmt|;
...
condition|)
block|{
comment|/* OK, we are about to patch up the name, so patch up 		   the temporary directory too to match.  The isdir 		   should "always" be true (since files have ,v), but 		   I guess we might as well make some attempt to not 		   get confused by stray files in the repository.  */
if|if
condition|(
name|isdir
argument_list|(
...
expr_stmt|;
block|}
comment|/* OK, we found a directory with the same name, maybe in 		   a different case.  Treat it as if the name were the 		   same.  */
name|finfo
operator|.
name|file
operator|=
