...
name|file_is_dead
decl_stmt|;
comment|/* Solely to suppress a warning from gcc -Wall.  */
name|backup
operator|=
name|NULL
expr_stmt|;
...
condition|)
block|{
comment|/* We are dealing with binary files, but real merging would 	   need to take place.  This is a conflict.  We give the user 	   the two files, and let them resolve it.  It is possible 	   that we should require a "touch foo" or similar step before 	   we allow a checkin.  */
if|if
condition|(
name|RCS_checkout
argument_list|(
