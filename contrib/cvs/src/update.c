...
init|=
block|{
literal|"Usage: %s %s [-APCdflRp] [-k kopt] [-r rev] [-D date] [-j rev]\n"
block|,
...
condition|)
block|{
comment|/* skip this step when the server is running since 	 * server_updated should have handled it */
comment|/* keep the vers structure up to date in case we do a join 	 * - if there isn't a file, it can't very well have a version number, can it? 	 */
if|if
condition|(
name|vers
operator|->
...
condition|)
block|{
comment|/* FIXME: It seems like we should be preserving ts_user 		 *& ts_rcs here, but setting them causes problems in 		 * join_file(). 		 */
if|if
condition|(
name|vers_ts
operator|->
...
literal|0
expr_stmt|;
comment|/* Duplicating the client working file, so use the original sticky options.      */
name|retcode
operator|=
name|RCS_checkout
argument_list|(
...
name|file2_info
decl_stmt|;
comment|/* Check to make sure the patch is really shorter */
if|if
condition|(
name|CVS_STAT
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If this is really Update and not Checkout, record history.  */
if|if
condition|(
name|strcmp
argument_list|(
...
begin_comment
comment|/* Reregister a file after a merge.  */
end_comment
...
condition|)
block|{
comment|/* FIXME: Throwing away the original revision info is almost 	   certainly wrong -- what if join_rev1 is "BASE"?  */
if|if
condition|(
name|vers
operator|->
...
begin_comment
comment|/*  * Do all the magic associated with a file which needs to be joined  * (reached via the -j option to checkout or update).  *  * INPUTS  *   finfo		File information about the destination file.  *   vers		The Vers_TS structure for finfo.  *  * GLOBALS  *   join_rev1		From the command line.  *   join_rev2		From the command line.  *   server_active	Natch.  *  * ASSUMPTIONS  *   1.  Is not called in client mode.  */
end_comment
...
name|stderr
argument_list|,
literal|"%s-> join_file(%s, %s%s%s%s, %s, %s)\n"
argument_list|,
...
expr_stmt|;
block|}
comment|/* FIXME: Need to handle "BASE" for jrev1 and/or jrev2.  Note caveat        below about vn_user.  */
...
if|if
condition|(
comment|/* may have changed on destination branch */
comment|/* file added locally */
operator|!
name|strcmp
argument_list|(
name|vers
...
argument_list|)
operator|||
comment|/* destination branch modified in repository */
name|strcmp
argument_list|(
name|rev1
argument_list|,
...
condition|)
block|{
comment|/* The removal should happen if either the file has never changed 	     * on the destination or the file has changed to be identical to 	     * the first join revision. 	     * 	     * ------R-----------D 	     *       | 	     *       \----J1---J2-----S 	     * 	     * So: 	     * 	     * J2 is dead. 	     * D is destination. 	     * R is source branch root/GCA. 	     * if J1 == D       removal should happen 	     * if D == R        removal should happen 	     * otherwise, fail. 	     * 	     * (In the source, J2 = REV2, D = user file (potentially VN_USER), 	     * R = GCA computed below) 	     */
name|char
modifier|*
name|gca_rev1
init|=
...
comment|/* FIXME - see the FIXME comment above the call to RCS_checkout 		 * in the patch_file function. 		 */
name|retcode
operator|=
name|RCS_checkout
argument_list|(
...
if|if
condition|(
comment|/* genuinely changed on destination branch */
name|RCS_cmp_file
argument_list|(
name|vers
operator|->
...
argument_list|)
operator|&&
comment|/* genuinely different from REV1 */
name|RCS_cmp_file
argument_list|(
name|vers
operator|->
...
literal|0
argument_list|,
literal|"file %s has been removed in revision %s as of %s, but the destination is incompatibly modified"
argument_list|,
name|finfo
operator|->
name|fullname
...
literal|0
argument_list|,
literal|"file %s has been removed in revision %s, but the destination is incompatibly modified"
argument_list|,
name|finfo
operator|->
name|fullname
...
argument_list|)
expr_stmt|;
comment|/* Register the conflict with the client.  */
comment|/* FIXME: vers->ts_user should always be set here but sometimes 	     * isn't, namely when checkout_file() has just created the file, 	     * but simply setting it in checkout_file() appears to cause other 	     * problems. 	     */
if|if
condition|(
name|isfile
argument_list|(
...
directive|ifdef
name|SERVER_SUPPORT
comment|/* Abuse server_checked_in() to send the updated entry without 	     * needing to update the file. 	     */
if|if
condition|(
name|server_active
condition|)
...
return|return;
block|}
comment|/* The user file exists and has not been modified.  Mark it            for removal.  FIXME: If we are doing a checkout, this has            the effect of first checking out the file, and then            removing it.  It would be better to just register the            removal.  	 	   The same goes for a removal then an add.  e.g. 	   cvs up -rbr -jbr2 could remove and readd the same file 	 */
comment|/* save the rev since server_updated might invalidate it */
name|mrev
operator|=
name|xmalloc
argument_list|(
...
return|return;
block|}
comment|/* If the two merge revisions are the same, then there is nothing      * to do.  This needs to be checked before the rev2 == up-to-date base      * revision check tha comes next.  Otherwise, rev1 can == rev2 and get an      * "already contains the changes between<rev1> and<rev1>" message.      */
if|if
condition|(
name|rev1
operator|&&
...
return|return;
block|}
comment|/* If we know that the user file is up-to-date, then it becomes an      * optimization to skip the merge when rev2 is the same as the base      * revision.  i.e. we know that diff3(file2,file1,file2) will produce      * file2.      */
if|if
condition|(
name|vers
operator|->
...
operator|==
literal|0
comment|/* Avoid this in the text file case.  See below for why. 	 */
operator|&&
operator|(
name|strcmp
argument_list|(
...
condition|)
block|{
comment|/* We are dealing with binary files, or files with a 	   permission/linkage mismatch (this second case only occurs when 	   PRESERVE_PERMISSIONS_SUPPORT is enabled), and real merging would 	   need to take place.  This is a conflict.  We give the user 	   the two files, and let them resolve it.  It is possible 	   that we should require a "touch foo" or similar step before 	   we allow a checkin.  */
if|if
condition|(
name|RCS_checkout
argument_list|(
...
expr_stmt|;
block|}
comment|/* and skip the registering and sending the new file since it 	     * hasn't been updated. 	     */
goto|goto
name|out
goto|;
block|}
