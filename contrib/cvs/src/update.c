...
comment|/* I think that RCS_FLAGS_FORCE is here only because 		       passing -f to co used to enable checking out 		       a dead revision in the old version of death 		       support which used a hacked RCS instead of using 		       the RCS state.  */
name|retcode
operator|=
name|RCS_checkout
argument_list|(
...
block|{
comment|/* If the file is dead and does not contain either of 		       the join revisions, then we don't want to check it 		       out. */
return|return
literal|0
return|;
block|}
...
block|if (join_rev1&& join_rev2) 	{ 	    rev = RCS_getversion (vers->srcfile, join_rev2, date_rev2, 1, 0); 	    if (rev == NULL) 	    { 		if (!quiet&& date_rev2 == NULL) 		    error (0, 0, 			   "cannot find revision %s in file %s", join_rev2, file); 		return; 	    } 	     	    baserev = RCS_getversion (vers->srcfile, join_rev1, date_rev1, 1, 0); 	    if (baserev == NULL) 	    { 		if (!quiet&& date_rev1 == NULL) 		    error (0, 0, 			   "cannot find revision %s in file %s", join_rev1, file); 		free (rev); 		return; 	    }
...
block|char *dot = strrchr (baserev, '.'); 		int len = (dot - baserev) + 1; 		 		if (strncmp (baserev, rev, len) == 0) 		    return; 	    } 	} 	else 	{ 	    rev = RCS_getversion (vers->srcfile, join_rev1, date_rev1, 1, 0); 	    if (rev == NULL) 		return; 	    if (strcmp (rev, vers->vn_user) == 0)
comment|/* no merge necessary */
block|{ 		free (rev); 		return; 	    } 	     	    baserev = RCS_whatbranch (file, join_rev1, rcsnode); 	    if (baserev) 	    { 		char *cp;
