...
begin_comment
comment|/* #define	CVSUMASK_DFLT		   Set by config.h */
end_comment
...
name|vers_ts
block|{
comment|/* rcs version user file derives from, from CVS/Entries.      * it can have the following special values:      *    empty = no user file      *    0 = user file is new      *    -vers = user file to be removed.  */
name|char
modifier|*
name|vn_user
decl_stmt|;
...
name|vn_rcs
decl_stmt|;
comment|/* If ->tag corresponds to a tag which really exists in this file,        this is just a copy of ->tag.  If not, this is either NULL or        the head revision.  (Or something like that, see RCS_getversion        and friends).  */
name|char
modifier|*
name|vn_tag
decl_stmt|;
comment|/* This is the timestamp from stating the file in the working directory.        It is NULL if there is no file in the working directory.  */
name|char
modifier|*
name|ts_user
decl_stmt|;
...
name|ts_rcs
decl_stmt|;
comment|/* Options from CVS/Entries (keyword expansion).  */
name|char
modifier|*
name|options
decl_stmt|;
...
begin_comment
comment|/*  * structure used for list-private storage by Entries_Open() and  * Version_TS().  */
end_comment
...
begin_comment
comment|/* Remove locks set by lock_tree_for_write.  Currently removes readlocks    too.  */
end_comment
...
name|void
argument_list|(
argument|*error_set_cleanup PROTO((void (*) (void)))
argument_list|)
end_macro
