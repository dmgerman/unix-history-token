...
parameter_list|)
define|\
value|sp->conv.file2int(sp, n, nlen,&buf,&wlen,&w)
end_define
...
parameter_list|)
define|\
value|sp->conv.int2file(sp, w, wlen,&sp->cw,&nlen,&n)
end_define
...
parameter_list|)
define|\
value|sp->conv.sys2int(sp, n, nlen,&buf,&wlen,&w)
end_define
...
parameter_list|)
define|\
value|sp->conv.int2sys(sp, w, wlen,&sp->cw,&nlen,&n)
end_define
...
parameter_list|)
define|\
value|sp->conv.input2int(sp, n, nlen,&(cw),&wlen,&w)
end_define
...
parameter_list|)
define|\
value|FILE2INT5(sp,sp->cw,n,nlen,w,wlen)
end_define
...
parameter_list|)
define|\
value|CHAR2INT5(sp,sp->cw,n,nlen,w,wlen)
end_define
...
begin_comment
comment|/*  * Ex/vi commands are generally separated by whitespace characters.  We  * can't use the standard isspace(3) macro because it returns true for  * characters like ^K in the ASCII character set.  The POSIX isblank(3)  * has the same problem for non-ASCII locale, so we need a standalone one.  *  * XXX  * Note side effect, ch is evaluated multiple times.  */
end_comment
