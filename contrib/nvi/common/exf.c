...
block|{
comment|/* 		 * XXX 		 * A seat of the pants calculation: try to keep the file in 		 * 15 pages or less.  Don't use a page size larger than 16K 		 * (vi should have good locality) or smaller than 1K. 		 */
name|psize
operator|=
operator|(
operator|(
...
name|frp
expr_stmt|;
comment|/* Detect and set the file encoding */
name|file_encinit
argument_list|(
name|sp
argument_list|)
...
condition|)
block|{
comment|/* 			 * Ignore the empty strings and ".", since we've already 			 * tried the current directory. 			 */
if|if
condition|(
name|t
operator|<
...
begin_comment
comment|/*  * file_encinit --  *	Read the first line and set the O_FILEENCODING.  */
end_comment
...
index|]
decl_stmt|;
comment|/* not need to be '\0'-terminated */
name|recno_t
name|ln
init|=
literal|1
...
expr_stmt|;
block|}
comment|/* 	 * Detect UTF-8 and fallback to the locale/preset encoding. 	 * 	 * XXX 	 * A manually set O_FILEENCODING indicates the "fallback 	 * encoding", but UTF-8, which can be safely detected, is not 	 * inherited from the old screen. 	 */
if|if
condition|(
name|looks_utf8
argument_list|(
