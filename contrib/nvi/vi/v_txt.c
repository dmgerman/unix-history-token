...
index|[]
init|=
literal|"$Id: v_txt.c,v 11.5 2013/05/19 20:37:45 bentley Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * v_tcmd --  *	Fill a buffer from the terminal for vi.  *  * PUBLIC: int v_tcmd(SCR *, VICMD *, ARG_CHAR_T, u_int);  */
end_comment
...
begin_comment
comment|/*  * v_txt --  *	Vi text input.  *  * PUBLIC: int v_txt(SCR *, VICMD *, MARK *,  * PUBLIC:    const CHAR_T *, size_t, ARG_CHAR_T, recno_t, u_long, u_int32_t);  */
end_comment
...
name|M_ERR
argument_list|,
literal|"len %zu != cno: %zu ai: %zu insert %zu overwrite %zu"
argument_list|,
name|tp
operator|->
name|len
...
begin_comment
comment|/*  * v_txt_auto --  *	Handle autoindent.  If aitp isn't NULL, use it, otherwise,  *	retrieve the line.  *  * PUBLIC: int v_txt_auto(SCR *, recno_t, TEXT *, size_t, TEXT *);  */
end_comment
...
begin_comment
comment|/*  * txt_fc --  *	File name and ex command completion.  */
end_comment
...
block|}
block|}
comment|/* 	 * If we are at the first word, do ex command completion instead of 	 * file name completion. 	 */
if|if
condition|(
name|fstwd
condition|)
...
case|:
comment|/* One match. */
comment|/* Always overwrite the old text. */
name|nlen
operator|=
name|STRLEN
argument_list|(
...
break|break;
block|}
comment|/* Escape the matched part of the path. */
if|if
condition|(
name|fstwd
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If not a single match of path, we've done. */
if|if
condition|(
name|argc
operator|!=
...
operator|)
return|;
comment|/* If a single match and it's a directory, append a '/'. */
name|INT2CHAR
argument_list|(
name|sp
argument_list|,
