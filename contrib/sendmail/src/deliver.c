...
return|return;
block|}
comment|/* Don't attempt deliveries if we want to bounce now */
if|if
condition|(
operator|!
name|bitset
...
name|syserr
argument_list|(
literal|"554 5.3.5 SMTP style mailer not implemented"
argument_list|)
expr_stmt|;
return|return
name|EX_SOFTWARE
...
name|syserr
argument_list|(
literal|"554 5.3.5 openmailer: no IPC"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|syserr
argument_list|(
literal|"openmailer: insufficient privileges to change gid"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
name|syserr
argument_list|(
literal|"openmailer: insufficient privileges to change uid"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
goto|;
block|}
comment|/* Don't cache non-clever connections */
if|if
condition|(
operator|!
name|clever
...
name|reconnect
label|:
comment|/* after switching to an authenticated connection */
endif|#
directive|endif
comment|/* STARTTLS || (SASL&& SFIO) */
if|#
...
condition|)
block|{
comment|/* 			**  should we require some minimum authentication? 			**  XXX ignore result? 			*/
if|if
condition|(
name|smtpauth
argument_list|(
...
name|NOQID
argument_list|,
literal|"SASL: outgoing connection to %.64s: mech=%.16s, bits=%d"
argument_list|,
name|mci
operator|->
name|mci_host
...
name|syserr
argument_list|(
literal|"SASL TLS switch failed in client"
argument_list|)
expr_stmt|;
block|}
comment|/* else? XXX */
...
name|syserr
argument_list|(
literal|"554 5.3.5 deliver: need SMTP compiled to use clever mailer"
argument_list|)
expr_stmt|;
name|rcode
operator|=
...
name|dprintf
argument_list|(
literal|"giveresponse: status=%d, dsn=%s, e->e_message=%s\n"
argument_list|,
name|status
argument_list|,
name|dsn
...
block|}
block|}
comment|/* paranoia: the df file should always be in a rewound state */
operator|(
name|void
operator|)
name|bfrewind
...
name|syserr
argument_list|(
literal|"putbody: %s/df%s: read error"
argument_list|,
name|qid_printqueue
argument_list|(
name|e
...
name|NULL
expr_stmt|;
comment|/* check file mode to see if setuid */
if|if
condition|(
name|stat
argument_list|(
...
name|dprintf
argument_list|(
literal|"mailfile: ignoring setuid/setgid bits\n"
argument_list|)
expr_stmt|;
block|}
...
name|syserr
argument_list|(
literal|"mailfile: insufficient privileges to change uid"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
name|syserr
argument_list|(
literal|"mailfile: insufficient privileges to change gid"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
begin_comment
comment|/* **  INITCLTTLS -- initialize client side TLS ** **	Parameters: **		none. ** **	Returns: **		succeeded? */
end_comment
...
name|e_id
argument_list|,
literal|"TLS: error: client: SSL_new failed"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|e_id
argument_list|,
literal|"TLS: error: SSL_set_xfd failed=%d"
argument_list|,
name|result
argument_list|)
expr_stmt|;
...
name|e_id
argument_list|,
literal|"TLS: error: SSL_connect failed=%d (%d)"
argument_list|,
name|result
argument_list|,
name|i
...
begin_comment
comment|/* **  ENDTLS -- shutdown secure connection ** **	Parameters: **		ssl -- SSL connection information. **		side -- srv/clt (for logging). ** **	Returns: **		success? */
end_comment
...
name|NOQID
argument_list|,
literal|"SSL_shutdown %s failed: %d"
argument_list|,
name|side
argument_list|,
name|r
...
name|NOQID
argument_list|,
literal|"SSL_shutdown %s not done"
argument_list|,
name|side
argument_list|)
expr_stmt|;
