...
begin_comment
comment|/*  * Copyright (c) 1998-2010, 2012-2014 Proofpoint, Inc. and its suppliers.  *	All rights reserved.  * Copyright (c) 1983, 1995-1997 Eric P. Allman.  All rights reserved.  * Copyright (c) 1988, 1993  *	The Regents of the University of California.  All rights reserved.  *  * By using this file, you agree to the terms and conditions set  * forth in the LICENSE file which can be found at the top level of  * the sendmail distribution.  *  */
end_comment
...
name|SM_RCSID
argument_list|(
literal|"@(#)$Id: srvrsmtp.c,v 8.1016 2013-11-22 20:51:56 ca Exp $"
argument_list|)
end_macro
...
begin_comment
comment|/* **  ADDRCPT -- Add a rcpt to sendq list ** **	Parameters: **		rcpt -- rcpt **		sendq -- a pointer to the head of a queue to put **			these people into. **		e -- the envelope in which to add these recipients. ** **	Returns: **		The number of addresses added to the list. */
end_comment
...
literal|"<>"
expr_stmt|;
comment|/* disable alias expansion? */
name|a
operator|=
name|recipient
argument_list|(
...
name|sm_dprintf
argument_list|(
literal|"addrcpt: rcpt=%s, flags=%#lx\n"
argument_list|,
name|rcpt
argument_list|,
name|a
...
begin_comment
comment|/* **  ADDBCC -- Maybe create a copy of an e-mail ** **	Parameters: **		a -- current RCPT **		e -- the envelope. ** **	Returns: **		nothing ** **	Side Effects: **		rscheck() can trigger an "exception" */
end_comment
...
name|sm_dprintf
argument_list|(
literal|"addbcc: nobcc=%d, Errors=%d, newrcpt=<%s>\n"
argument_list|,
name|nobcc
argument_list|,
name|Errors
...
begin_comment
comment|/* **  RCPTMODS -- Perform rcpt modifications if requested ** **	Parameters: **		rcpt -- current RCPT **		e -- the envelope. ** **	Returns: **		nothing. */
end_comment
...
name|sm_dprintf
argument_list|(
literal|"rcptmods: rcpt=%s, flags=%s\n"
argument_list|,
name|rcpt
operator|->
name|q_paddr
...
name|e_id
argument_list|,
literal|"rcpt=%s, rcpt_flags=%s, status=unknown"
argument_list|,
name|rcpt
operator|->
name|q_paddr
...
block|}
block|}
comment|/* reset macro to avoid confusion later on */
name|macdefine
argument_list|(
operator|&
name|e
...
begin_comment
comment|/* **  Determine the correct protocol keyword to use in the **  Received: header, following RFC 3848. */
end_comment
...
directive|define
name|auth_active
value|(authenticating == SASL_IS_AUTH)
end_define
...
parameter_list|()
define|\
value|(auth_active					\ 	    ? (tls_active ? "ESMTPSA" : "ESMTPA")	\ 	    : (tls_active ? "ESMTPS"  : "ESMTP"))
end_define
...
name|greetmsg
init|=
literal|"not accepting messages"
decl_stmt|;
name|char
modifier|*
name|hostname
...
name|e_id
argument_list|,
literal|"Milter: connect: host=%s, addr=%s, reject=%s"
argument_list|,
name|peerhostname
argument_list|,
name|anynet_ntoa
...
literal|4
expr_stmt|;
comment|/* skip over enhanced status code */
name|skip
operator|=
name|isenhsc
argument_list|(
...
name|e_id
argument_list|,
literal|"Milter: connect: host=%s, addr=%s, shutdown=%s"
argument_list|,
name|peerhostname
argument_list|,
name|anynet_ntoa
...
name|e_id
argument_list|,
literal|"Milter: connect: host=%s, addr=%s, temp failing commands=%s"
argument_list|,
name|peerhostname
argument_list|,
name|anynet_ntoa
...
name|message
argument_list|(
literal|"454 4.3.3 TLS not available: error setting options"
argument_list|)
expr_stmt|;
name|SSL_free
argument_list|(
...
condition|)
block|{
comment|/* is proxy mode possible? */
if|if
condition|(
name|e
operator|->
...
name|sm_dprintf
argument_list|(
literal|"srv: mode=%c, e=%p, sibling=%p, nrcpts=%d, sm_nrcpts=%d, sendqueue=%p, next=%p\n"
argument_list|,
name|e
operator|->
name|e_sendmode
...
expr_stmt|;
block|}
comment|/* switch to interactive mode */
name|e
operator|->
name|e_sendmode
operator|=
...
name|e_id
argument_list|,
literal|"proxy mode requested but not possible"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|sm_dprintf
argument_list|(
literal|"srv: mode=%c, e=%p, sibling=%p, nrcpts=%d, msg=%s, sendqueue=%p, next=%p, state=%d, SmtpError=%s, rcode=%d, renhsc=%s, text=%s\n"
argument_list|,
name|e
operator|->
name|e_sendmode
...
name|e
parameter_list|)
value|(((e)->e_message != NULL) ? (e)->e_message : "")
name|m
operator|=
name|e
operator|->
...
name|sm_dprintf
argument_list|(
literal|"srv: SmtpError=%s, rcode=%d, renhsc=%s, replycode=%s, enhsc=%s, offset=%d\n"
argument_list|,
name|SmtpError
argument_list|,
name|e
...
name|message
argument_list|(
literal|"450 4.5.1 Temporary error"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|message
argument_list|(
literal|"550 5.0.0 Permanent error"
argument_list|)
expr_stmt|;
block|}
block|}
