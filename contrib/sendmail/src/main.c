...
begin_comment
comment|/* **  INTSIG -- clean up on interrupt ** **	This just arranges to exit.  It pessimizes in that it **	may resend a message. ** **	Parameters: **		none. ** **	Returns: **		none. ** **	Side Effects: **		Unlocks the current job. */
end_comment
...
name|NOQID
argument_list|,
literal|"restarting %s %s"
argument_list|,
name|sig
operator|==
literal|0
condition|?
literal|"due to control command"
else|:
literal|"on signal"
argument_list|,
name|SaveArgv
...
argument_list|)
expr_stmt|;
comment|/* Control socket restart? */
if|if
condition|(
name|sig
operator|!=
