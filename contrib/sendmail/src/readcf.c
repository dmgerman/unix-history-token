...
begin_comment
comment|/* ** READSSLOPTIONS  -- read SSL_OP_* values ** **	Parameters: **		opt -- name of option (can be NULL) **		val -- string with SSL_OP_* values or hex value **		delim -- end of string (e.g., '\0' or ';') **		pssloptions -- return value (output) ** **	Returns: **		0 on success. */
end_comment
...
begin_comment
comment|/* ** GET_TLS_SE_OPTIONS -- get TLS session options (from ruleset) ** **	Parameters: **		e -- envelope **		ssl -- TLS session context **		srv -- server? ** **	Returns: **		0 on success. */
end_comment
...
directive|define
name|who
value|(srv ? "server" : "client")
define|#
directive|define
name|NAME_C_S
value|macvalue(macid(srv ? "{client_name}" : "{server_name}"), e)
define|#
directive|define
name|ADDR_C_S
value|macvalue(macid(srv ? "{client_addr}" : "{server_addr}"), e)
define|#
directive|define
name|WHICH
value|srv ? "srv" : "clt"
...
name|srv
condition|?
literal|"tls_srv_features"
else|:
literal|"tls_clt_features"
argument_list|,
name|NAME_C_S
argument_list|,
name|ADDR_C_S
...
name|NOQID
argument_list|,
literal|"rscheck(tls_%s_features)=failed, relay=%s [%s], errors=%d"
argument_list|,
name|WHICH
argument_list|,
name|NAME_C_S
...
name|NOQID
argument_list|,
literal|"tls_%s_features=%s, relay=%s [%s]"
argument_list|,
name|WHICH
argument_list|,
name|optionlist
...
name|NOQID
argument_list|,
literal|"tls_%s_features=empty, relay=%s [%s]"
argument_list|,
name|WHICH
argument_list|,
name|NAME_C_S
...
name|NOQID
argument_list|,
literal|"tls_%s_features=too_short, relay=%s [%s]"
argument_list|,
name|WHICH
argument_list|,
name|NAME_C_S
...
argument_list|)
expr_stmt|;
comment|/* this is not treated as error! */
return|return
literal|0
return|;
block|}
...
name|INVALIDSYNTAX
define|\
value|do {	\ 		if (LogLevel> 7)	\ 			sm_syslog(LOG_INFO, NOQID,	\ 				  "tls_%s_features=invalid_syntax, opt=%s, relay=%s [%s]",	\ 		  		  WHICH, opt, NAME_C_S, ADDR_C_S);	\ 		return -1;	\ 	} while (0)
define|#
directive|define
name|CHECKLEN
define|\
value|do {	\ 		if (i>= len)	\ 			INVALIDSYNTAX;	\ 	} while (0)
define|#
directive|define
name|SKIPWS
define|\
value|do {	\ 		while (i< len&& SM_ISSPACE(optionlist[i]))	\ 			++i;	\ 		CHECKLEN;	\ 	} while (0)
comment|/* parse and handle opt=val; */
do|do
block|{
name|char
name|sep
...
name|NOQID
argument_list|,
literal|"tls_%s_features=parsed, %s=%s, relay=%s [%s]"
argument_list|,
name|WHICH
argument_list|,
name|opt
...
name|NOQID
argument_list|,
literal|"tls_%s_features=%s, error=%s, relay=%s [%s]"
argument_list|,
name|WHICH
argument_list|,
name|val
...
name|NOQID
argument_list|,
literal|"STARTTLS=%s, error: SSL_set_cipher_list(%s) failed"
argument_list|,
name|who
argument_list|,
name|val
...
name|NOQID
argument_list|,
literal|"tls_%s_features=unknown_option, opt=%s, relay=%s [%s]"
argument_list|,
name|WHICH
argument_list|,
name|opt
...
condition|)
do|;
comment|/* need cert and key before we can use the options */
comment|/* does not implement the "," hack for 2nd cert/key pair */
if|if
condition|(
name|keyfile
operator|!=
...
name|NOQID
argument_list|,
literal|"tls_%s_features=only_one_of_CertFile/KeyFile_specified, relay=%s [%s]"
argument_list|,
name|WHICH
argument_list|,
name|NAME_C_S
...
define|#
directive|define
name|O_USECOMPRESSEDIPV6ADDRESSES
value|0xec
block|{
literal|"UseCompressedIPv6Addresses"
block|,
name|O_USECOMPRESSEDIPV6ADDRESSES
block|,
name|OI_NONE
