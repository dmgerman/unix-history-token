...
begin_comment
comment|/*  * Copyright (c) 1999-2009, 2012, 2013 Proofpoint, Inc. and its suppliers.  *	All rights reserved.  *  * By using this file, you agree to the terms and conditions set  * forth in the LICENSE file which can be found at the top level of  * the sendmail distribution.  *  */
end_comment
...
name|SM_RCSID
argument_list|(
literal|"@(#)$Id: milter.c,v 8.281 2013-11-22 20:51:56 ca Exp $"
argument_list|)
end_macro
...
begin_comment
comment|/* **  MilterMacros contains the milter macros for each milter and each stage. **  indices are (in order): stages, milter-index, macro **  milter-index == 0: "global" macros (not for a specific milter). */
end_comment
...
begin_comment
comment|/* set state in case of an error */
end_comment

begin_define
define|#
directive|define
name|MILTER_SET_STATE
define|\
value|if (bitnset(SMF_TEMPFAIL, m->mf_flags)) \ 		*state = SMFIR_TEMPFAIL; \ 	else if (bitnset(SMF_TEMPDROP, m->mf_flags)) \ 		*state = SMFIR_SHUTDOWN; \ 	else if (bitnset(SMF_REJECT, m->mf_flags)) \ 		*state = SMFIR_REJECT
end_define

begin_comment
comment|/* flow through code maybe using continue; don't wrap in do {} while */
end_comment
...
index|]
expr_stmt|;
comment|/* send macros (regardless of whether we send cmd) */
if|if
condition|(
name|macros
operator|!=
...
begin_comment
comment|/* flow through code using continue; don't wrap in do {} while */
end_comment
...
parameter_list|()
define|\
value|if (m->mf_state == SMFS_ERROR&& *state == SMFIR_CONTINUE) \ 	{ \ 			MILTER_SET_STATE;	\ 	} \ 	if (*state == SMFIR_ACCEPT || \ 	    m->mf_state == SMFS_DONE || \ 	    m->mf_state == SMFS_ERROR) \ 	{ \ 		if (m->mf_state != SMFS_ERROR) \ 			m->mf_state = SMFS_DONE; \ 		continue;
