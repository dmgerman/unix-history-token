...
index|[]
init|=
literal|"@(#)$Id: recipient.c,v 8.231.14.5 2000/06/27 20:15:46 gshapiro Exp $"
decl_stmt|;
end_decl_stmt
...
name|QINHERITEDBITS
expr_stmt|;
comment|/* original recipient information */
name|a
operator|->
name|q_orcpt
operator|=
...
begin_comment
comment|/* **  RECIPIENT -- Designate a message recipient ** **	Saves the named person for future mailing. ** **	Parameters: **		a -- the (preparsed) address header for the recipient. **		sendq -- a pointer to the head of a queue to put the **			recipient in.  Duplicate suppression is done **			in this queue. **		aliaslevel -- the current alias nesting depth. **		e -- the current envelope. ** **	Returns: **		The actual address in the queue.  This will be "a" if **		the address is not a duplicate, else the original address. ** **	Side Effects: **		none. */
end_comment
...
directive|if
literal|0
block|if (strcasecmp(pw->pw_name, name) == 0) 		{ 			if (tTd(29, 4)) 				dprintf("found (case wrapped)\n"); 			break; 		}
endif|#
directive|endif
comment|/* 0 */
name|buildfname
...
comment|/* additional: (?) 			ctladdr->q_state = QS_DONTSEND; 			**/
name|syserr
argument_list|(
literal|"Attempt to forward to more then %d addresses (in %s)!"
argument_list|,
name|MaxForwardEntries
argument_list|,
name|fname
