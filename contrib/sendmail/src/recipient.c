...
index|[]
init|=
literal|"@(#)recipient.c	8.163 (Berkeley) 1/23/1999"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* **  RECIPIENT -- Designate a message recipient ** **	Saves the named person for future mailing. ** **	Parameters: **		a -- the (preparsed) address header for the recipient. **		sendq -- a pointer to the head of a queue to put the **			recipient in.  Duplicate supression is done **			in this queue. **		aliaslevel -- the current alias nesting depth. **		e -- the current envelope. ** **	Returns: **		The actual address in the queue.  This will be "a" if **		the address is not a duplicate, else the original address. ** **	Side Effects: **		none. */
end_comment
...
name|newstr
argument_list|(
literal|"Unsafe for mailing to programs"
argument_list|)
expr_stmt|;
name|usrerr
argument_list|(
...
name|newstr
argument_list|(
literal|"Unsafe for mailing to files"
argument_list|)
expr_stmt|;
name|usrerr
argument_list|(
...
condition|)
block|{
comment|/* /dev/null is always accepted */
block|}
elseif|else
if|if
condition|(
