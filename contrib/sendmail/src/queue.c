...
name|SM_RCSID
argument_list|(
literal|"@(#)$Id: queue.c,v 8.834 2002/01/08 23:04:58 ca Exp $"
argument_list|)
end_macro
...
begin_comment
comment|/* **  use of DoQueueRun: **	NumQueue: indicates that a queue run is needed, look at individual bits **	0 - NumQueue-1: indicates that a queue run for this queue group **		is needed. */
end_comment
...
begin_comment
comment|/* **  SCHEDULE_QUEUE_RUNS -- schedule the next queue run for a work group. ** **	Parameters: **		runall -- schedule even if individual bit is not set. **		wgrp -- the work group id to schedule. ** **	Returns: **		nothing */
end_comment
...
name|NOQID
argument_list|,
literal|"sqr: wgrp=%d, cgrp=%d, qgrp=%d, intvl=%ld, QI=%ld, runall=%d, bit=%d, sched=%d"
argument_list|,
name|wgrp
argument_list|,
name|cgrp
...
argument_list|)
expr_stmt|;
comment|/* if it is a specific group: set that bit */
if|if
condition|(
name|qgrp
operator|!=
...
goto|;
block|}
comment|/* for all others: set the bit if it doesn't have a queue interval */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* the file is already on disk */
name|e
operator|->
name|e_flags
operator||=
...
begin_comment
comment|/* **  UPD_QS -- update information about queue when adding/deleting an entry ** **	Parameters: **		e -- envelope. **		delete -- delete/add entry. **		avail -- update the space available as well. ** **	Returns: **		none. ** **	Side Effects: **		Modifies available space in filesystem. **		Changes number of entries in queue directory. */
end_comment
...
control|)
block|{
comment|/* XXX: maybe allow read access for group? */
name|Pshm
operator|=
name|sm_shmstart
argument_list|(
...
directive|endif
comment|/* SM_CONF_SHM */
comment|/* initialize map for queue runs */
name|clrbitmap
argument_list|(
name|DoQueueRun
argument_list|)
...
directive|if
literal|0
block|qg->qg_sortorder = QueueSortOrder;
endif|#
directive|endif
comment|/* 0 */
name|qg
...
block|switch (*p) 			{ 			  case 'h':
comment|/* Host first */
block|case 'H': 				qg->qg_sortorder = QSO_BYHOST; 				break;  			  case 'p':
comment|/* Priority order */
block|case 'P': 				qg->qg_sortorder = QSO_BYPRIORITY; 				break;  			  case 't':
comment|/* Submission time */
block|case 'T': 				qg->qg_sortorder = QSO_BYTIME; 				break;  			  case 'f':
comment|/* File name */
block|case 'F': 				qg->qg_sortorder = QSO_BYFILENAME; 				break;  			  case 'm':
comment|/* Modification time */
block|case 'M': 				qgrp->qg_sortorder = QSO_BYMODTIME; 				break;  			  default: 				syserr("Invalid queue sort order \"%s\"", p); 			} 			break;
endif|#
directive|endif
comment|/* 0 */
default|default:
...
directive|endif
comment|/* _FFR_QUARANTINE */
comment|/* 	**  XXX Not sure if this copying is necessary. 	**  sendall() does this copying, but I don't know if that is 	**  because of the storage management discipline we were using 	**  before rpools were introduced, or if it is because these lists 	**  can be modified later. 	*/
name|ee
operator|->
name|e_header
operator|=
