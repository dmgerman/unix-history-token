...
begin_comment
comment|/*  *  Copyright (c) 2003-2004, 2007, 2009-2012 Proofpoint, Inc. and its suppliers.  *	All rights reserved.  *  * By using this file, you agree to the terms and conditions set  * forth in the LICENSE file which can be found at the top level of  * the sendmail distribution.  *  * Contributed by Jose Marcio Martins da Cruz - Ecole des Mines de Paris  *   Jose-Marcio.Martins@ensmp.fr  */
end_comment
...
name|SM_RCSID
argument_list|(
literal|"@(#)$Id: worker.c,v 8.25 2013-11-22 20:51:37 ca Exp $"
argument_list|)
end_macro
...
literal|0
decl_stmt|;
comment|/* this can happen if the milter is shutting down */
if|if
condition|(
name|Tskmgr
operator|.
...
begin_comment
comment|/* **  NONBLOCKING -- set nonblocking mode for a file descriptor. ** **	Parameters: **		fd -- file descriptor **		name -- name for (error) logging ** **	Returns: **		MI_SUCCESS/MI_FAILURE */
end_comment
...
name|SMI_LOG_ERR
argument_list|,
literal|"fcntl(%s, F_SETFL, O_NONBLOCK)=%s"
argument_list|,
name|name
argument_list|,
name|sm_errstring
