...
condition|)
block|{
comment|/* Bash 2.0 puts a special variable in the environment for each 	 command it runs, specifying which ARGV elements are the results of 	 file name wildcard expansion and therefore should not be 	 considered as options.  */
name|char
name|var
index|[
literal|100
...
name|var
argument_list|,
literal|"_%d_GNU_nonoption_argv_flags_"
argument_list|,
name|getpid
argument_list|()
argument_list|)
...
block|}
comment|/* Test whether ARGV[optind] points to a non-option argument.      Either it does not have option syntax, or there is an environment flag      from the shell indicating it is not an option.  */
define|#
directive|define
name|NONOPTION_P
value|(argv[optind][0] != '-' || argv[optind][1] == '\0'	      \ 		     || (optind< nonoption_flags_len			      \&& nonoption_flags[optind] == '1'))
if|if
condition|(
name|nextchar
operator|==
