...
begin_comment
comment|/* symbols.c -symbol table-    Copyright 1987, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007    Free Software Foundation, Inc.     This file is part of GAS, the GNU Assembler.     GAS is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2, or (at your option)    any later version.     GAS is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with GAS; see the file COPYING.  If not, write to the Free    Software Foundation, 51 Franklin Street - Fifth Floor, Boston, MA    02110-1301, USA.  */
end_comment
...
name|as_fatal
argument_list|(
literal|"bfd_make_empty_symbol: %s"
argument_list|,
name|bfd_errmsg
argument_list|(
name|bfd_get_error
...
name|_
argument_list|(
literal|"cannot define symbol `%s' in absolute section"
argument_list|)
argument_list|,
name|sym_name
argument_list|)
...
name|bsymnew
decl_stmt|;
comment|/* Running local_symbol_convert on a clone that's not the one currently      in local_hash would incorrectly replace the hash entry.  Thus the      symbol must be converted here.  Note that the rest of the function      depends on not encountering an unconverted symbol.  */
if|if
condition|(
name|LOCAL_SYMBOL_CHECK
argument_list|(
...
begin_comment
comment|/* Referenced symbols, if they are forward references, need to be cloned    (without replacing the original) so that the value of the referenced    symbols at the point of use .  */
end_comment
...
condition|)
block|{
comment|/* assign_symbol() clones volatile symbols; pre-existing expressions 	     hold references to the original instance, but want the current 	     value.  Just repeat the lookup.  */
if|if
condition|(
name|add_symbol
operator|&&
...
expr_stmt|;
block|}
comment|/* Re-using sy_resolving here, as this routine cannot get called from 	 symbol resolution code.  */
if|if
condition|(
name|symbolP
operator|->
...
operator|)
expr_stmt|;
comment|/* Any references to the symbol, except for the reference in      .weakref, must clear this flag, such that the symbol does not      turn into a weak symbol.  Note that we don't have to handle the      local_symbol case, since a weakrefd is always promoted out of the      local_symbol table when it is turned into a weak symbol.  */
if|if
condition|(
name|sym
operator|&&
...
name|_
argument_list|(
literal|"invalid sections for operation on `%s' and `%s'"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
name|_
argument_list|(
literal|"invalid section for operation on `%s'"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
name|_
argument_list|(
literal|"invalid sections for operation on `%s' and `%s' setting `%s'"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
name|_
argument_list|(
literal|"invalid section for operation on `%s' setting `%s'"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* For debugging, print out conversion input& output.  */
ifdef|#
directive|ifdef
name|DEBUG_SYMS
name|print_expr
...
name|_
argument_list|(
literal|"cannot convert expression symbol %s to complex relocation"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* S_CLEAR_EXTERNAL (relc_symbol); */
if|if
condition|(
name|symp
operator|->
...
name|BSF_RELC
expr_stmt|;
comment|/* symp->bsym->flags |= BSF_RELC; */
...
argument_list|)
expr_stmt|;
comment|/* By reducing these to the relevant dyadic operator, we get 	     	!S -> S == 0 	permitted on anything, 		-S -> 0 - S 	only permitted on absolute 		~S -> S ^ ~0 	only permitted on absolute  */
if|if
condition|(
name|op
operator|!=
...
argument_list|)
expr_stmt|;
comment|/* However do not move the symbol into the absolute section 		 if it cannot currently be resolved - this would confuse 		 other parts of the assembler into believing that the 		 expression had been evaluated to zero.  */
else|else
name|move_seg_ok
operator|=
literal|0
...
begin_comment
comment|/* Obtain the current value of a symbol without changing any    sub-expressions used.  */
end_comment
...
block|}
block|}
comment|/* Never change a defined symbol.  */
if|if
condition|(
name|symbolP
operator|->
...
literal|0
return|;
comment|/* Conceptually, a weakrefr is weak if the referenced symbol is.  We      could probably handle a WEAKREFR as always weak though.  E.g., if      the referenced symbol has lost its weak status, there's no reason      to keep handling the weakrefr as if it was weak.  */
if|if
condition|(
name|S_IS_WEAKREFR
argument_list|(
...
comment|/* Keep BSF_FILE symbols in order to allow debuggers to identify 	 the source file even when the object file is stripped.  */
operator|&&
operator|(
name|flags
operator|&
...
literal|1
expr_stmt|;
comment|/* If the alias was already used, make sure we mark the target as      used as well, otherwise it might be dropped from the symbol      table.  This may have unintended side effects if the alias is      later redirected to another symbol, such as keeping the unused      previous target in the symbol table.  Since it will be weak, it's      not a big deal.  */
if|if
condition|(
name|s
operator|->
...
literal|0
expr_stmt|;
comment|/* If a weakref target symbol is weak, then it was never 	 referenced directly before, not even in a .global directive, 	 so decay it to local.  If it remains undefined, it will be 	 later turned into a global, like any other undefined 	 symbol.  */
if|if
condition|(
name|s
operator|->
...
name|_
argument_list|(
literal|"Accessing function `%s' as thread-local object"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
name|_
argument_list|(
literal|"Accessing `%s' as thread-local object"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
begin_comment
comment|/* Return a pointer to the X_add_number component of a symbol.  */
end_comment
...
begin_comment
comment|/* Return whether a symbol was cloned and thus removed from the global    symbol list.  */
end_comment
...
name|bsym
expr_stmt|;
comment|/* else XXX - What do we do now ?  */
block|}
end_function
...
argument_list|)
expr_stmt|;
comment|/* Recurse to symbol_relc_make_expr if this symbol      is defined as an expression or a plain value.  */
if|if
condition|(
name|S_GET_SEGMENT
argument_list|(
...
argument_list|)
return|;
comment|/* This may be a "fake symbol" L0\001, referring to ".".      Write out a special null symbol to refer to this position.  */
if|if
condition|(
operator|!
name|strcmp
...
argument_list|)
return|;
comment|/* We hope this is a plain leaf symbol.  Construct the encoding      as {S,s}II...:CCCCCCC....      where 'S'/'s' means section symbol / plain symbol      III is decimal for the symbol name length      CCC is the symbol name itself.  */
name|symbol_mark_used_in_reloc
argument_list|(
name|sym
argument_list|)
...
begin_comment
comment|/* Convert given value to a new complex-relocation symbol name.  This    is a non-recursive function, since it is be called for leaf nodes    (plain values) in the expression tree.  The caller owns the    returning string, so should free() it eventually.  No errors.  */
end_comment
...
begin_comment
comment|/* Convert given expression to a new complex-relocation symbol name.    This is a recursive function, since it traverses the entire given    expression tree.  The caller owns the returning string, so should    free() it eventually.  Errors are indicated via as_bad() and a NULL    return value.  */
end_comment
...
name|NULL
decl_stmt|;
comment|/* Operator prefix string.  */
name|int
name|arity
init|=
literal|0
decl_stmt|;
comment|/* Arity of this operator.  */
name|char
modifier|*
name|operands
index|[
...
argument_list|)
expr_stmt|;
comment|/* Match known operators -> fill in opstr, arity, operands[] and fall      through to construct subexpression fragments; may instead return       string directly for leaf nodes.  */
comment|/* See expr.h for the meaning of all these enums.  Many operators       have an unnatural arity (X_add_number implicitly added).  The      conversion logic expands them to explicit "+" subexpressions.   */
switch|switch
condition|(
name|exp
operator|->
...
name|as_bad
argument_list|(
literal|"Unknown expression operator (enum %d)"
argument_list|,
name|exp
operator|->
name|X_op
...
argument_list|)
return|;
comment|/* Helper macros for nesting nodes.  */
define|#
directive|define
name|HANDLE_XADD_OPT1
parameter_list|(
name|str_
parameter_list|)
define|\
value|if (exp->X_add_number)						\         {								\           arity = 2;							\           opstr = "+:" str_;						\           operands[0] = symbol_relc_make_sym (exp->X_add_symbol);	\           operands[1] = symbol_relc_make_value (exp->X_add_number);	\           break;							\         }								\       else								\         {								\           arity = 1;							\           opstr = str_;							\           operands[0] = symbol_relc_make_sym (exp->X_add_symbol);	\         }								\       break
define|#
directive|define
name|HANDLE_XADD_OPT2
parameter_list|(
name|str_
parameter_list|)
define|\
value|if (exp->X_add_number)						\         {								\           arity = 3;							\           opstr = "+:" str_;						\           operands[0] = symbol_relc_make_sym (exp->X_add_symbol);	\           operands[1] = symbol_relc_make_sym (exp->X_op_symbol);	\           operands[2] = symbol_relc_make_value (exp->X_add_number);	\         }								\       else								\         {								\           arity = 2;							\           opstr = str_;							\           operands[0] = symbol_relc_make_sym (exp->X_add_symbol);	\           operands[1] = symbol_relc_make_sym (exp->X_op_symbol);	\         } 								\       break
comment|/* Nesting nodes.  */
case|case
name|O_uminus
case|:
name|HANDLE_XADD_OPT1
...
block|{
comment|/* Allocate new string; include inter-operand padding gaps etc.  */
name|concat_string
operator|=
name|xmalloc
argument_list|(
...
expr_stmt|;
block|}
comment|/* Free operand strings (not opstr).  */
if|if
condition|(
name|arity
operator|>=
