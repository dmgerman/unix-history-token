...
block|}
block|}
comment|/* We are at the beginning of a line, or similar place. 	     We expect a well-formed assembler statement. 	     A "symbol-name:" is a statement.  	     Depending on what compiler is used, the order of these tests 	     may vary to catch most common case 1st. 	     Each test is independent of all other tests at the (top) 	     level.  */
do|do
name|c
operator|=
operator|*
...
operator|--
expr_stmt|;
comment|/* Report unknown char as error.  */
name|demand_empty_rest_of_line
argument_list|()
expr_stmt|;
block|}
...
name|_
argument_list|(
literal|"incompatible flag %i in line directive"
argument_list|)
argument_list|,
name|this_flag
argument_list|)
...
name|_
argument_list|(
literal|"unsupported flag %i in line directive"
argument_list|)
argument_list|,
name|this_flag
argument_list|)
...
begin_comment
comment|/* Parse an attribute directive for VENDOR.  */
end_comment
...
name|_
argument_list|(
literal|"expected numeric constant"
argument_list|)
argument_list|)
expr_stmt|;
name|ignore_rest_of_line
...
begin_comment
comment|/* Parse a .gnu_attribute directive.  */
end_comment
...
name|OBJ_MAYBE_ELF
argument_list|)
comment|/* The ELF backend needs to know that we are changing sections, so      that .previous works correctly. */
if|if
condition|(
name|IS_ELF
condition|)
...
begin_comment
comment|/* .reloc offset, reloc_name, symbol+addend.  */
end_comment
...
name|_
argument_list|(
literal|"missing or bad offset expression"
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
