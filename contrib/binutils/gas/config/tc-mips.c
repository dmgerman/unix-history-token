...
literal|1
decl_stmt|;
comment|/* True if this instruction occured in a .set noreorder block.  */
name|unsigned
name|int
name|noreorder_p
range|:
...
begin_comment
comment|/* Return true if the given CPU supports the DSP ASE.  */
end_comment
...
begin_comment
comment|/* Return true if the given CPU supports the MT ASE.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Search until we get a match for NAME.  It is assumed here that      macros will never generate MDMX or MIPS-3D instructions.  */
while|while
condition|(
name|strcmp
argument_list|(
...
literal|'!'
case|:
comment|/* mt 1-bit unsigned immediate in bit 5 */
name|my_getExpression
argument_list|(
operator|&
name|imm_expr
...
name|_
argument_list|(
literal|"MT immediate not in range 0..%d (%lu)"
argument_list|)
argument_list|,
name|OP_MASK_MT_U
argument_list|,
...
literal|'$'
case|:
comment|/* mt 1-bit unsigned immediate in bit 4 */
name|my_getExpression
argument_list|(
operator|&
name|imm_expr
...
parameter_list|)
block|{
comment|/* Don't adjust MIPS16 jump relocations, so we don't have to worry      about the format of the offset in the .o file. */
if|if
condition|(
name|fixp
operator|->
...
directive|ifdef
name|OBJ_ELF
comment|/* Don't adjust relocations against mips16 symbols, so that the linker      can find them if it needs to set up a stub.  */
if|if
condition|(
name|OUTPUT_FLAVOR
operator|==
