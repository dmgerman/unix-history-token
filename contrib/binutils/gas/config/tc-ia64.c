...
parameter_list|)
define|\
value|{									   \     char *_prefix = special_section_name[special];			   \     size_t _prefix_len = strlen (_prefix), _text_len = strlen (text_name); \     char *_result = alloca (_prefix_len + _text_len + 1);		   \     memcpy(_result, _prefix, _prefix_len);				   \     memcpy(_result + _prefix_len, text_name, _text_len);		   \     _result[_prefix_len + _text_len] = '\0';				   \     result = _result;							   \   }									   \ while (0)
end_define
...
literal|0
expr_stmt|;
comment|/* Add 8 for the header + 8 more bytes for the personality offset.  */
name|mem
operator|=
name|xmalloc
argument_list|(
...
name|as_bad
argument_list|(
literal|"Unwind record is not a multiple of 8 bytes."
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Make sure the section has 8 byte alignment.  */
name|frag_align
argument_list|(
literal|3
argument_list|,
...
begin_comment
comment|/* Return true if idesc is a conditional branch instruction.  */
end_comment
...
decl_stmt|;
block|{
comment|/* br is a conditional branch.  Everything that starts with br. except      br.ia is a conditional branch.  Everything that starts with brl is a      conditional branch.  */
return|return
operator|(
name|idesc
operator|->
