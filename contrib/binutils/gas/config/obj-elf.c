...
argument_list|)
expr_stmt|;
comment|/* If this section already existed, we don't bother to change the      flag values.  */
if|if
condition|(
operator|!
name|new_sec
...
condition|)
block|{
comment|/* Pick up a string with a combination of a, w, x.  */
operator|++
name|input_line_pointer
expr_stmt|;
while|while
...
name|bad_msg
init|=
literal|"Bad .section directive: want a,w,x in string"
decl_stmt|;
ifdef|#
directive|ifdef
...
expr_stmt|;
block|}
comment|/* Skip the closing quote.  */
operator|++
name|input_line_pointer
expr_stmt|;
name|SKIP_WHITESPACE
...
begin_comment
comment|/* This handle the .symver pseudo-op, which is used to specify a    symbol version.  The syntax is ``.symver NAME,SYMVERNAME''.    SYMVERNAME may contain ELF_VER_CHR ('@') characters.  This    pseudo-op causes the assembler to emit a symbol named SYMVERNAME    with the same value as the symbol NAME.  */
end_comment
...
name|as_bad
argument_list|(
literal|"multiple .symver directives for symbol `%s'"
argument_list|,
name|S_GET_NAME
argument_list|(
name|sym
...
begin_comment
comment|/* Handle the ELF .type pseudo-op.  This sets the type of a symbol.    There are three syntaxes.  The first (used on Solaris) is        .type SYM,#function    The second (used on UnixWare) is        .type SYM,@function    The third (reportedly to be used on Irix 6.0) is        .type SYM STT_FUNC    */
end_comment
...
name|as_bad
argument_list|(
literal|"ignoring unrecognized symbol type \"%s\""
argument_list|,
name|typename
argument_list|)
expr_stmt|;
...
directive|ifdef
name|TC_MIPS
comment|/* The Irix 5 and 6 assemblers set the type of any common symbol and      any undefined non-function symbol to STT_OBJECT.  We try to be compatible,      since newer Irix 5 and 6 linkers care.  */
if|if
condition|(
name|S_IS_COMMON
argument_list|(
...
directive|ifdef
name|TC_PPC
comment|/* Frob the PowerPC, so that the symbol always has object type      if it is not some other type.  VxWorks needs this.  */
if|if
condition|(
operator|(
name|symp
