...
directive|if
literal|0
comment|/* FIXME: The current gcc, as of 2002-03-03, will emit  	.section .init_array,"aw",@progbits       for __attribute__ ((section (".init_array"))). "@progbits" marks      the incorrect section type. For now, we make them with      SHT_PROGBITS. BFD will fix the section type. Gcc should be changed      to emit  	.section .init_array     */
block|{ ".init_array",SHT_INIT_ARRAY, SHF_ALLOC + SHF_WRITE         },    { ".fini_array",SHT_FINI_ARRAY, SHF_ALLOC + SHF_WRITE         },   { ".preinit_array",SHT_PREINIT_ARRAY, SHF_ALLOC + SHF_WRITE   },
else|#
directive|else
block|{
literal|".init_array"
