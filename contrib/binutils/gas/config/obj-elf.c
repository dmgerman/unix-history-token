...
operator|==
name|NULL
comment|/* FIXME: gcc, as of 2002-10-22, will emit  		 .section .init_array,"aw",@progbits  		 for __attribute__ ((section (".init_array"))). 		 "@progbits" is incorrect.  Also for x86-64 large bss 		 sections, gcc, as of 2005-07-06, will emit  		 .section .lbss,"aw",@progbits  		 "@progbits" is incorrect.  */
ifdef|#
directive|ifdef
name|TC_I386
operator|&&
...
name|TRUE
expr_stmt|;
comment|/* .note.GNU-stack can have SHF_EXECINSTR.  */
elseif|else
if|if
condition|(
name|attr
...
name|_
argument_list|(
literal|"ignoring changed section type for %s"
argument_list|)
argument_list|,
name|name
argument_list|)
...
begin_comment
comment|/* Change to the *ABS* section.  */
end_comment
