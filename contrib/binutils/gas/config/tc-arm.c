...
directive|define
name|ARM_ARCH_V6
value|(ARM_ARCH_V5TEJ | ARM_EXT_V6)
end_define
...
value|(ARM_ARCH_XSCALE | ARM_CEXT_IWMMXT)
end_define
...
begin_comment
comment|/* The shift operand must be an immediate value, not a register.  */
end_comment
...
begin_comment
comment|/* The shift must be LSL or ASR and the operand must be an immediate.  */
end_comment
...
begin_comment
comment|/* The shift must be ASR and the operand must be an immediate.  */
end_comment
...
begin_comment
comment|/* The shift must be LSL and the operand must be an immediate.  */
end_comment
...
name|reg
parameter_list|)
value|((reg ^ WR_PREFIX)>= 0&& (reg ^ WR_PREFIX)<= 15)
end_define
...
name|reg
parameter_list|)
value|((reg ^ WC_PREFIX)>= 0&& (reg ^ WC_PREFIX)<= 15)
end_define
...
name|reg
parameter_list|)
value|((reg ^ WC_PREFIX)>= 8&& (reg ^ WC_PREFIX)<= 11)
end_define

begin_comment
comment|/* These are the standard names.  Users can add aliases with .req.    and delete them with .unreq.  */
end_comment
...
init|=
block|{
comment|/* Intel Wireless MMX technology register names.  */
block|{
literal|"wr0"
block|,
literal|0x0
...
name|N_
argument_list|(
literal|"Intel Wireless MMX technology register expected"
argument_list|)
block|}
block|, }
decl_stmt|;
...
specifier|static
name|int
name|cp_byte_address_required_here
name|PARAMS
argument_list|(
operator|(
name|char
...
begin_endif
unit|static void arm_add_note PARAMS ((const char *, const char *, unsigned int));
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* Check to see if an immediate can be computed as two separate immediate    values, added together.  We already know that this value cannot be    computed by just one ARM instruction.  */
end_comment
...
name|req_entry
decl_stmt|;
comment|/* Check to see if this alias is a builtin one.  */
name|req_entry
operator|=
name|hash_delete
argument_list|(
...
name|_
argument_list|(
literal|"unreq: missing hash entry for \"%s\""
argument_list|)
argument_list|,
name|name
argument_list|)
...
name|builtin
condition|)
comment|/* FIXME: We are deleting a built in register alias which 		   points to a const data structure, so we only need to 		   free up the memory used by the key in the hash table. 		   Unfortunately we have not recorded this value, so this 		   is a memory leak.  */
comment|/* FIXME: Should we issue a warning message ?  */
empty_stmt|;
else|else
block|{
comment|/* Deleting a user defined alias.  We need to free the 		     key and the value, but fortunately the key is the same 		     as the value->name field.  */
name|free
argument_list|(
operator|(
name|char
...
name|_
argument_list|(
literal|".unreq: unrecognized symbol \"%s\""
argument_list|)
argument_list|,
name|name
argument_list|)
...
begin_comment
comment|/* A Intel Wireless MMX technology register    must be given at this point.    Shift is the place to put it in inst.instruction.    Restores input start point on err.    Returns the reg#, or FAIL.  */
end_comment
...
name|_
argument_list|(
literal|"Intel Wireless MMX technology register expected, not '%.100s'"
argument_list|)
argument_list|,
name|start
argument_list|)
...
condition|)
block|{
comment|/* As an extension to the official ARM syntax we allow: 		  		   [Rn] 		    	         as a short hand for:  		   [Rn,#0]  */
name|inst
operator|.
name|instruction
operator||=
...
name|_
argument_list|(
literal|"comma expected after closing square bracket"
argument_list|)
expr_stmt|;
return|return
name|FAIL
...
name|_
argument_list|(
literal|"'option' field too large"
argument_list|)
expr_stmt|;
return|return
name|FAIL
...
name|_
argument_list|(
literal|"non-constant expressions for 'option' field not supported"
argument_list|)
expr_stmt|;
return|return
name|FAIL
...
name|_
argument_list|(
literal|"# or { expected after comma"
argument_list|)
expr_stmt|;
return|return
name|FAIL
...
operator|==
literal|0
comment|/* Lower case versions for backwards compatibility.  */
operator|||
name|strcmp
argument_list|(
name|str
...
literal|4
expr_stmt|;
comment|/* This is for backwards compatibility with older toolchains.  */
elseif|else
if|if
condition|(
name|strcmp
...
begin_comment
comment|/* ARM V5 (argument parse)      LDC2{L}<coproc>,<CRd>,<addressing mode>      STC2{L}<coproc>,<CRd>,<addressing mode>      Instruction is not conditional, and has 0xf in the condition field.      Otherwise, it's the same as LDC/STC.  */
end_comment
...
begin_comment
comment|/* ARM V6 umaal (argument parse). */
end_comment
...
name|_
argument_list|(
literal|"Rd equal to Rm or Rn yields unpredictable results"
argument_list|)
expr_stmt|;
return|return;
block|}
...
begin_comment
comment|/* ARM V6 srs (argument parse). */
end_comment
...
begin_comment
comment|/* ARM V6 SMMUL (argument parse). */
end_comment
...
begin_comment
comment|/* ARM V6 SMLALD (argument parse). */
end_comment
...
begin_comment
comment|/* ARM V6 SMLAD (argument parse).  Signed multiply accumulate dual.     smlad{x}{<cond>} Rd, Rm, Rs, Rn */
end_comment
...
begin_comment
comment|/* Returns true if the endian-specifier indicates big-endianness.  */
end_comment
...
name|_
argument_list|(
literal|"missing endian specifier"
argument_list|)
expr_stmt|;
elseif|else
if|if
...
decl_stmt|;
name|int
name|rotation_clear_mask
init|=
literal|0xfffff3ff
decl_stmt|;
name|int
name|rotation_eight_mask
init|=
literal|0x00000400
decl_stmt|;
name|int
name|rotation_sixteen_mask
init|=
literal|0x00000800
decl_stmt|;
name|int
name|rotation_twenty_four_mask
init|=
literal|0x00000c00
decl_stmt|;
name|skip_whitespace
...
return|return;
block|}
comment|/* Zero out the rotation field. */
name|inst
operator|.
name|instruction
operator|&=
name|rotation_clear_mask
expr_stmt|;
comment|/* Check for lack of optional rotation field. */
if|if
condition|(
name|skip_past_comma
argument_list|(
...
return|return;
block|}
comment|/* Get the immediate constant. */
name|skip_whitespace
argument_list|(
name|str
argument_list|)
...
literal|0
case|:
comment|/* Rotation field has already been zeroed. */
break|break;
case|case
literal|8
case|:
...
name|_
argument_list|(
literal|"rotation can be 8, 16, 24 or 0 when field is ommited"
argument_list|)
expr_stmt|;
break|break;
block|}
...
begin_comment
comment|/* ARM V6 REV (Byte Reverse Word) reverses the byte order in a 32-bit    register (argument parse).    REV{<cond>} Rd, Rm.    Condition defaults to COND_ALWAYS.    Error if Rd or Rm are R15. */
end_comment
...
begin_comment
comment|/* ARM V6 Perform Two Sixteen Bit Integer Additions. (argument parse).    QADD16{<cond>}<Rd>,<Rn>,<Rm>      Condition defaults to COND_ALWAYS.    Error if Rd, Rn or Rm are R15.  */
end_comment
...
return|return;
block|}
comment|/* Check for optional shift immediate constant. */
if|if
condition|(
name|skip_past_comma
argument_list|(
...
condition|)
block|{
comment|/* If the shift specifier is ommited, turn the instruction 	     into pkhbt rd, rm, rn.  First, switch the instruction 	     code, and clear the rn and rm fields.  */
name|inst
operator|.
name|instruction
operator|&=
literal|0xfff0f010
expr_stmt|;
comment|/* Now, re-encode the registers.  */
name|inst
operator|.
name|instruction
operator||=
...
begin_comment
comment|/* ARM V6 Load Register Exclusive instruction (argument parse).    LDREX{<cond>}<Rd, [<Rn>]    Condition defaults to COND_ALWAYS.    Error if Rd or Rn are R15.     See ARMARMv6 A4.1.27: LDREX. */
end_comment
...
begin_comment
comment|/* ARM V6 change processor state instruction (argument parse)       CPS, CPSIE, CSPID . */
end_comment
...
operator|++
expr_stmt|;
comment|/* Strip off the immediate signifier. */
if|if
condition|(
name|my_get_expression
argument_list|(
...
return|return;
block|}
comment|/* The mode is a 5 bit field.  Valid values are 0-31. */
if|if
condition|(
operator|(
operator|(
...
argument_list|)
expr_stmt|;
comment|/* Get the a, f and i flags. */
while|while
condition|(
operator|*
operator|*
...
name|_
argument_list|(
literal|"no 'a', 'i', or 'f' flags for 'cps'"
argument_list|)
expr_stmt|;
block|}
end_function
...
operator|!=
name|O_constant
comment|/* As a convenience we allow 'bkpt' without an operand.  */
operator|&&
name|expr
operator|.
name|X_op
...
begin_comment
comment|/* THUMB CPS instruction (argument parse).  */
end_comment
...
begin_comment
comment|/* THUMB SETEND instruction (argument parse).  */
end_comment
...
begin_comment
comment|/* Parse INSN_TYPE insn STR having a possible IMMEDIATE_SIZE immediate.  */
end_comment
...
name|_
argument_list|(
literal|"non-word size not supported with control register"
argument_list|)
argument_list|)
expr_stmt|;
name|inst
...
name|_
argument_list|(
literal|"conditional execution not supported with control register"
argument_list|)
argument_list|)
expr_stmt|;
if|if
...
begin_comment
comment|/* KIND indicates what kind of shifts are accepted.  */
end_comment
...
begin_comment
comment|/* Do those data_ops which can take a negative immediate constant    by altering the instruction.  A bit of a hack really.         MOV<-> MVN         AND<-> BIC         ADC<-> SBC         by inverting the second operand, and         ADD<-> SUB         CMP<-> CMN         by negating the second operand.  */
end_comment
...
begin_comment
comment|/* cfmv32sc<cond> MVDX[15:0],DSPSC.  */
end_comment
...
name|_
argument_list|(
literal|"failed to create an alias for %s, reason: %s"
argument_list|)
argument_list|,
name|str
argument_list|,
...
begin_comment
unit|typedef struct {   unsigned char	namesz[4];
comment|/* Size of entry's owner string.  */
end_comment

begin_comment
unit|unsigned char	descsz[4];
comment|/* Size of the note descriptor.  */
end_comment

begin_comment
unit|unsigned char	type[4];
...
begin_comment
comment|/* The description is kept to a fix sized in order to make updating    it and merging it easier.  */
end_comment

begin_define
define|#
directive|define
name|ARM_NOTE_DESCRIPTION_LENGTH
value|8
end_define

begin_endif
unit|static void arm_add_note (name, description, type)      const char * name;      const char * description;      unsigned int type; {   arm_Note     note ATTRIBUTE_UNUSED;   char *       p;   unsigned int name_len;    name_len = (strlen (name) + 1 + 3)& ~3;      p = frag_more (sizeof (note.namesz));   md_number_to_chars (p, (valueT) name_len, sizeof (note.namesz));    p = frag_more (sizeof (note.descsz));   md_number_to_chars (p, (valueT) ARM_NOTE_DESCRIPTION_LENGTH, sizeof (note.descsz));    p = frag_more (sizeof (note.type));   md_number_to_chars (p, (valueT) type, sizeof (note.type));    p = frag_more (name_len);   strcpy (p, name);    p = frag_more (ARM_NOTE_DESCRIPTION_LENGTH);   strncpy (p, description, ARM_NOTE_DESCRIPTION_LENGTH);   frag_align (2, 0, 0); }
endif|#
directive|endif
end_endif
...
name|OBJ_COFF
argument_list|)
comment|/* Create a .note section to fully identify this arm binary.  */
define|#
directive|define
...
directive|ifdef
name|TE_WINCE
comment|/* The pattern was adjusted to accommodate CE's off-by-one fixups,      so we un-adjust here to compensate for the accommodation.  */
return|return
name|fixP
operator|->
name|fx_where
...
begin_struct
struct|struct
name|arm_float_abi_option_table
block|{
name|char
modifier|*
name|name
