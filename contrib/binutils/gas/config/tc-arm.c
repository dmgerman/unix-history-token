...
begin_comment
comment|/* Undocumented instruction, its use is discouraged by ARM */
end_comment
...
block|}
block|,
comment|/* ScottB: our code uses 0x0128f000 for msr.    NickC:  but this is wrong because the bits 16 and 19 are handled            by the PSR_xxx defines above.  */
...
begin_comment
comment|/* Three possible forms: "<psr>, Rm", "<psrf>, Rm", "<psrf>, #expression".  */
end_comment
...
name|PSR_ALL
expr_stmt|;
comment|/* Sytax should be "<psr>, Rm" */
if|if
condition|(
name|skip_past_comma
argument_list|(
...
return|return;
block|}
comment|/* Syntax could be "<psrf>, rm", "<psrf>, #expression" */
if|if
condition|(
operator|(
name|reg
...
name|_
argument_list|(
literal|"Error: unrecognised syntax for second argument to msr instruction"
argument_list|)
expr_stmt|;
return|return;
block|}
...
literal|0
decl_stmt|;
comment|/* Pick a reloc ...    *    * @@ Should look at CPU word size.    */
switch|switch
condition|(
name|size
condition|)
