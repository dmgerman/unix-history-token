...
begin_comment
comment|/* Current architecture. Start with the smallest instruction set.  */
end_comment
...
decl_stmt|;
block|{
comment|/* We use nop pattern 0x0707.  */
if|if
condition|(
name|count
operator|>
...
condition|)
block|{
comment|/* Check if parethesed block can be skipped. If the next 		 operand is neiter an optional operand nor a base register 		 then we have a syntax error.  */
name|operand
operator|=
name|s390_operands
operator|+
...
operator|)
expr_stmt|;
comment|/* If there is a next operand it must be seperated by a comma.  */
if|if
condition|(
name|opindex_ptr
index|[
...
name|as_bad
argument_list|(
literal|"Opcode %s not available in this architecture"
argument_list|,
name|str
argument_list|)
expr_stmt|;
...
name|_
argument_list|(
literal|"unresolved fx_subsy symbol that must be resolved"
argument_list|)
argument_list|)
expr_stmt|;
name|value
