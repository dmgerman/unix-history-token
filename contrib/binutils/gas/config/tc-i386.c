...
expr_stmt|;
block|}
comment|/* Change the opcode based on the operand size given by i.suffix;      We need not change things for byte insns.  */
if|if
condition|(
operator|!
name|i
...
name|_
argument_list|(
literal|"segment override on `lea' is ineffectual"
argument_list|)
argument_list|)
expr_stmt|;
...
name|prefix
decl_stmt|;
comment|/* All opcodes on i386 have either 1 or 2 bytes.  Supplemental 	 Streaming SIMD extensions 3 Instructions have 3 bytes.  We may 	 use one more higher byte to specify a prefix the instruction 	 requires.  */
if|if
condition|(
operator|(
name|i
