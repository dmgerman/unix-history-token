...
argument_list|)
block|,
comment|/* The following are only valid for ppc64.  Negative values are        used instead of a flag.  */
name|MAP
argument_list|(
literal|"higher"
argument_list|,
...
begin_comment
comment|/* FIXME: `.machine' is a nop for the moment.  It would be nice to    accept this directive on the first line of input and set ppc_obj64    and the target format accordingly.  Unfortunately, the target    format is selected in output-file.c:output_file_create before we    even get to md_begin, so it's not possible without changing    as.c:main.  */
end_comment
