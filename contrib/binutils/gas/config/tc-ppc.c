...
begin_comment
comment|/* tc-ppc.c -- Assemble for the PowerPC or POWER (RS/6000)    Copyright 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003,    2004, 2005, 2006, 2007 Free Software Foundation, Inc.    Written by Ian Lance Taylor, Cygnus Support.     This file is part of GAS, the GNU Assembler.     GAS is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2, or (at your option)    any later version.     GAS is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with GAS; see the file COPYING.  If not, write to the Free    Software Foundation, 51 Franklin Street - Fifth Floor, Boston, MA    02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* Anything that can start an operand needs to be mentioned here,    to stop the input scrubber eating whitespace.  */
end_comment
...
name|i
decl_stmt|;
comment|/* Check operand masks.  Code here and in the disassembler assumes 	 all the 1's in the mask are contiguous.  */
for|for
control|(
name|i
operator|=
...
name|_
argument_list|(
literal|"powerpc_operands[%d].bitm invalid"
argument_list|)
argument_list|,
name|i
argument_list|)
...
name|_
argument_list|(
literal|"powerpc_operands[%d] duplicates powerpc_operands[%d]"
argument_list|)
argument_list|,
name|j
argument_list|,
...
name|mask
decl_stmt|;
comment|/* The mask had better not trim off opcode bits.  */
if|if
condition|(
operator|(
name|op
...
name|_
argument_list|(
literal|"mask trims opcode bits for %s"
argument_list|)
argument_list|,
name|op
operator|->
...
expr_stmt|;
block|}
comment|/* The operands must not overlap the opcode or each other.  */
for|for
control|(
name|o
operator|=
...
name|_
argument_list|(
literal|"operand index error for %s"
argument_list|)
argument_list|,
name|op
operator|->
...
name|_
argument_list|(
literal|"operand %d overlap in %s"
argument_list|)
argument_list|,
call|(
name|int
...
name|_
argument_list|(
literal|"duplicate instruction %s"
argument_list|)
argument_list|,
name|op
operator|->
...
condition|)
block|{
comment|/* Some people write constants with the sign extension done by 	 hand but only up to 32 bits.  This shouldn't really be valid, 	 but, to permit this code to assemble on a 64-bit host, we 	 sign extend the 32-bit value to 64 bits if so doing makes the 	 value valid.  */
if|if
condition|(
name|val
operator|>
...
literal|0x80000000
expr_stmt|;
comment|/* Similarly, people write expressions like ~(1<<15), and expect 	 this to be OK for a 32-bit unsigned value.  */
elseif|else
if|if
condition|(
name|val
...
begin_comment
comment|/* Implement HANDLE_ALIGN.  This writes the NOP pattern into an    rs_align_code frag.  */
end_comment
...
condition|)
block|{
comment|/* For power6, we want the last nop to be a group terminating 	     one, "ori 1,1,0".  Do this by inserting an rs_fill frag 	     immediately after this one, with its address set to the last 	     nop location.  This will automatically reduce the number of 	     nops in the current frag by one.  */
if|if
condition|(
name|count
operator|>
