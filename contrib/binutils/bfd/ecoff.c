...
directive|if
literal|0
block|const char *name; 	    asection *section; 	    arelent_chain *reloc_chain; 	    unsigned int bitsize;
...
parameter_list|)
define|\
value|if (symhdr->count == 0)						\     debug->ptr = NULL;							\   else									\     {									\       debug->ptr = (type) bfd_malloc ((size_t) (size * symhdr->count));	\       if (debug->ptr == NULL)						\ 	{								\           ret = false;							\           goto return_something;					\ 	}								\       if ((bfd_seek (input_bfd, (file_ptr) symhdr->offset, SEEK_SET)	\ 	   != 0)							\ 	  || (bfd_read (debug->ptr, size, symhdr->count,		\ 			input_bfd) != size * symhdr->count))		\ 	{								\           ret = false;							\           goto return_something;					\ 	}								\     }
...
literal|0
expr_stmt|;
comment|/* Move the information into a internal_reloc structure.  */
name|in
operator|.
name|r_vaddr
operator|=
