...
init|=
block|{
comment|/* name,      id,  index, next, prev, flags, user_set_vma,       */
literal|"*DEBUG*"
block|,
literal|0
block|,
...
literal|0
block|,
comment|/* linker_mark, linker_has_input, gc_mark, gc_mark_from_eh,      */
literal|0
block|,
literal|0
block|,
...
literal|0
block|,
comment|/* entsize, kept_section, moving_line_filepos,                   */
literal|0
block|,
name|NULL
block|,
...
parameter_list|)
define|\
value|if (internal_symhdr->off1 == 0)			\     debug->off2 = NULL;					\   else							\     debug->off2 = (type) ((char *) raw			\ 			  + (internal_symhdr->off1	\ 			     - raw_base))
name|FIX
argument_list|(
...
name|sym_ptr_ptr
expr_stmt|;
comment|/* If the howto field has not been initialised then skip this reloc. 		 This assumes that an error message has been issued elsewhere.  */
if|if
condition|(
name|reloc
operator|->
...
name|SARMAG
expr_stmt|;
comment|/* Already cleared by bfd_zalloc above.      bfd_ardata (abfd)->cache = NULL;      bfd_ardata (abfd)->archive_head = NULL;      bfd_ardata (abfd)->symdefs = NULL;      bfd_ardata (abfd)->extended_names = NULL;      bfd_ardata (abfd)->extended_names_size = 0;      bfd_ardata (abfd)->tdata = NULL;  */
if|if
condition|(
operator|!
name|_bfd_ecoff_slurp_armap
...
condition|)
block|{
comment|/* We ought to close `first' here, but we can't, because 		 we have no way to remove it from the archive cache. 		 It's almost impossible to figure out when we can 		 release bfd_ardata.  FIXME.  */
name|bfd_set_error
argument_list|(
...
parameter_list|)
define|\
value|if (symhdr->count == 0)						 \     debug->ptr = NULL;							 \   else									 \     {									 \       bfd_size_type amt = (bfd_size_type) size * symhdr->count;		 \       debug->ptr = bfd_malloc (amt);					 \       if (debug->ptr == NULL)						 \ 	{								 \           ret = FALSE;							 \           goto return_something;					 \ 	}								 \       if (bfd_seek (input_bfd, (file_ptr) symhdr->offset, SEEK_SET) != 0 \ 	  || bfd_bread (debug->ptr, amt, input_bfd) != amt)		 \ 	{								 \           ret = FALSE;							 \           goto return_something;					 \ 	}								 \     }
