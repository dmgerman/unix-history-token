...
begin_struct
struct|struct
name|elf_sh_pcrel_relocs_copied
block|{
...
decl_stmt|;
block|{
comment|/* We use got and plt entries for sh, but it would seem that the      existing SH code does no sort of reference counting or whatnot on      its GOT and PLT entries, so it is not possible to garbage collect      them at this time.  */
return|return
name|true
return|;
block|}
