...
begin_comment
comment|/* The toc is a set of bfd_vma fields. We use the fact that valid    addresses are even (i.e. the bit representing "1" is off) to allow    us to encode a little extra information in the field    - Unallocated addresses are initialized to 1.    - Allocated addresses are even numbers.    The first time we actually write a reference to the toc in the bfd,    we want to record that fact in a fixup file (if it is asked for), so    we keep track of whether or not an address has been written by marking    the low order bit with a "1" upon writing.  */
end_comment
...
begin_comment
comment|/* Stuff to handle immediate data when the number of bits in the    data is greater than the number of bits in the immediate field    We need to do (usually) 32 bit arithmetic on 16 bit chunks.  */
end_comment
...
literal|4
expr_stmt|;
comment|/* The size must fit in a 16-bit displacement.  */
if|if
condition|(
name|global_toc_size
operator|>
...
operator|)
expr_stmt|;
comment|/* The size must still fit in a 16-bit displacement.  */
if|if
condition|(
operator|(
name|bfd_vma
...
argument_list|)
expr_stmt|;
comment|/* The tricky part is that this is the address that 		       needs a .reloc entry for it.  */
name|fixit
operator|=
name|TRUE
expr_stmt|;
...
case|:
block|{
comment|/* To solve this, we need to know whether or not the symbol 	       appearing on the call instruction is a glue function or not. 	       A glue function must announce itself via a IMGLUE reloc, and  	       the reloc contains the required toc restore instruction.  */
name|bfd_vma
name|x
decl_stmt|;
specifier|const
...
begin_comment
comment|/* FIXME: There is a possibility that when we read in a reloc from a file,           that there are some bits encoded in the upper portion of the 	  type field. Not yet implemented.  */
end_comment
