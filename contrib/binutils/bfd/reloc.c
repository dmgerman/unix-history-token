...
begin_comment
comment|/* INTERNAL_FUNCTION 	bfd_generic_relax_section  SYNOPSIS 	bfd_boolean bfd_generic_relax_section 	  (bfd *abfd, 	   asection *section, 	   struct bfd_link_info *, 	   bfd_boolean *);  DESCRIPTION 	Provides default handling for relaxing for back ends which 	don't do relaxing -- i.e., does nothing except make sure that the 	final size of the section is set. */
end_comment
...
parameter_list|)
block|{
comment|/* We're not relaxing the section, so just copy the size info if it's      zero.  Someone else, like bfd_merge_sections, might have set it, so      don't overwrite a non-zero value.  */
if|if
condition|(
name|section
operator|->
