...
begin_comment
comment|/* Set plt output section type, htab->tls_get_addr, and call the    generic ELF tls_setup function.  */
end_comment
...
begin_comment
comment|/* Return TRUE iff REL is a branch reloc with a global symbol matching    HASH.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Make two passes through the relocs.  First time check that tls      relocs involved in setting up a tls_get_addr call are indeed      followed by such a call.  If they are not, exclude them from      the optimizations done on the second pass.  */
for|for
control|(
name|pass
operator|=
...
literal|1
expr_stmt|;
comment|/* If not using the newer R_PPC_TLSGD/LD to mark 		 __tls_get_addr calls, we must trust that the call 		 stays with its arg setup insns, ie. that the next 		 reloc is the __tls_get_addr call associated with 		 the current reloc.  Edit both insns.  */
if|if
condition|(
name|input_section
operator|->
