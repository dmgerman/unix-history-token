...
begin_comment
comment|/* IA-64 support for 64-bit ELF    Copyright 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007    Free Software Foundation, Inc.    Contributed by David Mosberger-Tang<davidm@hpl.hp.com>     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
name|r_sym
decl_stmt|;
comment|/* The number of elements in elfNN_ia64_dyn_sym_info array.  */
name|unsigned
name|int
name|count
decl_stmt|;
comment|/* The number of sorted elements in elfNN_ia64_dyn_sym_info array.  */
name|unsigned
name|int
name|sorted_count
decl_stmt|;
comment|/* The size of elfNN_ia64_dyn_sym_info array.  */
name|unsigned
name|int
name|size
decl_stmt|;
comment|/* The array of elfNN_ia64_dyn_sym_info.  */
name|struct
name|elfNN_ia64_dyn_sym_info
modifier|*
name|info
...
name|reloc_howto_type
modifier|*
name|elfNN_ia64_reloc_name_lookup
parameter_list|(
name|bfd
modifier|*
name|abfd
...
parameter_list|)
define|\
value|(((i)& (OPCODE_BITS | X6_BITS)) == (2LL<< OPCODE_SHIFT))
end_define
...
parameter_list|)
define|\
value|(((i)& (OPCODE_BITS | X_BITS | X6_BITS | Y_BITS)) \    == (0x1LL<< X6_SHIFT))
end_define
...
parameter_list|)
define|\
value|(((i)& (OPCODE_BITS | X3_BITS | X6_BITS | Y_BITS)) \    == (0x1LL<< X6_SHIFT))
end_define
...
parameter_list|)
define|\
value|(((i)& (OPCODE_BITS | X3_BITS | X2_BITS | X4_BITS | Y_BITS)) \    == (0x1LL<< X4_SHIFT))
end_define
...
parameter_list|)
define|\
value|(((i)& (OPCODE_BITS | BTYPE_BITS)) == (0x4LL<< OPCODE_SHIFT))
end_define
...
parameter_list|)
define|\
value|(((i)& OPCODE_BITS) == (0x5LL<< OPCODE_SHIFT))
end_define

begin_function
specifier|static
name|bfd_boolean
name|elfNN_ia64_relax_br
parameter_list|(
name|bfd_byte
modifier|*
name|contents
...
argument_list|)
expr_stmt|;
comment|/* Check if we can turn br into brl.  A label is always at the start      of the bundle.  Even if there are predicates on NOPs, we still      perform this optimization.  */
name|template
operator|=
name|t0
operator|&
...
literal|0
case|:
comment|/* Check if slot 1 and slot 2 are NOPs. Possible template is          BBB.  We only need to check nop.b.  */
if|if
condition|(
operator|!
operator|(
...
literal|1
case|:
comment|/* Check if slot 2 is NOP. Possible templates are MBB and BBB. 	 For BBB, slot 0 also has to be nop.b.  */
if|if
condition|(
operator|!
operator|(
...
literal|2
case|:
comment|/* Check if slot 1 is NOP. Possible templates are MIB, MBB, BBB, 	 MMB and MFB. For BBB, slot 0 also has to be nop.b.  */
if|if
condition|(
operator|!
operator|(
...
break|break;
default|default:
comment|/* It should never happen.  */
name|abort
argument_list|()
expr_stmt|;
block|}
comment|/* We can turn br.cond/br.call into brl.cond/brl.call.  */
if|if
condition|(
operator|!
operator|(
...
name|FALSE
return|;
comment|/* Turn br into brl by setting bit 40.  */
name|br_code
operator||=
literal|0x1LL
operator|<<
literal|40
expr_stmt|;
comment|/* Turn the old bundle into a MLX bundle with the same stop-bit      variety.  */
if|if
condition|(
name|t0
operator|&
...
condition|)
block|{
comment|/* For BBB, we need to put nop.m in slot 0.  We keep the original 	 predicate only if slot 0 isn't br.  */
if|if
condition|(
name|br_slot
operator|==
...
block|{
comment|/* Keep the original instruction in slot 0.  */
name|t0
operator|&=
literal|0x1ffffffffffLL
operator|<<
...
name|FALSE
return|;
comment|/* Nothing to do if there are no relocations or there is no need for      the current pass.  */
if|if
condition|(
operator|(
name|sec
...
name|R_IA64_PCREL21F
case|:
comment|/* In pass 1, all br relaxations are done. We can skip it. */
if|if
condition|(
name|link_info
operator|->
...
name|R_IA64_PCREL60B
case|:
comment|/* We can't optimize brl to br in pass 0 since br relaxations 	     will increase the code size. Defer it to pass 1.  */
if|if
condition|(
name|link_info
operator|->
...
name|R_IA64_LDXMOV
case|:
comment|/* We can't relax ldx/mov in pass 0 since br relaxations will 	     increase the code size. Defer it to pass 1.  */
if|if
condition|(
name|link_info
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Make the relocation offset point to slot 1.  */
name|irel
operator|->
name|r_offset
operator|=
...
continue|continue;
block|}
comment|/* We can't put a trampoline in a .init/.fini section. Issue 	     an error.  */
if|if
condition|(
name|strcmp
argument_list|(
...
name|_
argument_list|(
literal|"%B: Can't relax br at 0x%lx in section `%A'. Please use brl or indirect branch."
argument_list|)
argument_list|,
name|sec
operator|->
...
goto|;
block|}
comment|/* If the branch and target are in the same section, you've 	     got one honking big section and we can't help you unless 	     you are branching backwards.  You'll get an error message 	     later.  */
if|if
condition|(
name|tsec
operator|==
...
condition|)
block|{
comment|/* Pass 0 is only needed to relax br.  */
name|sec
operator|->
name|skip_relax_pass_0
operator|=
...
begin_comment
comment|/* Handle an IA-64 specific section when reading an object file.  This    is called when bfd_section_from_shdr finds a section with an unknown    type.  */
end_comment
...
name|SHF_IA_64_SHORT
expr_stmt|;
comment|/* Some HP linkers look for the SHF_IA_64_HP_TLS flag instead of SHF_TLS. */
if|if
condition|(
name|elfNN_ia64_hpux_vec
argument_list|(
...
specifier|static
name|bfd_boolean
name|elfNN_ia64_modify_program_headers
parameter_list|(
name|bfd
modifier|*
name|abfd
...
begin_comment
comment|/* Free the global elfNN_ia64_dyn_sym_info array.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|elfNN_ia64_global_dyn_info_free
parameter_list|(
name|void
modifier|*
modifier|*
...
begin_comment
comment|/* Free the local elfNN_ia64_dyn_sym_info array.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|elfNN_ia64_local_dyn_info_free
parameter_list|(
name|void
modifier|*
modifier|*
...
begin_comment
comment|/* Used to sort elfNN_ia64_dyn_sym_info array.  */
end_comment
...
begin_comment
comment|/* Sort elfNN_ia64_dyn_sym_info array and remove duplicates.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Find the first duplicate.  */
name|prev
operator|=
name|info
index|[
...
condition|)
block|{
comment|/* For duplicates, make sure that GOT_OFFSET is valid.  */
if|if
condition|(
name|got_offset
operator|==
...
expr_stmt|;
block|}
comment|/* We may move a block of elements to here.  */
name|dest
operator|=
name|i
operator|++
...
condition|)
block|{
comment|/* For duplicates, make sure that the kept one has a valid 	     got_offset.  */
name|kept
operator|=
name|dest
operator|-
...
name|got_offset
expr_stmt|;
comment|/* Move a block of elements whose first one is different from 	     the previous.  */
if|if
condition|(
name|curr
operator|==
...
expr_stmt|;
block|}
comment|/* Make sure that the kept one has a valid got_offset.  */
if|if
condition|(
name|got_offset
operator|!=
...
condition|)
break|break;
comment|/* Find the next duplicate.  SRC will be kept.  */
name|prev
operator|=
name|info
index|[
...
condition|)
block|{
comment|/* Make sure that got_offset is valid.  */
if|if
condition|(
name|got_offset
operator|==
...
condition|)
block|{
comment|/* If we only move 1 element, we combine it with the next 		 one.  There must be at least a duplicate.  Find the 		 next different one.  */
for|for
control|(
name|diff
operator|=
...
expr_stmt|;
block|}
comment|/* Makre sure that the last duplicated one has an valid 		 offset.  */
name|BFD_ASSERT
argument_list|(
name|curr
operator|==
...
condition|)
block|{
comment|/* Find the next duplicate.  Track the current valid 		     offset.  */
name|prev
operator|=
name|info
index|[
...
block|{
comment|/* When we get here, either there is no duplicate at all or 	 the only duplicate is the last element.  */
if|if
condition|(
name|dest
operator|<
...
condition|)
block|{
comment|/* If the last element is a duplicate, make sure that the 	     kept one has a valid got_offset.  We also update count.  */
if|if
condition|(
name|got_offset
operator|!=
...
condition|)
block|{
comment|/* When we create the array, we don't check for duplicates,          except in the previously sorted section if one exists, and 	 against the last inserted entry.  This allows insertions to 	 be fast.  */
if|if
condition|(
name|info
condition|)
...
condition|)
block|{
comment|/* Try bsearch first on the sorted section.  */
name|key
operator|.
name|addend
operator|=
...
block|}
block|}
comment|/* Do a quick check for the last inserted entry.  */
name|dyn_i
operator|=
name|info
operator|+
...
condition|)
block|{
comment|/* It is the very first element. We create the array of size 	     1.  */
name|size
operator|=
literal|1
expr_stmt|;
...
condition|)
block|{
comment|/* We double the array size every time when we reach the 	     size limit.  */
name|size
operator|+=
name|size
expr_stmt|;
...
name|has_space
label|:
comment|/* Append the new one to the array.  */
name|dyn_i
operator|=
name|info
operator|+
...
name|addend
expr_stmt|;
comment|/* We increment count only since the new ones are unsorted and 	 may have duplicate.  */
operator|(
operator|*
name|count_p
operator|)
...
block|{
comment|/* It is a lookup without insertion.  Sort array if part of the 	 array isn't sorted.  */
if|if
condition|(
name|count
operator|!=
...
name|reloc_count
expr_stmt|;
comment|/* We scan relocations first to create dynamic relocation arrays.  We      modified get_dyn_sym_info to allow fast insertion and support fast      lookup in the next loop.  */
for|for
control|(
name|rel
operator|=
...
return|;
block|}
comment|/* Now, we only do lookup without insertion, which is very fast      with the modified get_dyn_sym_info.  */
for|for
control|(
name|rel
operator|=
...
condition|)
block|{
comment|/* This is needed here, in case @pltoff is used in a non-shared 	     link.  */
if|if
condition|(
operator|!
name|pltoff
...
name|link
expr_stmt|;
comment|/* ??? Versioned symbols seem to lose NEEDS_PLT.  */
if|if
condition|(
name|elfNN_ia64_dynamic_symbol_p
argument_list|(
...
begin_comment
comment|/* Return the base VMA address which should be subtracted from real addresses    when resolving @tprel() relocation.    Main program TLS (whose template starts at PT_TLS p_vaddr)    is assigned offset round(2 * size of pointer, PT_TLS p_align).  */
end_comment
...
name|gp
decl_stmt|;
comment|/* We assume after gp is set, section size will only decrease. We 	 need to adjust gp for it.  */
name|_bfd_set_gp_value
argument_list|(
name|abfd
argument_list|,
...
expr_stmt|;
block|}
comment|/* We may have introduced duplicated entries. We need 		     to remove them properly.  */
name|count
operator|=
name|sort_dyn_sym_info
argument_list|(
...
name|_
argument_list|(
literal|"%B: non-pic code with imm relocation against dynamic symbol `%s'"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: @gprel relocation against dynamic symbol %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: linking non-pic code in a position independent executable"
argument_list|)
argument_list|,
name|input_bfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: @internal branch to dynamic symbol %s"
argument_list|)
expr_stmt|;
elseif|else
if|if
...
name|_
argument_list|(
literal|"%B: speculation fixup to dynamic symbol %s"
argument_list|)
expr_stmt|;
else|else
name|msg
...
name|_
argument_list|(
literal|"%B: @pcrel relocation against dynamic symbol %s"
argument_list|)
expr_stmt|;
call|(
modifier|*
...
name|R_IA64_SECREL64LSB
case|:
comment|/* Make output-section relative to section where the symbol 	     is defined. PR 475  */
if|if
condition|(
name|sym_sec
condition|)
...
condition|)
block|{
comment|/* Relaxtion is always performed for ELF output. 		       Overflow failures for those relocations mean 		       that the section is too big to relax.  */
call|(
modifier|*
name|_bfd_error_handler
call|)
...
name|_
argument_list|(
literal|"%B: Can't relax br (%s) to `%s' at 0x%lx in section `%A' with size 0x%lx (> 0x1000000)."
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: linking trap-on-NULL-dereference with non-trapping files"
argument_list|)
argument_list|,
name|ibfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: linking big-endian files with little-endian files"
argument_list|)
argument_list|,
name|ibfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: linking 64-bit files with 32-bit files"
argument_list|)
argument_list|,
name|ibfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: linking constant-gp files with non-constant-gp files"
argument_list|)
argument_list|,
name|ibfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: linking auto-pic files with non-auto-pic files"
argument_list|)
argument_list|,
name|ibfd
argument_list|)
...
name|TRUE
return|;
comment|/* Flags for fake group section.  */
name|flags
operator|=
operator|(
name|SEC_LINKER_CREATED
...
operator|)
expr_stmt|;
comment|/* We add a fake section group for each .gnu.linkonce.t.* section,      which isn't in a section group, and its unwind sections.  */
for|for
control|(
name|sec
operator|=
...
argument_list|)
expr_stmt|;
comment|/* We need to create a fake group section for it and its 	     unwind sections.  */
name|group
operator|=
...
name|FALSE
return|;
comment|/* Move the fake group section to the beginning.  */
...
expr_stmt|;
block|}
comment|/* Fake SHT_GROUP section header.  */
name|elf_section_data
argument_list|(
name|group
argument_list|)
...
define|\
value|elfNN_ia64_modify_program_headers
end_define
...
define|#
directive|define
name|bfd_elfNN_bfd_reloc_name_lookup
define|\
value|elfNN_ia64_reloc_name_lookup
end_define
...
value|_bfd_elf_link_hash_fixup_symbol
end_define
...
begin_comment
comment|/* FIXME: PR 290: The Intel C compiler generates SHT_IA_64_UNWIND with    SHF_LINK_ORDER. But it doesn't set the sh_link or sh_info fields.    We don't want to flood users with so many error messages. We turn    off the warning for now. It will be turned on later when the Intel    compiler is fixed.   */
end_comment

begin_define
define|#
directive|define
name|elf_backend_link_order_error_handler
value|NULL
end_define
...
directive|define
name|TARGET_LITTLE_SYM
value|bfd_elfNN_ia64_freebsd_vec
end_define
