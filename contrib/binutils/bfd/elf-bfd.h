...
begin_comment
comment|/* Cached start, size and alignment of PT_TLS segment.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* This function, if defined, sets  up the file positions for non PT_LOAD      segments, especially for segments containing non-allocated sections.  */
name|void
argument_list|(
argument|*set_nonloadable_filepos
argument_list|)
name|PARAMS
argument_list|(
operator|(
...
argument_list|)
expr_stmt|;
comment|/* This function, if defined, returns true if the section is contained      within the segment.  File positions are compared.  */
name|boolean
argument_list|(
argument|*is_contained_by_filepos
argument_list|)
name|PARAMS
argument_list|(
operator|(
...
argument_list|)
expr_stmt|;
comment|/* This function, if defined, returns true if copy_private_bfd_data      should be called.  It provides a way of overriding default      test conditions in _bfd_elf_copy_private_section_data.  */
name|boolean
argument_list|(
argument|*copy_private_bfd_data_p
argument_list|)
name|PARAMS
argument_list|(
operator|(
...
name|irix_compat_t
argument_list|(
argument|*elf_backend_mips_irix_compat
argument_list|)
name|PARAMS
argument_list|(
operator|(
...
literal|1
decl_stmt|;
comment|/* Nonzero when a group is COMDAT.  */
name|unsigned
name|int
name|linkonce_p
range|:
...
name|sec
parameter_list|)
value|(elf_section_data(sec)->linkonce_p)
end_define
...
parameter_list|)
define|\
value|(!bfd_is_abs_section(sec)						\&& bfd_is_abs_section((sec)->output_section)				\&& elf_section_data (sec)->sec_info_type != ELF_INFO_TYPE_MERGE	\&& elf_section_data (sec)->sec_info_type != ELF_INFO_TYPE_JUST_SYMS)
end_define
...
define|\
value|elf_link_record_local_dynamic_symbol
end_define
