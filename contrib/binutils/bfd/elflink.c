...
name|bfd_link_hash_undefined
expr_stmt|;
comment|/* FIXME: What if the new symbol is weak undefined?  */
name|h
operator|->
name|root
operator|.
...
name|_
argument_list|(
literal|"%s: warning: unexpected redefinition of indirect versioned symbol `%s'"
argument_list|)
argument_list|,
...
name|_
argument_list|(
literal|"%s: bad reloc symbol index (0x%lx>= 0x%lx) for offset 0x%lx in section `%s'"
argument_list|)
argument_list|,
name|bfd_archive_filename
argument_list|(
...
name|_
argument_list|(
literal|"%s: relocation size mismatch in %s section %s"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
name|ret
decl_stmt|;
comment|/* ld --just-symbols and dynamic objects don't mix very well. 	 Test for --just-symbols by looking at info set up by 	 _bfd_elf_link_just_syms.  */
if|if
condition|(
operator|(
name|s
...
name|error_return
goto|;
comment|/* If this dynamic lib was specified on the command line with 	 --as-needed in effect, then we don't want to add a DT_NEEDED 	 tag unless the lib is actually used.  Similary for libs brought 	 in by another lib's DT_NEEDED.  */
name|add_needed
operator|=
...
name|_
argument_list|(
literal|"Warning: alignment %u of symbol `%s' in %s is smaller than %u in %s"
argument_list|)
argument_list|,
literal|1
operator|<<
...
name|_
argument_list|(
literal|"Warning: size of symbol `%s' changed from %lu in %s to %lu in %s"
argument_list|)
argument_list|,
name|name
argument_list|,
...
argument_list|)
decl_stmt|;
comment|/* A symbol from a library loaded via DT_NEEDED of some 		 other library is referenced by a regular object. 		 Add a DT_NEEDED entry for it.  */
name|add_needed
operator|=
name|TRUE
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* If we have an undefined symbol reference here then it must have      come from a shared library that is being linked in.  (Undefined      references in regular files have already been handled).  If we      are reporting errors for this situation then do so now.  */
if|if
condition|(
name|h
operator|->
...
name|_
argument_list|(
literal|"%s: %s symbol `%s' in %s is referenced by DSO"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
name|_
argument_list|(
literal|"%s: %s symbol `%s' isn't defined"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
...
expr_stmt|;
block|}
comment|/* Run through the relocs looking for any against symbols 	     from discarded sections and section symbols from 	     removed link-once sections.  Complain about relocs 	     against discarded sections.  Zero relocs against removed 	     link-once sections.  Preserve debug information as much 	     as we can.  */
if|if
condition|(
operator|!
name|elf_section_ignore_discarded_relocs
...
argument_list|)
expr_stmt|;
comment|/* Try to preserve debug information.  */
if|if
condition|(
operator|(
name|o
...
name|_
argument_list|(
literal|"%T: discarded in section `%s' from %s\n"
argument_list|)
argument_list|,
name|h
operator|->
...
condition|)
block|{
comment|/* This is a reloc for a deleted entry or somesuch. 			 Turn it into an R_*_NONE reloc, at the same 			 offset as the last reloc.  elf_eh_frame.c and 			 elf_bfd_discard_info rely on reloc offsets 			 being ordered.  */
name|irela
operator|->
name|r_offset
operator|=
...
block|elfsym.st_value = 0;   elfsym.st_size = 0;   elfsym.st_info = ELF_ST_INFO (STB_LOCAL, STT_FILE);   elfsym.st_other = 0;   elfsym.st_shndx = SHN_ABS;   if (! elf_link_output_sym (&finfo, bfd_get_filename (abfd),&elfsym, bfd_abs_section_ptr, NULL))     goto error_return;
endif|#
directive|endif
...
name|_
argument_list|(
literal|"%s: could not find output section %s"
argument_list|)
argument_list|,
name|bfd_get_filename
argument_list|(
