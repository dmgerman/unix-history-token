...
begin_comment
comment|/* ELF linking support for BFD.    Copyright 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004,    2005, 2006 Free Software Foundation, Inc.     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
name|_
argument_list|(
literal|"%B: undefined versioned symbol name %s"
argument_list|)
argument_list|,
name|sinfo
operator|->
...
condition|)
block|{
comment|/* Symbols from discarded section are undefined, and have 		 default visibility.  */
name|sec
operator|=
name|bfd_und_section_ptr
expr_stmt|;
...
block|}
block|}
comment|/* Remove the symbol reference from the reloc, but 			 don't kill the reloc completely.  This is so that 			 a zero value will be written into the section, 			 which may have non-zero contents put there by the 			 assembler.  Zero in things like an eh_frame fde 			 pc_begin allows stack unwinders to recognize the 			 fde as bogus.  */
name|rel
operator|->
name|r_info
operator|&=
...
name|output_section
decl_stmt|;
comment|/* If we have discarded a section, the output 			     section will be the absolute section.  In 			     case of discarded link-once and discarded 			     SEC_MERGE sections, use the kept section.  */
if|if
condition|(
name|bfd_is_abs_section
argument_list|(
...
name|_
argument_list|(
literal|"warning: creating a DT_TEXTREL in a shared object."
argument_list|)
argument_list|)
expr_stmt|;
break|break;
...
name|group
decl_stmt|;
comment|/* A single member comdat group section may be discarded by a      linkonce section. See below.  */
if|if
condition|(
name|sec
operator|->
...
name|flags
expr_stmt|;
comment|/* Check if it belongs to a section group.  */
name|group
operator|=
name|elf_sec_group
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Return if it isn't a linkonce section nor a member of a group.  A      comdat group section also has SEC_LINK_ONCE set.  */
if|if
condition|(
operator|(
name|flags
...
condition|)
block|{
comment|/* If this is the member of a single member comdat group, check if 	 the group should be discarded.  */
if|if
condition|(
name|elf_next_in_group
argument_list|(
...
control|)
block|{
comment|/* We may have 3 different sections on the list: group section, 	 comdat section and linkonce section. SEC may be a linkonce or 	 group section. We match a group section with a group section, 	 a linkonce section with a linkonce section, and ignore comdat 	 section.  */
if|if
condition|(
operator|(
name|flags
...
condition|)
block|{
comment|/* If this is the member of a single member comdat group and the 	 group hasn't be discarded, we check if it matches a linkonce 	 section. We only record the discarded comdat group. Otherwise 	 the undiscarded group will be discarded incorrectly later since 	 itself has been recorded.  */
for|for
control|(
name|l
operator|=
...
comment|/* There is no direct match. But for linkonce section, we should        check if there is a match with comdat group member. We always        record the linkonce section, discarded or not.  */
for|for
control|(
name|l
operator|=
