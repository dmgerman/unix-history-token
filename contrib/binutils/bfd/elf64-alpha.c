...
specifier|static
name|int
name|elf64_alpha_additional_program_headers
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|void
name|elf64_alpha_strip_section_from_output
name|PARAMS
argument_list|(
operator|(
name|asection
...
specifier|static
name|boolean
name|elf64_alpha_adjust_dynindx
name|PARAMS
argument_list|(
operator|(
expr|struct
...
begin_comment
comment|/* Should we do forced local to this symbol?  */
end_comment

begin_define
define|#
directive|define
name|alpha_elf_force_local_symbol_p
parameter_list|(
name|h
parameter_list|,
name|info
parameter_list|)
define|\
value|((info)->shared							\&& ((h)->dynindx == -1						\        || ((info)->symbolic						\&& ((h)->elf_link_hash_flags& ELF_LINK_HASH_DEF_REGULAR))))
end_define
...
parameter_list|)
define|\
value|(((info)->shared							\&& ((h)->root.type == bfd_link_hash_undefweak			\ 	|| !alpha_elf_force_local_symbol_p ((h), (info))))		\    || (((h)->elf_link_hash_flags					\& (ELF_LINK_HASH_DEF_DYNAMIC | ELF_LINK_HASH_REF_REGULAR))	\        == (ELF_LINK_HASH_DEF_DYNAMIC | ELF_LINK_HASH_REF_REGULAR)))
end_define
...
begin_comment
comment|/* Handle an Alpha specific section when reading an object file.  This    is called when elfcode.h finds a section with an unknown type.    FIXME: We need to handle the SHF_MIPS_GPREL flag, but I'm not sure    how to.  */
end_comment
...
condition|)
block|{
comment|/* We need a PT_ALPHA_REGINFO segment.  */
operator|++
name|ret
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* We need a PT_ALPHA_RTPROC segment.  */
operator|++
name|ret
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* If this is a shared library, we need a RELATIVE reloc.  */
name|sreloc
operator|->
name|_raw_size
operator|+=
...
begin_comment
comment|/* Remove a section from the output BFD.  */
end_comment
...
name|gotobj
decl_stmt|;
comment|/* Don't play if there is no .got for this input file.  */
if|if
condition|(
name|this_got
operator|==
...
name|srel
decl_stmt|;
comment|/* We need it when we have .o files compiled with PIC, but 	 linked statically. */
for|for
control|(
name|relent
operator|=
...
block|}
block|}
comment|/* Shared objects require at least RELATIVE relocs for all REFQUAD      and REFLONG relocations.  */
if|if
condition|(
name|info
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Local got entries need RELATIVE relocs in shared 		       libraries.  */
if|if
condition|(
name|info
operator|->
...
define|\
value|elf64_alpha_additional_program_headers
end_define
