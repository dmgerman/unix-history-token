...
begin_comment
comment|/* Alpha specific support for 64-bit ELF    Copyright 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005,    2006, 2007 Free Software Foundation, Inc.    Contributed by Richard Henderson<rth@tamu.edu>.     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* Instruction data for plt generation and relaxation.  */
end_comment
...
begin_comment
comment|/* Set by ld emulation.  Putting this into the link_info or hash structure    is simply working too hard.  */
end_comment
...
begin_decl_stmt
name|bfd_boolean
name|elf64_alpha_use_secureplt
init|=
name|TRUE
decl_stmt|;
end_decl_stmt
...
define|\
value|(elf64_alpha_use_secureplt ? NEW_PLT_HEADER_SIZE : OLD_PLT_HEADER_SIZE)
end_define
...
name|PLT_ENTRY_SIZE
define|\
value|(elf64_alpha_use_secureplt ? NEW_PLT_ENTRY_SIZE : OLD_PLT_ENTRY_SIZE)
end_define
...
define|#
directive|define
name|ALPHA_ELF_LINK_HASH_LU_JSRDIRECT
value|0x40
define|#
directive|define
name|ALPHA_ELF_LINK_HASH_LU_PLT
value|0x38
define|#
directive|define
...
name|got_offset
decl_stmt|;
comment|/* The .plt offset for this entry.  */
name|int
name|plt_offset
decl_stmt|;
...
name|got_list
decl_stmt|;
comment|/* The most recent relax pass that we've seen.  The GOTs      should be regenerated if this doesn't match.  */
name|int
name|relax_trip
decl_stmt|;
block|}
...
name|reloc_howto_type
modifier|*
name|elf64_alpha_bfd_reloc_name_lookup
parameter_list|(
name|bfd
modifier|*
name|abfd
...
begin_comment
comment|/* Handle an Alpha specific section when reading an object file.  This    is called when bfd_section_from_shdr finds a section with an unknown    type.    FIXME: We need to handle the SHF_ALPHA_GPREL flag, but I'm not sure    how to.  */
end_comment
...
comment|/* ??? It appears that we won't make it into 		     adjust_dynamic_symbol for symbols that remain 		     totally undefined.  Copying this check here means 		     we can create a plt entry for them too.  */
name|h
operator|->
name|root
operator|.
...
name|h
expr_stmt|;
comment|/* Now that we've seen all of the input symbols, finalize our decision      about whether this symbol should get a .plt entry.  Irritatingly, it      is common for folk to leave undefined symbols in shared libraries,      and they still expect lazy binding; accept undefined symbols in lieu      of STT_FUNC.  */
if|if
condition|(
name|alpha_elf_dynamic_symbol_p
argument_list|(
...
name|FALSE
return|;
comment|/* We need one plt entry per got subsection.  Delay allocation of 	 the actual plt entries until size_plt_section, called from 	 size_dynamic_sections or during relaxation.  */
return|return
name|TRUE
return|;
block|}
...
begin_comment
comment|/* Record STO_ALPHA_NOPV and STO_ALPHA_STD_GPLOAD.  */
end_comment

begin_function
specifier|static
name|void
name|elf64_alpha_merge_symbol_attribute
parameter_list|(
name|struct
name|elf_link_hash_entry
modifier|*
...
name|_
argument_list|(
literal|"%B: .got subsegment exceeds 64K (size %d)"
argument_list|)
argument_list|,
name|i
argument_list|,
...
name|TRUE
return|;
comment|/* For each LITERAL got entry still in use, allocate a plt entry.  */
for|for
control|(
name|gotent
operator|=
...
expr_stmt|;
block|}
comment|/* If there weren't any, there's no longer a need for the PLT entry.  */
if|if
condition|(
operator|!
name|saw_one
...
argument_list|)
expr_stmt|;
comment|/* When using the secureplt, we need two words somewhere in the data      segment for the dynamic linker to tell us where to go.  This is the      entire contents of the .got.plt section.  */
if|if
condition|(
name|elf64_alpha_use_secureplt
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If the symbol is a hidden undefined weak, then we never have any      relocations.  Avoid the loop which may want to add RELATIVE relocs      based on info->shared.  */
if|if
condition|(
name|h
operator|->
...
name|link
expr_stmt|;
comment|/* If we're using a plt for this symbol, then all of its relocations      for its got entries go into .rela.plt.  */
if|if
condition|(
name|h
operator|->
...
call|)
argument_list|(
literal|"%B: %A+0x%lx: warning: %s relocation against unexpected insn"
argument_list|,
name|info
operator|->
name|abfd
...
condition|)
block|{
comment|/* Look for nice constant addresses.  This includes the not-uncommon 	 special case of 0 for undefweak symbols.  */
if|if
condition|(
operator|(
name|info
...
call|)
argument_list|(
literal|"%B: %A+0x%lx: warning: LITERAL relocation against unexpected insn"
argument_list|,
name|info
operator|->
name|abfd
...
name|odisp
decl_stmt|;
comment|/* For undefined weak symbols, we're mostly interested in getting 	       rid of the got entry whenever possible, so optimize this to a 	       use of the zero register.  */
if|if
condition|(
name|info
operator|->
...
name|info
decl_stmt|;
comment|/* There's nothing to change, yet.  */
operator|*
name|again
operator|=
name|FALSE
...
name|TRUE
return|;
comment|/* Make sure our GOT and PLT tables are up-to-date.  */
if|if
condition|(
name|alpha_elf_hash_table
argument_list|(
...
name|relax_trip
expr_stmt|;
comment|/* This should never fail after the initial round, since the only 	 error is GOT overflow, and relaxation only shrinks the table.  */
if|if
condition|(
operator|!
name|elf64_alpha_size_got_sections
...
condition|)
block|{
comment|/* For relocs against symbols from removed linkonce sections, 	     or sections discarded by a linker script, we just want the 	     section contents zeroed.  */
...
argument_list|)
expr_stmt|;
comment|/* If this is a tp-relative relocation against sym 0, 	     this is hackery from relax_section.  Force the value to 	     be the tls module base.  */
if|if
condition|(
name|r_symndx
operator|==
...
name|_
argument_list|(
literal|"%B: gp-relative relocation against dynamic symbol %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: pc-relative relocation against dynamic symbol %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: change in gp: BRSGP %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: !samegp reloc against symbol without .prologue: %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: unhandled dynamic relocation against %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: pc-relative relocation against undefined weak symbol %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: dtp-relative relocation against dynamic symbol %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: TLS local exec code cannot be linked into shared objects"
argument_list|)
argument_list|,
name|input_bfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: tp-relative relocation against dynamic symbol %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Fill in the entry in the .got.  */
name|bfd_put_64
argument_list|(
name|output_bfd
argument_list|,
...
expr_stmt|;
block|}
comment|/* Initialize the plt header.  */
if|if
condition|(
name|splt
operator|->
...
define|\
value|elf64_alpha_bfd_reloc_name_lookup
end_define
...
define|\
value|elf64_alpha_merge_symbol_attribute
end_define
