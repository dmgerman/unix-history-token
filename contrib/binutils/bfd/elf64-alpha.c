...
parameter_list|)
define|\
value|(((info)->shared&& !(info)->symbolic&& (h)->dynindx != -1)		\    || (((h)->elf_link_hash_flags					\& (ELF_LINK_HASH_DEF_DYNAMIC | ELF_LINK_HASH_REF_REGULAR))	\        == (ELF_LINK_HASH_DEF_DYNAMIC | ELF_LINK_HASH_REF_REGULAR)))
end_define
...
expr_stmt|;
block|}
comment|/* Only add a .rela.got entry if we're not using a .plt entry.  */
if|if
condition|(
name|h
operator|->
...
block|}
block|}
comment|/* Otherwise, shared objects require RELATIVE relocs for all REFQUAD      and REFLONG relocations.  */
elseif|else
if|if
condition|(
name|info
