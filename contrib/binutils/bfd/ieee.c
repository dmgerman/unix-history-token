...
block|}
block|}
comment|/* As far as I can see there is a bug in the Microtec IEEE output      which I'm using to scan, whereby the comma operator is omitted      sometimes in an expression, giving expressions with too many      terms.  We can tell if that's the case by ensuring that      sp == stack here.  If not, then we've pushed something too far,      so we keep adding.  */
while|while
condition|(
name|sp
operator|!=
...
parameter_list|)
define|\
value|do								\     {								\       ieee->h.input_p = ieee->h.first_byte + offset;		\       ieee->h.last_byte = (ieee->h.first_byte			\ 			   + ieee_part_after (ieee, offset));	\     }								\   while (0)
end_define
...
parameter_list|)
define|\
value|(ieee->h.input_p - ieee->h.first_byte)
end_define

begin_comment
comment|/* Find the first part of the ieee file after HERE.  */
end_comment

begin_function
specifier|static
name|file_ptr
name|ieee_part_after
parameter_list|(
name|ieee_data_type
modifier|*
name|ieee
...
name|me_record
decl_stmt|;
comment|/* File parts can come in any order, except that module end is      guaranteed to be last (and the header first).  */
for|for
control|(
name|part
operator|=
...
name|_
argument_list|(
literal|"%B: unimplemented ATI record %u for symbol %u"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: unexpected ATN type %d in external part"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: unexpected type after ATN"
argument_list|)
argument_list|,
name|abfd
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Ignore the return value here.  It doesn't matter if we don't read      the entire buffer.  We might have a very small ieee file.  */
name|bfd_bread
argument_list|(
operator|(
name|void
...
name|error_return
goto|;
comment|/* Again ignore return value of bfd_bread.  */
name|bfd_bread
argument_list|(
operator|(
name|void
...
name|fail
goto|;
comment|/* Read the first few bytes in to see if it makes sense.  Ignore      bfd_bread return value;  The file might be very small.  */
name|bfd_bread
argument_list|(
operator|(
name|void
...
define|#
directive|define
name|ieee_bfd_is_target_special_symbol
define|\
...
define|#
directive|define
name|ieee_bfd_is_group_section
...
define|#
directive|define
name|ieee_section_already_linked
define|\
...
block|,   }
block|,
comment|/* ieee_close_and_cleanup, ieee_bfd_free_cached_info, ieee_new_section_hook,      ieee_get_section_contents, ieee_get_section_contents_in_window.  */
...
argument_list|)
block|,
comment|/* ieee_slurp_armap, ieee_slurp_extended_name_table,      ieee_construct_extended_name_table, ieee_truncate_arname,      ieee_write_armap, ieee_read_ar_hdr, ieee_openr_next_archived_file,      ieee_get_elt_at_index, ieee_generic_stat_arch_elt,      ieee_update_armap_timestamp.  */
...
argument_list|)
block|,
comment|/* ieee_get_reloc_upper_bound, ieee_canonicalize_reloc,      ieee_bfd_reloc_type_lookup.   */
name|BFD_JUMP_TABLE_RELOCS
argument_list|(
name|ieee
argument_list|)
block|,
comment|/* ieee_set_arch_mach, ieee_set_section_contents.  */
