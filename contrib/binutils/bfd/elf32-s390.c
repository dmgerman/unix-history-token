...
argument_list|)
block|,
comment|/* Empty entry for R_390_64.  */
name|EMPTY_HOWTO
argument_list|(
name|R_390_PC64
argument_list|)
block|,
comment|/* Empty entry for R_390_PC64.  */
name|EMPTY_HOWTO
argument_list|(
name|R_390_GOT64
argument_list|)
block|,
comment|/* Empty entry for R_390_GOT64.  */
name|EMPTY_HOWTO
argument_list|(
name|R_390_PLT64
argument_list|)
block|,
comment|/* Empty entry for R_390_PLT64.  */
name|HOWTO
argument_list|(
name|R_390_GOTENT
argument_list|,
...
argument_list|)
block|,
comment|/* Empty entry for R_390_GOTOFF64.  */
name|HOWTO
argument_list|(
name|R_390_GOTPLT12
argument_list|,
...
argument_list|)
block|,
comment|/* Empty entry for R_390_GOTPLT64.  */
name|HOWTO
argument_list|(
...
argument_list|)
block|,
comment|/* Empty entry for R_390_PLTOFF64.  */
name|HOWTO
argument_list|(
name|R_390_TLS_LOAD
argument_list|,
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_GD64.  */
name|HOWTO
argument_list|(
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_GOTIE64.  */
name|HOWTO
argument_list|(
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_LDM64.  */
name|HOWTO
argument_list|(
name|R_390_TLS_IE32
argument_list|,
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_IE64.  */
name|HOWTO
argument_list|(
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_LE64.  */
name|HOWTO
argument_list|(
...
argument_list|)
block|,
comment|/* Empty entry for R_390_TLS_LDO64.  */
name|HOWTO
argument_list|(
...
begin_comment
comment|/* Handle the large displacement relocs.  */
end_comment
...
name|dyn_relocs
decl_stmt|;
comment|/* Number of GOTPLT references for a function.  */
name|bfd_signed_vma
...
define|#
directive|define
name|elf_s390_hash_entry
parameter_list|(
name|ent
parameter_list|)
define|\
value|((struct elf_s390_link_hash_entry *)(ent))
end_define

begin_struct
struct|struct
name|elf_s390_obj_tdata
block|{
name|struct
name|elf_obj_tdata
name|root
...
parameter_list|)
define|\
value|((struct elf_s390_obj_tdata *) (abfd)->tdata.any)
end_define

begin_define
define|#
directive|define
name|elf_s390_local_got_tls_type
parameter_list|(
name|abfd
parameter_list|)
define|\
value|(elf_s390_tdata (abfd)->local_got_tls_type)
end_define
...
decl_stmt|;
block|{
comment|/* Set the right machine number for an s390 elf32 file.  */
return|return
...
expr_stmt|;
block|}
comment|/* Create got section and local_got_refcounts array if they 	 are needed.  */
name|r_type
operator|=
name|elf_s390_tls_transition
argument_list|(
...
name|R_390_GOTPCDBL
case|:
comment|/* Got is created, nothing to be done.  */
break|break;
case|case
name|R_390_PLT16DBL
case|:
...
begin_comment
comment|/* Make sure we emit a GOT entry if the symbol was supposed to have a PLT    entry but we found we will not create any.  Called when we find we will    not have any PLT for this symbol, by for example    elf_s390_adjust_dynamic_symbol when we're doing a proper dynamic link,    or elf_s390_size_dynamic_sections if no dynamic sections will be    created (we're only linking static objects).  */
end_comment
...
condition|)
return|return;
comment|/* We simply add the number of gotplt references to the number    * of got references for this symbol.  */
name|h
operator|->
name|elf
operator|.
...
expr_stmt|;
block|}
comment|/* If R_390_TLS_{IE32,GOTIE32,GOTIE12,IEENT} symbol is now local to      the binary, we can optimize a bit. IE32 and GOTIE32 get converted      to R_390_TLS_LE32 requiring no TLS entry. For GOTIE12 and IEENT      we can save the dynamic TLS relocation.  */
if|if
condition|(
name|h
operator|->
...
name|GOT_TLS_IE_NLT
condition|)
comment|/* For the GOTIE access without a literal pool entry the offset has 	   to be stored somewhere. The immediate value in the instruction 	   is not bit enough so the value is stored in the got.  */
block|{
name|h
operator|->
name|got
...
name|GOT_ENTRY_SIZE
expr_stmt|;
comment|/* R_390_TLS_GD32 needs 2 consecutive GOT slots.  */
if|if
condition|(
name|tls_type
operator|==
...
name|dynamic_sections_created
expr_stmt|;
comment|/* R_390_TLS_IE32 needs one dynamic relocation, 	 R_390_TLS_GD32 needs one if local symbol and two if global.  */
if|if
condition|(
operator|(
name|tls_type
...
condition|)
block|{
comment|/* Allocate 2 got entries and 1 dynamic reloc for R_390_TLS_LDM32 	 relocs.  */
name|htab
operator|->
name|tls_ldm_got
operator|.
...
begin_comment
comment|/* Complain if TLS instruction relocation is against an invalid    instruction.  */
end_comment
...
name|_
argument_list|(
literal|"%B(%A+0x%lx): invalid instruction for TLS relocation %s"
argument_list|)
argument_list|,
name|input_bfd
argument_list|,
...
name|R_390_PLTOFF32
case|:
comment|/* Relocation is to the entry for this symbol in the 	     procedure linkage table relative to the start of the GOT.  */
comment|/* For local symbols or if we didn't make a PLT entry for 	     this symbol resolve the symbol directly.  */
if|if
condition|(
name|h
operator|==
...
block|}
break|break;
comment|/* Relocations for tls literal pool entries.  */
case|case
name|R_390_TLS_IE32
case|:
if|if
...
condition|)
block|{
comment|/* This relocation gets optimized away by the local exec 		 access optimization.  */
name|BFD_ASSERT
argument_list|(
operator|!
name|unresolved_reloc
...
name|shared
condition|)
comment|/* The literal pool entry this relocation refers to gets ignored 	       by the optimized code of the local exec model. Do nothing 	       and the value will turn out zero.  */
continue|continue;
if|if
condition|(
name|htab
...
condition|)
block|{
comment|/* Linking a shared library with non-fpic code requires 		 a R_390_TLS_TPOFF relocation.  */
name|Elf_Internal_Rela
name|outrel
decl_stmt|;
name|asection
...
expr_stmt|;
break|break;
comment|/* Relocations for tls instructions.  */
case|case
name|R_390_TLS_LOAD
case|:
case|case
...
condition|)
block|{
comment|/* IE->LE transition. Four valid cases: 		     l %rx,0(0,%ry)    -> lr %rx,%ry + bcr 0,0 		     l %rx,0(%ry,0)    -> lr %rx,%ry + bcr 0,0 		     l %rx,0(%ry,%r12) -> lr %rx,%ry + bcr 0,0 		     l %rx,0(%r12,%ry) -> lr %rx,%ry + bcr 0,0 */
name|unsigned
name|int
name|insn
decl_stmt|,
...
literal|0x58000000
condition|)
comment|/* l %rx,0(%ry,0) -> lr %rx,%ry + bcr 0,0  */
name|ry
operator|=
operator|(
name|insn
...
literal|0x58000000
condition|)
comment|/* l %rx,0(0,%ry) -> lr %rx,%ry + bcr 0,0  */
name|ry
operator|=
operator|(
name|insn
...
literal|0x5800c000
condition|)
comment|/* l %rx,0(%ry,%r12) -> lr %rx,%ry + bcr 0,0  */
name|ry
operator|=
operator|(
name|insn
...
literal|0x580c0000
condition|)
comment|/* l %rx,0(%r12,%ry) -> lr %rx,%ry + bcr 0,0  */
name|ry
operator|=
operator|(
name|insn
...
operator|)
condition|)
comment|/* GD->LE transition. 		   bas %r14,0(%rx,%r13) -> bc 0,0  */
name|insn
operator|=
literal|0x47000000
expr_stmt|;
else|else
comment|/* GD->IE transition. 		   bas %r14,0(%rx,%r13) -> l %r2,0(%r2,%r12)  */
name|insn
operator|=
literal|0x5822c000
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* LD->LE transition. 		     bas %r14,0(%rx,%r13) -> bc 0,0  */
name|insn
operator|=
literal|0x47000000
expr_stmt|;
