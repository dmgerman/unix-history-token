...
begin_comment
comment|/* opncls.c -- open and close a BFD.    Copyright 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 2000,    2001, 2002, 2003, 2004, 2005, 2006, 2007    Free Software Foundation, Inc.     Written by Cygnus Support.     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* SECTION 	Opening and closing BFDs  SUBSECTION 	Functions for opening and closing */
end_comment

begin_comment
comment|/* FUNCTION 	bfd_fopen  SYNOPSIS 	bfd *bfd_fopen (const char *filename, const char *target,                         const char *mode, int fd);  DESCRIPTION 	Open the file @var{filename} with the target @var{target}. 	Return a pointer to the created BFD.  If @var{fd} is not -1, 	then<<fdopen>> is used to open the file; otherwise,<<fopen>> 	is used.  @var{mode} is passed directly to<<fopen>> or<<fdopen>>.   	Calls<<bfd_find_target>>, so @var{target} is interpreted as by 	that function.  	The new BFD is marked as cacheable iff @var{fd} is -1.  	If<<NULL>> is returned then an error has occured.   Possible errors 	are<<bfd_error_no_memory>>,<<bfd_error_invalid_target>> or<<system_call>> error. */
end_comment
...
name|filename
expr_stmt|;
comment|/* Figure out whether the user is opening the file for reading,      writing, or both, by looking at the MODE argument.  */
if|if
condition|(
operator|(
name|mode
...
name|TRUE
expr_stmt|;
comment|/* If we opened the file by name, mark it cacheable; we can close it      and reopen it later.  However, if a file descriptor was provided,      then it may have been opened with special flags that make it      unsafe to close and reopen the file.  */
if|if
condition|(
name|fd
operator|==
...
name|iostream
decl_stmt|;
comment|/* Since the VEC's memory is bound to the bfd deleting the bfd will      free it.  */
name|int
name|status
init|=
literal|0
...
name|FALSE
return|;
comment|/* FIXME: cagney/2004-02-15: Need to implement a BFD_IN_MEMORY io      vector.  */
if|if
condition|(
operator|!
operator|(
...
begin_comment
comment|/* INTERNAL_FUNCTION 	bfd_alloc  SYNOPSIS 	void *bfd_alloc (bfd *abfd, bfd_size_type wanted);  DESCRIPTION 	Allocate a block of @var{wanted} bytes of memory attached to<<abfd>> and return a pointer to it. */
end_comment
...
begin_comment
comment|/* INTERNAL_FUNCTION 	bfd_alloc2  SYNOPSIS 	void *bfd_alloc2 (bfd *abfd, bfd_size_type nmemb, bfd_size_type size);  DESCRIPTION 	Allocate a block of @var{nmemb} elements of @var{size} bytes each 	of memory attached to<<abfd>> and return a pointer to it. */
end_comment
...
begin_comment
comment|/* INTERNAL_FUNCTION 	bfd_zalloc  SYNOPSIS 	void *bfd_zalloc (bfd *abfd, bfd_size_type wanted);  DESCRIPTION 	Allocate a block of @var{wanted} bytes of zeroed memory 	attached to<<abfd>> and return a pointer to it. */
end_comment
...
begin_comment
comment|/* INTERNAL_FUNCTION 	bfd_zalloc2  SYNOPSIS 	void *bfd_zalloc2 (bfd *abfd, bfd_size_type nmemb, bfd_size_type size);  DESCRIPTION 	Allocate a block of @var{nmemb} elements of @var{size} bytes each 	of zeroed memory attached to<<abfd>> and return a pointer to it. */
end_comment
