...
begin_comment
comment|/* ELF executable support for BFD.     Copyright 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001,    2002, 2003, 2004, 2005, 2006, 2007 Free Software Foundation, Inc.     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* DT_GNU_HASH hash function.  Do not change this function; you will    cause invalid hash tables to be generated.  */
end_comment
...
name|sh_size
expr_stmt|;
comment|/* Allocate and clear an extra byte at the end, to prevent crashes 	 in case the string table is not terminated.  */
if|if
condition|(
name|shstrtabsize
operator|+
...
name|_
argument_list|(
literal|"%B: invalid string offset %u>= %lu for section `%s'"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|_
argument_list|(
literal|"%B symbol number %lu references "
literal|"nonexistent SHT_SYMTAB_SHNDX section"
argument_list|)
argument_list|,
name|ibfd
argument_list|,
...
operator|==
name|STT_SECTION
comment|/* Check for a bogus st_shndx to avoid crashing.  */
operator|&&
name|isym
operator|->
name|st_shndx
...
name|isym
decl_stmt|;
comment|/* First we need to ensure the symbol table is available.  Make sure      that it is a symbol table section.  */
name|hdr
operator|=
name|elf_elfsections
argument_list|(
...
define|#
directive|define
name|IS_VALID_GROUP_SECTION_HEADER
parameter_list|(
name|shdr
parameter_list|)
define|\
value|(   (shdr)->sh_type == SHT_GROUP		\&& (shdr)->sh_size>= (2 * GRP_ENTRY_SIZE)	\&& (shdr)->sh_entsize == GRP_ENTRY_SIZE	\&& ((shdr)->sh_size % GRP_ENTRY_SIZE) == 0)
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* PR binutils/4110: Handle corrupt group headers.  */
if|if
condition|(
name|shdr
operator|->
...
name|_
argument_list|(
literal|"%B: Corrupt size field in group section header: 0x%lx"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|_
argument_list|(
literal|"%B: invalid SHT_GROUP entry"
argument_list|)
argument_list|,
name|abfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: no group info for section %A"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|s
decl_stmt|;
comment|/* Process SHF_LINK_ORDER.  */
for|for
control|(
name|s
operator|=
...
name|sh_link
decl_stmt|;
comment|/* FIXME: The old Intel compiler and old strip/objcopy may 	     not set the sh_link or sh_info fields.  Hence we could 	     get the situation where elfsec is 0.  */
if|if
condition|(
name|elfsec
operator|==
...
index|]
expr_stmt|;
comment|/* PR 1991, 2008: 		 Some strip/objcopy may leave an incorrect value in 		 sh_link.  We don't want to proceed.  */
name|link
operator|=
name|this_hdr
operator|->
...
name|_
argument_list|(
literal|"%B: sh_link [%d] in section `%A' is incorrect"
argument_list|)
argument_list|,
name|s
operator|->
...
block|}
block|}
comment|/* Process section groups.  */
if|if
condition|(
name|num_group
operator|==
...
name|SHT_REL
condition|)
comment|/* We won't include relocation sections in section groups in 	     output object files. We adjust the group section size here 	     so that relocatable link will work correctly when 	     relocation sections are in section group in input object 	     files.  */
name|shdr
operator|->
name|bfd_section
operator|->
...
block|{
comment|/* There are some unknown sections in the group.  */
call|(
modifier|*
name|_bfd_error_handler
call|)
...
name|_
argument_list|(
literal|"%B: unknown [%d] section `%s' in group [%s]"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
condition|)
block|{
comment|/* The debugging sections appear to be recognized only by name, 	 not any sort of flag.  Their SEC_ALLOC bits are cleared.  */
specifier|static
specifier|const
struct|struct
block|{
...
name|FALSE
return|;
comment|/* Go looking for SHT_SYMTAB_SHNDX too, since if there is one we 	 can't read symbols without that section loaded as well.  It 	 is most likely specified by the next section header.  */
if|if
condition|(
name|elf_elfsections
argument_list|(
...
return|;
block|}
comment|/* If the string table isn't one of the above, then treat it as a 	 regular section.  We need to scan all the headers to be sure, 	 just in case this strtab section appeared before the above.  */
if|if
condition|(
name|elf_onesymtab
argument_list|(
...
condition|)
block|{
comment|/* Prevent endless recursion on broken objects.  */
if|if
condition|(
name|i
operator|==
...
name|_
argument_list|(
literal|"%B: invalid link %lu for reloc section %s (index %u)"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
name|FALSE
return|;
comment|/* If this reloc section does not use the main symbol table we 	   don't treat it as a reloc section.  BFD can't adequately 	   represent such a section, so at least for now, we don't 	   try.  We just present it as a normal section.  We also 	   can't use it as a reloc section if it points to the null 	   section, an invalid section, or another reloc section.  */
if|if
condition|(
name|hdr
operator|->
...
name|SEC_LINK_DUPLICATES_DISCARD
expr_stmt|;
comment|/* We try to keep the same section order as it comes in.  */
name|idx
operator|+=
name|n_elt
expr_stmt|;
...
break|break;
default|default:
comment|/* Possibly an attributes section.  */
if|if
condition|(
name|hdr
operator|->
...
literal|0
condition|)
comment|/* FIXME: How to properly handle allocated section reserved 	       for applications?  */
call|(
modifier|*
name|_bfd_error_handler
call|)
...
name|_
argument_list|(
literal|"%B: don't know how to handle allocated, application "
literal|"specific section `%s' [0x%8x]"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
comment|/* Allow sections reserved for applications.  */
return|return
name|_bfd_elf_make_section_from_shdr
argument_list|(
name|abfd
...
name|SHT_HIPROC
condition|)
comment|/* FIXME: We should handle this section.  */
call|(
modifier|*
name|_bfd_error_handler
call|)
...
name|_
argument_list|(
literal|"%B: don't know how to handle processor specific section "
literal|"`%s' [0x%8x]"
argument_list|)
argument_list|,
name|abfd
...
condition|)
block|{
comment|/* Unrecognised OS-specific sections.  */
if|if
condition|(
operator|(
name|hdr
...
literal|0
condition|)
comment|/* SHF_OS_NONCONFORMING indicates that special knowledge is 	       required to correctly process the section and the file should 	       be rejected with an error message.  */
call|(
modifier|*
name|_bfd_error_handler
call|)
...
name|_
argument_list|(
literal|"%B: don't know how to handle OS specific section "
literal|"`%s' [0x%8x]"
argument_list|)
argument_list|,
name|abfd
...
comment|/* Otherwise it should be processed.  */
return|return
name|_bfd_elf_make_section_from_shdr
argument_list|(
name|abfd
...
name|_
argument_list|(
literal|"%B: don't know how to handle section `%s' [0x%8x]"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
block|}
block|,
comment|/* See struct bfd_elf_special_section declaration for the semantics of      this special case where .prefix_length != strlen (.prefix).  */
block|{
literal|".stabstr"
block|,
literal|5
...
return|;
default|default:
comment|/* Check for any processor-specific program segment types.  */
name|bed
operator|=
name|get_elf_backend_data
argument_list|(
...
return|return;
block|}
comment|/* Don't clear sh_flags. Assembler may set additional bits.  */
if|if
condition|(
operator|(
name|asect
...
condition|)
block|{
comment|/* Don't change the header type from NOBITS if we are being 	 called for objcopy --only-keep-debug.  */
name|this_hdr
operator|->
name|sh_type
operator|=
...
name|gas
decl_stmt|;
comment|/* Ignore linker created group section.  See elfNN_ia64_object_p in      elfxx-ia64.c.  */
if|if
condition|(
operator|(
operator|(
...
argument_list|)
expr_stmt|;
comment|/* SHT_GROUP sections are in relocatable files only.  */
if|if
condition|(
name|link_info
operator|==
...
condition|)
block|{
comment|/* Put SHT_GROUP sections first.  */
for|for
control|(
name|sec
operator|=
...
condition|)
block|{
comment|/* Remove the linker created SHT_GROUP sections.  */
...
expr_stmt|;
block|}
comment|/* We need to set up sh_link for SHF_LINK_ORDER.  */
if|if
condition|(
operator|(
name|d
...
condition|)
block|{
comment|/* elf_linked_to_section points to the input section.  */
if|if
condition|(
name|link_info
operator|!=
...
condition|)
block|{
comment|/* Check discarded linkonce section.  */
if|if
condition|(
...
name|_
argument_list|(
literal|"%B: sh_link of section `%A' points to discarded section `%A' of `%B'"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Point to the kept section if it has the same 			 size as the discarded one.  */
name|kept
operator|=
...
name|_
argument_list|(
literal|"%B: sh_link of section `%A' points to removed section `%A' of `%B'"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
block|{
comment|/* PR 290: 		 The Intel C compiler generates SHT_IA_64_UNWIND with 		 SHF_LINK_ORDER.  But it doesn't set the sh_link or 		 sh_info fields.  Hence we could get the situation 		 where s is NULL.  */
specifier|const
name|struct
name|elf_backend_data
modifier|*
...
name|SHT_GNU_LIBLIST
case|:
comment|/* sh_link is the section header index of the prelink library 	     list used for the dynamic entries, or the symbol table, or 	     the version strings.  */
name|s
operator|=
name|bfd_get_section_by_name
argument_list|(
...
begin_comment
comment|/* Don't output section symbols for sections that are not going to be    output.  Also, don't output section symbols for reloc and other    special sections.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|ignore_section_sym
parameter_list|(
name|bfd
modifier|*
name|abfd
...
expr_stmt|;
block|}
comment|/* We will be adding a section symbol for each normal BFD section.  Most      sections will already have a section symbol in outsymbols, but      eg. SHT_GROUP sections will not, and we need the section symbol mapped      at least in that case.  */
for|for
control|(
name|asect
operator|=
...
begin_comment
comment|/* Make an initial estimate of the size of the program header.  If we    get the number wrong here, we'll redo section placement.  */
end_comment
...
condition|)
block|{
comment|/* We need a PT_GNU_RELRO segment only when there is a 	     PT_DYNAMIC segment.  */
operator|++
name|segs
expr_stmt|;
block|}
...
begin_comment
comment|/* Create the PT_DYNAMIC segment, which includes DYNSEC.  Returns NULL    on failure.  */
end_comment
...
begin_comment
comment|/* Possibly add or remove segments from the segment map.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|elf_modify_segment_map
parameter_list|(
name|bfd
modifier|*
name|abfd
...
name|bed
decl_stmt|;
comment|/* The placement algorithm assumes that non allocated sections are      not in PT_LOAD segments.  We ensure this here by removing such      sections from the segment map.  We also remove excluded      sections.  Finally, any PT_LOAD segment without sections is      removed.  */
name|m
operator|=
operator|&
name|elf_tdata
...
expr_stmt|;
block|}
comment|/* Allow interested parties a chance to override our decision.  */
if|if
condition|(
name|last_hdr
operator|&&
...
condition|)
block|{
comment|/* We make a PT_GNU_RELRO segment only when there is a 	     PT_DYNAMIC segment.  */
name|amt
operator|=
sizeof|sizeof
argument_list|(
...
begin_comment
comment|/* Assign file positions to the sections based on the mapping from    sections to segments.  This function also sets up some fields in    the file header.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|assign_file_positions_for_load_sections
parameter_list|(
name|bfd
modifier|*
name|abfd
...
argument_list|)
expr_stmt|;
comment|/* An ELF segment (described by Elf_Internal_Phdr) may contain a 	 number of sections with contents contributing to both p_filesz 	 and p_memsz, followed by a number of sections with no contents 	 that just contribute to p_memsz.  In this loop, OFF tracks next 	 available file offset for PT_LOAD and PT_NOTE segments.  */
name|p
operator|->
name|p_type
operator|=
...
condition|)
block|{
comment|/* p_align in demand paged PT_LOAD segments effectively stores 	     the maximum page size.  When copying an executable with 	     objcopy, we set m->p_align from the input file.  Use this 	     value for maxpagesize rather than bed->maxpagesize, which 	     may be different.  Note that we use maxpagesize for PT_TLS 	     segment alignment later in this function, so we are relying 	     on at least one PT_LOAD segment appearing before a PT_TLS 	     segment.  */
if|if
condition|(
name|m
operator|->
...
literal|0
condition|)
comment|/* If we aren't making room for this section, then 		 it must be SHT_NOBITS regardless of what we've 		 set via struct bfd_elf_special_section.  */
name|elf_section_type
argument_list|(
name|m
operator|->
...
name|SHT_NOBITS
expr_stmt|;
comment|/* Find out whether this segment contains any loadable 	     sections.  If the first section isn't loadable, the same 	     holds for any other sections.  */
name|i
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* If a segment starts with .tbss, we need to look 		 at the next section to decide whether the segment 		 has any loadable sections.  */
if|if
condition|(
operator|(
name|elf_section_flags
...
condition|)
block|{
comment|/* We shouldn't need to align the segment on disk since 		 the segment doesn't need file space, but the gABI 		 arguably requires the alignment and glibc ld.so 		 checks it.  So to comply with the alignment 		 requirement but not waste file space, we adjust 		 p_offset for just this segment.  (OFF_ADJUST is 		 subtracted from OFF later.)  This may put p_offset 		 past the end of file, but that shouldn't matter.  */
block|}
else|else
name|off_adjust
operator|=
...
expr_stmt|;
block|}
comment|/* Make sure the .dynamic section is the first section in the 	 PT_DYNAMIC segment.  */
elseif|else
if|if
condition|(
name|p
...
name|_
argument_list|(
literal|"%B: The first section in the PT_DYNAMIC segment is not the .dynamic section"
argument_list|)
argument_list|,
name|abfd
argument_list|)
...
name|_
argument_list|(
literal|"%B: Not enough room for program headers, try linking with -N"
argument_list|)
argument_list|,
name|abfd
argument_list|)
...
block|}
block|}
comment|/* Set up p_filesz, p_memsz, p_align and p_flags from the section 	 maps.  Set filepos for sections in PT_LOAD segments, and in 	 core files, for sections in PT_NOTE segments. 	 assign_file_positions_for_non_load_sections will set filepos 	 for other sections and update p_filesz for other segments.  */
for|for
control|(
name|i
operator|=
...
name|_
argument_list|(
literal|"%B: section %A lma 0x%lx overlaps previous sections"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
condition|)
block|{
comment|/* The section at i == 0 is the one that actually contains 		 everything.  */
if|if
condition|(
name|i
operator|==
...
block|{
comment|/* The rest are fake sections that shouldn't be written.  */
name|sec
operator|->
name|filepos
operator|=
...
name|sh_size
expr_stmt|;
comment|/* A load section without SHF_ALLOC is something like 		     a note section in a PT_NOTE segment.  These take 		     file space but are not loaded into memory.  */
if|if
condition|(
operator|(
name|this_hdr
...
name|sh_size
expr_stmt|;
comment|/* .tbss is special.  It doesn't contribute to p_memsz of 		     normal segments.  */
elseif|else
if|if
condition|(
operator|(
...
name|off_adjust
expr_stmt|;
comment|/* Check that all sections are in a PT_LOAD segment. 	 Don't check funky gdb generated core files.  */
if|if
condition|(
name|p
operator|->
...
name|_
argument_list|(
literal|"%B: section `%A' can't be allocated in segment %d"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
specifier|static
name|bfd_boolean
name|assign_file_positions_for_non_load_sections
parameter_list|(
name|bfd
modifier|*
name|abfd
...
name|_
argument_list|(
literal|"%B: warning: allocated section `%s' not in segment"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
operator|)
expr_stmt|;
comment|/* We don't need to page align empty sections.  */
if|if
condition|(
operator|(
name|abfd
...
name|FALSE
return|;
comment|/* And for non-load sections.  */
if|if
condition|(
operator|!
name|assign_file_positions_for_non_load_sections
...
name|_
argument_list|(
literal|"%B: symbol `%s' required but not present"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
begin_comment
comment|/* Rewrite program header information.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|rewrite_elf_program_header
parameter_list|(
name|bfd
modifier|*
name|ibfd
...
parameter_list|)
define|\
value|(((section->flags& (SEC_HAS_CONTENTS | SEC_THREAD_LOCAL))		\     != SEC_THREAD_LOCAL || segment->p_type == PT_TLS)			\    ? section->size : 0)
...
parameter_list|)
define|\
value|(p->p_type == PT_NOTE							\&& bfd_get_format (ibfd) == bfd_core					\&& s->vma == 0&& s->lma == 0					\&& (bfd_vma) s->filepos>= p->p_offset				\&& ((bfd_vma) s->filepos + s->size					\<= p->p_offset + p->p_filesz))
...
parameter_list|)
define|\
value|(p->p_vaddr == 0							\&& p->p_paddr == 0							\&& p->p_memsz == 0							\&& p->p_filesz> 0							\&& (s->flags& SEC_HAS_CONTENTS) != 0				\&& s->size> 0							\&& (bfd_vma) s->filepos>= p->p_offset				\&& ((bfd_vma) s->filepos + s->size					\<= p->p_offset + p->p_filesz))
comment|/* Decide if the given section should be included in the given segment.      A section will be included if:        1. It is within the address space of the segment -- we use the LMA 	  if that is set for the segment and the VMA otherwise,        2. It is an allocated segment,        3. There is an output section associated with it,        4. The section has not already been allocated to a previous segment.        5. PT_GNU_STACK segments do not include any sections.        6. PT_TLS segment includes only SHF_TLS sections.        7. SHF_TLS sections are only in PT_TLS or PT_LOAD segments.        8. PT_DYNAMIC should not contain empty sections at the beginning 	  (with the possible exception of .dynamic).  */
define|#
directive|define
name|IS_SECTION_IN_INPUT_SEGMENT
parameter_list|(
name|section
parameter_list|,
name|segment
...
comment|/* If the output section of a section in the input segment is NULL,    it is removed from the corresponding output segment.   */
define|#
directive|define
...
parameter_list|)
define|\
value|(IS_SECTION_IN_INPUT_SEGMENT (section, segment, bed)		\&& section->output_section != NULL)
...
control|)
block|{
comment|/* Find the first section in the input segment, which may be 	     removed from the corresponding output segment.   */
if|if
condition|(
name|IS_SECTION_IN_INPUT_SEGMENT
argument_list|(
...
literal|1
expr_stmt|;
comment|/* If the first section in the input segment is removed, there is 	 no need to preserve segment physical address in the corresponding 	 output segment.  */
if|if
condition|(
operator|!
name|first_section
...
name|_
argument_list|(
literal|"%B: warning: Empty loadable segment detected, is this intentional ?\n"
argument_list|)
argument_list|,
name|ibfd
argument_list|)
...
name|includes_phdrs
condition|)
comment|/* There is some padding before the first section in the 	       segment.  So, we must account for that in the output 	       segment's vma.  */
name|map
operator|->
name|p_vaddr_offset
operator|=
...
begin_comment
comment|/* Copy ELF program header information.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|copy_elf_program_header
parameter_list|(
name|bfd
modifier|*
name|ibfd
...
name|NULL
decl_stmt|;
comment|/* FIXME: Do we need to copy PT_NULL segment?  */
if|if
condition|(
name|segment
operator|->
...
condition|)
continue|continue;
comment|/* Compute how many sections are in this segment.  */
for|for
control|(
name|section
operator|=
...
name|FALSE
return|;
comment|/* Initialize the fields of the output segment map with the 	 input segment.  */
name|map
operator|->
name|next
operator|=
...
name|includes_filehdr
condition|)
comment|/* There is some other padding before the first section.  */
name|map
operator|->
name|p_vaddr_offset
operator|=
...
begin_comment
comment|/* Copy private BFD data.  This copies or rewrites ELF program header    information.  */
end_comment
...
condition|)
block|{
comment|/* Check to see if any sections in the input BFD 	 covered by ELF program header have changed.  */
name|Elf_Internal_Phdr
modifier|*
name|segment
decl_stmt|;
...
name|this_hdr
decl_stmt|;
comment|/* Initialize the segment mark field.  */
for|for
control|(
name|section
operator|=
...
control|)
block|{
comment|/* PR binutils/3535.  The Solaris linker always sets the p_paddr 	     and p_memsz fields of special segments (DYNAMIC, INTERP) to 0 	     which severly confuses things, so always regenerate the segment 	     map in this case.  */
if|if
condition|(
name|segment
operator|->
...
control|)
block|{
comment|/* We mark the output section so that we know it comes 		 from the input BFD.  */
name|osec
operator|=
name|section
operator|->
...
name|TRUE
expr_stmt|;
comment|/* Check if this section is covered by the segment.  */
name|this_hdr
operator|=
operator|&
operator|(
...
condition|)
block|{
comment|/* FIXME: Check if its output section is changed or 		     removed.  What else do we need to check?  */
if|if
condition|(
name|osec
operator|==
...
block|}
block|}
comment|/* Check to see if any output section do not come from the 	 input BFD.  */
for|for
control|(
name|section
operator|=
...
begin_comment
comment|/* Initialize private output section information from input section.  */
end_comment
...
name|TRUE
return|;
comment|/* Don't copy the output ELF section type from input if the      output BFD section flags have been set to something different.      elf_fake_sections will set ELF section type based on BFD      section flags.  */
if|if
condition|(
name|elf_section_type
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* FIXME: Is this correct for all OS/PROC specific flags?  */
name|elf_section_flags
argument_list|(
name|osec
argument_list|)
...
operator|)
expr_stmt|;
comment|/* Set things up for objcopy and relocatable link.  The output      SHT_GROUP section will have its elf_next_in_group pointing back      to the input group members.  Ignore linker created group section.      See elfNN_ia64_object_p in elfxx-ia64.c.  */
if|if
condition|(
name|need_group
condition|)
...
name|this_hdr
expr_stmt|;
comment|/* We need to handle elf_linked_to_section for SHF_LINK_ORDER. We      don't use the output section of the linked-to section since it      may be NULL at this point.  */
if|if
condition|(
operator|(
name|ihdr
...
begin_comment
comment|/* Copy private header information.  */
end_comment
...
return|;
block|}
comment|/* _bfd_elf_copy_private_section_data copied over the SHF_GROUP flag      but this might be wrong if we deleted the group section.  */
for|for
control|(
name|isec
operator|=
...
begin_comment
comment|/* Return the size required for the dynamic reloc entries.  Any loadable    section that was actually installed in the BFD, and has type SHT_REL    or SHT_RELA, and uses the dynamic symbol table, is considered to be a    dynamic reloc section.  */
end_comment
...
name|bfd_byte
modifier|*
name|contents_end_def
decl_stmt|,
modifier|*
name|contents_end_aux
decl_stmt|;
name|hdr
operator|=
operator|&
...
expr_stmt|;
block|}
comment|/* Create a default version based on the soname.  */
if|if
condition|(
name|default_imported_symver
condition|)
...
name|p
decl_stmt|;
comment|/* ??? Given multiple file symbols, it is impossible to reliably      choose the right file name for global symbols.  File symbols are      local symbols, and thus all file symbols must sort before any      global symbols.  The ELF spec may be interpreted to say that a      file symbol must sort before other local symbols, but currently      ld -r doesn't do this.  So, for ld -r output, it is possible to      make a better choice of file name for local symbols by ignoring      file symbols appearing after a given local symbol.  */
enum|enum
block|{
name|nothing_seen
block|,
...
begin_comment
comment|/* Find the line for a symbol.  */
end_comment
...
begin_comment
comment|/* After a call to bfd_find_nearest_line, successive calls to    bfd_find_inliner_info can be used to get source information about    each level of function inlining that terminated at the address    passed to bfd_find_nearest_line.  Currently this is only supported    for DWARF2 with appropriate DWARF3 extensions. */
end_comment
...
name|_
argument_list|(
literal|"%B: unsupported relocation type %s"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
begin_comment
comment|/* Define HAVE_THRMISC_T for consistency with other similar GNU-type stubs. */
end_comment
...
begin_comment
comment|/* defined (HAVE_THRMISC_T) */
end_comment
...
begin_comment
comment|/* Function: elfcore_write_note     Inputs:      buffer to hold note, and current size of buffer      name of note      type of note      data for note      size of data for note     Writes note to end of buffer.  ELF64 notes are written exactly as    for ELF32, despite the current (as of 2006) ELF gabi specifying    that they ought to have 8-byte namesz and descsz field, and have    8-byte alignment.  Other writers, eg. Linux kernel, do the same.     Return:    Pointer to realloc'd buffer, *BUFSIZ updated.  */
end_comment
...
condition|)
block|{
comment|/* If we have changed the section, and our original section is 	     marked with SEC_EXCLUDE, it means that the original 	     SEC_MERGE section has been completely subsumed in some 	     other SEC_MERGE section.  In this case, we need to leave 	     some info around for --emit-relocs.  */
if|if
condition|(
operator|(
name|sec
...
name|sym_ptr_ptr
expr_stmt|;
comment|/* Undefined syms won't have BSF_LOCAL or BSF_GLOBAL set.  Since 	 we are defining a symbol, ensure one of them is set.  */
if|if
condition|(
operator|(
name|s
...
begin_comment
comment|/* Sort references to symbols by ascending section number.  */
end_comment

begin_function
specifier|static
name|int
name|elf_sort_elf_symbol
parameter_list|(
specifier|const
name|void
modifier|*
...
begin_comment
comment|/* Check if 2 sections define the same set of local and global    symbols.  */
end_comment
...
name|owner
expr_stmt|;
comment|/* If both are .gnu.linkonce sections, they have to have the same      section name.  */
if|if
condition|(
name|CONST_STRNEQ
argument_list|(
...
literal|0
return|;
comment|/* Both sections have to be in ELF.  */
if|if
condition|(
name|bfd_get_flavour
argument_list|(
...
condition|)
block|{
comment|/* If both are members of section groups, they have to have the 	 same group name.  */
if|if
condition|(
name|strcmp
argument_list|(
...
condition|)
block|{
comment|/* Optimized faster version.  */
name|bfd_size_type
name|lo
decl_stmt|,
name|hi
...
operator|++
control|)
comment|/* Two symbols must have the same binding, type and name.  */
if|if
condition|(
name|symtable1
index|[
...
name|done
goto|;
comment|/* Count definitions in the section.  */
name|count1
operator|=
literal|0
expr_stmt|;
...
begin_comment
comment|/* It is only used by x86-64 so far.  */
end_comment
...
begin_comment
comment|/* Return TRUE if 2 section types are compatible.  */
end_comment
...
begin_comment
comment|/* Return TRUE for ELF symbol types that represent functions.    This is the default version of this function, which is sufficient for    most targets.  It returns true if TYPE is STT_FUNC.  */
end_comment
