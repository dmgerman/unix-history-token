...
begin_comment
comment|/* ELF executable support for BFD.    Copyright 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001    Free Software Foundation, Inc.  This file is part of BFD, the Binary File Descriptor library.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Copy data from an indirect symbol to its direct symbol, hiding the    old indirect symbol.  */
end_comment
...
operator|)
expr_stmt|;
comment|/* Copy over the global and procedure linkage table offset entries.      These may have been already set up by a check_relocs routine.  */
if|if
condition|(
name|dir
operator|->
...
argument_list|)
expr_stmt|;
comment|/* The section may have artificial alignment forced by a 	     link script.  Notice this case by the gap between the 	     cumulative phdr vma and the section's vma.  */
if|if
condition|(
name|p
operator|->
...
name|_
argument_list|(
literal|"Error: First section in segment (%s) starts at 0x%x"
argument_list|)
argument_list|,
...
name|_
argument_list|(
literal|"       whereas segment starts at 0x%x"
argument_list|)
argument_list|,
name|p
operator|->
...
comment|/* Decide if the given section should be included in the given segment.      A section will be included if:        1. It is within the address space of the segment,        2. It is an allocated segment,        3. There is an output section associated with it,        4. The section has not already been allocated to a previous segment.  */
define|#
directive|define
...
parameter_list|)
define|\
value|((((IS_CONTAINED_BY_VMA (section, segment) 		\       || IS_SOLARIS_PT_INTERP (segment, section))	\&& (section->flags& SEC_ALLOC) != 0)		\     || IS_COREFILE_NOTE (segment, section))		\&& section->output_section != NULL			\&& section->segment_mark == false)
...
name|false
expr_stmt|;
comment|/* Scan through the segments specified in the program header      of the input BFD.  For this first scan we look for overlaps      in the loadable segments.  These can be created by wierd      parameters to objcopy.  */
for|for
control|(
name|i
operator|=
...
name|map_first
expr_stmt|;
comment|/* If we had to estimate the number of program headers that were      going to be needed, then check our estimate know and adjust      the offset if necessary.  */
if|if
condition|(
name|phdr_adjust_seg
operator|!=
...
condition|)
block|{
comment|/* Section symbols have no name.  */
name|sym
operator|.
name|st_name
operator|=
...
begin_comment
comment|/* Create a pseudosection containing the exact contents of NOTE.  This    actually creates up to two pseudosections:    - For the single-threaded case, a section named NAME, unless      such a section already exists.    - For the multi-threaded case, a section named "NAME/PID", where      PID is elfcore_make_pid (abfd).    Both pseudosections have identical contents: the contents of NOTE.  */
end_comment
...
begin_comment
comment|/* FIXME: This function is now unnecessary.  Callers can just call    bfd_section_from_phdr directly.  */
end_comment
