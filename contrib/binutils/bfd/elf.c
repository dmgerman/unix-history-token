...
literal|0
condition|)
comment|/* the actual "note" segment */
block|{
comment|/* this one actually contains everything. */
name|sec
operator|->
name|filepos
operator|=
...
parameter_list|)
define|\
value|((addr)>= (bottom)				 	  	\&& (   ((addr) + (len))<= ((bottom) + (phdr)->p_memsz)	\ 	       || ((addr) + (len))<= ((bottom) + (phdr)->p_filesz)))
...
comment|/* Scan through the segments specified in the program header      of the input BFD.  */
for|for
control|(
name|i
operator|=
...
name|j
decl_stmt|;
comment|/* For each section in the input BFD, decide if it should be 	 included in the current segment.  A section will be included 	 if it is within the address space of the segment, and it is 	 an allocated segment, and there is an output section 	 associated with it.  */
name|csecs
operator|=
literal|0
expr_stmt|;
...
name|lma
expr_stmt|;
comment|/* We assume that if the section fits within the segment 		     that it does not overlap any other section within that 		     segment.  */
name|m
operator|->
name|sections
index|[
...
block|if (mfirst != NULL)     {       struct elf_segment_map* prev;        prev = mfirst;       for (m = mfirst->next; m != NULL; prev = m, m = m->next) 	{
comment|/* Yes I know - its a bubble sort....*/
block|if (m->next != NULL&& (m->next->p_paddr< m->p_paddr)) 	    {
comment|/* swap m and m->next */
block|prev->next = m->next; 	      m->next = m->next->next; 	      prev->next->next = m;
...
begin_comment
comment|/* prstatus_t exists on:      solaris 2.[567]      linux 2.[01] + glibc      unixware 4.2 */
end_comment
...
name|pr_pid
expr_stmt|;
comment|/* pr_who exists on:        solaris 2.[567]        unixware 4.2      pr_who doesn't exist on:        linux 2.[01]   */
if|#
directive|if
name|defined
argument_list|(
