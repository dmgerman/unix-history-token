...
begin_comment
comment|/* ELF executable support for BFD.     Copyright 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001,    2002, 2003, 2004, 2005, 2006 Free Software Foundation, Inc.     This file is part of BFD, the Binary File Descriptor library.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
expr_stmt|;
comment|/* When we read a file, we don't need section type and flags unless      it is a linker created section.  They will be overridden in      _bfd_elf_make_section_from_shdr anyway.  */
if|if
condition|(
name|abfd
operator|->
...
name|_
argument_list|(
literal|" Section to Segment mapping:\n"
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
...
block|{
comment|/* The placement algorithm assumes that non allocated sections are 	 not in PT_LOAD segments.  We ensure this here by removing such 	 sections from the segment map.  We also remove excluded 	 sections.  */
for|for
control|(
name|m
operator|=
...
name|_
argument_list|(
literal|"%B: Not enough room for program headers (allocated %u, need %u)"
argument_list|)
argument_list|,
name|abfd
argument_list|,
...
condition|)
block|{
comment|/* If the first section isn't loadable, the same holds for 		 any other sections.  Since the segment won't need file 		 space, we can make p_offset overlap some prior segment. 		 However, .tbss is special.  If a segment starts with 		 .tbss, we need to look at the next section to decide 		 whether the segment has any loadable sections.  */
name|i
operator|=
literal|0
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* PR ld/594:  Sections in note segments which are not loaded 		 contribute to the file size but not the in-memory size.  */
elseif|else
if|if
condition|(
name|p
...
argument_list|)
expr_stmt|;
comment|/* If the section has not yet been assigned a file position, 	     do so now.  The ARM BPABI requires that .dynamic section 	     not be marked SEC_ALLOC because it is not part of any 	     PT_LOAD segment, so it will not be processed above.  */
if|if
condition|(
name|p
operator|->
...
condition|)
block|{
comment|/* We need a PT_GNU_RELRO segment.  */
operator|++
name|segs
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* Check if any sections in the input BFD covered by ELF program 	 header are changed.  */
name|Elf_Internal_Phdr
modifier|*
name|segment
decl_stmt|;
...
block|}
block|}
comment|/* Check to see if any output section doesn't come from the 	 input BFD.  */
for|for
control|(
name|section
operator|=
...
name|TRUE
return|;
comment|/* FIXME: What if the output ELF section type has been set to      something different?  */
if|if
condition|(
name|elf_section_type
argument_list|(
...
begin_comment
comment|/* Sort symbol by binding and section. We want to put definitions    sorted by section at the beginning.  */
end_comment
...
name|shndx
decl_stmt|;
comment|/* Make sure that undefined symbols are at the end.  */
name|s1
operator|=
operator|(
specifier|const
...
literal|1
return|;
comment|/* Sorted by section index.  */
name|shndx
operator|=
name|s1
operator|->
...
name|done
goto|;
comment|/* Sort symbols by binding and section. Global definitions are at      the beginning.  */
name|qsort
argument_list|(
name|isymbuf1
argument_list|,
