...
block|if (bfd_get_mach (obfd) == bfd_mach_sparc&& (bfd_get_mach (ibfd) == bfd_mach_sparc_v8plus 	  || bfd_get_mach (ibfd) == bfd_mach_sparc_v8plusa))     {       error = true;       (*_bfd_error_handler) 	(_("%s: compiled for a v8plus system and target is v8"), 	 bfd_get_filename (ibfd));     }
comment|/* If the output machine is v9, we can't allow v9+vis input files.  */
block|if (bfd_get_mach (obfd) == bfd_mach_sparc_v8plus&& bfd_get_mach (ibfd) == bfd_mach_sparc_v8plusa)     {       error = true;       (*_bfd_error_handler) 	(_("%s: compiled for a v8plusa system and target is v8plus"), 	 bfd_get_filename (ibfd));     }
else|#
directive|else
if|if
condition|(
