...
name|strs
decl_stmt|;
comment|/* An malloc buffer to hold the file name.  */
name|char
modifier|*
name|filename
decl_stmt|;
...
name|STABSIZE
value|(12)
comment|/* It would be nice if we could skip ahead to the stabs symbols for      the next compilation unit to quickly scan through the compilation      units.  Unfortunately, since each line number gets a separate      stabs entry, it is entirely plausible that a large source file      will overflow the 16 bit count of stabs entries.  */
name|fnaddr
operator|=
literal|0
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* An empty string indicates the end of the compilation              unit.  */
if|if
condition|(
operator|*
name|name
...
condition|)
block|{
comment|/* If there are functions in different sections, they                  may have addresses larger than val, but we don't want                  to forget the file name.  When there are functions in                  different cases, there is supposed to be an N_FUN at                  the end of the function indicating where it ends.  */
if|if
condition|(
name|low_func_vma
operator|<
...
break|break;
block|}
comment|/* We know that we have to get to at least this point in the              stabs entries for this offset.  */
name|info
operator|->
name|cached_stab
operator|=
...
name|name
expr_stmt|;
comment|/* Look ahead to the next symbol.  Two consecutive N_SO              symbols are a directory and a file name.  */
if|if
condition|(
name|stab
operator|+
...
argument_list|)
expr_stmt|;
comment|/* An empty string here indicates the end of a function, and 	     the value is relative to fnaddr.  */
if|if
condition|(
operator|*
name|name
