...
begin_comment
comment|/* FUNCTION 	bfd_set_archive_head  SYNOPSIS 	bfd_boolean bfd_set_archive_head (bfd *output, bfd *new_head);  DESCRIPTION 	Set the head of the chain of 	BFDs contained in the archive @var{output} to @var{new_head}. */
end_comment
...
name|cache
decl_stmt|;
comment|/* If the hash table hasn't been created, create it.  */
if|if
condition|(
name|hash_table
operator|==
...
expr_stmt|;
block|}
comment|/* Insert new_elt into the hash table by filepos.  */
name|cache
operator|=
name|bfd_zalloc
argument_list|(
...
name|SARMAG
expr_stmt|;
comment|/* Cleared by bfd_zalloc above.      bfd_ardata (abfd)->cache = NULL;      bfd_ardata (abfd)->archive_head = NULL;      bfd_ardata (abfd)->symdefs = NULL;      bfd_ardata (abfd)->extended_names = NULL;      bfd_ardata (abfd)->extended_names_size = 0;      bfd_ardata (abfd)->tdata = NULL;  */
if|if
condition|(
operator|!
name|BFD_SEND
...
begin_comment
comment|/* Some constants for a 32 bit BSD archive structure.  We do not    support 64 bit archives presently; so far as I know, none actually    exist.  Supporting them would require changing these constants, and    changing some H_GET_32 to H_GET_64.  */
end_comment
...
condition|)
block|{
comment|/* 64bit ELF (Irix 6) archive.  */
ifdef|#
directive|ifdef
name|BFD64
specifier|extern
...
parameter_list|)
block|{
comment|/* The size of the ranlib is the number of exported symbols in the      archive * the number of bytes in an int, + an int for the count.  */
name|unsigned
name|int
name|ranlibsize
init|=
