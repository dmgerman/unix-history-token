...
specifier|static
name|boolean
name|elf_link_renumber_dynsyms
name|PARAMS
argument_list|(
operator|(
expr|struct
...
argument_list|)
expr_stmt|;
comment|/* Pick up the name of the version.  */
name|extverdaux
operator|=
operator|(
operator|(
...
call|)
argument_list|(
literal|"%s: %s: invalid version %d (max %d)"
argument_list|,
name|abfd
operator|->
name|filename
...
name|link
expr_stmt|;
comment|/* FIXME: There are too many cases here, and it's too              confusing.  This code needs to be reorganized somehow.  */
comment|/* It's OK to change the type if it used to be a weak              definition, or if the current definition is weak (and              hence might be ignored).  */
if|if
condition|(
name|h
operator|->
...
condition|)
block|{
comment|/* In the special case of two symbols which look                      like common symbols in a dynamic object, set the                      size of the symbol to the larger of the two.  */
if|if
condition|(
operator|(
name|sec
...
condition|)
block|{
comment|/* Note that we only warn if the size is                          different.  If the size is the same, then we                          simply let the first shared library override                          the second.  */
if|if
condition|(
operator|!
operator|(
...
block|}
block|}
comment|/* If we already have a common symbol, and the symbol in the              shared library is in an uninitialized section, then treat              the shared library symbol as a common symbol.  This will              not always be correct, but it should do little harm.  */
if|if
condition|(
name|dynamic
operator|&&
...
expr_stmt|;
block|}
comment|/* If we are not looking at a dynamic object, and we have a 	     definition, we want to override any definition we may 	     have from a dynamic object.  Symbols from regular files 	     always take precedence over symbols from dynamic objects, 	     even if they are defined after the dynamic object in the 	     link.  */
if|if
condition|(
operator|!
name|dynamic
...
expr_stmt|;
block|}
comment|/* If we are not looking at a shared library and we have a              common symbol, and the symbol in the shared library is in              an uninitialized section, then treat the shared library              symbol as a common symbol.  This will not always be              correct, but it should do little harm.  Note that the              above condition already handled cases in which a common              symbol should simply override the definition in the              shared library.  */
if|if
condition|(
operator|!
name|dynamic
...
name|size
expr_stmt|;
comment|/* FIXME: We no longer know the alignment required by 		 the symbol in the shared library, so we just wind up 		 using the one from the regular object.  */
name|override
operator|=
name|true
expr_stmt|;
...
literal|'\0'
expr_stmt|;
comment|/* First look to see if we have an existing symbol                      with this name.  */
name|hold
operator|=
name|elf_link_hash_lookup
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If we are looking at a normal object, and the                      symbol was seen in a shared object, clobber the                      definition in the shared object.  */
if|if
condition|(
name|hold
operator|!=
...
condition|)
block|{
comment|/* Change the hash table entry to undefined, so                          that _bfd_generic_link_add_one_symbol will do                          the right thing.  */
name|hold
operator|->
name|root
operator|.
...
expr_stmt|;
block|}
comment|/* If we are looking at a shared object, and we have                      already seen this symbol defined elsewhere, then                      don't try to define it again. */
if|if
condition|(
name|hold
operator|!=
...
condition|)
block|{
comment|/* Don't add an indirect symbol.  */
block|}
else|else
block|{
name|struct
...
operator|)
expr_stmt|;
comment|/* Copy over the global table offset entry.                              This may have been already set up by a                              check_relocs routine.  */
if|if
condition|(
name|h
operator|->
...
argument_list|)
expr_stmt|;
comment|/* FIXME: There may be other information to                              copy over for particular targets.  */
...
name|verdefs
decl_stmt|;
comment|/* Whether we are exporting all dynamic symbols.  */
name|boolean
name|export_dynamic
decl_stmt|;
comment|/* Whether we removed any symbols from the dynamic symbol table.  */
name|boolean
name|removed_dynamic
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Attach all the symbols to their version information.  This          may cause some symbols to be unexported.  */
name|sinfo
operator|.
name|output_bfd
operator|=
...
name|spp
decl_stmt|;
comment|/* Don't include this section in the output file.  */
for|for
control|(
name|spp
operator|=
...
condition|)
block|{
comment|/* Some dynamic symbols were changed to be local 		 symbols.  In this case, we renumber all of the 		 dynamic symbols, so that we don't have a hole.  If 		 the backend changed dynsymcount, then assume that the 		 new symbols are at the start.  This is the case on 		 the MIPS.  FIXME: The names of the removed symbols 		 will still be in the dynamic string table, wasting 		 space.  */
name|elf_hash_table
argument_list|(
name|info
argument_list|)
...
name|spp
decl_stmt|;
comment|/* We don't have any version definitions, so we can just                remove the section.  */
for|for
control|(
name|spp
operator|=
...
name|spp
decl_stmt|;
comment|/* We don't need any symbol versions; just discard the              section.  */
for|for
control|(
name|spp
operator|=
...
literal|1
expr_stmt|;
comment|/* FIXME: The name of the symbol has 				 already been recorded in the dynamic 				 string table section.  */
block|}
break|break;
block|}
block|}
...
call|)
argument_list|(
literal|"%s: undefined version name %s"
argument_list|,
name|bfd_get_filename
argument_list|(
name|sinfo
...
name|d
decl_stmt|;
comment|/* See if can find what version this symbol is in.  If the          symbol is supposed to eb local, then don't actually register          it.  */
name|deflt
operator|=
name|NULL
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* We didn't include the version string in the dynamic string          table, so we must not consider it in the hash table.  */
name|name
operator|=
name|h
operator|->
