...
begin_comment
comment|/* This function is called to create an indirect symbol from the    default for the symbol with the default version if needed. The    symbol is described by H, NAME, SYM, PSEC, VALUE, and OVERRIDE.  We    set DYNSYM if the new indirect symbol is dynamic. DT_NEEDED    indicates if it comes from a DT_NEEDED entry of a shared object.  */
end_comment
...
condition|)
block|{
comment|/* We are overridden by an old defition. We need to check if we 	 need to create the indirect symbol from the default name.  */
name|hi
operator|=
name|elf_link_hash_lookup
argument_list|(
...
condition|)
block|{
comment|/* Here SHORTNAME is a versioned name, so we don't expect to see 	 the type of override we do in the case above unless it is 	 overridden by a versioned definiton.  */
if|if
condition|(
name|hi
operator|->
...
return|;
block|}
comment|/* Make all global versions with definiton.  */
for|for
control|(
name|t
operator|=
...
condition|)
block|{
comment|/* Check if all global versions have a definiton.  */
name|all_defined
operator|=
name|true
expr_stmt|;
...
control|)
block|{
comment|/* If the match is "*", keep looking for a more 		     explicit, perhaps even global, match.  */
if|if
condition|(
name|d
operator|->
...
name|symver_sec
decl_stmt|;
comment|/* first SHF_TLS section (if any).  */
name|asection
modifier|*
name|first_tls_sec
decl_stmt|;
...
begin_comment
comment|/* For DSOs loaded in via a DT_NEEDED entry, emulate ld.so in    allowing an unsatisfied unversioned symbol in the DSO to match a    versioned symbol that would normally require an explicit version.  */
end_comment
...
condition|)
block|{
comment|/* This is the oldest (default) sym.  We can use it.  */
name|free
argument_list|(
name|extversym
argument_list|)
...
condition|)
block|{
comment|/* Relocs have been edited.  Ensure edited version is 		 used later in relocate_section.  */
name|elf_section_data
argument_list|(
name|eh
argument_list|)
