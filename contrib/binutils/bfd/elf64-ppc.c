...
begin_comment
comment|/* .plt call stub instructions.  The normal stub is like this, but    sometimes the .plt entry crosses a 64k boundary and we need to    insert an addi to adjust r12.  */
end_comment
...
name|opd_adjust
decl_stmt|;
comment|/* An array for toc sections, indexed by offset/8.  */
struct|struct
name|_toc_sec_data
block|{
comment|/* Specifies the relocation symbol index used at a given toc offset.  */
name|unsigned
modifier|*
name|symndx
decl_stmt|;
comment|/* And the relocation addend.  */
name|bfd_vma
modifier|*
name|add
decl_stmt|;
...
begin_comment
comment|/* Of those relocs that might be copied as dynamic relocs, this function    selects those that must be copied when linking a shared library,    even when the symbol is local.  */
end_comment
...
operator|)
condition|)
comment|/* We have a new-style __tls_get_addr call with a marker 		 reloc.  */
empty_stmt|;
else|else
comment|/* Mark this section as having an old-style call.  */
name|sec
operator|->
...
name|R_PPC64_TLSLD
case|:
comment|/* These special tls relocs tie a call to __tls_get_addr with 	     its parameter symbol.  */
break|break;
case|case
name|R_PPC64_GOT_TLSLD16
case|:
...
begin_comment
comment|/* Return TRUE iff REL is a branch reloc with a global symbol matching    HASH1 or HASH2.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|branch_reloc_hash_match
parameter_list|(
specifier|const
name|bfd
modifier|*
...
name|NULL
decl_stmt|;
comment|/* Look at all the sections for this file.  Make two passes over 	 the relocs.  On the first pass, mark toc entries involved 	 with tls relocs, and check that tls relocs involved in 	 setting up a tls_get_addr call are indeed followed by such a 	 call.  If they are not, exclude them from the optimizations 	 done on the second pass.  */
for|for
control|(
name|pass
operator|=
...
condition|)
continue|continue;
comment|/* Uh oh, we didn't find the expected call.  We 			 could just mark this symbol to exclude it 			 from tls optimization but it's safer to skip 			 the entire section.  */
name|sec
operator|->
name|has_tls_reloc
operator|=
...
break|break;
block|}
comment|/* Calls to dynamic lib functions go through a plt call stub 	 that uses r2.  */
name|eh
operator|=
operator|(
expr|struct
...
literal|1
expr_stmt|;
comment|/* If not using the newer R_PPC64_TLSGD/LD to mark 		 __tls_get_addr calls, we must trust that the call 		 stays with its arg setup insns, ie. that the next 		 reloc is the __tls_get_addr call associated with 		 the current reloc.  Edit both insns.  */
if|if
condition|(
name|input_section
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Zap the reloc on the _tls_get_addr call too.  */
name|BFD_ASSERT
argument_list|(
name|offset
operator|==
