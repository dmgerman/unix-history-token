...
condition|)
block|{
comment|/* When the first option starts with '-' we support POSIX-compatible 	 option parsing.  */
name|do_posix
operator|=
literal|1
expr_stmt|;
...
block|}
block|}
comment|/* With POSIX-compatible option parsing continue with the next 	 argument if it starts with '-'.  */
if|if
condition|(
name|do_posix
operator|&&
