...
begin_comment
comment|/* Use alternative machine code?  */
end_comment
...
begin_comment
comment|/* True if --localize-hidden is in effect.  */
end_comment
...
begin_comment
comment|/* If this is TRUE, we retain BSF_FILE symbols.  */
end_comment

begin_decl_stmt
specifier|static
name|bfd_boolean
name|keep_file_symbols
init|=
name|FALSE
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* True if --extract-symbol was passed on the command line.  */
end_comment
...
begin_comment
comment|/* If `reverse_bytes' is nonzero, then reverse the order of every chunk    of<reverse_bytes> bytes within each output section.  */
end_comment
...
block|,
name|OPTION_STRIP_UNNEEDED_SYMBOL
block|,
name|OPTION_STRIP_UNNEEDED_SYMBOLS
block|,
name|OPTION_KEEP_SYMBOLS
block|,
name|OPTION_LOCALIZE_HIDDEN
block|,
name|OPTION_LOCALIZE_SYMBOLS
block|,
name|OPTION_GLOBALIZE_SYMBOL
block|,
...
name|OPTION_IMPURE
block|,
name|OPTION_EXTRACT_SYMBOL
block|,
name|OPTION_REVERSE_BYTES
block|}
enum|;
end_enum
...
block|,
block|{
literal|"strip-unneeded-symbols"
block|,
name|required_argument
block|,
literal|0
...
name|_
argument_list|(
literal|"%s:%d: Ignoring rubbish found on this line"
argument_list|)
argument_list|,
name|filename
argument_list|,
...
begin_comment
comment|/* Return a pointer to the symbol used as a signature for GROUP.  */
end_comment
...
name|gname
decl_stmt|;
comment|/* PR binutils/3166 	 Group sections look like debugging sections but they are not. 	 (They have a non-zero size but they are not ALLOCated).  */
if|if
condition|(
name|strip_symbols
operator|==
...
name|TRUE
return|;
comment|/* PR binutils/3181 	 If we are going to strip the group signature symbol, then 	 strip the group section too.  */
name|gsym
operator|=
name|group_signature
argument_list|(
...
begin_comment
comment|/* Return true if SYM is a hidden symbol.  */
end_comment
...
condition|)
block|{
comment|/* There are multiple ways to set 'keep' above, but if it 	     was the relocatable symbol case, then that's an error.  */
if|if
condition|(
name|used_in_reloc
condition|)
...
name|_
argument_list|(
literal|"not stripping symbol `%s' because it is named in a relocation"
argument_list|)
argument_list|,
name|name
argument_list|)
...
name|_
argument_list|(
literal|"%s:%d: garbage found at end of line"
argument_list|)
argument_list|,
name|filename
argument_list|,
...
name|_
argument_list|(
literal|"%s:%d: missing new symbol name"
argument_list|)
argument_list|,
name|filename
argument_list|,
...
name|_
argument_list|(
literal|"%s:%d: premature end of file"
argument_list|)
argument_list|,
name|filename
argument_list|,
...
begin_comment
comment|/* Copy unkown object file IBFD onto OBFD.    Returns TRUE upon success, FALSE otherwise.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|copy_unknown_object
parameter_list|(
name|bfd
modifier|*
name|ibfd
...
name|_
argument_list|(
literal|"copy from `%s' [unknown] to `%s' [unknown]\n"
argument_list|)
argument_list|,
name|bfd_get_archive_filename
argument_list|(
...
name|SEC_HAS_CONTENTS
expr_stmt|;
comment|/* bfd_make_section_with_flags() does not return very helpful 	     error codes, so check for the most likely user error first.  */
if|if
condition|(
...
name|_
argument_list|(
literal|"can't add section '%s' - it already exists!"
argument_list|)
argument_list|,
name|padd
operator|->
...
return|;
block|}
comment|/* Special processing for PE format files.  We 	 have no way to distinguish PE from COFF here.  */
if|if
condition|(
name|bfd_get_flavour
argument_list|(
...
name|asection
modifier|*
name|highest_section
decl_stmt|;
name|asection
modifier|*
name|sec
decl_stmt|;
comment|/* The PE spec requires that all sections be adjacent and sorted 	     in ascending order of VMA.  It also specifies that debug 	     sections should be last.  This is despite the fact that debug 	     sections are not loaded into memory and so in theory have no 	     use for a VMA.  	     This means that the debuglink section must be given a non-zero 	     VMA which makes it contiguous with other debug sections.  So 	     walk the current section list, find the section with the 	     highest VMA and start the debuglink section after that one.  */
for|for
control|(
name|sec
operator|=
...
name|size
argument_list|,
comment|/* FIXME: We ought to be using 					  COFF_PAGE_SIZE here or maybe 					  bfd_get_section_alignment() (if it 					  was set) but since this is for PE 					  and we know the required alignment 					  it is easier just to hard code it.  */
literal|0x1000
argument_list|)
expr_stmt|;
else|else
comment|/* Umm, not sure what to do in this case.  */
name|debuglink_vma
operator|=
literal|0x1000
expr_stmt|;
...
block|}
block|}
comment|/* Do not copy backend data if --extract-symbol is passed; anything      that needs to look at the section contents will fail.  */
if|if
condition|(
name|extract_symbol
condition|)
...
name|_
argument_list|(
literal|"this target does not support %lu alternative machine codes"
argument_list|)
argument_list|,
name|use_alt_mach_code
argument_list|)
...
name|_
argument_list|(
literal|"treating that number as an absolute e_machine value instead"
argument_list|)
argument_list|)
expr_stmt|;
name|elf_elfheader
...
name|_
argument_list|(
literal|"ignoring the alternative value"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
begin_comment
comment|/* Read each archive element in turn from IBFD, copy the    contents to temp file, and keep the temp file handle.    If 'force_output_target' is TRUE then make sure that    all elements in the new archive are of the type    'output_target'.  */
end_comment
...
name|_
argument_list|(
literal|"cannot create tempdir for archive copying (error: %s)"
argument_list|)
argument_list|,
name|strerror
argument_list|(
...
condition|)
block|{
comment|/* PR binutils/3110: Cope with archives 	     containing multiple target types.  */
if|if
condition|(
name|force_output_target
condition|)
...
begin_comment
comment|/* Once each of the sections is copied, we may still need to do some    finalization work for private section headers.  Do that here.  */
end_comment
...
condition|)
block|{
comment|/* We don't handle leftover bytes (too many possible behaviors, 	     and we don't know what the user wants).  The section length 	     must be a multiple of the number of bytes to swap.  */
if|if
condition|(
operator|(
name|size
...
comment|/* User must pad the section up in order to do this.  */
name|fatal
argument_list|(
name|_
argument_list|(
literal|"cannot reverse bytes: length of section %s must be evenly divisible by %d"
argument_list|)
argument_list|,
name|bfd_section_name
argument_list|(
...
name|_
argument_list|(
literal|"could not create temporary file to hold stripped copy of '%s'"
argument_list|)
argument_list|,
name|argv
index|[
...
name|_
argument_list|(
literal|"unable to parse alternative machine code"
argument_list|)
argument_list|)
expr_stmt|;
break|break;
...
name|_
argument_list|(
literal|"number of bytes to reverse must be positive and even"
argument_list|)
argument_list|)
expr_stmt|;
if|if
...
name|_
argument_list|(
literal|"Warning: ignoring previous --reverse-bytes value of %d"
argument_list|)
argument_list|,
name|prev
argument_list|)
...
name|_
argument_list|(
literal|"warning: could not create temporary file whilst copying '%s', (error: %s)"
argument_list|)
argument_list|,
name|input_filename
argument_list|,
