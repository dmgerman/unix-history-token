...
begin_comment
comment|/* This header file describes a generic debugging information format.    We may eventually have readers which convert different formats into    this generic format, and writers which write it out.  The initial    impetus for this was writing a converter from stabs to HP IEEE-695    debugging format.  */
end_comment
...
parameter_list|)
function_decl|;
comment|/* Push a bfd_boolean type onto the type stack, given the size.  */
name|bfd_boolean
function_decl|(
modifier|*
name|bool_type
...
parameter_list|)
function_decl|;
comment|/* Start writing out a function.  The return type must be popped off      the stack.  The bfd_boolean is TRUE if the function is global.  This      is followed by calls to function_parameter, followed by block      information.  */
name|bfd_boolean
function_decl|(
modifier|*
name|start_function
...
begin_comment
comment|/* Record a function definition.  This implicitly starts a function    block.  The debug_type argument is the type of the return value.    The bfd_boolean indicates whether the function is globally visible.    The bfd_vma is the address of the start of the function.  Currently    the parameter types are specified by calls to    debug_record_parameter.  */
end_comment
...
begin_comment
comment|/* Make an integer type of a given size.  The bfd_boolean argument is TRUE    if the integer is unsigned.  */
end_comment
...
begin_comment
comment|/* Make a set of a given type.  For example, a Pascal set type.  The    bfd_boolean argument is TRUE if this set is actually a bitstring, as in    CHILL.  */
end_comment
...
begin_comment
comment|/* Get the NULL terminated array of parameter types for a function or    method type (actually, parameter types are not currently stored for    function types).  This may be used to determine whether a method    type is a stub method or not.  The last argument points to a    bfd_boolean which is set to TRUE if the function takes a variable    number of arguments.  */
end_comment
...
begin_comment
comment|/* Write out the recorded debugging information.  This takes a set of    function pointers which are called to do the actual writing.  The    first void * is the debugging handle.  The second void * is a handle    which is passed to the functions.  */
end_comment
