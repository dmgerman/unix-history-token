...
begin_comment
comment|/* stabs.c -- Parse stabs debugging information    Copyright 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004,    2006, 2007 Free Software Foundation, Inc.    Written by Ian Lance Taylor<ian@cygnus.com>.     This file is part of GNU Binutils.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA    02110-1301, USA.  */
end_comment
...
name|tags
decl_stmt|;
comment|/* Set by parse_stab_type if it sees a structure defined as a cross      reference to itself.  Reset by parse_stab_type otherwise.  */
name|bfd_boolean
name|self_crossref
decl_stmt|;
block|}
...
name|debug_type
modifier|*
name|stab_demangle_v3_argtypes
parameter_list|(
name|void
modifier|*
parameter_list|,
...
condition|)
block|{
comment|/* If bfd_vma is larger than unsigned long, and the number is          meant to be negative, we have to make sure that we sign          extend properly.  */
if|if
condition|(
operator|*
name|orig
...
name|TRUE
return|;
comment|/* Just accumulate strings until we see a non N_SO symbol.  If          the string starts with a directory separator or some other 	 form of absolute path specification, we discard the previously          accumulated strings.  */
if|if
condition|(
name|info
operator|->
...
name|TRUE
return|;
comment|/* INFO->SELF_CROSSREF is set by parse_stab_type if this type is          a cross reference to itself.  These are generated by some          versions of g++.  */
name|self_crossref
operator|=
name|info
operator|->
...
name|dtype
expr_stmt|;
comment|/* See if we have a cross reference to this tag which we can now          fill in.  Avoid filling in a cross reference to ourselves,          because that would lead to circular debugging information.  */
if|if
condition|(
operator|!
name|self_crossref
...
block|}
block|}
comment|/* Some versions of g++ can emit stabs like 	       fleep:T20=xsfleep: 	   which define structures in terms of themselves.  We need to 	   tell the caller to avoid building a circular structure.  */
if|if
condition|(
name|typename
operator|!=
...
begin_comment
comment|/* Read the description of a structure (or union type) and return an object    describing the type.     PP points to a character pointer that points to the next unconsumed token    in the stabs string.  For example, given stabs "A:T4=s4a:1,0,32;;",    *PP will point to "4a:1,0,32;;".  */
end_comment
...
block|{
comment|/* This is a completely weird case.  In order to stuff in the 	     names that might contain colons (the usual name delimiter), 	     Mike Tiemann defined a different name format which is 	     signalled if the identifier is "op$".  In that case, the 	     format is "op$::XXXX." where XXXX is the name.  This is 	     used for names like "+" or "=".  YUUUUUUUK!  FIXME!  */
operator|*
name|pp
operator|=
name|p
...
name|minfo
decl_stmt|;
comment|/* Check for the g++ V3 ABI.  */
if|if
condition|(
name|physname
index|[
...
begin_comment
comment|/* Demangle a template.  If PNAME is not NULL, this sets *PNAME to a    string representation of the template.  */
end_comment
...
block|}
block|}
comment|/* We can translate this to a string fairly easily by invoking the      regular demangling routine.  */
if|if
condition|(
name|pname
operator|!=
...
return|;
block|}
comment|/* Eliminating all spaces, except those between> characters,          makes it more likely that the demangled name will match the          name which g++ used as the structure name.  */
for|for
control|(
name|from
operator|=
...
return|;
block|}
comment|/* We expect to see TYPED_NAME, and the right subtree describes the      function type.  */
if|if
condition|(
name|dc
operator|->
...
name|_
argument_list|(
literal|"Demangled name is not a function\n"
argument_list|)
argument_list|)
expr_stmt|;
name|free
...
begin_comment
comment|/* Demangle an argument list in a struct demangle_component tree.    Returns a DEBUG_TYPE_NULL terminated array of argument types, and    sets *PVARARGS to indicate whether this is a varargs function.  */
end_comment
...
name|_
argument_list|(
literal|"Unexpected type in v3 arglist demangling\n"
argument_list|)
argument_list|)
expr_stmt|;
name|free
...
begin_comment
comment|/* Convert a struct demangle_component tree describing an argument    type into a debug_type.  */
end_comment
...
condition|)
block|{
comment|/* FIXME: These are demangle component types which we probably 	 need to handle one way or another.  */
case|case
...
name|_
argument_list|(
literal|"Unrecognized demangle component %d\n"
argument_list|)
argument_list|,
operator|(
name|int
...
condition|)
block|{
comment|/* Try to find this type by looking through the context 		 class.  */
for|for
control|(
init|;
operator|*
...
name|alc
decl_stmt|;
comment|/* We print this component to get a class name which we can 	   use.  FIXME: This probably won't work if the template uses 	   template parameters which refer to an outer template.  */
name|p
operator|=
...
name|_
argument_list|(
literal|"Failed to print demangled template\n"
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
name|DEMANGLE_COMPONENT_RESTRICT
case|:
comment|/* FIXME: We have no way to represent restrict.  */
return|return
name|dt
return|;
case|case
...
condition|)
block|{
comment|/* In this case the return type is actually unknown. 	       However, I'm not sure this will ever arise in practice; 	       normally an unknown return type would only appear at 	       the top level, which is handled above.  */
name|dt
operator|=
name|debug_make_void_type
argument_list|(
...
name|ret
decl_stmt|;
comment|/* We print this component in order to find out the type name. 	   FIXME: Should we instead expose the 	   demangle_builtin_type_info structure?  */
name|p
operator|=
name|cplus_demangle_print
argument_list|(
...
name|_
argument_list|(
literal|"Couldn't get demangled builtin type\n"
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
return|;
block|}
comment|/* The mangling is based on the type, but does not itself 	   indicate what the sizes are.  So we have to guess.  */
if|if
condition|(
name|strcmp
argument_list|(
...
name|_
argument_list|(
literal|"Unexpected demangled varargs\n"
argument_list|)
argument_list|)
expr_stmt|;
else|else
...
name|_
argument_list|(
literal|"Unrecognized demangled builtin type\n"
argument_list|)
argument_list|)
expr_stmt|;
name|ret
