...
begin_comment
comment|/* Used to take a snapshot of the cref hash table when starting to    add syms from an as-needed library.  */
end_comment
...
begin_comment
comment|/* Add a symbol to the cref hash table.  This is called for every    global symbol that is seen during the link.  */
end_comment
...
name|_
argument_list|(
literal|"%X%P: cref alloc failed: %E\n"
argument_list|)
argument_list|)
expr_stmt|;
name|r
...
begin_comment
comment|/* Called before loading an as-needed library to take a snapshot of    the cref hash table, and after we have loaded or found that the    library was not needed.  */
end_comment
...
condition|)
block|{
comment|/* The only way old_tab can be NULL is if the cref hash table 	     had not been initialised when notice_as_needed.  */
...
begin_comment
comment|/* Check for prohibited cross references to local and section symbols.  */
end_comment
...
name|info
decl_stmt|;
comment|/* We need to look through the relocations for this BFD, to see      if any of the relocations which refer to this symbol are from      a prohibited section.  Note that we need to do this even for      the BFD in which the symbol is defined, since even a single      BFD might contain a prohibited cross reference.  */
name|li
operator|=
name|abfd
operator|->
...
condition|)
return|return;
comment|/* This section is one for which cross references are prohibited.      Look through the relocations, and see if any of them are to      INFO->SYM_NAME.  If INFO->SYMNAME is NULL, check for relocations      against the section symbol.  If INFO->GLOBAL is TRUE, the      definition is global, check for relocations against the global      symbols.  Otherwise check for relocations against the local and      section symbols.  */
name|symname
operator|=
name|info
operator|->
