...
name|_
argument_list|(
literal|"%P%F: out of memory during initialization"
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Callers of exp_fold_tree need to increment this.  */
name|lang_statement_iteration
operator|=
literal|0
expr_stmt|;
...
block|if (strcmp (name, DEFAULT_MEMORY_REGION) == 0)     {       if (lang_memory_region_list != NULL) 	return lang_memory_region_list;     }
endif|#
directive|endif
if|if
condition|(
...
name|FALSE
expr_stmt|;
comment|/* Discard sections marked with SEC_EXCLUDE if we are doing a final      link.  Discard debugging sections marked with SEC_EXCLUDE on a      relocatable link too.  */
if|if
condition|(
operator|(
name|flags
...
name|alignment_power
expr_stmt|;
comment|/* If supplied an alignment, then force it.  */
if|if
condition|(
name|output
operator|->
...
directive|if
literal|0
block|lang_output_section_statement_lookup (".text");   lang_output_section_statement_lookup (".data");    default_common_section = lang_output_section_statement_lookup (".bss");    if (!placed_commons)     {       lang_wild_statement_type *new =       new_stat (lang_wild_statement,&default_common_section->children);        new->section_name = "COMMON";       new->filename = NULL;       lang_list_init (&new->children);     }
endif|#
directive|endif
block|}
end_function
...
parameter_list|)
define|\
value|(((bfd_get_section_flags (bfd, s)& SEC_THREAD_LOCAL)			\     ? ((bfd_get_section_flags (bfd, s)& (SEC_LOAD | SEC_NEVER_LOAD))	\        != SEC_LOAD)							\     :  ((bfd_get_section_flags (bfd, s)& (SEC_ALLOC | SEC_NEVER_LOAD)) \ 	!= SEC_ALLOC))							\    || bfd_section_size (bfd, s) == 0)
end_define
...
name|_
argument_list|(
literal|"%P%F: error: no memory region specified for loadable section `%s'\n"
argument_list|)
argument_list|,
name|bfd_get_section_name
argument_list|(
...
name|_
argument_list|(
literal|"%P: warning: no memory region specified for loadable section `%s'\n"
argument_list|)
argument_list|,
name|bfd_get_section_name
argument_list|(
...
name|_
argument_list|(
literal|"%F%S: non constant or forward reference address expression for section %s\n"
argument_list|)
argument_list|,
name|os
operator|->
...
block|}
block|}
comment|/* Some backend relaxers want to refer to the output section size.  Give      them a section size that does not change on the next call while they      relax.  We can't set this at top because lang_reset_memory_regions      which is called before we get here, sets _raw_size to 0 on relaxing      rounds.  */
for|for
control|(
name|o
operator|=
...
name|_
argument_list|(
literal|"%P%X: failed to merge target specific data of file %B\n"
argument_list|)
argument_list|,
name|input_bfd
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* We must do this after lang_do_assignments, because it uses 	     _raw_size.  */
name|lang_reset_memory_regions
argument_list|()
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* If the normal relax is done and the relax finalize pass 	     is not performed yet, we perform another relax pass.  */
if|if
condition|(
operator|!
name|relax_again
