...
break|break;
block|}
comment|/* Set the output_section field so that wild_doit does not 	     create a lang_input_section structure for this section. 	     Since there might be a symbol in the section being 	     discarded, we must retain a pointer to the section which 	     we are really going to use.  */
name|sec
operator|->
name|output_section
operator|=
...
begin_comment
comment|/* Return true if the PATTERN argument is a wildcard pattern.    Although backslashes are treated specially if a pattern contains    wildcards, we do not consider the mere presence of a backslash to    be enough to cause the the pattern to be treated as a wildcard.    That lets us handle DOS filenames more naturally.  */
end_comment
...
begin_comment
comment|/* Expand a wild statement for a particular FILE, marking its sections KEEP    as needed.  SECTION may be NULL, in which case it is a wild card.  */
end_comment
...
begin_comment
comment|/* Handle a wild statement, marking it against GC.  SECTION or FILE or both    may be NULL, indicating that it is a wildcard.  */
end_comment
