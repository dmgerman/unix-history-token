...
expr_stmt|;
block|}
comment|/* For now make .tbss normal section.  */
if|if
condition|(
operator|(
name|flags
...
begin_comment
comment|/* Work out the load- and run-time regions from a script statement, and    store them in *LMA_REGION and *REGION respectively.     MEMSPEC is the name of the run-time region, or "*default*" if the    statement didn't specify one.  LMA_MEMSPEC is the name of the    load-time region, or null if the statement didn't specify one.    HAVE_LMA_P is true if the statement had an explicit load address.     It is an error to specify both a load region and a load address.  */
end_comment
...
name|name
expr_stmt|;
comment|/* For now, assume that "*default*" is the run-time memory region and      that no load-time region has been specified.  It doesn't really      matter what we say here, since lang_leave_overlay will override it.  */
name|lang_leave_output_section_statement
argument_list|(
name|fill
argument_list|,
