...
begin_comment
comment|/* Demangler for g++ V3 ABI.    Copyright (C) 2003, 2004, 2005, 2006, 2007 Free Software Foundation, Inc.    Written by Ian Lance Taylor<ian@wasabisystems.com>.     This file is part of the libiberty library, which is part of GCC.     This file is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     In addition to the permissions in the GNU General Public License, the    Free Software Foundation gives you unlimited permission to link the    compiled version of this file into combinations with other programs,    and to distribute those combinations without any restriction coming    from the use of this file.  (The General Public License restrictions    do apply in other respects; for example, they cover modification of    the file, and distribution when not linked into a combined    executable.)     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 51 Franklin Street - Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
value|d_print_callback
end_define

begin_function_decl
specifier|static
name|int
name|d_print_callback
parameter_list|(
name|int
parameter_list|,
specifier|const
...
begin_comment
comment|/* We use these structures to hold information during printing.  */
end_comment

begin_struct
struct|struct
name|d_growable_string
block|{
...
name|options
decl_stmt|;
comment|/* Fixed-length allocated buffer for demangled data, flushed to the      callback with a NUL termination once full.  */
name|char
name|buf
index|[
name|D_PRINT_BUFFER_LENGTH
...
name|len
decl_stmt|;
comment|/* The last character printed, saved individually so that it survives      any buffer flush.  */
name|char
name|last_char
decl_stmt|;
comment|/* Callback function to handle demangled buffer flush.  */
name|demangle_callbackref
name|callback
decl_stmt|;
comment|/* Opaque callback argument.  */
name|void
modifier|*
name|opaque
decl_stmt|;
...
name|modifiers
decl_stmt|;
comment|/* Set to 1 if we saw a demangling error.  */
name|int
name|demangle_failure
decl_stmt|;
block|}
struct|;
end_struct
...
specifier|inline
name|void
name|d_growable_string_append_buffer
parameter_list|(
name|struct
name|d_growable_string
modifier|*
...
begin_comment
comment|/* Initialize a growable string.  */
end_comment
...
begin_comment
comment|/* Grow a growable string to a given size.  */
end_comment
...
condition|)
return|return;
comment|/* Start allocation at two bytes to avoid any possibility of confusion      with the special value of 1 used as a return in *palc to indicate      allocation failures.  */
name|newalc
operator|=
name|dgs
operator|->
...
begin_comment
comment|/* Append a buffer to a growable string.  */
end_comment
...
begin_comment
comment|/* Bridge growable strings to the callback mechanism.  */
end_comment
...
begin_comment
comment|/* Initialize a print information structure.  */
end_comment
...
begin_comment
comment|/* Indicate that an error occurred during printing, and test for error.  */
end_comment
...
begin_comment
comment|/* Flush buffered characters to the callback.  */
end_comment
...
begin_comment
comment|/* Append characters and buffers for printing.  */
end_comment
...
begin_comment
comment|/* Turn components into a human readable string.  OPTIONS is the    options bits passed to the demangler.  DC is the tree to print.    CALLBACK is a function to call to flush demangled string segments    as they fill the intermediate buffer, and OPAQUE is a generalized    callback argument.  On success, this returns 1.  On failure,    it returns 0, indicating a bad parse.  It does not use heap    memory to build an output string, so cannot encounter memory    allocation failure.  */
end_comment
...
condition|)
block|{
comment|/* Special-case Java arrays, so that JArray<TYPE> appears                instead as TYPE[].  */
name|d_print_comp
argument_list|(
name|dpi
argument_list|,
...
begin_comment
comment|/* Internal implementation for the demangler.  If MANGLED is a g++ v3 ABI    mangled name, return strings in repeated callback giving the demangled    name.  OPTIONS is the usual libiberty demangler options.  On success,    this returns 1.  On failure, returns 0.  */
end_comment
...
begin_comment
comment|/* Entry point for the demangler.  If MANGLED is a g++ v3 ABI mangled    name, return a buffer allocated with malloc holding the demangled    name.  OPTIONS is the usual libiberty demangler options.  On    success, this sets *PALC to the allocated size of the returned    buffer.  On failure, this sets *PALC to 0 for a bad name, or 1 for    a memory allocation failure, and returns NULL.  */
end_comment
...
begin_comment
comment|/* Demangle a Java symbol.  Java uses a subset of the V3 ABI C++ mangling     conventions, but the output formatting is a little different.    This instructs the C++ demangler not to emit pointer characters ("*"), to    use Java's namespace separator symbol ("." instead of "::"), and to output    JArray<TYPE> as TYPE[].  */
end_comment
