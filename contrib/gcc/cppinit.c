...
begin_comment
comment|/* Remove duplicate non-system directories for which there is an equivalent    system directory later in the chain.  The range for removal is between    *HEAD_PTR and END.  Returns the directory before END, or NULL if none.    This algorithm is quadratic in the number of system directories, which is    acceptable since there aren't usually that many of them.  */
end_comment
...
init|=
block|{
comment|/*              c99 objc c++ xnum std dollar c++comm digr  */
comment|/* GNUC89 */
block|{
literal|0
block|,
...
begin_comment
comment|/* This structure defines one built-in identifier.  A node will be    entered in the hash table under the name NAME, with value VALUE (if    any).  If flags has OPERATOR, the node's operator field is used; if    flags has BUILTIN the node's builtin field is used.  Macros that are    known at build time should not be flagged BUILTIN, as then they do    not appear in macro dumps with e.g. -dM or -dD.     Two values are not compile time constants, so we tag    them in the FLAGS field instead:    VERS		value is the global version_string, quoted    ULP		value is the global user_label_prefix  */
end_comment
...
name|t
parameter_list|)
value|{ U n, 0, t, BUILTIN, sizeof n - 1 }
end_define
