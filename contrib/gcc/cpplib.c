...
begin_comment
comment|/* CYGNUS LOCAL: shebs -noquiet */
end_comment
...
begin_comment
comment|/* Make sure PFILE->token_buffer will hold at least N more chars. */
end_comment
...
condition|)
block|{
comment|/* Just leave the entire #define in the output stack. */
block|}
elseif|else
if|if
condition|(
...
index|]
decl_stmt|;
comment|/* Stringify it it hasn't already been */
if|if
condition|(
name|arg
operator|->
...
begin_comment
comment|/* Handle command-line options in (argc, argv).    Can be called multiple times, to handle multiple sets of options.    Returns if an unrecognized option is seen.    Returns number of handled arguments.  */
end_comment
...
begin_comment
comment|/* Free resources used by PFILE. */
end_comment
