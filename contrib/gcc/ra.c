...
begin_comment
comment|/* Things, one might do somewhen:     * Lattice based rematerialization    * create definitions of ever-life regs at the beginning of      the insn chain    * insert loads as soon, stores as late as possible    * insert spill insns as outward as possible (either looptree, or LCM)    * reuse stack-slots    * delete coalesced insns.  Partly done.  The rest can only go, when we get      rid of reload.    * don't destroy coalescing information completely when spilling    * use the constraints from asms   */
end_comment
...
comment|/* We currently rely on the existence of the return value USE as      one of the last insns.  Add it if it's not there anymore.  */
if|if
condition|(
name|last
condition|)
...
condition|)
block|{
comment|/* Some useful presets of the debug level, I often use.  */
case|case
literal|0
case|:
name|debug_new_regalloc
...
literal|0
expr_stmt|;
comment|/* Run regclass first, so we know the preferred and alternate classes      for each pseudo.  Deactivate emitting of debug info, if it's not      explicitly requested.  */
if|if
condition|(
operator|(
name|debug_new_regalloc
...
argument_list|()
expr_stmt|;
comment|/* And they need useful classes too.  */
name|regclass
argument_list|(
name|get_insns
argument_list|()
