...
return|;
comment|/* We can't inline functions that return BLKmode structures in registers.  */
if|if
condition|(
name|TYPE_MODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Must set DECL_ABSTRACT_ORIGIN here for local variables, to ensure 	 that we don't get -Wshadow warnings.  But don't set it here if 	 pushdecl might return a duplicate decl, as that will result in 	 incorrect DWARF debug info.  */
if|if
condition|(
operator|!
name|DECL_EXTERNAL
...
argument_list|)
expr_stmt|;
comment|/* If we didn't set DECL_ABSTRACT_ORIGIN above, then set it now. 	 Simpler to just set it always rather than checking. 	 If the decl we get back is the copy of 't' that we started with, 	 then set the DECL_ABSTRACT_ORIGIN.  Otherwise, we must have a 	 duplicate decl, and we got the older one back.  In that case, setting 	 DECL_ABSTRACT_ORIGIN is not appropriate.  */
if|if
condition|(
name|newd
operator|==
...
literal|0
expr_stmt|;
comment|/* Integrating function calls isn't safe anymore, so turn on      flag_no_inline.  */
name|flag_no_inline
operator|=
literal|1
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Reset flag_no_inline to its original value.  */
name|flag_no_inline
operator|=
name|save_flag_no_inline
expr_stmt|;
