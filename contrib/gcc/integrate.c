...
argument_list|)
decl_stmt|;
comment|/* For functions marked as inline increase the maximum size to      MAX_INLINE_INSNS_RTL (--param max-inline-insn-rtl=<n>). For      regular functions use the limit given by INTEGRATE_THRESHOLD.      Note that the RTL inliner is not used by the languages that use      the tree inliner (C, C++).  */
name|int
name|max_insns
init|=
operator|(
...
name|N_
argument_list|(
literal|"function using longjmp cannot be inline"
argument_list|)
return|;
if|if
condition|(
...
return|;
block|}
comment|/* If the function has a target specific attribute attached to it,      then we assume that we should not inline it.  This can be overridden      by the target if it defines TARGET_FUNCTION_ATTRIBUTE_INLINABLE_P.  */
if|if
condition|(
operator|!
name|function_attribute_inlinable_p
...
begin_comment
comment|/* Compare two BLOCKs for qsort.  The key we sort on is the    BLOCK_ABSTRACT_ORIGIN of the blocks.  We cannot just subtract the    two pointers, because it may overflow sizeof(int).  */
end_comment
...
return|;
block|}
comment|/* If there is a TARGET which is a readonly BLKmode MEM and DECL_RESULT      is also a mem, we are going to lose the readonly on the stores, so don't      inline.  */
if|if
condition|(
name|target
operator|!=
...
argument_list|()
expr_stmt|;
comment|/* The mode if LOC and ARG can differ if LOC was a variable 		 that had its mode promoted.  */
name|arg_vals
index|[
name|i
index|]
...
literal|0
expr_stmt|;
comment|/* If the formal type was const but the actual was not, we might 	 end up here with an rtx wrongly tagged unchanging in the caller's 	 context.  Fix that.  */
if|if
condition|(
name|arg_vals
index|[
...
argument_list|)
expr_stmt|;
comment|/* If the inlined function calls __builtin_constant_p, then we'll      need to call purge_builtin_constant_p on this function.  */
if|if
condition|(
name|inl_f
operator|->
...
begin_comment
comment|/* Make copies of each insn in the given list using the mapping    computed in expand_inline_function. This function may call itself for    insns containing sequences.     Copying is done in two passes, first the insns and then their REG_NOTES.     If static_chain_value is nonzero, it represents the context-pointer    register for the function.  */
end_comment
...
name|copy
condition|)
comment|/*Copied a line note, but line numbering is off*/
empty_stmt|;
elseif|else
if|if
condition|(
...
literal|0
expr_stmt|;
comment|/* If inlining, squish aliasing data that references the subroutine's 	 parameter list, since that's no longer applicable.  */
if|if
condition|(
name|inlining
operator|&&
...
argument_list|()
expr_stmt|;
comment|/* Enforce consistency between the addresses in the regular insn flow      and the ones in CALL_INSN_FUNCTION_USAGE lists, if any.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Make sure warnings emitted by the optimizers (e.g. control reaches      end of non-void function) is not wildly incorrect.  */
name|input_location
operator|=
name|DECL_SOURCE_LOCATION
argument_list|(
