...
condition|)
block|{
comment|/* After reload, REG_DEAD notes come sometimes an 		     instruction after the register actually dies.  */
if|if
condition|(
name|reload_completed
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* It is probably not worthwhile to try to split again in the 	     second pass.  However, if flag_schedule_insns is not set, 	     the first and only (if any) scheduling pass is after reload.  */
if|if
condition|(
name|reload_completed
operator|==
