...
expr_stmt|;
block|}
comment|/* If there is a LOOP_{BEG,END} note in the middle of a basic block, then      we must be sure that no instructions are scheduled across it.      Otherwise, the reg_n_refs info (which depends on loop_depth) would      become incorrect.  */
if|if
condition|(
name|loop_notes
condition|)
...
literal|1
expr_stmt|;
comment|/* Add a fake REG_NOTE which we will later convert 		 back into a NOTE_INSN_SETJMP note.  */
name|REG_NOTES
argument_list|(
name|insn
argument_list|)
...
condition|)
block|{
comment|/* ??? It is perhaps a dead_or_set_p bug that it does 		   not check for REG_UNUSED notes itself.  This is necessary 		   for the case where the SET_DEST is a subreg of regno, as 		   dead_or_set_p handles subregs specially.  */
if|if
condition|(
operator|!
name|all_needed
...
name|insn
expr_stmt|;
comment|/* Don't save away NOTE_INSN_SETJMPs, because they must remain 	 immediately after the call they follow.  We use a fake 	 (REG_DEAD (const_int -1)) note to remember them. 	 Likewise with NOTE_INSN_LOOP_BEG and NOTE_INSN_LOOP_END.  */
elseif|else
if|if
condition|(
name|NOTE_LINE_NUMBER
...
begin_comment
comment|/* Search INSN for fake REG_DEAD notes for NOTE_INSN_SETJMP,    NOTE_INSN_LOOP_BEG, and NOTE_INSN_LOOP_END; and convert them back    into NOTEs.  LAST is the last instruction output by the instruction    scheduler.  Return the new value of LAST.  */
end_comment
...
name|insn
expr_stmt|;
comment|/* Check to see if we need to re-emit any notes here.  */
name|last
operator|=
name|reemit_notes
argument_list|(
