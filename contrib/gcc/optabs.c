...
begin_comment
comment|/* Expand the basic unary and binary arithmetic operations, for GNU compiler.    Copyright (C) 1987, 1988, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
condition|)
block|{
comment|/* Load duplicate non-volatile operands once.  */
if|if
condition|(
name|rtx_equal_p
argument_list|(
...
expr_stmt|;
block|}
comment|/* Open-code the vector operations if we have no hardware support      for them.  */
if|if
condition|(
name|class
operator|==
...
begin_comment
comment|/* Like expand_binop, but for open-coding vectors binops.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Search for the widest vector mode with the same inner mode that is      still narrower than MODE and that allows to open-code this operator.      Note, if we find such a mode and the handler later decides it can't      do the expansion, we'll be called recursively with the narrower mode.  */
for|for
control|(
name|tmode
operator|=
...
name|subsize
expr_stmt|;
comment|/* If METHODS is OPTAB_DIRECT, we don't insist on the exact mode, 	 but that we operate on more than one element at a time.  */
if|if
condition|(
name|subsize
operator|==
...
argument_list|()
expr_stmt|;
comment|/* Errors can leave us with a const0_rtx as operand.  */
if|if
condition|(
name|GET_MODE
argument_list|(
...
control|)
block|{
comment|/* If this is part of a register, and not the first item in the 	     word, we can't store using a SUBREG - that would clobber 	     previous results. 	     And storing with a SUBREG is only possible for the least 	     significant part, hence we can't do it for big endian 	     (unless we want to permute the evaluation order.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Like expand_unop but for open-coding vector unops.  */
end_comment
...
expr_stmt|;
block|}
comment|/* If there is no negate operation, try doing a subtract from zero.  */
if|if
condition|(
name|unoptab
operator|==
...
operator|==
name|MODE_INT
comment|/* Avoid infinite recursion when an 	 error has left us with the wrong mode.  */
operator|&&
name|GET_MODE
argument_list|(
name|op0
...
return|return;
block|}
comment|/* Otherwise call a library function, memcmp if we've got it, 	 bcmp otherwise.  */
ifdef|#
directive|ifdef
...
begin_comment
comment|/* These functions attempt to generate an insn body, rather than    emitting the insn, but if the gen function already emits them, we    make no attempt to turn them back into naked patterns.     They do not protect from queued increments,    because they may be used 1) in protect_from_queue itself    and 2) in other passes where there is no queue.  */
end_comment
