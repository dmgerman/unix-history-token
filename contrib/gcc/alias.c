...
parameter_list|)
define|\
value|(reg_base_value&& REGNO (X)< VARRAY_SIZE (reg_base_value) \    ? VARRAY_RTX (reg_base_value, REGNO (X)) : 0)
end_define
...
name|GTY
argument_list|(
argument|(param_is (struct alias_set_entry))
argument_list|)
name|varray_type
name|alias_sets
expr_stmt|;
...
begin_comment
comment|/* Returns nonzero if a write to X might alias a previous read from    (or, if WRITEP is nonzero, a write to) MEM.  If CONSTP is nonzero,    honor the RTX_UNCHANGING_P flags on X and MEM.  */
end_comment
...
begin_comment
comment|/* Unchanging anti dependence: Like anti_dependence but ignores    the UNCHANGING_RTX_P property on const variable references.  */
end_comment
...
begin_comment
comment|/* Mark the function if it is pure or constant.  */
end_comment
...
begin_comment
comment|/* Return true when INSN possibly modify memory contents of MEM    (ie address can be modified).  */
end_comment
...
name|old_reg_base_value
expr_stmt|;
comment|/* If varray gets large zeroing cost may get important.  */
if|if
condition|(
name|VARRAY_SIZE
argument_list|(
