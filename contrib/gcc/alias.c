...
name|alias_set
decl_stmt|;
comment|/* The children of the alias set.  These are not just the immediate      children, but, in fact, all descendants.  So, if we have:         struct T { struct S s; float f; }       continuing our example above, the children here will be all of      `int', `double', `float', and `struct S'.  */
name|splay_tree
name|GTY
argument_list|(
operator|(
...
begin_comment
comment|/* We preserve the copy of old array around to avoid amount of garbage    produced.  About 8% of garbage produced were attributed to this    array.  */
end_comment
...
parameter_list|)
define|\
value|(REGNO (X)< VEC_length (rtx, reg_base_value)		\    ? VEC_index (rtx, reg_base_value, REGNO (X)) : 0)
end_define
...
begin_comment
comment|/* Return 1 if the two specified alias sets might conflict, or if any subtype    of these alias sets might conflict.  */
end_comment
...
literal|0
return|;
comment|/* If this is a declaration, return it.  If T is based on a restrict      qualified decl, return that decl.  */
if|if
condition|(
name|DECL_P
argument_list|(
...
condition|)
block|{
comment|/* If we're at the end, it vacuously uses its own alias set.  */
if|if
condition|(
operator|!
...
break|break;
default|default:
comment|/* Bitfields and casts are never addressable.  */
return|return
name|true
return|;
block|}
...
name|set
return|;
comment|/* First see if the actual object referenced is an INDIRECT_REF from a 	 restrict-qualified pointer or a "void *".  */
while|while
condition|(
name|handled_component_p
argument_list|(
...
return|;
block|}
comment|/* If we have an INDIRECT_REF via a void pointer, we don't 	     know anything about what that might alias.  Likewise if the 	     pointer is marked that way.  */
elseif|else
if|if
condition|(
name|TREE_CODE
...
return|;
block|}
comment|/* Otherwise, pick up the outermost object that we could have a pointer 	 to, processing conversions as above.  */
while|while
condition|(
name|component_uses_parent_alias_set
argument_list|(
...
begin_comment
comment|/* Indicate that things in SUBSET can alias things in SUPERSET, but that    not everything that aliases SUPERSET also aliases SUBSET.  For example,    in C, a store to an `int' can alias a load of a structure containing an    `int', and vice versa.  But it can't alias a load of a 'double' member    of the same structure.  Here, the structure would be the SUPERSET and    `int' the SUBSET.  This relationship is also described in the comment at    the beginning of this file.     This function should be called only once per SUPERSET/SUBSET pair.     It is illegal for SUPERSET to be zero; everything is implicitly a    subset of alias set zero.  */
end_comment
...
directive|if
literal|1
comment|/* We now lower VA_ARG_EXPR, and there's currently no way to attach the      varargs alias set to an INDIRECT_REF (FIXME!), so we can't      consistently use the varargs alias set for loads from the varargs      area.  So don't use it anywhere.  */
return|return
literal|0
return|;
else|#
...
argument_list|)
expr_stmt|;
comment|/* If this spans multiple hard registers, then we must indicate that every      register has an unusable value.  */
if|if
condition|(
name|regno
operator|<
...
return|return;
block|}
comment|/* If this is not the first set of REGNO, see whether the new value      is related to the old one.  There are two cases of interest:  	(1) The register might be assigned an entirely new value 	    that has the same base term as the original set.  	(2) The set might be a simple self-modification that 	    cannot change REGNO's base value.       If neither case holds, reject the original base value as invalid.      Note that the following situation is not detected:  	 extern int x, y;  int *p =&x; p += (&y-&x);       ANSI C does not allow computing the difference of addresses      of distinct top level objects.  */
if|if
condition|(
name|new_reg_base_value
index|[
...
begin_comment
comment|/* If a value is known for REGNO, return it.  */
end_comment
...
begin_comment
comment|/* Similarly for reg_known_equiv_p.  */
end_comment
...
begin_comment
comment|/* Return 1 if X and Y are identical-looking rtx's.    Expect that X and Y has been already canonicalized.     We use the data in reg_known_value above to see if two registers with    different numbers are, in fact, equivalent.  */
end_comment
...
break|break;
block|}
comment|/* canon_rtx knows how to handle plus.  No need to canonicalize.  */
if|if
condition|(
name|code
operator|==
...
name|AND
condition|)
comment|/* If the address is an AND, it's very hard to know at what it is        actually pointing.  */
return|return
literal|1
return|;
return|return
...
begin_comment
comment|/* Return nonzero if we can determine the exprs corresponding to memrefs    X and Y and they do not overlap.  */
end_comment
...
literal|0
return|;
comment|/* Read-only memory is by definition never modified, and therefore can't      conflict with anything.  We don't expect to find read-only set on MEM,      but stupid user tricks can produce them, so don't die.  */
if|if
condition|(
name|MEM_READONLY_P
argument_list|(
...
literal|0
return|;
comment|/* A read from read-only memory can't conflict with read-write memory.  */
if|if
condition|(
operator|!
name|writep
...
begin_comment
comment|/* Set MEMORY_MODIFIED when X modifies DATA (that is assumed    to be memory reference.  */
end_comment

begin_decl_stmt
specifier|static
name|bool
name|memory_modified
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Return true when INSN possibly modify memory contents of MEM    (i.e. address can be modified).  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If we have memory allocated from the previous run, use it.  */
if|if
condition|(
name|old_reg_base_value
condition|)
