...
argument_list|)
expr_stmt|;
comment|/* If this class doesn't want a group, determine if we have 		     a nongroup need or a regular need.  We have a nongroup 		     need if this reload conflicts with a group reload whose 		     class intersects with this reload's class.  */
name|nongroup_need
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* See if this register would conflict with any reload 		     that needs a group.  */
name|int
name|nongroup_need
init|=
literal|0
...
name|RELOAD_OTHER
case|:
comment|/* In use for anything except RELOAD_FOR_OTHER_ADDRESS means 	 we can't use it for RELOAD_OTHER.  */
if|if
condition|(
name|TEST_HARD_REG_BIT
argument_list|(
...
index|]
decl_stmt|;
comment|/* RELOAD_OTHER conflicts with everything except RELOAD_FOR_OTHER_ADDRESS. */
if|if
condition|(
name|r2_type
operator|==
