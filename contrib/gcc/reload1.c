...
argument_list|)
expr_stmt|;
comment|/* Look for REG_EQUIV notes; record what each pseudo is equivalent to.      Also find all paradoxical subregs and find largest such for each pseudo.      On machines with small register classes, record hard registers that      are used for user variables.  These can never be used for spills.  */
name|num_eliminable_invariants
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* It can happen that a REG_EQUIV note contains a MEM 		     that is not a legitimate memory operand.  As later 		     stages of reload assume that all addresses found 		     in the reg_equiv_* arrays were originally legitimate, 		     we ignore such REG_EQUIV notes.  		     It also can happen that a REG_EQUIV note contains a MEM 		     that carries the /u flag, for example when GCSE turns 		     the load of a constant into a move from a pseudo that 		     already contains the constant and attaches a REG_EQUAL 		     note to the insn, which is later promoted to REQ_EQUIV 		     by local-alloc.  If the destination pseudo happens not 		     to be assigned to a hard reg, it will be replaced by 		     the MEM as the destination of the move, thus generating 		     a store to a possibly read-only memory location.  */
if|if
condition|(
name|memory_operand
argument_list|(
