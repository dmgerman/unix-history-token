...
begin_comment
comment|/* Nonzero if the machine description has been fixed to accept    CONSTANT_P_RTX patterns.  We will emit a warning and continue    if we find we must actually use such a beast.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|can_handle_constant_p
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Don't check memory usage, since code is being emitted to check a memory    usage.  Used when flag_check_memory_usage is true, to avoid infinite    recursion.  */
end_comment
...
block|}
block|}
comment|/* Find out if CONSTANT_P_RTX is accepted.  */
name|SET_DEST
argument_list|(
name|pat
argument_list|)
...
expr_stmt|;
block|}
comment|/* Only check automatic variables.  Currently, function arguments are          not checked (this can be done at compile-time with prototypes).          Aggregates are not checked.  */
if|if
condition|(
name|flag_check_memory_usage
operator|&&
...
literal|1
expr_stmt|;
comment|/* If the pointer is actually a REFERENCE_TYPE, this could be pointing 	   into some aggregate too.  In theory we could fold this into the 	   previous check and use rtx_addr_varies_p there too.  	   However, this seems safer.  */
if|if
condition|(
operator|!
name|MEM_IN_STRUCT_P
...
operator|==
name|REFERENCE_TYPE
comment|/* This may have been an array reference to the first element 		   that was optimized away from being an addition.  */
operator|||
operator|(
name|TREE_CODE
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* We store the frame pointer and the address of lab1 in the buffer      and use the rest of it for the stack save area, which is      machine-dependent.  */
ifndef|#
directive|ifndef
...
directive|endif
comment|/* Set TARGET to zero and branch to the first-time-through label.  */
name|emit_move_insn
argument_list|(
name|target
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Tell flow about the strange goings on.  */
...
comment|/* Nothing */
block|}
comment|/* Set TARGET, and branch to the next-time-through label.  */
name|emit_move_insn
argument_list|(
name|target
argument_list|,
...
name|const1_rtx
return|;
comment|/* Only emit CONSTANT_P_RTX if CSE will be run.  	     Moreover, we don't want to expand trees that have side effects, 	     as the original __builtin_constant_p did not evaluate its       	     argument at all, and we would break existing usage by changing  	     this.  This quirk was generally useful, eliminating a bit of hair 	     in the writing of the macros that use this function.  Now the     	     same thing can be better accomplished in an inline function.  */
if|if
condition|(
operator|!
name|cse_not_expected
...
condition|)
block|{
comment|/* Lazy fixup of old code: issue a warning and fail the test.  */
if|if
condition|(
operator|!
name|can_handle_constant_p
...
name|warning
argument_list|(
literal|"Delayed evaluation of __builtin_constant_p not supported on this target."
argument_list|)
expr_stmt|;
name|warning
argument_list|(
literal|"Please report this as a bug to egcs-bugs@egcs.cygnus.com."
argument_list|)
expr_stmt|;
return|return
name|const0_rtx
