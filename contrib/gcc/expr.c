...
name|INTEGER_CST
case|:
comment|/* If the integer is expressable in a HOST_WIDE_INT, we can find the 	 lowest bit that's a one.  If the result is zero, return 	 BIGGEST_ALIGNMENT.  We need to handle this case since we can find it 	 in a COND_EXPR, a MIN_EXPR, or a MAX_EXPR.  If the constant overlows, 	 we have an erroneous program, so return BIGGEST_ALIGNMENT to avoid any 	 later ICE.  */
if|if
condition|(
