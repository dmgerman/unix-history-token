...
begin_comment
comment|/* Convert defined/undefined to boolean.  */
end_comment
...
specifier|static
name|bool
name|emit_block_move_via_movstr
parameter_list|(
name|rtx
parameter_list|,
name|rtx
...
parameter_list|)
define|\
value|(move_by_pieces_ninsns (SIZE, ALIGN)< (unsigned int) CLEAR_RATIO)
end_define
...
begin_comment
comment|/* Retrieve a mark on the queue.  */
end_comment
...
begin_comment
comment|/* Perform all the pending incrementations that have been enqueued    after MARK was retrieved.  If MARK is null, perform all the    pending incrementations.  */
end_comment

begin_function
specifier|static
name|void
name|emit_insns_enqueued_after_mark
parameter_list|(
name|rtx
name|mark
parameter_list|)
...
name|p
decl_stmt|;
comment|/* The marked incrementation may have been emitted in the meantime      through a call to emit_queue.  In this case, the mark is not valid      anymore so do nothing.  */
if|if
condition|(
name|mark
operator|&&
...
begin_comment
comment|/* A subroutine of emit_block_move.  Expand a movstr pattern;    return true if successful.  */
end_comment
...
begin_comment
comment|/* A subroutine of emit_block_move.  Expand a call to memcpy or bcopy.    Return the return value from memcpy, 0 otherwise.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* It is incorrect to use the libcall calling conventions to call      memcpy in this context.  This could be a user call to memcpy and      the user may wish to examine the return value from memcpy.  For      targets where libcalls and normal calls have different conventions      for returning pointers, we could end up generating incorrect code.       For convenience, we generate the call to bcopy this way as well.  */
if|if
condition|(
name|TARGET_MEM_FUNCTIONS
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If we are initializing a readonly value, show the above call clobbered      it. Otherwise, a load from it may erroneously be hoisted from a loop, or      the delay slot scheduler might overlook conflicts and take nasty      decisions.  */
if|if
condition|(
...
begin_comment
comment|/* Emit code to move a block ORIG_SRC of type TYPE to a block DST,    where DST is non-consecutive registers represented by a PARALLEL.    SSIZE represents the total size of block ORIG_SRC in bytes, or -1    if not known.  */
end_comment
...
begin_comment
comment|/* A subroutine of clear_storage.  Expand a clrstr pattern;    return true if successful.  */
end_comment
...
begin_comment
comment|/* A subroutine of clear_storage.  Expand a call to memset or bzero.    Return the return value of memset, 0 otherwise.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* It is incorrect to use the libcall calling conventions to call      memset in this context.  This could be a user call to memset and      the user may wish to examine the return value from memset.  For      targets where libcalls and normal calls have different conventions      for returning pointers, we could end up generating incorrect code.       For convenience, we generate the call to bzero this way as well.  */
name|object_tree
operator|=
name|make_tree
argument_list|(
...
argument_list|)
return|;
comment|/* This will handle any multi-word or full-word mode that lacks a move_insn      pattern.  However, you will get better code if you define such patterns,      even if they must turn into multiple assembler instructions.  */
elseif|else
if|if
condition|(
name|GET_MODE_SIZE
...
condition|)
block|{
comment|/* Use the size of the elt to compute offset.  */
name|rtx
name|elt
init|=
name|XEXP
...
begin_comment
comment|/* Expand an assignment that stores the value of FROM into TO.    If WANT_VALUE is nonzero, return an rtx for the value of TO.    (This may contain a QUEUED rtx;    if the value is constant, this rtx is a constant.)    Otherwise, the returned value is NULL_RTX.  */
end_comment
...
argument_list|)
argument_list|)
comment|/* We can't assert that a MEM won't be set more than once 	     if the component is not addressable because another 	     non-addressable component may be referenced by the same MEM.  */
operator|&&
operator|!
operator|(
name|GET_CODE
...
argument_list|)
expr_stmt|;
comment|/* If TEMP is a MEM and we want a result value, make the access 	 now so it gets done only once.  Strictly speaking, this is 	 only necessary if the MEM is volatile, or if the address 	 overlaps TARGET.  But not performing the load twice also 	 reduces the amount of rtl we generate and then have to CSE.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
operator|)
comment|/* If there's nothing to copy, don't bother.  Don't call expr_size 	 unless necessary, because some front-ends (C++) expr_size-hook 	 aborts on objects that are not supposed to be bit-copied or 	 bit-initialized.  */
operator|&&
name|expr_size
argument_list|(
name|exp
...
begin_comment
comment|/* Return 1 if EXP just contains zeros.  FIXME merge with initializer_zerop.  */
end_comment
...
condition|)
block|{
comment|/* ??? Emit a blockage to prevent the scheduler from 			 swapping the memory write issued above without the 			 /u flag and memory writes that will be issued later 			 with it.  */
name|emit_insn
argument_list|(
name|gen_rtx_ASM_INPUT
argument_list|(
...
continue|continue;
block|}
comment|/* We can go inside most conversions: all NON_VALUE_EXPRs, all normal 	 conversions that don't change the mode, and all view conversions 	 except those that need to "step up" the alignment.  */
elseif|else
if|if
condition|(
name|TREE_CODE
...
literal|1
return|;
comment|/* ??? Sure they are handled, but get_inner_reference may return        a different PBITSIZE, depending upon whether the expression is        wrapped up in a NOP_EXPR or not, e.g. for bitfields.  */
case|case
name|NOP_EXPR
case|:
case|case
...
argument_list|)
decl_stmt|;
comment|/* Labels in containing functions, or labels used from initializers, 	   must be forced.  */
if|if
condition|(
name|modifier
operator|==
...
name|error
argument_list|(
literal|"%Jprior parameter's size depends on '%D'"
argument_list|,
name|exp
argument_list|,
name|exp
...
argument_list|)
expr_stmt|;
comment|/* If the mode of TEMP does not match that of the expression, it 	     must be a promoted value.  We pass store_expr a SUBREG of the 	     wanted mode but mark it so that we know that it was already 	     extended.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Otherwise, if this object not in memory and we either have an  	   offset or a BLKmode result, put it there.  This case can't occur in  	   C, but can in Ada if we have unchecked conversion of an expression  	   from a scalar type to an array or record type or for an  	   ARRAY_RANGE_REF whose type is BLKmode.  */
elseif|else
if|if
condition|(
name|GET_CODE
...
argument_list|)
expr_stmt|;
comment|/* If the input and output modes are both the same, we are done. 	 Otherwise, if neither mode is BLKmode and both are integral and within 	 a word, we can use gen_lowpart.  If neither is true, make sure the 	 operand is in memory and convert the MEM to the new mode.  */
if|if
condition|(
name|TYPE_MODE
argument_list|(
...
operator|)
decl_stmt|;
comment|/* If we had X ? A : A + 1, do this as A + (X == 0).  */
if|if
condition|(
name|singleton
operator|==
...
condition|)
block|{
comment|/* We can't make a bitwise copy of this object, so fail.  */
name|error
argument_list|(
literal|"cannot take the address of an unaligned member"
argument_list|)
expr_stmt|;
return|return
name|const0_rtx
...
condition|)
block|{
comment|/* In this case, wrapping FINALLY_BLOCK in an UNSAVE_EXPR 	       is not sufficient, so we cannot expand the block twice. 	       So we play games with GOTO_SUBROUTINE_EXPR to let us 	       expand the thing only once.  */
comment|/* When not optimizing, we go ahead with this form since 	       (1) user breakpoints operate more predictably without 		   code duplication, and 	       (2) we're not running any of the global optimizers 	           that would explode in time/space with the highly 		   connected CFG created by the indirect branching.  */
name|rtx
name|finally_label
init|=
name|gen_label_rtx
...
condition|)
block|{
comment|/* We have a true reference to the value in OP0. 	 If there is an insn to add or subtract in this mode, queue it. 	 Queuing the increment insn avoids the register shuffling 	 that often results if we must increment now and first save 	 the old value for subsequent use.  */
if|#
directive|if
literal|0
