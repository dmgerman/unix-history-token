...
block|{
comment|/* Show the output dies here.  This is necessary for pseudos; 	     hard regs shouldn't appear here except as return values. 	     We never want to emit such a clobber after reload.  */
if|if
condition|(
name|x
operator|!=
...
argument_list|)
expr_stmt|;
comment|/* Tell flow about the strange goings on.  Putting `lab1' on      `nonlocal_goto_handler_labels' to indicates that function      calls may traverse the arc back to this label.  */
