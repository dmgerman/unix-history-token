...
literal|'e'
operator|)
comment|/* ...and is unsigned, and its type is smaller than ctype, 	     then we cannot pass through this widening.  */
operator|&&
operator|(
operator|(
name|TREE_UNSIGNED
...
operator|)
operator|)
comment|/* ...and its type is larger than ctype, 		 then we cannot pass through this truncation.  */
operator|||
operator|(
name|GET_MODE_SIZE
argument_list|(
...
block|}
block|}
comment|/* Change X>= CST to X> (CST - 1) and X< CST to X<= (CST - 1) 	 if CST is positive.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
