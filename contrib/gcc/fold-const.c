...
comment|/* Since ll_arg is a single bit bit mask, we can sign extend 	     it appropriately with a NEGATE_EXPR. 	     l_const is made a signed value here, but since for l_const != NULL 	     lr_unsignedp is not used, we don't need to clear the latter.  */
name|l_const
operator|=
name|fold
argument_list|(
...
name|warning
argument_list|(
literal|"comparison is always %s"
argument_list|,
name|wanted_code
operator|==
name|NE_EXPR
