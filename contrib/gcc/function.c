...
specifier|static
name|void
name|maybe_mark_struct_function
name|PARAMS
argument_list|(
operator|(
name|void
...
begin_comment
comment|/* Save the current context for compilation of a nested function.    This is called from language-specific code.  The caller should use    the save_lang_status callback to save any language-specific state,    since this function knows only about language-independent    variables.  */
end_comment
...
begin_comment
unit|void push_temp_slots_for_block () {   push_temp_slots ();    var_temp_slot_level = temp_slot_level; }
comment|/* Likewise, but save the new level as the place to allocate temporaries    for TARGET_EXPRs.  */
end_comment

begin_comment
unit|void push_temp_slots_for_target () {   push_temp_slots ();    target_temp_slot_level = temp_slot_level; }
...
begin_endif
unit|int get_target_temp_slot_level () {   return target_temp_slot_level; }  void set_target_temp_slot_level (level)      int level; {   target_temp_slot_level = level; }
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* Build up a (MEM (ADDRESSOF (REG))) rtx for a register REG that just had its    address taken.  DECL is the decl or SAVE_EXPR for the object stored in the    register, for later use if we do need to force REG into the stack.  REG is    overwritten by the MEM like in put_reg_into_stack.  */
end_comment
...
begin_comment
comment|/* Allocate a function structure and reset its contents to the defaults.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Clear out data used for inlining.  */
name|cfun
operator|->
name|inlinable
operator|=
...
name|GEN_CALL__MAIN
define|\
value|do {									\     emit_library_call (gen_rtx_SYMBOL_REF (Pmode, NAME__MAIN), LCT_NORMAL, \ 		       VOIDmode, 0);					\   } while (0)
end_define
...
argument_list|)
decl_stmt|;
comment|/* Structures that are returned in registers are not aggregate_value_p, 	 so we may see a PARALLEL.  Don't play pseudo games with this.  */
if|if
condition|(
operator|!
name|REG_P
...
comment|/* Either ``-W -Wunused'' or ``-Wunused-parameter'' enables this      warning.  WARN_UNUSED_PARAMETER is negative when set by      -Wunused.  */
if|if
condition|(
...
begin_comment
comment|/* Extend a vector that records the INSN_UIDs of INSNS (either a    sequence or a single insn).  */
end_comment
...
begin_comment
comment|/* Modify SEQ, a SEQUENCE that is part of the epilogue, to no modifications    to the stack pointer.  Return the new sequence.  */
end_comment
...
name|info
decl_stmt|;
comment|/* If the epilogue is just a single instruction, it ust be OK as is.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Retain a map of the epilogue insns.  Used in life analysis to 	 avoid getting rid of sibcall epilogue insns.  */
name|record_insns
argument_list|(
name|GET_CODE
argument_list|(
...
name|prev
decl_stmt|;
comment|/* GDB handles `break f' by setting a breakpoint on the first 	 line note after the prologue.  Which means (1) that if 	 there are line number notes before where we inserted the 	 prologue we should move them, and (2) we should generate a 	 note before the end of the first basic block, if there isn't 	 one already there.  	 ??? This behaviour is completely broken when dealing with 	 multiple entry functions.  We simply place the note always 	 into first basic block and let alternate entry points 	 to be missed.        */
for|for
control|(
name|insn
operator|=
...
name|next
decl_stmt|;
comment|/* Similarly, move any line notes that appear after the epilogue.          There is no need, however, to be quite so anal about the existence 	 of such a note.  */
for|for
control|(
name|insn
operator|=
...
begin_comment
comment|/* Mark the struct function pointed to by *ARG for GC, if it is not    NULL.  This is used to mark the current function and the outer    function chain.  */
end_comment
...
begin_comment
comment|/* Mark a struct function * for GC.  This is called from ggc-common.c.  */
end_comment
