...
argument_list|)
comment|/* When REG_PARM_STACK_SPACE is nonzero, stack space for 		 split parameters was allocated by our caller, so we 		 won't be pushing it in the prolog.  */
if|if
condition|(
name|REG_PARM_STACK_SPACE
argument_list|(
...
begin_comment
comment|/* Compute the size and offset from the start of the stacked arguments for a    parm passed in mode PASSED_MODE and with type TYPE.     INITIAL_OFFSET_PTR points to the current offset into the stacked    arguments.     The starting offset and size for this parm are returned in *OFFSET_PTR    and *ARG_SIZE_PTR, respectively.     IN_REGS is nonzero if the argument will be passed in registers.  It will    never be set if REG_PARM_STACK_SPACE is not defined.     FNDECL is the function in which the argument was defined.     There are two types of rounding that are done.  The first, controlled by    FUNCTION_ARG_BOUNDARY, forces the offset from the start of the argument    list to be aligned to the specific boundary (in bits).  This rounding    affects the initial and starting offsets, but not the argument size.     The second, controlled by FUNCTION_ARG_PADDING and PARM_BOUNDARY,    optionally rounds the size of the parm to PARM_BOUNDARY.  The    initial offset is not affected by this rounding, while the size always    is and the starting offset may be.  */
end_comment
...
operator|)
expr_stmt|;
comment|/* Pad_below needs the pre-rounded size to know how much to pad below.      We only pad parameters which are not in registers as they have their      padding done elsewhere.  */
if|if
condition|(
name|where_pad
operator|==
...
literal|0
expr_stmt|;
comment|/* By default assume not stdarg.  */
name|current_function_stdarg
operator|=
literal|0
expr_stmt|;
