...
begin_comment
comment|/* Nonzero if memory access checking be enabled in the current function.  */
end_comment
...
name|void
argument_list|(
argument|*restore_machine_status
argument_list|)
end_macro
...
block|{
comment|/* The KEY in HE will be a MEM.  */
name|struct
name|hash_entry
name|he
decl_stmt|;
...
begin_comment
comment|/* HAVE_prologue || HAVE_epilogue */
end_comment
...
operator|==
name|BLKmode
comment|/* We can't split slots if -fstrict-aliasing because the 	     information about the alias set for the new slot will be 	     lost.  */
operator|&&
operator|!
name|flag_strict_aliasing
condition|)
...
expr_stmt|;
block|}
comment|/* We may be reusing an old slot, so clear any MEM flags that may have been      set from before.  */
...
begin_comment
comment|/* Assign a temporary of given TYPE.    KEEP is as for assign_stack_temp.    MEMORY_REQUIRED is 1 if the result must be addressable stack memory;    it is 0 if a register is OK.    DONT_PROMOTE is 1 if we should not promote values in register    to wider modes.  */
end_comment
...
condition|)
return|return;
comment|/* If there are a lot of temp slots, don't do anything unless       high levels of optimizaton.  */
if|if
condition|(
operator|!
...
begin_comment
comment|/* Likewise, but save the new level as the place to allocate variables    for blocks.  */
end_comment

begin_function
name|void
name|push_temp_slots_for_block
parameter_list|()
block|{
name|push_temp_slots
argument_list|()
...
begin_comment
comment|/* Likewise, but save the new level as the place to allocate temporaries    for TARGET_EXPRs.  */
end_comment

begin_function
name|void
name|push_temp_slots_for_target
parameter_list|()
block|{
name|push_temp_slots
argument_list|()
...
begin_comment
comment|/* Set and get the value of target_temp_slot_level.  The only    permitted use of these functions is to save and restore this value.  */
end_comment

begin_function
name|int
name|get_target_temp_slot_level
parameter_list|()
block|{
return|return
name|target_temp_slot_level
...
begin_function
name|void
name|set_target_temp_slot_level
parameter_list|(
name|level
parameter_list|)
name|int
...
begin_comment
comment|/* Subroutine of put_var_into_stack.  This puts a single pseudo reg REG    into the stack frame of FUNCTION (0 means the current function).    DECL_MODE is the machine mode of the user-level data type.    PROMOTED_MODE is the machine mode of the register.    VOLATILE_P is nonzero if this is for a "volatile" decl.    USED_P is nonzero if this reg might have already been used in an insn.  */
end_comment
...
block|}
block|}
comment|/* Scan the catch clauses for exception handling too.  */
name|push_to_sequence
argument_list|(
name|catch_clauses
argument_list|)
...
name|NULL_RTX
decl_stmt|;
comment|/* If we already know which INSNs reference VAR there's no need      to walk the entire instruction chain.  */
if|if
condition|(
name|ht
condition|)
...
begin_comment
comment|/* Given X, an rtx of the form (SUBREG:m1 (MEM:m2 addr)),    return an rtx (MEM:m1 newaddr) which is equivalent.    If any insns must be emitted to compute NEWADDR, put them before INSN.     UNCRITICAL nonzero means accept paradoxical subregs.    This is used for subregs found inside REG_NOTES.  */
end_comment
...
begin_comment
comment|/* On a few machines, the CFA coincides with the arg pointer.  */
end_comment
...
begin_comment
comment|/* Build up a (MEM (ADDRESSOF (REG))) rtx for a register REG that just had    its address taken.  DECL is the decl for the object stored in the    register, for later use if we do need to force REG into the stack.    REG is overwritten by the MEM like in put_reg_into_stack.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If the original REG was a user-variable, then so is the REG whose      address is being taken.  */
name|REG_USERVAR_P
argument_list|(
name|XEXP
argument_list|(
...
label|:
empty_stmt|;
comment|/* else give up and put it into the stack */
block|}
elseif|else
if|if
condition|(
...
begin_comment
comment|/* Return a new hash table entry in HT.  */
end_comment
...
decl_stmt|;
block|{
comment|/* K is really a RTX.  Just use the address as the hash value.  */
return|return
operator|(
name|unsigned
name|long
...
begin_comment
comment|/* Return non-zero if K1 and K2 (two REGs) are the same.  */
end_comment
...
name|ht
decl_stmt|;
comment|/* The INSN we are currently proessing.  */
name|rtx
name|insn
decl_stmt|;
...
condition|)
block|{
comment|/* Lookup this MEM in the hashtable, creating it if necessary.  */
name|struct
name|insns_for_mem_entry
modifier|*
name|ifme
...
condition|)
block|{
comment|/* We do the allocation on the same obstack as is used for 	     the hash table since this memory will not be used once 	     the hash table is deallocated.  */
name|push_obstacks
argument_list|(
operator|&
name|ifmwi
...
name|pretend_named
decl_stmt|;
comment|/* Set LAST_NAMED if this is last named arg before some 	 anonymous args.  */
name|int
name|last_named
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|/* If this is a memory ref that contains aggregate components, 	     mark it as such for cse and loop optimize.  Likewise if it 	     is readonly.  */
name|MEM_SET_IN_STRUCT_P
argument_list|(
name|stack_parm
argument_list|,
...
block|if (nominal_mode != BLKmode&& nominal_mode != passed_mode&& stack_parm != 0) 	{ 	  rtx offset_rtx;  	  if (BYTES_BIG_ENDIAN&& GET_MODE_SIZE (nominal_mode)< UNITS_PER_WORD) 	    stack_offset.constant += (GET_MODE_SIZE (passed_mode) 				      - GET_MODE_SIZE (nominal_mode));  	  offset_rtx = ARGS_SIZE_RTX (stack_offset); 	  if (offset_rtx == const0_rtx) 	    stack_parm = gen_rtx_MEM (nominal_mode, internal_arg_pointer); 	  else 	    stack_parm = gen_rtx_MEM (nominal_mode, 				      gen_rtx_PLUS (Pmode, 						    internal_arg_pointer, 						    offset_rtx));
comment|/* If this is a memory ref that contains aggregate components, 	     mark it as such for cse and loop optimize.  */
block|MEM_SET_IN_STRUCT_P (stack_parm, aggregate); 	}
endif|#
directive|endif
comment|/* 0 */
ifdef|#
...
operator|==
name|REG
comment|/* Global optimizations can make it difficult to determine if a 	     particular variable has been initialized.  However, a VAR_DECL 	     with a nonzero DECL_INITIAL had an initializer, so do not 	     claim it is potentially uninitialized.  	     We do not care about the actual value in DECL_INITIAL, so we do 	     not worry that it may be a dangling pointer.  */
operator|&&
name|DECL_INITIAL
argument_list|(
name|decl
...
name|block
return|;
comment|/* Prune the old trees away, so that it doesn't get in the way.  */
name|BLOCK_SUBBLOCKS
argument_list|(
name|current_block
argument_list|)
...
begin_comment
comment|/* Count the subblocks of the list starting with BLOCK, and list them    all into the vector VECTOR.  Also clear TREE_ASM_WRITTEN in all    blocks.  */
end_comment
...
literal|0
expr_stmt|;
comment|/* Set up to allocate temporaries.  */
name|init_temp_slots
argument_list|()
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|/* Set this before generating any memory accesses.  */
name|current_function_check_memory_usage
operator|=
operator|(
...
condition|)
block|{
comment|/* arg_pointer_save_area may not be a valid memory address, so we 	 have to check it and fix it if necessary.  */
name|rtx
name|seq
decl_stmt|;
name|start_sequence
...
argument_list|)
expr_stmt|;
comment|/* Now handle any leftover exception regions that may have been      created for the parameters.  */
block|{
name|rtx
name|last
init|=
...
argument_list|()
expr_stmt|;
comment|/* If the above emitted any code, may sure we jump around it.  */
if|if
condition|(
name|last
operator|!=
...
literal|1
expr_stmt|;
comment|/* If this is a BLKmode structure being returned in registers, then use 	 the mode computed in expand_return.  */
if|if
condition|(
name|GET_MODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* If this is an implementation of __throw, do what's necessary to       communicate between __builtin_eh_return and the epilogue.  */
...
argument_list|()
expr_stmt|;
comment|/* If optimization is off, and perhaps in an empty function, 	 the entry block will have no successors.  */
if|if
condition|(
...
condition|)
block|{
comment|/* Can't deal with multiple successsors of the entry block.  */
if|if
condition|(
name|ENTRY_BLOCK_PTR
operator|->
...
argument_list|()
decl_stmt|;
comment|/* ??? This is gastly.  If function returns were not done via uses, 	 but via mark_regs_live_at_end, we could use insert_insn_on_edge 	 and all of this uglyness would go away.  */
switch|switch
condition|(
name|optimize
condition|)
block|{
default|default:
comment|/* If the exit block has no non-fake predecessors, we don't 	     need an epilogue.  Furthermore, only pay attention to the 	     fallthru predecessors; if (conditional) return insns were 	     generated, by definition we do not need to emit epilogue 	     insns.  */
for|for
control|(
name|e
operator|=
...
condition|)
break|break;
comment|/* We can't handle multiple epilogues -- if one is needed, 	     we won't be able to place it multiple times.  	     ??? Fix epilogue expanders to not assume they are the 	     last thing done compiling the function.  Either that 	     or copy_rtx each insn.  	     ??? Blah, it's not a simple expression to assert that 	     we've exactly one fallthru exit edge.  */
name|bb
operator|=
name|e
operator|->
...
name|end
expr_stmt|;
comment|/* ??? If the last insn of the basic block is a jump, then we 	     are creating a new basic block.  Wimp out and leave these 	     insns outside any block.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* If the end of the block is the use, grab hold of something 		   else so that we emit barriers etc in the right place.  */
if|if
condition|(
name|prev
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Insert the USE insns immediately before the return insn, which 	       must be the last instruction emitted in the sequence.  */
if|if
condition|(
name|first_use
condition|)
...
argument_list|)
expr_stmt|;
comment|/* Update the tail of the basic block.  */
if|if
condition|(
name|bb
condition|)
...
name|prev
decl_stmt|;
comment|/* GDB handles `break f' by setting a breakpoint on the first 	 line note *after* the prologue.  Which means (1) that if 	 there are line number notes before where we inserted the 	 prologue we should move them, and (2) if there is no such 	 note, then we should generate one at the prologue.  */
for|for
control|(
name|insn
operator|=
...
condition|)
block|{
comment|/* Note that we cannot reorder the first insn in the 		 chain, since rest_of_compilation relies on that 		 remaining constant.  Do the next best thing.  */
if|if
condition|(
name|prev
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Whether or not we can depend on BLOCK_HEAD,  		     attempt to keep it up-to-date.  */
if|if
condition|(
name|BLOCK_HEAD
argument_list|(
