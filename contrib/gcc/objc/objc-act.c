...
specifier|static
name|void
name|build_string_class_template
name|PARAMS
argument_list|(
operator|(
name|void
...
define|#
directive|define
name|STRING_OBJECT_GLOBAL_NAME
value|"_NSConstantStringClassReference"
end_define
...
name|error
argument_list|(
literal|"no class name specified as argument to -fconstant-string-class"
argument_list|)
expr_stmt|;
name|constant_string_class_name
operator|=
...
argument_list|)
expr_stmt|;
comment|/* If the underlying ObjC class does not have 			 protocols attached to it, perhaps there are 			 "one-off" protocols attached to the rhs? 			 E.g., 'id<MyProt> foo;'.  */
if|if
condition|(
operator|!
name|rproto_list
...
begin_comment
comment|/* Predefine the following data type:     struct STRING_OBJECT_CLASS_NAME     {      Object isa;      char *cString;      unsigned int length;    }; */
end_comment
...
literal|1
expr_stmt|;
comment|/* We could not properly create NXConstantString in synth_module_prologue,      because that's called before debugging is initialized.  Do it now.  */
if|if
condition|(
name|TYPE_FIELDS
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* The following statement fixes a bug when inheriting instance 	 variables that are declared to be bitfields. finish_struct 	 expects to find the width of the bitfield in DECL_INITIAL.  */
if|if
condition|(
name|DECL_BIT_FIELD
argument_list|(
...
begin_comment
comment|/* Generate a temporary FUNCTION_DECL node to be used in    hack_method_prototype below.  */
end_comment
...
begin_comment
comment|/* Generate the prototypes for protocol methods.  This is used to    generate method encodings for these.     NST_METHODS is the method to generate a _DECL node for TMP_DECL is    a decl node to be used.  This is also where the return value is    given.  */
end_comment
...
name|arg
decl_stmt|;
comment|/* The receiver is 'self' in the context of a class method.  */
if|if
condition|(
...
condition|)
do|;
comment|/* If no class (factory) method was found, check if an _instance_      method of the same name exists in the root class.  This is what      the Objective-C runtime will do.  */
return|return
name|lookup_instance_method_static
argument_list|(
name|root_inter
...
decl_stmt|;
block|{
comment|/* Make sure we copy the leaf ivars in case @defs is used in a local      context.  Otherwise finish_struct will overwrite the layout info      using temporary storage.  */
return|return
name|build_ivar_chain
argument_list|(
name|interface
...
decl_stmt|;
block|{
comment|/* The RECORD_TYPE may in fact be a typedef!  For purposes      of encoding, we need the real underlying enchilada.  */
if|if
condition|(
name|TYPE_MAIN_VARIANT
argument_list|(
...
decl_stmt|;
block|{
comment|/* Create a couple of function_template nodes at most once.  */
if|if
condition|(
operator|!
name|function1_template
...
begin_endif
unit|int lang_report_error_function (decl)       tree decl; {   if (objc_method_context)     {       fprintf (stderr, "In method `%s'\n", 	       IDENTIFIER_POINTER (METHOD_SEL_NAME (objc_method_context)));       return 1;     }    else     return 0; }
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* Adds the tree codes specific to the ObjC/ObjC++ front end to the    list of all tree codes.  */
end_comment
...
begin_comment
comment|/* Add GC roots for variables local to this file.  */
end_comment
