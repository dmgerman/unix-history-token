...
begin_comment
comment|/* Try to find an available hard register of mode MODE at    CURRENT_INSN, matching the register class in CLASS_STR. Registers    that already have bits set in REG_SET will not be considered.     If an appropriate register is available, it will be returned and the    corresponding bit(s) in REG_SET will be set; otherwise, NULL_RTX is    returned.  */
end_comment
