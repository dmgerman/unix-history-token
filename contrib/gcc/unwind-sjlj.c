...
begin_comment
comment|/* The setjmp side is dealt with in the except.c file.  */
end_comment
...
directive|define
name|setjmp
value|setjmp_should_not_be_used_in_this_file
end_define
...
directive|ifndef
name|DONT_USE_BUILTIN_SETJMP
comment|/* This is a crude imitation of the CFA: the saved stack pointer.      This is roughly the CFA of the frame before CONTEXT.  When using the      DWARF-2 unwinder _Unwind_GetCFA returns the CFA of the frame described      by CONTEXT instead; but for DWARF-2 the cleanups associated with      CONTEXT have already been run, and for SJLJ they have not yet been.  */
if|if
condition|(
name|context
operator|->
...
directive|endif
comment|/* Otherwise we're out of luck for now.  */
return|return
operator|(
name|_Unwind_Word
operator|)
