...
begin_comment
comment|/* Nonzero if can_copy_p has been initialized.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Store motion disabled until it is fixed.  */
if|if
condition|(
literal|0
operator|&&
...
begin_comment
comment|/* Top level function to create an expression or assignment hash table.     Expression entries are placed in the hash table if    - they are of the form (set (pseudo-reg) src),    - src is something we want to perform GCSE on,    - none of the operands are subsequently modified in the block     Assignment entries are placed in the hash table if    - they are of the form (set (pseudo-reg) src),    - src is something we want to perform const/copy propagation on,    - none of the operands or target are subsequently modified in the block     Currently src must be a pseudo-reg or a const_int.     F is the first insn.    TABLE is the table computed.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* First substitute in the INSN condition as the SET_SRC of the JUMP,      then substitute that given values in this expanded JUMP.  */
if|if
condition|(
name|setcc
operator|!=
...
condition|)
block|{
comment|/* If we find a case where we can't fix the retval REG_EQUAL notes 	     match the new register, we either have to abandom this replacement 	     or fix delete_trivially_dead_insns to preserve the setting insn, 	     or make it delete the REG_EUAQL note, and fix up all passes that 	     require the REG_EQUAL note there.  */
if|if
condition|(
operator|!
name|adjust_libcall_notes
...
name|NULL
expr_stmt|;
comment|/* Once basic block indices are stable, we should be able 	     to use redirect_edge_and_branch_force instead.  */
name|old_dest
operator|=
name|e
operator|->
...
begin_comment
comment|/* Find basic blocks with more than one predecessor that only contain a    single conditional jump.  If the result of the comparison is known at    compile-time from any incoming edge, redirect that edge to the    appropriate target.  Returns nonzero if a change was made.  */
end_comment
...
block|}
block|}
comment|/* If we bypassed any register setting insns, we inserted a      copy on the redirected edge.  These need to be commited.  */
if|if
condition|(
name|changed
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If the last insn is a jump, insert EXPR in front [taking care to      handle cc0, etc. properly].  Similary we need to care trapping      instructions in presence of non-call exceptions.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Return nonzero if the register operands of expression X are killed    anywhere in basic block BB.  */
end_comment
