...
begin_comment
comment|/* Must precede rtl.h for FFS.  */
end_comment
...
name|next_same_hash
decl_stmt|;
comment|/* List of anticipatable occurrences in basic blocks in the function.      An "anticipatable occurrence" is one that is the first occurrence in the      basic block and the operands are not modified in the basic block prior      to the occurrence.  */
name|struct
name|occr
modifier|*
name|antic_occr
...
specifier|static
name|void
name|compute_cprop_local_properties
name|PROTO
argument_list|(
operator|(
name|void
...
specifier|static
name|void
name|compute_pre_local_properties
name|PROTO
argument_list|(
operator|(
name|void
...
decl_stmt|;
comment|/* It's impossible to construct a correct control flow graph in the      presense of setjmp, so just punt to be safe.  */
if|if
condition|(
name|current_function_calls_setjmp
condition|)
...
expr_stmt|;
block|}
comment|/* Record where pseudo-registers are set.      This data is kept accurate during each pass.      ??? We could also record hard-reg and memory information here      [since it's unchanging], however it is currently done during      hash table computation.  */
name|alloc_reg_set_mem
argument_list|(
name|max_gcse_regno
argument_list|)
...
expr_stmt|;
block|}
comment|/* If we're doing PRE, do one last pass of copy propagation.  */
if|if
condition|(
operator|!
name|optimize_size
...
begin_comment
comment|/* For each register, the cuid of the first/last insn in the block to set it,    or zero if not set.  */
end_comment
...
begin_comment
comment|/* Set the appropriate bit in `rd_gen' [the gen for reaching defs] if the    register set in this insn is not set after this insn in this block.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Check if first/last set in this block for classic gcse,      but not for copy/constant propagation.  */
if|if
condition|(
name|optimize_size
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* Check if first/last set in this block for classic 		 gcse, but not for constant/copy propagation.  */
if|if
condition|(
name|optimize_size
operator|&&
...
begin_comment
comment|/* Dump copy/const propagation data.  */
end_comment
...
name|file
argument_list|,
literal|"CPROP partially locally available sets"
argument_list|,
literal|"BB"
argument_list|,
name|cprop_pavloc
...
name|file
argument_list|,
literal|"CPROP absolutely altered sets"
argument_list|,
literal|"BB"
argument_list|,
name|cprop_absaltered
...
name|file
argument_list|,
literal|"CPROP available incoming sets"
argument_list|,
literal|"BB"
argument_list|,
name|cprop_avin
...
name|file
argument_list|,
literal|"CPROP available outgoing sets"
argument_list|,
literal|"BB"
argument_list|,
name|cprop_avout
...
name|bitmap_index
decl_stmt|;
comment|/* The assignment is absolutely altered if any operand is modified 	     by this block [excluding the assignment itself]. 	     We start by assuming all are transparent [none are killed], and 	     then setting the bits for those that are.  */
name|compute_transp
argument_list|(
name|expr
operator|->
...
argument_list|)
expr_stmt|;
comment|/* The occurrences recorded in avail_occr are exactly those that 	     we want to set to non-zero in PAVLOC.  */
for|for
control|(
name|occr
operator|=
...
literal|0
decl_stmt|;
comment|/* ??? For now only propagate into SETs.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Compute PRE working variables.  */
end_comment
...
begin_comment
comment|/* Global properties (computed from the expression local properties).  */
end_comment

begin_comment
comment|/* Nonzero for expressions that are available on block entry/exit.  */
end_comment
...
begin_comment
comment|/* Nonzero for expressions that are anticipatable on block entry/exit.  */
end_comment
...
begin_comment
comment|/* Nonzero for expressions that are partially available on block entry/exit.  */
end_comment
...
begin_comment
comment|/* Nonzero for expressions that are "placement possible" on block entry/exit.  */
end_comment
...
begin_comment
comment|/* Used while performing PRE to denote which insns are redundant.  */
end_comment
...
name|file
argument_list|,
literal|"PRE locally transparent expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_transp
...
name|file
argument_list|,
literal|"PRE locally available expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_comp
...
name|file
argument_list|,
literal|"PRE locally anticipatable expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_antloc
...
name|file
argument_list|,
literal|"PRE available incoming expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_avin
...
name|file
argument_list|,
literal|"PRE available outgoing expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_avout
...
name|file
argument_list|,
literal|"PRE anticipatable incoming expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_antin
...
name|file
argument_list|,
literal|"PRE anticipatable outgoing expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_antout
...
name|file
argument_list|,
literal|"PRE partially available incoming expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_pavin
...
name|file
argument_list|,
literal|"PRE partially available outgoing expressions"
argument_list|,
literal|"BB"
argument_list|,
name|pre_pavout
...
name|file
argument_list|,
literal|"PRE placement possible on incoming"
argument_list|,
literal|"BB"
argument_list|,
name|pre_ppin
...
name|file
argument_list|,
literal|"PRE placement possible on outgoing"
argument_list|,
literal|"BB"
argument_list|,
name|pre_ppout
...
name|gcse_file
argument_list|,
literal|"antic expr computation: %d passes\n"
argument_list|,
name|passes
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Compute expression partial availability at entrance and exit of    each block.  */
end_comment
...
name|gcse_file
argument_list|,
literal|"partially avail expr computation: %d passes\n"
argument_list|,
name|passes
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Compute "placement possible" information on entrance and exit of    each block.     From Fred Chow's Thesis:    A computation `e' is PP at a point `p' if it is anticipated at `p' and    all the anticipated e's can be rendered redundant by zero or more    insertions at that point and some other points in the procedure, and    these insertions satisfy the conditions that the insertions are always    at points that `e' is anticipated and the first anticipated e's after the    insertions are rendered redundant.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* ??? Inefficient as we set pre_ppin[0] twice, but simple.  */
name|sbitmap_zero
argument_list|(
name|pre_ppin
index|[
...
argument_list|)
expr_stmt|;
comment|/* ??? Inefficient as we set pre_ppout[n_basic_blocks-1] twice, but simple.  */
name|sbitmap_zero
argument_list|(
name|pre_ppout
index|[
...
condition|)
continue|continue;
comment|/* If this is a back edge, propagate info along the back 		     edge to allow for loop invariant code motion.  		     See FOLLOW_BACK_EDGES at the top of this file for a longer 		     discussion about loop invariant code motion in pre.  */
if|if
condition|(
operator|!
name|FOLLOW_BACK_EDGES
...
name|gcse_file
argument_list|,
literal|"placement possible computation: %d passes\n"
argument_list|,
name|passes
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Return non-zero if occurrence OCCR of expression EXPR reaches block BB.     VISITED is a pointer to a working buffer for tracking which BB's have    been visited.  It is NULL for the top-level call.     We treat reaching expressions that go through blocks containing the same    reaching expression as "not reaching".  E.g. if EXPR is generated in blocks    2 and 3, INSN is in block 4, and 2->3->4, we treat the expression in block    2 as not reaching.  The intent is to improve the probability of finding    only one reaching expression and to reduce register lifetimes by picking    the closest such expression.  */
end_comment
...
name|gcse_file
argument_list|,
literal|"PRE: end of bb %d, insn %d, copying expression %d to reg %d\n"
argument_list|,
name|bb
argument_list|,
name|INSN_UID
...
begin_comment
comment|/* Insert partially redundant expressions at the ends of appropriate basic    blocks making them now redundant.  */
end_comment
...
name|size
decl_stmt|;
comment|/* Compute INSERT = PPOUT& (~AVOUT)& (~PPIN | ~TRANSP) for each      expression.  Where INSERT == TRUE, add the expression at the end of      the basic block.  */
name|size
operator|=
name|pre_ppout
index|[
...
operator|!=
literal|0
comment|/* If the basic block isn't reachable, PPOUT will be TRUE. 		     However, we don't want to insert a copy here because the 		     expression may not really be redundant.  So only insert 		     an insn if the expression was deleted.  */
operator|&&
name|index_map
index|[
name|j
...
begin_comment
comment|/* Delete redundant computations.    These are ones that satisy ANTLOC& PPIN.    Deletion is done by changing the insn to copy the `reaching_reg' of    the expression into the result of the SET.  It is left to later passes    (cprop, cse2, flow, combine, regmove) to propagate the copy or eliminate it.     Returns non-zero if a change is made.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* In theory this should never fail since we're creating 		     a reg->reg copy.  		     However, on the x86 some of the movXX patterns actually 		     contain clobbers of scratch regs.  This may cause the 		     insn created by validate_change to not patch any pattern 		     and thus cause validate_change to fail.   */
if|if
condition|(
name|validate_change
argument_list|(
