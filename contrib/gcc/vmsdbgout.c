...
begin_comment
comment|/* This is similar to the default ASM_OUTPUT_ASCII, except that no trailing    newline is produced.  When flag_verbose_asm is asserted, we add commentary    at the end of the line, so we must avoid output of a newline here.  */
end_comment
...
name|tm_gmtoff
expr_stmt|;
comment|/* VMS has multiple file format types.  */
name|rfo
operator|=
name|statbuf
operator|.
...
directive|else
comment|/* Is GMT adjustment an issue with a cross-compiler? */
name|gmtoff
operator|=
literal|0
expr_stmt|;
