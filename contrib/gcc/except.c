...
begin_comment
comment|/* Add a new eh_entry for this function, and base it off of the information    in the EH_ENTRY parameter. A NULL parameter is invalid. The number    returned is an number which uniquely identifies this exception range. */
end_comment
...
name|error
argument_list|(
literal|"Cannot duplicate EH region because new note region already exists"
argument_list|)
expr_stmt|;
name|new_region
operator|=
...
directive|else
block|{
comment|/* A label to continue execution for the no exception case.  */
name|rtx
name|noex
init|=
name|gen_label_rtx
...
expr_stmt|;
block|}
comment|/* We also have to start the handler if we aren't using the new model. */
if|if
condition|(
operator|!
name|flag_new_exceptions
...
begin_comment
comment|/* Given an actual address in addr_tree, set the return address register up    so the epilogue will return to that address.  If the return address is    not in a register, do nothing.  */
end_comment
...
begin_comment
comment|/* Choose two registers for communication between the main body of    __throw and the stub for adjusting the stack pointer.  The first register    is used to pass the address of the exception handler; the second register    is used to pass the stack pointer offset.     For register 1 we use the return value register for a void *.    For register 2 we use the static chain register if it exists and is      different from register 1, otherwise some arbitrary call-clobbered      register.  */
end_comment
...
begin_comment
comment|/* Emit inside of __throw a stub which adjusts the stack pointer and jumps    to the exception handler.  __throw will set up the necessary values    and then return to the stub.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Handler is in fact a pointer to the _eh_context structure, we need       to pick out the handler field (first element), and jump to there,       leaving the pointer to _eh_conext in the same hardware register. */
name|temp
operator|=
name|gen_rtx_MEM
argument_list|(
...
begin_comment
comment|/* Set up the registers for passing the handler address and stack offset    to the stub above.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* These will be used by the stub.  */
name|emit_insn
argument_list|(
name|gen_rtx_USE
argument_list|(
