...
begin_comment
comment|/* Declarations whose debug info was deferred till end of compilation.  */
end_comment
...
argument_list|)
name|varray_type
name|deferred_global_decls
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* The C front end may call sdbout_symbol before sdbout_init runs.    We save all such decls in this list and output them when we get    to sdbout_init.  */
end_comment
...
name|name
decl_stmt|;
comment|/* If we are called before sdbout_init is run, just save the symbol      for later.  */
if|if
condition|(
operator|!
name|sdbout_initialized
...
condition|)
return|return;
comment|/* Don't output intrinsic types.  GAS chokes on SDB .def 	 statements that contain identifiers with embedded spaces 	 (eg "unsigned long").  */
if|if
condition|(
...
literal|12
argument_list|,
literal|"deferred_global_decls"
argument_list|)
expr_stmt|;
comment|/* Emit debug information which was queued by sdbout_symbol before      we got here.  */
name|sdbout_initialized
operator|=
name|true
expr_stmt|;
