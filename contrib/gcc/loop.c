...
name|n_iterations
expr_stmt|;
comment|/* Check for host arithmetic overflow.  */
if|if
condition|(
name|total_incr
operator|/
...
block|}
block|}
comment|/* If we know the BIV is compared at run-time against an       invariant value, and the increment is +/- 1, we may also       be able to prove that the BIV cannot overflow.  */
elseif|else
if|if
condition|(
name|bl
