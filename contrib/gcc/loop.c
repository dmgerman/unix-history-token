...
begin_comment
comment|/* Perform various loop optimizations, including strength reduction.    Copyright (C) 1987, 1988, 1989, 1991, 1992, 1993, 1994, 1995, 1996, 1997,    1998, 1999, 2000, 2001, 2002, 2003, 2004 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Give up the prefetch optimizations once we exceed a given threshold.    It is unlikely that we would be able to optimize something in a loop    with so many detected prefetches.  */
end_comment
...
begin_comment
comment|/* Parameterize some prefetch heuristics so they can be turned on and off    easily for performance testing on new architectures.  These can be    defined in target-dependent files.  */
end_comment
...
literal|1
decl_stmt|;
comment|/* 1 means we copy to a new pseudo and replace 				    the original insn with a copy from that 				    pseudo, rather than deleting it.  */
name|struct
name|movable
modifier|*
name|match
...
begin_endif
unit|static void replace_call_address (rtx, rtx, rtx);
endif|#
directive|endif
end_endif
...
specifier|static
name|bool
name|extension_within_bounds_p
parameter_list|(
specifier|const
name|struct
name|loop
...
begin_comment
comment|/* Find any register references hidden inside X and add them to    the dependency list DEPS.  This is used to look inside CLOBBER (MEM    when checking whether a PARALLEL can be pulled out of a loop.  */
end_comment
...
condition|)
block|{
comment|/* Do not scan past an optimization barrier.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
block|}
block|}
comment|/* For parallels, add any possible uses to the dependencies, as 		 we can't move the insn without resolving them first. 		 MEMs inside CLOBBERs may also reference registers; these 		 count as implicit uses.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
operator|)
condition|)
comment|/* It is unsafe to move the set.  However, it may be OK to 		   move the source into a new pseudo, and substitute a 		   reg-to-reg copy for the original insn.  		   This code used to consider it OK to move a set of a variable 		   which was not created by the user and not used in an exit 		   test. 		   That behavior is incorrect and was removed.  */
name|insert_temp
operator|=
literal|1
expr_stmt|;
...
condition|)
empty_stmt|;
comment|/* Don't move the source and add a reg-to-reg copy: 		 - with -Os (this certainly increases size), 		 - if the mode doesn't support copy operations (obviously), 		 - if the source is already a reg (the motion will gain nothing), 		 - if the source is a legitimate constant (likewise).  */
elseif|else
if|if
condition|(
name|insert_temp
...
argument_list|)
expr_stmt|;
comment|/* Now consider each movable insn to decide whether it is worth moving.      Store 0 in regs->array[I].set_in_loop for each reg I that is moved.       For machines with few registers this increases code size, so do not      move moveables when optimizing for code size on such machines.      (The 18 below is the value for i386.)  */
if|if
condition|(
operator|!
name|optimize_size
...
literal|0
expr_stmt|;
comment|/* Increase the priority of the moving the first insn 	   since it permits the second to be moved as well. 	   Likewise for insns already forced by the first insn.  */
if|if
condition|(
name|m
operator|!=
...
condition|)
block|{
comment|/* Replace the original insn with a move from 			     our newly created temp.  */
name|start_sequence
argument_list|()
expr_stmt|;
name|emit_move_insn
...
begin_comment
unit|static void replace_call_address (rtx x, rtx reg, rtx addr) {   enum rtx_code code;   int i;   const char *fmt;    if (x == 0)     return;   code = GET_CODE (x);   switch (code)     {     case PC:     case CC0:     case CONST_INT:     case CONST_DOUBLE:     case CONST:     case SYMBOL_REF:     case LABEL_REF:     case REG:       return;      case SET:
...
literal|1
expr_stmt|;
comment|/* Calls initializing constant objects have CLOBBER of MEM /u in the 	     attached FUNCTION_USAGE expression list, not accounted for by the 	     code above. We should note these to avoid missing dependencies in 	     later references.  */
block|{
name|rtx
name|fusage_entry
decl_stmt|;
...
begin_comment
comment|/* Like rtx_equal_p, but attempts to swap commutative operands.  This is    important to get some addresses combined.  Later more sophisticated    transformations can be added when necessary.     ??? Same trick with swapping operand is done at several other places.    It can be nice to develop some common way to handle this.  */
end_comment
...
begin_comment
comment|/* Scan X for memory refs and check each memory address    as a possible giv.  INSN is the insn whose pattern X comes from.    NOT_EVERY_ITERATION is 1 if the insn might not be executed during    every loop iteration.  MAYBE_MULTIPLE is 1 if the insn might be executed    more than once in each loop iteration.  */
end_comment
...
name|temp
decl_stmt|;
comment|/* Attempt to prove constantness of the values.  Don't let simplify_rtx      undo the MULT canonicalization that we performed earlier.  */
name|temp
operator|=
name|simplify_rtx
argument_list|(
...
name|SIGN_EXTEND
case|:
comment|/* Ignore this BIV if signed arithmetic overflow is defined.  */
if|if
condition|(
name|flag_wrapv
condition|)
...
return|;
block|}
comment|/* Propagate the MULT expressions to the innermost nodes.  */
elseif|else
if|if
condition|(
name|GET_CODE
...
begin_comment
comment|/* See if BL is monotonic and has a constant per-iteration increment.    Return the increment if so, otherwise return 0.  */
end_comment
...
name|incr
decl_stmt|;
comment|/* Get the total increment and check that it is constant.  */
name|incr
operator|=
...
begin_comment
comment|/* Subroutine of biv_fits_mode_p.  Return true if biv BL, when biased by    BIAS, will never exceed the unsigned range of MODE.  LOOP is the loop    to which the biv belongs and INCR is its per-iteration increment.  */
end_comment
...
name|delta
decl_stmt|;
comment|/* We need to be able to manipulate MODE-size constants.  */
if|if
condition|(
name|HOST_BITS_PER_WIDE_INT
operator|<
...
name|false
return|;
comment|/* The number of loop iterations must be constant.  */
if|if
condition|(
name|LOOP_INFO
argument_list|(
...
name|false
return|;
comment|/* So must the biv's initial value.  */
if|if
condition|(
name|bl
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Make sure that the initial value is within range.  */
if|if
condition|(
name|initial
operator|>
...
name|false
return|;
comment|/* Set up DELTA and SPAN such that the number of iterations * DELTA      (calculated to arbitrary precision) must be<= SPAN.  */
if|if
condition|(
name|incr
operator|<
...
name|incr
expr_stmt|;
comment|/* Handle the special case in which MAXIMUM is the largest 	 unsigned HOST_WIDE_INT and INITIAL is 0.  */
if|if
condition|(
name|maximum
operator|+
...
begin_comment
comment|/* Return true if biv BL will never exceed the bounds of MODE.  LOOP is    the loop to which BL belongs and INCR is its per-iteration increment.    UNSIGNEDP is true if the biv should be treated as unsigned.  */
end_comment
...
name|bias
decl_stmt|;
comment|/* A biv's value will always be limited to its natural mode.      Larger modes will observe the same wrap-around.  */
if|if
condition|(
name|GET_MODE_SIZE
argument_list|(
...
condition|)
block|{
comment|/* If the increment is +1, and the exit test is a<, the BIV          cannot overflow.  (For<=, we have the problematic case that          the comparison value might be the maximum value of the range.)  */
if|if
condition|(
name|incr
operator|==
...
return|;
block|}
comment|/* Likewise for increment -1 and exit test>.  */
if|if
condition|(
name|incr
operator|==
...
begin_comment
comment|/* Given that X is an extension or truncation of BL, return true    if it is unaffected by overflow.  LOOP is the loop to which    BL belongs and INCR is its per-iteration increment.  */
end_comment
...
name|TRUNCATE
case|:
comment|/* We don't know whether this value is being used as signed 	 or unsigned, so check the conditions for both.  */
name|mode
operator|=
name|GET_MODE
argument_list|(
...
name|loop_dump_stream
argument_list|,
literal|"Failed ext dependent giv at %d\n"
argument_list|,
name|INSN_UID
argument_list|(
name|v
...
begin_comment
comment|/* Generate sequence for REG = B * M + A.  B is the initial value of    the basic induction variable, M a multiplicative constant, A an    additive constant and REG the destination register.  */
end_comment
...
begin_comment
comment|/* EMIT code before BEFORE_BB/BEFORE_INSN to set REG = B * M + A.  B    is the initial value of the basic induction variable, M a    multiplicative constant, A an additive constant and REG the    destination register.  */
end_comment
...
begin_comment
comment|/* Emit insns in loop pre-header to set REG = B * M + A.  B is the    initial value of the basic induction variable, M a multiplicative    constant, A an additive constant and REG the destination    register.  */
end_comment
...
begin_comment
comment|/* Emit insns after loop to set REG = B * M + A.  B is the initial    value of the basic induction variable, M a multiplicative constant,    A an additive constant and REG the destination register.  */
end_comment
...
break|break;
block|}
comment|/* Try swapping the comparison to identify a suitable biv.  */
if|if
condition|(
operator|!
name|bl
...
argument_list|)
condition|)
comment|/* An insn that doesn't mention the biv is okay.  */
empty_stmt|;
elseif|else
if|if
condition|(
...
operator|++
expr_stmt|;
comment|/* If we have a separate comparison insn that does more 		 than just set cc0, the result of the comparison might 		 be used outside the loop.  */
name|keep_first_compare
operator|=
operator|(
name|compare_and_branch
...
begin_comment
comment|/* bl->final_value can be either general_operand or PLUS of general_operand    and constant.  Emit sequence of instructions to load it into REG.  */
end_comment
