...
begin_endif
unit|static void check_conflict_numbers PARAMS ((void));
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* Convenience macro, that is cabable of unioning also non-roots.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* XXX for now we don't remember move insns involving any subregs. 	 Those would be difficult to coalesce (we would need to implement 	 handling of all the subwebs in the allocator, including that such 	 subwebs could be source and target of coalesing).  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
literal|1
return|;
comment|/* Now the more difficult part: the same regno is refered, but the 	   sizes of the references or the words differ.  E.g.            (subreg:SI (reg:CDI a) 0) and (subreg:DI (reg:CDI a) 2) do not 	   overlap, wereas the latter overlaps with (subreg:SI (reg:CDI a) 3). 	   */
block|{
name|unsigned
name|HOST_WIDE_INT
name|old_u
...
operator|++
expr_stmt|;
comment|/* We want to be as fast as possible, so explicitely write 	 this loop.  */
for|for
control|(
name|insn
operator|=
...
begin_comment
comment|/* Handle the uses which reach a block end, but were defered due    to it's regno not being mentioned in that block.  This adds the    remaining conflicts and updates also the crosses_call and    spanned_deaths members.  */
end_comment
...
literal|0
decl_stmt|;
comment|/* If there are no defered uses, just return.  */
if|if
condition|(
operator|(
name|first
...
begin_comment
comment|/* Toplevel function for the first part of this file.    Connect web parts, thereby implicitely building webs, and remember    their conflicts.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Distibute the references to their webs.  */
name|init_webs_defs_uses
argument_list|()
expr_stmt|;
...
name|NULL
expr_stmt|;
comment|/* Useless conflicts will be rebuilt completely.  But check 	     for cleanlyness, as the web might have come from the 	     free list.  */
if|if
condition|(
name|bitmap_first_set_bit
argument_list|(
...
begin_endif
unit|static void check_conflict_numbers () {   unsigned int i;   for (i = 0; i< num_webs; i++)     {       struct web *web = ID2WEB (i);       int new_conf = 0 * web->add_hardregs;       struct conflict_link *cl;       for (cl = web->conflict_list; cl; cl = cl->next) 	if (cl->t->type != SELECT&& cl->t->type != COALESCED) 	  new_conf += 1 + cl->t->add_hardregs;       if (web->type != PRECOLORED&& new_conf != web->num_conflicts) 	abort ();     } }
endif|#
directive|endif
end_endif
...
condition|)
continue|continue;
comment|/* A web with only defs and no uses can't be spilled.  Nevertheless 	 it must get a color, as it takes away an register from all webs 	 live at these defs.  So we make it a short web.  */
if|if
condition|(
name|web
operator|->
...
argument_list|)
operator|)
comment|/* Additionally also memrefs to stack-slots are usefull, when 		  we created them ourself.  They might not have set their 		  unchanging flag set, but nevertheless they are stable across 		  the livetime in question.  */
operator|||
operator|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* If we had no real constraints nothing was explicitely 	     allowed, so we allow the whole class (i.e. we make no 	     additional conflicts).  */
name|CLEAR_HARD_REG_SET
argument_list|(
name|conflict
argument_list|)
...
begin_comment
comment|/* Allocates or reallocates most memory for the interference graph and    assiciated structures.  If it reallocates memory (meaning, this is not    the first pass), this also changes some structures to reflect the    additional entries in various array, and the higher number of    defs and uses.  */
end_comment
