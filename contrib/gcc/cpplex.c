...
name|buffer
decl_stmt|;
comment|/* Handle CR-LF and LF-CR.  Most other implementations (e.g. java)      only accept CR-LF; maybe we should fall back to that behavior?  */
if|if
condition|(
name|buffer
operator|->
...
begin_comment
comment|/* Subroutine of skip_escaped_newlines; called when a 3-character    sequence beginning with "??" is encountered.  buffer->cur points to    the second '?'.     Warn if necessary, and returns true if the sequence forms a    trigraph and the trigraph should be honored.  */
end_comment
...
begin_comment
comment|/* Skip a C-style block comment.  We find the end of the comment by    seeing if an asterisk is before every '/' we encounter.  Returns    nonzero if comment terminated by EOF, zero otherwise.  */
end_comment
...
literal|0
decl_stmt|;
comment|/* Place any leading period.  */
if|if
condition|(
name|number_p
operator|==
...
break|break;
block|}
comment|/* Handle normal identifier characters in this loop.  */
do|do
block|{
name|prevc
operator|=
...
name|DL_PEDWARN
argument_list|,
literal|"'$' character(s) in identifier or number"
argument_list|)
expr_stmt|;
comment|/* Identifiers and numbers are null-terminated.  */
operator|*
name|plen
operator|=
...
begin_comment
comment|/* Parse a number, beginning with character C, skipping embedded    backslash-newlines.  LEADING_PERIOD is nonzero if there was a "."    before C.  Place the result in NUMBER.  */
end_comment
...
name|cur
decl_stmt|;
comment|/* Fast-path loop.  Skim over a normal number.      N.B. ISIDNUM does not include $.  */
name|cur
operator|=
name|pfile
operator|->
...
condition|)
block|{
comment|/* No string literal may extend over multiple lines.  In 	     assembly language, suppress the error except for<> 	     includes.  This is a kludge around not knowing where 	     comments are.  */
name|unterminated
label|:
if|if
condition|(
...
begin_comment
comment|/* A NUL terminates the current buffer.  For ISO preprocessing this is    EOF, but for traditional preprocessing it indicates we need a line    refill.  Returns TRUE to continue preprocessing a new buffer, FALSE    to return a CPP_EOF to the caller.  */
end_comment
...
block|{
comment|/* Stop parsing arguments with a CPP_EOF.  When we finally come 	 back here, do the work of popping the buffer.  */
if|if
condition|(
operator|!
name|pfile
...
expr_stmt|;
block|}
comment|/* Similarly, finish an in-progress directive with CPP_EOF 	     before popping the buffer.  */
if|if
condition|(
operator|!
name|pfile
...
name|DL_PEDWARN
argument_list|,
literal|"escape sequence out of range for its type"
argument_list|)
expr_stmt|;
name|c
operator|&=
...
begin_comment
comment|/* Interpret a (possibly wide) character constant in TOKEN.    WARN_MULTI warns about multi-character charconsts.  PCHARS_SEEN    points to a variable that is filled in with the number of    characters seen, and UNSIGNEDP to a variable that indicates whether    the result has signed type.  */
end_comment
...
operator|++
expr_stmt|;
comment|/* Truncate the character, scale the result and merge the two.  */
name|c
operator|&=
name|mask
expr_stmt|;
...
condition|)
block|{
comment|/* Multichar charconsts are of type int and therefore signed.  */
name|unsigned_p
operator|=
literal|0
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* Sign-extend or truncate the constant to cppchar_t.  The value is      in WIDTH bits, but for multi-char charconsts it's value is the      full target type's width.  */
if|if
condition|(
name|chars_seen
operator|>
