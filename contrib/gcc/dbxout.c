...
begin_comment
comment|/* Output dbx-format symbol table information from GNU compiler.    Copyright (C) 1987, 1988, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
directive|endif
comment|/* Get all permanent types that have typedef names, and output them      all, except for those already output.  Some language front ends      put these declarations in the top-level scope; some do not.  */
name|dbxout_typedefs
argument_list|(
call|(
modifier|*
...
control|)
block|{
comment|/* If on of the nodes is an error_mark or its type is then return early. */
if|if
condition|(
name|tem
operator|==
...
argument_list|)
expr_stmt|;
comment|/* If we are to generate only the symbols actually used then such      symbol nodees are flagged with TREE_USED.  Ignore any that      aren't flaged as TREE_USED.  */
if|if
condition|(
name|flag_debug_only_used_symbols
condition|)
...
argument_list|)
expr_stmt|;
comment|/* We now have a used symbol.  We need to generate the info for          the symbol's type in addition to the symbol itself.  These          type symbols are queued to be generated after were done with          the symbol itself (done because the symbol's info is generated          with fprintf's, etc. as it determines what's needed).           Note, because the TREE_TYPE(type) might be something like a          pointer to a named type we need to look for the first name          we see following the TREE_TYPE chain.  */
name|t
operator|=
name|type
expr_stmt|;
...
directive|endif
comment|/* Initialize variables used to communicate each symbol's debug      information to dbxout_finish_symbol with zeroes.  */
...
literal|0
condition|)
comment|/* The outermost block doesn't get LBB labels; use 		   the function symbol.  */
name|scope_start
operator|=
name|begin_label
expr_stmt|;
