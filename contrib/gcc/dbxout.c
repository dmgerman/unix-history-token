...
begin_comment
comment|/* Output dbx-format symbol table information from GNU compiler.    Copyright (C) 1987, 88, 92-97, 1998 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Nonzero means if the type has methods, only output debugging    information if methods are actually written to the asm file.  This    optimization only works if the debugger can detect the special C++    marker.  */
end_comment
...
name|marker
decl_stmt|;
comment|/* We can't optimize a method which uses an anonymous                  class, because the debugger will not be able to                  associate the arbitrary class name with the actual                  class.  */
ifndef|#
directive|ifndef
name|NO_DOLLAR_IN_LABEL
name|marker
...
block|{
comment|/* Try to find the "main variant" with the same name but not const 	 or volatile.  (Since stabs does not distinguish const and volatile, 	 there is no need to make them separate types.  But types with 	 different names are usefully distinguished.) */
for|for
control|(
name|tem
operator|=
...
expr_stmt|;
block|}
comment|/* This used to check if the type's precision was more than 	 HOST_BITS_PER_WIDE_INT.  That is wrong since gdb uses a 	 long (it has no concept of HOST_BITS_PER_WIDE_INT).  */
elseif|else
if|if
condition|(
name|use_gnu_debug_info_extensions
...
block|if (typevec[TYPE_SYMTAB_ADDRESS (type)].status == TYPE_DEFINED&& TYPE_NAME (TREE_TYPE (decl)) == decl) 	return;
endif|#
directive|endif
