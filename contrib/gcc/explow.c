...
condition|)
block|{
comment|/* Any rtl we create here must go in a saveable obstack, since 	     we might have been called from within combine.  */
...
name|BIGGEST_ALIGNMENT
operator|)
comment|/* If anyone creates a target with these characteristics, let them 	   know that our optimization cannot work correctly in such a case.  */
name|abort
argument_list|()
expr_stmt|;
endif|#
...
decl_stmt|;
block|{
comment|/* First see if we have an insn to check the stack.  Use it if so.  */
ifdef|#
directive|ifdef
...
argument_list|)
expr_stmt|;
comment|/* If will be doing stupid optimization, show test_addr is still live. */
if|if
condition|(
name|obey_regdecls
condition|)
