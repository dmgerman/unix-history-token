...
name|mark_write
block|,
comment|/* mark_access is for marking the destination regs in      REG_FRAME_RELATED_EXPR notes (as if they were read) so that the      note is updated properly.  */
name|mark_access
block|}
enum|;
end_enum
...
begin_comment
comment|/* Adapted from find_reloads_address_1.  CL is INDEX_REG_CLASS or    BASE_REG_CLASS depending on how the register is being considered.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Step 3B: Record updates for regs in REG_INC notes, and 	     source regs in REG_FRAME_RELATED_EXPR notes.  */
for|for
control|(
name|note
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Step 4: Close chains for registers that die here.  */
for|for
control|(
name|note
operator|=
...
expr_stmt|;
block|}
comment|/* Step 6B: Record destination regs in REG_FRAME_RELATED_EXPR 	     notes for update.  */
for|for
control|(
name|note
operator|=
...
begin_comment
comment|/* Dump all def/use chains in CHAINS to DUMP_FILE.  They are    printed in reverse order as that's how we build them.  */
end_comment
...
begin_comment
comment|/* Kill register REGNO.  This involves removing it from any value    lists, and resetting the value mode to VOIDmode.  This is only a    helper function; it does not handle any hard registers overlapping    with REGNO.  */
end_comment
...
begin_comment
comment|/* Kill the value in register REGNO for NREGS, and any other registers    whose values overlap.  */
end_comment
...
begin_comment
comment|/* Kill X.  This is a convenience function wrapping kill_value_regno    so that we mind the mode the register is in.  */
end_comment
...
condition|)
return|return;
comment|/* Do not propagate copies to fixed or global registers, patterns      can be relying to see particular fixed register or users can      expect the chosen global register in asm.  */
if|if
condition|(
name|fixed_regs
index|[
...
begin_comment
comment|/* Find the oldest copy of the value contained in REGNO that is in    register class CL and has mode MODE.  If found, return an rtx    of that oldest register, otherwise return NULL.  */
end_comment
...
begin_comment
comment|/* If possible, replace the register at *LOC with the oldest register    in register class CL.  Return true if successfully replaced.  */
end_comment
...
begin_comment
comment|/* Similar to replace_oldest_value_reg, but *LOC contains an address.    Adapted from find_reloads_address_1.  CL is INDEX_REG_CLASS or    BASE_REG_CLASS depending on how the register is being considered.  */
end_comment
...
name|dump_file
argument_list|,
literal|"insn %u: reg replacements not verified\n"
argument_list|,
name|INSN_UID
argument_list|(
name|insn
