...
begin_endif
unit|static void df_bb_refs_unlink (struct df *, basic_block); static void df_refs_unlink (struct df *, bitmap);
endif|#
directive|endif
end_endif
...
condition|)
return|return;
comment|/* Make the table a little larger than requested, so we do not need      to enlarge it so often.  */
name|size
operator|+=
name|df
operator|->
...
begin_comment
comment|/* Return nonzero if writes to paradoxical SUBREGs, or SUBREGs which    are too narrow, are read-modify-write.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Paradoxical subreg writes don't leave a trace of the old content.  */
return|return
operator|(
name|isize
operator|>
...
argument_list|)
decl_stmt|;
comment|/* Do not add ref's to the chain twice, i.e., only add new              refs.  XXX the same could be done by testing if the              current insn is a modified (or a new) one.  This would be              faster.  */
if|if
condition|(
name|DF_REF_ID
argument_list|(
...
literal|0
decl_stmt|;
comment|/* While we have to scan the chain of insns for this BB, we do not      need to allocate and queue a long chain of BB/INSN pairs.  Using      a bitmap for insns_modified saves memory and avoids queuing      duplicates.  */
for|for
control|(
name|insn
operator|=
...
begin_comment
comment|/* Analyze dataflow info for the basic blocks specified by the bitmap    BLOCKS, or for the whole CFG if BLOCKS is zero, or just for the    modified blocks if BLOCKS is -1.  */
end_comment
...
begin_comment
unit|static void df_bb_refs_unlink (struct df *df, basic_block bb) {   rtx insn;
comment|/* Scan the block an insn at a time from beginning to end.  */
end_comment

begin_comment
unit|for (insn = BB_HEAD (bb); ; insn = NEXT_INSN (insn))     {       if (INSN_P (insn)) 	{
comment|/* Unlink refs for INSN.  */
end_comment

begin_comment
unit|df_insn_refs_unlink (df, bb, insn); 	}       if (insn == BB_END (bb)) 	break;     } }
comment|/* Unlink all the refs in the basic blocks specified by BLOCKS.    Not currently used.  */
end_comment

begin_endif
unit|static void df_refs_unlink (struct df *df, bitmap blocks) {   basic_block bb;    if (blocks)     {       FOR_EACH_BB_IN_BITMAP (blocks, 0, bb,       { 	df_bb_refs_unlink (df, bb);       });     }   else     {       FOR_EACH_BB (bb) 	df_bb_refs_unlink (df, bb);     } }
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* Dump a chain of refs with the associated regno.  */
end_comment
...
argument|i
argument_list|,
argument|{     fibheap_insert (worklist, order[i], (void *) (size_t) i);     SET_BIT (pending, i);     if (dir == DF_FORWARD)       sbitmap_copy (out[i], gen[i]);     else       sbitmap_copy (in[i], gen[i]);   }
argument_list|)
empty_stmt|;
while|while
condition|(
...
argument|i
argument_list|,
argument|{     fibheap_insert (worklist, order[i], (void *) (size_t) i);     SET_BIT (pending, i);     if (dir == DF_FORWARD)       bitmap_copy (out[i], gen[i]);     else       bitmap_copy (in[i], gen[i]);   }
argument_list|)
empty_stmt|;
while|while
condition|(
