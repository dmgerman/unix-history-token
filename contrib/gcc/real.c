...
begin_comment
comment|/* Return R = A + (SUBTRACT_P ? -B : B).  */
end_comment
...
operator|)
expr_stmt|;
comment|/* Force at least one bit of the significand set.  */
for|for
control|(
name|d
operator|=
...
literal|2
expr_stmt|;
comment|/* Our intermediate format forces QNaNs to have MSB-1 set. 	 If the target format has QNaNs with the top bit unset, 	 mirror the output routines and invert the top two bits.  */
if|if
condition|(
operator|!
name|fmt
...
argument_list|)
expr_stmt|;
comment|/* If we've cleared the entire significand, we need one bit 	 set for this to continue to be a NaN.  */
for|for
control|(
name|i
operator|=
...
begin_comment
comment|/* IEEE extended double precision format.  This comes in three    flavours: Intel's as a 12 byte image, Intel's as a 16 byte image,    and Motorola's.  */
end_comment
...
name|exp
decl_stmt|;
comment|/* Recall that IEEE numbers are interpreted as 1.F x 2**exp, 	   whereas the intermediate representation is 0.F x 2**exp. 	   Which means we're off by one.   	   Except for Motorola, which consider exp=0 and explicit 	   integer bit set to continue to be normalized.  In theory 	   this descrepency has been taken care of by the difference 	   in fmt->emin in round_for_format.  */
if|if
condition|(
name|denormal
condition|)
...
name|rvc_zero
case|:
comment|/* Both doubles have sign bit set.  */
name|buf
index|[
literal|0
index|]
...
name|rvc_nan
case|:
comment|/* Both doubles set to Inf / NaN.  */
name|encode_ieee_double
argument_list|(
operator|&
name|ieee_double_format
...
argument_list|)
expr_stmt|;
comment|/* If the upper double is zero, we have a denormal double, so 	 move it to the first double and leave the second as zero.  */
if|if
condition|(
name|u
operator|.
...
block|{
comment|/* v = remainder containing additional 53 bits of significand.  */
name|do_add
argument_list|(
operator|&
name|v
...
begin_comment
comment|/* Set up default mode to format mapping for IEEE.  Everyone else has    to set these values in OVERRIDE_OPTIONS.  */
end_comment
...
block|,
comment|/* DFmode */
comment|/* We explicitly don't handle XFmode.  There are two formats,      pretty much equally common.  Choose one in OVERRIDE_OPTIONS.  */
name|NULL
block|,
comment|/* XFmode */
operator|&
