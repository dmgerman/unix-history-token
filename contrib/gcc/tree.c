...
name|q
decl_stmt|;
comment|/* If we already have a name we know to be unique, just use that.  */
if|if
condition|(
...
name|first_global_object_name
expr_stmt|;
comment|/* If the target is handling the constructors/destructors, they      will be local to this file and the name is only necessary for      debugging purposes.  */
elseif|else
if|if
condition|(
operator|(
...
name|input_filename
expr_stmt|;
comment|/* Just use the file's basename, because the full pathname 	 might be quite long.  */
name|p
operator|=
name|strrchr
argument_list|(
...
block|{
comment|/* Otherwise, the name must be unique across the entire link. 	 We don't have anything that we know to be unique to this translation 	 unit, so use what we do have and throw in some randomness.  */
name|unsigned
name|len
decl_stmt|;
specifier|const
...
argument_list|)
expr_stmt|;
comment|/* Fixed size integer types.  */
