...
begin_comment
comment|/* Language-independent node constructors for parse phase of GNU compiler.    Copyright (C) 1987, 1988, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Return a new VECTOR_CST node whose type is TYPE and whose values    are in a list pointed by VALS.  */
end_comment
...
name|vals
expr_stmt|;
comment|/* ??? May not be necessary.  Mirrors what build does.  */
if|if
condition|(
name|vals
condition|)
...
begin_comment
comment|/* Return the first expression in a sequence of COMPOUND_EXPRs.  */
end_comment
...
begin_comment
comment|/* Return the last expression in a sequence of COMPOUND_EXPRs.  */
end_comment
...
begin_comment
comment|/* Return the number of subexpressions in a sequence of COMPOUND_EXPRs.  */
end_comment
...
begin_comment
comment|/* Default lang hook for "unsave_expr_now".  */
end_comment
...
case|:
comment|/* EXIT_BLOCK_LABELED_BLOCK, a.k.a. TREE_OPERAND (expr, 0), holds 	 a reference to an ancestor LABELED_BLOCK, so we need to avoid 	 unbounded recursion in the 'e' traversal code below.  */
name|exp
operator|=
...
break|break;
block|}
comment|/* Now remove us from seen_types and return the result.  */
if|if
condition|(
name|seen_types
operator|==
...
condition|)
block|{
comment|/* The address of a volatile decl or reference does not have 	     side-effects.  But be careful not to ignore side-effects from 	     other sources deeper in the expression--if node is a _REF and 	     one of its operands has side-effects, so do we.  */
if|if
condition|(
name|TREE_THIS_VOLATILE
argument_list|(
...
begin_comment
comment|/* Return 1 if T is an INTEGER_CST that can be manipulated efficiently on    the host.  If POS is zero, the value can be represented in a single    HOST_WIDE_INT.  If POS is nonzero, the value must be positive and can    be represented in a single unsigned HOST_WIDE_INT.  */
end_comment
...
condition|)
block|{
comment|/* Decls we can just compare by pointer.  */
name|val
operator|=
name|iterative_hash_object
argument_list|(
...
begin_comment
comment|/* Construct, lay out and return the type of pointers to TO_TYPE    with mode MODE. If such a type has already been constructed,    reuse it.  */
end_comment
...
begin_comment
comment|/* Construct, lay out and return the type of references to TO_TYPE    with mode MODE. If such a type has already been constructed,    reuse it.  */
end_comment
...
decl_stmt|;
comment|/* Perform some generic filtering first, which may allow making a decision      even if the bounds are not constant.  First, negative integers never fit      in unsigned types, */
if|if
condition|(
operator|(
name|TREE_UNSIGNED
...
literal|0
operator|)
comment|/* Also, unsigned integers with top bit set never fit signed types.  */
operator|||
operator|(
operator|!
name|TREE_UNSIGNED
...
literal|1
return|;
comment|/* If we haven't been able to decide at this point, there nothing more we      can check ourselves here. Look at the base type if we have one.  */
elseif|else
if|if
condition|(
name|TREE_CODE
...
name|false
return|;
comment|/* If TYPE itself has variable size, it is variably modified.       We do not yet have a representation of the C99 '[*]' syntax.      When a representation is chosen, this function should be modified      to test for that case as well.  */
name|t
operator|=
name|TYPE_SIZE
argument_list|(
...
name|ARRAY_TYPE
case|:
comment|/* If TYPE is a pointer or reference, it is variably modified if 	 the type pointed to is variably modified.  Similarly for arrays; 	 note that VLAs are handled by the TYPE_SIZE check above.  */
return|return
name|variably_modified_type_p
argument_list|(
name|TREE_TYPE
...
name|METHOD_TYPE
case|:
comment|/* If TYPE is a function type, it is variably modified if any of the          parameters or the return type are variably modified.  */
block|{
name|tree
name|parm
decl_stmt|;
...
begin_comment
comment|/* For a new vector type node T, build the information necessary for    debugging output.  */
end_comment
...
case|:
block|{
comment|/* Set is empty if it has no elements.  */
if|if
condition|(
operator|(
name|TREE_CODE
