...
begin_comment
comment|/* Build a function type.  The RETURN_TYPE is the type retured by the    function.  If additional arguments are provided, they are    additional argument types.  The list of argument types must always    be terminated by NULL_TREE.  */
end_comment
...
name|true
return|;
comment|/* If TYPE is a pointer or reference, it is variably modified if       the type pointed to is variably modified.  */
if|if
condition|(
operator|(
name|TREE_CODE
...
name|true
return|;
comment|/* If TYPE is an array, it is variably modified if the array      elements are.  (Note that the VLA case has already been checked      above.)  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Set up a default flag_random_seed value, if there wasn't one already.  */
end_comment
...
directive|endif
comment|/* This slightly overestimates the space required.  */
name|new_random_seed
operator|=
name|xmalloc
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* This isn't a very good hash, but it does guarantee no collisions       when the random string is generated by the code above and the time      delta is small.  */
name|v
operator|=
literal|0
expr_stmt|;
