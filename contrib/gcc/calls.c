...
begin_comment
comment|/* Convert function calls to rtl insns, for GNU C compiler.    Copyright (C) 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
break|break;
block|}
comment|/* Only expressions and blocks can contain calls.  */
if|if
condition|(
operator|!
...
begin_comment
comment|/* Determine if the function identified by NAME and FNDECL is one with    special properties we wish to know about.     For example, if the function might return more than one time (setjmp), then    set RETURNS_TWICE to a nonzero value.     Similarly set LONGJMP for if the function is in the longjmp family.     Set MAY_BE_ALLOCA for any memory allocation function that might allocate    space from the stack such as alloca.  */
end_comment
...
begin_comment
comment|/* Do the register loads required for any wholly-register parms or any    parms which are passed both on the stack and in a register.  Their    expressions were already evaluated.     Mark all register-parms as living through the call, putting these USE    insns in the CALL_INSN_FUNCTION_USAGE field.     When IS_SIBCALL, perform the check_sibcall_overlap_argument_overlap    checking, setting *SIBCALL_FAILURE if appropriate.  */
end_comment
...
name|warning
argument_list|(
literal|"%Jinlining failed in call to '%F'"
argument_list|,
name|fndecl
argument_list|,
name|fndecl
...
begin_comment
comment|/* If function value *VALUE was returned at the most significant end of a    register, shift it towards the least significant end and convert it to    TYPE's mode.  Return true and update *VALUE if some action was needed.     TYPE is the type of the function's return value, which is known not    to have mode BLKmode.  */
end_comment
...
name|warning
argument_list|(
literal|"%Jcan't inline call to '%F'"
argument_list|,
name|fndecl
argument_list|,
name|fndecl
...
name|warning
argument_list|(
literal|"ignoring return value of `%D', "
...
name|warning
argument_list|(
literal|"ignoring return value of function "
literal|"declared with attribute warn_unused_result"
argument_list|)
expr_stmt|;
name|flags
...
condition|)
block|{
comment|/* The structure value address arg is already in actparms. 	       Pull it out.  It might be nice to just leave it there, but 	       we need to set structure_value_addr.  */
name|tree
name|return_arg
init|=
name|TREE_VALUE
...
expr_stmt|;
block|}
comment|/* Do the same for the function address if it is an expression.  */
if|if
condition|(
operator|!
name|fndecl
...
condition|)
block|{
comment|/* Second condition is added because "target" is freed at the 	     the end of "pass0" for -O2 when call is made to 	     expand_end_target_temps ().  Its "in_use" flag has been set 	     to false, so allocate a new temp.  */
if|if
condition|(
name|target
operator|==
...
expr_stmt|;
block|}
comment|/* If this function is returning into a memory location marked as 	 readonly, it means it is initializing that location. We normally treat 	 functions as not clobbering such locations, so we need to specify that 	 this one does. We do this by adding the appropriate CLOBBER to the 	 CALL_INSN function usage list.  This cannot be done by emitting a 	 standalone CLOBBER after the call because the latter would be ignored 	 by at least the delay slot scheduling pass. We do this now instead of 	 adding to call_fusage before the call to emit_call_1 because TARGET 	 may be modified in the meantime.  */
if|if
condition|(
name|structure_value_addr
operator|!=
...
condition|)
block|{
comment|/* Use the size of the elt to compute excess.  */
name|rtx
name|elt
init|=
name|XEXP
...
expr_stmt|;
block|}
comment|/* Some types will require stricter alignment, which will be 	 provided for elsewhere in argument layout.  */
name|parm_align
operator|=
name|MAX
argument_list|(
