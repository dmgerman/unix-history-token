...
begin_comment
comment|/* APPLE LOCAL begin C* property (Radar 4436866) (in 4.2 d) */
end_comment

begin_comment
comment|/* For checking property attribute keywords */
end_comment

begin_decl_stmt
specifier|static
name|int
name|objc_property_attr_context
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* APPLE LOCAL end C* property (Radar 4436866) (in 4.2 d) */
end_comment

begin_comment
comment|/* APPLE LOCAL radar 3803157 - objc attribute (in 4.2 e) */
end_comment

begin_decl_stmt
specifier|static
name|tree
name|objc_method_attributes
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* APPLE LOCAL begin C* language (in 4.2 f) */
end_comment

begin_comment
comment|/* For checking for 'foreach' context. */
end_comment

begin_decl_stmt
specifier|static
name|int
name|objc_foreach_context
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* APPLE LOCAL end C* language (in 4.2 f) */
end_comment
...
block|}
block|,
comment|/* APPLE LOCAL begin C* language (in 4.2 j) */
block|{
literal|"optional"
block|,
...
block|}
block|,
comment|/* APPLE LOCAL end C* language (in 4.2 j) */
comment|/* APPLE LOCAL C* property (Radar 4436866) (in 4.2 k) */
block|{
literal|"property"
block|,
...
block|}
block|,
comment|/* APPLE LOCAL begin C* property (Radar 4436866) (in 4.2 l) */
comment|/* These are recognized inside a property attribute list */
block|{
literal|"readonly"
block|,
name|RID_READONLY
...
block|}
block|,
comment|/* APPLE LOCAL end C* property (Radar 4436866) (in 4.2 l) */
comment|/* APPLE LOCAL radar 4947014 - objc atomic property */
block|{
literal|"nonatomic"
block|,
name|RID_NONATOMIC
...
begin_comment
comment|/* APPLE LOCAL C* language (in 4.2 ae) */
end_comment
...
condition|)
block|{
comment|/* This is dangerous, we assume we don't need 3 input tokens look ahead.  */
name|c_token
modifier|*
name|tk
init|=
...
comment|/* APPLE LOCAL radar 4529200 (in 4.2 af) */
operator|||
name|tk
operator|->
name|type
...
condition|)
block|{
comment|/* APPLE LOCAL begin switch these two */
name|parser
operator|->
name|tokens_avail
operator|=
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end switch these two */
block|}
return|return
operator|&
name|parser
...
argument_list|)
decl_stmt|;
comment|/* APPLE LOCAL begin radar 5277239 */
comment|/* Yes, we can have CLASS.method to mean property-style dot-syntax       notation to call a class method (equiv to [CLASS meth]). */
return|return
name|c_token_starts_declspecs
argument_list|(
name|token
...
operator|)
return|;
comment|/* APPLE LOCAL end radar 5277239 */
block|}
end_function
...
comment|/* A block declarator (maybe empty).  */
name|C_DTR_BLOCK
block|,
...
begin_comment
comment|/* APPLE LOCAL radar 4708210 (for_objc_collection in 4.2) */
end_comment
...
begin_comment
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ ca) */
end_comment

begin_function_decl
specifier|static
name|tree
name|c_parser_block_literal_expr
parameter_list|(
name|c_parser
modifier|*
parameter_list|)
...
begin_comment
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ ca) */
end_comment
...
begin_comment
comment|/* APPLE LOCAL radar 4548636 - class attributes. */
end_comment
...
begin_comment
comment|/* APPLE LOCAL C* property (Radar 4436866) (in 4.2 b) */
end_comment

begin_function_decl
specifier|static
name|void
name|c_parser_objc_interfacedecllist
parameter_list|(
name|c_parser
modifier|*
parameter_list|)
...
begin_comment
comment|/* APPLE LOCAL C* property (Radar 4436866) (in 4.2 x) */
end_comment

begin_function_decl
specifier|static
name|void
name|c_parser_objc_property_declaration
parameter_list|(
name|c_parser
modifier|*
parameter_list|)
...
comment|/* APPLE LOCAL begin C* property (Radar 4436866) (in 4.2 x) */
case|case
name|RID_AT_PROPERTY
case|:
name|c_parser_objc_property_declaration
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL end C* property (Radar 4436866) (in 4.2 x) */
case|case
name|RID_AT_END
case|:
name|gcc_assert
...
return|return;
block|}
comment|/* APPLE LOCAL begin radar 4548636 - class attributes. */
elseif|else
if|if
condition|(
name|c_parser_next_token_is_keyword
...
name|parser
argument_list|,
literal|"no type or storage class may be specified here"
argument_list|)
expr_stmt|;
name|c_parser_objc_class_definition
argument_list|(
...
return|return;
block|}
comment|/* APPLE LOCAL end radar 4548636 - class attributes. */
comment|/* APPLE LOCAL begin radar 4947311 - protocol attributes */
elseif|else
if|if
condition|(
...
condition|)
block|{
comment|/* APPLE LOCAL begin radar 5985368 */
if|if
condition|(
name|declarator
operator|->
...
name|error
argument_list|(
literal|"bad definition of a block"
argument_list|)
expr_stmt|;
elseif|else
if|if
...
name|pedantic
condition|)
comment|/* APPLE LOCAL end radar 5985368 */
name|pedwarn
argument_list|(
literal|"ISO C forbids nested functions"
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin nested functions 4258406 4357979 (in 4.2 m) */
elseif|else
if|if
condition|(
name|flag_nested_functions
operator|==
literal|0
condition|)
name|error
argument_list|(
literal|"nested functions are disabled, use -fnested-functions to re-enable"
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end nested functions 4258406 4357979 (in 4.2 m) */
...
specifier|static
name|tree
name|finish_parse_foreach_header
parameter_list|(
name|c_parser
modifier|*
name|parser
...
return|;
block|}
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cc) */
elseif|else
if|if
condition|(
name|flag_blocks
...
comment|/* APPLE LOCAL radar 5814025 (C++ cc) */
return|return
...
return|;
block|}
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cc) */
...
name|false
decl_stmt|;
comment|/* APPLE LOCAL radar 5732232 - blocks (not in C++) */
name|bool
name|first_stmt
init|=
name|true
...
name|RID_GOTO
case|:
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cb) */
if|if
condition|(
name|cur_block
condition|)
name|error
argument_list|(
literal|"goto not allowed in block literal"
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cb) */
name|c_parser_consume_token
argument_list|(
name|parser
argument_list|)
...
comment|/* APPLE LOCAL begin Radar 6144634  */
comment|/* Normal expr stmts, including modify exprs, get the location where      the statement began, i.e. 'loc'.  Assignments of Blocks to Block      pointer variables get the location of the end of the Block definition,      i.e. 'input_location', which should already be set by this point.  */
if|if
condition|(
name|stmt
operator|&&
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end Radar 6144634  */
block|}
end_function
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 5925639 */
if|if
condition|(
name|c_parser_next_token_is_keyword
argument_list|(
...
condition|)
block|{
comment|/* CLASS.class_method expression. */
name|tree
name|receiver
decl_stmt|,
name|component
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cd) */
comment|/* If a variabled declared as referenced variable, using |...| syntax, 	    is used in the block, it has to be derefrenced because this 	    variable holds address of the outside variable referenced in. */
comment|/* APPLE LOCAL begin radar 5932809 - copyable byref blocks (C++ cd) */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
condition|)
block|{
comment|/* What we have is an expression which is of type  		     struct __Block_byref_X. Must get to the value of the variable 		     embedded in this structure. It is at: 		     __Block_byref_X.__forwarding->x */
name|expr
operator|.
name|value
operator|=
...
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cd) */
name|expr
operator|.
name|original_code
operator|=
...
block|}
break|break;
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cf) */
case|case
name|CPP_XOR
case|:
if|if
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cf) */
case|case
name|CPP_OPEN_SQUARE
case|:
if|if
...
name|error
argument_list|(
literal|"attributes may not be specified on an implementation"
argument_list|)
expr_stmt|;
name|prefix_attrs
operator|=
...
condition|)
block|{
comment|/* APPLE LOCAL radar 4965989 */
name|tree
name|id2
init|=
name|NULL_TREE
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 4965989 */
if|if
condition|(
name|c_parser_next_token_is_not
argument_list|(
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 4965989 */
name|c_parser_skip_until_found
argument_list|(
name|parser
argument_list|,
...
name|error
argument_list|(
literal|"cannot implement anonymous category"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|error
argument_list|(
literal|"attributes may not be specified on a category"
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 4548636 - class attributes. */
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL C* property (Radar 4436866) (in 4.2 q) */
name|c_parser_objc_interfacedecllist
argument_list|(
name|parser
argument_list|)
...
begin_comment
comment|/* APPLE LOCAL begin C* property (Radar 4436866) (in 4.2 s) */
end_comment

begin_function
specifier|static
name|tree
name|c_parser_objc_eq_identifier
parameter_list|(
name|c_parser
modifier|*
name|parser
...
begin_comment
comment|/* Parse obj-property-attribute. */
end_comment

begin_function
specifier|static
name|void
name|c_parser_objc_property_attribute
parameter_list|(
name|c_parser
modifier|*
name|parser
...
name|parser
argument_list|,
literal|"expected a property attribute"
argument_list|)
expr_stmt|;
name|c_parser_consume_token
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Consume the ':' which must always follow the setter name. */
if|if
condition|(
name|c_parser_next_token_is
argument_list|(
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL begin radar 4947014 - objc atomic property */
case|case
name|RID_NONATOMIC
case|:
name|c_parser_consume_token
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL end radar 4947014 - objc atomic property */
default|default:
name|c_parser_error
argument_list|(
name|parser
...
specifier|static
name|void
name|c_parser_objc_property_attrlist
parameter_list|(
name|c_parser
modifier|*
name|parser
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 6302949 */
if|if
condition|(
name|c_parser_next_token_is_not
argument_list|(
...
literal|0
argument_list|,
literal|"property attributes must be separated by a comma"
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 6302949 */
if|if
condition|(
name|c_parser_next_token_is
argument_list|(
...
specifier|static
name|void
name|c_parser_objc_property_attr_decl
parameter_list|(
name|c_parser
modifier|*
name|parser
...
argument_list|)
expr_stmt|;
comment|/* Comma-separated properties are chained together in      reverse order; add them one by one.  */
name|prop
operator|=
name|nreverse
argument_list|(
...
begin_comment
comment|/* APPLE LOCAL end C* property (Radar 4436866) (in 4.2 s) */
end_comment
...
continue|continue;
block|}
comment|/* APPLE LOCAL begin radar 4564694 */
elseif|else
if|if
condition|(
name|c_parser_next_token_is_keyword
...
continue|continue;
block|}
comment|/* APPLE LOCAL end radar 4564694 */
elseif|else
if|if
condition|(
name|c_parser_next_token_is
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL C* property (Radar 4436866) (in 4.2 r) */
name|c_parser_objc_interfacedecllist
argument_list|(
name|parser
argument_list|)
...
literal|0
expr_stmt|;
comment|/* APPLE LOCAL begin radar 3803157 - objc attribute (in 4.2 a) */
...
name|NULL_TREE
expr_stmt|;
comment|/* APPLE LOCAL end radar 3803157 - objc attribute (in 4.2 a) */
name|add_stmt
argument_list|(
name|c_parser_compound_statement
argument_list|(
...
begin_comment
comment|/* APPLE LOCAL begin C* language (in 4.2 w) */
end_comment

begin_comment
comment|/* True iff the gioven TOKEN starts a methodproto.  */
end_comment

begin_function
specifier|static
name|bool
name|c_token_starts_methodproto
parameter_list|(
name|c_token
modifier|*
name|token
...
begin_comment
comment|/* APPLE LOCAL end C* language (in 4.2 w) */
end_comment
...
condition|)
block|{
comment|/* APPLE LOCAL begin C* property (Radar 4436866) (in 4.2 b) */
name|c_token
modifier|*
name|token
decl_stmt|;
...
continue|continue;
block|}
comment|/* APPLE LOCAL end C* property (Radar 4436866) (in 4.2 b) */
comment|/* APPLE LOCAL begin C* language (in 4.2 w) */
if|if
condition|(
name|c_token_starts_methodproto
...
break|break;
comment|/* APPLE LOCAL begin C* language (in 4.2 w) */
comment|/* CPP_PLUS and CPP_MINUS deleted */
comment|/* APPLE LOCAL end C* language (in 4.2 w) */
case|case
name|CPP_PRAGMA
case|:
...
literal|0
expr_stmt|;
comment|/* APPLE LOCAL begin radar 3803157 - objc attribute (in 4.2 c) */
...
name|NULL_TREE
expr_stmt|;
comment|/* APPLE LOCAL end radar 3803157 - objc attribute (in 4.2 c) */
name|c_parser_skip_until_found
argument_list|(
name|parser
argument_list|,
...
break|break;
block|}
comment|/* APPLE LOCAL begin radar 3803157 - objc attribute (in 4.2 y) */
if|if
condition|(
name|c_parser_next_token_is_keyword
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 3803157 - objc attribute (in 4.2 y) */
...
name|error
argument_list|(
literal|"method attributes must be specified at the end only"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|parser
argument_list|,
literal|"objective-c method declaration is expected"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
condition|)
block|{
comment|/* APPLE LOCAL radar 4301047 (in 4.2 z) */
name|quals
operator|=
name|chainon
argument_list|(
...
condition|)
break|break;
comment|/* APPLE LOCAL begin radar 2848255 */
if|if
condition|(
name|c_parser_next_token_is
argument_list|(
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 2848255 */
if|if
condition|(
name|c_parser_require
argument_list|(
...
begin_comment
comment|/* APPLE LOCAL begin radar 5982990 */
end_comment

begin_comment
comment|/* This routine is called from c_parser_objc_synchronized_statement    and is identical to c_parser_compound_statement with    the addition of volatizing local variables seen in the scope    of @synchroniz block. */
end_comment

begin_function
specifier|static
name|tree
name|c_parser_objc_synch_compound_statement
parameter_list|(
name|c_parser
modifier|*
name|parser
...
begin_comment
comment|/* APPLE LOCAL end radar 5982990 */
end_comment
...
name|error_mark_node
expr_stmt|;
comment|/* APPLE LOCAL radar 5982990 */
name|stmt
operator|=
name|c_parser_objc_synch_compound_statement
argument_list|(
...
begin_comment
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ ce) */
end_comment
...
name|get_identifier
argument_list|(
literal|"__block_literal_generic"
argument_list|)
argument_list|)
expr_stmt|;
name|field_decl
...
name|block_struct_type
decl_stmt|;
comment|/* Check and see if this block is required to have a Copy/Dispose      helper function. If yes, set BlockHasCopyDispose to TRUE. */
for|for
control|(
name|chain
operator|=
...
break|break;
block|}
comment|/* Further check to see that we have __block variables which require      Copy/Dispose helpers. */
for|for
control|(
name|chain
operator|=
...
argument_list|)
expr_stmt|;
comment|/* struct __block_descriptor *__descriptor */
name|field_decl
operator|=
name|build_decl
argument_list|(
...
condition|)
block|{
comment|/* If inner block of a nested block has BlockHasCopyDispose, so 	does its outer block. */
if|if
condition|(
name|block_impl
operator|->
...
expr_stmt|;
block|}
comment|/* int x; // ref variable list ... */
for|for
control|(
name|chain
operator|=
...
argument_list|)
decl_stmt|;
comment|/* Note! const-ness of copied in variable must not be carried over to the 	type of the synthesized struct field. It prevents to assign to this 	field when copy constructor is synthesized. */
name|field_decl
operator|=
name|build_decl
argument_list|(
...
expr_stmt|;
block|}
comment|/* int *y; // byref variable list */
for|for
control|(
name|chain
operator|=
...
begin_comment
comment|/** build_descriptor_block_decl -   This routine builds a static block_descriptior variable of type:   struct __block_descriptor; and initializes it to:   {0, sizeof(struct literal_block_n),     copy_helper_block_1, // only if block BLOCK_HAS_COPY_DISPOSE    destroy_helper_block_1, // only if block BLOCK_HAS_COPY_DISPOSE   } */
end_comment

begin_function
specifier|static
name|tree
name|build_descriptor_block_decl
parameter_list|(
name|tree
name|block_struct_type
parameter_list|,
...
name|name
argument_list|,
literal|"__block_descriptor_tmp_%d"
argument_list|,
operator|++
name|desc_unique_count
argument_list|)
...
literal|1
expr_stmt|;
comment|/* Initialize "reserved" field to 0 for now. */
name|fields
operator|=
name|TYPE_FIELDS
argument_list|(
...
condition|)
block|{
comment|/* Initialize "CopyFuncPtr" and "DestroyFuncPtr" fields. */
comment|/* Helpers were previously generated completeley as a nested 	 function (and context was required for code gen.) But they are not,  	 so context must be set to NULL so initialization logic does not complain. */
name|DECL_CONTEXT
argument_list|(
name|block_impl
operator|->
...
begin_comment
comment|/**  build_block_struct_initlist - builds the initializer list:  {&_NSConcreteStackBlock or&_NSConcreteGlobalBlock // __isa,    BLOCK_USE_STRET | BLOCK_HAS_COPY_DISPOSE | BLOCK_IS_GLOBAL // __flags,    0, // __reserved&helper_1, // __FuncPtr,&static_descriptor_variable // __descriptor,    x, // user variables.&y    ...  } */
end_comment

begin_function
specifier|static
name|tree
name|build_block_struct_initlist
parameter_list|(
name|tree
name|block_struct_type
parameter_list|,
...
specifier|static
name|tree
name|NSConcreteStackBlock_decl
init|=
name|NULL_TREE
decl_stmt|;
specifier|static
name|tree
name|NSConcreteGlobalBlock_decl
init|=
name|NULL_TREE
decl_stmt|;
name|tree
name|descriptor_block_decl
init|=
name|build_descriptor_block_decl
argument_list|(
name|block_struct_type
...
name|BlockHasCopyDispose
condition|)
comment|/* Note! setting of this flag merely indicates to the runtime that 	we have destroy_helper_block/copy_helper_block helper 	routines. */
name|flags
operator||=
name|BLOCK_HAS_COPY_DISPOSE
expr_stmt|;
comment|/* APPLE LOCAL begin radar 7735196 */
if|if
condition|(
name|block_impl
operator|->
...
expr_stmt|;
comment|/* APPLE LOCAL end 7735196 */
name|fields
operator|=
name|TYPE_FIELDS
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 6230297 */
if|if
condition|(
operator|!
name|current_function_decl
...
operator|)
condition|)
comment|/* APPLE LOCAL end radar 6230297 */
block|{
comment|/* This is a global block. */
comment|/* Find an existing declaration for _NSConcreteGlobalBlock or declare 	 extern void *_NSConcreteGlobalBlock; */
if|if
condition|(
name|NSConcreteGlobalBlock_decl
operator|==
...
name|get_identifier
argument_list|(
literal|"_NSConcreteGlobalBlock"
argument_list|)
decl_stmt|;
name|NSConcreteGlobalBlock_decl
operator|=
...
block|}
block|}
comment|/* APPLE LOCAL begin radar 6457359 */
name|initlist
operator|=
name|build_tree_list
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 6457359 */
name|flags
operator||=
...
block|{
comment|/* Find an existing declaration for _NSConcreteStackBlock or declare 	 extern void *_NSConcreteStackBlock; */
if|if
condition|(
name|NSConcreteStackBlock_decl
operator|==
...
condition|)
block|{
comment|/* For variables declared __block, either the original one 	     at the point of declaration or the imported version (which is 	     initialized in the helper function's prologue) is used to  	     initilize the byref variable field in the temporary. */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* We will be using the __block_struct_variable.__forwarding as the  	     initializer. */
name|forwarding_expr
operator|=
name|build_component_ref
argument_list|(
...
comment|/* Global variable is always assumed passed by its address. */
name|forwarding_expr
operator|=
name|build_fold_addr_expr
argument_list|(
...
begin_comment
comment|/**  build_block_literal_tmp - This routine:   1) builds block type:  struct __block_literal_n {   void *__isa; // initialized to&_NSConcreteStackBlock or&_NSConcreteGlobalBlock   int __flags;   int __reserved;   void *__FuncPtr   struct __block_descriptor {     unsigned long int reserved;     // NULL     unsigned long int Size;  // sizeof(struct Block_literal_1)      // optional helper functions     void *CopyFuncPtr; // When BLOCK_HAS_COPY_DISPOSE     void *DestroyFuncPtr; // When BLOCK_HAS_COPY_DISPOSE  } *__descriptor;   // imported variables  int x; // ref variable list ...  int *y; // byref variable list  };   2) build function prototype:  double helper_1(struct __block_literal_n *ii, int z);   3) build the temporary initialization:  struct __block_literal_n I = {&_NSConcreteStackBlock or&_NSConcreteGlobalBlock // __isa,    BLOCK_USE_STRET | BLOCK_HAS_COPY_DISPOSE | BLOCK_IS_GLOBAL // __flags,    0, // __reserved&helper_1, // __FuncPtr&static_descriptor_variable // __descriptor,    x, // user variables.&y    ...  }; It return the temporary. */
end_comment

begin_function
specifier|static
name|tree
name|build_block_literal_tmp
parameter_list|(
specifier|const
name|char
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* Context will not be known until when the literal is synthesized.      This is more so in the case of nested block literal blocks.  */
name|DECL_CONTEXT
argument_list|(
name|block_holder_tmp_decl
argument_list|)
...
name|current_function_decl
expr_stmt|;
comment|/* In the new ABI, helper function decl. is the initializer for the      descriptor variable which is always declared static. So, it must      have no context; otherwise, gcc thinks that it requires trampoline! when      address of this function is used as initializer. */
name|DECL_CONTEXT
argument_list|(
name|block_impl
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Temporary representing a global block is made global static.  */
comment|/* APPLE LOCAL radar 6230297 */
if|if
condition|(
name|staticBlockTmp
operator|||
...
begin_comment
comment|/** synth_copy_helper_block_func - This function synthesizes   void copy_helper_block (struct block* _dest, struct block *_src) function. */
end_comment

begin_function
specifier|static
name|void
name|synth_copy_helper_block_func
parameter_list|(
name|struct
name|block_sema_info
modifier|*
...
name|arg_info
decl_stmt|;
comment|/* Set up: (struct block* _dest, struct block *_src) parameters. */
name|dst_arg
operator|=
name|build_decl
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 5847213 */
name|DECL_CONTEXT
argument_list|(
name|src_arg
argument_list|)
...
decl_stmt|;
name|tree
name|dst_block_component
decl_stmt|,
name|src_block_component
decl_stmt|;
name|dst_block_component
operator|=
name|build_component_ref
...
name|BLOCK_POINTER_TYPE
condition|)
comment|/* _Block_object_assign(&_dest->myImportedBlock, _src->myImportedClosure, BLOCK_FIELD_IS_BLOCK) */
name|flag
operator|=
name|BLOCK_FIELD_IS_BLOCK
expr_stmt|;
else|else
comment|/* _Block_object_assign(&_dest->myImportedBlock, _src->myImportedClosure, BLOCK_FIELD_IS_OBJECT) */
name|flag
operator|=
...
block|}
comment|/* For each __block declared variable must generate call to:      _Block_object_assign(&_dest->myImportedBlock, _src->myImportedBlock, BLOCK_FIELD_IS_BYREF [|BLOCK_FIELD_IS_WEAK])   */
for|for
control|(
name|chain
operator|=
...
argument_list|)
expr_stmt|;
comment|/* _Block_object_assign(&_dest->myImportedClosure, _src->myImportedClosure, BLOCK_FIELD_IS_BYREF [|BLOCK_FIELD_IS_WEAK]) */
if|if
condition|(
...
specifier|static
name|void
name|synth_destroy_helper_block_func
parameter_list|(
name|struct
name|block_sema_info
modifier|*
...
name|arg_info
decl_stmt|;
comment|/* Set up: (struct block *_src) parameter. */
name|src_arg
operator|=
name|build_decl
argument_list|(
...
name|BLOCK_POINTER_TYPE
condition|)
comment|/* _Block_object_dispose(_src->imported_object_0, BLOCK_FIELD_IS_BLOCK); */
name|flag
operator|=
name|BLOCK_FIELD_IS_BLOCK
expr_stmt|;
else|else
comment|/* _Block_object_dispose(_src->imported_object_0, BLOCK_FIELD_IS_OBJECT); */
name|flag
operator|=
name|BLOCK_FIELD_IS_OBJECT
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* For each __block declared variable must generate call to:    _Block_object_dispose(_src->myImportedClosure, BLOCK_FIELD_IS_BYREF[|BLOCK_FIELD_IS_WEAK])    */
for|for
control|(
name|chain
operator|=
...
name|BLOCK_FIELD_IS_WEAK
expr_stmt|;
comment|/* _Block_object_dispose(_src->myImportedClosure, BLOCK_FIELD_IS_BYREF[|BLOCK_FIELD_IS_WEAK]) */
name|call_exp
operator|=
name|build_block_object_dispose_call_exp
argument_list|(
...
begin_comment
comment|/* Parse a block-id.     GNU Extension:     block-id:      specifier-qualifier-list block-declarator     Returns the DECL specified or implied.  */
end_comment

begin_function
specifier|static
name|tree
name|c_parser_block_id
parameter_list|(
name|c_parser
modifier|*
name|parser
...
begin_comment
comment|/* Parse a block-literal-expr.     GNU Extension:    block-literal-expr:     ^ parameter-declation-clause exception-specification [opt] compound-statement     ^ block-id compound-statement      It synthesizes the helper function for later generation and builds     the necessary data to represent the block literal where it is     declared.  */
end_comment
...
decl_stmt|;
name|tree
name|block_helper_function_decl
decl_stmt|;
name|tree
name|expr
decl_stmt|,
...
name|tree
name|declared_block_return_type
init|=
name|NULL_TREE
decl_stmt|;
comment|/* APPLE LOCAL radar 6237713 */
name|tree
name|attributes
init|=
name|NULL_TREE
...
condition|)
block|{
comment|/* Parse the optional argument list */
name|c_parser_consume_token
argument_list|(
name|parser
argument_list|)
expr_stmt|;
comment|/* Open the scope to collect parameter decls */
name|push_scope
argument_list|()
expr_stmt|;
name|args
...
argument_list|)
expr_stmt|;
comment|/* Check for args as it might be NULL due to error. */
if|if
condition|(
name|args
condition|)
...
condition|)
block|{
comment|/* Parse user declared return type. */
name|tree
name|decl
decl_stmt|;
comment|/* APPLE LOCAL begin radar 6237713 */
...
literal|0
argument_list|,
literal|"attributes before block type are ignored"
argument_list|)
expr_stmt|;
name|attributes
operator|=
...
expr_stmt|;
block|}
comment|/* Must also build hidden parameter .block_descriptor added to the helper    function, even though we do not know its type yet. */
...
literal|1
expr_stmt|;
comment|/* Prevent unused parameter '.block_descriptor' warning. */
name|TREE_CHAIN
argument_list|(
name|self_arg
argument_list|)
...
name|self_arg
expr_stmt|;
comment|/* APPLE LOCAL begin radar 6185344 */
comment|/* Build the declaration of the helper function (if we do not know its result      type yet, assume it is 'void'. If user provided it, use it).      Treat this as a nested function and use nested function infrastructure for      its generation. */
name|ftype
operator|=
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 6185344 */
comment|/* APPLE LOCAL radar 6160536 - radar 6411649 */
name|block_helper_function_decl
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Set block's scope to the scope of the helper function's main body.      This is primarily used when nested blocks are declared. */
comment|/* FIXME: Name of objc_get_current_scope needs to get changed. */
name|cur_block
operator|->
name|the_scope
operator|=
...
argument_list|()
expr_stmt|;
comment|/* Enter parameter list to the scope of the helper function. */
name|store_parm_decls_from
argument_list|(
name|cur_block
operator|->
...
expr_stmt|;
comment|/* APPLE LOCAL end radar 6237713 */
comment|/* Start parsing body or expression part of the block literal. */
if|if
condition|(
name|c_parser_next_token_is
argument_list|(
...
block|{
name|tree
name|save_c_break_label
init|=
name|c_break_label
decl_stmt|;
name|tree
name|save_c_cont_label
init|=
name|c_cont_label
decl_stmt|;
...
name|error
argument_list|(
literal|"return not allowed in block expression literal"
argument_list|)
expr_stmt|;
return|return
name|clean_and_exit
...
name|error
argument_list|(
literal|"argument list is required for block expression literals"
argument_list|)
expr_stmt|;
return|return
name|clean_and_exit
...
argument_list|)
return|;
comment|/* Now that we know type of the hidden .block_descriptor argument, fix its type. */
name|TREE_TYPE
argument_list|(
name|self_arg
argument_list|)
...
name|block_arg_ptr_type
expr_stmt|;
comment|/* The DECL_RESULT should already have the correct type by now.  */
name|gcc_assert
argument_list|(
name|TREE_TYPE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* We are done parsing of the block body. Return type of block is now known.      We also know all we need to know about the helper function. So, fix its     type here. */
comment|/* We moved this here because for global blocks, helper function body is      not nested and is gimplified in call to finish_function() and return type       of the function must be correct. */
name|ftype
operator|=
name|build_function_type
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Declare helper function; as in:      double helper_1(struct block_1 *ii, int z); */
name|typelist
operator|=
name|TYPE_ARG_TYPES
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* (struct block_1 *ii, int z, ...) */
name|typelist
operator|=
name|tree_cons
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* Build the declaration for copy_helper_block and destroy_helper_block    helper functions for later use. */
if|if
condition|(
name|cur_block
operator|->
...
condition|)
block|{
comment|/* void copy_helper_block (struct block*, struct block *); */
name|tree
name|s_ftype
init|=
name|build_function_type
...
name|name
argument_list|,
literal|"__copy_helper_block_%d"
argument_list|,
name|unique_count
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* void destroy_helper_block (struct block*); */
name|s_ftype
operator|=
name|build_function_type
argument_list|(
...
name|name
argument_list|,
literal|"__destroy_helper_block_%d"
argument_list|,
name|unique_count
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Build unqiue name of the temporary used in code gen. */
name|sprintf
argument_list|(
name|name
argument_list|,
literal|"__block_holder_tmp_%d"
argument_list|,
name|unique_count
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ ce) */
end_comment
