...
begin_comment
comment|/* Various versions of targetm.eh_frame_section.  Note these must appear    outside the DWARF2_DEBUGGING_INFO || DWARF2_UNWIND_INFO macro guards.  */
end_comment

begin_comment
comment|/* Version of targetm.eh_frame_section for systems with named sections.  */
end_comment

begin_function
name|void
name|named_section_eh_frame_section
parameter_list|(
name|void
parameter_list|)
block|{
...
begin_comment
comment|/* Version of targetm.eh_frame_section for systems using collect2.  */
end_comment

begin_function
name|void
name|collect2_eh_frame_section
parameter_list|(
name|void
parameter_list|)
block|{
...
begin_comment
comment|/* Default version of targetm.eh_frame_section.  */
end_comment
...
argument_list|)
name|size_t
name|file_table_last_lookup_index
expr_stmt|;
end_expr_stmt
...
argument_list|(
argument|(length (
literal|"decl_die_table_allocated"
argument|))
argument_list|)
name|dw_die_ref
operator|*
...
begin_comment
comment|/* Number of file tables emitted in maybe_emit_file().  */
end_comment
...
begin_endif
unit|static const char *dwarf_type_encoding_name (unsigned);
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* The DWARF2 pubname for a nested thingy looks like "A::f".  The    output of lang_hooks.decl_printable_name for C++ looks like    "A::f(int)".  Let's drop the argument list, and maybe the scope.  */
end_comment
...
name|idx
decl_stmt|;
comment|/* Handle the case where file_table is empty.  */
if|if
condition|(
...
literal|0
return|;
comment|/* Otherwise this is a generic code; we should just lists all of 	 these explicitly.  Aborting means we forgot one.  */
name|abort
argument_list|()
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* Need loc_descriptor_from_tree since that's where we know 	     how to handle TLS variables.  Want the object's address 	     since the top-level DW_AT_location assumes such.  See 	     the confusion in loc_descriptor for reference.  */
name|descr
operator|=
name|loc_descriptor_from_tree
argument_list|(
...
begin_endif
unit|static const char * decl_start_label (tree decl) {   rtx x;   const char *fnname;    x = DECL_RTL (decl);   if (GET_CODE (x) != MEM)     abort ();    x = XEXP (x, 0);   if (GET_CODE (x) != SYMBOL_REF)     abort ();    fnname = XSTR (x, 0);   return fnname; }
endif|#
directive|endif
end_endif
...
block|if (current_function_needs_context) 	add_AT_location_description (subr_die, DW_AT_static_link, 			     loc_descriptor (lookup_static_chain (decl)));
endif|#
directive|endif
block|}
...
begin_comment
comment|/* Returns the DIE for namespace NS or aborts.     Note that namespaces don't really have a lexical context, so there's no    need to pass in a context_die.  They always go inside their containing    namespace, or comp_unit_die if none.  */
end_comment
...
condition|)
block|{
comment|/* A reference to a file.  Make sure the file name is emitted.  */
name|a
operator|->
name|dw_attr_val
operator|.
...
argument_list|)
expr_stmt|;
comment|/* Get rid of nodes that aren't marked.  */
name|prune_unused_types_prune
argument_list|(
name|comp_unit_die
argument_list|)
