...
function_decl|)
parameter_list|(
comment|/* tree decl, int verbosity */
parameter_list|)
function_decl|;
end_function_decl
...
begin_comment
comment|/* Highest label number used at the end of reload.  */
end_comment
...
literal|1
block|,
literal|"Perform loop unrolling when interation count is known"
block|}
block|,
block|{
...
literal|1
block|,
literal|"Perofm loop onrolling for all loops"
block|}
block|,
block|{
...
literal|1
block|,
literal|"Assume structure / array reference and fixed scalar cannot alias"
block|}
block|,
block|{
...
literal|1
block|,
literal|"Use profiling information for branch porbabilities"
block|}
block|,
block|{
literal|"fast-math"
...
literal|1
block|,
literal|"Enables a regoster move optimisation"
block|}
block|,
block|{
...
block|,
literal|"Allow the use of $ inside indentifiers"
block|}
block|,
block|{
...
literal|"-fno-ident"
block|,
literal|"Ignore #ident directives"
block|}
block|,
block|{
...
block|,
literal|"Warn about subscripts whoes type is 'char'"
block|}
block|,
block|{
...
block|,
literal|"Warn about constructs whoes meaning change in ANSI C"
block|}
block|,
block|{
...
block|,
literal|"Generate code for GNU runtime envrionment"
block|}
block|,
block|{
...
expr_stmt|;
block|}
comment|/* Clear the dump files file.  */
if|if
condition|(
name|rtl_dump
condition|)
...
argument_list|,
argument|{ 	       recompute_reg_usage (insns); 	       regclass (insns, max_reg_num ()); 	       local_alloc (); 	     }
argument_list|)
empty_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Save the last label number used so far, so reorg can tell      when it's safe to kill spill regs.  */
name|max_label_num_after_reload
operator|=
name|max_label_num
argument_list|()
...
block|}
block|}
comment|/* Optimizing for size forces optimize to be no less than 2. */
if|if
condition|(
name|optimize_size
operator|&&
...
literal|1
expr_stmt|;
comment|/* We don't set flag_strict_aliasing here because we're still 	 testing the functionality.  After it has been tested, it 	 should be turned on here.  */
block|}
if|if
condition|(
name|optimize
...
argument_list|)
decl_stmt|;
comment|/* BEGIN CYGNUS LOCAL  --help/nickc */
if|if
condition|(
operator|!
name|strcmp
...
argument_list|)
expr_stmt|;
comment|/* ??? This error message is incorrect in the case of 			     -g4 -g.  */
name|warning
argument_list|(
...
directive|endif
comment|/* ! OS2&& ! VMS&& (! _WIN32 || CYGWIN32) */
if|if
condition|(
name|errorcount
condition|)
