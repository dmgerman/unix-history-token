...
begin_comment
comment|/* Top level of GNU C compiler    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Describes all the dump files.  Should be kept in order of the    pass and in sync with dump_file_index above.     Remaining -d letters:  	"              o q         " 	"       H JK   OPQ  TUV  YZ" */
end_comment
...
begin_comment
comment|/* Nonzero means disable transformations observable by signaling NaNs.    This option implies that any operation on a IEEE signaling NaN can    generate a (user-visible) trap.  */
end_comment
...
name|N_
argument_list|(
literal|"Perform superblock formation via tail duplication"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Perform the loop optimizations"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Perform cross-jumping optimization"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Perform conversion of conditional jumps to branchless equivalents"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Perform conversion of conditional jumps to conditional execution"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Enable basic program profiling code"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Reorder functions to improve code placement"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Add extra commentary to assembler output"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Attempt to merge identical constants across compilation units"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Put zero initialized data in the bss section"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Assume no NaNs or +-Infs are generated"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Disable optimizations observable by IEEE signaling NaNs"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Generate code to check bounds before indexing arrays"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Use graph coloring register allocation."
argument_list|)
block|}
block|, }
decl_stmt|;
...
name|N_
argument_list|(
literal|"Compile just for ISO C90"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Do not warn about compile-time integer division by zero"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Warn whenever a function's return-type defaults to int"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Warn about constructs whose meanings change in ISO C"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Warn about enumerated switches, with no default, missing a case"
argument_list|)
block|}
block|,
block|{
literal|"switch-default"
block|,
operator|&
name|warn_switch_default
block|,
...
name|N_
argument_list|(
literal|"Warn about enumerated switches missing a default case"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Warn about all enumerated switches missing a specific case"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"Warn about code which might break the strict aliasing rules"
argument_list|)
block|}
block|}
decl_stmt|;
...
begin_comment
comment|/* This calls abort and is used to avoid problems when abort is a macro.    It is used when we need to pass the address of abort.  */
end_comment
...
begin_comment
comment|/* Output NAME into FILE after having turned it into something    usable as an identifier in a target's assembly file.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* Make it look like a valid identifier for an assembler.  */
...
block|{
comment|/* ??? Note that we used to just make it look like if 		 the "inline" keyword was specified when we decide 		 to inline it (because of -finline-functions). 		 garloff at suse dot de, 2002-04-24: Add another flag to 		 actually record this piece of information.  */
if|if
condition|(
operator|!
name|DECL_INLINE
...
argument_list|()
expr_stmt|;
comment|/* We have to issue these warnings now already, because CFG cleanups      further down may destroy the required information.  */
...
argument_list|)
comment|/* If flow2 creates new instructions which need splitting      and scheduling after reload is not done, they might not be      splitten until final which doesn't allow splitting      if HAVE_ATTR_length.  */
ifdef|#
directive|ifdef
name|INSN_SCHEDULING
if|if
...
argument_list|)
expr_stmt|;
comment|/* Last attempt to optimize CFG, as scheduling, peepholing and insn 	 splitting possibly introduced more crossjumping oppurtuntities. 	 Except that we can't actually run crossjumping without running 	 another DCE pass, which we can't do after reg-stack.  */
name|cleanup_cfg
argument_list|(
name|CLEANUP_EXPENSIVE
operator||
...
name|_
argument_list|(
literal|"  -ftls-model=[global-dynamic | local-dynamic | initial-exec | local-exec] Indicates the default thread-local storage code generation model\n"
argument_list|)
argument_list|)
expr_stmt|;
name|printf
...
name|_
argument_list|(
literal|"  -fstack-limit-register=<register>  Trap if the stack goes past<register>\n"
argument_list|)
argument_list|)
expr_stmt|;
name|printf
...
name|_
argument_list|(
literal|"  -fstack-limit-symbol=<name>  Trap if the stack goes past symbol<name>\n"
argument_list|)
argument_list|)
expr_stmt|;
name|printf
...
name|_
argument_list|(
literal|"  -frandom-seed=<string>  Make compile reproducible using<string>\n"
argument_list|)
argument_list|)
expr_stmt|;
for|for
...
name|warning
argument_list|(
literal|"`%s': unknown tls-model option"
argument_list|,
name|arg
operator|-
literal|2
...
argument_list|)
condition|)
comment|/* Recognize this switch but do nothing.  This prevents warnings        about an unrecognized switch if cpplib has not been linked in.  */
empty_stmt|;
else|else
return|return
literal|0
...
name|p
decl_stmt|;
comment|/* Fill in the -frandom-seed option, if the user didn't pass it, so      that it can be printed below.  This helps reproducibility.  Of      course, the string may never be used, but we can't tell that at      this point in the compile.  */
...
begin_comment
comment|/* Parse command line options and set default flag values, called    after language-independent option-independent initialization.  Do    minimal options processing.  Outputting diagnostics is OK, but GC    and identifier hashtables etc. are not initialized yet.     Return nonzero to suppress compiler back end initialization.  */
end_comment
...
parameter_list|()
block|{
comment|/* init_emit_once uses reg_raw_mode and therefore must be called      after init_regs which initialized reg_raw_mode.  */
name|init_regs
argument_list|()
expr_stmt|;
name|init_emit_once
...
argument_list|()
expr_stmt|;
comment|/* If an error has already occurred, give up.  */
if|if
condition|(
name|errorcount
condition|)
