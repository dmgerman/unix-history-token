...
begin_comment
comment|/* Mainly the interface between cpplib and the C front ends.    Copyright (C) 1987, 1988, 1989, 1992, 1994, 1995, 1996, 1997    1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005    Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* If we need to translate characters received.  This is tri-state:    0 means use only the untranslated string; 1 means use only    the translated string; -1 means chain the translated string    to the untranslated one.  */
end_comment
...
begin_comment
comment|/* True if strings should be passed to the caller of c_lex completely    unmolested (no concatenation, no translation).  */
end_comment
...
name|toplevel
decl_stmt|;
comment|/* The get_fileinfo data structure must be initialized before      cpp_read_main_file is called.  */
name|toplevel
operator|=
name|get_fileinfo
argument_list|(
...
begin_comment
comment|/* Read a token and return its type.  Fill *VALUE with its value, if    applicable.  Fill *CPP_FLAGS with the token's flags, if it is    non-NULL.  */
end_comment
...
name|CPP_N_INTEGER
case|:
comment|/* C++ uses '0' to mark virtual functions as pure. 	       Set PURE_ZERO to pass this information to the C++ parser.  */
if|if
condition|(
name|tok
operator|->
...
begin_comment
comment|/* Returns the narrowest C-visible unsigned type, starting with the    minimum specified by FLAGS, that can fit HIGH:LOW, or itk_none if    there isn't one.  */
end_comment
...
name|pedwarn
argument_list|(
literal|"integer constant is too large for %qs type"
argument_list|,
operator|(
name|flags
operator|&
...
argument_list|)
expr_stmt|;
comment|/* Both C and C++ require a diagnostic for a floating constant      outside the range of representable values of its type.  Since we      have __builtin_inf* to produce an infinity, it might now be      appropriate for this to be a mandatory pedwarn rather than      conditioned on -pedantic.  */
if|if
condition|(
...
name|pedwarn
argument_list|(
literal|"floating constant exceeds range of %qT"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|/* Assume that, if we managed to translate the string above, 	     then the untranslated parsing will always succeed.  */
name|gcc_assert
argument_list|(
name|xlated
argument_list|)
...
condition|)
block|{
comment|/* Arrange for us to return the untranslated string in 		 *valp, but to set up the C type of the translated 		 one.  */
operator|*
name|valp
operator|=
name|build_string
...
name|char_type_node
expr_stmt|;
comment|/* Cast to cppchar_signed_t to get correct sign-extension of RESULT      before possibly widening to HOST_WIDE_INT for build_int_cst.  */
if|if
condition|(
name|unsignedp
operator|||
