...
begin_comment
comment|/* BEGIN stuff from gcc/cccp.c.  */
end_comment
...
begin_comment
comment|/* END stuff from gcc/cccp.c.  */
end_comment
...
define|#
directive|define
name|FFECOM_FASTER_ARRAY_REFS
value|0
end_define

begin_comment
comment|/* Generates faster code? */
end_comment
...
begin_comment
comment|/* tree.h declares a bunch of stuff that it expects the front end to    define.  Here are the definitions, which in the C front end are    found in the file c-decl.c.  */
end_comment
...
begin_comment
comment|/* ~~tree.h SHOULD declare this, because toplev.c and dwarfout.c reference    it.  */
end_comment
...
name|type
decl_stmt|;
comment|/* Base type (w/o size/array applied). */
name|tree
name|t
decl_stmt|;
name|ffetargetCharacterSize
...
ifdef|#
directive|ifdef
name|SOMEONE_GETS_DEBUG_SUPPORT_WORKING
end_ifdef
...
begin_endif
unit|static tree ffecom_type_permanent_copy_ (tree t);
endif|#
directive|endif
end_endif
...
specifier|static
name|void
name|ffecom_initialize_char_syntax_
parameter_list|(
name|void
parameter_list|)
function_decl|;
...
begin_comment
comment|/* NOTE: g77 currently doesn't use these; see setting of sizetype and    change that if you need to.	-- jcb 09/01/91. */
end_comment
...
define|#
directive|define
name|ffecom_start_compstmt_
value|bison_rule_pushlevel_
end_define

begin_define
define|#
directive|define
name|ffecom_end_compstmt_
value|bison_rule_compstmt_
end_define
...
begin_comment
comment|/* ffecom_char_args_x_ -- Return ptr/length args for char subexpression     tree ptr_arg;    tree length_arg;    ffebld expr;    bool with_null;    ffecom_char_args_x_(&ptr_arg,&length_arg,expr,with_null);     Handles CHARACTER-type CONTER, SYMTER, SUBSTR, ARRAYREF, and FUNCREF    subexpressions by constructing the appropriate trees for the ptr-to-    character-text and length-of-character-text arguments in a calling    sequence.     Note that if with_null is TRUE, and the expression is an opCONTER,    a null byte is appended to the string.  */
end_comment
...
begin_comment
comment|/* ffecom_concat_list_new_ -- Make list of concatenated string exprs     ffecomConcatList_ catlist;    ffebld expr;	 // Root expr of CHARACTER basictype.    ffetargetCharacterSize max;	// max chars to gather or _...NONE if no max    catlist = ffecom_concat_list_new_(expr,max);     Returns a flattened list of concatenated subexpressions given a    tree of such expressions.  */
end_comment
...
name|error_mark_node
return|;
comment|/* Make sure non-const ref is to 					   non-reg. */
comment|/* Build up ARRAY_REFs in reverse order (since we're column major 	   here in Fortran land). */
for|for
control|(
name|i
...
argument_list|()
expr_stmt|;
comment|/* Even though we might not call. */
block|{
name|ffebld
name|left
init|=
...
begin_comment
comment|/* ffecom_let_char_ -- Do assignment stuff for character type     tree dest_tree;  // destination (ADDR_EXPR)    tree dest_length;  // length (INT_CST/INDIRECT_REF(PARM_DECL))    ffetargetCharacterSize dest_size;  // length    ffebld source;  // source expression    ffecom_let_char_(dest_tree,dest_length,dest_size,source);     Generates code to do the assignment.	 Used by ordinary assignment    statement handler ffecom_let_stmt and by statement-function    handler to generate code for a statement function.  */
end_comment
...
begin_comment
comment|/* ffecom_type_localvar_ -- Build type info for non-dummy variable     tree type;    ffesymbol s;	 // the variable's symbol    ffeinfoBasictype bt;	 // it's basictype    ffeinfoKindtype kt; // it's kindtype     type = ffecom_type_localvar_(s,bt,kt);     Handles static arrays, CHARACTER type, etc.	*/
end_comment
...
begin_comment
comment|/* Make a copy of a type, assuming caller has switched to the permanent    obstacks and that the type is for an aggregate (array) initializer.  */
end_comment
...
specifier|static
name|tree
name|ffecom_type_permanent_copy_
parameter_list|(
name|tree
name|t
parameter_list|)
...
begin_comment
comment|/* ffecom_call_gfrt -- Generate call to run-time function     tree expr;    expr = ffecom_call_gfrt(FFECOM_gfrtSTOPNIL,NULL_TREE);     The first arg is the GNU Fortran Run-Time function index, the second    arg is the list of arguments to pass to it.	Returned is the expression    (WITHOUT TREE_SIDE_EFFECTS set!) that makes the call and returns the    result (which may be void).	*/
end_comment
...
begin_comment
comment|/* ffecom_constantunion -- Transform constant-union to tree     ffebldConstantUnion cu;  // the constant to transform    ffeinfoBasictype bt;	 // its basic type    ffeinfoKindtype kt;	// its kind type    tree tree_type;  // ffecom_tree_type[bt][kt]    ffecom_constantunion(&cu,bt,kt,tree_type);  */
end_comment
...
begin_comment
comment|/* ffecom_expand_let_stmt -- Compile let (assignment) statement     ffebld dest;    ffebld source;    ffecom_expand_let_stmt(dest,source);     Convert dest and source using ffecom_expr, then join them    with an ASSIGN op and pass the whole thing to expand_expr_stmt.  */
end_comment
...
begin_comment
comment|/* Wrapper for get_identifier.  pattern is like "...%s...", text is    inserted into final name in place of "%s", or if text is NULL,    pattern is like "...%d..." and text form of number is inserted    in place of "%d".  */
end_comment
...
block|}
block|}
comment|/* Set the sizetype before we do anything else.  This _should_ be the      first type we create.  */
name|t
operator|=
...
operator|==
name|FFECOM_targetGCC
block|ffetargetOffset size;
comment|/* The size of the entity. */
block|ffetargetAlign pad;
comment|/* Its initial padding. */
endif|#
directive|endif
if|if
condition|(
...
operator|==
name|FFECOM_targetGCC
comment|/* For GNU backend, just turn ACCTER into ARRTER and proceed. */
block|size = ffebld_accter_size (init);       pad = ffebld_accter_pad (init);       ffebit_kill (ffebld_accter_bits (init));       ffebld_set_op (init, FFEBLD_opARRTER);       ffebld_set_arrter (init, ffebld_accter (init));       ffebld_arrter_set_size (init, size);       ffebld_arrter_set_pad (init, size);
endif|#
directive|endif
if|#
directive|if
...
directive|if
name|FFECOM_ONEPASS
comment|/* Process the inits, wipe 'em out. */
name|ffestorag_set_init
argument_list|(
name|st
argument_list|,
...
condition|)
return|return;
comment|/* Oh, we already did this! */
if|#
directive|if
name|FFECOM_targetCURRENT
operator|==
...
begin_comment
comment|/* Mark latest temp with given tree as no longer in use.  */
end_comment
...
name|assert
argument_list|(
literal|"couldn't ffecom_pop_tempvar!"
operator|!=
name|NULL
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Prepare to make call-arg temps.     Call this in pairs with pop_calltemps around calls to    ffecom_arg_ptr_to_expr if the latter might use temporaries.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* This temp must be put in the same scope as the containing BLOCK      (aka function), but for reasons that should be explained elsewhere,      the GBE normally decides it should be in a "phantom BLOCK" associated      with the expand_start_stmt_expr() call.  So push the topmost      sequence back onto the GBE's internal stack before telling it      about the decl, then restore it afterwards.  */
...
begin_comment
comment|/* ~~tree.h SHOULD declare this, because toplev.c references it.  */
end_comment

begin_comment
comment|/* Can't 'yydebug' a front end not generated by yacc/bison!  */
end_comment
...
name|fatal
argument_list|(
literal|"Directory name must immediately follow -I option with no intervening spaces, as in `-Idir', not `-I dir'"
argument_list|)
expr_stmt|;
name|dirtmp
operator|->
...
block|if (index (fname, '.') == NULL) 		strcat (fname, ".h");
endif|#
directive|endif
block|}
endif|#
