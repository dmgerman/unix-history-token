...
specifier|static
name|tree
name|ffe_signed_or_unsigned_type
parameter_list|(
name|int
parameter_list|,
name|tree
...
specifier|static
name|tree
name|ffe_truthvalue_conversion
parameter_list|(
name|tree
parameter_list|)
function_decl|;
...
value|(struct f_binding_level *) NULL
end_define
...
begin_comment
comment|/* Fortran doesn't use either of these.  */
end_comment
...
begin_comment
comment|/* Return the subscript expression, modified to do range-checking.     `array' is the array type to be checked against.    `element' is the subscript expression to check.    `dim' is the dimension number (starting at 0).    `total_dims' is the total number of dimensions (0 for CHARACTER substring).    `item' is the array decl or NULL_TREE. */
end_comment
...
block|}
block|}
comment|/* If the array index is safe at compile-time, return element.  */
if|if
condition|(
...
begin_comment
comment|/* Return the computed element of an array reference.     `item' is NULL_TREE, or the transformed pointer to the array.    `expr' is the original opARRAYREF expression, which is transformed      if `item' is NULL_TREE.    `want_ptr' is nonzero if a pointer to the element, instead of      the element itself, is to be returned.  */
end_comment
...
begin_comment
comment|/* Transform constant-union to tree, with the type known.  */
end_comment
...
name|assert
argument_list|(
literal|"constant type not supported"
operator|==
name|NULL
argument_list|)
expr_stmt|;
...
decl_stmt|,
name|float_ftype_float_float
decl_stmt|;
name|tree
name|ldouble_ftype_ldouble
decl_stmt|,
name|ldouble_ftype_ldouble_ldouble
decl_stmt|;
name|tree
...
argument_list|)
expr_stmt|;
comment|/* Copy the assembler name.  */
...
begin_comment
comment|/* Create a new `struct f_binding_level'.  */
end_comment
...
directive|define
name|LANG_HOOKS_PRINT_ERROR_FUNCTION
value|ffe_print_error_function
end_define
...
directive|define
name|LANG_HOOKS_TRUTHVALUE_CONVERSION
value|ffe_truthvalue_conversion
end_define
...
directive|define
name|LANG_HOOKS_SIGNED_OR_UNSIGNED_TYPE
value|ffe_signed_or_unsigned_type
end_define
...
argument_list|)
expr_stmt|;
comment|/* FIXME: The ffelex_hash_kludge code needs to be cleaned up to      set the new file name.  Maybe in ffe_post_options.  */
return|return
name|true
return|;
block|}
...
name|name_map
decl_stmt|;
comment|/* Nonzero if name_map is valid.  */
name|int
name|got_name_map
decl_stmt|;
block|}
