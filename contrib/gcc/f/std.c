...
begin_elif
unit|ffestt_exprlist_dump (exprlist);   fputs ("),", dmpout);
elif|#
directive|elif
name|FFECOM_targetCURRENT
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Don't bother with name. */
else|#
directive|else
block|{
name|ffestdStmt_
...
directive|else
comment|/* Nothing to do, it's implicit. */
endif|#
directive|endif
block|}
end_function
...
directive|if
literal|0
comment|/* Normally no need to preserve the 				   expression. */
block|ffesymbol_set_sfexpr (s, NULL);
comment|/* Except expr.c sees NULL 					   as recursive reference! 					   So until we can use something 					   convenient, like a "permanent" 					   expression, don't worry about 					   wasting some memory in the 					   stand-alone FFE. */
else|#
directive|else
name|ffesta_set_outpooldisp
argument_list|(
...
directive|if
literal|0
comment|/* Must preserve the expression for gcc. */
block|ffesymbol_set_sfexpr (s, NULL);
else|#
directive|else
name|ffesta_set_outpooldisp
