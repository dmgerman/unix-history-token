...
block|if (ffe_is_warn_globals ()) 	    { 	      ffebad_start (FFEBAD_FILEWIDE_TIFF); 	      ffebad_string (ffelex_token_text (t)); 	      ffebad_string (ffeglobal_type_string_[type]); 	      ffebad_string (ffeglobal_type_string_[g->type]); 	      ffebad_here (0, ffelex_token_where_line (t), 			   ffelex_token_where_column (t)); 	      ffebad_here (1, ffelex_token_where_line (g->t), 			   ffelex_token_where_column (g->t)); 	      ffebad_finish (); 	    }
endif|#
directive|endif
block|}
elseif|else
...
expr_stmt|;
block|}
comment|/* Else, make sure there is type agreement.  */
elseif|else
if|if
condition|(
operator|(
