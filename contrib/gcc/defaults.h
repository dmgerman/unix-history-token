...
parameter_list|)
define|\
value|do { const char *const name_ = (NAME); \        char *const output_ = (OUTPUT) = alloca (strlen (name_) + 32);\        sprintf (output_, ASM_PN_FORMAT, name_, (unsigned long)(LABELNO)); \   } while (0)
end_define
...
parameter_list|)
define|\
value|do { fputs (integer_asm_op (POINTER_SIZE / UNITS_PER_WORD, TRUE), FILE); \      (*targetm.asm_out.internal_label) (FILE, "L", (VALUE));			\      fputc ('\n', FILE);						\    } while (0)
end_define
