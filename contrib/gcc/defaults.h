...
begin_comment
comment|/* When removal of CPP_PREDEFINES is complete, TARGET_CPU_CPP_BULITINS    can also be removed from here.  */
end_comment
...
parameter_list|)
define|\
value|do							\     {							\       HOST_WIDE_INT size_ = (SIZE);			\       fputs (SIZE_ASM_OP, STREAM);			\       assemble_name (STREAM, NAME);			\       fputs (", ", STREAM);				\       fprintf (STREAM, HOST_WIDE_INT_PRINT_DEC, size_);	\       putc ('\n', STREAM);				\     }							\   while (0)
end_define
