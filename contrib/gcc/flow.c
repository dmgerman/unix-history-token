...
begin_comment
comment|/* TODO:     Split out from life_analysis: 	- local property discovery 	- global property computation 	- log links creation 	- pre/post modify transformation */
end_comment
...
parameter_list|)
define|\
value|(GET_CODE ((x)) == reversed_comparison_code ((y), NULL))
end_define
...
begin_comment
comment|/* This is the maximum number of times we process any given block if the    latest loop depth count is smaller than this number.  Only used for the    failure strategy to avoid infinite loops in calculate_global_regs_live.  */
end_comment

begin_define
define|#
directive|define
name|MAX_LIVENESS_ROUNDS
value|20
end_define
...
name|flags
decl_stmt|;
comment|/* Index of instruction being processed.  */
name|int
name|insn_num
decl_stmt|;
block|}
...
begin_comment
comment|/* When PROP_REG_INFO set, array contains pbi->insn_num of instruction    where given register died.  When the register is marked alive, we use the    information to compute amount of instructions life range cross.    (remember, we are walking backward).  This can be computed as current    pbi->insn_num - reg_deaths[regno].    At the end of processing each basic block, the remaining live registers    are inspected and live ranges are increased same way so liverange of global    registers are computed correctly.       The array is maintained clear for dead registers, so it can be safely reused    for next basic block without expensive memset of the whole array after    reseting pbi->insn_num to 0.  */
end_comment
...
begin_comment
comment|/* Perform data flow analysis for the whole control flow graph.    FLAGS is a set of PROP_* flags to be used in accumulating flow info.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Removing dead insns should have made jumptables really dead.  */
...
condition|)
break|break;
comment|/* We repeat regardless of what cleanup_cfg says.  If there were 	     instructions deleted above, that might have been only a 	     partial improvement (see PARAM_MAX_FLOW_MEMORY_LOCATIONS  usage). 	     Further improvement may be possible.  */
name|cleanup_cfg
argument_list|(
name|CLEANUP_EXPENSIVE
argument_list|)
expr_stmt|;
comment|/* Zap the life information from the last round.  If we don't 	     do this, we can wind up with registers that no longer appear 	     in the code being marked live at entry.  */
name|FOR_EACH_BB
argument_list|(
argument|bb
argument_list|)
...
block|{
comment|/* FIXME: This can go when the dataflow branch has been merged in.  */
comment|/* For a local update, if we are creating new REG_DEAD notes, then we 	 must delete the old ones first to avoid conflicts if they are 	 different.  */
if|if
condition|(
name|prop_flags
operator|&
...
condition|)
block|{
comment|/* The bitmap may be flawed in that one of the basic 		 blocks may have been deleted before you get here.  */
name|COPY_REG_SET
argument_list|(
name|tmp
argument_list|,
...
name|EXECUTE_IF_SET_IN_REG_SET
argument_list|(
argument|ENTRY_BLOCK_PTR->il.rtl->global_live_at_end
argument_list|,
...
begin_comment
comment|/* Free the variables allocated by find_basic_blocks.  */
end_comment
...
name|bb
decl_stmt|;
comment|/* A dead jump table does not belong to any basic block.  Scan insns      between two adjacent basic blocks.  */
name|FOR_EACH_BB
argument_list|(
argument|bb
argument_list|)
...
decl_stmt|,
name|invalidated_by_eh_edge
decl_stmt|;
name|regset
name|registers_made_dead
decl_stmt|;
name|bool
name|failure_strategy_required
init|=
name|false
decl_stmt|;
name|int
...
name|local_sets
decl_stmt|;
comment|/* The registers that are conditionally modified within this block.      In other words, regs that are set only as part of a COND_EXEC.  */
name|regset
modifier|*
name|cond_local_sets
decl_stmt|;
name|unsigned
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|/* The exception handling registers die at eh edges.  */
ifdef|#
directive|ifdef
name|EH_RETURN_DATA_REGNO
for|for
...
directive|endif
comment|/* Allocate space for the sets of local properties.  */
name|local_sets
operator|=
name|XCNEWVEC
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Create a worklist.  Allocate an extra slot for the `head == tail'      style test for an empty queue doesn't work with a full queue.  */
name|queue
operator|=
name|XNEWVEC
argument_list|(
...
name|NULL
expr_stmt|;
comment|/* Should we start using the failure strategy?  */
if|if
condition|(
name|bb
operator|!=
...
continue|continue;
block|}
comment|/* On our first pass through this block, we'll go ahead and continue. 	 Recognize first pass by checking if local_set is NULL for this          basic block.  On subsequent passes, we get to skip out early if 	 live_at_end wouldn't have changed.  */
if|if
condition|(
name|local_sets
index|[
...
condition|)
continue|continue;
comment|/* If any of the changed bits overlap with local_sets[bb],  		 we'll have to rescan the block.  */
name|local_set
operator|=
name|local_sets
index|[
...
condition|)
block|{
comment|/* Get the list of registers that were removed from the 	         bb->global_live_at_start set.  */
...
name|pbb
decl_stmt|;
comment|/* It should not happen that one of registers we have 		     removed last time is disappears again before any other 		     register does.  */
name|pbb_changed
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Now remove the registers from all sets.  */
name|FOR_EACH_BB
argument_list|(
argument|pbb
argument_list|)
...
condition|)
continue|continue;
comment|/* Note the (possible) change.  */
if|if
condition|(
name|blocks_out
condition|)
...
argument_list|)
expr_stmt|;
comment|/* Makes sure to really rescan the block.  */
if|if
condition|(
name|local_sets
index|[
...
block|}
block|}
comment|/* end of failure_strategy_required */
name|COPY_REG_SET
argument_list|(
name|bb
operator|->
...
begin_comment
comment|/* Allocate the permanent data structures that represent the results    of life analysis.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Record sets.  Do this even for dead instructions, since they 	 would have killed the values if they hadn't been deleted.  To 	 be consistent, we also have to emit a clobber when we delete 	 an insn that clobbers a live register.  */
name|pbi
operator|->
name|flags
operator||=
...
argument_list|)
expr_stmt|;
comment|/* Still, we need to update local_set, lest ifcvt.c:dead_or_predicable 	 concludes that the stack pointer is not modified.  */
name|mark_set_regs
argument_list|(
name|pbi
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* We can only track conditional lifetimes if the condition is  	     in the form of a reversible comparison of a register against  	     zero.  If the condition is more complex than that, then it is  	     safe not to record any information.  */
name|inv_cond
operator|=
...
name|PARAM_VALUE
argument_list|(
name|PARAM_MAX_FLOW_MEMORY_LOCATIONS
argument_list|)
condition|)
block|{
ifdef|#
...
begin_comment
comment|/* EXP is a REG or MEM.  Remove any dependent entries from    pbi->mem_set_list.  */
end_comment
...
argument_list|)
operator|)
comment|/* When we get an EXP that is a mem here, we want to check if EXP 	     overlaps the *address* of any of the mems in the list (i.e. not 	     whether the mems actually overlap; that's done elsewhere).  */
operator|||
operator|(
name|MEM_P
argument_list|(
...
name|SIGN_EXTRACT
case|:
comment|/* SIGN_EXTRACT cannot be an lvalue.  */
...
break|break;
block|}
comment|/* If this set is a MEM, then it kills any aliased writes and any      other MEMs which use it.      If this set is a REG, then it kills any MEMs which use the reg.  */
if|if
condition|(
name|optimize
operator|&&
...
name|ncond
decl_stmt|;
comment|/* Otherwise this is a conditional set.  Record that fact. 	 It may have been conditionally used, or there may be a 	 subsequent set with a complementary condition.  */
name|node
operator|=
...
begin_comment
comment|/* Scan expression X for registers which have to be marked used in PBI.      X is considered to be the SET_DEST rtx of SET.  TRUE is returned if    X could be handled by this function.  */
end_comment

begin_function
specifier|static
name|bool
name|mark_used_dest_regs
parameter_list|(
name|struct
name|propagate_block_info
modifier|*
...
name|x
decl_stmt|;
comment|/* On some platforms calls return values spread over several       locations.  These locations are wrapped in a EXPR_LIST rtx      together with a CONST_INT offset.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* distribute_notes in combiner fails to convert some of the      REG_UNUSED notes to REG_DEAD notes.  This causes CHECK_DEAD_NOTES      in sched1 to die.  To solve this update the DEATH_NOTES      here.  */
name|update_life_info
argument_list|(
name|NULL
argument_list|,
...
begin_comment
comment|/* Optionally removes all the REG_DEAD and REG_UNUSED notes from a set of    blocks.  If BLOCKS is NULL, assume the universal set.  Returns a count    of the number of registers that died.    If KILL is 1, remove old REG_DEAD / REG_UNUSED notes.  If it is 0, don't.    if it is -1, remove them unless they pertain to a stack reg.  */
end_comment
...
name|unsigned
name|int
name|rest_of_handle_remove_death_notes
parameter_list|(
name|void
parameter_list|)
block|{
