...
begin_comment
comment|/* Return an exclusive upper bound on the registers occupied by hard    register (reg:MODE REGNO).  */
end_comment
...
begin_comment
comment|/* Likewise for hard register X.  */
end_comment
...
name|X
parameter_list|)
value|end_hard_regno (GET_MODE (X), REGNO (X))
end_define

begin_comment
comment|/* Likewise for hard or pseudo register X.  */
end_comment
...
name|X
parameter_list|)
value|(HARD_REGISTER_P (X) ? END_HARD_REGNO (X) : REGNO (X) + 1)
end_define

begin_comment
comment|/* Return true if (reg:MODE REGNO) includes an element of REGS.  */
end_comment
