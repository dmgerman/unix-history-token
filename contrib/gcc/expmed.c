...
begin_comment
comment|/* Check whether the multiplication X * MULT + ADD overflows.    X, MULT and ADD must be CONST_*.    MODE is the machine mode for the computation.    X and MULT must have mode MODE.  ADD may have a different mode.    So can X (defaults to same as MODE).    UNSIGNEDP is nonzero to do unsigned multiplication.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* In order to get a proper overflow indication from an unsigned      type, we have to pretend that it's a sizetype.  */
name|mult_type
operator|=
name|type
expr_stmt|;
