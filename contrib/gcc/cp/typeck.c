...
argument_list|)
return|;
comment|/* Make sure that the cv-quals change only in the same direction as 	 the target type.  */
block|{
name|int
name|t
decl_stmt|;
...
block|{
comment|/* Just act like build_offset_ref, since the object does                      not matter unless we're actually calling the function.  */
name|tree
name|t
decl_stmt|;
for|for
...
name|cp_error
argument_list|(
literal|"reinterpret_cast cannot convert to type `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"reinterpret_cast cannot convert from type `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"reinterpret_cast cannot convert non-pointer type `%T' to `%T'"
argument_list|,
name|intype
argument_list|,
name|type
...
name|cp_error
argument_list|(
literal|"reinterpret_cast cannot convert `%T' to non-pointer type `%T'"
argument_list|,
name|intype
argument_list|,
name|type
...
name|cp_error
argument_list|(
literal|"const_cast cannot convert to non-pointer type `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"const_cast cannot convert rvalue to type `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"const_cast cannot convert non-pointer type `%T' to type `%T'"
argument_list|,
name|intype
argument_list|,
name|type
...
name|cp_error
argument_list|(
literal|"const_cast cannot convert between pointers to members of different types `%T' and `%T'"
argument_list|,
name|TYPE_OFFSET_BASETYPE
argument_list|(
name|t2
...
name|cp_error
argument_list|(
literal|"const_cast cannot convert unrelated type `%T' to `%T'"
argument_list|,
name|t2
argument_list|,
name|t1
...
name|cp_error
argument_list|(
literal|"`%T' does not define operator="
argument_list|,
name|lhstype
argument_list|)
expr_stmt|;
...
name|warn_synth
condition|)
comment|/* If we care about this, do overload resolution.  */
name|build_opfncall
argument_list|(
name|MODIFY_EXPR
argument_list|,
...
directive|if
literal|0
block|if (IS_AGGR_TYPE (lhstype)) 	{ 	  if (result = build_opfncall (MODIFY_EXPR, 				       LOOKUP_NORMAL, lhs, newrhs, 				       make_node (NOP_EXPR))) 	    return result; 	}
endif|#
directive|endif
...
argument_list|)
expr_stmt|;
comment|/* Can't initialize directly from a TARGET_EXPR, since that would 	 cause the lhs to be constructed twice, and possibly result in 	 accidental self-initialization.  So we force the TARGET_EXPR to be 	 expanded.  expand_expr should really do this by itself.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Get difference in deltas for different pointer to member function    types.  Return integer_zero_node, if FROM cannot be converted to a    TO type.  If FORCE is true, then allow reverse conversions as well.  */
end_comment
...
name|warning
argument_list|(
literal|"pointer to member conversion to virtual base class will only work if you are very careful"
argument_list|)
expr_stmt|;
block|}
return|return
...
name|warning
argument_list|(
literal|"pointer to member conversion from virtual base class will only work if you are very careful"
argument_list|)
expr_stmt|;
block|}
return|return
...
name|sorry
argument_list|(
literal|"value casting of variable nonnull pointer to member functions not supported"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
comment|/* If we're returning in a register, we can't initialize the      return value from a TARGET_EXPR.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
