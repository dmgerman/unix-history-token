...
begin_comment
comment|/* Like complete_type, but issue an error if the TYPE cannot be    completed.  Returns NULL_TREE if the type cannot be made     complete.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* The type of fndecl is a function type, 			     not a pointer-to-function type, since 			     build_vfn_ref returns not the correct 			     vtable slot, but the indirection of the 			     correct vtable slot.  */
name|TREE_TYPE
argument_list|(
name|fndecl
argument_list|)
...
name|cp_error
argument_list|(
literal|"insufficient type information to resolve address of overloaded function `%D'"
argument_list|,
name|DECL_NAME
argument_list|(
name|get_first_fn
...
name|fn
decl_stmt|;
comment|/* We don't require a match here; it's possible that the 	     context (like a cast to a particular type) will resolve 	     the particular choice of template.  */
name|fn
operator|=
...
return|;
block|}
comment|/* Add in the offset to the field.  */
name|offset
operator|=
name|convert
argument_list|(
...
operator|==
name|METHOD_TYPE
comment|/* Don't do the default conversion if we want a 		     pointer to a function.  */
operator|&&
operator|!
operator|(
name|TREE_CODE
