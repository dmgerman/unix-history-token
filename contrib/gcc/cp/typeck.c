...
name|false
return|;
comment|/* fall through to checks for pointer types */
case|case
name|POINTER_TYPE
case|:
if|if
...
decl_stmt|;
name|bool
name|named_return_value_okay_p
decl_stmt|;
operator|*
name|no_warning
operator|=
...
argument_list|)
operator|)
comment|/* The cv-unqualified type of the returned value must be the         same as the cv-unqualified return type of the         function.  */
operator|&&
name|same_type_p
argument_list|(
operator|(
...
name|error_mark_node
return|;
comment|/* Under C++0x [12.8/16 class.copy], a returned lvalue is sometimes 	 treated as an rvalue for the purposes of overload resolution to 	 favor move constructors over copy constructors.  */
if|if
condition|(
name|flag_cpp0x
operator|&&
...
name|TYPE_QUAL_VOLATILE
operator|)
comment|/* The return type must be a class type.  */
operator|&&
name|CLASS_TYPE_P
argument_list|(
name|TREE_TYPE
