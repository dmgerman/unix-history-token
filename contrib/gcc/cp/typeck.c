...
begin_comment
comment|/* Return 1 if TYPE1 and TYPE2 are equivalent exception specifiers.    If EXACT is 0, T2 can be stricter than T1 (according to 15.4/7),    otherwise it must be exact. Exception lists are unordered, but    we've already filtered out duplicates. Most lists will be in order,    we should try to make use of that.  */
end_comment
...
name|strict
decl_stmt|;
comment|/* The special exemption for redeclaring array types without an      array bound only applies at the top level:         extern int (*i)[];        int (*i)[8];       is invalid, for example.  */
name|strict
operator|&=
operator|~
name|COMPARE_REDECLARATION
...
name|error
argument_list|(
literal|"invalid application of `%s' to non-static member"
argument_list|,
name|op_name
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* It's invalid to say `sizeof (X::i)' for `i' a non-static data      member unless you're in a non-static member of X.  So hand off to      resolve_offset_ref.  [expr.prim]  */
elseif|else
if|if
condition|(
name|TREE_CODE
...
argument_list|)
expr_stmt|;
comment|/* Transform `(a, b).x' into `a, b.x' and `(a ? b : c).x' into       `a ? b.x : c.x'.  These transformations should not really be      necessary, but they are.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
block|{
comment|/* The YACC parser sometimes gives us things that are not names. 	     These always indicate errors.  The recursive-descent parser 	     does not do this, so this code can go away once that parser 	     replaces the YACC parser.  */
name|error
argument_list|(
literal|"invalid use of `%D'"
argument_list|,
...
begin_comment
comment|/* Issue an error message if casting from SRC_TYPE to DEST_TYPE casts    away constness.  */
end_comment
...
condition|)
block|{
comment|/* At this point we have checked all of the conditions except 	 that B is not a virtual base class of D.  That will be 	 checked by build_base_path.  */
name|tree
name|base
init|=
name|lookup_base
