...
argument|HOST_PTR_PRINTF_FORMAT
argument_list|,
argument|HOST_PTR_PRINTF_TYPE global_binding_level
argument_list|)
empty_stmt|;
name|fprintf
argument_list|(
...
begin_comment
comment|/* Make a definition for a builtin function named NAME in the current    namespace, whose data type is TYPE and whose context is CONTEXT.    TYPE should be a function type with argument types.     CLASS and CODE tell later passes how to compile calls to this function.    See tree.h for possible values.     If LIBNAME is nonzero, use that for DECL_ASSEMBLER_NAME,    the name to be called if we can't opencode the function.  */
end_comment
...
begin_comment
comment|/* Entry point for the benefit of c_common_nodes_and_builtins.     Make a defintion for a builtin function named NAME and whose data type    is TYPE.  TYPE should be a function type with argument types.  This    function places the anticipated declaration in the global namespace    and additionally in the std namespace if appropriate.     CLASS and CODE tell later passes how to compile calls to this function.    See tree.h for possible values.     If LIBNAME is nonzero, use that for DECL_ASSEMBLER_NAME,    the name to be called if we can't opencode the function.  */
end_comment
...
name|error_mark_node
expr_stmt|;
comment|/* Otherwise, force_store_init_value will have already stored a 	 zero-init initializer in DECL_INITIAL, that should be 	 retained.  */
block|}
return|return
name|init
return|;
