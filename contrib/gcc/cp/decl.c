...
specifier|static
name|void
name|set_identifier_local_value_with_scope
name|PROTO
argument_list|(
operator|(
name|tree
...
begin_comment
comment|/* The value for __null (NULL), either of type `void *' or, with -ansi,    an integer type of the same size.  */
end_comment
...
name|incomplete
decl_stmt|;
comment|/* List of VAR_DECLS saved from a previous for statement.        These would be dead in ANSI-conforming code, but might        be referenced in ARM-era code.  */
name|tree
...
begin_comment
comment|/* Nonzero if we are currently in a toplevel binding level.  This    means either the global binding level or a namespace in a toplevel    binding level.    Since there are no non-toplevel namespace levels, this really    means any namespace or pseudo-global level.  */
end_comment
...
condition|)
block|{
comment|/* In this case keep the dead for-decl visible, 		 but remember what (if anything) it shadowed.  */
...
argument_list|)
expr_stmt|;
comment|/* We first restore the regular decls and *then* the dead_vars_from_for 	 to handle this case:  	 int i; // i#1 	 { 	   for (int i; ; ) { ...} // i#2            int i; // i#3 	 } // we are here  	 In this case, we want remove the binding for i#3, restoring 	 that of i#2.  Then we want to remove the binding for i#2, 	 and restore that of i#1.  */
name|link
operator|=
name|current_binding_level
operator|->
...
begin_comment
comment|/* Concatenate the binding levels of all namespaces. */
end_comment
...
name|names
expr_stmt|;
comment|/* The nested namespaces appear in the names list of their ancestors. */
for|for
control|(
name|current
operator|=
...
literal|0
expr_stmt|;
comment|/* Qualifiers must match, and they may be present on either, the type 	 or the decl.  */
elseif|else
if|if
condition|(
operator|(
...
condition|)
block|{
comment|/* A name N used in a class S shall refer to the same declaration 	     in its context and when re-evaluated in the completed scope of S.  	     Types, enums, and static vars are checked here; other 	     members are checked in finish_struct.  */
name|tree
name|icv
init|=
name|IDENTIFIER_CLASS_VALUE
...
comment|/* XXX: is this a real lookup, considering using-directives etc. ??? */
for|for
control|(
name|tail
operator|=
...
argument_list|)
expr_stmt|;
comment|/* During parsing, we need to complain. */
name|flags
operator||=
...
argument_list|)
expr_stmt|;
comment|/* Add implicit 'typename' to types from template bases.  lookup_field          will do this for us.  If classval is actually from an enclosing          scope, lookup_nested_field will get it for us.  */
if|if
condition|(
name|processing_template_decl
operator|&&
...
argument_list|)
condition|)
comment|/* Not in a nested function.  */
name|val
operator|=
name|locval
expr_stmt|;
...
block|{
comment|/* This is incredibly horrible.  The whole concept of 	     IDENTIFIER_LOCAL_VALUE / IDENTIFIER_CLASS_VALUE / 	     IDENTIFIER_GLOBAL_VALUE needs to be scrapped for local 	     classes.  */
name|tree
name|lctx
init|=
name|hack_decl_function_context
...
comment|/* I don't know which is right; let's just guess for now.  */
name|val
operator|=
name|locval
expr_stmt|;
...
condition|)
empty_stmt|;
comment|/* Caller wants a class-or-namespace-name. */
elseif|else
if|if
condition|(
name|prefer_type
...
directive|else
comment|/* ndef __CYGWIN32__ */
comment|/* Cygwin32 cannot handle catching signals other than      SIGABRT yet.  We hope this will cease to be the case soon. */
ifdef|#
directive|ifdef
name|SIGABRT
name|signal
...
condition|)
block|{
comment|/* See also grok_x_components.  */
name|tree
modifier|*
name|q
decl_stmt|;
...
name|UNKNOWN_TYPE
condition|)
comment|/* decay_conversion is probably wrong for references to functions.  */
name|init
operator|=
...
condition|)
block|{
comment|/* Dynamically initialized vars go into common.  */
if|if
condition|(
name|DECL_INITIAL
argument_list|(
...
block|{
comment|/* Statically initialized vars are weak or comdat, if                  supported.  */
if|if
condition|(
name|flag_weak
condition|)
...
literal|1
expr_stmt|;
comment|/* This decl is not from the current namespace. */
if|if
condition|(
name|in_namespace
condition|)
...
condition|)
block|{
comment|/* Assembler names live in the global namespace. */
name|tmp
operator|=
name|IDENTIFIER_GLOBAL_VALUE
argument_list|(
...
condition|)
block|{
comment|/* We don't do this for specializations since the 		 equivalent checks will be done later.  Also, at this 		 point the DECL_ASSEMBLER_NAME is not yet fully 		 accurate.  */
...
name|cp_pedwarn
argument_list|(
literal|"return-type of `%D' defaults to `int'"
argument_list|,
name|dname
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"`operator %T' cannot be cv-qualified"
argument_list|,
name|ctor_return_type
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* If this involves a template parameter, it'll be 		   constant, but we don't know what the value is yet.  */
if|if
condition|(
name|processing_template_decl
condition|)
...
name|constp
condition|)
comment|/* According to Mike Stump, we generate bad code for 		     this case, so we might as well always make it an 		     error.  */
name|cp_error
argument_list|(
...
name|NULL_TREE
return|;
comment|/* Among other times, could occur from check_explicit_specialization 	   returning an error_mark_node.  */
if|if
condition|(
name|decl
operator|==
...
condition|)
empty_stmt|;
comment|/* Unparsed default arg from in-class decl.  */
elseif|else
if|if
condition|(
name|TREE_CODE
...
name|cp_pedwarn
argument_list|(
literal|"invalid type `%T' for default argument to `%#D'"
argument_list|,
name|TREE_TYPE
argument_list|(
name|init
...
name|cp_pedwarn
argument_list|(
literal|"redeclaration of template type-parameter `%T'"
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|cp_pedwarn_at
argument_list|(
literal|"  previously declared here"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
directive|if
literal|0
block|if (processing_template_decl&& current_function_decl)
endif|#
directive|endif
name|end_temporary_allocation
argument_list|()
...
argument_list|)
expr_stmt|;
comment|/* We have to always copy here; not all INTEGER_CSTs are unshared, 	and there's no wedding ring. Look at size_int()...*/
name|value
operator|=
name|copy_node
argument_list|(
