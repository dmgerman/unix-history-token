...
begin_comment
comment|/* True if a declaration with an `extern' linkage specifier is being    processed.  */
end_comment
...
begin_comment
comment|/* Returns nonzero if T is a virtual function table.  */
end_comment
...
begin_comment
comment|/* Returns nonzero if T is a TYPE_DECL for a type with virtual    functions.  */
end_comment
...
begin_comment
comment|/* Walk the vtable declarations in NAMESPACE.  Whenever one is found    for which P returns nonzero, call F with its address.  If any call    to F returns a nonzero value, return a nonzero value.  */
end_comment
...
begin_comment
comment|/* Walk the vtable declarations.  Whenever one is found for which P    returns nonzero, call F with its address.  If any call to F    returns a nonzero value, return a nonzero value.  */
end_comment
...
begin_comment
comment|/* Walk the global declarations in NAMESPACE.  Whenever one is found    for which P returns nonzero, call F with its address.  If any call    to F returns a nonzero value, return a nonzero value.  */
end_comment
...
begin_comment
comment|/* Walk the global declarations.  Whenever one is found for which P    returns true, call F with its address.  If any call to F    returns true, return true.  */
end_comment
...
begin_comment
comment|/* If NEWDECL is a redeclaration of OLDDECL, merge the declarations.    If the redeclaration is invalid, a diagnostic is issued, and the    error_mark_node is returned.  Otherwise, OLDDECL is returned.     If NEWDECL is not a redeclaration of OLDDECL, NULL_TREE is    returned.  */
end_comment
...
name|warning
argument_list|(
literal|"%Jfunction '%D' redeclared as inline"
argument_list|,
name|newdecl
argument_list|,
name|newdecl
...
name|warning
argument_list|(
literal|"%Jprevious declaration of '%D' with attribute noinline"
argument_list|,
name|olddecl
argument_list|,
name|olddecl
...
name|warning
argument_list|(
literal|"%Jfunction '%D' redeclared with attribute noinline"
argument_list|,
name|newdecl
argument_list|,
name|newdecl
...
name|warning
argument_list|(
literal|"%Jprevious declaration of '%D' was inline"
argument_list|,
name|olddecl
argument_list|,
name|olddecl
...
condition|)
block|{
comment|/* Avoid warnings redeclaring anticipated built-ins.  */
if|if
condition|(
...
expr_stmt|;
block|}
comment|/* Even if the types match, prefer the new declarations type 	 for anticipated built-ins, for exception lists, etc...  */
elseif|else
if|if
condition|(
name|DECL_ANTICIPATED
...
name|error
argument_list|(
literal|"conflicting declaration '%#D'"
argument_list|,
name|newdecl
argument_list|)
expr_stmt|;
name|cp_error_at
argument_list|(
literal|"'%D' has a previous declaration as `%#D'"
argument_list|,
name|olddecl
argument_list|,
name|olddecl
...
name|olddecl
return|;
comment|/* [namespace.alias]           A namespace-name or namespace-alias shall not be declared as 	 the name of any other entity in the same declarative region. 	 A namespace-name defined at global scope shall not be 	 declared as the name of any other entity in any glogal scope 	 of the program.  */
name|error
argument_list|(
literal|"declaration of `namespace %D' conflicts with"
argument_list|,
name|newdecl
argument_list|)
expr_stmt|;
name|cp_error_at
argument_list|(
literal|"previous declaration of `namespace %D' here"
argument_list|,
name|olddecl
argument_list|)
expr_stmt|;
...
name|warning
argument_list|(
literal|"%J'%D': visibility attribute ignored because it"
argument_list|,
name|newdecl
argument_list|,
name|newdecl
...
name|warning
argument_list|(
literal|"%Jconflicts with previous declaration here"
argument_list|,
name|olddecl
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Resolve `CONTEXT::template NAME'.  Returns an appropriate type,    unless an error occurs, in which case error_mark_node is returned.    If we locate a TYPE_DECL, we return that, rather than the _TYPE it    corresponds to.  If COMPLAIN zero, don't complain about any errors    that occur.  */
end_comment
...
begin_comment
comment|/* A chain of TYPE_DECLs for the builtin types.  */
end_comment
...
begin_comment
comment|/* Return a chain of TYPE_DECLs for the builtin types.  */
end_comment
...
name|error
argument_list|(
literal|"aggregate `%#D' has incomplete type and cannot be defined"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
name|warning
argument_list|(
literal|"%J  you can work around this by removing the initializer"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Subroutine of reshape_init. Reshape the constructor for an array. INITP    is the pointer to the old constructor list (to the CONSTRUCTOR_ELTS of    the CONSTRUCTOR we are processing), while NEW_INIT is the CONSTRUCTOR we    are building.    ELT_TYPE is the element type of the array. MAX_INDEX is an INTEGER_CST    representing the size of the array minus one (the maximum index), or    NULL_TREE if the array was declared without specifying the size.  */
end_comment
...
condition|)
block|{
comment|/* Handle array designated initializers (GNU extension).  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"name `%D' used in a GNU-style designated "
...
argument_list|)
expr_stmt|;
comment|/* If the initializer is brace-enclosed, pull initializers from the      enclosed elements.  Advance past the brace-enclosed initializer      now.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|NULL_TREE
expr_stmt|;
comment|/* It is invalid to initialize a non-aggregate type with a 	   brace-enclosed initializer.  */
if|if
condition|(
name|brace_enclosed_p
condition|)
...
name|error
argument_list|(
literal|"brace-enclosed initializer used to initialize `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|old_init
expr_stmt|;
comment|/* Move past the initializer.  */
operator|*
name|initp
operator|=
name|TREE_CHAIN
...
block|{
comment|/* Build a CONSTRUCTOR to hold the contents of the aggregate.  */
name|new_init
operator|=
...
name|error
argument_list|(
literal|"initializer for `%T' must be brace-enclosed"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"`%T' has no non-static data member named `%D'"
argument_list|,
name|type
argument_list|,
name|TREE_PURPOSE
...
name|max_index
decl_stmt|;
comment|/* If the bound of the array is known, take no more initializers 	     than are allowed.  */
name|max_index
operator|=
operator|(
operator|(
...
argument_list|()
expr_stmt|;
comment|/* The initializers were placed in reverse order in the 	 CONSTRUCTOR.  */
name|CONSTRUCTOR_ELTS
argument_list|(
name|new_init
argument_list|)
...
expr_stmt|;
block|}
comment|/* If there are more initializers than necessary, issue a      diagnostic.  */
if|if
condition|(
operator|*
name|initp
...
name|error
argument_list|(
literal|"too many initializers for `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|NULL_TREE
expr_stmt|;
comment|/* If an initializer is present, DECL_INITIAL has been      error_mark_node, to indicate that an as-of-yet unevaluated      initialization will occur.  From now on, DECL_INITIAL reflects      the static initialization -- if any -- of DECL.  */
name|DECL_INITIAL
argument_list|(
name|decl
argument_list|)
...
condition|)
block|{
comment|/* [dcl.init] paragraph 13, 	     If T is a scalar type, then a declaration of the form 	     T x = { a }; 	     is equivalent to 	     T x = a; 	      	     reshape_init will complain about the extra braces, 	     and doesn't do anything useful in the case where TYPE is 	     scalar, so just don't call it.  */
if|if
condition|(
name|CP_AGGREGATE_TYPE_P
argument_list|(
...
name|error
argument_list|(
literal|"`%D' cannot be thread-local because it has non-POD type `%T'"
argument_list|,
name|decl
argument_list|,
name|TREE_TYPE
...
name|error
argument_list|(
literal|"`%D' is thread-local and so cannot be dynamically "
literal|"initialized"
argument_list|,
name|decl
argument_list|)
...
condition|)
block|{
comment|/* If it's either a pointer or an array type, strip through all 	   of them but the last one. If the last is an array type, issue  	   an error if the element type is abstract.  */
while|while
condition|(
name|POINTER_TYPE_P
argument_list|(
...
name|error
argument_list|(
literal|"size of array `%D' has non-integral type `%T'"
argument_list|,
name|name
argument_list|,
name|type
...
name|error
argument_list|(
literal|"size of array has non-integral type `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|ADDR_EXPR
case|:
comment|/* For any of these, the main declarator is the first operand.  */
return|return
name|get_scope_of_declarator
argument_list|(
name|TREE_OPERAND
...
name|SCOPE_REF
case|:
comment|/* For a pointer-to-member, continue descending.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
return|;
comment|/* Otherwise, if the declarator-id is a SCOPE_REF, the scope in 	 which the declaration occurs is the first operand.  */
return|return
name|TREE_OPERAND
argument_list|(
name|declarator
...
name|TREE_LIST
case|:
comment|/* Attributes to be applied. The declarator is TREE_VALUE.  */
return|return
name|get_scope_of_declarator
argument_list|(
name|TREE_VALUE
...
return|;
default|default:
comment|/* Otherwise, we have a declarator-id which is not a qualified 	 name; the entity will be declared in the current scope.  */
return|return
name|NULL_TREE
return|;
block|}
...
name|error
argument_list|(
literal|"variable or field `%E' declared void"
argument_list|,
name|identifier
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* This might be declaring a member of a template 		     parm to be a friend.  */
name|ctype
operator|=
name|cname
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Update the declarator so that when we process it 		 again the correct type is present.  */
name|TREE_OPERAND
argument_list|(
name|decl
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Diagnose "__thread extern" or "__thread static".  */
if|if
condition|(
name|RIDBIT_SETP
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Now figure out the structure of the declarator proper.      Descend through it, creating more complex types, until we reach      the declared identifier (or NULL_TREE, in an abstract declarator).  */
while|while
condition|(
name|declarator
operator|&&
...
name|ADDR_EXPR
condition|?
literal|"cannot declare reference to `%#T'"
else|:
literal|"cannot declare pointer to `%#T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"parameter may not have variably modified type `%T'"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"%Jinvalid type qualifier for non-member function type"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"storage class `__thread' invalid for function `%s'"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* DECL is a declaration for an overloaded operator.  Returns true if    the declaration is valid; false otherwise.  If COMPLAIN is true,    errors are issued for invalid declarations.  */
end_comment
...
name|error
argument_list|(
literal|"`%D' must have an argument of class or "
...
name|error
argument_list|(
literal|"redeclaration of `%T' as a non-template"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* This flag will be in the binfo of the base type, we must 	     	 clear it after copying the base binfos.  */
...
block|}
block|}
comment|/* Copy the base binfos, collect the virtual bases and set the      inheritance order chain.  */
...
name|error
argument_list|(
literal|"Java class '%T' cannot have multiple bases"
argument_list|,
name|ref
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Java class '%T' cannot have virtual bases"
argument_list|,
name|ref
argument_list|)
expr_stmt|;
...
name|value
expr_stmt|;
comment|/* Set the TREE_TYPE for the values as well.  That's so that when 	     we call decl_constant_value we get an entity of the right type 	     (but with the constant value).  But first make a copy so we 	     don't clobber shared INTEGER_CSTs.  */
if|if
condition|(
name|TREE_TYPE
argument_list|(
...
comment|/* The middle-end currently assumes that types with TYPE_PRECISION      narrower than their underlying type are suitably zero or sign      extended to fill their mode.  g++ doesn't make these guarantees.      Until the middle-end can represent such paradoxical types, we      set the TYPE_PRECISON to the width of the underlying type.  */
name|TYPE_PRECISION
argument_list|(
name|enumtype
argument_list|)
...
condition|)
block|{
comment|/* If this doesn't return integer_type, or a typedef to 	     integer_type, complain.  */
if|if
condition|(
operator|!
name|same_type_p
