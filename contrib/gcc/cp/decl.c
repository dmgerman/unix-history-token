...
condition|)
block|{
comment|/* In C++0x, the type we are creating a reference to might be 		 a typedef which is itself a reference type. In that case, 		 we follow the reference collapsing rules in 		 [7.1.3/8 dcl.typedef] to create the final reference type:  		 "If a typedef TD names a type that is a reference to a type 		 T, an attempt to create the type 'lvalue reference to cv TD' 		 creates the type 'lvalue reference to T,' while an attempt 		 to create the type "rvalue reference to cv TD' creates the 		 type TD."               */
if|if
condition|(
operator|!
name|VOID_TYPE_P
...
argument_list|)
expr_stmt|;
comment|/* In C++0x, we need this check for direct reference to 		 reference declarations, which are forbidden by 		 [8.3.2/5 dcl.ref]. Reference to reference declarations 		 are only allowed indirectly through typedefs and template 		 type arguments. Example:  		   void foo(int&&);      // invalid ref-to-ref decl  		   typedef int& int_ref; 		   void foo(int_ref&);    // valid ref-to-ref decl 	      */
if|if
condition|(
name|inner_declarator
operator|&&
...
name|error
argument_list|(
literal|"cannot declare reference to %q#T, which is not "
literal|"a typedef or a template type argument"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* D is a constructor or overloaded `operator='.     Let T be the class in which D is declared. Then, this function    returns true when D is a move constructor or move assignment    operator, false otherwise.  */
end_comment
...
name|flag_cpp0x
condition|)
comment|/* There are no move constructors if we aren't in C++0x mode.  */
return|return
name|false
return|;
if|if
