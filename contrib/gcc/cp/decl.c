...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin templated static data 6298605 */
comment|/* An in-class declaration of a static data member should be 	 external if the decl is accessible from outside this 	 translation unit (eg something not in an anonymous 	 namespace); it is only a declaration, and not a 	 definition.  */
if|if
condition|(
name|init
operator|==
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end templated static data 6298605 */
block|}
...
begin_comment
comment|/* APPLE LOCAL begin blocks 6040305 (cr) */
end_comment
...
name|ptr_type_node
expr_stmt|;
comment|/* arg_info = xcalloc (1, sizeof (struct c_arg_info)); */
name|TREE_CHAIN
argument_list|(
name|dst_arg
argument_list|)
...
name|src_arg
expr_stmt|;
comment|/* arg_info->parms = dst_arg; */
comment|/* arg_info->types = tree_cons (NULL_TREE, ptr_type_node, 				   tree_cons (NULL_TREE, 				   ptr_type_node, 				   NULL_TREE)); */
name|DECL_ARGUMENTS
argument_list|(
name|block_byref_id_object_copy
index|[
...
argument_list|()
expr_stmt|;
comment|/* start_block_helper_function (block_byref_id_object_copy[kind], true); */
comment|/* store_parm_decls_from (arg_info); */
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 6169580 */
...
begin_comment
comment|/**   This routine builds:    void __Block_byref_id_object_dispose(struct Block_byref_id_object *_src) {     _Block_object_dispose(_src->object, BLOCK_FIELD_IS_OBJECT[|BLOCK_FIELD_IS_WEAK]) // objects     _Block_object_dispose(_src->object, BLOCK_FIELD_IS_BLOCK[|BLOCK_FIELD_IS_WEAK]) // blocks   }  */
end_comment
...
name|ptr_type_node
expr_stmt|;
comment|/* arg_info = xcalloc (1, sizeof (struct c_arg_info));      arg_info->parms = src_arg;      arg_info->types = tree_cons (NULL_TREE, ptr_type_node, 			           NULL_TREE); */
name|DECL_ARGUMENTS
argument_list|(
name|block_byref_id_object_dispose
index|[
...
argument_list|()
expr_stmt|;
comment|/* start_block_helper_function (block_byref_id_object_dispose[kind], true); */
comment|/* store_parm_decls_from (arg_info); */
name|start_preparsed_function
argument_list|(
name|block_byref_id_object_dispose
...
name|s
decl_stmt|;
comment|/* The idea here is to mimic the actions that the C++ parser takes when    constructing 'extern "C" struct NAME {'.  */
...
literal|0
expr_stmt|;
comment|/* this is a 'struct', not a 'class'.  */
name|xref_basetypes
argument_list|(
name|s
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* insert one field at a time;  */
name|TREE_CHAIN
argument_list|(
name|field
argument_list|)
...
name|NULL_TREE
expr_stmt|;
comment|/* otherwise, grokfield croaks. */
...
argument_list|()
expr_stmt|;
comment|/* Block_byref_type = start_struct (RECORD_TYPE, get_identifier (string)); */
name|Block_byref_type
operator|=
name|block_start_struct
argument_list|(
...
literal|0
expr_stmt|;
comment|/* finish_struct (Block_byref_type, field_decl_chain, NULL_TREE); */
name|block_finish_struct
argument_list|(
name|Block_byref_type
argument_list|,
...
begin_comment
comment|/* APPLE LOCAL end blocks 6040305 (cr) */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin blocks 6040305 (cq) */
if|if
condition|(
...
block|}
block|}
comment|/* APPLE LOCAL end blocks 6040305 (cq) */
...
condition|)
return|return;
comment|/* APPLE LOCAL begin radar 5733674 */
if|if
condition|(
name|c_dialect_objc
argument_list|()
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 5733674 */
if|if
condition|(
name|DECL_FUNCTION_SCOPE_P
argument_list|(
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL begin blocks 6040305 (cj) */
case|case
name|cdk_block_pointer
case|:
if|if
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL end blocks 6040305 (cj) */
case|case
name|cdk_error
case|:
break|break;
...
block|}
block|}
comment|/* Don't warn about previously erroneous things that have the same      name.  */
if|if
condition|(
name|TREE_TYPE
argument_list|(
...
name|true
return|;
comment|/* Internally defined things still need a prototype to escape the      warning.  */
if|if
condition|(
name|DECL_ARTIFICIAL
argument_list|(
...
name|false
return|;
comment|/* These will match or error, don't also spew prototype warnings.  */
if|if
condition|(
name|DECL_EXTERN_C_P
argument_list|(
...
comment|/* APPLE LOCAL radar 6169580 */
name|int
name|in_blocks_helper_function
init|=
operator|(
name|flags
operator|&
...
name|RECORD_TYPE
condition|)
comment|/* For RECORD_TYPEs, we can refer to more precise flags than 	   TYPE_HAS_NONTRIVIAL_DESTRUCTOR. */
name|build_cleanup
operator|=
operator|(
name|dtor
