...
name|chain
decl_stmt|;
comment|/* The number of chains in this table.  This is the length of the      the member "chaiin" considered as an array.  */
name|size_t
name|chain_count
decl_stmt|;
...
begin_comment
comment|/* Free TABLE by making its entries ready for reuse. */
end_comment
...
name|vtables
decl_stmt|;
comment|/* A dictionary for looking up enums or class-types names.  */
name|binding_table
name|type_decls
decl_stmt|;
...
value|((struct cp_binding_level *) NULL)
end_define
...
name|verbatim
argument_list|(
literal|"XXX is_class_level != (current_binding_level "
literal|"== class_binding_level)\n"
argument_list|)
expr_stmt|;
block|}
name|is_class_level
...
name|verbatim
argument_list|(
literal|"suspend  %s level %p line %d\n"
argument_list|,
operator|(
name|is_class_level
operator|)
...
name|verbatim
argument_list|(
literal|"resume %s level %p line %d\n"
argument_list|,
operator|(
name|is_class_level
operator|)
...
begin_comment
comment|/* Create a new `struct cp_binding_level'.  */
end_comment
...
begin_comment
comment|/* If KEEP is nonzero, make a BLOCK node for the next binding level,    unconditionally.  Otherwise, use the normal logic to decide whether    or not to create a BLOCK.  */
end_comment
...
begin_comment
comment|/* Returns nonzero if this scope was created to store template    parameters.  */
end_comment
...
begin_comment
comment|/* For a binding between a name and an entity at a block scope,    this is the `struct cp_binding_level' for the block.  */
end_comment
...
operator|++
expr_stmt|;
comment|/* If appropriate, add decl to separate list of statics */
if|if
condition|(
name|b
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Although we don't pop the cxx_binding, we do clear 		 its BINDING_LEVEL since the level is going away now.  */
name|BINDING_LEVEL
argument_list|(
name|IDENTIFIER_BINDING
argument_list|(
...
begin_comment
comment|/* Walk the global declarations.  Whenever one is found for which P    returns nonzero, call F with its address.  If any call to F    returns a nonzero value, return a nonzero value.  */
end_comment
...
argument_list|,
argument|HOST_PTR_PRINTF_TYPE lvl->blocks
argument_list|)
empty_stmt|;
if|if
condition|(
...
argument|HOST_PTR_PRINTF_FORMAT
argument_list|,
argument|HOST_PTR_PRINTF_TYPE level
argument_list|)
empty_stmt|;
name|fprintf
argument_list|(
...
argument|HOST_PTR_PRINTF_FORMAT
argument_list|,
argument|HOST_PTR_PRINTF_TYPE current_binding_level
argument_list|)
empty_stmt|;
name|fprintf
argument_list|(
...
argument|HOST_PTR_PRINTF_FORMAT
argument_list|,
argument|HOST_PTR_PRINTF_TYPE class_binding_level
argument_list|)
empty_stmt|;
name|fprintf
argument_list|(
...
argument|HOST_PTR_PRINTF_FORMAT
argument_list|,
argument|HOST_PTR_PRINTF_TYPE NAMESPACE_LEVEL (global_namespace)
argument_list|)
empty_stmt|;
name|fprintf
argument_list|(
...
begin_comment
comment|/* Namespace binding access routines.   */
end_comment

begin_comment
comment|/* Check whether the a binding for the name to scope is known.    Returns the binding found, or NULL.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Even if the types match, prefer the new declarations type 	 for anitipated built-ins, for exception lists, etc...  */
elseif|else
if|if
condition|(
name|DECL_ANTICIPATED
...
condition|)
block|{
comment|/* Lookup found an implicit typename that we had 		     injected into the current scope. Doing things 		     properly would have located the exact same type, 		     so there is no error here.  We must remove the 		     implicitness so that we do not warn about it.  */
name|t
operator|=
name|copy_node
argument_list|(
...
begin_comment
comment|/* Entry point for the benefit of c_common_nodes_and_builtins.     Make a defintion for a builtin function named NAME and whose data type    is TYPE.  TYPE should be a function type with argument types.  This    function places the anticipated declaration in the global namespace    and additionally in the std namespace if appropriate.     CLASS and CODE tell later passes how to compile calls to this function.    See tree.h for possible values.     If LIBNAME is nonzero, use that for DECL_ASSEMBLER_NAME,    the name to be called if we can't opencode the function.     If ATTRS is nonzero, use that for the function's attribute    list.  */
end_comment
...
directive|endif
comment|/* If we haven't already layed out this declaration, do so now.      Note that we must not call complete type for an external object      because it's type might involve templates that we are not      supposed to isntantiate yet.  (And it's perfectly valid to say      `extern X x' for some incomplete type `X'.)  */
if|if
condition|(
operator|!
name|DECL_EXTERNAL
...
operator|)
expr_stmt|;
comment|/* For some parse errors, OLD_INIT_VALUE may be NULL.  */
if|if
condition|(
operator|!
name|old_init_value
...
operator|)
expr_stmt|;
comment|/* Loop through the array elements, gathering initializers.  */
for|for
control|(
name|index
operator|=
...
expr_stmt|;
block|}
comment|/* If this was a brace-enclosed initializer and all of the      initializers were not used up, there is a problem.  */
if|if
condition|(
name|brace_enclosed_p
operator|&&
...
name|error
argument_list|(
literal|"variable declaration is not allowed here"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
expr_stmt|;
block|}
comment|/* If the parser sees something like "void a::b" where 		 "a::b" is a namespace, it will build a SCOPE_REF with 		 a NAMESPACE_DECL, rather than an IDENTIFIER_NODE, as 		 the second operand.  Since the SCOPE_REF is being 		 used as a declarator, we recover from that here.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Name lookup in an elaborated-type-specifier (after the keyword    indicated by TAG_CODE) has found TYPE.  If the    elaborated-type-specifier is invalid, issue a diagnostic and return    error_mark_node; otherwise, return TYPE itself.  */
end_comment
...
expr_stmt|;
block|}
comment|/* In addition, transform the TYPE_VALUES list to contain the 	     values, rather than the CONST_DECLs for them.  */
name|TREE_VALUE
argument_list|(
name|pair
argument_list|)
...
expr_stmt|;
block|}
comment|/* Compute the minium and maximum values for the type, the size of      the type, and so forth.  */
name|TYPE_PRECISION
argument_list|(
name|enumtype
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* We use "int" or "unsigned int" as the underlying type, unless all      the values will not fit or the user has requested that we try to      use shorter types where possible.  */
if|if
condition|(
name|precision
operator|<
