...
name|names_used
decl_stmt|;
comment|/* Nonzero if this class is no longer open, because of a call to      push_to_top_level.  */
name|size_t
name|hidden
decl_stmt|;
block|}
...
begin_comment
comment|/* The size of the largest empty class seen in this translation unit.  */
end_comment
...
argument_list|)
name|tree
name|sizeof_biggest_empty_class
expr_stmt|;
end_expr_stmt
...
specifier|static
name|tree
name|dfs_find_final_overrider_pre
parameter_list|(
name|tree
parameter_list|,
name|void
...
name|error
argument_list|(
literal|"cannot convert from base %qT to derived type %qT via virtual base %qT"
argument_list|,
name|BINFO_TYPE
argument_list|(
name|binfo
...
argument_list|)
expr_stmt|;
comment|/* Do we need to look in the vtable for the real offset?  */
name|virtual_access
operator|=
operator|(
name|v_binfo
...
operator|)
expr_stmt|;
comment|/* Do we need to check for a null pointer?  */
if|if
condition|(
name|want_pointer
operator|&&
...
condition|)
block|{
comment|/* If we know the conversion will not actually change the value 	 of EXPR, then we can avoid testing the expression for NULL. 	 We have to avoid generating a COMPONENT_REF for a base class 	 field, because other parts of the compiler know that such 	 expressions are always non-NULL.  */
if|if
condition|(
operator|!
name|virtual_access
...
name|class_type
decl_stmt|;
comment|/* TARGET_TYPE has been extracted from BINFO, and, is 	     therefore always cv-unqualified.  Extract the 	     cv-qualifiers from EXPR so that the expression returned 	     matches the input.  */
name|class_type
operator|=
name|TREE_TYPE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Protect against multiple evaluation if necessary.  */
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* Now that we've saved expr, build the real null test.  */
if|if
condition|(
name|null_test
condition|)
...
expr_stmt|;
block|}
comment|/* If this is a simple base reference, express it as a COMPONENT_REF.  */
if|if
condition|(
name|code
operator|==
...
operator|!
name|virtual_access
comment|/* We don't build base fields for empty bases, and they aren't very 	 interesting to the optimizers anyway.  */
operator|&&
operator|!
name|has_empty
condition|)
...
condition|)
block|{
comment|/* In a base member initializer, we cannot rely on the 	     vtable being set up.  We have to indirect via the 	     vtt_parm.  */
name|tree
name|t
decl_stmt|;
name|t
...
begin_comment
comment|/* Subroutine of build_base_path; EXPR and BINFO are as in that function.    Perform a derived-to-base conversion by recursively building up a    sequence of COMPONENT_REFs to the appropriate base fields.  */
end_comment
...
argument_list|)
control|)
comment|/* Is this the base field created by build_base_field?  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
condition|)
block|{
comment|/* We don't use build_class_member_access_expr here, as that 	   has unnecessary checks, and more importantly results in 	   recursive calls to dfs_walk_once.  */
name|int
name|type_quals
init|=
name|cp_type_quals
...
return|;
block|}
comment|/* Didn't find the base field?!?  */
...
begin_comment
comment|/* Convert OBJECT to the base TYPE.  OBJECT is an expression whose    type is a class type or a pointer to a class type.  In the former    case, TYPE is also a class type; in the latter it is another    pointer type.  If CHECK_ACCESS is true, an error message is emitted    if TYPE is inaccessible.  If OBJECT has pointer type, the value is    assumed to be non-NULL.  */
end_comment
...
begin_comment
comment|/* EXPR is an expression with unqualified class type.  BASE is a base    binfo of that class type.  Returns EXPR, converted to the BASE    type.  This function assumes that EXPR is the most derived class;    therefore virtual bases can be found at their static offsets.  */
end_comment
...
name|error_mark_node
return|;
comment|/* First, convert to the requested type.  */
if|if
condition|(
operator|!
...
argument_list|)
expr_stmt|;
comment|/* Second, the requested type may not be the owner of its own vptr.      If not, convert to the base class that owns it.  We cannot use      convert_to_base here, because VCONTEXT may appear more than once      in the inheritance hierarchy of TYPE, and thus direct conversion      between the types may be ambiguous.  Following the path back up      one step at a time via primary bases avoids the problem.  */
name|vfield
operator|=
name|TYPE_VFIELD
argument_list|(
...
begin_comment
comment|/* Given a stable object pointer INSTANCE_PTR, return an expression which    yields a function pointer corresponding to vtable element INDEX.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Remember this as a method reference, for later devirtualization.  */
name|aref
operator|=
name|build3
argument_list|(
...
begin_comment
comment|/* DECL is an entity associated with TYPE, like a virtual table or an    implicitly generated constructor.  Determine whether or not DECL    should have external or internal linkage at the object file    level.  This routine does not deal with COMDAT linkage and other    similar complexities; it simply sets TREE_PUBLIC if it possible for    entities in other translation units to contain copies of DECL, in    the abstract.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* The vtable has not been defined -- yet.  */
name|DECL_EXTERNAL
argument_list|(
name|decl
argument_list|)
...
begin_comment
comment|/* Add method METHOD to class TYPE.  If USING_DECL is non-null, it is    the USING_DECL naming METHOD.  Returns true if the method could be    added to the method vec.  */
end_comment
...
condition|)
block|{
comment|/* Make a new method vector.  We start with 8 entries.  We must 	 allocate at least two (for constructors and destructors), and 	 we're going to end up with an assignment operator at some 	 point as well.  */
name|method_vec
operator|=
name|VEC_alloc
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Create slots for constructors and destructors.  */
name|VEC_quick_push
argument_list|(
name|tree
argument_list|,
...
expr_stmt|;
block|}
comment|/* Maintain TYPE_HAS_CONSTRUCTOR, etc.  */
...
name|error
argument_list|(
literal|"Java class %qT cannot have a destructor"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Java class %qT cannot have an implicit non-trivial "
literal|"destructor"
argument_list|,
name|type
argument_list|)
...
condition|)
continue|continue;
comment|/* For templates, the return type and template parameters 	     must be identical.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"repeated using declaration %q+D"
argument_list|,
name|using_decl
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"using declaration %q+D conflicts with a previous using declaration"
argument_list|,
name|using_decl
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"%q+#D cannot be overloaded"
argument_list|,
name|method
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/* A class should never have more than one destructor.  */
if|if
condition|(
name|current_fns
operator|&&
...
name|reallocated
decl_stmt|;
comment|/* We only expect to add few methods in the COMPLETE_P case, so 	 just make room for one more method in that case.  */
if|if
condition|(
name|complete_p
condition|)
...
comment|/* Replace the current slot.  */
name|VEC_replace
argument_list|(
name|tree
argument_list|,
...
name|error
argument_list|(
literal|"conflicting access specifications for method"
literal|" %q+D, ignored"
argument_list|,
name|TREE_TYPE
argument_list|(
...
name|error
argument_list|(
literal|"conflicting access specifications for field %qE, ignored"
argument_list|,
name|DECL_NAME
argument_list|(
name|fdecl
...
name|error
argument_list|(
literal|"  because of local method %q+#D with same name"
argument_list|,
name|OVL_CURRENT
argument_list|(
name|old_value
...
begin_comment
comment|/* Run through the base classes of T, updating CANT_HAVE_CONST_CTOR_P,    and NO_CONST_ASN_REF_P.  Also set flag bits in T based on    properties of the bases.  */
end_comment
...
name|OPT_Weffc__
argument_list|,
literal|"base class %q#T has a non-virtual destructor"
argument_list|,
name|basetype
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Determine all the primary bases within T.  Sets BINFO_PRIMARY_BASE_P for    those that are primaries.  Sets BINFO_LOST_PRIMARY_P for those    that have had a nearly-empty virtual primary base stolen by some    other base in the hierarchy.  Determines CLASSTYPE_PRIMARY_BASE for    T.  */
end_comment
...
name|base_binfo
decl_stmt|;
comment|/* Determine the primary bases of our bases.  */
for|for
control|(
name|base_binfo
operator|=
...
argument_list|)
decl_stmt|;
comment|/* See if we're the non-virtual primary of our inheritance 	 chain.  */
if|if
condition|(
operator|!
name|BINFO_VIRTUAL_P
...
argument_list|)
condition|)
comment|/* We are the primary binfo.  */
...
expr_stmt|;
block|}
comment|/* Determine if we have a virtual primary base, and mark it so.        */
if|if
condition|(
name|primary
operator|&&
...
argument_list|)
condition|)
comment|/* Someone already claimed this base.  */
...
block|}
block|}
comment|/* First look for a dynamic direct non-virtual base.  */
for|for
control|(
name|i
operator|=
...
block|}
block|}
comment|/* A "nearly-empty" virtual base class can be the primary base      class, if no non-virtual polymorphic base can be found.  Look for      a nearly-empty virtual dynamic base that is not already a primary      base of something in the hierarchy.  If there is no such base,      just pick the first nearly-empty virtual base.  */
for|for
control|(
name|base_binfo
operator|=
...
condition|)
block|{
comment|/* Found one that is not primary.  */
name|primary
operator|=
name|base_binfo
expr_stmt|;
...
name|primary
condition|)
comment|/* Remember the first candidate.  */
name|primary
operator|=
name|base_binfo
expr_stmt|;
...
argument_list|)
condition|)
comment|/* We are stealing a primary base.  */
name|BINFO_LOST_PRIMARY_P
argument_list|(
name|BINFO_INHERITANCE_CHAIN
argument_list|(
...
argument_list|,
literal|"all member functions in class %qT are private"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
name|OPT_Wctor_dtor_privacy
argument_list|,
literal|"%q#T only defines a private destructor and has no friends"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
name|t
argument_list|)
comment|/* Implicitly generated constructors are always public.  */
operator|&&
operator|(
operator|!
...
name|OPT_Wctor_dtor_privacy
argument_list|,
literal|"%q#T only defines private constructors and has no friends"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Warn about duplicate methods in fn_fields.     Sort methods that are not special (i.e., constructors, destructors,    and type conversion operators) so that we can find them faster in    search.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Clear DECL_IN_AGGR_P for all functions.  */
for|for
control|(
name|fn_fields
operator|=
...
name|error
argument_list|(
literal|"no unique final overrider for %qD in %qT"
argument_list|,
name|target_fn
argument_list|,
name|t
...
argument_list|)
argument_list|)
comment|/* If the overrider is invalid, don't even try.  */
operator|&&
operator|!
...
argument_list|)
condition|)
comment|/* A base without a vtable needs no modification, and its bases        are uninteresting.  */
return|return
name|dfs_skip_bases
return|;
if|if
...
argument_list|)
condition|)
comment|/* Don't do the primary vtable, if it's new.  */
return|return
name|NULL_TREE
return|;
if|if
...
condition|)
block|{
comment|/* When we handled the dllimport attribute we may not have known 	     that this function is virtual   We can't use dllimport 	     semantics for a virtual method because we need to initialize 	     the vtable entry with a constant address.  */
name|DECL_DLLIMPORT_P
argument_list|(
name|decl
argument_list|)
...
name|pedwarn
argument_list|(
literal|"%q+#D invalid; an anonymous union can "
literal|"only have non-static data members"
argument_list|,
name|elt
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Create default constructors, assignment operators, and so forth for    the type indicated by T, if they are needed.  CANT_HAVE_CONST_CTOR,    and CANT_HAVE_CONST_ASSIGNMENT are nonzero if, for whatever reason,    the class cannot have a default constructor, copy constructor    taking a const reference argument, or an assignment operator taking    a const reference, respectively.  */
end_comment
...
condition|)
block|{
comment|/* In general, we create destructors lazily.  */
...
literal|1
expr_stmt|;
comment|/* However, if the implicit destructor is non-trivial 	 destructor, we sometimes have to create it at this point.  */
if|if
condition|(
name|TYPE_HAS_NONTRIVIAL_DESTRUCTOR
argument_list|(
...
argument_list|)
condition|)
comment|/* If this a Java class, any non-trivial destructor is 	       invalid, even if compiler-generated.  Therefore, if the 	       destructor is non-trivial we create it now.  */
name|lazy_p
operator|=
name|false
expr_stmt|;
...
name|ix
decl_stmt|;
comment|/* If the implicit destructor will be virtual, then we must 		 generate it now because (unfortunately) we do not 		 generate virtual tables lazily.  */
name|binfo
operator|=
name|TYPE_BINFO
argument_list|(
...
block|}
block|}
comment|/* If we can't get away with being lazy, generate the destructor 	     now.  */
if|if
condition|(
operator|!
name|lazy_p
...
expr_stmt|;
block|}
comment|/* If there is no assignment operator, one will be created if and      when it is needed.  For now, just record whether or not the type      of the parameter to the assignment operator will be a const or      non-const reference.  */
if|if
condition|(
operator|!
name|TYPE_HAS_ASSIGN_REF
...
name|w
decl_stmt|;
comment|/* Extract the declared width of the bitfield, which has been      temporarily stashed in DECL_INITIAL.  */
name|w
operator|=
name|DECL_INITIAL
argument_list|(
...
name|error
argument_list|(
literal|"bit-field %q+#D with non-integral type"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"bit-field %q+D width not an integer constant"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"negative width in bit-field %q+D"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"zero width for bit-field %q+D"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"width of %q+D exceeds its type"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"%q+D is too small to hold all values of %q#T"
argument_list|,
name|field
argument_list|,
name|type
...
name|error
argument_list|(
literal|"member %q+#D with constructor not allowed in union"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"member %q+#D with destructor not allowed in union"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"member %q+#D with copy assignment operator not allowed in union"
argument_list|,
name|field
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"multiple fields in union %qT initialized"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"%q+D may not be static because it is a member of a union"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"%q+D may not have reference type %qT because"
literal|" it is a member of a union"
argument_list|,
name|x
argument_list|,
name|type
...
name|error
argument_list|(
literal|"field %q+D invalidly declared function type"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"field %q+D invalidly declared method type"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
name|OPT_Wextra
argument_list|,
literal|"non-static reference %q+#D in class without a constructor"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"ignoring packed attribute because of unpacked non-POD field %q+#D"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* This is used by -Weffc++ (see below). Warn only for pointers 	 to members which might hold dynamic memory. So do not warn 	 for pointers to functions or pointers to members.  */
if|if
condition|(
name|TYPE_PTR_P
argument_list|(
...
name|OPT_Wextra
argument_list|,
literal|"non-static const member %q+#D in class without a constructor"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
name|pedwarn
argument_list|(
literal|"field %q+#D with same name as class"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
name|OPT_Weffc__
argument_list|,
literal|"%q#T has pointer data members"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* If any of the fields couldn't be packed, unset TYPE_PACKED.  */
if|if
condition|(
name|cant_pack
condition|)
...
begin_comment
comment|/* Record all of the empty subobjects of TYPE (either a type or a    binfo).  If IS_DATA_MEMBER is true, then a non-static data member    is being placed at OFFSET; otherwise, it is a base class that is    being placed at OFFSET.  */
end_comment
...
name|max_offset
decl_stmt|;
comment|/* If recording subobjects for a non-static data member or a      non-empty base class , we do not need to record offsets beyond      the size of the biggest empty class.  Additional data members      will go at the end of the class.  Additional base classes will go      either at offset zero (if empty, in which case they cannot      overlap with offsets past the size of the biggest empty class) or      at the end of the class.       However, if we are placing an empty base class, then we must record      all offsets, as either the empty class is at offset zero (where      other empty classes might later be placed) or at the end of the      class (where other objects might then be placed, so other empty      subobjects might later overlap).  */
if|if
condition|(
name|is_data_member
operator|||
...
name|OPT_Wabi
argument_list|,
literal|"offset of empty base %qT may not be ABI-compliant and may"
...
name|OPT_Wabi
argument_list|,
literal|"class %qT will be considered nearly empty in a "
...
name|error
argument_list|(
literal|"initializer specified for non-virtual method %q+D"
argument_list|,
name|x
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* All user-declared destructors are non-trivial.  */
if|if
condition|(
name|DECL_DESTRUCTOR_P
argument_list|(
...
name|cant_have_const_ctor
decl_stmt|;
comment|/* Nonzero if the implicitly generated assignment operator      should take a non-const reference argument.  */
name|int
name|no_const_asn_ref
decl_stmt|;
name|tree
...
argument_list|)
expr_stmt|;
comment|/* Check all the data member declarations.  We cannot call      check_field_decls until we have called check_bases check_methods,      as check_field_decls depends on TYPE_HAS_NONTRIVIAL_DESTRUCTOR      being set appropriately.  */
name|check_field_decls
argument_list|(
name|t
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Synthesize any needed methods.  */
name|add_implicitly_declared_members
argument_list|(
name|t
argument_list|,
...
name|OPT_Wabi
argument_list|,
literal|"offset of virtual base %qT is not ABI-compliant and "
...
name|base_binfo
decl_stmt|;
comment|/* If there are no repeated bases, nothing can be ambiguous.  */
if|if
condition|(
operator|!
...
literal|0
argument_list|,
literal|"direct base %qT inaccessible in %qT due to ambiguity"
argument_list|,
name|basetype
argument_list|,
name|t
...
name|OPT_Wextra
argument_list|,
literal|"virtual base %qT inaccessible in %qT due to ambiguity"
argument_list|,
name|basetype
argument_list|,
name|t
...
argument_list|)
expr_stmt|;
comment|/* Mark all the primary bases in the hierarchy.  */
name|determine_primary_bases
argument_list|(
name|t
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* The visibility of static data members is determined 		 at their point of declaration, not their point of 		 definition.  */
...
name|OPT_Wabi
argument_list|,
literal|"size assigned to %qT may not be "
...
name|OPT_Wabi
argument_list|,
literal|"the offset of %qD may not be ABI-compliant and may "
literal|"change in a future version of GCC"
argument_list|,
name|field
argument_list|)
...
name|field
argument_list|)
comment|/* The TREE_NO_WARNING flag gets set by Objective-C when 	     laying out an Objective-C class.  The ObjC ABI differs 	     from the C++ ABI, and so we do not want a warning 	     here.  */
operator|&&
operator|!
...
name|OPT_Wabi
argument_list|,
literal|"offset of %q+D is not ABI-compliant and may "
literal|"change in a future version of GCC"
argument_list|,
name|field
argument_list|)
...
name|OPT_Wabi
argument_list|,
literal|"%q+D contains empty classes which may cause base "
...
argument_list|)
expr_stmt|;
comment|/* The middle end uses the type of expressions to determine the 	 possible range of expression values.  In order to optimize 	 "x.i> 7" to "false" for a 2-bit bitfield "i", the middle end 	 must be made aware of the width of "i", via its type.  	 Because C++ does not have integer types of arbitrary width, 	 we must (for the purposes of the front end) convert from the 	 type assigned here to the declared type of the bitfield 	 whenever a bitfield expression is used as an rvalue. 	 Similarly, when assigning a value to a bitfield, the value 	 must be converted to the type given the bitfield here.  */
if|if
condition|(
name|DECL_C_BIT_FIELD
argument_list|(
...
name|OPT_Wabi
argument_list|,
literal|"layout of classes derived from empty class %qT "
literal|"may change in a future version of GCC"
argument_list|,
name|t
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Now that we're done with layout, give the base fields the real types.  */
for|for
control|(
name|field
operator|=
...
begin_comment
comment|/* Determine the "key method" for the class type indicated by TYPE,    and set CLASSTYPE_KEY_METHOD accordingly.  */
end_comment
...
condition|)
return|return;
comment|/* The key method is the first non-pure virtual function that is not      inline at the point of class definition.  On some targets the      key function may not be inline; those targets should not call      this function until the end of the translation unit.  */
for|for
control|(
name|method
operator|=
...
expr_stmt|;
block|}
comment|/* Complain if one of the field types requires lower visibility.  */
...
argument_list|)
expr_stmt|;
comment|/* This warning does not make sense for Java classes, since they      cannot have destructors.  */
if|if
condition|(
operator|!
name|TYPE_FOR_JAVA
...
argument_list|)
expr_stmt|;
comment|/* Warn only if the dtor is non-private or the class has 	 friends.  */
if|if
condition|(
comment|/* An implicitly declared destructor is always public.  And, 	     if it were virtual, we would have created it by now.  */
operator|!
name|dtor
operator|||
operator|(
...
literal|0
argument_list|,
literal|"%q#T has virtual functions but non-virtual destructor"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Class layout, assignment of virtual table slots, etc., is now      complete.  Give the back end a chance to tweak the visibility of      the class or perform any other required target modifications.  */
name|targetm
operator|.
name|cxx
operator|.
...
name|size_zero_node
expr_stmt|;
comment|/* We need to emit an error message if this type was used as a parameter 	 and it is an abstract type, even if it is a template. We construct 	 a simple CLASSTYPE_PURE_VIRTUALS list without taking bases into 	 account and we call complete_vars with this type, which will check 	 the PARM_DECLS. Note that while the type is being defined, 	 CLASSTYPE_PURE_VIRTUALS contains the list of the inline friends 	 (see CLASSTYPE_INLINE_FRIENDS) so we need to clear it.  */
name|CLASSTYPE_PURE_VIRTUALS
argument_list|(
name|t
argument_list|)
...
condition|)
block|{
comment|/* Just because we see an ADDR_EXPR doesn't mean we're dealing 	     with a real object -- given&p->f, p can still be null.  */
name|tree
name|t
init|=
...
argument_list|)
decl_stmt|;
comment|/* ??? Probably should check DECL_WEAK here.  */
if|if
condition|(
name|t
operator|&&
...
name|COMPONENT_REF
case|:
comment|/* If this component is really a base class reference, then the field 	 itself isn't definitive.  */
if|if
condition|(
name|DECL_FIELD_IS_BASE
argument_list|(
...
condition|)
block|{
comment|/* We only need one hash table because it is always left empty.  */
specifier|static
name|htab_t
name|ht
decl_stmt|;
...
literal|1
expr_stmt|;
comment|/* Enter the INSTANCE in a table to prevent recursion; a 	     variable's initializer may refer to the variable 	     itself.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Restore the cached PREVIOUS_CLASS_LEVEL.  */
end_comment

begin_function
specifier|static
name|void
name|restore_class_cache
parameter_list|(
name|void
parameter_list|)
block|{
...
name|type
decl_stmt|;
comment|/* We are re-entering the same class we just left, so we don't      have to search the whole inheritance matrix to find all the      decls to bind again.  Instead, we install the cached      class_shadowed list and walk through it binding names.  */
...
name|previous_class_level
expr_stmt|;
comment|/* Restore IDENTIFIER_TYPE_VALUE.  */
for|for
control|(
name|type
operator|=
...
begin_comment
comment|/* When we exit a toplevel class scope, we save its binding level so    that we can restore it quickly.  Here, we've entered some other    class, so we must invalidate our cache.  */
end_comment
...
begin_comment
comment|/* Mark the top of the class stack as hidden.  */
end_comment
...
begin_comment
comment|/* Mark the top of the class stack as un-hidden.  */
end_comment
...
begin_comment
comment|/* Returns 1 if the class type currently being defined is either T or    a nested type of T.  */
end_comment
...
name|i
decl_stmt|;
comment|/* We start looking from 1 because entry 0 is from global scope,      and has no type.  */
for|for
control|(
name|i
operator|=
...
name|error
argument_list|(
literal|"language string %<\"%E\"%> not recognized"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"cannot resolve overloaded function %qD based on"
literal|" conversion to type %qT"
argument_list|,
name|DECL_NAME
argument_list|(
name|OVL_FUNCTION
...
continue|continue;
comment|/* Ignore functions which haven't been explicitly 	     declared.  */
if|if
condition|(
...
name|error
argument_list|(
literal|"no matches converting function %qD to type %q#T"
argument_list|,
name|DECL_NAME
argument_list|(
name|OVL_FUNCTION
...
name|error
argument_list|(
literal|"converting overloaded function %qD to type %q#T is ambiguous"
argument_list|,
name|DECL_NAME
argument_list|(
name|OVL_FUNCTION
...
name|pedwarn
argument_list|(
literal|"assuming pointer to member %qD"
argument_list|,
name|fn
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* We could not check access when this expression was originally 	 created since we did not know at that time to which function 	 the expression referred.  */
if|if
condition|(
...
name|error
argument_list|(
literal|"argument of type %qT does not match %qT"
argument_list|,
name|TREE_TYPE
argument_list|(
name|rhs
...
expr_stmt|;
block|}
comment|/* If we are in a template, and have a NON_DEPENDENT_EXPR, we cannot      deduce any type information.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
return|;
block|}
comment|/* There only a few kinds of expressions that may have a type      dependent on overload resolution.  */
name|gcc_assert
argument_list|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"changes meaning of %qD from %q+#D"
argument_list|,
name|DECL_NAME
argument_list|(
name|OVL_CURRENT
...
begin_comment
comment|/* Data for secondary VTT initialization.  */
end_comment

begin_typedef
typedef|typedef
struct|struct
name|secondary_vptr_vtt_init_data_s
block|{
comment|/* Is this the primary VTT? */
name|bool
name|top_level_p
decl_stmt|;
comment|/* Current index into the VTT.  */
name|tree
name|index
decl_stmt|;
comment|/* TREE_LIST of initializers built up.  */
name|tree
name|inits
decl_stmt|;
comment|/* The type being constructed by this secondary VTT.  */
name|tree
name|type_being_constructed
decl_stmt|;
block|}
name|secondary_vptr_vtt_init_data
typedef|;
end_typedef
...
name|top_level_p
condition|)
comment|/* Add the secondary VTTs for virtual bases in inheritance graph        order.  */
for|for
control|(
name|b
operator|=
...
comment|/* Remove the ctor vtables we created.  */
name|dfs_walk_all
argument_list|(
name|binfo
argument_list|,
...
begin_comment
comment|/* Called from build_vtt_inits via dfs_walk.  BINFO is the binfo for the base    in most derived. DATA is a SECONDARY_VPTR_VTT_INIT_DATA structure.  */
end_comment
...
name|dfs_skip_bases
return|;
comment|/* We're only interested in proper subobjects of the type being      constructed.  */
if|if
condition|(
name|SAME_BINFO_TYPE_P
argument_list|(
...
name|NULL_TREE
return|;
comment|/* We're only interested in bases with virtual bases or reachable      via a virtual path from the type being constructed.  */
if|if
condition|(
operator|!
operator|(
...
condition|)
block|{
comment|/* It's a primary virtual base, and this is not a 	     construction vtable.  Find the base this is primary of in 	     the inheritance graph, and use that base's vtable 	     now.  */
while|while
condition|(
name|BINFO_PRIMARY_P
argument_list|(
...
begin_comment
comment|/* Called from build_vtt_inits via dfs_walk. After building    constructor vtables and generating the sub-vtt from them, we need    to restore the BINFO_VTABLES that were scribbled on.  DATA is the    binfo of the base whose sub vtt was generated.  */
end_comment
...
argument_list|)
condition|)
comment|/* If this class has no vtable, none of its bases do.  */
return|return
name|dfs_skip_bases
return|;
if|if
...
name|vtable
condition|)
comment|/* This might be a primary base, so have no vtable in this        hierarchy.  */
return|return
name|NULL_TREE
return|;
...
argument_list|)
expr_stmt|;
comment|/* ??? We should call fold_convert to convert the address to 	 vtbl_ptr_type_node, which is the type of elements in the 	 vtable.  However, the resulting NOP_EXPRs confuse other parts 	 of the C++ front end.  */
name|gcc_assert
argument_list|(
name|TREE_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Add the offset-to-top entry.  It comes earlier in the vtable than      the typeinfo entry.  Convert the offset to look like a      function pointer, so that we can put it in the vtable.  */
name|init
operator|=
name|build_nop
argument_list|(
...
begin_comment
comment|/* Fold a OBJ_TYPE_REF expression to the address of a function.    KNOWN_TYPE carries the true type of OBJ_TYPE_REF_OBJECT(REF).  */
end_comment
