...
name|candidates
decl_stmt|;
comment|/* Binfos which inherited virtually on the current path.  */
name|tree
name|vpath
decl_stmt|;
block|}
...
condition|)
block|{
comment|/* We've found a path to the declaring base.  Walk the path from 	 derived to base, looking for an overrider for FN.  */
name|tree
name|path
decl_stmt|,
name|probe
...
name|vpath
decl_stmt|;
comment|/* Build the path, using the inheritance chain and record of 	 virtual inheritance.  */
for|for
control|(
name|path
operator|=
...
expr_stmt|;
block|}
comment|/* Now walk path, looking for overrides.  */
for|for
control|(
init|;
name|path
...
condition|)
block|{
comment|/* There was no existing virtual thunk (which takes 	     precedence).  */
name|tree
name|thunk_binfo
decl_stmt|;
name|base_kind
...
condition|)
block|{
comment|/* We convert via virtual base. Find the virtual 		     base and adjust the fixed offset to be from there.  */
while|while
condition|(
operator|!
name|TREE_VIA_VIRTUAL
