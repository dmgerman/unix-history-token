...
argument_list|)
expr_stmt|;
comment|/* The destructor type must be complete.  */
name|object_type
operator|=
...
return|;
block|}
comment|/* If the name could not be found, issue an error.  */
if|if
condition|(
operator|!
name|fn
...
begin_comment
comment|/* Find the possibly overloaded set of functions corresponding to a    call of the form SCOPE::NAME (...). NAME might be a    TEMPLATE_ID_EXPR, OVERLOAD, _DECL, IDENTIFIER_NODE or LOOKUP_EXPR. */
end_comment
...
name|error
argument_list|(
literal|"incomplete type '%T' cannot be used to name a scope"
argument_list|,
name|scope
argument_list|)
expr_stmt|;
...
operator|)
expr_stmt|;
comment|/* It might be the name of a function pointer member.  */
if|if
condition|(
name|fn
operator|&&
...
begin_comment
comment|/* Return an expression for a call to FN (a namespace-scope function)    with the ARGS.  */
end_comment
...
name|placement
expr_stmt|;
comment|/* Sometimes we have a COMPOUND_EXPR, rather than a simple 	 CALL_EXPR.  */
while|while
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Build a call to __builtin_trap which can be used in an expression.  */
end_comment
...
condition|)
block|{
comment|/* Undefined behavior [expr.call] 5.2.2/7.  We used to just warn 	 here and do a bitwise copy, but now cp_expr_size will abort if we 	 try to do that.  */
name|warning
argument_list|(
literal|"cannot pass objects of non-POD type `%#T' through `...'; \ call will abort at runtime"
argument_list|,
name|TREE_TYPE
argument_list|(
name|arg
...
argument_list|)
expr_stmt|;
comment|/* We must perform the conversion here so that we do not 	 subsequently check to see whether BINFO is an accessible 	 base.  (It is OK for a constructor to call a constructor in 	 an inaccessible base as long as the constructor being called 	 is accessible.)  */
if|if
condition|(
operator|!
name|same_type_ignoring_top_level_qualifiers_p
...
begin_comment
comment|/* Returns nonzero iff standard conversion sequence ICS1 is a proper    subsequence of ICS2.  */
end_comment
...
name|pedwarn
argument_list|(
literal|"ISO C++ says that `%D' and `%D' are ambiguous \ even though the worst conversion for the former is better than the worst \ conversion for the latter"
argument_list|,
name|w
operator|->
name|fn
