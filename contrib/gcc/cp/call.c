...
literal|1
decl_stmt|;
comment|/* The `this' and `in_chrg' arguments to constructors are not considered      in overload resolution.  */
if|if
condition|(
name|DECL_CONSTRUCTOR_P
argument_list|(
...
name|t
decl_stmt|;
comment|/* An overloaded function does not have an argument type */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Create an overload candidate for the conversion function FN which will    be invoked for expression OBJ, producing a pointer-to-function which    will in turn be called with the argument list ARGLIST, and add it to    CANDIDATES.  FLAGS is passed on to implicit_conversion.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* This handles the case where we're trying to delete 	   X (*a)[10]; 	   a=new X[5][10]; 	   delete[] a; */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
condition|)
block|{
comment|/* Strip off the pointer and the array.  */
name|arg1
operator|=
name|TREE_TYPE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* instantiate_type will always return a plain function; pretend it's      overloaded.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* If the current scope isn't allowed to access DECL along    BASETYPE_PATH, give an error.  */
end_comment
...
begin_comment
comment|/* ARG is a default argument expression being passed to a parameter of    the indicated TYPE.  Do any required conversions.  Return the    converted value.  */
end_comment
...
name|cp_pedwarn
argument_list|(
literal|"passing `%T' as `this' argument of `%#D' discards %s"
argument_list|,
name|TREE_TYPE
argument_list|(
name|argtype
...
condition|)
block|{
comment|/* This came from a template.  Instantiate the default arg here, 	     not in tsubst.  In the case of something like:  	       template<class T> 	       struct S { 	         static T t(); 		 void f(T = t()); 	       };  	     we must be careful to do name lookup in the scope of 	     S<T>, rather than in the current class.  */
if|if
condition|(
...
argument_list|)
decl_stmt|;
comment|/* Don't copy the padding byte; it might not have been allocated 	     if to is a base subobject.  */
if|if
condition|(
name|is_empty_class
argument_list|(
