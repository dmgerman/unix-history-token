...
literal|0
decl_stmt|;
comment|/* This will be 0 if we encounter a situation where we should not      link in libstdc++, or 2 if we should link in libg++ as well.  */
name|int
name|library
init|=
literal|1
...
argument_list|)
expr_stmt|;
comment|/* If we're called as g++ (or i386-aout-g++), link in libg++ as well.  */
if|if
condition|(
name|strcmp
argument_list|(
