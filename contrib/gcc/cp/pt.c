...
begin_comment
unit|int is_member_template_class (tree t) {   if (!DECL_CLASS_TEMPLATE_P (t))
...
begin_comment
comment|/* A explicit specialization or partial specialization TMPL is being    declared.  Check that the namespace in which the specialization is    occurring is permissible.  Returns false iff it is invalid to    specialize TMPL in the current namespace.  */
end_comment
...
name|pedwarn
argument_list|(
literal|"specialization of `%D' in different namespace"
argument_list|,
name|tmpl
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* Next, we check the members themselves.  In order to handle 	 a few tricky cases like  	   template<class T> friend void A<T>::g(T t); 	   template<class T> template<T t> friend void A<T>::h();  	 we need to figure out what ARGS is (corresponding to `T' in above 	 examples) from DECL for later processing.  */
name|tree
name|context
init|=
name|DECL_CONTEXT
...
condition|)
continue|continue;
comment|/* For a non-static member function, we need to make sure that 	     the const qualification is the same. This can be done by 	     checking the 'this' in the argument list.  */
if|if
condition|(
name|DECL_NONSTATIC_MEMBER_FUNCTION_P
argument_list|(
...
name|error
argument_list|(
literal|"`%D' is not a template function"
argument_list|,
name|dname
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"destructor `%D' declared as member template"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"invalid template declaration of `%D'"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/* The DECL_TI_ARGS of DECL contains full set of arguments refering      back to its most general template.  If TMPL is a specialization,      ARGS may only have the innermost set of arguments.  Add the missing      argument levels if necessary.  */
if|if
condition|(
name|DECL_TEMPLATE_INFO
argument_list|(
...
begin_comment
comment|/* Attempt to convert the non-type template parameter EXPR to the    indicated TYPE.  If the conversion is successful, return the    converted value.  If the conversion is unsuccessful, return    NULL_TREE if we issued an error message, or error_mark_node if we    did not.  We issue error messages for out-and-out bad template    parameters, but not simply because the conversion failed, since we    might be just trying to do argument deduction.  Both TYPE and EXPR    must be non-dependent.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* In a template, the initializer for a VAR_DECL may not be 	   marked as TREE_CONSTANT, in which case decl_constant_value 	   will not return the initializer.  Handle that special case 	   here.  */
if|if
condition|(
name|expr
operator|==
...
name|expr
argument_list|)
comment|/* DECL_INITIAL can be NULL if we are processing a 	       variable initialized to an expression involving itself. 	       We know it is initialized to a constant -- but not what 	       constant, yet.  */
operator|&&
name|DECL_INITIAL
argument_list|(
name|expr
...
name|error
argument_list|(
literal|"type '%T' cannot be used as a value for a non-type "
...
name|error
argument_list|(
literal|"invalid use of '%D' as a non-type template-argument"
argument_list|,
name|expr
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"invalid use of '%E' as a non-type template-argument"
argument_list|,
name|expr
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|/* If this expression already has reference type, get the 	   underlying object.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"  expected a class template, got `%E'"
argument_list|,
name|arg
argument_list|)
expr_stmt|;
...
operator|)
expr_stmt|;
comment|/* If the member is a class template, then -- even after 		 substituition -- there may be dependent types in the 		 template argument list for the class.  We increment 		 PROCESSING_TEMPLATE_DECL so that dependent_type_p, as 		 that function will assume that no types are dependent 		 when outside of a template.  */
if|if
condition|(
name|class_template_p
condition|)
...
comment|/* If the TREE_TYPE of ARG is not NULL_TREE, ARG is already 	    as simple as it's going to get, and trying to reprocess 	    the trees will break.  Once tsubst_expr et al DTRT for 	    non-dependent exprs, this code can go away, as the type 	    will always be set.  */
if|if
condition|(
operator|!
name|TREE_TYPE
...
argument_list|)
expr_stmt|;
comment|/* The default argument expression should not be considered to be      within the scope of FN.  Since push_access_scope sets      current_function_decl, we must explicitly clear it here.  */
name|current_function_decl
operator|=
name|NULL_TREE
expr_stmt|;
...
begin_comment
comment|/* Substitute the ARGS into the T, which is a _DECL.  TYPE is the    (already computed) substitution of ARGS into TREE_TYPE (T), if    appropriate.  Return the result of the substitution.  Issue error    and warning messages under control of COMPLAIN.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* It is not a dependent using decl any more.  */
name|TREE_TYPE
argument_list|(
name|r
argument_list|)
...
argument_list|)
decl_stmt|;
comment|/* The array dimension behaves like a non-type template arg, 	   in that we want to fold it as much as possible.  */
name|max
operator|=
name|tsubst_template_arg
argument_list|(
...
name|error
argument_list|(
literal|"creating array of `%T', which is an abstract class type"
argument_list|,
name|type
argument_list|)
expr_stmt|;
...
name|false
decl_stmt|;
comment|/* A baselink indicates a function from a base class.  The        BASELINK_ACCESS_BINFO and BASELINK_BINFO are going to have        non-dependent types; otherwise, the lookup could not have        succeeded.  However, they may indicate bases of the template        class, rather than the instantiated class.           In addition, lookups that were not ambiguous before may be        ambiguous now.  Therefore, we perform the lookup again.  */
name|qualifying_scope
operator|=
name|BINFO_TYPE
argument_list|(
...
name|error
argument_list|(
literal|"dependent-name `%E' is parsed as a non-type, but "
...
name|STMT_EXPR
case|:
comment|/* This processing should really occur in tsubst_expr.  However, 	 tsubst_expr does not recurse into expressions, since it 	 assumes that there aren't any statements inside them.  So, we 	 need to expand the STMT_EXPR here.  */
if|if
condition|(
operator|!
name|processing_template_decl
...
name|error
argument_list|(
literal|"`%T' is not a class or namespace"
argument_list|,
name|TREE_OPERAND
argument_list|(
name|member
...
name|error
argument_list|(
literal|"`%D' is not a class or namespace"
argument_list|,
name|TREE_OPERAND
argument_list|(
name|member
...
name|error
argument_list|(
literal|"`%T' is a variably modified type"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"integral expression `%E' is not constant"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
literal|0
decl_stmt|;
comment|/* If template argument deduction succeeds, we substitute the      resulting arguments into non-deduced contexts.  While doing that,      we must be aware that we may encounter dependent types.  */
operator|++
name|processing_template_decl
expr_stmt|;
name|targs
...
block|{
comment|/* This is done in analogous to `start_decl'.  It is 	     required for correct access checking.  */
name|push_nested_class
argument_list|(
name|DECL_CONTEXT
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Normally, pop_nested_class is called by cp_finish_decl 	     above.  But when instantiate_decl is triggered during 	     instantiate_class_template processing, its DECL_CONTEXT 	     is still not completed yet, and pop_nested_class isn't 	     called.  */
if|if
condition|(
operator|!
name|COMPLETE_TYPE_P
...
begin_comment
comment|/* Returns TRUE if the EXPRESSION is value-dependent, in the sense of    [temp.dep.constexpr] */
end_comment
...
name|true
return|;
comment|/* A non-type template parameter.  */
if|if
condition|(
operator|(
name|TREE_CODE
...
name|true
return|;
comment|/* So arenames that have not been looked up.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* TYPE is a TYPENAME_TYPE.  Returns the ordinary TYPE to which the    TYPENAME_TYPE corresponds.  Returns ERROR_MARK_NODE if no such TYPE    can be found.  Note that this function peers inside uninstantiated    templates and therefore should be used only in extremely limited    situations.  */
end_comment
...
name|expr
return|;
comment|/* Preserve THROW_EXPRs -- all throw-expressions have type "void".      There is at least one place where we want to know that a      particular expression is a throw-expression: when checking a ?:      expression, there are special rules if the second or third      argument is a throw-expresion.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
