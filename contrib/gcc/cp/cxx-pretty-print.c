...
begin_comment
comment|/* Pretty-print out the token sequence ":: template" in template codes    where it is needed to "inline declare" the (following) member as    a template.  This situation arises when SCOPE of T is dependent    on template parameters.  */
end_comment
...
condition|)
block|{
comment|/* A pointer-to-member is always qualified.  */
case|case
name|PTRMEM_CST
case|:
name|pp_cxx_nested_name_specifier
...
expr_stmt|;
break|break;
comment|/* In Standard C++, functions cannot possibly be used as 	 nested-name-specifiers.  However, there are situations where 	 is "makes sense" to output the surrounding function name for the 	 purpose of emphasizing on the scope kind.  Just printing the 	 function name might not be sufficient as it may be overloaded; so, 	 we decorate the function with its signature too. 	 FIXME:  This is probably the wrong pretty-printing for conversion 	 functions and some function templates.  */
case|case
name|OVERLOAD
case|:
name|t
...
begin_comment
comment|/* Pretty-print a compound assignment operator token as indicated by T.  */
end_comment
...
expr_stmt|;
break|break;
comment|/* selection-statement: 	    if ( expression ) statement 	    if ( expression ) statement else statement  */
case|case
name|IF_STMT
case|:
name|pp_cxx_identifier
...
begin_comment
comment|/* Initialization of a C++ pretty-printer object.  */
end_comment
