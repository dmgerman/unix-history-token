...
begin_comment
comment|/* Handle initialization things in C++.    Copyright (C) 1987, 89, 92-96, 1997 Free Software Foundation, Inc.    Contributed by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
block|my_friendly_assert (BINFO_INHERITANCE_CHAIN (base_binfo) == t_binfo, 999);
else|#
directive|else
...
argument_list|()
expr_stmt|;
comment|/* We special-case TARGET_EXPRs here to avoid an error about 	 private copy constructors for temporaries bound to reference vars. 	 If the TARGET_EXPR represents a call to a function that has 	 permission to create such objects, a reference can bind directly 	 to the return value.  An object variable must be initialized 	 via the copy constructor, even if the call is elided.  */
if|if
condition|(
operator|!
operator|(
...
directive|if
literal|1
comment|/* Get a little extra space to store a couple of things before the new'ed      array, if this isn't the default placement new.  */
name|use_cookie
operator|=
operator|(
name|has_array
...
directive|else
comment|/* Get a little extra space to store a couple of things before the new'ed      array, if this is either non-placement new or new (nothrow).  */
name|use_cookie
operator|=
operator|(
name|has_array
...
argument_list|)
expr_stmt|;
comment|/* If any part of the object initialization terminates by throwing 	 an exception and the new-expression does not contain a 	 new-placement, then the deallocation function is called to free 	 the memory in which the object was being constructed.  */
if|if
condition|(
name|flag_exceptions
operator|&&
...
directive|if
literal|0
comment|/* Disable this until flow is fixed so that it doesn't 		 think the initialization of sentry is a dead write.  */
block|tree end, sentry, begin, buf, t = TREE_TYPE (rval);  	      begin = get_target_expr (boolean_true_node); 	      sentry = TREE_OPERAND (begin, 0);  	      yes = suspend_momentary (); 	      TREE_OPERAND (begin, 2) 		= build (COND_EXPR, void_type_node, sentry, 			 cleanup, void_zero_node); 	      resume_momentary (yes);  	      rval = get_target_expr (rval);  	      end = build (MODIFY_EXPR, TREE_TYPE (sentry), 			   sentry, boolean_false_node); 	      TREE_SIDE_EFFECTS (end) = 1;  	      buf = TREE_OPERAND (rval, 0);  	      rval = build (COMPOUND_EXPR, t, begin, 			    build (COMPOUND_EXPR, t, rval, 				   build (COMPOUND_EXPR, t, end, buf)));
else|#
directive|else
comment|/* FIXME: this is a workaround for a crash due to overlapping 		 exception regions.  Cleanups shouldn't really happen here.  */
name|rval
operator|=
name|build1
argument_list|(
