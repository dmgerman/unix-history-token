...
begin_comment
comment|/* Handle initialization things in C++.    Copyright (C) 1987, 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003 Free Software Foundation, Inc.    Contributed by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
name|NULL_TREE
return|;
comment|/* At this point, TYPE is either a POD class type, an array of POD      classes, or something even more inoccuous.  */
return|return
name|build_zero_init
argument_list|(
name|type
...
name|warning
argument_list|(
literal|"`%D' should be initialized in the member initialization "
literal|"list"
argument_list|,
name|member
argument_list|)
...
condition|)
break|break;
comment|/* Issue a warning if the explicit initializer order does not 	 match that which will actually occur.  */
if|if
condition|(
name|warn_reorder
operator|&&
...
name|cp_warning_at
argument_list|(
literal|"`%D' will be initialized after"
argument_list|,
name|TREE_PURPOSE
argument_list|(
name|next_subobject
...
name|warning
argument_list|(
literal|"base `%T' will be initialized after"
argument_list|,
name|TREE_PURPOSE
argument_list|(
name|next_subobject
...
name|error
argument_list|(
literal|"multiple initializations given for base `%T'"
argument_list|,
name|subobject
argument_list|)
expr_stmt|;
...
name|warning
argument_list|(
literal|"base class `%#T' should be explicitly initialized in the "
...
begin_comment
comment|/* Like build_aggr_init, but not just for aggregates.  */
end_comment
...
return|;
block|}
comment|/* Convert 'this' to the specified type to disambiguate conversion      to the function's context.  */
if|if
condition|(
name|decl
operator|==
name|current_class_ref
comment|/* ??? this is wrong, but if this conversion is invalid we need to 	 defer it until we know whether we are calling a static or 	 non-static member function.  Be conservative for now.  */
operator|&&
...
name|error
argument_list|(
literal|"invalid pointer to bit-field `%D'"
argument_list|,
name|t
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Use an incomplete array type to avoid VLA headaches.  */
name|full_type
operator|=
...
literal|0
expr_stmt|;
comment|/* When using placement new, users may not realize that they need      the extra storage.  We require that the operator called be      the global placement operator new[].  */
elseif|else
if|if
condition|(
name|placement
...
name|error_mark_node
return|;
comment|/* The ALLOC_CALL should be a CALL_EXPR -- or a COMPOUND_EXPR whose      right-hand-side is ultimately a CALL_EXPR -- and the first      operand should be the address of a known FUNCTION_DECL.  */
name|t
operator|=
name|alloc_call
expr_stmt|;
...
name|use_cookie
condition|)
comment|/* Subtract the padding back out to get to the pointer returned 	       from operator new.  */
name|delete_node
operator|=
name|fold
argument_list|(
...
begin_comment
comment|/* `build_vec_init' returns tree structure that performs    initialization of a vector of aggregate types.     BASE is a reference to the vector, of ARRAY_TYPE.    MAXINDEX is the maximum index of the array (one less than the      number of elements).  It is only used if      TYPE_DOMAIN (TREE_TYPE (BASE)) == NULL_TREE.    INIT is the (possibly NULL) initializer.     FROM_ARRAY is 0 if we should init everything with INIT    (i.e., every element initialized from INIT).    FROM_ARRAY is 1 if we should index into INIT in parallel    with initialization of DECL.    FROM_ARRAY is 2 if we should index into INIT in parallel,    but use assignment instead of initialization.  */
end_comment
