...
begin_comment
comment|/* Definitions for C++ parsing and type checking.    Copyright (C) 1987, 1993, 1994, 1995 Free Software Foundation, Inc.    Hacked by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
name|NODE
parameter_list|)
value|LANG_ID_FIELD(type_desc, NODE)
end_define
...
parameter_list|)
define|\
value|SET_LANG_ID(NODE, DESC, type_desc)
end_define
...
name|NODE
parameter_list|)
value|LANG_ID_FIELD(as_list, NODE)
end_define
...
parameter_list|)
define|\
value|SET_LANG_ID(NODE, LIST, as_list)
end_define
...
begin_comment
comment|/* True for more efficient but incompatible (not not fully tested)    vtable implementation (using thunks).    0 is old behavior; 1 is new behavior. */
end_comment
...
begin_comment
comment|/* Override OFFSET_REFs from the back-end, as we want our very own. */
end_comment

begin_comment
comment|/* Allow complex pointer to members to work correctly. */
end_comment
...
name|t
parameter_list|)
value|(t == RECORD_TYPE || t == UNION_TYPE || t == UNINSTANTIATED_P_TYPE)
end_define
...
define|#
directive|define
name|IS_OVERLOAD_TYPE_CODE
parameter_list|(
name|t
parameter_list|)
value|(IS_AGGR_TYPE_CODE (t) || t == ENUMERAL_TYPE)
end_define
...
name|t
parameter_list|)
value|(IS_OVERLOAD_TYPE_CODE (TREE_CODE (t)))
end_define
...
name|NODE
parameter_list|)
value|(TYPE_LANG_SPECIFIC(NODE)->type_flags.has_real_assignment)
end_define
...
begin_comment
comment|/* Nonzero means that this signature type has a default implementation.  */
end_comment
...
name|NODE
parameter_list|)
value|(TYPE_LANG_SPECIFIC(NODE)->type_flags.has_default_implementation)
end_define

begin_comment
comment|/* Nonzero means that grokdeclarator works on a signature-local typedef.  */
end_comment
...
name|NODE
parameter_list|)
value|(TYPE_LANG_SPECIFIC(NODE)->type_flags.grokking_typedef)
end_define
...
begin_comment
comment|/* The is the VAR_DECL that contains NODE's rtti.  */
end_comment
...
begin_comment
comment|/* List of all explicit methods (chained using DECL_NEXT_METHOD),    in order they were parsed. */
end_comment
...
parameter_list|)
define|\
value|TREE_VEC_LENGTH (CLASSTYPE_METHOD_VEC (NODE))> 1 \     ? TREE_VEC_ELT (CLASSTYPE_METHOD_VEC (NODE), 1) \     : NULL_TREE;
end_define
...
begin_comment
comment|/* Many routines need to cons up a list of basetypes for access    checking.  This field contains a TREE_LIST node whose TREE_VALUE    is the main variant of the type, and whose TREE_VIA_PUBLIC    and TREE_VIA_VIRTUAL bits are correctly set.  */
end_comment
...
begin_comment
comment|/* whether this can be globalized.  */
end_comment
...
name|NODE
parameter_list|)
value|(TYPE_LANG_SPECIFIC(NODE)->type_flags.no_globalize)
end_define
...
parameter_list|)
define|\
value|(TYPE_SIZE (NODE) == NULL_TREE&& TREE_CODE (NODE) != TEMPLATE_TYPE_PARM)
end_define
...
parameter_list|)
define|\
value|(TREE_CODE (TREE_TYPE (NODE)) == METHOD_TYPE || DECL_STATIC_FUNCTION_P (NODE))
end_define
...
parameter_list|)
define|\
value|((TREE_CODE (NODE) == FUNCTION_DECL&& DECL_FUNCTION_MEMBER_P (NODE)) \    ? DECL_CLASS_CONTEXT (NODE) : DECL_CONTEXT (NODE))
end_define
...
begin_comment
comment|/* Next method in CLASSTYPE_METHODS list. */
end_comment
...
name|NODE
parameter_list|)
value|(DECL_LANG_SPECIFIC(NODE)->next_method)
end_define

begin_comment
comment|/* In a VAR_DECL for a variable declared in a for statement,    this is the shadowed variable. */
end_comment
...
name|NODE
parameter_list|)
value|(TREE_CODE (NODE) == CONSTRUCTOR \&& CONSTRUCTOR_ELTS (NODE) == NULL_TREE)
end_define
...
name|NODE
parameter_list|)
value|(TREE_CODE(NODE) == RECORD_TYPE&& TYPE_LANG_SPECIFIC(NODE)->type_flags.ptrmemfunc_flag)
end_define
...
begin_comment
comment|/* These are use to manipulate the the canonical RECORD_TYPE from the    hashed POINTER_TYPE, and can only be used on the POINTER_TYPE. */
end_comment
...
name|NODE
parameter_list|)
value|(build_component_ref (build_component_ref ((NODE), pfn_or_delta2_identifier, 0, 0), delta2_identifier, 0, 0))
end_define
...
name|NODE
parameter_list|)
value|(build_component_ref (build_component_ref ((NODE), pfn_or_delta2_identifier, 0, 0), pfn_identifier, 0, 0))
end_define
...
name|NODE
parameter_list|)
value|(DECL_NESTED_TYPENAME (TYPE_NAME (NODE)))
end_define
...
begin_comment
comment|/* The DECL_ACCESS is used to record under which context    special access rules apply.  */
end_comment
...
begin_comment
comment|/* This decl was declared or deduced to have internal linkage.  This is    only meaningful if TREE_PUBLIC is set.  */
end_comment
...
parameter_list|)
define|\
value|(DECL_LANG_SPECIFIC (NODE)->decl_flags.c_static)
end_define
...
parameter_list|)
define|\
value|(TREE_CODE (NODE) == FUNCTION_DECL \    ? ! DECL_C_STATIC (NODE) : TREE_PUBLIC (NODE))
end_define
