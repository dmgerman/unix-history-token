...
begin_comment
comment|/* Get a virtual binfo that is found inside BINFO's hierarchy that is    the same type as the type given in PARENT.  To be optimal, we want    the first one that is found by going through the least number of    virtual bases.  DEPTH should be NULL_PTR.  */
end_comment
...
name|cp_error
argument_list|(
literal|"return type of `%#D' adds const"
argument_list|,
name|fndecl
argument_list|)
expr_stmt|;
...
name|cp_error
argument_list|(
literal|"return type of `%#D' adds volatile"
argument_list|,
name|fndecl
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* If there is a rtti, it is the first field, though perhaps from      the base class.  Otherwise, the first fields are virtual base class      pointer fields.  */
if|if
condition|(
name|CLASSTYPE_RTTI
argument_list|(
...
begin_comment
comment|/* Fixups upcast offsets for one vtable.    Entries may stay within the VBASE given, or    they may upcast into a direct base, or    they may upcast into a different vbase.     We only need to do fixups in case 2 and 3.     This routine mirrors fixup_vtable_deltas in functionality, though    this one is runtime based, and the other is compile time based.    Conceivably that routine could be removed entirely, and all fixups    done at runtime.     VBASE_OFFSETS is an association list of virtual bases that contains    offset information, so the offsets are only calculated once.  */
end_comment
...
condition|)
block|{
comment|/* If this is set, we need to add in delta adjustments for 		 the other virtual base.  */
name|tree
name|vc_delta
init|=
name|purpose_member
...
directive|else
comment|/* This should should never work better than the above.  (mrs) */
name|tree
name|vbinfo
init|=
name|get_binfo
...
argument_list|)
expr_stmt|;
comment|/* If we are using computed offsets we can skip fixups.  */
if|if
condition|(
name|use_computed_offsets
condition|)
