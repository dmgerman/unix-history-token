...
begin_comment
comment|/* Returns true if type BASE is accessible in T.  (BASE is known to be    a base class of T.)  */
end_comment
...
name|decl
decl_stmt|;
comment|/* [class.access.base]       A base class is said to be accessible if an invented public      member of the base class is accessible.  */
...
begin_comment
comment|/* Returns nonzero if it is OK to access DECL through an object    indiated by BINFO in the context of DERIVED.  */
end_comment
...
begin_comment
comment|/* Recursive helper funciton for is_subobject_of_p; see that routine    for documentation of the parameters.  */
end_comment
...
literal|0
expr_stmt|;
comment|/* Iterate through the base types.  */
for|for
control|(
name|i
operator|=
...
continue|continue;
comment|/* Avoid walking into the same virtual base more than once.  */
if|if
condition|(
name|TREE_VIA_VIRTUAL
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Clear the mark bits on virtual bases.  */
for|for
control|(
name|vbase
operator|=
...
name|want_type
decl_stmt|;
comment|/* If nonzero, RVAL was found by looking through a dependent base.  */
name|int
...
name|base
decl_stmt|;
comment|/* Look for the QUALIFYING_CLASS as a base of the CONTEXT_CLASS. 	 Because we do not yet know which function will be chosen by 	 overload resolution, we cannot yet check either accessibility 	 or ambiguity -- in either case, the choice of a static member 	 function might make the usage valid.  */
name|base
operator|=
name|lookup_base
argument_list|(
