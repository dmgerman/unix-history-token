...
name|CST
parameter_list|)
value|do { sprintf (digit_buffer, "%d", (CST)); \ 			   OB_PUTCP (digit_buffer); } while (0)
end_define
...
expr_stmt|;
block|}
comment|/* DECL_CLASS_CONTEXT isn't being set in some cases.  Hmm...  */
if|if
condition|(
name|DECL_CONTEXT
argument_list|(
...
name|sorry
argument_list|(
literal|"operand of OFFSET_REF not understood"
argument_list|)
expr_stmt|;
goto|goto
name|error
...
begin_comment
comment|/* Same, but handtype a _TYPE.    Called from convert_to_reference, mangle_class_name_for_template,    build_unary_op, and GNU_xref_decl.  */
end_comment
