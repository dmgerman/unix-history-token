...
begin_comment
comment|/* Here, we only decide whether or not the tinfo node should be    emitted with the vtable.  IS_IN_LIBRARY is non-zero iff the    typeinfo for TYPE should be in the runtime library.  */
end_comment
...
condition|)
block|{
comment|/* If this using declaration introduces a function                      recognized as a built-in, no longer mark it as                      anticipated in this scope.  */
if|if
condition|(
name|DECL_ANTICIPATED
argument_list|(
...
name|RECORD_TYPE
condition|)
comment|/* It is legal to define a class with a different class key, 	   and this changes the default member access.  */
