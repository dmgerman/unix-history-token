...
name|NOP_EXPR
case|:
comment|/* If expression doesn't change the type, we consider it as an 	 lvalue even when cast_as_lvalue extension isn't selected. 	 That's because parts of the compiler are alleged to be sloppy 	 about sticking in NOP_EXPR node for no good reason.  */
if|if
condition|(
name|allow_cast_as_lvalue
operator|||
...
name|CONSTRUCTOR
condition|)
comment|/* We need to build up a copy constructor call.  COND_EXPR is a special        case because we already have copies on the arms and we don't want        another one here.  A CONSTRUCTOR is aggregate initialization, which        is handled separately.  */
return|return
name|force_rvalue
argument_list|(
name|init
...
name|error_mark_node
return|;
comment|/* Don't do the minimal thing just because processing_template_decl is      set; we want to give string constants the right type immediately, so      we don't have to fix them up at instantiation time.  */
if|if
condition|(
operator|(
...
decl_stmt|;
comment|/* We keep bad function qualifiers separate, so that we can decide      whether to implement DR 295 or not. DR 295 break existing code,      unfortunately. Remove this variable to implement the defect      report.  */
name|int
name|bad_func_quals
init|=
name|TYPE_UNQUALIFIED
...
name|type
return|;
comment|/* A reference, fucntion or method type shall not be cv qualified.      [dcl.ref], [dct.fct]  */
if|if
condition|(
name|type_quals
operator|&
...
begin_comment
comment|/* Return the OVERLOAD or FUNCTION_DECL inside FNS.  FNS can be an    OVERLOAD, FUNCTION_DECL, TEMPLATE_ID_EXPR, or baselink.  */
end_comment
...
begin_comment
comment|/* Record that we're about to start inlining FN, and return nonzero if    that's OK.  Used for lang_hooks.tree_inlining.start_inlining.  */
end_comment
