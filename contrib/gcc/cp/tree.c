...
name|clk_none
decl_stmt|;
comment|/* Expressions of reference type are sometimes wrapped in      INDIRECT_REFs.  INDIRECT_REFs are just internal compiler      representation, not part of the language, so we have to look      through them.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
condition|)
block|{
comment|/* unnamed rvalue references are rvalues */
if|if
condition|(
...
name|clk_none
return|;
comment|/* lvalue references and named rvalue refences are lvalues */
return|return
name|clk_ordinary
return|;
block|}
...
begin_comment
comment|/* Return a reference type node referring to TO_TYPE.  If RVAL is    true, return an rvalue reference type, otherwise return an lvalue    reference type.  If a type node exists, reuse it, otherwise create    a new one.  */
end_comment
...
name|lvalue_ref
return|;
comment|/* This code to create rvalue reference types is based on and tied      to the code creating lvalue reference types in the middle-end      functions build_reference_type_for_mode and build_reference_type.       It works by putting the rvalue reference type nodes after the      lvalue reference nodes in the TYPE_NEXT_REF_TO linked list, so      they will effectively be ignored by the middle end.  */
for|for
control|(
name|t
operator|=
...
if|if
condition|(
name|TYPE_STRUCTURAL_EQUALITY_P
argument_list|(
name|to_type
argument_list|)
condition|)
name|SET_TYPE_STRUCTURAL_EQUALITY
argument_list|(
name|t
argument_list|)
expr_stmt|;
