...
name|NULL
condition|)
comment|/* We're not generating any back-references.  */
return|return
literal|0
return|;
switch|switch
...
begin_comment
comment|/* Check to see if a tree node has been entered into the Kcode typelist    */
end_comment

begin_comment
comment|/* if not, add it. Return -1 if it isn't found, otherwise return the index */
end_comment
...
directive|if
literal|0
comment|/* We can turn this on at some point when we want 		     improved symbol mangling.  */
block|nrepeats++;
else|#
directive|else
comment|/* This is bug compatible with 2.7.x  */
name|flush_repeats
argument_list|(
name|nrepeats
argument_list|,
...
begin_comment
comment|/* handles emitting modifiers such as Constant, read-only, and volatile */
end_comment
...
literal|0
return|;
comment|/* We assume that our caller has put out any necessary      qualifiers.  */
name|type
operator|=
name|TYPE_MAIN_VARIANT
argument_list|(
...
begin_comment
comment|/* handle emitting the correct code for various node types */
end_comment
...
literal|0
expr_stmt|;
comment|/* These tree types are considered modifiers for B code squangling , */
comment|/* and therefore should not get entries in the Btypelist             */
comment|/* they are, however, repeatable types                               */
switch|switch
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"pointer/reference to array of unknown bound in parm type"
argument_list|)
expr_stmt|;
else|else
block|{
...
literal|1
expr_stmt|;
comment|/* By setting up PARMS in this way, the loop below will 		 automatically clear TREE_USED on THIS_TYPE.  */
name|parms
operator|=
name|temp_tree_cons
argument_list|(
...
literal|0
expr_stmt|;
comment|/* clear out the type variant in case we used it */
name|temp
operator|=
name|canonical_type_variant
argument_list|(
...
begin_comment
comment|/* Like build_decl_overload, but for template functions. */
end_comment
...
begin_comment
comment|/* Returns a DECL_ASSEMBLER_NAME for the destructor of type TYPE.  */
end_comment
...
argument_list|)
condition|)
comment|/* This is a placeholder; don't mark it used.  */
return|return
name|value
return|;
elseif|else
...
directive|if
literal|0
block|tree t = get_first_fn (value);       for (; t; t = DECL_CHAIN (t)) 	{ 	  if (TREE_CODE (t) == TEMPLATE_DECL) 	    continue;  	  assemble_external (t); 	  TREE_USED (t) = 1; 	}
endif|#
directive|endif
block|}
elseif|else
...
name|cp_error
argument_list|(
literal|"in default copy constructor"
argument_list|)
expr_stmt|;
else|else
