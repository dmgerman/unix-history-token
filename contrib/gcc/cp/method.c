...
argument_list|)
expr_stmt|;
comment|/* See if we already have the thunk in question.  */
for|for
control|(
name|thunk
operator|=
...
begin_comment
comment|/* Emit the definition of a C++ multiple inheritance vtable thunk.  If    EMIT_P is nonzero, the thunk is emitted immediately.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* It is expected that a value of zero means no vcall.  */
if|if
condition|(
operator|!
name|vcall_value
...
block|{
comment|/* If we don't have the necessary code for efficient thunks, 	 generate a thunk function that just makes a call to the real 	 function.  Unfortunately, this doesn't work for varargs.  */
name|tree
name|a
decl_stmt|,
name|t
