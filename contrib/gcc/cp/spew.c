...
name|locus
decl_stmt|;
comment|/* location we got the text from */
name|int
name|interface
decl_stmt|;
...
name|tokens
decl_stmt|;
comment|/* Start of the token list.  */
name|struct
name|token_chunk
modifier|*
name|last_chunk
decl_stmt|;
comment|/* End of the token list.  */
name|short
name|last_pos
decl_stmt|;
comment|/* Number of tokens used in the last chunk of 			   TOKENS.  */
name|short
name|cur_pos
decl_stmt|;
comment|/* Current token in 'cur_chunk', when rescanning.  */
name|struct
name|token_chunk
modifier|*
name|cur_chunk
decl_stmt|;
comment|/* Current chunk, when rescanning.  */
block|}
end_decl_stmt
...
specifier|static
name|void
name|snarf_parenthesized_expression
parameter_list|(
name|struct
name|unparsed_text
modifier|*
...
begin_comment
comment|/* Concatenate strings before returning them to the parser.  This isn't quite    as good as having it done in the lexer, but it's better than nothing.  */
end_comment
...
expr_stmt|;
block|}
comment|/* We will have always read one token too many.  */
...
begin_comment
comment|/* token[0] == AGGR (struct/union/enum)    Thus, token[1] is either a tTYPENAME or a TYPENAME_DEFN.    If token[2] == '{' or ':' then it's TYPENAME_DEFN.    It's also a definition if it's a forward declaration (as in 'struct Foo;')    which we can tell if token[2] == ';' *and* token[-1] != FRIEND or NEW.  */
end_comment
...
begin_comment
comment|/* Unget character CH from the input stream.    If RESCAN is nonzero, then we want to `see' this    character as the next input token.  */
end_comment
...
begin_comment
comment|/* Create a new token at the end of the token list in T.  */
end_comment
...
begin_comment
comment|/* Shrink the token list in T by one token.  */
end_comment
...
begin_comment
comment|/* Allocate an 'unparsed_text' structure, ready to use space_for_token.  */
end_comment
...
begin_comment
comment|/* Accumulate the tokens that make up a parenthesized expression in T,    having already read the opening parenthesis.  */
end_comment
...
name|error
argument_list|(
literal|"%Hend of file read inside definition"
argument_list|,
operator|&
name|t
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Add three END_OF_SAVED_INPUT tokens.  We used to provide an      infinite stream of END_OF_SAVED_INPUT tokens -- but that can      cause the compiler to get stuck in an infinite loop when      encountering invalid code.  We need more than one because the      parser sometimes peeks ahead several tokens.  */
name|memcpy
argument_list|(
name|space_for_token
argument_list|(
...
begin_comment
comment|/* Consume a no-commas expression - a default argument - and return    a DEFAULT_ARG tree node.  */
end_comment
...
name|error
argument_list|(
literal|"%Hend of file read inside default argument"
argument_list|,
operator|&
name|starting
argument_list|)
