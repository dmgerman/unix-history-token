...
begin_comment
comment|/* Common subexpression elimination library for GNU compiler.    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2003, 2004, 2005 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* The largest number of hard regs used by any entry added to the    REG_VALUES table.  Cleared on each cselib_clear_table() invocation.  */
end_comment
...
begin_comment
comment|/* Here the set of indices I with REG_VALUES(I) != 0 is saved.  This is used    in cselib_clear_table() for fast emptying.  */
end_comment
...
begin_comment
comment|/* Used as stop element of the containing_mem list so we can check    presence in the list by checking the next pointer.  */
end_comment
...
begin_comment
comment|/* Used to list all values that contain memory reference.    May or may not contain the useless values - the list is compacted    each time memory is invalidated.  */
end_comment
...
specifier|static
name|alloc_pool
name|elt_loc_list_pool
decl_stmt|,
name|elt_list_pool
decl_stmt|,
name|cselib_val_pool
decl_stmt|,
name|value_pool
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* The hash function for our hash table.  The value is always computed with    cselib_hash_rtx when adding an element; this function just extracts the    hash value from a cselib_val structure.  */
end_comment
...
begin_comment
comment|/* Return the mode in which a register was last set.  If X is not a    register, return its mode.  If the mode in which the register was    set is not known, or the value was already clobbered, return    VOIDmode.  */
end_comment
...
literal|0
return|;
comment|/* These won't be handled correctly by the code below.  */
switch|switch
condition|(
name|GET_CODE
argument_list|(
...
name|value
expr_stmt|;
comment|/* We use an alloc pool to allocate this RTL construct because it      accounts for about 8% of the overall memory usage.  We know      precisely when we can have VALUE RTXen (when cselib is active)      so we don't need to put them in garbage collected memory.      ??? Why should a VALUE be an RTX in the first place?  */
name|e
operator|->
name|u
operator|.
...
condition|)
block|{
comment|/* Maintain the invariant that the first entry of 	     REG_VALUES, if present, must be the value used to set the 	     register, or NULL.  */
name|used_regs
index|[
name|n_used_regs
operator|++
...
condition|)
block|{
comment|/* Maintain the invariant that the first entry of 		 REG_VALUES, if present, must be the value used to set 		 the register, or NULL.  This is also nice because 		 then we won't push the same regno onto user_regs 		 multiple times.  */
operator|(
operator|*
name|l
operator|)
...
parameter_list|)
block|{
comment|/* We actually don't need to verify very hard.  This is because      if X has actually changed, we invalidate the memory anyway,      so assume that all common memory addresses are      invariant.  */
return|return
literal|0
return|;
block|}
...
name|PARAM_VALUE
argument_list|(
name|PARAM_MAX_CSELIB_MEMORY_LOCATIONS
argument_list|)
operator|&&
operator|!
...
begin_comment
comment|/* Invalidate DEST, which is being assigned to or clobbered.  */
end_comment
...
begin_comment
comment|/* A wrapper for cselib_invalidate_rtx to be called via note_stores.  */
end_comment

begin_function
specifier|static
name|void
name|cselib_invalidate_rtx_note_stores
parameter_list|(
name|rtx
name|dest
parameter_list|,
...
block|{
comment|/* The register should have been invalidated.  */
name|gcc_assert
argument_list|(
name|REG_VALUES
argument_list|(
...
operator|>
name|MAX_USELESS_VALUES
comment|/* remove_useless_values is linear in the hash table size.  Avoid          quadratic behaviour for very large hashtables with very few 	 useless elements.  */
operator|&&
operator|(
name|unsigned
name|int
...
argument_list|()
expr_stmt|;
comment|/* We preserve reg_values to allow expensive clearing of the whole thing.      Reallocate it however if it happens to be too large.  */
if|if
condition|(
operator|!
name|reg_values
...
argument_list|)
expr_stmt|;
comment|/* Some space for newly emit instructions so we don't end up 	 reallocating in between passes.  */
name|reg_values_size
operator|=
name|cselib_nregs
operator|+
