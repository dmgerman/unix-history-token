...
begin_comment
comment|/* Common subexpression elimination for GNU compiler.    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998    1999, 2000, 2001, 2002, 2003, 2004, 2005 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* Structure used to pass arguments via for_each_rtx to function    cse_change_cc_mode.  */
end_comment

begin_struct
struct|struct
name|change_cc_mode_args
block|{
name|rtx
name|insn
decl_stmt|;
...
begin_comment
comment|/* Index by register number, gives the number of the next (or    previous) register in the chain of registers sharing the same    value.     Or -1 if this register is at the end of the chain.     If REG_QTY (N) == -N - 1, reg_eqv_table[N].next is undefined.  */
end_comment
...
name|cse_reg_info
block|{
comment|/* The timestamp at which this register is initialized.  */
name|unsigned
name|int
name|timestamp
decl_stmt|;
...
begin_comment
comment|/* A table of cse_reg_info indexed by register numbers.  */
end_comment
...
name|cse_reg_info
modifier|*
name|cse_reg_info_table
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* The size of the above table.  */
end_comment
...
name|unsigned
name|int
name|cse_reg_info_table_size
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* The index of the first entry that has not been initialized.  */
end_comment
...
name|unsigned
name|int
name|cse_reg_info_table_first_uninitialized
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* The timestamp at the beginning of the current run of    cse_basic_block.  We increment this variable at the beginning of    the current run of cse_basic_block.  The timestamp field of a    cse_reg_info entry matches the value of this variable if and only    if the entry has been initialized during the current run of    cse_basic_block.  */
end_comment
...
parameter_list|)
define|\
value|((REG_P (X)&& REGNO (X)>= FIRST_PSEUDO_REGISTER	\   ? (((unsigned) REG<< 7) + (unsigned) REG_QTY (REGNO (X)))	\   : canon_hash (X, M))& HASH_MASK)
end_define

begin_comment
comment|/* Like HASH, but without side-effects.  */
end_comment
...
parameter_list|)
define|\
value|((REG_P (X)&& REGNO (X)>= FIRST_PSEUDO_REGISTER	\   ? (((unsigned) REG<< 7) + (unsigned) REG_QTY (REGNO (X)))	\   : safe_hash (X, M))& HASH_MASK)
end_define
...
parameter_list|)
define|\
value|(REGNO_PTR_FRAME_P(N)							\    || (HARD_REGISTER_NUM_P (N)						\&& FIXED_REGNO_P (N)&& REGNO_REG_CLASS (N) != NO_REGS))
end_define
...
name|X
parameter_list|)
value|(REG_P (X) ? 0 : notreg_cost (X, SET))
end_define
...
name|OUTER
parameter_list|)
value|(REG_P (X) ? 0 : notreg_cost (X, OUTER))
end_define
...
begin_comment
comment|/* Number of elements in the hash table.  */
end_comment
...
begin_comment
comment|/* Returns a canonical version of X for the address, from the point of view,    that all multiplications are represented as MULT instead of the multiply    by a power of 2 being represented as ASHIFT.  */
end_comment

begin_function
specifier|static
name|rtx
name|canon_for_address
parameter_list|(
name|rtx
name|x
parameter_list|)
...
begin_comment
comment|/* Initialize CSE_REG_INFO_TABLE.  */
end_comment

begin_function
specifier|static
name|void
name|init_cse_reg_info
parameter_list|(
name|unsigned
name|int
name|nregs
parameter_list|)
block|{
comment|/* Do we need to grow the table?  */
if|if
condition|(
name|nregs
operator|>
...
condition|)
block|{
comment|/* Compute a new size that is a power of 2 and no smaller 	     than the large of NREGS and 64.  */
name|new_size
operator|=
operator|(
name|cse_reg_info_table_size
...
block|{
comment|/* If we need a big table, allocate just enough to hold 	     NREGS registers.  */
name|new_size
operator|=
name|nregs
expr_stmt|;
block|}
comment|/* Reallocate the table with NEW_SIZE entries.  */
if|if
condition|(
name|cse_reg_info_table
condition|)
...
expr_stmt|;
block|}
comment|/* Do we have all of the first NREGS entries initialized?  */
if|if
condition|(
name|cse_reg_info_table_first_uninitialized
operator|<
...
name|i
decl_stmt|;
comment|/* Put the old timestamp on newly allocated entries so that they 	 will all be considered out of date.  We do not touch those 	 entries beyond the first NREGS entries to be nice to the 	 virtual memory.  */
for|for
control|(
name|i
operator|=
...
begin_comment
comment|/* Given REGNO, initialize the cse_reg_info entry for REGNO.  */
end_comment
...
parameter_list|)
block|{
comment|/* Set TIMESTAMP field to CSE_REG_INFO_TIMESTAMP so that this      entry will be considered to have been initialized.  */
name|cse_reg_info_table
index|[
name|regno
index|]
...
name|cse_reg_info_timestamp
expr_stmt|;
comment|/* Initialize the rest of the entry.  */
name|cse_reg_info_table
index|[
name|regno
index|]
...
begin_comment
comment|/* Find a cse_reg_info entry for REGNO.  */
end_comment
...
index|]
decl_stmt|;
comment|/* If this entry has not been initialized, go ahead and initialize      it.  */
if|if
condition|(
name|p
operator|->
...
literal|0
expr_stmt|;
comment|/* Invalidate cse_reg_info_table.  */
name|cse_reg_info_timestamp
operator|++
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Suppose that 5 is hard reg and 100 and 101 are 		     pseudos.  Consider  		     (set (reg:si 100) (reg:si 5)) 		     (set (reg:si 5) (reg:si 100)) 		     (set (reg:di 101) (reg:di 5))  		     We would now set REG_QTY (101) = REG_QTY (5), but the 		     entry for 5 is in SImode.  When we use this later in 		     copy propagation, we get the register in wrong mode.  */
if|if
condition|(
name|qty_table
index|[
...
begin_comment
comment|/* Same as above, but X is a pseudo-register.  */
end_comment
...
name|elt
decl_stmt|;
comment|/* Because a pseudo-register can be referenced in more than one      mode, we might have to remove more than one table entry.  */
while|while
condition|(
operator|(
name|elt
...
parameter_list|)
define|\
value|(preferable ((X)->cost, (X)->regcost, (Y)->cost, (Y)->regcost)< 0)
end_define
...
begin_comment
comment|/* Hash an rtx.  We are careful to make sure the value is never negative.    Equivalent registers hash identically.    MODE is used in hashing for CONST_INTs only;    otherwise the mode of X is used.     Store 1 in DO_NOT_RECORD_P if any subexpression is volatile.     If HASH_ARG_IN_MEMORY_P is not NULL, store 1 in it if X contains    a MEM rtx which does not have the RTX_UNCHANGING_P bit set.     Note that cse_insn knows that the hash code of a MEM expression    is just (int) MEM plus the hash code of the address.  */
end_comment
...
name|USE
case|:
comment|/* A USE that mentions non-volatile memory needs special 	 handling since the MEM may be BLKmode which normally 	 prevents an entry from being made.  Pure calls are 	 marked by a USE which mentions BLKmode memory. 	 See calls.c:emit_call_1.  */
if|if
condition|(
name|MEM_P
argument_list|(
...
begin_comment
comment|/* Hash an rtx X for cse via hash_rtx.    Stores 1 in do_not_record if any subexpression is volatile.    Stores 1 in hash_arg_in_memory if X contains a mem rtx which    does not have the RTX_UNCHANGING_P bit set.  */
end_comment
...
begin_comment
comment|/* Like canon_hash but with no side effects, i.e. do_not_record    and hash_arg_in_memory are not changed.  */
end_comment
...
begin_comment
comment|/* Return 1 iff X and Y would canonicalize into the same thing,    without actually constructing the canonicalization of either one.    If VALIDATE is nonzero,    we assume X is an expression being processed from the rtl    and Y was found in the hash table.  We check register refs    in Y for being marked as valid.     If FOR_GCSE is true, we compare X and Y for equivalence for GCSE.  */
end_comment
...
begin_comment
comment|/* Subroutine of canon_reg.  Pass *XLOC through canon_reg, and validate    the result if necessary.  INSN is as for canon_reg.  */
end_comment

begin_function
specifier|static
name|void
name|validate_canon_reg
parameter_list|(
name|rtx
modifier|*
name|xloc
...
name|new_cost
decl_stmt|;
comment|/* Get the canonical version of the address so we can accept 		   more.  */
name|new
operator|=
name|canon_for_address
argument_list|(
...
comment|/* A SUBREG resulting from a zero extension may fold to zero 	   if it extracts higher bits than the ZERO_EXTEND's source 	   bits.  FIXME: if combine tried to, er, combine these 	   instructions, this transformation may be moved to 	   simplify_subreg.  */
for|for
control|(
init|;
name|elt
...
begin_comment
comment|/* Fold MEM.  Not to be called directly, see fold_rtx_mem instead.  */
end_comment
...
name|set
decl_stmt|;
comment|/* If we have an insn that loads the label from the 		   jumptable into a reg, we don't want to set the reg 		   to the label, because this may cause a reference to 		   the label to remain after the label is removed in 		   some very obscure cases (PR middle-end/18628).  */
if|if
condition|(
operator|!
name|insn
...
name|x
return|;
comment|/* If it's a jump, it's safe to reference the label.  */
if|if
condition|(
name|SET_DEST
argument_list|(
...
parameter_list|)
block|{
comment|/* To avoid infinite oscillations between fold_rtx and fold_rtx_mem,      refuse to allow recursion of the latter past n levels.  This can      happen because fold_rtx_mem will try to fold the address of the      memory reference it is passed, i.e. conceptually throwing away      the MEM and reinjecting the bare address into fold_rtx.  As a      result, patterns like         set (reg1) 	   (plus (reg) 		 (mem (plus (reg2) (const_int))))         set (reg2) 	   (plus (reg) 		 (mem (plus (reg1) (const_int))))       will defeat any "first-order" short-circuit put in either      function to prevent these infinite oscillations.       The heuristics for determining n is as follows: since each time      it is invoked fold_rtx_mem throws away a MEM, and since MEMs      are generically not nested, we assume that each invocation of      fold_rtx_mem corresponds to a new "top-level" operand, i.e.      the source or the destination of a SET.  So fold_rtx_mem is      bound to stop or cycle before n recursions, n being the number      of expressions recorded in the hash table.  We also leave some      play to account for the initial steps.  */
specifier|static
name|unsigned
name|int
name|depth
...
condition|)
break|break;
comment|/* It's not safe to substitute the operand of a conversion 	       operator with a constant, as the conversion's identity 	       depends upon the mode of its operand.  This optimization 	       is handled by the call to simplify_unary_operation.  */
if|if
condition|(
name|GET_RTX_CLASS
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* NEG of PLUS could be converted into MINUS, but that causes 	   expressions of the form 	   (CONST (MINUS (CONST_INT) (SYMBOL_REF))) 	   which many ports mistakenly treat as LEGITIMATE_CONSTANT_P. 	   FIXME: those ports should be fixed.  */
if|if
condition|(
name|new
operator|!=
...
comment|/* ??? Vector mode comparisons are not supported yet.  */
if|if
condition|(
name|VECTOR_MODE_P
argument_list|(
...
name|p
decl_stmt|;
comment|/* See if we can find an equivalent of folded_arg0 		     that gets us a cheaper expression, possibly a 		     constant through simplifications.  */
name|p
operator|=
name|lookup
argument_list|(
...
condition|)
continue|continue;
comment|/* Try to simplify using this equivalence.  */
name|simp_result
operator|=
...
block|}
block|}
comment|/* If we have a cheaper expression now, use that 			 and try folding it further, from the top.  */
if|if
condition|(
name|cheapest_simplification
operator|!=
...
argument_list|)
expr_stmt|;
comment|/* If the mode is a MODE_CC mode, we don't know what kinds of things      are being compared, so we can't do anything with this      comparison.  */
if|if
condition|(
name|GET_MODE_CLASS
argument_list|(
...
begin_comment
comment|/* Yet another form of subreg creation.  In this case, we want something in    MODE, and we should assume OP has MODE iff it is naturally modeless.  */
end_comment

begin_function
specifier|static
name|rtx
name|record_jump_cond_subreg
parameter_list|(
name|enum
name|machine_mode
name|mode
...
name|src_const_elt
decl_stmt|;
comment|/* Table entry for the destination address.  */
name|struct
name|table_elt
modifier|*
name|dest_addr_elt
...
block|if (GET_CODE (SET_DEST (sets[i].rtl)) == ZERO_EXTRACT) 	{ 	  rtx width = XEXP (SET_DEST (sets[i].rtl), 1);  	  if (GET_CODE (src) == CONST_INT&& GET_CODE (width) == CONST_INT&& INTVAL (width)< HOST_BITS_PER_WIDE_INT&& (INTVAL (src)& ((HOST_WIDE_INT) (-1)<< INTVAL (width)))) 	    src_folded 	      = GEN_INT (INTVAL (src)& (((HOST_WIDE_INT) 1<< INTVAL (width)) - 1)); 	}
endif|#
directive|endif
...
comment|/* If source is a paradoxical subreg (such as QI treated as an SI), 	 treat it as volatile.  It may do the work of an SI in one context 	 where the extra bits are not being used, but cannot replace an SI 	 in general.  */
...
directive|ifdef
name|LOAD_EXTEND_OP
comment|/* See if a MEM has already been loaded with a widening operation; 	 if it has, we can use a subreg of that.  Many CISC machines 	 also have such operations, but this is only likely to be 	 beneficial on these machines.  */
if|if
condition|(
name|flag_expensive_optimizations
operator|&&
...
condition|)
block|{
comment|/* Don't substitute non-local labels, this confuses CFG.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
break|break;
block|}
comment|/* Reject certain invalid forms of CONST that we create.  */
elseif|else
if|if
condition|(
name|CONSTANT_P
...
operator|==
name|CONST
comment|/* Reject cases that will cause decode_rtx_const to 		      die.  On the alpha when simplifying a switch, we 		      get (const (truncate (minus (label_ref) 		      (label_ref)))).  */
operator|&&
operator|(
name|GET_CODE
argument_list|(
...
operator|)
condition|)
comment|/* Do nothing for this case.  */
empty_stmt|;
...
literal|0
expr_stmt|;
comment|/* Look within any ZERO_EXTRACT to the MEM or REG within it.  */
while|while
condition|(
name|GET_CODE
argument_list|(
...
operator|++
expr_stmt|;
comment|/* Make sure to copy over REG_NON_LOCAL_GOTO.  */
name|note
operator|=
name|find_reg_note
argument_list|(
...
name|src_eqv_elt
expr_stmt|;
comment|/* Record destination addresses in the hash table.  This allows us to      check if they are invalidated by other sets.  */
for|for
control|(
name|i
operator|=
...
block|}
block|}
comment|/* We may have just removed some of the src_elt's from the hash table.      So replace each one with the current head of the same class.      Also check if destination addresses have been removed.  */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* The elt was removed, which means this destination is not 	       valid after this instruction.  */
name|sets
index|[
name|i
index|]
...
name|i
decl_stmt|;
comment|/* Update the previous branch path, if any.  If the last branch was      previously PATH_TAKEN, mark it PATH_NOT_TAKEN.      If it was previously PATH_NOT_TAKEN,      shorten the path by one and look at the previous branch.  We know that      at least one branch must have been taken if PATH_SIZE is nonzero.  */
while|while
condition|(
name|path_size
operator|>
...
begin_comment
comment|/* Process a single basic block.  FROM and TO and the limits of the basic    block.  NEXT_BRANCH points to the branch path when following jumps or    a null path when not following jumps.  */
end_comment
...
name|JUMP_INSN
case|:
comment|/* We expect dest to be NULL_RTX here.  If the insn may trap, mark        this fact by setting DEST to pc_rtx.  */
if|if
condition|(
...
name|ASM_OPERANDS
case|:
comment|/* If the asm is volatile, then this insn cannot be deleted, 	 and so the inputs *must* be live.  */
if|if
condition|(
name|MEM_VOLATILE_P
argument_list|(
...
name|dump_file
argument_list|,
literal|"Deleted %i trivially dead insns\n"
argument_list|,
name|ndead
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Change the mode of any reference to the register REGNO (NEWREG) to    GET_MODE (NEWREG) in INSN.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If the following assertion was triggered, there is most probably      something wrong with the cc_modes_compatible back end function.      CC modes only can be considered compatible if the insn - with the mode      replaced by any of the compatible modes - can still be recognized.  */
name|success
operator|=
name|apply_change_group
argument_list|()
...
name|comp_mode
expr_stmt|;
comment|/* The modified insn will be re-recognized later.  */
name|PUT_MODE
argument_list|(
name|cc_src
argument_list|,
