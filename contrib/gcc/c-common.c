...
begin_comment
comment|/* APPLE LOCAL radar 5932809 - copyable byref blocks */
end_comment

begin_function_decl
specifier|static
name|tree
name|handle_blocks_attribute
parameter_list|(
name|tree
modifier|*
parameter_list|,
...
block|}
block|,
comment|/* APPLE LOCAL weak types 5954418 */
block|{
literal|"weak"
block|,
literal|0
...
comment|/* APPLE LOCAL begin radar 4727659 */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
condition|)
comment|/* APPLE LOCAL end radar 4727659 */
name|TREE_THIS_VOLATILE
argument_list|(
operator|*
name|node
...
name|node
argument_list|)
comment|/* If the weak flag can be associated with something else, 	      prefer that. */
operator|&&
operator|(
name|flags
operator|&
...
condition|)
block|{
comment|/* Leave on the type for the C++ front end */
block|}
...
return|;
block|}
comment|/* APPLE LOCAL begin blocks 5925781 */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|BLOCK_POINTER_TYPE
condition|)
comment|/* APPLE LOCAL end blocks 5925781 */
block|{
name|error
argument_list|(
...
begin_comment
comment|/* Handle "blocks" attribute. */
end_comment
...
name|arg_ident
decl_stmt|;
comment|/* APPLE LOCAL radar 6217257 */
name|tree
name|type
decl_stmt|;
operator|*
...
name|error
argument_list|(
literal|"__block attribute can be specified on variables only"
argument_list|)
expr_stmt|;
return|return
name|NULL_TREE
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 6096219 */
if|if
condition|(
name|strcmp
argument_list|(
...
name|OPT_Wattributes
argument_list|,
literal|"Only \"byref\" is allowed - %qE attribute ignored"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* APPLE LOCAL begin radar 6217257 */
name|type
operator|=
name|TREE_TYPE
argument_list|(
...
name|error
argument_list|(
literal|"__block not allowed on a variable length array declaration"
argument_list|)
expr_stmt|;
return|return
name|NULL_TREE
...
block|}
block|}
comment|/* APPLE LOCAL end radar 6217257 */
...
begin_comment
comment|/* This routine builds:    *(void **)(EXP+20) expression which references the object pointer. */
end_comment
...
name|offset
decl_stmt|;
comment|/* dst->object In thid case 'object' is the field    of the object passed offset by: void * + void* + int + int + void* + void *    This must match definition of Block_byref structs. */
...
begin_comment
comment|/* This routine builds call to:  _Block_object_dispose(VAR_DECL.__forwarding, BLOCK_FIELD_IS_BYREF);  and adds it to the statement list.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* __block variables imported into Blocks are not _Block_object_dispose()    from within the Block statement itself; otherwise, each envokation of    the block causes a release. Make sure to release __block variables declared     and used locally in the block though. */
if|if
condition|(
name|cur_block
operator|&&
...
condition|)
block|{
comment|/* This is a "struct Block_byref_X *" type. Get its pointee. */
name|gcc_assert
argument_list|(
name|POINTER_TYPE_P
argument_list|(
...
literal|1
expr_stmt|;
comment|/* Declare: _Block_object_dispose(void*, BLOCK_FIELD_IS_BYREF) if not done already. */
name|exp
operator|=
...
begin_comment
comment|/** add_block_global_byref_list - Adds global variable decl to the list of     byref global declarations in the current block. */
end_comment
...
begin_comment
comment|/** in_block_global_byref_list - returns TRUE if global variable is     in the list of 'byref' declarations. */
end_comment
...
begin_comment
comment|/* APPLE LOCAL begin radar 6160536 */
end_comment
...
begin_comment
comment|/* APPLE LOCAL end radar 6160536 */
end_comment
...
begin_comment
comment|/* This routine is called for a "format" attribute. It adds the number of  hidden argument ('1') to the format's 2nd and 3rd argument to compensate  for these two arguments. This is to make rest of the "format" attribute  processing done in the middle-end to work seemlessly. */
end_comment

begin_function
specifier|static
name|void
name|block_delta_format_args
parameter_list|(
name|tree
name|format
parameter_list|)
...
begin_comment
comment|/* This routine recognizes legal block attributes. In case of block's "format"   attribute, it calls block_delta_format_args to compensate for hidden   argument _self getting passed to block's helper function. */
end_comment
...
argument_list|)
name|tree
name|block_object_dispose_func_decl
expr_stmt|;
end_expr_stmt

begin_comment
comment|/* This routine declares:    void _Block_object_assign (void *, void *, int) or uses an    existing one. */
end_comment

begin_function
specifier|static
name|tree
name|build_block_object_assign_decl
parameter_list|(
name|void
parameter_list|)
block|{
...
begin_comment
comment|/* This routine builds:    _Block_object_assign(dest, src, flag) */
end_comment
...
begin_comment
comment|/* This routine declares:    void _Block_object_dispose (void *, int) or uses an    existing one. */
end_comment

begin_function
specifier|static
name|tree
name|build_block_object_dispose_decl
parameter_list|(
name|void
parameter_list|)
block|{
...
begin_comment
comment|/* This routine builds the call tree:    _Block_object_dispose(src, flag) */
end_comment
...
name|s1
decl_stmt|;
comment|/* Check for possibility of an error condition. */
if|if
condition|(
name|TREE_CODE
argument_list|(
