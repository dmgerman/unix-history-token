...
begin_comment
comment|/* Subroutines shared by all languages that are variants of C.    Copyright (C) 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000,    2001, 2002, 2003, 2004 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Warn about comparison of signed and unsigned values.    If -1, neither -Wsign-compare nor -Wno-sign-compare has been specified    (in which case -Wextra gets to decide).  */
end_comment
...
condition|)
block|{
comment|/* They were called into existence, so add to statement tree.  Add 	 the DECL_STMTs inside the outermost scope.  */
name|tree
modifier|*
name|p
init|=
...
argument_list|)
decl_stmt|;
comment|/* Skip the dummy EXPR_STMT and any EH_SPEC_BLOCK.  */
while|while
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Return the text name of the current function, suitably prettified    by PRETTY_P.  */
end_comment
...
name|pedwarn
argument_list|(
literal|"'%D' is not defined outside of function scope"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
name|type
return|;
comment|/* Must check the mode of the types, not the precision.  Enumeral types      in C++ have precision set to match their range, but may use a wider      mode to match an ABI.  If we change modes, we may wind up with bad      conversions.  */
if|if
condition|(
name|TYPE_MODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* In C, if TYPE is an enumeration, then we need to get its 	 min/max values from it's underlying integral type, not the 	 enumerated type itself.  In C++, TYPE_MAX_VALUE and 	 TYPE_MIN_VALUE have already been set correctly on the 	 enumeration type.  */
if|if
condition|(
operator|!
name|c_dialect_cxx
...
name|warning
argument_list|(
literal|"the address of `%D', will always evaluate as `true'"
argument_list|,
name|TREE_OPERAND
argument_list|(
name|expr
...
name|error
argument_list|(
literal|"invalid application of `%s' to incomplete type `%T' "
argument_list|,
name|op_name
argument_list|,
name|type
...
name|warning
argument_list|(
literal|"%Hignoring return value of `%D', "
...
condition|)
block|{
comment|/* If it is the main variant, then pack the other variants    	     too. This happens in, 	      	     struct Foo { 	       struct Foo const *ptr; // creates a variant w/o packed flag 	       } __ attribute__((packed)); // packs it now. 	  */
name|tree
name|probe
decl_stmt|;
for|for
...
name|error
argument_list|(
literal|"invalid pointer mode `%s'"
argument_list|,
name|p
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* We cannot use layout_type here, because that will attempt 		 to re-layout all variants, corrupting our original.  */
name|TYPE_PRECISION
argument_list|(
name|type
argument_list|)
...
name|error
argument_list|(
literal|"mode `%s' applied to inappropriate type"
argument_list|,
name|p
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"%Jsection of '%D' conflicts with previous declaration"
argument_list|,
operator|*
name|node
argument_list|,
...
name|error
argument_list|(
literal|"%Jsection attribute not allowed for '%D'"
argument_list|,
operator|*
name|node
argument_list|,
...
name|error
argument_list|(
literal|"%Jalignment may not be specified for '%D'"
argument_list|,
name|decl
argument_list|,
name|decl
...
name|error
argument_list|(
literal|"%J'%D' defined both normally and as an alias"
argument_list|,
name|decl
argument_list|,
name|decl
...
name|error
argument_list|(
literal|"%J'%E' attribute applies only to functions"
argument_list|,
name|decl
argument_list|,
name|name
...
name|error
argument_list|(
literal|"%Jcan't set '%E' attribute after definition"
argument_list|,
name|decl
argument_list|,
name|name
...
condition|)
block|{
comment|/* Recognize assignments of large structures and constructors of        big arrays.  */
case|case
name|MODIFY_EXPR
case|:
case|case
...
break|break;
block|}
comment|/* Few special cases of expensive operations.  This is usefull        to avoid inlining on functions having too many of these.  */
case|case
name|TRUNC_DIV_EXPR
case|:
case|case
...
expr_stmt|;
break|break;
comment|/* Various containers that will produce no code themselves.  */
case|case
name|INIT_EXPR
case|:
case|case
...
name|DECL_STMT
case|:
comment|/* Do not account static initializers.  */
if|if
condition|(
name|TREE_STATIC
argument_list|(
...
begin_comment
comment|/*  Estimate number of instructions that will be created by expanding the body.  */
end_comment
...
begin_comment
comment|/* Used by c_decl_uninit to find where expressions like x = x + 1; */
end_comment

begin_function
specifier|static
name|tree
name|c_decl_uninit_1
parameter_list|(
name|tree
modifier|*
name|t
...
parameter_list|)
block|{
comment|/* If x = EXP(&x)EXP, then do not warn about the use of x.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
begin_comment
comment|/* Find out if a variable is uninitialized based on DECL_INITIAL.  */
end_comment
...
parameter_list|)
block|{
comment|/* int x = x; is GCC extension to turn off this warning, only if warn_init_self is zero.  */
if|if
condition|(
name|DECL_INITIAL
argument_list|(
...
name|false
return|;
comment|/* Walk the trees looking for the variable itself.  */
if|if
condition|(
name|walk_tree_without_duplicates
argument_list|(
...
begin_comment
comment|/* Issue the error given by MSGID, indicating that it occurred before    TOKEN, which had the associated VALUE.  */
end_comment
