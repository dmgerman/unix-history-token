...
name|char
modifier|*
name|alpha_strip_name_encoding
name|PARAMS
argument_list|(
operator|(
specifier|const
...
directive|define
name|TARGET_ENCODE_SECTION_INFO
value|alpha_encode_section_info
end_define
...
directive|define
name|TARGET_STRIP_NAME_ENCODING
value|alpha_strip_name_encoding
end_define
...
directive|define
name|TARGET_ASM_GLOBALIZE_LABEL
value|hook_FILEptr_constcharptr_void
end_define
...
argument_list|)
expr_stmt|;
comment|/* If @[LS], then alpha_encode_section_info sez it's local.  */
if|if
condition|(
name|str
index|[
...
literal|0
return|;
comment|/* Also verify that it's not a TLS symbol.  */
name|str
operator|=
name|XSTR
argument_list|(
...
condition|)
block|{
comment|/* ??? Be prepared for -ftls-model=local-dynamic.  Perhaps we shouldn't 	 have separately encoded local-ness.  On well, maybe the user will use 	 attribute visibility next time.  At least we don't crash...  */
if|if
condition|(
name|str
index|[
...
condition|)
block|{
comment|/* Local dynamic is a waste if we're not going to combine 	     the __tls_get_addr calls.  So avoid it if not optimizing.  */
if|if
condition|(
name|optimize
condition|)
...
condition|)
block|{
comment|/* 64-bit local exec is the same as initial exec except without 	     the dynamic relocation.  In either case we use a got entry.  */
if|if
condition|(
name|alpha_tls_size
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Care for TLS variables.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* Glibc likes to use $31 as an unwind stopper for crt0.  To 	 avoid hackery in unwind-dw2.c, we need to actively store a 	 zero in the prologue of _Unwind_RaiseException et al.  */
name|imask
operator||=
literal|1UL
operator|<<
...
expr_stmt|;
block|}
comment|/* Store a zero if requested for unwinding.  */
if|if
condition|(
name|imask
operator|&
...
directive|endif
comment|/* Show that we know this function if it is called again.       Do this only for functions whose symbols bind locally.       Don't do this for functions not defined in the .text section, as      otherwise it's not unlikely that the destination is out of range      for a direct branch.  */
if|if
condition|(
call|(
modifier|*
...
name|g_switch_value
condition|)
comment|/* ??? Consider using mergable sdata sections.  */
name|sdata_section
argument_list|()
expr_stmt|;
else|else
