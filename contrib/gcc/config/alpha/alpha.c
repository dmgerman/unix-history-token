...
expr_stmt|;
block|}
comment|/* Do some sanity checks on the above option. */
if|if
condition|(
operator|(
name|alpha_fptm
...
begin_comment
comment|/* HOST_BITS_PER_WIDE_INT == 64 */
end_comment
...
expr_stmt|;
block|}
comment|/* ??? We mark the the branch mode to be CCmode to prevent the compare      and cmov from being combined, since the compare insn follows IEEE      rules that the cmov does not.  */
if|if
condition|(
name|alpha_compare_fp_p
operator|&&
...
condition|)
block|{
comment|/* Whee!  Optimize the load to use the existing register.  */
name|data_regs
index|[
name|nregs
operator|++
...
goto|;
block|}
comment|/* ??? We could potentially be copying 3 bytes or whatnot from 	 a wider reg.  Probably not worth worrying about.  */
...
name|addr
decl_stmt|;
comment|/* ??? Something is wrong with VMS codegen in that we get aborts when      using ptr_mode.  Hack around it for now.  */
name|enum
name|machine_mode
name|mode
init|=
...
name|FALSE
expr_stmt|;
comment|/* Show that we know this function if it is called again.        Don't do this for global functions in object files destined for a      shared library because the function may be overridden by the application      or other libraries.      ??? Is this just ELF?  */
if|if
condition|(
operator|!
name|flag_pic
