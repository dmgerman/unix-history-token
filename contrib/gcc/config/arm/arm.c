...
name|LR_REGNUM
index|]
comment|/* If optimizing for size, then we save the link register if 	      any other integer register is saved.  This gives a smaller 	      return sequence.  */
operator|||
operator|(
name|optimize_size
operator|&&
...
operator|)
operator|)
comment|/* But if a stack frame is going to be created, the LR will 	     be saved as part of that, so we do not need to allow for 	     it here.  */
operator|&&
operator|!
name|frame_pointer_needed
condition|)
...
name|LAST_ARM_REGNUM
condition|)
comment|/* We allow any value to be stored in the general regisetrs.  */
return|return
literal|1
return|;
if|if
