...
directive|define
name|TARGET_STRIP_NAME_ENCODING
value|ia64_strip_name_encoding
end_define
...
begin_comment
comment|/* Return 0 if we are doing C++ code.  This optimization fails with    C++ because of GNAT c++/6685.  */
end_comment
...
name|ia64_init_machine_status
expr_stmt|;
comment|/* Tell the compiler which flavor of TFmode we're using.  */
if|if
condition|(
name|INTEL_EXTENDED_IEEE_FORMAT
condition|)
...
literal|8
return|;
comment|/* Intel docs say only LD, ST, IALU, ILOG, ISHF consumers have latency 4,      but HP engineers say any non-MM operation.  */
if|if
condition|(
operator|(
name|dep_class
...
expr_stmt|;
block|}
comment|/* Work around the pipeline flush that will occurr if the results of      an MM instruction are accessed before the result is ready.  Intel      documentation says this only happens with IALU, ISHF, ILOG, LD,      and ST consumers, but experimental evidence shows that *any* non-MM      type instruction will incurr the flush.  */
if|if
condition|(
name|reorder_type
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Ignore .pred.rel.mutex.  	     ??? Update this to ignore cycle display notes too 	     ??? once those are implemented  */
if|if
condition|(
name|insn_code
operator|==
...
condition|)
return|return;
comment|/* ??? Sdata became thread or thread becaome not thread.  Lose.  */
name|abort
argument_list|()
expr_stmt|;
block|}
...
expr_stmt|;
block|}
comment|/* This decl is marked as being in small data/bss but it shouldn't be;      one likely explanation for this is that the decl has been moved into      a different section from the one it was in when encode_section_info      was first called.  Remove the encoding.  */
elseif|else
if|if
condition|(
name|symbol_str
...
begin_comment
comment|/* Output the assembler code for a thunk function.  THUNK_DECL is the    declaration for the thunk function itself, FUNCTION is the decl for    the target function.  DELTA is an immediate constant offset to be    added to THIS.  If VCALL_OFFSET is non-zero, the word at    *(*this + vcall_offset) should be added to THIS.  */
end_comment
