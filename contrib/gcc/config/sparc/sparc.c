...
directive|endif
comment|/* At the moment we don't allow different pointer size and architecture */
if|if
condition|(
operator|!
name|TARGET_64BIT
...
name|error
argument_list|(
literal|"-mptr%d not allowed on -m%d"
argument_list|,
name|TARGET_PTR64
condition|?
literal|64
...
begin_comment
comment|/* Return 1 if OP is const0_rtx, used for TARGET_LIVE_G0 insns.  */
end_comment
...
comment|/* Getting this right wrt. reloading is really tricky. 	 We _MUST_ have a seperate temporary at this point, 	 if we don't barf immediately instead of generating 	 incorrect code.  */
if|if
condition|(
name|temp1
operator|==
...
block|{
comment|/* Getting this right wrt. reloading is really tricky. 	     We _MUST_ have a seperate temporary at this point, 	     so we barf immediately instead of generating 	     incorrect code.  */
if|if
condition|(
name|temp1
operator|==
...
parameter_list|)
define|\
value|gen_rtx_CONST_DOUBLE (VOIDmode, const0_rtx, \ 			      (__x)& 0xfffffc00, 0)
end_define
...
parameter_list|)
define|\
value|gen_rtx_CONST_DOUBLE (VOIDmode, const0_rtx, \ 			      (__x)& 0xffffffff, \ 			      ((__x)& 0x80000000 \ 			       ? 0xffffffff : 0))
end_define
...
argument_list|)
expr_stmt|;
comment|/* Insert the code in each nonlocal goto receiver.      If you make changes here or to the nonlocal_goto_receiver      pattern, make sure the unspec_volatile numbers still      match.  */
for|for
control|(
name|insn
operator|=
...
condition|)
block|{
comment|/* Check if the compiler has recorded some information 	     about the alignment of the base REG.  If reload has 	     completed, we already matched with proper alignments.  */
if|if
condition|(
operator|(
operator|(
...
condition|)
continue|continue;
comment|/* If it is not FMOV, FABS, FNEG, FDIV, or FSQRT then 	     we will get a stall.  Loads and stores are independant 	     of these rules.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
specifier|static
name|int
name|ultra_reorder_called_this_block
decl_stmt|;
end_decl_stmt
...
name|this_insn
decl_stmt|;
comment|/* We get called once unnecessarily per block of insns      scheduled.  */
if|if
condition|(
name|ultra_reorder_called_this_block
operator|==
...
name|file
argument_list|,
literal|"\tcall\t%s__bb_init_trace_func\n"
argument_list|,
