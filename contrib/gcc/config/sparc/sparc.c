...
specifier|static
name|void
name|sparc_encode_section_info
name|PARAMS
argument_list|(
operator|(
name|tree
...
directive|define
name|TARGET_ENCODE_SECTION_INFO
value|sparc_encode_section_info
end_define
...
name|intoffset
decl_stmt|;
comment|/* offset of the pending integer field.  */
name|unsigned
name|int
name|nregs
decl_stmt|;
...
expr_stmt|;
block|}
comment|/* SPARC v9 ABI states that structures up to 8 bytes in size are 	     given one 8 byte slot.  */
elseif|else
if|if
condition|(
name|size
...
begin_comment
comment|/* Return the string to output a conditional branch to LABEL, which is    the operand number of the label.  OP is the conditional expression.    XEXP (OP, 0) is assumed to be a condition code register (integer or    floating point) and its mode specifies what kind of comparison we made.     REVERSED is nonzero if we should reverse the sense of the comparison.     ANNUL is nonzero if we should generate an annulling branch.     NOOP is nonzero if we have to follow this branch by a noop.     INSN, if set, is the insn.  */
end_comment
...
begin_comment
comment|/* Return the string to output a conditional branch to LABEL, testing    register REG.  LABEL is the operand number of the label; REG is the    operand number of the reg.  OP is the conditional expression.  The mode    of REG says what kind of comparison we made.     REVERSED is nonzero if we should reverse the sense of the comparison.     ANNUL is nonzero if we should generate an annulling branch.     NOOP is nonzero if we have to follow this branch by a noop.  */
end_comment
...
parameter_list|)
define|\
value|((regs_ever_live[regno]&& !call_used_regs[regno])			\   || (regno == HARD_FRAME_POINTER_REGNUM&& frame_pointer_needed)	\   || (regno == RETURN_ADDR_REGNUM&& regs_ever_live[RETURN_ADDR_REGNUM]))
end_define
