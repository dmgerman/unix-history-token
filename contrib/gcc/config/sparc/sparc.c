...
begin_comment
comment|/* Coordinate with the md file wrt special insns created by    sparc_nonflat_function_epilogue.  */
end_comment
...
specifier|static
name|void
name|sparc_aout_select_section
argument_list|(
name|tree
argument_list|,
name|int
...
specifier|static
name|void
name|sparc_aout_select_rtx_section
argument_list|(
expr|enum
name|machine_mode
argument_list|,
...
specifier|static
name|int
name|sparc_use_dfa_pipeline_interface
parameter_list|(
name|void
parameter_list|)
function_decl|;
...
directive|define
name|TARGET_SCHED_USE_DFA_PIPELINE_INTERFACE
value|sparc_use_dfa_pipeline_interface
end_define
...
begin_comment
comment|/* Return nonzero only if OP is a register of mode MODE,    or const0_rtx.  */
end_comment
...
begin_comment
comment|/* Return nonzero only if OP is const1_rtx.  */
end_comment
...
begin_comment
comment|/* Nonzero if OP is a register operand in floating point register.  */
end_comment
...
begin_comment
comment|/* Nonzero if OP is a floating point condition code fcc0 register.  */
end_comment

begin_function
name|int
name|fcc0_reg_operand
parameter_list|(
name|rtx
name|op
parameter_list|,
...
begin_comment
comment|/* Return 1 if this is a 64-bit comparison operator.  This allows the use of    MATCH_OPERATOR to recognize all the branch insns.  */
end_comment

begin_function
name|int
name|noov_compare64_op
parameter_list|(
specifier|register
name|rtx
name|op
...
name|CCX_NOOVmode
condition|)
comment|/* These are the only branches which work with CCX_NOOVmode.  */
return|return
operator|(
name|code
operator|==
...
literal|0
return|;
comment|/* Accept CONSTANT_P_RTX, since it will be gone by CSE1 and result in 0/1.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Return 1 if OP is valid for the lhs of a compare insn.  */
end_comment
...
name|mode
argument_list|)
comment|/* This matches cmp_zero_extract.  */
operator|&&
operator|(
operator|(
name|mode
...
operator|)
operator|)
comment|/* This matches cmp_zero_extract_sp64.  */
operator|||
operator|(
name|mode
operator|==
...
begin_comment
comment|/* We know it can't be done in one insn when we get here,    the movsi expander guarantees this.  */
end_comment
...
name|__x
parameter_list|)
value|GEN_INT ((__x)& ~(HOST_WIDE_INT)0x3ff)
end_define
...
parameter_list|)
define|\
value|immed_double_const ((__x)& ~(HOST_WIDE_INT)0x3ff, 0, DImode)
end_define
...
parameter_list|)
define|\
value|immed_double_const ((__x)& 0xffffffff, \ 			    ((__x)& 0x80000000 ? -1 : 0), DImode)
end_define
...
begin_comment
comment|/* Build big number NUM in register REG and output the result to FILE.    REG is guaranteed to be the only clobbered register.  The function    will very likely emit several instructions, so it must not be called    from within a delay slot.  */
end_comment
...
condition|)
block|{
comment|/* We don't use the 'set' macro because it appears to be broken 	 in the Solaris 7 assembler.  */
name|fprintf
argument_list|(
name|file
argument_list|,
...
comment|/* TARGET_ARCH64 */
block|{
comment|/* We don't use the 'setx' macro because if requires a scratch register.          This is the translation of sparc_emit_set_const64_longway into asm.          Hopefully we will soon have prologue/epilogue emitted as RTL.  */
name|HOST_WIDE_INT
name|low1
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|/* If at least one field must be passed on the stack, generate      (parallel [(expr_list (nil) ...) ...]) so that all fields will      also be passed on the stack.  We can't do much better because the      semantics of FUNCTION_ARG_PARTIAL_NREGS doesn't handle the case      of structures for which the fields passed exclusively in registers      are not at the beginning of the structure.  */
if|if
condition|(
name|parms
operator|.
...
literal|1
expr_stmt|;
comment|/* ??? The mode needs 3 slots?  */
if|if
condition|(
name|slotno
operator|==
...
name|ARRAY_TYPE
operator|)
comment|/* Consider complex values as aggregates, so care 		 for CTImode and TCmode.  */
operator|||
name|GET_MODE_SIZE
argument_list|(
name|mode
...
condition|)
block|{
comment|/* SPARC-V9 ABI states that structures up to 16 bytes in size 	     are given whole slots as needed.  */
if|if
condition|(
name|size
operator|==
...
name|p
decl_stmt|;
comment|/* TurboSPARC is reported to have problems with      with 	foo: b,a foo      i.e. an empty loop with the annul bit set.  The workaround is to use          foo: b foo; nop      instead.  */
if|if
condition|(
operator|!
name|TARGET_V9
...
define|#
directive|define
name|HARD_FRAME_POINTER_MASK
value|(1<< (HARD_FRAME_POINTER_REGNUM))
end_define
...
specifier|static
name|bool
name|sparc_flat_must_save_register_p
parameter_list|(
name|int
name|regno
parameter_list|)
block|{
comment|/* General case: call-saved registers live at some point.  */
if|if
condition|(
operator|!
name|call_used_regs
...
name|true
return|;
comment|/* Frame pointer register (%i7) if needed.  */
if|if
condition|(
name|regno
operator|==
...
name|true
return|;
comment|/* PIC register (%l7) if needed.  */
if|if
condition|(
name|regno
operator|==
...
name|true
return|;
comment|/* Return address register (%o7) if needed.  */
if|if
condition|(
name|regno
operator|==
...
name|RETURN_ADDR_REGNUM
index|]
comment|/* When the PIC offset table is used, the PIC register 	     is set by using a bare call that clobbers %o7.  */
operator|||
operator|(
name|flag_pic
operator|&&
...
name|HOST_WIDE_INT_PRINT_DEC
literal|", "
literal|"regs= %d/%d, args= %d, extra= %d\n"
argument_list|,
name|ASM_COMMENT_START
argument_list|,
name|current_frame_info
...
name|HOST_WIDE_INT_PRINT_DEC
literal|", %s"
literal|"\t%s# set up frame pointer\n"
argument_list|,
name|sp_str
argument_list|,
name|size
...
literal|16
decl_stmt|;
comment|/* Amount to decrement %sp by, the second time.  */
name|HOST_WIDE_INT
name|size2
init|=
name|size
...
name|size1
decl_stmt|;
comment|/* Offset to register save area from %sp after first decrement.  */
name|int
name|offset
init|=
call|(
...
begin_comment
comment|/* ??? Similar to the standard section selection, but force reloc-y-ness    if SUNOS4_SHARED_LIBRARIES.  Unclear why this helps (as opposed to    pretending PIC always on), but that's what the old code did.  */
end_comment
...
block|}
block|}
comment|/* ??? Maybe mark integer compares as zero cost on 	 ??? all UltraSPARC processors because the result 	 ??? can be bypassed to a branch in the same group.  */
operator|*
name|total
operator|=
name|COSTS_N_INSNS
...
block|}
block|}
comment|/* The latency is actually variable for Ultra-I/II 	 And if one of the inputs have a known constant 	 value, we could calculate this precisely.  	 However, for that to be useful we would need to 	 add some machine description changes which would 	 make sure small constants ended up in rs1 of the 	 multiply instruction.  This is because the multiply 	 latency is determined by the number of clear (or 	 set if the value is negative) bits starting from 	 the most significant bit of the first input.  	 The algorithm for computing num_cycles of a multiply 	 on Ultra-I/II is:  	 	if (rs1< 0) 			highest_bit = highest_clear_bit(rs1); 		else 			highest_bit = highest_set_bit(rs1); 		if (num_bits< 3) 			highest_bit = 3; 		num_cycles = 4 + ((highest_bit - 3) / 2);  	 If we did that we would have to also consider register 	 allocation issues that would result from forcing such 	 a value into a register.  	 There are other similar tricks we could play if we 	 knew, for example, that one input was an array index.  	 Since we do not play any such tricks currently the 	 safest thing to do is report the worst case latency.  */
if|if
condition|(
name|sparc_cpu
operator|==
...
return|;
block|}
comment|/* Multiply latency on Ultra-III, fortunately, is constant.  */
if|if
condition|(
name|sparc_cpu
operator|==
...
name|MEM
case|:
comment|/* If outer-code is SIGN/ZERO extension we have to subtract 	 out COSTS_N_INSNS (1) from whatever we return in determining 	 the cost.  */
switch|switch
condition|(
name|sparc_cpu
condition|)
...
block|{
comment|/* This handles sign extension (3 cycles) 		 and everything else (2 cycles).  */
operator|*
name|total
operator|=
name|COSTS_N_INSNS
