...
directive|define
name|ASM_CPU_SPEC
value|"\ %{mcpu=ultrasparc:" DEF_ARCH32_SPEC("-xarch=v8plusa") DEF_ARCH64_SPEC("-xarch=v9a") "} \ %{mcpu=v9:" DEF_ARCH32_SPEC("-xarch=v8plus") DEF_ARCH64_SPEC("-xarch=v9") "} \ %{!mcpu=ultrasparc:%{!mcpu=v9:%{mcpu*:" DEF_ARCH32_SPEC("-xarch=v8") DEF_ARCH64_SPEC("-xarch=v9") "}}} \ %{!mcpu*:%(asm_cpu_default)} \ "
end_define
...
directive|define
name|CPP_ARCH64_SPEC
value|"-D__SIZE_TYPE__=long\\ unsigned\\ int -D__PTRDIFF_TYPE__=long\\ int \ -D__arch64__ -Acpu(sparc64) -Amachine(sparcv9) -D__sparcv9"
end_define
...
directive|define
name|CC1_SPEC
value|"\ %{sun4:} %{target:} \ %{mcypress:-mcpu=cypress} \ %{msparclite:-mcpu=sparclite} %{mf930:-mcpu=f930} %{mf934:-mcpu=f934} \ %{mv8:-mcpu=v8} %{msupersparc:-mcpu=supersparc} \ %{m64:-mptr64 -mcpu=v9 -mstack-bias -mno-v8plus} \ "
end_define
...
directive|define
name|CC1_SPEC
value|"\ %{sun4:} %{target:} \ %{mcypress:-mcpu=cypress} \ %{msparclite:-mcpu=sparclite} %{mf930:-mcpu=f930} %{mf934:-mcpu=f934} \ %{mv8:-mcpu=v8} %{msupersparc:-mcpu=supersparc} \ %{m32:-mptr32 -mcpu=cypress -mno-stack-bias} \ %{mv8plus:-m32 -mptr32 -mcpu=cypress -mno-stack-bias} \ "
end_define
...
value|);			\   if (prot != 7)							\     need_enable_exec_stack = 1;						\ }									\ 									\ void									\ __enable_execute_stack (addr)						\      void *addr;							\ {									\   if (!need_enable_exec_stack)						\     return;								\   else {								\     long size = getpagesize ();						\     long mask = ~(size-1);						\     char *page = (char *) (((long) addr)& mask); 			\     char *end  = (char *) ((((long) (addr + TRAMPOLINE_SIZE))& mask) + size); \ 									\
...
parameter_list|)
define|\
value|do {									\     ctors_section ();							\     fprintf (FILE, "\t%s\t ", TARGET_ARCH64 ? ASM_LONGLONG : INT_ASM_OP); \     assemble_name (FILE, NAME);						\     fprintf (FILE, "\n");						\   } while (0)
end_define
...
parameter_list|)
define|\
value|do {									\     dtors_section ();                   				\     fprintf (FILE, "\t%s\t ", TARGET_ARCH64 ? ASM_LONGLONG : INT_ASM_OP); \     assemble_name (FILE, NAME);              				\     fprintf (FILE, "\n");						\   } while (0)
end_define
