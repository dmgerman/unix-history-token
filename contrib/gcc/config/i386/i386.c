...
argument_list|)
decl_stmt|;
comment|/* Set the default values for switches whose default depends on TARGET_64BIT      in case they weren't overwriten by command line options.  */
if|if
condition|(
name|TARGET_64BIT
condition|)
...
name|size
expr_stmt|;
comment|/* Add outgoing arguments area.  Can be skipped if we eliminated      all the function calls as dead code.  */
if|if
condition|(
...
begin_comment
comment|/* Meaning of CODE:    L,W,B,Q,S,T -- print the opcode suffix for specified size of operand.    C -- print opcode suffix for set/cmov insn.    c -- like C, but print reversed condition    F,f -- likewise, but for floating-point.    O -- if CMOV_SUN_AS_SYNTAX, expand to "w.", "l." or "q.", otherwise         nothing    R -- print the prefix for register names.    z -- print the opcode suffix for the size of the current operand.    * -- print a star (in certain assembler syntax)    A -- print an absolute memory reference.    w -- print the operand as if it's a "word" (HImode) even if it isn't.    s -- print a shift double count, followed by the assemblers argument 	delimiter.    b -- print the QImode name of the register for the indicated operand. 	%b0 would print %al if operands[0] is reg 0.    w --  likewise, print the HImode name of the register.    k --  likewise, print the SImode name of the register.    q --  likewise, print the DImode name of the register.    h -- print the QImode name for a "high" register, either ah, bh, ch or dh.    y -- print "st(0)" instead of "st" as a register.    D -- print condition for SSE cmp instruction.    P -- if PIC, print an @PLT suffix.    X -- don't print any sort of PIC '@' suffix for a symbol.  */
end_comment
...
operator|-
literal|1
comment|/* For x86-64, the immediate field in the instruction is 32-bit 	 signed, so we can't increment a DImode value above 0x7fffffff.  */
operator|&&
operator|(
operator|!
name|TARGET_64BIT
...
name|output_asm_insn
argument_list|(
literal|"add{l}\t{%2+[.-%P1], %0|%0, OFFSET FLAT: %2+[.-%P1]}"
argument_list|,
name|xops
argument_list|)
expr_stmt|;
...
name|output_asm_insn
argument_list|(
literal|"mov{l}\t{%0@GOT(%%ebx), %%ecx|%%ecx, %0@GOT[%%ebx]}"
argument_list|,
name|xops
argument_list|)
expr_stmt|;
