...
directive|define
name|TARGET_VERSION
value|fprintf (stderr, " (80386, VxWorks BSD syntax)");
end_define
...
parameter_list|()
define|\
value|do							\     {							\       builtin_define ("__vxworks");			\       builtin_assert ("system=unix");			\ 							\       if (TARGET_386)					\ 	builtin_define ("CPU=I80386");			\       else if (TARGET_486)				\ 	builtin_define ("CPU=I80486");			\       else if (TARGET_PENTIUM)				\ 	{						\ 	  builtin_define ("CPU=PENTIUM");		\ 	  builtin_define ("CPU_VARIANT=PENTIUM");	\ 	}						\       else if (TARGET_PENTIUMPRO)			\ 	{						\ 	  builtin_define ("CPU=PENTIUM");		\ 	  builtin_define ("CPU_VARIANT=PENTIUMPRO");	\ 	}						\     }							\   while (0)
end_define
