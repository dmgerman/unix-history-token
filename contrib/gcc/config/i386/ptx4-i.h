...
begin_comment
comment|/* Rest of definitions (non architecture dependent) */
end_comment
...
name|CPP_PREDEFINES
define|\
value|"-Di386 -Dunix -D_SEQUENT_ -Asystem(unix) -Asystem(ptx4) -Acpu(i386) -Amachine(i386)"
end_define
...
parameter_list|)
define|\
value|do { long value;							\      REAL_VALUE_TO_TARGET_SINGLE ((VALUE), value);			\      if (sizeof (int) == sizeof (long))					\          fprintf((FILE), "%s\t0x%x\n", ASM_LONG, value);		\      else								\          fprintf((FILE), "%s\t0x%lx\n", ASM_LONG, value);		\    } while (0)
end_define
