...
begin_comment
comment|/* Nonzero if we have FPRs.  */
end_comment
...
begin_comment
comment|/* Private copy of original value of flag_pic for ABI_AIX.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|rs6000_flag_pic
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Call distance, overridden by -mlongcall and #pragma longcall(1).    The only place that looks at this is rs6000_set_default_type_attributes;    everywhere else should rely on the presence or absence of a longcall    attribute on the function declaration.  */
end_comment
...
name|char
modifier|*
name|rs6000_elf_strip_name_encoding
name|PARAMS
argument_list|(
operator|(
specifier|const
...
specifier|static
name|void
name|rs6000_xcoff_encode_section_info
name|PARAMS
argument_list|(
operator|(
name|tree
...
specifier|static
name|void
name|rs6000_parse_vrsave_option
name|PARAMS
argument_list|(
operator|(
name|void
...
begin_comment
comment|/* ??? Should work everywhere, but ask dje@watson.ibm.com before    enabling for AIX.  */
end_comment
...
expr_stmt|;
block|}
comment|/* For Darwin, always silently make -fpic and -fPIC identical.  */
if|if
condition|(
name|flag_pic
operator|==
...
argument_list|()
expr_stmt|;
comment|/* Handle -mvrsave= option.  */
name|rs6000_parse_vrsave_option
argument_list|()
expr_stmt|;
comment|/* Handle -misel= option.  */
name|rs6000_parse_isel_option
argument_list|()
expr_stmt|;
ifdef|#
...
name|error
argument_list|(
literal|"unknown -misel= option specified: '%s'"
argument_list|,
name|rs6000_isel_string
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Handle -mvrsave= options.  */
end_comment
...
parameter_list|()
block|{
comment|/* Generate VRSAVE instructions by default.  */
if|if
condition|(
name|rs6000_altivec_vrsave_string
operator|==
...
name|error
argument_list|(
literal|"unknown -mvrsave= option specified: '%s'"
argument_list|,
name|rs6000_altivec_vrsave_string
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Long long and SPE vectors are put in odd registers.  */
if|if
condition|(
name|n_words
operator|==
...
literal|1
expr_stmt|;
comment|/* Long long and SPE vectors are not split between registers 	     and stack.  */
if|if
condition|(
name|gregno
operator|+
...
condition|)
block|{
comment|/* SPE vectors in ... get split into 2 registers.  */
if|if
condition|(
name|TARGET_SPE
operator|&&
...
begin_comment
comment|/* A C expression that indicates when an argument must be passed by    reference.  If nonzero for an argument, a copy of that argument is    made in memory and a pointer to the argument is passed instead of    the argument itself.  The pointer is passed in whatever way is    appropriate for passing a pointer to that type.     Under V.4, structures and unions are passed by reference.     As an extension to all ABIs, variable sized types are passed by    reference.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/*  AltiVec vectors never go in registers.  */
if|if
condition|(
operator|!
name|TARGET_ALTIVEC
...
name|align
decl_stmt|;
comment|/* AltiVec vectors are 16 byte aligned.  */
if|if
condition|(
name|TARGET_ALTIVEC
operator|&&
...
begin_comment
comment|/* Search through a set of builtins and enable the mask bits.    DESC is an array of builtins.    SIZE is the totaly number of builtins.    START is the builtin enum at which to start.    END is the builtin enum at which to end.  */
end_comment
...
decl_stmt|;
name|tree
name|void_ftype_pcvoid_int_char
init|=
name|build_function_type_list
argument_list|(
name|void_type_node
...
argument_list|()
expr_stmt|;
comment|/* These should never be generated except for       flag_unsafe_math_optimizations and flag_finite_math_only.  */
if|if
condition|(
name|mode
operator|==
...
condition|)
block|{
comment|/* Synthesize GE/LE frome GT/LT || EQ.  */
name|emit_insn
argument_list|(
name|cmp
argument_list|)
...
block|{
comment|/* We only care about 1 bit (x1xx), so map everything to NE to 	     maintain rtl sanity.  We'll get to the right bit (x1xx) at 	     code output time.  */
if|if
condition|(
name|code
operator|==
...
name|LTGT
condition|)
comment|/* Do the inverse here because we have no cmpne 	       instruction.  We use the cmpeq instruction and expect 	       to get a 0 instead.  */
name|code
operator|=
name|EQ
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* We're going to try to implement comparions by performing      a subtract, then comparing against zero.  Unfortunately,      Inf - Inf is NaN which is not zero, and so if we don't      know that the operand is finite and the comparison      would treat EQ different to UNORDERED, we can't do it.  */
if|if
condition|(
operator|!
name|flag_unsafe_math_optimizations
...
name|insn
decl_stmt|;
comment|/* Irritatingly, there are two kinds of thunks -- those created with      TARGET_ASM_OUTPUT_MI_THUNK and those with DECL_THUNK_P that go      through the regular part of the compiler.  This is a very hacky      way to tell them apart.  */
if|if
condition|(
...
begin_comment
comment|/* Note that this is also used for PPC64 Linux.  */
end_comment
...
begin_comment
comment|/* Cross-module name binding.  For AIX and PPC64 Linux, which always are    PIC, use private copy of flag_pic.  */
end_comment
