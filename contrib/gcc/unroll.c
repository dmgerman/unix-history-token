...
directive|ifdef
name|HAIFA
comment|/* Fix the initial value for the loop as needed.  */
if|if
condition|(
name|loop_n_iterations
operator|<=
...
comment|/* Keep track of the unroll factor for each loop.  */
if|if
condition|(
name|unroll_type
operator|==
...
return|return;
block|}
comment|/* Reject iteration variables larger than the host wide int size, since they      could result in a number of iterations greater than the range of our      `unsigned HOST_WIDE_INT' variable loop_n_iterations.  */
elseif|else
if|if
condition|(
operator|(
