...
begin_comment
comment|/* Try to unroll loops, and split induction variables.    Copyright (C) 1992, 1993, 1994, 1995 Free Software Foundation, Inc.    Contributed by James E. Wilson, Cygnus Support/UC Berkeley.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* We must limit it to max_reg_before_loop, because only these pseudo      registers have valid regno_first_uid info.  Any register created after      that is unlikely to be local to the loop anyways.  */
name|local_regno
operator|=
operator|(
name|char
...
argument_list|()
expr_stmt|;
comment|/* Check for the case where the initial value is greater than or equal 	     to the final value.  In that case, we want to execute exactly 	     one loop iteration.  The code below will fail for this case.  */
name|emit_cmp_insn
argument_list|(
name|initial_value
argument_list|,
...
condition|)
block|{
comment|/* The rs6000 port loads some constants with IOR. 	     The alpha port loads some constants with ASHIFT.  */
name|rtx
name|second_part
init|=
name|XEXP
...
argument_list|)
decl_stmt|;
comment|/* First check to see if either address would fail.  */
if|if
condition|(
operator|!
...
literal|0
expr_stmt|;
comment|/* Now put things back the way they were before.  This will always    succeed.  */
name|validate_change
argument_list|(
name|v
operator|->
...
comment|/* As a special case, if we have multiple identical address givs 		 within a single instruction, then we do use a single pseudo 		 reg for both.  This is necessary in case one is a match_dup 		 of the other.  */
name|v
operator|->
name|const_adjust
operator|=
...
literal|0
expr_stmt|;
comment|/* We used to use pren_nonnote_insn here, but that fails because it might      accidentally get the branch for a contained loop if the branch for this      loop was deleted.  We can only trust branches immediately before the      loop_end.  */
name|last_loop_insn
operator|=
name|PREV_INSN
argument_list|(
