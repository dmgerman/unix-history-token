...
begin_comment
comment|/* Try to unroll loops, and split induction variables.    Copyright (C) 1992, 1993, 1994, 1995, 1997, 1998, 1999, 2000, 2001,    2002, 2003    Free Software Foundation, Inc.    Contributed by James E. Wilson, Cygnus Support/UC Berkeley.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* ??? Improve control of which loops get unrolled.  Could use profiling    info to only unroll the most commonly executed loops.  Perhaps have    a user specifiable option to control the amount of code expansion,    or the percent of loops to consider for unrolling.  Etc.  */
end_comment
...
specifier|static
name|rtx
name|simplify_cmp_and_jump_insns
parameter_list|(
name|enum
name|rtx_code
parameter_list|,
...
comment|/* FIXME this actually checks for a jump to the continue point, which 	 is not the same as the condition in a for loop.  As a result, this 	 optimization fails for most for loops.  We should really use flow 	 information rather than instruction pattern matching.  */
name|rtx
name|ujump
init|=
name|ujump_to_loop_cont
...
argument_list|()
expr_stmt|;
comment|/* We must copy the final and initial values here to avoid 	     improperly shared rtl.  */
name|final_value
operator|=
name|copy_rtx
argument_list|(
...
name|loop_preconditioned
expr_stmt|;
comment|/* Remember whether it was preconditioned for the second loop pass.  */
...
begin_comment
comment|/* A helper function for unroll_loop.  Emit a compare and branch to    satisfy (CMP OP1 OP2), but pass this through the simplifier first.    If the branch turned out to be conditional, return it, otherwise    return NULL.  */
end_comment
...
condition|)
block|{
comment|/* The rs6000 port loads some constants with IOR. 	     The alpha port loads some constants with ASHIFT and PLUS. 	     The sparc64 port loads some constants with LSHIFTRT.  */
name|rtx
name|second_part
init|=
name|XEXP
...
argument_list|)
expr_stmt|;
comment|/* If there is a REG_EQUAL note present whose value 	     is not loop invariant, then delete it, since it 	     may cause problems with later optimization passes.  */
if|if
condition|(
operator|(
name|tem
...
control|)
block|{
comment|/* VTOP notes are valid only before the loop exit test. 	     If placed anywhere else, loop may generate bad code. 	     Although COPY_NOTES_FROM will be at most one or two (for cc0) 	     instructions before the last insn in the loop, COPY_NOTES_FROM 	     can be a NOTE_INSN_LOOP_CONT note if there is no VTOP note, 	     as in a do .. while loop.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
comment|/* loop_number_exit_count is nonzero if the loop has an exit other than 	 a fall through at the end.  */
name|biv_splittable
operator|=
literal|1
expr_stmt|;
...
literal|0
expr_stmt|;
comment|/* If final value is nonzero, then must emit an instruction which sets 	 the value of the biv to the proper value.  This is done after 	 handling all of the givs, since some of them may need to use the 	 biv's value in their initialization code.  */
...
comment|/* If the final value is nonzero, and the giv has not been reduced, 	 then must emit an instruction to set the final value.  */
...
return|;
block|}
comment|/* Try swapping the comparison to identify a suitable iv.  */
if|if
condition|(
name|REG_IV_TYPE
argument_list|(
...
name|loop_dump_stream
argument_list|,
literal|"Loop iterations: Basic induction var add_val is REG %d.\n"
argument_list|,
name|REGNO
argument_list|(
name|biv_inc
...
condition|)
block|{
comment|/* The iterator value once through the loop is equal to the 	     comparison value.  Either we have an infinite loop, or 	     we'll loop twice.  */
if|if
condition|(
name|increment
operator|==
