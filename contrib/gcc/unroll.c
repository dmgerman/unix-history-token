...
begin_comment
comment|/* Indexed by register number, if this is a splittable induction variable,    this indicates if it was made from a derived giv.  */
end_comment
...
operator|--
expr_stmt|;
comment|/* If we have a target that uses cc0, then we also must not duplicate        the insn that sets cc0 before the jump insn.  */
ifdef|#
directive|ifdef
name|HAVE_cc0
if|if
...
block|}
block|}
comment|/* Givs that have been created from multiple biv increments always have        local registers.  */
for|for
control|(
name|j
operator|=
...
return|;
block|}
comment|/* Note that iteration_info biases the initial value for GIV iterators      such as "while (i--> 0)" so that we can calculate the number of      iterations just like for BIV iterators.       Also note that the absolute values of initial_value and      final_value are unimportant as only their difference is used for      calculating the number of loop iterations.  */
operator|*
name|initial_value
operator|=
name|loop_info
...
condition|)
block|{
comment|/* ??? This relies on SET_SRC (SET) to be of 		     the form (plus (reg) (const_int)), and thus 		     forces recombine_givs to restrict the kind 		     of giv derivations it does before unrolling.  */
name|giv_src_reg
operator|=
name|XEXP
argument_list|(
...
condition|)
block|{
comment|/* Don't let delete_insn delete the label referenced here, 		     because we might possibly need it later for some other 		     instruction in the loop.  */
if|if
condition|(
name|JUMP_LABEL
argument_list|(
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Iteration var rejected because mode too large.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
...
condition|)
block|{
comment|/* When reg_iv_type / reg_iv_info is resized for biv increments 	 that are turned into givs, reg_biv_class is not resized. 	 So check here that we don't make an out-of-bounds access.  */
if|if
condition|(
name|REGNO
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* The caller assumes that one full increment has occured at the 	     first loop test.  But that's not true when the biv is incremented 	     after the giv is set (which is the usual case), e.g.: 	     i = 6; do {;} while (i++< 9) . 	     Therefore, we bias the initial value by subtracting the amount of 	     the increment that occurs between the giv set and the giv test.  */
for|for
control|(
name|biv_inc
operator|=
...
name|loop_dump_stream
argument_list|,
literal|"Loop unrolling: Giv iterator, initial value bias %ld.\n"
argument_list|,
operator|(
name|long
operator|)
...
operator|)
operator|)
comment|/* Givs made from biv increments are missed by the above test, so 	     test explicitly for them.  */
operator|&&
operator|(
name|REGNO
argument_list|(
...
condition|)
block|{
comment|/* calculate_giv_inc doesn't work for derived givs. 			 copy_loop_body works around the problem for the 			 DEST_REG givs themselves, but it can't handle 			 DEST_ADDR givs that have been combined with 			 a derived DEST_REG giv. 			 So Handle V as if the giv from which V->SAME has 			 been derived has been combined with V. 			 recombine_givs only derives givs from givs that 			 are reduced the ordinary, so we need not worry 			 about same->derived_from being in turn derived.  */
name|same
operator|=
name|same
operator|->
...
name|ret
decl_stmt|;
comment|/* We can't use bl->initial_value to compute the initial 			 value, because the loop may have been preconditioned. 			 We must calculate it from NEW_REG.  Try using 			 force_operand instead of emit_iv_add_mult.  */
name|delete_insn
argument_list|(
name|PREV_INSN
argument_list|(
...
condition|)
block|{
comment|/* Handle V as if the giv from which V->SAME has 			 been derived has been combined with V.  */
name|v
operator|->
name|same
operator|=
...
name|derived_from
condition|)
comment|/* In this case, there is one set where the giv insn was and one 		set each after each biv increment.  (Most are likely dead.)  */
name|count
operator|++
expr_stmt|;
name|splittable_regs_updates
...
begin_comment
comment|/* Look back before LOOP_START for then insn that sets REG and return    the equivalent constant if there is a REG_EQUAL note otherwise just    the SET_SRC of REG.  */
end_comment
