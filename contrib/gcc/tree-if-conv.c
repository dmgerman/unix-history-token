...
argument_list|)
expr_stmt|;
comment|/* Clear aux field of incoming edges to a bb with a phi node.  */
if|if
condition|(
name|phi
condition|)
...
argument_list|)
expr_stmt|;
comment|/* Add the condition to aux field of the edge.  In case edge 	 destination is a PHI node, this condition will be ANDed with 	 block predicate to construct complete condition.  */
name|e
operator|->
name|aux
operator|=
...
begin_comment
comment|/* During if-conversion aux field from basic block structure is used to hold    predicate list. Clean each basic block's predicate list for the given LOOP.    Also clean aux field of succesor edges, used to hold true and false    condition from conditional expression.  */
end_comment
...
name|aux
expr_stmt|;
comment|/* If there is a condition on an incoming edge, 	 AND it with the incoming bb predicate.  */
if|if
condition|(
name|second_edge
operator|->
...
name|TRUTH_NOT_EXPR
condition|)
comment|/* We can be smart here and choose inverted 	   condition without switching bbs.  */
operator|*
name|cond
operator|=
name|invert_truthvalue
...
comment|/* Select non loop header bb.  */
name|first_edge
operator|=
name|second_edge
expr_stmt|;
