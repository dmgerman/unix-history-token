...
name|CPP_DL_ERROR
argument_list|,
comment|/* XXX should be DL_SORRY */
...
begin_comment
comment|/* Returns 1 if C is valid in an identifier, 2 if C is valid except at    the start of an identifier, and 0 if C is not valid in an    identifier.  We assume C has already gone through the checks of    _cpp_valid_ucn.  The algorithm is a simple binary search on the    table defined in cppucnid.h.  */
end_comment
...
condition|)
block|{
comment|/* We'll error when we try it out as the start of an identifier.  */
name|cpp_error
argument_list|(
name|pfile
argument_list|,
