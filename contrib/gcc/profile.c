...
name|bb
parameter_list|)
value|((bb) == ENTRY_BLOCK_PTR ? 0		\ 			       : ((bb) == EXIT_BLOCK_PTR		\ 				  ? last_basic_block + 1 : (bb)->index + 1))
end_define

begin_comment
comment|/* Instantiate the profile info structure.  */
end_comment
...
begin_comment
comment|/* If nonzero, we need to output a constructor to set up the    per-object-file data.  */
end_comment
...
begin_comment
comment|/* Computes hybrid profile for all matching entries in da_file.    Sets max_counter_in_program as a side effect.  */
end_comment
...
literal|0
expr_stmt|;
comment|/* No .da file, no execution counts.  */
if|if
condition|(
operator|!
name|da_file
...
expr_stmt|;
block|}
comment|/* now read and combine all matching profiles.  */
name|profile
operator|=
name|xmalloc
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* read the maximal counter.  */
...
argument_list|)
expr_stmt|;
comment|/* skip the rest of "statistics" emited by __bb_exit_func.  */
name|fseek
argument_list|(
name|da_file
argument_list|,
...
name|error
argument_list|(
literal|"Profile does not match flowgraph of function %s (out of date?)"
argument_list|,
...
name|rtl_dump_file
argument_list|,
literal|"Merged %i profiles with maximal count %i.\n"
argument_list|,
name|profile_info
operator|.
name|count_profiles_merged
...
begin_comment
comment|/* Compute checksum for the current function.  */
end_comment
