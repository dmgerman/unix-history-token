...
parameter_list|)
define|\
value|(sizeof (struct rtx_def) + ((NSLOTS) - 1) * sizeof (rtunion))
end_define
...
argument_list|)
block|,
comment|/* REG, MEM, PLUS, etc.  */
name|RTL_SIZE
argument_list|(
literal|10
argument_list|)
block|,
comment|/* INSN, CALL_INSN, JUMP_INSN */
block|}
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* For the 3.3 release, we will avoid prefetch, as it isn't tested widely.  */
end_comment
...
index|]
expr_stmt|;
comment|/* Peel back indicies in depth that index into by_depth, so that 	 as new elements are added to by_depth, we note the indicies 	 of those elements, if they are for new context depths.  */
while|while
condition|(
name|G
operator|.
...
index|]
decl_stmt|;
comment|/* If they are at the same depth, put top element into freed 	 slot.  */
if|if
condition|(
name|entry
operator|->
...
begin_comment
comment|/* Allocate a chunk of memory of SIZE bytes.  If ZERO is nonzero, the    memory is zeroed; otherwise, its contents are undefined.  */
end_comment
...
name|e
decl_stmt|;
comment|/* There can be only one object per "page" in a bucket for sizes      larger than half a machine page; it will always have offset zero.  */
if|if
condition|(
name|OBJECT_SIZE
argument_list|(
...
literal|1
expr_stmt|;
comment|/* The G.depth array is shortend so that the last index is the      context_depth of the top element of by_depth.  */
if|if
condition|(
name|depth
operator|+
...
name|by_depth_in_use
expr_stmt|;
comment|/* We might not have any PTEs of depth depth.  */
if|if
condition|(
name|depth
operator|<
...
condition|)
block|{
comment|/* First we go through all the pages at depth depth to 	 recalculate the in use bits.  */
for|for
control|(
name|i
operator|=
...
index|]
expr_stmt|;
comment|/* Check that all of the pages really are at the depth that 	     we expect.  */
if|if
condition|(
name|p
operator|->
...
block|}
block|}
comment|/* Then, we reset all page_entries with a depth greater than depth      to be at depth.  */
for|for
control|(
name|i
operator|=
...
index|]
decl_stmt|;
comment|/* Check that all of the pages really are at the depth we 	 expect.  */
ifdef|#
directive|ifdef
name|ENABLE_CHECKING
if|if
