...
begin_comment
comment|/* RTL simplification functions for GNU compiler.    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Nonzero if X has the form (PLUS frame-pointer integer).  We check for    virtual regs here because the simplify_*_operation routines are called    by integrate.c, which is called before virtual register instantiation.     ?!? NONZERO_BASE_PLUS_P needs to move into    a header file so that their definitions can be shared with the    simplification routines in simplify-rtx.c.  Until then, do not    change this macro without also changing the copy in simplify-rtx.c.  */
end_comment

begin_comment
comment|/* Allows reference to the stack pointer.     This used to include FIXED_BASE_PLUS_P, however, we can't assume that    arg_pointer_rtx by itself is nonzero, because on at least one machine,    the i960, the arg pointer is zero when it is unused.  */
end_comment
...
condition|)
block|{
comment|/* The following tests GET_RTX_CLASS (GET_CODE (op0)) == '<'.  */
name|enum
name|rtx_code
name|new
init|=
...
name|const0_rtx
return|;
comment|/* For modes without NaNs, if the two operands are equal, we know the      result.  Nevertheless, don't discard them if they have side-effects.  */
if|if
condition|(
operator|!
name|HONOR_NANS
...
name|op
return|;
comment|/* Simplify subregs of vector constants.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* ?? We probably don't need this copy_rtx because constants 	     can be shared.  ?? */
return|return
name|copy_rtx
argument_list|(
name|elt
...
condition|)
block|{
comment|/* This happens when the target register size is smaller then 	     the vector mode, and we synthesize operations with vectors 	     of elements that are smaller than the register size.  */
name|HOST_WIDE_INT
name|sum
init|=
literal|0
...
name|MODE_INT
condition|)
comment|/* This shouldn't happen, but let's not do anything stupid.  */
return|return
name|NULL_RTX
return|;
block|}
...
condition|)
block|{
comment|/* Construct a CONST_VECTOR from individual subregs.  */
name|enum
name|machine_mode
name|submode
init|=
...
control|)
block|{
comment|/* This might fail, e.g. if taking a subreg from a SYMBOL_REF.  */
comment|/* ??? It would be nice if we could actually make such subregs 		 on targets that allow such relocations.  */
if|if
condition|(
name|byte
operator|>=
