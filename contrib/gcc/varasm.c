...
begin_comment
comment|/* Output variables, constants and external declarations, for GNU compiler.    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997,    1998, 1999, 2000, 2001, 2002, 2003 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Uniquize all constants that appear in memory.    Each constant in memory thus far output is recorded    in `const_hash_table'.  */
end_comment
...
name|next
decl_stmt|;
comment|/* The label of the constant.  */
specifier|const
name|char
modifier|*
name|label
...
name|GTY
argument_list|(
argument|(param_is (struct deferred_string))
argument_list|)
name|htab_t
name|const_str_htab
expr_stmt|;
...
begin_comment
comment|/* Returns nonzero if the value represented by X (which is really a    struct deferred_string *) is the same as that given by Y    (which is really a char *).  */
end_comment
...
begin_comment
comment|/* Return an rtx representing a reference to constant data in memory    for the constant expression EXP.     If assembler code for such a constant has already been output,    return an rtx to refer to it.    Otherwise, output such a constant in memory (or defer it for later)    and generate an rtx for it.     If DEFER is nonzero, the output of string constants can be deferred    and output only if referenced in the function after all optimizations.     The TREE_CST_RTL of EXP is set up to point to that rtx.    The const_hash_table records which constants already have label strings.  */
end_comment
...
name|rtl
decl_stmt|;
comment|/* We can't just use the saved RTL if this is a deferred string constant      and we are not to defer anymore.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
condition|)
block|{
comment|/* Take care not to invoke targetm.encode_section_info for 	 constants which don't have a TREE_CST_RTL.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Like STRIP_NOPS except allow the operand mode to widen. 	     This works around a feature of fold that simplfies 	     (int)(p1 - p2) to ((int)p1 - (int)p2) under the theory 	     that the narrower operation is cheaper.  */
while|while
condition|(
name|TREE_CODE
argument_list|(
