...
name|end
expr_stmt|;
comment|/* Deleting a block may produce unreachable code warning even when we are      not deleting anything live.  Supress it by moving all the line number      notes out of the block.  */
for|for
control|(
name|insn
operator|=
...
begin_comment
comment|/* Return nonzero if X is an register set in regset DATA.    Called via for_each_rtx.  */
end_comment
...
name|true
expr_stmt|;
comment|/* We've possibly created trivially dead code.  Cleanup it right 	 now to introduce more oppurtunities for try_optimize_cfg.  */
if|if
condition|(
operator|!
operator|(
...
condition|)
block|{
comment|/* Cleaning up CFG introduces more oppurtunities for dead code 	     removal that in turn may introduce more oppurtunities for 	     cleaning up the CFG.  */
if|if
condition|(
operator|!
