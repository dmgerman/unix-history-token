...
name|BB
parameter_list|)
value|((BB)->flags& BB_FORWARDER_BLOCK)
end_define

begin_comment
comment|/* Set to true when we are running first pass of try_optimize_cfg loop.  */
end_comment
...
name|NULL
decl_stmt|;
comment|/* If we are partitioning hot/cold basic blocks, we don't want to      mess up unconditional or indirect jumps that cross between hot      and cold sections.       Basic block partitioning may result in some jumps that appear to      be optimizable (or blocks that appear to be mergeable), but which really m      ust be left untouched (they are required to make it safely across      partition boundaries).  See the comments at the top of      bb-reorder.c:partition_hot_cold_basic_blocks for complete details.  */
if|if
condition|(
name|find_reg_note
argument_list|(
...
begin_comment
comment|/* Return true iff the condbranches at the end of BB1 and BB2 match.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* Make the sources of the pc sets unreadable so that when we call      insns_match_p it won't process them.      The death_notes_match_p from insns_match_p won't see the local registers      used for the pc set, but that could only cause missed optimizations when      there are actually condjumps that use stack registers.  */
name|SET_SRC
argument_list|(
name|set1
argument_list|)
...
name|dump_file
argument_list|,
literal|"Outcomes of branch in bb %i and %i differ too much (%i %i)\n"
argument_list|,
name|bb1
operator|->
name|index
...
condition|)
block|{
comment|/* The labels should never be the same rtx.  If they really are same 	     the jump tables are same too. So disable crossjumping of blocks BB1 	     and BB2 because when deleting the common insns in the end of BB1 	     by delete_basic_block () the jump table would be deleted too.  */
...
return|;
block|}
comment|/* The same checks as in try_crossjump_to_edge. It is required for RTL      version of sequence abstraction.  */
name|FOR_EACH_EDGE
argument_list|(
argument|e1
argument_list|,
...
begin_comment
comment|/* Returns true if BB basic block has a preserve label.  */
end_comment

begin_function
specifier|static
name|bool
name|block_has_preserve_label
parameter_list|(
name|basic_block
name|bb
parameter_list|)
...
argument_list|)
expr_stmt|;
comment|/* Don't proceed with the crossjump unless we found a sufficient number      of matching instructions or the 'from' block was totally matched      (such that its predecessors will hopefully be redirected and the      block removed).  */
if|if
condition|(
operator|(
name|nmatch
...
name|PARAM_VALUE
argument_list|(
name|PARAM_MIN_CROSSJUMP_INSNS
argument_list|)
operator|)
operator|&&
operator|(
...
name|false
return|;
comment|/* Avoid deleting preserve label when redirecting ABNORMAL edges.  */
if|if
condition|(
name|block_has_preserve_label
argument_list|(
...
block|}
block|}
comment|/* Avoid splitting if possible.  We must always split when SRC2 has      EH predecessor edges, or we may end up with basic blocks with both      normal and EH predecessor edges.  */
if|if
condition|(
name|newpos2
operator|==
...
name|frequency
expr_stmt|;
comment|/* We may have some registers visible through the block.  */
name|redirect_to
operator|->
name|flags
operator||=
...
name|false
return|;
comment|/* Don't crossjump if this block ends in a computed jump,      unless we are optimizing for size.  */
if|if
condition|(
operator|!
name|optimize_size
...
condition|)
continue|continue;
comment|/* If nothing changed since the last attempt, there is nothing 	     we can do.  */
if|if
condition|(
operator|!
name|first_pass
...
expr_stmt|;
block|}
comment|/* Remove code labels no longer used.  */
if|if
condition|(
name|single_pred_p
argument_list|(
...
begin_comment
comment|/* Merges sequential blocks if possible.  */
end_comment
...
condition|)
block|{
comment|/* Merge the blocks and retry.  */
name|merge_blocks
argument_list|(
name|bb
argument_list|,
