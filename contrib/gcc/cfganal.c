...
name|true
return|;
comment|/* A clobber of the function return value exists for buggy       programs that fail to return a value.  It's effect is to      keep the return value from being live across the entire      function.  If we allow it to be skipped, we introduce the      possibility for register livetime aborts.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Return true if INSN should be kept in the same block as a preceding call.    This is done for a single-set whose destination is a fixed register or    whose source is the function return value.  This is a helper function for    flow_call_edges_add.  */
end_comment
