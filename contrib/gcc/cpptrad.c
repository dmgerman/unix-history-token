...
begin_comment
comment|/* CPP Library - traditional lexical analysis and macro expansion.    Copyright (C) 2002 Free Software Foundation, Inc.    Contributed by Neil Booth, May 2002  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* To be called whenever a newline character is encountered in the    input file, at CUR.  Handles DOS, Mac and Unix ends of line, and    increments pfile->line.     Returns a pointer the character after the newline sequence.  */
end_comment
...
begin_comment
comment|/* CUR points to any character in the current context, not necessarily    a backslash.  Advances CUR until all escaped newlines are skipped,    and returns the new position without updating the context.     Warns if a file buffer ends in an escaped newline.  */
end_comment
...
condition|)
block|{
comment|/* An immediate slash does not terminate the comment.  */
if|if
condition|(
name|out
index|[
...
argument_list|)
expr_stmt|;
comment|/* Canonicalize newline sequences and skip escaped ones.  */
if|if
condition|(
name|out
index|[
...
goto|;
block|}
comment|/* Premature end of file.  Fake a new line.  */
name|cur
operator|--
expr_stmt|;
if|if
...
block|{
comment|/* Skip escaped quotes here, it's easier than above, but 		 take care to first skip escaped newlines.  */
name|cur
operator|=
name|skip_escaped_newlines
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* #include changes pfile->buffer so we need to 			 update the limits of the current context.  */
goto|goto
name|start_logical_line
goto|;
block|}
...
expr_stmt|;
block|}
comment|/* Allocate room for the expansion plus NUL.  */
name|buff
operator|=
name|_cpp_get_buff
argument_list|(
...
condition|)
block|{
comment|/* Object-like and function-like macros without parameters 	 simply store their NUL-terminated replacement text.  */
name|exp
operator|=
...
condition|)
block|{
comment|/* Setting macro to NULL indicates an error occurred, and 	 prevents unnecessary work in scan_out_logical_line.  */
if|if
condition|(
operator|!
name|scan_parameters
