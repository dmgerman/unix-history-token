...
name|do_multiple_p
decl_stmt|;
comment|/* != 0 if we should handle&& and || blocks */
block|{
name|basic_block
name|test_bb
init|=
...
directive|if
literal|0
comment|/* In the case we are going to duplicate insn originally 		  present in the front of comparsion, verify that the 		  comparsion didn't clobbered the operands.  */
condition||| modified_between_p (SET_SRC (single_set (insn_b)), 				      if_info->cond_earliest, 				      NEXT_INSN (if_info->jump)))
endif|#
directive|endif
condition|)
goto|goto
...
name|n_insns
decl_stmt|;
comment|/* Determine if the preceeding block is an&& or || block.  */
if|if
condition|(
operator|(
name|n_insns
...
goto|;
block|}
comment|/* If there is a jump table following merge_bb, fail 	 if there are any insn between head and PREV_INSN (end) 	 references it.  */
if|if
condition|(
operator|(
name|label
