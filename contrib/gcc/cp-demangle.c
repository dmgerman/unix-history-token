...
begin_comment
comment|/*<special-name> ::= TV<type>                   ::= TT<type>                   ::= TI<type>                   ::= TS<type>                   ::= GV<(object) name>                   ::= T<call-offset><(base) encoding>                   ::= Tc<call-offset><call-offset><(base) encoding>    Also g++ extensions:                   ::= TC<type><(offset) number> _<(base) type>                   ::= TF<type>                   ::= TJ<type>                   ::= GR<name> */
end_comment
...
begin_comment
comment|/*<bare-function-type> ::=<type>+  */
end_comment
...
return|;
block|}
comment|/* The specification for __cxa_demangle() is that if the mangled      name could be either an extern "C" identifier, or an internal      built-in type name, then we resolve it as the identifier.  All      internal built-in type names are a single lower case character.      Frankly, this simplistic disambiguation doesn't make sense to me,      but it is documented, so we implement it here.  */
if|if
condition|(
name|IS_LOWER
argument_list|(
