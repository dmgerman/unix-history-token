...
begin_comment
comment|/* Optimize by combining instructions for GNU compiler.    Copyright (C) 1987, 88, 92-98, 1999 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Define a default value for REVERSIBLE_CC_MODE.    We can never assume that a condition code mode is safe to reverse unless    the md tells us so.  */
end_comment
...
parameter_list|)
define|\
value|do { rtx _new = (NEWVAL);					\       struct undo *_buf;					\ 								\       if (undobuf.frees)					\ 	_buf = undobuf.frees, undobuf.frees = _buf->next;	\       else							\ 	_buf = (struct undo *) xmalloc (sizeof (struct undo));	\ 								\       _buf->is_int = 0;						\       _buf->where.r =&INTO;					\       _buf->old_contents.r = INTO;				\       INTO = _new;						\       if (_buf->old_contents.r == INTO)				\ 	_buf->next = undobuf.frees, undobuf.frees = _buf;	\       else							\ 	_buf->next = undobuf.undos, undobuf.undos = _buf;	\     } while (0)
end_define
...
parameter_list|)
define|\
value|do { struct undo *_buf;					\ 								\       if (undobuf.frees)					\ 	_buf = undobuf.frees, undobuf.frees = _buf->next;	\       else							\ 	_buf = (struct undo *) xmalloc (sizeof (struct undo));	\ 								\       _buf->is_int = 1;						\       _buf->where.i = (int *)&INTO;				\       _buf->old_contents.i = INTO;				\       INTO = NEWVAL;						\       if (_buf->old_contents.i == INTO)				\ 	_buf->next = undobuf.frees, undobuf.frees = _buf;	\       else							\ 	_buf->next = undobuf.undos, undobuf.undos = _buf;	\      } while (0)
end_define
...
literal|0
decl_stmt|;
comment|/* Number of clobbers of SCRATCH we had to add.  */
name|int
name|i3_scratches
init|=
literal|0
...
argument_list|)
expr_stmt|;
comment|/* If we added any (clobber (scratch)), add them to the max for a        block.  This is a very pessimistic calculation, since we might        have had them already and this might not be the worst block, but        it's not worth doing any better.  */
name|max_scratch
operator|+=
name|i3_scratches
operator|+
...
return|;
block|}
comment|/* If we want a subreg of a constant, at offset 0, 	 take the low bits.  On a little-endian machine, that's 	 always valid.  On a big-endian machine, it's valid 	 only if the constant's mode fits in one word.   Note that we 	 cannot use subreg_lowpart_p since SUBREG_REG may be VOIDmode.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
argument_list|)
return|;
comment|/* (ashiftrt foo C) where C is the number of bits in FOO minus 1 	 is (lt foo (const_int 0)) if STORE_FLAG_VALUE is -1, so we can 	 perform the above simplification.  */
if|if
condition|(
name|STORE_FLAG_VALUE
operator|==
...
argument_list|)
return|;
comment|/* If we know that the value is already truncated, we can          replace the TRUNCATE with a SUBREG.  */
if|if
condition|(
name|num_sign_bit_copies
argument_list|(
...
argument_list|)
return|;
comment|/* If sign extension is cheaper than zero extension, then use it 	 if we know that no extraneous bits are set, and that the high 	 bit is not set.  */
if|if
condition|(
name|flag_expensive_optimizations
operator|&&
...
literal|1
decl_stmt|;
comment|/* If the considered data is wider then HOST_WIDE_INT, we can't 	     represent a mask for all its bits in a single scalar. 	     But we only care about the lower bits, so calculate these.  */
if|if
condition|(
name|GET_MODE_BITSIZE
argument_list|(
...
begin_comment
comment|/* Like recog, but we receive the address of a pointer to a new pattern.    We try to match the rtx that the pointer points to.    If that fails, we may try to modify or replace the pattern,    storing the replacement into the same pointer object.     Modifications include deletion or addition of CLOBBERs.     PNOTES is a pointer to a location where any REG_UNUSED notes added for    the CLOBBERs are placed.     PADDED_SCRATCHES is set to the number of (clobber (scratch)) patterns    we had to add.     The value is the final insn code from the pattern ultimately matched,    or -1.  */
end_comment
...
literal|0
return|;
comment|/* If the value was set in a later insn than the ones we are processing,      we can't use it even if the register was only set once, but make a quick      check to see if the previous insn set it to something.  This is commonly      the case when the same pseudo is used by repeated insns.       This does not work if there exists an instruction which is temporarily      not on the insn chain.  */
if|if
condition|(
name|INSN_CUID
argument_list|(
...
name|set
decl_stmt|;
comment|/* We can not do anything useful in this case, because there is 	 an instruction which is not on the insn chain.  */
if|if
condition|(
name|subst_prev_insn
condition|)
...
literal|0
return|;
comment|/* Skip over USE insns.  They are not useful here, and they may have 	 been made by combine, in which case they do not have a INSN_CUID 	 value.  We can't use prev_real_insn, because that would incorrectly 	 take us backwards across labels.  Skip over BARRIERs also, since 	 they could have been made by combine.  If we see one, we must be 	 optimizing dead code, so it doesn't matter what we do.  */
for|for
control|(
name|insn
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Make sure that VALUE doesn't reference X.  Replace any 	     explicit references with a CLOBBER.  If there are any remaining 	     references (rare), don't use the value.  */
if|if
condition|(
name|reg_mentioned_p
argument_list|(
...
block|}
block|}
comment|/* If we haven't found an insn for the death note and it 		 is still a REG_DEAD note, but we have hit a CODE_LABEL, 		 insert a USE insn for the register at that label and 		 put the death node there.  This prevents problems with 		 call-state tracking in caller-save.c.  */
if|if
condition|(
name|REG_NOTE_KIND
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If this insn was emitted between blocks, then update 		     basic_block_head of the current block to include it.  */
if|if
condition|(
