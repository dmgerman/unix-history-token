...
argument_list|)
return|;
comment|/* If we know that the value is already truncated, we can          replace the TRUNCATE with a SUBREG if TRULY_NOOP_TRUNCATION is 	 nonzero for the corresponding modes.  */
if|if
condition|(
name|TRULY_NOOP_TRUNCATION
argument_list|(
...
name|REG_EH_REGION
case|:
comment|/* This note must remain with the call.  It should not be possible 	     for both I2 and I3 to be a call.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If this insn was emitted between blocks, then update 		     BLOCK_HEAD of the current block to include it.  */
if|if
condition|(
name|BLOCK_END
argument_list|(
