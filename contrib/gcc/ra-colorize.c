...
name|ml
decl_stmt|;
comment|/* If we are not the first pass, put all stackwebs (which are still      backed by a new pseudo, but conceptually can stand for a stackslot,      i.e. it doesn't really matter if they get a color or not), on      the SELECT stack first, those with lowest cost first.  This way      they will be colored last, so do not constrain the coloring of the      normal webs.  But still those with the highest count are colored      before, i.e. get a color more probable.  The use of stackregs is      a pure optimization, and all would work, if we used real stackslots      from the begin.  */
if|if
condition|(
name|ra_pass
operator|>
...
block|{
comment|/* The main webs do _not_ conflict, only some parts of both.  This 	     means, that 4 is possibly true, so we need to check this too. 	     For this we go through all sub conflicts between T and C, and see if 	     the target part of C already conflicts with S.  When this is not 	     the case we disallow coalescing.  */
name|struct
name|sub_conflict
modifier|*
name|sl
...
block|}
block|}
comment|/* The next if() only gets true, if there was no wl->sub at all, in 	     which case we are only making one go through this loop with W being 	     a whole web.  */
if|if
condition|(
operator|!
name|sl
...
argument_list|)
decl_stmt|;
comment|/* If we are a spill-temp, we also look at webs coalesced 		 to precolored ones.  Otherwise we only look at webs which 		 themselves were colored, or coalesced to one.  */
if|if
condition|(
name|aw
operator|->
...
argument_list|)
expr_stmt|;
comment|/* For boehm-gc/misc.c.  If we are a difficult spilltemp, 		     also coalesced neighbors are a chance, _even_ if they 		     too are spilltemps.  At least their coalescing can be 		     broken up, which may be reset usable_regs, and makes 		     it easier colorable.  */
if|if
condition|(
name|web
operator|->
...
continue|continue;
block|}
comment|/* Mark colors for which some wide webs are involved.  For 	 those the independent sets are not simply one-node graphs, so 	 they can't be recolored independent from their neighborhood.  This 	 means, that our cost calculation can be incorrect (assuming it 	 can avoid spilling a web because it thinks some colors are available, 	 although it's neighbors which itself need recoloring might take 	 away exactly those colors).  */
if|if
condition|(
name|web2
operator|->
...
argument_list|()
expr_stmt|;
comment|/* Search the original (pre-coalesce) conflict list.  In the current 	 one some imprecise conflicts may be noted (due to combine() or 	 insert_coalesced_conflicts() relocating partial conflicts) making 	 it look like some wide webs are in conflict and having the same 	 color.  */
name|wl
operator|=
operator|(
name|web
