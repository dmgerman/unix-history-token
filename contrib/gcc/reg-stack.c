...
name|predecessors
decl_stmt|;
comment|/* Number of predecessors that need 				   to be visited.  */
block|}
typedef|*
name|block_info
typedef|;
...
begin_comment
comment|/* In the current_block, whether we're processing the first register    stack or call instruction, i.e. the regstack is currently the    same as BLOCK_INFO(current_block)->stack_in.  */
end_comment

begin_decl_stmt
specifier|static
name|bool
name|starting_stack_p
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Reorganize the stack into ascending numbers, before this insn.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Enforce rule #3: If any input operand uses the "f" constraint, all      output constraints must use the "&" earlyclobber.       ??? Detect this more deterministically by having constrain_asm_operands      record any earlyclobber.  */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* Something failed if the register wasn't on the stack.  If we had 	 malformed asms, we zapped the instruction itself, but that didn't 	 produce the same pattern of register sets as before.  To prevent 	 further failure, adjust REGSTACK to include REG at TOP.  */
name|gcc_assert
argument_list|(
name|any_malformed_asm
argument_list|)
...
return|return;
block|}
comment|/* Avoid emitting the swap if this is the first register stack insn      of the current_block.  Instead update the current_block's stack_in      and let compensate edges take care of this for us.  */
if|if
condition|(
name|current_block
operator|&&
...
name|regstack
expr_stmt|;
comment|/* Place operand 1 at the top of stack.  */
name|regno
operator|=
name|get_hard_regnum
argument_list|(
...
expr_stmt|;
block|}
comment|/* Place operand 2 next on the stack.  */
name|regno
operator|=
name|get_hard_regnum
argument_list|(
...
begin_comment
comment|/* Handle a move to or from a stack register in PAT, which is in INSN.    REGSTACK is the current stack.  Return whether a control flow insn    was deleted in the process.  */
end_comment
...
decl_stmt|;
name|bool
name|control_flow_insn_deleted
init|=
name|false
decl_stmt|;
name|src
...
condition|)
break|break;
comment|/* The destination must be dead, or life analysis is borked.  */
name|gcc_assert
argument_list|(
name|get_hard_regnum
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If the source is not live, this is yet another case of 	     uninitialized variables.  Load up a NaN instead.  */
if|if
condition|(
name|i
operator|<
...
begin_comment
comment|/* A helper function which replaces INSN with a pattern that loads up    a NaN into DEST, then invokes move_for_stack_reg.  */
end_comment
...
begin_comment
comment|/* Substitute new registers in PAT, which is part of INSN.  REGSTACK    is the current register layout.  Return whether a control flow insn    was deleted in the process.  */
end_comment
...
condition|)
block|{
comment|/* The fix_truncdi_1 pattern wants to be able to allocate 		   its own scratch register.  It does this by clobbering 		   an fp reg so that it is assured of an empty reg-stack 		   register.  If the register is live, kill it now. 		   Remove the DEAD/UNUSED note so we don't try to kill it 		   later too.  */
if|if
condition|(
name|note
condition|)
...
expr_stmt|;
block|}
comment|/* Keep operand 1 matching with destination.  */
if|if
condition|(
...
case|:
case|case
name|UNSPEC_FIST_FLOOR
case|:
case|case
name|UNSPEC_FIST_CEIL
case|:
...
case|:
case|case
name|UNSPEC_FRNDINT_FLOOR
case|:
case|case
name|UNSPEC_FRNDINT_CEIL
case|:
case|case
name|UNSPEC_FRNDINT_TRUNC
case|:
case|case
name|UNSPEC_FRNDINT_MASK_PM
case|:
comment|/* These insns only operate on the top of the stack.  */
name|src1
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Input should never die, it is 		   replaced with output.  */
name|src1_note
operator|=
name|find_regno_note
argument_list|(
...
name|UNSPEC_FYL2XP1
case|:
comment|/* These insns operate on the top two stack slots.  */
name|src1
operator|=
name|get_true_reg
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Pop both input operands from the stack.  */
name|CLEAR_HARD_REG_BIT
argument_list|(
name|regstack
operator|->
...
literal|2
expr_stmt|;
comment|/* Push the result back onto the stack.  */
name|regstack
operator|->
name|reg
index|[
...
break|break;
case|case
name|UNSPEC_FSCALE_FRACT
case|:
case|case
name|UNSPEC_FPREM_F
case|:
case|case
name|UNSPEC_FPREM1_F
case|:
comment|/* These insns operate on the top two stack slots. 		   first part of double input, double output insn.  */
name|src1
operator|=
name|get_true_reg
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Inputs should never die, they are 		   replaced with outputs.  */
name|gcc_assert
argument_list|(
operator|!
name|src1_note
...
argument_list|)
expr_stmt|;
comment|/* Push the result back onto stack. Empty stack slot 		   will be filled in second part of insn.  */
if|if
condition|(
name|STACK_REG_P
argument_list|(
...
break|break;
case|case
name|UNSPEC_FSCALE_EXP
case|:
case|case
name|UNSPEC_FPREM_U
case|:
case|case
name|UNSPEC_FPREM1_U
case|:
comment|/* These insns operate on the top two stack slots./ 		   second part of double input, double output insn.  */
name|src1
operator|=
name|get_true_reg
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Push the result back onto stack. Fill empty slot from 		   first part of insn and fix top of stack pointer.  */
if|if
condition|(
name|STACK_REG_P
argument_list|(
...
break|break;
case|case
name|UNSPEC_SINCOS_COS
case|:
case|case
name|UNSPEC_TAN_ONE
case|:
case|case
name|UNSPEC_XTRACT_FRACT
case|:
comment|/* These insns operate on the top two stack slots, 		   first part of one input, double output insn.  */
name|src1
operator|=
name|get_true_reg
argument_list|(
...
break|break;
case|case
name|UNSPEC_SINCOS_SIN
case|:
case|case
name|UNSPEC_TAN_TAN
case|:
case|case
name|UNSPEC_XTRACT_EXP
case|:
comment|/* These insns operate on the top two stack slots, 		   second part of one input, double output insn.  */
name|src1
operator|=
name|get_true_reg
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* If the register that dies is not at the top of 		       stack, then move the top of stack to the dead reg. 		       Top of stack should never die, as it is the 		       destination.  */
name|gcc_assert
argument_list|(
name|regno
operator|!=
...
condition|)
block|{
comment|/* Sigh - clobbers always have QImode.  But replace_reg knows 	     that these regs can't be MODE_INT and will assert.  Just put 	     the right reg there without calling replace_reg.  */
operator|*
name|clobber_loc
index|[
name|i
...
name|control_flow_insn_deleted
return|;
comment|/* If this a noreturn call, we can't insert pop insns after it.      Instead, reset the stack state to empty.  */
if|if
condition|(
name|CALL_P
argument_list|(
...
begin_comment
comment|/* Change the organization of the stack so that it fits a new basic    block.  Some registers might have to be popped, but there can never be    a register live in the new block that is not now live.     Insert any needed insns before or after INSN, as indicated by    WHERE.  OLD is the original stack layout, and NEW is the desired    form.  OLD is updated to reflect the code emitted, i.e., it will be    the same as NEW upon return.     This function will not preserve block_end[].  But that information    is no longer needed once this has executed.  */
end_comment
...
literal|0
decl_stmt|;
comment|/* Stack adjustments for the first insn in a block update the      current_block's stack_in instead of inserting insns directly.      compensate_edges will add the necessary code later.  */
if|if
condition|(
name|current_block
operator|&&
...
comment|/* If the destination block's stack already has a specified layout      and contains two or more registers, use a more intelligent algorithm      to pop registers that minimizes the number number of fxchs below.  */
if|if
condition|(
name|new
operator|->
...
name|topsrc
decl_stmt|;
comment|/* First pass to determine the free slots.  */
for|for
control|(
name|reg
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Second pass to allocate preferred slots.  */
name|topsrc
operator|=
operator|-
literal|1
...
condition|)
block|{
comment|/* If this is a preference for the new top of stack, record 		     the fact by remembering it's old->reg in topsrc.  */
if|if
condition|(
name|next
operator|==
...
name|reg
expr_stmt|;
comment|/* Intentionally, avoid placing the top of stack in it's correct 	 location, if we still need to permute the stack below and we 	 can usefully place it somewhere else.  This is the case if any 	 slot is still unallocated, in which case we should place the 	 top of stack there.  */
if|if
condition|(
name|topsrc
operator|!=
...
break|break;
block|}
comment|/* Third pass allocates remaining slots and emits pop insns.  */
name|next
operator|=
name|new
operator|->
...
block|{
comment|/* The following loop attempts to maximize the number of times we 	 pop the top of the stack, as this permits the use of the faster 	 ffreep instruction on platforms that support it.  */
name|int
name|live
decl_stmt|,
name|next
...
begin_comment
comment|/* Copy the stack info from the end of edge E's source block to the    start of E's destination block.  */
end_comment

begin_function
specifier|static
name|void
name|propagate_stack
parameter_list|(
name|edge
name|e
parameter_list|)
...
name|reg
decl_stmt|;
comment|/* Preserve the order of the original stack, but check whether      any pops are needed.  */
name|dest_stack
operator|->
name|top
operator|=
...
begin_comment
comment|/* Adjust the stack of edge E's source block on exit to match the stack    of it's target block upon input.  The stack layouts of both blocks    should have been defined by now.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Check whether stacks are identical.  */
if|if
condition|(
name|target_stack
operator|->
...
expr_stmt|;
block|}
comment|/* Abnormal calls may appear to have values live in st(0), but the      abnormal return path will not have actually loaded the values.  */
if|if
condition|(
name|e
operator|->
...
condition|)
block|{
comment|/* Assert that the lifetimes are as we expect -- one value          live at st(0) on the end of the source block, and no          values live at the beginning of the destination block. 	 For complex return values, we may have st(1) live as well.  */
name|gcc_assert
argument_list|(
name|source_stack
operator|->
...
return|;
block|}
comment|/* Handle non-call EH edges specially.  The normal return path have      values in registers.  These will be popped en masse by the unwind      library.  */
if|if
condition|(
name|e
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Make a copy of source_stack as change_stack is destructive.  */
name|regstack
operator|=
operator|*
name|source_stack
...
begin_comment
comment|/* Traverse all non-entry edges in the CFG, and emit the necessary    edge compensation code to change the stack from stack_out of the    source block to the stack_in of the destination block.  */
end_comment

begin_function
specifier|static
name|bool
name|compensate_edges
parameter_list|(
name|void
parameter_list|)
block|{
...
begin_comment
comment|/* Select the better of two edges E1 and E2 to use to determine the    stack layout for their shared destination basic block.  This is    typically the more frequently executed.  The edge E1 may be NULL    (in which case E2 is returned), but E2 is always non-NULL.  */
end_comment
...
name|e2
return|;
comment|/* Prefer critical edges to minimize inserting compensation code on      critical edges.  */
if|if
condition|(
...
name|e2
return|;
comment|/* Avoid non-deterministic behavior.  */
return|return
operator|(
name|e1
operator|->
...
name|false
expr_stmt|;
comment|/* Choose an initial stack layout, if one hasn't already been chosen.  */
if|if
condition|(
name|bi
operator|->
...
name|ei
decl_stmt|;
comment|/* Select the best incoming edge (typically the most frequent) to 	 use as a template for this basic block.  */
name|FOR_EACH_EDGE
argument_list|(
argument|e
argument_list|,
...
block|{
comment|/* No predecessors.  Create an arbitrary input stack.  */
name|bi
operator|->
name|stack_in
operator|.
...
name|sp
decl_stmt|;
comment|/* We process the blocks in a top-down manner, in a way such that one block      is only processed after all its predecessors.  The number of predecessors      of every block has already been computed.  */
name|stack
operator|=
name|XNEWVEC
argument_list|(
...
literal|1
expr_stmt|;
comment|/* ??? Future: process inner loops first, and give them arbitrary      initial stacks which emit_swap_insn can modify.  This ought to      prevent double fxch that often appears at the head of a loop.  */
...
name|false
return|;
comment|/* Ok, floating point instructions exist.  If not optimizing,      build the CFG and run life analysis.      Also need to rebuild life when superblock scheduling is done      as it don't update liveness yet.  */
if|if
condition|(
operator|!
name|optimize
...
argument_list|)
expr_stmt|;
comment|/* A QNaN for initializing uninitialized variables.       ??? We can't load from constant memory in PIC mode, because      we're inserting these instructions before the prologue and      the PIC register hasn't been set up.  In that case, fall back      on zero, which we can get from `ldz'.  */
if|if
condition|(
name|flag_pic
condition|)
