...
block|}
block|}
comment|/* The floating-point emulator can handle all conversions between      FP and integer operands.  This simplifies reload because it      doesn't have to deal with constructs like (subreg:DI      (const_double:SF ...)) or (subreg:DF (const_int ...)).  */
...
argument_list|)
expr_stmt|;
comment|/* At this point we don't know _why_ the address is invalid.  It      could have secondary memory refereces, multiplies or anything.       However, if we did go and rearrange things, we can wind up not      being able to recognize the magic around pic_offset_table_rtx.      This stuff is fragile, and is yet another example of why it is      bad to expose PIC machinery too early.  */
if|if
condition|(
operator|!
name|memory_address_p
...
begin_comment
comment|/* Similar to `copy_rtx' except that if MAY_SHARE is present, it is    placed in the result directly, rather than being copied.  MAY_SHARE is    either a MEM of an EXPR_LIST of MEMs.  */
end_comment
...
begin_comment
comment|/* Like emit_insn_after, but set INSN_SCOPE according to SCOPE.  */
end_comment
...
begin_comment
comment|/* Like emit_jump_insn_after, but set INSN_SCOPE according to SCOPE.  */
end_comment
...
begin_comment
comment|/* Like emit_call_insn_after, but set INSN_SCOPE according to SCOPE.  */
end_comment
...
begin_comment
comment|/* Like emit_insn_before, but set INSN_SCOPE according to SCOPE.  */
end_comment
...
begin_comment
comment|/* Generate the constant 0.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* We need to call this function after we to set CONST0_RTX first.  */
if|if
condition|(
operator|!
name|CONST0_RTX
...
begin_comment
comment|/* Generate a vector like gen_rtx_raw_CONST_VEC, but use the zero vector when    all elements are zero.  */
end_comment
