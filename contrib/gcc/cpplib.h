...
argument_list|)
expr_stmt|;
comment|/* A parse_marker indicates a previous position,    which we can backtrack to. */
struct|struct
name|parse_marker
block|{
name|cpp_buffer
...
block|}
struct|;
comment|/* If we have a huge buffer, may need to cache more recent counts */
define|#
directive|define
name|CPP_LINE_BASE
parameter_list|(
...
name|seen_eof
decl_stmt|;
comment|/* True if buffer contains escape sequences.      Currently there are are only two kind:      "@-" means following identifier should not be macro-expanded.      "@ " means a token-separator.  This turns into " " in final output           if not stringizing and needed to separate tokens; otherwise nothing.      "@@" means a normal '@'.      (An '@' inside a string stands for itself and is never an escape.) */
name|char
name|has_escapes
decl_stmt|;
block|}
...
name|PFILE
parameter_list|)
value|((PFILE)->limit - (PFILE)->token_buffer)
define|#
directive|define
name|CPP_PWRITTEN
parameter_list|(
...
parameter_list|)
define|\
value|(CPP_WRITTEN (PFILE) + N> (PFILE)->token_buffer_size \&& (cpp_grow_buffer (PFILE, N), 0))
...
parameter_list|)
define|\
value|(bcopy (STR, (PFILE)->limit, (N)), (PFILE)->limit += (N))
...
name|PFILE
parameter_list|)
value|(&(PFILE)->buffer_stack[CPP_STACK_MAX])
else|#
directive|else
define|#
directive|define
...
directive|endif
comment|/* Pointed to by parse_file::data. */
struct|struct
name|cpp_options
block|{
name|char
...
name|debug_output
decl_stmt|;
comment|/* Pending -D, -U and -A options, in reverse order. */
name|struct
name|cpp_pending
modifier|*
name|pending
