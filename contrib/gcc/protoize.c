...
begin_comment
comment|/* defined(__cplusplus)&& !defined(__STDC__) */
end_comment
...
begin_comment
comment|/* Users are not supposed to use _POSIX_SOURCE to say the    system is a POSIX system.  That is not what _POSIX_SOURCE means! -- rms  */
end_comment

begin_comment
comment|/* If the user asked for POSIX via _POSIX_SOURCE, turn on POSIX code.  */
end_comment
...
begin_comment
comment|/* We should be able to define _POSIX_SOURCE unconditionally, 		but some systems respond in buggy ways to it, 		including SunOS 4.1.1.  Which we don't classify as POSIX.  */
end_comment

begin_comment
comment|/* In case this is a POSIX system with an ANSI C compiler,    ask for definition of all POSIX facilities.  */
end_comment
...
name|flag
parameter_list|)
value|open((char *)file, mode, flag)
end_define
...
begin_comment
comment|/* These conflict with stdio.h on some systems.  */
end_comment
...
begin_comment
comment|/* size_t from sys/types.h may fail to match GCC. 	 If so, we would get a warning from this.  */
end_comment

begin_endif
unit|extern size_t   strlen ()
endif|#
directive|endif
end_endif

begin_comment
comment|/* Fork is not declared because the declaration caused a conflict    on the HPPA.  */
end_comment
...
begin_comment
comment|/* Make a table of default system include directories    just as it is done in cccp.c.  */
end_comment
...
begin_comment
comment|/* Allocate some space, but check that the allocation was successful.  */
end_comment

begin_comment
comment|/* alloca.c uses this, so don't make it static.  */
end_comment
...
begin_comment
comment|/* Reallocate some space, but check that the reallocation was successful.  */
end_comment
...
begin_comment
comment|/* Deallocate the area pointed to by an arbitrary pointer, but first, strip    the `const' qualifier from it and also make sure that the pointer value    is non-null.  */
end_comment
...
begin_comment
comment|/* Make a copy of the concatenation of INPUT1 and INPUT2.  */
end_comment
...
begin_comment
unit|}    if (my_access (path, W_OK))     return 0;    return 1; }
...
name|size
expr_stmt|;
comment|/* Assume the there will not be another replacement required                  within the text just replaced.  */
name|s
operator|+=
name|len
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|/* Note that we don't care here if the given file was converted or not.  It      is possible that the given file was *not* converted, simply because there      was nothing in it which actually required conversion.  Even in this case,      we want to do the renaming.  Note that we only rename files with the .c      suffix.  */
if|if
condition|(
name|filename
index|[
...
name|stderr
argument_list|,
literal|"%s: warning: can't link file `%s' to `%s': %s\n"
argument_list|,
name|pname
argument_list|,
name|shortpath
...
name|stderr
argument_list|,
literal|"%s: warning: can't delete file `%s': %s\n"
argument_list|,
name|pname
argument_list|,
name|shortpath
...
directive|if
literal|0
block|else               fprintf (stderr, "%s: %d: warning: no extern definition for `%s'\n", 		       shortpath (NULL, file), user->line, 		       user->hash_entry->symbol);
endif|#
directive|endif
block|}
block|}
