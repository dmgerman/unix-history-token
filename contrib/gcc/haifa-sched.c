...
literal|0
expr_stmt|;
comment|/* reg_last_sets[r] is now a list of insns */
for|for
control|(
name|u
operator|=
...
argument|{
comment|/* reg_last_sets[r] is now a list of insns */
argument|free_list (&reg_last_sets[i],&unused_insn_list); 			       reg_last_sets[i] 				 = alloc_INSN_LIST (insn, NULL_RTX); 			     }
argument_list|)
empty_stmt|;
name|CLEAR_REG_SET
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* In interblock scheduling, basic_block_live_at_start may have changed.  */
if|if
condition|(
name|current_nr_blocks
operator|>
...
begin_comment
comment|/* Do the splitting of insns in the block b.  */
end_comment
