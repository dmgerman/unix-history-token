...
begin_comment
comment|/* Parameters affecting the decision of rank_for_schedule().    ??? Nope.  But MIN_PROBABILITY is used in compute_trg_info.  */
end_comment
...
directive|ifdef
name|HAVE_cc0
comment|/* There may be other cc0 setters earlier on in this block.      Look for them and include them in the set not to be disturbed.  */
if|if
condition|(
name|insn
operator|!=
...
name|edge_list
decl_stmt|;
comment|/* The scheduler runs after estimate_probabilities; therefore, we 	     can't blindly call back into find_basic_blocks since doing so 	     could invalidate the branch probability info.  We could, 	     however, call cleanup_cfg.  */
name|edge_list
operator|=
