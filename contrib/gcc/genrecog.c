...
begin_comment
comment|/* Generate code from machine description to recognize rtl as insns.    Copyright (C) 1987, 1988, 1992, 1993, 1994, 1995, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004, 2005 Free Software Foundation, Inc.     This file is part of GCC.     GCC is free software; you can redistribute it and/or modify it    under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2, or (at your option)    any later version.     GCC is distributed in the hope that it will be useful, but WITHOUT    ANY WARRANTY; without even the implied warranty of MERCHANTABILITY    or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public    License for more details.     You should have received a copy of the GNU General Public License    along with GCC; see the file COPYING.  If not, write to the Free    Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA    02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* A listhead of decision trees.  The alternatives to a node are kept    in a doubly-linked list so we can easily add nodes to the proper    place when merging.  */
end_comment
...
name|num_insns
decl_stmt|;
comment|/* Number if insn in a define_peephole2.  */
name|enum
name|machine_mode
name|mode
decl_stmt|;
...
name|data
decl_stmt|;
comment|/* Optimization hints for this predicate.  */
name|enum
name|machine_mode
name|mode
decl_stmt|;
...
parameter_list|)
define|\
value|((a) == I ? ((b) == N ? N : I) :		\    (b) == I ? ((a) == N ? N : I) :		\    (a)&& (b))
end_define
...
parameter_list|)
define|\
value|((a) == I ? ((b) == Y ? Y : I) :		\    (b) == I ? ((a) == Y ? Y : I) :		\    (a) || (b))
end_define
...
begin_comment
comment|/* 0 means no warning about that code yet, 1 means warned.  */
end_comment

begin_decl_stmt
specifier|static
name|char
name|did_you_mean_codes
index|[
name|NUM_RTX_CODE
index|]
decl_stmt|;
...
begin_comment
comment|/* Recursively calculate the set of rtx codes accepted by the    predicate expression EXP, writing the result to CODES.  */
end_comment

begin_function
specifier|static
name|void
name|compute_predicate_codes
parameter_list|(
name|rtx
name|exp
parameter_list|,
...
name|IF_THEN_ELSE
case|:
comment|/* a ? b : c  accepts the same codes as (a& b) | (!a& c).  */
name|compute_predicate_codes
argument_list|(
name|XEXP
argument_list|(
...
name|MATCH_CODE
case|:
comment|/* MATCH_CODE allows a specified list of codes.  However, if it 	 does not apply to the top level of the expression, it does not 	 constrain the set of codes for the top level.  */
if|if
condition|(
name|XSTR
argument_list|(
...
name|pattern_lineno
argument_list|,
literal|"empty match_code expression"
argument_list|)
expr_stmt|;
name|error_count
operator|++
...
name|pattern_lineno
argument_list|,
literal|"match_code \"%.*s\" matches nothing"
argument_list|,
operator|(
name|int
operator|)
...
name|MATCH_OPERAND
case|:
comment|/* MATCH_OPERAND disallows the set of codes that the named predicate 	 disallows, and is indeterminate for the codes that it does allow.  */
block|{
name|struct
name|pred_data
modifier|*
...
name|pattern_lineno
argument_list|,
literal|"reference to unknown predicate '%s'"
argument_list|,
name|XSTR
argument_list|(
name|exp
...
name|MATCH_TEST
case|:
comment|/* (match_test WHATEVER) is completely indeterminate.  */
name|memset
argument_list|(
name|codes
argument_list|,
...
name|pattern_lineno
argument_list|,
literal|"'%s' cannot be used in a define_predicate expression"
argument_list|,
name|GET_RTX_NAME
argument_list|(
name|GET_CODE
...
begin_comment
comment|/* Process a define_predicate expression: compute the set of predicates    that can be matched, and record this as a known predicate.  */
end_comment
...
begin_comment
comment|/* Search for and return operand N, stop when reaching node STOP.  */
end_comment
...
name|pattern_lineno
argument_list|,
literal|"operand %i duplicated before defined"
argument_list|,
name|XINT
argument_list|(
name|pattern
...
name|pattern_lineno
argument_list|,
literal|"warning: unknown predicate '%s'"
argument_list|,
name|pred_name
argument_list|)
expr_stmt|;
...
name|pattern_lineno
argument_list|,
literal|"warning: destination operand %d "
literal|"allows non-lvalue"
argument_list|,
name|XINT
argument_list|(
name|pattern
...
argument_list|)
expr_stmt|;
comment|/* Find the referent for a DUP.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|num_insns
decl_stmt|;
comment|/* Check we have sufficient insns.  This avoids complications 	     because we then know peep2_next_insn never fails.  */
name|num_insns
operator|=
name|XVECLEN
argument_list|(
...
name|mode
expr_stmt|;
comment|/* See if we know about this predicate. 	       If we do, remember it for use below.  	       We can optimize the generated code a little if either 	       (a) the predicate only accepts one code, or (b) the 	       predicate does not allow CONST_INT, in which case it 	       can match only if the modes match.  */
name|pred
operator|=
name|lookup_predicate
argument_list|(
...
name|pattern_lineno
argument_list|,
literal|"predicate '%s' used in match_parallel "
literal|"does not allow only PARALLEL"
argument_list|,
name|pred
operator|->
name|name
...
name|pattern_lineno
argument_list|,
literal|"warning: unknown predicate '%s' in '%s' expression"
argument_list|,
name|pred_name
argument_list|,
name|GET_RTX_NAME
...
name|VOIDmode
expr_stmt|;
comment|/* Accept the operand, i.e. record it in `operands'.  */
name|test
operator|=
name|new_decision_test
argument_list|(
...
begin_comment
comment|/* Emit a HOST_WIDE_INT as an integer constant expression.  We need to take    special care to avoid "decimal constant is so large that it is unsigned"    warnings in the resulting code.  */
end_comment

begin_function
specifier|static
name|void
name|print_host_wide_int
parameter_list|(
name|HOST_WIDE_INT
name|val
parameter_list|)
...
block|{
comment|/* None of the other tests are amenable.  */
return|return
name|p
return|;
block|}
...
name|printf
argument_list|(
literal|"x%d == const_int_rtx[MAX_SAVED_CONST_INT + (%d)]"
argument_list|,
name|depth
argument_list|,
operator|(
...
name|printf
argument_list|(
literal|"%sreturn gen_split_%d (insn, operands);\n"
argument_list|,
name|indent
argument_list|,
name|test
...
name|uncond
decl_stmt|;
comment|/* Scan the tests and simplify comparisons against small      constants.  */
for|for
control|(
name|test
operator|=
...
name|RECOG
case|:
comment|/* If this is a DEFINE_INSN and X is a PARALLEL, see if it ends 	 with a group of CLOBBERs of (hard) registers or MATCH_SCRATCHes. 	 If so, set up to recognize the pattern without these CLOBBERs.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|printf
argument_list|(
literal|"extern rtx gen_peephole2_%d (rtx, rtx *);\n"
argument_list|,
name|next_insn_code
argument_list|)
expr_stmt|;
