...
begin_comment
comment|/* varargs must always be included after *config.h.  */
end_comment
...
name|printf
argument_list|(
literal|"insn_addresses[INSN_UID (operands[%d])]"
argument_list|,
name|XINT
argument_list|(
name|exp
...
directive|if
literal|0
block|code = GET_CODE (orig);   switch (code)     {     case CONST_INT:     case CONST_DOUBLE:     case SYMBOL_REF:     case CODE_LABEL:       return orig;     }    copy = rtx_alloc (code);   PUT_MODE (copy, GET_MODE (orig));   RTX_UNCHANGING_P (copy) = 1;      bcopy ((char *)&XEXP (orig, 0), (char *)&XEXP (copy, 0), 	 GET_RTX_LENGTH (GET_CODE (copy)) * sizeof (rtx));   return copy;
endif|#
directive|endif
block|}
end_function
