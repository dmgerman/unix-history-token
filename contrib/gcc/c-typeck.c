...
begin_comment
comment|/* Build expressions with type checking for C compiler.    Copyright (C) 1987, 1988, 1991, 1992, 1993, 1994, 1995, 1996, 1997,    1998, 1999, 2000, 2001, 2002, 2003, 2004 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
name|POINTER_TYPE
case|:
comment|/* We must give ObjC the first crack at comparing pointers, since 	   protocol qualifiers may be involved.  */
if|if
condition|(
name|c_dialect_objc
argument_list|()
...
name|RECORD_TYPE
case|:
comment|/* We are dealing with two distinct structs.  In assorted Objective-C 	 corner cases, however, these can still be deemed equivalent.  */
if|if
condition|(
name|c_dialect_objc
argument_list|()
...
name|VECTOR_TYPE
case|:
comment|/* The target might allow certain vector types to be compatible.  */
name|val
operator|=
call|(
modifier|*
...
begin_comment
comment|/* Determine whether two types derive from the same translation unit.    If the CONTEXT chain ends in a null, that type's context is still    being parsed, so if two types have context chains ending in null,    they're in the same translation unit.  */
end_comment
...
begin_comment
comment|/* The C standard says that two structures in different translation    units are compatible with each other only if the types of their    fields are compatible (among other things).  So, consider two copies    of this structure:  */
end_comment
...
begin_comment
comment|/* Can they be compatible with each other?  We choose to break the    recursion by allowing those types to be compatible.  */
end_comment
...
name|false
decl_stmt|;
comment|/* We have to verify that the tags of the types are the same.  This      is harder than it looks because this may be a typedef, so we have      to go look at the original type.  It may even be a typedef of a      typedef...  */
while|while
condition|(
name|TYPE_NAME
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 'volatile' qualifiers on a function's return type mean the function      is noreturn.  */
if|if
condition|(
name|pedantic
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* Check that the function is called through a compatible prototype.      If it is not, replace the call by a trap, wrapped up in a compound      expression if necessary.  This has the nice side-effect to prevent      the tree-inliner from generating invalid assignment trees which may      blow up in the RTL expander later.       ??? This doesn't work for Objective-C because objc_comptypes      refuses to compare function prototypes, yet the compiler appears      to build calls that are flagged as invalid by C's comptypes.  */
if|if
condition|(
operator|!
name|c_dialect_objc
...
name|pedwarn
argument_list|(
literal|"use of conditional expressions as lvalues is deprecated"
argument_list|)
expr_stmt|;
break|break;
case|case
...
name|pedwarn
argument_list|(
literal|"use of compound expressions as lvalues is deprecated"
argument_list|)
expr_stmt|;
break|break;
default|default:
name|pedwarn
argument_list|(
literal|"use of cast expressions as lvalues is deprecated"
argument_list|)
expr_stmt|;
break|break;
block|}
...
begin_comment
comment|/* Convert VALUE for assignment into inlined parameter PARM.  ARGNUM    is used for error and waring reporting and indicates which argument    is being processed.  */
end_comment
...
parameter_list|)
define|\
value|{									\   int depth = SPELLING_DEPTH ();					\ 									\   if (depth>= spelling_size)						\     {									\       spelling_size += 10;						\       if (spelling_base == 0)						\ 	spelling_base = xmalloc (spelling_size * sizeof (struct spelling)); \       else								\         spelling_base = xrealloc (spelling_base,		\ 				  spelling_size * sizeof (struct spelling)); \       RESTORE_SPELLING_DEPTH (depth);					\     }									\ 									\   spelling->kind = (KIND);						\   spelling->MEMBER = (VALUE);						\   spelling++;								\ }
end_define
...
condition|)
block|{
comment|/* Attach the first case label to the SWITCH_BODY.  */
name|SWITCH_BODY
argument_list|(
name|switch_stack
operator|->
...
name|switch_stack
decl_stmt|;
comment|/* If we've not seen any case labels (or a default), we may still      need to chain any statements that were seen as the SWITCH_BODY.  */
if|if
condition|(
name|SWITCH_BODY
argument_list|(
...
expr_stmt|;
block|}
comment|/* Rechain the next statements to the SWITCH_STMT.  */
name|last_tree
operator|=
name|cs
operator|->
