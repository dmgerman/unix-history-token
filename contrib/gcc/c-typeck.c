...
name|pedwarn
argument_list|(
literal|"ISO C89 forbids subscripting non-lvalue array"
argument_list|)
expr_stmt|;
block|}
name|type
...
argument_list|)
expr_stmt|;
comment|/* constructor_max_index needs to be an INTEGER_CST.  Attempts 	     to initialize VLAs will cause an proper error; avoid tree 	     checking errors as well by setting a safe value.  */
if|if
condition|(
name|constructor_max_index
operator|&&
...
begin_comment
comment|/* Common handling for both array range and field name designators.    ARRAY argument is non-zero for array ranges.  Returns zero for success.  */
end_comment
