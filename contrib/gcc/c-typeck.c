...
begin_comment
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cm) */
end_comment

begin_function_decl
specifier|static
name|bool
name|types_are_block_compatible
parameter_list|(
name|tree
name|lhptee
parameter_list|,
...
begin_comment
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cm) */
end_comment
...
name|t1
return|;
comment|/* APPLE LOCAL begin blocks 6065211 */
name|gcc_assert
argument_list|(
operator|(
name|TREE_CODE
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end blocks 6065211 */
comment|/* Merge the attributes.  */
name|attributes
operator|=
name|targetm
...
condition|)
block|{
comment|/* APPLE LOCAL begin radar 5795493 */
case|case
name|BLOCK_POINTER_TYPE
case|:
name|val
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL end radar 5795493 */
case|case
name|POINTER_TYPE
case|:
...
begin_comment
comment|/* APPLE LOCAL radar 6212722 */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 4697411 */
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ ci) */
if|if
condition|(
name|decl
operator|&&
...
condition|)
block|{
comment|/* APPLE LOCAL begin radar 5803005 (C++ ci) */
name|bool
name|gdecl
decl_stmt|;
comment|/* We are referencing a variable inside a block whose declaration 	     is outside.  */
name|gcc_assert
argument_list|(
name|decl
operator|&&
...
name|VAR_DECL
operator|&&
comment|/* APPLE LOCAL radar 6177162 */
operator|(
name|DECL_EXTERNAL
argument_list|(
name|decl
...
operator|)
expr_stmt|;
comment|/* Treat all 'global' variables as 'byref' by default. */
comment|/* APPLE LOCAL begin radar 6014138 (C++ ci) */
if|if
condition|(
name|gdecl
operator|||
...
operator|)
condition|)
comment|/* APPLE LOCAL end radar 6014138 (C++ ci) */
block|{
comment|/* APPLE LOCAL begin radar 5803600 (C++ ci) */
comment|/* byref globals are directly accessed. */
comment|/* APPLE LOCAL begin radar 7760213 */
if|if
condition|(
operator|!
name|gdecl
...
name|error
argument_list|(
literal|"cannot access __block variable of array type inside block"
argument_list|)
expr_stmt|;
comment|/* build a decl for the byref variable. */
name|decl
operator|=
name|build_block_byref_decl
argument_list|(
name|id
argument_list|,
name|decl
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 7760213 */
else|else
name|add_block_global_byref_list
argument_list|(
name|decl
argument_list|)
expr_stmt|;
...
block|{
comment|/* 'byref' globals are never copied-in. So, do not add 		 them to the copied-in list. */
if|if
condition|(
operator|!
name|in_block_global_byref_list
argument_list|(
name|decl
argument_list|)
condition|)
block|{
comment|/* APPLE LOCAL begin radar 7721728 */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"cannot access copied-in variable of array type inside block"
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 7721728 */
comment|/* build a new decl node. set its type to 'const' type 		   of the old decl. */
name|decl
operator|=
name|build_block_ref_decl
argument_list|(
name|id
argument_list|,
name|decl
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 5803600 (C++ ci) */
comment|/* APPLE LOCAL end radar 5803005 (C++ ci) */
block|}
block|}
name|ref
operator|=
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ ci) */
elseif|else
if|if
condition|(
name|fun
...
name|error
argument_list|(
literal|"too many arguments to block call"
argument_list|)
expr_stmt|;
break|break;
block|}
comment|/* APPLE LOCAL end radar 5732232 - blocks */
comment|/* APPLE LOCAL begin radar 4491608 */
name|error
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 4491608 */
break|break;
block|}
if|if
condition|(
...
name|error
argument_list|(
literal|"too few arguments to block %qE"
argument_list|,
name|function
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ co) */
comment|/* APPLE LOCAL radar 5957801 */
elseif|else
if|if
condition|(
name|code1
...
name|error
argument_list|(
literal|"block pointer/integer type mismatch in conditional expression"
argument_list|)
expr_stmt|;
else|else
block|{
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ co) */
if|if
condition|(
operator|!
name|result_type
...
name|value
expr_stmt|;
comment|/* APPLE LOCAL begin don't sign-extend pointers cast to integers */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end don't sign-extend pointers cast to integers */
name|value
operator|=
name|convert
argument_list|(
...
specifier|static
name|bool
name|functiontypes_are_block_compatible
parameter_list|(
name|tree
name|f1
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 6246965, 6196572 */
return|return
operator|(
operator|!
name|arg1
...
argument_list|)
return|;
comment|/* APPLE LOCAL begin radar 5882266 (C++ cm) */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
return|;
comment|/* APPLE LOCAL end radar 5882266 (C++ cm) */
comment|/* APPLE LOCAL begin radar 5988995 (C++ cm) */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
argument_list|)
return|;
comment|/* APPLE LOCAL end radar 5988995 (C++ cm) */
return|return
name|false
return|;
block|}
...
begin_comment
comment|/* APPLE LOCAL begin radar 5847213 - radar 6329245 */
end_comment

begin_comment
comment|/**  build_block_call - Routine to build a block call; as in:   ((double(*)(void *, int))(BLOCK_PTR_EXP->__FuncPtr))(I, 42);  FNTYPE is the original function type derived from the syntax.  BLOCK_PTR_EXP is the block pointer variable.  PARAMS is the parameter list. */
end_comment
...
block|{
name|tree
name|function_ptr_exp
decl_stmt|;
name|tree
name|typelist
decl_stmt|;
name|bool
name|block_ptr_exp_side_effect
init|=
name|TREE_SIDE_EFFECTS
argument_list|(
name|block_ptr_exp
argument_list|)
decl_stmt|;
comment|/* First convert BLOCK_PTR_EXP to 'void *'. */
name|block_ptr_exp
operator|=
name|convert
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* BLOCK_PTR_VAR->__FuncPtr */
name|function_ptr_exp
operator|=
name|build_component_ref
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Build: result_type(*)(void *, function-arg-type-list) */
name|typelist
operator|=
name|TYPE_ARG_TYPES
argument_list|(
...
begin_comment
comment|/* APPLE LOCAL end radar 5847213 - radar 6329245 */
end_comment
...
name|lhstype
decl_stmt|;
comment|/* APPLE LOCAL __block assign sequence point 6639533 */
name|bool
name|insert_sequence_point
init|=
name|false
decl_stmt|;
comment|/* APPLE LOCAL begin radar 4426814 */
if|if
condition|(
name|c_dialect_objc
argument_list|()
...
condition|)
block|{
comment|/* APPLE LOCAL radar 5276085 */
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 4426814 */
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin __block assign sequence point 6639533 */
comment|/* For byref = x;, we have to transform this into {( typeof(x) x' =      x; byref = x`; )} to ensure there is a sequence point before the      evaluation of the byref, inorder to ensure that the access      expression for byref doesn't start running before x is evaluated,      as it will access the __forwarding pointer and that must be done      after x is evaluated.  */
comment|/* First we check to see if lhs is a byref...  byrefs look like: 	__Block_byref_X.__forwarding->x  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|old_rhs
condition|)
comment|/* And arrange for the sequence point to be inserted.  */
name|insert_sequence_point
operator|=
name|true
expr_stmt|;
...
block|}
block|}
comment|/* APPLE LOCAL end __block assign sequence point 6639533 */
name|newrhs
operator|=
name|rhs
expr_stmt|;
...
break|break;
block|}
comment|/* APPLE LOCAL radar 6231433 */
name|objc_ok
operator|=
...
operator|==
name|REFERENCE_TYPE
comment|/* APPLE LOCAL begin radar 4502186 */
operator|&&
name|comptypes
argument_list|(
...
literal|1
condition|)
comment|/* APPLE LOCAL end radar 4502186 */
block|{
if|if
condition|(
operator|!
...
condition|)
block|{
comment|/* APPLE LOCAL begin radar 4193359 */
...
argument_list|)
decl_stmt|;
comment|/* APPLE LOCAL end radar 4193359 */
name|tree
name|mvl
init|=
name|ttl
...
name|error
argument_list|(
literal|"invalid conversion %s integer 'int', expected block pointer"
argument_list|,
name|errtype
operator|==
name|ic_assign
...
name|error
argument_list|(
literal|"incompatible block pointer types %s %qT, expected %qT"
argument_list|,
name|errtype
operator|==
name|ic_assign
...
return|;
block|}
comment|/* APPLE LOCAL begin radar 5831855 */
comment|/* APPLE LOCAL radar 5878380 */
elseif|else
if|if
condition|(
name|codel
...
operator|)
condition|)
comment|/* APPLE LOCAL end radar 5831855 */
return|return
name|convert
argument_list|(
name|type
...
name|error
argument_list|(
literal|"convert_for_assignment: incompatible type for argument %d of %qE"
argument_list|,
name|parmnum
argument_list|,
name|rname
...
comment|/* APPLE LOCAL begin radar 5822844 */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|BLOCK_POINTER_TYPE
condition|)
comment|/* APPLE LOCAL end radar 5822844 */
name|inside_init
operator|=
name|convert_for_assignment
argument_list|(
...
name|error_mark_node
expr_stmt|;
comment|/* APPLE LOCAL begin radar 4188876 */
elseif|else
if|if
condition|(
operator|!
...
name|error
argument_list|(
literal|"Initializer is a non-const vector type"
argument_list|)
expr_stmt|;
name|ret
operator|.
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 4188876 */
else|else
block|{
name|ret
operator|.
...
begin_comment
comment|/** c_finish_block_return_stmt - Utilty routine to figure out block's return  type.  */
end_comment

begin_function
specifier|static
name|tree
name|c_finish_block_return_stmt
parameter_list|(
name|tree
name|retval
parameter_list|)
...
name|valtype
decl_stmt|;
comment|/* If this is the first return we've seen in the block, infer the type of      the block from it. */
if|if
condition|(
name|cur_block
operator|->
...
expr_stmt|;
block|}
comment|/* Verify that this result type matches the previous one.  We are      pickier with blocks than for normal functions because this is a      new feature and we set the rules. */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"void block should not return a value"
argument_list|)
expr_stmt|;
name|retval
operator|=
...
name|error
argument_list|(
literal|"non-void block should return a value"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
return|;
block|}
comment|/* We have a non-void block with an expression, continue checking.  */
name|valtype
operator|=
name|TREE_TYPE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* For now, restrict multiple return statements in a block to have      strict compatible types only. */
if|if
condition|(
operator|!
name|types_are_block_compatible
...
name|error
argument_list|(
literal|"incompatible type returning %qT, expected %qT"
argument_list|,
name|valtype
argument_list|,
name|cur_block
...
name|false
decl_stmt|;
comment|/* APPLE LOCAL radar 5822844 - radar 6185344 */
if|if
condition|(
name|cur_block
operator|&&
...
name|current_function_decl
condition|)
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cn) */
block|{
if|if
condition|(
name|TREE_CODE
...
name|BLOCK_POINTER_TYPE
condition|)
comment|/* APPLE LOCAL radar 6048570 */
name|error
argument_list|(
literal|"returning block that lives on the local stack"
argument_list|)
expr_stmt|;
else|else
name|warning
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cn) */
break|break;
default|default:
break|break;
block|}
...
operator|==
name|POINTER_TYPE
comment|/* APPLE LOCAL radar 5928316 */
operator|||
name|code0
operator|==
name|BLOCK_POINTER_TYPE
...
name|pedwarn
argument_list|(
literal|"comparison of distinct block types lacks a cast"
argument_list|)
expr_stmt|;
else|else
name|pedwarn
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cl) */
elseif|else
if|if
condition|(
name|code0
...
name|type1
expr_stmt|;
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cl) */
break|break;
case|case
name|LE_EXPR
case|:
