...
name|cpp_options
init|=
literal|"%(cpp_unique_options) %1 %{m*} %{std*} %{ansi} %{W*&pedantic*} %{w} %{f*}\  %{O*} %{undef}"
decl_stmt|;
end_decl_stmt
...
name|cc1_options
init|=
literal|"%{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\  %1 %{!Q:-quiet} -dumpbase %B %{d*} %{m*} %{a*}\  -auxbase%{c|S:%{o*:-strip %*}%{!o*: %b}}%{!c:%{!S: %b}}\  %{g*} %{O*} %{W*&pedantic*} %{w} %{std*} %{ansi}\  %{v:-version} %{pg:-p} %{p} %{f*} %{undef}\  %{Qn:-fno-ident} %{--help:--help}\  %{--target-help:--target-help}\  %{!fsyntax-only:%{S:%W{o*}%{!o*:-o %b.s}}}\  %{fsyntax-only:-o %j} %{-param*}"
decl_stmt|;
end_decl_stmt
...
literal|"%{E|M|MM:%(trad_capable_cpp) %(cpp_options) %(cpp_debug_options)}\       %{!E:%{!M:%{!MM:\           %{traditional|ftraditional:\ %eGNU C no longer supports -traditional without -E}\ 	  %{save-temps|traditional-cpp|no-integrated-cpp:%(trad_capable_cpp) \ 		%(cpp_options) %{save-temps:%b.i} %{!save-temps:%g.i} \n\ 		    cc1 -fpreprocessed %{save-temps:%b.i} %{!save-temps:%g.i} %(cc1_options)}\ 	  %{!save-temps:%{!traditional-cpp:%{!no-integrated-cpp:\ 		cc1 %(cpp_unique_options) %(cc1_options)}}}\         %{!fsyntax-only:%(invoke_as)}}}}"
block|,
literal|0
block|}
block|,
...
literal|"@c-header"
block|,
literal|"%{!E:%ecompilation of header file requested} \     %(trad_capable_cpp) %(cpp_options) %(cpp_debug_options)"
block|,
literal|0
block|}
block|,
...
literal|"@assembler-with-cpp"
block|,
literal|"%(trad_capable_cpp) -lang-asm %(cpp_options)\       %{E|M|MM:%(cpp_debug_options)}\       %{!M:%{!MM:%{!E:%{!S:-o %{|!pipe:%g.s} |\n\        as %(asm_debug) %(asm_options) %{!pipe:%g.s} %A }}}}"
block|,
literal|0
block|}
block|,
...
comment|/* Run the each command through valgrind.  To simplifiy prepending the      path to valgrind and the option "-q" (for quiet operation unless      something triggers), we allocate a separate argv array.  */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* If not cross-compiling, look for startfiles in the standard places.      Similarly, don't add the standard prefixes if startfile handling      will be under control of startfile_prefix_spec.  */
if|if
condition|(
operator|*
name|cross_compile
