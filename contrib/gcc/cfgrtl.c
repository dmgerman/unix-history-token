...
literal|0
decl_stmt|;
comment|/* In the case the last instruction is conditional jump to the next      instruction, first redirect the jump itself and then continue      by creating an basic block afterwards to redirect fallthru edge.  */
if|if
condition|(
name|e
operator|->
...
name|end
argument_list|)
comment|/* When called from cfglayout, fallthru edges do not          neccessarily go to the next block.  */
operator|&&
name|e
operator|->
name|src
...
name|error
argument_list|(
literal|"verify_flow_info: REG_BR_PROB does not match cfg %i %i"
argument_list|,
name|INTVAL
argument_list|(
name|XEXP
