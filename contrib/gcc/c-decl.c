...
expr_stmt|;
block|}
comment|/* duplicate_decls might write to TREE_PUBLIC (x) and DECL_EXTERNAL (x) 	 to make it identical to the initial declaration.  */
name|declared_global
operator|=
name|TREE_PUBLIC
argument_list|(
...
name|name
argument_list|)
comment|/* should this be '&& ! declared_global' ?  */
operator|&&
operator|!
name|TREE_PUBLIC
argument_list|(
...
comment|/* A test for TREE_PUBLIC (x) will fail for variables that have 	     been declared static first, and extern now.  */
if|if
condition|(
operator|!
name|declared_global
...
argument_list|)
expr_stmt|;
comment|/* For C and Objective-C, we by default put things in .common when      possible.  */
name|DECL_COMMON
argument_list|(
name|decl
argument_list|)
...
comment|/* We must use DECL_CONTEXT instead of current_binding_level, 		 because a duplicate_decls call could have changed the binding 		 level of this decl.  */
operator|(
name|DECL_INITIAL
argument_list|(
name|decl
...
name|warning
argument_list|(
literal|"cannot make `%s' a transparent union"
argument_list|)
expr_stmt|;
block|}
