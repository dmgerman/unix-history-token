...
condition|)
block|{
comment|/* If both decls are in the same TU and the new declaration 		 isn't overriding an extern inline reject the new decl. 		 When we handle c99 style inline rules we'll want to reject 		 the following:  		 DECL_EXTERN_INLINE (olddecl)&& !DECL_EXTERN_INLINE (newdecl)  		 if they're in the same translation unit. Until we implement 		 the full semantics we accept the construct.  */
if|if
condition|(
operator|!
operator|(
...
expr_stmt|;
block|}
comment|/* Inline declaration after use or definition. 	 ??? Should we still warn about this now we have unit-at-a-time 	 mode and can get it right? 	 Definitely don't complain if the decls are in different translation 	 units.  */
if|if
condition|(
name|DECL_DECLARED_INLINE_P
argument_list|(
...
literal|0
argument_list|,
literal|"C99 inline functions are not supported; using GNU89"
argument_list|)
expr_stmt|;
if|if
condition|(
...
literal|0
argument_list|,
literal|"to disable this warning use -fgnu89-inline or "
literal|"the gnu_inline function attribute"
argument_list|)
expr_stmt|;
name|info
operator|=
