...
specifier|static
name|tree
name|build_bitfield_integer_type
name|PARAMS
argument_list|(
operator|(
name|tree
...
begin_comment
comment|/* Nonzero means to treat bit-fields as signed unless they say `unsigned'.  */
end_comment
...
begin_comment
comment|/* Decode the string P as a language-specific option for C.    Return the number of strings consumed.  Should not complain    if it does not recognise the option.  */
end_comment
...
name|warning
argument_list|(
literal|"-traditional is deprecated and may be removed"
argument_list|)
expr_stmt|;
name|flag_traditional
operator|=
...
expr_stmt|;
block|}
comment|/* Keep count of variables in this level with incomplete type. 	 If the input is erroneous, we can have error_mark in the type 	 slot (e.g. "f(void a, ...)") - that doesn't count as an 	 incomplete type.  */
if|if
condition|(
name|TREE_TYPE
argument_list|(
...
expr_stmt|;
comment|/* With GCC, C99's _Bool is always of size 1.  */
...
name|pedwarn
argument_list|(
literal|"ISO C89 does not support `static' or type qualifiers in parameter array declarators"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|pedwarn
argument_list|(
literal|"ISO C89 does not support `[*]' array declarators"
argument_list|)
expr_stmt|;
block|}
if|if
...
condition|)
block|{
comment|/* This decl needs a name for the assembler output.  We also need 	 a unique suffix to be added to the name, for which DECL_CONTEXT 	 must be set.  */
name|DECL_NAME
argument_list|(
name|decl
argument_list|)
...
begin_comment
comment|/* A bit-field NAME should have an integer type whose precision    accurately reflects its WIDTH.  If TYPE is good for that, return    it, otherwise create and return the appropriate type.     This routine also performs sanity checks on the bit-field's type    and width, and uses appropriate values if they are invalid.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* The type of a bit-field should have precision the same as the      bit-field's width.  */
if|if
condition|(
name|w
operator|!=
...
name|pedwarn
argument_list|(
literal|"ISO C89 does not support `long long'"
argument_list|)
expr_stmt|;
name|longlong
operator|=
...
operator|)
name|RID_UNSIGNED
comment|/* Traditionally, all bit-fields are unsigned.  */
operator|||
operator|(
name|bitfield
operator|&&
...
name|pedwarn
argument_list|(
literal|"ISO C89 does not support complex types"
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/* A bit-field needs its type to have precision equal to its width,      rather than the precision of the specified standard type.  */
if|if
condition|(
name|bitfield
condition|)
...
name|pedwarn
argument_list|(
literal|"ISO C89 forbids array `%s' whose size can't be evaluated"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
name|pedwarn
argument_list|(
literal|"ISO C89 forbids variable-size array `%s'"
argument_list|,
name|name
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* ??? Need to check somewhere that this is a structure 		 and not a union, that this field is last, and that 		 this structure has at least one other named member.  */
if|if
condition|(
name|pedantic
operator|&&
...
name|pedwarn
argument_list|(
literal|"ISO C89 does not support flexible array members"
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Bit-field width 0 => force desired amount of alignment.  */
if|if
condition|(
name|TYPE_PRECISION
argument_list|(
...
condition|)
block|{
comment|/* This is an unnamed decl.  We only support unnamed 	 structs/unions, so check for other things and refuse them.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|error
argument_list|(
literal|"unnamed fields of type other than struct or union are not allowed"
argument_list|)
expr_stmt|;
return|return
name|NULL_TREE
...
expr_stmt|;
block|}
comment|/* Install struct as DECL_CONTEXT of each field decl.  */
name|saw_named_field
operator|=
literal|0
expr_stmt|;
...
begin_comment
comment|/* Generate the RTL for the body of FNDECL.  If NESTED_P is non-zero,    then we are already in the process of generating RTL for another    function.  If can_defer_p is zero, we won't attempt to defer the    generation of RTL.  */
end_comment
...
condition|)
block|{
comment|/* Stop pointing to the local nodes about to be freed. 	 But DECL_INITIAL must remain nonzero so we know this 	 was an actual function definition. 	 For a nested function, this is done in pop_c_function_context. 	 If rest_of_compilation set this to 0, leave it 0.  */
if|if
condition|(
name|DECL_INITIAL
argument_list|(
...
begin_comment
comment|/* Mark the language specific parts of F for GC.  */
end_comment
...
begin_comment
comment|/* Copy the DECL_LANG_SPECIFIC data associated with NODE.  */
end_comment
...
begin_comment
comment|/* Mark the language specific bits in T for GC.  */
end_comment
