...
name|BLOCKDEF
block|,
comment|/* Block literal declaration */
name|PARM
block|,
...
begin_comment
comment|/* APPLE LOCAL begin blocks 6339747 */
end_comment

begin_comment
comment|/* Decode a block literal type, such as "int **", returning a ...FUNCTION_DECL node.  */
end_comment
...
begin_comment
comment|/* APPLE LOCAL end blocks 6339747 */
end_comment
...
begin_comment
comment|/* APPLE LOCAL begin radar 5932809 - copyable byref blocks (C++ cr) */
end_comment

begin_define
define|#
directive|define
name|BLOCK_ALIGN_MAX
value|18
end_define
...
specifier|static
name|tree
name|block_byref_id_object_copy
index|[
...
specifier|static
name|tree
name|block_byref_id_object_dispose
index|[
name|BLOCK_BYREF_CURRENT_MAX
operator|*
operator|(
...
argument_list|)
expr_stmt|;
comment|/* Set up: (void* _dest, void*_src) parameters. */
name|dst_arg
operator|=
name|build_decl
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* function header synthesis. */
...
argument_list|)
expr_stmt|;
comment|/* src_obj is: _src->object. */
name|src_obj
operator|=
name|build_indirect_object_id_exp
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 6180456 */
comment|/* _Block_object_assign (&_dest->object, _src->object, BLOCK_FIELD_IS_OBJECT) or :      _Block_object_assign (&_dest->object, _src->object, BLOCK_FIELD_IS_BLOCK) */
comment|/* APPLE LOCAL begin radar 6573923 */
comment|/* Also add the new flag when calling _Block_object_dispose      from byref dispose helper. */
name|flag
operator||=
name|BLOCK_BYREF_CALLER
expr_stmt|;
comment|/* APPLE LOCAL end radar 6573923 */
name|call_exp
operator|=
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radar 6180456 */
name|fnbody
operator|=
...
begin_comment
comment|/**   This routine builds:    void __Block_byref_id_object_dispose(struct Block_byref_id_object *_src) {     _Block_object_dispose(_src->object, BLOCK_FIELD_IS_OBJECT[|BLOCK_FIELD_IS_WEAK]) // object     _Block_object_dispose(_src->object, BLOCK_FIELD_IS_BLOCK[|BLOCK_FIELD_IS_WEAK]) // block   }  */
end_comment

begin_function
specifier|static
name|void
name|synth_block_byref_id_object_dispose_func
parameter_list|(
name|int
name|flag
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* Set up: (void *_src) parameter. */
name|src_arg
operator|=
name|build_decl
argument_list|(
...
expr_stmt|;
comment|/* APPLE LOCAL begin radar 6180456 */
comment|/* _Block_object_dispose(_src->object, BLOCK_FIELD_IS_OBJECT) : or      _Block_object_dispose(_src->object, BLOCK_FIELD_IS_BLOCK) */
comment|/* APPLE LOCAL begin radar 6573923 */
comment|/* Also add the new flag when calling _Block_object_dispose      from byref dispose helper. */
name|flag
operator||=
...
begin_comment
comment|/* new_block_byref_decl - This routine changes a 'typex x' declared variable into:    struct __Block_byref_x {     // APPLE LOCAL radar 6244520     void *__isa;			// NULL for everything except __weak pointers     struct Block_byref_x *__forwarding;     int32_t __flags;     int32_t __size;     void *__ByrefKeepFuncPtr;    // Only if variable is __block ObjC object     void *__ByrefDestroyFuncPtr; // Only if variable is __block ObjC object     typex x;   } x; */
end_comment

begin_function
specifier|static
name|tree
name|new_block_byref_decl
parameter_list|(
name|tree
name|decl
parameter_list|)
...
name|int
name|save_flag_objc_gc
decl_stmt|;
name|tree
name|Block_byref_type
decl_stmt|;
name|tree
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 6244520 */
comment|/* void *__isa; */
name|field_decl
operator|=
name|build_decl
...
name|field_decl
expr_stmt|;
comment|/* APPLE LOCAL end radar 6244520 */
comment|/* struct Block_byref_x *__forwarding; */
name|field_decl
operator|=
name|build_decl
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 6244520 */
name|chainon
argument_list|(
name|field_decl_chain
argument_list|,
...
condition|)
block|{
comment|/* void *__ByrefKeepFuncPtr; */
name|field_decl
operator|=
name|build_decl
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* void *__ByrefDestroyFuncPtr; */
name|field_decl
operator|=
name|build_decl
argument_list|(
...
comment|/* Hack so we don't issue warning on a field_decl having __weak attribute */
name|save_flag_objc_gc
operator|=
name|flag_objc_gc
expr_stmt|;
...
name|Block_byref_type
expr_stmt|;
comment|/* Force layout_decl to recompute these fields. */
name|DECL_SIZE
argument_list|(
name|decl
argument_list|)
...
begin_comment
comment|/* init_byref_decl - This routine builds the initializer for the __Block_byref_x    type in the form of:    { NULL,&x, 0, sizeof(struct __Block_byref_x), initializer-expr};     or:    { NULL,&x, 0, sizeof(struct __Block_byref_x)};    when INIT is NULL_TREE     For __block ObjC objects, it also adds "byref_keep" and "byref_destroy"    Funtion pointers. So the most general initializers would be:     { NULL,&x, 0, sizeof(struct __Block_byref_x),&byref_keep,&byref_destroy,&initializer-expr}; */
end_comment
...
name|error
argument_list|(
literal|"invalid alignment for __block variable"
argument_list|)
expr_stmt|;
name|kind
operator|=
...
name|flag
expr_stmt|;
comment|/* Add&__Block_byref_id_object_copy,&__Block_byref_id_object_dispose 	 initializers. */
if|if
condition|(
operator|!
name|block_byref_id_object_copy
...
condition|)
block|{
comment|/* Build a void __Block_byref_id_object_copy(void*, void*) type. */
name|tree
name|func_type
init|=
name|build_function_type
...
name|name
argument_list|,
literal|"__Block_byref_id_object_copy%d"
argument_list|,
name|kind
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Synthesize function definition. */
name|synth_block_byref_id_object_copy_func
argument_list|(
name|flag
argument_list|,
...
condition|)
block|{
comment|/* Synthesize void __Block_byref_id_object_dispose (void*) and 	     build&__Block_byref_id_object_dispose. */
name|tree
name|func_type
init|=
name|build_function_type
...
name|name
argument_list|,
literal|"__Block_byref_id_object_dispose%d"
argument_list|,
name|kind
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* APPLE LOCAL end radar 5932809 - copyable byref blocks (C++ cr) */
end_comment
...
literal|0
expr_stmt|;
comment|/* APPLE LOCAL begin radar 5932809 - copyable byref blocks (C++ cq) */
comment|/* We build a new type for each local variable declared as __block      and initialize it to a list of initializers. */
elseif|else
if|if
condition|(
name|TREE_CODE
...
name|error
argument_list|(
literal|"__block attribute on %q+D not allowed, only allowed on local variables"
argument_list|,
name|decl
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 6289031 */
if|#
directive|if
literal|0
block|if (! flag_objc_gc_only)
endif|#
directive|endif
block|{
name|push_cleanup
...
expr_stmt|;
block|}
comment|/* APPLE LOCAL end radar 6289031 */
comment|/* APPLE LOCAL begin radar 5847976 */
...
block|}
block|}
comment|/* APPLE LOCAL end radar 5932809 - copyable byref blocks (C++ cq) */
if|if
condition|(
name|init
condition|)
...
break|break;
block|}
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ cj) */
case|case
name|cdk_block_pointer
case|:
block|{
...
name|error
argument_list|(
literal|"block pointer to non-function type is invalid"
argument_list|)
expr_stmt|;
name|type
operator|=
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radar 5814025 (C++ cj) */
comment|/* Process type qualifiers (such as const or volatile) 		   that were given inside the `^'.  */
name|type_quals
operator|=
name|declarator
...
name|pointer_quals
expr_stmt|;
comment|/* APPLE LOCAL end radar 5814025 (C++ cj) */
name|declarator
operator|=
name|declarator
operator|->
...
break|break;
block|}
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ cj) */
default|default:
name|gcc_unreachable
argument_list|()
expr_stmt|;
...
name|error
argument_list|(
literal|"block declared as returning an array"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
expr_stmt|;
break|break;
comment|/* APPLE LOCAL begin radar 6268817 */
case|case
name|FUNCTION_DECL
case|:
comment|/* Block helper function can be declared in the statement block 	 for the for-loop declarations. */
if|if
condition|(
...
condition|)
break|break;
comment|/* APPLE LOCAL end radar 6268817 */
default|default:
name|error
argument_list|(
...
begin_comment
comment|/* APPLE LOCAL begin radar 5932809 - copyable byref blocks (C++ ch) */
end_comment

begin_comment
comment|/* build_byref_local_var_access - converts EXPR to:    EXPR.__forwarding-><decl-name>. */
end_comment
...
begin_comment
comment|/* APPLE LOCAL end radar 5932809 - copyable byref blocks (C++ ch) */
end_comment

begin_comment
comment|/* APPLE LOCAL begin radar 5732232 - blocks (C++ ch) */
end_comment

begin_comment
comment|/**   build_block_byref_decl - This routine inserts a variable declared as a   'byref' variable using the |...| syntax in helper function's outer-most scope. */
end_comment
...
name|byref_decl
decl_stmt|;
comment|/* APPLE LOCAL begin radar 6225809 */
if|if
condition|(
name|cur_block
operator|->
...
condition|)
block|{
comment|/* Traverse enclosing blocks. Insert a __block variable in      each enclosing block which has no declaration of this      variable. This is to ensure that the current (inner) block      gets the __block version of the variable; */
name|struct
...
argument_list|)
decl_stmt|;
comment|/* Find the first declaration not in current block. */
while|while
condition|(
name|b
operator|&&
...
name|shadowed
expr_stmt|;
comment|/* Is the next declaration not in enclosing block? */
if|if
condition|(
name|b
operator|&&
...
condition|)
block|{
comment|/* No declaration of variable seen in the block. Must insert one. */
name|struct
name|c_scope
modifier|*
name|save_scope
...
block|}
block|}
comment|/* APPLE LOCAL end radar 6225809 */
comment|/* If it is already a byref declaration, do not add the pointer type      because such declarations already have the pointer type      added. This happens when we have two nested byref declarations in      nested blocks. */
name|ptr_type
operator|=
operator|(
name|TREE_CODE
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL begin radars 6144664& 6145471  */
name|DECL_SOURCE_LOCATION
argument_list|(
name|byref_decl
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL end radars 6144664& 6145471  */
name|BLOCK_DECL_BYREF
argument_list|(
name|byref_decl
argument_list|)
...
block|}
comment|/* APPLE LOCAL end radar 5932809 - copyable byref blocks (C++ ch) */
comment|/* In the presence of nested "{"       move up the scope chain until reaching the main function body's scope.   */
while|while
condition|(
name|scope
operator|&&
...
name|outer
expr_stmt|;
comment|/* Current scope must be that of the main function body. */
name|gcc_assert
argument_list|(
name|scope
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL radar 5847213 - building block_original_byref_decl_list list removed. */
comment|/* APPLE LOCAL begin radar 6289031 */
if|#
directive|if
literal|0
...
begin_comment
comment|/**   build_block_ref_decl - This routine inserts a copied-in variable (a variable   referenced in the block but whose scope is outside the block) in helper   function's outer-most scope. It also sets its type to 'const' as such   variables are read-only. */
end_comment
...
decl_stmt|;
comment|/* APPLE LOCAL begin radar 5932809 - copyable byref blocks (C++ ch) */
comment|/* 'decl' was previously declared as __block. Simply, copy the value      embedded in the above variable. */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
block|{
comment|/* APPLE LOCAL begin radar 5988451 (C++ ch) */
if|if
condition|(
name|cur_block
operator|->
...
condition|)
block|{
comment|/* Traverse enclosing blocks. Insert a copied-in variable in 	  each enclosing block which has no declaration of this 	  variable. This is to ensure that the current (inner) block 	  has the 'frozen' value of the copied-in variable; which means 	  the value of the copied in variable is at the point of the 	  block declaration and *not* when the inner block is 	  invoked.  */
name|struct
name|block_sema_info
modifier|*
name|cb
...
condition|)
block|{
comment|/* No declaration of variable seen in the block. Must insert one, 	      so it 'freezes' the variable in this block. */
name|struct
name|c_scope
modifier|*
name|save_scope
...
block|}
block|}
comment|/* APPLE LOCAL end radar 5988451 (C++ ch) */
block|}
comment|/* APPLE LOCAL end radar 5932809 - copyable byref blocks (C++ ch) */
...
name|error_mark_node
expr_stmt|;
comment|/* APPLE LOCAL radar 5805175 - blocks (C++ ch) */
name|c_apply_type_quals_to_decl
argument_list|(
name|TYPE_QUAL_CONST
argument_list|,
...
literal|1
expr_stmt|;
comment|/* Find the scope for function body (outer-most scope) and insert      this variable in that scope. This is to avoid duplicate      declaration of the save variable. */
while|while
condition|(
name|scope
operator|&&
...
name|outer
expr_stmt|;
comment|/* We are enterring the copied-in variable in helper function's      outer scope; that of its main body. */
name|gcc_assert
argument_list|(
name|scope
argument_list|)
...
name|get_identifier
argument_list|(
literal|"__block_descriptor_withcopydispose"
argument_list|)
argument_list|)
else|:
name|start_struct
...
name|get_identifier
argument_list|(
literal|"__block_descriptor"
argument_list|)
argument_list|)
expr_stmt|;
comment|/* unsigned long int reserved; */
name|field_decl
operator|=
name|build_decl
argument_list|(
...
name|field_decl
expr_stmt|;
comment|/* unsigned long int Size; */
name|field_decl
operator|=
name|build_decl
argument_list|(
...
expr_stmt|;
block|}
comment|/* Mark this struct as being a block struct rather than a 'normal'       struct.  */
...
begin_comment
comment|/* APPLE LOCAL begin radar 5814025 (C++ ch) */
end_comment
...
name|cdk_block_pointer
expr_stmt|;
comment|/* APPLE LOCAL radar 5882266 (C++ ch) */
name|ret
operator|->
name|declarator
operator|=
...
begin_comment
comment|/* APPLE LOCAL end radar 5814025 (C++ ch) */
end_comment
...
parameter_list|)
block|{
comment|/* APPLE LOCAL radar 5988451 (C++ ch) */
return|return
operator|(
name|cur_block
operator|&&
...
name|true
return|;
comment|/* Check for common case of block nested inside a non-block. */
if|if
condition|(
operator|!
name|cur_block
...
name|false
return|;
comment|/* Check for less common case of nested blocks. */
comment|/* Declaration not in current block. Find the first user 	 declaration of 'name' in outer scope. */
comment|/* APPLE LOCAL begin radar 5988451 (C++ ch) */
comment|/* Check for variables only, as we may have parameters, such as 	 'self' */
comment|/* Note that if a copied-in variable (BLOCK_DECL_COPIED) in the 	  enclosing block is found, it must be returned as this is 	  where the variable in current (nested block) will have to get 	  its value. */
while|while
condition|(
name|b
operator|&&
...
specifier|static
name|tree
name|save_current_function_decl
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/**   build_helper_func_decl - This routine builds a FUNCTION_DECL for   a block helper function. */
end_comment
...
begin_comment
comment|/**  start_block_helper_function - This is a light-weight version of start_function().  It has removed all the fuss in the start_function().  */
end_comment
...
literal|1
expr_stmt|;
comment|/* A helper function is not global */
name|TREE_PUBLIC
argument_list|(
name|decl1
argument_list|)
...
begin_comment
comment|/**  declare_block_prologue_local_vars - utility routine to do the actual  declaration and initialization for each referecned block variable. */
end_comment

begin_function
specifier|static
name|void
name|declare_block_prologue_local_vars
parameter_list|(
name|tree
name|self_parm
parameter_list|,
...
name|block_component
expr_stmt|;
comment|/* Prepend a DECL_EXPR statement to the statement list. */
name|i
operator|=
name|tsi_start
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* APPLE LOCAL Radar 5811961, Fix location of block prologue vars (C++ ch) */
name|SET_EXPR_LOCATION
argument_list|(
name|decl_stmt
argument_list|,
...
comment|/* Give the prologue statements a line number of one before the beginning of      the function, to make them easily identifiable later.  */
name|EXPR_LINENO
argument_list|(
name|decl_stmt
argument_list|)
...
begin_comment
comment|/**  block_build_prologue  - This routine builds the declarations for the  variables referenced in the block; as in:  int *y = .block_descriptor->y;  int x = .block_descriptor->x;   The decl_expr declaration for each initialization is enterred at the  beginning of the helper function's statement-list which is passed  in block_impl->block_body. */
end_comment
...
name|chain
decl_stmt|;
comment|/* APPLE LOCAL radar 6404979 */
name|tree
name|self_parm
init|=
name|lookup_name
...
begin_comment
comment|/* APPLE LOCAL end radar 5732232 - blocks (C++ ch) */
end_comment
