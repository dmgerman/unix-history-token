...
directive|ifdef
name|__DJGPP__
comment|/* For DJGPP redirected input is opened in text mode. Change it          to binary mode.  */
if|if
condition|(
operator|!
name|isatty
...
name|perror_fail
goto|;
comment|/* We must tell Valgrind that the byte at buf[size] is actually 	     readable.  Discard the handle to avoid handle leak.  */
name|VALGRIND_DISCARD
argument_list|(
...
condition|)
block|{
comment|/* Undo the previous annotation for the 	     known-zero-byte-after-mmap.  Discard the handle to avoid 	     handle leak.  */
name|VALGRIND_DISCARD
argument_list|(
...
begin_comment
comment|/* Create a dependency for file FNAME, or issue an error message as    appropriate.  ANGLE_BRACKETS is nonzero if the file was bracketed    like<..>.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If -M was specified, then don't count this as an error, because      we can still produce correct output.  Otherwise, we can't produce      correct output, because there may be dependencies we need inside      the missing file, and we don't know what directory this missing      file exists in.  */
else|else
name|cpp_errno
argument_list|(
name|pfile
