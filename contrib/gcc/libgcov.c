...
begin_comment
comment|/* Size of the longest file name. */
end_comment

begin_decl_stmt
specifier|static
name|size_t
name|gcov_max_filename
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Make sure path component of the given FILENAME exists, create     missing directories. FILENAME must be writable.     Returns zero on success, or -1 if an error occurred.  */
end_comment

begin_function
specifier|static
name|int
name|create_file_directory
parameter_list|(
name|char
modifier|*
name|filename
...
literal|'\0'
expr_stmt|;
comment|/* Try to make directory if it doesn't already exist.  */
if|if
condition|(
name|access
argument_list|(
...
operator|-
literal|1
comment|/* The directory might have been made by another process.  */
operator|&&
name|errno
operator|!=
name|EEXIST
...
name|stderr
argument_list|,
literal|"profiling:%s:Cannot create directory\n"
argument_list|,
name|filename
argument_list|)
expr_stmt|;
...
begin_comment
comment|/* Check if VERSION of the info block PTR matches libgcov one.    Return 1 on success, or zero in case of versions mismatch.    If FILENAME is not NULL, its value used for reporting purposes     instead of value from the info block.  */
end_comment
...
block|}
block|}
comment|/* Get file name relocation prefix.  Non-absolute values are ignored. */
name|gcov_prefix
operator|=
name|getenv
argument_list|(
...
condition|)
block|{
comment|/* Check if the level of dirs to strip off specified. */
name|char
modifier|*
name|tmp
init|=
...
argument_list|)
expr_stmt|;
comment|/* Do not consider negative values. */
if|if
condition|(
name|gcov_prefix_strip
operator|<
...
argument_list|)
expr_stmt|;
comment|/* Remove an unnecessary trailing '/' */
if|if
condition|(
name|IS_DIR_SEPARATOR
argument_list|(
...
literal|0
expr_stmt|;
comment|/* Allocate and initialize the filename scratch space.  */
name|gi_filename
operator|=
operator|(
name|char
...
argument_list|)
expr_stmt|;
comment|/* Build relocated filename, stripping off leading           directories from the initial filename if requested. */
if|if
condition|(
name|gcov_prefix_strip
operator|>
...
name|s
decl_stmt|;
comment|/* Skip selected directory levels. */
for|for
control|(
name|s
operator|=
...
block|}
empty_stmt|;
comment|/* Update complete filename with stripped original. */
name|strcpy
argument_list|(
name|gi_filename_up
argument_list|,
...
directive|ifdef
name|TARGET_POSIX_IO
comment|/* Open failed likely due to missed directory. 	     Create directory and retry to open file. */
if|if
condition|(
name|create_file_directory
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* Refresh the longest file name information */
if|if
condition|(
name|filename_length
operator|>
...
ifdef|#
directive|ifdef
name|L_gcov_interval_profiler
end_ifdef

begin_comment
comment|/* If VALUE is in interval<START, START + STEPS - 1>, then increases the    corresponding counter in COUNTERS.  If the VALUE is above or below    the interval, COUNTERS[STEPS] or COUNTERS[STEPS + 1] is increased    instead.  */
end_comment
...
ifdef|#
directive|ifdef
name|L_gcov_pow2_profiler
end_ifdef

begin_comment
comment|/* If VALUE is a power of two, COUNTERS[1] is incremented.  Otherwise    COUNTERS[0] is incremented.  */
end_comment
...
ifdef|#
directive|ifdef
name|L_gcov_one_value_profiler
end_ifdef

begin_comment
comment|/* Tries to determine the most common value among its inputs.  Checks if the    value stored in COUNTERS[0] matches VALUE.  If this is the case, COUNTERS[1]    is incremented.  If this is not the case and COUNTERS[1] is not zero,    COUNTERS[1] is decremented.  Otherwise COUNTERS[1] is set to one and    VALUE is stored to COUNTERS[0].  This algorithm guarantees that if this    function is called more than 50% of the time with one value, this value    will be in COUNTERS[0] in the end.     In any case, COUNTERS[2] is incremented.  */
end_comment

begin_function
name|void
name|__gcov_one_value_profiler
parameter_list|(
name|gcov_type
modifier|*
name|counters
...
begin_comment
comment|/* A wrapper for the fork function.  Flushes the accumulated profiling data, so    that they are not counted twice.  */
end_comment
...
begin_comment
comment|/* A wrapper for the execl function.  Flushes the accumulated profiling data, so    that they are not lost.  */
end_comment
...
begin_comment
comment|/* A wrapper for the execlp function.  Flushes the accumulated profiling data, so    that they are not lost.  */
end_comment
...
begin_comment
comment|/* A wrapper for the execle function.  Flushes the accumulated profiling data, so    that they are not lost.  */
end_comment
...
begin_comment
comment|/* A wrapper for the execv function.  Flushes the accumulated profiling data, so    that they are not lost.  */
end_comment
...
begin_comment
comment|/* A wrapper for the execvp function.  Flushes the accumulated profiling data, so    that they are not lost.  */
end_comment
...
begin_comment
comment|/* A wrapper for the execve function.  Flushes the accumulated profiling data, so    that they are not lost.  */
end_comment
