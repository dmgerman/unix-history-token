...
name|tree_pruner
decl_stmt|;
comment|/* Decl of function we are inlining into.  */
name|tree
name|decl
decl_stmt|;
name|tree
...
return|;
block|}
comment|/* We do need a copy.  build and register it now.  */
name|new
operator|=
name|copy_node
argument_list|(
...
expr_stmt|;
block|}
comment|/* Local variables and labels need to be replaced by equivalent      variables.  We don't want to copy static variables; there's only      one of those, no matter how many times we inline the containing      function.      We do not also want to copy the label which we put into      GOTO_STMT which replaced RETURN_STMT.  */
elseif|else
if|if
condition|(
operator|*
...
condition|)
block|{
comment|/* Get rid of&* from inline substitutions.  It can occur when 	     someone takes the address of a parm or return slot passed by 	     invisible reference.  */
name|tree
name|decl
init|=
name|TREE_OPERAND
...
name|N_
argument_list|(
literal|"%Jfunction '%F' can never be inlined because it uses setjmp"
argument_list|)
expr_stmt|;
return|return
name|node
...
name|N_
argument_list|(
literal|"%Jfunction '%F' can never be inlined because "
...
name|N_
argument_list|(
literal|"%Jfunction '%F' can never be inlined "
literal|"because it contains a nested function"
argument_list|)
expr_stmt|;
return|return
name|node
...
argument_list|(
literal|"%Jfunction '%F' can never be inlined "
literal|"because it contains a nonlocal goto"
argument_list|)
expr_stmt|;
return|return
name|node
...
name|sorry
argument_list|(
literal|"%Jinlining failed in call to '%F': %s"
argument_list|,
name|fn
argument_list|,
name|fn
...
directive|endif
comment|/* INLINER_FOR_JAVA */
comment|/* After the body of the function comes the RET_LABEL.  This must come      before we evaluate the returned value below, because that evaluation      may cause RTL to be generated.  */
ifndef|#
directive|ifndef
name|INLINER_FOR_JAVA
name|COMPOUND_BODY
