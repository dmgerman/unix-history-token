...
specifier|static
name|tree
name|find_builtin_longjmp_call_1
name|PARAMS
argument_list|(
operator|(
name|tree
...
specifier|static
name|tree
name|find_builtin_longjmp_call
name|PARAMS
argument_list|(
operator|(
name|tree
...
name|new_var
decl_stmt|;
comment|/* All local class initialization flags go in the outermost 	 scope.  */
if|if
condition|(
name|LOCAL_CLASS_INITIALIZATION_FLAG_P
argument_list|(
name|old_var
argument_list|)
condition|)
block|{
comment|/* We may already have one.  */
if|if
condition|(
operator|!
name|splay_tree_lookup
...
condition|)
block|{
comment|/* We need a new copy of this labeled block; the EXIT_BLOCK_EXPR          will refer to it, so save a copy ready for remapping.  We          save it in the decl_map, although it isn't a decl.  */
name|tree
name|new_block
init|=
name|copy_node
...
argument_list|)
decl_stmt|;
comment|/* We _must_ have seen the enclosing LABELED_BLOCK_EXPR.  */
if|if
condition|(
operator|!
name|n
...
argument_list|)
expr_stmt|;
comment|/* If this is a constant, make sure it has the right type.  */
elseif|else
if|if
condition|(
name|TREE_TYPE
...
block|{
comment|/* Java objects don't ever need constructing when being              passed as arguments because only call by reference is              supported.  */
name|abort
argument_list|()
expr_stmt|;
block|}
...
begin_comment
comment|/* If *TP is possibly call to alloca, return nonzero.  */
end_comment
...
begin_comment
comment|/* Returns nonzero if FN is a function that can be inlined into the    inlining context ID_.  If ID_ is NULL, check whether the function    can be inlined at all.  */
end_comment
...
literal|0
expr_stmt|;
comment|/* We may be here either because fn is declared inline or because      we use -finline-functions.  For the second case, we are more      restrictive.  */
if|if
condition|(
...
expr_stmt|;
comment|/* The number of instructions (estimated) of current function.  */
name|currfn_insns
operator|=
name|DECL_NUM_STMTS
argument_list|(
...
directive|ifdef
name|INLINER_FOR_JAVA
comment|/* Synchronized methods can't be inlined.  This is a bug.  */
elseif|else
if|if
condition|(
name|METHOD_SYNCHRONIZED
argument_list|(
name|fn
argument_list|)
condition|)
...
directive|endif
comment|/* INLINER_FOR_JAVA */
comment|/* We can't inline functions that are too big.  Only allow a single      function to be of MAX_INLINE_INSNS_SINGLE size.  Make special      allowance for extern inline functions, though.  */
elseif|else
if|if
condition|(
operator|!
...
condition|)
empty_stmt|;
comment|/* We can't inline functions that call __builtin_longjmp at all.      The non-local goto machenery really requires the destination      be in a different function.  If we allow the function calling      __builtin_longjmp to be inlined into the function calling      __builtin_setjmp, Things will Go Awry.  */
comment|/* ??? Need front end help to identify "regular" non-local goto.  */
elseif|else
if|if
condition|(
name|find_builtin_longjmp_call
...
name|inlinable
expr_stmt|;
comment|/* In case we don't disregard the inlining limits and we basically      can inline this function, investigate further.  */
if|if
condition|(
operator|!
call|(
...
name|currfn_insns
decl_stmt|;
comment|/* In the extreme case that we have exceeded the recursive inlining          limit by a huge factor (128), we just say no. Should not happen          in real life.  */
if|if
condition|(
name|sum_insns
operator|>
...
literal|0
expr_stmt|;
comment|/* If we did not hit the extreme limit, we use a linear function          with slope -1/MAX_INLINE_SLOPE to exceedingly decrease the          allowable size. We always allow a size of MIN_INLINE_INSNS          though.  */
elseif|else
if|if
condition|(
name|sum_insns
...
condition|)
block|{
comment|/* We're walking the subtree directly.  */
operator|*
name|walk_subtrees
operator|=
literal|0
expr_stmt|;
comment|/* Update the source position.  */
name|push_srcloc
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Restore the original source position.  */
name|pop_srcloc
argument_list|()
expr_stmt|;
return|return
...
name|new_body
decl_stmt|;
name|java_inlining_map_static_initializers
argument_list|(
name|fn
argument_list|,
name|id
...
condition|)
block|{
comment|/* Mention the retvar.  If the return type of the function was 	 promoted, convert it back to the expected type.  */
if|if
condition|(
name|TREE_TYPE
argument_list|(
...
expr_stmt|;
block|}
name|java_inlining_merge_static_initializers
argument_list|(
name|fn
argument_list|,
name|id
...
argument_list|)
expr_stmt|;
comment|/* For accounting, subtract one for the saved call/ret.  */
name|id
operator|->
name|inlined_stmts
operator|+=
...
begin_comment
comment|/* Add STMT to EXISTING if possible, otherwise create a new    COMPOUND_EXPR and add STMT to it.  */
end_comment
