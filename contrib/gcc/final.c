...
operator|)
expr_stmt|;
comment|/* ??? This _really_ ought to be done with a structure layout 	 and with assemble_constructor.  If long_bytes != pointer_bytes 	 we'll be emitting unaligned data at some point.  */
if|if
condition|(
name|long_bytes
operator|!=
...
block|}
block|}
comment|/* In case block is frequent and reached mostly by non-fallthru edge, 	 align it.  It is most likely an first block of loop.  */
if|if
condition|(
name|has_fallthru
operator|&&
...
comment|/* Output label after the prologue of the function.  */
if|if
condition|(
name|write_symbols
operator|==
...
directive|ifdef
name|STACK_REGS
comment|/* Irritatingly, the reg-stack pass is creating new instructions 	     and because of REG_DEAD note abuse it has to run after 	     shorten_branches.  Fake address of -1 then.  */
name|insn_current_address
operator|=
operator|-
literal|1
...
begin_comment
comment|/* Return 1 if branch is an forward branch.    Uses insn_shuid array, so it works only in the final pass.  May be used by    output templates to customary add branch prediction hints.  */
end_comment
