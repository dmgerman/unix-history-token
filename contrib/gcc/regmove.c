...
begin_comment
comment|/* Move registers around to reduce number of move instructions needed.    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997, 1998,    1999, 2000, 2001, 2002, 2003, 2004, 2005 Free Software Foundation, Inc.  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
condition|)
block|{
comment|/* If there is a REG_DEAD note on this insn, we must 		     change this not to REG_UNUSED meaning that the register 		     is set, but the value is dead.  Failure to do so will 		     result in a sched1 dieing -- when it recomputes lifetime 		     information, the number of REG_DEAD notes will have 		     changed.  */
name|rtx
name|note
init|=
name|find_reg_note
...
argument_list|)
expr_stmt|;
comment|/* In either case, birth is denoted simply by its presence 		 as the destination of a set.  */
name|flags_set_1_set
operator|=
literal|0
expr_stmt|;
...
argument_list|)
operator|)
comment|/* Don't change hard registers used by a call.  */
operator|||
operator|(
name|CALL_P
argument_list|(
...
condition|)
return|return;
comment|/* Be conservative: although this optimization is also valid for      volatile memory references, that could cause trouble in later passes.  */
if|if
condition|(
name|MEM_VOLATILE_P
argument_list|(
...
condition|)
continue|continue;
comment|/* Make a tentative change.  */
...
begin_comment
comment|/* reg_set_in_bb[REGNO] points to basic block iff the register is set    only once in the given block and has REG_EQUAL note.  */
end_comment
...
begin_comment
comment|/* Size of reg_set_in_bb array.  */
end_comment
...
begin_comment
comment|/* cse disrupts preincrement / postdecrement sequences when it finds a    hard register as ultimate source, like the frame pointer.  */
end_comment
...
condition|)
return|return;
comment|/* Find out where a potential flags register is live, and so that we      can suppress some optimizations in those zones.  */
name|mark_flags_life_zones
argument_list|(
name|discover_flags_reg
argument_list|()
...
condition|)
block|{
comment|/* We used to force the copy here like in other cases, but 		     it produces worse code, as it eliminates no copy 		     instructions and the copy emitted will be produced by 		     reload anyway.  On patterns with multiple alternatives, 		     there may be better solution available.  		     In particular this change produced slower code for numeric 		     i387 programs.  */
continue|continue;
block|}
if|if
condition|(
...
block|}
block|}
comment|/* If we weren't able to replace any of the alternatives, try an 	     alternative approach of copying the source to the destination.  */
if|if
condition|(
operator|!
name|success
...
begin_comment
comment|/* Return nonzero if X is stable and mentions no registers but for    mentioning SRC or mentioning / changing DST .  If in doubt, presume    it is unstable.    The rationale is that we want to check if we can move an insn easily    while just paying attention to SRC and DST.  */
end_comment
...
block|}
block|}
comment|/* Combination failed.  Restart processing from here.  If 		 deallocation+allocation conspired to cancel, we can 		 delete the old deallocation insn.  */
if|if
condition|(
name|last_sp_set
operator|&&
...
specifier|static
name|bool
name|gate_handle_stack_adjustments
parameter_list|(
name|void
parameter_list|)
block|{
...
name|unsigned
name|int
name|rest_of_handle_stack_adjustments
parameter_list|(
name|void
parameter_list|)
block|{
