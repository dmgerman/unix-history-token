...
begin_comment
comment|/* Such a structure is allocated initially for each def and use.    In the process of building the interference graph web parts are    connected together, if they have common instructions and reference the    same register.  That way live ranges are build (by connecting defs and    uses) and implicitely complete webs (by connecting web parts in common    uses).  */
end_comment
...
name|num_freedom
decl_stmt|;
comment|/* After successfull coloring the graph each web gets a new reg rtx,      with which the original uses and defs are replaced.  This is it.  */
name|rtx
name|reg_rtx
decl_stmt|;
...
begin_comment
comment|/* This is the bitmap of all (even partly) conflicting super webs.    If bit I*num_webs+J or J*num_webs+I is set, then I and J (both being    super web indices) conflict, maybe only partially.  Note the    assymetry.  */
end_comment
...
begin_comment
comment|/* One pass is: collecting registers refs, buiding I-graph, spilling.    And this is how often we already ran that for the current function.  */
end_comment
...
begin_comment
comment|/* The largest DF_REF_ID of defs resp. uses, as it was in the    last pass.  In the first pass this is zero.  Used to distinguish new    from old refrences.  */
end_comment
...
begin_comment
comment|/* Nonzero if we want to spill at every use, instead of at deaths,    or intereference region borders.  */
end_comment
