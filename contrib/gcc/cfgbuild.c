...
name|CALL_INSN
case|:
comment|/* Noreturn and sibling call instructions terminate the basic blocks 	 (but only if they happen unconditionally).  */
if|if
condition|(
operator|(
name|SIBLING_CALL_P
...
name|INSN
case|:
comment|/* Treat trap instructions like noreturn calls (same provision).  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|BARRIER
case|:
comment|/* It is nonsense to reach barrier when looking for the 	 end of basic block, but before dead code is eliminated 	 this may happen.  */
return|return
name|false
return|;
default|default:
...
control|)
block|{
comment|/* Code labels and barriers causes current basic block to be 	 terminated at previous real insn.  */
if|if
condition|(
operator|(
name|LABEL_P
...
begin_comment
comment|/* States of basic block as seen by find_many_sub_basic_blocks.  */
end_comment
...
name|state
block|{
comment|/* Basic blocks created via split_block belong to this state.      make_edges will examine these basic blocks to see if we need to      create edges going out of them.  */
name|BLOCK_NEW
init|=
literal|0
block|,
comment|/* Basic blocks that do not need examining belong to this state.      These blocks will be left intact.  In particular, make_edges will      not create edges going out of these basic blocks.  */
name|BLOCK_ORIGINAL
block|,
comment|/* Basic blocks that may need splitting (due to a label appearing in      the middle, etc) belong to this state.  After splitting them,      make_edges will create edges going out of them as needed.  */
name|BLOCK_TO_SPLIT
block|}
enum|;
end_enum
...
begin_comment
comment|/* Used internally by purge_dead_tablejump_edges, ORed into state.  */
end_comment

begin_define
define|#
directive|define
name|BLOCK_USED_BY_TABLEJUMP
value|32
end_define
...
begin_comment
comment|/* Identify the edges going out of basic blocks between MIN and MAX,    inclusive, that have their states set to BLOCK_NEW or    BLOCK_TO_SPLIT.     UPDATE_P should be nonzero if we are updating CFG and zero if we    are building CFG from scratch.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* By nature of the way these get numbered, ENTRY_BLOCK_PTR->next_bb block      is always the entry.  */
if|if
condition|(
name|min
operator|==
...
condition|)
continue|continue;
comment|/* If we have an edge cache, cache edges going out of BB.  */
if|if
condition|(
name|edge_cache
condition|)
...
expr_stmt|;
block|}
comment|/* If this is a computed jump, then mark it as reaching 	     everything on the forced_labels list.  */
elseif|else
if|if
condition|(
...
begin_comment
comment|/* Find basic blocks of the current function.    F is the first insn of the function.  */
end_comment
...
name|PROFILE_ABSENT
expr_stmt|;
comment|/* Tell make_edges to examine every block for out-going edges.  */
name|FOR_EACH_BB
argument_list|(
argument|bb
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* See comment in make_label_edge.  */
if|if
condition|(
name|INSN_UID
argument_list|(
...
specifier|static
name|void
name|purge_dead_tablejump_edges
parameter_list|(
name|basic_block
name|bb
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* purge_dead_edges doesn't handle tablejump's, but if we have split the      basic block, we might need to kill some edges.  */
if|if
condition|(
name|bb
operator|!=
...
begin_comment
comment|/* Assume that some pass has inserted labels or control flow    instructions within a basic block.  Split basic blocks as needed    and create edges.  */
end_comment
