...
literal|1
decl_stmt|;
comment|/* True if we may not use TBAA to prune references to this      variable.  This is used for C++ placement new.  */
name|unsigned
name|int
name|no_tbaa_pruning
range|:
literal|1
decl_stmt|;
...
begin_comment
comment|/* Set bits in INTO corresponding to the variable uids in solution set    FROM, which came from variable PTR.    For variables that are actually dereferenced, we also use type    based alias analysis to prune the points-to sets.    IS_DEREFED is true if PTR was directly dereferenced, which we use to    help determine whether we are we are allowed to prune using TBAA.    If NO_TBAA_PRUNING is true, we do not perform any TBAA pruning of    the from set.  */
end_comment
...
begin_comment
comment|/* Compute the set of variables we can't TBAA prune.  */
end_comment

begin_function
specifier|static
name|void
name|compute_tbaa_pruning
parameter_list|(
name|void
parameter_list|)
block|{
...
argument_list|)
expr_stmt|;
comment|/* Mark all initial no_tbaa_pruning nodes as changed.  */
name|any
operator|=
name|false
expr_stmt|;
...
name|changed_count
expr_stmt|;
comment|/* Process the complex copy constraints.  */
for|for
control|(
name|j
operator|=
...
block|}
block|}
comment|/* Propagate to all successors.  */
name|EXECUTE_IF_IN_NONNULL_BITMAP
argument_list|(
argument|graph->succs[i]
argument_list|,
...
argument_list|)
decl_stmt|;
comment|/* Don't propagate to ourselves.  */
if|if
condition|(
name|to
operator|==
...
literal|1
expr_stmt|;
comment|/* Tell the RTL layer that this pointer can alias 		     anything.  */
...
argument_list|)
expr_stmt|;
comment|/* The information in CHANGE_DYNAMIC_TYPE_EXPR nodes has now 	     been captured, and we can remove them.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
