...
argument_list|)
condition|)
comment|/* If this is the upper bound of a C array, leave the minus 1 outside        the SAVE_EXPR so it can be folded away.  */
name|TREE_OPERAND
argument_list|(
name|size
argument_list|,
...
name|type
expr_stmt|;
comment|/* A zero-length bit-field affects the alignment of the next 	     field.  */
if|if
condition|(
name|integer_zerop
argument_list|(
...
name|warning
argument_list|(
literal|"%Jsize of '%D' is larger than %d bytes"
argument_list|,
name|decl
argument_list|,
name|decl
...
condition|)
block|{
comment|/* Named bit-fields cause the entire structure to have the 	 alignment implied by their type.  */
if|if
condition|(
name|DECL_NAME
argument_list|(
...
name|warning
argument_list|(
literal|"%Jpacked attribute causes inefficient alignment "
literal|"for '%D'"
argument_list|,
name|field
argument_list|,
...
name|warning
argument_list|(
literal|"%Jpacked attribute is unnecessary for '%D'"
argument_list|,
name|field
argument_list|,
name|field
...
name|warning
argument_list|(
literal|"%Jpadding struct to align '%D'"
argument_list|,
name|field
argument_list|,
name|field
...
begin_comment
comment|/* TYPE is an integral type, i.e., an INTEGRAL_TYPE, ENUMERAL_TYPE,    BOOLEAN_TYPE, or CHAR_TYPE.  Set TYPE_MIN_VALUE and TYPE_MAX_VALUE    for TYPE, based on the PRECISION and whether or not the TYPE    IS_UNSIGNED.  PRECISION need not correspond to a width supported    natively by the hardware; for example, on a machine with 8-bit,    16-bit, and 32-bit register modes, PRECISION might be 7, 23, or    61.  */
end_comment
