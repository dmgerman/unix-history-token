...
directive|endif
comment|/* See the docs for TARGET_MS_BITFIELD_LAYOUT_P for details.  */
if|if
condition|(
call|(
modifier|*
...
argument_list|)
argument_list|)
comment|/* If the previous field was a zero-sized bit-field, either 	     it was ignored, in which case we must ensure the proper 	     alignment of this field here, or it already forced the 	     alignment of this field, in which case forcing the 	     alignment again is harmless.  So, do it in both cases.  */
operator|||
operator|(
name|DECL_BIT_FIELD_TYPE
argument_list|(
...
name|prev_field
argument_list|)
comment|/* If the previous bit-field is zero-sized, we've already 	     accounted for its alignment needs (or ignored it, if 	     appropriate) while placing it.  */
operator|&&
operator|!
name|integer_zerop
argument_list|(
