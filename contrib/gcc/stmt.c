...
name|label_chain
decl_stmt|;
comment|/* Nonzero if this is associated with an EH region.  */
name|int
...
name|ALLOC_NESTING
parameter_list|()
value|ggc_alloc (sizeof (struct nesting))
end_define
...
decl_stmt|;
comment|/* Location of last line-number note, whether we actually      emitted it or not.  */
name|location_t
name|x_emit_locus
decl_stmt|;
name|struct
...
define|#
directive|define
name|last_expr_alt_rtl
value|(cfun->stmt->x_last_expr_alt_rtl)
end_define
...
specifier|static
name|bool
name|decl_conflicts_with_clobbers_p
parameter_list|(
name|tree
parameter_list|,
specifier|const
...
name|error
argument_list|(
literal|"%Jlabel '%D' used before containing binding contour"
argument_list|,
name|f
operator|->
name|target
...
begin_comment
comment|/* Check for overlap between registers marked in CLOBBERED_REGS and    anything inappropriate in DECL.  Emit error and return TRUE for error,    FALSE for ok.  */
end_comment
...
name|error
argument_list|(
literal|"asm-specifier for variable `%s' conflicts with asm clobber list"
argument_list|,
name|IDENTIFIER_POINTER
argument_list|(
name|DECL_NAME
...
name|error
argument_list|(
literal|"PIC register `%s' clobbered in `asm'"
argument_list|,
name|regname
argument_list|)
expr_stmt|;
...
literal|0
expr_stmt|;
comment|/* If -Wextra, warn about statements with no side effects,      except for an explicit cast to void (e.g. for assert()), and      except for last statement in ({...}) where they may be useful.  */
if|if
condition|(
operator|!
name|want_value
...
argument_list|)
return|;
comment|/* Check if we definitely won't need a fixup.  */
if|if
condition|(
name|whichloop
operator|==
...
begin_comment
comment|/* Return nonzero if we should preserve sub-expressions as separate    pseudos.  We never do so if we aren't optimizing.  We always do so    if -fexpensive-optimizations.     Otherwise, we only do so if we are in the "early" part of a loop.  I.e.,    the loop may still be a small one.  */
end_comment
...
return|;
comment|/* Constant return values are also usually erors,          zero/one often mean booleans so exclude them from the 	 heuristics.  */
if|if
condition|(
name|CONSTANT_P
argument_list|(
...
begin_comment
comment|/* If the current function returns values in the most significant part    of a register, shift return value VAL appropriately.  The mode of    the function's return type is known not to be BLKmode.  */
end_comment
...
begin_comment
comment|/* Generate RTL code to terminate a binding contour.     VARS is the chain of VAR_DECL nodes for the variables bound in this    contour.  There may actually be other nodes in this chain, but any    nodes other than VAR_DECLS are ignored.     MARK_ENDS is nonzero if we should put a note at the beginning    and end of this binding contour.     DONT_JUMP_IN is positive if it is not valid to jump into this contour,    zero if we can jump into this contour only if it does not have a saved    stack level, and negative if we are not to check for invalid use of    labels (because the front end does that).  */
end_comment
...
begin_comment
comment|/* Return 1 if there are any pending cleanups at this point.    Check the current contour as well as contours that enclose    the current contour.  */
end_comment
...
name|warning
argument_list|(
literal|"%Hunreachable code at beginning of %s"
argument_list|,
operator|&
name|locus
argument_list|,
...
argument_list|)
decl_stmt|;
comment|/* The time complexity of this code is normally O(N), where 	 N being the number of members in the enumerated type. 	 However, if type is an ENUMERAL_TYPE whose values do not 	 increase monotonically, O(N*log(N)) time may be needed.  */
name|mark_seen_cases
argument_list|(
name|type
argument_list|,
...
name|warning
argument_list|(
literal|"switch missing default case"
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Share the exit label if possible.  */
if|if
condition|(
name|thiscase
operator|->
...
operator|++
expr_stmt|;
comment|/* Count the number of unique case node targets.  */
name|uniq
operator|++
expr_stmt|;
name|lab
...
expr_stmt|;
block|}
comment|/* For constant index expressions we need only 		 issue an unconditional branch to the appropriate 		 target code.  The job of removing any unreachable 		 code is left to the optimization phase if the 		 "-O" option is specified.  */
for|for
control|(
name|n
operator|=
...
begin_comment
comment|/* Determine whether two case labels branch to the same target.  */
end_comment
...
begin_comment
comment|/* Delete nodes that branch to the default label from a list of    case nodes.  Eg. case 5: default: becomes just default:  */
end_comment
