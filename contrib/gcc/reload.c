...
begin_comment
comment|/* True if X is a constant that can be forced into the constant pool.  */
end_comment

begin_define
define|#
directive|define
name|CONST_POOL_OK_P
parameter_list|(
name|X
parameter_list|)
define|\
value|(CONSTANT_P (X)				\&& GET_CODE (X) != HIGH			\&& !targetm.cannot_force_const_mem (X))
end_define

begin_comment
comment|/* True if C is a non-empty register class that has too few registers    to be safely used as a reload target class.  */
end_comment

begin_define
define|#
directive|define
name|SMALL_REGISTER_CLASS_P
parameter_list|(
name|C
parameter_list|)
define|\
value|(reg_class_size [(C)] == 1 \    || (reg_class_size [(C)]>= 1&& CLASS_LIKELY_SPILLED_P (C)))
end_define
...
parameter_list|)
define|\
value|(x == y || (x != 0&& (REG_P (x)				\ 			? REG_P (y)&& REGNO (x) == REGNO (y)	\ 			: rtx_equal_p (x, y)&& ! side_effects_p (x))))
end_define
...
begin_comment
comment|/* Add NEW to reg_equiv_alt_mem_list[REGNO] if it's not present in the    list yet.  */
end_comment

begin_function
specifier|static
name|void
name|push_reg_equiv_alt_mem
parameter_list|(
name|int
name|regno
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* If we will be using an insn, the secondary reload is for a      scratch register.  */
if|if
condition|(
name|icode
operator|!=
...
comment|/* ??? It would be useful to be able to handle only two, or more than 	 three, operands, but for now we can only handle the case of having 	 exactly three: output, input and one temp/scratch.  */
name|gcc_assert
argument_list|(
name|insn_data
index|[
...
argument_list|)
expr_stmt|;
comment|/* ??? We currently have no way to represent a reload that needs 	 an icode to reload from an intermediate tertiary reload register. 	 We should probably have a new field in struct reload to tag a 	 chain of scratch operand reloads onto.   */
name|gcc_assert
argument_list|(
name|class
operator|==
...
begin_comment
comment|/* If a secondary reload is needed, return its class.  If both an intermediate    register and a scratch register is needed, we return the class of the    intermediate register.  */
end_comment
...
name|icode
expr_stmt|;
comment|/* If there are no secondary reloads at all, we return NO_REGS.      If an intermediate register is needed, we return its class.  */
if|if
condition|(
name|icode
operator|==
...
name|class
return|;
comment|/* No intermediate register is needed, but we have a special reload      pattern, which we assume for now needs a scratch register.  */
return|return
...
begin_comment
comment|/* ICODE is the insn_code of a reload pattern.  Check that it has exactly    three operands, verify that operand 2 is an output operand, and return    its register class.    ??? We'd like to be able to handle any pattern with at least 2 operands,    for zero or more scratch registers, but that needs more infrastructure.  */
end_comment
...
begin_comment
comment|/* Find the largest class which has at least one register valid in    mode INNER, and which for every such register, that register number    plus N is also valid in OUTER (if in range) and is cheap to move    into REGNO.  Such a class must exist.  */
end_comment
...
directive|endif
comment|/* Discard what the target said if we cannot do it.  */
if|if
condition|(
name|preferred_class
operator|!=
...
name|this_insn
argument_list|,
literal|"cannot reload integer constant "
literal|"operand in %<asm%>"
argument_list|)
expr_stmt|;
name|mode
operator|=
...
name|this_insn
argument_list|,
literal|"impossible register constraint "
literal|"in %<asm%>"
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* We used to continue here setting class to ALL_REGS, but it triggers 	     sanity check on i386 for: 	     void foo(long double d) 	     { 	       asm("" :: "a" (d)); 	     } 	     Returning zero here ought to be safe as we take care in 	     find_reloads to not process the reloads when instruction was 	     replaced by USE.  */
return|return
literal|0
return|;
block|}
...
name|in
argument_list|)
comment|/* Check that we don't use a hardreg for an uninitialized 	       pseudo.  See also find_dummy_reload().  */
operator|&&
operator|(
name|ORIGINAL_REGNO
argument_list|(
...
operator|)
argument_list|)
comment|/* But only do all this if we can be sure, that this input            operand doesn't correspond with an uninitialized pseudoreg.            global can assign some hardreg to it, which is the same as 	   a different pseudo also currently live (as it can ignore the 	   conflict).  So we never must introduce writes to such hardregs, 	   as they would clobber the other live pseudo using the same. 	   See also PR20973.  */
operator|&&
operator|(
name|ORIGINAL_REGNO
argument_list|(
...
index|]
decl_stmt|;
comment|/* Nonzero for a MEM operand whose entire address needs a reload.       May be -1 to indicate the entire address may or may not need a reload.  */
name|int
...
index|]
decl_stmt|;
comment|/* Nonzero for an address operand that needs to be completely reloaded.      May be -1 to indicate the entire operand may or may not need a reload.  */
name|int
...
block|}
break|break;
comment|/* Use of ISDIGIT is tempting here, but it may get expensive because 	       of locale support we don't want.  */
case|case
literal|'0'
case|:
case|case
...
literal|1
expr_stmt|;
comment|/* If we can't reload this value at all, reject this 		 alternative.  Note that we could also lose due to 		 LIMIT_RELOAD_CLASS, but we don't check that 		 here.  */
if|if
condition|(
operator|!
name|CONSTANT_P
...
condition|)
block|{
comment|/* If the output is in a non-empty few-regs class, 		     it's costly to reload it, so reload the input instead.  */
if|if
condition|(
name|SMALL_REGISTER_CLASS_P
argument_list|(
...
expr_stmt|;
block|}
comment|/* Likewise any invalid constants appearing as operand of a PLUS      that is to be reloaded.  */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* If the address to be reloaded is a VOIDmode constant, 	       use Pmode as mode of the reload register, as would have 	       been done by find_reloads_address.  */
name|enum
name|machine_mode
name|address_mode
decl_stmt|;
...
name|insn
argument_list|,
literal|"inconsistent operand constraints "
literal|"in an %<asm%>"
argument_list|)
expr_stmt|;
comment|/* Avoid further trouble with this insn.  */
...
control|)
block|{
comment|/* We only do this on the last pass through reload, because it is 	 possible for some data (like reg_equiv_address) to be changed during 	 later passes.  Moreover, we lose the opportunity to get a useful 	 reload_{in,out}_reg when we do these replacements.  */
if|if
condition|(
name|replace
condition|)
...
block|for (i = 0; i< n_reloads; i++)     if (rld[i].reg_rtx == 0&& rld[i].in != 0&& REG_P (rld[i].in)&& rld[i].out == 0)       { 	rld[i].reg_rtx 	  = find_equiv_reg (rld[i].in, insn, rld[i].class, -1, 			    static_reload_reg_p, 0, rld[i].inmode);
...
directive|endif
comment|/* If we detected error and replaced asm instruction by USE, forget about the      reloads.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* Check for SUBREG containing a REG that's equivalent to a 	 constant.  If the constant has a known value, truncate it 	 right now.  Similarly if we are extracting a single-word of a 	 multi-word constant.  If the constant is symbolic, allow it 	 to be substituted normally.  push_reload will strip the 	 subreg later.  The constant must not be VOIDmode, because we 	 will lose the mode of the register (this should never happen 	 because one of the cases above should handle it).  */
name|int
name|regno
init|=
name|REGNO
...
argument_list|)
expr_stmt|;
comment|/* Form the adjusted address.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
begin_comment
comment|/* Note that we take shortcuts assuming that no multi-reg machine mode    occurs as part of an address.    Also, this is not fully machine-customizable; it works for machines    such as VAXen and 68000's and 32000's, but other possible machines    could have addressing modes that this does not handle right.    If you add push_reload calls here, you need to make sure gen_reload    handles those cases gracefully.  */
end_comment
...
parameter_list|)
define|\
value|((CONTEXT) == 0							\    ? regno_ok_for_base_p (REGNO, MODE, OUTER, INDEX)			\    : REGNO_OK_FOR_INDEX_P (REGNO))
name|enum
name|reg_class
name|context_reg_class
decl_stmt|;
name|RTX_CODE
name|code
init|=
...
expr_stmt|;
block|}
comment|/* Plus in the index register may be created only as a result of 	   register rematerialization for expression like&localvar*4.  Reload it. 	   It may be possible to combine the displacement on the outer level, 	   but it is probably not worthwhile to do so.  */
if|if
condition|(
name|context
operator|==
...
name|ZERO_EXTEND
case|:
comment|/* Look for parts to reload in the inner expression and reload them 	 too, in addition to this operation.  Reloading all inner parts in 	 addition to this one shouldn't be necessary, but at this point, 	 we don't know if we can possibly omit any part that *can* be 	 reloaded.  Targets that are better off reloading just either part 	 (or perhaps even a different part of an outer expression), should 	 define LEGITIMIZE_RELOAD_ADDRESS.  */
name|find_reloads_address_1
argument_list|(
name|GET_MODE
argument_list|(
...
block|if (reg_equiv_mem[regno] != 0) 	  { 	    push_reload (reg_equiv_mem[regno], NULL_RTX, loc, (rtx*) 0, 			 context_reg_class, 			 GET_MODE (x), VOIDmode, 0, 0, opnum, type); 	    return 1; 	  }
endif|#
directive|endif
if|if
condition|(
...
literal|'e'
condition|)
comment|/* Pass SCRATCH for INDEX_CODE, since CODE can never be a PLUS once 	     we get here.  */
name|find_reloads_address_1
argument_list|(
name|mode
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* ??? Do we need to handle nonzero offsets somehow?  */
if|if
condition|(
operator|!
name|offset
...
parameter_list|)
define|\
value|gcc_assert (!ARRAY[check_regno]				\ 			  || !loc_mentioned_in_p (r->where,		\ 						  ARRAY[check_regno]))
name|CHECK_MODF
argument_list|(
name|reg_equiv_constant
argument_list|)
...
begin_comment
comment|/* Return 1 if registers from REGNO to ENDREGNO are the subjects of a    REG_INC note in insn INSN.  REGNO must refer to a hard register.  */
end_comment
...
specifier|static
name|int
name|reg_inc_found_and_valid_p
parameter_list|(
name|unsigned
name|int
name|regno
...
begin_comment
comment|/* Return 1 if register REGNO is the subject of a clobber in insn INSN.    If SETS is 1, also consider SETs.  If SETS is 2, enable checking    REG_INC.  REGNO must refer to a hard register.  */
end_comment
...
name|endregno
decl_stmt|;
comment|/* regno must be a hard register.  */
name|gcc_assert
argument_list|(
name|regno
operator|<
