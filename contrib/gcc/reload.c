...
begin_comment
comment|/* Search an insn for pseudo regs that must be in hard regs and are not.    Copyright (C) 1987, 1988, 1989, 1992, 1993, 1994, 1995, 1996, 1997,    1998, 1999, 2000 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
comment|/* Similar issue for (SUBREG constant ...) if it was not handled by the      code above.  This can happen if SUBREG_WORD != 0.  */
if|if
condition|(
name|in
operator|!=
...
literal|0
decl_stmt|;
comment|/* Cache the last regno for the last pseudo we did an output reload      for in case the next insn uses it.  */
specifier|static
name|int
name|last_output_reload_regno
init|=
operator|-
literal|1
decl_stmt|;
...
directive|ifdef
name|HAVE_cc0
comment|/* If we made any reloads for addresses, see if they violate a      "no input reloads" requirement for this insn.  */
if|if
condition|(
name|no_input_reloads
condition|)
...
directive|if
literal|0
comment|/* If this is a pseudo-register that is set in the previous 		 insns, there's a good chance that it will already be in a 		 spill register and we can use that spill register.  So 		 make this case cheaper.   		 Disabled for egcs.  egcs has better inheritance code and 		 this change causes problems with the improved reload 		 inheritance code.  */
block|if (GET_CODE (operand) == REG&& REGNO (operand)>= FIRST_PSEUDO_REGISTER&& REGNO (operand) == last_output_reload_regno) 		reject--;
endif|#
directive|endif
...
expr_stmt|;
block|}
comment|/* Set which reloads must use registers not used in any group.  Start      with those that conflict with a group and then include ones that      conflict with ones that are already known to conflict with a group.  */
name|changed
operator|=
literal|0
expr_stmt|;
...
name|recog_n_operands
expr_stmt|;
comment|/* Return if the insn needs no reload processing.  */
if|if
condition|(
name|noperands
operator|==
...
name|tem
decl_stmt|;
comment|/* If this is a CONST_INT, it could have been created by a 	 plus_constant call in eliminate_regs, which means it may be 	 on the reload_obstack.  reload_obstack will be freed later, so 	 we can't allow such RTL to be put in the constant pool.  There 	 is code in force_const_mem to check for this case, but it doesn't 	 work because we have already popped off the reload_obstack, so 	 rtl_obstack == saveable_obstack is true at this point.  */
if|if
condition|(
name|GET_CODE
argument_list|(
