...
argument_list|)
expr_stmt|;
comment|/* Emit a USE that shows what register is being used/modified.  */
name|REG_NOTES
argument_list|(
name|emit_insn_before
argument_list|(
...
operator|)
operator|)
comment|/* A reloaded auto-increment address is offsettable, 		       because it is now just a simple register indirect.  */
operator|||
operator|(
name|GET_CODE
argument_list|(
...
literal|0
comment|/* This might screw code in reload1.c to delete prior output-reload 	 that feeds this insn.  */
block|if (reg_equiv_mem[regno] != 0) 	  { 	    push_reload (reg_equiv_mem[regno], NULL_RTX, loc, NULL_PTR, 			 (context 			  ? reload_address_index_reg_class 			  : reload_address_base_reg_class), 			 GET_MODE (x), VOIDmode, 0, 0, opnum, type); 	    return 1; 	  }
endif|#
directive|endif
if|if
condition|(
