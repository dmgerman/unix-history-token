...
begin_comment
comment|/* Copyright (C) 1989, 92-97, 1998 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Retrieve the return address for frame UDATA, where SUB_UDATA is a    frame called by UDATA or 0.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* We need to get the value from the CFA register.  At this point in      compiling __throw we don't know whether or not we will use the frame      pointer register for the CFA, so we check our unwind info.  */
if|if
condition|(
name|udata
operator|->
name|cfa_reg
operator|==
name|__builtin_dwarf_fp_regnum
argument_list|()
condition|)
name|udata
operator|->
...
argument_list|)
expr_stmt|;
comment|/* If we found one, we can stop searching.  */
if|if
condition|(
name|handler
condition|)
...
comment|/* Emit the stub to adjust sp and jump to the handler.  */
if|if
condition|(
name|new_exception_model
condition|)
name|retaddr
operator|=
name|__builtin_eh_stub
argument_list|()
expr_stmt|;
else|else
name|retaddr
operator|=
name|__builtin_eh_stub_old
argument_list|()
expr_stmt|;
comment|/* And then set our return address to point to the stub.  */
if|if
condition|(
name|my_udata
operator|->
...
name|__builtin_set_return_addr_reg
argument_list|(
name|retaddr
argument_list|)
expr_stmt|;
comment|/* Set up the registers we use to communicate with the stub.      We check STACK_GROWS_DOWNWARD so the stub can use adjust_stack.  */
if|if
condition|(
name|new_exception_model
condition|)
name|__builtin_set_eh_regs
argument_list|(
argument|(void *)eh
argument_list|,
ifdef|#
directive|ifdef
name|STACK_GROWS_DOWNWARD
argument|udata->cfa - my_udata->cfa
else|#
directive|else
argument|my_udata->cfa - udata->cfa
endif|#
directive|endif
argument|+ args_size
argument_list|)
