...
directive|endif
comment|/* The XMODE value can be seen as an vector of NREGS_XMODE      values.  The subreg must represent an lowpart of given field.      Compute what field it is.  */
name|offset
operator|-=
name|subreg_lowpart_offset
argument_list|(
...
begin_comment
comment|/* Return true if we should avoid inserting code between INSN and preceeding    call instruction.  */
end_comment
...
name|true
return|;
comment|/* Allow subreg of X in case it is not writting just part of multireg pseudo.      Then we would need to update all users to care hoisting the store too.      Caller may represent that by specifying whole subreg as val.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|false
return|;
comment|/* Pseudo registers can be allways replaced by another pseudo to avoid      the side effect, for hard register we must ensure that they are dead.      Eventually we may want to add code to try turn pseudos to hards, but it      is unlikely usefull.  */
if|if
condition|(
name|REGNO
argument_list|(
...
name|USE
case|:
comment|/* We need to fix callers to really ensure availability 	         of all values inisn uses, but for now it is safe to prohibit 		 hoisting of any insn having such a hiden uses.  */
return|return
name|false
return|;
break|break;
