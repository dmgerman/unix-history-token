...
begin_comment
comment|/* Scan linker error messages for missing template instantiations and provide    them.     Copyright (C) 1995, 1998, 1999, 2000, 2001, 2003, 2004, 2005    Free Software Foundation, Inc.    Contributed by Jason Merrill (jason@cygnus.com).  This file is part of GCC.  GCC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GCC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GCC; see the file COPYING.  If not, write to the Free Software Foundation, 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* Update the repo files for each of the object files we have adjusted and    recompile.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* On Windows "rename" returns -1 and sets ERRNO to EACCESS if 	 the new file name already exists.  Therefore, we explicitly 	 remove the old file first.  */
if|if
condition|(
name|remove
argument_list|(
...
name|error
argument_list|(
literal|"repository file '%s' does not contain command-line "
literal|"arguments"
argument_list|,
name|f
operator|->
...
return|;
block|}
comment|/* Build a null-terminated argv array suitable for 	 tlink_execute().  Manipulate arguments on the arg_stack while 	 building argv on the temporary_obstack.  */
name|obstack_init
argument_list|(
operator|&
name|arg_stack
...
control|)
block|{
comment|/* Arguments are delimited by single-quotes.  Find the 	     opening quote.  */
name|p
operator|=
name|strchr
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Replace '\'' with '.  This is how set_collect_gcc_options 	     encodes a single-quote.  */
while|while
condition|(
name|q
index|[
...
literal|0
decl_stmt|;
comment|/* On darwin9, we might have to skip " in " lines as well.  */
if|if
condition|(
name|skip_next_in_line
operator|&&
...
literal|0
expr_stmt|;
comment|/* On darwin9, we look for "foo" referenced from:\n\(.* in .*\n\)*  */
if|if
condition|(
name|strcmp
argument_list|(
name|oldq
argument_list|,
literal|"referenced from:"
argument_list|)
operator|==
literal|0
condition|)
block|{
comment|/* We have to remember that we found a symbol to tweak.  */
name|ok
operator|=
literal|1
expr_stmt|;
comment|/* We actually want to start from the first word on the 		 line.  */
name|oldq
operator|=
name|p
expr_stmt|;
comment|/* Since the format is multiline, we have to skip 		 following lines with " in ".  */
name|skip_next_in_line
operator|=
literal|1
expr_stmt|;
...
name|error
argument_list|(
literal|"'%s' was assigned to '%s', but was not defined "
literal|"during recompilation, or vice versa"
argument_list|,
name|sym
operator|->
name|key
