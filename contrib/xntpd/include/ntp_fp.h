...
comment|/* v = -v */
define|\
value|do { \ 		if ((v_f) == 0) \ 			(v_i) = -(v_i); \ 		else { \ 			(v_f) = -(v_f); \ 			(v_i) = ~(v_i); \ 		} \ 	} while(0)
end_define
...
comment|/* r += a */
define|\
value|do { \ 		register U_LONG lo_tmp; \ 		register U_LONG hi_tmp; \ 		\ 		lo_tmp = ((r_f)& 0xffff) + ((a_f)& 0xffff); \ 		hi_tmp = (((r_f)>> 16)& 0xffff) + (((a_f)>> 16)& 0xffff); \ 		if (lo_tmp& 0x10000) \ 			hi_tmp++; \ 		(r_f) = ((hi_tmp& 0xffff)<< 16) | (lo_tmp& 0xffff); \ 		\ 		(r_i) += (a_i); \ 		if (hi_tmp& 0x10000) \ 			(r_i)++; \ 	} while (0)
end_define
...
define|\
value|do { \ 		register U_LONG lo_tmp; \ 		register U_LONG hi_tmp; \ 		\ 		lo_tmp = ((r_f)& 0xffff) + ((a_f)& 0xffff); \ 		hi_tmp = (((r_f)>> 16)& 0xffff) + (((a_f)>> 16)& 0xffff); \ 		if (lo_tmp& 0x10000) \ 			hi_tmp++; \ 		(r_f) = ((hi_tmp& 0xffff)<< 16) | (lo_tmp& 0xffff); \ 		\ 		lo_tmp = ((r_i)& 0xffff) + ((a_i)& 0xffff); \ 		if (hi_tmp& 0x10000) \ 			lo_tmp++; \ 		hi_tmp = (((r_i)>> 16)& 0xffff) + (((a_i)>> 16)& 0xffff); \ 		if (lo_tmp& 0x10000) \ 			hi_tmp++; \ 		(r_i) = ((hi_tmp& 0xffff)<< 16) | (lo_tmp& 0xffff); \ 		\ 		(r_ovr) += (a_ovr); \ 		if (hi_tmp& 0x10000) \ 			(r_ovr)++; \ 	} while (0)
end_define
...
comment|/* r -= a */
define|\
value|do { \ 		register U_LONG lo_tmp; \ 		register U_LONG hi_tmp; \ 		\ 		if ((a_f) == 0) { \ 			(r_i) -= (a_i); \ 		} else { \ 			lo_tmp = ((r_f)& 0xffff) + ((-(a_f))& 0xffff); \ 			hi_tmp = (((r_f)>> 16)& 0xffff) \ 			    + (((-(a_f))>> 16)& 0xffff); \ 			if (lo_tmp& 0x10000) \ 				hi_tmp++; \ 			(r_f) = ((hi_tmp& 0xffff)<< 16) | (lo_tmp& 0xffff); \ 			\ 			(r_i) += ~(a_i); \ 			if (hi_tmp& 0x10000) \ 				(r_i)++; \ 		} \ 	} while (0)
end_define
...
define|\
value|do { \ 		(v_f) = (U_LONG)(v_f)>> 1; \ 		if ((v_i)& 01) \ 			(v_f) |= 0x80000000; \ 		(v_i) = (U_LONG)(v_i)>> 1; \ 	} while (0)
end_define
...
define|\
value|do { \ 		(v_f) = (U_LONG)(v_f)>> 1; \ 		if ((v_i)& 01) \ 			(v_f) |= 0x80000000; \ 		if ((v_i)& 0x80000000) \ 			(v_i) = ((v_i)>> 1) | 0x80000000; \ 		else \ 			(v_i) = (v_i)>> 1; \ 	} while (0)
end_define
...
name|uf
parameter_list|)
comment|/* r += uf, uf is U_LONG fraction */
define|\
...
name|f
parameter_list|)
comment|/* r += f, f is a LONG fraction */
define|\
...
define|\
value|(((U_LONG)(a_i))> ((U_LONG)(b_i)) || \ 	  ((a_i) == (b_i)&& ((U_LONG)(a_f))>= ((U_LONG)(b_f))))
end_define
...
comment|/* a>= b signed */
define|\
value|(((LONG)(a_i))> ((LONG)(b_i)) || \ 	  ((a_i) == (b_i)&& ((U_LONG)(a_f))>= ((U_LONG)(b_f))))
end_define
