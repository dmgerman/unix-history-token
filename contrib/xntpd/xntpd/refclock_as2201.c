...
begin_comment
comment|/*  * refclock_gps - clock driver for the Austron 2201A GPS Timing Receiver  */
end_comment
...
begin_comment
comment|/*  * This driver supports the Austron 2200A/2201A GPS Receiver with  * Buffered RS-232-C Interface Module. Note that the original 2200/2201  * receivers will not work reliably with this driver, since the older  * design cannot accept input commands at any reasonable data rate.  *  * The program sends a "*toc\r" to the radio and expects a response of  * the form "yy:ddd:hh:mm:ss.mmm\r" where yy = year of century, ddd =  * day of year, hh:mm:ss = second of day and mmm = millisecond of  * second. Then, it sends statistics commands to the radio and expects  * a multi-line reply showing the corresponding statistics or other  * selected data. Statistics commands are sent in order as determined by  * a vector of commands; these might have to be changed with different  * radio options.  *  * In order for this code to work, the radio must be placed in non-  * interactive mode using the "off" command and with a single<cr>  * resonse using the "term cr" command. The setting of the "echo"  * and "df" commands does not matter. The radio should select UTC  * timescale using the "ts utc" command.  *  * There are two modes of operation for this driver. The first with  * undefined AS2201PPS is used with stock kernels and serial-line drivers  * and works with almost any machine. In this mode the driver assumes  * the radio captures a timestamp upon receipt of the "*" that begins  * the driver query. Accuracies in this mode are in the order of a  * millisecond or two and the receiver can be connected to only one  * host. The second with AS2201PPS defined can be used for SunOS kernels  * that have been modified with the ppsclock streams module included in  * this distribution. In this mode a precise timestamp is available  * using a gadget box and 1-pps signal from the receiver; however, the  * sample rate is limited to the polling rate, normally about one poll  * every 16 seconds. This improves the accuracy to the order of a few  * tens of microseconds. In addition, the serial output and 1-pps signal  * can be bussed to additional receivers. For the utmost accuracy, the  * sample rate can be increased to one per second using the PPSCD  * define. This improves the accuracy to the order of a few  * microseconds.   */
end_comment
...
begin_comment
comment|/* precision assumed (about 30 us) */
end_comment
...
begin_comment
comment|/* yy:ddd:hh:mm:ss.mmm datecode length */
end_comment
...
begin_comment
comment|/*  * as2201_init - initialize internal gps driver data  */
end_comment
...
begin_comment
comment|/*  * as2201_start - open the GPS devices and initialize data for processing  */
end_comment
...
name|LOG_ERR
argument_list|,
literal|"gps_start: unit %d invalid"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"gps_start: unit %d in use"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, TCGETA): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, TCSETA): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: tcgetattr(%s): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: tcsetattr(%s): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: tcflush(%s): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, I_PUSH, clk): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, CLK_SETSTR): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, I_PUSH, ppsclock): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|HAVE_BSD_TTYS
argument_list|)
comment|/* 	 * 4.3bsd serial line parameters (sgttyb interface) 	 * 	 * The AS2201CLK option provides timestamping at the driver level.  	 * It requires the tty_clk line discipline and 4.3bsd or later. 	 */
block|{
name|struct
name|sgttyb
name|ttyb
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, TIOCGETP): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, TIOCSETP): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"as2201_start: ioctl(%s, TIOCSETD): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"gps_start: ioctl(%s, CIOGETEV): %m"
argument_list|,
name|as2201dev
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * as2201_shutdown - shut down a GPS clock  */
end_comment
...
name|LOG_ERR
argument_list|,
literal|"gps_shutdown: unit %d invalid"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"gps_shutdown: unit %d not in use"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * as2201_report_event - note the occurance of an event  *  * This routine presently just remembers the report and logs it, but  * does nothing heroic for the trap handler.  */
end_comment
...
begin_comment
comment|/*  * as2201_receive - receive data from the serial interface  */
end_comment
...
literal|'\0'
expr_stmt|;
comment|/* 	 * We check the timecode format and decode its contents. The 	 * timecode has format yy:ddd:hh:mm:ss.mmm). If it has invalid 	 * length or is not in proper format, the driver declares bad 	 * format and exits. If the converted decimal values are out of 	 * range, the driver declares bad data and exits. 	 */
if|if
condition|(
name|gps
operator|->
...
name|AS2201PPS
argument_list|)
comment|/* 	 * If the pps signal is available and the local time is within 	 * +-0.5 second of the timecode, use the pps offset instead. 	 * Note that we believe the ppsclock timestamp only if the ioctl 	 * works and the new timestamp is greater than the previous one. 	 */
name|gps
operator|->
name|lastrec
operator|=
...
directive|endif
comment|/* 	 * If the statistics-record switch (CLK_FLAG4) is set, 	 * initialize the statistics buffer and send the next command. 	 * If not, simply write the timecode to the clockstats file. 	 */
operator|(
name|void
operator|)
name|strcpy
...
begin_comment
comment|/*  * as2201_process - process a pile of samples from the clock  *  * This routine uses a three-stage median filter to calculate offset and  * dispersion and reduce jitter. The dispersion is calculated as the  * span of the filter (max - min).  */
end_comment
...
name|LOG_ERR
argument_list|,
literal|"gps_poll: unit %d invalid"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"gps_poll: unit %d not in use"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * as2201_control - set fudge factors, return statistics  */
end_comment
...
name|LOG_ERR
argument_list|,
literal|"gps_control: unit %d invalid"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * as2201_buginfo - return clock dependent debugging info  */
end_comment
...
name|LOG_ERR
argument_list|,
literal|"gps_buginfo: unit %d invalid"
argument_list|,
name|unit
argument_list|)
expr_stmt|;
