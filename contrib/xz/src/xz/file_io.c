...
begin_comment
comment|// Using this macro to silence a warning from gcc -Wlogical-op.
end_comment
...
begin_comment
comment|/// True if the conditions for sandboxing (described in main()) have been met.
end_comment

begin_decl_stmt
specifier|static
name|bool
name|sandbox_allowed
init|=
name|false
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/// Enables operating-system-specific sandbox if it is possible.
end_comment

begin_comment
comment|/// src_fd is the file descriptor of the input file.
end_comment

begin_function
specifier|static
name|void
name|io_sandbox_enter
parameter_list|(
name|int
name|src_fd
parameter_list|)
...
name|_
argument_list|(
literal|"Sandbox is disabled due "
literal|"to incompatible command line arguments"
argument_list|)
argument_list|)
expr_stmt|;
return|return;
...
literal|"x"
decl_stmt|;
comment|// Try to ensure that both libc and xz locale files have been
comment|// loaded when NLS is enabled.
name|snprintf
argument_list|(
name|NULL
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Try to ensure that iconv data files needed for handling multibyte
comment|// characters have been loaded. This is needed at least with glibc.
...
directive|ifdef
name|HAVE_CAPSICUM
comment|// Capsicum needs FreeBSD 10.0 or later.
name|cap_rights_t
name|rights
decl_stmt|;
if|if
...
name|_
argument_list|(
literal|"Sandbox was successfully enabled"
argument_list|)
argument_list|)
expr_stmt|;
return|return;
...
name|_
argument_list|(
literal|"Failed to enable the sandbox"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
name|HAVE__FUTIME
argument_list|)
comment|// Use one-second precision with Windows-specific _futime().
comment|// We could use utime() too except that for some reason the
comment|// timestamp will get reset at close(). With _futime() it works.
comment|// This struct cannot be const as _futime() takes a non-const pointer.
name|struct
name|_utimbuf
name|buf
init|=
...
condition|)
block|{
comment|// Close the file before possibly unlinking it. On DOS-like
comment|// systems this is always required since unlinking will fail
comment|// if the file is open. On POSIX systems it usually works
comment|// to unlink open files, but in some cases it doesn't and
comment|// one gets EBUSY in errno.
comment|//
comment|// xz 5.2.2 and older unlinked the file before closing it
comment|// (except on DOS-like systems). The old code didn't handle
comment|// EBUSY and could fail e.g. on some CIFS shares. The
comment|// advantage of unlinking before closing is negligible
comment|// (avoids a race between close() and stat()/lstat() and
comment|// unlink()), so let's keep this simple.
operator|(
name|void
operator|)
name|close
