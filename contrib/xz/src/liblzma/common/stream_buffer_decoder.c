...
begin_comment
comment|/// \file       stream_buffer_decoder.c
end_comment

begin_comment
comment|/// \brief      Single-call .xz Stream decoder
end_comment
...
return|;
comment|// Catch flags that are not allowed in buffer-to-buffer decoding.
if|if
condition|(
name|flags
operator|&
...
name|LZMA_PROG_ERROR
return|;
comment|// Initialize the Stream decoder.
comment|// TODO: We need something to tell the decoder that it can use the
comment|// output buffer as workspace, and thus save significant amount of RAM.
...
block|{
comment|// Something went wrong, restore the positions.
operator|*
name|in_pos
operator|=
name|in_start
...
comment|// processing the last byte of the Stream
...
condition|)
block|{
comment|// Let the caller know how much memory would
comment|// have been needed.
name|uint64_t
name|memusage
decl_stmt|;
operator|(
