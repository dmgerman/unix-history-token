...
typedef|typedef
name|struct
name|arena_avail_links_s
name|arena_avail_links_t
typedef|;
end_typedef
...
name|arena_chunk_map_misc_s
block|{
comment|/* 	 * Linkage for run heaps.  There are two disjoint uses: 	 * 	 * 1) arena_t's runs_avail heaps. 	 * 2) arena_run_t conceptually uses this linkage for in-use non-full 	 *    runs, rather than directly embedding linkage. 	 */
name|phn
argument_list|(
...
name|deadline
decl_stmt|;
comment|/* 	 * Number of dirty pages at beginning of current epoch.  During epoch 	 * advancement we use the delta between arena->decay.ndirty and 	 * arena->ndirty to determine how many dirty pages, if any, were 	 * generated. 	 */
name|size_t
name|ndirty
decl_stmt|;
...
name|runcur
decl_stmt|;
comment|/* 	 * Heap of non-full runs.  This heap is used when looking for an 	 * existing run when runcur is no longer usable.  We choose the 	 * non-full run that is lowest in memory; this policy tends to keep 	 * objects packed well, and it can also help reduce the number of 	 * almost-empty chunks. 	 */
name|arena_run_heap_t
name|runs
decl_stmt|;
...
name|chunks_cache
decl_stmt|;
comment|/* Decay-based purging state. */
name|arena_decay_t
name|decay
decl_stmt|;
...
index|]
decl_stmt|;
comment|/* 	 * Size-segregated address-ordered heaps of this arena's available runs, 	 * used for first-best-fit run allocation.  Runs are quantized, i.e. 	 * they reside in the last heap which corresponds to a size class less 	 * than or equal to the run size. 	 */
name|arena_run_heap_t
name|runs_avail
index|[
name|NPSIZES
