...
literal|"      ndalloc    nrequests      curregs      curruns regs"
literal|" pgs  util       nfills     nflushes      newruns"
literal|"       reruns\n"
argument_list|)
expr_stmt|;
block|}
...
literal|"bins:           size ind    allocated      nmalloc"
literal|"      ndalloc    nrequests      curregs      curruns regs"
literal|" pgs  util      newruns       reruns\n"
argument_list|)
expr_stmt|;
block|}
name|CTL_GET
...
name|cbopaque
argument_list|,
literal|"large:          size ind    allocated      nmalloc      ndalloc"
literal|"    nrequests      curruns\n"
argument_list|)
expr_stmt|;
name|CTL_GET
argument_list|(
...
name|cbopaque
argument_list|,
literal|"huge:           size ind    allocated      nmalloc      ndalloc"
literal|"    nrequests   curhchunks\n"
argument_list|)
expr_stmt|;
name|CTL_GET
argument_list|(
...
parameter_list|)
define|\
value|if (je_mallctl("opt."#n,&bv,&bsz, NULL, 0) == 0) {	\ 			malloc_cprintf(write_cb, cbopaque,		\ 			    "  opt."#n": %s\n", bv ? "true" : "false");	\ 		}
define|#
directive|define
...
parameter_list|)
define|\
value|if (je_mallctl("opt."#n,&sv,&ssz, NULL, 0) == 0) {	\ 			malloc_cprintf(write_cb, cbopaque,		\ 			"  opt."#n": %zu\n", sv);			\ 		}
define|#
directive|define
...
parameter_list|)
define|\
value|if (je_mallctl("opt."#n,&ssv,&sssz, NULL, 0) == 0) {	\ 			malloc_cprintf(write_cb, cbopaque,		\ 			    "  opt."#n": %zd\n", ssv);			\ 		}
define|#
directive|define
...
name|m
parameter_list|)
value|{				\ 		ssize_t ssv2;						\ 		if (je_mallctl("opt."#n,&ssv,&sssz, NULL, 0) == 0&&	\ 		    je_mallctl(#m,&ssv2,&sssz, NULL, 0) == 0) {	\ 			malloc_cprintf(write_cb, cbopaque,		\ 			    "  opt."#n": %zd ("#m": %zd)\n",		\ 			    ssv, ssv2);					\ 		}							\ }
define|#
directive|define
...
parameter_list|)
define|\
value|if (je_mallctl("opt."#n,&cpv,&cpsz, NULL, 0) == 0) {	\ 			malloc_cprintf(write_cb, cbopaque,		\ 			    "  opt."#n": \"%s\"\n", cpv);		\ 		}
name|malloc_cprintf
argument_list|(
name|write_cb
argument_list|,
