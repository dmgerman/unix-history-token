...
operator|)
return|;
comment|/* 	 * mmap.  Requesting an address is not implemented for 	 * chunk_alloc_mmap(), so only call it if (new_addr == NULL). 	 */
if|if
condition|(
name|new_addr
operator|==
...
name|arena
decl_stmt|;
comment|/* Dodge tsd for a0 in order to avoid bootstrapping issues. */
name|arena
operator|=
operator|(
name|arena_ind
