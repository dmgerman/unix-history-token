...
condition|)
block|{
comment|/* 		 * Convert chunk back to initial THP state, so that all 		 * subsequent chunk allocations start out in a consistent state. 		 */
if|if
condition|(
name|thp_initially_huge
condition|)
...
specifier|static
name|void
name|arena_decay_epoch_advance_purge
parameter_list|(
name|tsdn_t
modifier|*
name|tsdn
...
expr_stmt|;
block|}
comment|/* 	 * If the deadline has been reached, advance to the current epoch and 	 * purge to the new limit if necessary.  Note that dirty pages created 	 * during the current epoch are not subject to purge until a future 	 * epoch, so as a result purging only happens during epoch advances. 	 */
if|if
condition|(
name|arena_decay_deadline_reached
argument_list|(
...
name|LG_PAGE
expr_stmt|;
comment|/* 			 * If this is the first run purged within chunk, mark 			 * the chunk as non-THP-capable.  This will prevent all 			 * use of THPs for this chunk until the chunk as a whole 			 * is deallocated. 			 */
if|if
condition|(
name|config_thp
operator|&&
...
parameter_list|)
block|{
comment|/* 	 * Make sure that if bin->runcur is non-NULL, it refers to the 	 * oldest/lowest non-full run.  It is okay to NULL runcur out rather 	 * than proactively keeping it pointing at the oldest/lowest non-full 	 * run. 	 */
if|if
condition|(
name|bin
operator|->
...
specifier|const
name|bool
name|thp_initially_huge_states
index|[]
init|=
block|{
name|true
