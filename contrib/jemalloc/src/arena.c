...
begin_comment
comment|/* Max run size used for small size classes. */
end_comment
...
name|JEMALLOC_INLINE_C
name|size_t
name|arena_miscelm_key_size_get
parameter_list|(
specifier|const
name|arena_chunk_map_misc_t
modifier|*
...
argument_list|)
decl_stmt|;
comment|/* 	 * Compare based on quantized size rather than size, in order to sort 	 * equally useful runs only by address. 	 */
name|ret
operator|=
operator|(
name|a_qsize
...
block|{
comment|/* 			 * Treat keys as if they are lower than anything else. 			 */
name|ret
operator|=
operator|-
literal|1
...
operator|)
return|;
comment|/* 	 * Initialize the map to contain one maximal free untouched run.  Mark 	 * the pages as zeroed if chunk_alloc() returned a zeroed or decommitted 	 * chunk. 	 */
name|flag_unzeroed
operator|=
operator|(
name|zero
...
begin_comment
comment|/*  * Do first-best-fit run selection, i.e. select the lowest run that best fits.  * Run sizes are quantized, so not all candidate runs are necessarily exactly  * the same size.  */
end_comment
...
literal|0
decl_stmt|;
comment|/* Stash at least npurge pages. */
for|for
control|(
name|rdelm
operator|=
...
specifier|static
name|void
name|arena_run_dalloc_decommit
parameter_list|(
name|arena_t
modifier|*
name|arena
...
condition|)
block|{
comment|/* 			 * OOM.  tbin->avail isn't yet filled down to its first 			 * element, so the successful allocations (if any) must 			 * be moved to the base of tbin->avail before bailing 			 * out. 			 */
if|if
condition|(
name|i
operator|>
...
name|UINT64_C
argument_list|(
literal|6364136223846793009
argument_list|)
argument_list|,
name|UINT64_C
argument_list|(
literal|1442695040888963409
argument_list|)
argument_list|)
expr_stmt|;
name|random_offset
...
name|usize
decl_stmt|;
comment|/* Make sure extra can't cause size_t overflow. */
if|if
condition|(
name|unlikely
argument_list|(
