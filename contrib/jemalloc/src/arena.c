...
condition|)
block|{
comment|/* 		 * Convert chunk back to the default state, so that all 		 * subsequent chunk allocations start out with chunks that can 		 * be backed by transparent huge pages. 		 */
name|pages_huge
argument_list|(
name|chunk
argument_list|,
...
name|LG_PAGE
expr_stmt|;
comment|/* 			 * If this is the first run purged within chunk, mark 			 * the chunk as non-huge.  This will prevent all use of 			 * transparent huge pages for this chunk until the chunk 			 * as a whole is deallocated. 			 */
if|if
condition|(
name|chunk
operator|->
