...
begin_comment
comment|/* Protects arenas initialization (arenas, narenas_total). */
end_comment
...
name|arena
decl_stmt|;
comment|/* Expand arenas if necessary. */
name|assert
argument_list|(
name|ind
operator|<=
...
name|NULL
expr_stmt|;
comment|/* 		 * Deallocate only if arenas came from a0malloc() (not 		 * base_alloc()). 		 */
if|if
condition|(
name|narenas_total
operator|!=
...
argument_list|()
decl_stmt|;
comment|/* Deallocate old cache if it's too small. */
if|if
condition|(
name|arenas_cache
operator|!=
...
expr_stmt|;
block|}
comment|/* Allocate cache if it's missing. */
if|if
condition|(
name|arenas_cache
operator|==
...
condition|)
block|{
comment|/* 			 * This function must always tell the truth, even if 			 * it's slow, so don't let OOM, thread cleanup (note 			 * tsd_nominal check), nor recursive allocation 			 * avoidance (note arenas_cache_bypass check) get in the 			 * way. 			 */
if|if
condition|(
name|ind
operator|>=
...
expr_stmt|;
block|}
comment|/* 	 * Copy to cache.  It's possible that the actual number of arenas has 	 * increased since narenas_total_get() was called above, but that causes 	 * no correctness issues unless two threads concurrently execute the 	 * arenas.extend mallctl, which we trust mallctl synchronization to 	 * prevent. 	 */
name|malloc_mutex_lock
argument_list|(
operator|&
name|arenas_lock
...
expr_stmt|;
block|}
comment|/* Read the refreshed cache, and init the arena if necessary. */
name|arena
operator|=
name|arenas_cache
index|[
...
argument_list|)
expr_stmt|;
comment|/* Clamp extra if necessary to avoid (size + extra) overflow. */
if|if
condition|(
name|unlikely
argument_list|(
...
condition|)
block|{
comment|/* Check for size overflow. */
if|if
condition|(
name|unlikely
argument_list|(
