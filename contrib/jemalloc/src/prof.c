...
begin_comment
comment|/*  * This buffer is rather large for stack allocation, so use a single buffer for  * all profile dumps.  The buffer is implicitly protected by bt2ctx_mtx, since  * it must be locked anyway during dumping.  */
end_comment
...
return|;
block|}
comment|/* 			 * Artificially raise curobjs, in order to avoid a race 			 * condition with prof_ctx_merge()/prof_ctx_destroy(). 			 * 			 * No locking is necessary for ctx here because no other 			 * threads have had the opportunity to fetch it from 			 * bt2ctx yet. 			 */
name|ctx
operator|.
name|p
operator|->
...
block|{
comment|/* 			 * Artificially raise curobjs, in order to avoid a race 			 * condition with prof_ctx_merge()/prof_ctx_destroy(). 			 */
name|malloc_mutex_lock
argument_list|(
name|ctx
operator|.
...
argument_list|)
expr_stmt|;
comment|/* 	 * Check that ctx is still unused by any thread cache before destroying 	 * it.  prof_lookup() artificially raises ctx->cnt_merge.curobjs in 	 * order to avoid a race condition with this function, as does 	 * prof_ctx_merge() in order to avoid a race between the main body of 	 * prof_ctx_merge() and entry into this function. 	 */
name|prof_enter
argument_list|()
expr_stmt|;
name|malloc_mutex_lock
...
condition|)
block|{
comment|/* 		 * Artificially raise ctx->cnt_merged.curobjs in order to keep 		 * another thread from winning the race to destroy ctx while 		 * this one has ctx->lock dropped.  Without this, it would be 		 * possible for another thread to: 		 * 		 * 1) Sample an allocation associated with ctx. 		 * 2) Deallocate the sampled object. 		 * 3) Successfully prof_ctx_destroy(ctx). 		 * 		 * The result would be that ctx no longer exists by the time 		 * this thread accesses it in prof_ctx_destroy(). 		 */
name|ctx
operator|->
name|cnt_merged
operator|.
...
condition|)
block|{
comment|/* 		 * Avoid doing multiple hashes, since a single hash provides 		 * enough bits. 		 */
name|ret1
operator|=
name|h
operator|&
