...
begin_comment
comment|/* Print a message reporting an internal error. Ask the user if they    want to continue, dump core, or just exit. */
end_comment
...
index|[]
init|=
literal|"Internal GDB error: recursive internal error.\n"
decl_stmt|;
specifier|static
name|int
name|dejavu
...
name|dump_core_p
decl_stmt|;
comment|/* don't allow infinite error recursion. */
switch|switch
condition|(
name|dejavu
condition|)
...
expr_stmt|;
block|}
comment|/* Try to get the message out */
name|target_terminal_ours
argument_list|()
expr_stmt|;
name|fprintf_unfiltered
...
name|gdb_stderr
argument_list|,
literal|"%s:%d: gdb-internal-error: "
argument_list|,
name|file
argument_list|,
name|line
...
name|query
argument_list|(
literal|"\ An internal GDB error was detected.  This may make further\n\ debugging unreliable.  Quit this debugging session? "
argument_list|)
expr_stmt|;
comment|/* Default (yes/batch case) is to dump core.  This leaves a GDB      dropping so that it is easier to see that something went wrong to      GDB. */
name|dump_core_p
operator|=
name|query
argument_list|(
literal|"\ Create a core file containing the current state of GDB? "
argument_list|)
expr_stmt|;
if|if
condition|(
...
begin_comment
comment|/* NOTE: These must use PTR so that their definition matches the    declaration found in "mmalloc.h". */
end_comment
...
begin_comment
comment|/* Convert to / from the hosts pointer to GDB's internal CORE_ADDR    using the target's conversion routines. */
end_comment
...
name|__LINE__
argument_list|,
literal|"core_addr_to_void_ptr: bad cast"
argument_list|)
expr_stmt|;
return|return
