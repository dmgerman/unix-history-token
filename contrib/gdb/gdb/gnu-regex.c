...
begin_comment
comment|/* We need this for `gnu-regex.h', and perhaps for the Emacs include files.  */
end_comment
...
begin_comment
comment|/* CYGNUS LOCAL: ../intl will handle this for us */
end_comment
...
begin_comment
comment|/* GDB LOCAL: define _REGEX_RE_COMP to get BSD style re_comp and re_exec */
end_comment
...
begin_comment
comment|/* CYGNUS LOCAL: call it gnu-regex.h, not regex.h, to avoid name conflicts */
end_comment
...
begin_comment
comment|/* Specify the precise syntax of regexps for compilation.  This provides    for compatibility for various utilities which historically have    different, incompatible syntaxes.     The argument SYNTAX is a bit mask comprised of the various bits    defined in gnu-regex.h.  We return the old syntax.  */
end_comment
...
begin_comment
comment|/* This table gives an error message for each of the error codes listed    in gnu-regex.h.  Obviously the order here has to be same as there.    POSIX doesn't require that we do anything for REG_NOERROR,    but why not be nice?  */
end_comment
...
condition|)
block|{
comment|/* CYGNUS LOCAL: Skip this code if we don't have btowc().  btowc() is */
comment|/* defined in the 1994 Amendment 1 to ISO C and may not be present on */
comment|/* systems where we have wchar.h and wctype.h.   */
if|#
directive|if
name|defined
name|_LIBC
