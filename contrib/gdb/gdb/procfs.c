...
begin_comment
comment|/*  * Temporary debugging code:  *  * These macros allow me to trace the system calls that we make  * to control the child process.  This is quite handy for comparing  * with the older version of procfs.  */
end_comment
...
begin_comment
comment|/* Provide default composite pid manipulation macros for systems that    don't have threads. */
end_comment
...
begin_comment
comment|/*  * Function: proc_iterate_over_mappings  *  * Given a pointer to a function, call that function once for every  * mapped address space in the process.  The callback function   * receives an open file descriptor for the file corresponding to  * that mapped address space (if there is one), and the base address  * of the mapped space.  Quit when the callback function returns a  * nonzero value, or at teh end of the mappings.  *  * Returns: the first non-zero return value of the callback function,  * or zero.  */
end_comment

begin_comment
comment|/* FIXME: it's probably a waste to cache this FD.     It doesn't get called that often... and if I open it    every time, I don't need to lseek it.  */
end_comment
...
name|pi
argument_list|,
literal|"proc_iterate_over_mappings (open)"
argument_list|,
name|__LINE__
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Allocate space for mapping (lifetime only for this function). */
name|map
operator|=
name|alloca
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Now read the mappings from the file,       open a file descriptor for those that have a name,       and call the callback function.  */
while|while
condition|(
name|read
argument_list|(
...
block|}
comment|/* Stop looping if the callback returns non-zero.  */
if|if
condition|(
operator|(
name|funcstat
...
directive|else
comment|/* Get the number of mapping entries.  */
if|if
condition|(
name|ioctl
argument_list|(
...
name|pi
argument_list|,
literal|"proc_iterate_over_mappings (PIOCNMAP)"
argument_list|,
name|__LINE__
argument_list|)
expr_stmt|;
comment|/* Allocate space for mappings (lifetime only this function).  */
name|map
operator|=
operator|(
expr|struct
...
argument_list|)
expr_stmt|;
comment|/* Read in all the mappings.  */
if|if
condition|(
name|ioctl
argument_list|(
...
name|pi
argument_list|,
literal|"proc_iterate_over_mappings (PIOCMAP)"
argument_list|,
name|__LINE__
argument_list|)
expr_stmt|;
comment|/* Now loop through the mappings, open an fd for each, and      call the callback function.  */
for|for
control|(
name|i
operator|=
...
name|perror_with_name
argument_list|(
literal|"procfs: malloc failed in update_threads"
argument_list|)
expr_stmt|;
name|old_chain
operator|=
...
argument_list|)
expr_stmt|;
comment|/* If the event thread is not the same as GDB's requested thread       (ie. inferior_pid), then look up procinfo for the requested       thread.  */
if|if
condition|(
operator|(
name|tid
...
argument_list|)
expr_stmt|;
comment|/* If current lwp for process is not the same as requested thread      (ie. inferior_pid), then find procinfo for the requested thread.  */
if|if
condition|(
operator|(
name|tid
...
name|SYS_sproc
case|:
comment|/* Nothing to do here for now.  The old procfs 		     seemed to use this event to handle threads on 		     older (non-LWP) systems, where I'm assuming that 		     threads were actually separate processes.  Irix, 		     maybe?  Anyway, low priority for now.  */
endif|#
directive|endif
ifdef|#
directive|ifdef
...
name|SYS_fork
case|:
comment|/* FIXME: do we need to handle this?  Investigate.  */
endif|#
directive|endif
ifdef|#
directive|ifdef
...
argument_list|)
expr_stmt|;
comment|/* In addition, it's possible that this is the first 		   * new thread we've seen, in which case we may not  		   * have created entries for inferior_pid yet. 		   */
if|if
condition|(
name|TIDGET
argument_list|(
...
block|proc_warn (pi, "target_resume, set_gregs", 		   __LINE__);
ifdef|#
directive|ifdef
name|FP0_REGNUM
block|if (pi->fpregs_dirty)     if (parent == NULL || 	proc_get_current_thread (parent) != pi->tid)       if (!proc_set_fpregs (pi))
...
begin_ifdef
unit|static int make_signal_thread_runnable (process, pi, ptr)      procinfo *process;      procinfo *pi;      void     *ptr; {
ifdef|#
directive|ifdef
name|PR_ASLWP
end_ifdef
...
name|warning
argument_list|(
literal|"procfs: resume can't find thread %d -- resuming all."
argument_list|,
name|TIDGET
argument_list|(
name|pid
...
begin_comment
comment|/*  * Function: procfs_find_LDT_entry   *  * Input:  *   int pid;	// The GDB-style pid-plus-LWP.  *  * Return:  *   pointer to the corresponding LDT entry.  */
end_comment
...
name|warning
argument_list|(
literal|"procfs_find_LDT_entry: could not find procinfi for %d."
argument_list|,
name|pid
argument_list|)
expr_stmt|;
...
name|warning
argument_list|(
literal|"procfs_find_LDT_entry: could not read gregs for %d."
argument_list|,
name|pid
argument_list|)
expr_stmt|;
...
name|printf
argument_list|(
literal|"File descriptor %d, base address 0x%08x\n"
argument_list|,
name|fd
argument_list|,
name|core_addr
...
name|printf
argument_list|(
literal|"iterate_over_mappings returned %d.\n"
argument_list|,
name|ret
argument_list|)
expr_stmt|;
...
name|info_proc_cmd
argument_list|,
literal|"Show /proc process information about any running process.\ Default is the process being debugged."
argument_list|)
expr_stmt|;
name|add_com
argument_list|(
...
name|test_mapping_cmd
argument_list|,
literal|"test iterate-over-mappings"
argument_list|)
expr_stmt|;
block|}
end_function
