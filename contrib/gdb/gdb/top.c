...
expr_stmt|;
block|}
comment|/* Add 1 because we eventually want the next line.  */
name|operate_saved_history
operator|=
name|where_history
argument_list|()
...
begin_comment
comment|/*  * int get_prompt_1 (char * buf);  *  * Work-horse for get_prompt (called via catch_errors).  * Argument is buffer to hold the formatted prompt.  *  * Returns: 1 for success (use formatted prompt)  *          0 for failure (use gdb_prompt_string).  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|gdb_prompt_escape
decl_stmt|;
end_decl_stmt
...
literal|0
return|;
comment|/* do no formatting */
block|}
else|else
comment|/* formatted prompt */
block|{
name|char
name|fmt
index|[
...
literal|'\0'
expr_stmt|;
comment|/* assume null format string */
if|if
condition|(
name|promptp
index|[
...
name|gdb_prompt_escape
condition|)
comment|/* double esc char */
block|{
name|promptp
operator|+=
literal|2
expr_stmt|;
comment|/* skip past two escape chars. */
block|}
else|else
block|{
comment|/* extract format string from between two esc chars */
name|i
operator|=
literal|0
expr_stmt|;
...
name|error
argument_list|(
literal|"Syntax error at prompt position %d"
argument_list|,
name|promptp
operator|-
name|local_prompt
...
operator|++
expr_stmt|;
comment|/* skip second escape char */
name|fmt
index|[
name|i
operator|++
...
literal|'\0'
expr_stmt|;
comment|/* terminate the format string */
block|}
block|}
name|arg_val
operator|=
...
expr_stmt|;
comment|/* overflow protect */
comment|/* FIXME: how to protect GDB from crashing 		         from bad user-supplied format string? */
if|if
condition|(
name|fmt
index|[
...
condition|)
block|{
comment|/* display it as a string */
name|char
modifier|*
name|default_fmt
init|=
...
literal|0
decl_stmt|;
comment|/* Limiting the number of bytes that the following call 		         will read protects us from sprintf overflow later. */
name|i
operator|=
...
literal|'\0'
expr_stmt|;
comment|/* force-terminate string */
comment|/* FIXME: how to protect GDB from crashing 		         from bad user-supplied format string? */
name|sprintf
argument_list|(
name|outp
...
block|{
comment|/* display it as a pointer */
name|char
modifier|*
name|default_fmt
init|=
...
case|:
block|{
comment|/* no default format for bool */
name|longval
operator|=
name|value_as_long
argument_list|(
...
case|:
block|{
comment|/* no default format for enum */
name|longval
operator|=
name|value_as_long
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* find enum name if possible */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* in casel available< strlen (name), */
name|outp
index|[
name|available
index|]
...
expr_stmt|;
break|break;
comment|/* void type -- no output */
default|default:
name|error
argument_list|(
literal|"bad data type at prompt position %d"
argument_list|,
name|promptp
operator|-
name|local_prompt
...
literal|'\0'
expr_stmt|;
comment|/* terminate prompt string */
return|return
literal|1
return|;
block|}
...
index|]
return|;
comment|/* successful formatted prompt */
block|}
else|else
block|{
comment|/* Prompt could not be formatted.  */
if|if
condition|(
name|event_loop_p
condition|)
...
name|gdb_prompt_escape
argument_list|,
literal|"Set escape character for formatting of gdb's prompt"
argument_list|,
operator|&
name|setlist
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Install the default UI */
if|if
condition|(
operator|!
name|init_ui_hook
...
argument_list|)
expr_stmt|;
comment|/* All the interpreters should have had a look at things by now. 	 Initialize the selected interpreter. */
if|if
condition|(
name|interpreter_p
condition|)
