...
begin_comment
comment|/* Support routines for manipulating internal types for GDB.    Copyright 1992, 1993, 1994, 1995, 1996, 1998, 1999, 2000, 2001, 2002    Free Software Foundation, Inc.    Contributed by Cygnus Support, using pieces from other GDB modules.     This file is part of GDB.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 59 Temple Place - Suite 330,    Boston, MA 02111-1307, USA.  */
end_comment
...
begin_endif
unit|static void cfront_mangle_name (struct type *, int, int);
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* Make an address-space-delimited variant of a type -- a type that    is identical to the one supplied except that it has an address    space attribute attached to it (such as "code" or "data").     This is for Harvard architectures. */
end_comment
...
condition|)
do|;
comment|/* Create a new, duplicate type. */
name|ntype
operator|=
name|alloc_type
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Pointers or references to the original type are not relevant to      the new type; but if the original type is a pointer, the new type      points to the same thing (so TYPE_TARGET_TYPE remains unchanged). */
name|TYPE_POINTER_TYPE
argument_list|(
name|ntype
argument_list|)
...
name|ntype
expr_stmt|;
comment|/* Chain the new address-space-specific type to the old type. */
name|ntype
operator|->
name|as_type
operator|=
...
name|ntype
expr_stmt|;
comment|/* Now set the address-space flag, and return the new type. */
name|ntype
operator|->
name|flags
operator||=
...
name|ntype
expr_stmt|;
comment|/* Need new address-space kind. */
...
begin_comment
comment|/* When reading in a class type, we may have created references to    cv-qualified versions of the type (in method arguments, for    instance).  Update everything on the cv ring from the primary    type TYPE.     The only reason we do not need to do the same thing for address    spaces is that type readers do not create address space qualified    types.  */
end_comment
...
operator|)
expr_stmt|;
comment|/* If any reference or pointer types were created, save them too.  */
name|ptr_type
operator|=
name|TYPE_POINTER_TYPE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Don't disturb the CV chain.  */
name|cv_type
operator|=
name|TYPE_CV_TYPE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Verify that we haven't added any address-space qualified types, 	 for the future.  */
name|gdb_assert
argument_list|(
name|ntype
operator|==
...
begin_comment
comment|/* Replace the contents of ntype with the type *type.     This function should not be necessary, but is due to quirks in the stabs    reader.  This should go away.  It does not handle the replacement type    being cv-qualified; it could be easily fixed to, but it should go away,    remember?  */
end_comment
...
name|f
decl_stmt|;
comment|/* Build the field structure.  */
name|f
operator|=
name|xmalloc
argument_list|(
...
name|elt_name
expr_stmt|;
comment|/* Build a struct type with that field.  */
name|t
operator|=
name|init_type
argument_list|(
...
directive|if
literal|0
block|union __gdb_builtin_type_vec128   {     struct __builtin_v16qi v16qi;     struct __builtin_v8hi v8hi;     struct __builtin_v4si v4si;     struct __builtin_v4sf v4sf;     uint128_t uint128;   };
endif|#
directive|endif
name|struct
name|type
...
literal|"v16qi"
expr_stmt|;
comment|/* Build a union type with those fields.  */
name|t
operator|=
name|init_type
argument_list|(
...
begin_comment
unit|void cfront_mangle_name (struct type *type, int i, int j) {   struct fn_field *f;   char *mangled_name = gdb_mangle_name (type, i, j);    f = TYPE_FN_FIELDLIST1 (type, i);
...
begin_endif
unit|xasprintf (&arm_mangled_name, "%s%s", mangled_name, extras.str);       xfree (mangled_name);       mangled_name = arm_mangled_name;     } }
endif|#
directive|endif
end_endif
