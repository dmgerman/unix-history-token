...
begin_comment
comment|/* Add a new piece to CTX's piece list.  */
end_comment
...
literal|1
expr_stmt|;
comment|/* Default is initialized.  */
while|while
condition|(
name|op_ptr
operator|<
...
argument_list|)
expr_stmt|;
comment|/* Pop off the address/regnum, and clear the in_reg flag.  */
name|dwarf_expr_pop
argument_list|(
name|ctx
argument_list|)
...
name|_
argument_list|(
literal|"DWARF-2 expression error: DW_OP_GNU_unint must always "
literal|"be the very last op."
argument_list|)
argument_list|)
expr_stmt|;
name|ctx
