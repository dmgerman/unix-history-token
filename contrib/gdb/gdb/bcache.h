...
begin_comment
comment|/* Find a copy of the LENGTH bytes at ADDR in BCACHE.  If BCACHE has    never seen those bytes before, add a copy of them to BCACHE.  In    either case, return a pointer to BCACHE's copy of that string.    Since the cached value is ment to be read-only, return a const    buffer.  */
end_comment
...
begin_comment
comment|/* Free all the storage used by BCACHE.  */
end_comment
...
begin_comment
comment|/* Create a new bcache object.  */
end_comment
