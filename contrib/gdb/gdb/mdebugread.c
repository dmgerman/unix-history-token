...
name|TYPE_CODE_UNDEF
condition|)
comment|/* If the type of the member is Nil (or Void), 		     without qualifiers, assume the tag is an 		     enumeration. 		     Alpha cc -migrate enums are recognized by a zero 		     index and a zero symbol value.  */
if|if
condition|(
name|tsym
operator|.
...
name|stBlock
case|:
comment|/* It is an enumerated type if the next symbol entry is a stMember 	 and its auxiliary index is indexNil or its auxiliary entry 	 is a plain btNil or btVoid. 	 Alpha cc -migrate enums are recognized by a zero index and 	 a zero symbol value.  */
call|(
modifier|*
name|swap_sym_in
call|)
