...
comment|/* For stProc symbol records, we need to check the storage class          as well, as only (stProc, scText) entries represent "real"          procedures - See the Compaq document titled "Object File /          Symbol Table Format Specification" for more information.          If the storage class is not scText, we discard the whole block          of symbol records for this stProc.  */
if|if
condition|(
name|sh
operator|->
...
name|stEnd
case|:
comment|/* C++ encodes class types as structures where there the                    methods are encoded as stProc. The scope of stProc                    symbols also ends with stEnd, thus creating a risk of                    taking the wrong stEnd symbol record as the end of                    the current struct, which would cause GDB to undercount                    the real number of fields in this struct.  To make sure                    we really reached the right stEnd symbol record, we                    check the associated name, and match it against the                    struct name.  Since method names are mangled while                    the class name is not, there is no risk of having a                    method whose name is identical to the class name                    (in particular constructor method names are different                    from the class name).  There is therefore no risk that                    this check stops the count on the StEnd of a method. 		    		   Also, assume that we're really at the end when tsym.iss 		   is 0 (issNull).  */
if|if
condition|(
name|tsym
operator|.
...
argument_list|)
decl_stmt|;
comment|/* Inhibit core dumps if TIR is corrupted.  */
if|if
condition|(
name|bs
operator|==
...
block|FDR *save_cur_fdr = cur_fdr;       s = lookup_symbol (sh_name, NULL, VAR_DOMAIN, 0, NULL);       cur_fdr = save_cur_fdr;
else|#
directive|else
name|s
operator|=
...
name|symfile_complaints
argument_list|,
literal|"bad ifd for external symbol: %d (max %ld)"
argument_list|,
name|ext_in
operator|->
name|ifd
...
name|stProc
case|:
comment|/* Ignore all parameter symbol records.  */
if|if
condition|(
name|sh
operator|.
...
begin_comment
comment|/* Get the next symbol.  OBJFILE is unused. */
end_comment
...
condition|)
block|{
comment|/* If we found a trailing N_SO with no name, process                      it here instead of in process_one_symbol, so we                      can keep a handle to its symtab.  The symtab                      would otherwise be ended twice, once in                      process_one_symbol, and once after this loop. */
if|if
condition|(
name|type_code
operator|==
...
begin_comment
comment|/* Add a new symbol S to a block B.  */
end_comment
...
begin_comment
comment|/* Allocate a new symtab for NAME.  Needs an estimate of how many    linenumbers MAXLINES we'll put in it */
end_comment
...
begin_comment
comment|/* Allocate and zero a new block, and set its BLOCK_DICT.  If function    is non-zero, assume the block is associated to a function, and make    sure that the symbols are stored linearly; otherwise, store them    hashed.  */
end_comment
...
parameter_list|)
block|{
comment|/* FIXME: carlton/2003-09-11: This should use allocate_block to      allocate the block.  Which, in turn, suggests that the block      should be allocated on an obstack.  */
name|struct
name|block
modifier|*
name|retval
...
name|back_to
decl_stmt|;
comment|/* FIXME: It's not clear whether we should be getting minimal symbol      information from .mdebug in an ELF file, or whether we will.      Re-initialize the minimal symbol reader in case we do.  */
