...
begin_comment
comment|/* defines needed for GDBSERVER */
end_comment
...
begin_comment
comment|/* Some pseudo register numbers */
end_comment
...
name|N
parameter_list|)
value|(((N) * 8) \   + ((N)<= IA64_FR0_REGNUM ? 0 : 8 * (((N)> IA64_FR127_REGNUM) ? 128 : (N) - IA64_FR0_REGNUM)))
end_define
...
parameter_list|)
define|\
value|((IA64_FR0_REGNUM<= (N)&& (N)<= IA64_FR127_REGNUM) ? 16 : 8)
end_define
...
define|#
directive|define
name|GDBSERVER_RESUME_REGS
value|{ IA64_IP_REGNUM, IA64_PSR_REGNUM, SP_REGNUM, IA64_BSP_REGNUM, IA64_CFM_REGNUM }
end_define
