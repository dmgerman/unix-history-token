...
begin_comment
comment|/* Multi-threaded debugging support for GNU/Linux (LWP layer).    Copyright 2000, 2001 Free Software Foundation, Inc.     This file is part of GDB.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 59 Temple Place - Suite 330,    Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Structure describing a LWP.  */
end_comment
...
name|lwp_info
block|{
comment|/* The process id of the LWP.  This is a combination of the LWP id      and overall process id.  */
name|ptid_t
name|ptid
decl_stmt|;
comment|/* Non-zero if this LWP is cloned.  In this context "cloned" means      that the LWP is reporting to its parent using a signal other than      SIGCHLD.  */
name|int
name|cloned
decl_stmt|;
comment|/* Non-zero if we sent this LWP a SIGSTOP (but the LWP didn't report      it back yet).  */
name|int
name|signalled
decl_stmt|;
comment|/* Non-zero if this LWP is stopped.  */
name|int
name|stopped
decl_stmt|;
comment|/* Non-zero if this LWP will be/has been resumed.  Note that an LWP      can be marked both as stopped and resumed at the same time.  This      happens if we try to resume an LWP that has a wait status      pending.  We shouldn't let the LWP run until that wait status has      been processed, but we should not report that wait status if GDB      didn't try to let the LWP run.  */
name|int
name|resumed
decl_stmt|;
comment|/* If non-zero, a pending wait status.  */
name|int
name|status
decl_stmt|;
comment|/* Non-zero if we were stepping this LWP.  */
name|int
name|step
decl_stmt|;
comment|/* Next LWP in list.  */
name|struct
name|lwp_info
modifier|*
name|next
...
begin_comment
comment|/* Implementation of the PREPARE_TO_PROCEED hook for the GNU/Linux LWP    layer.     Note that this implementation is potentially redundant now that    default_prepare_to_proceed() has been added.     FIXME This may not support switching threads after Ctrl-C    correctly. The default implementation does support this. */
end_comment
...
condition|)
block|{
comment|/* Switched over from TRAP_PID.  */
name|CORE_ADDR
name|stop_pc
init|=
name|read_pc
...
name|trap_pc
decl_stmt|;
comment|/* Avoid switching where it wouldn't do any good, i.e. if both          threads are at the same breakpoint.  */
name|trap_pc
operator|=
name|read_pc_pid
argument_list|(
...
condition|)
block|{
comment|/* User hasn't deleted the breakpoint.  Return non-zero, and              switch back to TRAP_PID.  */
name|inferior_ptid
operator|=
name|trap_ptid
expr_stmt|;
comment|/* FIXME: Is this stuff really necessary?  */
...
name|gdb_stdlog
argument_list|,
literal|"Pending %s for LWP %ld on detach.\n"
argument_list|,
name|strsignal
argument_list|(
name|WSTOPSIG
...
begin_comment
comment|/* Return non-zero if TP corresponds to the LWP specified by DATA    (which is assumed to be a pointer to a `struct lwp_info'.  */
end_comment

begin_function
specifier|static
name|int
name|find_lwp_callback
parameter_list|(
name|struct
name|thread_info
modifier|*
...
directive|if
literal|0
comment|/* FIXME: kettenis/2000-08-26: This should really be handled          properly by core GDB.  */
block|tp = find_thread_pid (lp->ptid);       if (tp == NULL) 	tp = iterate_over_threads (find_lwp_callback, lp);       gdb_assert (tp);
comment|/* If we were previously stepping the thread, and now continue          the thread we must invalidate the stepping range.  However,          if there is a step_resume breakpoint for this thread, we must          preserve the stepping range to make it possible to continue          stepping once we hit it.  */
block|if (tp->step_range_end&& tp->step_resume_breakpoint == NULL) 	{ 	  gdb_assert (lp->step); 	  tp->step_range_start = tp->step_range_end = 0; 	}
endif|#
directive|endif
name|child_resume
argument_list|(
...
name|resume_all
decl_stmt|;
comment|/* Apparently the interpretation of PID is dependent on STEP: If      STEP is non-zero, a specific PID means `step only this process      id'.  But if STEP is zero, then PID means `continue *all*      processes, but give the signal only to this one'.  */
name|resume_all
operator|=
operator|(
name|PIDGET
...
name|ECHILD
condition|)
comment|/* OK, the proccess has disappeared.  We'll catch the actual 	   exit event in lin_lwp_wait.  */
return|return
literal|0
return|;
name|gdb_assert
...
name|gdb_stderr
argument_list|,
literal|"SWC: Candidate SIGTRAP event in %ld\n"
argument_list|,
name|GET_LWP
argument_list|(
name|lp
...
name|gdb_stderr
argument_list|,
literal|"SWC: Pending event %d in %ld\n"
argument_list|,
name|WSTOPSIG
argument_list|(
name|status
...
name|gdb_stdlog
argument_list|,
literal|"Push back breakpoint for LWP %ld\n"
argument_list|,
name|GET_LWP
argument_list|(
name|lp
...
name|gdb_stdlog
argument_list|,
literal|"Select single-step LWP %ld\n"
argument_list|,
name|GET_LWP
argument_list|(
name|event_lp
...
name|gdb_stdlog
argument_list|,
literal|"Found %d SIGTRAP events, selecting #%d\n"
argument_list|,
name|num_events
argument_list|,
name|random_selector
...
name|gdb_stdlog
argument_list|,
literal|"Using pending wait status %s for LWP %ld.\n"
argument_list|,
name|status_to_str
argument_list|(
name|status
...
name|gdb_stdlog
argument_list|,
literal|"Waiting for specific LWP %ld.\n"
argument_list|,
name|GET_LWP
argument_list|(
name|ptid
...
block|}
block|}
comment|/* Make sure we don't report a TARGET_WAITKIND_EXITED or              TARGET_WAITKIND_SIGNALLED event if there are still LWP's              left in the process.  */
if|if
condition|(
operator|(
name|WIFEXITED
...
name|gdb_stdlog
argument_list|,
literal|"Delayed SIGSTOP caught for %s.\n"
argument_list|,
name|target_pid_to_str
argument_list|(
name|lp
...
name|gdb_stdlog
argument_list|,
literal|"Candidate event %s in LWP %ld.\n"
argument_list|,
name|status_to_str
argument_list|(
name|status
