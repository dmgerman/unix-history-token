...
specifier|static
name|void
name|unknown_attribute_form_complaint
parameter_list|(
name|int
name|arg1
parameter_list|,
...
specifier|static
name|void
name|dup_user_type_definition_complaint
parameter_list|(
name|int
name|arg1
parameter_list|,
...
specifier|static
name|void
name|bad_array_element_type_complaint
parameter_list|(
name|int
name|arg1
parameter_list|,
...
begin_comment
comment|/*     LOCAL FUNCTION     free_utypes -- free the utypes array and reset pointer& count     SYNOPSIS     static void free_utypes (void *dummy)     DESCRIPTION     Called via do_cleanups to free the utypes array, reset the pointer to NULL,    and set numutypes back to zero.  This ensures that the utypes does not get    referenced after being freed.  */
end_comment
...
name|TAG_member
case|:
comment|/* Static fields can be either TAG_global_variable (GCC) or else 	     TAG_member with no location (Diab).  We could treat the latter like 	     the former... but since we don't support the former, just avoid 	     crashing on the latter for now.  */
if|if
condition|(
name|mbr
operator|.
...
name|symfile_complaints
argument_list|,
literal|"DIE @ 0x%x \"%s\", OP_DEREF2 address 0x%lx not handled"
argument_list|,
name|DIE_ID
argument_list|,
name|DIE_NAME
...
name|symfile_complaints
argument_list|,
literal|"DIE @ 0x%x \"%s\", OP_DEREF4 address 0x%lx not handled"
argument_list|,
name|DIE_ID
argument_list|,
name|DIE_NAME
...
directive|if
literal|0
comment|/* This part of the test would always be true, and it triggers a compiler    warning.  */
then|&& (unsigned char) modifier<= MOD_hi_user))
endif|#
directive|endif
block|{
name|complaint
...
name|symfile_complaints
argument_list|,
literal|"DIE @ 0x%x \"%s\", malformed DIE, bad length (%ld bytes)"
argument_list|,
name|DIE_ID
argument_list|,
name|DIE_NAME
