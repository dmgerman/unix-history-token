...
begin_comment
unit|static int there_is_a_visible_common_named (char *comname) {   SAVED_F77_COMMON_PTR the_common;   struct frame_info *fi;   register char *funname = 0;   struct symbol *func;    if (comname == NULL)     error ("Cannot deal with NULL common name!");    fi = selected_frame;    if (fi == NULL)     error ("No frame selected");
comment|/* The following is generally ripped off from stack.c's routine       print_frame_info() */
end_comment
