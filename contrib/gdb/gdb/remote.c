...
begin_comment
comment|/* Remote target communications for serial-line targets in custom GDB protocol     Copyright 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996,    1997, 1998, 1999, 2000, 2001, 2002 Free Software Foundation, Inc.     This file is part of GDB.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 59 Temple Place - Suite 330,    Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Description of the remote protocol.  Strictly speeking, when the    target is open()ed, remote.c should create a per-target description    of the remote protocol using that target's architecture.    Unfortunatly, the target stack doesn't include local state.  For    the moment keep the information in the target's architecture    object.  Sigh..  */
end_comment
...
comment|/* long size in bytes;  == REGISTER_RAW_SIZE (regnum); at present.  */
...
argument_list|)
decl_stmt|;
comment|/* Start out by having the remote protocol mimic the existing      behavour - just copy in the description of the register cache.  */
name|rs
operator|->
name|sizeof_g_packet
operator|=
...
operator|)
expr_stmt|;
comment|/* ...size = REGISTER_RAW_SIZE (regnum); */
...
begin_comment
comment|/* This is set by the target (thru the 'S' message)    to denote that the target is in kernel mode.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|cisco_kernel_mode
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  * Cisco version of section offsets:  *  * Instead of having GDB query the target for the section offsets,  * Cisco lets the target volunteer the information!  It's also in  * a different format, so here are the functions that will decode  * a section offset packet from a Cisco target.  */
end_comment

begin_comment
comment|/*   * Function: remote_cisco_section_offsets  *  * Returns:  zero for success, non-zero for failure   */
end_comment

begin_function
specifier|static
name|int
name|remote_cisco_section_offsets
parameter_list|(
name|bfd_vma
name|text_addr
parameter_list|,
...
literal|1
return|;
comment|/* Can't find "_start" symbol */
name|data_base
operator|=
name|bss_base
operator|=
...
name|gdb_stdlog
argument_list|,
literal|"Reloc offset: text = 0x%s data = 0x%s bss = 0x%s\n"
argument_list|,
name|paddr_nz
argument_list|(
operator|*
...
begin_comment
comment|/*  * Function: remote_cisco_objfile_relocate  *  * Relocate the symbol file for a remote target.   */
end_comment
...
condition|)
block|{
comment|/* FIXME: This code assumes gdb-stabs.h is being used; it's          broken for xcoff, dwarf, sdb-coff, etc.  But there is no          simple canonical representation for this stuff.  */
name|offs
operator|=
operator|(
expr|struct
...
name|bss_off
expr_stmt|;
comment|/* First call the standard objfile_relocate.  */
name|objfile_relocate
argument_list|(
name|symfile_objfile
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Now we need to fix up the section entries already attached to          the exec target.  These entries will control memory transfers          from the exec file.  */
...
begin_comment
comment|/* Same as remote_detach, but with async support. */
end_comment
...
condition|)
block|{
comment|/* Export Cisco kernel mode as a convenience variable 	         (so that it can be used in the GDB prompt if desired). */
if|if
condition|(
name|cisco_kernel_mode
operator|==
...
literal|'N'
case|:
comment|/* Cisco special: status and offsets */
block|{
name|bfd_vma
name|text_addr
decl_stmt|,
...
name|warning
argument_list|(
literal|"Relocation packet received with no symbol file.  \ Packet Dropped"
argument_list|)
expr_stmt|;
goto|goto
name|got_status
goto|;
block|}
comment|/* Relocate object file.  Buffer format is NAATT;DD;BB 	     * where AA is the signal number, TT is the new text 	     * address, DD * is the new data address, and BB is the 	     * new bss address.  */
name|p
operator|=
operator|&
name|buf
...
name|warning
argument_list|(
literal|"Relocation packet recieved with no symbol file.  \ Packet Dropped"
argument_list|)
expr_stmt|;
goto|goto
name|got_status
...
name|PACKET_SUPPORT_UNKNOWN
case|:
comment|/* NOTE: This isn't rs->sizeof_g_packet because here, we are          forcing the register cache to read its and not the target          registers.  */
...
argument_list|)
expr_stmt|;
comment|/* Determine the max packet size. */
name|max_buf_size
operator|=
name|get_memory_write_packet_size
argument_list|()
...
begin_comment
unit|void remote_search (int len, char *data, char *mask, CORE_ADDR startaddr, 	       int increment, CORE_ADDR lorange, CORE_ADDR hirange, 	       CORE_ADDR *addr_found, char *data_found) {   if (increment == -4&& len == 4)     {       long mask_long, data_long;       long data_found_long;       CORE_ADDR addr_we_found;       char *buf = alloca (rs->remote_packet_size);       long returned_long[2];       char *p;        mask_long = extract_unsigned_integer (mask, len);       data_long = extract_unsigned_integer (data, len);       sprintf (buf, "t%x:%x,%x", startaddr, data_long, mask_long);       putpkt (buf);       getpkt (buf, (rs->remote_packet_size), 0);       if (buf[0] == '\0') 	{
...
block|{
comment|/* Cisco's run-length encoding variant uses two  		   hex chars to represent the repeat count. */
name|c
operator|=
name|readchar
argument_list|(
...
begin_comment
comment|/* FIXME: This function should be static and a member of the remote    target vector. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* minimum outbuf size is (rs->remote_packet_size) - if bufsiz is not large enough let       the caller know and return what the minimum size is   */
...
begin_comment
comment|/*  * Command: info remote-process  *  * This implements Cisco's version of the "info proc" command.  *  * This query allows the target stub to return an arbitrary string  * (or strings) giving arbitrary information about the target process.  * This is optional; the target stub isn't required to implement it.  *  * Syntax: qfProcessInfo        request first string  *         qsProcessInfo        request subsequent string  * reply:  'O'<hex-encoded-string>  *         'l'                  last reply (empty)  */
end_comment

begin_function
specifier|static
name|void
name|remote_info_process
parameter_list|(
name|char
modifier|*
name|args
...
condition|)
return|return;
comment|/* Silently: target does not support this feature. */
if|if
condition|(
name|buf
index|[
...
name|error
argument_list|(
literal|"info proc: target error."
argument_list|)
expr_stmt|;
while|while
condition|(
...
begin_comment
comment|/*  * Target Cisco   */
end_comment

begin_function
specifier|static
name|void
name|remote_cisco_open
parameter_list|(
name|char
modifier|*
name|name
...
argument_list|(
literal|"To open a remote debug connection, you need to specify what \n"
literal|"device is attached to the remote system (e.g. host:port)."
argument_list|)
expr_stmt|;
comment|/* See FIXME above */
name|wait_forever_enabled_p
...
argument_list|)
expr_stmt|;
comment|/*    * If a baud rate was specified on the gdb  command line it will    * be greater than the initial value of -1.  If it is, use it otherwise    * default to 9600    */
name|baud_rate
operator|=
operator|(
name|baud_rate
...
argument_list|)
expr_stmt|;
comment|/* Switch to using cisco target now */
name|init_all_packet_configs
argument_list|()
expr_stmt|;
name|general_thread
...
argument_list|)
expr_stmt|;
comment|/* Start the remote connection; if error (0), discard this target. */
if|if
condition|(
operator|!
name|catch_errors
...
block|}
name|minitelnet_return
enum|;
end_enum

begin_comment
comment|/* Shared between readsocket() and readtty().  The size is arbitrary,    however all targets are known to support a 400 character packet.  */
end_comment
...
block|{
comment|/* This is a '|', but not the fourth in a row.  				   Continue without echoing it.  If it isn't actually  				   one of four in a row, it'll be echoed later.  */
continue|continue;
block|}
block|}
else|else
comment|/* Not a '|' */
block|{
comment|/* Ensure any pending '|'s are flushed.  */
for|for
control|(
init|;
name|escape_count
...
literal|1
condition|)
comment|/* Check for echo of user input.  */
block|{
if|if
condition|(
name|tty_input
...
operator|++
expr_stmt|;
comment|/* Character matched user input: */
continue|continue;
comment|/* Continue without echoing it.  */
block|}
elseif|else
if|if
condition|(
...
condition|)
block|{
comment|/* End of the line (and of echo checking).  */
name|echo_check
operator|=
operator|-
literal|1
...
continue|continue;
comment|/* Continue without echoing.  */
block|}
else|else
block|{
comment|/* Failed check for echo of user input. 				   We now have some suppressed output to flush!  */
name|int
name|j
decl_stmt|;
for|for
...
argument_list|)
expr_stmt|;
comment|/* Default case: output the char.  */
block|}
if|if
condition|(
name|data
...
name|SERIAL_TIMEOUT
condition|)
comment|/* Timeout returned from readchar.  */
return|return
name|READ_MORE
return|;
comment|/* Try to read some more */
else|else
return|return
name|FATAL_ERROR
return|;
comment|/* Trouble, bail out */
block|}
end_function
...
return|;
block|}
comment|/* Remove a quoted newline.  */
if|if
condition|(
name|tty_input
index|[
...
literal|'\\'
condition|)
comment|/* line ending in backslash */
block|{
name|tty_input
index|[
operator|--
...
literal|'\r'
expr_stmt|;
comment|/* If the line consists of a ~, enter debugging mode.  */
if|if
condition|(
operator|(
name|tty_input
...
name|tablesize
decl_stmt|;
comment|/* max number of FDs for select */
name|int
name|status
decl_stmt|;
name|int
...
name|escape_count
decl_stmt|;
comment|/* global shared by readsocket */
specifier|extern
name|int
name|echo_check
decl_stmt|;
...
name|error
argument_list|(
literal|"Debugging terminated by communications error"
argument_list|)
expr_stmt|;
block|}
elseif|else
...
name|error
argument_list|(
literal|"Communications error on select %d"
argument_list|,
name|errno
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Debugging session terminated by protocol error"
argument_list|)
expr_stmt|;
block|}
name|putpkt
...
name|to_longname
operator|=
literal|"Remote serial target in cisco-specific protocol"
expr_stmt|;
name|remote_cisco_ops
operator|.
name|to_doc
operator|=
literal|"Use a remote machine via TCP, using a cisco-specific protocol.\n\ Specify the serial device it is connected to (e.g. host:2020)."
expr_stmt|;
name|remote_cisco_ops
operator|.
name|to_open
...
name|add_info
argument_list|(
literal|"remote-process"
argument_list|,
name|remote_info_process
argument_list|,
literal|"Query the remote system for process info."
argument_list|)
expr_stmt|;
name|add_packet_config_cmd
argument_list|(
