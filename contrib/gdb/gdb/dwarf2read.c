...
begin_comment
comment|/* No ops in location in expression, 				   so object was optimized out.  */
end_comment
...
begin_comment
comment|/* Object's address is the sum of the 				   register specified by basereg, plus 				   the offset returned.  */
end_comment
...
begin_comment
comment|/* Value described by flags above is 				   the address of a pointer to the object.  */
end_comment
...
begin_comment
comment|/* Variable is at the returned offset 				   from the frame start, but there's 				   no identified frame pointer for 				   this function, so we can't say 				   which register it's relative to; 				   use LOC_LOCAL.  */
end_comment
...
name|struct
name|complaint
name|dwarf2_complex_location_expr
init|=
block|{
...
begin_endif
unit|static void dwarf2_build_psymtabs_easy (struct objfile *objfile, int mainline) {   bfd *abfd = objfile->obfd;   char *aranges_buffer, *pubnames_buffer;   char *aranges_ptr, *pubnames_ptr;   unsigned int entry_length, version, info_offset, info_size;    pubnames_buffer = dwarf2_read_section (objfile, 					 dwarf_pubnames_offset, 					 dwarf_pubnames_size);   pubnames_ptr = pubnames_buffer;   while ((pubnames_ptr - pubnames_buffer)< dwarf_pubnames_size)     {       struct comp_unit_head cu_header;       int bytes_read;        entry_length = read_initial_length (abfd, pubnames_ptr,&cu_header,&bytes_read);       pubnames_ptr += bytes_read;       version = read_1_byte (abfd, pubnames_ptr);       pubnames_ptr += 1;       info_offset = read_4_bytes (abfd, pubnames_ptr);       pubnames_ptr += 4;       info_size = read_4_bytes (abfd, pubnames_ptr);       pubnames_ptr += 4;     }    aranges_buffer = dwarf2_read_section (objfile, 					dwarf_aranges_offset, 					dwarf_aranges_size);  }
endif|#
directive|endif
end_endif
...
name|physname
decl_stmt|;
comment|/* C++ static member. 	 Get name of field.  */
name|attr
operator|=
name|dwarf_attr
argument_list|(
...
name|__LINE__
argument_list|,
literal|"read_address: bad switch, signed"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|__LINE__
argument_list|,
literal|"read_address: bad switch, unsigned"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|__LINE__
argument_list|,
literal|"read_offset: bad switch"
argument_list|)
expr_stmt|;
block|}
return|return
...
name|error
argument_list|(
literal|"DW_FORM_strp used without .debug_str section"
argument_list|)
expr_stmt|;
return|return
name|NULL
...
name|error
argument_list|(
literal|"DW_FORM_strp pointing outside of .debug_str section"
argument_list|)
expr_stmt|;
return|return
name|NULL
...
literal|1
expr_stmt|;
comment|/* Don't call record_line here.  The end_sequence 		     instruction provides the address of the first byte 		     *after* the last line in the sequence; it's not the 		     address of any real source line.  However, the GDB 		     linetable structure only records the starts of lines, 		     not the ends.  This is a weakness of GDB.  */
break|break;
case|case
...
begin_endif
unit|static struct type * type_at_offset (unsigned int offset, struct objfile *objfile) {   struct die_info *die;   struct type *type;    die = follow_die_ref (offset);   if (!die)     {       error ("Dwarf Error: Cannot find type referent at offset %d.", offset);       return NULL;     }   type = tag_type_to_type (die, objfile);   return type; }
endif|#
directive|endif
end_endif
...
literal|1
expr_stmt|;
comment|/* If we're not the last op, then we definitely can't encode 	     this using GDB's address_class enum.  */
if|if
condition|(
name|i
operator|<
