...
begin_comment
comment|/* FIXME: ezannoni/2004-02-13 Verify if the include below is really needed.  */
end_comment
...
decl_stmt|;
comment|/* Offset in data section to TOC anchor.  */
name|CORE_ADDR
name|toc_offset
decl_stmt|;
block|}
...
begin_endif
unit|static void add_stab_to_list (char *, struct pending_stabs **);
endif|#
directive|endif
end_endif
...
begin_comment
comment|/* Return the BFD section that CS points to.  */
end_comment
...
begin_comment
comment|/* Given a line table with function entries are marked, arrange its functions    in ascending order and strip off function entry markers and return it in    a newly created table. If the old one is good enough, return the old one. */
end_comment
...
parameter_list|)
define|\
value|{						\   char *namestr;				\   namestr = (NAME); \   if (namestr[0] == '.') ++namestr; \   prim_record_minimal_symbol_and_info (namestr, (ADDR), (TYPE), \ 				       (char *)NULL, (SECTION), (asection *)NULL, (OBJFILE)); \   misc_func_recorded = 1;					\ }
end_define
...
block|}
block|}
comment|/* If explicitly specified as a function, treat is as one.  This check 	 evaluates to true for @FIX* bigtoc CSECT symbols, so it must occur 	 after the above CSECT check.  */
if|if
condition|(
name|ISFCN
argument_list|(
...
parameter_list|)
define|\
value|(SYMBOL2) = (struct symbol *)		\   	obstack_alloc (&objfile->objfile_obstack, sizeof (struct symbol)); \   *(SYMBOL2) = *(SYMBOL1);
end_define
...
parameter_list|)
define|\
value|(ALLOCED) ? (NAME) : obsavestring ((NAME), strlen (NAME),&objfile->objfile_obstack);
end_define
...
begin_comment
comment|/* Extract the file name from the aux entry of a C_FILE symbol.    Result is in static storage and is only good for temporary use.  */
end_comment
...
condition|)
return|return;
comment|/* Allocate string table from objfile_obstack. We will need this table      as long as we have its symbol table around. */
name|strtbl
operator|=
operator|(
name|char
...
block|}
block|}
comment|/* Activate the misc_func_recorded mechanism for 		       compiler- and linker-generated CSECTs like ".strcmp" 		       and "@FIX1".  */
if|if
condition|(
name|namestring
operator|&&
...
name|n_value
expr_stmt|;
comment|/* Make TOC offset relative to start address of section.  */
name|bfd_sect
operator|=
name|secnum_to_bfd_section
argument_list|(
...
expr_stmt|;
block|}
comment|/* Record the toc offset value of this symbol table into objfile structure.      If no XMC_TC0 is found, toc_offset should be zero. Another place to obtain      this information would be file auxiliary header. */
operator|(
operator|(
expr|struct
name|coff_symfile_info
...
begin_comment
comment|/* Return the toc offset value for a given objfile.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Initialize the section indexes for future use. */
name|sect
operator|=
name|bfd_get_section_by_name
argument_list|(
