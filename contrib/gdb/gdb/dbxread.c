...
begin_comment
comment|/* Read dbx symbol tables and convert to internal format, for GDB.    Copyright 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995,    1996, 1997, 1998, 1999, 2000, 2001, 2002    Free Software Foundation, Inc.     This file is part of GDB.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2 of the License, or    (at your option) any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software    Foundation, Inc., 59 Temple Place - Suite 330,    Boston, MA 02111-1307, USA.  */
end_comment
...
name|symloc
block|{
comment|/* The start (inclusive) and end (exclusive) addresses for this        partial symtab's text.  STABS doesn't reliably give us nice        start and end addresses for each function.  Instead, we are        told the addresses of various boundary points, and we have to        gather those together to build ranges.  These are our running        best guess as to the range of text addresses for this psymtab.  */
name|CORE_ADDR
name|textlow
decl_stmt|,
name|texthigh
...
begin_endif
unit|static struct type ** explicit_lookup_type (int real_filenum, int index) {   register struct header_file *f =&HEADER_FILES (current_objfile)[real_filenum];    if (index>= f->length)     {       f->length *= 2;       f->vector = (struct type **) 	xrealloc (f->vector, f->length * sizeof (struct type *));       memset (&f->vector[f->length / 2], 	      '\0', f->length * sizeof (struct type *) / 2);     }   return&f->vector[index]; }
endif|#
directive|endif
end_endif
...
argument_list|)
expr_stmt|;
comment|/* Take the text ranges the STABS partial symbol scanner computed      for each of the psymtabs and convert it into the canonical form      for psymtabs.  */
block|{
name|struct
name|partial_symtab
modifier|*
...
name|cont_elem
block|{
comment|/* sym and stabstring for continuing information in cfront */
name|struct
name|symbol
modifier|*
name|sym
...
name|stabs
decl_stmt|;
comment|/* state dependencies (statics that must be preserved) */
name|int
name|sym_idx
decl_stmt|;
name|int
...
parameter_list|)
function_decl|;
comment|/* other state dependencies include:        (assumption is that these will not change since process_now FIXME!!)        stringtab_global        n_stabs        objfile        symfile_bfd */
block|}
struct|;
end_struct
...
name|struct
name|complaint
name|function_outside_compilation_unit
init|=
block|{
...
directive|ifdef
name|SOFUN_ADDRESS_MAYBE_MISSING
comment|/* Solaris 2 end of module, finish current partial symbol table. 	     end_psymtab will set TEXTHIGH (pst) to the proper value, which 	     is necessary if a module compiled without debugging info 	     follows this module.  */
if|if
condition|(
name|pst
condition|)
...
name|NULL
expr_stmt|;
comment|/* In a Solaris elf file, this variable, which comes from the      value of the N_SO symbol, will still be 0.  Luckily, text_offset,      which comes from TEXTLOW (pst) is correct. */
if|if
condition|(
...
name|text_offset
expr_stmt|;
comment|/* In reordered executables last_source_start_addr may not be the      lower bound for this symtab, instead use text_offset which comes      from TEXTLOW (pst) which is correct.  */
if|if
condition|(
name|last_source_start_addr
operator|>
...
argument_list|)
expr_stmt|;
comment|/* Process items which we had to "process_later" due to dependencies       on other stabs.  */
name|process_now
argument_list|(
name|objfile
argument_list|)
...
name|last_function_start
decl_stmt|;
comment|/* If this is nonzero, we've seen an N_SLINE since the start of the current      function.  Initialized to nonzero to assure that last_function_start      is never used uninitialized.  */
specifier|static
name|int
