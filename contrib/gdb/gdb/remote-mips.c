...
begin_comment
unit|static int pmon_insert_breakpoint (CORE_ADDR addr, char *contents_cache) {   int status;    if (monitor_supports_breakpoints)     {       char tbuff[12];
...
begin_comment
unit|if (mips_exit_debug ()) 	mips_error ("Failed to exit debug mode");        sprintf (tbuff, "b %08x\r", addr);       mips_send_command (tbuff, 0);        mips_expect ("Bpt ");        if (!mips_getstring (tbuff, remote_timeout)) 	return 1;       tbuff[2] = '\0';
...
begin_comment
unit|tbuff[0] = '0';       tbuff[1] = 'x';
comment|/* FIXME!! only 8 bytes!  need to expand for Bfd64;           which targets return 64-bit addresses?  PMON returns only 32! */
end_comment
...
begin_function
name|int
name|remote_mips_can_use_hardware_watchpoint
parameter_list|(
name|int
name|cnt
parameter_list|)
...
begin_comment
comment|/* Insert a hardware breakpoint.  This works only on LSI targets, which    implement ordinary breakpoints using hardware facilities.  */
end_comment

begin_function
name|int
name|remote_mips_insert_hw_breakpoint
parameter_list|(
name|CORE_ADDR
name|addr
parameter_list|,
...
begin_comment
comment|/* Remove a hardware breakpoint.  This works only on LSI targets, which    implement ordinary breakpoints using hardware facilities.  */
end_comment

begin_function
name|int
name|remote_mips_remove_hw_breakpoint
parameter_list|(
name|CORE_ADDR
name|addr
parameter_list|,
