...
specifier|static
name|void
name|mips_elf_final_write_processing
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|boolean
name|mips_elf_set_private_flags
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|boolean
name|mips_elf_copy_private_bfd_data
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|boolean
name|mips_elf_merge_private_bfd_data
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|boolean
name|mips_elf_section_from_shdr
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|boolean
name|mips_elf_section_from_bfd_section
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|boolean
name|mips_elf_section_processing
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|void
name|mips_elf_symbol_processing
name|PARAMS
argument_list|(
operator|(
name|bfd
...
specifier|static
name|boolean
name|mips_elf_find_nearest_line
name|PARAMS
argument_list|(
operator|(
name|bfd
...
block|,
comment|/* pcrel_offset */
comment|/* A 64 bit relocation.  Presumably not used in 32 bit ELF.  */
block|{
name|R_MIPS_64
block|}
block|,
...
begin_comment
comment|/* Do a R_MIPS_HI16 relocation.  This has to be done in combination    with a R_MIPS_LO16 reloc, because there is a carry from the LO16 to    the HI16.  Here we just save the information we need; we do the    actual relocation when we see the LO16.  MIPS ELF requires that the    LO16 immediately follow the HI16, so this ought to work.  */
end_comment
...
name|E_MIPS_ARCH_1
case|:
comment|/* Just use the default, which was set in elfcode.h.  */
break|break;
case|case
name|E_MIPS_ARCH_2
case|:
...
begin_comment
comment|/* The final processing done just before writing out a MIPS ELF object    file.  This gets the MIPS architecture right based on the machine    number.  */
end_comment
...
block|{
comment|/* Warn about -fPIC mismatch */
if|if
condition|(
operator|(
name|new_flags
...
call|)
argument_list|(
literal|"%s: needs all files compiled with -fPIC"
argument_list|,
name|bfd_get_filename
argument_list|(
name|ibfd
...
call|)
argument_list|(
literal|"%s: needs all files compiled with -mabicalls"
argument_list|,
name|bfd_get_filename
argument_list|(
name|ibfd
...
begin_comment
comment|/* Handle a MIPS specific section when reading an object file.  This    is called when elfcode.h finds a section with an unknown type.    FIXME: We need to handle the SHF_MIPS_GPREL flag, but I'm not sure    how to.  */
end_comment
...
begin_comment
comment|/* Set the correct type for a MIPS ELF section.  We do this by the    section name, which is a hack, but ought to work.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* FIXME: Set the sh_link field.  */
block|}
elseif|else
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* The sh_info field is set in mips_elf_final_write_processing.  */
block|}
elseif|else
if|if
condition|(
...
begin_comment
comment|/* Work over a section just before writing it out.  We update the GP    value in the .reginfo section based on the value we are using.    FIXME: We recognize sections that need the SHF_MIPS_GPREL flag by    name; there has to be a better way.  */
end_comment
...
begin_comment
comment|/* Handle the special MIPS section numbers that a symbol may use.  */
end_comment
...
index|]
expr_stmt|;
comment|/* ri_gp_value is set by the function 		 mips_elf_section_processing when the section is 		 finally written out.  */
...
call|)
argument_list|(
literal|"ELF MIPS linker: more got entries are needed for hipage: %x"
argument_list|,
name|hipage
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* When creating a shared object, we must copy these                  reloc types into the output file as R_MIPS_REL32 		 relocs.  We create the .rel.dyn reloc section in 		 dynobj and make room for this reloc.  */
if|if
condition|(
name|sreloc
operator|==
...
name|struct
name|ecoff_debug_swap
name|mips_elf_ecoff_debug_swap
init|=
block|{
...
value|mips_elf_section_from_shdr
end_define
...
define|\
value|mips_elf_section_from_bfd_section
end_define
...
value|mips_elf_section_processing
end_define
...
value|mips_elf_symbol_processing
end_define
...
define|\
value|mips_elf_final_write_processing
end_define
...
directive|define
name|elf_backend_ecoff_debug_swap
value|&mips_elf_ecoff_debug_swap
end_define
...
value|mips_elf_find_nearest_line
end_define
...
define|\
value|mips_elf_copy_private_bfd_data
end_define
...
define|\
value|mips_elf_merge_private_bfd_data
end_define
...
value|mips_elf_set_private_flags
end_define
