...
begin_comment
comment|/* POWER/PowerPC XCOFF linker support.    Copyright 1995, 1996 Free Software Foundation, Inc.    Written by Ian Lance Taylor<ian@cygnus.com>, Cygnus Support.  This file is part of BFD, the Binary File Descriptor library.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* In order to support linking different object file formats into an    XCOFF format, we need to be able to determine whether a particular    bfd_target is an XCOFF vector.  FIXME: We need to rethink this    whole approach.  */
end_comment
...
parameter_list|)
define|\
value|(strcmp ((xv)->name, "aixcoff-rs6000") == 0 \    || strcmp ((xv)->name, "xcoff-powermac") == 0)
end_define
...
begin_comment
comment|/* XCOFF relocation types.  These probably belong in a header file    somewhere.  The relocations are described in the function    _bfd_ppc_xcoff_relocate_section in this file.  */
end_comment
...
begin_comment
comment|/* The first word of global linkage code.  This must be modified by    filling in the correct TOC offset.  */
end_comment

begin_define
define|#
directive|define
name|XCOFF_GLINK_FIRST
value|(0x81820000)
end_define
...
begin_comment
comment|/* The remaining words of global linkage code.  */
end_comment
...
define|#
directive|define
name|XCOFF_GLINK_SIZE
define|\
value|(((sizeof xcoff_glink_code / sizeof xcoff_glink_code[0]) * 4) + 4)
end_define
...
name|internal_ldhdr
block|{
comment|/* The version number: currently always 1.  */
name|unsigned
name|long
name|l_version
decl_stmt|;
...
name|toc_offset
decl_stmt|;
comment|/* If the TOC entry comes from an input file, this is set to the          symbo lindex of the C_HIDEXT XMC_TC symbol.  */
name|long
name|toc_indx
decl_stmt|;
block|}
...
name|ldsym
decl_stmt|;
comment|/* The .loader symbol table index.  */
name|long
name|ldindx
decl_stmt|;
...
begin_comment
comment|/* As it happens, we never need to swap in the ldrel structure.  */
end_comment
...
name|XTY_LD
case|:
comment|/* This is a label definition.  The x_scnlen field is the              symbol index of the csect.  I believe that this must              always follow the appropriate XTY_SD symbol, so I will              insist on it.  */
block|{
name|boolean
name|bad
decl_stmt|;
...
name|XTY_CM
case|:
comment|/* This is an unitialized csect.  We could base the name on              the storage mapping class, but we don't bother.  If this              csect is externally visible, it is a common symbol.  */
if|if
condition|(
name|csect
operator|!=
...
condition|)
block|{
comment|/* If we are statically linking a shared object, it is                  OK for symbol redefinitions to occur.  I can't figure                  out just what the XCOFF linker is doing, but                  something like this is required for -bnso to work.  */
if|if
condition|(
operator|!
...
control|)
block|{
comment|/* Reset the section size and the line numebr count, since the 	 data is now attached to the csects.  Don't reset the size of 	 the .debug section, since we need to read it below in 	 bfd_xcoff_size_dynamic_sections.  */
if|if
condition|(
name|strcmp
argument_list|(
...
name|debug_index
expr_stmt|;
comment|/* Grab the contents of the .debug section.  We use malloc and 	 copy the neams into the debug stringtab, rather than 	 bfd_alloc, because I expect that, when linking many files 	 together, many of the strings will be the same.  Storing the 	 strings in the hash table should save space in this case.  */
name|debug_contents
operator|=
operator|(
name|bfd_byte
...
name|len
decl_stmt|;
comment|/* If all defined symbols should be exported, mark them now.  */
if|if
condition|(
name|ldinfo
operator|->
...
name|XCOFF_MARK
expr_stmt|;
comment|/* If this symbol is called and defined in a dynamic object, or not      defined at all when building a shared object, or imported, then      we need to set up global linkage code for it.  (Unless we did      garbage collection and we didn't need this symbol.)  */
if|if
condition|(
operator|(
name|h
...
call|)
argument_list|(
literal|"attempt to export undefined symbol `%s'"
argument_list|,
name|h
operator|->
name|root
...
comment|/* h->ldsym will already have been allocated for an explicitly      imported symbol.  */
if|if
condition|(
name|h
operator|->
...
block|}
block|}
comment|/* The first 3 symbol table indices are reserved to indicate the      sections.  */
name|h
operator|->
name|ldindx
operator|=
...
name|line_filepos
expr_stmt|;
comment|/* Figure out the largest number of symbols in an input BFD.  Take      the opportunity to clear the output_has_begun fields of all the      input BFD's.  We want at least 4 symbols, since that is the      number which xcoff_write_global_symbol may need.  */
name|max_sym_count
operator|=
literal|4
expr_stmt|;
...
operator|)
expr_stmt|;
comment|/* We want to find out if tocval is a good value to use                  as the TOC anchor--that is, whether we can access all                  of the TOC using a 16 bit offset from tocval.  This                  test assumes that the TOC comes at the end of the                  output section, as it does in the default linker                  script.  If the TOC anchor is too far into the .toc                  section, the relocation routine will report                  overflows.  */
name|tocend
operator|=
operator|(
operator|(
...
expr_stmt|;
block|}
comment|/* If this symbol is a specially defined function descriptor, write      it out.  The first word is the address of the function code      itself, the second word is the address of the TOC, and the third      word is zero.  */
if|if
condition|(
operator|(
name|h
