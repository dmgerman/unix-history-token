...
argument_list|)
expr_stmt|;
comment|/* Now that the segment addresses have been worked out, take a better      guess at whether the file is executable.  If the entry point      is within the text segment, assume it is.  (This makes files      executable even if their entry point address is 0, as long as      their text starts at zero.).  */
if|if
condition|(
operator|(
name|execp
...
name|false
return|;
comment|/* Relocate and write out the sections.  These functions use the      symbol map created by aout_link_write_symbols.  */
if|if
condition|(
operator|!
name|aout_link_input_section
