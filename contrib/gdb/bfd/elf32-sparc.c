...
specifier|static
name|boolean
name|elf32_sparc_adjust_dynindx
name|PARAMS
argument_list|(
operator|(
expr|struct
...
begin_comment
comment|/* The howto table and associated functions.    ??? elf64-sparc.c has its own copy for the moment to ease transition    since some of the relocation values have changed.  At some point we'll    want elf64-sparc.c to switch over and use this table.    ??? Do we want to recognize (or flag as errors) some of the 64 bit entries    if the target is elf32-sparc. */
end_comment
...
argument_list|)
block|,
comment|/* ??? If we need to handle R_SPARC_64 then we need (figuratively)      --enable-64-bit-bfd.  That causes objdump to print address as 64 bits      which we really don't want on an elf32-sparc system.  There may be other      consequences which we may not want (at least not until it's proven they're      necessary) so for now these are only enabled ifdef BFD64.  */
ifdef|#
directive|ifdef
name|BFD64
name|HOWTO
...
argument_list|)
block|,
comment|/* ??? These don't make sense except in 64 bit systems so they're disabled      ifndef BFD64 too (for now).  */
name|HOWTO
argument_list|(
name|R_SPARC_OLO10
argument_list|,
...
directive|if
literal|0
comment|/* ??? The native linker doesn't do this so we can't (otherwise gcc would      have to know which linker is being used).  Instead, the native linker      bumps up the architecture level when it has to.  However, I still think      warnings like these are good, so it would be nice to have them turned on      by some option.  */
comment|/* If the output machine is normal sparc, we can't allow v9 input files.  */
block|if (bfd_get_mach (obfd) == bfd_mach_sparc&& (bfd_get_mach (ibfd) == bfd_mach_sparc_v8plus 	  || bfd_get_mach (ibfd) == bfd_mach_sparc_v8plusa))     {       error = true;       (*_bfd_error_handler) 	("%s: compiled for a v8plus system and target is v8", 	 bfd_get_filename (ibfd));     }
comment|/* If the output machine is v9, we can't allow v9+vis input files.  */
block|if (bfd_get_mach (obfd) == bfd_mach_sparc_v8plus&& bfd_get_mach (ibfd) == bfd_mach_sparc_v8plusa)     {       error = true;       (*_bfd_error_handler) 	("%s: compiled for a v8plusa system and target is v8plus", 	 bfd_get_filename (ibfd));     }
else|#
directive|else
if|if
condition|(
