...
begin_comment
comment|/*   * tclExecute.c --  *  *	This file contains procedures that execute byte-compiled Tcl  *	commands.  *  * Copyright (c) 1996-1997 Sun Microsystems, Inc.  *  * See the file "license.terms" for information on usage and redistribution  * of this file, and for a DISCLAIMER OF ALL WARRANTIES.  *  * SCCS: @(#) tclExecute.c 1.81 97/06/26 13:50:03  */
end_comment
...
name|stdout
argument_list|,
literal|"  Chars=%d, bytes=%u, objs=%u, stk depth=%u, exc depth=%d, aux items=%d\n"
argument_list|,
name|codePtr
operator|->
name|numSrcChars
...
directive|else
comment|/* not TCL_COMPILE_DEBUG - print generic trace if so requested */
if|if
condition|(
name|traceInstructions
condition|)
...
name|TCL_ERROR
case|:
comment|/* 		     * The invoked command returned an error. Record 		     * information about what was being executed when the 		     * error occurred, then look for an enclosing catch 		     * exception range, if any. 		     */
name|TRACE_WITH_OBJ
argument_list|(
operator|(
...
argument_list|)
expr_stmt|;
comment|/* done with popped object */
name|ADJUST_PC
argument_list|(
literal|1
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* done with element name. */
name|result
operator|=
name|TCL_ERROR
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* done with array name */
name|Tcl_DecrRefCount
argument_list|(
name|elemPtr
argument_list|)
expr_stmt|;
comment|/* and element name. */
name|result
operator|=
name|TCL_ERROR
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* done with popped value. */
name|result
operator|=
name|TCL_ERROR
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* done w elem name */
name|Tcl_DecrRefCount
argument_list|(
name|valuePtr
argument_list|)
expr_stmt|;
comment|/* done w incr amount */
goto|goto
name|checkForCatch
goto|;
block|}
...
case|:
block|{
comment|/* 		 * Operands must be numeric, but no int->double conversions 		 * are performed. 		 */
name|long
name|i2
decl_stmt|,
name|iResult
...
name|panic
argument_list|(
literal|"TclExecuteByteCode execution failure: foreach loop iter temp %d is link\n"
argument_list|,
name|iterTmpIndex
argument_list|)
expr_stmt|;
...
name|panic
argument_list|(
literal|"TclExecuteByteCode execution failure: foreach loop iter temp %d has shared object\n"
argument_list|,
name|iterTmpIndex
argument_list|)
expr_stmt|;
...
block|{
comment|/* update object in place */
name|Tcl_SetLongObj
argument_list|(
name|oldValuePtr
argument_list|,
...
name|panic
argument_list|(
literal|"TclExecuteByteCode execution failure: foreach loop iter temp %d is link, undefined, or array\n"
argument_list|,
name|iterTmpIndex
argument_list|)
expr_stmt|;
...
name|panic
argument_list|(
literal|"TclExecuteByteCode execution failure: foreach loop iter count object is bad\n"
argument_list|)
expr_stmt|;
block|}
endif|#
...
name|panic
argument_list|(
literal|"TclExecuteByteCode execution failure: foreach loop list temp %d is link, undefined, or array\n"
argument_list|,
name|listTmpIndex
argument_list|)
expr_stmt|;
...
name|panic
argument_list|(
literal|"TclExecuteByteCode execution failure: NULL foreach list temp %d: \"%s\"\n"
argument_list|,
name|listTmpIndex
argument_list|,
name|Tcl_GetStringFromObj
...
name|TCL_ERROR
expr_stmt|;
comment|/* 	 * Execution has generated an "exceptional return" (or "exception") 	 * such as TCL_ERROR. Look for the closest enclosing catch exception 	 * range, if any. If no enclosing catch range is found, stop 	 * execution and return the "exceptional return" code. 	 */
name|checkForCatch
label|:
name|rangePtr
operator|=
...
comment|/* 	     * Print the command in the error message (up to a certain 	     * number of characters, or up to the first new-line). 	     */
if|if
condition|(
name|cmdIndex
operator|!=
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * TclGetSrcInfoForPc --  *  *	Procedure that given a program counter value, returns an index  *	of the closest command's element in the bytecode code unit's  *	CmdLocation array. This element provides information about that  *	command's source: a pointer to its first byte and the number  *	of its characters.  *  * Results:  *	If a command in the bytecode code unit is found that encloses  *	the program counter value, the index of the command's element  *	in the CmdLocation array is returned. If multiple commands  *	resulted in code at pc, the index for the command with code that  *	starts closest to pc is returned. If no matching command is  *	found, -1 is returned.  *  * Side effects:  *	None.  *  *----------------------------------------------------------------------  */
end_comment
...
literal|1
decl_stmt|;
comment|/* Index of current candidate for closest 				 * command. */
name|int
name|bestDist
init|=
name|INT_MAX
decl_stmt|;
comment|/* Distance of pc to bestCmd's start pc. */
name|int
name|startOffset
decl_stmt|,
name|endOffset
...
name|stdout
argument_list|,
literal|"Number of ByteCode executions: %ld\n"
argument_list|,
name|numExecutions
argument_list|)
expr_stmt|;
...
name|stdout
argument_list|,
literal|"Number of Tcl objects in use: %ld, allocated %ld, freed %ld\n"
argument_list|,
operator|(
name|tclObjsAlloced
operator|-
...
name|stdout
argument_list|,
literal|"Number of instructions executed: %.0f\n\n"
argument_list|,
name|total
argument_list|)
expr_stmt|;
