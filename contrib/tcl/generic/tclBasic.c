...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * Tcl_HideCommand --  *  *	Makes a command hidden so that it cannot be invoked from within  *	an interpreter, only from within an ancestor.  *  * Results:  *	A standard Tcl result; also leaves a message in interp->result  *	if an error occurs.  *  * Side effects:  *	Moves a command from the command table to the hidden command  *	table.  *  *----------------------------------------------------------------------  */
end_comment
...
name|hiddenCmdName
decl_stmt|;
comment|/* Name of to-be-hidden command. */
block|{
name|Interp
modifier|*
name|iPtr
...
argument_list|)
argument_list|,
literal|"hidden command names can't have namespace qualifiers"
argument_list|,
operator|(
name|char
operator|*
...
return|;
block|}
comment|/*      * Find the command to hide. An error is returned if cmdName can't      * be found.      */
name|cmd
operator|=
...
name|cmd
expr_stmt|;
comment|/*      * If this command is the "bgerror" command in the global namespace,      * make note of it now. We'll need to know this later so that we can      * handle its "tkerror" twin below.      */
name|isBgerror
operator|=
literal|0
expr_stmt|;
...
expr_stmt|;
block|}
comment|/*      * It is an error to move an exposed command to a hidden command with      * hiddenCmdName if a hidden command with the name hiddenCmdName already      * exists.      */
name|hPtr
operator|=
name|Tcl_CreateHashEntry
argument_list|(
...
expr_stmt|;
block|}
comment|/*      * If we are creating a hidden command named "bgerror", share the      * command data structure with another command named "tkerror". This      * code should eventually be removed.      */
if|if
condition|(
name|isBgerror
condition|)
...
name|panic
argument_list|(
literal|"Tcl_HideCommand: hiding bgerror while tkerror is already hidden!"
argument_list|)
expr_stmt|;
block|}
name|Tcl_SetHashValue
...
block|}
block|}
comment|/*      * Now link the hash table entry with the command structure. Keep the      * containing namespace the same. After all, the command really      * "belongs" to that namespace.      */
name|cmdPtr
operator|->
name|hPtr
operator|=
...
argument_list|)
expr_stmt|;
comment|/*      * Normally, the command will go right back into its containing      * namespace. But if the exposed command name has "::" namespace      * qualifiers, it is being moved to another context.      */
if|if
condition|(
name|strstr
argument_list|(
...
expr_stmt|;
block|}
comment|/*      * If we are creating a command named "bgerror", share the command      * data structure with another command named "tkerror". This code      * should eventually be removed.      */
if|if
condition|(
operator|(
operator|*
...
name|panic
argument_list|(
literal|"Tcl_ExposeCommand: exposing bgerror while tkerror is already exposed!"
argument_list|)
expr_stmt|;
block|}
name|Tcl_SetHashValue
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * TclRenameCommand --  *  *      Called to give an existing Tcl command a different name. Both the  *      old command name and the new command name can have "::" namespace  *      qualifiers. If the new command has a different namespace context,  *      the command is automatically moved to that namespace.  *  *      If the new command name is NULL or the null string, the command is  *      deleted.  *  * Results:  *      Returns TCL_OK if successful, and TCL_ERROR if anything goes wrong.  *  * Side effects:  *      If anything goes wrong, an error message is returned in the  *      interpreter's result object.  *  *----------------------------------------------------------------------  */
end_comment
...
operator|)
expr_stmt|;
comment|/*      * Put the command in the new namespace, so we can check for an alias      * loop. Since we are adding a new command to a namespace, we must      * handle any shadowing of the global commands that this might create.      * Note that the renamed command has a different hashtable pointer than      * it used to have. This allows the command caching code in tclExecute.c      * to recognize that a command pointer it has cached for this command is      * now invalid.      */
name|oldHPtr
operator|=
name|cmdPtr
operator|->
...
argument_list|)
expr_stmt|;
comment|/*      * Everything is in place so we can check for an alias loop. If we      * detect one, put everything back the way it was and report the error.      */
name|result
operator|=
...
literal|0
expr_stmt|;
comment|/*      * Save information for the history module, if needed.      * BTL: setting these NULL disables history revisions.      */
if|if
condition|(
name|flags
operator|&
...
name|length
decl_stmt|;
comment|/* 	 * Compute the line number where the error occurred. 	 * BTL: no line # information yet. 	 */
name|iPtr
operator|->
name|errorLine
operator|=
