...
condition|)
block|{
comment|/* 	     * We couldn't read the old value: either the var doesn't yet 	     * exist or it's an array element. If it's new, we will try to 	     * create it with Tcl_ObjSetVar2 below. 	     */
name|char
modifier|*
name|name
decl_stmt|,
...
condition|)
block|{
comment|/* last char is ')' => array ref */
name|createVar
operator|=
literal|0
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 	 * Now store the list object back into the variable. If there is an 	 * error setting the new value, decrement its ref count if it 	 * was new and we didn't create the variable. 	 */
name|newValuePtr
operator|=
name|Tcl_ObjSetVar2
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* free unneeded name obj */
return|return
name|result
return|;
block|}
...
condition|)
block|{
comment|/* 		 * The list is empty and the array variable doesn't 		 * exist yet: create the variable with an empty array 		 * as the value. 		 */
name|Tcl_Obj
modifier|*
name|namePtr
decl_stmt|,
...
return|;
block|}
comment|/* 	 * Check that we are not trying to create a namespace var linked to 	 * a local variable in a procedure. If we allowed this, the local 	 * variable in the shorter-lived procedure frame could go away 	 * leaving the namespace var's reference invalid. 	 */
if|if
condition|(
name|otherPtr
operator|->
...
name|myName
argument_list|,
literal|"\": upvar won't create namespace variable that refers to procedure variable"
argument_list|,
operator|(
name|char
operator|*
