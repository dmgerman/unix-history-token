...
begin_comment
comment|/*  * Static buffer used to sprintf channel option values and return  * them to the caller.  */
end_comment
...
condition|)
block|{
comment|/*              * Switch the channel back into synchronous mode to ensure that it              * gets flushed now.              */
operator|(
name|void
operator|)
...
argument_list|)
expr_stmt|;
comment|/*      * On error, cause a background error and remove the channel handler      * and the script record.      */
if|if
condition|(
name|result
operator|!=
