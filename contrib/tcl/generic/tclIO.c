...
begin_comment
comment|/*   * tclIO.c --  *  *	This file provides the generic portions (those that are the same on  *	all platforms and for all channel types) of Tcl's IO facilities.  *  * Copyright (c) 1995-1996 Sun Microsystems, Inc.  *  * See the file "license.terms" for information on usage and redistribution  * of this file, and for a DISCLAIMER OF ALL WARRANTIES.  *  * SCCS: @(#) tclIO.c 1.227 96/07/30 09:26:30  */
end_comment
...
begin_comment
comment|/*  * Forward declaration of Channel; being used in struct EventScriptRecord,  * below.  */
end_comment
...
begin_comment
comment|/*  * Forward declaration of ChannelHandler; being used in struct Channel,  * below.  */
end_comment
...
name|inFile
decl_stmt|;
comment|/* File to use for input, or NULL. */
name|Tcl_File
name|outFile
decl_stmt|;
comment|/* File to use for output, or NULL. */
...
begin_comment
comment|/* Current output buffer (the 					 * curOutPtr field in the                                          * channel structure) should be                                          * output as soon as possible event                                          * though it may not be full. */
end_comment
...
begin_comment
comment|/*  * Has the channel event source been created and registered with the  * notifier?  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|channelEventSourceCreated
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
specifier|static
name|void
name|ChannelEventSourceExitProc
name|_ANSI_ARGS_
argument_list|(
operator|(
name|ClientData
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * TclFindChannel --  *  *	Finds a channel given two Tcl_Files.  *  * Results:  *	The Tcl_Channel found. Also returns nonzero in fileUsedPtr output  *	parameter if it finds that the Tcl_File is already used in another  *	channel.  *  * Side effects:  *	None.  *  *----------------------------------------------------------------------  */
end_comment
...
name|outFile
decl_stmt|;
comment|/* Channel has these Tcl_Files. */
name|int
modifier|*
name|fileUsedPtr
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/*              * And close the OS level handles using the driver function:              */
call|(
name|chanPtr
operator|->
name|typePtr
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * Tcl_UnregisterChannel --  *  *	Deletes the hash entry for a channel associated with an interpreter.  *  * Results:  *	A standard Tcl result.  *  * Side effects:  *	Deletes the hash entry for a channel associated with an interpreter.  *  *----------------------------------------------------------------------  */
end_comment
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * Tcl_RegisterChannel --  *  *	Adds an already-open channel to the channel table of an interpreter.  *  * Results:  *	None.  *  * Side effects:  *	May increment the reference count of a channel.  *  *----------------------------------------------------------------------  */
end_comment
...
expr_stmt|;
block|}
comment|/*      * Install this channel in the first empty standard channel slot.      */
if|if
condition|(
name|Tcl_GetStdChannel
argument_list|(
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * Tcl_GetChannelFile --  *  *	Returns a file associated with a channel.  *  * Results:  *	The file or NULL if failed (e.g. the channel is not open for the  *	requested direction).  *  * Side effects:  *	None.  *  *----------------------------------------------------------------------  */
end_comment
...
continue|continue;
block|}
comment|/*              * If we would have blocked, attempt to set up an asynchronous              * background flushing for this channel if the channel is              * nonblocking, or block until more output can be written if              * the channel is blocking.              */
if|if
condition|(
operator|(
name|errorCode
...
block|{
comment|/*                      * If the device driver did not emulate blocking behavior                      * then we must do it it here.                      */
name|TclWaitForFile
argument_list|(
name|chanPtr
operator|->
...
block|}
block|}
comment|/*              * Decide whether to report the error upwards or defer it. If              * we got an error during async flush we discard all queued              * output.              */
if|if
condition|(
name|calledFromAsyncFlush
condition|)
...
comment|/*      * If the queue became empty and we have an asynchronous flushing      * mechanism active, cancel the asynchronous flushing.      */
if|if
condition|(
operator|(
name|chanPtr
...
comment|/*      * Remove the channel from the standard channel table.      */
if|if
condition|(
name|Tcl_GetStdChannel
argument_list|(
...
expr_stmt|;
block|}
comment|/*      * And remove any events for this channel from the event queue.      */
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * ChannelEventDeleteProc --  *  *	This procedure returns 1 if the event passed in is for the  *	channel passed in as the second argument. This procedure is  *	used as a filter for events to delete in a call to  *	Tcl_DeleteEvents in CloseChannel.  *  * Results:  *	1 if matching, 0 otherwise.  *  * Side effects:  *	None.  *  *----------------------------------------------------------------------  */
end_comment
...
name|evPtr
decl_stmt|;
comment|/* The event to check for a match. */
name|ClientData
name|clientData
decl_stmt|;
comment|/* The channel to check for. */
block|{
name|ChannelHandlerEvent
modifier|*
name|cEvPtr
...
block|{
comment|/*                      * If the device driver did not emulate blocking behavior                      * then we have to do it here.                      */
name|TclWaitForFile
argument_list|(
name|chanPtr
operator|->
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * GetEOL --  *  *	Accumulate input into the channel input buffer queue until an  *	end of line has been seen.  *  * Results:  *	Number of bytes buffered or -1 on failure.  *  * Side effects:  *	Consumes input from the channel.  *  *----------------------------------------------------------------------  */
end_comment
...
name|result
decl_stmt|;
comment|/* Of getting another buffer from the                                  * channel. */
name|int
name|bytesToEOL
decl_stmt|;
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * Tcl_Gets --  *  *	Reads a complete line of input from the channel.  *  * Results:  *	Length of line read or -1 if error, EOF or blocked. If -1, use  *	Tcl_GetErrno() to retrieve the POSIX error code for the  *	error or condition that occurred.  *  * Side effects:  *	May flush output on the channel. May cause input to be  *	consumed from the channel.  *  *----------------------------------------------------------------------  */
end_comment
...
name|bufPtr
decl_stmt|;
comment|/* Iterates over queued input                                  * and output buffers. */
name|int
name|inputBuffered
decl_stmt|,
name|outputBuffered
...
expr_stmt|;
block|}
comment|/*      * Allow only buffers that are smaller than one megabyte.      */
if|if
condition|(
name|sz
operator|>
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * Tcl_GetChannelOption --  *  *	Gets a mode associated with an IO channel. If the optionName arg  *	is non NULL, retrieves the value of that option. If the optionName  *	arg is NULL, retrieves a list of alternating option names and  *	values for the given channel.  *  * Results:  *	A standard Tcl result. Also sets the supplied DString to the  *	string value of the option(s) returned.  *  * Side effects:  *	The string returned by this function is in static storage and  *	may be reused at any time subsequent to the call.  *  *----------------------------------------------------------------------  */
end_comment
...
name|result
decl_stmt|;
comment|/* Result of channel type operation. */
name|int
name|newMode
decl_stmt|;
...
name|optionName
argument_list|,
literal|"\": should be -blocking, -buffering, -buffersize, "
argument_list|,
literal|"-eofchar, -translation, "
argument_list|,
literal|"or channel type specific option"
argument_list|,
operator|(
name|char
operator|*
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * ChannelEventSourceExitProc --  *  *	This procedure is called during exit cleanup to delete the channel  *	event source. It deletes the event source for channels.  *  * Results:  *	None.  *  * Side effects:  *	Destroys the channel event source.  *  *----------------------------------------------------------------------  */
end_comment
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * ChannelHandlerSetupProc --  *  *	This procedure is part of the event source for channel handlers.  *	It is invoked by Tcl_DoOneEvent before it waits for events. The  *	job of this procedure is to provide information to Tcl_DoOneEvent  *	on how to wait for events (what files to watch).  *  * Results:  *	None.  *  * Side effects:  *	Tells the notifier what channels to watch.  *  *----------------------------------------------------------------------  */
end_comment
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * ChannelHandlerCheckProc --  *  *	This procedure is the second part (of three) of the event source  *	for channels. It is invoked by Tcl_DoOneEvent after the wait for  *	events is over. The job of this procedure is to test each channel  *	to see if it is ready now, and if so, to create events and put them  *	on the Tcl event queue.  *  * Results:  *	None.  *  * Side effects:  *	Makes entries on the Tcl event queue for each channel that is  *	ready now.  *  *----------------------------------------------------------------------  */
end_comment
...
condition|)
block|{
comment|/*              * The channel is considered ready for writing if there is no              * output buffered waiting to be written to the device, AND the              * underlying file is ready.              */
if|if
condition|(
operator|(
name|chanPtr
...
block|}
block|}
comment|/*          * If there are any events for this channel, put a notice into the          * Tcl event queue.          */
if|if
condition|(
name|readyMask
operator|!=
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * FlushEventProc --  *  *	This routine dispatches a background flush event.  *  *	Errors that occur during the write operation are stored  *	inside the channel structure for future reporting by the next  *	operation that uses this channel.  *  * Results:  *	None.  *  * Side effects:  *	Causes production of output on a channel.  *  *----------------------------------------------------------------------  */
end_comment
...
name|clientData
decl_stmt|;
comment|/* Channel to produce output on. */
name|int
name|mask
decl_stmt|;
comment|/* Not used. */
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * ChannelHandlerEventProc --  *  *	This procedure is called by Tcl_DoOneEvent when a channel event  *	reaches the front of the event queue. This procedure is responsible  *	for actually handling the event by invoking the callback for the  *	channel handler.  *  * Results:  *	Returns 1 if the event was handled, meaning that it should be  *	removed from the queue. Returns 0 if the event was not handled  *	meaning that it should stay in the queue. The only time the event  *	will not be handled is if the TCL_FILE_EVENTS flag bit is not  *	set in the flags passed.  *  *	NOTE: If the handler is deleted between the time the event is added  *	to the queue and the time it reaches the head of the queue, the  *	event is silently discarded (i.e. we return 1).  *  * Side effects:  *	Whatever the channel handler callback procedure does.  *  *----------------------------------------------------------------------  */
end_comment
...
name|chan
expr_stmt|;
comment|/*      * Ensure that the channel event source is registered with the Tcl      * notification mechanism.      */
if|if
condition|(
operator|!
name|channelEventSourceCreated
...
block|}
block|}
comment|/*      * If found, splice the entry out of the list.      */
if|if
condition|(
name|chPtr
operator|==
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * ReturnScriptRecord --  *  *	Get a script stored for this channel with this interpreter.  *  * Results:  *	A standard Tcl result.  *  * Side effects:  *	Sets interp->result to the script.  *  *----------------------------------------------------------------------  */
end_comment
...
name|interp
decl_stmt|;
comment|/* The interpreter in which the script                                  * is to be executed. */
name|Channel
modifier|*
name|chanPtr
decl_stmt|;
comment|/* The channel for which the script is                                  * stored. */
name|int
name|mask
decl_stmt|;
comment|/* Events in mask must overlap with events                                  * for which this script is stored. */
block|{
name|EventScriptRecord
modifier|*
name|esPtr
...
name|script
expr_stmt|;
comment|/*      * We must preserve the channel, script and interpreter because each of      * these may be deleted in the evaluation. If an error later occurs, we      * want to have the relevant data around for error reporting and so we      * can safely delete it.      */
name|Tcl_Preserve
argument_list|(
operator|(
name|ClientData
