...
block|}
block|}
comment|/*      * See if the desired file is already loaded.  If so, its package      * name must agree with ours (if we have one).      */
for|for
control|(
name|pkgPtr
operator|=
...
condition|)
block|{
comment|/* 			 * We're looking for a statically loaded package; 			 * the file name is basically irrelevant here, so 			 * don't get upset that there's some other package 			 * with the same (empty string) file name.  Just 			 * skip this package and go on to the next. 			 */
goto|goto
name|nextPackage
goto|;
block|}
...
name|gotPkg
label|:
comment|/*      * If the file is already loaded in the target interpreter then      * there's nothing for us to do.      */
name|ipFirstPtr
operator|=
operator|(
name|InterpPackage
...
name|pkgGuess
decl_stmt|;
comment|/* 		 * The platform-specific code couldn't figure out the 		 * module name.  Make a guess by taking the last element 		 * of the file name, stripping off any leading "lib", and 		 * then using all of the alphabetic characters that follow 		 * that. 		 */
name|Tcl_SplitPath
argument_list|(
name|fullFileName
argument_list|,
