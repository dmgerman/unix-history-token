...
begin_comment
comment|/*  * The command trace below is used by the "testcmdtraceCmd" command  * to test the command tracing facilities.  */
end_comment
...
condition|)
block|{
comment|/* 	 * Create a command trace then eval a script to check whether it is 	 * called. Note that this trace procedure removes itself as a 	 * further check of the robustness of the trace proc calling code in 	 * TclExecuteByteCode. 	 */
name|cmdTrace
operator|=
name|Tcl_CreateTrace
argument_list|(
...
index|]
argument_list|,
literal|"\": must be tracetest or deletetest"
argument_list|,
operator|(
name|char
operator|*
...
comment|/* Argument strings. */
block|{
comment|/*      * Remove ourselves to test whether calling Tcl_DeleteTrace within      * a trace callback causes the for loop in TclExecuteByteCode that      * calls traces to reference freed memory.      */
name|Tcl_DeleteTrace
argument_list|(
name|interp
argument_list|,
...
begin_comment
comment|/*  *----------------------------------------------------------------------  *  * TestsetrecursionlimitCmd --  *  *	This procedure implements the "testsetrecursionlimit" command. It is  *	used to change the interp recursion limit (to test the effects  *      of Tcl_SetRecursionLimit).  *  * Results:  *	A standard Tcl result.  *  * Side effects:  *	Sets the interp's recursion limit.  *  *----------------------------------------------------------------------  */
end_comment
