...
argument_list|)
argument_list|,
literal|"unable to determine name of host"
argument_list|,
operator|-
literal|1
argument_list|)
...
condition|)
block|{
comment|/* 	 * The following code must reflect the logic in Tcl_DuplicateObj() 	 * except that it must duplicate the list object directly into the 	 * interpreter's result. 	 */
