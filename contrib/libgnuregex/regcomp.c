...
directive|ifdef
name|_LIBC
comment|/* See if we have to try all bytes which start multiple collation 	     elements. 	     e.g. In da_DK, we want to catch 'a' since "aa" is a valid 		  collation element, and don't catch 'b' since 'b' is 		  the only collation element which starts from 'b' (and 		  it is caught by SIMPLE_BRACKET).  */
if|if
condition|(
...
directive|endif
comment|/* _LIBC */
comment|/* See if we have to start the match at all multibyte characters, 	     i.e. where we would not find an invalid sequence.  This only 	     applies to multibyte character sets; for single byte character 	     sets, the SIMPLE_BRACKET again suffices.  */
if|if
condition|(
name|dfa
operator|->
...
block|{
comment|/* ... Else catch all bytes which can start the mbchars.  */
for|for
control|(
name|i
operator|=
...
directive|ifdef
name|DEBUG
comment|/* Note: length+1 will not overflow since it is checked in init_dfa.  */
name|dfa
operator|->
name|re_str
operator|=
...
name|REG_ESPACE
return|;
comment|/* Set the bits corresponding to single byte chars.  */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* The extra temporaries here avoid "implicitly truncated"              warnings in the case when this is dead code, i.e. 32-bit.  */
specifier|const
name|uint64_t
name|wc0
init|=
...
break|break;
default|default:
comment|/* Word anchors etc. cannot be handled.  It's okay to test 	       opr.ctx_type since constraints (for all DFA nodes) are 	       created by ORing one or more opr.ctx_type values.  */
return|return;
block|}
break|break;
case|case
...
name|SIMPLE_BRACKET
case|:
comment|/* Just double check.  The non-ASCII range starts at 0x80.  */
name|assert
argument_list|(
literal|0x80
operator|%
...
argument_list|)
expr_stmt|;
comment|/* If the node is root_node itself, it means the epsilon clsoure 	     has a loop.   Then tie it to the destination of the root_node.  */
if|if
condition|(
name|org_node
operator|==
...
break|break;
block|}
comment|/* In case of the node has another constraint, add it.  */
name|constraint
operator||=
name|dfa
operator|->
...
condition|)
block|{
comment|/* There is no such duplicated node, create a new one.  */
name|reg_errcode_t
name|err
decl_stmt|;
name|clone_dest
...
block|{
comment|/* There is a duplicated node which satisfies the constraint, 		 use it to avoid infinite loop.  */
name|ret
operator|=
name|re_node_set_insert
argument_list|(
...
begin_comment
comment|/* Duplicate the node whose index is ORG_IDX and set the constraint CONSTRAINT.    Return the index of the new node, or -1 if insufficient storage is    available.  */
end_comment
...
block|}
block|}
comment|/* An epsilon closure includes itself.  */
name|ret
operator|=
name|re_node_set_insert
argument_list|(
...
name|extra
decl_stmt|;
comment|/* Local function for parse_bracket_exp used in _LIBC environement.      Seek the collating symbol entry correspondings to NAME.      Return the index of the symbol in the SYMB_TABLE,      or -1 if not found.  */
specifier|auto
specifier|inline
name|int32_t
name|__attribute
...
return|;
block|}
comment|/* Local function for parse_bracket_exp used in _LIBC environment.      Look up the collation sequence value of BR_ELEM.      Return the value if succeeded, UINT_MAX otherwise.  */
specifier|auto
specifier|inline
name|unsigned
name|int
...
comment|/* This isn't a valid character.  */
continue|continue;
comment|/* Compare only if the length matches and the collation rule 	     index is the same.  */
if|if
condition|(
name|len
operator|==
...
parameter_list|)
define|\
value|do {						\     if (BE (trans != NULL, 0))			\       {						\ 	for (i = 0; i< SBC_MAX; ++i)		\   	  if (ctype_func (i))			\ 	    bitset_set (sbcset, trans[i]);	\       }						\     else					\       {						\ 	for (i = 0; i< SBC_MAX; ++i)		\   	  if (ctype_func (i))			\ 	    bitset_set (sbcset, i);		\       }						\   } while (0)
if|if
condition|(
name|strcmp
argument_list|(
