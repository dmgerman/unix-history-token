...
parameter_list|)
define|\
value|f = (fontNameAttributes& bit) ? \ 		fontName->field \ 		: (char *)"*"; \ 	if ((left -= strlen (f))< 0) \ 		return False; \ 	while (*f) \ 		if ((*name++ = *f++) == '-') \ 			return False;
define|#
directive|define
name|PutHyphen
parameter_list|()
...
parameter_list|)
define|\
value|f = (fontNameAttributes& bit) ? \ 		utoa (fontName->field, number, sizeof (number)) \ 		: (char *)"*"; \ 	if ((left -= strlen (f))< 0) \ 		return False; \ 	while (*f) \ 		*name++ = *f++;
name|PutString
argument_list|(
argument|Registry
argument_list|,
