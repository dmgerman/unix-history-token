...
begin_comment
comment|/* deflate.c -- compress data using the deflation algorithm  * Copyright (C) 1995-2017 Jean-loup Gailly and Mark Adler  * For conditions of distribution and use, see copyright notice in zlib.h  */
end_comment
...
name|message
name|(
literal|"Assembler code may have bugs -- use at your own risk"
name|)
end_pragma
...
begin_comment
comment|/* rank Z_BLOCK between Z_NO_FLUSH and Z_PARTIAL_FLUSH */
end_comment
...
begin_comment
comment|/* ===========================================================================  * Update a hash value with the given input byte  * IN  assertion: all calls to UPDATE_HASH are made with consecutive input  *    characters, so that a running hash key can be computed from the previous  *    key instead of complete recalculation each time.  */
end_comment
...
begin_comment
comment|/* ===========================================================================  * Insert string str in the dictionary and set match_head to the previous head  * of the hash chain (the most recent string with same hash key). Return  * the previous length of the hash chain.  * If this file is compiled with -DFASTEST, the compression level is forced  * to 1, and no hash chains are maintained.  * IN  assertion: all calls to INSERT_STRING are made with consecutive input  *    characters and the first MIN_MATCH bytes of str are valid (except for  *    the last MIN_MATCH-1 bytes of the input file).  */
end_comment
...
parameter_list|)
define|\
value|s->head[s->hash_size-1] = NIL; \     zmemzero((Bytef *)s->head, (unsigned)(s->hash_size-1)*sizeof(*s->head));
end_define

begin_comment
comment|/* ===========================================================================  * Slide the hash table when sliding the window down (could be avoided with 32  * bit values at the expense of memory usage). We slide even when level == 0 to  * keep the hash table consistent if we switch back to level> 0 later.  */
end_comment
...
name|INIT_STATE
expr_stmt|;
comment|/* to pass state test in deflateReset() */
name|s
operator|->
name|wrap
operator|=
...
literal|0
expr_stmt|;
comment|/* nothing written to s->window yet */
name|s
operator|->
name|lit_bufsize
operator|=
...
begin_comment
comment|/* =========================================================================  * Check for a valid deflate stream state. Return 0 if ok, 1 if not.  */
end_comment
...
name|Z_STREAM_ERROR
return|;
comment|/* when using zlib wrappers, compute Adler-32 for provided dictionary */
if|if
condition|(
name|wrap
operator|==
...
literal|0
expr_stmt|;
comment|/* avoid computing Adler-32 in read_buf */
comment|/* if dictionary would fill window, just replace the history */
if|if
condition|(
name|dictLength
operator|>=
...
condition|)
block|{
comment|/* already empty otherwise */
name|CLEAR_HASH
argument_list|(
name|s
argument_list|)
...
expr_stmt|;
block|}
comment|/* insert dictionary into window and hash */
name|avail
operator|=
name|strm
operator|->
...
name|wraplen
decl_stmt|;
comment|/* conservative upper bound for compressed data */
name|complen
operator|=
name|sourceLen
operator|+
...
literal|5
expr_stmt|;
comment|/* if can't get parameters, return conservative bound plus zlib wrapper */
if|if
condition|(
name|deflateStateCheck
argument_list|(
...
literal|6
return|;
comment|/* compute wrapper length */
name|s
operator|=
name|strm
operator|->
...
condition|)
block|{
comment|/* user-supplied gzip header */
name|Bytef
modifier|*
name|str
decl_stmt|;
...
directive|endif
default|default:
comment|/* for compiler happiness */
name|wraplen
operator|=
literal|6
expr_stmt|;
...
begin_comment
comment|/* =========================================================================  * Flush as much pending output as possible. All deflate() output, except for  * some deflate_stored() output, goes through this function so some  * applications may wish to modify it to avoid allocating a large  * strm->next_out buffer and copying into it. (See also read_buf()).  */
end_comment
...
begin_comment
comment|/* ===========================================================================  * Update the header CRC with the bytes s->pending_buf[beg..s->pending - 1].  */
end_comment
...
parameter_list|)
define|\
value|do { \         if (s->gzhead->hcrc&& s->pending> (beg)) \             strm->adler = crc32(strm->adler, s->pending_buf + (beg), \                                 s->pending - (beg)); \     } while (0)
end_define
...
name|BUSY_STATE
expr_stmt|;
comment|/* Compression must start with an empty pending buffer */
name|flush_pending
argument_list|(
name|strm
argument_list|)
...
begin_comment
comment|/* ---------------------------------------------------------------------------  * Optimized version for FASTEST only  */
end_comment
...
name|MIN_LOOKAHEAD
argument_list|,
literal|"already enough lookahead"
argument_list|)
expr_stmt|;
do|do
block|{
...
condition|)
do|;
comment|/* If the WIN_INIT bytes after the end of the current data have never been      * written, then zero those bytes in order to avoid memory check reports of      * the use of uninitialized (or uninitialised as Julian writes) bytes by      * the longest match routines.  Update the high water mark for the next      * time through here.  WIN_INIT is set to MAX_MATCH since the longest match      * routines allow scanning to strstart + MAX_MATCH, ignoring lookahead.      */
if|if
condition|(
name|s
operator|->
...
condition|)
block|{
comment|/* Previous high water mark below current data -- zero WIN_INIT              * bytes or up to end of window, whichever is less.              */
name|init
operator|=
name|s
operator|->
...
condition|)
block|{
comment|/* High water mark at or above current data, but below current data              * plus WIN_INIT -- zero out to current data plus WIN_INIT, or up              * to end of window, whichever is less.              */
name|init
operator|=
operator|(
name|ulg
...
name|MIN_LOOKAHEAD
argument_list|,
literal|"not enough room for search"
argument_list|)
expr_stmt|;
block|}
end_function
...
name|last
parameter_list|)
value|{ \    _tr_flush_block(s, (s->block_start>= 0L ? \                    (charf *)&s->window[(unsigned)s->block_start] : \                    (charf *)Z_NULL), \                 (ulg)((long)s->strstart - s->block_start), \                 (last)); \    s->block_start = s->strstart; \    flush_pending(s->strm); \    Tracev((stderr,"[FLUSH]")); \ }
end_define
...
name|last
parameter_list|)
value|{ \    FLUSH_BLOCK_ONLY(s, last); \    if (s->strm->avail_out == 0) return (last) ? finish_started : need_more; \ }
end_define

begin_comment
comment|/* Maximum stored block length in deflate format (not including header). */
end_comment
...
begin_comment
comment|/* ===========================================================================  * Copy without compression as much as possible from the input stream, return  * the current block state.  *  * In case deflateParams() is used to later switch to a non-zero compression  * level, s->matches (otherwise unused when storing) keeps track of the number  * of hash table slides to perform. If s->matches is 1, then one hash table  * slide will be done when switching. If s->matches is 2, the maximum value  * allowed here, then the hash table will be cleared, since two or more slides  * is the same as a clear.  *  * deflate_stored() is written to minimize the number of times an input byte is  * copied. It is most efficient with large input and output buffers, which  * maximizes the opportunites to have a single copy from next_in to next_out.  */
end_comment
...
decl_stmt|;
block|{
comment|/* Smallest worthy block size when not flushing or finishing. By default      * this is 32K. This can be as small as 507 bytes for memLevel == 1. For      * large input and output buffers, the stored block size will be larger.      */
name|unsigned
name|min_block
init|=
name|MIN
...
argument_list|)
decl_stmt|;
comment|/* Copy as many min_block or larger stored blocks directly to next_out as      * possible. If flushing, copy the remaining available input to next_out as      * stored blocks, if there is enough space.      */
name|unsigned
name|len
decl_stmt|,
name|left
...
do|do
block|{
comment|/* Set len to the maximum size block that we can copy directly with the          * available input data and output space. Set left to how much of that          * would be copied from what's left in the window.          */
name|len
operator|=
name|MAX_STORED
expr_stmt|;
comment|/* maximum deflate stored block length */
name|have
operator|=
operator|(
name|s
...
literal|3
expr_stmt|;
comment|/* number of header bytes */
if|if
condition|(
name|s
operator|->
...
name|have
condition|)
comment|/* need room for header */
break|break;
comment|/* maximum stored block length that will fit in avail_out: */
name|have
operator|=
name|s
operator|->
...
name|avail_in
expr_stmt|;
comment|/* limit len to the input */
if|if
condition|(
name|len
operator|>
...
condition|)
break|break;
comment|/* Make a dummy stored block in pending to get the header bytes,          * including any pending bits. This also updates the debugging counts.          */
name|last
operator|=
name|flush
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Replace the lengths in the dummy stored block with len. */
name|s
operator|->
name|pending_buf
index|[
...
literal|8
expr_stmt|;
comment|/* Write the stored block header bytes. */
name|flush_pending
argument_list|(
name|s
operator|->
...
directive|endif
comment|/* Copy uncompressed bytes from the window to next_out. */
if|if
condition|(
name|left
condition|)
...
expr_stmt|;
block|}
comment|/* Copy uncompressed bytes directly from next_in to next_out, updating          * the check value.          */
if|if
condition|(
name|len
condition|)
...
condition|)
do|;
comment|/* Update the sliding window with the last s->w_size bytes of the copied      * data, or append all of the copied data to the existing window if less      * than s->w_size bytes were copied. Also update the number of bytes to      * insert in the hash tables, in the event that deflateParams() switches to      * a non-zero compression level.      */
name|used
operator|-=
name|s
operator|->
...
name|avail_in
expr_stmt|;
comment|/* number of input bytes directly copied */
if|if
condition|(
name|used
condition|)
block|{
comment|/* If any input was used, then no unused input remains in the window,          * therefore s->block_start == s->strstart.          */
if|if
condition|(
name|used
operator|>=
...
condition|)
block|{
comment|/* supplant the previous history */
name|s
operator|->
name|matches
operator|=
...
operator|++
expr_stmt|;
comment|/* add a pending slide_hash() */
block|}
name|zmemcpy
argument_list|(
name|s
...
name|finish_done
return|;
comment|/* If flushing and all input has been consumed, then done. */
if|if
condition|(
name|flush
operator|!=
...
name|block_done
return|;
comment|/* Fill the window with any remaining input. */
name|have
operator|=
name|s
operator|->
...
expr_stmt|;
comment|/* number of header bytes */
comment|/* maximum stored block length that will fit in pending: */
name|have
operator|=
name|MIN
argument_list|(
...
expr_stmt|;
block|}
comment|/* We've done all we can with the available input and output. */
return|return
name|last
condition|?
name|finish_started
...
name|strend
decl_stmt|;
comment|/* scan goes up to strend for length of run */
for|for
control|(
init|;
condition|;
control|)
block|{
comment|/* Make sure that we always have enough lookahead, except          * at the end of the input file. We need MAX_MATCH bytes          * for the longest run, plus one for the unrolled loop.          */
if|if
condition|(
name|s
operator|->
...
begin_comment
comment|/* ===========================================================================  * For Z_HUFFMAN_ONLY, do not look for matches.  Do not maintain a hash table.  * (It will be regenerated if this run of deflate switches away from Huffman.)  */
end_comment
...
control|)
block|{
comment|/* Make sure that we have a literal to write. */
if|if
condition|(
name|s
operator|->
