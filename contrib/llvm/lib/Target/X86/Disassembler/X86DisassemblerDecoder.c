...
name|readerCursor
expr_stmt|;
comment|/* If we fail reading prefixes, just stop here and let the opcode reader deal with it */
if|if
condition|(
name|consumeByte
argument_list|(
...
condition|)
block|{
comment|/*        * If the byte is 0xf2 or 0xf3, and any of the following conditions are        * met:        * - it is followed by a LOCK (0xf0) prefix        * - it is followed by an xchg instruction        * then it should be disassembled as a xacquire/xrelease not repne/rep.        */
if|if
condition|(
operator|(
name|byte
...
name|TRUE
expr_stmt|;
comment|/*        * Also if the byte is 0xf3, and the following condition is met:        * - it is followed by a "mov mem, reg" (opcode 0x88/0x89) or        *                       "mov mem, imm" (opcode 0xc6/0xc7) instructions.        * then it should be disassembled as an xrelease not rep.        */
if|if
condition|(
name|byte
operator|==
...
name|insn
argument_list|,
literal|"Couldn't read second byte of VEX"
argument_list|)
expr_stmt|;
return|return
operator|-
...
argument_list|)
expr_stmt|;
comment|/* We simulate the REX prefix for simplicity's sake */
if|if
condition|(
name|insn
operator|->
...
name|insn
argument_list|,
literal|"Found VEX prefix 0x%hhx 0x%hhx 0x%hhx"
argument_list|,
name|insn
operator|->
name|vexXopPrefix
...
name|insn
argument_list|,
literal|"Found VEX prefix 0x%hhx 0x%hhx"
argument_list|,
name|insn
operator|->
name|vexXopPrefix
...
name|insn
argument_list|,
literal|"Couldn't read second byte of XOP"
argument_list|)
expr_stmt|;
return|return
operator|-
...
condition|)
block|{
comment|/* 0 in these 3 bits is a POP instruction. */
name|insn
operator|->
name|vexXopType
operator|=
...
name|insn
argument_list|,
literal|"Found XOP prefix 0x%hhx 0x%hhx 0x%hhx"
argument_list|,
name|insn
operator|->
name|vexXopPrefix
...
name|insn
argument_list|,
literal|"Unhandled m-mmmm field for instruction (0x%hhx)"
argument_list|,
name|mmmmmFromVEX2of3
argument_list|(
name|insn
...
condition|)
block|{
comment|/*      * NOOP shouldn't decode as NOOP if REX.b is set. Instead      * it should decode as XCHG %r8, %eax.      */
specifier|const
name|struct
name|InstructionSpecifier
modifier|*
...
decl_stmt|;
name|uint16_t
name|instructionIDWithNewOpcode
decl_stmt|;
specifier|const
name|struct
name|InstructionSpecifier
modifier|*
name|specWithNewOpcode
decl_stmt|;
name|spec
operator|=
name|specifierForUID
...
argument_list|)
expr_stmt|;
comment|/* Borrow opcode from one of the other XCHGar opcodes */
name|insn
operator|->
name|opcode
operator|=
...
begin_comment
comment|/*  * readVVVV - Consumes vvvv from an instruction if it has a VEX prefix.  *  * @param insn  - The instruction whose operand is to be read.  * @return      - 0 if the vvvv was successfully consumed; nonzero  *                otherwise.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If non-zero vvvv specified, need to make sure one of the operands      uses it. */
name|hasVVVV
operator|=
operator|!
name|readVVVV
...
condition|)
block|{
comment|/* Saw a register immediate so don't read again and instead split the            previous immediate.  FIXME: This is a hack. */
name|insn
operator|->
name|immediates
index|[
...
literal|0
expr_stmt|;
comment|/* Mark that we have found a VVVV operand. */
if|if
condition|(
operator|!
name|hasVVVV
...
block|}
block|}
comment|/* If we didn't find ENCODING_VVVV operand, but non-zero vvvv present, fail */
if|if
condition|(
name|needVVVV
condition|)
