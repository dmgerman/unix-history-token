...
condition|)
block|{
comment|/*      * Some VEX instructions ignore the L-bit, but use the W-bit. Normally L-bit      * has precedence since there are no L-bit with W-bit entries in the tables.      * So if the L-bit isn't significant we should use the W-bit instead.      */
specifier|const
name|struct
name|InstructionSpecifier
modifier|*
