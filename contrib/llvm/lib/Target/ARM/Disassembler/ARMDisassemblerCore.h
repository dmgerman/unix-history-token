...
return|;
block|}
comment|/// A NEON Domain instruction has cond field (Inst{31-28}) as 0b1111.
specifier|static
specifier|inline
name|bool
name|isNEONDomain
...
name|Ctx
decl_stmt|;
comment|// The address of the instruction being disassembled.
name|uint64_t
name|Address
decl_stmt|;
name|public
label|:
name|void
name|setupBuilderForSymbolicDisassembly
parameter_list|(
name|LLVMOpInfoCallback
name|getOpInfo
parameter_list|,
...
return|;
block|}
comment|/// tryAddingSymbolicOperand - tryAddingSymbolicOperand trys to add a symbolic
comment|/// operand in place of the immediate Value in the MCInst.  The immediate
comment|/// Value has had any PC adjustment made by the caller.  If the getOpInfo()
comment|/// function was set as part of the setupBuilderForSymbolicDisassembly() call
comment|/// then that function is called to get any symbolic information at the
comment|/// builder's Address for this instrution.  If that returns non-zero then the
comment|/// symbolic information it returns is used to create an MCExpr and that is
comment|/// added as an operand to the MCInst.  This function returns true if it adds
comment|/// an operand to the MCInst and false otherwise.
name|bool
