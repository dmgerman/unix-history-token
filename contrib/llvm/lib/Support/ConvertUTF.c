...
specifier|static
name|unsigned
name|findMaximalSubpartOfIllFormedUTF8Sequence
parameter_list|(
specifier|const
name|UTF8
modifier|*
...
argument_list|)
expr_stmt|;
comment|/*    * Unicode 6.3.0, D93b:    *    *   Maximal subpart of an ill-formed subsequence: The longest code unit    *   subsequence starting at an unconvertible offset that is either:    *   a. the initial subsequence of a well-formed code unit sequence, or    *   b. a subsequence of length one.    */
if|if
condition|(
name|source
operator|==
...
literal|0
return|;
comment|/*    * Perform case analysis.  See Unicode 6.3.0, Table 3-7. Well-Formed UTF-8    * Byte Sequences.    */
name|b1
operator|=
operator|*
name|source
...
condition|)
block|{
comment|/*      * First byte is valid, but we know that this code unit sequence is      * invalid, so the maximal subpart has to end after the first byte.      */
return|return
literal|1
return|;
block|}
...
argument_list|)
expr_stmt|;
comment|/*    * There are no valid sequences that start with these bytes.  Maximal subpart    * is defined to have length 1 in these cases.    */
return|return
literal|1
return|;
block|}
...
name|sourceIllegal
expr_stmt|;
comment|/*                  * Replace the maximal subpart of ill-formed sequence with                  * replacement character.                  */
name|source
operator|+=
name|findMaximalSubpartOfIllFormedUTF8Sequence
argument_list|(
