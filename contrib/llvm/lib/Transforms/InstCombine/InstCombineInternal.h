...
block|}
block|}
comment|/// Return the source operand of a potentially bitcasted value while optionally
comment|/// checking if it has one use. If there is no bitcast or the one use check is
comment|/// not met, return the input value itself.
specifier|static
specifier|inline
name|Value
modifier|*
name|peekThroughBitcast
parameter_list|(
name|Value
modifier|*
name|V
...
argument_list|)
return|;
comment|// V is not a bitcast or V has more than one use and OneUseOnly is true.
return|return
name|V
return|;
block|}
...
name|Value
modifier|*
name|foldAndOrOfICmpsOfAndWithPow2
parameter_list|(
name|ICmpInst
modifier|*
name|LHS
...
argument_list|)
decl_stmt|;
comment|/// Match a select chain which produces one of three values based on whether
comment|/// the LHS is less than, equal to, or greater than RHS respectively.
comment|/// Return true if we matched a three way compare idiom. The LHS, RHS, Less,
comment|/// Equal and Greater values are saved in the matching process and returned to
comment|/// the caller.
name|bool
name|matchThreeWayIntCompare
parameter_list|(
name|SelectInst
modifier|*
name|SI
...
name|Instruction
modifier|*
name|SimplifyElementUnorderedAtomicMemCpy
parameter_list|(
