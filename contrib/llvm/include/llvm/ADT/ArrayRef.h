...
name|Length
decl_stmt|;
comment|/// \brief A dummy "optional" type that is only created by implicit
comment|/// conversion from a reference to T.
comment|///
comment|/// This type must *only* be used in a function argument or as a copy of
comment|/// a function argument, as otherwise it will hold a pointer to a temporary
comment|/// past that temporaries' lifetime.
struct|struct
name|TRefOrNothing
block|{
specifier|const
...
name|false
return|;
comment|// Don't use std::equal(), since it asserts in MSVC on nullptr iterators.
for|for
control|(
name|auto
name|L
...
name|R
control|)
comment|// Match std::equal() in using == (instead of !=) to minimize API
comment|// requirements of ArrayRef'ed types.
if|if
condition|(
operator|!
operator|(
...
comment|/// @}
comment|/// @{
comment|/// @name Convenience methods
comment|/// @brief Predicate for testing that the array equals the exact sequence of
comment|/// arguments.
comment|///
comment|/// Will return false if the size is not equal to the exact number of
comment|/// arguments given or if the array elements don't equal the argument
comment|/// elements in order. Currently supports up to 16 arguments, but can
comment|/// easily be extended.
name|bool
name|equals
parameter_list|(
name|TRefOrNothing
...
name|false
return|;
comment|// Either the size is exactly as many args, or the next arg must be null.
return|return
name|size
argument_list|()
operator|==
