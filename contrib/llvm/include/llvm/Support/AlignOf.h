...
name|_MSC_VER
argument_list|)
comment|// Disables "structure was padded due to __declspec(align())" warnings that are
comment|// generated by any class using AlignOf<T> with a manually specified alignment.
comment|// Although the warning is disabled in the LLVM project we need this pragma
comment|// as AlignOf.h is a published support header that's available for use
comment|// out-of-tree, and we would like that to compile cleanly at /W4.
pragma|#
directive|pragma
name|warning
name|(
