...
comment|///   /// Determine if two keys are equal. Optional, only needed by contains.
comment|///   static bool EqualKey(key_type_ref Key1, key_type_ref Key2);
comment|/// };
comment|/// \endcode
name|template
operator|<
...
expr_stmt|;
block|}
comment|/// \brief Determine whether an entry has been inserted.
name|bool
name|contains
argument_list|(
name|typename
...
directive|else
comment|// In asserts mode, check that the users length matches the data they
comment|// wrote.
name|uint64_t
name|KeyStart
init|=
...
name|first
operator|&&
literal|"key length does not match bytes written"
argument_list|)
expr_stmt|;
name|assert
argument_list|(
...
name|second
operator|&&
literal|"data length does not match bytes written"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
argument_list|)
block|;   }
comment|/// Read the number of buckets and the number of entries from a hash table
comment|/// produced by OnDiskHashTableGenerator::Emit, and advance the Buckets
comment|/// pointer past them.
specifier|static
name|std
operator|::
name|pair
...
begin_comment
comment|/// \brief Look up the stored data for a particular key with a known hash.
end_comment
...
expr_stmt|;
block|}
comment|/// Get the start of the item as written by the trait (after the hash and
comment|/// immediately before the key and value length).
specifier|const
name|unsigned
name|char
operator|*
