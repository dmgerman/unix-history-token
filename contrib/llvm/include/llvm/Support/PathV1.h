...
begin_comment
comment|//===- llvm/Support/PathV1.h - Path Operating System Concept ----*- C++ -*-===//
end_comment
...
define|#
directive|define
name|LLVM_PATH_DEPRECATED_MSG
parameter_list|(
name|replacement
parameter_list|)
define|\
value|"PathV1 has been deprecated and will be removed as soon as all LLVM and" \   " Clang clients have been moved over to PathV2. Please use `" #replacement \   "` from PathV2 instead."
end_define
...
comment|/// it is file:///. Other operating systems may have different notions of
...
argument_list|(
argument|static Path GetRootDirectory()
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(NOTHING)
argument_list|)
empty_stmt|;
...
comment|/// @brief Construct a path to an new, unique, existing temporary
comment|/// directory.
specifier|static
name|Path
...
comment|/// library paths suitable for linking into programs.
...
parameter_list|()
function_decl|;
comment|/// Return the suffix commonly used on file names that contain an
comment|/// executable.
comment|/// @returns The executable file suffix for the current platform.
comment|/// @brief Return the executable file suffix.
specifier|static
name|StringRef
name|GetEXESuffix
parameter_list|()
...
comment|/// component is the file or directory name occurring after the last
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|StringRef getLast() const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(path::filename)
argument_list|)
empty_stmt|;
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|StringRef getBasename() const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(path::stem)
argument_list|)
empty_stmt|;
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|StringRef getDirname() const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(path::parent_path)
argument_list|)
empty_stmt|;
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|StringRef getSuffix() const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(path::extension)
argument_list|)
empty_stmt|;
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|static bool isAbsolute(const char *NameStart, unsigned NameLen)
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(path::is_absolute)
argument_list|)
empty_stmt|;
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|bool hasMagicNumber(StringRef magic) const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(fs::has_magic)
argument_list|)
empty_stmt|;
...
expr_stmt|;
comment|/// This function determines if the path name in the object references a
comment|/// native object file by looking at it's magic number. The term object
comment|/// file is defined as "an organized collection of separate, named
comment|/// sequences of binary data." This covers the obvious file formats such
comment|/// as COFF and ELF, but it also includes llvm ir bitcode, archives,
comment|/// libraries, etc...
comment|/// @returns true if the file starts with the magic number for an object
comment|/// file.
comment|/// @brief Determine if the path references an object file.
name|bool
name|isObjectFile
parameter_list|()
function_decl|const;
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|bool exists() const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(fs::exists)
argument_list|)
empty_stmt|;
comment|/// This function determines if the path name references an
comment|/// existing directory.
comment|/// @returns true if the pathname references an existing directory.
comment|/// @brief Determines if the path is a directory in the file system.
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|bool isDirectory() const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(fs::is_directory)
argument_list|)
empty_stmt|;
comment|/// This function determines if the path name references an
comment|/// existing symbolic link.
comment|/// @returns true if the pathname references an existing symlink.
comment|/// @brief Determines if the path is a symlink in the file system.
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|bool isSymLink() const
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(fs::is_symlink)
argument_list|)
empty_stmt|;
...
specifier|const
expr_stmt|;
comment|/// This function checks that what we're trying to work only on a regular
comment|/// file. Check for things like /dev/null, any block special file, or
comment|/// other things that aren't "regular" regular files.
...
comment|/// When the \p suffix is empty, no action is performed.
...
name|LLVM_ATTRIBUTE_DEPRECATED
argument_list|(
argument|void makeAbsolute()
argument_list|,
argument|LLVM_PATH_DEPRECATED_MSG(fs::make_absolute)
argument_list|)
empty_stmt|;
comment|/// @}
...
comment|/// operating systems so we want to be careful and explicit about where we
...
name|Mach_O_PreloadExecutable_FileType
block|,
