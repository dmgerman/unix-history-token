...
specifier|const
block|{
comment|// It's theoretically possible for a thing to hash to zero.  In that case,
comment|// we will just compute the hash a few extra times, which is no worse that
comment|// we did before, which was to compute it always.
if|if
condition|(
name|static_cast
operator|<
...
return|;
block|}
comment|// Return true if the two expressions are exactly the same, including the
comment|// normally ignored fields.
name|virtual
name|bool
name|exactlyEquals
argument_list|(
