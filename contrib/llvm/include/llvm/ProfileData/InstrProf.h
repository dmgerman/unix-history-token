...
name|ValueData
argument_list|(
argument|RHS.ValueData                       ? llvm::make_unique<ValueProfData>(*RHS.ValueData)                       : nullptr
argument_list|)
block|{}
name|InstrProfRecord
operator|&
...
argument_list|)
block|{
comment|// Cast to /add/ const (should be an implicit_cast, ideally, if that's ever
comment|// implemented in LLVM) to call the const overload of this function, then
comment|// cast away the constness from the result.
name|auto
name|AR
operator|=
name|const_cast
...
operator|>
operator|&
name|getOrCreateValueSitesForKind
argument_list|(
argument|uint32_t ValueKind
argument_list|)
block|{
...
argument|uint32_t ValueKind
argument_list|,
argument|ValueMapType *HashKeys
argument_list|)
end_macro
