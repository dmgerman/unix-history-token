...
return|;
block|}
comment|/// \brief Return the actual namespace token, if this token starts a namespace
comment|/// block.
decl|const
name|FormatToken
modifier|*
...
argument_list|()
expr_stmt|;
comment|// Detect "(inline)? namespace" in the beginning of a line.
if|if
condition|(
name|NamespaceTok
operator|&&
...
argument_list|)
expr_stmt|;
comment|// Keep this at the end of the constructor to make sure everything here is
comment|// already initialized.
name|JsExtraKeywords
operator|=
name|std
operator|::
name|unordered_set
...
name|kw_yield
block|,
comment|// Keywords from the Java section.
name|kw_abstract
block|,
name|kw_extends
block|,
...
name|kw_qslots
decl_stmt|;
comment|/// \brief Returns \c true if \p Tok is a true JavaScript identifier, returns
comment|/// \c false if it is a keyword or a pseudo keyword.
name|bool
name|IsJavaScriptIdentifier
argument_list|(
specifier|const
name|FormatToken
operator|&
...
name|private
label|:
comment|/// \brief The JavaScript keywords beyond the C++ keyword set.
name|std
operator|::
name|unordered_set
operator|<
