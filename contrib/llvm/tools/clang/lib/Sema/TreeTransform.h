...
begin_comment
comment|// FIXME: this test is meant to workaround a problem (failing assertion)
end_comment

begin_comment
comment|// occurring if directly executing the code in the else branch.
end_comment
...
argument_list|)
block|{
comment|// If this implicit setter/getter refers to class methods, it cannot have any
comment|// dependent parts. Just retain the existing declaration.
if|if
condition|(
name|E
operator|->
...
begin_comment
comment|// We don't need to transform the getters/setters; they will never change.
end_comment
...
argument_list|)
block|{
comment|// Can never occur in a dependent context.
return|return
name|SemaRef
operator|.
name|Owned
