...
argument_list|,
argument|const UnexpandedParameterPack *Unexpanded
argument_list|,
argument|unsigned NumUnexpanded
argument_list|,
...
argument_list|,
argument|llvm::SmallVectorImpl<Expr *>&Outputs
argument_list|,
...
argument_list|)
block|;
comment|/// \brief Build a new C++0x auto type.
comment|///
...
condition|)
block|{
comment|// The object type and qualifier-in-scope really apply to the
comment|// leftmost entity.
name|ObjectType
operator|=
name|QualType
argument_list|()
...
begin_comment
unit|}
comment|/// \brief Transforms a type that was written in a scope specifier,
end_comment

begin_comment
comment|/// given an object type, the results of unqualified lookup, and
end_comment

begin_comment
comment|/// an already-instantiated prefix.
end_comment
...
begin_comment
comment|/// The object type is provided iff the scope specifier qualifies the
end_comment

begin_comment
comment|/// member of a dependent member-access expression.  The prefix is
end_comment

begin_comment
comment|/// provided iff the the scope specifier in which this appears has a
end_comment
...
begin_comment
comment|/// This is private to TreeTransform.
end_comment
...
argument_list|)
block|{
comment|// TODO: in some cases, we might be some verification to do here.
if|if
condition|(
name|ObjectType
operator|.
...
argument|const QualType *ParamTypes
argument_list|,
argument|llvm::SmallVectorImpl<QualType>&OutParamTypes
argument_list|,
argument|llvm::SmallVectorImpl<ParmVarDecl*> *PVars
argument_list|)
block|{
...
begin_comment
comment|// If the return type has not been determined yet, leave it as a dependent
end_comment

begin_comment
comment|// type; it'll get set when we process the body.
end_comment
...
begin_comment
comment|// If the return type wasn't explicitly set, it will have been marked as a
end_comment

begin_comment
comment|// dependent type (DependentTy); clear out the return type setting so
end_comment

begin_comment
comment|// we will deduce the return type when type-checking the block's body.
end_comment
