...
expr_stmt|;
enum|enum
name|PragmaVtorDispKind
block|{
name|PVDK_Push
block|,
comment|///< #pragma vtordisp(push, mode)
name|PVDK_Set
block|,
comment|///< #pragma vtordisp(mode)
name|PVDK_Pop
block|,
comment|///< #pragma vtordisp(pop)
name|PVDK_Reset
comment|///< #pragma vtordisp()
block|}
enum|;
enum|enum
...
name|PSK_Push_Set
block|,
comment|// #pragma (push[, id], "name")
name|PSK_Pop
block|,
comment|// #pragma (pop[, id])
name|PSK_Pop_Set
block|,
comment|// #pragma (pop[, id], "name")
block|}
enum|;
...
comment|///    objects
comment|///
comment|/// The stack always has at least one element in it.
name|SmallVector
operator|<
name|MSVtorDispAttr
operator|::
...
decl_stmt|;
comment|/// \brief counter for internal MS Asm label names.
name|unsigned
name|MSAsmLabelNameCounter
decl_stmt|;
...
block|}
empty_stmt|;
comment|/// Records and restores the vtordisp state on entry/exit of C++ method body.
name|class
name|VtorDispStackRAII
block|{
name|public
label|:
name|VtorDispStackRAII
...
argument|Sema&S
argument_list|,
argument|bool ShouldSaveAndRestore
argument_list|)
block|:
name|S
argument_list|(
...
argument_list|)
operator|,
name|ShouldSaveAndRestore
argument_list|(
name|ShouldSaveAndRestore
argument_list|)
operator|,
name|OldVtorDispStack
argument_list|()
block|{
if|if
condition|(
...
begin_comment
comment|/// \brief For compatibility with MSVC, we delay parsing of some default
end_comment

begin_comment
comment|/// template type arguments until instantiation time.  Emits a warning and
end_comment

begin_comment
comment|/// returns a synthesized DependentNameType that isn't really dependent on any
end_comment

begin_comment
comment|/// other template arguments.
end_comment

begin_function_decl
name|ParsedType
name|ActOnDelayedDefaultTemplateArg
parameter_list|(
specifier|const
name|IdentifierInfo
modifier|&
...
begin_function_decl
name|void
name|ActOnFinishInlineMethodDef
parameter_list|(
name|CXXMethodDecl
modifier|*
name|D
...
begin_comment
comment|/// \brief Check if module import may be found in the current context,
end_comment

begin_comment
comment|/// emit error if not.
end_comment

begin_function_decl
name|void
name|diagnoseMisplacedModuleImport
parameter_list|(
name|Module
modifier|*
name|M
...
begin_comment
comment|/// given selector. If no such method or only one method found, function returns
end_comment

begin_comment
comment|/// false; otherwise, it returns true
end_comment
...
begin_comment
comment|/// reference in the provided scope \a RSI. This will take into account the
end_comment

begin_comment
comment|/// semantics of the directive and associated clauses.
end_comment
...
begin_function_decl
name|bool
name|isOpenMPTargetCapturedVar
parameter_list|(
name|VarDecl
modifier|*
name|VD
...
begin_comment
comment|// this routine performs the default function/array converions, if ConvertRHS
end_comment
...
name|CFP_LastResort
block|,
comment|// Lowest priority. Only in effect if
comment|// LangOpts.CUDADisableTargetCallChecks is true.
name|CFP_Fallback
block|,
comment|// Low priority caller/callee combination
name|CFP_Best
block|,
comment|// Preferred caller/callee combination
block|}
enum|;
end_enum
