...
parameter_list|)
define|\
value|case Stmt::CLASS##Class:                                                     \     {                                                                          \       auto ILE = static_cast<CLASS *>(S);                                      \       if (auto Syn = ILE->isSemanticForm() ? ILE->getSyntacticForm() : ILE)    \         TRY_TO(WalkUpFrom##CLASS(Syn));                                        \       if (auto Sem = ILE->isSemanticForm() ? ILE : ILE->getSemanticForm())     \         TRY_TO(WalkUpFrom##CLASS(Sem));                                        \       break;                                                                   \     }
include|#
directive|include
file|"clang/AST/StmtNodes.inc"
block|}
...
value|\     if (TypeSourceInfo *TSI = D->getTypeAsWritten())                           \       TRY_TO(TraverseTypeLoc(TSI->getTypeLoc()));                              \                                                                                \     TRY_TO(TraverseNestedNameSpecifierLoc(D->getQualifierLoc()));              \     if (!getDerived().shouldVisitTemplateInstantiations()&&                   \         D->getTemplateSpecializationKind() != TSK_ExplicitSpecialization)      \
...
operator|>
operator|::
name|VisitOMPTaskReductionClause
argument_list|(
argument|OMPTaskReductionClause *C
argument_list|)
block|{
name|TRY_TO
argument_list|(
