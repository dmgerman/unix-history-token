...
begin_comment
comment|//== GRState.h - Path-sensitive "State" for tracking values -----*- C++ -*--==//
end_comment
...
name|GRStateManager
decl_stmt|;
comment|/// GRState - This class encapsulates:
comment|///
...
comment|///
comment|///  GRState is intended to be used as a functional object; that is,
...
argument_list|()
expr_stmt|;
comment|/// Return the GRStateManager associated with this state.
name|GRStateManager
operator|&
...
expr_stmt|;
block|}
comment|/// Profile - Profile the contents of a GRState object for use in a
comment|///  FoldingSet.  Two GRState objects are considered equal if they
...
comment|// for modeling path-sensitivity in ExprEngine/GRState.
comment|//
...
comment|//
comment|// The output of "assume*" is a new GRState object with the added constraints.
...
decl|const
decl_stmt|;
comment|/// invalidateRegion - Returns the state with bindings for the given region
comment|///  cleared from the store. See invalidateRegions.
specifier|const
name|GRState
modifier|*
...
name|Alloc
expr_stmt|;
comment|/// A vector of recently allocated GRStates that can potentially be
comment|/// reused.
name|std
operator|::
name|vector
...
expr_stmt|;
comment|/// A vector of GRStates that we can reuse.
name|std
operator|::
name|vector
operator|<
...
parameter_list|)
function_decl|;
comment|/// Periodically called by ExprEngine to recycle GRStates that were
