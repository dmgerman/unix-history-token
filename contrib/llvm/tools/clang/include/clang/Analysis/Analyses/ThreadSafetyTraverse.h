...
ifndef|#
directive|ifndef
name|LLVM_CLANG_ANALYSIS_ANALYSES_THREADSAFETYTRAVERSE_H
end_ifndef
...
comment|// E is a reference, so this can be use for in-place updates.
comment|// The type T must be a subclass of SExpr.
name|template
operator|<
name|class
name|T
...
return|;
block|}
comment|// Override this method to do something for every expression.
comment|// Does not allow in-place updates.
name|typename
name|R
operator|::
name|R_SExpr
...
name|TRV_Normal
block|,
comment|// ordinary subexpressions
name|TRV_Decl
block|,
comment|// declarations (e.g. function bodies)
name|TRV_Lazy
block|,
comment|// expressions that require lazy evaluation
name|TRV_Type
...
name|compareByCase
argument_list|(
argument|const SExpr *E1
argument_list|,
argument|const SExpr* E2
argument_list|)
block|{
switch|switch
condition|(
...
argument_list|)
block|{
comment|// Wildcards match anything.
if|if
condition|(
name|E1
operator|->
...
begin_comment
comment|// otherwise normal equality.
end_comment
...
name|enterScope
argument_list|(
argument|const Variable* V1
argument_list|,
argument|const Variable* V2
argument_list|)
end_macro
...
begin_comment
unit|};
comment|// inline std::ostream& operator<<(std::ostream& SS, StringRef R) {
end_comment

begin_comment
comment|//   return SS.write(R.data(), R.size());
end_comment
...
name|CStyle
block|;
comment|// Print exprs in C-like syntax.
name|public
operator|:
name|PrettyPrinter
argument_list|(
...
name|printLiteralT
argument_list|(
argument|const LiteralT<uint8_t> *E
argument_list|,
argument|StreamType&SS
argument_list|)
block|{
