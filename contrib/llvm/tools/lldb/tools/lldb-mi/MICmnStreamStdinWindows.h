...
function_decl|;
name|bool
name|InputAvailableApplication
parameter_list|(
name|bool
modifier|&
name|vwbAvail
...
name|m_pStdinBuffer
decl_stmt|;
comment|// Custom buffer to store std input
name|MIuint
name|m_nBytesToBeRead
decl_stmt|;
comment|// Checks that ::fgets() is holding on to data while ::PeekNamedPipe() returns nothing which causes a problem
name|bool
name|m_bRunningInConsoleWin
decl_stmt|;
comment|// True = The application is being run in a Windows command line prompt window, false = by other means
block|}
end_decl_stmt
