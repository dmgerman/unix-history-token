...
block|{
comment|/* 		 * If we have arguments, use them as our manpaths. 		 * If we don't, grok from manpath(1) or however else 		 * manconf_parse() wants to do it. 		 */
if|if
condition|(
name|argc
operator|>
...
parameter_list|)
block|{
comment|/* 	 * The mpage list is processed in the opposite order to which pages are 	 * added, so traverse the hierarchy in reverse alpha order, resulting 	 * in database inserts in alpha order. This is not required for correct 	 * operation, but is helpful when inspecting the database during 	 * development. 	 */
return|return
operator|-
name|strcmp
argument_list|(
