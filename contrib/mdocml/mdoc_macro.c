...
begin_comment
comment|/*	$Id: mdoc_macro.c,v 1.183 2015/02/12 12:24:33 schwarze Exp $ */
end_comment
...
name|out
goto|;
comment|/* 	 * If there is an open (i.e., unvalidated) sub-block requiring 	 * explicit close-out, postpone switching the current block from 	 * head to body until the rew_pending() call closing out that 	 * sub-block. 	 */
for|for
control|(
name|n
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * If there is an open sub-block requiring explicit close-out, 	 * postpone closing out the current block until the 	 * rew_pending() call closing out the sub-block. 	 */
for|for
control|(
name|n
operator|=
