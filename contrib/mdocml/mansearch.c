...
parameter_list|)
define|\
value|do { if (SQLITE_OK != sqlite3_bind_text \ 		((_s), (_i)++, (_v), -1, SQLITE_STATIC)) \ 		errx((int)MANDOCLEVEL_SYSERR, "%s", sqlite3_errmsg((_db))); \ 	} while (0)
end_define
...
parameter_list|)
define|\
value|do { if (SQLITE_OK != sqlite3_bind_int64 \ 		((_s), (_i)++, (_v))) \ 		errx((int)MANDOCLEVEL_SYSERR, "%s", sqlite3_errmsg((_db))); \ 	} while (0)
end_define
...
parameter_list|)
define|\
value|do { if (SQLITE_OK != sqlite3_bind_blob \ 		((_s), (_i)++, (&_v), sizeof(_v), SQLITE_STATIC)) \ 		errx((int)MANDOCLEVEL_SYSERR, "%s", sqlite3_errmsg((_db))); \ 	} while (0)
end_define
...
expr_stmt|;
block|}
comment|/* Fetch the next name, rejecting sec/arch mismatches. */
name|sec
operator|=
operator|(
specifier|const
...
condition|)
continue|continue;
comment|/* Handle unusual file name extensions. */
if|if
condition|(
name|firstname
operator|==
...
argument_list|)
expr_stmt|;
comment|/* If none of the files is usable, use the first name. */
if|if
condition|(
name|mpage
operator|->
