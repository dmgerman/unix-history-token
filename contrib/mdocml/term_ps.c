...
parameter_list|)
block|{
comment|/* 	 * Strong closure: if we have a last-char, spit it out after 	 * checking that we're in the right font mode.  This will of 	 * course open a new scope, if applicable. 	 * 	 * Following this, close out any scope that's open. 	 */
if|if
condition|(
name|p
operator|->
...
expr_stmt|;
block|}
comment|/* 	 * Do not print the current character yet because font 	 * instructions might follow; only remember it. 	 * For the first character, nothing else is done. 	 * The final character will get printed from ps_fclose(). 	 */
name|p
operator|->
name|ps
operator|->
...
name|c
expr_stmt|;
comment|/* 	 * For an overstrike, back up to the previous position. 	 */
if|if
condition|(
name|savecol
operator|!=
