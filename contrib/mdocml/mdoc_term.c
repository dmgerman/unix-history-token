...
begin_comment
comment|/*	$Id: mdoc_term.c,v 1.238 2011/11/13 13:15:14 schwarze Exp $ */
end_comment
...
directive|define
name|DECL_ARGS
value|struct termp *p, \ 		  struct termpair *pair, \ 	  	  const struct mdoc_meta *m, \ 		  const struct mdoc_node *n
end_define
...
name|pair
expr_stmt|;
comment|/* 	 * Keeps only work until the end of a line.  If a keep was 	 * invoked in a prior line, revert it to PREKEEP. 	 * 	 * Also let SYNPRETTY sections behave as if they were wrapped 	 * in a `Bk' block. 	 */
if|if
condition|(
name|TERMP_KEEP
operator|&
...
block|}
block|}
comment|/* 	 * Since SYNPRETTY sections aren't "turned off" with `Ek', 	 * we have to intuit whether we should disable formatting. 	 */
if|if
condition|(
operator|!
operator|(
