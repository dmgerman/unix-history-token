...
name|copyfrom_root
decl_stmt|;
comment|/* E.g. when verifying corrupted repositories, their changed path          lists may contain an ADD for "/".  The delta path driver will          call us with a NULL parent in that case. */
if|if
condition|(
operator|*
name|edit_path
...
name|_
argument_list|(
literal|"Root directory already exists."
argument_list|)
argument_list|)
return|;
comment|/* A NULL parent_baton will cause a segfault.  It should never be           NULL for non-root paths. */
name|SVN_ERR_ASSERT
argument_list|(
name|parent_baton
argument_list|)
