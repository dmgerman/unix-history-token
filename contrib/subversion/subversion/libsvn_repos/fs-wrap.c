...
argument_list|)
expr_stmt|;
comment|/* Remove any ephemeral transaction properties.  If the commit fails      we will attempt to restore the properties but if that fails, or      the process is killed, the properties will be lost. */
name|SVN_ERR
argument_list|(
...
condition|)
block|{
comment|/* The commit failed, try to restore the ephemeral properties. */
name|iterpool
operator|=
name|svn_pool_create
argument_list|(
...
argument_list|)
return|;
comment|/* We have API promise that *TXN_P is unaffected on failure. */
operator|*
name|txn_p
operator|=
name|txn
...
begin_comment
comment|/* Implements svn_fs_lock_callback_t.  Used by svn_repos_fs_lock_many    and svn_repos_fs_unlock_many to record the paths for use by post-    hooks, forward to the supplied callback and record any callback    error. */
end_comment
...
name|NULL
argument_list|,
literal|"Cannot lock path, no authenticated username available."
argument_list|)
return|;
comment|/* Run pre-lock hook.  This could throw error, preventing      svn_fs_lock2() from happening for that path. */
for|for
control|(
name|hi
operator|=
...
argument_list|)
expr_stmt|;
comment|/* If there are locks run the post-lock even if there is an error. */
if|if
condition|(
name|baton
operator|.
...
name|_
argument_list|(
literal|"Locking succeeded, but post-lock hook failed"
argument_list|)
argument_list|)
expr_stmt|;
name|err
...
begin_comment
comment|/* Implements svn_fs_lock_callback_t.  Used by svn_repos_fs_lock and    svn_repos_fs_unlock to record the lock and error from    svn_repos_fs_lock_many and svn_repos_fs_unlock_many. */
end_comment
...
name|_
argument_list|(
literal|"Cannot unlock, no authenticated username available"
argument_list|)
argument_list|)
return|;
comment|/* Run pre-unlock hook.  This could throw error, preventing      svn_fs_unlock_many() from happening for that path. */
for|for
control|(
name|hi
operator|=
...
argument_list|)
expr_stmt|;
comment|/* If there are 'unlocks' run the post-unlock even if there is an error. */
if|if
condition|(
name|baton
operator|.
