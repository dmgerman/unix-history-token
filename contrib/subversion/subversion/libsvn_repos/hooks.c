...
directive|ifdef
name|WIN32
comment|/* For WIN32, we need to check with file name extension(s) added.       As Windows Scripting Host (.wsf) files can accommodate (at least)      JavaScript (.js) and VB Script (.vbs) code, extensions for the      corresponding file types need not be enumerated explicitly. */
literal|".exe"
block|,
literal|".cmd"
block|,
...
condition|)
block|{
comment|/* Special handling for svn_fs_access_t * created by using deprecated              svn_fs_access_add_lock_token() function. */
name|path
operator|=
literal|""
expr_stmt|;
