...
name|TRUE
decl_stmt|;
comment|/* Check related files for content changes to avoid reporting        * unchanged copies of files to the client as an open_file() call        * and change_file_prop()/apply_textdelta() calls with no-op changes.        * The client will otherwise raise unnecessary tree conflicts. */
if|if
condition|(
operator|!
name|b
...
name|NULL
expr_stmt|;
comment|/* The only special cases where we don't process the entry are               - When requested_depth is files but the reported path is              a directory.  This is technically a client error, but we              handle it anyway, by skipping the entry.               - When the reported depth is svn_depth_exclude.           */
if|if
condition|(
operator|!
operator|(
...
expr_stmt|;
block|}
comment|/* iterpool is destroyed by destroying its parent (subpool) below */
block|}
name|svn_pool_destroy
argument_list|(
name|subpool
