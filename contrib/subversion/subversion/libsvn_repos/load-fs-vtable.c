...
decl_stmt|;
comment|/* The oldest revision loaded from the dump stream.  If no revisions      have been loaded yet, this is set to SVN_INVALID_REVNUM. */
name|svn_revnum_t
...
name|revision_baton
block|{
comment|/* rev num from dump file */
name|svn_revnum_t
name|rev
decl_stmt|;
name|svn_fs_txn_t
...
name|datestamp
decl_stmt|;
comment|/* (rev num from dump file) minus (rev num to be committed) */
name|apr_int32_t
name|rev_offset
decl_stmt|;
name|svn_boolean_t
name|skipped
decl_stmt|;
comment|/* Array of svn_prop_t with revision properties. */
...
begin_comment
comment|/* Examine the mergeinfo in INITIAL_VAL, renumber revisions in rangelists    as appropriate, and return the (possibly new) mergeinfo in *FINAL_VAL    (allocated from POOL).     Adjust any mergeinfo revisions not older than OLDEST_DUMPSTREAM_REV by    using REV_MAP which maps (svn_revnum_t) old rev to (svn_revnum_t) new rev.     Adjust any mergeinfo revisions older than OLDEST_DUMPSTREAM_REV by    (-OLDER_REVS_OFFSET), dropping any that become<= 0.  */
end_comment
...
condition|)
block|{
comment|/* predates_stream_mergeinfo := mergeinfo that refers to revs before          oldest_dumpstream_rev */
name|SVN_ERR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* mergeinfo := mergeinfo that refers to revs>= oldest_dumpstream_rev */
name|SVN_ERR
argument_list|(
name|svn_mergeinfo__filter_mergeinfo_by_ranges
argument_list|(
...
condition|)
block|{
comment|/* Since the start revision of svn_merge_range_t are not                  inclusive there is one possible valid start revision that                  won't be found in the REV_MAP mapping of load stream                  revsions to loaded revisions: The revision immediately                  preceding the oldest revision from the load stream.                  This is a valid revision for mergeinfo, but not a valid                  copy from revision (which REV_MAP also maps for) so it                  will never be in the mapping.                   If that is what we have here, then find the mapping for the                  oldest rev from the load stream and subtract 1 to get the                  renumbered, non-inclusive, start revision. */
name|rev_from_map
operator|=
name|get_revision_mapping
argument_list|(
...
begin_comment
comment|/* Make a node baton, parsing the relevant HEADERS.  *  * If RB->pb->parent_dir:  *   prefix it to NB->path  *   prefix it to NB->copyfrom_path (if present)  */
end_comment
...
begin_comment
comment|/* Make a revision baton, parsing the relevant HEADERS.  *  * Set RB->skipped iff the revision number is outside the range given in PB.  */
end_comment
...
expr_stmt|;
block|}
comment|/* If we're parsing revision 0, only the revision props are (possibly)      interesting to us: when loading the stream into an empty      filesystem, then we want new filesystem's revision 0 to have the      same props.  Otherwise, we just ignore revision 0 in the stream. */
operator|*
name|revision_baton
operator|=
name|rb
...
begin_comment
comment|/* Perform a copy or a plain add.  *  * For a copy, also adjust the copy-from rev, check any copy-source checksum,  * and send a notification.  */
end_comment
...
name|SVN_NO_ERROR
return|;
comment|/* If we're ignoring dates, and this is one, we're done here. */
if|if
condition|(
name|is_date
operator|&&
...
name|SVN_NO_ERROR
return|;
comment|/* Collect property changes to apply them in one FS call in      close_revision. */
name|prop
operator|=
operator|&
name|APR_ARRAY_PUSH
...
condition|)
block|{
comment|/* Prefix the merge source paths with PARENT_DIR. */
...
name|warning
operator|=
name|svn_repos_notify_warning_invalid_mergeinfo
expr_stmt|;
name|notify
operator|->
name|warning_str
...
return|;
block|}
comment|/* If the dumpstream doesn't have an 'svn:date' property and we      aren't ignoring the dates in the dumpstream altogether, remove      any 'svn:date' revision property that was set by FS layer when      the TXN was created.  */
if|if
condition|(
operator|!
operator|(
...
expr_stmt|;
block|}
comment|/* Apply revision property changes. */
if|if
condition|(
name|rb
operator|->
...
name|svn_error_t
modifier|*
name|svn_repos_get_fs_build_parser5
parameter_list|(
specifier|const
