...
ifdef|#
directive|ifdef
name|SVN_UNICODE_NORMALIZATION_FIXES
end_ifdef
...
begin_comment
comment|/* SVN_UNICODE_NORMALIZATION_FIXES */
end_comment
...
begin_comment
comment|/* Limit the length of a GLOB or LIKE pattern. */
end_comment
...
directive|ifdef
name|SVN_UNICODE_NORMALIZATION_FIXES
comment|/* Buffers for SQLite extensoins. */
name|svn_membuf_t
name|sqlext_buf1
decl_stmt|;
name|svn_membuf_t
...
name|pool
parameter_list|)
value|do                          \ {                                                                 \   int sqlite_err__temp = (x);                                     \   if (sqlite_err__temp != SQLITE_OK)                              \     {                                                             \       const char *sqlite_err__msg                                 \         = apr_pstrdup(pool, sqlite3_errmsg((db)->db3));           \       return svn_error_compose_create(                            \            svn_error_createf(SQLITE_ERROR_CODE(sqlite_err__temp), \                              NULL, "sqlite[S%d]: %s",             \                              sqlite_err__temp, sqlite_err__msg),  \            svn_sqlite__close(db));                                \     }                                                             \ } while (0)
end_define
...
parameter_list|)
block|{
comment|/* No need to reset again after a first attempt */
name|stmt
operator|->
name|needs_reset
operator|=
name|FALSE
expr_stmt|;
comment|/* Clear bindings first, as there are no documented reasons      why this would ever fail, but keeping variable bindings      when reset is not what we expect. */
name|SQLITE_ERR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Reset last, as this *will* fail if the statement failed since      the last time it was reset, while reporting just the same failure.      (In this case the statement is also properly reset).       See the sqlite3_reset() documentation for more details. */
name|SQLITE_ERR
argument_list|(
name|sqlite3_reset
argument_list|(
...
condition|)
block|{
comment|/* Another thread may have created the file, that's OK. */
name|svn_error_t
modifier|*
name|err
init|=
...
directive|endif
comment|/* Open the database. Note that a handle is returned, even when an error        occurs (except for out-of-memory); thus, we can safely use it to        extract an error message and construct an svn_error_t.  SQLite always        requires sqlite3_close() after sqlite3_open_v2() while Subversion        typically does not require close() after an open() that returns an        error.  So we must ensure we close the handle if this function, or        the caller svn_sqlite__open, returns an error to the application. */
block|{
specifier|const
name|char
modifier|*
...
name|vFs
operator|=
literal|"win32-longpath"
expr_stmt|;
comment|/* Enable long paths in sqlite */
comment|/* Long paths must be absolute */
if|if
condition|(
operator|!
...
argument_list|)
expr_stmt|;
comment|/* Convert the path to a properly canonicalized \\?\C:\long\path */
name|SVN_ERR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* And convert it back to UTF-8 because there is no               sqlite3_open16_v2() yet */
name|SVN_ERR
argument_list|(
...
name|APR_SUCCESS
return|;
comment|/* Finalize any prepared statements. */
if|if
condition|(
name|db
operator|->
...
begin_comment
comment|/* Unicode normalizing collation for WC paths */
end_comment

begin_function
specifier|static
name|int
name|collate_ucs_nfd
parameter_list|(
name|void
modifier|*
name|baton
...
condition|)
block|{
comment|/* There is really nothing we can do here if an error occurs          during Unicode normalizetion, and attempting to recover could          result in the wc.db index being corrupted. Presumably this          can only happen if the index already contains invalid UTF-8          strings, which should never happen in any case ... */
name|SVN_ERR_MALFUNCTION_NO_RETURN
argument_list|()
expr_stmt|;
block|}
...
name|context
argument_list|,
literal|"LIKE or GLOB pattern too complex"
argument_list|,
operator|-
literal|1
argument_list|)
...
begin_comment
comment|/* Unicode normalizing implementation of GLOB */
end_comment
...
begin_comment
comment|/* Unicode normalizing implementation of LIKE */
end_comment
...
directive|ifdef
name|SVN_UNICODE_NORMALIZATION_FIXES
comment|/* Create extension buffers with space for 200 UCS-4 characters. */
...
argument_list|)
expr_stmt|;
comment|/* Register collation and LIKE and GLOB operator replacements. */
name|SQLITE_ERR_CLOSE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* ### Is it really necessary to override these functions?          I would assume the default implementation to be collation agnostic?          And otherwise our implementation should be...           The default implementation is in some cases index backed, while our          implementation can't be. With an index based on the collation it could          be. */
name|SQLITE_ERR_CLOSE
argument_list|(
...
name|db
argument_list|,
comment|/* The default behavior of the LIKE operator is to ignore case                  for ASCII characters. Hence, by default 'a' LIKE 'A' is true.                  The case_sensitive_like pragma installs a new application-                  defined LIKE function that is either case sensitive or                  insensitive depending on the value of the case_sensitive_like                  pragma. */
...
condition|)
block|{
comment|/* Rollback failed, use a specific error code. */
name|err
operator|=
name|svn_error_create
argument_list|(
...
name|_
argument_list|(
literal|"SQLite transaction rollback failed"
argument_list|)
argument_list|)
expr_stmt|;
block|}
