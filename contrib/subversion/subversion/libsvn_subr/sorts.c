...
name|idx
decl_stmt|;
comment|/* If provided, try the index following *HINT (i.e. probably the last    * hit location) first.  This speeds up linear scans. */
if|if
condition|(
name|hint
condition|)
block|{
comment|/* We intend to insert right behind *HINT.        * Exit this function early, if we actually can. */
name|idx
operator|=
operator|*
name|hint
...
condition|)
block|{
comment|/* We intend to insert after the last entry.            * That is only allowed if that last entry is smaller than KEY.            * In that case, there will be no current entry, i.e. we must            * return NULL. */
name|apr_size_t
name|offset
decl_stmt|;
operator|*
...
condition|)
block|{
comment|/* Intend to insert at a position inside the array, i.e. not            * at one of the boundaries.  The predecessor must be smaller            * and the current entry at IDX must be larger than KEY. */
name|void
modifier|*
name|previous
decl_stmt|;
...
condition|)
block|{
comment|/* Intend to insert at the beginning of an non-empty array.            * That requires the first entry to be larger than KEY. */
operator|*
name|hint
operator|=
literal|0
...
begin_comment
comment|/* Our priority queue data structure:  * Simply remember the constructor parameters.  */
end_comment
...
block|{
comment|/* the queue elements, ordered as a heap according to COMPARE_FUNC */
name|apr_array_header_t
modifier|*
name|elements
decl_stmt|;
comment|/* predicate used to order the heap */
name|int
function_decl|(
modifier|*
name|compare_func
...
begin_comment
comment|/* Return TRUE, if heap element number LHS in QUEUE is smaller than element  * number RHS according to QUEUE->COMPARE_FUNC  */
end_comment
...
name|elt_size
decl_stmt|;
comment|/* nelts is never negative */
name|assert
argument_list|(
name|lhs
operator|<
...
begin_comment
comment|/* Exchange elements number LHS and RHS in QUEUE.  */
end_comment
...
begin_comment
comment|/* Move element number IDX to lower indexes until the heap criterion is  * fulfilled again.  */
end_comment

begin_function
specifier|static
name|void
name|heap_bubble_down
parameter_list|(
name|svn_priority_queue__t
modifier|*
name|queue
...
begin_comment
comment|/* Move element number IDX to higher indexes until the heap criterion is  * fulfilled again.  */
end_comment
...
parameter_list|)
block|{
comment|/* we cannot duplicate elements due to potential array re-allocs */
name|assert
argument_list|(
name|element
operator|&&
