...
begin_comment
comment|/* Does the SDK assume Windows Vista+? */
end_comment

begin_typedef
typedef|typedef
struct|struct
name|_FILE_RENAME_INFO
block|{
name|BOOL
...
decl_stmt|;
block|}
name|FILE_RENAME_INFO
operator|,
typedef|*
name|PFILE_RENAME_INFO
typedef|;
end_typedef
...
begin_comment
comment|/* One-time initialization of the late bound Windows API functions. */
end_comment
...
specifier|volatile
name|svn_atomic_t
name|win_dynamic_imports_state
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* Pointer to GetFinalPathNameByHandleW function from kernel32.dll. */
end_comment
...
name|WINAPI
modifier|*
name|SetFileInformationByHandle_t
function_decl|)
parameter_list|(
name|HANDLE
name|hFile
...
specifier|static
name|GETFINALPATHNAMEBYHANDLE
name|get_final_path_name_by_handle_proc
init|=
name|NULL
decl_stmt|;
end_decl_stmt
...
specifier|static
name|SetFileInformationByHandle_t
name|set_file_information_by_handle_proc
init|=
name|NULL
decl_stmt|;
end_decl_stmt
...
name|status
return|;
comment|/* Can't create if there is something */
if|if
condition|(
name|flag
operator|&
...
name|status
return|;
comment|/* Retrying won't fix this */
block|}
block|}
block|}
endif|#
...
directive|ifdef
name|WIN32
comment|/* Don't retry on ERROR_ACCESS_DENIED, as that typically signals a      permanent error */
if|if
condition|(
name|apr_err
operator|==
...
condition|)
block|{
comment|/* Our caller doesn't know if we left a file or not if we return          an error. Better to cleanup after ourselves if we created the          file. */
return|return
name|svn_error_trace
argument_list|(
name|svn_error_compose_create
...
begin_comment
comment|/* This is semantically the same as the APR utf8_to_unicode_path    function, but reimplemented here because APR does not export it. */
end_comment
...
condition|)
block|{
comment|/* This is an ordinary absolute path. */
name|prefix
operator|=
literal|L"\\\\?\\"
expr_stmt|;
...
condition|)
block|{
comment|/* This is a UNC path */
name|source
operator|+=
literal|2
expr_stmt|;
comment|/* Skip the leading slashes */
name|prefix
operator|=
literal|L"\\\\?\\UNC\\"
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Convert slashes to backslashes because the \\?\ path format        does not allow backslashes as path separators. */
operator|*
name|result
operator|=
name|buffer
...
begin_comment
comment|/* This is semantically the same as the APR unicode_to_utf8_path    function, but reimplemented here because APR does not export it. */
end_comment
...
return|;
block|}
comment|/* We know that the non-empty buffer returned from the UTF-16 to        UTF-8 conversion function is in fact writable. */
name|buffer
operator|=
operator|(
name|char
...
name|utf8_buffer
expr_stmt|;
comment|/* Skip the leading 4 characters if the path begins \\?\, or substitute      * // for the \\?\UNC\ path prefix, allocating the maximum string      * length based on the remaining string, plus the trailing null.      * then transform \\'s back into /'s since the \\?\ form never      * allows '/' path separators, and APR always uses '/'s.      */
if|if
condition|(
literal|0
operator|==
...
name|_
argument_list|(
literal|"Can't get attributes of file '%s'"
argument_list|)
argument_list|,
name|svn_dirent_local_style
argument_list|(
...
name|_
argument_list|(
literal|"Can't set attributes of file '%s'"
argument_list|)
argument_list|,
name|svn_dirent_local_style
argument_list|(
...
name|kernel32
argument_list|,
literal|"GetFinalPathNameByHandleW"
argument_list|)
expr_stmt|;
name|set_file_information_by_handle_proc
operator|=
...
name|kernel32
argument_list|,
literal|"SetFileInformationByHandle"
argument_list|)
expr_stmt|;
block|}
return|return
...
name|data
decl_stmt|;
comment|/* reserve one char for terminating zero. */
name|DWORD
name|wdest_len
init|=
sizeof|sizeof
...
argument_list|()
expr_stmt|;
comment|/* Close file/directory handle in any case. */
name|apr_file_close
argument_list|(
name|file
argument_list|)
expr_stmt|;
comment|/* GetFinaPathNameByHandleW returns number of characters copied to          * output buffer. Returns zero on error. Returns required buffer size          * if supplied buffer is not enough. */
if|if
condition|(
name|rv
operator|>
...
return|;
block|}
comment|/* GetFinaPathNameByHandleW doesn't add terminating NUL. */
name|wdest
index|[
name|rv
index|]
...
argument_list|)
expr_stmt|;
comment|/* The result is already in the correct pool, so avoid copying            it to create the string. */
operator|*
name|dest
operator|=
...
begin_comment
comment|/* Wrapper around Windows API function SetFileInformationByHandle() that  * returns APR status instead of boolean flag. */
end_comment

begin_function
specifier|static
name|apr_status_t
name|win32_set_file_information_by_handle
parameter_list|(
name|HANDLE
name|hFile
parameter_list|,
...
name|path_len
expr_stmt|;
comment|/* The rename info struct doesn't need hacks for long paths,      so no ugly escaping calls here */
name|rename_info
operator|=
name|apr_pcalloc
argument_list|(
...
condition|)
block|{
comment|/* Set the destination file writable because Windows will not allow          us to rename when final_abspath is read-only. */
name|SVN_ERR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* This file creation is racy.          We don't care as long as file gets created at all. */
name|err
operator|=
name|svn_io_file_create_empty
argument_list|(
...
expr_stmt|;
block|}
comment|/* Finally, lock the file - if it exists */
if|if
condition|(
operator|!
name|err
...
name|filehand
decl_stmt|;
comment|/* ### In apr 1.4+ we could delegate most of this function to          apr_file_sync(). The only major difference is that this doesn't          contain the retry loop for EINTR on linux. */
...
block|}
decl_stmt|;
comment|/* In some cases we get size 0 and no error for non files,           so we also check for the name. (= cached in apr_file_t) */
if|if
condition|(
operator|!
...
condition|)
block|{
comment|/* we've got the file length. Now, read it in one go. */
name|svn_boolean_t
name|eof
decl_stmt|;
name|res_initial_len
...
name|path_apr
decl_stmt|;
comment|/* APR doesn't like our canonical path format for current directory */
if|if
condition|(
name|path
index|[
...
name|FALSE
decl_stmt|;
comment|/* paranoia check: huge blocks on 32 bit machines may cause overflows */
name|SVN_ERR_ASSERT
argument_list|(
name|block_size
operator|==
...
argument_list|)
expr_stmt|;
comment|/* default for invalid block sizes */
if|if
condition|(
name|block_size
operator|==
...
argument_list|)
expr_stmt|;
comment|/* don't try to set a buffer size for non-buffered files! */
if|if
condition|(
name|file_buffer_size
operator|==
...
condition|)
block|{
comment|/* FILE has the wrong buffer size. correct it */
name|char
modifier|*
name|buffer
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* seek to the start of the block and cause APR to read 1 block */
name|aligned_offset
operator|=
name|offset
operator|-
...
argument_list|)
expr_stmt|;
comment|/* read may fail if we seek to or behind EOF.  That's ok then. */
if|if
condition|(
name|status
operator|!=
...
return|;
block|}
comment|/* finally, seek to the OFFSET the caller wants */
name|desired_offset
operator|=
name|offset
expr_stmt|;
...
name|N_
argument_list|(
literal|"Can't seek in file '%s'"
argument_list|)
argument_list|,
name|N_
argument_list|(
literal|"Can't seek in stream"
argument_list|)
argument_list|,
name|scratch_pool
argument_list|)
return|;
comment|/* return the buffer start that we (probably) enforced */
if|if
condition|(
name|buffer_start
condition|)
...
condition|)
block|{
comment|/* svn_io_file_flush_to_disk() can be very expensive, so use the          cheaper standard flush if the file is created as temporary file          anyway */
if|if
condition|(
name|delete_when
operator|==
...
name|_
argument_list|(
literal|"Can't write '%s' atomically"
argument_list|)
argument_list|,
name|svn_dirent_local_style
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Restore original file pointer, if necessary.      It's currently at OFFSET. */
if|if
condition|(
name|position
operator|<
