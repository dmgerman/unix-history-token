...
name|spill_size
decl_stmt|;
comment|/* When false, do not delete the spill file when it is closed. */
name|svn_boolean_t
name|delete_on_close
decl_stmt|;
comment|/* When true, and the amount of data written to the spillbuf is      larger than MAXSIZE, all spillbuf contents will be written to the      spill file. */
name|svn_boolean_t
name|spill_all_contents
decl_stmt|;
comment|/* The directory in which the spill file is created. */
specifier|const
name|char
modifier|*
name|dirpath
decl_stmt|;
comment|/* The name of the temporary spill file. */
specifier|const
name|char
modifier|*
name|filename
...
begin_comment
comment|/* Extended spillbuf initialization. */
end_comment

begin_function
specifier|static
name|void
name|init_spillbuf_extended
parameter_list|(
name|svn_spillbuf_t
modifier|*
name|buf
...
begin_comment
comment|/* Common constructor for initializing spillbufs.    Used by svn_spillbuf__create, svn_spilbuff__reader_create. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Optionally write the memory contents into the file. */
if|if
condition|(
name|buf
operator|->
...
expr_stmt|;
block|}
comment|/* Adjust the start offset for reading from the spill file.               ### FIXME: Instead, we should simply discard the memory              buffers; but currently some tests expect to read data in              the same chunk sizes as were written, so we'll leave this              change for later.*/
name|buf
operator|->
name|spill_start
operator|=
