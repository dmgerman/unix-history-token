...
name|FALSE
return|;
comment|/* '.' are rare. So, search for them globally. There will often be no    * more than one hit.  Also note that we already checked for invalid    * starts and endings, i.e. we only need to check for "/./"    */
for|for
control|(
name|dot_pos
operator|=
...
argument_list|)
return|;
comment|/* Find the HOSTNAME portion and the PATH portion of the URL.  The host      name is between the "file://" prefix and the next occurrence of '/'.  We      are considering everything from that '/' until the end of the URL to be      the absolute path portion of the URL.      If we got just "file://", treat it the same as "file:///". */
name|hostname
operator|=
name|url
operator|+
