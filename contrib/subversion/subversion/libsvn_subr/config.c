...
begin_comment
comment|/* CONFIG_DIR provides an override for the default behavior of reading    the default set of overlay files described by read_all()'s doc    string.  Returns non-NULL *CFG or an error. */
end_comment
...
begin_comment
comment|/* Return the value for KEY in HASH.  If CASE_SENSITIVE is FALSE,    BUFFER will be used to construct the normalized hash key. */
end_comment
...
name|tmp_pool
decl_stmt|;
comment|/* setting read-only mode should have expanded all values            * automatically. */
name|assert
argument_list|(
operator|!
name|cfg
...
name|FALSE
return|;
comment|/* does the option even exist? */
name|opt
operator|=
name|find_option
argument_list|(
...
name|FALSE
return|;
comment|/* no expansion necessary */
return|return
name|TRUE
return|;
block|}
...
block|}
elseif|else
comment|/* before attempting to expand an option, check for the placeholder.          * If there is none, there is no point in calling expand_option_value.          */
if|if
condition|(
name|default_value
operator|&&
...
name|opt
decl_stmt|;
comment|/* Ignore write attempts to r/o configurations.    *    * Since we should never try to modify r/o data, trigger an assertion    * in debug mode.    */
ifdef|#
directive|ifdef
name|SVN_DEBUG
