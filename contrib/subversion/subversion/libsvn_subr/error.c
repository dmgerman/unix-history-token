...
condition|)
block|{
comment|/* I don't think we can change the behavior here, but the additional          error chain doesn't define the root cause. Perhaps we should rev          this function. */
if|if
condition|(
name|err
operator|->
name|child
comment|/*&& err->child->apr_err != SVN_ERR_COMPOSED_ERROR*/
condition|)
name|err
operator|=
name|err
...
begin_comment
comment|/* Defines svn__errno and svn__apr_errno */
end_comment
...
directive|ifdef
name|SVN_DEBUG
comment|/* Try errno.h symbols. */
comment|/* Linear search through a sorted array */
for|for
control|(
name|i
operator|=
...
directive|endif
comment|/* SVN_DEBUG */
comment|/* ### TODO: do we need APR_* error macros?  What about APR_TO_OS_ERROR()? */
return|return
name|NULL
return|;
block|}
