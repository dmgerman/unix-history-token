...
parameter_list|)
block|{
comment|/* apr_palloc will allocate multiples of 8.    * Thus, we would waste some of that memory if we stuck to the    * smaller size. Note that this is safe even if apr_palloc would    * use some other alignment or none at all. */
name|minimum_size
operator|=
...
argument_list|)
expr_stmt|;
comment|/* If we re-allocated MEMBUF->DATA, it cannot be NULL.    * Statically initialized membuffers (OLD_DATA) may be NULL, though. */
if|if
condition|(
name|old_data
operator|&&
...
name|FALSE
return|;
comment|/* now the strings must have identical lengths */
if|if
condition|(
operator|(
name|memcmp
...
name|size
expr_stmt|;
comment|/* If SIZE is 0, NULL is valid for BYTES. */
if|if
condition|(
name|size
condition|)
...
name|count
condition|)
comment|/* Allow BYTES to be NULL by avoiding passing it to memcpy. */
return|return;
name|total_len
operator|=
name|str
...
argument_list|)
expr_stmt|;
comment|/* update buffer length and always NUL-terminate it */
name|str
operator|->
name|len
operator|=
...
parameter_list|)
block|{
comment|/* For COUNT==0, we allow BYTES to be NULL. It's a no-op in that case. */
if|if
condition|(
name|count
operator|==
...
parameter_list|)
block|{
comment|/* For COUNT==0, we allow BYTES to be NULL.    * In that case, this is just a substring removal. */
if|if
condition|(
name|new_count
operator|==
...
condition|)
block|{
comment|/* pretty frequent and trivial case. Make it fast. */
operator|*
operator|(
name|dest
operator|++
...
name|buffer
decl_stmt|;
comment|/* write result as little-endian to buffer */
while|while
condition|(
name|value
operator|>
...
expr_stmt|;
block|}
comment|/* copy as big-endian to DEST */
while|while
condition|(
name|p
operator|>
...
index|]
decl_stmt|;
comment|/* convert digits to numerical values and count the number of places.    * Also, prevent buffer overflow. */
while|while
condition|(
name|i
operator|<
...
condition|)
block|{
comment|/* includes detection of NUL terminator */
if|if
condition|(
name|c
operator|<
...
expr_stmt|;
block|}
comment|/* fold digits into the result */
while|while
condition|(
name|i
operator|>
