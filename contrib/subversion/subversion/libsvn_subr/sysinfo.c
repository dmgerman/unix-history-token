...
begin_comment
comment|/* Split a stringbuf into a key/value pair.    Return the key, leaving the stripped value in the stringbuf. */
end_comment
...
begin_comment
comment|/* Read /etc/os-release, as documented here:  * http://www.freedesktop.org/software/systemd/man/os-release.html  */
end_comment
...
return|;
block|}
comment|/* Look for the PRETTY_NAME line. */
while|while
condition|(
name|TRUE
condition|)
...
name|release_name
decl_stmt|;
comment|/* The value may or may not be enclosed by double quotes.  We don't            * attempt to strip them. */
name|release_name
operator|=
...
break|break;
block|}
comment|/* The file did not contain a PRETTY_NAME line. */
name|svn_error_clear
argument_list|(
name|svn_stream_close
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* Try the systemd way (covers Arch). */
if|if
condition|(
operator|!
name|release_name
...
argument_list|)
expr_stmt|;
comment|/* Kill warnings with the Windows 8 and later platform SDK */
if|#
directive|if
name|_MSC_VER
operator|>
...
operator|>=
name|_0x06020000
comment|/* Windows 8 deprecated the API to retrieve the Windows version to avoid      backwards compatibility problems... It might return a constant version      in future Windows versions... But let's kill the warning.       We can implementation this using a different function later. */
pragma|#
directive|pragma
name|warning
name|(
...
directive|endif
comment|/* Prototype supports OSVERSIONINFO */
return|return
name|GetVersionExW
argument_list|(
operator|(
...
begin_comment
comment|/* Get system info, and try to tell the difference between the native    system type and the runtime environment of the current process.    Populate results in SYSINFO and LOCAL_SYSINFO (optional). */
end_comment
...
begin_comment
comment|/* implements svn_write_fn_t to copy the data into a CFMutableDataRef that's  * in the baton. */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* failed getting the CFMutableDataRef, shouldn't happen */
if|if
condition|(
operator|!
name|resource
...
name|NULL
return|;
comment|/* Try to open the plist files to get the data */
name|err
operator|=
name|svn_stream_open_readonly
argument_list|(
...
expr_stmt|;
block|}
comment|/* copy the data onto the CFMutableDataRef to allow us to provide it to    * the CoreFoundation functions that parse proprerty lists */
name|write_stream
operator|=
...
operator|>=
name|MAC_OS_X_VERSION_10_6
comment|/* This function is only available from Mac OS 10.6 onward. */
name|plist
operator|=
...
directive|else
comment|/* Mac OS 10.5 or earlier */
comment|/* This function obsolete and deprecated since Mac OS 10.10. */
name|plist
operator|=
