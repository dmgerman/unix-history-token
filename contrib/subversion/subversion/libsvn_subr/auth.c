...
argument_list|)
expr_stmt|;
comment|/* ab->slave_parameters = NULL; */
name|ab
operator|->
name|creds_cache
operator|=
...
begin_comment
comment|/* Magic pointer value to allow storing 'NULL' in an apr_hash_t */
end_comment
...
name|_
argument_list|(
literal|"No authentication providers registered"
argument_list|)
argument_list|)
return|;
...
block|}
block|}
comment|/* Windows has two providers without a store to allow easy access to      SSL servers. We enable these unconditionally.      (This behavior was moved here from svn_cmdline_create_auth_baton()) */
name|SVN_ERR
argument_list|(
name|svn_auth_get_platform_specific_provider
argument_list|(
...
name|svn_error_t
modifier|*
name|svn_auth__make_session_auth
parameter_list|(
name|svn_auth_baton_t
modifier|*
modifier|*
name|session_auth_baton
parameter_list|,
specifier|const
name|svn_auth_baton_t
modifier|*
...
name|FALSE
expr_stmt|;
comment|/* All the svn_auth_set_parameter() calls below this not only affect the      to be created ra session, but also all the ra sessions that are already      use this auth baton!       Please try to key things based on the realm string instead of this      construct.  */
if|if
condition|(
name|config
condition|)
