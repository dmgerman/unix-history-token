...
begin_comment
comment|/* (Note: *ARGV_P is an output parameter.) */
end_comment
...
argument_list|)
return|;
comment|/* Calc number of the fixed arguments. */
for|for
control|(
name|n
operator|=
...
begin_comment
comment|/* This structure is used as a baton for the pool cleanup function to    store tunnel parameters used by the close-tunnel callback. */
end_comment
...
begin_comment
comment|/* Pool cleanup function that invokes the close-tunnel callback. */
end_comment

begin_function
specifier|static
name|apr_status_t
name|close_tunnel_cleanup
parameter_list|(
name|void
modifier|*
name|baton
...
argument_list|)
expr_stmt|;
comment|/* We might have one stream to use for both request and response! */
if|if
condition|(
name|td
operator|->
...
begin_comment
comment|/* Open a session to URL, returning it in *SESS_P, allocating it in POOL.    URI is a parsed version of URL.  CALLBACKS and CALLBACKS_BATON    are provided by the caller of ra_svn_open. If TUNNEL_NAME is not NULL,    it is the name of the tunnel type parsed from the URL scheme.    If TUNNEL_ARGV is not NULL, it points to a program argument list to use    when invoking the tunnel agent. */
end_comment
...
begin_comment
comment|/* A simple whitelist to ensure the following are valid:  *   user@server  *   [::1]:22  *   server-name  *   server_name  *   127.0.0.1  * with an extra restriction that a leading '-' is invalid.  */
end_comment

begin_function
specifier|static
name|svn_boolean_t
name|is_valid_hostinfo
parameter_list|(
specifier|const
name|char
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* Use the default tunnel implementation if we got a tunnel name,      but either do not have tunnel handler callbacks installed, or      the handlers don't like the tunnel name. */
if|if
condition|(
name|tunnel
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* Nothing to sanitize here.  Any multi-segment path is simply          illegal in the hash returned by svn_ra_get_dir2. */
if|if
condition|(
name|strchr
argument_list|(
...
name|_
argument_list|(
literal|"Invalid directory entry name '%s'"
argument_list|)
argument_list|,
name|name
argument_list|)
...
name|NULL
expr_stmt|;
comment|/* Invoke RECEIVER           - Except if the server sends more than a>= 1 limit top level items           - Or when the callback reported a SVN_ERR_CEASE_INVOCATION             in an earlier invocation. */
if|if
condition|(
operator|!
operator|(
...
argument_list|)
expr_stmt|;
comment|/* If we don't support native iprop handling, use the implementation      in libsvn_ra */
if|if
condition|(
operator|!
name|iprop_capable
