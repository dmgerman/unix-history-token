...
begin_comment
comment|/* Return a deep copy of a (char * -> char *) hash. */
end_comment
...
comment|/* ### This seems to cater only for use cases where the revisions being          processed are not following on from revisions that will already          exist in the destination repository. If the revisions being          processed do follow on, then we might want to keep the mergeinfo          that refers to those older revisions. */
if|if
condition|(
name|rb
operator|->
...
name|SVN_NO_ERROR
return|;
comment|/* Try to detect if a delta-mode property occurs unexpectedly. HAS_PROPS      can be false here only if the parser didn't call remove_node_props(),      so this may indicate a bug rather than bad data. */
if|if
condition|(
operator|!
operator|(
...
begin_comment
comment|/* The parser calls this method if the node record has a non-delta  * property content section, before any calls to set_node_property().  * If the node record uses property deltas, this is not called.  */
end_comment
...
name|node_baton
decl_stmt|;
comment|/* In this case, not actually indicating that the node *has* props,      rather that it has a property content section. */
name|nb
operator|->
name|has_props
operator|=
...
literal|"Subversion repository dump filtering tool.\n"
