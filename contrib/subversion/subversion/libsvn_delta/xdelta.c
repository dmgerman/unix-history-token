...
begin_comment
comment|/* Size of the checksum presence FLAGS array in BLOCKS_T.  With standard    MATCH_BLOCKSIZE and SVN_DELTA_WINDOW_SIZE, 32k entries is about 20x    the number of checksums that actually occur, i.e. we expect a>95%    probability that non-matching checksums get already detected by checking    against the FLAGS array.    Must be a power of 2.  */
end_comment
...
begin_comment
comment|/* Return the offset in BLOCKS.FLAGS for the adler32 SUM. */
end_comment
...
parameter_list|)
block|{
comment|/* The upper half of SUM has a wider value range than the lower 16 bit.      Also, we want to a different folding than HASH_FUNC to minimize      correlation between different hash levels. */
return|return
operator|(
name|sum
operator|>>
...
expr_stmt|;
block|}
comment|/* No checksum entries in SLOTS, yet => reset all checksum flags. */
name|memset
argument_list|(
name|blocks
operator|->
...
name|MATCH_BLOCKSIZE
expr_stmt|;
comment|/* this is now known to be>= LO */
...
name|apos
decl_stmt|;
comment|/* Quickly skip positions whose respective ROLLING checksums          definitely do not match any SLOT in BLOCKS. */
while|while
condition|(
operator|!
operator|(
...
expr_stmt|;
block|}
comment|/* LO is still<= UPPER, i.e. the following lookup is legal:          Closely check whether we've got a match for the current location.          Due to the above pre-filter, chances are that we find one. */
name|matchlen
operator|=
name|find_match
argument_list|(
