...
parameter_list|)
block|{
comment|/* If the source and target overlap, repeat the overlapping pattern      in the target buffer. Always copy from the source buffer because      presumably it will be in the L1 cache after the first iteration      and doing this should avoid pipeline stalls due to write/read      dependencies. */
specifier|const
name|apr_size_t
name|overlap
init|=
...
expr_stmt|;
block|}
comment|/* Copy any remaining source pattern. */
if|if
condition|(
name|len
condition|)
...
literal|0
decl_stmt|;
comment|/* Nothing to do for empty buffers.    * This check allows for NULL TBUF in that case. */
if|if
condition|(
operator|*
name|tlen
...
comment|/* Just update the context here. */
if|if
condition|(
name|ab
operator|->
