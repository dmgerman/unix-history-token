...
operator|)
decl_stmt|;
comment|/* Ideally, we'd do this check before assigning fix_offset and          fix_limit; but then we couldn't make them const whilst still          adhering to C90 rules. Instead, we're going to assume that a          smart optimizing compiler will reorder this check before the          local variable initialization. */
if|if
condition|(
name|off
index|[
...
argument_list|)
expr_stmt|;
comment|/* Unconditionally issue the second subrange of the                  pattern. This is always correct, since the outer                  condition already verifies that there is an overlap                  in the target copy. */
block|{
specifier|const
name|apr_size_t
name|length
