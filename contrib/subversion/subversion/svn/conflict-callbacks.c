...
parameter_list|)
define|\
value|(ARRAY_LEN((aryx))> ARRAY_LEN((aryz))        \    ? ARRAY_LEN((aryx)) : ARRAY_LEN((aryz)))
end_define
...
name|choice
decl_stmt|;
comment|/* or ..._undefined if not a simple choice */
block|}
name|resolver_option_t
typedef|;
end_typedef
...
name|N_
argument_list|(
literal|"accept merged version of file  [working]"
argument_list|)
block|,
...
name|N_
argument_list|(
literal|"use merge tool to resolve conflict"
argument_list|)
block|,
name|svn_wc_conflict_choose_undefined
block|}
...
name|N_
argument_list|(
literal|"launch external merge tool to resolve "
...
name|N_
argument_list|(
literal|"internal merge tool"
argument_list|)
block|,
name|N_
argument_list|(
literal|"use built-in merge tool to "
literal|"resolve conflict"
argument_list|)
block|,
name|svn_wc_conflict_choose_undefined
block|}
...
begin_comment
comment|/* Resolver options for a binary file conflict. */
end_comment
...
name|N_
argument_list|(
literal|"accept the working copy version of file "
literal|" [working]"
argument_list|)
block|,
name|svn_wc_conflict_choose_merged
...
name|N_
argument_list|(
literal|"accept the incoming version of file "
literal|" [theirs-full]"
argument_list|)
block|,
name|svn_wc_conflict_choose_theirs_full
...
name|N_
argument_list|(
literal|"apply update to move destination (recommended)"
argument_list|)
block|,
name|N_
argument_list|(
...
specifier|const
name|resolver_option_t
name|tree_conflict_options_update_edit_deleted_dir
index|[]
init|=
block|{
block|{
...
name|N_
argument_list|(
literal|"prepare for updating moved-away children, if any (recommended)"
argument_list|)
block|,
name|N_
argument_list|(
literal|"allow updating moved-away children "
literal|"with 'svn resolve' [mine-conflict]"
argument_list|)
block|,
name|svn_wc_conflict_choose_mine_conflict
block|}
...
name|FALSE
decl_stmt|;
comment|/* Have they done something that might have affected the merged      file (so that we need to save a .edited copy by setting the      result->save_merge flag)? */
name|svn_boolean_t
name|performed_edit
init|=
name|FALSE
...
name|_
argument_list|(
literal|"Conflict discovered in binary file '%s'.\n"
argument_list|)
argument_list|,
name|local_relpath
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* ### TODO This whole feature availability check is grossly outdated.      DIFF_ALLOWED needs either to be redefined or to go away.    */
...
condition|)
block|{
comment|/* We need one more path for this feature. */
if|if
condition|(
name|desc
operator|->
...
literal|"r"
expr_stmt|;
comment|/* The 'mine-full' option selects the ".mine" file so only offer            * it if that file exists. It does not exist for binary files,            * for example (questionable historical behaviour since 1.0). */
if|if
condition|(
name|desc
operator|->
...
condition|)
block|{
comment|/* Re-check preconditions. */
if|if
condition|(
operator|!
name|diff_allowed
...
name|TRUE
decl_stmt|;
comment|/* Try the internal merge tool. */
name|svn_error_clear
argument_list|(
name|err
argument_list|)
...
block|{
comment|/* The external merge tool's exit code was either 0 or 1.                * The tool may leave the file conflicted by exiting with                * exit code 1, and we allow the user to mark the conflict                * resolved in this case. */
name|performed_edit
operator|=
name|TRUE
expr_stmt|;
...
break|break;
block|}
comment|/* Print a summary of conflicts before starting interactive resolution */
if|if
condition|(
operator|!
name|b
