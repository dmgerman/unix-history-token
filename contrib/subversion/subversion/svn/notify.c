...
begin_comment
comment|/* The body for notify() function with standard error handling semantic.  * Handling of errors implemented at caller side. */
end_comment
...
block|{
comment|/* Skip the path prefix in N, if supplied, or else the path prefix          in NB (which was set to the current working directory). */
if|if
condition|(
name|n
operator|->
...
name|_
argument_list|(
literal|"Performing cleanup on external item at '%s'.\n"
argument_list|)
argument_list|,
name|path_local
argument_list|)
