...
begin_comment
comment|/* Make a relative path containing '..' elements as needed.    TARGET_ABSPATH shall be the absolute version of TARGET_PATH.    TARGET_ABSPATH, TARGET_PATH and LOCAL_ABSPATH shall be canonical     If above conditions are met, a relative path that leads to PATH    from TARGET_PATH is returned, but there is no error checking involved.     The returned path is allocated from RESULT_POOL, all other    allocations are made in SCRATCH_POOL.     Note that it is not possible to just join the resulting path to    reconstruct the real path as the "../" paths are relative from    a different base than the normal relative paths!  */
end_comment
...
expr_stmt|;
block|}
comment|/* This returns a ../ style path relative from the status target */
return|return
name|svn_dirent_join
argument_list|(
name|parent_dir_els
