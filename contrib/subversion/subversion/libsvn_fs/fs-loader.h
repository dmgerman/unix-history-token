...
parameter_list|)
function_decl|;
comment|/* The open_fs/create/open_fs_for_recovery/upgrade_fs functions must      use the common_pool_lock to serialize the access to the common_pool      parameter for allocating fs-global objects such as an env cache. */
name|svn_error_t
modifier|*
function_decl|(
modifier|*
...
parameter_list|)
function_decl|;
comment|/* For svn_fs_info_fsfs_dup(). */
name|void
modifier|*
function_decl|(
modifier|*
...
parameter_list|)
function_decl|;
comment|/* info_fsap_dup is in the library vtable. */
name|svn_error_t
modifier|*
function_decl|(
modifier|*
...
comment|/* Set to "0" at the start of the txn, to "1" when svn:date changes. */
define|#
directive|define
name|SVN_FS__PROP_TXN_CLIENT_DATE
value|SVN_PROP_PREFIX "client-date"
struct|struct
name|svn_fs_t
block|{
...
name|username
decl_stmt|;
comment|/* A collection of lock-tokens supplied by the fs caller.      Hash maps (const char *) UUID --> path where path can be the      magic value (void *) 1 if no path was specified.      fs functions should really only be interested whether a UUID      exists as a hash key at all;  the value is irrelevant. */
name|apr_hash_t
modifier|*
name|lock_tokens
decl_stmt|;
