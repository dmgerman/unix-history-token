...
name|copyfrom_rev
decl_stmt|;
comment|/* Headers accumulated so far for this directory */
...
decl_stmt|;
comment|/* Flag to trigger dumping props. */
name|svn_boolean_t
name|dump_props
decl_stmt|;
block|}
...
decl_stmt|;
comment|/* The baton of the directory node whose block of      dump stream data has not been fully completed; NULL if there's no      such item. */
name|struct
name|dir_baton
modifier|*
name|pending_db
...
begin_comment
comment|/* Append to HEADERS the required headers, and set *CONTENT to the property  * content section, to represent the property delta of PROPS/DELETED_PROPS.  */
end_comment
...
begin_comment
comment|/* A special case of dump_node(), for a delete record.  *  * The only thing special about this version is it only writes one blank  * line, not two, after the headers. Why? Historical precedent for the  * case where a delete record is used as part of a (delete + add-with-history)  * in implementing a replacement.  */
end_comment
...
block|{
comment|/* More complex case: is_copy is true, and copyfrom_path/              copyfrom_rev are present: delete the original, and then re-add              it */
...
break|break;
block|}
comment|/* Return the headers so far. We don't necessarily have all the headers      yet -- there may be property-related and content length headers to      come, if this was not a 'delete' record. */
operator|*
name|headers_p
operator|=
name|headers
...
begin_comment
comment|/* Dump pending headers and properties for the directory EB->pending_db (if  * not null), to allow starting the dump of a child node */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* This deletion record is complete -- write an extra newline */
name|SVN_ERR
argument_list|(
name|svn_stream_puts
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Make sure we eventually output the props */
name|db
operator|->
name|dump_props
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Start dumping this node, by collecting some basic headers for it. */
name|SVN_ERR
argument_list|(
name|dump_node
argument_list|(
...
expr_stmt|;
block|}
comment|/* Dump the headers and props now */
name|SVN_ERR
argument_list|(
name|svn_repos__dump_node_record
argument_list|(
