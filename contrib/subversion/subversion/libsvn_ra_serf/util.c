...
literal|0
decl_stmt|;
comment|/* Implicitly approve any non-server certs. */
if|if
condition|(
...
block|}
block|}
comment|/* Match server certificate CN with the hostname of the server */
if|if
condition|(
operator|!
name|found_matching_hostname
...
argument_list|)
expr_stmt|;
comment|/* Tell the parser that no more content will be parsed. Ignore the          return status. We just don't care.  */
operator|(
name|void
operator|)
name|XML_Parse
...
condition|)
block|{
comment|/* We got a new response to an existing parser...          This tells us the connection has restarted and we should continue          where we stopped last time.        */
comment|/* Is this a second attempt?? */
if|if
condition|(
operator|!
name|ctx
...
literal|0
expr_stmt|;
comment|/* New request, nothing read */
block|}
if|if
condition|(
operator|!
...
condition|)
block|{
comment|/* Handle restarted requests correctly: Skip what we already read */
name|apr_size_t
name|skip
decl_stmt|;
if|if
...
argument_list|)
expr_stmt|;
comment|/* Ignore the return status. We just don't care.  */
operator|(
name|void
operator|)
name|XML_Parse
...
argument_list|)
expr_stmt|;
comment|/* Requeue another request for this handler.          ### how do we know if the handler can deal with this?!  */
name|svn_ra_serf__request_create
argument_list|(
name|handler
argument_list|)
