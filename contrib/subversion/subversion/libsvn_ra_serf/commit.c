...
decl_stmt|;
name|svn_boolean_t
name|non_recursive_if
decl_stmt|;
comment|/* Only create a non-recursive If header */
block|}
name|delete_context_t
typedef|;
end_typedef
...
condition|)
block|{
comment|/* Try again with non-standard body to overcome Apache Httpd          header limit */
name|delete_ctx
operator|->
name|non_recursive_if
operator|=
...
expr_stmt|;
block|}
comment|/* We have the same problem as with DELETE here: if there are too many      locks, the request fails. But in this case there is no way to retry      with a non-standard request. #### How to fix? */
name|SVN_ERR
argument_list|(
name|svn_ra_serf__context_run_one
argument_list|(
