...
begin_comment
comment|/* auxiliary structure representing the content of a directory array */
end_comment
...
name|dir_data_t
block|{
comment|/* number of entries in the directory    * (it's int because the directory is an APR array) */
name|int
name|count
decl_stmt|;
...
argument_list|)
decl_stmt|;
comment|/* copy the hash entries to an auxiliary struct of known layout */
name|dir_data
operator|.
name|count
operator|=
...
begin_comment
comment|/* Utility function to reconstruct a dir entries array from serialized data  * in BUFFER and DIR_DATA. Allocation will be made form POOL.  */
end_comment
...
name|serialized
decl_stmt|;
comment|/* initialize the serialization process and allocate a buffer large    * enough to do prevent re-allocations. */
name|svn_temp_serializer__context_t
modifier|*
name|context
init|=
...
begin_comment
comment|/* Auxiliary structure representing the content of a properties hash.    This structure is much easier to (de-)serialize than an apr_hash.  */
end_comment
...
name|i
decl_stmt|;
comment|/* create our auxiliary data structure */
name|properties
operator|.
name|count
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Replacement or removal? */
if|if
condition|(
name|replace_baton
operator|->
...
condition|)
block|{
comment|/* Replace ENTRY with / insert the NEW_ENTRY */
if|if
condition|(
name|entry
condition|)
...
name|context
decl_stmt|;
comment|/* after quite a number of operations, let's re-pack everything.    * This is to limit the number of wasted space as we cannot overwrite    * existing data but must always append. */
if|if
condition|(
name|dir_data
operator|->
...
name|i
decl_stmt|;
comment|/* initialize our auxiliary data structure and link it to the    * array elements */
name|changes
operator|.
name|count
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Use the changes buffer as the array's data buffer    * (DATA remains valid for at least as long as POOL). */
name|array
operator|->
name|elts
operator|=
