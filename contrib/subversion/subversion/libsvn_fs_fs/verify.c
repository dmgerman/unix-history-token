...
begin_comment
comment|/* verify.c --- verification of FSFS filesystems  *  * ====================================================================  *    Licensed to the Apache Software Foundation (ASF) under one  *    or more contributor license agreements.  See the NOTICE file  *    distributed with this work for additional information  *    regarding copyright ownership.  The ASF licenses this file  *    to you under the Apache License, Version 2.0 (the  *    "License"); you may not use this file except in compliance  *    with the License.  You may obtain a copy of the License at  *  *      http://www.apache.org/licenses/LICENSE-2.0  *  *    Unless required by applicable law or agreed to in writing,  *    software distributed under the License is distributed on an  *    "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY  *    KIND, either express or implied.  See the License for the  *    specific language governing permissions and limitations  *    under the License.  * ====================================================================  */
end_comment
...
decl_stmt|;
comment|/* cached hint for successive calls to svn_fs_fs__check_rep() */
name|void
modifier|*
name|hint
decl_stmt|;
...
begin_comment
comment|/* Verify the rep cache DB's consistency with our rev / pack data.  * The function signature is similar to svn_fs_fs__verify.  * The values of START and END have already been auto-selected and  * verified.  */
end_comment
...
begin_comment
comment|/* Verify that for all log-to-phys index entries for revisions START to  * START + COUNT-1 in FS there is a consistent entry in the phys-to-log  * index.  If given, invoke CANCEL_FUNC with CANCEL_BATON at regular  * intervals. Use POOL for allocations.  */
end_comment
...
begin_comment
comment|/* Verify that for all phys-to-log index entries for revisions START to  * START + COUNT-1 in FS there is a consistent entry in the log-to-phys  * index.  If given, invoke CANCEL_FUNC with CANCEL_BATON at regular  * intervals. Use POOL for allocations.  *  * Please note that we can only check on pack / rev file granularity and  * must only be called for a single rev / pack file.  */
end_comment
...
condition|)
block|{
comment|/* There is no L2P entry for unused rev file sections.                * And its P2L index data is hardly ever used.  But we                * should still check whether someone tempered with it. */
if|if
condition|(
name|entry
operator|->
...
name|_
argument_list|(
literal|"Empty P2L entry for PHYS %s "
literal|"refers to revision %ld outside "
literal|"the rev / pack file (%ld-%ld)"
argument_list|)
argument_list|,
name|apr_off_t_toa
argument_list|(
...
name|_
argument_list|(
literal|"l2p index entry PHYS %s"
literal|"does not match p2l index value "
literal|"LOG r%ld:i%ld for PHYS %s"
argument_list|)
argument_list|,
name|apr_off_t_toa
argument_list|(
...
begin_comment
comment|/* Verify that the next SIZE bytes read from FILE are NUL.  * SIZE must not exceed STREAM_THRESHOLD.  Use POOL for allocations.  */
end_comment
...
begin_comment
comment|/* Verify that the next SIZE bytes read from FILE are NUL.  * Use POOL for allocations.  */
end_comment
...
begin_comment
comment|/* Compare the ACTUAL checksum with the one expected by ENTRY.  * Return an error in case of mismatch.  Use the name of FILE  * in error message.  Allocate data in POOL.  */
end_comment
...
begin_comment
comment|/* Verify that the FNV checksum over the next ENTRY->SIZE bytes read  * from FILE will match ENTRY's expected checksum.  SIZE must not  * exceed STREAM_THRESHOLD.  Use POOL for allocations.  */
end_comment
...
begin_comment
comment|/* Verify that the FNV checksum over the next ENTRY->SIZE bytes read from  * FILE will match ENTRY's expected checksum.  Use POOL for allocations.  */
end_comment
...
begin_comment
comment|/* Verify that for all phys-to-log index entries for revisions START to  * START + COUNT-1 in FS match the actual pack / rev file contents.  * If given, invoke CANCEL_FUNC with CANCEL_BATON at regular intervals.  * Use POOL for allocations.  *  * Please note that we can only check on pack / rev file granularity and  * must only be called for a single rev / pack file.  */
end_comment
...
name|svn_error_t
modifier|*
name|verify_f7_metadata_consistency
parameter_list|(
name|svn_fs_t
modifier|*
name|fs
...
argument_list|)
decl_stmt|;
comment|/* Be careful to not leak ERR. */
if|if
condition|(
name|err2
condition|)
