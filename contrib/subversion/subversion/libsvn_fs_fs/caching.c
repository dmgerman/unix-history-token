...
begin_comment
comment|/* *CACHE_TXDELTAS, *CACHE_FULLTEXTS flags will be set according to    FS->CONFIG.  *CACHE_NAMESPACE receives the cache prefix to use.     Use FS->pool for allocating the memcache and CACHE_NAMESPACE, and POOL    for temporary allocations. */
end_comment
...
directive|ifdef
name|SVN_DEBUG_CACHE_DUMP_STATS
comment|/* schedule printing the global access statistics upon pool cleanup,    * i.e. when the repo instance gets closed / cleaned up.    */
if|if
condition|(
name|membuffer
condition|)
...
name|to_reset
decl_stmt|;
comment|/* pool that TXN_CACHE was allocated in */
name|apr_pool_t
modifier|*
name|txn_pool
decl_stmt|;
comment|/* pool that the FS containing the TO_RESET pointer was allocator */
name|apr_pool_t
modifier|*
name|fs_pool
decl_stmt|;
...
begin_comment
comment|/* APR pool cleanup handler that will reset the cache pointer given in    BATON_VOID when the TXN_POOL gets cleaned up. */
end_comment

begin_function
specifier|static
name|apr_status_t
name|remove_txn_cache_txn
parameter_list|(
name|void
modifier|*
name|baton_void
...
expr_stmt|;
block|}
comment|/* It's cleaned up now. Prevent double cleanup. */
...
begin_comment
comment|/* APR pool cleanup handler that will reset the cache pointer given in    BATON_VOID when the FS_POOL gets cleaned up. */
end_comment
...
begin_comment
comment|/* This function sets / registers the required callbacks for a given  * transaction-specific *CACHE object in FS, if CACHE is not NULL and  * a no-op otherwise. In particular, it will ensure that *CACHE gets  * reset to NULL upon POOL or FS->POOL destruction latest.  */
end_comment
...
name|pool
expr_stmt|;
comment|/* If any of these pools gets cleaned, we must reset the cache.        * We don't know which one will get cleaned up first, so register        * cleanup actions for both and during the cleanup action, unregister        * the respective other action. */
name|apr_pool_cleanup_register
argument_list|(
name|pool
argument_list|,
