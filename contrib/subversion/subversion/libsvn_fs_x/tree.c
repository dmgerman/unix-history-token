...
begin_comment
comment|/* The root structures.     Why do they contain different data?  Well, transactions are mutable    enough that it isn't safe to cache the DAG node for the root    directory or the hash of copyfrom data: somebody else might modify    them concurrently on disk!  (Why is the DAG node cache safer than    the root DAG node?  When cloning transaction DAG nodes in and out    of the cache, all of the possibly-mutable data from the    svn_fs_x__noderev_t inside the dag_node_t is dropped.)  Additionally,    revisions are immutable enough that their DAG node cache can be    kept in the FS object and shared among multiple revision root    objects. */
end_comment
...
begin_comment
comment|/* Find and return the DAG node cache for ROOT and the key that    should be used for PATH.     RESULT_POOL will only be used for allocating a new keys if necessary. */
end_comment
...
begin_comment
comment|/* Return NODE for PATH from ROOT's node cache, or NULL if the node    isn't cached; read it from the FS. *NODE remains valid until either    POOL or the FS gets cleared or destroyed (whichever comes first).  */
end_comment
...
begin_comment
comment|/* Set *NODE_P to a freshly opened dag node referring to the root    directory of ROOT, allocating from RESULT_POOL.  Use SCRATCH_POOL    for temporary allocations.  */
end_comment
...
begin_comment
comment|/* Set *NODE_P to a mutable root directory for ROOT, cloning if    necessary, allocating in RESULT_POOL.  ROOT must be a transaction root.    Use ERROR_PATH in error messages.  Use SCRATCH_POOL for temporaries.*/
end_comment
...
begin_comment
comment|/* Allocate a new parent_path_t node from RESULT_POOL, referring to NODE,    ENTRY, PARENT, and COPY_ID.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* The NODE in PARENT_PATH always lives in POOL, i.e. it will        * survive the cleanup of ITERPOOL and the DAG cache.*/
name|here
operator|=
name|parent_path
operator|->
...
literal|'\0'
expr_stmt|;
comment|/* Given the behavior of svn_fs__next_entry_name(), ENTRY may be an          empty string when the path either starts or ends with a slash.          In either case, we stay put: the current directory stays the          same, and we add nothing to the parent path.  We only need to          process non-empty path segments. */
if|if
condition|(
operator|*
name|entry
...
block|{
comment|/* Build a better error message than svn_fs_x__dag_open                      can provide, giving the root and full path name.  */
return|return
...
begin_comment
comment|/* Make the node referred to by PARENT_PATH mutable, if it isn't already,    allocating from RESULT_POOL.  ROOT must be the root from which    PARENT_PATH descends.  Clone any parent directories as needed.    Adjust the dag nodes in PARENT_PATH to refer to the clones.  Use    ERROR_PATH in error messages.  Use SCRATCH_POOL for temporaries. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* In FSX, even in-txn IDs are globally unique.    * So, we can simply compare them. */
if|if
condition|(
name|svn_fs_x__id_eq
argument_list|(
...
begin_comment
comment|/* Set *KIND_P to the type of node located at PATH under ROOT.    Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Set *KIND_P to the type of node present at PATH under ROOT.  If    PATH does not exist under ROOT, set *KIND_P to svn_node_none.  Use    SCRATCH_POOL for temporary allocation. */
end_comment
...
begin_comment
comment|/* Change, add, or delete a node's property value.  The affected node    is PATH under ROOT, the property value to modify is NAME, and VALUE    points to either a string value to set the new contents to, or NULL    if the property should be deleted.  Perform temporary allocations    in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Compare the directory representations at nodes LHS and RHS in FS and set  * *CHANGED to TRUE, if at least one entry has been added or removed them.  * Use SCRATCH_POOL for temporary allocations.  */
end_comment
...
condition|)
continue|continue;
comment|/* We get here rarely. */
name|svn_pool_clear
argument_list|(
name|iterpool
argument_list|)
expr_stmt|;
comment|/* Modified but not copied / replaced or anything? */
name|SVN_ERR
argument_list|(
...
continue|continue;
block|}
comment|/* This is a different entry. */
operator|*
name|changed
operator|=
name|TRUE
...
argument_list|)
return|;
comment|/* Fetch DAG nodes to efficiently access ID parts. */
name|SVN_ERR
argument_list|(
name|svn_fs_x__dag_get_node
argument_list|(
...
parameter_list|)
block|{
comment|/* Deltify is a no-op for fs_x. */
return|return
name|SVN_NO_ERROR
return|;
block|}
...
begin_comment
comment|/* Create a new directory named PATH in ROOT.  The new directory has    no entries, and no properties.  ROOT must be the root of a    transaction, not a revision.  Do any necessary temporary allocation    in SCRATCH_POOL.  */
end_comment
...
begin_comment
comment|/* Delete the node at PATH under ROOT.  ROOT must be a transaction    root.  Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Set *SAME_P to TRUE if FS1 and FS2 have the same UUID, else set to FALSE.    Use SCRATCH_POOL for temporary allocation only.    Note: this code is duplicated between libsvn_fs_x and libsvn_fs_base. */
end_comment
...
begin_comment
comment|/* Copy the node at FROM_PATH under FROM_ROOT to TO_PATH under    TO_ROOT.  If PRESERVE_HISTORY is set, then the copy is recorded in    the copies table.  Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Create a copy of FROM_PATH in FROM_ROOT named TO_PATH in TO_ROOT.    If FROM_PATH is a directory, copy it recursively.  Temporary    allocations are from SCRATCH_POOL.*/
end_comment
...
begin_comment
comment|/* Create a copy of FROM_PATH in FROM_ROOT named TO_PATH in TO_ROOT.    If FROM_PATH is a directory, copy it recursively.  No history is    preserved.  Temporary allocations are from SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Create the empty file PATH under ROOT.  Temporary allocations are    in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Set *LENGTH_P to the size of the file PATH under ROOT.  Temporary    allocations are in SCRATCH_POOL. */
end_comment
...
name|svn_error_t
modifier|*
name|x_try_process_file_contents
parameter_list|(
name|svn_boolean_t
modifier|*
name|success
...
begin_comment
comment|/* A wrapper around svn_fs_x__dag_finalize_edits, but for  * fulltext data, not text deltas.  Closes BATON->file_stream.  *  * Note: If you're confused about how this function relates to another  * of similar name, think of it this way:  *  * svn_fs_apply_textdelta() ==> ... ==> txn_body_txdelta_finalize_edits()  * svn_fs_apply_text()      ==> ... ==> txn_body_fulltext_finalize_edits()  */
end_comment
...
begin_comment
comment|/* Copy CHANGE into a FS API object allocated in RESULT_POOL and return    it in *RESULT_P.  Pass CONTEXT to the ID API object being created. */
end_comment
...
begin_comment
comment|/* Find the youngest copyroot for path PARENT_PATH or its parents in    filesystem FS, and store the copyroot in *REV_P and *PATH_P. */
end_comment
...
begin_comment
comment|/* Set *PATH and *REVISION to the path and revision for the HISTORY    object.  Allocate *PATH in RESULT_POOL. */
end_comment
...
begin_comment
comment|/* Return a new history object (marked as "interesting") for PATH and    REVISION, allocated in RESULT_POOL, and with its members set to the    values of the parameters provided.  Note that PATH and PATH_HINT get    normalized and duplicated in RESULT_POOL. */
end_comment
...
begin_comment
comment|/* Return the cache key as a combination of REV_ROOT->REV, the inheritance    flags INHERIT and ADJUST_INHERITED_MERGEINFO, and the PATH.  The result    will be allocated in RESULT_POOL.  */
end_comment
...
begin_comment
comment|/* Construct a new root object in FS, allocated from RESULT_POOL.  */
end_comment
...
begin_comment
comment|/* Construct a root object referring to the root of revision REV in FS.    Create the new root in RESULT_POOL.  */
end_comment
...
begin_comment
comment|/* Construct a root object referring to the root of the transaction    named TXN and based on revision BASE_REV in FS, with FLAGS to    describe transaction's behavior.  Create the new root in RESULT_POOL.  */
end_comment
...
comment|/* Callers should disable caches by setting SVN_FS_CONFIG_FSX_CACHE_NS;      see r1462436.       When this code is called in the library, we want to ensure we      use the on-disk data --- rather than some data that was read      in the possibly-distance past and cached since. */
name|SVN_ERR
argument_list|(
name|root_node
argument_list|(
