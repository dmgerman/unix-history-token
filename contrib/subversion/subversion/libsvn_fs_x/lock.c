...
begin_comment
comment|/* Callback type for walk_digest_files().  *  * LOCK come from a read_digest_file(digest_path) call.  */
end_comment
...
begin_comment
comment|/* A function that calls WALK_DIGESTS_FUNC/WALK_DIGESTS_BATON for    all lock digest files in and under PATH in FS.    HAVE_WRITE_LOCK should be true if the caller (directly or indirectly)    has the FS write lock. */
end_comment
...
begin_comment
comment|/* The main routine for lock enforcement, used throughout libsvn_fs_x. */
end_comment

begin_function
name|svn_error_t
modifier|*
name|svn_fs_x__allow_locked_operation
parameter_list|(
specifier|const
name|char
modifier|*
...
begin_comment
comment|/* The body of svn_fs_x__lock(), which see.     BATON is a 'lock_baton_t *' holding the effective arguments.    BATON->targets is an array of 'svn_sort__item_t' targets, sorted by    path, mapping canonical path to 'svn_fs_lock_target_t'.  Set    BATON->infos to an array of 'lock_info_t' holding the results.  For    the other arguments, see svn_fs_lock_many().     This implements the svn_fs_x__with_write_lock() 'body' callback    type, and assumes that the write lock is held.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Given the paths:         /foo/bar/f        /foo/bar/g        /zig/x       we loop through repeatedly.  The first pass sees '/' on all paths      and writes the '/' index.  The second pass sees '/foo' twice and      writes that index followed by '/zig' and that index. The third      pass sees '/foo/bar' twice and writes that index, and then writes      the lock for '/zig/x'.  The fourth pass writes the locks for      '/foo/bar/f' and '/foo/bar/g'.       Writing indices before locks is correct: if interrupted it leaves      indices without locks rather than locks without indices.  An      index without a lock is consistent in that it always shows up as      unlocked in svn_fs_x__allow_locked_operation.  A lock without an      index is inconsistent, svn_fs_x__allow_locked_operation will      show locked on the file but unlocked on the parent. */
while|while
condition|(
name|outstanding
condition|)
...
condition|)
block|{
comment|/* The component is a path to lock, this cannot                          match a previous path that need to be indexed. */
if|if
condition|(
name|paths
operator|->
...
block|{
comment|/* The component is a path to an index. */
name|apr_size_t
name|len
init|=
name|info
...
condition|)
block|{
comment|/* No match to the previous paths to index. */
name|SVN_ERR
argument_list|(
name|add_to_digest
argument_list|(
