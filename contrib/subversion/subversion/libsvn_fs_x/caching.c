...
begin_comment
comment|/* Take the ORIGINAL string and replace all occurrences of ":" without  * limiting the key space.  Allocate the result in RESULT_POOL.  */
end_comment
...
begin_comment
comment|/* *CACHE_TXDELTAS, *CACHE_FULLTEXTS and *CACHE_REVPROPS flags will be set    according to FS->CONFIG.  *CACHE_NAMESPACE receives the cache prefix    to use.     Allocate CACHE_NAMESPACE in RESULT_POOL. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* don't cache revprops by default.    * Revprop caching significantly speeds up operations like    * svn ls -v. However, it requires synchronization that may    * not be available or efficient in the current server setup.    * Option "2" is equivalent to "1".    */
if|if
condition|(
name|strcmp
argument_list|(
...
specifier|static
name|apr_status_t
name|dump_global_cache_statistics
parameter_list|(
name|void
modifier|*
name|baton_void
...
directive|ifdef
name|SVN_DEBUG_CACHE_DUMP_STATS
comment|/* schedule printing the access statistics upon pool cleanup,        * i.e. end of FSX session.        */
name|dump_cache_baton_t
modifier|*
name|baton
decl_stmt|;
...
begin_comment
comment|/* Sets *CACHE_P to cache instance based on provided options.  * Creates memcache if MEMCACHE is not NULL. Creates membuffer cache if  * MEMBUFFER is not NULL. Fallbacks to inprocess cache if MEMCACHE and  * MEMBUFFER are NULL and pages is non-zero.  Sets *CACHE_P to NULL  * otherwise.  Use the given PRIORITY class for the new cache.  If it  * is 0, then use the default priority class.  *  * Unless NO_HANDLER is true, register an error handler that reports errors  * as warnings to the FS warning callback.  *  * Cache is allocated in RESULT_POOL, temporaries in SCRATCH_POOL.  * */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* General rules for assigning cache priorities:    *    * - Data that can be reconstructed from other elements has low prio    *   (e.g. fulltexts, directories etc.)    * - Index data required to find any of the other data has high prio    *   (e.g. noderevs, L2P and P2L index pages)    * - everthing else should use default prio    */
ifdef|#
directive|ifdef
name|SVN_DEBUG_CACHE_DUMP_STATS
comment|/* schedule printing the global access statistics upon pool cleanup,    * i.e. end of FSX session.    */
if|if
condition|(
name|membuffer
condition|)
...
literal|32
argument_list|,
comment|/* ~200 byte / entry; 1k entries total */
...
argument_list|)
expr_stmt|;
comment|/* initialize representation header cache, if caching has been enabled */
name|SVN_ERR
argument_list|(
name|create_cache
argument_list|(
...
literal|1000
argument_list|,
comment|/* ~8 bytes / entry; 1k entries total */
...
literal|8
argument_list|,
comment|/* 1k / entry; 8 entries total, rarely used */
...
expr_stmt|;
block|}
comment|/* if enabled, cache revprops */
if|if
condition|(
name|cache_revprops
condition|)
...
literal|4
argument_list|,
comment|/* Important, largish objects */
...
literal|16
argument_list|,
comment|/* entry size varies but we must cover                                   a reasonable number of revisions (1k) */
...
literal|1
argument_list|,
comment|/* Large entries. Rarely used. */
...
literal|16
argument_list|,
comment|/* Variably sized entries. Rarely used. */
