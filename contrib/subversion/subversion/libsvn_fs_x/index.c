...
begin_comment
comment|/* index.c indexing support for FSX support  *  * ====================================================================  *    Licensed to the Apache Software Foundation (ASF) under one  *    or more contributor license agreements.  See the NOTICE file  *    distributed with this work for additional information  *    regarding copyright ownership.  The ASF licenses this file  *    to you under the Apache License, Version 2.0 (the  *    "License"); you may not use this file except in compliance  *    with the License.  You may obtain a copy of the License at  *  *      http://www.apache.org/licenses/LICENSE-2.0  *  *    Unless required by applicable law or agreed to in writing,  *    software distributed under the License is distributed on an  *    "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY  *    KIND, either express or implied.  See the License for the  *    specific language governing permissions and limitations  *    under the License.  * ====================================================================  */
end_comment
...
name|offsets
decl_stmt|;
comment|/* In case that the item is stored inside a container, this is the    * identifying index of the item within that container.  0 for the    * container itself or for items that aren't containers. */
name|apr_uint32_t
modifier|*
name|sub_items
decl_stmt|;
...
name|item_index
decl_stmt|;
comment|/* index within the container starting @ offset.  0 for "new revision"    * entries and for items with no outer container. */
name|apr_uint32_t
name|sub_item
decl_stmt|;
block|}
...
begin_struct
struct|struct
name|svn_fs_x__packed_number_stream_t
block|{
...
name|_
argument_list|(
literal|"Can't read index file '%s' at offset 0x%"
argument_list|)
argument_list|)
return|;
...
name|_
argument_list|(
literal|"Unexpected end of index file %s at offset 0x%"
argument_list|)
argument_list|)
return|;
...
name|item_index
expr_stmt|;
comment|/* no limits on the container sub-item index */
name|entry
operator|.
name|sub_item
operator|=
...
begin_comment
comment|/* Run-length-encode the uint64 numbers in ARRAY starting at index START  * up to but not including END.  All numbers must be> 0.  * Return the number of remaining entries in ARRAY after START.  */
end_comment
...
begin_comment
comment|/* Utility data structure describing an log-2-phys page entry.  * This is only used as a transient representation during index creation.  */
end_comment

begin_typedef
typedef|typedef
struct|struct
name|l2p_page_entry_t
block|{
name|apr_uint64_t
name|offset
decl_stmt|;
...
begin_comment
comment|/* qsort-compatible compare function taking two l2p_page_entry_t and  * ordering them by offset.  */
end_comment

begin_function
specifier|static
name|int
name|compare_l2p_entries_by_offset
parameter_list|(
specifier|const
name|l2p_page_entry_t
modifier|*
...
argument_list|)
decl_stmt|;
comment|/* SORTED: relevant items from ENTRIES, sorted by offset */
name|l2p_page_entry_t
modifier|*
name|sorted
init|=
...
argument_list|)
expr_stmt|;
comment|/* identify container offsets and create container list */
for|for
control|(
name|i
operator|=
...
condition|)
continue|continue;
comment|/* offset already covered? */
if|if
condition|(
name|i
operator|>
...
condition|)
continue|continue;
comment|/* is this a container item        * (appears more than once or accesses to sub-items other than 0)? */
if|if
condition|(
operator|(
name|i
...
block|}
block|}
comment|/* write container list to BUFFER */
name|SVN_ERR
argument_list|(
...
block|}
block|}
comment|/* we are now done with the source file */
name|SVN_ERR
argument_list|(
...
begin_comment
comment|/* Return the base revision used to identify the p2l or lp2 index covering  * REVISION in FS.  */
end_comment
...
begin_comment
comment|/* Read COUNT run-length-encoded (see rle_array) uint64 from STREAM and  * return them in VALUES.  */
end_comment
...
begin_comment
comment|/* Get the page info requested in *BATON from FS and set the output fields  * in *BATON.  * To maximize efficiency, use or return the data stream in *STREAM.  * Use SCRATCH_POOL for temporary allocations.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* container offsets array */
name|SVN_ERR
argument_list|(
name|packed_stream_get
argument_list|(
...
literal|1
expr_stmt|;
comment|/* '-1' is represented as '0' in the index file */
block|}
...
begin_comment
comment|/* Request data structure for l2p_page_access_func.  */
end_comment

begin_typedef
typedef|typedef
struct|struct
name|l2p_page_baton_t
block|{
comment|/* in data */
...
name|offset
decl_stmt|;
comment|/* 0 -> container / item itself; sub-item in container otherwise */
name|apr_uint32_t
name|sub_item
decl_stmt|;
block|}
...
begin_comment
comment|/* Return the rev / pack file offset of the item at BATON->PAGE_OFFSET in  * OFFSETS of PAGE and write it to *OFFSET.  * Allocate temporaries in SCRATCH_POOL.  */
end_comment
...
name|_
argument_list|(
literal|"Item index %s too large in"
literal|" revision %ld"
argument_list|)
argument_list|,
name|apr_psprintf
...
begin_comment
comment|/* Implement svn_cache__partial_getter_func_t: copy the data requested in  * l2p_page_baton_t *BATON from l2p_page_t *DATA into apr_off_t *OUT.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Add trailer: rev / pack file offset of the next item */
name|SVN_ERR
argument_list|(
name|write_uint64_to_proto_index
argument_list|(
...
name|svn_error_t
modifier|*
name|read_p2l_sub_items_from_proto_index
parameter_list|(
name|apr_file_t
modifier|*
name|proto_index
...
argument_list|)
expr_stmt|;
comment|/* Do the inverse REVSION number conversion (see        * svn_fs_x__p2l_proto_index_add_entry), if we actually read a        * complete record.       */
if|if
condition|(
operator|!
name|eof
...
block|{
comment|/* The last 64 bits contain the value we are looking for. */
name|offset
operator|-=
sizeof|sizeof
argument_list|(
...
expr_stmt|;
block|}
comment|/* Read entry trailer. However, we won't need its content. */
if|if
condition|(
operator|!
name|eof
...
expr_stmt|;
block|}
comment|/* write simple item / container entry */
name|SVN_ERR
argument_list|(
name|svn_spillbuf__write
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* container contents (only one for non-container items) */
for|for
control|(
name|sub_item
operator|=
...
name|_
argument_list|(
literal|"Unused regions must be empty and have checksum 0"
argument_list|)
argument_list|)
return|;
if|if
...
name|_
argument_list|(
literal|"Only containers may have more than one sub-item"
argument_list|)
argument_list|)
return|;
for|for
...
begin_comment
comment|/* qsort-compatible compare function that compares the OFFSET of the  * svn_fs_x__p2l_entry_t in *LHS with the apr_off_t in *RHS. */
end_comment
...
begin_comment
comment|/* From the PAGE_ENTRIES array of svn_fs_x__p2l_entry_t, ordered  * by their OFFSET member, copy all elements overlapping the range  * [BLOCK_START, BLOCK_END) to ENTRIES.  If RESOLVE_PTR is set, the ITEMS  * sub-array in each entry needs to be de-serialized. */
end_comment
...
name|entry
expr_stmt|;
comment|/* Copy the items of that entries. */
if|if
condition|(
name|entry
operator|->
...
begin_comment
comment|/* Body of svn_fs_x__p2l_index_lookup.  However, do a single index page  * lookup and append the result to the ENTRIES array provided by the caller.  * Use successive calls to cover larger ranges.  */
end_comment
...
begin_comment
comment|/* compare_fn_t comparing a svn_fs_x__p2l_entry_t at LHS with an offset  * RHS.  */
end_comment
...
begin_comment
comment|/* Baton structure for p2l_item_lookup_func.  It describes which sub_item  * info shall be returned.  */
end_comment

begin_typedef
typedef|typedef
struct|struct
name|p2l_item_lookup_baton_t
block|{
comment|/* file offset to find the P2L index entry for */
name|apr_off_t
name|offset
decl_stmt|;
comment|/* return the sub-item at this position within that entry */
name|apr_uint32_t
name|sub_item
decl_stmt|;
block|}
...
begin_comment
comment|/* Implements svn_cache__partial_getter_func_t for P2L index pages, copying  * the svn_fs_x__id_t for the item described 2l_item_lookup_baton_t  * *BATON.  *OUT will be NULL if there is no matching index entry or the  * sub-item is out of range.  */
end_comment
...
begin_comment
comment|/* Decorator for svn_fs_x__p2l_entry_t that associates it with a sorted  * variant of its ITEMS array.  */
end_comment

begin_typedef
typedef|typedef
struct|struct
name|sub_item_ordered_t
block|{
comment|/* ENTRY that got wrapped */
name|svn_fs_x__p2l_entry_t
modifier|*
name|entry
decl_stmt|;
comment|/* Array of pointers into ENTRY->ITEMS, sorted by their revision member    * _descending_ order.  May be NULL if ENTRY->ITEM_COUNT< 2. */
name|svn_fs_x__id_t
modifier|*
modifier|*
name|order
...
begin_comment
comment|/* implements compare_fn_t. Place LHS before RHS, if the latter is younger.  * Used to sort sub_item_ordered_t::order  */
end_comment

begin_function
specifier|static
name|int
name|compare_sub_items
parameter_list|(
specifier|const
name|svn_fs_x__id_t
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* wrap P2L entries such that we have access to the sub-items in revision      order.  The ENTRY_COUNT member will point to the next item to read+1. */
name|sub_item_orders
operator|=
name|apr_array_make
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* skip unused regions (e.g. padding) */
if|if
condition|(
name|entry
operator|->
...
block|}
block|}
comment|/* we need to write the index in ascending revision order */
name|queue
operator|=
...
condition|)
block|{
comment|/* if there is only one item, we skip the overhead of having an              extra array for the item order */
name|sub_item
operator|=
name|ordered
operator|->
...
argument_list|)
expr_stmt|;
comment|/* make ITEM_COUNT point the next sub-item to use+1 */
operator|--
name|ordered
operator|->
name|entry
...
expr_stmt|;
block|}
comment|/* process remaining sub-items (if any) of that container later */
if|if
condition|(
name|ordered
operator|->
...
argument_list|)
expr_stmt|;
comment|/* resolve sub-struct pointers*/
name|entries
operator|=
operator|(
name|svn_fs_x__p2l_entry_t
