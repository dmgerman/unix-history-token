...
begin_comment
comment|/* If PATH needs to be escaped, return an escaped version of it, allocated  * from RESULT_POOL. Otherwise, return PATH directly. */
end_comment
...
begin_comment
comment|/* If PATH has been escaped, return the un-escaped version of it, allocated  * from RESULT_POOL. Otherwise, return PATH directly. */
end_comment
...
begin_comment
comment|/* Find entry HEADER_NAME in HEADERS and parse its value into *ID. */
end_comment
...
name|_
argument_list|(
literal|"Missing %s field in node-rev"
argument_list|)
argument_list|,
name|header_name
argument_list|)
...
return|;
block|}
comment|/* Get the mergeinfo-mod flag. */
name|str
operator|=
name|svn_cstring_tokenize
argument_list|(
...
begin_comment
comment|/* Write a single change entry, path PATH, change CHANGE, to STREAM.     All temporary allocations are in SCRATCH_POOL. */
end_comment
...
name|i
decl_stmt|;
comment|/* For the sake of the repository administrator sort the changes so      that the final file is deterministic and repeatable, however the      rest of the FSX code doesn't require any particular order here.       Also, this sorting is only effective in writing all entries with      a single call as write_final_changed_path_info() does.  For the      list being written incrementally during transaction, we actually      *must not* change the order of entries from different calls.    */
name|sorted_changed_paths
operator|=
name|svn_sort__hash
argument_list|(
