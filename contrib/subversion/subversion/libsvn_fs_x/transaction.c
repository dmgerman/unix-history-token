...
begin_comment
comment|/* FSX-specific data being attached to svn_fs_txn_t.  */
end_comment
...
begin_comment
comment|/* Get a lock on empty file LOCK_FILENAME, creating it in RESULT_POOL. */
end_comment
...
begin_comment
comment|/* Wraps with_some_lock_file, protecting it with BATON->MUTEX.     SCRATCH_POOL is unused here and only provided for signature compatibility    with WITH_LOCK_BATON_T.BODY. */
end_comment
...
begin_comment
comment|/* Return the  baton for the innermost lock of a (potential) lock chain.    The baton shall take out LOCK_ID from FS and execute BODY with BATON    while the lock is being held.  Allocate the result in a sub-pool of    RESULT_POOL.  */
end_comment
...
begin_comment
comment|/* Unlock the prototype revision file for transaction TXN_ID in filesystem    FS using cookie LOCKCOOKIE.  The original prototype revision file must    have been closed _before_ calling this function.     Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
typedef|typedef
struct|struct
name|get_writable_proto_rev_baton_t
block|{
name|void
modifier|*
modifier|*
...
begin_comment
comment|/* Purge the shared data for transaction TXN_ID in filesystem FS.    Perform all temporary allocations in SCRATCH_POOL. */
end_comment
...
name|FALSE
return|;
comment|/* ... with no prop change in that txn?      (Once we set a property, the prop rep will never become NULL again.) */
if|if
condition|(
name|noderev
operator|->
...
name|FALSE
return|;
comment|/* ... and no sub-tree change?      (Once we set a text, the data rep will never become NULL again.) */
if|if
condition|(
name|noderev
operator|->
...
name|FALSE
return|;
comment|/* Root node of a txn with no changes. */
return|return
name|TRUE
return|;
block|}
...
begin_comment
comment|/* Write the directory given as array of dirent structs in ENTRIES to STREAM.    Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Merge the internal-use-only CHANGE into a hash of public-FS    svn_fs_x__change_t CHANGED_PATHS, collapsing multiple changes into a    single summarical (is that real word?) change per path.  DELETIONS is    also a path->svn_fs_x__change_t hash and contains all the deletions    that got turned into a replacement. */
end_comment
...
comment|/* Sanity check:  only allow unused node revision IDs in the          `reset' case. */
if|if
condition|(
operator|(
operator|!
...
begin_comment
comment|/* An implementation of svn_fs_x__change_receiver_t.    Examine all the changed path entries in CHANGES and store them in    *CHANGED_PATHS.  Folding is done to remove redundant or unnecessary    data. Use SCRATCH_POOL for temporary allocations. */
end_comment
...
begin_comment
comment|/* Copy a revision node-rev SRC into the current transaction TXN_ID in    the filesystem FS.  This is only used to create the root of a transaction.    Temporary allocations are from SCRATCH_POOL.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* This must be a root node. */
name|SVN_ERR_ASSERT
argument_list|(
name|noderev
operator|->
...
typedef|typedef
struct|struct
name|get_and_increment_txn_key_baton_t
block|{
name|svn_fs_t
modifier|*
name|fs
...
begin_comment
comment|/* Create a new transaction in filesystem FS, based on revision REV,    and store it in *TXN_P, allocated in RESULT_POOL.  Allocate necessary    temporaries from SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Save the property list PROPS as the revprops for transaction TXN_ID    in FS.  Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* If it is supported by the format of file system FS, store the (ITEM_INDEX,  * OFFSET) pair in the log-to-phys proto index file of transaction TXN_ID.  * Use SCRATCH_POOL for temporary allocations.  */
end_comment
...
begin_comment
comment|/* If it is supported by the format of file system FS, store ENTRY in the  * phys-to-log proto index file of transaction TXN_ID.  * Use SCRATCH_POOL for temporary allocations.  */
end_comment
...
begin_comment
comment|/* Allocate an item index in the transaction TXN_ID of file system FS and  * return it in *ITEM_INDEX.  Use SCRATCH_POOL for temporary allocations.  */
end_comment
...
begin_comment
comment|/* Write out the currently available next node_id NODE_ID and copy_id    COPY_ID for transaction TXN_ID in filesystem FS.  The next node-id is    used both for creating new unique nodes for the given transaction, as    well as uniquifying representations.  Perform temporary allocations in    SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Get a new and unique to this transaction node-id for transaction    TXN_ID in filesystem FS.  Store the new node-id in *NODE_ID_P.    Node-ids are guaranteed to be unique to this transction, but may    not necessarily be sequential.    Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
name|copy_id
expr_stmt|;
comment|/* Noderev-id = Change set and item number within this change set. */
name|noderev
operator|->
name|noderev_id
operator|.
...
argument_list|)
expr_stmt|;
comment|/* Provide the parent with a data rep if it had none before          (directories so far empty). */
if|if
condition|(
operator|!
name|rep
...
expr_stmt|;
block|}
comment|/* update directory cache */
block|{
...
decl_stmt|;
comment|/* Receives the low-level checksum when closing REP_STREAM. */
name|apr_uint32_t
name|fnv1a_checksum
decl_stmt|;
comment|/* Local pool, available for allocations that must remain valid as long      as this baton is used but may be cleaned up immediately afterwards. */
name|apr_pool_t
modifier|*
name|local_pool
decl_stmt|;
...
begin_comment
comment|/* Handler for the write method of the representation writable stream.    BATON is a rep_write_baton_t, DATA is the data to write, and *LEN is    the length of this data. */
end_comment
...
begin_comment
comment|/* Set *SPANNED to the number of shards touched when walking WALK steps on  * NODEREV's predecessor chain in FS.  * Use SCRATCH_POOL for temporary allocations.  */
end_comment
...
begin_comment
comment|/* Get a rep_write_baton_t, allocated from RESULT_POOL, and store it in    WB_P for the representation indicated by NODEREV in filesystem FS.    Only appropriate for file contents, not for props or directory contents.  */
end_comment
...
begin_comment
comment|/* Copy the hash sum calculation results from MD5_CTX, SHA1_CTX into REP.  * Use SCRATCH_POOL for temporary allocations.  */
end_comment
...
begin_comment
comment|/* Close handler for the representation write stream.  BATON is a    rep_write_baton_t.  Writes out a new node-rev that correctly    references the representation we just finished writing. */
end_comment
...
begin_comment
comment|/* Store a writable stream in *CONTENTS_P, allocated in RESULT_POOL, that    will receive all data written and store it as the file data representation    referenced by NODEREV in filesystem FS.  Only appropriate for file data,    not props or directory contents. */
end_comment
...
begin_comment
comment|/* The handler for the write_container_rep stream.  BATON is a    write_container_baton_t, DATA has the data to write and *LEN is the number    of bytes to write. */
end_comment
...
begin_comment
comment|/* Implement collection_writer_t writing the svn_fs_x__dirent_t* array given    as BATON. */
end_comment
...
begin_comment
comment|/* Sanity check ROOT_NODEREV, a candidate for being the root node-revision    of (not yet committed) revision REV in FS.  Use SCRATCH_POOL for temporary    allocations.     If you change this function, consider updating svn_fs_x__verify() too.  */
end_comment
...
begin_comment
comment|/* Given the potentially txn-local id PART, update that to a permanent ID  * based on the REVISION.  */
end_comment
...
begin_comment
comment|/* Write the changed path info CHANGED_PATHS from transaction TXN_ID to the    permanent rev-file FILE representing NEW_REV in filesystem FS.  *OFFSET_P    is set the to offset in the file of the beginning of this information.    NEW_REV is the revision currently being committed.    Perform temporary allocations in SCRATCH_POOL. */
end_comment
...
begin_comment
comment|/* Open a new svn_fs_t handle to FS, set that handle's concept of "current    youngest revision" to NEW_REV, and call svn_fs_x__verify_root() on    NEW_REV's revision root.     Intended to be called as the very last step in a commit before 'current'    is bumped.  This implies that we are holding the write lock. */
end_comment
...
begin_comment
comment|/* The work-horse for svn_fs_x__commit, called with the FS write lock.    This implements the svn_fs_x__with_write_lock() 'body' callback    type.  BATON is a 'commit_baton_t *'. */
end_comment
...
begin_comment
comment|/* Add the representations in REPS_TO_CACHE (an array of  * svn_fs_x__representation_t *) to the rep-cache database of FS. */
end_comment
