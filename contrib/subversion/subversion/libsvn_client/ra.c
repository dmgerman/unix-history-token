...
name|ctx
decl_stmt|;
comment|/* Last progress reported by progress callback. */
name|apr_off_t
name|last_progress
decl_stmt|;
block|}
...
begin_comment
comment|/* Implements svn_ra_progress_notify_func_t. Accumulates progress information  * for different RA sessions and reports total progress to caller. */
end_comment
...
condition|)
block|{
comment|/* All RA implementations currently provide -1 for total. So it doesn't          make sense to develop some complex logic to combine total across all          RA sessions. */
name|public_ctx
operator|->
name|progress_func
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Handle another common case: The repository root can't move */
if|if
condition|(
operator|!
name|strcmp
