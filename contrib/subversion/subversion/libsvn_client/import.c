...
argument_list|)
expr_stmt|;
comment|/* Get a root dir baton.  We pass the revnum we used for testing our      assumptions and obtaining inherited properties. */
name|SVN_ERR
argument_list|(
name|editor
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Obtain properties before opening the commit editor, as at that point we are      not allowed to use the existing ra-session */
if|if
condition|(
operator|!
name|no_ignore
comment|/*|| ! no_autoprops*/
condition|)
block|{
name|SVN_ERR
argument_list|(
...
condition|)
block|{
comment|/* ### This should use inherited_props and url_props to avoid creating              another ra session to obtain the same values, but using a possibly              different HEAD revision */
name|SVN_ERR
argument_list|(
...
block|}
block|}
comment|/* If an error occurred during the commit, properly abort the edit.  */
name|err
operator|=
name|svn_error_trace
argument_list|(
