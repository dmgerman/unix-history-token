...
condition|)
block|{
comment|/* Ok, now we know that we perform an out-of-date check                      on the copyfrom location. Remember this for a fixup                      round right before committing. */
if|if
condition|(
operator|!
name|move_youngest
...
argument_list|)
decl_stmt|;
comment|/* We delete the original side with its original revision and will              receive an out-of-date error if that node changed since that              revision.               The copy is of that same revision and we know that this revision              didn't change between this revision and youngest. So we can just              as well commit a copy from youngest.              Note that it is still possible to see gaps between the delete and             copy revisions as the repository might handle multiple commits             at the same time (or when an out of date proxy is involved), but             in general it should decrease the number of gaps. */
if|if
condition|(
name|item
operator|->
