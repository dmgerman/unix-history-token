...
begin_comment
comment|/* Quote a string if it would be handled as multiple or different tokens    during externals parsing */
end_comment
...
begin_comment
comment|/* In *NEW_EXTERNALS_DESCRIPTION, return a new external description for  * use as a line in an svn:externals property, based on the external item  * ITEM and the additional parser information in INFO. Pin the external  * to EXTERNAL_PEGREV. Use POOL for all allocations. */
end_comment
...
name|svn_error_t
modifier|*
name|make_external_description
parameter_list|(
specifier|const
name|char
modifier|*
modifier|*
name|new_external_description
parameter_list|,
specifier|const
name|char
modifier|*
...
condition|)
block|{
comment|/* If info->rev_str is NULL, this yields an empty string. */
name|rev_str
operator|=
name|apr_pstrcat
argument_list|(
...
block|{
comment|/* ### can't handle svn_opt_revision_date without info->rev_str */
name|SVN_ERR_ASSERT
argument_list|(
name|external_pegrev
operator|.
...
name|_
argument_list|(
literal|"%s property defined at '%s' is using an unsupported "
literal|"syntax"
argument_list|)
argument_list|,
...
begin_comment
comment|/* Pin all externals listed in EXTERNALS_PROP_VAL to their  * last-changed revision. Set *PINNED_EXTERNALS to a new property  * value allocated in RESULT_POOL, or to NULL if none of the externals  * in EXTERNALS_PROP_VAL were changed. LOCAL_ABSPATH_OR_URL is the  * path or URL defining the svn:externals property. Use SCRATCH_POOL  * for temporary allocations.  */
end_comment
...
name|svn_error_t
modifier|*
name|pin_externals_prop
parameter_list|(
name|svn_string_t
modifier|*
modifier|*
name|pinned_externals
parameter_list|,
name|svn_string_t
modifier|*
...
condition|)
block|{
comment|/* No pinning at all for this path. */
operator|*
name|pinned_externals
operator|=
name|NULL
...
block|}
block|}
comment|/* If this item is not in our list of external items to pin then            * simply keep the external at its original value. */
if|if
condition|(
operator|!
name|item_to_pin
...
condition|)
block|{
comment|/* Already pinned ... copy the peg date. */
name|external_pegrev
operator|.
name|kind
operator|=
...
condition|)
block|{
comment|/* Already pinned ... copy the peg revision number. */
name|external_pegrev
operator|.
name|kind
operator|=
...
argument_list|)
expr_stmt|;
comment|/* We're actually going to change the peg revision. */
operator|++
name|pinned_items
expr_stmt|;
if|if
...
name|_
argument_list|(
literal|"Cannot pin external '%s' defined "
literal|"in %s at '%s' because it is not "
literal|"checked out in the working copy "
literal|"at '%s'"
argument_list|)
argument_list|,
name|item
...
name|max_rev
decl_stmt|;
comment|/* Perform some sanity checks on the checked-out external. */
name|SVN_ERR
argument_list|(
...
argument_list|(
literal|"Cannot pin external '%s' defined "
literal|"in %s at '%s' because '%s' has "
literal|"switched subtrees (switches "
literal|"cannot be represented in %s)"
argument_list|)
argument_list|,
name|item
operator|->
...
literal|"Cannot pin external '%s' defined "
literal|"in %s at '%s' because '%s' has "
literal|"local modifications (local "
literal|"modifications cannot be "
literal|"represented in %s)"
argument_list|)
argument_list|,
name|item
operator|->
...
argument_list|(
literal|"Cannot pin external '%s' defined "
literal|"in %s at '%s' because '%s' is a "
literal|"mixed-revision working copy "
literal|"(mixed-revisions cannot be "
literal|"represented in %s)"
argument_list|)
argument_list|,
name|item
...
begin_comment
comment|/* Return, in *PINNED_EXTERNALS, a new hash mapping URLs or local abspaths  * to svn:externals property values (as const char *), where some or all  * external references have been pinned.  * If EXTERNALS_TO_PIN is NULL, pin all externals, else pin the externals  * mentioned in EXTERNALS_TO_PIN.  * The pinning operation takes place as part of the copy operation for  * the source/destination pair PAIR. Use RA_SESSION and REPOS_ROOT_URL  * to contact the repository containing the externals definition, if neccesary.  * Use CX to fopen additional RA sessions to external repositories, if  * neccessary. Allocate *NEW_EXTERNALS in RESULT_POOL.  * Use SCRATCH_POOL for temporary allocations. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* ### gather_definitions returns propvals as const char * */
for|for
control|(
name|hi
operator|=
...
name|_
argument_list|(
literal|"Directory '%s' is not under "
...
name|externals
decl_stmt|;
comment|/* new externals definitions for the target */
name|svn_boolean_t
name|only_pin_externals
decl_stmt|;
block|}
name|path_driver_info_t
typedef|;
...
argument_list|)
expr_stmt|;
comment|/* Check to see if we need to add the path as a parent directory. */
if|if
condition|(
name|path_info
operator|->
...
begin_comment
comment|/* Queue property changes for pinning svn:externals properties set on  * descendants of the path corresponding to PARENT_INFO. PINNED_EXTERNALS  * is keyed by the relative path of each descendant which should have some  * or all of its externals pinned, with the corresponding pinned svn:externals  * properties as values. Property changes are queued in a new list of path  * infos *NEW_PATH_INFOS, or in an existing item of the PATH_INFOS list if an  * existing item is found for the descendant. Allocate results in RESULT_POOL.  * Use SCRATCH_POOL for temporary allocations. */
end_comment
...
name|svn_error_t
modifier|*
name|queue_externals_change_path_infos
parameter_list|(
name|apr_array_header_t
modifier|*
name|new_path_infos
...
argument_list|)
expr_stmt|;
comment|/* Try to find a path info the external change can be applied to. */
name|info
operator|=
name|NULL
expr_stmt|;
...
condition|)
block|{
comment|/* A copied-along child needs its externals pinned.              Create a new path info for this property change. */
name|info
operator|=
name|apr_pcalloc
argument_list|(
...
name|NULL
expr_stmt|;
comment|/* Only needed on copied dirs */
name|info
operator|->
name|dst_path
operator|=
...
name|src_rel
expr_stmt|;
comment|/* May be NULL, if outside RA session scope */
name|info
operator|->
name|dst_path
operator|=
...
expr_stmt|;
block|}
comment|/* Add any items which only need their externals pinned. */
if|if
condition|(
name|pin_externals_only_infos
condition|)
...
begin_comment
comment|/* Queue a property change on a copy of LOCAL_ABSPATH to COMMIT_URL  * in the COMMIT_ITEMS list.  * If the list does not already have a commit item for COMMIT_URL  * add a new commit item for the property change.  * Allocate results in RESULT_POOL.  * Use SCRATCH_POOL for temporary allocations. */
end_comment
...
name|svn_error_t
modifier|*
name|queue_prop_change_commit_items
parameter_list|(
specifier|const
name|char
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* Add the commit items to the DAV commit item list to provide access      to dav properties (for pre http-v2 DAV) */
name|apr_array_cat
argument_list|(
name|commit_items_for_dav
argument_list|,
...
name|nb
expr_stmt|;
comment|/* Avoid a chicken-and-egg problem:              * If pinning externals we'll need to adjust externals              * properties before checking out any externals.              * But copy needs to happen before pinning because else there              * are no svn:externals properties to pin. */
if|if
condition|(
name|pin_externals
condition|)
...
expr_stmt|;
block|}
comment|/* Now update all externals in the newly created copy. */
name|SVN_ERR
argument_list|(
name|svn_wc__externals_gather_definitions
argument_list|(
...
name|i
decl_stmt|;
comment|/* Assert instead of crashing if the sources list is empty. */
name|SVN_ERR_ASSERT
argument_list|(
name|sources
operator|->
...
argument_list|,
name|FALSE
comment|/* pin_externals */
argument_list|,
name|NULL
comment|/* externals_to_pin */
argument_list|,
name|revprop_table
argument_list|,
name|commit_callback
