...
name|char
modifier|*
name|url_from_externals_definition
parameter_list|,
specifier|const
...
name|_
argument_list|(
literal|"The external '%s' defined in %s at '%s' "
literal|"cannot be checked out because '%s' is "
literal|"already a versioned path."
argument_list|)
argument_list|,
name|url_from_externals_definition
argument_list|,
...
condition|)
block|{
comment|/* This can't be a directory external! */
name|err
operator|=
name|svn_wc__external_remove
argument_list|(
...
condition|)
block|{
comment|/* New external... No problem that we can't remove it */
name|svn_error_clear
argument_list|(
name|err
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* We just decided that this existing directory is an external,                  so update the external registry with this information, like                  when checking out an external */
name|SVN_ERR
argument_list|(
...
begin_comment
comment|/* Try to update a file external at LOCAL_ABSPATH to SWITCH_LOC. This function    assumes caller has a write lock in CTX.  Use SCRATCH_POOL for temporary    allocations, and use the client context CTX. */
end_comment
...
comment|/* Get the RA connection, if needed. */
if|if
condition|(
name|ra_session
condition|)
