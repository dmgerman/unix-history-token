...
block|}
decl_stmt|;
comment|/* We are honoring mergeinfo so do not blindly record            * a conflict describing the merge of            * SOURCE->LOC1->URL@SOURCE->LOC1->REV through            * SOURCE->LOC2->URL@SOURCE->LOC2->REV            * but figure out the actual revision range merged. */
operator|(
name|void
operator|)
name|find_nearest_ancestor_with_intersecting_ranges
...
name|style
decl_stmt|;
comment|/* We used to use svn_client__get_normalized_stream() here, but              that doesn't work in 100% of the cases because it doesn't              convert EOLs to the repository form; just to '\n'.            */
if|if
condition|(
name|eol_style
condition|)
...
name|result_pool
expr_stmt|;
comment|/* Reparent the session to TARGET_LOC if this target location        * exists within the unmerged revision range. */
if|if
condition|(
name|target_loc
operator|->
