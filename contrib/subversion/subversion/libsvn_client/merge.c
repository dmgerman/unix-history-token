...
argument_list|)
expr_stmt|;
comment|/* None of the descendants was modified, but maybe there are          descendants we haven't walked?           Note that we aren't interested in changes, as we already verified          changes in the paths touched by the merge. And the existance of          other paths is enough to mark the directory edited */
name|err
operator|=
...
begin_comment
comment|/* Wrapper around svn_client_mergeinfo_log2. All arguments are as per    that API.  The discover_changed_paths, depth, and revprops args to    svn_client_mergeinfo_log2 are always TRUE, svn_depth_infinity_t,    and NULL respectively.     If RECEIVER raises a SVN_ERR_CEASE_INVOCATION error, but still sets    *REVISION to a valid revnum, then clear the error.  Otherwise return    any error. */
end_comment
