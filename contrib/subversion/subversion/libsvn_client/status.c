...
name|NULL
decl_stmt|;
comment|/* Override invalid combinations of the check_out_of_date and      check_working_copy flags. */
if|if
condition|(
operator|!
name|check_out_of_date
...
expr_stmt|;
block|}
comment|/* We only descend into an external if depth is svn_depth_infinity or      svn_depth_unknown.  However, there are conceivable behaviors that      would involve descending under other circumstances; thus, we pass      depth anyway, so the code will DTRT if we change the conditional      in the future.   */
if|if
condition|(
