...
begin_comment
comment|/* FSFS format 7's "block-read" feature performs poorly with small caches.  * Enable it only if caches above this threshold have been configured.  * The current threshold is 64MB. */
end_comment

begin_define
define|#
directive|define
name|BLOCK_READ_CACHE_THRESHOLD
value|(0x40 * 0x100000)
end_define
...
parameter_list|)
block|{
comment|/* Enable the "block-read" feature (where it applies)? */
name|svn_boolean_t
name|use_block_read
init|=
...
block|,
name|svnadmin__check_normalization
block|,
name|svnadmin__metadata_only
block|}
enum|;
end_enum
...
name|N_
argument_list|(
literal|"ignore revision datestamps found in the stream"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"type of repository:\n"
literal|"                             'fsfs' (default), 'bdb' or 'fsx'\n"
literal|"                             CAUTION: FSX is for EXPERIMENTAL use only!"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"continue verification after detecting a corruption"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"report any names within the same directory or\n"
literal|"                             svn:mergeinfo property value that differ only\n"
literal|"                             in character representation, but are otherwise\n"
literal|"                             identical"
argument_list|)
block|}
block|,
block|{
literal|"metadata-only"
block|,
name|svnadmin__metadata_only
block|,
literal|0
...
name|N_
argument_list|(
literal|"verify metadata only (ignored for BDB),\n"
literal|"                             checking against external corruption in\n"
literal|"                             Subversion 1.9+ format repositories.\n"
argument_list|)
block|}
block|,
block|{
...
name|N_
argument_list|(
literal|"usage: 1. svnadmin delrevprop REPOS_PATH -r REVISION NAME\n"
literal|"                   2. svnadmin delrevprop REPO_PATH -t TXN NAME\n\n"
literal|"1. Delete the property NAME on revision REVISION.\n\n"
literal|"Use --use-pre-revprop-change-hook/--use-post-revprop-change-hook to\n"
literal|"trigger the revision property-related hooks (for example, if you want\n"
literal|"an email notification sent from your post-revprop-change hook).\n\n"
literal|"NOTE: Revision properties are not versioned, so this command will\n"
literal|"irreversibly destroy the previous value of the property.\n\n"
literal|"2. Delete the property NAME on transaction TXN.\n"
argument_list|)
block|,
block|{
literal|'r'
...
name|N_
argument_list|(
literal|"usage: svnadmin info REPOS_PATH\n\n"
literal|"Print information about the repository at REPOS_PATH.\n"
argument_list|)
block|,
block|{
literal|0
...
name|N_
argument_list|(
literal|"usage: 1. svnadmin setrevprop REPOS_PATH -r REVISION NAME FILE\n"
literal|"                   2. svnadmin setrevprop REPOS_PATH -t TXN NAME FILE\n\n"
literal|"1. Set the property NAME on revision REVISION to the contents of FILE.\n\n"
literal|"Use --use-pre-revprop-change-hook/--use-post-revprop-change-hook to\n"
literal|"trigger the revision property-related hooks (for example, if you want\n"
literal|"an email notification sent from your post-revprop-change hook).\n\n"
literal|"NOTE: Revision properties are not versioned, so this command will\n"
literal|"overwrite the previous value of the property.\n\n"
literal|"2. Set the property NAME on transaction TXN to the contents of FILE.\n"
argument_list|)
block|,
block|{
literal|'r'
...
begin_comment
comment|/* This implements 'svn_error_malfunction_handler_t. */
end_comment
...
name|svn_error_t
modifier|*
name|crashtest_malfunction_handler
parameter_list|(
name|svn_boolean_t
name|can_return
parameter_list|,
...
name|_
argument_list|(
literal|"Successfully opened repository '%s'.\n"
literal|"Will now crash to simulate a crashing "
...
argument_list|)
expr_stmt|;
comment|/* In 1.9, we figured out that we didn't have to keep extending this          madness indefinitely. */
name|svn_hash_sets
argument_list|(
name|fs_config
argument_list|,
...
name|_
argument_list|(
literal|"Repositories compatible with 1.0.x must "
literal|"use --fs-type=bdb"
argument_list|)
argument_list|)
return|;
block|}
...
name|_
argument_list|(
literal|"Repositories compatible with 1.8.x or "
literal|"earlier cannot use --fs-type=%s"
argument_list|)
argument_list|,
name|SVN_FS_TYPE_FSX
argument_list|)
...
begin_comment
comment|/* Structure for errors encountered during 'svnadmin verify --keep-going'. */
end_comment
...
begin_struct
struct|struct
name|repos_verify_callback_baton
block|{
comment|/* Should we continue after receiving a first verification error? */
name|svn_boolean_t
name|keep_going
decl_stmt|;
comment|/* List of errors encountered during 'svnadmin verify --keep-going'. */
name|apr_array_header_t
modifier|*
name|error_summary
decl_stmt|;
comment|/* Pool for data collected during callback invocations. */
name|apr_pool_t
modifier|*
name|result_pool
decl_stmt|;
...
begin_comment
comment|/* Implementation of svn_repos_verify_callback_t to handle errors coming    from svn_repos_verify_fs3(). */
end_comment
...
name|_
argument_list|(
literal|"* Error verifying repository metadata.\n"
argument_list|)
argument_list|,
name|stderr
argument_list|,
...
name|_
argument_list|(
literal|"* Error verifying revision %ld.\n"
argument_list|)
argument_list|,
name|revision
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Remember the error in B->ERROR_SUMMARY. */
name|verr
operator|=
name|apr_palloc
argument_list|(
...
begin_comment
comment|/* Implementation of svn_repos_notify_func_t to wrap the output to a    response stream for svn_repos_dump_fs2(), svn_repos_verify_fs(),    svn_repos_hotcopy3() and others. */
end_comment
...
name|_
argument_list|(
literal|"Packed revision properties in shard %s\n"
argument_list|)
argument_list|,
name|shardstr
argument_list|)
...
name|_
argument_list|(
literal|"Removed non-packed revision properties"
literal|" in shard %s\n"
argument_list|)
argument_list|,
name|shardstr
...
name|_
argument_list|(
literal|"Bumped repository format to %ld\n"
argument_list|)
argument_list|,
name|notify
operator|->
...
name|_
argument_list|(
literal|"* Copied revisions from %ld to %ld.\n"
argument_list|)
argument_list|,
name|notify
operator|->
...
literal|"Subversion repository administration tool.\n"
...
begin_comment
comment|/* A helper for the 'setrevprop' and 'setlog' commands.  Expects    OPT_STATE->txn_id, OPT_STATE->use_pre_revprop_change_hook and    OPT_STATE->use_post_revprop_change_hook to be set appropriately.    If FILENAME is NULL, delete property PROP_NAME.  */
end_comment
...
name|_
argument_list|(
literal|"Calling hooks is incompatible with "
literal|"--transaction (-t)"
argument_list|)
argument_list|)
return|;
block|}
...
argument_list|)
expr_stmt|;
comment|/* Show the --keep-going error summary. */
if|if
condition|(
operator|!
name|opt_state
...
argument_list|)
expr_stmt|;
comment|/* The standard column width for the revision number is 6 characters.          If the revision number can potentially be larger (i.e. if end_revnum          is larger than 1000000), we increase the column width as needed. */
name|rev_maxlength
operator|=
literal|6
expr_stmt|;
...
name|_
argument_list|(
literal|"Failed to verify repository '%s'"
argument_list|)
argument_list|,
name|svn_dirent_local_style
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* fs_format will be printed later. */
name|SVN_ERR_ASSERT
argument_list|(
name|repos_version
operator|->
...
name|_
argument_list|(
literal|"Compatible With Version: %d.%d.0\n"
argument_list|)
argument_list|,
name|SVN_VER_MAJOR
argument_list|,
...
name|_
argument_list|(
literal|"Repository Capability: %s\n"
argument_list|)
argument_list|,
name|capability
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Print packing statistics, if enabled on the FS. */
if|if
condition|(
name|fsfs_info
operator|->
...
name|_
argument_list|(
literal|"FSFS Shards Packed: %ld/%ld\n"
argument_list|)
argument_list|,
name|shards_packed
argument_list|,
...
name|_
argument_list|(
literal|"FSFS Logical Addressing: yes\n"
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
...
name|_
argument_list|(
literal|"FSFS Logical Addressing: no\n"
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* 'svn info' prints an extra newline here, to support multiple targets.      We'll do the same. */
name|SVN_ERR
argument_list|(
name|svn_cmdline_printf
argument_list|(
...
name|prop_name
decl_stmt|;
comment|/* Expect one more argument: NAME */
name|SVN_ERR
argument_list|(
name|parse_args
argument_list|(
