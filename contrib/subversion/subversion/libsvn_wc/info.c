...
condition|)
block|{
comment|/* We have a working not-present status. We don't know anything              about this node, but it *is visible* in STATUS.               Let's tell that it is excluded */
name|wc_info
operator|->
name|depth
operator|=
...
condition|)
block|{
comment|/* We have a WORKING exclude. Avoid segfault on no repos info */
name|SVN_ERR
argument_list|(
name|svn_wc__db_read_repos_info
argument_list|(
...
name|first
decl_stmt|;
comment|/* The set of tree conflicts that have been found but not (yet) visited by    * the tree walker.  Map of abspath -> empty string. */
name|apr_hash_t
modifier|*
name|tree_conflicts
decl_stmt|;
