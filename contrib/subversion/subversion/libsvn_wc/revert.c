...
begin_comment
comment|/* Make the working tree under LOCAL_ABSPATH to depth DEPTH match the    versioned tree.  This function is called after svn_wc__db_op_revert    has done the database revert and created the revert list.  Notifies    for all paths equal to or below LOCAL_ABSPATH that are reverted.     REVERT_ROOT is true for explicit revert targets and FALSE for targets    reached via recursion.     Sets *RUN_WQ to TRUE when the caller should (eventually) run the workqueue.    (The function sets it to FALSE when it has run the WQ itself)     If INFO is NULL, LOCAL_ABSPATH doesn't exist in DB. Otherwise INFO    specifies the state of LOCAL_ABSPATH in DB.  */
end_comment
...
expr_stmt|;
block|}
comment|/* We delete these marker files even though they are not strictly metadata.      But for users that use revert as an API with metadata_only, these are. */
if|if
condition|(
name|conflict_files
condition|)
...
expr_stmt|;
block|}
comment|/* Run the queue per directory */
if|if
condition|(
operator|*
name|run_wq
...
begin_comment
comment|/* Perform the in-working copy revert of LOCAL_ABSPATH, to what is stored in DB */
end_comment
...
directive|endif
comment|/* Would be nice to use svn_io_dirent2_t here, but the performance      improvement that provides doesn't work, because we need the read      only and executable bits later on, in the most likely code path */
name|err
operator|=
name|svn_io_stat
argument_list|(
...
comment|/* Side effect: fixes recorded timestamps */
name|SVN_ERR
argument_list|(
...
condition|)
block|{
comment|/* Install will replace the file */
name|on_disk
operator|=
name|svn_node_none
expr_stmt|;
