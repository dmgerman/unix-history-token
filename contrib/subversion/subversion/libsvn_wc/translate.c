...
argument_list|)
expr_stmt|;
comment|/* streams enforce our contract that TO_NF streams are read-only            * by returning SVN_ERR_STREAM_NOT_SUPPORTED when trying to            * write to them. */
block|}
else|else
block|{
operator|*
...
argument_list|)
expr_stmt|;
comment|/* streams enforce our contract that FROM_NF streams are write-only            * by returning SVN_ERR_STREAM_NOT_SUPPORTED when trying to            * read them. */
block|}
block|}
return|return
name|SVN_NO_ERROR
...
argument_list|)
expr_stmt|;
comment|/* Handle special statuses (e.g. added) */
if|if
condition|(
operator|!
name|repos_relpath
