...
condition|)
continue|continue;
comment|/* Nothing added (deleted copy) */
block|}
elseif|else
if|if
condition|(
...
name|SVN_NO_ERROR
return|;
comment|/* Check for local property mods on this directory, if we haven't      already reported them. */
if|if
condition|(
operator|!
name|skip
...
expr_stmt|;
block|}
comment|/* svn_wc__db_status_incomplete should never happen, as the result won't be      stable or guaranteed related to what is in the repository for this      revision, but without this it would be hard to diagnose that status... */
name|assert
argument_list|(
name|kind
operator|==
...
argument_list|)
expr_stmt|;
comment|/* Must exist at one point. */
name|assert
argument_list|(
operator|!
name|left_source
...
argument_list|)
expr_stmt|;
comment|/* Either existed or added. */
...
begin_comment
comment|/* =====================================================================  * A tree processor filter that filters by changelist membership  * =====================================================================  *  * The current implementation queries the WC for the changelist of each  * file as it comes through, and sets the 'skip' flag for a non-matching  * file.  *  * (It doesn't set the 'skip' flag for a directory, as we need to receive  * the changed/added/deleted/closed call to know when it is closed, in  * order to preserve the strict open-close semantics for the wrapped tree  * processor.)  *  * It passes on the opening and closing of every directory, even if there  * are no file changes to be passed on inside that directory.  */
end_comment
...
name|wc_ctx
decl_stmt|;
comment|/* WC path of the root of the diff (where relpath = "") */
specifier|const
name|char
modifier|*
...
argument_list|)
decl_stmt|;
comment|/* Skip if not a member of a given changelist */
if|if
condition|(
operator|!
