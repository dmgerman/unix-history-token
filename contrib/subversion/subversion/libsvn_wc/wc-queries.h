...
begin_comment
comment|/* This file is automatically generated from wc-queries.sql and .dist_sandbox/subversion-1.9.7/subversion/libsvn_wc/token-map.h.  * Do not edit this file -- edit the source and rerun gen-make.py */
end_comment
...
name|STMT_4
define|\
value|"SELECT local_relpath, nodes.repos_id, nodes.repos_path, presence, kind, " \   "  revision, depth, file_external " \   "FROM nodes " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 AND op_depth = 0 " \   ""
end_define

begin_define
define|#
directive|define
name|STMT_SELECT_BASE_CHILDREN_INFO_LOCK
value|5
end_define
...
directive|define
name|STMT_5_INFO
value|{"STMT_SELECT_BASE_CHILDREN_INFO_LOCK", NULL}
end_define
...
name|STMT_7
define|\
value|"SELECT repos_id, repos_path, presence, kind, revision, checksum, " \   "  translated_size, changed_revision, changed_date, changed_author, depth, " \   "  symlink_target, properties, moved_to, moved_here " \   "FROM nodes " \   "WHERE wc_id = ?1 AND local_relpath = ?2 AND op_depth = ?3 " \   ""
end_define
...
define|#
directive|define
name|STMT_SELECT_BASE_NODE_CHILDREN_INFO
value|12
end_define
...
directive|define
name|STMT_12_INFO
value|{"STMT_SELECT_BASE_NODE_CHILDREN_INFO", NULL}
end_define
...
name|STMT_13
define|\
value|"SELECT local_relpath, op_depth, presence, kind " \   "FROM nodes_current " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 " \   "ORDER BY local_relpath " \   ""
end_define
...
name|STMT_24
define|\
value|"DELETE FROM nodes " \   "WHERE wc_id = ?1 AND local_relpath = ?2 " \   "  AND presence = 'base-deleted' " \   "  AND op_depth> ?3 " \   "  AND op_depth = (SELECT MIN(op_depth) FROM nodes n " \   "                    WHERE n.wc_id = ?1 " \   "                      AND n.local_relpath = nodes.local_relpath " \   "                      AND op_depth> ?3) " \   ""
end_define

begin_define
define|#
directive|define
name|STMT_DELETE_WORKING_BASE_DELETE_RECURSIVE
value|25
end_define
...
directive|define
name|STMT_25_INFO
value|{"STMT_DELETE_WORKING_BASE_DELETE_RECURSIVE", NULL}
end_define
...
name|STMT_25
define|\
value|"DELETE FROM nodes " \   "WHERE wc_id = ?1 AND (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   "  AND presence = 'base-deleted' " \   "  AND op_depth> ?3 " \   "  AND op_depth = (SELECT MIN(op_depth) FROM nodes n " \   "                    WHERE n.wc_id = ?1 " \   "                      AND n.local_relpath = nodes.local_relpath " \   "                      AND op_depth> ?3) " \   ""
end_define
...
name|STMT_27
define|\
value|"DELETE FROM nodes " \   "WHERE wc_id = ?1 AND (local_relpath = ?2 " \   "                      OR (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END))) " \   "  AND op_depth = 0 " \   ""
end_define
...
define|#
directive|define
name|STMT_SELECT_LAYER_FOR_REPLACE
value|29
end_define
...
directive|define
name|STMT_29_INFO
value|{"STMT_SELECT_LAYER_FOR_REPLACE", NULL}
end_define
...
name|STMT_29
define|\
value|"SELECT s.local_relpath, s.kind, " \   "  (CASE WHEN (?2) = '' THEN (CASE WHEN (?4) = '' THEN (s.local_relpath) WHEN (s.local_relpath) = '' THEN (?4) ELSE (?4) || '/' || (s.local_relpath) END) WHEN (?4) = '' THEN (CASE WHEN (?2) = '' THEN (s.local_relpath)  WHEN SUBSTR((s.local_relpath), 1, LENGTH(?2)) = (?2)  THEN CASE WHEN LENGTH(?2) = LENGTH(s.local_relpath) THEN '' WHEN SUBSTR((s.local_relpath), LENGTH(?2)+1, 1) = '/' THEN SUBSTR((s.local_relpath), LENGTH(?2)+2) END END) WHEN SUBSTR((s.local_relpath), 1, LENGTH(?2)) = (?2) THEN CASE WHEN LENGTH(?2) = LENGTH(s.local_relpath) THEN (?4) WHEN SUBSTR((s.local_relpath), LENGTH(?2)+1, 1) = '/' THEN (?4) || SUBSTR((s.local_relpath), LENGTH(?2)+1) END END) drp, 'normal' " \   "FROM nodes s " \   "WHERE s.wc_id = ?1 AND s.local_relpath = ?2 AND s.op_depth = ?3 " \   "UNION ALL " \   "SELECT s.local_relpath, s.kind, " \   "  (CASE WHEN (?2) = '' THEN (CASE WHEN (?4) = '' THEN (s.local_relpath) WHEN (s.local_relpath) = '' THEN (?4) ELSE (?4) || '/' || (s.local_relpath) END) WHEN (?4) = '' THEN (CASE WHEN (?2) = '' THEN (s.local_relpath)  WHEN SUBSTR((s.local_relpath), 1, LENGTH(?2)) = (?2)  THEN CASE WHEN LENGTH(?2) = LENGTH(s.local_relpath) THEN '' WHEN SUBSTR((s.local_relpath), LENGTH(?2)+1, 1) = '/' THEN SUBSTR((s.local_relpath), LENGTH(?2)+2) END END) WHEN SUBSTR((s.local_relpath), 1, LENGTH(?2)) = (?2) THEN CASE WHEN LENGTH(?2) = LENGTH(s.local_relpath) THEN (?4) WHEN SUBSTR((s.local_relpath), LENGTH(?2)+1, 1) = '/' THEN (?4) || SUBSTR((s.local_relpath), LENGTH(?2)+1) END END) drp, d.presence " \   "FROM nodes s " \   "LEFT OUTER JOIN nodes d ON d.wc_id= ?1 AND d.op_depth = ?5 " \   "     AND d.local_relpath = drp " \   "WHERE s.wc_id = ?1 " \   "  AND (((s.local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((s.local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   "  AND s.op_depth = ?3 " \   "ORDER BY s.local_relpath " \   ""
end_define
...
directive|define
name|STMT_30_INFO
value|{"STMT_SELECT_DESCENDANTS_OP_DEPTH_RV", NULL}
end_define
...
name|STMT_30
define|\
value|"SELECT local_relpath, kind " \   "FROM nodes " \   "WHERE wc_id = ?1 " \   "  AND (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   "  AND op_depth = ?3 " \   "  AND presence in ('normal', 'incomplete') " \   "ORDER BY local_relpath DESC " \   ""
end_define
...
define|#
directive|define
name|STMT_SELECT_NO_LONGER_MOVED_RV
value|32
end_define
...
directive|define
name|STMT_32_INFO
value|{"STMT_SELECT_NO_LONGER_MOVED_RV", NULL}
end_define
...
name|STMT_32
define|\
value|"SELECT d.local_relpath, (CASE WHEN (?2) = '' THEN (CASE WHEN (?4) = '' THEN (d.local_relpath) WHEN (d.local_relpath) = '' THEN (?4) ELSE (?4) || '/' || (d.local_relpath) END) WHEN (?4) = '' THEN (CASE WHEN (?2) = '' THEN (d.local_relpath)  WHEN SUBSTR((d.local_relpath), 1, LENGTH(?2)) = (?2)  THEN CASE WHEN LENGTH(?2) = LENGTH(d.local_relpath) THEN '' WHEN SUBSTR((d.local_relpath), LENGTH(?2)+1, 1) = '/' THEN SUBSTR((d.local_relpath), LENGTH(?2)+2) END END) WHEN SUBSTR((d.local_relpath), 1, LENGTH(?2)) = (?2) THEN CASE WHEN LENGTH(?2) = LENGTH(d.local_relpath) THEN (?4) WHEN SUBSTR((d.local_relpath), LENGTH(?2)+1, 1) = '/' THEN (?4) || SUBSTR((d.local_relpath), LENGTH(?2)+1) END END) srp, " \   "       b.presence, b.op_depth " \   "FROM nodes d " \   "LEFT OUTER JOIN nodes b ON b.wc_id = ?1 AND b.local_relpath = d.local_relpath " \   "            AND b.op_depth = (SELECT MAX(x.op_depth) FROM nodes x " \   "                              WHERE x.wc_id = ?1 " \   "                                AND x.local_relpath = b.local_relpath " \   "                                AND x.op_depth< ?3) " \   "WHERE d.wc_id = ?1 " \   "  AND (((d.local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((d.local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   "  AND d.op_depth = ?3 " \   "  AND NOT EXISTS(SELECT * FROM nodes s " \   "                 WHERE s.wc_id = ?1 " \   "                   AND s.local_relpath = srp " \   "                   AND s.op_depth = ?5) " \   "ORDER BY d.local_relpath DESC " \   ""
end_define
...
name|STMT_33
define|\
value|"SELECT local_relpath, kind FROM nodes " \   "WHERE wc_id = ?1 " \   "  AND parent_relpath = ?2 " \   "  AND op_depth = ?3 " \   "  AND presence != 'base-deleted' " \   "  AND file_external is NULL " \   "ORDER BY local_relpath " \   ""
end_define
...
directive|define
name|STMT_34_INFO
value|{"STMT_SELECT_OP_DEPTH_CHILDREN_EXISTS", NULL}
end_define
...
name|STMT_34
define|\
value|"SELECT local_relpath, kind FROM nodes " \   "WHERE wc_id = ?1 " \   "  AND parent_relpath = ?2 " \   "  AND op_depth = ?3 " \   "  AND presence IN ('normal', 'incomplete') " \   "ORDER BY local_relpath " \   ""
end_define
...
name|STMT_35
define|\
value|"SELECT 1 FROM nodes " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 " \   "  AND (op_depth> ?3 OR (op_depth = ?3 " \   "                         AND presence IN ('normal', 'incomplete'))) " \   "UNION ALL " \   "SELECT 1 FROM ACTUAL_NODE a " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 " \   "  AND NOT EXISTS (SELECT 1 FROM nodes n " \   "                   WHERE wc_id = ?1 AND n.local_relpath = a.local_relpath) " \   ""
end_define
...
name|STMT_39
define|\
value|"UPDATE NODES SET op_depth = 0, " \   "                 repos_id = ?4, " \   "                 repos_path = (CASE WHEN (?2) = '' THEN (CASE WHEN (?5) = '' THEN (local_relpath) WHEN (local_relpath) = '' THEN (?5) ELSE (?5) || '/' || (local_relpath) END) WHEN (?5) = '' THEN (CASE WHEN (?2) = '' THEN (local_relpath)  WHEN SUBSTR((local_relpath), 1, LENGTH(?2)) = (?2)  THEN CASE WHEN LENGTH(?2) = LENGTH(local_relpath) THEN '' WHEN SUBSTR((local_relpath), LENGTH(?2)+1, 1) = '/' THEN SUBSTR((local_relpath), LENGTH(?2)+2) END END) WHEN SUBSTR((local_relpath), 1, LENGTH(?2)) = (?2) THEN CASE WHEN LENGTH(?2) = LENGTH(local_relpath) THEN (?5) WHEN SUBSTR((local_relpath), LENGTH(?2)+1, 1) = '/' THEN (?5) || SUBSTR((local_relpath), LENGTH(?2)+1) END END), " \   "                 revision = ?6, " \   "                 dav_cache = NULL, " \   "                 moved_here = NULL, " \   "                 moved_to = NULL, " \   "                 presence = CASE presence " \   "                              WHEN 'normal' THEN 'normal' " \   "                              WHEN 'excluded' THEN 'excluded' " \   "                              ELSE 'not-present' " \   "                            END " \   "WHERE wc_id = ?1 " \   "  AND (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   "  AND op_depth = ?3 " \   ""
end_define
...
name|STMT_40
define|\
value|"SELECT DISTINCT local_relpath FROM nodes " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 " \   "ORDER BY local_relpath " \   ""
end_define
...
name|STMT_41
define|\
value|"SELECT DISTINCT local_relpath FROM nodes " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 " \   "  AND (op_depth> (SELECT MAX(op_depth) FROM nodes " \   "                   WHERE wc_id = ?1 AND local_relpath = ?2) " \   "       OR " \   "       (op_depth = (SELECT MAX(op_depth) FROM nodes " \   "                    WHERE wc_id = ?1 AND local_relpath = ?2) " \   "        AND presence IN ('normal', 'incomplete'))) " \   "ORDER BY local_relpath " \   ""
end_define

begin_define
define|#
directive|define
name|STMT_SELECT_BASE_NOT_PRESENT_CHILDREN
value|42
end_define
...
directive|define
name|STMT_42_INFO
value|{"STMT_SELECT_BASE_NOT_PRESENT_CHILDREN", NULL}
end_define
...
name|STMT_42
define|\
value|"SELECT local_relpath FROM nodes " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 AND op_depth = 0 " \   "  AND presence = 'not-present' " \   "ORDER BY local_relpath " \   ""
end_define
...
name|STMT_52
define|\
value|"SELECT b.presence, w.presence, w.op_depth, w.moved_to " \   "FROM nodes w " \   "LEFT JOIN nodes b ON b.wc_id = ?1 AND b.local_relpath = ?2 AND b.op_depth = 0 " \   "WHERE w.wc_id = ?1 AND w.local_relpath = ?2 " \   "  AND w.op_depth = (SELECT MAX(op_depth) FROM nodes d " \   "                    WHERE d.wc_id = ?1 AND d.local_relpath = ?2 " \   "                      AND d.op_depth> 0) " \   "LIMIT 1 " \   ""
end_define
...
name|STMT_54
define|\
value|"SELECT op_depth, moved_to " \   "FROM nodes " \   "WHERE wc_id = ?1 AND local_relpath = ?2 AND op_depth> ?3 " \   "  AND EXISTS(SELECT * from nodes " \   "             WHERE wc_id = ?1 AND local_relpath = ?2 AND op_depth = ?3 " \   "             AND presence IN ('normal', 'incomplete')) " \   "ORDER BY op_depth ASC " \   "LIMIT 1 " \   ""
end_define
...
define|#
directive|define
name|STMT_INSERT_ACTUAL_EMPTIES_FILES
value|84
end_define
...
directive|define
name|STMT_84_INFO
value|{"STMT_INSERT_ACTUAL_EMPTIES_FILES", NULL}
end_define
...
name|STMT_84
define|\
value|"INSERT OR IGNORE INTO actual_node ( " \   "     wc_id, local_relpath, parent_relpath) " \   "SELECT wc_id, local_relpath, parent_relpath " \   "FROM targets_list " \   "WHERE kind='file' " \   ""
end_define
...
name|STMT_86
define|\
value|"DELETE FROM actual_node " \   "WHERE wc_id = ?1 " \   "  AND (local_relpath = ?2 OR (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END))) " \   "  AND properties IS NULL " \   "  AND conflict_data IS NULL " \   "  AND changelist IS NULL " \   "  AND text_mod IS NULL " \   "  AND older_checksum IS NULL " \   "  AND right_checksum IS NULL " \   "  AND left_checksum IS NULL " \   ""
end_define
...
define|#
directive|define
name|STMT_CLEAR_ACTUAL_NODE_LEAVING_CONFLICT
value|97
end_define
...
directive|define
name|STMT_97_INFO
value|{"STMT_CLEAR_ACTUAL_NODE_LEAVING_CONFLICT", NULL}
end_define
...
name|STMT_97
define|\
value|"UPDATE actual_node " \   "SET properties = NULL, " \   "    text_mod = NULL, " \   "    tree_conflict_data = NULL, " \   "    older_checksum = NULL, " \   "    left_checksum = NULL, " \   "    right_checksum = NULL, " \   "    changelist = NULL " \   "WHERE wc_id = ?1 AND local_relpath = ?2 " \   ""
end_define
...
name|STMT_99
define|\
value|"UPDATE nodes SET depth = ?3 " \   "WHERE wc_id = ?1 AND local_relpath = ?2 AND op_depth = 0 " \   "  AND kind='dir' " \   "  AND presence IN ('normal', 'incomplete') " \   ""
end_define
...
define|#
directive|define
name|STMT_FIND_CONFLICT_DESCENDANT
value|121
end_define
...
directive|define
name|STMT_121_INFO
value|{"STMT_FIND_CONFLICT_DESCENDANT", NULL}
end_define
...
name|STMT_121
define|\
value|"SELECT 1 FROM actual_node " \   "WHERE wc_id = ?1 " \   "  AND local_relpath> (?2 || '/') " \   "  AND local_relpath< (?2 || '0') " \   "  AND conflict_data IS NOT NULL " \   "LIMIT 1 " \   ""
end_define
...
name|STMT_125
define|\
value|"INSERT INTO nodes ( " \   "    wc_id, local_relpath, op_depth, " \   "    parent_relpath, presence, kind) " \   "VALUES(?1, ?2, ?3, ?4, 'base-deleted', ?5) " \   ""
end_define
...
name|STMT_126
define|\
value|"INSERT OR REPLACE INTO nodes (wc_id, local_relpath, op_depth, parent_relpath, " \   "                              kind, moved_to, presence) " \   "SELECT wc_id, local_relpath, op_depth, parent_relpath, " \   "       kind, moved_to, 'base-deleted' " \   "  FROM nodes " \   " WHERE wc_id = ?1 " \   "   AND local_relpath = ?2 " \   "   AND op_depth = ?3 " \   ""
end_define
...
define|#
directive|define
name|STMT_COPY_OP_DEPTH_RECURSIVE
value|131
end_define
...
directive|define
name|STMT_131_INFO
value|{"STMT_COPY_OP_DEPTH_RECURSIVE", NULL}
end_define
...
name|STMT_151
define|\
value|"PRAGMA locking_mode = exclusive; " \   "PRAGMA journal_mode = DELETE " \   ""
end_define
...
name|STMT_170
define|\
value|"INSERT INTO delete_list(local_relpath) " \   "SELECT ?2 " \   "UNION ALL " \   "SELECT local_relpath FROM nodes AS n " \   "WHERE wc_id = ?1 " \   "  AND (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   "  AND op_depth>= ?3 " \   "  AND op_depth = (SELECT MAX(s.op_depth) FROM nodes AS s " \   "                  WHERE s.wc_id = ?1 " \   "                    AND s.local_relpath = n.local_relpath) " \   "  AND presence NOT IN ('base-deleted', 'not-present', 'excluded', 'server-excluded') " \   "  AND file_external IS NULL " \   "ORDER by local_relpath " \   ""
end_define
...
name|STMT_173
define|\
value|"DROP TABLE IF EXISTS update_move_list; " \   "CREATE TEMPORARY TABLE update_move_list ( " \   "  local_relpath TEXT PRIMARY KEY NOT NULL UNIQUE, " \   "  action INTEGER NOT NULL, " \   "  kind TEXT NOT NULL, " \   "  content_state INTEGER NOT NULL, " \   "  prop_state  INTEGER NOT NULL " \   "  ) " \   ""
end_define
...
define|#
directive|define
name|STMT_MOVE_NOTIFY_TO_REVERT
value|177
end_define
...
directive|define
name|STMT_177_INFO
value|{"STMT_MOVE_NOTIFY_TO_REVERT", NULL}
end_define
...
name|STMT_177
define|\
value|"INSERT INTO revert_list (local_relpath, notify, kind, actual) " \   "       SELECT local_relpath, 2, kind, 1 FROM update_move_list; " \   "DROP TABLE update_move_list " \   ""
end_define
...
directive|define
name|STMT_194_INFO
value|{"STMT_SELECT_MOVED_DESCENDANTS_SRC", NULL}
end_define
...
name|STMT_195
define|\
value|"UPDATE nodes SET repos_id = ?4, " \   "                 repos_path = (CASE WHEN (?2) = '' THEN (CASE WHEN (?5) = '' THEN (local_relpath) WHEN (local_relpath) = '' THEN (?5) ELSE (?5) || '/' || (local_relpath) END) WHEN (?5) = '' THEN (CASE WHEN (?2) = '' THEN (local_relpath)  WHEN SUBSTR((local_relpath), 1, LENGTH(?2)) = (?2)  THEN CASE WHEN LENGTH(?2) = LENGTH(local_relpath) THEN '' WHEN SUBSTR((local_relpath), LENGTH(?2)+1, 1) = '/' THEN SUBSTR((local_relpath), LENGTH(?2)+2) END END) WHEN SUBSTR((local_relpath), 1, LENGTH(?2)) = (?2) THEN CASE WHEN LENGTH(?2) = LENGTH(local_relpath) THEN (?5) WHEN SUBSTR((local_relpath), LENGTH(?2)+1, 1) = '/' THEN (?5) || SUBSTR((local_relpath), LENGTH(?2)+1) END END), " \   "                 revision = ?6 " \   "WHERE wc_id = ?1 " \   "  AND (local_relpath = ?2 " \   "       OR (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END))) " \   "  AND op_depth = ?3 " \   ""
end_define
...
name|STMT_198
define|\
value|"SELECT 1 FROM nodes " \   "WHERE wc_id = ?1 AND parent_relpath = ?2 AND op_depth = ?3 AND kind != 'file' " \   "LIMIT 1 " \   ""
end_define
...
name|STMT_199
define|\
value|"SELECT 1 FROM nodes " \   "WHERE wc_id = ?1 " \   "  AND (((parent_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((parent_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   "  AND op_depth = ?3 " \   "  AND file_external IS NULL " \   "LIMIT 1 " \   ""
end_define
...
directive|define
name|STMT_206_INFO
value|{"STMT_HAVE_STAT1_TABLE", NULL}
end_define
...
name|STMT_206
define|\
value|"SELECT 1 FROM sqlite_master WHERE name='sqlite_stat1' AND type='table' " \   "LIMIT 1 " \   ""
end_define
