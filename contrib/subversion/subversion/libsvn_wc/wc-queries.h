...
begin_comment
comment|/* This file is automatically generated from wc-queries.sql and .dist_sandbox/subversion-1.8.1/subversion/libsvn_wc/token-map.h.  * Do not edit this file -- edit the source and rerun gen-make.py */
end_comment
...
name|STMT_190
define|\
value|"SELECT local_relpath, moved_to, op_depth, kind FROM nodes " \   "WHERE wc_id = ?1 " \   "  AND (local_relpath = ?2 OR (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END))) " \   "  AND op_depth> ?3 " \   "  AND moved_to IS NOT NULL " \   ""
end_define
...
name|STMT_191
define|\
value|"SELECT local_relpath, moved_to FROM nodes " \   "WHERE wc_id = ?1 " \   "  AND (local_relpath = ?2 OR (((local_relpath)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((local_relpath)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END))) " \   "  AND op_depth>= ?3 " \   "  AND moved_to IS NOT NULL " \   "  AND NOT (((moved_to)> (CASE (?2) WHEN '' THEN '' ELSE (?2) || '/' END)) AND ((moved_to)< CASE (?2) WHEN '' THEN X'FFFF' ELSE (?2) || '0' END)) " \   ""
end_define
