...
name|char
modifier|*
name|defining_directory_display
init|=
...
argument_list|)
return|;
comment|/* The appearance of -r N or -rN forces the type of external.          If -r is at the beginning of the line or the first token is          an absolute URL or if the second token is not an absolute          URL, then the URL supports peg revisions. */
if|if
condition|(
literal|0
operator|==
...
name|NULL
decl_stmt|;
comment|/* The file wasn't updated, but its url or revision might have...          e.g. switch between branches for relative externals.           Just bump the information as that is just as expensive as          investigating when we should and shouldn't update it...          and avoid hard to debug edge cases */
if|if
condition|(
name|eb
operator|->
...
name|pool
argument_list|)
comment|/* exclude_relpaths */
argument_list|,
name|wcroot_iprops
argument_list|,
name|TRUE
comment|/* empty update */
argument_list|,
name|eb
operator|->
name|notify_func
