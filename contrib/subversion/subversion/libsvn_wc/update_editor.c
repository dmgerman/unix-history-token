...
name|wcroot_abspath
decl_stmt|;
comment|/* After closing the root directory a copy of its edited value */
name|svn_boolean_t
name|edited
decl_stmt|;
name|apr_pool_t
...
decl_stmt|;
comment|/* A hash of file names (only the hash key matters) seen by add_file and      add_directory and not yet added to the database, mapping to a const      char * node kind (via svn_node_kind_to_word(). */
name|apr_hash_t
modifier|*
name|not_present_nodes
decl_stmt|;
...
comment|/* The calculated checksum of the text source or NULL if the actual      checksum is not being calculated. The checksum kind is identical to the      kind of expected_source_checksum. */
name|svn_checksum_t
modifier|*
...
begin_comment
comment|/* Calculate the new repos_relpath for a directory or file */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* Figure out the new_repos_relpath for this directory. */
if|if
condition|(
name|eb
operator|->
...
condition|)
block|{
comment|/* No parent baton and target_basename=="" means that we are                  the target of the switch. Thus, our new_repos_relpath will be                  the switch_repos_relpath.  */
operator|*
name|new_repos_relpath
operator|=
name|eb
...
block|{
comment|/* This directory is *not* the root (has a parent). If there is              no grandparent, then we may have anchored at the parent,              and self is the target. If we match the target, then set              new_repos_relpath to the switch_repos_relpath.               Otherwise, we simply extend new_repos_relpath from the parent.  */
if|if
condition|(
name|pb
operator|->
...
block|{
comment|/* The node is locally replaced but could also be moved-away,                but we can't report that it is moved away and replaced.                 And we wouldn't be able to store that each of a dozen                descendants was moved to other locations...                 Replaced is what actually happened... */
name|reason
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Whatever the kind of conflict, we can just clear BASE          by turning whatever is there into a copy */
block|}
comment|/* Calculate the repository-relative path of the entry which was    * deleted. For updates it's the same as REPOS_RELPATH but for    * switches it is within the switch target. */
name|SVN_ERR
argument_list|(
name|calculate_repos_relpath
argument_list|(
...
condition|)
block|{
comment|/* We have an editor violation. Github sometimes does this              in its subversion compatibility code, when changing the              depth of a working copy, or on updates from incomplete */
block|}
else|else
block|{
...
argument_list|)
expr_stmt|;
comment|/* Else fall through as we should update the revision anyway */
if|if
condition|(
name|status
operator|==
...
block|{
comment|/* We have a normal local node that will now be hidden for the                  user. Let's try to delete what is there. This may introduce                  tree conflicts if there are local changes */
name|SVN_ERR
argument_list|(
name|delete_entry
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* delete_entry() promises that BASE is empty after the operation,                  so we can just fall through now */
block|}
block|}
block|}
elseif|else
...
block|{
comment|/* We have a local addition. If this would be a BASE node it would have          been deleted before we get here. (Which might have turned it into          a copy). */
name|SVN_ERR_ASSERT
argument_list|(
name|status
operator|!=
...
name|install_data
decl_stmt|;
comment|/* By convention return value is undefined on error, but we rely      on HB->INSTALL_DATA value in window_handler() and abort      INSTALL_STREAM if is not NULL on error.      So we store INSTALL_DATA to local variable first, to leave      HB->INSTALL_DATA unchanged on error. */
name|SVN_ERR
argument_list|(
...
begin_comment
comment|/* Implements svn_wc__proplist_receiver_t.  * Check for the presence of an svn:keywords property and queues an install_file  * work queue item if present. Thus, when the work queue is run to complete the  * switch operation, all files with keywords will go through the translation  * process so URLs etc are updated. */
end_comment
...
name|svn_error_t
modifier|*
name|update_keywords_after_switch_cb
parameter_list|(
name|void
modifier|*
name|baton
...
block|}
block|}
comment|/* Update keywords in switched files.      GOTO #1975 (the year of the Altair 8800). */
if|if
condition|(
name|eb
operator|->
...
name|char
modifier|*
name|tmp_dir_abspath
decl_stmt|;
comment|/* We are not installing a PRISTINE file, but we use the same code to          create whatever we want to install */
name|SVN_ERR
argument_list|(
name|svn_wc__db_temp_wcroot_tempdir
argument_list|(
