...
directive|endif
comment|/* __cplusplus */
comment|/* Returns TRUE if WI refers to a single work item. Returns FALSE if    WI is a list of work items. WI must not be NULL.     A work item looks like: (OP_CODE arg1 arg2 ...)     If we see OP_CODE (an atom) as WI's first child, then this is a    single work item. Otherwise, it is a list of work items.  */
define|#
directive|define
...
parameter_list|)
function_decl|;
comment|/* For the WCROOT identified by the DB and WRI_ABSPATH pair, run any    work items that may be present in its workqueue.  */
name|svn_error_t
modifier|*
name|svn_wc__wq_run
parameter_list|(
...
parameter_list|)
function_decl|;
comment|/* Set *WORK_ITEM to a new work item that will install the working    copy file at LOCAL_ABSPATH. If USE_COMMIT_TIMES is TRUE, then the newly    installed file will use the nodes CHANGE_DATE for the file timestamp.    If RECORD_FILEINFO is TRUE, then the resulting RECORDED_TIME and    RECORDED_SIZE will be recorded in the database.     If SOURCE_ABSPATH is NULL, then the pristine contents will be installed    (with appropriate translation). If SOURCE_ABSPATH is not NULL, then it    specifies a source file for the translation. The file must exist for as    long as *WORK_ITEM exists (and is queued). Typically, it will be a    temporary file, and an OP_FILE_REMOVE will be queued to later remove it. */
name|svn_error_t
modifier|*
...
parameter_list|)
function_decl|;
comment|/* Set *WORK_ITEM to a new work item that will remove a single    file LOCAL_ABSPATH from the working copy identified by the pair DB,    WRI_ABSPATH.  */
name|svn_error_t
modifier|*
...
parameter_list|)
function_decl|;
comment|/* Set *WORK_ITEM to a new work item that will remove a single    directory or if RECURSIVE is TRUE a directory with all its    descendants.  */
name|svn_error_t
modifier|*
...
parameter_list|)
function_decl|;
comment|/* Set *WORK_ITEM to a new work item that describes a move of    a file or directory from SRC_ABSPATH to DST_ABSPATH, ready for    storing in the working copy managing DST_ABSPATH.     Perform temporary allocations in SCRATCH_POOL and *WORK_ITEM in    RESULT_POOL. */
name|svn_error_t
modifier|*
...
parameter_list|)
function_decl|;
comment|/* Set *WORK_ITEM to a new work item that describes a copy from    SRC_ABSPATH to DST_ABSPATH, while translating the stream using    the information from LOCAL_ABSPATH. */
name|svn_error_t
modifier|*
...
parameter_list|)
function_decl|;
comment|/* Set *WORK_ITEM to a new work item that will synchronize the    target node's readonly and executable flags with the values defined    by its properties and lock status.  */
name|svn_error_t
modifier|*
...
parameter_list|)
function_decl|;
comment|/* Handle the final post-commit step of retranslating and recording the    working copy state of a committed file.     If PROP_MODS is false, assume that properties are not changed.     (Property modifications are read when svn_wc__wq_build_file_commit     is called and processed when the working queue is being evaluated)      Allocate *work_item in RESULT_POOL. Perform temporary allocations     in SCRATCH_POOL.    */
name|svn_error_t
modifier|*
...
parameter_list|)
function_decl|;
comment|/* Set *WORK_ITEM to a new work item that will install the working    copy directory at LOCAL_ABSPATH. */
name|svn_error_t
modifier|*
