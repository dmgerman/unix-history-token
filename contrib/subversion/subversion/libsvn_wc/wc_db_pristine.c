...
return|;
block|}
comment|/* Open the file as a readable stream.  It will remain readable even when    * deleted from disk; APR guarantees that on Windows as well as Unix.    *    * We also don't enable APR_BUFFERED on this file to maximize throughput    * e.g. for fulltext comparison.  As we use SVN__STREAM_CHUNK_SIZE buffers    * where needed in streams, there is no point in having another layer of    * buffers. */
if|if
condition|(
name|contents
condition|)
...
name|_
argument_list|(
literal|"Unable to create pristine install stream"
argument_list|)
argument_list|)
expr_stmt|;
operator|(
...
name|svn_error_t
modifier|*
name|svn_wc__db_pristine_install_abort
parameter_list|(
name|svn_wc__db_install_data_t
modifier|*
name|install_data
