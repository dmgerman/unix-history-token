...
begin_comment
comment|/* This is the reverse of svn_wc__db_extend_parent_delete.     When removing a node if the parent has a higher working node then    the parent node and this node are both deleted or replaced and any    delete over this node must be removed.  */
end_comment
...
name|repos_uuid
decl_stmt|;
comment|/* Pass the right repos-id for the destination db! */
name|SVN_ERR
argument_list|(
name|svn_wc__db_fetch_repos_info
argument_list|(
...
condition|)
block|{
comment|/* Update the op-depth of an moved node below this tree */
name|fixup
operator|=
name|TRUE
expr_stmt|;
...
name|below_status
decl_stmt|;
comment|/* Use STMT_SELECT_NODE_INFO directly instead of read_info plus          info_below_working */
name|SVN_ERR
argument_list|(
name|info_below_working
argument_list|(
...
operator|)
expr_stmt|;
comment|/* Moved-to can only exist at op_depth> 0. */
comment|/* ### Should we really do this for every layer where op_depth> 0                  in undefined order? */
name|moved_to_relpath
operator|=
name|svn_sqlite__column_text
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Moved-here can only exist at op_depth> 0. */
comment|/* ### Should we really do this for every layer where op_depth> 0                  in undefined order? */
name|child_item
operator|->
name|info
