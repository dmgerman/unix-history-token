...
decl_stmt|;
comment|/* Scan the working copy for local modifications and missing nodes. */
name|svn_boolean_t
...
name|prop_status
expr_stmt|;
comment|/* 5. Easy out:  unless we're fetching -every- node, don't bother      to allocate a struct for an uninteresting node.       This filter should match the filter in is_sendable_status() */
if|if
condition|(
operator|!
name|get_all
...
operator|->
name|has_descendants
comment|/* is dir, or was dir and tc descendants */
condition|)
block|{
name|SVN_ERR
argument_list|(
...
condition|)
block|{
comment|/* We have a node, but its not visible in the WC. It can be a marker          node (not present, (server) excluded), *or* it can be the explictly          passed target of the status walk operation that doesn't exist.           We only report the node when the caller explicitly as       */
if|if
condition|(
name|dirent
operator|==
...
name|status
decl_stmt|;
comment|/* Copy the internal/private data. */
name|svn_wc__internal_status_t
modifier|*
name|is
init|=
...
begin_comment
comment|/**  * Return a boolean answer to the question "Is @a status something that  * should be reported?".  @a no_ignore and @a get_all are the same as  * svn_wc_get_status_editor4().  *  * This implementation should match the filter in assemble_status()  */
end_comment
...
operator|->
name|has_descendants
comment|/* is dir, or was dir and has tc descendants */
operator|&&
name|info
operator|->
name|status
...
argument_list|)
expr_stmt|;
comment|/* Even though passing parent_repos_* is not required, assemble_status needs      these values to determine if a node is switched */
if|if
condition|(
operator|!
name|is_root
...
parameter_list|)
block|{
comment|/* Allocate slightly more room */
name|svn_wc__internal_status_t
modifier|*
name|new_istat
init|=
