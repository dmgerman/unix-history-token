...
begin_comment
comment|/* #define VERIFY_ON_CLOSE */
end_comment
...
begin_comment
comment|/* Implements svn_wc__db_verify_cb_t */
end_comment
...
argument_list|(
operator|(
literal|"DB-VRFY: %s: %s (%d): SV%04d %s"
operator|,
name|wc_abspath
operator|,
name|local_relpath
...
name|getenv
argument_list|(
literal|"SVN_CMDLINE_VERIFY_SQL_AT_CLOSE"
argument_list|)
condition|)
block|{
name|apr_pool_t
...
name|SVN_CONFIG_SECTION_WORKING_COPY
argument_list|,
name|SVN_CONFIG_OPTION_SQLITE_BUSY_TIMEOUT
argument_list|,
literal|0
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* Verify that no work items exists. If they do, then our integrity is      suspect and, thus, we cannot upgrade this database.  */
if|if
condition|(
name|format
operator|>=
...
begin_comment
comment|/* Verify if the sqlite_stat1 table exists and if not tries to add    this table (but ignores errors on adding the schema) */
end_comment
...
begin_comment
comment|/* Sqlite transaction helper for opening the db in    svn_wc__db_wcroot_parse_local_abspath() to avoid multiple    db operations that each obtain and release a lock */
end_comment
