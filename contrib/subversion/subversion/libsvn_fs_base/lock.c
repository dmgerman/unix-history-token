...
begin_comment
comment|/* The effective arguments for txn_body_lock() below. */
end_comment
...
begin_comment
comment|/* The body of svn_fs_base__lock(), which see.     BATON is a 'struct lock_args *' holding the effective arguments.    BATON->path is the canonical abspath to lock.  Set *BATON->lock_p    to the resulting lock.  For the other arguments, see    svn_fs_lock_many().     This implements the svn_fs_base__retry_txn() 'body' callback type.  */
end_comment
...
name|path
expr_stmt|;
comment|/* Already in result_pool. */
name|lock
operator|->
name|owner
operator|=
...
begin_comment
comment|/* The effective arguments for txn_body_unlock() below. */
end_comment
...
begin_comment
comment|/* The body of svn_fs_base__unlock(), which see.     BATON is a 'struct unlock_args *' holding the effective arguments.    BATON->path is the canonical path and BATON->token is the token.    For the other arguments, see svn_fs_unlock_many().     This implements the svn_fs_base__retry_txn() 'body' callback type.  */
end_comment
