...
directive|define
name|USER_AGENT
value|"SVN/" SVN_VER_NUMBER " (" SVN_BUILD_TARGET ")" \                    " ra_local"
end_define
...
name|svn_error_t
modifier|*
name|svn_ra_local__dup_session
parameter_list|(
name|svn_ra_session_t
modifier|*
name|new_session
...
name|callback_baton
expr_stmt|;
comment|/* ### Re-use existing FS handle? */
comment|/* Reuse existing code */
name|SVN_ERR
argument_list|(
name|svn_ra_local__split_URL
argument_list|(
...
begin_comment
comment|/* The repository layer does not correctly fill in REPOS_ROOT in    commit_info, as it doesn't know the url that is used to access    it. This hooks the callback to fill in the missing pieces. */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* Set repos_root_url in commit info */
name|remap_commit_callback
argument_list|(
operator|&
name|callback
...
begin_comment
comment|/* Obtain the properties for a node, including its 'entry props */
end_comment
...
begin_comment
comment|/* Implements svn_fs_lock_callback_t.  Used by svn_ra_local__lock and    svn_ra_local__unlock to forward to supplied callback and record any    callback error. */
end_comment
...
comment|/* This means the library was loaded as a DSO, so use the DSO pool. */
name|SVN_ERR
argument_list|(
