...
begin_comment
comment|/* If CFG specifies a path to the password DB, read that DB through  * CONFIG_POOL and store it in REPOSITORY->PWDB.  */
end_comment
...
begin_comment
comment|/* Canonicalize *ACCESS_FILE based on the type of argument.  Results are  * placed in *ACCESS_FILE.  REPOSITORY is used to convert relative paths to  * absolute paths rooted at the server root.  REPOS_ROOT is used to calculate  * an absolute URL for repos-relative URLs. */
end_comment
...
begin_comment
comment|/* Load the authz database for the listening server through AUTHZ_POOL    based on the entries in the SERVER struct.     SERVER and CONN must not be NULL. The real errors will be logged with    SERVER and CONN but return generic errors to the client. */
end_comment
...
begin_comment
comment|/* If ERROR is a AUTH* error as returned by load_pwdb_config or  * load_authz_config, write it to SERVER's log file.  * Return a sanitized version of ERROR.  */
end_comment
...
begin_comment
comment|/* Return the access level specified for OPTION in CFG.  If no such  * setting exists, use DEF.  If READ_ONLY is set, unconditionally disable  * write access.  */
end_comment
...
begin_comment
comment|/* Set the *_ACCESS members in REPOSITORY according to the settings in  * CFG.  If READ_ONLY is set, unconditionally disable write access.  */
end_comment
...
begin_comment
comment|/* Return the access level for the user in B.  */
end_comment
...
begin_comment
comment|/* Implements svn_fs_lock_callback_t. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Fetch the directories' entries before starting the response, to allow      proper error handling in cases like when FULL_PATH doesn't exist */
if|if
condition|(
name|want_contents
condition|)
...
argument_list|)
expr_stmt|;
comment|/* send LOG_ENTRY->CHANGED_PATHS2 */
name|SVN_ERR
argument_list|(
name|svn_ra_svn__start_list
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* send LOG_ENTRY main members */
name|SVN_ERR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* send LOG_ENTRY->REVPROPS */
name|SVN_ERR
argument_list|(
name|svn_ra_svn__start_list
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* send LOG_ENTRY members that were added in later SVN releases */
name|SVN_ERR
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* No START_REV or PEG_REVISION?  We'll use HEAD. */
if|if
condition|(
operator|!
name|SVN_IS_VALID_REVNUM
...
argument_list|)
expr_stmt|;
comment|/* Parse the lock requests from PATH_REVS into TARGETS. */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Any duplicate paths, once canonicalized, get collapsed into a          single path that is processed once.  The result is then          returned multiple times. */
name|svn_hash_sets
argument_list|(
name|targets
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Check authz.       Note: From here on we need to make sure any errors in authz_results, or      results, are cleared before returning from this function. */
for|for
control|(
name|hi
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Return results in the same order as the paths were supplied. */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* No result?  Something really odd happened, create a              placeholder error so that any other results can be              reported in the correct order. */
name|result
operator|=
name|apr_palloc
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Parse the unlock requests from PATH_REVS into TARGETS. */
for|for
control|(
name|i
operator|=
...
begin_comment
comment|/* Look for the repository given by URL, using ROOT as the virtual  * repository root.  If we find one, fill in the repos, fs, repos_url,  * and fs_path fields of REPOSITORY.  VHOST and READ_ONLY flags are the  * same as in the server baton.  *  * CONFIG_POOL and AUTHZ_POOL shall be used to load any object of the  * respective type.  *  * Use SCRATCH_POOL for temporary allocations.  *  */
end_comment
...
begin_comment
comment|/* Construct the server baton for CONN using PARAMS and return it in *BATON.  * It's lifetime is the same as that of CONN.  SCRATCH_POOL  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* Prepare command parser. */
name|apr_hash_t
modifier|*
name|cmd_hash
init|=
...
argument_list|)
expr_stmt|;
comment|/* Auto-initialize connection */
if|if
condition|(
operator|!
name|connection
...
block|}
comment|/* create the connection, configure ports etc. */
name|connection
operator|->
name|conn
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Construct server baton and open the repository for the first time. */
name|err
operator|=
name|construct_server_baton
argument_list|(
...
expr_stmt|;
block|}
comment|/* If we can't access the repo for some reason, end this connection. */
if|if
condition|(
name|err
condition|)
...
name|TRUE
expr_stmt|;
comment|/* Process incoming commands. */
while|while
condition|(
operator|!
name|terminate
...
name|has_command
decl_stmt|;
comment|/* If the server is busy, execute just one command and only if            * there is one currently waiting in our receive buffers.            */
name|err
operator|=
...
block|{
comment|/* The server is not busy, thus let's serve whichever command            * comes in next and whenever it comes in.  This requires the            * busy() callback test to return TRUE while there are still some            * resources left.            */
name|err
operator|=
name|svn_ra_svn__handle_command
argument_list|(
...
block|}
block|}
comment|/* error or normal end of session. Close the connection */
name|svn_pool_destroy
argument_list|(
name|iterpool
argument_list|)
