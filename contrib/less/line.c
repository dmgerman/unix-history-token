...
name|n
decl_stmt|;
comment|/* 	 * We display the line number at the start of each line 	 * only if the -N option is set. 	 */
if|if
condition|(
name|linenums
operator|!=
...
condition|)
return|return;
comment|/* 	 * Get the line number and put it in the current line. 	 * {{ Note: since find_linenum calls forw_raw_line, 	 *    it may seek in the input file, requiring the caller  	 *    of plinenum to re-seek if necessary. }} 	 */
name|lno
operator|=
name|find_linenum
argument_list|(
...
literal|0
expr_stmt|;
comment|/* 	 * Append enough spaces to bring us to the next tab stop. 	 * {{ We could avoid this at the cost of adding some 	 *    complication to the tab stop logic in pappend(). }} 	 */
if|if
condition|(
name|tabstop
operator|==
