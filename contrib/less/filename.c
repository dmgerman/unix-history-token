...
begin_comment
comment|/*  * Get the characters which the shell considers to be "metacharacters".  */
end_comment
...
condition|)
block|{
comment|/* 				 * We've got a metachar, but this shell  				 * doesn't support escape chars.  Use quotes. 				 */
name|use_quotes
operator|=
literal|1
expr_stmt|;
...
name|have_quotes
condition|)
comment|/* 			 * We can't quote a string that contains quotes. 			 */
return|return
operator|(
name|NULL
operator|)
...
block|}
block|}
comment|/* 	 * Call it a binary file if there are more than 5 binary characters 	 * in the first 256 bytes of the file. 	 */
return|return
operator|(
name|bin_count
operator|>
...
name|esccmd
decl_stmt|;
comment|/* 		 * Read the output of<$SHELL -c cmd>.   		 * Escape any metacharacters in the command. 		 */
name|esccmd
operator|=
name|shell_quote
argument_list|(
...
begin_comment
comment|/*  * Return number of %s escapes in a string.  * Return a large number if there are any other % escapes besides %s.  */
end_comment
...
name|error
argument_list|(
literal|"LESSOPEN ignored: must contain exactly one %%s"
argument_list|,
name|NULL_PARG
argument_list|)
expr_stmt|;
...
name|f
decl_stmt|;
comment|/* 		 * The first time we open the file, read one char  		 * to see if the pipe will produce any data. 		 * If it does, push the char back on the pipe. 		 */
name|f
operator|=
name|fileno
argument_list|(
...
condition|)
block|{
comment|/* 			 * Pipe is empty. 			 * If more than 1 pipe char was specified, 			 * the exit status tells whether the file itself  			 * is empty, or if there is no alt file. 			 * If only one pipe char, just assume no alt file. 			 */
name|int
name|status
init|=
name|pclose
...
name|error
argument_list|(
literal|"LESSCLOSE ignored; must contain no more than 2 %%s"
argument_list|,
name|NULL_PARG
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * Return last component of a pathname.  */
end_comment
