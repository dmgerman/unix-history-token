...
name|c_mtime
expr_stmt|;
comment|/* Debian hack: Silently ignore EROFS because reading the file          won't have upset its timestamp if it's on a read-only          filesystem.  This has been submitted as a suggestion to          "bug-gnu-utils@prep.ai.mit.edu".  -BEM */
if|if
condition|(
name|utime
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Debian hack: The type of dev_t has changed in glibc.  Fixed output          to ensure that a long int is passed to sprintf.  This has been          reported to "bug-gnu-utils@prep.ai.mit.edu". (1998/5/26) -BEM */
name|sprintf
argument_list|(
name|ascii_header
argument_list|,
literal|"%06ho%06lo%06lo%06lo%06lo%06lo%06lo%06lo%011lo%06lo%011lo"
argument_list|,
name|file_hdr
operator|->
name|c_magic
