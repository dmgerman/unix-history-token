...
name|f
parameter_list|)
value|tar_otoa(f, sizeof f)
end_define

begin_comment
comment|/* Convert the string of octal digits S into a number.  * Converted from GNU cpio 2.6 sources in  * order to fix tar breakage caused by using  * from_ascii.  */
end_comment
...
begin_comment
comment|/* Try to split a long file name into prefix and suffix parts separated    by a slash. Return the length of the prefix (not counting the slash). */
end_comment

begin_function
specifier|static
name|size_t
name|split_long_name
parameter_list|(
specifier|const
name|char
modifier|*
...
condition|)
do|;
comment|/* Add leading zeroes, if necessary.  */
while|while
condition|(
name|digits
operator|>
...
expr_stmt|;
block|}
comment|/* Ustar standard (POSIX.1-1988) requires the mode to contain only 3 octal      digits */
name|to_oct
argument_list|(
name|file_hdr
operator|->
...
literal|07777
expr_stmt|;
comment|/* Debian hack: This version of cpio uses the -n flag also to extract      tar archives using the numeric UID/GID instead of the user/group      names in /etc/passwd and /etc/groups.  (98/10/15) -BEM */
if|if
condition|(
name|archive_format
operator|==
...
name|true
return|;
comment|/* See whether we can split up the name into acceptably-sized      `prefix' and `name' (`p') pieces. */
name|prefix_name_len
operator|=
name|split_long_name
argument_list|(
