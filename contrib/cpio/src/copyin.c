...
begin_comment
comment|/* Now in util.c, but different */
end_comment
...
begin_comment
unit|static const char * safer_name_suffix (char const *file_name) {   char const *p;
comment|/* Skip file system prefixes, leading file name components that contain      "..", and leading slashes.  */
end_comment

begin_endif
unit|size_t prefix_len = FILE_SYSTEM_PREFIX_LEN (file_name);    for (p = file_name + prefix_len; *p;)     {       if (p[0] == '.'&& p[1] == '.'&& (ISSLASH (p[2]) || !p[2])) 	prefix_len = p + 2 - file_name;        do 	{ 	  char c = *p++; 	  if (ISSLASH (c)) 	    break; 	}       while (*p);     }    for (p = file_name + prefix_len; ISSLASH (*p); p++)     continue;   prefix_len = p - file_name;    if (prefix_len)     {       char *prefix = alloca (prefix_len + 1);       memcpy (prefix, file_name, prefix_len);       prefix[prefix_len] = '\0';         error (0, 0, _("Removing leading `%s' from member names"), prefix);     }    if (!*p)     p = ".";    return p; }
endif|#
directive|endif
end_endif
