...
name|type
parameter_list|)
value|offsetof (struct { char c; type x; }, x)
end_define
...
literal|1
expr_stmt|;
comment|/* Sums of previous lengths, properly aligned.  There's no need to      align gsum, since struct group is aligned at least as strictly as      void * (since it contains a void * member).  And there's no need      to align lsum, since struct option is aligned at least as      strictly as char.  */
name|gsum
operator|=
name|glen
expr_stmt|;
