...
name|argp_domain
argument_list|,
literal|"\ ARGP_HELP_FMT: %s value is less than or equal to %s"
argument_list|)
argument_list|,
literal|"rmargin"
argument_list|,
...
name|p
parameter_list|)
value|do { while (isspace ((unsigned char) *p)) p++; } while (0);
if|if
condition|(
name|var
condition|)
...
name|argp_domain
argument_list|,
literal|"\ %.*s: ARGP_HELP_FMT parameter must be positive"
argument_list|)
argument_list|,
operator|(
name|int
...
parameter_list|)
block|{
comment|/* If one cluster is deeper than the other, use its ancestor at the same      level, so that finding the common ancestor is straightforward.            clN->depth> 0 means that clN->parent != NULL (see hol_add_cluster) */
while|while
condition|(
name|cl1
operator|->
...
define|#
directive|define
name|HOL_ENTRY_PTRCMP
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|((a)->ord< (b)->ord ? -1 : 1)
end_define
