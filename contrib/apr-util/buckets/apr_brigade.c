...
literal|0
expr_stmt|;
comment|/* If there is a heap bucket at the end of the brigade      * already, copy into the existing bucket.      */
name|e
operator|=
name|APR_BRIGADE_LAST
argument_list|(
...
condition|)
block|{
comment|/* If there is enough space available in a heap bucket          * at the end of the brigade, copy the string directly          * into the heap bucket          */
name|apr_bucket_heap
modifier|*
name|h
init|=
...
block|}
block|}
comment|/* If the string could not be copied into an existing heap      * bucket, delegate the work to apr_brigade_write(), which      * knows how to grow the brigade      */
return|return
name|apr_brigade_write
argument_list|(
name|bb
