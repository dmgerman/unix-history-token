...
begin_comment
comment|/* The following are some math routines for numbers. */
end_comment

begin_macro
name|_PROTOTYPE
argument_list|(
argument|static int _do_compare
argument_list|,
argument|(bc_num n1, bc_num n2, int use_sign, 				    int ignore_last)
argument_list|)
end_macro
...
name|_PROTOTYPE
argument_list|(
argument|static void _rm_leading_zeros
argument_list|,
argument|(bc_num num)
argument_list|)
end_macro
...
name|_PROTOTYPE
argument_list|(
argument|static bc_num _do_add
argument_list|,
argument|(bc_num n1, bc_num n2, int scale_min)
argument_list|)
end_macro
...
name|_PROTOTYPE
argument_list|(
argument|static bc_num _do_sub
argument_list|,
argument|(bc_num n1, bc_num n2, int scale_min)
argument_list|)
end_macro
...
name|_PROTOTYPE
argument_list|(
argument|static void _one_mult
argument_list|,
argument|(unsigned char *num, int size, int digit, 				   unsigned char *result)
argument_list|)
end_macro
...
begin_comment
comment|/* For many things, we may have leading zeros in a number NUM.    _rm_leading_zeros just moves the data to the correct    place and adjusts the length. */
end_comment

begin_function
specifier|static
name|void
name|_rm_leading_zeros
parameter_list|(
name|num
parameter_list|)
name|bc_num
...
name|src
decl_stmt|;
comment|/* Do a quick check to see if we need to do it. */
if|if
condition|(
operator|*
name|num
...
condition|)
return|return;
comment|/* The first "digit" is 0, find the first non-zero digit in the second      or greater "digit" to the left of the decimal place. */
name|bytes
operator|=
name|num
operator|->
...
begin_comment
comment|/* The multiply routine.  N2 time N1 is put int PROD with the scale of    the result being MIN(N2 scale+N1 scale, MAX (SCALE, N2 scale, N1 scale)).    */
end_comment
...
name|pval
decl_stmt|;
comment|/* For the working storage. */
name|char
modifier|*
name|n1ptr
decl_stmt|,
...
begin_comment
comment|/* Debugging procedures.  Some are just so one can call them from the    debugger.  */
end_comment

begin_comment
comment|/* p_n prints the number NUM in base 10. */
end_comment
...
begin_comment
comment|/* p_b prints a character array as if it was a string of bcd digits. */
end_comment
