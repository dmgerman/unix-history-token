...
condition|)
block|{
comment|/* POS is within this xref, we're done */
name|nearest
operator|=
name|this_xref
expr_stmt|;
break|break;
block|}
comment|/* See how far POS is from this xref.  Take into account the          `*Note' that begins the xref, since as far as the user is          concerned, that's where it starts.  */
name|delta
operator|=
name|MIN
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* It's the<= instead of< that makes us choose the forward xref          of POS if two are equidistant.  Of course, because of all the          punctuation surrounding xrefs, it's not necessarily obvious          where one ends.  */
if|if
condition|(
name|delta
operator|<=
...
block|}
block|}
comment|/* Maybe there was no list to search through.  */
if|if
condition|(
name|nearest
operator|<
...
name|NULL
return|;
comment|/* Ok, we have a nearest xref, make a list of it.  */
block|{
name|REFERENCE
modifier|*
modifier|*
...
literal|0
decl_stmt|;
comment|/* For xrefs, find the closest reference to point,                    unless we only have one reference (as we will if                    we've called nearest_xref above).  It would be better                    to have only one piece of code, but the conditions                    when we call this are tangled.  */
if|if
condition|(
name|builder
operator|==
...
literal|0
condition|)
comment|/* next_line starts at line 1?  Anyway, the -1 makes it                  move to the right line.  */
name|info_next_line
argument_list|(
name|window
argument_list|,
