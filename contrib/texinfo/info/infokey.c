...
parameter_list|)
define|\
value|do { \ 		    if (slen< sizeof seq) \ 		      seq[slen++] = meta ? Meta(c) : (c); \ 		    else \ 		      { \ 			syntax_error(filename, lnum, _("key sequence too long"), \                             NULL, NULL, NULL, NULL); \ 			error = 1; \ 		      } \ 		    meta = 0; \ 		  } while (0)
name|sections
index|[
name|info
index|]
