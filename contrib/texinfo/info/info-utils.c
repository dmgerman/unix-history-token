...
begin_comment
comment|/* Variable which holds the most recent line number parsed as a result of    calling info_parse_xxx (). */
end_comment

begin_decl_stmt
name|int
name|info_parsed_line_number
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
expr_stmt|;
block|}
comment|/* Parse ``(line ...)'' part of menus, if any.  */
block|{
name|char
modifier|*
name|rest
...
name|i
decl_stmt|;
comment|/* Advance only if it's not already at end of string.  */
if|if
condition|(
operator|*
name|rest
...
operator|++
expr_stmt|;
comment|/* Skip any whitespace first, and then a newline in case the item        was so long to contain the ``(line ...)'' string in the same        physical line.  */
while|while
condition|(
name|whitespace
argument_list|(
...
expr_stmt|;
block|}
comment|/* Are we looking at an opening parenthesis?  That can only mean        we have a winner. :)  */
if|if
condition|(
name|strncmp
argument_list|(
...
begin_comment
comment|/* Copy a reference structure.  Since we tend to free everything at    every opportunity, we don't share any points, but copy everything into    new memory.  */
end_comment
