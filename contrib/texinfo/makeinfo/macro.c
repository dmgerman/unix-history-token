...
condition|)
block|{
comment|/* Remember text for future expansions.  */
name|remember_itext
argument_list|(
name|input_text
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Bizarrely, output the @macro itself.  This is so texinfo.tex          will have a chance to read it when texi2dvi calls makeinfo -E.          The problem is that we don't really expand macros in all          contexts; a @table's @item is one.  And a fix is not obvious to          me, since it appears virtually identical to any other internal          expansion.  Just setting a variable in cm_item caused other          strange expansion problems.  */
