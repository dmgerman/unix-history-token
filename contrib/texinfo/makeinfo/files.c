...
begin_comment
comment|/* files.c -- file-related functions for makeinfo.    $Id: files.c,v 1.5 2004/07/27 00:06:31 karl Exp $     Copyright (C) 1998, 1999, 2000, 2001, 2002, 2003, 2004 Free Software    Foundation, Inc.     This program is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2, or (at your option)    any later version.     This program is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with this program; if not, write to the Free Software Foundation,    Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.  */
end_comment
...
begin_comment
comment|/* Prepend and append new paths to include_files_path.  */
end_comment
...
begin_comment
comment|/* Remove the first path from the include_files_path.  */
end_comment
...
control|)
empty_stmt|;
comment|/* Advance include_files_path to the next char from ':'  */
name|tmp
operator|=
operator|(
name|char
...
begin_comment
comment|/* Find and load the file named FILENAME.  Return a pointer to    the loaded file, or NULL if it can't be loaded.  If USE_PATH is zero,    just look for the given file (this is used in handle_delayed_writes),    else search along include_files_path.   */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* VMS stat lies about the st_size value.  The actual number of      readable bytes is always less than this value.  The arcane      mysteries of VMS/RMS are too much to probe, so this hack     suffices to make things work.  It's also needed on Cygwin.  And so     we might as well use it everywhere.  */
name|bytes_to_read
operator|=
name|file_size
expr_stmt|;
...
begin_comment
comment|/* Modify the file name FNAME so that it fits the limitations of the    underlying filesystem.  In particular, truncate the file name as it    would be truncated by the filesystem.  We assume the result can    never be longer than the original, otherwise we couldn't be sure we    have enough space in the original string to modify it in place.  */
end_comment
...
name|fname
return|;
comment|/* only a directory name -- don't modify */
name|strcpy
argument_list|(
name|orig
argument_list|,
...
literal|'.'
condition|)
comment|/* leading dots are not allowed */
name|orig
index|[
literal|0
index|]
...
literal|14
case|:
comment|/* old Unix systems with 14-char limitation */
name|strcpy
argument_list|(
name|fname
operator|+
...
begin_comment
comment|/* Delayed writing functions.  A few of the commands    needs to be handled at the end, namely @contents,    @shortcontents, @printindex and @listoffloats.    These functions take care of that.  */
end_comment
...
condition|)
block|{
comment|/* Cannot register if we don't know what the output file is.  */
name|warning
argument_list|(
name|_
argument_list|(
literal|"`%s' omitted before output filename"
argument_list|)
argument_list|,
name|delayed_command
argument_list|)
...
condition|)
block|{
comment|/* Do not register a new write if the output file is not seekable.          Let the user know about it first, though.  */
name|warning
argument_list|(
name|_
argument_list|(
literal|"`%s' omitted since writing to stdout"
argument_list|)
argument_list|,
name|delayed_command
argument_list|)
...
return|return;
block|}
comment|/* Don't complain if the user is writing /dev/null, since surely they      don't care, but don't register the delayed write, either.  */
if|if
condition|(
name|FILENAME_CMP
argument_list|(
...
condition|)
return|return;
comment|/* We need the HTML header in the output,      to get a proper output_position.  */
if|if
condition|(
operator|!
name|executing_string
...
argument_list|()
expr_stmt|;
comment|/* Get output_position updated.  */
name|flush_output
argument_list|()
expr_stmt|;
name|new
...
decl_stmt|;
name|int
name|position_shift_amount
decl_stmt|,
name|line_number_shift_amount
decl_stmt|;
name|char
modifier|*
name|delayed_buf
...
name|_
argument_list|(
literal|"Output buffer not empty."
argument_list|)
argument_list|)
expr_stmt|;
return|return;
...
block|{
name|int
name|output_position_at_start
init|=
name|output_position
decl_stmt|;
name|int
name|line_number_at_start
init|=
name|output_line_number
decl_stmt|;
comment|/* In order to make warnings and errors            refer to the correct line number.  */
name|input_filename
operator|=
name|temp
operator|->
...
argument_list|()
expr_stmt|;
comment|/* Since the output file is modified, following delayed writes            need to be updated by this amount.  */
name|position_shift_amount
operator|=
name|output_position
operator|-
...
argument_list|)
expr_stmt|;
comment|/* Done with the buffer.  */
name|free
argument_list|(
name|delayed_buf
argument_list|)
expr_stmt|;
comment|/* Update positions in tag table for nodes that are defined after          the line this delayed write is registered.  */
if|if
condition|(
operator|!
name|html
...
expr_stmt|;
block|}
comment|/* Something similar for the line numbers in all of the defined          indices.  */
block|{
name|int
name|i
decl_stmt|;
...
block|}
block|}
comment|/* Shift remaining delayed positions          by the length of this write.  */
block|{
name|DELAYED_WRITE
modifier|*
name|future_write
