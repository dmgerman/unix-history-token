...
name|tag_table
expr_stmt|;
comment|/* Increment the order counter, and save it.  */
name|node_order
operator|++
expr_stmt|;
name|new
...
begin_comment
comment|/* Output the<a name="..."></a> constructs for NODE.  We output both    the new-style conversion and the old-style, if they are different.    See comments at `add_escaped_anchor_name' in html.c.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* Determine if the old and new schemes come up with different names;      only output the old scheme if that is so.  We don't want to output      the same name twice.  */
name|canon_white
argument_list|(
name|otem
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* old anchor name conversion */
name|add_word
argument_list|(
literal|"\"></a>\n"
argument_list|)
...
expr_stmt|;
block|}
comment|/* Always output the new scheme.  */
name|canon_white
argument_list|(
name|tem
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* Reset the line number in each node for Info output, so that      index entries will save the line numbers of parent node.  */
...
condition|)
block|{
comment|/* Emacs Info reader cannot grok indented escape sequence.  */
...
condition|)
block|{
comment|/* cross refs need a name="#anchor" even if not writing headers */
name|add_html_names
argument_list|(
name|node
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* In the split HTML case, the filename is wrong for the               old-style converted names, but we'll add them anyway, for              consistency.  (And we need them in the normal (not              no_headers) nonsplit case.)  */
name|add_html_names
argument_list|(
name|node
argument_list|)
...
begin_comment
comment|/* Remove previously split files, to avoid    lingering parts of shrinked documents.  */
end_comment
...
name|i
decl_stmt|;
comment|/* We break as soon as we hit an inexistent file,      so looping until large numbers is harmless.  */
for|for
control|(
name|i
operator|=
...
condition|)
block|{
comment|/* Give feedback if requested, removing a file is important.  */
if|if
condition|(
name|verbose_mode
condition|)
...
argument_list|)
expr_stmt|;
comment|/* Warn user that we cannot remove the file.  */
if|if
condition|(
name|unlink
argument_list|(
