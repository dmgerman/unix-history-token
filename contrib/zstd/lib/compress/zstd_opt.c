...
index|]
expr_stmt|;
comment|/* If opt[cur].off == ZSTD_REP_MOVE_OPT, then mlen != 1.                  * offset ZSTD_REP_MOVE_OPT is used for the special case                  * litLength == 0, where offset 0 means something special.                  * mlen == 1 means the previous byte was stored as a literal,                  * so they are mutually exclusive.                  */
name|assert
argument_list|(
operator|!
operator|(
