...
parameter_list|)
define|\
value|if (displayLevel>= l) {                                                     \     if ((clock() - g_time> refreshRate) || (displayLevel>= 4)) {             \       g_time = clock();                                                        \       DISPLAY(__VA_ARGS__);                                                    \       if (displayLevel>= 4)                                                   \         fflush(stdout);                                                        \     }                                                                          \   }
end_define
...
begin_comment
comment|/**  * Translate from COVER_params_t to ZDICT_params_t required for finalizing the  * dictionary.  */
end_comment

begin_function
specifier|static
name|ZDICT_params_t
name|COVER_translateParams
parameter_list|(
name|COVER_params_t
name|parameters
parameter_list|)
