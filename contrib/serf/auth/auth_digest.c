...
begin_comment
comment|/* Stores the context information related to Digest authentication.    The context is per connection. */
end_comment
...
expr_stmt|;
comment|/* Store the digest authentication parameters in the context relative        to this connection, so we can use it to create the Authorization header        when setting up requests. */
name|digest_info
operator|->
name|pool
operator|=
...
parameter_list|)
block|{
comment|/* Digest authentication is done per connection, so keep all progress        information per connection. */
if|if
condition|(
name|code
operator|==
