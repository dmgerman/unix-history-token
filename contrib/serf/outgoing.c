...
condition|)
block|{
comment|/* If we haven't started to write the connection, bring it over          * unchanged to our new socket.          */
if|if
condition|(
name|requeue_requests
operator|&&
...
name|next
expr_stmt|;
comment|/* assert: request != NULL || conn->vec_len */
...
condition|)
block|{
comment|/* No more requests (with data) are registered with the              * connection. Let's update the pollset so that we don't              * try to write to this socket again.              */
name|conn
operator|->
name|dirty_conn
operator|=
