...
begin_comment
comment|/*!  * \brief  *	Extract as much good data as possible, but if there isn't enough  *	at hand, return what is available.  This flag only makes sense  *	when used with _GOODONLY.  */
end_comment
...
begin_comment
comment|/*!  * \brief  *	Block the task until data is available.  This is contrary to the  *	ISC task system, where tasks should never block.  However, if  *	this is a special purpose application where blocking a task is  *	acceptable (say, an offline zone signer) this flag may be set.  *	This flag only makes sense when used with _GOODONLY, and will  *	block regardless of the setting for _PARTIAL.  */
end_comment
...
begin_comment
comment|/*  * For use with isc_entropy_usebestsource().  */
end_comment
...
begin_comment
comment|/*!  * \brief  *	Use the keyboard as an entropy source only if opening the  *	random device fails.  */
end_comment
...
begin_comment
comment|/*!<  * \brief Create a new entropy object.  */
end_comment
...
begin_comment
comment|/*!<  * \brief Detaches from an entropy object.  */
end_comment
...
begin_comment
comment|/*!<  * \brief Removes an entropy source from the entropy system.  */
end_comment
...
begin_comment
comment|/*!<  * \brief Call the stop functions for callback sources that have had their  * start functions called.  */
end_comment
...
begin_comment
comment|/*!<  * \brief Extract data from the entropy pool.  This may load the pool from various  * sources.  *  * Do this by stiring the pool and returning a part of hash as randomness.  * Note that no secrets are given away here since parts of the hash are  * xored together before returned.  *  * Honor the request from the caller to only return good data, any data,  * etc.  */
end_comment
...
begin_comment
comment|/*!<  * \brief Dump some (trivial) stats to the stdio stream "out".  */
end_comment
...
begin_comment
comment|/*  * Returns the number of bits the pool currently contains.  This is just  * an estimate.  */
end_comment
