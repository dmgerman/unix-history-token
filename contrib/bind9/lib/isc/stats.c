...
begin_comment
comment|/*%  * Local macro confirming prescence of 64-bit  * increment and store operations, just to make  * the later macros simpler  */
end_comment
...
begin_comment
comment|/*%  * Only lock the counters if 64-bit atomic operations are  * not available but cheap atomic lock operations are.  * On a modern 64-bit system this should never be the case.  *  * Normal locks are too expensive to be used whenever a counter  * is updated.  */
end_comment
...
begin_comment
comment|/*%  * If 64-bit atomic operations are not available but  * 32-bit operations are then split the counter into two,  * using the atomic operations to try to ensure that any carry  * from the low word is correctly carried into the high word.  *  * Otherwise, just rely on standard 64-bit data types  * and operations  */
end_comment
...
directive|elif
name|ISC_STATS_HAVEATOMICQ
comment|/* use xaddq(..., 0) as an atomic load */
name|stats
operator|->
name|copiedcounters
index|[
