...
literal|0
decl_stmt|;
comment|/* 	 * It's okay to delete a signature if there is an active ZSK 	 * with the same algorithm 	 */
for|for
control|(
name|i
operator|=
...
name|ENTER
expr_stmt|;
comment|/* 	 * Configuring the view of this zone may have 	 * failed, for example because the config file 	 * had a syntax error.	In that case, the view 	 * db or resolver will be NULL, and we had better not try 	 * to do maintenance on it. 	 */
if|if
condition|(
name|zone
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * For anything fewer than 1000 zones we use 10 tasks in 	 * the task pool.  More than that, and we'll scale at one 	 * task per 100 zones. 	 */
if|if
condition|(
name|ntasks
operator|<
...
literal|10
expr_stmt|;
comment|/* Create or resize the zone task pool. */
if|if
condition|(
name|zmgr
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we're doing key maintenance, set the key refresh timer to 	 * the next scheduled key event or to one hour in the future, 	 * whichever is sooner. 	 */
if|if
condition|(
name|DNS_ZONEKEY_OPTION
argument_list|(
