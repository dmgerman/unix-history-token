...
begin_comment
comment|/* Required for storage size of dns_label_t. */
end_comment
...
begin_comment
comment|/*****  ***** Labels  *****  ***** A 'label' is basically a region.  It contains one DNS wire format  ***** label of type 00 (ordinary).  *****/
end_comment
...
begin_comment
comment|/*  * Attributes below 0x0100 reserved for name.c usage.  */
end_comment
...
begin_comment
comment|/*%<  * Convert 'name' into text format, storing the result in 'target'.  *  * Notes:  *\li	If 'omit_final_dot' is true, then the final '.' in absolute  *	names other than the root name will be omitted.  *  *\li	If DNS_NAME_OMITFINALDOT is set in options, then the final '.'  *	in absolute names other than the root name will be omitted.  *  *\li	If DNS_NAME_MASTERFILE is set in options, '$' and '@' will also  *	be escaped.  *  *\li	If dns_name_countlabels == 0, the name will be "@", representing the  *	current origin as described by RFC1035.  *  *\li	The name is not NUL terminated.  *  * Requires:  *  *\li	'name' is a valid name  *  *\li	'target' is a valid buffer.  *  *\li	if dns_name_isabsolute == FALSE, then omit_final_dot == FALSE  *  * Ensures:  *  *\li	If the result is success:  *		the used space in target is updated.  *  * Returns:  *\li	#ISC_R_SUCCESS  *\li	#ISC_R_NOSPACE  */
end_comment
...
begin_comment
comment|/*%<  * Returns whether there is dynamic memory associated with this name.  *  * Requires:  *  *\li	'name' is a valid name.  *  * Returns:  *  *\li	'ISC_TRUE' if the name is dynamic otherwise 'ISC_FALSE'.  */
end_comment
...
begin_comment
comment|/*%<  * Determine if the 'name' is in the RFC 1918 reverse namespace.  */
end_comment
...
begin_comment
comment|/*%<  * Determine if the 'name' is in the ULA reverse namespace.  */
end_comment
...
begin_comment
comment|/*  *** High Performance Macros  ***/
end_comment

begin_comment
comment|/*  * WARNING:  Use of these macros by applications may require recompilation  *           of the application in some situations where calling the function  *           would not.  *  * WARNING:  No assertion checking is done for these macros.  */
end_comment
...
parameter_list|)
define|\
value|do { \ 	dns_name_t *_n = (n); \
comment|/* memset(_n, 0, sizeof(*_n)); */
value|\ 	_n->magic = DNS_NAME_MAGIC; \ 	_n->ndata = NULL; \ 	_n->length = 0; \ 	_n->labels = 0; \ 	_n->attributes = 0; \ 	_n->offsets = (o); \ 	_n->buffer = NULL; \ 	ISC_LINK_INIT(_n, link); \ 	ISC_LIST_INIT(_n->list); \ } while (0)
end_define
...
parameter_list|)
define|\
value|do { \ 	(n)->ndata = NULL; \ 	(n)->length = 0; \ 	(n)->labels = 0; \ 	(n)->attributes&= ~DNS_NAMEATTR_ABSOLUTE; \ 	if ((n)->buffer != NULL) \ 		isc_buffer_clear((n)->buffer); \ } while (0)
end_define
...
parameter_list|)
define|\
value|(((n)->attributes& DNS_NAMEATTR_ABSOLUTE) != 0 ? ISC_TRUE : ISC_FALSE)
end_define
...
parameter_list|)
define|\
value|do { \ 	(r)->base = (n)->ndata; \ 	(r)->length = (n)->length; \ } while (0)
end_define
...
parameter_list|)
define|\
value|do { \ 	dns_name_t *_n = (n); \ 	dns_name_t *_p = (p); \ 	dns_name_t *_s = (s); \ 	unsigned int _l = (l); \ 	if (_p != NULL) \ 		dns_name_getlabelsequence(_n, 0, _n->labels - _l, _p); \ 	if (_s != NULL) \ 		dns_name_getlabelsequence(_n, _n->labels - _l, _l, _s); \ } while (0)
end_define
