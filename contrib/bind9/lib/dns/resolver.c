...
name|FCTXTRACE
argument_list|(
literal|"too much NS indirection"
argument_list|)
expr_stmt|;
return|return
operator|(
...
control|)
block|{
comment|/* 				 * Only pass DNAME or RRSIG(DNAME). 				 */
if|if
condition|(
name|rdataset
operator|->
...
condition|)
continue|continue;
comment|/* 				 * If we're not chaining, then the DNAME and 				 * its signature should not be external. 				 */
if|if
condition|(
operator|!
name|chaining
...
name|fctx
argument_list|,
literal|"external DNAME or "
literal|"RRSIG covering DNAME "
literal|"in answer: %s is "
literal|"not in %s"
argument_list|,
name|qbuf
argument_list|,
