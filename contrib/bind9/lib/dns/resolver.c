...
begin_comment
comment|/* $Id: resolver.c,v 1.384.14.20.10.3 2010/06/23 23:46:25 tbox Exp $ */
end_comment
...
expr_stmt|;
block|}
comment|/* Remember that the server has been tried. */
if|if
condition|(
operator|!
name|TRIED
...
name|udpsize
expr_stmt|;
comment|/* 	 * Set EXTENDED-RCODE and Z to 0, DO to 1. 	 */
name|rdatalist
operator|->
name|ttl
operator|=
...
condition|)
block|{
comment|/* Send empty NSID option (RFC5001) */
name|unsigned
name|char
name|data
index|[
...
begin_comment
comment|/*  * Return 'bits' bits of random entropy from fctx->rand_buf,  * refreshing it by calling isc_random_get() whenever the requested  * number of bits is greater than the number in the buffer.  */
end_comment
...
condition|)
block|{
comment|/* if rand_bits == 0, this is unnecessary but harmless */
name|bits
operator|-=
name|fctx
operator|->
...
name|bits
expr_stmt|;
comment|/* refresh random buffer now */
name|isc_random_get
argument_list|(
operator|&
name|fctx
...
begin_comment
comment|/*  * Add some random jitter to a server's RTT value so that the  * order of queries will be unpredictable.  *  * RTT values of servers which have been tried are fuzzed by 128 ms.  * Servers that haven't been tried yet have their RTT set to a random  * value between 0 ms and 7 ms; they should get to go first, but in  * unpredictable order.  */
end_comment
...
literal|10
expr_stmt|;
comment|/* convert to milliseconds, near enough */
name|ai
operator|->
name|srtt
operator||=
...
literal|10
expr_stmt|;
comment|/* now back to microseconds */
block|}
else|else
name|ai
operator|->
...
begin_comment
comment|/*  * Sort addrinfo list by RTT (with random jitter)  */
end_comment
...
name|sorted
decl_stmt|;
comment|/* Add jitter to SRTT values */
name|curr
operator|=
name|ISC_LIST_HEAD
argument_list|(
...
begin_comment
comment|/*  * Sort a list of finds by server RTT (with random jitter)  */
end_comment
...
name|bestaddrinfo
decl_stmt|;
comment|/* Sort each find's addrinfo list by SRTT (after adding jitter) */
for|for
control|(
name|curr
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 		 * If this is a active validator wait for the cancel 		 * to complete before calling dns_validator_destroy(). 		 */
if|if
condition|(
name|validator
operator|==
...
name|nsid
decl_stmt|;
comment|/* Extract rdata from OPT rdataset */
name|result
operator|=
name|dns_rdataset_first
argument_list|(
...
name|ISC_TRUE
expr_stmt|;
comment|/* 			 * If this is a network error on an exclusive query 			 * socket, mark the server as bad so that we won't try 			 * it for this fetch again. 			 */
if|if
condition|(
name|query
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * Did we request NSID?  If so, and if the response contains 	 * NSID data, log it at INFO level. 	 */
name|opt
operator|=
...
literal|0
expr_stmt|;
comment|/* meaningless at this point, but init it */
name|res
operator|->
name|nbuckets
operator|=
...
literal|"timeout:%u,lame:%u,neterr:%u,badresp:%u,"
literal|"adberr:%u,findfail:%u,valfail:%u]"
argument_list|,
name|__FILE__
argument_list|,
name|fctx
