...
begin_comment
comment|/* $Id: resolver.c,v 1.218.2.18.4.43 2004/08/28 06:25:19 marka Exp $ */
end_comment
...
block|{
comment|/* 			 * We're in forward-only mode.  Set the query domain 			 * to ".". 			 */
name|result
operator|=
name|dns_name_dup
argument_list|(
...
condition|)
block|{
comment|/* 			 * Look for NS RRset first. 			 */
for|for
control|(
name|rdataset
operator|=
...
condition|)
block|{
comment|/* 					 * NS or SIG NS. 					 * 					 * Only one set of NS RRs is allowed. 					 */
if|if
condition|(
name|rdataset
operator|->
...
condition|)
block|{
comment|/* 					 * SOA, RRSIG SOA, NSEC, or RRSIG NSEC. 					 * 					 * Only one SOA is allowed. 					 */
if|if
condition|(
name|rdataset
operator|->
