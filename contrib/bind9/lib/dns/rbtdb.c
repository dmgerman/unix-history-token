...
begin_comment
comment|/* $Id: rbtdb.c,v 1.270.12.6.10.2 2009/12/31 21:44:36 each Exp $ */
end_comment
...
name|lru_link
expr_stmt|;
comment|/*%< 	 * Used for LRU-based cache management.  We should probably make 	 * these cache-DB specific.  We might also make it a pointer and 	 * ensure only the top header has a valid link to save memory. 	 * The linked-list is locked by the rbtdb->lrulock. 	 */
comment|/* 	 * It's possible this should not be here anymore, but instead 	 * referenced from the bucket's heap directly. 	 */
if|#
directive|if
literal|0
block|isc_heap_t                      *heap;
endif|#
directive|endif
name|unsigned
name|int
