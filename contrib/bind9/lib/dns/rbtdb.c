...
expr_stmt|;
block|}
comment|/* 	 * Certain DNSSEC types are not subject to CNAME matching 	 * (RFC 2535, section 2.3.5). 	 * 	 * We don't check for RRSIG, because we don't store RRSIG records 	 * directly. 	 */
if|if
condition|(
name|type
operator|==
...
block|{
comment|/* 			 * Look for active extant "other data". 			 * 			 * "Other data" is any rdataset whose type is not 			 * DNSKEY, RRSIG DNSKEY, NSEC, RRSIG NSEC, 			 * or RRSIG CNAME. 			 */
name|rdtype
operator|=
name|RBTDB_RDATATYPE_BASE
argument_list|(
...
condition|)
block|{
comment|/* 				 * We're adding an NXDOMAIN negative cache 				 * entry. 				 * 				 * We make all other data stale so that the 				 * only rdataset that can be found at this 				 * node is the NXDOMAIN negative cache entry. 				 */
for|for
control|(
name|topheader
operator|=
...
block|{
comment|/* 			 * We're adding something that isn't a 			 * negative cache entry.  Look for an extant 			 * non-stale NXDOMAIN negative cache entry. 			 */
for|for
control|(
name|topheader
operator|=
...
condition|)
block|{
comment|/* 					 * The NXDOMAIN is more trusted. 					 */
name|free_rdataset
argument_list|(
name|rbtdb
operator|->
...
return|;
block|}
comment|/* 				 * The new rdataset is better.  Expire the 				 * NXDOMAIN. 				 */
name|topheader
operator|->
name|ttl
operator|=
