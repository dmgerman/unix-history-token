...
begin_comment
comment|/* $Id: rbtdb.c,v 1.310.8.1.2.1 2011-06-21 20:15:48 each Exp $ */
end_comment
...
condition|)
continue|continue;
comment|/* 		 * Choose the policy with the longest matching prefix. 		 * Between policies with the same prefix, choose the first 		 * configured. 		 */
if|if
condition|(
name|st
operator|->
...
name|policy
expr_stmt|;
comment|/* 		 * We know the new prefix is at least as long as the current. 		 * Prefer the new answer if the new prefix is longer. 		 * Prefer the zone configured first if the prefixes are equal. 		 * With two actions from the same zone, prefer the action 		 * on the "smallest" name. 		 */
if|if
condition|(
name|st
operator|->
