...
parameter_list|)
define|\
value|(((header)->attributes& RDATASET_ATTR_PREFETCH) != 0)
end_define
...
parameter_list|)
define|\
value|(((header)->attributes& RDATASET_ATTR_CASESET) != 0)
end_define
...
parameter_list|)
define|\
value|(((header)->attributes& RDATASET_ATTR_ZEROTTL) != 0)
end_define
...
parameter_list|)
define|\
value|(((header)->rdh_ttl> (now)) || \ 	 ((header)->rdh_ttl == (now)&& ZEROTTL(header)))
end_define
...
argument_list|)
argument_list|,
literal|"delete_node(): %p %s (bucket %d)"
argument_list|,
name|node
argument_list|,
...
goto|;
block|}
comment|/* 	 * Update the zone's secure status in version before making 	 * it the current version. 	 */
if|if
condition|(
name|version
operator|->
