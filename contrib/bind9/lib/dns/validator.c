...
begin_comment
comment|/* $Id: validator.c,v 1.164.12.11.8.3 2010/04/21 04:29:01 marka Exp $ */
end_comment
...
directive|if
literal|0
comment|/* 			 * The closest encloser may be the zone name. 			 */
block|if (closest != NULL&& 			    dns_name_countlabels(closest) == 0&& 			    !dns_nsec3_typepresent(&rdata, dns_rdatatype_ds)&& 			    !dns_nsec3_typepresent(&rdata, dns_rdatatype_dname)&& 			    (dns_nsec3_typepresent(&rdata, dns_rdatatype_soa) || 			     !dns_nsec3_typepresent(&rdata, dns_rdatatype_ns))) 			{ 				char namebuf[DNS_NAME_FORMATSIZE];  				dns_name_format(zone, namebuf, 						sizeof(namebuf)); 				validator_log(val, ISC_LOG_DEBUG(3), 					      "NSEC3 potential closest " 					       "encloser from zone name: '%s'", 					       namebuf); 				dns_name_copy(zone, closest, NULL); 				*setclosest = ISC_TRUE; 			}
endif|#
directive|endif
operator|*
name|exists
...
return|;
block|}
comment|/* 	if ((val->attributes& VALATTR_NEEDOPTOUT) == 0&& 	    val->event->message != NULL) { 		val->attributes |= VALATTR_NEEDOPTOUT; 		return (nsecvalidate(val, ISC_FALSE)); 	} */
name|validator_log
argument_list|(
name|val
argument_list|,
