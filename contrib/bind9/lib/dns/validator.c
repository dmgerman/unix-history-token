...
name|val
parameter_list|)
value|((val->attributes& VALATTR_FOUNDNODATA) != 0)
end_define
...
name|val
parameter_list|)
value|((val->attributes& VALATTR_FOUNDNOQNAME) != 0)
end_define

begin_define
define|#
directive|define
name|FOUNDNOWILDCARD
parameter_list|(
name|val
parameter_list|)
value|((val->attributes& VALATTR_FOUNDNOWILDCARD) != 0)
end_define
...
name|val
parameter_list|)
value|((val->attributes& VALATTR_FOUNDCLOSEST) != 0)
end_define
...
name|val
parameter_list|)
value|((val->attributes& VALATTR_FOUNDOPTOUT) != 0)
end_define
...
begin_comment
comment|/*%  * Mark the RRsets as a answer.  */
end_comment
...
begin_comment
comment|/*  * Check that we have atleast one supported algorithm in the DLV RRset.  */
end_comment
...
name|trynsec3
label|:
comment|/* 	 * Iterate over the ncache entry. 	 */
name|found
operator|=
name|ISC_FALSE
expr_stmt|;
...
condition|)
continue|continue;
comment|/* 			 * Does this optout span cover the name? 			 */
name|scope
operator|=
name|memcmp
argument_list|(
...
begin_comment
comment|/*%  * We have been asked to look for a key.  * If found resume the validation process.  * If not found fail the validation process.  */
end_comment
...
argument_list|,
literal|"must be secure failure, no DS"
literal|" and this is a delegation"
argument_list|)
expr_stmt|;
name|validator_done
argument_list|(
...
begin_comment
comment|/*%  * Callback when the CNAME record has been validated.  *  * Resumes validation of the unsecure zone proof.  */
end_comment
...
argument_list|)
argument_list|,
literal|"NSEC3 indicates potential "
literal|"closest encloser: '%s'"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*%  * Callback for when NSEC records have been validated.  *  * Looks for NOQNAME, NODATA and OPTOUT proofs.  *  * Resumes nsecvalidate.  */
end_comment
...
begin_comment
comment|/*%  * Looks for the requested name and type in the view (zones and cache).  *  * When looking for a DLV record also checks to make sure the NSEC record  * returns covers the query name as part of aggressive negative caching.  *  * Returns:  * \li	ISC_R_SUCCESS  * \li	ISC_R_NOTFOUND  * \li	DNS_R_NCACHENXDOMAIN  * \li	DNS_R_NCACHENXRRSET  * \li	DNS_R_NXRRSET  * \li	DNS_R_NXDOMAIN  * \li	DNS_R_BROKENCHAIN  */
end_comment
...
argument_list|)
operator|&&
comment|/* 		     * As NSEC3 records are meta data you sometimes 		     * need to prove a NSEC3 record which says that 		     * itself doesn't exist. 		     */
operator|(
name|parent
operator|->
name|event
...
argument_list|)
argument_list|,
literal|"deadlock found (create_fetch)"
argument_list|)
expr_stmt|;
return|return
operator|(
...
argument_list|)
argument_list|,
literal|"deadlock found (create_validator)"
argument_list|)
expr_stmt|;
return|return
operator|(
...
condition|)
block|{
comment|/* 			 * We know the key but haven't validated it yet or 			 * we have a key of trust answer but a DS/DLV 			 * record for the zone may have been added. 			 */
name|result
operator|=
name|create_validator
argument_list|(
...
name|ISC_LOG_INFO
argument_list|,
literal|"accepted expired %sRRSIG (keyid=%u)"
argument_list|,
operator|(
name|result
operator|==
...
name|ISC_LOG_INFO
argument_list|,
literal|"verify failed due to bad signature (keyid=%u): "
literal|"%s"
argument_list|,
name|keyid
argument_list|,
...
name|labels
decl_stmt|;
comment|/* 			 * Compute the closest encloser in case we need it 			 * for the NSEC3 NOQNAME proof. 			 */
name|closest
operator|=
name|dns_fixedname_name
argument_list|(
...
return|;
block|}
comment|/* 		 * There isn't a secure DNSKEY for this signature so move 		 * onto the next RRSIG. 		 */
if|if
condition|(
name|val
operator|->
...
literal|"noqname proof not needed"
argument_list|)
expr_stmt|;
return|return
operator|(
...
begin_comment
comment|/*%  * Check whether this DNSKEY (keyrdata) signed the DNSKEY RRset  * (val->event->rdataset).  */
end_comment
...
begin_comment
comment|/*%  * Find the DNSKEY that corresponds to the DS.  */
end_comment
...
name|ISC_FALSE
expr_stmt|;
comment|/* 	 * If DNS_DSDIGEST_SHA256 is present we are required to prefer 	 * it over DNS_DSDIGEST_SHA1.  This in practice means that we 	 * need to ignore DNS_DSDIGEST_SHA1 if a DNS_DSDIGEST_SHA256 	 * is present. 	 */
name|memset
argument_list|(
name|digest_types
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 		 * Convert to DLV to DS and find matching DNSKEY. 		 */
name|dlvrdata
operator|.
name|type
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 		 * Check that this DNSKEY signed the DNSKEY rrset. 		 */
name|result
operator|=
name|checkkey
argument_list|(
...
begin_comment
comment|/*%  * Attempts positive response validation of an RRset containing zone keys  * (i.e. a DNSKEY rrset).  *  * Returns:  * \li	ISC_R_SUCCESS	Validation completed successfully  * \li	DNS_R_WAIT	Validation has started but is waiting  *			for an event.  * \li	Other return codes are possible and all indicate failure.  */
end_comment
...
condition|)
block|{
comment|/* 		 * We have a dlv sep.  Skip looking up the SEP from 		 * {trusted,managed}-keys.  If the dlv sep is for the 		 * root then it will have been handled above so we don't 		 * need to check whether val->event->name is "." prior to 		 * looking up the DS. 		 */
if|if
condition|(
name|val
operator|->
...
literal|"the DNSKEY RRset and also matches a "
literal|"trusted key for '%s'"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
name|ISC_LOG_NOTICE
argument_list|,
literal|"please check the 'trusted-keys' for "
literal|"'%s' in named.conf."
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
argument_list|)
argument_list|,
literal|"root key failed to validate"
argument_list|)
expr_stmt|;
return|return
operator|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Find matching DNSKEY from DS. 		 */
name|result
operator|=
name|keyfromds
argument_list|(
...
name|ISC_LOG_INFO
argument_list|,
literal|"no valid signature found (DS)"
argument_list|)
expr_stmt|;
return|return
operator|(
...
begin_comment
comment|/*%  * val_rdataset_first and val_rdataset_next provide iteration methods  * that hide whether we are iterating across a message or a  negative  * cache rdataset.  */
end_comment

begin_function
specifier|static
name|isc_result_t
name|val_rdataset_first
parameter_list|(
name|dns_validator_t
modifier|*
name|val
...
argument_list|)
argument_list|,
literal|"in checkwildcard: no wildcard to check"
argument_list|)
expr_stmt|;
return|return
operator|(
...
operator|)
return|;
comment|/* 	 * If the val->closest is set then we want to use it otherwise 	 * we need to discover it. 	 */
if|if
condition|(
name|dns_name_countlabels
argument_list|(
...
argument_list|)
argument_list|,
literal|"closest encloser from "
literal|"wildcard signature '%s'"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
condition|)
continue|continue;
comment|/* 		 * We process all NSEC3 records to find the closest 		 * encloser and nearest name to the closest encloser. 		 */
name|setclosest
operator|=
name|setnearest
operator|=
...
name|ISC_R_SUCCESS
expr_stmt|;
comment|/* 	 * To know we have a valid noqname and optout proofs we need to also 	 * have a valid closest encloser.  Otherwise we could still be looking 	 * at proofs from the parent zone. 	 */
if|if
condition|(
name|dns_name_countlabels
argument_list|(
...
begin_comment
comment|/*%  * Validate the authority section records.  */
end_comment
...
begin_comment
comment|/*%  * Validate the ncache elements.  */
end_comment

begin_function
specifier|static
name|isc_result_t
name|validate_ncache
parameter_list|(
name|dns_validator_t
modifier|*
name|val
...
argument_list|)
argument_list|,
literal|"marking as secure, noqname proof found"
argument_list|)
expr_stmt|;
name|marksecure
argument_list|(
...
argument_list|)
argument_list|,
literal|"unknown NSEC3 hash algorithm found"
argument_list|)
expr_stmt|;
name|markanswer
argument_list|(
...
argument_list|)
argument_list|,
literal|"DLV %s found with no supported algorithms"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
argument_list|)
argument_list|,
literal|"DLV %s found with no supported "
literal|"algorithms"
argument_list|,
name|namebuf
argument_list|)
...
argument_list|,
literal|"must be secure failure, "
literal|" %s is under DLV (startfinddlvsep)"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
argument_list|,
literal|"must be secure failure, "
literal|"%s is under DLV (finddlvsep)"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 		 * If we have a DS rdataset and it is secure then check if 		 * the DS rdataset has a supported algorithm combination. 		 * If not this is an insecure delegation as far as this 		 * resolver is concerned.  Fall back to DLV if available. 		 */
if|if
condition|(
name|have_ds
operator|&&
...
literal|"can't fall back to DLV"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* 			 * There is no DS.  If this is a delegation, 			 * we may be done. 			 */
comment|/* 			 * If we have "trust == answer" then this namespace 			 * has switched from insecure to should be secure. 			 */
if|if
condition|(
name|DNS_TRUST_PENDING
argument_list|(
...
return|;
block|}
comment|/* 			 * Zones using NSEC3 don't return a NSEC RRset so 			 * we need to use dns_view_findzonecut2 to find 			 * the zone cut. 			 */
if|if
condition|(
name|result
operator|==
...
name|ISC_LOG_WARNING
argument_list|,
literal|"can't validate existing "
literal|"negative responses (no DS)"
argument_list|)
expr_stmt|;
name|result
operator|=
...
goto|;
block|}
comment|/* 			 * Validate / re-validate answer. 			 */
name|result
operator|=
name|create_validator
argument_list|(
...
argument_list|,
literal|"can't validate existing "
literal|"negative responses "
literal|"(not a zone cut)"
argument_list|)
expr_stmt|;
name|result
operator|=
...
name|ISC_LOG_INFO
argument_list|,
literal|"got insecure response; "
literal|"parent indicates it should be secure"
argument_list|)
expr_stmt|;
block|}
elseif|else
