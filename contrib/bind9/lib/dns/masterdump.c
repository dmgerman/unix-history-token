...
begin_comment
comment|/*  * Copyright (C) 2004-2006, 2008, 2009, 2011  Internet Systems Consortium, Inc. ("ISC")  * Copyright (C) 1999-2003  Internet Software Consortium.  *  * Permission to use, copy, modify, and/or distribute this software for any  * purpose with or without fee is hereby granted, provided that the above  * copyright notice and this permission notice appear in all copies.  *  * THE SOFTWARE IS PROVIDED "AS IS" AND ISC DISCLAIMS ALL WARRANTIES WITH  * REGARD TO THIS SOFTWARE INCLUDING ALL IMPLIED WARRANTIES OF MERCHANTABILITY  * AND FITNESS.  IN NO EVENT SHALL ISC BE LIABLE FOR ANY SPECIAL, DIRECT,  * INDIRECT, OR CONSEQUENTIAL DAMAGES OR ANY DAMAGES WHATSOEVER RESULTING FROM  * LOSS OF USE, DATA OR PROFITS, WHETHER IN AN ACTION OF CONTRACT, NEGLIGENCE  * OR OTHER TORTIOUS ACTION, ARISING OUT OF OR IN CONNECTION WITH THE USE OR  * PERFORMANCE OF THIS SOFTWARE.  */
end_comment

begin_comment
comment|/* $Id: masterdump.c,v 1.73.18.23 2011-06-09 00:42:47 each Exp $ */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 		 * Do not return ISC_R_NOSPACE if the line break string 		 * buffer is too small, because that would just make 		 * dump_rdataset() retry indefinitely with ever 		 * bigger target buffers.  That's a different buffer, 		 * so it won't help.  Use DNS_R_TEXTTOOLONG as a substitute. 		 */
if|if
condition|(
name|result
operator|==
...
begin_comment
comment|/*  * Dump given RRsets in the "raw" format.  */
end_comment

begin_function
specifier|static
name|isc_result_t
name|dump_rdataset_raw
parameter_list|(
name|isc_mem_t
modifier|*
name|mctx
...
argument_list|)
expr_stmt|;
comment|/* 	 * Common header and owner name (length followed by name) 	 * These fields should be in a moderate length, so we assume we 	 * can store all of them in the initial buffer. 	 */
name|isc_buffer_availableregion
argument_list|(
name|buffer
argument_list|,
...
name|length
expr_stmt|;
comment|/* 		 * Copy the rdata into the buffer.  If the buffer is too small, 		 * grow it.  This should be rare, so we'll simply restart the 		 * entire procedure (or should we copy the old data and 		 * continue?). 		 */
if|if
condition|(
name|isc_buffer_availablelength
argument_list|(
...
operator|)
return|;
comment|/* 	 * Fill in the total length field. 	 * XXX: this is a bit tricky.  Since we have already "used" the space 	 * for the total length in the buffer, we first remember the entire 	 * buffer length in the region, "rewind", and then write the value. 	 */
name|isc_buffer_usedregion
argument_list|(
name|buffer
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * Write the buffer contents to the raw master file. 	 */
name|result
operator|=
name|isc_stdio_write
argument_list|(
...
name|__LINE__
argument_list|,
literal|"raw master file write failed: %s"
argument_list|,
name|isc_result_totext
argument_list|(
name|result
...
name|ISC_LOG_INFO
argument_list|,
literal|"dumping master file in raw "
literal|"format: stdtime is not 32bits"
argument_list|)
expr_stmt|;
name|now32
operator|=
...
expr_stmt|;
block|}
comment|/* 	 * Work out how many nodes can be written in the time between 	 * two requests to the nameserver.  Smooth the resulting number and 	 * use it as a estimate for the number of nodes to be written in the 	 * next iteration. 	 */
if|if
condition|(
name|dctx
operator|->
...
argument_list|)
argument_list|,
literal|"dumptostreaminc(%p) new nodes -> %d\n"
argument_list|,
name|dctx
argument_list|,
name|dctx
...
begin_comment
comment|/*  * Dump a database node into a master file.  * XXX: this function assumes the text format.  */
end_comment
