...
begin_comment
comment|/* $Id: cache.c,v 1.91 2011/08/26 05:12:56 marka Exp $ */
end_comment
...
begin_comment
comment|/*  * A cache_cleaner_t encapsulates the state of the periodic  * cache cleaning.  */
end_comment
...
goto|;
block|}
comment|/* 	 * For databases of type "rbt" we pass hmctx to dns_db_create() 	 * via cache->db_argv, followed by the rest of the arguments in 	 * db_argv (of which there really shouldn't be any). 	 */
if|if
condition|(
name|strcmp
argument_list|(
...
name|CACHE_MAGIC
expr_stmt|;
comment|/* 	 * RBT-type cache DB has its own mechanism of cache cleaning and doesn't 	 * need the control of the generic cleaner. 	 */
if|if
condition|(
name|strcmp
argument_list|(
...
condition|)
block|{
comment|/* 		 * We don't free db_argv[0] in "rbt" cache databases 		 * as it's a pointer to hmctx 		 */
name|int
name|extra
init|=
literal|0
...
argument_list|)
expr_stmt|;
comment|/* 	 * Impose a minimum cache size; pathological things happen if there 	 * is too little room. 	 */
if|if
condition|(
name|size
operator|!=
...
name|cleanup
goto|;
comment|/* 		 * If clearnode fails record and move onto the next node. 		 */
name|result
operator|=
name|clearnode
argument_list|(
