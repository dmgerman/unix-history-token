...
begin_comment
comment|/* $Id: cache.c,v 1.45.2.4.8.9 2005/03/17 03:58:30 marka Exp $ */
end_comment
...
name|c
parameter_list|)
value|((c)->state == cleaner_s_idle&& \ 			 (c)->iterator == NULL&& \ 			 (c)->resched_event != NULL)
end_define
...
argument_list|)
expr_stmt|;
comment|/* 	 * Create an iterator and position it at the beginning of the cache. 	 */
name|result
operator|=
...
condition|)
block|{
comment|/* 			 * Either the end was reached (ISC_R_NOMORE) or 			 * some error was signaled.  If the cache is still 			 * overmem and no error was encountered, 			 * keep trying to clean it, otherwise stop cleanng. 			 */
if|if
condition|(
name|result
operator|!=
