...
argument_list|)
expr_stmt|;
comment|/* 	 * If there are no rdata then we can just need to allocate a header 	 * with zero a record count. 	 */
if|if
condition|(
name|nitems
operator|==
...
operator|)
return|;
comment|/* 	 * Remember the original number of items. 	 */
name|nalloc
operator|=
name|nitems
expr_stmt|;
...
condition|)
block|{
comment|/* 		 * Somehow we iterated over fewer rdatas than 		 * dns_rdataset_count() said there were or there 		 * were more items than dns_rdataset_count said 		 * there were. 		 */
name|result
operator|=
name|ISC_R_FAILURE
expr_stmt|;
