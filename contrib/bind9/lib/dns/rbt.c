...
begin_comment
comment|/*  * Copyright (C) 2004, 2005, 2007-2009, 2011-2016  Internet Systems Consortium, Inc. ("ISC")  * Copyright (C) 1999-2003  Internet Software Consortium.  *  * Permission to use, copy, modify, and/or distribute this software for any  * purpose with or without fee is hereby granted, provided that the above  * copyright notice and this permission notice appear in all copies.  *  * THE SOFTWARE IS PROVIDED "AS IS" AND ISC DISCLAIMS ALL WARRANTIES WITH  * REGARD TO THIS SOFTWARE INCLUDING ALL IMPLIED WARRANTIES OF MERCHANTABILITY  * AND FITNESS.  IN NO EVENT SHALL ISC BE LIABLE FOR ANY SPECIAL, DIRECT,  * INDIRECT, OR CONSEQUENTIAL DAMAGES OR ANY DAMAGES WHATSOEVER RESULTING FROM  * LOSS OF USE, DATA OR PROFITS, WHETHER IN AN ACTION OF CONTRACT, NEGLIGENCE  * OR OTHER TORTIOUS ACTION, ARISING OUT OF OR IN CONNECTION WITH THE USE OR  * PERFORMANCE OF THIS SOFTWARE.  */
end_comment
...
begin_comment
comment|/*%  * Chain management.  *  * The "ancestors" member of chains were removed, with their job now  * being wholly handled by parent pointers (which didn't exist, because  * of memory concerns, when chains were first implemented).  */
end_comment
...
begin_comment
comment|/*  * Upper node is the parent of the root of the passed node's  * subtree. The passed node must not be NULL.  */
end_comment
...
begin_comment
comment|/* The passed node must not be NULL. */
end_comment
...
name|hash
decl_stmt|;
comment|/* 			 * The case of current not being a subtree root, 			 * that means a left or right pointer was 			 * followed, only happens when the algorithm 			 * fell through to the traditional binary search 			 * because of a bitstring label.  Since we 			 * dropped the bitstring support, this should 			 * not happen. 			 */
name|INSIST
argument_list|(
name|IS_ROOT
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 			 * current is the root of the current level, so 			 * its parent is the same as its "up" pointer. 			 */
name|up_current
operator|=
name|PARENT
argument_list|(
...
condition|)
continue|continue;
comment|/* 				 * This checks that the hashed label 				 * sequence being looked up is at the 				 * same tree level, so that we don't 				 * match a labelsequence from some other 				 * subdomain. 				 */
if|if
condition|(
name|ISC_LIKELY
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * This node now has no down pointer, so now it needs 	 * to be removed from this level. 	 */
name|deletefromlevel
argument_list|(
name|node
argument_list|,
...
block|{
comment|/* 				 * Child is parent's right child. 				 * Everything is done the same as above, 				 * except mirrored. 				 */
name|sibling
operator|=
name|LEFT
argument_list|(
...
condition|)
block|{
comment|/* 		 * If there is a left, right or down node, walk into it 		 * and iterate. 		 */
if|if
condition|(
name|LEFT
argument_list|(
...
block|{
comment|/* 			 * There are no left, right or down nodes, so we 			 * can free this one and go back to its parent. 			 */
name|dns_rbtnode_t
modifier|*
name|node
init|=
...
condition|)
block|{
comment|/* 		 * The successor is up, either in this level or a previous one. 		 * Head back toward the root of the tree, looking for any path 		 * that was via a left link; the successor is the node that has 		 * that left link.  In the event the root of the level is 		 * reached without having traversed any left links, ascend one 		 * level and look for either a right link off the point of 		 * ascent, or search for a left link upward again, repeating 		 * ascends until either case is true. 		 */
do|do
block|{
while|while
condition|(
