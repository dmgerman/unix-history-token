...
name|x
parameter_list|)
value|(((x)->attributes& DNS_RDATASETATTR_OPTOUT) != 0)
end_define
...
name|DO_FORMERR
expr_stmt|;
comment|/* 		 * If this is not a TKEY query/response then the KEY 		 * record's class needs to match. 		 */
if|if
condition|(
name|msg
operator|->
...
expr_stmt|;
block|}
comment|/* Append this rdata to the rdataset. */
name|dns_rdatalist_fromrdataset
argument_list|(
name|rdataset
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 		 * If this is an OPT, SIG(0) or TSIG record, remember it. 		 * Also, set the extended rcode for TSIG. 		 * 		 * Note msg->opt, msg->sig0 and msg->tsig will only be 		 * already set if best-effort parsing is enabled otherwise 		 * there will only be at most one of each. 		 */
if|if
condition|(
name|rdtype
operator|==
...
begin_comment
comment|/*  * Decide whether to not answer with an AAAA record and its RRSIG  */
end_comment
...
directive|ifdef
name|ALLOW_FILTER_AAAA_ON_V4
comment|/* 				 * Suppress AAAAs if asked and we are 				 * not doing DNSSEC or are breaking DNSSEC. 				 * Say so in the AD bit if we break DNSSEC. 				 */
if|if
condition|(
name|norender_rdataset
argument_list|(
...
return|;
block|}
comment|/* 	 * If we're adding a OPT, TSIG or SIG(0) to a truncated message, 	 * clear all rdatasets from the message except for the question 	 * before adding the OPT, TSIG or SIG(0).  If the question doesn't 	 * fit, don't include it. 	 */
if|if
condition|(
operator|(
name|msg
...
name|msg
argument_list|,
literal|"dns_message_checksig#1"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
operator|)
return|;
comment|/* Print EDNS info, if any */
name|dns_rdata_init
argument_list|(
operator|&
name|rdata
...
continue|continue;
block|}
comment|/* 				 * For non-COOKIE options, add a printable 				 * version 				 */
for|for
control|(
name|i
operator|=
...
name|udpsize
expr_stmt|;
comment|/* 	 * Set EXTENDED-RCODE and Z to 0. 	 */
name|rdatalist
operator|->
name|ttl
operator|=
