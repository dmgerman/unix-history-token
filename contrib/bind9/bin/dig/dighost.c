...
name|fatal
argument_list|(
literal|"couldn't parse digest bits"
argument_list|)
expr_stmt|;
name|tmp
operator|=
...
begin_comment
comment|/*  * Parse HMAC algorithm specification  */
end_comment
...
begin_comment
comment|/*%  * Add EDNS0 option record to a message.  Currently, the only supported  * options are UDP buffer size, the DO bit, and NSID request.  */
end_comment
...
name|debug
argument_list|(
literal|"query to %s still connecting"
argument_list|,
name|q
operator|->
name|servname
...
index|]
expr_stmt|;
comment|/* 				 * Verify temp is really the lowest 				 * WARNING 				 */
block|}
if|if
condition|(
name|dstkey
...
operator|)
return|;
comment|/* 	 * Check for a absolute name or ndots being met. 	 */
name|dns_fixedname_init
argument_list|(
operator|&
name|fixed
...
condition|)
block|{
comment|/* 		 * Force TCP mode if we're doing an axfr. 		 */
if|if
condition|(
name|lookup
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * The timer may have expired if, for example, get_address() takes 	 * long time and the timer was running on a different thread. 	 * We need to cancel the possible timeout event not to confuse 	 * ourselves due to the duplicate events. 	 */
if|if
condition|(
name|l
operator|->
...
condition|)
block|{
comment|/* 		 * This servname doesn't have an address.  Try the next server 		 * by triggering an immediate 'timeout' (we lie, but the effect 		 * is the same). 		 */
name|force_timeout
argument_list|(
name|l
argument_list|,
...
condition|)
block|{
comment|/* This servname doesn't have an address. */
name|force_timeout
argument_list|(
name|l
argument_list|,
...
name|debug
argument_list|(
literal|"this is the first serial %u"
argument_list|,
name|serial
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"this is the second serial %u"
argument_list|,
name|serial
argument_list|)
expr_stmt|;
...
name|printf
argument_list|(
literal|";; Question section mismatch: "
literal|"got %s/%s/%s\n"
argument_list|,
name|namestr
argument_list|,
...
name|debug
argument_list|(
literal|"canceling pending query %p, belonging to %p"
argument_list|,
name|q
argument_list|,
name|current_lookup
...
name|debug
argument_list|(
literal|"canceling connecting query %p, belonging to %p"
argument_list|,
name|q
argument_list|,
name|current_lookup
...
name|result
argument_list|,
literal|"dns_rcode_totext failed"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"error response code %.*s\n"
argument_list|,
operator|(
name|int
operator|)
...
name|printf
argument_list|(
literal|"no answer or authority section\n"
argument_list|)
expr_stmt|;
name|error_message
operator|=
...
name|printf
argument_list|(
literal|"We want to prove the non-existence of a type of rdata %d"
