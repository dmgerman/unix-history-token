...
argument_list|)
expr_stmt|;
comment|/* 	 * The isc_mem_put of master_servers must be before the 	 * isc_mem_put of servers as it sets the servers pointer 	 * to NULL. 	 */
if|if
condition|(
name|master_servers
operator|!=
...
name|fatal
argument_list|(
literal|"isc_string_printf(mykeystr) failed: %s"
argument_list|,
name|isc_result_totext
argument_list|(
name|result
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we have both the zone and the servers we have enough information 	 * to send the update straight away otherwise we need to discover 	 * the zone and / or the master server. 	 */
if|if
condition|(
name|userzone
operator|!=
