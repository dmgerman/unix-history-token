...
begin_comment
comment|/* $Id: dnssec-signzone.c,v 1.209.12.18 2009/11/03 23:47:45 tbox Exp $ */
end_comment
...
begin_comment
comment|/*%  * Find the key if it is in our list.  If it is, return it, otherwise null.  * No locking is performed here, this must be done by the caller.  */
end_comment
...
begin_comment
comment|/*%  * Verify that certain things are sane:  *  *   The apex has a DNSKEY record with at least one KSK and at least  *   one ZSK.  *  *   The DNSKEY record was signed with at least one of the KSKs in this  *   set.  *  *   The rest of the zone was signed with at least one of the ZSKs  *   present in the DNSKEY RRSET.  */
end_comment
...
directive|endif
comment|/* 	 * Check that the DNSKEY RR has at least one self signing KSK and 	 * one ZSK per algorithm in it. 	 */
for|for
control|(
name|result
operator|=
...
name|fatal
argument_list|(
literal|"no self signing keys found"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"No self signing KSK found. Using self signed "
literal|"ZSK's for active algorithm list.\n"
argument_list|)
expr_stmt|;
name|memcpy
argument_list|(
...
name|stderr
argument_list|,
literal|"Algorithm: %s: ZSKs: %u, "
literal|"KSKs: %u active, %u revoked, %u "
literal|"stand-by\n"
argument_list|,
name|algbuf
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * Delete any NSEC3PARAM records at the apex. 	 */
name|result
operator|=
name|dns_db_allrdatasets
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Delete any NSEC records at the apex. 	 */
name|result
operator|=
name|dns_db_allrdatasets
argument_list|(
...
name|stderr
argument_list|,
literal|"\t\tRRSIG start time - absolute|offset "
literal|"(now - 1 hour)\n"
argument_list|)
expr_stmt|;
name|fprintf
...
name|stderr
argument_list|,
literal|"\t\trandomize signature end time up to jitter "
literal|"seconds\n"
argument_list|)
expr_stmt|;
name|fprintf
...
directive|define
name|CMDLINE_FLAGS
value|"3:aAc:d:e:f:FghH:i:I:j:k:l:m:n:N:o:O:pPr:s:StUv:z"
