...
name|pat
operator|=
literal|"response policy %s rewrite %s NO-OP using %s"
expr_stmt|;
break|break;
case|case
...
name|pat
operator|=
literal|"response policy %s rewrite %s to NXDOMAIN using %s"
expr_stmt|;
break|break;
case|case
...
name|pat
operator|=
literal|"response policy %s rewrite %s to NODATA using %s"
expr_stmt|;
break|break;
case|case
...
name|pat
operator|=
literal|"response policy %s rewrite %s using %s"
expr_stmt|;
break|break;
default|default:
name|INSIST
...
name|level
argument_list|,
literal|"response policy %s rewrite %s via %s %sfailed: %s"
argument_list|,
name|dns_rpz_type2str
argument_list|(
name|rpz_type
...
condition|)
return|return;
comment|/* 	 * Bound the validated ttls then minimise. 	 */
if|if
condition|(
name|sigrdataset
operator|->
...
begin_comment
comment|/*  * Get NS, A, or AAAA rrset for rpz nsdname or nsip checking.  */
end_comment
...
condition|)
block|{
comment|/* 		 * Recurse to get NS rrset or A or AAAA rrset for an NS name. 		 */
name|rpz_clean
argument_list|(
name|NULL
argument_list|,
...
name|name
argument_list|,
literal|"NS address rewrite nsip "
argument_list|,
name|result
argument_list|)
expr_stmt|;
...
name|DNS_R_DNAME
case|:
comment|/* 		 * DNAME policy RRs have very few if any uses that are not 		 * better served with simple wildcards.  Making the work would 		 * require complications to get the number of labels matched 		 * in the name or the found name itself to the main DNS_R_DNAME 		 * case in query_find(). So fall through to treat them as NODATA. 		 */
case|case
name|DNS_R_NXRRSET
case|:
name|policy
...
control|)
block|{
comment|/* 		 * Construct the rule's owner name. 		 */
name|dns_fixedname_init
argument_list|(
operator|&
name|prefixf
...
expr_stmt|;
block|}
comment|/* 		 * See if the qname rule (or RR) exists. 		 */
name|result
operator|=
name|rpz_find
argument_list|(
...
return|;
default|default:
comment|/* 			 * when more than one name or address hits a rule, 			 * prefer the first set of names (qname or NS), 			 * the first policy zone, and the smallest name 			 */
if|if
condition|(
name|st
operator|->
...
begin_comment
comment|/*  * Look for response policy zone NSIP and NSDNAME rewriting.  */
end_comment
...
block|}
block|}
comment|/* 			 * Check all IP addresses for this NS name, but don't 			 * bother without NSIP rules or with a NSDNAME hit. 			 */
name|version
operator|=
name|NULL
expr_stmt|;
...
goto|;
block|}
comment|/* 		 * Check all A and AAAA records in all response policy 		 * IP address zones 		 */
name|rpz_st
operator|=
name|client
operator|->
...
goto|;
block|}
comment|/* 			 * After a hit in the radix tree for the policy domain, 			 * either stop trying to rewrite (DNS_RPZ_POLICY_NO_OP) 			 * or restart to ask the ordinary database of the 			 * policy zone for the DNS record corresponding to the 			 * record in the radix tree. 			 */
switch|switch
condition|(
name|rpz_st
operator|->
