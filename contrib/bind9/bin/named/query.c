...
begin_comment
comment|/* $Id: query.c,v 1.257.18.46.2.1 2009/11/19 00:25:17 marka Exp $ */
end_comment
...
directive|endif
comment|/* If successfull, Transfer ownership of zone. */
if|if
condition|(
name|result
operator|==
...
begin_comment
comment|/*  * Find the secure key that corresponds to rrsig.  * Note: 'keyrdataset' maintains state between sucessive calls,  * there may be multiple keys with the same keyid.  * Return ISC_FALSE if we have exhausted all the possible keys.  */
end_comment
...
condition|)
block|{
comment|/* 		 * If the client doesn't want DNSSEC we still want to 		 * look for any data pending validation to save a remote 		 * lookup if possible. 		 */
name|dns_rdataset_init
argument_list|(
operator|&
name|tmprdataset
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we have found pending data try to validate it. 	 * If the data does not validate as secure and we can't 	 * use the unvalidated data requery the database with 	 * pending disabled to prevent infinite looping. 	 */
if|if
condition|(
name|result
operator|!=
