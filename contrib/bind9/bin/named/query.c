...
begin_comment
comment|/* $Id: query.c,v 1.198.2.13.4.30 2004/06/30 14:13:05 marka Exp $ */
end_comment
...
condition|)
block|{
comment|/* 		 * RFC 2535 section 3.5 says that when A or AAAA records are 		 * retrieved as additional data, any KEY RRs for the owner name 		 * should be added to the additional data section. 		 * 		 * XXXRTH  We should lower the priority here.  Alternatively, 		 * we could raise the priority of glue records. 		 */
name|eresult
operator|=
name|query_addadditional
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * RFC 2535 section 3.5 says that when NS, SOA, A, or AAAA records 	 * are retrieved, any KEY RRs for the owner name should be added 	 * to the additional data section.  We treat A6 records the same way. 	 * 	 * We don't care if query_addadditional() fails. 	 */
if|if
condition|(
name|type
operator|==
...
condition|)
block|{
comment|/* 		 * XXXRTH  We should lower the priority here.  Alternatively, 		 * we could raise the priority of glue records. 		 */
operator|(
name|void
operator|)
name|query_addadditional
...
argument_list|,
literal|"recursive-clients limit exceeded, "
...
expr_stmt|;
block|}
comment|/* 		 * Add SOA.  If the query was for a SOA record force the 		 * ttl to zero so that it is possible for clients to find 		 * the containing zone of a arbitary name with a stub 		 * resolver and not have it cached. 		 */
if|if
condition|(
name|qtype
operator|==
