...
name|level
argument_list|,
literal|"query client=%p thread=0x%lx "
literal|"(%s/%s): %s"
argument_list|,
name|client
argument_list|,
...
name|NULL
expr_stmt|;
comment|/* 	 * This mutex is destroyed when the client is destroyed in 	 * exit_check(). 	 */
name|result
operator|=
name|isc_mutex_init
argument_list|(
...
name|DNS_RPZ_INFO_LEVEL
argument_list|,
literal|" unrecognized NS"
literal|" rpz_rrset_find() "
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
literal|1
argument_list|,
literal|"client attr:0x%x, query attr:0x%X, restarts:%d, "
literal|"origqname:%s, timer:%d, authdb:%d, referral:%d"
argument_list|,
name|client
operator|->
name|attributes
...
argument_list|)
argument_list|,
literal|"resume from RPZ recursion"
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
...
literal|1
argument_list|,
literal|"rpz qname %s, rname:%s, fname:%s"
argument_list|,
name|qbuf
argument_list|,
name|rbuf
...
condition|)
block|{
comment|/* 		 * If the query type is DS, look to see if we are 		 * authoritative for the child zone. 		 */
name|dns_db_t
modifier|*
name|tdb
init|=
