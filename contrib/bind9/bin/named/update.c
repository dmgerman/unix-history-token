...
begin_comment
comment|/* $Id: update.c,v 1.151.12.16 2011-03-26 00:47:01 each Exp $ */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 		 * Replace records added in this UPDATE request. 		 */
if|if
condition|(
name|db_rr
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 	 * Preserve the status of the OPT bit in the origin's NSEC record. 	 */
if|if
condition|(
name|dns_name_equal
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we are not checking the ZSK flag then all DNSKEY's are 	 * already signing all RRsets so we don't need to trigger special 	 * changes. 	 */
if|if
condition|(
operator|*
name|deleted_zsk
...
argument_list|,
literal|"disabling update-check-ksk"
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * If we have deleted a ZSK and we we still have some ZSK's 	 * we don't need to convert the KSK's to a ZSK's. 	 */
if|if
condition|(
operator|*
name|deleted_zsk
...
argument_list|)
expr_stmt|;
comment|/* 	 * If we don't have a NSEC record at the origin then we need to 	 * update the NSEC3 records. 	 */
name|CHECK
argument_list|(
name|rrset_exists
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * Check if we have any active NSEC3 chains by looking for a 	 * NSEC3PARAM RRset. 	 */
name|CHECK
argument_list|(
name|rrset_exists
argument_list|(
...
expr_stmt|;
block|}
comment|/* 			 * Remove the temporary add record. 			 */
name|CHECK
argument_list|(
name|dns_difftuple_create
argument_list|(
...
expr_stmt|;
block|}
comment|/* 	 * Reverse any pending changes. 	 */
for|for
control|(
name|tuple
operator|=
...
block|}
block|}
comment|/* 	 * Convert deletions into delayed deletions. 	 */
for|for
control|(
name|tuple
operator|=
...
begin_comment
comment|/*  * Mark all NSEC3 chains for deletion without creating a NSEC chain as  * a side effect of deleting the last chain.  */
end_comment
...
continue|continue;
block|}
comment|/* 				 * Set the NSEC3CHAIN creation flag. 				 */
name|INSIST
argument_list|(
name|rdata
operator|.
...
name|buf
expr_stmt|;
comment|/* 				 * Force the TTL to zero for NSEC3PARAM records. 				 */
name|ttl
operator|=
literal|0
expr_stmt|;
...
name|LOGLEVEL_PROTOCOL
argument_list|,
literal|"attempt to delete a NSEC3PARAM "
literal|"records ignored"
argument_list|)
expr_stmt|;
continue|continue;
endif|#
...
name|LOGLEVEL_PROTOCOL
argument_list|,
literal|"update rejected: all DNSKEY records "
literal|"removed"
argument_list|)
expr_stmt|;
name|result
...
name|LOGLEVEL_PROTOCOL
argument_list|,
literal|"update rejected: DNSKEY record added"
argument_list|)
expr_stmt|;
name|result
operator|=
