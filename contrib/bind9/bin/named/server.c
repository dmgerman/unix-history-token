...
parameter_list|)
define|\
value|do { tresult = (op);					 \ 		if (tresult != ISC_R_SUCCESS) {			 \ 			isc_buffer_clear(text);			 \ 			goto cleanup;	 			 \ 		}						 \ 	} while (0)
end_define
...
begin_comment
comment|/*%  * Holds state information for the initial zone loading process.  * Uses the isc_refcount structure to count the number of views  * with pending zone loads, dereferencing as each view finishes.  */
end_comment
...
name|ISC_LOG_ERROR
argument_list|,
literal|"invalid managed-keys-directory %s: %s"
argument_list|,
name|directory
argument_list|,
name|isc_result_totext
...
name|DNS_RPZ_ERROR_LEVEL
argument_list|,
literal|"no memory for response policy zones"
argument_list|)
expr_stmt|;
return|return
operator|(
...
parameter_list|)
define|\
value|do {								\ 		if (!(cond)) {						\ 			cfg_obj_log(obj, ns_g_lctx, ISC_LOG_ERROR,	\ 				    pat, val1, val2);			\ 			result = ISC_R_RANGE;				\ 			goto cleanup;					\ 		    }							\ 	} while (0)
end_define
...
parameter_list|)
define|\
value|do {								\ 		obj = NULL;						\ 		rrl->rate.str = name;					\ 		result = cfg_map_get(map, name,&obj);			\ 		if (result == ISC_R_SUCCESS) {				\ 			rrl->rate.r = cfg_obj_asuint32(obj);		\ 			CHECK_RRL(rrl->rate.r<= max_rate,		\ 				  name" %d> %d",			\ 				  rrl->rate.r, max_rate);		\ 		} else {						\ 			rrl->rate.r = def;				\ 		}							\ 		rrl->rate.scaled = rrl->rate.r;				\ 	} while (0)
end_define
...
name|j
decl_stmt|;
comment|/* 	 * Most DNS servers have few clients, but intentinally open 	 * recursive and authoritative servers often have many. 	 * So start with a small number of entries unless told otherwise 	 * to reduce cold-start costs. 	 */
name|min_entries
operator|=
literal|500
expr_stmt|;
...
name|min_entries
argument_list|,
literal|"max-table-size %d< min-table-size %d"
argument_list|,
name|i
argument_list|,
name|min_entries
...
literal|32
argument_list|,
literal|"invalid 'ipv4-prefix-length %d'%s"
argument_list|,
name|i
argument_list|,
literal|""
...
argument_list|,
literal|"ipv6-prefix-length %d< 16 or> %d"
argument_list|,
name|i
argument_list|,
name|DNS_RRL_MAX_PREFIX
...
argument_list|)
expr_stmt|;
comment|/* 	 * Look for forward "zones" beneath this empty zone and if so 	 * create a custom db for the empty zone. 	 */
for|for
control|(
name|element
operator|=
...
expr_stmt|;
block|}
comment|/* 	 * Is the existing zone the ok to use? 	 */
if|if
condition|(
name|zone
operator|!=
...
literal|"u' "
literal|"is too large for this "
literal|"system; reducing to %lu"
argument_list|,
name|value
argument_list|,
operator|(
...
expr_stmt|;
block|}
comment|/* 	 * Make the list of response policy zone names for a view that 	 * is used for real lookups and so cares about hints. 	 */
name|obj
operator|=
name|NULL
expr_stmt|;
...
name|DNS_RPZ_ERROR_LEVEL
argument_list|,
literal|"'%s' is not a master or slave zone"
argument_list|,
name|namebuf
argument_list|)
expr_stmt|;
...
name|udpsize
expr_stmt|;
comment|/* 	 * Set the maximum rsa exponent bits. 	 */
name|obj
operator|=
name|NULL
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* 	 * Ignore case when compressing responses to the specified 	 * clients. This causes case not always to be preserved, 	 * and is needed by some broken clients. 	 */
name|CHECK
argument_list|(
name|configure_view_acl
argument_list|(
...
name|ISC_LOG_ERROR
argument_list|,
literal|"view '%s': class must not be meta"
argument_list|,
name|viewname
argument_list|)
expr_stmt|;
...
name|NULL
decl_stmt|;
comment|/* New or reused raw zone */
name|dns_zone_t
modifier|*
name|dupzone
init|=
...
argument_list|)
expr_stmt|;
comment|/* 		 * Forward zones may also set delegation only. 		 */
name|only
operator|=
name|NULL
expr_stmt|;
...
goto|;
block|}
comment|/* 	 * Redirect zones only require minimal configuration. 	 */
if|if
condition|(
name|strcasecmp
argument_list|(
...
name|ISC_LOG_ERROR
argument_list|,
literal|"redirect zone already exists"
argument_list|)
expr_stmt|;
name|result
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * Note whether this is a response policy zone. 	 */
name|is_rpz
operator|=
name|ISC_FALSE
expr_stmt|;
...
block|}
block|}
comment|/* 	 * See if we can reuse an existing zone.  This is 	 * only possible if all of these are true: 	 *   - The zone's view exists 	 *   - A zone with the right name exists in the view 	 *   - The zone is compatible with the config 	 *     options (e.g., an existing master zone cannot 	 *     be reused if the options specify a slave zone) 	 *   - The zone was and is or was not and is not a policy zone 	 */
name|result
operator|=
name|dns_viewlist_find
argument_list|(
...
name|ISC_LOG_ERROR
argument_list|,
literal|"zone '%s': incompatible"
literal|" masterfile-format or database"
literal|" for a response policy zone"
argument_list|,
name|zname
argument_list|)
expr_stmt|;
...
name|ISC_LOG_ERROR
argument_list|,
literal|"'recursive-clients %d' too low when "
literal|"running with %d worker threads"
argument_list|,
name|server
operator|->
name|recursionquota
...
name|ISC_LOG_INFO
argument_list|,
literal|"using default UDP/IPv4 port range: "
literal|"[%d, %d]"
argument_list|,
name|udpport_low
argument_list|,
...
name|ISC_LOG_INFO
argument_list|,
literal|"using default UDP/IPv6 port range: "
literal|"[%d, %d]"
argument_list|,
name|udpport_low
argument_list|,
...
name|ISC_LOG_INFO
argument_list|,
literal|"not using config file logging "
literal|"statement for logging due to "
literal|"-g option"
argument_list|)
expr_stmt|;
operator|(
...
name|ISC_LOG_ERROR
argument_list|,
literal|"checking logging configuration "
literal|"failed: %s"
argument_list|,
name|isc_result_totext
argument_list|(
...
name|refs
decl_stmt|;
comment|/* 	 * Force zone maintenance.  Do this after loading 	 * so that we know when we need to force AXFR of 	 * slave zones whose master files are missing. 	 * 	 * We use the zoneload reference counter to let us 	 * know when all views are finished. 	 */
...
argument_list|)
argument_list|,
literal|"forcing zone maintenance"
argument_list|)
expr_stmt|;
name|ns_os_started
argument_list|()
...
argument_list|)
expr_stmt|;
comment|/* 	 * Schedule zones to be loaded from disk. 	 */
for|for
control|(
name|view
operator|=
...
goto|;
block|}
comment|/* 		 * 'dns_view_asyncload' calls view_loaded if there are no 		 * zones. 		 */
...
condition|)
block|{
comment|/* 		 * Place the task manager into privileged mode.  This 		 * ensures that after we leave task-exclusive mode, no 		 * other tasks will be able to run except for the ones 		 * that are loading zones. (This should only be done during 		 * the initial server setup; it isn't necessary during 		 * a reload.) 		 */
...
argument_list|)
expr_stmt|;
comment|/* 	 * Setup the server task, which is responsible for coordinating 	 * startup and shutdown of the server, as well as all exclusive 	 * tasks. 	 */
name|CHECKFATAL
argument_list|(
...
operator|)
return|;
comment|/* Copy zonetxt because it'll be overwritten by next_token() */
name|strlcpy
argument_list|(
name|zonebuf
argument_list|,
...
argument_list|)
argument_list|,
literal|"no matching zone '%s' in any view"
argument_list|,
name|zonebuf
argument_list|)
expr_stmt|;
...
argument_list|)
argument_list|,
literal|"zone '%s' was found in multiple views"
argument_list|,
name|zonebuf
argument_list|)
expr_stmt|;
...
argument_list|)
argument_list|,
literal|"no matching zone '%s' in view '%s'"
argument_list|,
name|zonebuf
argument_list|,
name|viewtxt
...
condition|)
block|{
comment|/* only dump zones, suppress caches */
name|dctx
operator|->
name|dumpadb
operator|=
...
condition|)
block|{
comment|/* only dump adb, suppress other caches */
name|dctx
operator|->
name|dumpbad
operator|=
...
condition|)
block|{
comment|/* only dump badcache, suppress other caches */
name|dctx
operator|->
name|dumpadb
operator|=
...
name|ISC_LOG_ERROR
argument_list|,
literal|"flushing %s '%s' in cache view '%s' "
literal|"failed: %s"
argument_list|,
name|tree
condition|?
...
name|ISC_LOG_INFO
argument_list|,
literal|"flushing %s '%s' in all cache views "
literal|"succeeded"
argument_list|,
name|tree
condition|?
...
begin_comment
comment|/*  * Act on a "sync" command from the command channel. */
end_comment
...
name|ISC_LOG_INFO
argument_list|,
literal|"dumping all zones%s: %s"
argument_list|,
name|cleanup
condition|?
literal|", removing journal files"
else|:
literal|""
argument_list|,
name|isc_result_totext
...
name|ISC_LOG_INFO
argument_list|,
literal|"sync: dumping zone '%s/%s'%s%s%s: %s"
argument_list|,
name|zonename
argument_list|,
name|classstr
...
begin_comment
comment|/*  * Emit a comment at the top of the nzf file containing the viewname  * Expects the fp to already be open for writing  */
end_comment
...
name|text
argument_list|,
literal|"dns_zone_loadnew failed: "
argument_list|)
argument_list|)
expr_stmt|;
name|TCHECK
...
argument_list|)
expr_stmt|;
comment|/* Emit the zone name, quoted and escaped */
name|isc_buffer_init
argument_list|(
operator|&
name|buf
...
name|text
argument_list|,
literal|"' cannot be deleted: response-policy zone."
argument_list|)
argument_list|)
expr_stmt|;
name|result
...
name|ISC_FALSE
expr_stmt|;
comment|/* 			 * Any other lines not starting with zone, copy 			 * them out and continue. 			 */
if|if
condition|(
name|strncasecmp
argument_list|(
...
literal|4
expr_stmt|;
comment|/* This is a zone; find its name. */
while|while
condition|(
operator|*
name|p
...
operator|++
expr_stmt|;
comment|/* 			 * If it's not the zone we're looking for, copy 			 * it out and continue 			 */
if|if
condition|(
name|strncasecmp
argument_list|(
...
continue|continue;
block|}
comment|/* 			 * But if it is the zone we want, skip over it 			 * so it will be omitted from the new file 			 */
name|p
operator|+=
name|znamelen
expr_stmt|;
...
break|break;
block|}
comment|/* Copy the rest of the buffer out and continue */
name|fputs
argument_list|(
name|buf
argument_list|,
...
expr_stmt|;
block|}
comment|/* 			 * Close files before overwriting the nzfile 			 * with the temporary file as it's necessary on 			 * some platforms (win32). 			 */
operator|(
name|void
operator|)
name|isc_stdio_close
...
name|text
argument_list|,
literal|"No signing records found"
argument_list|)
expr_stmt|;
name|putnull
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* 	 * Use>= to leave space for NUL termination. 	 */
if|if
condition|(
name|l
operator|>=
