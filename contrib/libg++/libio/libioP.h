...
comment|/* The 'underflow' hook tries to fills the get buffer.    It returns the next character (as an unsigned char) or EOF.  The next    character remains in the get buffer, and the get postion is not changed.    It matches the streambuf::underflow virtual function. */
typedef|typedef
name|int
argument_list|(
...
comment|/* The 'sysread' hook is used to read data from the external file into    an existing buffer.  It generalizes the Unix read(2) function.    It matches the streambuf::sys_read virtual function, which is    specific to this implementaion. */
typedef|typedef
name|_IO_ssize_t
argument_list|(
argument|*_IO_read_t
...
comment|/* The 'syswrite' hook is used to write data from an existing buffer    to an external file.  It generalizes the Unix write(2) function.    It matches the streambuf::sys_write virtual function, which is    specific to this implementaion. */
typedef|typedef
name|_IO_ssize_t
argument_list|(
...
comment|/* The 'sysseek' hook is used to re-position an external file.    It generalizes the Unix lseek(2) function.    It matches the streambuf::sys_seek virtual function, which is    specific to this implementaion. */
typedef|typedef
name|_IO_fpos_t
argument_list|(
argument|*_IO_seek_t
...
comment|/* The 'sysstat' hook is used to get information about an external file    into a struct stat buffer.  It generalizes the Unix fstat(2) call.    It matches the streambuf::sys_stat virtual function, which is    specific to this implementaion. */
typedef|typedef
name|int
argument_list|(
argument|*_IO_stat_t
...
parameter_list|)
define|\
value|if ((FILE) == NULL) { MAYBE_SET_EINVAL; return RET; } \ 	else { COERCE_FILE(FILE); \ 	       if (((FILE)->_IO_file_flags& _IO_MAGIC_MASK) != _IO_MAGIC) \ 	  { errno = EINVAL; return RET; }}
end_define
