...
argument_list|)
expr_stmt|;
comment|/* 	 * Since we are about to unref fields_arr[0], we want to find 	 * any fields that still point into it, and have them point 	 * into the new field zero.  This has to be done intelligently, 	 * so that unrefing a field doesn't try to unref into the old $0. 	 */
for|for
control|(
name|cops
operator|=
...
begin_comment
comment|/* assign is not NULL if this field is on the LHS of an assign */
end_comment
...
condition|)
block|{
comment|/* 		 * Skip the work if first arg is the null string. 		 * Check after clearing the array, to preserve 		 * correct semantics. 		 */
name|tmp
operator|=
name|tmp_number
argument_list|(
...
begin_comment
comment|/* update_PROCINFO --- update PROCINFO[sub] when FS or FIELDWIDTHS set */
end_comment
