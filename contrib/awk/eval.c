...
begin_comment
comment|/* This rather ugly macro is for VMS C */
end_comment
...
literal|"Node_ahash"
block|,
literal|"Node_array_ref"
block|,
literal|"Node_BINMODE"
block|,
...
literal|"Node_RS"
block|,
literal|"Node_TEXTDOMAIN"
block|,
literal|"Node_final --- this should never appear"
block|,
name|NULL
block|}
decl_stmt|;
...
begin_comment
comment|/* nodetype2str --- convert a node type into a printable value */
end_comment
...
begin_comment
comment|/* genflags2str --- general routine to convert a flag value to a string */
end_comment
...
control|)
block|{
comment|/* 		 * note the trick, we want 1 or 0 for whether we need 		 * the '|' character. 		 */
name|space_needed
operator|=
operator|(
name|strlen
...
condition|)
block|{
comment|/* using the lnode exec_count is kludgey */
if|if
condition|(
name|tree
operator|->
...
argument_list|)
expr_stmt|;
comment|/* sanity: do nothing if empty */
if|if
condition|(
name|array
operator|->
...
comment|/* from switch */
block|}
comment|/* allocate space for array */
name|num_elems
operator|=
name|array
operator|->
...
expr_stmt|;
comment|/* shazzam! */
comment|/* now we can run the loop */
name|PUSH_BINDING
argument_list|(
name|loop_tag_stack
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* could add a lint check here */
if|if
condition|(
name|in_function
argument_list|()
...
name|n
decl_stmt|;
comment|/* Here lies the wumpus's brother. R.I.P. */
name|n
operator|=
name|force_string
argument_list|(
...
name|NULL
decl_stmt|;
comment|/* 	 * For ++ and --, get the lhs when doing the op and then 	 * return.  For += etc, do the rhs first, since it can 	 * rearrange things, and *then* get the lhs. 	 */
switch|switch
condition|(
name|tree
operator|->
...
comment|/* handled below */
block|}
comment|/* 	 * It's a += kind of thing.  Do the rhs, then the lhs. 	 */
name|tmp
operator|=
name|tree_eval
argument_list|(
...
condition|)
block|{
comment|/* type changed, copy array back for call by reference */
...
condition|)
block|{
comment|/* we must also reassign f here; see below */
name|f
operator|=
operator|&
name|fcall_list
...
begin_comment
comment|/* dump_fcall_stack --- print a backtrace of the awk function calls */
end_comment
...
index|]
decl_stmt|;
comment|/* 		 * This test should always be true, due to the code 		 * above, before the switch, that handles parameters. 		 */
if|if
condition|(
name|n
operator|->
...
directive|else
comment|/* 		 * This is how Christos at Deshaw did it. 		 * Does this buy us anything? 		 */
if|if
condition|(
name|ptr
operator|->
...
begin_comment
comment|/* set_BINMODE --- set translation mode (OS/2, DOS, others) */
end_comment
...
condition|)
block|{
comment|/* arbitrary string, assume both */
name|BINMODE
operator|=
literal|3
expr_stmt|;
name|warning
argument_list|(
literal|"BINMODE: arbitary string value treated as \"rw\""
argument_list|)
expr_stmt|;
block|}
block|}
...
begin_comment
comment|/* set_LINT --- update LINT as appropriate */
end_comment
...
name|warning
expr_stmt|;
comment|/* explicitly use warning() here, in case lintfunc == r_fatal */
if|if
condition|(
name|old_lint
operator|!=
...
begin_comment
comment|/* set_TEXTDOMAIN --- update TEXTDOMAIN variable when TEXTDOMAIN assigned to */
end_comment
...
literal|'\0'
expr_stmt|;
comment|/* 	 * Note: don't call textdomain(); this value is for 	 * the awk program, not for gawk itself. 	 */
block|}
end_function

begin_comment
comment|/*  * assign_val --- do mechanics of assignment, for calling from multiple  *		  places.  */
end_comment
...
begin_comment
comment|/* update_ERRNO --- update the value of ERRNO */
end_comment
...
begin_comment
comment|/* comp_func --- array index comparison function for qsort */
end_comment
...
operator|)
expr_stmt|;
comment|/* 	t1 = force_string(t1); 	t2 = force_string(t2); */
name|len1
operator|=
name|t1
operator|->
...
name|stptr
expr_stmt|;
comment|/* Array indexes are strings, compare as such, always! */
if|if
condition|(
name|len1
operator|==
