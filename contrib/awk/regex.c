...
begin_comment
comment|/* Gawk: initialize to force this one, 					   not the one in the C library. */
end_comment
...
begin_comment
comment|/* Gawk: 12 Dec 2000 --- revert to array of char * for use with K&R Compilers. */
end_comment
...
value|\     if (old_buffer != bufp->buffer)					\       {									\ 	int incr = bufp->buffer - old_buffer;				\ 	MOVE_BUFFER_POINTER (b);					\ 	MOVE_BUFFER_POINTER (begalt);					\         if (fixup_alt_jump)						\ 	  MOVE_BUFFER_POINTER (fixup_alt_jump);				\         if (laststart)							\ 	  MOVE_BUFFER_POINTER (laststart);				\         if (pending_exact)						\ 	  MOVE_BUFFER_POINTER (pending_exact);				\       }									\     ELSE_EXTEND_BUFFER_HIGH_BOUND					\   } while (0)
end_define
