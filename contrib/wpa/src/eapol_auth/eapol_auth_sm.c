...
name|Auto
expr_stmt|;
comment|/* 	 * Clearing keyRun here is not specified in IEEE Std 802.1X-2004, but 	 * it looks like this would be logical thing to do here since the 	 * EAPOL-Key exchange is not possible in this state. It is possible to 	 * get here on disconnection event without advancing to the 	 * AUTHENTICATING state to clear keyRun before the IEEE 802.11 RSN 	 * authenticator state machine runs and that may advance from 	 * AUTHENTICATION2 to INITPMK if keyRun = TRUE has been left from the 	 * last association. This can be avoided by clearing keyRun here. 	 */
name|sm
operator|->
name|keyRun
operator|=
...
specifier|static
name|int
name|eapol_sm_get_erp_send_reauth_start
parameter_list|(
name|void
modifier|*
name|ctx
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: External reauthentication trigger for "
name|MACSTR
argument_list|,
name|MAC2STR
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: External configuration operation for "
name|MACSTR
literal|" - param=%s value=%s"
argument_list|,
name|MAC2STR
argument_list|(
name|sm
...
name|param
argument_list|,
literal|"AdminControlledDirections"
argument_list|)
operator|==
literal|0
condition|)
...
name|param
argument_list|,
literal|"AdminControlledPortControl"
argument_list|)
operator|==
literal|0
condition|)
...
name|param
argument_list|,
literal|"KeyTransmissionEnabled"
argument_list|)
operator|==
literal|0
condition|)
...
name|erp_add_key
expr_stmt|;
comment|/* Acct-Multi-Session-Id should be unique over reboots. If reliable 	 * clock is not available, this could be replaced with reboot counter, 	 * etc. */
name|os_get_time
argument_list|(
operator|&
name|now
