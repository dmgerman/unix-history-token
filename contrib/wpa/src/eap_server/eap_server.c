...
specifier|static
name|int
name|eap_get_erp_send_reauth_start
parameter_list|(
name|struct
name|eap_sm
modifier|*
...
name|wpabuf
modifier|*
name|eap_sm_buildInitiateReauthStart
parameter_list|(
name|struct
name|eap_sm
modifier|*
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: Could not find suitable EAP method"
argument_list|)
expr_stmt|;
name|eap_log_msg
argument_list|(
name|sm
argument_list|,
literal|"Could not find suitable EAP method"
argument_list|)
expr_stmt|;
name|sm
operator|->
...
name|sm
argument_list|,
literal|"Propose EAP method vendor=%u method=%u"
argument_list|,
name|vendor
argument_list|,
name|sm
...
argument|EAP
argument_list|,
argument|INITIATE_REAUTH_START
argument_list|)
end_macro
...
name|EAP
argument_list|,
name|INITIATE_REAUTH_START
argument_list|)
expr_stmt|;
name|sm
operator|->
name|initiate_reauth_start_sent
operator|=
name|TRUE
expr_stmt|;
name|sm
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: building EAP-Initiate-Re-auth-Start: Identifier %d"
argument_list|,
name|sm
operator|->
name|currentId
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: Send EAP-Finish/Re-auth (%s)"
argument_list|,
name|flags
operator|&
literal|0x80
...
literal|0x80
decl_stmt|;
comment|/* default to failure; cleared on success */
name|SM_ENTRY
argument_list|(
name|EAP
argument_list|,
name|INITIATE_RECEIVED
argument_list|)
expr_stmt|;
name|sm
operator|->
...
name|MSG_INFO
argument_list|,
literal|"EAP-Initiate: Invalid frame"
argument_list|)
expr_stmt|;
goto|goto
name|fail
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: EAP-Initiate/Re-Auth"
argument_list|,
name|pos
argument_list|,
name|len
...
name|MSG_INFO
argument_list|,
literal|"EAP: Too short EAP-Initiate/Re-auth"
argument_list|)
expr_stmt|;
goto|goto
name|fail
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: No keyName-NAI in EAP-Initiate/Re-auth Packet"
argument_list|)
expr_stmt|;
goto|goto
name|fail
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: EAP-Initiate/Re-auth - keyName-NAI"
argument_list|,
name|parse
operator|.
name|keyname
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: Too long keyName-NAI in EAP-Initiate/Re-auth"
argument_list|)
expr_stmt|;
goto|goto
name|fail
...
condition|)
block|{
comment|/* 		 * In passthrough case, EAP-Initiate/Re-auth replaces 		 * EAP Identity exchange. Use keyName-NAI as the user identity 		 * and forward EAP-Initiate/Re-auth to the backend 		 * authentication server. 		 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"EAP: Use keyName-NAI as user identity for backend authentication"
argument_list|)
expr_stmt|;
name|eap_server_clear_identity
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: SEQ=%u replayed (already received SEQ=%u)"
argument_list|,
name|seq
argument_list|,
name|erp
...
block|}
block|}
comment|/* Check if any supported CS results in matching tag */
if|if
condition|(
operator|!
name|hash_len
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: Authentication Tag match using HMAC-SHA256-256"
argument_list|)
expr_stmt|;
name|hash_len
operator|=
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: Authentication Tag match using HMAC-SHA256-128"
argument_list|)
expr_stmt|;
name|hash_len
operator|=
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: No supported cryptosuite matched Authentication Tag"
argument_list|)
expr_stmt|;
goto|goto
name|fail
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: EAP-Initiate/Re-Auth TVs/TLVs"
argument_list|,
name|tlvs
argument_list|,
name|end
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: ERP key %s SEQ updated to %u"
argument_list|,
name|erp
operator|->
name|keyname_nai
...
name|sm
argument_list|,
literal|"Discard received EAP message"
argument_list|)
expr_stmt|;
name|SM_ENTER
argument_list|(
...
condition|)
block|{
comment|/* 			 * This transition is not mentioned in RFC 4137, but it 			 * is needed to handle cleanly a case where EAP method 			 * initialization fails. 			 */
name|SM_ENTER
argument_list|(
name|EAP
argument_list|,
...
condition|)
block|{
comment|/* 			 * This transition is not mentioned in RFC 4137, but it 			 * is needed to handle cleanly a case where EAP method 			 * buildReq fails. 			 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"EAP: Method did not return a request"
argument_list|)
expr_stmt|;
name|SM_ENTER
argument_list|(
...
name|decision
operator|==
name|DECISION_INITIATE_REAUTH_START
condition|)
name|SM_ENTER
argument_list|(
name|EAP
...
break|break;
case|case
name|EAP_INITIATE_REAUTH_START
case|:
name|SM_ENTER
argument_list|(
name|EAP
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: retransmit timeout 1 second for EAP-Initiate-Re-auth-Start"
argument_list|)
expr_stmt|;
return|return
literal|1
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: parseEapResp: rxResp=%d rxInitiate=%d respId=%d respMethod=%u respVendor=%u respVendorMethod=%u"
argument_list|,
name|sm
operator|->
name|rxResp
...
name|MSG_DEBUG
argument_list|,
literal|"EAP: getDecision: send EAP-Initiate/Re-auth-Start"
argument_list|)
expr_stmt|;
return|return
name|DECISION_INITIATE_REAUTH_START
...
begin_function
name|void
name|eap_server_mschap_rx_callback
parameter_list|(
name|struct
name|eap_sm
modifier|*
...
index|]
decl_stmt|;
comment|/* Print out Challenge and Response in format supported by asleap. */
if|if
condition|(
name|username
condition|)
...
name|MSG_DEBUG
argument_list|,
literal|"[%s/user=%s] asleap -C %s -R %s"
argument_list|,
name|source
argument_list|,
name|user
