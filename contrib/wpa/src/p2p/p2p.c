...
name|p2p
argument_list|,
literal|"Do not update peer entry based on old frame (rx_time=%u.%06u last_seen=%u.%06u)"
argument_list|,
operator|(
name|unsigned
name|int
...
condition|)
continue|continue;
comment|/* If asking for wildcard, don't do others */
if|if
condition|(
name|os_memcmp
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Wildcard always matches if we have actual services */
if|if
condition|(
name|os_memcmp
argument_list|(
...
condition|)
block|{
comment|/* We found match(es) but wildcard 					 * will return all */
name|p2p
operator|->
name|query_count
operator|=
...
break|break;
block|}
comment|/* Save each matching hash */
if|if
condition|(
name|p2p
operator|->
...
block|{
comment|/* We found match(es) but too many to 					 * return all */
name|p2p
operator|->
name|query_count
operator|=
...
name|p2p
argument_list|,
literal|"Added ASP advertisement adv_id=0x%x config_methods=0x%x svc_state=0x%x adv_str='%s'"
argument_list|,
name|adv_id
argument_list|,
name|adv_data
...
literal|"flags=%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n"
literal|"status=%d\n"
