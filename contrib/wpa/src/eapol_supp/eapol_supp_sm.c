...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: disable timer tick"
argument_list|)
expr_stmt|;
name|sm
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: enable timer tick"
argument_list|)
expr_stmt|;
name|sm
operator|->
...
name|FALSE
expr_stmt|;
comment|/* 	 * IEEE Std 802.1X-2004 does not clear heldWhile here, but doing so 	 * allows the timer tick to be stopped more quickly when the port is 	 * not enabled. Since this variable is used only within HELD state, 	 * clearing it on initialization does not change actual state machine 	 * behavior. 	 */
name|sm
operator|->
name|heldWhile
operator|=
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: Using shorter startWhen for WPS"
argument_list|)
expr_stmt|;
name|sm
operator|->
...
name|FALSE
expr_stmt|;
comment|/* 	 * IEEE Std 802.1X-2004 does not clear authWhile here, but doing so 	 * allows the timer tick to be stopped more quickly when the port is 	 * not enabled. Since this variable is used only within RECEIVE state, 	 * clearing it on initialization does not change actual state machine 	 * behavior. 	 */
name|sm
operator|->
name|authWhile
operator|=
...
name|SUPP_BE_REQUEST
case|:
comment|/* 		 * IEEE Std 802.1X-2004 has transitions from REQUEST to FAIL 		 * and SUCCESS based on eapFail and eapSuccess, respectively. 		 * However, IEEE Std 802.1X-2004 is also specifying that 		 * eapNoResp should be set in conjunction with eapSuccess and 		 * eapFail which would mean that more than one of the 		 * transitions here would be activated at the same time. 		 * Skipping RESPONSE and/or RECEIVE states in these cases can 		 * cause problems and the direct transitions to do not seem 		 * correct. Because of this, the conditions for these 		 * transitions are verified only after eapNoResp. They are 		 * unlikely to be used since eapNoResp should always be set if 		 * either of eapSuccess or eapFail is set. 		 */
if|if
condition|(
name|sm
operator|->
...
condition|)
block|{
comment|/* Get EAP Response from EAP Proxy */
name|resp
operator|=
...
name|MSG_WARNING
argument_list|,
literal|"EAPOL: txSuppRsp - EAP Proxy "
literal|"response data not available"
argument_list|)
expr_stmt|;
return|return;
block|}
...
condition|)
block|{
comment|/* Drive the EAP proxy state machine */
if|if
condition|(
...
begin_comment
comment|/**  * eapol_sm_get_method_name - Get EAPOL method name  * @sm: Pointer to EAPOL state machine allocated with eapol_sm_init()  * Returns: Static string containing name of current eap method or NULL  */
end_comment
...
condition|)
block|{
comment|/* 			 * Buffalo WHR-G125 Ver.1.47 seems to send EAP-WPS 			 * packets with too short EAPOL header length field 			 * (14 octets). This is fixed in firmware Ver.1.49. 			 * As a workaround, fix the EAPOL header based on the 			 * correct length in the EAP packet. 			 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: Workaround - fix EAPOL "
literal|"payload length based on EAP header: "
literal|"%d -> %d"
argument_list|,
operator|(
name|int
...
condition|)
block|{
comment|/* 			 * An AP has been reported to send out EAP message with 			 * undocumented code 10 at some point near the 			 * completion of EAP authentication. This can result in 			 * issues with the unexpected EAP message triggering 			 * restart of EAPOL authentication. Avoid this by 			 * skipping the message without advancing the state 			 * machine. 			 */
specifier|const
name|struct
name|eap_hdr
modifier|*
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: Ignore EAP packet with unknown code 10"
argument_list|)
expr_stmt|;
break|break;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: eap_proxy "
literal|"EAP Req updated"
argument_list|)
expr_stmt|;
block|}
endif|#
...
name|MSG_EXCESSIVE
argument_list|,
literal|"EAPOL type %d will be handled by MKA"
argument_list|,
name|hdr
operator|->
name|type
...
begin_comment
comment|/**  * eapol_sm_notify_config - Notification of EAPOL configuration change  * @sm: Pointer to EAPOL state machine allocated with eapol_sm_init()  * @config: Pointer to current network EAP configuration  * @conf: Pointer to EAPOL configuration data  *  * Notify EAPOL state machine that configuration has changed. config will be  * stored as a backpointer to network configuration. This can be %NULL to clear  * the stored pointed. conf will be copied to local EAPOL/EAP configuration  * data. If conf is %NULL, this part of the configuration change will be  * skipped.  */
end_comment
...
condition|)
block|{
comment|/* Using EAP Proxy, so skip EAP state machine update */
return|return;
block|}
endif|#
directive|endif
...
condition|)
block|{
comment|/* Get key from EAP proxy */
if|if
condition|(
name|sm
operator|==
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: EAP key not available"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: Failed to get eapKeyData"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: Requested key length (%lu) not "
literal|"available (len=%lu)"
argument_list|,
operator|(
name|unsigned
name|long
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: Successfully fetched key (len=%lu)"
argument_list|,
operator|(
name|unsigned
name|long
...
begin_comment
comment|/**  * eapol_sm_get_session_id - Get EAP Session-Id  * @sm: Pointer to EAPOL state machine allocated with eapol_sm_init()  * @len: Pointer to variable that will be set to number of bytes in the session  * Returns: Pointer to the EAP Session-Id or %NULL on failure  *  * The Session-Id is available only after a successful authentication.  */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: EAP Session-Id not available"
argument_list|)
expr_stmt|;
return|return
name|NULL
...
condition|)
block|{
comment|/* If there is a delayed txStart queued, start now. */
name|sm
operator|->
name|startWhen
operator|=
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: PMKSA caching was used - skip EAPOL"
argument_list|)
expr_stmt|;
name|sm
operator|->
...
begin_comment
comment|/**  * eapol_sm_notify_pmkid_attempt - Notification of PMKSA caching  * @sm: Pointer to EAPOL state machine allocated with eapol_sm_init()  *  * Notify EAPOL state machines if PMKSA caching is used.  */
end_comment
...
begin_comment
comment|/**  * eapol_sm_notify_lower_layer_success - Notification of lower layer success  * @sm: Pointer to EAPOL state machine allocated with eapol_sm_init()  * @in_eapol_sm: Whether the caller is already running inside EAPOL state  * machine loop (eapol_sm_step())  *  * Notify EAPOL (and EAP) state machines that a lower layer has detected a  * successful authentication. This is used to recover from dropped EAP-Success  * messages.  */
end_comment
...
begin_comment
comment|/**  * eapol_sm_invalidate_cached_session - Mark cached EAP session data invalid  * @sm: Pointer to EAPOL state machine allocated with eapol_sm_init()  */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: received notification from EAP "
literal|"state machine - retrying pending EAP Request"
argument_list|)
expr_stmt|;
name|sm
operator|->
...
specifier|static
name|void
name|eapol_sm_eap_param_needed
parameter_list|(
name|void
modifier|*
name|ctx
...
name|MSG_DEBUG
argument_list|,
literal|"EAPOL: EAP parameter needed"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|MSG_ERROR
argument_list|,
literal|"Unable to initialize EAP Proxy"
argument_list|)
expr_stmt|;
block|}
endif|#
