...
begin_comment
comment|/*  * WPA Supplicant - WPA state machine and EAPOL-Key processing  * Copyright (c) 2003-2015, Jouni Malinen<j@w1.fi>  *  * This software may be distributed under the terms of the BSD license.  * See README for more details.  */
end_comment
...
begin_comment
comment|/**  * wpa_eapol_key_send - Send WPA/RSN EAPOL-Key message  * @sm: Pointer to WPA state machine data from wpa_sm_init()  * @kck: Key Confirmation Key (KCK, part of PTK)  * @kck_len: KCK length in octets  * @ver: Version field from Key Info  * @dest: Destination address for the frame  * @proto: Ethertype (usually ETH_P_EAPOL)  * @msg: EAPOL-Key message  * @msg_len: Length of message  * @key_mic: Pointer to the buffer to which the EAPOL-Key MIC is written  */
end_comment
...
name|MSG_ERROR
argument_list|,
literal|"WPA: Failed to generate EAPOL-Key version %d key_mgmt 0x%x MIC"
argument_list|,
name|ver
argument_list|,
name|sm
...
specifier|static
name|void
name|wpa_supplicant_key_mgmt_set_pmk
parameter_list|(
name|struct
name|wpa_sm
modifier|*
...
name|MSG_DEBUG
argument_list|,
literal|"RSN: Cannot set low order 256 bits of MSK for key management offload"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|MSG_DEBUG
argument_list|,
literal|"RSN: Cannot set PMK for key management offload"
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
...
name|key_mic
expr_stmt|;
comment|/* same offset for reply and reply192 */
if|if
condition|(
name|mic_len
operator|==
...
name|MSG_DEBUG
argument_list|,
literal|"P2P: Add IP Address Request KDE "
literal|"into EAPOL-Key 2/4"
argument_list|)
expr_stmt|;
name|os_memcpy
argument_list|(
...
return|;
block|}
comment|/* TK is not needed anymore in supplicant */
name|os_memset
argument_list|(
name|sm
operator|->
...
name|MSG_INFO
argument_list|,
literal|"WPA: Group Key Handshake started prior to completion of 4-way handshake"
argument_list|)
expr_stmt|;
goto|goto
name|failed
...
name|MSG_WARNING
argument_list|,
literal|"WPA: Unsupported AES-WRAP len %u"
argument_list|,
operator|(
name|unsigned
name|int
...
expr_stmt|;
block|}
comment|/* 	 * Make a copy of the frame since we need to modify the buffer during 	 * MAC validation and Key Data decryption. 	 */
name|tmp
operator|=
name|os_malloc
argument_list|(
...
literal|"frame - key_data overflow (%u> %u)"
argument_list|,
operator|(
name|unsigned
name|int
...
name|MSG_INFO
argument_list|,
literal|"OSEN: Unsupported EAPOL-Key descriptor version %d"
argument_list|,
name|ver
argument_list|)
expr_stmt|;
...
name|MSG_INFO
argument_list|,
literal|"RSN: Unsupported EAPOL-Key descriptor version %d (expected AKM defined = 0)"
argument_list|,
name|ver
argument_list|)
expr_stmt|;
...
name|MSG_INFO
argument_list|,
literal|"WPA: Interoperability workaround: allow incorrect (should have been HMAC-SHA1), but stronger (is AES-128-CMAC), descriptor version to be used"
argument_list|)
expr_stmt|;
block|}
else|else
...
directive|endif
comment|/* CONFIG_IEEE80211R */
comment|/* Keys are not needed in the WPA state machine anymore */
name|wpa_sm_drop_sa
argument_list|(
name|sm
argument_list|)
...
begin_comment
comment|/**  * wpa_sm_set_pmk - Set PMK  * @sm: Pointer to WPA state machine data from wpa_sm_init()  * @pmk: The new PMK  * @pmk_len: The length of the new PMK in bytes  * @bssid: AA to add into PMKSA cache or %NULL to not cache the PMK  *  * Configure the PMK for WPA state machine.  */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"Updated key replay counter"
argument_list|)
expr_stmt|;
block|}
end_function
