...
begin_comment
comment|/*  * EAP peer state machine functions (RFC 4137)  * Copyright (c) 2004-2012, Jouni Malinen<j@w1.fi>  *  * This software may be distributed under the terms of the BSD license.  * See README for more details.  */
end_comment
...
block|{
comment|/** 	 * get_config - Get pointer to the current network configuration 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 */
name|struct
...
parameter_list|)
function_decl|;
comment|/** 	 * set_bool - Set a boolean EAPOL state variable 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @variable: EAPOL boolean variable to set 	 * @value: Value for the EAPOL variable 	 */
name|void
function_decl|(
modifier|*
name|set_bool
...
parameter_list|)
function_decl|;
comment|/** 	 * get_int - Get an integer EAPOL state variable 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @variable: EAPOL integer variable to get 	 * Returns: Value of the EAPOL variable 	 */
name|unsigned
name|int
function_decl|(
modifier|*
...
parameter_list|)
function_decl|;
comment|/** 	 * set_int - Set an integer EAPOL state variable 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @variable: EAPOL integer variable to set 	 * @value: Value for the EAPOL variable 	 */
name|void
function_decl|(
modifier|*
name|set_int
...
parameter_list|)
function_decl|;
comment|/** 	 * get_eapReqData - Get EAP-Request data 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @len: Pointer to variable that will be set to eapReqDataLen 	 * Returns: Reference to eapReqData (EAP state machine will not free 	 * this) or %NULL if eapReqData not available. 	 */
name|struct
name|wpabuf
modifier|*
function_decl|(
...
parameter_list|)
function_decl|;
comment|/** 	 * set_config_blob - Set named configuration blob 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @blob: New value for the blob 	 * 	 * Adds a new configuration blob or replaces the current value of an 	 * existing blob. 	 */
name|void
function_decl|(
modifier|*
...
parameter_list|)
function_decl|;
comment|/** 	 * get_config_blob - Get a named configuration blob 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @name: Name of the blob 	 * Returns: Pointer to blob data or %NULL if not found 	 */
specifier|const
name|struct
name|wpa_config_blob
modifier|*
...
parameter_list|)
function_decl|;
comment|/** 	 * notify_pending - Notify that a pending request can be retried 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * 	 * An EAP method can perform a pending operation (e.g., to get a 	 * response from an external process). Once the response is available, 	 * this callback function can be used to request EAPOL state machine to 	 * retry delivering the previously received (and still unanswered) EAP 	 * request to EAP state machine. 	 */
name|void
function_decl|(
modifier|*
name|notify_pending
...
parameter_list|)
function_decl|;
comment|/** 	 * eap_param_needed - Notify that EAP parameter is needed 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @field: Field indicator (e.g., WPA_CTRL_REQ_EAP_IDENTITY) 	 * @txt: User readable text describing the required parameter 	 */
name|void
function_decl|(
modifier|*
...
parameter_list|)
function_decl|;
comment|/** 	 * notify_status - Notification of the current EAP state 	 * @ctx: eapol_ctx from eap_peer_sm_init() call 	 * @status: Step in the process of EAP authentication 	 * @parameter: Step-specific parameter, e.g., EAP method name 	 */
name|void
function_decl|(
modifier|*
name|notify_status
