...
begin_comment
comment|/*  * EAP peer: EAP-TLS/PEAP/TTLS/FAST common functions  * Copyright (c) 2004-2013, Jouni Malinen<j@w1.fi>  *  * This software may be distributed under the terms of the BSD license.  * See README for more details.  */
end_comment
...
name|txt
argument_list|,
literal|"tls_disable_time_checks=1"
argument_list|)
condition|)
name|params
operator|->
...
name|txt
argument_list|,
literal|"tls_disable_session_ticket=1"
argument_list|)
condition|)
name|params
operator|->
...
name|txt
argument_list|,
literal|"tls_disable_session_ticket=0"
argument_list|)
condition|)
name|params
operator|->
...
condition|)
block|{
comment|/* 		 * Some deployed authentication servers seem to be unable to 		 * handle the TLS Session Ticket extension (they are supposed 		 * to ignore unrecognized TLS extensions, but end up rejecting 		 * the ClientHello instead). As a workaround, disable use of 		 * TLS Sesson Ticket extension for EAP-TLS, EAP-PEAP, and 		 * EAP-TTLS (EAP-FAST uses session ticket, so any server that 		 * supports EAP-FAST does not need this workaround). 		 */
name|params
operator|->
name|flags
operator||=
...
name|NULL
return|;
comment|/* Session-Id = EAP type || client.random || server.random */
name|out
index|[
literal|0
index|]
...
begin_comment
comment|/**  * eap_peer_tls_reassemble_fragment - Reassemble a received fragment  * @data: Data for TLS processing  * @in_data: Next incoming TLS segment  * Returns: 0 on success, 1 if more data is needed for the full message, or  * -1 on error  */
end_comment
