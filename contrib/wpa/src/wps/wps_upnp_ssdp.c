...
begin_comment
comment|/* No. of chars excluding trailing whitespace */
end_comment

begin_function
specifier|static
name|int
name|line_length_stripped
parameter_list|(
specifier|const
name|char
modifier|*
...
begin_comment
comment|/**  * advertisement_state_machine_stop - Stop SSDP advertisements  * @sm: WPS UPnP state machine from upnp_wps_device_init()  */
end_comment
...
begin_comment
comment|/**  * ssdp_listener_stop - Stop SSDP listered  * @sm: WPS UPnP state machine from upnp_wps_device_init()  *  * This function stops the SSDP listerner that was started by calling  * ssdp_listener_start().  */
end_comment
...
begin_comment
comment|/**  * ssdp_listener_start - Set up for receiving discovery (UDP) packets  * @sm: WPS UPnP state machine from upnp_wps_device_init()  * Returns: 0 on success, -1 on failure  *  * The SSDP listerner is stopped by calling ssdp_listener_stop().  */
end_comment
...
block|if (fcntl(sd, F_SETFL, O_NONBLOCK) != 0) 		return -1;
endif|#
directive|endif
if|if
condition|(
