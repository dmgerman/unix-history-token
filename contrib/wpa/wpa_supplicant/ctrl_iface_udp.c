begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * WPA Supplicant / UDP socket -based control interface  * Copyright (c) 2004-2005, Jouni Malinen<j@w1.fi>  *  * This software may be distributed under the terms of the BSD license.  * See README for more details.  */
end_comment

begin_include
include|#
directive|include
file|"includes.h"
end_include

begin_include
include|#
directive|include
file|"common.h"
end_include

begin_include
include|#
directive|include
file|"eloop.h"
end_include

begin_include
include|#
directive|include
file|"config.h"
end_include

begin_include
include|#
directive|include
file|"eapol_supp/eapol_supp_sm.h"
end_include

begin_include
include|#
directive|include
file|"wpa_supplicant_i.h"
end_include

begin_include
include|#
directive|include
file|"ctrl_iface.h"
end_include

begin_include
include|#
directive|include
file|"common/wpa_ctrl.h"
end_include

begin_define
define|#
directive|define
name|COOKIE_LEN
value|8
end_define

begin_comment
comment|/* Per-interface ctrl_iface */
end_comment

begin_comment
comment|/**  * struct wpa_ctrl_dst - Internal data structure of control interface monitors  *  * This structure is used to store information about registered control  * interface monitors into struct wpa_supplicant. This data is private to  * ctrl_iface_udp.c and should not be touched directly from other files.  */
end_comment

begin_struct
struct|struct
name|wpa_ctrl_dst
block|{
name|struct
name|wpa_ctrl_dst
modifier|*
name|next
decl_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|struct
name|sockaddr_in6
name|addr
decl_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|struct
name|sockaddr_in
name|addr
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|socklen_t
name|addrlen
decl_stmt|;
name|int
name|debug_level
decl_stmt|;
name|int
name|errors
decl_stmt|;
block|}
struct|;
end_struct

begin_struct
struct|struct
name|ctrl_iface_priv
block|{
name|struct
name|wpa_supplicant
modifier|*
name|wpa_s
decl_stmt|;
name|int
name|sock
decl_stmt|;
name|struct
name|wpa_ctrl_dst
modifier|*
name|ctrl_dst
decl_stmt|;
name|u8
name|cookie
index|[
name|COOKIE_LEN
index|]
decl_stmt|;
block|}
struct|;
end_struct

begin_function_decl
specifier|static
name|void
name|wpa_supplicant_ctrl_iface_send
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|,
name|int
name|level
parameter_list|,
specifier|const
name|char
modifier|*
name|buf
parameter_list|,
name|size_t
name|len
parameter_list|)
function_decl|;
end_function_decl

begin_function
specifier|static
name|int
name|wpa_supplicant_ctrl_iface_attach
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|,
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|struct
name|sockaddr_in6
modifier|*
name|from
parameter_list|,
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|struct
name|sockaddr_in
modifier|*
name|from
parameter_list|,
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|socklen_t
name|fromlen
parameter_list|)
block|{
name|struct
name|wpa_ctrl_dst
modifier|*
name|dst
decl_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|char
name|addr
index|[
name|INET6_ADDRSTRLEN
index|]
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_UDP_IPV6 */
name|dst
operator|=
name|os_zalloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|dst
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|dst
operator|==
name|NULL
condition|)
return|return
operator|-
literal|1
return|;
name|os_memcpy
argument_list|(
operator|&
name|dst
operator|->
name|addr
argument_list|,
name|from
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|from
argument_list|)
argument_list|)
expr_stmt|;
name|dst
operator|->
name|addrlen
operator|=
name|fromlen
expr_stmt|;
name|dst
operator|->
name|debug_level
operator|=
name|MSG_INFO
expr_stmt|;
name|dst
operator|->
name|next
operator|=
name|priv
operator|->
name|ctrl_dst
expr_stmt|;
name|priv
operator|->
name|ctrl_dst
operator|=
name|dst
expr_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE monitor attached %s:%d"
argument_list|,
name|inet_ntop
argument_list|(
name|AF_INET6
argument_list|,
operator|&
name|from
operator|->
name|sin6_addr
argument_list|,
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|from
argument_list|)
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|from
operator|->
name|sin6_port
argument_list|)
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE monitor attached %s:%d"
argument_list|,
name|inet_ntoa
argument_list|(
name|from
operator|->
name|sin_addr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|from
operator|->
name|sin_port
argument_list|)
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
return|return
literal|0
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|wpa_supplicant_ctrl_iface_detach
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|,
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|struct
name|sockaddr_in6
modifier|*
name|from
parameter_list|,
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|struct
name|sockaddr_in
modifier|*
name|from
parameter_list|,
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|socklen_t
name|fromlen
parameter_list|)
block|{
name|struct
name|wpa_ctrl_dst
modifier|*
name|dst
decl_stmt|,
modifier|*
name|prev
init|=
name|NULL
decl_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|char
name|addr
index|[
name|INET6_ADDRSTRLEN
index|]
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|dst
operator|=
name|priv
operator|->
name|ctrl_dst
expr_stmt|;
while|while
condition|(
name|dst
condition|)
block|{
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
if|if
condition|(
name|from
operator|->
name|sin6_port
operator|==
name|dst
operator|->
name|addr
operator|.
name|sin6_port
operator|&&
operator|!
name|os_memcmp
argument_list|(
operator|&
name|from
operator|->
name|sin6_addr
argument_list|,
operator|&
name|dst
operator|->
name|addr
operator|.
name|sin6_addr
argument_list|,
sizeof|sizeof
argument_list|(
name|from
operator|->
name|sin6_addr
argument_list|)
argument_list|)
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE monitor detached %s:%d"
argument_list|,
name|inet_ntop
argument_list|(
name|AF_INET6
argument_list|,
operator|&
name|from
operator|->
name|sin6_addr
argument_list|,
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|from
argument_list|)
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|from
operator|->
name|sin6_port
argument_list|)
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
if|if
condition|(
name|from
operator|->
name|sin_addr
operator|.
name|s_addr
operator|==
name|dst
operator|->
name|addr
operator|.
name|sin_addr
operator|.
name|s_addr
operator|&&
name|from
operator|->
name|sin_port
operator|==
name|dst
operator|->
name|addr
operator|.
name|sin_port
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE monitor detached "
literal|"%s:%d"
argument_list|,
name|inet_ntoa
argument_list|(
name|from
operator|->
name|sin_addr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|from
operator|->
name|sin_port
argument_list|)
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
if|if
condition|(
name|prev
operator|==
name|NULL
condition|)
name|priv
operator|->
name|ctrl_dst
operator|=
name|dst
operator|->
name|next
expr_stmt|;
else|else
name|prev
operator|->
name|next
operator|=
name|dst
operator|->
name|next
expr_stmt|;
name|os_free
argument_list|(
name|dst
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|prev
operator|=
name|dst
expr_stmt|;
name|dst
operator|=
name|dst
operator|->
name|next
expr_stmt|;
block|}
return|return
operator|-
literal|1
return|;
block|}
specifier|static
name|int
name|wpa_supplicant_ctrl_iface_level
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|,
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|struct
name|sockaddr_in6
modifier|*
name|from
parameter_list|,
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|struct
name|sockaddr_in
modifier|*
name|from
parameter_list|,
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|socklen_t
name|fromlen
parameter_list|,
name|char
modifier|*
name|level
parameter_list|)
block|{
name|struct
name|wpa_ctrl_dst
modifier|*
name|dst
decl_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|char
name|addr
index|[
name|INET6_ADDRSTRLEN
index|]
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE LEVEL %s"
argument_list|,
name|level
argument_list|)
expr_stmt|;
name|dst
operator|=
name|priv
operator|->
name|ctrl_dst
expr_stmt|;
while|while
condition|(
name|dst
condition|)
block|{
if|#
directive|if
name|CONFIG_CTRL_IFACE_UDP_IPV6
if|if
condition|(
name|from
operator|->
name|sin6_port
operator|==
name|dst
operator|->
name|addr
operator|.
name|sin6_port
operator|&&
operator|!
name|os_memcmp
argument_list|(
operator|&
name|from
operator|->
name|sin6_addr
argument_list|,
operator|&
name|dst
operator|->
name|addr
operator|.
name|sin6_addr
argument_list|,
sizeof|sizeof
argument_list|(
name|from
operator|->
name|sin6_addr
argument_list|)
argument_list|)
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE changed monitor level %s:%d"
argument_list|,
name|inet_ntop
argument_list|(
name|AF_INET6
argument_list|,
operator|&
name|from
operator|->
name|sin6_addr
argument_list|,
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|from
argument_list|)
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|from
operator|->
name|sin6_port
argument_list|)
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
if|if
condition|(
name|from
operator|->
name|sin_addr
operator|.
name|s_addr
operator|==
name|dst
operator|->
name|addr
operator|.
name|sin_addr
operator|.
name|s_addr
operator|&&
name|from
operator|->
name|sin_port
operator|==
name|dst
operator|->
name|addr
operator|.
name|sin_port
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE changed monitor "
literal|"level %s:%d"
argument_list|,
name|inet_ntoa
argument_list|(
name|from
operator|->
name|sin_addr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|from
operator|->
name|sin_port
argument_list|)
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|dst
operator|->
name|debug_level
operator|=
name|atoi
argument_list|(
name|level
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|dst
operator|=
name|dst
operator|->
name|next
expr_stmt|;
block|}
return|return
operator|-
literal|1
return|;
block|}
specifier|static
name|char
modifier|*
name|wpa_supplicant_ctrl_iface_get_cookie
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|,
name|size_t
modifier|*
name|reply_len
parameter_list|)
block|{
name|char
modifier|*
name|reply
decl_stmt|;
name|reply
operator|=
name|os_malloc
argument_list|(
literal|7
operator|+
literal|2
operator|*
name|COOKIE_LEN
operator|+
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|reply
operator|==
name|NULL
condition|)
block|{
operator|*
name|reply_len
operator|=
literal|1
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|os_memcpy
argument_list|(
name|reply
argument_list|,
literal|"COOKIE="
argument_list|,
literal|7
argument_list|)
expr_stmt|;
name|wpa_snprintf_hex
argument_list|(
name|reply
operator|+
literal|7
argument_list|,
literal|2
operator|*
name|COOKIE_LEN
operator|+
literal|1
argument_list|,
name|priv
operator|->
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
expr_stmt|;
operator|*
name|reply_len
operator|=
literal|7
operator|+
literal|2
operator|*
name|COOKIE_LEN
expr_stmt|;
return|return
name|reply
return|;
block|}
specifier|static
name|void
name|wpa_supplicant_ctrl_iface_receive
parameter_list|(
name|int
name|sock
parameter_list|,
name|void
modifier|*
name|eloop_ctx
parameter_list|,
name|void
modifier|*
name|sock_ctx
parameter_list|)
block|{
name|struct
name|wpa_supplicant
modifier|*
name|wpa_s
init|=
name|eloop_ctx
decl_stmt|;
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
init|=
name|sock_ctx
decl_stmt|;
name|char
name|buf
index|[
literal|256
index|]
decl_stmt|,
modifier|*
name|pos
decl_stmt|;
name|int
name|res
decl_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|struct
name|sockaddr_in6
name|from
decl_stmt|;
ifndef|#
directive|ifndef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
name|char
name|addr
index|[
name|INET6_ADDRSTRLEN
index|]
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|struct
name|sockaddr_in
name|from
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|socklen_t
name|fromlen
init|=
sizeof|sizeof
argument_list|(
name|from
argument_list|)
decl_stmt|;
name|char
modifier|*
name|reply
init|=
name|NULL
decl_stmt|;
name|size_t
name|reply_len
init|=
literal|0
decl_stmt|;
name|int
name|new_attached
init|=
literal|0
decl_stmt|;
name|u8
name|cookie
index|[
name|COOKIE_LEN
index|]
decl_stmt|;
name|res
operator|=
name|recvfrom
argument_list|(
name|sock
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
operator|-
literal|1
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|from
argument_list|,
operator|&
name|fromlen
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|<
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_ERROR
argument_list|,
literal|"recvfrom(ctrl_iface): %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
return|return;
block|}
ifndef|#
directive|ifndef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|inet_ntop
argument_list|(
name|AF_INET6
argument_list|,
operator|&
name|from
operator|.
name|sin6_addr
argument_list|,
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
name|from
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|os_strcmp
argument_list|(
name|addr
argument_list|,
literal|"::1"
argument_list|)
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL: Drop packet from unexpected source %s"
argument_list|,
name|addr
argument_list|)
expr_stmt|;
block|}
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
if|if
condition|(
name|from
operator|.
name|sin_addr
operator|.
name|s_addr
operator|!=
name|htonl
argument_list|(
operator|(
literal|127
operator|<<
literal|24
operator|)
operator||
literal|1
argument_list|)
condition|)
block|{
comment|/* 		 * The OS networking stack is expected to drop this kind of 		 * frames since the socket is bound to only localhost address. 		 * Just in case, drop the frame if it is coming from any other 		 * address. 		 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL: Drop packet from unexpected "
literal|"source %s"
argument_list|,
name|inet_ntoa
argument_list|(
name|from
operator|.
name|sin_addr
argument_list|)
argument_list|)
expr_stmt|;
return|return;
block|}
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|buf
index|[
name|res
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|os_strcmp
argument_list|(
name|buf
argument_list|,
literal|"GET_COOKIE"
argument_list|)
operator|==
literal|0
condition|)
block|{
name|reply
operator|=
name|wpa_supplicant_ctrl_iface_get_cookie
argument_list|(
name|priv
argument_list|,
operator|&
name|reply_len
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
comment|/* 	 * Require that the client includes a prefix with the 'cookie' value 	 * fetched with GET_COOKIE command. This is used to verify that the 	 * client has access to a bidirectional link over UDP in order to 	 * avoid attacks using forged localhost IP address even if the OS does 	 * not block such frames from remote destinations. 	 */
if|if
condition|(
name|os_strncmp
argument_list|(
name|buf
argument_list|,
literal|"COOKIE="
argument_list|,
literal|7
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTLR: No cookie in the request - "
literal|"drop request"
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|hexstr2bin
argument_list|(
name|buf
operator|+
literal|7
argument_list|,
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
operator|<
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTLR: Invalid cookie format in the "
literal|"request - drop request"
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|os_memcmp
argument_list|(
name|cookie
argument_list|,
name|priv
operator|->
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTLR: Invalid cookie in the request - "
literal|"drop request"
argument_list|)
expr_stmt|;
return|return;
block|}
name|pos
operator|=
name|buf
operator|+
literal|7
operator|+
literal|2
operator|*
name|COOKIE_LEN
expr_stmt|;
while|while
condition|(
operator|*
name|pos
operator|==
literal|' '
condition|)
name|pos
operator|++
expr_stmt|;
if|if
condition|(
name|os_strcmp
argument_list|(
name|pos
argument_list|,
literal|"ATTACH"
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|wpa_supplicant_ctrl_iface_attach
argument_list|(
name|priv
argument_list|,
operator|&
name|from
argument_list|,
name|fromlen
argument_list|)
condition|)
name|reply_len
operator|=
literal|1
expr_stmt|;
else|else
block|{
name|new_attached
operator|=
literal|1
expr_stmt|;
name|reply_len
operator|=
literal|2
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|os_strcmp
argument_list|(
name|pos
argument_list|,
literal|"DETACH"
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|wpa_supplicant_ctrl_iface_detach
argument_list|(
name|priv
argument_list|,
operator|&
name|from
argument_list|,
name|fromlen
argument_list|)
condition|)
name|reply_len
operator|=
literal|1
expr_stmt|;
else|else
name|reply_len
operator|=
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|os_strncmp
argument_list|(
name|pos
argument_list|,
literal|"LEVEL "
argument_list|,
literal|6
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|wpa_supplicant_ctrl_iface_level
argument_list|(
name|priv
argument_list|,
operator|&
name|from
argument_list|,
name|fromlen
argument_list|,
name|pos
operator|+
literal|6
argument_list|)
condition|)
name|reply_len
operator|=
literal|1
expr_stmt|;
else|else
name|reply_len
operator|=
literal|2
expr_stmt|;
block|}
else|else
block|{
name|reply
operator|=
name|wpa_supplicant_ctrl_iface_process
argument_list|(
name|wpa_s
argument_list|,
name|pos
argument_list|,
operator|&
name|reply_len
argument_list|)
expr_stmt|;
block|}
name|done
label|:
if|if
condition|(
name|reply
condition|)
block|{
name|sendto
argument_list|(
name|sock
argument_list|,
name|reply
argument_list|,
name|reply_len
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|from
argument_list|,
name|fromlen
argument_list|)
expr_stmt|;
name|os_free
argument_list|(
name|reply
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|reply_len
operator|==
literal|1
condition|)
block|{
name|sendto
argument_list|(
name|sock
argument_list|,
literal|"FAIL\n"
argument_list|,
literal|5
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|from
argument_list|,
name|fromlen
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|reply_len
operator|==
literal|2
condition|)
block|{
name|sendto
argument_list|(
name|sock
argument_list|,
literal|"OK\n"
argument_list|,
literal|3
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|from
argument_list|,
name|fromlen
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|new_attached
condition|)
name|eapol_sm_notify_ctrl_attached
argument_list|(
name|wpa_s
operator|->
name|eapol
argument_list|)
expr_stmt|;
block|}
specifier|static
name|void
name|wpa_supplicant_ctrl_iface_msg_cb
parameter_list|(
name|void
modifier|*
name|ctx
parameter_list|,
name|int
name|level
parameter_list|,
name|int
name|global
parameter_list|,
specifier|const
name|char
modifier|*
name|txt
parameter_list|,
name|size_t
name|len
parameter_list|)
block|{
name|struct
name|wpa_supplicant
modifier|*
name|wpa_s
init|=
name|ctx
decl_stmt|;
if|if
condition|(
name|wpa_s
operator|==
name|NULL
operator|||
name|wpa_s
operator|->
name|ctrl_iface
operator|==
name|NULL
condition|)
return|return;
name|wpa_supplicant_ctrl_iface_send
argument_list|(
name|wpa_s
operator|->
name|ctrl_iface
argument_list|,
name|level
argument_list|,
name|txt
argument_list|,
name|len
argument_list|)
expr_stmt|;
block|}
name|struct
name|ctrl_iface_priv
modifier|*
name|wpa_supplicant_ctrl_iface_init
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
name|wpa_s
parameter_list|)
block|{
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
decl_stmt|;
name|int
name|port
init|=
name|WPA_CTRL_IFACE_PORT
decl_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|struct
name|sockaddr_in6
name|addr
decl_stmt|;
name|int
name|domain
init|=
name|PF_INET6
decl_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|struct
name|sockaddr_in
name|addr
decl_stmt|;
name|int
name|domain
init|=
name|PF_INET
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|priv
operator|=
name|os_zalloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|priv
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|priv
operator|==
name|NULL
condition|)
return|return
name|NULL
return|;
name|priv
operator|->
name|wpa_s
operator|=
name|wpa_s
expr_stmt|;
name|priv
operator|->
name|sock
operator|=
operator|-
literal|1
expr_stmt|;
name|os_get_random
argument_list|(
name|priv
operator|->
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
expr_stmt|;
if|if
condition|(
name|wpa_s
operator|->
name|conf
operator|->
name|ctrl_interface
operator|==
name|NULL
condition|)
return|return
name|priv
return|;
name|priv
operator|->
name|sock
operator|=
name|socket
argument_list|(
name|domain
argument_list|,
name|SOCK_DGRAM
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|priv
operator|->
name|sock
operator|<
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_ERROR
argument_list|,
literal|"socket(PF_INET): %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
name|fail
goto|;
block|}
name|os_memset
argument_list|(
operator|&
name|addr
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|addr
argument_list|)
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|addr
operator|.
name|sin6_family
operator|=
name|AF_INET6
expr_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
name|addr
operator|.
name|sin6_addr
operator|=
name|in6addr_any
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|inet_pton
argument_list|(
name|AF_INET6
argument_list|,
literal|"::1"
argument_list|,
operator|&
name|addr
operator|.
name|sin6_addr
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|addr
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
name|addr
operator|.
name|sin_addr
operator|.
name|s_addr
operator|=
name|INADDR_ANY
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|addr
operator|.
name|sin_addr
operator|.
name|s_addr
operator|=
name|htonl
argument_list|(
operator|(
literal|127
operator|<<
literal|24
operator|)
operator||
literal|1
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|try_again
label|:
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|addr
operator|.
name|sin6_port
operator|=
name|htons
argument_list|(
name|port
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|addr
operator|.
name|sin_port
operator|=
name|htons
argument_list|(
name|port
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
if|if
condition|(
name|bind
argument_list|(
name|priv
operator|->
name|sock
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
name|addr
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
block|{
name|port
operator|--
expr_stmt|;
if|if
condition|(
operator|(
name|WPA_CTRL_IFACE_PORT
operator|-
name|port
operator|)
operator|<
name|WPA_CTRL_IFACE_PORT_LIMIT
condition|)
goto|goto
name|try_again
goto|;
name|wpa_printf
argument_list|(
name|MSG_ERROR
argument_list|,
literal|"bind(AF_INET): %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
name|fail
goto|;
block|}
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
name|wpa_msg
argument_list|(
name|wpa_s
argument_list|,
name|MSG_DEBUG
argument_list|,
literal|"ctrl_iface_init UDP port: %d"
argument_list|,
name|port
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|eloop_register_read_sock
argument_list|(
name|priv
operator|->
name|sock
argument_list|,
name|wpa_supplicant_ctrl_iface_receive
argument_list|,
name|wpa_s
argument_list|,
name|priv
argument_list|)
expr_stmt|;
name|wpa_msg_register_cb
argument_list|(
name|wpa_supplicant_ctrl_iface_msg_cb
argument_list|)
expr_stmt|;
return|return
name|priv
return|;
name|fail
label|:
if|if
condition|(
name|priv
operator|->
name|sock
operator|>=
literal|0
condition|)
name|close
argument_list|(
name|priv
operator|->
name|sock
argument_list|)
expr_stmt|;
name|os_free
argument_list|(
name|priv
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|void
name|wpa_supplicant_ctrl_iface_deinit
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|)
block|{
name|struct
name|wpa_ctrl_dst
modifier|*
name|dst
decl_stmt|,
modifier|*
name|prev
decl_stmt|;
if|if
condition|(
name|priv
operator|->
name|sock
operator|>
operator|-
literal|1
condition|)
block|{
name|eloop_unregister_read_sock
argument_list|(
name|priv
operator|->
name|sock
argument_list|)
expr_stmt|;
if|if
condition|(
name|priv
operator|->
name|ctrl_dst
condition|)
block|{
comment|/* 			 * Wait before closing the control socket if 			 * there are any attached monitors in order to allow 			 * them to receive any pending messages. 			 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE wait for attached "
literal|"monitors to receive messages"
argument_list|)
expr_stmt|;
name|os_sleep
argument_list|(
literal|0
argument_list|,
literal|100000
argument_list|)
expr_stmt|;
block|}
name|close
argument_list|(
name|priv
operator|->
name|sock
argument_list|)
expr_stmt|;
name|priv
operator|->
name|sock
operator|=
operator|-
literal|1
expr_stmt|;
block|}
name|dst
operator|=
name|priv
operator|->
name|ctrl_dst
expr_stmt|;
while|while
condition|(
name|dst
condition|)
block|{
name|prev
operator|=
name|dst
expr_stmt|;
name|dst
operator|=
name|dst
operator|->
name|next
expr_stmt|;
name|os_free
argument_list|(
name|prev
argument_list|)
expr_stmt|;
block|}
name|os_free
argument_list|(
name|priv
argument_list|)
expr_stmt|;
block|}
specifier|static
name|void
name|wpa_supplicant_ctrl_iface_send
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|,
name|int
name|level
parameter_list|,
specifier|const
name|char
modifier|*
name|buf
parameter_list|,
name|size_t
name|len
parameter_list|)
block|{
name|struct
name|wpa_ctrl_dst
modifier|*
name|dst
decl_stmt|,
modifier|*
name|next
decl_stmt|;
name|char
name|levelstr
index|[
literal|10
index|]
decl_stmt|;
name|int
name|idx
decl_stmt|;
name|char
modifier|*
name|sbuf
decl_stmt|;
name|int
name|llen
decl_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|char
name|addr
index|[
name|INET6_ADDRSTRLEN
index|]
decl_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|dst
operator|=
name|priv
operator|->
name|ctrl_dst
expr_stmt|;
if|if
condition|(
name|priv
operator|->
name|sock
operator|<
literal|0
operator|||
name|dst
operator|==
name|NULL
condition|)
return|return;
name|os_snprintf
argument_list|(
name|levelstr
argument_list|,
sizeof|sizeof
argument_list|(
name|levelstr
argument_list|)
argument_list|,
literal|"<%d>"
argument_list|,
name|level
argument_list|)
expr_stmt|;
name|llen
operator|=
name|os_strlen
argument_list|(
name|levelstr
argument_list|)
expr_stmt|;
name|sbuf
operator|=
name|os_malloc
argument_list|(
name|llen
operator|+
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
name|sbuf
operator|==
name|NULL
condition|)
return|return;
name|os_memcpy
argument_list|(
name|sbuf
argument_list|,
name|levelstr
argument_list|,
name|llen
argument_list|)
expr_stmt|;
name|os_memcpy
argument_list|(
name|sbuf
operator|+
name|llen
argument_list|,
name|buf
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|idx
operator|=
literal|0
expr_stmt|;
while|while
condition|(
name|dst
condition|)
block|{
name|next
operator|=
name|dst
operator|->
name|next
expr_stmt|;
if|if
condition|(
name|level
operator|>=
name|dst
operator|->
name|debug_level
condition|)
block|{
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_IPV6
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE monitor send %s:%d"
argument_list|,
name|inet_ntop
argument_list|(
name|AF_INET6
argument_list|,
operator|&
name|dst
operator|->
name|addr
operator|.
name|sin6_addr
argument_list|,
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
name|dst
operator|->
name|addr
argument_list|)
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|dst
operator|->
name|addr
operator|.
name|sin6_port
argument_list|)
argument_list|)
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE monitor send %s:%d"
argument_list|,
name|inet_ntoa
argument_list|(
name|dst
operator|->
name|addr
operator|.
name|sin_addr
argument_list|)
argument_list|,
name|ntohs
argument_list|(
name|dst
operator|->
name|addr
operator|.
name|sin_port
argument_list|)
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_IPV6 */
if|if
condition|(
name|sendto
argument_list|(
name|priv
operator|->
name|sock
argument_list|,
name|sbuf
argument_list|,
name|llen
operator|+
name|len
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|dst
operator|->
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
name|dst
operator|->
name|addr
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_ERROR
argument_list|,
literal|"sendto(CTRL_IFACE monitor): %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|dst
operator|->
name|errors
operator|++
expr_stmt|;
if|if
condition|(
name|dst
operator|->
name|errors
operator|>
literal|10
condition|)
block|{
name|wpa_supplicant_ctrl_iface_detach
argument_list|(
name|priv
argument_list|,
operator|&
name|dst
operator|->
name|addr
argument_list|,
name|dst
operator|->
name|addrlen
argument_list|)
expr_stmt|;
block|}
block|}
else|else
name|dst
operator|->
name|errors
operator|=
literal|0
expr_stmt|;
block|}
name|idx
operator|++
expr_stmt|;
name|dst
operator|=
name|next
expr_stmt|;
block|}
name|os_free
argument_list|(
name|sbuf
argument_list|)
expr_stmt|;
block|}
name|void
name|wpa_supplicant_ctrl_iface_wait
parameter_list|(
name|struct
name|ctrl_iface_priv
modifier|*
name|priv
parameter_list|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL_IFACE - %s - wait for monitor"
argument_list|,
name|priv
operator|->
name|wpa_s
operator|->
name|ifname
argument_list|)
expr_stmt|;
name|eloop_wait_for_read_sock
argument_list|(
name|priv
operator|->
name|sock
argument_list|)
expr_stmt|;
block|}
comment|/* Global ctrl_iface */
struct|struct
name|ctrl_iface_global_priv
block|{
name|int
name|sock
decl_stmt|;
name|u8
name|cookie
index|[
name|COOKIE_LEN
index|]
decl_stmt|;
block|}
struct|;
specifier|static
name|char
modifier|*
name|wpa_supplicant_global_get_cookie
parameter_list|(
name|struct
name|ctrl_iface_global_priv
modifier|*
name|priv
parameter_list|,
name|size_t
modifier|*
name|reply_len
parameter_list|)
block|{
name|char
modifier|*
name|reply
decl_stmt|;
name|reply
operator|=
name|os_malloc
argument_list|(
literal|7
operator|+
literal|2
operator|*
name|COOKIE_LEN
operator|+
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|reply
operator|==
name|NULL
condition|)
block|{
operator|*
name|reply_len
operator|=
literal|1
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|os_memcpy
argument_list|(
name|reply
argument_list|,
literal|"COOKIE="
argument_list|,
literal|7
argument_list|)
expr_stmt|;
name|wpa_snprintf_hex
argument_list|(
name|reply
operator|+
literal|7
argument_list|,
literal|2
operator|*
name|COOKIE_LEN
operator|+
literal|1
argument_list|,
name|priv
operator|->
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
expr_stmt|;
operator|*
name|reply_len
operator|=
literal|7
operator|+
literal|2
operator|*
name|COOKIE_LEN
expr_stmt|;
return|return
name|reply
return|;
block|}
specifier|static
name|void
name|wpa_supplicant_global_ctrl_iface_receive
parameter_list|(
name|int
name|sock
parameter_list|,
name|void
modifier|*
name|eloop_ctx
parameter_list|,
name|void
modifier|*
name|sock_ctx
parameter_list|)
block|{
name|struct
name|wpa_global
modifier|*
name|global
init|=
name|eloop_ctx
decl_stmt|;
name|struct
name|ctrl_iface_global_priv
modifier|*
name|priv
init|=
name|sock_ctx
decl_stmt|;
name|char
name|buf
index|[
literal|256
index|]
decl_stmt|,
modifier|*
name|pos
decl_stmt|;
name|int
name|res
decl_stmt|;
name|struct
name|sockaddr_in
name|from
decl_stmt|;
name|socklen_t
name|fromlen
init|=
sizeof|sizeof
argument_list|(
name|from
argument_list|)
decl_stmt|;
name|char
modifier|*
name|reply
decl_stmt|;
name|size_t
name|reply_len
decl_stmt|;
name|u8
name|cookie
index|[
name|COOKIE_LEN
index|]
decl_stmt|;
name|res
operator|=
name|recvfrom
argument_list|(
name|sock
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
operator|-
literal|1
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|from
argument_list|,
operator|&
name|fromlen
argument_list|)
expr_stmt|;
if|if
condition|(
name|res
operator|<
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_ERROR
argument_list|,
literal|"recvfrom(ctrl_iface): %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
return|return;
block|}
ifndef|#
directive|ifndef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
if|if
condition|(
name|from
operator|.
name|sin_addr
operator|.
name|s_addr
operator|!=
name|htonl
argument_list|(
operator|(
literal|127
operator|<<
literal|24
operator|)
operator||
literal|1
argument_list|)
condition|)
block|{
comment|/* 		 * The OS networking stack is expected to drop this kind of 		 * frames since the socket is bound to only localhost address. 		 * Just in case, drop the frame if it is coming from any other 		 * address. 		 */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTRL: Drop packet from unexpected "
literal|"source %s"
argument_list|,
name|inet_ntoa
argument_list|(
name|from
operator|.
name|sin_addr
argument_list|)
argument_list|)
expr_stmt|;
return|return;
block|}
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|buf
index|[
name|res
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|os_strcmp
argument_list|(
name|buf
argument_list|,
literal|"GET_COOKIE"
argument_list|)
operator|==
literal|0
condition|)
block|{
name|reply
operator|=
name|wpa_supplicant_global_get_cookie
argument_list|(
name|priv
argument_list|,
operator|&
name|reply_len
argument_list|)
expr_stmt|;
goto|goto
name|done
goto|;
block|}
if|if
condition|(
name|os_strncmp
argument_list|(
name|buf
argument_list|,
literal|"COOKIE="
argument_list|,
literal|7
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTLR: No cookie in the request - "
literal|"drop request"
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|hexstr2bin
argument_list|(
name|buf
operator|+
literal|7
argument_list|,
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
operator|<
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTLR: Invalid cookie format in the "
literal|"request - drop request"
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|os_memcmp
argument_list|(
name|cookie
argument_list|,
name|priv
operator|->
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"CTLR: Invalid cookie in the request - "
literal|"drop request"
argument_list|)
expr_stmt|;
return|return;
block|}
name|pos
operator|=
name|buf
operator|+
literal|7
operator|+
literal|2
operator|*
name|COOKIE_LEN
expr_stmt|;
while|while
condition|(
operator|*
name|pos
operator|==
literal|' '
condition|)
name|pos
operator|++
expr_stmt|;
name|reply
operator|=
name|wpa_supplicant_global_ctrl_iface_process
argument_list|(
name|global
argument_list|,
name|pos
argument_list|,
operator|&
name|reply_len
argument_list|)
expr_stmt|;
name|done
label|:
if|if
condition|(
name|reply
condition|)
block|{
name|sendto
argument_list|(
name|sock
argument_list|,
name|reply
argument_list|,
name|reply_len
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|from
argument_list|,
name|fromlen
argument_list|)
expr_stmt|;
name|os_free
argument_list|(
name|reply
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|reply_len
condition|)
block|{
name|sendto
argument_list|(
name|sock
argument_list|,
literal|"FAIL\n"
argument_list|,
literal|5
argument_list|,
literal|0
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|from
argument_list|,
name|fromlen
argument_list|)
expr_stmt|;
block|}
block|}
name|struct
name|ctrl_iface_global_priv
modifier|*
name|wpa_supplicant_global_ctrl_iface_init
parameter_list|(
name|struct
name|wpa_global
modifier|*
name|global
parameter_list|)
block|{
name|struct
name|ctrl_iface_global_priv
modifier|*
name|priv
decl_stmt|;
name|struct
name|sockaddr_in
name|addr
decl_stmt|;
name|int
name|port
init|=
name|WPA_GLOBAL_CTRL_IFACE_PORT
decl_stmt|;
name|priv
operator|=
name|os_zalloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|priv
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|priv
operator|==
name|NULL
condition|)
return|return
name|NULL
return|;
name|priv
operator|->
name|sock
operator|=
operator|-
literal|1
expr_stmt|;
name|os_get_random
argument_list|(
name|priv
operator|->
name|cookie
argument_list|,
name|COOKIE_LEN
argument_list|)
expr_stmt|;
if|if
condition|(
name|global
operator|->
name|params
operator|.
name|ctrl_interface
operator|==
name|NULL
condition|)
return|return
name|priv
return|;
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"Global control interface '%s'"
argument_list|,
name|global
operator|->
name|params
operator|.
name|ctrl_interface
argument_list|)
expr_stmt|;
name|priv
operator|->
name|sock
operator|=
name|socket
argument_list|(
name|PF_INET
argument_list|,
name|SOCK_DGRAM
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|priv
operator|->
name|sock
operator|<
literal|0
condition|)
block|{
name|wpa_printf
argument_list|(
name|MSG_ERROR
argument_list|,
literal|"socket(PF_INET): %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
name|fail
goto|;
block|}
name|os_memset
argument_list|(
operator|&
name|addr
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|addr
argument_list|)
argument_list|)
expr_stmt|;
name|addr
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
name|addr
operator|.
name|sin_addr
operator|.
name|s_addr
operator|=
name|INADDR_ANY
expr_stmt|;
else|#
directive|else
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|addr
operator|.
name|sin_addr
operator|.
name|s_addr
operator|=
name|htonl
argument_list|(
operator|(
literal|127
operator|<<
literal|24
operator|)
operator||
literal|1
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|try_again
label|:
name|addr
operator|.
name|sin_port
operator|=
name|htons
argument_list|(
name|port
argument_list|)
expr_stmt|;
if|if
condition|(
name|bind
argument_list|(
name|priv
operator|->
name|sock
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
name|addr
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
block|{
name|port
operator|++
expr_stmt|;
if|if
condition|(
operator|(
name|port
operator|-
name|WPA_GLOBAL_CTRL_IFACE_PORT
operator|)
operator|<
name|WPA_GLOBAL_CTRL_IFACE_PORT_LIMIT
condition|)
goto|goto
name|try_again
goto|;
name|wpa_printf
argument_list|(
name|MSG_ERROR
argument_list|,
literal|"bind(AF_INET): %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
name|fail
goto|;
block|}
ifdef|#
directive|ifdef
name|CONFIG_CTRL_IFACE_UDP_REMOTE
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"global_ctrl_iface_init UDP port: %d"
argument_list|,
name|port
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* CONFIG_CTRL_IFACE_UDP_REMOTE */
name|eloop_register_read_sock
argument_list|(
name|priv
operator|->
name|sock
argument_list|,
name|wpa_supplicant_global_ctrl_iface_receive
argument_list|,
name|global
argument_list|,
name|priv
argument_list|)
expr_stmt|;
return|return
name|priv
return|;
name|fail
label|:
if|if
condition|(
name|priv
operator|->
name|sock
operator|>=
literal|0
condition|)
name|close
argument_list|(
name|priv
operator|->
name|sock
argument_list|)
expr_stmt|;
name|os_free
argument_list|(
name|priv
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|void
name|wpa_supplicant_global_ctrl_iface_deinit
parameter_list|(
name|struct
name|ctrl_iface_global_priv
modifier|*
name|priv
parameter_list|)
block|{
if|if
condition|(
name|priv
operator|->
name|sock
operator|>=
literal|0
condition|)
block|{
name|eloop_unregister_read_sock
argument_list|(
name|priv
operator|->
name|sock
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|priv
operator|->
name|sock
argument_list|)
expr_stmt|;
block|}
name|os_free
argument_list|(
name|priv
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

