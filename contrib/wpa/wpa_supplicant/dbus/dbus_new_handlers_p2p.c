...
argument_list|,
literal|"There is no such persistent group in this P2P device."
argument_list|)
return|;
block|}
end_function
...
name|WPAS_DBUS_ERROR_UNKNOWN_ERROR
argument_list|,
literal|"Could not start P2P listen"
argument_list|)
return|;
block|}
return|return
...
name|err_msg
operator|=
literal|"connect failed due to channel unavailability."
expr_stmt|;
name|iface
operator|=
...
begin_comment
comment|/**  * wpas_dbus_handler_p2p_cancel - Cancel P2P group formation  * @message: Pointer to incoming dbus message  * @wpa_s: %wpa_supplicant data structure  * Returns: NULL on success or DBus error on failure  *  * Handler for "Cancel" method call. Returns NULL if P2P cancel succeeds or DBus  * error on P2P cancel failure  */
end_comment
...
name|DBUS_ERROR_FAILED
argument_list|,
literal|"%s: failed to construct message 2 (%d)"
argument_list|,
name|__func__
argument_list|,
name|i
...
specifier|static
name|int
name|match_group_where_peer_is_client
parameter_list|(
name|struct
name|p2p_group
modifier|*
...
begin_comment
comment|/**  * wpas_dbus_setter_persistent_group_properties - Set options for a persistent  *	group  * @iter: Pointer to incoming dbus message iter  * @error: Location to store error on failure  * @user_data: Function specific data  * Returns: TRUE on success, FALSE on failure  *  * Setter for "Properties" property of a persistent group.  */
end_comment
...
name|MSG_ERROR
argument_list|,
literal|"dbus: %s: Cannot add new persistent group"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
name|message
argument_list|,
literal|"wpa_supplicant could not add a persistent group on this interface."
argument_list|)
expr_stmt|;
goto|goto
name|err
...
name|MSG_DEBUG
argument_list|,
literal|"dbus: %s: Control interface could not set persistent group properties"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
name|DBUS_ERROR_INVALID_ARGS
argument_list|,
literal|"Failed to set network properties"
argument_list|)
expr_stmt|;
...
name|MSG_ERROR
argument_list|,
literal|"dbus: %s: error occurred when removing persistent group %d"
argument_list|,
name|__func__
argument_list|,
name|id
...
name|message
argument_list|,
literal|"error removing the specified persistent group on this interface."
argument_list|)
expr_stmt|;
goto|goto
name|out
...
name|FALSE
return|;
comment|/* 	 * This is supposed to be array of bytearrays (aay), but the earlier 	 * implementation used a dict with "WPSVendorExtensions" as the key in 	 * this setter function which does not match the format used by the 	 * getter function. For backwards compatibility, allow both formats to 	 * be used in the setter. 	 */
if|if
condition|(
...
condition|)
block|{
comment|/* This is the proper format matching the getter */
name|struct
name|wpabuf
modifier|*
name|vals
...
name|MSG_DEBUG
argument_list|,
literal|"dbus: Not an array of array of bytes"
argument_list|)
expr_stmt|;
return|return
name|FALSE
...
name|MSG_DEBUG
argument_list|,
literal|"dbus: Too many WPSVendorExtensions values"
argument_list|)
expr_stmt|;
name|i
operator|=
...
name|MSG_DEBUG
argument_list|,
literal|"dbus: WPSVendorExtentions[]"
argument_list|,
name|val
argument_list|,
name|len
...
name|MSG_DEBUG
argument_list|,
literal|"dbus: Try to use backwards compatibility version of WPSVendorExtensions setter"
argument_list|)
expr_stmt|;
if|if
condition|(
