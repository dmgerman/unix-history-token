...
begin_comment
comment|/*  * WPA Supplicant - Driver event processing  * Copyright (c) 2003-2015, Jouni Malinen<j@w1.fi>  *  * This software may be distributed under the terms of the BSD license.  * See README for more details.  */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"Try to associate due to network getting re-enabled"
argument_list|)
expr_stmt|;
if|if
condition|(
...
specifier|static
name|void
name|wpa_supplicant_update_current_bss
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
literal|0
return|;
comment|/* current profile still in use */
name|res
operator|=
...
name|MSG_INFO
argument_list|,
literal|"Failed to read SSID from driver"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
comment|/* try to use current profile */
block|}
name|drv_ssid_len
operator|=
name|res
...
name|MSG_DEBUG
argument_list|,
literal|"Driver-initiated BSS selection changed the SSID to %s"
argument_list|,
name|wpa_ssid_txt
argument_list|(
name|drv_ssid
...
argument_list|)
expr_stmt|;
comment|/* continue selecting a new network profile */
block|}
name|wpa_dbg
argument_list|(
name|wpa_s
...
name|MSG_DEBUG
argument_list|,
literal|"Could not set WPA suites"
argument_list|)
expr_stmt|;
block|}
else|else
...
argument_list|)
expr_stmt|;
comment|/* 		 * It is possible that the device is sched scanning, which means 		 * that a connection attempt will be done only when we receive 		 * scan results. However, in this case, it would be preferable 		 * to scan and connect immediately, so cancel the sched_scan and 		 * issue a regular scan flow. 		 */
name|wpa_supplicant_cancel_sched_scan
argument_list|(
name|wpa_s
argument_list|)
...
continue|continue;
block|}
comment|/* There's also a VHT selector for 802.11ac */
if|if
condition|(
name|flagged
operator|&&
...
name|MSG_DEBUG
argument_list|,
literal|"   hardware does not support required rate %d.%d Mbps (freq=%d mode==%d num_rates=%d)"
argument_list|,
name|r
operator|/
literal|10
...
begin_comment
comment|/*  * Test whether BSS is in an ESS.  * This is done differently in DMG (60 GHz) and non-DMG bands  */
end_comment
...
name|MACSTR
literal|" ssid='%s' "
literal|"wpa_ie_len=%u rsn_ie_len=%u caps=0x%x level=%d freq=%d %s%s%s"
argument_list|,
name|i
argument_list|,
name|MAC2STR
...
name|MSG_DEBUG
argument_list|,
literal|"   skip - BSSID blacklisted"
argument_list|)
expr_stmt|;
continue|continue;
block|}
comment|/* if there is a whitelist, only accept those APs */
if|if
condition|(
name|ssid
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"   skip - BSSID not in whitelist"
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"   skip - ignore WPA/WPA2 AP for WEP network block"
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"   skip - could not fetch P2P element"
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"   skip - no matching GO P2P Device Address in P2P element"
argument_list|)
expr_stmt|;
name|wpabuf_free
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"   skip - scan result not recent enough (%u.%06u seconds too old)"
argument_list|,
operator|(
name|unsigned
name|int
...
name|MSG_DEBUG
argument_list|,
literal|"Try to find BSS matching pre-selected network id=%d"
argument_list|,
name|group
operator|->
name|id
...
condition|)
block|{
comment|/* check that next_ssid is still valid */
for|for
control|(
name|ssid
operator|=
...
name|MSG_DEBUG
argument_list|,
literal|"PSK/passphrase not yet available for the selected network"
argument_list|)
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"Considering connect request: reassociate: %d  selected: "
name|MACSTR
literal|"  bssid: "
name|MACSTR
literal|"  pending: "
name|MACSTR
literal|"  wpa_state: %s  ssid=%p  current_ssid=%p"
argument_list|,
name|wpa_s
operator|->
name|reassociate
...
name|MSG_DEBUG
argument_list|,
literal|"Request association with "
name|MACSTR
argument_list|,
name|MAC2STR
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"Already associated or trying to "
literal|"connect with the selected AP"
argument_list|)
expr_stmt|;
block|}
return|return
...
literal|"Current BSS: "
name|MACSTR
literal|" level=%d snr=%d est_throughput=%u"
argument_list|,
name|MAC2STR
argument_list|(
name|current_bss
...
name|MSG_DEBUG
argument_list|,
literal|"Allow reassociation - selected BSS has better estimated throughput"
argument_list|)
expr_stmt|;
return|return
literal|1
...
name|MSG_DEBUG
argument_list|,
literal|"New scan results available (own=%u ext=%u)"
argument_list|,
name|wpa_s
operator|->
name|own_scan_running
...
name|MSG_DEBUG
argument_list|,
literal|"Do not use results from externally requested scan operation for network selection"
argument_list|)
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"Postpone network selection by %d seconds since all networks are disabled"
argument_list|,
name|time_to_reenable
argument_list|)
expr_stmt|;
...
literal|0
return|;
comment|/* no normal connection on p2p_mgmt interface */
name|selected
operator|=
name|wpa_supplicant_pick_network
argument_list|(
...
name|MSG_INFO
argument_list|,
literal|"Avoiding join because we already joined a mesh group"
argument_list|)
expr_stmt|;
return|return
literal|0
...
condition|)
block|{
comment|/* 			 * No SSID found. If SCAN results are as a result of 			 * own scan request and not due to a scan request on 			 * another shared interface, try another scan. 			 */
name|int
name|timeout_sec
init|=
name|wpa_s
...
condition|)
block|{
comment|/* 				 * Use shorter wait during P2P Provisioning 				 * state and during P2P join-a-group operation 				 * to speed up group formation. 				 */
name|timeout_sec
operator|=
literal|0
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"Use shorter wait during WPS processing"
argument_list|)
expr_stmt|;
name|timeout_sec
operator|=
...
condition|)
block|{
comment|/* 		 * Interface may have been removed, so must not dereference 		 * wpa_s after this. 		 */
return|return
literal|1
return|;
block|}
...
return|;
block|}
comment|/* 	 * Check other interfaces to see if they share the same radio. If 	 * so, they get updated with this same scan info. 	 */
name|dl_list_for_each
argument_list|(
argument|ifs
argument_list|,
...
name|MSG_DEBUG
argument_list|,
literal|"Fast associate: Old scan results"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"Interworking: QoS Map Set"
argument_list|,
name|qos_map
argument_list|,
name|len
...
name|MSG_DEBUG
argument_list|,
literal|"Interworking: Failed to configure QoS Map Set to the driver"
argument_list|)
expr_stmt|;
block|}
return|return
...
specifier|static
name|void
name|interworking_process_assoc_resp
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
directive|endif
comment|/* CONFIG_SME */
comment|/* Process FT when SME is in the driver */
if|if
condition|(
operator|!
operator|(
...
name|MSG_DEBUG
argument_list|,
literal|"FT: Reassociation Response done"
argument_list|)
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* 			 * Set the key after having received joined-IBSS event 			 * from the driver. 			 */
...
literal|"reconnect (wps=%d/%d wpa_state=%d)"
argument_list|,
name|wpa_s
operator|->
name|key_mgmt
...
condition|)
block|{
comment|/* 		 * Could not reconnect to the same BSS due to network being 		 * disabled. Use a new scan to match the alternative behavior 		 * above, i.e., to continue automatic reconnection attempt in a 		 * way that enforces disabled network rules. 		 */
name|wpa_supplicant_req_scan
argument_list|(
name|wpa_s
argument_list|,
...
name|MSG_DEBUG
argument_list|,
literal|"Removing P2P Device interface"
argument_list|)
expr_stmt|;
...
specifier|static
name|void
name|wpa_supplicant_event_ibss_auth
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
condition|)
return|return;
comment|/* check if the ssid is correctly configured as IBSS/RSN */
if|if
condition|(
name|ssid
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"Ignore deauth event in AP mode"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"Ignore disassoc event in AP mode"
argument_list|)
expr_stmt|;
return|return;
block|}
...
specifier|static
name|void
name|wpa_supplicant_update_channel_list
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
expr_stmt|;
block|}
comment|/* Restart sched_scan with updated channel list */
if|if
condition|(
name|wpa_s
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"Channel list changed restart sched scan."
argument_list|)
expr_stmt|;
name|wpa_supplicant_cancel_sched_scan
argument_list|(
...
specifier|static
name|void
name|wpas_event_rx_mgmt_action
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
name|MSG_DEBUG
argument_list|,
literal|"TDLS: Received Discovery Response from "
name|MACSTR
argument_list|,
name|MAC2STR
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"Interworking: Received QoS Map Configure frame from "
name|MACSTR
argument_list|,
name|MAC2STR
argument_list|(
...
specifier|static
name|void
name|wpa_supplicant_notify_avoid_freq
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
name|MSG_ERROR
argument_list|,
literal|"%s: Failed to parse freq range"
argument_list|,
name|__func__
argument_list|)
expr_stmt|;
...
name|MSG_DEBUG
argument_list|,
literal|"P2P: Update channel list based on frequency avoid event"
argument_list|)
expr_stmt|;
comment|/* 		 * The update channel flow will also take care of moving a GO 		 * from the unsafe frequency if needed. 		 */
name|wpas_p2p_update_channel_list
argument_list|(
name|wpa_s
argument_list|,
...
specifier|static
name|void
name|wpa_supplicant_event_assoc_auth
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
name|MSG_DEBUG
argument_list|,
literal|"Connection authorized by device, previous state %d"
argument_list|,
name|wpa_s
operator|->
name|wpa_state
...
name|MSG_DEBUG
argument_list|,
literal|"Own scan request started a scan in %ld.%06ld seconds"
argument_list|,
name|diff
operator|.
name|sec
...
name|MSG_DEBUG
argument_list|,
literal|"External program started a scan"
argument_list|)
expr_stmt|;
name|wpa_s
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"Scan completed in %ld.%06ld seconds"
argument_list|,
name|diff
operator|.
name|sec
...
condition|)
break|break;
comment|/* interface may have been removed */
name|wpa_s
operator|->
name|own_scan_running
operator|=
...
case|:
comment|/* It is possible to get this event from earlier connection */
if|if
condition|(
name|wpa_s
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"Ignore AUTH_TIMED_OUT in mesh configuration"
argument_list|)
expr_stmt|;
break|break;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"Ignore ASSOC_TIMED_OUT in mesh configuration"
argument_list|)
expr_stmt|;
break|break;
block|}
...
name|MSG_INFO
argument_list|,
literal|"MGMT-RX freq=%d datarate=%u ssi_signal=%d %s"
argument_list|,
name|rx
operator|->
name|freq
...
literal|"above=%d signal=%d noise=%d txrate=%d"
argument_list|,
name|data
operator|->
name|signal_change
...
condition|)
block|{
comment|/* 			 * Mark interface disabled if this happens to end up not 			 * being removed as a separate P2P group interface. 			 */
name|wpa_supplicant_set_state
argument_list|(
name|wpa_s
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 			 * The interface was externally disabled. Remove 			 * it assuming an external entity will start a 			 * new session if needed. 			 */
if|if
condition|(
name|wpa_s
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 			 * wpa_s instance may have been freed, so must not use 			 * it here anymore. 			 */
break|break;
block|}
if|if
condition|(
...
condition|)
block|{
comment|/* This radio work will be cancelled, so clear P2P 			 * state as well. 			 */
name|p2p_stop_find
argument_list|(
name|wpa_s
operator|->
...
condition|)
block|{
comment|/* 			 * Indicate disconnection to keep ctrl_iface events 			 * consistent. 			 */
name|wpa_supplicant_event_disassoc
argument_list|(
name|wpa_s
argument_list|,
...
condition|)
break|break;
comment|/* 		 * Start a new sched scan to continue searching for more SSIDs 		 * either if timed out or PNO schedule scan is pending. 		 */
if|if
condition|(
name|wpa_s
operator|->
