...
name|VERSION_STR
literal|"\n"
literal|"Copyright (c) 2003-2015, Jouni Malinen<j@w1.fi> and contributors"
decl_stmt|;
end_decl_stmt
...
condition|)
block|{
comment|/* Assure normal scan when interface is restored */
name|wpa_s
operator|->
name|normal_scans
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Reinitialize normal_scan counter */
name|wpa_s
operator|->
name|normal_scans
operator|=
...
directive|ifdef
name|CONFIG_P2P
comment|/* 	 * P2PS client has to reply to Probe Request frames received on the 	 * group operating channel. Enable Probe Request frame reporting for 	 * P2P connected client in case p2p_cli_probe configuration property is 	 * set to 1. 	 */
if|if
condition|(
name|wpa_s
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"P2P: Enable CLI Probe Request RX reporting"
argument_list|)
expr_stmt|;
name|wpa_s
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"P2P: Disable CLI Probe Request RX reporting"
argument_list|)
expr_stmt|;
name|wpa_s
operator|->
...
argument_list|)
expr_stmt|;
comment|/* 		 * Notify the P2P Device interface about a state change in one 		 * of the interfaces. 		 */
...
expr_stmt|;
block|}
comment|/* 	 * TODO: should notify EAPOL SM about changes in opensc_engine_path, 	 * pkcs11_engine_path, pkcs11_module_path, openssl_ciphers. 	 */
if|if
condition|(
...
name|MSG_DEBUG
argument_list|,
literal|"WPA: ssid proto=0x%x pairwise_cipher=0x%x group_cipher=0x%x key_mgmt=0x%x"
argument_list|,
name|ssid
operator|->
name|proto
...
name|MSG_DEBUG
argument_list|,
literal|"Could not parse RSN element"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|MSG_DEBUG
argument_list|,
literal|"RSN: pairwise_cipher=0x%x group_cipher=0x%x key_mgmt=0x%x"
argument_list|,
name|ie
operator|.
name|pairwise_cipher
...
name|MSG_DEBUG
argument_list|,
literal|"Could not parse WPA element"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|MSG_DEBUG
argument_list|,
literal|"WPA: pairwise_cipher=0x%x group_cipher=0x%x key_mgmt=0x%x"
argument_list|,
name|ie
operator|.
name|pairwise_cipher
...
name|MSG_DEBUG
argument_list|,
literal|"WPA: using KEY_MGMT 802.1X with Suite B (192-bit)"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|MSG_DEBUG
argument_list|,
literal|"WPA: using KEY_MGMT 802.1X with Suite B"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|MSG_DEBUG
argument_list|,
literal|"HS 2.0: using KEY_MGMT OSEN"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|MSG_INFO
argument_list|,
literal|"No PSK available for association"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_INFO
argument_list|,
literal|"Not enough room for building extended capabilities element"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"Previously selected random MAC address has not yet expired"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|MSG_INFO
argument_list|,
literal|"Failed to set random MAC address"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_INFO
argument_list|,
literal|"Could not update MAC address information"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"Using random MAC address "
name|MACSTR
argument_list|,
name|MAC2STR
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"Re-association to the same ESS"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|MSG_INFO
argument_list|,
literal|"Could not restore permanent MAC address"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"Using permanent MAC address"
argument_list|)
expr_stmt|;
block|}
name|wpa_s
...
name|MSG_INFO
argument_list|,
literal|"Driver does not support mesh mode"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_ERROR
argument_list|,
literal|"mesh mode support not included in the build"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|MSG_DEBUG
argument_list|,
literal|"Reject wpa_supplicant_associate() call since connect_work exist"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"Reject wpa_supplicant_associate() call since pending work exist"
argument_list|)
expr_stmt|;
return|return;
block|}
...
condition|)
break|break;
comment|/* Don't adjust control freq in case of fixed_freq */
if|if
condition|(
name|ssid
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"IBSS already found in scan results, adjust control freq: %d"
argument_list|,
name|bss
operator|->
name|freq
...
block|}
block|}
comment|/* For IBSS check HT_IBSS flag */
if|if
condition|(
name|ssid
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"IBSS: WEP/TKIP detected, do not try to enable HT"
argument_list|)
expr_stmt|;
return|return;
block|}
...
condition|)
return|return;
comment|/* Setup higher BW only for 5 GHz */
if|if
condition|(
name|mode
operator|->
...
condition|)
return|return;
comment|/* Check primary channel flags */
if|if
condition|(
name|pri_chan
operator|->
...
block|}
block|}
comment|/* Find secondary channel */
for|for
control|(
name|i
operator|=
...
condition|)
return|return;
comment|/* Check secondary channel flags */
if|if
condition|(
name|sec_chan
operator|->
...
literal|1
case|:
comment|/* Configuration allowed */
break|break;
case|case
literal|2
case|:
comment|/* Switch pri/sec channels */
name|freq
operator|->
name|freq
operator|=
...
name|MSG_DEBUG
argument_list|,
literal|"IBSS/mesh: setup freq channel %d, sec_channel_offset %d"
argument_list|,
name|freq
operator|->
name|channel
...
argument_list|)
expr_stmt|;
comment|/* Not sure if mesh is ready for VHT */
if|if
condition|(
name|ssid
operator|->
...
condition|)
return|return;
comment|/* For IBSS check VHT_IBSS flag */
if|if
condition|(
operator|!
operator|(
...
condition|)
return|return;
comment|/* setup center_freq1, bandwidth */
for|for
control|(
name|j
operator|=
...
condition|)
return|return;
comment|/* Back to HT configuration if channel not usable */
if|if
condition|(
name|chan
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"IBSS: VHT setup freq cf1 %d, cf2 %d, bw %d"
argument_list|,
name|freq
operator|->
name|center_freq1
...
name|NULL
expr_stmt|;
comment|/* cancel possible auth. timeout */
name|eloop_cancel_timeout
argument_list|(
name|wpa_supplicant_timeout
argument_list|,
...
name|MSG_DEBUG
argument_list|,
literal|"BSS/SSID entry for association not valid anymore - drop connection attempt"
argument_list|)
expr_stmt|;
name|wpas_connect_work_done
argument_list|(
...
directive|endif
comment|/* CONFIG_HS20 */
comment|/* 	 * Workaround: Add Extended Capabilities element only if the AP 	 * included this element in Beacon/Probe Response frames. Some older 	 * APs seem to have interoperability issues if this element is 	 * included, so while the standard may require us to include the 	 * element in all cases, it is justifiable to skip it to avoid 	 * interoperability issues. 	 */
if|if
condition|(
operator|!
name|bss
...
expr_stmt|;
block|}
comment|/* Initial frequency for IBSS/mesh */
if|if
condition|(
operator|(
name|ssid
...
name|MSG_DEBUG
argument_list|,
literal|"Assoc conflicting freq found (%d != %d)"
argument_list|,
name|freq
argument_list|,
name|params
...
specifier|static
name|void
name|wpa_supplicant_enable_one_network
parameter_list|(
name|struct
name|wpa_supplicant
modifier|*
...
name|MSG_DEBUG
argument_list|,
literal|"Stop ongoing sched_scan to add "
literal|"new network to scan filters"
argument_list|)
expr_stmt|;
name|wpa_supplicant_cancel_sched_scan
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"Stop ongoing sched_scan "
literal|"to remove network from filters"
argument_list|)
expr_stmt|;
name|wpa_supplicant_cancel_sched_scan
argument_list|(
...
name|NULL
expr_stmt|;
comment|/* 		 * Don't optimize next scan freqs since a new ESS has been 		 * selected. 		 */
name|os_free
argument_list|(
name|wpa_s
operator|->
...
begin_comment
comment|/**  * wpas_set_pkcs11_engine_and_module_path - Set PKCS #11 engine and module path  * @wpa_s: wpa_supplicant structure for a network interface  * @pkcs11_engine_path: PKCS #11 engine path or NULL  * @pkcs11_module_path: PKCS #11 module path or NULL  * Returns: 0 on success; -1 on failure  *  * Sets the PKCS #11 engine and module path. Both have to be NULL or a valid  * path. If resetting the EAPOL state machine with the new PKCS #11 engine and  * module path fails the paths will be reset to the default value (NULL).  */
end_comment
...
name|char
modifier|*
name|pkcs11_engine_path_copy
init|=
name|NULL
decl_stmt|;
name|char
modifier|*
name|pkcs11_module_path_copy
init|=
name|NULL
decl_stmt|;
if|if
...
condition|)
block|{
comment|/* Error -> Reset paths to the default value (NULL) once. */
if|if
condition|(
name|pkcs11_engine_path
operator|!=
...
name|MSG_INFO
argument_list|,
literal|"Note: nl80211 driver interface is not designed to be used with ap_scan=2; this can result in connection failures"
argument_list|)
expr_stmt|;
block|}
ifdef|#
...
name|MSG_DEBUG
argument_list|,
literal|"RSN: Processed PeerKey EAPOL-Key"
argument_list|)
expr_stmt|;
return|return;
block|}
...
condition|)
block|{
comment|/* Use longer timeout for IEEE 802.1X/EAP */
name|timeout
operator|=
literal|70
expr_stmt|;
...
condition|)
block|{
comment|/* 			 * Use shorter timeout if going through WPS AP iteration 			 * for PIN config method with an AP that does not 			 * advertise Selected Registrar. 			 */
name|struct
name|wpabuf
modifier|*
name|wps_ie
...
parameter_list|)
block|{
comment|/* Masking these out disables LDPC */
name|le16
name|msk
init|=
name|host_to_le16
...
directive|ifdef
name|CONFIG_HT_OVERRIDES
comment|/* if max ampdu is<= 3, we have to make the HT cap the same */
if|if
condition|(
name|ssid
operator|->
...
parameter_list|)
define|\
value|if (ssid->vht_tx_mcs_nss_ ##i>= 0) {				\ 		vhtcaps_mask->vht_supported_mcs_set.tx_map |=		\ 			3<< 2 * (i - 1);				\ 		vhtcaps->vht_supported_mcs_set.tx_map |=		\ 			ssid->vht_tx_mcs_nss_ ##i<< 2 * (i - 1);	\ 	}								\ 	if (ssid->vht_rx_mcs_nss_ ##i>= 0) {				\ 		vhtcaps_mask->vht_supported_mcs_set.rx_map |=		\ 			3<< 2 * (i - 1);				\ 		vhtcaps->vht_supported_mcs_set.rx_map |=		\ 			ssid->vht_rx_mcs_nss_ ##i<< 2 * (i - 1);	\ 	}
name|OVERRIDE_MCS
argument_list|(
literal|1
argument_list|)
...
specifier|static
name|void
name|wpas_fst_get_channel_info_cb
parameter_list|(
name|void
modifier|*
name|ctx
...
name|MSG_DEBUG
argument_list|,
literal|"Add interface %s to existing radio %s"
argument_list|,
name|wpa_s
operator|->
name|ifname
...
name|MSG_DEBUG
argument_list|,
literal|"Add interface %s to a new radio %s"
argument_list|,
name|wpa_s
operator|->
name|ifname
...
condition|)
block|{
comment|/* This should not really happen. */
name|wpa_dbg
argument_list|(
name|work
operator|->
...
name|MSG_INFO
argument_list|,
literal|"Freeing radio work '%s'@%p (started=%d) that is marked as scan_work"
argument_list|,
name|work
operator|->
name|type
...
name|MSG_INFO
argument_list|,
literal|"Freeing radio work '%s'@%p (started=%d) that is marked as p2p_scan_work"
argument_list|,
name|work
operator|->
name|type
...
condition|)
return|return;
comment|/* already started and still in progress */
name|wpa_s
operator|=
name|dl_list_first
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"Delay radio work start until externally triggered scan completes"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"Starting radio work '%s'@%p after %ld.%06ld second wait"
argument_list|,
name|work
operator|->
name|type
...
begin_comment
comment|/*  * This function removes both started and pending radio works running on  * the provided interface's radio.  * Prior to the removal of the radio work, its callback (cb) is called with  * deinit set to be 1. Each work's callback is responsible for clearing its  * internal data and restoring to a correct state.  * @wpa_s: wpa_supplicant data  * @type: type of works to be removed  * @remove_all: 1 to remove all the works on this radio, 0 to remove only  * this interface's works.  */
end_comment
...
condition|)
continue|continue;
comment|/* skip other ifaces' works */
if|if
condition|(
operator|!
name|remove_all
...
name|MSG_DEBUG
argument_list|,
literal|"Remove radio work '%s'@%p%s"
argument_list|,
name|work
operator|->
name|type
...
expr_stmt|;
block|}
comment|/* in case we removed the started work */
...
name|MSG_DEBUG
argument_list|,
literal|"Remove interface %s from radio %s"
argument_list|,
name|wpa_s
operator|->
name|ifname
...
condition|)
return|return;
comment|/* Interfaces remain for this radio */
name|wpa_printf
argument_list|(
name|MSG_DEBUG
argument_list|,
literal|"Remove radio %s"
argument_list|,
name|radio
operator|->
name|name
...
name|MSG_DEBUG
argument_list|,
literal|"External radio work in progress - delay start of pending item"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"First radio work item in the queue - schedule start immediately"
argument_list|)
expr_stmt|;
name|radio_work_check_next
argument_list|(
...
begin_comment
comment|/**  * radio_work_done - Indicate that a radio work item has been completed  * @work: Completed work  *  * This function is called once the callback function registered with  * radio_add_work() has completed its work.  */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"Radio work '%s'@%p %s in %ld.%06ld seconds"
argument_list|,
name|work
operator|->
name|type
...
literal|1000
expr_stmt|;
comment|/* 	 * Only take p2p_mgmt parameters when P2P Device is supported. 	 * Doing it here as it determines whether l2_packet_init() will be done 	 * during wpa_supplicant_driver_init(). 	 */
if|if
condition|(
name|wpa_s
operator|->
...
name|MSG_ERROR
argument_list|,
literal|"FST: Cannot attach iface %s to group %s"
argument_list|,
name|wpa_s
operator|->
name|ifname
...
literal|1
return|;
comment|/* 	 * Set Wake-on-WLAN triggers, if configured. 	 * Note: We don't restore/remove the triggers on shutdown (it doesn't 	 * have effect anyway when the interface is down). 	 */
if|if
condition|(
name|capa_res
operator|==
...
name|MSG_DEBUG
argument_list|,
literal|"Remove remaining child interface %s from parent %s"
argument_list|,
name|iface
operator|->
name|ifname
...
name|MSG_INFO
argument_list|,
literal|"P2P: Failed to enable P2P Device interface"
argument_list|)
expr_stmt|;
comment|/* Try to continue without. P2P will be disabled. */
block|}
endif|#
directive|endif
comment|/* CONFIG_P2P */
...
name|MSG_ERROR
argument_list|,
literal|"mesh: Failed to malloc ifname"
argument_list|)
expr_stmt|;
return|return
operator|-
...
ifndef|#
directive|ifndef
name|WPA_SUPPLICANT_CLEANUP_INTERVAL
end_ifndef
...
begin_comment
comment|/* WPA_SUPPLICANT_CLEANUP_INTERVAL */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 	 * There is no point in blacklisting the AP if this event is 	 * generated based on local request to disconnect. 	 */
if|if
condition|(
name|wpa_s
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"Ignore connection failure due to local request to disconnect"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"Ignore connection failure "
literal|"indication since interface has been put into "
literal|"disconnected state"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"Continuous association failures - "
literal|"consider temporary network disabling"
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/* 			 * Driver does not support BIP -- ignore pmf=1 default 			 * since the connection with PMF would fail and the 			 * configuration does not require PMF to be enabled. 			 */
return|return
name|NO_MGMT_FRAME_PROTECTION
return|;
block|}
...
condition|)
block|{
comment|/* 		 * Skip the wait time since there is a short timeout on the 		 * connection to a P2P group. 		 */
return|return;
block|}
endif|#
directive|endif
...
literal|"id=%d ssid=\"%s\" auth_failures=%u duration=%d reason=%s"
argument_list|,
name|ssid
operator|->
name|id
...
name|MSG_DEBUG
argument_list|,
literal|"Shared frequencies (len=%u): %s"
argument_list|,
name|len
argument_list|,
name|title
...
begin_comment
comment|/*  * Find the operating frequencies of any of the virtual interfaces that  * are using the same radio as the current interface, and in addition, get  * information about the interface types that are using the frequency.  */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"Determining shared radio frequencies (max len %u)"
argument_list|,
name|len
argument_list|)
expr_stmt|;
...
continue|continue;
comment|/* Hold only distinct freqs */
for|for
control|(
name|i
operator|=
...
begin_comment
comment|/*  * Find the operating frequencies of any of the virtual interfaces that  * are using the same radio as the current interface.  */
end_comment
...
specifier|static
name|void
name|wpas_rrm_neighbor_rep_timeout_handler
parameter_list|(
name|void
modifier|*
name|data
...
name|MSG_ERROR
argument_list|,
literal|"RRM: Unexpected neighbor report timeout"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Notifying neighbor report - NONE"
argument_list|)
expr_stmt|;
name|rrm
operator|->
...
begin_comment
comment|/*  * wpas_rrm_reset - Clear and reset all RRM data in wpa_supplicant  * @wpa_s: Pointer to wpa_supplicant  */
end_comment
...
begin_comment
comment|/*  * wpas_rrm_process_neighbor_rep - Handle incoming neighbor report  * @wpa_s: Pointer to wpa_supplicant  * @report: Neighbor report buffer, prefixed by a 1-byte dialog token  * @report_len: Length of neighbor report buffer  */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: New Neighbor Report"
argument_list|,
name|report
argument_list|,
name|report_len
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Discarding neighbor report with token %d (expected %d)"
argument_list|,
name|report
index|[
literal|0
...
name|MSG_ERROR
argument_list|,
literal|"RRM: Unexpected neighbor report"
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* skipping the first byte, which is only an id (dialog token) */
name|neighbor_rep
operator|=
name|wpabuf_alloc
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Notifying neighbor report (token = %d)"
argument_list|,
name|report
index|[
literal|0
...
begin_comment
comment|/* Workaround different, undefined for Windows, error codes used here */
end_comment
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: No connection, no RRM."
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: No RRM in current connection."
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: No network support for Neighbor Report."
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Neighbor Report request must provide a callback."
argument_list|)
expr_stmt|;
return|return
operator|-
...
return|;
block|}
comment|/* Refuse if there's a live request */
if|if
condition|(
name|wpa_s
operator|->
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Currently handling previous Neighbor Report."
argument_list|)
expr_stmt|;
return|return
operator|-
...
return|;
block|}
comment|/* 3 = action category + action code + dialog token */
name|buf
operator|=
name|wpabuf_alloc
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Failed to allocate Neighbor Report Request"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Neighbor report request (for %s), token=%d"
argument_list|,
operator|(
name|ssid
condition|?
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Failed to send Neighbor Report Request"
argument_list|)
expr_stmt|;
name|wpabuf_free
argument_list|(
...
name|MSG_INFO
argument_list|,
literal|"RRM: Ignoring link measurement request. Not associated"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_INFO
argument_list|,
literal|"RRM: Ignoring link measurement request. Not RRM network"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_INFO
argument_list|,
literal|"RRM: Measurement report failed. TX power insertion not supported"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_INFO
argument_list|,
literal|"RRM: Link measurement report failed. Request too short"
argument_list|)
expr_stmt|;
return|return;
block|}
...
literal|255
expr_stmt|;
comment|/* 255 indicates that RSNI is not available */
name|report
operator|.
name|dialog_token
operator|=
...
name|dialog_token
expr_stmt|;
comment|/* 	 * It's possible to estimate RCPI based on RSSI in dBm. This 	 * calculation will not reflect the correct value for high rates, 	 * but it's good enough for Action frames which are transmitted 	 * with up to 24 Mbps rates. 	 */
if|if
condition|(
operator|!
name|rssi
...
literal|2
expr_stmt|;
comment|/* action_category + action_code */
name|buf
operator|=
name|wpabuf_alloc
argument_list|(
...
name|MSG_ERROR
argument_list|,
literal|"RRM: Link measurement report failed. Buffer allocation failed"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"RRM: Link measurement report:"
argument_list|,
name|wpabuf_head
argument_list|(
name|buf
...
name|MSG_ERROR
argument_list|,
literal|"RRM: Link measurement report failed. Send action failed"
argument_list|)
expr_stmt|;
block|}
name|wpabuf_free
