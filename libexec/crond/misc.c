...
argument_list|,
argument|(
literal|"strcmp_until(%s,%s,%c) ... "
argument|, left, right, until)
argument_list|)
while|while
condition|(
operator|*
...
parameter_list|()
block|{
comment|/* not being a system V wiz, I don't know if this is what you have 	 * to do to release your control terminal.  what I want to accomplish 	 * is to keep this process from getting any signals from the tty. 	 * 	 * some system V person should let me know if this works... (vixie) 	 */
name|int
name|setpgrp
argument_list|()
decl_stmt|,
...
begin_comment
comment|/* acquire_daemonlock() - write our PID into /etc/crond.pid, unless  *	another daemon is already running, which we detect here.  */
end_comment
...
name|buf
argument_list|,
literal|"can't open or create %s, errno %d"
argument_list|,
name|PIDFILE
argument_list|,
name|errno
...
name|buf
argument_list|,
literal|"can't lock %s, otherpid may be %d, errno %d"
argument_list|,
name|PIDFILE
argument_list|,
name|otherpid
...
index|]
decl_stmt|;
comment|/* let's be persnickety today. 	 */
if|if
condition|(
operator|!
name|file
...
name|stderr
argument_list|,
literal|"in_file(\"%s\", 0x%x): called with NULL file--botch"
argument_list|,
name|string
argument_list|,
name|file
...
argument_list|)
expr_stmt|;
comment|/* we have to run strlen() because sprintf() returns (char*) on BSD 	 */
if|if
condition|(
name|log_fd
operator|<
...
expr_stmt|;
block|}
comment|/* I suppose we could use alloca()... 	 */
name|free
argument_list|(
name|msg
argument_list|)
