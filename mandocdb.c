...
begin_comment
comment|/*	$Id: mandocdb.c,v 1.46 2012/03/23 06:52:17 kristaps Exp $ */
end_comment
...
parameter_list|)
define|\
value|do if (warnings) { \ 		fprintf(stderr, "%s: ", (_b)); \ 		fprintf(stderr, (_fmt), ##_args); \ 		if ('\0' != *(_f)) \ 			fprintf(stderr, ": %s", (_f)); \ 		fprintf(stderr, "\n"); \ 	} while (
comment|/* CONSTCOND */
value|0)
end_define
...
name|stderr
argument_list|,
literal|"usage: %s [-av] [-C file] | dir ... | -t file ...\n"
literal|"                        -d dir [file ...] | "
literal|"-u dir [file ...]\n"
argument_list|,
name|progname
...
name|basedir
argument_list|,
literal|"Probably unreachable, title "
literal|"is %s"
argument_list|,
operator|(
name|char
...
name|printf
argument_list|(
literal|"%s: Deleting from index: %s\n"
argument_list|,
name|basedir
argument_list|,
name|fn
...
name|basedir
argument_list|,
literal|"Cannot deduce title from filename"
argument_list|)
expr_stmt|;
name|title
operator|=
...
begin_comment
comment|/*  * Recursively build up a list of files to parse.  * We use this instead of ftw() and so on because I don't want global  * variables hanging around.  * This ignores the mandocdb.db and mandocdb.index files, but assumes that  * everything else is a manual.  * Pass in a pointer to a NULL structure for the first invocation.  */
end_comment
