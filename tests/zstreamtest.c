begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 2016-present, Yann Collet, Facebook, Inc.  * All rights reserved.  *  * This source code is licensed under both the BSD-style license (found in the  * LICENSE file in the root directory of this source tree) and the GPLv2 (found  * in the COPYING file in the root directory of this source tree).  * You may select, at your option, one of the above-listed licenses.  */
end_comment

begin_comment
comment|/*-************************************ *  Compiler specific **************************************/
end_comment

begin_ifdef
ifdef|#
directive|ifdef
name|_MSC_VER
end_ifdef

begin_comment
comment|/* Visual Studio */
end_comment

begin_define
define|#
directive|define
name|_CRT_SECURE_NO_WARNINGS
end_define

begin_comment
comment|/* fgets */
end_comment

begin_pragma
pragma|#
directive|pragma
name|warning
name|(
name|disable
name|:
name|4127
name|)
end_pragma

begin_comment
comment|/* disable: C4127: conditional expression is constant */
end_comment

begin_pragma
pragma|#
directive|pragma
name|warning
name|(
name|disable
name|:
name|4146
name|)
end_pragma

begin_comment
comment|/* disable: C4146: minus unsigned expression */
end_comment

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/*-************************************ *  Includes **************************************/
end_comment

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_comment
comment|/* free */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_comment
comment|/* fgets, sscanf */
end_comment

begin_include
include|#
directive|include
file|<time.h>
end_include

begin_comment
comment|/* clock_t, clock() */
end_comment

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_comment
comment|/* strcmp */
end_comment

begin_include
include|#
directive|include
file|<assert.h>
end_include

begin_comment
comment|/* assert */
end_comment

begin_include
include|#
directive|include
file|"mem.h"
end_include

begin_define
define|#
directive|define
name|ZSTD_STATIC_LINKING_ONLY
end_define

begin_comment
comment|/* ZSTD_maxCLevel, ZSTD_customMem, ZSTD_getDictID_fromFrame */
end_comment

begin_include
include|#
directive|include
file|"zstd.h"
end_include

begin_comment
comment|/* ZSTD_compressBound */
end_comment

begin_include
include|#
directive|include
file|"zstd_errors.h"
end_include

begin_comment
comment|/* ZSTD_error_srcSize_wrong */
end_comment

begin_include
include|#
directive|include
file|"zstdmt_compress.h"
end_include

begin_include
include|#
directive|include
file|"zdict.h"
end_include

begin_comment
comment|/* ZDICT_trainFromBuffer */
end_comment

begin_include
include|#
directive|include
file|"datagen.h"
end_include

begin_comment
comment|/* RDG_genBuffer */
end_comment

begin_define
define|#
directive|define
name|XXH_STATIC_LINKING_ONLY
end_define

begin_comment
comment|/* XXH64_state_t */
end_comment

begin_include
include|#
directive|include
file|"xxhash.h"
end_include

begin_comment
comment|/* XXH64_* */
end_comment

begin_comment
comment|/*-************************************ *  Constants **************************************/
end_comment

begin_define
define|#
directive|define
name|KB
value|*(1U<<10)
end_define

begin_define
define|#
directive|define
name|MB
value|*(1U<<20)
end_define

begin_define
define|#
directive|define
name|GB
value|*(1U<<30)
end_define

begin_decl_stmt
specifier|static
specifier|const
name|U32
name|nbTestsDefault
init|=
literal|10000
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|U32
name|g_cLevelMax_smallTests
init|=
literal|10
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|COMPRESSIBLE_NOISE_LENGTH
value|(10 MB)
end_define

begin_define
define|#
directive|define
name|FUZ_COMPRESSIBILITY_DEFAULT
value|50
end_define

begin_decl_stmt
specifier|static
specifier|const
name|U32
name|prime32
init|=
literal|2654435761U
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*-************************************ *  Display Macros **************************************/
end_comment

begin_define
define|#
directive|define
name|DISPLAY
parameter_list|(
modifier|...
parameter_list|)
value|fprintf(stderr, __VA_ARGS__)
end_define

begin_define
define|#
directive|define
name|DISPLAYLEVEL
parameter_list|(
name|l
parameter_list|,
modifier|...
parameter_list|)
value|if (g_displayLevel>=l) {                     \                                   DISPLAY(__VA_ARGS__);                    \                                   if (g_displayLevel>=4) fflush(stderr); }
end_define

begin_decl_stmt
specifier|static
name|U32
name|g_displayLevel
init|=
literal|2
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|DISPLAYUPDATE
parameter_list|(
name|l
parameter_list|,
modifier|...
parameter_list|)
value|if (g_displayLevel>=l) { \             if ((FUZ_GetClockSpan(g_displayClock)> g_refreshRate) || (g_displayLevel>=4)) \             { g_displayClock = clock(); DISPLAY(__VA_ARGS__); \               if (g_displayLevel>=4) fflush(stderr); } }
end_define

begin_decl_stmt
specifier|static
specifier|const
name|clock_t
name|g_refreshRate
init|=
name|CLOCKS_PER_SEC
operator|/
literal|6
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|clock_t
name|g_displayClock
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
name|clock_t
name|g_clockTime
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*-******************************************************* *  Fuzzer functions *********************************************************/
end_comment

begin_define
define|#
directive|define
name|MAX
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|((a)>(b)?(a):(b))
end_define

begin_function
specifier|static
name|clock_t
name|FUZ_GetClockSpan
parameter_list|(
name|clock_t
name|clockStart
parameter_list|)
block|{
return|return
name|clock
argument_list|()
operator|-
name|clockStart
return|;
comment|/* works even when overflow. Max span ~ 30 mn */
block|}
end_function

begin_comment
comment|/*! FUZ_rand() :     @return : a 27 bits random value, from a 32-bits `seed`.     `seed` is also modified */
end_comment

begin_define
define|#
directive|define
name|FUZ_rotl32
parameter_list|(
name|x
parameter_list|,
name|r
parameter_list|)
value|((x<< r) | (x>> (32 - r)))
end_define

begin_function
name|unsigned
name|int
name|FUZ_rand
parameter_list|(
name|unsigned
name|int
modifier|*
name|seedPtr
parameter_list|)
block|{
specifier|static
specifier|const
name|U32
name|prime2
init|=
literal|2246822519U
decl_stmt|;
name|U32
name|rand32
init|=
operator|*
name|seedPtr
decl_stmt|;
name|rand32
operator|*=
name|prime32
expr_stmt|;
name|rand32
operator|+=
name|prime2
expr_stmt|;
name|rand32
operator|=
name|FUZ_rotl32
argument_list|(
name|rand32
argument_list|,
literal|13
argument_list|)
expr_stmt|;
operator|*
name|seedPtr
operator|=
name|rand32
expr_stmt|;
return|return
name|rand32
operator|>>
literal|5
return|;
block|}
end_function

begin_define
define|#
directive|define
name|CHECK_Z
parameter_list|(
name|f
parameter_list|)
value|{                                         \     size_t const err = f;                                    \     if (ZSTD_isError(err)) {                                 \         DISPLAY("Error => %s : %s ",                         \                 #f, ZSTD_getErrorName(err));                 \         DISPLAY(" (seed %u, test nb %u)  \n", seed, testNb); \         goto _output_error;                                  \ }   }
end_define

begin_comment
comment|/*====================================================== *   Basic Unit tests ======================================================*/
end_comment

begin_typedef
typedef|typedef
struct|struct
block|{
name|void
modifier|*
name|start
decl_stmt|;
name|size_t
name|size
decl_stmt|;
name|size_t
name|filled
decl_stmt|;
block|}
name|buffer_t
typedef|;
end_typedef

begin_decl_stmt
specifier|static
specifier|const
name|buffer_t
name|g_nullBuffer
init|=
block|{
name|NULL
block|,
literal|0
block|,
literal|0
block|}
decl_stmt|;
end_decl_stmt

begin_function
specifier|static
name|buffer_t
name|FUZ_createDictionary
parameter_list|(
specifier|const
name|void
modifier|*
name|src
parameter_list|,
name|size_t
name|srcSize
parameter_list|,
name|size_t
name|blockSize
parameter_list|,
name|size_t
name|requestedDictSize
parameter_list|)
block|{
name|buffer_t
name|dict
init|=
block|{
name|NULL
block|,
literal|0
block|,
literal|0
block|}
decl_stmt|;
name|size_t
specifier|const
name|nbBlocks
init|=
operator|(
name|srcSize
operator|+
operator|(
name|blockSize
operator|-
literal|1
operator|)
operator|)
operator|/
name|blockSize
decl_stmt|;
name|size_t
modifier|*
specifier|const
name|blockSizes
init|=
operator|(
name|size_t
operator|*
operator|)
name|malloc
argument_list|(
name|nbBlocks
operator|*
sizeof|sizeof
argument_list|(
name|size_t
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|blockSizes
condition|)
return|return
name|dict
return|;
name|dict
operator|.
name|start
operator|=
name|malloc
argument_list|(
name|requestedDictSize
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|dict
operator|.
name|start
condition|)
block|{
name|free
argument_list|(
name|blockSizes
argument_list|)
expr_stmt|;
return|return
name|dict
return|;
block|}
block|{
name|size_t
name|nb
decl_stmt|;
for|for
control|(
name|nb
operator|=
literal|0
init|;
name|nb
operator|<
name|nbBlocks
operator|-
literal|1
condition|;
name|nb
operator|++
control|)
name|blockSizes
index|[
name|nb
index|]
operator|=
name|blockSize
expr_stmt|;
name|blockSizes
index|[
name|nbBlocks
operator|-
literal|1
index|]
operator|=
name|srcSize
operator|-
operator|(
name|blockSize
operator|*
operator|(
name|nbBlocks
operator|-
literal|1
operator|)
operator|)
expr_stmt|;
block|}
block|{
name|size_t
specifier|const
name|dictSize
init|=
name|ZDICT_trainFromBuffer
argument_list|(
name|dict
operator|.
name|start
argument_list|,
name|requestedDictSize
argument_list|,
name|src
argument_list|,
name|blockSizes
argument_list|,
operator|(
name|unsigned
operator|)
name|nbBlocks
argument_list|)
decl_stmt|;
name|free
argument_list|(
name|blockSizes
argument_list|)
expr_stmt|;
if|if
condition|(
name|ZDICT_isError
argument_list|(
name|dictSize
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|dict
operator|.
name|start
argument_list|)
expr_stmt|;
return|return
name|g_nullBuffer
return|;
block|}
name|dict
operator|.
name|size
operator|=
name|requestedDictSize
expr_stmt|;
name|dict
operator|.
name|filled
operator|=
name|dictSize
expr_stmt|;
return|return
name|dict
return|;
comment|/* how to return dictSize ? */
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|FUZ_freeDictionary
parameter_list|(
name|buffer_t
name|dict
parameter_list|)
block|{
name|free
argument_list|(
name|dict
operator|.
name|start
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|basicUnitTests
parameter_list|(
name|U32
name|seed
parameter_list|,
name|double
name|compressibility
parameter_list|,
name|ZSTD_customMem
name|customMem
parameter_list|)
block|{
name|size_t
specifier|const
name|CNBufferSize
init|=
name|COMPRESSIBLE_NOISE_LENGTH
decl_stmt|;
name|void
modifier|*
name|CNBuffer
init|=
name|malloc
argument_list|(
name|CNBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|skippableFrameSize
init|=
literal|11
decl_stmt|;
name|size_t
specifier|const
name|compressedBufferSize
init|=
operator|(
literal|8
operator|+
name|skippableFrameSize
operator|)
operator|+
name|ZSTD_compressBound
argument_list|(
name|COMPRESSIBLE_NOISE_LENGTH
argument_list|)
decl_stmt|;
name|void
modifier|*
name|compressedBuffer
init|=
name|malloc
argument_list|(
name|compressedBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|decodedBufferSize
init|=
name|CNBufferSize
decl_stmt|;
name|void
modifier|*
name|decodedBuffer
init|=
name|malloc
argument_list|(
name|decodedBufferSize
argument_list|)
decl_stmt|;
name|size_t
name|cSize
decl_stmt|;
name|int
name|testResult
init|=
literal|0
decl_stmt|;
name|U32
name|testNb
init|=
literal|1
decl_stmt|;
name|ZSTD_CStream
modifier|*
name|zc
init|=
name|ZSTD_createCStream_advanced
argument_list|(
name|customMem
argument_list|)
decl_stmt|;
name|ZSTD_DStream
modifier|*
name|zd
init|=
name|ZSTD_createDStream_advanced
argument_list|(
name|customMem
argument_list|)
decl_stmt|;
name|ZSTDMT_CCtx
modifier|*
name|mtctx
init|=
name|ZSTDMT_createCCtx
argument_list|(
literal|2
argument_list|)
decl_stmt|;
name|ZSTD_inBuffer
name|inBuff
decl_stmt|,
name|inBuff2
decl_stmt|;
name|ZSTD_outBuffer
name|outBuff
decl_stmt|;
name|buffer_t
name|dictionary
init|=
name|g_nullBuffer
decl_stmt|;
name|size_t
specifier|const
name|dictSize
init|=
literal|128
name|KB
decl_stmt|;
name|unsigned
name|dictID
init|=
literal|0
decl_stmt|;
comment|/* Create compressible test buffer */
if|if
condition|(
operator|!
name|CNBuffer
operator|||
operator|!
name|compressedBuffer
operator|||
operator|!
name|decodedBuffer
operator|||
operator|!
name|zc
operator|||
operator|!
name|zd
condition|)
block|{
name|DISPLAY
argument_list|(
literal|"Not enough memory, aborting \n"
argument_list|)
expr_stmt|;
goto|goto
name|_output_error
goto|;
block|}
name|RDG_genBuffer
argument_list|(
name|CNBuffer
argument_list|,
name|CNBufferSize
argument_list|,
name|compressibility
argument_list|,
literal|0.
argument_list|,
name|seed
argument_list|)
expr_stmt|;
comment|/* Create dictionary */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"creating dictionary for unit tests \n"
argument_list|)
expr_stmt|;
name|dictionary
operator|=
name|FUZ_createDictionary
argument_list|(
name|CNBuffer
argument_list|,
name|CNBufferSize
operator|/
literal|2
argument_list|,
literal|8
name|KB
argument_list|,
literal|40
name|KB
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|dictionary
operator|.
name|start
condition|)
block|{
name|DISPLAY
argument_list|(
literal|"Error creating dictionary, aborting \n"
argument_list|)
expr_stmt|;
goto|goto
name|_output_error
goto|;
block|}
name|dictID
operator|=
name|ZDICT_getDictID
argument_list|(
name|dictionary
operator|.
name|start
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|)
expr_stmt|;
comment|/* generate skippable frame */
name|MEM_writeLE32
argument_list|(
name|compressedBuffer
argument_list|,
name|ZSTD_MAGIC_SKIPPABLE_START
argument_list|)
expr_stmt|;
name|MEM_writeLE32
argument_list|(
operator|(
operator|(
name|char
operator|*
operator|)
name|compressedBuffer
operator|)
operator|+
literal|4
argument_list|,
operator|(
name|U32
operator|)
name|skippableFrameSize
argument_list|)
expr_stmt|;
name|cSize
operator|=
name|skippableFrameSize
operator|+
literal|8
expr_stmt|;
comment|/* Basic compression test */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : compress %u bytes : "
argument_list|,
name|testNb
operator|++
argument_list|,
name|COMPRESSIBLE_NOISE_LENGTH
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_initCStream_usingDict
argument_list|(
name|zc
argument_list|,
name|CNBuffer
argument_list|,
name|dictSize
argument_list|,
literal|1
argument_list|)
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
operator|(
name|char
operator|*
operator|)
operator|(
name|compressedBuffer
operator|)
operator|+
name|cSize
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* error, or some data not flushed */
name|cSize
operator|+=
name|outBuff
operator|.
name|pos
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes : %.2f%%)\n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|,
operator|(
name|double
operator|)
name|cSize
operator|/
name|COMPRESSIBLE_NOISE_LENGTH
operator|*
literal|100
argument_list|)
expr_stmt|;
comment|/* context size functions */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : estimate CStream size : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|ZSTD_compressionParameters
specifier|const
name|cParams
init|=
name|ZSTD_getCParams
argument_list|(
literal|1
argument_list|,
name|CNBufferSize
argument_list|,
name|dictSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|cstreamSize
init|=
name|ZSTD_estimateCStreamSize_usingCParams
argument_list|(
name|cParams
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|cdictSize
init|=
name|ZSTD_estimateCDictSize_advanced
argument_list|(
name|dictSize
argument_list|,
name|cParams
argument_list|,
name|ZSTD_dlm_byCopy
argument_list|)
decl_stmt|;
comment|/* uses ZSTD_initCStream_usingDict() */
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|cstreamSize
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|cdictSize
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes) \n"
argument_list|,
call|(
name|U32
call|)
argument_list|(
name|cstreamSize
operator|+
name|cdictSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : check actual CStream size : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|s
init|=
name|ZSTD_sizeof_CStream
argument_list|(
name|zc
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|s
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes) \n"
argument_list|,
operator|(
name|U32
operator|)
name|s
argument_list|)
expr_stmt|;
block|}
comment|/* Attempt bad compression parameters */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : use bad compression parameters : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
name|r
decl_stmt|;
name|ZSTD_parameters
name|params
init|=
name|ZSTD_getParams
argument_list|(
literal|1
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
decl_stmt|;
name|params
operator|.
name|cParams
operator|.
name|searchLength
operator|=
literal|2
expr_stmt|;
name|r
operator|=
name|ZSTD_initCStream_advanced
argument_list|(
name|zc
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|params
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"init error : %s \n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|r
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* skippable frame test */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress skippable frame : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|CNBuffer
argument_list|,
name|dictSize
argument_list|)
argument_list|)
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|compressedBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|cSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|decodedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
decl_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|" ( ZSTD_decompressStream => %u ) "
argument_list|,
operator|(
name|U32
operator|)
name|r
argument_list|)
expr_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
comment|/* skippable frame output len is 0 */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* Basic decompression test */
name|inBuff2
operator|=
name|inBuff
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress %u bytes : "
argument_list|,
name|testNb
operator|++
argument_list|,
name|COMPRESSIBLE_NOISE_LENGTH
argument_list|)
expr_stmt|;
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|CNBuffer
argument_list|,
name|dictSize
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_setDStreamParameter
argument_list|(
name|zd
argument_list|,
name|DStream_p_maxWindowSize
argument_list|,
literal|1000000000
argument_list|)
argument_list|)
expr_stmt|;
comment|/* large limit */
block|{
name|size_t
specifier|const
name|remaining
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|remaining
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* should reach end of frame == 0; otherwise, some data left, or an error */
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|!=
name|CNBufferSize
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should regenerate the same amount */
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should have read the entire frame */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* Re-use without init */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress again without init (re-use previous settings): "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
block|{
name|size_t
specifier|const
name|remaining
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff2
argument_list|)
decl_stmt|;
if|if
condition|(
name|remaining
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* should reach end of frame == 0; otherwise, some data left, or an error */
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|!=
name|CNBufferSize
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should regenerate the same amount */
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should have read the entire frame */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* check regenerated data is byte exact */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : check decompressed result : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|CNBufferSize
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|(
operator|(
name|BYTE
operator|*
operator|)
name|decodedBuffer
operator|)
index|[
name|i
index|]
operator|!=
operator|(
operator|(
name|BYTE
operator|*
operator|)
name|CNBuffer
operator|)
index|[
name|i
index|]
condition|)
goto|goto
name|_output_error
goto|;
block|}
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* context size functions */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : estimate DStream size : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|ZSTD_frameHeader
name|fhi
decl_stmt|;
specifier|const
name|void
modifier|*
name|cStart
init|=
operator|(
name|char
operator|*
operator|)
name|compressedBuffer
operator|+
operator|(
name|skippableFrameSize
operator|+
literal|8
operator|)
decl_stmt|;
name|size_t
specifier|const
name|gfhError
init|=
name|ZSTD_getFrameHeader
argument_list|(
operator|&
name|fhi
argument_list|,
name|cStart
argument_list|,
name|cSize
argument_list|)
decl_stmt|;
if|if
condition|(
name|gfhError
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|" (windowSize : %u) "
argument_list|,
operator|(
name|U32
operator|)
name|fhi
operator|.
name|windowSize
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|s
init|=
name|ZSTD_estimateDStreamSize
argument_list|(
name|fhi
operator|.
name|windowSize
argument_list|)
comment|/* uses ZSTD_initDStream_usingDict() */
operator|+
name|ZSTD_estimateDDictSize
argument_list|(
name|dictSize
argument_list|,
name|ZSTD_dlm_byCopy
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|s
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes) \n"
argument_list|,
operator|(
name|U32
operator|)
name|s
argument_list|)
expr_stmt|;
block|}
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : check actual DStream size : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|s
init|=
name|ZSTD_sizeof_DStream
argument_list|(
name|zd
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|s
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes) \n"
argument_list|,
operator|(
name|U32
operator|)
name|s
argument_list|)
expr_stmt|;
block|}
comment|/* Byte-by-byte decompression test */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress byte-by-byte : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
comment|/* skippable frame */
name|size_t
name|r
init|=
literal|1
decl_stmt|;
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|CNBuffer
argument_list|,
name|dictSize
argument_list|)
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|decodedBuffer
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
while|while
condition|(
name|r
condition|)
block|{
comment|/* skippable frame */
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
literal|1
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
literal|1
expr_stmt|;
name|r
operator|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
expr_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* normal frame */
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|CNBuffer
argument_list|,
name|dictSize
argument_list|)
expr_stmt|;
name|r
operator|=
literal|1
expr_stmt|;
while|while
condition|(
name|r
condition|)
block|{
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
literal|1
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
literal|1
expr_stmt|;
name|r
operator|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
expr_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
block|}
block|}
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|!=
name|CNBufferSize
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should regenerate the same amount */
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|cSize
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should have read the entire frame */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* check regenerated data is byte exact */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : check decompressed result : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|CNBufferSize
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|(
operator|(
name|BYTE
operator|*
operator|)
name|decodedBuffer
operator|)
index|[
name|i
index|]
operator|!=
operator|(
operator|(
name|BYTE
operator|*
operator|)
name|CNBuffer
operator|)
index|[
name|i
index|]
condition|)
goto|goto
name|_output_error
goto|;
empty_stmt|;
block|}
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* _srcSize compression test */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : compress_srcSize %u bytes : "
argument_list|,
name|testNb
operator|++
argument_list|,
name|COMPRESSIBLE_NOISE_LENGTH
argument_list|)
expr_stmt|;
name|ZSTD_initCStream_srcSize
argument_list|(
name|zc
argument_list|,
literal|1
argument_list|,
name|CNBufferSize
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
operator|(
name|char
operator|*
operator|)
operator|(
name|compressedBuffer
operator|)
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* error, or some data not flushed */
block|{
name|unsigned
name|long
name|long
name|origSize
init|=
name|ZSTD_findDecompressedSize
argument_list|(
name|outBuff
operator|.
name|dst
argument_list|,
name|outBuff
operator|.
name|pos
argument_list|)
decl_stmt|;
if|if
condition|(
operator|(
name|size_t
operator|)
name|origSize
operator|!=
name|CNBufferSize
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* exact original size must be present */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes : %.2f%%)\n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|,
operator|(
name|double
operator|)
name|cSize
operator|/
name|COMPRESSIBLE_NOISE_LENGTH
operator|*
literal|100
argument_list|)
expr_stmt|;
comment|/* wrong _srcSize compression test */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : wrong srcSize : %u bytes : "
argument_list|,
name|testNb
operator|++
argument_list|,
name|COMPRESSIBLE_NOISE_LENGTH
operator|-
literal|1
argument_list|)
expr_stmt|;
name|ZSTD_initCStream_srcSize
argument_list|(
name|zc
argument_list|,
literal|1
argument_list|,
name|CNBufferSize
operator|-
literal|1
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
operator|(
name|char
operator|*
operator|)
operator|(
name|compressedBuffer
operator|)
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_getErrorCode
argument_list|(
name|r
argument_list|)
operator|!=
name|ZSTD_error_srcSize_wrong
condition|)
goto|goto
name|_output_error
goto|;
comment|/* must fail : wrong srcSize */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (error detected : %s) \n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|r
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* Complex context re-use scenario */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : context re-use : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
name|ZSTD_freeCStream
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|zc
operator|=
name|ZSTD_createCStream_advanced
argument_list|(
name|customMem
argument_list|)
expr_stmt|;
if|if
condition|(
name|zc
operator|==
name|NULL
condition|)
goto|goto
name|_output_error
goto|;
comment|/* memory allocation issue */
comment|/* use 1 */
block|{
name|size_t
specifier|const
name|inSize
init|=
literal|513
decl_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"use1 "
argument_list|)
expr_stmt|;
name|ZSTD_initCStream_advanced
argument_list|(
name|zc
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|ZSTD_getParams
argument_list|(
literal|19
argument_list|,
name|inSize
argument_list|,
literal|0
argument_list|)
argument_list|,
name|inSize
argument_list|)
expr_stmt|;
comment|/* needs btopt + search3 to trigger hashLog3 */
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
operator|(
name|char
operator|*
operator|)
operator|(
name|compressedBuffer
operator|)
operator|+
name|cSize
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|ZSTD_compressBound
argument_list|(
name|inSize
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"compress1 "
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"end1 "
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* error, or some data not flushed */
block|}
comment|/* use 2 */
block|{
name|size_t
specifier|const
name|inSize
init|=
literal|1025
decl_stmt|;
comment|/* will not continue, because tables auto-adjust and are therefore different size */
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"use2 "
argument_list|)
expr_stmt|;
name|ZSTD_initCStream_advanced
argument_list|(
name|zc
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|ZSTD_getParams
argument_list|(
literal|19
argument_list|,
name|inSize
argument_list|,
literal|0
argument_list|)
argument_list|,
name|inSize
argument_list|)
expr_stmt|;
comment|/* needs btopt + search3 to trigger hashLog3 */
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
operator|(
name|char
operator|*
operator|)
operator|(
name|compressedBuffer
operator|)
operator|+
name|cSize
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|ZSTD_compressBound
argument_list|(
name|inSize
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"compress2 "
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"end2 "
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* error, or some data not flushed */
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* CDict scenario */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : digested dictionary : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|ZSTD_CDict
modifier|*
specifier|const
name|cdict
init|=
name|ZSTD_createCDict
argument_list|(
name|dictionary
operator|.
name|start
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|,
literal|1
comment|/*byRef*/
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|initError
init|=
name|ZSTD_initCStream_usingCDict
argument_list|(
name|zc
argument_list|,
name|cdict
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|initError
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|cSize
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* error, or some data not flushed */
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
name|ZSTD_freeCDict
argument_list|(
name|cdict
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes : %.2f%%)\n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|,
operator|(
name|double
operator|)
name|cSize
operator|/
name|CNBufferSize
operator|*
literal|100
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : check CStream size : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|s
init|=
name|ZSTD_sizeof_CStream
argument_list|(
name|zc
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|s
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes) \n"
argument_list|,
operator|(
name|U32
operator|)
name|s
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|4
argument_list|,
literal|"test%3i : check Dictionary ID : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|unsigned
specifier|const
name|dID
init|=
name|ZSTD_getDictID_fromFrame
argument_list|(
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
decl_stmt|;
if|if
condition|(
name|dID
operator|!=
name|dictID
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|4
argument_list|,
literal|"OK (%u) \n"
argument_list|,
name|dID
argument_list|)
expr_stmt|;
block|}
comment|/* DDict scenario */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress %u bytes with digested dictionary : "
argument_list|,
name|testNb
operator|++
argument_list|,
operator|(
name|U32
operator|)
name|CNBufferSize
argument_list|)
expr_stmt|;
block|{
name|ZSTD_DDict
modifier|*
specifier|const
name|ddict
init|=
name|ZSTD_createDDict
argument_list|(
name|dictionary
operator|.
name|start
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|initError
init|=
name|ZSTD_initDStream_usingDDict
argument_list|(
name|zd
argument_list|,
name|ddict
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|initError
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|inBuff
operator|.
name|src
operator|=
name|compressedBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|cSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|decodedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* should reach end of frame == 0; otherwise, some data left, or an error */
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|!=
name|CNBufferSize
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should regenerate the same amount */
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* should have read the entire frame */
name|ZSTD_freeDDict
argument_list|(
name|ddict
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
block|}
comment|/* test ZSTD_setDStreamParameter() resilience */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : wrong parameter for ZSTD_setDStreamParameter(): "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_setDStreamParameter
argument_list|(
name|zd
argument_list|,
operator|(
name|ZSTD_DStreamParameter_e
operator|)
literal|999
argument_list|,
literal|1
argument_list|)
decl_stmt|;
comment|/* large limit */
if|if
condition|(
operator|!
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* Memory restriction */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : maxWindowSize< frame requirement : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|CNBuffer
argument_list|,
name|dictSize
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_setDStreamParameter
argument_list|(
name|zd
argument_list|,
name|DStream_p_maxWindowSize
argument_list|,
literal|1000
argument_list|)
argument_list|)
expr_stmt|;
comment|/* too small limit */
name|inBuff
operator|.
name|src
operator|=
name|compressedBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|cSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|decodedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
comment|/* must fail : frame requires> 100 bytes */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%s)\n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|r
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : ZSTD_initCStream_usingCDict_advanced with masked dictID : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|ZSTD_compressionParameters
specifier|const
name|cParams
init|=
name|ZSTD_getCParams
argument_list|(
literal|1
argument_list|,
name|CNBufferSize
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|)
decl_stmt|;
name|ZSTD_frameParameters
specifier|const
name|fParams
init|=
block|{
literal|1
comment|/* contentSize */
block|,
literal|1
comment|/* checksum */
block|,
literal|1
comment|/* noDictID */
block|}
decl_stmt|;
name|ZSTD_CDict
modifier|*
specifier|const
name|cdict
init|=
name|ZSTD_createCDict_advanced
argument_list|(
name|dictionary
operator|.
name|start
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|,
name|ZSTD_dlm_byRef
argument_list|,
name|ZSTD_dm_auto
argument_list|,
name|cParams
argument_list|,
name|customMem
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|initError
init|=
name|ZSTD_initCStream_usingCDict_advanced
argument_list|(
name|zc
argument_list|,
name|cdict
argument_list|,
name|fParams
argument_list|,
name|CNBufferSize
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|initError
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
name|cSize
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* error, or some data not flushed */
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
name|ZSTD_freeCDict
argument_list|(
name|cdict
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes : %.2f%%)\n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|,
operator|(
name|double
operator|)
name|cSize
operator|/
name|CNBufferSize
operator|*
literal|100
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : try retrieving dictID from frame : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|U32
specifier|const
name|did
init|=
name|ZSTD_getDictID_fromFrame
argument_list|(
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
decl_stmt|;
if|if
condition|(
name|did
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (not detected) \n"
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress without dictionary : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_decompress
argument_list|(
name|decodedBuffer
argument_list|,
name|CNBufferSize
argument_list|,
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
comment|/* must fail : dictionary not used */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%s)\n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|r
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : compress with ZSTD_CCtx_refPrefix : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_CCtx_refPrefix
argument_list|(
name|zc
argument_list|,
name|dictionary
operator|.
name|start
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|)
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compress_generic
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|,
name|ZSTD_e_end
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes : %.2f%%)\n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|,
operator|(
name|double
operator|)
name|cSize
operator|/
name|CNBufferSize
operator|*
literal|100
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress with dictionary : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_decompress_usingDict
argument_list|(
name|zd
argument_list|,
name|decodedBuffer
argument_list|,
name|CNBufferSize
argument_list|,
name|compressedBuffer
argument_list|,
name|cSize
argument_list|,
name|dictionary
operator|.
name|start
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
comment|/* must fail : dictionary not used */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress without dictionary (should fail): "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTD_decompress
argument_list|(
name|decodedBuffer
argument_list|,
name|CNBufferSize
argument_list|,
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|ZSTD_isError
argument_list|(
name|r
argument_list|)
condition|)
goto|goto
name|_output_error
goto|;
comment|/* must fail : dictionary not used */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%s)\n"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|r
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : compress again with ZSTD_compress_generic : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compress_generic
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|,
name|ZSTD_e_end
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK (%u bytes : %.2f%%)\n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|,
operator|(
name|double
operator|)
name|cSize
operator|/
name|CNBufferSize
operator|*
literal|100
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : decompress without dictionary (should work): "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_decompress
argument_list|(
name|decodedBuffer
argument_list|,
name|CNBufferSize
argument_list|,
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* Empty srcSize */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : ZSTD_initCStream_advanced with pledgedSrcSize=0 and dict : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|ZSTD_parameters
name|params
init|=
name|ZSTD_getParams
argument_list|(
literal|5
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
decl_stmt|;
name|params
operator|.
name|fParams
operator|.
name|contentSizeFlag
operator|=
literal|1
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_initCStream_advanced
argument_list|(
name|zc
argument_list|,
name|dictionary
operator|.
name|start
argument_list|,
name|dictionary
operator|.
name|filled
argument_list|,
name|params
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* cstream advanced shall write content size = 0 */
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
if|if
condition|(
name|ZSTD_findDecompressedSize
argument_list|(
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : pledgedSrcSize == 0 behaves properly : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
name|ZSTD_parameters
name|params
init|=
name|ZSTD_getParams
argument_list|(
literal|5
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
decl_stmt|;
name|params
operator|.
name|fParams
operator|.
name|contentSizeFlag
operator|=
literal|1
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_initCStream_advanced
argument_list|(
name|zc
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|params
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* cstream advanced shall write content size = 0 */
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
if|if
condition|(
name|ZSTD_findDecompressedSize
argument_list|(
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
name|ZSTD_resetCStream
argument_list|(
name|zc
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* resetCStream should treat 0 as unknown */
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|compressedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
if|if
condition|(
name|ZSTD_findDecompressedSize
argument_list|(
name|compressedBuffer
argument_list|,
name|cSize
argument_list|)
operator|!=
name|ZSTD_CONTENTSIZE_UNKNOWN
condition|)
goto|goto
name|_output_error
goto|;
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* Basic multithreading compression test */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : compress %u bytes with multiple threads : "
argument_list|,
name|testNb
operator|++
argument_list|,
name|COMPRESSIBLE_NOISE_LENGTH
argument_list|)
expr_stmt|;
block|{
name|ZSTD_parameters
specifier|const
name|params
init|=
name|ZSTD_getParams
argument_list|(
literal|1
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
decl_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_initCStream_advanced
argument_list|(
name|mtctx
argument_list|,
name|CNBuffer
argument_list|,
name|dictSize
argument_list|,
name|params
argument_list|,
name|CNBufferSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|outBuff
operator|.
name|dst
operator|=
operator|(
name|char
operator|*
operator|)
operator|(
name|compressedBuffer
operator|)
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|compressedBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|CNBuffer
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_compressStream_generic
argument_list|(
name|mtctx
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|,
name|ZSTD_e_end
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|inBuff
operator|.
name|pos
operator|!=
name|inBuff
operator|.
name|size
condition|)
goto|goto
name|_output_error
goto|;
comment|/* entire input should be consumed */
block|{
name|size_t
specifier|const
name|r
init|=
name|ZSTDMT_endStream
argument_list|(
name|mtctx
argument_list|,
operator|&
name|outBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
operator|!=
literal|0
condition|)
goto|goto
name|_output_error
goto|;
block|}
comment|/* error, or some data not flushed */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
comment|/* Overlen overwriting window data bug */
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"test%3i : wildcopy doesn't overwrite potential match data : "
argument_list|,
name|testNb
operator|++
argument_list|)
expr_stmt|;
block|{
comment|/* This test has a window size of 1024 bytes and consists of 3 blocks:             1. 'a' repeated 517 times             2. 'b' repeated 516 times             3. a compressed block with no literals and 3 sequence commands:                 litlength = 0, offset = 24, match length = 24                 litlength = 0, offset = 24, match length = 3 (this one creates an overlength write of length 2*WILDCOPY_OVERLENGTH - 3)                 litlength = 0, offset = 1021, match length = 3 (this one will try to read from overwritten data if the buffer is too small) */
specifier|const
name|char
modifier|*
name|testCase
init|=
literal|"\x28\xB5\x2F\xFD\x04\x00\x4C\x00\x00\x10\x61\x61\x01\x00\x00\x2A"
literal|"\x80\x05\x44\x00\x00\x08\x62\x01\x00\x00\x2A\x20\x04\x5D\x00\x00"
literal|"\x00\x03\x40\x00\x00\x64\x60\x27\xB0\xE0\x0C\x67\x62\xCE\xE0"
decl_stmt|;
name|ZSTD_DStream
modifier|*
specifier|const
name|zds
init|=
name|ZSTD_createDStream
argument_list|()
decl_stmt|;
if|if
condition|(
name|zds
operator|==
name|NULL
condition|)
goto|goto
name|_output_error
goto|;
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream
argument_list|(
name|zds
argument_list|)
argument_list|)
expr_stmt|;
name|inBuff
operator|.
name|src
operator|=
name|testCase
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
literal|47
expr_stmt|;
name|inBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
name|outBuff
operator|.
name|dst
operator|=
name|decodedBuffer
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|CNBufferSize
expr_stmt|;
name|outBuff
operator|.
name|pos
operator|=
literal|0
expr_stmt|;
while|while
condition|(
name|inBuff
operator|.
name|pos
operator|<
name|inBuff
operator|.
name|size
condition|)
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_decompressStream
argument_list|(
name|zds
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|ZSTD_freeDStream
argument_list|(
name|zds
argument_list|)
expr_stmt|;
block|}
name|DISPLAYLEVEL
argument_list|(
literal|3
argument_list|,
literal|"OK \n"
argument_list|)
expr_stmt|;
name|_end
label|:
name|FUZ_freeDictionary
argument_list|(
name|dictionary
argument_list|)
expr_stmt|;
name|ZSTD_freeCStream
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|ZSTD_freeDStream
argument_list|(
name|zd
argument_list|)
expr_stmt|;
name|ZSTDMT_freeCCtx
argument_list|(
name|mtctx
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|CNBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|compressedBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|decodedBuffer
argument_list|)
expr_stmt|;
return|return
name|testResult
return|;
name|_output_error
label|:
name|testResult
operator|=
literal|1
expr_stmt|;
name|DISPLAY
argument_list|(
literal|"Error detected in Unit tests ! \n"
argument_list|)
expr_stmt|;
goto|goto
name|_end
goto|;
block|}
end_function

begin_comment
comment|/* ======   Fuzzer tests   ====== */
end_comment

begin_function
specifier|static
name|size_t
name|findDiff
parameter_list|(
specifier|const
name|void
modifier|*
name|buf1
parameter_list|,
specifier|const
name|void
modifier|*
name|buf2
parameter_list|,
name|size_t
name|max
parameter_list|)
block|{
specifier|const
name|BYTE
modifier|*
name|b1
init|=
operator|(
specifier|const
name|BYTE
operator|*
operator|)
name|buf1
decl_stmt|;
specifier|const
name|BYTE
modifier|*
name|b2
init|=
operator|(
specifier|const
name|BYTE
operator|*
operator|)
name|buf2
decl_stmt|;
name|size_t
name|u
decl_stmt|;
for|for
control|(
name|u
operator|=
literal|0
init|;
name|u
operator|<
name|max
condition|;
name|u
operator|++
control|)
block|{
if|if
condition|(
name|b1
index|[
name|u
index|]
operator|!=
name|b2
index|[
name|u
index|]
condition|)
break|break;
block|}
name|DISPLAY
argument_list|(
literal|"Error at position %u / %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|u
argument_list|,
operator|(
name|U32
operator|)
name|max
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" %02X %02X %02X  :%02X:  %02X %02X %02X %02X %02X \n"
argument_list|,
name|b1
index|[
name|u
operator|-
literal|3
index|]
argument_list|,
name|b1
index|[
name|u
operator|-
literal|2
index|]
argument_list|,
name|b1
index|[
name|u
operator|-
literal|1
index|]
argument_list|,
name|b1
index|[
name|u
operator|-
literal|0
index|]
argument_list|,
name|b1
index|[
name|u
operator|+
literal|1
index|]
argument_list|,
name|b1
index|[
name|u
operator|+
literal|2
index|]
argument_list|,
name|b1
index|[
name|u
operator|+
literal|3
index|]
argument_list|,
name|b1
index|[
name|u
operator|+
literal|4
index|]
argument_list|,
name|b1
index|[
name|u
operator|+
literal|5
index|]
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" %02X %02X %02X  :%02X:  %02X %02X %02X %02X %02X \n"
argument_list|,
name|b2
index|[
name|u
operator|-
literal|3
index|]
argument_list|,
name|b2
index|[
name|u
operator|-
literal|2
index|]
argument_list|,
name|b2
index|[
name|u
operator|-
literal|1
index|]
argument_list|,
name|b2
index|[
name|u
operator|-
literal|0
index|]
argument_list|,
name|b2
index|[
name|u
operator|+
literal|1
index|]
argument_list|,
name|b2
index|[
name|u
operator|+
literal|2
index|]
argument_list|,
name|b2
index|[
name|u
operator|+
literal|3
index|]
argument_list|,
name|b2
index|[
name|u
operator|+
literal|4
index|]
argument_list|,
name|b2
index|[
name|u
operator|+
literal|5
index|]
argument_list|)
expr_stmt|;
return|return
name|u
return|;
block|}
end_function

begin_function
specifier|static
name|size_t
name|FUZ_rLogLength
parameter_list|(
name|U32
modifier|*
name|seed
parameter_list|,
name|U32
name|logLength
parameter_list|)
block|{
name|size_t
specifier|const
name|lengthMask
init|=
operator|(
operator|(
name|size_t
operator|)
literal|1
operator|<<
name|logLength
operator|)
operator|-
literal|1
decl_stmt|;
return|return
operator|(
name|lengthMask
operator|+
literal|1
operator|)
operator|+
operator|(
name|FUZ_rand
argument_list|(
name|seed
argument_list|)
operator|&
name|lengthMask
operator|)
return|;
block|}
end_function

begin_function
specifier|static
name|size_t
name|FUZ_randomLength
parameter_list|(
name|U32
modifier|*
name|seed
parameter_list|,
name|U32
name|maxLog
parameter_list|)
block|{
name|U32
specifier|const
name|logLength
init|=
name|FUZ_rand
argument_list|(
name|seed
argument_list|)
operator|%
name|maxLog
decl_stmt|;
return|return
name|FUZ_rLogLength
argument_list|(
name|seed
argument_list|,
name|logLength
argument_list|)
return|;
block|}
end_function

begin_define
define|#
directive|define
name|MIN
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
value|( (a)< (b) ? (a) : (b) )
end_define

begin_comment
comment|/* Return value in range minVal<= v<= maxVal */
end_comment

begin_function
specifier|static
name|U32
name|FUZ_randomClampedLength
parameter_list|(
name|U32
modifier|*
name|seed
parameter_list|,
name|U32
name|minVal
parameter_list|,
name|U32
name|maxVal
parameter_list|)
block|{
name|U32
specifier|const
name|mod
init|=
name|maxVal
operator|<
name|minVal
condition|?
literal|1
else|:
operator|(
name|maxVal
operator|+
literal|1
operator|)
operator|-
name|minVal
decl_stmt|;
return|return
call|(
name|U32
call|)
argument_list|(
operator|(
name|FUZ_rand
argument_list|(
name|seed
argument_list|)
operator|%
name|mod
operator|)
operator|+
name|minVal
argument_list|)
return|;
block|}
end_function

begin_define
define|#
directive|define
name|CHECK
parameter_list|(
name|cond
parameter_list|,
modifier|...
parameter_list|)
value|{                                   \     if (cond) {                                              \         DISPLAY("Error => ");                                \         DISPLAY(__VA_ARGS__);                                \         DISPLAY(" (seed %u, test nb %u)  \n", seed, testNb); \         goto _output_error;                                  \ }   }
end_define

begin_function
specifier|static
name|int
name|fuzzerTests
parameter_list|(
name|U32
name|seed
parameter_list|,
name|U32
name|nbTests
parameter_list|,
name|unsigned
name|startTest
parameter_list|,
name|double
name|compressibility
parameter_list|,
name|int
name|bigTests
parameter_list|)
block|{
name|U32
specifier|const
name|maxSrcLog
init|=
name|bigTests
condition|?
literal|24
else|:
literal|22
decl_stmt|;
specifier|static
specifier|const
name|U32
name|maxSampleLog
init|=
literal|19
decl_stmt|;
name|size_t
specifier|const
name|srcBufferSize
init|=
operator|(
name|size_t
operator|)
literal|1
operator|<<
name|maxSrcLog
decl_stmt|;
name|BYTE
modifier|*
name|cNoiseBuffer
index|[
literal|5
index|]
decl_stmt|;
name|size_t
specifier|const
name|copyBufferSize
init|=
name|srcBufferSize
operator|+
operator|(
literal|1
operator|<<
name|maxSampleLog
operator|)
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|copyBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|copyBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|cBufferSize
init|=
name|ZSTD_compressBound
argument_list|(
name|srcBufferSize
argument_list|)
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|cBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|cBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBufferSize
init|=
name|srcBufferSize
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|dstBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|dstBufferSize
argument_list|)
decl_stmt|;
name|U32
name|result
init|=
literal|0
decl_stmt|;
name|U32
name|testNb
init|=
literal|0
decl_stmt|;
name|U32
name|coreSeed
init|=
name|seed
decl_stmt|;
name|ZSTD_CStream
modifier|*
name|zc
init|=
name|ZSTD_createCStream
argument_list|()
decl_stmt|;
comment|/* will be re-created sometimes */
name|ZSTD_DStream
modifier|*
name|zd
init|=
name|ZSTD_createDStream
argument_list|()
decl_stmt|;
comment|/* will be re-created sometimes */
name|ZSTD_DStream
modifier|*
specifier|const
name|zd_noise
init|=
name|ZSTD_createDStream
argument_list|()
decl_stmt|;
name|clock_t
specifier|const
name|startClock
init|=
name|clock
argument_list|()
decl_stmt|;
specifier|const
name|BYTE
modifier|*
name|dict
init|=
name|NULL
decl_stmt|;
comment|/* can keep same dict on 2 consecutive tests */
name|size_t
name|dictSize
init|=
literal|0
decl_stmt|;
name|U32
name|oldTestLog
init|=
literal|0
decl_stmt|;
name|U32
specifier|const
name|cLevelMax
init|=
name|bigTests
condition|?
operator|(
name|U32
operator|)
name|ZSTD_maxCLevel
argument_list|()
else|:
name|g_cLevelMax_smallTests
decl_stmt|;
comment|/* allocations */
name|cNoiseBuffer
index|[
literal|0
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|1
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|2
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|3
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|4
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
operator|!
name|cNoiseBuffer
index|[
literal|0
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|1
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|2
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|3
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|4
index|]
operator|||
operator|!
name|copyBuffer
operator|||
operator|!
name|dstBuffer
operator|||
operator|!
name|cBuffer
operator|||
operator|!
name|zc
operator|||
operator|!
name|zd
operator|||
operator|!
name|zd_noise
argument_list|,
literal|"Not enough memory, fuzzer tests cancelled"
argument_list|)
expr_stmt|;
comment|/* Create initial samples */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|0
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.00
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* pure noise */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|1
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.05
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* barely compressible */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|2
index|]
argument_list|,
name|srcBufferSize
argument_list|,
name|compressibility
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|3
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.95
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* highly compressible */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|4
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|1.00
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* sparse content */
name|memset
argument_list|(
name|copyBuffer
argument_list|,
literal|0x65
argument_list|,
name|copyBufferSize
argument_list|)
expr_stmt|;
comment|/* make copyBuffer considered initialized */
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* ensure at least one init */
comment|/* catch up testNb */
for|for
control|(
name|testNb
operator|=
literal|1
init|;
name|testNb
operator|<
name|startTest
condition|;
name|testNb
operator|++
control|)
name|FUZ_rand
argument_list|(
operator|&
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* test loop */
for|for
control|(
init|;
operator|(
name|testNb
operator|<=
name|nbTests
operator|)
operator|||
operator|(
name|FUZ_GetClockSpan
argument_list|(
name|startClock
argument_list|)
operator|<
name|g_clockTime
operator|)
condition|;
name|testNb
operator|++
control|)
block|{
name|U32
name|lseed
decl_stmt|;
specifier|const
name|BYTE
modifier|*
name|srcBuffer
decl_stmt|;
name|size_t
name|totalTestSize
decl_stmt|,
name|totalGenSize
decl_stmt|,
name|cSize
decl_stmt|;
name|XXH64_state_t
name|xxhState
decl_stmt|;
name|U64
name|crcOrig
decl_stmt|;
name|U32
name|resetAllowed
init|=
literal|1
decl_stmt|;
name|size_t
name|maxTestSize
decl_stmt|;
comment|/* init */
if|if
condition|(
name|nbTests
operator|>=
name|testNb
condition|)
block|{
name|DISPLAYUPDATE
argument_list|(
literal|2
argument_list|,
literal|"\r%6u/%6u    "
argument_list|,
name|testNb
argument_list|,
name|nbTests
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|DISPLAYUPDATE
argument_list|(
literal|2
argument_list|,
literal|"\r%6u          "
argument_list|,
name|testNb
argument_list|)
expr_stmt|;
block|}
name|FUZ_rand
argument_list|(
operator|&
name|coreSeed
argument_list|)
expr_stmt|;
name|lseed
operator|=
name|coreSeed
operator|^
name|prime32
expr_stmt|;
comment|/* states full reset (deliberately not synchronized) */
comment|/* some issues can only happen when reusing states */
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0xFF
operator|)
operator|==
literal|131
condition|)
block|{
name|ZSTD_freeCStream
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|zc
operator|=
name|ZSTD_createCStream
argument_list|()
expr_stmt|;
name|CHECK
argument_list|(
name|zc
operator|==
name|NULL
argument_list|,
literal|"ZSTD_createCStream : allocation error"
argument_list|)
expr_stmt|;
name|resetAllowed
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0xFF
operator|)
operator|==
literal|132
condition|)
block|{
name|ZSTD_freeDStream
argument_list|(
name|zd
argument_list|)
expr_stmt|;
name|zd
operator|=
name|ZSTD_createDStream
argument_list|()
expr_stmt|;
name|CHECK
argument_list|(
name|zd
operator|==
name|NULL
argument_list|,
literal|"ZSTD_createDStream : allocation error"
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
comment|/* ensure at least one init */
block|}
comment|/* srcBuffer selection [0-4] */
block|{
name|U32
name|buffNb
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0x7F
decl_stmt|;
if|if
condition|(
name|buffNb
operator|&
literal|7
condition|)
name|buffNb
operator|=
literal|2
expr_stmt|;
comment|/* most common : compressible (P) */
else|else
block|{
name|buffNb
operator|>>=
literal|3
expr_stmt|;
if|if
condition|(
name|buffNb
operator|&
literal|7
condition|)
block|{
specifier|const
name|U32
name|tnb
index|[
literal|2
index|]
init|=
block|{
literal|1
block|,
literal|3
block|}
decl_stmt|;
comment|/* barely/highly compressible */
name|buffNb
operator|=
name|tnb
index|[
name|buffNb
operator|>>
literal|3
index|]
expr_stmt|;
block|}
else|else
block|{
specifier|const
name|U32
name|tnb
index|[
literal|2
index|]
init|=
block|{
literal|0
block|,
literal|4
block|}
decl_stmt|;
comment|/* not compressible / sparse */
name|buffNb
operator|=
name|tnb
index|[
name|buffNb
operator|>>
literal|3
index|]
expr_stmt|;
block|}
block|}
name|srcBuffer
operator|=
name|cNoiseBuffer
index|[
name|buffNb
index|]
expr_stmt|;
block|}
comment|/* compression init */
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
operator|)
comment|/* at beginning, to keep same nb of rand */
operator|&&
name|oldTestLog
comment|/* at least one test happened */
operator|&&
name|resetAllowed
condition|)
block|{
name|maxTestSize
operator|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|oldTestLog
operator|+
literal|2
argument_list|)
expr_stmt|;
name|maxTestSize
operator|=
name|MIN
argument_list|(
name|maxTestSize
argument_list|,
name|srcBufferSize
operator|-
literal|16
argument_list|)
expr_stmt|;
block|{
name|U64
specifier|const
name|pledgedSrcSize
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
condition|?
literal|0
else|:
name|maxTestSize
decl_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_resetCStream
argument_list|(
name|zc
argument_list|,
name|pledgedSrcSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|U32
specifier|const
name|testLog
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
name|maxSrcLog
decl_stmt|;
name|U32
specifier|const
name|dictLog
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
name|maxSrcLog
decl_stmt|;
name|U32
specifier|const
name|cLevelCandidate
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|ZSTD_maxCLevel
argument_list|()
operator|-
operator|(
name|MAX
argument_list|(
name|testLog
argument_list|,
name|dictLog
argument_list|)
operator|/
literal|3
operator|)
operator|)
operator|)
operator|+
literal|1
decl_stmt|;
name|U32
specifier|const
name|cLevel
init|=
name|MIN
argument_list|(
name|cLevelCandidate
argument_list|,
name|cLevelMax
argument_list|)
decl_stmt|;
name|maxTestSize
operator|=
name|FUZ_rLogLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|testLog
argument_list|)
expr_stmt|;
name|oldTestLog
operator|=
name|testLog
expr_stmt|;
comment|/* random dictionary selection */
name|dictSize
operator|=
operator|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|7
operator|)
operator|==
literal|1
operator|)
condition|?
name|FUZ_rLogLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|dictLog
argument_list|)
else|:
literal|0
expr_stmt|;
block|{
name|size_t
specifier|const
name|dictStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|dictSize
operator|)
decl_stmt|;
name|dict
operator|=
name|srcBuffer
operator|+
name|dictStart
expr_stmt|;
block|}
block|{
name|U64
specifier|const
name|pledgedSrcSize
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
condition|?
literal|0
else|:
name|maxTestSize
decl_stmt|;
name|ZSTD_parameters
name|params
init|=
name|ZSTD_getParams
argument_list|(
name|cLevel
argument_list|,
name|pledgedSrcSize
argument_list|,
name|dictSize
argument_list|)
decl_stmt|;
name|params
operator|.
name|fParams
operator|.
name|checksumFlag
operator|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
expr_stmt|;
name|params
operator|.
name|fParams
operator|.
name|noDictIDFlag
operator|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_initCStream_advanced
argument_list|(
name|zc
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|,
name|params
argument_list|,
name|pledgedSrcSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* multi-segments compression test */
name|XXH64_reset
argument_list|(
operator|&
name|xxhState
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|{
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|cBuffer
block|,
name|cBufferSize
block|,
literal|0
block|}
decl_stmt|;
name|U32
name|n
decl_stmt|;
for|for
control|(
name|n
operator|=
literal|0
operator|,
name|cSize
operator|=
literal|0
operator|,
name|totalTestSize
operator|=
literal|0
init|;
name|totalTestSize
operator|<
name|maxTestSize
condition|;
name|n
operator|++
control|)
block|{
comment|/* compress random chunks into randomly sized dst buffers */
block|{
name|size_t
specifier|const
name|randomSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|srcSize
init|=
name|MIN
argument_list|(
name|maxTestSize
operator|-
name|totalTestSize
argument_list|,
name|randomSrcSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|srcStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|srcSize
operator|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBuffSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|srcBuffer
operator|+
name|srcStart
block|,
name|srcSize
block|,
literal|0
block|}
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|dstBuffSize
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
name|XXH64_update
argument_list|(
operator|&
name|xxhState
argument_list|,
name|srcBuffer
operator|+
name|srcStart
argument_list|,
name|inBuff
operator|.
name|pos
argument_list|)
expr_stmt|;
name|memcpy
argument_list|(
name|copyBuffer
operator|+
name|totalTestSize
argument_list|,
name|srcBuffer
operator|+
name|srcStart
argument_list|,
name|inBuff
operator|.
name|pos
argument_list|)
expr_stmt|;
name|totalTestSize
operator|+=
name|inBuff
operator|.
name|pos
expr_stmt|;
block|}
comment|/* random flush operation, to mess around */
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|15
operator|)
operator|==
literal|0
condition|)
block|{
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_flushStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* final frame epilogue */
block|{
name|size_t
name|remainingToFlush
init|=
call|(
name|size_t
call|)
argument_list|(
operator|-
literal|1
argument_list|)
decl_stmt|;
while|while
condition|(
name|remainingToFlush
condition|)
block|{
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|remainingToFlush
operator|=
name|ZSTD_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|ZSTD_isError
argument_list|(
name|remainingToFlush
argument_list|)
argument_list|,
literal|"end error : %s"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|remainingToFlush
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
name|crcOrig
operator|=
name|XXH64_digest
argument_list|(
operator|&
name|xxhState
argument_list|)
expr_stmt|;
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
block|}
comment|/* multi - fragments decompression test */
if|if
condition|(
operator|!
name|dictSize
comment|/* don't reset if dictionary : could be different */
operator|&&
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
operator|)
condition|)
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_resetDStream
argument_list|(
name|zd
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|{
name|size_t
name|decompressionResult
init|=
literal|1
decl_stmt|;
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|cBuffer
block|,
name|cSize
block|,
literal|0
block|}
decl_stmt|;
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|dstBuffer
block|,
name|dstBufferSize
block|,
literal|0
block|}
decl_stmt|;
for|for
control|(
name|totalGenSize
operator|=
literal|0
init|;
name|decompressionResult
condition|;
control|)
block|{
name|size_t
specifier|const
name|readCSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBuffSize
init|=
name|MIN
argument_list|(
name|dstBufferSize
operator|-
name|totalGenSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
name|readCSrcSize
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|dstBuffSize
expr_stmt|;
name|decompressionResult
operator|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
expr_stmt|;
if|if
condition|(
name|ZSTD_getErrorCode
argument_list|(
name|decompressionResult
argument_list|)
operator|==
name|ZSTD_error_checksum_wrong
condition|)
block|{
name|DISPLAY
argument_list|(
literal|"checksum error : \n"
argument_list|)
expr_stmt|;
name|findDiff
argument_list|(
name|copyBuffer
argument_list|,
name|dstBuffer
argument_list|,
name|totalTestSize
argument_list|)
expr_stmt|;
block|}
name|CHECK
argument_list|(
name|ZSTD_isError
argument_list|(
name|decompressionResult
argument_list|)
argument_list|,
literal|"decompression error : %s"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|decompressionResult
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|CHECK
argument_list|(
name|decompressionResult
operator|!=
literal|0
argument_list|,
literal|"frame not fully decoded"
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|outBuff
operator|.
name|pos
operator|!=
name|totalTestSize
argument_list|,
literal|"decompressed data : wrong size (%u != %u)"
argument_list|,
operator|(
name|U32
operator|)
name|outBuff
operator|.
name|pos
argument_list|,
operator|(
name|U32
operator|)
name|totalTestSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
argument|inBuff.pos != cSize
argument_list|,
literal|"compressed data should be fully read"
argument_list|)
block|{
name|U64
specifier|const
name|crcDest
init|=
name|XXH64
argument_list|(
name|dstBuffer
argument_list|,
name|totalTestSize
argument_list|,
literal|0
argument_list|)
decl_stmt|;
if|if
condition|(
name|crcDest
operator|!=
name|crcOrig
condition|)
name|findDiff
argument_list|(
name|copyBuffer
argument_list|,
name|dstBuffer
argument_list|,
name|totalTestSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|crcDest
operator|!=
name|crcOrig
argument_list|,
literal|"decompressed data corrupted"
argument_list|)
expr_stmt|;
block|}
block|}
comment|/*=====   noisy/erroneous src decompression test   =====*/
comment|/* add some noise */
block|{
name|U32
specifier|const
name|nbNoiseChunks
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|7
operator|)
operator|+
literal|2
decl_stmt|;
name|U32
name|nn
decl_stmt|;
for|for
control|(
name|nn
operator|=
literal|0
init|;
name|nn
operator|<
name|nbNoiseChunks
condition|;
name|nn
operator|++
control|)
block|{
name|size_t
specifier|const
name|randomNoiseSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|noiseSize
init|=
name|MIN
argument_list|(
operator|(
name|cSize
operator|/
literal|3
operator|)
argument_list|,
name|randomNoiseSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|noiseStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|noiseSize
operator|)
decl_stmt|;
name|size_t
specifier|const
name|cStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|cSize
operator|-
name|noiseSize
operator|)
decl_stmt|;
name|memcpy
argument_list|(
name|cBuffer
operator|+
name|cStart
argument_list|,
name|srcBuffer
operator|+
name|noiseStart
argument_list|,
name|noiseSize
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* try decompression on noisy data */
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream
argument_list|(
name|zd_noise
argument_list|)
argument_list|)
expr_stmt|;
comment|/* note : no dictionary */
block|{
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|cBuffer
block|,
name|cSize
block|,
literal|0
block|}
decl_stmt|;
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|dstBuffer
block|,
name|dstBufferSize
block|,
literal|0
block|}
decl_stmt|;
while|while
condition|(
name|outBuff
operator|.
name|pos
operator|<
name|dstBufferSize
condition|)
block|{
name|size_t
specifier|const
name|randomCSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|dstBufferSize
operator|-
name|outBuff
operator|.
name|pos
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedCSrcSize
init|=
name|MIN
argument_list|(
name|cSize
operator|-
name|inBuff
operator|.
name|pos
argument_list|,
name|randomCSrcSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
name|adjustedCSrcSize
expr_stmt|;
block|{
name|size_t
specifier|const
name|decompressError
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|decompressError
argument_list|)
condition|)
break|break;
comment|/* error correctly detected */
comment|/* No forward progress possible */
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|<
name|outBuff
operator|.
name|size
operator|&&
name|inBuff
operator|.
name|pos
operator|==
name|cSize
condition|)
break|break;
block|}
block|}
block|}
block|}
name|DISPLAY
argument_list|(
literal|"\r%u fuzzer tests completed   \n"
argument_list|,
name|testNb
argument_list|)
expr_stmt|;
name|_cleanup
label|:
name|ZSTD_freeCStream
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|ZSTD_freeDStream
argument_list|(
name|zd
argument_list|)
expr_stmt|;
name|ZSTD_freeDStream
argument_list|(
name|zd_noise
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|2
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|3
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|4
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|copyBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|dstBuffer
argument_list|)
expr_stmt|;
return|return
name|result
return|;
name|_output_error
label|:
name|result
operator|=
literal|1
expr_stmt|;
goto|goto
name|_cleanup
goto|;
block|}
end_function

begin_comment
comment|/* Multi-threading version of fuzzer Tests */
end_comment

begin_function
specifier|static
name|int
name|fuzzerTests_MT
parameter_list|(
name|U32
name|seed
parameter_list|,
name|U32
name|nbTests
parameter_list|,
name|unsigned
name|startTest
parameter_list|,
name|double
name|compressibility
parameter_list|,
name|int
name|bigTests
parameter_list|)
block|{
specifier|const
name|U32
name|maxSrcLog
init|=
name|bigTests
condition|?
literal|24
else|:
literal|22
decl_stmt|;
specifier|static
specifier|const
name|U32
name|maxSampleLog
init|=
literal|19
decl_stmt|;
name|size_t
specifier|const
name|srcBufferSize
init|=
operator|(
name|size_t
operator|)
literal|1
operator|<<
name|maxSrcLog
decl_stmt|;
name|BYTE
modifier|*
name|cNoiseBuffer
index|[
literal|5
index|]
decl_stmt|;
name|size_t
specifier|const
name|copyBufferSize
init|=
name|srcBufferSize
operator|+
operator|(
literal|1
operator|<<
name|maxSampleLog
operator|)
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|copyBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|copyBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|cBufferSize
init|=
name|ZSTD_compressBound
argument_list|(
name|srcBufferSize
argument_list|)
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|cBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|cBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBufferSize
init|=
name|srcBufferSize
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|dstBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|dstBufferSize
argument_list|)
decl_stmt|;
name|U32
name|result
init|=
literal|0
decl_stmt|;
name|U32
name|testNb
init|=
literal|0
decl_stmt|;
name|U32
name|coreSeed
init|=
name|seed
decl_stmt|;
name|U32
name|nbThreads
init|=
literal|2
decl_stmt|;
name|ZSTDMT_CCtx
modifier|*
name|zc
init|=
name|ZSTDMT_createCCtx
argument_list|(
name|nbThreads
argument_list|)
decl_stmt|;
comment|/* will be reset sometimes */
name|ZSTD_DStream
modifier|*
name|zd
init|=
name|ZSTD_createDStream
argument_list|()
decl_stmt|;
comment|/* will be reset sometimes */
name|ZSTD_DStream
modifier|*
specifier|const
name|zd_noise
init|=
name|ZSTD_createDStream
argument_list|()
decl_stmt|;
name|clock_t
specifier|const
name|startClock
init|=
name|clock
argument_list|()
decl_stmt|;
specifier|const
name|BYTE
modifier|*
name|dict
init|=
name|NULL
decl_stmt|;
comment|/* can keep same dict on 2 consecutive tests */
name|size_t
name|dictSize
init|=
literal|0
decl_stmt|;
name|U32
name|oldTestLog
init|=
literal|0
decl_stmt|;
name|int
specifier|const
name|cLevelMax
init|=
name|bigTests
condition|?
operator|(
name|U32
operator|)
name|ZSTD_maxCLevel
argument_list|()
operator|-
literal|1
else|:
name|g_cLevelMax_smallTests
decl_stmt|;
name|U32
specifier|const
name|nbThreadsMax
init|=
name|bigTests
condition|?
literal|4
else|:
literal|2
decl_stmt|;
comment|/* allocations */
name|cNoiseBuffer
index|[
literal|0
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|1
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|2
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|3
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|4
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
operator|!
name|cNoiseBuffer
index|[
literal|0
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|1
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|2
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|3
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|4
index|]
operator|||
operator|!
name|copyBuffer
operator|||
operator|!
name|dstBuffer
operator|||
operator|!
name|cBuffer
operator|||
operator|!
name|zc
operator|||
operator|!
name|zd
operator|||
operator|!
name|zd_noise
argument_list|,
literal|"Not enough memory, fuzzer tests cancelled"
argument_list|)
expr_stmt|;
comment|/* Create initial samples */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|0
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.00
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* pure noise */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|1
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.05
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* barely compressible */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|2
index|]
argument_list|,
name|srcBufferSize
argument_list|,
name|compressibility
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|3
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.95
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* highly compressible */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|4
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|1.00
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* sparse content */
name|memset
argument_list|(
name|copyBuffer
argument_list|,
literal|0x65
argument_list|,
name|copyBufferSize
argument_list|)
expr_stmt|;
comment|/* make copyBuffer considered initialized */
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* ensure at least one init */
comment|/* catch up testNb */
for|for
control|(
name|testNb
operator|=
literal|1
init|;
name|testNb
operator|<
name|startTest
condition|;
name|testNb
operator|++
control|)
name|FUZ_rand
argument_list|(
operator|&
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* test loop */
for|for
control|(
init|;
operator|(
name|testNb
operator|<=
name|nbTests
operator|)
operator|||
operator|(
name|FUZ_GetClockSpan
argument_list|(
name|startClock
argument_list|)
operator|<
name|g_clockTime
operator|)
condition|;
name|testNb
operator|++
control|)
block|{
name|U32
name|lseed
decl_stmt|;
specifier|const
name|BYTE
modifier|*
name|srcBuffer
decl_stmt|;
name|size_t
name|totalTestSize
decl_stmt|,
name|totalGenSize
decl_stmt|,
name|cSize
decl_stmt|;
name|XXH64_state_t
name|xxhState
decl_stmt|;
name|U64
name|crcOrig
decl_stmt|;
name|U32
name|resetAllowed
init|=
literal|1
decl_stmt|;
name|size_t
name|maxTestSize
decl_stmt|;
comment|/* init */
if|if
condition|(
name|testNb
operator|<
name|nbTests
condition|)
block|{
name|DISPLAYUPDATE
argument_list|(
literal|2
argument_list|,
literal|"\r%6u/%6u    "
argument_list|,
name|testNb
argument_list|,
name|nbTests
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|DISPLAYUPDATE
argument_list|(
literal|2
argument_list|,
literal|"\r%6u          "
argument_list|,
name|testNb
argument_list|)
expr_stmt|;
block|}
name|FUZ_rand
argument_list|(
operator|&
name|coreSeed
argument_list|)
expr_stmt|;
name|lseed
operator|=
name|coreSeed
operator|^
name|prime32
expr_stmt|;
comment|/* states full reset (deliberately not synchronized) */
comment|/* some issues can only happen when reusing states */
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0xFF
operator|)
operator|==
literal|131
condition|)
block|{
name|nbThreads
operator|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
name|nbThreadsMax
operator|)
operator|+
literal|1
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Creating new context with %u threads \n"
argument_list|,
name|nbThreads
argument_list|)
expr_stmt|;
name|ZSTDMT_freeCCtx
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|zc
operator|=
name|ZSTDMT_createCCtx
argument_list|(
name|nbThreads
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
argument|zc==NULL
argument_list|,
literal|"ZSTDMT_createCCtx allocation error"
argument_list|)
name|resetAllowed
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0xFF
operator|)
operator|==
literal|132
condition|)
block|{
name|ZSTD_freeDStream
argument_list|(
name|zd
argument_list|)
expr_stmt|;
name|zd
operator|=
name|ZSTD_createDStream
argument_list|()
expr_stmt|;
name|CHECK
argument_list|(
argument|zd==NULL
argument_list|,
literal|"ZSTDMT_createCCtx allocation error"
argument_list|)
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* ensure at least one init */
block|}
comment|/* srcBuffer selection [0-4] */
block|{
name|U32
name|buffNb
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0x7F
decl_stmt|;
if|if
condition|(
name|buffNb
operator|&
literal|7
condition|)
name|buffNb
operator|=
literal|2
expr_stmt|;
comment|/* most common : compressible (P) */
else|else
block|{
name|buffNb
operator|>>=
literal|3
expr_stmt|;
if|if
condition|(
name|buffNb
operator|&
literal|7
condition|)
block|{
specifier|const
name|U32
name|tnb
index|[
literal|2
index|]
init|=
block|{
literal|1
block|,
literal|3
block|}
decl_stmt|;
comment|/* barely/highly compressible */
name|buffNb
operator|=
name|tnb
index|[
name|buffNb
operator|>>
literal|3
index|]
expr_stmt|;
block|}
else|else
block|{
specifier|const
name|U32
name|tnb
index|[
literal|2
index|]
init|=
block|{
literal|0
block|,
literal|4
block|}
decl_stmt|;
comment|/* not compressible / sparse */
name|buffNb
operator|=
name|tnb
index|[
name|buffNb
operator|>>
literal|3
index|]
expr_stmt|;
block|}
block|}
name|srcBuffer
operator|=
name|cNoiseBuffer
index|[
name|buffNb
index|]
expr_stmt|;
block|}
comment|/* compression init */
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
operator|)
comment|/* at beginning, to keep same nb of rand */
operator|&&
name|oldTestLog
comment|/* at least one test happened */
operator|&&
name|resetAllowed
condition|)
block|{
name|maxTestSize
operator|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|oldTestLog
operator|+
literal|2
argument_list|)
expr_stmt|;
if|if
condition|(
name|maxTestSize
operator|>=
name|srcBufferSize
condition|)
name|maxTestSize
operator|=
name|srcBufferSize
operator|-
literal|1
expr_stmt|;
block|{
name|int
specifier|const
name|compressionLevel
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
literal|5
operator|)
operator|+
literal|1
decl_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_initCStream
argument_list|(
name|zc
argument_list|,
name|compressionLevel
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|U32
specifier|const
name|testLog
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
name|maxSrcLog
decl_stmt|;
name|U32
specifier|const
name|dictLog
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
name|maxSrcLog
decl_stmt|;
name|int
specifier|const
name|cLevelCandidate
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|ZSTD_maxCLevel
argument_list|()
operator|-
operator|(
name|MAX
argument_list|(
name|testLog
argument_list|,
name|dictLog
argument_list|)
operator|/
literal|2
operator|)
operator|)
operator|)
operator|+
literal|1
decl_stmt|;
name|int
specifier|const
name|cLevelThreadAdjusted
init|=
name|cLevelCandidate
operator|-
operator|(
name|nbThreads
operator|*
literal|2
operator|)
operator|+
literal|2
decl_stmt|;
comment|/* reduce cLevel when multiple threads to reduce memory consumption */
name|int
specifier|const
name|cLevelMin
init|=
name|MAX
argument_list|(
name|cLevelThreadAdjusted
argument_list|,
literal|1
argument_list|)
decl_stmt|;
comment|/* no negative cLevel yet */
name|int
specifier|const
name|cLevel
init|=
name|MIN
argument_list|(
name|cLevelMin
argument_list|,
name|cLevelMax
argument_list|)
decl_stmt|;
name|maxTestSize
operator|=
name|FUZ_rLogLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|testLog
argument_list|)
expr_stmt|;
name|oldTestLog
operator|=
name|testLog
expr_stmt|;
comment|/* random dictionary selection */
name|dictSize
operator|=
operator|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|63
operator|)
operator|==
literal|1
operator|)
condition|?
name|FUZ_rLogLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|dictLog
argument_list|)
else|:
literal|0
expr_stmt|;
block|{
name|size_t
specifier|const
name|dictStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|dictSize
operator|)
decl_stmt|;
name|dict
operator|=
name|srcBuffer
operator|+
name|dictStart
expr_stmt|;
block|}
block|{
name|U64
specifier|const
name|pledgedSrcSize
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
condition|?
literal|0
else|:
name|maxTestSize
decl_stmt|;
name|ZSTD_parameters
name|params
init|=
name|ZSTD_getParams
argument_list|(
name|cLevel
argument_list|,
name|pledgedSrcSize
argument_list|,
name|dictSize
argument_list|)
decl_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Init with windowLog = %u and pledgedSrcSize = %u \n"
argument_list|,
name|params
operator|.
name|cParams
operator|.
name|windowLog
argument_list|,
operator|(
name|U32
operator|)
name|pledgedSrcSize
argument_list|)
expr_stmt|;
name|params
operator|.
name|fParams
operator|.
name|checksumFlag
operator|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
expr_stmt|;
name|params
operator|.
name|fParams
operator|.
name|noDictIDFlag
operator|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
expr_stmt|;
name|params
operator|.
name|fParams
operator|.
name|contentSizeFlag
operator|=
name|pledgedSrcSize
operator|>
literal|0
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"checksumFlag : %u \n"
argument_list|,
name|params
operator|.
name|fParams
operator|.
name|checksumFlag
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_initCStream_advanced
argument_list|(
name|zc
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|,
name|params
argument_list|,
name|pledgedSrcSize
argument_list|)
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_setMTCtxParameter
argument_list|(
name|zc
argument_list|,
name|ZSTDMT_p_overlapSectionLog
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
literal|12
argument_list|)
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_setMTCtxParameter
argument_list|(
name|zc
argument_list|,
name|ZSTDMT_p_sectionSize
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
literal|2
operator|*
name|maxTestSize
operator|+
literal|1
operator|)
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* multi-segments compression test */
name|XXH64_reset
argument_list|(
operator|&
name|xxhState
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|{
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|cBuffer
block|,
name|cBufferSize
block|,
literal|0
block|}
decl_stmt|;
name|U32
name|n
decl_stmt|;
for|for
control|(
name|n
operator|=
literal|0
operator|,
name|cSize
operator|=
literal|0
operator|,
name|totalTestSize
operator|=
literal|0
init|;
name|totalTestSize
operator|<
name|maxTestSize
condition|;
name|n
operator|++
control|)
block|{
comment|/* compress random chunks into randomly sized dst buffers */
block|{
name|size_t
specifier|const
name|randomSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|srcSize
init|=
name|MIN
argument_list|(
name|maxTestSize
operator|-
name|totalTestSize
argument_list|,
name|randomSrcSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|srcStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|srcSize
operator|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBuffSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|srcBuffer
operator|+
name|srcStart
block|,
name|srcSize
block|,
literal|0
block|}
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|dstBuffSize
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Sending %u bytes to compress \n"
argument_list|,
operator|(
name|U32
operator|)
name|srcSize
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_compressStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"%u bytes read by ZSTDMT_compressStream \n"
argument_list|,
operator|(
name|U32
operator|)
name|inBuff
operator|.
name|pos
argument_list|)
expr_stmt|;
name|XXH64_update
argument_list|(
operator|&
name|xxhState
argument_list|,
name|srcBuffer
operator|+
name|srcStart
argument_list|,
name|inBuff
operator|.
name|pos
argument_list|)
expr_stmt|;
name|memcpy
argument_list|(
name|copyBuffer
operator|+
name|totalTestSize
argument_list|,
name|srcBuffer
operator|+
name|srcStart
argument_list|,
name|inBuff
operator|.
name|pos
argument_list|)
expr_stmt|;
name|totalTestSize
operator|+=
name|inBuff
operator|.
name|pos
expr_stmt|;
block|}
comment|/* random flush operation, to mess around */
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|15
operator|)
operator|==
literal|0
condition|)
block|{
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Flushing into dst buffer of size %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|adjustedDstSize
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTDMT_flushStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* final frame epilogue */
block|{
name|size_t
name|remainingToFlush
init|=
call|(
name|size_t
call|)
argument_list|(
operator|-
literal|1
argument_list|)
decl_stmt|;
while|while
condition|(
name|remainingToFlush
condition|)
block|{
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Ending into dst buffer of size %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|adjustedDstSize
argument_list|)
expr_stmt|;
name|remainingToFlush
operator|=
name|ZSTDMT_endStream
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|ZSTD_isError
argument_list|(
name|remainingToFlush
argument_list|)
argument_list|,
literal|"ZSTDMT_endStream error : %s"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|remainingToFlush
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"endStream : remainingToFlush : %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|remainingToFlush
argument_list|)
expr_stmt|;
block|}
block|}
name|crcOrig
operator|=
name|XXH64_digest
argument_list|(
operator|&
name|xxhState
argument_list|)
expr_stmt|;
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Frame completed : %u bytes \n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|)
expr_stmt|;
block|}
comment|/* multi - fragments decompression test */
if|if
condition|(
operator|!
name|dictSize
comment|/* don't reset if dictionary : could be different */
operator|&&
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
operator|)
condition|)
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_resetDStream
argument_list|(
name|zd
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|{
name|size_t
name|decompressionResult
init|=
literal|1
decl_stmt|;
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|cBuffer
block|,
name|cSize
block|,
literal|0
block|}
decl_stmt|;
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|dstBuffer
block|,
name|dstBufferSize
block|,
literal|0
block|}
decl_stmt|;
for|for
control|(
name|totalGenSize
operator|=
literal|0
init|;
name|decompressionResult
condition|;
control|)
block|{
name|size_t
specifier|const
name|readCSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBuffSize
init|=
name|MIN
argument_list|(
name|dstBufferSize
operator|-
name|totalGenSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
name|readCSrcSize
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|dstBuffSize
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"ZSTD_decompressStream input %u bytes \n"
argument_list|,
operator|(
name|U32
operator|)
name|readCSrcSize
argument_list|)
expr_stmt|;
name|decompressionResult
operator|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|ZSTD_isError
argument_list|(
name|decompressionResult
argument_list|)
argument_list|,
literal|"decompression error : %s"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|decompressionResult
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"inBuff.pos = %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|readCSrcSize
argument_list|)
expr_stmt|;
block|}
name|CHECK
argument_list|(
name|outBuff
operator|.
name|pos
operator|!=
name|totalTestSize
argument_list|,
literal|"decompressed data : wrong size (%u != %u)"
argument_list|,
operator|(
name|U32
operator|)
name|outBuff
operator|.
name|pos
argument_list|,
operator|(
name|U32
operator|)
name|totalTestSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|inBuff
operator|.
name|pos
operator|!=
name|cSize
argument_list|,
literal|"compressed data should be fully read (%u != %u)"
argument_list|,
operator|(
name|U32
operator|)
name|inBuff
operator|.
name|pos
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|)
expr_stmt|;
block|{
name|U64
specifier|const
name|crcDest
init|=
name|XXH64
argument_list|(
name|dstBuffer
argument_list|,
name|totalTestSize
argument_list|,
literal|0
argument_list|)
decl_stmt|;
if|if
condition|(
name|crcDest
operator|!=
name|crcOrig
condition|)
name|findDiff
argument_list|(
name|copyBuffer
argument_list|,
name|dstBuffer
argument_list|,
name|totalTestSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|crcDest
operator|!=
name|crcOrig
argument_list|,
literal|"decompressed data corrupted"
argument_list|)
expr_stmt|;
block|}
block|}
comment|/*=====   noisy/erroneous src decompression test   =====*/
comment|/* add some noise */
block|{
name|U32
specifier|const
name|nbNoiseChunks
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|7
operator|)
operator|+
literal|2
decl_stmt|;
name|U32
name|nn
decl_stmt|;
for|for
control|(
name|nn
operator|=
literal|0
init|;
name|nn
operator|<
name|nbNoiseChunks
condition|;
name|nn
operator|++
control|)
block|{
name|size_t
specifier|const
name|randomNoiseSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|noiseSize
init|=
name|MIN
argument_list|(
operator|(
name|cSize
operator|/
literal|3
operator|)
argument_list|,
name|randomNoiseSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|noiseStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|noiseSize
operator|)
decl_stmt|;
name|size_t
specifier|const
name|cStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|cSize
operator|-
name|noiseSize
operator|)
decl_stmt|;
name|memcpy
argument_list|(
name|cBuffer
operator|+
name|cStart
argument_list|,
name|srcBuffer
operator|+
name|noiseStart
argument_list|,
name|noiseSize
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* try decompression on noisy data */
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream
argument_list|(
name|zd_noise
argument_list|)
argument_list|)
expr_stmt|;
comment|/* note : no dictionary */
block|{
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|cBuffer
block|,
name|cSize
block|,
literal|0
block|}
decl_stmt|;
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|dstBuffer
block|,
name|dstBufferSize
block|,
literal|0
block|}
decl_stmt|;
while|while
condition|(
name|outBuff
operator|.
name|pos
operator|<
name|dstBufferSize
condition|)
block|{
name|size_t
specifier|const
name|randomCSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|dstBufferSize
operator|-
name|outBuff
operator|.
name|pos
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedCSrcSize
init|=
name|MIN
argument_list|(
name|cSize
operator|-
name|inBuff
operator|.
name|pos
argument_list|,
name|randomCSrcSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
name|adjustedCSrcSize
expr_stmt|;
block|{
name|size_t
specifier|const
name|decompressError
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|decompressError
argument_list|)
condition|)
break|break;
comment|/* error correctly detected */
comment|/* No forward progress possible */
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|<
name|outBuff
operator|.
name|size
operator|&&
name|inBuff
operator|.
name|pos
operator|==
name|cSize
condition|)
break|break;
block|}
block|}
block|}
block|}
name|DISPLAY
argument_list|(
literal|"\r%u fuzzer tests completed   \n"
argument_list|,
name|testNb
argument_list|)
expr_stmt|;
name|_cleanup
label|:
name|ZSTDMT_freeCCtx
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|ZSTD_freeDStream
argument_list|(
name|zd
argument_list|)
expr_stmt|;
name|ZSTD_freeDStream
argument_list|(
name|zd_noise
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|2
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|3
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|4
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|copyBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|dstBuffer
argument_list|)
expr_stmt|;
return|return
name|result
return|;
name|_output_error
label|:
name|result
operator|=
literal|1
expr_stmt|;
goto|goto
name|_cleanup
goto|;
block|}
end_function

begin_comment
comment|/** If useOpaqueAPI, sets param in cctxParams.  *  Otherwise, sets the param in zc. */
end_comment

begin_function
specifier|static
name|size_t
name|setCCtxParameter
parameter_list|(
name|ZSTD_CCtx
modifier|*
name|zc
parameter_list|,
name|ZSTD_CCtx_params
modifier|*
name|cctxParams
parameter_list|,
name|ZSTD_cParameter
name|param
parameter_list|,
name|unsigned
name|value
parameter_list|,
name|U32
name|useOpaqueAPI
parameter_list|)
block|{
if|if
condition|(
name|useOpaqueAPI
condition|)
block|{
return|return
name|ZSTD_CCtxParam_setParameter
argument_list|(
name|cctxParams
argument_list|,
name|param
argument_list|,
name|value
argument_list|)
return|;
block|}
else|else
block|{
return|return
name|ZSTD_CCtx_setParameter
argument_list|(
name|zc
argument_list|,
name|param
argument_list|,
name|value
argument_list|)
return|;
block|}
block|}
end_function

begin_comment
comment|/* Tests for ZSTD_compress_generic() API */
end_comment

begin_function
specifier|static
name|int
name|fuzzerTests_newAPI
parameter_list|(
name|U32
name|seed
parameter_list|,
name|U32
name|nbTests
parameter_list|,
name|unsigned
name|startTest
parameter_list|,
name|double
name|compressibility
parameter_list|,
name|int
name|bigTests
parameter_list|,
name|U32
specifier|const
name|useOpaqueAPI
parameter_list|)
block|{
name|U32
specifier|const
name|maxSrcLog
init|=
name|bigTests
condition|?
literal|24
else|:
literal|22
decl_stmt|;
specifier|static
specifier|const
name|U32
name|maxSampleLog
init|=
literal|19
decl_stmt|;
name|size_t
specifier|const
name|srcBufferSize
init|=
operator|(
name|size_t
operator|)
literal|1
operator|<<
name|maxSrcLog
decl_stmt|;
name|BYTE
modifier|*
name|cNoiseBuffer
index|[
literal|5
index|]
decl_stmt|;
name|size_t
specifier|const
name|copyBufferSize
init|=
name|srcBufferSize
operator|+
operator|(
literal|1
operator|<<
name|maxSampleLog
operator|)
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|copyBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|copyBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|cBufferSize
init|=
name|ZSTD_compressBound
argument_list|(
name|srcBufferSize
argument_list|)
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|cBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|cBufferSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBufferSize
init|=
name|srcBufferSize
decl_stmt|;
name|BYTE
modifier|*
specifier|const
name|dstBuffer
init|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|dstBufferSize
argument_list|)
decl_stmt|;
name|U32
name|result
init|=
literal|0
decl_stmt|;
name|U32
name|testNb
init|=
literal|0
decl_stmt|;
name|U32
name|coreSeed
init|=
name|seed
decl_stmt|;
name|ZSTD_CCtx
modifier|*
name|zc
init|=
name|ZSTD_createCCtx
argument_list|()
decl_stmt|;
comment|/* will be reset sometimes */
name|ZSTD_DStream
modifier|*
name|zd
init|=
name|ZSTD_createDStream
argument_list|()
decl_stmt|;
comment|/* will be reset sometimes */
name|ZSTD_DStream
modifier|*
specifier|const
name|zd_noise
init|=
name|ZSTD_createDStream
argument_list|()
decl_stmt|;
name|clock_t
specifier|const
name|startClock
init|=
name|clock
argument_list|()
decl_stmt|;
specifier|const
name|BYTE
modifier|*
name|dict
init|=
name|NULL
decl_stmt|;
comment|/* can keep same dict on 2 consecutive tests */
name|size_t
name|dictSize
init|=
literal|0
decl_stmt|;
name|U32
name|oldTestLog
init|=
literal|0
decl_stmt|;
name|U32
name|windowLogMalus
init|=
literal|0
decl_stmt|;
comment|/* can survive between 2 loops */
name|U32
specifier|const
name|cLevelMax
init|=
name|bigTests
condition|?
operator|(
name|U32
operator|)
name|ZSTD_maxCLevel
argument_list|()
operator|-
literal|1
else|:
name|g_cLevelMax_smallTests
decl_stmt|;
name|U32
specifier|const
name|nbThreadsMax
init|=
name|bigTests
condition|?
literal|4
else|:
literal|2
decl_stmt|;
name|ZSTD_CCtx_params
modifier|*
name|cctxParams
init|=
name|ZSTD_createCCtxParams
argument_list|()
decl_stmt|;
comment|/* allocations */
name|cNoiseBuffer
index|[
literal|0
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|1
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|2
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|3
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|cNoiseBuffer
index|[
literal|4
index|]
operator|=
operator|(
name|BYTE
operator|*
operator|)
name|malloc
argument_list|(
name|srcBufferSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
operator|!
name|cNoiseBuffer
index|[
literal|0
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|1
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|2
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|3
index|]
operator|||
operator|!
name|cNoiseBuffer
index|[
literal|4
index|]
operator|||
operator|!
name|copyBuffer
operator|||
operator|!
name|dstBuffer
operator|||
operator|!
name|cBuffer
operator|||
operator|!
name|zc
operator|||
operator|!
name|zd
operator|||
operator|!
name|zd_noise
argument_list|,
literal|"Not enough memory, fuzzer tests cancelled"
argument_list|)
expr_stmt|;
comment|/* Create initial samples */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|0
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.00
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* pure noise */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|1
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.05
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* barely compressible */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|2
index|]
argument_list|,
name|srcBufferSize
argument_list|,
name|compressibility
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|3
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|0.95
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* highly compressible */
name|RDG_genBuffer
argument_list|(
name|cNoiseBuffer
index|[
literal|4
index|]
argument_list|,
name|srcBufferSize
argument_list|,
literal|1.00
argument_list|,
literal|0.
argument_list|,
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* sparse content */
name|memset
argument_list|(
name|copyBuffer
argument_list|,
literal|0x65
argument_list|,
name|copyBufferSize
argument_list|)
expr_stmt|;
comment|/* make copyBuffer considered initialized */
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
comment|/* ensure at least one init */
comment|/* catch up testNb */
for|for
control|(
name|testNb
operator|=
literal|1
init|;
name|testNb
operator|<
name|startTest
condition|;
name|testNb
operator|++
control|)
name|FUZ_rand
argument_list|(
operator|&
name|coreSeed
argument_list|)
expr_stmt|;
comment|/* test loop */
for|for
control|(
init|;
operator|(
name|testNb
operator|<=
name|nbTests
operator|)
operator|||
operator|(
name|FUZ_GetClockSpan
argument_list|(
name|startClock
argument_list|)
operator|<
name|g_clockTime
operator|)
condition|;
name|testNb
operator|++
control|)
block|{
name|U32
name|lseed
decl_stmt|;
specifier|const
name|BYTE
modifier|*
name|srcBuffer
decl_stmt|;
name|size_t
name|totalTestSize
decl_stmt|,
name|totalGenSize
decl_stmt|,
name|cSize
decl_stmt|;
name|XXH64_state_t
name|xxhState
decl_stmt|;
name|U64
name|crcOrig
decl_stmt|;
name|U32
name|resetAllowed
init|=
literal|1
decl_stmt|;
name|size_t
name|maxTestSize
decl_stmt|;
comment|/* init */
if|if
condition|(
name|nbTests
operator|>=
name|testNb
condition|)
block|{
name|DISPLAYUPDATE
argument_list|(
literal|2
argument_list|,
literal|"\r%6u/%6u    "
argument_list|,
name|testNb
argument_list|,
name|nbTests
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|DISPLAYUPDATE
argument_list|(
literal|2
argument_list|,
literal|"\r%6u          "
argument_list|,
name|testNb
argument_list|)
expr_stmt|;
block|}
name|FUZ_rand
argument_list|(
operator|&
name|coreSeed
argument_list|)
expr_stmt|;
name|lseed
operator|=
name|coreSeed
operator|^
name|prime32
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|" ***  Test %u  *** \n"
argument_list|,
name|testNb
argument_list|)
expr_stmt|;
comment|/* states full reset (deliberately not synchronized) */
comment|/* some issues can only happen when reusing states */
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0xFF
operator|)
operator|==
literal|131
condition|)
block|{
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Creating new context \n"
argument_list|)
expr_stmt|;
name|ZSTD_freeCCtx
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|zc
operator|=
name|ZSTD_createCCtx
argument_list|()
expr_stmt|;
name|CHECK
argument_list|(
name|zc
operator|==
name|NULL
argument_list|,
literal|"ZSTD_createCCtx allocation error"
argument_list|)
expr_stmt|;
name|resetAllowed
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0xFF
operator|)
operator|==
literal|132
condition|)
block|{
name|ZSTD_freeDStream
argument_list|(
name|zd
argument_list|)
expr_stmt|;
name|zd
operator|=
name|ZSTD_createDStream
argument_list|()
expr_stmt|;
name|CHECK
argument_list|(
name|zd
operator|==
name|NULL
argument_list|,
literal|"ZSTD_createDStream allocation error"
argument_list|)
expr_stmt|;
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* ensure at least one init */
block|}
comment|/* srcBuffer selection [0-4] */
block|{
name|U32
name|buffNb
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|0x7F
decl_stmt|;
if|if
condition|(
name|buffNb
operator|&
literal|7
condition|)
name|buffNb
operator|=
literal|2
expr_stmt|;
comment|/* most common : compressible (P) */
else|else
block|{
name|buffNb
operator|>>=
literal|3
expr_stmt|;
if|if
condition|(
name|buffNb
operator|&
literal|7
condition|)
block|{
specifier|const
name|U32
name|tnb
index|[
literal|2
index|]
init|=
block|{
literal|1
block|,
literal|3
block|}
decl_stmt|;
comment|/* barely/highly compressible */
name|buffNb
operator|=
name|tnb
index|[
name|buffNb
operator|>>
literal|3
index|]
expr_stmt|;
block|}
else|else
block|{
specifier|const
name|U32
name|tnb
index|[
literal|2
index|]
init|=
block|{
literal|0
block|,
literal|4
block|}
decl_stmt|;
comment|/* not compressible / sparse */
name|buffNb
operator|=
name|tnb
index|[
name|buffNb
operator|>>
literal|3
index|]
expr_stmt|;
block|}
block|}
name|srcBuffer
operator|=
name|cNoiseBuffer
index|[
name|buffNb
index|]
expr_stmt|;
block|}
comment|/* compression init */
name|CHECK_Z
argument_list|(
name|ZSTD_CCtx_loadDictionary
argument_list|(
name|zc
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
argument_list|)
expr_stmt|;
comment|/* cancel previous dict /*/
if|if
condition|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
operator|)
comment|/* at beginning, to keep same nb of rand */
operator|&&
name|oldTestLog
comment|/* at least one test happened */
operator|&&
name|resetAllowed
condition|)
block|{
name|maxTestSize
operator|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|oldTestLog
operator|+
literal|2
argument_list|)
expr_stmt|;
if|if
condition|(
name|maxTestSize
operator|>=
name|srcBufferSize
condition|)
name|maxTestSize
operator|=
name|srcBufferSize
operator|-
literal|1
expr_stmt|;
block|{
name|int
specifier|const
name|compressionLevel
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
literal|5
operator|)
operator|+
literal|1
decl_stmt|;
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_compressionLevel
argument_list|,
name|compressionLevel
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|U32
specifier|const
name|testLog
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
name|maxSrcLog
decl_stmt|;
name|U32
specifier|const
name|dictLog
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
name|maxSrcLog
decl_stmt|;
name|U32
specifier|const
name|cLevelCandidate
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|ZSTD_maxCLevel
argument_list|()
operator|-
operator|(
name|MAX
argument_list|(
name|testLog
argument_list|,
name|dictLog
argument_list|)
operator|/
literal|2
operator|)
operator|)
operator|)
operator|+
literal|1
decl_stmt|;
name|U32
specifier|const
name|cLevel
init|=
name|MIN
argument_list|(
name|cLevelCandidate
argument_list|,
name|cLevelMax
argument_list|)
decl_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"t%u: cLevel : %u \n"
argument_list|,
name|testNb
argument_list|,
name|cLevel
argument_list|)
expr_stmt|;
name|maxTestSize
operator|=
name|FUZ_rLogLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|testLog
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"t%u: maxTestSize : %u \n"
argument_list|,
name|testNb
argument_list|,
operator|(
name|U32
operator|)
name|maxTestSize
argument_list|)
expr_stmt|;
name|oldTestLog
operator|=
name|testLog
expr_stmt|;
comment|/* random dictionary selection */
name|dictSize
operator|=
operator|(
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|63
operator|)
operator|==
literal|1
operator|)
condition|?
name|FUZ_rLogLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|dictLog
argument_list|)
else|:
literal|0
expr_stmt|;
block|{
name|size_t
specifier|const
name|dictStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|dictSize
operator|)
decl_stmt|;
name|dict
operator|=
name|srcBuffer
operator|+
name|dictStart
expr_stmt|;
if|if
condition|(
operator|!
name|dictSize
condition|)
name|dict
operator|=
name|NULL
expr_stmt|;
block|}
block|{
name|U64
specifier|const
name|pledgedSrcSize
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
condition|?
name|ZSTD_CONTENTSIZE_UNKNOWN
else|:
name|maxTestSize
decl_stmt|;
name|ZSTD_compressionParameters
name|cParams
init|=
name|ZSTD_getCParams
argument_list|(
name|cLevel
argument_list|,
name|pledgedSrcSize
argument_list|,
name|dictSize
argument_list|)
decl_stmt|;
specifier|static
specifier|const
name|U32
name|windowLogMax
init|=
literal|24
decl_stmt|;
comment|/* mess with compression parameters */
name|cParams
operator|.
name|windowLog
operator|+=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
operator|-
literal|1
expr_stmt|;
name|cParams
operator|.
name|windowLog
operator|=
name|MIN
argument_list|(
name|windowLogMax
argument_list|,
name|cParams
operator|.
name|windowLog
argument_list|)
expr_stmt|;
name|cParams
operator|.
name|hashLog
operator|+=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
operator|-
literal|1
expr_stmt|;
name|cParams
operator|.
name|chainLog
operator|+=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
operator|-
literal|1
expr_stmt|;
name|cParams
operator|.
name|searchLog
operator|+=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
operator|-
literal|1
expr_stmt|;
name|cParams
operator|.
name|searchLength
operator|+=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
operator|)
operator|-
literal|1
expr_stmt|;
name|cParams
operator|.
name|targetLength
operator|=
call|(
name|U32
call|)
argument_list|(
operator|(
name|cParams
operator|.
name|targetLength
operator|+
literal|1
operator|)
operator|*
operator|(
literal|0.5
operator|+
operator|(
call|(
name|double
call|)
argument_list|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|127
argument_list|)
operator|/
literal|128
operator|)
operator|)
argument_list|)
expr_stmt|;
name|cParams
operator|=
name|ZSTD_adjustCParams
argument_list|(
name|cParams
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
block|{
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_windowLog
argument_list|,
name|cParams
operator|.
name|windowLog
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
name|assert
argument_list|(
name|cParams
operator|.
name|windowLog
operator|>=
name|ZSTD_WINDOWLOG_MIN
argument_list|)
expr_stmt|;
comment|/* guaranteed by ZSTD_adjustCParams() */
name|windowLogMalus
operator|=
operator|(
name|cParams
operator|.
name|windowLog
operator|-
name|ZSTD_WINDOWLOG_MIN
operator|)
operator|/
literal|5
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"t%u: windowLog : %u \n"
argument_list|,
name|testNb
argument_list|,
name|cParams
operator|.
name|windowLog
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
block|{
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_hashLog
argument_list|,
name|cParams
operator|.
name|hashLog
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"t%u: hashLog : %u \n"
argument_list|,
name|testNb
argument_list|,
name|cParams
operator|.
name|hashLog
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
block|{
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_chainLog
argument_list|,
name|cParams
operator|.
name|chainLog
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"t%u: chainLog : %u \n"
argument_list|,
name|testNb
argument_list|,
name|cParams
operator|.
name|chainLog
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_searchLog
argument_list|,
name|cParams
operator|.
name|searchLog
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_minMatch
argument_list|,
name|cParams
operator|.
name|searchLength
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_targetLength
argument_list|,
name|cParams
operator|.
name|targetLength
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
comment|/* mess with long distance matching parameters */
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_enableLongDistanceMatching
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|63
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_ldmHashLog
argument_list|,
name|FUZ_randomClampedLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|ZSTD_HASHLOG_MIN
argument_list|,
literal|23
argument_list|)
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_ldmMinMatch
argument_list|,
name|FUZ_randomClampedLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|ZSTD_LDM_MINMATCH_MIN
argument_list|,
name|ZSTD_LDM_MINMATCH_MAX
argument_list|)
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_ldmBucketSizeLog
argument_list|,
name|FUZ_randomClampedLength
argument_list|(
operator|&
name|lseed
argument_list|,
literal|0
argument_list|,
name|ZSTD_LDM_BUCKETSIZELOG_MAX
argument_list|)
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|3
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_ldmHashEveryLog
argument_list|,
name|FUZ_randomClampedLength
argument_list|(
operator|&
name|lseed
argument_list|,
literal|0
argument_list|,
name|ZSTD_WINDOWLOG_MAX
operator|-
name|ZSTD_HASHLOG_MIN
argument_list|)
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
comment|/* mess with frame parameters */
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_checksumFlag
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_dictIDFlag
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_contentSizeFlag
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|ZSTD_CCtx_setPledgedSrcSize
argument_list|(
name|zc
argument_list|,
name|pledgedSrcSize
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"t%u: pledgedSrcSize : %u \n"
argument_list|,
name|testNb
argument_list|,
operator|(
name|U32
operator|)
name|pledgedSrcSize
argument_list|)
expr_stmt|;
comment|/* multi-threading parameters */
block|{
name|U32
specifier|const
name|nbThreadsCandidate
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|4
operator|)
operator|+
literal|1
decl_stmt|;
name|U32
specifier|const
name|nbThreadsAdjusted
init|=
operator|(
name|windowLogMalus
operator|<
name|nbThreadsCandidate
operator|)
condition|?
name|nbThreadsCandidate
operator|-
name|windowLogMalus
else|:
literal|1
decl_stmt|;
name|U32
specifier|const
name|nbThreads
init|=
name|MIN
argument_list|(
name|nbThreadsAdjusted
argument_list|,
name|nbThreadsMax
argument_list|)
decl_stmt|;
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_nbThreads
argument_list|,
name|nbThreads
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"t%u: nbThreads : %u \n"
argument_list|,
name|testNb
argument_list|,
name|nbThreads
argument_list|)
expr_stmt|;
if|if
condition|(
name|nbThreads
operator|>
literal|1
condition|)
block|{
name|U32
specifier|const
name|jobLog
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|testLog
operator|+
literal|1
operator|)
decl_stmt|;
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_overlapSizeLog
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
literal|10
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_jobSize
argument_list|,
operator|(
name|U32
operator|)
name|FUZ_rLogLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|jobLog
argument_list|)
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
name|CHECK_Z
argument_list|(
name|setCCtxParameter
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|,
name|ZSTD_p_forceMaxWindow
argument_list|,
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
argument_list|,
name|useOpaqueAPI
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Apply parameters */
if|if
condition|(
name|useOpaqueAPI
condition|)
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_CCtx_setParametersUsingCCtxParams
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
block|{
if|if
condition|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
condition|)
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_CCtx_loadDictionary
argument_list|(
name|zc
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_CCtx_loadDictionary_byReference
argument_list|(
name|zc
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|dict
operator|&&
name|dictSize
condition|)
block|{
comment|/* test that compression parameters are rejected (correctly) after loading a non-NULL dictionary */
if|if
condition|(
name|useOpaqueAPI
condition|)
block|{
name|size_t
specifier|const
name|setError
init|=
name|ZSTD_CCtx_setParametersUsingCCtxParams
argument_list|(
name|zc
argument_list|,
name|cctxParams
argument_list|)
decl_stmt|;
name|CHECK
argument_list|(
operator|!
name|ZSTD_isError
argument_list|(
name|setError
argument_list|)
argument_list|,
literal|"ZSTD_CCtx_setParametersUsingCCtxParams should have failed"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|size_t
specifier|const
name|setError
init|=
name|ZSTD_CCtx_setParameter
argument_list|(
name|zc
argument_list|,
name|ZSTD_p_windowLog
argument_list|,
name|cParams
operator|.
name|windowLog
operator|-
literal|1
argument_list|)
decl_stmt|;
name|CHECK
argument_list|(
operator|!
name|ZSTD_isError
argument_list|(
name|setError
argument_list|)
argument_list|,
literal|"ZSTD_CCtx_setParameter should have failed"
argument_list|)
expr_stmt|;
block|}
block|}
block|}
else|else
block|{
name|CHECK_Z
argument_list|(
name|ZSTD_CCtx_refPrefix
argument_list|(
name|zc
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
block|}
comment|/* multi-segments compression test */
name|XXH64_reset
argument_list|(
operator|&
name|xxhState
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|{
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|cBuffer
block|,
name|cBufferSize
block|,
literal|0
block|}
decl_stmt|;
for|for
control|(
name|cSize
operator|=
literal|0
operator|,
name|totalTestSize
operator|=
literal|0
init|;
operator|(
name|totalTestSize
operator|<
name|maxTestSize
operator|)
condition|;
control|)
block|{
comment|/* compress random chunks into randomly sized dst buffers */
name|size_t
specifier|const
name|randomSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|srcSize
init|=
name|MIN
argument_list|(
name|maxTestSize
operator|-
name|totalTestSize
argument_list|,
name|randomSrcSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|srcStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|srcSize
operator|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
operator|+
literal|1
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBuffSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|ZSTD_EndDirective
specifier|const
name|flush
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|15
operator|)
condition|?
name|ZSTD_e_continue
else|:
name|ZSTD_e_flush
decl_stmt|;
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|srcBuffer
operator|+
name|srcStart
block|,
name|srcSize
block|,
literal|0
block|}
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|dstBuffSize
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_compress_generic
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|6
argument_list|,
literal|"compress consumed %u bytes (total : %u) \n"
argument_list|,
operator|(
name|U32
operator|)
name|inBuff
operator|.
name|pos
argument_list|,
call|(
name|U32
call|)
argument_list|(
name|totalTestSize
operator|+
name|inBuff
operator|.
name|pos
argument_list|)
argument_list|)
expr_stmt|;
name|XXH64_update
argument_list|(
operator|&
name|xxhState
argument_list|,
name|srcBuffer
operator|+
name|srcStart
argument_list|,
name|inBuff
operator|.
name|pos
argument_list|)
expr_stmt|;
name|memcpy
argument_list|(
name|copyBuffer
operator|+
name|totalTestSize
argument_list|,
name|srcBuffer
operator|+
name|srcStart
argument_list|,
name|inBuff
operator|.
name|pos
argument_list|)
expr_stmt|;
name|totalTestSize
operator|+=
name|inBuff
operator|.
name|pos
expr_stmt|;
block|}
comment|/* final frame epilogue */
block|{
name|size_t
name|remainingToFlush
init|=
call|(
name|size_t
call|)
argument_list|(
operator|-
literal|1
argument_list|)
decl_stmt|;
while|while
condition|(
name|remainingToFlush
condition|)
block|{
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|NULL
block|,
literal|0
block|,
literal|0
block|}
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
operator|+
literal|1
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|cBufferSize
operator|-
name|cSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|6
argument_list|,
literal|"End-flush into dst buffer of size %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|adjustedDstSize
argument_list|)
expr_stmt|;
name|remainingToFlush
operator|=
name|ZSTD_compress_generic
argument_list|(
name|zc
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|,
name|ZSTD_e_end
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|ZSTD_isError
argument_list|(
name|remainingToFlush
argument_list|)
argument_list|,
literal|"ZSTD_compress_generic w/ ZSTD_e_end error : %s"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|remainingToFlush
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
name|crcOrig
operator|=
name|XXH64_digest
argument_list|(
operator|&
name|xxhState
argument_list|)
expr_stmt|;
name|cSize
operator|=
name|outBuff
operator|.
name|pos
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"Frame completed : %u bytes \n"
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|)
expr_stmt|;
block|}
comment|/* multi - fragments decompression test */
if|if
condition|(
operator|!
name|dictSize
comment|/* don't reset if dictionary : could be different */
operator|&&
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|1
operator|)
condition|)
block|{
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"resetting DCtx (dict:%08X) \n"
argument_list|,
operator|(
name|U32
operator|)
operator|(
name|size_t
operator|)
name|dict
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_resetDStream
argument_list|(
name|zd
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|DISPLAYLEVEL
argument_list|(
literal|5
argument_list|,
literal|"using dict of size %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|dictSize
argument_list|)
expr_stmt|;
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream_usingDict
argument_list|(
name|zd
argument_list|,
name|dict
argument_list|,
name|dictSize
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|{
name|size_t
name|decompressionResult
init|=
literal|1
decl_stmt|;
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|cBuffer
block|,
name|cSize
block|,
literal|0
block|}
decl_stmt|;
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|dstBuffer
block|,
name|dstBufferSize
block|,
literal|0
block|}
decl_stmt|;
for|for
control|(
name|totalGenSize
operator|=
literal|0
init|;
name|decompressionResult
condition|;
control|)
block|{
name|size_t
specifier|const
name|readCSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|dstBuffSize
init|=
name|MIN
argument_list|(
name|dstBufferSize
operator|-
name|totalGenSize
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
name|readCSrcSize
expr_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|dstBuffSize
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|6
argument_list|,
literal|"ZSTD_decompressStream input %u bytes (pos:%u/%u)\n"
argument_list|,
operator|(
name|U32
operator|)
name|readCSrcSize
argument_list|,
operator|(
name|U32
operator|)
name|inBuff
operator|.
name|pos
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|)
expr_stmt|;
name|decompressionResult
operator|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|ZSTD_isError
argument_list|(
name|decompressionResult
argument_list|)
argument_list|,
literal|"decompression error : %s"
argument_list|,
name|ZSTD_getErrorName
argument_list|(
name|decompressionResult
argument_list|)
argument_list|)
expr_stmt|;
name|DISPLAYLEVEL
argument_list|(
literal|6
argument_list|,
literal|"inBuff.pos = %u \n"
argument_list|,
operator|(
name|U32
operator|)
name|readCSrcSize
argument_list|)
expr_stmt|;
block|}
name|CHECK
argument_list|(
name|outBuff
operator|.
name|pos
operator|!=
name|totalTestSize
argument_list|,
literal|"decompressed data : wrong size (%u != %u)"
argument_list|,
operator|(
name|U32
operator|)
name|outBuff
operator|.
name|pos
argument_list|,
operator|(
name|U32
operator|)
name|totalTestSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|inBuff
operator|.
name|pos
operator|!=
name|cSize
argument_list|,
literal|"compressed data should be fully read (%u != %u)"
argument_list|,
operator|(
name|U32
operator|)
name|inBuff
operator|.
name|pos
argument_list|,
operator|(
name|U32
operator|)
name|cSize
argument_list|)
expr_stmt|;
block|{
name|U64
specifier|const
name|crcDest
init|=
name|XXH64
argument_list|(
name|dstBuffer
argument_list|,
name|totalTestSize
argument_list|,
literal|0
argument_list|)
decl_stmt|;
if|if
condition|(
name|crcDest
operator|!=
name|crcOrig
condition|)
name|findDiff
argument_list|(
name|copyBuffer
argument_list|,
name|dstBuffer
argument_list|,
name|totalTestSize
argument_list|)
expr_stmt|;
name|CHECK
argument_list|(
name|crcDest
operator|!=
name|crcOrig
argument_list|,
literal|"decompressed data corrupted"
argument_list|)
expr_stmt|;
block|}
block|}
comment|/*=====   noisy/erroneous src decompression test   =====*/
comment|/* add some noise */
block|{
name|U32
specifier|const
name|nbNoiseChunks
init|=
operator|(
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|&
literal|7
operator|)
operator|+
literal|2
decl_stmt|;
name|U32
name|nn
decl_stmt|;
for|for
control|(
name|nn
operator|=
literal|0
init|;
name|nn
operator|<
name|nbNoiseChunks
condition|;
name|nn
operator|++
control|)
block|{
name|size_t
specifier|const
name|randomNoiseSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|noiseSize
init|=
name|MIN
argument_list|(
operator|(
name|cSize
operator|/
literal|3
operator|)
argument_list|,
name|randomNoiseSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|noiseStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|srcBufferSize
operator|-
name|noiseSize
operator|)
decl_stmt|;
name|size_t
specifier|const
name|cStart
init|=
name|FUZ_rand
argument_list|(
operator|&
name|lseed
argument_list|)
operator|%
operator|(
name|cSize
operator|-
name|noiseSize
operator|)
decl_stmt|;
name|memcpy
argument_list|(
name|cBuffer
operator|+
name|cStart
argument_list|,
name|srcBuffer
operator|+
name|noiseStart
argument_list|,
name|noiseSize
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* try decompression on noisy data */
name|CHECK_Z
argument_list|(
name|ZSTD_initDStream
argument_list|(
name|zd_noise
argument_list|)
argument_list|)
expr_stmt|;
comment|/* note : no dictionary */
block|{
name|ZSTD_inBuffer
name|inBuff
init|=
block|{
name|cBuffer
block|,
name|cSize
block|,
literal|0
block|}
decl_stmt|;
name|ZSTD_outBuffer
name|outBuff
init|=
block|{
name|dstBuffer
block|,
name|dstBufferSize
block|,
literal|0
block|}
decl_stmt|;
while|while
condition|(
name|outBuff
operator|.
name|pos
operator|<
name|dstBufferSize
condition|)
block|{
name|size_t
specifier|const
name|randomCSrcSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|randomDstSize
init|=
name|FUZ_randomLength
argument_list|(
operator|&
name|lseed
argument_list|,
name|maxSampleLog
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedDstSize
init|=
name|MIN
argument_list|(
name|dstBufferSize
operator|-
name|outBuff
operator|.
name|pos
argument_list|,
name|randomDstSize
argument_list|)
decl_stmt|;
name|size_t
specifier|const
name|adjustedCSrcSize
init|=
name|MIN
argument_list|(
name|cSize
operator|-
name|inBuff
operator|.
name|pos
argument_list|,
name|randomCSrcSize
argument_list|)
decl_stmt|;
name|outBuff
operator|.
name|size
operator|=
name|outBuff
operator|.
name|pos
operator|+
name|adjustedDstSize
expr_stmt|;
name|inBuff
operator|.
name|size
operator|=
name|inBuff
operator|.
name|pos
operator|+
name|adjustedCSrcSize
expr_stmt|;
block|{
name|size_t
specifier|const
name|decompressError
init|=
name|ZSTD_decompressStream
argument_list|(
name|zd
argument_list|,
operator|&
name|outBuff
argument_list|,
operator|&
name|inBuff
argument_list|)
decl_stmt|;
if|if
condition|(
name|ZSTD_isError
argument_list|(
name|decompressError
argument_list|)
condition|)
break|break;
comment|/* error correctly detected */
comment|/* Good so far, but no more progress possible */
if|if
condition|(
name|outBuff
operator|.
name|pos
operator|<
name|outBuff
operator|.
name|size
operator|&&
name|inBuff
operator|.
name|pos
operator|==
name|cSize
condition|)
break|break;
block|}
block|}
block|}
block|}
name|DISPLAY
argument_list|(
literal|"\r%u fuzzer tests completed   \n"
argument_list|,
name|testNb
operator|-
literal|1
argument_list|)
expr_stmt|;
name|_cleanup
label|:
name|ZSTD_freeCCtx
argument_list|(
name|zc
argument_list|)
expr_stmt|;
name|ZSTD_freeDStream
argument_list|(
name|zd
argument_list|)
expr_stmt|;
name|ZSTD_freeDStream
argument_list|(
name|zd_noise
argument_list|)
expr_stmt|;
name|ZSTD_freeCCtxParams
argument_list|(
name|cctxParams
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|2
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|3
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cNoiseBuffer
index|[
literal|4
index|]
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|copyBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cBuffer
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|dstBuffer
argument_list|)
expr_stmt|;
return|return
name|result
return|;
name|_output_error
label|:
name|result
operator|=
literal|1
expr_stmt|;
goto|goto
name|_cleanup
goto|;
block|}
end_function

begin_comment
comment|/*-******************************************************* *  Command line *********************************************************/
end_comment

begin_function
name|int
name|FUZ_usage
parameter_list|(
specifier|const
name|char
modifier|*
name|programName
parameter_list|)
block|{
name|DISPLAY
argument_list|(
literal|"Usage :\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|"      %s [args]\n"
argument_list|,
name|programName
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|"Arguments :\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" -i#    : Nb of tests (default:%u) \n"
argument_list|,
name|nbTestsDefault
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" -s#    : Select seed (default:prompt user)\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" -t#    : Select starting test number (default:0)\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" -P#    : Select compressibility in %% (default:%i%%)\n"
argument_list|,
name|FUZ_COMPRESSIBILITY_DEFAULT
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" -v     : verbose\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" -p     : pause at the end\n"
argument_list|)
expr_stmt|;
name|DISPLAY
argument_list|(
literal|" -h     : display help and exit\n"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

begin_typedef
typedef|typedef
enum|enum
block|{
name|simple_api
block|,
name|mt_api
block|,
name|advanced_api
block|}
name|e_api
typedef|;
end_typedef

begin_function
name|int
name|main
parameter_list|(
name|int
name|argc
parameter_list|,
specifier|const
name|char
modifier|*
modifier|*
name|argv
parameter_list|)
block|{
name|U32
name|seed
init|=
literal|0
decl_stmt|;
name|int
name|seedset
init|=
literal|0
decl_stmt|;
name|int
name|argNb
decl_stmt|;
name|int
name|nbTests
init|=
name|nbTestsDefault
decl_stmt|;
name|int
name|testNb
init|=
literal|0
decl_stmt|;
name|int
name|proba
init|=
name|FUZ_COMPRESSIBILITY_DEFAULT
decl_stmt|;
name|int
name|result
init|=
literal|0
decl_stmt|;
name|int
name|mainPause
init|=
literal|0
decl_stmt|;
name|int
name|bigTests
init|=
operator|(
sizeof|sizeof
argument_list|(
name|size_t
argument_list|)
operator|==
literal|8
operator|)
decl_stmt|;
name|e_api
name|selected_api
init|=
name|simple_api
decl_stmt|;
specifier|const
name|char
modifier|*
specifier|const
name|programName
init|=
name|argv
index|[
literal|0
index|]
decl_stmt|;
name|ZSTD_customMem
specifier|const
name|customNULL
init|=
name|ZSTD_defaultCMem
decl_stmt|;
name|U32
name|useOpaqueAPI
init|=
literal|0
decl_stmt|;
comment|/* Check command line */
for|for
control|(
name|argNb
operator|=
literal|1
init|;
name|argNb
operator|<
name|argc
condition|;
name|argNb
operator|++
control|)
block|{
specifier|const
name|char
modifier|*
name|argument
init|=
name|argv
index|[
name|argNb
index|]
decl_stmt|;
if|if
condition|(
operator|!
name|argument
condition|)
continue|continue;
comment|/* Protection if argument empty */
comment|/* Parsing commands. Aggregated commands are allowed */
if|if
condition|(
name|argument
index|[
literal|0
index|]
operator|==
literal|'-'
condition|)
block|{
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|argument
argument_list|,
literal|"--mt"
argument_list|)
condition|)
block|{
name|selected_api
operator|=
name|mt_api
expr_stmt|;
name|testNb
operator|+=
operator|!
name|testNb
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|argument
argument_list|,
literal|"--newapi"
argument_list|)
condition|)
block|{
name|selected_api
operator|=
name|advanced_api
expr_stmt|;
name|testNb
operator|+=
operator|!
name|testNb
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|argument
argument_list|,
literal|"--opaqueapi"
argument_list|)
condition|)
block|{
name|selected_api
operator|=
name|advanced_api
expr_stmt|;
name|testNb
operator|+=
operator|!
name|testNb
expr_stmt|;
name|useOpaqueAPI
operator|=
literal|1
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|argument
argument_list|,
literal|"--no-big-tests"
argument_list|)
condition|)
block|{
name|bigTests
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
name|argument
operator|++
expr_stmt|;
while|while
condition|(
operator|*
name|argument
operator|!=
literal|0
condition|)
block|{
switch|switch
condition|(
operator|*
name|argument
condition|)
block|{
case|case
literal|'h'
case|:
return|return
name|FUZ_usage
argument_list|(
name|programName
argument_list|)
return|;
case|case
literal|'v'
case|:
name|argument
operator|++
expr_stmt|;
name|g_displayLevel
operator|++
expr_stmt|;
break|break;
case|case
literal|'q'
case|:
name|argument
operator|++
expr_stmt|;
name|g_displayLevel
operator|--
expr_stmt|;
break|break;
case|case
literal|'p'
case|:
comment|/* pause at the end */
name|argument
operator|++
expr_stmt|;
name|mainPause
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'i'
case|:
comment|/* limit tests by nb of iterations (default) */
name|argument
operator|++
expr_stmt|;
name|nbTests
operator|=
literal|0
expr_stmt|;
name|g_clockTime
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
operator|*
name|argument
operator|>=
literal|'0'
operator|)
operator|&&
operator|(
operator|*
name|argument
operator|<=
literal|'9'
operator|)
condition|)
block|{
name|nbTests
operator|*=
literal|10
expr_stmt|;
name|nbTests
operator|+=
operator|*
name|argument
operator|-
literal|'0'
expr_stmt|;
name|argument
operator|++
expr_stmt|;
block|}
break|break;
case|case
literal|'T'
case|:
comment|/* limit tests by time */
name|argument
operator|++
expr_stmt|;
name|nbTests
operator|=
literal|0
expr_stmt|;
name|g_clockTime
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
operator|*
name|argument
operator|>=
literal|'0'
operator|)
operator|&&
operator|(
operator|*
name|argument
operator|<=
literal|'9'
operator|)
condition|)
block|{
name|g_clockTime
operator|*=
literal|10
expr_stmt|;
name|g_clockTime
operator|+=
operator|*
name|argument
operator|-
literal|'0'
expr_stmt|;
name|argument
operator|++
expr_stmt|;
block|}
if|if
condition|(
operator|*
name|argument
operator|==
literal|'m'
condition|)
name|g_clockTime
operator|*=
literal|60
operator|,
name|argument
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|argument
operator|==
literal|'n'
condition|)
name|argument
operator|++
expr_stmt|;
name|g_clockTime
operator|*=
name|CLOCKS_PER_SEC
expr_stmt|;
break|break;
case|case
literal|'s'
case|:
comment|/* manually select seed */
name|argument
operator|++
expr_stmt|;
name|seed
operator|=
literal|0
expr_stmt|;
name|seedset
operator|=
literal|1
expr_stmt|;
while|while
condition|(
operator|(
operator|*
name|argument
operator|>=
literal|'0'
operator|)
operator|&&
operator|(
operator|*
name|argument
operator|<=
literal|'9'
operator|)
condition|)
block|{
name|seed
operator|*=
literal|10
expr_stmt|;
name|seed
operator|+=
operator|*
name|argument
operator|-
literal|'0'
expr_stmt|;
name|argument
operator|++
expr_stmt|;
block|}
break|break;
case|case
literal|'t'
case|:
comment|/* select starting test number */
name|argument
operator|++
expr_stmt|;
name|testNb
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
operator|*
name|argument
operator|>=
literal|'0'
operator|)
operator|&&
operator|(
operator|*
name|argument
operator|<=
literal|'9'
operator|)
condition|)
block|{
name|testNb
operator|*=
literal|10
expr_stmt|;
name|testNb
operator|+=
operator|*
name|argument
operator|-
literal|'0'
expr_stmt|;
name|argument
operator|++
expr_stmt|;
block|}
break|break;
case|case
literal|'P'
case|:
comment|/* compressibility % */
name|argument
operator|++
expr_stmt|;
name|proba
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
operator|*
name|argument
operator|>=
literal|'0'
operator|)
operator|&&
operator|(
operator|*
name|argument
operator|<=
literal|'9'
operator|)
condition|)
block|{
name|proba
operator|*=
literal|10
expr_stmt|;
name|proba
operator|+=
operator|*
name|argument
operator|-
literal|'0'
expr_stmt|;
name|argument
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|proba
operator|<
literal|0
condition|)
name|proba
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|proba
operator|>
literal|100
condition|)
name|proba
operator|=
literal|100
expr_stmt|;
break|break;
default|default:
return|return
name|FUZ_usage
argument_list|(
name|programName
argument_list|)
return|;
block|}
block|}
block|}
block|}
comment|/* for(argNb=1; argNb<argc; argNb++) */
comment|/* Get Seed */
name|DISPLAY
argument_list|(
literal|"Starting zstream tester (%i-bits, %s)\n"
argument_list|,
call|(
name|int
call|)
argument_list|(
sizeof|sizeof
argument_list|(
name|size_t
argument_list|)
operator|*
literal|8
argument_list|)
argument_list|,
name|ZSTD_VERSION_STRING
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|seedset
condition|)
block|{
name|time_t
specifier|const
name|t
init|=
name|time
argument_list|(
name|NULL
argument_list|)
decl_stmt|;
name|U32
specifier|const
name|h
init|=
name|XXH32
argument_list|(
operator|&
name|t
argument_list|,
sizeof|sizeof
argument_list|(
name|t
argument_list|)
argument_list|,
literal|1
argument_list|)
decl_stmt|;
name|seed
operator|=
name|h
operator|%
literal|10000
expr_stmt|;
block|}
name|DISPLAY
argument_list|(
literal|"Seed = %u\n"
argument_list|,
name|seed
argument_list|)
expr_stmt|;
if|if
condition|(
name|proba
operator|!=
name|FUZ_COMPRESSIBILITY_DEFAULT
condition|)
name|DISPLAY
argument_list|(
literal|"Compressibility : %i%%\n"
argument_list|,
name|proba
argument_list|)
expr_stmt|;
if|if
condition|(
name|nbTests
operator|<=
literal|0
condition|)
name|nbTests
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|testNb
operator|==
literal|0
condition|)
block|{
name|result
operator|=
name|basicUnitTests
argument_list|(
literal|0
argument_list|,
operator|(
operator|(
name|double
operator|)
name|proba
operator|)
operator|/
literal|100
argument_list|,
name|customNULL
argument_list|)
expr_stmt|;
comment|/* constant seed for predictability */
block|}
if|if
condition|(
operator|!
name|result
condition|)
block|{
switch|switch
condition|(
name|selected_api
condition|)
block|{
case|case
name|simple_api
case|:
name|result
operator|=
name|fuzzerTests
argument_list|(
name|seed
argument_list|,
name|nbTests
argument_list|,
name|testNb
argument_list|,
operator|(
operator|(
name|double
operator|)
name|proba
operator|)
operator|/
literal|100
argument_list|,
name|bigTests
argument_list|)
expr_stmt|;
break|break;
case|case
name|mt_api
case|:
name|result
operator|=
name|fuzzerTests_MT
argument_list|(
name|seed
argument_list|,
name|nbTests
argument_list|,
name|testNb
argument_list|,
operator|(
operator|(
name|double
operator|)
name|proba
operator|)
operator|/
literal|100
argument_list|,
name|bigTests
argument_list|)
expr_stmt|;
break|break;
case|case
name|advanced_api
case|:
name|result
operator|=
name|fuzzerTests_newAPI
argument_list|(
name|seed
argument_list|,
name|nbTests
argument_list|,
name|testNb
argument_list|,
operator|(
operator|(
name|double
operator|)
name|proba
operator|)
operator|/
literal|100
argument_list|,
name|bigTests
argument_list|,
name|useOpaqueAPI
argument_list|)
expr_stmt|;
break|break;
default|default :
name|assert
argument_list|(
literal|0
argument_list|)
expr_stmt|;
comment|/* impossible */
block|}
block|}
if|if
condition|(
name|mainPause
condition|)
block|{
name|int
name|unused
decl_stmt|;
name|DISPLAY
argument_list|(
literal|"Press Enter \n"
argument_list|)
expr_stmt|;
name|unused
operator|=
name|getchar
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|unused
expr_stmt|;
block|}
return|return
name|result
return|;
block|}
end_function

end_unit

