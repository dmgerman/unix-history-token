...
name|name
parameter_list|)
value|static regex_t *name = NULL
end_define
...
name|name
parameter_list|)
value|static struct regexp *name = NULL;
end_define
...
begin_comment
comment|/*  * Convert text.  Perform the transformations specified by ops.  */
end_comment
...
begin_comment
comment|/*  * Determine which conversions to perform.  */
end_comment
...
directive|if
literal|0
comment|/* 		 * If any character in the line is highlighted,  		 * repaint the line. 		 * 		 * {{ This doesn't work -- if line is drawn with highlights 		 * which should be erased (e.g. toggle -i with status column), 		 * we must redraw the line even if it has no highlights. 		 * For now, just repaint every line. }} 		 */
block|if (is_hilited(pos, epos, 1, NULL))
endif|#
directive|endif
block|{
operator|(
...
operator|)
return|;
comment|/* 	 * Look at each filter and see if the start position 	 * equals the start position of the line. 	 */
for|for
control|(
name|hl
operator|=
...
begin_comment
comment|/*  * Adjust hl_startpos& hl_endpos to account for processing by cvt_text.  */
end_comment
...
name|ncwidth
decl_stmt|;
comment|/* 	 * The line was already scanned and hilites were added (in hilite_line). 	 * But it was assumed that each char position in the line  	 * correponds to one char position in the file. 	 * This may not be true if cvt_text modified the line. 	 * Get the raw line again.  Look at each character. 	 */
operator|(
name|void
operator|)
name|forw_raw_line
...
name|ncwidth
expr_stmt|;
comment|/* Figure out how this char was processed by cvt_text. */
if|if
condition|(
operator|(
name|cvt_ops
...
condition|)
block|{
comment|/* Skip the backspace and the following char. */
name|oline
operator|=
name|line
expr_stmt|;
...
condition|)
block|{
comment|/* Converted uppercase to lower. 			 * Note that this may have changed the number of bytes  			 * that the character occupies. */
name|char
name|dbuf
index|[
literal|6
...
block|{
comment|/* Ordinary unprocessed character. */
name|opos
operator|+=
name|ncwidth
expr_stmt|;
...
condition|)
block|{
comment|/* Adjust highlight position. */
name|hl_npos
operator|=
name|npos
expr_stmt|;
...
condition|)
block|{
comment|/*              * We've moved past the highlight position; store the              * adjusted highlight position and move to the next highlight.              */
if|if
condition|(
name|checkstart
condition|)
