...
begin_comment
comment|/*  * Set a character in the line buffer.  */
end_comment
...
begin_comment
comment|/*  * Append a character to the line buffer.  */
end_comment
...
begin_comment
comment|/*  * Can a char appear in an ANSI escape sequence, before the end char?  */
end_comment
...
begin_comment
comment|/*  * Skip past an ANSI escape sequence.  * pp is initially positioned just after the CSI_START char.  */
end_comment
...
condition|)
do|;
comment|/* Note that we discard final char, for which is_ansi_middle is false. */
block|}
end_function
...
condition|)
block|{
comment|/* 		 * Display the right scrolling char. 		 * If we've already filled the rightmost screen char  		 * (in the buffer), overwrite it. 		 */
if|if
condition|(
name|column
operator|>=
...
condition|)
block|{
comment|/* We've already written in the rightmost char. */
name|column
operator|=
name|right_column
expr_stmt|;
...
condition|)
block|{
comment|/* 			 * Space to last (rightmost) char on screen. 			 * This may be necessary if the char we overwrote 			 * was double-width. 			 */
name|add_linebuf
argument_list|(
literal|' '
argument_list|,
...
expr_stmt|;
block|}
comment|/* Print rscroll char. It must be single-width. */
name|add_linebuf
argument_list|(
name|rscroll_char
argument_list|,
