...
name|data
expr_stmt|;
comment|/* skip over names that have no NSEC3 records (whether it 		 * actually should or should not should have been checked 		 * already */
if|if
condition|(
operator|!
name|next_name
...
expr_stmt|;
block|}
comment|/* we keep track of what 'so far' is the next hashed name; 		 * it must of course be 'larger' than the current name 		 * if we find one that is larger, but smaller than what we 		 * previously thought was the next one, that one is the next 		 */
name|cmp
operator|=
name|ldns_dname_compare
argument_list|(
...
block|}
block|}
comment|/* in case the hashed name of the nsec we are checking is the 		 * last one, we need the first hashed name of the zone */
if|if
condition|(
operator|!
name|cur_first_name
