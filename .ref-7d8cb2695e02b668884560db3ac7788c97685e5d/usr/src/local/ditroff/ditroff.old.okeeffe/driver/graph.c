...
begin_comment
comment|/* graph.c	1.12	84/03/27  *  *	This file contains the functions for producing the graphics  *   images in the varian/versatec drivers for ditroff.  */
end_comment
...
begin_comment
comment|/*----------------------------------------------------------------------------  * Routine:	drawellip (horizontal_diameter, vertical_diameter)  *  * Results:	Draws regular ellipses given the major "diameters."  It does  *		so using a modified circle algorithm (see RoundEnd) that  *		increments x and y proportionally to their axes.  *  * Side Efct:	Resulting position is at (hpos + hd, vpos).  *----------------------------------------------------------------------------*/
end_comment
...
name|vd
expr_stmt|;
comment|/* start at top of the ellipse */
name|xs
operator|=
literal|0.0
expr_stmt|;
comment|/*   (y = 1/2 diameter, x = 0) */
if|if
condition|(
operator|(
name|thick
...
block|{
comment|/* do the perimeter only (no fill) */
name|point
argument_list|(
name|basex
argument_list|,
...
name|ys
decl_stmt|;
comment|/* floating point distance form center of circle */
name|double
name|epsilon
decl_stmt|;
...
name|nx
decl_stmt|;
comment|/* integer distance from center */
specifier|register
name|int
name|ny
decl_stmt|;
