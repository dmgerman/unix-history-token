begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1989, 1993  *	The Regents of the University of California.  All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Eamonn McManus of Trinity College Dublin.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1989, 1993\n\ 	The Regents of the University of California.  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)arithmetic.c	8.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_comment
comment|/*  * By Eamonn McManus, Trinity College Dublin<emcmanus@cs.tcd.ie>.  *  * The operation of this program mimics that of the standard Unix game  * `arithmetic'.  I've made it as close as I could manage without examining  * the source code.  The principal differences are:  *  * The method of biasing towards numbers that had wrong answers in the past  * is different; original `arithmetic' seems to retain the bias forever,  * whereas this program lets the bias gradually decay as it is used.  *  * Original `arithmetic' delays for some period (3 seconds?) after printing  * the score.  I saw no reason for this delay, so I scrapped it.  *  * There is no longer a limitation on the maximum range that can be supplied  * to the program.  The original program required it to be less than 100.  * Anomalous results may occur with this program if ranges big enough to  * allow overflow are given.  *  * I have obviously not attempted to duplicate bugs in the original.  It  * would go into an infinite loop if invoked as `arithmetic / 0'.  It also  * did not recognise an EOF in its input, and would continue trying to read  * after it.  It did not check that the input was a valid number, treating any  * garbage as 0.  Finally, it did not flush stdout after printing its prompt,  * so in the unlikely event that stdout was not a terminal, it would not work  * properly.  */
end_comment

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/signal.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_decl_stmt
name|char
name|keylist
index|[]
init|=
literal|"+-x/"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|defaultkeys
index|[]
init|=
literal|"+-"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|keys
init|=
name|defaultkeys
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nkeys
init|=
sizeof|sizeof
argument_list|(
name|defaultkeys
argument_list|)
operator|-
literal|1
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|rangemax
init|=
literal|10
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|nright
decl_stmt|,
name|nwrong
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|time_t
name|qtime
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|NQUESTS
value|20
end_define

begin_comment
comment|/*  * Select keys from +-x/ to be asked addition, subtraction, multiplication,  * and division problems.  More than one key may be given.  The default is  * +-.  Specify a range to confine the operands to 0 - range.  Default upper  * bound is 10.  After every NQUESTS questions, statistics on the performance  * so far are printed.  */
end_comment

begin_function
name|void
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
block|{
specifier|extern
name|char
modifier|*
name|optarg
decl_stmt|;
specifier|extern
name|int
name|optind
decl_stmt|;
name|int
name|ch
decl_stmt|,
name|cnt
decl_stmt|;
name|void
name|intr
parameter_list|()
function_decl|;
while|while
condition|(
operator|(
name|ch
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"r:o:"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|ch
condition|)
block|{
case|case
literal|'o'
case|:
block|{
specifier|register
name|char
modifier|*
name|p
decl_stmt|;
for|for
control|(
name|p
operator|=
name|keys
operator|=
name|optarg
init|;
operator|*
name|p
condition|;
operator|++
name|p
control|)
if|if
condition|(
operator|!
name|index
argument_list|(
name|keylist
argument_list|,
operator|*
name|p
argument_list|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"arithmetic: unknown key.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|nkeys
operator|=
name|p
operator|-
name|optarg
expr_stmt|;
break|break;
block|}
case|case
literal|'r'
case|:
if|if
condition|(
operator|(
name|rangemax
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|<=
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"arithmetic: invalid range.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
literal|'?'
case|:
default|default:
name|usage
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
name|argc
operator|-=
name|optind
condition|)
name|usage
argument_list|()
expr_stmt|;
comment|/* Seed the random-number generator. */
name|srandom
argument_list|(
operator|(
name|int
operator|)
name|time
argument_list|(
operator|(
name|time_t
operator|*
operator|)
name|NULL
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|intr
argument_list|)
expr_stmt|;
comment|/* Now ask the questions. */
for|for
control|(
init|;
condition|;
control|)
block|{
for|for
control|(
name|cnt
operator|=
name|NQUESTS
init|;
name|cnt
operator|--
condition|;
control|)
if|if
condition|(
name|problem
argument_list|()
operator|==
name|EOF
condition|)
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|showstats
argument_list|()
expr_stmt|;
block|}
comment|/* NOTREACHED */
block|}
end_function

begin_comment
comment|/* Handle interrupt character.  Print score and exit. */
end_comment

begin_function
name|void
name|intr
parameter_list|()
block|{
name|showstats
argument_list|()
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Print score.  Original `arithmetic' had a delay after printing it. */
end_comment

begin_macro
name|showstats
argument_list|()
end_macro

begin_block
block|{
if|if
condition|(
name|nright
operator|+
name|nwrong
operator|>
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n\nRights %d; Wrongs %d; Score %d%%"
argument_list|,
name|nright
argument_list|,
name|nwrong
argument_list|,
call|(
name|int
call|)
argument_list|(
literal|100L
operator|*
name|nright
operator|/
operator|(
name|nright
operator|+
name|nwrong
operator|)
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|nright
operator|>
literal|0
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\nTotal time %ld seconds; %.1f seconds per problem\n\n"
argument_list|,
operator|(
name|long
operator|)
name|qtime
argument_list|,
operator|(
name|float
operator|)
name|qtime
operator|/
name|nright
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Pick a problem and ask it.  Keeps asking the same problem until supplied  * with the correct answer, or until EOF or interrupt is typed.  Problems are  * selected such that the right operand and either the left operand (for +, x)  * or the correct result (for -, /) are in the range 0 to rangemax.  Each wrong  * answer causes the numbers in the problem to be penalised, so that they are  * more likely to appear in subsequent problems.  */
end_comment

begin_macro
name|problem
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|char
modifier|*
name|p
decl_stmt|;
name|time_t
name|start
decl_stmt|,
name|finish
decl_stmt|;
name|int
name|left
decl_stmt|,
name|op
decl_stmt|,
name|right
decl_stmt|,
name|result
decl_stmt|;
name|char
name|line
index|[
literal|80
index|]
decl_stmt|;
name|op
operator|=
name|keys
index|[
name|random
argument_list|()
operator|%
name|nkeys
index|]
expr_stmt|;
if|if
condition|(
name|op
operator|!=
literal|'/'
condition|)
name|right
operator|=
name|getrandom
argument_list|(
name|rangemax
operator|+
literal|1
argument_list|,
name|op
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|retry
label|:
comment|/* Get the operands. */
switch|switch
condition|(
name|op
condition|)
block|{
case|case
literal|'+'
case|:
name|left
operator|=
name|getrandom
argument_list|(
name|rangemax
operator|+
literal|1
argument_list|,
name|op
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|result
operator|=
name|left
operator|+
name|right
expr_stmt|;
break|break;
case|case
literal|'-'
case|:
name|result
operator|=
name|getrandom
argument_list|(
name|rangemax
operator|+
literal|1
argument_list|,
name|op
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|left
operator|=
name|right
operator|+
name|result
expr_stmt|;
break|break;
case|case
literal|'x'
case|:
name|left
operator|=
name|getrandom
argument_list|(
name|rangemax
operator|+
literal|1
argument_list|,
name|op
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|result
operator|=
name|left
operator|*
name|right
expr_stmt|;
break|break;
case|case
literal|'/'
case|:
name|right
operator|=
name|getrandom
argument_list|(
name|rangemax
argument_list|,
name|op
argument_list|,
literal|1
argument_list|)
operator|+
literal|1
expr_stmt|;
name|result
operator|=
name|getrandom
argument_list|(
name|rangemax
operator|+
literal|1
argument_list|,
name|op
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|left
operator|=
name|right
operator|*
name|result
operator|+
name|random
argument_list|()
operator|%
name|right
expr_stmt|;
break|break;
block|}
comment|/* 	 * A very big maxrange could cause negative values to pop 	 * up, owing to overflow. 	 */
if|if
condition|(
name|result
operator|<
literal|0
operator|||
name|left
operator|<
literal|0
condition|)
goto|goto
name|retry
goto|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%d %c %d =   "
argument_list|,
name|left
argument_list|,
name|op
argument_list|,
name|right
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|time
argument_list|(
operator|&
name|start
argument_list|)
expr_stmt|;
comment|/* 	 * Keep looping until the correct answer is given, or until EOF or 	 * interrupt is typed. 	 */
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|!
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|stdin
argument_list|)
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
return|return
operator|(
name|EOF
operator|)
return|;
block|}
for|for
control|(
name|p
operator|=
name|line
init|;
operator|*
name|p
operator|&&
name|isspace
argument_list|(
operator|*
name|p
argument_list|)
condition|;
operator|++
name|p
control|)
empty_stmt|;
if|if
condition|(
operator|!
name|isdigit
argument_list|(
operator|*
name|p
argument_list|)
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Please type a number.\n"
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|atoi
argument_list|(
name|p
argument_list|)
operator|==
name|result
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Right!\n"
argument_list|)
expr_stmt|;
operator|++
name|nright
expr_stmt|;
break|break;
block|}
comment|/* Wrong answer; penalise and ask again. */
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"What?\n"
argument_list|)
expr_stmt|;
operator|++
name|nwrong
expr_stmt|;
name|penalise
argument_list|(
name|right
argument_list|,
name|op
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|op
operator|==
literal|'x'
operator|||
name|op
operator|==
literal|'+'
condition|)
name|penalise
argument_list|(
name|left
argument_list|,
name|op
argument_list|,
literal|0
argument_list|)
expr_stmt|;
else|else
name|penalise
argument_list|(
name|result
argument_list|,
name|op
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Accumulate the time taken.  Obviously rounding errors happen here; 	 * however they should cancel out, because some of the time you are 	 * charged for a partially elapsed second at the start, and some of 	 * the time you are not charged for a partially elapsed second at the 	 * end. 	 */
operator|(
name|void
operator|)
name|time
argument_list|(
operator|&
name|finish
argument_list|)
expr_stmt|;
name|qtime
operator|+=
name|finish
operator|-
name|start
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Here is the code for accumulating penalties against the numbers for which  * a wrong answer was given.  The right operand and either the left operand  * (for +, x) or the result (for -, /) are stored in a list for the particular  * operation, and each becomes more likely to appear again in that operation.  * Initially, each number is charged a penalty of WRONGPENALTY, giving it that  * many extra chances of appearing.  Each time it is selected because of this,  * its penalty is decreased by one; it is removed when it reaches 0.  *  * The penalty[] array gives the sum of all penalties in the list for  * each operation and each operand.  The penlist[] array has the lists of  * penalties themselves.  */
end_comment

begin_decl_stmt
name|int
name|penalty
index|[
sizeof|sizeof
argument_list|(
name|keylist
argument_list|)
operator|-
literal|1
index|]
index|[
literal|2
index|]
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|penalty
block|{
name|int
name|value
decl_stmt|,
name|penalty
decl_stmt|;
comment|/* Penalised value and its penalty. */
name|struct
name|penalty
modifier|*
name|next
decl_stmt|;
block|}
modifier|*
name|penlist
index|[
sizeof|sizeof
argument_list|(
name|keylist
argument_list|)
operator|-
literal|1
index|]
index|[
literal|2
index|]
struct|;
end_struct

begin_define
define|#
directive|define
name|WRONGPENALTY
value|5
end_define

begin_comment
comment|/* Perhaps this should depend on maxrange. */
end_comment

begin_comment
comment|/*  * Add a penalty for the number `value' to the list for operation `op',  * operand number `operand' (0 or 1).  If we run out of memory, we just  * forget about the penalty (how likely is this, anyway?).  */
end_comment

begin_macro
name|penalise
argument_list|(
argument|value
argument_list|,
argument|op
argument_list|,
argument|operand
argument_list|)
end_macro

begin_decl_stmt
name|int
name|value
decl_stmt|,
name|op
decl_stmt|,
name|operand
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|penalty
modifier|*
name|p
decl_stmt|;
name|char
modifier|*
name|malloc
parameter_list|()
function_decl|;
name|op
operator|=
name|opnum
argument_list|(
name|op
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|p
operator|=
operator|(
expr|struct
name|penalty
operator|*
operator|)
name|malloc
argument_list|(
operator|(
name|u_int
operator|)
sizeof|sizeof
argument_list|(
operator|*
name|p
argument_list|)
argument_list|)
operator|)
operator|==
name|NULL
condition|)
return|return;
name|p
operator|->
name|next
operator|=
name|penlist
index|[
name|op
index|]
index|[
name|operand
index|]
expr_stmt|;
name|penlist
index|[
name|op
index|]
index|[
name|operand
index|]
operator|=
name|p
expr_stmt|;
name|penalty
index|[
name|op
index|]
index|[
name|operand
index|]
operator|+=
name|p
operator|->
name|penalty
operator|=
name|WRONGPENALTY
expr_stmt|;
name|p
operator|->
name|value
operator|=
name|value
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * Select a random value from 0 to maxval - 1 for operand `operand' (0 or 1)  * of operation `op'.  The random number we generate is either used directly  * as a value, or represents a position in the penalty list.  If the latter,  * we find the corresponding value and return that, decreasing its penalty.  */
end_comment

begin_macro
name|getrandom
argument_list|(
argument|maxval
argument_list|,
argument|op
argument_list|,
argument|operand
argument_list|)
end_macro

begin_decl_stmt
name|int
name|maxval
decl_stmt|,
name|op
decl_stmt|,
name|operand
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|int
name|value
decl_stmt|;
specifier|register
name|struct
name|penalty
modifier|*
modifier|*
name|pp
decl_stmt|,
modifier|*
name|p
decl_stmt|;
name|op
operator|=
name|opnum
argument_list|(
name|op
argument_list|)
expr_stmt|;
name|value
operator|=
name|random
argument_list|()
operator|%
operator|(
name|maxval
operator|+
name|penalty
index|[
name|op
index|]
index|[
name|operand
index|]
operator|)
expr_stmt|;
comment|/* 	 * 0 to maxval - 1 is a number to be used directly; bigger values 	 * are positions to be located in the penalty list. 	 */
if|if
condition|(
name|value
operator|<
name|maxval
condition|)
return|return
operator|(
name|value
operator|)
return|;
name|value
operator|-=
name|maxval
expr_stmt|;
comment|/* 	 * Find the penalty at position `value'; decrement its penalty and 	 * delete it if it reaches 0; return the corresponding value. 	 */
for|for
control|(
name|pp
operator|=
operator|&
name|penlist
index|[
name|op
index|]
index|[
name|operand
index|]
init|;
operator|(
name|p
operator|=
operator|*
name|pp
operator|)
operator|!=
name|NULL
condition|;
name|pp
operator|=
operator|&
name|p
operator|->
name|next
control|)
block|{
if|if
condition|(
name|p
operator|->
name|penalty
operator|>
name|value
condition|)
block|{
name|value
operator|=
name|p
operator|->
name|value
expr_stmt|;
name|penalty
index|[
name|op
index|]
index|[
name|operand
index|]
operator|--
expr_stmt|;
if|if
condition|(
operator|--
operator|(
name|p
operator|->
name|penalty
operator|)
operator|<=
literal|0
condition|)
block|{
name|p
operator|=
name|p
operator|->
name|next
expr_stmt|;
operator|(
name|void
operator|)
name|free
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|*
name|pp
argument_list|)
expr_stmt|;
operator|*
name|pp
operator|=
name|p
expr_stmt|;
block|}
return|return
operator|(
name|value
operator|)
return|;
block|}
name|value
operator|-=
name|p
operator|->
name|penalty
expr_stmt|;
block|}
comment|/* 	 * We can only get here if the value from the penalty[] array doesn't 	 * correspond to the actual sum of penalties in the list.  Provide an 	 * obscure message. 	 */
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"arithmetic: bug: inconsistent penalties\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
comment|/* NOTREACHED */
block|}
end_block

begin_comment
comment|/* Return an index for the character op, which is one of [+-x/]. */
end_comment

begin_macro
name|opnum
argument_list|(
argument|op
argument_list|)
end_macro

begin_decl_stmt
name|int
name|op
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|char
modifier|*
name|p
decl_stmt|;
if|if
condition|(
name|op
operator|==
literal|0
operator|||
operator|(
name|p
operator|=
name|index
argument_list|(
name|keylist
argument_list|,
name|op
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"arithmetic: bug: op %c not in keylist %s\n"
argument_list|,
name|op
argument_list|,
name|keylist
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|p
operator|-
name|keylist
operator|)
return|;
block|}
end_block

begin_comment
comment|/* Print usage message and quit. */
end_comment

begin_macro
name|usage
argument_list|()
end_macro

begin_block
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: arithmetic [-o +-x/] [-r range]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

end_unit

