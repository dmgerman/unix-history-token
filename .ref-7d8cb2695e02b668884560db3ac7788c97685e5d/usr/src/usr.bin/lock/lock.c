begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1980, 1987 Regents of the University of California.  * All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Bob Toxen.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1980, 1987 Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)lock.c	5.14 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_comment
comment|/*  * Lock a terminal up until the given key is entered, until the root  * password is entered, or the given interval times out.  *  * Timeout interval is by default TIMEOUT, it can be changed with  * an argument of the form -time where time is in minutes  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/signal.h>
end_include

begin_include
include|#
directive|include
file|<sgtty.h>
end_include

begin_include
include|#
directive|include
file|<pwd.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_define
define|#
directive|define
name|TIMEOUT
value|15
end_define

begin_decl_stmt
name|void
name|quit
argument_list|()
decl_stmt|,
name|bye
argument_list|()
decl_stmt|,
name|hi
argument_list|()
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|timeval
name|timeout
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|timeval
name|zerotime
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|sgttyb
name|tty
decl_stmt|,
name|ntty
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
name|nexttime
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* keep the timeout time */
end_comment

begin_comment
comment|/*ARGSUSED*/
end_comment

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
modifier|*
name|argv
decl_stmt|;
block|{
specifier|extern
name|char
modifier|*
name|optarg
decl_stmt|;
specifier|extern
name|int
name|errno
decl_stmt|,
name|optind
decl_stmt|;
name|struct
name|passwd
modifier|*
name|pw
decl_stmt|;
name|struct
name|timeval
name|timval
decl_stmt|;
name|struct
name|itimerval
name|ntimer
decl_stmt|,
name|otimer
decl_stmt|;
name|struct
name|tm
modifier|*
name|timp
decl_stmt|;
name|int
name|ch
decl_stmt|,
name|sectimeout
decl_stmt|,
name|usemine
decl_stmt|;
name|char
modifier|*
name|ap
decl_stmt|,
modifier|*
name|mypw
decl_stmt|,
modifier|*
name|ttynam
decl_stmt|,
modifier|*
name|tzn
decl_stmt|;
name|char
name|hostname
index|[
name|MAXHOSTNAMELEN
index|]
decl_stmt|,
name|s
index|[
name|BUFSIZ
index|]
decl_stmt|,
name|s1
index|[
name|BUFSIZ
index|]
decl_stmt|;
name|char
modifier|*
name|crypt
argument_list|()
decl_stmt|,
modifier|*
name|ttyname
argument_list|()
decl_stmt|;
name|sectimeout
operator|=
name|TIMEOUT
expr_stmt|;
name|mypw
operator|=
name|NULL
expr_stmt|;
name|usemine
operator|=
literal|0
expr_stmt|;
while|while
condition|(
operator|(
name|ch
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"pt:"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
operator|(
name|char
operator|)
name|ch
condition|)
block|{
case|case
literal|'t'
case|:
if|if
condition|(
operator|(
name|sectimeout
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|<=
literal|0
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"lock: illegal timeout value.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
literal|'p'
case|:
name|usemine
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|pw
operator|=
name|getpwuid
argument_list|(
name|getuid
argument_list|()
argument_list|)
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"lock: unknown uid %d.\n"
argument_list|,
name|getuid
argument_list|()
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|mypw
operator|=
name|strdup
argument_list|(
name|pw
operator|->
name|pw_passwd
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'?'
case|:
default|default:
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: lock [-p] [-t timeout]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|timeout
operator|.
name|tv_sec
operator|=
name|sectimeout
operator|*
literal|60
expr_stmt|;
name|setuid
argument_list|(
name|getuid
argument_list|()
argument_list|)
expr_stmt|;
comment|/* discard privs */
if|if
condition|(
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCGETP
argument_list|,
operator|&
name|tty
argument_list|)
condition|)
comment|/* get information for header */
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|gethostname
argument_list|(
name|hostname
argument_list|,
sizeof|sizeof
argument_list|(
name|hostname
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|ttynam
operator|=
name|ttyname
argument_list|(
literal|0
argument_list|)
operator|)
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"lock: not a terminal?\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|gettimeofday
argument_list|(
operator|&
name|timval
argument_list|,
operator|(
expr|struct
name|timezone
operator|*
operator|)
name|NULL
argument_list|)
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"lock: gettimeofday: %s\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|nexttime
operator|=
name|timval
operator|.
name|tv_sec
operator|+
operator|(
name|sectimeout
operator|*
literal|60
operator|)
expr_stmt|;
name|timp
operator|=
name|localtime
argument_list|(
operator|&
name|timval
operator|.
name|tv_sec
argument_list|)
expr_stmt|;
name|ap
operator|=
name|asctime
argument_list|(
name|timp
argument_list|)
expr_stmt|;
name|tzn
operator|=
name|timp
operator|->
name|tm_zone
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|quit
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGQUIT
argument_list|,
name|quit
argument_list|)
expr_stmt|;
name|ntty
operator|=
name|tty
expr_stmt|;
name|ntty
operator|.
name|sg_flags
operator|&=
operator|~
name|ECHO
expr_stmt|;
operator|(
name|void
operator|)
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|ntty
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|mypw
condition|)
block|{
comment|/* get key and check again */
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Key: "
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|fgets
argument_list|(
name|s
argument_list|,
sizeof|sizeof
argument_list|(
name|s
argument_list|)
argument_list|,
name|stdin
argument_list|)
operator|||
operator|*
name|s
operator|==
literal|'\n'
condition|)
name|quit
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\nAgain: "
argument_list|)
expr_stmt|;
comment|/* 		 * Don't need EOF test here, if we get EOF, then s1 != s 		 * and the right things will happen. 		 */
operator|(
name|void
operator|)
name|fgets
argument_list|(
name|s1
argument_list|,
sizeof|sizeof
argument_list|(
name|s1
argument_list|)
argument_list|,
name|stdin
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|putchar
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|s1
argument_list|,
name|s
argument_list|)
condition|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\07lock: passwords didn't match.\n"
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|tty
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|s
index|[
literal|0
index|]
operator|=
name|NULL
expr_stmt|;
name|mypw
operator|=
name|s1
expr_stmt|;
block|}
comment|/* set signal handlers */
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|hi
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGQUIT
argument_list|,
name|hi
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGTSTP
argument_list|,
name|hi
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|bye
argument_list|)
expr_stmt|;
name|ntimer
operator|.
name|it_interval
operator|=
name|zerotime
expr_stmt|;
name|ntimer
operator|.
name|it_value
operator|=
name|timeout
expr_stmt|;
name|setitimer
argument_list|(
name|ITIMER_REAL
argument_list|,
operator|&
name|ntimer
argument_list|,
operator|&
name|otimer
argument_list|)
expr_stmt|;
comment|/* header info */
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"lock: %s on %s. timeout in %d minutes\ntime now is %.20s%s%s"
argument_list|,
name|ttynam
argument_list|,
name|hostname
argument_list|,
name|sectimeout
argument_list|,
name|ap
argument_list|,
name|tzn
argument_list|,
name|ap
operator|+
literal|19
argument_list|)
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"Key: "
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|fgets
argument_list|(
name|s
argument_list|,
sizeof|sizeof
argument_list|(
name|s
argument_list|)
argument_list|,
name|stdin
argument_list|)
condition|)
block|{
name|clearerr
argument_list|(
name|stdin
argument_list|)
expr_stmt|;
name|hi
argument_list|()
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|usemine
condition|)
block|{
name|s
index|[
name|strlen
argument_list|(
name|s
argument_list|)
operator|-
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|mypw
argument_list|,
name|crypt
argument_list|(
name|s
argument_list|,
name|mypw
argument_list|)
argument_list|)
condition|)
break|break;
block|}
elseif|else
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|s
argument_list|,
name|s1
argument_list|)
condition|)
break|break;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\07\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCGETP
argument_list|,
operator|&
name|ntty
argument_list|)
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|quit
argument_list|()
expr_stmt|;
block|}
end_function

begin_function
name|void
name|hi
parameter_list|()
block|{
name|struct
name|timeval
name|timval
decl_stmt|;
if|if
condition|(
operator|!
name|gettimeofday
argument_list|(
operator|&
name|timval
argument_list|,
operator|(
expr|struct
name|timezone
operator|*
operator|)
name|NULL
argument_list|)
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"lock: type in the unlock key. timeout in %ld:%ld minutes\n"
argument_list|,
operator|(
name|nexttime
operator|-
name|timval
operator|.
name|tv_sec
operator|)
operator|/
literal|60
argument_list|,
operator|(
name|nexttime
operator|-
name|timval
operator|.
name|tv_sec
operator|)
operator|%
literal|60
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|quit
parameter_list|()
block|{
operator|(
name|void
operator|)
name|putchar
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|tty
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|bye
parameter_list|()
block|{
operator|(
name|void
operator|)
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|tty
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"lock: timeout\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

