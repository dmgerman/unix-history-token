...
index|[]
init|=
literal|"@(#)pat_rep.c	8.1 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt
...
operator|)
return|;
comment|/* 	 * when we are NOT limited to a single match per pattern mark the 	 * the pattern and return 	 */
if|if
condition|(
operator|!
name|nflag
...
operator|)
return|;
comment|/* 		 * ok we matched a directory and we are allowing 		 * subtree matches. We add this as a DIR_MTCH pattern 		 * so all its children will match. Note we know that 		 * when successful, pat_add() puts the pattern at the 		 * tail (yup a kludge). In the code below will make 		 * a dir match pattern 		 */
if|if
condition|(
operator|(
name|pat_add
...
expr_stmt|;
block|}
comment|/* 	 * we reach this point only when we allow a single selected match per 	 * pattern, or we have to add a DIR_MATCH pattern. if the pattern 	 * matched a directory and we do not have -d * (dflag) we are done 	 * with this pattern. We may also be handed a file in the subtree of a 	 * directory. in that case when we are operating with -d, this pattern 	 * was already selected and we are done 	 */
if|if
condition|(
name|pt
operator|->
...
condition|)
block|{
comment|/* 		 * we are allowing subtree matches at directories, mark the 		 * node as a directory match so pat_match() will only match 		 * children of this directory (we replace the pattern with the 		 * directory name to enforce this subtree only match) 		 * pat_match() looks for DIR_MTCH to determine what comparison 		 * technique to use when it checks for a pattern match 	 	 */
if|if
condition|(
operator|(
name|pt
...
return|;
block|}
comment|/* 	 * it is not a directory, we are then done with this pattern, so we 	 * delete it from the list, as it can never be used for another match 	 * Seems kind of strange to do for a -c, but the pax spec is really 	 * vague on the interaction of -c -n and -d. We assume that when -c 	 * and the pattern rejects a member (i.e. it matched it) it is done. 	 * In effect we place the order of the flags as having -c last. 	 */
name|pt
operator|=
name|pathead
expr_stmt|;
...
condition|)
block|{
comment|/* 			 * this pattern was matched before to a directory 			 * as we must have -n set for this (but not -d). We can 			 * only match CHILDREN of that directory so we must use 			 * an exact prefix match 			 */
if|if
condition|(
operator|(
name|strncmp
