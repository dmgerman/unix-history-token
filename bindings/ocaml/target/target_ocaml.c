...
name|CAMLprim
name|value
name|llvm_register_target_exns
parameter_list|(
name|value
name|Error
parameter_list|)
...
begin_comment
comment|/* [<Llvm.PassManager.any] Llvm.PassManager.t -> DataLayout.t -> unit */
end_comment

begin_function
name|CAMLprim
name|value
name|llvm_datalayout_add_to_pass_manager
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|/* LLVMGetTargetMachineData returns a pointer owned by the TargetMachine,      so it is impossible to wrap it with llvm_alloc_target_data, which assumes      that OCaml owns the pointer. */
name|LLVMTargetDataRef
name|OrigDataLayout
decl_stmt|;
name|OrigDataLayout
