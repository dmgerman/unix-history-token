...
begin_comment
comment|/*  * crypto_encrypt - construct encrypted cookie and signature from  * extension field and cookie  *  * Returns  * XEVNT_OK	success  * XEVNT_CKY	bad or missing cookie  * XEVNT_PUB	bad or missing public key  */
end_comment
...
begin_comment
comment|/*  * crypto_send - construct extension field from value components  *  * The value and signature fields are zero-padded to a word boundary.  * Note: it is not polite to send a nonempty signature with zero  * timestamp or a nonzero timestamp with an empty signature, but those  * rules are not enforced here.  */
end_comment
...
begin_comment
comment|/*  * asn2ntp - convert ASN1_TIME time structure to NTP time.  *  * Returns NTP seconds (no errors)  */
end_comment
...
comment|/* 	 * Extract time string YYMMDDHHMMSSZ from ASN1 time structure. 	 * Note that the YY, MM, DD fields start with one, the HH, MM, 	 * SS fiels start with zero and the Z character is ignored. 	 * Also note that years less than 50 map to years greater than 	 * 100. Dontcha love ASN.1? Better than MIL-188. 	 */
name|v
operator|=
operator|(
name|char
...
name|LOG_NOTICE
argument_list|,
literal|"cert_parse: invalid times %s first %u last %u fstamp %u"
argument_list|,
name|ret
operator|->
name|subject
...
name|subject
expr_stmt|;
comment|/* 	 * If trusted certificate, the subject name must match the group 	 * name. 	 */
if|if
condition|(
name|cinfo
operator|->
...
name|LOG_ERR
argument_list|,
literal|"crypto_setup: trusted certificate name %s does not match group name %s"
argument_list|,
name|hostval
operator|.
name|ptr
...
condition|)
block|{
comment|/* 		 * Load optional IFF parameters from file 		 * "ntpkey_iffkey_<groupname>". 		 */
name|snprintf
argument_list|(
name|filename
argument_list|,
...
name|CRYPTO_FLAG_IFF
expr_stmt|;
comment|/* 		 * Load optional GQ parameters from file 		 * "ntpkey_gqkey_<groupname>". 		 */
name|snprintf
argument_list|(
name|filename
argument_list|,
...
name|CRYPTO_FLAG_GQ
expr_stmt|;
comment|/* 		 * Load optional MV parameters from file 		 * "ntpkey_mvkey_<groupname>". 		 */
name|snprintf
argument_list|(
name|filename
argument_list|,
