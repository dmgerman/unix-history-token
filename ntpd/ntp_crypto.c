...
name|macbytes
decl_stmt|;
comment|/* length of MAC field, signed by intention */
name|int
name|authlen
decl_stmt|;
...
condition|)
block|{
comment|/* We can be reasonably sure that we can read at least 		 * the opcode and the size field here. More stringent 		 * checks follow up shortly. 		 */
name|pkt
operator|=
operator|(
name|u_int32
...
argument_list|,
operator|(
literal|"crypto_recv: possible attack detected, associd %d\n"
operator|,
name|associd
operator|)
argument_list|)
...
return|;
block|}
comment|/* Check if the paylod of the extension fits into the 		 * declared frame. 		 */
if|if
condition|(
name|len
operator|>=
...
condition|)
break|break;
comment|/* Check if we can update the basic TAI offset 			 * for our current leap frame. This is a hack 			 * and ignores the time stamps in the autokey 			 * message. 			 */
if|if
condition|(
name|sys_leap
operator|!=
...
expr_stmt|;
comment|/* i += j; */
comment|/* We don't use i after this */
block|}
name|opcode
operator|=
name|ntohl
...
name|fstamp
expr_stmt|;
comment|/* Get the leap second era. We might need a full lookup early 	 * after start, when the cache is not yet loaded. 	 */
name|leapsec_frame
argument_list|(
operator|&
name|leap_data
...
expr_stmt|;
block|}
comment|/* Create the data block. The protocol does not work without. */
name|len
operator|=
literal|3
operator|*
...
condition|)
block|{
comment|/* create a TAI / leap era block. The end time is a 		 * fake -- maybe we can do better. 		 */
name|ptr
index|[
literal|0
index|]
...
argument_list|)
decl_stmt|;
comment|/* check if the signing digest algo is available */
if|if
condition|(
name|sign_digest
operator|==
...
condition|)
return|return;
comment|/* check size of TAI extension block */
if|if
condition|(
name|tai_leap
operator|.
...
condition|)
return|return;
comment|/* crypto_update should at least not crash here! */
name|crypto_update
argument_list|()
expr_stmt|;
block|}
...
name|len
decl_stmt|;
comment|/* extension field value length */
specifier|const
name|BIGNUM
modifier|*
name|p
...
name|LOG_ERR
argument_list|,
literal|"crypto_bob: signature is too big: %u"
argument_list|,
name|len
argument_list|)
expr_stmt|;
