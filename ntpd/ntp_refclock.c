...
begin_comment
comment|/*  * refclock_gtraw - get next line/chunk of data  *  * This routine returns the raw data received from the clock in both  * canonical or raw modes. The terminal interface routines map CR to LF.  * In canonical mode this results in two lines, one containing data  * followed by LF and another containing only LF. In raw mode the  * interface routines can deliver arbitraty chunks of data from one  * character to a maximum specified by the calling routine. In either  * mode the routine returns the number of characters in the line  * followed by a NULL character ('\0'), which is not included in the  * count.  *  * If a timestamp is present in the timecode, as produced by the tty_clk  * STREAMS module, it returns that as the timestamp; otherwise, it  * returns the buffer timestamp.  */
end_comment
...
block|}
block|}
comment|/* 	 * Copy the raw buffer to the user string. The string is padded 	 * with a NULL, which is not included in the character count. 	 */
if|if
condition|(
name|dpend
operator|-
...
name|LOG_NOTICE
argument_list|,
literal|"refclock_ioctl fd %d I_PUSH: %m"
argument_list|,
name|fd
argument_list|)
expr_stmt|;
...
name|LOG_ERR
argument_list|,
literal|"refclock_ioctl fd %d CLK_SETSTR: %m"
argument_list|,
name|fd
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/* 	 * If flag3 is lit, select the kernel PPS. 	 */
if|if
condition|(
name|mode
operator|&
...
operator|)
return|;
comment|/* 	 * There can be zero, one or two PPS pulses between polls, 	 * depending on the poll interval relative to the PPS interval. 	 * The pulse must be newer and within the range gate relative 	 * to the last pulse. 	 */
if|if
condition|(
name|ap
operator|->
