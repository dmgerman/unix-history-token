...
block|{
comment|/* 			 * Here the peer is reachable. Send a burst if 			 * enabled and the peer is fit. 			 */
name|hpoll
operator|=
name|sys_poll
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* 		 * Watch for timeout. If preemptable, toss the rascal; 		 * otherwise, bump the poll interval. Note the 		 * poll_update() routine will clamp it to maxpoll. 		 */
if|if
condition|(
name|peer
operator|->
...
comment|/* access denied */
block|}
comment|/* 	 * Update the MRU list and finger the cloggers. It can be a 	 * little expensive, so turn it off for production use. 	 */
name|restrict_mask
operator|=
name|ntp_monitor
argument_list|(
...
directive|ifdef
name|OPENSSL
comment|/* 		 * Do not respond if Autokey and the opcode is not a 		 * CRYPTO_ASSOC response with associationn ID. 		 */
if|if
condition|(
name|crypto_flags
operator|&&
...
block|}
block|}
comment|/* 		 * Do not respond if synchronized and stratum is either 		 * below the floor or at or above the ceiling. Note, 		 * this allows an unsynchronized peer to synchronize to 		 * us. It would be very strange if he did and then was 		 * nipped, but that could only happen if we were 		 * operating at the top end of the range. 		 */
if|if
condition|(
name|hisleap
operator|!=
...
comment|/* no help */
block|}
comment|/* 		 * The message is correctly authenticated and 		 * allowed. Mobiliae a symmetric passive association. 		 */
if|if
condition|(
operator|(
name|peer
...
expr_stmt|;
return|return;
comment|/* 	 * If this is a broadcast mode packet, skip further checking. If 	 * an intial volley, bail out now and let the client do its 	 * stuff. If the origin timestamp is nonzero, this is an 	 * interleaved broadcast. so restart the protocol. 	 */
block|}
elseif|else
if|if
condition|(
...
expr_stmt|;
comment|/* unsynch */
comment|/* 	 * Check for bogus packet in interleaved symmetric mode. This 	 * can happen if a packet is lost, duplicat or crossed. If 	 * found, flip and resynchronize. 	 */
block|}
elseif|else
if|if
condition|(
...
return|return;
block|}
comment|/* 	 * Set the peer ppoll to the maximum of the packet ppoll and the 	 * peer minpoll. If a kiss-o'-death, set the peer minpoll to 	 * this maximumn and advance the headway to give the sender some 	 * headroom. Very intricate. 	 */
name|peer
operator|->
name|ppoll
operator|=
...
condition|)
block|{
comment|/* 		 * Decrement remaining audokey hashes. This isn't 		 * perfect if a packet is lost, but results in no harm. 		 */
name|ap
operator|=
operator|(
expr|struct
...
expr_stmt|;
comment|/* bad autokey */
comment|/* 		 * The maximum lifetime of the protocol is about one 		 * week before restarting the Autokey protocol to 		 * refreshed certificates and leapseconds values. 		 */
if|if
condition|(
name|current_time
operator|>
...
comment|/* OPENSSL */
block|}
comment|/* 		 * If the leapseconds values are from file or network 		 * and the leap is in the future, schedule a leap at the 		 * given epoch. Otherwise, if the number of survivor 		 * leap bits is greater than half the number of 		 * survivors, schedule a leap for the end of the current 		 * month. 		 */
name|get_systime
argument_list|(
operator|&
name|now
...
name|utemp
decl_stmt|;
comment|/* 	 * This routine figures out when the next poll should be sent. 	 * That turns out to be wickedly complicated. One problem is 	 * that sometimes the time for the next poll is in the past when 	 * the poll interval is reduced. We watch out for races here 	 * between the receive process and the poll process. 	 * 	 * First, bracket the poll interval according to the type of 	 * association and options. If a fixed interval is configured, 	 * use minpoll. This primarily is for reference clocks, but 	 * works for any association. Otherwise, clamp the poll interval 	 * between minpoll and maxpoll. 	 */
if|if
condition|(
name|peer
operator|->
...
directive|endif
comment|/* REFCLOCK */
comment|/* 	 * During initialization use the association count to spread out 	 * the polls at one-second intervals. Otherwise, randomize over 	 * the minimum poll interval in order to avoid broadcast 	 * implosion. 	 */
name|peer
operator|->
name|nextdate
operator|=
...
expr_stmt|;
block|}
comment|/* 	 * If the clock discipline has stabilized, sort the samples by 	 * distance.   	 */
if|if
condition|(
name|sys_leap
operator|!=
...
break|break;
block|}
comment|/* 	 * Clustering algorithm. Construct candidate list in order first 	 * by stratum then by root distance, but keep only the best 	 * NTP_MAXASSOC of them. Scan the list to find falsetickers, who 	 * leave the island immediately. The TRUE peer is always a 	 * truechimer. We must leave at least one peer to collect the 	 * million bucks. 	 */
name|j
operator|=
literal|0
expr_stmt|;
...
directive|endif
comment|/* REFCLOCK */
comment|/* 		 * The metric is the scaled root distance at the next 		 * poll interval plus the peer stratum. 		 */
name|d
operator|=
operator|(
name|root_distance
...
expr_stmt|;
block|}
comment|/* 	 * What remains is a list usually not greater than sys_minclock 	 * peers. Note that the head of the list is the system peer at 	 * the lowest stratum and that unsynchronized peers cannot 	 * survive this far. 	 * 	 * While at it, count the number of leap warning bits found. 	 * This will be used later to vote the system leap warning bit. 	 * If a leap warning bit is found on a reference clock, the vote 	 * is always won. 	 */
name|leap_vote
operator|=
literal|0
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* 	 * Unless there are at least sys_misane survivors, leave the 	 * building dark. Otherwise, do a clockhop dance. Ordinarily, 	 * use the first survivor on the survivor list. However, if the 	 * last selection is not first on the list, use it as long as 	 * it doesn't get too old or too ugly. 	 */
if|if
condition|(
name|nlist
operator|>
...
comment|/* 	 * Loop to find precision value in seconds. 	 */
name|tick
operator|=
name|MAXSTEP
expr_stmt|;
...
name|LOG_NOTICE
argument_list|,
literal|"proto: precision = %.3f usec"
argument_list|,
name|tick
operator|*
literal|1e6
