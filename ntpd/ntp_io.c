...
begin_comment
comment|/* TODO burnicki: This should be moved to ntp_timer.c, but if we do so  * we get a linker error. Since we're running out of time before the leap  * second occurs, we let it here where it just works.  */
end_comment
...
directive|endif
comment|/* INCLUDE_IPV6_SUPPORT&& SIOCGIFAFLAG_IN6 */
return|return
name|ISC_FALSE
return|;
block|}
...
condition|)
continue|continue;
comment|/* 		 * skip any address that is an invalid state to be used 		 */
if|if
condition|(
operator|!
name|is_valid
...
condition|)
continue|continue;
comment|/* Find remote address that was linked to this interface */
for|for
control|(
name|entry
operator|=
...
name|LOG_ERR
argument_list|,
literal|"bind(%d) AF_INET%s %s#%d%s flags 0x%x failed: %m"
argument_list|,
name|fd
argument_list|,
name|IS_IPV6
...
argument_list|,
operator|(
literal|"fetch_timestamp: system usec network time stamp: %jd.%06ld\n"
operator|,
operator|(
name|intmax_t
operator|)
