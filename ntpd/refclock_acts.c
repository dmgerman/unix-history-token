...
directive|define
name|LOCKFILE
value|"/var/spool/locks/LCK..cua%d"
end_define
...
begin_comment
comment|/* max clockstats line length */
end_comment
...
begin_comment
comment|/*  * Modem setup strings. These may have to be changed for some modems.  *  * AT	command prefix  * B1	US answer tone  *&C0	disable carrier detect  *&D2	hang up and return to command mode on DTR transition  * E0	modem command echo disabled  * l1	set modem speaker volume to low level  * M1	speaker enabled until carrier detect  * Q0	return result codes  * V1	return result codes as English words  */
end_comment
...
begin_comment
comment|/* first valid message timeout */
end_comment
...
begin_comment
comment|/* all valid messages timeout */
end_comment
...
begin_comment
comment|/* wait for first valid message */
end_comment
...
begin_comment
comment|/* wait after sending disconnect */
end_comment
...
directive|endif
comment|/* 	 * Extract the first token in the line. A NO token sends the 	 * message to the clockstats. 	 */
name|strncpy
argument_list|(
name|tbuf
argument_list|,
...
condition|)
block|{
comment|/* 	 * We are waiting for the OK response to the modem setup 	 * command. When this happens, raise DTR and dial the number 	 * followed by \r. 	 */
case|case
name|S_OK
case|:
if|if
...
expr_stmt|;
return|return;
comment|/* 	 * We are waiting for the call to be answered. All we care about 	 * here is token CONNECT. Send the message to the clockstats. 	 */
case|case
name|S_CONNECT
case|:
name|report_event
...
expr_stmt|;
return|return;
comment|/* 	 * We are waiting for a timecode. Pass it to the parser. 	 */
case|case
name|S_FIRST
case|:
case|case
...
break|break;
return|return;
comment|/* 	 * ACTS format: "jjjjj yy-mm-dd hh:mm:ss ds l uuu aaaaa 	 * UTC(NIST) *" 	 */
case|case
name|LENACTS
case|:
if|if
...
return|return;
block|}
comment|/* 		 * Wait until ACTS has calculated the roundtrip delay. 		 * We don't need to do anything, as ACTS adjusts the 		 * on-time epoch. 		 */
if|if
condition|(
name|flag
operator|!=
...
name|pp
decl_stmt|;
comment|/* 	 * This routine implments a timeout which runs for a programmed 	 * interval. The counter is initialized by the state machine and 	 * counts down to zero. Upon reaching zero, the state machine is 	 * called. If flag1 is set while in S_IDLE state, force a 	 * timeout. 	 */
name|pp
operator|=
name|peer
operator|->
...
index|]
decl_stmt|;
comment|/* 	 * The state machine is driven by messages from the modem, when 	 * first stated and at timeout. 	 */
name|pp
operator|=
name|peer
operator|->
...
condition|)
block|{
comment|/* 	 * System poll event. Lock the modem port and open the device. 	 */
case|case
name|S_IDLE
case|:
...
return|return;
block|}
comment|/* 		 * Initialize the modem. This works with Hayes commands. 		 */
ifdef|#
directive|ifdef
name|DEBUG
if|if
...
expr_stmt|;
return|return;
comment|/* 	 * In OK state the modem did not respond to setup. 	 */
case|case
name|S_OK
case|:
name|msyslog
...
name|LOG_ERR
argument_list|,
literal|"acts: no modem"
argument_list|)
expr_stmt|;
break|break;
comment|/* 	 * In DTR state we are waiting for the modem to settle down 	 * before hammering it with a dial command. 	 */
case|case
name|S_DTR
case|:
name|snprintf
...
expr_stmt|;
return|return;
comment|/* 	 * In CONNECT state the call did not complete. 	 */
case|case
name|S_CONNECT
case|:
name|msyslog
...
name|LOG_ERR
argument_list|,
literal|"acts: no answer"
argument_list|)
expr_stmt|;
break|break;
comment|/* 	 * In FIRST state no messages were received. 	 */
case|case
name|S_FIRST
case|:
name|msyslog
...
name|LOG_ERR
argument_list|,
literal|"acts: no messages"
argument_list|)
expr_stmt|;
break|break;
comment|/* 	 * In CLOSE state hangup is complete. Close the doors and 	 * windows and get some air. 	 */
case|case
name|S_CLOSE
case|:
comment|/* 		 * Close the device and unlock a shared modem. 		 */
if|if
condition|(
name|pp
operator|->
...
expr_stmt|;
block|}
comment|/* 		 * If messages were received, fold the tent and wait for 		 * the next poll. If no messages and there are more 		 * numbers to dial, retry after a short wait. 		 */
name|up
operator|->
name|bufptr
operator|=
...
begin_comment
comment|/*  * acts_disc - disconnect the call and clean the place up.  */
end_comment
...
name|TIOCM_DTR
decl_stmt|;
comment|/* 	 * We get here if the call terminated successfully or if an 	 * error occured. If the median filter has something in it, 	 * feed the data to the clock filter. If a modem port, drop DTR 	 * to force command mode and send modem hangup. 	 */
name|pp
operator|=
name|peer
operator|->
