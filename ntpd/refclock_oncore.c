...
name|pps_control
decl_stmt|;
comment|/* PPS control, M12 only */
name|s_char
name|pps_control_msg_seen
decl_stmt|;
block|}
struct|;
end_struct
...
begin_comment
comment|/* 12	PPS Control: Off, On, 1+satellite,TRAIM */
end_comment
...
literal|1
expr_stmt|;
comment|/* Have seen response to Gc msg */
name|instance
operator|->
name|model
operator|=
...
name|LOG_ERR
argument_list|,
literal|"ONCORE: time_pps_setparams fails %m"
argument_list|)
expr_stmt|;
return|return
operator|(
...
literal|2
expr_stmt|;
comment|/* PPS on when satellite is available */
block|}
elseif|else
if|if
condition|(
...
literal|3
expr_stmt|;
comment|/* PPS on when TRAIM status is OK */
block|}
else|else
block|{
name|oncore_log_f
...
name|LOG_WARNING
argument_list|,
literal|"Unknown value \"%s\" for PPSCONTROL, ignored"
argument_list|,
name|cc
argument_list|)
expr_stmt|;
...
name|FLAG_PPS
expr_stmt|;
comment|/* problem - clear PPS FLAG */
return|return;
block|}
...
name|LOG_ERR
argument_list|,
literal|"time_pps_fetch failed %m"
argument_list|)
expr_stmt|;
name|peer
operator|->
...
directive|ifdef
name|HAVE_STRUCT_TIMESPEC
block|oncore_log_f(instance, LOG_DEBUG, 				     "serial/j (%lu, %lu) %ld.%09ld", i, 				     j, (long)tsp->tv_sec, 				     (long)tsp->tv_nsec);
else|#
directive|else
block|oncore_log_f(instance, LOG_DEBUG, 				     "serial/j (%lu, %lu) %ld.%06ld", i, 				     j, (long)tsp->tv_sec, 				     (long)tsp->tv_usec);
endif|#
directive|endif
block|}
endif|#
...
expr_stmt|;
comment|/* Ba, Ea or Ha */
comment|/* check if we saw a response to Gc (M12 or M12+T */
if|if
condition|(
name|instance
operator|->
...
name|LOG_INFO
argument_list|,
literal|"PPSCONTROL set, but not implemented (not M12)"
argument_list|)
expr_stmt|;
block|}
name|instance
...
block|}
block|}
comment|/*  * Reg only wants the following output for "deeper" driver debugging.  * See Bug 2142 and Bug 1866  */
if|#
directive|if
literal|0
block|oncore_log_f(instance, LOG_DEBUG, 		     "dt_ls = %d  dt_lsf = %d  WN = %d  DN = %d  WN_lsf = %d  DNlsf = %d  wn_flg = %d  lsf_flg = %d  Bl_day = %d", 		     instance->Bl.dt_ls, instance->Bl.dt_lsf, 		     instance->Bl.WN, instance->Bl.DN, 		     instance->Bl.WN_lsf, instance->Bl.DN_lsf, 		     instance->Bl.wn_flg, instance->Bl.lsf_flg, 		     instance->Bl.Bl_day);
endif|#
directive|endif
block|}
specifier|static
...
argument_list|)
expr_stmt|;
comment|/* PPS off/continuous/Tracking 1+sat/TRAIM */
block|}
name|instance
operator|->
name|count
...
expr_stmt|;
block|}
comment|/* Response to PPS Control message (M12 and M12+T only ) */
specifier|static
name|void
name|oncore_msg_Gc
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|/* There seems to be eternal confusion about when a leap second 	 * takes place. It's the second *before* the new TAI offset 	 * becomes effective. But since the ONCORE receiver tells us 	 * just that, we would have to do some time/date calculations to 	 * get the actual leap second -- that is, the one that is 	 * deleted or inserted. 	 * 	 * Going through all this for a simple log is probably overkill, 	 * so for fixing bug#1050 the message output is changed to 	 * reflect the fact that it tells the second after the leap 	 * second. 	 */
if|if
condition|(
name|dt
condition|)
...
name|LOG_NOTICE
argument_list|,
literal|"Leap second %s (%d) before %04u-%02u-%02u/%02u:%02u:%02u"
argument_list|,
name|insrem
index|[
operator|(
