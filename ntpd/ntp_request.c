...
block|fl = 0; 	while (items--> 0&& !fl) { 		if (((temp_cp.version)> NTP_VERSION) 		    || ((temp_cp.version)< NTP_OLDVERSION)) 		    fl = 1; 		if (temp_cp.hmode != MODE_ACTIVE&& temp_cp.hmode != MODE_CLIENT&& temp_cp.hmode != MODE_BROADCAST) 		    fl = 1; 		if (temp_cp.flags& ~(CONF_FLAG_PREFER | CONF_FLAG_BURST | 		    CONF_FLAG_IBURST | CONF_FLAG_SKEY)) 			fl = 1; 		cp = (struct conf_peer *) 		    ((char *)cp + INFO_ITEMSIZE(inpkt->mbz_itemsize)); 	}  	if (fl) { 		req_ack(srcadr, inter, inpkt, INFO_ERR_FMT); 		return; 	}
endif|#
directive|endif
...
begin_comment
unit|static void dns_a( 	sockaddr_u *srcadr, 	struct interface *inter, 	struct req_pkt *inpkt 	) { 	register struct info_dns_assoc *dp; 	register int items; 	struct sockaddr_in peeraddr;
comment|/* 	 * Do a check of everything to see that it looks 	 * okay.  If not, complain about it.  Note we are 	 * very picky here. 	 */
end_comment
...
begin_comment
unit|if (!ISREFCLOCKADR(&peeraddr)&& ISBADADR(&peeraddr)) { 		msyslog(LOG_ERR, "dns_a: !ISREFCLOCKADR&& ISBADADR"); 		req_ack(srcadr, inter, inpkt, INFO_ERR_FMT); 		return; 	}  	while (items--> 0) { 		associd_t associd; 		size_t hnl; 		struct peer *peer; 		int bogon = 0;  		associd = dp->associd; 		peer = findpeerbyassoc(associd); 		if (peer == 0 || peer->flags& FLAG_REFCLOCK) { 			msyslog(LOG_ERR, "dns_a: %s", 				(peer == 0) 				? "peer == 0" 				: "peer->flags& FLAG_REFCLOCK"); 			++bogon; 		} 		peeraddr.sin_addr.s_addr = dp->peeraddr; 		for (hnl = 0; dp->hostname[hnl]&& hnl< sizeof dp->hostname; ++hnl) ; 		if (hnl>= sizeof dp->hostname) { 			msyslog(LOG_ERR, "dns_a: hnl (%ld)>= %ld", 				(long)hnl, (long)sizeof dp->hostname); 			++bogon; 		}  		msyslog(LOG_INFO, "dns_a:<%s> for %s, AssocID %d, bogon %d", 			dp->hostname, 			stoa((sockaddr_u *)&peeraddr), associd, 			bogon);  		if (bogon) {
...
argument_list|)
expr_stmt|;
comment|/* scope no longer in struct interface, in in6_addr typically */
name|ifs
operator|->
name|scopeid
operator|=
