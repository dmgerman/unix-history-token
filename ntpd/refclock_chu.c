...
begin_comment
comment|/* offset for narrow filter (kHz) */
end_comment
...
begin_comment
comment|/* minutes in a probe cycle */
end_comment
...
name|unitptr
expr_stmt|;
comment|/* 	 * In a format B burst, a character is considered valid only if 	 * the first occurrence matches the last occurrence. The burst 	 * is considered valid only if all characters are valid; that 	 * is, only if the distance is 40. Note that once a valid frame 	 * has been found errors are ignored. 	 */
name|sprintf
argument_list|(
name|tbuf
argument_list|,
...
operator|++
expr_stmt|;
comment|/* 	 * If once in sync and the radio has not been heard for awhile 	 * (30 m), it is no longer reachable. If not heard in a long 	 * while (one day), turn out the lights and start from scratch. 	 */
name|minset
operator|=
operator|(
operator|(
...
name|a_lastcode
argument_list|,
literal|"%c%1X %04d %3d %02d:%02d:%02d %c%x %+d %d %d %s %.0f %d"
argument_list|,
name|synchar
argument_list|,
name|qual
...
name|a_lastcode
argument_list|,
literal|"%c%1X %04d %3d %02d:%02d:%02d %c%x %+d %d %s %.0f %d"
argument_list|,
name|synchar
argument_list|,
name|qual
...
argument_list|)
expr_stmt|;
comment|/* 	 * If in sync and the signal metric is above threshold, the 	 * timecode is ipso fatso valid and can be selected to 	 * discipline the clock. Be sure not to leave stray timestamps 	 * around if signals are too weak or the clock time is invalid. 	 */
if|if
condition|(
name|up
operator|->
...
name|unitptr
expr_stmt|;
comment|/* 	 * Majority decoder. Each burst encodes two replications at each 	 * digit position in the timecode. Each row of the decoding 	 * matrix encodes the number of occurrences of each digit found 	 * at the corresponding position. The maximum over all 	 * occurrences at each position is the distance for this 	 * position and the corresponding digit is the maximum 	 * likelihood candidate. If the distance is zero, assume a miss 	 * '_'; if the distance is not more than half the total number 	 * of occurrences, assume a soft error '*'; if two different 	 * digits with the same distance are found, assume a hard error 	 * '='. These will later cause a format error when the timecode 	 * is interpreted. The decoding distance is defined as the 	 * minimum distance over the first nine digits. The tenth digit 	 * varies over the seconds, so we don't count it. 	 */
name|mindist
operator|=
literal|16
expr_stmt|;
...
literal|0
expr_stmt|;
comment|/* 	 * A valid timecode requires a minimum distance at least half 	 * the total number of occurrences. A valid timecode also 	 * requires at least 20 valid timestamps. 	 */
if|if
condition|(
name|up
operator|->
...
name|unitptr
expr_stmt|;
comment|/* 	 * The radio can be tuned to three channels: 0 (3330 kHz), 1 	 * (7335 kHz) and 2 (14670 kHz). There are five one-minute 	 * dwells in each cycle. During the first dwell the radio is 	 * tuned to one of three probe channels; during the remaining 	 * four dwells the radio is tuned to the data channel. The probe 	 * channel is selects as the least recently used. At the end of 	 * each dwell the channel metrics are measured and the highest 	 * one is selected as the data channel.  	 */
if|if
condition|(
name|up
operator|->
...
name|tbuf
argument_list|,
literal|"chu: QSY to %.3f MHz metric %.0f"
argument_list|,
name|qsy
index|[
name|up
...
directive|endif
block|}
comment|/* 	 * Start the next dwell. We speed up the initial sync a little. 	 * If not in sync and no bursts were heard the previous dwell, 	 * restart the probe. 	 */
name|rval
operator|=
literal|0
expr_stmt|;
...
name|printf
argument_list|(
literal|"chu: at %ld dwell %d achan %d metric %.0f chan %d\n"
argument_list|,
name|current_time
argument_list|,
name|up
...
name|printf
argument_list|(
literal|"chu: at %ld probe channel %d\n"
argument_list|,
name|current_time
argument_list|,
name|up
