...
name|complain
argument_list|(
literal|"too many buffers to list"
argument_list|)
expr_stmt|;
block|}
block|}
...
begin_comment
comment|/* Free a buffer structure.  * The actual struct is preserved to reduce the damage  * from dangling references to it.  They seem to be pervasive.  * We try to reset enough that a dangling reference will be useless.  */
end_comment
...
begin_comment
comment|/* Returns pointer to buffer with name NAME, or if NAME is a string of digits    returns the buffer whose number equals those digits.  Otherwise, returns    NULL. */
end_comment
