...
begin_comment
comment|/* DoAutoExec: NEW and OLD are file names, and if NEW and OLD aren't the    same kind of file (i.e., match the same pattern) or OLD is NULL and it    matches, OR if the pattern is NULL (none was specified) then, we execute    the command associated with that kind of file. */
end_comment
...
do|do
block|{
comment|/* This peculiar delayed EOF testing allows the last line to 		 * end without a NL.  We add NL later, so we leave room for it. 		 */
name|eof
operator|=
name|f_gets
argument_list|(
