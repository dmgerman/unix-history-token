...
begin_comment
comment|/* Define some macros for dealing with assorted signalling disciplines.     These macros provide a way to use signal blocking and disabling    without smothering your code in a pile of #ifdef's.     SIGNALS_UNBLOCK;			Stop blocking all signals.     {      SIGNALS_DECLARE_SAVED (name);	Declare a variable to save the  					signal blocking state. 	...      SIGNALS_BLOCK (SIGSTOP, name);	Block a signal, and save the previous 					state for restoration later. 	...      SIGNALS_RESTORE (name);		Restore previous signals.    }  */
end_comment
...
define|\
value|do { sigset_t set;	\ 	sigemptyset (&set);	\ 	sigprocmask (SIG_SETMASK,&set, (sigset_t *)NULL);	\       } while (0)
end_define

begin_define
define|#
directive|define
name|SIGNALS_DECLARE_SAVED
parameter_list|(
name|name
parameter_list|)
value|sigset_t name
...
parameter_list|)
define|\
value|do { sigset_t set;		\ 	  sigemptyset (&set);		\ 	  sigaddset (&set, SIG);	\ 	  sigprocmask (SIG_BLOCK,&set,&saved);	\ 	} while (0)
end_define

begin_define
define|#
directive|define
name|SIGNALS_RESTORE
parameter_list|(
name|saved
parameter_list|)
define|\
value|sigprocmask (SIG_SETMASK,&saved, (sigset_t *)NULL)
end_define
...
name|saved
parameter_list|)
value|saved = sigblock (sigmask (SIG))
end_define
...
begin_comment
comment|/*  End of signal handling definitions.  */
end_comment
...
begin_comment
comment|/* Posix macro to check file in statbuf for directory-ness. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Set up input and output if they aren't already.  */
if|if
condition|(
operator|!
name|rl_instream
...
begin_comment
comment|/* On Solaris2, sys/types.h brings in sys/reg.h,    which screws up the Termcap variable PC, used below.  */
end_comment
...
condition|)
return|return;
comment|/* Try to keep this function from being INTerrupted.  We can do it      on POSIX and systems with BSD-like signal handling. */
name|SIGNALS_BLOCK
argument_list|(
name|SIGINT
argument_list|,
...
begin_comment
comment|/* The list of characters which are used to quote a substring of the command    line.  Command completion occurs on the entire substring, and within the    substring rl_completer_word_break_characters are treated as any other    character, unless they also appear within this list. */
end_comment
...
condition|)
block|{
comment|/* We have a list of characters which can be used in pairs to quote 	     substrings for completion.  Try to find the start of an unclosed 	     quoted substring. 	     FIXME:  Doesn't yet handle '\' escapes to hid embedded quotes */
for|for
control|(
name|scan
operator|=
...
condition|)
block|{
comment|/* Found matching close quote. Abandon this substring. */
name|quote_char
operator|=
literal|'\0'
expr_stmt|;
...
condition|)
block|{
comment|/* We didn't find an unclosed quoted substring upon which to do 	     completion, so use the word break characters to find the 	     substring on which to do completion. */
while|while
condition|(
operator|--
name|rl_point
...
name|some_matches
label|:
comment|/* It seems to me that in all the cases we handle we would like 	 to ignore duplicate possibilities.  Scan for the text to 	 insert being identical to the other completions. */
if|if
condition|(
name|rl_ignore_completion_duplicates
condition|)
...
condition|)
block|{
comment|/* Found an embedded word break character in a potential 			 match, so need to prepend a quote character if we are 			 replacing the completion string. */
name|replacement
operator|=
operator|(
name|char
...
name|VI_MODE
argument_list|)
comment|/* Vi mode always inserts a space befoe yanking the argument, and it      inserts it right *after* rl_point. */
if|if
condition|(
name|rl_editing_mode
operator|==
...
expr_stmt|;
block|}
comment|/* At this point we should entertain the possibility of hacking wildcarded      filenames, like /usr/man/man<WILD>/te<TAB>.  If the directory name      contains globbing characters, then build an array of directories to      glob on, and glob on the first one. */
...
begin_comment
comment|/* Don't know what to do, but this is a guess */
end_comment
...
name|NOTDEF
argument_list|)
comment|/* What state is the terminal in?  I'll tell you: 	     non-determinate!  That means we cannot do any output. */
name|ding
argument_list|()
expr_stmt|;
endif|#
...
begin_comment
comment|/* Return pointer to first occurance in STRING1 of any character from STRING2,    or NULL if no occurance found. */
end_comment
