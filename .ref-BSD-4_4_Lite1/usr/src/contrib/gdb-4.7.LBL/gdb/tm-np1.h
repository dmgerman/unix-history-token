...
begin_comment
comment|/* Parameters for targeting on a Gould NP1, for GDB, the GNU debugger.    Copyright (C) 1986, 1987, 1989, 1991 Free Software Foundation, Inc.  This file is part of GDB.  This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with this program; if not, write to the Free Software Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* N_ENTRY appears in libraries on Gould machines.    Don't know what 0xa4 is; it's mentioned in stab.h    but only in the sdb symbol list.  */
end_comment
...
begin_comment
comment|/* Address of blocks in N_LBRAC and N_RBRAC symbols are absolute addresses,    not relative to start of source address.  */
end_comment
...
begin_comment
comment|/* Sequence of bytes for breakpoint instruction.    This is padded out to the size of a machine word.  When it was just    {0x28, 0x09} it gave problems if hit breakpoint on returning from a    function call.  */
end_comment
...
begin_comment
comment|/* Gr2 => return value from function */
end_comment
...
parameter_list|)
define|\
value|((N)> VE_REGNUM ? builtin_type_np1_vector : builtin_type_int)
end_define
...
parameter_list|)
define|\
value|bcopy (((int *)(REGBUF)) + 2, VALBUF, TYPE_LENGTH (TYPE))
end_define
...
parameter_list|)
define|\
value|write_register_bytes (REGISTER_BYTE (R2_REGNUM), VALBUF,      \ 			      TYPE_LENGTH (TYPE))
end_define
...
begin_comment
comment|/* Both gcc and cc return small structs in registers (i.e. in GDB    terminology, small structs don't use the struct return convention).  */
end_comment
...
define|\
value|{ register CORE_ADDR sp = read_register (SP_REGNUM); \   register int regnum;                               \   for (regnum = 0; regnum< FP_REGNUM; regnum++)     \     sp = push_word (sp, read_register (regnum));     \   sp = push_word (sp, read_register (PS_REGNUM));    \   sp = push_word (sp, read_register (PC_REGNUM));    \   write_register (SP_REGNUM, sp);}
end_define
...
name|POP_FRAME
define|\
value|{ CORE_ADDR sp = read_register(SP_REGNUM);             \   REGISTER_TYPE reg;                                   \   int regnum;                                          \   for(regnum = 0;regnum< FP_REGNUM;regnum++){         \     sp-=sizeof(REGISTER_TYPE);                         \     read_memory(sp,&reg,sizeof(REGISTER_TYPE));        \     write_register(regnum,reg);}                       \   sp-=sizeof(REGISTER_TYPE);                           \   read_memory(sp,&reg,sizeof(REGISTER_TYPE));          \   write_register(PS_REGNUM,reg);                       \   sp-=sizeof(REGISTER_TYPE);                           \   read_memory(sp,&reg,sizeof(REGISTER_TYPE));          \   write_register(PC_REGNUM,reg);}
end_define

begin_comment
comment|/* MJD - Size of dummy frame pushed onto stack by PUSH_DUMMY_FRAME */
end_comment
...
directive|define
name|CALL_DUMMY
value|{0x00000000,  \ 		    0x00000000,  \ 		    0x59000000,  \ 		    0x598a0000,  \ 		    0xb5030008,  \ 		    0x5c820000,  \ 		    0x44810008,  \ 		    0x00000000,  \ 		    0x590a0000,  \ 		    0x28090000 }
end_define
