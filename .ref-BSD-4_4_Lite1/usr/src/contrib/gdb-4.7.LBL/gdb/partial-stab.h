...
argument_list|()
expr_stmt|;
comment|/* Check for __DYNAMIC, which is used by Sun shared libraries.  	     Record it even if it's local, not global, so we can find it. 	     Same with virtual function tables, both global and static.  */
if|if
condition|(
operator|(
name|namestring
...
expr_stmt|;
continue|continue;
comment|/* Global functions were ignored here, but now they 	         are put into the global psymtab like one would expect. 		 They're also in the misc fn vector...  		 FIXME, why did it used to ignore these?  That broke 		 "i fun" on these functions.  */
case|case
literal|'F'
case|:
ifdef|#
