...
include|#
directive|include
file|"nindy-share/ttycntl.h"
end_include

begin_include
include|#
directive|include
file|"nindy-share/demux.h"
end_include
...
begin_comment
comment|/* Descriptor for I/O to NINDY  */
end_comment
...
begin_comment
comment|/* Restore TTY to normal operation */
end_comment
...
begin_comment
comment|/* TTY attributes before entering passthrough */
end_comment
...
begin_comment
comment|/* Signal handlers before entering passthrough */
end_comment
...
begin_comment
comment|/* Clean up anything that needs cleaning when losing control.  */
end_comment
...
begin_comment
comment|/* Open a connection to a remote debugger.       FIXME, there should be a way to specify the various options that are    now specified with gdb command-line options.  (baud_rate, old_protocol,    and initial_brk)  */
end_comment
...
name|printf
argument_list|(
literal|"\tAttached to %s at %s bps%s%s.\n"
argument_list|,
name|savename
argument_list|,
name|baud_rate
...
begin_comment
comment|/******************************************************************************  * remote_load:  *	Download an object file to the remote system by invoking the "comm960"  *	utility.  We look for "comm960" in $G960BIN, $G960BASE/bin, and  *	DEFAULT_BASE/bin/HOST/bin where  *		DEFAULT_BASE is defined in env.h, and  *		HOST must be defined on the compiler invocation line.  ******************************************************************************/
end_comment
...
begin_comment
comment|/* Wait until the remote machine stops. While waiting, operate in passthrough  * mode; i.e., pass everything NINDY sends to stdout, and everything from  * stdin to NINDY.  *  * Return to caller, storing status in 'status' just as `wait' would.  */
end_comment
...
name|DEMUX_DECL
expr_stmt|;
comment|/* OS-dependent data needed by DEMUX... macros */
name|char
name|buf
index|[
literal|500
index|]
decl_stmt|;
comment|/* FIXME, what is "500" here? */
name|int
name|i
decl_stmt|,
name|n
...
comment|/* Save current tty attributes, set up signals to restore them. 	 */
name|ioctl
argument_list|(
literal|0
argument_list|,
...
condition|)
block|{
comment|/* Go to sleep until there's something for us on either 		 * the remote port or stdin. 		 */
name|DEMUX_WAIT
argument_list|(
name|nindy_fd
argument_list|)
expr_stmt|;
comment|/* Pass input through to correct place */
name|n
operator|=
name|DEMUX_READ
argument_list|(
...
comment|/* Write out any characters in buffer preceding DLE */
name|i
operator|=
name|non_dle
argument_list|(
...
condition|)
block|{
comment|/* There *was* a DLE in the buffer */
name|stop_exit
operator|=
name|ninStopWhy
argument_list|(
...
name|TRACE_STEP
case|:
comment|/* Make it look like a VAX trace trap */
name|stop_code
operator|=
name|SIGTRAP
expr_stmt|;
break|break;
default|default:
comment|/* The target is not running Unix, and its 			   faults/traces do not map nicely into Unix signals. 			   Make sure they do not get confused with Unix signals 			   by numbering them with values higher than the highest 			   legal Unix signal.  code in i960_print_fault(), 			   called via PRINT_RANDOM_SIGNAL, will interpret the 			   value.  */
name|stop_code
operator|+=
name|NSIG
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Float regs.  Only works on IEEE_FLOAT hosts.  */
for|for
control|(
name|regnum
operator|=
...
expr_stmt|;
comment|/* dub now in host byte order */
comment|/* FIXME-someday, the arguments to unpack_double are backward.        It expects a target double and returns a host; we pass the opposite.        This mostly works but not quite.  */
name|dub
operator|=
name|unpack_double
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* dub now in target byte order */
name|bcopy
argument_list|(
operator|(
name|char
...
begin_comment
comment|/* The data cache records all the data read from the remote machine    since the last time it stopped.     Each cache block holds 16 bytes of data    starting at a multiple-of-16 address.  */
end_comment
