...
name|valaddr
decl_stmt|;
comment|/* SWAP_TARGET_AND_HOST (&retval, sizeof (unsigned char)); */
return|return
name|retval
return|;
block|}
...
name|error
argument_list|(
literal|"That operation is not possible on an integer of that size."
argument_list|)
expr_stmt|;
block|}
block|}
...
name|retval
decl_stmt|;
comment|/* plain chars might be unsigned on host */
name|memcpy
argument_list|(
operator|&
name|retval
...
argument_list|)
expr_stmt|;
comment|/* Move the `this' pointer according to the offset. */
name|VALUE_OFFSET
argument_list|(
operator|*
name|arg1p
...
directive|endif
comment|/* Mask out old value, while avoiding shifts>= longword size */
if|if
condition|(
name|bitsize
operator|<
...
argument_list|)
decl_stmt|;
comment|/* FIXME, we assume that pointers have the same form and byte order as      integers, and that all pointers have the same form.  */
if|if
condition|(
name|code
operator|==
...
parameter_list|)
define|\
value|(!((gcc_p)&& (TYPE_LENGTH (value_type) == 1                \ 		 || TYPE_LENGTH (value_type) == 2             \ 	         || TYPE_LENGTH (value_type) == 4             \ 		 || TYPE_LENGTH (value_type) == 8             \ 		 )                                            \      ))
end_define
