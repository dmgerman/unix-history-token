...
begin_comment
comment|/* Define to 1 to compile in some maintenance stuff */
end_comment
...
begin_comment
comment|/* Kludge to get frame pointer register number */
end_comment
...
begin_comment
comment|/* This value is added to each symbol value.  FIXME:  Generalize to     the section_offsets structure used by dbxread.  */
end_comment
...
begin_comment
comment|/*  LOCAL FUNCTION  	record_minimal_symbol -- add entry to gdb's minimal symbol table  SYNOPSIS  	static void record_minimal_symbol (char *name, CORE_ADDR address, 					  enum minimal_symbol_type ms_type, 					  struct objfile *objfile)  DESCRIPTION  	Given a pointer to the name of a symbol that should be added to the 	minimal symbol table, and the address associated with that 	symbol, records this information for later use in building the 	minimal symbol table.   */
end_comment
...
name|stderr
argument_list|,
literal|"warning: DWARF ref 0x%x: "
argument_list|,
name|curdie
operator|->
name|die_ref
...
name|dwarfwarn
argument_list|(
literal|"reference to DIE (0x%x) outside compilation unit"
argument_list|,
name|die_ref
argument_list|)
expr_stmt|;
...
argument_list|(
operator|(
literal|"internal error: dup user type allocation"
operator|)
argument_list|)
expr_stmt|;
block|}
...
argument_list|(
operator|(
literal|"missing class, structure, or union tag"
operator|)
argument_list|)
expr_stmt|;
break|break;
...
argument_list|(
operator|(
literal|"bad array element type attribute 0x%x"
operator|,
name|attribute
operator|)
argument_list|)
...
argument_list|(
operator|(
literal|"array subscripts must be integral types, not type 0x%x"
operator|,
name|fundtype
operator|)
argument_list|)
...
argument_list|(
operator|(
literal|"array subscript format 0x%x not handled yet"
operator|,
name|format
operator|)
argument_list|)
...
argument_list|(
operator|(
literal|"unknown array subscript format %x"
operator|,
name|format
operator|)
argument_list|)
...
argument_list|(
operator|(
literal|"array not row major; not handled correctly"
operator|)
argument_list|)
expr_stmt|;
block|}
...
comment|/* Actually, we compute the value as if register has 0 */
name|offreg
operator|=
literal|1
expr_stmt|;
...
argument_list|(
operator|(
literal|"OP_DEREF2 address 0x%x not handled"
operator|,
name|stack
index|[
name|stacki
...
argument_list|(
operator|(
literal|"OP_DEREF4 address 0x%x not handled"
operator|,
name|stack
index|[
name|stacki
...
begin_comment
comment|/*  LOCAL FUNCTION  	read_ofile_symtab -- build a full symtab entry from chunk of DIE's  SYNOPSIS  	static struct symtab *read_ofile_symtab (struct partial_symtab *pst)  DESCRIPTION  	When expanding a partial symbol table entry to a full symbol table 	entry, this is the function that gets called to read in the symbols 	for the compilation unit.  	Returns a pointer to the newly constructed symtab (which is now 	the new first one on the objfile's symtab list).  */
end_comment
...
argument_list|(
operator|(
literal|"botched modified type decoding (mtype 0x%x)"
operator|,
name|mtype
operator|)
argument_list|)
...
argument_list|(
operator|(
literal|"type modifier 'const' ignored"
operator|)
argument_list|)
expr_stmt|;
comment|/* FIXME */
...
argument_list|(
operator|(
literal|"type modifier 'volatile' ignored"
operator|)
argument_list|)
expr_stmt|;
comment|/* FIXME */
...
argument_list|(
operator|(
literal|"unknown type modifier %u"
operator|,
operator|(
name|unsigned
name|char
...
argument_list|(
operator|(
literal|"unexpected fundamental type 0x%x"
operator|,
name|fundtype
operator|)
argument_list|)
...
name|dwarfwarn
argument_list|(
literal|"malformed DIE, bad length (%d bytes)"
argument_list|,
name|dip
operator|->
name|die_length
...
argument_list|(
operator|(
literal|"unknown attribute length, skipped remaining attributes"
operator|)
argument_list|)
expr_stmt|;
empty_stmt|;
...
argument_list|(
operator|(
literal|"unknown attribute form (0x%x)"
operator|,
name|form
operator|)
argument_list|)
...
name|dwarfwarn
argument_list|(
literal|"no bfd support for %d byte data object"
argument_list|,
name|nbytes
argument_list|)
expr_stmt|;
