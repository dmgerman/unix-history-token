...
begin_comment
comment|/* The follow stuff is an abstract data type "bpstat" ("breakpoint status").    This provides the ability to determine whether we have stopped at a    breakpoint, and what we should do about it.  */
end_comment
...
begin_comment
comment|/* Get a bpstat associated with having just stopped at address *PC    and frame address FRAME_ADDRESS.  Update *PC to point at the    breakpoint (if we hit a breakpoint).  */
end_comment
...
begin_comment
comment|/* Nonzero if we should print the frame.  */
end_comment
...
name|bs
parameter_list|)
value|((bs) != NULL&& (bs)->print)
end_define

begin_comment
comment|/* Nonzero if we should stop.  */
end_comment
...
parameter_list|(
comment|/* bpstat, breakpoint */
parameter_list|)
function_decl|;
end_function_decl

begin_comment
comment|/* Nonzero if we hit a momentary breakpoint.  */
end_comment

begin_define
define|#
directive|define
name|bpstat_momentary_breakpoint
parameter_list|(
name|bs
parameter_list|)
value|((bs) != NULL&& (bs)->momentary)
end_define
...
begin_comment
comment|/* Currently that is true iff we have hit a breakpoint.  */
end_comment
...
begin_comment
comment|/* Perform actions associated with having stopped at *BSP.  */
end_comment
...
name|old_val
decl_stmt|;
comment|/* Nonzero if we should print the frame.  Only significant for the first      bpstat in the chain.  */
name|char
name|print
decl_stmt|;
comment|/* Nonzero if we should stop.  Only significant for the first bpstat in      the chain.  */
name|char
name|stop
decl_stmt|;
comment|/* Nonzero if we hit a momentary breakpoint.  Only significant for the      first bpstat in the chain.  */
name|char
name|momentary
decl_stmt|;
block|}
...
name|address
decl_stmt|;
comment|/* Line number of this address.  Redundant.  Only matters if address      is non-NULL.  */
name|int
name|line_number
decl_stmt|;
comment|/* Symtab of file of this address.  Redundant.  Only matters if address      is non-NULL.  */
name|struct
name|symtab
modifier|*
name|symtab
