...
init|=
block|{
literal|"block at 0x%x out of order"
block|,
literal|0
block|,
literal|0
...
begin_comment
comment|/* Find a symbol on a pending list.  */
end_comment
...
begin_comment
comment|/* Start recording information about source code that came from an included    (or otherwise merged-in) source file with a different name.  */
end_comment
...
begin_comment
comment|/* Manage the vector of line numbers for each subfile.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* Debug: if context stack still has something in it, 	 we are in trouble.  */
if|if
condition|(
name|context_stack_depth
operator|>
...
do|;
block|}
comment|/* Cleanup any undefined types that have been left hanging around      (this needs to be done before the finish_blocks so that      file_symbols is still good).      FIXME:  Stabs specific. */
...
directive|ifdef
name|IBM6000_TARGET
comment|/* In case we need to duplicate symbol tables (to represent include 	     files), and in case our system needs relocation, we want to 	     relocate the main symbol table node only (for the main file, 	     not for the include files). */
name|symtab
operator|->
name|nonreloc
operator|=
...
directive|ifdef
name|IBM6000_TARGET
comment|/* all include symbol tables are non-relocatable, except the main source      file's. */
if|if
condition|(
name|symtab
condition|)
