...
argument_list|)
expr_stmt|;
comment|/* Force output out, print_insn not using _filtered */
name|next_address
operator|=
name|VALUE_ADDRESS
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* If no size is specified, try to guess a reasonable one from the type. */
if|if
condition|(
name|size
operator|==
...
condition|)
block|{
comment|/* XXX This array is machine dependent. (FIXME) */
specifier|static
name|unsigned
name|char
name|sizetab
...
condition|)
block|{
comment|/* ok, we're going to have to get fancy here.  Assumption: a          long is four bytes.  FIXME.  */
name|unsigned
name|long
name|v1
decl_stmt|,
...
operator|==
name|LITTLE_ENDIAN
comment|/* Swap the two for printing */
block|{
name|unsigned
name|long
name|tmp
...
begin_comment
comment|/* Optionally print address ADDR symbolically as<SYMBOL+OFFSET> on STREAM,    after LEADIN.  Print nothing if no symbolic name is found nearby.    DO_DEMANGLE controls whether to print a symbol in its native "raw" form,    or to interpret it as a possible C++ name and convert it back to source    form. */
end_comment
...
name|printf
argument_list|(
literal|"a local variable at offset %ld from register %s"
argument_list|,
name|val
argument_list|,
name|reg_names
...
begin_comment
comment|/* Print arguments of a stack frame that have debugging symbols. */
end_comment

begin_function
specifier|static
name|int
name|print_frame_named_args
parameter_list|(
name|func
parameter_list|,
name|fi
...
operator|)
expr_stmt|;
comment|/* If this is the highest offset seen yet, remember it.  */
if|if
condition|(
name|current_offset
operator|>
...
continue|continue;
block|}
comment|/* We have to re-look-up the symbol because arguments often have 	 two entries (one a parameter, one a register or local), and the one 	 we want is the non-parm, which lookup_symbol will find for 	 us.  After this, sym could be any SYMBOL_CLASS...  */
ifdef|#
directive|ifdef
name|IBM6000_TARGET
comment|/* AIX/RS6000 implements a concept of traceback tables, in which case          it creates nameless parameters. Looking for those parameter symbols          will result in an error. */
if|if
condition|(
operator|*
name|SYMBOL_NAME
...
literal|0
condition|)
comment|/* If we don't have a debugging symbol for this routine, and we        can't tell how many args there are, we should at least print         something useful */
name|num
operator|=
name|default_funargs
expr_stmt|;
...
begin_comment
comment|/* Print nameless args on STREAM.    ARGSADDR is the address of the arglist, START is the offset    of the first nameless arg, and NUM is the number of nameless args to    print.  FIRST is nonzero if this is the first argument (not just    the first nameless arg).  */
end_comment
...
block|}
block|}
comment|/* There is not a standard way to make a va_list, so we need      to do various things for different systems.  */
if|#
directive|if
name|defined
argument_list|(
...
begin_comment
comment|/*  * A command to dump out memory as an array of chars.  Output is  * much easier to read than "x /c".  */
end_comment

begin_function
specifier|static
name|void
name|chardump_command
parameter_list|(
name|arg
parameter_list|,
name|from_tty
...
name|error
argument_list|(
literal|"Must supply address argument."
argument_list|)
expr_stmt|;
name|cp
operator|=
...
argument_list|)
expr_stmt|;
comment|/* XXX Should take length arg as an expression. */
name|len
operator|=
operator|(
name|cp
...
name|call_command
argument_list|,
literal|"Call a function in the inferior process.\n\ The argument is the function name and arguments, in the notation of the\n\ current working language.  The result is printed and saved in the value\n\ history, if it is not void."
argument_list|)
expr_stmt|;
name|add_cmd
argument_list|(
...
name|chardump_command
argument_list|,
literal|"Dump out a range of memory as characters.  Two arguments are starting\n\ address (as an arbitrary expression) and constant integer length."
argument_list|)
expr_stmt|;
...
name|asm_symbolic
argument_list|,
literal|"Set printing of symbolic offsets in disassmebly listings"
argument_list|,
operator|&
name|setprintlist
argument_list|)
