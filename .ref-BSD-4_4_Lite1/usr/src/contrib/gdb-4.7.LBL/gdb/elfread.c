...
begin_comment
comment|/************************************************************************  *									*  *				NOTICE					*  *									*  * This file is still under construction.  When it is complete, this	*  * notice will be removed.  Until then, direct any questions or changes	*  * to Fred Fish at Cygnus Support (fnf@cygnus.com)			*  *									*   * FIXME	Still needs support for shared libraries.		*  * FIXME	Still needs support for core files.			*  * FIXME	The ".debug" and ".line" section names are hardwired.	*  *									*  ************************************************************************/
end_comment
...
begin_endif
unit|static void record_minimal_symbol PARAMS ((char *, CORE_ADDR, enum minimal_symbol_type, 			       struct objfile *));
endif|#
directive|endif
end_endif
...
begin_comment
comment|/*  LOCAL FUNCTION  	record_minimal_symbol -- add entry to minimal symbol table  SYNOPSIS  	static void record_minimal_symbol (char *name, CORE_ADDR address)  DESCRIPTION  	Given a pointer to the name of a symbol that should be added to the 	minimal symbol table and the address associated with that symbol, records 	this information for later use in building the minimal symbol table.   */
end_comment
...
begin_endif
unit|static void record_minimal_symbol (name, address, ms_type, objfile)      char *name;      CORE_ADDR address;      enum minimal_symbol_type ms_type;      struct objfile *objfile; {   name = obsavestring (name, strlen (name),&objfile -> symbol_obstack);   prim_record_minimal_symbol (name, address, ms_type); }
endif|#
directive|endif
end_endif
...
name|ms_type
decl_stmt|;
comment|/* If sectinfo is nonzero, it contains section info that should end up      filed in the objfile.  */
name|struct
...
index|]
expr_stmt|;
comment|/* Select global/weak symbols that are defined in a specific section. 	     Note that bfd now puts abs symbols in their own section, so 	     all symbols we are interested in will have a section. */
if|if
condition|(
operator|(
name|sym
...
block|{
comment|/* FIXME:  Solaris2 shared libraries include lots of 		     odd "absolute" and "undefined" symbols, that play  		     hob with actions like finding what function the PC 		     is in.  Ignore them if they aren't text or data.  */
...
expr_stmt|;
block|}
comment|/* See if this is a debugging symbol that helps Solaris 	     stabs-in-elf debugging.  */
elseif|else
if|if
condition|(
name|sym
...
condition|)
block|{
comment|/* Chain any old one onto the objfile; remember new sym.  */
if|if
condition|(
name|sectinfo
condition|)
...
condition|)
block|{
comment|/* Named Local variable in a Data section.  Check its name.  */
name|index
operator|=
operator|-
literal|1
...
condition|)
block|{
comment|/* We have some new info.  Allocate a sectinfo, if 		     needed, and fill it in.  */
if|if
condition|(
operator|!
name|sectinfo
...
argument_list|)
expr_stmt|;
comment|/* Process the normal ELF symbol table first.  This may write some       chain of info into the dbx_symfile_info in objfile->sym_private,      which can later be used by elfstab_offset_sections.  */
...
begin_comment
comment|/* This cleans up the objfile's sym_private pointer, and the chain of    stab_section_info's, that might be dangling from it.  */
end_comment
