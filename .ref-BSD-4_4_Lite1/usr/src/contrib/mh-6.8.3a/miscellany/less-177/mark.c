...
begin_comment
comment|/*  * The table of marks.  * Each mark is identified by a lowercase or uppercase letter.  */
end_comment
...
begin_comment
comment|/*  * Special mark for the "last mark"; addressed by the apostrophe.  */
end_comment
...
expr_stmt|;
block|}
comment|/* 	 * If we're using lmark, we must save the screen position now, 	 * because if we call edit() below, lmark will change. 	 * (We save the screen position even if we're not using lmark.) 	 */
name|scrpos
operator|=
name|m
operator|->
