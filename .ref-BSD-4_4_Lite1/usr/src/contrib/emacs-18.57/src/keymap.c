...
begin_comment
comment|/* Keymap used for minibuffers with self-inserting space.  */
end_comment
...
begin_comment
comment|/* Keymap used for minibuffers when space does not self insert.  */
end_comment
...
begin_comment
comment|/* A char over 0200 in a key sequence    is equivalent to prefixing with this character.  */
end_comment
...
begin_comment
comment|/* Install a standard key binding at initialization time.    For example,      ndefkey (Vctl_x_map, Ctl ('X'), "exchange-point-and-mark");  */
end_comment
...
literal|"copy-keymap"
argument_list|,
argument|Fcopy_keymap
argument_list|,
argument|Scopy_keymap
argument_list|,
literal|1
argument_list|,
literal|1
...
literal|0
argument_list|,
literal|"Return a copy of the keymap KEYMAP.\n\ The copy starts out with the same definitions of KEYMAP,\n\ but changing either the copy or KEYMAP does not affect the other.\n\ Any key definitions that are subkeymaps are recursively copied."
argument_list|)
end_macro
...
begin_comment
comment|/* Value is number if KEY is too long; nil if valid but has no definition. */
end_comment
...
name|DEFUN
argument_list|(
literal|"current-global-map"
argument_list|,
argument|Fcurrent_global_map
argument_list|,
argument|Scurrent_global_map
argument_list|,
literal|0
argument_list|,
literal|0
...
literal|0
argument_list|,
literal|"Return the current global keymap."
argument_list|)
end_macro
...
argument_list|)
expr_stmt|;
comment|/* If the MAP is a vector, I increments and eventually reaches 0200. 	 Otherwise I remains 0; MAP is cdr'd and eventually becomes nil.  */
while|while
condition|(
operator|!
name|NULL
...
argument_list|)
expr_stmt|;
comment|/* End this iteration if this element does not match 	     the target.  */
if|if
condition|(
name|XTYPE
argument_list|(
...
condition|)
continue|continue;
comment|/* We have found a match. 	     Construct the key sequence where we found it.  */
name|dummy
operator|=
name|concat2
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Verify that this key binding is not shadowed 	     by another binding for the same key, 	     before we say it exists. 	     The mechanism: look for local definition of this key 	     and if it is defined and does not match what we found 	     then ignore this key. 	     Either nil or number as value from Flookup_key 	     means undefined.  */
if|if
condition|(
operator|!
name|NULL
...
continue|continue;
block|}
comment|/* It is a true unshadowed match  Record it.  */
if|if
condition|(
operator|!
name|NULL
...
begin_comment
comment|/* Insert a desription of the key bindings in STARTMAP,    followed by those of all maps reachable through STARTMAP.    If PARTIAL is nonzero, omit certain "uninteresting" commands    (such as `undefined').    If SHADOW is non-nil, don't mention keys which would be shadowed by it */
end_comment
...
begin_comment
comment|/* Describe the contents of map MAP, assuming that this map    itself is reached by the sequence of prefix keys STRING (a string).    PARTIAL and SHADOW are the same as in `describe_map_tree' above.  */
end_comment
