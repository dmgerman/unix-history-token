...
argument_list|)
expr_stmt|;
comment|/* Propagate error indication if worse than mere failure.  */
if|if
condition|(
name|val
operator|==
...
literal|2
return|;
comment|/* Return position on success.  */
if|if
condition|(
literal|0
operator|<=
...
name|dend2
decl_stmt|;
comment|/* Don't allow matching a register that hasn't been used. 	       This isn't fully reliable in the current version, 	       but it is better than crashing.  */
if|if
condition|(
operator|(
name|int
