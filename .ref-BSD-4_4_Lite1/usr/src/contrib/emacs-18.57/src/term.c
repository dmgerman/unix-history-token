...
begin_comment
comment|/* terminal control module for terminals described by TERMCAP    Copyright (C) 1985, 1986, 1987 Free Software Foundation, Inc.  This file is part of GNU Emacs.  GNU Emacs is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GNU Emacs is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU Emacs; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* Nonzero for "xs".  If set together with 			   TN_standout_width == 0, it means don't bother 			   to write any end-standout cookies.  */
end_comment
...
comment|/* If all N identical chars are too few, 		 don't even consider the last N-1, the last N-2,...  */
name|first_check
operator|=
name|p
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Always call CalcIDCosts because it allocates some vectors.      That function handles dont_calculate_costs.  */
if|if
condition|(
name|s
operator|&&
...
name|fatal
argument_list|(
literal|"Cannot open termcap database file.\n"
argument_list|)
expr_stmt|;
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* Obsolete name for "do" */
ifdef|#
directive|ifdef
name|VMS
comment|/* VMS puts a carriage return before each linefeed,      so it is not safe to use linefeeds.  */
if|if
condition|(
name|Down
operator|&&
...
comment|/* (Actually, "bs" is obsolete...) */
name|Left
operator|=
name|tgetstr
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Get screen size from system, or else from termcap.  */
...
directive|ifdef
name|VMS
comment|/* These capabilities commonly use ^J.      I don't know why, but sending them on VMS does not work;      it causes following spaces to be lost, sometimes.      For now, the simplest fix is to avoid using these capabilities ever.  */
if|if
condition|(
name|Down
operator|&&
...
literal|8
expr_stmt|;
comment|/* Turned off since /etc/termcap seems to have :ta= for most terminals    and newer termcap doc does not seem to say there is a default.   if (!Wcm.cm_tab)     Wcm.cm_tab = "\t"; */
if|if
condition|(
name|TS_standout_mode
operator|==
