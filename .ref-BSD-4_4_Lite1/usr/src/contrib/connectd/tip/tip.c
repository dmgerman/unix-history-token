begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1983 Regents of the University of California.  * All rights reserved.  The Berkeley software License Agreement  * specifies the terms and conditions for redistribution.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1983 Regents of the University of California.\n\  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
endif|not lint
end_endif

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)tip.c	5.4 (Berkeley) 4/3/86"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
endif|not lint
end_endif

begin_comment
comment|/*  * tip - UNIX link to other systems  *  tip [-v] system-name [-option ....]  * or  *  tip [-v] phone-number [-option ....]  * or  *  tip [-v] [-option ....]  */
end_comment

begin_include
include|#
directive|include
file|"tip.h"
end_include

begin_decl_stmt
name|int
name|disc
init|=
name|OTTYDISC
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* tip normally runs this way */
end_comment

begin_function_decl
name|int
name|intprompt
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|int
name|timeout
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|int
name|cleanup
parameter_list|()
function_decl|;
end_function_decl

begin_function_decl
name|char
modifier|*
name|sname
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|struct
name|phonenumberent
name|pent
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|PNbuf
index|[
literal|256
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* This limits the size of a number */
end_comment

begin_function_decl
specifier|extern
name|char
modifier|*
name|sprintf
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
name|int
name|dataset
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|optionbuf
index|[
literal|1024
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|connectdomain
name|cd
decl_stmt|;
end_decl_stmt

begin_function
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
name|char
modifier|*
name|system
init|=
name|NOSTR
decl_stmt|;
specifier|register
name|int
name|i
decl_stmt|;
specifier|register
name|char
modifier|*
name|p
decl_stmt|;
name|char
name|sbuf
index|[
literal|12
index|]
decl_stmt|;
specifier|register
name|char
modifier|*
name|optp
decl_stmt|;
if|if
condition|(
name|argc
operator|>
literal|4
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: tip [-v] [system or number] [-options ...]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|isatty
argument_list|(
literal|0
argument_list|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"tip: must be interactive\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|optp
operator|=
name|optionbuf
expr_stmt|;
for|for
control|(
init|;
name|argc
operator|>
literal|1
condition|;
name|argv
operator|++
operator|,
name|argc
operator|--
control|)
block|{
if|if
condition|(
name|argv
index|[
literal|1
index|]
index|[
literal|0
index|]
operator|!=
literal|'-'
condition|)
name|system
operator|=
name|argv
index|[
literal|1
index|]
expr_stmt|;
else|else
switch|switch
condition|(
name|argv
index|[
literal|1
index|]
index|[
literal|1
index|]
condition|)
block|{
case|case
literal|'v'
case|:
name|vflag
operator|++
expr_stmt|;
break|break;
default|default:
name|strcpy
argument_list|(
name|optp
argument_list|,
operator|&
name|argv
index|[
literal|1
index|]
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|optp
operator|+=
name|strlen
argument_list|(
operator|&
name|argv
index|[
literal|1
index|]
index|[
literal|1
index|]
argument_list|)
operator|+
literal|1
expr_stmt|;
break|break;
block|}
block|}
ifdef|#
directive|ifdef
name|notdef
comment|/*if (system == NOSTR) 		goto notnumber; 	if (isalpha(*system)) 		goto notnumber;*/
if|if
condition|()
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"tip: can't find phone number for %s\n"
argument_list|,
name|system
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|strncpy
argument_list|(
name|PNbuf
argument_list|,
name|system
argument_list|,
sizeof|sizeof
name|PNbuf
operator|-
literal|1
argument_list|)
expr_stmt|;
comment|/* 	 * System name is really a phone number... 	 * Copy the number then stomp on the original (in case the number 	 *	is private, we don't want 'ps' or 'w' to find it). 	 */
if|if
condition|(
name|strlen
argument_list|(
name|system
argument_list|)
operator|>
sizeof|sizeof
name|PNbuf
operator|-
literal|1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"tip: phone number too long (max = %d bytes)\n"
argument_list|,
sizeof|sizeof
name|PNbuf
operator|-
literal|1
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|strncpy
argument_list|(
name|PNbuf
argument_list|,
name|system
argument_list|,
sizeof|sizeof
name|PNbuf
operator|-
literal|1
argument_list|)
expr_stmt|;
for|for
control|(
name|p
operator|=
name|system
init|;
operator|*
name|p
condition|;
name|p
operator|++
control|)
operator|*
name|p
operator|=
literal|'\0'
expr_stmt|;
name|PN
operator|=
name|PNbuf
expr_stmt|;
name|system
operator|=
name|sprintf
argument_list|(
name|sbuf
argument_list|,
literal|"tip%d"
argument_list|,
name|BR
argument_list|)
expr_stmt|;
name|notnumber
label|:
endif|#
directive|endif
name|strcpy
argument_list|(
name|cd
operator|.
name|cd_address
argument_list|,
name|system
argument_list|)
expr_stmt|;
name|cd
operator|.
name|cd_alen
operator|=
name|strlen
argument_list|(
name|cd
operator|.
name|cd_address
argument_list|)
operator|+
literal|1
expr_stmt|;
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|cleanup
argument_list|)
expr_stmt|;
name|signal
argument_list|(
name|SIGQUIT
argument_list|,
name|cleanup
argument_list|)
expr_stmt|;
name|signal
argument_list|(
name|SIGHUP
argument_list|,
name|cleanup
argument_list|)
expr_stmt|;
name|signal
argument_list|(
name|SIGTERM
argument_list|,
name|cleanup
argument_list|)
expr_stmt|;
name|setbuf
argument_list|(
name|stdout
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|vinit
argument_list|()
expr_stmt|;
comment|/* init variables */
name|setparity
argument_list|(
literal|"even"
argument_list|)
expr_stmt|;
comment|/* set the parity table */
if|if
condition|(
operator|(
name|FD
operator|=
name|externalconnect
argument_list|(
operator|&
name|cd
argument_list|,
name|optionbuf
argument_list|,
name|optp
operator|-
name|optionbuf
argument_list|,
literal|2
argument_list|)
operator|)
operator|<=
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"\07%s\n[EOT]\n"
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|dataset
operator|=
literal|1
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCGETP
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|defarg
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCGETC
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|defchars
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCGLTC
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|deflchars
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCGETD
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|odisc
argument_list|)
expr_stmt|;
name|arg
operator|=
name|defarg
expr_stmt|;
name|arg
operator|.
name|sg_flags
operator|=
name|ANYP
operator||
name|CBREAK
expr_stmt|;
name|tchars
operator|=
name|defchars
expr_stmt|;
name|tchars
operator|.
name|t_intrc
operator|=
name|tchars
operator|.
name|t_quitc
operator|=
operator|-
literal|1
expr_stmt|;
name|ltchars
operator|=
name|deflchars
expr_stmt|;
name|ltchars
operator|.
name|t_suspc
operator|=
name|ltchars
operator|.
name|t_dsuspc
operator|=
name|ltchars
operator|.
name|t_flushc
operator|=
name|ltchars
operator|.
name|t_lnextc
operator|=
operator|-
literal|1
expr_stmt|;
name|raw
argument_list|()
expr_stmt|;
name|pipe
argument_list|(
name|fildes
argument_list|)
expr_stmt|;
name|pipe
argument_list|(
name|repdes
argument_list|)
expr_stmt|;
name|signal
argument_list|(
name|SIGALRM
argument_list|,
name|timeout
argument_list|)
expr_stmt|;
comment|/* 	 * Everything's set up now: 	 *	connection established (hardwired or dialup) 	 *	line conditioned (baud rate, mode, etc.) 	 *	internal data structures (variables) 	 * so, fork one process for local side and one for remote. 	 */
name|printf
argument_list|(
name|cumode
condition|?
literal|"Connected\r\n"
else|:
literal|"\07connected\r\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|pid
operator|=
name|fork
argument_list|()
condition|)
name|tipin
argument_list|()
expr_stmt|;
else|else
name|tipout
argument_list|()
expr_stmt|;
comment|/*NOTREACHED*/
block|}
end_function

begin_macro
name|cleanup
argument_list|()
end_macro

begin_block
block|{
if|if
condition|(
name|odisc
condition|)
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETD
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|odisc
argument_list|)
expr_stmt|;
name|unraw
argument_list|()
expr_stmt|;
name|externalfinish
argument_list|(
name|FD
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * put the controlling keyboard into raw mode  */
end_comment

begin_macro
name|raw
argument_list|()
end_macro

begin_block
block|{
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETP
argument_list|,
operator|&
name|arg
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETC
argument_list|,
operator|&
name|tchars
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSLTC
argument_list|,
operator|&
name|ltchars
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETD
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|disc
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * return keyboard to normal mode  */
end_comment

begin_macro
name|unraw
argument_list|()
end_macro

begin_block
block|{
if|if
condition|(
operator|!
name|dataset
condition|)
return|return;
comment|/* premature death blasts unset tty state */
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETD
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|odisc
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETP
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|defarg
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSETC
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|defchars
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSLTC
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|deflchars
argument_list|)
expr_stmt|;
block|}
end_block

begin_decl_stmt
specifier|static
name|jmp_buf
name|promptbuf
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Print string ``s'', then read a string  *  in from the terminal.  Handles signals& allows use of  *  normal erase and kill characters.  */
end_comment

begin_macro
name|prompt
argument_list|(
argument|s
argument_list|,
argument|p
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|s
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|char
modifier|*
name|p
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
modifier|*
name|b
init|=
name|p
decl_stmt|;
name|int
argument_list|(
operator|*
name|oint
argument_list|)
argument_list|()
decl_stmt|,
argument_list|(
operator|*
name|oquit
argument_list|)
argument_list|()
decl_stmt|;
name|stoprompt
operator|=
literal|0
expr_stmt|;
name|oint
operator|=
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|intprompt
argument_list|)
expr_stmt|;
name|oint
operator|=
name|signal
argument_list|(
name|SIGQUIT
argument_list|,
name|SIG_IGN
argument_list|)
expr_stmt|;
name|unraw
argument_list|()
expr_stmt|;
name|printf
argument_list|(
literal|"%s"
argument_list|,
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
name|setjmp
argument_list|(
name|promptbuf
argument_list|)
operator|==
literal|0
condition|)
while|while
condition|(
operator|(
operator|*
name|p
operator|=
name|getchar
argument_list|()
operator|)
operator|!=
name|EOF
operator|&&
operator|*
name|p
operator|!=
literal|'\n'
condition|)
name|p
operator|++
expr_stmt|;
operator|*
name|p
operator|=
literal|'\0'
expr_stmt|;
name|raw
argument_list|()
expr_stmt|;
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|oint
argument_list|)
expr_stmt|;
name|signal
argument_list|(
name|SIGQUIT
argument_list|,
name|oint
argument_list|)
expr_stmt|;
return|return
operator|(
name|stoprompt
operator|||
name|p
operator|==
name|b
operator|)
return|;
block|}
end_block

begin_comment
comment|/*  * Interrupt service routine during prompting  */
end_comment

begin_macro
name|intprompt
argument_list|()
end_macro

begin_block
block|{
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|SIG_IGN
argument_list|)
expr_stmt|;
name|stoprompt
operator|=
literal|1
expr_stmt|;
name|printf
argument_list|(
literal|"\r\n"
argument_list|)
expr_stmt|;
name|longjmp
argument_list|(
name|promptbuf
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_block

begin_comment
comment|/*  * ****TIPIN   TIPIN****  */
end_comment

begin_macro
name|tipin
argument_list|()
end_macro

begin_block
block|{
name|char
name|gch
decl_stmt|,
name|bol
init|=
literal|1
decl_stmt|;
comment|/* 	 * Kinda klugey here... 	 *   check for scripting being turned on from the .tiprc file, 	 *   but be careful about just using setscript(), as we may 	 *   send a SIGEMT before tipout has a chance to set up catching 	 *   it; so wait a second, then setscript() 	 */
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|SCRIPT
argument_list|)
argument_list|)
condition|)
block|{
name|sleep
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|setscript
argument_list|()
expr_stmt|;
block|}
while|while
condition|(
literal|1
condition|)
block|{
name|gch
operator|=
name|getchar
argument_list|()
operator|&
literal|0177
expr_stmt|;
if|if
condition|(
operator|(
name|gch
operator|==
name|character
argument_list|(
name|value
argument_list|(
name|ESCAPE
argument_list|)
argument_list|)
operator|)
operator|&&
name|bol
condition|)
block|{
if|if
condition|(
operator|!
operator|(
name|gch
operator|=
name|escape
argument_list|()
operator|)
condition|)
continue|continue;
block|}
elseif|else
if|if
condition|(
operator|!
name|cumode
operator|&&
name|gch
operator|==
name|character
argument_list|(
name|value
argument_list|(
name|RAISECHAR
argument_list|)
argument_list|)
condition|)
block|{
name|boolean
argument_list|(
name|value
argument_list|(
name|RAISE
argument_list|)
argument_list|)
operator|=
operator|!
name|boolean
argument_list|(
name|value
argument_list|(
name|RAISE
argument_list|)
argument_list|)
expr_stmt|;
continue|continue;
block|}
elseif|else
if|if
condition|(
name|gch
operator|==
literal|'\r'
condition|)
block|{
name|bol
operator|=
literal|1
expr_stmt|;
name|pwrite
argument_list|(
name|FD
argument_list|,
operator|&
name|gch
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|HALFDUPLEX
argument_list|)
argument_list|)
condition|)
name|printf
argument_list|(
literal|"\r\n"
argument_list|)
expr_stmt|;
continue|continue;
block|}
elseif|else
if|if
condition|(
operator|!
name|cumode
operator|&&
name|gch
operator|==
name|character
argument_list|(
name|value
argument_list|(
name|FORCE
argument_list|)
argument_list|)
condition|)
name|gch
operator|=
name|getchar
argument_list|()
operator|&
literal|0177
expr_stmt|;
name|bol
operator|=
name|any
argument_list|(
name|gch
argument_list|,
name|value
argument_list|(
name|EOL
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|RAISE
argument_list|)
argument_list|)
operator|&&
name|islower
argument_list|(
name|gch
argument_list|)
condition|)
name|gch
operator|=
name|toupper
argument_list|(
name|gch
argument_list|)
expr_stmt|;
name|pwrite
argument_list|(
name|FD
argument_list|,
operator|&
name|gch
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|boolean
argument_list|(
name|value
argument_list|(
name|HALFDUPLEX
argument_list|)
argument_list|)
condition|)
name|printf
argument_list|(
literal|"%c"
argument_list|,
name|gch
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Escape handler --  *  called on recognition of ``escapec'' at the beginning of a line  */
end_comment

begin_macro
name|escape
argument_list|()
end_macro

begin_block
block|{
specifier|register
name|char
name|gch
decl_stmt|;
specifier|register
name|esctable_t
modifier|*
name|p
decl_stmt|;
name|char
name|c
init|=
name|character
argument_list|(
name|value
argument_list|(
name|ESCAPE
argument_list|)
argument_list|)
decl_stmt|;
specifier|extern
name|esctable_t
name|etable
index|[]
decl_stmt|;
name|gch
operator|=
operator|(
name|getchar
argument_list|()
operator|&
literal|0177
operator|)
expr_stmt|;
for|for
control|(
name|p
operator|=
name|etable
init|;
name|p
operator|->
name|e_char
condition|;
name|p
operator|++
control|)
if|if
condition|(
name|p
operator|->
name|e_char
operator|==
name|gch
condition|)
block|{
if|if
condition|(
operator|(
name|p
operator|->
name|e_flags
operator|&
name|PRIV
operator|)
operator|&&
name|getuid
argument_list|()
condition|)
continue|continue;
name|printf
argument_list|(
literal|"%s"
argument_list|,
name|ctrl
argument_list|(
name|c
argument_list|)
argument_list|)
expr_stmt|;
call|(
modifier|*
name|p
operator|->
name|e_func
call|)
argument_list|(
name|gch
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/* ESCAPE ESCAPE forces ESCAPE */
if|if
condition|(
name|c
operator|!=
name|gch
condition|)
name|pwrite
argument_list|(
name|FD
argument_list|,
operator|&
name|c
argument_list|,
literal|1
argument_list|)
expr_stmt|;
return|return
operator|(
name|gch
operator|)
return|;
block|}
end_block

begin_expr_stmt
name|any
argument_list|(
name|c
argument_list|,
name|p
argument_list|)
specifier|register
name|char
name|c
operator|,
operator|*
name|p
expr_stmt|;
end_expr_stmt

begin_block
block|{
while|while
condition|(
name|p
operator|&&
operator|*
name|p
condition|)
if|if
condition|(
operator|*
name|p
operator|++
operator|==
name|c
condition|)
return|return
operator|(
literal|1
operator|)
return|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_block

begin_expr_stmt
name|size
argument_list|(
name|s
argument_list|)
specifier|register
name|char
operator|*
name|s
expr_stmt|;
end_expr_stmt

begin_block
block|{
specifier|register
name|int
name|i
init|=
literal|0
decl_stmt|;
while|while
condition|(
name|s
operator|&&
operator|*
name|s
operator|++
condition|)
name|i
operator|++
expr_stmt|;
return|return
operator|(
name|i
operator|)
return|;
block|}
end_block

begin_function
name|char
modifier|*
name|interp
parameter_list|(
name|s
parameter_list|)
specifier|register
name|char
modifier|*
name|s
decl_stmt|;
block|{
specifier|static
name|char
name|buf
index|[
literal|256
index|]
decl_stmt|;
specifier|register
name|char
modifier|*
name|p
init|=
name|buf
decl_stmt|,
name|c
decl_stmt|,
modifier|*
name|q
decl_stmt|;
while|while
condition|(
name|c
operator|=
operator|*
name|s
operator|++
condition|)
block|{
for|for
control|(
name|q
operator|=
literal|"\nn\rr\tt\ff\033E\bb"
init|;
operator|*
name|q
condition|;
name|q
operator|++
control|)
if|if
condition|(
operator|*
name|q
operator|++
operator|==
name|c
condition|)
block|{
operator|*
name|p
operator|++
operator|=
literal|'\\'
expr_stmt|;
operator|*
name|p
operator|++
operator|=
operator|*
name|q
expr_stmt|;
goto|goto
name|next
goto|;
block|}
if|if
condition|(
name|c
operator|<
literal|040
condition|)
block|{
operator|*
name|p
operator|++
operator|=
literal|'^'
expr_stmt|;
operator|*
name|p
operator|++
operator|=
name|c
operator|+
literal|'A'
operator|-
literal|1
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|c
operator|==
literal|0177
condition|)
block|{
operator|*
name|p
operator|++
operator|=
literal|'^'
expr_stmt|;
operator|*
name|p
operator|++
operator|=
literal|'?'
expr_stmt|;
block|}
else|else
operator|*
name|p
operator|++
operator|=
name|c
expr_stmt|;
name|next
label|:
empty_stmt|;
block|}
operator|*
name|p
operator|=
literal|'\0'
expr_stmt|;
return|return
operator|(
name|buf
operator|)
return|;
block|}
end_function

begin_function
name|char
modifier|*
name|ctrl
parameter_list|(
name|c
parameter_list|)
name|char
name|c
decl_stmt|;
block|{
specifier|static
name|char
name|s
index|[
literal|3
index|]
decl_stmt|;
if|if
condition|(
name|c
operator|<
literal|040
operator|||
name|c
operator|==
literal|0177
condition|)
block|{
name|s
index|[
literal|0
index|]
operator|=
literal|'^'
expr_stmt|;
name|s
index|[
literal|1
index|]
operator|=
name|c
operator|==
literal|0177
condition|?
literal|'?'
else|:
name|c
operator|+
literal|'A'
operator|-
literal|1
expr_stmt|;
name|s
index|[
literal|2
index|]
operator|=
literal|'\0'
expr_stmt|;
block|}
else|else
block|{
name|s
index|[
literal|0
index|]
operator|=
name|c
expr_stmt|;
name|s
index|[
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
block|}
return|return
operator|(
name|s
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Help command  */
end_comment

begin_macro
name|help
argument_list|(
argument|c
argument_list|)
end_macro

begin_decl_stmt
name|char
name|c
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|esctable_t
modifier|*
name|p
decl_stmt|;
specifier|extern
name|esctable_t
name|etable
index|[]
decl_stmt|;
name|printf
argument_list|(
literal|"%c\r\n"
argument_list|,
name|c
argument_list|)
expr_stmt|;
for|for
control|(
name|p
operator|=
name|etable
init|;
name|p
operator|->
name|e_char
condition|;
name|p
operator|++
control|)
block|{
if|if
condition|(
operator|(
name|p
operator|->
name|e_flags
operator|&
name|PRIV
operator|)
operator|&&
name|getuid
argument_list|()
condition|)
continue|continue;
name|printf
argument_list|(
literal|"%2s"
argument_list|,
name|ctrl
argument_list|(
name|character
argument_list|(
name|value
argument_list|(
name|ESCAPE
argument_list|)
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%-2s %c   %s\r\n"
argument_list|,
name|ctrl
argument_list|(
name|p
operator|->
name|e_char
argument_list|)
argument_list|,
name|p
operator|->
name|e_flags
operator|&
name|EXP
condition|?
literal|'*'
else|:
literal|' '
argument_list|,
name|p
operator|->
name|e_help
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Return "simple" name from a file name,  * strip leading directories.  */
end_comment

begin_function
name|char
modifier|*
name|sname
parameter_list|(
name|s
parameter_list|)
specifier|register
name|char
modifier|*
name|s
decl_stmt|;
block|{
specifier|register
name|char
modifier|*
name|p
init|=
name|s
decl_stmt|;
while|while
condition|(
operator|*
name|s
condition|)
if|if
condition|(
operator|*
name|s
operator|++
operator|==
literal|'/'
condition|)
name|p
operator|=
name|s
expr_stmt|;
return|return
operator|(
name|p
operator|)
return|;
block|}
end_function

begin_decl_stmt
specifier|static
name|char
name|partab
index|[
literal|0200
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Do a write to the remote machine with the correct parity.  * We are doing 8 bit wide output, so we just generate a character  * with the right parity and output it.  */
end_comment

begin_macro
name|pwrite
argument_list|(
argument|fd
argument_list|,
argument|buf
argument_list|,
argument|n
argument_list|)
end_macro

begin_decl_stmt
name|int
name|fd
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|buf
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|int
name|n
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
specifier|register
name|char
modifier|*
name|bp
decl_stmt|;
specifier|extern
name|int
name|errno
decl_stmt|;
name|bp
operator|=
name|buf
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|n
condition|;
name|i
operator|++
control|)
block|{
operator|*
name|bp
operator|=
name|partab
index|[
operator|(
operator|*
name|bp
operator|)
operator|&
literal|0177
index|]
expr_stmt|;
name|bp
operator|++
expr_stmt|;
block|}
if|if
condition|(
name|write
argument_list|(
name|fd
argument_list|,
name|buf
argument_list|,
name|n
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|==
name|EIO
condition|)
name|abort
argument_list|(
literal|"Lost carrier."
argument_list|)
expr_stmt|;
comment|/* this is questionable */
name|perror
argument_list|(
literal|"write"
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_comment
comment|/*  * Build a parity table with appropriate high-order bit.  */
end_comment

begin_macro
name|setparity
argument_list|(
argument|defparity
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|defparity
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|int
name|i
decl_stmt|;
name|char
modifier|*
name|parity
decl_stmt|;
specifier|extern
name|char
name|evenpartab
index|[]
decl_stmt|;
if|if
condition|(
name|value
argument_list|(
name|PARITY
argument_list|)
operator|==
name|NOSTR
condition|)
name|value
argument_list|(
name|PARITY
argument_list|)
operator|=
name|defparity
expr_stmt|;
name|parity
operator|=
name|value
argument_list|(
name|PARITY
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|0200
condition|;
name|i
operator|++
control|)
name|partab
index|[
name|i
index|]
operator|=
name|evenpartab
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|equal
argument_list|(
name|parity
argument_list|,
literal|"even"
argument_list|)
condition|)
return|return;
if|if
condition|(
name|equal
argument_list|(
name|parity
argument_list|,
literal|"odd"
argument_list|)
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|0200
condition|;
name|i
operator|++
control|)
name|partab
index|[
name|i
index|]
operator|^=
literal|0200
expr_stmt|;
comment|/* reverse bit 7 */
return|return;
block|}
if|if
condition|(
name|equal
argument_list|(
name|parity
argument_list|,
literal|"none"
argument_list|)
operator|||
name|equal
argument_list|(
name|parity
argument_list|,
literal|"zero"
argument_list|)
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|0200
condition|;
name|i
operator|++
control|)
name|partab
index|[
name|i
index|]
operator|&=
operator|~
literal|0200
expr_stmt|;
comment|/* turn off bit 7 */
return|return;
block|}
if|if
condition|(
name|equal
argument_list|(
name|parity
argument_list|,
literal|"one"
argument_list|)
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|0200
condition|;
name|i
operator|++
control|)
name|partab
index|[
name|i
index|]
operator||=
literal|0200
expr_stmt|;
comment|/* turn on bit 7 */
return|return;
block|}
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: unknown parity value\n"
argument_list|,
name|PA
argument_list|)
expr_stmt|;
name|fflush
argument_list|(
name|stderr
argument_list|)
expr_stmt|;
block|}
end_block

end_unit

