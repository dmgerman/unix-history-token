...
begin_comment
comment|/* m68k.c  All the m68020 specific stuff in one convenient, huge,    slow to compile, easy to find file.    Copyright (C) 1987 Free Software Foundation, Inc.  This file is part of GAS, the GNU Assembler.  GAS is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GAS is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GAS; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/*  17-24 Eight FP registers */
end_comment
...
begin_comment
comment|/*  25-32 Co-processor #1-#8 */
end_comment
...
begin_comment
comment|/*  34 Hack for Program space, but 0 addressing */
end_comment
...
begin_comment
comment|/*  38 Interrupt stack pointer */
end_comment
...
begin_comment
comment|/* Macros for adding things to the m68_it struct */
end_comment
...
name|pc_rel
parameter_list|)
value|{\ 	the_ins.reloc[the_ins.nrel].n= ((width)=='B') ? (the_ins.numo*2-1) : \ 		(((width)=='b') ? ((the_ins.numo-1)*2) : (the_ins.numo*2));\ 	the_ins.reloc[the_ins.nrel].add=adds((exp));\ 	the_ins.reloc[the_ins.nrel].sub=subs((exp));\ 	the_ins.reloc[the_ins.nrel].off=offs((exp));\ 	the_ins.reloc[the_ins.nrel].wid=width;\ 	the_ins.reloc[the_ins.nrel++].pcrel=pc_rel;\ }
end_define
...
begin_comment
comment|/* handle of the OPCODE hash table 				   NULL means any use before m68_ip_begin() 				   will crash */
end_comment
...
index|]
expr_stmt|;
comment|/* bcopy((char *)(&the_ins.operands[0]),(char *)(&the_ins.operands[1]),opsfound*sizeof(the_ins.operands[0])); */
name|bzero
argument_list|(
operator|(
name|char
...
name|as_fatal
argument_list|(
literal|"Internal error:  Operand mode %c unknown"
argument_list|,
operator|*
name|s
argument_list|)
...
control|)
block|{
comment|/* This switch is a doozy. 			   What the first step; its a big one! */
switch|switch
condition|(
name|s
index|[
...
name|as_fatal
argument_list|(
literal|"Internal error:  Can't decode %c%c"
argument_list|,
operator|*
name|s
argument_list|,
...
literal|0x100
expr_stmt|;
comment|/* If the guy specified a width, we assume that 					   it is wide enough.  Maybe it isn't.  Ifso, we lose 					 */
switch|switch
condition|(
name|siz1
condition|)
...
name|as_fatal
argument_list|(
literal|"Internal error:  Unknown mode #%c"
argument_list|,
name|s
index|[
literal|1
...
condition|)
block|{
comment|/* Deal with fixed size stuff by hand */
switch|switch
condition|(
name|opP
operator|->
...
name|as_fatal
argument_list|(
literal|"Bad size for expression %d"
argument_list|,
name|opP
operator|->
name|con1
...
block|{
comment|/* JF:  This is the WRONG thing to do 					add_frag((symbolS *)0,offs(opP->con1),TAB(BRANCH,BYTE)); */
name|the_ins
operator|.
name|opcode
index|[
...
name|as_bad
argument_list|(
literal|"Bad size for offset, must be word or long"
argument_list|)
expr_stmt|;
break|break;
block|}
...
name|as_fatal
argument_list|(
literal|"Internal error:  operand type B%c unknown"
argument_list|,
name|s
index|[
literal|1
...
name|as_fatal
argument_list|(
literal|"Internal error:  Operand type %c unknown"
argument_list|,
name|s
index|[
literal|0
...
name|s
parameter_list|)
value|((*s == ',' || *s == '}' || *s == '{' \                    || (*s == ':'&& index("aAdD#", s[1]))) \                ? 0 : 1)
end_define
...
literal|0xB8
expr_stmt|;
comment|/* JBSR with ABSL WORD offset */
name|subseg_change
argument_list|(
name|SEG_TEXT
argument_list|,
...
literal|0xF8
expr_stmt|;
comment|/* JMP	with ABSL WORD offset */
name|subseg_change
argument_list|(
name|SEG_TEXT
argument_list|,
...
literal|'m'
case|:
comment|/* Gas almost ignores this option! */
operator|(
operator|*
name|argP
operator|)
