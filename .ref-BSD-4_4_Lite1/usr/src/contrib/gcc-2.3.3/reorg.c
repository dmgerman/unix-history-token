...
argument_list|)
expr_stmt|;
comment|/* Skip any labels between the CALL_INSN and possible USE insns.  */
while|while
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Skip any possible labels between the CALL_INSN and CLOBBERs.  */
while|while
condition|(
name|GET_CODE
argument_list|(
...
decl_stmt|;
block|{
comment|/* If we have an empty list, just make a new list element.  */
if|if
condition|(
name|delay_list
operator|==
...
begin_comment
comment|/* Return truth value of the statement that this branch    is mostly taken.  If we think that the branch is extremely likely    to be taken, we return 2.  If the branch is slightly more likely to be    taken, return 1.  Otherwise, return 0.     CONDITION, if non-zero, is the condition that JUMP_INSN is testing.  */
end_comment
...
name|insn
decl_stmt|;
comment|/* If this is a conditional return insn, assume it won't return.  */
if|if
condition|(
name|target_label
operator|==
...
literal|0
return|;
comment|/* If TARGET_LABEL has no jumps between it and the end of the function,      this is essentially a conditional return, so predict it as false.  */
for|for
control|(
name|insn
operator|=
...
condition|)
block|{
comment|/* Stop for a CALL and its delay slots because it difficult to track 	     its resource needs correctly.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/* This insn needs, or can use, some delay slots.  SLOTS_TO_FILL 	 says how many.  After initialization, first try optimizing  	 call _foo		call _foo 	 nop			add %o7,.-L1,%o7 	 b,a L1 	 nop  	 If this case applies, the delay slot of the call is filled with 	 the unconditional jump.  This is done first to avoid having the 	 delay slot of the call filled in the backward scan.  Also, since 	 the unconditional jump is likely to also have a delay slot, that 	 insn must exist when it is subsequently scanned.  */
name|slots_filled
operator|=
literal|0
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* If we can't make the jump into a RETURN, redirect it to the best 	 RETURN and go on to the next insn.  */
if|if
condition|(
operator|!
name|redirect_jump
