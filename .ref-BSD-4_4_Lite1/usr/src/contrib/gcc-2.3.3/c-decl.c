...
comment|/* 2 for a definition, 1 for a declaration.  */
name|char
name|parm_flag
decl_stmt|;
...
begin_comment
comment|/* Identify this binding level as a level of parameters.    DEFINITION_FLAG is 1 for a definition, 0 for a declaration.  */
end_comment
...
literal|1
condition|)
comment|/* Don't warn about the parm names in a declaration.  */
empty_stmt|;
elseif|else
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* ``integer_tpe_node'' misspelling corrected: North-Keys 30 Mar 91 */
name|builtin_function
argument_list|(
...
name|decl
argument_list|,
literal|"ANSI C forbids parameter `%s' shadowing typedef"
argument_list|)
expr_stmt|;
block|}
name|decl
...
name|double_type_node
expr_stmt|;
comment|/* Don't use TYPE_PREISION to decide whether to promote, 	   because we should convert short if it's the same size as int, 	   but we should not convert long if it's the same size as int.  */
elseif|else
if|if
condition|(
name|C_PROMOTING_INTEGER_TYPE_P
...
argument_list|)
decl_stmt|;
comment|/* Preserve unsignedness if traditional or if not really any wider.  */
if|if
condition|(
name|TREE_UNSIGNED
argument_list|(
