...
begin_comment
comment|/* Definitions for code generation pass of GNU compiler.    Copyright (C) 1987, 1991 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* Nonzero if we do not know how to pass TYPE solely in registers.    We cannot do so in the following cases:     - if the type has variable size    - if the type is marked as addressable (it is required to be constructed      into the stack)    - if the padding and mode of the type is such that a copy into a register      would put it into the wrong part of the register    - when STRICT_ALIGNMENT and the type is BLKmode and is is not      aligned to a boundary corresponding to what can be loaded into a      register.  */
end_comment

begin_define
define|#
directive|define
name|MUST_PASS_IN_STACK_BAD_ALIGN
parameter_list|(
name|MODE
parameter_list|,
name|TYPE
parameter_list|)
define|\
value|(STRICT_ALIGNMENT&& MODE == BLKmode			        \&& TYPE_ALIGN (TYPE)< (BIGGEST_ALIGNMENT< BITS_PER_WORD	\ 			   ? BIGGEST_ALIGNMENT : BITS_PER_WORD))
end_define

begin_comment
comment|/* Which padding can't be supported depends on the byte endianness.  */
end_comment

begin_comment
comment|/* A value in a register is implicitly padded at the most significant end.    On a big-endian machine, that is the lower end in memory.    So a value padded in memory at the upper end can't go in a register.    For a little-endian machine, the reverse is true.  */
end_comment
...
parameter_list|)
define|\
value|((TYPE) != 0						\&& (TREE_CODE (TYPE_SIZE (TYPE)) != INTEGER_CST	\        || TREE_ADDRESSABLE (TYPE)			\        || ((MODE) == BLKmode 				\&& (FUNCTION_ARG_PADDING (MODE, TYPE)	\ 	       == MUST_PASS_IN_STACK_BAD_PADDING))	\        || MUST_PASS_IN_STACK_BAD_ALIGN (MODE, TYPE)))
end_define
