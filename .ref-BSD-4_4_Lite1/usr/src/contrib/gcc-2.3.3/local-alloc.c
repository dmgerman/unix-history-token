...
begin_comment
comment|/* Allocate registers within a basic block, for GNU compiler.    Copyright (C) 1987, 1988, 1991 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* Element Q is non-zero if there is a suggested register in    qty_phys_copy_sugg.  */
end_comment
...
name|char
modifier|*
name|qty_phys_has_copy_sugg
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Element Q is non-zero if there is a suggested register in qty_phys_sugg. */
end_comment
...
break|break;
block|}
comment|/* If CLASS has only one register, don't allocate the SCRATCH here since      it will prevent that register from being used as a spill register.      reload will do the allocation.  */
if|if
condition|(
name|class
operator|==
...
parameter_list|)
define|\
value|bzero ((vector), (sizeof (*(vector))) * next_qty);
name|CLEAR
argument_list|(
name|qty_scratch_rtx
argument_list|)
...
literal|0
decl_stmt|;
comment|/* If P is a CALL_INSN, SRC crosses one more call, since it 	     used to die there.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
literal|1
expr_stmt|;
comment|/* Is this insn suitable for tying two registers? 	     If so, try doing that. 	     Suitable insns are those with at least two operands and where 	     operand 0 is an output that is a register that is not 	     earlyclobber. 	     For a commutative operation, try (set reg0 (arithop ... reg1)). 	     Subregs in place of regs are also ok.  	     If tying is done, WIN is set nonzero.  */
if|if
condition|(
name|insn_code_number
operator|>=
...
index|]
expr_stmt|;
comment|/* If the first operand is an address, find a register in it. 		 There may be more than one register, but we only try one of 		 them.  */
if|if
condition|(
ifdef|#
directive|ifdef
...
expr_stmt|;
block|}
comment|/* Now every register that is local to this basic block      should have been given a quantity, or else -1 meaning ignore it.      Every quantity should have a known birth and death.         Order the qtys so we assign them registers in order of       decreasing length of life.  Normally call qsort, but if we       have only a very small number of quantities, sort them ourselves.  */
name|qty_order
operator|=
operator|(
name|short
...
begin_comment
comment|/* Return 1 if the constraint string P indicates that the a the operand    must be equal to operand 0 and that no register is acceptable.  */
end_comment
...
case|:
default|default:
comment|/* These mean a register is allowed.  Fail if so.  */
return|return
literal|0
return|;
block|}
