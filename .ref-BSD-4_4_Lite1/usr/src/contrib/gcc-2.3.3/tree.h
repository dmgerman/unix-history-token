...
begin_comment
comment|/* In a CONVERT_EXPR or NOP_EXPR, this means the node was made    implicitly and should not lead to an "unused value" warning.  */
end_comment
...
begin_comment
comment|/* In an INTEGER_CST, this means there was overflow in folding.  */
end_comment
...
begin_comment
comment|/* In a RECORD_TYPE or UNION_TYPE, it means the type has BLKmode    only because it lacks the alignment requirement for its size.  */
end_comment
...
begin_comment
comment|/* This macro is marked for death.  */
end_comment
...
begin_comment
comment|/*  For FIELD_DECLs, this is the     RECORD_TYPE or UNION_TYPE node that the field is a member of.  For     VAR_DECL, PARM_DECL, FUNCTION_DECL, LABEL_DECL, and CONST_DECL nodes,     this points to the FUNCTION_DECL for the containing function, or else     yields NULL_TREE if the given decl has "file scope".  */
end_comment
...
parameter_list|)
define|\
value|((enum built_in_function) (NODE)->decl.frame_size)
end_define
...
parameter_list|)
define|\
value|((NODE)->decl.frame_size = (int) (VAL))
end_define
...
begin_comment
comment|/* Used in VAR_DECLs to indicate that the variable is a vtable.    It is also used in FIELD_DECLs for vtable pointers.  */
end_comment
...
name|abstract_origin
decl_stmt|;
comment|/* The PRINT_NAME field is marked for death.  */
name|char
modifier|*
name|print_name
decl_stmt|;
...
begin_comment
comment|/* Format for global names of constructor and destructor functions.  */
end_comment
...
begin_comment
comment|/* Some machines need to override this.  */
end_comment
...
begin_else
unit|extern char *xrealloc			PROTO((void *, unsigned));
else|#
directive|else
end_else
...
begin_comment
comment|/* We cannot define prototypes for the variable argument functions,    since they have not been ANSI-fied, and an ANSI compiler would    complain when compiling the definition of these functions.  */
end_comment

begin_else
unit|extern tree build			PROTO((enum tree_code, tree, ...)); extern tree build_nt			PROTO((enum tree_code, ...)); extern tree build_parse_node		PROTO((enum tree_code, ...));
else|#
directive|else
end_else
...
begin_comment
comment|/* Construct expressions, performing type checking.    GCC and G++ use different calling sequences for build_binary_op,    and build_indirect_ref, so don't include a prototype here.  */
end_comment
...
begin_comment
comment|/* Return the RECORD_TYPE or UNION_TYPE which provides this _DECL    with its context, or zero if none.  */
end_comment
