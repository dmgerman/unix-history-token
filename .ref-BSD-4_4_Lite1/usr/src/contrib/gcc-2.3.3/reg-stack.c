...
begin_comment
comment|/* This is the basic stack record.  TOP is an index into REG[] such    that REG[TOP] is the top of stack.  If TOP is -1 the stack is empty.     If TOP is -2 the stack is not yet initialized: reg_set indicates    which registers are live.  Stack initialization consists of placing    each live reg in array `reg' and setting `top' appropriately. */
end_comment
...
parameter_list|)
define|\
value|(((INSN_UID (INSN)> max_uid)	\     ? (short *)(abort() , 0)		\     : block_number)[INSN_UID (INSN)])
end_define
...
name|clobber_reg
decl_stmt|;
comment|/* Find out what the constraints require.  If no constraint      alternative matches, that is a compiler bug: we should have caught      such an insn during reload.  */
name|i
operator|=
name|constrain_asm_operands
argument_list|(
...
begin_comment
comment|/* Emit an insn before or after INSN to swap virtual register REG with the    top of stack.  See comments before emit_hard_swap_insn. */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* The first argument must always be a stack reg. */
comment|/* ??? why? */
if|if
condition|(
operator|!
...
block|{
comment|/* Both operands are REG.  If neither operand is already 	       at the top of stack, choose to make the one that is the dest 	       the new top of stack.  	       ??? A later optimization here would be to look forward 	       in the insns and see which source reg will be needed at top 	       of stack soonest. */
name|int
