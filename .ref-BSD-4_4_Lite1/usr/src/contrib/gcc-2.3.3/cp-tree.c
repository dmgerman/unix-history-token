...
begin_comment
comment|/* Language-dependent node constructors for parse phase of GNU compiler.    Copyright (C) 1987, 1988, 1992 Free Software Foundation, Inc.    Hacked by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|ADDR_EXPR
case|:
comment|/* ANSI C++ June 5 1992 WP 5.4.14.  The result of a cast to a 	   reference is an lvalue.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
directive|if
literal|0
comment|/* it is wrong to flag the object the pointer points to as readonly      when flag_this_is_variable is 0. */
block|ptype = build_type_variant (ptype, flag_this_is_variable<= 0, 0);
else|#
directive|else
name|ptype
operator|=
...
begin_comment
comment|/* Lay out the base types of a record type, REC.    Tentatively set the size and alignment of REC    according to the base types alone.     Offsets for immediate nonvirtual baseclasses are also computed here.     Returns list of virtual base classes in a FIELD_DECL chain.  */
end_comment
...
name|warning
argument_list|(
literal|"in inheritance relationship `%s:%s %s'"
argument_list|,
name|TYPE_NAME_STRING
argument_list|(
name|rec
...
condition|)
block|{
comment|/* If the type name and constructor name are different, don't 	     write constructor name into type.  */
specifier|extern
name|tree
...
begin_comment
comment|/* Return the binfo value for ELEM in TYPE.  Due to structure    sharing, we may find ELEM only in the association list    belonging to a basetype of TYPE.     COPYING is 0 if we just want an binfo value without needing    to modify it.    COPYING is 1 if we want the binfo value in order to modify it.    In this case, if we don't find ELEM immediately in the binfo    values of TYPE, we return a copy.    COPYING is -1 if we are called recursively and need a copy.    In this case we return a copy of ELEM at the point we find it.  */
end_comment
...
name|binfo
return|;
comment|/* Look for ELEM in two passes.  First pass checks the entire binfo list.      Second pass recursively searches the binfo lists of binfos.  */
while|while
condition|(
name|binfo
condition|)
...
control|)
block|{
comment|/* ??? Should this condition instead test 	 BINFO_TYPE (binfo) != TYPE_MAIN_VARIANT (type) ???  */
if|if
condition|(
name|BINFO_TYPE
argument_list|(
...
begin_comment
comment|/* Return the offset (as an INTEGER_CST) for ELEM in LIST.    INITIAL_OFFSET is the value to add to the offset that ELEM's    binfo entry in LIST provides.     Returns NULL if ELEM does not have an binfo value in LIST.  */
end_comment
