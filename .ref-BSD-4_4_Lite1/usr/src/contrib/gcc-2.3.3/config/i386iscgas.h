...
begin_comment
comment|/* Underscores are not used on ISC systems (probably not on any COFF    system), despite the comments in i386gas.h.  If this is not defined,    enquire (for example) will fail to link.  --karl@cs.umb.edu  */
end_comment
...
parameter_list|)
define|\
value|fprintf (asmfile, "%s \"%s/\",%d,0,0,%s\n", ASM_STABS_OP,		\ 	  	     cwd, N_SO, ltext_label_name)
end_define
...
parameter_list|)
define|\
value|fprintf (asmfile, "%s \"%s\",%d,0,0,%s\n", ASM_STABS_OP, input_file_name,\ 	   N_SO, ltext_label_name);					\   text_section ();							\   ASM_OUTPUT_INTERNAL_LABEL (asmfile, "Ltext", 0)
end_define
...
begin_comment
comment|/* But we can't use crtbegin.o and crtend.o, because gas 1.38.1 doesn't    grok .section.  The definitions here are otherwise identical to those    in isc.h.  */
end_comment
