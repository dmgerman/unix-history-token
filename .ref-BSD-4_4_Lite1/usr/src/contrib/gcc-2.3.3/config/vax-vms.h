...
begin_comment
comment|/* This enables certain macros in vax.h, which will make an indirect    reference to an external symbol an invalid address.  This needs to be    defined before we include vax.h, since it determines which macros    are used for GO_IF_*.  */
end_comment
...
directive|define
name|CPP_PREDEFINES
value|"-Dvax -Dvms -DVMS -D__GNUC__=2"
end_define

begin_comment
comment|/* These match the definitions used in VAXCRTL, the VMS C run-time library */
end_comment
...
begin_comment
comment|/* Strictly speaking, VMS does not use DBX at all, but the interpreter built    into gas only speaks straight DBX.  */
end_comment
...
begin_comment
comment|/* We redefine this because there is a hidden variable on the stack    that VAXC$ESTABLISH uses.  We just need to add four bytes to whatever    gcc thinks that we need.  Similarly, we need to move all local variables    down 4 bytes in the stack.  */
end_comment
...
parameter_list|)
define|\
value|{ register int regno;						\   register int mask = 0;					\   register int newsize = SIZE + 4;				\   extern char call_used_regs[];					\   for (regno = 0; regno< FIRST_PSEUDO_REGISTER; regno++)	\     if (regs_ever_live[regno]&& !call_used_regs[regno])	\        mask |= 1<< regno;					\   fprintf (FILE, "\t.word 0x%x\n", mask);			\   MAYBE_VMS_FUNCTION_PROLOGUE(FILE)				\   if (newsize>= 64) fprintf (FILE, "\tmovab %d(sp),sp\n", -newsize);\   else fprintf (FILE, "\tsubl2 $%d,sp\n", newsize); }
end_define
...
value|c_expand_return (integer_one_node)
end_define
...
begin_comment
comment|/* This makes use of a hook in varasm.c to mark all external variables    for us.  We use this to make sure that external variables are correctly    addressed.  Under VMS there is some brain damage in the linker that requires    us to do this.  */
end_comment
...
parameter_list|)
define|\
value|if (DECL_EXTERNAL (decl)&& TREE_PUBLIC (decl)) 		\     SYMBOL_REF_FLAG (XEXP (DECL_RTL (decl), 0)) = 1;
end_define

begin_comment
comment|/* Under VMS we write the actual size of the storage to be allocated even    though the symbol is external.  Although it is possible to give external    symbols a size of 0 (as unix does), the VMS linker does not make the    distinction between a variable definition and an external reference of a    variable, and thus the linker will not complain about a missing definition.    If we followed the unix example of giving external symbols a size of    zero, you tried to link a program where a given variable was externally    defined but none of the object modules contained a non-extern definition,    the linker would allocate 0 bytes for the variable, and any attempt to    use that variable would use the storage allocated to some other variable.     We must also select either const_section or data_section: this will indicate    whether or not the variable will get the readonly bit set.  Since the    VMS linker does not distinguish between a variable's definition and an    external reference, all usages of a given variable must have the readonly    bit set the same way, or the linker will get confused and give warning    messages.  */
end_comment

begin_comment
comment|/* We used to round the size up to a multiple of 4,    but that causes linker errors sometimes when the variable was initialized    since the size of its definition was not likewise rounded up.  */
end_comment
...
parameter_list|)
define|\
value|( ((TREE_READONLY (decl)&& ! TREE_THIS_VOLATILE (decl))	\    ? (const_section (), 0) : (data_section (), 0)),		\   fputs (".comm ", (FILE)),					\   assemble_name ((FILE), (NAME)),				\   fprintf ((FILE), ",%u\n", (SIZE)))
end_define

begin_comment
comment|/* We define this to prevent the name mangler from putting dollar signs into    function names.  This isn't really needed, but it has been here for    some time and  removing it would cause the object files generated by the    compiler to be incompatible with the object files from a compiler that    had this defined.  Since it does no harm, we leave it in.  */
end_comment
...
define|\
value|void							\ const_section ()					\ {							\   if (in_section != in_const) {				\     fprintf(asm_out_file,".const\n");			\     in_section = in_const;				\   }							\ }
end_define

begin_comment
comment|/* This macro contains the logic to decide which section a variable    should be stored in.  Static constant variables go in the text_section,    non-const variables go in the data_section, and non-static const    variables go in the const_section.     Since this macro is used in a number of places, we must also be able    to decide where to place string constants.  */
end_comment
...
parameter_list|)
define|\
value|{									\   if (TREE_CODE (T) == VAR_DECL)					\     {									\       if (TREE_READONLY (T)&& ! TREE_THIS_VOLATILE (T))		\ 	{								\ 	  if (TREE_PUBLIC (T))						\ 	    const_section ();						\ 	  else								\ 	    text_section ();						\ 	}								\       else								\ 	data_section ();						\     }									\   if (*tree_code_type[(int) TREE_CODE (T)] == 'c')			\     {									\       if ((TREE_CODE (T) == STRING_CST&& flag_writable_strings))	\ 	data_section ();						\       else								\ 	text_section ();						\     }									\ }
end_define

begin_comment
comment|/* This is used by a hook in varasm.c to write the assembler directives    that are needed to tell the startup code which constructors need to    be run.  */
end_comment
...
parameter_list|)
define|\
value|{									\   fprintf ((FILE),".globl $$PsectAttributes_NOOVR$$__gxx_init_1\n"); 	\   data_section();							\   fprintf ((FILE),"$$PsectAttributes_NOOVR$$__gxx_init_1:\n\t.long\t"); \   assemble_name ((FILE), (NAME));					\   fputc ('\n', (FILE));							\ }
end_define

begin_comment
comment|/* This is used by a hook in varasm.c to write the assembler directives    that are needed to tell the startup code which destructors need to    be run.  */
end_comment
...
define|#
directive|define
name|__CTOR_LIST_SHR__
value|$$PsectAttributes_NOSHR$$__gxx_init_0_shr
end_define

begin_define
define|#
directive|define
name|__CTOR_LIST_SHR_END__
value|$$PsectAttributes_NOSHR$$__gxx_init_2_shr
end_define
...
define|#
directive|define
name|__DTOR_LIST_SHR__
value|$$PsectAttributes_NOSHR$$__gxx_clean_0_shr
end_define

begin_define
define|#
directive|define
name|__DTOR_LIST_SHR_END__
value|$$PsectAttributes_NOSHR$$__gxx_clean_2_shr
end_define
...
define|\
value|do {									\   func_ptr *p;								\   extern func_ptr __DTOR_LIST__[1];					\   extern func_ptr __DTOR_LIST_END__[1];					\   extern func_ptr __DTOR_LIST_SHR__[1];					\   extern func_ptr __DTOR_LIST_SHR_END__[1];				\   for (p = __DTOR_LIST__ + 1; p< __DTOR_LIST_END__ ; p++ )		\     if (*p) (*p) ();							\   if(&__DTOR_LIST_SHR__[0] !=&__DTOR_LIST__[1])			\   for (p = __DTOR_LIST_SHR__ + 1; p< __DTOR_LIST_SHR_END__ ; p++ )	\     if (*p) (*p) ();							\ } while (0)
end_define
