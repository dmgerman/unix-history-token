...
directive|define
name|LIB_SPEC
value|"%{msoft-float:%{!p:%{!pg:-lc}}%{p:-lc_p}%{pg:-lc_p}} \ %{!msoft-float:%{!p:%{!pg:-lmc}}%{p:-lmc_p}%{pg:-lmc_p}} \ %{g:-lg}"
end_define
...
directive|define
name|LIB_SPEC
value|"%{!m68881:%{!p:%{!pg:-lc}}%{p:-lc_p}%{pg:-lc_p}} \ %{m68881:%{!p:%{!pg:-lmc}}%{p:-lmc_p}%{pg:-lmc_p}} \ %{g:-lg}"
end_define
...
begin_comment
comment|/* Override parts of m68k.h to fit the ISI 68k machine.  */
end_comment
