...
parameter_list|)
define|\
value|((TARGET_80387&& ((MODE) == SFmode || (MODE) == DFmode))
end_define

begin_define
unit|? FIRST_FLOAT_REG : 0)
define|#
directive|define
...
parameter_list|)
define|\
value|((REGNO)< 2 ? 1							\    : (REGNO)< 4 ? 1							\    : (REGNO)>= 8 ? ((GET_MODE_CLASS (MODE) == MODE_FLOAT		\ 		      || GET_MODE_CLASS (MODE) == MODE_COMPLEX_FLOAT)	\&& TARGET_80387					\&& GET_MODE_UNIT_SIZE (MODE)<= 8)			\    : (MODE) != QImode)
end_define
