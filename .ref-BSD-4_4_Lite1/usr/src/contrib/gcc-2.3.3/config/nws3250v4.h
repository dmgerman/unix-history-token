...
directive|define
name|CPP_PREDEFINES
value|"-Dmips -Dhost_mips -Dsony -Dsonyrisc -Dunix \ -DLANGUAGE_C -DMIPSEB -DSYSTYPE_SYSV"
end_define
...
directive|define
name|ASM_SPEC
value|"\ %{!mgas: \ 	%{!mrnames: -nocpp} \ 	%{pipe:%e:-pipe not supported} \ 	%{EB} %{!EB:-EB} \ 	%{EL: %e-EL not supported} \ 	%{O:-O2} %{O1:-O2} %{O2:-O2} %{O3:-O3} \ 	%{g} %{g1} %{g2} %{g3} %{g0} %{v} %{K}} \ %{G*}"
end_define
...
directive|define
name|LINK_SPEC
value|"\ %{G*} \ %{!mgas: \ 	%{EB} %{!EB:-EB} \ 	%{EL: %e-EL not supported} \ 	%{bestGnum}}"
end_define
...
directive|define
name|LIB_SPEC
value|"%{p:-lprof1} %{pg:-lprof1} -lc crtn.o%s values-Xt.o%s"
end_define
