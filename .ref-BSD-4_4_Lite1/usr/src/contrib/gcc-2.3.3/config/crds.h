...
begin_comment
comment|/* Charles River Data Systems assembler */
end_comment
...
begin_comment
comment|/* Without STRUCTURE_SIZE_BOUNDARY, we can't ensure that structures are    aligned such that we can correctly extract bitfields from them.    Someone should check whether the usual compiler on the crds machine    provides the equivalent behavior of STRUCTURE_SIZE_BOUNDARY.  */
end_comment
...
begin_comment
comment|/* special flags to the unos assembler.  */
end_comment
...
directive|define
name|LIB_SPEC
value|"%{!p:%{!pg:-lunos}}%{p:-lc_p}%{pg:-lc_p}"
end_define
...
name|STARTFILE_SPEC
define|\
value|"%{pg:gcrt0.o%s}%{!pg:%{p:mc68rt0.o%s}%{!p:c68rt0.o%s}}"
end_define
...
begin_comment
comment|/* c.sac only used in _s_call_r() in libunos.a and malloc() in libmalloc.a */
end_comment

begin_comment
comment|/* so we do not need to bother ! */
end_comment
...
begin_comment
comment|/* -O2 for MAX optimization */
end_comment
...
begin_comment
comment|/* #define SDB_DEBUGGING_INFO UNOS casm has no debugging :-( */
end_comment

begin_comment
comment|/* UNOS has vprintf() */
end_comment
...
name|gen_probe
parameter_list|()
value|gen_rtx(ASM_INPUT, VOIDmode, "tstb -2048(sp)\t;probe\n")
end_define

begin_comment
comment|/* use memcpy, memset instead of bcopy, etc. */
end_comment
...
directive|define
name|CPP_PREDEFINES
value|"-Dmc68k -DM68000 -Dmc68000 -Dunos -Dunix"
end_define
...
begin_comment
comment|/* unos uses ".comm c.sac" returns&c.sac in d0 */
end_comment

begin_comment
comment|/* make pointer to c.sac ? #undef STRUCT_VALUE_REGNUM #define STRUCT_VALUE gen_rtx(MEM, Pmode, gen_rtx( , , ) ) */
end_comment
...
define|\
value|void									\ bss_section ()								\ {									\   if (in_section != in_bss)						\     {									\       fprintf (asm_out_file, ".bss\n");					\       in_section = in_bss;						\     }									\ }
end_define
...
begin_comment
comment|/* Override parts of m68k.h to fit the CRuDS assembler.  */
end_comment
...
directive|define
name|TARGET_VERSION
value|fprintf (stderr, " (68k, CRDS/UNOS)");
end_define
...
parameter_list|)
define|\
value|do { union { double f; long l[2];} tem;			\      tem.f = (VALUE);					\      fprintf (FILE, "\t.long 0x%x, 0x%x\n", tem.l[0], tem.l[1]);	\    } while (0)
end_define

begin_comment
comment|/*unos has no .skip :-( */
end_comment
...
begin_comment
comment|/* should use bss_section instead of data_section but this makes casm die ? */
end_comment
...
parameter_list|)
define|\
value|{ data_section ();				\   if ((SIZE)> 1) fprintf (FILE, "\t.even\n");	\   assemble_name ((FILE), (NAME));		\   fprintf ((FILE), ":\t. = . + %u\n", (ROUNDED));}
end_define
...
parameter_list|)
define|\
value|{  int i;								\ 	  fprintf ((FILE), "\t.ascii \"");				\ 	  for (i = 0; i< (SIZE); i++)					\ 	    {								\ 	      register int c = (P)[i];					\ 	      if (i != 0&& (i / 200) * 200 == i)			\ 		fprintf ((FILE), "\"\n\t.ascii \"");			\ 	      if (c>= ' '&& c< 0177) {				\ 	        if (c != '\"'&& c != '\\') {				\ 		  putc (c, (FILE));					\ 		  continue;						\ 	        }							\ 	       }							\
comment|/* brain dead asm doesn't understand char escapes */
value|\ 	       fprintf ((FILE), "\"\n\t.byte\t%d\n\t.ascii \"", c);	\ 	    }								\ 	  fprintf ((FILE), "\"\n");					\  }
end_define

begin_comment
comment|/* Change all JBxx to Bxx.  Also change all DBRA to DBF.    Also change divs.l, etc., to divs, etc.  But don't change divsl.l.  */
end_comment
...
parameter_list|)
define|\
value|{ if ((PTR)[0] == 'j'&& (PTR)[1] == 'b')		\     { ++(PTR); }					\   else if ((PTR)[0] == 'd')				\     {							\       if (!strncmp ((PTR), "dbra", 4))			\ 	{ fprintf ((FILE), "dbf"); (PTR) += 4; }	\       else if (!strncmp ((PTR), "div", 3)&& (PTR)[5] == ' ')  \ 	{ fprintf ((FILE), "div%c", (PTR)[3]); (PTR) += 6; }   \     }							\ }
end_define
...
parameter_list|)
define|\
value|{ int i;								\   if (CODE == '.') ;							\   else if (CODE == '#') fprintf (FILE, "$");				\   else if (CODE == '-') fprintf (FILE, "-(sp)");			\   else if (CODE == '+') fprintf (FILE, "(sp)+");			\   else if (CODE == '@') fprintf (FILE, "(sp)");				\   else if (CODE == '!') fprintf (FILE, "fpcr");				\   else if (GET_CODE (X) == REG)						\     { if (REGNO (X)< 16&& (CODE == 'y' || CODE == 'x')&& GET_MODE (X) == DFmode)	\         fprintf (FILE, "%s:%s", reg_names[REGNO (X)], reg_names[REGNO (X)+1]); \       else								\         fprintf (FILE, "%s", reg_names[REGNO (X)]);			\     }									\   else if (GET_CODE (X) == MEM)						\     {									\       output_address (XEXP (X, 0));					\       if (CODE == 'd'&& ! TARGET_68020					\&& CONSTANT_ADDRESS_P (XEXP (X, 0)))				\
comment|/* fprintf (FILE, ".l") */
value|;					\     }									\   else if ((CODE == 'y' || CODE == 'w')					\&& GET_CODE(X) == CONST_DOUBLE				\&& (i = standard_sun_fpa_constant_p (X)))			\     fprintf (FILE, "%%%d", i& 0x1ff);					\   else if (GET_CODE (X) == CONST_DOUBLE&& GET_MODE (X) == SFmode)	\     { union { double d; int i[2]; } u;					\       union { float f; int i; } u1;					\       u.i[0] = CONST_DOUBLE_LOW (X); u.i[1] = CONST_DOUBLE_HIGH (X);	\       u1.f = u.d;							\       if (CODE == 'f')							\ 	ASM_OUTPUT_FLOAT_OPERAND (FILE, u1.f);				\       else								\         fprintf (FILE, "$0x%x", u1.i); }				\   else if (GET_CODE (X) == CONST_DOUBLE&& GET_MODE (X) != DImode)	\     { union { double d; int i[2]; } u;					\       u.i[0] = CONST_DOUBLE_LOW (X); u.i[1] = CONST_DOUBLE_HIGH (X);	\       ASM_OUTPUT_DOUBLE_OPERAND (FILE, u.d); }				\   else { putc ('$', FILE); output_addr_const (FILE, X); }}
end_define
...
parameter_list|)
define|\
value|fprintf (FILE, "\tmovl&LP%%%d,%%a0\n\tjsr mcount\n", (LABEL_NO))
end_define
