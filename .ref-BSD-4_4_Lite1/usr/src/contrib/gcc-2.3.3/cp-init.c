...
begin_comment
comment|/* Handle initialization things in C++.    Copyright (C) 1987, 1989, 1992 Free Software Foundation, Inc.    Contributed by Michael Tiemann (tiemann@cygnus.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
expr_stmt|;
block|}
comment|/* ARM $7.1.1: "[register] may be ignored and in most implementations      it will be ignored if the address of the variable is taken."      Since we're likely to do just that in the ctor call, clear this.  */
name|DECL_REGISTER
argument_list|(
name|exp
argument_list|)
...
condition|)
block|{
comment|/* Simple "new dynamic" construct.  */
if|if
condition|(
operator|!
name|IS_AGGR_TYPE
...
name|error
argument_list|(
literal|"dynamic new can only allocate objects of aggregate type"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
condition|)
block|{
comment|/* A "new dynamic" construct with filename argument.  */
if|if
condition|(
operator|!
name|IS_AGGR_TYPE
...
block|if (alignment) 	{ 	  rval = build (PLUS_EXPR, TYPE_POINTER_TO (type), rval, alignment); 	  rval = build (BIT_AND_EXPR, TYPE_POINTER_TO (type), 			rval, build1 (BIT_NOT_EXPR, integer_type_node, alignment)); 	}
endif|#
directive|endif
goto|goto
name|done
...
name|integer_zero_node
decl_stmt|;
comment|/* This variable is supposed to be the address of a "struct ref"      object, but how and where should it be defined?  */
name|tree
name|lookup_tmp
init|=
name|integer_zero_node
...
argument_list|)
expr_stmt|;
comment|/* This is one parameter which could be (but should not be) evaluated twice.  */
name|TREE_VALUE
argument_list|(
name|parms
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* SOS?? Pass the address of a temporary which can hold the pointer      to dynamic class table, but how and where is it defined? */
name|import_parms
operator|=
name|tree_cons
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* SOS?? Now, generate call to ctor, but using `import_ptr' as the function      table.  Return the result of the call to the ctor.  */
name|import_ptr
operator|=
name|build1
argument_list|(
...
begin_comment
comment|/* Return the name of the link table (as an IDENTIFIER_NODE)    for the given TYPE.  */
end_comment
...
begin_comment
comment|/* For a given type TYPE, grovel for a function table which    can be used to support dynamic linking.  */
end_comment
