...
begin_comment
comment|/* Reload pseudo regs into hard regs for insns that require hard regs.    Copyright (C) 1987, 1988, 1989, 1992 Free Software Foundation, Inc.  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
begin_comment
comment|/* This reg set indicates registers that are not good for spill registers.    They will not be used to complete groups of spill registers.  This includes    all fixed registers, registers that may be eliminated, and registers    explicitly used in the rtl.     (spill_reg_order prevents these registers from being used to start a    group.)  */
end_comment
...
comment|/* Initialize the optabs for doing special input and output reloads.  */
for|for
control|(
name|i
operator|=
...
index|]
operator|=
name|CODE_FOR_reload_inqi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_inhi
if|if
condition|(
name|HAVE_reload_inhi
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_inhi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_insi
if|if
condition|(
name|HAVE_reload_insi
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_insi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_indi
if|if
condition|(
name|HAVE_reload_indi
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_indi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_inti
if|if
condition|(
name|HAVE_reload_inti
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_inti
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_insf
if|if
condition|(
name|HAVE_reload_insf
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_insf
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_indf
if|if
condition|(
name|HAVE_reload_indf
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_indf
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_inxf
if|if
condition|(
name|HAVE_reload_inxf
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_inxf
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_intf
if|if
condition|(
name|HAVE_reload_intf
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_intf
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outqi
if|if
condition|(
name|HAVE_reload_outqi
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outqi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outhi
if|if
condition|(
name|HAVE_reload_outhi
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outhi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outsi
if|if
condition|(
name|HAVE_reload_outsi
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outsi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outdi
if|if
condition|(
name|HAVE_reload_outdi
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outdi
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outti
if|if
condition|(
name|HAVE_reload_outti
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outti
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outsf
if|if
condition|(
name|HAVE_reload_outsf
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outsf
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outdf
if|if
condition|(
name|HAVE_reload_outdf
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outdf
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outxf
if|if
condition|(
name|HAVE_reload_outxf
condition|)
...
index|]
operator|=
name|CODE_FOR_reload_outxf
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|HAVE_reload_outtf
if|if
condition|(
name|HAVE_reload_outtf
condition|)
...
argument_list|)
expr_stmt|;
comment|/* Round size of stack frame to BIGGEST_ALIGNMENT.  This must be done here      because the stack size may be a part of the offset computation for      register elimination.   */
...
directive|endif
comment|/* SMALL_REGISTER_CLASSES */
...
argument_list|)
expr_stmt|;
comment|/* First find all regs alone in their class 	 and count them (if desired) for non-groups. 	 We would be screwed if a group took the only reg in a class 	 for which a non-group reload is needed. 	 (Note there is still a bug; if a class has 2 regs, 	 both could be stolen by groups and we would lose the same way. 	 With luck, no machine will need a nongroup in a 2-reg class.)  */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Now count all spill regs against the individual need, 	 This includes those counted above for groups, 	 but not those previously counted for nongroups.  	 Those that weren't counted_for_groups can also count against 	 the not-in-group need.  */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Those counted at the beginning shouldn't be counted twice.  */
if|if
condition|(
operator|!
name|TEST_HARD_REG_BIT
...
argument_list|)
expr_stmt|;
comment|/* If we were able to eliminate the frame pointer, show that it is no      longer live at the start of any basic block.  If it is live by      virtue of being in a pseudo, that pseudo will be marked live      and hence the frame pointer will be known to be live via that      pseudo.  */
if|if
condition|(
operator|!
name|frame_pointer_needed
...
comment|/* Make a pass over all the insns and remove death notes for things that      are no longer registers or no longer die in the insn (e.g., an input      and output pseudo being tied).  */
for|for
control|(
name|insn
operator|=
...
begin_comment
comment|/* Count any groups that can be formed from the registers recently spilled.    This is done class by class, in order of ascending class number.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Skip to the last reg in this group.  When j is incremented 		 above, it will then point to the first reg of the next 		 possible group.  */
name|j
operator|+=
name|k
operator|-
...
name|ADDR_DIFF_VEC
case|:
comment|/* Each of the labels in the address vector must be at their initial 	 offsets.  We want the first first for ADDR_VEC and the second 	 field for ADDR_DIFF_VEC.  */
for|for
control|(
name|i
operator|=
...
comment|/* Otherwise, ensure NEW isn't shared in case we have to reload 	       it.  */
name|new
operator|=
name|copy_rtx
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/* If SET_DEST changed from a REG to a MEM and INSN is non-zero, 	   write a CLOBBER insn.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* In this case this insn isn't serving a useful purpose.  We 	       will delete it in reload_as_needed once we know that this 	       elimination is, in fact, being done.  	       If REPLACE isn't set, we can't delete this insn, but neededn't 	       process it since it won't be used unless something changes.  */
if|if
condition|(
name|replace
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If we had a move insn but now we don't, rerecognize it.  */
if|if
condition|(
operator|(
name|GET_CODE
...
name|class
decl_stmt|;
comment|/* If this block has not had spilling done for a 		 particular class, deactivate any optional reloads 		 of that class lest they try to use a spill-reg which isn't 		 available here.  If we have any non-optionals that need a 		 spill reg, abort.  */
for|for
control|(
name|class
operator|=
...
begin_comment
comment|/* If reg is in use for a RELOAD_FOR_INPUT_RELOAD_ADDRESS reload.  */
end_comment
...
begin_comment
comment|/* If reg is in use for a RELOAD_FOR_OUTPUT_RELOAD_ADDRESS reload.  */
end_comment
...
begin_comment
comment|/* If reg is in use for a RELOAD_FOR_INPUT reload.  */
end_comment
...
begin_comment
comment|/* If reg is in use for a RELOAD_FOR_OUTPUT reload.  */
end_comment
...
begin_comment
comment|/* Mark reg REGNO as in use for a reload of the sort spec'd by WHEN_NEEDED.    MODE is used to indicate how many consecutive regs are actually used.  */
end_comment
...
argument_list|)
return|;
comment|/* The other kinds of use can sometimes share a register.  */
case|case
name|RELOAD_FOR_INPUT
case|:
return|return
...
begin_comment
comment|/* Index of last register assigned as a spill register.  We allocate in    a round-robin fashio.  */
end_comment
...
name|insn
decl_stmt|;
comment|/* This argument is currently ignored.  */
name|rtx
name|avoid_return_reg
decl_stmt|;
block|{
...
argument_list|)
expr_stmt|;
comment|/* Distinguish output-only and input-only reloads      because they can overlap with other things.  */
for|for
control|(
name|j
operator|=
...
condition|)
block|{
comment|/* But earlyclobber operands must stay as RELOAD_OTHER.  */
for|for
control|(
name|i
operator|=
...
directive|ifdef
name|SMALL_REGISTER_CLASSES
block|int tem = (avoid_return_reg != 0);
endif|#
directive|endif
block|for (j = 0; j< n_reloads; j++)       if (! reload_optional[j]&& (reload_in[j] != 0 || reload_out[j] != 0 || reload_secondary_p[j])&& (reload_reg_rtx[j] == 0 	      || (! rtx_equal_p (reload_reg_rtx[j], reload_in[j])&& ! rtx_equal_p (reload_reg_rtx[j], reload_out[j])))) 	tem++;     if (tem> n_spills)       must_reuse = 1;   }
endif|#
directive|endif
ifdef|#
directive|ifdef
...
break|break;
block|}
comment|/* JRV: If the equiv register we have found is explicitly 		 clobbered in the current insn, mark but don't use, as above. */
if|if
condition|(
name|equiv
operator|!=
...
index|]
decl_stmt|;
comment|/* If this is a CALL_INSN preceded by USE insns, any reload insns      must go in front of the first USE insn, not in front of INSN.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
name|old
decl_stmt|;
comment|/* If OLDEQUIV is a pseudo with a MEM, get the real MEM 		     and similarly for OLD. 		     See comments in find_secondary_reload in reload.c.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
comment|/* A strict-low-part output operand needs to be reloaded 	     in the mode of the entire value.  */
if|if
condition|(
name|reload_strict_low
index|[
...
argument_list|)
expr_stmt|;
comment|/* Encapsulate OLD into that mode.  */
comment|/* If OLD is a subreg, then strip it, since the subreg will 		 be altered by this very reload.  */
while|while
condition|(
name|GET_CODE
argument_list|(
...
comment|/* If we need two reload regs, set RELOADREG to the intermediate 	     one, since it will be stored into OUT.  We might need a secondary 	     register only for an input reload, so check again here.  */
if|if
condition|(
name|reload_secondary_reload
index|[
...
begin_comment
comment|/* Emit code before BEFORE_INSN to perform an input reload of IN to RELOADREG.    Returns first insn emitted.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* How to do this reload can get quite tricky.  Normally, we are being      asked to reload a simple operand, such as a MEM, a constant, or a pseudo      register that didn't get a hard register.  In that case we can just      call emit_move_insn.       We can also be asked to reload a PLUS that adds either two registers or      a register and a constant or MEM.  This can occur during frame pointer      elimination.  That case if handled by trying to emit a single insn      to perform the add.  If it is not valid, we use a two insn sequence.       Finally, we could be called to handle an 'o' constraint by putting      an address into a register.  In that case, we first try to do this      with a named pattern of "reload_load_address".  If no such pattern      exists, we just emit a SET insn and hope for the best (it will normally      be valid on machines that use 'o').       This entire process is made complex because reload will never      process the insns we generate here and so we must ensure that      they will fit their constraints and also by the fact that parts of      IN might be being reloaded separately and replaced with spill registers.      Because of this, we are, in some sense, just guessing the right approach      here.  The one listed above seems to work.       ??? At some point, this whole thing needs to be rethought.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* If OP0 and OP1 are the same, we can use RELOADREG for OP1. 	 This fixes a problem on the 32K where the stack pointer cannot 	 be used as an operand of an add insn.  */
if|if
condition|(
name|rtx_equal_p
argument_list|(
...
directive|endif
comment|/* Otherwise, just write (set REGLOADREG IN) and hope for the best.  */
else|else
name|emit_insn_before
argument_list|(
name|gen_rtx
...
argument_list|)
expr_stmt|;
comment|/* Return the first insn emitted.      We can not just return PREV_INSN (before_insn), because there may have      been multiple instructions emitted.  Also note that gen_move_insn may      emit more than one insn itself, so we can not assume that there is one      insn emitted per emit_insn_before call.  */
return|return
name|NEXT_INSN
argument_list|(
name|prev_insn
...
argument_list|)
expr_stmt|;
comment|/* See if we can directly increment INCLOC.  Use a method similar to that      in gen_input_reload.  */
name|add_insn
operator|=
name|emit_insn_before
argument_list|(
