...
argument_list|)
expr_stmt|;
comment|/* USE of frame_pointer_rtx added for consistency; not clear if 	     really needed.  */
name|emit_insn
argument_list|(
name|gen_rtx
argument_list|(
...
literal|0
return|;
comment|/* Assignment to a cast usually results in a cast of a modify. 	 Don't complain about that.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
literal|0
return|;
comment|/* Sometimes it results in a cast of a cast of a modify. 	 Don't complain about that.  */
if|if
condition|(
operator|(
name|TREE_CODE
...
begin_comment
comment|/* The exception handling nesting looks like this:<-- Level N-1     {<-- exception handler block<-- Level N<-- in an exception handler 	{<-- try block 	:<-- in a TRY block 	:<-- in an exception handler 	: 	}  	{<-- except block 	:<-- in an except block 	:<-- in an exception handler 	: 	}      } */
end_comment

begin_comment
comment|/* Return nonzero iff in a try block at level LEVEL.  */
end_comment
...
begin_comment
comment|/* Return nonzero iff in an except block at level LEVEL.  */
end_comment
...
begin_comment
comment|/* Return nonzero iff in an exception handler at level LEVEL.  */
end_comment
...
begin_comment
comment|/* Record the fact that the current exception nesting raises    exception EX.  If not in an exception handler, return 0.  */
end_comment
...
begin_comment
comment|/* Start an `except' nesting contour.    EXITFLAG says whether this contour should be able to `exit' something.    ESCAPEFLAG says whether this contour should be escapable.  */
end_comment
...
begin_comment
comment|/* Generate code to `escape' from an exception contour.  This    is like `exiting', but does not conflict with constructs which    use `exit_label'.     Return nonzero if this contour is escapable, otherwise    return zero, and language-specific code will emit the    appropriate error message.  */
end_comment
...
begin_comment
comment|/* Finish processing and `except' contour.    Culls out all exceptions which might be raise but not    handled, and returns the list to the caller.    Language-specific code is responsible for dealing with these    exceptions.  */
end_comment
...
begin_comment
comment|/* Record that exception EX is caught by this exception handler.    Return nonzero if in exception handling construct, otherwise return 0.  */
end_comment
...
begin_comment
comment|/* Record that this exception handler catches all exceptions.    Return nonzero if in exception handling construct, otherwise return 0.  */
end_comment
...
argument_list|()
expr_stmt|;
comment|/* This is equivalent to calling alloca.  */
