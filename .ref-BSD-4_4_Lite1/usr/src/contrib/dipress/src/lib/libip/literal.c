...
begin_comment
comment|/*  *  Definitions for the primitives suggested in the Interpress standard  *  (XSIS 048302).  The following primitives are defined with macros in  *  "literal.h":  *  *	AppendComment  *	AppendIdentifier  *	AppendInsertFile  *	AppendString  */
end_comment

begin_comment
comment|/*  * Append a string but interpret the escape characters (different from  * 	"AppendString").  *  * Algorithum: copy the old string, substituting escaped characters.  */
end_comment
...
begin_comment
comment|/*  * append an Op Code to the end of the master  */
end_comment
...
begin_comment
comment|/*  * note that although the routine is called AppendInteger, it is really  * AppendLong.  This is because alot of code wants to use 32 bit numbers.  * If you want to pass it a 16bit int on the Vax, that will work too.  */
end_comment
...
begin_comment
comment|/*  * Append Packed Pixel Vector  *	if the data pointer is null, then just append the header and leave it  *	at that.  */
end_comment
...
argument|length
argument_list|,
argument|bitsPerPixel
argument_list|,
argument|pixelsPerScanLine
argument_list|,
argument|data
argument_list|)
end_macro
...
begin_comment
comment|/*  * Append a Compressed Pixel Vector  *	if the data pointer is null, then just append the header and leave it  *	at that.  */
end_comment
...
begin_comment
comment|/*  *  append_Sequence(type, length, buff)  *  *  Append a sequence descriptor and its data bytes.  The descriptor is of  *  type "type" and length "length".  "Buff" points to the buffer containing  *  the data.  If "Buff" is null, don't write any data.  */
end_comment
...
directive|endif
comment|/* check for use of an obsolete feature of this program */
if|if
condition|(
name|length
operator|<
...
begin_comment
comment|/* this routine assumes two's complement notation.  * this routine sometime over estimates the size of an integer. why?   * is it the sign bit that must be watched out for?  */
end_comment
...
condition|)
block|{
comment|/* not an Intepress or RES file -- initialize it */
name|append_bytes
argument_list|(
name|strlen
argument_list|(
...
begin_comment
comment|/*  *  res_select(fd) - select file descriptor "fd" as the RES file for  *		    later use by the i/o routines supplied in this library.  */
end_comment
...
condition|)
block|{
comment|/* not an RES or Interpress file -- initialize it */
name|append_bytes
argument_list|(
name|strlen
argument_list|(
