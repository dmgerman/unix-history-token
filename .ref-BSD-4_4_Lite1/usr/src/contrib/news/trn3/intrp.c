...
begin_comment
comment|/* $Id: intrp.c,v 3.0 1992/02/01 03:09:32 davison Trn $  */
end_comment
...
directive|else
operator|???
literal|"Define either HAS_GETPWENT or TILDENAME to get NEWS_ADMIN"
endif|#
directive|endif
comment|/* TILDENAME */
endif|#
directive|endif
comment|/* HAS_GETPWENT */
block|}
endif|#
directive|endif
comment|/* NEWS_ADMIN */
...
directive|ifdef
name|NEWS_ADMIN
comment|/* if this is the news admin than load his UID into newsuid */
if|if
condition|(
name|strEQ
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* host name that goes in postings (%H) */
name|phostname
operator|=
name|PHOSTNAME
expr_stmt|;
...
name|printf
argument_list|(
literal|"Warning: Couldn't open %s to determine hostname!\n"
argument_list|,
name|phostname
argument_list|)
expr_stmt|;
...
name|printf
argument_list|(
literal|"%s is an unknown user. Using default.\n"
argument_list|,
argument|tildename
argument_list|)
name|FLUSH
...
argument_list|)
expr_stmt|;
comment|/* no more than 3 prior references PLUS the 			** root article allowed, including the one 			** concatenated below */
if|if
condition|(
operator|(
name|s
...
name|i
expr_stmt|;
comment|/* adjust the size now. */
comment|/* A maze of twisty little conditions, all alike... */
if|if
condition|(
name|metabit
condition|)
block|{
comment|/* set meta bit while copying. */
name|i
operator|=
name|metabit
expr_stmt|;
...
condition|)
block|{
comment|/* put a backslash before regexp specials while copying. */
if|if
condition|(
name|s
operator|==
...
condition|)
block|{
comment|/* copy out so we can copy in. */
name|safecpy
argument_list|(
name|scrbuf
argument_list|,
