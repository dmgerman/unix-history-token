...
index|[]
init|=
literal|"$Header: main.c,v 2.3 87/03/23 12:29:53 ed Exp $"
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/* $Log:	main.c,v $  * Revision 2.3  87/03/23  12:29:53  ed  * Added -c switch to allow xnsftp commands to be specified directly  * on commandline.  *   * Revision 2.2  87/01/09  16:51:37  ed  * Use FilingSubset, if rejected attempt Filing  * Allows user override with -F switch  * Maintain FilingSubset mandatory attributes  * User niceties:  echo file name/type on transfer commands  * 		prompt on delete  * guess type which will determine file type implied by content  * New commands: (type related) Guess, Whatis  * 	      (file transfer) Copy, Move, Rename  *   * Revision 2.1  86/12/11  06:12:14  jqj  * Eliminated form, mode, and struct commands.  Started adding support for  * more file types.  *   * Revision 2.0  85/11/21  07:22:49  jqj  * 4.3BSD standard release  *   * Revision 1.1  85/05/27  06:31:05  jqj  * Initial revision  *   * Revision 1.1  85/05/27  06:31:05  jqj  * Initial revision  *   * Based on Berkeley tcp/ftp  */
end_comment
...
name|stderr
argument_list|,
literal|"xnsftp: %c: unknown option\n"
argument_list|,
operator|*
name|cp
argument_list|)
...
begin_comment
comment|/*  * Execute commands from command line.  *	command is of form  "command1 ; command2 ; command3 ; ..."  */
end_comment
...
name|endptr
condition|)
comment|/* semi as last character */
return|return
operator|(
literal|1
operator|)
