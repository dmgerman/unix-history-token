begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1986, 1991, 1993  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|copyright
index|[]
init|=
literal|"@(#) Copyright (c) 1986, 1991, 1993\n\ 	The Regents of the University of California.  All rights reserved.\n"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|sccsid
index|[]
init|=
literal|"@(#)iostat.c	8.3 (Berkeley) %G%"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* not lint */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/buf.h>
end_include

begin_include
include|#
directive|include
file|<sys/dkstat.h>
end_include

begin_include
include|#
directive|include
file|<err.h>
end_include

begin_include
include|#
directive|include
file|<ctype.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<kvm.h>
end_include

begin_include
include|#
directive|include
file|<limits.h>
end_include

begin_include
include|#
directive|include
file|<nlist.h>
end_include

begin_include
include|#
directive|include
file|<paths.h>
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_decl_stmt
name|struct
name|nlist
name|namelist
index|[]
init|=
block|{
define|#
directive|define
name|X_DK_TIME
value|0
block|{
literal|"_dk_time"
block|}
block|,
define|#
directive|define
name|X_DK_XFER
value|1
block|{
literal|"_dk_xfer"
block|}
block|,
define|#
directive|define
name|X_DK_WDS
value|2
block|{
literal|"_dk_wds"
block|}
block|,
define|#
directive|define
name|X_TK_NIN
value|3
block|{
literal|"_tk_nin"
block|}
block|,
define|#
directive|define
name|X_TK_NOUT
value|4
block|{
literal|"_tk_nout"
block|}
block|,
define|#
directive|define
name|X_DK_SEEK
value|5
block|{
literal|"_dk_seek"
block|}
block|,
define|#
directive|define
name|X_CP_TIME
value|6
block|{
literal|"_cp_time"
block|}
block|,
define|#
directive|define
name|X_DK_WPMS
value|7
block|{
literal|"_dk_wpms"
block|}
block|,
define|#
directive|define
name|X_HZ
value|8
block|{
literal|"_hz"
block|}
block|,
define|#
directive|define
name|X_STATHZ
value|9
block|{
literal|"_stathz"
block|}
block|,
define|#
directive|define
name|X_DK_NDRIVE
value|10
block|{
literal|"_dk_ndrive"
block|}
block|,
define|#
directive|define
name|X_END
value|10
if|#
directive|if
name|defined
argument_list|(
name|hp300
argument_list|)
operator|||
name|defined
argument_list|(
name|luna68k
argument_list|)
define|#
directive|define
name|X_HPDINIT
value|(X_END+1)
block|{
literal|"_hp_dinit"
block|}
block|,
endif|#
directive|endif
ifdef|#
directive|ifdef
name|mips
define|#
directive|define
name|X_SCSI_DINIT
value|(X_END+1)
block|{
literal|"_scsi_dinit"
block|}
block|,
endif|#
directive|endif
ifdef|#
directive|ifdef
name|tahoe
define|#
directive|define
name|X_VBDINIT
value|(X_END+1)
block|{
literal|"_vbdinit"
block|}
block|,
endif|#
directive|endif
ifdef|#
directive|ifdef
name|vax
block|{
literal|"_mbdinit"
block|}
block|,
define|#
directive|define
name|X_MBDINIT
value|(X_END+1)
block|{
literal|"_ubdinit"
block|}
block|,
define|#
directive|define
name|X_UBDINIT
value|(X_END+2)
endif|#
directive|endif
block|{
name|NULL
block|}
block|, }
decl_stmt|;
end_decl_stmt

begin_struct
struct|struct
name|_disk
block|{
name|long
name|cp_time
index|[
name|CPUSTATES
index|]
decl_stmt|;
name|long
modifier|*
name|dk_time
decl_stmt|;
name|long
modifier|*
name|dk_wds
decl_stmt|;
name|long
modifier|*
name|dk_seek
decl_stmt|;
name|long
modifier|*
name|dk_xfer
decl_stmt|;
name|long
name|tk_nin
decl_stmt|;
name|long
name|tk_nout
decl_stmt|;
block|}
name|cur
struct|,
name|last
struct|;
end_struct

begin_decl_stmt
name|kvm_t
modifier|*
name|kd
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|double
name|etime
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|long
modifier|*
name|dk_wpms
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|dk_ndrive
decl_stmt|,
modifier|*
name|dr_select
decl_stmt|,
name|hz
decl_stmt|,
name|kmemfd
decl_stmt|,
name|ndrives
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
modifier|*
name|dr_name
decl_stmt|;
end_decl_stmt

begin_define
define|#
directive|define
name|nlread
parameter_list|(
name|x
parameter_list|,
name|v
parameter_list|)
define|\
value|kvm_read(kd, namelist[x].n_value,&(v), sizeof(v))
end_define

begin_include
include|#
directive|include
file|"names.c"
end_include

begin_comment
comment|/* XXX */
end_comment

begin_decl_stmt
name|void
name|cpustats
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|dkstats
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|phdr
name|__P
argument_list|(
operator|(
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|void
name|usage
name|__P
argument_list|(
operator|(
name|void
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
specifier|register
name|int
name|i
decl_stmt|;
name|long
name|tmp
decl_stmt|;
name|int
name|ch
decl_stmt|,
name|hdrcnt
decl_stmt|,
name|reps
decl_stmt|,
name|interval
decl_stmt|,
name|stathz
decl_stmt|,
name|ndrives
decl_stmt|;
name|char
modifier|*
modifier|*
name|cp
decl_stmt|,
modifier|*
name|memf
decl_stmt|,
modifier|*
name|nlistf
decl_stmt|,
name|buf
index|[
literal|30
index|]
decl_stmt|;
name|char
name|errbuf
index|[
name|_POSIX2_LINE_MAX
index|]
decl_stmt|;
name|interval
operator|=
name|reps
operator|=
literal|0
expr_stmt|;
name|nlistf
operator|=
name|memf
operator|=
name|NULL
expr_stmt|;
while|while
condition|(
operator|(
name|ch
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"c:M:N:w:"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|ch
condition|)
block|{
case|case
literal|'c'
case|:
if|if
condition|(
operator|(
name|reps
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|<=
literal|0
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"repetition count<= 0."
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'M'
case|:
name|memf
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'N'
case|:
name|nlistf
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'w'
case|:
if|if
condition|(
operator|(
name|interval
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
operator|)
operator|<=
literal|0
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"interval<= 0."
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'?'
case|:
default|default:
name|usage
argument_list|()
expr_stmt|;
block|}
name|argc
operator|-=
name|optind
expr_stmt|;
name|argv
operator|+=
name|optind
expr_stmt|;
comment|/* 	 * Discard setgid privileges if not the running kernel so that bad 	 * guys can't print interesting stuff from kernel memory. 	 */
if|if
condition|(
name|nlistf
operator|!=
name|NULL
operator|||
name|memf
operator|!=
name|NULL
condition|)
name|setgid
argument_list|(
name|getgid
argument_list|()
argument_list|)
expr_stmt|;
name|kd
operator|=
name|kvm_openfiles
argument_list|(
name|nlistf
argument_list|,
name|memf
argument_list|,
name|NULL
argument_list|,
name|O_RDONLY
argument_list|,
name|errbuf
argument_list|)
expr_stmt|;
if|if
condition|(
name|kd
operator|==
literal|0
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"kvm_openfiles: %s"
argument_list|,
name|errbuf
argument_list|)
expr_stmt|;
if|if
condition|(
name|kvm_nlist
argument_list|(
name|kd
argument_list|,
name|namelist
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"kvm_nlist: %s"
argument_list|,
name|kvm_geterr
argument_list|(
name|kd
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|namelist
index|[
name|X_DK_NDRIVE
index|]
operator|.
name|n_type
operator|==
literal|0
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"dk_ndrive not found in namelist"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|nlread
argument_list|(
name|X_DK_NDRIVE
argument_list|,
name|dk_ndrive
argument_list|)
expr_stmt|;
if|if
condition|(
name|dk_ndrive
operator|<=
literal|0
condition|)
name|errx
argument_list|(
literal|1
argument_list|,
literal|"invalid dk_ndrive %d\n"
argument_list|,
name|dk_ndrive
argument_list|)
expr_stmt|;
name|cur
operator|.
name|dk_time
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|cur
operator|.
name|dk_wds
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|cur
operator|.
name|dk_seek
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|cur
operator|.
name|dk_xfer
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|last
operator|.
name|dk_time
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|last
operator|.
name|dk_wds
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|last
operator|.
name|dk_seek
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|last
operator|.
name|dk_xfer
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
name|dr_select
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|int
argument_list|)
argument_list|)
expr_stmt|;
name|dr_name
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|char
operator|*
argument_list|)
argument_list|)
expr_stmt|;
name|dk_wpms
operator|=
name|calloc
argument_list|(
name|dk_ndrive
argument_list|,
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dk_ndrive
condition|;
name|i
operator|++
control|)
block|{
operator|(
name|void
operator|)
name|sprintf
argument_list|(
name|buf
argument_list|,
literal|"dk%d"
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|dr_name
index|[
name|i
index|]
operator|=
name|strdup
argument_list|(
name|buf
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|read_names
argument_list|()
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|nlread
argument_list|(
name|X_HZ
argument_list|,
name|hz
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|nlread
argument_list|(
name|X_STATHZ
argument_list|,
name|stathz
argument_list|)
expr_stmt|;
if|if
condition|(
name|stathz
condition|)
name|hz
operator|=
name|stathz
expr_stmt|;
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_DK_WPMS
index|]
operator|.
name|n_value
argument_list|,
name|dk_wpms
argument_list|,
name|dk_ndrive
operator|*
sizeof|sizeof
argument_list|(
name|dk_wpms
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 	 * Choose drives to be displayed.  Priority goes to (in order) drives 	 * supplied as arguments and default drives.  If everything isn't 	 * filled in and there are drives not taken care of, display the first 	 * few that fit. 	 * 	 * The backward compatibility #ifdefs permit the syntax: 	 *	iostat [ drives ] [ interval [ count ] ] 	 */
define|#
directive|define
name|BACKWARD_COMPATIBILITY
for|for
control|(
name|ndrives
operator|=
literal|0
init|;
operator|*
name|argv
condition|;
operator|++
name|argv
control|)
block|{
ifdef|#
directive|ifdef
name|BACKWARD_COMPATIBILITY
if|if
condition|(
name|isdigit
argument_list|(
operator|*
operator|*
name|argv
argument_list|)
condition|)
break|break;
endif|#
directive|endif
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dk_ndrive
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|strcmp
argument_list|(
name|dr_name
index|[
name|i
index|]
argument_list|,
operator|*
name|argv
argument_list|)
condition|)
continue|continue;
name|dr_select
index|[
name|i
index|]
operator|=
literal|1
expr_stmt|;
operator|++
name|ndrives
expr_stmt|;
block|}
block|}
ifdef|#
directive|ifdef
name|BACKWARD_COMPATIBILITY
if|if
condition|(
operator|*
name|argv
condition|)
block|{
name|interval
operator|=
name|atoi
argument_list|(
operator|*
name|argv
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
operator|++
name|argv
condition|)
name|reps
operator|=
name|atoi
argument_list|(
operator|*
name|argv
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
if|if
condition|(
name|interval
condition|)
block|{
if|if
condition|(
operator|!
name|reps
condition|)
name|reps
operator|=
operator|-
literal|1
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|reps
condition|)
name|interval
operator|=
literal|1
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dk_ndrive
operator|&&
name|ndrives
operator|<
literal|4
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|dr_select
index|[
name|i
index|]
operator|||
name|dk_wpms
index|[
name|i
index|]
operator|==
literal|0
condition|)
continue|continue;
for|for
control|(
name|cp
operator|=
name|defdrives
init|;
operator|*
name|cp
condition|;
name|cp
operator|++
control|)
if|if
condition|(
name|strcmp
argument_list|(
name|dr_name
index|[
name|i
index|]
argument_list|,
operator|*
name|cp
argument_list|)
operator|==
literal|0
condition|)
block|{
name|dr_select
index|[
name|i
index|]
operator|=
literal|1
expr_stmt|;
operator|++
name|ndrives
expr_stmt|;
break|break;
block|}
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dk_ndrive
operator|&&
name|ndrives
operator|<
literal|4
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|dr_select
index|[
name|i
index|]
condition|)
continue|continue;
name|dr_select
index|[
name|i
index|]
operator|=
literal|1
expr_stmt|;
operator|++
name|ndrives
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|signal
argument_list|(
name|SIGCONT
argument_list|,
name|phdr
argument_list|)
expr_stmt|;
for|for
control|(
name|hdrcnt
operator|=
literal|1
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|!
operator|--
name|hdrcnt
condition|)
block|{
name|phdr
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|hdrcnt
operator|=
literal|20
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_DK_TIME
index|]
operator|.
name|n_value
argument_list|,
name|cur
operator|.
name|dk_time
argument_list|,
name|dk_ndrive
operator|*
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_DK_XFER
index|]
operator|.
name|n_value
argument_list|,
name|cur
operator|.
name|dk_xfer
argument_list|,
name|dk_ndrive
operator|*
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_DK_WDS
index|]
operator|.
name|n_value
argument_list|,
name|cur
operator|.
name|dk_wds
argument_list|,
name|dk_ndrive
operator|*
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_DK_SEEK
index|]
operator|.
name|n_value
argument_list|,
name|cur
operator|.
name|dk_seek
argument_list|,
name|dk_ndrive
operator|*
sizeof|sizeof
argument_list|(
name|long
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_TK_NIN
index|]
operator|.
name|n_value
argument_list|,
operator|&
name|cur
operator|.
name|tk_nin
argument_list|,
sizeof|sizeof
argument_list|(
name|cur
operator|.
name|tk_nin
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_TK_NOUT
index|]
operator|.
name|n_value
argument_list|,
operator|&
name|cur
operator|.
name|tk_nout
argument_list|,
sizeof|sizeof
argument_list|(
name|cur
operator|.
name|tk_nout
argument_list|)
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|kvm_read
argument_list|(
name|kd
argument_list|,
name|namelist
index|[
name|X_CP_TIME
index|]
operator|.
name|n_value
argument_list|,
name|cur
operator|.
name|cp_time
argument_list|,
sizeof|sizeof
argument_list|(
name|cur
operator|.
name|cp_time
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dk_ndrive
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|!
name|dr_select
index|[
name|i
index|]
condition|)
continue|continue;
define|#
directive|define
name|X
parameter_list|(
name|fld
parameter_list|)
value|tmp = cur.fld[i]; cur.fld[i] -= last.fld[i]; last.fld[i] = tmp
name|X
argument_list|(
name|dk_xfer
argument_list|)
expr_stmt|;
name|X
argument_list|(
name|dk_seek
argument_list|)
expr_stmt|;
name|X
argument_list|(
name|dk_wds
argument_list|)
expr_stmt|;
name|X
argument_list|(
name|dk_time
argument_list|)
expr_stmt|;
block|}
name|tmp
operator|=
name|cur
operator|.
name|tk_nin
expr_stmt|;
name|cur
operator|.
name|tk_nin
operator|-=
name|last
operator|.
name|tk_nin
expr_stmt|;
name|last
operator|.
name|tk_nin
operator|=
name|tmp
expr_stmt|;
name|tmp
operator|=
name|cur
operator|.
name|tk_nout
expr_stmt|;
name|cur
operator|.
name|tk_nout
operator|-=
name|last
operator|.
name|tk_nout
expr_stmt|;
name|last
operator|.
name|tk_nout
operator|=
name|tmp
expr_stmt|;
name|etime
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|CPUSTATES
condition|;
name|i
operator|++
control|)
block|{
name|X
argument_list|(
name|cp_time
argument_list|)
expr_stmt|;
name|etime
operator|+=
name|cur
operator|.
name|cp_time
index|[
name|i
index|]
expr_stmt|;
block|}
if|if
condition|(
name|etime
operator|==
literal|0.0
condition|)
name|etime
operator|=
literal|1.0
expr_stmt|;
name|etime
operator|/=
operator|(
name|float
operator|)
name|hz
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%4.0f%5.0f"
argument_list|,
name|cur
operator|.
name|tk_nin
operator|/
name|etime
argument_list|,
name|cur
operator|.
name|tk_nout
operator|/
name|etime
argument_list|)
expr_stmt|;
name|dkstats
argument_list|()
expr_stmt|;
name|cpustats
argument_list|()
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
if|if
condition|(
name|reps
operator|>=
literal|0
operator|&&
operator|--
name|reps
operator|<=
literal|0
condition|)
break|break;
operator|(
name|void
operator|)
name|sleep
argument_list|(
name|interval
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* ARGUSED */
end_comment

begin_function
name|void
name|phdr
parameter_list|(
name|signo
parameter_list|)
name|int
name|signo
decl_stmt|;
block|{
specifier|register
name|int
name|i
decl_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"      tty"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dk_ndrive
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|dr_select
index|[
name|i
index|]
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"          %3.3s "
argument_list|,
name|dr_name
index|[
name|i
index|]
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"         cpu\n tin tout"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|dk_ndrive
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|dr_select
index|[
name|i
index|]
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|" sps tps msps "
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|" us ni sy in id\n"
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|dkstats
parameter_list|()
block|{
specifier|register
name|int
name|dn
decl_stmt|;
name|double
name|atime
decl_stmt|,
name|itime
decl_stmt|,
name|msps
decl_stmt|,
name|words
decl_stmt|,
name|xtime
decl_stmt|;
for|for
control|(
name|dn
operator|=
literal|0
init|;
name|dn
operator|<
name|dk_ndrive
condition|;
operator|++
name|dn
control|)
block|{
if|if
condition|(
operator|!
name|dr_select
index|[
name|dn
index|]
condition|)
continue|continue;
name|words
operator|=
name|cur
operator|.
name|dk_wds
index|[
name|dn
index|]
operator|*
literal|32
expr_stmt|;
comment|/* words xfer'd */
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%4.0f"
argument_list|,
comment|/* sectors */
name|words
operator|/
operator|(
name|DEV_BSIZE
operator|/
literal|2
operator|)
operator|/
name|etime
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%4.0f"
argument_list|,
name|cur
operator|.
name|dk_xfer
index|[
name|dn
index|]
operator|/
name|etime
argument_list|)
expr_stmt|;
if|if
condition|(
name|dk_wpms
index|[
name|dn
index|]
operator|&&
name|cur
operator|.
name|dk_xfer
index|[
name|dn
index|]
condition|)
block|{
name|atime
operator|=
name|cur
operator|.
name|dk_time
index|[
name|dn
index|]
expr_stmt|;
comment|/* ticks disk busy */
name|atime
operator|/=
operator|(
name|float
operator|)
name|hz
expr_stmt|;
comment|/* ticks to seconds */
name|xtime
operator|=
name|words
operator|/
name|dk_wpms
index|[
name|dn
index|]
expr_stmt|;
comment|/* transfer time */
name|itime
operator|=
name|atime
operator|-
name|xtime
expr_stmt|;
comment|/* time not xfer'ing */
if|if
condition|(
name|itime
operator|<
literal|0
condition|)
name|msps
operator|=
literal|0
expr_stmt|;
else|else
name|msps
operator|=
name|itime
operator|*
literal|1000
operator|/
name|cur
operator|.
name|dk_xfer
index|[
name|dn
index|]
expr_stmt|;
block|}
else|else
name|msps
operator|=
literal|0
expr_stmt|;
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%5.1f "
argument_list|,
name|msps
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_function
name|void
name|cpustats
parameter_list|()
block|{
specifier|register
name|int
name|state
decl_stmt|;
name|double
name|time
decl_stmt|;
name|time
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|state
operator|=
literal|0
init|;
name|state
operator|<
name|CPUSTATES
condition|;
operator|++
name|state
control|)
name|time
operator|+=
name|cur
operator|.
name|cp_time
index|[
name|state
index|]
expr_stmt|;
for|for
control|(
name|state
operator|=
literal|0
init|;
name|state
operator|<
name|CPUSTATES
condition|;
operator|++
name|state
control|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%3.0f"
argument_list|,
literal|100.
operator|*
name|cur
operator|.
name|cp_time
index|[
name|state
index|]
operator|/
operator|(
name|time
condition|?
name|time
else|:
literal|1
operator|)
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|void
name|usage
parameter_list|()
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: iostat [-c count] [-M core] [-N system] [-w wait] [drives]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

