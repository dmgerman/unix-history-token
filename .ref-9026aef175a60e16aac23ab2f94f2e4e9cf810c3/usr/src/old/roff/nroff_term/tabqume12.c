begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * %sccs.include.proprietary.c%  *  *	@(#)tabqume12.c	4.3 (Berkeley) %G%  */
end_comment

begin_define
define|#
directive|define
name|INCH
value|240
end_define

begin_comment
comment|/*  * QUME 12 Pitch  * nroff driving table  * ASCII Prestige Elite 12 M2167  * by Albert Einstein College of Medicine  */
end_comment

begin_struct
struct|struct
block|{
name|int
name|bset
decl_stmt|;
name|int
name|breset
decl_stmt|;
name|int
name|Hor
decl_stmt|;
name|int
name|Vert
decl_stmt|;
name|int
name|Newline
decl_stmt|;
name|int
name|Char
decl_stmt|;
name|int
name|Em
decl_stmt|;
name|int
name|Halfline
decl_stmt|;
name|int
name|Adj
decl_stmt|;
name|char
modifier|*
name|twinit
decl_stmt|;
name|char
modifier|*
name|twrest
decl_stmt|;
name|char
modifier|*
name|twnl
decl_stmt|;
name|char
modifier|*
name|hlr
decl_stmt|;
name|char
modifier|*
name|hlf
decl_stmt|;
name|char
modifier|*
name|flr
decl_stmt|;
name|char
modifier|*
name|bdon
decl_stmt|;
name|char
modifier|*
name|bdoff
decl_stmt|;
name|char
modifier|*
name|ploton
decl_stmt|;
name|char
modifier|*
name|plotoff
decl_stmt|;
name|char
modifier|*
name|up
decl_stmt|;
name|char
modifier|*
name|down
decl_stmt|;
name|char
modifier|*
name|right
decl_stmt|;
name|char
modifier|*
name|left
decl_stmt|;
name|char
modifier|*
name|codetab
index|[
literal|256
operator|-
literal|32
index|]
decl_stmt|;
name|int
name|zzz
decl_stmt|;
block|}
name|t
init|=
block|{
comment|/*bset*/
literal|0
block|,
comment|/*breset*/
literal|0177420
block|,
comment|/*Hor*/
name|INCH
operator|/
literal|120
block|,
comment|/*Vert*/
name|INCH
operator|/
literal|48
block|,
comment|/*Newline*/
name|INCH
operator|/
literal|6
block|,
comment|/*Char*/
name|INCH
operator|/
literal|12
block|,
comment|/*Em*/
name|INCH
operator|/
literal|12
block|,
comment|/*Halfline*/
name|INCH
operator|/
literal|12
block|,
comment|/*Adj*/
name|INCH
operator|/
literal|12
block|,
comment|/*twinit*/
literal|"\0334\033\037\013"
block|,
comment|/*plot off, horiz. motion = 10/120 inch*/
comment|/*twrest*/
literal|"\0334\033\037\013"
block|,
comment|/*twnl*/
literal|"\r\n"
block|,
comment|/*hlr*/
literal|"\033D"
block|,
comment|/*hlf*/
literal|"\033U"
block|,
comment|/*flr*/
literal|"\033\n"
block|,
comment|/*bdon*/
literal|""
block|,
comment|/*bdoff*/
literal|""
block|,
comment|/*ploton*/
literal|"\033G"
block|,
comment|/*plotoff*/
literal|"\0334"
block|,
comment|/*up*/
literal|"\033\n"
block|,
comment|/*down*/
literal|"\n"
block|,
comment|/*right*/
literal|" "
block|,
comment|/*left*/
literal|"\b"
block|,
comment|/*codetab*/
literal|"\001 "
block|,
comment|/*space*/
literal|"\001!"
block|,
comment|/*!*/
literal|"\001\""
block|,
comment|/*"*/
literal|"\001#"
block|,
comment|/*#*/
literal|"\001$"
block|,
comment|/*$*/
literal|"\001%"
block|,
comment|/*%*/
literal|"\001&"
block|,
comment|/*&*/
literal|"\001'"
block|,
comment|/*' close*/
literal|"\001("
block|,
comment|/*(*/
literal|"\001)"
block|,
comment|/*)*/
literal|"\001*"
block|,
comment|/***/
literal|"\001+"
block|,
comment|/*+*/
literal|"\001,"
block|,
comment|/*,*/
literal|"\001-"
block|,
comment|/*- hyphen*/
literal|"\001."
block|,
comment|/*.*/
literal|"\001/"
block|,
comment|/*/*/
literal|"\2010"
block|,
comment|/*0*/
literal|"\2011"
block|,
comment|/*1*/
literal|"\2012"
block|,
comment|/*2*/
literal|"\2013"
block|,
comment|/*3*/
literal|"\2014"
block|,
comment|/*4*/
literal|"\2015"
block|,
comment|/*5*/
literal|"\2016"
block|,
comment|/*6*/
literal|"\2017"
block|,
comment|/*7*/
literal|"\2018"
block|,
comment|/*8*/
literal|"\2019"
block|,
comment|/*9*/
literal|"\001:"
block|,
comment|/*:*/
literal|"\001;"
block|,
comment|/*;*/
literal|"\001<"
block|,
comment|/*<*/
literal|"\001="
block|,
comment|/*=*/
literal|"\001>"
block|,
comment|/*>*/
literal|"\001?"
block|,
comment|/*?*/
literal|"\001@"
block|,
comment|/*@*/
literal|"\201A"
block|,
comment|/*A*/
literal|"\201B"
block|,
comment|/*B*/
literal|"\201C"
block|,
comment|/*C*/
literal|"\201D"
block|,
comment|/*D*/
literal|"\201E"
block|,
comment|/*E*/
literal|"\201F"
block|,
comment|/*F*/
literal|"\201G"
block|,
comment|/*G*/
literal|"\201H"
block|,
comment|/*H*/
literal|"\201I"
block|,
comment|/*I*/
literal|"\201J"
block|,
comment|/*J*/
literal|"\201K"
block|,
comment|/*K*/
literal|"\201L"
block|,
comment|/*L*/
literal|"\201M"
block|,
comment|/*M*/
literal|"\201N"
block|,
comment|/*N*/
literal|"\201O"
block|,
comment|/*O*/
literal|"\201P"
block|,
comment|/*P*/
literal|"\201Q"
block|,
comment|/*Q*/
literal|"\201R"
block|,
comment|/*R*/
literal|"\201S"
block|,
comment|/*S*/
literal|"\201T"
block|,
comment|/*T*/
literal|"\201U"
block|,
comment|/*U*/
literal|"\201V"
block|,
comment|/*V*/
literal|"\201W"
block|,
comment|/*W*/
literal|"\201X"
block|,
comment|/*X*/
literal|"\201Y"
block|,
comment|/*Y*/
literal|"\201Z"
block|,
comment|/*Z*/
literal|"\001["
block|,
comment|/*[*/
literal|"\001\\"
block|,
comment|/*\*/
literal|"\001]"
block|,
comment|/*]*/
literal|"\001^"
block|,
comment|/*^*/
literal|"\001_"
block|,
comment|/*_ dash*/
literal|"\001`"
block|,
comment|/*` open*/
literal|"\201a"
block|,
comment|/*a*/
literal|"\201b"
block|,
comment|/*b*/
literal|"\201c"
block|,
comment|/*c*/
literal|"\201d"
block|,
comment|/*d*/
literal|"\201e"
block|,
comment|/*e*/
literal|"\201f"
block|,
comment|/*f*/
literal|"\201g"
block|,
comment|/*g*/
literal|"\201h"
block|,
comment|/*h*/
literal|"\201i"
block|,
comment|/*i*/
literal|"\201j"
block|,
comment|/*j*/
literal|"\201k"
block|,
comment|/*k*/
literal|"\201l"
block|,
comment|/*l*/
literal|"\201m"
block|,
comment|/*m*/
literal|"\201n"
block|,
comment|/*n*/
literal|"\201o"
block|,
comment|/*o*/
literal|"\201p"
block|,
comment|/*p*/
literal|"\201q"
block|,
comment|/*q*/
literal|"\201r"
block|,
comment|/*r*/
literal|"\201s"
block|,
comment|/*s*/
literal|"\201t"
block|,
comment|/*t*/
literal|"\201u"
block|,
comment|/*u*/
literal|"\201v"
block|,
comment|/*v*/
literal|"\201w"
block|,
comment|/*w*/
literal|"\201x"
block|,
comment|/*x*/
literal|"\201y"
block|,
comment|/*y*/
literal|"\201z"
block|,
comment|/*z*/
literal|"\001{"
block|,
comment|/*{*/
literal|"\001|"
block|,
comment|/*|*/
literal|"\001}"
block|,
comment|/*}*/
literal|"\001~"
block|,
comment|/*~*/
literal|"\000\0"
block|,
comment|/*narrow sp*/
literal|"\001-"
block|,
comment|/*hyphen*/
literal|"\0010\b+"
block|,
comment|/*bullet*/
literal|"\000\0"
block|,
comment|/*square*/
literal|"\001-"
block|,
comment|/*3/4 em*/
literal|"\001_"
block|,
comment|/*rule*/
literal|"\000\0"
block|,
comment|/*1/4*/
literal|"\000\0"
block|,
comment|/*1/2*/
literal|"\000\0"
block|,
comment|/*3/4*/
literal|"\001-"
block|,
comment|/*minus*/
literal|"\202fi"
block|,
comment|/*fi*/
literal|"\202fl"
block|,
comment|/*fl*/
literal|"\202ff"
block|,
comment|/*ff*/
literal|"\203ffi"
block|,
comment|/*ffi*/
literal|"\203ffl"
block|,
comment|/*ffl*/
literal|"\000\0"
block|,
comment|/*degree*/
literal|"\000\0"
block|,
comment|/*dagger*/
literal|"\000\0"
block|,
comment|/*section*/
literal|"\001'"
block|,
comment|/*foot mark*/
literal|"\001'"
block|,
comment|/*acute accent*/
literal|"\001`"
block|,
comment|/*grave accent*/
literal|"\001_"
block|,
comment|/*underrule*/
literal|"\001/"
block|,
comment|/*slash (longer)*/
literal|"\000\0"
block|,
comment|/*half narrow space*/
literal|"\001 "
block|,
comment|/*unpaddable space*/
literal|"\000\0"
block|,
comment|/*alpha*/
literal|"\000\0"
block|,
comment|/*beta*/
literal|"\000\0"
block|,
comment|/*gamma*/
literal|"\000\0"
block|,
comment|/*delta*/
literal|"\000\0"
block|,
comment|/*epsilon*/
literal|"\000\0"
block|,
comment|/*zeta*/
literal|"\000\0"
block|,
comment|/*eta*/
literal|"\000\0"
block|,
comment|/*theta*/
literal|"\000\0"
block|,
comment|/*iota*/
literal|"\000\0"
block|,
comment|/*kappa*/
literal|"\000\0"
block|,
comment|/*lambda*/
literal|"\000\0"
block|,
comment|/*mu*/
literal|"\000\0"
block|,
comment|/*nu*/
literal|"\000\0"
block|,
comment|/*xi*/
literal|"\000\0"
block|,
comment|/*omicron*/
literal|"\000\0"
block|,
comment|/*pi*/
literal|"\000\0"
block|,
comment|/*rho*/
literal|"\000\0"
block|,
comment|/*sigma*/
literal|"\000\0"
block|,
comment|/*tau*/
literal|"\000\0"
block|,
comment|/*upsilon*/
literal|"\000\0"
block|,
comment|/*phi*/
literal|"\000\0"
block|,
comment|/*chi*/
literal|"\000\0"
block|,
comment|/*psi*/
literal|"\000\0"
block|,
comment|/*omega*/
literal|"\000\0"
block|,
comment|/*Gamma*/
literal|"\000\0"
block|,
comment|/*Delta*/
literal|"\000\0"
block|,
comment|/*Theta*/
literal|"\000\0"
block|,
comment|/*Lambda*/
literal|"\000\0"
block|,
comment|/*Xi*/
literal|"\000\0"
block|,
comment|/*Pi*/
literal|"\000\0"
block|,
comment|/*Sigma*/
literal|"\000\0"
block|,
comment|/**/
literal|"\000\0"
block|,
comment|/*Upsilon*/
literal|"\000\0"
block|,
comment|/*Phi*/
literal|"\000\0"
block|,
comment|/*Psi*/
literal|"\000\0"
block|,
comment|/*Omega*/
literal|"\000\0"
block|,
comment|/*square root*/
literal|"\000\0"
block|,
comment|/*terminal sigma*/
literal|"\000\0"
block|,
comment|/*root en*/
literal|"\001>\b_"
block|,
comment|/*>=*/
literal|"\001<\b_"
block|,
comment|/*<=*/
literal|"\000\0"
block|,
comment|/*identically equal*/
literal|"\001-"
block|,
comment|/*equation minus*/
literal|"\001~\b_"
block|,
comment|/*approx =*/
literal|"\001~"
block|,
comment|/*approximates*/
literal|"\001=\b/"
block|,
comment|/*not equal*/
literal|"\002->"
block|,
comment|/*right arrow*/
literal|"\002<-"
block|,
comment|/*left arrow*/
literal|"\000\0"
block|,
comment|/*up arrow*/
literal|"\000\0"
block|,
comment|/*down arrow*/
literal|"\001="
block|,
comment|/*equation equal*/
literal|"\001*"
block|,
comment|/*multiply*/
literal|"\001/"
block|,
comment|/*divide*/
literal|"\001+\b_"
block|,
comment|/*plus-minus*/
literal|"\001U"
block|,
comment|/*cup (union)*/
literal|"\000\0"
block|,
comment|/*cap (intersection)*/
literal|"\000\0"
block|,
comment|/*subset of*/
literal|"\000\0"
block|,
comment|/*superset of*/
literal|"\000\0"
block|,
comment|/*improper subset*/
literal|"\000\0"
block|,
comment|/* improper superset*/
literal|"\000\0"
block|,
comment|/*infinity*/
literal|"\000\0"
block|,
comment|/*partial derivative*/
literal|"\000\0"
block|,
comment|/*gradient*/
literal|"\000\0"
block|,
comment|/*not*/
literal|"\000\0"
block|,
comment|/*integral sign*/
literal|"\000\0"
block|,
comment|/*proportional to*/
literal|"\0010\b/"
block|,
comment|/*empty set*/
literal|"\001<\b-"
block|,
comment|/*member of*/
literal|"\001+"
block|,
comment|/*equation plus*/
literal|"\000\0"
block|,
comment|/*registered*/
literal|"\000\0"
block|,
comment|/*copyright*/
literal|"\001|"
block|,
comment|/*box rule */
literal|"\001\033 "
block|,
comment|/*cent sign*/
literal|"\000\0"
block|,
comment|/*dbl dagger*/
literal|"\000\0"
block|,
comment|/*right hand*/
literal|"\000\0"
block|,
comment|/*left hand*/
literal|"\001*"
block|,
comment|/*math * */
literal|"\000\0"
block|,
comment|/*bell system sign*/
literal|"\001|"
block|,
comment|/*or (was star)*/
literal|"\000\0"
block|,
comment|/*circle*/
literal|"\000\0"
block|,
comment|/*left top (of big curly)*/
literal|"\000\0"
block|,
comment|/*left bottom*/
literal|"\000\0"
block|,
comment|/*right top*/
literal|"\000\0"
block|,
comment|/*right bot*/
literal|"\000\0"
block|,
comment|/*left center of big curly bracket*/
literal|"\000\0"
block|,
comment|/*right center of big curly bracket*/
literal|"\000\0"
block|,
comment|/*bold vertical*/
literal|"\000\0"
block|,
comment|/*left floor (left bot of big sq bract)*/
literal|"\000\0"
block|,
comment|/*right floor (rb of ")*/
literal|"\000\0"
block|,
comment|/*left ceiling (lt of ")*/
literal|"\000\0"
block|,
comment|/*right ceiling (rt of ")*/
literal|"\000\0"
block|,
comment|/*superscript 0	 */
literal|"\000\0"
block|,
comment|/*superscript 1	 */
literal|"\000\0"
block|,
comment|/*superscript 2	 */
literal|"\000\0"
block|,
comment|/*superscript 3	 */
literal|"\000\0"
block|,
comment|/*superscript 4	 */
literal|"\000\0"
block|,
comment|/*superscript 5	 */
literal|"\000\0"
block|,
comment|/*superscript 6	 */
literal|"\000\0"
block|,
comment|/*superscript 7	 */
literal|"\000\0"
block|,
comment|/*superscript 8	 */
literal|0
block|,
literal|0
block|,
literal|0
block|,
comment|/*0374, 0375, 0376 cannot be accessed */
literal|"\000\0"
block|,
comment|/*superscript 9	 */
comment|/*"\001\016;\017",		umlaut	dotdot	*/
block|}
struct|;
end_struct

end_unit

