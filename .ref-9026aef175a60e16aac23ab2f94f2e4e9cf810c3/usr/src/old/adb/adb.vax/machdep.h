...
begin_comment
comment|/*-  * Copyright (c) 1988 The Regents of the University of California.  * All rights reserved.  *  * %sccs.include.proprietary.c%  *  *	@(#)machdep.h	5.3 (Berkeley) %G%  */
end_comment
...
begin_comment
comment|/*  * The reglist structure holds information needed to set and examine  * registers.  It must contain an r_name field; this name must be unique  * across the register set, cannot be a single letter or digit, and  * cannot be a substring of any other register name.  *  * On the VAX, we keep an offset into the u. area, either from the  * base of the u. area (in the pcb), or, for those registers that  * are saved by syscalls, in the save area pointed to by u.u_ar0.  * Offsets into the latter region are negative.  *  * We also keep a pointer into the current pcb for use when debugging  * the kernel.  */
end_comment
