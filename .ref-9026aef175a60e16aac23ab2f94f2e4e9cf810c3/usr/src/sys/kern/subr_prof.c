begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*-  * Copyright (c) 1982, 1986, 1993  *	The Regents of the University of California.  All rights reserved.  *  * %sccs.include.redist.c%  *  *	@(#)subr_prof.c	8.4 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/user.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/syscallargs.h>
end_include

begin_include
include|#
directive|include
file|<machine/cpu.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|GPROF
end_ifdef

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/gmon.h>
end_include

begin_comment
comment|/*  * Froms is actually a bunch of unsigned shorts indexing tos  */
end_comment

begin_decl_stmt
name|struct
name|gmonparam
name|_gmonparam
init|=
block|{
name|GMON_PROF_OFF
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|char
name|etext
index|[]
decl_stmt|;
end_decl_stmt

begin_function
name|void
name|kmstartup
parameter_list|()
block|{
name|char
modifier|*
name|cp
decl_stmt|;
name|struct
name|gmonparam
modifier|*
name|p
init|=
operator|&
name|_gmonparam
decl_stmt|;
comment|/* 	 * Round lowpc and highpc to multiples of the density we're using 	 * so the rest of the scaling (here and in gprof) stays in ints. 	 */
name|p
operator|->
name|lowpc
operator|=
name|ROUNDDOWN
argument_list|(
name|KERNBASE
argument_list|,
name|HISTFRACTION
operator|*
sizeof|sizeof
argument_list|(
name|HISTCOUNTER
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|->
name|highpc
operator|=
name|ROUNDUP
argument_list|(
operator|(
name|u_long
operator|)
name|etext
argument_list|,
name|HISTFRACTION
operator|*
sizeof|sizeof
argument_list|(
name|HISTCOUNTER
argument_list|)
argument_list|)
expr_stmt|;
name|p
operator|->
name|textsize
operator|=
name|p
operator|->
name|highpc
operator|-
name|p
operator|->
name|lowpc
expr_stmt|;
name|printf
argument_list|(
literal|"Profiling kernel, textsize=%d [%x..%x]\n"
argument_list|,
name|p
operator|->
name|textsize
argument_list|,
name|p
operator|->
name|lowpc
argument_list|,
name|p
operator|->
name|highpc
argument_list|)
expr_stmt|;
name|p
operator|->
name|kcountsize
operator|=
name|p
operator|->
name|textsize
operator|/
name|HISTFRACTION
expr_stmt|;
name|p
operator|->
name|hashfraction
operator|=
name|HASHFRACTION
expr_stmt|;
name|p
operator|->
name|fromssize
operator|=
name|p
operator|->
name|textsize
operator|/
name|HASHFRACTION
expr_stmt|;
name|p
operator|->
name|tolimit
operator|=
name|p
operator|->
name|textsize
operator|*
name|ARCDENSITY
operator|/
literal|100
expr_stmt|;
if|if
condition|(
name|p
operator|->
name|tolimit
operator|<
name|MINARCS
condition|)
name|p
operator|->
name|tolimit
operator|=
name|MINARCS
expr_stmt|;
elseif|else
if|if
condition|(
name|p
operator|->
name|tolimit
operator|>
name|MAXARCS
condition|)
name|p
operator|->
name|tolimit
operator|=
name|MAXARCS
expr_stmt|;
name|p
operator|->
name|tossize
operator|=
name|p
operator|->
name|tolimit
operator|*
sizeof|sizeof
argument_list|(
expr|struct
name|tostruct
argument_list|)
expr_stmt|;
name|cp
operator|=
operator|(
name|char
operator|*
operator|)
name|malloc
argument_list|(
name|p
operator|->
name|kcountsize
operator|+
name|p
operator|->
name|fromssize
operator|+
name|p
operator|->
name|tossize
argument_list|,
name|M_GPROF
argument_list|,
name|M_NOWAIT
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp
operator|==
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"No memory for profiling.\n"
argument_list|)
expr_stmt|;
return|return;
block|}
name|bzero
argument_list|(
name|cp
argument_list|,
name|p
operator|->
name|kcountsize
operator|+
name|p
operator|->
name|tossize
operator|+
name|p
operator|->
name|fromssize
argument_list|)
expr_stmt|;
name|p
operator|->
name|tos
operator|=
operator|(
expr|struct
name|tostruct
operator|*
operator|)
name|cp
expr_stmt|;
name|cp
operator|+=
name|p
operator|->
name|tossize
expr_stmt|;
name|p
operator|->
name|kcount
operator|=
operator|(
name|u_short
operator|*
operator|)
name|cp
expr_stmt|;
name|cp
operator|+=
name|p
operator|->
name|kcountsize
expr_stmt|;
name|p
operator|->
name|froms
operator|=
operator|(
name|u_short
operator|*
operator|)
name|cp
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Return kernel profiling information.  */
end_comment

begin_function
name|int
name|sysctl_doprof
parameter_list|(
name|name
parameter_list|,
name|namelen
parameter_list|,
name|oldp
parameter_list|,
name|oldlenp
parameter_list|,
name|newp
parameter_list|,
name|newlen
parameter_list|,
name|p
parameter_list|)
name|int
modifier|*
name|name
decl_stmt|;
name|u_int
name|namelen
decl_stmt|;
name|void
modifier|*
name|oldp
decl_stmt|;
name|size_t
modifier|*
name|oldlenp
decl_stmt|;
name|void
modifier|*
name|newp
decl_stmt|;
name|size_t
name|newlen
decl_stmt|;
block|{
name|struct
name|gmonparam
modifier|*
name|gp
init|=
operator|&
name|_gmonparam
decl_stmt|;
name|int
name|error
decl_stmt|;
comment|/* all sysctl names at this level are terminal */
if|if
condition|(
name|namelen
operator|!=
literal|1
condition|)
return|return
operator|(
name|ENOTDIR
operator|)
return|;
comment|/* overloaded */
switch|switch
condition|(
name|name
index|[
literal|0
index|]
condition|)
block|{
case|case
name|GPROF_STATE
case|:
name|error
operator|=
name|sysctl_int
argument_list|(
name|oldp
argument_list|,
name|oldlenp
argument_list|,
name|newp
argument_list|,
name|newlen
argument_list|,
operator|&
name|gp
operator|->
name|state
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|gp
operator|->
name|state
operator|==
name|GMON_PROF_OFF
condition|)
name|stopprofclock
argument_list|(
operator|&
name|proc0
argument_list|)
expr_stmt|;
else|else
name|startprofclock
argument_list|(
operator|&
name|proc0
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
case|case
name|GPROF_COUNT
case|:
return|return
operator|(
name|sysctl_struct
argument_list|(
name|oldp
argument_list|,
name|oldlenp
argument_list|,
name|newp
argument_list|,
name|newlen
argument_list|,
name|gp
operator|->
name|kcount
argument_list|,
name|gp
operator|->
name|kcountsize
argument_list|)
operator|)
return|;
case|case
name|GPROF_FROMS
case|:
return|return
operator|(
name|sysctl_struct
argument_list|(
name|oldp
argument_list|,
name|oldlenp
argument_list|,
name|newp
argument_list|,
name|newlen
argument_list|,
name|gp
operator|->
name|froms
argument_list|,
name|gp
operator|->
name|fromssize
argument_list|)
operator|)
return|;
case|case
name|GPROF_TOS
case|:
return|return
operator|(
name|sysctl_struct
argument_list|(
name|oldp
argument_list|,
name|oldlenp
argument_list|,
name|newp
argument_list|,
name|newlen
argument_list|,
name|gp
operator|->
name|tos
argument_list|,
name|gp
operator|->
name|tossize
argument_list|)
operator|)
return|;
case|case
name|GPROF_GMONPARAM
case|:
return|return
operator|(
name|sysctl_rdstruct
argument_list|(
name|oldp
argument_list|,
name|oldlenp
argument_list|,
name|newp
argument_list|,
name|gp
argument_list|,
sizeof|sizeof
expr|*
name|gp
argument_list|)
operator|)
return|;
default|default:
return|return
operator|(
name|EOPNOTSUPP
operator|)
return|;
block|}
comment|/* NOTREACHED */
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* GPROF */
end_comment

begin_comment
comment|/*  * Profiling system call.  *  * The scale factor is a fixed point number with 16 bits of fraction, so that  * 1.0 is represented as 0x10000.  A scale factor of 0 turns off profiling.  */
end_comment

begin_comment
comment|/* ARGSUSED */
end_comment

begin_function
name|int
name|profil
parameter_list|(
name|p
parameter_list|,
name|uap
parameter_list|,
name|retval
parameter_list|)
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
specifier|register
name|struct
name|profil_args
comment|/* { 		syscallarg(caddr_t) samples; 		syscallarg(u_int) size; 		syscallarg(u_int) offset; 		syscallarg(u_int) scale; 	} */
modifier|*
name|uap
decl_stmt|;
name|register_t
modifier|*
name|retval
decl_stmt|;
block|{
specifier|register
name|struct
name|uprof
modifier|*
name|upp
decl_stmt|;
name|int
name|s
decl_stmt|;
if|if
condition|(
name|SCARG
argument_list|(
name|uap
argument_list|,
name|scale
argument_list|)
operator|>
operator|(
literal|1
operator|<<
literal|16
operator|)
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
if|if
condition|(
name|SCARG
argument_list|(
name|uap
argument_list|,
name|scale
argument_list|)
operator|==
literal|0
condition|)
block|{
name|stopprofclock
argument_list|(
name|p
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|upp
operator|=
operator|&
name|p
operator|->
name|p_stats
operator|->
name|p_prof
expr_stmt|;
comment|/* Block profile interrupts while changing state. */
name|s
operator|=
name|splstatclock
argument_list|()
expr_stmt|;
name|upp
operator|->
name|pr_off
operator|=
name|SCARG
argument_list|(
name|uap
argument_list|,
name|offset
argument_list|)
expr_stmt|;
name|upp
operator|->
name|pr_scale
operator|=
name|SCARG
argument_list|(
name|uap
argument_list|,
name|scale
argument_list|)
expr_stmt|;
name|upp
operator|->
name|pr_base
operator|=
name|SCARG
argument_list|(
name|uap
argument_list|,
name|samples
argument_list|)
expr_stmt|;
name|upp
operator|->
name|pr_size
operator|=
name|SCARG
argument_list|(
name|uap
argument_list|,
name|size
argument_list|)
expr_stmt|;
name|startprofclock
argument_list|(
name|p
argument_list|)
expr_stmt|;
name|splx
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Scale is a fixed-point number with the binary point 16 bits  * into the value, and is<= 1.0.  pc is at most 32 bits, so the  * intermediate result is at most 48 bits.  */
end_comment

begin_define
define|#
directive|define
name|PC_TO_INDEX
parameter_list|(
name|pc
parameter_list|,
name|prof
parameter_list|)
define|\
value|((int)(((u_quad_t)((pc) - (prof)->pr_off) * \ 	    (u_quad_t)((prof)->pr_scale))>> 16)& ~1)
end_define

begin_comment
comment|/*  * Collect user-level profiling statistics; called on a profiling tick,  * when a process is running in user-mode.  This routine may be called  * from an interrupt context.  We try to update the user profiling buffers  * cheaply with fuswintr() and suswintr().  If that fails, we revert to  * an AST that will vector us to trap() with a context in which copyin  * and copyout will work.  Trap will then call addupc_task().  *  * Note that we may (rarely) not get around to the AST soon enough, and  * lose profile ticks when the next tick overwrites this one, but in this  * case the system is overloaded and the profile is probably already  * inaccurate.  */
end_comment

begin_function
name|void
name|addupc_intr
parameter_list|(
name|p
parameter_list|,
name|pc
parameter_list|,
name|ticks
parameter_list|)
specifier|register
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
specifier|register
name|u_long
name|pc
decl_stmt|;
name|u_int
name|ticks
decl_stmt|;
block|{
specifier|register
name|struct
name|uprof
modifier|*
name|prof
decl_stmt|;
specifier|register
name|caddr_t
name|addr
decl_stmt|;
specifier|register
name|u_int
name|i
decl_stmt|;
specifier|register
name|int
name|v
decl_stmt|;
if|if
condition|(
name|ticks
operator|==
literal|0
condition|)
return|return;
name|prof
operator|=
operator|&
name|p
operator|->
name|p_stats
operator|->
name|p_prof
expr_stmt|;
if|if
condition|(
name|pc
operator|<
name|prof
operator|->
name|pr_off
operator|||
operator|(
name|i
operator|=
name|PC_TO_INDEX
argument_list|(
name|pc
argument_list|,
name|prof
argument_list|)
operator|)
operator|>=
name|prof
operator|->
name|pr_size
condition|)
return|return;
comment|/* out of range; ignore */
name|addr
operator|=
name|prof
operator|->
name|pr_base
operator|+
name|i
expr_stmt|;
if|if
condition|(
operator|(
name|v
operator|=
name|fuswintr
argument_list|(
name|addr
argument_list|)
operator|)
operator|==
operator|-
literal|1
operator|||
name|suswintr
argument_list|(
name|addr
argument_list|,
name|v
operator|+
name|ticks
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|prof
operator|->
name|pr_addr
operator|=
name|pc
expr_stmt|;
name|prof
operator|->
name|pr_ticks
operator|=
name|ticks
expr_stmt|;
name|need_proftick
argument_list|(
name|p
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * Much like before, but we can afford to take faults here.  If the  * update fails, we simply turn off profiling.  */
end_comment

begin_function
name|void
name|addupc_task
parameter_list|(
name|p
parameter_list|,
name|pc
parameter_list|,
name|ticks
parameter_list|)
specifier|register
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
specifier|register
name|u_long
name|pc
decl_stmt|;
name|u_int
name|ticks
decl_stmt|;
block|{
specifier|register
name|struct
name|uprof
modifier|*
name|prof
decl_stmt|;
specifier|register
name|caddr_t
name|addr
decl_stmt|;
specifier|register
name|u_int
name|i
decl_stmt|;
name|u_short
name|v
decl_stmt|;
comment|/* Testing P_PROFIL may be unnecessary, but is certainly safe. */
if|if
condition|(
operator|(
name|p
operator|->
name|p_flag
operator|&
name|P_PROFIL
operator|)
operator|==
literal|0
operator|||
name|ticks
operator|==
literal|0
condition|)
return|return;
name|prof
operator|=
operator|&
name|p
operator|->
name|p_stats
operator|->
name|p_prof
expr_stmt|;
if|if
condition|(
name|pc
operator|<
name|prof
operator|->
name|pr_off
operator|||
operator|(
name|i
operator|=
name|PC_TO_INDEX
argument_list|(
name|pc
argument_list|,
name|prof
argument_list|)
operator|)
operator|>=
name|prof
operator|->
name|pr_size
condition|)
return|return;
name|addr
operator|=
name|prof
operator|->
name|pr_base
operator|+
name|i
expr_stmt|;
if|if
condition|(
name|copyin
argument_list|(
name|addr
argument_list|,
operator|(
name|caddr_t
operator|)
operator|&
name|v
argument_list|,
sizeof|sizeof
argument_list|(
name|v
argument_list|)
argument_list|)
operator|==
literal|0
condition|)
block|{
name|v
operator|+=
name|ticks
expr_stmt|;
if|if
condition|(
name|copyout
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|v
argument_list|,
name|addr
argument_list|,
sizeof|sizeof
argument_list|(
name|v
argument_list|)
argument_list|)
operator|==
literal|0
condition|)
return|return;
block|}
name|stopprofclock
argument_list|(
name|p
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

