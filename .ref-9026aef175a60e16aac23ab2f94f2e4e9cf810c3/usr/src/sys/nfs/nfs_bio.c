begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1989, 1993  *	The Regents of the University of California.  All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Rick Macklem at The University of Guelph.  *  * %sccs.include.redist.c%  *  *	@(#)nfs_bio.c	8.9 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/resourcevar.h>
end_include

begin_include
include|#
directive|include
file|<sys/signalvar.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/buf.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<sys/trace.h>
end_include

begin_include
include|#
directive|include
file|<sys/mount.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<vm/vm.h>
end_include

begin_include
include|#
directive|include
file|<nfs/rpcv2.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nfsproto.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nfs.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nfsmount.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nqnfs.h>
end_include

begin_include
include|#
directive|include
file|<nfs/nfsnode.h>
end_include

begin_function_decl
name|struct
name|buf
modifier|*
name|nfs_getcacheblk
parameter_list|()
function_decl|;
end_function_decl

begin_decl_stmt
specifier|extern
name|struct
name|proc
modifier|*
name|nfs_iodwant
index|[
name|NFS_MAXASYNCDAEMON
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|int
name|nfs_numasync
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|struct
name|nfsstats
name|nfsstats
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Vnode op for read using bio  * Any similarity to readip() is purely coincidental  */
end_comment

begin_function
name|int
name|nfs_bioread
parameter_list|(
name|vp
parameter_list|,
name|uio
parameter_list|,
name|ioflag
parameter_list|,
name|cred
parameter_list|)
specifier|register
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
specifier|register
name|struct
name|uio
modifier|*
name|uio
decl_stmt|;
name|int
name|ioflag
decl_stmt|;
name|struct
name|ucred
modifier|*
name|cred
decl_stmt|;
block|{
specifier|register
name|struct
name|nfsnode
modifier|*
name|np
init|=
name|VTONFS
argument_list|(
name|vp
argument_list|)
decl_stmt|;
specifier|register
name|int
name|biosize
decl_stmt|,
name|diff
decl_stmt|,
name|i
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
init|=
literal|0
decl_stmt|,
modifier|*
name|rabp
decl_stmt|;
name|struct
name|vattr
name|vattr
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
name|struct
name|nfsmount
modifier|*
name|nmp
init|=
name|VFSTONFS
argument_list|(
name|vp
operator|->
name|v_mount
argument_list|)
decl_stmt|;
name|daddr_t
name|lbn
decl_stmt|,
name|bn
decl_stmt|,
name|bn2
decl_stmt|,
name|rabn
decl_stmt|;
name|caddr_t
name|baddr
decl_stmt|;
name|int
name|got_buf
init|=
literal|0
decl_stmt|,
name|nra
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|,
name|n
init|=
literal|0
decl_stmt|,
name|on
init|=
literal|0
decl_stmt|,
name|not_readin
decl_stmt|;
name|nfsquad_t
name|tquad
decl_stmt|;
ifdef|#
directive|ifdef
name|DIAGNOSTIC
if|if
condition|(
name|uio
operator|->
name|uio_rw
operator|!=
name|UIO_READ
condition|)
name|panic
argument_list|(
literal|"nfs_read mode"
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|uio
operator|->
name|uio_resid
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
if|if
condition|(
name|uio
operator|->
name|uio_offset
operator|<
literal|0
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
name|p
operator|=
name|uio
operator|->
name|uio_procp
expr_stmt|;
if|if
condition|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
operator|(
name|NFSMNT_NFSV3
operator||
name|NFSMNT_GOTFSINFO
operator|)
operator|)
operator|==
name|NFSMNT_NFSV3
condition|)
operator|(
name|void
operator|)
name|nfs_fsinfo
argument_list|(
name|nmp
argument_list|,
name|vp
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|biosize
operator|=
name|nmp
operator|->
name|nm_rsize
expr_stmt|;
comment|/* 	 * For nfs, cache consistency can only be maintained approximately. 	 * Although RFC1094 does not specify the criteria, the following is 	 * believed to be compatible with the reference port. 	 * For nqnfs, full cache consistency is maintained within the loop. 	 * For nfs: 	 * If the file's modify time on the server has changed since the 	 * last read rpc or you have written to the file, 	 * you may have lost data cache consistency with the 	 * server, so flush all of the file's data out of the cache. 	 * Then force a getattr rpc to ensure that you have up to date 	 * attributes. 	 * NB: This implies that cache data can be read when up to 	 * NFS_ATTRTIMEO seconds out of date. If you find that you need current 	 * attributes this could be forced by setting n_attrstamp to 0 before 	 * the VOP_GETATTR() call. 	 */
if|if
condition|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_NQNFS
operator|)
operator|==
literal|0
operator|&&
name|vp
operator|->
name|v_type
operator|!=
name|VLNK
condition|)
block|{
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NMODIFIED
condition|)
block|{
if|if
condition|(
name|vp
operator|->
name|v_type
operator|!=
name|VREG
condition|)
block|{
if|if
condition|(
name|vp
operator|->
name|v_type
operator|!=
name|VDIR
condition|)
name|panic
argument_list|(
literal|"nfs: bioread, not dir"
argument_list|)
expr_stmt|;
name|nfs_invaldir
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
block|}
name|np
operator|->
name|n_attrstamp
operator|=
literal|0
expr_stmt|;
name|error
operator|=
name|VOP_GETATTR
argument_list|(
name|vp
argument_list|,
operator|&
name|vattr
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|np
operator|->
name|n_mtime
operator|=
name|vattr
operator|.
name|va_mtime
operator|.
name|ts_sec
expr_stmt|;
block|}
else|else
block|{
name|error
operator|=
name|VOP_GETATTR
argument_list|(
name|vp
argument_list|,
operator|&
name|vattr
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|np
operator|->
name|n_mtime
operator|!=
name|vattr
operator|.
name|va_mtime
operator|.
name|ts_sec
condition|)
block|{
if|if
condition|(
name|vp
operator|->
name|v_type
operator|==
name|VDIR
condition|)
name|nfs_invaldir
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|np
operator|->
name|n_mtime
operator|=
name|vattr
operator|.
name|va_mtime
operator|.
name|ts_sec
expr_stmt|;
block|}
block|}
block|}
do|do
block|{
comment|/* 	     * Get a valid lease. If cached data is stale, flush it. 	     */
if|if
condition|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_NQNFS
condition|)
block|{
if|if
condition|(
name|NQNFS_CKINVALID
argument_list|(
name|vp
argument_list|,
name|np
argument_list|,
name|ND_READ
argument_list|)
condition|)
block|{
do|do
block|{
name|error
operator|=
name|nqnfs_getlease
argument_list|(
name|vp
argument_list|,
name|ND_READ
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
block|}
do|while
condition|(
name|error
operator|==
name|NQNFS_EXPIRED
condition|)
do|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|np
operator|->
name|n_lrev
operator|!=
name|np
operator|->
name|n_brev
operator|||
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
operator|)
operator|||
operator|(
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NMODIFIED
operator|)
operator|&&
name|vp
operator|->
name|v_type
operator|==
name|VDIR
operator|)
condition|)
block|{
if|if
condition|(
name|vp
operator|->
name|v_type
operator|==
name|VDIR
condition|)
name|nfs_invaldir
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|np
operator|->
name|n_brev
operator|=
name|np
operator|->
name|n_lrev
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|vp
operator|->
name|v_type
operator|==
name|VDIR
operator|&&
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NMODIFIED
operator|)
condition|)
block|{
name|nfs_invaldir
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
block|}
block|}
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
condition|)
block|{
switch|switch
condition|(
name|vp
operator|->
name|v_type
condition|)
block|{
case|case
name|VREG
case|:
return|return
operator|(
name|nfs_readrpc
argument_list|(
name|vp
argument_list|,
name|uio
argument_list|,
name|cred
argument_list|)
operator|)
return|;
case|case
name|VLNK
case|:
return|return
operator|(
name|nfs_readlinkrpc
argument_list|(
name|vp
argument_list|,
name|uio
argument_list|,
name|cred
argument_list|)
operator|)
return|;
case|case
name|VDIR
case|:
break|break;
default|default:
name|printf
argument_list|(
literal|" NQNFSNONCACHE: type %x unexpected\n"
argument_list|,
name|vp
operator|->
name|v_type
argument_list|)
expr_stmt|;
block|}
empty_stmt|;
block|}
name|baddr
operator|=
operator|(
name|caddr_t
operator|)
literal|0
expr_stmt|;
switch|switch
condition|(
name|vp
operator|->
name|v_type
condition|)
block|{
case|case
name|VREG
case|:
name|nfsstats
operator|.
name|biocache_reads
operator|++
expr_stmt|;
name|lbn
operator|=
name|uio
operator|->
name|uio_offset
operator|/
name|biosize
expr_stmt|;
name|on
operator|=
name|uio
operator|->
name|uio_offset
operator|&
operator|(
name|biosize
operator|-
literal|1
operator|)
expr_stmt|;
name|bn
operator|=
name|lbn
operator|*
operator|(
name|biosize
operator|/
name|DEV_BSIZE
operator|)
expr_stmt|;
name|not_readin
operator|=
literal|1
expr_stmt|;
comment|/* 		 * Start the read ahead(s), as required. 		 */
if|if
condition|(
name|nfs_numasync
operator|>
literal|0
operator|&&
name|nmp
operator|->
name|nm_readahead
operator|>
literal|0
condition|)
block|{
for|for
control|(
name|nra
operator|=
literal|0
init|;
name|nra
operator|<
name|nmp
operator|->
name|nm_readahead
operator|&&
operator|(
name|lbn
operator|+
literal|1
operator|+
name|nra
operator|)
operator|*
name|biosize
operator|<
name|np
operator|->
name|n_size
condition|;
name|nra
operator|++
control|)
block|{
name|rabn
operator|=
operator|(
name|lbn
operator|+
literal|1
operator|+
name|nra
operator|)
operator|*
operator|(
name|biosize
operator|/
name|DEV_BSIZE
operator|)
expr_stmt|;
if|if
condition|(
operator|!
name|incore
argument_list|(
name|vp
argument_list|,
name|rabn
argument_list|)
condition|)
block|{
name|rabp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
name|rabn
argument_list|,
name|biosize
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|rabp
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
if|if
condition|(
operator|(
name|rabp
operator|->
name|b_flags
operator|&
operator|(
name|B_DELWRI
operator||
name|B_DONE
operator|)
operator|)
operator|==
literal|0
condition|)
block|{
name|rabp
operator|->
name|b_flags
operator||=
operator|(
name|B_READ
operator||
name|B_ASYNC
operator|)
expr_stmt|;
if|if
condition|(
name|nfs_asyncio
argument_list|(
name|rabp
argument_list|,
name|cred
argument_list|)
condition|)
block|{
name|rabp
operator|->
name|b_flags
operator||=
name|B_INVAL
expr_stmt|;
name|brelse
argument_list|(
name|rabp
argument_list|)
expr_stmt|;
block|}
block|}
else|else
name|brelse
argument_list|(
name|rabp
argument_list|)
expr_stmt|;
block|}
block|}
block|}
comment|/* 		 * If the block is in the cache and has the required data 		 * in a valid region, just copy it out. 		 * Otherwise, get the block and write back/read in, 		 * as required. 		 */
if|if
condition|(
operator|(
name|bp
operator|=
name|incore
argument_list|(
name|vp
argument_list|,
name|bn
argument_list|)
operator|)
operator|&&
operator|(
name|bp
operator|->
name|b_flags
operator|&
operator|(
name|B_BUSY
operator||
name|B_WRITEINPROG
operator|)
operator|)
operator|==
operator|(
name|B_BUSY
operator||
name|B_WRITEINPROG
operator|)
condition|)
name|got_buf
operator|=
literal|0
expr_stmt|;
else|else
block|{
name|again
label|:
name|bp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
name|bn
argument_list|,
name|biosize
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|bp
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
name|got_buf
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|(
name|bp
operator|->
name|b_flags
operator|&
operator|(
name|B_DONE
operator||
name|B_DELWRI
operator|)
operator|)
operator|==
literal|0
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_READ
expr_stmt|;
name|not_readin
operator|=
literal|0
expr_stmt|;
name|error
operator|=
name|nfs_doio
argument_list|(
name|bp
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
block|}
block|}
name|n
operator|=
name|min
argument_list|(
call|(
name|unsigned
call|)
argument_list|(
name|biosize
operator|-
name|on
argument_list|)
argument_list|,
name|uio
operator|->
name|uio_resid
argument_list|)
expr_stmt|;
name|diff
operator|=
name|np
operator|->
name|n_size
operator|-
name|uio
operator|->
name|uio_offset
expr_stmt|;
if|if
condition|(
name|diff
operator|<
name|n
condition|)
name|n
operator|=
name|diff
expr_stmt|;
if|if
condition|(
name|not_readin
operator|&&
name|n
operator|>
literal|0
condition|)
block|{
if|if
condition|(
name|on
operator|<
name|bp
operator|->
name|b_validoff
operator|||
operator|(
name|on
operator|+
name|n
operator|)
operator|>
name|bp
operator|->
name|b_validend
condition|)
block|{
if|if
condition|(
operator|!
name|got_buf
condition|)
block|{
name|bp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
name|bn
argument_list|,
name|biosize
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|bp
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
name|got_buf
operator|=
literal|1
expr_stmt|;
block|}
name|bp
operator|->
name|b_flags
operator||=
name|B_INVAFTERWRITE
expr_stmt|;
if|if
condition|(
name|bp
operator|->
name|b_dirtyend
operator|>
literal|0
condition|)
block|{
if|if
condition|(
operator|(
name|bp
operator|->
name|b_flags
operator|&
name|B_DELWRI
operator|)
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"nfsbioread"
argument_list|)
expr_stmt|;
if|if
condition|(
name|VOP_BWRITE
argument_list|(
name|bp
argument_list|)
operator|==
name|EINTR
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
block|}
else|else
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
goto|goto
name|again
goto|;
block|}
block|}
name|vp
operator|->
name|v_lastr
operator|=
name|lbn
expr_stmt|;
name|diff
operator|=
operator|(
name|on
operator|>=
name|bp
operator|->
name|b_validend
operator|)
condition|?
literal|0
else|:
operator|(
name|bp
operator|->
name|b_validend
operator|-
name|on
operator|)
expr_stmt|;
if|if
condition|(
name|diff
operator|<
name|n
condition|)
name|n
operator|=
name|diff
expr_stmt|;
break|break;
case|case
name|VLNK
case|:
name|nfsstats
operator|.
name|biocache_readlinks
operator|++
expr_stmt|;
name|bp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
operator|(
name|daddr_t
operator|)
literal|0
argument_list|,
name|NFS_MAXPATHLEN
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|bp
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
if|if
condition|(
operator|(
name|bp
operator|->
name|b_flags
operator|&
name|B_DONE
operator|)
operator|==
literal|0
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_READ
expr_stmt|;
name|error
operator|=
name|nfs_doio
argument_list|(
name|bp
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
block|}
name|n
operator|=
name|min
argument_list|(
name|uio
operator|->
name|uio_resid
argument_list|,
name|NFS_MAXPATHLEN
operator|-
name|bp
operator|->
name|b_resid
argument_list|)
expr_stmt|;
name|got_buf
operator|=
literal|1
expr_stmt|;
name|on
operator|=
literal|0
expr_stmt|;
break|break;
case|case
name|VDIR
case|:
name|nfsstats
operator|.
name|biocache_readdirs
operator|++
expr_stmt|;
name|lbn
operator|=
name|uio
operator|->
name|uio_offset
operator|/
name|NFS_DIRBLKSIZ
expr_stmt|;
name|on
operator|=
name|uio
operator|->
name|uio_offset
operator|&
operator|(
name|NFS_DIRBLKSIZ
operator|-
literal|1
operator|)
expr_stmt|;
name|bp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
name|lbn
argument_list|,
name|NFS_DIRBLKSIZ
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|bp
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
if|if
condition|(
operator|(
name|bp
operator|->
name|b_flags
operator|&
name|B_DONE
operator|)
operator|==
literal|0
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_READ
expr_stmt|;
name|error
operator|=
name|nfs_doio
argument_list|(
name|bp
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
while|while
condition|(
name|error
operator|==
name|NFSERR_BAD_COOKIE
condition|)
block|{
name|nfs_invaldir
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
literal|0
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|/* 			     * Yuck! The directory has been modified on the 			     * server. The only way to get the block is by 			     * reading from the beginning to get all the 			     * offset cookies. 			     */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<=
name|lbn
operator|&&
operator|!
name|error
condition|;
name|i
operator|++
control|)
block|{
name|bp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
name|i
argument_list|,
name|NFS_DIRBLKSIZ
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|bp
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
if|if
condition|(
operator|(
name|bp
operator|->
name|b_flags
operator|&
name|B_DONE
operator|)
operator|==
literal|0
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_READ
expr_stmt|;
name|error
operator|=
name|nfs_doio
argument_list|(
name|bp
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
block|}
block|}
block|}
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
block|}
block|}
comment|/* 		 * If not eof and read aheads are enabled, start one. 		 * (You need the current block first, so that you have the 		 *  directory offset cookie of the next block.) 		 */
if|if
condition|(
name|nfs_numasync
operator|>
literal|0
operator|&&
name|nmp
operator|->
name|nm_readahead
operator|>
literal|0
operator|&&
operator|(
name|np
operator|->
name|n_direofoffset
operator|==
literal|0
operator|||
operator|(
name|lbn
operator|+
literal|1
operator|)
operator|*
name|NFS_DIRBLKSIZ
operator|<
name|np
operator|->
name|n_direofoffset
operator|)
operator|&&
operator|!
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
operator|)
operator|&&
operator|!
name|incore
argument_list|(
name|vp
argument_list|,
name|lbn
operator|+
literal|1
argument_list|)
condition|)
block|{
name|rabp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
name|lbn
operator|+
literal|1
argument_list|,
name|NFS_DIRBLKSIZ
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|rabp
condition|)
block|{
if|if
condition|(
operator|(
name|rabp
operator|->
name|b_flags
operator|&
operator|(
name|B_DONE
operator||
name|B_DELWRI
operator|)
operator|)
operator|==
literal|0
condition|)
block|{
name|rabp
operator|->
name|b_flags
operator||=
operator|(
name|B_READ
operator||
name|B_ASYNC
operator|)
expr_stmt|;
if|if
condition|(
name|nfs_asyncio
argument_list|(
name|rabp
argument_list|,
name|cred
argument_list|)
condition|)
block|{
name|rabp
operator|->
name|b_flags
operator||=
name|B_INVAL
expr_stmt|;
name|brelse
argument_list|(
name|rabp
argument_list|)
expr_stmt|;
block|}
block|}
else|else
name|brelse
argument_list|(
name|rabp
argument_list|)
expr_stmt|;
block|}
block|}
name|n
operator|=
name|min
argument_list|(
name|uio
operator|->
name|uio_resid
argument_list|,
name|NFS_DIRBLKSIZ
operator|-
name|bp
operator|->
name|b_resid
operator|-
name|on
argument_list|)
expr_stmt|;
name|got_buf
operator|=
literal|1
expr_stmt|;
break|break;
default|default:
name|printf
argument_list|(
literal|" nfsbioread: type %x unexpected\n"
argument_list|,
name|vp
operator|->
name|v_type
argument_list|)
expr_stmt|;
break|break;
block|}
empty_stmt|;
if|if
condition|(
name|n
operator|>
literal|0
condition|)
block|{
if|if
condition|(
operator|!
name|baddr
condition|)
name|baddr
operator|=
name|bp
operator|->
name|b_data
expr_stmt|;
name|error
operator|=
name|uiomove
argument_list|(
name|baddr
operator|+
name|on
argument_list|,
operator|(
name|int
operator|)
name|n
argument_list|,
name|uio
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|vp
operator|->
name|v_type
condition|)
block|{
case|case
name|VREG
case|:
break|break;
case|case
name|VLNK
case|:
name|n
operator|=
literal|0
expr_stmt|;
break|break;
case|case
name|VDIR
case|:
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
condition|)
name|bp
operator|->
name|b_flags
operator||=
name|B_INVAL
expr_stmt|;
break|break;
default|default:
name|printf
argument_list|(
literal|" nfsbioread: type %x unexpected\n"
argument_list|,
name|vp
operator|->
name|v_type
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|got_buf
condition|)
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
block|}
do|while
condition|(
name|error
operator|==
literal|0
operator|&&
name|uio
operator|->
name|uio_resid
operator|>
literal|0
operator|&&
name|n
operator|>
literal|0
condition|)
do|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Vnode op for write using bio  */
end_comment

begin_function
name|int
name|nfs_write
parameter_list|(
name|ap
parameter_list|)
name|struct
name|vop_write_args
comment|/* { 		struct vnode *a_vp; 		struct uio *a_uio; 		int  a_ioflag; 		struct ucred *a_cred; 	} */
modifier|*
name|ap
decl_stmt|;
block|{
specifier|register
name|int
name|biosize
decl_stmt|;
specifier|register
name|struct
name|uio
modifier|*
name|uio
init|=
name|ap
operator|->
name|a_uio
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
init|=
name|uio
operator|->
name|uio_procp
decl_stmt|;
specifier|register
name|struct
name|vnode
modifier|*
name|vp
init|=
name|ap
operator|->
name|a_vp
decl_stmt|;
name|struct
name|nfsnode
modifier|*
name|np
init|=
name|VTONFS
argument_list|(
name|vp
argument_list|)
decl_stmt|;
specifier|register
name|struct
name|ucred
modifier|*
name|cred
init|=
name|ap
operator|->
name|a_cred
decl_stmt|;
name|int
name|ioflag
init|=
name|ap
operator|->
name|a_ioflag
decl_stmt|;
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|struct
name|vattr
name|vattr
decl_stmt|;
name|struct
name|nfsmount
modifier|*
name|nmp
init|=
name|VFSTONFS
argument_list|(
name|vp
operator|->
name|v_mount
argument_list|)
decl_stmt|;
name|daddr_t
name|lbn
decl_stmt|,
name|bn
decl_stmt|;
name|int
name|n
decl_stmt|,
name|on
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|,
name|iomode
decl_stmt|,
name|must_commit
decl_stmt|;
ifdef|#
directive|ifdef
name|DIAGNOSTIC
if|if
condition|(
name|uio
operator|->
name|uio_rw
operator|!=
name|UIO_WRITE
condition|)
name|panic
argument_list|(
literal|"nfs_write mode"
argument_list|)
expr_stmt|;
if|if
condition|(
name|uio
operator|->
name|uio_segflg
operator|==
name|UIO_USERSPACE
operator|&&
name|uio
operator|->
name|uio_procp
operator|!=
name|curproc
condition|)
name|panic
argument_list|(
literal|"nfs_write proc"
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|vp
operator|->
name|v_type
operator|!=
name|VREG
condition|)
return|return
operator|(
name|EIO
operator|)
return|;
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NWRITEERR
condition|)
block|{
name|np
operator|->
name|n_flag
operator|&=
operator|~
name|NWRITEERR
expr_stmt|;
return|return
operator|(
name|np
operator|->
name|n_error
operator|)
return|;
block|}
if|if
condition|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
operator|(
name|NFSMNT_NFSV3
operator||
name|NFSMNT_GOTFSINFO
operator|)
operator|)
operator|==
name|NFSMNT_NFSV3
condition|)
operator|(
name|void
operator|)
name|nfs_fsinfo
argument_list|(
name|nmp
argument_list|,
name|vp
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|ioflag
operator|&
operator|(
name|IO_APPEND
operator||
name|IO_SYNC
operator|)
condition|)
block|{
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NMODIFIED
condition|)
block|{
name|np
operator|->
name|n_attrstamp
operator|=
literal|0
expr_stmt|;
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
block|}
if|if
condition|(
name|ioflag
operator|&
name|IO_APPEND
condition|)
block|{
name|np
operator|->
name|n_attrstamp
operator|=
literal|0
expr_stmt|;
name|error
operator|=
name|VOP_GETATTR
argument_list|(
name|vp
argument_list|,
operator|&
name|vattr
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|uio
operator|->
name|uio_offset
operator|=
name|np
operator|->
name|n_size
expr_stmt|;
block|}
block|}
if|if
condition|(
name|uio
operator|->
name|uio_offset
operator|<
literal|0
condition|)
return|return
operator|(
name|EINVAL
operator|)
return|;
if|if
condition|(
name|uio
operator|->
name|uio_resid
operator|==
literal|0
condition|)
return|return
operator|(
literal|0
operator|)
return|;
comment|/* 	 * Maybe this should be above the vnode op call, but so long as 	 * file servers have no limits, i don't think it matters 	 */
if|if
condition|(
name|p
operator|&&
name|uio
operator|->
name|uio_offset
operator|+
name|uio
operator|->
name|uio_resid
operator|>
name|p
operator|->
name|p_rlimit
index|[
name|RLIMIT_FSIZE
index|]
operator|.
name|rlim_cur
condition|)
block|{
name|psignal
argument_list|(
name|p
argument_list|,
name|SIGXFSZ
argument_list|)
expr_stmt|;
return|return
operator|(
name|EFBIG
operator|)
return|;
block|}
comment|/* 	 * I use nm_rsize, not nm_wsize so that all buffer cache blocks 	 * will be the same size within a filesystem. nfs_writerpc will 	 * still use nm_wsize when sizing the rpc's. 	 */
name|biosize
operator|=
name|nmp
operator|->
name|nm_rsize
expr_stmt|;
do|do
block|{
comment|/* 		 * XXX make sure we aren't cached in the VM page cache 		 */
operator|(
name|void
operator|)
name|vnode_pager_uncache
argument_list|(
name|vp
argument_list|)
expr_stmt|;
comment|/* 		 * Check for a valid write lease. 		 */
if|if
condition|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_NQNFS
operator|)
operator|&&
name|NQNFS_CKINVALID
argument_list|(
name|vp
argument_list|,
name|np
argument_list|,
name|ND_WRITE
argument_list|)
condition|)
block|{
do|do
block|{
name|error
operator|=
name|nqnfs_getlease
argument_list|(
name|vp
argument_list|,
name|ND_WRITE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
block|}
do|while
condition|(
name|error
operator|==
name|NQNFS_EXPIRED
condition|)
do|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|np
operator|->
name|n_lrev
operator|!=
name|np
operator|->
name|n_brev
operator|||
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
operator|)
condition|)
block|{
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|np
operator|->
name|n_brev
operator|=
name|np
operator|->
name|n_lrev
expr_stmt|;
block|}
block|}
if|if
condition|(
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
operator|)
operator|&&
name|uio
operator|->
name|uio_iovcnt
operator|==
literal|1
condition|)
block|{
name|iomode
operator|=
name|NFSV3WRITE_FILESYNC
expr_stmt|;
name|error
operator|=
name|nfs_writerpc
argument_list|(
name|vp
argument_list|,
name|uio
argument_list|,
name|cred
argument_list|,
operator|&
name|iomode
argument_list|,
operator|&
name|must_commit
argument_list|)
expr_stmt|;
if|if
condition|(
name|must_commit
condition|)
name|nfs_clearcommit
argument_list|(
name|vp
operator|->
name|v_mount
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
name|nfsstats
operator|.
name|biocache_writes
operator|++
expr_stmt|;
name|lbn
operator|=
name|uio
operator|->
name|uio_offset
operator|/
name|biosize
expr_stmt|;
name|on
operator|=
name|uio
operator|->
name|uio_offset
operator|&
operator|(
name|biosize
operator|-
literal|1
operator|)
expr_stmt|;
name|n
operator|=
name|min
argument_list|(
call|(
name|unsigned
call|)
argument_list|(
name|biosize
operator|-
name|on
argument_list|)
argument_list|,
name|uio
operator|->
name|uio_resid
argument_list|)
expr_stmt|;
name|bn
operator|=
name|lbn
operator|*
operator|(
name|biosize
operator|/
name|DEV_BSIZE
operator|)
expr_stmt|;
name|again
label|:
name|bp
operator|=
name|nfs_getcacheblk
argument_list|(
name|vp
argument_list|,
name|bn
argument_list|,
name|biosize
argument_list|,
name|p
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|bp
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
if|if
condition|(
name|bp
operator|->
name|b_wcred
operator|==
name|NOCRED
condition|)
block|{
name|crhold
argument_list|(
name|cred
argument_list|)
expr_stmt|;
name|bp
operator|->
name|b_wcred
operator|=
name|cred
expr_stmt|;
block|}
name|np
operator|->
name|n_flag
operator||=
name|NMODIFIED
expr_stmt|;
if|if
condition|(
name|uio
operator|->
name|uio_offset
operator|+
name|n
operator|>
name|np
operator|->
name|n_size
condition|)
block|{
name|np
operator|->
name|n_size
operator|=
name|uio
operator|->
name|uio_offset
operator|+
name|n
expr_stmt|;
name|vnode_pager_setsize
argument_list|(
name|vp
argument_list|,
operator|(
name|u_long
operator|)
name|np
operator|->
name|n_size
argument_list|)
expr_stmt|;
block|}
comment|/* 		 * If the new write will leave a contiguous dirty 		 * area, just update the b_dirtyoff and b_dirtyend, 		 * otherwise force a write rpc of the old dirty area. 		 */
if|if
condition|(
name|bp
operator|->
name|b_dirtyend
operator|>
literal|0
operator|&&
operator|(
name|on
operator|>
name|bp
operator|->
name|b_dirtyend
operator|||
operator|(
name|on
operator|+
name|n
operator|)
operator|<
name|bp
operator|->
name|b_dirtyoff
operator|)
condition|)
block|{
name|bp
operator|->
name|b_proc
operator|=
name|p
expr_stmt|;
if|if
condition|(
name|VOP_BWRITE
argument_list|(
name|bp
argument_list|)
operator|==
name|EINTR
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
goto|goto
name|again
goto|;
block|}
comment|/* 		 * Check for valid write lease and get one as required. 		 * In case getblk() and/or bwrite() delayed us. 		 */
if|if
condition|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_NQNFS
operator|)
operator|&&
name|NQNFS_CKINVALID
argument_list|(
name|vp
argument_list|,
name|np
argument_list|,
name|ND_WRITE
argument_list|)
condition|)
block|{
do|do
block|{
name|error
operator|=
name|nqnfs_getlease
argument_list|(
name|vp
argument_list|,
name|ND_WRITE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|)
expr_stmt|;
block|}
do|while
condition|(
name|error
operator|==
name|NQNFS_EXPIRED
condition|)
do|;
if|if
condition|(
name|error
condition|)
block|{
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
if|if
condition|(
name|np
operator|->
name|n_lrev
operator|!=
name|np
operator|->
name|n_brev
operator|||
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
operator|)
condition|)
block|{
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
name|np
operator|->
name|n_brev
operator|=
name|np
operator|->
name|n_lrev
expr_stmt|;
goto|goto
name|again
goto|;
block|}
block|}
name|error
operator|=
name|uiomove
argument_list|(
operator|(
name|char
operator|*
operator|)
name|bp
operator|->
name|b_data
operator|+
name|on
argument_list|,
name|n
argument_list|,
name|uio
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_ERROR
expr_stmt|;
name|brelse
argument_list|(
name|bp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
if|if
condition|(
name|bp
operator|->
name|b_dirtyend
operator|>
literal|0
condition|)
block|{
name|bp
operator|->
name|b_dirtyoff
operator|=
name|min
argument_list|(
name|on
argument_list|,
name|bp
operator|->
name|b_dirtyoff
argument_list|)
expr_stmt|;
name|bp
operator|->
name|b_dirtyend
operator|=
name|max
argument_list|(
operator|(
name|on
operator|+
name|n
operator|)
argument_list|,
name|bp
operator|->
name|b_dirtyend
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|bp
operator|->
name|b_dirtyoff
operator|=
name|on
expr_stmt|;
name|bp
operator|->
name|b_dirtyend
operator|=
name|on
operator|+
name|n
expr_stmt|;
block|}
if|if
condition|(
name|bp
operator|->
name|b_validend
operator|==
literal|0
operator|||
name|bp
operator|->
name|b_validend
operator|<
name|bp
operator|->
name|b_dirtyoff
operator|||
name|bp
operator|->
name|b_validoff
operator|>
name|bp
operator|->
name|b_dirtyend
condition|)
block|{
name|bp
operator|->
name|b_validoff
operator|=
name|bp
operator|->
name|b_dirtyoff
expr_stmt|;
name|bp
operator|->
name|b_validend
operator|=
name|bp
operator|->
name|b_dirtyend
expr_stmt|;
block|}
else|else
block|{
name|bp
operator|->
name|b_validoff
operator|=
name|min
argument_list|(
name|bp
operator|->
name|b_validoff
argument_list|,
name|bp
operator|->
name|b_dirtyoff
argument_list|)
expr_stmt|;
name|bp
operator|->
name|b_validend
operator|=
name|max
argument_list|(
name|bp
operator|->
name|b_validend
argument_list|,
name|bp
operator|->
name|b_dirtyend
argument_list|)
expr_stmt|;
block|}
comment|/* 		 * If the lease is non-cachable or IO_SYNC do bwrite(). 		 */
if|if
condition|(
operator|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
operator|)
operator|||
operator|(
name|ioflag
operator|&
name|IO_SYNC
operator|)
condition|)
block|{
name|bp
operator|->
name|b_proc
operator|=
name|p
expr_stmt|;
name|error
operator|=
name|VOP_BWRITE
argument_list|(
name|bp
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NQNFSNONCACHE
condition|)
block|{
name|error
operator|=
name|nfs_vinvalbuf
argument_list|(
name|vp
argument_list|,
name|V_SAVE
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
return|return
operator|(
name|error
operator|)
return|;
block|}
block|}
elseif|else
if|if
condition|(
operator|(
name|n
operator|+
name|on
operator|)
operator|==
name|biosize
operator|&&
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_NQNFS
operator|)
operator|==
literal|0
condition|)
block|{
name|bp
operator|->
name|b_proc
operator|=
operator|(
expr|struct
name|proc
operator|*
operator|)
literal|0
expr_stmt|;
name|bp
operator|->
name|b_flags
operator||=
name|B_ASYNC
expr_stmt|;
operator|(
name|void
operator|)
name|nfs_writebp
argument_list|(
name|bp
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
name|bdwrite
argument_list|(
name|bp
argument_list|)
expr_stmt|;
block|}
do|while
condition|(
name|uio
operator|->
name|uio_resid
operator|>
literal|0
operator|&&
name|n
operator|>
literal|0
condition|)
do|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Get an nfs cache block.  * Allocate a new one if the block isn't currently in the cache  * and return the block marked busy. If the calling process is  * interrupted by a signal for an interruptible mount point, return  * NULL.  */
end_comment

begin_function
name|struct
name|buf
modifier|*
name|nfs_getcacheblk
parameter_list|(
name|vp
parameter_list|,
name|bn
parameter_list|,
name|size
parameter_list|,
name|p
parameter_list|)
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|daddr_t
name|bn
decl_stmt|;
name|int
name|size
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
specifier|register
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|struct
name|nfsmount
modifier|*
name|nmp
init|=
name|VFSTONFS
argument_list|(
name|vp
operator|->
name|v_mount
argument_list|)
decl_stmt|;
if|if
condition|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_INT
condition|)
block|{
name|bp
operator|=
name|getblk
argument_list|(
name|vp
argument_list|,
name|bn
argument_list|,
name|size
argument_list|,
name|PCATCH
argument_list|,
literal|0
argument_list|)
expr_stmt|;
while|while
condition|(
name|bp
operator|==
operator|(
expr|struct
name|buf
operator|*
operator|)
literal|0
condition|)
block|{
if|if
condition|(
name|nfs_sigintr
argument_list|(
name|nmp
argument_list|,
operator|(
expr|struct
name|nfsreq
operator|*
operator|)
literal|0
argument_list|,
name|p
argument_list|)
condition|)
return|return
operator|(
operator|(
expr|struct
name|buf
operator|*
operator|)
literal|0
operator|)
return|;
name|bp
operator|=
name|getblk
argument_list|(
name|vp
argument_list|,
name|bn
argument_list|,
name|size
argument_list|,
literal|0
argument_list|,
literal|2
operator|*
name|hz
argument_list|)
expr_stmt|;
block|}
block|}
else|else
name|bp
operator|=
name|getblk
argument_list|(
name|vp
argument_list|,
name|bn
argument_list|,
name|size
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
operator|(
name|bp
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Flush and invalidate all dirty buffers. If another process is already  * doing the flush, just wait for completion.  */
end_comment

begin_function
name|int
name|nfs_vinvalbuf
parameter_list|(
name|vp
parameter_list|,
name|flags
parameter_list|,
name|cred
parameter_list|,
name|p
parameter_list|,
name|intrflg
parameter_list|)
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|int
name|flags
decl_stmt|;
name|struct
name|ucred
modifier|*
name|cred
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
name|int
name|intrflg
decl_stmt|;
block|{
specifier|register
name|struct
name|nfsnode
modifier|*
name|np
init|=
name|VTONFS
argument_list|(
name|vp
argument_list|)
decl_stmt|;
name|struct
name|nfsmount
modifier|*
name|nmp
init|=
name|VFSTONFS
argument_list|(
name|vp
operator|->
name|v_mount
argument_list|)
decl_stmt|;
name|int
name|error
init|=
literal|0
decl_stmt|,
name|slpflag
decl_stmt|,
name|slptimeo
decl_stmt|;
if|if
condition|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_INT
operator|)
operator|==
literal|0
condition|)
name|intrflg
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|intrflg
condition|)
block|{
name|slpflag
operator|=
name|PCATCH
expr_stmt|;
name|slptimeo
operator|=
literal|2
operator|*
name|hz
expr_stmt|;
block|}
else|else
block|{
name|slpflag
operator|=
literal|0
expr_stmt|;
name|slptimeo
operator|=
literal|0
expr_stmt|;
block|}
comment|/* 	 * First wait for any other process doing a flush to complete. 	 */
while|while
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NFLUSHINPROG
condition|)
block|{
name|np
operator|->
name|n_flag
operator||=
name|NFLUSHWANT
expr_stmt|;
name|error
operator|=
name|tsleep
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|np
operator|->
name|n_flag
argument_list|,
name|PRIBIO
operator|+
literal|2
argument_list|,
literal|"nfsvinval"
argument_list|,
name|slptimeo
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|&&
name|intrflg
operator|&&
name|nfs_sigintr
argument_list|(
name|nmp
argument_list|,
operator|(
expr|struct
name|nfsreq
operator|*
operator|)
literal|0
argument_list|,
name|p
argument_list|)
condition|)
return|return
operator|(
name|EINTR
operator|)
return|;
block|}
comment|/* 	 * Now, flush as required. 	 */
name|np
operator|->
name|n_flag
operator||=
name|NFLUSHINPROG
expr_stmt|;
name|error
operator|=
name|vinvalbuf
argument_list|(
name|vp
argument_list|,
name|flags
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
name|slpflag
argument_list|,
literal|0
argument_list|)
expr_stmt|;
while|while
condition|(
name|error
condition|)
block|{
if|if
condition|(
name|intrflg
operator|&&
name|nfs_sigintr
argument_list|(
name|nmp
argument_list|,
operator|(
expr|struct
name|nfsreq
operator|*
operator|)
literal|0
argument_list|,
name|p
argument_list|)
condition|)
block|{
name|np
operator|->
name|n_flag
operator|&=
operator|~
name|NFLUSHINPROG
expr_stmt|;
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NFLUSHWANT
condition|)
block|{
name|np
operator|->
name|n_flag
operator|&=
operator|~
name|NFLUSHWANT
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|np
operator|->
name|n_flag
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
name|EINTR
operator|)
return|;
block|}
name|error
operator|=
name|vinvalbuf
argument_list|(
name|vp
argument_list|,
name|flags
argument_list|,
name|cred
argument_list|,
name|p
argument_list|,
literal|0
argument_list|,
name|slptimeo
argument_list|)
expr_stmt|;
block|}
name|np
operator|->
name|n_flag
operator|&=
operator|~
operator|(
name|NMODIFIED
operator||
name|NFLUSHINPROG
operator|)
expr_stmt|;
if|if
condition|(
name|np
operator|->
name|n_flag
operator|&
name|NFLUSHWANT
condition|)
block|{
name|np
operator|->
name|n_flag
operator|&=
operator|~
name|NFLUSHWANT
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|np
operator|->
name|n_flag
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Initiate asynchronous I/O. Return an error if no nfsiods are available.  * This is mainly to avoid queueing async I/O requests when the nfsiods  * are all hung on a dead server.  */
end_comment

begin_function
name|int
name|nfs_asyncio
parameter_list|(
name|bp
parameter_list|,
name|cred
parameter_list|)
specifier|register
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|struct
name|ucred
modifier|*
name|cred
decl_stmt|;
block|{
specifier|register
name|int
name|i
decl_stmt|;
if|if
condition|(
name|nfs_numasync
operator|==
literal|0
condition|)
return|return
operator|(
name|EIO
operator|)
return|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|NFS_MAXASYNCDAEMON
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|nfs_iodwant
index|[
name|i
index|]
condition|)
block|{
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
name|B_READ
condition|)
block|{
if|if
condition|(
name|bp
operator|->
name|b_rcred
operator|==
name|NOCRED
operator|&&
name|cred
operator|!=
name|NOCRED
condition|)
block|{
name|crhold
argument_list|(
name|cred
argument_list|)
expr_stmt|;
name|bp
operator|->
name|b_rcred
operator|=
name|cred
expr_stmt|;
block|}
block|}
else|else
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_WRITEINPROG
expr_stmt|;
if|if
condition|(
name|bp
operator|->
name|b_wcred
operator|==
name|NOCRED
operator|&&
name|cred
operator|!=
name|NOCRED
condition|)
block|{
name|crhold
argument_list|(
name|cred
argument_list|)
expr_stmt|;
name|bp
operator|->
name|b_wcred
operator|=
name|cred
expr_stmt|;
block|}
block|}
name|TAILQ_INSERT_TAIL
argument_list|(
operator|&
name|nfs_bufq
argument_list|,
name|bp
argument_list|,
name|b_freelist
argument_list|)
expr_stmt|;
name|nfs_iodwant
index|[
name|i
index|]
operator|=
operator|(
expr|struct
name|proc
operator|*
operator|)
literal|0
expr_stmt|;
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
operator|&
name|nfs_iodwant
index|[
name|i
index|]
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
comment|/* 	 * If it is a read or a write already marked B_WRITEINPROG or B_NOCACHE 	 * return EIO so the process will call nfs_doio() and do it 	 * synchronously. 	 */
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
operator|(
name|B_READ
operator||
name|B_WRITEINPROG
operator||
name|B_NOCACHE
operator|)
condition|)
return|return
operator|(
name|EIO
operator|)
return|;
comment|/* 	 * Just turn the async write into a delayed write, instead of 	 * doing in synchronously. Hopefully, at least one of the nfsiods 	 * is currently doing a write for this file and will pick up the 	 * delayed writes before going back to sleep. 	 */
name|bp
operator|->
name|b_flags
operator||=
name|B_DELWRI
expr_stmt|;
name|reassignbuf
argument_list|(
name|bp
argument_list|,
name|bp
operator|->
name|b_vp
argument_list|)
expr_stmt|;
name|biodone
argument_list|(
name|bp
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Do an I/O operation to/from a cache block. This may be called  * synchronously or from an nfsiod.  */
end_comment

begin_function
name|int
name|nfs_doio
parameter_list|(
name|bp
parameter_list|,
name|cr
parameter_list|,
name|p
parameter_list|)
specifier|register
name|struct
name|buf
modifier|*
name|bp
decl_stmt|;
name|struct
name|ucred
modifier|*
name|cr
decl_stmt|;
name|struct
name|proc
modifier|*
name|p
decl_stmt|;
block|{
specifier|register
name|struct
name|uio
modifier|*
name|uiop
decl_stmt|;
specifier|register
name|struct
name|vnode
modifier|*
name|vp
decl_stmt|;
name|struct
name|nfsnode
modifier|*
name|np
decl_stmt|;
name|struct
name|nfsmount
modifier|*
name|nmp
decl_stmt|;
name|int
name|error
init|=
literal|0
decl_stmt|,
name|diff
decl_stmt|,
name|len
decl_stmt|,
name|iomode
decl_stmt|,
name|must_commit
init|=
literal|0
decl_stmt|;
name|struct
name|uio
name|uio
decl_stmt|;
name|struct
name|iovec
name|io
decl_stmt|;
name|nfsquad_t
name|tquad
decl_stmt|;
name|vp
operator|=
name|bp
operator|->
name|b_vp
expr_stmt|;
name|np
operator|=
name|VTONFS
argument_list|(
name|vp
argument_list|)
expr_stmt|;
name|nmp
operator|=
name|VFSTONFS
argument_list|(
name|vp
operator|->
name|v_mount
argument_list|)
expr_stmt|;
name|uiop
operator|=
operator|&
name|uio
expr_stmt|;
name|uiop
operator|->
name|uio_iov
operator|=
operator|&
name|io
expr_stmt|;
name|uiop
operator|->
name|uio_iovcnt
operator|=
literal|1
expr_stmt|;
name|uiop
operator|->
name|uio_segflg
operator|=
name|UIO_SYSSPACE
expr_stmt|;
name|uiop
operator|->
name|uio_procp
operator|=
name|p
expr_stmt|;
comment|/* 	 * Historically, paging was done with physio, but no more. 	 */
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
name|B_PHYS
condition|)
block|{
comment|/* 	     * ...though reading /dev/drum still gets us here. 	     */
name|io
operator|.
name|iov_len
operator|=
name|uiop
operator|->
name|uio_resid
operator|=
name|bp
operator|->
name|b_bcount
expr_stmt|;
comment|/* mapping was done by vmapbuf() */
name|io
operator|.
name|iov_base
operator|=
name|bp
operator|->
name|b_data
expr_stmt|;
name|uiop
operator|->
name|uio_offset
operator|=
operator|(
operator|(
name|off_t
operator|)
name|bp
operator|->
name|b_blkno
operator|)
operator|*
name|DEV_BSIZE
expr_stmt|;
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
name|B_READ
condition|)
block|{
name|uiop
operator|->
name|uio_rw
operator|=
name|UIO_READ
expr_stmt|;
name|nfsstats
operator|.
name|read_physios
operator|++
expr_stmt|;
name|error
operator|=
name|nfs_readrpc
argument_list|(
name|vp
argument_list|,
name|uiop
argument_list|,
name|cr
argument_list|)
expr_stmt|;
block|}
else|else
name|panic
argument_list|(
literal|"physio write"
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_ERROR
expr_stmt|;
name|bp
operator|->
name|b_error
operator|=
name|error
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
name|B_READ
condition|)
block|{
name|io
operator|.
name|iov_len
operator|=
name|uiop
operator|->
name|uio_resid
operator|=
name|bp
operator|->
name|b_bcount
expr_stmt|;
name|io
operator|.
name|iov_base
operator|=
name|bp
operator|->
name|b_data
expr_stmt|;
name|uiop
operator|->
name|uio_rw
operator|=
name|UIO_READ
expr_stmt|;
switch|switch
condition|(
name|vp
operator|->
name|v_type
condition|)
block|{
case|case
name|VREG
case|:
name|uiop
operator|->
name|uio_offset
operator|=
operator|(
operator|(
name|off_t
operator|)
name|bp
operator|->
name|b_blkno
operator|)
operator|*
name|DEV_BSIZE
expr_stmt|;
name|nfsstats
operator|.
name|read_bios
operator|++
expr_stmt|;
name|error
operator|=
name|nfs_readrpc
argument_list|(
name|vp
argument_list|,
name|uiop
argument_list|,
name|cr
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|error
condition|)
block|{
name|bp
operator|->
name|b_validoff
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|uiop
operator|->
name|uio_resid
condition|)
block|{
comment|/* 			 * If len> 0, there is a hole in the file and 			 * no writes after the hole have been pushed to 			 * the server yet. 			 * Just zero fill the rest of the valid area. 			 */
name|diff
operator|=
name|bp
operator|->
name|b_bcount
operator|-
name|uiop
operator|->
name|uio_resid
expr_stmt|;
name|len
operator|=
name|np
operator|->
name|n_size
operator|-
operator|(
operator|(
operator|(
name|u_quad_t
operator|)
name|bp
operator|->
name|b_blkno
operator|)
operator|*
name|DEV_BSIZE
operator|+
name|diff
operator|)
expr_stmt|;
if|if
condition|(
name|len
operator|>
literal|0
condition|)
block|{
name|len
operator|=
name|min
argument_list|(
name|len
argument_list|,
name|uiop
operator|->
name|uio_resid
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|bp
operator|->
name|b_data
operator|+
name|diff
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|bp
operator|->
name|b_validend
operator|=
name|diff
operator|+
name|len
expr_stmt|;
block|}
else|else
name|bp
operator|->
name|b_validend
operator|=
name|diff
expr_stmt|;
block|}
else|else
name|bp
operator|->
name|b_validend
operator|=
name|bp
operator|->
name|b_bcount
expr_stmt|;
block|}
if|if
condition|(
name|p
operator|&&
operator|(
name|vp
operator|->
name|v_flag
operator|&
name|VTEXT
operator|)
operator|&&
operator|(
operator|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_NQNFS
operator|)
operator|&&
name|NQNFS_CKINVALID
argument_list|(
name|vp
argument_list|,
name|np
argument_list|,
name|ND_READ
argument_list|)
operator|&&
name|np
operator|->
name|n_lrev
operator|!=
name|np
operator|->
name|n_brev
operator|)
operator|||
operator|(
operator|!
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_NQNFS
operator|)
operator|&&
name|np
operator|->
name|n_mtime
operator|!=
name|np
operator|->
name|n_vattr
operator|.
name|va_mtime
operator|.
name|ts_sec
operator|)
operator|)
condition|)
block|{
name|uprintf
argument_list|(
literal|"Process killed due to text file modification\n"
argument_list|)
expr_stmt|;
name|psignal
argument_list|(
name|p
argument_list|,
name|SIGKILL
argument_list|)
expr_stmt|;
name|p
operator|->
name|p_flag
operator||=
name|P_NOSWAP
expr_stmt|;
block|}
break|break;
case|case
name|VLNK
case|:
name|uiop
operator|->
name|uio_offset
operator|=
operator|(
name|off_t
operator|)
literal|0
expr_stmt|;
name|nfsstats
operator|.
name|readlink_bios
operator|++
expr_stmt|;
name|error
operator|=
name|nfs_readlinkrpc
argument_list|(
name|vp
argument_list|,
name|uiop
argument_list|,
name|cr
argument_list|)
expr_stmt|;
break|break;
case|case
name|VDIR
case|:
name|nfsstats
operator|.
name|readdir_bios
operator|++
expr_stmt|;
name|uiop
operator|->
name|uio_offset
operator|=
operator|(
operator|(
name|u_quad_t
operator|)
name|bp
operator|->
name|b_lblkno
operator|)
operator|*
name|NFS_DIRBLKSIZ
expr_stmt|;
if|if
condition|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_RDIRPLUS
condition|)
block|{
name|error
operator|=
name|nfs_readdirplusrpc
argument_list|(
name|vp
argument_list|,
name|uiop
argument_list|,
name|cr
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|==
name|NFSERR_NOTSUPP
condition|)
name|nmp
operator|->
name|nm_flag
operator|&=
operator|~
name|NFSMNT_RDIRPLUS
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|nmp
operator|->
name|nm_flag
operator|&
name|NFSMNT_RDIRPLUS
operator|)
operator|==
literal|0
condition|)
name|error
operator|=
name|nfs_readdirrpc
argument_list|(
name|vp
argument_list|,
name|uiop
argument_list|,
name|cr
argument_list|)
expr_stmt|;
break|break;
default|default:
name|printf
argument_list|(
literal|"nfs_doio:  type %x unexpected\n"
argument_list|,
name|vp
operator|->
name|v_type
argument_list|)
expr_stmt|;
break|break;
block|}
empty_stmt|;
if|if
condition|(
name|error
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_ERROR
expr_stmt|;
name|bp
operator|->
name|b_error
operator|=
name|error
expr_stmt|;
block|}
block|}
else|else
block|{
name|io
operator|.
name|iov_len
operator|=
name|uiop
operator|->
name|uio_resid
operator|=
name|bp
operator|->
name|b_dirtyend
operator|-
name|bp
operator|->
name|b_dirtyoff
expr_stmt|;
name|uiop
operator|->
name|uio_offset
operator|=
operator|(
operator|(
name|off_t
operator|)
name|bp
operator|->
name|b_blkno
operator|)
operator|*
name|DEV_BSIZE
operator|+
name|bp
operator|->
name|b_dirtyoff
expr_stmt|;
name|io
operator|.
name|iov_base
operator|=
operator|(
name|char
operator|*
operator|)
name|bp
operator|->
name|b_data
operator|+
name|bp
operator|->
name|b_dirtyoff
expr_stmt|;
name|uiop
operator|->
name|uio_rw
operator|=
name|UIO_WRITE
expr_stmt|;
name|nfsstats
operator|.
name|write_bios
operator|++
expr_stmt|;
if|if
condition|(
operator|(
name|bp
operator|->
name|b_flags
operator|&
operator|(
name|B_ASYNC
operator||
name|B_NEEDCOMMIT
operator||
name|B_NOCACHE
operator|)
operator|)
operator|==
name|B_ASYNC
condition|)
name|iomode
operator|=
name|NFSV3WRITE_UNSTABLE
expr_stmt|;
else|else
name|iomode
operator|=
name|NFSV3WRITE_FILESYNC
expr_stmt|;
name|bp
operator|->
name|b_flags
operator||=
name|B_WRITEINPROG
expr_stmt|;
name|error
operator|=
name|nfs_writerpc
argument_list|(
name|vp
argument_list|,
name|uiop
argument_list|,
name|cr
argument_list|,
operator|&
name|iomode
argument_list|,
operator|&
name|must_commit
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|error
operator|&&
name|iomode
operator|==
name|NFSV3WRITE_UNSTABLE
condition|)
name|bp
operator|->
name|b_flags
operator||=
name|B_NEEDCOMMIT
expr_stmt|;
else|else
name|bp
operator|->
name|b_flags
operator|&=
operator|~
name|B_NEEDCOMMIT
expr_stmt|;
name|bp
operator|->
name|b_flags
operator|&=
operator|~
name|B_WRITEINPROG
expr_stmt|;
comment|/* 	     * For an interrupted write, the buffer is still valid and the 	     * write hasn't been pushed to the server yet, so we can't set 	     * B_ERROR and report the interruption by setting B_EINTR. For 	     * the B_ASYNC case, B_EINTR is not relevant, so the rpc attempt 	     * is essentially a noop. 	     * For the case of a V3 write rpc not being committed to stable 	     * storage, the block is still dirty and requires either a commit 	     * rpc or another write rpc with iomode == NFSV3WRITE_FILESYNC 	     * before the block is reused. This is indicated by setting the 	     * B_DELWRI and B_NEEDCOMMIT flags. 	     */
if|if
condition|(
name|error
operator|==
name|EINTR
operator|||
operator|(
operator|!
name|error
operator|&&
operator|(
name|bp
operator|->
name|b_flags
operator|&
name|B_NEEDCOMMIT
operator|)
operator|)
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_DELWRI
expr_stmt|;
comment|/* 		 * Since for the B_ASYNC case, nfs_bwrite() has reassigned the 		 * buffer to the clean list, we have to reassign it back to the 		 * dirty one. Ugh. 		 */
if|if
condition|(
name|bp
operator|->
name|b_flags
operator|&
name|B_ASYNC
condition|)
name|reassignbuf
argument_list|(
name|bp
argument_list|,
name|vp
argument_list|)
expr_stmt|;
else|else
name|bp
operator|->
name|b_flags
operator||=
name|B_EINTR
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|error
condition|)
block|{
name|bp
operator|->
name|b_flags
operator||=
name|B_ERROR
expr_stmt|;
name|bp
operator|->
name|b_error
operator|=
name|np
operator|->
name|n_error
operator|=
name|error
expr_stmt|;
name|np
operator|->
name|n_flag
operator||=
name|NWRITEERR
expr_stmt|;
block|}
name|bp
operator|->
name|b_dirtyoff
operator|=
name|bp
operator|->
name|b_dirtyend
operator|=
literal|0
expr_stmt|;
block|}
block|}
name|bp
operator|->
name|b_resid
operator|=
name|uiop
operator|->
name|uio_resid
expr_stmt|;
if|if
condition|(
name|must_commit
condition|)
name|nfs_clearcommit
argument_list|(
name|vp
operator|->
name|v_mount
argument_list|)
expr_stmt|;
name|biodone
argument_list|(
name|bp
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
end_function

end_unit

