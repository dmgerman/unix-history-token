...
comment|/* { 		struct vnode *a_vp; 		struct uio *a_uio; 		struct ucred *a_cred; 		int *a_eofflag; 		u_long *a_cookies; 		int ncookies; 	} */
modifier|*
name|ap
decl_stmt|;
block|{
...
name|a_ncookies
decl_stmt|;
comment|/* 		 * Only the NFS server uses cookies, and it loads the 		 * directory block into system space, so we can just look at 		 * it directly. 		 */
if|if
condition|(
name|uio
operator|->
...
name|panic
argument_list|(
literal|"ufs_readdir: lost in space"
argument_list|)
expr_stmt|;
name|dp
operator|=
