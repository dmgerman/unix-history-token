begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (c) 1982, 1986, 1989, 1993  *	The Regents of the University of California.  All rights reserved.  *  * This code is derived from software contributed to Berkeley by  * Scooter Morris at Genentech Inc.  *  * %sccs.include.redist.c%  *  *	@(#)ufs_lockf.c	8.4 (Berkeley) %G%  */
end_comment

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/kernel.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/proc.h>
end_include

begin_include
include|#
directive|include
file|<sys/vnode.h>
end_include

begin_include
include|#
directive|include
file|<sys/malloc.h>
end_include

begin_include
include|#
directive|include
file|<sys/fcntl.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/lockf.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/quota.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/inode.h>
end_include

begin_include
include|#
directive|include
file|<ufs/ufs/ufs_extern.h>
end_include

begin_comment
comment|/*  * This variable controls the maximum number of processes that will  * be checked in doing deadlock detection.  */
end_comment

begin_decl_stmt
name|int
name|maxlockdepth
init|=
name|MAXDEPTH
decl_stmt|;
end_decl_stmt

begin_ifdef
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
end_ifdef

begin_include
include|#
directive|include
file|<vm/vm.h>
end_include

begin_include
include|#
directive|include
file|<sys/sysctl.h>
end_include

begin_decl_stmt
name|int
name|lockf_debug
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|ctldebug
name|debug4
init|=
block|{
literal|"lockf_debug"
block|,
operator|&
name|lockf_debug
block|}
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_define
define|#
directive|define
name|NOLOCKF
value|(struct lockf *)0
end_define

begin_define
define|#
directive|define
name|SELF
value|0x1
end_define

begin_define
define|#
directive|define
name|OTHERS
value|0x2
end_define

begin_define
define|#
directive|define
name|HASBLOCK
value|0x100
end_define

begin_comment
comment|/*  * Set a byte-range lock.  */
end_comment

begin_function
name|int
name|lf_setlock
parameter_list|(
name|lock
parameter_list|)
specifier|register
name|struct
name|lockf
modifier|*
name|lock
decl_stmt|;
block|{
specifier|register
name|struct
name|lockf
modifier|*
name|block
decl_stmt|;
name|struct
name|inode
modifier|*
name|ip
init|=
name|lock
operator|->
name|lf_inode
decl_stmt|;
name|struct
name|lockf
modifier|*
modifier|*
name|prev
decl_stmt|,
modifier|*
name|overlap
decl_stmt|,
modifier|*
name|ltmp
decl_stmt|;
specifier|static
name|char
name|lockstr
index|[]
init|=
literal|"lockf"
decl_stmt|;
name|int
name|ovcase
decl_stmt|,
name|priority
decl_stmt|,
name|needtolink
decl_stmt|,
name|error
decl_stmt|;
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|1
condition|)
name|lf_print
argument_list|(
literal|"lf_setlock"
argument_list|,
name|lock
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
comment|/* 	 * Set the priority 	 */
name|priority
operator|=
name|PLOCK
expr_stmt|;
if|if
condition|(
name|lock
operator|->
name|lf_type
operator|==
name|F_WRLCK
condition|)
name|priority
operator|+=
literal|4
expr_stmt|;
name|priority
operator||=
name|PCATCH
expr_stmt|;
comment|/* 	 * Scan lock list for this file looking for locks that would block us. 	 */
while|while
condition|(
name|block
operator|=
name|lf_getblock
argument_list|(
name|lock
argument_list|)
condition|)
block|{
comment|/* 		 * Free the structure and return if nonblocking. 		 */
if|if
condition|(
operator|(
name|lock
operator|->
name|lf_flags
operator|&
name|F_WAIT
operator|)
operator|==
literal|0
condition|)
block|{
name|FREE
argument_list|(
name|lock
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
return|return
operator|(
name|EAGAIN
operator|)
return|;
block|}
comment|/* 		 * We are blocked. Since flock style locks cover 		 * the whole file, there is no chance for deadlock. 		 * For byte-range locks we must check for deadlock. 		 * 		 * Deadlock detection is done by looking through the 		 * wait channels to see if there are any cycles that 		 * involve us. MAXDEPTH is set just to make sure we 		 * do not go off into neverland. 		 */
if|if
condition|(
operator|(
name|lock
operator|->
name|lf_flags
operator|&
name|F_POSIX
operator|)
operator|&&
operator|(
name|block
operator|->
name|lf_flags
operator|&
name|F_POSIX
operator|)
condition|)
block|{
specifier|register
name|struct
name|proc
modifier|*
name|wproc
decl_stmt|;
specifier|register
name|struct
name|lockf
modifier|*
name|waitblock
decl_stmt|;
name|int
name|i
init|=
literal|0
decl_stmt|;
comment|/* The block is waiting on something */
name|wproc
operator|=
operator|(
expr|struct
name|proc
operator|*
operator|)
name|block
operator|->
name|lf_id
expr_stmt|;
while|while
condition|(
name|wproc
operator|->
name|p_wchan
operator|&&
operator|(
name|wproc
operator|->
name|p_wmesg
operator|==
name|lockstr
operator|)
operator|&&
operator|(
name|i
operator|++
operator|<
name|maxlockdepth
operator|)
condition|)
block|{
name|waitblock
operator|=
operator|(
expr|struct
name|lockf
operator|*
operator|)
name|wproc
operator|->
name|p_wchan
expr_stmt|;
comment|/* Get the owner of the blocking lock */
name|waitblock
operator|=
name|waitblock
operator|->
name|lf_next
expr_stmt|;
if|if
condition|(
operator|(
name|waitblock
operator|->
name|lf_flags
operator|&
name|F_POSIX
operator|)
operator|==
literal|0
condition|)
break|break;
name|wproc
operator|=
operator|(
expr|struct
name|proc
operator|*
operator|)
name|waitblock
operator|->
name|lf_id
expr_stmt|;
if|if
condition|(
name|wproc
operator|==
operator|(
expr|struct
name|proc
operator|*
operator|)
name|lock
operator|->
name|lf_id
condition|)
block|{
name|free
argument_list|(
name|lock
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
return|return
operator|(
name|EDEADLK
operator|)
return|;
block|}
block|}
block|}
comment|/* 		 * For flock type locks, we must first remove 		 * any shared locks that we hold before we sleep 		 * waiting for an exclusive lock. 		 */
if|if
condition|(
operator|(
name|lock
operator|->
name|lf_flags
operator|&
name|F_FLOCK
operator|)
operator|&&
name|lock
operator|->
name|lf_type
operator|==
name|F_WRLCK
condition|)
block|{
name|lock
operator|->
name|lf_type
operator|=
name|F_UNLCK
expr_stmt|;
operator|(
name|void
operator|)
name|lf_clearlock
argument_list|(
name|lock
argument_list|)
expr_stmt|;
name|lock
operator|->
name|lf_type
operator|=
name|F_WRLCK
expr_stmt|;
block|}
comment|/* 		 * Add our lock to the blocked list and sleep until we're free. 		 * Remember who blocked us (for deadlock detection). 		 */
name|lock
operator|->
name|lf_next
operator|=
name|block
expr_stmt|;
name|TAILQ_INSERT_TAIL
argument_list|(
operator|&
name|block
operator|->
name|lf_blkhd
argument_list|,
name|lock
argument_list|,
name|lf_block
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|1
condition|)
block|{
name|lf_print
argument_list|(
literal|"lf_setlock: blocking on"
argument_list|,
name|block
argument_list|)
expr_stmt|;
name|lf_printlist
argument_list|(
literal|"lf_setlock"
argument_list|,
name|block
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
if|if
condition|(
name|error
operator|=
name|tsleep
argument_list|(
operator|(
name|caddr_t
operator|)
name|lock
argument_list|,
name|priority
argument_list|,
name|lockstr
argument_list|,
literal|0
argument_list|)
condition|)
block|{
comment|/* 			 * We may have been awakened by a signal (in 			 * which case we must remove ourselves from the 			 * blocked list) and/or by another process 			 * releasing a lock (in which case we have already 			 * been removed from the blocked list and our 			 * lf_next field set to NOLOCKF). 			 */
if|if
condition|(
name|lock
operator|->
name|lf_next
condition|)
name|TAILQ_REMOVE
argument_list|(
operator|&
name|lock
operator|->
name|lf_next
operator|->
name|lf_blkhd
argument_list|,
name|lock
argument_list|,
name|lf_block
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|lock
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
return|return
operator|(
name|error
operator|)
return|;
block|}
block|}
comment|/* 	 * No blocks!!  Add the lock.  Note that we will 	 * downgrade or upgrade any overlapping locks this 	 * process already owns. 	 * 	 * Skip over locks owned by other processes. 	 * Handle any locks that overlap and are owned by ourselves. 	 */
name|prev
operator|=
operator|&
name|ip
operator|->
name|i_lockf
expr_stmt|;
name|block
operator|=
name|ip
operator|->
name|i_lockf
expr_stmt|;
name|needtolink
operator|=
literal|1
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
name|ovcase
operator|=
name|lf_findoverlap
argument_list|(
name|block
argument_list|,
name|lock
argument_list|,
name|SELF
argument_list|,
operator|&
name|prev
argument_list|,
operator|&
name|overlap
argument_list|)
condition|)
name|block
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
comment|/* 		 * Six cases: 		 *	0) no overlap 		 *	1) overlap == lock 		 *	2) overlap contains lock 		 *	3) lock contains overlap 		 *	4) overlap starts before lock 		 *	5) overlap ends after lock 		 */
switch|switch
condition|(
name|ovcase
condition|)
block|{
case|case
literal|0
case|:
comment|/* no overlap */
if|if
condition|(
name|needtolink
condition|)
block|{
operator|*
name|prev
operator|=
name|lock
expr_stmt|;
name|lock
operator|->
name|lf_next
operator|=
name|overlap
expr_stmt|;
block|}
break|break;
case|case
literal|1
case|:
comment|/* overlap == lock */
comment|/* 			 * If downgrading lock, others may be 			 * able to acquire it. 			 */
if|if
condition|(
name|lock
operator|->
name|lf_type
operator|==
name|F_RDLCK
operator|&&
name|overlap
operator|->
name|lf_type
operator|==
name|F_WRLCK
condition|)
name|lf_wakelock
argument_list|(
name|overlap
argument_list|)
expr_stmt|;
name|overlap
operator|->
name|lf_type
operator|=
name|lock
operator|->
name|lf_type
expr_stmt|;
name|FREE
argument_list|(
name|lock
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
name|lock
operator|=
name|overlap
expr_stmt|;
comment|/* for debug output below */
break|break;
case|case
literal|2
case|:
comment|/* overlap contains lock */
comment|/* 			 * Check for common starting point and different types. 			 */
if|if
condition|(
name|overlap
operator|->
name|lf_type
operator|==
name|lock
operator|->
name|lf_type
condition|)
block|{
name|free
argument_list|(
name|lock
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
name|lock
operator|=
name|overlap
expr_stmt|;
comment|/* for debug output below */
break|break;
block|}
if|if
condition|(
name|overlap
operator|->
name|lf_start
operator|==
name|lock
operator|->
name|lf_start
condition|)
block|{
operator|*
name|prev
operator|=
name|lock
expr_stmt|;
name|lock
operator|->
name|lf_next
operator|=
name|overlap
expr_stmt|;
name|overlap
operator|->
name|lf_start
operator|=
name|lock
operator|->
name|lf_end
operator|+
literal|1
expr_stmt|;
block|}
else|else
name|lf_split
argument_list|(
name|overlap
argument_list|,
name|lock
argument_list|)
expr_stmt|;
name|lf_wakelock
argument_list|(
name|overlap
argument_list|)
expr_stmt|;
break|break;
case|case
literal|3
case|:
comment|/* lock contains overlap */
comment|/* 			 * If downgrading lock, others may be able to 			 * acquire it, otherwise take the list. 			 */
if|if
condition|(
name|lock
operator|->
name|lf_type
operator|==
name|F_RDLCK
operator|&&
name|overlap
operator|->
name|lf_type
operator|==
name|F_WRLCK
condition|)
block|{
name|lf_wakelock
argument_list|(
name|overlap
argument_list|)
expr_stmt|;
block|}
else|else
block|{
while|while
condition|(
name|ltmp
operator|=
name|overlap
operator|->
name|lf_blkhd
operator|.
name|tqh_first
condition|)
block|{
name|TAILQ_REMOVE
argument_list|(
operator|&
name|overlap
operator|->
name|lf_blkhd
argument_list|,
name|ltmp
argument_list|,
name|lf_block
argument_list|)
expr_stmt|;
name|TAILQ_INSERT_TAIL
argument_list|(
operator|&
name|lock
operator|->
name|lf_blkhd
argument_list|,
name|ltmp
argument_list|,
name|lf_block
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* 			 * Add the new lock if necessary and delete the overlap. 			 */
if|if
condition|(
name|needtolink
condition|)
block|{
operator|*
name|prev
operator|=
name|lock
expr_stmt|;
name|lock
operator|->
name|lf_next
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
name|prev
operator|=
operator|&
name|lock
operator|->
name|lf_next
expr_stmt|;
name|needtolink
operator|=
literal|0
expr_stmt|;
block|}
else|else
operator|*
name|prev
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
name|free
argument_list|(
name|overlap
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
continue|continue;
case|case
literal|4
case|:
comment|/* overlap starts before lock */
comment|/* 			 * Add lock after overlap on the list. 			 */
name|lock
operator|->
name|lf_next
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
name|overlap
operator|->
name|lf_next
operator|=
name|lock
expr_stmt|;
name|overlap
operator|->
name|lf_end
operator|=
name|lock
operator|->
name|lf_start
operator|-
literal|1
expr_stmt|;
name|prev
operator|=
operator|&
name|lock
operator|->
name|lf_next
expr_stmt|;
name|lf_wakelock
argument_list|(
name|overlap
argument_list|)
expr_stmt|;
name|needtolink
operator|=
literal|0
expr_stmt|;
continue|continue;
case|case
literal|5
case|:
comment|/* overlap ends after lock */
comment|/* 			 * Add the new lock before overlap. 			 */
if|if
condition|(
name|needtolink
condition|)
block|{
operator|*
name|prev
operator|=
name|lock
expr_stmt|;
name|lock
operator|->
name|lf_next
operator|=
name|overlap
expr_stmt|;
block|}
name|overlap
operator|->
name|lf_start
operator|=
name|lock
operator|->
name|lf_end
operator|+
literal|1
expr_stmt|;
name|lf_wakelock
argument_list|(
name|overlap
argument_list|)
expr_stmt|;
break|break;
block|}
break|break;
block|}
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|1
condition|)
block|{
name|lf_print
argument_list|(
literal|"lf_setlock: got the lock"
argument_list|,
name|lock
argument_list|)
expr_stmt|;
name|lf_printlist
argument_list|(
literal|"lf_setlock"
argument_list|,
name|lock
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Remove a byte-range lock on an inode.  *  * Generally, find the lock (or an overlap to that lock)  * and remove it (or shrink it), then wakeup anyone we can.  */
end_comment

begin_function
name|int
name|lf_clearlock
parameter_list|(
name|unlock
parameter_list|)
specifier|register
name|struct
name|lockf
modifier|*
name|unlock
decl_stmt|;
block|{
name|struct
name|inode
modifier|*
name|ip
init|=
name|unlock
operator|->
name|lf_inode
decl_stmt|;
specifier|register
name|struct
name|lockf
modifier|*
name|lf
init|=
name|ip
operator|->
name|i_lockf
decl_stmt|;
name|struct
name|lockf
modifier|*
name|overlap
decl_stmt|,
modifier|*
modifier|*
name|prev
decl_stmt|;
name|int
name|ovcase
decl_stmt|;
if|if
condition|(
name|lf
operator|==
name|NOLOCKF
condition|)
return|return
operator|(
literal|0
operator|)
return|;
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|unlock
operator|->
name|lf_type
operator|!=
name|F_UNLCK
condition|)
name|panic
argument_list|(
literal|"lf_clearlock: bad type"
argument_list|)
expr_stmt|;
if|if
condition|(
name|lockf_debug
operator|&
literal|1
condition|)
name|lf_print
argument_list|(
literal|"lf_clearlock"
argument_list|,
name|unlock
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
name|prev
operator|=
operator|&
name|ip
operator|->
name|i_lockf
expr_stmt|;
while|while
condition|(
name|ovcase
operator|=
name|lf_findoverlap
argument_list|(
name|lf
argument_list|,
name|unlock
argument_list|,
name|SELF
argument_list|,
operator|&
name|prev
argument_list|,
operator|&
name|overlap
argument_list|)
condition|)
block|{
comment|/* 		 * Wakeup the list of locks to be retried. 		 */
name|lf_wakelock
argument_list|(
name|overlap
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|ovcase
condition|)
block|{
case|case
literal|1
case|:
comment|/* overlap == lock */
operator|*
name|prev
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
name|FREE
argument_list|(
name|overlap
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
break|break;
case|case
literal|2
case|:
comment|/* overlap contains lock: split it */
if|if
condition|(
name|overlap
operator|->
name|lf_start
operator|==
name|unlock
operator|->
name|lf_start
condition|)
block|{
name|overlap
operator|->
name|lf_start
operator|=
name|unlock
operator|->
name|lf_end
operator|+
literal|1
expr_stmt|;
break|break;
block|}
name|lf_split
argument_list|(
name|overlap
argument_list|,
name|unlock
argument_list|)
expr_stmt|;
name|overlap
operator|->
name|lf_next
operator|=
name|unlock
operator|->
name|lf_next
expr_stmt|;
break|break;
case|case
literal|3
case|:
comment|/* lock contains overlap */
operator|*
name|prev
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
name|lf
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
name|free
argument_list|(
name|overlap
argument_list|,
name|M_LOCKF
argument_list|)
expr_stmt|;
continue|continue;
case|case
literal|4
case|:
comment|/* overlap starts before lock */
name|overlap
operator|->
name|lf_end
operator|=
name|unlock
operator|->
name|lf_start
operator|-
literal|1
expr_stmt|;
name|prev
operator|=
operator|&
name|overlap
operator|->
name|lf_next
expr_stmt|;
name|lf
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
continue|continue;
case|case
literal|5
case|:
comment|/* overlap ends after lock */
name|overlap
operator|->
name|lf_start
operator|=
name|unlock
operator|->
name|lf_end
operator|+
literal|1
expr_stmt|;
break|break;
block|}
break|break;
block|}
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|1
condition|)
name|lf_printlist
argument_list|(
literal|"lf_clearlock"
argument_list|,
name|unlock
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Check whether there is a blocking lock,  * and if so return its process identifier.  */
end_comment

begin_function
name|int
name|lf_getlock
parameter_list|(
name|lock
parameter_list|,
name|fl
parameter_list|)
specifier|register
name|struct
name|lockf
modifier|*
name|lock
decl_stmt|;
specifier|register
name|struct
name|flock
modifier|*
name|fl
decl_stmt|;
block|{
specifier|register
name|struct
name|lockf
modifier|*
name|block
decl_stmt|;
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|1
condition|)
name|lf_print
argument_list|(
literal|"lf_getlock"
argument_list|,
name|lock
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
if|if
condition|(
name|block
operator|=
name|lf_getblock
argument_list|(
name|lock
argument_list|)
condition|)
block|{
name|fl
operator|->
name|l_type
operator|=
name|block
operator|->
name|lf_type
expr_stmt|;
name|fl
operator|->
name|l_whence
operator|=
name|SEEK_SET
expr_stmt|;
name|fl
operator|->
name|l_start
operator|=
name|block
operator|->
name|lf_start
expr_stmt|;
if|if
condition|(
name|block
operator|->
name|lf_end
operator|==
operator|-
literal|1
condition|)
name|fl
operator|->
name|l_len
operator|=
literal|0
expr_stmt|;
else|else
name|fl
operator|->
name|l_len
operator|=
name|block
operator|->
name|lf_end
operator|-
name|block
operator|->
name|lf_start
operator|+
literal|1
expr_stmt|;
if|if
condition|(
name|block
operator|->
name|lf_flags
operator|&
name|F_POSIX
condition|)
name|fl
operator|->
name|l_pid
operator|=
operator|(
operator|(
expr|struct
name|proc
operator|*
operator|)
operator|(
name|block
operator|->
name|lf_id
operator|)
operator|)
operator|->
name|p_pid
expr_stmt|;
else|else
name|fl
operator|->
name|l_pid
operator|=
operator|-
literal|1
expr_stmt|;
block|}
else|else
block|{
name|fl
operator|->
name|l_type
operator|=
name|F_UNLCK
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Walk the list of locks for an inode and  * return the first blocking lock.  */
end_comment

begin_function
name|struct
name|lockf
modifier|*
name|lf_getblock
parameter_list|(
name|lock
parameter_list|)
specifier|register
name|struct
name|lockf
modifier|*
name|lock
decl_stmt|;
block|{
name|struct
name|lockf
modifier|*
modifier|*
name|prev
decl_stmt|,
modifier|*
name|overlap
decl_stmt|,
modifier|*
name|lf
init|=
name|lock
operator|->
name|lf_inode
operator|->
name|i_lockf
decl_stmt|;
name|int
name|ovcase
decl_stmt|;
name|prev
operator|=
operator|&
name|lock
operator|->
name|lf_inode
operator|->
name|i_lockf
expr_stmt|;
while|while
condition|(
name|ovcase
operator|=
name|lf_findoverlap
argument_list|(
name|lf
argument_list|,
name|lock
argument_list|,
name|OTHERS
argument_list|,
operator|&
name|prev
argument_list|,
operator|&
name|overlap
argument_list|)
condition|)
block|{
comment|/* 		 * We've found an overlap, see if it blocks us 		 */
if|if
condition|(
operator|(
name|lock
operator|->
name|lf_type
operator|==
name|F_WRLCK
operator|||
name|overlap
operator|->
name|lf_type
operator|==
name|F_WRLCK
operator|)
condition|)
return|return
operator|(
name|overlap
operator|)
return|;
comment|/* 		 * Nope, point to the next one on the list and 		 * see if it blocks us 		 */
name|lf
operator|=
name|overlap
operator|->
name|lf_next
expr_stmt|;
block|}
return|return
operator|(
name|NOLOCKF
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Walk the list of locks for an inode to  * find an overlapping lock (if any).  *  * NOTE: this returns only the FIRST overlapping lock.  There  *	 may be more than one.  */
end_comment

begin_function
name|int
name|lf_findoverlap
parameter_list|(
name|lf
parameter_list|,
name|lock
parameter_list|,
name|type
parameter_list|,
name|prev
parameter_list|,
name|overlap
parameter_list|)
specifier|register
name|struct
name|lockf
modifier|*
name|lf
decl_stmt|;
name|struct
name|lockf
modifier|*
name|lock
decl_stmt|;
name|int
name|type
decl_stmt|;
name|struct
name|lockf
modifier|*
modifier|*
modifier|*
name|prev
decl_stmt|;
name|struct
name|lockf
modifier|*
modifier|*
name|overlap
decl_stmt|;
block|{
name|off_t
name|start
decl_stmt|,
name|end
decl_stmt|;
operator|*
name|overlap
operator|=
name|lf
expr_stmt|;
if|if
condition|(
name|lf
operator|==
name|NOLOCKF
condition|)
return|return
operator|(
literal|0
operator|)
return|;
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|lf_print
argument_list|(
literal|"lf_findoverlap: looking for overlap in"
argument_list|,
name|lock
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
name|start
operator|=
name|lock
operator|->
name|lf_start
expr_stmt|;
name|end
operator|=
name|lock
operator|->
name|lf_end
expr_stmt|;
while|while
condition|(
name|lf
operator|!=
name|NOLOCKF
condition|)
block|{
if|if
condition|(
operator|(
operator|(
name|type
operator|&
name|SELF
operator|)
operator|&&
name|lf
operator|->
name|lf_id
operator|!=
name|lock
operator|->
name|lf_id
operator|)
operator|||
operator|(
operator|(
name|type
operator|&
name|OTHERS
operator|)
operator|&&
name|lf
operator|->
name|lf_id
operator|==
name|lock
operator|->
name|lf_id
operator|)
condition|)
block|{
operator|*
name|prev
operator|=
operator|&
name|lf
operator|->
name|lf_next
expr_stmt|;
operator|*
name|overlap
operator|=
name|lf
operator|=
name|lf
operator|->
name|lf_next
expr_stmt|;
continue|continue;
block|}
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|lf_print
argument_list|(
literal|"\tchecking"
argument_list|,
name|lf
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
comment|/* 		 * OK, check for overlap 		 * 		 * Six cases: 		 *	0) no overlap 		 *	1) overlap == lock 		 *	2) overlap contains lock 		 *	3) lock contains overlap 		 *	4) overlap starts before lock 		 *	5) overlap ends after lock 		 */
if|if
condition|(
operator|(
name|lf
operator|->
name|lf_end
operator|!=
operator|-
literal|1
operator|&&
name|start
operator|>
name|lf
operator|->
name|lf_end
operator|)
operator|||
operator|(
name|end
operator|!=
operator|-
literal|1
operator|&&
name|lf
operator|->
name|lf_start
operator|>
name|end
operator|)
condition|)
block|{
comment|/* Case 0 */
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|printf
argument_list|(
literal|"no overlap\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
if|if
condition|(
operator|(
name|type
operator|&
name|SELF
operator|)
operator|&&
name|end
operator|!=
operator|-
literal|1
operator|&&
name|lf
operator|->
name|lf_start
operator|>
name|end
condition|)
return|return
operator|(
literal|0
operator|)
return|;
operator|*
name|prev
operator|=
operator|&
name|lf
operator|->
name|lf_next
expr_stmt|;
operator|*
name|overlap
operator|=
name|lf
operator|=
name|lf
operator|->
name|lf_next
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
operator|(
name|lf
operator|->
name|lf_start
operator|==
name|start
operator|)
operator|&&
operator|(
name|lf
operator|->
name|lf_end
operator|==
name|end
operator|)
condition|)
block|{
comment|/* Case 1 */
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|printf
argument_list|(
literal|"overlap == lock\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
return|return
operator|(
literal|1
operator|)
return|;
block|}
if|if
condition|(
operator|(
name|lf
operator|->
name|lf_start
operator|<=
name|start
operator|)
operator|&&
operator|(
name|end
operator|!=
operator|-
literal|1
operator|)
operator|&&
operator|(
operator|(
name|lf
operator|->
name|lf_end
operator|>=
name|end
operator|)
operator|||
operator|(
name|lf
operator|->
name|lf_end
operator|==
operator|-
literal|1
operator|)
operator|)
condition|)
block|{
comment|/* Case 2 */
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|printf
argument_list|(
literal|"overlap contains lock\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
return|return
operator|(
literal|2
operator|)
return|;
block|}
if|if
condition|(
name|start
operator|<=
name|lf
operator|->
name|lf_start
operator|&&
operator|(
name|end
operator|==
operator|-
literal|1
operator|||
operator|(
name|lf
operator|->
name|lf_end
operator|!=
operator|-
literal|1
operator|&&
name|end
operator|>=
name|lf
operator|->
name|lf_end
operator|)
operator|)
condition|)
block|{
comment|/* Case 3 */
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|printf
argument_list|(
literal|"lock contains overlap\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
return|return
operator|(
literal|3
operator|)
return|;
block|}
if|if
condition|(
operator|(
name|lf
operator|->
name|lf_start
operator|<
name|start
operator|)
operator|&&
operator|(
operator|(
name|lf
operator|->
name|lf_end
operator|>=
name|start
operator|)
operator|||
operator|(
name|lf
operator|->
name|lf_end
operator|==
operator|-
literal|1
operator|)
operator|)
condition|)
block|{
comment|/* Case 4 */
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|printf
argument_list|(
literal|"overlap starts before lock\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
return|return
operator|(
literal|4
operator|)
return|;
block|}
if|if
condition|(
operator|(
name|lf
operator|->
name|lf_start
operator|>
name|start
operator|)
operator|&&
operator|(
name|end
operator|!=
operator|-
literal|1
operator|)
operator|&&
operator|(
operator|(
name|lf
operator|->
name|lf_end
operator|>
name|end
operator|)
operator|||
operator|(
name|lf
operator|->
name|lf_end
operator|==
operator|-
literal|1
operator|)
operator|)
condition|)
block|{
comment|/* Case 5 */
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|printf
argument_list|(
literal|"overlap ends after lock\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
return|return
operator|(
literal|5
operator|)
return|;
block|}
name|panic
argument_list|(
literal|"lf_findoverlap: default"
argument_list|)
expr_stmt|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Split a lock and a contained region into  * two or three locks as necessary.  */
end_comment

begin_function
name|void
name|lf_split
parameter_list|(
name|lock1
parameter_list|,
name|lock2
parameter_list|)
specifier|register
name|struct
name|lockf
modifier|*
name|lock1
decl_stmt|;
specifier|register
name|struct
name|lockf
modifier|*
name|lock2
decl_stmt|;
block|{
specifier|register
name|struct
name|lockf
modifier|*
name|splitlock
decl_stmt|;
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
block|{
name|lf_print
argument_list|(
literal|"lf_split"
argument_list|,
name|lock1
argument_list|)
expr_stmt|;
name|lf_print
argument_list|(
literal|"splitting from"
argument_list|,
name|lock2
argument_list|)
expr_stmt|;
block|}
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
comment|/* 	 * Check to see if spliting into only two pieces. 	 */
if|if
condition|(
name|lock1
operator|->
name|lf_start
operator|==
name|lock2
operator|->
name|lf_start
condition|)
block|{
name|lock1
operator|->
name|lf_start
operator|=
name|lock2
operator|->
name|lf_end
operator|+
literal|1
expr_stmt|;
name|lock2
operator|->
name|lf_next
operator|=
name|lock1
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|lock1
operator|->
name|lf_end
operator|==
name|lock2
operator|->
name|lf_end
condition|)
block|{
name|lock1
operator|->
name|lf_end
operator|=
name|lock2
operator|->
name|lf_start
operator|-
literal|1
expr_stmt|;
name|lock2
operator|->
name|lf_next
operator|=
name|lock1
operator|->
name|lf_next
expr_stmt|;
name|lock1
operator|->
name|lf_next
operator|=
name|lock2
expr_stmt|;
return|return;
block|}
comment|/* 	 * Make a new lock consisting of the last part of 	 * the encompassing lock 	 */
name|MALLOC
argument_list|(
name|splitlock
argument_list|,
expr|struct
name|lockf
operator|*
argument_list|,
sizeof|sizeof
expr|*
name|splitlock
argument_list|,
name|M_LOCKF
argument_list|,
name|M_WAITOK
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|caddr_t
operator|)
name|lock1
argument_list|,
operator|(
name|caddr_t
operator|)
name|splitlock
argument_list|,
sizeof|sizeof
expr|*
name|splitlock
argument_list|)
expr_stmt|;
name|splitlock
operator|->
name|lf_start
operator|=
name|lock2
operator|->
name|lf_end
operator|+
literal|1
expr_stmt|;
name|TAILQ_INIT
argument_list|(
operator|&
name|splitlock
operator|->
name|lf_blkhd
argument_list|)
expr_stmt|;
name|splitlock
operator|->
name|lf_spare
operator|=
name|NOLOCKF
expr_stmt|;
name|lock1
operator|->
name|lf_end
operator|=
name|lock2
operator|->
name|lf_start
operator|-
literal|1
expr_stmt|;
comment|/* 	 * OK, now link it in 	 */
name|splitlock
operator|->
name|lf_next
operator|=
name|lock1
operator|->
name|lf_next
expr_stmt|;
name|lock2
operator|->
name|lf_next
operator|=
name|splitlock
expr_stmt|;
name|lock1
operator|->
name|lf_next
operator|=
name|lock2
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Wakeup a blocklist  */
end_comment

begin_function
name|void
name|lf_wakelock
parameter_list|(
name|listhead
parameter_list|)
name|struct
name|lockf
modifier|*
name|listhead
decl_stmt|;
block|{
specifier|register
name|struct
name|lockf
modifier|*
name|wakelock
decl_stmt|;
if|if
condition|(
name|listhead
operator|->
name|lf_block
operator|!=
name|NOLOCKF
condition|)
block|{
if|if
condition|(
operator|(
name|listhead
operator|->
name|lf_flags
operator|&
name|HASBLOCK
operator|)
operator|==
literal|0
condition|)
name|panic
argument_list|(
literal|"lf_wakelock: listhead unexpected ptr"
argument_list|)
expr_stmt|;
if|if
condition|(
name|listhead
operator|->
name|lf_block
operator|!=
name|listhead
operator|->
name|lf_spare
condition|)
name|panic
argument_list|(
literal|"lf_wakelock: listhead corrupted ptr"
argument_list|)
expr_stmt|;
block|}
while|while
condition|(
name|wakelock
operator|=
name|listhead
operator|->
name|lf_blkhd
operator|.
name|tqh_first
condition|)
block|{
name|TAILQ_REMOVE
argument_list|(
operator|&
name|listhead
operator|->
name|lf_blkhd
argument_list|,
name|wakelock
argument_list|,
name|lf_block
argument_list|)
expr_stmt|;
name|wakelock
operator|->
name|lf_next
operator|=
name|NOLOCKF
expr_stmt|;
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
if|if
condition|(
name|lockf_debug
operator|&
literal|2
condition|)
name|lf_print
argument_list|(
literal|"lf_wakelock: awakening"
argument_list|,
name|wakelock
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* LOCKF_DEBUG */
name|wakeup
argument_list|(
operator|(
name|caddr_t
operator|)
name|wakelock
argument_list|)
expr_stmt|;
block|}
name|listhead
operator|->
name|lf_flags
operator|&=
operator|~
name|HASBLOCK
expr_stmt|;
name|listhead
operator|->
name|lf_spare
operator|=
name|NOLOCKF
expr_stmt|;
name|listhead
operator|->
name|lf_block
operator|=
name|NOLOCKF
expr_stmt|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|LOCKF_DEBUG
end_ifdef

begin_comment
comment|/*  * Print out a lock.  */
end_comment

begin_macro
name|lf_print
argument_list|(
argument|tag
argument_list|,
argument|lock
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|tag
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|register
name|struct
name|lockf
modifier|*
name|lock
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|printf
argument_list|(
literal|"%s: lock 0x%lx for "
argument_list|,
name|tag
argument_list|,
name|lock
argument_list|)
expr_stmt|;
if|if
condition|(
name|lock
operator|->
name|lf_flags
operator|&
name|F_POSIX
condition|)
name|printf
argument_list|(
literal|"proc %d"
argument_list|,
operator|(
operator|(
expr|struct
name|proc
operator|*
operator|)
operator|(
name|lock
operator|->
name|lf_id
operator|)
operator|)
operator|->
name|p_pid
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"id 0x%x"
argument_list|,
name|lock
operator|->
name|lf_id
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|" in ino %d on dev<%d, %d>, %s, start %d, end %d"
argument_list|,
name|lock
operator|->
name|lf_inode
operator|->
name|i_number
argument_list|,
name|major
argument_list|(
name|lock
operator|->
name|lf_inode
operator|->
name|i_dev
argument_list|)
argument_list|,
name|minor
argument_list|(
name|lock
operator|->
name|lf_inode
operator|->
name|i_dev
argument_list|)
argument_list|,
name|lock
operator|->
name|lf_type
operator|==
name|F_RDLCK
condition|?
literal|"shared"
else|:
name|lock
operator|->
name|lf_type
operator|==
name|F_WRLCK
condition|?
literal|"exclusive"
else|:
name|lock
operator|->
name|lf_type
operator|==
name|F_UNLCK
condition|?
literal|"unlock"
else|:
literal|"unknown"
argument_list|,
name|lock
operator|->
name|lf_start
argument_list|,
name|lock
operator|->
name|lf_end
argument_list|)
expr_stmt|;
if|if
condition|(
name|lock
operator|->
name|lf_blkhd
operator|.
name|tqh_first
condition|)
name|printf
argument_list|(
literal|" block 0x%x\n"
argument_list|,
name|lock
operator|->
name|lf_blkhd
operator|.
name|tqh_first
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
end_block

begin_macro
name|lf_printlist
argument_list|(
argument|tag
argument_list|,
argument|lock
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|tag
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|struct
name|lockf
modifier|*
name|lock
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|lockf
modifier|*
name|lf
decl_stmt|,
modifier|*
name|blk
decl_stmt|;
name|printf
argument_list|(
literal|"%s: Lock list for ino %d on dev<%d, %d>:\n"
argument_list|,
name|tag
argument_list|,
name|lock
operator|->
name|lf_inode
operator|->
name|i_number
argument_list|,
name|major
argument_list|(
name|lock
operator|->
name|lf_inode
operator|->
name|i_dev
argument_list|)
argument_list|,
name|minor
argument_list|(
name|lock
operator|->
name|lf_inode
operator|->
name|i_dev
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|lf
operator|=
name|lock
operator|->
name|lf_inode
operator|->
name|i_lockf
init|;
name|lf
condition|;
name|lf
operator|=
name|lf
operator|->
name|lf_next
control|)
block|{
name|printf
argument_list|(
literal|"\tlock 0x%lx for "
argument_list|,
name|lf
argument_list|)
expr_stmt|;
if|if
condition|(
name|lf
operator|->
name|lf_flags
operator|&
name|F_POSIX
condition|)
name|printf
argument_list|(
literal|"proc %d"
argument_list|,
operator|(
operator|(
expr|struct
name|proc
operator|*
operator|)
operator|(
name|lf
operator|->
name|lf_id
operator|)
operator|)
operator|->
name|p_pid
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"id 0x%x"
argument_list|,
name|lf
operator|->
name|lf_id
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|", %s, start %d, end %d"
argument_list|,
name|lf
operator|->
name|lf_type
operator|==
name|F_RDLCK
condition|?
literal|"shared"
else|:
name|lf
operator|->
name|lf_type
operator|==
name|F_WRLCK
condition|?
literal|"exclusive"
else|:
name|lf
operator|->
name|lf_type
operator|==
name|F_UNLCK
condition|?
literal|"unlock"
else|:
literal|"unknown"
argument_list|,
name|lf
operator|->
name|lf_start
argument_list|,
name|lf
operator|->
name|lf_end
argument_list|)
expr_stmt|;
for|for
control|(
name|blk
operator|=
name|lf
operator|->
name|lf_blkhd
operator|.
name|tqh_first
init|;
name|blk
condition|;
name|blk
operator|=
name|blk
operator|->
name|lf_block
operator|.
name|tqe_next
control|)
block|{
name|printf
argument_list|(
literal|"\n\t\tlock request 0x%lx for "
argument_list|,
name|blk
argument_list|)
expr_stmt|;
if|if
condition|(
name|blk
operator|->
name|lf_flags
operator|&
name|F_POSIX
condition|)
name|printf
argument_list|(
literal|"proc %d"
argument_list|,
operator|(
operator|(
expr|struct
name|proc
operator|*
operator|)
operator|(
name|blk
operator|->
name|lf_id
operator|)
operator|)
operator|->
name|p_pid
argument_list|)
expr_stmt|;
else|else
name|printf
argument_list|(
literal|"id 0x%x"
argument_list|,
name|blk
operator|->
name|lf_id
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|", %s, start %d, end %d"
argument_list|,
name|blk
operator|->
name|lf_type
operator|==
name|F_RDLCK
condition|?
literal|"shared"
else|:
name|blk
operator|->
name|lf_type
operator|==
name|F_WRLCK
condition|?
literal|"exclusive"
else|:
name|blk
operator|->
name|lf_type
operator|==
name|F_UNLCK
condition|?
literal|"unlock"
else|:
literal|"unknown"
argument_list|,
name|blk
operator|->
name|lf_start
argument_list|,
name|blk
operator|->
name|lf_end
argument_list|)
expr_stmt|;
if|if
condition|(
name|blk
operator|->
name|lf_blkhd
operator|.
name|tqh_first
condition|)
name|panic
argument_list|(
literal|"lf_printlist: bad list"
argument_list|)
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* LOCKF_DEBUG */
end_comment

end_unit

