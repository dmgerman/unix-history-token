...
name|stop_stack_dummy
condition|)
comment|/* This used to say        "Cannot continue previously requested operation".  */
name|error
argument_list|(
...
begin_comment
comment|/*  * detach_command --  * takes a program previously attached to and detaches it.  * The program resumes execution and will no longer stop  * on signals, etc.  We call clear_breakpoints() to make sure  * all the breakpoints are out when we let it go.  Otherwise,  * the program will die when it hits one.  For this to work,  * it may be necessary for the process to have been  * previously attached.  It *might* work if the program was  * started via the normal ptrace (PTRACE_TRACEME).  */
end_comment
