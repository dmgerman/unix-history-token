...
decl_stmt|;
name|char
name|n_hist_dimension
index|[
literal|15
index|]
decl_stmt|;
name|char
name|n_hist_dimension_abbrev
decl_stmt|;
name|double
name|n_hist_scale
decl_stmt|;
...
name|_
argument_list|(
literal|"%s: dimension unit changed between histogram records\n"
literal|"%s: from '%s'\n"
literal|"%s: to '%s'\n"
argument_list|)
argument_list|,
...
name|_
argument_list|(
literal|"%s: dimension abbreviation changed between histogram records\n"
literal|"%s: from '%c'\n"
literal|"%s: to '%c'\n"
argument_list|)
argument_list|,
...
expr_stmt|;
block|}
comment|/* The only reason we require the same scale for histograms is that 	 there's code (notably printing code), that prints units, 	 and it would be very confusing to have one unit mean different 	 things for different functions.  */
if|if
condition|(
name|fabs
argument_list|(
...
name|_
argument_list|(
literal|"%s: different scales in histogram records"
argument_list|)
argument_list|,
name|whoami
argument_list|)
...
name|i
decl_stmt|;
comment|/* 1. Read the header and see if there's existing record for the      same address range and that there are no overlapping records.  */
name|read_histogram_header
argument_list|(
operator|&
name|n_record
...
block|{
comment|/* If this record overlaps, but does not completely match an existing 	 record, it's an error.  */
name|lowpc
operator|=
name|n_record
operator|.
...
name|_
argument_list|(
literal|"%s: overlapping histogram records\n"
argument_list|)
argument_list|,
name|whoami
argument_list|)
...
expr_stmt|;
block|}
comment|/* This is new record.  Add it to global array and allocate space for 	 the samples.  */
name|histograms
operator|=
name|xrealloc
argument_list|(
...
expr_stmt|;
block|}
comment|/* 2. We have either a new record (with zeroed histogram data), or an existing      record with some data in the histogram already.  Read new data into the      record, adding hit counts.  */
name|DBG
argument_list|(
name|SAMPLEDEBUG
argument_list|,
...
begin_comment
comment|/* Write all execution histograms file OFP.  FILENAME is the name    of OFP and is provided for formatting error-messages only.  */
end_comment
...
begin_comment
comment|/* Calls 'hist_assign_sampes_1' for all histogram records read so far. */
end_comment
...
name|_
argument_list|(
literal|"%s: found a symbol that covers "
literal|"several histogram records"
argument_list|)
argument_list|,
name|whoami
argument_list|)
...
begin_comment
comment|/* Find and return exising histogram record having the same lowpc and    highpc as passed via the parameters.  Return NULL if nothing is found.    The return value is valid until any new histogram is read.  */
end_comment
...
begin_comment
comment|/* Given a PC, return histogram record which address range include this PC.    Return NULL if there's no such record.  */
end_comment
