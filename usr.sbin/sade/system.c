begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * The new sysinstall program.  *  * This is probably the last program in the `sysinstall' line - the next  * generation being essentially a complete rewrite.  *  * $Id: system.c,v 1.1.1.1 1995/04/27 12:50:34 jkh Exp $  *  * Jordan Hubbard  *  * My contributions are in the public domain.  *  * Parts of this file are also blatently stolen from Poul-Henning Kamp's  * previous version of sysinstall, and as such fall under his "BEERWARE"  * license, so buy him a beer if you like it!  Buy him a beer for me, too!  */
end_comment

begin_include
include|#
directive|include
file|"sysinstall.h"
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<sys/reboot.h>
end_include

begin_include
include|#
directive|include
file|<sys/fcntl.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/wait.h>
end_include

begin_comment
comment|/* Handle interrupt signals (duh!) */
end_comment

begin_function
specifier|static
name|void
name|handle_intr
parameter_list|(
name|int
name|sig
parameter_list|)
block|{ }
end_function

begin_comment
comment|/* Welcome the user to the system */
end_comment

begin_function
name|void
name|systemWelcome
parameter_list|(
name|void
parameter_list|)
block|{ }
end_function

begin_comment
comment|/* Initialize system defaults */
end_comment

begin_function
name|void
name|systemInitialize
parameter_list|(
name|int
name|argc
parameter_list|,
name|char
modifier|*
modifier|*
name|argv
parameter_list|)
block|{
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|SIG_IGN
argument_list|)
expr_stmt|;
name|globalsInit
argument_list|()
expr_stmt|;
comment|/* Are we running as init? */
if|if
condition|(
name|getpid
argument_list|()
operator|==
literal|1
condition|)
block|{
name|setsid
argument_list|()
expr_stmt|;
if|if
condition|(
name|argc
operator|>
literal|1
operator|&&
name|strchr
argument_list|(
name|argv
index|[
literal|1
index|]
argument_list|,
literal|'C'
argument_list|)
condition|)
block|{
comment|/* Kernel told us that we are on a CDROM root */
name|close
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|open
argument_list|(
literal|"/bootcd/dev/console"
argument_list|,
name|O_RDWR
argument_list|)
expr_stmt|;
name|close
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|dup
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|close
argument_list|(
literal|2
argument_list|)
expr_stmt|;
name|dup
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|CpioFD
operator|=
name|open
argument_list|(
literal|"/floppies/cpio.flp"
argument_list|,
name|O_RDONLY
argument_list|)
expr_stmt|;
name|OnCDROM
operator|=
name|TRUE
expr_stmt|;
name|chroot
argument_list|(
literal|"/bootcd"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|close
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|open
argument_list|(
literal|"/dev/console"
argument_list|,
name|O_RDWR
argument_list|)
expr_stmt|;
name|close
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|dup
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|close
argument_list|(
literal|2
argument_list|)
expr_stmt|;
name|dup
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
name|msgInfo
argument_list|(
literal|"%s running as init"
argument_list|,
name|argv
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|ioctl
argument_list|(
literal|0
argument_list|,
name|TIOCSCTTY
argument_list|,
operator|(
name|char
operator|*
operator|)
name|NULL
argument_list|)
expr_stmt|;
name|setlogin
argument_list|(
literal|"root"
argument_list|)
expr_stmt|;
name|setbuf
argument_list|(
name|stdin
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|setbuf
argument_list|(
name|stdout
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|setbuf
argument_list|(
name|stderr
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|set_termcap
argument_list|()
operator|==
operator|-
literal|1
condition|)
name|msgFatal
argument_list|(
literal|"Can't find terminal entry"
argument_list|)
expr_stmt|;
comment|/* XXX - libdialog has particularly bad return value checking */
name|init_dialog
argument_list|()
expr_stmt|;
comment|/* If we haven't crashed I guess dialog is running ! */
name|DialogActive
operator|=
name|TRUE
expr_stmt|;
name|signal
argument_list|(
name|SIGINT
argument_list|,
name|handle_intr
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Close down and prepare to exit */
end_comment

begin_function
name|void
name|systemShutdown
parameter_list|(
name|void
parameter_list|)
block|{
if|if
condition|(
name|DialogActive
condition|)
block|{
name|end_dialog
argument_list|()
expr_stmt|;
name|DialogActive
operator|=
name|FALSE
expr_stmt|;
block|}
comment|/* REALLY exit! */
if|if
condition|(
name|getpid
argument_list|()
operator|==
literal|1
condition|)
name|reboot
argument_list|(
name|RB_HALT
argument_list|)
expr_stmt|;
else|else
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Run some general command */
end_comment

begin_function
name|int
name|systemExecute
parameter_list|(
name|char
modifier|*
name|command
parameter_list|)
block|{
name|int
name|status
decl_stmt|;
name|dialog_clear
argument_list|()
expr_stmt|;
name|dialog_update
argument_list|()
expr_stmt|;
name|end_dialog
argument_list|()
expr_stmt|;
name|DialogActive
operator|=
name|FALSE
expr_stmt|;
name|status
operator|=
name|system
argument_list|(
name|command
argument_list|)
expr_stmt|;
name|DialogActive
operator|=
name|TRUE
expr_stmt|;
name|dialog_clear
argument_list|()
expr_stmt|;
name|dialog_update
argument_list|()
expr_stmt|;
return|return
name|status
return|;
block|}
end_function

begin_comment
comment|/* Find and execute a shell */
end_comment

begin_function
name|int
name|systemShellEscape
parameter_list|(
name|void
parameter_list|)
block|{
name|char
modifier|*
name|sh
init|=
name|NULL
decl_stmt|;
if|if
condition|(
name|file_executable
argument_list|(
literal|"/bin/sh"
argument_list|)
condition|)
name|sh
operator|=
literal|"/bin/sh"
expr_stmt|;
elseif|else
if|if
condition|(
name|file_executable
argument_list|(
literal|"/stand/sh"
argument_list|)
condition|)
name|sh
operator|=
literal|"/stand/sh"
expr_stmt|;
else|else
block|{
name|msgWarn
argument_list|(
literal|"No shell available, sorry!"
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
name|setenv
argument_list|(
literal|"PS1"
argument_list|,
literal|"freebsd% "
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|dialog_clear
argument_list|()
expr_stmt|;
name|dialog_update
argument_list|()
expr_stmt|;
name|move
argument_list|(
literal|0
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|standout
argument_list|()
expr_stmt|;
name|addstr
argument_list|(
literal|"Type `exit' to leave this shell and continue installallation"
argument_list|)
expr_stmt|;
name|standend
argument_list|()
expr_stmt|;
name|refresh
argument_list|()
expr_stmt|;
name|end_dialog
argument_list|()
expr_stmt|;
name|DialogActive
operator|=
name|FALSE
expr_stmt|;
if|if
condition|(
name|fork
argument_list|()
operator|==
literal|0
condition|)
name|execlp
argument_list|(
name|sh
argument_list|,
literal|"-sh"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
else|else
name|wait
argument_list|(
name|NULL
argument_list|)
expr_stmt|;
name|dialog_clear
argument_list|()
expr_stmt|;
name|DialogActive
operator|=
name|TRUE
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

begin_comment
comment|/* Display a file in a filebox */
end_comment

begin_function
name|int
name|systemDisplayFile
parameter_list|(
name|char
modifier|*
name|file
parameter_list|)
block|{
name|char
name|buf
index|[
name|FILENAME_MAX
index|]
decl_stmt|,
modifier|*
name|cp
decl_stmt|,
modifier|*
name|fname
init|=
name|NULL
decl_stmt|;
if|if
condition|(
name|file_readable
argument_list|(
name|file
argument_list|)
condition|)
name|fname
operator|=
name|file
expr_stmt|;
elseif|else
if|if
condition|(
operator|(
name|cp
operator|=
name|getenv
argument_list|(
literal|"LANG"
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
name|snprintf
argument_list|(
name|buf
argument_list|,
name|FILENAME_MAX
argument_list|,
literal|"%s/%s"
argument_list|,
name|cp
argument_list|,
name|file
argument_list|)
expr_stmt|;
if|if
condition|(
name|file_readable
argument_list|(
name|buf
argument_list|)
condition|)
name|fname
operator|=
name|buf
expr_stmt|;
block|}
else|else
block|{
name|snprintf
argument_list|(
name|buf
argument_list|,
name|FILENAME_MAX
argument_list|,
literal|"english/%s"
argument_list|,
name|file
argument_list|)
expr_stmt|;
if|if
condition|(
name|file_readable
argument_list|(
name|buf
argument_list|)
condition|)
name|fname
operator|=
name|buf
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|fname
condition|)
block|{
name|snprintf
argument_list|(
name|buf
argument_list|,
name|FILENAME_MAX
argument_list|,
literal|"The %s file is not provided on this particular floppy image."
argument_list|,
name|file
argument_list|)
expr_stmt|;
name|dialog_msgbox
argument_list|(
literal|"Sorry!"
argument_list|,
name|buf
argument_list|,
operator|-
literal|1
argument_list|,
operator|-
literal|1
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|dialog_clear_norefresh
argument_list|()
expr_stmt|;
return|return
literal|1
return|;
block|}
else|else
block|{
name|dialog_clear_norefresh
argument_list|()
expr_stmt|;
name|dialog_textbox
argument_list|(
name|file
argument_list|,
name|fname
argument_list|,
name|LINES
argument_list|,
name|COLS
argument_list|)
expr_stmt|;
name|dialog_clear_norefresh
argument_list|()
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function

end_unit

