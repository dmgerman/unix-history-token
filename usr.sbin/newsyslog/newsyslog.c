...
name|warn
argument_list|(
literal|"can't expand pattern: %s"
argument_list|,
name|p
operator|->
name|log
...
name|stderr
argument_list|,
literal|"usage: newsyslog [-Fnrsv] [-f config-file] [-a directory] [ filename ... ]\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
...
literal|'\0'
expr_stmt|;
comment|/* 		 * If newsyslog was run with a list of specific filenames, 		 * then this line of the config file should be skipped if 		 * it is NOT one of those given files (except that we do 		 * want any line that defines the<default> action). 		 * 		 * XXX - note that CE_GLOB processing is *NOT* done when 		 *       trying to match a filename given on the command! 		 */
if|if
condition|(
operator|*
name|files
...
block|{
comment|/* 			 * If no files were specified on the command line, 			 * then we can skip any line which defines the 			 * default action. 			 */
if|if
condition|(
name|strcasecmp
argument_list|(
...
condition|)
block|{
comment|/* 			 * This entry did not specify the 'n' flag, which 			 * means it should signal syslogd unless it had 			 * specified some other pid-file.  But we only 			 * try to notify syslog if we are root 			 */
if|if
condition|(
name|needroot
condition|)
...
argument_list|)
expr_stmt|;
comment|/* 	 * The entire config file has been processed.  If there were 	 * no specific files given on the run command, then the work 	 * of this routine is done. 	 */
if|if
condition|(
operator|*
name|files
...
operator|)
return|;
comment|/* 	 * If the program was given a specific list of files to process, 	 * it may be that some of those files were not listed in the 	 * config file.  Those unlisted files should get the default 	 * rotation action.  First, create the default-rotation action 	 * if none was found in the config file. 	 */
if|if
condition|(
name|defconf
operator|==
...
name|printf
argument_list|(
literal|"\t+ No entry for %s  (will use %s)\n"
argument_list|,
operator|*
name|given
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 		 * This given file was not found in the config file. 		 * Add another item on to our work list, based on the 		 * default entry. 		 */
name|working
operator|=
name|init_entry
argument_list|(
...
name|working
expr_stmt|;
comment|/* This is a file that was *not* found in config file */
name|working
operator|->
name|def_cfg
operator|=
