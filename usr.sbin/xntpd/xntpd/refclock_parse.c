...
begin_comment
comment|/*  * Trimble SV6 GPS receivers (TAIP and TSIP protocols)  */
end_comment
...
value|(B9600|CS8|CLOCAL|CREAD|PARENB|PARODD)
end_define
...
value|(TRIMBLETAIP_FLAGS|PARSE_F_NOPOLLONLY)
end_define
...
begin_comment
comment|/* 20 ms as a l_uf - avg GPS time message latency */
end_comment
...
name|LOG_ERR
argument_list|,
literal|"PARSE receiver #%d: trimbletaip_init: tcgetattr(fd,&tm): %m"
argument_list|,
name|CL_UNIT
argument_list|(
name|parse
...
name|LOG_ERR
argument_list|,
literal|"PARSE receiver #%d: trimbletaip_init: tcsetattr(fd,&tm): %m"
argument_list|,
name|CL_UNIT
argument_list|(
name|parse
...
begin_comment
comment|/*-------------------------------------------------------------------  * sendcmd, sendbyte, sendetx, sendflt, sendint implement the command  * interface to the receiver.  *  * CAVEAT: the sendflt, sendint routines are byte order dependend and  * float implementation dependend - these must be converted to portable  * versions !  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* UTC, compute on seconds, send only on request */
name|sendbyte
argument_list|(
operator|&
name|buf
