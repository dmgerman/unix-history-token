...
begin_comment
comment|/*  * refclock_nmea.c - clock driver for an NMEA GPS CLOCK  *		Michael Petry Jun 20, 1994  *		 based on refclock_heath.c  */
end_comment
...
begin_comment
comment|/*  * This driver supports the NMEA GPS Receiver with  *  * Protype was refclock_trak.c, Thanks a lot.  *  * The reciever used spits out the NMEA sentences for boat navigation.  * And you thought it was an information superhighway.  Try a raging river  * filled with rapids and whirlpools that rip away your data and warp time.  */
end_comment
...
condition|)
return|return;
comment|/* 	 * We get a buffer and timestamp for each<cr>; however, we use 	 * the timestamp of "now" since this may be a broadcast instead 	 * of a poll.  This needs to be checked empeerically 	 */
name|gettstamp
argument_list|(
operator|&
name|up
...
directive|endif
comment|/* 	 * We check the timecode format and decode its contents. The 	 * we only care about a few of them.  The most important being 	 * the $GPRMC format 	 * $GPRMC,hhmmss,a,fddmm.xx,n,dddmmm.xx,w,zz.z,yyy.,ddmmyy,dd,v*CC 	 */
define|#
directive|define
name|GPRMC
value|0
...
return|return;
block|}
comment|/* 	 * Test for synchronization  Check for quality byte. (soon) 	 */
name|pp
operator|->
name|leap
operator|=
