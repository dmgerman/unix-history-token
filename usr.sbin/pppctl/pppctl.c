begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<sys/un.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_include
include|#
directive|include
file|<signal.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_define
define|#
directive|define
name|LINELEN
value|2048
end_define

begin_decl_stmt
specifier|static
name|char
name|Buffer
index|[
name|LINELEN
index|]
decl_stmt|,
name|Command
index|[
name|LINELEN
index|]
decl_stmt|;
end_decl_stmt

begin_function
specifier|static
name|int
name|Usage
parameter_list|()
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Usage: pppctl [-v] [ -t n ] [ -p passwd ] Port|LocalSock command[;command]...\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"              -v tells pppctl to output all conversation\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"              -t n specifies a timeout of n seconds (default 2)\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"              -p passwd specifies your password\n"
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
end_function

begin_decl_stmt
specifier|static
name|int
name|TimedOut
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_function
name|void
name|Timeout
parameter_list|(
name|int
name|Sig
parameter_list|)
block|{
name|TimedOut
operator|=
literal|1
expr_stmt|;
block|}
end_function

begin_define
define|#
directive|define
name|REC_PASSWD
value|(1)
end_define

begin_define
define|#
directive|define
name|REC_SHOW
value|(2)
end_define

begin_define
define|#
directive|define
name|REC_VERBOSE
value|(4)
end_define

begin_decl_stmt
specifier|static
name|char
modifier|*
name|passwd
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|Receive
parameter_list|(
name|int
name|fd
parameter_list|,
name|unsigned
name|TimeoutVal
parameter_list|,
name|int
name|display
parameter_list|)
block|{
name|int
name|Result
decl_stmt|;
name|struct
name|sigaction
name|act
decl_stmt|,
name|oact
decl_stmt|;
name|int
name|len
decl_stmt|;
name|char
modifier|*
name|last
decl_stmt|;
name|TimedOut
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|TimeoutVal
condition|)
block|{
name|act
operator|.
name|sa_handler
operator|=
name|Timeout
expr_stmt|;
name|sigemptyset
argument_list|(
operator|&
name|act
operator|.
name|sa_mask
argument_list|)
expr_stmt|;
name|act
operator|.
name|sa_flags
operator|=
literal|0
expr_stmt|;
name|sigaction
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|act
argument_list|,
operator|&
name|oact
argument_list|)
expr_stmt|;
name|alarm
argument_list|(
name|TimeoutVal
argument_list|)
expr_stmt|;
block|}
name|len
operator|=
literal|0
expr_stmt|;
while|while
condition|(
name|Result
operator|=
name|read
argument_list|(
name|fd
argument_list|,
name|Buffer
operator|+
name|len
argument_list|,
sizeof|sizeof
argument_list|(
name|Buffer
argument_list|)
operator|-
name|len
operator|-
literal|1
argument_list|)
operator|,
name|Result
operator|!=
operator|-
literal|1
condition|)
block|{
name|len
operator|+=
name|Result
expr_stmt|;
name|Buffer
index|[
name|len
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|TimedOut
condition|)
block|{
if|if
condition|(
name|display
operator|&
name|REC_VERBOSE
condition|)
name|write
argument_list|(
literal|1
argument_list|,
name|Buffer
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|Result
operator|=
operator|-
literal|1
expr_stmt|;
break|break;
block|}
elseif|else
if|if
condition|(
name|len
operator|>
literal|2
operator|&&
operator|!
name|strcmp
argument_list|(
name|Buffer
operator|+
name|len
operator|-
literal|2
argument_list|,
literal|"> "
argument_list|)
condition|)
block|{
if|if
condition|(
name|display
operator|&
operator|(
name|REC_SHOW
operator||
name|REC_VERBOSE
operator|)
condition|)
block|{
if|if
condition|(
name|display
operator|&
name|REC_VERBOSE
condition|)
name|last
operator|=
name|Buffer
operator|+
name|len
operator|-
literal|1
expr_stmt|;
else|else
name|last
operator|=
name|rindex
argument_list|(
name|Buffer
argument_list|,
literal|'\n'
argument_list|)
expr_stmt|;
if|if
condition|(
name|last
condition|)
block|{
operator|*
operator|++
name|last
operator|=
literal|'\0'
expr_stmt|;
name|write
argument_list|(
literal|1
argument_list|,
name|Buffer
argument_list|,
name|last
operator|-
name|Buffer
argument_list|)
expr_stmt|;
block|}
block|}
for|for
control|(
name|last
operator|=
name|Buffer
operator|+
name|len
operator|-
literal|2
init|;
name|last
operator|>
name|Buffer
operator|&&
operator|*
name|last
operator|!=
literal|' '
condition|;
name|last
operator|--
control|)
empty_stmt|;
if|if
condition|(
name|last
operator|>
name|Buffer
operator|+
literal|3
operator|&&
operator|!
name|strncmp
argument_list|(
name|last
operator|-
literal|3
argument_list|,
literal|" on"
argument_list|,
literal|3
argument_list|)
condition|)
block|{
comment|/* a password is required ! */
if|if
condition|(
name|display
operator|&
name|REC_PASSWD
condition|)
block|{
if|if
condition|(
name|TimeoutVal
condition|)
block|{
name|alarm
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|sigaction
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|oact
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
comment|/* password time */
if|if
condition|(
operator|!
name|passwd
condition|)
name|passwd
operator|=
name|getpass
argument_list|(
literal|"Password: "
argument_list|)
expr_stmt|;
name|sprintf
argument_list|(
name|Buffer
argument_list|,
literal|"passwd %s\n"
argument_list|,
name|passwd
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|passwd
argument_list|,
name|strlen
argument_list|(
name|passwd
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|display
operator|&
name|REC_VERBOSE
condition|)
name|write
argument_list|(
literal|1
argument_list|,
name|Buffer
argument_list|,
name|strlen
argument_list|(
name|Buffer
argument_list|)
argument_list|)
expr_stmt|;
name|write
argument_list|(
name|fd
argument_list|,
name|Buffer
argument_list|,
name|strlen
argument_list|(
name|Buffer
argument_list|)
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
name|Buffer
argument_list|,
name|strlen
argument_list|(
name|Buffer
argument_list|)
argument_list|)
expr_stmt|;
return|return
name|Receive
argument_list|(
name|fd
argument_list|,
name|TimeoutVal
argument_list|,
name|display
operator|&
operator|~
name|REC_PASSWD
argument_list|)
return|;
block|}
name|Result
operator|=
literal|1
expr_stmt|;
block|}
else|else
name|Result
operator|=
literal|0
expr_stmt|;
break|break;
block|}
block|}
if|if
condition|(
name|TimedOut
condition|)
name|Result
operator|=
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|TimeoutVal
condition|)
block|{
name|alarm
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|sigaction
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|oact
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
return|return
name|Result
return|;
block|}
end_function

begin_function
name|int
name|main
parameter_list|(
name|int
name|argc
parameter_list|,
name|char
modifier|*
modifier|*
name|argv
parameter_list|)
block|{
name|struct
name|servent
modifier|*
name|s
decl_stmt|;
name|struct
name|hostent
modifier|*
name|h
decl_stmt|;
name|struct
name|sockaddr
modifier|*
name|sock
decl_stmt|;
name|struct
name|sockaddr_in
name|ifsin
decl_stmt|;
name|struct
name|sockaddr_un
name|ifsun
decl_stmt|;
name|int
name|socksz
decl_stmt|,
name|arg
decl_stmt|,
name|fd
decl_stmt|,
name|len
decl_stmt|,
name|verbose
decl_stmt|;
name|unsigned
name|TimeoutVal
decl_stmt|;
name|char
modifier|*
name|DoneWord
init|=
literal|"x"
decl_stmt|,
modifier|*
name|next
decl_stmt|,
modifier|*
name|start
decl_stmt|;
name|struct
name|sigaction
name|act
decl_stmt|,
name|oact
decl_stmt|;
name|verbose
operator|=
literal|0
expr_stmt|;
name|TimeoutVal
operator|=
literal|2
expr_stmt|;
for|for
control|(
name|arg
operator|=
literal|1
init|;
name|arg
operator|<
name|argc
condition|;
name|arg
operator|++
control|)
if|if
condition|(
operator|*
name|argv
index|[
name|arg
index|]
operator|==
literal|'-'
condition|)
block|{
for|for
control|(
name|start
operator|=
name|argv
index|[
name|arg
index|]
operator|+
literal|1
init|;
operator|*
name|start
condition|;
name|start
operator|++
control|)
switch|switch
condition|(
operator|*
name|start
condition|)
block|{
case|case
literal|'t'
case|:
name|TimeoutVal
operator|=
operator|(
name|unsigned
operator|)
name|atoi
argument_list|(
name|start
index|[
literal|1
index|]
condition|?
name|start
operator|+
literal|1
else|:
name|argv
index|[
operator|++
name|arg
index|]
argument_list|)
expr_stmt|;
name|start
operator|=
name|DoneWord
expr_stmt|;
break|break;
case|case
literal|'v'
case|:
name|verbose
operator|=
name|REC_VERBOSE
expr_stmt|;
break|break;
case|case
literal|'p'
case|:
name|passwd
operator|=
operator|(
name|start
index|[
literal|1
index|]
condition|?
name|start
operator|+
literal|1
else|:
name|argv
index|[
operator|++
name|arg
index|]
operator|)
expr_stmt|;
name|start
operator|=
name|DoneWord
expr_stmt|;
break|break;
default|default:
return|return
name|Usage
argument_list|()
return|;
block|}
block|}
else|else
break|break;
if|if
condition|(
name|argc
operator|<
name|arg
operator|+
literal|2
condition|)
return|return
name|Usage
argument_list|()
return|;
if|if
condition|(
operator|*
name|argv
index|[
name|arg
index|]
operator|==
literal|'/'
condition|)
block|{
name|sock
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|ifsun
expr_stmt|;
name|socksz
operator|=
sizeof|sizeof
name|ifsun
expr_stmt|;
name|ifsun
operator|.
name|sun_len
operator|=
name|strlen
argument_list|(
name|argv
index|[
name|arg
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|ifsun
operator|.
name|sun_len
operator|>
sizeof|sizeof
name|ifsun
operator|.
name|sun_path
operator|-
literal|1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: Path too long\n"
argument_list|,
name|argv
index|[
name|arg
index|]
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
name|ifsun
operator|.
name|sun_family
operator|=
name|AF_LOCAL
expr_stmt|;
name|strcpy
argument_list|(
name|ifsun
operator|.
name|sun_path
argument_list|,
name|argv
index|[
name|arg
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|=
name|socket
argument_list|(
name|AF_LOCAL
argument_list|,
name|SOCK_STREAM
argument_list|,
literal|0
argument_list|)
operator|,
name|fd
operator|<
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Cannot create local domain socket\n"
argument_list|)
expr_stmt|;
return|return
literal|2
return|;
block|}
block|}
else|else
block|{
name|sock
operator|=
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|ifsin
expr_stmt|;
name|socksz
operator|=
sizeof|sizeof
name|ifsin
expr_stmt|;
if|if
condition|(
operator|(
name|h
operator|=
name|gethostbyname
argument_list|(
literal|"localhost"
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Cannot resolve localhost\n"
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
if|if
condition|(
name|strspn
argument_list|(
name|argv
index|[
name|arg
index|]
argument_list|,
literal|"0123456789"
argument_list|)
operator|==
name|strlen
argument_list|(
name|argv
index|[
name|arg
index|]
argument_list|)
condition|)
name|ifsin
operator|.
name|sin_port
operator|=
name|htons
argument_list|(
name|atoi
argument_list|(
name|argv
index|[
name|arg
index|]
argument_list|)
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|s
operator|=
name|getservbyname
argument_list|(
name|argv
index|[
name|arg
index|]
argument_list|,
literal|"tcp"
argument_list|)
operator|,
operator|!
name|s
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s isn't a valid port or service!\n"
argument_list|,
name|argv
index|[
name|arg
index|]
argument_list|)
expr_stmt|;
return|return
name|Usage
argument_list|()
return|;
block|}
else|else
name|ifsin
operator|.
name|sin_port
operator|=
name|s
operator|->
name|s_port
expr_stmt|;
name|ifsin
operator|.
name|sin_len
operator|=
sizeof|sizeof
argument_list|(
name|ifsin
argument_list|)
expr_stmt|;
name|ifsin
operator|.
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|ifsin
operator|.
name|sin_addr
operator|.
name|s_addr
operator|=
operator|*
operator|(
name|u_long
operator|*
operator|)
name|h
operator|->
name|h_addr_list
index|[
literal|0
index|]
expr_stmt|;
if|if
condition|(
name|fd
operator|=
name|socket
argument_list|(
name|AF_INET
argument_list|,
name|SOCK_STREAM
argument_list|,
literal|0
argument_list|)
operator|,
name|fd
operator|<
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Cannot create internet socket\n"
argument_list|)
expr_stmt|;
return|return
literal|2
return|;
block|}
block|}
name|TimedOut
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|TimeoutVal
condition|)
block|{
name|act
operator|.
name|sa_handler
operator|=
name|Timeout
expr_stmt|;
name|sigemptyset
argument_list|(
operator|&
name|act
operator|.
name|sa_mask
argument_list|)
expr_stmt|;
name|act
operator|.
name|sa_flags
operator|=
literal|0
expr_stmt|;
name|sigaction
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|act
argument_list|,
operator|&
name|oact
argument_list|)
expr_stmt|;
name|alarm
argument_list|(
name|TimeoutVal
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|connect
argument_list|(
name|fd
argument_list|,
name|sock
argument_list|,
name|socksz
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|TimeoutVal
condition|)
block|{
name|alarm
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|sigaction
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|oact
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|TimedOut
condition|)
name|fputs
argument_list|(
literal|"Timeout: "
argument_list|,
name|stderr
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Cannot connect to socket %s\n"
argument_list|,
name|argv
index|[
name|arg
index|]
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
return|return
literal|3
return|;
block|}
if|if
condition|(
name|TimeoutVal
condition|)
block|{
name|alarm
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|sigaction
argument_list|(
name|SIGALRM
argument_list|,
operator|&
name|oact
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
name|len
operator|=
literal|0
expr_stmt|;
name|Command
index|[
sizeof|sizeof
argument_list|(
name|Command
argument_list|)
operator|-
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
for|for
control|(
name|arg
operator|++
init|;
name|arg
operator|<
name|argc
condition|;
name|arg
operator|++
control|)
block|{
if|if
condition|(
name|len
operator|&&
name|len
operator|<
sizeof|sizeof
argument_list|(
name|Command
argument_list|)
operator|-
literal|1
condition|)
name|strcpy
argument_list|(
name|Command
operator|+
name|len
operator|++
argument_list|,
literal|" "
argument_list|)
expr_stmt|;
name|strncpy
argument_list|(
name|Command
operator|+
name|len
argument_list|,
name|argv
index|[
name|arg
index|]
argument_list|,
sizeof|sizeof
argument_list|(
name|Command
argument_list|)
operator|-
name|len
operator|-
literal|1
argument_list|)
expr_stmt|;
name|len
operator|+=
name|strlen
argument_list|(
name|Command
operator|+
name|len
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|Receive
argument_list|(
name|fd
argument_list|,
name|TimeoutVal
argument_list|,
name|verbose
operator||
name|REC_PASSWD
argument_list|)
condition|)
block|{
case|case
literal|1
case|:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Password incorrect\n"
argument_list|)
expr_stmt|;
break|break;
case|case
literal|0
case|:
name|start
operator|=
name|Command
expr_stmt|;
do|do
block|{
name|next
operator|=
name|index
argument_list|(
name|start
argument_list|,
literal|';'
argument_list|)
expr_stmt|;
while|while
condition|(
operator|*
name|start
operator|==
literal|' '
operator|||
operator|*
name|start
operator|==
literal|'\t'
condition|)
name|start
operator|++
expr_stmt|;
if|if
condition|(
name|next
condition|)
operator|*
name|next
operator|=
literal|'\0'
expr_stmt|;
name|strcpy
argument_list|(
name|Buffer
argument_list|,
name|start
argument_list|)
expr_stmt|;
name|Buffer
index|[
sizeof|sizeof
argument_list|(
name|Buffer
argument_list|)
operator|-
literal|2
index|]
operator|=
literal|'\0'
expr_stmt|;
name|strcat
argument_list|(
name|Buffer
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|verbose
condition|)
name|write
argument_list|(
literal|1
argument_list|,
name|Buffer
argument_list|,
name|strlen
argument_list|(
name|Buffer
argument_list|)
argument_list|)
expr_stmt|;
name|write
argument_list|(
name|fd
argument_list|,
name|Buffer
argument_list|,
name|strlen
argument_list|(
name|Buffer
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|Receive
argument_list|(
name|fd
argument_list|,
name|TimeoutVal
argument_list|,
name|verbose
operator||
name|REC_SHOW
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"No reply from ppp\n"
argument_list|)
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|next
condition|)
name|start
operator|=
operator|++
name|next
expr_stmt|;
block|}
do|while
condition|(
name|next
operator|&&
operator|*
name|next
condition|)
do|;
if|if
condition|(
name|verbose
condition|)
name|puts
argument_list|(
literal|""
argument_list|)
expr_stmt|;
break|break;
default|default:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"ppp is not responding\n"
argument_list|)
expr_stmt|;
break|break;
block|}
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

end_unit

