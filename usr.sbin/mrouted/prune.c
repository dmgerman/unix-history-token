...
begin_comment
comment|/*  * The mrouted program is covered by the license in the accompanying file  * named "LICENSE".  Use of the mrouted program represents acceptance of  * the terms and conditions listed in that file.  *  * The mrouted program is COPYRIGHT 1989 by The Board of Trustees of  * Leland Stanford Junior University.  *  *  * $Id: prune.c,v 1.9 1995/07/10 16:13:05 wollman Exp $  */
end_comment
...
name|gt
parameter_list|)
value|{ \ 	register int _i; \ 	if ((ntohl((gt)->gt_mcastgrp)& 0xff000000) == 0xef000000) \ 	    for (_i = 0; _i< numvifs; _i++) \ 		if (scoped_addr(_i, (gt)->gt_mcastgrp)) \ 		    VIFM_SET(_i, (gt)->gt_scope); \ 	}
end_define
...
literal|0
argument_list|,
literal|"reset_neighbor_state del prunes (%s %s)"
argument_list|,
name|inet_fmts
argument_list|(
name|r
...
condition|)
block|{
comment|/* 		     * Expire prune, send again on this vif. 		     */
name|VIFM_SET
argument_list|(
name|pt
operator|->
...
literal|0
argument_list|,
literal|"reset_neighbor_state del sg (%s %s)"
argument_list|,
name|inet_fmt
argument_list|(
name|st
...
name|errno
argument_list|,
literal|"reset_neighbor_state trying to delete (%s %s)"
argument_list|,
name|inet_fmt
argument_list|(
name|st
...
name|NULL
expr_stmt|;
comment|/* 		 * Keep the group entries themselves around since the 		 * state will likely just come right back, and if not, 		 * the group entries will time out with no kernel entries 		 * and no prune state. 		 */
name|g
operator|->
name|gt_prsent_timer
operator|=
...
condition|)
block|{
comment|/* 	     * If it's about to expire, then it's only still around because 	     * of timer granularity, so don't warn about it. 	     */
if|if
condition|(
name|pt
operator|->
...
continue|continue;
block|}
comment|/* 	     * If this entry was pruned, but all downstream prunes 	     * have expired, then it is safe to simply delete it. 	     * Otherwise, check for traffic before deleting. 	     */
if|if
condition|(
name|gt
operator|->
...
condition|)
block|{
comment|/* At least one source in the list still has traffic */
name|gt
operator|->
name|gt_timer
operator|=
...
literal|0
argument_list|,
literal|"age_table_entry (P) deleting (%s %s)"
argument_list|,
name|inet_fmt
argument_list|(
name|st
...
name|errno
argument_list|,
literal|"age_table_entry (P) trying to delete (%s %s)"
argument_list|,
name|inet_fmt
argument_list|(
name|st
