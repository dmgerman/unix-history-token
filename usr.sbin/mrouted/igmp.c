...
index|[]
init|=
literal|"@(#) $Id: \ igmp.c,v 3.8.4.19 1998/01/06 01:57:43 fenner Exp $"
decl_stmt|;
end_decl_stmt
...
name|IF_DEBUG
argument_list|(
argument|DEBUG_PKT|igmp_debug_kind(igmp->igmp_type, igmp->igmp_code)
argument_list|)
name|log
argument_list|(
name|LOG_DEBUG
...
begin_comment
comment|/*  * Construct an IGMP message in the output packet buffer.  The caller may  * have already placed data in that buffer, of length 'datalen'.  */
end_comment
...
begin_comment
comment|/*   * Call build_igmp() to build an IGMP message in the output packet buffer.  * Then send the message from the interface with IP address 'src' to  * destination 'dst'.  */
end_comment
