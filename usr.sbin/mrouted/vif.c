...
begin_comment
comment|/*  * The mrouted program is covered by the license in the accompanying file  * named "LICENSE".  Use of the mrouted program represents acceptance of  * the terms and conditions listed in that file.  *  * The mrouted program is COPYRIGHT 1989 by The Board of Trustees of  * Leland Stanford Junior University.  *  *  * $Id: vif.c,v 3.5 1995/05/09 01:00:39 fenner Exp $  */
end_comment
...
index|]
expr_stmt|;
comment|/* If we consider ourselves the querier for this vif, but hear a      * query from a router with a lower IP address, yield to them.      *      * This is done here as well as in the neighbor discovery in case      * there is a querier that doesn't speak DVMRP.      */
if|if
condition|(
operator|(
name|v
...
argument_list|)
expr_stmt|;
comment|/*      * Check if the router gen-ids are the same (only if vers> 3.2)      * Need to reset the prune state of the router if not.      */
if|if
condition|(
name|msgtype
operator|==
...
literal|0
argument_list|,
literal|"reset neighbor %s on vif %d [old genid:%x, new:%x]"
argument_list|,
name|inet_fmt
argument_list|(
name|addr
...
argument_list|)
expr_stmt|;
comment|/*  		     * need to do a full route report here  		     * it gets done by accept_probe() 		     */
return|return
operator|(
name|TRUE
operator|)
return|;
block|}
comment|/*XXX nflags shouldn't be dealt with in 2 places in the same                  *XXX routine...*/
if|if
condition|(
name|n
operator|->
...
expr_stmt|;
block|}
comment|/* Neighbor flags changed, do a full report */
return|return
name|TRUE
return|;
block|}
...
literal|0
argument_list|,
literal|"resetting neighbor %s [old:%d.%d, new:%d.%d]"
argument_list|,
name|inet_fmt
argument_list|(
name|addr
...
begin_comment
comment|/*  * Return the neighbor's version number  * returns (protocol_version<< 8 + mrouted_version) of neighbor  */
end_comment
