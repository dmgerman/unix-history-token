...
begin_comment
comment|/*  * The mrouted program is covered by the license in the accompanying file  * named "LICENSE".  Use of the mrouted program represents acceptance of  * the terms and conditions listed in that file.  *  * The mrouted program is COPYRIGHT 1989 by The Board of Trustees of  * Leland Stanford Junior University.  *  *  * $Id: route.c,v 1.8 1994/08/24 23:54:42 thyagara Exp $  */
end_comment
...
begin_comment
comment|/*  * Search the entire routing table, looking for an entry which conflicts  * with the given origin and mask, for example, an entry which has the same  * origin under a different mask.  If a conflicting entry is found, return  * a pointer to the entry preceding it (to facilitate deletion); if no  * conflict is found, return NULL.  */
end_comment
...
return|return;
block|}
comment|/* 	 * If the new origin and mask are inconsistent with an entry 	 * already in the routing table, either ignore this update 	 * (if it came from another router), or delete the conflicting 	 * entry (if the update is for a directly-connected subnet). 	 */
if|if
condition|(
operator|(
name|prev_r
...
literal|0
argument_list|,
literal|"%s reports a conflicting origin (%s) and mask (%08x)"
argument_list|,
name|inet_fmt
argument_list|(
name|src
...
literal|0
argument_list|,
literal|"deleting route with conflicting origin (%s), mask (%08x)"
argument_list|,
name|inet_fmt
argument_list|(
name|r
...
begin_comment
comment|/*  * send the next chunk of our routing table to all neighbors.  */
end_comment
...
literal|"entries"
argument_list|,
literal|" Origin-Subnet   From-Gateway    Metric  In-Vif  Out-Vifs\n"
argument_list|)
expr_stmt|;
for|for
control|(
