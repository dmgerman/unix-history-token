...
block|;
comment|// A predicate which gets signalled when an exception
comment|// is finished processing and the debug loop can be
comment|// continued.
name|HANDLE
name|m_debugging_ended_event
block|;
comment|// An event which gets signalled by the debugger thread when it
comment|// exits the debugger loop and is detached from the inferior.
name|std
operator|::
name|atomic
operator|<
...
block|;
comment|// Signals the debug loop to stop processing certain types of
comment|// events that block shutdown.
name|bool
name|m_detached
block|;
comment|// Indicates we've detached from the inferior process and the debug loop can exit.
specifier|static
name|lldb
operator|::
