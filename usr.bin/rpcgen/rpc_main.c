...
argument_list|)
expr_stmt|;
comment|/* convert to a valid C macro name, and upper case 	 * =~ m,[A-Za-z_][A-Za-z_0-9]*,   else map other chars to '_'. 	 */
for|for
control|(
name|tmp
operator|=
...
expr_stmt|;
block|}
comment|/* When the filename started with "." (wow, the is Poor Form) 	 * lets put in the word "DOT" so we don't violate ANSI's reservation 	 * of macros that start with "_" -- rpc at ksb.npcguild.org 	 */
if|if
condition|(
literal|'\000'
operator|==
