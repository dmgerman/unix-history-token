...
name|argv
argument_list|,
literal|"0123456789cd:hl:mpst:w:"
argument_list|)
operator|)
operator|!=
operator|-
...
name|optarg
argument_list|,
literal|"output tab width must be positive"
argument_list|,
literal|1
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* We need a new paragraph if and only if:        *   this line is blank,        *   OR it's a mail header,        *   OR it's not a mail header AND the last line was one,        *   OR the indentation has changed        *      AND the line isn't a mail header continuation line        *      AND this isn't the second line of an indented paragraph.        */
if|if
condition|(
name|length
operator|==
