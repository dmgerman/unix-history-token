begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * file - find type of a file or files - main program.  *  * Copyright (c) Ian F. Darwin, 1987.  * Written by Ian F. Darwin.  *  * This software is not subject to any license of the American Telephone  * and Telegraph Company or of the Regents of the University of California.  *  * Permission is granted to anyone to use this software for any purpose on  * any computer system, and to alter it and redistribute it freely, subject  * to the following restrictions:  *  * 1. The author is not responsible for the consequences of use of this  *    software, no matter how awful, even if they arise from flaws in it.  *  * 2. The origin of this software must not be misrepresented, either by  *    explicit claim or by omission.  Since few users ever read sources,  *    credits must appear in the documentation.  *  * 3. Altered versions must be plainly marked as such, and must not be  *    misrepresented as being the original software.  Since few users  *    ever read sources, credits must appear in the documentation.  *  * 4. This notice may not be removed or altered.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
modifier|*
name|moduleid
init|=
literal|"@(#)$Id: file.c,v 1.34 1995/10/27 23:12:01 christos Exp $"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* lint */
end_comment

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_comment
comment|/* for MAXPATHLEN */
end_comment

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_comment
comment|/* for open() */
end_comment

begin_if
if|#
directive|if
operator|(
name|__COHERENT__
operator|>=
literal|0x420
operator|)
end_if

begin_include
include|#
directive|include
file|<sys/utime.h>
end_include

begin_else
else|#
directive|else
end_else

begin_include
include|#
directive|include
file|<utime.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_comment
comment|/* for read() */
end_comment

begin_ifdef
ifdef|#
directive|ifdef
name|__ELF__
end_ifdef

begin_include
include|#
directive|include
file|<elf.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"patchlevel.h"
end_include

begin_include
include|#
directive|include
file|"file.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|S_IFLNK
end_ifdef

begin_define
define|#
directive|define
name|USAGE
value|"Usage: %s [-vczL] [-f namefile] [-m magicfiles] file...\n"
end_define

begin_else
else|#
directive|else
end_else

begin_define
define|#
directive|define
name|USAGE
value|"Usage: %s [-vcz] [-f namefile] [-m magicfiles] file...\n"
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_ifndef
ifndef|#
directive|ifndef
name|MAGIC
end_ifndef

begin_define
define|#
directive|define
name|MAGIC
value|"/etc/magic"
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
name|int
comment|/* Global command-line options 		*/
name|debug
init|=
literal|0
decl_stmt|,
comment|/* debugging 				*/
name|lflag
init|=
literal|0
decl_stmt|,
comment|/* follow Symlinks (BSD only) 		*/
name|zflag
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* follow (uncompress) compressed files */
end_comment

begin_decl_stmt
name|int
comment|/* Misc globals				*/
name|nmagic
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* number of valid magic[]s 		*/
end_comment

begin_decl_stmt
name|struct
name|magic
modifier|*
name|magic
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* array of magic entries		*/
end_comment

begin_decl_stmt
name|char
modifier|*
name|magicfile
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* where magic be found 		*/
end_comment

begin_decl_stmt
name|char
modifier|*
name|progname
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* used throughout 			*/
end_comment

begin_decl_stmt
name|int
name|lineno
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* line number in the magic file	*/
end_comment

begin_decl_stmt
specifier|static
name|void
name|unwrap
name|__P
argument_list|(
operator|(
name|char
operator|*
name|fn
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * main - parse arguments and handle options  */
end_comment

begin_function
name|int
name|main
parameter_list|(
name|argc
parameter_list|,
name|argv
parameter_list|)
name|int
name|argc
decl_stmt|;
name|char
modifier|*
name|argv
index|[]
decl_stmt|;
block|{
name|int
name|c
decl_stmt|;
name|int
name|check
init|=
literal|0
decl_stmt|,
name|didsomefiles
init|=
literal|0
decl_stmt|,
name|errflg
init|=
literal|0
decl_stmt|,
name|ret
init|=
literal|0
decl_stmt|,
name|app
init|=
literal|0
decl_stmt|;
if|if
condition|(
operator|(
name|progname
operator|=
name|strrchr
argument_list|(
name|argv
index|[
literal|0
index|]
argument_list|,
literal|'/'
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
name|progname
operator|++
expr_stmt|;
else|else
name|progname
operator|=
name|argv
index|[
literal|0
index|]
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|magicfile
operator|=
name|getenv
argument_list|(
literal|"MAGIC"
argument_list|)
operator|)
condition|)
name|magicfile
operator|=
name|MAGIC
expr_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"vcdf:Lm:z"
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'v'
case|:
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stdout
argument_list|,
literal|"%s-%d.%d\n"
argument_list|,
name|progname
argument_list|,
name|FILE_VERSION_MAJOR
argument_list|,
name|patchlevel
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
case|case
literal|'c'
case|:
operator|++
name|check
expr_stmt|;
break|break;
case|case
literal|'d'
case|:
operator|++
name|debug
expr_stmt|;
break|break;
case|case
literal|'f'
case|:
if|if
condition|(
operator|!
name|app
condition|)
block|{
name|ret
operator|=
name|apprentice
argument_list|(
name|magicfile
argument_list|,
name|check
argument_list|)
expr_stmt|;
if|if
condition|(
name|check
condition|)
name|exit
argument_list|(
name|ret
argument_list|)
expr_stmt|;
name|app
operator|=
literal|1
expr_stmt|;
block|}
name|unwrap
argument_list|(
name|optarg
argument_list|)
expr_stmt|;
operator|++
name|didsomefiles
expr_stmt|;
break|break;
ifdef|#
directive|ifdef
name|S_IFLNK
case|case
literal|'L'
case|:
operator|++
name|lflag
expr_stmt|;
break|break;
endif|#
directive|endif
case|case
literal|'m'
case|:
name|magicfile
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'z'
case|:
name|zflag
operator|++
expr_stmt|;
break|break;
case|case
literal|'?'
case|:
default|default:
name|errflg
operator|++
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|errflg
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
name|USAGE
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|2
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|app
condition|)
block|{
name|ret
operator|=
name|apprentice
argument_list|(
name|magicfile
argument_list|,
name|check
argument_list|)
expr_stmt|;
if|if
condition|(
name|check
condition|)
name|exit
argument_list|(
name|ret
argument_list|)
expr_stmt|;
name|app
operator|=
literal|1
expr_stmt|;
block|}
if|if
condition|(
name|optind
operator|==
name|argc
condition|)
block|{
if|if
condition|(
operator|!
name|didsomefiles
condition|)
block|{
operator|(
name|void
operator|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
name|USAGE
argument_list|,
name|progname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|2
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|int
name|i
decl_stmt|,
name|wid
decl_stmt|,
name|nw
decl_stmt|;
for|for
control|(
name|wid
operator|=
literal|0
operator|,
name|i
operator|=
name|optind
init|;
name|i
operator|<
name|argc
condition|;
name|i
operator|++
control|)
block|{
name|nw
operator|=
name|strlen
argument_list|(
name|argv
index|[
name|i
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|nw
operator|>
name|wid
condition|)
name|wid
operator|=
name|nw
expr_stmt|;
block|}
for|for
control|(
init|;
name|optind
operator|<
name|argc
condition|;
name|optind
operator|++
control|)
name|process
argument_list|(
name|argv
index|[
name|optind
index|]
argument_list|,
name|wid
argument_list|)
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_comment
comment|/*  * unwrap -- read a file of filenames, do each one.  */
end_comment

begin_function
specifier|static
name|void
name|unwrap
parameter_list|(
name|fn
parameter_list|)
name|char
modifier|*
name|fn
decl_stmt|;
block|{
name|char
name|buf
index|[
name|MAXPATHLEN
index|]
decl_stmt|;
name|FILE
modifier|*
name|f
decl_stmt|;
name|int
name|wid
init|=
literal|0
decl_stmt|,
name|cwid
decl_stmt|;
if|if
condition|(
operator|(
name|f
operator|=
name|fopen
argument_list|(
name|fn
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|error
argument_list|(
literal|"Cannot open `%s' (%s).\n"
argument_list|,
name|fn
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/*NOTREACHED*/
block|}
while|while
condition|(
name|fgets
argument_list|(
name|buf
argument_list|,
name|MAXPATHLEN
argument_list|,
name|f
argument_list|)
operator|!=
name|NULL
condition|)
block|{
name|cwid
operator|=
name|strlen
argument_list|(
name|buf
argument_list|)
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|cwid
operator|>
name|wid
condition|)
name|wid
operator|=
name|cwid
expr_stmt|;
block|}
name|rewind
argument_list|(
name|f
argument_list|)
expr_stmt|;
while|while
condition|(
name|fgets
argument_list|(
name|buf
argument_list|,
name|MAXPATHLEN
argument_list|,
name|f
argument_list|)
operator|!=
name|NULL
condition|)
block|{
name|buf
index|[
name|strlen
argument_list|(
name|buf
argument_list|)
operator|-
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
name|process
argument_list|(
name|buf
argument_list|,
name|wid
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * process - process input file  */
end_comment

begin_function
name|void
name|process
parameter_list|(
name|inname
parameter_list|,
name|wid
parameter_list|)
specifier|const
name|char
modifier|*
name|inname
decl_stmt|;
name|int
name|wid
decl_stmt|;
block|{
name|int
name|fd
init|=
literal|0
decl_stmt|;
specifier|static
specifier|const
name|char
name|stdname
index|[]
init|=
literal|"standard input"
decl_stmt|;
name|unsigned
name|char
name|buf
index|[
name|HOWMANY
operator|+
literal|1
index|]
decl_stmt|;
comment|/* one extra for terminating '\0' */
name|struct
name|utimbuf
name|utbuf
decl_stmt|;
name|struct
name|stat
name|sb
decl_stmt|;
name|int
name|nbytes
init|=
literal|0
decl_stmt|;
comment|/* number of bytes read from a datafile */
name|char
name|match
init|=
literal|'\0'
decl_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
literal|"-"
argument_list|,
name|inname
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|fstat
argument_list|(
literal|0
argument_list|,
operator|&
name|sb
argument_list|)
operator|<
literal|0
condition|)
block|{
name|error
argument_list|(
literal|"cannot fstat `%s' (%s).\n"
argument_list|,
name|stdname
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/*NOTREACHED*/
block|}
name|inname
operator|=
name|stdname
expr_stmt|;
block|}
if|if
condition|(
name|wid
operator|>
literal|0
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|"%s:%*s "
argument_list|,
name|inname
argument_list|,
call|(
name|int
call|)
argument_list|(
name|wid
operator|-
name|strlen
argument_list|(
name|inname
argument_list|)
argument_list|)
argument_list|,
literal|""
argument_list|)
expr_stmt|;
if|if
condition|(
name|inname
operator|!=
name|stdname
condition|)
block|{
comment|/* 	     * first try judging the file based on its filesystem status 	     */
if|if
condition|(
name|fsmagic
argument_list|(
name|inname
argument_list|,
operator|&
name|sb
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|putchar
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
operator|(
name|fd
operator|=
name|open
argument_list|(
name|inname
argument_list|,
name|O_RDONLY
argument_list|)
operator|)
operator|<
literal|0
condition|)
block|{
comment|/* We can't open it, but we were able to stat it. */
if|if
condition|(
name|sb
operator|.
name|st_mode
operator|&
literal|0002
condition|)
name|ckfputs
argument_list|(
literal|"writeable, "
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
if|if
condition|(
name|sb
operator|.
name|st_mode
operator|&
literal|0111
condition|)
name|ckfputs
argument_list|(
literal|"executable, "
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
name|ckfprintf
argument_list|(
name|stdout
argument_list|,
literal|"can't read `%s' (%s).\n"
argument_list|,
name|inname
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
comment|/* 	 * try looking at the first HOWMANY bytes 	 */
if|if
condition|(
operator|(
name|nbytes
operator|=
name|read
argument_list|(
name|fd
argument_list|,
operator|(
name|char
operator|*
operator|)
name|buf
argument_list|,
name|HOWMANY
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
block|{
name|error
argument_list|(
literal|"read failed (%s).\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/*NOTREACHED*/
block|}
if|if
condition|(
name|nbytes
operator|==
literal|0
condition|)
name|ckfputs
argument_list|(
literal|"empty"
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
else|else
block|{
name|buf
index|[
name|nbytes
operator|++
index|]
operator|=
literal|'\0'
expr_stmt|;
comment|/* null-terminate it */
name|match
operator|=
name|tryit
argument_list|(
name|buf
argument_list|,
name|nbytes
argument_list|,
name|zflag
argument_list|)
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|__ELF__
comment|/* 	 * ELF executables have multiple section headers in arbitrary 	 * file locations and thus file(1) cannot determine it from easily. 	 * Instead we traverse thru all section headers until a symbol table 	 * one is found or else the binary is stripped. 	 * XXX: This will not work for binaries of a different byteorder. 	 *	Should come up with a better fix. 	 */
if|if
condition|(
name|match
operator|==
literal|'s'
operator|&&
name|nbytes
operator|>
sizeof|sizeof
argument_list|(
name|Elf32_Ehdr
argument_list|)
operator|&&
name|buf
index|[
name|EI_MAG0
index|]
operator|==
name|ELFMAG0
operator|&&
name|buf
index|[
name|EI_MAG1
index|]
operator|==
name|ELFMAG1
operator|&&
name|buf
index|[
name|EI_MAG2
index|]
operator|==
name|ELFMAG2
operator|&&
name|buf
index|[
name|EI_MAG3
index|]
operator|==
name|ELFMAG3
condition|)
block|{
union|union
block|{
name|long
name|l
decl_stmt|;
name|char
name|c
index|[
sizeof|sizeof
argument_list|(
name|long
argument_list|)
index|]
decl_stmt|;
block|}
name|u
union|;
name|Elf32_Ehdr
name|elfhdr
decl_stmt|;
name|int
name|stripped
init|=
literal|1
decl_stmt|;
name|u
operator|.
name|l
operator|=
literal|1
expr_stmt|;
operator|(
name|void
operator|)
name|memcpy
argument_list|(
operator|&
name|elfhdr
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
name|elfhdr
argument_list|)
expr_stmt|;
comment|/* 		 * If the system byteorder does not equal the object byteorder 		 * then don't test. 		 */
if|if
condition|(
operator|(
name|u
operator|.
name|c
index|[
sizeof|sizeof
argument_list|(
name|long
argument_list|)
operator|-
literal|1
index|]
operator|+
literal|1
operator|)
operator|==
name|elfhdr
operator|.
name|e_ident
index|[
literal|5
index|]
condition|)
block|{
if|if
condition|(
name|lseek
argument_list|(
name|fd
argument_list|,
name|elfhdr
operator|.
name|e_shoff
argument_list|,
name|SEEK_SET
argument_list|)
operator|<
literal|0
condition|)
name|error
argument_list|(
literal|"lseek failed (%s).\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
init|;
name|elfhdr
operator|.
name|e_shnum
condition|;
name|elfhdr
operator|.
name|e_shnum
operator|--
control|)
block|{
if|if
condition|(
name|read
argument_list|(
name|fd
argument_list|,
name|buf
argument_list|,
name|elfhdr
operator|.
name|e_shentsize
argument_list|)
operator|<
literal|0
condition|)
name|error
argument_list|(
literal|"read failed (%s).\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
operator|(
name|Elf32_Shdr
operator|*
operator|)
operator|&
name|buf
operator|)
operator|->
name|sh_type
operator|==
name|SHT_SYMTAB
condition|)
block|{
name|stripped
operator|=
literal|0
expr_stmt|;
break|break;
block|}
block|}
if|if
condition|(
name|stripped
condition|)
operator|(
name|void
operator|)
name|printf
argument_list|(
literal|", stripped"
argument_list|)
expr_stmt|;
block|}
block|}
endif|#
directive|endif
if|if
condition|(
name|inname
operator|!=
name|stdname
condition|)
block|{
comment|/* 		 * Try to restore access, modification times if read it. 		 */
name|utbuf
operator|.
name|actime
operator|=
name|sb
operator|.
name|st_atime
expr_stmt|;
name|utbuf
operator|.
name|modtime
operator|=
name|sb
operator|.
name|st_mtime
expr_stmt|;
operator|(
name|void
operator|)
name|utime
argument_list|(
name|inname
argument_list|,
operator|&
name|utbuf
argument_list|)
expr_stmt|;
comment|/* don't care if loses */
operator|(
name|void
operator|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
block|}
operator|(
name|void
operator|)
name|putchar
argument_list|(
literal|'\n'
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
name|int
name|tryit
parameter_list|(
name|buf
parameter_list|,
name|nb
parameter_list|,
name|zflag
parameter_list|)
name|unsigned
name|char
modifier|*
name|buf
decl_stmt|;
name|int
name|nb
decl_stmt|,
name|zflag
decl_stmt|;
block|{
comment|/* try compression stuff */
if|if
condition|(
name|zflag
operator|&&
name|zmagic
argument_list|(
name|buf
argument_list|,
name|nb
argument_list|)
condition|)
return|return
literal|'z'
return|;
comment|/* try tests in /etc/magic (or surrogate magic file) */
if|if
condition|(
name|softmagic
argument_list|(
name|buf
argument_list|,
name|nb
argument_list|)
condition|)
return|return
literal|'s'
return|;
comment|/* try known keywords, check whether it is ASCII */
if|if
condition|(
name|ascmagic
argument_list|(
name|buf
argument_list|,
name|nb
argument_list|)
condition|)
return|return
literal|'a'
return|;
comment|/* abandon hope, all ye who remain here */
name|ckfputs
argument_list|(
literal|"data"
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
return|return
literal|'\0'
return|;
block|}
end_function

end_unit

