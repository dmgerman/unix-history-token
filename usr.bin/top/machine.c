...
begin_comment
comment|/* data used for recalculating pctcpu */
end_comment
...
begin_comment
comment|/* Adjust display based on ncpus and the ARC state. */
end_comment
...
name|sysctlbyname
argument_list|(
literal|"kstat.zfs.misc.arcstats.size"
argument_list|,
operator|&
name|arc_size
argument_list|,
...
name|GETSYSCTL
argument_list|(
literal|"kstat.zfs.misc.arcstats.hdr_size"
argument_list|,
name|arc_stat
argument_list|)
expr_stmt|;
name|GETSYSCTL
argument_list|(
literal|"kstat.zfs.misc.arcstats.l2_hdr_size"
argument_list|,
name|arc_stat2
argument_list|)
expr_stmt|;
...
name|GETSYSCTL
argument_list|(
literal|"kstat.zfs.misc.arcstats.other_size"
argument_list|,
name|arc_stat
argument_list|)
expr_stmt|;
...
name|GETSYSCTL
argument_list|(
literal|"kstat.zfs.misc.arcstats.compressed_size"
argument_list|,
name|arc_stat
argument_list|)
expr_stmt|;
...
begin_comment
comment|/*  * If there was a previous update, use the delta in ki_runtime over  * the previous interval to calculate pctcpu.  Otherwise, fall back  * to using the kernel's ki_pctcpu.  */
end_comment

begin_function
specifier|static
name|double
name|proc_calc_pctcpu
parameter_list|(
name|struct
name|kinfo_proc
modifier|*
...
operator|)
return|;
comment|/* 		 * If this process/thread was created during the previous 		 * interval, charge it's total runtime to the previous 		 * interval. 		 */
elseif|else
if|if
condition|(
name|pp
...
begin_comment
comment|/*  * Return true if this process has used any CPU time since the  * previous update.  */
end_comment
...
name|show_kidle
decl_stmt|;
comment|/* 	 * If thread state was toggled, don't cache the previous processes. 	 */
if|if
condition|(
name|previous_thread
operator|!=
...
name|b
parameter_list|)
value|do { \ 	double diff; \ 	if (ps.wcpu) \ 		diff = weighted_cpu(PCTCPU((b)), (b)) - \ 		    weighted_cpu(PCTCPU((a)), (a)); \ 	else \ 		diff = PCTCPU((b)) - PCTCPU((a)); \ 	if (diff != 0) \ 		return (diff> 0 ? 1 : -1); \ } while (0)
end_define
...
name|GETSYSCTL
argument_list|(
literal|"vm.swap_maxpages"
argument_list|,
name|swap_maxpages
argument_list|)
expr_stmt|;
comment|/* ksw_total contains the total size of swap all devices which may 	   exceed the maximum swap size allocatable in the system */
if|if
condition|(
name|swapary
index|[
