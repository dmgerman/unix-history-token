...
operator|)
expr_stmt|;
comment|/* only the first 3 pseudo ops (word, file, loc) are in IQ10 */
break|break;
default|default:
return|return
literal|0
...
name|_
argument_list|(
literal|"IQ2000 specific command line options:\n"
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
...
name|_
argument_list|(
literal|"-m2000<default>           IQ2000 processor\n"
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
...
parameter_list|()
block|{
comment|/* Allow all iq2k macros in iq10, instead of just LA. */
name|iq2000_load_macros
argument_list|()
expr_stmt|;
if|#
directive|if
literal|0
block|char *la_sem = "\n lui \\reg,%hi(\\label)\n ori \\reg,\\reg,%lo(\\label)\n";    char *la_arg_1 = "reg";   char *la_arg_2 = "label";   const char *la_args[3] = { la_arg_1, la_arg_2, NULL };    iq2000_add_macro ("la", la_sem, la_args);
endif|#
directive|endif
block|}
end_function
...
argument_list|)
expr_stmt|;
comment|/* Warn about odd numbered base registers for paired-register      instructions like LDW.  On iq10, result is always rd. */
if|if
condition|(
name|iq2000_mach
operator|==
...
argument_list|)
expr_stmt|;
comment|/* The assumption here is that the target of a load is always rt.      That is true for iq2000& iq10. */
name|delayed_load_register
operator|=
name|insn
operator|.
...
expr_stmt|;
return|return;
comment|/*as_warn (_("Tried to set unrecognized symbol: %s\n"), name);*/
block|}
operator|*
name|input_line_pointer
operator|=
...
directive|endif
comment|/*   iq2000_emit_delays (false); */
switch|switch
condition|(
name|sec
condition|)
...
condition|(
literal|1
comment|/*iq2000_mach == bfd_mach_iq2000*/
condition|)
block|{
ifdef|#
directive|ifdef
