...
decl_stmt|;
name|int
name|linker_stubs_should_be_generated
init|=
literal|0
decl_stmt|;
name|tmp
...
name|_
argument_list|(
literal|"expression dangerous with linker stubs"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
begin_comment
comment|/* Parse special CONS expression: pm (expression)    or alternatively: gs (expression).    These are used for addressing program memory.    Relocation: BFD_RELOC_AVR_16_PM.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* len must be the same for both pm identifiers.  */
if|if
condition|(
name|strncasecmp
argument_list|(
