...
name|FIXP
parameter_list|)
value|crx_force_relocation (FIXP)
end_define
...
begin_comment
comment|/* CRX instructions, with operands included, are a multiple    of two bytes long.  */
end_comment
...
begin_comment
comment|/* This is called by emit_expr when creating a reloc for a cons.    We could use the definition there, except that we want to handle     the CRX reloc type specially, rather than the BFD_RELOC type.  */
end_comment
...
parameter_list|)
define|\
value|fix_new_exp (FRAG, OFF, (int) LEN, EXP, 0, \ 	LEN == 1 ? BFD_RELOC_CRX_NUM8 \ 	: LEN == 2 ? BFD_RELOC_CRX_NUM16 \ 	: LEN == 4 ? BFD_RELOC_CRX_NUM32 \ 	: BFD_RELOC_NONE);
end_define
