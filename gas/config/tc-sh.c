...
begin_comment
comment|/* tc-sh.c -- Assemble code for the Renesas / SuperH SH    Copyright 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002,    2003, 2004, 2005, 2006  Free Software Foundation, Inc.     This file is part of GAS, the GNU Assembler.     GAS is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2, or (at your option)    any later version.     GAS is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with GAS; see the file COPYING.  If not, write to    the Free Software Foundation, 51 Franklin Street - Fifth Floor,    Boston, MA 02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* Flag to generate relocations against symbol values for local symbols.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|dont_adjust_reloc_32
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag to indicate that '$' is allowed as a register prefix.  */
end_comment

begin_decl_stmt
specifier|static
name|int
name|allow_dollar_register_prefix
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Preset architecture set, if given; zero otherwise.  */
end_comment
...
begin_comment
comment|/* The bit mask of architectures that could    accommodate the insns seen so far.  */
end_comment
...
begin_comment
comment|/* These are the three types of relaxable instruction.  */
end_comment
...
begin_comment
comment|/* NBYTES 1=.byte, 2=.word, 4=.long */
end_comment
...
begin_comment
comment|/* The regular frag_offset_fixed_p doesn't work for rs_align_test    frags.  */
end_comment

begin_function
specifier|static
name|bfd_boolean
name|align_test_frag_offset_fixed_p
parameter_list|(
specifier|const
name|fragS
modifier|*
...
name|off
decl_stmt|;
comment|/* Start with offset initialised to difference between the two frags.      Prior to assigning frag addresses this will be zero.  */
name|off
operator|=
name|frag1
operator|->
...
return|;
block|}
comment|/* Maybe frag2 is after frag1.  */
name|frag
operator|=
name|frag1
expr_stmt|;
...
block|}
block|}
comment|/* Maybe frag1 is after frag2.  */
name|off
operator|=
name|frag1
operator|->
...
begin_comment
comment|/* Optimize a difference of symbols which have rs_align_test frag if    possible.  */
end_comment
...
begin_comment
comment|/* Like parse_reg_without_prefix, but this version supports    $-prefixed register names if enabled by the user.  */
end_comment
...
name|_
argument_list|(
literal|"illegal double indirection"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* Check a few key IMM* fields for overflow.  */
name|int
name|opf
decl_stmt|;
name|long
...
name|_
argument_list|(
literal|"insn cannot be combined with non-nopy"
argument_list|)
argument_list|)
expr_stmt|;
name|movx
...
name|_
argument_list|(
literal|"insn cannot be combined with non-nopx"
argument_list|)
argument_list|)
expr_stmt|;
name|movy
...
name|_
argument_list|(
literal|"previous movy requires nopx"
argument_list|)
argument_list|)
expr_stmt|;
if|if
...
name|_
argument_list|(
literal|"previous movx requires nopy"
argument_list|)
argument_list|)
expr_stmt|;
if|if
...
literal|0x8100
expr_stmt|;
comment|/* pclr Dz pmuls Se,Sf,Dg */
elseif|else
if|if
condition|(
operator|(
...
name|_
argument_list|(
literal|"bad combined pmuls output operand"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* The opcode is not in the hash table. 	 This means we definitely have an assembly failure, 	 but the instruction may be valid in another CPU variant. 	 In this case emit something better than 'unknown opcode'. 	 Search the full table in sh-opc.h to check. */
name|char
modifier|*
name|name
init|=
...
literal|0
decl_stmt|;
comment|/* identify opcode in string */
while|while
condition|(
name|ISSPACE
argument_list|(
...
expr_stmt|;
block|}
comment|/* search for opcode in full list */
for|for
control|(
name|op
operator|=
...
name|_
argument_list|(
literal|"opcode not valid for this cpu variant"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* Since we skip get_specific here, we have to check& update 	     valid_arch now.  */
if|if
condition|(
name|SH_MERGE_ARCH_SET_VALID
argument_list|(
...
name|_
argument_list|(
literal|"Delayed branches not available on SH1"
argument_list|)
argument_list|)
expr_stmt|;
name|parse_exp
...
argument_list|)
expr_stmt|;
comment|/* All branches are currently 16 bit.  */
name|size
operator|=
literal|2
expr_stmt|;
...
directive|endif
name|OPTION_DUMMY
comment|/* Not used.  This is just here to make it easy to add and subtract options from this enum.  */
block|}
enum|;
end_enum
...
block|}
block|,
comment|/* The next two switches are here because the      generic parts of the linker testsuite uses them.  */
block|{
literal|"EB"
block|,
name|no_argument
...
name|_
argument_list|(
literal|"\ --isa=[shmedia		set as the default instruction set for SH64\n\     | SHmedia\n\     | shcompact\n\     | SHcompact]\n"
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
...
name|_
argument_list|(
literal|"\ --abi=[32|64]		set size of expanded SHmedia operands and object\n\ 			file type\n\ --shcompact-const-crange  emit code-range descriptors for constants in\n\ 			SHcompact code sections\n\ --no-mix		disallow SHmedia code in the same section as\n\ 			constants and SHcompact code\n\ --no-expand		do not expand MOVI, PT, PTA or PTB instructions\n\ --expand-pt32		with -abi=64, expand PT, PTA and PTB instructions\n\ 			to 32 bits only\n"
argument_list|)
argument_list|)
expr_stmt|;
endif|#
...
begin_comment
comment|/* Count the number of fixups in a section which refer to a particular    symbol.  This is called via bfd_map_over_sections.  */
end_comment
...
begin_comment
comment|/* Handle the count relocs for a particular section.    This is called via bfd_map_over_sections.  */
end_comment
...
name|fx_addsy
expr_stmt|;
comment|/* Check for a local_symbol.  */
if|if
condition|(
name|sym
operator|&&
...
name|sym
decl_stmt|;
comment|/* See if it's been converted.  If so, canonicalize.  */
if|if
condition|(
...
begin_comment
comment|/* This static variable is set by s_uacons to tell sh_cons_align that    the expression does not need to be aligned.  */
end_comment
...
begin_comment
comment|/* See whether the relocation should be resolved locally.  */
end_comment
...
parameter_list|)
block|{
comment|/* These relocations can't make it into a DSO, so no use forcing      them for global symbols.  */
if|if
condition|(
name|sh_local_pcrel
argument_list|(
...
literal|0
return|;
comment|/* Make sure some relocations get emitted.  */
if|if
condition|(
name|fix
operator|->
...
name|val
decl_stmt|;
comment|/* Set file-specific flags to indicate if this code needs      a processor with the sh-dsp / sh2e ISA to execute.  */
ifdef|#
directive|ifdef
name|HAVE_SH64
...
specifier|extern
name|int
name|sh_symbian_find_elf_flags
argument_list|(
name|unsigned
name|int
argument_list|)
...
begin_comment
comment|/* Apply fixup FIXP to SIZE-byte field BUF given that VAL is its    assembly-time value.  If we're generating a reloc for FIXP,    see whether the addend should be stored in-place or whether    it should be in an ELF r_addend field.  */
end_comment

begin_function
specifier|static
name|void
name|apply_full_field_fix
parameter_list|(
name|fixS
modifier|*
name|fixP
...
name|BFD_RELOC_SH_USES
case|:
comment|/* Pass the value into sh_reloc().  */
name|fixP
operator|->
name|fx_addnumber
operator|=
...
literal|0
condition|)
comment|/* Stop the generic code from trying to overlow check the value as well.        It may not have the correct value anyway, as we do not store val back        into *valP.  */
name|fixP
operator|->
name|fx_no_overflow
operator|=
...
begin_comment
comment|/* This version is used in obj-coff.c eg. for the sh-hms target.  */
end_comment
