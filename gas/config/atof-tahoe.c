...
begin_comment
comment|/* Precision in chars.  */
end_comment

begin_define
define|#
directive|define
name|D_PRECISION_CHARS
value|(8)
end_define
...
begin_comment
comment|/* Shared between flonum_gen2tahoe and next_bits.  */
end_comment
...
comment|/* The 1st MAX_PRECISION are zeroed, the last contain flonum bits.  */
name|char
modifier|*
name|return_value
decl_stmt|;
...
begin_comment
comment|/* In: a flonum, a Tahoe floating point format.    Out: a Tahoe floating-point bit pattern.  */
end_comment
...
return|;
block|}
comment|/* All tahoe floating_point formats have: 	     Bit 15 is sign bit. 	     Bits 14:n are excess-whatever exponent. 	     Bits n-1:0 (if any) are most significant bits of fraction. 	     Bits 15:0 of the next word are the next most significant bits. 	     And so on for each other word.  	     So we need: number of bits of exponent, number of bits of 	     mantissa.  */
name|bits_left_in_littlenum
operator|=
name|LITTLENUM_NUMBER_OF_BITS
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* The atof_tahoe() builds up 16-bit numbers. 	 Since the assembler may not be running on 	 a different-endian machine, be very careful about 	 converting words to chars.  */
name|number_of_chars
operator|=
operator|(
name|kind_of_float
