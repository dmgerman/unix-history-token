...
begin_comment
comment|/* This file is tc-tahoe.c     Copyright 1987, 1988, 1989, 1990, 1991, 1992, 1995, 2000, 2001, 2002    Free Software Foundation, Inc.     This file is part of GAS, the GNU Assembler.     GAS is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2, or (at your option)    any later version.     GAS is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with GAS; see the file COPYING.  If not, write to the Free    Software Foundation, 59 Temple Place - Suite 330, Boston, MA    02111-1307, USA.  */
end_comment
...
name|top_access
decl_stmt|;
comment|/* Access type wanted for this operand 				   'b'branch ' 'no-instruction 'amrvw' */
name|char
name|top_width
decl_stmt|;
...
name|seg_of_operand
decl_stmt|;
comment|/* segment as returned by expression()*/
name|expressionS
name|exp_of_operand
decl_stmt|;
...
begin_comment
comment|/* The addressing modes for an operand. These numbers are the actual values    for certain modes, so be careful if you screw with them.  */
end_comment
...
begin_comment
comment|/* INDEXED_REG is decided by the existence or lack of a [reg].  */
end_comment
...
begin_comment
comment|/* Chars that mean this number is a floating point constant    as in 0f123.456    or    0d1.234E-12 (see exp chars above)    Note: The Tahoe port doesn't support floating point constants. This is          consistent with 'as' If it's needed, I can always add it later.  */
end_comment
...
block|,
comment|/* unused	    2,3 */
comment|/* Another type of reversible branch. But this only has a word      displacement.  */
block|{
literal|1
block|,
literal|1
...
block|,
comment|/* unused	    3,3 */
comment|/* These are the non reversible branches, all of which have a word      displacement. If I can't reach, branch over a byte branch, to a      jump that will reach. The jumped branch jumps over the reaching      branch, to continue with the flow of the program. It's like playing      leap frog.  */
block|{
literal|1
block|,
literal|1
...
name|_
argument_list|(
literal|"The -a option doesn't exist. (Despite what the man page says!"
argument_list|)
argument_list|)
expr_stmt|;
break|break;
...
literal|0
decl_stmt|;
comment|/* The width of the absolute displacement */
name|int
name|com_width
init|=
literal|0
...
comment|/*    * Using the ^ operator is ambiguous. If I have an absolute label    * called 'w' set to, say 2, and I have the expression 'w^1', do I get    * 1, forced to be in word displacement mode, or do I get the value of    * 'w' or'ed with 1 (3 in this case).    * The default is 'w' as an offset, so that's what I use.    * Stick with `, it does the same, and isn't ambig.    */
if|if
condition|(
operator|*
name|point
...
condition|)
block|{
comment|/* Statement has no syntax goofs yet: let's sniff the expression.  */
...
name|O_constant
case|:
comment|/* for SEG_ABSOLUTE, we shouldn't need to set X_op_symbol, 	     X_add_symbol to any particular value.  */
...
block|{
comment|/*        * We found a match! So let's pick up as many operands as the        * instruction wants, and even gripe if there are too many.        * We expect comma to separate each operand.        * We let instring track the text, while p tracks a part of the        * struct tot.        */
name|count
operator|=
literal|0
expr_stmt|;
...
operator|)
expr_stmt|;
comment|/* Do we know how big this operand is? */
name|dispsize
operator|=
name|operandP
operator|->
...
literal|0
expr_stmt|;
comment|/* Deal with the branch possibilities. (Note, this doesn't include 	 jumps.)*/
if|if
condition|(
name|operandP
operator|->
...
comment|/* These are "branches" what will always be branches around a jump 	     to the correct address in real life. 	     If to_seg is SEG_ABSOLUTE, just encode the branch in, 	     else let GAS fix the address.  */
switch|switch
condition|(
name|operandP
operator|->
...
condition|)
block|{
comment|/* The theory: 	       For SEG_ABSOLUTE, then mode is ABSOLUTE_ADDR, jump 	       to that address (not pc_rel). 	       For other segs, address is a long word PC rel jump.  */
case|case
name|TAHOE_WIDTH_CONDITIONAL_JUMP
case|:
comment|/* b<cond> */
...
block|{
comment|/* It's an integer, and I know it's size.  */
if|if
condition|(
operator|(
name|unsigned
...
comment|/* Is this a value out of this segment? 	     The first part of this conditional is a cludge to make gas 	     produce the same output as 'as' when there is a lable, in 	     the current segment, displacing a register. It's strange, 	     and no one in their right mind would do it, but it's easy 	     to cludge.  */
if|if
condition|(
operator|(
name|dispsize
