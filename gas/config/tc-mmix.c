...
begin_comment
comment|/* We'll very rarely have sections longer than LONG_MAX, but we'll make a    feeble attempt at getting 64-bit values.  */
end_comment
...
condition|)
block|{
comment|/* We must initialize data that's supposed to be "fixed up" to 	     avoid emitting garbage, because md_apply_fix won't do 	     anything for undefined symbols.  */
name|md_number_to_chars
argument_list|(
name|var_partp
argument_list|,
...
condition|)
block|{
comment|/* Resolve this reloc now, as md_apply_fix would have done (not 	     called if -linkrelax).  There is no point in keeping a reloc 	     to an absolute symbol.  No reloc that is subject to 	     relaxation must be to an absolute symbol; difference 	     involving symbols in a specific section must be signalled as 	     an error if the relaxing cannot be expressed; having a reloc 	     to the resolved (now absolute) value does not help.  */
name|md_number_to_chars
argument_list|(
name|buf
argument_list|,
...
block|}
comment|/* FALLTHROUGH.  */
comment|/* The others are supposed to be handled by md_apply_fix. 	 FIXME: ... which isn't called when -linkrelax.  Move over 	 md_apply_fix code here for everything reasonable.  */
name|badop
label|:
default|default:
name|as_bad_where
...
expr_stmt|;
block|}
comment|/* Back off to the last space before the opcode so we don't handle 	 the opcode as a label.  */
name|s
operator|--
expr_stmt|;
block|}
...
index|]
condition|)
comment|/* We avoid handling empty lines here.  */
return|return;
if|if
condition|(
...
name|s
expr_stmt|;
comment|/* If there is a label, skip over it.  */
while|while
condition|(
operator|*
name|s
...
operator|++
expr_stmt|;
comment|/* Find the start of the instruction or pseudo following the label,      if there is one.  */
for|for
control|(
name|insn
operator|=
...
comment|/* Empty */
empty_stmt|;
comment|/* Remove a trailing ":" off labels, as they'd otherwise be considered      part of the name.  But don't do this for local labels.  */
if|if
condition|(
name|s
operator|!=
...
operator|!=
name|NULL
comment|/* For a lone label on a line, we don't attach it to the next 	      instruction or MMIXAL-pseudo (getting its alignment).  Thus 	      is acts like a "normal" :-ended label.  Ditto if it's 	      followed by a non-MMIXAL pseudo.  */
operator|&&
operator|!
name|is_end_of_line
index|[
...
expr_stmt|;
block|}
comment|/* If we have a non-MMIXAL pseudo, we have not business with the rest of      the line.  */
if|if
condition|(
operator|*
name|insn
...
index|]
argument_list|)
comment|/* Don't treat e.g. #1F as a local-label reference.  */
operator|||
operator|(
name|s
operator|!=
...
literal|1
return|;
comment|/* All our pcrel relocations are must-keep.  Note that md_apply_fix is      called *after* this, and will handle getting rid of the presumed      reloc; a relocation isn't *forced* other than to be handled by      md_apply_fix (or tc_gen_reloc if linkrelax).  */
if|if
condition|(
name|fixP
operator|->
