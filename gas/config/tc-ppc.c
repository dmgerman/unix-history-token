...
begin_comment
comment|/* tc-ppc.c -- Assemble for the PowerPC or POWER (RS/6000)    Copyright 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003,    2004, 2005 Free Software Foundation, Inc.    Written by Ian Lance Taylor, Cygnus Support.     This file is part of GAS, the GNU Assembler.     GAS is free software; you can redistribute it and/or modify    it under the terms of the GNU General Public License as published by    the Free Software Foundation; either version 2, or (at your option)    any later version.     GAS is distributed in the hope that it will be useful,    but WITHOUT ANY WARRANTY; without even the implied warranty of    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the    GNU General Public License for more details.     You should have received a copy of the GNU General Public License    along with GAS; see the file COPYING.  If not, write to the Free    Software Foundation, 51 Franklin Street - Fifth Floor, Boston, MA    02110-1301, USA.  */
end_comment
...
begin_comment
comment|/* '+' and '-' can be used as postfix predicate predictors for conditional    branches.  So they need to be accepted as symbol characters.    Also, anything that can start an operand needs to be mentioned here,    to stop the input scrubber eating whitespace.  */
end_comment
...
begin_comment
comment|/* Whether to target xcoff64/elf64.  */
end_comment
...
define|#
directive|define
name|PPC_APUINFO_BRLOCK
value|0x102
end_define

begin_comment
comment|/*  * We keep a list of APUinfo  */
end_comment
...
begin_comment
comment|/* Handle -m options that set cpu type, and .machine arg.  */
end_comment
...
begin_comment
comment|/* Figure out the BFD architecture to use.  This function and ppc_mach    are called well before md_begin, when the output file is opened.  */
end_comment
...
begin_comment
comment|/* Insert opcodes and macros into hash tables.  Called at startup and    for .cpu pseudo.  */
end_comment

begin_function
specifier|static
name|void
name|ppc_setup_opcodes
parameter_list|(
name|void
parameter_list|)
block|{
...
literal|0
operator|)
comment|/* Certain instructions (eg: extsw) do not exist in the 	     32-bit BookE instruction set, but they do exist in the 	     64-bit BookE instruction set, and other PPC instruction 	     sets.  Check to see if the opcode has the BOOKE64 flag set. 	     If it does make sure that the target CPU is not the BookE32.  */
operator|&&
operator|(
operator|(
name|op
...
argument_list|()
expr_stmt|;
comment|/* Tell the main code what the endianness is if it is not overridden      by the user.  */
if|if
condition|(
operator|!
name|set_target_endian
...
condition|)
return|return;
comment|/* Ok, so write the section info out.  We have this layout:    byte	data		what   ----	----		----   0	8		length of "APUinfo\0"   4	(n*4)		number of APU's (4 bytes each)   8	2		note type 2   12	"APUinfo\0"	name   20	APU#1		first APU's info   24	APU#2		second APU's info   ...	...   */
block|{
name|char
modifier|*
name|p
...
name|i
decl_stmt|;
comment|/* Create the .PPC.EMB.apuinfo section.  */
name|apuinfo_secp
operator|=
name|subseg_new
argument_list|(
...
name|reloc
parameter_list|)
value|{ str, sizeof (str) - 1, 1, 1, reloc }
define|#
directive|define
name|MAP32
parameter_list|(
...
name|reloc
parameter_list|)
value|{ str, sizeof (str) - 1, 1, 0, reloc }
define|#
directive|define
name|MAP64
parameter_list|(
...
name|reloc
parameter_list|)
value|{ str, sizeof (str) - 1, 0, 1, reloc }
specifier|static
specifier|const
name|struct
name|map_bfd
...
condition|)
block|{
comment|/* Change the symbol so that the dummy .TOC. symbol can be 	       omitted from the object file.  */
name|exp_p
operator|->
name|X_add_symbol
operator|=
...
begin_comment
comment|/* Prevent elf_frob_file_before_adjust removing a weak undefined    function descriptor sym if the corresponding code sym is used.  */
end_comment
...
name|_
argument_list|(
literal|"TOC section size exceeds 64k"
argument_list|)
argument_list|)
expr_stmt|;
...
name|i
decl_stmt|;
comment|/* Check we don't already exist.  */
for|for
control|(
name|i
operator|=
...
block|}
block|}
comment|/* Compute the number of expected operands. 	     Do not count fake operands.  */
for|for
control|(
name|num_operands_expected
operator|=
...
name|num_operands_expected
expr_stmt|;
comment|/* If there are fewer operands in the line then are called 	     for by the instruction, we want to skip the optional 	     operands.  */
if|if
condition|(
name|opcount
operator|<
...
expr_stmt|;
break|break;
comment|/* We'll only use the 32 (or 64) bit form of these relocations 	     in constants.  Instructions get the 16 bit form.  */
case|case
name|BFD_RELOC_PPC_DTPREL
case|:
name|reloc
...
directive|ifdef
name|OBJ_ELF
comment|/* Do we need/want a APUinfo section? */
if|if
condition|(
name|ppc_cpu
operator|&
...
condition|)
block|{
comment|/* These are all version "1".  */
if|if
condition|(
name|opcode
operator|->
...
name|_
argument_list|(
literal|"instruction address is not a multiple of 4"
argument_list|)
argument_list|)
expr_stmt|;
name|frag_now
...
name|_
argument_list|(
literal|"Bad .section directive: want a,e,w,x,M,S,G,T in string"
argument_list|)
expr_stmt|;
return|return
operator|-
...
begin_comment
comment|/* The .comm and .lcomm pseudo-ops for XCOFF.  XCOFF puts common    symbols in the .bss segment as though they were local common    symbols, and uses a different smclas.  The native Aix 4.3.3 assembler    aligns .comm and .lcomm to 4 bytes.  */
end_comment
...
name|_
argument_list|(
literal|".machine stack overflow"
argument_list|)
argument_list|)
expr_stmt|;
else|else
...
name|_
argument_list|(
literal|".machine stack underflow"
argument_list|)
argument_list|)
expr_stmt|;
else|else
...
literal|1
return|;
comment|/* This one will disappear anyway.  Don't make a csect sym for it.  */
if|if
condition|(
name|sym
operator|==
...
begin_comment
comment|/* If this function returns non-zero, it guarantees that a relocation    will be emitted for a fixup.  */
end_comment
...
decl_stmt|;
block|{
comment|/* Branch prediction relocations must force a relocation, as must      the vtable description relocs.  */
switch|switch
condition|(
name|fix
operator|->
...
literal|0xfffc
expr_stmt|;
comment|/* lq insns reserve the four lsbs.  */
if|if
condition|(
operator|(
name|ppc_cpu
...
condition|)
block|{
comment|/* This can occur if there is a bug in the input assembler, eg: 		 ".byte<undefined_symbol> - ."  */
if|if
condition|(
name|fixP
operator|->
...
name|_
argument_list|(
literal|"Unable to handle reference to symbol %s"
argument_list|)
argument_list|,
name|S_GET_NAME
argument_list|(
...
name|_
argument_list|(
literal|"Unable to resolve expression"
argument_list|)
argument_list|)
expr_stmt|;
name|fixP
...
name|value
expr_stmt|;
comment|/* PowerPC uses RELA relocs, ie. the reloc addend is stored separately      from the section contents.  If we are going to be emitting a reloc      then the section contents are immaterial, so don't warn if they      happen to overflow.  Leave such warnings to ld.  */
if|if
condition|(
operator|!
name|fixP
