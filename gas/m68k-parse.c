...
begin_comment
comment|/* A Bison parser, made from m68k-parse.y    by GNU bison 1.35.  */
end_comment
...
name|x
parameter_list|)
value|((unsigned)(x)<= 267 ? yytranslate[x] : 44)
end_define
...
literal|'+'
case|:
comment|/* It so happens that a '+' can only appear at the end of an          operand.  If it appears anywhere else, it must be a unary          plus on an expression.  */
if|if
condition|(
name|str
index|[
...
literal|'-'
case|:
comment|/* A '-' can only appear in -(ar), rn-rn, or ar@-.  If it          appears anywhere else, it must be a unary minus on an          expression.  */
if|if
condition|(
name|str
index|[
