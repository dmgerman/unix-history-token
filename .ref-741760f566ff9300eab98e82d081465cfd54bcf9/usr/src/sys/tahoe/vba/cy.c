...
begin_comment
comment|/*  * Raw tape operations use rcybuf.  The driver notices when  * rcybuf is being used and allows the user program to contine  * after errors and read records not of the standard length.  */
end_comment
...
begin_comment
comment|/*  * Software state and shared command areas per controller.  *  * The i/o buffer must be defined statically to insure  * it's address will fit in 20-bits (YECH!!!!!!!!!!!!!!)  */
end_comment
...
name|cy_map
decl_stmt|;
comment|/* pte's for mapped buffer i/o */
name|caddr_t
name|cy_utl
decl_stmt|;
comment|/* mapped virtual address */
name|int
name|cy_bs
decl_stmt|;
...
goto|;
block|}
comment|/* 	 * The following checks handle boundary cases for operation 	 * on no-raw tapes.  On raw tapes the initialization of 	 * yc->yc_nxrec by cyphys causes them to be skipped normally 	 * (except in the case of retries). 	 */
if|if
condition|(
name|bdbtofsb
argument_list|(
...
name|cmd
decl_stmt|;
comment|/* 		 * Choose the appropriate i/o command based on the 		 * transfer size, the estimated block size, 		 * and the controller's internal buffer size. 		 * If we're retrying a read on a raw device because 		 * the original try was a buffer request which failed 		 * due to a record length error, then we force the use 		 * of the raw controller read (YECH!!!!). 		 */
if|if
condition|(
name|bp
operator|->
...
name|next
label|:
comment|/* 	 * Done with this operation due to error or the 	 * fact that it doesn't do anything.  Release VERSAbus 	 * resource (if any), dequeue the transfer and continue 	 * processing this slave. 	 */
if|if
condition|(
name|bp
operator|==
...
begin_comment
comment|/*  * Check that a raw device exits.  * If it does, set up the yc_blkno and yc_nxrec  * so that the tape will appear positioned correctly.  */
end_comment
