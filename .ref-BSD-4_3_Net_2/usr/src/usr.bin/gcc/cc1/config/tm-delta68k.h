...
directive|define
name|CPP_PREDEFINES
value|"-Dm68k -Dmc68k -Dunix -Dunixpc"
end_define
...
parameter_list|)
define|\
value|fprintf (FILE, "#0x%.8x", (VALUE))
end_define
...
parameter_list|)
define|\
value|{ if ((PTR)[0] == 'j'&& (PTR)[1] == 'b')		\     { ++(PTR);						\       while (*(PTR) != ' ')				\ 	{ putc (*(PTR), (FILE)); ++(PTR); }		\       fprintf ((FILE), ".w"); }				\   else if ((PTR)[0] == 'f')				\     {							\       if (!strncmp ((PTR), "fmove", 5))			\ 	{ fprintf ((FILE), "fmov"); (PTR) += 5; }	\       else if (!strncmp ((PTR), "ftst", 4))		\ 	{ fprintf ((FILE), "ftest"); (PTR) += 4; }	\       else if (!strncmp ((PTR), "fbne", 4))		\ 	{ fprintf ((FILE), "fbgl"); (PTR) += 4; }	\       else if (!strncmp ((PTR), "fbeq", 4))		\ 	{ fprintf ((FILE), "fbngl"); (PTR) += 4; }	\     }							\
...
name|a
parameter_list|)
value|fprintf(asm_out_file, "\tdim\t%d;", a)
end_define
...
name|X
parameter_list|)
value|(GET_CODE(X)==CONST_DOUBLE?0:1)
end_define
