...
begin_comment
comment|/* Nonzero means (MEM (REG n)) is valid even if (REG n) is spilled.  */
end_comment
...
name|y
parameter_list|)
value|(x == y || (x != 0&& GET_CODE (x) != REG&& rtx_equal_p (x, y)))
...
expr_stmt|;
block|}
comment|/* If we are reloading a (SUBREG (MEM ...) ...) or (SUBREG constant ...),      really reload just the inside expression in its own mode.      Note that the case of (SUBREG (CONST_INT...)...) is handled elsewhere;      we can't handle it here because CONST_INT does not indicate a mode.  */
if|if
condition|(
name|in
operator|!=
...
argument_list|)
decl_stmt|;
comment|/* When we consider whether the insn uses OUT, 	 ignore references within IN.  They don't prevent us 	 from copying IN into OUT, because those refs would 	 move into the insn that reloads IN.  	 However, we only ignore IN in its role as this operand. 	 If the insn uses IN elsewhere and it contains OUT, 	 that counts.  We can't be sure it's the "same" operand 	 so it might not go through this reload.  */
operator|*
name|inloc
operator|=
name|const0_rtx
...
expr_stmt|;
block|}
comment|/* Consider using IN if OUT was not acceptable      or if OUT dies in this insn (like the quotient in a divmod insn).      We can't use IN unless it is free after this insn,      which means we must know accurately which hard regs are live.      Also, the result can't go in IN if IN is used within OUT.  */
if|if
condition|(
name|hard_regs_live_known
operator|&&
...
begin_comment
comment|/* Return 1 if hard reg number REGNO is stored in by expression X,    either explicitly or in the guise of a pseudo-reg allocated to REGNO.    X should be the body of an instruction.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/* See if this reg includes the specified one.  */
if|if
condition|(
name|r
operator|<=
...
name|SET
case|:
comment|/* Dispose quickly of (set (reg..) (reg..)) if both have hard regs.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
block|{
comment|/* This is equivalent to calling find_reloads_toplev. 	     The code is duplicated for speed.  */
specifier|register
name|int
name|regno
init|=
...
literal|0
decl_stmt|;
comment|/* REJECT is a count of how undesirable this alternative says it is 	 if any reloading is required.  If the alternative matches exactly 	 then REJECT is ignored, but otherwise it gets this much 	 counted against it in addition to the reloading needed.  */
name|int
name|reject
init|=
literal|0
...
goto|;
block|}
comment|/* REJECT, set by the ! and ? constraint characters, 	 discourages the use of this alternative for a reload goal.  */
if|if
condition|(
name|reject
operator|>
...
expr_stmt|;
block|}
comment|/* Any constants that aren't allowed and can't be reloaded      into memory locations are here changed into memory references.  */
for|for
control|(
name|i
operator|=
...
index|]
expr_stmt|;
comment|/* If this insn pattern contains any MATCH_DUP's, make sure that      they will be substituted if the operands they match are substituted.      Also do now any substitutions we already did on the operands.  */
if|if
condition|(
name|insn_code_number
operator|>=
...
literal|0
decl_stmt|;
comment|/* This reload is needed only for the address of something. 	     Determine whether it is needed for addressing an operand 	     being reloaded for input, whether it is needed for an 	     operand being reloaded for output, and whether it is needed 	     for addressing an operand that won't really be reloaded.  */
for|for
control|(
name|j
operator|=
...
begin_comment
comment|/* Scan X for memory references and scan the addresses for reloading.    Also checks for references to "constant" regs that we want to eliminate    and replaces them with the values they stand for.    We may alter X descructively if it contains a reference to such.    If X is just a constant reg, we return the equivalent value    instead of X.  */
end_comment
...
condition|)
block|{
comment|/* Check for SUBREG containing a REG that's equivalent to a constant.  */
specifier|register
name|int
name|regno
init|=
...
condition|)
block|{
comment|/* If the constant has a known value, truncate it right now.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
return|;
block|}
comment|/* If the constant is symbolic, allow it to be substituted normally. 	     push_reload will strip the subreg later.  */
block|}
comment|/* If the subreg contains a reg that will be converted to a mem, 	 convert the subreg to a narrower memref now. 	 Otherwise, we would get (subreg (mem ...) ...), 	 which would force reload of the mem.  */
elseif|else
if|if
condition|(
name|regno
...
literal|0
comment|/* This might screw code in reload1.c to delete prior output-reload 	 that feeds this insn.  */
block|if (reg_equiv_mem[regno] != 0) 	{ 	  if (strict_memory_address_p (mode, reg_equiv_mem[regno])) 	    { 	      *loc = ad = reg_equiv_mem[regno]; 	      return 1; 	    } 	}
endif|#
directive|endif
if|if
condition|(
...
literal|0
comment|/* This might screw code in reload1.c to delete prior output-reload 	 that feeds this insn.  */
block|if (reg_equiv_mem[regno] != 0) 	{ 	  push_reload (reg_equiv_mem[regno], 0, loc, 0, 		       context ? INDEX_REG_CLASS : BASE_REG_CLASS, 		       GET_MODE (x), 0, VOIDmode, 0, operand); 	  return 1; 	}
endif|#
directive|endif
if|if
condition|(
...
operator|==
name|INSN
comment|/* If we don't want spill regs (true for all calls in this file) */
operator|&&
operator|(
operator|!
operator|(
...
comment|/* Don't find the sp as an equiv, since pushes that we don't notice      would invalidate it.  */
if|if
condition|(
name|valueno
operator|==
...
condition|)
block|{
comment|/* If this insn P stores in either GOAL or VALUE, return 0. 	     If GOAL is a memory ref and this insn writes memory, return 0. 	     If GOAL is a memory ref and its address is not constant, 	     and this insn P changes a register, return 0. 	     That is in lieue of checking whether GOAL uses this register.  */
name|pat
operator|=
name|PATTERN
argument_list|(
...
block|}
block|}
comment|/* If this insn auto-increments or auto-decrements 	     either regno or valueno, return 0 now. 	     If GOAL is a memory ref and its address is not constant, 	     and this insn P increments a register, return 0. 	     That is in lieue of checking whether GOAL uses this register.  */
block|{
specifier|register
name|rtx
name|link
