...
begin_comment
comment|/* Handle initialization things in C++.    Copyright (C) 1987 Free Software Foundation, Inc.    Contributed by Michael Tiemann (tiemann@mcc.com)  This file is part of GNU CC.  GNU CC is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 1, or (at your option) any later version.  GNU CC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.  You should have received a copy of the GNU General Public License along with GNU CC; see the file COPYING.  If not, write to the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.  */
end_comment
...
name|sorry
argument_list|(
literal|"passing initializations up multiple inheritance lattice"
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
condition|)
block|{
comment|/* If there are virtual baseclasses without initialization 		 specified, and this is a default X(X&) construcotr, 		 build the initialization list so that each virtual baseclass 		 of the new object is initialized from the virtual baseclass 		 of the incoming arg.  */
name|tree
name|init_arg
init|=
name|build_unary_op
...
argument_list|)
argument_list|)
comment|/* BASE_INIT_LIST has already initialized these guys.  */
operator|||
operator|!
name|value_member
argument_list|(
...
begin_comment
comment|/* This code sets up the virtual function tables appropriate for    the pointer DECL.  It is a one-ply initialization.     TYPE is the exact type that DECL is supposed to be.  In    muliple inheritance, this might mean "C's A" if C : A, B.  */
end_comment
...
begin_comment
comment|/* Subroutine of `expand_aggr_vbase_init'.    BASETYPE is type that is being initialized.  INIT_LIST    is the list of initializers for the virtual baseclass.  */
end_comment
...
literal|30
argument_list|,
literal|"! end of vbase initialization"
argument_list|)
argument_list|,
literal|0
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Use a function returning the desired type to initialize EXP for us.      If the function is a constructor, and its first argument is      NULL_TREE, know that it was meant for us--just slide exp on      in and expand the constructor.  Constructors now come      as NEW_EXPRs.  */
if|if
condition|(
name|init
condition|)
...
directive|endif
comment|/* This happens when we use C++'s functional cast notation 	     to act as the initializer for something not of that same 	     type.  In that case, we need to create the initializer 	     separately from the object being initialized.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
condition|)
block|{
comment|/* A CALL_EXPR is a legitmate form of initialization, so 		 we should not print this warning message.  */
if|#
directive|if
literal|0
...
directive|if
literal|0
block|if (TYPE_GETS_ASSIGNMENT (type)) 		warning ("bitwise copy: `%s' defines operator=()", 			 TYPE_NAME_STRING (type));
endif|#
directive|endif
name|expand_assignment
argument_list|(
...
return|return;
block|}
comment|/* See whether we can go through a type conversion operator. 	 This wins over going through a constructor because we may be 	 able to avoid an X(X&) constructor.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
name|warning
argument_list|(
literal|"bitwise copy in initialization of type `%s'"
argument_list|,
name|TYPE_NAME_STRING
argument_list|(
name|type
...
begin_comment
comment|/* This code could just as well go in `cplus-class.c', but is placed here for    modularity.  */
end_comment
...
name|sorry
argument_list|(
literal|"multiple scope qualifications in build_member_call"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
begin_comment
comment|/* Build a reference to a member of an aggregate.  This is not a    C++ `&', but really something which can have its address taken,    and then act as a pointer to member, for example CNAME :: FIELD    can have its address taken by saying& CNAME :: FIELD.     @@ Prints out lousy diagnostics for operator<typename>    @@ fields.     @@ This function should be rewritten and placed in cplus-search.c.  */
end_comment
...
name|sorry
argument_list|(
literal|"multiple scope qualifications in build_offset_ref"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
return|;
block|}
comment|/* Unresolved multi-arity operator.  */
if|if
condition|(
name|TREE_CODE
argument_list|(
...
expr_stmt|;
block|}
comment|/* This is really hairy: if the function pointer is a pointer 	 to a non-virtual member function, then we can't go mucking 	 with the `this' pointer (any more than we aleady have to 	 this point).  If it is a pointer to a virtual member function, 	 then we have to adjust the `this' pointer according to 	 what the virtual function table tells us.  */
name|e3
operator|=
name|build_vfn_ref
argument_list|(
...
expr_stmt|;
block|}
comment|/* If this is a reference to a member function, then return      the address of the member function (which may involve going      through the object's vtable), otherwise, return an expression      for the derefernced pointer-to-member construct.  */
name|addr
operator|=
name|build_unary_op
argument_list|(
...
condition|&& TREE_CODE (DECL_INITIAL (decl)) != CONSTRUCTOR&& DECL_MODE (decl) != BLKmode
endif|#
directive|endif
condition|)
return|return
...
begin_comment
comment|/* Tell if this function specified by FUNCTION_DECL    can be a friend of type TYPE.    Return nonzero if friend, zero otherwise.     DECL can be zero if we are calling a constructor or accessing a    member in global scope.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* If this type has special alignment requirements, deal with them here.  */
if|if
condition|(
name|TYPE_ALIGN
argument_list|(
...
name|error
argument_list|(
literal|"placement syntax invalid for arrays"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
name|type
argument_list|,
literal|"no constructor for type `%s'"
argument_list|)
expr_stmt|;
name|rval
operator|=
...
begin_comment
comment|/* Free up storage of type TYPE, at address ADDR.    TYPE is a POINTER_TYPE.     This does not call any destructors.  */
end_comment
...
block|{
comment|/* This can get visibilties wrong.  */
name|int
name|i
decl_stmt|,
name|n_baseclasses
...
name|body
decl_stmt|;
comment|/* This is the LOOP_STMT that governs the deletetion of the elements.  */
name|tree
name|loop
decl_stmt|;
comment|/* This is the IF_STMT that governs whether the loop is executed at all.  */
name|tree
name|if_stmt
decl_stmt|;
comment|/* This is the ELSE_STMT that governs what to do after the loop has run.  */
name|tree
name|else_stmt
init|=
literal|0
decl_stmt|;
comment|/* This is the LET_STMT which holds the outermost iterator of the      loop.  It is convenient to set this variable up and test it before      executing any other code in the loop.  */
name|tree
name|block
decl_stmt|;
if|if
...
name|error
argument_list|(
literal|"must specify size for non array type"
argument_list|)
expr_stmt|;
return|return
name|error_mark_node
...
condition|)
block|{
comment|/* Convert from 0-based to 1-based indexing.  */
name|maxindex
operator|=
name|fold
argument_list|(
...
condition|)
block|{
comment|/* Convert back to 0-based indexing.  */
name|maxindex
operator|=
name|fold
argument_list|(
...
name|warning
argument_list|(
literal|"argument to vector delete disagrees with declared type of array"
argument_list|)
expr_stmt|;
name|base
operator|=
...
name|warning
argument_list|(
literal|"array size expression for delete ignored"
argument_list|)
expr_stmt|;
name|rval
operator|=
...
literal|1
expr_stmt|;
comment|/* ELSE_STMT holds the code which deletes the container.      If you don't want to delete the container, make ELSE_STMT      zero here.  */
if|if
condition|(
name|auto_delete_vec
operator|==
...
literal|1
expr_stmt|;
comment|/* BASE may be a SAVE_EXPR which must be evaluated before either      branch of the IF/THEN/ELSE.  */
name|STMT_BODY
argument_list|(
name|block
argument_list|)
