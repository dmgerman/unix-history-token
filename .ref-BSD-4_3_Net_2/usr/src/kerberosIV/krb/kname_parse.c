begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * $Source: /usr/src/kerberosIV/krb/RCS/kname_parse.c,v $  * $Author: kfall $  *  * Copyright 1987, 1988 by the Massachusetts Institute of Technology.  *  * For copying and distribution information, please see the file  *<mit-copyright.h>.  */
end_comment

begin_ifndef
ifndef|#
directive|ifndef
name|lint
end_ifndef

begin_decl_stmt
specifier|static
name|char
name|rcsid_kname_parse_c
index|[]
init|=
literal|"$Header: /usr/src/kerberosIV/krb/RCS/kname_parse.c,v 4.5 90/06/25 20:56:35 kfall Exp $"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* lint */
end_comment

begin_include
include|#
directive|include
file|<mit-copyright.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<des.h>
end_include

begin_include
include|#
directive|include
file|<krb.h>
end_include

begin_include
include|#
directive|include
file|<strings.h>
end_include

begin_comment
comment|/* max size of full name */
end_comment

begin_define
define|#
directive|define
name|FULL_SZ
value|(ANAME_SZ + INST_SZ + REALM_SZ)
end_define

begin_define
define|#
directive|define
name|NAME
value|0
end_define

begin_comment
comment|/* which field are we in? */
end_comment

begin_define
define|#
directive|define
name|INST
value|1
end_define

begin_define
define|#
directive|define
name|REALM
value|2
end_define

begin_decl_stmt
specifier|extern
name|char
modifier|*
name|krb_err_txt
index|[]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * This file contains four routines for handling Kerberos names.  *  * kname_parse() breaks a Kerberos name into its name, instance,  * and realm components.  *  * k_isname(), k_isinst(), and k_isrealm() check a given string to see if  * it's a syntactically legitimate respective part of a Kerberos name,  * returning 1 if it is, 0 if it isn't.  *  * Definition of "syntactically legitimate" names is according to  * the Project Athena Technical Plan Section E.2.1, page 7 "Specifying  * names", version dated 21 Dec 1987.  * /  /*  * kname_parse() takes a Kerberos name "fullname" of the form:  *  *		username[.instance][@realm]  *  * and returns the three components ("name", "instance", and "realm"  * in the example above) in the given arguments "np", "ip", and "rp".  *  * If successful, it returns KSUCCESS.  If there was an error,  * KNAME_FMT is returned.  */
end_comment

begin_macro
name|kname_parse
argument_list|(
argument|np
argument_list|,
argument|ip
argument_list|,
argument|rp
argument_list|,
argument|fullname
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|np
decl_stmt|,
modifier|*
name|ip
decl_stmt|,
modifier|*
name|rp
decl_stmt|,
modifier|*
name|fullname
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|static
name|char
name|buf
index|[
name|FULL_SZ
index|]
decl_stmt|;
name|char
modifier|*
name|rnext
decl_stmt|,
modifier|*
name|wnext
decl_stmt|;
comment|/* next char to read, write */
specifier|register
name|char
name|c
decl_stmt|;
name|int
name|backslash
decl_stmt|;
name|int
name|field
decl_stmt|;
name|backslash
operator|=
literal|0
expr_stmt|;
name|rnext
operator|=
name|buf
expr_stmt|;
name|wnext
operator|=
name|np
expr_stmt|;
name|field
operator|=
name|NAME
expr_stmt|;
if|if
condition|(
name|strlen
argument_list|(
name|fullname
argument_list|)
operator|>
name|FULL_SZ
condition|)
return|return
name|KNAME_FMT
return|;
operator|(
name|void
operator|)
name|strcpy
argument_list|(
name|buf
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
while|while
condition|(
name|c
operator|=
operator|*
name|rnext
operator|++
condition|)
block|{
if|if
condition|(
name|backslash
condition|)
block|{
operator|*
name|wnext
operator|++
operator|=
name|c
expr_stmt|;
name|backslash
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'\\'
case|:
name|backslash
operator|++
expr_stmt|;
break|break;
case|case
literal|'.'
case|:
switch|switch
condition|(
name|field
condition|)
block|{
case|case
name|NAME
case|:
if|if
condition|(
name|wnext
operator|==
name|np
condition|)
return|return
name|KNAME_FMT
return|;
operator|*
name|wnext
operator|=
literal|'\0'
expr_stmt|;
name|field
operator|=
name|INST
expr_stmt|;
name|wnext
operator|=
name|ip
expr_stmt|;
break|break;
case|case
name|INST
case|:
return|return
name|KNAME_FMT
return|;
comment|/* break; */
case|case
name|REALM
case|:
operator|*
name|wnext
operator|++
operator|=
name|c
expr_stmt|;
break|break;
default|default:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"unknown field value\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
literal|'@'
case|:
switch|switch
condition|(
name|field
condition|)
block|{
case|case
name|NAME
case|:
if|if
condition|(
name|wnext
operator|==
name|np
condition|)
return|return
name|KNAME_FMT
return|;
operator|*
name|ip
operator|=
literal|'\0'
expr_stmt|;
comment|/* fall through */
case|case
name|INST
case|:
operator|*
name|wnext
operator|=
literal|'\0'
expr_stmt|;
name|field
operator|=
name|REALM
expr_stmt|;
name|wnext
operator|=
name|rp
expr_stmt|;
break|break;
case|case
name|REALM
case|:
return|return
name|KNAME_FMT
return|;
default|default:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"unknown field value\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
break|break;
default|default:
operator|*
name|wnext
operator|++
operator|=
name|c
expr_stmt|;
block|}
block|}
operator|*
name|wnext
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
operator|(
name|strlen
argument_list|(
name|np
argument_list|)
operator|>
name|ANAME_SZ
operator|-
literal|1
operator|)
operator|||
operator|(
name|strlen
argument_list|(
name|ip
argument_list|)
operator|>
name|INST_SZ
operator|-
literal|1
operator|)
operator|||
operator|(
name|strlen
argument_list|(
name|rp
argument_list|)
operator|>
name|REALM_SZ
operator|-
literal|1
operator|)
condition|)
return|return
name|KNAME_FMT
return|;
return|return
name|KSUCCESS
return|;
block|}
end_block

begin_comment
comment|/*  * k_isname() returns 1 if the given name is a syntactically legitimate  * Kerberos name; returns 0 if it's not.  */
end_comment

begin_macro
name|k_isname
argument_list|(
argument|s
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|s
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
name|c
decl_stmt|;
name|int
name|backslash
init|=
literal|0
decl_stmt|;
if|if
condition|(
operator|!
operator|*
name|s
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|strlen
argument_list|(
name|s
argument_list|)
operator|>
name|ANAME_SZ
operator|-
literal|1
condition|)
return|return
literal|0
return|;
while|while
condition|(
name|c
operator|=
operator|*
name|s
operator|++
condition|)
block|{
if|if
condition|(
name|backslash
condition|)
block|{
name|backslash
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'\\'
case|:
name|backslash
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'.'
case|:
return|return
literal|0
return|;
comment|/* break; */
case|case
literal|'@'
case|:
return|return
literal|0
return|;
comment|/* break; */
block|}
block|}
return|return
literal|1
return|;
block|}
end_block

begin_comment
comment|/*  * k_isinst() returns 1 if the given name is a syntactically legitimate  * Kerberos instance; returns 0 if it's not.  */
end_comment

begin_macro
name|k_isinst
argument_list|(
argument|s
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|s
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
name|c
decl_stmt|;
name|int
name|backslash
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|strlen
argument_list|(
name|s
argument_list|)
operator|>
name|INST_SZ
operator|-
literal|1
condition|)
return|return
literal|0
return|;
while|while
condition|(
name|c
operator|=
operator|*
name|s
operator|++
condition|)
block|{
if|if
condition|(
name|backslash
condition|)
block|{
name|backslash
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'\\'
case|:
name|backslash
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'.'
case|:
return|return
literal|0
return|;
comment|/* break; */
case|case
literal|'@'
case|:
return|return
literal|0
return|;
comment|/* break; */
block|}
block|}
return|return
literal|1
return|;
block|}
end_block

begin_comment
comment|/*  * k_isrealm() returns 1 if the given name is a syntactically legitimate  * Kerberos realm; returns 0 if it's not.  */
end_comment

begin_macro
name|k_isrealm
argument_list|(
argument|s
argument_list|)
end_macro

begin_decl_stmt
name|char
modifier|*
name|s
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|char
name|c
decl_stmt|;
name|int
name|backslash
init|=
literal|0
decl_stmt|;
if|if
condition|(
operator|!
operator|*
name|s
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|strlen
argument_list|(
name|s
argument_list|)
operator|>
name|REALM_SZ
operator|-
literal|1
condition|)
return|return
literal|0
return|;
while|while
condition|(
name|c
operator|=
operator|*
name|s
operator|++
condition|)
block|{
if|if
condition|(
name|backslash
condition|)
block|{
name|backslash
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'\\'
case|:
name|backslash
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'@'
case|:
return|return
literal|0
return|;
comment|/* break; */
block|}
block|}
return|return
literal|1
return|;
block|}
end_block

end_unit

