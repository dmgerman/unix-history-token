...
begin_comment
comment|/* $Id: os.c,v 1.89.12.5 2009-03-02 03:03:54 marka Exp $ */
end_comment
...
parameter_list|)
define|\
value|do { \ 		capval = (flag); \ 		cap_flag_value_t curval; \ 		err = cap_get_flag(curcaps, capval, CAP_PERMITTED,&curval); \ 		if (err != -1&& curval) { \ 			err = cap_set_flag(caps, CAP_EFFECTIVE, 1,&capval, CAP_SET); \ 			if (err == -1) { \ 				isc__strerror(errno, strbuf, sizeof(strbuf)); \ 				ns_main_earlyfatal("cap_set_proc failed: %s", strbuf); \ 			} \ 			\ 			err = cap_set_flag(caps, CAP_PERMITTED, 1,&capval, CAP_SET); \ 			if (err == -1) { \ 				isc__strerror(errno, strbuf, sizeof(strbuf)); \ 				ns_main_earlyfatal("cap_set_proc failed: %s", strbuf); \ 			} \ 		} \ 	} while (0)
define|#
directive|define
name|INIT_CAP
define|\
value|do { \ 		caps = cap_init(); \ 		if (caps == NULL) { \ 			isc__strerror(errno, strbuf, sizeof(strbuf)); \ 			ns_main_earlyfatal("cap_init failed: %s", strbuf); \ 		} \ 		curcaps = cap_get_proc(); \ 		if (curcaps == NULL) { \ 			isc__strerror(errno, strbuf, sizeof(strbuf)); \ 			ns_main_earlyfatal("cap_get_proc failed: %s", strbuf); \ 		} \ 	} while (0)
define|#
directive|define
name|FREE_CAP
define|\
value|{ \ 		cap_free(caps); \ 		cap_free(curcaps); \ 	} while (0)
else|#
directive|else
define|#
directive|define
...
name|flag
parameter_list|)
value|do { caps |= (1<< (flag)); } while (0)
define|#
directive|define
name|INIT_CAP
value|do { caps = 0; } while (0)
endif|#
directive|endif
comment|/* HAVE_LIBCAP */
specifier|static
...
argument_list|(
literal|"prctl(PR_SET_DUMPABLE) failed: %s"
argument_list|,
name|strbuf
argument_list|)
expr_stmt|;
...
name|newvalue
decl_stmt|;
comment|/* 	 * Linux: max number of open files specified by one thread doesn't seem 	 * to apply to other threads on Linux. 	 */
name|newvalue
operator|=
...
name|ns_main_earlywarning
argument_list|(
literal|"couldn't adjust limit on open files"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
argument_list|)
expr_stmt|;
comment|/* 	 * Make the containing directory if it doesn't exist. 	 */
if|if
condition|(
name|mkdirpath
argument_list|(
