...
begin_comment
comment|/* $Id: dnssec-signzone.c,v 1.209.12.3 2009/01/18 23:25:15 marka Exp $ */
end_comment
...
directive|if
literal|0
comment|/* 		 * Delete all NSEC records and RRSIG(NSEC) if we are in 		 * NSEC3 mode and vica versa. 		 */
block|for (result = dns_rdatasetiter_first(rdsiter2); 		     result == ISC_R_SUCCESS; 		     result = dns_rdatasetiter_next(rdsiter2)) { 			dns_rdatasetiter_current(rdsiter,&rdataset); 			type = rdataset.type; 			covers = rdataset.covers; 			if (type == dns_rdatatype_rrsig) 				type = covers; 			dns_rdataset_disassociate(&rdataset); 			if (type == nsec_datatype || 			    (type != dns_rdatatype_nsec&& 			     type != dns_rdatatype_nsec3)) 				continue; 			if (covers != 0) 				type = dns_rdatatype_rrsig; 			result = dns_db_deleterdataset(gdb, node, gversion, 						       type, covers); 			check_result(result, "dns_db_deleterdataset()"); 		}
endif|#
directive|endif
block|}
name|dns_rdatasetiter_destroy
...
begin_comment
comment|/*%  * Does this node only contain NSEC3 records or RRSIG records or is empty.  */
end_comment
...
condition|)
return|return;
comment|/* 	 * Delete any matching NSEC3 records which have parameters that 	 * match the NSEC3 chain we are building. 	 */
for|for
control|(
name|result
operator|=
...
condition|)
break|break;
comment|/* 			 * Cleanout NSEC3 RRsets which don't exist in the 			 * hash table. 			 */
name|nsec3clean
argument_list|(
name|nextname
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 			 * Skip NSEC3 only nodes when looking for the next 			 * node in the zone.  Also skips now empty nodes. 			 */
if|if
condition|(
name|nsec3only
argument_list|(
...
name|stderr
argument_list|,
literal|"\t-H iterations (NSEC3 iterations)\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
...
directive|define
name|CMDLINE_FLAGS
value|"3:aAc:d:e:f:ghH:i:I:j:k:l:m:n:N:o:O:pr:s:StUv:z"
