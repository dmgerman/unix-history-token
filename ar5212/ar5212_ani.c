...
name|HAL_DEBUG_ANY
argument_list|,
literal|"No more channel states left. Using channel 0\n"
argument_list|)
expr_stmt|;
return|return
literal|0
...
argument_list|)
decl_stmt|;
comment|/* update mib stats so we return current data */
comment|/* XXX? side-effects to doing this here? */
name|ar5212UpdateMibCounters
argument_list|(
name|ah
argument_list|,
...
begin_comment
comment|/*  * Setup ANI handling.  Sets all thresholds and reset the  * channel statistics.  Note that ar5212AniReset should be  * called by ar5212Reset before anything else happens and  * that's where we force initial settings.  */
end_comment
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: rssi %d zero ST (was %u)\n"
argument_list|,
name|__func__
argument_list|,
name|rssi
...
directive|if
literal|0
block|ath_hal_printf(ah,"%s: chan %u/0x%x restore %d setup %d opmode %u\n", 	    __func__, chan->channel, chan->channelFlags, restore, 	    aniState->isSetup, opmode);
else|#
directive|else
name|HALDEBUG
argument_list|(
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: chan %u/0x%x restore %d setup %d opmode %u\n"
argument_list|,
name|__func__
argument_list|,
name|chan
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: OFDM err %u listenTime %u\n"
argument_list|,
name|__func__
argument_list|,
name|aniState
...
name|HAL_DEBUG_ANI
argument_list|,
literal|"%s: CCK err %u listenTime %u\n"
argument_list|,
name|__func__
argument_list|,
name|aniState
