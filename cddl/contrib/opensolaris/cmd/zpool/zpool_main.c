...
literal|"\t    [-d dir | -c cachefile] [-D] [-f] [-R root] -a\n"
literal|"\timport [-o mntopts] [-o property=value] ... \n"
literal|"\t    [-d dir | -c cachefile] [-D] [-f] [-R root] "
...
begin_comment
comment|/*  * zpool remove<pool><vdev> ...  *  * Removes the given vdev from the pool.  Currently, this only supports removing  * spares and cache devices from the pool.  Eventually, we'll want to support  * removing leaf vdevs (as an alias for 'detach') as well as toplevel vdevs.  */
end_comment
...
begin_comment
comment|/*  * zpool import [-d dir] [-D]  *       import [-o mntopts] [-o prop=value] ... [-R root] [-D]  *              [-d dir | -c cachefile] [-f] -a  *       import [-o mntopts] [-o prop=value] ... [-R root] [-D]  *              [-d dir | -c cachefile] [-f]<pool | id> [newpool]  *  *	 -c	Read pool information from a cachefile instead of searching  *		devices.  *  *       -d	Scan in a specific directory, other than /dev/dsk.  More than  *		one directory can be specified using multiple '-d' options.  *  *       -D     Scan for previously destroyed pools or import all or only  *              specified destroyed pools.  *  *       -R	Temporarily import the pool, with all mountpoints relative to  *		the given root.  The pool will remain exported when the machine  *		is rebooted.  *  *       -f	Force import, even if it appears that the pool is active.  *  *       -F	Import even in the presence of faulted vdevs.  This is an  *       	intentionally undocumented option for testing purposes, and  *       	treats the pool configuration as complete, leaving any bad  *		vdevs in the FAULTED state. In other words, it does verbatim  *		import.  *  *       -a	Import all pools found.  *  *       -o	Set property=value and/or temporary mount options (without '=').  *  * The import command scans for pools to import, and import pools based on pool  * name and GUID.  The pool can also be renamed as part of the import process.  */
end_comment
...
block|{
comment|/* 		 * It's OK to search by guid even if searchguid is 0. 		 */
name|pools
operator|=
...
index|[]
init|=
literal|"name,size,used,available,capacity,health,altroot"
decl_stmt|;
name|char
modifier|*
name|props
...
name|gettext
argument_list|(
literal|"%s %s after %lluh%um with %llu errors "
literal|"on %s"
argument_list|)
argument_list|,
name|scrub_type
...
name|gettext
argument_list|(
literal|"%s in progress for %lluh%um, %.2f%% done, "
...
begin_comment
comment|/*  * Print log vdevs.  * Logs are recorded as top level vdevs in the main pool child array but with  * "is_log" set to 1. We use print_status_config() to print the top level logs  * then any log children (eg mirrored slogs) are printed recursively - which  * works because only the top level vdev is marked "is_log"  */
end_comment
...
name|stderr
argument_list|,
literal|"Unable to stat root file system: %s.\n"
argument_list|,
name|strerror
argument_list|(
name|errno
...
name|printf
argument_list|(
literal|"If you boot from pool '%s', don't forget to update boot code.\n"
literal|"Assuming you use GPT partitioning and da0 is your boot disk\n"
literal|"the following command will do it:\n"
literal|"\n"
literal|"\tgpart bootcode -b /boot/pmbr -p /boot/gptzfsboot -i 1 da0\n\n"
argument_list|,
name|cb
operator|.
name|cb_poolname
