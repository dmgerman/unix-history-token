...
begin_comment
comment|/*  * Use nextboot(8) to tell the loader to load DTrace kernel modules during  * the next boot of the system. The nextboot(8) configuration is removed during  * boot, so it will not persist indefinitely.  */
end_comment
...
literal|"-e"
block|,
literal|"dtraceall_load=\"YES\""
block|,
literal|"-e"
block|,
literal|"dtrace_dof_load=\"YES\""
block|,
literal|"-e"
block|,
literal|"dtrace_dof_name=\"/boot/dtrace.dof\""
block|,
literal|"-e"
block|,
literal|"dtrace_dof_type=\"dtrace_dof\""
block|,
name|NULL
block|, 	}
decl_stmt|;
...
name|error
argument_list|(
literal|"failed to execute nextboot: %s"
argument_list|,
name|strerror
argument_list|(
name|err
...
name|error
argument_list|(
literal|"nextboot returned with status %d"
argument_list|,
name|status
argument_list|)
expr_stmt|;
...
directive|ifdef
name|__FreeBSD__
comment|/* 	 * On FreeBSD, the DOF file is read directly during boot - just write 	 * two hex characters per byte. 	 */
name|oprintf
argument_list|(
literal|"dof-data-%d="
argument_list|,
...
directive|else
comment|/* 			 * On FreeBSD, anonymous DOF data is written to 			 * the DTrace DOF file. 			 */
name|g_ofile
operator|=
