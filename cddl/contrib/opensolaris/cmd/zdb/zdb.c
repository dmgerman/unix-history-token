...
name|stderr
argument_list|,
literal|"Usage: %s [-udibcsvL] [-U cachefile_path] [-t txg]\n"
...
name|printf
argument_list|(
literal|"\tvdev %5llu   offset %12llx   spacemap %6llu   free    %5s\n"
argument_list|,
operator|(
name|u_longlong_t
operator|)
...
argument_list|)
expr_stmt|;
comment|/* 	 * if we do metadata-only checksumming there's no need to checksum 	 * indirect blocks here because it is done during traverse 	 */
name|is_l0_metadata
operator|=
operator|(
name|BP_GET_LEVEL
