...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright (c) 2012 Pawel Jakub Dawidek<pawel@dawidek.net>.  * All rights reserved.  * Copyright 2013 Nexenta Systems, Inc.  All rights reserved.  * Copyright (c) 2013 by Delphix. All rights reserved.  */
end_comment
...
begin_comment
comment|/*  * Include snaps if they were requested or if this a zfs list where types  * were not specified and the "listsnapshots" property is set on this pool.  */
end_comment

begin_function
specifier|static
name|boolean_t
name|zfs_include_snapshots
parameter_list|(
name|zfs_handle_t
modifier|*
name|zhp
...
begin_comment
comment|/*  * Called for each dataset.  If the object is of an appropriate type,  * add it to the avl tree and recurse over any children as necessary.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* 			 * Both lcreate and rcreate being 0 means we don't have 			 * properties and we should compare full name. 			 */
if|if
condition|(
name|lcreate
operator|==
