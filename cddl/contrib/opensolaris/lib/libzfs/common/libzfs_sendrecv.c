...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright (c) 2012 by Delphix. All rights reserved.  * Copyright (c) 2012, Joyent, Inc. All rights reserved.  * Copyright (c) 2012 Pawel Jakub Dawidek<pawel@dawidek.net>.  * All rights reserved.  */
end_comment
...
name|B_FALSE
expr_stmt|;
comment|/* Initialize the write-by-reference block. */
name|wbr_drr
operator|.
name|drr_type
operator|=
...
operator|)
return|;
comment|/* 	 * zfs_send() only opens a cleanup_fd for sends that need it, 	 * e.g. replication and doall. 	 */
if|if
condition|(
name|sdd
operator|->
...
literal|'@'
expr_stmt|;
comment|/* 	 * It's OK if the parent no longer exists.  The send code will 	 * handle that error. 	 */
if|if
condition|(
name|pzhp
condition|)
...
condition|)
block|{
comment|/* 		 * Do a verbose no-op dry run to get all the verbose output 		 * before generating any data.  Then do a non-verbose real 		 * run to generate the streams. 		 */
name|sdd
operator|.
name|dryrun
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * Search progressively larger portions of the hierarchy.  This will 	 * select the "most local" version of the origin snapshot in the case 	 * that there are multiple matching snapshots in the system. 	 */
while|while
condition|(
operator|(
name|cp
...
operator|)
return|;
comment|/* 		 * Remember the dataset that we already searched, so we 		 * skip it next time through. 		 */
name|gtnd
operator|.
name|skip
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 		 * Check if we're going to rename based on parent guid change 		 * and the current parent guid was also deleted. If it was then 		 * rename will fail and is likely unneeded, so avoid this and 		 * force an early retry to determine the new 		 * parent_fromsnap_guid. 		 */
if|if
condition|(
name|stream_parent_fromsnap_guid
operator|!=
...
name|TEXT_DOMAIN
argument_list|,
literal|"invalid substream header"
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
argument_list|()
expr_stmt|;
comment|/* 	 * For forward compatibility hide tosnap in zc_value 	 */
if|if
condition|(
name|zfs_ioctl_version
operator|<
