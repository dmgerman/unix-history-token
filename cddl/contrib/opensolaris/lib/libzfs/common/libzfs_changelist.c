...
begin_comment
comment|/*  * Copyright 2010 Sun Microsystems, Inc.  All rights reserved.  * Use is subject to license terms.  *  * Portions Copyright 2007 Ramprakash Jelari  * Copyright (c) 2011 Pawel Jakub Dawidek<pawel@dawidek.net>.  * All rights reserved.  * Copyright (c) 2014, 2015 by Delphix. All rights reserved.  * Copyright 2016 Igor Kozhukhov<ikozhukhov@gmail.com>  */
end_comment
...
name|cl_shareprop
decl_stmt|;
comment|/* used with sharenfs/sharesmb */
name|uu_list_pool_t
modifier|*
name|cl_pool
decl_stmt|;
...
begin_comment
comment|/*  * If the property is 'mountpoint', go through and unmount filesystems as  * necessary.  We don't do the same for 'sharenfs', because we can just re-share  * with different options without interrupting service. We do handle 'sharesmb'  * since there may be old resource names that need to be removed.  */
end_comment
...
control|)
block|{
comment|/* if a previous loop failed, set the remaining to false */
if|if
condition|(
name|ret
operator|==
...
condition|)
block|{
comment|/* 			 * Do the property specific processing. 			 */
switch|switch
condition|(
name|clp
operator|->
...
expr_stmt|;
block|}
comment|/* 	 * We walk the datasets in reverse, because we want to mount any parent 	 * datasets before mounting the children.  We walk all datasets even if 	 * there are errors. 	 */
for|for
control|(
name|cn
operator|=
...
condition|)
continue|continue;
comment|/* Only do post-processing if it's required */
if|if
condition|(
operator|!
name|cn
...
condition|)
continue|continue;
comment|/* 		 * Remount if previously mounted or mountpoint was legacy, 		 * or sharenfs or sharesmb  property is set. 		 */
name|sharenfs
operator|=
operator|(
operator|(
...
begin_comment
comment|/*  * Given a gathered changelist for the 'sharenfs' or 'sharesmb' property,  * unshare all the datasets in the list.  */
end_comment
...
return|;
block|}
comment|/* 	 * If we are "watching" sharenfs or sharesmb 	 * then check out the companion property which is tracked 	 * in cl_shareprop 	 */
if|if
condition|(
name|clp
operator|->
...
operator|)
return|;
comment|/* 	 * If watching SHARENFS or SHARESMB then 	 * also watch its companion property. 	 */
if|if
condition|(
name|clp
operator|->
...
block|{
comment|/* 		 * Add the target dataset to the end of the list. 		 * The list is not really unsorted. The list will be 		 * in reverse dataset name order. This is necessary 		 * when the original mountpoint is legacy or none. 		 */
name|verify
argument_list|(
...
expr_stmt|;
block|}
comment|/* 	 * If the mountpoint property was previously 'legacy', or 'none', 	 * record it as the behavior of changelist_postfix() will be different. 	 */
if|if
condition|(
operator|(
name|clp
...
condition|)
block|{
comment|/* 		 * do not automatically mount ex-legacy datasets if 		 * we specifically set canmount to noauto 		 */
if|if
condition|(
name|zfs_prop_get_int
argument_list|(
