...
begin_comment
comment|/*  * Iterate over all children of the given dataset, adding any vertices  * as necessary.  Returns -1 if there was an error, or 0 otherwise.  * This is a simple recursive algorithm - the ZFS namespace typically  * is very flat.  We manually invoke the necessary ioctl() calls to  * avoid the overhead and additional semantics of zfs_open().  */
end_comment
...
operator|)
return|;
comment|/* 			 * Count origins only if they are contained in the graph 			 */
if|if
condition|(
name|isa_child_of
argument_list|(
...
operator|)
return|;
comment|/* 		 * Recursively visit child 		 */
if|if
condition|(
name|iterate_children
argument_list|(
...
begin_comment
comment|/*  * Returns false if there are no snapshots with dependent clones in this  * subtree or if all of those clones are also in this subtree.  Returns  * true if there is an error or there are external dependents.  */
end_comment
...
block|}
decl_stmt|;
comment|/* 	 * Check whether this dataset is a clone or has clones since 	 * iterate_children() only checks the children. 	 */
operator|(
name|void
operator|)
name|strlcpy
...
begin_comment
comment|/*  * Construct a complete graph of all necessary vertices.  First, iterate over  * only our object's children.  If no cloned snapshots are found, or all of  * the cloned snapshots are in this subtree then return a graph of the subtree.  * Otherwise, start at the root of the pool and iterate over all datasets.  */
end_comment
