...
argument_list|(
argument|zfs_handle_t *zhp
argument_list|,
argument|zfs_cmd_t *zc
argument_list|)
end_macro
...
return|;
block|}
comment|/* If we can successfully roll it back, reset the stats */
if|if
condition|(
name|ioctl
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 			 * If we tried to use a default value for a 			 * readonly property, it means that it was not 			 * present; return an error. 			 */
if|if
condition|(
name|zfs_prop_readonly
argument_list|(
...
begin_comment
comment|/* Check if a string contains only digits */
end_comment

begin_function
specifier|static
name|int
name|string_is_digits
parameter_list|(
name|char
modifier|*
name|cp
...
directive|else
comment|/* sun */
comment|/* 	 * In FreeBSD user and group names can begin with a digit so treat 	 * as a uid/gid if string contains digits only 	 */
block|}
elseif|else
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * FreeBSD compatibility with pre-v15 kernel module. 	 * Ignore private dataset names. 	 */
if|if
condition|(
name|strchr
argument_list|(
