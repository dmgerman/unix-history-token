...
begin_comment
comment|/*  * Accepts a property and value and checks that the value  * matches the one found by the channel program. If they are  * not equal, print both of them.  */
end_comment
...
init|=
literal|"args = ...\n"
literal|"ds = args['dataset']\n"
literal|"prop = args['property']\n"
literal|"value, setpoint = zfs.get_prop(ds, prop)\n"
literal|"return {value=value, setpoint=setpoint}\n"
decl_stmt|;
name|nvlist_t
modifier|*
name|outnvl
...
name|stderr
argument_list|,
literal|"%s: zfs found %lld, but zcp found %lld\n"
argument_list|,
name|zfs_prop_to_name
argument_list|(
name|prop
...
name|stderr
argument_list|,
literal|"%s: zfs found %s, but zcp found %s\n"
argument_list|,
name|zfs_prop_to_name
argument_list|(
name|prop
...
name|stderr
argument_list|,
literal|"zcp check failed, channel program error: %u\n"
argument_list|,
name|error
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* 	 * Pass both the filesystem and the wanted snapshot names, 	 * we would get an error back if the snapshot is destroyed or 	 * a new snapshot is created before this request is processed. 	 */
name|err
operator|=
...
name|TEXT_DOMAIN
argument_list|,
literal|"'%s' is not the latest snapshot"
argument_list|)
argument_list|,
name|snap
operator|->
