...
condition|)
block|{
comment|/* its one of our caches, see if it is better            * i.e. the endtime is farther, and if the endtimes          * are the same, take the larger, as he who has the           * most tickets wins.          * it must also had the same set of flags at least          * i.e. if the forwarded TGT is forwardable, this one must           * be as well.             */
name|DEEDEBUG2
argument_list|(
literal|"Cache:%s"
argument_list|,
