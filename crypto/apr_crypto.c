...
parameter_list|)
define|\
value|{   \         extern const apr_crypto_driver_t driver; \         apr_hash_set(drivers,name,APR_HASH_KEY_STRING,&driver); \         if (driver.init) {     \             driver.init(pool, params); \         }  \     }
end_define
...
condition|)
block|{
comment|/* APR_EDSOOPEN or APR_ESYMNOTFOUND? */
if|if
condition|(
name|rv
operator|==
