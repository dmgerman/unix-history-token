...
name|RCSID
argument_list|(
literal|"$Id: tf_util.c,v 1.24 1997/04/20 06:24:32 assar Exp $"
argument_list|)
expr_stmt|;
end_expr_stmt
...
return|;
block|}
comment|/* The old code tried to guess when the calling program was    * running set-uid, this is now removed - the kerberos library    * does not (or shouldn't) know anything about user-ids.     * All library functions now assume that the right userids are set    * upon entry, therefore there is no need to test permissions like    * before. If the file is openable, just open it.    */
if|if
condition|(
operator|!
name|S_ISREG
