...
name|RCSID
argument_list|(
literal|"$Id: tf_util.c,v 1.35 1999/06/29 21:18:11 bg Exp $"
argument_list|)
expr_stmt|;
end_expr_stmt
...
name|TKT_FIL_ACC
return|;
comment|/* The code tries to guess when the calling program is running    * set-uid and prevent unauthorized access.    *    * All library functions now assume that the right set of userids    * are set upon entry, therefore it's not strictly necessary to    * perform these test for programs adhering to these assumptions.    */
block|{
name|uid_t
name|me
init|=
