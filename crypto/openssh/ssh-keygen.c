begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/* $OpenBSD: ssh-keygen.c,v 1.184 2010/03/07 22:16:01 djm Exp $ */
end_comment

begin_comment
comment|/*  * Author: Tatu Ylonen<ylo@cs.hut.fi>  * Copyright (c) 1994 Tatu Ylonen<ylo@cs.hut.fi>, Espoo, Finland  *                    All rights reserved  * Identity and host key generation and maintenance.  *  * As far as I am concerned, the code I have written for this software  * can be used freely for any purpose.  Any derived versions of this  * software must be clearly marked as such, and if the derived work is  * incompatible with the protocol description in the RFC file, it must be  * called by a name other than "ssh" or "Secure Shell".  */
end_comment

begin_include
include|#
directive|include
file|"includes.h"
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/stat.h>
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<openssl/evp.h>
end_include

begin_include
include|#
directive|include
file|<openssl/pem.h>
end_include

begin_include
include|#
directive|include
file|"openbsd-compat/openssl-compat.h"
end_include

begin_include
include|#
directive|include
file|<errno.h>
end_include

begin_include
include|#
directive|include
file|<fcntl.h>
end_include

begin_include
include|#
directive|include
file|<netdb.h>
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|HAVE_PATHS_H
end_ifdef

begin_include
include|#
directive|include
file|<paths.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<pwd.h>
end_include

begin_include
include|#
directive|include
file|<stdarg.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<stdlib.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<unistd.h>
end_include

begin_include
include|#
directive|include
file|"xmalloc.h"
end_include

begin_include
include|#
directive|include
file|"key.h"
end_include

begin_include
include|#
directive|include
file|"rsa.h"
end_include

begin_include
include|#
directive|include
file|"authfile.h"
end_include

begin_include
include|#
directive|include
file|"uuencode.h"
end_include

begin_include
include|#
directive|include
file|"buffer.h"
end_include

begin_include
include|#
directive|include
file|"pathnames.h"
end_include

begin_include
include|#
directive|include
file|"log.h"
end_include

begin_include
include|#
directive|include
file|"misc.h"
end_include

begin_include
include|#
directive|include
file|"match.h"
end_include

begin_include
include|#
directive|include
file|"hostfile.h"
end_include

begin_include
include|#
directive|include
file|"dns.h"
end_include

begin_include
include|#
directive|include
file|"ssh2.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|ENABLE_PKCS11
end_ifdef

begin_include
include|#
directive|include
file|"ssh-pkcs11.h"
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* Number of bits in the RSA/DSA key.  This value can be set on the command line. */
end_comment

begin_define
define|#
directive|define
name|DEFAULT_BITS
value|2048
end_define

begin_define
define|#
directive|define
name|DEFAULT_BITS_DSA
value|1024
end_define

begin_decl_stmt
name|u_int32_t
name|bits
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Flag indicating that we just want to change the passphrase.  This can be  * set on the command line.  */
end_comment

begin_decl_stmt
name|int
name|change_passphrase
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Flag indicating that we just want to change the comment.  This can be set  * on the command line.  */
end_comment

begin_decl_stmt
name|int
name|change_comment
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|quiet
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|log_level
init|=
name|SYSLOG_LEVEL_INFO
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag indicating that we want to hash a known_hosts file */
end_comment

begin_decl_stmt
name|int
name|hash_hosts
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag indicating that we want lookup a host in known_hosts file */
end_comment

begin_decl_stmt
name|int
name|find_host
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag indicating that we want to delete a host from a known_hosts file */
end_comment

begin_decl_stmt
name|int
name|delete_host
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag indicating that we want to show the contents of a certificate */
end_comment

begin_decl_stmt
name|int
name|show_cert
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag indicating that we just want to see the key fingerprint */
end_comment

begin_decl_stmt
name|int
name|print_fingerprint
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|print_bubblebabble
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* The identity file name, given on the command line or entered by the user. */
end_comment

begin_decl_stmt
name|char
name|identity_file
index|[
literal|1024
index|]
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|have_identity
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* This is set to the passphrase if given on the command line. */
end_comment

begin_decl_stmt
name|char
modifier|*
name|identity_passphrase
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* This is set to the new passphrase if given on the command line. */
end_comment

begin_decl_stmt
name|char
modifier|*
name|identity_new_passphrase
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* This is set to the new comment if given on the command line. */
end_comment

begin_decl_stmt
name|char
modifier|*
name|identity_comment
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Path to CA key when certifying keys. */
end_comment

begin_decl_stmt
name|char
modifier|*
name|ca_key_path
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Key type when certifying */
end_comment

begin_decl_stmt
name|u_int
name|cert_key_type
init|=
name|SSH2_CERT_TYPE_USER
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* "key ID" of signed key */
end_comment

begin_decl_stmt
name|char
modifier|*
name|cert_key_id
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Comma-separated list of principal names for certifying keys */
end_comment

begin_decl_stmt
name|char
modifier|*
name|cert_principals
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Validity period for certificates */
end_comment

begin_decl_stmt
name|u_int64_t
name|cert_valid_from
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|u_int64_t
name|cert_valid_to
init|=
operator|~
literal|0ULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Certificate constraints */
end_comment

begin_define
define|#
directive|define
name|CONSTRAINT_X_FWD
value|(1)
end_define

begin_define
define|#
directive|define
name|CONSTRAINT_AGENT_FWD
value|(1<<1)
end_define

begin_define
define|#
directive|define
name|CONSTRAINT_PORT_FWD
value|(1<<2)
end_define

begin_define
define|#
directive|define
name|CONSTRAINT_PTY
value|(1<<3)
end_define

begin_define
define|#
directive|define
name|CONSTRAINT_USER_RC
value|(1<<4)
end_define

begin_define
define|#
directive|define
name|CONSTRAINT_DEFAULT
value|(CONSTRAINT_X_FWD|CONSTRAINT_AGENT_FWD| \ 				CONSTRAINT_PORT_FWD|CONSTRAINT_PTY| \ 				CONSTRAINT_USER_RC)
end_define

begin_decl_stmt
name|u_int32_t
name|constraint_flags
init|=
name|CONSTRAINT_DEFAULT
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|constraint_command
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|constraint_src_addr
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Dump public key file in format used by real and the original SSH 2 */
end_comment

begin_decl_stmt
name|int
name|convert_to_ssh2
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|convert_from_ssh2
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|print_public
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|print_generic
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
modifier|*
name|key_type_name
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* argv0 */
end_comment

begin_decl_stmt
specifier|extern
name|char
modifier|*
name|__progname
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|char
name|hostname
index|[
name|MAXHOSTNAMELEN
index|]
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* moduli.c */
end_comment

begin_function_decl
name|int
name|gen_candidates
parameter_list|(
name|FILE
modifier|*
parameter_list|,
name|u_int32_t
parameter_list|,
name|u_int32_t
parameter_list|,
name|BIGNUM
modifier|*
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
name|int
name|prime_test
parameter_list|(
name|FILE
modifier|*
parameter_list|,
name|FILE
modifier|*
parameter_list|,
name|u_int32_t
parameter_list|,
name|u_int32_t
parameter_list|)
function_decl|;
end_function_decl

begin_function
specifier|static
name|void
name|ask_filename
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|,
specifier|const
name|char
modifier|*
name|prompt
parameter_list|)
block|{
name|char
name|buf
index|[
literal|1024
index|]
decl_stmt|;
name|char
modifier|*
name|name
init|=
name|NULL
decl_stmt|;
if|if
condition|(
name|key_type_name
operator|==
name|NULL
condition|)
name|name
operator|=
name|_PATH_SSH_CLIENT_ID_RSA
expr_stmt|;
else|else
block|{
switch|switch
condition|(
name|key_type_from_name
argument_list|(
name|key_type_name
argument_list|)
condition|)
block|{
case|case
name|KEY_RSA1
case|:
name|name
operator|=
name|_PATH_SSH_CLIENT_IDENTITY
expr_stmt|;
break|break;
case|case
name|KEY_DSA
case|:
name|name
operator|=
name|_PATH_SSH_CLIENT_ID_DSA
expr_stmt|;
break|break;
case|case
name|KEY_RSA
case|:
name|name
operator|=
name|_PATH_SSH_CLIENT_ID_RSA
expr_stmt|;
break|break;
default|default:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"bad key type\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
name|snprintf
argument_list|(
name|identity_file
argument_list|,
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
argument_list|,
literal|"%s/%s"
argument_list|,
name|pw
operator|->
name|pw_dir
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s (%s): "
argument_list|,
name|prompt
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
if|if
condition|(
name|fgets
argument_list|(
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|,
name|stdin
argument_list|)
operator|==
name|NULL
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|buf
index|[
name|strcspn
argument_list|(
name|buf
argument_list|,
literal|"\n"
argument_list|)
index|]
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|buf
argument_list|,
literal|""
argument_list|)
operator|!=
literal|0
condition|)
name|strlcpy
argument_list|(
name|identity_file
argument_list|,
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
argument_list|)
expr_stmt|;
name|have_identity
operator|=
literal|1
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|Key
modifier|*
name|load_identity
parameter_list|(
name|char
modifier|*
name|filename
parameter_list|)
block|{
name|char
modifier|*
name|pass
decl_stmt|;
name|Key
modifier|*
name|prv
decl_stmt|;
name|prv
operator|=
name|key_load_private
argument_list|(
name|filename
argument_list|,
literal|""
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
if|if
condition|(
name|prv
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
name|identity_passphrase
condition|)
name|pass
operator|=
name|xstrdup
argument_list|(
name|identity_passphrase
argument_list|)
expr_stmt|;
else|else
name|pass
operator|=
name|read_passphrase
argument_list|(
literal|"Enter passphrase: "
argument_list|,
name|RP_ALLOW_STDIN
argument_list|)
expr_stmt|;
name|prv
operator|=
name|key_load_private
argument_list|(
name|filename
argument_list|,
name|pass
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|memset
argument_list|(
name|pass
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|pass
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|pass
argument_list|)
expr_stmt|;
block|}
return|return
name|prv
return|;
block|}
end_function

begin_define
define|#
directive|define
name|SSH_COM_PUBLIC_BEGIN
value|"---- BEGIN SSH2 PUBLIC KEY ----"
end_define

begin_define
define|#
directive|define
name|SSH_COM_PUBLIC_END
value|"---- END SSH2 PUBLIC KEY ----"
end_define

begin_define
define|#
directive|define
name|SSH_COM_PRIVATE_BEGIN
value|"---- BEGIN SSH2 ENCRYPTED PRIVATE KEY ----"
end_define

begin_define
define|#
directive|define
name|SSH_COM_PRIVATE_KEY_MAGIC
value|0x3f6ff9eb
end_define

begin_function
specifier|static
name|void
name|do_convert_to_ssh2
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|)
block|{
name|Key
modifier|*
name|k
decl_stmt|;
name|u_int
name|len
decl_stmt|;
name|u_char
modifier|*
name|blob
decl_stmt|;
name|char
name|comment
index|[
literal|61
index|]
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|k
operator|=
name|key_load_public
argument_list|(
name|identity_file
argument_list|,
name|NULL
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
operator|(
name|k
operator|=
name|load_identity
argument_list|(
name|identity_file
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"load failed\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|k
operator|->
name|type
operator|==
name|KEY_RSA1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"version 1 keys are not supported\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|key_to_blob
argument_list|(
name|k
argument_list|,
operator|&
name|blob
argument_list|,
operator|&
name|len
argument_list|)
operator|<=
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"key_to_blob failed\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Comment + surrounds must fit into 72 chars (RFC 4716 sec 3.3) */
name|snprintf
argument_list|(
name|comment
argument_list|,
sizeof|sizeof
argument_list|(
name|comment
argument_list|)
argument_list|,
literal|"%u-bit %s, converted by %s@%s from OpenSSH"
argument_list|,
name|key_size
argument_list|(
name|k
argument_list|)
argument_list|,
name|key_type
argument_list|(
name|k
argument_list|)
argument_list|,
name|pw
operator|->
name|pw_name
argument_list|,
name|hostname
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stdout
argument_list|,
literal|"%s\n"
argument_list|,
name|SSH_COM_PUBLIC_BEGIN
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stdout
argument_list|,
literal|"Comment: \"%s\"\n"
argument_list|,
name|comment
argument_list|)
expr_stmt|;
name|dump_base64
argument_list|(
name|stdout
argument_list|,
name|blob
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stdout
argument_list|,
literal|"%s\n"
argument_list|,
name|SSH_COM_PUBLIC_END
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|k
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|blob
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|buffer_get_bignum_bits
parameter_list|(
name|Buffer
modifier|*
name|b
parameter_list|,
name|BIGNUM
modifier|*
name|value
parameter_list|)
block|{
name|u_int
name|bignum_bits
init|=
name|buffer_get_int
argument_list|(
name|b
argument_list|)
decl_stmt|;
name|u_int
name|bytes
init|=
operator|(
name|bignum_bits
operator|+
literal|7
operator|)
operator|/
literal|8
decl_stmt|;
if|if
condition|(
name|buffer_len
argument_list|(
name|b
argument_list|)
operator|<
name|bytes
condition|)
name|fatal
argument_list|(
literal|"buffer_get_bignum_bits: input buffer too small: "
literal|"need %d have %d"
argument_list|,
name|bytes
argument_list|,
name|buffer_len
argument_list|(
name|b
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|BN_bin2bn
argument_list|(
name|buffer_ptr
argument_list|(
name|b
argument_list|)
argument_list|,
name|bytes
argument_list|,
name|value
argument_list|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"buffer_get_bignum_bits: BN_bin2bn failed"
argument_list|)
expr_stmt|;
name|buffer_consume
argument_list|(
name|b
argument_list|,
name|bytes
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|Key
modifier|*
name|do_convert_private_ssh2_from_blob
parameter_list|(
name|u_char
modifier|*
name|blob
parameter_list|,
name|u_int
name|blen
parameter_list|)
block|{
name|Buffer
name|b
decl_stmt|;
name|Key
modifier|*
name|key
init|=
name|NULL
decl_stmt|;
name|char
modifier|*
name|type
decl_stmt|,
modifier|*
name|cipher
decl_stmt|;
name|u_char
modifier|*
name|sig
decl_stmt|,
name|data
index|[]
init|=
literal|"abcde12345"
decl_stmt|;
name|int
name|magic
decl_stmt|,
name|rlen
decl_stmt|,
name|ktype
decl_stmt|,
name|i1
decl_stmt|,
name|i2
decl_stmt|,
name|i3
decl_stmt|,
name|i4
decl_stmt|;
name|u_int
name|slen
decl_stmt|;
name|u_long
name|e
decl_stmt|;
name|buffer_init
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|buffer_append
argument_list|(
operator|&
name|b
argument_list|,
name|blob
argument_list|,
name|blen
argument_list|)
expr_stmt|;
name|magic
operator|=
name|buffer_get_int
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
if|if
condition|(
name|magic
operator|!=
name|SSH_COM_PRIVATE_KEY_MAGIC
condition|)
block|{
name|error
argument_list|(
literal|"bad magic 0x%x != 0x%x"
argument_list|,
name|magic
argument_list|,
name|SSH_COM_PRIVATE_KEY_MAGIC
argument_list|)
expr_stmt|;
name|buffer_free
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|i1
operator|=
name|buffer_get_int
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|type
operator|=
name|buffer_get_string
argument_list|(
operator|&
name|b
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|cipher
operator|=
name|buffer_get_string
argument_list|(
operator|&
name|b
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|i2
operator|=
name|buffer_get_int
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|i3
operator|=
name|buffer_get_int
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|i4
operator|=
name|buffer_get_int
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|debug
argument_list|(
literal|"ignore (%d %d %d %d)"
argument_list|,
name|i1
argument_list|,
name|i2
argument_list|,
name|i3
argument_list|,
name|i4
argument_list|)
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|cipher
argument_list|,
literal|"none"
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|error
argument_list|(
literal|"unsupported cipher %s"
argument_list|,
name|cipher
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|cipher
argument_list|)
expr_stmt|;
name|buffer_free
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|type
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|xfree
argument_list|(
name|cipher
argument_list|)
expr_stmt|;
if|if
condition|(
name|strstr
argument_list|(
name|type
argument_list|,
literal|"dsa"
argument_list|)
condition|)
block|{
name|ktype
operator|=
name|KEY_DSA
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|strstr
argument_list|(
name|type
argument_list|,
literal|"rsa"
argument_list|)
condition|)
block|{
name|ktype
operator|=
name|KEY_RSA
expr_stmt|;
block|}
else|else
block|{
name|buffer_free
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|type
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|key
operator|=
name|key_new_private
argument_list|(
name|ktype
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|type
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|key
operator|->
name|type
condition|)
block|{
case|case
name|KEY_DSA
case|:
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|dsa
operator|->
name|p
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|dsa
operator|->
name|g
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|dsa
operator|->
name|q
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|dsa
operator|->
name|pub_key
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|dsa
operator|->
name|priv_key
argument_list|)
expr_stmt|;
break|break;
case|case
name|KEY_RSA
case|:
name|e
operator|=
name|buffer_get_char
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|debug
argument_list|(
literal|"e %lx"
argument_list|,
name|e
argument_list|)
expr_stmt|;
if|if
condition|(
name|e
operator|<
literal|30
condition|)
block|{
name|e
operator|<<=
literal|8
expr_stmt|;
name|e
operator|+=
name|buffer_get_char
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|debug
argument_list|(
literal|"e %lx"
argument_list|,
name|e
argument_list|)
expr_stmt|;
name|e
operator|<<=
literal|8
expr_stmt|;
name|e
operator|+=
name|buffer_get_char
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|debug
argument_list|(
literal|"e %lx"
argument_list|,
name|e
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|BN_set_word
argument_list|(
name|key
operator|->
name|rsa
operator|->
name|e
argument_list|,
name|e
argument_list|)
condition|)
block|{
name|buffer_free
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|key
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|rsa
operator|->
name|d
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|rsa
operator|->
name|n
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|rsa
operator|->
name|iqmp
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|rsa
operator|->
name|q
argument_list|)
expr_stmt|;
name|buffer_get_bignum_bits
argument_list|(
operator|&
name|b
argument_list|,
name|key
operator|->
name|rsa
operator|->
name|p
argument_list|)
expr_stmt|;
name|rsa_generate_additional_parameters
argument_list|(
name|key
operator|->
name|rsa
argument_list|)
expr_stmt|;
break|break;
block|}
name|rlen
operator|=
name|buffer_len
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
if|if
condition|(
name|rlen
operator|!=
literal|0
condition|)
name|error
argument_list|(
literal|"do_convert_private_ssh2_from_blob: "
literal|"remaining bytes in key blob %d"
argument_list|,
name|rlen
argument_list|)
expr_stmt|;
name|buffer_free
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
comment|/* try the key */
name|key_sign
argument_list|(
name|key
argument_list|,
operator|&
name|sig
argument_list|,
operator|&
name|slen
argument_list|,
name|data
argument_list|,
sizeof|sizeof
argument_list|(
name|data
argument_list|)
argument_list|)
expr_stmt|;
name|key_verify
argument_list|(
name|key
argument_list|,
name|sig
argument_list|,
name|slen
argument_list|,
name|data
argument_list|,
sizeof|sizeof
argument_list|(
name|data
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|sig
argument_list|)
expr_stmt|;
return|return
name|key
return|;
block|}
end_function

begin_function
specifier|static
name|int
name|get_line
parameter_list|(
name|FILE
modifier|*
name|fp
parameter_list|,
name|char
modifier|*
name|line
parameter_list|,
name|size_t
name|len
parameter_list|)
block|{
name|int
name|c
decl_stmt|;
name|size_t
name|pos
init|=
literal|0
decl_stmt|;
name|line
index|[
literal|0
index|]
operator|=
literal|'\0'
expr_stmt|;
while|while
condition|(
operator|(
name|c
operator|=
name|fgetc
argument_list|(
name|fp
argument_list|)
operator|)
operator|!=
name|EOF
condition|)
block|{
if|if
condition|(
name|pos
operator|>=
name|len
operator|-
literal|1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"input line too long.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|c
condition|)
block|{
case|case
literal|'\r'
case|:
name|c
operator|=
name|fgetc
argument_list|(
name|fp
argument_list|)
expr_stmt|;
if|if
condition|(
name|c
operator|!=
name|EOF
operator|&&
name|c
operator|!=
literal|'\n'
operator|&&
name|ungetc
argument_list|(
name|c
argument_list|,
name|fp
argument_list|)
operator|==
name|EOF
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"unget: %s\n"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
return|return
name|pos
return|;
case|case
literal|'\n'
case|:
return|return
name|pos
return|;
block|}
name|line
index|[
name|pos
operator|++
index|]
operator|=
name|c
expr_stmt|;
name|line
index|[
name|pos
index|]
operator|=
literal|'\0'
expr_stmt|;
block|}
comment|/* We reached EOF */
return|return
operator|-
literal|1
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|do_convert_from_ssh2
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|)
block|{
name|Key
modifier|*
name|k
decl_stmt|;
name|int
name|blen
decl_stmt|;
name|u_int
name|len
decl_stmt|;
name|char
name|line
index|[
literal|1024
index|]
decl_stmt|;
name|u_char
name|blob
index|[
literal|8096
index|]
decl_stmt|;
name|char
name|encoded
index|[
literal|8096
index|]
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|int
name|escaped
init|=
literal|0
decl_stmt|,
name|private
init|=
literal|0
decl_stmt|,
name|ok
decl_stmt|;
name|FILE
modifier|*
name|fp
decl_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|fp
operator|=
name|fopen
argument_list|(
name|identity_file
argument_list|,
literal|"r"
argument_list|)
expr_stmt|;
if|if
condition|(
name|fp
operator|==
name|NULL
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|encoded
index|[
literal|0
index|]
operator|=
literal|'\0'
expr_stmt|;
while|while
condition|(
operator|(
name|blen
operator|=
name|get_line
argument_list|(
name|fp
argument_list|,
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|)
operator|)
operator|!=
operator|-
literal|1
condition|)
block|{
if|if
condition|(
name|line
index|[
name|blen
operator|-
literal|1
index|]
operator|==
literal|'\\'
condition|)
name|escaped
operator|++
expr_stmt|;
if|if
condition|(
name|strncmp
argument_list|(
name|line
argument_list|,
literal|"----"
argument_list|,
literal|4
argument_list|)
operator|==
literal|0
operator|||
name|strstr
argument_list|(
name|line
argument_list|,
literal|": "
argument_list|)
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
name|strstr
argument_list|(
name|line
argument_list|,
name|SSH_COM_PRIVATE_BEGIN
argument_list|)
operator|!=
name|NULL
condition|)
name|private
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|strstr
argument_list|(
name|line
argument_list|,
literal|" END "
argument_list|)
operator|!=
name|NULL
condition|)
block|{
break|break;
block|}
comment|/* fprintf(stderr, "ignore: %s", line); */
continue|continue;
block|}
if|if
condition|(
name|escaped
condition|)
block|{
name|escaped
operator|--
expr_stmt|;
comment|/* fprintf(stderr, "escaped: %s", line); */
continue|continue;
block|}
name|strlcat
argument_list|(
name|encoded
argument_list|,
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|encoded
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|len
operator|=
name|strlen
argument_list|(
name|encoded
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
operator|(
name|len
operator|%
literal|4
operator|)
operator|==
literal|3
operator|)
operator|&&
operator|(
name|encoded
index|[
name|len
operator|-
literal|1
index|]
operator|==
literal|'='
operator|)
operator|&&
operator|(
name|encoded
index|[
name|len
operator|-
literal|2
index|]
operator|==
literal|'='
operator|)
operator|&&
operator|(
name|encoded
index|[
name|len
operator|-
literal|3
index|]
operator|==
literal|'='
operator|)
condition|)
name|encoded
index|[
name|len
operator|-
literal|3
index|]
operator|=
literal|'\0'
expr_stmt|;
name|blen
operator|=
name|uudecode
argument_list|(
name|encoded
argument_list|,
name|blob
argument_list|,
sizeof|sizeof
argument_list|(
name|blob
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|blen
operator|<
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"uudecode failed.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|k
operator|=
name|private
condition|?
name|do_convert_private_ssh2_from_blob
argument_list|(
name|blob
argument_list|,
name|blen
argument_list|)
else|:
name|key_from_blob
argument_list|(
name|blob
argument_list|,
name|blen
argument_list|)
expr_stmt|;
if|if
condition|(
name|k
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"decode blob failed.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|ok
operator|=
name|private
condition|?
operator|(
name|k
operator|->
name|type
operator|==
name|KEY_DSA
condition|?
name|PEM_write_DSAPrivateKey
argument_list|(
name|stdout
argument_list|,
name|k
operator|->
name|dsa
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|)
else|:
name|PEM_write_RSAPrivateKey
argument_list|(
name|stdout
argument_list|,
name|k
operator|->
name|rsa
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|)
operator|)
else|:
name|key_write
argument_list|(
name|k
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|ok
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"key write failed\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|key_free
argument_list|(
name|k
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|private
condition|)
name|fprintf
argument_list|(
name|stdout
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|fp
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|do_print_public
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|)
block|{
name|Key
modifier|*
name|prv
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|prv
operator|=
name|load_identity
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
if|if
condition|(
name|prv
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"load failed\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|key_write
argument_list|(
name|prv
argument_list|,
name|stdout
argument_list|)
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"key_write failed"
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|prv
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stdout
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|do_download
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|,
name|char
modifier|*
name|pkcs11provider
parameter_list|)
block|{
ifdef|#
directive|ifdef
name|ENABLE_PKCS11
name|Key
modifier|*
modifier|*
name|keys
init|=
name|NULL
decl_stmt|;
name|int
name|i
decl_stmt|,
name|nkeys
decl_stmt|;
name|pkcs11_init
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|nkeys
operator|=
name|pkcs11_add_provider
argument_list|(
name|pkcs11provider
argument_list|,
name|NULL
argument_list|,
operator|&
name|keys
argument_list|)
expr_stmt|;
if|if
condition|(
name|nkeys
operator|<=
literal|0
condition|)
name|fatal
argument_list|(
literal|"cannot read public key from pkcs11"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|nkeys
condition|;
name|i
operator|++
control|)
block|{
name|key_write
argument_list|(
name|keys
index|[
name|i
index|]
argument_list|,
name|stdout
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|keys
index|[
name|i
index|]
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stdout
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
name|xfree
argument_list|(
name|keys
argument_list|)
expr_stmt|;
name|pkcs11_terminate
argument_list|()
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
else|#
directive|else
name|fatal
argument_list|(
literal|"no pkcs11 support"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* ENABLE_PKCS11 */
block|}
end_function

begin_function
specifier|static
name|void
name|do_fingerprint
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|)
block|{
name|FILE
modifier|*
name|f
decl_stmt|;
name|Key
modifier|*
name|public
decl_stmt|;
name|char
modifier|*
name|comment
init|=
name|NULL
decl_stmt|,
modifier|*
name|cp
decl_stmt|,
modifier|*
name|ep
decl_stmt|,
name|line
index|[
literal|16
operator|*
literal|1024
index|]
decl_stmt|,
modifier|*
name|fp
decl_stmt|,
modifier|*
name|ra
decl_stmt|;
name|int
name|i
decl_stmt|,
name|skip
init|=
literal|0
decl_stmt|,
name|num
init|=
literal|0
decl_stmt|,
name|invalid
init|=
literal|1
decl_stmt|;
name|enum
name|fp_rep
name|rep
decl_stmt|;
name|enum
name|fp_type
name|fptype
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|fptype
operator|=
name|print_bubblebabble
condition|?
name|SSH_FP_SHA1
else|:
name|SSH_FP_MD5
expr_stmt|;
name|rep
operator|=
name|print_bubblebabble
condition|?
name|SSH_FP_BUBBLEBABBLE
else|:
name|SSH_FP_HEX
expr_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|public
operator|=
name|key_load_public
argument_list|(
name|identity_file
argument_list|,
operator|&
name|comment
argument_list|)
expr_stmt|;
if|if
condition|(
name|public
operator|!=
name|NULL
condition|)
block|{
name|fp
operator|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|fptype
argument_list|,
name|rep
argument_list|)
expr_stmt|;
name|ra
operator|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|SSH_FP_MD5
argument_list|,
name|SSH_FP_RANDOMART
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u %s %s (%s)\n"
argument_list|,
name|key_size
argument_list|(
name|public
argument_list|)
argument_list|,
name|fp
argument_list|,
name|comment
argument_list|,
name|key_type
argument_list|(
name|public
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|log_level
operator|>=
name|SYSLOG_LEVEL_VERBOSE
condition|)
name|printf
argument_list|(
literal|"%s\n"
argument_list|,
name|ra
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|ra
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|fp
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|comment
condition|)
block|{
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
name|comment
operator|=
name|NULL
expr_stmt|;
block|}
name|f
operator|=
name|fopen
argument_list|(
name|identity_file
argument_list|,
literal|"r"
argument_list|)
expr_stmt|;
if|if
condition|(
name|f
operator|!=
name|NULL
condition|)
block|{
while|while
condition|(
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|f
argument_list|)
condition|)
block|{
if|if
condition|(
operator|(
name|cp
operator|=
name|strchr
argument_list|(
name|line
argument_list|,
literal|'\n'
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|error
argument_list|(
literal|"line %d too long: %.40s..."
argument_list|,
name|num
operator|+
literal|1
argument_list|,
name|line
argument_list|)
expr_stmt|;
name|skip
operator|=
literal|1
expr_stmt|;
continue|continue;
block|}
name|num
operator|++
expr_stmt|;
if|if
condition|(
name|skip
condition|)
block|{
name|skip
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
operator|*
name|cp
operator|=
literal|'\0'
expr_stmt|;
comment|/* Skip leading whitespace, empty and comment lines. */
for|for
control|(
name|cp
operator|=
name|line
init|;
operator|*
name|cp
operator|==
literal|' '
operator|||
operator|*
name|cp
operator|==
literal|'\t'
condition|;
name|cp
operator|++
control|)
empty_stmt|;
if|if
condition|(
operator|!
operator|*
name|cp
operator|||
operator|*
name|cp
operator|==
literal|'\n'
operator|||
operator|*
name|cp
operator|==
literal|'#'
condition|)
continue|continue;
name|i
operator|=
name|strtol
argument_list|(
name|cp
argument_list|,
operator|&
name|ep
argument_list|,
literal|10
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|==
literal|0
operator|||
name|ep
operator|==
name|NULL
operator|||
operator|(
operator|*
name|ep
operator|!=
literal|' '
operator|&&
operator|*
name|ep
operator|!=
literal|'\t'
operator|)
condition|)
block|{
name|int
name|quoted
init|=
literal|0
decl_stmt|;
name|comment
operator|=
name|cp
expr_stmt|;
for|for
control|(
init|;
operator|*
name|cp
operator|&&
operator|(
name|quoted
operator|||
operator|(
operator|*
name|cp
operator|!=
literal|' '
operator|&&
operator|*
name|cp
operator|!=
literal|'\t'
operator|)
operator|)
condition|;
name|cp
operator|++
control|)
block|{
if|if
condition|(
operator|*
name|cp
operator|==
literal|'\\'
operator|&&
name|cp
index|[
literal|1
index|]
operator|==
literal|'"'
condition|)
name|cp
operator|++
expr_stmt|;
comment|/* Skip both */
elseif|else
if|if
condition|(
operator|*
name|cp
operator|==
literal|'"'
condition|)
name|quoted
operator|=
operator|!
name|quoted
expr_stmt|;
block|}
if|if
condition|(
operator|!
operator|*
name|cp
condition|)
continue|continue;
operator|*
name|cp
operator|++
operator|=
literal|'\0'
expr_stmt|;
block|}
name|ep
operator|=
name|cp
expr_stmt|;
name|public
operator|=
name|key_new
argument_list|(
name|KEY_RSA1
argument_list|)
expr_stmt|;
if|if
condition|(
name|key_read
argument_list|(
name|public
argument_list|,
operator|&
name|cp
argument_list|)
operator|!=
literal|1
condition|)
block|{
name|cp
operator|=
name|ep
expr_stmt|;
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
name|public
operator|=
name|key_new
argument_list|(
name|KEY_UNSPEC
argument_list|)
expr_stmt|;
if|if
condition|(
name|key_read
argument_list|(
name|public
argument_list|,
operator|&
name|cp
argument_list|)
operator|!=
literal|1
condition|)
block|{
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
continue|continue;
block|}
block|}
name|comment
operator|=
operator|*
name|cp
condition|?
name|cp
else|:
name|comment
expr_stmt|;
name|fp
operator|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|fptype
argument_list|,
name|rep
argument_list|)
expr_stmt|;
name|ra
operator|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|SSH_FP_MD5
argument_list|,
name|SSH_FP_RANDOMART
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u %s %s (%s)\n"
argument_list|,
name|key_size
argument_list|(
name|public
argument_list|)
argument_list|,
name|fp
argument_list|,
name|comment
condition|?
name|comment
else|:
literal|"no comment"
argument_list|,
name|key_type
argument_list|(
name|public
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|log_level
operator|>=
name|SYSLOG_LEVEL_VERBOSE
condition|)
name|printf
argument_list|(
literal|"%s\n"
argument_list|,
name|ra
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|ra
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|fp
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
name|invalid
operator|=
literal|0
expr_stmt|;
block|}
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|invalid
condition|)
block|{
name|printf
argument_list|(
literal|"%s is not a public key file.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|printhost
parameter_list|(
name|FILE
modifier|*
name|f
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|,
name|Key
modifier|*
name|public
parameter_list|,
name|int
name|ca
parameter_list|,
name|int
name|hash
parameter_list|)
block|{
if|if
condition|(
name|print_fingerprint
condition|)
block|{
name|enum
name|fp_rep
name|rep
decl_stmt|;
name|enum
name|fp_type
name|fptype
decl_stmt|;
name|char
modifier|*
name|fp
decl_stmt|,
modifier|*
name|ra
decl_stmt|;
name|fptype
operator|=
name|print_bubblebabble
condition|?
name|SSH_FP_SHA1
else|:
name|SSH_FP_MD5
expr_stmt|;
name|rep
operator|=
name|print_bubblebabble
condition|?
name|SSH_FP_BUBBLEBABBLE
else|:
name|SSH_FP_HEX
expr_stmt|;
name|fp
operator|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|fptype
argument_list|,
name|rep
argument_list|)
expr_stmt|;
name|ra
operator|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|SSH_FP_MD5
argument_list|,
name|SSH_FP_RANDOMART
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%u %s %s (%s)\n"
argument_list|,
name|key_size
argument_list|(
name|public
argument_list|)
argument_list|,
name|fp
argument_list|,
name|name
argument_list|,
name|key_type
argument_list|(
name|public
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|log_level
operator|>=
name|SYSLOG_LEVEL_VERBOSE
condition|)
name|printf
argument_list|(
literal|"%s\n"
argument_list|,
name|ra
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|ra
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|hash
operator|&&
operator|(
name|name
operator|=
name|host_hash
argument_list|(
name|name
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"hash_host failed"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|f
argument_list|,
literal|"%s%s%s "
argument_list|,
name|ca
condition|?
name|CA_MARKER
else|:
literal|""
argument_list|,
name|ca
condition|?
literal|" "
else|:
literal|""
argument_list|,
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|key_write
argument_list|(
name|public
argument_list|,
name|f
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"key_write failed"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|f
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_function
specifier|static
name|void
name|do_known_hosts
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
name|FILE
modifier|*
name|in
decl_stmt|,
modifier|*
name|out
init|=
name|stdout
decl_stmt|;
name|Key
modifier|*
name|pub
decl_stmt|;
name|char
modifier|*
name|cp
decl_stmt|,
modifier|*
name|cp2
decl_stmt|,
modifier|*
name|kp
decl_stmt|,
modifier|*
name|kp2
decl_stmt|;
name|char
name|line
index|[
literal|16
operator|*
literal|1024
index|]
decl_stmt|,
name|tmp
index|[
name|MAXPATHLEN
index|]
decl_stmt|,
name|old
index|[
name|MAXPATHLEN
index|]
decl_stmt|;
name|int
name|c
decl_stmt|,
name|skip
init|=
literal|0
decl_stmt|,
name|inplace
init|=
literal|0
decl_stmt|,
name|num
init|=
literal|0
decl_stmt|,
name|invalid
init|=
literal|0
decl_stmt|,
name|has_unhashed
init|=
literal|0
decl_stmt|;
name|int
name|ca
decl_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
block|{
name|cp
operator|=
name|tilde_expand_filename
argument_list|(
name|_PATH_SSH_USER_HOSTFILE
argument_list|,
name|pw
operator|->
name|pw_uid
argument_list|)
expr_stmt|;
if|if
condition|(
name|strlcpy
argument_list|(
name|identity_file
argument_list|,
name|cp
argument_list|,
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"Specified known hosts path too long"
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|cp
argument_list|)
expr_stmt|;
name|have_identity
operator|=
literal|1
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|in
operator|=
name|fopen
argument_list|(
name|identity_file
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"fopen: %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 	 * Find hosts goes to stdout, hash and deletions happen in-place 	 * A corner case is ssh-keygen -HF foo, which should go to stdout 	 */
if|if
condition|(
operator|!
name|find_host
operator|&&
operator|(
name|hash_hosts
operator|||
name|delete_host
operator|)
condition|)
block|{
if|if
condition|(
name|strlcpy
argument_list|(
name|tmp
argument_list|,
name|identity_file
argument_list|,
sizeof|sizeof
argument_list|(
name|tmp
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|tmp
argument_list|)
operator|||
name|strlcat
argument_list|(
name|tmp
argument_list|,
literal|".XXXXXXXXXX"
argument_list|,
sizeof|sizeof
argument_list|(
name|tmp
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|tmp
argument_list|)
operator|||
name|strlcpy
argument_list|(
name|old
argument_list|,
name|identity_file
argument_list|,
sizeof|sizeof
argument_list|(
name|old
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|old
argument_list|)
operator|||
name|strlcat
argument_list|(
name|old
argument_list|,
literal|".old"
argument_list|,
sizeof|sizeof
argument_list|(
name|old
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|old
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"known_hosts path too long"
argument_list|)
expr_stmt|;
name|umask
argument_list|(
literal|077
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|c
operator|=
name|mkstemp
argument_list|(
name|tmp
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"mkstemp: %s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|out
operator|=
name|fdopen
argument_list|(
name|c
argument_list|,
literal|"w"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|c
operator|=
name|errno
expr_stmt|;
name|unlink
argument_list|(
name|tmp
argument_list|)
expr_stmt|;
name|fatal
argument_list|(
literal|"fdopen: %s"
argument_list|,
name|strerror
argument_list|(
name|c
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|inplace
operator|=
literal|1
expr_stmt|;
block|}
while|while
condition|(
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|in
argument_list|)
condition|)
block|{
if|if
condition|(
operator|(
name|cp
operator|=
name|strchr
argument_list|(
name|line
argument_list|,
literal|'\n'
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|error
argument_list|(
literal|"line %d too long: %.40s..."
argument_list|,
name|num
operator|+
literal|1
argument_list|,
name|line
argument_list|)
expr_stmt|;
name|skip
operator|=
literal|1
expr_stmt|;
name|invalid
operator|=
literal|1
expr_stmt|;
continue|continue;
block|}
name|num
operator|++
expr_stmt|;
if|if
condition|(
name|skip
condition|)
block|{
name|skip
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
operator|*
name|cp
operator|=
literal|'\0'
expr_stmt|;
comment|/* Skip leading whitespace, empty and comment lines. */
for|for
control|(
name|cp
operator|=
name|line
init|;
operator|*
name|cp
operator|==
literal|' '
operator|||
operator|*
name|cp
operator|==
literal|'\t'
condition|;
name|cp
operator|++
control|)
empty_stmt|;
if|if
condition|(
operator|!
operator|*
name|cp
operator|||
operator|*
name|cp
operator|==
literal|'\n'
operator|||
operator|*
name|cp
operator|==
literal|'#'
condition|)
block|{
if|if
condition|(
name|inplace
condition|)
name|fprintf
argument_list|(
name|out
argument_list|,
literal|"%s\n"
argument_list|,
name|cp
argument_list|)
expr_stmt|;
continue|continue;
block|}
comment|/* Check whether this is a CA key */
if|if
condition|(
name|strncasecmp
argument_list|(
name|cp
argument_list|,
name|CA_MARKER
argument_list|,
sizeof|sizeof
argument_list|(
name|CA_MARKER
argument_list|)
operator|-
literal|1
argument_list|)
operator|==
literal|0
operator|&&
operator|(
name|cp
index|[
sizeof|sizeof
argument_list|(
name|CA_MARKER
argument_list|)
operator|-
literal|1
index|]
operator|==
literal|' '
operator|||
name|cp
index|[
sizeof|sizeof
argument_list|(
name|CA_MARKER
argument_list|)
operator|-
literal|1
index|]
operator|==
literal|'\t'
operator|)
condition|)
block|{
name|ca
operator|=
literal|1
expr_stmt|;
name|cp
operator|+=
sizeof|sizeof
argument_list|(
name|CA_MARKER
argument_list|)
expr_stmt|;
block|}
else|else
name|ca
operator|=
literal|0
expr_stmt|;
comment|/* Find the end of the host name portion. */
for|for
control|(
name|kp
operator|=
name|cp
init|;
operator|*
name|kp
operator|&&
operator|*
name|kp
operator|!=
literal|' '
operator|&&
operator|*
name|kp
operator|!=
literal|'\t'
condition|;
name|kp
operator|++
control|)
empty_stmt|;
if|if
condition|(
operator|*
name|kp
operator|==
literal|'\0'
operator|||
operator|*
operator|(
name|kp
operator|+
literal|1
operator|)
operator|==
literal|'\0'
condition|)
block|{
name|error
argument_list|(
literal|"line %d missing key: %.40s..."
argument_list|,
name|num
argument_list|,
name|line
argument_list|)
expr_stmt|;
name|invalid
operator|=
literal|1
expr_stmt|;
continue|continue;
block|}
operator|*
name|kp
operator|++
operator|=
literal|'\0'
expr_stmt|;
name|kp2
operator|=
name|kp
expr_stmt|;
name|pub
operator|=
name|key_new
argument_list|(
name|KEY_RSA1
argument_list|)
expr_stmt|;
if|if
condition|(
name|key_read
argument_list|(
name|pub
argument_list|,
operator|&
name|kp
argument_list|)
operator|!=
literal|1
condition|)
block|{
name|kp
operator|=
name|kp2
expr_stmt|;
name|key_free
argument_list|(
name|pub
argument_list|)
expr_stmt|;
name|pub
operator|=
name|key_new
argument_list|(
name|KEY_UNSPEC
argument_list|)
expr_stmt|;
if|if
condition|(
name|key_read
argument_list|(
name|pub
argument_list|,
operator|&
name|kp
argument_list|)
operator|!=
literal|1
condition|)
block|{
name|error
argument_list|(
literal|"line %d invalid key: %.40s..."
argument_list|,
name|num
argument_list|,
name|line
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|pub
argument_list|)
expr_stmt|;
name|invalid
operator|=
literal|1
expr_stmt|;
continue|continue;
block|}
block|}
if|if
condition|(
operator|*
name|cp
operator|==
name|HASH_DELIM
condition|)
block|{
if|if
condition|(
name|find_host
operator|||
name|delete_host
condition|)
block|{
name|cp2
operator|=
name|host_hash
argument_list|(
name|name
argument_list|,
name|cp
argument_list|,
name|strlen
argument_list|(
name|cp
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|cp2
operator|==
name|NULL
condition|)
block|{
name|error
argument_list|(
literal|"line %d: invalid hashed "
literal|"name: %.64s..."
argument_list|,
name|num
argument_list|,
name|line
argument_list|)
expr_stmt|;
name|invalid
operator|=
literal|1
expr_stmt|;
continue|continue;
block|}
name|c
operator|=
operator|(
name|strcmp
argument_list|(
name|cp2
argument_list|,
name|cp
argument_list|)
operator|==
literal|0
operator|)
expr_stmt|;
if|if
condition|(
name|find_host
operator|&&
name|c
condition|)
block|{
name|printf
argument_list|(
literal|"# Host %s found: "
literal|"line %d type %s%s\n"
argument_list|,
name|name
argument_list|,
name|num
argument_list|,
name|key_type
argument_list|(
name|pub
argument_list|)
argument_list|,
name|ca
condition|?
literal|" (CA key)"
else|:
literal|""
argument_list|)
expr_stmt|;
name|printhost
argument_list|(
name|out
argument_list|,
name|cp
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|delete_host
operator|&&
operator|!
name|c
operator|&&
operator|!
name|ca
condition|)
name|printhost
argument_list|(
name|out
argument_list|,
name|cp
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|hash_hosts
condition|)
name|printhost
argument_list|(
name|out
argument_list|,
name|cp
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|find_host
operator|||
name|delete_host
condition|)
block|{
name|c
operator|=
operator|(
name|match_hostname
argument_list|(
name|name
argument_list|,
name|cp
argument_list|,
name|strlen
argument_list|(
name|cp
argument_list|)
argument_list|)
operator|==
literal|1
operator|)
expr_stmt|;
if|if
condition|(
name|find_host
operator|&&
name|c
condition|)
block|{
name|printf
argument_list|(
literal|"# Host %s found: "
literal|"line %d type %s%s\n"
argument_list|,
name|name
argument_list|,
name|num
argument_list|,
name|key_type
argument_list|(
name|pub
argument_list|)
argument_list|,
name|ca
condition|?
literal|" (CA key)"
else|:
literal|""
argument_list|)
expr_stmt|;
name|printhost
argument_list|(
name|out
argument_list|,
name|name
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
name|hash_hosts
operator|&&
operator|!
name|ca
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|delete_host
operator|&&
operator|!
name|c
operator|&&
operator|!
name|ca
condition|)
name|printhost
argument_list|(
name|out
argument_list|,
name|cp
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|hash_hosts
condition|)
block|{
for|for
control|(
name|cp2
operator|=
name|strsep
argument_list|(
operator|&
name|cp
argument_list|,
literal|","
argument_list|)
init|;
name|cp2
operator|!=
name|NULL
operator|&&
operator|*
name|cp2
operator|!=
literal|'\0'
condition|;
name|cp2
operator|=
name|strsep
argument_list|(
operator|&
name|cp
argument_list|,
literal|","
argument_list|)
control|)
block|{
if|if
condition|(
name|ca
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Warning: "
literal|"ignoring CA key for host: "
literal|"%.64s\n"
argument_list|,
name|cp2
argument_list|)
expr_stmt|;
name|printhost
argument_list|(
name|out
argument_list|,
name|cp2
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|strcspn
argument_list|(
name|cp2
argument_list|,
literal|"*?!"
argument_list|)
operator|!=
name|strlen
argument_list|(
name|cp2
argument_list|)
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Warning: "
literal|"ignoring host name with "
literal|"metacharacters: %.64s\n"
argument_list|,
name|cp2
argument_list|)
expr_stmt|;
name|printhost
argument_list|(
name|out
argument_list|,
name|cp2
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
name|printhost
argument_list|(
name|out
argument_list|,
name|cp2
argument_list|,
name|pub
argument_list|,
name|ca
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
name|has_unhashed
operator|=
literal|1
expr_stmt|;
block|}
block|}
name|key_free
argument_list|(
name|pub
argument_list|)
expr_stmt|;
block|}
name|fclose
argument_list|(
name|in
argument_list|)
expr_stmt|;
if|if
condition|(
name|invalid
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s is not a valid known_hosts file.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
if|if
condition|(
name|inplace
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Not replacing existing known_hosts "
literal|"file because of errors\n"
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|out
argument_list|)
expr_stmt|;
name|unlink
argument_list|(
name|tmp
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|inplace
condition|)
block|{
name|fclose
argument_list|(
name|out
argument_list|)
expr_stmt|;
comment|/* Backup existing file */
if|if
condition|(
name|unlink
argument_list|(
name|old
argument_list|)
operator|==
operator|-
literal|1
operator|&&
name|errno
operator|!=
name|ENOENT
condition|)
name|fatal
argument_list|(
literal|"unlink %.100s: %s"
argument_list|,
name|old
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|link
argument_list|(
name|identity_file
argument_list|,
name|old
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"link %.100s to %.100s: %s"
argument_list|,
name|identity_file
argument_list|,
name|old
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Move new one into place */
if|if
condition|(
name|rename
argument_list|(
name|tmp
argument_list|,
name|identity_file
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|error
argument_list|(
literal|"rename\"%s\" to \"%s\": %s"
argument_list|,
name|tmp
argument_list|,
name|identity_file
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
name|unlink
argument_list|(
name|tmp
argument_list|)
expr_stmt|;
name|unlink
argument_list|(
name|old
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s updated.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Original contents retained as %s\n"
argument_list|,
name|old
argument_list|)
expr_stmt|;
if|if
condition|(
name|has_unhashed
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"WARNING: %s contains unhashed "
literal|"entries\n"
argument_list|,
name|old
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Delete this file to ensure privacy "
literal|"of hostnames\n"
argument_list|)
expr_stmt|;
block|}
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Perform changing a passphrase.  The argument is the passwd structure  * for the current user.  */
end_comment

begin_function
specifier|static
name|void
name|do_change_passphrase
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|)
block|{
name|char
modifier|*
name|comment
decl_stmt|;
name|char
modifier|*
name|old_passphrase
decl_stmt|,
modifier|*
name|passphrase1
decl_stmt|,
modifier|*
name|passphrase2
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|Key
modifier|*
name|private
decl_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Try to load the file with empty passphrase. */
name|private
operator|=
name|key_load_private
argument_list|(
name|identity_file
argument_list|,
literal|""
argument_list|,
operator|&
name|comment
argument_list|)
expr_stmt|;
if|if
condition|(
name|private
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
name|identity_passphrase
condition|)
name|old_passphrase
operator|=
name|xstrdup
argument_list|(
name|identity_passphrase
argument_list|)
expr_stmt|;
else|else
name|old_passphrase
operator|=
name|read_passphrase
argument_list|(
literal|"Enter old passphrase: "
argument_list|,
name|RP_ALLOW_STDIN
argument_list|)
expr_stmt|;
name|private
operator|=
name|key_load_private
argument_list|(
name|identity_file
argument_list|,
name|old_passphrase
argument_list|,
operator|&
name|comment
argument_list|)
expr_stmt|;
name|memset
argument_list|(
name|old_passphrase
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|old_passphrase
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|old_passphrase
argument_list|)
expr_stmt|;
if|if
condition|(
name|private
operator|==
name|NULL
condition|)
block|{
name|printf
argument_list|(
literal|"Bad passphrase.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
name|printf
argument_list|(
literal|"Key has comment '%s'\n"
argument_list|,
name|comment
argument_list|)
expr_stmt|;
comment|/* Ask the new passphrase (twice). */
if|if
condition|(
name|identity_new_passphrase
condition|)
block|{
name|passphrase1
operator|=
name|xstrdup
argument_list|(
name|identity_new_passphrase
argument_list|)
expr_stmt|;
name|passphrase2
operator|=
name|NULL
expr_stmt|;
block|}
else|else
block|{
name|passphrase1
operator|=
name|read_passphrase
argument_list|(
literal|"Enter new passphrase (empty for no "
literal|"passphrase): "
argument_list|,
name|RP_ALLOW_STDIN
argument_list|)
expr_stmt|;
name|passphrase2
operator|=
name|read_passphrase
argument_list|(
literal|"Enter same passphrase again: "
argument_list|,
name|RP_ALLOW_STDIN
argument_list|)
expr_stmt|;
comment|/* Verify that they are the same. */
if|if
condition|(
name|strcmp
argument_list|(
name|passphrase1
argument_list|,
name|passphrase2
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|memset
argument_list|(
name|passphrase1
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase1
argument_list|)
argument_list|)
expr_stmt|;
name|memset
argument_list|(
name|passphrase2
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase2
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase1
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase2
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"Pass phrases do not match.  Try again.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Destroy the other copy. */
name|memset
argument_list|(
name|passphrase2
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase2
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase2
argument_list|)
expr_stmt|;
block|}
comment|/* Save the file using the new passphrase. */
if|if
condition|(
operator|!
name|key_save_private
argument_list|(
name|private
argument_list|,
name|identity_file
argument_list|,
name|passphrase1
argument_list|,
name|comment
argument_list|)
condition|)
block|{
name|printf
argument_list|(
literal|"Saving the key failed: %s.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|memset
argument_list|(
name|passphrase1
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase1
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase1
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|private
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Destroy the passphrase and the copy of the key in memory. */
name|memset
argument_list|(
name|passphrase1
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase1
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase1
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|private
argument_list|)
expr_stmt|;
comment|/* Destroys contents */
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"Your identification has been saved with the new passphrase.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Print the SSHFP RR.  */
end_comment

begin_function
specifier|static
name|int
name|do_print_resource_record
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|,
name|char
modifier|*
name|fname
parameter_list|,
name|char
modifier|*
name|hname
parameter_list|)
block|{
name|Key
modifier|*
name|public
decl_stmt|;
name|char
modifier|*
name|comment
init|=
name|NULL
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
if|if
condition|(
name|fname
operator|==
name|NULL
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|fname
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|errno
operator|==
name|ENOENT
condition|)
return|return
literal|0
return|;
name|perror
argument_list|(
name|fname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|public
operator|=
name|key_load_public
argument_list|(
name|fname
argument_list|,
operator|&
name|comment
argument_list|)
expr_stmt|;
if|if
condition|(
name|public
operator|!=
name|NULL
condition|)
block|{
name|export_dns_rr
argument_list|(
name|hname
argument_list|,
name|public
argument_list|,
name|stdout
argument_list|,
name|print_generic
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
if|if
condition|(
name|comment
condition|)
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"failed to read v2 public key from %s.\n"
argument_list|,
name|fname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Change the comment of a private key file.  */
end_comment

begin_function
specifier|static
name|void
name|do_change_comment
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|)
block|{
name|char
name|new_comment
index|[
literal|1024
index|]
decl_stmt|,
modifier|*
name|comment
decl_stmt|,
modifier|*
name|passphrase
decl_stmt|;
name|Key
modifier|*
name|private
decl_stmt|;
name|Key
modifier|*
name|public
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|FILE
modifier|*
name|f
decl_stmt|;
name|int
name|fd
decl_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|private
operator|=
name|key_load_private
argument_list|(
name|identity_file
argument_list|,
literal|""
argument_list|,
operator|&
name|comment
argument_list|)
expr_stmt|;
if|if
condition|(
name|private
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
name|identity_passphrase
condition|)
name|passphrase
operator|=
name|xstrdup
argument_list|(
name|identity_passphrase
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|identity_new_passphrase
condition|)
name|passphrase
operator|=
name|xstrdup
argument_list|(
name|identity_new_passphrase
argument_list|)
expr_stmt|;
else|else
name|passphrase
operator|=
name|read_passphrase
argument_list|(
literal|"Enter passphrase: "
argument_list|,
name|RP_ALLOW_STDIN
argument_list|)
expr_stmt|;
comment|/* Try to load using the passphrase. */
name|private
operator|=
name|key_load_private
argument_list|(
name|identity_file
argument_list|,
name|passphrase
argument_list|,
operator|&
name|comment
argument_list|)
expr_stmt|;
if|if
condition|(
name|private
operator|==
name|NULL
condition|)
block|{
name|memset
argument_list|(
name|passphrase
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"Bad passphrase.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|passphrase
operator|=
name|xstrdup
argument_list|(
literal|""
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|private
operator|->
name|type
operator|!=
name|KEY_RSA1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Comments are only supported for RSA1 keys.\n"
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|private
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"Key now has comment '%s'\n"
argument_list|,
name|comment
argument_list|)
expr_stmt|;
if|if
condition|(
name|identity_comment
condition|)
block|{
name|strlcpy
argument_list|(
name|new_comment
argument_list|,
name|identity_comment
argument_list|,
sizeof|sizeof
argument_list|(
name|new_comment
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|printf
argument_list|(
literal|"Enter new comment: "
argument_list|)
expr_stmt|;
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|fgets
argument_list|(
name|new_comment
argument_list|,
sizeof|sizeof
argument_list|(
name|new_comment
argument_list|)
argument_list|,
name|stdin
argument_list|)
condition|)
block|{
name|memset
argument_list|(
name|passphrase
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase
argument_list|)
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|private
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|new_comment
index|[
name|strcspn
argument_list|(
name|new_comment
argument_list|,
literal|"\n"
argument_list|)
index|]
operator|=
literal|'\0'
expr_stmt|;
block|}
comment|/* Save the file using the new passphrase. */
if|if
condition|(
operator|!
name|key_save_private
argument_list|(
name|private
argument_list|,
name|identity_file
argument_list|,
name|passphrase
argument_list|,
name|new_comment
argument_list|)
condition|)
block|{
name|printf
argument_list|(
literal|"Saving the key failed: %s.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|memset
argument_list|(
name|passphrase
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|private
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|memset
argument_list|(
name|passphrase
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase
argument_list|)
expr_stmt|;
name|public
operator|=
name|key_from_private
argument_list|(
name|private
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|private
argument_list|)
expr_stmt|;
name|strlcat
argument_list|(
name|identity_file
argument_list|,
literal|".pub"
argument_list|,
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
argument_list|)
expr_stmt|;
name|fd
operator|=
name|open
argument_list|(
name|identity_file
argument_list|,
name|O_WRONLY
operator||
name|O_CREAT
operator||
name|O_TRUNC
argument_list|,
literal|0644
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|==
operator|-
literal|1
condition|)
block|{
name|printf
argument_list|(
literal|"Could not save your public key in %s\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|f
operator|=
name|fdopen
argument_list|(
name|fd
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
if|if
condition|(
name|f
operator|==
name|NULL
condition|)
block|{
name|printf
argument_list|(
literal|"fdopen %s failed\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|key_write
argument_list|(
name|public
argument_list|,
name|f
argument_list|)
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"write key failed\n"
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|f
argument_list|,
literal|" %s\n"
argument_list|,
name|new_comment
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|comment
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"The comment in your key file has been changed.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
specifier|const
name|char
modifier|*
name|fmt_validity
parameter_list|(
name|u_int64_t
name|valid_from
parameter_list|,
name|u_int64_t
name|valid_to
parameter_list|)
block|{
name|char
name|from
index|[
literal|32
index|]
decl_stmt|,
name|to
index|[
literal|32
index|]
decl_stmt|;
specifier|static
name|char
name|ret
index|[
literal|64
index|]
decl_stmt|;
name|time_t
name|tt
decl_stmt|;
name|struct
name|tm
modifier|*
name|tm
decl_stmt|;
operator|*
name|from
operator|=
operator|*
name|to
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|valid_from
operator|==
literal|0
operator|&&
name|valid_to
operator|==
literal|0xffffffffffffffffULL
condition|)
return|return
literal|"forever"
return|;
if|if
condition|(
name|valid_from
operator|!=
literal|0
condition|)
block|{
comment|/* XXX revisit INT_MAX in 2038 :) */
name|tt
operator|=
name|valid_from
operator|>
name|INT_MAX
condition|?
name|INT_MAX
else|:
name|valid_from
expr_stmt|;
name|tm
operator|=
name|localtime
argument_list|(
operator|&
name|tt
argument_list|)
expr_stmt|;
name|strftime
argument_list|(
name|from
argument_list|,
sizeof|sizeof
argument_list|(
name|from
argument_list|)
argument_list|,
literal|"%Y-%m-%dT%H:%M:%S"
argument_list|,
name|tm
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|valid_to
operator|!=
literal|0xffffffffffffffffULL
condition|)
block|{
comment|/* XXX revisit INT_MAX in 2038 :) */
name|tt
operator|=
name|valid_to
operator|>
name|INT_MAX
condition|?
name|INT_MAX
else|:
name|valid_to
expr_stmt|;
name|tm
operator|=
name|localtime
argument_list|(
operator|&
name|tt
argument_list|)
expr_stmt|;
name|strftime
argument_list|(
name|to
argument_list|,
sizeof|sizeof
argument_list|(
name|to
argument_list|)
argument_list|,
literal|"%Y-%m-%dT%H:%M:%S"
argument_list|,
name|tm
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|valid_from
operator|==
literal|0
condition|)
block|{
name|snprintf
argument_list|(
name|ret
argument_list|,
sizeof|sizeof
argument_list|(
name|ret
argument_list|)
argument_list|,
literal|"before %s"
argument_list|,
name|to
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
if|if
condition|(
name|valid_to
operator|==
literal|0xffffffffffffffffULL
condition|)
block|{
name|snprintf
argument_list|(
name|ret
argument_list|,
sizeof|sizeof
argument_list|(
name|ret
argument_list|)
argument_list|,
literal|"after %s"
argument_list|,
name|from
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
name|snprintf
argument_list|(
name|ret
argument_list|,
sizeof|sizeof
argument_list|(
name|ret
argument_list|)
argument_list|,
literal|"from %s to %s"
argument_list|,
name|from
argument_list|,
name|to
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|add_flag_constraint
parameter_list|(
name|Buffer
modifier|*
name|c
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
name|debug3
argument_list|(
literal|"%s: %s"
argument_list|,
name|__func__
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|buffer_put_cstring
argument_list|(
name|c
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|buffer_put_string
argument_list|(
name|c
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|add_string_constraint
parameter_list|(
name|Buffer
modifier|*
name|c
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|,
specifier|const
name|char
modifier|*
name|value
parameter_list|)
block|{
name|Buffer
name|b
decl_stmt|;
name|debug3
argument_list|(
literal|"%s: %s=%s"
argument_list|,
name|__func__
argument_list|,
name|name
argument_list|,
name|value
argument_list|)
expr_stmt|;
name|buffer_init
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
name|buffer_put_cstring
argument_list|(
operator|&
name|b
argument_list|,
name|value
argument_list|)
expr_stmt|;
name|buffer_put_cstring
argument_list|(
name|c
argument_list|,
name|name
argument_list|)
expr_stmt|;
name|buffer_put_string
argument_list|(
name|c
argument_list|,
name|buffer_ptr
argument_list|(
operator|&
name|b
argument_list|)
argument_list|,
name|buffer_len
argument_list|(
operator|&
name|b
argument_list|)
argument_list|)
expr_stmt|;
name|buffer_free
argument_list|(
operator|&
name|b
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|prepare_constraint_buf
parameter_list|(
name|Buffer
modifier|*
name|c
parameter_list|)
block|{
name|buffer_clear
argument_list|(
name|c
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|constraint_flags
operator|&
name|CONSTRAINT_X_FWD
operator|)
operator|!=
literal|0
condition|)
name|add_flag_constraint
argument_list|(
name|c
argument_list|,
literal|"permit-X11-forwarding"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|constraint_flags
operator|&
name|CONSTRAINT_AGENT_FWD
operator|)
operator|!=
literal|0
condition|)
name|add_flag_constraint
argument_list|(
name|c
argument_list|,
literal|"permit-agent-forwarding"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|constraint_flags
operator|&
name|CONSTRAINT_PORT_FWD
operator|)
operator|!=
literal|0
condition|)
name|add_flag_constraint
argument_list|(
name|c
argument_list|,
literal|"permit-port-forwarding"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|constraint_flags
operator|&
name|CONSTRAINT_PTY
operator|)
operator|!=
literal|0
condition|)
name|add_flag_constraint
argument_list|(
name|c
argument_list|,
literal|"permit-pty"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|constraint_flags
operator|&
name|CONSTRAINT_USER_RC
operator|)
operator|!=
literal|0
condition|)
name|add_flag_constraint
argument_list|(
name|c
argument_list|,
literal|"permit-user-rc"
argument_list|)
expr_stmt|;
if|if
condition|(
name|constraint_command
operator|!=
name|NULL
condition|)
name|add_string_constraint
argument_list|(
name|c
argument_list|,
literal|"force-command"
argument_list|,
name|constraint_command
argument_list|)
expr_stmt|;
if|if
condition|(
name|constraint_src_addr
operator|!=
name|NULL
condition|)
name|add_string_constraint
argument_list|(
name|c
argument_list|,
literal|"source-address"
argument_list|,
name|constraint_src_addr
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|do_ca_sign
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|,
name|int
name|argc
parameter_list|,
name|char
modifier|*
modifier|*
name|argv
parameter_list|)
block|{
name|int
name|i
decl_stmt|,
name|fd
decl_stmt|;
name|u_int
name|n
decl_stmt|;
name|Key
modifier|*
name|ca
decl_stmt|,
modifier|*
name|public
decl_stmt|;
name|char
modifier|*
name|otmp
decl_stmt|,
modifier|*
name|tmp
decl_stmt|,
modifier|*
name|cp
decl_stmt|,
modifier|*
name|out
decl_stmt|,
modifier|*
name|comment
decl_stmt|,
modifier|*
modifier|*
name|plist
init|=
name|NULL
decl_stmt|;
name|FILE
modifier|*
name|f
decl_stmt|;
name|tmp
operator|=
name|tilde_expand_filename
argument_list|(
name|ca_key_path
argument_list|,
name|pw
operator|->
name|pw_uid
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|ca
operator|=
name|load_identity
argument_list|(
name|tmp
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"Couldn't load CA key \"%s\""
argument_list|,
name|tmp
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|tmp
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|argc
condition|;
name|i
operator|++
control|)
block|{
comment|/* Split list of principals */
name|n
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|cert_principals
operator|!=
name|NULL
condition|)
block|{
name|otmp
operator|=
name|tmp
operator|=
name|xstrdup
argument_list|(
name|cert_principals
argument_list|)
expr_stmt|;
name|plist
operator|=
name|NULL
expr_stmt|;
for|for
control|(
init|;
operator|(
name|cp
operator|=
name|strsep
argument_list|(
operator|&
name|tmp
argument_list|,
literal|","
argument_list|)
operator|)
operator|!=
name|NULL
condition|;
name|n
operator|++
control|)
block|{
name|plist
operator|=
name|xrealloc
argument_list|(
name|plist
argument_list|,
name|n
operator|+
literal|1
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|plist
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
operator|(
name|plist
index|[
name|n
index|]
operator|=
name|xstrdup
argument_list|(
name|cp
argument_list|)
operator|)
operator|==
literal|'\0'
condition|)
name|fatal
argument_list|(
literal|"Empty principal name"
argument_list|)
expr_stmt|;
block|}
name|xfree
argument_list|(
name|otmp
argument_list|)
expr_stmt|;
block|}
name|tmp
operator|=
name|tilde_expand_filename
argument_list|(
name|argv
index|[
name|i
index|]
argument_list|,
name|pw
operator|->
name|pw_uid
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|public
operator|=
name|key_load_public
argument_list|(
name|tmp
argument_list|,
operator|&
name|comment
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"%s: unable to open \"%s\""
argument_list|,
name|__func__
argument_list|,
name|tmp
argument_list|)
expr_stmt|;
if|if
condition|(
name|public
operator|->
name|type
operator|!=
name|KEY_RSA
operator|&&
name|public
operator|->
name|type
operator|!=
name|KEY_DSA
condition|)
name|fatal
argument_list|(
literal|"%s: key \"%s\" type %s cannot be certified"
argument_list|,
name|__func__
argument_list|,
name|tmp
argument_list|,
name|key_type
argument_list|(
name|public
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Prepare certificate to sign */
if|if
condition|(
name|key_to_certified
argument_list|(
name|public
argument_list|)
operator|!=
literal|0
condition|)
name|fatal
argument_list|(
literal|"Could not upgrade key %s to certificate"
argument_list|,
name|tmp
argument_list|)
expr_stmt|;
name|public
operator|->
name|cert
operator|->
name|type
operator|=
name|cert_key_type
expr_stmt|;
name|public
operator|->
name|cert
operator|->
name|key_id
operator|=
name|xstrdup
argument_list|(
name|cert_key_id
argument_list|)
expr_stmt|;
name|public
operator|->
name|cert
operator|->
name|nprincipals
operator|=
name|n
expr_stmt|;
name|public
operator|->
name|cert
operator|->
name|principals
operator|=
name|plist
expr_stmt|;
name|public
operator|->
name|cert
operator|->
name|valid_after
operator|=
name|cert_valid_from
expr_stmt|;
name|public
operator|->
name|cert
operator|->
name|valid_before
operator|=
name|cert_valid_to
expr_stmt|;
name|prepare_constraint_buf
argument_list|(
operator|&
name|public
operator|->
name|cert
operator|->
name|constraints
argument_list|)
expr_stmt|;
name|public
operator|->
name|cert
operator|->
name|signature_key
operator|=
name|key_from_private
argument_list|(
name|ca
argument_list|)
expr_stmt|;
if|if
condition|(
name|key_certify
argument_list|(
name|public
argument_list|,
name|ca
argument_list|)
operator|!=
literal|0
condition|)
name|fatal
argument_list|(
literal|"Couldn't not certify key %s"
argument_list|,
name|tmp
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|cp
operator|=
name|strrchr
argument_list|(
name|tmp
argument_list|,
literal|'.'
argument_list|)
operator|)
operator|!=
name|NULL
operator|&&
name|strcmp
argument_list|(
name|cp
argument_list|,
literal|".pub"
argument_list|)
operator|==
literal|0
condition|)
operator|*
name|cp
operator|=
literal|'\0'
expr_stmt|;
name|xasprintf
argument_list|(
operator|&
name|out
argument_list|,
literal|"%s-cert.pub"
argument_list|,
name|tmp
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|tmp
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|fd
operator|=
name|open
argument_list|(
name|out
argument_list|,
name|O_WRONLY
operator||
name|O_CREAT
operator||
name|O_TRUNC
argument_list|,
literal|0644
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"Could not open \"%s\" for writing: %s"
argument_list|,
name|out
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|f
operator|=
name|fdopen
argument_list|(
name|fd
argument_list|,
literal|"w"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"%s: fdopen: %s"
argument_list|,
name|__func__
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|key_write
argument_list|(
name|public
argument_list|,
name|f
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"Could not write certified key to %s"
argument_list|,
name|out
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|f
argument_list|,
literal|" %s\n"
argument_list|,
name|comment
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|quiet
condition|)
name|logit
argument_list|(
literal|"Signed %s key %s: id \"%s\"%s%s valid %s"
argument_list|,
name|cert_key_type
operator|==
name|SSH2_CERT_TYPE_USER
condition|?
literal|"user"
else|:
literal|"host"
argument_list|,
name|out
argument_list|,
name|cert_key_id
argument_list|,
name|cert_principals
operator|!=
name|NULL
condition|?
literal|" for "
else|:
literal|""
argument_list|,
name|cert_principals
operator|!=
name|NULL
condition|?
name|cert_principals
else|:
literal|""
argument_list|,
name|fmt_validity
argument_list|(
name|cert_valid_from
argument_list|,
name|cert_valid_to
argument_list|)
argument_list|)
expr_stmt|;
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|out
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|u_int64_t
name|parse_relative_time
parameter_list|(
specifier|const
name|char
modifier|*
name|s
parameter_list|,
name|time_t
name|now
parameter_list|)
block|{
name|int64_t
name|mul
decl_stmt|,
name|secs
decl_stmt|;
name|mul
operator|=
operator|*
name|s
operator|==
literal|'-'
condition|?
operator|-
literal|1
else|:
literal|1
expr_stmt|;
if|if
condition|(
operator|(
name|secs
operator|=
name|convtime
argument_list|(
name|s
operator|+
literal|1
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"Invalid relative certificate time %s"
argument_list|,
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
name|mul
operator|==
operator|-
literal|1
operator|&&
name|secs
operator|>
name|now
condition|)
name|fatal
argument_list|(
literal|"Certificate time %s cannot be represented"
argument_list|,
name|s
argument_list|)
expr_stmt|;
return|return
name|now
operator|+
call|(
name|u_int64_t
call|)
argument_list|(
name|secs
operator|*
name|mul
argument_list|)
return|;
block|}
end_function

begin_function
specifier|static
name|u_int64_t
name|parse_absolute_time
parameter_list|(
specifier|const
name|char
modifier|*
name|s
parameter_list|)
block|{
name|struct
name|tm
name|tm
decl_stmt|;
name|time_t
name|tt
decl_stmt|;
name|char
name|buf
index|[
literal|32
index|]
decl_stmt|,
modifier|*
name|fmt
decl_stmt|;
comment|/* 	 * POSIX strptime says "The application shall ensure that there  	 * is white-space or other non-alphanumeric characters between 	 * any two conversion specifications" so arrange things this way. 	 */
switch|switch
condition|(
name|strlen
argument_list|(
name|s
argument_list|)
condition|)
block|{
case|case
literal|8
case|:
name|fmt
operator|=
literal|"%Y-%m-%d"
expr_stmt|;
name|snprintf
argument_list|(
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|,
literal|"%.4s-%.2s-%.2s"
argument_list|,
name|s
argument_list|,
name|s
operator|+
literal|4
argument_list|,
name|s
operator|+
literal|6
argument_list|)
expr_stmt|;
break|break;
case|case
literal|14
case|:
name|fmt
operator|=
literal|"%Y-%m-%dT%H:%M:%S"
expr_stmt|;
name|snprintf
argument_list|(
name|buf
argument_list|,
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
argument_list|,
literal|"%.4s-%.2s-%.2sT%.2s:%.2s:%.2s"
argument_list|,
name|s
argument_list|,
name|s
operator|+
literal|4
argument_list|,
name|s
operator|+
literal|6
argument_list|,
name|s
operator|+
literal|8
argument_list|,
name|s
operator|+
literal|10
argument_list|,
name|s
operator|+
literal|12
argument_list|)
expr_stmt|;
break|break;
default|default:
name|fatal
argument_list|(
literal|"Invalid certificate time format %s"
argument_list|,
name|s
argument_list|)
expr_stmt|;
block|}
name|bzero
argument_list|(
operator|&
name|tm
argument_list|,
sizeof|sizeof
argument_list|(
name|tm
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|strptime
argument_list|(
name|buf
argument_list|,
name|fmt
argument_list|,
operator|&
name|tm
argument_list|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"Invalid certificate time %s"
argument_list|,
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|tt
operator|=
name|mktime
argument_list|(
operator|&
name|tm
argument_list|)
operator|)
operator|<
literal|0
condition|)
name|fatal
argument_list|(
literal|"Certificate time %s cannot be represented"
argument_list|,
name|s
argument_list|)
expr_stmt|;
return|return
operator|(
name|u_int64_t
operator|)
name|tt
return|;
block|}
end_function

begin_function
specifier|static
name|void
name|parse_cert_times
parameter_list|(
name|char
modifier|*
name|timespec
parameter_list|)
block|{
name|char
modifier|*
name|from
decl_stmt|,
modifier|*
name|to
decl_stmt|;
name|time_t
name|now
init|=
name|time
argument_list|(
name|NULL
argument_list|)
decl_stmt|;
name|int64_t
name|secs
decl_stmt|;
comment|/* +timespec relative to now */
if|if
condition|(
operator|*
name|timespec
operator|==
literal|'+'
operator|&&
name|strchr
argument_list|(
name|timespec
argument_list|,
literal|':'
argument_list|)
operator|==
name|NULL
condition|)
block|{
if|if
condition|(
operator|(
name|secs
operator|=
name|convtime
argument_list|(
name|timespec
operator|+
literal|1
argument_list|)
operator|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"Invalid relative certificate life %s"
argument_list|,
name|timespec
argument_list|)
expr_stmt|;
name|cert_valid_to
operator|=
name|now
operator|+
name|secs
expr_stmt|;
comment|/* 		 * Backdate certificate one minute to avoid problems on hosts 		 * with poorly-synchronised clocks. 		 */
name|cert_valid_from
operator|=
operator|(
operator|(
name|now
operator|-
literal|59
operator|)
operator|/
literal|60
operator|)
operator|*
literal|60
expr_stmt|;
return|return;
block|}
comment|/* 	 * from:to, where 	 * from := [+-]timespec | YYYYMMDD | YYYYMMDDHHMMSS 	 *   to := [+-]timespec | YYYYMMDD | YYYYMMDDHHMMSS 	 */
name|from
operator|=
name|xstrdup
argument_list|(
name|timespec
argument_list|)
expr_stmt|;
name|to
operator|=
name|strchr
argument_list|(
name|from
argument_list|,
literal|':'
argument_list|)
expr_stmt|;
if|if
condition|(
name|to
operator|==
name|NULL
operator|||
name|from
operator|==
name|to
operator|||
operator|*
operator|(
name|to
operator|+
literal|1
operator|)
operator|==
literal|'\0'
condition|)
name|fatal
argument_list|(
literal|"Invalid certificate life specification %s"
argument_list|,
name|timespec
argument_list|)
expr_stmt|;
operator|*
name|to
operator|++
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
operator|*
name|from
operator|==
literal|'-'
operator|||
operator|*
name|from
operator|==
literal|'+'
condition|)
name|cert_valid_from
operator|=
name|parse_relative_time
argument_list|(
name|from
argument_list|,
name|now
argument_list|)
expr_stmt|;
else|else
name|cert_valid_from
operator|=
name|parse_absolute_time
argument_list|(
name|from
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|to
operator|==
literal|'-'
operator|||
operator|*
name|to
operator|==
literal|'+'
condition|)
name|cert_valid_to
operator|=
name|parse_relative_time
argument_list|(
name|to
argument_list|,
name|cert_valid_from
argument_list|)
expr_stmt|;
else|else
name|cert_valid_to
operator|=
name|parse_absolute_time
argument_list|(
name|to
argument_list|)
expr_stmt|;
if|if
condition|(
name|cert_valid_to
operator|<=
name|cert_valid_from
condition|)
name|fatal
argument_list|(
literal|"Empty certificate validity interval"
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|from
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|add_cert_constraint
parameter_list|(
name|char
modifier|*
name|opt
parameter_list|)
block|{
name|char
modifier|*
name|val
decl_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|opt
argument_list|,
literal|"clear"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator|=
literal|0
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"no-x11-forwarding"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator|&=
operator|~
name|CONSTRAINT_X_FWD
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"permit-x11-forwarding"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator||=
name|CONSTRAINT_X_FWD
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"no-agent-forwarding"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator|&=
operator|~
name|CONSTRAINT_AGENT_FWD
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"permit-agent-forwarding"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator||=
name|CONSTRAINT_AGENT_FWD
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"no-port-forwarding"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator|&=
operator|~
name|CONSTRAINT_PORT_FWD
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"permit-port-forwarding"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator||=
name|CONSTRAINT_PORT_FWD
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"no-pty"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator|&=
operator|~
name|CONSTRAINT_PTY
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"permit-pty"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator||=
name|CONSTRAINT_PTY
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"no-user-rc"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator|&=
operator|~
name|CONSTRAINT_USER_RC
expr_stmt|;
elseif|else
if|if
condition|(
name|strcasecmp
argument_list|(
name|opt
argument_list|,
literal|"permit-user-rc"
argument_list|)
operator|==
literal|0
condition|)
name|constraint_flags
operator||=
name|CONSTRAINT_USER_RC
expr_stmt|;
elseif|else
if|if
condition|(
name|strncasecmp
argument_list|(
name|opt
argument_list|,
literal|"force-command="
argument_list|,
literal|14
argument_list|)
operator|==
literal|0
condition|)
block|{
name|val
operator|=
name|opt
operator|+
literal|14
expr_stmt|;
if|if
condition|(
operator|*
name|val
operator|==
literal|'\0'
condition|)
name|fatal
argument_list|(
literal|"Empty force-command constraint"
argument_list|)
expr_stmt|;
if|if
condition|(
name|constraint_command
operator|!=
name|NULL
condition|)
name|fatal
argument_list|(
literal|"force-command already specified"
argument_list|)
expr_stmt|;
name|constraint_command
operator|=
name|xstrdup
argument_list|(
name|val
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|strncasecmp
argument_list|(
name|opt
argument_list|,
literal|"source-address="
argument_list|,
literal|15
argument_list|)
operator|==
literal|0
condition|)
block|{
name|val
operator|=
name|opt
operator|+
literal|15
expr_stmt|;
if|if
condition|(
operator|*
name|val
operator|==
literal|'\0'
condition|)
name|fatal
argument_list|(
literal|"Empty source-address constraint"
argument_list|)
expr_stmt|;
if|if
condition|(
name|constraint_src_addr
operator|!=
name|NULL
condition|)
name|fatal
argument_list|(
literal|"source-address already specified"
argument_list|)
expr_stmt|;
if|if
condition|(
name|addr_match_cidr_list
argument_list|(
name|NULL
argument_list|,
name|val
argument_list|)
operator|!=
literal|0
condition|)
name|fatal
argument_list|(
literal|"Invalid source-address list"
argument_list|)
expr_stmt|;
name|constraint_src_addr
operator|=
name|xstrdup
argument_list|(
name|val
argument_list|)
expr_stmt|;
block|}
else|else
name|fatal
argument_list|(
literal|"Unsupported certificate constraint \"%s\""
argument_list|,
name|opt
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|do_show_cert
parameter_list|(
name|struct
name|passwd
modifier|*
name|pw
parameter_list|)
block|{
name|Key
modifier|*
name|key
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|char
modifier|*
name|key_fp
decl_stmt|,
modifier|*
name|ca_fp
decl_stmt|;
name|Buffer
name|constraints
decl_stmt|,
name|constraint
decl_stmt|;
name|u_char
modifier|*
name|name
decl_stmt|,
modifier|*
name|data
decl_stmt|;
name|u_int
name|i
decl_stmt|,
name|dlen
decl_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which the key is"
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|(
name|key
operator|=
name|key_load_public
argument_list|(
name|identity_file
argument_list|,
name|NULL
argument_list|)
operator|)
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"%s is not a public key"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|key_is_cert
argument_list|(
name|key
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"%s is not a certificate"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|key_fp
operator|=
name|key_fingerprint
argument_list|(
name|key
argument_list|,
name|SSH_FP_MD5
argument_list|,
name|SSH_FP_HEX
argument_list|)
expr_stmt|;
name|ca_fp
operator|=
name|key_fingerprint
argument_list|(
name|key
operator|->
name|cert
operator|->
name|signature_key
argument_list|,
name|SSH_FP_MD5
argument_list|,
name|SSH_FP_HEX
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%s:\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"        %s certificate %s\n"
argument_list|,
name|key_type
argument_list|(
name|key
argument_list|)
argument_list|,
name|key_fp
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"        Signed by %s CA %s\n"
argument_list|,
name|key_type
argument_list|(
name|key
operator|->
name|cert
operator|->
name|signature_key
argument_list|)
argument_list|,
name|ca_fp
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"        Key ID \"%s\"\n"
argument_list|,
name|key
operator|->
name|cert
operator|->
name|key_id
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"        Valid: %s\n"
argument_list|,
name|fmt_validity
argument_list|(
name|key
operator|->
name|cert
operator|->
name|valid_after
argument_list|,
name|key
operator|->
name|cert
operator|->
name|valid_before
argument_list|)
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"        Principals: "
argument_list|)
expr_stmt|;
if|if
condition|(
name|key
operator|->
name|cert
operator|->
name|nprincipals
operator|==
literal|0
condition|)
name|printf
argument_list|(
literal|"(none)\n"
argument_list|)
expr_stmt|;
else|else
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|key
operator|->
name|cert
operator|->
name|nprincipals
condition|;
name|i
operator|++
control|)
name|printf
argument_list|(
literal|"\n                %s"
argument_list|,
name|key
operator|->
name|cert
operator|->
name|principals
index|[
name|i
index|]
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
block|}
name|printf
argument_list|(
literal|"        Constraints: "
argument_list|)
expr_stmt|;
if|if
condition|(
name|buffer_len
argument_list|(
operator|&
name|key
operator|->
name|cert
operator|->
name|constraints
argument_list|)
operator|==
literal|0
condition|)
name|printf
argument_list|(
literal|"(none)\n"
argument_list|)
expr_stmt|;
else|else
block|{
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
name|buffer_init
argument_list|(
operator|&
name|constraints
argument_list|)
expr_stmt|;
name|buffer_append
argument_list|(
operator|&
name|constraints
argument_list|,
name|buffer_ptr
argument_list|(
operator|&
name|key
operator|->
name|cert
operator|->
name|constraints
argument_list|)
argument_list|,
name|buffer_len
argument_list|(
operator|&
name|key
operator|->
name|cert
operator|->
name|constraints
argument_list|)
argument_list|)
expr_stmt|;
name|buffer_init
argument_list|(
operator|&
name|constraint
argument_list|)
expr_stmt|;
while|while
condition|(
name|buffer_len
argument_list|(
operator|&
name|constraints
argument_list|)
operator|!=
literal|0
condition|)
block|{
name|name
operator|=
name|buffer_get_string
argument_list|(
operator|&
name|constraints
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|data
operator|=
name|buffer_get_string_ptr
argument_list|(
operator|&
name|constraints
argument_list|,
operator|&
name|dlen
argument_list|)
expr_stmt|;
name|buffer_append
argument_list|(
operator|&
name|constraint
argument_list|,
name|data
argument_list|,
name|dlen
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"                %s"
argument_list|,
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|name
argument_list|,
literal|"permit-X11-forwarding"
argument_list|)
operator|==
literal|0
operator|||
name|strcmp
argument_list|(
name|name
argument_list|,
literal|"permit-agent-forwarding"
argument_list|)
operator|==
literal|0
operator|||
name|strcmp
argument_list|(
name|name
argument_list|,
literal|"permit-port-forwarding"
argument_list|)
operator|==
literal|0
operator|||
name|strcmp
argument_list|(
name|name
argument_list|,
literal|"permit-pty"
argument_list|)
operator|==
literal|0
operator|||
name|strcmp
argument_list|(
name|name
argument_list|,
literal|"permit-user-rc"
argument_list|)
operator|==
literal|0
condition|)
name|printf
argument_list|(
literal|"\n"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|name
argument_list|,
literal|"force-command"
argument_list|)
operator|==
literal|0
operator|||
name|strcmp
argument_list|(
name|name
argument_list|,
literal|"source-address"
argument_list|)
operator|==
literal|0
condition|)
block|{
name|data
operator|=
name|buffer_get_string
argument_list|(
operator|&
name|constraint
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|" %s\n"
argument_list|,
name|data
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|data
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|printf
argument_list|(
literal|" UNKNOWN CONSTRAINT (len %u)\n"
argument_list|,
name|buffer_len
argument_list|(
operator|&
name|constraint
argument_list|)
argument_list|)
expr_stmt|;
name|buffer_clear
argument_list|(
operator|&
name|constraint
argument_list|)
expr_stmt|;
block|}
name|xfree
argument_list|(
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
name|buffer_len
argument_list|(
operator|&
name|constraint
argument_list|)
operator|!=
literal|0
condition|)
name|fatal
argument_list|(
literal|"Constraint corrupt: extra data at end"
argument_list|)
expr_stmt|;
block|}
name|buffer_free
argument_list|(
operator|&
name|constraint
argument_list|)
expr_stmt|;
name|buffer_free
argument_list|(
operator|&
name|constraints
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|void
name|usage
parameter_list|(
name|void
parameter_list|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"usage: %s [options]\n"
argument_list|,
name|__progname
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Options:\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -a trials   Number of trials for screening DH-GEX moduli.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -B          Show bubblebabble digest of key file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -b bits     Number of bits in the key to create.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -C comment  Provide new comment.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -c          Change comment in private and public key files.\n"
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|ENABLE_PKCS11
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -D pkcs11   Download public key from pkcs11 token.\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -e          Convert OpenSSH to RFC 4716 key file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -F hostname Find hostname in known hosts file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -f filename Filename of the key file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -G file     Generate candidates for DH-GEX moduli.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -g          Use generic DNS resource record format.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -H          Hash names in known_hosts file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -h          Generate host certificate instead of a user certificate.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -I key_id   Key identifier to include in certificate.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -i          Convert RFC 4716 to OpenSSH key file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -L          Print the contents of a certificate.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -l          Show fingerprint of key file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -M memory   Amount of memory (MB) to use for generating DH-GEX moduli.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -n name,... User/host principal names to include in certificate\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -N phrase   Provide new passphrase.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -O cnstr    Specify a certificate constraint.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -P phrase   Provide old passphrase.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -p          Change passphrase of private key file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -q          Quiet.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -R hostname Remove host from known_hosts file.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -r hostname Print DNS resource record.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -s ca_key   Certify keys with CA key.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -S start    Start point (hex) for generating DH-GEX moduli.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -T file     Screen candidates for DH-GEX moduli.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -t type     Specify type of key to create.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -V from:to  Specify certificate validity interval.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -v          Verbose.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -W gen      Generator to use for generating DH-GEX moduli.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -y          Read private key file and print public key.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Main program for key management.  */
end_comment

begin_function
name|int
name|main
parameter_list|(
name|int
name|argc
parameter_list|,
name|char
modifier|*
modifier|*
name|argv
parameter_list|)
block|{
name|char
name|dotsshdir
index|[
name|MAXPATHLEN
index|]
decl_stmt|,
name|comment
index|[
literal|1024
index|]
decl_stmt|,
modifier|*
name|passphrase1
decl_stmt|,
modifier|*
name|passphrase2
decl_stmt|;
name|char
name|out_file
index|[
name|MAXPATHLEN
index|]
decl_stmt|,
modifier|*
name|pkcs11provider
init|=
name|NULL
decl_stmt|;
name|char
modifier|*
name|rr_hostname
init|=
name|NULL
decl_stmt|;
name|Key
modifier|*
name|private
decl_stmt|,
modifier|*
name|public
decl_stmt|;
name|struct
name|passwd
modifier|*
name|pw
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|int
name|opt
decl_stmt|,
name|type
decl_stmt|,
name|fd
decl_stmt|;
name|u_int32_t
name|memory
init|=
literal|0
decl_stmt|,
name|generator_wanted
init|=
literal|0
decl_stmt|,
name|trials
init|=
literal|100
decl_stmt|;
name|int
name|do_gen_candidates
init|=
literal|0
decl_stmt|,
name|do_screen_candidates
init|=
literal|0
decl_stmt|;
name|BIGNUM
modifier|*
name|start
init|=
name|NULL
decl_stmt|;
name|FILE
modifier|*
name|f
decl_stmt|;
specifier|const
name|char
modifier|*
name|errstr
decl_stmt|;
specifier|extern
name|int
name|optind
decl_stmt|;
specifier|extern
name|char
modifier|*
name|optarg
decl_stmt|;
comment|/* Ensure that fds 0, 1 and 2 are open or directed to /dev/null */
name|sanitise_stdfd
argument_list|()
expr_stmt|;
name|__progname
operator|=
name|ssh_get_progname
argument_list|(
name|argv
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|SSLeay_add_all_algorithms
argument_list|()
expr_stmt|;
name|log_init
argument_list|(
name|argv
index|[
literal|0
index|]
argument_list|,
name|SYSLOG_LEVEL_INFO
argument_list|,
name|SYSLOG_FACILITY_USER
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|init_rng
argument_list|()
expr_stmt|;
name|seed_rng
argument_list|()
expr_stmt|;
comment|/* we need this for the home * directory.  */
name|pw
operator|=
name|getpwuid
argument_list|(
name|getuid
argument_list|()
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|pw
condition|)
block|{
name|printf
argument_list|(
literal|"You don't exist, go away!\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|gethostname
argument_list|(
name|hostname
argument_list|,
sizeof|sizeof
argument_list|(
name|hostname
argument_list|)
argument_list|)
operator|<
literal|0
condition|)
block|{
name|perror
argument_list|(
literal|"gethostname"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
while|while
condition|(
operator|(
name|opt
operator|=
name|getopt
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
literal|"degiqpclBHLhvxXyF:b:f:t:D:I:P:N:n:"
literal|"O:C:r:g:R:T:G:M:S:s:a:V:W:"
argument_list|)
operator|)
operator|!=
operator|-
literal|1
condition|)
block|{
switch|switch
condition|(
name|opt
condition|)
block|{
case|case
literal|'b'
case|:
name|bits
operator|=
operator|(
name|u_int32_t
operator|)
name|strtonum
argument_list|(
name|optarg
argument_list|,
literal|768
argument_list|,
literal|32768
argument_list|,
operator|&
name|errstr
argument_list|)
expr_stmt|;
if|if
condition|(
name|errstr
condition|)
name|fatal
argument_list|(
literal|"Bits has bad value %s (%s)"
argument_list|,
name|optarg
argument_list|,
name|errstr
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'F'
case|:
name|find_host
operator|=
literal|1
expr_stmt|;
name|rr_hostname
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'H'
case|:
name|hash_hosts
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'I'
case|:
name|cert_key_id
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'R'
case|:
name|delete_host
operator|=
literal|1
expr_stmt|;
name|rr_hostname
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'L'
case|:
name|show_cert
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'l'
case|:
name|print_fingerprint
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'B'
case|:
name|print_bubblebabble
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'n'
case|:
name|cert_principals
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'p'
case|:
name|change_passphrase
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'c'
case|:
name|change_comment
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'f'
case|:
if|if
condition|(
name|strlcpy
argument_list|(
name|identity_file
argument_list|,
name|optarg
argument_list|,
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"Identity filename too long"
argument_list|)
expr_stmt|;
name|have_identity
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'g'
case|:
name|print_generic
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'P'
case|:
name|identity_passphrase
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'N'
case|:
name|identity_new_passphrase
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'O'
case|:
name|add_cert_constraint
argument_list|(
name|optarg
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'C'
case|:
name|identity_comment
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'q'
case|:
name|quiet
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'e'
case|:
case|case
literal|'x'
case|:
comment|/* export key */
name|convert_to_ssh2
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'h'
case|:
name|cert_key_type
operator|=
name|SSH2_CERT_TYPE_HOST
expr_stmt|;
name|constraint_flags
operator|=
literal|0
expr_stmt|;
break|break;
case|case
literal|'i'
case|:
case|case
literal|'X'
case|:
comment|/* import key */
name|convert_from_ssh2
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'y'
case|:
name|print_public
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'d'
case|:
name|key_type_name
operator|=
literal|"dsa"
expr_stmt|;
break|break;
case|case
literal|'s'
case|:
name|ca_key_path
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'t'
case|:
name|key_type_name
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'D'
case|:
name|pkcs11provider
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'v'
case|:
if|if
condition|(
name|log_level
operator|==
name|SYSLOG_LEVEL_INFO
condition|)
name|log_level
operator|=
name|SYSLOG_LEVEL_DEBUG1
expr_stmt|;
else|else
block|{
if|if
condition|(
name|log_level
operator|>=
name|SYSLOG_LEVEL_DEBUG1
operator|&&
name|log_level
operator|<
name|SYSLOG_LEVEL_DEBUG3
condition|)
name|log_level
operator|++
expr_stmt|;
block|}
break|break;
case|case
literal|'r'
case|:
name|rr_hostname
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'W'
case|:
name|generator_wanted
operator|=
operator|(
name|u_int32_t
operator|)
name|strtonum
argument_list|(
name|optarg
argument_list|,
literal|1
argument_list|,
name|UINT_MAX
argument_list|,
operator|&
name|errstr
argument_list|)
expr_stmt|;
if|if
condition|(
name|errstr
condition|)
name|fatal
argument_list|(
literal|"Desired generator has bad value: %s (%s)"
argument_list|,
name|optarg
argument_list|,
name|errstr
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'a'
case|:
name|trials
operator|=
operator|(
name|u_int32_t
operator|)
name|strtonum
argument_list|(
name|optarg
argument_list|,
literal|1
argument_list|,
name|UINT_MAX
argument_list|,
operator|&
name|errstr
argument_list|)
expr_stmt|;
if|if
condition|(
name|errstr
condition|)
name|fatal
argument_list|(
literal|"Invalid number of trials: %s (%s)"
argument_list|,
name|optarg
argument_list|,
name|errstr
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'M'
case|:
name|memory
operator|=
operator|(
name|u_int32_t
operator|)
name|strtonum
argument_list|(
name|optarg
argument_list|,
literal|1
argument_list|,
name|UINT_MAX
argument_list|,
operator|&
name|errstr
argument_list|)
expr_stmt|;
if|if
condition|(
name|errstr
condition|)
block|{
name|fatal
argument_list|(
literal|"Memory limit is %s: %s"
argument_list|,
name|errstr
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
literal|'G'
case|:
name|do_gen_candidates
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|strlcpy
argument_list|(
name|out_file
argument_list|,
name|optarg
argument_list|,
sizeof|sizeof
argument_list|(
name|out_file
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|out_file
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"Output filename too long"
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'T'
case|:
name|do_screen_candidates
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|strlcpy
argument_list|(
name|out_file
argument_list|,
name|optarg
argument_list|,
sizeof|sizeof
argument_list|(
name|out_file
argument_list|)
argument_list|)
operator|>=
sizeof|sizeof
argument_list|(
name|out_file
argument_list|)
condition|)
name|fatal
argument_list|(
literal|"Output filename too long"
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'S'
case|:
comment|/* XXX - also compare length against bits */
if|if
condition|(
name|BN_hex2bn
argument_list|(
operator|&
name|start
argument_list|,
name|optarg
argument_list|)
operator|==
literal|0
condition|)
name|fatal
argument_list|(
literal|"Invalid start point."
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'V'
case|:
name|parse_cert_times
argument_list|(
name|optarg
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'?'
case|:
default|default:
name|usage
argument_list|()
expr_stmt|;
block|}
block|}
comment|/* reinit */
name|log_init
argument_list|(
name|argv
index|[
literal|0
index|]
argument_list|,
name|log_level
argument_list|,
name|SYSLOG_FACILITY_USER
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|argv
operator|+=
name|optind
expr_stmt|;
name|argc
operator|-=
name|optind
expr_stmt|;
if|if
condition|(
name|ca_key_path
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
name|argc
operator|<
literal|1
condition|)
block|{
name|printf
argument_list|(
literal|"Too few arguments.\n"
argument_list|)
expr_stmt|;
name|usage
argument_list|()
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|argc
operator|>
literal|0
condition|)
block|{
name|printf
argument_list|(
literal|"Too many arguments.\n"
argument_list|)
expr_stmt|;
name|usage
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
name|change_passphrase
operator|&&
name|change_comment
condition|)
block|{
name|printf
argument_list|(
literal|"Can only have one of -p and -c.\n"
argument_list|)
expr_stmt|;
name|usage
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
name|print_fingerprint
operator|&&
operator|(
name|delete_host
operator|||
name|hash_hosts
operator|)
condition|)
block|{
name|printf
argument_list|(
literal|"Cannot use -l with -D or -R.\n"
argument_list|)
expr_stmt|;
name|usage
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
name|ca_key_path
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
name|cert_key_id
operator|==
name|NULL
condition|)
name|fatal
argument_list|(
literal|"Must specify key id (-I) when certifying"
argument_list|)
expr_stmt|;
name|do_ca_sign
argument_list|(
name|pw
argument_list|,
name|argc
argument_list|,
name|argv
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|show_cert
condition|)
name|do_show_cert
argument_list|(
name|pw
argument_list|)
expr_stmt|;
if|if
condition|(
name|delete_host
operator|||
name|hash_hosts
operator|||
name|find_host
condition|)
name|do_known_hosts
argument_list|(
name|pw
argument_list|,
name|rr_hostname
argument_list|)
expr_stmt|;
if|if
condition|(
name|print_fingerprint
operator|||
name|print_bubblebabble
condition|)
name|do_fingerprint
argument_list|(
name|pw
argument_list|)
expr_stmt|;
if|if
condition|(
name|change_passphrase
condition|)
name|do_change_passphrase
argument_list|(
name|pw
argument_list|)
expr_stmt|;
if|if
condition|(
name|change_comment
condition|)
name|do_change_comment
argument_list|(
name|pw
argument_list|)
expr_stmt|;
if|if
condition|(
name|convert_to_ssh2
condition|)
name|do_convert_to_ssh2
argument_list|(
name|pw
argument_list|)
expr_stmt|;
if|if
condition|(
name|convert_from_ssh2
condition|)
name|do_convert_from_ssh2
argument_list|(
name|pw
argument_list|)
expr_stmt|;
if|if
condition|(
name|print_public
condition|)
name|do_print_public
argument_list|(
name|pw
argument_list|)
expr_stmt|;
if|if
condition|(
name|rr_hostname
operator|!=
name|NULL
condition|)
block|{
name|unsigned
name|int
name|n
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|have_identity
condition|)
block|{
name|n
operator|=
name|do_print_resource_record
argument_list|(
name|pw
argument_list|,
name|identity_file
argument_list|,
name|rr_hostname
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
block|{
name|perror
argument_list|(
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|n
operator|+=
name|do_print_resource_record
argument_list|(
name|pw
argument_list|,
name|_PATH_HOST_RSA_KEY_FILE
argument_list|,
name|rr_hostname
argument_list|)
expr_stmt|;
name|n
operator|+=
name|do_print_resource_record
argument_list|(
name|pw
argument_list|,
name|_PATH_HOST_DSA_KEY_FILE
argument_list|,
name|rr_hostname
argument_list|)
expr_stmt|;
if|if
condition|(
name|n
operator|==
literal|0
condition|)
name|fatal
argument_list|(
literal|"no keys found."
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|pkcs11provider
operator|!=
name|NULL
condition|)
name|do_download
argument_list|(
name|pw
argument_list|,
name|pkcs11provider
argument_list|)
expr_stmt|;
if|if
condition|(
name|do_gen_candidates
condition|)
block|{
name|FILE
modifier|*
name|out
init|=
name|fopen
argument_list|(
name|out_file
argument_list|,
literal|"w"
argument_list|)
decl_stmt|;
if|if
condition|(
name|out
operator|==
name|NULL
condition|)
block|{
name|error
argument_list|(
literal|"Couldn't open modulus candidate file \"%s\": %s"
argument_list|,
name|out_file
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
if|if
condition|(
name|bits
operator|==
literal|0
condition|)
name|bits
operator|=
name|DEFAULT_BITS
expr_stmt|;
if|if
condition|(
name|gen_candidates
argument_list|(
name|out
argument_list|,
name|memory
argument_list|,
name|bits
argument_list|,
name|start
argument_list|)
operator|!=
literal|0
condition|)
name|fatal
argument_list|(
literal|"modulus candidate generation failed"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
if|if
condition|(
name|do_screen_candidates
condition|)
block|{
name|FILE
modifier|*
name|in
decl_stmt|;
name|FILE
modifier|*
name|out
init|=
name|fopen
argument_list|(
name|out_file
argument_list|,
literal|"w"
argument_list|)
decl_stmt|;
if|if
condition|(
name|have_identity
operator|&&
name|strcmp
argument_list|(
name|identity_file
argument_list|,
literal|"-"
argument_list|)
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
operator|(
name|in
operator|=
name|fopen
argument_list|(
name|identity_file
argument_list|,
literal|"r"
argument_list|)
operator|)
operator|==
name|NULL
condition|)
block|{
name|fatal
argument_list|(
literal|"Couldn't open modulus candidate "
literal|"file \"%s\": %s"
argument_list|,
name|identity_file
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
else|else
name|in
operator|=
name|stdin
expr_stmt|;
if|if
condition|(
name|out
operator|==
name|NULL
condition|)
block|{
name|fatal
argument_list|(
literal|"Couldn't open moduli file \"%s\": %s"
argument_list|,
name|out_file
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|prime_test
argument_list|(
name|in
argument_list|,
name|out
argument_list|,
name|trials
argument_list|,
name|generator_wanted
argument_list|)
operator|!=
literal|0
condition|)
name|fatal
argument_list|(
literal|"modulus screening failed"
argument_list|)
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
name|arc4random_stir
argument_list|()
expr_stmt|;
if|if
condition|(
name|key_type_name
operator|==
name|NULL
condition|)
name|key_type_name
operator|=
literal|"rsa"
expr_stmt|;
name|type
operator|=
name|key_type_from_name
argument_list|(
name|key_type_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|KEY_UNSPEC
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"unknown key type %s\n"
argument_list|,
name|key_type_name
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|bits
operator|==
literal|0
condition|)
name|bits
operator|=
operator|(
name|type
operator|==
name|KEY_DSA
operator|)
condition|?
name|DEFAULT_BITS_DSA
else|:
name|DEFAULT_BITS
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|KEY_DSA
operator|&&
name|bits
operator|!=
literal|1024
condition|)
name|fatal
argument_list|(
literal|"DSA keys must be 1024 bits"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|quiet
condition|)
name|printf
argument_list|(
literal|"Generating public/private %s key pair.\n"
argument_list|,
name|key_type_name
argument_list|)
expr_stmt|;
name|private
operator|=
name|key_generate
argument_list|(
name|type
argument_list|,
name|bits
argument_list|)
expr_stmt|;
if|if
condition|(
name|private
operator|==
name|NULL
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"key_generate failed\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|public
operator|=
name|key_from_private
argument_list|(
name|private
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|have_identity
condition|)
name|ask_filename
argument_list|(
name|pw
argument_list|,
literal|"Enter file in which to save the key"
argument_list|)
expr_stmt|;
comment|/* Create ~/.ssh directory if it doesn't already exist. */
name|snprintf
argument_list|(
name|dotsshdir
argument_list|,
sizeof|sizeof
name|dotsshdir
argument_list|,
literal|"%s/%s"
argument_list|,
name|pw
operator|->
name|pw_dir
argument_list|,
name|_PATH_SSH_USER_DIR
argument_list|)
expr_stmt|;
if|if
condition|(
name|strstr
argument_list|(
name|identity_file
argument_list|,
name|dotsshdir
argument_list|)
operator|!=
name|NULL
operator|&&
name|stat
argument_list|(
name|dotsshdir
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|mkdir
argument_list|(
name|dotsshdir
argument_list|,
literal|0700
argument_list|)
operator|<
literal|0
condition|)
name|error
argument_list|(
literal|"Could not create directory '%s'."
argument_list|,
name|dotsshdir
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|!
name|quiet
condition|)
name|printf
argument_list|(
literal|"Created directory '%s'.\n"
argument_list|,
name|dotsshdir
argument_list|)
expr_stmt|;
block|}
comment|/* If the file already exists, ask the user to confirm. */
if|if
condition|(
name|stat
argument_list|(
name|identity_file
argument_list|,
operator|&
name|st
argument_list|)
operator|>=
literal|0
condition|)
block|{
name|char
name|yesno
index|[
literal|3
index|]
decl_stmt|;
name|printf
argument_list|(
literal|"%s already exists.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"Overwrite (y/n)? "
argument_list|)
expr_stmt|;
name|fflush
argument_list|(
name|stdout
argument_list|)
expr_stmt|;
if|if
condition|(
name|fgets
argument_list|(
name|yesno
argument_list|,
sizeof|sizeof
argument_list|(
name|yesno
argument_list|)
argument_list|,
name|stdin
argument_list|)
operator|==
name|NULL
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|yesno
index|[
literal|0
index|]
operator|!=
literal|'y'
operator|&&
name|yesno
index|[
literal|0
index|]
operator|!=
literal|'Y'
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Ask for a passphrase (twice). */
if|if
condition|(
name|identity_passphrase
condition|)
name|passphrase1
operator|=
name|xstrdup
argument_list|(
name|identity_passphrase
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|identity_new_passphrase
condition|)
name|passphrase1
operator|=
name|xstrdup
argument_list|(
name|identity_new_passphrase
argument_list|)
expr_stmt|;
else|else
block|{
name|passphrase_again
label|:
name|passphrase1
operator|=
name|read_passphrase
argument_list|(
literal|"Enter passphrase (empty for no "
literal|"passphrase): "
argument_list|,
name|RP_ALLOW_STDIN
argument_list|)
expr_stmt|;
name|passphrase2
operator|=
name|read_passphrase
argument_list|(
literal|"Enter same passphrase again: "
argument_list|,
name|RP_ALLOW_STDIN
argument_list|)
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|passphrase1
argument_list|,
name|passphrase2
argument_list|)
operator|!=
literal|0
condition|)
block|{
comment|/* 			 * The passphrases do not match.  Clear them and 			 * retry. 			 */
name|memset
argument_list|(
name|passphrase1
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase1
argument_list|)
argument_list|)
expr_stmt|;
name|memset
argument_list|(
name|passphrase2
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase2
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase1
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase2
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"Passphrases do not match.  Try again.\n"
argument_list|)
expr_stmt|;
goto|goto
name|passphrase_again
goto|;
block|}
comment|/* Clear the other copy of the passphrase. */
name|memset
argument_list|(
name|passphrase2
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase2
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase2
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|identity_comment
condition|)
block|{
name|strlcpy
argument_list|(
name|comment
argument_list|,
name|identity_comment
argument_list|,
sizeof|sizeof
argument_list|(
name|comment
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/* Create default comment field for the passphrase. */
name|snprintf
argument_list|(
name|comment
argument_list|,
sizeof|sizeof
name|comment
argument_list|,
literal|"%s@%s"
argument_list|,
name|pw
operator|->
name|pw_name
argument_list|,
name|hostname
argument_list|)
expr_stmt|;
block|}
comment|/* Save the key with the given passphrase and comment. */
if|if
condition|(
operator|!
name|key_save_private
argument_list|(
name|private
argument_list|,
name|identity_file
argument_list|,
name|passphrase1
argument_list|,
name|comment
argument_list|)
condition|)
block|{
name|printf
argument_list|(
literal|"Saving the key failed: %s.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|memset
argument_list|(
name|passphrase1
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase1
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase1
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Clear the passphrase. */
name|memset
argument_list|(
name|passphrase1
argument_list|,
literal|0
argument_list|,
name|strlen
argument_list|(
name|passphrase1
argument_list|)
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|passphrase1
argument_list|)
expr_stmt|;
comment|/* Clear the private key and the random number generator. */
name|key_free
argument_list|(
name|private
argument_list|)
expr_stmt|;
name|arc4random_stir
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
name|quiet
condition|)
name|printf
argument_list|(
literal|"Your identification has been saved in %s.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|strlcat
argument_list|(
name|identity_file
argument_list|,
literal|".pub"
argument_list|,
sizeof|sizeof
argument_list|(
name|identity_file
argument_list|)
argument_list|)
expr_stmt|;
name|fd
operator|=
name|open
argument_list|(
name|identity_file
argument_list|,
name|O_WRONLY
operator||
name|O_CREAT
operator||
name|O_TRUNC
argument_list|,
literal|0644
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|==
operator|-
literal|1
condition|)
block|{
name|printf
argument_list|(
literal|"Could not save your public key in %s\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
name|f
operator|=
name|fdopen
argument_list|(
name|fd
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
if|if
condition|(
name|f
operator|==
name|NULL
condition|)
block|{
name|printf
argument_list|(
literal|"fdopen %s failed\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|key_write
argument_list|(
name|public
argument_list|,
name|f
argument_list|)
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"write key failed\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|f
argument_list|,
literal|" %s\n"
argument_list|,
name|comment
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|quiet
condition|)
block|{
name|char
modifier|*
name|fp
init|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|SSH_FP_MD5
argument_list|,
name|SSH_FP_HEX
argument_list|)
decl_stmt|;
name|char
modifier|*
name|ra
init|=
name|key_fingerprint
argument_list|(
name|public
argument_list|,
name|SSH_FP_MD5
argument_list|,
name|SSH_FP_RANDOMART
argument_list|)
decl_stmt|;
name|printf
argument_list|(
literal|"Your public key has been saved in %s.\n"
argument_list|,
name|identity_file
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"The key fingerprint is:\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%s %s\n"
argument_list|,
name|fp
argument_list|,
name|comment
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"The key's randomart image is:\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"%s\n"
argument_list|,
name|ra
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|ra
argument_list|)
expr_stmt|;
name|xfree
argument_list|(
name|fp
argument_list|)
expr_stmt|;
block|}
name|key_free
argument_list|(
name|public
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

