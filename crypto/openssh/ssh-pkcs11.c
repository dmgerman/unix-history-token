...
begin_comment
comment|/* $OpenBSD: ssh-pkcs11.c,v 1.4 2010/02/24 06:12:53 djm Exp $ */
end_comment
...
literal|1
decl_stmt|;
comment|/* some compilers complain about non-constant initializer so we 	   use NULL in CK_ATTRIBUTE above and set the values here */
name|key_filter
index|[
literal|0
index|]
...
name|error
argument_list|(
literal|"C_FindObjects failed (%lu nfound): %lu"
argument_list|,
name|nfound
argument_list|,
name|rv
...
block|}
decl_stmt|;
comment|/* some compilers complain about non-constant initializer so we 	   use NULL in CK_ATTRIBUTE above and set the value here */
name|pubkey_filter
index|[
literal|0
index|]
...
continue|continue;
block|}
comment|/* allocate buffers for attributes, XXX check ulValueLen? */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* retrieve ID, modulus and public exponent of RSA key */
if|if
condition|(
operator|(
name|rv
