...
begin_comment
comment|/* $OpenBSD: servconf.c,v 1.234 2013/02/06 00:20:42 dtucker Exp $ */
end_comment
...
condition|)
block|{
comment|/* 		 * HPN buffer size option not explicitly set.  Try to figure 		 * out what value to use or resort to default. 		 */
name|options
operator|->
name|hpn_buffer_size
operator|=
...
block|{
comment|/* 		 * In the case that the user sets both values in a 		 * contradictory manner hpn_disabled overrrides hpn_buffer_size. 		 */
if|if
condition|(
name|options
operator|->
...
literal|1
expr_stmt|;
comment|/* Limit the maximum buffer to BUFFER_MAX_LEN. */
if|if
condition|(
name|options
operator|->
...
block|,
name|sZeroKnowledgePasswordAuthentication
block|,
...
name|fatal
argument_list|(
literal|"%s line %d: address family must be specified "
literal|"before ListenAddress."
argument_list|,
name|filename
argument_list|,
name|linenum
