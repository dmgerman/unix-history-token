...
begin_comment
comment|/*  * This is called just before select() to add any bits relevant to channels  * in the select bitmasks.  */
end_comment
...
name|fatal
argument_list|(
literal|"cannot happen: IN_DRAIN"
argument_list|)
expr_stmt|;
if|if
condition|(
...
decl_stmt|;
comment|/* Loop over all channels... */
for|for
control|(
name|i
operator|=
...
name|debug
argument_list|(
literal|"Connection to port %d forwarding to %.100s port %d requested."
argument_list|,
name|ch
operator|->
name|listening_port
...
name|buf
argument_list|,
literal|"listen port %d for %.100s port %d, connect from %.200s port %d"
argument_list|,
name|ch
operator|->
name|listening_port
...
name|SSH_CHANNEL_OPEN
case|:
comment|/* 			 * This is an open two-way communication channel. It 			 * is not of interest to us at this point what kind 			 * of data is being transmitted. 			 */
comment|/* 			 * Read available incoming data and append it to 			 * buffer; shutdown socket, if read or write failes 			 */
if|if
condition|(
name|FD_ISSET
argument_list|(
...
name|packet_disconnect
argument_list|(
literal|"Received data for free channel %d."
argument_list|,
name|ch
operator|->
name|self
...
begin_comment
comment|/* This is called after receiving CHANNEL_CLOSE/IEOF. */
end_comment
...
condition|)
block|{
comment|/* proto version 1.5 overloads CLOSE with IEOF */
name|chan_rcvd_ieof
argument_list|(
operator|&
name|channels
...
begin_comment
comment|/* This is called after receiving CHANNEL_CLOSE_CONFIRMATION/OCLOSE. */
end_comment
...
name|packet_disconnect
argument_list|(
literal|"Received close confirmation for out-of-range channel %d."
argument_list|,
name|channel
argument_list|)
expr_stmt|;
...
name|packet_disconnect
argument_list|(
literal|"Received close confirmation for non-closed channel %d (type %d)."
argument_list|,
name|channel
argument_list|,
name|channels
...
name|packet_disconnect
argument_list|(
literal|"Received open confirmation for non-opening channel %d."
argument_list|,
name|channel
argument_list|)
expr_stmt|;
comment|/* Get remote side's id for this channel. */
name|remote_channel
operator|=
name|packet_get_int
argument_list|()
...
name|packet_disconnect
argument_list|(
literal|"Received open failure for non-opening channel %d."
argument_list|,
name|channel
argument_list|)
expr_stmt|;
...
name|buf
argument_list|,
literal|"  #%d %.300s (t%d r%d i%d/%d o%d/%d)\r\n"
argument_list|,
name|c
operator|->
name|self
...
argument_list|)
expr_stmt|;
comment|/* 	 * Initiate forwarding, 	 * bind port to localhost only (gateway ports == 0). 	 */
name|channel_request_local_forwarding
argument_list|(
name|port
argument_list|,
...
name|error
argument_list|(
literal|"channel_input_port_open: getnameinfo failed"
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
break|break;
block|}
comment|/* (ai = aitop, ai; ai = ai->ai_next) */
name|freeaddrinfo
argument_list|(
name|aitop
argument_list|)
...
name|success
label|:
comment|/* We have successfully obtained a connection to the real X display. */
comment|/* Allocate a channel for this connection. */
if|if
condition|(
name|x11_saved_proto
