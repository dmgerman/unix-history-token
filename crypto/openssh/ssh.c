begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Author: Tatu Ylonen<ylo@cs.hut.fi>  * Copyright (c) 1995 Tatu Ylonen<ylo@cs.hut.fi>, Espoo, Finland  *                    All rights reserved  * Created: Sat Mar 18 16:36:11 1995 ylo  * Ssh client program.  This program can be used to log into a remote machine.  * The software supports strong authentication, encryption, and forwarding  * of X11, TCP/IP, and authentication connections.  *  * Modified to work with SSL by Niels Provos<provos@citi.umich.edu> in Canada.  *  * $FreeBSD$  */
end_comment

begin_include
include|#
directive|include
file|"includes.h"
end_include

begin_expr_stmt
name|RCSID
argument_list|(
literal|"$Id: ssh.c,v 1.41 2000/02/28 19:51:58 markus Exp $"
argument_list|)
expr_stmt|;
end_expr_stmt

begin_include
include|#
directive|include
file|"xmalloc.h"
end_include

begin_include
include|#
directive|include
file|"ssh.h"
end_include

begin_include
include|#
directive|include
file|"packet.h"
end_include

begin_include
include|#
directive|include
file|"buffer.h"
end_include

begin_include
include|#
directive|include
file|"authfd.h"
end_include

begin_include
include|#
directive|include
file|"readconf.h"
end_include

begin_include
include|#
directive|include
file|"uidswap.h"
end_include

begin_comment
comment|/* Flag indicating whether IPv4 or IPv6.  This can be set on the command line.    Default value is AF_UNSPEC means both IPv4 and IPv6. */
end_comment

begin_decl_stmt
name|int
name|IPv4or6
init|=
name|AF_UNSPEC
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag indicating whether debug mode is on.  This can be set on the command line. */
end_comment

begin_decl_stmt
name|int
name|debug_flag
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|tty_flag
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Flag indicating that nothing should be read from stdin.  This can be set  * on the command line.  */
end_comment

begin_decl_stmt
name|int
name|stdin_null_flag
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Flag indicating that ssh should fork after authentication.  This is useful  * so that the pasphrase can be entered manually, and then ssh goes to the  * background.  */
end_comment

begin_decl_stmt
name|int
name|fork_after_authentication_flag
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * General data structure for command line options and options configurable  * in configuration files.  See readconf.h.  */
end_comment

begin_decl_stmt
name|Options
name|options
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Name of the host we are connecting to.  This is the name given on the  * command line, or the HostName specified for the user-supplied name in a  * configuration file.  */
end_comment

begin_decl_stmt
name|char
modifier|*
name|host
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* socket address the host resolves to */
end_comment

begin_decl_stmt
name|struct
name|sockaddr_storage
name|hostaddr
decl_stmt|;
end_decl_stmt

begin_comment
comment|/*  * Flag to indicate that we have received a window change signal which has  * not yet been processed.  This will cause a message indicating the new  * window size to be sent to the server a little later.  This is volatile  * because this is updated in a signal handler.  */
end_comment

begin_decl_stmt
specifier|volatile
name|int
name|received_window_change_signal
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Value of argv[0] (set in the main program). */
end_comment

begin_decl_stmt
name|char
modifier|*
name|av0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Flag indicating whether we have a valid host private key loaded. */
end_comment

begin_decl_stmt
name|int
name|host_private_key_loaded
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Host private key. */
end_comment

begin_decl_stmt
name|RSA
modifier|*
name|host_private_key
init|=
name|NULL
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Original real UID. */
end_comment

begin_decl_stmt
name|uid_t
name|original_real_uid
decl_stmt|;
end_decl_stmt

begin_comment
comment|/* Prints a help message to the user.  This function never returns. */
end_comment

begin_function
name|void
name|usage
parameter_list|()
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Usage: %s [options] host [command]\n"
argument_list|,
name|av0
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Options:\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -l user     Log in using this user name.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -n          Redirect input from /dev/null.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -a          Disable authentication agent forwarding.\n"
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|AFS
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -k          Disable Kerberos ticket and AFS token forwarding.\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* AFS */
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -x          Disable X11 connection forwarding.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -X          Enable X11 connection forwarding.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -i file     Identity for RSA authentication (default: ~/.ssh/identity).\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -t          Tty; allocate a tty even if command is given.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -v          Verbose; display verbose debugging messages.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -V          Display version number only.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -P          Don't allocate a privileged port.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -q          Quiet; don't display any warning messages.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -f          Fork into background after authentication.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -e char     Set escape character; ``none'' = disable (default: ~).\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -c cipher   Select encryption algorithm: "
literal|"``3des'', "
literal|"``blowfish''\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -p port     Connect to this port.  Server must be on the same port.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -L listen-port:host:port   Forward local port to remote address\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -R listen-port:host:port   Forward remote port to local address\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"              These cause %s to listen for connections on a port, and\n"
argument_list|,
name|av0
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"              forward them to the other side by connecting to host:port.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -C          Enable compression.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -g          Allow remote hosts to connect to forwarded ports.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -4          Use IPv4 only.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -6          Use IPv6 only.\n"
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"  -o 'option' Process the option as if it was read from a configuration file.\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Connects to the given host using rsh (or prints an error message and exits  * if rsh is not available).  This function never returns.  */
end_comment

begin_function
name|void
name|rsh_connect
parameter_list|(
name|char
modifier|*
name|host
parameter_list|,
name|char
modifier|*
name|user
parameter_list|,
name|Buffer
modifier|*
name|command
parameter_list|)
block|{
name|char
modifier|*
name|args
index|[
literal|10
index|]
decl_stmt|;
name|int
name|i
decl_stmt|;
name|log
argument_list|(
literal|"Using rsh.  WARNING: Connection will not be encrypted."
argument_list|)
expr_stmt|;
comment|/* Build argument list for rsh. */
name|i
operator|=
literal|0
expr_stmt|;
ifndef|#
directive|ifndef
name|_PATH_RSH
define|#
directive|define
name|_PATH_RSH
value|"/usr/bin/rsh"
endif|#
directive|endif
name|args
index|[
name|i
operator|++
index|]
operator|=
name|_PATH_RSH
expr_stmt|;
comment|/* host may have to come after user on some systems */
name|args
index|[
name|i
operator|++
index|]
operator|=
name|host
expr_stmt|;
if|if
condition|(
name|user
condition|)
block|{
name|args
index|[
name|i
operator|++
index|]
operator|=
literal|"-l"
expr_stmt|;
name|args
index|[
name|i
operator|++
index|]
operator|=
name|user
expr_stmt|;
block|}
if|if
condition|(
name|buffer_len
argument_list|(
name|command
argument_list|)
operator|>
literal|0
condition|)
block|{
name|buffer_append
argument_list|(
name|command
argument_list|,
literal|"\0"
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|args
index|[
name|i
operator|++
index|]
operator|=
name|buffer_ptr
argument_list|(
name|command
argument_list|)
expr_stmt|;
block|}
name|args
index|[
name|i
operator|++
index|]
operator|=
name|NULL
expr_stmt|;
if|if
condition|(
name|debug_flag
condition|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
name|args
index|[
name|i
index|]
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|i
operator|!=
literal|0
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|" "
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s"
argument_list|,
name|args
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\n"
argument_list|)
expr_stmt|;
block|}
name|execv
argument_list|(
name|_PATH_RSH
argument_list|,
name|args
argument_list|)
expr_stmt|;
name|perror
argument_list|(
name|_PATH_RSH
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/*  * Main program for the ssh client.  */
end_comment

begin_function
name|int
name|main
parameter_list|(
name|int
name|ac
parameter_list|,
name|char
modifier|*
modifier|*
name|av
parameter_list|)
block|{
name|int
name|i
decl_stmt|,
name|opt
decl_stmt|,
name|optind
decl_stmt|,
name|type
decl_stmt|,
name|exit_status
decl_stmt|,
name|ok
decl_stmt|,
name|authfd
decl_stmt|;
name|u_short
name|fwd_port
decl_stmt|,
name|fwd_host_port
decl_stmt|;
name|char
modifier|*
name|optarg
decl_stmt|,
modifier|*
name|cp
decl_stmt|,
name|buf
index|[
literal|256
index|]
decl_stmt|;
name|Buffer
name|command
decl_stmt|;
name|struct
name|winsize
name|ws
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|struct
name|passwd
modifier|*
name|pw
decl_stmt|,
name|pwcopy
decl_stmt|;
name|int
name|interactive
init|=
literal|0
decl_stmt|,
name|dummy
decl_stmt|;
name|uid_t
name|original_effective_uid
decl_stmt|;
name|int
name|plen
decl_stmt|;
comment|/* 	 * Save the original real uid.  It will be needed later (uid-swapping 	 * may clobber the real uid). 	 */
name|original_real_uid
operator|=
name|getuid
argument_list|()
expr_stmt|;
name|original_effective_uid
operator|=
name|geteuid
argument_list|()
expr_stmt|;
comment|/* If we are installed setuid root be careful to not drop core. */
if|if
condition|(
name|original_real_uid
operator|!=
name|original_effective_uid
condition|)
block|{
name|struct
name|rlimit
name|rlim
decl_stmt|;
name|rlim
operator|.
name|rlim_cur
operator|=
name|rlim
operator|.
name|rlim_max
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|setrlimit
argument_list|(
name|RLIMIT_CORE
argument_list|,
operator|&
name|rlim
argument_list|)
operator|<
literal|0
condition|)
name|fatal
argument_list|(
literal|"setrlimit failed: %.100s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Use uid-swapping to give up root privileges for the duration of 	 * option processing.  We will re-instantiate the rights when we are 	 * ready to create the privileged port, and will permanently drop 	 * them when the port has been created (actually, when the connection 	 * has been made, as we may need to create the port several times). 	 */
name|temporarily_use_uid
argument_list|(
name|original_real_uid
argument_list|)
expr_stmt|;
comment|/* 	 * Set our umask to something reasonable, as some files are created 	 * with the default umask.  This will make them world-readable but 	 * writable only by the owner, which is ok for all files for which we 	 * don't set the modes explicitly. 	 */
name|umask
argument_list|(
literal|022
argument_list|)
expr_stmt|;
comment|/* Save our own name. */
name|av0
operator|=
name|av
index|[
literal|0
index|]
expr_stmt|;
comment|/* Initialize option structure to indicate that no values have been set. */
name|initialize_options
argument_list|(
operator|&
name|options
argument_list|)
expr_stmt|;
comment|/* Parse command-line arguments. */
name|host
operator|=
name|NULL
expr_stmt|;
comment|/* If program name is not one of the standard names, use it as host name. */
if|if
condition|(
name|strchr
argument_list|(
name|av0
argument_list|,
literal|'/'
argument_list|)
condition|)
name|cp
operator|=
name|strrchr
argument_list|(
name|av0
argument_list|,
literal|'/'
argument_list|)
operator|+
literal|1
expr_stmt|;
else|else
name|cp
operator|=
name|av0
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|cp
argument_list|,
literal|"rsh"
argument_list|)
operator|!=
literal|0
operator|&&
name|strcmp
argument_list|(
name|cp
argument_list|,
literal|"ssh"
argument_list|)
operator|!=
literal|0
operator|&&
name|strcmp
argument_list|(
name|cp
argument_list|,
literal|"rlogin"
argument_list|)
operator|!=
literal|0
operator|&&
name|strcmp
argument_list|(
name|cp
argument_list|,
literal|"slogin"
argument_list|)
operator|!=
literal|0
condition|)
name|host
operator|=
name|cp
expr_stmt|;
for|for
control|(
name|optind
operator|=
literal|1
init|;
name|optind
operator|<
name|ac
condition|;
name|optind
operator|++
control|)
block|{
if|if
condition|(
name|av
index|[
name|optind
index|]
index|[
literal|0
index|]
operator|!=
literal|'-'
condition|)
block|{
if|if
condition|(
name|host
condition|)
break|break;
if|if
condition|(
operator|(
name|cp
operator|=
name|strchr
argument_list|(
name|av
index|[
name|optind
index|]
argument_list|,
literal|'@'
argument_list|)
operator|)
condition|)
block|{
if|if
condition|(
name|cp
operator|==
name|av
index|[
name|optind
index|]
condition|)
name|usage
argument_list|()
expr_stmt|;
name|options
operator|.
name|user
operator|=
name|av
index|[
name|optind
index|]
expr_stmt|;
operator|*
name|cp
operator|=
literal|'\0'
expr_stmt|;
name|host
operator|=
operator|++
name|cp
expr_stmt|;
block|}
else|else
name|host
operator|=
name|av
index|[
name|optind
index|]
expr_stmt|;
continue|continue;
block|}
name|opt
operator|=
name|av
index|[
name|optind
index|]
index|[
literal|1
index|]
expr_stmt|;
if|if
condition|(
operator|!
name|opt
condition|)
name|usage
argument_list|()
expr_stmt|;
if|if
condition|(
name|strchr
argument_list|(
literal|"eilcpLRo"
argument_list|,
name|opt
argument_list|)
condition|)
block|{
comment|/* options with arguments */
name|optarg
operator|=
name|av
index|[
name|optind
index|]
operator|+
literal|2
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|optarg
argument_list|,
literal|""
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|optind
operator|>=
name|ac
operator|-
literal|1
condition|)
name|usage
argument_list|()
expr_stmt|;
name|optarg
operator|=
name|av
index|[
operator|++
name|optind
index|]
expr_stmt|;
block|}
block|}
else|else
block|{
if|if
condition|(
name|av
index|[
name|optind
index|]
index|[
literal|2
index|]
condition|)
name|usage
argument_list|()
expr_stmt|;
name|optarg
operator|=
name|NULL
expr_stmt|;
block|}
switch|switch
condition|(
name|opt
condition|)
block|{
case|case
literal|'4'
case|:
name|IPv4or6
operator|=
name|AF_INET
expr_stmt|;
break|break;
case|case
literal|'6'
case|:
name|IPv4or6
operator|=
name|AF_INET6
expr_stmt|;
break|break;
case|case
literal|'n'
case|:
name|stdin_null_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'f'
case|:
name|fork_after_authentication_flag
operator|=
literal|1
expr_stmt|;
name|stdin_null_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'x'
case|:
name|options
operator|.
name|forward_x11
operator|=
literal|0
expr_stmt|;
break|break;
case|case
literal|'X'
case|:
name|options
operator|.
name|forward_x11
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'g'
case|:
name|options
operator|.
name|gateway_ports
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'P'
case|:
name|options
operator|.
name|use_privileged_port
operator|=
literal|0
expr_stmt|;
break|break;
case|case
literal|'a'
case|:
name|options
operator|.
name|forward_agent
operator|=
literal|0
expr_stmt|;
break|break;
ifdef|#
directive|ifdef
name|AFS
case|case
literal|'k'
case|:
name|options
operator|.
name|krb4_tgt_passing
operator|=
literal|0
expr_stmt|;
name|options
operator|.
name|krb5_tgt_passing
operator|=
literal|0
expr_stmt|;
name|options
operator|.
name|afs_token_passing
operator|=
literal|0
expr_stmt|;
break|break;
endif|#
directive|endif
case|case
literal|'i'
case|:
if|if
condition|(
name|stat
argument_list|(
name|optarg
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Warning: Identity file %s does not exist.\n"
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|options
operator|.
name|num_identity_files
operator|>=
name|SSH_MAX_IDENTITY_FILES
condition|)
name|fatal
argument_list|(
literal|"Too many identity files specified (max %d)"
argument_list|,
name|SSH_MAX_IDENTITY_FILES
argument_list|)
expr_stmt|;
name|options
operator|.
name|identity_files
index|[
name|options
operator|.
name|num_identity_files
operator|++
index|]
operator|=
name|xstrdup
argument_list|(
name|optarg
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'t'
case|:
name|tty_flag
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'v'
case|:
case|case
literal|'V'
case|:
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"SSH Version %s, protocol version %d.%d.\n"
argument_list|,
name|SSH_VERSION
argument_list|,
name|PROTOCOL_MAJOR
argument_list|,
name|PROTOCOL_MINOR
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Compiled with SSL.\n"
argument_list|)
expr_stmt|;
if|if
condition|(
name|opt
operator|==
literal|'V'
condition|)
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|debug_flag
operator|=
literal|1
expr_stmt|;
name|options
operator|.
name|log_level
operator|=
name|SYSLOG_LEVEL_DEBUG
expr_stmt|;
break|break;
case|case
literal|'q'
case|:
name|options
operator|.
name|log_level
operator|=
name|SYSLOG_LEVEL_QUIET
expr_stmt|;
break|break;
case|case
literal|'e'
case|:
if|if
condition|(
name|optarg
index|[
literal|0
index|]
operator|==
literal|'^'
operator|&&
name|optarg
index|[
literal|2
index|]
operator|==
literal|0
operator|&&
operator|(
name|unsigned
name|char
operator|)
name|optarg
index|[
literal|1
index|]
operator|>=
literal|64
operator|&&
operator|(
name|unsigned
name|char
operator|)
name|optarg
index|[
literal|1
index|]
operator|<
literal|128
condition|)
name|options
operator|.
name|escape_char
operator|=
operator|(
name|unsigned
name|char
operator|)
name|optarg
index|[
literal|1
index|]
operator|&
literal|31
expr_stmt|;
elseif|else
if|if
condition|(
name|strlen
argument_list|(
name|optarg
argument_list|)
operator|==
literal|1
condition|)
name|options
operator|.
name|escape_char
operator|=
operator|(
name|unsigned
name|char
operator|)
name|optarg
index|[
literal|0
index|]
expr_stmt|;
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|optarg
argument_list|,
literal|"none"
argument_list|)
operator|==
literal|0
condition|)
name|options
operator|.
name|escape_char
operator|=
operator|-
literal|2
expr_stmt|;
else|else
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Bad escape character '%s'.\n"
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
literal|'c'
case|:
name|options
operator|.
name|cipher
operator|=
name|cipher_number
argument_list|(
name|optarg
argument_list|)
expr_stmt|;
if|if
condition|(
name|options
operator|.
name|cipher
operator|==
operator|-
literal|1
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Unknown cipher type '%s'\n"
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
literal|'p'
case|:
name|options
operator|.
name|port
operator|=
name|atoi
argument_list|(
name|optarg
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'l'
case|:
name|options
operator|.
name|user
operator|=
name|optarg
expr_stmt|;
break|break;
case|case
literal|'R'
case|:
if|if
condition|(
name|sscanf
argument_list|(
name|optarg
argument_list|,
literal|"%hu/%255[^/]/%hu"
argument_list|,
operator|&
name|fwd_port
argument_list|,
name|buf
argument_list|,
operator|&
name|fwd_host_port
argument_list|)
operator|!=
literal|3
operator|&&
name|sscanf
argument_list|(
name|optarg
argument_list|,
literal|"%hu:%255[^:]:%hu"
argument_list|,
operator|&
name|fwd_port
argument_list|,
name|buf
argument_list|,
operator|&
name|fwd_host_port
argument_list|)
operator|!=
literal|3
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Bad forwarding specification '%s'.\n"
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
name|usage
argument_list|()
expr_stmt|;
comment|/* NOTREACHED */
block|}
name|add_remote_forward
argument_list|(
operator|&
name|options
argument_list|,
name|fwd_port
argument_list|,
name|buf
argument_list|,
name|fwd_host_port
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'L'
case|:
if|if
condition|(
name|sscanf
argument_list|(
name|optarg
argument_list|,
literal|"%hu/%255[^/]/%hu"
argument_list|,
operator|&
name|fwd_port
argument_list|,
name|buf
argument_list|,
operator|&
name|fwd_host_port
argument_list|)
operator|!=
literal|3
operator|&&
name|sscanf
argument_list|(
name|optarg
argument_list|,
literal|"%hu:%255[^:]:%hu"
argument_list|,
operator|&
name|fwd_port
argument_list|,
name|buf
argument_list|,
operator|&
name|fwd_host_port
argument_list|)
operator|!=
literal|3
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Bad forwarding specification '%s'.\n"
argument_list|,
name|optarg
argument_list|)
expr_stmt|;
name|usage
argument_list|()
expr_stmt|;
comment|/* NOTREACHED */
block|}
name|add_local_forward
argument_list|(
operator|&
name|options
argument_list|,
name|fwd_port
argument_list|,
name|buf
argument_list|,
name|fwd_host_port
argument_list|)
expr_stmt|;
break|break;
case|case
literal|'C'
case|:
name|options
operator|.
name|compression
operator|=
literal|1
expr_stmt|;
break|break;
case|case
literal|'o'
case|:
name|dummy
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|process_config_line
argument_list|(
operator|&
name|options
argument_list|,
name|host
condition|?
name|host
else|:
literal|""
argument_list|,
name|optarg
argument_list|,
literal|"command-line"
argument_list|,
literal|0
argument_list|,
operator|&
name|dummy
argument_list|)
operator|!=
literal|0
condition|)
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
break|break;
default|default:
name|usage
argument_list|()
expr_stmt|;
block|}
block|}
comment|/* Check that we got a host name. */
if|if
condition|(
operator|!
name|host
condition|)
name|usage
argument_list|()
expr_stmt|;
comment|/* check if RSA support exists */
if|if
condition|(
name|rsa_alive
argument_list|()
operator|==
literal|0
condition|)
block|{
specifier|extern
name|char
modifier|*
name|__progname
decl_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%s: no RSA support in libssl and libcrypto.  See ssl(8).\n"
argument_list|,
name|__progname
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Initialize the command to execute on remote host. */
name|buffer_init
argument_list|(
operator|&
name|command
argument_list|)
expr_stmt|;
comment|/* 	 * Save the command to execute on the remote host in a buffer. There 	 * is no limit on the length of the command, except by the maximum 	 * packet size.  Also sets the tty flag if there is no command. 	 */
if|if
condition|(
name|optind
operator|==
name|ac
condition|)
block|{
comment|/* No command specified - execute shell on a tty. */
name|tty_flag
operator|=
literal|1
expr_stmt|;
block|}
else|else
block|{
comment|/* A command has been specified.  Store it into the 		   buffer. */
for|for
control|(
name|i
operator|=
name|optind
init|;
name|i
operator|<
name|ac
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|i
operator|>
name|optind
condition|)
name|buffer_append
argument_list|(
operator|&
name|command
argument_list|,
literal|" "
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|buffer_append
argument_list|(
operator|&
name|command
argument_list|,
name|av
index|[
name|i
index|]
argument_list|,
name|strlen
argument_list|(
name|av
index|[
name|i
index|]
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* Cannot fork to background if no command. */
if|if
condition|(
name|fork_after_authentication_flag
operator|&&
name|buffer_len
argument_list|(
operator|&
name|command
argument_list|)
operator|==
literal|0
condition|)
name|fatal
argument_list|(
literal|"Cannot fork into background without a command to execute."
argument_list|)
expr_stmt|;
comment|/* Allocate a tty by default if no command specified. */
if|if
condition|(
name|buffer_len
argument_list|(
operator|&
name|command
argument_list|)
operator|==
literal|0
condition|)
name|tty_flag
operator|=
literal|1
expr_stmt|;
comment|/* Do not allocate a tty if stdin is not a tty. */
if|if
condition|(
operator|!
name|isatty
argument_list|(
name|fileno
argument_list|(
name|stdin
argument_list|)
argument_list|)
condition|)
block|{
if|if
condition|(
name|tty_flag
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Pseudo-terminal will not be allocated because stdin is not a terminal.\n"
argument_list|)
expr_stmt|;
name|tty_flag
operator|=
literal|0
expr_stmt|;
block|}
comment|/* Get user data. */
name|pw
operator|=
name|getpwuid
argument_list|(
name|original_real_uid
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|pw
condition|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"You don't exist, go away!\n"
argument_list|)
expr_stmt|;
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Take a copy of the returned structure. */
name|memset
argument_list|(
operator|&
name|pwcopy
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|pwcopy
argument_list|)
argument_list|)
expr_stmt|;
name|pwcopy
operator|.
name|pw_name
operator|=
name|xstrdup
argument_list|(
name|pw
operator|->
name|pw_name
argument_list|)
expr_stmt|;
name|pwcopy
operator|.
name|pw_passwd
operator|=
name|xstrdup
argument_list|(
name|pw
operator|->
name|pw_passwd
argument_list|)
expr_stmt|;
name|pwcopy
operator|.
name|pw_uid
operator|=
name|pw
operator|->
name|pw_uid
expr_stmt|;
name|pwcopy
operator|.
name|pw_gid
operator|=
name|pw
operator|->
name|pw_gid
expr_stmt|;
name|pwcopy
operator|.
name|pw_dir
operator|=
name|xstrdup
argument_list|(
name|pw
operator|->
name|pw_dir
argument_list|)
expr_stmt|;
name|pwcopy
operator|.
name|pw_shell
operator|=
name|xstrdup
argument_list|(
name|pw
operator|->
name|pw_shell
argument_list|)
expr_stmt|;
name|pw
operator|=
operator|&
name|pwcopy
expr_stmt|;
comment|/* Initialize "log" output.  Since we are the client all output 	   actually goes to the terminal. */
name|log_init
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
name|options
operator|.
name|log_level
argument_list|,
name|SYSLOG_FACILITY_USER
argument_list|,
literal|0
argument_list|)
expr_stmt|;
comment|/* Read per-user configuration file. */
name|snprintf
argument_list|(
name|buf
argument_list|,
sizeof|sizeof
name|buf
argument_list|,
literal|"%.100s/%.100s"
argument_list|,
name|pw
operator|->
name|pw_dir
argument_list|,
name|SSH_USER_CONFFILE
argument_list|)
expr_stmt|;
name|read_config_file
argument_list|(
name|buf
argument_list|,
name|host
argument_list|,
operator|&
name|options
argument_list|)
expr_stmt|;
comment|/* Read systemwide configuration file. */
name|read_config_file
argument_list|(
name|HOST_CONFIG_FILE
argument_list|,
name|host
argument_list|,
operator|&
name|options
argument_list|)
expr_stmt|;
comment|/* Fill configuration defaults. */
name|fill_default_options
argument_list|(
operator|&
name|options
argument_list|)
expr_stmt|;
comment|/* reinit */
name|log_init
argument_list|(
name|av
index|[
literal|0
index|]
argument_list|,
name|options
operator|.
name|log_level
argument_list|,
name|SYSLOG_FACILITY_USER
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|options
operator|.
name|user
operator|==
name|NULL
condition|)
name|options
operator|.
name|user
operator|=
name|xstrdup
argument_list|(
name|pw
operator|->
name|pw_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|options
operator|.
name|hostname
operator|!=
name|NULL
condition|)
name|host
operator|=
name|options
operator|.
name|hostname
expr_stmt|;
comment|/* Find canonic host name. */
if|if
condition|(
name|strchr
argument_list|(
name|host
argument_list|,
literal|'.'
argument_list|)
operator|==
literal|0
condition|)
block|{
name|struct
name|addrinfo
name|hints
decl_stmt|;
name|struct
name|addrinfo
modifier|*
name|ai
init|=
name|NULL
decl_stmt|;
name|int
name|errgai
decl_stmt|;
name|memset
argument_list|(
operator|&
name|hints
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|hints
argument_list|)
argument_list|)
expr_stmt|;
name|hints
operator|.
name|ai_family
operator|=
name|IPv4or6
expr_stmt|;
name|hints
operator|.
name|ai_flags
operator|=
name|AI_CANONNAME
expr_stmt|;
name|hints
operator|.
name|ai_socktype
operator|=
name|SOCK_STREAM
expr_stmt|;
name|errgai
operator|=
name|getaddrinfo
argument_list|(
name|host
argument_list|,
name|NULL
argument_list|,
operator|&
name|hints
argument_list|,
operator|&
name|ai
argument_list|)
expr_stmt|;
if|if
condition|(
name|errgai
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|ai
operator|->
name|ai_canonname
operator|!=
name|NULL
condition|)
name|host
operator|=
name|xstrdup
argument_list|(
name|ai
operator|->
name|ai_canonname
argument_list|)
expr_stmt|;
name|freeaddrinfo
argument_list|(
name|ai
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* Disable rhosts authentication if not running as root. */
if|if
condition|(
name|original_effective_uid
operator|!=
literal|0
operator|||
operator|!
name|options
operator|.
name|use_privileged_port
condition|)
block|{
name|options
operator|.
name|rhosts_authentication
operator|=
literal|0
expr_stmt|;
name|options
operator|.
name|rhosts_rsa_authentication
operator|=
literal|0
expr_stmt|;
block|}
comment|/* 	 * If using rsh has been selected, exec it now (without trying 	 * anything else).  Note that we must release privileges first. 	 */
if|if
condition|(
name|options
operator|.
name|use_rsh
condition|)
block|{
comment|/* 		 * Restore our superuser privileges.  This must be done 		 * before permanently setting the uid. 		 */
name|restore_uid
argument_list|()
expr_stmt|;
comment|/* Switch to the original uid permanently. */
name|permanently_set_uid
argument_list|(
name|original_real_uid
argument_list|)
expr_stmt|;
comment|/* Execute rsh. */
name|rsh_connect
argument_list|(
name|host
argument_list|,
name|options
operator|.
name|user
argument_list|,
operator|&
name|command
argument_list|)
expr_stmt|;
name|fatal
argument_list|(
literal|"rsh_connect returned"
argument_list|)
expr_stmt|;
block|}
comment|/* Restore our superuser privileges. */
name|restore_uid
argument_list|()
expr_stmt|;
comment|/* 	 * Open a connection to the remote host.  This needs root privileges 	 * if rhosts_{rsa_}authentication is enabled. 	 */
name|ok
operator|=
name|ssh_connect
argument_list|(
name|host
argument_list|,
operator|&
name|hostaddr
argument_list|,
name|options
operator|.
name|port
argument_list|,
name|options
operator|.
name|connection_attempts
argument_list|,
operator|!
name|options
operator|.
name|rhosts_authentication
operator|&&
operator|!
name|options
operator|.
name|rhosts_rsa_authentication
argument_list|,
name|original_real_uid
argument_list|,
name|options
operator|.
name|proxy_command
argument_list|)
expr_stmt|;
comment|/* 	 * If we successfully made the connection, load the host private key 	 * in case we will need it later for combined rsa-rhosts 	 * authentication. This must be done before releasing extra 	 * privileges, because the file is only readable by root. 	 */
if|if
condition|(
name|ok
condition|)
block|{
name|host_private_key
operator|=
name|RSA_new
argument_list|()
expr_stmt|;
if|if
condition|(
name|load_private_key
argument_list|(
name|HOST_KEY_FILE
argument_list|,
literal|""
argument_list|,
name|host_private_key
argument_list|,
name|NULL
argument_list|)
condition|)
name|host_private_key_loaded
operator|=
literal|1
expr_stmt|;
block|}
comment|/* 	 * Get rid of any extra privileges that we may have.  We will no 	 * longer need them.  Also, extra privileges could make it very hard 	 * to read identity files and other non-world-readable files from the 	 * user's home directory if it happens to be on a NFS volume where 	 * root is mapped to nobody. 	 */
comment|/* 	 * Note that some legacy systems need to postpone the following call 	 * to permanently_set_uid() until the private hostkey is destroyed 	 * with RSA_free().  Otherwise the calling user could ptrace() the 	 * process, read the private hostkey and impersonate the host. 	 * OpenBSD does not allow ptracing of setuid processes. 	 */
name|permanently_set_uid
argument_list|(
name|original_real_uid
argument_list|)
expr_stmt|;
comment|/* 	 * Now that we are back to our own permissions, create ~/.ssh 	 * directory if it doesn\'t already exist. 	 */
name|snprintf
argument_list|(
name|buf
argument_list|,
sizeof|sizeof
name|buf
argument_list|,
literal|"%.100s/%.100s"
argument_list|,
name|pw
operator|->
name|pw_dir
argument_list|,
name|SSH_USER_DIR
argument_list|)
expr_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|buf
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
if|if
condition|(
name|mkdir
argument_list|(
name|buf
argument_list|,
literal|0755
argument_list|)
operator|<
literal|0
condition|)
name|error
argument_list|(
literal|"Could not create directory '%.200s'."
argument_list|,
name|buf
argument_list|)
expr_stmt|;
comment|/* Check if the connection failed, and try "rsh" if appropriate. */
if|if
condition|(
operator|!
name|ok
condition|)
block|{
if|if
condition|(
name|options
operator|.
name|port
operator|!=
literal|0
condition|)
name|log
argument_list|(
literal|"Secure connection to %.100s on port %hu refused%.100s."
argument_list|,
name|host
argument_list|,
name|options
operator|.
name|port
argument_list|,
name|options
operator|.
name|fallback_to_rsh
condition|?
literal|"; reverting to insecure method"
else|:
literal|""
argument_list|)
expr_stmt|;
else|else
name|log
argument_list|(
literal|"Secure connection to %.100s refused%.100s."
argument_list|,
name|host
argument_list|,
name|options
operator|.
name|fallback_to_rsh
condition|?
literal|"; reverting to insecure method"
else|:
literal|""
argument_list|)
expr_stmt|;
if|if
condition|(
name|options
operator|.
name|fallback_to_rsh
condition|)
block|{
name|rsh_connect
argument_list|(
name|host
argument_list|,
name|options
operator|.
name|user
argument_list|,
operator|&
name|command
argument_list|)
expr_stmt|;
name|fatal
argument_list|(
literal|"rsh_connect returned"
argument_list|)
expr_stmt|;
block|}
name|exit
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
comment|/* Expand ~ in options.identity_files. */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|options
operator|.
name|num_identity_files
condition|;
name|i
operator|++
control|)
name|options
operator|.
name|identity_files
index|[
name|i
index|]
operator|=
name|tilde_expand_filename
argument_list|(
name|options
operator|.
name|identity_files
index|[
name|i
index|]
argument_list|,
name|original_real_uid
argument_list|)
expr_stmt|;
comment|/* Expand ~ in known host file names. */
name|options
operator|.
name|system_hostfile
operator|=
name|tilde_expand_filename
argument_list|(
name|options
operator|.
name|system_hostfile
argument_list|,
name|original_real_uid
argument_list|)
expr_stmt|;
name|options
operator|.
name|user_hostfile
operator|=
name|tilde_expand_filename
argument_list|(
name|options
operator|.
name|user_hostfile
argument_list|,
name|original_real_uid
argument_list|)
expr_stmt|;
comment|/* Log into the remote system.  This never returns if the login fails. */
name|ssh_login
argument_list|(
name|host_private_key_loaded
argument_list|,
name|host_private_key
argument_list|,
name|host
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
operator|&
name|hostaddr
argument_list|,
name|original_real_uid
argument_list|)
expr_stmt|;
comment|/* We no longer need the host private key.  Clear it now. */
if|if
condition|(
name|host_private_key_loaded
condition|)
name|RSA_free
argument_list|(
name|host_private_key
argument_list|)
expr_stmt|;
comment|/* Destroys contents safely */
comment|/* Close connection cleanly after attack. */
name|cipher_attack_detected
operator|=
name|packet_disconnect
expr_stmt|;
comment|/* Enable compression if requested. */
if|if
condition|(
name|options
operator|.
name|compression
condition|)
block|{
name|debug
argument_list|(
literal|"Requesting compression at level %d."
argument_list|,
name|options
operator|.
name|compression_level
argument_list|)
expr_stmt|;
if|if
condition|(
name|options
operator|.
name|compression_level
operator|<
literal|1
operator|||
name|options
operator|.
name|compression_level
operator|>
literal|9
condition|)
name|fatal
argument_list|(
literal|"Compression level must be from 1 (fast) to 9 (slow, best)."
argument_list|)
expr_stmt|;
comment|/* Send the request. */
name|packet_start
argument_list|(
name|SSH_CMSG_REQUEST_COMPRESSION
argument_list|)
expr_stmt|;
name|packet_put_int
argument_list|(
name|options
operator|.
name|compression_level
argument_list|)
expr_stmt|;
name|packet_send
argument_list|()
expr_stmt|;
name|packet_write_wait
argument_list|()
expr_stmt|;
name|type
operator|=
name|packet_read
argument_list|(
operator|&
name|plen
argument_list|)
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|SSH_SMSG_SUCCESS
condition|)
name|packet_start_compression
argument_list|(
name|options
operator|.
name|compression_level
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|type
operator|==
name|SSH_SMSG_FAILURE
condition|)
name|log
argument_list|(
literal|"Warning: Remote host refused compression."
argument_list|)
expr_stmt|;
else|else
name|packet_disconnect
argument_list|(
literal|"Protocol error waiting for compression response."
argument_list|)
expr_stmt|;
block|}
comment|/* Allocate a pseudo tty if appropriate. */
if|if
condition|(
name|tty_flag
condition|)
block|{
name|debug
argument_list|(
literal|"Requesting pty."
argument_list|)
expr_stmt|;
comment|/* Start the packet. */
name|packet_start
argument_list|(
name|SSH_CMSG_REQUEST_PTY
argument_list|)
expr_stmt|;
comment|/* Store TERM in the packet.  There is no limit on the 		   length of the string. */
name|cp
operator|=
name|getenv
argument_list|(
literal|"TERM"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|cp
condition|)
name|cp
operator|=
literal|""
expr_stmt|;
name|packet_put_string
argument_list|(
name|cp
argument_list|,
name|strlen
argument_list|(
name|cp
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Store window size in the packet. */
if|if
condition|(
name|ioctl
argument_list|(
name|fileno
argument_list|(
name|stdin
argument_list|)
argument_list|,
name|TIOCGWINSZ
argument_list|,
operator|&
name|ws
argument_list|)
operator|<
literal|0
condition|)
name|memset
argument_list|(
operator|&
name|ws
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|ws
argument_list|)
argument_list|)
expr_stmt|;
name|packet_put_int
argument_list|(
name|ws
operator|.
name|ws_row
argument_list|)
expr_stmt|;
name|packet_put_int
argument_list|(
name|ws
operator|.
name|ws_col
argument_list|)
expr_stmt|;
name|packet_put_int
argument_list|(
name|ws
operator|.
name|ws_xpixel
argument_list|)
expr_stmt|;
name|packet_put_int
argument_list|(
name|ws
operator|.
name|ws_ypixel
argument_list|)
expr_stmt|;
comment|/* Store tty modes in the packet. */
name|tty_make_modes
argument_list|(
name|fileno
argument_list|(
name|stdin
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Send the packet, and wait for it to leave. */
name|packet_send
argument_list|()
expr_stmt|;
name|packet_write_wait
argument_list|()
expr_stmt|;
comment|/* Read response from the server. */
name|type
operator|=
name|packet_read
argument_list|(
operator|&
name|plen
argument_list|)
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|SSH_SMSG_SUCCESS
condition|)
name|interactive
operator|=
literal|1
expr_stmt|;
elseif|else
if|if
condition|(
name|type
operator|==
name|SSH_SMSG_FAILURE
condition|)
name|log
argument_list|(
literal|"Warning: Remote host failed or refused to allocate a pseudo tty."
argument_list|)
expr_stmt|;
else|else
name|packet_disconnect
argument_list|(
literal|"Protocol error waiting for pty request response."
argument_list|)
expr_stmt|;
block|}
comment|/* Request X11 forwarding if enabled and DISPLAY is set. */
if|if
condition|(
name|options
operator|.
name|forward_x11
operator|&&
name|getenv
argument_list|(
literal|"DISPLAY"
argument_list|)
operator|!=
name|NULL
condition|)
block|{
name|char
name|line
index|[
literal|512
index|]
decl_stmt|,
name|proto
index|[
literal|512
index|]
decl_stmt|,
name|data
index|[
literal|512
index|]
decl_stmt|;
name|FILE
modifier|*
name|f
decl_stmt|;
name|int
name|forwarded
init|=
literal|0
decl_stmt|,
name|got_data
init|=
literal|0
decl_stmt|,
name|i
decl_stmt|;
ifdef|#
directive|ifdef
name|XAUTH_PATH
comment|/* Try to get Xauthority information for the display. */
name|snprintf
argument_list|(
name|line
argument_list|,
sizeof|sizeof
name|line
argument_list|,
literal|"%.100s list %.200s 2>/dev/null"
argument_list|,
name|XAUTH_PATH
argument_list|,
name|getenv
argument_list|(
literal|"DISPLAY"
argument_list|)
argument_list|)
expr_stmt|;
name|f
operator|=
name|popen
argument_list|(
name|line
argument_list|,
literal|"r"
argument_list|)
expr_stmt|;
if|if
condition|(
name|f
operator|&&
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|f
argument_list|)
operator|&&
name|sscanf
argument_list|(
name|line
argument_list|,
literal|"%*s %s %s"
argument_list|,
name|proto
argument_list|,
name|data
argument_list|)
operator|==
literal|2
condition|)
name|got_data
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|f
condition|)
name|pclose
argument_list|(
name|f
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* XAUTH_PATH */
comment|/* 		 * If we didn't get authentication data, just make up some 		 * data.  The forwarding code will check the validity of the 		 * response anyway, and substitute this data.  The X11 		 * server, however, will ignore this fake data and use 		 * whatever authentication mechanisms it was using otherwise 		 * for the local connection. 		 */
if|if
condition|(
operator|!
name|got_data
condition|)
block|{
name|u_int32_t
name|rand
init|=
literal|0
decl_stmt|;
name|strlcpy
argument_list|(
name|proto
argument_list|,
literal|"MIT-MAGIC-COOKIE-1"
argument_list|,
sizeof|sizeof
name|proto
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|16
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|i
operator|%
literal|4
operator|==
literal|0
condition|)
name|rand
operator|=
name|arc4random
argument_list|()
expr_stmt|;
name|snprintf
argument_list|(
name|data
operator|+
literal|2
operator|*
name|i
argument_list|,
sizeof|sizeof
name|data
operator|-
literal|2
operator|*
name|i
argument_list|,
literal|"%02x"
argument_list|,
name|rand
operator|&
literal|0xff
argument_list|)
expr_stmt|;
name|rand
operator|>>=
literal|8
expr_stmt|;
block|}
block|}
comment|/* 		 * Got local authentication reasonable information. Request 		 * forwarding with authentication spoofing. 		 */
name|debug
argument_list|(
literal|"Requesting X11 forwarding with authentication spoofing."
argument_list|)
expr_stmt|;
name|x11_request_forwarding_with_spoofing
argument_list|(
name|proto
argument_list|,
name|data
argument_list|)
expr_stmt|;
comment|/* Read response from the server. */
name|type
operator|=
name|packet_read
argument_list|(
operator|&
name|plen
argument_list|)
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|SSH_SMSG_SUCCESS
condition|)
block|{
name|forwarded
operator|=
literal|1
expr_stmt|;
name|interactive
operator|=
literal|1
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|type
operator|==
name|SSH_SMSG_FAILURE
condition|)
name|log
argument_list|(
literal|"Warning: Remote host denied X11 forwarding."
argument_list|)
expr_stmt|;
else|else
name|packet_disconnect
argument_list|(
literal|"Protocol error waiting for X11 forwarding"
argument_list|)
expr_stmt|;
block|}
comment|/* Tell the packet module whether this is an interactive session. */
name|packet_set_interactive
argument_list|(
name|interactive
argument_list|,
name|options
operator|.
name|keepalives
argument_list|)
expr_stmt|;
comment|/* Clear agent forwarding if we don\'t have an agent. */
name|authfd
operator|=
name|ssh_get_authentication_socket
argument_list|()
expr_stmt|;
if|if
condition|(
name|authfd
operator|<
literal|0
condition|)
name|options
operator|.
name|forward_agent
operator|=
literal|0
expr_stmt|;
else|else
name|ssh_close_authentication_socket
argument_list|(
name|authfd
argument_list|)
expr_stmt|;
comment|/* Request authentication agent forwarding if appropriate. */
if|if
condition|(
name|options
operator|.
name|forward_agent
condition|)
block|{
name|debug
argument_list|(
literal|"Requesting authentication agent forwarding."
argument_list|)
expr_stmt|;
name|auth_request_forwarding
argument_list|()
expr_stmt|;
comment|/* Read response from the server. */
name|type
operator|=
name|packet_read
argument_list|(
operator|&
name|plen
argument_list|)
expr_stmt|;
name|packet_integrity_check
argument_list|(
name|plen
argument_list|,
literal|0
argument_list|,
name|type
argument_list|)
expr_stmt|;
if|if
condition|(
name|type
operator|!=
name|SSH_SMSG_SUCCESS
condition|)
name|log
argument_list|(
literal|"Warning: Remote host denied authentication agent forwarding."
argument_list|)
expr_stmt|;
block|}
comment|/* Initiate local TCP/IP port forwardings. */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|options
operator|.
name|num_local_forwards
condition|;
name|i
operator|++
control|)
block|{
name|debug
argument_list|(
literal|"Connections to local port %d forwarded to remote address %.200s:%d"
argument_list|,
name|options
operator|.
name|local_forwards
index|[
name|i
index|]
operator|.
name|port
argument_list|,
name|options
operator|.
name|local_forwards
index|[
name|i
index|]
operator|.
name|host
argument_list|,
name|options
operator|.
name|local_forwards
index|[
name|i
index|]
operator|.
name|host_port
argument_list|)
expr_stmt|;
name|channel_request_local_forwarding
argument_list|(
name|options
operator|.
name|local_forwards
index|[
name|i
index|]
operator|.
name|port
argument_list|,
name|options
operator|.
name|local_forwards
index|[
name|i
index|]
operator|.
name|host
argument_list|,
name|options
operator|.
name|local_forwards
index|[
name|i
index|]
operator|.
name|host_port
argument_list|,
name|options
operator|.
name|gateway_ports
argument_list|)
expr_stmt|;
block|}
comment|/* Initiate remote TCP/IP port forwardings. */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|options
operator|.
name|num_remote_forwards
condition|;
name|i
operator|++
control|)
block|{
name|debug
argument_list|(
literal|"Connections to remote port %d forwarded to local address %.200s:%d"
argument_list|,
name|options
operator|.
name|remote_forwards
index|[
name|i
index|]
operator|.
name|port
argument_list|,
name|options
operator|.
name|remote_forwards
index|[
name|i
index|]
operator|.
name|host
argument_list|,
name|options
operator|.
name|remote_forwards
index|[
name|i
index|]
operator|.
name|host_port
argument_list|)
expr_stmt|;
name|channel_request_remote_forwarding
argument_list|(
name|options
operator|.
name|remote_forwards
index|[
name|i
index|]
operator|.
name|port
argument_list|,
name|options
operator|.
name|remote_forwards
index|[
name|i
index|]
operator|.
name|host
argument_list|,
name|options
operator|.
name|remote_forwards
index|[
name|i
index|]
operator|.
name|host_port
argument_list|)
expr_stmt|;
block|}
comment|/* If requested, let ssh continue in the background. */
if|if
condition|(
name|fork_after_authentication_flag
condition|)
if|if
condition|(
name|daemon
argument_list|(
literal|1
argument_list|,
literal|1
argument_list|)
operator|<
literal|0
condition|)
name|fatal
argument_list|(
literal|"daemon() failed: %.200s"
argument_list|,
name|strerror
argument_list|(
name|errno
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 	 * If a command was specified on the command line, execute the 	 * command now. Otherwise request the server to start a shell. 	 */
if|if
condition|(
name|buffer_len
argument_list|(
operator|&
name|command
argument_list|)
operator|>
literal|0
condition|)
block|{
name|int
name|len
init|=
name|buffer_len
argument_list|(
operator|&
name|command
argument_list|)
decl_stmt|;
if|if
condition|(
name|len
operator|>
literal|900
condition|)
name|len
operator|=
literal|900
expr_stmt|;
name|debug
argument_list|(
literal|"Sending command: %.*s"
argument_list|,
name|len
argument_list|,
name|buffer_ptr
argument_list|(
operator|&
name|command
argument_list|)
argument_list|)
expr_stmt|;
name|packet_start
argument_list|(
name|SSH_CMSG_EXEC_CMD
argument_list|)
expr_stmt|;
name|packet_put_string
argument_list|(
name|buffer_ptr
argument_list|(
operator|&
name|command
argument_list|)
argument_list|,
name|buffer_len
argument_list|(
operator|&
name|command
argument_list|)
argument_list|)
expr_stmt|;
name|packet_send
argument_list|()
expr_stmt|;
name|packet_write_wait
argument_list|()
expr_stmt|;
block|}
else|else
block|{
name|debug
argument_list|(
literal|"Requesting shell."
argument_list|)
expr_stmt|;
name|packet_start
argument_list|(
name|SSH_CMSG_EXEC_SHELL
argument_list|)
expr_stmt|;
name|packet_send
argument_list|()
expr_stmt|;
name|packet_write_wait
argument_list|()
expr_stmt|;
block|}
comment|/* Enter the interactive session. */
name|exit_status
operator|=
name|client_loop
argument_list|(
name|tty_flag
argument_list|,
name|tty_flag
condition|?
name|options
operator|.
name|escape_char
else|:
operator|-
literal|1
argument_list|)
expr_stmt|;
comment|/* Close the connection to the remote host. */
name|packet_close
argument_list|()
expr_stmt|;
comment|/* Exit with the status returned by the program on the remote side. */
name|exit
argument_list|(
name|exit_status
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

