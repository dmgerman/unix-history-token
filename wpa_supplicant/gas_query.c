...
begin_comment
comment|/*  * Generic advertisement service (GAS) query  * Copyright (c) 2009, Atheros Communications  * Copyright (c) 2011-2014, Qualcomm Atheros, Inc.  * Copyright (c) 2011-2014, Jouni Malinen<j@w1.fi>  *  * This software may be distributed under the terms of the BSD license.  * See README for more details.  */
end_comment
...
literal|"addr="
name|MACSTR
literal|" dialog_token=%u freq=%d status_code=%u result=%s"
argument_list|,
name|MAC2STR
argument_list|(
name|query
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: Unexpected TX status: freq=%u dst="
name|MACSTR
literal|" result=%d - no query in progress"
argument_list|,
name|freq
argument_list|,
name|MAC2STR
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: TX status: freq=%u dst="
name|MACSTR
literal|" result=%d query=%p dialog_token=%u dur=%d ms"
argument_list|,
name|freq
argument_list|,
name|MAC2STR
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: TX status for unexpected destination"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: Drop frame as possible "
literal|"retry of previous fragment"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: Drop unexpected protected GAS frame when PMF is disabled"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: Drop unexpected unprotected GAS frame when PMF is enabled"
argument_list|)
expr_stmt|;
return|return
literal|0
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: Response in %d ms from "
name|MACSTR
argument_list|,
name|ms_from_time
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: Allow non-zero status for outstanding comeback response"
argument_list|)
expr_stmt|;
block|}
elseif|else
...
name|MSG_INFO
argument_list|,
literal|"Failed to assign random MAC address for GAS"
argument_list|)
expr_stmt|;
name|gas_query_free
argument_list|(
...
name|MSG_DEBUG
argument_list|,
literal|"GAS: Starting query timeout for dialog token %u"
argument_list|,
name|query
operator|->
name|dialog_token
...
begin_comment
comment|/**  * gas_query_req - Request a GAS query  * @gas: GAS query data from gas_query_init()  * @dst: Destination MAC address for the query  * @freq: Frequency (in MHz) for the channel on which to send the query  * @req: GAS query payload (to be freed by gas_query module in case of success  *	return)  * @cb: Callback function for reporting GAS query result and response  * @ctx: Context pointer to use with the @cb call  * Returns: dialog token (>= 0) on success or -1 on failure  */
end_comment
