...
name|pool
decl_stmt|;
comment|/* Mapping (const char *) wcroot_abspath to svn_wc__db_commit_queue_t * */
name|apr_hash_t
modifier|*
name|wc_queues
decl_stmt|;
...
name|recurse
decl_stmt|;
comment|/* Use legacy recursion */
name|svn_boolean_t
name|committed
decl_stmt|;
comment|/* Process the node as committed */
name|svn_boolean_t
name|remove_lock
decl_stmt|;
comment|/* Remove existing lock on node */
name|svn_boolean_t
name|remove_changelist
decl_stmt|;
comment|/* Remove changelist on node */
comment|/* The pristine text checksum. NULL if the old value should be kept      and for directories */
specifier|const
name|svn_checksum_t
modifier|*
...
name|new_dav_cache
decl_stmt|;
comment|/* New DAV cache for the node */
block|}
name|committed_queue_item_t
typedef|;
end_typedef
...
literal|0
expr_stmt|;
comment|/* Process the wc's in order of their paths. */
name|wcs
operator|=
name|svn_sort__hash
argument_list|(
