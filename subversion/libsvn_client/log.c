...
argument_list|)
expr_stmt|;
comment|/* For some peg revisions we must resolve revision and url via a local path      so use the original RA_TARGET. For others, use the potentially corrected      (redirected) ra session URL. */
if|if
condition|(
name|peg_rev
operator|.
