...
literal|0
argument_list|,
literal|"Multiple `DEFAULT' lines (%d and %d) in %s file"
argument_list|,
name|default_line
argument_list|,
name|line_number
...
begin_comment
comment|/* Parse the CVS config file.  The syntax right now is a bit ad hoc    but tries to draw on the best or more common features of the other    *info files and various unix (or non-unix) config file syntaxes.    Lines starting with # are comments.  Settings are lines of the form    KEYWORD=VALUE.  There is currently no way to have a multi-line    VALUE (would be nice if there was, probably).     CVSROOT is the $CVSROOT directory    (current_parsed_root->directory might not be set yet, so this    function takes the cvsroot as a function argument).     Returns 0 for success, negative value for failure.  Call    error(0, ...) on errors in addition to the return value.  */
end_comment
...
comment|/* Strip the trailing newline.  There will be one unless we 	   read a partial line without a newline, and then got end of 	   file (or error?).  */
name|len
operator|=
name|strlen
argument_list|(
...
literal|0
argument_list|,
literal|"unrecognized value '%s' for TopLevelAdmin"
argument_list|,
name|p
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Could try some validity checking, like whether we can 	       opendir it or something, but I don't see any particular 	       reason to do that now rather than waiting until lock.c.  */
block|}
elseif|else
if|if
condition|(
...
name|line
argument_list|,
literal|"LocalKeyword"
argument_list|)
operator|==
literal|0
condition|)
block|{
comment|/* Recognize cvs-1.12-style keyword control rather than erroring out. */
name|RCS_setlocalid
argument_list|(
name|p
argument_list|)
