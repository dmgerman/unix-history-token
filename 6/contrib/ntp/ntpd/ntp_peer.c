...
argument_list|)
expr_stmt|;
comment|/* XXXMEMLEAK? peer_clear->crypto allocation */
name|hash
operator|=
name|NTP_HASH_ADDR
argument_list|(
...
condition|)
block|{
comment|/* 			 * don't accept updates to a true multicast reception 			 * interface while a BCLNT peer is running it's 			 * unicast protocol 			 */
return|return;
block|}
if|if
condition|(
...
argument_list|,
operator|(
literal|"set_peerdstadr(%s): change interface from %s to %s\n"
operator|,
name|stoa
argument_list|(
operator|&
...
name|printf
argument_list|(
literal|"fd=%d, bfd=%d, name=%.16s, flags=0x%x, scope=%d, "
argument_list|,
name|niface
operator|->
name|fd
...
name|printf
argument_list|(
literal|"Found *-cast interface address %s, for address %s\n"
argument_list|,
name|stoa
argument_list|(
operator|&
...
argument_list|)
expr_stmt|;
comment|/* set up interface link chain */
name|peer
operator|->
name|srcadr
operator|=
