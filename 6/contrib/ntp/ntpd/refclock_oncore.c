...
operator|)
argument_list|,
literal|"PPSAPI not found in kernel"
argument_list|)
expr_stmt|;
return|return
operator|(
...
name|Msg
argument_list|,
literal|"Initializing timeing to %s."
argument_list|,
name|cp
argument_list|)
expr_stmt|;
...
name|Msg
argument_list|,
literal|"Can't set timeing to %s, exiting..."
argument_list|,
name|cp
argument_list|)
expr_stmt|;
...
name|Msg
argument_list|,
literal|"Can't set PPS_%sCLEAR, this will increase jitter."
argument_list|,
name|cp
argument_list|)
expr_stmt|;
...
operator|)
argument_list|,
literal|"ONCORE: Can't malloc buffer for shmem_old"
argument_list|)
expr_stmt|;
else|else
name|read
...
operator|)
argument_list|,
literal|"ONCORE: Can't malloc buffer for shmem"
argument_list|)
expr_stmt|;
name|close
argument_list|(
...
name|a_lastcode
argument_list|,
comment|/* MAX length 128, currently at 121 */
...
directive|if
literal|0
block|{ 		char Msg[160];  		sprintf(Msg, "ONCORE[%d]: DEBUG BITS: (%x %x), (%x %x %x),  %x %x %x %x %x\n", 		instance->unit, 		instance->BEHa[129], instance->BEHa[130], bits1, bits2, bits3, instance->mode == MODE_0D, 		instance->mode == MODE_2D, instance->mode == MODE_3D, 		instance->rsm.bad_almanac, instance->rsm.bad_fix); 		record_clock_stats(&(instance->peer->srcadr), Msg); }
endif|#
directive|endif
block|}
block|}
...
directive|if
literal|0
block|{ char Msg[160]; sprintf(Msg, "SHMEM posn = %ld (%d, %d, %d)", (long) (cp-instance->shmem), ii, jj, kk); record_clock_stats(&(instance->peer->srcadr), Msg); }
endif|#
directive|endif
if|if
condition|(
