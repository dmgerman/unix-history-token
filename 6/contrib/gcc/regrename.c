...
name|rtl_dump_file
argument_list|,
literal|"; no available better choice\n"
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
name|cc0_rtx
expr_stmt|;
comment|/* For match_dup of match_operator or match_parallel, share 		 them, so that we don't miss changes in the dup.  */
if|if
condition|(
name|icode
operator|>=
...
parameter_list|)
block|{
comment|/* SUBREGS are supposed to have been eliminated by now.  But some      ports, e.g. i386 sse, use them to smuggle vector type information      through to instruction selection.  Each such SUBREG should simplify,      so if we get a NULL  we've done something wrong elsewhere.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
condition|)
return|return;
comment|/* Do not propagate copies to the stack pointer, as that can leave      memory accesses with no scheduling dependency on the stack update.  */
if|if
condition|(
name|dr
operator|==
...
begin_comment
comment|/* Register REGNO was originally set in ORIG_MODE.  It - or a copy of it -    was copied in COPY_MODE to COPY_REGNO, and then COPY_REGNO was accessed    in NEW_MODE.    Return a NEW_MODE rtx for REGNO if that's OK, otherwise return NULL_RTX.  */
end_comment
...
comment|/* ??? Ought to use more intelligent queuing of blocks.  */
if|if
condition|(
name|bb
operator|->
