...
begin_comment
comment|/* Add ELEM wrapped in an INSN_LIST with reg note kind DEP_TYPE to the    LOG_LINKS of INSN, if not already there.  DEP_TYPE indicates the    type of dependence that this link represents.  The function returns    nonzero if a new entry has been added to insn's LOG_LINK.  */
end_comment
...
block|if (GET_CODE (insn) == CALL_INSN&& (INSN_BB (elem) != INSN_BB (insn)))     return 0;
endif|#
directive|endif
...
argument|i
argument_list|,
argument|{ 	      struct deps_reg *reg_last =&deps->reg_last[i]; 	      add_dependence_list (insn, reg_last->sets, REG_DEP_ANTI); 	      add_dependence_list (insn, reg_last->clobbers, REG_DEP_ANTI); 	      reg_last->uses_length++; 	      reg_last->uses = alloc_INSN_LIST (insn, reg_last->uses); 	    }
argument_list|)
empty_stmt|;
name|IOR_REG_SET
argument_list|(
...
argument|i
argument_list|,
argument|{ 	      struct deps_reg *reg_last =&deps->reg_last[i]; 	      add_dependence_list (insn, reg_last->uses, REG_DEP_ANTI); 	      add_dependence_list 		(insn, reg_last->sets, 		 reg_pending_barrier == TRUE_BARRIER ?
literal|0
argument|: REG_DEP_ANTI); 	      add_dependence_list 		(insn, reg_last->clobbers, 		 reg_pending_barrier == TRUE_BARRIER ?
literal|0
argument|: REG_DEP_ANTI); 	    }
argument_list|)
empty_stmt|;
block|}
else|else
...
argument|i
argument_list|,
argument|{ 	      struct deps_reg *reg_last =&deps->reg_last[i]; 	      add_dependence_list_and_free (insn,&reg_last->uses, 					    REG_DEP_ANTI); 	      add_dependence_list_and_free 		(insn,&reg_last->sets, 		 reg_pending_barrier == TRUE_BARRIER ?
literal|0
argument|: REG_DEP_ANTI); 	      add_dependence_list_and_free 		(insn,&reg_last->clobbers, 		 reg_pending_barrier == TRUE_BARRIER ?
literal|0
argument|: REG_DEP_ANTI); 	      reg_last->uses_length =
literal|0
