...
specifier|static
name|int
name|secondary_memlocs_elim_used
init|=
literal|0
decl_stmt|;
end_decl_stmt
...
condition|)
block|{
comment|/* If IN_P is nonzero, the reload register will be the output in 	 operand 0.  If IN_P is zero, the reload register will be the input 	 in operand 1.  Outputs should have an initial "=", which we must 	 skip.  */
name|enum
name|reg_class
name|insn_class
decl_stmt|;
...
name|struct
name|recog_data
name|save_recog_data
decl_stmt|;
comment|/* For matching constraints, we often get notional input reloads where      we want to use the original register as the reload register.  I.e.      technically this is a non-optional input-output reload, but IN is      already a valid register, and has been chosen as the reload register.      Speed this up, since it trivially works.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
literal|1
return|;
comment|/* To test MEMs properly, we'd have to take into account all the reloads      that are already scheduled, which can become quite complicated.      And since we've already handled address reloads for this MEM, it      should always succeed anyway.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
literal|1
return|;
comment|/* If we can make a simple SET insn that does the job, everything should      be fine.  */
name|dst
operator|=
name|gen_rtx_REG
argument_list|(
...
begin_comment
comment|/* Duplicate any replacement we have recorded to apply at    location ORIG_LOC to also be performed at DUP_LOC.    This is used in insn patterns that use match_dup.  */
end_comment
...
begin_comment
comment|/* IN_RTX is the value loaded by a reload that we now decided to inherit,    or a subpart of it.  If we have any replacements registered for IN_RTX,    cancel the reloads that were supposed to load them.    Return nonzero if we canceled any reloads.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* On a WORDS_BIG_ENDIAN machine, point to the last register of a 	 multiple hard register group of scalar integer registers, so that 	 for example (reg:DI 0) and (reg:SI 1) will be considered the same 	 register.  */
if|if
condition|(
...
return|;
block|}
comment|/* If two operands must match, because they are really a single      operand of an assembler insn, then two postincrements are invalid      because the assembler insn would increment only once.      On the other hand, a postincrement matches ordinary indexing      if the postincrement is the output operand.  */
if|if
condition|(
name|code
operator|==
...
argument_list|)
return|;
comment|/* Two preincrements are invalid      because the assembler insn would increment only once.      On the other hand, a preincrement matches ordinary indexing      if the preincrement is the input operand.      In this case, return 2, since some callers need to do special      things when this happens.  */
if|if
condition|(
name|GET_CODE
argument_list|(
...
index|]
decl_stmt|;
comment|/* Nonzero for an address operand that needs to be completely reloaded.  */
name|int
name|address_operand_reloaded
index|[
name|MAX_RECOG_OPERANDS
index|]
decl_stmt|;
...
argument_list|()
expr_stmt|;
comment|/* We currently only support one commutative pair of 		 operands.  Some existing asm code currently uses more 		 than one pair.  Previously, that would usually work, 		 but sometimes it would crash the compiler.  We 		 continue supporting that case as well as we can by 		 silently ignoring all but the first pair.  In the 		 future we may handle it correctly.  */
if|if
condition|(
name|commutative
operator|<
...
index|]
expr_stmt|;
comment|/* Address operands are reloaded in their existing mode, 	     no matter what is specified in the machine description.  */
name|operand_mode
index|[
name|i
index|]
...
literal|'%'
case|:
comment|/* We only support one commutative marker, the first 		   one.  We already set commutative above.  */
break|break;
case|case
literal|'?'
case|:
...
literal|1
expr_stmt|;
comment|/* But count the pair only once in the total badness of 		       this alternative, if the pair can be a dummy reload. 		       The pointers in operand_loc are not swapped; swap 		       them by hand if necessary.  */
if|if
condition|(
name|swapped
operator|&&
...
literal|1
expr_stmt|;
comment|/* If the address was already reloaded, 			   we win as well.  */
elseif|else
if|if
condition|(
name|GET_CODE
...
literal|1
expr_stmt|;
comment|/* Likewise if the address will be reloaded because 			   reg_equiv_address is nonzero.  For reg_equiv_mem 			   we have to check.  */
elseif|else
if|if
condition|(
name|GET_CODE
...
literal|1
expr_stmt|;
comment|/* If we didn't already win, we can reload 			   constants via force_const_mem, and other 			   MEMs by reloading the address like for 'o'.  */
if|if
condition|(
operator|(
name|CONSTANT_P
...
literal|1
expr_stmt|;
comment|/* If we didn't already win, we can reload 			   the address into a base register.  */
name|this_alternative
index|[
name|i
index|]
...
begin_comment
comment|/* Returns true if AD could be turned into a valid memory reference    to mode MODE by reloading the part pointed to by PART into a    register.  */
end_comment
...
expr_stmt|;
block|}
comment|/* Plus in the index register may be created only as a result of 	   register remateralization for expression like&localvar*4.  Reload it. 	   It may be possible to combine the displacement on the outer level, 	   but it is probably not worthwhile to do so.  */
if|if
condition|(
name|context
condition|)
...
name|offset
decl_stmt|;
comment|/* For big-endian paradoxical subregs, SUBREG_BYTE does not 		 hold the correct (negative) byte offset.  */
if|if
condition|(
name|BYTES_BIG_ENDIAN
operator|&&
...
condition|)
block|{
comment|/* We actually want to know if X is mentioned somewhere inside IN. 	 We must not say that (plus (sp) (const_int 124)) is in 	 (plus (sp) (const_int 64)), since that can lead to incorrect reload 	 allocation when spuriously changing a RELOAD_FOR_OUTPUT_ADDRESS 	 into a RELOAD_OTHER on behalf of another RELOAD_OTHER.  */
while|while
condition|(
name|GET_CODE
argument_list|(
...
name|PARAM_VALUE
argument_list|(
name|PARAM_MAX_RELOAD_SEARCH_INSNS
argument_list|)
condition|)
return|return
literal|0
...
begin_comment
comment|/* Find the low part, with mode MODE, of a hard regno RELOADREG.  */
end_comment
