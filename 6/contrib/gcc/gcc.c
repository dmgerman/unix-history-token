...
directive|define
name|LINK_COMMAND_SPEC
value|"\ %{!fsyntax-only:%{!c:%{!M:%{!MM:%{!E:%{!S:\     %(linker) %l " LINK_PIE_SPEC "%X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} %{r}\     %{s} %{t} %{u*} %{x} %{z} %{Z} %{!A:%{!nostdlib:%{!nostartfiles:%S}}}\     %{static:} %{L*} %(link_libgcc) %o %{fprofile-arcs|fprofile-generate:-lgcov}\     %{!nostdlib:%{!nodefaultlibs:%(link_gcc_c_sequence)}}\     %{!A:%{!nostdlib:%{!nostartfiles:%E}}} %{T*} }}}}}}"
end_define
...
literal|"%{E|M|MM:%(trad_capable_cpp) %(cpp_options) %(cpp_debug_options)}\       %{!E:%{!M:%{!MM:\           %{traditional|ftraditional:\ %eGNU C no longer supports -traditional without -E}\ 	  %{save-temps|traditional-cpp|no-integrated-cpp:%(trad_capable_cpp) \ 		%(cpp_options) -o %{save-temps:%b.i} %{!save-temps:%g.i} \n\ 		    cc1 -fpreprocessed %{save-temps:%b.i} %{!save-temps:%g.i} \ 			%(cc1_options)}\ 	  %{!save-temps:%{!traditional-cpp:%{!no-integrated-cpp:\ 		cc1 %(cpp_unique_options) %(cc1_options)}}}\         %{!fsyntax-only:%(invoke_as)}}}}"
block|,
literal|0
block|}
block|,
...
begin_comment
comment|/* Build a list of search directories from PATHS.    PREFIX is a string to prepend to the list.    If CHECK_DIR_P is nonzero we ensure the directory exists.    This is used mostly by putenv_from_prefixes so we use `collect_obstack'.    It is also used by the --print-search-dirs flag.  */
end_comment
...
block|}
block|}
comment|/* Now try just the multilib_name.  */
name|strcpy
argument_list|(
name|temp
argument_list|,
...
block|}
block|}
comment|/* If there is a -V or -b option (or both), process it now, before      trying to interpret the rest of the command line.  */
if|if
condition|(
name|argc
operator|>
...
begin_comment
comment|/* Inline subroutine of handle_braces.  Returns true if a switch    matching the atom bracketed by ATOM and END_ATOM appeared on the    command line.  */
end_comment
...
block|add_prefix (&startfile_prefixes, "./", NULL, 		  PREFIX_PRIORITY_LAST, 1, NULL, 0);
endif|#
directive|endif
block|}
...
name|fatal
argument_list|(
literal|"cannot specify -o with -c or -S and multiple languages"
argument_list|)
expr_stmt|;
else|else
block|{
