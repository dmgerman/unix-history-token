begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/* $OpenBSD: buffer.c,v 1.31 2006/08/03 03:34:41 deraadt Exp $ */
end_comment

begin_comment
comment|/*  * Author: Tatu Ylonen<ylo@cs.hut.fi>  * Copyright (c) 1995 Tatu Ylonen<ylo@cs.hut.fi>, Espoo, Finland  *                    All rights reserved  * Functions for manipulating fifo buffers (that can grow if needed).  *  * As far as I am concerned, the code I have written for this software  * can be used freely for any purpose.  Any derived versions of this  * software must be clearly marked as such, and if the derived work is  * incompatible with the protocol description in the RFC file, it must be  * called by a name other than "ssh" or "Secure Shell".  */
end_comment

begin_include
include|#
directive|include
file|"includes.h"
end_include

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<stdio.h>
end_include

begin_include
include|#
directive|include
file|<string.h>
end_include

begin_include
include|#
directive|include
file|<stdarg.h>
end_include

begin_include
include|#
directive|include
file|"xmalloc.h"
end_include

begin_include
include|#
directive|include
file|"buffer.h"
end_include

begin_include
include|#
directive|include
file|"log.h"
end_include

begin_define
define|#
directive|define
name|BUFFER_MAX_CHUNK
value|0x100000
end_define

begin_define
define|#
directive|define
name|BUFFER_MAX_LEN
value|0xa00000
end_define

begin_define
define|#
directive|define
name|BUFFER_ALLOCSZ
value|0x008000
end_define

begin_comment
comment|/* Initializes the buffer structure. */
end_comment

begin_function
name|void
name|buffer_init
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|)
block|{
specifier|const
name|u_int
name|len
init|=
literal|4096
decl_stmt|;
name|buffer
operator|->
name|alloc
operator|=
literal|0
expr_stmt|;
name|buffer
operator|->
name|buf
operator|=
name|xmalloc
argument_list|(
name|len
argument_list|)
expr_stmt|;
name|buffer
operator|->
name|alloc
operator|=
name|len
expr_stmt|;
name|buffer
operator|->
name|offset
operator|=
literal|0
expr_stmt|;
name|buffer
operator|->
name|end
operator|=
literal|0
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Frees any memory used for the buffer. */
end_comment

begin_function
name|void
name|buffer_free
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|)
block|{
if|if
condition|(
name|buffer
operator|->
name|alloc
operator|>
literal|0
condition|)
block|{
name|memset
argument_list|(
name|buffer
operator|->
name|buf
argument_list|,
literal|0
argument_list|,
name|buffer
operator|->
name|alloc
argument_list|)
expr_stmt|;
name|buffer
operator|->
name|alloc
operator|=
literal|0
expr_stmt|;
name|xfree
argument_list|(
name|buffer
operator|->
name|buf
argument_list|)
expr_stmt|;
block|}
block|}
end_function

begin_comment
comment|/*  * Clears any data from the buffer, making it empty.  This does not actually  * zero the memory.  */
end_comment

begin_function
name|void
name|buffer_clear
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|)
block|{
name|buffer
operator|->
name|offset
operator|=
literal|0
expr_stmt|;
name|buffer
operator|->
name|end
operator|=
literal|0
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Appends data to the buffer, expanding it if necessary. */
end_comment

begin_function
name|void
name|buffer_append
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
specifier|const
name|void
modifier|*
name|data
parameter_list|,
name|u_int
name|len
parameter_list|)
block|{
name|void
modifier|*
name|p
decl_stmt|;
name|p
operator|=
name|buffer_append_space
argument_list|(
name|buffer
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|memcpy
argument_list|(
name|p
argument_list|,
name|data
argument_list|,
name|len
argument_list|)
expr_stmt|;
block|}
end_function

begin_function
specifier|static
name|int
name|buffer_compact
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|)
block|{
comment|/* 	 * If the buffer is quite empty, but all data is at the end, move the 	 * data to the beginning. 	 */
if|if
condition|(
name|buffer
operator|->
name|offset
operator|>
name|MIN
argument_list|(
name|buffer
operator|->
name|alloc
argument_list|,
name|BUFFER_MAX_CHUNK
argument_list|)
condition|)
block|{
name|memmove
argument_list|(
name|buffer
operator|->
name|buf
argument_list|,
name|buffer
operator|->
name|buf
operator|+
name|buffer
operator|->
name|offset
argument_list|,
name|buffer
operator|->
name|end
operator|-
name|buffer
operator|->
name|offset
argument_list|)
expr_stmt|;
name|buffer
operator|->
name|end
operator|-=
name|buffer
operator|->
name|offset
expr_stmt|;
name|buffer
operator|->
name|offset
operator|=
literal|0
expr_stmt|;
return|return
operator|(
literal|1
operator|)
return|;
block|}
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * Appends space to the buffer, expanding the buffer if necessary. This does  * not actually copy the data into the buffer, but instead returns a pointer  * to the allocated region.  */
end_comment

begin_function
name|void
modifier|*
name|buffer_append_space
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|u_int
name|len
parameter_list|)
block|{
name|u_int
name|newlen
decl_stmt|;
name|void
modifier|*
name|p
decl_stmt|;
if|if
condition|(
name|len
operator|>
name|BUFFER_MAX_CHUNK
condition|)
name|fatal
argument_list|(
literal|"buffer_append_space: len %u not supported"
argument_list|,
name|len
argument_list|)
expr_stmt|;
comment|/* If the buffer is empty, start using it from the beginning. */
if|if
condition|(
name|buffer
operator|->
name|offset
operator|==
name|buffer
operator|->
name|end
condition|)
block|{
name|buffer
operator|->
name|offset
operator|=
literal|0
expr_stmt|;
name|buffer
operator|->
name|end
operator|=
literal|0
expr_stmt|;
block|}
name|restart
label|:
comment|/* If there is enough space to store all data, store it now. */
if|if
condition|(
name|buffer
operator|->
name|end
operator|+
name|len
operator|<
name|buffer
operator|->
name|alloc
condition|)
block|{
name|p
operator|=
name|buffer
operator|->
name|buf
operator|+
name|buffer
operator|->
name|end
expr_stmt|;
name|buffer
operator|->
name|end
operator|+=
name|len
expr_stmt|;
return|return
name|p
return|;
block|}
comment|/* Compact data back to the start of the buffer if necessary */
if|if
condition|(
name|buffer_compact
argument_list|(
name|buffer
argument_list|)
condition|)
goto|goto
name|restart
goto|;
comment|/* Increase the size of the buffer and retry. */
name|newlen
operator|=
name|roundup
argument_list|(
name|buffer
operator|->
name|alloc
operator|+
name|len
argument_list|,
name|BUFFER_ALLOCSZ
argument_list|)
expr_stmt|;
if|if
condition|(
name|newlen
operator|>
name|BUFFER_MAX_LEN
condition|)
name|fatal
argument_list|(
literal|"buffer_append_space: alloc %u not supported"
argument_list|,
name|newlen
argument_list|)
expr_stmt|;
name|buffer
operator|->
name|buf
operator|=
name|xrealloc
argument_list|(
name|buffer
operator|->
name|buf
argument_list|,
literal|1
argument_list|,
name|newlen
argument_list|)
expr_stmt|;
name|buffer
operator|->
name|alloc
operator|=
name|newlen
expr_stmt|;
goto|goto
name|restart
goto|;
comment|/* NOTREACHED */
block|}
end_function

begin_comment
comment|/*  * Check whether an allocation of 'len' will fit in the buffer  * This must follow the same math as buffer_append_space  */
end_comment

begin_function
name|int
name|buffer_check_alloc
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|u_int
name|len
parameter_list|)
block|{
if|if
condition|(
name|buffer
operator|->
name|offset
operator|==
name|buffer
operator|->
name|end
condition|)
block|{
name|buffer
operator|->
name|offset
operator|=
literal|0
expr_stmt|;
name|buffer
operator|->
name|end
operator|=
literal|0
expr_stmt|;
block|}
name|restart
label|:
if|if
condition|(
name|buffer
operator|->
name|end
operator|+
name|len
operator|<
name|buffer
operator|->
name|alloc
condition|)
return|return
operator|(
literal|1
operator|)
return|;
if|if
condition|(
name|buffer_compact
argument_list|(
name|buffer
argument_list|)
condition|)
goto|goto
name|restart
goto|;
if|if
condition|(
name|roundup
argument_list|(
name|buffer
operator|->
name|alloc
operator|+
name|len
argument_list|,
name|BUFFER_ALLOCSZ
argument_list|)
operator|<=
name|BUFFER_MAX_LEN
condition|)
return|return
operator|(
literal|1
operator|)
return|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_comment
comment|/* Returns the number of bytes of data in the buffer. */
end_comment

begin_function
name|u_int
name|buffer_len
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|)
block|{
return|return
name|buffer
operator|->
name|end
operator|-
name|buffer
operator|->
name|offset
return|;
block|}
end_function

begin_comment
comment|/* Gets data from the beginning of the buffer. */
end_comment

begin_function
name|int
name|buffer_get_ret
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|void
modifier|*
name|buf
parameter_list|,
name|u_int
name|len
parameter_list|)
block|{
if|if
condition|(
name|len
operator|>
name|buffer
operator|->
name|end
operator|-
name|buffer
operator|->
name|offset
condition|)
block|{
name|error
argument_list|(
literal|"buffer_get_ret: trying to get more bytes %d than in buffer %d"
argument_list|,
name|len
argument_list|,
name|buffer
operator|->
name|end
operator|-
name|buffer
operator|->
name|offset
argument_list|)
expr_stmt|;
return|return
operator|(
operator|-
literal|1
operator|)
return|;
block|}
name|memcpy
argument_list|(
name|buf
argument_list|,
name|buffer
operator|->
name|buf
operator|+
name|buffer
operator|->
name|offset
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|buffer
operator|->
name|offset
operator|+=
name|len
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_function
name|void
name|buffer_get
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|void
modifier|*
name|buf
parameter_list|,
name|u_int
name|len
parameter_list|)
block|{
if|if
condition|(
name|buffer_get_ret
argument_list|(
name|buffer
argument_list|,
name|buf
argument_list|,
name|len
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"buffer_get: buffer error"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Consumes the given number of bytes from the beginning of the buffer. */
end_comment

begin_function
name|int
name|buffer_consume_ret
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|u_int
name|bytes
parameter_list|)
block|{
if|if
condition|(
name|bytes
operator|>
name|buffer
operator|->
name|end
operator|-
name|buffer
operator|->
name|offset
condition|)
block|{
name|error
argument_list|(
literal|"buffer_consume_ret: trying to get more bytes than in buffer"
argument_list|)
expr_stmt|;
return|return
operator|(
operator|-
literal|1
operator|)
return|;
block|}
name|buffer
operator|->
name|offset
operator|+=
name|bytes
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_function
name|void
name|buffer_consume
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|u_int
name|bytes
parameter_list|)
block|{
if|if
condition|(
name|buffer_consume_ret
argument_list|(
name|buffer
argument_list|,
name|bytes
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"buffer_consume: buffer error"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Consumes the given number of bytes from the end of the buffer. */
end_comment

begin_function
name|int
name|buffer_consume_end_ret
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|u_int
name|bytes
parameter_list|)
block|{
if|if
condition|(
name|bytes
operator|>
name|buffer
operator|->
name|end
operator|-
name|buffer
operator|->
name|offset
condition|)
return|return
operator|(
operator|-
literal|1
operator|)
return|;
name|buffer
operator|->
name|end
operator|-=
name|bytes
expr_stmt|;
return|return
operator|(
literal|0
operator|)
return|;
block|}
end_function

begin_function
name|void
name|buffer_consume_end
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|,
name|u_int
name|bytes
parameter_list|)
block|{
if|if
condition|(
name|buffer_consume_end_ret
argument_list|(
name|buffer
argument_list|,
name|bytes
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|fatal
argument_list|(
literal|"buffer_consume_end: trying to get more bytes than in buffer"
argument_list|)
expr_stmt|;
block|}
end_function

begin_comment
comment|/* Returns a pointer to the first used byte in the buffer. */
end_comment

begin_function
name|void
modifier|*
name|buffer_ptr
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|)
block|{
return|return
name|buffer
operator|->
name|buf
operator|+
name|buffer
operator|->
name|offset
return|;
block|}
end_function

begin_comment
comment|/* Dumps the contents of the buffer to stderr. */
end_comment

begin_function
name|void
name|buffer_dump
parameter_list|(
name|Buffer
modifier|*
name|buffer
parameter_list|)
block|{
name|u_int
name|i
decl_stmt|;
name|u_char
modifier|*
name|ucp
init|=
name|buffer
operator|->
name|buf
decl_stmt|;
for|for
control|(
name|i
operator|=
name|buffer
operator|->
name|offset
init|;
name|i
operator|<
name|buffer
operator|->
name|end
condition|;
name|i
operator|++
control|)
block|{
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"%02x"
argument_list|,
name|ucp
index|[
name|i
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|i
operator|-
name|buffer
operator|->
name|offset
operator|)
operator|%
literal|16
operator|==
literal|15
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\r\n"
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
operator|(
name|i
operator|-
name|buffer
operator|->
name|offset
operator|)
operator|%
literal|2
operator|==
literal|1
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|" "
argument_list|)
expr_stmt|;
block|}
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"\r\n"
argument_list|)
expr_stmt|;
block|}
end_function

end_unit

