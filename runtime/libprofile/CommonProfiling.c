...
begin_comment
comment|/* check_environment_variable - Check to see if the LLVMPROF_OUTPUT environment  * variable is set.  If it is then save it and set OutputFilename.  */
end_comment

begin_function
specifier|static
name|void
name|check_environment_variable
parameter_list|(
name|void
parameter_list|)
block|{
...
condition|)
return|return;
comment|/* Guarantee that we can't leak memory. */
if|if
condition|(
operator|(
name|EnvVar
...
condition|)
block|{
comment|/* The string that getenv returns is allowed to be statically allocated,      * which means it may be changed by future calls to getenv, so copy it.      */
name|SavedEnvVar
operator|=
name|strdup
argument_list|(
