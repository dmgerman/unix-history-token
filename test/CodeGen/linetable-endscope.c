...
begin_comment
comment|// RUN: %clang_cc1 -emit-llvm -debug-info-kind=limited -triple x86_64-apple-darwin10 %s -o - | FileCheck %s
end_comment
...
name|i
decl_stmt|;
comment|// CHECK: ![[CONV]] = !DILocation(line: [[@LINE+1]], scope: !{{.*}})
name|i
operator|=
name|c
expr_stmt|;
comment|// CHECK: ![[RET]] = !DILocation(line: [[@LINE+1]], scope: !{{.*}})
block|}
end_function
