...
begin_comment
comment|// RUN: %llvmgcc %s -S -fno-math-errno -O0 -o - | grep {call.*exp2\\.f32}
end_comment

begin_comment
comment|// RUN: %llvmgcc %s -S -fmath-errno -O0 -o - | grep {call.*exp2f}
end_comment
...
begin_comment
comment|// RUN: %llvmgcc %s -S -O3 -fno-builtin -o - | grep {call.*exp2f}
end_comment
