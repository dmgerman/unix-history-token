...
begin_comment
comment|// RUN: %clang_cc1 -analyze -analyzer-checker=core,alpha.deadcode.UnreachableCode,alpha.core.CastSize,unix.Malloc,debug.ExprInspection -analyzer-store=region -verify %s
end_comment
...
expr_stmt|;
return|return;
comment|// expected-warning {{Memory is never released; potential leak of memory pointed to by 'hp.p'}}
block|}
end_function
...
begin_comment
comment|// expected-warning {{Memory is never released; potential leak of memory pointed to by 'ss.p'}}
end_comment
...
argument_list|)
decl_stmt|;
comment|// expected-warning {{Memory is never released; potential leak of memory pointed to by 'string'}}
block|}
end_function
