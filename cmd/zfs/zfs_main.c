...
name|gettext
argument_list|(
literal|"\tdestroy [-rRf]<filesystem|volume>\n"
literal|"\tdestroy [-rRd]<snapshot>\n"
argument_list|)
operator|)
return|;
case|case
...
name|cb_defer_destroy
decl_stmt|;
comment|/* 			 * Temporarily ignore the defer_destroy setting since 			 * it's not supported for clones. 			 */
name|cb
operator|.
name|cb_defer_destroy
operator|=
...
literal|"unique identifier (FUID)\n"
argument_list|)
argument_list|)
expr_stmt|;
operator|(
...
name|error
decl_stmt|;
comment|/* 	 * Try the python version.  If the execv fails, we'll continue 	 * and do a simplistic implementation. 	 */
operator|(
name|void
operator|)
name|execv
...
begin_comment
comment|/*  * zfs send [-vDp] -R [-i|-I<@snap>]<fs@snap>  * zfs send [-vDp] [-i|-I<@snap>]<fs@snap>  *  * Send a backup stream to stdout.  */
end_comment
