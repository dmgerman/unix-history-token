...
begin_comment
comment|/*  * Copyright (c) 2005, 2010, Oracle and/or its affiliates. All rights reserved.  * Copyright 2012 Nexenta Systems, Inc. All rights reserved.  * Copyright (c) 2012 by Delphix. All rights reserved.  * Copyright 2012 Milan Jurik. All rights reserved.  * Copyright (c) 2012, Joyent, Inc. All rights reserved.  */
end_comment
...
name|gettext
argument_list|(
literal|"\tsend [-DnPpRrv] [-[iI] snapshot] "
literal|"<snapshot>\n"
argument_list|)
operator|)
return|;
...
directive|define
name|USTYPE_ALL
value|(USTYPE_PSX_USR | USTYPE_SMB_USR \ 		| USTYPE_PSX_GRP | USTYPE_SMB_GRP)
end_define

begin_define
define|#
directive|define
name|USPROP_USED_BIT
value|(0)
end_define
...
block|}
block|}
comment|/*  *	if (prop == ZFS_PROP_GROUPUSED || prop == ZFS_PROP_GROUPQUOTA)  *		ug = "group";  *	else  *		ug = "user";  */
if|if
condition|(
name|prop
operator|==
...
directive|define
name|USFIELD_ALL
value|(USFIELD_TYPE | USFIELD_NAME | USFIELD_USED | USFIELD_QUOTA)
end_define
...
name|optind
expr_stmt|;
comment|/* ok, now we have sorted by default colums (type,name) avl tree */
if|if
condition|(
name|sortcol
condition|)
...
expr_stmt|;
block|}
comment|/* print& free node`s nvlist memory */
name|print_us
argument_list|(
name|scripted
argument_list|,
...
begin_comment
comment|/*  * zfs receive [-vnFu] [-d | -e]<fs@snap>  *  * Restore a backup stream from stdin.  */
end_comment
...
begin_comment
comment|/*  * zfs allow [-r] [-t]<tag><snap> ...  *  *	-r	Recursively hold  *	-t	Temporary hold (hidden option)  *  * Apply a user-hold with the given tag to the list of snapshots.  */
end_comment
...
begin_comment
comment|/*  * zfs unallow [-r] [-t]<tag><snap> ...  *  *	-r	Recursively hold  *	-t	Temporary hold (hidden option)  *  * Apply a user-hold with the given tag to the list of snapshots.  */
end_comment
...
begin_comment
comment|/*  * zfs hold [-r] [-t]<tag><snap> ...  *  *	-r	Recursively hold  *	-t	Temporary hold (hidden option)  *  * Apply a user-hold with the given tag to the list of snapshots.  */
end_comment
