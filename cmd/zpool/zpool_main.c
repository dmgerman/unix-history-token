...
begin_comment
comment|/*  * zpool reopen<pool>  *  * Reopen the pool so that the kernel can update the sizes of all vdevs.  *  * NOTE: This command is currently undocumented.  If the command is ever  * exposed then the appropriate usage() messages will need to be made.  */
end_comment
...
condition|)
continue|continue;
comment|/* is it an internal event or a standard event? */
if|if
condition|(
name|nvlist_lookup_string
argument_list|(
