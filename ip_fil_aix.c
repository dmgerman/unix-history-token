begin_unit|revision:0.9.5;language:C;cregit-version:0.0.1
begin_comment
comment|/*  * Copyright (C) 2012 by Darren Reed.  *  * See the IPFILTER.LICENCE file for details on licencing.  */
end_comment

begin_define
define|#
directive|define
name|__FULL_PROTO
end_define

begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|lint
argument_list|)
end_if

begin_decl_stmt
specifier|static
specifier|const
name|char
name|sccsid
index|[]
init|=
literal|"@(#)ip_fil.c	2.41 6/5/96 (C) 1993-2000 Darren Reed"
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|static
specifier|const
name|char
name|rcsid
index|[]
init|=
literal|"@(#)$Id$"
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_if
if|#
directive|if
name|defined
argument_list|(
name|KERNEL
argument_list|)
operator|||
name|defined
argument_list|(
name|_KERNEL
argument_list|)
end_if

begin_undef
undef|#
directive|undef
name|KERNEL
end_undef

begin_undef
undef|#
directive|undef
name|_KERNEL
end_undef

begin_define
define|#
directive|define
name|KERNEL
value|1
end_define

begin_define
define|#
directive|define
name|_KERNEL
value|1
end_define

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/param.h>
end_include

begin_include
include|#
directive|include
file|<sys/errno.h>
end_include

begin_include
include|#
directive|include
file|<sys/types.h>
end_include

begin_include
include|#
directive|include
file|<sys/file.h>
end_include

begin_include
include|#
directive|include
file|<sys/ioctl.h>
end_include

begin_include
include|#
directive|include
file|<sys/time.h>
end_include

begin_include
include|#
directive|include
file|<sys/systm.h>
end_include

begin_include
include|#
directive|include
file|<sys/dir.h>
end_include

begin_include
include|#
directive|include
file|<sys/mbuf.h>
end_include

begin_include
include|#
directive|include
file|<sys/protosw.h>
end_include

begin_include
include|#
directive|include
file|<sys/socket.h>
end_include

begin_include
include|#
directive|include
file|<sys/device.h>
end_include

begin_include
include|#
directive|include
file|<net/if.h>
end_include

begin_include
include|#
directive|include
file|<net/route.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_var.h>
end_include

begin_include
include|#
directive|include
file|<netinet/in_systm.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip_var.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcp_var.h>
end_include

begin_include
include|#
directive|include
file|<netinet/udp.h>
end_include

begin_include
include|#
directive|include
file|<netinet/tcpip.h>
end_include

begin_include
include|#
directive|include
file|<netinet/ip_icmp.h>
end_include

begin_include
include|#
directive|include
file|"netinet/ip_compat.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|USE_INET6
end_ifdef

begin_include
include|#
directive|include
file|<netinet/icmp6.h>
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"netinet/ip_fil.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_nat.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_frag.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_state.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_proxy.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_auth.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_sync.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_lookup.h"
end_include

begin_include
include|#
directive|include
file|"netinet/ip_dstlist.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|IPFILTER_SCAN
end_ifdef

begin_include
include|#
directive|include
file|"netinet/ip_scan.h"
end_include

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|"netinet/ip_pool.h"
end_include

begin_ifdef
ifdef|#
directive|ifdef
name|INET
end_ifdef

begin_decl_stmt
specifier|extern
name|int
name|ip_optcopy
name|__P
argument_list|(
operator|(
expr|struct
name|ip
operator|*
operator|,
expr|struct
name|ip
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* ipstat inbound_fw nethsque rw_read_locked ip_output netisr_dispatch ipforwarding ip_output_post_fw ipintr_noqueue_post_fw max_linkhdr in_localaddr ip_optcopy outbound_fw */
end_comment

begin_decl_stmt
specifier|static
name|u_short
name|ipid
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_expr_stmt
specifier|static
name|int
argument_list|(
argument|*fr_savep
argument_list|)
name|__P
argument_list|(
operator|(
name|ip_t
operator|*
operator|,
name|int
operator|,
name|void
operator|*
operator|,
name|int
operator|,
expr|struct
name|mbuf
operator|*
operator|*
operator|)
argument_list|)
expr_stmt|;
end_expr_stmt

begin_decl_stmt
specifier|static
name|int
name|fr_send_ip
name|__P
argument_list|(
operator|(
name|fr_info_t
operator|*
operator|,
name|mb_t
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_ifdef
ifdef|#
directive|ifdef
name|KMUTEX_T
end_ifdef

begin_decl_stmt
specifier|extern
name|ipfmutex_t
name|ipf_rw
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|extern
name|ipfrwlock_t
name|ipf_mutex
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_ifdef
ifdef|#
directive|ifdef
name|USE_INET6
end_ifdef

begin_decl_stmt
specifier|static
name|int
name|ipfr_fastroute6
name|__P
argument_list|(
operator|(
expr|struct
name|mbuf
operator|*
operator|,
expr|struct
name|mbuf
operator|*
operator|*
operator|,
name|fr_info_t
operator|*
operator|,
name|frdest_t
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_endif
endif|#
directive|endif
end_endif

begin_include
include|#
directive|include
file|<sys/conf.h>
end_include

begin_include
include|#
directive|include
file|<sys/device.h>
end_include

begin_decl_stmt
name|void
name|ipf_check_inbound
name|__P
argument_list|(
operator|(
expr|struct
name|ifnet
operator|*
operator|,
expr|struct
name|mbuf
operator|*
operator|,
name|inbound_fw_args_t
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipf_check_outbound
name|__P
argument_list|(
operator|(
expr|struct
name|ifnet
operator|*
operator|,
expr|struct
name|mbuf
operator|*
operator|,
name|outbound_fw_args_t
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipfopen
name|__P
argument_list|(
operator|(
name|dev_t
operator|,
name|u_long
operator|,
name|chan_t
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipfclose
name|__P
argument_list|(
operator|(
name|dev_t
operator|,
name|chan_t
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipfread
name|__P
argument_list|(
operator|(
name|dev_t
operator|,
expr|struct
name|uio
operator|*
operator|,
name|chan_t
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipfwrite
name|__P
argument_list|(
operator|(
name|dev_t
operator|,
expr|struct
name|uio
operator|*
operator|,
name|chan_t
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipfioctl
name|__P
argument_list|(
operator|(
name|dev_t
operator|,
name|int
operator|,
name|caddr_t
operator|,
name|int
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipfconfig
name|__P
argument_list|(
operator|(
name|dev_t
operator|,
name|int
operator|,
expr|struct
name|uio
operator|*
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|ipfmutex_t
name|ipl_mutex
decl_stmt|,
name|ipf_auth_mx
decl_stmt|,
name|ipf_rw
decl_stmt|,
name|ipf_stinsert
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|ipfmutex_t
name|ipf_nat_new
decl_stmt|,
name|ipf_natio
decl_stmt|,
name|ipf_timeoutlock
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|ipfrwlock_t
name|ipf_mutex
decl_stmt|,
name|ipf_global
decl_stmt|,
name|ipf_ipidfrag
decl_stmt|,
name|ipf_tokens
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|ipfrwlock_t
name|ipf_frag
decl_stmt|,
name|ipf_state
decl_stmt|,
name|ipf_nat
decl_stmt|,
name|ipf_natfrag
decl_stmt|,
name|ipf_authlk
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|int
name|ipf_locks_done
init|=
literal|0
decl_stmt|;
end_decl_stmt

begin_decl_stmt
specifier|const
name|struct
name|devsw
name|ipfdevsw
init|=
block|{
name|ipfopen
block|,
comment|/* d_open	entry point for open routine */
name|ipfclose
block|,
comment|/* d_close	entry point for close routine */
name|ipfread
block|,
comment|/* d_read	entry point for read routine */
name|ipfwrite
block|,
comment|/* d_write	entry point for write routine */
name|ipfioctl
block|,
comment|/* d_ioctl	entry point for ioctl routine */
name|nodev
block|,
comment|/* d_strategy	entry point for strategy routine */
name|NULL
block|,
comment|/* d_ttys	pointer to tty device structure */
name|nodev
block|,
comment|/* d_select	entry point for select routine */
name|ipfconfig
block|,
comment|/* d_config	entry point for config routine */
name|nodev
block|,
comment|/* d_print	entry point for print routine */
name|nodev
block|,
comment|/* d_dump	entry point for dump routine */
name|nodev
block|,
comment|/* d_mpx	entry point for mpx routine */
name|nodev
block|,
comment|/* d_revoke	entry point for revoke routine */
name|NULL
block|,
comment|/* d_dsdptr     pointer to device specific data */
literal|0
block|,
comment|/* d_selptr	ptr to outstanding select cntl blks */
ifdef|#
directive|ifdef
name|_IA64
name|DEV_MPSAFE
operator||
name|DEV_64BIT
else|#
directive|else
literal|0
comment|/* d_opts	internal device switch control field */
endif|#
directive|endif
block|}
decl_stmt|;
end_decl_stmt

begin_function
name|int
name|ipfconfig
parameter_list|(
name|devno
parameter_list|,
name|cmd
parameter_list|,
name|uiop
parameter_list|)
name|dev_t
name|devno
decl_stmt|;
name|int
name|cmd
decl_stmt|;
name|struct
name|uio
modifier|*
name|uiop
decl_stmt|;
block|{
name|int
name|error
init|=
name|EINVAL
decl_stmt|;
name|printf
argument_list|(
literal|"ipfconfig(%u,%x,%p)\n"
argument_list|,
name|devno
argument_list|,
name|cmd
argument_list|,
name|uiop
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|cmd
condition|)
block|{
case|case
name|CFG_INIT
case|:
name|error
operator|=
name|devswadd
argument_list|(
name|devno
argument_list|,
operator|&
name|ipfdevsw
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|==
literal|0
condition|)
block|{
name|ipf_load_all
argument_list|()
expr_stmt|;
name|ipf_create_all
argument_list|(
operator|&
name|ipfmain
argument_list|)
expr_stmt|;
name|error
operator|=
name|ipfattach
argument_list|(
operator|&
name|ipfmain
argument_list|)
expr_stmt|;
block|}
break|break;
case|case
name|CFG_TERM
case|:
name|error
operator|=
name|devswdel
argument_list|(
name|devno
argument_list|)
expr_stmt|;
break|break;
case|case
name|CFG_QVPD
case|:
name|error
operator|=
literal|0
expr_stmt|;
break|break;
default|default :
return|return
name|EINVAL
return|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_function
name|int
name|ipfattach
parameter_list|(
name|softc
parameter_list|)
name|ipf_main_softc_t
modifier|*
name|softc
decl_stmt|;
block|{
name|int
name|s
decl_stmt|;
name|SPL_NET
argument_list|(
name|s
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|ipf_running
operator|>
literal|0
operator|)
operator|||
operator|(
name|inbound_fw
operator|==
name|ipf_check_inbound
operator|)
condition|)
block|{
name|printf
argument_list|(
literal|"IP Filter: already initialized\n"
argument_list|)
expr_stmt|;
name|SPL_X
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
name|EBUSY
return|;
block|}
name|inbound_fw
operator|=
name|ipf_check_inbound
expr_stmt|;
name|outbound_fw
operator|=
name|ipf_check_outbound
expr_stmt|;
if|if
condition|(
name|fr_control_forwarding
operator|&
literal|1
condition|)
name|ipforwarding
operator|=
literal|1
expr_stmt|;
name|ipid
operator|=
literal|0
expr_stmt|;
name|ipf_init_all
argument_list|(
name|softc
argument_list|)
expr_stmt|;
name|SPL_X
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

begin_comment
comment|/*  * Disable the filter by removing the hooks from the IP input/output  * stream.  */
end_comment

begin_function
name|int
name|ipfdetach
parameter_list|(
name|softc
parameter_list|)
name|ipf_main_softc_t
modifier|*
name|softc
decl_stmt|;
block|{
name|int
name|s
decl_stmt|;
name|SPL_NET
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|inbound_fw
operator|=
name|NULL
expr_stmt|;
name|outbound_fw
operator|=
name|NULL
expr_stmt|;
operator|(
name|void
operator|)
name|frflush
argument_list|(
name|IPL_LOGIPF
argument_list|,
literal|0
argument_list|,
name|FR_INQUE
operator||
name|FR_OUTQUE
operator||
name|FR_INACTIVE
argument_list|)
expr_stmt|;
operator|(
name|void
operator|)
name|frflush
argument_list|(
name|IPL_LOGIPF
argument_list|,
literal|0
argument_list|,
name|FR_INQUE
operator||
name|FR_OUTQUE
argument_list|)
expr_stmt|;
if|if
condition|(
name|fr_control_forwarding
operator|&
literal|2
condition|)
name|ipforwarding
operator|=
literal|0
expr_stmt|;
name|ipf_fini_all
argument_list|(
name|softc
argument_list|)
expr_stmt|;
name|SPL_X
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function

begin_comment
comment|/*  * explicit inbound hook to call ipf_check from  */
end_comment

begin_function
name|void
name|ipf_check_inbound
parameter_list|(
name|ifp
parameter_list|,
name|m
parameter_list|,
name|args
parameter_list|)
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|inbound_fw_args_t
modifier|*
name|args
decl_stmt|;
block|{
name|ip_t
modifier|*
name|ip
decl_stmt|;
if|if
condition|(
name|ipf_check_mbuf
argument_list|(
operator|&
name|m
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
if|if
condition|(
name|m
operator|!=
name|NULL
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
return|return;
block|}
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|ip_t
operator|*
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|ipf_check
argument_list|(
name|ip
argument_list|,
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
argument_list|,
name|ifp
argument_list|,
literal|0
argument_list|,
operator|&
name|m
argument_list|)
condition|)
block|{
case|case
literal|0
case|:
name|ipintr_noqueue_post_fw
argument_list|(
name|ifp
argument_list|,
name|m
argument_list|,
name|args
argument_list|)
expr_stmt|;
break|break;
default|default :
if|if
condition|(
name|m
operator|!=
name|NULL
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
break|break;
block|}
return|return;
block|}
end_function

begin_comment
comment|/*  * explicit outbound hook to call ipf_check from  */
end_comment

begin_function
name|int
name|ipf_check_outbound
parameter_list|(
name|ifp
parameter_list|,
name|m
parameter_list|,
name|args
parameter_list|)
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|outbound_fw_args_t
modifier|*
name|args
decl_stmt|;
block|{
name|ip_t
modifier|*
name|ip
decl_stmt|;
if|if
condition|(
name|ipf_check_mbuf
argument_list|(
operator|&
name|m
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
if|if
condition|(
name|m
operator|!=
name|NULL
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
return|return
literal|1
return|;
comment|/* FIREWALL_NOTOK */
block|}
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|ip_t
operator|*
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|ipf_check
argument_list|(
name|ip
argument_list|,
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
argument_list|,
name|ifp
argument_list|,
literal|1
argument_list|,
operator|&
name|m
argument_list|)
condition|)
block|{
case|case
literal|0
case|:
name|ip_output_post_fw
argument_list|(
name|ifp
argument_list|,
name|m
argument_list|,
name|args
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
comment|/* FIREWALL_OK */
default|default :
break|break;
block|}
if|if
condition|(
name|m
operator|!=
name|NULL
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
return|return
literal|1
return|;
comment|/* FIREWALL_NOTOK */
block|}
end_function

begin_function
name|int
name|ipf_check_mbuf
parameter_list|(
name|mp
parameter_list|)
name|struct
name|mbuf
modifier|*
modifier|*
name|mp
decl_stmt|;
block|{
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|,
modifier|*
name|m0
decl_stmt|;
name|int
name|i
decl_stmt|,
name|hlen
decl_stmt|;
name|ip_t
modifier|*
name|ip
decl_stmt|;
name|m
operator|=
operator|*
name|mp
expr_stmt|;
if|if
condition|(
operator|(
name|m
operator|->
name|m_len
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
operator|)
operator|&&
operator|(
name|m
operator|=
name|m_pullup
argument_list|(
name|m
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
operator|*
name|mp
operator|=
name|m
expr_stmt|;
name|ipstat
operator|.
name|ips_toosmall
operator|++
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|ip
operator|->
name|ip_v
condition|)
block|{
case|case
literal|4
case|:
name|hlen
operator|=
name|ip
operator|->
name|ip_hl
operator|<<
literal|2
expr_stmt|;
if|if
condition|(
name|hlen
operator|<
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
condition|)
block|{
comment|/* minimum header length */
name|ipstat
operator|.
name|ips_badhlen
operator|++
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
if|if
condition|(
operator|(
name|hlen
operator|>
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
operator|)
operator|&&
operator|(
name|hlen
operator|>
name|m
operator|->
name|m_len
operator|)
condition|)
block|{
if|if
condition|(
operator|(
name|m
operator|=
name|m_pullup
argument_list|(
name|m
argument_list|,
name|hlen
argument_list|)
operator|)
operator|==
literal|0
condition|)
block|{
operator|*
name|mp
operator|=
name|m
expr_stmt|;
name|ipstat
operator|.
name|ips_badhlen
operator|++
expr_stmt|;
return|return
operator|-
literal|2
return|;
block|}
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|ip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|hlen
argument_list|)
condition|)
block|{
name|ipstat
operator|.
name|ips_badsum
operator|++
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
comment|/* 		 * Convert fields to host representation. 		 * XXX - no need for NTOHS on big endian (sparc) 		 */
name|NTOHS
argument_list|(
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
if|if
condition|(
name|ip
operator|->
name|ip_len
operator|<
name|hlen
condition|)
block|{
name|ipstat
operator|.
name|ips_badlen
operator|++
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|NTOHS
argument_list|(
name|ip
operator|->
name|ip_id
argument_list|)
expr_stmt|;
name|NTOHS
argument_list|(
name|ip
operator|->
name|ip_off
argument_list|)
expr_stmt|;
comment|/* 		 * Check that the amount of data in the buffers 		 * is as at least much as the IP header would have us expect. 		 * Trim mbufs if longer than we expect. 		 * Drop packet if shorter than we expect. 		 */
name|i
operator|=
operator|-
operator|(
name|u_short
operator|)
name|ip
operator|->
name|ip_len
expr_stmt|;
name|m0
operator|=
name|m
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|i
operator|+=
name|m
operator|->
name|m_len
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_next
operator|==
literal|0
condition|)
break|break;
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
block|}
if|if
condition|(
name|i
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|i
operator|<
literal|0
condition|)
block|{
name|ipstat
operator|.
name|ips_tooshort
operator|++
expr_stmt|;
name|m
operator|=
name|m0
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
if|if
condition|(
name|i
operator|<=
name|m
operator|->
name|m_len
condition|)
name|m
operator|->
name|m_len
operator|-=
name|i
expr_stmt|;
else|else
name|m_adj
argument_list|(
name|m0
argument_list|,
operator|-
name|i
argument_list|)
expr_stmt|;
block|}
break|break;
ifdef|#
directive|ifdef
name|USE_INET6
case|case
literal|6
case|:
break|break;
endif|#
directive|endif
default|default :
return|return
operator|-
literal|1
return|;
block|}
return|return
literal|0
return|;
block|}
end_function

begin_comment
comment|/*  * Filter ioctl interface.  */
end_comment

begin_function
name|int
name|ipfioctl
parameter_list|(
name|dev
parameter_list|,
name|cmd
parameter_list|,
name|data
parameter_list|,
name|mode
parameter_list|)
name|dev_t
name|dev
decl_stmt|;
name|int
name|cmd
decl_stmt|;
name|caddr_t
name|data
decl_stmt|;
name|int
name|mode
decl_stmt|;
block|{
name|int
name|error
init|=
literal|0
decl_stmt|,
name|unit
init|=
literal|0
decl_stmt|;
name|SPL_INT
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|unit
operator|=
name|GET_MINOR
argument_list|(
name|dev
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|IPL_LOGMAX
operator|<
name|unit
operator|)
operator|||
operator|(
name|unit
operator|<
literal|0
operator|)
condition|)
block|{
name|ipfmain
operator|.
name|ipf_interror
operator|=
literal|130002
expr_stmt|;
return|return
name|ENXIO
return|;
block|}
if|if
condition|(
name|ipf_running
operator|<=
literal|0
condition|)
block|{
if|if
condition|(
name|unit
operator|!=
name|IPL_LOGIPF
operator|&&
name|cmd
operator|!=
name|SIOCIPFINTERROR
condition|)
block|{
name|ipfmain
operator|.
name|ipf_interror
operator|=
literal|130003
expr_stmt|;
return|return
name|EIO
return|;
block|}
if|if
condition|(
name|cmd
operator|!=
name|SIOCIPFGETNEXT
operator|&&
name|cmd
operator|!=
name|SIOCIPFGET
operator|&&
name|cmd
operator|!=
name|SIOCIPFSET
operator|&&
name|cmd
operator|!=
name|SIOCFRENB
operator|&&
name|cmd
operator|!=
name|SIOCGETFS
operator|&&
name|cmd
operator|!=
name|SIOCGETFF
operator|&&
name|cmd
operator|!=
name|SIOCIPFINTERROR
condition|)
block|{
name|ipfmain
operator|.
name|ipf_interror
operator|=
literal|130004
expr_stmt|;
return|return
name|EIO
return|;
block|}
block|}
name|SPL_NET
argument_list|(
name|s
argument_list|)
expr_stmt|;
name|error
operator|=
name|ipf_ioctlswitch
argument_list|(
name|unit
argument_list|,
name|data
argument_list|,
name|cmd
argument_list|,
name|mode
argument_list|,
name|curproc
operator|->
name|p_uid
argument_list|,
name|curproc
argument_list|)
expr_stmt|;
if|if
condition|(
name|error
operator|!=
operator|-
literal|1
condition|)
block|{
name|SPL_X
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
name|error
return|;
block|}
name|SPL_X
argument_list|(
name|s
argument_list|)
expr_stmt|;
return|return
name|error
return|;
block|}
end_function

begin_comment
comment|/*  * routines below for saving IP headers to buffer  */
end_comment

begin_function
name|int
name|ipfopen
parameter_list|(
name|dev_t
name|dev
parameter_list|,
name|u_long
name|flags
parameter_list|,
name|chan_t
name|chan
parameter_list|,
name|int
name|ext
parameter_list|)
block|{
name|u_int
name|unit
init|=
name|GET_MINOR
argument_list|(
name|dev
argument_list|)
decl_stmt|;
name|int
name|error
decl_stmt|;
if|if
condition|(
name|IPL_LOGMAX
operator|<
name|unit
condition|)
name|error
operator|=
name|ENXIO
expr_stmt|;
else|else
block|{
switch|switch
condition|(
name|unit
condition|)
block|{
case|case
name|IPL_LOGIPF
case|:
case|case
name|IPL_LOGNAT
case|:
case|case
name|IPL_LOGSTATE
case|:
case|case
name|IPL_LOGAUTH
case|:
case|case
name|IPL_LOGLOOKUP
case|:
case|case
name|IPL_LOGSYNC
case|:
ifdef|#
directive|ifdef
name|IPFILTER_SCAN
case|case
name|IPL_LOGSCAN
case|:
endif|#
directive|endif
name|error
operator|=
literal|0
expr_stmt|;
break|break;
default|default :
name|error
operator|=
name|ENXIO
expr_stmt|;
break|break;
block|}
block|}
return|return
name|error
return|;
block|}
end_function

begin_function
name|int
name|ipfclose
parameter_list|(
name|dev_t
name|dev
parameter_list|,
name|chan_t
name|chan
parameter_list|)
block|{
name|u_int
name|unit
init|=
name|GET_MINOR
argument_list|(
name|dev
argument_list|)
decl_stmt|;
if|if
condition|(
name|IPL_LOGMAX
operator|<
name|unit
condition|)
name|unit
operator|=
name|ENXIO
expr_stmt|;
else|else
name|unit
operator|=
literal|0
expr_stmt|;
return|return
name|unit
return|;
block|}
end_function

begin_comment
comment|/*  * ipfread/ipllog  * both of these must operate with at least splnet() lest they be  * called during packet processing and cause an inconsistancy to appear in  * the filter lists.  */
end_comment

begin_function
name|int
name|ipfread
parameter_list|(
name|dev_t
name|dev
parameter_list|,
name|struct
name|uio
modifier|*
name|uio
parameter_list|,
name|chan_t
name|chan
parameter_list|,
name|int
name|ext
parameter_list|)
block|{
if|if
condition|(
name|ipf_running
operator|<
literal|1
condition|)
block|{
name|ipfmain
operator|.
name|ipf_interror
operator|=
literal|130006
expr_stmt|;
return|return
name|EIO
return|;
block|}
if|if
condition|(
name|GET_MINOR
argument_list|(
name|dev
argument_list|)
operator|==
name|IPL_LOGSYNC
condition|)
return|return
name|ipfsync_read
argument_list|(
name|uio
argument_list|)
return|;
ifdef|#
directive|ifdef
name|IPFILTER_LOG
return|return
name|ipflog_read
argument_list|(
name|GET_MINOR
argument_list|(
name|dev
argument_list|)
argument_list|,
name|uio
argument_list|)
return|;
else|#
directive|else
name|ipfmain
operator|.
name|ipf_interror
operator|=
literal|130007
expr_stmt|;
return|return
name|ENXIO
return|;
endif|#
directive|endif
block|}
end_function

begin_comment
comment|/*  * ipfwrite  * both of these must operate with at least splnet() lest they be  * called during packet processing and cause an inconsistancy to appear in  * the filter lists.  */
end_comment

begin_function
name|int
name|ipfwrite
parameter_list|(
name|dev_t
name|dev
parameter_list|,
name|struct
name|uio
modifier|*
name|uio
parameter_list|,
name|chan_t
name|chan
parameter_list|,
name|int
name|ext
parameter_list|)
block|{
if|if
condition|(
name|GET_MINOR
argument_list|(
name|dev
argument_list|)
operator|==
name|IPL_LOGSYNC
condition|)
return|return
name|ipfsync_write
argument_list|(
name|uio
argument_list|)
return|;
name|ipfmain
operator|.
name|ipf_interror
operator|=
literal|130009
expr_stmt|;
return|return
name|ENXIO
return|;
block|}
end_function

begin_comment
comment|/*  * ipf_send_reset - this could conceivably be a call to tcp_respond(), but that  * requires a large amount of setting up and isn't any more efficient.  */
end_comment

begin_function
name|int
name|ipf_send_reset
parameter_list|(
name|fin
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
block|{
name|struct
name|tcphdr
modifier|*
name|tcp
decl_stmt|,
modifier|*
name|tcp2
decl_stmt|;
name|int
name|tlen
init|=
literal|0
decl_stmt|,
name|hlen
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
name|ip6_t
modifier|*
name|ip6
decl_stmt|;
endif|#
directive|endif
name|ip_t
modifier|*
name|ip
decl_stmt|;
name|tcp
operator|=
name|fin
operator|->
name|fin_dp
expr_stmt|;
if|if
condition|(
name|tcp
operator|->
name|th_flags
operator|&
name|TH_RST
condition|)
return|return
operator|-
literal|1
return|;
comment|/* feedback loop */
if|if
condition|(
name|ipf_checkl4sum
argument_list|(
name|fin
argument_list|)
operator|==
operator|-
literal|1
condition|)
return|return
operator|-
literal|1
return|;
name|tlen
operator|=
name|fin
operator|->
name|fin_dlen
operator|-
operator|(
name|TCP_OFF
argument_list|(
name|tcp
argument_list|)
operator|<<
literal|2
operator|)
operator|+
operator|(
operator|(
name|tcp
operator|->
name|th_flags
operator|&
name|TH_SYN
operator|)
condition|?
literal|1
else|:
literal|0
operator|)
operator|+
operator|(
operator|(
name|tcp
operator|->
name|th_flags
operator|&
name|TH_FIN
operator|)
condition|?
literal|1
else|:
literal|0
operator|)
expr_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
name|hlen
operator|=
operator|(
name|fin
operator|->
name|fin_v
operator|==
literal|6
operator|)
condition|?
sizeof|sizeof
argument_list|(
name|ip6_t
argument_list|)
else|:
sizeof|sizeof
argument_list|(
name|ip_t
argument_list|)
expr_stmt|;
else|#
directive|else
name|hlen
operator|=
sizeof|sizeof
argument_list|(
name|ip_t
argument_list|)
expr_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|MGETHDR
name|MGETHDR
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
else|#
directive|else
name|MGET
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
sizeof|sizeof
argument_list|(
operator|*
name|tcp2
argument_list|)
operator|+
name|hlen
operator|>
name|MHLEN
condition|)
block|{
name|MCLGET
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
operator|(
name|m
operator|->
name|m_flags
operator|&
name|M_EXT
operator|)
operator|==
literal|0
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
block|}
name|m
operator|->
name|m_len
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|tcp2
argument_list|)
operator|+
name|hlen
expr_stmt|;
name|m
operator|->
name|m_data
operator|+=
name|max_linkhdr
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|m
operator|->
name|m_len
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|rcvif
operator|=
operator|(
expr|struct
name|ifnet
operator|*
operator|)
literal|0
expr_stmt|;
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|ip
argument_list|,
name|hlen
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
name|ip6
operator|=
operator|(
name|ip6_t
operator|*
operator|)
name|ip
expr_stmt|;
endif|#
directive|endif
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|ip
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|tcp2
argument_list|)
operator|+
name|hlen
argument_list|)
expr_stmt|;
name|tcp2
operator|=
operator|(
expr|struct
name|tcphdr
operator|*
operator|)
operator|(
operator|(
name|char
operator|*
operator|)
name|ip
operator|+
name|hlen
operator|)
expr_stmt|;
name|tcp2
operator|->
name|th_sport
operator|=
name|tcp
operator|->
name|th_dport
expr_stmt|;
name|tcp2
operator|->
name|th_dport
operator|=
name|tcp
operator|->
name|th_sport
expr_stmt|;
if|if
condition|(
name|tcp
operator|->
name|th_flags
operator|&
name|TH_ACK
condition|)
block|{
name|tcp2
operator|->
name|th_seq
operator|=
name|tcp
operator|->
name|th_ack
expr_stmt|;
name|tcp2
operator|->
name|th_flags
operator|=
name|TH_RST
expr_stmt|;
name|tcp2
operator|->
name|th_ack
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|tcp2
operator|->
name|th_seq
operator|=
literal|0
expr_stmt|;
name|tcp2
operator|->
name|th_ack
operator|=
name|ntohl
argument_list|(
name|tcp
operator|->
name|th_seq
argument_list|)
expr_stmt|;
name|tcp2
operator|->
name|th_ack
operator|+=
name|tlen
expr_stmt|;
name|tcp2
operator|->
name|th_ack
operator|=
name|htonl
argument_list|(
name|tcp2
operator|->
name|th_ack
argument_list|)
expr_stmt|;
name|tcp2
operator|->
name|th_flags
operator|=
name|TH_RST
operator||
name|TH_ACK
expr_stmt|;
block|}
name|tcp2
operator|->
name|th_x2
operator|=
literal|0
expr_stmt|;
name|TCP_OFF_A
argument_list|(
name|tcp2
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|tcp2
argument_list|)
operator|>>
literal|2
argument_list|)
expr_stmt|;
name|tcp2
operator|->
name|th_win
operator|=
name|tcp
operator|->
name|th_win
expr_stmt|;
name|tcp2
operator|->
name|th_sum
operator|=
literal|0
expr_stmt|;
name|tcp2
operator|->
name|th_urp
operator|=
literal|0
expr_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
if|if
condition|(
name|fin
operator|->
name|fin_v
operator|==
literal|6
condition|)
block|{
name|ip6
operator|->
name|ip6_flow
operator|=
operator|(
operator|(
name|ip6_t
operator|*
operator|)
name|fin
operator|->
name|fin_ip
operator|)
operator|->
name|ip6_flow
expr_stmt|;
name|ip6
operator|->
name|ip6_plen
operator|=
name|htons
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|tcphdr
argument_list|)
argument_list|)
expr_stmt|;
name|ip6
operator|->
name|ip6_nxt
operator|=
name|IPPROTO_TCP
expr_stmt|;
name|ip6
operator|->
name|ip6_hlim
operator|=
literal|0
expr_stmt|;
name|ip6
operator|->
name|ip6_src
operator|=
name|fin
operator|->
name|fin_dst6
expr_stmt|;
name|ip6
operator|->
name|ip6_dst
operator|=
name|fin
operator|->
name|fin_src6
expr_stmt|;
name|tcp2
operator|->
name|th_sum
operator|=
name|in6_cksum
argument_list|(
name|m
argument_list|,
name|IPPROTO_TCP
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ip6
argument_list|)
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|tcp2
argument_list|)
argument_list|)
expr_stmt|;
return|return
name|ipf_send_ip
argument_list|(
name|fin
argument_list|,
name|m
argument_list|)
return|;
block|}
endif|#
directive|endif
ifdef|#
directive|ifdef
name|INET
name|ip
operator|->
name|ip_p
operator|=
name|IPPROTO_TCP
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
sizeof|sizeof
argument_list|(
expr|struct
name|tcphdr
argument_list|)
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_src
operator|.
name|s_addr
operator|=
name|fin
operator|->
name|fin_daddr
expr_stmt|;
name|ip
operator|->
name|ip_dst
operator|.
name|s_addr
operator|=
name|fin
operator|->
name|fin_saddr
expr_stmt|;
name|tcp2
operator|->
name|th_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|hlen
operator|+
sizeof|sizeof
argument_list|(
operator|*
name|tcp2
argument_list|)
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|=
name|hlen
operator|+
sizeof|sizeof
argument_list|(
operator|*
name|tcp2
argument_list|)
expr_stmt|;
return|return
name|ipf_send_ip
argument_list|(
name|fin
argument_list|,
name|m
argument_list|)
return|;
else|#
directive|else
return|return
literal|0
return|;
endif|#
directive|endif
block|}
end_function

begin_function
specifier|static
name|int
name|ipf_send_ip
parameter_list|(
name|fin
parameter_list|,
name|m
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
name|mb_t
modifier|*
name|m
decl_stmt|;
block|{
name|fr_info_t
name|fnew
decl_stmt|;
ifdef|#
directive|ifdef
name|INET
name|ip_t
modifier|*
name|oip
decl_stmt|;
endif|#
directive|endif
name|ip_t
modifier|*
name|ip
decl_stmt|;
name|int
name|hlen
decl_stmt|;
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|ip_t
operator|*
argument_list|)
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|fnew
argument_list|,
sizeof|sizeof
argument_list|(
name|fnew
argument_list|)
argument_list|)
expr_stmt|;
name|fnew
operator|.
name|fin_main_soft
operator|=
name|fin
operator|->
name|fin_main_soft
expr_stmt|;
name|IP_V_A
argument_list|(
name|ip
argument_list|,
name|fin
operator|->
name|fin_v
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|fin
operator|->
name|fin_v
condition|)
block|{
ifdef|#
directive|ifdef
name|INET
case|case
literal|4
case|:
name|oip
operator|=
name|fin
operator|->
name|fin_ip
expr_stmt|;
name|hlen
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|oip
argument_list|)
expr_stmt|;
name|fnew
operator|.
name|fin_v
operator|=
literal|4
expr_stmt|;
name|fnew
operator|.
name|fin_p
operator|=
name|ip
operator|->
name|ip_p
expr_stmt|;
name|fnew
operator|.
name|fin_plen
operator|=
name|ntohs
argument_list|(
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|IP_HL_A
argument_list|(
name|ip
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|oip
argument_list|)
operator|>>
literal|2
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_tos
operator|=
name|oip
operator|->
name|ip_tos
expr_stmt|;
name|ip
operator|->
name|ip_id
operator|=
name|ipf_nextipid
argument_list|(
name|fin
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|=
literal|0
expr_stmt|;
name|ip
operator|->
name|ip_ttl
operator|=
name|IPDEFTTL
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
break|break;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|USE_INET6
case|case
literal|6
case|:
block|{
name|ip6_t
modifier|*
name|ip6
init|=
operator|(
name|ip6_t
operator|*
operator|)
name|ip
decl_stmt|;
name|ip6
operator|->
name|ip6_vfc
operator|=
literal|0x60
expr_stmt|;
name|ip6
operator|->
name|ip6_hlim
operator|=
name|IPDEFTTL
expr_stmt|;
name|hlen
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|ip6
argument_list|)
expr_stmt|;
name|fnew
operator|.
name|fin_p
operator|=
name|ip6
operator|->
name|ip6_nxt
expr_stmt|;
name|fnew
operator|.
name|fin_v
operator|=
literal|6
expr_stmt|;
name|fnew
operator|.
name|fin_plen
operator|=
name|ntohs
argument_list|(
name|ip6
operator|->
name|ip6_plen
argument_list|)
operator|+
name|hlen
expr_stmt|;
break|break;
block|}
endif|#
directive|endif
default|default :
return|return
name|EINVAL
return|;
block|}
ifdef|#
directive|ifdef
name|IPSEC
name|m
operator|->
name|m_pkthdr
operator|.
name|rcvif
operator|=
name|NULL
expr_stmt|;
endif|#
directive|endif
name|fnew
operator|.
name|fin_ifp
operator|=
name|fin
operator|->
name|fin_ifp
expr_stmt|;
name|fnew
operator|.
name|fin_flx
operator|=
name|FI_NOCKSUM
expr_stmt|;
name|fnew
operator|.
name|fin_m
operator|=
name|m
expr_stmt|;
name|fnew
operator|.
name|fin_ip
operator|=
name|ip
expr_stmt|;
name|fnew
operator|.
name|fin_mp
operator|=
operator|&
name|m
expr_stmt|;
name|fnew
operator|.
name|fin_hlen
operator|=
name|hlen
expr_stmt|;
name|fnew
operator|.
name|fin_dp
operator|=
operator|(
name|char
operator|*
operator|)
name|ip
operator|+
name|hlen
expr_stmt|;
operator|(
name|void
operator|)
name|ipf_makefrip
argument_list|(
name|hlen
argument_list|,
name|ip
argument_list|,
operator|&
name|fnew
argument_list|)
expr_stmt|;
return|return
name|ipf_fastroute
argument_list|(
name|m
argument_list|,
operator|&
name|m
argument_list|,
operator|&
name|fnew
argument_list|,
name|NULL
argument_list|)
return|;
block|}
end_function

begin_function
name|int
name|ipf_send_icmp_err
parameter_list|(
name|type
parameter_list|,
name|fin
parameter_list|,
name|dst
parameter_list|)
name|int
name|type
decl_stmt|;
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
name|int
name|dst
decl_stmt|;
block|{
name|int
name|err
decl_stmt|,
name|hlen
decl_stmt|,
name|xtra
decl_stmt|,
name|iclen
decl_stmt|,
name|ohlen
decl_stmt|,
name|avail
decl_stmt|,
name|code
decl_stmt|;
name|struct
name|in_addr
name|dst4
decl_stmt|;
name|struct
name|icmp
modifier|*
name|icmp
decl_stmt|;
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
name|void
modifier|*
name|ifp
decl_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
name|ip6_t
modifier|*
name|ip6
decl_stmt|;
name|struct
name|in6_addr
name|dst6
decl_stmt|;
endif|#
directive|endif
name|ip_t
modifier|*
name|ip
decl_stmt|,
modifier|*
name|ip2
decl_stmt|;
if|if
condition|(
operator|(
name|type
operator|<
literal|0
operator|)
operator|||
operator|(
name|type
operator|>
name|ICMP_MAXTYPE
operator|)
condition|)
return|return
operator|-
literal|1
return|;
name|code
operator|=
name|fin
operator|->
name|fin_icode
expr_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
if|if
condition|(
operator|(
name|code
operator|<
literal|0
operator|)
operator|||
operator|(
name|code
operator|>
sizeof|sizeof
argument_list|(
name|icmptoicmp6unreach
argument_list|)
operator|/
sizeof|sizeof
argument_list|(
name|int
argument_list|)
operator|)
condition|)
return|return
operator|-
literal|1
return|;
endif|#
directive|endif
if|if
condition|(
name|ipf_checkl4sum
argument_list|(
name|fin
argument_list|)
operator|==
operator|-
literal|1
condition|)
return|return
operator|-
literal|1
return|;
ifdef|#
directive|ifdef
name|MGETHDR
name|MGETHDR
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
else|#
directive|else
name|MGET
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
return|return
operator|-
literal|1
return|;
name|avail
operator|=
name|MHLEN
expr_stmt|;
name|xtra
operator|=
literal|0
expr_stmt|;
name|hlen
operator|=
literal|0
expr_stmt|;
name|ohlen
operator|=
literal|0
expr_stmt|;
name|ifp
operator|=
name|fin
operator|->
name|fin_ifp
expr_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_v
operator|==
literal|4
condition|)
block|{
if|if
condition|(
operator|(
name|fin
operator|->
name|fin_p
operator|==
name|IPPROTO_ICMP
operator|)
operator|&&
operator|!
operator|(
name|fin
operator|->
name|fin_flx
operator|&
name|FI_SHORT
operator|)
condition|)
switch|switch
condition|(
name|ntohs
argument_list|(
name|fin
operator|->
name|fin_data
index|[
literal|0
index|]
argument_list|)
operator|>>
literal|8
condition|)
block|{
case|case
name|ICMP_ECHO
case|:
case|case
name|ICMP_TSTAMP
case|:
case|case
name|ICMP_IREQ
case|:
case|case
name|ICMP_MASKREQ
case|:
break|break;
default|default :
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
name|dst
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|fr_ifpaddr
argument_list|(
name|softc
argument_list|,
literal|4
argument_list|,
name|FRI_NORMAL
argument_list|,
name|ifp
argument_list|,
operator|&
name|dst4
argument_list|,
name|NULL
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
block|}
else|else
name|dst4
operator|.
name|s_addr
operator|=
name|fin
operator|->
name|fin_daddr
expr_stmt|;
name|hlen
operator|=
sizeof|sizeof
argument_list|(
name|ip_t
argument_list|)
expr_stmt|;
name|ohlen
operator|=
name|fin
operator|->
name|fin_hlen
expr_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_hlen
operator|<
name|fin
operator|->
name|fin_plen
condition|)
name|xtra
operator|=
name|MIN
argument_list|(
name|fin
operator|->
name|fin_dlen
argument_list|,
literal|8
argument_list|)
expr_stmt|;
else|else
name|xtra
operator|=
literal|0
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|USE_INET6
elseif|else
if|if
condition|(
name|fin
operator|->
name|fin_v
operator|==
literal|6
condition|)
block|{
name|hlen
operator|=
sizeof|sizeof
argument_list|(
name|ip6_t
argument_list|)
expr_stmt|;
name|ohlen
operator|=
sizeof|sizeof
argument_list|(
name|ip6_t
argument_list|)
expr_stmt|;
name|type
operator|=
name|icmptoicmp6types
index|[
name|type
index|]
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|ICMP6_DST_UNREACH
condition|)
name|code
operator|=
name|icmptoicmp6unreach
index|[
name|code
index|]
expr_stmt|;
if|if
condition|(
name|hlen
operator|+
sizeof|sizeof
argument_list|(
operator|*
name|icmp
argument_list|)
operator|+
name|max_linkhdr
operator|+
name|fin
operator|->
name|fin_plen
operator|>
name|avail
condition|)
block|{
name|MCLGET
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
operator|(
name|m
operator|->
name|m_flags
operator|&
name|M_EXT
operator|)
operator|==
literal|0
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|avail
operator|=
name|MCLBYTES
expr_stmt|;
block|}
name|xtra
operator|=
name|MIN
argument_list|(
name|fin
operator|->
name|fin_plen
argument_list|,
name|avail
operator|-
name|hlen
operator|-
sizeof|sizeof
argument_list|(
operator|*
name|icmp
argument_list|)
operator|-
name|max_linkhdr
argument_list|)
expr_stmt|;
if|if
condition|(
name|dst
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|fr_ifpaddr
argument_list|(
name|softc
argument_list|,
literal|6
argument_list|,
name|FRI_NORMAL
argument_list|,
name|ifp
argument_list|,
operator|(
expr|struct
name|in_addr
operator|*
operator|)
operator|&
name|dst6
argument_list|,
name|NULL
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
block|}
else|else
name|dst6
operator|=
name|fin
operator|->
name|fin_dst6
expr_stmt|;
block|}
endif|#
directive|endif
else|else
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|iclen
operator|=
name|hlen
operator|+
sizeof|sizeof
argument_list|(
operator|*
name|icmp
argument_list|)
expr_stmt|;
name|avail
operator|-=
operator|(
name|max_linkhdr
operator|+
name|iclen
operator|)
expr_stmt|;
if|if
condition|(
name|avail
operator|<
literal|0
condition|)
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
if|if
condition|(
name|xtra
operator|>
name|avail
condition|)
name|xtra
operator|=
name|avail
expr_stmt|;
name|iclen
operator|+=
name|xtra
expr_stmt|;
name|m
operator|->
name|m_data
operator|+=
name|max_linkhdr
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|rcvif
operator|=
operator|(
expr|struct
name|ifnet
operator|*
operator|)
literal|0
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|iclen
expr_stmt|;
name|m
operator|->
name|m_len
operator|=
name|iclen
expr_stmt|;
name|ip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
name|ip_t
operator|*
argument_list|)
expr_stmt|;
name|icmp
operator|=
operator|(
expr|struct
name|icmp
operator|*
operator|)
operator|(
operator|(
name|char
operator|*
operator|)
name|ip
operator|+
name|hlen
operator|)
expr_stmt|;
name|ip2
operator|=
operator|(
name|ip_t
operator|*
operator|)
operator|&
name|icmp
operator|->
name|icmp_ip
expr_stmt|;
name|icmp
operator|->
name|icmp_type
operator|=
name|type
expr_stmt|;
name|icmp
operator|->
name|icmp_code
operator|=
name|fin
operator|->
name|fin_icode
expr_stmt|;
name|icmp
operator|->
name|icmp_cksum
operator|=
literal|0
expr_stmt|;
ifdef|#
directive|ifdef
name|icmp_nextmtu
if|if
condition|(
name|type
operator|==
name|ICMP_UNREACH
operator|&&
name|fin
operator|->
name|fin_icode
operator|==
name|ICMP_UNREACH_NEEDFRAG
condition|)
block|{
if|if
condition|(
name|fin
operator|->
name|fin_mtu
operator|!=
literal|0
condition|)
block|{
name|icmp
operator|->
name|icmp_nextmtu
operator|=
name|htons
argument_list|(
name|fin
operator|->
name|fin_mtu
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|ifp
operator|!=
name|NULL
condition|)
block|{
name|icmp
operator|->
name|icmp_nextmtu
operator|=
name|htons
argument_list|(
name|GETIFMTU_4
argument_list|(
name|ifp
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
comment|/* make up a number... */
name|icmp
operator|->
name|icmp_nextmtu
operator|=
name|htons
argument_list|(
name|fin
operator|->
name|fin_plen
operator|-
literal|20
argument_list|)
expr_stmt|;
block|}
block|}
endif|#
directive|endif
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_ip
argument_list|,
operator|(
name|char
operator|*
operator|)
name|ip2
argument_list|,
name|ohlen
argument_list|)
expr_stmt|;
if|#
directive|if
name|defined
argument_list|(
name|M_CSUM_IPv4
argument_list|)
comment|/* 	 * Clear any in-bound checksum flags for this packet. 	 */
name|m
operator|->
name|m_pkthdr
operator|.
name|csuminfo
operator|=
literal|0
expr_stmt|;
endif|#
directive|endif
comment|/* __NetBSD__&& M_CSUM_IPv4 */
ifdef|#
directive|ifdef
name|USE_INET6
name|ip6
operator|=
operator|(
name|ip6_t
operator|*
operator|)
name|ip
expr_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_v
operator|==
literal|6
condition|)
block|{
name|ip6
operator|->
name|ip6_flow
operator|=
operator|(
operator|(
name|ip6_t
operator|*
operator|)
name|fin
operator|->
name|fin_ip
operator|)
operator|->
name|ip6_flow
expr_stmt|;
name|ip6
operator|->
name|ip6_plen
operator|=
name|htons
argument_list|(
name|iclen
operator|-
name|hlen
argument_list|)
expr_stmt|;
name|ip6
operator|->
name|ip6_nxt
operator|=
name|IPPROTO_ICMPV6
expr_stmt|;
name|ip6
operator|->
name|ip6_hlim
operator|=
literal|0
expr_stmt|;
name|ip6
operator|->
name|ip6_src
operator|=
name|dst6
expr_stmt|;
name|ip6
operator|->
name|ip6_dst
operator|=
name|fin
operator|->
name|fin_src6
expr_stmt|;
if|if
condition|(
name|xtra
operator|>
literal|0
condition|)
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_ip
operator|+
name|ohlen
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|icmp
operator|->
name|icmp_ip
operator|+
name|ohlen
argument_list|,
name|xtra
argument_list|)
expr_stmt|;
name|icmp
operator|->
name|icmp_cksum
operator|=
name|in6_cksum
argument_list|(
name|m
argument_list|,
name|IPPROTO_ICMPV6
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ip6
argument_list|)
argument_list|,
name|iclen
operator|-
name|hlen
argument_list|)
expr_stmt|;
block|}
else|else
endif|#
directive|endif
block|{
name|ip2
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
name|ip2
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|ip2
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
name|ip2
operator|->
name|ip_off
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_p
operator|=
name|IPPROTO_ICMP
expr_stmt|;
name|ip
operator|->
name|ip_src
operator|.
name|s_addr
operator|=
name|dst4
operator|.
name|s_addr
expr_stmt|;
name|ip
operator|->
name|ip_dst
operator|.
name|s_addr
operator|=
name|fin
operator|->
name|fin_saddr
expr_stmt|;
if|if
condition|(
name|xtra
operator|>
literal|0
condition|)
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_ip
operator|+
name|ohlen
argument_list|,
operator|(
name|char
operator|*
operator|)
operator|&
name|icmp
operator|->
name|icmp_ip
operator|+
name|ohlen
argument_list|,
name|xtra
argument_list|)
expr_stmt|;
name|icmp
operator|->
name|icmp_cksum
operator|=
name|ipf_cksum
argument_list|(
operator|(
name|u_short
operator|*
operator|)
name|icmp
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|icmp
argument_list|)
operator|+
literal|8
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|=
name|iclen
expr_stmt|;
name|ip
operator|->
name|ip_p
operator|=
name|IPPROTO_ICMP
expr_stmt|;
block|}
name|err
operator|=
name|ipf_send_ip
argument_list|(
name|fin
argument_list|,
name|m
argument_list|)
expr_stmt|;
return|return
name|err
return|;
block|}
end_function

begin_comment
comment|/*  * m0 - pointer to mbuf where the IP packet starts  * mpp - pointer to the mbuf pointer that is the start of the mbuf chain  */
end_comment

begin_function
name|int
name|ipf_fastroute
parameter_list|(
name|m0
parameter_list|,
name|mpp
parameter_list|,
name|fin
parameter_list|,
name|fdp
parameter_list|)
name|mb_t
modifier|*
name|m0
decl_stmt|,
decl|*
modifier|*
name|mpp
decl_stmt|;
end_function

begin_decl_stmt
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|frdest_t
modifier|*
name|fdp
decl_stmt|;
end_decl_stmt

begin_block
block|{
specifier|register
name|struct
name|ip
modifier|*
name|ip
decl_stmt|,
modifier|*
name|mhip
decl_stmt|;
specifier|register
name|struct
name|mbuf
modifier|*
name|m
init|=
operator|*
name|mpp
decl_stmt|;
specifier|register
name|struct
name|route
modifier|*
name|ro
decl_stmt|;
name|int
name|len
decl_stmt|,
name|off
decl_stmt|,
name|error
init|=
literal|0
decl_stmt|,
name|hlen
decl_stmt|,
name|code
decl_stmt|;
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|,
modifier|*
name|sifp
decl_stmt|;
name|struct
name|sockaddr_in
modifier|*
name|dst
decl_stmt|;
name|struct
name|route
name|iproute
decl_stmt|;
name|u_short
name|ip_off
decl_stmt|;
name|frdest_t
name|node
decl_stmt|;
name|frentry_t
modifier|*
name|fr
decl_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_v
operator|==
literal|6
condition|)
block|{
ifdef|#
directive|ifdef
name|USE_INET6
name|error
operator|=
name|ipfr_fastroute6
argument_list|(
name|m0
argument_list|,
name|mpp
argument_list|,
name|fin
argument_list|,
name|fdp
argument_list|)
expr_stmt|;
else|#
directive|else
name|error
operator|=
name|EPROTONOSUPPORT
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
operator|(
name|error
operator|!=
literal|0
operator|)
operator|&&
operator|(
operator|*
name|mpp
operator|!=
name|NULL
operator|)
condition|)
block|{
name|FREE_MB_T
argument_list|(
operator|*
name|mpp
argument_list|)
expr_stmt|;
block|}
return|return
name|error
return|;
block|}
ifndef|#
directive|ifndef
name|INET
name|FREE_MB_T
argument_list|(
operator|*
name|mpp
argument_list|)
expr_stmt|;
return|return
name|EPROTONOSUPPORT
return|;
else|#
directive|else
name|hlen
operator|=
name|fin
operator|->
name|fin_hlen
expr_stmt|;
name|ip
operator|=
name|mtod
argument_list|(
name|m0
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
if|#
directive|if
name|defined
argument_list|(
name|M_CSUM_IPv4
argument_list|)
comment|/* 	 * Clear any in-bound checksum flags for this packet. 	 */
name|m0
operator|->
name|m_pkthdr
operator|.
name|csuminfo
operator|=
literal|0
expr_stmt|;
endif|#
directive|endif
comment|/* __NetBSD__&& M_CSUM_IPv4 */
comment|/* 	 * Route packet. 	 */
name|ro
operator|=
operator|&
name|iproute
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
name|ro
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ro
argument_list|)
argument_list|)
expr_stmt|;
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|ro
operator|->
name|ro_dst
expr_stmt|;
name|dst
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|dst
operator|->
name|sin_addr
operator|=
name|ip
operator|->
name|ip_dst
expr_stmt|;
name|ifp
operator|=
name|NULL
expr_stmt|;
name|fr
operator|=
name|fin
operator|->
name|fin_fr
expr_stmt|;
if|if
condition|(
operator|(
name|fr
operator|==
name|NULL
operator|)
operator|||
operator|!
operator|(
name|fr
operator|->
name|fr_flags
operator|&
name|FR_FASTROUTE
operator|)
condition|)
block|)
end_block

begin_block
block|{
name|error
operator|=
operator|-
literal|2
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
end_block

begin_if
if|if
condition|(
operator|(
name|fr
operator|!=
name|NULL
operator|)
operator|&&
operator|!
operator|(
name|fr
operator|->
name|fr_flags
operator|&
name|FR_KEEPSTATE
operator|)
operator|&&
operator|(
name|fdp
operator|!=
name|NULL
operator|)
operator|&&
operator|(
name|fdp
operator|->
name|fd_type
operator|==
name|FRD_DSTLIST
operator|)
condition|)
block|{
if|if
condition|(
name|ipf_dstlist_select_node
argument_list|(
name|fin
argument_list|,
name|fdp
operator|->
name|fd_ptr
argument_list|,
name|NULL
argument_list|,
operator|&
name|node
argument_list|)
operator|==
literal|0
condition|)
name|fdp
operator|=
operator|&
name|node
expr_stmt|;
block|}
end_if

begin_if
if|if
condition|(
name|fdp
operator|!=
name|NULL
condition|)
name|ifp
operator|=
name|fdp
operator|->
name|fd_ptr
expr_stmt|;
else|else
name|ifp
operator|=
name|fin
operator|->
name|fin_ifp
expr_stmt|;
end_if

begin_if
if|if
condition|(
name|ifp
operator|==
name|NULL
condition|)
block|{
name|error
operator|=
operator|-
literal|2
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
end_if

begin_if
if|if
condition|(
operator|(
name|fdp
operator|!=
name|NULL
operator|)
operator|&&
operator|(
name|fdp
operator|->
name|fd_ip
operator|.
name|s_addr
operator|!=
literal|0
operator|)
condition|)
name|dst
operator|->
name|sin_addr
operator|=
name|fdp
operator|->
name|fd_ip
expr_stmt|;
end_if

begin_expr_stmt
name|dst
operator|->
name|sin_len
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|dst
argument_list|)
expr_stmt|;
end_expr_stmt

begin_expr_stmt
name|rtalloc
argument_list|(
name|ro
argument_list|)
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
operator|(
name|ifp
operator|==
name|NULL
operator|)
operator|&&
operator|(
name|ro
operator|->
name|ro_rt
operator|!=
name|NULL
operator|)
condition|)
name|ifp
operator|=
name|ro
operator|->
name|ro_rt
operator|->
name|rt_ifp
expr_stmt|;
end_if

begin_if
if|if
condition|(
operator|(
name|ro
operator|->
name|ro_rt
operator|==
name|NULL
operator|)
operator|||
operator|(
name|ifp
operator|==
name|NULL
operator|)
condition|)
block|{
if|if
condition|(
name|in_localaddr
argument_list|(
name|ip
operator|->
name|ip_dst
argument_list|)
condition|)
name|error
operator|=
name|EHOSTUNREACH
expr_stmt|;
else|else
name|error
operator|=
name|ENETUNREACH
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
end_if

begin_if
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|->
name|rt_flags
operator|&
name|RTF_GATEWAY
condition|)
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|ro
operator|->
name|ro_rt
operator|->
name|rt_gateway
expr_stmt|;
end_if

begin_if
if|if
condition|(
name|ro
operator|->
name|ro_rt
condition|)
name|ro
operator|->
name|ro_rt
operator|->
name|rt_use
operator|++
expr_stmt|;
end_if

begin_comment
comment|/* 	 * For input packets which are being "fastrouted", they won't 	 * go back through output filtering and miss their chance to get 	 * NAT'd and counted.  Duplicated packets aren't considered to be 	 * part of the normal packet stream, so do not NAT them or pass 	 * them through stateful checking, etc. 	 */
end_comment

begin_if
if|if
condition|(
operator|(
name|fdp
operator|!=
operator|&
name|fr
operator|->
name|fr_dif
operator|)
operator|&&
operator|(
name|fin
operator|->
name|fin_out
operator|==
literal|0
operator|)
condition|)
block|{
name|sifp
operator|=
name|fin
operator|->
name|fin_ifp
expr_stmt|;
name|fin
operator|->
name|fin_ifp
operator|=
name|ifp
expr_stmt|;
name|fin
operator|->
name|fin_out
operator|=
literal|1
expr_stmt|;
operator|(
name|void
operator|)
name|ipf_acctpkt
argument_list|(
name|fin
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|fin
operator|->
name|fin_fr
operator|=
name|NULL
expr_stmt|;
if|if
condition|(
operator|!
name|fr
operator|||
operator|!
operator|(
name|fr
operator|->
name|fr_flags
operator|&
name|FR_RETMASK
operator|)
condition|)
block|{
name|u_32_t
name|pass
decl_stmt|;
operator|(
name|void
operator|)
name|ipf_state_check
argument_list|(
name|fin
argument_list|,
operator|&
name|pass
argument_list|)
expr_stmt|;
block|}
switch|switch
condition|(
name|ipf_nat_checkout
argument_list|(
name|fin
argument_list|,
name|NULL
argument_list|)
condition|)
block|{
case|case
literal|0
case|:
break|break;
case|case
literal|1
case|:
name|ip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
break|break;
case|case
operator|-
literal|1
case|:
name|error
operator|=
operator|-
literal|1
expr_stmt|;
goto|goto
name|bad
goto|;
break|break;
block|}
name|fin
operator|->
name|fin_ifp
operator|=
name|sifp
expr_stmt|;
name|fin
operator|->
name|fin_out
operator|=
literal|0
expr_stmt|;
block|}
else|else
name|ip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
end_if

begin_comment
comment|/* 	 * If small enough for interface, can just send directly. 	 */
end_comment

begin_if
if|if
condition|(
name|ip
operator|->
name|ip_len
operator|<=
name|ifp
operator|->
name|if_mtu
condition|)
block|{
name|int
name|i
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|m
operator|->
name|m_flags
operator|&
name|M_EXT
condition|)
name|i
operator|=
literal|1
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
name|ip
operator|->
name|ip_off
argument_list|)
expr_stmt|;
if|#
directive|if
name|defined
argument_list|(
name|M_CSUM_IPv4
argument_list|)
if|#
directive|if
operator|(
name|__NetBSD_Version__
operator|>=
literal|105009999
operator|)
if|if
condition|(
name|ifp
operator|->
name|if_csum_flags_tx
operator|&
name|M_CSUM_IPv4
condition|)
name|m
operator|->
name|m_pkthdr
operator|.
name|csuminfo
operator||=
name|M_CSUM_IPv4
expr_stmt|;
else|#
directive|else
if|if
condition|(
name|ifp
operator|->
name|if_capabilities
operator|&
name|IFCAP_CSUM_IPv4
condition|)
name|m
operator|->
name|m_pkthdr
operator|.
name|csuminfo
operator||=
name|M_CSUM_IPv4
expr_stmt|;
endif|#
directive|endif
comment|/* (__NetBSD_Version__>= 105009999) */
elseif|else
if|if
condition|(
name|ip
operator|->
name|ip_sum
operator|==
literal|0
condition|)
name|ip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|hlen
argument_list|)
expr_stmt|;
else|#
directive|else
if|if
condition|(
operator|!
name|ip
operator|->
name|ip_sum
condition|)
name|ip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|hlen
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|/* M_CSUM_IPv4 */
name|error
operator|=
call|(
modifier|*
name|ifp
operator|->
name|if_output
call|)
argument_list|(
name|ifp
argument_list|,
name|m
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|dst
argument_list|,
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
condition|)
block|{
name|ip
operator|->
name|ip_len
operator|=
name|ntohs
argument_list|(
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|=
name|ntohs
argument_list|(
name|ip
operator|->
name|ip_off
argument_list|)
expr_stmt|;
block|}
goto|goto
name|done
goto|;
block|}
end_if

begin_comment
comment|/* 	 * Too large for interface; fragment if possible. 	 * Must be able to put at least 8 bytes per fragment. 	 */
end_comment

begin_expr_stmt
name|ip_off
operator|=
name|ip
operator|->
name|ip_off
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
name|ip_off
operator|&
name|IP_DF
condition|)
block|{
name|error
operator|=
name|EMSGSIZE
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
end_if

begin_expr_stmt
name|len
operator|=
operator|(
name|ifp
operator|->
name|if_mtu
operator|-
name|hlen
operator|)
operator|&
operator|~
literal|7
expr_stmt|;
end_expr_stmt

begin_if
if|if
condition|(
name|len
operator|<
literal|8
condition|)
block|{
name|error
operator|=
name|EMSGSIZE
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
end_if

begin_block
block|{
name|int
name|mhlen
decl_stmt|,
name|firstlen
init|=
name|len
decl_stmt|;
name|struct
name|mbuf
modifier|*
modifier|*
name|mnext
init|=
operator|&
name|m
operator|->
name|m_act
decl_stmt|;
comment|/* 	 * Loop through length of segment after first fragment, 	 * make new header and copy data of each part and link onto chain. 	 */
name|m0
operator|=
name|m
expr_stmt|;
name|mhlen
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
for|for
control|(
name|off
operator|=
name|hlen
operator|+
name|len
init|;
name|off
operator|<
name|ip
operator|->
name|ip_len
condition|;
name|off
operator|+=
name|len
control|)
block|{
ifdef|#
directive|ifdef
name|MGETHDR
name|MGETHDR
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
else|#
directive|else
name|MGET
argument_list|(
name|m
argument_list|,
name|M_DONTWAIT
argument_list|,
name|MT_HEADER
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
name|m
operator|==
literal|0
condition|)
block|{
name|m
operator|=
name|m0
expr_stmt|;
name|error
operator|=
name|ENOBUFS
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|m
operator|->
name|m_data
operator|+=
name|max_linkhdr
expr_stmt|;
name|mhip
operator|=
name|mtod
argument_list|(
name|m
argument_list|,
expr|struct
name|ip
operator|*
argument_list|)
expr_stmt|;
name|bcopy
argument_list|(
operator|(
name|char
operator|*
operator|)
name|ip
argument_list|,
operator|(
name|char
operator|*
operator|)
name|mhip
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ip
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|hlen
operator|>
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
condition|)
block|{
name|mhlen
operator|=
name|ip_optcopy
argument_list|(
name|ip
argument_list|,
name|mhip
argument_list|)
operator|+
sizeof|sizeof
argument_list|(
expr|struct
name|ip
argument_list|)
expr_stmt|;
name|IP_HL_A
argument_list|(
name|mhip
argument_list|,
name|mhlen
operator|>>
literal|2
argument_list|)
expr_stmt|;
block|}
name|m
operator|->
name|m_len
operator|=
name|mhlen
expr_stmt|;
name|mhip
operator|->
name|ip_off
operator|=
operator|(
operator|(
name|off
operator|-
name|hlen
operator|)
operator|>>
literal|3
operator|)
operator|+
name|ip_off
expr_stmt|;
if|if
condition|(
name|off
operator|+
name|len
operator|>=
name|ip
operator|->
name|ip_len
condition|)
name|len
operator|=
name|ip
operator|->
name|ip_len
operator|-
name|off
expr_stmt|;
else|else
name|mhip
operator|->
name|ip_off
operator||=
name|IP_MF
expr_stmt|;
name|mhip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
call|(
name|u_short
call|)
argument_list|(
name|len
operator|+
name|mhlen
argument_list|)
argument_list|)
expr_stmt|;
name|m
operator|->
name|m_next
operator|=
name|m_copy
argument_list|(
name|m0
argument_list|,
name|off
argument_list|,
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
name|m
operator|->
name|m_next
operator|==
literal|0
condition|)
block|{
name|error
operator|=
name|ENOBUFS
expr_stmt|;
comment|/* ??? */
goto|goto
name|sendorfree
goto|;
block|}
name|m
operator|->
name|m_pkthdr
operator|.
name|len
operator|=
name|mhlen
operator|+
name|len
expr_stmt|;
name|m
operator|->
name|m_pkthdr
operator|.
name|rcvif
operator|=
name|NULL
expr_stmt|;
name|mhip
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|mhip
operator|->
name|ip_off
argument_list|)
expr_stmt|;
name|mhip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
name|mhip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m
argument_list|,
name|mhlen
argument_list|)
expr_stmt|;
operator|*
name|mnext
operator|=
name|m
expr_stmt|;
name|mnext
operator|=
operator|&
name|m
operator|->
name|m_act
expr_stmt|;
block|}
comment|/* 	 * Update first fragment by trimming what's been copied out 	 * and updating header, then send each fragment (in order). 	 */
name|m_adj
argument_list|(
name|m0
argument_list|,
name|hlen
operator|+
name|firstlen
operator|-
name|ip
operator|->
name|ip_len
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_len
operator|=
name|htons
argument_list|(
call|(
name|u_short
call|)
argument_list|(
name|hlen
operator|+
name|firstlen
argument_list|)
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_off
operator|=
name|htons
argument_list|(
operator|(
name|u_short
operator|)
name|IP_MF
argument_list|)
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
literal|0
expr_stmt|;
name|ip
operator|->
name|ip_sum
operator|=
name|in_cksum
argument_list|(
name|m0
argument_list|,
name|hlen
argument_list|)
expr_stmt|;
name|sendorfree
label|:
for|for
control|(
name|m
operator|=
name|m0
init|;
name|m
condition|;
name|m
operator|=
name|m0
control|)
block|{
name|m0
operator|=
name|m
operator|->
name|m_act
expr_stmt|;
name|m
operator|->
name|m_act
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|error
operator|==
literal|0
condition|)
name|error
operator|=
call|(
modifier|*
name|ifp
operator|->
name|if_output
call|)
argument_list|(
name|ifp
argument_list|,
name|m
argument_list|,
operator|(
expr|struct
name|sockaddr
operator|*
operator|)
name|dst
argument_list|,
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
else|else
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
block|}
block|}
end_block

begin_label
name|done
label|:
end_label

begin_if
if|if
condition|(
operator|!
name|error
condition|)
name|ipf_frouteok
index|[
literal|0
index|]
operator|++
expr_stmt|;
else|else
name|ipf_frouteok
index|[
literal|1
index|]
operator|++
expr_stmt|;
end_if

begin_if
if|if
condition|(
name|ro
operator|->
name|ro_rt
condition|)
block|{
name|RTFREE
argument_list|(
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
block|}
end_if

begin_return
return|return
name|error
return|;
end_return

begin_label
name|bad
label|:
end_label

begin_if
if|if
condition|(
name|error
operator|==
name|EMSGSIZE
condition|)
block|{
name|sifp
operator|=
name|fin
operator|->
name|fin_ifp
expr_stmt|;
name|code
operator|=
name|fin
operator|->
name|fin_icode
expr_stmt|;
name|fin
operator|->
name|fin_icode
operator|=
name|ICMP_UNREACH_NEEDFRAG
expr_stmt|;
name|fin
operator|->
name|fin_ifp
operator|=
name|ifp
expr_stmt|;
operator|(
name|void
operator|)
name|ipf_send_icmp_err
argument_list|(
name|ICMP_UNREACH
argument_list|,
name|fin
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|fin
operator|->
name|fin_ifp
operator|=
name|sifp
expr_stmt|;
name|fin
operator|->
name|fin_icode
operator|=
name|code
expr_stmt|;
block|}
end_if

begin_expr_stmt
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
end_expr_stmt

begin_goto
goto|goto
name|done
goto|;
end_goto

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* INET */
end_comment

begin_if
unit|}
if|#
directive|if
name|defined
argument_list|(
name|USE_INET6
argument_list|)
end_if

begin_comment
comment|/*  * This is the IPv6 specific fastroute code.  It doesn't clean up the mbuf's  * or ensure that it is an IPv6 packet that is being forwarded, those are  * expected to be done by the called (ipf_fastroute).  */
end_comment

begin_function
unit|static
name|int
name|ipf_fastroute6
parameter_list|(
name|m0
parameter_list|,
name|mpp
parameter_list|,
name|fin
parameter_list|,
name|fdp
parameter_list|)
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|,
decl|*
modifier|*
name|mpp
decl_stmt|;
end_function

begin_decl_stmt
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
end_decl_stmt

begin_decl_stmt
name|frdest_t
modifier|*
name|fdp
decl_stmt|;
end_decl_stmt

begin_block
block|{
name|struct
name|route_in6
name|ip6route
decl_stmt|;
name|struct
name|sockaddr_in6
modifier|*
name|dst6
decl_stmt|;
name|struct
name|route_in6
modifier|*
name|ro
decl_stmt|;
name|struct
name|rtentry
modifier|*
name|rt
decl_stmt|;
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
name|frentry_t
modifier|*
name|fr
decl_stmt|;
name|u_long
name|mtu
decl_stmt|;
name|int
name|error
decl_stmt|;
name|ro
operator|=
operator|&
name|ip6route
expr_stmt|;
name|fr
operator|=
name|fin
operator|->
name|fin_fr
expr_stmt|;
name|bzero
argument_list|(
operator|(
name|caddr_t
operator|)
name|ro
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|ro
argument_list|)
argument_list|)
expr_stmt|;
name|dst6
operator|=
operator|(
expr|struct
name|sockaddr_in6
operator|*
operator|)
operator|&
name|ro
operator|->
name|ro_dst
expr_stmt|;
name|dst6
operator|->
name|sin6_family
operator|=
name|AF_INET6
expr_stmt|;
name|dst6
operator|->
name|sin6_len
operator|=
sizeof|sizeof
argument_list|(
expr|struct
name|sockaddr_in6
argument_list|)
expr_stmt|;
name|dst6
operator|->
name|sin6_addr
operator|=
name|fin
operator|->
name|fin_fi
operator|.
name|fi_dst
operator|.
name|in6
expr_stmt|;
if|if
condition|(
name|fdp
operator|!=
name|NULL
condition|)
name|ifp
operator|=
name|fdp
operator|->
name|fd_ifp
expr_stmt|;
else|else
name|ifp
operator|=
name|fin
operator|->
name|fin_ifp
expr_stmt|;
if|if
condition|(
name|fdp
operator|!=
name|NULL
condition|)
block|{
if|if
condition|(
name|IP6_NOTZERO
argument_list|(
operator|&
name|fdp
operator|->
name|fd_ip6
argument_list|)
condition|)
name|dst6
operator|->
name|sin6_addr
operator|=
name|fdp
operator|->
name|fd_ip6
operator|.
name|in6
expr_stmt|;
block|}
name|rtalloc
argument_list|(
operator|(
expr|struct
name|route
operator|*
operator|)
name|ro
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|ifp
operator|==
name|NULL
operator|)
operator|&&
operator|(
name|ro
operator|->
name|ro_rt
operator|!=
name|NULL
operator|)
condition|)
name|ifp
operator|=
name|ro
operator|->
name|ro_rt
operator|->
name|rt_ifp
expr_stmt|;
if|if
condition|(
operator|(
name|ro
operator|->
name|ro_rt
operator|==
name|NULL
operator|)
operator|||
operator|(
name|ifp
operator|==
name|NULL
operator|)
condition|)
block|{
name|error
operator|=
name|EHOSTUNREACH
expr_stmt|;
goto|goto
name|bad
goto|;
block|}
name|rt
operator|=
name|fdp
condition|?
name|NULL
else|:
name|ro
operator|->
name|ro_rt
expr_stmt|;
comment|/* KAME */
if|if
condition|(
name|IN6_IS_ADDR_LINKLOCAL
argument_list|(
operator|&
name|dst6
operator|->
name|sin6_addr
argument_list|)
condition|)
name|dst6
operator|->
name|sin6_addr
operator|.
name|s6_addr16
index|[
literal|1
index|]
operator|=
name|htons
argument_list|(
name|ifp
operator|->
name|if_index
argument_list|)
expr_stmt|;
block|{
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|->
name|rt_flags
operator|&
name|RTF_GATEWAY
condition|)
name|dst6
operator|=
operator|(
expr|struct
name|sockaddr_in6
operator|*
operator|)
name|ro
operator|->
name|ro_rt
operator|->
name|rt_gateway
expr_stmt|;
name|ro
operator|->
name|ro_rt
operator|->
name|rt_use
operator|++
expr_stmt|;
name|error
operator|=
name|ip6_getpmtu
argument_list|(
name|ro
argument_list|,
name|ro
argument_list|,
name|ifp
argument_list|,
operator|&
name|finaldst
argument_list|,
operator|&
name|mtu
argument_list|,
operator|&
name|frag
argument_list|)
expr_stmt|;
if|if
condition|(
operator|(
name|error
operator|==
literal|0
operator|)
operator|&&
operator|(
name|m0
operator|->
name|m_pkthdr
operator|.
name|len
operator|<=
name|mtu
operator|)
condition|)
block|{
name|error
operator|=
name|nd6_output
argument_list|(
name|ifp
argument_list|,
name|ifp
argument_list|,
operator|*
name|mpp
argument_list|,
name|dst6
argument_list|,
name|rt
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|error
operator|=
name|EMSGSIZE
expr_stmt|;
block|}
block|}
name|bad
label|:
if|if
condition|(
name|ro
operator|->
name|ro_rt
operator|!=
name|NULL
condition|)
block|{
name|RTFREE
argument_list|(
name|ro
operator|->
name|ro_rt
argument_list|)
expr_stmt|;
block|}
return|return
name|error
return|;
block|}
end_block

begin_endif
endif|#
directive|endif
end_endif

begin_function
name|int
name|ipf_verifysrc
parameter_list|(
name|fin
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
block|{
name|struct
name|sockaddr_in
modifier|*
name|dst
decl_stmt|;
name|struct
name|route
name|iproute
decl_stmt|;
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
operator|&
name|iproute
argument_list|,
sizeof|sizeof
argument_list|(
name|iproute
argument_list|)
argument_list|)
expr_stmt|;
name|dst
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
operator|&
name|iproute
operator|.
name|ro_dst
expr_stmt|;
name|dst
operator|->
name|sin_len
operator|=
sizeof|sizeof
argument_list|(
operator|*
name|dst
argument_list|)
expr_stmt|;
name|dst
operator|->
name|sin_family
operator|=
name|AF_INET
expr_stmt|;
name|dst
operator|->
name|sin_addr
operator|=
name|fin
operator|->
name|fin_src
expr_stmt|;
name|rtalloc
argument_list|(
operator|&
name|iproute
argument_list|)
expr_stmt|;
if|if
condition|(
name|iproute
operator|.
name|ro_rt
operator|==
name|NULL
condition|)
return|return
literal|0
return|;
return|return
operator|(
name|fin
operator|->
name|fin_ifp
operator|==
name|iproute
operator|.
name|ro_rt
operator|->
name|rt_ifp
operator|)
return|;
block|}
end_function

begin_comment
comment|/*  * return the first IP Address associated with an interface  */
end_comment

begin_function
name|int
name|ipf_ifpaddr
parameter_list|(
name|softc
parameter_list|,
name|v
parameter_list|,
name|atype
parameter_list|,
name|ifptr
parameter_list|,
name|inp
parameter_list|,
name|inpmask
parameter_list|)
name|ipf_main_softc_t
modifier|*
name|softc
decl_stmt|;
name|int
name|v
decl_stmt|,
name|atype
decl_stmt|;
name|void
modifier|*
name|ifptr
decl_stmt|;
name|struct
name|in_addr
modifier|*
name|inp
decl_stmt|,
decl|*
name|inpmask
decl_stmt|;
end_function

begin_block
block|{
ifdef|#
directive|ifdef
name|USE_INET6
name|struct
name|in6_addr
modifier|*
name|inp6
init|=
name|NULL
decl_stmt|;
endif|#
directive|endif
name|struct
name|sockaddr
modifier|*
name|sock
decl_stmt|,
modifier|*
name|mask
decl_stmt|;
name|struct
name|sockaddr_in
modifier|*
name|sin
decl_stmt|;
name|struct
name|ifaddr
modifier|*
name|ifa
decl_stmt|;
name|struct
name|ifnet
modifier|*
name|ifp
decl_stmt|;
if|if
condition|(
operator|(
name|ifptr
operator|==
name|NULL
operator|)
operator|||
operator|(
name|ifptr
operator|==
operator|(
name|void
operator|*
operator|)
operator|-
literal|1
operator|)
condition|)
return|return
operator|-
literal|1
return|;
name|ifp
operator|=
name|ifptr
expr_stmt|;
name|mask
operator|=
name|NULL
expr_stmt|;
if|if
condition|(
name|v
operator|==
literal|4
condition|)
name|inp
operator|->
name|s_addr
operator|=
literal|0
expr_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
elseif|else
if|if
condition|(
name|v
operator|==
literal|6
condition|)
name|bzero
argument_list|(
operator|(
name|char
operator|*
operator|)
name|inp
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|in6_addr
argument_list|)
argument_list|)
expr_stmt|;
endif|#
directive|endif
name|ifa
operator|=
name|ifp
operator|->
name|if_addrlist
expr_stmt|;
name|sock
operator|=
name|ifa
operator|->
name|ifa_addr
expr_stmt|;
while|while
condition|(
name|sock
operator|!=
name|NULL
operator|&&
name|ifa
operator|!=
name|NULL
condition|)
block|{
name|sin
operator|=
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|sock
expr_stmt|;
if|if
condition|(
operator|(
name|v
operator|==
literal|4
operator|)
operator|&&
operator|(
name|sin
operator|->
name|sin_family
operator|==
name|AF_INET
operator|)
condition|)
break|break;
ifdef|#
directive|ifdef
name|USE_INET6
if|if
condition|(
operator|(
name|v
operator|==
literal|6
operator|)
operator|&&
operator|(
name|sin
operator|->
name|sin_family
operator|==
name|AF_INET6
operator|)
condition|)
block|{
name|inp6
operator|=
operator|&
operator|(
operator|(
expr|struct
name|sockaddr_in6
operator|*
operator|)
name|sin
operator|)
operator|->
name|sin6_addr
expr_stmt|;
if|if
condition|(
operator|!
name|IN6_IS_ADDR_LINKLOCAL
argument_list|(
name|inp6
argument_list|)
operator|&&
operator|!
name|IN6_IS_ADDR_LOOPBACK
argument_list|(
name|inp6
argument_list|)
condition|)
break|break;
block|}
endif|#
directive|endif
name|ifa
operator|=
name|ifa
operator|->
name|ifa_next
expr_stmt|;
if|if
condition|(
name|ifa
operator|!=
name|NULL
condition|)
name|sock
operator|=
name|ifa
operator|->
name|ifa_addr
expr_stmt|;
block|}
if|if
condition|(
name|ifa
operator|==
name|NULL
operator|||
name|sock
operator|==
name|NULL
condition|)
return|return
operator|-
literal|1
return|;
name|mask
operator|=
name|ifa
operator|->
name|ifa_netmask
expr_stmt|;
if|if
condition|(
name|atype
operator|==
name|FRI_BROADCAST
condition|)
name|sock
operator|=
name|ifa
operator|->
name|ifa_broadaddr
expr_stmt|;
elseif|else
if|if
condition|(
name|atype
operator|==
name|FRI_PEERADDR
condition|)
name|sock
operator|=
name|ifa
operator|->
name|ifa_dstaddr
expr_stmt|;
ifdef|#
directive|ifdef
name|USE_INET6
if|if
condition|(
name|v
operator|==
literal|6
condition|)
return|return
name|ipf_ifpfillv6addr
argument_list|(
name|atype
argument_list|,
operator|(
expr|struct
name|sockaddr_in6
operator|*
operator|)
name|sock
argument_list|,
operator|(
expr|struct
name|sockaddr_in6
operator|*
operator|)
name|mask
argument_list|,
name|inp
argument_list|,
name|inpmask
argument_list|)
return|;
endif|#
directive|endif
return|return
name|ipf_ifpfillv4addr
argument_list|(
name|atype
argument_list|,
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|sock
argument_list|,
operator|(
expr|struct
name|sockaddr_in
operator|*
operator|)
name|mask
argument_list|,
name|inp
argument_list|,
name|inpmask
argument_list|)
return|;
block|}
end_block

begin_function
name|u_32_t
name|ipf_newisn
parameter_list|(
name|fin
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
block|{
name|u_32_t
name|newiss
decl_stmt|;
if|#
directive|if
literal|0
block|static int iss_seq_off = 0; 	u_char hash[16]; 	MD5_CTX ctx;
comment|/* 	 * Compute the base value of the ISS.  It is a hash 	 * of (saddr, sport, daddr, dport, secret). 	 */
block|MD5Init(&ctx);  	MD5Update(&ctx, (u_char *)&fin->fin_fi.fi_src, 		  sizeof(fin->fin_fi.fi_src)); 	MD5Update(&ctx, (u_char *)&fin->fin_fi.fi_dst, 		  sizeof(fin->fin_fi.fi_dst)); 	MD5Update(&ctx, (u_char *)&fin->fin_dat, sizeof(fin->fin_dat));  	MD5Update(&ctx, ipf_iss_secret, sizeof(ipf_iss_secret));  	MD5Final(hash,&ctx);  	memcpy(&newiss, hash, sizeof(newiss));
comment|/* 	 * Now increment our "timer", and add it in to 	 * the computed value. 	 * 	 * XXX Use `addin'? 	 * XXX TCP_ISSINCR too large to use? 	 */
block|iss_seq_off += 0x00010000; 	newiss += iss_seq_off;
endif|#
directive|endif
return|return
name|newiss
return|;
block|}
end_function

begin_comment
comment|/* ------------------------------------------------------------------------ */
end_comment

begin_comment
comment|/* Function:    ipf_nextipid                                                */
end_comment

begin_comment
comment|/* Returns:     int - 0 == success, -1 == error (packet should be droppped) */
end_comment

begin_comment
comment|/* Parameters:  fin(I) - pointer to packet information                      */
end_comment

begin_comment
comment|/*                                                                          */
end_comment

begin_comment
comment|/* Returns the next IPv4 ID to use for this packet.                         */
end_comment

begin_comment
comment|/* ------------------------------------------------------------------------ */
end_comment

begin_function
name|u_short
name|ipf_nextipid
parameter_list|(
name|fin
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
block|{
name|u_short
name|id
decl_stmt|;
name|MUTEX_ENTER
argument_list|(
operator|&
name|ipf_rw
argument_list|)
expr_stmt|;
name|id
operator|=
name|ipid
operator|++
expr_stmt|;
name|MUTEX_EXIT
argument_list|(
operator|&
name|ipf_rw
argument_list|)
expr_stmt|;
return|return
name|id
return|;
block|}
end_function

begin_function
name|INLINE
name|int
name|ipf_checkv4sum
parameter_list|(
name|fin
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
block|{
ifdef|#
directive|ifdef
name|M_CSUM_TCP_UDP_BAD
name|int
name|manual
decl_stmt|,
name|pflag
decl_stmt|,
name|cflags
decl_stmt|,
name|active
decl_stmt|;
name|mb_t
modifier|*
name|m
decl_stmt|;
if|if
condition|(
operator|(
name|fin
operator|->
name|fin_flx
operator|&
name|FI_NOCKSUM
operator|)
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
if|if
condition|(
operator|(
name|fin
operator|->
name|fin_flx
operator|&
name|FI_SHORT
operator|)
operator|!=
literal|0
condition|)
return|return
literal|1
return|;
if|if
condition|(
name|fin
operator|->
name|fin_cksum
operator|!=
name|FI_CK_NEEDED
condition|)
return|return
operator|(
name|fin
operator|->
name|fin_cksum
operator|>
name|FI_CK_NEEDED
operator|)
condition|?
literal|0
else|:
operator|-
literal|1
return|;
name|manual
operator|=
literal|0
expr_stmt|;
name|m
operator|=
name|fin
operator|->
name|fin_m
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
block|{
name|manual
operator|=
literal|1
expr_stmt|;
goto|goto
name|skipauto
goto|;
block|}
switch|switch
condition|(
name|fin
operator|->
name|fin_p
condition|)
block|{
case|case
name|IPPROTO_UDP
case|:
name|pflag
operator|=
name|M_CSUM_UDPv4
expr_stmt|;
break|break;
case|case
name|IPPROTO_TCP
case|:
name|pflag
operator|=
name|M_CSUM_TCPv4
expr_stmt|;
break|break;
default|default :
name|pflag
operator|=
literal|0
expr_stmt|;
name|manual
operator|=
literal|1
expr_stmt|;
break|break;
block|}
name|active
operator|=
operator|(
operator|(
expr|struct
name|ifnet
operator|*
operator|)
name|fin
operator|->
name|fin_ifp
operator|)
operator|->
name|if_csum_flags_rx
operator|&
name|pflag
expr_stmt|;
name|active
operator||=
name|M_CSUM_TCP_UDP_BAD
operator||
name|M_CSUM_DATA
expr_stmt|;
name|cflags
operator|=
name|m
operator|->
name|m_pkthdr
operator|.
name|csum_flags
operator|&
name|active
expr_stmt|;
if|if
condition|(
name|pflag
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|cflags
operator|==
operator|(
name|pflag
operator||
name|M_CSUM_TCP_UDP_BAD
operator|)
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_BAD
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|cflags
operator|==
operator|(
name|pflag
operator||
name|M_CSUM_DATA
operator|)
condition|)
block|{
if|if
condition|(
operator|(
name|m
operator|->
name|m_pkthdr
operator|.
name|csum_data
operator|^
literal|0xffff
operator|)
operator|!=
literal|0
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_BAD
expr_stmt|;
block|}
else|else
block|{
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_SUMOK
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|cflags
operator|==
name|pflag
condition|)
block|{
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_SUMOK
expr_stmt|;
block|}
else|else
block|{
name|manual
operator|=
literal|1
expr_stmt|;
block|}
block|}
name|skipauto
label|:
if|if
condition|(
name|manual
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|ipf_checkl4sum
argument_list|(
name|fin
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
block|}
else|#
directive|else
if|if
condition|(
name|ipf_checkl4sum
argument_list|(
name|fin
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
endif|#
directive|endif
return|return
literal|0
return|;
block|}
end_function

begin_ifdef
ifdef|#
directive|ifdef
name|USE_INET6
end_ifdef

begin_function
name|INLINE
name|int
name|ipf_checkv6sum
parameter_list|(
name|fin
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
block|{
ifdef|#
directive|ifdef
name|M_CSUM_TCP_UDP_BAD
name|int
name|manual
decl_stmt|,
name|pflag
decl_stmt|,
name|cflags
decl_stmt|,
name|active
decl_stmt|;
name|mb_t
modifier|*
name|m
decl_stmt|;
if|if
condition|(
operator|(
name|fin
operator|->
name|fin_flx
operator|&
name|FI_NOCKSUM
operator|)
operator|!=
literal|0
condition|)
return|return
literal|0
return|;
if|if
condition|(
operator|(
name|fin
operator|->
name|fin_flx
operator|&
name|FI_SHORT
operator|)
operator|!=
literal|0
condition|)
return|return
literal|1
return|;
if|if
condition|(
name|fin
operator|->
name|fin_cksum
operator|!=
name|FI_CK_NEEDED
condition|)
return|return
operator|(
name|fin
operator|->
name|fin_cksum
operator|>
name|FI_CK_NEEDED
operator|)
condition|?
literal|0
else|:
operator|-
literal|1
return|;
name|manual
operator|=
literal|0
expr_stmt|;
name|m
operator|=
name|fin
operator|->
name|fin_m
expr_stmt|;
switch|switch
condition|(
name|fin
operator|->
name|fin_p
condition|)
block|{
case|case
name|IPPROTO_UDP
case|:
name|pflag
operator|=
name|M_CSUM_UDPv6
expr_stmt|;
break|break;
case|case
name|IPPROTO_TCP
case|:
name|pflag
operator|=
name|M_CSUM_TCPv6
expr_stmt|;
break|break;
default|default :
name|pflag
operator|=
literal|0
expr_stmt|;
name|manual
operator|=
literal|1
expr_stmt|;
break|break;
block|}
name|active
operator|=
operator|(
operator|(
expr|struct
name|ifnet
operator|*
operator|)
name|fin
operator|->
name|fin_ifp
operator|)
operator|->
name|if_csum_flags_rx
operator|&
name|pflag
expr_stmt|;
name|active
operator||=
name|M_CSUM_TCP_UDP_BAD
operator||
name|M_CSUM_DATA
expr_stmt|;
name|cflags
operator|=
name|m
operator|->
name|m_pkthdr
operator|.
name|csum_flags
operator|&
name|active
expr_stmt|;
if|if
condition|(
name|pflag
operator|!=
literal|0
condition|)
block|{
if|if
condition|(
name|cflags
operator|==
operator|(
name|pflag
operator||
name|M_CSUM_TCP_UDP_BAD
operator|)
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_BAD
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|cflags
operator|==
operator|(
name|pflag
operator||
name|M_CSUM_DATA
operator|)
condition|)
block|{
if|if
condition|(
operator|(
name|m
operator|->
name|m_pkthdr
operator|.
name|csum_data
operator|^
literal|0xffff
operator|)
operator|!=
literal|0
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_BAD
expr_stmt|;
block|}
else|else
block|{
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_SUMOK
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
name|cflags
operator|==
name|pflag
condition|)
block|{
name|fin
operator|->
name|fin_cksum
operator|=
name|FI_CK_SUMOK
expr_stmt|;
block|}
else|else
block|{
name|manual
operator|=
literal|1
expr_stmt|;
block|}
block|}
if|if
condition|(
name|manual
operator|!=
literal|0
condition|)
if|if
condition|(
name|ipf_checkl4sum
argument_list|(
name|fin
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
else|#
directive|else
if|if
condition|(
name|ipf_checkl4sum
argument_list|(
name|fin
argument_list|)
operator|==
operator|-
literal|1
condition|)
block|{
name|fin
operator|->
name|fin_flx
operator||=
name|FI_BAD
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
endif|#
directive|endif
return|return
literal|0
return|;
block|}
end_function

begin_endif
endif|#
directive|endif
end_endif

begin_comment
comment|/* USE_INET6 */
end_comment

begin_function
name|size_t
name|mbufchainlen
parameter_list|(
name|m0
parameter_list|)
name|struct
name|mbuf
modifier|*
name|m0
decl_stmt|;
block|{
name|size_t
name|len
decl_stmt|;
if|if
condition|(
operator|(
name|m0
operator|->
name|m_flags
operator|&
name|M_PKTHDR
operator|)
operator|!=
literal|0
condition|)
block|{
name|len
operator|=
name|m0
operator|->
name|m_pkthdr
operator|.
name|len
expr_stmt|;
block|}
else|else
block|{
name|struct
name|mbuf
modifier|*
name|m
decl_stmt|;
for|for
control|(
name|m
operator|=
name|m0
operator|,
name|len
operator|=
literal|0
init|;
name|m
operator|!=
name|NULL
condition|;
name|m
operator|=
name|m
operator|->
name|m_next
control|)
name|len
operator|+=
name|m
operator|->
name|m_len
expr_stmt|;
block|}
return|return
name|len
return|;
block|}
end_function

begin_comment
comment|/* ------------------------------------------------------------------------ */
end_comment

begin_comment
comment|/* Function:    ipf_pullup                                                  */
end_comment

begin_comment
comment|/* Returns:     NULL == pullup failed, else pointer to protocol header      */
end_comment

begin_comment
comment|/* Parameters:  xmin(I)- pointer to buffer where data packet starts         */
end_comment

begin_comment
comment|/*              fin(I) - pointer to packet information                      */
end_comment

begin_comment
comment|/*              len(I) - number of bytes to pullup                          */
end_comment

begin_comment
comment|/*                                                                          */
end_comment

begin_comment
comment|/* Attempt to move at least len bytes (from the start of the buffer) into a */
end_comment

begin_comment
comment|/* single buffer for ease of access.  Operating system native functions are */
end_comment

begin_comment
comment|/* used to manage buffers - if necessary.  If the entire packet ends up in  */
end_comment

begin_comment
comment|/* a single buffer, set the FI_COALESCE flag even though ipf_coalesce() has */
end_comment

begin_comment
comment|/* not been called.  Both fin_ip and fin_dp are updated before exiting _IF_ */
end_comment

begin_comment
comment|/* and ONLY if the pullup succeeds.                                         */
end_comment

begin_comment
comment|/*                                                                          */
end_comment

begin_comment
comment|/* We assume that 'xmin' is a pointer to a buffer that is part of the chain */
end_comment

begin_comment
comment|/* of buffers that starts at *fin->fin_mp.                                  */
end_comment

begin_comment
comment|/* ------------------------------------------------------------------------ */
end_comment

begin_function
name|void
modifier|*
name|ipf_pullup
parameter_list|(
name|xmin
parameter_list|,
name|fin
parameter_list|,
name|len
parameter_list|)
name|mb_t
modifier|*
name|xmin
decl_stmt|;
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
name|int
name|len
decl_stmt|;
block|{
name|int
name|out
init|=
name|fin
operator|->
name|fin_out
decl_stmt|,
name|dpoff
decl_stmt|,
name|ipoff
decl_stmt|;
name|mb_t
modifier|*
name|m
init|=
name|xmin
decl_stmt|;
name|char
modifier|*
name|ip
decl_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
return|return
name|NULL
return|;
name|ip
operator|=
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_ip
expr_stmt|;
if|if
condition|(
operator|(
name|fin
operator|->
name|fin_flx
operator|&
name|FI_COALESCE
operator|)
operator|!=
literal|0
condition|)
return|return
name|ip
return|;
name|ipoff
operator|=
name|fin
operator|->
name|fin_ipoff
expr_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_dp
operator|!=
name|NULL
condition|)
name|dpoff
operator|=
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_dp
operator|-
operator|(
name|char
operator|*
operator|)
name|ip
expr_stmt|;
else|else
name|dpoff
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|M_LEN
argument_list|(
name|m
argument_list|)
operator|<
name|len
condition|)
block|{
name|mb_t
modifier|*
name|n
init|=
operator|*
name|fin
operator|->
name|fin_mp
decl_stmt|;
comment|/* 		 * Assume that M_PKTHDR is set and just work with what is left 		 * rather than check.. 		 * Should not make any real difference, anyway. 		 */
if|if
condition|(
name|m
operator|!=
name|n
condition|)
block|{
comment|/* 			 * Record the mbuf that points to the mbuf that we're 			 * about to go to work on so that we can update the 			 * m_next appropriately later. 			 */
for|for
control|(
init|;
name|n
operator|->
name|m_next
operator|!=
name|m
condition|;
name|n
operator|=
name|n
operator|->
name|m_next
control|)
empty_stmt|;
block|}
else|else
block|{
name|n
operator|=
name|NULL
expr_stmt|;
block|}
ifdef|#
directive|ifdef
name|MHLEN
if|if
condition|(
name|len
operator|>
name|MHLEN
condition|)
else|#
directive|else
if|if
condition|(
name|len
operator|>
name|MLEN
condition|)
endif|#
directive|endif
block|{
name|FREE_MB_T
argument_list|(
operator|*
name|fin
operator|->
name|fin_mp
argument_list|)
expr_stmt|;
name|m
operator|=
name|NULL
expr_stmt|;
block|}
else|else
block|{
name|m
operator|=
name|m_pullup
argument_list|(
name|m
argument_list|,
name|len
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|n
operator|!=
name|NULL
condition|)
name|n
operator|->
name|m_next
operator|=
name|m
expr_stmt|;
if|if
condition|(
name|m
operator|==
name|NULL
condition|)
block|{
comment|/* 			 * When n is non-NULL, it indicates that m pointed to 			 * a sub-chain (tail) of the mbuf and that the head 			 * of this chain has not yet been free'd. 			 */
if|if
condition|(
name|n
operator|!=
name|NULL
condition|)
block|{
name|FREE_MB_T
argument_list|(
operator|*
name|fin
operator|->
name|fin_mp
argument_list|)
expr_stmt|;
block|}
operator|*
name|fin
operator|->
name|fin_mp
operator|=
name|NULL
expr_stmt|;
name|fin
operator|->
name|fin_m
operator|=
name|NULL
expr_stmt|;
return|return
name|NULL
return|;
block|}
if|if
condition|(
name|n
operator|==
name|NULL
condition|)
operator|*
name|fin
operator|->
name|fin_mp
operator|=
name|m
expr_stmt|;
while|while
condition|(
name|M_LEN
argument_list|(
name|m
argument_list|)
operator|==
literal|0
condition|)
block|{
name|m
operator|=
name|m
operator|->
name|m_next
expr_stmt|;
block|}
name|fin
operator|->
name|fin_m
operator|=
name|m
expr_stmt|;
name|ip
operator|=
name|MTOD
argument_list|(
name|m
argument_list|,
name|char
operator|*
argument_list|)
operator|+
name|ipoff
expr_stmt|;
name|fin
operator|->
name|fin_ip
operator|=
operator|(
name|ip_t
operator|*
operator|)
name|ip
expr_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_dp
operator|!=
name|NULL
condition|)
name|fin
operator|->
name|fin_dp
operator|=
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_ip
operator|+
name|dpoff
expr_stmt|;
if|if
condition|(
name|fin
operator|->
name|fin_fraghdr
operator|!=
name|NULL
condition|)
name|fin
operator|->
name|fin_fraghdr
operator|=
operator|(
name|char
operator|*
operator|)
name|ip
operator|+
operator|(
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_fraghdr
operator|-
operator|(
name|char
operator|*
operator|)
name|fin
operator|->
name|fin_ip
operator|)
expr_stmt|;
block|}
if|if
condition|(
name|len
operator|==
name|fin
operator|->
name|fin_plen
condition|)
name|fin
operator|->
name|fin_flx
operator||=
name|FI_COALESCE
expr_stmt|;
return|return
name|ip
return|;
block|}
end_function

begin_function
name|void
modifier|*
name|getifp
parameter_list|(
name|name
parameter_list|,
name|v
parameter_list|)
name|char
modifier|*
name|name
decl_stmt|;
name|int
name|v
decl_stmt|;
block|{
return|return
name|NULL
return|;
block|}
end_function

begin_function
name|int
name|ipf_inject
parameter_list|(
name|fin
parameter_list|,
name|m
parameter_list|)
name|fr_info_t
modifier|*
name|fin
decl_stmt|;
name|mb_t
modifier|*
name|m
decl_stmt|;
block|{
name|FREE_MB_T
argument_list|(
name|m
argument_list|)
expr_stmt|;
name|fin
operator|->
name|fin_m
operator|=
name|NULL
expr_stmt|;
name|fin
operator|->
name|fin_ip
operator|=
name|NULL
expr_stmt|;
return|return
name|EINVAL
return|;
block|}
end_function

begin_comment
comment|/*  * In the face of no kernel random function, this is implemented...it is  * not meant to be random, just a fill in.  */
end_comment

begin_function
name|int
name|ipf_random
parameter_list|()
block|{
specifier|static
name|int
name|last
init|=
literal|0
decl_stmt|;
specifier|static
name|int
name|calls
init|=
literal|0
decl_stmt|;
name|struct
name|timeval
name|tv
decl_stmt|;
name|int
name|number
decl_stmt|;
name|GETKTIME
argument_list|(
operator|&
name|tv
argument_list|)
expr_stmt|;
name|last
operator|*=
name|tv
operator|.
name|tv_usec
operator|+
name|calls
operator|++
expr_stmt|;
name|last
operator|+=
operator|(
name|int
operator|)
operator|&
name|range
operator|*
name|ipf_ticks
expr_stmt|;
name|number
operator|=
name|last
operator|+
name|tv
operator|.
name|tv_sec
expr_stmt|;
return|return
name|number
return|;
block|}
end_function

end_unit

